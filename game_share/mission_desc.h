...
enum TMissionType { Solo = 0, Group, Guild };
...
enum TDifficulty { Easy = 0, Medium, Hard, NbDifficulties };

enum TLength { Short = 0, Normal, Long, NbLength };
...
/** This enum describe the visual on client. This is what filled in ICON
 *Database field If you change this enum, you MUST also change <options
 *type="mission_icons" name="mission_icons"> in client config.xml. You must also
 *change the IconToClientMissionType below
 */
enum TIconId {
  IconNone = 0,
...
const TClientMissionType IconToClientMissionType[] = {
    ...
    Mission,   Mission,       Mission,
    Mission,   Mission,       Mission,
    ZCCharge,  RMBuy,         RMBuy,
    RMBuy,     RMUpgrade,     RMUpgrade,
    RMUpgrade, ZCRequirement, BuildingRequirement,
    Building};

// The pre-requesit state of a mission (the client know why he can't do a
// mission)
enum TPreReqState {
  PreReqSuccess = 0,

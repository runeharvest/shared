...
NLMISC::CVariable<uint32> NbProcessedEventsInTimerManagerUpdate(
    "egs", "NbProcessedEventInTimerManagerUpdate", "", 0);
NLMISC::CVariable<uint32> NbEventsToProcessInTimerManagerUpdate(
    "egs", "NbEventsToProcessInTimerManagerUpdate", "", 0);
...
void CTimerManager::syncTick() {
  ...
  // re-locate the event vectors to line them back up with the time values that
  // they represent
  for (NLMISC::TGameCycle i = 0; i < 256; ++i) {
...
void CTimerManager::tickUpdate() {
  ...
  for (uint32 i = 0; i < size; ++i) {
    ...
    // BUG when event time is too high, like 0xffffffff which is used in special
    // cases, so change the test if ((sint32)(eventPtr->getTime()-time)>0)
  ...
  if (!vect.empty()) {
    // nlinfo("TimerManagerUpdate: Processed %d of %d
    // events",vect.size()-nextFreeSlot, vect.size());
    NbEventsToProcessInTimerManagerUpdate = (uint32)vect.size();
    NbProcessedEventsInTimerManagerUpdate =
        NbEventsToProcessInTimerManagerUpdate.get() - nextFreeSlot;
  }
  // resize the vector back down to keep only the events that we haven't dealt
  // with yet

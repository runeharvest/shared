 ...
 * Define this macro to enable the storing and checking of which service changes
 * the value of a property (in addition to storing the timestamp of the change).
 */
#define STORE_CHANGE_SERVICEIDS

/*
 * CHECK_DATASETROW_VALIDITY
 * Define this macro to enable checking if a datasetrow is still valid every
 * time we access the mirror with it. A datasetrow becomes invalid when the
 * corresponding entity is removed.
 */
 ...
 * These two macros are equivalent to nlinfo & nldebug respectively except that
 * they allow logging to be dissabled via the variable VerboseMIRROR
 */
...
///return _Index&index; } /	bool	operator	!=(const
///TDataSetIndex	&other)	const	{	return _Index!=other._Index; }
////	bool	operator	==(const	TDataSetIndex	&other)	const
///{	return _Index==other._Index; } /	uint32	operator
///>>(uint32 decal)		const	{	return _Index>>decal; } /
///bool	operator	<(TDataSetIndex other)	const	{	return
///_Index<other._Index; }
//
//	uint32	operator	++()	{	_Index++;	return _Index; }
//	uint32	operator	--()	{	_Index--;	return _Index; }
//	uint32	operator	+=(uint32 index)	{	_Index+=index;
//return _Index; } 	uint32	operator	-=(uint32 index)	{
//_Index+=index;	return _Index; } 	uint32	operator	|=(uint32 index)
...
public:
  ...
  /// Explicit index
  TDataSetIndex getIndex() const {
    return _EntityIndexAndRemCounter & 0xFFFFFF;
  }

  /**
   * Get a compressed 20-bit index.
   * - Assumes the max size of the corresponding dataset is lower than 2^20
   * (about 1 million rows)
   * - Does not include the assignment counter of the dataset row (use with
   * caution)
   * - An invalid dataset row returns the compressed value 0xFFFFF.
   * - To uncompress this value to a dataset row, use
   * CMirrorDataSet::getCurrentDataSetRow(). Check the validity of the returned
   * dataset row (!isValid()), as it will be invalid if the index is not used
   * anymore (but you will not be able to know if it has be reassigned, so don't
   * use this scheme if you don't check the uncompressed index before the min
   * reassignment time.
   */
  ...
  //	!!! be careful, I removed operator= because TDataSetIndex is an int and
  //there is permissious side effects with the use of CEntityId.
  ...
  bool operator<(const TDataSetRow &other) const {
    return _EntityIndexAndRemCounter < other._EntityIndexAndRemCounter;
  }
  bool operator!=(const TDataSetRow &other) const {
    return _EntityIndexAndRemCounter != other._EntityIndexAndRemCounter;
  }
  ...
  bool isValid() const { return (getIndex() != INVALID_DATASET_INDEX); }

  /// Return true if the datasetrow is not ininitialized
  bool isNull() const { return (getIndex() == INVALID_DATASET_INDEX); }

  /// Serial (index and removal counter)
  void serial(NLMISC::IStream &s) { s.serial(_EntityIndexAndRemCounter); }

  /// Return a string representing the datasetrow
  std::string toString() const;

  /// Set to LAST_CHANGED
  void initToLastChanged() { _EntityIndexAndRemCounter = LAST_CHANGED; }
  ...
  public:
    ...
    size_t operator()(const TDataSetRow &index) const {
      return index.getHashCode();
    }

    bool operator()(const TDataSetRow &index1,
                    const TDataSetRow &index2) const {
      return index1 < index2;
    }
  };

  /// Warning: method to avoid (use it only when using rows as a static array)
  static TDataSetRow createFromRawIndex(TDataSetIndex entityIndex) {
    return TDataSetRow(entityIndex);
  }
...
private:
  ...
  explicit TDataSetRow(TDataSetIndex entityIndex, uint8 remCounter) {
    //		_EntityIndexAndRemCounter = ((uint32)entityIndex) |
    //((uint32)remCounter << 24);
    _EntityIndexAndRemCounter = entityIndex;
  ...
  /// Get Hash code
  uint32 getHashCode() const { return NLMISC::wangHash((uint32)(getIndex())); }
 ...
 * Constants
 */
const TDataSetIndex INVALID_DATASET_ROW(
    (uint32)0x00FFFFFF); // leaving blank the most significant byte (see removal
                         // counter when CHECK_DATASETROW_VALIDITY is enabled)
...
const TSharedListRow INVALID_SHAREDLIST_ROW =
    std::numeric_limits<uint16>::max(); // std::numeric_limits<uint16>::max()-1;
const uint NB_SHAREDLIST_CELLS =
    500000; // property+list container footprint with data size of 32 bit and
            // 500000 rows: 5.8 MB
...
const TSharedListRow INVALID_SHAREDLIST_ROW =
    std::numeric_limits<uint16>::max() - 1;
const uint NB_SHAREDLIST_CELLS =
    INVALID_SHAREDLIST_ROW; // property+list container footprint with data size
                            // of 32 bit and 500000 rows: 1.3 MB

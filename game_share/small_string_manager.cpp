...
uint32 CSmallStringManager::registerString(const std::string &phrase,
                                           uint32 nb) {
  ...
  if (found == _StringToId.end()) {
    ...
    _IdToString.insert(std::pair<uint32, std::pair<std::string, uint32>>(
        id, std::pair<std::string, uint32>(phrase, nb)));
  } else // already registered text, increment counter
  {
    id = found->second;
    std::map<uint32, std::pair<std::string, uint32>>::iterator found2(
        _IdToString.find(found->second));
...
void CSmallStringManager::unregisterString(const std::string &phrase,
                                           uint32 nb) {
  std::map<std::string, uint32>::iterator found(_StringToId.find(phrase));
...
void CSmallStringManager::unregisterStringById(uint32 id, uint32 nb) {
  ...
  count -= nb;
  if (count == 0) {
    std::map<std::string, uint32>::iterator found =
        _StringToId.find(found2->second.first);
    if (found != _StringToId.end()) {
      _StringToId.erase(found);
    }
...
const std::string &CSmallStringManager::getString(uint32 id) const {
  ...
  std::map<uint32, std::pair<std::string, uint32>>::const_iterator found(
      _IdToString.find(id));
  if (found == _IdToString.end()) {
    nlwarning("Try to access un accessible string");
    return empty;
  }
  return found->second.first;
}

CSmallStringManager::CSmallStringManager() { _MaxId = 0; }
...
CSmallStringManager::CSmallStringManager(CObject *textManager) {
  ...
  uint32 maxId =
      static_cast<uint32>(textManager->getAttr("MaxId")->toInteger());
  ...
  for (uint32 i = 0; i < max; i++) {
    ...
    uint32 textCount =
        static_cast<uint32>(entry->getAttr("Count")->toInteger());
    _StringToId.insert(std::pair<std::string, uint32>(text, textId));
    _IdToString.insert(std::pair<uint32, std::pair<std::string, uint32>>(
        textId, std::pair<std::string, uint32>(text, textCount)));
  }
}
...
void CSmallStringManager::serial(NLMISC::IStream &stream) {
  ...
  std::set<uint32>::const_iterator first(_FreeIds.begin()),
      last(_FreeIds.end());
  ...
  std::map<uint32, std::pair<std::string, uint32>>::const_iterator first2(
      _IdToString.begin()),
      last2(_IdToString.end());
...
CSmallStringManager::~CSmallStringManager() { release(); }
...
void CStringTableManager::setValue(uint32 tableId, const std::string &localId,
                                   const std::string &value) {
  TLocalTable *localTable = getLocalTable(tableId);
  if (localTable == NULL) {
    nlwarning("unknown scenario Id or no string table for this scenario! : %d",
              tableId);
    return;
  }
  ...
  if (found2 != localTable->end()) {
    ...
    if (oldValue.empty())
      nlwarning("error! no string %u in string manager! ", found2->second);
...
std::string CStringTableManager::getValue(uint32 tableId,
...
CStringTableManager::TLocalTable *
CStringTableManager::getLocalTable(uint32 tableId) const {
...
CStringTableManager::~CStringTableManager() { release(); }

...
CDynChatSession::CDynChatSession(CDynChatClient *client, CDynChatChan *channel)
    : StringID(0), WriteRight(false), _Client(client), _Channel(channel) {
  nlassert(client);
  nlassert(channel);
  // insert at head of client linked list of session
  _NextClientSession = client->_FirstSession;
  if (client->_FirstSession) {
    nlassert(client->_FirstSession->_PrevClientSession ==
             &client->_FirstSession);
  ...
  if (channel->_FirstSession) {
    nlassert(channel->_FirstSession->_PrevChannelSession ==
             &channel->_FirstSession);
...
CDynChatSession *CDynChatClient::getSession(TChanID chan) const {
  CDynChatSession *currSession = _FirstSession;
  while (currSession) {
    if (currSession->getChan()->getID() == chan)
      break;
    currSession = currSession->getNextClientSession();
  }
  return currSession;
...
CDynChatChan::CDynChatChan()
    : HistoricSize(0), HideBubble(false), UniversalChannel(false),
      _FirstSession(NULL), _ID(CEntityId::Unknown),
      _DontBroadcastPlayerInputs(false),
      _ForwardPlayerIntputToOwnerService(false), _UnifyChannel(false) {}

//================================================================
// CDynChatChan::CDynChatChan(TChanID id) : _ID(id), _FirstSession(NULL),
// HistoricSize(0)
CDynChatChan::CDynChatChan(TChanID id, bool noBroadcast, bool forwardInput,
                           bool unified)
    : HistoricSize(0), HideBubble(false), UniversalChannel(false),
      _FirstSession(NULL), _ID(id), _DontBroadcastPlayerInputs(noBroadcast),
      _ForwardPlayerIntputToOwnerService(forwardInput), _UnifyChannel(unified) {
}
...
//================================================================
bool CDynChat::addChan(TChanID chan, bool noBroadcast, bool forwardInput,
                       bool unify) {
  if (_Chans.count(chan))
    return false;
...
//================================================================
CDynChatSession *CDynChat::addSession(TChanID chanID,
                                      const TDataSetRow &clientID) {
  ...
  if (!client)
    return NULL;
  // look for channel in session (faster because there are few channels used by
  // a single player)
  CDynChatSession *session = client->getSession(chanID);
  if (session) {
    nlwarning("Session already created for player %s in channel %p",
              clientID.toString().c_str(), chan);
    return NULL;
  }

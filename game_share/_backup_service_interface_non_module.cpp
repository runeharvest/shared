...
CVariable<string>
    BackupServiceIP("variables", "BSHost",
                    "Host address and port of backup service (ip:port)",
                    "localhost", 0, true);
CVariable<uint16> BackupServiceL3Port("variables", "L3BSPort",
                                      "Port for the layer 3 backup connection",
                                      0, 0, true);
CVariable<string> SlaveBackupServiceIP(
    "variables", "SlaveBSHost",
    "Host address and port of slave backup service (ip:port)", "", 0, true);
CVariable<uint16>
    SlaveBackupServiceL3Port("variables", "L3SlaveBSPort",
                             "Port for the layer 3 slave backup connection", 0,
                             0, true);
// CVariable<string>	PDBackupServiceIP("variables","PDBSHost", "Host address
// and port of backup service (ip:port)", "", 0, true ); CVariable<string>
// SlavePDBackupServiceIP("variables","SlavePDBSHost", "Host address and port of
// slave backup service (ip:port)", "", 0, true );
CVariable<bool> UseBS("variables", "UseBS",
                      "if 1, use the backup service or use local save", true, 0,
                      true);

// a variable written by the BS interface for debugging purposes
CVariable<string> BSPingHistory("BSIF", "BSPingHistory",
                                "The recent BS ping time history", "", 0, true);
...
void CGenericRequestIdRegister::pushRequestId(uint32 requestId,
                                              const std::string &fileName) {
  // check for out of order entries in the request list
  BOMB_IF(!_RequestIds.empty() && (requestId < _RequestIds.back().RequestId),
          "Ignoring out of order request id in generic callback registration "
          "for file " +
              CSString(fileName).quote(),
          return);
...
void CGenericRequestIdRegister::processCallbacks(uint32 lastRequestId) {
  while (!_RequestIds.empty() &&
         sint32(lastRequestId - _RequestIds.front().RequestId) >= 0) {
    ...
    CBackupInterfaceSingleton *singleton =
        CBackupInterfaceSingleton::getInstance();
    CBackupServiceInterface *itf;
    NLMISC::CSmartPtr<IBackupGenericAckCallback> cb =
        singleton->popGenericCallback(theRequestId, itf);
    if (cb != NULL) {
      // call the calback
...
// count of the number of backup services that are up (map indexed by backup
// service name)
...
struct SPingPongStruct {
  ...
  SPingPongStruct() : Count(0), ReadyToPing(true), LastGenericRequestId(0) {
    Times.resize(20, 0);
    LastAckDelay = 0;
    LastSendTime = 0;
  }
...
static void cbBSConnection(const std::string &serviceName,
                           NLNET::TServiceId serviceId, void * /* arg */) {
  nlinfo("Backup service connects: (name %s, Id %hu)", serviceName.c_str(),
         serviceId.get());

  // instantiate the PingPong record for the service that just connected and
  // dispatch a first PING message
  PingPongs[serviceId];

  // increment the counter of active backup services
  ++BackupServiceUp[serviceName];
  if (BackupServiceUp[serviceName] == 1 &&
      CBackupInterfaceSingleton::getInstance()->getBSIImplementation() ==
          &CBSIINonModule::getInstance()) {
    CBackupInterfaceSingleton::getInstance()->connect();
  }
}

static void cbBSDisconnection(const std::string &serviceName,
                              NLNET::TServiceId serviceId, void * /* arg */) {
  nlinfo("Backup service disconnects: (name %s, Id %hu)", serviceName.c_str(),
         serviceId.get());
  PingPongs.erase(serviceId);

  // decrement the counter of active backup services
  --BackupServiceUp[serviceName];
  if (BackupServiceUp[serviceName] == 0 &&
      CBackupInterfaceSingleton::getInstance()->getBSIImplementation() ==
          &CBSIINonModule::getInstance()) {
...
static void cbFile(NLNET::CMessage &msgin,
  ...
  NLMISC::CSmartPtr<IBackupFileReceiveCallback> cb =
      singleton->popFileCallback(msg.RequestId, itf);
  BOMB_IF(cb == NULL,
          "Received a file from backup service - but can't find a matching "
          "request: " +
              msg.FileDescription.FileName +
              NLMISC::toString(" (RequestId=%d)", msg.RequestId),
          return);

  // make sure the file size reported by the FileDescription corresponds to the
  // quantity of data remaining in the message
  BOMB_IF(msg.FileDescription.FileSize != msg.Data.length() - msg.Data.getPos(),
          "Throwing out file because message header is corrupt or message is "
          "incomplete: " +
              msg.FileDescription.FileName,
          return);

  // Restore the original filename (without the remote path). Assumes there
  // can't be more than one BS interface. This will fail if the remote path has
  // changed between the request and now.
...
void CBSIINonModule::cbBsReadMode(CMessage &msgin, TSockId /* from */,
...
static void cbSyncFile(CMessage &msgin, TSockId /* from */,
                       CCallbackNetBase & /* netbase */) {
  // just recall the layer 5 callback with fake service info
  cbFile(msgin, "", TServiceId(0));
}

static void cbFileClass(NLNET::CMessage &msgin,
                        const std::string & /* serviceName */,
                        NLNET::TServiceId /* serviceId */) {
  // decorticate the input message
  ...
  NLMISC::CSmartPtr<IBackupFileClassReceiveCallback> cb =
      singleton->popFileClassCallback(msg.RequestId, itf);
  BOMB_IF(cb == NULL,
          "Received a file class from backup service - but can't find a "
          "matching request!" +
              NLMISC::toString(" RequestId=%d", msg.RequestId),
          return);

  // Restore the original filenames (without the remote path). Assumes there
  // can't be more than one BS interface. This will fail if the remote path has
  // changed between the request and now.
...
static void cbSyncFileClass(CMessage &msgin, TSockId /* from */,
                            CCallbackNetBase & /* netbase */) {
  // just recall the layer 5 callback with fake service info
  cbFileClass(msgin, "", TServiceId(0));
}

void cbAppend(NLNET::CMessage &msgin, const std::string & /* serviceName */,
              NLNET::TServiceId /* serviceId */) {
  CBackupMsgAppendCallback inMsg;
  msgin.serial(inMsg);
}

void cbBSPingPong(NLNET::CMessage & /* msgin */,
                  const std::string & /* serviceName */,
                  NLNET::TServiceId serviceId) {
  ...
  // process the generic callbacks that are implicitly acked with this pong
  // message
  CGenericRequestIdRegister::getInstance().processCallbacks(
      thePingPong.LastGenericRequestId);

  // store away the last request id for the next time round the loop
  thePingPong.LastGenericRequestId =
      CGenericRequestIdRegister::getInstance().getLastRequestId();
}
...
// Callback array for layer 3 comm (for synchronous file loading and initial
// handshake)
...
void CBSIINonModule::serviceUpdate() {
  ...
  for (TPingPongs::iterator it = PingPongs.begin(); it != PingPongs.end();
       ++it) {
  ...
  for (TPingPongs::iterator it = PingPongs.begin(); it != PingPongs.end();
       ++it) {
    displayStr += NLMISC::toString("%sBS(%hu)[", displayStr.empty() ? "" : "; ",
                                   it->first.get());
    ...
    for (uint32 i = numTimes; --i;) {
      ...
      displayStr +=
          NLMISC::toString("%s%u", (displayStr.right(1) == "[") ? "" : ",",
                           (uint32)(nextTime - prevTime));
    }
...
void CBSIINonModule::activate() {
  ...
  if (_Initialised) {
    ...
    if (BackupServiceUp["BS"] !=
        0 /* && (!_HaveSeparatePDBS || BackupServiceUp["PDBS"]!=0)*/) {
      CBackupInterfaceSingleton::getInstance()->connect();
    }
    return;
  }
  ...
  CUnifiedNetwork::getInstance()->setServiceUpCallback("BS", cbBSConnection,
                                                       NULL);
  CUnifiedNetwork::getInstance()->setServiceDownCallback(
      "BS", cbBSDisconnection, NULL);

  //	CUnifiedNetwork::getInstance()->setServiceUpCallback ("PDBS",
  //cbBSConnection, NULL);
  //	CUnifiedNetwork::getInstance()->setServiceDownCallback ("PDBS",
  //cbBSDisconnection, NULL);
  ...
  //		if (host.find (":") == string::npos)
  //			host+= ":43990";
  //		CUnifiedNetwork::getInstance()->addService ("PDBS",
  //CInetHost(host));
  //
...
void CBSIINonModule::dispatchRequestFile(const std::string &bsiname,
                                         uint32 requestId,
                                         const std::string &fileName) {
...
void CBSIINonModule::dispatchSendFile(const std::string &bsiname,
                                      uint32 requestId,
                                      const CBackupMsgSaveFile &msg) {
  nlassert(bsiname == "BS" /* || bsiname=="PDBS"*/);

  // register the request id for later use
  CGenericRequestIdRegister::getInstance().pushRequestId(requestId,
                                                         msg.FileName);
...
void CBSIINonModule::dispatchAppendData(const std::string &bsiname,
                                        uint32 requestId,
                                        const CBackupMsgSaveFile &msg) {
  nlassert(bsiname == "BS" /* || bsiname=="PDBS"*/);
...
void CBSIINonModule::dispatchAppendText(const std::string &bsiname,
                                        uint32 requestId,
                                        const std::string &filename,
...
void CBSIINonModule::dispatchDeleteFile(const std::string &bsiname,
                                        uint32 requestId,
                                        const std::string &fileToDelete,
...
void CBSIINonModule::dispatchRequestFileClass(
    const std::string &bsiname, uint32 requestId, const std::string &directory,
    ...
    breakable {
      ...
      try {
        ...
        try {
          ...
          nlinfo("Connection to BS Master: %s or %s Failed",
                 addr1.toStringLong().c_str(), addr2.toStringLong().c_str());
          break;
        }
      }

      if (isConnectedL3BSInReadWriteMode()) {
        nlinfo("Connection to BS Master: %s in read/write mode succeeded",
               bsName.c_str());
        return;
      }

      nlinfo(
          "Connection to BS Master: %s succeeded but BS is in Write Only mode",
          bsName.c_str());
      _L3BSConn.disconnect();
    }
    ...
    breakable {
      ...
      try {
        ...
        try {
          ...
          nlinfo("Connection to BS Slave: %s or %s Failed",
                 addr1.toStringLong().c_str(), addr2.toStringLong().c_str());
          break;
        }
      }
      ...
      if (isConnectedL3BSInReadWriteMode()) {
        nlinfo("Connection to BS Slave: %s in read/write mode succeeded",
               bsName.c_str());
        return;
      }

      nlinfo(
          "Connection to BS Slave: %s succeeded but BS is in Write Only mode",
          bsName.c_str());
      _L3BSConn.disconnect();
    }

    nlinfo("Sleeping a few seconds before retrying connection to backup "
           "services...");
    nlSleep(10);
  }
}
...
void CBSIINonModule::dispatchSyncLoadFileClass(
    const std::string &bsiname, uint32 requestId, const std::string &directory,
    const std::vector<CBackupFileClass> &classes) {
  nlassert(bsiname == "BS" /* || bsiname=="PDBS"*/);
  ...
  do {
    ...
    do {
      _L3BSConn.update2(-1, 10);
    } while (!singleton->fileClassCallbackDone(requestId) &&
             _L3BSConn.connected());

  } while (!_L3BSConn.connected());
}

void CBSIINonModule::dispatchSyncLoadFile(const std::string &bsiname,
                                          uint32 requestId,
                                          const std::string &fileName,
                                          bool notBlocking) {
  nlassert(bsiname == "BS" /* || bsiname=="PDBS"*/);
  ...
  if (notBlocking) {
    bool insertResult;
    insertResult =
        _L3PendingResquest.insert(make_pair(requestId, fileName)).second;
    STOP_IF(!insertResult, "Duplicate request ID found - this is BAD!: id="
                               << requestId << " for file: " << fileName);
  }
...
void CBSIINonModule::terminateSyncLoads() {
  ...
  while (!_L3PendingResquest.empty()) {
    ...
    map<uint32, std::string>::iterator first(_L3PendingResquest.begin()),
        last(_L3PendingResquest.end());
    ...
    if (!_L3BSConn.connected()) {
      ...
      nlinfo("Lost connection with BS during loadSyncFiles !, reconnecting and "
             "reloading %u left files",
             _L3PendingResquest.size());
      connectL3();

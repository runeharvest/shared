...
static const struct {
  ...
  CSeason::TSeason Value;
    ...
    {"Spring", CSeason::Spring},   {"Summer", CSeason::Summer},
    {"Autumn", CSeason::Autumn},   {"Winter", CSeason::Winter},
...
void CSeason::init() {
  ...
  for (i = 0; i < 5; ++i) {
    ...
    _ValueMap[NLMISC::toLowerAscii(std::string(TSeasonConvert[i].Name))] =
        TSeasonConvert[i].Value;
  }
  _Initialised = true;
}

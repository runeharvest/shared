...
public:
  ...
  bool serialParam(bool debug, NLMISC::IStream &f,
                   TParamType type = NB_PARAM_TYPES);

  TParam() : Type(invalid_value) {}

  TParam(TParamType type) : Type(type) {}
...
public:
  TParam(TParamType type, sint32 i) : Type(type), Int(i) {}
  TParam(TParamType type, uint32 i) : Type(type), Time(i) {}
  TParam(TParamType type, uint64 money) : Type(type), Money(money) {}
 ...
 *	database mecanisme.
 */
uint32 sendStringToClient(const TDataSetRow &clientIndex,
 ...
 *	database mecanisme.
 */
inline uint32 sendStringToClient(const TDataSetRow &clientIndex,
 ...
 *	originator.
 */
void sendCustomEmoteTextToClientAudience(
    const TDataSetRow &clientIndex,
    const std::vector<NLMISC::CEntityId> &excluded, const char *stringName,
    const std::vector<STRING_MANAGER::TParam> &params, ISender *sender = NULL);
 ...
 *	originator.
 */
void sendSystemStringToClientAudience(
    const TDataSetRow &clientIndex,
    const std::vector<NLMISC::CEntityId> &excluded,
    CChatGroup::TGroupType audience, const char *stringName,
    const std::vector<TParam> &params, ISender *sender = NULL);

/** Send a string to a user (i.e.: a player that has not chosen a character yet)
 *through IOS. This method create a message and send it to the IOS service. The
 *return value is the string sequence number. This number can then be send to
 *the client via the database mecanisme.
 */
uint32 sendStringToUser(uint32 userId, const std::string &stringName,
                        const std::vector<TParam> &params,
                        ISender *sender = NULL);
 ...
 *the client via the database mecanisme.
 */
inline uint32 sendStringToUser(uint32 userId, const char *stringName,
                               const std::vector<TParam> &params,
                               ISender *sender = NULL) {
  std::string str(stringName);
...
public:
  ...
  explicit CVectorParamCheck(size_type s)
      : std::vector<STRING_MANAGER::TParam>(s) {}
  CVectorParamCheck(const std::vector<STRING_MANAGER::TParam> &o)
      : std::vector<STRING_MANAGER::TParam>(o) {}
  template <class InputIterator>
  CVectorParamCheck(InputIterator first, InputIterator last)
      : std::vector<STRING_MANAGER::TParam>(first, last) {}
  ...
  std::vector<STRING_MANAGER::TParam>::const_reference
...
#define SM_STATIC_PARAMS_1(__vectorName, __paramType1)                         \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 1)
...
#define SM_STATIC_PARAMS_2(__vectorName, __paramType1, __paramType2)           \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 2)

#define SM_STATIC_PARAMS_3(__vectorName, __paramType1, __paramType2,           \
                           __paramType3)                                       \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 3)

#define SM_STATIC_PARAMS_4(__vectorName, __paramType1, __paramType2,           \
                           __paramType3, __paramType4)                         \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 4)

#define SM_STATIC_PARAMS_5(__vectorName, __paramType1, __paramType2,           \
                           __paramType3, __paramType4, __paramType5)           \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 5)

#define SM_STATIC_PARAMS_6(__vectorName, __paramType1, __paramType2,           \
                           __paramType3, __paramType4, __paramType5,           \
                           __paramType6)                                       \
  ...
  static STRING_MANAGER::CVectorParamCheck __vectorName(                       \
      __vectorName##Params, __vectorName##Params + 6)

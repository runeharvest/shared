...
CObject *CScenario::find(const std::string &instanceId,
                         const std::string &attrName, sint32 position,
                         const std::string &key) {
  CObject *src = _InstanceMap->find(instanceId);
  if (!src) {
    nlwarning("Can't find object with id '%s' (looking for attribute '%s[%d]' "
              "or '...[%s]')",
              instanceId.c_str(), attrName.c_str(), (int)position, key.c_str());
    return NULL;
  }
  if (!attrName.empty()) {
  ...
  if (position != -1) {
    ...
    if (!subObj) {
      nlwarning(
          "Can't find attribute %s[%d] inside object with InstanceId =  %s",
          attrName.c_str(), (int)position, instanceId.c_str());
      return NULL;
    }
    src = subObj;
  }
  if (!key.empty()) {
    ...
    if (!subObj) {
      nlwarning(
          "Can't find attribute %s['%s'] inside object with InstanceId =  %s",
          attrName.c_str(), key.c_str(), instanceId.c_str());
      return NULL;
    }
    src = subObj;
  }
...
bool CScenario::setNode(const std::string &instanceId,
                        const std::string &attrName, CObject *value) {
  _Clean = false;
  CObject *found = _InstanceMap->find(instanceId);

  if (!found) {
    nlwarning("CScenario::setNode : couldn't find object with id = %s",
              instanceId.c_str());
    return false;
  }
...
bool CScenario::insertNode(const std::string &instanceId,
                           const std::string &attrName, sint32 position,
  ...
  if (!found) {
    nlwarning("CScenario::insertNode : couldn't find object with id = %s",
              instanceId.c_str());
    return false;
  }
  if (!attrName.empty()) {
  ...
  if (!found) {
    nlwarning("CScenario::insertNode : couldn't find attribute '%s' in object "
              "with id = %s",
              attrName.c_str(), instanceId.c_str());
    return false;
  }
...
bool CScenario::eraseNode(const std::string &instanceId,
                          const std::string &attrName, sint32 position) {
  _Clean = false;
  CObject *found = _InstanceMap->find(instanceId);
  if (!found) {
    nlwarning("CScenario::eraseNode : couldn't find object with id = %s",
              instanceId.c_str());
    return false;
  }
  ...
  if (!found) {
    nlwarning("CScenario::eraseNode : couldn't find attribute '%s' in object "
              "with id = %s",
              attrName.c_str(), instanceId.c_str());
    return false;
  }
  ...
  if (found->isTable()) {
    ...
    if (!canRemove) {
      nlwarning("CScenario::eraseNode : can not remove a non-existing object "
                "from a table (%s, %s, %d)",
                instanceId.c_str(), attrName.c_str(), position);
      return false;
    }

    removed = found->take(position);
    if (!removed) {
      nlwarning("CScenario::eraseNode remove a NULL object (%s, %s, %d)",
                instanceId.c_str(), attrName.c_str(), position);
      return true;
    }
  } else if (position == -1) {
    ...
    nlwarning("CScenario::eraseNode when removing (%s, %s) : position must be "
              "-1, because object is not a table",
...
bool CScenario::moveNode(const std::string &instanceId1,
                         const std::string &attrName1, int position1,
                         const std::string &instanceId2,
                         const std::string &attrName2, int position2) {
  _Clean = false;

  static volatile bool verboseMoveNode = false;
  CObject *from = _InstanceMap->find(instanceId1);
  if (from && !attrName1.empty()) {
    from = from->getAttr(attrName1);
  }
  if (!from) {
    nlwarning("<CScenario::moveNode> 'from' node with id (%s, %s) not found",
              instanceId1.c_str(), attrName1.c_str());
    return false;
  }
  ...
  if (!to) {
    nlwarning("<CScenario::moveNode> 'to' node with id (%s, %s) not found",
              instanceId2.c_str(), attrName2.c_str());
    return false;
  }

  if (!(to->isTable() && -1 <= position2 &&
        position2 <= static_cast<sint32>(to->getSize()))) {
    nlwarning("<CScenario::moveNode> Can not move node to (%s, %s, %d)",
              instanceId2.c_str(), attrName2.c_str(), position2);
    return false;
  }

  if (!from->canTake(position1)) {
    nlwarning("<CScenario::moveNode> Can not move node from (%s, %s, %d)",
              instanceId1.c_str(), attrName1.c_str(), position1);
    return false;
  }
  ...
  if (!removed) {
    nlwarning("<CScenario::moveNode> Error: try to move a non-existing object "
              "from a table (%s, %s, %d)",
              instanceId1.c_str(), attrName1.c_str(), position1);
    return false;
  }
  ...
  if (root->isTable()) {
    if (root->isString(_IdName)) {
      ...
      if (_Map.find(instanceId) != _Map.end()) {
        nlwarning("Trying to add object to the instance map but object already "
                  "exist. Objec t is :");
        root->dump();
        return;
      }
...
void CInstanceMap::remove(CObject *root) {
  if (root->isTable()) {
    if (root->isString("InstanceId")) {
      ...
      if (found == _Map.end()) {
        nlwarning("Trying to remove object from instance map but object is not "
                  "found. Objec t is :");
        root->dump();
        return;
      }
...
CObject *CInstanceMap::find(const std::string &instanceId) {
  std::map<std::string, CObject *>::const_iterator found =
      _Map.find(instanceId);
  if (found != _Map.end()) {
    return found->second;
  }
...
bool CScenario::isRunning() const { return !isEditing(); }

bool CScenario::isWaiting() const { return false; }
...
CUserComponent::CUserComponent(const std::string &filename,
    ...
    : Filename(filename), UncompressedData(uncompressedData),
...
uint32 CUserComponent::getUncompressedDataLength() const {
...
std::string CEmoteBehavior::get(const std::string &emoteId) const {
  ...
  std::map<std::string, std::string>::const_iterator found(
      _EmotesMap.find(emoteId));
  if (found == _EmotesMap.end())
    return "";
  return found->second;
}
...
void CEmoteBehavior::load() const {
  ...
  _EmotesMap["Lustful"] = "";
  _EmotesMap["Malevolent"] =
      "cold_contemptuous_disdainful_haughty_megalomaniac_obnoxious";
  _EmotesMap["Malicious"] = "giggle";
  _EmotesMap["Mean"] = "";
  _EmotesMap["Megalomaniac"] =
      "cold_contemptuous_disdainful_haughty_megalomaniac_obnoxious";
  _EmotesMap["Merciful"] = "";
...
bool CScenarioValidator::setScenarioToLoad(const std::string &filename,
  ...
  try {
    ...
    if (_ScenarioBody.size() < headerLen ||
        _ScenarioBody.substr(0, headerLen) != header) {
      md5.clear();
      signature.clear();
      inf.close();
      return true;
    ...
    std::string::size_type endHeader =
        _ScenarioBody.find(slashheader, headerLen);
    ...
    NLMISC::splitString(
        _ScenarioBody.substr(startHeader, endHeader - startHeader), "'\n",
        lines);
    std::vector<std::string>::const_iterator firstLine(lines.begin()),
        lastLine(lines.end());
    ...
    if (values.size() >= 2 && values[0].first == "Version" &&
        values[1].first == "Signature" && values[2].first == "HeaderMD5" &&
        values[3].first == "BodyMD5") {
      std::string headerBodyMd5;
      std::string::size_type subHeader =
          _ScenarioBody.find("-- BodyMD5", startHeader);
      if (checkMD5) {
        std::string md5Id1 =
            NLMISC::getMD5((uint8 *)(_ScenarioBody.data() + subHeader),
                           (uint32)(endHeader - subHeader))
                .toString();
        ...
        std::string md5Id2 =
            NLMISC::getMD5(
                (uint8 *)(_ScenarioBody.data() + endHeader + slasheaderLen),
                (uint32)(_ScenarioBody.size() - (endHeader + slasheaderLen)))
                .toString();
...
void CScenarioValidator::applyLoad(std::string &filename, std::string &body,
                                   CScenarioValidator::TValues &values) {
  filename = _Filename;
  body = _ScenarioBody;
  values = _Values;
...
bool CScenarioValidator::setScenarioToSave(
    const std::string &filename, CObject *scenario,
    const CScenarioValidator::TValues &values, std::string &headerMD5) {
  _Filename = filename;
  ...
  _ScenarioBody = out2;
  {
    NLMISC::CHashKeyMD5 md5Id = NLMISC::getMD5((uint8 *)_ScenarioBody.data(),
                                               (uint32)_ScenarioBody.size());
  ...
  std::string headerBodyMd5;
  {
    NLMISC::CHashKeyMD5 md5Id =
        NLMISC::getMD5((uint8 *)_HeaderBody.data(), (uint32)_HeaderBody.size());
...
CScenarioValidator::CScenarioValidator(
...
/**********************************************************/
bool CUserComponentValidator::setUserComponentToLoad(
    const std::string &filename, CScenarioValidator::TValues &values,
    std::string &md5, std::string &signature, bool checkMD5) {
  ...
  try {
    ...
    // Scenario without header
    if (_UserComponentBody.size() < headerLen ||
        _UserComponentBody.substr(0, headerLen) != header) {
      md5.clear();
      signature.clear();
      inf.close();
      return true;
    ...
    std::string::size_type endHeader =
        _UserComponentBody.find(slashheader, headerLen);
    if (endHeader == std::string::npos) {
      inf.close();
      return false;
    }
    ...
    std::vector<std::string> lines;
    NLMISC::splitString(
        _UserComponentBody.substr(startHeader, endHeader - startHeader), "'\n",
        lines);
    std::vector<std::string>::const_iterator firstLine(lines.begin()),
        lastLine(lines.end());
    std::vector<std::string> result;
    ...
    if (values.size() >= 2 && values[0].first == "Version" &&
      ...
      std::string headerBodyMd5;
      std::string::size_type subHeader =
          _UserComponentBody.find("-- BodyMD5", startHeader);
      if (checkMD5) {
        std::string md5Id1 =
            NLMISC::getMD5((uint8 *)(_UserComponentBody.data() + subHeader),
                           (uint32)(endHeader - subHeader))
                .toString();
        if (values[2].second != md5Id1) {
          return false;
        ...
        std::string md5Id2 =
            NLMISC::getMD5((uint8 *)(_UserComponentBody.data() + endHeader +
                                     slasheaderLen),
                           (uint32)(_UserComponentBody.size() -
                                    (endHeader + slasheaderLen)))
                .toString();
        if (values[3].second != md5Id2) {
          return false;
        }
...
void CUserComponentValidator::applyLoad(std::string &filename,
                                        std::string &body,
                                        CScenarioValidator::TValues &values) {
  filename = _Filename;
...
bool CUserComponentValidator::setUserComponentToSave(
    const std::string &filename, const CUserComponentValidator::TValues &values,
    std::string &headerMD5, const std::string &body) {
  _Filename = filename;
  //	std::ostringstream out2;
  //	out2.str(body);
  ...
  {
    NLMISC::CHashKeyMD5 md5Id = NLMISC::getMD5(
        (uint8 *)_UserComponentBody.data(), (uint32)_UserComponentBody.size());
    _BodyMd5 = md5Id.toString().c_str();
  }
...
bool CUserComponentValidator::applySave(const std::string &signature) {
  ...
  try {
    ...
    nlwarning("Can't save user component: the file %s can not be saved.",
              _Filename.c_str());
    out.close();
...
CUserComponentValidator::CUserComponentValidator(
    CUserComponentValidatorLoadSuccededCallback *loadCb)
    : _LoadCb(loadCb) {}

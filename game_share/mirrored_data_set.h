 ...
 * The dataset contains the values of these properties for the entities that
 * have these (ex: a player has a property X therefore its entity will be added
 * into the dataset that handles property X).
 *
 ...
 *
 * Most of the following method comments give an indication on the
 * time-complexity. Fast means it uses indexed-array addressing, while slow
 * means that some map or hash map look-ups are involved.
 *
 * \seealso CMirror
...
public:
  ...
  /**
   * Declare an entity previously added by CMirror::createEntity() in the
   * dataset (if the entity type is matching the dataset) to other services.
   *
   ...
   * with the mirror. Refer to CMirror::isRunningSynchronizedCode() to know the
   * conditions.
   *
  ...
  //
  // You can get the "change timestamp" calling getChangeTimestamp() in the base
  // class.
  //

  /**
   * Return true if the row is currently accessible, meaning the corresponding
   *entity is online now and can be accessed in the mirror, If row.isNull(),
   *return false. If another service B on the same machine despawns the entity
   *in the same game cycle, after isAccessible() returned true in A, the row can
   *still be accessed (read/written) by A.
   *
   ...
   *      cout << value();
   *   }
   * Why? Because AT ANY MOMENT the status of isAccessible() can change to
   *false. It happens when a service B on the same machine despawns the entity.
   * However, reading the value will still work even if the entity despawned
   *between the call to isAccessible() and the call to doRead(). The doRead()
   *function will be CORRECT without the nlassert or by replacing it with an
   *harmless 'if(TheDataset.isAccessible(r))'.
   */
   ...
   * Thus it will return true for a row that is created locally but yet
   * unpublished (which is not called "online"). Caution: it will also return
   * true if the row is offline, provided the current counter matches the
   * counter in 'row'.
   */
  inline bool isAccessible2(const TDataSetRow &row) const;

  /**
   * Get the entity id corresponding to entityIndex in the dataset (linear
   * access).
   *
  ...
  /**
   * Get the row corresponding to the specified entity id in the dataset (result
   * isNull() if not found) (map-style access).
   *
   * getDataSetRow() will return the entityId:
   * - Between createEntity() and removeEntity() if the entity is managed by the
   * same service
   * - Between the adding and the removal notifications if the entity is managed
   * by another service
   */
  TDataSetRow getDataSetRow(const NLMISC::CEntityId &entityId) const;

  /**
   * Get a valid datasetrow usable with the current entity at entity index, if
   * the entity is online. If the entity is offline, the method returns an
   * invalid datasetrow (offline means the row is not used; warning: after
   * despawning an entity by a service A, and before notifying the entity
   * removal in a service B, the entity will be reported as offline).
   */
  TDataSetRow getCurrentDataSetRow(TDataSetIndex entityIndex) const;

  /**
   * Get a valid datasetrow usable with the current entity at entity index, be
   * the entity online or not. warning: after despawning an entity by a service
   * A, and before notifying the entity removal in a service B, the entity will
   * be reported as offline).
   */
  TDataSetRow forceCurrentDataSetRow(TDataSetIndex entityIndex) const;

  /** Access the map of entities (begin iteration). Use GET_ENTITY_INDEX(it) to
   * access the TDataSetRow index. You can iterate it to get the online entities
   * (use GET_ENTITY_INDEX to obtain a dataset index from the iterator, then
   * call getCurrentDataSetRow() to have a dataset row, and check isValid() in
   * the case when the entity would have been despawned and the service not
   * notified yet.
   */
  ...
  void setValue(const TDataSetRow &entityIndex, TPropertyIndex propertyIndex,
                T value);

  /// Set a property value, by slow ids
  template <class T, class CPropLocationClass>
  void setValue(const NLMISC::CEntityId &entityId, const std::string &propName,
                T value);

  /// Get a property value, by fast index
  template <class T>
  void getValue(const TDataSetRow &entityIndex, TPropertyIndex propIndex,
                T &returnedValue) const {
    T *ptvalue;
  ...
  template <class T>
  void getValue(const NLMISC::CEntityId &entityId, const std::string &propName,
                T &returnedValue) const;
  ...
  // and you must call processRemovedEntities() (if you don't use removal
  // notification) or
  ...
  // while ( (entityIndex = TheDataset.getNextRemovedEntity( &pEntityId )) !=
  // LAST_CHANGED )
  //    (...) // your own processing here
  // TheDataset.endRemovedEntities();
  //
  // Important note: it is NOT garanteed that an addition notification for an
  // entity occurs before a change notification for this entity. If you use rows
  // (TDataSetRow) to manage your objects, this should not be a problem.
  //

  /**
   * Begin a browsing session (see getNextAddedEntity(), endAddedEntities()),
   * only in the notification callback provided to
   * CMirror::setNotificationCallback(). The browsing of added entities must be
   * done BEFORE the browsing of the removed entities.
   */
  void beginAddedEntities() {}

  /**
   * Continue browsing the added entities, until the method returns
   * LAST_CHANGED. They can have been added either by the current (caller)
   * service or by another service that owns some entities concerned by the
   * dataset.
   */
   ...
   * endAddedEntities(). Must be done BEFORE the browsing of removed entities.
   * Call only when mirrorIsReady() is true.
   */
  void processAddedEntities();

  /**
   * Begin a browsing session (see getNextRemovedEntity(),
   * endRemovedEntities()), only in the notification callback provided to
   * CMirror::setNotificationCallback(). The browsing of removed entities must
   * be done AFTER the browsing of the removed entities.
   */
  void beginRemovedEntities() {}

  /**
   * Continue browsing the removed entities, until the method returns
   * LAST_CHANGED. They can have been removed either by the current (caller)
   * service or by another service that owns some entities concerned by the
   * dataset. You must not use the entity index anymore. That is why the method
   * also outputs the entity id of the removed entity (note: it makes the
   * pointer you provide pointing to the entity id. The pointer will not remain
   * valid after calling endRemovedEntities()). Note: The returned datasetrow
   * has the same counter as it had in getNextAddedEntity(). It means you can
   * use the whole TDataSetRow as a key of a map/set, inserting the result of
   * getNextAddedEntity() and erasing the result of getNextRemovedEntity().
   */
  TDataSetRow getNextRemovedEntity(NLMISC::CEntityId **entityId);

  /// End browsing the removed entities. Be sure to call this method every
  /// cycle.
   ...
   * endRemovedEntities(). Must be done AFTER the browsing of removed entities.
   * Call only when mirrorIsReady() is true.
   */
   ...
   * changes (see ...forProp() methods below). Don't use both schemes in the
   * same service (they are not compatible).
   */
  void beginChangedValues();

  /**
   * Continue browsing the changed values, until the method returns LAST_CHANGED
   * in entityIndex. See also setFastCallbackForAdditionHasBeenNotified().
   */
   ...
   * endChangedValuesForProp(), not both notifications schemes in the same
   ...
   * beginChangedValuesForProp(), until the method returns LAST_CHANGED. See
   * also setFastCallbackForAdditionHasBeenNotified().
   */
  ...
  /// Return the number of our entities currently existing (only the ones
  ...
  const TDeclaredEntityRangeOfType &getDeclaredEntityRanges() const {
    return _DeclaredEntityRanges;
  }

  /// Return the name of the dataset
  const std::string &name() const { return _DataSetName; }

  /// Return true if the specified property represents a list and not a single
  /// value
  bool propIsList(TPropertyIndex propIndex) const;

  /// Display all or part of the entities in the dataset (0xFF=no filter).
  /// Return the number of entities matching the filters
  uint displayEntities(NLMISC::CLog &log = *NLMISC::InfoLog,
                       uint8 onlyEntityType = 0xFF, uint8 onlyCreatorId = 0xFF,
                       uint8 onlyDynamicId = 0xFF,
                       bool hideUndeclaredbool = false,
                       bool displayLocalCreationTimestamp = true,
                       bool sortByEntityId = false,
                       bool sortByDatasetRow = false) const;

  /// Look for a property value (if anyValue if true, ignores searchedValue and
  /// displays all values for the property) (see displayEntities() for common
  /// params)
  uint lookForValue(NLMISC::CLog &log = *NLMISC::InfoLog,
                    const std::string &propName = "", bool anyValue = false,
                    const std::string &searchedValueStr = "0",
                    uint8 onlyEntityType = 0xFF, uint8 onlyCreatorId = 0xFF,
                    uint8 onlyDynamicId = 0xFF, bool hideUndeclaredbool = false,
                    bool displayLocalCreationTimestamp = true,
                    bool sortByEntityId = false,
                    bool sortByDatasetRow = false) const;

  /// Return the number of declared entities
  uint getNbOnlineEntities() const {
    return (uint)_EntityIdToEntityIndex.size() - _NumberOfUndeclaredEntities;
  }
  ...
  template <class T, class CPropLocationClass>
  void displayPropValues(TPropertyIndex propIndex, T *pt,
                         NLMISC::CLog &log = *NLMISC::InfoLog) const;

  /// Display a value
  void displayPropValue(const std::string &propName,
                        const TDataSetRow &entityIndex,
                        TPropertyIndex propIndex, const char *flagsStr,
                        NLMISC::CLog &log = *NLMISC::InfoLog) const;

  /// Set a display callback for the specified property. It will be used by
  /// CMirror::displayRows() (command "displayMirrorRow")
  void setDisplayCallback(TPropertyIndex propIndex, TValueToStringFunc cb);

  /// Change a value from a string
  void setValueFromString(const TDataSetRow &entityIndex,
                          TPropertyIndex propIndex,
                          const std::string &valueStr);

  /// Return a property value as string
  void getValueToString(const TDataSetRow &entityIndex,
                        TPropertyIndex propIndex, std::string &result) const;
  ...
  void displayUnknownEntities(TDataSetIndex first, TDataSetIndex end,
                              NLMISC::CLog &log);
...
protected:
  ...
  const TEntityRange &addEntityTypeRange(uint8 entityTypeId,
                                         TDataSetIndex first,
                                         TDataSetIndex last);

  /// Add an entity (of owned entity type)
  uint addEntityToDataSet(bool fillEntityId, NLMISC::CEntityId &entityId,
                          bool declare);
  ...
  void accessNewPropTracker(bool self, TPropertyIndex propIndex, sint32 smid,
                            sint32 mutid);

  /// Add trackers or set the self trackers for adding/removing entities
  void accessEntityTrackers(bool self, sint32 smidAdd, sint32 smidRemove,
                            sint32 mutidAdd, sint32 mutidRemove);

  /// Serial owned entity ranges (without entity types) and return the number of
  /// ranges
  ...
  /// entities added
  uint scanAndResyncExistingEntities(
  ...
  /// Scan the rows for 'already set' values for the property, set by the
  /// specified service.
  uint16 scanAndResyncProp(TPropertyIndex propIndex,
  ...
  ///
  void addDeclaredEntityRange(uint8 entityType, const TEntityRange &src,
                              NLNET::TServiceId declaratorService);

  ///
  void addDeclaredEntityRanges(const TEntityRangeOfType &ranges,
                               NLNET::TServiceId declaratorService);
  ...
  ///
  bool entityMatchesFilter(const NLMISC::CEntityId &entityId,
                           TDataSetIndex entityIndex, uint8 onlyEntityType,
...
private:
   ...
   * Self addition/removal: mapped in addEntityToDataSet(), unmapped in
   * removeEntityFromDataSet(). Remote addition/removal: mapped in
   * getNextAddedEntity(), unmapped in endRemovedEntities().
   */
  ...
  /// List of trackers (not indexed by property), always allocated because they
  /// are added and allocated at the same time
  TSelfPropTrackers _SelfPropTrackers;

  /// List of trackers (indexed by TPropertyIndex), always allocated because
  /// they are added and allocated at the same time
  TTrackersLists _PropTrackers;

  // List of entity tracker filters for other services (same indices as
  // _EntityTrackers[*]) TEntityTrackerFilterList	_EntityTrackerFilters;

  /// Lists of entity trackers for other services, always allocated because they
  /// are added and allocated at the same time
...
public:
  ...
  CMirroredDataSet()
      : _PropAllocator(NULL), _NumberOfUndeclaredEntities(0),
        _NumberOfCurrentCreatedEntities(0),
        _IsEntityNotificationEnabled(false) {}
  ...
  TValueToStringFunc valueToStringCb(TPropertyIndex propIndex) const {
    return _ValueToStringCb[propIndex];
  }
};

...
enum TBotChatFlags {
  ...
  Attackable,           // 256     8 ( entity is attackable == (Attackable ||
                        // CProperties::attackable ) )
...
enum TBotChatSellerType {
  ...
  NPC = 0,
  // this item was created and sold by a third-party Player. appears only in the
  // "Resale list".
  // => PRICE_RETIRE and RESALE_TIME_LEFT are not relevant.
  Resale,
  // this item was created and sold by the player who whatch the list. appears
  ...
  UserRetirable,
  // this item was created and sold by the player who whatch the list. appears
  // only in the "Resale list". but useful to know that it belongs to the player
  // and therefore PRICE_RETIRE and RESALE_TIME_LEFT are relevant
  ResaleAndUser,
 ...
 *	For instance if an item cost 100 (minimum price), the price recommended
 *for resell is 200
 */

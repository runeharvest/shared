...
CVariable<uint> MaxNpcs("DSS", "MaxNpcs",
                        "Maximum number of Npc's allowed in an act", 100, 0,
                        true);
CVariable<uint>
    MaxStaticObjects("DSS", "MaxStaticObjects",
                     "Maximum number of Static Object allowed in an act", 100,
                     0, true);

CVariable<bool> AutoCreateSession(
    "DSS", "AutoCreateSession",
    "Create a session for each Client Edition Module and add char as an editor",
    false, 0, true);
CVariable<bool> AutoCreateAnimationSession("DSS", "AutoCreateAnimationSession",
                                           "Create an animation session", false,
                                           0, true);
CVariable<bool>
    AutoCreateAddCharacter("DSS", "AutoCreateAddCharacter",
                           "Add a character to a session witout creating it",
                           false, 0, true);
CVariable<string> AutoCreateRole(
    "DSS", "AutoCreateRole",
    "Set the Role of default connected ur_player | ur_editor | ur_animator",
    "ur_editor", 0, true);
CVariable<bool>
    AutoCreateSessionMultiChar("DSS", "AutoCreateSessionMultiChar",
                               "Allow a Cooperative edition with local server",
                               false, 0, true);
CVariable<uint32> AutoCreateSessionMultiCharSessionId(
    "DSS", "AutoCreateSessionMultiCharSessionId",
    "Session Id of a AutoCreateSession", 1, 0, true);

// 30 secondes
CVariable<uint32> TimeBeforeDisconnectionAfterKick(
    "DSS", "TimeBeforeDisconnectionAfterKick",
    "Delay in second the client has before quit a session after being kicked "
    "from it",
    30, 0, true);

// If true load hibernating session and saved session at startup
CVariable<bool> DssUseBs("DSS", "DssUseBs",
                         "If activate the resume/hibernation system is active",
                         true, 0, true);
// 1 minutes
CVariable<uint32> TimeBeforeAutoSaveTimeEditionSession(
    "DSS", "TimeBeforeAutoSaveTimeEditionSession",
    "Delay in seconde before empty edition is saved (save occurs only if "
    "scenario has changed)",
    60, 0, true);
// 5 minutes
CVariable<uint32> TimeBeforeStopTestInEditionSessionWithNoPlayer(
    "DSS", "TimeBeforeStopTestInEditionSessionWithNoPlayer",
    "Delay in secondes before test in empty edition is stopped", 5 * 60, 0,
    true);
// 15 minutes
CVariable<uint32> TimeBeforeAutoHibernateEditionSessionWithNoPlayer(
    "DSS", "TimeBeforeAutoHibernateEditionSessionWithNoPlayer ",
    "Delay in secondes before an empty edition is hibernating", 15 * 60, 0,
    true);
// 7 days
CVariable<uint32> TimeBeforeCloseHibernatingEditionSession(
    "DSS", "TimeBeforeCloseHibernatingEditionSession",
    "Delay in secondes before a empty hibernating edition is remove",
    7 * 24 * 60, 0, true);

// 15 minutes
CVariable<uint32> TimeBeforeAutoCloseAnimationSessionWithNoPlayer(
    "DSS", "TimeBeforeAutoCloseAnimationSessionWithNoPlayer",
    "Delay in secondes before a empty animation session is closed", 15 * 60, 0,
    true);

CVariable<std::string> DefaultCharRingAccess(
    "DSS", "DefaultCharRingAccess",
    "Default Access Ring for player eg 'f1:j1:l1:d1:p1:g1:a1:'", "a1", 0, true);
CVariable<bool>
    DontUseSU("DSS", "DontUseSU",
              "If 1, allow running a shard with no Shard Unifier Service",
              false, 0, true);

CVariable<bool> AcceptScenarioWithoutHeader("DSS",
                                            "AcceptScenarioWithoutHeader",
                                            "AcceptScenarioWithoutHeader",
                                            false, 0, true);
CVariable<bool> MustVerifyRingAccessWhileLoadingAnimation(
    "DSS", "MustVerifyRingAccessWhileLoadingAnimation",
    "MustVerifyRingAccessWhileLoadingAnimation", false, 0, true);
CVariable<int> OverrideRoSValue("DSS", "OverrideRoSValue", "OverrideRoSValue",
                                -1, 0, true);
...
static inline void debugInfo(bool ok, const std::string &msg) {
  if (ok) {
    nlinfo("Ed: %s", msg.c_str());
  } else {
    nlwarning("Ed: %s", msg.c_str());
  }
}
} // namespace
...
/*! Identify an user
 * Identify a user (it's editor id / characterId / playerId type of session
 * (edition / animation ....)
 */
class CCurrentChar {
public:
...
public:
  CCurrentChar(TCharId charId, uint32 editSlotId,
               const R2::TUserRole &userRole = R2::TUserRole::ur_editor)
      : _EditSlotId(editSlotId), _CharId(charId), _UserRole(userRole) {}

  void setUserRole(const R2::TUserRole &userRole) { _UserRole = userRole; }
...
public:
  typedef uint32 TCharId;
  //		typedef CSessionId TSessionId; //		typedef uint32
  //TSessionId;
  typedef uint32 TAiInstanceId;
  typedef std::list<CCurrentChar *> TCurrentChars;

public:
  CEditionSession(RSMGR::TSessionType type = RSMGR::TSessionType::invalid_val,
                  TSessionId sessionId = TSessionId(0), TCharId ownerCharId = 0,
                  TAiInstanceId aiInstanceId = 0, CScenario *scenario = 0);
  // This session shared data with Scenario
  CEditionSession(RSMGR::TSessionType type, TSessionId sessionId,
                  CEditionSession *editionSession);
  ...
  void setSessionType(RSMGR::TSessionType sessionType) {
  ...
  void setStartParams(sint32 x, sint32 y, uint8 season) {
    InitialX = x;
    InitialY = y;
    InitialSeason = season;
  }

  void getStartParamsWithDelay(CServerEditionModule *module, uint32 charId,
...
public:
  CTaskUpdateScenarioVision(NLMISC::TTime now, CServerEditionModule *module,
                            TSessionId sessionId)
      : CTask<NLMISC::TTime>(now), _SessionId(sessionId), _Module(module) {}
  virtual void doOperation() { _Module->updateScenarioVision(_SessionId); }
...
public:
  CTaskGetStartParamsWhenLoaded(CServerEditionModule *module, TCharId charId,
                                TSessionId sessionId)
...
public:
//		typedef CSessionId TSessionId; //		typedef uint32
TSessionId; typedef uint32 TCharId;

public:

    CTaskConnectPlayer(CServerEditionModule* serverEditionModule, TSessionId
sessionId, TCharId charId, TUserRole userRole, const std::string& ringAccess,
bool newcomer) :CTask<NLMISC::TTime>(0)
        ,_ServerEditionModule(serverEditionModule), _SessionId(sessionId),
_CharId(charId), _UserRole(userRole), _RingAccess(ringAccess),
_Newcomer(newcomer){}

    virtual void doOperation()
    {
        _ServerEditionModule->connectChar(_SessionId, _CharId, _UserRole,
_RingAccess, _Newcomer);
    }
private:
...
public:
  CTaskTryConnectPlayer(CServerEditionModule *serverEditionModule,
                        TCharId charId)
      : CTask<NLMISC::TTime>(0), _ServerEditionModule(serverEditionModule),
        _CharId(charId) {}
...
public:
  CKickPlayerIfStillConnected(NLMISC::TTime taskDate,
                              CServerEditionModule *serverEditionModule,
                              TSessionId sessionId, TCharId charId)
      : CTask<NLMISC::TTime>(taskDate),
  ...
  virtual void doOperation() {
    ...
    if (_ServerEditionModule->getSessionIdByCharId(_CharId) == _SessionId) {
      ...
      std::string msg;
      bool ok = _ServerEditionModule->removeCharacterFromSessionImpl(
          _SessionId, _CharId, msg);
      debugInfo(ok, msg);
    }
  }
  uint32 getCharId() const { return _CharId; }
...
public:
  CVerifyRingAccess(NLMISC::TTime taskDate,
                    CServerEditionModule *serverEditionModule,
                    TSessionId sessionId, TCharId charId)
      : CTask<NLMISC::TTime>(taskDate),
        _ServerEditionModule(serverEditionModule), _CharId(charId),
...
public:
  CTestIfMustWakeUpSession(CServerEditionModule *module, TSessionId sessionId,
                           TCharId ownerId)
...
public:
  ...
  void addKeyPolicy(const std::string &key, const std::string &value,
                    TPolicy policy);
  ...
  std::string getSignature(const std::string &md5,
...
void CKeysHolder::addKeyPolicy(const std::string &key, const std::string &value,
                               TPolicy policy) {
...
std::string CKeysHolder::getSignature(const std::string &md5,
                                      const std::string &key) const {
  if (md5.empty() && key.empty()) {
    return "";
  }
  ...
  std::string wantedSignature =
      NLMISC::CHashKeyMD5(
          NLMISC::getMD5((uint8 *)tmp.data(), (uint32)tmp.size()))
          .toString();
  return key + ":" + wantedSignature;
}

bool CKeysHolder::verifyKey(const std::string &md5,
...
void CIdRecycle::releaseId(uint32 id) {
  ...
  _UsedId.erase(used);
  // If Recycle id are bigger thant used id then remove it (int order to decrese
  // the size of container) eg; Used = { 2, 6} Recyled = {1,3, 4, 5} => if 6
  // removed => Used {2}, Recycled = {1}
  //
  uint32 maxUsed = 0;
  if (!_UsedId.empty()) {
  ...
  std::set<uint32>::iterator upper(_RecycledId.upper_bound(maxUsed)),
      last(_RecycledId.end());
...
//------------------------------------- <CEditionSession>
//---------------------------------------------------------------------------------------
void CEditionSession::touch() { DateSinceNoPlayer = 0; }

bool CEditionSession::isLoaded() const {

  return _SessionType == RSMGR::TSessionType::st_anim ||
         (_SessionType == RSMGR::TSessionType::st_edit && _Loaded);
}
...
void CEditionSession::getStartParamsWithDelay(CServerEditionModule *module,
                                              uint32 charId,
                                              TSessionId lastStoredSessionId) {
  _WaitingForConnectionTasks.addTask(
      new CTaskGetStartParamsWhenLoaded(module, charId, lastStoredSessionId));
}
...
uint32 CEditionSession::connectChar(TCharId charId, TUserRole connectedAs) {
  TCurrentChars::const_iterator first(_CurrentChars.begin()),
      last(_CurrentChars.end());
  for (; first != last; ++first) {
    if ((*first)->getCharId() == charId) {
...
void CEditionSession::swap(CEditionSession &other) {
  nldebug("pre  CEditionSession::swap() - ownerCharId=%u, aiInstanceId=%u, "
          "sessionId=%u",
          _OwnerCharId, _AiInstance, SessionId.asInt());
  ...
  nldebug("post CEditionSession::swap() - ownerCharId=%u, aiInstanceId=%u, "
          "sessionId=%u",
          _OwnerCharId, _AiInstance, SessionId.asInt());
}
...
CEditionSession::CEditionSession(RSMGR::TSessionType sessionType,
                                 TSessionId sessionId, TCharId ownerCharId,
                                 TAiInstanceId aiInstanceId,
                                 CScenario *scenario)
    : _Scenario(scenario), SessionId(sessionId), _AiInstance(aiInstanceId),
      _EditSlotIdMaker(1) {
  if (_Scenario.isNull()) {
  ...
  InitialSeason = 0;
  _Loaded = false;
  nldebug("Creating new CEditionSession - ownerCharId=%u, aiInstanceId=%u, "
          "sessionId=%u",
          ownerCharId, aiInstanceId, sessionId.asInt());
}

CEditionSession::CEditionSession(RSMGR::TSessionType sessionType,
                                 TSessionId sessionId, CEditionSession *edit)
...
CScenario *CEditionSession::getScenario() const { return _Scenario.getPtr(); }
...
TScenarioSessionType CEditionSession::getScenarioSessionType() const {
  ...
  case RSMGR::TSessionType::st_edit:
    return st_edit;
  case RSMGR::TSessionType::st_anim:
    return st_anim;
  default:
...
bool CEditionSession::mustClose(uint32 currentTime) const {
  if (TimeBeforeAutoCloseAnimationSessionWithNoPlayer == 0) {
    return false;
  }

  if (DateSinceNoPlayer == 0) {
    return false;
  } // There is still player connected

  if (!_Scenario.getPtr()) {
    return false;
  } // Must not arrived

  if (getSessionType() != RSMGR::TSessionType::st_anim) {
    return false;
  } // close only animation session

  if (currentTime >
      DateSinceNoPlayer + TimeBeforeAutoCloseAnimationSessionWithNoPlayer) {
    return true;
  }
  return false;
}
...
bool CEditionSession::mustSave(uint32 currentTime) const {
  ...
  if (TimeBeforeAutoSaveTimeEditionSession == 0) {
    return false;
  }
  ...
  if (_SessionType != RSMGR::TSessionType::st_edit) {
    return false;
  } // backup only editing session
  ...
  if (currentTime > _LastSaveTime + TimeBeforeAutoSaveTimeEditionSession &&
      !_Scenario->getClean()) {
    return true;
  }
  return false;
...
bool CEditionSession::mustStopAnimation(uint32 currentTime) const {
  if (TimeBeforeStopTestInEditionSessionWithNoPlayer == 0) {
    return false;
  }
  ...
  if (_SessionType != RSMGR::TSessionType::st_edit ||
      _Scenario->getMode() != 2) {
    return false;
  } // close only editing session in test mode

  if (currentTime >
      DateSinceNoPlayer + TimeBeforeStopTestInEditionSessionWithNoPlayer) {
    return true;
  }
  return false;
}
...
bool CEditionSession::mustHibernate(uint32 currentTime) const {
  ...
  if (_SessionType != RSMGR::TSessionType::st_edit) {
    return false;
  } // hibernate only editing session

  if (currentTime >
      DateSinceNoPlayer + TimeBeforeAutoHibernateEditionSessionWithNoPlayer) {
    return true;
  }
  return false;
}
...
void CEditionSession::serial(NLMISC::IStream &stream) {
  ...
  if (!stream.isReading()) // Writing
  {
    ...
    if (version >=
        2) // There was a "Cost" in needed prop of CNpcGrpFeatureSerializer
    {
  ...
  nldebug("CEditionSession::serial(%s) - ownerCharId=%u, aiInstanceId=%u, "
          "sessionId=%u",
          (stream.isReading() ? " IN" : "OUT"), _OwnerCharId, _AiInstance,
          SessionId.asInt());
}
...
CServerEditionModule::CPioneerInfo::~CPioneerInfo() { delete WaitingMsg; }
...
CServerEditionModule::~CServerEditionModule() {
  ...
  for (; first != last; ++first) {
    ...
    //_IdRecycle is already deleted so don't do
    //_IdRecycle->release(aiInstanceId);
    delete session;
  }
  _Sessions.clear();
...
public:
  COverrideRingAccessCallback(CServerEditionModule *editionModule)
      : _EditionModule(editionModule) {}

  // call back for bs file asynchronous read
  void callback(const CFileDescription &fileDescription,
                NLMISC::IStream &dataStream) {
    nldebug("BSIcb COverrideRingAccessCallback::callback() for file: %s",
            fileDescription.FileName.c_str());
  ...
  void parseStream(const CFileDescription &fileDescription,
                   NLMISC::IStream &dataStream,
                   CServerEditionModule::TOverrideRingAccess &access) {
    ...
    std::vector<std::string>::iterator first(entries.begin()),
        last(entries.end());
    ...
    for (; first != last; ++first) {
      ...
      if (firstLoop) {
        ...
        if (tokens.size() != 2 || tokens[0] != "Version" ||
          ...
          nlwarning("Obsolete File Discard '%s'",
                    fileDescription.FileName.c_str());
          return;
        }
      } else {
...
void CServerEditionModule::init(NLNET::IModuleSocket *gateway,
                                CDynamicMapService *server) {
  _BsGoingUp = true;
  _Server = server;
  _WaitingForBS = false;
  ...
  addKeyPolicy(0, "EMPTY", "EMPTY",
               AcceptScenarioWithoutHeader.get()
                   ? "ACCEPT"
                   : "REFUSE"); // Deprecated (scenario older than 22 juin 2006)
  ...
  addKeyPolicy(0, "EMPTY", "EMPTY", "REFUSE"); // Deprecated (scenario older
  than 22 juin 2006) addKeyPolicy(0, "PROD",
  "PROD:098085b10aa27157d57587464e3b2390 ", "TEST"); addKeyPolicy(0, "DEV",
  "DEV:26289fe9502f4db1fa3229e4fb2d8803", "TEST"); // accept current dev key
  addKeyPolicy(0, "DEVOBSOLETE", "DEVOBSOLETE:26289fe9502f4db1fa3229e4fb2d8803",
  "REFUSE");  // refuse obsolete dev key
  */
}
...
void CServerEditionModule::updateRSMGR() {
  ...
  vector<RSMGR::TRunningSessionInfo>::iterator runningSession(
      runningSessions.begin());

  {
    ...
    for (; first != last; ++first, ++runningSession) {
      ...
      runningSession->setNbPlayingChars(
          (uint32)first->second->getCurrentChars().size());
...
void CServerEditionModule::onModuleUp(NLNET::IModuleProxy *senderModuleProxy) {
  ...
  if (moduleName == "RingSessionManager") {
    ...
    for (; first != last; ++first) {
      ...
      CEditionSession::TCurrentChars::const_iterator f2(
          session->getCurrentChars().begin()),
          e2(session->getCurrentChars().end());
      for (; f2 != e2; ++f2) {
    ...
    bool ok = getCharInfo(senderModuleProxy, charId, clientEid, userPriv,
                          extendedPriv);

    simulateSU(clientEid);

    nldebug("R2Ed: onModuleUp( ClientEditionModule %u '%s' '%s'", charId,
            clientEid.toString().c_str(), userPriv.c_str());

    if (!ok) {
      nlwarning("R2Ed: client module '%s' try to connect but has Invalid "
                "Security Info",
    ...
    if (_ClientsEditionModule.getA(senderModuleProxy) != NULL) {
      nlwarning("R2Ed: The client edition module '%s' just got up but was "
                "already identified",
                senderModuleProxy->getModuleName().c_str());
    ...
    if (_ClientsEditionModule.getB(charId) != NULL) {
      nlwarning("R2Ed: The client edition module '%s' just got up but was "
                "already there. It means that one charId %u has 2 Modules... "
                "We can not tolerate this: so we kick him.",
                senderModuleProxy->getModuleName().c_str(), charId);
...
void CServerEditionModule::tryCharConnection(TCharId charId) {
  ...
  const NLNET::TModuleProxyPtr *senderModuleProxyPtr =
      getClientProxyPtr(charId);
  if (!senderModuleProxyPtr) {
    return;
  } // player disconnect before BS an EGS Up
  NLNET::IModuleProxy *senderModuleProxy = *senderModuleProxyPtr;
  bool ok = checkSecurityInfo(senderModuleProxy, charId, clientEid, userPriv,
                              extendedPriv);
  if (!ok) {
    return;
  }
  ...
  TPioneersSessionsAlloweds::iterator found(
      _PioneersSessionsAllowed.find(charId));

  if (found == _PioneersSessionsAllowed.end()) {
    nlwarning("R2Ed: The char %u is trying to connect but is not allowed so we "
              "was kicked",
              charId);
  ...
  if (!getSession(sessionId)) {
    nlwarning("R2Ed: The char %u is trying to connect to session %u but this "
              "session does not exist (he was kicked)",
...
void CServerEditionModule::connectChar(TSessionId sessionId, TCharId charId,
                                       TUserRole userRole,
                                       const std::string &ringAccess,
                                       bool newcomer) {
  const NLNET::TModuleProxyPtr *clientEditionProxyPtr =
      getClientProxyPtr(charId);
...

  {
    ...
    CMessage msg("HELLO");
    clientEditionProxy->sendModuleMessage(
        this->getAnimationModule()->getModule(), msg);
  }
  ...
  if (_SessionManager) {
    ...
    RSMGR::CRingSessionManagerProxy rsm(_SessionManager);
    rsm.reportSessionEvent(this, RSMGR::TSessionEvent::se_char_enter, sessionId,
                           charId);
  }
  ...
  */
  ...
  // If its a dev he has specials ring Acess
  {
    ...
    // Do not useeckSecurityInfo because we put the client in authorized list
    // here.
    bool ok = getCharInfo(clientEditionProxy, charId, clientEid, userPriv,
                          extendedPriv);
    if (ok) {
      ...
      std::vector<std::string>::const_iterator first(values.begin()),
          last(values.end());
      for (; first != last; ++first) {
        if (first->size() >= length && first->substr(0, length) == r2a) {

          std::string ra =
              toLowerAscii(CSString(first->substr(length)).replace("_", ":"));
...
void CServerEditionModule::disconnectChar(TCharId charId) {
  ...
  DROP_IF(_CharacterControlProxy.isNull(),
          "Try to disconnect a player but no EGS?", return);
  CCharacterControlItfProxy proxy(_CharacterControlProxy);
  proxy.returnToPreviousSession(this, charId);
  // proxy.disconnectChar(this, charId); // removed because prevents the client
  // from completing its FarTP to the previous session
  //  Kick the client out of game
}

void CServerEditionModule::createSessionWithoutSu(TCharId charId,
                                                  NLMISC::CEntityId clientEid) {
  // Simulate SU : createSession, put char in session, set the AIInstance of the
  // player
  bool mustCreateSession = AutoCreateSessionMultiChar || AutoCreateSession ||
                           !CDynamicMapService::getInstance()->useNetwork();
  TSessionId sessionId = TSessionId(0);
  if (mustCreateSession || AutoCreateAddCharacter) {

    nlwarning("Creating session without SU - this code is dangerous when "
              "running with SU!!!");
    ...
    if (mustCreateSession) {
      ...
      if (scenarioIt == _Sessions.end()) {
        createSession(_SessionManager, charId, sessionId,
                      AutoCreateAnimationSession
                          ? RSMGR::TSessionType::st_anim
                          : RSMGR::TSessionType::st_edit);
      }
    }
    ...
    addCharacterInSession(_SessionManager, sessionId, charId,
                          WS::TUserRole(AutoCreateRole), "", false);
  }
}
...
void CServerEditionModule::onModuleDown(NLNET::IModuleProxy *moduleProxy) {
  ...
  if (moduleName == "RingSessionManager") {
    ...
    if (_ClientsEditionModule.getA(moduleProxy) != NULL) {
      ...
      // here.
      bool ok =
          getCharInfo(moduleProxy, charId, clientEid, userPriv, extendedPriv);

      BOMB_IF(
          !ok,
          "R2Ed: User Module '"
              << moduleProxy->getModuleName()
              << "' is disconnecting but it does not have any security info",
          return);
      ...
      if (session) {
        ...
        if (!_SessionManager.isNull()) {
          RSMGR::CRingSessionManagerProxy rsm(_SessionManager);
          rsm.reportSessionEvent(this, RSMGR::TSessionEvent::se_char_leave,
                                 session->SessionId.asInt(), charId);
        }
        if (session->getSessionType() == RSMGR::TSessionType::st_anim &&
            _CharacterControlProxy) {
          CCharacterControlItfProxy proxy(_CharacterControlProxy);
      ...
      //			// Don't remove from _PioneersSessionsAllowed
      //otherwise joining a distinct session
      //			// on the same DSS will fail.
      //			TPioneersSessionsAlloweds::iterator
      //foundPioneersSessionsAllowed(_PioneersSessionsAllowed.find(userId)); 			if
      //(foundPioneersSessionsAllowed != _PioneersSessionsAllowed.end() )
      //			{
      //				_PioneersSessionsAllowed.erase(foundPioneersSessionsAllowed);
      //			}

      TPioneersSessions::iterator foundPioneersSessions(
          _PioneersSessions.find(charId));
...
void CServerEditionModule::rtScenarioUpdateRequested(
    NLNET::IModuleProxy *senderModuleProxy, uint32 paramCharId,
    CObject *rtScenario) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
...
void CServerEditionModule::onMessageReceivedCreatePrimitives(
    NLNET::IModuleProxy *senderModuleProxy, TCharId charId) {
  ...
  if (!rtData) {
    nlwarning(
        "R2Ed: char %u is trying to create primitives but there is no Rt Data",
        charId);
    return;
  }
...
void CServerEditionModule::stopTest(TSessionId sessionId) {
  ...
  if (!ok) {
    nlinfo("INF: The destruction of an CAnimationSession in the session %u was "
           "(delayed) ",
           sessionId.asInt());
  }
  ...
  CShareClientEditionItfProxy::broadcast_onTestModeDisconnected(
      broadcastList.begin(), broadcastList.end(), this, sessionId,
      lastCurrentActIndex, session->getScenarioSessionType());
}

void CServerEditionModule::stopTestRequested(
    NLNET::IModuleProxy *senderModuleProxy, TCharId charId) {
...
void CServerEditionModule::onMapConnectionAsked(
    NLNET::IModuleProxy *clientEditionProxy, TSessionId sessionId,
    bool updateHighLevel, bool mustTp, TUserRole role) {
  ...
  bool ok = checkSecurityInfo(clientEditionProxy, charId, clientEid, userPriv,
                              extendedPriv);
  if (!ok) {
    return;
  }
  ...
  TPioneersSessions::iterator previousSessionIdIt =
      _PioneersSessions.find(charId);
  if (previousSessionIdIt != _PioneersSessions.end()) {
    ...
    if (previousSessionId != sessionId) {
      ...
      nldebug("R2Ed: char %u that was connected to %u is connecting to %u",
              charId, previousSessionId.asInt(), sessionId.asInt());
      _PioneersSessions[charId] = sessionId;
      if (scenario ==
          previousSession
              ->getScenario()) { // Same HL data between an edition Session and
                                 // its linked animation session
        mustTp = false;
  ...
  if (clientEditionProxy) {
    ...
    if (sessionType == RSMGR::TSessionType::st_anim) {
      ...
      // Add charId connected in animation Module
      {
        ...
        animationSessionExist =
            getAnimationModule()->getConnectedChars(sessionId, connectedChars);
      }
    ...
    if (_CharacterControlProxy && previousSessionId.asInt() != 0 &&
        previousSessionId != sessionId) {
      double x, y, orient;
      uint8 season;
      _Server->getAdminModule()->getPosition(sessionId, x, y, orient, season,
                                             clientConnectBody.InitialActIndex);
      CFarPosition pos;
      ...
      CCharacterControlItfProxy proxy(_CharacterControlProxy);
      proxy.setUserCharCurrentSession(this, charId, previousSessionId, pos,
                                      role);
    }
...
void CServerEditionModule::advConnACK(NLNET::IModuleProxy *senderModuleProxy) {
  ...
  if (!_PioneersInfo[charId].WaitingTp)
    return;
  _PioneersInfo[charId].WaitingTp = false;
}

void CServerEditionModule::broadcastToCharConnected(const NLNET::CMessage &msg,
                                                    CEditionSession *session) {
  nlassert(session);

  // Warning : do not set this vector 'static' because it will hold smart
  // pointer on module
  vector<NLNET::TModuleProxyPtr> broadcastList;

  fillBroadcastVector(session, broadcastList);

  vector<NLNET::TModuleProxyPtr>::iterator first(broadcastList.begin()),
      last(broadcastList.end());
  for (; first != last; ++first) {
...
void CServerEditionModule::fillBroadcastVector(
    CEditionSession *session,
    std::vector<NLNET::TModuleProxyPtr> &broadcastList) {
  nlassert(session);

  // cleanup the list
  broadcastList.clear();

  CEditionSession::TCurrentChars::const_iterator first(
      session->getCurrentChars().begin()),
      last(session->getCurrentChars().end());
  for (; first != last; ++first) {
    TCharId charId = (*first)->getCharId();
    const NLNET::TModuleProxyPtr *ptrClientEditionProxyPtr =
        getClientProxyPtr(charId);
...
void CServerEditionModule::fillBroadcastVectorNoSender(
    CEditionSession *session,
    std::vector<NLNET::TModuleProxyPtr> &broadcastList,
  ...
  for (; first != last; ++first) {
    ...
    TModuleProxyPtr clientEditionProxy = *ptrClientEditionProxyPtr;
    if (!senderModuleProxy || senderModuleProxy->getModuleProxyId() !=
                                  clientEditionProxy->getModuleProxyId()) {
      broadcastList.push_back(clientEditionProxy);
    }
  }
}

void CServerEditionModule::replyToAll(NLNET::IModuleProxy *senderModuleProxy,
                                      const NLNET::CMessage &msg) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
...
std::vector<NLNET::TModuleProxyPtr> CServerEditionModule::removeSender(
    const std::vector<NLNET::TModuleProxyPtr> &allModule,
    NLNET::IModuleProxy *senderModuleProxy) {
  std::vector<NLNET::TModuleProxyPtr> ret;
  vector<TModuleProxyPtr>::const_iterator first(allModule.begin()),
      last(allModule.end());
  for (; first != last; ++first) {
    if (!senderModuleProxy ||
        (*first)->getModuleProxyId() != senderModuleProxy->getModuleProxyId()) {
      ret.push_back(*first);
    }
  }
  return ret;
...
void CServerEditionModule::onScenarioUploadAsked(
    NLNET::IModuleProxy *senderModuleProxy, uint32 msgId,
  ...
  if (found == _PioneersSessions.end()) {
    nlwarning("R2Ed: char %u is trying to create a scenario but he is not in "
              "an instance",
              charId);
    sender.ackMsg(this, msgId, false);
    return;
  ...
  if (mustBroadCast) {
    ...
    // no need  to send the same msg that the sender has sent. Just send an ack
    // OK.
    sender.ackMsg(this, msgId, true);

    // Send the update message to all client except the one that send intial
    // message
    vector<TModuleProxyPtr> otherThantSender =
        removeSender(broadcastList, senderModuleProxy);

    // Warning : Very BAD reinterpret cast due to necessity to convert a server
    // to client object serial. This should not harm because the two objects are
    // completely similar.
    CShareClientEditionItfProxy::broadcast_onScenarioUploaded(
        otherThantSender.begin(), otherThantSender.end(), this,
        reinterpret_cast<const CObjectSerializerClient &>(hlScenario));

    vector<TModuleProxyPtr>::const_iterator first(broadcastList.begin()),
        last(broadcastList.end());
    for (; first != last; ++first) {
...
void CServerEditionModule::onNodeInsertAsked(
    NLNET::IModuleProxy *senderModuleProxy, uint32 messageId,
  ...
  CShareClientEditionItfProxy sender(senderModuleProxy);
  if (!session) {
    nlwarning("R2Ed: char %u insert node an empty scenario %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }

  ok = session->getScenario()->insertNode(instanceId, attrName, position, key,
                                          value);
  if (!ok) {
  ...
  // Warning : Very BAD reinterpret cast due to necessity to convert a server to
  // client object serial. This should not harm because the two objects are
  // completely similar.
  CShareClientEditionItfProxy::broadcast_onNodeInserted(
      broadcastList.begin(), broadcastList.end(), this, instanceId, attrName,
      position, key, reinterpret_cast<const CObjectSerializerClient &>(value2));
}

void CServerEditionModule::onNodeSetAsked(
    NLNET::IModuleProxy *senderModuleProxy, uint32 messageId,
    const std::string &instanceId, const std::string &attrName,
    const CObjectSerializerServer &value2) {
  ...
  CShareClientEditionItfProxy sender(senderModuleProxy);
  if (!session) {
    nlwarning("R2Ed: char %u insert node an empty session %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }
  ...
  if (!ok) {
    nlwarning("RE2D: 'setNode' refused char %u, session %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }
  ...
  sender.ackMsg(this, messageId, true);
  fillBroadcastVectorNoSender(session, broadcastList, senderModuleProxy);
  CShareClientEditionItfProxy::broadcast_onNodeSet(
      broadcastList.begin(), broadcastList.end(), this, instanceId, attrName,
      reinterpret_cast<const CObjectSerializerClient &>(value2));
}

void CServerEditionModule::onNodeEraseAsked(
    NLNET::IModuleProxy *senderModuleProxy, uint32 messageId,
    const std::string &instanceId, const std::string &attrName,
    sint32 position) {
  ...
  if (!ok) {
    nlwarning("RE2D: 'eraseNode' refused char %u, session %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }
  ...
  fillBroadcastVectorNoSender(session, broadcastList, senderModuleProxy);
  sender.ackMsg(this, messageId, true);
  CShareClientEditionItfProxy::broadcast_onNodeErased(
      broadcastList.begin(), broadcastList.end(), this, instanceId, attrName,
      position);
}

void CServerEditionModule::onNodeMoveAsked(
    NLNET::IModuleProxy *senderModuleProxy, uint32 messageId,
  ...
  CShareClientEditionItfProxy sender(senderModuleProxy);
  if (!session) {
    nlwarning("R2Ed: char %u move node an empty scenario %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }
  ...
  ok = scenario->moveNode(instanceId1, attrName1, position1, instanceId2,
                          attrName2, position2);

  if (!ok) {
    nlwarning("RE2D: 'moveNode' refused char %u, session %u", charId,
              sessionId.asInt());
    sender.ackMsg(this, messageId, false);
    return;
  }
  ...
  fillBroadcastVectorNoSender(session, broadcastList, senderModuleProxy);
  sender.ackMsg(this, messageId, true);
  CShareClientEditionItfProxy::broadcast_onNodeMoved(
      broadcastList.begin(), broadcastList.end(), this, instanceId1, attrName1,
      position1, instanceId2, attrName2, position2);
}

void CServerEditionModule::registerSession(TSessionId sessionId,
                                           CEditionSession *session) {
...
void CServerEditionModule::onUserComponentUploaded(
  ...
  if (!session) {
    delete component;
    nlwarning("R2Ed: char %u mupload component on an empty session %u", charId,
              sessionId.asInt());
    return;
  }
  CScenario *scenario = session->getScenario();
  ...
  TUserComponentRefCounter::const_iterator found =
      _UserComponentRefCounter.find(component->Md5);
  ...
  vector<TModuleProxyPtr> broadcastList;
  fillBroadcastVector(session, broadcastList);
  CShareClientEditionItfProxy::broadcast_onUserComponentRegistered(
      broadcastList.begin(), broadcastList.end(), this, component->Md5);
}

CUserComponent *
CServerEditionModule::getUserComponent(const NLMISC::CHashKeyMD5 &md5) const {

  TUserComponentRefCounter::const_iterator found =
      _UserComponentRefCounter.find(md5);
  if (found != _UserComponentRefCounter.end()) {
...
void CServerEditionModule::onUserComponentDownloading(
    NLNET::IModuleProxy *senderModuleProxy, const NLMISC::CHashKeyMD5 &md5) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
...
void CServerEditionModule::onUserComponentRegistered(
    NLNET::IModuleProxy *senderModuleProxy, const NLMISC::CHashKeyMD5 &md5) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  ...
  if (!session) {
    nlwarning(
        "R2Ed: char %u try to register user component on a empty session %u",
        charId, sessionId.asInt());
    return;
  }
  ...
  fillBroadcastVector(session, broadcastList);
  CShareClientEditionItfProxy::broadcast_onUserComponentRegistered(
      broadcastList.begin(), broadcastList.end(), this, md5);
}

bool CServerEditionModule::onProcessModuleMessage(
    NLNET::IModuleProxy *senderModuleProxy, const NLNET::CMessage &message) {
  ...
  if (senderModuleProxy->getModuleClassName() == "ClientEditionModule") {
    ...
    if (operationName == "requestUpdateRtScenario") {
      ...
      bool ok = checkSecurityInfo(senderModuleProxy, charId, clientEid,
                                  userPriv, extendedPriv);
      if (!ok) {
        return true;
      }
...
TSessionId CServerEditionModule::getSessionIdByCharId(TCharId charId) const {
  {
    TPioneersSessionsAlloweds::const_iterator found =
        _PioneersSessionsAllowed.find(charId);
    if (found != _PioneersSessionsAllowed.end())
      return found->second.SessionId;
  }
  {
    TPioneersSessions /*std::map<TCharId, TSessionId>*/ ::const_iterator found =
        _PioneersSessions.find(charId);
    if (found != _PioneersSessions.end())
      return found->second;
  }
...
TPioneersSessionsAllowed *
CServerEditionModule::getSessionAllowedForChar(TCharId charId) const {
  TPioneersSessionsAlloweds::const_iterator found =
      _PioneersSessionsAllowed.find(charId);
  if (found == _PioneersSessionsAllowed.end())
    return NULL;
...
CEditionSession *
CServerEditionModule::getSessionByCharId(TCharId charId) const {
  TSessionId sessionId = getSessionIdByCharId(charId);
  if (sessionId.asInt() == 0)
    return 0;
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, createSession) {
  ...
  log.displayNL("Creating session %u %s", sessionId.asInt(),
...
void CServerEditionModule::createSession(NLNET::IModuleProxy *sender,
                                         TCharId ownerCharId,
                                         TSessionId sessionId,
                                         const RSMGR::TSessionType &type) {
  // todo
  //  create a new session
  //  sender == NULL <=> test the command

  nldebug("R2Ed: createSession char %u, session %u, %u", ownerCharId,
          sessionId.asInt(), static_cast<uint32>(type.getValue()));
  // create an aiInstanceId

  if (_Sessions.find(sessionId) != _Sessions.end()) {
    nlwarning("R2Ed: trying to create an already created scenario %u (use the "
              "previous one)",
              sessionId.asInt());
    return;
  ...
  if (type.getValue() == RSMGR::TSessionType::st_edit) {
    ...
    registerSession(sessionId,
                    new CEditionSession(RSMGR::TSessionType::st_edit, sessionId,
                                        ownerCharId, aiInstanceId, scenario));
    ...
    if (CDynamicMapService::getInstance()->useNetwork()) {
      BsiGlobal.requestFileClass(
          "r2/sessions/", classes,
          new CTestIfMustWakeUpSession(this, sessionId, ownerCharId));
    } else {
      setLoaded(sessionId);
    }
    ...
    {
      _OwnerInfos.insert(std::make_pair(
          ownerCharId, TOwnerInfo(sessionId, TSessionId(0),
                                  aiInstanceId))); // create an edition Session
    } else {
    ...
    if (owner == _OwnerInfos.end()) // launch a animation from the born
    {
      owner = _OwnerInfos
                  .insert(std::make_pair(
                      ownerCharId,
                      TOwnerInfo(TSessionId(0), sessionId, aiInstanceId)))
                  .first; // create an edition Session
    } else {
    ...
    if (!editSession) {
      aiInstanceId = _IdRecycle->newId();
      nldebug("Session %u create AiInstance %u", sessionId.asInt(),
              aiInstanceId);
      CAiWrapper::getInstance().startInstance(sessionId, aiInstanceId);

      CScenario *scenario = new CScenario(0, st_anim);
      scenario->setMode(0);
      registerSession(sessionId, new CEditionSession(
                                     RSMGR::TSessionType::st_anim, sessionId,
                                     ownerCharId, aiInstanceId, scenario));
    } else {
      registerSession(sessionId,
                      new CEditionSession(RSMGR::TSessionType::st_anim,
  ...
  if (type.getValue() == RSMGR::TSessionType::st_anim && linkedSession) {
    ...
    if (!ok) {
      nlwarning(
          "Session%u, the linked session %u does not have any animation data",
          getLinkedSessionId(sessionId).asInt(), sessionId.asInt());
    } else {
      TRunningScenarioInfo info;
      TScenarioHeaderSerializer::TValueType::const_iterator first(
          values.begin()),
          last(values.end());
      ...
      for (; first != last; ++first) {
        ...
        else if (first->first == "Title") {
          info.setScenarioTitle(first->second);
        } else if (first->first == "ShortDescription") {
          info.setScenarioDesc(first->second);
        ...
        } else if (first->first == "CreateBy") {
          info.setScenarioAuthorName(first->second);
        } else if (first->first == "Rules") {
          info.setDMLess(first->second != "Mastered");
        } else if (first->first == "NevraxScenario" && first->second == "1") {
          nevraxScenario = true;
        } else if (first->first == "TrialAllowed" && first->second == "1") {
          trialAllowed = true;
        } else if (first->first == "MissionTag" && !first->second.empty()) {
          missionTag = first->second;
        }
      }
      ...
      const NLNET::TModuleProxyPtr *ownerModule =
          getClientProxyPtr(ownerCharId);
      ...
      if (!trialAllowed && getIsCharRoS(ownerCharId) &&
          extendedPriv.find("TRIAL") != std::string::npos) {
        nlwarning(
            "ERROR: A client %u(CharId) (RoS) try to launch an animation that "
            "is not RoS. That means that the client has not check the RoS "
            "Flags or that this user is trying to hack use. So we will close "
            "he animation session (%u) so he will be kicked back to edition.",
            ownerCharId, sessionId.asInt());
        return;
      }
      ...
      if (_CharacterControlProxy) {
        ...
        // ie the animator and player that just arrived before the scenario has
        // uploaded
        CEditionSession::TCurrentChars::const_iterator first(
            session->getCurrentChars().begin()),
            last(session->getCurrentChars().end());
      ...
      if (_SessionManager) {
        ...
        nldebug("Scenario Started %d %s", sessionId.asInt(),
                info.getScenarioKey().toString().c_str());
      }
    }
  }
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, addCharacterInSession) {
  if (args.size() < 3 || args.size() > 5) {
    log.displayNL("addCharacterInSession.addCharacterInSession <sessionId> "
                  "<clientId> <roleAsString> <[ringAcess]> <[isnewcomer]>");
    return false;
  }
  uint32 id;
  ...
  this->addCharacterInSession(0, sessionId, charId, userRole, ringAccess,
                              newcomer);

  return true;
}

void CServerEditionModule::addCharacterInSession(
    NLNET::IModuleProxy *sender, TSessionId sessionId, TCharId charId,
    const WS::TUserRole &connectedAs, const std::string &ringAccess,
    bool newcomer) {

  nldebug("R2Ed: addCharacterInSession %u %u %s", sessionId.asInt(), charId,
          connectedAs.toString().c_str());
  ...
  TUserRole role;
  switch (connectedAs.getValue()) // WS::TUserRole and R2::TUserRole are
                                  // distinct because WS is in NeLNS
  {
  case WS::TUserRole::ur_animator:
    role = R2::TUserRole::ur_animator;
    break;
  case WS::TUserRole::ur_editor:
    role = R2::TUserRole::ur_editor;
    break;
  case WS::TUserRole::ur_player:
    role = R2::TUserRole::ur_player;
    break;
  ...
  std::string upgradedRingAccess = CRingAccess::getInstance().upgradeRingAccess(
      toLowerAscii(DefaultCharRingAccess.get()), toLowerAscii(ringAccess));
  ...
  // Append only if pioneer is already in a session (like edition/test) and
  // start an animation
  const NLNET::TModuleProxyPtr *clientEditionProxyPtr =
      getClientProxyPtr(charId);
  if (clientEditionProxyPtr) {
...
void CServerEditionModule::resetSession(NLNET::IModuleProxy *sender,
                                        TSessionId sessionId, bool reconnect) {
...
bool CServerEditionModule::closeSessionImpl(TSessionId sessionId,
                                            std::string &msg) {
  //(*) remove player from connecting queue
  // This very small map contains player that are connecting to dss (allowed by
  // SU to connect but not yet connected because they are loading data) so its
  // ok to iterate
  ...
  if (_Sessions.find(sessionId) != _Sessions.end()) {
    ...
    msg += toString("No session (running or hiberning) with id %u",
                    sessionId.asInt());
    ret = false;
  }

  TPioneersSessionsAlloweds::iterator first(_PioneersSessionsAllowed.begin()),
      last(_PioneersSessionsAllowed.end());
  for (; first != last;) {
  ...
  if (session) {
    ...
    if (it != _OwnerInfos.end()) {
      if (session->getSessionType() == RSMGR::TSessionType::st_edit) {
        ...
        if (it->second.EditionSessionId && _CharacterControlProxy) {
          CCharacterControlItfProxy proxy(_CharacterControlProxy);
          proxy.reportUnlinkedSession(this, it->second.EditionSessionId,
                                      oldAnimationSession);
        }
      }
      if (it->second.EditionSessionId == TSessionId(0) &&
          it->second.AnimationSessionId == TSessionId(0)) {
    ...
    const CEditionSession::TCurrentChars &charIdContainer =
        session->getCurrentChars();
    CEditionSession::TCurrentChars::const_iterator first(
        charIdContainer.begin()),
        last(charIdContainer.end());
    for (; first != last; ++first) {
  ...
  bool animationSessionExist;
  {
    ...
    animationSessionExist = getAnimationModule()->getConnectedChars(
        linkedSessionId, connectedChars);
  }
...

  {
    ...
    if (found != _Sessions.end()) {
      ...
      if (session && mustStopAiInstance) {
        ...
        nldebug("Session %u destroy AiInstance %u", sessionId.asInt(),
                aiInstanceId);
  ...
  if (DssUseBs) {
    ...
    if (ownerCharId == 0) {
      nlwarning("Try to close a session that was not opened %u",
                sessionId.asInt());
    } else {
  ...
  if (_SessionManager) {
    ...
    rsm.reportSessionEvent(this, RSMGR::TSessionEvent::se_session_closing,
                           sessionId, 0);
  }
...
void CServerEditionModule::closeSession(NLNET::IModuleProxy *sender,
                                        TSessionId sessionId) {
  // #pragma message (NL_LOC_WRN "TODO : Vianney : remove hibernation file for
...
void CServerEditionModule::stopHibernation(NLNET::IModuleProxy *sender,
                                           TSessionId sessionId,
                                           uint32 charId) {
  // #pragma message (NL_LOC_WRN "TODO : Vianney : remove hibernation file for
  // the specified session")
  BsiGlobal.deleteFile(std::string("r2/sessions/") +
                           getSessionFilename(sessionId, charId),
                       false);
}

void CServerEditionModule::hibernateSession(NLNET::IModuleProxy *sender,
                                            TSessionId sessionId) {
...
std::string CServerEditionModule::getSessionFilename(TSessionId sessionId,
                                                     TCharId charId) const {
  return NLMISC::toString("r2_session_%.08u_%08u.dat", sessionId.asInt(),
                          charId);
}
...
bool CServerEditionModule::unkickPioneerImpl(TSessionId sessionId,
                                             TCharId charId,
                                             std::string &outMsg) {

  // The web/dm has asked to session manager to unkick a character from a
  // session,

  outMsg =
      NLMISC::toString("Trying to unckick the character %u from session %u",
                       charId, sessionId.asInt());

  TPioneersSessions::iterator foundPioneerSession =
      _PioneersSessions.find(charId);
  ...
  if (foundPioneerSession != _PioneersSessions.end()) {
    ...
    if (connectedSessionId != sessionId) {
      outMsg += NLMISC::toString(
          "Refuse to unkick the character %u from session %u because he is "
          "connected to another session %u",
          charId, sessionId.asInt(), connectedSessionId.asInt());
      return false;
    }

    outMsg += NLMISC::toString("Send a message to character %u from session %u "
                               "to indicates that he is no more banned",
                               charId, sessionId.asInt());
    ...
    for (; first != last;) {
      ...
      CKickPlayerIfStillConnected *t =
          dynamic_cast<CKickPlayerIfStillConnected *>(task);
    ...
    // here the client is first disconnected, then firewalled again an finally
    // out of the game
    return true;
  }
    ...
    outMsg += NLMISC::toString(
        "R2Ed: The character %u was unkick from session %u (but because he is "
        "not connected we have nothing to do)",
        charId, sessionId.asInt());
    return true;
  }
...
// implementation of characterKicked and command
bool CServerEditionModule::kickPioneerImpl(TSessionId sessionId, TCharId charId,
                                           std::string &outMsg) {
  ...
  outMsg = NLMISC::toString("Trying to kick the character %u from session %u",
                            charId, sessionId.asInt());
  ...
  if (foundPioneerSession != _PioneersSessions.end()) {
    ...
    if (connectedSessionId != sessionId) {
      outMsg += NLMISC::toString(
          "Refuse to kick the character %u from session %u because he is "
          "connected to another session %u",
          charId, sessionId.asInt(), connectedSessionId.asInt());
      return false;
    }
    ...
    outMsg += NLMISC::toString("Send a message to character %u from session %u "
                               "to ask him to go away or he will be kicked",
                               charId, sessionId.asInt());
    ...
    NLMISC::TTime now = NLMISC::CTime::getLocalTime();
    _Tasks.addTask(new CKickPlayerIfStillConnected(
        now + 1000 * TimeBeforeDisconnectionAfterKick, this, sessionId,
        charId));
    // here the client is first disconnected, then firewalled again an finally
    // out of the game
    return true;
    ...
    TPioneersSessionsAlloweds::iterator allowedChar =
        _PioneersSessionsAllowed.find(charId);
    if (allowedChar == _PioneersSessionsAllowed.end()) {
      // no player are in the waiting for connection of this session

      outMsg += NLMISC::toString("Refuse to kick the character %u from session "
                                 "%u because he is offline",
                                 charId, sessionId.asInt());
    ...
    if (allowedChar->second.SessionId != sessionId) {
      outMsg += NLMISC::toString(
          "R2Ed: Refuse to kick the character %u from session %u because he is "
          "connecting to another session %u",
    ...
    outMsg += NLMISC::toString("R2Ed: The character %u was kick from session "
                               "%u while he was connecting",
                               charId, sessionId.asInt());
    return true;
  }
...
void CServerEditionModule::verifyRingAccess(TSessionId sessionId,
                                            TCharId charId) {
  CEditionSession *session = getSession(sessionId);
  if (!session) {
    return;
  ...
  if (found != _PioneersInfo.end()) {
    if (!CRingAccess::getInstance().verifyRight(session->RingAccess,
                                                found->second.RingAccess)) {
      resetSession(0, sessionId, false); // TODO kick if invited, reset if owner
    }
  }
}

bool CServerEditionModule::removeCharacterFromSessionImpl(TSessionId sessionId,
                                                          TCharId charId,
                                                          std::string &outMsg) {
  ...
  outMsg = NLMISC::toString("Trying to remove the character %u from session %u",
                            charId, sessionId.asInt());
  ...
  if (foundPioneerSession != _PioneersSessions.end()) {
    ...
    if (connectedSessionId != sessionId) {
      outMsg += NLMISC::toString(
          "Refuse to remove the character %u from session %u because he is "
          "connected to another session %u",
          charId, sessionId.asInt(), connectedSessionId.asInt());
      return false;
    }
    ...
    // disconnect the client
    disconnectChar(charId);
    // here the client is first disconnected, then firewaled again an finally
    // out of the game
    return true;
  }
    ...
    if (allowedChar == _PioneersSessionsAllowed.end()) {
      ...
      outMsg += NLMISC::toString("Refuse to remove the character %u from "
                                 "session %u because he is offline",
                                 charId, sessionId.asInt());
    ...
    if (allowedChar->second.SessionId != sessionId) {
      outMsg += NLMISC::toString(
          "R2Ed: Refuse to remove the character %u from session %u because he "
          "is connecting to another session %u",
          charId, sessionId.asInt(), allowedChar->second.SessionId.asInt());
    ...
    outMsg += NLMISC::toString("R2Ed: The character %u was remove from session "
                               "%u while he was connecting",
                               charId, sessionId.asInt());
...
void CServerEditionModule::characterKicked(NLNET::IModuleProxy *sender,
                                           TSessionId sessionId,
                                           TCharId charId) {
  // sender is SU
...
void CServerEditionModule::characterUnkicked(NLNET::IModuleProxy *sender,
                                             TSessionId sessionId,
                                             TCharId charId) {
  // sender is SU
...
// that a character has been kicked by the web
void CServerEditionModule::teleportOneCharacterToAnother(
    NLNET::IModuleProxy *sender, TSessionId sessionId, TCharId source,
    TCharId dest) {
  // sender is SU
...
bool CServerEditionModule::teleportOneCharacterToAnotherImpl(
    TSessionId sessionId, TCharId source, TCharId dest, std::string &msg) {
  CEditionSession *session = getSession(sessionId);
  ...
  if (!session || !scenario) {
    msg = toString("Try to teleport a character %u to another character %u but "
                   "in a session  %u that do not exist",
                   source, dest, session);
    return false;
  }
  ...
  bool sourceCharOk =
      (session->getCharRole(source) != TUserRole::invalid && srcClient);
  bool destCharOk =
      (session->getCharRole(dest) != TUserRole::invalid && destClient);

  if (!sourceCharOk || !destCharOk) {
    msg = toString(
        "Try to teleport a character %u %sto another character %u %sin the "
        "session  %u",
        source,
        sourceCharOk ? "" : "that is not present in the current session ", dest,
        destCharOk ? "" : "that is not present in the current session ",
        session);

    if (srcClient) {
      CShareClientEditionItfProxy proxy(*srcClient);
      proxy.systemMsg(this, "BC", "",
                      "uiR2EDTryToTeleportToAnInvalidCharacter");
    }
    /*
  ...
  // TSessionId sessionId = this->getSessionIdByCharId(charId);
  {
    ...
    this->_Server->getAdminModule()->getPosition(sessionId, sx, sy, sorient,
                                                 season);
  }

  proxy.teleportOneCharacterToAnother(this, source, dest, season);

  msg = NLMISC::toString("teleportCharacter %u %u in session(%u) at season(%u)",
                         source, dest, sessionId.asInt(), season);
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, teleportOneCharacterToAnother) {
  ...
  if (args.size() != 3) {
    log.displayNL("Wrong parameters CServerEditionModule.teleportPioneer "
                  "<sessionId> <sourceCharId> <destCharId>");
    return false;
  }
  ...
  std::string msg;
  teleportOneCharacterToAnotherImpl((TSessionId)sessionId, sourceCharId,
                                    destCharId, msg);
      ...
      if (npcs && npcs->isTable()) {
        ...
        if (i == 0) {
          ...
          if (baseActCost + max2 > MaxNpcs + MaxStaticObjects + 5) {
            nlwarning("R2Ed: checkScenario: too many npcs: %u",
                      (baseActCost + max2));
            return false;
          }
...
void CServerEditionModule::startingScenario(
    NLNET::IModuleProxy *senderModuleProxy) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  ...
  vector<TModuleProxyPtr> broadcastList;
  fillBroadcastVector(session, broadcastList);
  CShareClientEditionItfProxy::broadcast_startingScenario(
      broadcastList.begin(), broadcastList.end(), this, charId);
}

void CServerEditionModule::startScenario(
    NLNET::IModuleProxy *senderModuleProxy, bool ok,
    const TScenarioHeaderSerializer &header,
    const CObjectSerializerServer &data, uint32 startingAct) {
  ...
  if (!checkSecurityInfo(senderModuleProxy, charId, clientEid, userPriv,
                         extendedPriv)) {
    return;
  }
  ...
  if (MustVerifyRingAccessWhileLoadingAnimation && ok) {
    ...
    if (!CRingAccess::getInstance().verifyRtScenario(rtData, charAccess, err)) {
      nlwarning(
          "R2Ed: char %u try to run a scenario with too high level content. ",
          charId);
      if (err) {
  ...
  if (ok) {
    ...
    CScenario *scenario = session ? session->getScenario() : 0;
    if (!scenario) {
      nlwarning("R2Ed: char %u is trying to act on an invalid scenario",
                charId);
      delete rtData;
      return;
    }
    ...
    TPioneersSessions /*std::map<uint32, uint32>*/ ::const_iterator
        sessionIdIt = _PioneersSessions.find(charId);
...

    {
      ...
      const CEditionSession::TCurrentChars &animatorCharId =
          session->getCurrentChars();
      CEditionSession::TCurrentChars::const_iterator first(
          animatorCharId.begin()),
          last(animatorCharId.end());
      for (; first != last; ++first) {
      ...
      if (session->getEditSlotId(charId) == 0) {
        nlwarning("R2Ed: char %u is not connected to scenario %u but wants to "
                  "do a start Test.",
                  charId, body.SessionId.asInt());
        return;
      }

      if (!rtData) {
        nlwarning("R2Ed: char %u is connected to scenario %u but wants to do a "
                  "start Test with no rt Data.",
                  charId, body.SessionId.asInt());
        delete rtData;
        return;
      }
      ...
      if (getLinkedSessionId(body.SessionId) == TSessionId(0) &&
          session->getSessionType() == RSMGR::TSessionType::st_anim) {
        TRunningScenarioInfo info;
        TScenarioHeaderSerializer::TValueType::const_iterator first(
            header.Value.begin()),
            last(header.Value.end());
        for (; first != last; ++first) {
          if (first->first == "BodyMD5") {
            CHashKeyMD5 md5;
            md5.fromString(first->second);
        ...
        if (isCharRoS) {
          if (extendedPriv.find("TRIAL") != std::string::npos &&
              !trialAllowed) {
            errorReason = "uiR2EDRoSTryMainland";
            ok = false;
            nlwarning(
                "ERROR: A client %u(CharId) (RoS) try to launch an animation "
                "(from the born) that is not RoS. That means that the client "
                "has not check the RoS Flags or that this user is trying to "
                "hack use. So we will close he animation session (%u) so he "
                "will be kicked back to edition.",
                charId, body.SessionId.asInt());
          }
        }
        ...
        if (ok && _CharacterControlProxy) {
          ...
          BOMB_IF(!session, "ERROR: session must exist", return);
          // ie the animator and player that just arrived before the scenario
          // has uploaded
          CEditionSession::TCurrentChars::const_iterator first(
              session->getCurrentChars().begin()),
              last(session->getCurrentChars().end());
          for (; first != last; ++first) {
            proxy.charJoinAnimSession(this, (*first)->getCharId(),
                                      body.SessionId);
          }
        }
        ...
        if (_SessionManager) {
          ...
          nldebug("Scenario Started %d %s", body.SessionId.asInt(),
                  info.getScenarioKey().toString().c_str());
        }
      }
    }
  ...
  CMessage message;
  replyToAll(senderModuleProxy,
             CShareClientEditionItfProxy::buildMessageFor_startScenario(
                 message, ok, startingAct, errorReason));

  if (ok) {
    _Tasks.addTask(
        new CTaskUpdateScenarioVision(0, this, getSessionIdByCharId(charId)));
  }
}
...
bool CServerEditionModule::getIsCharRoS(TCharId charId) const {
  ...
  if (info == _PioneersInfo.end()) {
    nlwarning("ERROR: the code is looking to see if a character(%u/%u) is a "
              "newcomer, but the character is not log yet",
              charId, (charId >> 4));
    return false;
  }
...
void CServerEditionModule::updateScenarioVision(TSessionId sessionId) {
  ...
  // Reinitialise the Vision He would be smarter to use an new message where whe
  // serialize the list of co editor
  const CEditionSession::TCurrentChars &charIds = session->getCurrentChars();
  CEditionSession::TCurrentChars::const_iterator first(charIds.begin()),
      last(charIds.end());
  for (; first != last; ++first) {
    TCharId charId = (*first)->getCharId();
...
bool CServerEditionModule::getPosition(TSessionId sessionId, double &x,
                                       double &y, double &orient, uint8 &season,
                                       uint32 actIndex) {
  x = 0;
  y = 0;
  orient = 0;
  ...
  BOMB_IF(!locations || !locations->isTable(), "Invalid location.",
          return false);
  ...
  BOMB_IF(!acts || !acts->isTable() || actIndex >= acts->getSize(),
          "Invalid acts.", return false);

  CObject *selectedAct = acts->getValueAtPos(actIndex);
  BOMB_IF(!selectedAct || !selectedAct->isString("LocationId"), "Invalid act.",
          return false);
  ...
  for (; firstLocationIndex != lastLocationIndex; ++firstLocationIndex) {
    ...
    if (location && location->isString("InstanceId") &&
        location->toString("InstanceId") == locationInstanceId) {
      locationId = firstLocationIndex;
      break;
    }
  }

  BOMB_IF(locationId >= locations->getSize(), "Invalid location.",
          return false);

  CObject *firstLocation = locations->getValueAtPos(locationId);
  BOMB_IF(!firstLocation || !firstLocation->isTable(), "Invalid location.",
          return false);
  ...
  bool locationInfo = !entryPointObj || !islandName || !seasonObj ||
                      islandName->isString() || !islandName->isString() ||
                      !seasonObj->isString();
  ...
  CScenarioEntryPoints::CCompleteIsland *island =
      epManager.getIslandFromId(islandName->toString());
  BOMB_IF(!island,
          NLMISC::toString("Session %u No Island '%s'.", sessionId.asInt(),
                           islandName->toString().c_str()),
          return false);

  CScenarioEntryPoints::CShortEntryPoint *entryPoint =
      epManager.getEntryPointFromIds(islandName->toString(),
                                     entryPointObj->toString());

  if (!entryPoint) {
    entryPoint = epManager.getEntryPointFromIds(
        islandName->toString(),
        island->EntryPoints[0].Location); // Evil Hack must be removed
  }

  BOMB_IF(!entryPoint,
          NLMISC::toString("Session %u: No EntryPoint '%s' '%s'.",
                           sessionId.asInt(), islandName->toString().c_str(),
                           entryPointObj->toString().c_str()),
          return false);

  x = entryPoint->X;
  y = entryPoint->Y;

  if (seasonObj->toString() == "Automatic") {
    season = 0;
  } else if (seasonObj->toString() == "Spring") {
    season = 1;
  } else if (seasonObj->toString() == "Summer") {
    season = 2;
  } else if (seasonObj->toString() == "Autumn") {
    season = 3;
  } else if (seasonObj->toString() == "Winter") {
    season = 4;
  }
...
TSessionId
CServerEditionModule::getLinkedSession(CEditionSession *session) const {
  ...
  if (it != _OwnerInfos.end()) {
    if (it->second.EditionSessionId.asInt() != 0 &&
        session->SessionId != it->second.EditionSessionId) {
      linkedSessionId = it->second.EditionSessionId;
    }
    if (it->second.AnimationSessionId.asInt() != 0 &&
        session->SessionId != it->second.AnimationSessionId) {
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, kickPioneer) {
  if (args.size() != 2) {
    log.displayNL(
        "Wrong usage ServerEditionModule.kickPioneer <sessionId> <charId>");
    return false;
  }
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, unkickPioneer) {
  if (args.size() != 2) {
    log.displayNL(
        "Wrong usage ServerEditionModule.unkickPioneer <sessionId> <charId>");
    return false;
  }
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, listAllowedPioneers) {
  ...
  TPioneersSessionsAlloweds::const_iterator first(
      _PioneersSessionsAllowed.begin()),
      last(_PioneersSessionsAllowed.end());
  for (; first != last; ++first) {
    log.displayNL("Session %u: chars:%u", first->second.SessionId.asInt(),
                  first->first);
  }
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, listSessions) {
  ...
  for (; first != last; ++first) {
    ...
    if (it != _OwnerInfos.end()) {
      if (it->second.EditionSessionId.asInt() != 0 &&
          sessionId != it->second.EditionSessionId) {
        linkedSessionId = it->second.EditionSessionId;
      }
      if (it->second.AnimationSessionId.asInt() != 0 &&
          sessionId != it->second.AnimationSessionId) {
        linkedSessionId = it->second.AnimationSessionId;
      }
    }

    const CEditionSession::TCurrentChars &chars = session->getCurrentChars();
    CEditionSession::TCurrentChars::const_iterator first(chars.begin()),
        last(chars.end());
    std::string clients;
    for (; first != last; ++first) {
      if (!clients.empty()) {
    ...
    log.displayNL(
        "Session %u: type '%s': chars:(%u)[%s], AiInstance:%u Owner:%u %s",
        sessionId.asInt(), session->getSessionType().toString().c_str(),
        session->getCurrentChars().size(), clients.c_str(),
        session->getAiInstanceId(), ownerCharId,
        linkedSessionId.asInt() != 0
            ? toString(" Linked to %u", linkedSessionId.asInt()).c_str()
            : "");
  }
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, displaySession) {
  if (args.size() < 1) {
    log.displayNL("Wrong usage: ServerEditionModule.displaySession <sessionId> "
                  "[<displayLevel>]");
    return false;
  }
  ...
  if (session) {
    ...
    log.displayNL(
        "Session %u: chars:%u, AiInstance %u Owner %u %s", sessionId.asInt(),
        session->getCurrentChars().size(), session->getAiInstanceId(),
        ownerCharId,
        linkedSessionId.asInt() != 0
            ? toString(" Linked to %u", linkedSessionId.asInt()).c_str()
            : "");
    ...
    const CEditionSession::TCurrentChars &chars = session->getCurrentChars();
    CEditionSession::TCurrentChars::const_iterator firstChars(chars.begin()),
        lastChars(chars.end());
    for (; firstChars != lastChars; ++firstChars) {
      log.displayNL("Chars: %u , EditSlot: %u:%u", (*firstChars)->getCharId(),
                    sessionId.asInt(), (*firstChars)->getEditSlotId());
    }
    ...
    if (hl) {
      ...
      if (logLevel != 1) {
        ...
        for (; first != last; ++first) {
          log.displayNL("%s", lines[first].c_str());
        }
      }
    } else {
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, listPioneers) {
  ...
  TPioneersSessions::const_iterator first(_PioneersSessions.begin()),
      last(_PioneersSessions.end());

  for (; first != last; ++first) {
    log.displayNL("Chars: %u , Session: %u", first->first,
                  first->second.asInt());
  }
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, displayPioneer) {
  ...
  if (!session) {
    nlwarning("R2Ed: Error a Pioneer(%u) belongs to a Session(%u), but the "
              "session seems to not exist",
              charId, sessionId.asInt());
    return false;
  }
  ...
  for (; firstChars != lastChars && (*firstChars)->getCharId() != charId;
       ++firstChars) {
  }

  if (firstChars == lastChars) {
    nlwarning("R2Ed: Error a Pioneer(%u) belongs to a Session(%u), but the "
              "session does not hold the Pioneer",
              found->first, found->second.asInt());
    return false;
  }

  log.displayNL("'%s' '%s'",
                NLMISC::toString("Chars: %u, IsConnected %u", found->first,
                                 (getClientProxyPtr(found->first) != 0))
                    .c_str(),
                NLMISC::toString("EditSlot: %u:%u", found->second.asInt(),
                                 (*firstChars)->getEditSlotId())
                    .c_str());
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, reconnectSession) {
  if (args.size() < 1) {
    log.displayNL(
        "Wrong usage: ServerEditionModule.reconnectSession <sessionId>");
    return false;
  }
  uint32 sessionId;
  fromString(args[0], sessionId);
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, setCharRingAccess) {
  if (args.size() != 2) {
    log.displayNL("wrong usage ServerEditionModule.setCharRingAccess <charId> "
                  "<RingAccess>");
    return false;
  }
  ...
  log.displayNL("Ring Access changed to '%s' for user %u",
                newRingAccess.c_str(), charId);
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, displayCharRingAccess) {
  if (args.size() != 1) {
    log.displayNL(
        "wrong usage ServerEditionModule.displayCharRingAccess <charId>");
    return false;
  }
  ...
  log.displayNL("Char %u: RingAccess='%s'", charId,
                found->second.RingAccess.c_str());

  return true;
}

void CServerEditionModule::onCharModeUpdateAsked(
    NLNET::IModuleProxy *senderModuleProxy, TCharMode mode) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
...
void CServerEditionModule::characterReady(TCharId charId) {
  // Check for editor or animator character that need to be added in ring
  // universe chat.
  ...
  CEditionSession *session = getSessionByCharId(charId);
  if (session == NULL) {
    nlwarning("CServerEditionModule::characterReady : char %u is not "
              "associated to any session",
              charId);
    return;
  }
  ...
  if (session->getOwnerCharId() == charId && _IOSRingProxy != NULL) {
    ...
    nldebug("CServerEditionModule::characterReady : subscribing the character "
            "%u to universe chat with EGS '%s'",
            charId, _CharacterControlProxy->getModuleName().c_str());
    CCharacterControlItfProxy proxy(_CharacterControlProxy);
    proxy.subscribeCharacterInRingUniverse(this, charId);
  }

  // Indicates the EGS that a player connect in animation session (for reward
  // calculation)
  if (session->getSessionType() == RSMGR::TSessionType::st_anim) {
    if (_CharacterControlProxy) {
      CCharacterControlItfProxy proxy(_CharacterControlProxy);
...
bool CServerEditionModule::isInitialized() const {
  ...
  if (DssUseBs) {
    return _CharacterControlProxy != 0 && !_WaitingForBS;
  }
...
static std::string getSecondesAsString(uint32 secondes) {
  ...
  std::string ret;
  if (d) {
    ret += NLMISC::toString("%u day%s ", d, d > 1 ? "s" : "");
  }
  if (h) {
    ret += NLMISC::toString("%u hour%s ", h, h > 1 ? "s" : "");
  }
  if (m) {
    ret += NLMISC::toString("%u minute%s ", m, m > 1 ? "s" : "");
  }
  if (s) {
    ret += NLMISC::toString("%u seconde%s ", s, s > 1 ? "s" : "");
  }
  return ret;
}
...
void CServerEditionModule::onModuleUpdate() {
  ...
  if (DssUseBs && _BsGoingUp) {
    nldebug("BSI requesting files: '%s'",
            getOverrideRingAccessFilename().c_str());
    Bsi.requestFile(getOverrideRingAccessFilename(),
                    new COverrideRingAccessCallback(this));
    _BsGoingUp = false;
  }
  ...
  // close, hibernate, stop, save session
  {
    ...
    if (now - lastCheck > 1000) {
      ...
      // !!!! When we close an edition session, it is possible that the system
      // also close the linked animation session. So we create a temporary list
      // of ids and we iterate through this list

      {
        TSessions::const_iterator first(_Sessions.begin()),
            last(_Sessions.end());
        for (; first != last; ++first) {
      ...
      std::set<TSessionId>::iterator first(sessionIds.begin()),
          last(sessionIds.end());
      for (; first != last; ++first) {
        ...
        if (session) {
          ...
          if (session->mustHibernate(nowSeconds)) {
            std::string msg;
            nlinfo(
                "Hibernating Edition Session %u (because no player since %s)",
                sessionId.asInt(),
                getSecondesAsString(TimeBeforeCloseHibernatingEditionSession)
                    .c_str());
            bool ok = hibernateSessionImpl(sessionId, msg);
            debugInfo(ok, msg);
            // session is remove from _Sessions here
          } else if (session->mustClose(nowSeconds)) {
            nlinfo("Closing Animation Session %u (because no player since %s)",
                   sessionId.asInt(),
                   getSecondesAsString(
                       TimeBeforeAutoCloseAnimationSessionWithNoPlayer)
                       .c_str());
            closeSession(0, sessionId);
            // session is remove from _Sessions here
          } else if (session->mustStopAnimation(nowSeconds) &&
                     getLinkedSessionId(sessionId).asInt() == 0) {

            nlinfo(
                "Stop test in Edition Session %u (because no player since %s)",
                sessionId.asInt(),
                getSecondesAsString(
                    TimeBeforeStopTestInEditionSessionWithNoPlayer)
                    .c_str());
            stopTest(sessionId);
          } else if (session->mustSave(nowSeconds)) {
            std::string msg;
            nlinfo("Save edition Session %u (because scenario changed and was "
                   "not save since %s)",
                   sessionId.asInt(),
                   getSecondesAsString(TimeBeforeAutoSaveTimeEditionSession)
                       .c_str());
...

  {
    ...
    if (lastCheck + 1 * 1000 < now) // Update kick list every second
    {
      ...
      for (; first != last;) {
        if (now - first->second >
            30 * 1000) // remove from kick list 30 secondes
        {
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, hibernateSession) {
  if (args.size() != 1) {
    log.displayNL(
        "Wrong parameters: 1 parameter needed : the Session to hibernate");
    return false;
  }
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, listCharOverrideRingAccess) {
  ...
  log.displayNL("There is %u  char that have overridden ring access session",
                _OverrideRingAccess.size());
  TOverrideRingAccess::const_iterator first(_OverrideRingAccess.begin()),
      last(_OverrideRingAccess.end());
  for (; first != last; ++first) {
    log.displayNL("Overridden ring access charId %u, access='%s'", first->first,
                  first->second.c_str());
  }
  return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, displayCharOverrideRingAccess) {
  ...
  if (args.size() != 1) {
    log.displayNL(
        "Wrong parameters CServerEditionModule.displayCharOverrideRingAccess "
        "<charId>");
    return false;
  }
  ...
  if (found != _OverrideRingAccess.end()) {
    log.displayNL("Overridden ring access charId %u, access='%s'", found->first,
                  found->second.c_str());
  } else {
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, setCharOverrideRingAccess) {
  ...
  if (args.size() != 2) {
    log.displayNL(
        "Wrong parameters CServerEditionModule.removeCharOverrideRingAccess "
        "<charId> <newValue>");
    return false;
  }
...

  {
    ...
    if (found != _OverrideRingAccess.end()) {
      log.displayNL("The Overridden ring access charId %u, access='%s' has "
                    "been been change to '%s'",
                    found->first, found->second.c_str(), newRingAccess.c_str());
      found->second = newRingAccess;
    } else {
      _OverrideRingAccess.insert(std::make_pair(charId, newRingAccess));
      log.displayNL("The char %u has now a overridden ring access '%s'", charId,
                    newRingAccess.c_str());
    }
  }
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, removeCharOverrideRingAccess) {
  ...
  if (args.size() != 1) {
    log.displayNL("Wrong parameters "
                  "CServerEditionModule.removeCharOverrideRingAccess <charId>");
    return false;
  }
  ...
  if (found != _OverrideRingAccess.end()) {
    log.displayNL(
        "The Overridden ring access charId %u, access='%s' has been removed",
        found->first, found->second.c_str());
...
public:
  CWakeUpHibernatingSessionCallback(CServerEditionModule *editionModule)
      : _EditionModule(editionModule) {}

  // call back for bs file asynchronous read
  void callback(const CFileDescription &fileDescription,
                NLMISC::IStream &dataStream) {
    nldebug("BSIcb CWakeUpHibernatingSessionCallback::callback() for file: %s",
            fileDescription.FileName.c_str());

    if (fileDescription.FileSize == 0) {
      nlwarning("ERROR: try to wakeup a session that does not exit or has been "
                "deleted");
      return;
    }
...
bool CServerEditionModule::wakeUpSession(TSessionId sessionId,
                                         TCharId ownerCharId,
                                         std::string &msg) {
  if (!DssUseBs) {
  ...
  nldebug("WAKING session %u - BSI requesting file: %s", sessionId.asInt(),
          filename.c_str());
  BsiGlobal.requestFile(std::string("r2/sessions/") + filename,
                        new CWakeUpHibernatingSessionCallback(this));
...
void CServerEditionModule::wakeUpSessionImpl(CEditionSession *session) {
  ...
  if (found != _Sessions.end()) {
    ...
    bool ok = _Sessions.insert(std::make_pair(sessionId, sessionPtr.release()))
                  .second;
    ...
    if (owner == _OwnerInfos.end()) // launch a animation from the born
    {
      owner = _OwnerInfos
                  .insert(std::make_pair(
                      ownerCharId,
                      TOwnerInfo(sessionId, TSessionId(0), aiInstanceId)))
                  .first; // create an edition Session
    } else {
      owner->second.AiInstanceId = aiInstanceId;
      owner->second.EditionSessionId = sessionId;
...
bool CServerEditionModule::saveSessionImpl(TSessionId sessionId,
                                           std::string &msg) {
  ...
  if (found == _Sessions.end()) {
    msg = NLMISC::toString("Try to hibernate a non existing session %u",
                           sessionId.asInt());
    return false;
  }
  ...
  {
    nldebug("SAVING session %u - BSI send file: %s", sessionId.asInt(),
            getSessionFilename(sessionId, owner).c_str());
    CBackupMsgSaveFile bsMsg(std::string("r2/sessions/") +
                                 getSessionFilename(sessionId, owner),
                             CBackupMsgSaveFile::SaveFileCheck, BsiGlobal);
...
bool CServerEditionModule::hibernateSessionImpl(TSessionId sessionId,
                                                std::string &msg) {
  bool ret = true;
  //
  // The session must be empty
  ...
  // Suppress the link to the edition/anim session otherwise owner will return
  // to edition instead of previous
  if (session) {
  ...
  //(*) remove player from connecting queue
  // This very small map contains player that are connecting to dss (allowed by
  // SU to connect but not yet connected because they are loading data)
  TPioneersSessionsAlloweds::iterator first(_PioneersSessionsAllowed.begin()),
      last(_PioneersSessionsAllowed.end());
  for (; first != last;) {
    TPioneersSessionsAlloweds::iterator copy = first++;
  ...
  // Add charId connected in animation Module
  {
    ...
    animationSessionExist =
        getAnimationModule()->getConnectedChars(sessionId, connectedChars);
    std::vector<TCharId>::const_iterator first(connectedChars.begin()),
        last(connectedChars.end());
    for (; first != last; ++first) {
  ...
  nldebug("ADD HIBERNATE SESSION: "
          "CServerEditionModule::hibernateSessionImpl(): sessionId=%u ",
          sessionId.asInt());
  _HibernatingSessions[sessionId] = THibernatingSession(
      hibernationDate, rsmSessionType, positionX, positionY, orient, season);

  {
    nldebug("HIBERNATE - SAVING session %u - BSI send file: %s",
            sessionId.asInt(),
            getSessionFilename(sessionId, ownerCharId).c_str());
    CBackupMsgSaveFile bsMsg(std::string("r2/sessions/") +
                                 getSessionFilename(sessionId, ownerCharId),
                             CBackupMsgSaveFile::SaveFileCheck, BsiGlobal);
    bsMsg.DataMsg.serial(*session);
    BsiGlobal.sendFile(bsMsg);
  }
  ...
  msg = NLMISC::toString("Sucess: Session %u is now hibernating",
                         sessionId.asInt());
...
void CServerEditionModule::release() {
  ...
  if (DssUseBs) {
    ...
    if (!_WaitingForBS) {
      ...
      {
        std::set<TSessionId>::iterator first(sessions.begin()),
            last(sessions.end());
        for (; first != last; ++first) {
          std::string msg;
...
void CServerEditionModule::updateSessionListFile() {
  ...
  if (_MustUpdateHibernatingFileList) {
    ...
    if (_R2SessionBackupModule) {
      ...
      THibernatingSessions::const_iterator first(_HibernatingSessions.begin()),
          last(_HibernatingSessions.end());
      for (; first != last; ++first) {
        sessionIds.push_back(first->first);
      }
  ...
  if (!_ClosedSessions.empty()) {
    std::vector<TSessionId> sessionIds;
    TClosedSessions::iterator first(_ClosedSessions.begin()),
        last(_ClosedSessions.end());
    for (; first != last; ++first) {
      TSessionId sessionId = first->first;
      TCharId ownerCharId = first->second;
      nldebug("CLEANING closed session - BSI delete file: %s",
              getSessionFilename(sessionId, ownerCharId).c_str());
      BsiGlobal.deleteFile(std::string("r2/sessions/") +
                               getSessionFilename(sessionId, ownerCharId),
                           false);
  ...
  if (_MustUpdateOverrideRingAcess) {
    ...
    //first->first, first->second.c_str()); 			ss << tmp;
    //
    //		}
    ...
    nldebug("UPDATING ring access override file list - BSI send file: %s",
            getOverrideRingAccessFilename().c_str());
    CBackupMsgSaveFile msg(getOverrideRingAccessFilename(),
                           CBackupMsgSaveFile::SaveFileCheck, Bsi);
...
void CServerEditionModule::onServiceDown(const std::string &serviceName,
  ...
  if (serviceName == "AIS") {
    ...
    for (; first != last; ++first) {
      ...
      } else if (first->second->getSessionType() ==
                 RSMGR::TSessionType::st_anim) {
    ...
    {
      std::set<TSessionId>::iterator first(toClose.begin()),
          last(toClose.end());
      for (; first != last; ++first) {
        std::string msg;
    ...
    nlinfo("AIS is down so close all edit session");
    {
      ...
      std::set<TSessionId>::const_iterator first(toStop.begin()),
          last(toStop.end());
      for (; first != last; ++first) {
        stopTest(*first);
      }
...
void CServerEditionModule::onServiceUp(const std::string &serviceName,
                                       NLNET::TServiceId serviceId) {
  if (serviceName == "BS") {
...
const NLNET::TModuleProxyPtr *
CServerEditionModule::getClientProxyPtr(TCharId charId) const {
  return _ClientsEditionModule.getB(charId);
}

void CServerEditionModule::getTpContext(TCharId charId,
  ...
  bool isLive = getLinkedSessionId(sessionId) != 0 ||
                session->getSessionType() == RSMGR::TSessionType::st_anim;
...
void CServerEditionModule::onTpPositionAsked(
    NLNET::IModuleProxy *senderModuleProxy, float x, float y, float z) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
  ...
  uint8 season;
  TSessionId sessionId =
      this->_Server->getEditionModule()->getSessionIdByCharId(charId);
  // TSessionId sessionId = this->getSessionIdByCharId(charId);
  {
    double sx, sy, sorient;
    this->_Server->getAdminModule()->getPosition(sessionId, sx, sy, sorient,
...
void CServerEditionModule::tpPosition(NLNET::IModuleProxy *sender,
                                      const NLMISC::CEntityId &eid, float x,
                                      float y, float z, uint8 season,
                                      const R2::TR2TpInfos &tpInfos) {
  if (!_CharacterControlProxy.isNull()) {
  ...
  if (isLocal && sender) {
    ...
    if (clientEditionProxyPtr) {
      ...
      TSessionId sessionId = getSessionIdByCharId(charId);
      client.onTpPositionSimulated(this, sessionId, eid.getRawId(), sint32(x),
                                   sint32(y), sint32(z), season);
    }
...
void CServerEditionModule::setStartingAct(
    NLNET::IModuleProxy *senderModuleProxy, uint32 actId) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
...
void CServerEditionModule::tpToEntryPoint(
    NLNET::IModuleProxy *senderModuleProxy, uint32 actId) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  ...
  TSessionId sessionId =
      _Server->getAdminModule()->getSessionIdByCharId(charId);
  uint32 slot = 0;
  ...
  ok = _Server->getAdminModule()->getPosition(sessionId, x, y, orient, season,
                                              actId);
  if (!ok) {
    return;
  }
  ...
  this->tpPosition(senderModuleProxy, clientEid, (float)x, (float)y, 0, season,
                   tpInfos);
}

void CServerEditionModule::onScenarioRingAccessUpdated(
    NLNET::IModuleProxy *senderModuleProxy, bool ok,
    const std::string &ringAccess, const std::string &errMsg) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
  ...
  if (!ok) {
    std::set<uint32>::const_iterator found =
        session->CharsToVerify.find(charId);
    if (found != session->CharsToVerify.end()) {
      return;
    }
    ...
    NLMISC::TTime now = NLMISC::CTime::getLocalTime();
    _Tasks.addTask(
        new CVerifyRingAccess(now + 1000 * TimeBeforeDisconnectionAfterKick,
                              this, sessionId, charId));
  }
}
...
bool CServerEditionModule::isSessionHibernating(
    TSessionId sessionId, RSMGR::TSessionType &sessionType, double &x,
    double &y, double &orient, uint8 &season) {

  THibernatingSessions::const_iterator session =
      _HibernatingSessions.find(sessionId);
...
void CServerEditionModule::saveScenarioFile(
    NLNET::IModuleProxy *sender, const std::string &md5,
    const TScenarioHeaderSerializer &header) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  ...
  if (!checkSecurityInfo(sender, charId, clientEid, userPriv, extendedPriv)) {
    return;
  }
  ...
  for (uint32 i = 0; i < value.size(); ++i) {
    ...
    if (value[i].first == "NevraxScenario" && value[i].second == "1" &&
        userPriv.find("DEV") == std::string::npos && !override) {
      nlwarning("The client %u(CharId) try to save a scenario that is RoSOnly "
                "so it means that the gui has let the user the possibility to "
                "try or that the user is trying to hack us",
                charId);
      ok = false;
      break;
    }
...
void CServerEditionModule::loadScenarioFile(
...
void CServerEditionModule::saveUserComponentFile(
    NLNET::IModuleProxy *sender, const std::string &md5,
    const TScenarioHeaderSerializer &header) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
...
// a message to validate a file waiting to be loaded
void CServerEditionModule::loadUserComponentFile(
    NLNET::IModuleProxy *senderModuleProxy, const std::string &md5,
    const std::string &signature) {
  TCharId charId;
  NLMISC::CEntityId clientEid;
...
void CServerEditionModule::resetKeyPolicies(
    NLNET::IModuleProxy *senderModuleProxy) {
...
void CServerEditionModule::setDefaultKey(NLNET::IModuleProxy *sender,
                                         const std::string &defaultKeyName) {
  nlassert(_KeysHolder);
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, setDefaultKey) {
  ...
  if (args.size() != 1) {
    log.displayNL(
        "Wrong parameters CServerEditionModule.setDefaultKey <defaultKey>");
    return false;
  }
...
void CServerEditionModule::addKeyPolicy(NLNET::IModuleProxy *sender,
                                        const std::string &keyName,
                                        const std::string &privateKeyValue,
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, addKeyPolicy) {
  ...
  if (args.size() != 3) {
    log.displayNL(
        "Wrong parameters CServerEditionModule.addKeyPolicy "
        "<key|EMPTY|DEFAULT> <privatekey|EMPTY> <ACCEPT|REFUSE|TEST>");
    return false;
  }
...
TSessionId
CServerEditionModule::getLinkedSessionId(TSessionId sessionId) const {
  TSessions::const_iterator found = _Sessions.find(sessionId);
  if (found == _Sessions.end()) {
    return TSessionId(0);
  }
  ...
  if (ownerIt == _OwnerInfos.end()) {
    return TSessionId(0);
  }
  if (ownerIt->second.EditionSessionId.asInt() == 0 ||
      ownerIt->second.AnimationSessionId.asInt() == 0) {
    return TSessionId(0);
  }
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, setSessionStartParams) {
  ...
  if (args.size() != 5) {
    log.displayNL(
        "Wrong parameters CServerEditionModule.setSessionStartParams charId, "
        "sessionId, initialIslandLocation, initialEntryPoint, initialSeason");
    return false;
  }
  ...
  setSessionStartParams(0, charId, (TSessionId)sessionId, initialIslandLocation,
                        initialEntryPoint, initialSeason);
  return true;
}

void CServerEditionModule::setSessionStartParams(
    NLNET::IModuleProxy *sender, TCharId charId, TSessionId sessionId,
    const std::string &initialIslandLocation,
    const std::string &initialEntryPoint, const std::string &initialSeason) {

  CScenarioEntryPoints &epManager = CScenarioEntryPoints::getInstance();

  CScenarioEntryPoints::CCompleteIsland *island =
      epManager.getIslandFromId(initialIslandLocation);
  BOMB_IF(!island,
          NLMISC::toString("Session %u No Island '%s'.", sessionId.asInt(),
                           initialIslandLocation.c_str()),
          return);

  CScenarioEntryPoints::CShortEntryPoint *entryPoint =
      epManager.getEntryPointFromIds(initialIslandLocation, initialEntryPoint);

  BOMB_IF(!entryPoint,
          NLMISC::toString("Session %u: No EntryPoint '%s' '%s'.",
                           sessionId.asInt(), initialIslandLocation.c_str(),
                           initialEntryPoint.c_str()),
          return);
  ...
  if (initialSeason == "Automatic") {
    season = 0;
  } else if (initialSeason == "Spring") {
    season = 1;
  } else if (initialSeason == "Summer") {
    season = 2;
  } else if (initialSeason == "Autumn") {
    season = 3;
  } else if (initialSeason == "Winter") {
    season = 4;
  }
...
void CServerEditionModule::setSessionStartParams(TSessionId sessionId, sint32 x,
                                                 sint32 y, uint8 season) {
  CEditionSession *session = getSession(sessionId);
  if (session) {
  ...
  if (session2) {
    if (session2) {
      session2->setStartParams(x, y, season);
    }
    getAnimationModule()->setSessionStartParams(linkedId, x, y, season);
  }
}

void CServerEditionModule::teleportWhileUploadingScenario(
    NLNET::IModuleProxy *sender, const std::string &initialIslandLocation,
    const std::string &initialEntryPoint, const std::string &initialSeason) {
  TCharId charId;
  CEntityId clientEid;
  std::string userPriv;
  ...
  bool ok =
      checkSecurityInfo(sender, charId, clientEid, userPriv, extendedPriv);
  if (!ok) {
    return;
  }
  ...
  for (; first != last; ++first) {
    ...
    if (foundPioneersEId != _PioneersInfo.end()) {
      ...
      getTpContext(charId, tpInfos.TpCancelTextId, tpInfos.TpContext);
      tpPosition(0, foundPioneersEId->second.EntityId, float(x), float(y), 0,
                 season, tpInfos);
    }
  }
}
...
void CServerEditionModule::returnToPreviousSession(TCharId charId) {
  DROP_IF(_CharacterControlProxy.isNull(),
          "Try to disconnect a player but no EGS?", return);
  nlinfo("The charId '%u' was send back to its previous session (maybe the "
         "session he was invited was closed)",
         charId);
  CCharacterControlItfProxy proxy(_CharacterControlProxy);
  proxy.returnToPreviousSession(this, charId);
}

void CServerEditionModule::getStartParams(uint32 charId,
                                          TSessionId lastStoredSessionId) {
  ...
  TPioneersSessionsAllowed *psa =
      getSessionAllowedForChar(charId); // the client is not ready yet
  if (!psa) {
  ...
  CEditionSession *session = getSession(sessionId);
  if (!session) {
    nlwarning("Char %u try to connect to an empty session %u", charId,
              sessionId.asInt());
    returnToPreviousSession(charId);
    return;
  }
  ...
  if (session->isLoaded()) {
    ...
    nldebug("Getting start position from session %u (that is hibernating)",
            sessionId.asInt());
    session->getStartParamsWithDelay(this, charId, lastStoredSessionId);
  }
}

void CServerEditionModule::getStartParamsImpl(uint32 charId,
                                              TSessionId lastStoredSessionId) {
  ...
  TSessionId sessionId = psa->SessionId;
  bool reloadPos = (sessionId == lastStoredSessionId) &&
                   (psa->Role != R2::TUserRole::ur_editor);
  if (reloadPos) {
    reloadPos = getAnimationModule()->mustReloadPosition(sessionId, charId);
  }

  // Get the starting pos for the current act of the animation session (at least
  // for respawn point)
  ...
  CScenario *scenario = getScenarioById(sessionId);
  if (!scenario || scenario->getHighLevel() == 0) {
    reloadPos = false;
  }

  nldebug("R2An: Char %u will start at position (%f, %f) when he will arrive "
          "in session %u",
          charId, x, y, sessionId.asInt());

  // Send the answer
  CCharacterControlItfProxy ccip(_CharacterControlProxy);
  ccip.setUserCharStartParams(this, charId, actStartPosition, reloadPos, season,
                              psa->Role);
  if (reloadPos) {
    psa->EgsReloadPos = true;
  }
  bool isEd = isEditingSession(psa->SessionId);
  nldebug("R2An: Char %u will %s %s session %u", charId,
          (reloadPos ? "resume" : "join"), (isEd ? "edition" : "animation"),
          sessionId.asInt());
}

CServerEditionModule::CPioneerInfo *
CServerEditionModule::getPioneerInfo(TCharId charId) {
  TPionnerInfos::iterator found = _PioneersInfo.find(charId);
  if (found != _PioneersInfo.end()) {
    return &found->second;
  }
...
void CServerEditionModule::multiPartMsgHead(NLNET::IModuleProxy *sbs,
                                            uint32 charId,
                                            const std::string &msgName,
                                            uint32 nbPacket, uint32 size) {
  nldebug(" Received a multi-part message %s (Start, part=%u, size=%u)",
          msgName.c_str(), nbPacket, size);
  CPioneerInfo *info = getPioneerInfo(charId);
  if (!info) {
    nlwarning("Error: received a multipartMsgHead from a unknown char %u",
              charId);
    return;
  }
  if (info->WaitingMsg) {
    nlwarning("Error: Char %u was ask to receive a multi-part message but the "
              "previous one seems to have be lost.",
              charId);
  ...
  info->WaitingMsg =
      new NLNET::CMessage(); // Message name is directly in the data
...
void CServerEditionModule::multiPartMsgBody(NLNET::IModuleProxy *sbs,
                                            uint32 charId, uint32 partId,
                                            const std::vector<uint8> &data) {
  nldebug("Received a multi-part message from %u (Body, part=%u, size=%u)",
          charId, partId, data.size());
  CPioneerInfo *info = getPioneerInfo(charId);
  if (!info) {
    nlwarning("Error: received a multipartMsgHead from a unknown char %u",
              charId);
  ...
  if (!info->WaitingMsg) {
    nlwarning("Error: Char %u was ask to receive a multi-part message  part "
              "but the first part seems to have been lost.",
              charId);
    return;
  }

  info->WaitingMsg->serialBuffer(const_cast<uint8 *>(&data[0]),
                                 (uint)data.size());
...
void CServerEditionModule::multiPartMsgFoot(NLNET::IModuleProxy *sbs,
                                            uint32 charId) {
  ...
  if (!info->WaitingMsg) {
    nlwarning("Error: Char %u was ask to receive a multi-part message end but "
              "the first part seems to have been lost.",
              charId);
    return;
  }
  ...
  if (clientEditionProxyPtr) {
    ...
    nlwarning(
        "Error: Char %u was ask to receive the last part of  a multi-part "
        "message  part but the player seem to not be connected.",
        charId);
  }
...
// Simulate the dss
void CServerEditionModule::forwardToDss(NLNET::IModuleProxy *senderModuleProxy,
                                        uint32 charId,
                                        const NLNET::CMessage &msg) {

...
{
	if (ok) { nlinfo("Ed: %s", msg.c_str()); }
	else { nlwarning("Ed: %s", msg.c_str()); }
}
}
...
public:
	...
	CIdRecycle(uint32 initialValue)
...
public:
	CTaskTryConnectPlayer(CServerEditionModule *serverEditionModule, TCharId charId)
	    : CTask<NLMISC::TTime>(0)
	    , _ServerEditionModule(serverEditionModule)
	    , _CharId(charId)
	{
	}
...
bool CServerEditionModule::getPosition(TSessionId sessionId, double &x, double &y, double &orient, uint8 &season, uint32 actIndex)
{
	...
	bool locationInfo = !entryPointObj || !islandName || !seasonObj || islandName->isString() || !islandName->isString() || !seasonObj->isString();
...
void CServerEditionModule::multiPartMsgHead(NLNET::IModuleProxy *sbs, uint32 charId, const std::string &msgName, uint32 nbPacket, uint32 size)
{
	...
	if (!info)
	{
		nlwarning("Error: received a multipartMsgHead from a unknown char %u", charId);
		return;
	}
	if (info->WaitingMsg)
	{
...
void CServerEditionModule::multiPartMsgBody(NLNET::IModuleProxy *sbs, uint32 charId, uint32 partId, const std::vector<uint8> &data)
{
	...
	if (!info->WaitingMsg)
	{
		nlwarning("Error: Char %u was ask to receive a multi-part message  part but the first part seems to have been lost.", charId);
		return;
	}
...
void CServerEditionModule::multiPartMsgFoot(NLNET::IModuleProxy *sbs, uint32 charId)
{
	...
	if (!info->WaitingMsg)
	{
		nlwarning("Error: Char %u was ask to receive a multi-part message end but the first part seems to have been lost.", charId);
		return;
	}

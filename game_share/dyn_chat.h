...
//===================================================================================================================
/** This class represents an interaction between a client and a channel in a
 * dynamic chat A session is initiated from a CDynChat instance.
 */
...
private:
  ...
  CDynChatSession *
      *_PrevClientSession; // Pointer on 'next' field in previous client
                           // session, or start of linked list
  CDynChatSession *_NextChannelSession;
  CDynChatSession *
      *_PrevChannelSession; // Pointer on 'next' field in previous channel
                            // session, or start of linked list
  // remove the session
  void unlink();
  // ctor
...
public:
  ...
  ~CDynChatClient();
  // Head of linked list of sessions for this channel. List must be followed by
  // calling CDynChatSession::getNextClientSession.
  CDynChatSession *getFirstSession() { return _FirstSession; }
  const TDataSetRow &getID() const { return _ID; }
  // Return session for this client in channel 'chan', or NULL if no such
  // session exists
...
public:
  ...
  ucstring Title;  // gives the title of the channel when it is not translated
                   // (e.g Localized == false)
  bool HideBubble; // hide the display of bubble
  bool UniversalChannel; // treat like universe channel
public:
  CDynChatChan();
  //	CDynChatChan(TChanID id = NLMISC::CEntityId::Unknown, NLNET::TServiceId
  //ownerServiceId, bool noBroadcast, bool forwadInput);
  ...
  // Head of linked list of sessions for this channel. List must be followed by
  // calling CDynChatSession::getNextChannelSession.
  CDynChatSession *getFirstSession() { return _FirstSession; }
  ...
  bool getForwardPlayerIntputToOwnerService() {
    return _ForwardPlayerIntputToOwnerService;
  }
 ...
 *
 * \TODO : Would be good to generalize the pattern used as something like
 * NLMISC::CBinaryRelation<CompoundA, CompoundB, Relation> This would avoid to
 * have fields for one service that aren't used by another (like the
 * historic...)
 *
...
public:
  ...
  // dtor
  ~CDynChat();
  /** Create a new channel with the given ID. A warning is issued if channel
   * already exists. \return true if creation succeeded
   */
  ...
  /** Create a new session in channel 'chan' for the client 'client'. No-op if
   * session already exists \return pointer on created session
   */

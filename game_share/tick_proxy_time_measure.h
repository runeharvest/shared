...
// The accuracy can be tuned in this function (using rdtsc, getPerformanceTime,
// etc.)
inline TAccurateTime getAccurateTime() { return NLMISC::CTime::getLocalTime(); }
 ...
 *
 */
template <int N> class CTimeMeasure {
public:
  uint16 V[N];
  ...
  ///
  void displayStat(NLMISC::CLog *log, TMeasureTypeToCString toCstr,
                   uint divideBy = 1) const {
    if (divideBy != 0) {
      for (uint i = 0; i != N; ++i)
  ...
  /*#if defined( _MSC_VER )
      friend CTimeMeasure	operator/ ( const CTimeMeasure& m, uint16
  constant ); #else template <int M> friend CTimeMeasure operator/ ( const
  CTimeMeasure& m, uint16 constant ); #endif
  */
  ///

...
public:
  ...
  uint32
      CreatorName; // player character crafted item (if is a item crafted by PC)
  uint32
      CurrentDamage; // if is weapons or ammo, damage made by item whith current
                     // PC skill, if item is an range weapon, it is a modifier
                     // (+/- and base is ammo) (can be modified by action)
  uint32 MaxDamage; // if is weapons or ammo, maximum damage can be made by item
                    // (due to his recommended skill (= old quality for weapon
                    // and ammos), this is reached when skill = recommended (can
                    // be modified by action)
  ...
  float Range; // range of weapon in milimeter, if item is an ammo, it is a
               // modifier (+/-, base is range weapon)
  ...
  PROTECTION_TYPE::TProtectionType
      MagicProtection[MaxMagicProtectionByJewel]; // magic protection, only
                                                  // gived by jewel
  ...
  float WearEquipmentMalus; // Malus for wearing this equipment (malus is used
                            // when execute an magic, forage action, craft
                            // action...), malus is only applicable for weapon
                            // and armor pieces

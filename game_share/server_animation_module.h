...
/** Handle the animation Mode (Play Mode and DM Mode in animation session,
 * DM/Test Mode in Edition Session). Creations from RT data of Primitives that
 * are read by AI start/stop of act DM functions (Kill, Heal)
 */
...
public:
  ...
  virtual bool getConnectedChars(TSessionId sessionId,
                                 std::vector<TCharId> &chars) const;
  virtual void setSessionStartParams(TSessionId sessionId, sint32 x, sint32 y,
                                     uint8 season);
  virtual void teleportCharacter(NLNET::IModuleProxy *ais,
  ...
  // from client (the module will upload mission item description to dm that
  // will store it to its db)
  ...
  virtual bool
  getHeaderInfo(TSessionId sessionId,
                TScenarioHeaderSerializer::TValueType &values) const;

  /////////////////////////////////////////////////////
  //// DM
  /////////////////////////////////////////////////////
  virtual void activateEasterEgg(class NLNET::IModuleProxy *aisControl,
                                 uint32 easterEggId, TSessionId scenarioId,
                                 uint32 actId, const std::string &items,
                                 float x, float y, float z, float heading,
                                 const std::string &grpControler,
                                 const std::string &name,
                                 const std::string &look);
  virtual void deactivateEasterEgg(class NLNET::IModuleProxy *aisControl,
                                   uint32 easterEggId, TSessionId scenarioId,
                                   uint32 actId);
  virtual void deactivateEasterEggsFromAct(TSessionId scenarioId, uint32 actId);
  virtual void onEasterEggLooted(class NLNET::IModuleProxy *egs,
                                 uint32 easterEggId, TSessionId scenarioId);
  virtual void dssMessage(class NLNET::IModuleProxy *ais, TSessionId sessionId,
                          const std::string &msgType, const std::string &who,
                          const std::string &msg);
  // a dm target a npc and want to know what et can do on it (control, kill) and
  // execute dm action (kill, heal...)
  virtual void onDssTarget(NLNET::IModuleProxy *senderModuleProxy,
  ...
  virtual void setScenarioPoints(NLNET::IModuleProxy *ais, TSessionId sessionId,
                                 float scenarioPoints);
  virtual void startScenarioTiming(NLNET::IModuleProxy *ais,
                                   TSessionId sessionId);
  virtual void endScenarioTiming(NLNET::IModuleProxy *ais,
                                 TSessionId sessionId);
  ...
  virtual void onUserTriggerTriggered(NLNET::IModuleProxy *client, uint32 actId,
                                      uint32 triggerId);
   ...
   */
  void stopTalk(const NLMISC::CEntityId &eid,
                const NLMISC::CEntityId &creatureId, TDataSetRow entityRowId);

  /*
   * stopIncarn calls a AIWrapper method that sends a message to the AIS to
   * indicate the DM has stopped controlling a npc. eid is the DM Id. creatureId
   * is the targeted npc's id.
   */
  void stopIncarn(const NLMISC::CEntityId &eid,
                  const NLMISC::CEntityId &creatureId);

  /*
   * stop the control of all Npc controlled by the player(incarn or talk as)
   * clientProxyPtr (use when client Go from DM -> Player)
   */
  virtual void stopControlNpcs(TCharId charId);

  /*
   * When a bot is despawned, this method checks if it was controlled and
   * "talked as" by a DM. If controlled, the stopControl method is called and
   * the _ControlledEntities map is updated: if no more players control the npc
   * the map entry corresponding to the creature is deleted. If there is still
   * one player or more that controls the npc, the map entry is maintained and
   * its eid vector is updated. It does the same treatment on the
  ...
  bool getPosition(TSessionId sessionId, double &x, double &y, double &orient,
                   uint8 &season, uint32 actIndex = 0);
  ...
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, loadPrimitiveFile,
                             "load Primitive file ", "filename")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, loadPdrFile,
                             "load AiActions from Prd", "filename")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, loadRtFile, "", "filename")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, savePdrFile,
                             "load AiActions from Pdr", "filename")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, displayPdr, "display pdr",
                             "no args")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, clearPdr, "clear prd",
                             "no args")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, startTest, "run test",
                             "the sessionId")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, stopTest, "stop a test",
                             "the sessionId")
  //			NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule,
  //stopTest, "stop a test", "the sessionId")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, displayMissionItems,
                             "display Mission items of a session",
                             "the sessionId")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, displayUserTriggers,
                             "display UserTrigger of a session",
                             "the sessionId")
  NLMISC_COMMAND_HANDLER_ADD(CServerAnimationModule, triggerUserTrigger,
                             "trigger a user trigger",
                             "the sessionId, the trigger act, the trigger Id");
...
private: // private functions
  ...
  ////////////////////////////
  /*
   * If the creature is already controlled by a player, updates its eid vector
   * by adding the id of the new controlling player. If the creature is not
   * already controlled by any player, this method adds an entry in the
   * _ControlledEntities map.
   */
  bool setIncarningPlayer(TSessionId sessionId,
                          const NLMISC::CEntityId &creatureId,
                          const NLMISC::CEntityId &eid, TDataSetRow entityRowId,
                          TAIAlias alias);

  /*
   * If the creature is already "talked as" by a player, updates its eid vector
   * by adding the id of the new talking player. If the creature is not already
   * "talked as"by any player, this method adds an entry in the
   * _TalkedAsEntities map.
   */
  bool setTalkingAsPlayer(TSessionId sessionId,
                          const NLMISC::CEntityId &creatureId,
                          const NLMISC::CEntityId &eid, TDataSetRow entityRowId,
                          TAIAlias alias);
   ...
   */
  bool isTalkingAs(const NLMISC::CEntityId &creatureId,
                   const NLMISC::CEntityId &eid) const;

  /*
   * Checks whether a creature is already controlled by the player or not.
   */
  bool isIncarnedByPlayer(const NLMISC::CEntityId &creatureId,
                          const NLMISC::CEntityId &eid) const;

  /*
   * Deletes an eid from the eid vector of a creature (in the _TalkedAsEntities
   * map).If the vector is empty, the whole map entry is deleted.
   */
  void removeTalkingAsPlayer(TSessionId sessionId,
                             const NLMISC::CEntityId &creatureId,
                             const NLMISC::CEntityId &eid);

  /*
   * Deletes an eid from the eid vector of a creature (in the
   * _ControlledEntities map).If the vector is empty, the whole map entry is
   * deleted.
   */
  void removeIncarningPlayer(TSessionId sessionId,
                             const NLMISC::CEntityId &creatureId,
                             const NLMISC::CEntityId &eid);
  ...
  // message from  Server Admin Module for starting Animation Session (calle
  // queueSession)
  ...
  bool translateActToPrimitive(CInstanceMap &components,
                               CAnimationSession *animSession, CObject *act,
                               uint32 actId, NLLIGO::CPrimitives &primDoc);
  // Create an Actuib  (for translateActToPrimitive implementation)
  virtual NLLIGO::IPrimitive *
  getAction(CObject *object, const std::string &prefix, TSessionId scenarioId);
  // Create an Event  (for translateActToPrimitive implementation)
  virtual NLLIGO::IPrimitive *getEvent(CObject *object,
  ...
  void updateAnimationProperties(NLNET::IModuleProxy *senderModuleProxy,
                                 const NLMISC::CEntityId &eid, CRtNpc *rtNpc,
                                 CRtGrp *rtGrp);
...
private: // privates members
  ...
  // Task that are waiting to be executed (like starting an act in 30
  // seconds...)
   ...
   * -value: a COwnedCreatureInfo structure. This structure contains a
   * TDataSetRow corresponding to the creature's id in the mirror, and a vector
   * of CEntityId (eids) corresponding to the players respectively controlling
   * or "talking as" the npc.
   */

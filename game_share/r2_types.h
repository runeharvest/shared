...
struct TActPositionDescription {
  ...
  TActPositionDescription(const std::string &name, const std::string &island,
                          uint8 season, uint32 locationId)
...
// Mode of a player if (player or test) npc speed == NORMAL, if ( dm or editor)
// then character speed == Quicker
struct TCharMode {
  enum TValues { Player, Tester, Editer, Dm, Invalid };

  TValues _Value;

public:
  TCharMode() : _Value(Invalid) {}
  TCharMode(TValues value) : _Value(value) {}
  ...
  bool operator<(const TCharMode &other) const { return _Value < other._Value; }
...
public:
  TPioneersSessionsAllowed() : SessionId(uint32(-1)), Role(TUserRole::invalid) {
    EgsReloadPos = false;
  }

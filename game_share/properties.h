  ...
  typedef struct {
    union {
      ...
      struct {
        ...
        uint16 canExchangeItem : 1; // It is possible to exchange items with the
                                    // entity
        ...
        uint16 isInvulnerable : 1;  // entity is invulnerable (traders,
                                    // rolemasters...)
...
public:
  ...
  // Operator ==
  bool operator==(const CProperties &p) const {
    return p().properties == _Properties.properties;
  }
  ...
  /// serial
  void serial(NLMISC::IStream &f) { f.serial(_Properties.properties); }

  std::string toString() const {
    return NLMISC::toString(_Properties.properties);
  }
};

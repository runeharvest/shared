...
public:
  ...
  //		TValue(float x, float y, float z) { Vector.x = x; Vector.y = y;
  Vector.z = z; }
  //		TValue(const NLMISC::CVector& v ) { Vector.x = v.x; Vector.y =
  v.y; Vector.z = v.z; } public:
  //		NLMISC::CVector				getVector() const {
  return NLMISC::CVector(Vector.x, Vector.y, Vector.z); }
  //		float						getFloat() const
  { return Float; }
  //		double						getDouble()
  const { return Double; } sint32
  getInt() const { return Int; } uint64
  getUInt64() const { return UInt64; } sint64
  getInt64() const  { return SInt64; } public:
...
public:
   ...
   * connection: the size is *IN BITS*, not in bytes (the actual size is this
   * one plus the header size)
   */
  virtual uint32 size() { return 0; }

  /// Returns the priority of this action, it can changed dynamically if you
  /// want (1 is very urgent)
  ...
  /// This value must be set (in ms) in the ctor of your action to know when
  /// this action must be resent to the other side.

...
When we can speak as a npc 'In this case we open a channel' and when we write in
this channel we simulate that the npc is speaking When a npc is controlled and
he want to talk. Its message are forwarded to this channel
*/
...
class CStringManagerModule : public NLNET::CModuleBase,
...
public:
  ...
  virtual void registerTableRequested(
      TSessionId sessionId,
      std::vector<std::pair<std::string, std::string>> &entries);

  virtual void unregisterTableRequested(TSessionId sessionId);

  virtual void translateAndForward(TDataSetRow senderId,
                                   CChatGroup::TGroupType groupType,
                                   std::string id, TSessionId sessionId);

  virtual void translateAndForwardWithArg(TDataSetRow senderId,
                                          CChatGroup::TGroupType groupType,
                                          std::string id, TSessionId sessionId,
                                          std::vector<float> &val);

  virtual void talkAs(NLNET::TModuleId &id, TDataSetRow &creaturRowId,
                      std::string &name, TSessionId sessionId);

  virtual void stopTalkAs(NLNET::TModuleId &id, TDataSetRow &npcId);

  virtual void forwardIncarnChat(TChanID id, TDataSetRow senderId,
                                 ucstring sentence);

  virtual void sendTable(TSessionId sessionId,
                         NLNET::IModuleProxy *moduleProxy);

  virtual void sendIdList(TSessionId sessionId,
                          NLNET::IModuleProxy *senderModuleProxy);

  virtual void sendStringValue(TSessionId sessionId,
                               NLNET::IModuleProxy *moduleProxy,
                               std::string id);
...
private:
  ...
  virtual void send(TDataSetRow &senderId, CChatGroup::TGroupType groupType,
                    const std::string &toSend);

  virtual void insertClient(NLNET::IModuleProxy *moduleProxy,
                            TChanID channelId);

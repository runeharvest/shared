...
CActionFactory::~CActionFactory() {
  uint nbDeleted = 0;
  for (std::vector<TActionStore>::iterator iv = RegisteredAction.begin();
       iv != RegisteredAction.end(); ++iv) {
    TActionStore &actionStore = (*iv);
    for (std::vector<CAction *>::iterator ias = actionStore.second.begin();
         ias != actionStore.second.end(); ++ias) {
      delete (*ias);
      ++nbDeleted;
    }
  }
...
void CActionFactory::registerAction(uint32 code, CAction *(*creator)()) {
  /*
      ...
      {
          nlerror ("the code %u already registered in the CActionFactory",
     code);
      }
  ...
  */
  ...
  if (RegisteredAction.size() <= code)
    RegisteredAction.resize(
        code + 1, make_pair((CAction * (*)()) NULL, vector<CAction *>()));
...
CAction *CActionFactory::create(TCLEntityId slot, TActionCode code) {
  if (RegisteredAction.size() <= code || RegisteredAction[code].first == NULL) {
    nlwarning("CActionFactory::create() try to create an unknown action (%u)",
              code);
    return NULL;
  } else if (RegisteredAction[code].second.empty()) {
    // no action left in the store
    CAction *action =
        RegisteredAction[code].first(); // execute the factory function
    // nlinfo( "No action in store for code %u, creating action (total %u, total
    // for code %u)", code, getNbActionsInStore(),
    // getNbActionsInStore(action->Code) );
    action->Code = code;
    action->PropertyCode = code; // default, set the property code to the action
                                 // code (see create(TProperty,TPropIndex))
    action->Slot = slot;
    action->reset();
    return action;
  } else {
    ...
    // nlinfo( "Found action in store for code %u (total %u, total for code
    // %u)", code, getNbActionsInStore(), getNbActionsInStore(action->Code) );
...
/* Create the action from a property code, fills property index and fill the
 * internal propindex if needed (it assumes the frontend and the client have the
 * same mapping property/propindex).
 */
CAction *CActionFactory::createByPropIndex(TCLEntityId slot,
...
void CActionFactory::remove(CAction *&action) {
  if (action != NULL) {
    ...
    // nlinfo( "Inserting action in store for code %u (total %u, total for code
    // %u)", action->Code, getNbActionsInStore(),
    // getNbActionsInStore(action->Code) );
    action = NULL;
  }
}
...
/* Pack an action to a bit stream. Set transmitTimestamp=true for
 * server-->client, false for client-->server. If true, set the current
 * gamecycle.
 */
void CActionFactory::pack(CAction *action, NLMISC::CBitMemStream &message,
                          NLMISC::TGameCycle /* currentCycle */) {
  ...
  // OLIV: nlassertex (message.getPosInBit () - val ==
  // (sint32)CActionFactory::getInstance()->size (action),
  // ("CActionFactory::pack () : action %d packed %u bits, should be %u, size()
  // is wrong", action->Code, message.getPosInBit () - val,
  // CActionFactory::getInstance()->size (action)));

  //	nlinfo ("ac:%p pack one action in message %d %hu %u %d", action,
  //action->Code, (uint16)(action->CLEntityId), val, message.getPosInBit()-val);
}

/* Pack an action to a bit stream, for server-->client, actioncode < 4 only
 */
// void CActionFactory::packFast( CAction *action, NLMISC::CBitMemStream&
// message, NLMISC::TGameCycle currentCycle )

/* Unpack some actions from a bit stream. Set transmitTimestamp=true for
 * server-->client, false for client-->server. If true, set the current
 * gamecycle.
 */
void CActionFactory::unpack(NLMISC::CBitMemStream &message,
  ...
  while ((sint32)message.length() * 8 - message.getPosInBit() >= 8) {
    ...
    // nlinfo ("m%d size: p:%d s:%d c:%d (actionsize: %d) slot:%hu", n,
    // message.getPosInBit (), message.length() * 8, code, action->size(),
    // (uint16)action->CLEntityId);
 ...
 */
CAction *CActionFactory::unpack(NLMISC::CBitMemStream &message,
                                NLMISC::TGameCycle /* currentCycle */) {
  CAction *action = NULL;
...
uint32 CActionFactory::size(CAction *action) {
  // If you change this size, please update IMPULSE_ACTION_HEADER_SIZE in the
  // front-end
...
void CActionFactory::initVolatileProperties() {
  ...
  _VolatilePosition =
      (CActionPosition *)(createByPropIndex(0, PROPERTY_POSITION));
}
...
NLMISC_COMMAND(displayActionVectors,
               "Display the size of action vector in factory", "") {
  uint nbActionsInStore =
      CLFECOMMON::CActionFactory::getInstance()->getNbActionsInStore();

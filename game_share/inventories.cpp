...
TInventory toInventory(const std::string &str) {
  ...
  // support for pack_animalx for backward compatibility (necessary to load old
  // save games)
  if (inv == UNDEFINED) {
    if (str == "pack_animal1")
      inv = pet_animal1;
...
//
const char *CInventoryCategoryForCharacter::InventoryStr
    [CInventoryCategoryForCharacter::NbInventoryIds] = {
        "BAG",          "PACK_ANIMAL0", "PACK_ANIMAL1",
        "PACK_ANIMAL2", "PACK_ANIMAL3", "ROOM"};
const uint CInventoryCategoryForCharacter::InventoryNbSlots
    [CInventoryCategoryForCharacter::NbInventoryIds] = {
        NbBagSlots,    NbPackerSlots, NbPackerSlots,
        NbPackerSlots, NbPackerSlots, NbRoomSlots};
...
// - data/gamedev/interfaces_v3/inventory.xml: inventory:content:bag param
// inv_branch_nb

const char *CInventoryCategoryForGuild::InventoryStr
    [CInventoryCategoryForGuild::NbInventoryIds] = {"GUILD"};
const uint CInventoryCategoryForGuild::InventoryNbSlots
    [CInventoryCategoryForGuild::NbInventoryIds] = {NbGuildSlots};
...
// - data/gamedev/interfaces_v3/inventory.xml: inventory:content:guild param
// inv_branch_nb

const char *InfoVersionStr = "INFO_VERSION";

const char *CItemSlot::ItemPropStr[NbItemPropId] = {"SHEET",
                                                    "QUALITY",
                                                    "QUANTITY",
                                                    "USER_COLOR",
                                                    "CHARAC_BUFFS",

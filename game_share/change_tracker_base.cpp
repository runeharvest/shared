...
#endif
  MIRROR_DEBUG("MIRROR: TRACKER: Mutex %s (id %d) ",
               createNew ? "created" : "accessed", mutid);
  _MutId = mutid;
  return res;
}
...
void CChangeTrackerBase::recordChange(TDataSetIndex entityIndex) {
...
#ifdef NL_DEBUG
  nlassertex((entityIndex != INVALID_DATASET_INDEX) &&
                 (entityIndex != LAST_CHANGED),
             ("E%d", entityIndex));
#endif

  // Protect consistency of two parallel calls to recordChange() for the same
  // tracker and between recordChange() and popFirstChanged()
  ...
  if (_Array[entityIndex].NextChanged == INVALID_DATASET_INDEX) {
    ...
    if (_Header->Last != INVALID_DATASET_INDEX) {
      ...
      // nlassertex( _Header->Last != entityIndex, ( "Looping at %u",
      // entityIndex ) );
      ...
      // nldebug( "_Array = %p, _Array[%d].NextChanged = %d", _Array,
      // entityIndex, _Array[entityIndex].NextChanged );
    }
  ...
  // else
  //	nldebug( "Tracker %p: E%d already in list (%d)", _Array, entityIndex,
  //_Array[entityIndex].NextChanged );

#ifdef COUNT_MIRROR_PROP_CHANGES
  //++_Header->NbValuesSet; // should be atomic!
#endif
  // nldebug( "Tracker (smid %u): E%d already in list (pointing to %d)", smid(),
  // entityIndex, _Array[entityIndex].NextChanged );
}
...
void CChangeTrackerBase::cancelChange(TDataSetIndex entityIndex) {
  ...
  if (row != LAST_CHANGED) {
    if (row == entityIndex) {
      ...
      while ((_Array[row].NextChanged != entityIndex) &&
             (_Array[row].NextChanged != LAST_CHANGED)) {
 ...
 */
void CChangeTrackerBase::displayTrackerInfo(const char *headerStr,
                                            const char *footerStrNotAllocd,
                                            NLMISC::CLog *log) const {
  if (isAllocated())
    log->displayNL("%sAllocated, %d changes, smid %d mutid %d", headerStr,

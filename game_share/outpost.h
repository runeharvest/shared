...
const uint32 MAX_OUTPOST = 16; // Same as (#x) in the database
                               // SERVER:GUILD:OUTPOST:O#x:S see (database.xml)

const uint32 OUTPOST_MAX_SPAWN_ZONE =
    16; // Same as (#y) in the database SERVER:GUILD:OUTPOST:O#x:SPAWN_ZONE:#y
        // see (database.xml)
const uint32 OUTPOST_MAX_SQUAD_SHOP =
    16; // Same as (#y) in the database SERVER:GUILD:OUTPOST:O#x:SQUAD_SHOP:#y
        // see (database.xml)

const uint32 OUTPOST_NB_SQUAD_SLOTS = 12;
const uint32 OUTPOST_MAX_SQUAD_TRAINING =
    OUTPOST_NB_SQUAD_SLOTS *
    2; // Same as (#y) in the database SERVER:GUILD:OUTPOST:O#x:S:T#y see
       // (database.xml)
const uint32 OUTPOST_MAX_SQUAD_SPAWNED =
    OUTPOST_MAX_SQUAD_TRAINING; // Same as (#y) in the database
                                // SERVER:GUILD:OUTPOST:O#x:S:S#y see
                                // (database.xml)

const uint32 OUTPOST_MAX_BUILDINGS =
    4; // Same as (#y) in the database SERVER:GUILD:OUTPOST:O#x:BUILDINGS:#y see
       // (database.xml)
...
enum TPVPType {
  UnknownPVPType,
  PVE, // can only be attacked if the outpost is held by a tribe and if the
       // attacking guild comes from the same continent as the outpost
  PVP, // can only be attacked if the attacking guild comes from the same
       // continent as the outpost
  RVR, // can only be attacked if the attacking guild comes from another
       // continent as the outpost
  Full // same as RVR but cant be set by the high council
};

enum TPVPSide { OutpostOwner = 0, OutpostAttacker = 1, UnknownPVPSide };
...
enum TOutpostState {
  ...
  AttackBefore,   // the delay between war declaration and war has elapsed.
                  // Attacker tries to take the outpost
  AttackRound,    // ...
  AttackAfter,
  DefenseBefore, // the attack was successful. The attacker must now defend the
                 // outpost
  DefenseRound,  // ...
  DefenseAfter
};
...
enum TSpecialOutpostEvent {
  ...
  TribeOwnershipBegin, // Owner of the outpost is now a tribe (even if not in
                       // peace)
  TribeOwnershipEnd,   // Owner of the outpost is no more a tribe
  GuildOwnershipBegin, // Owner of the outpost is now a guild (even if not in
                       // peace)

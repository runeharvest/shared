...
// TickSpeedLoop variable. We store 1 min to get the average (see "help
// TickSpeedLoop")
CVariable<sint32> TickSpeedLoop("tick", "TickSpeedLoop",
                                "Duration of the tick update (in ms)", -1, 600);

// TotalSpeedLoop variable. We store 1 min to get the average.
CVariable<sint32>
    TotalSpeedLoop("tick", "TotalSpeedLoop",
                   "Time of game cycle (from TICK from end of UMM) (in ms)", -1,
                   600);

// Time of state C1 is TickSpeedLoop.

// Time of state C2 (see ms_automaton.cpp)
CVariable<sint16> ProcessMirrorUpdatesSpeed("tick", "ProcessMirrorUpdatesSpeed",
                                            "", -1, 600);

// Time of state C3 (see ms_automaton.cpp)
CVariable<sint16> ReceiveMessagesViaMirrorSpeed("tick",
...
//
//-----------------------------------------------
static void cbRegistered(CMessage &msgin, const std::string &serviceName,
...
//
//-----------------------------------------------
static void cbTick(CMessage &msgin, const std::string &serviceName,
...
//
//-----------------------------------------------
static void cbStepAndTick(CMessage &msgin, const std::string &serviceName,
                          TServiceId serviceId) {
  TGameTime gameTimeStep = 0;
  msgin.serial(gameTimeStep);
  CTickEventHandler::setGameTimeStep(gameTimeStep);
...
//
//-----------------------------------------------
static void cbDisplayTime(CMessage &msgin, const string &serviceName,
                          TServiceId serviceId) {
  nlinfo("<CTickEventHandler::cbDisplayTime> : Time in the service : %f",
         (double)CTickEventHandler::getGameTime());
...
void CTickEventHandler::tickUpdate(TServiceId serviceId) {
  ...
  time_t t;
  time(&t);
  _QuickLog.displayNL("%s: %u: Tick", IDisplayer::dateToHumanString(t),
                      getGameCycle());
...
void CTickEventHandler::sendTockBack(NLNET::TServiceId serviceId) {
  ...
  uint16 v =
      (TickSpeedLoop.get() < 0xFFFF) ? (uint16)TickSpeedLoop.get() : 0xFFFF;
  msgout.serial(v);
  ...
  time_t t;
  time(&t);
  _QuickLog.displayNL("%s: %u: Tocked", IDisplayer::dateToHumanString(t),
                      getGameCycle());
}
...
void cbTicksUp(const std::string &serviceName, TServiceId id, void *arg) {
  if (CUnifiedNetwork::getInstance()->isServiceLocal(id)) {
    ...
    bool tocking = true;
    uint16 threshold = 0;
    CConfigFile::CVar *cvTocking =
        IService::getInstance()->ConfigFile.getVarPtr("Tocking");
    if (cvTocking)
      tocking = (cvTocking->asInt() == 0) ? false : true;

    if (!tocking) {
      CConfigFile::CVar *cvThreshold =
          IService::getInstance()->ConfigFile.getVarPtr("Threshold");
      if (cvThreshold)
        threshold = cvThreshold->asInt();
    }

    nlinfo("This service %s and has a threshold of %d",
           (tocking ? "tocks" : "doesn't tock"), threshold);
...
//
//--------------------------------------------------------------
void CTickEventHandler::init(void (*updateFunc)(), void (*syncFunc)(),
                             bool tockAtBeginOfTickUpdate) {
  // set the callback
  userCbUpdate = updateFunc;
  nlassert(!userCbSync); // the callback must be init first by this method
                         // (mirror must be init
  userCbSync = syncFunc;
  ...
  CUnifiedNetwork::getInstance()->setServiceDownCallback("MS", cbTicksDown,
                                                         NULL);
...
/*
 * Set a callback to call when receiving the first game cycle (call this method
 * in your init()) \param syncFunc will be call when the ticks send the syncro
 * \param allowReplaceCallback true if we allow the callback to be replaced
 * \return previous callback if exist, otherwise NULL
 */
TUserSyncCallback
CTickEventHandler::setSyncCallback(TUserSyncCallback syncFunc,
                                   bool allowReplaceCallback) {
  if (userCbSync) {
...
NLMISC_CATEGORISED_DYNVARIABLE(tick, NLMISC::TGameCycle, TickGameCycle,
                               "game cycle (in tick)") {
...
NLMISC_CATEGORISED_DYNVARIABLE(tick, NLMISC::TGameTime, TickGameTime,
                               "game time (in second)") {
  // we can only read the value
  if (get)
...
NLMISC_CATEGORISED_COMMAND(tick, displayTickRecentHistory,
                           "Display the history of tick events", "") {
  RecentHistory.write(&log);
  return true;
}

NLMISC_CATEGORISED_COMMAND(tick, setDayTime, "set the current time of the day",
                           "<hour> <minute>") {
  if (args.size() != 2)
    return false;
  ...
  float prevHour = (float)fmod(
      CTickEventHandler::getGameCycle() / float(RYZOM_HOURS_IN_TICKS), 24.0f);

  if (newHour >= prevHour) {
    CTickEventHandler::setGameCycle(
        (NLMISC::TGameCycle)(CTickEventHandler::getGameCycle() +
                             (newHour - prevHour) * RYZOM_HOURS_IN_TICKS));
  } else {
    CTickEventHandler::setGameCycle((NLMISC::TGameCycle)(
        CTickEventHandler::getGameCycle() +
        (24.0f + newHour - prevHour) * RYZOM_HOURS_IN_TICKS));
  }
  return true;
}

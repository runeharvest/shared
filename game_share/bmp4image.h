...
template <unsigned int W, unsigned int H> class CBMP4Image {
public:
  CBMP4Image() : hdr(W, H) {
    // nlassert((W&0xf)==0);
  ...
  void circle() {
    ...
    for (int j = 0; j < H; j++)
      for (int i = 0; i < W; i++) {
        ...
        double angle =
            atan2(double(i) - double(W - 1) / 2, double(j) - double(H - 1) / 2);
  ...
  struct SHdr {
    SHdr(int w, int h) {
      ...
      height = h;
      bitdepth = 0x0004;
      hdrsize = sizeof(CBMP4Image::SHdr) +
                sizeof(CBMP4Image::SPalette); // 0x00000076;
      datasize = w * h / 2;
    ...
    //		unsigned short magicBM;		// must have value "BM" (0x4D42)
    //if it's a .bmp
    ...
    unsigned long
        paloffs;          // offset from here to the start of the palette (0x28)
    ...
    unsigned long
        datasize; // size of data block (0x00000080 for 16x16 4bit image)
...
public:
  ...
  CTGAImage()
      : IdLength(0), ColourMapType(0), DataTypeCode(2), ColourMapOrigin(0),
        ColourMapLength(0), ColourMapDepth(0), XOrigin(0), YOrigin(0),
        BitsPerPixel(16), ImageDescriptor(0) {}

  void setup(uint16 width, uint16 height, const std::string &filename,
             uint16 xorigin, uint16 yorigin) {
    Width = width;
    Height = height;
    XOrigin = xorigin;
    YOrigin = yorigin;
  ...
  static uint16 getBlue(uint16 color16) { return (color16 & 31) << 3; }
  ...
  void set16(uint col, uint16 color16) { Line[col] = color16; }
...
public:
  ...
  void writeLine() {
    File.serialBuffer((uint8 *)(&(GreyLine[0])),
                      (uint)GreyLine.size() * sizeof(GreyLine[0]));
  }
};

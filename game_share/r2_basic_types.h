...
/// a very simple type to replace the uint32 session ids used evrywhere in order
/// to reduce bugs
class CSessionId {
public:
...
inline bool operator<(const CSessionId &lh, uint32 rh) {
  return lh.asInt() < rh;
}
inline bool operator==(const CSessionId &lh, uint32 rh) {
  return lh.asInt() == rh;
...
// useful for lh != 0
inline bool operator!=(const CSessionId &lh, int rh) {
  return lh.asInt() != uint32(rh);
}

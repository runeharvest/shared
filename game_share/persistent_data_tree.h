 ...
 *  It represents the contents of arbitrary CPersistentDataRecord records in a
 *tree structure that can be interrogated or written to / read from easy to read
 *text files
 *
 **/
...
public:
  ...
  // ctor
  CPersistentDataTreeNode(const NLMISC::CSString &name = NLMISC::CSString(),
                          CPersistentDataTreeNode *parent = NULL);
  CPersistentDataTreeNode(const NLMISC::CSString &name,
                          CPersistentDataTreeNode *parent, uint32 idx);
  ...
  // contains a value or if the branch contains children that are not map
  // entries
  bool flagAsMap();

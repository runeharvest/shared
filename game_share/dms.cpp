...
CDynamicMapService::CDynamicMapService(NLMISC::CConfigFile &confFile,
  ...
  {
    IModule *imodule =
        mm.createModule("ServerAnimationModule", "ServerAnimationModule", "");
  ...
  {
    IModule *imodule =
        mm.createModule("ServerAdminModule", "ServerAdminModule", "");
    nlassert(imodule != NULL);
  ...
  {
    IModule *imodule =
        mm.createModule("ServerEditionModule", "ServerEditionModule", "");
    nlassert(imodule != NULL);
  ...
  {
    IModule *imodule =
        mm.createModule("StringManagerModule", "StringManagerModule", "");
    nlassert(imodule != NULL);
...
CDynamicMapService *CDynamicMapService::getInstance() { return _Instance; }
...
void CDynamicMapService::init2() { _UseNetwork = true; }

void CDynamicMapService::translateAndForwardRequested(
...
void CDynamicMapService::forwardIncarnChat(TChanID id, TDataSetRow senderId,
                                           ucstring sentence) {
  _StringMgrModule->forwardIncarnChat(id, senderId, sentence);
}

IServerAnimationModule *CDynamicMapService::getAnimationModule() const {
  return _AnimationModule;
}

IServerEditionModule *CDynamicMapService::getEditionModule() const {
  return _EditionModule;
}

IServerAdminModule *CDynamicMapService::getAdminModule() const {
  return _AdminModule;
}

std::string CDynamicMapService::getValue(TSessionId sessionId,
...
bool getCharInfo(NLNET::IModuleProxy *senderModuleProxy, uint32 &charId,
  ...
  const NLNET::TSecurityData *securityData =
      senderModuleProxy->findSecurityData(rmst_client_info);
  ...
  if (securityData != NULL) {
    const struct TClientInfo *clientInfo =
        static_cast<const struct TClientInfo *>(securityData); // safe_cast ???
...
// same getCharInfo but return true only if client has been allowad by su to
// connect
bool checkSecurityInfo(NLNET::IModuleProxy *senderModuleProxy, uint32 &charId,
                       NLMISC::CEntityId &clientEid, std::string &userPriv,
                       std::string &extendedPriv) {
  bool ok =
      getCharInfo(senderModuleProxy, charId, clientEid, userPriv, extendedPriv);
  if (!ok) {
    nlwarning("Warning: Security issues: a client '%s' without security data "
              "try to connect... ",
              senderModuleProxy->getModuleName().c_str());
    return false;
  }
  IServerEditionModule *edition =
      CDynamicMapService::getInstance()->getEditionModule();
  if (!edition) {
    return false;
  }
  ok = edition->isClientAuthorized(charId);
  if (!ok) {
    nlwarning("Warning: A client '%s' '%s' %u '%s' try to send messages, "
              "without begin allowed (he was disconnected)",
              senderModuleProxy->getModuleName().c_str(),
              clientEid.toString().c_str(), charId, userPriv.c_str());

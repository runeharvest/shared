 ...
 */
static inline void addAndCheckOverFlow(TUInt &dest, TUInt src,
                                       TUInt &overflow) {
...
void CSeeds::tradeSubtract(TBigUInt toSubTotal) {
  ...
  CSeeds left;
  left.setTotal(1000 -
                (toSubTotal % 100)); // give back medium, little & big seeds
  add(left, overflow);
  nlassert(overflow.getTotal() == 0);
}
...
//=========================================================
bool CSeeds::canTradeSubtract(TBigUInt rhs) const { return rhs <= getTotal(); }
...
bool CSeeds::canSubtract(const CSeeds &other) const {
  return _LS >= other._LS && _MS >= other._MS && _BS >= other._BS &&
         _VBS >= other._VBS;
}

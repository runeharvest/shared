...
public:
   ...
   */
  static void setGameTimeStep(NLMISC::TGameTime gameTimeStep) {
    _GameTimeStep = gameTimeStep;
  }
   ...
   * Get the number of ellapsed game cycles
   */
  static void setGameCycle(NLMISC::TGameCycle gameCycle) {
  ...
  /**
   * initialize the callback Array and get a pointer to the service's specific
   * update function
   *
   * \param updateFunc will be call when we receive a new tick
   * \param syncFunc will be call when the ticks send the syncro
   * \param tockAtBeginOfTickUpdate indicates if we will send TOCK before
   * calling updateFunc() instead of after.
   */
  static void init(void (*updateFunc)(), void (*syncFunc)() = NULL,
                   bool tockAtBeginOfTickUpdate = false);

  /**
   * Set a callback to call when receiving the first game cycle (call this
   * method in your init()) \param syncFunc will be call when the ticks send the
   * syncro \param allowReplaceCallback true if we allow the callback to be
   * replaced \return previous callback if exist, otherwise NULL
   */
  static TUserSyncCallback setSyncCallback(TUserSyncCallback syncFunc,
                                           bool allowReplaceCallback = false);

  static bool getTockAtBeginOfTickUpdate() { return _TockAtBeginOfTickUpdate; }

private:
  /// Time according to the game (used for determining day, night...) (double in
  /// seconds)

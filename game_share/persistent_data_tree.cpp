...
/*
  ...
  NOTE: The file format for storing trees to disk is line based. It follows the
  following rules:

  NOTE too that this file format is intended as an alternative for storage of
  PDR data and that the PDR format doesn't differentiate between single values
  and array elements

  - Leading spaces are ignored when scanning lines, as are all spaces preceding
  the '=' or '==' signs
  ...
  - lines of format <path>'=='<value> contain values that will be stripped and
  unquoted if they were previously quoted
  ...
  #		class ClassTheWholeFile { vector<ClassA> a;
...
CPersistentDataTreeNode::CPersistentDataTreeNode(
    const NLMISC::CSString &name, CPersistentDataTreeNode *parent) {
  _Name = name;
  _Parent = NULL;
  _IsMap = false;
  _IsValue = false;
...
CPersistentDataTreeNode::CPersistentDataTreeNode(
    const NLMISC::CSString &name, CPersistentDataTreeNode *parent, uint32 idx) {
  _Name = name;
  _Parent = NULL;
  _IsMap = false;
  _IsValue = false;
...
bool CPersistentDataTreeNode::attachToParent(CPersistentDataTreeNode *parent) {
  return attachToParent(parent, parent == NULL
                                    ? std::numeric_limits<uint32>::max()
                                    : (uint32)parent->_Children.size());
}
...
bool CPersistentDataTreeNode::attachToParent(CPersistentDataTreeNode *parent,
  ...
  // check value of 'idx'
  BOMB_IF(
      parent != NULL && idx > 1024 * 1024,
      "Implausibly high number of children requested (" +
          NLMISC::toString("%d", idx) +
          ") for persistent data tree node: " + parent->getNodeName().c_str(),
      return false);

  // check for attachment to previous parent
  BOMB_IF(_Parent != NULL,
          "Attempting to attach a persistent data node to parent '" +
              (parent == NULL ? "NULL" : parent->getNodeName()) +
              "' when it's already attached to another parent as: " +
              getNodeName().c_str(),
          return false);

  // split the name into its component parts
  CSString mapIndex = _Name;
  CSString nameBase = cleanQuotes(
      mapIndex.splitToSeparator('#', true, false, true, true, false));
  ...
  if (parent != NULL) {
    ...
    BOMB_IF(parent->_IsValue,
            "Attempting to attach a persistent data node to parent that has a "
            "value '" +
                parent->getNodeName() + "' = " + parent->_Value.c_str(),
            return false);

    if (hasExplicitIndex) {
      // there is an explicit '#' number so ensure that it really is followed by
      // a valid number
      sint32 num = mapIndex.atoi();
      if (NLMISC::toString("%u", num) != mapIndex) {
        WARN("WARNING: Strange node name detected after last '#' - truncating "
             "and replacing with a number: " +
             getNodeName());
        hasExplicitIndex = false;
      } else {
        // if this is the highest '#' number for this root node name then ajust
        // the _NextPoundValue value
        if (num >= parent->_NextPoundValue[nameBase]) {
          parent->_NextPoundValue[nameBase] = num + 1;
        }

        // ensure that there isn't already a node with the same '#' value
        // attached to the same parent
        DROP_IF(parent->_ChildIndex.find(nameBase + '#' + mapIndex.c_str()) !=
                    parent->_ChildIndex.end(),
                "Failed to add child '" + _Name + "' to parent '" +
                    parent->getNodeName().c_str() +
                    "' because another child of same name already exists",
                return false);
      }
    }
    if (!hasExplicitIndex) {
      // we don't have an explicit '#' number in the name, so we need to
      // generate one
    ...
    // setup own _Parent property and ensure that there are no trailing spaces
    // round the _Name
    _Parent = parent;
    ...
    // ensure that there isn't another child already assigned to this parent
    // slot
    BOMB_IF(_Parent->_Children[idx] != NULL,
            "Ignoring attempt to add second child to same slot (" +
                NLMISC::toString("%d", idx) +
                ") in persistent data tree node's children: " +
                _Parent->getNodeName().c_str(),
            return false);
...
bool CPersistentDataTreeNode::readFromPdr(CPersistentDataRecord &pdr) {
  static uint16 mapKeyToken = std::numeric_limits<uint16>::max();
  pdr.addString("__Key__", mapKeyToken);
  static uint16 mapValToken = std::numeric_limits<uint16>::max();
  pdr.addString("__Val__", mapValToken);
  ...
  while (!pdr.isEndOfData()) {
    if (pdr.isStartOfStruct()) {
      ...
      DROP_IF(pdr.peekNextToken() != token,
              "ERROR: End of " + pdr.lookupString(token) +
                  " block expected but not found at: " + getNodeName().c_str(),
              return false);
      ...
      if (nextToken == mapKeyToken) {
        // we've matched a map key so we need to extract both key and related
        ...
        // extract the map key and ensure that it's followed by a valid __Val__
        // entry
        CSString mapKey;
        pdr.pop(mapKeyToken, mapKey);
        DROP_IF(pdr.isEndOfData() || pdr.peekNextToken() != mapValToken,
                "ERROR: Ignoring map key (__Key__) because __Val__ token "
                "expected but not found at: " +
                    getNodeName() + ":" + mapKey.c_str(),
                continue);
        ...
        if (pdr.isStartOfStruct()) {
          ...
          DROP_IF(pdr.peekNextToken() != mapValToken,
                  "ERROR: End of __Val__ block expected but not found at: " +
                      getNodeName() + ":" + mapKey.c_str(),
                  return false);
...
static bool isFloat(double d, float f) { return (double(f) == d); }

static void pdrPushCompactValue(CPersistentDataRecord &pdr, uint16 token,
                                const NLMISC::CSString &value) {
  // if this is an empty string or doesn't begin with a valid numeric character
  // then just push it to the pdr and return
  ...
  if (value[0] == '-') {
    ...
    if (si64 != 0) {
      // we can store this value as a signed int so decide how big an int we
      // need
...
bool CPersistentDataTreeNode::writeToPdr(CPersistentDataRecord &pdr) const {
  ...
  if (_IsValue) {
    ...
    for (uint32 i = 0; i < _Children.size() && ok; ++i) {
      BOMB_IF(_Children[i] == NULL,
              "ERROR: NULL pointer in persistent data tree node children",
              return false);
...
const CSString &CPersistentDataTreeNode::getName() const { return _Name; }
...
CSString CPersistentDataTreeNode::getNodeName() const {
  ...
  // check to see whether there are more then one child in our parent that match
  // our base name
  if (_Parent->_NextPoundValue[name] > 1) {
    // other siblings exist so we need to retain th full name including the '#'
    // segment
    name = _Name;
  }
  ...
  if (parentName.empty())
    return name;
  if (isMapEntry())
    return parentName + ":" + name.c_str();
  else
    return parentName + "." + name.c_str();
}

void CPersistentDataTreeNode::setValue(const TValue &value) {
  DROP_IF(!_Children.empty(),
          "Ignoring attempt to set value of a node that has children: " +
              getNodeName(),
          return);
  _Value = value;
  _IsValue = true;
...
const CPersistentDataTreeNode::TValue &
CPersistentDataTreeNode::getValue() const {
  WARN_IF(
      !_IsValue,
      "Attempting to get value of a node that isn't flagged as being one: " +
          getNodeName());
  return _Value;
}

const CPersistentDataTreeNode::TValue &
CPersistentDataTreeNode::getValue(const CSString &nameList) const {
  // get hold of the named node, or NULL if it doesn't exist
  CPersistentDataTreeNode *node =
      const_cast<CPersistentDataTreeNode *>(this)->getDescendant(nameList,
                                                                 false);

  // check whether the node exists
  static TValue nullValue;
  //	DROP_IF(node==NULL,"Failed to locate node in
  //getValue("+nameList+")",return nullValue);
  if (node == NULL)
    return nullValue;
...
bool CPersistentDataTreeNode::flagAsMap() {
  DROP_IF(_IsValue,
          "ERROR: Failed to flag persistent data tree node '" + getNodeName() +
              "' as a map as it already has a value: " + _Value.c_str(),
          return false);
  ...
  DROP_IF(!_Children.empty(),
          "ERROR: Failed to flag persistent data tree node '" + getNodeName() +
              "' as a map as it already has children: ",
          return false);
...
const CPersistentDataTreeNode *
CPersistentDataTreeNode::getChild(const NLMISC::CSString &name) const {
...
const CPersistentDataTreeNode::TChildren &
CPersistentDataTreeNode::getChildren() const {
  WARN_IF(!_Value.empty(),
          "Attempting to access children of a node that has a value: " +
              getNodeName());
  return _Children;
}

CPersistentDataTreeNode *
CPersistentDataTreeNode::getDescendant(const NLMISC::CSString &nameList,
  ...
  // extract the next chunk from the name list and get a pointer to the
  // identified node
  CSString nextChunk =
      s.splitToSeparator('.', true, false, true, true, true).strip();

  //  break the chunk into elements and perform a syntax check
  CVectorSString elements;
  nextChunk.splitByOneOfSeparators("#:", elements, false, true, true, true,
                                   false);
  switch (elements.size()) {
  case 7:
    DROP_IF(elements[1] != '#' || elements[3] != ':' || elements[5] != '#',
            "SYNTAX ERROR in persistent data tree node name: " + nextChunk,
            return NULL);
    break;
  case 5:
    DROP_IF(elements[3] != '#' && elements[3] != ':',
            "SYNTAX ERROR in persistent data tree node name: " + nextChunk,
            return NULL);
    DROP_IF(elements[1] == ':' && elements[3] == ':',
            "SYNTAX ERROR in persistent data tree node name: " + nextChunk,
            return NULL);
    // drop through (no break)
  case 3:
    DROP_IF(elements[1] != '#' && elements[1] != ':',
            "SYNTAX ERROR in persistent data tree node name: " + nextChunk,
            return NULL);
    // drop through (no break)
  case 1:
    break;
  default:
    DROP("SYNTAX ERROR in persistent data tree node name: " + nextChunk,
         return NULL);
  }
  ...
  if (elements.size() >= 5 || (elements.size() >= 3 && elements[1] == ":")) {
    ...
    // this is a true map entry so it's composed of 2 parts ... we treat the
    // first part here and drop through to the normal code to treat the second
    // part
...
bool CPersistentDataTreeNode::operator==(
    const CPersistentDataTreeNode &other) const {
...
bool CPersistentDataTree::readFromBuffer(const CSString &buffer) {
  ...
  for (uint32 i = 0; i < lines.size(); ++i) {
    ...
    CSString s1 = line;
    CSString s0 =
        s1.splitToSeparator('=', true, false, true, true, false).strip();
    s1 = s1.leftStrip();
    ...
    CSString mapName =
        childName.splitToSeparator(':', true, false, true, true, true);
    if (childName.empty()) {
      childName = mapName;
      mapName.clear();
    } else {
    ...
    // get hold of a pointer to the parent node, creating new tree structure if
    // need be
    CPersistentDataTreeNode *parent = _Child->getDescendant(parentName, true);
    DROP_IF(parent == NULL,
            "ERROR Failed to get handle to node in readFromBuffer: " +
                parentName,
            return false);
    ...
    // note that by using a smart ptr (TNodePtr) we avoid memory leakage if
    // attach fails
    CPersistentDataTree::TNodePtr child =
        new CPersistentDataTreeNode(childName, parent);

    // if there is a value then add it
    if (!s1.empty()) {
      // note that we use the 2 parameter version of setValue here and NOT
      // child->setValue because it is possible that the attempt to attach a
      // child failed and that there is another node in the tree that will
      // accept the 'setValue()' instead
      if (s1.left(2) == "==")
...
bool CPersistentDataTree::readFromFile(const CSString &fileName) {
  ...
  DROP_IF(buffer.empty(),
          "Failed to read input file for persistent data tree: " + fileName,
          return false);
...
bool CPersistentDataTree::writeToFile(const CSString &fileName) const {
  ...
  // in the case of an error display a warning and drop throught to write an
  // empty buffer to the output file
  WARN_IF(!ok, "Failed to build output buffer for persistent data tree to "
               "write to file: " +
                   fileName);
...
bool CPersistentDataTree::writeToPdr(CPersistentDataRecord &pdr) const {
  ...
  for (uint32 i = 0; i < _Child->getChildren().size() && ok; ++i) {
    BOMB_IF(_Child->getChildren()[i] == NULL,
            "ERROR: NULL pointer in persistent data tree node children",
            return false);
    ok = _Child->getChildren()[i]->writeToPdr(pdr);
  }

  // if an error occurred during write then clear out the result pdr and return
  // false
  if (!ok)
    pdr.clear();
...
void CPersistentDataTree::setValue(const TValue &nodeName,
  ...
  DROP_IF(node == NULL,
          "ERROR: Failed get to handle to node in order to set value: " +
              nodeName,
          return);
...
const CPersistentDataTree::TValue &
CPersistentDataTree::getValue(const CSString &nodeName) const {
  return _Child->getValue(nodeName);
}

CPersistentDataTreeNode *
CPersistentDataTree::getNode(const NLMISC::CSString &nodeName) {

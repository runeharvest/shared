...
inline bool CMirroredDataSet::isAccessible(const TDataSetRow &row) const
{
	return //(! row.isNull()) && // guaranteed by (row.getIndex() < maxNbRows()) *as long as the index is unsigned*
	    (row.getIndex() < maxNbRows()) && (_PropertyContainer.EntityIdArray.isOnline(row.getIndex())) && ((row.counter() == _PropertyContainer.EntityIdArray.Counts[row.getIndex()]) || (row.counter() == 0));
}
...
{
	return //(! row.isNull()) && // guaranteed by (row.getIndex() < maxNbRows()) *as long as the index is unsigned*
	    (row.getIndex() < maxNbRows()) && ((row.counter() == _PropertyContainer.EntityIdArray.Counts[row.getIndex()]) || (row.counter() == 0));
}

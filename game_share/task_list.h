...
//
//
template <typename T> class CTask : public NLMISC::CRefCount {
public:
...
template <typename T> class CTaskHolder {
public:
...
template <typename T> class CTaskHolderLess {
public:
...
//
// Manages a list of Task that must be executed at a specific time (NLMISC::Time
// or tick) It is not a scheduler (the priority of task can not be dynamicaly
// changed) If 2 task are added at the same time, it is garenty that the first
// task will be donne before the seconde one ( so internal sort are stable)
template <typename T> class CTaskList {

public:
  // execute all task that are waiting
  // now is the current time
  // eg CTimeInterface::gameCycle(), if T == tick  or
  ...
  void addTask(CTask<T> *task) {
    _Tasks.push_back(task);
    _Clean = false;
  }
  void addTaskAt(T t, CTask<T> *task) {
    task->setTime(t);
    _Tasks.push_back(task);
    _Clean = false;
  }
  uint32 getSize() const { return (uint32)_Tasks.size(); }
  CTask<T> *getTaskAt(uint32 index) const {
    nlassert(index < _Tasks.size());
    return _Tasks[index].getTask();
  }
  void removeTaskAt(uint32 index) {
    nlassert(index < _Tasks.size());
...
// Implementation
template <typename T> void CTaskList<T>::execute(T now) {
...
template <typename T> CTaskList<T>::~CTaskList() {
  //	_Tasks.clear();
}

...
public:
  ...
  virtual void createSessionWithoutSu(uint32 charId,
                                      NLMISC::CEntityId clientEid) = 0;
  virtual TPioneersSessionsAllowed *
  getSessionAllowedForChar(TCharId charId) const = 0;
  ...
  // editing and animation mode)
  virtual bool getPosition(TSessionId sessionId, double &x, double &y,
                           double &orient, uint8 &season,
                           uint32 locationIndex = 0) = 0;
  ...
  virtual const NLNET::TModuleProxyPtr *
  getClientProxyPtr(TCharId charId) const = 0;
  ...
  // clients
  virtual bool wakeUpSession(TSessionId sessionId, TCharId ownerId,
                             std::string &msg) = 0;
  ...
  virtual void getStartParams(uint32 charId,
...
public:
  ...
  virtual bool getPosition(TSessionId sessionId, double &x, double &y,
                           ...
                           uint32 locationIndex = 0) = 0;
  virtual void
  scheduleStartSession(const CAnimationMessageAnimationStart &msg) = 0;
  ...
  virtual bool getConnectedChars(TSessionId sessionId,
                                 std::vector<TCharId> &chars) const = 0;
  ...
  virtual void setSessionStartParams(TSessionId sessionId, sint32 x, sint32 y,
                                     uint8 season) = 0;
  virtual bool mustReloadPosition(TSessionId sessionId,
                                  TCharId charId) const = 0;
  virtual bool
  getHeaderInfo(TSessionId sessionId,
                TScenarioHeaderSerializer::TValueType &values) const = 0;
  virtual bool getScore(TSessionId sessionId, uint32 &score,
...
class CDynamicMapService {
public:
  CDynamicMapService(NLMISC::CConfigFile &confFile,
                     NLNET::IModuleSocket *gateway);
  ...
  virtual void translateAndForwardRequested(TDataSetRow senderId,
...
bool getCharInfo(NLNET::IModuleProxy *senderModuleProxy, uint32 &charId,
                 NLMISC::CEntityId &clientEid, std::string &userPriv,
                 std::string &extendedPriv);

// get the security info of the module and verify that the su has allowed the
// client to connect to the dss
bool checkSecurityInfo(NLNET::IModuleProxy *senderModuleProxy, uint32 &charId,
                       NLMISC::CEntityId &clientEid, std::string &userPriv,
                       std::string &extendedPriv);

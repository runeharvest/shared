...
{
	return fam == ARMOR || fam == MELEE_WEAPON || fam == RANGE_WEAPON || fam == AMMO || fam == RAW_MATERIAL || fam == SHIELD || fam == JEWELRY || fam == CRYSTALLIZED_SPELL || fam == CONSUMABLE || fam == ITEM_SAP_RECHARGE;
}

bool isTextCustomizable(EItemFamily fam)
{
	return fam == ARMOR || fam == MELEE_WEAPON || fam == RANGE_WEAPON || fam == SHIELD || fam == JEWELRY || fam == SCROLL;
}

...
void CShardNames::init(NLMISC::CConfigFile &configFile) {
  ...
  if (sessionNames == NULL) {
    nlwarning("No variable 'HomeMainlandNames', domain unified character names "
    ...
    nlinfo("Read %u home session names from config files",
           _SessionNames.size());
  }

  // read the var to append or not (). Use DontUseSU one for simplicity (but
...
// ***************************************************************************
void CShardNames::parseRelativeName(TSessionId contextSessionId,
                                    const string &inputCharName,
                                    string &outCharName,
  ...
  if (pos != string::npos) {
    ...
    if (pos == string::npos) {
      ...
      string sessionName =
          inputCharName.substr(pos + 1, inputCharName.size() - pos - 2);
...
// ***************************************************************************
std::string CShardNames::makeFullName(const std::string &charName,
...
// ***************************************************************************
std::string
CShardNames::makeFullNameFromRelative(TSessionId contextSessionId,

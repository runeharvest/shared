 ...
 * Constructor
 */
CGenericXmlMsgHeaderManager::CGenericXmlMsgHeaderManager() { _Root = NULL; }
...
void CGenericXmlMsgHeaderManager::init(const string &filename) {
  ...
  if (!file.open(filename)) {
    nlwarning(
        "Cannot open xml file '%s', unable to initialize generic messages",
        filename.c_str());
    return;
  }
...
// set callback
bool CGenericXmlMsgHeaderManager::setCallback(const string &msgName,
                                              TMsgHeaderCallback callback) {
  // check root
  if (_Root == NULL) {
    nlwarning(
        "Can't set callback for message '%s', Root not properly initialized.",
        msgName.c_str());
    return false;
  }
  ...
  // check node
  if (node == NULL) {
    nlwarning("Can't set callback for message '%s', message not found.",
              msgName.c_str());
    return false;
  }
...
bool CGenericXmlMsgHeaderManager::setUserData(const string &msgName,
                                              const uint64 &data, uint index) {
  // check root
  if (_Root == NULL) {
    nlwarning(
        "Can't set user data for message '%s', Root not properly initialized.",
        msgName.c_str());
    return false;
  }
  ...
  // check node
  if (node == NULL) {
    nlwarning("Can't set user data for message '%s', message not found.",
              msgName.c_str());
    return false;
  }
...
//
bool CGenericXmlMsgHeaderManager::pushNameToStream(const string &msgName,
                                                   CBitMemStream &strm) {
  bool res = (_Root->select(msgName.c_str(), strm) != NULL);

  if (!res)
    nlwarning("pushNameToStream failed: Unknown message name '%s'",
              msgName.c_str());
...
//
bool CGenericXmlMsgHeaderManager::pushNameToStream(const char *msgName,
                                                   CBitMemStream &strm) {
  bool res = (_Root->select(msgName, strm) != NULL);

  if (!res)
    nlwarning("pushNameToStream failed: Unknown message name '%s'", msgName);

  return res;
}

//
void CGenericXmlMsgHeaderManager::popNameFromStream(string &resultName,
                                                    CBitMemStream &strm) {
  _Root->select(strm, resultName);
}

//
void CGenericXmlMsgHeaderManager::popNameAndDescriptionFromStream(
    string &resultName, string &description, CBitMemStream &strm) {
...
CGenericXmlMsgHeaderManager::CNode::CNode(xmlNodePtr xmlNode, uint32 value)
    : Value(value), UseCycle(false), NbBits(0), Callback(NULL) {
  UserData[0] = 0;
  UserData[1] = 0;
  UserData[2] = 0;
  UserData[3] = 0;
  ...
  if (!strcmp((const char *)xmlNode->name, "leaf")) {
      ...
      while (*scan != '\0') {
        ...
        case 's': {
          ...
          if (numBits == 0)
            ...
            nlwarning("Can't use sint in format with other size than 8, 16, 32 "
                      "or 64");
          }
        } break;
        case 'u': {
    ...
    while (xmlChild != NULL) {
      ...
      if (!strcmp((const char *)xmlChild->name, "branch") ||
          !strcmp((const char *)xmlChild->name, "leaf")) {
        ...
        if (NodesByName.find(child->Name) == NodesByName.end()) {
          ...
          nlwarning("Child '%s' in node '%s' already exists, unable to add it",

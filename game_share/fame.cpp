...
CVariable<bool> UseAsymmetricStaticFames(
    "variables", "UseAsymmetricStaticFames",
    "if 1, fame between faction A and B may be different that between B and A",
    false, 0, true);
...
//----------------------------------------------------------------------------
NLMISC_COMMAND(displayStaticFames, "Display the fame for each factions",
               "[<faction>]") {
  if (args.size() > 1)
    return false;
    ...
    for (uint j = 0; j < names.size(); ++j) {
      ...
      if (fame != 0)
        log.displayNL("Fame between %s and %s : %7.3f",
                      ...
                      sf.getFactionName(j).c_str(), fame / 1000.0f);
    }
  }
...
//----------------------------------------------------------------------------
NLMISC_COMMAND(displayFame,
               "List the fame value between an entity and one or all faction.",
               "<entityId> [<detail>] [<factionIndex|factionName>]* ") {
  if (args.size() < 1)
    return false;
  ...
  if (!fi.getFameDataSet()->isAccessible(dsr)) {
    log.display("Invalid entity id '%s', can't map in fame dataset",
                args[0].c_str());
    return false;
  }

  const vector<TStringId> factionNames =
  ...
  for (uint i = (detail ? 2 : 1); i < args.size(); ++i) {
    ...
    if (factionIndex == CStaticFames::INVALID_FACTION_INDEX) {
      log.display("Invalid faction '%s'.\nTry 'displayFactions' to list the "
                  "available factions.",
                  args[i].c_str());
      return false;
    }
  ...
  for (uint i = 0; i < factionNames.size(); /*MAX_FAME_OWNER;*/ ++i) {
    ...
    if (factionFilter.empty() ||
        find(factionFilter.begin(), factionFilter.end(), i) !=
            factionFilter.end()) {
      ...
      if (!detail) {
        ...
        if (!factionNames.empty()) {
          if (fame == NO_FAME)
            log.displayNL("NO Fame between \t%s and \t%s",
                          eid.toString().c_str(), factionName.c_str());
          else
            log.displayNL("Fame between %s and %35s = %7.3f",
                          eid.toString().c_str(), factionName.c_str(),
                          fame / 1000.0f);
        }
      } else {
        ...
        */
        ...
        case RYZOMID::player: {
          ...
          fame = fi.getFameIndexed(fi.getFameDataSet()->getEntityId(mdsr), i,
                                   false, true);
        ...
        case RYZOMID::guild:
          if (fi.getFameDataSet()->isAccessible(gdsr)) {
            sint32 fame = fi.getFameIndexed(
                fi.getFameDataSet()->getEntityId(gdsr), i, false, true);
        ...
        case RYZOMID::civilisation:
          if (fi.getFameDataSet()->isAccessible(cdsr)) {
            sint32 fame = fi.getFameIndexed(
                fi.getFameDataSet()->getEntityId(cdsr), i, false, true);
        ...
        // display the result
        log.displayNL(
            "Fame between \t%s and %35s = P: %7s, G: %7s, M: %7s, C: %7s",
            eid.toString().c_str(), factionName.c_str(), fp.c_str(), fg.c_str(),
            fm.c_str(), fc.c_str());
      }
    }
  }
...
NLMISC_COMMAND(addFame, "Add some fame value between an entity and a faction.",
               "<entityId> <factionIndex|factionName> <fameDelta>") {
  if (args.size() != 3)
    return false;
  ...
  TDataSetRow dsr =
      CFameInterface::getInstance().getFameDataSet()->getDataSetRow(eid);
...
void CStaticFames::loadStaticFame(const string &filename) {
  ...
  if (path.empty()) {
    nlwarning("FAME: Error can't find static fame file '%s' ! No static fame "
              "data available.",
              filename.c_str());
    return;
  }
  ...
  if (loadExcelSheet(path, ws, false)) {
    ...
    for (uint i = 2; i < ws.ColCount; ++i) {
      ...
      if (name.empty()) {
        if (i - 2 != _FactionNameIndex.size()) {
          nlwarning("FAME: Error while reading fame file '%s' : missing column "
                    "identifier, found only %u column, need %u !",
                    path.c_str(), i, _FactionNameIndex.size());
      ...
      TFactionNameIndexList::iterator it(
          _FactionNameIndex.find(CStringMapper::map(name)));
      if (it == _FactionNameIndex.end()) {
        nlwarning("FAME: Error while reading fame file '%s' : column "
                  "identifier '%s' not found in row identifiers !",
      ...
      if (it->second != i - 2) {
        nlwarning(
            "FAME: Error while reading fame file '%s' : identifier '%s' is in "
            "row %u but in column %u, column and row index must match !",
            path.c_str(), name.c_str(), it->second, i - 2);
        nlwarning("FAME: The faction data will not be available !");
    ...
    for (uint i = 1; i < _FameTableSize + 1; ++i) {
      ...
      for (uint j = 2; j < _FameTableSize + 2; ++j) {
        ...
        if (fame != 0) {
          /*					nldebug ("FAME: Setting fame
             between '%s' and '%s' to value %f, propagation factor %f",
    ...
    nlwarning("FAME: Error while reading the static fame file '%s' ! No static "
              "fame data available.",
              path.c_str());
    return;
  }
  ...
  nlinfo("FAME: Loaded fame for %u factions, total of %u fame values",
         _FameTableSize, _FameTableSize * _FameTableSize);
}
...
void CStaticFames::loadTribeThreshold(const string &filename) {
  string path = CPath::lookup(filename, false);
  if (path.empty()) {
    nlwarning("FAME: Error can't find tribe threshold fame file '%s' ! No "
              "tribe threshold available.",
              filename.c_str());
    return;
  }
  ...
  if (loadExcelSheet(path, ws, false)) {
    ...
    nlassert(ws.ColCount ==
             16); // 5 ( 4 people + neutral ) * 3 cult + 1 for tribe name
    ...
    for (uint i = 2; i < ws.size(); ++i) {
      ...
      for (uint c = 1; c < ws.ColCount; c += 3) {
        ...
        // This message removed by Sadge because there is no context displayed,
        // meaning that message must be useless nldebug(" %s", ws.getData(i,
        // c).toString().c_str() );
      }
    }
  }
...
//----------------------------------------------------------------------------
sint32 CStaticFames::getStaticFameIndexed(uint factionIndex1,
                                          uint factionIndex2) {
...
//----------------------------------------------------------------------------
float CStaticFames::getPropagationFactorIndexed(uint factionIndex1,
                                                uint factionIndex2) const {
  if ((factionIndex1 < factionIndex2) && !UseAsymmetricStaticFames)
    swap(factionIndex1, factionIndex2);

  if (factionIndex1 >= _FameTableSize || factionIndex2 >= _FameTableSize) {
    nlwarning("FAME: CStaticFames::getPropagationFactorIndexed invalid "
              "faction, return 0");
    return 0.0f;
  }
  ...
  return _PropagationFactorTable[factionIndex1 * _FameTableSize +
                                 factionIndex2];
}

//----------------------------------------------------------------------------
sint32 CStaticFames::getStaticFame(NLMISC::TStringId faction1,
                                   NLMISC::TStringId faction2) {
  uint index1, index2;
...
//----------------------------------------------------------------------------
sint32 CStaticFames::getStaticFame(const std::string &faction1,
                                   const std::string &faction2) {
  return getStaticFame(CStringMapper::map(toLowerAscii(faction1)),
                       CStringMapper::map(toLowerAscii(faction2)));
}

//----------------------------------------------------------------------------
sint32 CFameInterface::getFameIndexed(const CEntityId &entityId,
                                      uint32 factionIndex, bool modulated,
                                      bool returnUnknownValue) {
  if (_FameOverload) {
    return _FameOverload->getFameIndexed(entityId, factionIndex, modulated,
                                         returnUnknownValue);
  } else {
    ...
    if (it == _FamesOwners.end()) {
      //			nlwarning("FAME: entity %x doesn't exist in fame
      //owners containers!", entityIndex.getIndex());
      return 0;
    }

    if (factionIndex >= MAX_FACTION) {
      nlwarning("FAME: entity %s : faction %u is out of limit (%u)",
                entityId.toString().c_str(), factionIndex, MAX_FACTION);
      return 0;
    }
    ...
    // The return values *should* be clamped here, but the values are stored in
    // the
...
//----------------------------------------------------------------------------
sint32 CFameInterface::getFame(const CEntityId &entityId, TStringId faction,
                               bool modulated, bool returnUnknowValue) {
  CStaticFames &staticFame = CStaticFames::getInstance();
...
//----------------------------------------------------------------------------
void CFameInterface::addFame(const CEntityId &entityId,
                             NLMISC::TStringId faction, sint32 deltaFame,
                             bool propagate) {
  // get the fame faction index
  CStaticFames &staticFame = CStaticFames::getInstance();
  uint32 oi = staticFame.getFactionIndex(faction);
  if (oi == CStaticFames::INVALID_FACTION_INDEX) {
    nlwarning("FAME:addFame(): trying to add fame to faction '%s'(stringId=%u) "
              "which is not a valid faction name !",
              CStringMapper::unmap(faction).c_str(), faction);
...
//----------------------------------------------------------------------------
void CFameInterface::addFameIndexed(const CEntityId &entityId,
                                    uint32 factionIndex, sint32 deltaFame,
                                    bool propagate) {
  if (_FameOverload) {
    ...
    if (it == _FamesOwners.end()) {
      nlwarning("FAME:addFameIndexed(): can't find info for entity "
                "%s(index=%u) in _FamesOwners, can't add fame",
                entityId.toString().c_str(), dsr.toString().c_str());
      return;
    }

    if (factionIndex >= MAX_FACTION) {
      nlwarning("FAME:addFameIndexed(): trying to add fame for entity %s to an "
                "invalid faction %u, can't add fame",
                entityId.toString().c_str(), factionIndex);
      return;
    }
...
//----------------------------------------------------------------------------
const TDataSetRow &
CFameInterface::getCivilisationIndex(const CEntityId &entityId) {
...
//----------------------------------------------------------------------------
const TDataSetRow &
CFameInterface::getFameMemoryIndex(const CEntityId &entityId) {
  static const TDataSetRow invalidIndex;
...
//----------------------------------------------------------------------------
void CFameInterface::setFameDataSet(CMirroredDataSet *fameDataSet,
                                    bool declareProperty) {
  ...
  TFameOwner::CivilisationPropIndex =
      _FameDataSet->getPropertyIndex("Civilisation");
  TFameOwner::GuildPropIndex = _FameDataSet->getPropertyIndex("Guild");
  TFameOwner::FameMemoryPropIndex =
      _FameDataSet->getPropertyIndex("FameMemory");
  TFameOwner::FirstFamePropIndex = _FameDataSet->getPropertyIndex("Fame_0");

  // set display call backs
  _FameDataSet->setDisplayCallback(TFameOwner::CivilisationPropIndex,
                                   RowIdToStringCb);
  _FameDataSet->setDisplayCallback(TFameOwner::GuildPropIndex, RowIdToStringCb);
  _FameDataSet->setDisplayCallback(TFameOwner::FameMemoryPropIndex,
                                   RowIdToStringCb);

  for (uint i = 0; i < MAX_FACTION; ++i)
    _FameDataSet->setDisplayCallback(TFameOwner::FirstFamePropIndex + i,
                                     FameToStringCb);
}

...
class CTestContext {
public:
  CTestContext(const NLMISC::CSString &title,
               const NLMISC::CSString &anticipatedResult) {
    _Title = title;
  ...
  virtual ~CTestContext() {
    ...
    if (!_Errors && _Index != _Lines.size()) {
      ...
      if (NLMISC::WarningLog != NULL) {
        nlwarning("TEST FAILED: No more output but expecting: %s",
                  _Lines[_Index].c_str());
      }
    }
    ...
    if (NLMISC::InfoLog != NULL) {
      nlinfo("TEST: %s: %s", _Title.c_str(),
             (_Errors
                  ? (NLMISC::toString("FAILED AT LINE %d", _ErrorLine).c_str())
                  : "SUCCESS"));
      if (_Errors) {
        ...
        for (line = 0; line < _Lines.size() && line < _Result.size(); ++line) {
          nlinfo("%4d %-*s %s", line, lineLength, _Lines[line].c_str(),
                 _Result[line].c_str());
        }
  ...
  static void testOutput(const char *file, uint32 line,
                         const NLMISC::CSString &txt) {
    // if we're not in a test context then just quit
    if (_getTopContext() == NULL) {
#ifdef NL_DEBUG
      NLMISC::DebugLog->displayNL("%s: %d: Test output with no context: %s",
                                  file, line, txt.c_str());
#endif
      return;
    }
...
protected:
  virtual void _testOutput(const char *file, uint32 line,
                           const NLMISC::CSString &txt) {
    ...
    if (_Errors) {
      NLMISC::DebugLog->displayNL("%s: %d: After error: %s", file, line,
                                  txt.c_str());
      return;
    }
    ...
    if (_Index >= _Lines.size()) {
      _Errors = true;
      NLMISC::WarningLog->displayNL(
          "%s: %d: TEST FAILED: Found extra output after end of test: %s", file,
          line, txt.c_str());
      _ErrorLine = _Index;
      return;
    }
...
#define DOTEST(cmd, result)                                                    \
  {                                                                            \
    CTestContext tc(NLMISC::toString("TEST("__FILE__                           \
                                     ":%d)",                                   \
                                     __LINE__),                                \
                    result);                                                   \
    cmd;                                                                       \
    TEST_ASSERT(!tc.foundErrors())                                             \
  }
#define MUTETEST(cmd)                                                          \
  {                                                                            \
    CTestContextMute tcm;                                                      \
    cmd;                                                                       \
  }
#define TEST(txt_and_args)                                                     \
  CTestContext::testOutput(__FILE__, __LINE__, NLMISC::toString txt_and_args)

...
public:
  ...
  RYZOM_STARTING_POINT::TStartPoint
      StartPoint; // enum of starting point choosing by player
  ...
  void serialBitMemStream(NLMISC::CBitMemStream &f) {
    ...
    // color for equipement slots (Only for pre-equipped perso created with
    // sheet)
...
class CUserCharMsg {
public:
  static void write(NLMISC::CBitMemStream &s, const COfflineEntityState &state,
                    uint8 season, uint32 userRole, bool isInRingSession,
                    TSessionId highestMainlandSessionId,
                    uint32 firstConnectedTime, uint32 playedTime) {
  ...
  static void read(NLMISC::CBitMemStream &s, COfflineEntityState &state,
                   uint8 &season, uint32 &userRole, bool &isInRingSession,
                   TSessionId &highestMainlandSessionId,
                   uint32 &firstConnectedTime, uint32 &playedTime) {
...
public:
  ...
  CRespawnPointsMsg() { NeedToReset = false; }
...
public:
  ...
  void serial(NLMISC::CBitMemStream &f) { f.serialCont(FactionWarOccurs); }
};
...
const NLMISC::TGameCycle DefaultClientNPCIconRefreshTimerDelayGC =
    60 * 10; // 1 min
} // namespace NPC_ICON

...
public:
  ...
  // if we're already ready then we should call
  // CBackupInterfaceSingleton::connect() here
  ...
  virtual void
  dispatchRequestFileClass(const std::string &bsiname, uint32 requestId,
                           const std::string &directory,
                           const std::vector<CBackupFileClass> &classes) = 0;

  // send a request for info on files matching one or more patterns
  // (synchrously)
  virtual void
  dispatchSyncLoadFileClass(const std::string &bsiname, uint32 requestId,
                            const std::string &directory,
                            const std::vector<CBackupFileClass> &classes) = 0;

  // request a file (this is a rad)
  virtual void dispatchRequestFile(const std::string &bsiname, uint32 requestId,
                                   const std::string &fileName) = 0;

  // load a file (synchronously)
  virtual void dispatchSyncLoadFile(const std::string &bsiname,
                                    uint32 requestId,
                                    const std::string &fileName,
                                    bool notBlocking) = 0;

  // terminate all synchronous file loading pending (to terminate a synchronous
  // multi file loading)
  virtual void terminateSyncLoads() = 0;

  // send a file (this is a write)
  virtual void dispatchSendFile(const std::string &bsiname, uint32 requestId,
                                const CBackupMsgSaveFile &msg) = 0;

  // append binary data blob to a file (create file if need be)
  virtual void dispatchAppendData(const std::string &bsiname, uint32 requestId,
                                  const CBackupMsgSaveFile &msg) = 0;

  // append text line to a file (create file if need be)
  virtual void dispatchAppendText(const std::string &bsiname, uint32 requestId,
                                  const std::string &filename,
                                  const std::string &line) = 0;

  // delete a file (and store away a copy if required)
  virtual void dispatchDeleteFile(const std::string &bsiname, uint32 requestId,
                                  const std::string &fileToDelete,
                                  bool keepBackupOfFile) = 0;
...
class CBSIINonModule : public IBackupServiceInterfaceImplementation,
                       public NLMISC::CSingleton<CBSIINonModule>,
...
public:
  ...
  void dispatchRequestFileClass(const std::string &bsiname, uint32 requestId,
                                const std::string &directory,
                                const std::vector<CBackupFileClass> &classes);
  void dispatchSyncLoadFileClass(const std::string &bsiname, uint32 requestId,
                                 const std::string &directory,
                                 const std::vector<CBackupFileClass> &classes);
  void dispatchRequestFile(const std::string &bsiname, uint32 requestId,
                           const std::string &fileName);
  void dispatchSyncLoadFile(const std::string &bsiname, uint32 requestId,
                            const std::string &fileName, bool notBlocking);
  void terminateSyncLoads();
  void dispatchSendFile(const std::string &bsiname, uint32 requestId,
                        const CBackupMsgSaveFile &msg);
  void dispatchAppendData(const std::string &bsiname, uint32 requestId,
                          const CBackupMsgSaveFile &msg);
  void dispatchAppendText(const std::string &bsiname, uint32 requestId,
                          const std::string &filename, const std::string &line);
  void dispatchDeleteFile(const std::string &bsiname, uint32 requestId,
                          const std::string &fileToDelete,
...
private:
  ...
  static void cbBsReadMode(NLNET::CMessage &msgin, NLNET::TSockId from,

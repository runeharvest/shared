...
public:
TUserRole() : _Value(invalid_val) {}
TUserRole(TValues value) : _Value(value) {}
...
bool operator==(const TUserRole &other) const { return _Value == other._Value; }
...
private:
  typedef void (CWelcomeServiceSkel::*TMessageHandler)(
  ...
  void welcomeUser_skel(NLNET::IModuleProxy *sender,
                        const NLNET::CMessage &__message);

  void disconnectUser_skel(NLNET::IModuleProxy *sender,
                           const NLNET::CMessage &__message);
...
public:
  ...
  virtual void welcomeUser(NLNET::IModuleProxy *sender, uint32 charId,
                           const std::string &userName,
                           const NLNET::CLoginCookie &cookie,
                           const std::string &priviledge,
                           const std::string &exPriviledge, WS::TUserRole mode,
                           uint32 instanceId) = 0;
...
public:
  ...
  // ask the welcome service to welcome a character
  void welcomeUser(NLNET::IModule *sender, uint32 charId,
                   const std::string &userName,
                   const NLNET::CLoginCookie &cookie,
                   const std::string &priviledge,
                   const std::string &exPriviledge, WS::TUserRole mode,
  ...
  // integration
  static const NLNET::CMessage &buildMessageFor_welcomeUser(
  ...
  buildMessageFor_disconnectUser(NLNET::CMessage &__message, uint32 userId);
};
...
private:
  typedef void (CLoginServiceSkel::*TMessageHandler)(
      NLNET::IModuleProxy *sender, const NLNET::CMessage &message);
  typedef std::map<std::string, TMessageHandler> TMessageHandlerMap;

  const TMessageHandlerMap &getMessageHandlers() const;

  void pendingUserLost_skel(NLNET::IModuleProxy *sender,
                            const NLNET::CMessage &__message);
...
public:
  ...
  virtual void pendingUserLost(NLNET::IModuleProxy *sender,
...
public:
  ...
  // An awaited user did not connect before the allowed timeout expire
  void pendingUserLost(NLNET::IModule *sender,
                       const NLNET::CLoginCookie &cookie);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_pendingUserLost(NLNET::CMessage &__message,
                                  const NLNET::CLoginCookie &cookie);
};
...
private:
  typedef void (CWelcomeServiceClientSkel::*TMessageHandler)(
      NLNET::IModuleProxy *sender, const NLNET::CMessage &message);
  typedef std::map<std::string, TMessageHandler> TMessageHandlerMap;

  const TMessageHandlerMap &getMessageHandlers() const;

  void registerWS_skel(NLNET::IModuleProxy *sender,
                       const NLNET::CMessage &__message);

  void reportWSOpenState_skel(NLNET::IModuleProxy *sender,
                              const NLNET::CMessage &__message);

  void welcomeUserResult_skel(NLNET::IModuleProxy *sender,
                              const NLNET::CMessage &__message);

  void updateConnectedPlayerCount_skel(NLNET::IModuleProxy *sender,
                                       const NLNET::CMessage &__message);
...
public:
  ...
  virtual void registerWS(NLNET::IModuleProxy *sender, uint32 shardId,
                          uint32 fixedSessionId, bool isOnline) = 0;
  // WS report it's current open state
  virtual void reportWSOpenState(NLNET::IModuleProxy *sender,
                                 bool isOnline) = 0;
  // return for welcome user
  virtual void welcomeUserResult(NLNET::IModuleProxy *sender, uint32 userId,
                                 bool ok, const std::string &shardAddr,
                                 const std::string &errorMsg) = 0;
  // transmits the current player counts
  virtual void updateConnectedPlayerCount(NLNET::IModuleProxy *sender,
                                          uint32 nbOnlinePlayers,
                                          uint32 nbPendingPlayers) = 0;
};
...
public:
  ...
  // return for welcome user
  void welcomeUserResult(NLNET::IModule *sender, uint32 userId, bool ok,
                         const std::string &shardAddr,
                         const std::string &errorMsg);
  // transmits the current player counts
  void updateConnectedPlayerCount(NLNET::IModule *sender,
                                  uint32 nbOnlinePlayers,
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_registerWS(NLNET::CMessage &__message, uint32 shardId,
                             uint32 fixedSessionId, bool isOnline);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_reportWSOpenState(NLNET::CMessage &__message, bool isOnline);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_welcomeUserResult(NLNET::CMessage &__message, uint32 userId,
                                    bool ok, const std::string &shardAddr,
                                    const std::string &errorMsg);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_updateConnectedPlayerCount(NLNET::CMessage &__message,
                                             uint32 nbOnlinePlayers,
                                             uint32 nbPendingPlayers);
};
}

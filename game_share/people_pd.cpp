...
} TPeopleConvert[] = {{"Humanoid", CPeople::Humanoid},
...
void CPeople::init() {
  ...
  for (i = 0; i < 159; ++i) {
    ...
    _ValueMap[NLMISC::toLowerAscii(std::string(TPeopleConvert[i].Name))] =
        TPeopleConvert[i].Value;
  }
  _Initialised = true;
}
...
void CClassificationType::init() {
  ...
  uint i;
  for (i = 0; i < 20; ++i) {
    _StrTable[TClassificationTypeConvert[i].Value] =
        TClassificationTypeConvert[i].Name;
    _ValueMap[NLMISC::toLowerAscii(
        std::string(TClassificationTypeConvert[i].Name))] =
        TClassificationTypeConvert[i].Value;
  }
  _Initialised = true;
}
...
std::map<std::string, CClassificationType::TClassificationType>

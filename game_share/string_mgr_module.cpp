 ...
 *Send a npc chat
 */
void sendChatGroup(TDataSetRow &sender, CChatGroup::TGroupType groupType,
                   ucstring &sentence) {
...
void sendChatChannel(TDataSetRow &sender, TChanID &chanId,
                     ucstring &ucsentence) {
...
CStringManagerModule::CStringManagerModule() { _AnimChan = false; }

CStringManagerModule::~CStringManagerModule() { _ClientChannels.clear(); }

void CStringManagerModule::init(NLNET::IModuleSocket *clientGW,
...
void CStringManagerModule::onModuleSecurityChange(
 ...
 */
void CStringManagerModule::insertClient(NLNET::IModuleProxy *moduleProxy,
                                        TChanID channelId) {
  ClientInfo *cInfo = new ClientInfo();
  // cInfo.ChanId = channelId;
  cInfo->Proxy = moduleProxy;
  _ClientChannels.insert(std::pair<TModuleId, ClientInfo *>(
      moduleProxy->getModuleProxyId(), cInfo));
}

void CStringManagerModule::removeClient(NLNET::IModuleProxy *moduleProxy) {
  std::map<TModuleId, ClientInfo *>::iterator found =
      _ClientChannels.find(moduleProxy->getModuleProxyId());
...
bool CStringManagerModule::onProcessModuleMessage(
  ...
  if (operationName == "registerTable") {
  ...
  } else if (operationName == "translateAndForwardArg") {
    ...
    map<NLNET::TModuleId, ClientInfo *>::const_iterator found =
        _ClientChannels.find(mId);
    if (found != _ClientChannels.end()) {
...
void CStringManagerModule::addAnimSession(TModuleId id, TSessionId scenarioId) {
  ...
  if (found != _ClientChannels.end()) {
    ...
    if (!getCharInfo(found->second->Proxy, charId, clientEid, userPriv,
                     extendedPriv)) {
      nlwarning("R2Ed: module '%s' has Invalid Security Info",
                found->second->Proxy->getModuleName().c_str());
    } else {
...
// unused function
void CStringManagerModule::registerTableRequested(
    TSessionId sessionId,
...
// unused function
void CStringManagerModule::registerTableRequested(TSessionId sessionId,
                                                  CObject *table) {
  std::vector<std::pair<std::string, std::string>> entries;
...
void CStringManagerModule::translateAndForwardWithArg(
    TDataSetRow senderId, CChatGroup::TGroupType groupType, std::string id,
    TSessionId sessionId, std::vector<float> &args) {
...
void CStringManagerModule::translateAndForward(TDataSetRow senderId,
                                               CChatGroup::TGroupType groupType,
                                               std::string id,
                                               TSessionId sessionId) {
...
void CStringManagerModule::send(TDataSetRow &senderId,
                                CChatGroup::TGroupType groupType,
  ...
  if (!toSend.empty()) {
    ...
    std::map<NLNET::TModuleId, ClientInfo *>::const_iterator first(
        _ClientChannels.begin()),
...
void CStringManagerModule::talkAs(TModuleId &id, TDataSetRow &creaturRowId,
                                  std::string &name, TSessionId sessionId) {
  // looking for the client's channel
  std::map<NLNET::TModuleId, ClientInfo *>::iterator found =
      _ClientChannels.find(id);
  if (found == _ClientChannels.end()) {
  ...
  if (found->second->getIncarnation(creaturRowId) != TChanID())
    return;
...
void CStringManagerModule::releaseChannels(TSessionId sessionId) {
  std::map<NLNET::TModuleId, ClientInfo *>::iterator first(
      _ClientChannels.begin()),
      last(_ClientChannels.end());
  while (first != last) {
    if (first->second->SessionId == sessionId) {
      nldebug("<releaseChannels> : found an animation module with sessionId %u",
              sessionId.asInt());
      first->second->clear();
    }
    ++first;
...
void CStringManagerModule::forwardIncarnChat(TChanID id, TDataSetRow senderId,
                                             ucstring sentence) {
  nldebug("dyn chat '%s' in channel '%s'", sentence.c_str(),
          id.toString().c_str());
  map<NLNET::TModuleId, ClientInfo *>::const_iterator first(
      _ClientChannels.begin()),
      last(_ClientChannels.end());
  while (first != last) {
...
void CStringManagerModule::sendIdList(TSessionId scenarioId,
                                      NLNET::IModuleProxy *senderModuleProxy) {
  TLocalTable *localTable = getLocalTable(scenarioId.asInt());

  if (localTable != NULL) {
    TLocalTable::const_iterator first(localTable->begin()),
        last(localTable->end());
    CMessage msg("idList");
...
void CStringManagerModule::sendStringValue(TSessionId scenarioId,
                                           NLNET::IModuleProxy *moduleProxy,
                                           std::string id) {
...
void CStringManagerModule::sendTable(TSessionId scenarioId,
  ...
  if (localTable != NULL) {
    ...
    // number of entries
    msg.serial(nb);
    std::map<std::string, uint32>::const_iterator first(localTable->begin()),
        last(localTable->end());
    while (first != last) {

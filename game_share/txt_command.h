...
#define TXT_COMMAND_SET(setName, CONTEXT_CLASS)                                \
  ...
  public:                                                                      \
    static __CTxtCommandSet_##setName *getInstance() {                         \
      ...
      if (p == NULL)                                                           \
        p = new __CTxtCommandSet_##setName;                                    \
      return p;                                                                \
    }                                                                          \
  };                                                                           \
...
#define TXT_COMMAND(cmdName, setName, CONTEXT_CLASS)                           \
  struct __CTxtCommand_##cmdName##CONTEXT_CLASS                                \
      : public ITxtCommand<CONTEXT_CLASS> {                                    \
    static __CTxtCommand_##cmdName##CONTEXT_CLASS *getInstance() {             \
      static __CTxtCommand_##cmdName##CONTEXT_CLASS *p = NULL;                 \
      if (p == NULL)                                                           \
        p = new __CTxtCommand_##cmdName##CONTEXT_CLASS;                        \
      return p;                                                                \
    }                                                                          \
    virtual const char *getName() const { return #cmdName; }                   \
    virtual CTxtCommandResult execute(CONTEXT_CLASS &context,                  \
                                      const NLMISC::CVectorSString &args,      \
                                      const NLMISC::CSString &rawArgs,         \
                                      const NLMISC::CSString &fullCmdLine);    \
                                                                               \
  private:                                                                     \
    __CTxtCommand_##cmdName##CONTEXT_CLASS() {}                                \
  };                                                                           \
  static ITxtCommandRegisterer<__CTxtCommand_##cmdName##CONTEXT_CLASS,         \
                               __CTxtCommandSet_##setName>                     \
      __CTxtCommand_##cmdName##CONTEXT_CLASS##_Registerer;                     \
  CTxtCommandResult __CTxtCommand_##cmdName##CONTEXT_CLASS::execute(           \
      CONTEXT_CLASS &context, const NLMISC::CVectorSString &args,              \
      const NLMISC::CSString &rawArgs, const NLMISC::CSString &fullCmdLine)
...
public:
  ...
  CTxtCommandResult(const bool &success)
      : _Type(success ? SUCCESS : SYNTAX_ERROR) {}
  CTxtCommandResult(const TType &type) : _Type(type) {}
  CTxtCommandResult(const TType &type, const NLMISC::CSString &reason)
      : _Type(type), _Reason(reason) {}
...
public:
  ...
  virtual CTxtCommandResult execute(CONTEXT_CLASS &context,
                                    const NLMISC::CVectorSString &args,
                                    const NLMISC::CSString &rawArgs,
                                    const NLMISC::CSString &fullCmdLine) = 0;
};
...
public:
  ...
  CTxtCommandResult execute(CONTEXT_CLASS &context,
                            const NLMISC::CSString &cmdLine) {
    ...
    if (it == _TxtCommands.end())
      return CTxtCommandResult::UNKNOWN_COMMAND;
...
template <class SET> struct CTxtCommandSetPtr {
  CTxtCommandSetPtr() { SET::getInstance(); }

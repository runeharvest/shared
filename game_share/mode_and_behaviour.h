...
// Mode
enum EMode {
  // IMPORTANT : IF YOU MODIFY THIS ENUM DO NOT FORGET TO CHANGE stringToMode()
  // TOO
 ...
 * \return the EMode associated to this string (UNKNOWN_MODE if the string
...
struct TMode {
  ...
  inline operator uint64() const { return RawModeAndParam; }
  ...
  /// Set mode and fill pos from mirror and compress (must be defined in every
  /// service calling it!)
  void setModeAndPos(EMode mode, const TDataSetRow &entityIndex);

  /// Set mode and theta
  void setModeAndTheta(EMode mode, float theta) {
    Mode = mode;
    Theta = theta;
  }
  ...
  inline void serial(NLMISC::IStream &f) { f.serial(RawModeAndParam); }

  /// Return true if the entity is in a mount mode
  bool isMountMode() const {
    return (Mode == MOUNT_NORMAL) || (Mode == MOUNT_SWIM);
  }
};

// Behaviour
enum EBehaviour {
  // IMPORTANT : IF YOU MODIFY THIS ENUM DO NOT FORGET TO CHANGE
  // stringToBehaviour() TOO
  UNKNOWN_BEHAVIOUR = 0,
  IDLE, // 1
  ...
  //--------- COMBAT---------//
  COMBAT_BEHAVIOUR_BEGIN, // not a behaviour, just here to know if a behaviour
                          // is a combat one
...
struct TBehaviour8 {
  ...
  /// Constructor
  TBehaviour8(EBehaviour b = UNKNOWN_BEHAVIOUR) { Behaviour8 = (uint8)b; }
  ...
  operator EBehaviour() const { return (EBehaviour)Behaviour8; }
};

/**
 * get the right behaviour from the input string
 * \param str the input string
 * \return the EBehaviour associated to this string (UNKNOWN_BEHAVIOUR if the
...
struct CBehaviour {
  ...
  union {
    ...
    struct {
      uint16 Time : 4; // bits 3..6 of TGameCycle (mandatory for reissuing the
                       // behaviour)
      ...
      uint16
          KillingBlow : 1; // 0 = false, 1 = killing blow (kill the main target)
                           // 16 bits used
    } Combat;
    ...
    struct {
      ...
      uint16 ImpactIntensity : 3; // 1 - 5
      uint16 Localisation : 3;    // 6 possible localisation (TBodyPart). For
                                  // generic weapons only
      uint16 Unused : 4;
    } Range;
    ...
    struct {
      uint16 Time : 4; // bits 3..6 of TGameCycle (mandatory for reissuing the
                       // behaviour)
      uint16
          SpellMode : 2;  // MAGICFX::TSpellMode 0 = bomb, 1 = chain, 2 = spray
    ...
    // IMPORTANT: If you add another member struct in the enum, reserve at the
    // beginning:
    //	uint16	Time			: 3; // bits 3..6 of TGameCycle
    //(mandatory for reissuing the behaviour)
  ...
  sint16 DeltaHP;
  uint16 Unused; /// Keep it, used to make the class size = 64 bits
                 /// (sizeof(CBehaviour) MUST return 8 (bytes))

  inline CBehaviour()
      : Data(0), Data2(0), Behaviour(UNKNOWN_BEHAVIOUR), DeltaHP(0), Unused(0) {
  }
  ...
  inline CBehaviour(uint64 rawdata) { *this = rawdata; }

  /// Cast operator (uint64).
  inline operator uint64() const {
    /************************************************************************/
    /* Problem: Olivier use a (sint64*) cast on a CBehaviour*, so this operator
     * isn't called ! :'( */
    /* must use Member declaration order to match 'brutal' cast             */
    /************************************************************************/
    // return (((uint64)(DeltaHP)) << 32) + (((uint64)(Data2)) << 24) +
    // (((uint64)(Data)) << 8) + Behaviour;
    return (((uint64)(DeltaHP)) << 32) + (((uint64)(Behaviour)) << 24) +
           (((uint64)(Data2)) << 16) + Data;
  }
  ...
  inline bool operator==(const CBehaviour &p) const {
    return (DeltaHP == p.DeltaHP && Behaviour == (EBehaviour)p.Behaviour &&
            Data == p.Data && Data2 == p.Data2);
  }

  inline bool operator!=(const CBehaviour &p) const {
    return (Behaviour != (EBehaviour)p.Behaviour || Data != p.Data ||
            Data2 != p.Data2 || DeltaHP != p.DeltaHP);
  }
  ...
  /// methode toString()
  inline std::string toString() const {
    std::string text = behaviourToString(Behaviour) + " " +
                       NLMISC::toString(Data) + " " + NLMISC::toString(Data2) +
                       " " + NLMISC::toString(DeltaHP);
    return text;
  }

  /// Return 'true' if the behaviour is a magic one.
  inline bool isMagic() const {
    return (Behaviour >= MAGIC_CASTING_BEHAVIOUR_BEGIN &&
            Behaviour <= MAGIC_END_CASTING_BEHAVIOUR_END);
  }
  /// Return 'true' if the behaviour is a combat one.
  inline bool isCombat() const {
    return (Behaviour >= COMBAT_BEHAVIOUR_BEGIN &&
            Behaviour <= COMBAT_BEHAVIOUR_END);
  }
  /// Return 'true' if the behaviour is a creature related one
  inline bool isCreatureAttack() const {
    return (Behaviour >= CREATURE_ATTACK_BEGIN &&
            Behaviour <= CREATURE_ATTACK_END);
  }
  /// Return 'true' if the behaviour is an emote one.
  inline bool isEmote() const {
    return (Behaviour >= EMOTE_BEGIN && Behaviour <= EMOTE_END);
  }
};
}; // namespace MBEHAV

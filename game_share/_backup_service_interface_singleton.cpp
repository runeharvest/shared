...
CVariable<string>
    SaveShardRootGameShare("variables", "SaveShardRoot",
                           "Root directory of all files saved by any shard", "",
                           0, true, onSaveShardRootModified, false);

// stats variables
CVariable<NLMISC::TTime>
    BSLastAckTime("BSIF", "BSLastAckTime",
                  "The timestamp of the last ack received from backup system",
                  0, 0, true);
CVariable<NLMISC::TTime>
    BSTimeSinceLastAck("BSIF", "BSTimeSinceLastAck",
                       "The time since the last ack from the backup system", 0,
                       0, true);
CVariable<NLMISC::TTime>
    BSLastAckDelay("BSIF", "BSLastAckDelay",
                   "The time it took for the last received ack to be received "
                   "from slowest of our connected BS services",
                   0, 0, true);
CVariable<uint32> BSResponseTime(
    "BSIF", "BSResponseTime",
    "Latest/average time (in sec) between BS request and response", 0, 20,
    false);
...
CBackupServiceInterface &getShardDependentBsi() {
  return CBackupInterfaceSingleton::getInstance()->_ShardDependentBsi;
}
CBackupServiceInterface &getGlobalBsi() {
  return CBackupInterfaceSingleton::getInstance()->_GlobalBsi;
}
// CBackupServiceInterface& getPDBsi() { return
// CBackupInterfaceSingleton::getInstance()->_PDBsi; }
...
void CBackupInterfaceSingleton::init() {
  ...
  if (packingSheets) {
    nlinfo("Not initialising BS Interface because -Q flag found meaning that "
           "we are just packing sheets and exitting");
    return;
  }
  ...
  _ShardDependentBsi.setRemotePath(
      IService::getInstance()->SaveFilesDirectory.toString());
  _ShardDependentBsi.setLocalPath(
      CPath::standardizePath(SaveShardRootGameShare.get()) +
      IService::getInstance()->SaveFilesDirectory.toString());
  ...
  //	_PDBsi.init("PDBS");
  //	_PDBsi.setRemotePath(
  //IService::getInstance()->SaveFilesDirectory.toString() );
  //	_PDBsi.setLocalPath( CPath::standardizePath(
  //SaveShardRootGameShare.get() ) +
  //IService::getInstance()->SaveFilesDirectory.toString() );
...
uint32 CBackupInterfaceSingleton::pushFileCallback(
    NLMISC::CSmartPtr<IBackupFileReceiveCallback> &callback,
    CBackupServiceInterface *itf) {
  BOMB_IF(callback == NULL, "Illegal attempt to register a NULL callback!",
          return 0);
...
uint32 CBackupInterfaceSingleton::pushFileClassCallback(
    NLMISC::CSmartPtr<IBackupFileClassReceiveCallback> &callback,
    CBackupServiceInterface *itf) {
...
uint32 CBackupInterfaceSingleton::pushGenericAckCallback(
    NLMISC::CSmartPtr<IBackupGenericAckCallback> &callback,
    CBackupServiceInterface *itf) {
...
NLMISC::CSmartPtr<IBackupFileReceiveCallback>
CBackupInterfaceSingleton::popFileCallback(uint32 requestId,
                                           CBackupServiceInterface *&itf) {
  TBSCallbackInfo<IBackupFileReceiveCallback> &bsInfo =
      _FileResponses[requestId];
...
NLMISC::CSmartPtr<IBackupFileClassReceiveCallback>
CBackupInterfaceSingleton::popFileClassCallback(uint32 requestId,
                                                CBackupServiceInterface *&itf) {
  TBSCallbackInfo<IBackupFileClassReceiveCallback> &bsInfo =
      _FileClassResponses[requestId];
...
NLMISC::CSmartPtr<IBackupGenericAckCallback>
CBackupInterfaceSingleton::popGenericCallback(uint32 requestId,
                                              CBackupServiceInterface *&itf) {
  // if there are untreated callbacks in the generic responses container then
  // yell
  while (!_GenericResponses.empty() &&
         sint32(_GenericResponses.front().first - requestId) < 0) {
    STOP("Skipping untreated generic callback for request: "
         << _GenericResponses.front().first
         << " because we are treating request id: " << requestId);
  ...
  // if we're here it means that the front entry in the generic responses
  // container has the same request id as requestId

  // get hold of the callback info object
  TBSCallbackInfo<IBackupGenericAckCallback> &bsInfo =
      _GenericResponses.front().second;
  NLMISC::CSmartPtr<IBackupGenericAckCallback> callback = bsInfo.Callback;

  // setup the itf return value before we pop the front entry off the generic
  // responses container
  itf = bsInfo.Interface;
  ...
  // pop the entry that we just treated off the front of the generic responses
  // container
  _GenericResponses.pop_front();
...
void CBackupInterfaceSingleton::pushBSConnectCallback(
    IBackupServiceConnection *cb) {
...
bool CBackupInterfaceSingleton::isConnected() const { return _IsConnected; }

//-------------------------------------------------------------------------------------------------
// methods CBackupInterfaceSingleton - accessors for BSI implmentation
//-------------------------------------------------------------------------------------------------

IBackupServiceInterfaceImplementation *
CBackupInterfaceSingleton::getBSIImplementation() {
  return _BackupServiceInterfaceImplementation;
}

void CBackupInterfaceSingleton::setBSIImplementation(
    IBackupServiceInterfaceImplementation *bsii) {
...
//-------------------------------------------------------------------------------------------------
// CBackupInterfaceSingleton routines for responding to configuration variable
// changes
//-------------------------------------------------------------------------------------------------
...
void CBackupInterfaceSingleton::onVariableChanged(NLMISC::IVariable &var) {
  if (var.getName() == "SaveFilesDirectory") {
    ...
    _ShardDependentBsi.setLocalPath(
        CPath::standardizePath(SaveShardRootGameShare.get()) + var.toString());
    //		_PDBsi.setRemotePath( var.toString() );
    //		_PDBsi.setLocalPath( CPath::standardizePath(
    //SaveShardRootGameShare.get() ) + var.toString() );
  } else if (var.getName() == "SaveShardRoot") {
    _ShardDependentBsi.setLocalPath(
        CPath::standardizePath(var.toString()) +
        IService::getInstance()->SaveFilesDirectory.toString());
    _GlobalBsi.setLocalPath(var.toString());
    //		_PDBsi.setLocalPath( CPath::standardizePath( var.toString() ) +
    //IService::getInstance()->SaveFilesDirectory.toString() );
  }
}
...
IBackupServiceInterfaceImplementation::
  ...
  if (CBackupInterfaceSingleton::getInstance()->getBSIImplementation() ==
      this) {
...
// The instantiation of the singleton provokes registration in the singleton
// registry meaning that the init() routine gets called properly in service
// init, etc.
//-------------------------------------------------------------------------------------------------

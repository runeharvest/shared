...
class CBackupInterfaceSingleton : public IServiceSingleton,
                                  public NLMISC::IVariableChangedCallback {
...
public:
  ...
  uint32
  pushFileCallback(NLMISC::CSmartPtr<IBackupFileReceiveCallback> &callback,
                   CBackupServiceInterface *itf);
  uint32 pushFileClassCallback(
      NLMISC::CSmartPtr<IBackupFileClassReceiveCallback> &callback,
      CBackupServiceInterface *itf);
  uint32
  pushGenericAckCallback(NLMISC::CSmartPtr<IBackupGenericAckCallback> &callback,
                         CBackupServiceInterface *itf);

  // methods for getting the callback associated with a request and removing it
  // from the associated callbacks container
  NLMISC::CSmartPtr<IBackupFileReceiveCallback>
  popFileCallback(uint32 requestId, CBackupServiceInterface *&itf);
  NLMISC::CSmartPtr<IBackupFileClassReceiveCallback>
  popFileClassCallback(uint32 requestId, CBackupServiceInterface *&itf);
  NLMISC::CSmartPtr<IBackupGenericAckCallback>
  popGenericCallback(uint32 requestId, CBackupServiceInterface *&itf);
  ...
  // setup a callback to be invoked on connection of backup system (modules or
  // services)
  void pushBSConnectCallback(IBackupServiceConnection *);

  // register connecteion or deconnection of backup system (called at first
  // connection and last disconnection)
  void connect();
  void disconnect();
...
public:
  ...
  template <class T> struct TBSCallbackInfo {
  ...
  std::map<uint32, TBSCallbackInfo<IBackupFileClassReceiveCallback>>
      _FileClassResponses;
  std::list<std::pair<uint32, TBSCallbackInfo<IBackupGenericAckCallback>>>
      _GenericResponses;

  // container of callbacks for backup system connection events (for first
  // service or module up and last service or module down)
  std::vector<IBackupServiceConnection *> _BSConnectCallbacks;

  // a handy global to track state of the backup system
  bool _IsConnected;

  // the current active IBackupServiceInterfaceImplementation object (initially
  // NULL)
  ...
  // the backup service interfaces used for shard dependent and global stuff -
  // these are accessible via Bsi and BsiGlobal macros th e_PDBsi is used by the
  // pd lib for its logs

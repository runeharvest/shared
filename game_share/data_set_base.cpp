...
// The following variable controls the logging of nlinfo and nldebug messages
// produced by the MIRROR_INFO and MIRROR_DEBUG macros
CVariable<bool> VerboseMIRROR("ms", "VerboseMIRROR",
                              "Enable verbose logging in Mirror operations",
                              true, 0, true);
 ...
 * Constructor
 */
CDataSetBase::CDataSetBase()
    : _MaxNbRows(INVALID_DATASET_INDEX) //, _SizeOfRow(Row32)
{}

/*
 * Initialize
 */
void CDataSetBase::init(const NLMISC::CSheetId &sheetId,
  ...
  // nlinfo( "\t Dataset %s:      Rows will be transfered using %s-bit index",
  // _DataSetName.c_str(),
  // (_SizeOfRow==Row32)?"32":((_SizeOfRow==Row16)?"16":"8") );
 ...
 * NULL. If propName is the special entityId property, the behaviour is
 * different (propName need not exist)
 */
void CDataSetBase::setPropertyPointer(
    std::string &propName, TPropertyIndex propIndex, void *segmentPt,
    bool initValues, uint32 dataTypeSize, bool isReadOnly,
    bool mustMonitorAssignment, std::vector<uint8> *ptCountsToSet,
    NLMISC::TGameCycle /* timestamp */, uint32 segmentSize) {
  ...
  if (propName.substr(0, LENGTH_OF_ENTITYID_PREFIX) == ENTITYID_PREFIX) {
    ...
    _PropertyContainer.EntityIdArray.OnlineBitfieldPt =
        (uint32 *)((uint8 *)segmentPt + (sizeof(CEntityId) * maxNbRows()));
    _PropertyContainer.EntityIdArray.OnlineTimestamps =
        (NLMISC::TGameCycle *)(((uint8 *)_PropertyContainer.EntityIdArray
                                    .OnlineBitfieldPt) +
                               maxNbRows() / 8 + 4);
    _PropertyContainer.EntityIdArray.Counts =
        ((uint8 *)_PropertyContainer.EntityIdArray.OnlineTimestamps) +
        maxNbRows() * sizeof(NLMISC::TGameCycle);
    _PropertyContainer.EntityIdArray.SpawnerServiceIds =
        ((TServiceId8 *)_PropertyContainer.EntityIdArray.Counts) +
        maxNbRows() * sizeof(TServiceId8);
    endPt = ((uint8 *)_PropertyContainer.EntityIdArray.SpawnerServiceIds) +
            maxNbRows() * sizeof(TServiceId8);
    ...
    if (initValues) {
      ...
      for (TDataSetIndex i = 0; i != (uint32)maxNbRows(); ++i) {
        ...
        if (ptCountsToSet != NULL)
          ...
          _PropertyContainer.EntityIdArray.Counts[i] =
              1; // begins at 1, 0 is reserved for "no counter"

        // Always init the online timestamps to 0 (otherwise the assert in
        // declareEntity() will fail)
    ...
    if ((uint)propIndex < (uint)nbProperties()) {
      _PropertyContainer.PropertyValueArrays[propIndex].ChangeTimestamps =
          (TGameCycle *)segmentPt;
#ifdef STORE_CHANGE_SERVICEIDS
      _PropertyContainer.PropertyValueArrays[propIndex].ChangeServiceIds =
          (TServiceId8 *)((uint8 *)segmentPt +
                          (sizeof(TGameCycle) * maxNbRows()));
      _PropertyContainer.PropertyValueArrays[propIndex].Values =
          (void *)((uint8 *)_PropertyContainer.PropertyValueArrays[propIndex]
                       .ChangeServiceIds +
                   (sizeof(TServiceId8) * maxNbRows()));
#else
      _PropertyContainer.PropertyValueArrays[propIndex].Values =
          (void *)((uint8 *)segmentPt + (sizeof(TGameCycle) * maxNbRows()));
#endif
      _PropertyContainer.PropertyValueArrays[propIndex].DataTypeSize =
          dataTypeSize;
#ifdef NL_DEBUG
      _PropertyContainer.PropertyValueArrays[propIndex].IsReadOnly = isReadOnly;
      _PropertyContainer.PropertyValueArrays[propIndex].IsMonitored =
          mustMonitorAssignment;
#endif

      // Cell container for list properties
      if (_PropIsList[propIndex]) {
        uint8 *ptFreeCellsFront =
            ((uint8 *)(_PropertyContainer.PropertyValueArrays[propIndex]
                           .Values)) +
            sizeof(TSharedListRow) * maxNbRows();
        _PropertyContainer.PropertyValueArrays[propIndex].ListCellContainer =
            ptFreeCellsFront + sizeof(TSharedListRow);

        // In the Mirror Service, set up the list headers (1 per datasetrow) and
        // container
        if (initValues) {
          // Init the headers
          ...
          // nldebug( "%p + %u bytes + %u",
          // _PropertyContainer.PropertyValueArrays[propIndex].Values,
          // sizeof(TSharedListRow)*maxNbRows(), sizeof(TSharedListRow) );
          // nldebug( "%p",
          // _PropertyContainer.PropertyValueArrays[propIndex].ListCellContainer
          // ); nldebug( "Segment size: %u", segmentSize );
          nldebug("Shared List container uses %u bytes (at %u bytes per cell)",
                  segmentSize - ((uint8 *)(_PropertyContainer
                                               .PropertyValueArrays[propIndex]
                                               .ListCellContainer) -
                                 (uint8 *)segmentPt),
                  sizeof(TSharedListRow) + datasize);
          ...
          uint8 *cellPt =
              (uint8 *)(_PropertyContainer.PropertyValueArrays[propIndex]
                            .ListCellContainer);
          for (TSharedListRow j = 0; j != NB_SHAREDLIST_CELLS - 1 /*TEMP*/;
               ++j) {
            TSharedListRow *nextField = (TSharedListRow *)cellPt;
            *nextField = j + 1;
            cellPt +=
                sizeof(TSharedListRow) + datasize; // assumes #pragma pack!
          }
          TSharedListRow *nextField = (TSharedListRow *)cellPt;
          *nextField = INVALID_SHAREDLIST_ROW;

          endPt = ((uint8 *)_PropertyContainer.PropertyValueArrays[propIndex]
                       .ListCellContainer) +
                  (NB_SHAREDLIST_CELLS * (sizeof(TSharedListRow) +
                                          datasize)); // assumes #pragma pack!
        }
      } else {
        _PropertyContainer.PropertyValueArrays[propIndex].ListCellContainer =
            NULL;
...
void CDataSetBase::fillDataSizeByProp() {
  ...
  for (itp = _PropIndexToType.begin(), itd = _DataSizeByProp.begin();
       itp != _PropIndexToType.end(); ++itp, ++itd) {
    switch (*itp) {
    case TypeUint8:
      *itd = 1;
      break;
 ...
 * right size
 */
void CDataSetBase::checkTemplateSize(uint32 passedSize,
                                     TPropertyIndex propIndex,
                                     const TDataSetRow &entityIndex) const {
  nlassertex(passedSize ==
                 _PropertyContainer.PropertyValueArrays[propIndex].DataTypeSize,
             ("Wrong CMirrorPropValue template argument: size=%u (expected: "
              "%u) (%s/E%d/P%hd)",
              passedSize,
              _PropertyContainer.PropertyValueArrays[propIndex].DataTypeSize,
              name().c_str(), entityIndex.getIndex(), propIndex));
}

/*
 * Read the sheet
 */
void TDataSetSheet::readGeorges(const NLMISC::CSmartPtr<NLGEORGES::UForm> &form,
  ...
  if ((UForm *)form) {
    ...
    // Get the max number of rows (will be used only if not overriden in the
    // .cfg)
    if (root.getValueByName(MaxNbRows, "max number of rows or entities")) {
      MIRROR_INFO("MIRROR: \tMaxNbRows = %d in sheet", MaxNbRows);
    } else {
      MaxNbRows = 0;
      nlinfo("MaxNbRows of %s not specified in sheet, expecting value in .cfg",
             DataSetName.c_str());
    }
      ...
      for (sint p = 0; p != NbProperties; ++p) {
        ...
        if (arrayProperties->getArrayNode(&property, p) && property) {
          ...
          if (typeStr == "uint8")
            ...
            theType = TypeDouble;
          else if (typeStr == "entityid")
            theType = TypeCEntityId;
          else if (typeStr == "boolean")
            theType = TypeBool;
          else {
            nlerror("MIRROR: CDataSetBase::readGeorges: unknown data type '%s'",
                    typeStr.c_str()); // not forcing to TypeUint64 anymore
            theType = TypeUint64;
          }
          ...
          */
          ...
          MIRROR_INFO("MIRROR: \tProperty '%s' : Type %s%s" /*", Weight %d"*/,
                      PropertyNames[p].c_str(), PropIsList[p] ? "list of " : "",
                      typeStr.c_str() /*, Weights[p]*/);
    ...
    if (root.getNodeByName(&arrayEntityTypes, "entity types") &&
        arrayEntityTypes) {
      uint size;
      ...
      for (uint et = 0; et != size; ++et) {
        ...
        MIRROR_INFO("MIRROR: \tLinking to entity type %hu",
                    (uint16)EntityTypesFilter[et]);
      }
    } else {
...
void TDataSetSheet::serial(NLMISC::IStream &s) {
  ...
  for (TPropertyIndex p = 0; p != NbProperties; ++p) {
    ...
    MIRROR_DEBUG("MIRROR: Serializing %s, type %u", PropertyNames[p].c_str(),
                 PropIndexToType[p]);
    // s.serial( Weights[p] );
    bool b = PropIsList[p];
    s.serial(b);
 ...
 * If found in the .cfg, use the value of the property DataSetSize +
 * DataSetName. If not found, use the value in the sheet.
 */
TDataSetIndex TDataSetSheet::getConfigDataSetSize() const {
  string cfgVarName = "DatasetSize" + DataSetName;
  CConfigFile::CVar *cfgVar =
      NLNET::IService::getInstance()->ConfigFile.getVarPtr(cfgVarName);

...
- search in file MAX_INVENTORY_ANIMAL in all XML (including database.xml AND
local_database.xml) to do correct changes
* ******/
...
// This give by which value the WEIGHT in database must be divided to get the
// value in Kg valueKg= valueDb / DB_WEIGHT_SCALE
#define DB_WEIGHT_SCALE 100

namespace INVENTORIES {
/** Domain wide unique id for item (on 64 bits)
 *	Use a combination of unix time, shardId and current second serial
 *number.
 */
struct TItemId {
  ...
  TItemId(uint64 rawId) { _RawId = rawId; }
  ...
  TItemId(const std::string &idStr) {
    ...
    uint32 shardId = 0;
    uint32 serialNum = 0;
    if (sscanf(idStr.c_str(), "[%u:%x:%u]", &shardId, &_CreateTime,
               &serialNum) == 3) {
      _ShardId = shardId;
  ...
  /// hum... serial
  void serial(NLMISC::IStream &s) { s.serial(_RawId); }

  /// convert to a standard string representation (that can be parsed back as
  /// itemId)
  std::string toString() const {
    return NLMISC::toString("[%u:0x%x:%u]", _ShardId, _CreateTime,
                            _SerialNumber);
  }

  bool operator==(const TItemId &other) const { return _RawId == other._RawId; }

  bool operator<(const TItemId &other) const { return _RawId < other._RawId; }

  uint32 getCreateTime() const { return _CreateTime; }

  uint32 getSerialNumber() const { return _SerialNumber; }
...
enum THandSlot { right = 0, left, NB_HAND_SLOT };
...
enum TInventory {
  ...
  trading,              // 11
  reward_sharing,   // 12 fake inventory, not in database.xml. Used by the item
                    // info protocol only
 ...
 * \return the TInventory associated to this string (UNDEFINED if the string
...
/**
 * get the name of an inventory in the data base. Use this API to fill the user
 * database \return if the specified inventory is supported \param inventory :
 * the inventory enum \param retValue ; the returned string
 */
...
public:
  ...
  enum TSlotBitSize { SlotBitSize = 10 };

  static std::string getDbStr(TInventoryId invId);
  static bool needPlainInfoVersionTransfer() {
    return false;
...
public:
  enum TInventoryId { GuildInvId, InvalidInvId, NbInventoryIds = InvalidInvId };
  static const char *InventoryStr[NbInventoryIds];
  static const uint InventoryNbSlots[NbInventoryIds];
  enum TSlotBitSize { SlotBitSize = 10 };
  ...
  static bool needPlainInfoVersionTransfer() {
    return true;
  } // incrementation is not sufficient because can incremented when player
    // offline, and some values are skipped
};
...
const uint NbBitsForItemPropId =
    4; // TODO: replace this constant by an inline function using NbItemPropId
...
toString(CInventoryCategoryForCharacter::TInventoryId invId) {
  return std::string(CInventoryCategoryForCharacter::InventoryStr[invId]);
}

/// toString(), alternate version for TInventoryId
inline const std::string
toString(CInventoryCategoryForGuild::TInventoryId invId) {
  return std::string(CInventoryCategoryForGuild::InventoryStr[invId]);
}

/// Return the inventory db root string
inline std::string
CInventoryCategoryForCharacter::getDbStr(TInventoryId invId) {
...
public:
  ...
  /// Set a property
  void setItemProp(TItemPropId id, sint32 value) { _ItemProp[id] = value; }

  /// Get a property
  sint32 getItemProp(TItemPropId id) const { return _ItemProp[id]; }

  /// Serial from/to bit stream
  template <class CInventoryCategoryTemplate>
  void serialAll(NLMISC::CBitMemStream &bms,
                 const CInventoryCategoryTemplate * /* templ */ = 0) {
    ...
    // For all other the compression is simple the first bit indicates if the
    // value is zero
    if (bms.isReading()) {
  ...
  /// Serial from/to bit stream
  template <class CInventoryCategoryTemplate>
  void serialOneProp(NLMISC::CBitMemStream &bms,
                     const CInventoryCategoryTemplate * /* templ */ = 0) {
    bms.serial(_SlotIndex, CInventoryCategoryTemplate::SlotBitSize);
    bms.serial(_OneProp);
  }
...
private:
  ...
  union {
    ...
    /// Only one prop (needs to be here because a CItemSlot can't be stored in
    /// an union because of the constructors)
    COneProp _OneProp;
  };

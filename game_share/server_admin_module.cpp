...
// const std::string CServerAdminModule::_AdminModuleSaveFilename(
...
TSessionId CServerAdminModule::getSessionIdByCharId(uint32 charId) const {
  TSessionId editionSessionId =
      _Server->getEditionModule()->getSessionIdByCharId(charId);
  if (editionSessionId.asInt() != 0)
    return editionSessionId;
  TSessionId animationSessionId =
      _Server->getAnimationModule()->getSessionIdByCharId(charId);
  if (animationSessionId.asInt() != 0)
    return animationSessionId;
  return TSessionId(0);
}

bool CServerAdminModule::getPosition(TSessionId sessionId, double &x, double &y,
  ...
  if (scenario->isRunning()) {
    ...
    if (!ok) {
      return edit->getPosition(sessionId, x, y, orient, season,
                               actId); // return start position
    }
    return ok;
  }
  return false;
...
void CServerAdminModule::init(NLNET::IModuleSocket *gateway,
                              CDynamicMapService *server) {
  _Server = server;
  this->plugModule(gateway);
}

bool CServerAdminModule::onProcessModuleMessage(
...
void CServerAdminModule::onModuleSecurityChange(
    IModuleProxy * /* moduleProxy */) {}

CServerAdminModule::CServerAdminModule() { _Instance = this; }
...
NLMISC_CLASS_COMMAND_IMPL(CServerAdminModule, displayIslands) {
  ...
  const CScenarioEntryPoints::TCompleteIslands &islands =
      epManager.getCompleteIslands();

  CScenarioEntryPoints::TCompleteIslands::const_iterator first(islands.begin()),
      last(islands.end());

  uint32 id = 0;
  for (; first != last; ++first) {
    ++id;
    log.displayNL("%u: '%s' '%s' %s'", id, first->Package.c_str(),
                  first->Island.c_str(), first->Continent.c_str());

    CScenarioEntryPoints::TShortEntryPoints::const_iterator firstPoint(
        first->EntryPoints.begin()),
        lastPoint(first->EntryPoints.end());

    for (; firstPoint != lastPoint; ++firstPoint) {
      log.displayNL("\t-> %u: '%s' '%d' '%d'", id, firstPoint->Location.c_str(),

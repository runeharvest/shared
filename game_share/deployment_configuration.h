...
public:
  ...
  // get hold of the instance of the singleton that derives from
  ...
  //		'domain' <domain_name>	// flag this block as a domain and give
  //it a name 		'shard' <domain_name>	// flag this block as a shard and give
  //it a name 		'use' <block_name>		// add a child block to this
  //block 		'name' <block_name>		// set the unique name to use for the
  //executables derived from this block 		'cmdLine' <cmd_line>	// setup the
  //command line to use for exes derived from this block 		'host' <host_name>
  //// assign the exes derived from this block to a given host 		'cfg' <cfg_line>
  //// add a line to be included in the cfg file of executables derived from
  //this block 		'cfgAfter' <cfg_line>	// as above but cfg lines appended at
  //end of cfg file (not at start) 		'data' <data_name>		// add
  //dependency on a given data block to exes derived from this block
  //
  ...
  // serialise the deployment configuration, for dispatch to / reception from
  // other apps
  ...
  virtual void getShardNames(const TDomainName &domainName,
                             TShardNames &result) const = 0;

  // get the apps for a given host and domain
  virtual void getAppNames(const THostName &hostName,
                           const TDomainName &domainName,
                           TAppNames &result) const = 0;
  ...
  // get a named host
  virtual void getHost(const THostName &hostName,
                       SHostDescription &result) const = 0;

  // get a named domains
  virtual void getDomain(const TDomainName &domainName,
                         SDomainDescription &result) const = 0;

  // get a named shard for a given domain
  virtual void getShard(const TDomainName &domainName,
                        const TShardName &shardName,
                        SShardDescription &result) const = 0;

  // get a named app for a given domain
  virtual void getApp(const TDomainName &domainName, const TAppName &appName,
                      SAppDescription &result) const = 0;

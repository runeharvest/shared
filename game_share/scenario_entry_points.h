...
public:
  ...
  struct CEntryPoint {
    ...
    CEntryPoint(NLMISC::CSString package, NLMISC::CSString island,
                NLMISC::CSString location, sint32 x, sint32 y) {
      Package = package;
      Island = island;
      Location = location;
      X = x;
  ...
  public:
    CCompleteIsland() { XMin = YMin = XMax = YMax = 0; }
    bool isIn(const NLMISC::CVector2f &pos) const {
      return (sint32)pos.x >= XMin && (sint32)pos.x < XMax &&
             (sint32)pos.y >= YMin && (sint32)pos.y < YMax;
    }
  };
...
public:
  ...
  CShortEntryPoint *getEntryPointFromIds(const NLMISC::CSString &islandId,
  ...
  void setFiles(const std::string &completeIslandsFilename,
...
private:
  ...
  void saveXMLFile(const TCompleteIslands &entryPoints,
  ...
  // - meaningful lines have the following syntax: <package name> <island name>
  // <entry point name> <x> <y> <orientation>
  ...
  // get the vector of island names that correspond to the given package
  // definition string void getIslands(const NLMISC::CSString&
  // packageDefinition, NLMISC::CVectorSString& islands);

  // get the vector of valid entry points for a given island that correspond to
  // the given package definition string void getEntryPoints(const
  // NLMISC::CSString& packageDefinition, const NLMISC::CSString& island,
  // NLMISC::CVectorSString& entryPoints);

  // lookup the integer id for an entry point for a given island and package
  // definition uint32 getEntryPointId(const NLMISC::CSString&
  // packageDefinition, const NLMISC::CSString& island, const NLMISC::CSString&
  // entryPoint);

  // lookup the coordinates of an entry point from its id and package definition
  // sets x,y to 0,0 if the id is invalid
  // void getEntryPointCoordsFromId(const NLMISC::CSString& packageDefinition,
  // uint32 id, sint32& x, sint32& y);

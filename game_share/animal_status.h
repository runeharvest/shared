...
enum EAnimalStatusFlag {
  /* There is no PresentFlag: the animal is not present if AliveFlag and
     InLandscapeFlag are not set (cause an animal cannot be dead in a stable!)
      "Present" is tested if not all 0!
  */
  AliveFlag = 0x0001, // if set, the animal is alive
  InLandscapeFlag =
      0x0002, // if set, the animal is in Landscape. not set: in Stable
  InventoryAvailableFlag = 0x0004, // if set, the animal inventory is available
  CanEnterLeaveStableFlag =
      0x0008, // if set, an order "Leave" or "Enter Stable" can be issued
};

// true if the animal is present and spawned
inline bool isSpawned(EAnimalStatus e) { return e != 0; }
...
// true if the animal is present, spawned, and in landscape (dead or not, and
// whatever the inventory state)

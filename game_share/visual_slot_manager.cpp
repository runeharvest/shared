...
// Constructor
//-----------------------------------------------
CVisualSlotManager::CVisualSlotManager() {} // CVisualSlotManager //
...
void CVisualSlotManager::init() {
  ...
  if (f.open(NLMISC::CPath::lookup(filename, false, false))) {
    ...
    for (uint i = 0, len = _VisualSlot.size(); i < len; ++i) {
      ...
      //			for(uint j=0; j<_VisualSlot[i].Element.size();
      //++j) 				nlinfo("Visu: %d Num: %d Id: %d sheet: %s", i,
      //_VisualSlot[i].Element[j].Index,
      //_VisualSlot[i].Element[j].SheetId.asInt(),
      //_VisualSlot[i].Element[j].SheetId.toString().c_str());
    }
  } else
...
//-----------------------------------------------
uint32 CVisualSlotManager::sheet2Index(const NLMISC::CSheetId &id,
                                       SLOTTYPE::EVisualSlot slot) {
  if ((uint)slot < _VisualSlot.size()) {
    const TElementList &el = _VisualSlot[slot];
    TElementList::SheetIdToIndexMapType::const_iterator it =
        el.SheetIdToIndexMap.find(id);
    if (it != el.SheetIdToIndexMap.end())
      return it->second;
  } else
    nlwarning("VSMngr:sheet2Index: Bad slot '%d' -> you probably need to "
              "rebuild the tab.",
              (sint)slot);
...
//-----------------------------------------------
NLMISC::CSheetId *CVisualSlotManager::index2Sheet(uint32 index,
                                                  SLOTTYPE::EVisualSlot slot) {
  if ((uint)slot < _VisualSlot.size()) {
    ...
    nlwarning("VSMngr:index2Sheet: Bad slot '%d' -> you probably need to "
              "rebuild the tab.",
              (sint)slot);
...
//-----------------------------------------------
void CVisualSlotManager::sheet2Index(const NLMISC::CSheetId &id,
                                     std::vector<TIdxbyVS> &result) {
...
uint32 CVisualSlotManager::getNbIndex(SLOTTYPE::EVisualSlot slot) const {
  ...
  if ((uint)slot < _VisualSlot.size()) {
    ...
    nlwarning("VSMngr:getNbIndex: Bad slot '%d' -> you probably need to "
              "rebuild the tab.",
              (sint)slot);

...
NLMISC::CVariable<bool>
    DebugPhraseSender("debug", "DebugPhraseSender",
                      "Send debug phrase message to the IOS", false, 0, true);

namespace STRING_MANAGER {

void TParam::serial(NLMISC::IStream &f) { serialParam(true, f, invalid_value); }
...
uint32 pickStringSerialNumber() {
  ...
  stringSeq = (NLNET::IService::getInstance()->getServiceId().get() << 24) |
              (stringSeq & 0xffffff);
...
/*
 * Generic method used to send localised texts to clients ( whether they are
 * ingame or in the character creation process ) this method is inline and
 * declared there so that it cannot be accessed from elsewhere. Use
 * sendStringToClient or sendStringToUser
 */
inline uint32 sendString(NLNET::CMessage &msg, const std::string &stringName,
                         const std::vector<TParam> &params, ISender *sender) {
...
// sendStringToClient
//-----------------------------------------------
uint32 sendStringToClient(const TDataSetRow &clientRowId,
                          const std::string &stringName,
                          const std::vector<TParam> &params, ISender *sender) {
  BOMB_IF(stringName.empty(),
          "Trying to send a phrase to IOS with an empty phraseName", return 0;);
...
void sendSystemStringToClientAudience(
    const TDataSetRow &clientRowId,
    const std::vector<NLMISC::CEntityId> &excluded,
    CChatGroup::TGroupType audience, const char *stringName,
    const std::vector<TParam> &params, ISender *sender) {
  BOMB_IF(strlen(stringName) == 0,
          "Trying to send a phrase to IOS with an empty stringName", return;);
  if (DebugPhraseSender) {
...
void sendCustomEmoteTextToClientAudience(
    ...
    const std::vector<STRING_MANAGER::TParam> &params, ISender *sender) {
  BOMB_IF(strlen(stringName) == 0,
          "Trying to send a custom emot phrase to IOS with an empty stringName",
          return;);
  ...
  for (uint i = 0; i < size; i++) {
    ...
    const_cast<STRING_MANAGER::TParam &>(params[i]).serialParam(false, msgout2,
                                                                params[i].Type);
  }
...
NLMISC::CEntityId TParam::getEId() const { return EId; }

TAIAlias TParam::getAIAlias() const { return _AIAlias; }

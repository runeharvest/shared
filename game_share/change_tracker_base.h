 ...
 * Important note: all the user services and the mirror service should have the
 * same value!
 */
...
struct TChangeTrackerHeader {
  ...
  /** Last changed item (useful to link the last changed to a newly changed
   * item). When there is no change yet, Last equals INVALID_DATASET_ROW.
   */
  TDataSetIndex Last;
...
#endif
  /*
   * Number of values set (used in COUNT_MIRROR_CHANGES mode only, always
   * allocated for mode interoperability) Currently, not implemented.
   */
  // sint32				NbValuesSet;

  /**
   * Number of changes really recorded (used in COUNT_MIRROR_CHANGES mode only,
   * always allocated for mode interoperability). NbValuesSet-NbDistinctChanges
   * is the number of skipped changes.
   */
...
public:
  ...
  CChangeTrackerBase &operator=(const CChangeTrackerBase &src) {
    ...
    _MutId = src._MutId;
    _Header =
        src._Header; // leave the pointed data where they are (in shared memory)
  ...
  bool isAllocated() const { return _Array != NULL; }
  ...
  /// Get the entity index of the first changed (assumes isAllocated()). Returns
  /// LAST_CHANGED if there is no change.
  uint32 getFirstChanged()
      const { /*nlinfo( "Array = %p, First = %d, _Array[First].NextChanged = %d,
                 _Array[0].NextChanged = %d", _Array, _Header->First,
                 _Array[_Header->First].NextChanged, _Array[0].NextChanged );*/
    return _Header->First;
  }

  /// Pop the first change out of the tracker. Do not call if getFirstChanged()
  /// returned LAST_CHANGED.
  ...
  void displayTrackerInfo(const char *headerStr = "",
...
protected:
  /// Get the entity index of the next changed (assumes isAllocated() and
  /// entityIndex is valid). Returns LAST_CHANGED if there is no more change.
  TDataSetRow getNextChanged(const TDataSetRow &entityIndex)
      const { /*nlinfo( "_Array[%d].NextChanged = %d", entityIndex,
                 _Array[entityIndex].NextChanged );*/
  ...
  /// Pointer to the handling variables of the tracker (may be pointing to
  /// shared memory)
  TChangeTrackerHeader *_Header;

  /// Pointer to the array of entities, indexed by TDataSetRow (may be pointing
  /// to shared memory)

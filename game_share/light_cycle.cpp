...
CSeasonLightCycle::CSeasonLightCycle()
    : DayHour(7), DayToDuskHour(19), DuskToNightHour(20), NightHour(21),
      NightToDayHour(5) {}

//============================================================
/// Tool fct : get a value from a light cycle form, and display a warning if not
/// available
template <class T>
static void getLightCycleValue(const NLGEORGES::UFormElm &item, T &dest,
                               const char *name) {
  nlassert(name);
  if (!item.getValueByName(dest, name))
    nlwarning("Can't get field %s in a season_light_cycle sheet!", name);
}
...
void CLightCycle::build(const NLGEORGES::UFormElm &item) {
  ...
  for (uint k = 0; k < EGSPD::CSeason::Invalid; ++k) {
    ...
    if (item.getNodeByName(&seasonsItem, EGSPD::CSeason::toString(
                                             (EGSPD::CSeason::TSeason)k)) &&
        seasonsItem) {
      SeasonLightCycle[k].build(*seasonsItem);
    } else
      nlwarning("CLightCycle::build : can't get infos about %s",
...
CLightCycle::CLightCycle()
    : RealDayLength(3000), NumHours(24), MaxNumColorSteps(128) {}

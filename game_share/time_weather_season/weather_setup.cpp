 ...
 */
static inline void blendValue(NLMISC::CRGBA &dest, NLMISC::CRGBA c1,
                              NLMISC::CRGBA c2, float blendFactor) {
 ...
 */
static inline void blendValue(uint32 &dest, uint32 u1, uint32 u2,
                              float blendFactor) {
  dest = (uint32)(blendFactor * u2 + (1.f - blendFactor) * u1);
}

//==================================================================================
void CWeatherState::blend(CWeatherState &dest, const CWeatherState &s1,
  ...
  // NB nico : changed this because precipitation starts as soon as a setup
  // starts to blend, so display works better when using localised name of the
  // next setup.
  if (blendFactor == 0.f) {
  ...
  blendValue(dest.FogColorNight, s1.FogColorNight, s2.FogColorNight,
             blendFactor);
  blendValue(dest.FogGradientFactor, s1.FogGradientFactor, s2.FogGradientFactor,
             blendFactor);
  uint k;
  ...
  // Wind intensity
  blendValue(dest.WindIntensity, s1.WindIntensity, s2.WindIntensity,
             blendFactor);
...
//==================================================================================
void CCloudState::blend(CCloudState &dest, const CCloudState &s1,
  ...
  blendValue(dest.DiffusionSpeed, s1.DiffusionSpeed, s2.DiffusionSpeed,
             blendFactor);

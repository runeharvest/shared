 ...
 */
template <class T>
static void GetWeatherFormValue(const NLGEORGES::UFormElm &item, T &destItem,
                                const char *name) {
  nlassert(name);
  if (!item.getValueByName(destItem, name))
    nlwarning("WeatherSetup : can't get %s value, keeping default", name);
}

//==================================================================================
CWeatherStateSheet::CWeatherStateSheet()
    : FogRatio(0), FogColorDay(NLMISC::CRGBA::Black),
      FogColorDusk(NLMISC::CRGBA::Black), FogColorNight(NLMISC::CRGBA::Black),
      FogGradientFactor(1.f), Lighting(1.f), WindIntensity(0),
      ThunderIntensity(0)
...
CCloudStateSheet::CCloudStateSheet()
    : AmbientDay(120, 150, 155), DiffuseDay(220, 250, 255),
      AmbientNight(60, 75, 125), DiffuseNight(110, 125, 200),
      AmbientDusk(210, 116, 21), DiffuseDusk(234, 183, 77), NumClouds(50),
      DiffusionSpeed(8.f) {}
...
void CWeatherSetupSheetBase::readGeorges(
    const NLGEORGES::UForm *form, const NLMISC::CSheetId & /* sheetId */) {

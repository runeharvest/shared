...
CWeatherFunctionParamsSheetBase::CWeatherFunctionParamsSheetBase()
    : DayLength(24), CycleLength(25), MinThunderPeriod(1.f),
      ThunderLength(0.5f), CloudWindSpeedFactor(1.f), CloudMinSpeed(0.f) {}

//=======================================================================
void CWeatherFunctionParamsSheetBase::readGeorges(
...
void CWeatherFunctionParamsSheetBase::build(const std::string &sheetName) {
  ...
  NLMISC::CSmartPtr<NLGEORGES::UForm> form =
      loader->loadForm(sheetName.c_str());
  if (form) {
    build(form->getRootNode());
  } else {

...
/*
  ...
  The user creates classes derived from CTimerEvent to represent their event
  handlers. When the classes' set() or setRemaining() methods are called an
  entry is added to the event system provoking a callback of the user's
  timerCallback() method at the given time. If a CTimer object is destroyed its
  callback is automatically canceled. A single timer may only have a single time
  set for it. Setting the time more than once will cancel the previous time.
  NOTE: The only requirement for the system is that the
  CTimerManager::tickUpdate() method is called every tick
  ...
  Example 2: - a class using a timer and a custom event handler to trigger
  regular actions
  ----------
    class CMyClass;
    class CMyTimerEvent;
...
public:
  ...
  // set the timer for a given target time with given custom event handler
  // object
  ...
  // set the timer for somewhere between (current time + time) and (current time
  // + time + variation) also clear the custom event object
  void setRemaining(NLMISC::TGameCycle time, uint32 variation);

  // set the timer for (current time + time) with given custom event handler
  // object
  void setRemaining(NLMISC::TGameCycle time, CTimerEvent *eventObject);

  // set the timer for somewhere between (current time + time) and (current time
  // + time + variation) also set given custom event handler object
  void setRemaining(NLMISC::TGameCycle time, CTimerEvent *eventObject,
                    uint32 variation);
  ...
  // get the time remaining before the target time for the timer - return 0 if
...
private:
  ...
  // a smart pointer to the timer event - the event is reffed by a second smart
  // ptr from the event system
...
public:
  ...
  // get hold of the object that created the event (or NULL if the object has
  // been deleted)
...
private:
  ...
  // method used by the event system to call the timer callback and kill off the
  // event
  void processEvent();
...
private:
  ...
  // a pointer to the owner object - is NULL if owner object has been deleted or
  // this event has been invalidated
  CTimer *_Owner;
};
...
public:
  ...
  // callback called when the tick service connects - used to ajust time values
  // of event objects
  void syncTick();
...
inline CTimer::~CTimer() { reset(); }

inline void CTimer::set(NLMISC::TGameCycle time) { set(time, new CTimerEvent); }
...
inline void CTimer::setRemaining(NLMISC::TGameCycle time,
                                 CTimerEvent *eventObject) {
  set(CTickEventHandler::getGameCycle() + time, eventObject);
}

inline void CTimer::setRemaining(NLMISC::TGameCycle time,
                                 CTimerEvent *eventObject, uint32 variation) {
...
inline CTimerEvent *CTimer::getEvent() { return _Event; }
...
inline void CTimerEvent::set(CTimer *owner, NLMISC::TGameCycle time) {
  ...
  BOMB_IF(_Owner != NULL && _Owner != owner,
          "Attempt to change owner of an active event", return);
  _Owner = owner;
  _Time = time;
...
inline void CTimerEvent::set(CTimer *owner, NLMISC::TGameCycle time,
                             uint32 variation) {
  BOMB_IF(variation == 0,
          "shouldn't call this method with variation value of 0",
          set(owner, time));
  BOMB_IF(variation > 256,
          "shouldn't call this method with variation value of >256",
          variation = 256);
...
inline NLMISC::TGameCycle CTimerEvent::getTime() const { return _Time; }

inline CTimer *CTimerEvent::getOwner() const { return _Owner; }

inline bool CTimerEvent::isActive() const { return _Owner != NULL; }

inline void CTimerEvent::clear() { _Owner = NULL; }

inline void CTimerEvent::processEvent() {
  CTimer *owner = _Owner;
  BOMB_IF(owner == NULL,
          "Attempt to process an event that no longer has a valid owner",
          return);

  // mark the event as expired - the state may be chnaged during the timer
  // callback... NOTE: This operation results in '_Owner' being set to NULL
  _Owner->reset();
...
inline CTimerManager::TEventVector &
CTimerManager::getEventVector(NLMISC::TGameCycle time) {

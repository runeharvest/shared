...
void CMirroredDataSet::displayUnknownEntities(TDataSetIndex first, TDataSetIndex end, NLMISC::CLog &log)
{
	for (TDataSetIndex i = first; i != end; ++i)
	{
		...
		if (_PropertyContainer.EntityIdArray.isOnline(i) && (_EntityIdToEntityIndex.find(entityId) == _EntityIdToEntityIndex.end()))
		{
...
uint16 CMirroredDataSet::scanAndResyncProp(TPropertyIndex propIndex, CChangeTrackerClientProp *tracker, NLNET::TServiceId serviceId)
{
	...
	for (TDataSetIndex i = 0; i != (uint32)maxNbRows(); ++i)
	{
		...
		if (_PropertyContainer.EntityIdArray.isOnline(i) && (_PropertyContainer.PropertyValueArrays[propIndex].ChangeTimestamps[i] != 0))
		{
...
uint CMirroredDataSet::removeEntityFromDataSet(const NLMISC::CEntityId &entityId)
{
	...
	if (ir != _EntityTypesRanges.end())
	{
		...
		if ((entityIndex < GET_ENTITY_TYPE_RANGE(ir).first()) || (entityIndex > GET_ENTITY_TYPE_RANGE(ir).last()))
		{
...
{
	return (((onlyEntityType == 0xFF) || (entityId.getType() == onlyEntityType)) && ((onlyCreatorId == 0xFF) || (entityId.getCreatorId() == onlyCreatorId)) && ((onlyDynamicId == 0xFF) || (entityId.getDynamicId() == onlyDynamicId)) && ((!hideUndeclared) || _PropertyContainer.EntityIdArray.isOnline(entityIndex)));
}

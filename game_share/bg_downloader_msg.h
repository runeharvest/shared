...
enum TMsgType {
  ...
  BGD_UpdateStatusString = 0, // Sent by background downloader to reflect an
                              // update of its status string in the client
  BGD_State,                  // The downloader gives its state to the client
  BGD_Mode,                   // The downloader gives its mode to the client
  BGD_TaskResult, // Return the result of the last task after a 'GetTaskResult'
                  // request will be followed by a uint32 bitfield giving the
                  // name of the available downloads (as indexed by TDownloadID)
                  // (or 0 else)
  ...
  CL_Stop, // Client ask the downloader to stop (interrupt) its current task,
           // then remain in 'idle' state
  CL_StartTask,  // Client ask the downloader to start a new specific task, the
                 // downloader should be in 'idle' state
  CL_SetMode,    // Put the downloader in 'slave' mode or autonomous, 1 param is
                 // 'TDownloaderMode'
  CL_SetVerbose, // Followed by 'true' or 'false'
                 // In verbose mode, the downloader will send a status string
  CL_Shutdown,   // Client wants the downloader to be shutdown. Happens before a
                 // reboot of both client & downloader
  CL_GetDescFile, // Client ask the description file that is updated when the
                  // last "CheckPatch" succeeded
  CL_Show,
  CL_Hide,
  CL_SetPriority,
...
enum TDownloaderMode {
  ...
  DownloaderMode_Slave,          // In this mode, the downloader won't take the
                        // initiative to start a download task. Client has
                        // authority on the tasks to start
...
public:
  CTaskDesc(TDownloaderState state = DLState_Idle, uint32 downloadBitfield = 0)
      : State(state), DownloadBitfield(downloadBitfield) {}

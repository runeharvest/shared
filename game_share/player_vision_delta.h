...
public:
  ...
  bool empty() const { return EntitiesIn.empty() && EntitiesOut.empty(); }
  ...
  static void
  decodeVisionDelta(NLNET::CMessage &msgin,
                    std::list<CPlayerVisionDelta> &listVisionDelta) {
  ...
  static void
  decodeVisionDelta(NLNET::CMessage &msgin,
                    std::vector<CPlayerVisionDelta> &vectorVisionDelta) {
    while ((uint)msgin.getPos() < (uint)msgin.length()) {

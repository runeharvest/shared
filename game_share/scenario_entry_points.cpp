...
const CScenarioEntryPoints::TEntryPoints &
CScenarioEntryPoints::getEntryPoints() {
...
void CScenarioEntryPoints::setFiles(const std::string &completeIslandsFilename,
                                    const std::string &entryPointsFilename) {
...
uint32 CScenarioEntryPoints::getIslandId(const CSString &island) {
  ...
  for (uint32 i = 0; i < _CompleteIslands.size(); ++i) {
    // if this entry point corresponds to the one we're looking for then stop
    // here
    if (island == _CompleteIslands[i].Island)
      return count;

    // increment the entry point id each time we get here (ie for each entry
    // point in package definition)
    ++count;
  }
...
//-----------------------------------------------------------------------------
CScenarioEntryPoints::CCompleteIsland *
CScenarioEntryPoints::getIslandFromId(const NLMISC::CSString &islandId) {
  // if need be load the entry points vector from disk file
  loadCompleteIslands();
...
//-----------------------------------------------------------------------------
CScenarioEntryPoints::CShortEntryPoint *
CScenarioEntryPoints::getEntryPointFromIds(
    const NLMISC::CSString &islandId, const NLMISC::CSString &entryPointId) {
  // if need be load the entry points vector from disk file
  loadCompleteIslands();
...
void CScenarioEntryPoints::loadFromFile() {
  ...
  for (uint32 i = 0; i < lines.size(); ++i) {
    // strip away any comment in the line and get rid of leading and trailing
    // spaces
    ...
    BOMB_IF((words.size() != 6) ||
            ...
            NLMISC::toString("%s:%d: Invalid syntax: %s",
                             _EntryPointsFilename.c_str(), i + 1,
                             lines[i].splitTo("//").c_str()),
            continue);

    // display an info message
    // nlinfo("Adding Entry point: %s",lines[i].splitTo("//").c_str());

    // split the line into constituent parts and feed it to the entry points
    // vector
    _EntryPoints.push_back(CEntryPoint(words[0], words[1], words[2],
                                       words[3].atoui(), words[4].atosi()));
  }
...
//-----------------------------------------------------------------------------
CScenarioEntryPoints::CCompleteIsland *
CScenarioEntryPoints::getCompleteIslandFromCoords(
  ...
  if (!_LastFoundIsland || fabs((double)pos.x - _LastTestedCoords.x) > 20.f ||
...
void CScenarioEntryPoints::loadFromXMLFile() {
  ...
  for (size_t i = 0; i < _CompleteIslandsFilenames.size(); ++i) {
    ...
    std::string pathFileName =
        _HardIslandsPath ? _CompleteIslandsFilenames[i]
    ...
    if (input.init(file)) {
      ...
      xmlNodePtr islandNode =
          input.getFirstChildNode(islands, "complete_island");
      ...
      while (islandNode != 0) {
        ...
        // island name
        const char *island =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"island");
        if (island == 0) {
        ...
        // package
        /*
        const char *package = (const char*) xmlGetProp(islandNode, (xmlChar*)
        "package"); if(package == 0) nlinfo("no 'package' tag in %s island",
        island); else completeIsland.Package = CSString(package);
        */

        // continent
        const char *continent =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"continent");
        if (continent == 0)
        ...
        // xmin
        const char *xmin =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"xmin");
        if (xmin == 0)
        ...
        // ymin
        const char *ymin =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"ymin");
        if (ymin == 0)
        ...
        // xmax
        const char *xmax =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"xmax");
        if (xmax == 0)
        ...
        // ymax
        const char *ymax =
            (const char *)xmlGetProp(islandNode, (xmlChar *)"ymax");
        if (ymax == 0)
        ...
        for (uint e = 0; e < _EntryPoints.size(); e++) {
          ...
          if (entryPoint.Island == island) {
            ...
            if (package.empty())
              ...
              nlinfo("Different packages for island '%s' in file %s", island,
                     _EntryPointsFilename.c_str());
          }
        }
        if (package.empty())
        ...
        while (zoneNode != 0) {
          // island name
          const char *zoneName =
              (const char *)xmlGetProp(zoneNode, (xmlChar *)"name");
          if (zoneName == 0) {
            nlinfo("no 'zone name' tag in %s",
                   _CompleteIslandsFilenames[i].c_str());
          } else
        ...
        for (std::list<std::string>::iterator it = completeIsland.Zones.begin();
             it != completeIsland.Zones.end(); ++it) {
          sint x, y;
          if (getZonePosFromZoneName(*it, x, y)) {
            completeIsland.ZoneIDs.push_back(((uint16)x & 255) +
                                             ((uint16)(-y - 1) << 8));
          }
        }
...
//-----------------------------------------------------------------------------
const CScenarioEntryPoints::TCompleteIslands &
CScenarioEntryPoints::getCompleteIslands() {
  loadCompleteIslands();
...
//-----------------------------------------------------------------------------
void CScenarioEntryPoints::saveXMLFile(const TCompleteIslands &completeIslands,
  ...
  // setup the file name
  std::string pathFilename = (fileName.find('/') != std::string::npos ||
  ...
  if (output.init(&file, "1.0")) {
    ...
    xmlNodePtr root =
        xmlNewDocNode(xmlDoc, NULL, (const xmlChar *)"islands", NULL);
    ...
    for (uint32 i = 0; i < completeIslands.size(); ++i) {
      ...
      if (islandNodes.find(completeIslands[i].Island) == islandNodes.end()) {
        xmlNodePtr islandNode =
            xmlNewChild(root, NULL, (const xmlChar *)"complete_island", NULL);
        xmlSetProp(islandNode, (const xmlChar *)"island",
                   (const xmlChar *)completeIslands[i].Island.c_str());
        // xmlSetProp(islandNode, (const xmlChar*)"package", (const
        // xmlChar*)completeIslands[i].Package.c_str());
        xmlSetProp(islandNode, (const xmlChar *)"continent",
                   (const xmlChar *)completeIslands[i].Continent.c_str());
        ...
        for (itZone = completeIslands[i].Zones.begin();
             itZone != completeIslands[i].Zones.end(); itZone++) {
          xmlNodePtr zoneNode =
              xmlNewChild(islandNode, NULL, (const xmlChar *)"zone", NULL);
          xmlSetProp(zoneNode, (const xmlChar *)"name",
                     (const xmlChar *)(*itZone).c_str());
        }
...
void CScenarioEntryPoints::getIslands(const CSString& packageDefinition,
...
void CScenarioEntryPoints::getEntryPoints(const CSString& packageDefinition,
const CSString& island, CVectorSString& entryPoints)
{
    // if need be load the entry points vector from disk file
    if (!_IsLoaded)
    {
...
uint32 CScenarioEntryPoints::getEntryPointId(const CSString& packageDefinition,
const CSString& island, const CSString& entryPoint)
{
    // if need be load the entry points vector from disk file
    if (!_IsLoaded)
    {
    ...
    for (uint32 i=0;i<_EntryPoints.size();++i)
    {
        ...
        // if this entry point corresponds to the one we're looking for then
stop here if (island==_EntryPoints[i].Island &&
entryPoint==_EntryPoints[i].Location) return count;
...
point in package definition)
    ...
    // bomb out - we didn't find a match - on live shards this case is to be
ignored as it only means we have a data
    // error and not necessarily a code bug... in debug we might as well STOP to
flag that there's a problem STOP("ERROR: entry point '"+island+":"+entryPoint+"'
not found in package description: "+packageDefinition); return count;
}

//-----------------------------------------------------------------------------

void CScenarioEntryPoints::getEntryPointCoordsFromId(const CSString&
packageDefinition, uint32 id, sint32& x, sint32& y)
{
    // if need be load the entry points vector from disk file
    if (!_IsLoaded)
    {
...
ignored as it only means we have a data
    // error and not necessarily a code bug... in debug we might as well STOP to
flag that there's a problem STOP("ERROR: entry point '"+NLMISC::toString(id)+"'
not found in package description: "+packageDefinition);
...
/*
NLMISC_COMMAND(displayScenarioEntryPoints,"display the list of scenario entry
points (either the complete list or the list for a given input
string","[<package definition string>]")
{
    ...
    nlinfo("Displaying scenario entry points correspoding to package definition:
'%s'",packageDefinition.c_str());
    ...
    for (uint32 i=0;i<islands.size();++i)
    {
        ...
        {
            // lookup the entry point id from the package definition, island
name and entry point name uint32 entryPointId=
R2::CScenarioEntryPoints::getInstance().getEntryPointId(packageDefinition,islands[i],entryPoints[j]);
            ...
            nlinfo("    - Entry point %d: %s (%d,%d)", entryPointId,
entryPoints[j].c_str(), x, y);
        }
    }

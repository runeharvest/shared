...
inline void CPersistentDataRecord::addString(const std::string &name,
                                             uint16 &result) {
...
inline void CPersistentDataRecord::push(TToken token, sint64 val) {
  // create a union for splitting the i64 value into 2 32bit parts and map the
  // union onto the input value
...
inline void CPersistentDataRecord::push(TToken token,
                                        const NLMISC::CEntityId &val) {
  // this one is a bit complictaed to unrole by hand - better leave it to
  // standard CArg::push()
...
inline void CPersistentDataRecord::pushStructEnd(TToken token) {
  ...
  BOMB_IF(((token << 3) >> 3) != token,
          "Invalid token - Insufficient numeric precision", return);
  BOMB_IF(_WritingStructStack.empty(), "Trying to pop past end of stack",
          return);
  BOMB_IF(_WritingStructStack.back() != token,
          "Attempting to end a structure with the wrong delimiting token",
          return);
#endif
...
inline bool CPersistentDataRecord::isEndOfData() const {
  DROP_IF((_TokenOffset == _TokenTable.size()) &&
              !(_ArgOffset == _ArgTable.size()),
          "Argument table and token table sizes don't match", return true);
  DROP_IF(_TokenOffset > _TokenTable.size(),
          "Attempt to access beyond end of data...", return true);
...
inline bool CPersistentDataRecord::isEndOfStruct() const {
  ...
  if (peekNextTokenType() != CArg::STRUCT_END)
    return false;
  DROP_IF(_ReadingStructStack.back() != peekNextToken(),
          "Opening and closing structure tokens don't match", return false)
  return true;
}

inline bool CPersistentDataRecord::isStartOfStruct() const {
  //	DROP_IF(isEndOfData(),"Attempt to read past end of input data",return
  //0);
...
inline CPersistentDataRecord::TToken
...
inline void
CPersistentDataRecord::peekNextArg(CPersistentDataRecord::CArg &result) const {
  result.setType(peekNextTokenType());
  if (result.isExtended()) {
    BOMB_IF(_ArgOffset + 1 >= _ArgTable.size(),
            "Attempt to overrun end of input data", return);
    DROP_IF((_TokenTable[_TokenOffset + 0] & ~7) !=
                (_TokenTable[_TokenOffset + 1] & ~7),
            "2 dwords of 64 bit have non-matching identifiers", return);
    ...
    if (result._Type == CArg::EXTEND_TYPE &&
        result._Value.ExType >= CArg::ET_64_BIT_EXTENDED_TYPES) {
      // this is a 96 bit extended type, read one more value
      BOMB_IF(_ArgOffset + 2 >= _ArgTable.size(),
              "Attempt to overrun end of input data", return);
      BOMB_IF((_TokenTable[_TokenOffset + 0] & ~7) !=
                  (_TokenTable[_TokenOffset + 2] & ~7),
              "3 dwords of 96 bit have non-matching identifiers", return);
      result._Value.ex32_2 = _ArgTable[_ArgOffset + 2];

      nlassert((uint64(result._Value.ex32_2) << 32 | result._Value.ex32_1) ==
               result._Value.ExData64);
    }
...
//	if (arg.isExtended())
//	{
//		BOMB_IF(_ArgOffset+1>=_ArgTable.size(),"Attempt to overrun end
//of input data",return arg);
//		DROP_IF((_TokenTable[_TokenOffset+0]&~7)!=(_TokenTable[_TokenOffset+1]&~7),"2
//dwords of 64 bit have non-matching identifiers",return arg); 		arg._Value.i32_1
//= _ArgTable[_ArgOffset+0]; 		arg._Value.i32_2 = _ArgTable[_ArgOffset+1];
...
//
//		if (arg._Type == CArg::EXTEND_TYPE && arg._Value.ExType >=
//CArg::ET_64_BIT_EXTENDED_TYPES)
//		{
//			// this is a 96 bit extended type, read one more value
//			BOMB_IF(_ArgOffset+2>=_ArgTable.size(),"Attempt to
//overrun end of input data",return arg);
//			BOMB_IF((_TokenTable[_TokenOffset+0]&~7)!=(_TokenTable[_TokenOffset+2]&~7),"3
//dwords of 96 bit have non-matching identifiers",return arg); 			arg._Value.ex32_2
//= _ArgTable[_ArgOffset+2];
//
//			nlassert((uint64(arg._Value.ex32_2)<<32|arg._Value.ex32_1)
//== arg._Value.ExData64);
//		}
//	}
//	else if (!arg.isFlag())
...
inline void
CPersistentDataRecord::popNextArg(TToken token,
                                  CPersistentDataRecord::CArg &result) {
#ifdef NL_DEBUG
  BOMB_IF(peekNextToken() != token,
          "Error on read code - token requested doesn't match token found",
          return);
#else
...
// inline CPersistentDataRecord::CArg CPersistentDataRecord::popNextArg(TToken
// token)
//{
//	CArg arg;
//	DROP_IF(peekNextToken()!=token,"Error on read code - token requested
//doesn't match token found",return arg); 	arg= peekNextArg(); 	if (arg.isFlag())
//	{
//		++_TokenOffset;
//	}
...
inline void CPersistentDataRecord::pop(TToken token,
                                       NLMISC::CEntityId &result) {
  popNextArg(token, TempArg);
  result = TempArg.asEntityId();
}

inline void CPersistentDataRecord::pop(TToken /* token */) { ++_TokenOffset; }

inline void CPersistentDataRecord::popStructBegin(TToken token) {
  DROP_IF(peekNextToken() != token,
          "Attempting to enter a structure with the wrong delimiting token",
          return);
  DROP_IF(
      peekNextTokenType() != CArg::STRUCT_BEGIN,
      "Attempting to enter a structure with the wrong delimiting token type",
      return);
...
inline void CPersistentDataRecord::popStructEnd(TToken token) {
  DROP_IF(_ReadingStructStack.empty(),
          "Attempting to pop end of a structure with nothing left in the open "
          "structure stack",
          return);
  ...
  DROP_IF(
      topToken != token,
      "Attempting to pop end of a structure with the wrong delimiting token",
      return);
  DROP_IF(
      nextToken != token,
      "Attempting to pop end of a structure with the wrong delimiting token",
      return);
  DROP_IF(
      peekNextTokenType() != CArg::STRUCT_END,
      "Attempting to leave a structure with the wrong delimiting token type",
      return);
...
inline CPersistentDataRecord::CArg::CArg(const std::string &type,
  ...
  switch (_Type) {
  case SINT32:
    _Value.i32 = NLMISC::CSString(value).atoi();
    break;
  case UINT32:
    _Value.i32 = NLMISC::CSString(value).atoi();
    break;
  case SINT64:
    _Value.i64 = NLMISC::atoiInt64(value.c_str());
    break;
  case UINT64:
    _Value.i64 = NLMISC::atoiInt64(value.c_str());
    break;
  case FLOAT32:
    NLMISC::fromString(value, _Value.f32);
    break;
  case FLOAT64:
    NLMISC::fromString(value, _Value.f64);
    break;
  case STRING:
    _Value.i32 = pdr.addString(value);
    _String = value;
    break;
  case EXTEND_TYPE:
    ...
    case ET_SHEET_ID: {
      ...
      if (value.size() != 0 && value[0] == '#') {
        ...
        STOP_IF((sheetId == NLMISC::CSheetId::Unknown &&
                 value != "unknown.unknown"),
                "sheet_id.bin version does not match save game version");
...
inline uint64 CPersistentDataRecord::CArg::asUint() const {
  ...
  case SINT32:
    return (uint64)(sint32)_Value.i32;
  case UINT32:
    return (uint64)(uint32)_Value.i32; // 2 casts to ensure no sign extend
  case SINT64:
    return (uint64)_Value.i64;
  case UINT64:
    return (uint64)_Value.i64;
  case FLOAT32:
    return (uint64)_Value.f32;
  case FLOAT64:
    return (uint64)_Value.f64;
  case STRING:
    return (uint64)NLMISC::atoiInt64(_String.c_str());
  case FLAG:
    return 1;
  case EXTEND_TYPE:
    switch (_Value.ExType) {
    case ET_SHEET_ID:
      return _Value.ExData32;
    case ET_ENTITY_ID:
      return _Value.ExData64;
    default:
      break;
    }
  default:
...
inline sint64 CPersistentDataRecord::CArg::asSint() const {
  ...
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter", return 0);
  case SINT32:
    return (sint64)(sint32)_Value.i32;
  case UINT32:
    return (sint64)(uint32)_Value.i32;
  case SINT64:
    return (sint64)_Value.i64;
  case UINT64:
    return (sint64)_Value.i64;
  case FLOAT32:
    return (sint64)_Value.f32;
  case FLOAT64:
    return (sint64)_Value.f64;
  case STRING:
    return (sint64)NLMISC::atoiInt64(_String.c_str());
  case FLAG:
    return 1;
  case EXTEND_TYPE:
    switch (_Value.ExType) {
...
inline float CPersistentDataRecord::CArg::asFloat() const {
  ...
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter", return 0);
  case SINT32:
    return (float)(sint32)_Value.i32;
  case UINT32:
    return (float)(uint32)_Value.i32;
  case SINT64:
    return (float)(sint64)_Value.i64;
  case UINT64:
    return (float)(uint64)_Value.i64;
  case FLOAT32:
    return (float)_Value.f32;
  case FLOAT64:
    return (float)_Value.f64;
  case STRING: {
    float val;
  ...
  case EXTEND_TYPE:
    switch (_Value.ExType) {
    case ET_SHEET_ID:
      return float(_Value.ExData32);
    case ET_ENTITY_ID:
      return float(_Value.ExData64);
    default:
      break;
    }
  default:
...
inline double CPersistentDataRecord::CArg::asDouble() const {
  ...
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter", return 0);
  case SINT32:
    return (double)(sint32)_Value.i32;
  case UINT32:
    return (double)(uint32)_Value.i32;
  case SINT64:
    return (double)(sint64)_Value.i64;
  case UINT64:
    return (double)(uint64)_Value.i64;
  case FLOAT32:
    return (double)_Value.f32;
  case FLOAT64:
    return (double)_Value.f64;
  case STRING: {
    double val;
    NLMISC::fromString(_String, val);
    return val;
  ...
  case EXTEND_TYPE:
    switch (_Value.ExType) {
    case ET_SHEET_ID:
      return double(_Value.ExData32);
    case ET_ENTITY_ID:
      return double(_Value.ExData64);
    default:
      break;
    }
  default:
...
inline NLMISC::CSString CPersistentDataRecord::CArg::asString() const {
  ...
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter", return 0);
  case SINT32:
    return NLMISC::toString((sint32)_Value.i32);
  case UINT32:
    return NLMISC::toString((uint32)_Value.i32);
  case SINT64:
    return NLMISC::toString((sint64)_Value.i64);
  case UINT64:
    return NLMISC::toString((uint64)_Value.i64);
  case FLOAT32:
    return NLMISC::toString(_Value.f32);
  case FLOAT64:
    return NLMISC::toString(_Value.f64);
  case STRING:
    return _String;
  case FLAG:
    return "1";
...
inline ucstring CPersistentDataRecord::CArg::asUCString() const {
  ...
  case SINT32:
    return (ucstring)NLMISC::toString((sint32)_Value.i32);
  case UINT32:
    return (ucstring)NLMISC::toString((uint32)_Value.i32);
  case SINT64:
    return (ucstring)NLMISC::toString((sint64)_Value.i64);
  case UINT64:
    return (ucstring)NLMISC::toString((uint64)_Value.i64);
  case FLOAT32:
    return (ucstring)NLMISC::toString(_Value.f32);
  case FLOAT64:
    return (ucstring)NLMISC::toString(_Value.f64);
  case STRING: {
    ucstring s;
    s.fromUtf8(_String);
    return s;
...
inline NLMISC::CEntityId CPersistentDataRecord::CArg::asEntityId() const {
  ...
  case STRUCT_BEGIN:
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter",
         return NLMISC::CEntityId());
  case SINT32:
    return NLMISC::CEntityId((uint64)_Value.i32);
  case UINT32:
    return NLMISC::CEntityId((uint64)_Value.i32);
  case SINT64:
    return NLMISC::CEntityId((uint64)_Value.i64);
  case UINT64:
    return NLMISC::CEntityId((uint64)_Value.i64);
  case FLOAT32:
    return NLMISC::CEntityId((uint64)_Value.f32);
  case FLOAT64:
    return NLMISC::CEntityId((uint64)_Value.f64);
  case STRING: {
...
inline NLMISC::CSheetId CPersistentDataRecord::CArg::asSheetId() const {
  ...
  case STRUCT_END:
    BOMB("Can't extract a value from a structure delimiter",
         return NLMISC::CSheetId(0u));
  case SINT32:
    return NLMISC::CSheetId((uint32)_Value.i32);
  case UINT32:
    return NLMISC::CSheetId((uint32)_Value.i32);
  case SINT64:
    return NLMISC::CSheetId((uint32)_Value.i64);
  case UINT64:
    return NLMISC::CSheetId((uint32)_Value.i64);
  case FLOAT32:
    return NLMISC::CSheetId((uint32)_Value.f32);
  case FLOAT64:
    return NLMISC::CSheetId((uint32)_Value.f64);
  case STRING:
...
inline bool CPersistentDataRecord::CArg::setType(const std::string &name) {
  ...
  _Type = EXTEND_TYPE;
  {
    _Value.ExType = ET_SHEET_ID;
    if (typeName() == name)
      return true;
    _Value.ExType = ET_ENTITY_ID;
    if (typeName() == name)
      return true;
  }
  DROP(("Failed to find match for type name '" + name +
...
inline void
CPersistentDataRecord::CArg::setType(CPersistentDataRecord::CArg::TType value) {
...
inline void
CPersistentDataRecord::CArg::push(TToken token, std::vector<TToken> &tokenTable,
                                  std::vector<uint32> &argTable) const {
  BOMB_IF(((token << 3) >> 3) != token,
          "Invalid token - Insufficient numeric precision", return);
  switch (_Type) {
  case FLAG:
...
inline CPersistentDataRecord::CArg
...
inline CPersistentDataRecord::CArg
CPersistentDataRecord::CArg::String(const std::string &value,
                                    CPersistentDataRecord &pdr) {
  CArg arg;
  arg._Type = STRING;
...
inline CPersistentDataRecord::CArg
CPersistentDataRecord::CArg::UCString(const ucstring &value,
                                      CPersistentDataRecord &pdr) {
...
inline CPersistentDataRecord::CArg::TType
CPersistentDataRecord::CArg::token2Type(uint32 token, bool extend) {
  switch (token) {
  case BEGIN_TOKEN:
    return STRUCT_BEGIN;
  case END_TOKEN:
  ...
  case FLAG_TOKEN:
    return FLAG;
  case SINT_TOKEN:
    return extend ? SINT64 : SINT32;
  case UINT_TOKEN:
    return extend ? UINT64 : UINT32;
  case FLOAT_TOKEN:
    return extend ? FLOAT64 : FLOAT32;
  case STRING_TOKEN:
    return extend ? EXTEND_TYPE : STRING;
  }
  STOP("This should never happen!");
  return CArg::TType(0);
}

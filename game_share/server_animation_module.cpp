...
// Debug
CVariable<bool> WriteScenarioDebugDataToFile(
    "DSS", "WriteScenarioDebugDataToFile",
    "Set to true to write scenario rtdata and xml primitive to file", false, 0,
    true);
CVariable<uint32> DelayBeforeStartAct(
    "DSS", "DelayBeforeStartAct",
    "The delay wait by player/dm between the moment a dm/ai has decided the "
    "change of act and the moment of the real change",
    30, 0, true);
CVariable<uint32>
    DelayBeforeNewLocation("DSS", "DelayBeforeNewLocation",
                           "The min delay between 2 change of location", 25, 0,
                           true);
CVariable<bool> UseSheetClientWithLevel(
    "DSS", "UseSheetClientWithLevel",
    "use sheet client like basic_fyros_male_f2.creature instead of "
    "basic_fyros_male_f2.creature",
    true, 0, true);
...
public:
  CRtNpc(TAIAlias alias, CObject *objectData, TAIAlias grpAlias,
         uint32 dmProperty) {
    Alias = alias;
...
public:
  ...
  bool activateEasterEgg(uint32 easterEggId, const std::string &grpControler) {
    ...
    if (first == last) {
      nlwarning("SAM: Error while activating easter egg. The controler grp is "
                "not valid.");
      return false;
    }
    return ActiveEasterEggs.insert(std::make_pair(easterEggId, grpControler))
        .second;
  }
...
public:
  ...
  bool remove(const NLMISC::CEntityId &eid,
              const NLMISC::CEntityId &creatureId);

  NLMISC::CEntityId getEntity(const NLMISC::CEntityId &eid, uint32 id) const;

  void getBots(const NLMISC::CEntityId &eid,
               std::map<uint32, NLMISC::CEntityId> &bots);
...
void CBotControler::getBots(uint32 charId,
                            std::map<uint32, NLMISC::CEntityId> &bots) {
  TControlledEntitiesList::iterator found(_List.find(charId));
  if (found == _List.end()) {
    return;
  }
  const std::map<uint32, NLMISC::CEntityId> &entities =
      found->second.getAToBMap();
  bots = entities;
}
...
bool CBotControler::add(const NLMISC::CEntityId &eid,
  ...
  if (found == _List.end()) {
    found = _List
                .insert(std::make_pair(charId,
                                       CTwinMap<uint32, NLMISC::CEntityId>()))
                .first;
  }
...
bool CBotControler::remove(const NLMISC::CEntityId &eid,
                           const NLMISC::CEntityId &creatureId) {
  TCharId charId = static_cast<TCharId>(eid.getShortId());
  TControlledEntitiesList::iterator found = _List.find(charId);
  if (found == _List.end()) {
  ...
  if (twin.getAToBMap().empty()) {
    _List.erase(found);
  }
  return true;
}

NLMISC::CEntityId CBotControler::getEntity(const NLMISC::CEntityId &eid,
                                           uint32 id) const {
  TCharId charId = static_cast<TCharId>(eid.getShortId());
  TControlledEntitiesList::const_iterator found = _List.find(charId);
...
public:
  ...
  uint16 WeatherValue; // current weather in the [1, 1023] range, or 0 for auto
                       // weather weather value can come from the scenario
                       // description, or may have been changed by an animator
  uint8 CurrSeason;  // current season or ~0 for no season set yet
  bool StartingAct;  // 1 if a player/ia has schedule a startAct
  uint32 InitialAct; // act at which begin the scenario (1) for animation
                     // currentAct for test
...
public:
  ...
  void updateUserTriggerDescriptions(
      TUserTriggerDescriptions &userTriggerDescriptions);

  void updateActPositionDescriptions(
      TActPositionDescriptions &actPositionDescriptions);

  void easterEggLooted(uint32 easterEggId, TSessionId scenarioId);
};

void CAnimationSession::easterEggLooted(uint32 easterEggId,
                                        TSessionId /* scenarioId */) {

  TActiveEasterEggs::const_iterator found = ActiveEasterEggs.find(easterEggId);
  DROP_IF(found == ActiveEasterEggs.end(),
          "Error try to loot an unactive easter egg", return);
  uint32 actId = found->second;
  DROP_IF(actId >= Acts.size(), "Error try to loot an unactive easter egg",
          return);
  CRtAct *rtAct = Acts[actId];

  CRtAct::TActiveEasterEggs::const_iterator found2 =
      rtAct->ActiveEasterEggs.find(easterEggId);
  DROP_IF(found2 == rtAct->ActiveEasterEggs.end(),
          "Error try to loot an unactive easter egg", return);
...
void CAnimationSession::sendWeatherValueToChar(uint32 charId,
                                               uint16 weatherValue) const {
...
void CAnimationSession::updateUserTriggerDescriptions(
    TUserTriggerDescriptions &userTriggerDescriptions) {
...
void CAnimationSession::updateActPositionDescriptions(
    TActPositionDescriptions &actPositionDescriptions) {
...
public:
  ...
  void setAttributeAsString(const std::string &attrName,
  ...
  void setAttributeAsStringWithPrefix(const std::string &attrName,
                                      const std::string &propName,
                                      const std::string &prefix) {
    CObject *attr = _Object->getAttr(attrName);
    if (attr && attr->isString()) {
      std::string str = attr->toString();
      _Primitive->addPropertyByName(propName.c_str(),
                                    new CPropertyString(prefix + str));
    }
  }

  void setAiStateName(const std::string &prefix);

  void setAttributeAsStringArray(const std::string &attrName,
                                 const std::string &propName);

  void setAttributeAsStringArrayWithPrefix(const std::string &attrName,
                                           const std::string &propName,
                                           const std::string &prefix) {
    CObject *attr = _Object->getAttr(attrName);
    if (attr && attr->isString()) {
      ...
      _Primitive->addPropertyByName(propName.c_str(),
                                    new CPropertyStringArray(result));
    }
  }

  void setAttributeAsBool(const std::string &attrName,
                          const std::string &propName) {
    CObject *attr = _Object->getAttr(attrName);
    if (attr && attr->isInteger()) {
      sint value = static_cast<sint>(attr->toInteger());
      _Primitive->addPropertyByName(
          propName.c_str(), new CPropertyString(value ? "true" : "false"));
    }
  }
  ...
  void setPrimPoint() {
    ...
    if (pt) {
      ...
      if (attr && attr->isNumber()) {
        point->Point.x = static_cast<float>(attr->toNumber());
      }

      attr = pt->getAttr("y");
      if (attr && attr->isNumber()) {
        point->Point.y = static_cast<float>(attr->toNumber());
      }

      attr = pt->getAttr("z");
      if (attr && attr->isNumber()) {
        point->Point.z = static_cast<float>(0.0);
      }
    }

    attr = _Object->getAttr("Angle");
    if (attr && attr->isNumber()) {
      point->Angle = static_cast<float>(attr->toNumber());
    }
  }
...
void CAttributeToProperty::setAttributeAsStringArray(
    const std::string &attrName, const std::string &propName) {
  CObject *attr = _Object->getAttr(attrName);
  if (attr && attr->isString()) {
    string str = attr->toString();
    vector<string> result;
...
void CAttributeToProperty::setPrimPath() {
  ...
  if (pts) {
    ...
    for (; first != last; ++first) {
      ...
      attr = pt->getAttr("x");
      if (attr && attr->isNumber()) {
        points->VPoints[first].x = static_cast<float>(attr->toNumber());
      }

      attr = pt->getAttr("y");
      if (attr && attr->isNumber()) {
        points->VPoints[first].y = static_cast<float>(attr->toNumber());
      }

      attr = pt->getAttr("z");
      if (attr && attr->isNumber()) {
        points->VPoints[first].z = static_cast<float>(attr->toNumber());
      }
    }
  }
}
...
public:
  ...
  CTaskBroadcast(CServerAnimationModule *animationModule, TSessionId sessionId,
                 CMessage &msg)
      : _AnimationModule(animationModule), _SessionId(sessionId) {
    _Msg.swap(msg);
  }

  void doOperation() { _AnimationModule->broadcastMsg(_SessionId, _Msg); }
...
public:
  CTaskStartAct(NLMISC::TTime t, CServerAnimationModule *animationModule,
                TSessionId sessionId, uint32 actId, bool mustTp)
      : CTask<NLMISC::TTime>(t), _AnimationModule(animationModule),
  ...
  void doOperation() {
    ...
    if (session && session->StartingAct) {
      ...
      if (_MustTp) {
        ...
        std::vector<uint32>::const_iterator first(
            session->ConnectedChars.begin()),
            last(session->ConnectedChars.end());
        for (; first != last; ++first) {
...
public:
  CTaskScheduleStartSession(CServerAnimationModule *animationModule,
                            const CAnimationMessageAnimationStart &msg)
      : _AnimationModule(animationModule), _Msg(msg) {}

  void doOperation() { _AnimationModule->scheduleStartSessionImpl(_Msg); }
...
void CServerAnimationModule::broadcastMessage(CAnimationSession *session,
                                              const NLNET::CMessage &msg) {
  if (!session->ConnectedChars.empty()) {
    std::vector<uint32>::const_iterator first(session->ConnectedChars.begin()),
        last(session->ConnectedChars.end());
    for (; first != last; ++first) {
      const NLNET::TModuleProxyPtr *ptr =
          getEditionModule()->getClientProxyPtr(*first);
      if (ptr) {
...
void CServerAnimationModule::scheduleStartAct(TSessionId sessionId,
                                              uint32 actId) {
  ...
  if (session) {
    ...
    if (!session->StartingAct) {
      ...
      if (0 < actId && actId < session->getActCount()) {
        ...
        DROP_IF(nextLocationId >= session->Locations.size(), "Invalid Location",
                return);
        DROP_IF(currentLoctionId >= session->Locations.size(),
                "Invalid Location", return);

        bool mustTp = nextLocationId != currentLoctionId;

        NLMISC::TTime delay = mustTp ? 3000  // Tp with jump
                                     : 1000; // Normal TP
        ...
        _Tasks.addTask(new CTaskStartAct(NLMISC::CTime::getLocalTime() + delay,
                                         this, sessionId, actId, mustTp));
        if (mustTp) {
          broadcastMessage(
              session,
              CShareClientEditionItfProxy::buildMessageFor_scheduleStartAct(
                  msg, 0, actId, uint32(delay / 1000)));
        }

        if (!session->Acts[actId]->PreActDescription.empty()) {
          NLNET::CMessage msg;
          CShareClientEditionItfProxy::buildMessageFor_systemMsg(
              msg, "BC_ML", "", session->Acts[actId]->PreActDescription);
          _Tasks.addTaskAt(NLMISC::CTime::getLocalTime() + delay - 3000,
                           new CTaskBroadcast(this, sessionId, msg));
        }
      } else {
        broadcastMessage(
            session,
            CShareClientEditionItfProxy::buildMessageFor_scheduleStartAct(
                msg, 2, actId, DelayBeforeStartAct));
      }
    } else {
      broadcastMessage(
          session,
          CShareClientEditionItfProxy::buildMessageFor_scheduleStartAct(
              msg, 1, actId, DelayBeforeStartAct));
    }
  }
}

void CServerAnimationModule::init(NLNET::IModuleSocket *gateway,
                                  CDynamicMapService *server) {
  _Server = server;

  CAiWrapper::getInstance().init(
  ...
  const std::vector<TServiceId> &connectionList =
      CUnifiedNetwork::getInstance()->getConnectionList();
  for (uint i = (uint)connectionList.size(); i > 0; --i) {
    nldebug("R2An: %s is already up",
            CUnifiedNetwork::getInstance()
                ->getServiceName(connectionList[i - 1])
                .c_str());
    if ("AIS" ==
        CUnifiedNetwork::getInstance()->getServiceName(connectionList[i - 1])) {
      _ReadyForNextSession = true;
    }
  }
}

CServerAnimationModule::CServerAnimationModule() : _ReadyForNextSession(false) {
...
IPrimitive *CServerAnimationModule::getAction(CObject *action,
                                              const std::string &prefix,
                                              TSessionId scenarioId) {
  if (!action) {
    nlwarning("Error while generating primitives in scenario '%u'",
              scenarioId.asInt());
    return 0;
  }
  IPrimitive *pAction =
      dynamic_cast<IPrimitive *>(CClassRegistry::create("CPrimNode"));
  pAction->addPropertyByName("class",
                             new CPropertyString("npc_event_handler_action"));
  pAction->addPropertyByName("ai_type",
                             new CPropertyString("NPC_EVENT_ACTION")); // AJM
  ...
  a2pAction.setAttributeAsString(
      "Action", "name"); // AJM: default value for name <- action
  ...
  if (tmp && tmp->isString() && tmp->toString() == "npc_say") {
    ...
    if (tmp2 && tmp2->isString()) {
      ...
      if (param.splitLines(result)) {
        ...
        if (result.empty()) {
          nlwarning("ERROR: npc_say but no parameters !!! %u in session ",
                    scenarioId.asInt());
        } else {
          ...
          if (result.size() >= 1) {
            ...
            if (result.size() >= 2) {
              res[1] =
                  toString("DSS_%u %s", scenarioId.asInt(), result[1].c_str());
    ...
    if (caction == "begin_state" || caction.left(14) == "trigger_event_" ||
        caction == "facing") {
      a2pAction.setAttributeAsStringArrayWithPrefix("Parameters", "parameters",
                                                    prefix);
      ...
      if (attr && attr->isString()) {
        ...
        pAction->addPropertyByName(propName.c_str(),
                                   new CPropertyStringArray(result));
      }
    } else if (caction == "modify_variable" || caction == "condition_if" ||
               caction == "condition_if_else" || caction == "dynamic_if") {
      CSString str = action->toString("Parameters");
      if (str.find(":") != string::npos) {
        a2pAction.setAttributeAsStringArrayWithPrefix("Parameters",
                                                      "parameters", prefix);
      } else {
  ...
  for (uint32 i = 0; i < nb; ++i) {
    ...
    if (!tmp) {
      nlwarning("Error in action %s nb: %u", action->toString("Name").c_str(),
                i);
      return 0;
    }
...
bool CServerAnimationModule::queueSession(CAnimationSession *session,
                                          bool /* runTest */) {
...
void CServerAnimationModule::startTest(TSessionId sessionId,
  ...
  nlinfo("R2An: startTest for sessionId %u in ai instance %u",
         sessionId.asInt(), aiInstance);
...
void CServerAnimationModule::startAct(TSessionId sessionId, uint32 actId) {
  ...
  DROP_IF(actId < 1 || animSession->Pdrs.size() <= actId, "Invalid Act",
          return);
  ...
  // update entry point
  IServerEditionModule *svEditionModule =
      _Server->getEditionModule(); // note: in the future the modules could be
                                   // in two distinct process, thus they would
                                   // need an interface/proxy communication
  BOMB_IF(!svEditionModule, "Server edition module not found",
          return); // Can not happend

  CScenario *scenarioSession = svEditionModule->getScenarioById(sessionId);
  BOMB_IF(!scenarioSession,
          NLMISC::toString("Scenario Session not found for session %u",
                           sessionId.asInt()),
          return);
  ...
  if (!scenario) {
    ...
    nlwarning("ERROR: you must check the backup : session_%d_?.r2",
              sessionId.asInt());
    BOMB(NLMISC::toString("BIG ERROR: Scenario was not found for session %u, "
                          "it previously lead to dss crash",
                          sessionId.asInt()),
         return);
    return;
  }
...

  {
    ...
    NLNET::CMessage msg;
    CShareClientEditionItfProxy::buildMessageFor_scheduleStartAct(msg, 0, actId,
                                                                  0);
    _Tasks.addTaskAt(NLMISC::CTime::getLocalTime() + 3000,
                     new CTaskBroadcast(this, sessionId, msg));
  }
  ...
  if (!animSession->Acts[actId]->ActDescription.empty()) {
    NLNET::CMessage msg;
    CShareClientEditionItfProxy::buildMessageFor_systemMsg(
        msg, "BC_ML", "", animSession->Acts[actId]->ActDescription);
    _Tasks.addTaskAt(NLMISC::CTime::getLocalTime() + 3000,
                     new CTaskBroadcast(this, sessionId, msg));
  }
  ...
  BOMB_IF(
      !session,
      NLMISC::toString("Session not found for session %u", sessionId.asInt()),
      return);
  for (uint32 i = 0; i < session->ConnectedChars.size(); ++i) {
    uint32 charId = session->ConnectedChars[i];

    const NLNET::TModuleProxyPtr *pUserModuleProxy =
        getEditionModule()->getClientProxyPtr(charId);
    if (pUserModuleProxy) {
      if (!_CharacterControlProxy.isNull()) {
      ...
      nlinfo("The user %u has quit the animation session %u before its start",
             charId, sessionId.asInt());
    }
  }
  // Use Initial session in case of linked act
  CAiWrapper::getInstance().startAct(session->SessionId, aiInstance,
                                     animSession->Pdrs[actId]);
}

void CServerAnimationModule::setWeatherValue(TSessionId sessionId,
                                             uint16 weatherValue) {
  ...
  if (weatherValue == session->WeatherValue)
    return;
  session->setWeatherValue(weatherValue);
}

void CServerAnimationModule::setSeasonValue(TSessionId sessionId,
                                            uint8 seasonValue) {
  if (!_Server->useNetwork()) {
  ...
  if (seasonValue == session->CurrSeason)
    return;
  session->setSeason(seasonValue);
}

bool CServerAnimationModule::translateActToPrimitive(
    CInstanceMap &components, CAnimationSession *animSession, CObject *act,
  ...
  if (act->isInteger("LocationId") && act->isString("Name") &&
      act->isString("ActDescription") && act->isString("PreActDescription")) {
    ...
    nlwarning("Error in session '%u' data Corrupted",
              animSession->SessionId.asInt());
    return false;
  }

  nlassert(primDoc.RootNode != NULL);
  primDoc.RootNode->addPropertyByName("class",
                                      new CPropertyString("root"));     // AJM
  ...
  if (cfg) {
    ...
    if (r2Cfg) {
      ...
      if (actId == 0) {
        type = CR2LigoConfig::Base;
      } else {
        type = CR2LigoConfig::Act;
      }
      primDoc.setAliasStaticPart(
          r2Cfg->getStaticAliasMapping(animSession->AiInstance, type));
    }
  }
  ...
  std::string managerPrefix =
      toString("r2.%04d.", animSession->SessionId.asInt());
  ...
  {
    IPrimitive *npc_manager =
        dynamic_cast<IPrimitive *>(CClassRegistry::create("CPrimZone"));
    ...
    npc_manager->addPropertyByName("ai_type",
                                   new CPropertyString("MANAGER")); // AJM
    npc_manager->addPropertyByName("ai_manager_type",
                                   new CPropertyString("NPC")); // AJM
    npc_manager->addPropertyByName("trigger_type",
                                   new CPropertyString("npc_zone")); // AJM
    primDoc.RootNode->insertChild(npc_manager);
    CPrimAlias *npc_manager_alias =
        dynamic_cast<CPrimAlias *>(CClassRegistry::create("CPrimAlias"));
  ...
  // Trigger Manager
  {
    ...
    std::string managerName =
        managerPrefix + (actId == 0 ? "base.zone_trigger" : "act.zone_trigger");
    npc_manager->addPropertyByName("name", new CPropertyString(managerName));
    npc_manager->addPropertyByName("ai_type",
                                   new CPropertyString("MANAGER")); // AJM
    npc_manager->addPropertyByName("ai_manager_type",
  ...
  for (; firstAiState != lastAiState; ++firstAiState) {
    ...
    if (aiMovement == "follow_route") {
      ...
      state->addPropertyByName("ai_type",
                               new CPropertyString("NPC_STATE_ROUTE")); // AJM
      state->addPropertyByName("ai_profile_params",
                               new CPropertyStringArray()); // AJM
      state->addPropertyByName("vertical_pos",
                               new CPropertyString("auto")); // AJM
    } else {
      state = dynamic_cast<IPrimitive *>(CClassRegistry::create("CPrimZone"));
      state->addPropertyByName("class", new CPropertyString("npc_zone"));
      state->addPropertyByName("ai_type",
                               new CPropertyString("NPC_STATE_ZONE")); // AJM
      state->addPropertyByName("ai_profile_params",
                               new CPropertyStringArray()); // AJM
      state->addPropertyByName("vertical_pos",
                               new CPropertyString("auto"));            // AJM
    ...
    a2pAiState.setAttributeAsStringArray("AiProfilesParams",
                                         "ai_profiles_params");
    ...
    for (; firstChild != lastChild; ++firstChild) {
      ...
      IPrimitive *npc_group =
          dynamic_cast<IPrimitive *>(CClassRegistry::create("CPrimNode"));
      npc_group->addPropertyByName("class", new CPropertyString("npc_group"));

      npc_group->addPropertyByName(
          "name", new CPropertyString(prefix + component->toString("Id")));
      npc_group->addPropertyByName("ai_type",
                                   new CPropertyString("GROUP_NPC")); // AJM
      if (component->isInteger("AutoSpawn") &&
          component->toInteger("AutoSpawn") == 0) {
        npc_group->addPropertyByName("autoSpawn", new CPropertyString("false"));
      } else {
        npc_group->addPropertyByName("autoSpawn",
                                     new CPropertyString("true")); // AJM
      }

      npc_group->addPropertyByName("grp_keywords",
                                   new CPropertyStringArray());        // AJM
      npc_group->addPropertyByName("count", new CPropertyString("0")); // AJM
      npc_group->addPropertyByName("bot_keywords",
                                   new CPropertyStringArray()); // AJM
      npc_group->addPropertyByName("bot_equipment",
                                   new CPropertyStringArray()); // AJM
      npc_group->addPropertyByName("bot_chat_parameters",
                                   new CPropertyStringArray()); // AJM
      npc_group->addPropertyByName("bot_sheet_client",
                                   new CPropertyString("")); // AJM
      npc_group->addPropertyByName("bot_vertical_pos",
                                   new CPropertyString("auto")); // AJM
      ...
      IPrimitive *npc_group_parameters = 0;
      {
        ...
        npc_group_parameters->addPropertyByName(
            "class", new CPropertyString("npc_group_parameters"));
        npc_group_parameters->addPropertyByName(
            "name", new CPropertyString("parameters"));
        npc_group_parameters->addPropertyByName(
            "ai_type", new CPropertyString("GRP_PARAMETERS")); // AJM
      ...
      //			nlinfo("R2Anim: Group %u %s",
      //npc_group_alias->getFullAlias(),
      //std::string(prefix+component->toString("Id")).c_str());
      CRtGrp *rtGrp =
          new CRtGrp(npc_group_alias->getFullAlias(), component, fullName);
      ...
      for (; firstNpc != lastNpc; ++firstNpc) {
        ...
        if (!objectNpc || !objectNpc->isString("Name") ||
        ...
        CPrimPoint *npc_bot =
            dynamic_cast<CPrimPoint *>(CClassRegistry::create("CPrimPoint"));

        CAttributeToProperty a2p(objectNpc, npc_bot);

        a2p.setPrimPoint();
        npc_bot->addPropertyByName("class", new CPropertyString("npc_bot"));
        npc_bot->addPropertyByName("ai_type",
                                   new CPropertyString("BOT_NPC")); // AJM
        npc_bot->addPropertyByName("vertical_pos",
                                   new CPropertyString("auto")); // AJM
        ...
        if (UseSheetClientWithLevel) {
          ...
          if ((sheetClientSize > basicSize &&
               sheet_client.substr(0, basicSize) == basic) &&
              !sheet.empty() &&
              ((sheetClientSize > femaleSize &&
                sheet_client.substr(sheetClientSize - femaleSize) == female) ||
               (sheetClientSize > maleSize &&
                sheet_client.substr(sheetClientSize - maleSize) == male))) {

            std::string::size_type pos = sheet.rfind('_');
            if (pos != std::string::npos) {

              std::string level = sheet.substr(pos);
              if (level.size() == 3 && 'a' <= level[1] && level[1] <= 'f' &&
                  '1' <= level[2] && level[2] <= '4') {
                sheet_client += level;
              }
            }
        ...
        /*
            add an SHEET_CLIENT:SheetClient into the equipment if a alterantiv
           sheet_client is given
        */

        if (sheet.empty()) {

          a2p.setAttributeAsStringArray("Equipment", "equipment");
          npc_bot->addPropertyByName("sheet_client",
                                     new CPropertyString(sheet_client));
        }
          ...
          if (attr && attr->isString()) {
            ...
            NLMISC::splitString(str, "\n", result);
            result.push_back(
                NLMISC::toString("CLIENT_SHEET:%s", sheet_client.c_str()));
            npc_bot->addPropertyByName("equipment",
                                       new CPropertyStringArray(result));
          }

          npc_bot->addPropertyByName("sheet_client",
                                     new CPropertyString(sheet));
        }

        a2p.setAttributeAsBool("IsStuck", "is_stuck");
        npc_group->insertChild(npc_bot);

        CPrimAlias *npc_bot_alias =
            dynamic_cast<CPrimAlias *>(CClassRegistry::create("CPrimAlias"));
        ...
        //				nlinfo("R2Anim: Bot %u %s",
        //npc_bot_alias->getFullAlias(),
        //std::string(prefix+objectNpc->toString("Id")).c_str());
        uint32 dmProperty = 0;
        if (objectNpc->isInteger("DmProperty")) {
          dmProperty = static_cast<uint32>(objectNpc->toInteger("DmProperty"));
        }
        CRtNpc *rtNpc = new CRtNpc(npc_bot_alias->getFullAlias(), objectNpc,
                                   npc_group_alias->getFullAlias(), dmProperty);
  ...
  if (events)
    lastEvent = events->getSize();

  // for each event
  for (; firstEvent != lastEvent; ++firstEvent) {
    // create the primitive event and its associated actions
    IPrimitive *pEvent = getEvent(events->getValueAtPos(firstEvent), components,
                                  prefix, animSession->SessionId);
    if (!pEvent) {
    ...
    if (eventObject->isInteger("IsTriggerZone")) {
      isTriggerZone =
          static_cast<uint32>(eventObject->toInteger("IsTriggerZone")) == 1;
    }
    if (isTriggerZone) {
  ...
  // nodeId
  {
    ...
    for (; firstnode != lastnode; ++firstnode) {
      ...
      if (!node || !node->isTable() || !node->isString("Name") ||
          !node->isString("Grp")) {
...
bool CServerAnimationModule::makeAnimationSession(
    CAnimationSession *animSession, bool /* runTest */) {
  if (!animSession) {
  ...
  nldebug("R2An: animation session %u received, %u in queue",
          animSession->SessionId.asInt(), _QueuedSessions.size());
  return true;
}

bool CServerAnimationModule::doMakeAnimationSession(
  ...
  bool sessionOk =
      _Sessions.insert(std::make_pair(sessionId, animSession)).second;
  if (!sessionOk) {
    nlwarning("R2An: Can't start test, previous session (%u) not closed and "
  ...
  if (!plotItems || !plotItems->isTable()) {
    nlwarning("R2An: Data corrupted:session '%u': missing PlotItems",
              sessionId.asInt());
    return false;
  }
  ...
  for (; firstPlotItem != lastPlotItem; ++firstPlotItem) {
    ...
    if (!plotItem || !plotItem->isTable() || !plotItem->isInteger("SheetId") ||
        !plotItem->isString("Name") || !plotItem->isString("Description") ||
        !plotItem->isString("Comment")) {
      nlwarning("R2An: Data corrupted:session '%u': Invalid PlotItem",
                sessionId.asInt());
      return false;
    }
    ...
    if (!CRingAccess::getInstance().isPlotItemSheetId(plotItemSheetId)) {
      ...
      nlwarning("!!!!!!!!!!!!");
      nlwarning("ERROR: a session %u has faked a Plot item sheetId, or new "
                "plot items have been added. SheetId='%s' sheetIdAsInt=%u",
                sessionId.asInt(), plotItemSheetId.toString().c_str(),
                sheetIdAsInt);
      std::vector<uint32>::const_iterator first(
          animSession->ConnectedChars.begin()),
          last(animSession->ConnectedChars.end());
      nlwarning("There is %u connected Chars:",
                animSession->ConnectedChars.size());
      for (; first != last; ++first) {
    ...
    missionItem.Description =
        ucstring::makeFromUtf8(plotItem->toString("Description"));
  ...
  // LocationId
  {
    ...
    for (; firstLocation != lastLocation; ++firstLocation) {
      ...
      if (!location || !location->isTable() || !location->isInteger("Season") ||
          !location->isString("Island") || !location->isString("EntryPoint")

      ) {
        nlwarning("R2An: Data corrupted:session '%u': Invalid Location",
                  sessionId.asInt());
        return false;
      }
  ...
  for (; firstAct != lastAct; ++firstAct) {
    ...
    bool ok = translateActToPrimitive(components, animSession, act, firstAct,
                                      primDocs[firstAct]); // TODO
    if (!ok) {
      nlwarning("R2An: Data corrupted:session '%u': Failed Act translation",
                sessionId.asInt());
      return false;
    }
  }
  ...
  for (; first != last; ++first) {
    ...
    {
      nldebug("writing xml primitive file %s",
              toString("r2.%04u.act%u.primitive", sessionId.asInt(), first)
                  .c_str());
      saveXmlPrimitiveFile(*primDoc, toString("r2.%04u.act%u.primitive",
                                              sessionId.asInt(),
                                              first)); // save for debug use
    }

    CAiWrapper::getInstance().primsToPdr(primDoc, streamFileName,
                                         animSession->Pdrs[first]);
  ...
  // Update animator session info
  {
    ...
    for (; first != last; ++first) {
      ...
      bool inserted =
          _CharSessions.insert(std::make_pair(*first, sessionId)).second;
      if (!inserted) {
        if (_CharSessions[*first] != sessionId) {
          TSessionId previousCharSessionId = _CharSessions[*first];
          CAnimationSession *previousSession =
              getSession(previousCharSessionId);
          BOMB_IF(!previousSession,
                  "BUG: Failed to get animation session object with id " +
                      NLMISC::toString(previousCharSessionId.asInt()),
                  return false);
          std::vector<uint32> &chars = previousSession->ConnectedChars;
          chars.erase(std::remove(chars.begin(), chars.end(), *first),
                      chars.end());
        }
        _CharSessions[*first] = sessionId;
      }
      nlinfo("R2An: Char %u is connected to session %u as animator / "
             "tester(edition)",
             *first, sessionId.asInt());
    }
  }

  // Not linked session. so we need to tp at first start (in order to avoid to
  // be stuck at the entry point of the scenario if the player has not enought
  // ring point
  if (_Server->getEditionModule()->getLinkedSessionId(sessionId) ==
          TSessionId(0) &&
  ...
  // start the first act 0.5 second after the creation of AINSTANCE to be sure
  // that setPionerRight is done after the creation of the instance
  _Tasks.addTask(new CTaskStartAct(NLMISC::CTime::getLocalTime() + 500, this,
                                   sessionId, animSession->InitialAct, false));
  // startAct(sessionId, animSession->InitialAct);

  return true;
}

bool CServerAnimationModule::getConnectedChars(
    TSessionId sessionId, std::vector<TCharId> &chars) const {
...
bool CServerAnimationModule::stopTestImpl(TSessionId sessionId,
                                          uint32 &lastAct) {
  bool useNetwork = _Server->useNetwork();

  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    nlwarning("R2An: Error can not stop a nonexistent animation %u",
              sessionId.asInt());
    return false;
  }
  uint32 aiInstance = session->AiInstance;
  ...
  if (found == _Sessions.end()) {
    TSessionId remappedSession =
        _Server->getEditionModule()->getLinkedSessionId(sessionId);
...
bool CServerAnimationModule::stopTest(TSessionId sessionId, uint32 &lastAct) {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    nlinfo("R2An: trying to stop a nonexistent animation %u (will try later)",
           sessionId.asInt());
    _Tasks.addTaskAt(NLMISC::CTime::getLocalTime() + 1000,
                     new CTaskStopTest(this, sessionId));
    return false;
  }
...
void CServerAnimationModule::onModuleUpdate() {
  ...
  if (queuedSession) {
    ...
    nldebug("R2An: animation session %u processed, %u in queue",
            animSession->SessionId.asInt(), _QueuedSessions.size());
  }
...
void CServerAnimationModule::onModuleUp(
...
void CServerAnimationModule::onModuleDown(
    NLNET::IModuleProxy *senderModuleProxy) {
  std::string moduleName = senderModuleProxy->getModuleClassName();
  ...
  } else if (moduleName == "ClientEditionModule") // a client has disconnected
  {
    ...
    bool ok =
        getCharInfo(senderModuleProxy, charId, eid, userPriv, extendedPriv);
    if (!ok) {
      return;
    }
...

    {
      ...
      if (session) {
        ...
        std::vector<uint32>::iterator found(
            std::find(connectChars.begin(), connectChars.end(), charId));
        if (found != connectChars.end()) {
          ...
          {
            std::map<uint32, NLMISC::CEntityId>::const_iterator first(
                talkingAsBots.begin()),
                last(talkingAsBots.end());
            for (; first != last; ++first) {
...
bool CServerAnimationModule::getPosition(TSessionId sessionId, double &x,
                                         double &y, double &orient,
                                         uint8 &season, uint32 actIndex) {
  CAnimationSession *animSession = getSession(sessionId);

  if (!animSession) {
    return false;
  } // normal case when getStart position while creating scenario
  ...
  DROP_IF(currentAct >= animSession->Acts.size(), "Error: invalide act",
          return false);

  uint32 locationId = animSession->Acts[currentAct]->LocationId;

  DROP_IF(locationId >= animSession->Locations.size(),
          "Error: invalide location", return false);
  ...
  CScenarioEntryPoints::CCompleteIsland *island =
      epManager.getIslandFromId(location.Island);
  DROP_IF(!island, "No Island.", return false);

  CScenarioEntryPoints::CShortEntryPoint *entryPoint =
      epManager.getEntryPointFromIds(location.Island, location.EntryPoint);
  if (!entryPoint) {
...
void CServerAnimationModule::getStartParams(NLNET::IModuleProxy * /* sender */,
                                            uint32 charId,
                                            TSessionId lastStoredSessionId) {
  _Server->getEditionModule()->getStartParams(charId, lastStoredSessionId);
}

void CServerAnimationModule::askSetUserCharActPosition(
    NLNET::IModuleProxy * /* sender */, uint32 charId) {
  // get entry point
  double x, y, orient;
  uint8 season;
...
CAnimationSession *
CServerAnimationModule::getSession(TSessionId sessionId) const {
...
void CServerAnimationModule::connectAnimationModePlay(
...
CAnimationSession *
CServerAnimationModule::getSessionByCharId(TCharId charId) const {
  if (charId == 0)
    return NULL;
...
void CServerAnimationModule::addPioneer(TSessionId sessionId, TCharId charId) {
  ...
  if (!session)
    return;
  bool isLocal = !_Server->useNetwork();
  std::vector<uint32>::const_iterator found(std::find(
      session->ConnectedChars.begin(), session->ConnectedChars.end(), charId));
  bool added = false;
  ...
  // Warning the player can receive an addPioneer Message before it client
  // Module went down

  {
    bool inserted =
        _CharSessions.insert(std::make_pair(charId, sessionId)).second;
    if (!inserted) {
      // lookup the previous session id and make sure that we have a session for
      // this id
      ...
      if (previousCharSessionId != sessionId) {
        ...
        if (previousSession != session) {
          std::vector<uint32> &chars = previousSession->ConnectedChars;
          chars.erase(std::remove(chars.begin(), chars.end(), charId),
                      chars.end());
        }
      }
  ...
  const TModuleProxyPtr *clientProxyPtr =
      getEditionModule()->getClientProxyPtr(charId);
  if (!clientProxyPtr) {
    nlwarning("A pioneer has just entered in a session %u but has no proxy %u",
              sessionId.asInt(), charId);
    return;
  }
  ...
  if (session->CharacternValidePosition.find(charId) ==
      session->CharacternValidePosition.end()) {
    session->CharacternValidePosition.insert(charId);
  }

  if (added) {
    uint32 actId = session->CurrentAct;
    if (session->Acts.size() > actId &&
        !session->Acts[actId]->ActDescription.empty()) {
      CShareClientEditionItfProxy proxy(*clientProxyPtr);
...
void CServerAnimationModule::onDssTarget(
    IModuleProxy *senderModuleProxy, const std::vector<std::string> &params) {
  uint32 charId;
  NLMISC::CEntityId eid;
  std::string userPriv;
  std::string extendedPriv;
  ...
  bool ok =
      checkSecurityInfo(senderModuleProxy, charId, eid, userPriv, extendedPriv);
  if (!ok) {
    return;
  }

  DROP_IF(!_CharacterControlProxy, "Try to send message to EGS must he is down",
          return);
...
**/
void CServerAnimationModule::stopTalk(
    const NLMISC::CEntityId &eid, const NLMISC::CEntityId & /* creatureId */,
    TDataSetRow entityRowId) {
  TCharId charId = static_cast<TCharId>(eid.getShortId());
  const NLNET::TModuleProxyPtr *foundModule =
      getEditionModule()->getClientProxyPtr(charId);
  BOMB_IF(
      foundModule == NULL,
      "stopTalk failed because getClientProxyPtr() returned NULL for entity " +
          eid.toString(),
      return);
...
void CServerAnimationModule::stopIncarn(const NLMISC::CEntityId &eid,
                                        const NLMISC::CEntityId &creatureId) {
  CAiWrapper::getInstance().stopControlNpc(eid, creatureId);
}

CRtNpc *CServerAnimationModule::getNpcByAlias(TSessionId sessionId,
                                              TAIAlias alias) const {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return 0;
  }
  ...
  uint32 currentAct = session->CurrentAct;
  if (currentAct != 0) {
    CRtAct::TRtNpcs::const_iterator found(
        session->Acts[currentAct]->RtNpcs.find(alias));
...
void CServerAnimationModule::stopControlNpcs(TCharId charId) {
  ...
  bool ok = checkSecurityInfo(clientProxyPtr, charId, clientEid, userPriv,
                              extendedPriv);
  if (!ok) {
    return;
  }
  ...
  // Remove Incarn
  {
    ...
    {
      std::map<uint32, NLMISC::CEntityId>::const_iterator botEid(
          entities.begin()),
          last(entities.end());
...

    {
      ...
      for (; botEid != last; ++botEid) {
        ...
        if (isTalkingAs(botEid->second, clientEid)) {
          TOwnedEntities::iterator iter =
              _TalkedAsEntities.find(botEid->second);
...
void CServerAnimationModule::askUpdateScenarioHeader(
    NLNET::IModuleProxy *clientProxyPtr) {
...
void CServerAnimationModule::askIncarnatingListUpdate(
    NLNET::IModuleProxy *clientProxyPtr) {
  ...
  std::map<uint32, NLMISC::CEntityId> entities;
  session->IncarningBots.getBots(charId, entities);
  std::map<uint32, NLMISC::CEntityId>::const_iterator first(entities.begin()),
      last(entities.end());
...
void CServerAnimationModule::askTalkingAsListUpdate(
    NLNET::IModuleProxy *clientProxyPtr) {
  uint32 charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
...
void CServerAnimationModule::updateIncarningList(TSessionId /* sessionId */,
                                                 uint32 charId) {
  const NLNET::TModuleProxyPtr *foundModule =
      getEditionModule()->getClientProxyPtr(charId);
  if (foundModule) {
...
void CServerAnimationModule::updateTalkingAsList(TSessionId /* sessionId */,
                                                 uint32 charId) {
...
bool CServerAnimationModule::setIncarningPlayer(
    TSessionId sessionId, const NLMISC::CEntityId &creatureId,
    const NLMISC::CEntityId &eid, TDataSetRow entityRowId, TAIAlias alias) {
...
bool CServerAnimationModule::setTalkingAsPlayer(
    TSessionId sessionId, const NLMISC::CEntityId &creatureId,
    const NLMISC::CEntityId &eid, TDataSetRow entityRowId, TAIAlias alias) {
  bool ok = true;
  CAnimationSession *session = getSession(sessionId);
...
bool CServerAnimationModule::isIncarnedByPlayer(
    const NLMISC::CEntityId &creatureId, const NLMISC::CEntityId &eid) const {
  TOwnedEntities::const_iterator iter = _IncarnedEntities.find(creatureId);
  if (iter == _IncarnedEntities.end()) {
    return false;
  }
  COwnedCreatureInfo info = (*iter).second;
  std::vector<NLMISC::CEntityId>::const_iterator eidIt(info.PlayerIds.begin()),
      last(info.PlayerIds.end());
...
bool CServerAnimationModule::isTalkingAs(const NLMISC::CEntityId &creatureId,
                                         const NLMISC::CEntityId &eid) const {
...
void CServerAnimationModule::removeTalkingAsPlayer(
    TSessionId sessionId, const NLMISC::CEntityId &creatureId,
    const NLMISC::CEntityId &eid) {
  TOwnedEntities::iterator iter = _TalkedAsEntities.find(creatureId);
  COwnedCreatureInfo info = (*iter).second;
  std::vector<NLMISC::CEntityId>::iterator eidIt(info.PlayerIds.begin()),
      last(info.PlayerIds.end());
  for (; eidIt != last; ++eidIt) {
    if ((*eidIt) == eid) {
...
void CServerAnimationModule::removeIncarningPlayer(
    TSessionId sessionId, const NLMISC::CEntityId &creatureId,
    const NLMISC::CEntityId &eid) {
  TOwnedEntities::iterator iter = _IncarnedEntities.find(creatureId);
  COwnedCreatureInfo info = (*iter).second;
...
void CServerAnimationModule::updateAnimationProperties(
    NLNET::IModuleProxy *senderModuleProxy, const NLMISC::CEntityId &eid,
    CRtNpc *rtNpc, CRtGrp *rtGrp) {
...
void CServerAnimationModule::onCharTargetReceived(
    NLNET::IModuleProxy *senderModuleProxy, const NLMISC::CEntityId &eid,
    const NLMISC::CEntityId &creatureId, TAIAlias alias,
    TDataSetRow entityRowId, const ucstring & /* ucName */, uint32 nameId,
    const std::vector<std::string> &param, bool alived) {
  ...
  if (!alived || entityRowId == TDataSetRow() || alias == 0 ||
      creatureId == CEntityId::Unknown) {
    updateAnimationProperties(*foundModule, CEntityId::Unknown, 0, 0);
    return;
  }

  TCharSessions::const_iterator charSessionFound(_CharSessions.find(charId));
  DROP_IF(charSessionFound == _CharSessions.end(),
          NLMISC::toString("Invalid Session for Char %u", charId), return);

  TSessionId sessionId = charSessionFound->second;

  CAnimationSession *session = getSession(sessionId);
  DROP_IF(session->Acts.empty(),
          NLMISC::toString("Invalid Session %u for Char %u (No act available)",
                           charSessionFound->second.asInt(), charId),
          return);
  ...
  CAnimationSession::TCharacterInfos::iterator charInfos =
      session->CharacterInfos.find(charId);
  ...
  // Add to targeted entities if necessary
  {
    ...
    std::pair<TOwnedEntities::iterator, bool> ret =
        _TargetedEntities.insert(std::make_pair(creatureId, tmp));
    ...
    std::vector<NLMISC::CEntityId>::iterator it =
        std::find(container.begin(), container.end(), eid);
    if (it == container.end()) {
      container.push_back(eid);
    }
    if (firstTime) {
  ...
  if (oldTargetId != NLMISC::CEntityId::Unknown && oldTargetId != creatureId) {
    ...
    if (charInfos != session->CharacterInfos.end()) {
      ...
      TOwnedEntities::iterator oldTargetIt =
          _TargetedEntities.find(oldTargetId);
      if (oldTargetIt != _TargetedEntities.end()) {
        std::vector<NLMISC::CEntityId> &container =
            oldTargetIt->second.PlayerIds;
        std::vector<NLMISC::CEntityId>::iterator it =
            std::find(container.begin(), container.end(), eid);
        if (it != container.end()) {
          container.erase(it);
  ...
  CRtAct::TRtNpcs::const_iterator npcFound(
      session->Acts[0]->RtNpcs.find(alias));
  CRtNpc *rtNpc = 0;
  std::string name;
  ...
  if ((animationProp & CAnimationProp::Alive)) {
    ...
    if (args[0] == "ADD_HP") {
      CAiWrapper::getInstance().setHPLevel(rtNpc->EntityId, alias, 1);
      return;
    }

    if (args[0] == "KILL_NPC" && alived) {
      CAiWrapper::getInstance().setHPLevel(rtNpc->EntityId, alias, 0);
      return;
    }
    if (args[0] == "ADD_HP") {
      CAiWrapper::getInstance().setHPLevel(rtNpc->EntityId, alias, 1);
    ...
    if (args[0] == "GRP_KILL" && alived) {
      CAiWrapper::getInstance().setGrpHPLevel(rtNpc->EntityId, alias, 0);
      return;
    }
    if (args[0] == "GRP_HEAL") {
      CAiWrapper::getInstance().setGrpHPLevel(rtNpc->EntityId, alias, 1);
      return;
    }

    if (args[0] == "AGGRO_RANGE_BIG" || args[0] == "AGGRO_RANGE_NORMAL" ||
        args[0] == "AGGRO_RANGE_SMALL" || args[0] == "AGGRO_RANGE_NONE") {
      if (args[0] == "AGGRO_RANGE_BIG") {
        CAiWrapper::getInstance().setAggroRange(rtNpc->EntityId, 100);
        return;
      }
      if (args[0] == "AGGRO_RANGE_NORMAL") {
        CAiWrapper::getInstance().setAggroRange(rtNpc->EntityId, 30);
        return;
      }
      if (args[0] == "AGGRO_RANGE_SMALL") {
        CAiWrapper::getInstance().setAggroRange(rtNpc->EntityId, 15);
        return;
      }
      if (args[0] == "AGGRO_RANGE_NONE") {
        CAiWrapper::getInstance().setAggroRange(rtNpc->EntityId, 0);
        return;
      }
  ...
  if ((animationProp & CAnimationProp::Controlable)) {
    if (args[0] == "CONTROL" && alived) {
      if (!isIncarnedByPlayer(creatureId, eid)) {
        ...
        if (session->IncarningBots.getBotsCount(charId) ==
            0) // We can now only incarnate One Npc
        {
          if (setIncarningPlayer(sessionId, creatureId, eid, entityRowId,
                                 alias)) {
            ...
            {
                CCharacterControlItfProxy proxy(_CharacterControlProxy);
                proxy.teleportCharacterToNpc(this, charId, creatureId,
            session->CurrSeason );
            }
            */
            CAiWrapper::getInstance().askBotDespawnNotification(creatureId,
                                                                alias);
  ...
  if ((animationProp & CAnimationProp::Speaking)) {
    if (args[0] == "TALK_AS" && alived) {
      ...
      if (session->TalkingAsBots.getBotsCount(charId) < 8) {
        if (setTalkingAsPlayer(sessionId, creatureId, eid, entityRowId,
                               alias)) {
...
void CServerAnimationModule::characterReady(NLNET::IModuleProxy * /* sender */,
                                            const NLMISC::CEntityId &charEid) {
...
bool CServerAnimationModule::onProcessModuleMessage(
    IModuleProxy *senderModuleProxy, const CMessage &msgin) {
  std::string operationName = msgin.getName();

  //	if (CServerAnimationItfSkel::onDispatchMessage(senderModuleProxy,
  //msgin))
  //	{
  //		return;
  //	}
  //
  //	// From Client
  //	if (CShareServerAnimationItfSkel::onDispatchMessage(senderModuleProxy,
  //msgin))
  //	{
  //		return;
  //	}
  ...
  if (!senderModuleProxy) {
    ...
    if (operationName == "requestStartAct") {
      ...
      if (charFound == _CharSessions.end()) {
        ...
        if (!_QueuedSessions.empty()) {
          ...
          do {
            ...
            if (!bFoundSession) {
              ...
              std::vector<uint32>::const_iterator
                  it = animSession->ConnectedChars.begin(),
          ...
          if (bFoundSession) {
            nlinfo("R2An: startAct received from char %u, anim session is in "
                   "queue.",
                   charId);
            return true;
          }
    ...
    if (operationName == "requestSetWeather") {
      ...
      TCharSessions::const_iterator charFound(_CharSessions.find(charId));
      if (charFound == _CharSessions.end()) {
        nlwarning("R2An: not connected char(%d) try to set the weather.",
                  charId);
        return true;
      }
...
void CServerAnimationModule::disconnectChar(TCharId charId) {
  const NLNET::TModuleProxyPtr *client =
      getEditionModule()->getClientProxyPtr(charId);
  if (client) {
    onModuleDown(*client);
  }
...
void CServerAnimationModule::scheduleStartSession(
    const CAnimationMessageAnimationStart &msg) {
  _Tasks.addTaskAt(NLMISC::CTime::getLocalTime() + 1000,
                   new CTaskScheduleStartSession(this, msg));
}

void CServerAnimationModule::scheduleStartSessionImpl(
    const CAnimationMessageAnimationStart &msg) {
  // create new Session
  ...
  std::vector<uint32>::const_iterator first(msg.AnimatorCharId.begin()),
      last(msg.AnimatorCharId.end());
  for (; first != last; ++first) {
  ...
  // Update animator session info
  {
    ...
    for (; first != last; ++first) {
      ...
      bool inserted =
          _CharSessions.insert(std::make_pair(*first, msg.SessionId)).second;
      if (!inserted) {
        // lookup the previous sessin id and make sure that we have a session
        // for thsi id
        TSessionId previousCharSessionId = _CharSessions[*first];
        nlinfo("R2An::scheduleStartSession Moving char (%u) from Session %u to "
               "Session %u",
               *first, previousCharSessionId.asInt(), msg.SessionId.asInt());
        BOMB_IF(_Sessions.find(previousCharSessionId) == _Sessions.end(),
                "scheduleStartSession giving up because failed to find "
                "_Sessions entry for character",
                return);
        // disconnect from previous scenario
        if (previousCharSessionId != msg.SessionId) {
          CAnimationSession *previousSession =
              getSession(previousCharSessionId);
          BOMB_IF(!previousSession,
                  "BUG: Failed to get pointer to session object with Id: " +
                      NLMISC::toString(previousCharSessionId.asInt()),
                  return);
          std::vector<uint32> &chars = previousSession->ConnectedChars;
          chars.erase(std::remove(chars.begin(), chars.end(), *first),
...
IPrimitive *CServerAnimationModule::getEvent(CObject *event,
                                             CInstanceMap &components,
                                             const std::string &prefix,
                                             TSessionId scenarioId) {
  ...
  pEvent->addPropertyByName("state_keyword_filter",
                            new CPropertyStringArray()); // AJM
  pEvent->addPropertyByName("group_keyword_filter",
                            new CPropertyStringArray()); // AJM
  ...
  a2pEvent.setAttributeAsStringArrayWithPrefix(
      "StatesByName", "states_by_name", prefix); // TODO more verification
  a2pEvent.setAttributeAsStringArrayWithPrefix("GroupsByName", "groups_by_name",
                                               prefix); // TODO???

  CPrimAlias *event_alias =
      dynamic_cast<CPrimAlias *>(CClassRegistry::create("CPrimAlias"));
  ...
  if (actions_id)
    lastAction = actions_id->getSize();
  IPrimitive *father;
  if (lastAction > 1) {
    father = dynamic_cast<IPrimitive *>(CClassRegistry::create("CPrimNode"));
    father->addPropertyByName("class",
                              new CPropertyString("npc_event_handler_action"));
    father->addPropertyByName("ai_type",
                              new CPropertyString("NPC_EVENT_ACTION")); // AJM
    ...
    father->addPropertyByName("name",
                              new CPropertyString("multi_actions")); // AJM
  ...
  for (; firstAction != lastAction; ++firstAction) {
    ...
    if (!action) {
      nlwarning(
          "Error while generating primitives in session '%u' in action '%s'",
          scenarioId.asInt(), id.c_str());
      return 0;
    }
    // create the primitive action
    IPrimitive *pAction = getAction(action, prefix, scenarioId);
    if (!pAction) {
      nlwarning("Error for '%u'th action '%s' in states '%s' with group '%s'",
                firstAction, event->toString("Name").c_str(),
                event->toString("StatesByName").c_str(),
                event->toString("GroupsByName").c_str());
      return 0;
    }
...
void CServerAnimationModule::requestLoadTable(CAnimationSession *session) {
...

  {
    ...
    for (uint32 i = 0; i < size; ++i) {
      const NLNET::TModuleProxyPtr *ptr =
          getEditionModule()->getClientProxyPtr(session->ConnectedChars[i]);
      if (ptr) {
        connected.push_back(ptr);
      }
    }
  ...
  if ((texts == NULL) || (texts->getAttr("Texts")->getSize() == 0)) {
    ...
    uint32 size;
    if (textsTable && textsTable->isTable() &&
        (size = textsTable->getSize()) != 0) {
...
void CServerAnimationModule::onServiceUp(const std::string &serviceName,
...
void CServerAnimationModule::onServiceDown(const std::string &serviceName,
                                           TServiceId /* serviceId */) {
...
void CServerAnimationModule::askMissionItemsDescription(
    NLNET::IModuleProxy *senderModuleProxy) {
  uint32 charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  ...
  if (session) {
    ...
    clientproxy.updateMissionItemsDescription(this, sessionId,
                                              session->MissionItems);
  }
}

void CServerAnimationModule::deactivateEasterEggsFromAct(TSessionId scenarioId,
                                                         uint32 actId) {
  ...
  CRtAct::TActiveEasterEggs::const_iterator first(
      rtAct->ActiveEasterEggs.begin()),
      last(rtAct->ActiveEasterEggs.end());

  for (; first != last; ++first) {
    easterEggs.insert(first->first);
    CAnimationSession::TActiveEasterEggs::iterator toErase(
        session->ActiveEasterEggs.find(first->first));
    DROP_IF(toErase == session->ActiveEasterEggs.end(),
            "Error in activateEasterEgg ", return);
...
void CServerAnimationModule::deactivateEasterEgg(
    class NLNET::IModuleProxy * /* aisControl */, uint32 easterEggId,
    TSessionId scenarioId, uint32 actId) {
  DROP_IF(_CharacterControlProxy.isNull(), "No CharacterControlProxy", return);
  ...
  CAnimationSession::TActiveEasterEggs::iterator found(
      session->ActiveEasterEggs.find(easterEggId));
...
void CServerAnimationModule::activateEasterEgg(
    class NLNET::IModuleProxy * /* aisControl */, uint32 easterEggId,
    TSessionId scenarioId, uint32 actId, const std::string &items, float x,
    float y, float z, float heading, const std::string &grpControler,
    const std::string &name, const std::string &look) {
  DROP_IF(_CharacterControlProxy.isNull(), "No CharacterControlProxy", return);
  ...
  for (; first != last; ++first) {
    ...
    DROP_IF(item >= session->MissionItems.size(), "Error  activateEasterEgg",
            return);
  ...
  CCharacterControlItfProxy proxy(_CharacterControlProxy);
  proxy.activateEasterEgg(this, easterEggId, scenarioId, session->AiInstance,
                          itemsAndQuantities, pos, name, look);
}

void CServerAnimationModule::onEasterEggLooted(
    class NLNET::IModuleProxy * /* egs */, uint32 easterEggId,
    TSessionId scenarioId) {
  CAnimationSession *session = getSession(scenarioId);
  DROP_IF(!session, toString("No Session %d", scenarioId.asInt()), return);
...
void CServerAnimationModule::onUserTriggerTriggered(
    NLNET::IModuleProxy *senderModuleProxy, uint32 actId, uint32 triggerId) {
  uint32 charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
  std::string extendedPriv;
...
void CServerAnimationModule::triggerUserTrigger(TSessionId sessionId,
                                                uint32 actId,
                                                uint32 triggerId) {
  CAnimationSession *animationSession = getSession(sessionId);

  if (animationSession && actId < animationSession->Acts.size() &&
      triggerId < animationSession->Acts[actId]->UserTriggers.size()) {
    std::string groupName =
        animationSession->Acts[actId]->UserTriggers[triggerId].FullName;
    CAiWrapper::getInstance().triggerUserTrigger(groupName, 1);
  } else {
    nlwarning("error in CServerAnimationModule::triggerUserTrigger(%d, %d, %d)",
              sessionId.asInt(), actId, triggerId);
  }
}

void CServerAnimationModule::askActPositionDescriptions(
    NLNET::IModuleProxy *senderModuleProxy) {
  uint32 charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
...
void CServerAnimationModule::askUserTriggerDescriptions(
    NLNET::IModuleProxy *senderModuleProxy) {
  uint32 charId;
  NLMISC::CEntityId clientEid;
  std::string userPriv;
...
void CServerAnimationModule::onBotDeathNotification(
    NLMISC::CEntityId &creatureId) {
  onBotDespawnNotification(creatureId);
}

void CServerAnimationModule::onBotDespawnNotification(
    NLMISC::CEntityId &creatureId) {
  ...
  if (itControl == _IncarnedEntities.end() &&
      itTalk == _TalkedAsEntities.end() &&
      itTarget == _TargetedEntities.end()) {
    return;
  }
  ...
  if (itTarget != _TargetedEntities.end()) {
    ...
    std::vector<NLMISC::CEntityId>::iterator first(playerId.begin()),
        last(playerId.end());
    for (; first != last; ++first) {
      CEntityId eid = *first;
      uint32 charId = static_cast<uint32>(first->getShortId());
      const NLNET::TModuleProxyPtr *pClient =
          getEditionModule()->getClientProxyPtr(charId);
      if (pClient) {
...
void CServerAnimationModule::onStopNpcControlNotification(
    NLMISC::CEntityId &creatureId) {
  TOwnedEntities::iterator itControl = _IncarnedEntities.find(creatureId);
...
NLMISC_CLASS_COMMAND_IMPL(CServerAnimationModule, displayUserTriggers) {
  ...
  if (animationSession) {
    ...
    for (; first != last; ++first) {
      log.displayNL("Trigger %d: Name='%s' act='%d' id='%d'", first,
...
NLMISC_CLASS_COMMAND_IMPL(CServerAnimationModule, triggerUserTrigger) {
  if (args.size() != 3) {
    log.displayNL(
        "ServerAnimationModule.triggerUserTrigger sessionId actId triggerId");
    return false;
  }
...
NLNET::IModule *CServerAnimationModule::getModule() const {
  return const_cast<R2::CServerAnimationModule *>(this);
}
...
void CServerAnimationModule::dssMessage(NLNET::IModuleProxy * /* ais */,
                                        TSessionId sessionId,
  ...
  if (session) {
    ...
    CShareClientEditionItfProxy::buildMessageFor_systemMsg(
        message, msgType, who, translatedMessage);
...
void CServerAnimationModule::setSessionStartParams(TSessionId sessionId,
                                                   sint32 x, sint32 y,
                                                   uint8 season) {
...
bool CServerAnimationModule::mustReloadPosition(TSessionId sessionId,
                                                TCharId charId) const {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return false;
  }
  bool ok = session->CharacternValidePosition.find(charId) !=
            session->CharacternValidePosition.end();
  return ok;
}

bool CServerAnimationModule::getHeaderInfo(
    TSessionId sessionId, TScenarioHeaderSerializer::TValueType &values) const {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return false;
  }
...
void CServerAnimationModule::teleportCharacter(NLNET::IModuleProxy * /* ais */,
                                               const NLMISC::CEntityId &eid,
...
void CServerAnimationModule::broadcastMsg(TSessionId sessionId,
                                          const NLNET::CMessage &msg) {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return;
...
void CServerAnimationModule::setScenarioPoints(NLNET::IModuleProxy * /* ais */,
                                               TSessionId sessionId,
...
void CServerAnimationModule::startScenarioTiming(
    NLNET::IModuleProxy * /* ais */, TSessionId sessionId) {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return;
  }
...
void CServerAnimationModule::endScenarioTiming(NLNET::IModuleProxy * /* ais */,
                                               TSessionId sessionId) {
  CAnimationSession *session = getSession(sessionId);
  if (!session) {
    return;
  ...
  if (!session->TimingIsFinished) {
    ...
    session->ScenarioTime =
        NLMISC::CTime::getLocalTime() - session->ScenarioTime;
...
bool CServerAnimationModule::getScore(TSessionId sessionId, uint32 &score,

...
void normaliseBnpFileName(std::string &fileName) {
  ...
  if (NLMISC::CFile::getExtension(fileName).empty() &&
...
bool CBNPFileVersion::setup(const std::string &fileName, uint32 versionNumber) {
  ...
  BOMB_IF(!NLMISC::CFile::fileExists(fileName),
          ("File not found: " + fileName).c_str(), return false);
...
// accessors
uint32 CBNPFileVersion::getVersionNumber() const { return _VersionNumber; }

uint32 CBNPFileVersion::getTimeStamp() const { return _FileTime; }

uint32 CBNPFileVersion::getFileSize() const { return _FileSize; }

uint32 CBNPFileVersion::get7ZFileSize() const { return _7ZFileSize; }

uint32 CBNPFileVersion::getPatchSize() const { return _PatchSize; }
...
CBNPFile::CBNPFile() { _IsIncremental = true; }

bool CBNPFile::addVersion(const std::string &bnpDirectory,
  ...
  // perform a quick check to see if the time stamp and file size of the new BNP
  // file match the last version in the index
  std::string fullFileName = bnpDirectory + _FileName;
  if (!NLMISC::CFile::fileExists(fullFileName))
    return false;
  if (!_Versions.empty()) {
    if ((NLMISC::CFile::getFileSize(fullFileName) ==
    ...
    NLMISC::InfoLog->displayNL(
        "File: %s\n size(%d != %d) || time(%d != %d)", fullFileName.c_str(),
  ...
  if (_Versions.empty() || _Versions.back() != fileVersion) {
    // if we haven't yet generated the version number for this version then go
    // for it now
    version.grabVersionNumber();
    fileVersion.setVersionNumber(version.getPackageVersionNumber());

    // make sure that our version numbers are ever increasing... it would be
    // fatal to have an out-of-order version
    if (!_Versions.empty())
      nlassert(_Versions.back().getVersionNumber() <
               version.getPackageVersionNumber());

    // the file's current checksum doesn't match the previous checksum so add
    // the new version
    nlinfo("- Adding version %05u to file: %s",
           version.getPackageVersionNumber(), _FileName.c_str());
    _Versions.push_back(fileVersion);

    // copy the file to create a new reference file...
    //		NLMISC::CSString refFileName=
    //NLMISC::CSString(refDirectory+_FileName).replace(".",NLMISC::toString("_%05u.",version.getPackageVersionNumber()).c_str());
    //		NLMISC::CFile::copyFile(refFileName, fullFileName);
  } else {
    // the file's size & current checksum match the previous version so just fix
    // the file's timestamp
    nlinfo("Files contents matches previous version but time stamp is "
           "different: %s",
           fullFileName.c_str());
...
uint32 CBNPFile::versionCount() const { return (uint32)_Versions.size(); }
...
const std::string &CBNPFile::getFileName() const { return _FileName; }
...
bool CBNPFile::isIncremental() { return _IsIncremental; }
...
uint32 CBNPFileSet::addVersion(const std::string &bnpDirectory,
                               const std::string &refDirectory,
                               IVersionNumberGenerator &version) {
...
void CBNPFileSet::clear() { _Files.clear(); }

uint32 CBNPFileSet::fileCount() const { return (uint32)_Files.size(); }
...
#define PERSISTENT_DATA STRUCT_VECT(_Files)
// #      pragma message( PERSISTENT_GENERATION_MESSAGE )
#include "persistent_data_template.h"
...
const std::string &CBNPCategory::getName() const { return _Name; }

void CBNPCategory::setName(const std::string &name) { _Name = name; }

void CBNPCategory::setOptional(bool value) { _IsOptional = value; }

bool CBNPCategory::isOptional() const { return _IsOptional; }

void CBNPCategory::setUnpackTo(const std::string &n) { _UnpackTo = n; }

const std::string &CBNPCategory::getUnpackTo() const { return _UnpackTo; }

void CBNPCategory::setIncremental(bool value) { _IsIncremental = value; }

bool CBNPCategory::isIncremental() const { return _IsIncremental; }

void CBNPCategory::setCatRequired(const std::string &cat) {
  _CatRequired = cat;
}

const std::string &CBNPCategory::getCatRequired() const { return _CatRequired; }

void CBNPCategory::setHidden(bool value) { _Hidden = value; }

bool CBNPCategory::isHidden() const { return _Hidden; }

uint32 CBNPCategory::fileCount() const { return (uint32)_Files.size(); }
...
void CBNPCategorySet::clear() { _Category.clear(); }

const CBNPCategory *
CBNPCategorySet::getCategoryFromFile(const std::string &fileName) const {
  for (std::vector<CBNPCategory>::const_iterator it = _Category.begin();
       it != _Category.end(); ++it) {
...
const CBNPCategory *
CBNPCategorySet::getCategory(const std::string &categoryName) const {
...
CBNPCategory *CBNPCategorySet::getCategory(const std::string &categoryName,
                                           bool addIfNotExist) {
  // look for a category with matching name
  for (uint32 i = 0; i < categoryCount(); ++i)
    if (getCategory(i).getName() == categoryName)
...
void CBNPCategorySet::addFile(const std::string &categoryName,
  ...
  nlinfo("- File added to category %s::%s", categoryName.c_str(),
         fileName.c_str());
}
...
bool CBNPCategorySet::isFileIncremental(const std::string &fileName) const {
  ...
  for (uint32 i = 0; i < categoryCount(); ++i) {
    ...
    // if the file exists in this category then return 'false' meaning
    // non-incremental
    for (uint32 i = 0; i < theCategory.fileCount(); ++i)
      if (theCategory.getFile(i) == fileName)
        return false;
  ...
  // the file wasn't found in a non-incremental category so it must be
  // incremental
  return true;
}
...
#define PERSISTENT_DATA STRUCT_VECT(_Category)
// #      pragma message( PERSISTENT_GENERATION_MESSAGE )
#include "persistent_data_template.h"
...
//{
//	return
//NLMISC::CFile::getFilenameWithoutExtension(getTargetFileName())+NLMISC::toString("_%05d.patch",getVersion());
//}
//
//
////-----------------------------------------------------------------------------
...
//// initialisation
//
// CBNPUnpatcher::CBNPUnpatcher(const std::string& productName,uint32
// version,const std::string& appRootDirectory,const std::string&
// patchDirectory)
//{
//	_AppRootDirectory=
//NLMISC::CPath::standardizePath(appRootDirectory,true); 	_PatchDirectory=
//NLMISC::CPath::standardizePath(patchDirectory,true); 	_ProductName=
//productName; 	_Version=			version;
//}
//
...
//{
//	return
//CBNPPatchDescription(productName+"_patch_index",_Version,0,!isIndexUpToDate())
//}
//
//
////-----------------------------------------------------------------------------
...
//}
//
// bool CBNPUnpatcher::_isPatch(bool isBySelectionFlag,bool
...
//		}
//
//		// if one of the files is not up to date then we return 'true'
//as we need to patch 		for (uint32 j=0;j<theCategory->fileCount();++j) 			if
//(!isFileUpToDate(theCategory->getFile(j))) 				return true;
//	}
//
//	return false;
//}
...
// }
//
//// return true if it's necessary to download the next patch that needs to be
///applied (in order)
...
//{
//	// note that if the index isn't up to date then it is classed as the
//next patch 	nlassert(!isNextPatchDownloadRequired()); 	xxx
// }
//
//
////-----------------------------------------------------------------------------
...
// void CBNPUnpatcher::getAllOptionalCategories(std::vector<std::string>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//
...
// void CBNPUnpatcher::getPatchableOptionalCategories(std::vector<std::string>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//
...
//
//		// if one of the files is not up to date then we return 'true'
//as we need to patch 		for (j=0;j<theCategory->fileCount();++j) 			if
//(!isFileUpToDate(theCategory->getFile(j))) 				break;
//
//		// if we broke out before the end of the for loop then we need
//to add this category 		if (j<theCategory->fileCount())
//			result.push_back(_Categories.getCategory(i).getName());
//	}
// }
//
//// select or unselect an optional package
// void CBNPUnpatcher::setOptionalCategorySelectFlag(const std::string&
// categoryName, bool value)
//{
//	nlassert(isIndexUpToDate()==true);
...
///applied to update selected packages
// void CBNPUnpatcher::getSelectedPatches(std::vector<CBNPPatchDescription>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//
...
// }
//
//// get the ordered list of optional patches that need to be applied to update
///selected packages
// void
// CBNPUnpatcher::getSelectedOptionalPatches(std::vector<CBNPPatchDescription>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//	xxx
...
///update
// void CBNPUnpatcher::getMandatoryPatches(std::vector<CBNPPatchDescription>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//	xxx
...
//	return
//patch.getTargetFileName()+NLMISC::toString(":%d",patch.getVersion());
// }
//
...
//
//	// treat the case of !uptodate() here and get the index file as the next
//patch 	if (!isIndexUpToDate())
//	{
...
// void
// CBNPUnpatcher::getSelectedDownloadPatches(std::vector<CBNPPatchDescription>&
// result)
//{
//	nlassert(isIndexUpToDate()==true);
//	xxx
...
//
//// get the list of patches that need to be downloaded
// void CBNPUnpatcher::getAllDownloadPatches(std::vector<CBNPPatchDescription>&
// result)
//{
//	// store the selected category set in temporary variable
//	std::set<std::string> selectedCategories= _SelectedCategories;
//
//	// select all of the categories and delegate to
//getSelectedDownloadPatches() 	setAllOptionalCategorySelectFlags();

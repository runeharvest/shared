...
TPVPClan getClanFromIndex(uint32 theIndex) {
  ...
  // The first two clans, "None" and "Neutral", don't count.  Subtract 2 from
  // the lookup.
  std::string FactionNames[] = {"kami",  "karavan", "fyros",
                                "matis", "tryker",  "zorai"};

  for (int looper = BeginClans; looper <= EndClans; looper += 1) {
    if (CStaticFames::getInstance().getFactionIndex(
            FactionNames[looper - BeginClans]) == theIndex) {

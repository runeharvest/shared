...
/****************************************************************\
                        buildTableFormat()
\****************************************************************/
void CSTLoader::buildTableFormat(const string &fileName,
                                 list<pair<string, TDataType>> &tableFormat) {
...
\****************************************************************/
void CSTLoader::generateDerivedClasses(
    const std::list<std::pair<std::string, TDataType>> &format,
    const std::list<std::list<std::string>> &data) {
  std::string content;

  std::list<std::list<std::string>>::const_iterator it_dl = data.begin();

  while (it_dl != data.end()) {
    std::list<std::pair<std::string, TDataType>>::const_iterator it_def =
        format.begin();
    ...
    std::list<std::pair<std::string, TDataType>>::const_iterator it_obj =
        format.begin();
    it_obj++;
    ...
    while (it_def != format.end() && it_val != (*it_dl).end()) {
      ...
      case BOOL:
        ...
        break;
      default:
        content += "ERROR: unsupported type " +
                   toString((uint)(*it_def).second) + "\n";
        break;
      }
      content += ");\n";
...
/****************************************************************\
                            init()
\****************************************************************/
void CSTLoader::init(const string &fileName,
                     const map<string, TDataType> &fileFormat) {
      ...
      if (itf != _FileFormat.end()) {
        ...
        case SINT32: {
          ...
          if (tmp == 0) {
            if (token[0] != '0') {
              nlerror(
                  "Data type error at column %s, at line %d : not an integer",
                  columnLabel.c_str(), _LineCount);
            }
          }
        } break;
        case FLOAT: {
          ...
          if (tmp == 0.0) {
            if (token[0] != '0') {
              nlerror("Data type error at column %s, at line %d : not a float",
                      columnLabel.c_str(), _LineCount);
            }
          }
        } break;
        case BOOL:
          if (strcmp(token, "0") && strcmp(token, "1")) {
            nlerror("Data type error at column %s, at line %d : not a bool",
                    columnLabel.c_str(), _LineCount);
          }
          break;
        default:
        ...
        _Tokens.erase(
            columnLabel); // do not forget to delete the old value (insert does
                          // not assure proper remplacement)

        _Tokens.insert(make_pair(columnLabel, string(token)));
      }
      // if this column is not expected by the user we go on to next token
      else {
        nlinfo("%s : this colum is not in the file format",
               columnLabel.c_str());
      }
    }
    ++j;

...
CFileDescription::CFileDescription(const string &name, uint32 time,
                                   uint32 size) {
  FileName = name;
  FileTimeStamp = time;
  FileSize = size;
...
bool CFileDescription::set(const string &name) {
  ...
  // if the file doesn't exist then zero out time stamp and size and return
  // false
...
CSString CFileDescription::toString(uint32 maxFileNameLen) const {
  return NLMISC::toString("%*s (size: %u, time stamp: %s)", maxFileNameLen,
...
void CFileDescriptionContainer::addFile(
    const CFileDescription &fileDescription) {
  _FileDescriptions.push_back(fileDescription);
}

void CFileDescriptionContainer::addFile(const string &fileName,
                                        uint32 timeStamp, uint32 size) {
...
void CFileDescriptionContainer::addFile(const string &fileName) {
  if (CFile::fileExists(fileName)) {
    addFile(fileName, CFile::getFileModificationDate(fileName),
            CFile::getFileSize(fileName));
  }
}

void CFileDescriptionContainer::addFileSpec(const string &fileSpec,
                                            bool recurse) {
...
void CFileDescriptionContainer::addFiles(
    const std::string &directory, const std::vector<std::string> &wildcards,
    bool recurse) {
  H_AUTO(fdcAddFiles)
  ...
  vector<string> rawFileNames;
  {
    ...
    NLMISC::CPath::getPathContent(path.c_str(), recurse, false, true,
                                  rawFileNames);
  }
  ...
  // extract the files that match the given wildcard and build a result vector
  {
    ...
    for (uint32 i = (uint32)rawFileNames.size(); i--;) {
      ...
      for (j = 0; j < wildcards.size(); ++j) {
        if (testWildCard(NLMISC::CFile::getFilename(rawFileNames[i]),
                         wildcards[j]))
          break;
      }
      if (j != wildcards.size()) {
...
void CFileDescriptionContainer::addFiles(
    const std::string &directory, const NLMISC::CVectorSString &wildcards,
    bool recurse) {
  // note that a CVectorSString is guaranteed to be bitwise identical to a
  // vector of std::string
  addFiles(directory,
           reinterpret_cast<const std::vector<std::string> &>(wildcards),
           recurse);
}

void CFileDescriptionContainer::addFiles(
    const CFileDescriptionContainer &other) {
  ...
  // make room in the file description container for the data from the other
  // object
...
void CFileDescriptionContainer::display(NLMISC::CLog *log) const {
  ...
  vector<string> lines;
  for (uint32 i = 0; i < _FileDescriptions.size(); ++i)
    lines.push_back(NLMISC::toString(
        "%-*s %12d %s", longest, _FileDescriptions[i].FileName.c_str(),
  ...
  log->displayNL("%-*s %12s %12s", longest, "File_Name", "File_Size",
                 "File_Time");
...
void CFileDescriptionContainer::clear() { _FileDescriptions.clear(); }

const CFileDescription &
CFileDescriptionContainer::operator[](uint32 idx) const {
...
void CFileDescriptionContainer::removeFile(uint32 idx) {
  BOMB_IF(
      idx >= _FileDescriptions.size(),
      "trying to remove files beyond the end of the file description vector",
      return);
...
void CFileDescriptionContainer::stripFilename(const std::string &header) {
  for (std::vector<CFileDescription>::iterator it = _FileDescriptions.begin();
       it != _FileDescriptions.end(); ++it) {

...
public:
  ...
  /// Return the version of this class, increments this value when the content
  /// of this class changed
  ...
  void addACustomProperty(std::string &name,
                          TTypeOfProp typeOfProp /*, sint8 weight*/,
                          bool propIsList) {
  ...
  /// Return the dataset size to use. If found in the .cfg, use the value of the
  /// property DataSetSize + DataSetName., otherwise use the value in the sheet.
  ...
  // std::vector<sint8>		Weights; // not used: currently all property
  // values are always sent
...
/**
 * Pointers for accessing the values and the change timestamps of a specific
 * property
 */
struct TPropertyValueArrayHeader {
  ...
  /// Pointer on the list container (points into shared memory if the property
  /// is a list property)
  void *ListCellContainer;

  /// Return the location of the index of the first free cell (beginning the
  /// list of free cells)
  static TSharedListRow *getFreeCellsFront(void *listCellContainer) {
    return (TSharedListRow *)(((uint8 *)listCellContainer) -
...
/**
 * Pointers for accessing the entity ids and the booleans to check if there are
 * still online or if they have been removed
 */
struct TEntityIdArrayHeader {
  ...
  void setSpawnerServiceId(TDataSetIndex entityIndex,
  ...
  void checkSpawnerId(TDataSetIndex entityIndex,
                      NLNET::TServiceId serviceId) const {
    nlassert(SpawnerServiceIds[entityIndex] == NLNET::TServiceId8(serviceId));
  }

  /// Update the online timestamp of an entity. Done when creating an entity,
  /// when setting/unsetting it online
  ...
  /// Check if the row has not been reassigned and invalidated the dataset row
  /// object
  bool isDataSetRowStillValid(const TDataSetRow &dataSetRow) const {
    return (dataSetRow.counter() ==
            0) // no removal counter in the object
#ifdef NL_DEBUG
               // check if the row has not been reassigned to another entity;
               // if the entity has left, check if the entity timestamp is not
               // too old
           ...
           ((Counts[dataSetRow.getIndex()] == dataSetRow.counter() + 1) &&
            (CTickEventHandler::getGameCycle() <
             OnlineTimestamps[dataSetRow.getIndex()] + 15)) ||
           (((Counts[dataSetRow.getIndex()] == 1) &&
             (dataSetRow.counter() == 255)) && // 0 is skipped
            (CTickEventHandler::getGameCycle() <
             OnlineTimestamps[dataSetRow.getIndex()] + 15));
#else
               // check if the row has not been reassigned to another entity
               // (but it can have been deleted)
           || (Counts[dataSetRow.getIndex()] == dataSetRow.counter()) ||
           (Counts[dataSetRow.getIndex()] == dataSetRow.counter() + 1) ||
           ((Counts[dataSetRow.getIndex()] == 1) &&
            (dataSetRow.counter() == 255)); // 0 is skipped
  ...
  /// These counters are there to check if we don't forget to remove our row
  /// references when entities leave
  uint8 *Counts;

  /// These service ids tell which service is responsible for managing each
  /// entity.
  NLNET::TServiceId8 *SpawnerServiceIds;
};

/**
 * Pointers to entity ids and property values of one container, corresponding to
 * one dataset. Data pointed to may be in shared memory.
 *
...
struct TPropertyContainerHeader {
  ...
  /// Array (indexed by TPropertyIndex) of property value arrays (indexed by
  /// TDataSetRow)
...
typedef CHashMap<NLMISC::CEntityId, TDataSetIndex,
                 NLMISC::CEntityIdHashMapTraits>
    TEntityIdToEntityIndexMap;
...
public:
  ...
  TPropertyIndex nbProperties() const {
    return (TPropertyIndex)_PropIndexToType.size();
  }
  ...
  //	void						serialOutRow(
  //NLMISC::CMemStream& msgout, const TDataSetRow& entityIndex ) const
  //	{
  ...
  //		//case Row16 : { TDataSetRow16 row16 =
  //(TDataSetRow16)entityIndex.getIndex(); msgout.fastWrite( row16 ); /*nldebug(
  //"Written %hu", row16 );*/ break; }
  //		//case Row8  : { TDataSetRow8 row8 =
  //(TDataSetRow8)entityIndex.getIndex(); msgout.fastWrite( row8 ); break; }
  //		//default  : ;
  //		//}
  //	}
  ...
  //	void						serialInRow(
  //NLMISC::CMemStream& msgin, TDataSetRow& entityIndex ) const
  //	{
  ...
  template <class T>
  void getPropPointer(T **ppt, TPropertyIndex propIndex,
                      const TDataSetRow &entityIndex) const {
#ifndef FAST_MIRROR
    nlassert((propIndex != INVALID_PROPERTY_INDEX) &&
             (propIndex < (TPropertyIndex)_PropertyContainer.PropertyValueArrays
                              .size())); // Wrong property
    nlassert(_PropertyContainer.PropertyValueArrays[propIndex]
                 .Values); // Pointer not initialized
    nlassertex(
        entityIndex.getIndex() < (uint32)maxNbRows(),
        ("E%d",
         entityIndex.getIndex())); // Wrong entity (including
                                   // INVALID_ENTITY_INDEX and LAST_CHANGED)
    checkTemplateSize(sizeof(T), propIndex, entityIndex); // Wrong template type
#endif
#ifdef CHECK_DATASETROW_VALIDITY
    // nlassertex( isDataSetRowStillValid( entityIndex ), ("E%u GIVEN:%hu_%u
    // REAL:%hu_%u", entityIndex.getIndex(), entityIndex.counter(),
    // CTickEventHandler::getGameCycle(),
    ...
    {
        nlwarning( "isDataSetRowStillValid E%u GIVEN:%hu_%u REAL:%hu_%u",
    entityIndex.getIndex(), entityIndex.counter(),
    CTickEventHandler::getGameCycle(),
    _PropertyContainer.EntityIdArray.Counts[entityIndex.getIndex()],
    _PropertyContainer.EntityIdArray.OnlineTimestamps[entityIndex.getIndex()] );
...
#endif
    *ppt = &(((T *)(_PropertyContainer.PropertyValueArrays[propIndex]
                        .Values))[entityIndex.getIndex()]);
  }

  // PUBLIC because template friend classes are not supported
  /// Return the pointer to the property value
  template <class T>
  void getPropPointerForList(TSharedListRow **ppt, TPropertyIndex propIndex,
    ...
    nlassertex(
        ...
        ("E%d",
         entityIndex.getIndex())); // Wrong entity (including
                                   // INVALID_ENTITY_INDEX and LAST_CHANGED)
    // Following test commented out, because property actually in row is not the
    // size of the property in list!
    checkTemplateSize(sizeof(T), propIndex, entityIndex); // Wrong template type
#endif
#ifdef CHECK_DATASETROW_VALIDITY
    // nlassertex( isDataSetRowStillValid( entityIndex ), ("E%u GIVEN:%hu_%u
...
#endif
    *ppt =
        &(((TSharedListRow *)(_PropertyContainer.PropertyValueArrays[propIndex]
                                  .Values))[entityIndex.getIndex()]);
  }
  ...
  /// Check that the template argument T passed to CMirrorPropValue<T> has the
  /// right size (debug feature)
  void checkTemplateSize(uint32 passedSize, TPropertyIndex propIndex,
                         const TDataSetRow &entityIndex) const;
  ...
  NLMISC::TGameCycle getChangeTimestamp(TPropertyIndex propIndex,
                                        const TDataSetRow &entityIndex) const {
#ifndef FAST_MIRROR
    nlassert((propIndex != INVALID_PROPERTY_INDEX) &&
             (propIndex < (TPropertyIndex)_PropertyContainer.PropertyValueArrays
                              .size())); // Wrong property
    nlassert(_PropertyContainer.PropertyValueArrays[propIndex]
                 .ChangeTimestamps); // Pointer not initialized
    nlassertex(
        entityIndex.getIndex() < (uint32)maxNbRows(),
        ("E%d",
         entityIndex.getIndex())); // Wrong entity (including
                                   // INVALID_ENTITY_INDEX and LAST_CHANGED)
#endif
    return _PropertyContainer.PropertyValueArrays[propIndex]
        .ChangeTimestamps[entityIndex.getIndex()];
  }

#ifdef STORE_CHANGE_SERVICEIDS
  /// Return the id of the latest service who changed the property value
  NLNET::TServiceId8 getWriterServiceId(TPropertyIndex propIndex,
                                        const TDataSetRow &entityIndex) const {
#ifndef FAST_MIRROR
    nlassert((propIndex != INVALID_PROPERTY_INDEX) &&
             (propIndex < (TPropertyIndex)_PropertyContainer.PropertyValueArrays
                              .size())); // Wrong property
    nlassert(_PropertyContainer.PropertyValueArrays[propIndex]
                 .ChangeServiceIds); // Pointer not initialized
    nlassertex(
        entityIndex.getIndex() < (uint32)maxNbRows(),
        ("E%d",
         entityIndex.getIndex())); // Wrong entity (including
...
#endif
    return _PropertyContainer.PropertyValueArrays[propIndex]
        .ChangeServiceIds[entityIndex.getIndex()];
  }
#endif
  ...
  NLMISC::TGameCycle getOnlineTimestamp(const TDataSetRow &entityIndex) const {
    ...
    nlassert(entityIndex.isValid());
#endif
    return _PropertyContainer.EntityIdArray
        .OnlineTimestamps[entityIndex.getIndex()];
  }

protected:
  /** Fill property info.
   * When the flag initValues is set:
   * If the counts must be initialised from a remote MS, use the vector
   * ptCountsToSet (it will be read then deleted) and timestamp, otherwise set
   * NULL. segmentSize is used only if initValues is true. If propName is the
   * special entityId property, the behaviour is different (propName need not
   * exist)
   */
  void setPropertyPointer(std::string &propName, TPropertyIndex propIndex,
                          void *segmentPt, bool initValues, uint32 dataTypeSize,
                          bool isReadOnly, bool mustMonitorAssignment,
                          std::vector<uint8> *ptCountsToSet = NULL,
                          NLMISC::TGameCycle timestamp = 0,
  ...
  /// Return the id of the service who spawned the entity (even after it was
  /// despawned, while the row is not reassigned)
...
public:
    ...
    /// Add an entity type into the list of accepted entity types (no duplicate
check) void						addAcceptedEntityType(
uint8 entityType ) { _AcceptedTypes.push_back( entityType ); }

    /// Test if an entity type is in the list
    void						isAccepted( uint8
entityType ) { return (find( _AcceptedTypes.begin(), _AcceptedTypes.end(),
entityType ) != _AcceptedTypes.end()); }

 ...
 */
template <class TDS> struct TPropertyInfoBase {
  /// Constructor
  TPropertyInfoBase(TDS *ds, TPropertyIndex propindex)
      : Segment(NULL), DataSet(ds), PropertyIndex(propindex), SMId(-1) {}
...
public:
  ...
  TDS *getDataSetByPropName(const std::string &propName,
...
enum TPropertyDebugFlag {
  FlagReadOnly = 0,
  FlagWriteOnly = 1,
  FlagNotifyChanges = 3,
  FlagMonitorAssignment = 4,
  FlagGroupNotifiedByOtherProp = 5
};
...
struct TPropertyInfo : public TPropertyInfoBase<CMirroredDataSet> {
  /// Constructor
  TPropertyInfo(CMirroredDataSet *ds = NULL,
                TPropertyIndex propindex = INVALID_PROPERTY_INDEX)
      : TPropertyInfoBase<CMirroredDataSet>(ds, propindex), Pending(false),
        PropTrackersPending(false), _PropertyDebugFlags(0),
        _PropNotifyingTheGroup(INVALID_PROPERTY_INDEX) {}

  // True if allocation of the property is being processed (property not
  // allocated yet)
  bool Pending;
  ...
  bool flagWriteOnly() const {
    return (_PropertyDebugFlags & PSOWriteOnly) != 0;
  }
  bool flagNotifyChanges() const {
    return (_PropertyDebugFlags & PSONotifyChanges) != 0;
  }
  bool flagMonitorAssignment() const {
    return (_PropertyDebugFlags & PSOMonitorAssignment) != 0;
  }
  bool flagGroupNotifiedByOtherProp() const {
    return (_PropertyDebugFlags & (1 << FlagGroupNotifiedByOtherProp)) != 0;
  }
  TPropertyIndex propNotifyingTheGroup() const {
    return _PropNotifyingTheGroup;
  }
  std::string getFlagsString() const;
  void setFlags(TPropSubscribingOptions options, bool groupNotif) {
    _PropertyDebugFlags =
        options | (groupNotif << FlagGroupNotifiedByOtherProp);
  }
  void setFlagReadOnly(bool b) { _PropertyDebugFlags |= (b << FlagReadOnly); }
  void setFlagNotifyChanges(bool b) {
    _PropertyDebugFlags |= (b << FlagNotifyChanges);
  }
  void setFlagMonitorAssignment(bool b) {
    _PropertyDebugFlags |= (b << FlagMonitorAssignment);
  }
  void setFlagGroupNotifiedByOtherProp(bool b, TPropertyIndex otherProp) {
    _PropertyDebugFlags |= (b << FlagGroupNotifiedByOtherProp);
    _PropNotifyingTheGroup = otherProp;
  }
...
private:
  ...
  /// Index of another prop notifying this prop, if
  /// flagGroupNotifiedByOtherProp() (note: in the same dataset)
  TPropertyIndex _PropNotifyingTheGroup;
};

typedef TMapOfPropertyInfoBase<TPropertyInfo, CMirroredDataSet>
    TPropertiesInMirror;
...
public:
  ...
  /// Return a pointer to the property, or NULL if it is not ready yet (try
  /// again)

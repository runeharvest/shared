...
/*
 * Activate this define if you want to set a value into the mirror only if it is
 * different from the previous value (more comparisons but less transfers if you
 * ofter set a property to the same value)
 */
...
/// Constructor
inline CPropLocationPacked1DS::CPropLocationPacked1DS(
    CMirroredDataSet &dataSet, TPropertyIndex propIndex,
    const TDataSetRow &entityIndex) {
  init(dataSet, propIndex, entityIndex);
}

/// Initialize
inline void CPropLocationPacked1DS::init(CMirroredDataSet &dataSet,
                                         TPropertyIndex propIndex,
                                         const TDataSetRow &entityIndex) {
  _DataSetRow16 = (TDataSetRow16)entityIndex.getIndex();
  _PropIndexAndDSR4 =
      propIndex | (uint16)((((uint32)entityIndex.getIndex()) & 0xF0000) >> 4);
}
...
/// Get entity index
inline TDataSetRow CPropLocationPacked1DS::dataSetRow() const {
  return (TDataSetRow)((uint32)_DataSetRow16 |
                       (((uint32)_PropIndexAndDSR4) & 0xF000) << 4);
}
...
inline bool CPropLocationPacked1DS::rowIsUsed() const {
  if (dataSet()->getEntityId(dataSetRow()).isUnknownId()) {
    nlwarning("MIRROR/%s: Writing value of prop %hd for row %d which is empty "
              "(no CEntityId)",
              dataSet()->name().c_str(), propIndex(), dataSetRow().getIndex());
    return false;
  } else
    return true;
}
...
/*
 * Display the new value when it has changed, if monitoring is enabled for the
 * property
 */
inline void CPropLocationPacked1DS::monitorValue(const char *valueStr) const {
  if (dataSet()->isMonitored(propIndex())) {
    MIRROR_INFO("MIRROR:MON: Value of %s/E%d/P%hd changed to %s",
                dataSet()->name().c_str(), dataSetRow().getIndex(), propIndex(),
                valueStr);
  }
}
...
/// Set changed
inline void CPropLocationPacked1DS::setChanged() {
  dataSet()->setChanged(dataSetRow(), propIndex());
}
...
/// Constructor
template <int nbits>
inline CPropLocationPacked<nbits>::CPropLocationPacked(
    CMirroredDataSet &dataSet, TPropertyIndex propIndex,
    const TDataSetRow &entityIndex) {
  init(dataSet, propIndex, entityIndex);
}
...
/// Initialize
template <int nbits>
inline void CPropLocationPacked<nbits>::init(CMirroredDataSet &dataSet,
                                             TPropertyIndex propIndex,
                                             const TDataSetRow &entityIndex) {
  _DataSetRow16 = (TDataSetRow16)entityIndex.getIndex();
  _PropIndexAndDataSetAndDSR4 =
      (uint16)propIndex | ((datasetToBitIndex(&dataSet)) << (12 - nbits)) |
      (uint16)((((uint32)entityIndex.getIndex()) & 0xF0000) >> 4);
}

/// Go back to default state (deinit)
template <int nbits> inline void CPropLocationPacked<nbits>::reset() {
  _DataSetRow16 = 0;
  _PropIndexAndDataSetAndDSR4 = (datasetToBitIndex(NULL))
                                << (12 - nbBitsForDataset);
}
...
inline CMirroredDataSet *CPropLocationPacked<nbits>::dataSet() const {
  return bitIndexToDataset((_PropIndexAndDataSetAndDSR4 & 0x0FFF) >>
                           (12 - nbits));
}

/// Get entity index
template <int nbits>
inline TDataSetRow CPropLocationPacked<nbits>::dataSetRow() const {
  return dataSet()->getCurrentDataSetRow(
      ((uint32)_DataSetRow16 | (((uint32)_PropIndexAndDataSetAndDSR4) & 0xF000)
                                   << 4));
}
...
inline TPropertyIndex CPropLocationPacked<nbits>::propIndex() const {
  return (TPropertyIndex)(_PropIndexAndDataSetAndDSR4 &
                          ((1 << (12 - nbits)) - 1));
}
 ...
 * Return true if the row is valid for writing/reading
 */
template <int nbits> inline bool CPropLocationPacked<nbits>::rowIsUsed() const {
  if (dataSet()->getEntityId(dataSetRow()).isUnknownId()) {
    nlwarning("MIRROR/%s: Writing value of prop %hd for row %d which is empty "
              "(no CEntityId)",
              dataSet()->name().c_str(), propIndex(), dataSetRow().getIndex());
...
template <int nbits>
inline void
CPropLocationPacked<nbits>::monitorValue(const char *valueStr) const {
  if (dataSet()->isMonitored(propIndex())) {
    MIRROR_INFO("MIRROR:MON: Value of %s/E%d/P%hd changed to %s",
                dataSet()->name().c_str(), dataSetRow().getIndex(), propIndex(),
                valueStr);
...
/// Set changed
template <int nbits> inline void CPropLocationPacked<nbits>::setChanged() {
  dataSet()->setChanged(dataSetRow(), propIndex());
}
...
/// Initialize
inline void CPropLocationUnpacked::init(CMirroredDataSet &dataSet,
                                        TPropertyIndex propIndex,
                                        const TDataSetRow &entityIndex) {
  _DataSet = &dataSet;
...
/// Get dataset
inline CMirroredDataSet *CPropLocationUnpacked::dataSet() const {
  return _DataSet;
}

/// Get entity index
inline TDataSetRow CPropLocationUnpacked::dataSetRow() const {
  return _DataSetRow;
}

/// Get property index
inline TPropertyIndex CPropLocationUnpacked::propIndex() const {
  return _PropIndex;
}
...
inline bool CPropLocationUnpacked::rowIsUsed() const {
  if (_DataSet->getEntityId(_DataSetRow).isUnknownId()) {
    nlwarning("MIRROR/%s: Writing value of prop %hd for row %u which is empty "
              "(no CEntityId)",
              _DataSet->name().c_str(), _PropIndex, _DataSetRow.getIndex());
    return false;
  } else
    return true;
}
...
inline void CPropLocationUnpacked::monitorValue(const char *valueStr) const {
  if (_DataSet->isMonitored(_PropIndex)) {
    MIRROR_INFO("MIRROR:MON: Value of %s/E%u/P%hd changed to %s",
                _DataSet->name().c_str(), _DataSetRow.getIndex(), _PropIndex,
                valueStr);
  }
}
...
/// Set changed
inline void CPropLocationUnpacked::setChanged() {
 ...
 */
template <class T>
CMirrorPropValueBase<T>::CMirrorPropValueBase
NL_TMPL_PARAM_ON_METHOD_1(T)(CMirroredDataSet &dataSet,
                             const TDataSetRow &entityIndex,
                             TPropertyIndex propIndex) {
...
CMirrorPropValue<T, CPropLocationClass>::CMirrorPropValue
NL_TMPL_PARAM_ON_METHOD_2(T, CPropLocationClass)(CMirroredDataSet &dataSet,
                                                 const TDataSetRow &entityIndex,
                                                 TPropertyIndex propIndex)
...
template <class T, class CPropLocationClass>
CMirrorPropValue<T, CPropLocationClass>::CMirrorPropValue
NL_TMPL_PARAM_ON_METHOD_2(T,
                          CPropLocationClass)(CMirroredDataSet &dataSet,
                                              const NLMISC::CEntityId &entityId,
                                              const std::string &propName) {
  init(dataSet, entityId, propName);
}
 ...
 */
template <class T>
void CMirrorPropValueBase<T>::init(CMirroredDataSet &dataSet,
                                   const TDataSetRow &entityIndex,
                                   TPropertyIndex propIndex) {
#ifndef FAST_MIRROR
  nlassertex(entityIndex.getIndex() < (uint32)dataSet.maxNbRows(),
             ("E%d", entityIndex.getIndex()));
  /*if ( dataSet.getEntityId( entityIndex ).isUnknownId() )
  {
      nlwarning( "MIRROR: Accessing a value of prop %hu for row %u which is
  empty (no CEntityId)", propIndex, entityIndex.getIndex() );
  }*/
#endif
 ...
 */
template <class T, class CPropLocationClass>
void CMirrorPropValue<T, CPropLocationClass>::init(
    CMirroredDataSet &dataSet, const TDataSetRow &entityIndex,
    TPropertyIndex propIndex) {
...
void CMirrorPropValueBase<T>::init(CMirroredDataSet &dataSet,
                                   const NLMISC::CEntityId &entityId,
                                   const std::string &propName) {
  dataSet.getPropPointer(&_Pt, dataSet.getPropertyIndex(propName),
                         dataSet.getDataSetRow(entityId));
}
...
template <class T, class CPropLocationClass>
void CMirrorPropValue<T, CPropLocationClass>::init(
    CMirroredDataSet &dataSet, const NLMISC::CEntityId &entityId,
    const std::string &propName) {
  _PropLocation.init(dataSet, dataSet.getPropertyIndex(propName),
                     dataSet.getDataSetRow(entityId));
  CMirrorPropValueBase<T>::init(dataSet, _PropLocation.dataSetRow(),
                                _PropLocation.propIndex());
}

/*
 * Return true if the CMirrorPropValue<T,CPropLocationClass> object is
 * initialized or contains a temporary value (i.e. operator () can be called)
 */
template <class T> bool CMirrorPropValueBase<T>::isReadable() const {
  return (_Pt != NULL);
}
 ...
 * Get the value
 */
template <class T> const T &CMirrorPropValueBase<T>::operator()() const {
#ifndef FAST_MIRROR
 ...
 * CMirrorPropValue<T,CPropLocationClass> object must be UNitialized (i.e. the
 * default constructor has been called but not any other constructor or init()).
 */
template <class T> void CMirrorPropValueBase<T>::tempStore(const T &srcValue) {
#ifndef FAST_MIRROR
  nlassert(!_Pt);
#endif
  _Pt = new T();
 ...
 */
template <class T> void CMirrorPropValueBase<T>::tempAllocate() {
#ifndef FAST_MIRROR
  nlassert(!_Pt);
#endif
  _Pt = new T();
 ...
 */
template <class T> const T &CMirrorPropValueBase<T>::tempValue() const {
#ifndef FAST_MIRROR
  nlassert(_Pt);
#endif
  return *_Pt;
 ...
 */
template <class T> void CMirrorPropValueBase<T>::tempDelete() {
#ifndef FAST_MIRROR
  nlassert(_Pt);
#endif
  delete _Pt;
...
template <class T, class CPropLocationClass>
NLMISC::TGameCycle
CMirrorPropValue<T, CPropLocationClass>::getTimestamp() const {
#ifndef FAST_MIRROR
  nlassert(_PropLocation.dataSet());
#endif
  return _PropLocation.dataSet()->getChangeTimestamp(
      _PropLocation.propIndex(), _PropLocation.dataSetRow());
}
...
template <class T, class CPropLocationClass>
NLNET::TServiceId8
CMirrorPropValue<T, CPropLocationClass>::getWriterServiceId() const {
#ifndef FAST_MIRROR
  nlassert(_PropLocation.dataSet());
#endif
  return _PropLocation.dataSet()->getWriterServiceId(
      _PropLocation.propIndex(), _PropLocation.dataSetRow());
}
#endif
...
CMirrorPropValue<T, CPropLocationClass>::operator=(const T &srcValue) {
  // Debug checks
#ifndef FAST_MIRROR
  ...
  if (_PropLocation.dataSet()) {
    nlassert((_PropLocation.dataSetRow().isValid()) &&
             (_PropLocation.dataSetRow().getIndex() <
              (uint32)_PropLocation.dataSet()->maxNbRows()));
    if (_PropLocation.isReadOnly())
      nlwarning("MIRROR: Overwriting a value of read-only property %s/%hd",
                _PropLocation.dataSet()->name().c_str(),
                _PropLocation.propIndex());
  }
#endif

  // Copy the value (if MIRROR_ASSIGN_ONLY_IF_CHANGED: only if the new value is
  // different)
#ifdef MIRROR_ASSIGN_ONLY_IF_CHANGED
  if (srcValue != *CMirrorPropValueBase<T>::_Pt) {
#endif

    // Atomicity is not ensured (e.g. for int64) but setChanged() is called
    // after complete assigment
#ifdef NL_DEBUG
 ...
 */
template <class T, class CPropLocationClass>
const CPropLocationClass &
CMirrorPropValue<T, CPropLocationClass>::location() const {
  return _PropLocation;
}
 ...
 * tempStore() must have been called before and not tempMirrorize() yet.
 */
template <class T, class CPropLocationClass>
 ...
 */
template <class T> std::string CMirrorPropValueBase<T>::toString() const {
...
#ifndef FAST_MIRROR
  nlassertex(!isInitialized(), ("tempStore used on CMirrorPropValue "
                                "initialized in the mirror (E%u P%hd)!",
                                _PropLocation.dataSetRow().getIndex(),
                                _PropLocation.propIndex()));
#endif
...
#ifndef FAST_MIRROR
  nlassertex(isInitialized(), ("tempRestore used on CMirrorPropValue not "
                               "initialized in the mirror (E%u P%hd)!",
                               _PropLocation.dataSetRow().getIndex(),
                               _PropLocation.propIndex()));
#endif
...
#ifndef FAST_MIRROR
  nlassertex(!isInitialized(), ("tempAllocate used on CMirrorPropValue "
                                "initialized in the mirror (E%u P%hd)!",
                                _PropLocation.dataSetRow().getIndex(),
                                _PropLocation.propIndex()));
#endif
...
void CMirrorPropValue<T, CPropLocationClass>::tempReassign(const T &srcValue) {
  ...
  nlassertex(!isInitialized(), ("tempReassign used on CMirrorPropValue "
                                "initialized in the mirror (E%u P%hd)!",
                                _PropLocation.dataSetRow().getIndex(),
                                _PropLocation.propIndex()));
#endif
 ...
 */
template <class T, class CPropLocationClass>
void CMirrorPropValue<T, CPropLocationClass>::tempMirrorize(
    CMirroredDataSet &dataSet, const TDataSetRow &entityIndex,
    TPropertyIndex propIndex) {
#ifndef FAST_MIRROR
  nlassert(CMirrorPropValueBase<T>::_Pt);
 ...
 * SET_STRUCT_MEMBER (setChanged() must be called after!)
 */
template <class T, class CPropLocationClass>
...
#ifdef NL_DEBUG
  nlassert(_PropLocation.dataSetRow().getIndex() <
           _PropLocation.dataSet()->maxNbRows());
  if (_PropLocation.dataSet()
          ->getEntityId(_PropLocation.dataSetRow())
          .isUnknownId()) {
    nlwarning("MIRROR: Writing value of prop %hd for row %d which is empty (no "
              "CEntityId)",
              _PropLocation.propIndex(), _PropLocation.dataSetRow().getIndex());
  } else {
 ...
 */
template <class T, class CPropLocationClass>
CMirrorPropValueAlice<T, CPropLocationClass>::CMirrorPropValueAlice
NL_TMPL_PARAM_ON_METHOD_2(T, CPropLocationClass)(CMirroredDataSet &dataSet,
                                                 const TDataSetRow &entityIndex,
                                                 TPropertyIndex propIndex)
 ...
 */
template <class T, class CPropLocationClass>
void CMirrorPropValueAlice<T, CPropLocationClass>::init(
    CMirroredDataSet &dataSet, const TDataSetRow &entityIndex,
    TPropertyIndex propIndex) {
#ifndef FAST_MIRROR
  nlassert(!_InMirror);
  ...
  CMirrorPropValue<T, CPropLocationClass>::init(dataSet, entityIndex,
                                                propIndex);
  _InMirror = true;
 ...
 */
template <class T, class CPropLocationClass>
void CMirrorPropValueAlice<T, CPropLocationClass>::changeLocation(
    CMirroredDataSet &dataSet, const TDataSetRow &newEntityIndex,
    TPropertyIndex newPropIndex) {
#ifndef FAST_MIRROR
  nlassert(_InMirror);
#endif
  CMirrorPropValue<T, CPropLocationClass>::init(dataSet, newEntityIndex,
                                                newPropIndex);
}
 ...
 */
template <class T, class CPropLocationClass>
CMirrorPropValueAlice<T, CPropLocationClass> &
CMirrorPropValueAlice<T, CPropLocationClass>::operator=(const T &srcValue) {
  if (_InMirror) {
...
void CMirrorPropValueAlice<T, CPropLocationClass>::serial(NLMISC::IStream &s) {
  if (s.isReading()) {
    T temp;
    s.serial(temp);
    (*this) = temp; // CMirrorPropValueAlice::operator= and not the one from
                    // CMirrorPropValue
  } else {
    s.serial(const_cast<T &>((*this)()));
  }
...
template <class T, class CPropLocationClass>
void CMirrorPropValueAlice<T, CPropLocationClass>::tempMirrorize(
    CMirroredDataSet &dataSet, const TDataSetRow &entityIndex,
    TPropertyIndex propIndex) {
  CMirrorPropValue<T, CPropLocationClass>::tempMirrorize(dataSet, entityIndex,
                                                         propIndex);
  _InMirror =
      true; /*nlinfo( "Alice %p now IN mirror because mirrorize", this );*/
}
...
void CMirrorPropValueAlice<T, CPropLocationClass>::tempMirrorize(
    CMirroredDataSet &dataSet, const NLMISC::CEntityId &entityId,
    const std::string &propName) {
  CMirrorPropValue<T, CPropLocationClass>::tempMirrorize(dataSet, entityId,
                                                         propName);
  _InMirror =
      true; /*nlinfo( "Alice %p now IN mirror because mirrorize", this );*/
}
...
template <class T, class CPropLocationClass>
NLMISC::TGameCycle
CMirrorPropValueAlice<T, CPropLocationClass>::getTimestamp() const {
  if (_InMirror)
...
template <class T, class CPropLocationClass>
NLNET::TServiceId8
CMirrorPropValueAlice<T, CPropLocationClass>::getWriterServiceId() const {
  if (_InMirror)
...
template <class T, class CPropLocationClass>
CMirrorPropValueAlice<T, CPropLocationClass>::~CMirrorPropValueAlice
NL_TMPL_PARAM_ON_METHOD_2(T, CPropLocationClass)() {
 ...
 */
template <class T>
CMirrorPropValueBaseCF<T>::CMirrorPropValueBaseCF
NL_TMPL_PARAM_ON_METHOD_1(T)(CMirroredDataSet &dataSet,
                             const TDataSetRow &entityIndex,
                             TPropertyIndex propIndex)
    : CMirrorPropValueBase<T>(dataSet, entityIndex, propIndex),
 ...
 */
template <class T, class CPropLocationClass>
CMirrorPropValueCF<T, CPropLocationClass>::CMirrorPropValueCF
NL_TMPL_PARAM_ON_METHOD_2(T, CPropLocationClass)(CMirroredDataSet &dataSet,
                                                 const TDataSetRow &entityIndex,
                                                 TPropertyIndex propIndex)
    : CMirrorPropValue<T, CPropLocationClass>(dataSet, entityIndex, propIndex),
 ...
 * resetChangeFlag()
 */
template <class T> bool CMirrorPropValueBaseCF<T>::hasChanged() const {
 ...
 */
template <class T> void CMirrorPropValueBaseCF<T>::resetChangeFlag() {
 ...
 */
template <class T> T CMirrorPropValueBaseCF<T>::previousValue() const {
 ...
 */
template <class T, class CPropLocationClass>
CMirrorPropValueList<T, CPropLocationClass>::CMirrorPropValueList
NL_TMPL_PARAM_ON_METHOD_2(T, CPropLocationClass)(CMirroredDataSet &dataSet,
                                                 const TDataSetRow &entityIndex,
                                                 TPropertyIndex propIndex)
    : _PropLocation(dataSet, propIndex, entityIndex) {
...
#endif
#ifndef FAST_MIRROR
  // This warning is disabled, because it occurs when called by killList() or
  // clear() when clearing orphan lists. if (
  // _PropLocation.dataSet()->getEntityId( _PropLocation.dataSetRow()
  // ).isUnknownId() )
  //{
  //	nlwarning( "MIRROR: Accessing a value of prop %hd for row %d which is
  //empty (no CEntityId)", _PropLocation.propIndex(),
  //_PropLocation.dataSetRow().getIndex() );
  //}
...
#endif
  dataSet.getPropPointerForList(&_PtFront, _PropLocation.propIndex(),
                                _PropLocation.dataSetRow(), (T *)NULL);
  _Container = (TSharedListCell *)(dataSet.getListCellContainer(
      _PropLocation.propIndex()));
}
...
template <class T, class CPropLocationClass>
CMirrorPropValueItem<T, CPropLocationClass>
CMirrorPropValueList<T, CPropLocationClass>::front() {
  return CMirrorPropValueItem<T, CPropLocationClass>(
      this, &(_Container[*_PtFront].Value));
}
...
#ifdef NL_DEBUG
    _PropLocation.setChanged(
        NLMISC::toString("pushed %s", NLMISC::toString(value).c_str()).c_str());
#else
    _PropLocation.setChanged();
#endif
  } else
    nlwarning("No shared list storage left (prop %hd)",
              _PropLocation.propIndex());
}
...
TSharedListRow CMirrorPropValueList<T, CPropLocationClass>::allocateNewCell() {
  ...
  while ( (_Container[row].Next != UNUSED_SHAREDLIST_CELL) && (row <
  ...
  ++NbAllocdListCells; // not correct if there's no free cell (but it's just a
                       // simple debugging feature)
  if (NbAllocdListCells > MaxNbAllocdListCells)
    MaxNbAllocdListCells = NbAllocdListCells;

  // New linked-list code
  TSharedListRow *firstFreeCell =
      TPropertyValueArrayHeader::getFreeCellsFront((void *)_Container);
...
template <class T, class CPropLocationClass>
void CMirrorPropValueList<T, CPropLocationClass>::releaseCell(
    TSharedListRow row) {
  /*// Old trivial linear browsing code
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::size_type
CMirrorPropValueList<T, CPropLocationClass>::size() const {
  size_type s = 0;
  const_iterator it;
...
template <class T, class CPropLocationClass>
NLMISC::TGameCycle
CMirrorPropValueList<T, CPropLocationClass>::getTimestamp() const {
#ifndef FAST_MIRROR
  nlassert(_PropLocation.dataSet());
#endif
  return _PropLocation.dataSet()->getChangeTimestamp(
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::size_type
CMirrorPropValueList<T, CPropLocationClass>::max_size() const {
  return INVALID_SHAREDLIST_ROW; // TEMP (less the size of other lists!)
}

/*
 * Ensures the size of the list is n:
 * - If the requested size is greater than the previous size, new elements are
 * pushed at front
 * - If the requested size is smaller than the previous size, elements are
 * popped at front
 */
template <class T, class CPropLocationClass>
 ...
 */
template <class T, class CPropLocationClass>
void CMirrorPropValueList<T, CPropLocationClass>::testList(
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::iterator
CMirrorPropValueList<T, CPropLocationClass>::erase(
    typename CMirrorPropValueList<T, CPropLocationClass>::iterator pos) {
#ifdef NL_DEBUG
  nlassert(!_PropLocation.isReadOnly());
#endif
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::iterator
CMirrorPropValueList<T, CPropLocationClass>::erase_after(
    typename CMirrorPropValueList<T, CPropLocationClass>::iterator pos) {
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::iterator
CMirrorPropValueList<T, CPropLocationClass>::begin() {
  iterator it;
  it._ParentList = this;
  it._Index = *_PtFront;
  return it;
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::iterator
CMirrorPropValueList<T, CPropLocationClass>::end() {
  iterator it;
  it._ParentList = this;
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::const_iterator
CMirrorPropValueList<T, CPropLocationClass>::const_begin() const {
  const_iterator it;
  it._ParentList =
      const_cast<CMirrorPropValueList<T, CPropLocationClass> *>(this);
  it._Index = *_PtFront;
  return it;
}
...
template <class T, class CPropLocationClass>
typename CMirrorPropValueList<T, CPropLocationClass>::const_iterator
CMirrorPropValueList<T, CPropLocationClass>::const_end() const {
  const_iterator it;
  it._ParentList =
      const_cast<CMirrorPropValueList<T, CPropLocationClass> *>(this);
  it._Index = INVALID_SHAREDLIST_ROW;
...
template <class T, class CPropLocationClass>
CMirrorPropValueItem<T, CPropLocationClass> &
CMirrorPropValueItem<T, CPropLocationClass>::operator=(const T &srcValue) {
#ifdef NL_DEBUG
  ...
  *_Pt = srcValue;
#ifdef NL_DEBUG
  _ParentList->_PropLocation.setChanged(NLMISC::toString(
      "assigned an item to %s", NLMISC::toString(srcValue).c_str()));
#else

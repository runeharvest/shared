...
// ***************************************************************************
CSabrinaCom::~CSabrinaCom() { _BC = NULL; }

// ***************************************************************************
void CSabrinaCom::getPhraseCost(const std::vector<NLMISC::CSheetId> &bricks,
                                uint32 &pos, uint32 &neg) const {
  pos = 0;
  neg = 0;
  float rpos = 1.0f;
  float rneg = 1.0f;
...
// ***************************************************************************
uint32 CSabrinaCom::getPhraseMaxBrickCost(
    const std::vector<NLMISC::CSheetId> &bricks) const {
  uint32 costMax = 0;
  for (uint i = 0; i < bricks.size(); i++) {
    sint32 cost = _BC->getSabrinaCost(bricks[i]);
    if (cost > 0)
...
// ***************************************************************************
sint32 CSabrinaCom::getPhraseBrickAndParamCost(
    const std::vector<NLMISC::CSheetId> &bricks, uint brickIndex) const {
  ...
  for (uint i = brickIndex;
       i < min((uint)bricks.size(), brickIndex + numParams + 1); i++) {
...
// ***************************************************************************
float CSabrinaCom::getPhraseBrickAndParamRelativeCost(
    const std::vector<NLMISC::CSheetId> &bricks, uint brickIndex) const {
  if (brickIndex >= bricks.size())
    return 0.f;
...
// ***************************************************************************
void CSabrinaCom::filterMandatoryComposition(
    const std::vector<NLMISC::CSheetId> & /* phraseBricks */,
    std::vector<NLMISC::CSheetId> & /* mandatoryBricks */) const {
  // Yoyo: for now, no simple rule known => never exclude mandatories.
}

// ***************************************************************************
TOOL_TYPE::TCraftingToolType CSabrinaCom::getPhraseFaberPlanToolType(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  // Valid only if not empty, and a faber one,
  if (phraseBricks.size() >= 1 &&
      _BC->getBrickType(phraseBricks[0]) == BRICK_TYPE::FABER) {
...
// ***************************************************************************
BRICK_FAMILIES::TBrickFamily CSabrinaCom::getPhraseForageFamily(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  if (phraseBricks.empty())
    return BRICK_FAMILIES::Unknown;

  BRICK_TYPE::EBrickType bType = _BC->getBrickType(phraseBricks[0]);
  if ((bType == BRICK_TYPE::FORAGE_PROSPECTION) ||
      (bType == BRICK_TYPE::FORAGE_EXTRACTION)) {
    for (uint i = 1; i < phraseBricks.size();
         ++i) // skip the root brick (index 0)
    {
      const CSheetId &brickId = phraseBricks[i];

      uint indexInFamily;
      BRICK_FAMILIES::TBrickFamily brickFamily =
          _BC->getBrickFamily(brickId, indexInFamily);
      // FPMA=prospection, FEMA=extraction
      if ((brickFamily == BRICK_FAMILIES::BHFPMA ||
           brickFamily == BRICK_FAMILIES::BHFEMA)) {
        ...
        BRICK_FAMILIES::TBrickFamily bf =
            (BRICK_FAMILIES::TBrickFamily)(brickFamily + indexInFamily - 1);
...
// ***************************************************************************
NLMISC::CSheetId CSabrinaCom::getPhraseBestDisplayBrick(
    const std::vector<NLMISC::CSheetId> &phraseBricks) const {
  if (phraseBricks.empty())
    return CSheetId();
  ...
  if (bType == BRICK_TYPE::MAGIC || bType == BRICK_TYPE::COMBAT) {
    ...
    for (uint i = 1; i < phraseBricks.size();) {
      ...
      BRICK_FAMILIES::TBrickFamily bf =
          _BC->getBrickFamily(brickId, indexInFamily);
      if ((BRICK_FAMILIES::isMandatoryFamily(bf) &&
           bType == BRICK_TYPE::MAGIC) ||
  ...
  // Take the last most expensive brick (except the root and ecotype
  // specializing)
  else if ((bType == BRICK_TYPE::FORAGE_PROSPECTION) ||
           (bType == BRICK_TYPE::FORAGE_EXTRACTION)) {
    uint bestIndex = 0;
    uint bestCost = 0;
         ...
         ++i) // skip the root brick (index 0)
    {
      ...
      // Skip counterpart (because their value is positive), and skip any
      // ecotype specializing brick
      uint indexInFamily;
      BRICK_FAMILIES::TBrickFamily brickFamily =
          _BC->getBrickFamily(brickId, indexInFamily);
      ...
      if ((brickFamily == BRICK_FAMILIES::BHFPMA) && (phraseBricks.size() > 4))
        continue;
      if ((brickFamily == BRICK_FAMILIES::BHFEMA) &&
          ((maxIndexInExtractionFamily != 1) ||
           (phraseBricks.size() !=
            7))) // works only for default phrase where eco. spec. is at the end
        continue;
      ...
      if ((brickFamily >= BRICK_FAMILIES::BHFEEA) &&
          (brickFamily <= BRICK_FAMILIES::BHFEEC) &&
          (indexInFamily > maxIndexInExtractionFamily))
        maxIndexInExtractionFamily = indexInFamily;
    }
    if ((bType == BRICK_TYPE::FORAGE_EXTRACTION) &&
        (maxIndexInExtractionFamily == 1) && (phraseBricks.size() == 5))
...
// ***************************************************************************
bool CSabrinaCom::isMainDisplayIconInOverSlot(
    const NLMISC::CSheetId &brickId,
    bool &iconOver2NotSuitableForActionDisplay) const {
  uint indexInFamily;
  BRICK_FAMILIES::TBrickFamily brickFamily =
      _BC->getBrickFamily(brickId, indexInFamily);

  iconOver2NotSuitableForActionDisplay =
      ((brickFamily >= BRICK_FAMILIES::BeginForageProspection) &&
       (brickFamily <= BRICK_FAMILIES::EndForageExtraction));

  // True only for the 3 extraction bricks
  return ((brickFamily >= BRICK_FAMILIES::BHFEEA) &&
          (brickFamily <= BRICK_FAMILIES::BHFEEC));
}

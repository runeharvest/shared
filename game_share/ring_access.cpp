...
std::string CVerfiyRightRtScenarioError::toString() const {
  ...
  const char *typeTrad[] = {"None", "InvalidData", "InvalidIslandLevel",
                            "InvalidBotLevel"};
  TType type = Type;
  ...
  return NLMISC::toString(
      "Type: '%s' '%s'  : Package: '%s' needed: %d, user: %d)", typeStr.c_str(),
      Name.c_str(), Package.c_str(), Level, CharLevel);
}

CRingAccess::CRingAccess() { _Initialised = false; }
...
void CRingAccess::init() {
  ...
  for (uint32 i = 0; i <= 184; ++i) {
    _R2PlotItemSheetId.insert(
        CSheetId(NLMISC::toString("r2_plot_item_%d.sitem", i)));
  }
  ...
  std::string pathFileName =
      CPath::lookup(RingAccessFilename, false, false, false);
  // Open the file
  ...
  if (input.init(file)) {
    ...
    xmlNodePtr entityAccess =
        input.getFirstChildNode(entitiesAccess, "entityAccess");

    while (entityAccess != 0) {

      // island name
      CXMLAutoPtr namePtr(
          (const char *)xmlGetProp(entityAccess, (xmlChar *)"name"));
      CXMLAutoPtr packagePtr(
          (const char *)xmlGetProp(entityAccess, (xmlChar *)"package"));
      CXMLAutoPtr sheetClientPtr(
          (const char *)xmlGetProp(entityAccess, (xmlChar *)"sheetClient"));
      CXMLAutoPtr sheetPtr(
          (const char *)xmlGetProp(entityAccess, (xmlChar *)"sheetServer"));

      if (!namePtr.getDatas() || !packagePtr.getDatas() ||
          !sheetClientPtr.getDatas() || !sheetPtr.getDatas()) {
      ...
      if (sheet.empty()) {
        bool ok = _SheetIdToAccess
                      .insert(std::make_pair(
                          std::make_pair(sheetClientId, sheetId), package))
                      .second;
        if (!ok) {
          std::string previousPackage =
              _SheetIdToAccess[std::make_pair(sheetClientId, sheetId)];
          // only display warning if one key has multiple package
          if (previousPackage != package) {
            nlwarning("%s: Entity %s sheet(%s) is defined more than once with "
                      "different package definition. Previous definition is "
                      "'%s', current definition is '%s'",
                      RingAccessFilename.c_str(), namePtr.getDatas(),
                      sheetClientPtr.getDatas(), previousPackage.c_str(),
                      package.c_str());
          }
        }
      } else {
        ...
        if (_CustomNpcSheetId.find(sheetClientId) != _CustomNpcSheetId.end()) {
          ...
          if (!ok) {
            ...
            if (previousPackage != package) {
              nlwarning(
                  "%s: Entity %s sheet(%s) is defined more than once. Previous "
                  "definition is '%s', current definition is '%s'",
                  RingAccessFilename.c_str(), namePtr.getDatas(),
                  sheetPtr.getDatas(), previousPackage.c_str(),
                  package.c_str());
            }
          }
        } else {
          nlwarning("%s: Entity %s has invalid sheets %s %s",
                    RingAccessFilename.c_str(), namePtr.getDatas(),
                    sheetClientPtr.getDatas(), sheetPtr.getDatas());
        }
      }
...
bool CRingAccess::isSheetClient(const NLMISC::CSheetId &sheet) const {
  ...
  static const std::string racesStr[] = {"matis_", "fyros_", "tryker_",
                                         "zorai_"};
  ...
  if (sheetLength >= basicLength && sheetStr.substr(0, basicLength) == basic) {
    ...
    for (uint32 i = 0; i < 4; ++i) {
      ...
      if (sheetLength > basicLength + raceLength &&
          sheetStr.substr(basicLength, raceLength) == racesStr[i]) {

        // good thing is a basic_race_
        for (uint32 j = 0; j < 2; ++j) {
          uint32 sexLength = sexsLen[j];
          if (sheetLength > basicLength + raceLength + sexLength &&
              sheetStr.substr(basicLength + raceLength, sexLength) ==
                  sexsStr[j]) {
            return true;
          }
        }
...
std::string
CRingAccess::getSheetIdAccessInfo(const NLMISC::CSheetId &sheetClientId,
                                  const NLMISC::CSheetId &sheetServerId) const {
  ...
  TSheetIdToAccess::const_iterator found(
      _SheetIdToAccess.find(std::make_pair(sheetClientId, sheetServerId)));
  if (found == _SheetIdToAccess.end()) {
    return "";
  }
  return found->second;
}
...
std::string
CRingAccess::getSheetAccessInfo(const std::string &sheetClient,
                                const std::string &sheetServer) const {

  const_cast<R2::CRingAccess *>(this)->init(); // lazy initialization;

  NLMISC::CSheetId sheetClientId(sheetClient);
  NLMISC::CSheetId sheetServerId;
  if (!sheetServer.empty()) {
    sheetServerId = NLMISC::CSheetId(sheetServer);
  }
...
std::string
CRingAccess::getIslandAccessInfo(const std::string &islandeName) const {
...
void CRingAccess::getRingAccessAsMap(
    const std::string &ringAccess,
    std::map<std::string, int> &ringAccessAsMap) const {
...
std::string
CRingAccess::upgradeRingAccess(const std::string &defaultValue,
  ...
  {
    std::map<std::string, int>::const_iterator first(bonusMap.begin()),
        last(bonusMap.end());
    for (; first != last; ++first) {
      std::map<std::string, int>::iterator found =
          defaultMap.find(first->first);
  ...
  std::string toRet;
  {
    ...
    std::map<std::string, int>::const_iterator first(defaultMap.begin()),
        last(defaultMap.end());
    for (; first != last; ++first) {
      if (!toRet.empty()) {
        toRet += ":";
...
bool CRingAccess::verifyRight(const std::string &askedAccess,
                              const std::string &allowedAccess) const {
  ...
  return verifyRight(askedAccessMap, allowedAccessMap, package, neededLevel,
                     charLevel);
}

bool CRingAccess::verifyRight(
    const std::map<std::string, int> &askedAccessMap,
    const std::map<std::string, int> &allowedAccessMap, std::string &package,
    int &neededLevel, int &charLevel) const {
  std::map<std::string, int>::const_iterator first(askedAccessMap.begin()),
      last(askedAccessMap.end());
  for (; first != last; ++first) {
    std::map<std::string, int>::const_iterator found(
        allowedAccessMap.find(first->first));
    if (found == allowedAccessMap.end()) {
      package = first->first;
      neededLevel = first->second;
      charLevel = 0;
      return false;
    }
    if (found->second < first->second) {
...
bool CRingAccess::verifyRtScenario(CObject *rtScenario,
                                   const std::string &charRingAccess,
                                   CVerfiyRightRtScenarioError *&err) const {
  if (!rtScenario) {
    err = new CVerfiyRightRtScenarioError(
        CVerfiyRightRtScenarioError::InvalidData);
    return false;
  }
  ...
  for (; firstLocation != lastLocation; ++firstLocation) {
    ...
    if (!verifyRight(entityAccessMap, charRingAccessMap, package, neededLevel,
                     charLevel)) {
      err = new CVerfiyRightRtScenarioError(
          CVerfiyRightRtScenarioError::InvalidIslandLevel, islandName, package,
          neededLevel, charLevel);
      return false;
    }
  }
  ...
  for (; firstActIndex != lastActIndex; ++firstActIndex) {
    ...
    for (; firstNpcIndex != lastNpcIndex; ++firstNpcIndex) {
      ...
      if (npc && npc->isTable() && npc->isString("SheetClient") &&
          npc->isString("Sheet")) {
        std::string botName;
        ...
        getRingAccessAsMap(access, entityAccessMap);
        if (!verifyRight(entityAccessMap, charRingAccessMap, package,
                         neededLevel, charLevel)) {
          err = new CVerfiyRightRtScenarioError(
              CVerfiyRightRtScenarioError::InvalidBotLevel, botName, package,
              neededLevel, charLevel);
          return false;
        }
      }

...
public:
  ...
  public:
    ...
    CMessageField(TFieldType type, uint bitSize = 0)
  ...
  /**
   * The message format.
   * Each field might a bool, a sint32, an uint32, a bitsized uint, a float, a
   * double, a string, or an entity id. See also TFieldType enum and
   * CMessageField class for more information.
   */
  ...
  protected:
    ...
    CNodeId(CNode *node, NLMISC::CBitMemStream *stream)
        : _Node(node), _BitMemStream(stream), _CurrentField(0) {}
  ...
  public:
    ...
    /// Set stream (if not yet has), and return true if node id hadn't a stream
    /// yet
    ...
    template <typename T> bool serial(T &b) {
      bool valid = true, stream = true, field = true, stc = true;
      if (!(valid = isValid()) || !(stream = hasStream()) ||
          (field = (_CurrentField >= _Node->Format.size())) ||
          (stc = !serialTypeCheck(b))) {
        if (!valid)
          nlwarning(
              "Failed to serialize from CBitMemStream, node is not valid");
        if (!stream)
          nlwarning(
              "Failed to serialize from CBitMemStream, node has no stream");
        if (!field)
          nlwarning("Failed to serialize from CBitMemStream, no more fields to "
                    "serialize");
        if (!stc)
          nlwarning(
              "Failed to serialize from CBitMemStream, bad type provided");
        return false;
      }
      ++_CurrentField;
      return true;
...
#define DEFINE_SERIAL_TYPE_CHECK(stc__type, stc__typecheck)                    \
  bool serialTypeCheck(stc__type &b) {                                         \
    if (_Node->Format[_CurrentField].Type != stc__typecheck)                   \
      return false;                                                            \
...
public:
   ...
   * \param TMsgHeaderCallback callback : the callback to associate to the
   * message 'msgName'. \return bool : 'false' if the message does not exist.
   */
   ...
   */
  bool setUserData(const std::string &msgName, const uint64 &data,
                   uint index = 0);
   ...
   * \return bool : 'false' if the method cannot write the message Name into the
   * stream (probably because de message name is wrong).
   */
  bool pushNameToStream(const std::string &msgName,
                        NLMISC::CBitMemStream &strm);
   ...
   */
  void popNameAndDescriptionFromStream(std::string &resultName,
   ...
   */
  const std::string &getNodeDescription(CNodeId &nodeId) {
    return nodeId._Node->Description;
  }

  /**
   * Get the message node format
   */
  const TMessageFormat &getNodeFormat(CNodeId &nodeId) {
    return nodeId._Node->Format;
  }

  /**
   * Get the message node send to
   */
  const std::string &getNodeSendTo(CNodeId &nodeId) {
    return nodeId._Node->SendTo;
  }
   ...
   * Set node user data
   */
  void setNodeUserData(CNodeId &nodeId, const uint64 &data, uint index = 0) {
    nodeId._Node->UserData[index] = data;
  }

  /**
   * Set node user data
   */
  const uint64 &getNodeUserData(CNodeId &nodeId, uint index = 0) {
    return nodeId._Node->UserData[index];
  }
  ...
  public:
    ...
    CNode *select(const char *name) {
      ...
      for (;;) {
        ...
        it = node->NodesByName.find(
            nlsvf(NLMISC::CStringView(&name[subIdx], nameIdx - subIdx)));
        if (it == node->NodesByName.end()) {
          nlwarning("Couldn't select node '%s', not found in parent '%s'",
                    nlsvc(NLMISC::CStringView(&name[subIdx], nameIdx - subIdx)),
                    node->Name.c_str());
          return NULL;
        }
        node = (*it).second;
    ...
    CNode *select(const char *name, NLMISC::CBitMemStream &strm) {
      ...
      for (;;) {
        ...
        if (node->NbBits == 0) {
          nlwarning(
              "Couldn't select node '%s', parent '%s' has no bit per child",
              nlsvc(NLMISC::CStringView(&name[subIdx], nameIdx - subIdx)),
              node->Name.c_str());
          return 0;
        }
    ...
    CNode *select(NLMISC::CBitMemStream &strm) {
      ...
      while (node != NULL && node->NbBits != 0) {
        ...
        if (index >= node->Nodes.size()) {
          nlwarning("Couldn't select node from stream, invalid index %d in "
                    "parent '%s'",
                    index, node->Name.c_str());
          return NULL;
        }
    ...
    /// display nodes
    void xmlDisplay() {
      std::string ntype = (Name.empty()  ? "client_messages_description"
                           : NbBits == 0 ? "leaf"
                                         : "branch");

      nlinfo("<%s name=\"%s\" description=\"%s\">", ntype.c_str(), Name.c_str(),
             Description.c_str());

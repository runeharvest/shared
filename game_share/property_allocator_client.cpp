...
void CPropertyAllocatorClient::allocProperty(
  ...
  // TODO: check if simple property or STL container (special case with no
  // shared memory)
    ...
    if (!(propinfo.allocated() || propinfo.Pending)) {
      ...
      if (_LocalMSId != TServiceId(std::numeric_limits<uint16>::max())) {
        ...
        if ((options & PSOReadOnly) && (options & PSOWriteOnly)) {
          nlwarning("MIRROR: Property %s declared with both RO and WO options, "
                    "keeping RO",
                    propName.c_str());
          options &= ~PSOWriteOnly;
        }
        if ((options & PSOWriteOnly) && (options & PSONotifyChanges)) {
          nlwarning("MIRROR: Property %s declared with both WO and "
                    "notification, keeping notification",
                    propName.c_str());
          options &= ~PSOWriteOnly;
        }
        ...
        if ((!((options & PSOReadOnly) || (options && PSOWriteOnly))) ||
            (options &
             PSONotifyChanges)) // we don't expect to receive a prop trackers
                                // list if readonly && !notify
          propinfo.PropTrackersPending = true;

        // Set debug flags
        propinfo.setFlags(options, false);
        if (!notifyGroupByPropName.empty()) {
          TPropertiesInMirror::iterator ipimGroup =
              _PropertiesInMirror.find(notifyGroupByPropName);
          if (ipimGroup != _PropertiesInMirror.end()) {
            if (GET_PROPERTY_INFO(ipimGroup).DataSet != propinfo.DataSet) {
              nlwarning("MIRROR: The group property %s does not belong to the "
                        "same dataset as the declared property %s!",
                        notifyGroupByPropName.c_str(), propName.c_str());
            } else if (notifyGroupByPropName != propName) {
              propinfo.setFlagGroupNotifiedByOtherProp(
                  true, GET_PROPERTY_INFO(ipimGroup).PropertyIndex);
            }
          } else {
            nlwarning("MIRROR: Invalid group property name %s",
                      notifyGroupByPropName.c_str());
          }
        }
      }
    }
 ...
 */
void *
CPropertyAllocatorClient::accessPropertySegment(const std::string &propName,
                                                sint32 smid) {
...
/*
 * Access a shared property segment if this service did not subscribed to it
 * (return NULL if already accessed + warning)
 */
void *CPropertyAllocatorClient::accessOtherPropertySegment(sint32 smid) {
  for (TPropertiesInMirror::iterator ipim = _PropertiesInMirror.begin();
       ipim != _PropertiesInMirror.end(); ++ipim) {
    TPropertyInfo &propinfo = GET_PROPERTY_INFO(ipim);
...
void CPropertyAllocatorClient::serialOutMirrorInfo(NLNET::CMessage &msgout) {
  ...
  for (itr = _PropertiesInMirror.begin(); itr != _PropertiesInMirror.end();
       ++itr) {

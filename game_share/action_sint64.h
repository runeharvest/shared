...
public:
  ...
  void setNbBits(TPropIndex propIndex) {
    _NbBits = _PropertyToNbBit[propIndex];
#ifdef NL_DEBUG
    if (_NbBits == 0)
      nlwarning(
          "PropertyToNbBits[%hu] is null, check CNetworkConnection::init()",
          propIndex);
#endif
  }
   ...
   * BUT the property may be continuous, without using the benefits of
   * CContinuousAction (deltas).
   */
  ...
  void setAndPackValueNBits(const TValue &value, NLMISC::CBitMemStream &outMsg,
                            uint nbits) {
    _Value = value;

...
public:
  ...
  static bool isInstanceAllocated() { return Instance != NULL; }
   ...
   * \param code the code associate with this action. at the end, it should be
   * an huffman code (todo) \param creator a function that returns a new
   * instances of this registered class
   */
  void registerAction(uint32 code, CAction *(*creator)());

  // uint getNbBits( TProperty pr

  /** Create an instance of the action associated with the code. The allocation
   * will be done with a special memory management so you could *not* delete
   * this pointer. Use the factory function to delete them. \param code the code
   * of the action you want to instanciate \return a pointer to the new instance
   * of this code
   */
  CAction *create(TCLEntityId slot, TActionCode code);

  /** Create the action from a property code, fills property index and fill the
   * internal propindex if needed (it assumes the frontend and the client have
   * the same mapping property/propindex).
   */
  CAction *createByPropIndex(TCLEntityId slot, TPropIndex index);

  /** Pack an action to a bit stream.
   */
  void pack(CAction *action, NLMISC::CBitMemStream &message,
            NLMISC::TGameCycle currentCycle = 0);

  /* Pack an action to a bit stream, for server-->client, actioncode < 4 only
   */
  void packFast(
      CAction *action,
      NLMISC::CBitMemStream &message /*, NLMISC::TGameCycle currentCycle*/) {
    ...
    uint32 value = (1 << 14) | (action->Code << 12) | (timestampDelta << 8) |
    action->CLEntityId; message.serial( value, 15 );	*/
   ...
   * gamecycle.
   */
  void unpack(NLMISC::CBitMemStream &message, std::vector<CAction *> &actions,
              NLMISC::TGameCycle currentCycle = 0);

  /** Unpack an action from a bit stream. Set transmitTimestamp=true for
   * server-->client, false for client-->server. If true, set the current
   * gamecycle.
   */
  CAction *unpack(NLMISC::CBitMemStream &message,
                  NLMISC::TGameCycle currentCycle = 0);
...
private:
  /** The ctor is private because CActionFactory is a singelton and you can't
   * instanciate it, use getInstance() instead.
   */
  CActionFactory();

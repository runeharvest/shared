...
static bool isPhraseAvailable(const std::string &brickFilter,
...
bool canSellPhrase(uint32 rmfBitField, const std::string &sheetName) {
  ...
  if (rmfBitField & (1 << CaracteristicUpgrades)) {
    ...
    if (rmfBitField & (1 << FightActions)) {
      if (isPhraseAvailable("bppc", sheetName))
        return true;
      if (isPhraseAvailable("bppm", sheetName))
        return true;
      if (isPhraseAvailable("bpps", sheetName))
        return true;
      if (isPhraseAvailable("bppb", sheetName))
        return true;
    }
    if (rmfBitField & (1 << MagicActions)) {
      ...
      if (isPhraseAvailable("bppm", sheetName))
        return true;
      if (isPhraseAvailable("bppi", sheetName))
        return true;
      if (isPhraseAvailable("bppw", sheetName))
        return true;
    }
    if (rmfBitField & (1 << ForageActions)) {
      ...
      if (isPhraseAvailable("bppm", sheetName))
        return true;
      if (isPhraseAvailable("bppd", sheetName))
        return true;
      if (isPhraseAvailable("bppl", sheetName))
        return true;
    }
  ...
  if (rmfBitField & (1 << SpecialPowers)) {
    ...
    if (rmfBitField & (1 << FightActions)) {
      if (isPhraseAvailable("bsf", sheetName))
        return true;
      testGeneric = true;
    }
    if (rmfBitField & (1 << MagicActions)) {
      if (isPhraseAvailable("bsm", sheetName))
        return true;
      testGeneric = true;
    }
    if (rmfBitField & (1 << ForageActions)) {
      if (isPhraseAvailable("bsg", sheetName))
        return true;
      testGeneric = true;
    }
    if (rmfBitField & (1 << CraftActions)) {
    ...
    // test generic
    if (testGeneric) {
      if (isPhraseAvailable("bsx", sheetName))
        return true;
      if (isPhraseAvailable("bsd", sheetName))
        return true;
    }
  }
  ...
  // *** Test Normal Action
  if (rmfBitField & (1 << FightActions)) {
    if (isPhraseAvailable("bf", sheetName))
      return true;
  }
  if (rmfBitField & (1 << MagicActions)) {
    if (isPhraseAvailable("bm", sheetName))
      return true;
  }
  if (rmfBitField & (1 << ForageActions)) {
    if (isPhraseAvailable("bhf", sheetName))
      return true;
  }
  if (rmfBitField & (1 << CraftActions)) {
    if (isPhraseAvailable("bc", sheetName))
      return true;
  }

...
/** A list of targets, which can be packed/unpacked in visual properties (see
 * PROPERTY_TARGET_LIST_x in entity_types.h)
 *
...
public:
  ...
  public:
    ...
    uint8 Info; // Damage shield for melee  5:3 format (damage shield io : power
                // ) Distance for range attacks (that do not have damage shield)
                // format is 7:1
  public:
    CTarget(uint8 slot = CLFECOMMON::INVALID_SLOT, bool resist = false,
            uint8 dist = 0)
        : TargetSlot(slot) {
...
public:
   ...
   * caller must provide enough room to store the result (assertion is raised
   * otherwise)
   */
...
inline uint16 CMultiTarget::CTarget::getPacked() const {
  // return (uint16) TargetSlot | ((uint16) (Resist ? 1 : 0) << 15) | ((uint16)
  // Distance << 8);

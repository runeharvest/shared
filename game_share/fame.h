 ...
 *			1 - change the mirror description file
 *data_share/mirror_sheet/fame.dataset to add the additionnal entry (or remove
 *if you reduce the value) 2 - change the excel worksheet
 *\\server\leveldesign\static_fame.xls :
 *						* resize the sheet (on x and y
 *axis, the sheet must remain square),
 *						* update the macro
 *Module1.ExportFame ligne 6 : Range("A1:CV100").Select : update the range to
 *the new worksheet size
 */
 ...
 *	Static fames values include tribe to tribe and tribe to civilisation
 *fames.
 */
class CStaticFames {
public:
  ...
  public:
    bool getCultThresholdForCiv(PVP_CLAN::TPVPClan civ, sint32 &kami,
                                sint32 &karavan, sint32 &neutral) const {
  ...
  const std::vector<NLMISC::TStringId> &getFactionNames() {
  ...
  /// Convert the mapped name of a faction into an indexed value (allowing very
  /// fast access).
  ...
  /** Get the fame value between two factions. There is no constraint on faction
   *order because fame values are reciprocal.
   */
  ...
  sint32 getStaticFame(const std::string &faction1,
  ...
  /** Get the propagation factor value between two factions. There is no
   *constraint on faction order because values are reciprocal.
   */
  float getPropagationFactorIndexed(uint factionIndex1,
  ...
  uint getDatabaseIndex(uint fameIndex) {
    nlassert(_FameIndexToDatabaseIndex.size() > fameIndex);
    return _FameIndexToDatabaseIndex[fameIndex];
  }
  ...
  const std::vector<CTribeCultThresholdPerCiv> &
  getTribeThresholdVector() const {
    return _TribeCultThresholdPerCiv;
  }
...
private:
  ...
  typedef CHashMap<NLMISC::TStringId, uint, NLMISC::CStringIdHashMapTraits>
      TFactionNameIndexList;
  ...
  typedef CHashMap<NLMISC::CSheetId, uint, NLMISC::CSheetIdHashMapTraits>
      TFactionNameSheetList;
...
public:
  ...
  void setFameDataSet(CMirroredDataSet *fameDataSet,
                      bool declareProperty = true);
   ...
   */
  void createFameOwner(CMirroredDataSet &dataSet,
                       const TDataSetRow &entityRow) {
    nlassertex(
        _FameDataSet,
        ("You must set the fame dataset pointer before calling this function"));
   ...
   *	This method is called from the service mirror notification callback.
   */
  void removeFameOwner(CMirroredDataSet & /* dataSet */,
                       const TDataSetRow &entityRow) {
    nlassertex(
        _FameDataSet,
        ("You must set the fame dataset pointer before calling this function"));
  ...
  //@{
  //@name Fame querying
  /** Get the fame of a civilisation, guild or player versus a tribe or
   *civilisation The name is a mapped name and is the same as in CStaticFame
   *class.
   */
  sint32 getFame(const NLMISC::CEntityId &entityId, NLMISC::TStringId faction,
                 bool modulated = false, bool returnUnknowValue = false);
  /** Get the fame of a civilisation, guild or player versus a tribe or
   *civilisation The name is a mapped name and is the same as in CStaticFame
   *class.
   */
  sint32 getFameIndexed(const NLMISC::CEntityId &entityId, uint32 factionIndex,
                        bool modulated = false, bool returnUnknowValue = false);
  //@}

  //@{
  //@name Fame modification
  /** Add some fame value for a civilisation, a guild or a player vs a tribe or
   *civilisation. The value can positive or negative. Fame modifications are not
   *immediately received because it's the EGS that apply the value.
   */
  void addFame(const NLMISC::CEntityId &entityId, NLMISC::TStringId faction,
               sint32 deltaFame, bool propagate = false);
  /** Add some fame value for a civilisation, a guild or a player vs a tribe or
   *civilisation. The value can positive or negative. Fame modifications are not
   *immediately received because it's the EGS that apply the value.
   */
  void addFameIndexed(const NLMISC::CEntityId &entityId, uint32 factionIndex,
                      sint32 deltaFame, bool propagate = false);
  /** Set the value of the fame.
   */
  // void	setFame(const NLMISC::CEntityId &entityId, NLMISC::TStringId
  // faction, sint32 fameValue);
  //@}
  ...
  /** Interface class designed to implemented and registered in the
   *FameInterface by the EGS services. This class directly receive fame delta
   *from addFame. You must not use this feature with more than one service !
   *(only the EGS actualy).
   */
  class IFameOverload {
  public:
     ...
     */
    virtual void addFameIndexed(const NLMISC::CEntityId &entityId,
                                uint factionIndex, sint32 deltaFame,
                                bool propagate) = 0;
    /** Get the fame of a civilisation, guild or player versus a tribe or
     *civilisation The name is a mapped name and is the same as in CStaticFame
     *class.
     */
    virtual sint32 getFameIndexed(const NLMISC::CEntityId &entityId,
                                  uint32 factionIndex, bool modulated = true,
                                  bool returnUnknownValue = false) = 0;
    virtual const TDataSetRow &
    getCivilisationIndex(const NLMISC::CEntityId &entityId) = 0;
    virtual const TDataSetRow &
    getGuildIndex(const NLMISC::CEntityId &entityId) = 0;
    virtual const TDataSetRow &
    getFameMemoryIndex(const NLMISC::CEntityId &entityId) = 0;
  };
  /// Register the fame adder interface.
  void registerFameOverload(IFameOverload *fameOverload) {
    _FameOverload = fameOverload;
  }
...
private:
  CFameInterface() : _FameDataSet(NULL), _FameOverload(NULL) {}

  // fame data for civilisation, guild or player entity
  struct TFameOwner {
    /// Civilisation index inside the fame dataset (valid for player and guild
    /// entity)
    ...
    TFameOwner(CMirroredDataSet &dataSet, const TDataSetRow &entityRow) {
      // WARNING : you must initialise your property in the same order as they
      // are declared in the dataset
  ...
  typedef CHashMap<TDataSetRow, TFameOwner *, TDataSetRow::CHashCode>
      TFameContainer;

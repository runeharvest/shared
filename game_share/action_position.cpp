...
void CActionPosition::pack(NLMISC::CBitMemStream &message) {
  ...
  // scale precision from 1 mm to 16 mm and take only 16 lower bits (=> 1048 m
  // range)
  uint32 pxy16;
  ...
  if (Interior)
    posz16 |= (uint16)0x2;
  // nlinfo( "Slot% hu: Pos: %d %d %d, Pos16: %hu %hu %hu, Date %u",
  // (uint16)CLEntityId, (sint32)Position[0], (sint32)Position[1],
  // (sint32)Position[2], posx, posy, poszr, GameCycle );
...
uint32 CActionPosition::priority() { return 1; }
...
uint		CActionPosition::_MaxDateDelta = (1 << DefaultDateBitPack) -
1;*/

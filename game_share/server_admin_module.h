 ...
 *
 * Management of features as package upload features needed by users. Features
 * that are the same for two adventures are only store once. Enable search upon
 * adventures. (Database) Manage right upon users (a users of level X that pays
 * Y is allowed to use sheet A, B, C and put N elements). Code to allow / refuse
 * map creation, map edition, go Live .... Store path of rt-data and et-data.
...
public:
  ...
  */
  ...
  NLMISC_COMMAND_HANDLER_ADD(CServerAdminModule, displayIslands,
                             "display islands", "no args")

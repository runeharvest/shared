...
#define WRITE(type, what)                                                              \
	{                                                                                  \
		BOMB_IF(offset + sizeof(type) > bufferSize, "Buffer overflow!", return false); \
...
#define READ(type, what)                                                                                                            \
	{                                                                                                                               \
		DROP_IF(offset + sizeof(type) > bufferSize, "PDR ERROR: Buffer overflow reading: " #type " " #what, clear(); return false); \
		what = *(const type *)&src[offset];                                                                                         \
		offset += sizeof(type);                                                                                                     \
	}
#define READBUF(type, count, what)                                                                                                       \
	{                                                                                                                                    \
		DROP_IF(offset + sizeof(type) * count > bufferSize, "PDR ERROR: Buffer overflow reading buffer: " #what, clear(); return false); \
		if (count > 0)                                                                                                                   \
		{                                                                                                                                \

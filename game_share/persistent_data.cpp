...
/*
  ...
  Strings are limited to 256 characters - this is OK for the basic uses of the
  system - may require review later...
  - Note that for many apps use of a string vector in place of a string will
  solve the 255 limit.

  => deprecated: the test checking the string length has been disabled, which
  allows string to exceed a 256 char length
*/
...
uint16 CPersistentDataRecord::addString(const string &name) {
  ...
  // store the length of the input string for speed of access (just to help the
  // optimiser do its job)
  uint32 len = (uint32)name.size();

  // Disabled to allow >=256 char strings.
  // DROP_IF(len>=256,"Attempt to add a string of > 256 characters to the string
  // table",return 0);

  // depending on the string length choose a well suited algorithm for
  // performing a fast search of the string table
  switch (len) {
  case 0:
  ...
  case 8: {
    ...
    uint32 cLast = *(uint32 *)&name[endOffs]; // last 4 chars of name (touch or
                                              // overlap with first 4 chars)

    // run through the existing strings looking for a match
    for (uint32 i = (uint32)_StringTable.size(); i--;) {
      const string &s = _StringTable[i];
      if (s.size() == len && (*(uint32 *)&s[0] == cFirst) &&
          (*(uint32 *)&s[endOffs] == cLast))
        return (uint16)i;
    }
  } break;
  ...
  case 12: {
    uint32 cFirst = *(uint32 *)&name[0]; // first 4 chars of name
    uint32 cMid =
        *(uint32 *)&name[4];  // middle 4 chars of name (touch first 4 chars)
    uint32 endOffs = len - 4; // offset to last 4 characters of the name
    uint32 cLast = *(uint32 *)&name[endOffs]; // last 4 chars of name (touch or
                                              // overlap with middle 4 chars)

    // run through the existing strings looking for a match
    for (uint32 i = (uint32)_StringTable.size(); i--;) {
      const string &s = _StringTable[i];
      if (s.size() == len && (*(uint32 *)&s[0] == cFirst) &&
          (*(uint32 *)&s[endOffs] == cLast) && (*(uint32 *)&s[4] == cMid))
        return (uint16)i;
    }
  } break;
  ...
  case 16: {
    uint32 cFirst = *(uint32 *)&name[0]; // first 4 chars of name
    uint32 cSecond =
        *(uint32 *)&name[4]; // second 4 chars of name (touch first 4 chars)
    uint32 cThird =
        *(uint32 *)&name[8];  // third 4 chars of name (touch second 4 chars)
    uint32 endOffs = len - 4; // offset to last 4 characters of the name
    uint32 cLast = *(uint32 *)&name[endOffs]; // last 4 chars of name (touch or
                                              // overlap with third 4 chars)

    // run through the existing strings looking for a match
    for (uint32 i = (uint32)_StringTable.size(); i--;) {
      const string &s = _StringTable[i];
      if (s.size() == len && (*(uint32 *)&s[0] == cFirst) &&
          (*(uint32 *)&s[endOffs] == cLast) && (*(uint32 *)&s[4] == cSecond) &&
          (*(uint32 *)&s[8] == cThird))
        return (uint16)i;
    }
  } break;
  ...
  default: {
    ...
    uint32 endOffs = len - 4; // offset to last 4 characters of the name
    uint32 *nameEnd =
        (uint32 *)&name[endOffs]; // pointer to the last 4 chars of name
    uint32 cLast =
        *nameEnd; // last 4 chars of name (touch or overlap with middle 4 chars)

    // run through the existing strings looking for a match
    for (uint32 i = 0; i < _StringTable.size(); ++i) {
      // store a ref to the next string in the string table just to help
      // optimiser do its job
      const string &s = _StringTable[i];

      // if string lengths or first r last dwords  don't match then abort
      // compare
      if (s.size() != len || (*(uint32 *)&s[0] != cFirst) ||
          (*(uint32 *)&s[endOffs] != cLast))
        continue;

      uint32 *sIt = (uint32 *)&s[4]; // iterator for 's' - init to point to
                                     // second dword of string
  ...
  // no match found so add this string to the string table and return its index
  {
    ...
    BOMB_IF(result == std::numeric_limits<uint16>::max(),
            "No more room in string table!!!", _StringTable.pop_back());
    return result;
  }
}

const NLMISC::CSString &CPersistentDataRecord::lookupString(uint32 idx) const {
  // note that the string table size is never less than 1 as entry 0 is pre-set
  // with the 'invalid string' value
  BOMB_IF(idx >= _StringTable.size(),
          "Attempting to access past end of string table",
          return lookupString(0));
...
void CPersistentDataRecord::skipStruct() {
  DROP_IF(!isStartOfStruct(),
          "Attempting to skip a struct whereas next token is not a struct",
          return);
  skipData();
}
...
bool CPersistentDataRecord::operator==(
    const CPersistentDataRecord &other) const {
#define RESTORE_STATE_VARS                                                     \
  {                                                                            \
    _ArgOffset = oldArgOffset;                                                 \
    _TokenOffset = oldTokenOffset;                                             \
    _ReadingStructStack = oldRSS;                                              \
    other._ArgOffset = otherOldArgOffset;                                      \
    other._TokenOffset = otherOldTokenOffset;                                  \
    other._ReadingStructStack = otherOldRSS;                                   \
  }
#define RETURN_FALSE                                                           \
  { RESTORE_STATE_VARS return false; }
#define RETURN_TRUE                                                            \
  { RESTORE_STATE_VARS return true; }
  ...
  while (!isEndOfData() && !other.isEndOfData()) {
    ...
    if (isStartOfStruct()) {
      ...
      const_cast<CPersistentDataRecord *>(this)->popStructBegin(
          peekNextToken());
      const_cast<CPersistentDataRecord &>(other).popStructBegin(
          other.peekNextToken());
    } else if (isEndOfStruct()) {
      // skip end of struct token
      const_cast<CPersistentDataRecord *>(this)->popStructEnd(peekNextToken());
      const_cast<CPersistentDataRecord &>(other).popStructEnd(
          other.peekNextToken());
  ...
  // all of the failure tests passed so we can conclude that our structures
  // match
  RETURN_TRUE
...
NLMISC::CSString CPersistentDataRecord::getInfo() const {
  ...
  return NLMISC::toString("TotalSize=%u TokenCount=%u DataCount=%u "
                          "StringCount=%u StringSize=%u ValueCount=%u",
                          totalDataSize(), _TokenTable.size(), _ArgTable.size(),
                          _StringTable.size(), stringDataSize(),
                          getNumValues());
}
...
NLMISC::CSString CPersistentDataRecord::getInfoAsCSV() const {
  ...
  return NLMISC::toString(
      "%u,%u,%u,%u,%u,%u", totalDataSize(), _TokenTable.size(),
      _ArgTable.size(), _StringTable.size(), stringDataSize(), getNumValues());
}

const NLMISC::CSString &CPersistentDataRecord::getCSVHeaderLine() {
  static NLMISC::CSString headerLine =
      "TotalSize,TokenCount,DataCount,StringCount,StringSize,ValueCount";
  return headerLine;
}
...
uint32 CPersistentDataRecord::totalDataSize() const {
  ...
  result +=
      sizeof(uint32); // sizeof 'number of tokens in the token table' variable
  result += sizeof(uint32); // sizeof 'number of args in the arg table' variable
  result +=
      sizeof(uint32); // sizeof 'number of strings in the string table' variable
  result += sizeof(uint32); // sizeof 'string table data size' variable
  result +=
      (uint32)_TokenTable.size() * sizeof(TToken); // sizeof the token data
  result +=
      (uint32)_ArgTable.size() * sizeof(_ArgTable[0]); // size of the args data
  result +=
      stringDataSize(); // the data size for the strings in the string table
...
uint32 CPersistentDataRecord::stringDataSize() const {
  uint32 result = 0;
  for (uint32 i = 0; i < _StringTable.size(); ++i)
    result += (uint32)_StringTable[i].size() +
              1; // the data size for the strings in the string table
  return result;
}
...
#define WRITE(type, what)                                                      \
  {                                                                            \
    type v = (type)(what);                                                     \
    dest.serial(v);                                                            \
  }
#define WRITE_BUFF(type, what)                                                 \
  dest.serialBuffer((uint8 *)&what[0], sizeof(type) * (uint)what.size())

  // mark the amount of data in output stream before we start adding pdr
  // contents
  ...
  // mark the amount of data in output stream before we start adding string
  // table
  ...
  // make sure the info written to the header corresponds to the reality of data
  // written to file
  BOMB_IF(dest.getPos() - stringTableStart != stringDataSize(),
          "Error writing pdr string table to output stream", return false);
  BOMB_IF(dest.getPos() - dataStart != totalDataSize(),
          "Error writing pdr to output stream", return false);
...
bool CPersistentDataRecord::toBuffer(char *dest, uint32 bufferSize) {
  ...
  BOMB_IF(bufferSize < totalDataSize(), "Buffer too small to write data to",
          return false);

  uint32 offset = 0;
#define WRITE(type, what)                                                      \
  {                                                                            \
    BOMB_IF(offset + sizeof(type) > bufferSize, "Buffer overflow!",            \
            return false);                                                     \
    *(type *)&dest[offset] = what;                                             \
    offset += sizeof(type);                                                    \
  }
...
#undef WRITE
  ...
  BOMB_IF(offset != totalDataSize(),
          "Buffer size calculation doesn't match with data written",
          return false);
  return true;
}

bool CPersistentDataRecord::toString(std::string &result,
  ...
  case XML_STRING:
    return toXML(result);
  case LINES_STRING:
    return toLines(reinterpret_cast<CSString &>(result));
  }
...
bool CPersistentDataRecord::toXML(std::string &result) {
  ...
  while (!isEndOfData()) {
    if (isStartOfStruct()) {
      ...
      for (uint32 i = 0; i <= _ReadingStructStack.size(); ++i)
        result += '\t';
      result += "<";
      ...
      for (uint32 i = 0; i <= _ReadingStructStack.size() - 1; ++i)
        result += '\t';
      result += "</";
      result += lookupString(peekNextToken());
      result += ">\n";
...
bool CPersistentDataRecord::toLines(std::string &result) {
  ...
  return pdt.readFromPdr(*this) &&
         pdt.writeToBuffer(reinterpret_cast<NLMISC::CSString &>(result));
}
...
bool CPersistentDataRecord::writeToBinFile(const std::string &fileName) {
  ...
  DROP_IF(!open,
          NLMISC::toString("Failed to open output file %s", fileName.c_str())
              .c_str(),
          return false);
  ...
  try {
    ...
    DROP(NLMISC::toString("Failed to write output file: %s", fileName.c_str()),
         return false);
  }
...
bool CPersistentDataRecord::writeToTxtFile(const std::string &fileName,
                                           TStringFormat stringFormat) {
...
bool CPersistentDataRecord::writeToFile(const std::string &fileName,
  ...
  case ANY_FILE: {
    if (CSString(fileName).right(4) == ".xml")
      goto xml_file;
    if (CSString(fileName).right(4) == ".txt")
      goto lines_file;
    goto binary_file;
  }
  }
  BOMB("Bad file type supplied to writeToFile() - file not saved: " << fileName,
       return false);
}
...
bool CPersistentDataRecord::fromBuffer(const char *src, uint32 bufferSize) {
  ...
  // the second dword of a bin buffer contains the buffer length so use it to
  // check whether this buffer looks like a correct binary buffer.
  ...
  DROP_IF(!isValidBinary, "Failed to parse buffer due to invalid header",
          return false);

  // make sure the persistent data record is cleared out before we fill it with
  // data
  clear();
  // Must clear the string table, because read from file (and clear() init it
  // with token family)
  _StringTable.clear();

  uint32 offset = 0;
#define READ(type, what)                                                       \
  {                                                                            \
    DROP_IF(offset + sizeof(type) > bufferSize,                                \
            "PDR ERROR: Buffer overflow reading: " #type " " #what, clear();   \
            return false);                                                     \
    what = *(const type *)&src[offset];                                        \
    offset += sizeof(type);                                                    \
  }
#define READBUF(type, count, what)                                             \
  {                                                                            \
    DROP_IF(offset + sizeof(type) * count > bufferSize,                        \
            "PDR ERROR: Buffer overflow reading buffer: " #what, clear();      \
            return false);                                                     \
    if (count > 0) {                                                           \
      memcpy(what, &src[offset], sizeof(type) * count);                        \
  ...
  uint32 tokenCount;
  READ(uint32, tokenCount);
  uint32 argCount;
  READ(uint32, argCount);
  uint32 stringCount;
  READ(uint32, stringCount);
  uint32 stringsSize;
  READ(uint32, stringsSize);

  DROP_IF(version > 0, "PDR ERROR: Wrong file format version!", clear();
          return false);
  DROP_IF(totalSize != bufferSize, "PDR ERROR: Invalid source data", clear();
          return false);
  DROP_IF(totalSize != offset + tokenCount * sizeof(TToken) +
                           argCount * sizeof(uint32) + stringsSize,
          "PDR ERROR: Invalid source data", clear();
          return false);
  ...
  DROP_IF((stringsSize == 0) != (stringCount == 0),
          "PDR ERROR: Invalid string table parameters", clear();
          return false);
  ...
  if (stringCount != 0) {
    ...
    DROP_IF(stringDataEnd[-1] != 0,
            "PDR ERROR: Last string table entry isn't zero terminated", clear();
            return false);
    ...
    do {
      ...
      DROP_IF((stringTableIt != stringTableEnd) !=
                  (stringDataIt != stringDataEnd),
              "PDR ERROR: Invalid string table", clear();
              return false);
    } while (stringDataIt != stringDataEnd);

    DROP_IF(stringTableIt != stringTableEnd,
            "PDR ERROR: Too few strings found in string table", clear();
            return false);
    offset += stringsSize;
  }
...
bool CPersistentDataRecord::fromXML(const std::string &s) {
  ...
  // we need to treat our input data as a CSString - we static cast because
  // CSStrings are not supposed to contain any data of their own - they are just
  // a fonctionality wrapper round a std::string
  ...
  // we have a buffer of xml-like blocks so we're going to start by chunking it
  // up (from the back)
  ...
  for (uint32 i = len; i--;) {
    ...
    case '>':
      DROP_IF(clauseOpen == true,
              calculateLineAndColumn(buff, i) +
                  "Found 2 '>'s with no matching '<'",
              return false);
      clauseOpen = true;
      clauseEnd = i;
      break;
    ...
    default:
      DROP_IF((uint8)(buff[i]) < 32,
              calculateLineAndColumn(buff, i) +
                  NLMISC::toString(
                      "Invalid (non-ascii text) character encountered: %i",
                      buff[i]),
              return false);
      break;
    }
  ...
  DROP_IF(clauses.size() < 2 || clauses[0] != "/xml" || clauses.back() != "xml",
          "Invalid data file - didn't find <xml>..</xml> structure",
          return false)
  // run through the set of clauses to add them to the data block...
  for (uint32 i = (uint32)clauses.size() - 1; --i;) {
    // clauses are of four types: <...>=struct_begin  </..>=struct_end,
    // <../>=prop, <!..>=comment
    if (clauses[i].left(1) == "!" || clauses[i].left(1) == "?") {
      ...
      DROP_IF(keyword0 != "type" || keyword1 != "value",
              "Expecting 'type' and 'value' in property - but not found",
              continue);
...
bool CPersistentDataRecord::fromLines(const std::string &inputBuffer) {
  ...
  // setup a persistent data tree and have it scan our input buffer
  CPersistentDataTree pdt;
  return pdt.readFromBuffer(
             reinterpret_cast<const NLMISC::CSString &>(inputBuffer)) &&
         pdt.writeToPdr(*this);
}
...
bool CPersistentDataRecord::fromStream(NLMISC::IStream &stream, uint32 size) {
  ...
  if (isBinary)
    return fromBuffer(&buff[0], size);

  // it's not a valid binary file so see whether it looks like a valid text file
  DROP_IF(!buff.isValidText(),
          "File is binary but 'file size' header entry doesn't match true file "
          "size",
          return false);
...
bool CPersistentDataRecord::fromBuffer(NLMISC::IStream &stream) {
  ...
  if (memStream != NULL) {
    return fromBuffer((const char *)(memStream->buffer() + memStream->getPos()),
                      memStream->length() - memStream->getPos());
  }
  // try with a IFile
...
bool CPersistentDataRecord::readFromFile(const std::string &fileName) {
  ...
  fclose(inf);
  DROP_IF(blocksRead != 1, "Failed to read data from file " << fileName,
          return false);
...
bool CPersistentDataRecord::readFromBinFile(const std::string &fileName) {
  ...
  bool open = f.open(fileName);
  DROP_IF(!open,
          NLMISC::toString("Failed to open input file %s", fileName.c_str()),
          return false)
  ...
  try {
    ...
    DROP(NLMISC::toString("Failed to read input file: %s", fileName.c_str()),
         return false);
  }
...
CPDRLookupTbl::TEnumValue CPDRLookupTbl::operator[](uint32 idx) const {
  BOMB_IF(idx >= _TheTbl.size(),
          "ERROR: Failed to retrieve entry from PDR lookup table (idx is out "
          "of bounds) - pdr must be corrupt",
          return -1);
  return _TheTbl[idx];
}
...
uint16 CPdrTokenRegistry::addToken(const std::string &family,
  ...
  for (uint16 i = 0; i < stringTable.size(); ++i) {
    if (stringTable[i] == token)
      return i;
    BOMB_IF(i >= 8191,
            "Failed to add more then 8192 static token to a pdr string table",
            return 0);
  }
...
const CPersistentDataRecord::TStringTable &
CPdrTokenRegistry::getStringTable(const std::string &family) {
  // return the map entry correponding to 'family' (creating a new one if need
  // be)

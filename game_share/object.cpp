...
public:
  ...
  ~CTableIntegrityChecker() { _Table.checkIntegrity(); }
...
void CObject::visit(IObjectVisitor &visitor) {
  ...
  // must work on a copy here, because the 'visit' method may change the current
  // list of sons of this table Example of a scenario where this happened :
  // - Create a bandit camp
  // - onPostCreate (1) is called (see in client : CInstance::onPostCreate)
  // - create ghost bandit as sons of current bandit camps, their 'onPostCreate'
  // method is called
  // - returns to previous onPostCreate (1) -> the list of sons has changed
  //   -> as a result, onPostCreated was called twice on instances that were
  //   just created, leading to a crash (the onPostCreate method registered
...
void CObject::copyMismatchMsg(const CObject &src) {
  ...
  nlwarning("Can't copy object of type %s into object of type %s",
            src.getTypeAsString(), this->getTypeAsString());
}
...
sint32 CObject::findIndex(const std::string & /* key */) const {
  ...
  BOMB("Try to use the method findIndex() on a object that is not an "
       "CObjectTable",
       return 0);
  return 0;
}
  ...
  if (!prop.empty()) {
    ...
    if (!attr) {
      BOMB(NLMISC::toString(
               "Try to access to the property '%s' that does not exist.",
               prop.c_str()),
           return "");
      return "";
    }
...
CObject *CObject::take(sint32 /* position */) {
  ...
  BOMB("Try to use the take function on an object that is not a table",
       return 0);
  return 0;
}
...
CObject *CObject::getValueAtPos(uint32 /* pos */) const {
  BOMB("Try to call the function getValueAtPos() on an object that is not a "
       "table",
       return 0);
  return 0;
}

uint32 CObject::getSize() const {
  BOMB("Try to call the function getSize() on an object that is not a table",
       return 0);
  return 0;
}

CObject *CObject::clone() const {
  BOMB("Try to call the function clone() on an object that is not a table",
       return 0);
  return 0;
}
...
bool CObject::set(const std::string & /* key */,
                  const std::string & /* value */) {
  BOMB("Try to set the value of an object with a string on an object that does "
       "not allowed it",
       return false);
  return false;
}
...
bool CObject::set(const std::string & /* key */, double /* value */) {
  BOMB("Try to set the value of an object with a double on an object that does "
       "not allowed it",
       return false);
  return false;
}
...
bool CObject::set(const std::string & /* key */, sint64 /* value */) {
  BOMB("Try to set the value of an object with an integer on an object that "
       "does not allowed it",
       return false);
  return false;
}
...
bool CObject::setObject(const std::string & /* key */, CObject * /* value */) {
  BOMB("Try to set the value of an object with an object that does not allowed "
       "it",
       return false);
  return false;
}
...
bool CObject::insert(const std::string & /* key */, CObject * /* value */,
                     sint32 /* position */) {
  // H_AUTO(R2_CObject_insert)
  BOMB("Try to call the function insert() on an object that is not a table",
       return false);
  return false;
}
...
CObject *CObject::findAttr(const std::string &first,
                           const std::string &second) const {
  // H_AUTO(R2_CObject_findAttr)
  CObject *ret = getAttr(first);
  if (ret) {
    ret = ret->getAttr(second);
  }
  return ret;
}

CObject *CObject::findAttr(const std::string &first, const std::string &second,
...
CObject *CObject::findAttr(const std::vector<std::string> &attrNames) const {
  // H_AUTO(R2_CObject_findAttr)
  std::vector<std::string>::const_iterator first(attrNames.begin()),
      last(attrNames.end());
...
sint32 CObject::findIndex(const CObject * /* child */) const {
  // H_AUTO(R2_CObject_findIndex)
  BOMB("Try to call the function findIndex() on an object that is not a table",
       return 0);
  return 0;
}

bool CObject::getShortestName(std::string &instanceId, std::string &attrName,
...
bool CObject::getNameInParent(std::string &instanceId, std::string &attrName,
                              sint32 &position) const {
...
CObjectString::CObjectString(const std::string &value)
    : CObject(), _Value(value) {}
...
void CObjectString::doSerialize(std::string &out,
                                CSerializeContext & /* context */) const {
...
bool CObjectString::set(const std::string &key, const std::string &value) {
  ...
  BOMB_IF(!key.empty(),
          "Try to set the a sub value of an object that does not allowed it",
          return false);
  _Value = value;
  return true;
}
...
bool CObjectString::setObject(const std::string &key, CObject *value) {
  ...
  BOMB_IF(!key.empty() ||
              !(value->isString() || value->isNumber() || value->isInteger()),
          "Try to set the a sub value of an object that does not allowed it",
          return false);
...
void CObjectRefId::doSerialize(std::string &out,
                               CSerializeContext & /* context */) const {
...
const char *CObjectNumber::getTypeAsString() const { return "Number"; }
...
std::string CObjectNumber::doToString() const {
  return m_IsInteger ? NLMISC::toString(m_Value.Integer)
                     : NLMISC::toString("%lf", m_Value.Number);
}

void CObjectNumber::doSerialize(std::string &out,
                                CSerializeContext & /* context */) const {
  ...
  if (doIsInteger()) {
    ...
    std::string value = NLMISC::toString(
        double(sint64(num * 1000.0 + (num >= 0.0 ? 0.5 : -0.5))) / 1000.0);
...
bool CObjectNumber::set(const std::string &key, sint64 value) {
  ...
  BOMB_IF(!key.empty(),
          "Try to set an element of a table on an object that is not a table",
          return false);
...
CObject *CObjectNumber::clone() const {
  ...
  CObjectNumber *result = m_IsInteger ? new CObjectNumber(m_Value.Integer)
                                      : new CObjectNumber(m_Value.Number);
  result->setGhost(getGhost());
  return result;
}

bool CObjectNumber::doIsNumber() const {
  return !m_IsInteger || (sint64)getNumberValue() == m_Value.Integer;
}

double CObjectNumber::doToNumber() const { return getNumberValue(); }

bool CObjectNumber::doIsInteger() const {
  return m_IsInteger || (double)getIntegerValue() == m_Value.Number;
}
...
bool CObjectNumber::setObject(const std::string & /* key */, CObject *value) {
  ...
  BOMB_IF(!value->isNumber() && !value->isInteger(),
          NLMISC::toString("Try to set an element of a type '%s' with  a value "
                           "of type '%s' on an object that is not a number",
                           this->getTypeAsString(), value->getTypeAsString()),
          return false);
...
bool CObjectNumber::equal(const CObject *other) const {
  ...
  // if difference between 2 values less than epsilon, we consider they are
  // equals
...
void CObjectTable::inPlaceCopy(const CObjectTable &src) {
  ...
  for (uint k = 0; k < src.getSize(); ++k) {
    if (!src.getKey(k).empty()) {
      ...
      if (!dest) {
        nlwarning("No dest object %s found when copying in place",
                  src.getKey(k).c_str());
      } else {
...
CObjectTable::~CObjectTable() {
  {CHECK_TABLE_INTEGRITY} TContainer::iterator first(_Value.begin()),
      last(_Value.end());
  for (; first != last; ++first) {
    delete first->second;
    first->second = 0;
...
void CObjectTable::sort() {
  ...
  }
  {
    ...
    uint32 lastValue = (uint32)_Value.size();
    for (; firstValue != lastValue; ++firstValue) {
      if (_Value[firstValue].first != "Keys" &&
          _Value[firstValue].second != 0) {
        data.push_back(_Value[firstValue]);
      }
    }
...
CObject *CObjectTable::clone() const {
  ...
  for (; first != last; ++first) {
    BOMB_IF(!first->second, "Try to clone a table with an NULL component",
            return 0);
...
namespace {
struct CValueIndex {
  CValueIndex(unsigned int index, const std::string &name, bool isTable,
              bool isDefaultFeature)
      : Index(index), Name(name), IsTable(isTable),
  ...
  bool operator<(const CValueIndex &rh) const {
    ...
    if (!IsTable) {
      if (Name == "InstanceId" && (rh.Name != "InstanceId")) {
        return true;
      }
      if (Name != "InstanceId" && (rh.Name == "InstanceId")) {
        return false;
      ...
      if (Name == "Version" && (rh.Name != "Version")) {
        return true;
      }
      if (Name != "Version" && (rh.Name == "Version")) {
        return false;
...
void CObjectTable::doSerialize(std::string &out,
                               CSerializeContext &context) const {
  ...
  for (; i < size; ++i) {
    ...
    indexs.push_back(CValueIndex(i, _Value[i].first,
                                 _Value[i].second->isTable(), isDefault));
  }
  ...
  for (; j < size; ++j) {
    ...
    if (!_Value[i].second->getGhost() &&
        !(key.size() > ghostStrSize &&
          key.substr(0, ghostStrSize) == ghostStr)) {
      out += "\n";
...
CObject *CObjectTable::getAttr(const std::string &name) const {
  ...
  if (name.size() >= 1 && '0' <= name[0] && name[0] <= '9') {
    ...
    for (; first2 != end2 && '0' <= name[first2] && name[first2] <= '9';
         ++first2) {
    }
    if (first2 == end2) {
      uint32 position;
...
bool CObjectTable::set(const std::string &key, const std::string &value) {
  ...
  if (!attr) {
    ...
    this->add(key, str);
    // nlwarning("Can't find attribute '%s' in object of class '%s'",
    // key.c_str(),  toString("Class").c_str());
    return true;
  }
...
CObjectTable *CObjectTable::doToTable() const {
  return const_cast<CObjectTable *>(this);
}
...
CObject *CObjectTable::take(sint32 position) {
  ...
  CHECK_TABLE_INTEGRITY
  BOMB_IF(!(-1 <= position && position < static_cast<sint32>(_Value.size())),
          "Try to take an element that does not exist", return 0);
...
bool CObjectTable::canTake(sint32 position) const {
  ...
  if (position == -1) {
    ...
    BOMB_IF(pos >= getParent()->getSize(),
            "Try to take an element that does not exist", return false);
...
bool CObjectTable::insert(const std::string &key, CObject *value,
                          sint32 position) {
  // H_AUTO(R2_CObjectTable_insert)
  CHECK_TABLE_INTEGRITY
  uint32 count = (uint32)_Value.size();

  BOMB_IF(!(-1 <= position && position <= static_cast<sint32>(count)),
          "Try to take an element that does not exist", return false);
  BOMB_IF(!value, "Try to insert a Null value", return false);
  BOMB_IF(value->getParent() != 0,
          "Try to insert an element that not at the root of the tree.",
          return false);
  ...
  if (0 <= position && position < static_cast<sint32>(count)) {
    ...
    _Value.insert(_Value.begin() + position,
                  std::pair<std::string, CObject *>(key, value));
  } else {
...
bool CObjectTable::equal(const CObject *other) const {
  ...
  for (i = 0; i != size; ++i) {
    ...
    if (key != this->getKey(i))
      return false;
    if (!value->equal(this->getValueAtPos(i)))
      return false;
  }
  return true;
}
...
void CObjectFactory::registerGenerator(CObject *objectClass) {
  ...
  bool exist =
      _Map.insert(std::make_pair(classType,
                                 new CObjectGenerator(objectClass, this)))
          .second;
  if (!exist) {
...
CObjectGenerator::~CObjectGenerator() {
  delete _ObjectClass;
  TDefaultValues::iterator first(_DefaultValues.begin()),
      last(_DefaultValues.end());
  for (; first != last; ++first) {
...
CObjectGenerator *CObjectFactory::getGenerator(const std::string &type) {
  ...
  std::map<std::string, CObjectGenerator *>::const_iterator found(
      _Map.find(type));
  if (found != _Map.end()) {
...
void CObjectGenerator::createDefaultValues(CObjectFactory *factory) {
  ...
  for (; first != last; ++first) {
    ...
    if (found && found->isString("DefaultValue") && found->isString("Name")) {
      ...
      if (!instance) {
        nlwarning(
            "<CObjectGenerator::instanciate> Can't create component of type %s",
            type.c_str());
        return;
      }
...
CObject *CObjectGenerator::instanciate(CObjectFactory *factory) const {
  ...
  for (; first != last; ++first) {
    ...
    if (found && found->isString("Type")) {
      ...
      if (!defaultInBase ||
          (defaultInBase->isInteger() && defaultInBase->toInteger() != 1)) {
        ...
        */
        CObject *instance = factory->newComponent(type);
        if (!instance) {
          nlwarning("<CObjectGenerator::instanciate> Can't create component of "
                    "type %s",
                    type.c_str());
        } else {
          nlassert(instance);
      ...
      if (!found) {
        nlwarning("Field 'Type' not found for a property in class %s",
                  classType.c_str());
      } else {
        nlwarning("Field 'Type' shoud has type 'String'  (class = %s)",
                  classType.c_str());
      }
    }
  }
...
void CObjectString::dump(const std::string prefix, uint depth) const {
  ...
  result += NLMISC::toString("%sString, ptr = Ox%p, value = %s, ghost = %s",
                             prefix.c_str(), this, _Value.c_str(),
                             _Ghost ? "true" : "false");
...
void CObjectNumber::dump(const std::string prefix, uint depth) const {
  ...
  if (m_IsInteger)
    result += NLMISC::toString(
        "%sNumber, ptr = 0x%p, isNumber, value = %lf, ghost = %s",
        prefix.c_str(), this, m_Value.Integer, _Ghost ? "true" : "false");
  else
    result += NLMISC::toString(
        "%sNumber, ptr = 0x%p, isInteger, value = %lf, ghost = %s",
        prefix.c_str(), this, m_Value.Number, _Ghost ? "true" : "false");
  nlwarning(result.c_str());
}
...
void CObjectTable::dump(const std::string prefix, uint depth) const {
  ...
  std::string result(depth * 4, ' ');
  result += NLMISC::toString("%sTable, ptr = 0x%p, , ghost = %s",
                             prefix.c_str(), this, _Ghost ? "true" : "false");
  nlwarning(result.c_str());
  for (uint k = 0; k < _Value.size(); ++k) {
    std::string prefix = NLMISC::toString("Index = %d, key = %s ", (int)k,
                                          _Value[k].first.c_str());
...
enum {
  ...
  ObjectTablePosition,
  ObjectTableNpc,
  ObjectTableNpcCustom,
  ObjectTableWayPoint,
  ObjectTableRegionVertex,
  ObjectTableRegion,
  ObjectTableRoad,
  ObjectTableNpcGrpFeature,
  ObjectTableBehavior,

  ObjectTableActivitySequence,
  ObjectTableActivityStep,
  ObjectTableChatSequence,
  ObjectTableChatStep,
  ObjectTableChatAction,
  ObjectTableLogicEntityAction,
  ObjectTableActionStep,
  ObjectTableActionType,
  ObjectTableEventType,
  ObjectTableLogicEntityReaction,
  ObjectTableTextManagerEntry,
  ObjectTableConditionStep,
  ObjectTableConditionType,

  ObjectTableRtAct,
  ObjectTableRtNpcGrp,
  ObjectTableRtNpc,
  ObjectTableRtPosition,
  RtAiState,
  ObjectTableRtAiState,
  ObjectTableRtNpcEventHandler,
  ObjectTableRtNpcEventHandlerAction,
  ObjectRefIdEmpty,
  ObjectRefId8,
  ObjectRefId16,
  ObjectRefId,
  ...
  ObjectNumberDouble,
  ObjectHeaderTag // If we "sort" this list we bust remove old session save
};
...
uint32 CObject::instanceIdToUint32(const std::string &instanceId) {
  ...
  std::string componentIdStr =
      instanceId.substr(clientIdIt + 1, size - clientIdIt);
  bool ko;
  uint32 clientId;
...
void CObjectSerializer::serialStringInstanceId(NLMISC::IStream &stream,
                                               std::string &data) {
...
static void serialNumberFixedPoint(NLMISC::IStream &stream, CObject *&data,
                                   CObjectSerializer *serializer) {
  if (serializer->getVersion() == 0) {
    nlwarning("Using oldScenario Version (must only be use when loading) old "
...
public:
  ...
  void serialImpl(NLMISC::IStream &stream, CObject *&data,
                  CObjectSerializer *serializer, bool init = false);
...
class CClassSerializer {
public:
  virtual void serialClass(NLMISC::IStream &stream, CObject *&data,
                           CObjectSerializer *serializer) {
    if (!stream.isReading()) {
      // serial other prop;
      ...
      */
      {
        ...
        for (; first != last; ++first) {
          ...
          } else if (std::find(_NeededProp.begin(), _NeededProp.end(), key) ==
                     _NeededProp.end()) {
            std::vector<std::string>::const_iterator optionalFound =
                std::find(_OptionalProp.begin(), _OptionalProp.end(), key);
      ...
      */
      if (!_NeededProp.empty()) {
        ...
        std::vector<std::string>::const_iterator first(_NeededProp.begin()),
            last(_NeededProp.end());
        ...
        for (; first != last; ++first) {
          ...
          if (!value) {
            nlwarning(
                "About to serialize a NULL value at key %s, className %s!",
                first->c_str(), _ClassName.c_str());
          }
          if (serializer->Log) {
            nldebug("R2NET: (%u) Field '%s'", serializer->Level,
                    first->c_str());
          }
        ...
        if (serializer->Log) {
          nldebug("R2NET: (%u) Needed Properties sent %u bytes",
                  serializer->Level + 1, endLength - initLength);
        }
      }
      ...
      if (!optionalPropFoundIndex.empty()) {
        ...
        for (; first != last; ++first) {
          ...
          if (serializer->Log) {
            nldebug("R2NET: (%u) Field '%s'", serializer->Level, key.c_str());
          }
        ...
        uint endLength = stream.getPos();
        if (serializer->Log) {
          nldebug("R2NET: (%u) Optional Properties sent %u bytes",
                  serializer->Level + 1, endLength - initLength);
        }
      }
      ...
      if (!valuePropFoundIndex.empty()) {
        ...
        for (; first != last; ++first) {
          ...
          if (serializer->Log) {
            nldebug("R2NET: (%u) Field [%u]", serializer->Level, first);
          }
          CObjectSerializerImpl::getInstance().serialImpl(stream, value,
                                                          serializer);
        }
        ...
        uint endLength = stream.getPos();
        if (serializer->Log) {
          nldebug("R2NET: (%u) Values Properties sent %u bytes",
                  serializer->Level + 1, endLength - initLength);
        }
      }
      ...
      if (!otherPropFoundIndex.empty()) {
        ...
        uint endLength = stream.getPos();
        if (serializer->Log) {
          nldebug("R2NET: (%u) Other Properties sent %u bytes",
                  serializer->Level + 1, endLength - initLength);
        }
      }
    } else {
      ...
      if ((next & 1)) {
        ...
        for (; first != last; ++first) {
          ...
          if (!value) {
            nlwarning(
                "Error the needed param '%s' of Class '%s' is a NULL value.",
  ...
  virtual void onSerial(NLMISC::IStream &stream, const std::string &key,
                        CObject *&data, CObjectSerializer *serializer) {
    if (key == "InstanceId") {
      serialStringInstanceId(stream, data);
      return;
    }
...
CNpcSerializer::CNpcSerializer() {
  static const char *neededProp[] = {"InstanceId", "Base", "Position", "Angle",
                                     "Behavior"};
  static const char *optionalProp[] = {"GabaritHeight",
                                       "GabaritTorsoWidth",
                                       "GabaritArmsWidth",
                                       "GabaritLegsWidth",
                                       "GabaritBreastSize",
  ...
  std::vector<std::string> tmp2(
      &optionalProp[0],
      &optionalProp[sizeof(optionalProp) / sizeof(optionalProp[0])]);
  _OptionalProp.swap(tmp2);
}

void CNpcSerializer::onSerial(NLMISC::IStream &stream, const std::string &key,
                              CObject *&data, CObjectSerializer *serializer) {
  // H_AUTO(R2_CNpcSerializer_onSerial)
  if (key == "Angle") {
    serialNumberFixedPoint(stream, data, serializer);
    return;
  }
...
CNpcCustomSerializer::CNpcCustomSerializer() {
  static const char *neededProp[] = {"InstanceId",
                                     "Base",
                                     "Position",
                                     "Angle",
                                     "Behavior",
  ...
  static const char *optionalProp[] = {
      "Name",     "Profile",          "Speed",
      "Aggro",    "PlayerAttackable", "BotAttackable",
      "Function", "WeaponRightHand",  "WeaponLeftHand",
      "Sex"};
...
void CNpcCustomSerializer::onSerial(NLMISC::IStream &stream,
                                    const std::string &key, CObject *&data,
                                    CObjectSerializer *serializer) {
...
static const char *ActivitySequenceNeededProp[] = {"InstanceId", "Name",
                                                   "Repeating", "Components"};
R2_CLASS_SERIALIZER_NEEDED(ActivitySequence, ActivitySequenceNeededProp);

static const char *ActivityStepNeededProp[] = {
    "InstanceId",     "Type", "TimeLimitValue", "EventsIds", "Chat",
    "ActivityZoneId", "Name", "TimeLimit",      "Activity"};
R2_CLASS_SERIALIZER_NEEDED(ActivityStep, ActivityStepNeededProp);

static const char *ChatSequenceNeededProp[] = {"InstanceId", "Name",
                                               "Components"};
R2_CLASS_SERIALIZER_NEEDED(ChatSequence, ChatSequenceNeededProp);

static const char *ChatStepNeededProp[] = {"InstanceId", "Time", "Actions",
                                           "Name"};
R2_CLASS_SERIALIZER_NEEDED(ChatStep, ChatStepNeededProp);

static const char *ChatActionNeededProp[] = {"InstanceId", "Emote", "Who",
                                             "Facing", "Says"};
R2_CLASS_SERIALIZER_NEEDED(ChatAction, ChatActionNeededProp);

static const char *LogicEntityActionNeededProp[] = {"InstanceId", "Conditions",
                                                    "Actions", "Event", "Name"};
...
static const char *LogicEntityReactionNeededProp[] = {
    "InstanceId", "LogicEntityAction", "ActionStep", "Name"};
R2_CLASS_SERIALIZER_NEEDED(LogicEntityReaction, LogicEntityReactionNeededProp);

static const char *TextManagerEntryNeededProp[] = {"InstanceId", "Count",
                                                   "Text"};
R2_CLASS_SERIALIZER_NEEDED(TextManagerEntry, TextManagerEntryNeededProp);

static const char *ConditionStepNeededProp[] = {"InstanceId", "Entity",
                                                "Condition"};
...
void CPositionSerializer::onSerial(NLMISC::IStream &stream,
                                   const std::string &key, CObject *&data,
                                   CObjectSerializer *serializer) {
...
public:
  CNpcGrpFeatureSerializer() {
    ...
    static const char *neededProp[] = {"InstanceId", "Name", "Components",
                                       "ActivitiesId"};
...
public:
  void onSerial(NLMISC::IStream &stream, const std::string &key, CObject *&data,
                CObjectSerializer *serializer) {
    if (key == "Id") {
      serialRtId(stream, data);
      return;
    }
    this->CClassSerializer::onSerial(stream, key, data, serializer);
  }
  ...
  void serialRtId(NLMISC::IStream &stream, CObject *&data) {
    if (!stream.isReading()) {
      ...
      std::string strInstanceId =
          NLMISC::toString("%s_%d", _ClassName.c_str(), id);
      data = new CObjectString(strInstanceId);
    }
  }
};
...
public:
  CRtActSerializer() {
    ...
    static const char *neededProp[] = {
        "Id", "NpcGrps", "FaunaGrps", "AiStates", "Npcs", "Events", "Actions"};
...
public:
  CRtNpcGrpSerializer() {
    ...
    static const char *neededProp[] = {"Id",
                                       "Name",
                                       "Children",
                                       "AutoSpawn",
                                       "BotChat_parameters",
...
public:
  CRtNpcSerializer() {
    ...
    static const char *neededProp[] = {
        "Id",        "Name",        "Children",       "ChatParameters",
        "Equipment", "IsStuck",     "Keywords",       "Keywords",
        "Sheet",     "SheetClient", "BotVerticalPos", "Angle",
        "Pt"};
...
public:
  CRtAiStateSerializer() {
    ...
    static const char *neededProp[] = {
        "Id",         "Name",        "Children",
        "AiActivity", "AiMovement",  "AiProfileParams",
        "Keywords",   "VerticalPos", "Pts"};
...
public:
  CRtNpcEventHandlerSerializer() {
    ...
    static const char *neededProp[] = {
        "Id", "Name", "Event", "StatesByName", "GroupsByName", "ActionsId"};
...
public:
  CRtNpcEventHandlerActionSerializer() {
    ...
    static const char *neededProp[] = {"Id",         "Action",   "Name",
                                       "Parameters", "Children", "Weight"};
...
public:
    CNpcGrpFeatureSerializer()
    {
        ...
        std::vector<std::string> tmp(&(neededProp[0]),
&(neededProp[nbNeededProp])); _NeededProp.swap(tmp);
    }
};
*/
...
CObjectSerializerImpl::~CObjectSerializerImpl() {
  std::map<uint8, CClassSerializer *>::iterator first(
      _ClassSerializersById.begin()),
      last(_ClassSerializersById.end());
  for (; first != last; ++first) {
    delete first->second;
  }
...
void CObjectSerializerImpl::registerSerializer(CClassSerializer *serializer) {
  bool insert = _ClassSerializersById
                    .insert(std::make_pair(serializer->getId(), serializer))
                    .second;
  if (!insert) {
    nlinfo("R2Share: Prototype register two time");
  }
  insert = _ClassSerializersByName
               .insert(std::make_pair(serializer->getClassName(), serializer))
               .second;
  if (!insert) {
    nlinfo("R2Share: Prototype register two time");
  }
...
void CObjectSerializerImpl::serialImpl(NLMISC::IStream &stream, CObject *&data,
                                       CObjectSerializer *serializer,
                                       bool init) {
  ...
  if (!stream.isReading()) {
    ...
    if (data == 0) {
      ...
      uint endLength = stream.getPos();
      if (serializer->Log) {
        nldebug("R2NET: (%u) Null sent %u bytes", serializer->Level,
                endLength - initLength);
      }
      ...
      uint endLength = stream.getPos();
      if (serializer->Log) {
        nldebug("R2NET: (%u) Integer sent %u bytes", serializer->Level,
                endLength - initLength);
      }
      ...
      uint endLength = stream.getPos();
      if (serializer->Log) {
        nldebug("R2NET: (%u) Number sent %u bytes", serializer->Level,
                endLength - initLength);
      }
      ...
      if (size == 0) {
        ...
        type = data->isRefId()
                   ? ObjectRefIdEmpty
                   : ObjectStringEmpty; // NB : 'isString()' would be true in
                                        // both test because CObjectRefId
                                        // derives from CObjectString
        stream.serial(type);
      }
        ...
        type = data->isRefId()
                   ? ObjectRefId8
                   : ObjectString8; // NB : 'isString()' would be true in both
                                    // test because CObjectRefId derives from
                                    // CObjectString
        ...
        type = data->isRefId()
                   ? ObjectRefId16
                   : ObjectString16; // NB : 'isString()' would be true in both
                                     // test because CObjectRefId derives from
                                     // CObjectString
        ...
        type = data->isRefId()
                   ? ObjectRefId
                   : ObjectString; // NB : 'isString()' would be true in both
                                   // test because CObjectRefId derives from
                                   // CObjectString
        stream.serial(type);
        stream.serial(value);
      ...
      uint endLength = stream.getPos();
      if (serializer->Log) {
        nldebug("R2NET: (%u) String Send %u bytes", serializer->Level,
                endLength - initLength);
      }
      return;
      ...
      if (!className.empty()) {
        std::map<std::string, CClassSerializer *>::const_iterator found(
            _ClassSerializersByName.find(className));
        if (found != _ClassSerializersByName.end()) {
          found->second->serialClass(stream, data, serializer);
          uint endLength = stream.getPos();
          if (serializer->Log) {
            nldebug("R2NET: (%u) Class '%s' sent %u bytes", serializer->Level,
                    className.c_str(), endLength - initLength);
          }
          return;
        }
      }
      ...
      if (serializer->Log) {
        if (className.empty()) {
          nldebug("R2NET: (%u) Table sent %u bytes", serializer->Level,
                  endLength - initLength);
        } else {
          nldebug("R2NET: (%u) Generic Class(%s) sent %u bytes",
                  serializer->Level, className.c_str(), endLength - initLength);
        }
      }
    ...
    // NB nico : use factory here instead of plain 'new' because client may use
    // derived classes internally NB 2 : if server is created locally, not a
    // problem because derived object fonctinnality
    ...
    case ObjectNumber: {
      double value;
      stream.serial(value);
      data = serializer->Factory ? serializer->Factory->newBasic("Number")
                                 : new CObjectNumber((sint64)0);
    ...
    case ObjectStringEmpty: {
      data = serializer->Factory ? serializer->Factory->newBasic("String")
                                 : new CObjectString("");
      return;
    }
    ...
    case ObjectRefId: {
      std::string value;
      stream.serial(value);
      data = serializer->Factory ? serializer->Factory->newBasic("RefId")
                                 : new CObjectRefId("");
    ...
    default:
      std::map<uint8, CClassSerializer *>::const_iterator found(
          _ClassSerializersById.find(type));
...
CObjectSerializer::CObjectSerializer(CObjectFactory *factory, CObject *data)
    : Factory(factory), Level(0), Log(false) {
...
void CObjectSerializer::compress() {
  ...
  int ok =
      ::compress(dest, &destLen, (Bytef *)buffer.buffer(), buffer.length());
...
void CObjectSerializer::uncompressImpl() {
  ...
  if (_Compressed && _MustUncompress) {
    ...
    sint32 state = ::uncompress(data, &dataLen,
                                reinterpret_cast<Bytef *>(_CompressedBuffer),
                                _CompressedLen);

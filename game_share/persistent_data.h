...
/**
  This file contains a system for loading and saving persistent data in a robust
  manor The system is hierarchical and all data elements are tagged.

  The system supports saving to text files and to binary files

  NOTE: The text file format is similar to XML but is not currently XML
  feature-complete
...
#define DECLARE_PERSISTENCE_APPLY_METHOD void apply(CPersistentDataRecord &pdr);
...
// A friendly macro to place before each inclusion of persistent_data_template.h
// to ease debug
#define PERSISTENT_GENERATION_MESSAGE                                          \
  NL_LOC_MSG                                                                   \
      "generating persistence code for " NL_MACRO_TO_STR(PERSISTENT_CLASS)
...
private:
  ...
  struct CArg {
    ...
    CArg();
    CArg(const std::string &type, const std::string &value,
    ...
    void push(TToken token, std::vector<TToken> &tokenTable,
...
public:
  ...
  // note that the tokenFamily is used to setup the initial state of the string
  // table (at init time)
  ...
  void skipData(); // Added by Ben, will skip next record, a struct as well as a
                   // simple token
  ...
  // compare two persistent data records (verify that their contents are
  // eqivalent)
  ...
  // get info as a single text string including field names and values in a
  // man-readable format
  NLMISC::CSString getInfo() const;
  // get info as a string of comma separated values
  NLMISC::CSString getInfoAsCSV() const;
  // get the header row that correspond to the comma separated values in
  // getInfoAsCSV() - note that this method is static
  static const NLMISC::CSString &getCSVHeaderLine();
  // get the number of values in a pdr (a value is a string, a uint32, uint64,
  // CEntitiyId, flag or other such primitive)
  ...
  // write data to a binary buffer - the size required for the buffer is
  // totalDataSize()
  ...
  bool writeToTxtFile(const std::string &fileName,
                      TStringFormat stringFormat = XML_STRING);

  // if the format is set to 'ANY_FILE' then use the extension provided in the
  // 'fileName' argument to determine the file type. In this case 'txt' and
  // 'xml' have specific meanings returns writeToTxtFile(...) or
  // writeToBinFile(...) depending on the file format
  bool writeToFile(const std::string &fileName,
                   TFileFormat fileFormat = ANY_FILE);
...
//-----------------------------------------------------------------------------
// This is just a specialisation of the class that register it into the
// "RyzomTokenFamily" Use it only to store (else useless copy of default string
// table at clear())
...
public:
  ...
  // in release mode if the index 'idx' falls outside the table we return -1;
  // (This is the 'Bad String' value)
...
public:
  ...
  getStringTable(const std::string &family);

private:
  // private data
  typedef std::map<NLMISC::CSString, CPersistentDataRecord::TStringTable>
      TRegistry;
  TRegistry _Registry;

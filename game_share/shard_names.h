...
public:
  struct TSessionName {
    ...
    /// Display name, as displayed in user interface and appended to player
    /// character names
  ...
  // This container is just a vector because it is very small and brute force
...
public:
   ...
   *	Input name can be :
   *		- a short name, session id is deduced to be the same as context
   *session id
   *		- a located name (<shortShardName>.<characterName> : the session
   *is is deduced from the short shard name
   *		- a full name (<characterName>(<shardName>) : the session is
   *deduced from the shard name.
   */
  void parseRelativeName(TSessionId contextSessionId,
                         const std::string &inputCharName,
                         std::string &outCharName, TSessionId &outSessionId);
   ...
   */
  std::string makeFullName(const std::string &charName,
   ...
   */
  std::string makeFullNameFromRelative(TSessionId contextSessionId,
                                       const std::string &inputcharName);
};

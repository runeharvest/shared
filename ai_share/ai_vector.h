...
public: // Methods.
  ...
  CAIVector &operator=(const NLMISC::CVector2d &v) {
    _x = v.x;
    _y = v.y;
    return *this;
  }
  ...
  template <class C> void setXY(const C &x, const C &y) {
    _x = x;
    _y = y;
  }
  ...
  template <class V> CAIVector &operator+=(const V &v) {
    _x += v.x();
    _y += v.y();
    return *this;
  }
  ...
  template <class V> CAIVector operator+(const V &v) const {
    CAIVector rv(*this);
    return (rv += v);
  }
  template <class V> CAIVector operator-(const V &v) const {
  ...
  template <class C> CAIVector &operator*=(C c) {
    _x *= c;
    _y *= c;
    return *this;
  }
  ...
  //	template <class C> CAIVector operator*(C c) const
  //{ return CAIVector(_x*c, _y*c); }
  ...
  // Advanced Maths.
  // Dot product.
  template <class V> double dot(const V &v) const {
    return _x.asDouble() * v.x().asDouble() + _y.asDouble() * v.y().asDouble();
  }
  template <class V> double vectorDot(const V &v) const {
    return _x.asDouble() * v.y().asDouble() - _y.asDouble() * v.x().asDouble();
  }
  ...
  double sqrnorm() const {
    return _x.asDouble() * _x.asDouble() + _y.asDouble() * _y.asDouble();
  }

  double quickNorm() const {
    double dx = fabs(x().asDouble()), dy = fabs(y().asDouble());
  ...
  */
  ...
  CAngle asAngle() const { return CAngle(atan2(_y.asDouble(), _x.asDouble())); }
  ...
  template <class V> CAngle angleTo(const V &v) const {
    return CAngle(atan2((v.y() - y()).asDouble(), (v.x() - x()).asDouble()));
  }
  template <class V> double distTo(const V &v) const {
    return (*this - v).norm();
  }
  template <class V> double distSqTo(const V &v) const {
    return (*this - v).sqrnorm();
  }
  template <class V> double quickDistTo(const V &v) const {
    double dx = fabs((v.x() - x()).asDouble()),
  ...
  std::string toString() const {
    return NLMISC::toString("(%.3f,%.3f)", _x.asDouble(), _y.asDouble());
  }

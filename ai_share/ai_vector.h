...
public: // Methods.
	...
	CAIVector() { }
	explicit CAIVector(const NLMISC::CVector2d &v)
	    : _x(v.x)
	    , _y(v.y)
	{
	}
	explicit CAIVector(const CAICoord &x, const CAICoord &y)
	    : _x(x)
	    , _y(y)
	{
	}
	explicit CAIVector(double x, double y)
	    : _x(x)
	    , _y(y)
	{
	}
	...
	template <class C>
	void setX(const C &x) { _x = x; }
	template <class C>
	void setY(const C &y) { _y = y; }
	template <class C>
	void setXY(const C &x, const C &y)
	{
		_x = x;
		_y = y;
	}
	template <class C>
	void setXY(const C &xy) { *this = xy; }

	// Base Maths: +=, +, -=, - vs CVector2d, CAIVector, CAIVectorMirror
	template <class V>
	CAIVector &operator+=(const V &v)
	{
		_x += v.x();
		_y += v.y();
		return *this;
	...
	template <class C>
	CAIVector operator/(C c) const { return CAIVector(_x / c, _y / c); }
	...
	// Dot product.
	template <class V>
	double dot(const V &v) const { return _x.asDouble() * v.x().asDouble() + _y.asDouble() * v.y().asDouble(); }
	template <class V>
	double vectorDot(const V &v) const { return _x.asDouble() * v.y().asDouble() - _y.asDouble() * v.x().asDouble(); }

	// Return the norm of the vector.
	double norm() const { return sqrt(sqrnorm()); }
	// Return the square of the norm of the vector.
	double sqrnorm() const { return _x.asDouble() * _x.asDouble() + _y.asDouble() * _y.asDouble(); }

	double quickNorm() const
	{
		double dx = fabs(x().asDouble()),
		       dy = fabs(y().asDouble());
	...
	*/
	...
	template <class V>
	bool operator==(const V &v) const { return _x == v.x() && _y == v.y(); }
	template <class V>
	bool operator!=(const V &v) const { return _x != v.x() || _y != v.y(); }
	...
	template <class V>
	CAngle angleTo(const V &v) const { return CAngle(atan2((v.y() - y()).asDouble(),
		(v.x() - x()).asDouble())); }
	template <class V>
	double distTo(const V &v) const { return (*this - v).norm(); }
	template <class V>
	double distSqTo(const V &v) const { return (*this - v).sqrnorm(); }
	template <class V>
	double quickDistTo(const V &v) const
	{

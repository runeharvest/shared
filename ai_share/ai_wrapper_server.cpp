...
void CAiWrapperServer::streamToPdr(NLMISC::IStream &stream,
...
void CAiWrapperServer::primsToPdr(NLLIGO::CPrimitives *prims,
                                  const std::string &primName,
                                  CPersistentDataRecord &pdr) {
...
void CAiWrapperServer::pdrToFile(CPersistentDataRecord &pdr,
                                 const std::string &pdrName) {
  CAIPrimitiveParser::init(&pdr);
  CAIPrimitiveParser::getInstance().writeFile(pdrName.c_str());
  CAIPrimitiveParser::release();
}

void CAiWrapperServer::fileToPdr(const std::string &pdrName,
                                 CPersistentDataRecord &pdr) {
  CAIPrimitiveParser::init(&pdr);
...
void CAiWrapperServer::primitiveFileToPdr(const std::string &primitiveName,
                                          CPersistentDataRecord &pdr) {
  pdr.clear();
  CAIPrimitiveParser::init(&pdr);
...
void CAiWrapperServer::stopTest(TSessionId sessionId, uint32 aiInstanceId) {
  nldebug("Stop Test in session %u (aiInstance %u)", sessionId.asInt(),
          aiInstanceId);
...
void CAiWrapperServer::startTest(TSessionId sessionId, uint32 aiInstanceId,
                                 CPersistentDataRecord &pdr) {

  nldebug("Start Test in session %u (aiInstance %u)", sessionId.asInt(),
          aiInstanceId);
...
void CAiWrapperServer::startInstance(TSessionId sessionId,
                                     uint32 aiInstanceId) {
  nldebug("Start Instance in session %u (aiInstance %u)", sessionId.asInt(),
          aiInstanceId);
...
void CAiWrapperServer::stopAct(TSessionId sessionId, uint32 aiInstanceId) {
  nldebug("Stop Act in session %u (aiInstance %u)", sessionId.asInt(),
          aiInstanceId);
  CMessage msgout("R2_STOPLIVE");
  msgout.serial(aiInstanceId);
  bool isBase = false;
...
void CAiWrapperServer::startAct(TSessionId sessionId, uint32 aiInstanceId,
                                CPersistentDataRecord &pdr) {
  nldebug("Start Act in session %u (aiInstance %u)", sessionId.asInt(),
          aiInstanceId);
  uint32 totalDataSize = pdr.totalDataSize();
  char *dest = new char[totalDataSize];
  if (!pdr.toBuffer(dest, totalDataSize)) {
...
void CAiWrapperServer::setAggroRange(NLMISC::CEntityId entityId, float range) {
  ...
  std::string command = NLMISC::toString("eventSetNpcGroupAggroRange %s %f",
                                         entityId.toString().c_str(), range);
...
void CAiWrapperServer::despawnEntity(NLMISC::CEntityId entityId, uint32 alias) {
  ...
  std::string str = NLMISC::toString("()despawnBotByAlias(\"%s\");",
                                     aliasToString(alias).c_str());
...
void CAiWrapperServer::setGrpHPLevel(NLMISC::CEntityId entityId, uint32 alias,
                                     float hp) {
  uint32 messageVersion = 1;
  uint32 nbString = 2;
  std::string eid = entityId.toString();
...
void CAiWrapperServer::setHPLevel(NLMISC::CEntityId entityId, uint32 alias,
                                  float hp) {
  ...
  std::string hpstr = NLMISC::toString("()setBotHPScaleByAlias(%f, \"%s\");",
                                       hp, aliasToString(alias).c_str());
...
void CAiWrapperServer::triggerGrpEvent(NLMISC::CEntityId entityId,
                                       float eventId) {
...
void CAiWrapperServer::controlNpc(NLMISC::CEntityId clientId,
  ...
  std::string str =
      NLMISC::toString("()setPlayerController(\"%s\",\"%s\");",
                       npc.toString().c_str(), clientId.toString().c_str());
...
void CAiWrapperServer::stopControlNpc(NLMISC::CEntityId clientId,
                                      NLMISC::CEntityId npc) {

  uint32 messageVersion = 1;
  uint32 nbString = 2;
  std::string eid = npc.toString();
  std::string str = NLMISC::toString("()clearPlayerController(\"%s\");",
                                     npc.toString().c_str());
...
void CAiWrapperServer::triggerUserTrigger(const std::string &groupName,
                                          uint32 triggerId) {
  /*
      uint32 messageVersion = 1;
      uint32 nbString=2;
  ...
  */

  {
    ...
    CMessage msgout("EXEC_COMMAND");
    std::string command =
        NLMISC::toString("script %s %s", groupName.c_str(), script.c_str());
    msgout.serial(command);
    CUnifiedNetwork::getInstance()->send("AIS", msgout);
  }
}

void CAiWrapperServer::setPioneerRight(NLMISC::CEntityId entityId,
                                       const R2::TPioneerRight &right) {

  if (right == R2::TPioneerRight::DM) {
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("God %s %d", entityId.toString().c_str(), 1);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("Invisible %s %d", entityId.toString().c_str(), 1);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("Aggro %s %d", entityId.toString().c_str(), 0);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
  } else if (right == R2::TPioneerRight::Tester) {
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("God %s %d", entityId.toString().c_str(), 0);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("Invisible %s %d", entityId.toString().c_str(), 0);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
    {
      CMessage msgout("EXEC_COMMAND");
      std::string command =
          NLMISC::toString("Aggro %s %d", entityId.toString().c_str(), 1);
      msgout.serial(command);
      CUnifiedNetwork::getInstance()->send("EGS", msgout);
    }
  } else {
...
void CAiWrapperServer::askBotDespawnNotification(NLMISC::CEntityId creatureId,
                                                 TAIAlias alias) {

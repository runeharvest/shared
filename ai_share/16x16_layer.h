...
template <typename T, int NumObjects> class CFastBitField {
private:
  enum {
    TypeSize = sizeof(T) * 8,
...
public:
  ...
  CFastBitField &operator=(const uint &val) {
    Field = (T)val;
    return *this;
  }
  CFastBitField &operator=(const CFastBitField &bf) {
    Field = bf.Field;
    return *this;
  }
...
public:
   ...
   * Data are assumed to be stored in a c-style array like sint Array[16][16],
   * that is accessed through Array[i][j]. i and j MUST be clamped to interval
   * [0, 16], as they might not be tested.
   */
  ...
  /**
   * Loads a 16x16Layer and returns a pointer to it. Layer is automatically
   * allocated.
   */
  ...
  /**
   * Compresses a 16x16Layer. Returns a new layer if compression was successful,
   * otherwise returns same layer. If compression was successful, previous layer
   * is deleted.
   */
...
public:
  ...
  sint get(uint i, uint j) const {
    nlassert(i < 16 && j < 16);
    return Array[i][j];
  }
  void set(uint i, uint j, sint value) {
    nlassert(i < 16 && j < 16);
    Array[i][j] = value;
  }
...
public:
  ...
  C8Bits16x16Layer(sint32 mean = 0) : Mean(mean) {
    memset(Array, 0, sizeof(Array));
  }
  sint get(uint i, uint j) const {
    nlassert(i < 16 && j < 16);
    return Mean + Array[i][j];
  }
  void set(uint i, uint j, sint value) {
    nlassert(i < 16 && j < 16);
    Array[i][j] = (uint8)(value - Mean);
  }
...
public:
  ...
  C4Bits16x16Layer(sint32 mean = 0) : Mean(mean) {
    memset(Array, 0, sizeof(Array));
  }
  sint get(uint i, uint j) const {
    nlassert(i < 16 && j < 16);
    return Mean + Array[i][j >> 3].get(j);
  }
  void set(uint i, uint j, sint value) {
    nlassert(i < 16 && j < 16);
    Array[i][j >> 3].set(j, (value - Mean) & 0xf);
  }
...
public:
  ...
  C2Bits16x16Layer(sint32 v0 = 0, sint32 v1 = 1, sint32 v2 = 2, sint32 v3 = 3) {
    Values[0] = v0;
    Values[1] = v1;
    Values[2] = v2;
    Values[3] = v3;
  ...
  sint get(uint i, uint j) const {
    nlassert(i < 16 && j < 16);
    return Values[Array[i].get(j)];
  }
  void set(uint i, uint j, sint value) {
    nlassert(i < 16 && j < 16);
    Array[i].set(j, getIndex(value));
  }
...
public:
  ...
  C1Bit16x16Layer(sint32 v0 = 0, sint32 v1 = 1) {
    Values[0] = v0;
    Values[1] = v1;
    memset(Array, 0, sizeof(Array));
  }

...
public:
	...
	inline CAngle(const CAngle &other)
	    : _data(other._data)
	{
	}
	inline CAngle(const sint16 &val)
	    : _data(val)
	{
	}
	inline CAngle(const sint32 &val)
	    : _data((sint16)val)
	{
	}
	inline CAngle(const uint16 &val)
	    : _data((sint16)val)
	{
	}
	inline CAngle(const uint32 &val)
	    : _data((sint16)val)
	{
	}
...
#ifdef NL_COMP_VC6
	inline CAngle(const sint &val)
	    : _data((sint16)val)
	{
	}
	inline CAngle(const uint &val)
	    : _data((sint16)val)
	{
	}
#endif
	...
	template <class C>
	CAngle &operator=(const C &val)
	{
		_data = CAngle(val)._data;
		return *this;
	}
	template <class C>
	CAngle &operator+=(const C &val)
	...
	template <class C>
	CAngle operator+(const C &val) { return CAngle(_data + CAngle(val)._data); }
	template <class C>
	CAngle operator-(const C &val) { return CAngle(_data - CAngle(val)._data); }
	template <class C>
	CAngle operator*(const C &val) { return CAngle(sint16(_data * val)); }
	...
	NLMISC::CVector2d asVector2d() const
	{
		float radians = asRadians();
		return NLMISC::CVector2d(cos(radians), sin(radians));
	}

	inline static const CAngle &pi()
	{
		static CAngle p(PI);
		return p;
	}

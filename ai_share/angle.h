...
public:
  ...
  inline CAngle(const double &val) {
    ...
    // NB : Fundamentaly, the problem is that it is bad to cast something with
    // an overflow... ;)
  ...
  template <class C> bool operator>(const C &other) const {
  ...
  template <class C> CAngle &operator=(const C &val) {
  ...
  template <class C> CAngle operator+(const C &val) {
    return CAngle(_data + CAngle(val)._data);
  }
  template <class C> CAngle operator-(const C &val) {
    return CAngle(_data - CAngle(val)._data);
  }
  template <class C> CAngle operator*(const C &val) {
    return CAngle(sint16(_data * val));
  }

  CAngle operator/(const double &val) const {
    return CAngle((sint16)(_data / val));
  }
  float operator/(const CAngle &val) const {
    return (float)_data / (float)val._data;
  }

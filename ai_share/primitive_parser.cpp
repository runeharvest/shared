...
static void parsePrimGroupFamilyProfileFaunaContent(
    const CAIAliasDescriptionNode *aliasNode, const IPrimitive *prim);
static void parsePrimGroupFamilyProfileTribeContent(
    const CAIAliasDescriptionNode *aliasNode, const IPrimitive *prim);
static void
parsePrimGroupFamilyProfileNpcContent(const CAIAliasDescriptionNode *aliasNode,
...
uint32 nodeAlias(const IPrimitive *prim, bool canFail = false) {
  ...
  IPrimitive *aliasNode =
      getPrimitiveChild(const_cast<IPrimitive *>(prim), pred);
  if (aliasNode) {
...
//	nlassert(!s.empty());
  ...
  //	// for legacy reasons the field may be called 'unique_id' instead of
  //'alias' 	if (s.empty()) 		prim->getPropertyByName("unique_id",s);
  ...
  //			&&	nodeType(prim)!=AITypeFaunaSpawnAtom)	//
  //legacy reasons .. again (bad). 			nlwarning("Failed to find alias for prim
  //node: '%s': '%s' (using generated alias: %u)", 				buildPrimPath(prim).c_str(),
  ...
  //		&&	nodeClass(prim) != "npc_group_parameters" )  // <= for
  //legacy reason
  {
    nlassert(false);
    ...
    //			++alias;
    // #if !FINAL_VERSION
    //		nlwarning("Alias %u not unique - remaping to
    //%u",oldAlias,alias);
    // #endif
  }
...
static bool parseVerticalPos(const IPrimitive *prim, uint32 &verticalPos,
                             const char *propertyName = "vertical_pos") {
  string s;
...
//{
//	static bool inited;					//	todo,
//change this as we can need to reload definitions .. (like new tribes or other
//..) 	static vector<string>	familyNames; 	static vector<string>
//tribeNames;
//
//	if (!inited)
//	{
...
//		const std::vector<std::pair<std::string, NLMISC::TStringId> >
//&names = TPopulationFamily::getTribesNames();
//
...
//				string s;
//				if (prim->getPropertyByName(flagName.c_str(), s)
//&& s=="true") 					result.insert(flagName);
//			}
//		}
//		else
//		{
...
//			string s;
//			if (prim->getPropertyByName(familyNames[i].c_str(), s)
//&& s=="true") 				result.insert(familyNames[i]);
//		}
//
//	}
//
...
static void lookForMissions(const IPrimitive *prim, std::vector<int> &missions,
                            std::vector<std::string> &missionsNames) {
...
// the following variables are setup by parsePrimGrpNpc() and referenced by
// parsePrimNpcBot oldlevel static std::string DefaultBotLevel;
...
static const std::vector<std::string> *DefaultGrpParameters =
    &EmptyStringVector;
...
// the following routine looks though the children of treeNode for one that
// matches prim if no child is found then treeNode is returned
static const CAIAliasDescriptionNode *
nextTreeNode(const CAIAliasDescriptionNode *treeNode, const IPrimitive *prim) {
  ...
  for (uint i = 0; i < treeNode->getChildCount(); ++i) {
    ...
    if (childNode->getAlias() == uniqueId) {
      if (childNode->getType() != type || childNode->getName() != name) {
        nlwarning("nextTreeNode(): Unique ID conflict in node: (%s, %u, %s): "
                  "looking for (%s, %u, %s) but found (%s, %u, %s)",
                  getName(treeNode->getType()), treeNode->getAlias(),
                  treeNode->fullName().c_str(), getName(childNode->getType()),
                  childNode->getAlias(), childNode->fullName().c_str(),
...
// the following routine recursively scans 'prim' and its children, constructing
// a CAIAliasDescriptionNode tree to represent ai type tree entries
static void buildAliasTree(CAIAliasDescriptionNode *treeNode,
                           CAIAliasDescriptionNode *rootNode,
                           const IPrimitive *prim,
                           std::vector<SFolderRef> &folders) {
  // run through the node children looking for nodes with types that we
  // recognize
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    // get a pointer to the child and make sure its valid
    const IPrimitive *child;
    if (prim->getChild(child, i)) {
      ...
      //			CAIAliasDescriptionNode *parentNode=
      //(type==AITypeFolder)?	rootNode: treeNode;
      ...
      if (parentNode->findNodeChildByNameAndType(name, type) != NULL) {
        LOG("Name not unique: %s: '%s:%s' @ '%s'", getName(type),
            parentNode->fullName().c_str(), name.c_str(),
            treeNode->fullName().c_str());
      }

      // make sure the unique id is unique (SLOW!, must be replace with a fast
      // hash_map access!)
      ...
      NLMISC::CSmartPtr<CAIAliasDescriptionNode> node =
          new CAIAliasDescriptionNode(name, uniqueId, type, parentNode);
...
//////////////////////////////////////////////////////////////////////////
//	predecl.
static void parsePrimGrpNpc(const CAIAliasDescriptionNode *treeNode,
                            const IPrimitive *prim,
                            const std::string initialState);
static void parsePrimGrpFauna(const CAIAliasDescriptionNode *treeNode,
                              const IPrimitive *prim);

static void parsePrimStateChat(const CAIAliasDescriptionNode *treeNode,
                               const IPrimitive *prim) {
  ...
  if (!botKeywords->empty())
    CAIActions::execute("BOTKEYS", *botKeywords);
  if (!botNames->empty())
    CAIActions::execute("BOTNAMES", *botNames);
  if (!chat->empty())
    CAIActions::execute("CHAT", *chat);
  CAIActions::end(treeNode->getAlias());
}

static void parsePrimStateProfile(const CAIAliasDescriptionNode *treeNode,
                                  const IPrimitive *prim) {
  // read the main body of the parameters
  ...
  if (!grpKeywords->empty())
    CAIActions::execute("GRPKEYS", *grpKeywords);
  if (!grpNames->empty())
    CAIActions::execute("GRPNAMES", *grpNames);
  CAIActions::end(treeNode->getAlias());
}

static CTmpPropertyZone::TSmartPtr
parseMachineStatePropertyZone(const CAIAliasDescriptionNode *treeNode,
                              const IPrimitive *prim) {
  CTmpPropertyZone::TSmartPtr propertyZone = new CTmpPropertyZone();

  //	void setPatat (AITYPES::TVerticalPos verticalPos, const std::vector
  //<CAIVector> &points)
  ...
  if (numPoints != 0) {
    ...
    for (uint i = 0; i < numPoints; ++i) {
      propertyZone->points.push_back(
          CAIVector(pointArray[i].x, pointArray[i].y));
    }
  }
...
static CAIEventActionNode::TSmartPtr
parsePrimEventAction(const CAIAliasDescriptionNode *treeNode,
                     const IPrimitive *prim) {
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    case AITypeEventAction: {
      ...
      childAction = parsePrimEventAction(
          treeNode ? nextTreeNode(treeNode, child) : NULL, child);
      if (childAction)
        result->Children.push_back(childAction);
    } break;
    case AITypeActionZone: {
      result->_PropertyZones.push_back(parseMachineStatePropertyZone(
          treeNode ? nextTreeNode(treeNode, child) : NULL, child));
    ...
    case AITypeNpcActionZone: {
      result->_PropertyZones.push_back(parseMachineStatePropertyZone(
          treeNode ? nextTreeNode(treeNode, child) : treeNode, child));
...
/*
<PRIMITIVE CLASS_NAME="fauna_action_zone" TYPE="zone" R="128" G="128" B="128"
A="128" AUTO_INIT="true" DELETABLE="true" NUMBERIZE="false"> <PARAMETER
NAME="name" TYPE="string" VISIBLE="true"/> <PARAMETER NAME="ai_type"
TYPE="string" VISIBLE="false"> <DEFAULT_VALUE VALUE="FAUNA_ACTION_ZONE"/>
</PARAMETER>
<PARAMETER NAME="params" TYPE="string_array" VISIBLE="true"
WIDGET_HEIGHT="100"/>
</PRIMITIVE>
 */

static void
parsePrimGroupDescriptionsForAction(const CAIAliasDescriptionNode *aliasNode,
                                    const IPrimitive *prim,
  ...
  // run through the dynsystem children looking for nodes with types that we
  // recognise
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    // get a pointer to the child and make sure its valid
    const IPrimitive *child;
    if (prim->getChild(child, i)) {
      if (nodeType(child) == AITypeBadType)
        continue; // skip alias node
      const CAIAliasDescriptionNode *childTreeNode =
          nextTreeNode(aliasNode, child);
      string familyTag;
      child->getPropertyByName("family", familyTag);
      CAIActions::exec("GRPFAM", childTreeNode, familyTag, name,
                       logicActionAlias);
      TAIType type = nodeType(child);
      switch (type) {
      case AITypeGroupFamily: {
        nlwarning("Parsing a group_family, primitive is outdated. Please "
                  "report to jvuarand.");
      ...
      case AITypeGroupFamilyProfileFauna:
        ...
        //				parsePrimGroupFamilyProfileGeneric(nextTreeNode(aliasNode,child),child,
        //GroupFamilyFauna);
        break;
...
static void
addGroupDescriptionToEventAction(const CAIAliasDescriptionNode *treeNode,
                                 const IPrimitive *prim, uint depth) {
  uint32 uniqueId = 0;
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    switch (type) {
    case AITypeEventAction: {
      addGroupDescriptionToEventAction(
          treeNode ? nextTreeNode(treeNode, child) : NULL, child, depth + 1);
    } break;
    case AITypeActionZone:
    case AITypeFaunaActionZone:
    case AITypeNpcActionZone:
    ...
    case AITypeFolder: {
      ...
      if (cname == "group_descriptions") {
        ...
        parsePrimGroupDescriptionsForAction(
            treeNode ? nextTreeNode(treeNode, child) : NULL, child,
            treeNode ? treeNode->getAlias() : uniqueId);
...
static void parsePrimEvent(const CAIAliasDescriptionNode *treeNode,
                           const IPrimitive *prim) {
  ...
  const IPrimitive *child;
  std::string type;
  if (prim->getNumChildren() != 2 || !prim->getChild(child, 1) ||
      !child->getPropertyByName("ai_type", type) ||
      getType<TAIType>(type.c_str()) != AITypeEventAction) {
    nlwarning("Failed to find the action associated with event: %s (in %s)",
              name.c_str(), treeNode ? treeNode->fullName().c_str() : "");
    return;
  }

  result->Action = parsePrimEventAction(
      treeNode ? nextTreeNode(treeNode, child) : NULL, child);
  ...
  addGroupDescriptionToEventAction(
      treeNode ? nextTreeNode(treeNode, child) : NULL, child, 1);
...
static void parsePrimBotKami(const CAIAliasDescriptionNode *treeNode,
                             const IPrimitive *prim) {
  nlassert(false);
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  ...
  std::string kamiType;
  if (!prim->getPropertyByName("ai_kami_type", kamiType) ||
      (kamiType != "PREACHER" && kamiType != "GUARDIAN")) {
    nlwarning("ai_kami_type property not found in kami record: %s",
              treeNode->fullName().c_str());
    return;
  }
  ...
  if (!prim->getPropertyByName("sheet", sheet)) {
    nlwarning("'sheet' property not found in kami record: %s",
              treeNode->fullName().c_str());
    return;
  }
  ...
  // do the business
  LOG("Adding kami npc bot: %s: %s  pos: (%d,%d)  orientation: %.2f",
      kamiType.c_str(), name.c_str(), x, y, theta);
  CAIActions::begin(treeNode->getAlias());
...
static void parsePrimGrpKami(const CAIAliasDescriptionNode *treeNode,
                             const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  // run through the group children looking for nodes with types that we
  // recognise
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    // get a pointer to the child and make sure its valid
    const IPrimitive *child;
    if (prim->getChild(child, i)) {
      ...
      case AITypeFolder:
        break;
      default:
        nlwarning("Didn't found ai_type when expecting 'BOT_NPC' in "
                  "parsePrimGrpKami");
        break;
      }
    }
...
static void parsePrimBotKaravan(const CAIAliasDescriptionNode *treeNode,
                                const IPrimitive *prim) {
  nlassert(false);
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  ...
  if (!prim->getPropertyByName("ai_karavan_type", karavanType)) {
    nlwarning("ai_karavan_type property not found in karavan record: %s",
              treeNode->fullName().c_str());
    return;
  }
  ...
  std::string sheet;
  if (!prim->getPropertyByName("sheet", sheet)) {
    nlwarning("'sheet' property not found in karavan record: %s",
              treeNode->fullName().c_str());
    return;
  }
  ...
  LOG("Adding karavan npc bot: %s: %s  pos: (%d,%d)  orientation: %.2f",
      karavanType.c_str(), name.c_str(), x, y, theta);
...
static void parsePrimGrpKaravan(const CAIAliasDescriptionNode *treeNode,
                                const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimState(const CAIAliasDescriptionNode *treeNode,
                           const IPrimitive *prim, const char *pointsType) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  for (j = 0; j < prim->getNumChildren(); ++j) {
    ...
    if (prim->getChild(child, j)) {
      ...
      case AITypeGrp: {
        ...
        default:
          nlwarning(
              "Don't know how to treat ai_group of type '%s' in primState",
              getName(nodeTypeSpec(child)));
          break;
        }
      } break;
      case AITypeEvent:
      ...
      case AITypeFolder:
        break;
      default:
        nlwarning("Don't know how to treat ai_type '%s'",
                  getName(nodeType(child)));
        break;
      }
    }
  }
...
static void parsePrimPlaces(const CAIAliasDescriptionNode *treeNode,
                            const IPrimitive *prim) {
  // run through the group children looking for nodes with types that we
  // recognise
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      // try to get a type for the child
      TAIType type = nodeType(child);
      if (type != AITypeBadType && type != AITypeFolder) {
        // get hold of the unique id
        uint32 uniqueId = nodeAlias(child);

        // it's one of ours! - so match against the types we recognise
        if (type == AITypePlace || type == AITypeOutpost ||
            type == AITypePlaceFauna) {
          // read the radius
          ...
          if (radius == 0) {
            nlwarning("Ignoring place '%s' because bad radius: '%s' (converted "
                      "to int as %u)",
                      placeName.c_str(), radiusString.c_str(), radius);
            continue;
          }
          //					if
          ...
          //					{
          //						nlwarning("Ignoring
          //place '%s' because bad radius:
          //'%s'",placeName.c_str(),radiusString.c_str()); 						continue;
          //					}
          //
          // read the xy coordinates
          if (child->getNumVector() != 1) {
            nlwarning("Ignoring place '%s' because num points not 1 (%d)",
                      placeName.c_str(), child->getNumVector());
            continue;
          }
          ...
          if (type == AITypePlaceFauna) {
            ...
            LOG("Adding place (type XYRFauna): %s at: (%.0f,%.0f) x %d (%s)",
                placeName.c_str(), child->getPrimVector()->x,
                child->getPrimVector()->y, radius,
                verticalPosToString((TVerticalPos)verticalPos).c_str());

            // add the place to the current context
            CAIActions::begin(uniqueId);
            CAIActions::exec("PLACEXYR", uniqueId, x, y, radius * 1000,
                             verticalPos);
            CAIActions::exec("PLXYRFAF", uniqueId, flagSpawn, flagRest,
                             flagFood); // set flags
            CAIActions::exec("PLXYRFAS", uniqueId, stayTimeMin,
                             stayTimeMax); // set stay times
            CAIActions::exec("PLXYRFAI", uniqueId, index,
                             indexNext); // set indices
            CAIActions::exec("PLXYRFAA", uniqueId, active, timeDriven,
                             timeInterval, dayInterval); // set indices
...
static void parsePopulation(const IPrimitive *prim, std::string &sheet,
                            uint &count) {
  // try to get a type for the child
  std::string type;
  ...
  if (NLMISC::nlstricmp(type, "FAUNA_SPAWN_ATOM") != 0) {
    throw parsePopException(
        std::string("Expected ai_type FAUNA_SPAWN_ATOM but found") + type);
  }
  ...
  if (!prim->getPropertyByName("count", countStr)) {
    throw parsePopException(
        std::string("FAUNA_SPAWN_ATOM failed to find property ''count'"));
  }
  ...
  if (count <= 0) {
    throw parsePopException(
        std::string("FAUNA_SPAWN_ATOM property 'count' invalid: ") + countStr);
  }
}
...
static void parsePrimGrpFaunaSpawn(const CAIAliasDescriptionNode *treeNode,
                                   const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    try {
      ...
      nlwarning("FaunaGroup: %s of %s : %s", nodeName(child).c_str(),
                treeNode->fullName().c_str(), e.what());
    }
  }
...
static void parsePrimGrpFauna(const CAIAliasDescriptionNode *treeNode,
  ...
  /*	std::string faunaTypeStr;
      ...
      if	(
     getType<TFaunaType>(faunaTypeStr.c_str())==FaunaTypeBadType	)
      {
          nlwarning("Ignoring group %s due to unknown type:
     '%s'",name.c_str(),faunaTypeStr.c_str()); return;
      }
  */
  // open the parse context
  ...
  //		{
  //			if (name=="FAUNA_FOOD")
  //feedPlace=treeNode->getChild(i)->getAlias(); 			if (name=="FAUNA_REST")
  //restPlace=treeNode->getChild(i)->getAlias(); 			if (name=="FAUNA_SPAWN")
  //spawnPlace=treeNode->getChild(i)->getAlias();
  //		}
  //
  //	}
  ...
  // time variables used for determining time spent eating, rate of progress of
  // hunger, etc
  std::string s;
  ...
  // time variables used for determining time corpses stay on ground and time
  // before creatures respawn
  s.clear();
  prim->getPropertyByName(
      "spawn_times", s); // This code is a nasty hack to provide legacy support
  if (s.empty())         // for .primitive files generated with a defective
    prim->getPropertyByName(
        "Spawn_times", s); // world_editor.xml (case problem in 'spawn_times')
  if (!s.empty()) {
  ...
  for (uint j = 0; j < prim->getNumChildren(); ++j) {
    ...
    case AITypeGrpFaunaPop:
      parsePrimGrpFaunaSpawn(nextTreeNode(nextTreeNode(treeNode, prim), prim),
                             child);
      break;
    case AITypeEvent:
      parsePrimEvent(nextTreeNode(treeNode, child), child);
...
static void parsePrimMgrFauna(const std::string &mapName,
                              const CAIAliasDescriptionNode *treeNode,
                              const IPrimitive *prim,
                              std::vector<SFolderRef> &folders,
  ...
  if (firstTime) {
    CAIActions::begin(treeNode->getAlias());
    CAIActions::exec("MGRFAUNA", treeNode->getAlias(), treeNode->getName(),
                     mapName, filename);
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    case AITypeBadType:
      parsePrimMgrFauna(mapName, nextTreeNode(treeNode, child), child, folders,
                        filename, false);
      break;
    case AITypeFolder:
      break;
    default:
      nlwarning("Found ai_type: '%s' when expecting 'GROUP_FAUNA'",
                getName(type));
      break;
    }
  }
...
static void parsePrimNPCPunctualState(const CAIAliasDescriptionNode *treeNode,
                                      const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimMgrKami(const std::string &mapName,
                             const CAIAliasDescriptionNode *treeNode,
                             const IPrimitive *prim,
                             std::vector<SFolderRef> &folders,
                             const std::string &filename,
  ...
  if (firstTime) {
    CAIActions::begin(treeNode->getAlias());
    CAIActions::exec("MGRNPC", treeNode->getAlias(), treeNode->getName(),
                     mapName, filename);
    CAIActions::exec("IDTREE", treeNode);
  }
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    case AITypeBadType:
      parsePrimMgrKami(mapName, nextTreeNode(treeNode, child), child, folders,
                       filename, false);
      break;
    case AITypeFolder:
      break;
...
static void parsePrimMgrKaravan(const std::string &mapName,
                                const CAIAliasDescriptionNode *treeNode,
                                const IPrimitive *prim,
                                std::vector<SFolderRef> &folders,
                                const std::string &filename,
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeKaravanState:
        ...
        break;
      case AITypeBadType:
        parsePrimMgrKaravan(mapName, nextTreeNode(treeNode, child), child,
                            folders, filename, false);
        break;
      case AITypeFolder:
        break;
      default:
...
void mergeEquipement(const std::vector<std::string> &grpEquip,
                     const std::vector<std::string> &botEquip,
...
static void parsePrimBotNpc(const CAIAliasDescriptionNode *treeNode,
                            const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);

  // setup the set of parameters that are initialised by default in
  // parsePrimGrpNpc() oldlevel 	std::string
  // levelStr; /*	= DefaultBotLevel;	*/
  ...
  const std::vector<std::string> *equipment =
      NULL; //&EmptyStringVector;//	= DefaultBotEquipment;
  ...
  // try to read the above parameters from prim (on failure default value
  // persists) oldlevel 	if (!prim->getPropertyByName("level", levelStr)
  // || levelStr.empty()) oldlevel 		levelStr = DefaultBotLevel;
  ...
  // setup the bots
  LOG("Adding npc bot: %s  pos: (%d,%d)  orientation: %.2f", name.c_str(), x, y,
      theta);
...
static void parsePrimGrpParameters(const CAIAliasDescriptionNode *treeNode,
                                   const IPrimitive *prim) {
  vector<string> *profileParams = &EmptyStringVector;
...
static void parsePrimGrpNpc(const CAIAliasDescriptionNode *treeNode,
  ...
  // oldlevel 	DefaultBotLevel.clear();
  // prim->getPropertyByName("bot_level",DefaultBotLevel);

  DefaultBotLook.clear();
  prim->getPropertyByName("bot_sheet_client", DefaultBotLook);
  //	DefaultBotStats.clear();
  //prim->getPropertyByName("bot_sheet_server",DefaultBotStats);

  DefaultBotKeywords = &EmptyStringVector;
  prim->getPropertyByName("bot_keywords", DefaultBotKeywords);
  DefaultBotEquipment = &EmptyStringVector;
  prim->getPropertyByName("bot_equipment", DefaultBotEquipment);
  DefaultBotChat = &EmptyStringVector;
  prim->getPropertyByName("bot_chat_parameters", DefaultBotChat);
  DefaultMissionNames.clear();
  DefaultMissions.clear();
  lookForMissions(prim, DefaultMissions, DefaultMissionNames);
  ...
  for (uint j = 0; !foundBots && j < prim->getNumChildren(); ++j) {
    ...
    const IPrimitive *child;
    if (prim->getChild(child, j)) {
      // try to get a type for the child - we're looking for children of type
      // 'NPC_BOT'
      TAIType type = nodeType(child);
      if (type != AITypeBadType && type == AITypeBot)
        foundBots = true;
    }
  }
  ...
  if (foundBots && botCount != 0) {
    nlwarning("Mixing of automatic and explicit bot is not supported, only "
              "explicit bot will be spawned ! (in '%s')",
              treeNode->fullName().c_str());
  }
  // if we didn't find any explicit bots then generate a set of 'default' bots
  if (!foundBots) {
    if (botCount > 200 || s != NLMISC::toString(botCount)) {
      nlwarning("Invalid 'count' (value: '%s') parameter in NPC bot group: %s",
                s.c_str(), treeNode->fullName().c_str());
      return;
    }
    if (botCount == 0) {
      if (!DefaultBotLook.empty()) {
        nlwarning("No bots found in NPC group: %s",
                  treeNode->fullName().c_str());
        return;
      }
    }
    ...
    for (uint i = 0; i < botCount; ++i) {
      ...
      CAIActions::exec("STATS"); //,	level	);	//
                                 //+NLMISC::toString("_lvl_%02d",level)
    ...
    CAIActions::exec("BOTCOUNT", 0); // tell the system that we are not using
                                     // auto generated bots but named bots
...
/*static void parsePrimNPCPunctualState(const CAIAliasDescriptionNode
*treeNode,const IPrimitive *prim)
{
    // get hold of the node name and unique id
    std::string name=nodeName(prim);
    uint32 uniqueId=nodeAlias(prim);
    ...
    // run through the group children looking for nodes with types that we
recognise for (uint i=0;i<prim->getNumChildren();++i)
    {
        // get a pointer to the child and make sure its valid
        const IPrimitive *child;
        if (prim->getChild(child,i))
        {
                ...
                &&	type!=AITypeFolder)
            {
                if (type==AITypeNpcStateProfile)
                    ...
                    nlwarning("Don't know how to treat ai_type
'%s'",getName(type));
            }
        }
    }
...
*/
static void parsePrimMgrNpc(const std::string &mapName,
                            const CAIAliasDescriptionNode *treeNode,
                            const IPrimitive *prim,
                            std::vector<SFolderRef> &folders,
                            const std::string &filename,
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeBadType:
        if (nodeClass(child) != "alias")
          parsePrimMgrNpc(mapName, nextTreeNode(treeNode, child), child,
                          folders, filename, false);
        break;
      case AITypeFolder:
        break;
      default:
        nlwarning("unrecognised ai_type in NPC manager %s: '%s'",
                  treeNode->fullName().c_str(), getName(nodeType(child)));
        break;
      }
    }
  }
  ...
  if (firstTime) {
    ...
    for (uint i = 0; i < folders.size(); ++i)
      parsePrimMgrNpc(mapName, folders[i].Node, folders[i].Prim, folders,
                      filename, false);
    // close the manger
    CAIActions::end(treeNode->getAlias());
  }
...
static void parsePrimMgrOutpost(std::string const &mapName,
                                CAIAliasDescriptionNode const *treeNode,
                                IPrimitive const *prim,
                                std::vector<SFolderRef> &folders,
  ...
  if (firstTime) {
    ...
    CAIActions::begin(treeNode->getAlias());
    CAIActions::exec("MGROUTPO", treeNode->getAlias(), treeNode->getName(),
                     mapName, filename, manualSpawn);
    CAIActions::exec("IDTREE", treeNode);
  }
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeBadType:
        if (nodeClass(child) != "alias")
          parsePrimMgrOutpost(mapName, nextTreeNode(treeNode, child), child,
                              folders, filename, false);
        break;
      case AITypeFolder:
        break;
      default:
        nlwarning("unrecognised ai_type in outpost manager %s: '%s'",
                  treeNode->fullName().c_str(), getName(nodeType(child)));
        break;
      }
    }
  ...
  if (firstTime) {
    // now add the folders to the manager
    for (uint i = 0; i < folders.size(); ++i)
      parsePrimMgrOutpost(mapName, folders[i].Node, folders[i].Prim, folders,
                          filename, false);
    // close the manger
    CAIActions::end(treeNode->getAlias());
  }
...
static void parsePrimMgr(const IPrimitive *prim, const std::string &mapName,
                         const std::string &filename) {
  ...
  if (mgrType == MgrTypeBadType) {
    nlwarning("Ignoring manager due to unknown type: '%s'",
              mgrTypeName.c_str());
    return;
  }
  ...
  //	switch (mgrType)
  //	{
  //		case MgrTypeFauna:		typeSpec = AITypeSpecFauna;
  //break; 		case MgrTypeKaravan:	typeSpec = AITypeSpecKaravan;	break; 		case
  //MgrTypeKami:		typeSpec = AITypeSpecKami;		break;
  ////		case MgrTypeTribe:		typeSpec = AITypeSpecTribe;
  ///break;
  //		case MgrTypeNpc:		typeSpec = AITypeSpecNpc;
  //break;
  //
  //		// TODO: case MgrTypePet:
  //		// TODO: case MgrTypeGuildNpc:
  //
  //		default: nlwarning("Unhandled ai manager type:
  //'%s'",mgrTypeName.c_str());
  //	}
  ...
  NLMISC::CSmartPtr<CAIAliasDescriptionNode> aliasTree;
  {
    ...
    aliasTree =
        new CAIAliasDescriptionNode(name, uniqueId, AITypeManager, NULL);
    buildAliasTree(aliasTree, aliasTree, prim, folders);
  }

  switch (mgrType) {
  case MgrTypeFauna:
    parsePrimMgrFauna(mapName, aliasTree, prim, folders, filename);
    break;
  case MgrTypeKaravan:
    parsePrimMgrKaravan(mapName, aliasTree, prim, folders, filename);
    break;
  case MgrTypeKami:
    parsePrimMgrKami(mapName, aliasTree, prim, folders, filename);
    break;
    //		case MgrTypeTribe:
    //parsePrimMgrTribe(mapName,aliasTree,prim,folders);
    //break;
  case MgrTypeNpc:
    parsePrimMgrNpc(mapName, aliasTree, prim, folders, filename);
    break;
  case MgrTypeOutpost:
    parsePrimMgrOutpost(mapName, aliasTree, prim, folders, filename);
    break;
  ...
  default:
    nlwarning("Unhandled ai manager type: '%s'", mgrTypeName.c_str());
  }
...
// parsing spires
/*
static void parsePrimMgrSpire(std::string const& mapName,
CAIAliasDescriptionNode const* treeNode, IPrimitive const* prim,
std::vector<SFolderRef>& folders, std::string const& filename, bool
firstTime=true)
{
    uint32 uniqueId = nodeAlias(prim);
    ...
    // run through the mgr children looking for nodes with types that we
recognise for (uint i=0; i<prim->getNumChildren(); ++i)
    {
        // get a pointer to the child and make sure its valid
        ...
        if (prim->getChild(child, i))
        {
            ...
            case AITypeEvent:
                ...
                break;
            default:
                nlwarning("unrecognised ai_type in NPC manager %s:
'%s'",treeNode->fullName().c_str(),getName(nodeType(child))); break;
            }
        }
    }
...
*/
static void parsePrimSpire(IPrimitive const *prim, std::string const &mapName,
                           std::string const &filename) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  //		return;
  //	}
  if (!prim->getPropertyByName("sheet_socle", sheet_socle) ||
      sheet_socle.empty())
    sheet_socle = DefaultBotLook;
  if (!prim->getPropertyByName("sheet_spire", sheet_spire) || !sheet_spire ||
      sheet_spire->empty())
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeEvent:
        ...
        break;
      default:
        nlwarning("unrecognised ai_type in spire %s: '%s'", name.c_str(),
                  getName(nodeType(child)));
        break;
      }
    }
...
static void parsePrimDynFaunaZone(const CAIAliasDescriptionNode *aliasNode,
                                  const IPrimitive *prim) {
  float x, y, r;
...
static void parsePrimDynNpcZonePlace(const CAIAliasDescriptionNode *aliasNode,
                                     const IPrimitive *prim) {
  float x, y, r;
  x = prim->getPrimVector()->x;
  y = prim->getPrimVector()->y;
...
static void parsePrimDynNpcZoneShape(const CAIAliasDescriptionNode *aliasNode,
                                     const IPrimitive *prim) {
  uint32 verticalPos;
  parseVerticalPos(prim, verticalPos);
...
static void parsePrimRoadTrigger(const CAIAliasDescriptionNode *aliasNode,
                                 const IPrimitive *prim) {
...
static void parsePrimDynRoad(const CAIAliasDescriptionNode *aliasNode,
                             const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimGeomItems(const CAIAliasDescriptionNode *aliasNode,
                               const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimCell(const CAIAliasDescriptionNode *aliasNode,
                          const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
// void	parsePrimCellZoneEnergy(const CAIAliasDescriptionNode *aliasNode, const
// IPrimitive *prim)
//{
//	std::string	family;
//	std::string	energy;
...
static void parsePrimCellZone(const CAIAliasDescriptionNode *aliasNode,
                              const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimCellZones(const CAIAliasDescriptionNode *aliasNode,
                               const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimBotTemplate(const CAIAliasDescriptionNode *aliasNode,
                                 const IPrimitive *prim,
                                 std::string const &familyType) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  if (nodeClass(prim) == "bot_template_npc") {
    ...
    if (!lookSheet.empty()) {
      ...
      // oldlevel 			lookSheet =
      // lookSheet+"_lvl_"+toString("%02u", level)+".creature";
    }
  }

  CAIActions::begin(aliasNode->getAlias());
  CAIActions::exec("BOTTMPL" + familyType, aliasNode, lookSheet,
                   multiLevel); //, caracSheet);
...
static void parsePrimGroupTemplate(const CAIAliasDescriptionNode *aliasNode,
                                   const IPrimitive *prim,
                                   std::string const &familyType) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  ...
  CAIActions::begin(aliasNode->getAlias());
  CAIActions::exec("GRPTMPL", aliasNode, CurrentGroupFamily, botCount,
                   countMultipliedBySheet, multiLevel);
  if (nodeClass(prim) == "squad_template_variant") {
    ...
    CAIActions::execute("IDTREE_F",
                        *botSheets); // create the bot descs as well (group
                                     // descs created in exec("GRPTMPL"))
  }
  CAIActions::exec("GT_SHEE" + familyType, lookSheet);
  CAIActions::exec("GT_LVLD" + familyType, levelDelta);
  CAIActions::exec("GT_SEAS" + familyType, seasons[0], seasons[1], seasons[2],
                   seasons[3]);
  CAIActions::exec("GT_NRG" + familyType, weights[0], weights[1], weights[2],
                   weights[3]);
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeFaunaSpawnAtom: {
        ...
        try {
          ...
          nlwarning("FaunaGroup: %s of %s : %s", nodeName(child).c_str(),
                    aliasNode->fullName().c_str(), e.what());
        }
      } break;
      default:
        break;
...
static void
parsePrimGroupFamilyProfileFauna(const CAIAliasDescriptionNode *aliasNode,
                                 const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
...
static void
parsePrimGroupFamilyProfileTribe(const CAIAliasDescriptionNode *aliasNode,
                                 const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
...
static void
parsePrimGroupFamilyProfileNpc(const CAIAliasDescriptionNode *aliasNode,
                               const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  string familyTag;
...
static void parsePrimGroupDescriptions(const CAIAliasDescriptionNode *aliasNode,
                                       const IPrimitive *prim) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimDynRegion(const CAIAliasDescriptionNode *aliasNode,
                               const IPrimitive *prim,
                               const std::string &filename) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
...
/*
static void parsePrimOutpostCharge(const CAIAliasDescriptionNode *aliasNode,
const IPrimitive *prim)
{
    // get hold of the node name and unique id
    std::string name=nodeName(prim);
...
*/
// static void parsePrimOutpostSquadFamily(const CAIAliasDescriptionNode
// *aliasNode, const IPrimitive *prim)
//{
//	// get hold of the node name and unique id
//	std::string name = nodeName(prim);
////	uint32 uniqueId = nodeAlias(prim);
...
static void parsePrimOutpostSpawnZone(const CAIAliasDescriptionNode *aliasNode,
                                      const IPrimitive *prim) {
  float x, y, r;
  x = prim->getPrimVector()->x;
  y = prim->getPrimVector()->y;
...
static void parsePrimOutpostBuilding(CAIAliasDescriptionNode const *aliasNode,
                                     IPrimitive const *prim) {
  ...
  CAIActions::exec("STARTPOS", (sint32)(x * 1000.f), (sint32)(y * 1000.f),
                   theta, verticalPos);
...
static void
parsePrimSquadTemplateVariant(const CAIAliasDescriptionNode *aliasNode,
                              const IPrimitive *prim,
...
static void parsePrimSquadTemplate(const IPrimitive *prim,
  ...
  // build the tree of aliases and the vector of folders
  std::vector<SFolderRef> folders;
  NLMISC::CSmartPtr<CAIAliasDescriptionNode> aliasNode =
      new CAIAliasDescriptionNode(name, uniqueId, AITypeOutpost, NULL);
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      ...
      case AITypeSquadTemplateVariant:
        parsePrimSquadTemplateVariant(nextTreeNode(aliasNode, child), child,
                                      name);
        break;
      default:
        break;
...
static void parsePrimOutpost(const IPrimitive *prim, const std::string &mapName,
                             const std::string &filename) {
  // get hold of the node name and unique id
  string name = nodeName(prim);
  uint32 uniqueId = nodeAlias(prim);
  ...
  // link squads
  const char *props[] = {"tribe_squads", "tribe_squads2", "default_squads",
                         "buyable_squads"};
  ...
  for (size_t i = 0; i != nprops; ++i) {
    ...
    if (prim->getPropertyByName(props[i], propSquads) && propSquads) {
      ...
      squadsToLink.insert(squadsToLink.end(), (*propSquads).begin(),
                          (*propSquads).end());
      CAIActions::execute("OUTP_SQD", squadsToLink);
    } else
      nlerror("Missing property %s in %s in %s", props[i], name.c_str(),
              filename.c_str());
  }
  ...
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    ...
    if (prim->getChild(child, i)) {
      switch (nodeType(child)) {
        ...
        //				parsePrimOutpostSquadFamily(nextTreeNode(aliasNode,child),
        //child); 				break;
...
static void parsePrimDynSystem(const IPrimitive *prim,
                               const std::string &mapName,
                               const std::string &filename) {
  // get hold of the node name and unique id
  std::string name = nodeName(prim);
  ...
  nlassertex(!contName.empty(), ("Error while loading dynamic system from "
                                 "'%s', the continent name is empty !",
                                 filename.c_str()));
  // build the tree of aliases and the vector of folders
  std::vector<SFolderRef> folders;
  NLMISC::CSmartPtr<CAIAliasDescriptionNode> aliasNode =
      new CAIAliasDescriptionNode(name, uniqueId, AITypeDynamicSystem, NULL);
  buildAliasTree(aliasNode, aliasNode, prim, folders);
...
static void parsePrimNogoPointList(const IPrimitive *prim,
                                   const std::string &mapName,
                                   const std::string &filename) {
  // get hold of the node name and unique id
  string name = nodeName(prim);
  ...
  // run through the charge children looking for nodes with types that we
  // recognise
  for (uint i = 0; i < prim->getNumChildren(); ++i) {
    // get a pointer to the child and make sure its valid
    const IPrimitive *child;
...
static void parsePrimSafeZone(const IPrimitive *prim,
                              const std::string &mapName,
                              const std::string &filename) {
...
static void parsePrimScript(const IPrimitive *prim, const std::string &mapName,
                            const std::string &filename) {
  string primName;
  ...
  if (pcode != NULL && !pcode->empty()) {
    std::vector<std::string>::const_iterator it = pcode->begin(),
                                             itEnd = pcode->end();
    code = *it;
    ++it;
    for (; it != itEnd; ++it) {
...
/**
 * parse a user model
 * a user model contains an id, a base SheetId (used later to fill unmodified
 * attributes of the dynamic sheet) and a script defining all attributes to be
 * modified along with the new values
 */
    ...
    for (uint j = 0; j < prim->getNumChildren(); ++j) {
      ...
      if (type == AITypeUserModel) {
        ...
        if (pcode != NULL && !pcode->empty()) {
          ...
          for (std::vector<std::string>::const_iterator it = start;
               it != pcode->end(); ++it) {
            if (it != start) {
              script += "\n";
            }
        ...
        nldebug("<ParsePrimUserModelList> Add user model '%s'",
                userModelId.c_str());
      }
    }
  } else {
...
/**
 * Parse method called on the main user model node, which is a list of user
 * models builds a CArgs vector and executes an AiAction USR_MDL This aiAction
 * is defined in the CAIUserModelManager class
 */
static void parsePrimUserModelList(const IPrimitive *prim,
  ...
  if (args.size() == 0) {
    nlinfo(
        "<ParsePrimUserModelList> No User Model List found in primitive '%s'",
        filename.c_str());
    return;
  }
 ...
 * A custom loot table contains an id and money values, and a list of custom
 * loot sets. A custom loot set contains a drop probability and a script
 * defining what item it will drop (item name, quantity, quality)
 */
static void parseCustomLootTableRec(const IPrimitive *prim,
  ...
  if (type == AITypeCustomLootTable) {
    ...
    float moneyFactor =
        static_cast<float>(strtod(strMoneyFactor.c_str(), &ptr));
    if (ptr != NULL && *ptr == '\0' && errno != ERANGE) {
      args.push_back(CAIActions::CArg(moneyFactor));
    }

    uint32 moneyBase =
        static_cast<uint32>(strtol(strMoneyBase.c_str(), &ptr, 10));
    if (ptr != NULL && *ptr == '\0' && errno != ERANGE) {
    ...
    nldebug("<ParseCustomLootTable> Add custom loot table'%s'",
            customTableId.c_str());
  } else {
    for (uint i = 0; i < prim->getNumChildren(); ++i) {
      const IPrimitive *child;
...
/**
 * Main parsing method used to build the CArgs vector containing the custom loot
 * tables data defined in the primitive. Called on the main node which is a list
 * of custom loot tables Once the parsing is done and the CArgs vector is built,
 * an AiAction CUSTOMLT (CustomLootTables) is executed. This aiAction is defined
 * in the CAIUserModelManager class
 */
static void parsePrimCustomLootTable(const IPrimitive *prim,
  ...
  if (type == AITypeCustomLootTables) {
    ...
    if (size == 0) {
      nlinfo("<ParsePrimCustomLootTable> Custom loot tables folder declared "
             "but empty in primitive '%s'",
             filename.c_str());
      return;
    }
    ...
    // TODO: add the primitive aliasStaticPart in the vector in order to remove
    // all userModels/custom loot table from AIS and EGS managers when a
    // primitive is unloaded
...
static void parsePrim(const IPrimitive *prim, const std::string &mapName,
                      const std::string &filename) {
  AITYPES::TAIType type = nodeType(prim);
...
//	std::vector<std::string>::const_iterator first(mapNames.begin()),
//last(mapNames.end()); 	for (; first != last; ++first)
//	{
//		const std::vector<std::string> &primitives =
//CPrimitiveCfg::getMap(*first); 		std::vector<std::string>::const_iterator
//first2(primitives.begin()), last2(primitives.end()); 		for (; first2 != last2;
//++first2)
//		{
//			TFileInfo fi;
//			fi.MapName = *first;
...
void parsePrimNoStream(CPrimitives *primDoc, const std::string &streamName) {
  ...
  CAIActions::openFile(
      streamName); // AJM: note this does not open a file or stream
  ...
  // do the real parsing work
  {
    ...
    parsePrimUserModelList(
        (IPrimitive *)primDoc->RootNode,
    ...
    parsePrim(
        ...
        CPrimitiveCfg::getContinentNameOf(streamName) + ":" +
            CFile::getFilenameWithoutExtension(CFile::getFilename(streamName)),
        CFile::getFilename(CFile::getFilenameWithoutExtension(
            CFile::getFilename(streamName))));
  }

  CPrimitiveContext::instance().CurrentPrimitive = NULL;

  // allow for action executor housekeeping
  CAIActions::closeFile(
      streamName); // AJM: note this does not close a file or a stream
}
...
void parsePrimFile(const std::string &filename) {
  ...
  if (fileIn.open(filename)) {
    nlinfo("Opening %s in %s", filename.c_str(),
  ...
  nlinfo("Load&parse primitive file '%s' (in '%s')",
         CFile::getFilename(filename).c_str(), sFilename.c_str());

  // Test if binary caching is wanted
  bool cachePrims = true;
  CConfigFile::CVar *cachePrimsVar =
      IService::getInstance()->ConfigFile.getVarPtr("CachePrims");
  if (cachePrimsVar) {
    cachePrims = cachePrimsVar->asInt() != 0;
  }
  bool cachePrimsLog = false;
  CConfigFile::CVar *cachePrimsLogVar =
      IService::getInstance()->ConfigFile.getVarPtr("CachePrimsLog");
  if (cachePrimsLogVar) {
  ...
  if (opened) {
    ...
    bool readXml = true;
    string binFileName =
        NLNET::IService::getInstance()->WriteFilesDirectory.toString() +
        "primitive_cache/" + CFile::getFilename(filename) + ".binprim";
    if (cachePrims && CFile::fileExists(binFileName) &&
        CFile::getFileModificationDate(binFileName) >
            CFile::getFileModificationDate(sFilename)) {
      if (cachePrimsLog) {
        // ok, the cache is here and up to date !
        nlinfo("Loading '%s' from binary file '%s'", sFilename.c_str(),
               binFileName.c_str());
      }
      try {
    ...
    // if file not found sFilename is the result of CPath::lookup so if not in
    // path sFilename is an empty string
    nlwarning("Failed to open file '%s'  for reading.",
              sFilename.empty() ? filename.c_str() : sFilename.c_str());
...
NLMISC_COMMAND(
    loadPdrFile,
    "load a primitive file. don't forget to call BuildPrimitiveDependencies",
    "<file name>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(loadMapsFromCommon,
               "load all primitive defined in usedPrimitives in common.cfg for "
               "the given map",
               "<map name>") {
  if (args.size() != 1)
    return false;
  ...
  // We create a static set of 'loaded primitive file' names via this method in
  // order to avoid loading the same primitive file more than once at AIS
  // startup
  ...
  {
    CConfigFile::CVar &usedPrimitives =
        IService::getInstance()->ConfigFile.getVar("UsedPrimitives");
    ...
    for (uint i = 0; i < usedPrimitives.size(); ++i) {
      const vector<string> &prims =
          CPrimitiveCfg::getMap(usedPrimitives.asString(i));
      for (uint j = 0; j < prims.size(); ++j) {
        if (filter.find(prims[j]) != filter.end()) {
          // check for primitive filter
          CConfigFile::CVar *pvar =
              IService::getInstance()->ConfigFile.getVarPtr("PrimitiveFilter");
          if (pvar) {
            ...
            for (uint k = 0; k < pvar->size(); ++k) {
              ...
              if (filename.find(nameFilter) != string::npos) {
                log.displayNL("loadMap : loading of primitive '%s' canceled by "
                              "PrimitiveFilter '%s'",
                              filename.c_str(), nameFilter.c_str());
                load = false;
                break;
              }
            }
          ...
          ICommand::execute(toString("loadPrimitiveFile %s", prims[j].c_str()),
                            log);
        }
      }
    }
...
NLMISC_COMMAND(loadMap, "load a complete set of primitive files",
               "<map name>") {
  if (args.size() != 1)
    return false;
  ...
  for (; first != last; ++first) {
    ...
    if (in == std::numeric_limits<uint32>::max()) {
      log.displayNL("loadMap : while loading map '%s', can't load primitive "
                    "'%s' coz continent '%s' is not active",
                    args[0].c_str(), filename.c_str(), continentName.c_str());
    } else {
      ICommand::execute(
          toString("createStaticAIInstance %s", continentName.c_str()), log);
...
NLMISC_COMMAND(unloadMap, "unload a complete set of primitive files",
               "<map name>") {
  if (args.size() != 1)
    return false;
  ...
  if (!continentName.empty()) {
    ...
    for (; first != last; ++first) {
      ...
      uint32 in = uc.getInstanceForContinent(continentName);
      if (in == std::numeric_limits<uint32>::max()) {
        log.displayNL("unloadMap : while loading map '%s', can't load "
                      "primitive '%s' coz continent '%s' is not active",
                      args[0].c_str(), filename.c_str(), continentName.c_str());
      } else
        ICommand::execute(toString("unloadPrimitiveFile %s", filename.c_str()),
                          log);
    }
  } else

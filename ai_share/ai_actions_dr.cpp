...
void CAIActionsDataRecordPtr::clear() { _PdrPtr->clear(); }
...
void CAIActionsDataRecordPtr::serial(NLMISC::IStream &stream) {
  if (!stream.isReading()) {
    ...
    // allocate a buffer to store the data block temporarily and dump data into
    // it
...
void CAIActionsDataRecordPtr::addExecute(
    uint64 action, const std::vector<CAIActions::CArg> &args) {
...
static uint64 stringToInt64(const char *str) {
  ...
  // the following assert ensures that we never try to cram >8 letters into an
  // int64
  nlassert(str[i] == 0);
...
void CAIActionsDataRecordPtr::applyToExecutor(CAIActions::IExecutor &executor) {
  ...
  while (!_PdrPtr->isEndOfData()) {
    ...
    if (_PdrPtr->isStartOfStruct()) {
      if (tokenName == "file") {
        BOMB_IF(!args.empty(), "Found args with unmatched action in pdr",
                return);
        BOMB_IF(contextAlias != ~0u,
                "Found context alias without matching context clause", return);
      ...
      if (tokenName == "context") {
        BOMB_IF(!args.empty(), "Found args with unmatched action in pdr",
                return);
        //	BOMB_IF(!fileName.empty(),"Found file name without matching file
        //clause",return);
    ...
    if (_PdrPtr->isEndOfStruct()) {
      ...
      if (tokenName == "file") {
        BOMB_IF(fileName.empty(),
                "PDR file invalid: found end of file marker but no file open",
                return);
      ...
      if (tokenName == "context") {
        BOMB_IF(contextStack.empty(),
                "PDR file invalid: found more section closes than opens",
                return);
    ...
    if (_PdrPtr->isTokenWithNoData()) {
      BOMB_IF(tokenName.size() > 8,
              "The action name '" + tokenName +
                  "' is invalid - it must be <= 8 characters long",
              return);
    ...
    if (tokenName == "fileName") {
      BOMB_IF(!fileName.empty(),
              "Found a file clause within another file clause!", return);
      BOMB_IF(!args.empty(), "Found args with unmatched action in pdr", return);

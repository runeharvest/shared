...
public:
  ...
  public:
    ...
    CArg(const CAIAliasDescriptionNode *arg) : _type(TypeAliasTree) {
      _a = const_cast<CAIAliasDescriptionNode *>(arg);
    } //	->copyAliasDescriptionNode(); }
    CArg(const CAIEventDescription *arg) : _type(TypeEventTree) {
      _e = const_cast<CAIEventDescription *>(arg);
    }

    ~CArg() {}

#ifdef NL_COMP_VC6
    bool get(sint &arg) const {
      if (_type != TypeInt)
        return false;
      arg = _i;
      return true;
    }
    bool get(uint &arg) const {
      if (_type != TypeInt)
        return false;
      arg = _i;
      return true;
...
#endif
    bool get(bool &arg) const {
      if (_type != TypeBool)
        return false;
      arg = _b;
      return true;
    }
    bool get(sint32 &arg) const {
      if (_type != TypeInt)
        return false;
      arg = _i;
      return true;
    }
    bool get(uint32 &arg) const {
      if (_type != TypeInt)
        return false;
      arg = _i;
      return true;
    }
    bool get(float &arg) const {
      if (_type != TypeFloat)
        return false;
      arg = _f;
      return true;
    }
    bool get(double &arg) const {
      if (_type != TypeFloat)
        return false;
      arg = _f;
      return true;
    }
    bool get(std::string &arg) const {
      if (_type != TypeString)
        return false;
      arg = _s;
      return true;
    }
    bool get(CAIAliasDescriptionNode *&arg) const {
      if (_type != TypeAliasTree)
        return false;
      arg = _a;
      return true;
    }
    bool get(CAIEventDescription::TSmartPtr &arg) const {
      if (_type != TypeEventTree)
        return false;
      arg = _e;
      return true;
    }
    ...
    const CArg &operator=(const CArg &other) {
      ...
      case TypeAliasTree:
        _a = other._a;
        break; //->copyAliasDescriptionNode();	break;
      case TypeEventTree:
        _e = other._e;
        break;
      }
    ...
    std::string toString() const {
      switch (_type) {
      case TypeString:
        return NLMISC::toString(_s);
      case TypeBool:
        return NLMISC::toString(_b);
      case TypeInt:
        return NLMISC::toString(_i);
      case TypeFloat:
        return NLMISC::toString(_f);
        //			case TypeAliasTree:	return
        //_a->treeToString(); 			case TypeEventTree: return _e->toString();
      default:
        break;
      }
    ...
    void serial(NLMISC::IStream &f) {
      ...
      case TypeFloat:
        f.serial(_f);
        break;
        //				case TypeAliasTree:	f.serial(_a);
        //break; 				case TypeEventTree: f.serial(_e);	break;
      default:
        break;
      }
    }
    ...
    void pushToPdr(CPersistentDataRecord &pdr) const {
      switch (_type) {
      case TypeString:
        pdr.push(pdr.addString("string"), _s);
        break;
      case TypeBool:
        pdr.push(pdr.addString("bool"), _b ? (sint32)1 : (sint32)0);
        break;
      case TypeInt:
        pdr.push(pdr.addString("int"), _i);
        break;
      case TypeFloat:
        pdr.push(pdr.addString("float"), _f);
        break;
      case TypeAliasTree:
    ...
    void serialToString(std::string &s) throw() {
      ...
      case TypeAliasTree:
        ...
        // compact string table by removing strings that match tail end of
        // existing strings
        // - use an output buffer - containing all existing strings
        // (directly in output string?)
        // - a buffer index containing indices where strings end (start is
        // implicit :o)
    ...
    static CArg serialFromString(std::string &data, uint &index) throw() {
      ...
      case TypeInt: // ints are stored drectly as binary (beware of byte
                    // order!!!)
        index += sizeof(sint32);
        if (index > data.size()) {
          nlwarning("Unexpected end of input data in serialFromString()");
      ...
      case TypeFloat: // floats are stored directly as binary (beware of byte
                      // order!!!)
        index += sizeof(float);
        if (index > data.size()) {
          nlwarning("Unexpected end of input data in serialFromString()");
  ...
  private:
    ...
    /*
            // I've had to re-code strdup() here as the default one doesn't use
       the
            // same memory manager as free() and so causes crashes in debug
       mode static char *strdup(const char *src)
            {
                // calculate strlen() - *NOTE: i must be signed for use in copy
       (below) sint i; for (i=0;src[i];++i);
                // allocate memory
  ...
  static void init(IExecutor *executor) {
    _init = true;
    _executor = executor;
  }

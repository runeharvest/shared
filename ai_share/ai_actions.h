...
public:
	...
	public:
		...
		CArg(const std::string &arg)
		    : _type(TypeString)
		{
			_s = arg;
		}
		CArg(const CAIAliasDescriptionNode *arg)
		    : _type(TypeAliasTree)
		{
			_a = const_cast<CAIAliasDescriptionNode *>(arg);
		} //	->copyAliasDescriptionNode(); }
		CArg(const CAIEventDescription *arg)
		    : _type(TypeEventTree)
		{
		...
		{
			if (_type != TypeBool) return false;
			arg = _b;
			return true;
		}
		...
		{
			if (_type != TypeFloat) return false;
			arg = _f;
			return true;
		}
		...
		{
			if (_type != TypeString) return false;
			arg = _s;
			return true;
		}
		bool get(CAIAliasDescriptionNode *&arg) const
		{
			if (_type != TypeAliasTree) return false;
			arg = _a;
			return true;
		}
		bool get(CAIEventDescription::TSmartPtr &arg) const
		{
			if (_type != TypeEventTree) return false;
			arg = _e;
			return true;
		}

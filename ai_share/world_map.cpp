...
const struct CDirection::CDirectionData CDirection::directionDatas[] = {
    {+1, 0, ORTHO_COST}, {+1, +1, DIAG_COST}, {0, +1, ORTHO_COST},
    {-1, +1, DIAG_COST}, {-1, 0, ORTHO_COST}, {-1, -1, DIAG_COST},
    {0, -1, ORTHO_COST}, {+1, -1, DIAG_COST}, {0, 0, NO_COST}};
...
inline CABaseStarNode::CABaseStarNode(uint father, float distance, bool open)
    : _Father(father), _Distance(distance), _Open(open) {}
...
public:
  explicit CAStarHeapNode(CTopology::TTopologyRef Ref, uint Father,
                          float Distance, bool Open);
...
inline CAStarHeapNode::CAStarHeapNode(CTopology::TTopologyRef Ref, uint Father,
                                      float Distance, bool Open)
...
public:
  ...
  explicit CAStarNode(CWorldPosition const &pos)
      : _x(pos.xCoord().getUnitId()), _y(pos.yCoord().getUnitId()),
        _slot(pos.slot()) {}
...
public:
  ...
  explicit CInsideAStarHeapNode(const CAStarNode &node, uint Father,
                                CDirection Direction, float Distance, bool Open)
      : CABaseStarNode(Father, Distance, Open), _Direction(Direction),
        _Node(node) {}

  inline const CDirection &getDirection() const { return _Direction; }

  inline const CAStarNode &getNode() const { return _Node; }
...
void CSuperCell::countCells(uint &compute, uint &white, uint &simple,
                            uint &multi, uint &other) const {
  for (uint32 i = 0; i < 16 * 16; ++i) {
    if (!_Grid[i])
      continue;
  ...
  if (f.isReading()) {
    ...
    //	made to calculate some random pos ..
    {
      ...
      for (scan = min; scan.y() != max.y(); scan = scan.stepCell(0, 1)) {
        for (scanline = scan; scanline.x() != max.x();
             scanline = scanline.stepCell(1, 0)) {
          ...
          // if we have found some valid positions but not all, fill the array
          // with the last pos found.
          if (ind < 4 && ind > 0) {
            while (ind < 4) {
              rootCell->setWorldPosition(rootCell->getWorldPosition(ind - 1),
                                         ind);
              ind++;
            }
          }
...
void CWorldMap::setFlagOnPosAndRadius(const CMapPosition &pos, float radius,
                                      uint32 flag) {
...
void CWorldMap::countCells(uint &compute, uint &white, uint &simple,
                           uint &multi, uint &other) const {
  for (uint32 i = 0; i < 65536; ++i) {
    if (_GridFastAccess[i])
  ...
  //			for (j=0; j<256; ++j)
  //				if (_Grid[i][j] != NULL)
  //					_Grid[i][j]->countCells(compute, white,
  //simple, multi, other);
}
...
CNeighbourhood CWorldMap::neighbours(const CWorldPosition &wpos) const {
  ...
  if (lnks.isSSlotValid()) {
    ...
    const CCellLinkage &nlnk =
        wpos.getPosS()
            .getCellLinkage(); // CWorldPosition(wpos.getPosS(),lnks.NSlot()));
...
//
bool CWorldMap::customCheckDiagMove(const CWorldPosition &pos,
                                    const CDirection &direction,
                                    TAStarFlag denyFlags) const {
  ...
  case CDirection::SE: {
    {
      ...
      if ((tmpPos.getTopologyRef().getCstTopologyNode().getFlags() &
           denyFlags) != 0)
        return false;
...

    {
      ...
      //					tmpPos=tmpPos.getPosS();
      //					if
      //((tmpPos.getTopologyRef().getCstTopologyNode().getFlags()&denyFlags)!=0)
      //						return	false;
    }
    return true;
  }
...
/*
    bool	CWorldMap::customCheckDiagMove(const CWorldPosition &pos, const
   CDirection	&direction, TAStarFlag denyFlags) const
    {
        H_AUTO(AI_WorldMap_move);
        // get straight links
        const	CCellLinkage	&lnk = pos.getCellLinkage();
...
//
bool CWorldMap::moveSecure(CWorldPosition &pos, const CDirection &direction,
                           uint16 maskFlags) const {
  ...
  case CDirection::SE: {
    ...
    if (p1.moveS() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1.moveE() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveE() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveS() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1 == p2) {
      pos = p1;
      return true;
    }
  ...
  case CDirection::NE: {
    CWorldPosition p1(pos), p2(pos);
    if (p1.moveN() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1.moveE() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveE() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveN() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1 == p2) {
      pos = p1;
      return true;
    }
  ...
  case CDirection::NW: {
    CWorldPosition p1(pos), p2(pos);
    if (p1.moveN() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1.moveW() && (((p1.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveW() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p2.moveN() && (((p2.getFlags() ^ pflags) & maskFlags) == 0) &&
        p1 == p2) {
      pos = p1;
      return true;
...
//
bool CWorldMap::moveDiagTestBothSide(CWorldPosition &pos,
                                     const CDirection &direction) const {
  H_AUTO(AI_WorldMap_move);
  // get straight links
  const CCellLinkage &lnk = pos.getCellLinkage();
...
void areCompatiblesWithoutStartRestriction(const CWorldPosition &startPos,
                                           const CWorldPosition &endPos,
                                           const TAStarFlag &denyflags,
  ...
  const CTopology &startTopoNode =
      startPos.getTopologyRef().getCstTopologyNode();
  ...
  for (TAStarFlag possibleFlag = Nothing; possibleFlag <= WaterAndNogo;
       possibleFlag =
           (TAStarFlag)(possibleFlag + 2)) //	tricky !! -> to replace with a
                                           //defined list of flags to checks.
  {
    const uint32 incompatibilityFlags =
        (possibleFlag & (denyflags & ~startFlag)) & WaterAndNogo;
    ...
    if ((startMasterTopo ^ endMasterTopo) != 0 ||
        startMasterTopo ==
            std::numeric_limits<uint32>::max()) // if not same masterTopo or
                                                // invalid masterTopo then
                                                // bypass ..
      continue;
    ...
    if (((possibleFlag & denyflags) & WaterAndNogo) ==
...
NLMISC_CATEGORISED_COMMAND(ais, dumpAStarSteps,
  ...
  for (std::map<uint, uint>::const_iterator first = MapAStarNbSteps.begin(),
                                            last = MapAStarNbSteps.end();
       first != last; ++first) {
...
bool CWorldMap::findAStarPath(CWorldPosition const &start,
  ...
  // Get the master topology inside which to compute the path (reminder: no path
  // between different master topo)
  ...
  // :TODO: Check if STL heap is not better suited, or if another data structure
  // would be more useful in AIS
  ...
  // Add start topology to visited nodes (father holds start topo node index for
  // the moment)
  ...
  while (!heap.empty()) {
#ifdef CHECK_HEAP
    if (heap.size() >
        maxHeap) // if too much calculs, not found (to remove when debugged).
      break;
#endif
    ...
    father =
        std::numeric_limits<uint>::max(); // :TODO: Remove that useless
                                          // statement (since do while first
                                          // loop ALWAYS overwrite it)
    do {
      father = heap.pop();
    ...
    for (vector<CTopology::CNeighbourLink>::const_iterator
             it = ctp.Neighbours.begin(),
             itEnd = ctp.Neighbours.end();
         it != itEnd; ++it) {
      ...
      if (next.getCstTopologyNode().getMasterTopo(movementFlags) !=
          choosenMasterTopo)
        continue;
...
#else
  if (!found) {
    nlwarning("(!!Appeler StepH!!)Path not found from %s : %d to %s : %d",
              start.toString().c_str(), start.slot(), end.toString().c_str(),
              end.slot());
  }
#endif
...
// Finds an A* path
bool CWorldMap::findAStarPath(const CTopology::TTopologyId &start,
                              const CTopology::TTopologyId &end,
                              CAStarPath &path, TAStarFlag denyflags) const {
  ...
  while (!heap.empty()) {
    ...
    for (uint i = 0; i < ctp.Neighbours.size(); ++i) {
      ...
      // if node is open or closed and previous visit has shorter path, skip
      // node
      map<CTopology::TTopologyId, uint>::iterator itv = visited.find(next);
      if (itv != visited.end() &&
...
bool CWorldMap::findInsideAStarPath(CWorldPosition const &start,
                                    CWorldPosition const &end,
  ...
  nodes.push_back(
      CInsideAStarHeapNode(startNode, 0xffffffff, CDirection(), 0.f, true));
  // Take it as first father
  uint father = (uint)nodes.size() - 1;

  // Add start node to visited nodes (father holds start node index for the
  // moment)
  ...
  while (!heap.empty()) {
    ...
    CMapPosition pos = start; // The map has the same cell than the start pos
                              // (coz in same topo)
    ...
    for (uint i = 0; i < 8; ++i) {
      ...
      if ((denyflags & mv.getTopologyRef().getCstTopologyNode().getFlags()) !=
          0)
        continue;
      ...
      // If node has already been visited and previous distance was better skip
      // it
      ...
      // Else create a new node
        ...
        child = (uint)nodes.size();
        nodes.push_back(
            CInsideAStarHeapNode(next, father, dir, distance, true));
      }
      // Compute h(n) as an euclidian distance heuristic
...
bool CWorldMap::moveTowards(CWorldPosition &pos,
...
bool CWorldMap::move(CWorldPosition &pos, CAStarPath &path,
...
bool CWorldMap::move(CWorldPosition &pos, CMapPosition const &end,
                     TAStarFlag const denyFlags) const {
  CWorldPosition tempPos(pos);
  BOMB_IF((tempPos.getTopologyRef().getCstTopologyNode().getFlags() &
           denyFlags) != 0,
          "Error in CWorldMap::mode, invalid flag "
              << RYAI_MAP_CRUNCH::toString(
                     tempPos.getTopologyRef().getCstTopologyNode().getFlags())
              << "on world pos " << pos.toString() << " while going to map pos "
              << end.toString() << " with denyflags "
              << RYAI_MAP_CRUNCH::toString(denyFlags),
          return false);
  ...
  for (scan = min; scan.yCoord() != max.yCoord(); scan = scan.stepCell(0, 1)) {
    for (scanline = scan; scanline.x() != max.x();
      ...
      for (i = 0; i < topologies.size(); ++i) {
        ...
        for (y = -16; y < 32; ++y) {
          for (x = -16; x < 32; ++x) {
            ...
            for (slot = 0; slot < 3; ++slot) {
              ...
              // if current topo is not a neighbour of checked topo, go to the
              // next pos
              ...
              do {
                ...
                if (allow != 0) {
                  nlwarning(
                      "Unallowed move from (%04X,%04X,%d-topo=%08X) to topo "
                      "%08X, fromflags=%04X, toflags=%04X, cflags=%04X",
                      cpos.x(), cpos.y(), slot, ctopoId.getVal(), tref.getVal(),
                      fromflags, toflags, cflags);
                  failed = true;
                }
              } while (moveTowards(cwpos, tref) && cwpos.isValid() &&
                       numSteps++ < 128);

              if (!found) {
                nlwarning("Failed to go from (%d,%d,%d-topo=%08X) to topo %08X",
                          cpos.x(), cpos.y(), slot, ctopoId.getVal(),
                          tref.getVal());
              }
            }
          }
        ...
        if (failed) {
          nlinfo("---- scanline = %04X %04X topo = %d ----", scanline.x(),
                 scanline.y(), i);
...
void CWorldMap::buildMasterTopo(bool allowWater, bool allowNogo) {
    ...
    for (scanline = scan; scanline.x() != max.x();
      ...
      for (i = 0; i < topos.size(); ++i) {
        ...
        if (!tp.isCompatible(allowWater, allowNogo) ||
            tp.getMasterTopo(allowWater, allowNogo) !=
  ...
  nlinfo("Built %d master topologies (%d topos tested)", masterTopo,
         totalTopos);
}
...
void CWorldMap::countSuperTopo() {
  ...
  nlinfo("Found %d super topologies (%d topos tested, %d topos visited)",
         superTopo, totalTopos, visited.size());
}

bool CWorldMap::setWorldPosition(sint32 z, CWorldPosition &wpos,
                                 const CAIVector &pos,
                                 const CRootCell *originCell) const {
  CSlot slot;
  ...
  {
    // :KLUDGE: Water hack
    // If error is too big and player is either not in water or under slot (ie
    // not floating above slot at surface)
    if (minDistZ > 2000 &&
        ((wpos.getFlags() & RYAI_MAP_CRUNCH::Water) == 0 || z < bestZ)) {
      return false;
    }
  }
  return true;
...
bool CWorldMap::setWorldPosition(double z, CWorldPosition &wpos,
                                 const CAIVector &pos,
                                 const CRootCell *originCell) const {
  nlassert(false && "Double version of setWorldPosition isn't tested !!! "
                    "Verify it's the same than above sint32 version !");
  CSlot slot;
  const CMapPosition mapPos(pos);
  ...
  for (uint32 s = 0; s < 3; ++s) {
    ...
    CSlot sslot = CSlot(s);
    double sh =
        ((double)cell->getMetricHeight(CWorldPosition(cell, mapPos, sslot))) /
        1000.0;
  ...
  if (minDistZ > 2.000) {
    //	nldebug("Setting a WorldPosition too far from specified z: x=%d y=%d
    //z=%f slotz=%f", pos.x(), pos.y(), z, bestZ);
    wpos = CWorldPosition(cell, mapPos, bestSlot, true);
    return false;
  }
  wpos = CWorldPosition(cell, mapPos, bestSlot);

...
public:
  ...
  CTmpPropertyZone() : verticalPos(AITYPES::vp_auto), Target(All) {}
...
public:
  ...
  void pushToPdr(CPersistentDataRecord &pdr) const {
    ...
    if (Weight != 1)
      pdr.push(pdr.addString("weight"), Weight);
    if (Alias != 0)
      pdr.push(pdr.addString("alias"), Alias);

    for (uint32 i = 0; i < Args.size(); ++i)
      pdr.push(pdr.addString("arg"), Args[i]);

    // Note I haven't implemented property zones for now so make sure that we
    // don't need them!

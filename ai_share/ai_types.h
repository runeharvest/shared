...
//	always create a badtype at the end of lists ( see others implementations
//as examples ). 	don't forget to fill the .cpp part .. :)
...
template <class T> class CDescType {
public:
...
template <class T> class CTypeSpecialization {};
...
template <class type> extern type getType(const char *s) {
  ...
  do {
    ...
    index++;
  } while (CDescType<type>::_entries[index]._value !=
           (type)CTypeSpecialization<type>::BadType);
...
enum TEffectType {
  ...
  //		EffectCurseBlind,
  //		EffectCurseFear,
  //		EffectCurseForbidItem,		//	disable active magic
  //item use. 		EffectCurseHatredHomin, 		EffectCurseHatredFauna,
...
//
//		TPopulationFamily(TFamilyTag familyTag = family_bad,
//NLMISC::TStringId tribeName = NLMISC::CStringMapper::emptyId()) 			:
...
//useful ? 				return	TribeName < other.TribeName; 			return	false;
//		}
//
//		const std::string &getFamilyName() const;
//
//		static const std::vector<std::pair<std::string,
//NLMISC::TStringId> > &getTribesNames(); 		static void
//getFamilyNames(std::vector<std::string> &result); 		static const std::string
//&toString(TFamilyTag familyTag);
//
//	private:
//		static void init();
//		static std::vector<std::pair<std::string, NLMISC::TStringId> >
//_TribeNames;
//	};

enum TVerticalPos { vp_auto, vp_upper, vp_middle, vp_lower };
 ...
 */
template <class T> class CPropertySetWithExtraList : public CPropertySet {
public:
  /// Add to the set of additional properties (silent if adding a property that
  /// is already present)
...
inline CPropertyId::CPropertyId() : _Id(0) {}
inline CPropertyId::CPropertyId(std::string const &str)
    : _Id(NLMISC::CStringMapper::map(str)) {}
inline CPropertyId::CPropertyId(CPropertyId const &other) : _Id(other._Id) {}
inline CPropertyId::CPropertyId(NLMISC::TStringId id) : _Id(id) {}
inline bool CPropertyId::operator<(CPropertyId const &other) const {
  return _Id < other._Id;
}
inline bool CPropertyId::operator==(CPropertyId const &other) const {
  return _Id == other._Id;
}
inline CPropertySet CPropertyId::operator+(CPropertyId const &other) const {
  CPropertySet set(*this);
  set.addProperty(other);
  return set;
}

inline bool CPropertyId::empty() const { return _Id != 0; }

inline CPropertyId CPropertyId::create(std::string const &str) {
  return NLMISC::CStringMapper::map(str);
}

inline std::string CPropertyId::toString() const {
  return NLMISC::CStringMapper::unmap(_Id);
}
inline NLMISC::TStringId CPropertyId::toStringId() const { return _Id; }

inline TProperty::TProperty(CPropertyId const &property, size_t id)
    : _Property(property), _Id(id) {}
inline TProperty::TProperty(TProperty const &property)
    : _Property(property._Property), _Id(property._Id) {}

inline bool TProperty::operator<(TProperty const &property) const {
...
inline bool TProperty::operator==(CPropertyId const &property) const {
  return _Property == property;
}

inline std::string TProperty::toString() const { return _Property.toString(); }

inline CPropertySet::CPropertySet(CPropertyId const &property) {
  addProperty(property);
}
inline CPropertySet::CPropertySet(std::string const &str) { addProperty(str); }
inline CPropertySet::CPropertySet() {}

inline bool CPropertySet::containsAllOf(CPropertySet const &other) const {
  if (other.empty())
    return true;
  return std::includes(_Activities.begin(), _Activities.end(), other.begin(),
                       other.end());
}

inline bool
CPropertySet::containsPartOfStrict(CPropertySet const &other) const {
  if (other.empty())
    return false;
  for (TAdditionalPropertyList::const_iterator it = other.begin(),
                                               itEnd = other.end();
       it != itEnd; ++it) {
...
inline bool
CPropertySet::containsPartOfNotStrict(CPropertySet const &other) const {
  if (other.empty())
    return true;
  for (TAdditionalPropertyList::const_iterator it = other.begin(),
                                               itEnd = other.end();
...
inline bool
CPropertySet::containsPartOfStrictFilter(std::string const &other) const {
  if (other.empty())
    return false;
  for (TAdditionalPropertyList::const_iterator it = _Activities.begin(),
                                               itEnd = _Activities.end();
       it != itEnd; ++it) {
...
inline std::string CPropertySet::toString() const {
  std::string returnString;
  bool firstTime = true;
  for (TAdditionalPropertyList::const_iterator it = begin(), itEnd = end();
       it != itEnd; ++it) {
    if (!firstTime)
      returnString += ",";
    else
...
inline bool CPropertySet::have(CPropertyId const &property) const {
...
inline bool CPropertySet::empty() const { return _Activities.empty(); }
...
template <class T> inline void CPropertySetWithExtraList<T>::clearExtra() {
  _ExtraList.clear();
}

inline void CPropertySet::addProperty(CPropertyId const &property, size_t id) {
...
inline void CPropertySet::removeProperties(size_t id) {
  for (TAdditionalPropertyList::iterator it = _Activities.begin(),
                                         itEnd = _Activities.end();
       it != itEnd;) {
    if (it->_Id == id) {
...
inline void CPropertySet::merge(CPropertySet const &activities, size_t id) {
  for (TAdditionalPropertyList::const_iterator it = activities.begin(),
                                               itEnd = activities.end();
       it != itEnd; ++it)
    _Activities.insert(TProperty(it->_Property, id));
}

inline CPropertySet &CPropertySet::operator+(CPropertyId const &other) {
  addProperty(other);
  return *this;
}

inline size_t CPropertySet::size() const { return _Activities.size(); }

inline bool CPropertySet::operator==(CPropertySet const &other) const {
...
inline bool CPropertySet::operator<(CPropertySet const &other) const {
  ...
  TAdditionalPropertyList::const_iterator i1(_Activities.begin()),
      i2(other._Activities.begin());
...
inline std::set<NLMISC::TStringId> CPropertySet::properties() const {
...
inline CPropertySet::TAdditionalPropertyList::iterator CPropertySet::begin() {
  return _Activities.begin();
}

inline CPropertySet::TAdditionalPropertyList::iterator CPropertySet::end() {
  return _Activities.end();
}

inline CPropertySet::TAdditionalPropertyList::const_iterator
CPropertySet::begin() const {
  return _Activities.begin();
}

inline CPropertySet::TAdditionalPropertyList::const_iterator
CPropertySet::end() const {
  return _Activities.end();
}
} // namespace AITYPES
#endif

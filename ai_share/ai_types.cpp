...
template <>
CDescType<TAIType>::CDescTypeEntry CDescType<TAIType>::_entries[] = {
    CDescTypeEntry("MANAGER", AITypeManager),
    CDescTypeEntry("NOGO", AITypeNoGo), CDescTypeEntry("GRP", AITypeGrp),
    //		CDescTypeEntry(	"GRP_PARAMETERS",
    //AITypeGrpParameters	),
    ...
    //	CDescTypeEntry(	"GROUP_TEMPLATE_GENERIC",
    ...
    //	CDescTypeEntry(	"GENERIC_DYN_COMP",
    ...
    CDescTypeEntry("NPC_BOT", AITypeBot), CDescTypeEntry("NO_GO", AITypeNoGo),
...
template <>
CDescType<TAITypeSpec>::CDescTypeEntry CDescType<TAITypeSpec>::_entries[] = {
    //	made for debug.
...
template <>
CDescType<TMgrType>::CDescTypeEntry CDescType<TMgrType>::_entries[] = {
    CDescTypeEntry("FAUNA", MgrTypeFauna),
    CDescTypeEntry("KARAVAN", MgrTypeKaravan),
    CDescTypeEntry("KAMI", MgrTypeKami), CDescTypeEntry("NPC", MgrTypeNpc),
...
template <>
CDescType<TFaunaType>::CDescTypeEntry CDescType<TFaunaType>::_entries[] = {
...
template <>
CDescType<TEffectType>::CDescTypeEntry CDescType<TEffectType>::_entries[] = {
...
template <>
CDescType<TSpawnType>::CDescTypeEntry CDescType<TSpawnType>::_entries[] = {
...
template <>
CDescType<TProfiles>::CDescTypeEntry CDescType<TProfiles>::_entries[] = {
...
//	CDescType<TFaunaActivity>::CDescTypeEntry
//CDescType<TFaunaActivity>::_entries[]	=
//	{
...
/*	NL_BEGIN_STRING_CONVERSION_TABLE(TFaunaZoneActivity)
    ...
    NL_END_STRING_CONVERSION_TABLE(TFaunaZoneActivity,
   faunaActivityConversionTable, act_fz_none) const std::string
   &toString(TFaunaZoneActivity activity)
    {
    ...
    NL_END_STRING_CONVERSION_TABLE(TNpcZoneActivity, npcActivityConversionTable,
   act_nz_none) const std::string &toString(TNpcZoneActivity activity)
    {
...
//	NL_END_STRING_CONVERSION_TABLE(TZoneActivity, activityConversionTable,
//act_none) 	const std::string &toString(TZoneActivity activity)
//	{
//		return activityConversionTable.toString(activity);
//	}

//	std::vector<std::pair<std::string, NLMISC::TStringId> >
//TPopulationFamily::_TribeNames;
//
...
//		{ "goo",	family_goo},
//		{ "mp",		family_mp},
//	NL_END_STRING_CONVERSION_TABLE(TFamilyTag, familyConversionTable,
//family_bad)
//
...
//IService::getInstance()->ConfigFile.getVar("TribeNamePath"); 				vector<string>
//files; 				CPath::getPathContent(var.asString(), false, false, true, files);
//
//				for (uint i=0; i<files.size(); ++i)
//				{
//					string fn =
//CFile::getFilename(files[i]); 					if (CFile::getExtension(fn) == "html")
//					{
//						if (fn.find("tribe_") == 0)
//						{
//							// this is a tribe name
//							string tn =
//CFile::getFilenameWithoutExtension(fn); 							_TribeNames.push_back(make_pair(tn,
//CStringMapper::map(tn)));
//						}
//					}
//				}
//			}
//			catch(...)
//			{
//				nlwarning("Can't find var 'TribeNamePath' in
//config file. Tribe name will not be available.");
//			}
//
//			inited = true;
//		}
...
//		else
//			FamilyTag =
//familyConversionTable.fromString(familyName);
//	}
//
...
//&TPopulationFamily::getTribesNames()
//	{
//		init();
//		return _TribeNames;
//	}
...
NL_BEGIN_STRING_CONVERSION_TABLE(TVerticalPos){"auto", vp_auto},
    {"upper", vp_upper}, {"middle", vp_middle}, {"lower", vp_lower},
    NL_END_STRING_CONVERSION_TABLE(TVerticalPos, verticalPosConversionTable,
                                   vp_auto)

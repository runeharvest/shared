...
extern void parsePrimStream(NLMISC::IStream &stream,
                            const std::string &streamName);

// the parser for 'AiActions' primitives in pdr (R2 format)
extern void parsePrimNoStream(NLLIGO::CPrimitives *primDoc,
                              const std::string &streamName);
...
// previous content)
inline bool stringToKeywordAndTail(const std::string &input,
                                   std::string &keyword, std::string &tail) {
  uint i = 0, j, k;

  // skip white space
  while (i < input.size() && isWhiteSpace(input[i]))
    ++i; // i points to start of keyword

  // look for the end of the keyword
  for (j = i; j < input.size() && input[j] != ':';)
    ++j; // j points to ':' after keyword

  // prune any unnecessary white space before ':'
  for (k = j; k > i && isWhiteSpace(input[k - 1]);)
    --k; // k points to character after end of keyword
  ...
  for (k = (uint)input.size(); k > j && isWhiteSpace(input[k - 1]);)
    --k; // k points to character after end of tail text

  // find start of tail text
  do {
    ++j;
  } while (j < k && isWhiteSpace(input[j])); // j points to start of tail text

...
// The following serve for controling verbose nature of logging - LOG is
// undefined at end of file
extern bool VerboseAliasDescriptionNodeLog;
#define LOG                                                                    \
  if (!VerboseAliasDescriptionNodeLog) {                                       \
  } else                                                                       \
    nlinfo
...
public:
  ...
  CAIAliasDescriptionNode(std::string name, uint32 alias, AITYPES::TAIType type,
                          CAIAliasDescriptionNode *parent)
  ...
  static std::vector<NLMISC::CSmartPtr<CAIAliasDescriptionNode>>
      _aliasDescriptionList;
  static void flushUnusedAliasDescription() { _aliasDescriptionList.clear(); }
...
// remove my own children
  ...
  const NLMISC::CSmartPtr<CAIAliasDescriptionNode> &getParent() const {
    return _parent;
  }
  ...
  uint32 getChildCount() const {
    nlassert(this != NULL);
    return (uint32)_children.size();
  }
  CAIAliasDescriptionNode *const &getChild(uint32 idx) const {
  ...
  //
  //		std::string
  //result=NLMISC::toString("[%08x:%s:%s]",_alias,AITYPES::getName(_type),_name.c_str());
  //		if (!_children.empty())
  //		{
  //			result+='(';
  //			for (uint i=0;i<_children.size();++i)
  ...
  findNodeChildByNameAndType(const std::string &name, AITYPES::TAIType type,
                             bool recurse = false) const {
    // first look for a child here with the correct properties
    for (uint i = 0; i < getChildCount(); ++i) {
      if (type != getChild(i)->getType())
        continue;
      if (NLMISC::nlstricmp(name, getChild(i)->getName()) != 0)
        continue;
      return getChild(i);
    }
    ...
    if (recurse)
      for (uint i = 0; i < getChildCount(); ++i) {
        const CAIAliasDescriptionNode *result =
            getChild(i)->findNodeChildByNameAndType(name, type, true);
        if (result)
          return result;
      }
  ...
  const CAIAliasDescriptionNode *
  findNodeByFullNameAndType(const std::string &name,
                            AITYPES::TAIType type) const {
    // first off - if our type matches, look and see whether current node name
    // ends with the searched for name
    if (getType() == type) {
      std::string nodeName = fullName();
      if (nodeName.size() >= name.size() &&
          NLMISC::nlstricmp(
              name, nodeName.substr(nodeName.size() - name.size())) == 0)
        if (nodeName.size() == name.size() ||
            nodeName[nodeName.size() - name.size() - 1] == ':')
          return this;
    }
    ...
    for (uint i = 0; i < getChildCount(); ++i) {
      const CAIAliasDescriptionNode *result =
          getChild(i)->findNodeByFullNameAndType(name, type);
      if (result)
        return result;
    }
  ...
  uint32 findAliasByNameAndType(const std::string &name,
                                AITYPES::TAIType type) const {
    ...
    for (const CAIAliasDescriptionNode *it = getParent(); it != NULL;
         it = it->getParent()) {
  ...
  static CAIAliasDescriptionNode *popFromPdr(CPersistentDataRecord &pdr) {
    ...
    while (!pdr.isEndOfStruct()) {
      ...
      if (tokenName == "name") {
        name = pdr.popNextArg(token).asString();
        continue;
      }
      if (tokenName == "alias") {
        alias = (uint32)pdr.popNextArg(token).asUint();
        continue;
      }
      if (tokenName == "type") {
        type = (AITYPES::TAIType)pdr.popNextArg(token).asUint();
        continue;
      }
      if (tokenName == "child") {
    ...
    CAIAliasDescriptionNode *result =
        new CAIAliasDescriptionNode(name, alias, type, NULL);

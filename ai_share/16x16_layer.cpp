...
I16x16Layer *I16x16Layer::load(NLMISC::IStream &f) {
  ...
  switch (type) {
  case 1:
    layer = new CFull16x16Layer();
    break;
  case 2:
    layer = new C8Bits16x16Layer();
    break;
  case 3:
    layer = new C4Bits16x16Layer();
    break;
  case 4:
    layer = new C2Bits16x16Layer();
    break;
  case 5:
    layer = new C1Bit16x16Layer();
    break;
  case 6:
    layer = new CWhite16x16Layer();
    break;
  }
...
void I16x16Layer::save(NLMISC::IStream &f, I16x16Layer *layer) {
  ...
  if (dynamic_cast<CFull16x16Layer *>(layer))
    type = 1;
  else if (dynamic_cast<C8Bits16x16Layer *>(layer))
    type = 2;
  else if (dynamic_cast<C4Bits16x16Layer *>(layer))
    type = 3;
  else if (dynamic_cast<C2Bits16x16Layer *>(layer))
    type = 4;
  else if (dynamic_cast<C1Bit16x16Layer *>(layer))
    type = 5;
  else if (dynamic_cast<CWhite16x16Layer *>(layer))
    type = 6;
...
bool I16x16Layer::compare(I16x16Layer *original, I16x16Layer *copy,
                          sint32 avoid) {
  uint i, j;
  bool failed = false;
  for (i = 0; i < 16; ++i) {
    for (j = 0; j < 16; ++j) {
      if (original->get(i, j) != avoid &&

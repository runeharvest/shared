...
NLMISC_COMMAND(setGrp, "set the active group within a manager",
               "<name>|<slot>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(newFaunaManager, "create the fauna manager for a region",
               "<name> [<slot>]") {
  uint slot;
  switch (args.size()) {
  case 1: // <name>
...
NLMISC_COMMAND(newPlaceXYR, "", "<place name> <x> <y> <r> <verticalPos>") {
  ...
  if (toString(x) + toString(y) + toString(r) != args[1] + args[2] + args[3]) {
    nlinfo("Cumulated numeric parameter test failed: '%s' != '%s'",
           (toString(x) + toString(y) + toString(r)).c_str(),
           (args[1] + args[2] + args[3]).c_str());
    return false;
  }
...
NLMISC_COMMAND(newPredatorGroup, "",
...
NLMISC_COMMAND(newScavengerGroup, "",
               "<group name> <spawn point> <eat point> <rest point>") {
  if (args.size() != 4)
    return false;
...
NLMISC_COMMAND(addHerbivorePopulation,
               "add a population version to the current group",
               "<creature type> <qty> [<type> <qty>] [...] [...]") {
...
NLMISC_COMMAND(newUrbanManager, "create the urban npc manager for a sttlement",
               "<name> [<slot>]") {
  uint slot;
  switch (args.size()) {
  case 1: // <name>
...
NLMISC_COMMAND(npcSetColours, "set the npc clothing colour scheme",
               "<colour scheme>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(npcSetGameForm, "set the .CREATURE sheet to use for game stats",
               "<sheet name>") {
  if (args.size() != 1)
    return false;
...
NLMISC_COMMAND(npcSetLookForm,
               "set the .CREATURE sheet to use for look on client",
               "<sheet name>") {
  if (args.size() != 1)
    return false;

...
public:
  ...
  /// Num instances
  uint getNumInstance() { return (uint)_ShapeNames.size(); }

  /// get shape name
  const std::string &getShapeName(uint i) { return _ShapeNames[i]; }

  /// get shape name
  const std::string &getInstanceName(uint i) { return _InstanceNames[i]; }

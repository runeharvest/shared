...
CVariable<bool> MSWStrictMode("msw", "MSWStrictMode",
                              "Set the strict mode on SQL request", true, 0,
                              true);
CVariable<uint32> MSWRequestDuration("msw", "MSWRequestDuration",
                                     "Measure the duration of SQL request", 0,
                                     1000);
CVariable<bool> MSWAutoReconnect("msw", "MSWAutoReconnect",
                                 "MYSQL_OPT_RECONNECT", true, 0, true);
...
std::string encodeDate(NLMISC::TTime date) {
  ...
  str = NLMISC::toString("%4u-%02u-%02u %u:%02u:%02u",
                         converted->tm_year + 1900, converted->tm_mon + 1,
                         converted->tm_mday, converted->tm_hour,
                         converted->tm_min, converted->tm_sec);
...
const std::string &escapeString(const std::string &str, CConnection &dbCnx) {
  ...
  unsigned long resultSize = mysql_escape_string(
      (char *)buffer.data(), str.data(), (unsigned long)str.size());
...
void CConnection::clearOption(mysql_option option) { _Options.erase(option); }
...
bool CConnection::connect(const TParsedCommandLine &dbInfo) {
  ...
  return connect(dbHost->ParamValue, dbUser->ParamValue, dbPassword->ParamValue,
                 dbBase->ParamValue, port);
}

bool CConnection::connect(const std::string &hostName,
                          const std::string &userName,
                          const std::string &password,
                          const std::string &defaultDatabase, uint port) {
...
bool CConnection::_connect() {
  ...
  MYSQL *res = mysql_real_connect(_MysqlContext, _ConnHostName.c_str(),
                                  _ConnUserName.c_str(), _ConnPassword.c_str(),
                                  _ConnDefaultDatabase.c_str(), _ConnPort, NULL,
                                  CLIENT_FOUND_ROWS);

  if (res == NULL) {
    nlwarning("Error during connection to database '%s:%s@%s:%d' :",
              _ConnUserName.c_str(), _ConnDefaultDatabase.c_str(),
              _ConnHostName.c_str(), _ConnPort);
...
bool CConnection::query(const std::string &queryString) {
  ...
  nldebug("Executing query : [%s%s", queryString.substr(0, 100).c_str(),
          100 < queryString.size() ? "]" : "");
  for (uint i = 100; i < queryString.size(); i += 100)
    nldebug("\t%s%s", queryString.substr(i, 100).c_str(),
            i + 100 > queryString.size() ? "]" : "");

  TTime startDate = CTime::getLocalTime();

  int result = mysql_real_query(_MysqlContext, queryString.c_str(),
                                (unsigned long)queryString.size());
  if (result != 0) {
    ...
    // if (result == CR_SERVER_GONE_ERROR)
    {
      ...
      nlinfo("%p Mysql error errno:%d result:%d : %s, try to reconnect...",
             _MysqlContext, merrno, result, mysql_error(_MysqlContext));
      if (_connect())
        result = mysql_real_query(_MysqlContext, queryString.c_str(),
                                  (unsigned long)queryString.size());
      else {
        nlwarning("Failed to reopen closed connection to send query '%s'",
                  queryString.c_str());
        if (MSWStrictMode) {
          nlstopex(("SQL Strict mode, the above error is fatal"));
        }
    ...
    if (result != 0) {
      nlwarning("Mysql error errno:%d result:%d : %s", merrno, result,
...
void CResultBase::getDateField(uint32 fieldIndex, uint32 &time) {
  ...
  default: {
    // any other case, consider the field as a second unix time (seconds since
    // 1970) format is 'SSSSSSSSSS..'
    uint32 t;
...
NLMISC_CATEGORISED_DYNVARIABLE(
    "nope", bool, NOPEAllowReleaseDirtyObject,
    "Set to true to allow dirty object to be removed from cache (this mean "
    "some modified data are lost)") {
  if (get)

...
private:
  typedef void (CMailForumNotifierSkel::*TMessageHandler)(
  ...
  void notifyMail_skel(NLNET::IModuleProxy *sender,
                       const NLNET::CMessage &__message);

  void notifyForumMessage_skel(NLNET::IModuleProxy *sender,
                               const NLNET::CMessage &__message);
...
public:
  ...
  virtual void notifyForumMessage(NLNET::IModuleProxy *sender, uint32 charId,
                                  uint32 guildId, uint32 threadId) = 0;
};
...
public:
  ...
  // A new message have been posted in a guild forum
  // the notifier client send a notification for each member character
  void notifyForumMessage(NLNET::IModule *sender, uint32 charId, uint32 guildId,
                          uint32 threadId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_notifyMail(NLNET::CMessage &__message, uint32 charId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_notifyForumMessage(NLNET::CMessage &__message, uint32 charId,
                                     uint32 guildId, uint32 threadId);
};
...
public:
  ...
  void invokeResult(NLNET::TSockId dest, uint32 resultCode,
  ...
  static void cb_notifyMail(NLNET::CMessage &message, NLNET::TSockId from,
                            NLNET::CCallbackNetBase &netbase) {
    H_AUTO(notifyMail_on_notifyMail);
#ifdef NL_DEBUG
    nldebug("CMailForumWeb::cb_notifyMail received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackServerAdaptor *adaptor =
        static_cast<ICallbackServerAdaptor *>(netbase.getUserData());

    CMailForumWebItf *callback =
        (CMailForumWebItf *)adaptor->getContainerClass();
  ...
  static void cb_notifyForumMessage(NLNET::CMessage &message,
                                    NLNET::TSockId from,
                                    NLNET::CCallbackNetBase &netbase) {
    H_AUTO(notifyForumMessage_on_notifyForumMessage);
#ifdef NL_DEBUG
    nldebug("CMailForumWeb::cb_notifyForumMessage received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackServerAdaptor *adaptor =
        static_cast<ICallbackServerAdaptor *>(netbase.getUserData());
...
#ifdef NL_DEBUG
    nldebug(
        "CMailForumWeb::cb_notifyForumMessage : calling on_notifyForumMessage");
#endif
  ...
  // A new message have been posted in a guild forum
  virtual void on_notifyForumMessage(NLNET::TSockId from, uint32 guildId,
                                     uint32 forumId) = 0;
};
...
protected:
  ...
  static void _cbDisconnection(NLNET::TSockId from, void *arg) {
    CMailForumWebClientItf *_this =
        reinterpret_cast<CMailForumWebClientItf *>(arg);
...
public:
  ...
  static const std::string &getMessageName(const std::string &methodName) {
    ...
    if (!initialized) {
      messageNames.insert(std::make_pair(std::string("on_invokeResult"),
                                         std::string("MFS_RET")));
  ...
  static void cb_invokeResult(NLNET::CMessage &message, NLNET::TSockId from,
                              NLNET::CCallbackNetBase &netbase) {
#ifdef NL_DEBUG
    nldebug("CMailForumWebClient::cb_invokeResult received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackClientAdaptor *adaptor =
        static_cast<ICallbackClientAdaptor *>(netbase.getUserData());

    CMailForumWebClientItf *callback =
        (CMailForumWebClientItf *)adaptor->getContainerClass();
  ...
  // Generic response to invoke.
  // result contains 0 if no error, more than 0 in case of error
  virtual void on_invokeResult(NLNET::TSockId from, uint32 resultCode,

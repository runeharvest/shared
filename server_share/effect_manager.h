...
public:
  ...
  inline static void addEffect(const TDataSetRow &entityRow,
                               const CBasicEffect &effect) {
    TEntitiesEffectMap::iterator it = _Effects.find(entityRow);
    if (it != _Effects.end()) {
      if ((*it).second != NULL) {
        (*it).second->push_back(effect);
      } else {
        nlwarning("<CEffectManager::addEffect> NULL pointer instead of effect "
                  "vector for entity RowId %u",
                  entityRow.getIndex());
  ...
  inline static void addEffects(const TDataSetRow &entityRow,
                                const std::vector<CBasicEffect> &spells) {
    TEntitiesEffectMap::iterator it = _Effects.find(entityRow);
    if (it != _Effects.end()) {
      if ((*it).second != NULL) {
        ...
        (*it).second = new TEffectVector();
        if ((*it).second)
          (*it).second->insert((*it).second->end(), spells.begin(),
                               spells.end());
      }
    } else {
      // create new vector
  ...
  /// remove a effect
  inline static void removeEffect(const TDataSetRow &entityRow,
                                  uint32 effectId) {
    TEntitiesEffectMap::iterator it = _Effects.find(entityRow);
    if (it == _Effects.end()) {
      return;

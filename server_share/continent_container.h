...
protected:
  ...
  public:
    CContinentMoveContainer()
        : GlobalRetriever(NULL), RetrieverBank(NULL), MoveContainer(NULL),
          AllowAutoSpawn(false) {}
    /// Then name of the continent
    std::string Name;
    /// The PACS global retriever for this continent
    NLPACS::UGlobalRetriever *
        GlobalRetriever; // Global retriever retrieved surfaces of all territory
    ...
    /// True if entities are allowed to try spawning in this continent if no
    /// continent index is provided
    bool AllowAutoSpawn;
  };
    ...
    void readGeorges(const NLMISC::CSmartPtr<NLGEORGES::UForm> &form,
      ...
      if (form->getRootNode().getNodeByName(&node, "Villages") && node &&
        ...
        for (uint village = 0; village < numVillages; ++village) {
          if (form->getRootNode().getNodeByName(
                  &node,
                  NLMISC::toString("Villages[%d].IgList", village).c_str()) &&
              node && node->getArraySize(numIgs)) {
            for (uint ig = 0; ig < numIgs; ++ig) {
              std::string igName;
              if (form->getRootNode().getValueByName(
                      igName, NLMISC::toString("Villages[%d].IgList[%d].IgName",
                                               village, ig)
                                  .c_str()))
...
public:
  ...
  void init(uint gridWidth, uint gridHeight, double primitiveMaxSize,
            uint nbWorldImages, const std::string &packedSheetsDirectory,
            double cellSize = 0.0, bool loadPacsPrims = true);

  /// Build sheets
  void buildSheets(const std::string &packedSheetsDirectory);

  /// Init pacs prims
  void initPacsPrim(const std::string &path =
                        std::string("landscape_col_prim_pacs_list.txt"));

  /// Load continent
  void loadContinent(std::string name, std::string file, sint index,
                     bool allowAutoSpawn = true);
  ...
  sint findContinent(const NLMISC::CVectorD &worldPosition,
    ...
    for (i = 0; i < _Continents.size(); ++i) {
      ...
      NLPACS::UGlobalPosition globalPosition =
          _Continents[i].GlobalRetriever->retrievePosition(worldPosition);
      if (globalPosition.InstanceId != -1 &&
          globalPosition.LocalPosition.Surface != -1) {
        if (continent != -1) {
          nlwarning("findContinent%s: continent not provided, and sole "
                    "position (%.3f, %.3f, %.3f) refers to several continents "
                    "(%d, %d) -- use first found",
                    id.toString().c_str(), worldPosition.x, worldPosition.y,
                    worldPosition.z, i, continent);
          continue;
        }
    ...
    if (continent == -1) {
      if (worldPosition.x != 0.0 || worldPosition.y != 0.0 ||
          worldPosition.z != 0.0)
        nlwarning(
            "findContinent%s: can't find best fitting continent for position "
            "(%.3f, %.3f, %.3f) -- use first available continent",
            id.toString().c_str(), worldPosition.x, worldPosition.y,
            worldPosition.z);
  ...
  void displayTriggers(NLMISC::CLog *log = NLMISC::InfoLog) const {
    ...
    for (it = _TriggerMap.begin(); it != _TriggerMap.end(); ++it)
      log->displayNL("Trigger %d: (%.3f, %.3f, %.3f)", (*it).first,
                     (*it).second.x, (*it).second.y, (*it).second.z);
  }

protected:
  void loadPacsPrims(const CSheet &sheet,

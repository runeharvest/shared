...
public:
  ...
  virtual void description() {
    ...
    property("InstanceNumber", PropUInt32, std::numeric_limits<uint32>::max(),
             InstanceNumber);
...
//----------------------------------------------------------------------------
// Fauna bot description (AIS->EGS) send each frame with info about all spawned
// fauna bots
//----------------------------------------------------------------------------
...
//	}
//
//	virtual void callback (const std::string &/* name */, NLNET::TServiceId
///* id */) {}
//};
...
//		property ("GuildIndex", PropDataSetRow, TDataSetRow()
//,GuildIndex);
//	}
//
//	virtual void callback (const std::string &/* name */, NLNET::TServiceId
...
public:
  ...
  CAILostAggroMsg(const TDataSetRow &targetRowId,
                  const TDataSetRow &playerRowId)
      : PlayerRowId(playerRowId), TargetRowId(targetRowId) {
#ifdef NL_DEBUG
...
public:
  ...
  CAIGainAggroMsg(const TDataSetRow &targetRowId,
                  const TDataSetRow &playerRowId)
      : PlayerRowId(playerRowId), TargetRowId(targetRowId) {
#ifdef NL_DEBUG
    nlassert(playerRowId.isValid());
...
//----------------------------------------------------------------------------
// EGS -> AIS informe an AIS that one of it's instance spoof an already used
// instance number
//				or associate a static instance number with a bad
//continent. 				The AIS must react to this by despawning the AIInstance.
//----------------------------------------------------------------------------
...
struct COutpostDespawnAllSquadsMsg {
  ...
  void serial(NLMISC::IStream &s) { s.serial(Outpost); }
};
...
//----------------------------------------------------------------------------
// EGS -> AIS Ask to add a handle on a group (spawn it if necessary) (called
// handledAIGroup)
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// EGS -> AIS Ask to del a handle on a group (despawn it if necessary) (called
// handledAIGroup)
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// AIS -> EGS AIS send a item request, message interface for manage in same way
// give and receive msg
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
// AIS -> EGS Ais script send a query to EGS that will be answered via script
// (user event)
//----------------------------------------------------------------------------
// see CUserEventMsg

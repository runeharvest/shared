...
const CLoggerServiceSkel::TMessageHandlerMap &
CLoggerServiceSkel::getMessageHandlers() const {
  ...
  if (!init) {
    ...
    res = handlers.insert(std::make_pair(
        std::string("RC"), &CLoggerServiceSkel::registerClient_skel));
    // if this assert, you have a doubly message name in your interface
    // definition !
    nlassert(res.second);

    res = handlers.insert(
        std::make_pair(std::string("LG"), &CLoggerServiceSkel::reportLog_skel));
    // if this assert, you have a doubly message name in your interface
    // definition !
    nlassert(res.second);
...
bool CLoggerServiceSkel::fwdOnProcessModuleMessage(
...
void CLoggerServiceSkel::registerClient_skel(NLNET::IModuleProxy *sender,
...
void CLoggerServiceSkel::reportLog_skel(NLNET::IModuleProxy *sender,
                                        const NLNET::CMessage &__message) {
...
// A client send a log
void CLoggerServiceProxy::reportLog(NLNET::IModule *sender,
                                    const std::vector<TLogInfo> &logInfos) {
  if (_LocalModuleSkel && _LocalModule->isImmediateDispatchingSupported()) {
    // immediate local synchronous dispatching
    _LocalModuleSkel->reportLog(
        _ModuleProxy->getModuleGateway()->getPluggedModuleProxy(sender),
        logInfos);
  } else {
    // send the message for remote dispatching and execution or local queing
...
// Message serializer. Return the message received in reference for easier
// integration
const NLNET::CMessage &CLoggerServiceProxy::buildMessageFor_reportLog(
    NLNET::CMessage &__message, const std::vector<TLogInfo> &logInfos) {

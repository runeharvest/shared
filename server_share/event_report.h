...
struct SEventReport {
  ...
  std::string toString() const {
    char buffer[512];
    sprintf(buffer,
            "Actor %s, Target %s. Delta Level = %d, skill = %s, spe = %s. "
            "Success = %s, FullResist = %s",
            ActingEntity.toString().c_str(), TargetEntity.toString().c_str(),
            DeltaLevel, Skill.c_str(), Specialization.c_str(),
            (Success ? "yes" : "no"), (FullResist ? "yes" : "no"));

...
/**
 * Class COPSAddEffectModifier, message class used to add a new modifier effect
 * on a player \author David Fleury \author Nevrax France \date 2002
 */
...
public:
  ...
  TDataSetRow
      PlayerId; // id of the entity on which this effect is to be applied
  uint16 EffectType;

  enum EMode { set = 0, modify, variation };

  //	NLMISC::TGameCycle	Duration;			// Apply
  //duration of modifier 	NLMISC::TGameCycle	RepeatInterval;		// 0 =
  //apply effect only once, no repeatition
  uint32 Duration;       // Apply duration of modifier
  uint32 RepeatInterval; // 0 = apply effect only once, no repeatition

  uint16 NbRepeat; // means nothing if RepeatInterval == 0, otherwise it's the
                   // max number of repeatition, 0 == NO LIMIT
  ...
  std::list<uint16>
      PreventedEffects; // the effect types prevented by this effect

  /// name of the string to display in the chat of the player at each
  /// application of the effect (MUST have NO param)
  std::string ApplyPrivateMsgName;

  /// name of the string to display in the general chat at each application of
  /// the effect (MUST have ONE param of type $e)
  std::string ApplySpectatorsMsgName;

  /// name of the string to display in the chat of the effect creator player
  /// (MUST have ONE param of type $e)
  std::string ApplyCreatorMsgName;

  /// name of the string to display in the chat of the player at the end of the
  /// effect (MUST have NO param)
  std::string EndPrivateMsgName;

  /// name of the string to display in the general chat at the end of the effect
  /// (MUST have ONE param of type $e)
  ...
  /// name of the impulsion that will be sent to the client at the end of the
  /// effect
  ...
  virtual void description() {
    ...
    property("ApplyPrivateMsgName", PropString, std::string(""),
             ApplyPrivateMsgName);
    property("ApplySpectatorsMsgName", PropString, std::string(""),
             ApplySpectatorsMsgName);
    property("ApplyCreatorMsgName", PropString, std::string(""),
             ApplyCreatorMsgName);
    property("EndPrivateMsgName", PropString, std::string(""),
             EndPrivateMsgName);
    property("EndSpectatorsMsgName", PropString, std::string(""),
             EndSpectatorsMsgName);
    property("EndCreatorMsgName", PropString, std::string(""),
             EndCreatorMsgName);

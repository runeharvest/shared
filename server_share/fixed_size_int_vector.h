 ...
 * A special custom allocator is used for managing data blocks allocated and
 * freed here NOTE: The custom allocator never returns memory to the system - it
 * retains all RAM that it has ever allocated
 */
template <typename T> class CFixedSizeIntVector {
public:
  /// ctor
  CFixedSizeIntVector();
  ...
  /// init
  /// This metod sets the size of the container, allocating memory either from
  /// the free data buffer pool or, if need be, from system memory Initialises
  /// all container contents to 0
  void init(uint32 size, const T &value);

  /// release
  /// free up the data buffer used by this container (return it to the free data
  /// buffer pool)
  void release();
...
private:
  // a vector of data buffers (there is no size info here - they are assumed to
  // be all of the same size) this data type is used to contain the vector of
  // free (reusable) buffers of a given size
  ...
  /// static method used to wrap a static map of size to uint32 data buffer
  /// counter
...
template <typename T> CFixedSizeIntVector<T>::~CFixedSizeIntVector() {
...
void CFixedSizeIntVector<T>::init(uint32 size, const T &value) {
  ...
  TDataBufferPtrVector &theFreeDataBufferPtrVector =
      getFreeDataBufferVector(_Size);
  if (!theFreeDataBufferPtrVector.empty()) {
    // there are spare data buffers of this size so pop the next one off the
    // back of the buffer vector
    ...
    nldebug("CFixedSizeDataVector_resize allocating a new buffer of size %u x "
            "%u (%u buffers in all)",
            _Size, sizeof(T), theDataBufferCounter);
...
template <typename T> void CFixedSizeIntVector<T>::release() {
  // if we have a data buffer then add it to the relevant free data buffer
  // vector in the free data buffer pool
  if (_DataBuffer != NULL) {
...
template <typename T> T &CFixedSizeIntVector<T>::operator[](uint32 index) {
  // test for array overflows
  if (index >= _Size)
    nlerror("Array overflow in Client Property Database");
...
template <typename T> uint CFixedSizeIntVector<T>::size() const {
  return _Size;
}

...
public:
TFailInfo() : _Value(invalid_val) {}
TFailInfo(TValues value) : _Value(value) {}
...
bool operator==(const TFailInfo &other) const { return _Value == other._Value; }
...
private:
  typedef void (CChatUnifierSkel::*TMessageHandler)(
  ...
  void sendFarTell_skel(NLNET::IModuleProxy *sender,
...
public:
  ...
  virtual void sendFarTell(NLNET::IModuleProxy *sender,
                           const NLMISC::CEntityId &senderCharId,
                           bool havePrivilege, const ucstring &destName,
                           const ucstring &text) = 0;
};
...
public:
  ...
  // If IOS can't find the player localy, it forward
  // the tell to the unifier
  void sendFarTell(NLNET::IModule *sender,
                   const NLMISC::CEntityId &senderCharId, bool havePrivilege,
                   const ucstring &destName, const ucstring &text);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_sendFarTell(
...
private:
  typedef void (CChatUnifierClientSkel::*TMessageHandler)(
      NLNET::IModuleProxy *sender, const NLNET::CMessage &message);
  typedef std::map<std::string, TMessageHandler> TMessageHandlerMap;

  const TMessageHandlerMap &getMessageHandlers() const;

  void recvFarTellFail_skel(NLNET::IModuleProxy *sender,
                            const NLNET::CMessage &__message);

  void recvFarTell_skel(NLNET::IModuleProxy *sender,
                        const NLNET::CMessage &__message);

  void farGuildChat_skel(NLNET::IModuleProxy *sender,
                         const NLNET::CMessage &__message);

  void farGuildChat2_skel(NLNET::IModuleProxy *sender,
                          const NLNET::CMessage &__message);

  void farGuildChat2Ex_skel(NLNET::IModuleProxy *sender,
                            const NLNET::CMessage &__message);

  void universeBroadcast_skel(NLNET::IModuleProxy *sender,
                              const NLNET::CMessage &__message);

  void dynChanBroadcast_skel(NLNET::IModuleProxy *sender,
                             const NLNET::CMessage &__message);

  void recvBroadcastMessage_skel(NLNET::IModuleProxy *sender,
                                 const NLNET::CMessage &__message);
...
public:
  ...
  // unknow
  virtual void recvFarTellFail(NLNET::IModuleProxy *sender,
                               const NLMISC::CEntityId &senderCharId,
                               const ucstring &destName,
                               TFailInfo failInfo) = 0;
  // SU send a far tell to the IOS hosting the addresse character
  virtual void recvFarTell(NLNET::IModuleProxy *sender,
                           const NLMISC::CEntityId &senderCharId,
                           const ucstring &senderName, bool havePrivilege,
                           const ucstring &destName, const ucstring &text) = 0;
  // IOS forward a guild chat message to the IOS
  virtual void farGuildChat(NLNET::IModuleProxy *sender,
                            const ucstring &senderName, uint32 guildId,
                            const ucstring &text) = 0;
  // IOS forward a guild chat message to the IOS
  virtual void farGuildChat2(NLNET::IModuleProxy *sender,
                             const ucstring &senderName, uint32 guildId,
                             const ucstring &phraseName) = 0;
  // IOS forward a guild chat message to the IOS
  virtual void farGuildChat2Ex(NLNET::IModuleProxy *sender,
                               const ucstring &senderName, uint32 guildId,
                               uint32 phraseId) = 0;
  // IOS forward a univers chat message to the IOSs
  virtual void universeBroadcast(NLNET::IModuleProxy *sender,
                                 const ucstring &senderName,
                                 uint32 senderHomeSession,
                                 const ucstring &text) = 0;
  // IOS forward a dyn chat chat message to the IOSs
  virtual void dynChanBroadcast(NLNET::IModuleProxy *sender,
                                const NLMISC::CEntityId &chanId,
                                const ucstring &senderName,
                                const ucstring &text) = 0;
  // SU send a broadcast message to the IOS
  virtual void recvBroadcastMessage(NLNET::IModuleProxy *sender,
                                    const ucstring &message) = 0;
};
...
public:
  ...
  // SU send a far tell failure to IOS. This mean that the player is offline or
  // unknow
  void recvFarTellFail(NLNET::IModule *sender,
                       const NLMISC::CEntityId &senderCharId,
                       const ucstring &destName, TFailInfo failInfo);
  // SU send a far tell to the IOS hosting the addresse character
  void recvFarTell(NLNET::IModule *sender,
                   const NLMISC::CEntityId &senderCharId,
                   const ucstring &senderName, bool havePrivilege,
                   const ucstring &destName, const ucstring &text);
  // IOS forward a guild chat message to the IOS
  void farGuildChat(NLNET::IModule *sender, const ucstring &senderName,
                    uint32 guildId, const ucstring &text);
  // IOS forward a guild chat message to the IOS
  void farGuildChat2(NLNET::IModule *sender, const ucstring &senderName,
                     uint32 guildId, const ucstring &phraseName);
  // IOS forward a guild chat message to the IOS
  void farGuildChat2Ex(NLNET::IModule *sender, const ucstring &senderName,
                       uint32 guildId, uint32 phraseId);
  // IOS forward a univers chat message to the IOSs
  void universeBroadcast(NLNET::IModule *sender, const ucstring &senderName,
                         uint32 senderHomeSession, const ucstring &text);
  // IOS forward a dyn chat chat message to the IOSs
  void dynChanBroadcast(NLNET::IModule *sender, const NLMISC::CEntityId &chanId,
  ...
  static void broadcast_farGuildChat(ProxyIterator first, ProxyIterator last,
  ...
  template <class ProxyIterator>
  static void
  broadcast_farGuildChat2(ProxyIterator first, ProxyIterator last,
                          NLNET::IModule *sender, const ucstring &senderName,
                          uint32 guildId, const ucstring &phraseName) {
    NLNET::CMessage message;
  ...
  // This is the broadcast version of the method.
  template <class ProxyIterator>
  static void broadcast_farGuildChat2Ex(ProxyIterator first, ProxyIterator last,
                                        NLNET::IModule *sender,
                                        const ucstring &senderName,
                                        uint32 guildId, uint32 phraseId) {
    NLNET::CMessage message;
  ...
  template <class ProxyIterator>
  static void
  broadcast_universeBroadcast(ProxyIterator first, ProxyIterator last,
                              NLNET::IModule *sender,
                              const ucstring &senderName,
                              uint32 senderHomeSession, const ucstring &text) {
    NLNET::CMessage message;
    ...
    // create the message to send to multiple dest
    buildMessageFor_universeBroadcast(message, senderName, senderHomeSession,
                                      text);
  ...
  template <class ProxyIterator>
  static void
  broadcast_dynChanBroadcast(ProxyIterator first, ProxyIterator last,
                             NLNET::IModule *sender,
                             const NLMISC::CEntityId &chanId,
                             const ucstring &senderName, const ucstring &text) {
    NLNET::CMessage message;
  ...
  buildMessageFor_recvFarTellFail(NLNET::CMessage &__message,
                                  const NLMISC::CEntityId &senderCharId,
                                  const ucstring &destName, TFailInfo failInfo);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_recvFarTell(NLNET::CMessage &__message,
                              const NLMISC::CEntityId &senderCharId,
                              const ucstring &senderName, bool havePrivilege,
                              const ucstring &destName, const ucstring &text);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_farGuildChat(NLNET::CMessage &__message,
                               const ucstring &senderName, uint32 guildId,
                               const ucstring &text);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_farGuildChat2(NLNET::CMessage &__message,
                                const ucstring &senderName, uint32 guildId,
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_farGuildChat2Ex(NLNET::CMessage &__message,
                                  const ucstring &senderName, uint32 guildId,
                                  uint32 phraseId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_universeBroadcast(
      NLNET::CMessage &__message, const ucstring &senderName,
      uint32 senderHomeSession, const ucstring &text);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_dynChanBroadcast(
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_recvBroadcastMessage(NLNET::CMessage &__message,

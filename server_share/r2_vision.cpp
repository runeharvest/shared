...
void CInstance::updateVision()
{
	...
	for (uint32 i = (uint32)visionGroups.size(); i--;)
	{
		...
		while (quickDist(vi->xMin(), vi->yMin(), vi->xMax(), vi->yMax()) > MinVisionGroupSplitDiameter || (vi->numViewers() > MaxViewersPerGroup && quickDist(vi->xMin(), vi->yMin(), vi->xMax(), vi->yMax()) > SmallestBucketSize))
		{

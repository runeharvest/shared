...
/*
  TODO:
    ...
    - Deal with biasing
    - add a solution for split() if all of the entities in the group are
  super-imposed
*/
...
NLMISC::CVariable<bool>
    VerboseVisionDelta("r2vision", "VerboseVisionDelta",
                       "enable verbose logging of vision changes", false, 0,
                       true);
NLMISC::CVariable<uint32> VisionResetDuration(
    "r2vision", "VisionResetDuration",
    "the number of updates to be skipped when reseting player vision", 10, 0,
    true);
...
static const uint32 MinVisionGroupSplitDiameter =
    ((MaxVisionGroupDiameter * 5) / 4);
...
// a big and nasty hack to get a zero value for a TDataSetRow (needed for
// sorting etc)
...
static const uint32 MaxVisionEntries =
    250; // max 256 - (1 slot for invalid entry, 1 slot for viewer when
         // invisible)
...
inline bool visionGroupsOverlap(const CVisionGroup *va,
                                const CVisionGroup *vb) {
  // if either of the vision groups are empty return false
  if (va == NULL || vb == NULL || va->numViewers() == 0 ||
      vb->numViewers() == 0)
    return false;
  ...
  // if the combined groups' bounding space would be too large then don't bother
  // looking any further
  if (quickDist(combinedXMin, combinedYMin, combinedXMax, combinedYMax) >
      MaxVisionGroupDiameter)
    return false;

  // the overlap is confirmed if the sum of the players in the 2 groups is small
  // enough
  if (va->numViewers() + vb->numViewers() < MaxViewersPerGroup)
    return true;

  // calculate the distance from extreme x min to extreme x max and extreme y
  // min to exetrem y max
  ...
  // we consider that a real overlap occurs when both extreme values are < 3/4
  // of the equivalent sum values
...
void CUniverse::createInstance(uint32 aiInstance, uint32 groupId) {
  // just ignore attempts to create the std::numeric_limits<uint32>::max()
  // instance
  if (aiInstance == std::numeric_limits<uint32>::max()) {
    return;
  }

  // ensure that the new AIInstance value is reasonable & increase _Instances
  // vector size if required
  BOMB_IF(aiInstance > 65535,
          "Failed to create Instance with implausible AIInstance value",
          return);
  if (_Instances.size() <= aiInstance) {
    nlinfo("Increasing Universe::AIInstance vector size to: %d",
           aiInstance + 1);
  ...
  if (theInstance == NULL) {
    ...
    nlinfo("Allocating new AIInstance: %d at address: %p", aiInstance,
           &*theInstance);
  }
}

void CUniverse::removeInstance(uint32 aiInstance) {
  // just ignore attempts to remove the std::numeric_limits<uint32>::max()
  // instance
  if (aiInstance == std::numeric_limits<uint32>::max()) {
    return;
  }
  ...
  DROP_IF(
      aiInstance >= _Instances.size() || _Instances[aiInstance] == NULL,
      NLMISC::toString("Ignoring attempt to remove non-existant instance: %d",
                       aiInstance),
      return);
...
void CUniverse::removeInsancesByGroup(uint32 groupId) {
  // iterate over complete vector of instances releasing any that match the
  // given group id
...
void CUniverse::addEntity(TDataSetRow dataSetRow, uint32 aiInstance, sint32 x,
                          sint32 y, TInvisibilityLevel invisibilityLevel,
                          bool isViewer) {
  ...
  BOMB_IF(row == 0, "Illegal attempt to manipulate entity with dataset row=0",
          return);
  BOMB_IF(
      row > (1 << 20),
      NLMISC::toString("Ignoring entity with implausible row value: %d", row),
      return);
...
void CUniverse::forceRefreshVision(TDataSetRow dataSetRow) {
  ...
  if (theEntity.ViewerRecord) {
    ...
    TInvisibilityLevel invisibilityLevel =
        theEntity.ViewerRecord->getVisionLevel();
    uint32 aiInstance = theEntity.AIInstance;

    _addEntity(dataSetRow, aiInstance, static_cast<sint32>(x),
               static_cast<sint32>(y), invisibilityLevel, true);
  }
}
...
void CUniverse::forceResetVision(TDataSetRow dataSetRow) {
  ...
  SUniverseEntity &theEntity = _Entities[row];
  BOMB_IF(theEntity.ViewerRecord == NULL,
          "IGNORING ResetVision for non-viewer entity" + dataSetRow.toString(),
          return);

  // if we're in verbose logging mode then do some logging...
  if (VerboseVisionDelta) {
    nlinfo("FRR- Force Reset Vision received for entitiy: %s",
           dataSetRow.toString().c_str());
  }
...
void CUniverse::removeEntity(TDataSetRow dataSetRow) {
  ...
  // ensure that the row value is valid
  BOMB_IF(row == 0, "Illegal attempt to manipulate entity with dataset=0",
          return);
  BOMB_IF(
      row >= _Entities.size(),
      NLMISC::toString(
          "Ignoring attempt to remove entity with invalid row value: %d", row),
      return);
...
SUniverseEntity *CUniverse::getEntity(uint32 row) {
  ...
  BOMB_IF(
      row >= _Entities.size(),
      NLMISC::toString(
          "Attempting to get hold of entity with invalid row value: %d", row),
      return NULL);
...
void CUniverse::setEntityPosition(TDataSetRow dataSetRow, sint32 x, sint32 y) {
  ...
  BOMB_IF(
      row >= _Entities.size(),
      NLMISC::toString(
          "Ignoring attempt to set entity position with invalid row value: %d",
          row),
      return);
...
void CUniverse::setEntityPositionDelayed(TDataSetRow dataSetRow, sint32 x,
                                         sint32 y, uint32 ticks) {
  H_AUTO(CUniverse_setEntityPositionDelayed)

  nlinfo("adding delayed entity position to the list: %s (%d,%d) @ time: %d",
         dataSetRow.toString().c_str(), x, y, ticks);
  _DelayedPositions.push_back(SDelayedPlayerPosition(ticks, dataSetRow, x, y));
}

void CUniverse::teleportEntity(TDataSetRow dataSetRow, uint32 aiInstance,
                               sint32 x, sint32 y,
                               TInvisibilityLevel invisibilityLevel) {
  ...
  // ensure that the row value is valid
  BOMB_IF(row == 0, "Illegal attempt to manipulate entity with datasetrow=0",
          return);
  BOMB_IF(
      row >= _Entities.size(),
      NLMISC::toString(
  ...
  BOMB_IF(
      aiInstance != std::numeric_limits<uint32>::max() &&
          (aiInstance >= _Instances.size() || _Instances[aiInstance] == NULL),
      NLMISC::toString(
          "ERROR: Failed to add entity %d to un-initialised instance: %d", row,
          aiInstance),
      aiInstance = std::numeric_limits<uint32>::max());
...
void CUniverse::setEntityInvisibilityInfo(TDataSetRow dataSetRow,
                                          uint32 whoSeesMe) {
  ...
  if (theEntity.AIInstance < _Instances.size()) {
    NLMISC::CSmartPtr<CInstance> &theInstance =
        _Instances[theEntity.AIInstance];

    // extract the invisibility level from the whoSeesMe value
    TInvisibilityLevel invisibilityLevel =
        (TInvisibilityLevel)(whoSeesMe & ((1 << NUM_WHOSEESME_BITS) - 1));

    // set the visibility info for us as a viewed entity:
    theInstance->setEntityInvisibility(theEntity.InstanceIndex,
                                       invisibilityLevel);
    ...
    if (theEntity.ViewerRecord != NULL) {
      ...
      TInvisibilityLevel visionLevel =
          (TInvisibilityLevel)(whoSeesMe >> NUM_WHOSEESME_BITS);

      // if the vision level has changed then we have stuff to do
      if (theEntity.ViewerRecord->getVisionLevel() != visionLevel) {
        if (VerboseVisionDelta) {
          nlinfo("Vision Delta: %s: Changes vision level from %d to %d",
                 dataSetRow.toString().c_str(),
                 theEntity.ViewerRecord->getVisionLevel(), visionLevel);
        }
...
inline void CUniverse::_addEntity(TDataSetRow dataSetRow, uint32 aiInstance,
                                  sint32 x, sint32 y,
                                  TInvisibilityLevel invisibilityLevel,
                                  bool isViewer) {
  uint32 row = dataSetRow.getIndex();
...
inline void CUniverse::_removeEntity(TDataSetRow dataSetRow) {
  ...
  if (theEntity.AIInstance < _Instances.size() &&
      _Instances[theEntity.AIInstance] != NULL) {
...
inline void CUniverse::_setEntityPosition(TDataSetRow dataSetRow, sint32 x,
                                          sint32 y) {
  // get a handle to the entity
  SUniverseEntity &theEntity = _Entities[dataSetRow.getIndex()];

#ifdef NL_DEBUG
  nlassert(theEntity.AIInstance == std::numeric_limits<uint32>::max() ||
           theEntity.AIInstance < _Instances.size());
  nlassert(theEntity.AIInstance == std::numeric_limits<uint32>::max() ||
           _Instances[theEntity.AIInstance] != NULL);
#endif
  ...
  _Instances[theEntity.AIInstance]->setEntityPosition(theEntity.InstanceIndex,
                                                      x, y);
}

inline void CUniverse::_teleportEntity(TDataSetRow dataSetRow,
                                       uint32 aiInstance, sint32 x, sint32 y,
                                       TInvisibilityLevel invisibilityLevel) {
  // get a handle to the entity
  SUniverseEntity &theEntity = _Entities[dataSetRow.getIndex()];
  ...
  // if the aiInstance is set to std::numeric_limits<uint32>::max() (a reserved
  // value) then we stop here
  if (aiInstance == std::numeric_limits<uint32>::max()) {
    // clear out the vision for an entity in aiInstance
    ...
    if (getEntity(dataSetRow)->ViewerRecord != NULL) {
      ...
      getEntity(dataSetRow)
          ->ViewerRecord->updateVision(emptyVision, this, false);
    }
    return;
  }
  ...
  _Instances[aiInstance]->addEntity(dataSetRow, x, y, invisibilityLevel,
                                    theEntity);
}

void CUniverse::dump(NLMISC::CLog &log) {
  log.displayNL(
      "--------------------------------- Start of Ring Vision Universe");
  ...
  for (uint32 i = 0; i < _Instances.size(); ++i) {
    if (_Instances[i] != NULL) {
      log.displayNL("--------------------------------- Start of Instance %u",
                    i);
      _Instances[i]->dump(log);
    }
  }
  log.displayNL(
      "--------------------------------- End of Ring Vision Universe");
}
...
uint32 CInstance::getGroupId() const { return _GroupId; }

void CInstance::addEntity(TDataSetRow dataSetRow, uint32 x, uint32 y,
                          TInvisibilityLevel invisibilityLevel,
                          const SUniverseEntity &entity) {
  H_AUTO(CInstance_addEntity)

  // setup the field in the entity to say where it is in the instance's
  // _Entities vector
  ...
  // call setEntityPosition() and setEntityInvisibilityLevel() to set the entity
  // position and invisibility level
  ...
  if (entity.ViewerRecord != NULL) {
    // try to add add the entity into an existing vision group (must be within
    // 50 meters of the group)
    ...
    for (uint32 i = (uint32)_VisionGroups.size(); i--;) {
      ...
      if (_VisionGroups[i]->getVisionLevel() !=
          entity.ViewerRecord->getVisionLevel())
        continue;

      // determine the vision group size if the entity were a member
      uint32 dist = quickDist(
          x, y, (_VisionGroups[i]->xMin() + _VisionGroups[i]->xMax()) >> 1,
...
inline void CInstance::setEntityPosition(uint32 entityIndex, uint32 x,
                                         uint32 y) {
  if (entityIndex < _Entities.size()) {
    ...
    BOMB(NLMISC::toString(
             "BIG BAD BUG - Failed to move entity with invalid index: %d",
             entityIndex),
         return);
  }
}
...
inline void
CInstance::setEntityInvisibility(uint32 entityIndex,
                                 TInvisibilityLevel invisibilityLevel) {
  if (entityIndex < _Entities.size()) {
    if (VerboseVisionDelta &&
        (_Entities[entityIndex].InvisibilityLevel != invisibilityLevel)) {
      nlinfo("Vision Delta: %s: Changes invisibility level from %x to %x",
             _Entities[entityIndex].DataSetRow.toString().c_str(),
             _Entities[entityIndex].InvisibilityLevel, invisibilityLevel);
    }

    _Entities[entityIndex].InvisibilityLevel = invisibilityLevel;
  } else {
    BOMB(NLMISC::toString("BIG BAD BUG - Failed to set invisibilty level for "
                          "entity with invalid index: %d",
                          entityIndex),
         return);
  }
}
...
void CInstance::updateVision() {
  ...
  // run throught the vision groups once updating the bounding coordinates and
  // sort keys
  for (uint32 i = (uint32)_VisionGroups.size(); i--;) {
    ...
    uint32 j =
        (uint32)visionGroups.size(); // point to back entry in vision group
    visionGroups.push_back(
        NULL); // create the new space that we're going to fill
  ...
  for (uint32 i = (uint32)visionGroups.size(); i--;) {
    ...
    // if there are no viewers in the group (it was probably merged with another
    // group) then skip
    if (vi->numViewers() == 0)
      continue;
    ...
    while (quickDist(vi->xMin(), vi->yMin(), vi->xMax(), vi->yMax()) >
               MinVisionGroupSplitDiameter ||
           (vi->numViewers() > MaxViewersPerGroup &&
            quickDist(vi->xMin(), vi->yMin(), vi->xMax(), vi->yMax()) >
                SmallestBucketSize)) {
      // find a free vision id to use
      uint32 visionSlot;
      for (visionSlot = 0; visionSlot < _VisionGroups.size(); ++visionSlot) {
        if (_VisionGroups[visionSlot] == NULL ||
            _VisionGroups[visionSlot]->numViewers() == 0)
          break;
      }

      // if no free slot was found in the vision group vector then add a new
      // group
...
void CInstance::isolateViewer(SUniverseEntity &entity) {
  ...
  if (viewerRecord != NULL) {
    ...
    BOMB_IF(visionId >= _VisionGroups.size() || oldVisionGroup == NULL,
            "Trying to remove entity from vision group with unknown vision id",
            viewerRecord->VisionId = std::numeric_limits<uint32>::max();
            return);
    ...
    // find a free vision id to use for the new vision group we're going to
    // create
    uint32 visionSlot;
    for (visionSlot = 0; visionSlot < _VisionGroups.size(); ++visionSlot) {
      if (_VisionGroups[visionSlot] == NULL ||
    ...
    if (VerboseVisionDelta) {
      nlinfo("Vision Delta: %s: Isolating viewer from vision group %d to new "
             "group %d",
             viewerRecord->getViewerId().toString().c_str(),
             oldVisionGroup->getVisionId(), visionSlot);
    }
  }
}
...
void CInstance::removeEntity(SUniverseEntity &entity) {
  ...
  if (viewerRecord != NULL) {
    uint32 visionId = viewerRecord->VisionId;
    BOMB_IF(visionId >= _VisionGroups.size() || _VisionGroups[visionId] == NULL,
            "Trying to remove entity with unknown vision id",
            viewerRecord->VisionId = std::numeric_limits<uint32>::max();
            return);
  ...
  BOMB_IF(entityIndex >= _Entities.size(),
          "BIG BAD BUG - Entity's InstanceIndex is invalid!", return);

  // move the entity currently at the back of the vector to the slot that we're
  // liberating (NOTE: this could be us!)
  SInstanceEntity &theEntitySlot = _Entities[entityIndex];
  theEntitySlot = _Entities.back();
  _TheUniverse->getEntity(theEntitySlot.DataSetRow)->InstanceIndex =
      entityIndex;
...
void CInstance::release() {
  ...
  for (uint32 i = (uint32)_Entities.size(); i--;) {
    SUniverseEntity *theEntity =
        _TheUniverse->getEntity(_Entities[i].DataSetRow);
    BOMB_IF(theEntity == NULL,
            "Failed to locate universe entity corresponding to instance entity "
            "in CInstance::realease()",
            continue);
...
void CInstance::dump(NLMISC::CLog &log) {
  ...
  for (uint32 i = 0; i < _Entities.size(); ++i) {
    log.displayNL("- Entity %u: %s (%d,%d) InvisibilityLevel(%x)", i,
                  _Entities[i].DataSetRow.toString().c_str(), _Entities[i].X,
                  _Entities[i].Y, _Entities[i].InvisibilityLevel);
  }
...
#endif
  TEST(
      ("add viewer %d to grp %d", viewer->getViewerId().getIndex(), _VisionId));
...
// ensure that the viewer was aleady attached to this vision group
...
#endif
  TEST(("remove viewer %d from grp %d", viewer->getViewerId().getIndex(),
        _VisionId));

  // move the back entry in the viewers buffer into our slot (NOTE: the back
  // entry can be us!)
  ...
  // NOTE: after this the boundary may be out of date - this will be
  // recalculated at the next vision update so we don't take time to do it here
}
...
void CVisionGroup::merge(CVisionGroup *other) {
  ...
  for (uint32 i = (uint32)other->_Viewers.size(); i--;) {
    // get a pointer to the viewer object that we're moving from the other
    // vision group
    NLMISC::CSmartPtr<CViewer> &theViewer = other->_Viewers[i];

    // update the vision id to point to ourselves and the next slot in our
    // viewers vector
    theViewer->VisionId = _VisionId;
    theViewer->VisionIndex = (uint32)_Viewers.size();

    TEST(("moveGroup ent=%d old grp=%d new grp=%d",
          theViewer->getViewerId().getIndex(), other->_VisionId, _VisionId));
  ...
  other->_Vision.resize(
      1); // keep the dummy entry intact at the base of the vision vector
...
void CVisionGroup::split(CVisionGroup *other) {
  ...
  BOMB_IF(_Viewers.empty(),
          "Attempting to split a vision group with no members", return);
  ...
  // setup a bias to favour splitting in the middle of large evenly spread
  // groups
  ...
  if (longestIsX) {
    uint32 leftIdx = 0, rightIdx = viewerSize - 1, otherIdx = 0,
           splitVal = xvect[cutPos];
    while (otherIdx != splitCount) {
      ...
      while (_Viewers[rightIdx]->getX() >= splitVal) {
        TEST(("moveGroup ent=%d old grp=%d new grp=%d",
              _Viewers[rightIdx]->getViewerId().getIndex(), _VisionId,
              other->_VisionId));
      ...
      // deal with elements off the right end of this vector that need to be
      // re-housed
      while (_Viewers[rightIdx]->getX() < splitVal && otherIdx != splitCount) {
        // skip elements at the left end of this vector that are to be left
        // untouched
        ...
        TEST(("moveGroup ent=%d old grp=%d new grp=%d",
              _Viewers[leftIdx]->getViewerId().getIndex(), _VisionId,
              other->_VisionId));

        // move the element reffered to by leftIdx to the other vector and
        // replace it from the rightIdx
...
void CVisionGroup::buildVision(const TInstanceEntities &entities) {
  ...
  uint32 xref =
      _XMin / 2 +
      _XMax / 2; // note - we do divide before add to avoid math errors !!!
  uint32 yref =
      _YMin / 2 +
      _YMax / 2; // note - we do divide before add to avoid math errors !!!

  // build a little set of buckets to sort our entities into
  // these are static purely for reasons of optimisation
  static std::vector<SInstanceEntity const *> buckets(MaxVisionEntries *
                                                      NumVisionBuckets);
  ...
  // setup a vision slot iterator for filling in the vision buffer (=1 to skip
  // passed the dummy entry)
  ...
  for (uint32 i = 0; i < NumVisionBuckets; ++i) {
    ...
    if (numBucketEntries + nextVisionSlot > MaxVisionEntries) {
      BOMB_IF(
          i == 0,
          "ERROR: Vision calculated with >250 entities in first vision bucket!",
          break);
      break;
    }
    ...
    while (numBucketEntries--) {
      theBucketIndex -= NumVisionBuckets;
      _Vision[nextVisionSlot++].DataSetRow =
          buckets[theBucketIndex]->DataSetRow;
    }
  }
...
void CVisionGroup::dump(NLMISC::CLog &log) {
  // display a few stats
  log.displayNL("VisionId(%u) Min(%d,%d)..Max(%d,%d) sortKey(%u)", _VisionId,
                _XMin, _YMin, _XMax, _YMax, _SortKey);
...
inline uint32 CVisionGroup::xMin() const { return _XMin; }

inline uint32 CVisionGroup::xMax() const { return _XMax; }

inline uint32 CVisionGroup::yMin() const { return _YMin; }

inline uint32 CVisionGroup::yMax() const { return _YMax; }

inline uint32 CVisionGroup::getSortKey() const { return _SortKey; }

inline void CVisionGroup::setSortKey(uint32 sortKey) { _SortKey = sortKey; }
...
inline void CVisionGroup::setVisionId(uint32 visionId) { _VisionId = visionId; }

inline uint32 CVisionGroup::getVisionId() const { return _VisionId; }
...
void CViewer::init(TDataSetRow viewerId, CUniverse *theUniverse) {
  ...
  BOMB_IF(_ViewerId != TDataSetRow(),
          "Call to init() for already initialised CViewer object", return);
  ...
  // setup the vision slots in reverse order from 254..0 (because they're popped
  // from the back)
...
void CViewer::_updateVisionBlind(CUniverse *theUniverse,
                                 CPlayerVisionDelta &result) {
  ...
  // iterate down from the last vision entry to vision entry 1 (don't touch
  // entry 0 because it's reserved)
  for (uint32 i = (uint32)_Vision.size() - 1; i > 0; i--) {
    ...
    result.EntitiesOut.push_back(CPlayerVisionDelta::CIdSlot(
        visionEntry.DataSetRow, (uint8)visionEntry.VisionSlot));
    TEST(("Vision DROP entity %d[%d] x %d for vision reset",
          _ViewerId.getIndex(), visionEntry.VisionSlot,
          visionEntry.DataSetRow.getIndex()));

    // if we're in verbose log mode then do a bit of logging
    if (VerboseVisionDelta) {
      nlinfo("Vision Delta: %s: Generating empty vision because "
             "_VisionResetCounter!=0",
             _ViewerId.toString().c_str());
    }
...
void CViewer::_updateVisionNormal(const TVision &vision, CUniverse *theUniverse,
                                  bool firstTime, CPlayerVisionDelta &result) {
  // prepare to apply our group's vision to our own...
  sint32 oldVisionIdx =
      (sint32)_Vision.size() -
      1; // note that there is always a dummy entry at start of vision vector
  sint32 newVisionIdx =
      (sint32)vision.size() -
      1; // note that there is always a dummy entry at start of vision vector
  static std::vector<uint32>
      newVisionEntries; // temp vector - static for reasons of optimisation
  newVisionEntries.clear();

  // run through the two sorted vision vectors from top to bottom, stopping when
  // they both reach the bottom
  while ((newVisionIdx | oldVisionIdx) != 0) {
    ...
    // see whether we have a new entry to add, an unchanged entry to ignore or
    // an out of date entry to delete
    if (newVisionEntry.DataSetRow == oldVisionEntry.DataSetRow) {
      ...
      if (newVisionEntry.DataSetRow.getIndex() >
          oldVisionEntry.DataSetRow.getIndex()) {
        // make sure we don't add ourselves back into our own vision a second
        // time after teleportation etc but do allow us to be added in on the
        // first pass when _Vision is empty (there is always a vision terminator
        // entry so the empty vision case comes when _Vision.size()==1)
        if (newVisionEntry.DataSetRow != _ViewerId || firstTime) {
          // add a new entry to the vision
          // add to a temp vector of new entries that haven't yet had a vision
          // slot allocation
          ...
          if (VerboseVisionDelta) {
            nlinfo("Vision Delta: %s: Becomes visible to self",
                   _ViewerId.toString().c_str());
          }
        }
        --newVisionIdx;
        ...
        if (visionSlot != 0) {
          ...
          // add to the 'removed vision entries' result vector
          result.EntitiesOut.push_back(CPlayerVisionDelta::CIdSlot(
              oldVisionEntry.DataSetRow, (uint8)visionSlot));
          TEST(("Vision DROP entity %d[%d] x %d (grp %d)", _ViewerId.getIndex(),
                visionSlot, oldVisionEntry.DataSetRow.getIndex(), VisionId));
          ...
          if (VerboseVisionDelta) {
            nlinfo(
                "Vision Delta: %s: Rescuing Vision entry when going invisible",
                _ViewerId.toString().c_str());
          }
        }
        --oldVisionIdx;
  ...
  // calculate the number of free vision slots remaining once we deal with the
  // new vision entries
  sint32 freeVisionSlotOffset =
      (sint32)(_FreeVisionSlots.size() - newVisionEntries.size());
  BOMB_IF(freeVisionSlotOffset < 0,
          "BIG BAD BUG generating vision ... too few free vision slots found",
          newVisionEntries.resize(_FreeVisionSlots.size()));
  ...
  for (uint32 i = (uint32)newVisionEntries.size(); i--;) {
    ...
    result.EntitiesIn.push_back(CPlayerVisionDelta::CIdSlot(
        visionEntry.DataSetRow, (uint8)visionEntry.VisionSlot));
    TEST(("Vision ADD entity %d[%d] = %d (grp %d)", _ViewerId.getIndex(),
          visionEntry.VisionSlot, visionEntry.DataSetRow.getIndex(), VisionId));
  }
...
void CViewer::updateVision(const TVision &vision, CUniverse *theUniverse,
                           bool firstTime) {
  H_AUTO(CViewer_UpdateVision)

  // we conserve a static variable that we use for building our results in as it
  // saves time
  ...
  BOMB_IF(_ViewerId == TDataSetRow(),
          "Call to updateVision() for uninitialised CViewer object", return);

  // if we're resting the player vision then treat ourselves as blind unless
  // this is the first update in which case we need to be allowed to add
  // ourselves into our own vision
  if (_VisionResetCount != 0 && !firstTime) {
    ...
    // decrement the count of vision iterations still to be skipped to complete
    // this reset
    --_VisionResetCount;

    // perform a vision update that clears out all vision contents other than
    // self
  ...
  if (!result.empty()) {
    // if we're in verbose log mode then do a bit of logging
    if (VerboseVisionDelta) {
      nlinfo("Vision Delta: %s: %s", _ViewerId.toString().c_str(),
             result.toString().c_str());
    }

    // add our result to the messages to be dispatched to the FES at end of
    // tick...
...
void CViewer::dump(NLMISC::CLog &log) {
  // display a few stats
  log.displayNL("%s (%d,%d) VisionLvl(%u)", _ViewerId.toString().c_str(), _X,
                _Y, _VisionLevel);
  ...
  for (uint32 i = 0; i < _Vision.size(); ++i) {
    if (!visionString.empty())
      visionString += ' ';
    visionString += NLMISC::toString("[%u]", _Vision[i].VisionSlot) +
                    _Vision[i].DataSetRow.toString();
  }
  log.displayNL("--- Vision: %s", visionString.c_str());
...
inline TDataSetRow CViewer::getViewerId() const { return _ViewerId; }

inline uint32 CViewer::getViewerIdx() const { return _ViewerId.getIndex(); }
...
inline uint32 CViewer::getX() const { return _X; }

inline uint32 CViewer::getY() const { return _Y; }

inline void CViewer::forceResetVision() {
  // if we're not already resetting vision...
  if (_VisionResetCount == 0) {
    // setup the vision counter to reset vision for next n updates in order to
    // allow FES time to clear out player vision

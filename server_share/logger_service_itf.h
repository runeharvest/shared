...
struct TSupportedParamType {
  ...
  static const NLMISC::CStringConversion<TValues> &getConversionTable() {
    NL_BEGIN_STRING_CONVERSION_TABLE(TValues) NL_STRING_CONVERSION_TABLE_ENTRY(
        spt_uint32) NL_STRING_CONVERSION_TABLE_ENTRY(spt_uint64)
...
public:
TSupportedParamType() : _Value(invalid_val) {}
TSupportedParamType(TValues value) : _Value(value) {}
...
//	template <class T>	LGS::TSupportedParamType getParamType<uint32>()
//{ return LGS::TSupportedParamType::spt_uint32;} 	template <class T>
//LGS::TSupportedParamType getParamType<uint64>() { return
//LGS::TSupportedParamType::spt_uint64;} 	template <class T>
//LGS::TSupportedParamType getParamType<sint32>() { return
//LGS::TSupportedParamType::spt_sint32;} 	template <class T>
//LGS::TSupportedParamType getParamType<float>() { return
//LGS::TSupportedParamType::spt_float;} 	template <class T>
//LGS::TSupportedParamType getParamType<std::string>() { return
//LGS::TSupportedParamType::spt_string;} 	template <class T>
//LGS::TSupportedParamType getParamType<NLMISC::CEntityId>() { return
//LGS::TSupportedParamType::spt_entityId;} 	template <class T>
//LGS::TSupportedParamType getParamType<NLMISC::CSheetId>() { return
//LGS::TSupportedParamType::spt_sheetId;} 	template <class T>
//LGS::TSupportedParamType getParamType<INVENTORIES::TItemId>() { return
//LGS::TSupportedParamType::spt_itemId;}

struct TParamValue {
  TParamValue() : _Type(TSupportedParamType::invalid_val) {}
  ...
  TSupportedParamType getType() const { return _Type; }
...
public:
  ...
  void setType(TSupportedParamType value) { _Type = value; }
  // Flag indicating that this parameter is a list
  bool getList() const { return _List; }

  void setList(bool value) { _List = value; }

  bool operator==(const TParamDesc &other) const {
    return _Name == other._Name && _Type == other._Type && _List == other._List;
  }
...
public:
  //
  const std::list<TParamValue> &getParams() const { return _Params; }

  std::list<TParamValue> &getParams() { return _Params; }

  void setParams(const std::list<TParamValue> &value) { _Params = value; }
  ...
  void serial(NLMISC::IStream &s) { s.serialCont(_Params); }
...
protected:
  ...
  bool _Context;
  // The textual content of the log, contains '__CLOSE_CONTEXT__' to close a log
  // context
  std::string _LogText;
  //
...
public:
  ...
  void setLogName(std::string value) { _LogName = value; }
  // This log is a context log
  bool getContext() const { return _Context; }

  void setContext(bool value) { _Context = value; }
  // The textual content of the log, contains '__CLOSE_CONTEXT__' to close a log
  // context
  std::string getLogText() const { return _LogText; }

  void setLogText(std::string value) { _LogText = value; }
  //
  const std::vector<TParamDesc> &getParams() const { return _Params; }

  std::vector<TParamDesc> &getParams() { return _Params; }

  void setParams(const std::vector<TParamDesc> &value) { _Params = value; }
  //
  const std::vector<TParamDesc> &getListParams() const { return _ListParams; }

  std::vector<TParamDesc> &getListParams() { return _ListParams; }
  ...
  bool operator==(const TLogDefinition &other) const {
    return _LogName == other._LogName && _Context == other._Context &&
           _LogText == other._LogText && _Params == other._Params &&
...
public:
  ...
  void setTimeStamp(uint32 value) { _TimeStamp = value; }
  //
  const std::vector<TParamValue> &getParams() const { return _Params; }

  std::vector<TParamValue> &getParams() { return _Params; }

  void setParams(const std::vector<TParamValue> &value) { _Params = value; }
  //
  const std::vector<TListParamValues> &getListParams() const {
    return _ListParams;
  }

  std::vector<TListParamValues> &getListParams() { return _ListParams; }
  ...
  bool operator==(const TLogInfo &other) const {
    return _LogName == other._LogName && _TimeStamp == other._TimeStamp &&
           _Params == other._Params && _ListParams == other._ListParams;
  }
...
private:
  typedef void (CLoggerServiceSkel::*TMessageHandler)(
  ...
  void registerClient_skel(NLNET::IModuleProxy *sender,
                           const NLNET::CMessage &__message);

  void reportLog_skel(NLNET::IModuleProxy *sender,
                      const NLNET::CMessage &__message);
...
public:
  ...
  virtual void registerClient(NLNET::IModuleProxy *sender, uint32 shardId,
                              const std::vector<TLogDefinition> &logDef) = 0;
  // A client send a log
  virtual void reportLog(NLNET::IModuleProxy *sender,
                         const std::vector<TLogInfo> &logInfos) = 0;
};
...
public:
  ...
  // the log content. It is mandatory that ALL client share
  // Exactly the same definition of log.
  void registerClient(NLNET::IModule *sender, uint32 shardId,
                      const std::vector<TLogDefinition> &logDef);
  // A client send a log
  ...
  buildMessageFor_registerClient(NLNET::CMessage &__message, uint32 shardId,
                                 const std::vector<TLogDefinition> &logDef);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_reportLog(NLNET::CMessage &__message,
                            const std::vector<TLogInfo> &logInfos);
};
}

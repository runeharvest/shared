...
//
void CContinentContainer::init(uint gridWidth, uint gridHeight,
                               double primitiveMaxSize, uint nbWorldImages,
                               const string &packedSheetsDirectory,
                               double cellSize, bool loadPacsPrims) {
...
void CContinentContainer::buildSheets(const string &packedSheetsDirectory) {
  ...
  if (NLNET::IService::isServiceInitialized() &&
      (NLNET::IService::getInstance()->ConfigFile.getVarPtr(
           std::string("GeorgePaths")) != NULL)) {
    loadForm(filters, packedSheetsDirectory + "continents.packed_sheets",
             _SheetMap, false, false);
  }
...
//
void CContinentContainer::loadContinent(string name, string file, sint index,
                                        bool allowAutoSpawn) {
  nlinfo("loadContinent(\"%s\", \"%s\", %d)", name.c_str(), file.c_str(),
         index);

  // check if the continent is already loaded
  /*	{
          TContinentContainer::iterator first(_Continents.begin()),
     last(_Continents.end()); for (; first != last; ++first)
          {
              if (first->Name == name)
              {
                  nlinfo("loadContinent(\"%s\", \"%s\", %d) : continent already
     loaded, ignoring second load.", name.c_str(), file.c_str(), index); return;
              }
          }
      }
  */
  ...
  for (its = _SheetMap.begin(); its != _SheetMap.end(); ++its) {
    if (NLMISC::toLowerAscii((*its).second.Name) ==
            NLMISC::toLowerAscii(name + ".continent") ||
        NLMISC::toLowerAscii((*its).second.PacsRBank) ==
            NLMISC::toLowerAscii(name + ".rbank")) {
      if (found == _SheetMap.end()) {
        found = its;
      } else {
        nlinfo("Found 2 different possible continent sheets for %s: %s and %s, "
               "continent is not loaded twice",
               name.c_str(), (*its).second.Name.c_str(),
  ...
  if (found != _SheetMap.end()) {
    ...
    nlwarning("Couldn't find continent sheet for '%s', use name instead",
              name.c_str());
  }

  for (uint i = 0; i < _Continents.size(); ++i) {
    if (_Continents[i].Name == name) {
      nlinfo("Continent '%s' already loaded, loading aborted for this new "
             "continent.",
             name.c_str());
      return;
    }
  ...
  if (_Continents[index].RetrieverBank != NULL ||
    ...
    nlwarning(
        "Init retriever bank failed, index %d already used by continent '%s'",
        index, _Continents[index].Name.c_str());
    return;
  }
  ...
  // load the rbank
  filename = file + ".rbank";
  _Continents[index].RetrieverBank =
      URetrieverBank::createRetrieverBank(filename.c_str(), true);
  if (_Continents[index].RetrieverBank == NULL) {
  ...
  // load the gr
  filename = file + ".gr";
  _Continents[index].GlobalRetriever = UGlobalRetriever::createGlobalRetriever(
      filename.c_str(), _Continents[index].RetrieverBank);
  if (_Continents[index].GlobalRetriever == NULL) {
    nlwarning("Init global retriever failed, file load is %s",
              filename.c_str());
  ...
  _Continents[index].MoveContainer = UMoveContainer::createMoveContainer(
      _Continents[index].GlobalRetriever, gw, gh, _PrimitiveMaxSize,
      _NbWorldImages);
...
void CContinentContainer::removeContinent(sint index) {
  ...
  nlinfo("Remove continent %d '%s'... Entities shouldn't point any longer to "
         "this continent !",
         index, _Continents[index].Name.c_str());
...
void CContinentContainer::initPacsPrim(const string &path) {
  ...
  if (CFile::fileExists(CPath::lookup(path, false, false))) {
    ...
    nlwarning("CContinentContainer: can't initPacsPrim(), path '%s' for pacs "
  ...
  for (k = 0; k < fileNames.size(); ++k) {
    // check extension
    if (NLMISC::toLowerAscii(CFile::getExtension(fileNames[k])) !=
        "pacs_prim") {
      // not a pacs primitive, skip it..
      continue;
    }
    try {
      string ppName = NLMISC::toLowerAscii(
          CFile::getFilenameWithoutExtension(fileNames[k]));

      if (_PacsPrimMap.find(ppName) != _PacsPrimMap.end())
        continue;

      CUniquePtr<UPrimitiveBlock> pb(
          UPrimitiveBlock::createPrimitiveBlockFromFile(
              CPath::lookup(fileNames[k], false)));
      ...
      nlwarning("Couldn't load Pacs Primitive Block file '%s': %s",
                fileNames[k].c_str(), e.what());
    }
  }
...
//
void CContinentContainer::loadPacsPrims(const CSheet &sheet,
  ...
  if (!igFilename.empty()) {
    ...
    if (igFile.open(igFilename)) {
      ...
      nlwarning("Couldn't open file '%s' to instantiate landscape pacs_prims",
                igFilename.c_str());
    }
  } else {
    nlwarning("Couldn't find file '%s' to instantiate landscape pacs_prims",
              sheet.LandscapeIG.c_str());
  }
  ...
  for (i = 0; i < igs.size(); ++i) {
    ...
    try {
      ...
      for (uint k = 0; k < numInstances; ++k) {
        ...
        string shapeName = NLMISC::toLowerAscii(
            CFile::getFilenameWithoutExtension(igLoader.getShapeName(k)));
        string instanceName = NLMISC::toLowerAscii(
            CFile::getFilenameWithoutExtension(igLoader.getInstanceName(k)));

        bool isTrigger = false;
        bool isZC = false;

        if ((pbIt = _PacsPrimMap.find(shapeName)) != _PacsPrimMap.end() ||
            // nice hardcoded trick that allows graphists to spawn ghost
            // collisions in ZC when shapename is like 'bat_zc_0?', spaw a pacs
            // prim bloc called gen_bt_col_ext, so nice I just shit my pants
            (isZC = (testWildCard(shapeName.c_str(), "bat_zc_0?") &&
                     shapeName != "bat_zc_00" &&
                     (pbIt = _PacsPrimMap.find("gen_bt_col_ext")) !=
                         _PacsPrimMap.end())) || // the magic hack
            (isTrigger = ((pbIt = _PacsPrimMap.find(instanceName)) !=
                          _PacsPrimMap.end()))) {
          if (_LoadPacsPrims || isTrigger) {
            ...
            moveContainer->addCollisionnablePrimitiveBlock(
                pbIt->second, 0, 1, &insertedPrimitives, angle, pos, true);

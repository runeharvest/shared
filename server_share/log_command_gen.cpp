...
public:
  /// constructor
  CCommandDesc() : _NoContextCount(0) {
    _LogDefs.resize(4);
...
TLogNoContext_Command::TLogNoContext_Command() { CommandDesc.pushNoContext(); }

TLogNoContext_Command::~TLogNoContext_Command() { CommandDesc.popNoContext(); }

void _log_Command_Exec(const std::string &cmdName, const std::string &cmdArg,
  ...
  if (!CommandDesc.getContextVar_charId(charId)) {
    ...
    // 'noContext').
    STOP_IF(CommandDesc.getNoContextCount() == 0,
            _filename_ << "(" << _lineNo_
...
void _log_Command_ExecOnTarget(const NLMISC::CEntityId &targetId,
...
void _log_Command_TPOutsideNewbieland(const NLMISC::CEntityId &movedCharId,
                                      const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {

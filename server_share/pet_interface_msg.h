...
public:
  ...
  virtual void description() {
    ...
    property("AIInstanceId", PropUInt32, std::numeric_limits<uint32>::max(),
             AIInstanceId);
    property("SpawnMode", PropUInt16, (uint16)NEAR_PLAYER, SpawnMode);
    property("CharacterMirrorRow", PropDataSetRow, TDataSetRow(),
             CharacterMirrorRow);
...
public:
  ...
  enum TCommandError {
    COLLISION_NOT_VALIDNO_ERROR_SPAWN = 0,
    CHARATER_UNKNOWN,
    PET_SHEETID_UNKNOWN,
    CHARACTER_LAND_MAP_UNKNOWN,
    COORDINATE_NOT_IN_WORLD,
  ...
  sint32 Coordinate_X, Coordinate_Y,
      Coordinate_H; // For GOTO_POINT, GOTO_POINT_DESPAWN and SPAWN_POINT
                    // commands
...
public:
  ...
  CPetCommandConfirmationMsg(uint16 commandError, const CPetCommandMsg &msg)
      : CommandError(commandError), CharacterMirrorRow(msg.CharacterMirrorRow),
        PetMirrorRow(msg.PetMirrorRow) {}

  virtual void description() {
    className("CPetCommandConfirmationMsg");
    property("CommandError", PropUInt16, (uint16)NO_ERROR_COMMAND,
             CommandError);
  }
  virtual void callback(const std::string & /* name */,
                        NLNET::TServiceId /* id */) {}

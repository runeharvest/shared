...
public:
  ...
  // the date of last disconnection of the character
  uint32 getLastDisconnectionDate() const { return _LastDisconnectionDate; }
...
private:
  typedef void (CEntityLocatorSkel::*TMessageHandler)(
  ...
  void initState_skel(NLNET::IModuleProxy *sender,
                      const NLNET::CMessage &__message);

  void playerConnected_skel(NLNET::IModuleProxy *sender,
                            const NLNET::CMessage &__message);

  void playerDisconnected_skel(NLNET::IModuleProxy *sender,
                               const NLNET::CMessage &__message);

  void charConnected_skel(NLNET::IModuleProxy *sender,
                          const NLNET::CMessage &__message);

  void charDisconnected_skel(NLNET::IModuleProxy *sender,
                             const NLNET::CMessage &__message);
...
public:
  ...
  // connections
  virtual void
  initState(NLNET::IModuleProxy *sender,
            const std::vector<uint32> &connectedUsers,
            const std::vector<TConnectedCharInfo> &connectedChars) = 0;
  ...
  virtual void playerDisconnected(NLNET::IModuleProxy *sender,
                                  uint32 userId) = 0;
  // A character has connected
  virtual void charConnected(NLNET::IModuleProxy *sender,
                             const NLMISC::CEntityId &charEId,
                             uint32 lastDisconnectionDate) = 0;
  // A character has disconnected
  virtual void charDisconnected(NLNET::IModuleProxy *sender,
...
public:
  ...
  // The locator client send the initial state of active player and character
  // connections
  void initState(NLNET::IModule *sender,
                 const std::vector<uint32> &connectedUsers,
  ...
  // A character has connected
  void charConnected(NLNET::IModule *sender, const NLMISC::CEntityId &charEId,
                     uint32 lastDisconnectionDate);
  // A character has disconnected
  void charDisconnected(NLNET::IModule *sender,
                        const NLMISC::CEntityId &charEId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_initState(
  ...
  buildMessageFor_playerConnected(NLNET::CMessage &__message, uint32 userId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_playerDisconnected(NLNET::CMessage &__message, uint32 userId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_charConnected(NLNET::CMessage &__message,
                                const NLMISC::CEntityId &charEId,
                                uint32 lastDisconnectionDate);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_charDisconnected(NLNET::CMessage &__message,
                                   const NLMISC::CEntityId &charEId);
};
...
public:
  ...
  // Type of the event : true for a connection, false otherwise
  bool getConnection() const { return _Connection; }

  void setConnection(bool value) { _Connection = value; }
  // The privilege of the character (e.g :GM:DEV:)
  std::string getPrivilege() const { return _Privilege; }

  void setPrivilege(std::string value) { _Privilege = value; }
  // Last Connection Date
  uint32 getlastConnectionDate() const { return _lastConnectionDate; }

  void setlastConnectionDate(uint32 value) { _lastConnectionDate = value; }

  bool operator==(const TCharConnectionEvent &other) const {
    return _CharId == other._CharId && _Connection == other._Connection &&
...
private:
  typedef void (CEntityLocatorClientSkel::*TMessageHandler)(
      NLNET::IModuleProxy *sender, const NLNET::CMessage &message);
  typedef std::map<std::string, TMessageHandler> TMessageHandlerMap;

  const TMessageHandlerMap &getMessageHandlers() const;

  void connectionEvents_skel(NLNET::IModuleProxy *sender,
                             const NLNET::CMessage &__message);
...
public:
  ...
  virtual void
  connectionEvents(NLNET::IModuleProxy *sender,
                   const std::vector<TCharConnectionEvent> &events) = 0;
};
...
public:
  ...
  // The entity locator send a list of connection event to EGS
  void connectionEvents(NLNET::IModule *sender,
  ...
  static void
  broadcast_connectionEvents(ProxyIterator first, ProxyIterator last,
  ...
  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_connectionEvents(

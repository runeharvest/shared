...
private:
  typedef void (CBackupServiceSkel::*TMessageHandler)(
  ...
  void saveFile_skel(NLNET::IModuleProxy *sender,
                     const NLNET::CMessage &__message);

  void loadFile_skel(NLNET::IModuleProxy *sender,
                     const NLNET::CMessage &__message);
...
public:
  ...
  virtual void saveFile(NLNET::IModuleProxy *sender,
                        const std::string &fileName,
                        const NLNET::TBinBuffer &data) = 0;
  // A module ask to load a file
  virtual void loadFile(NLNET::IModuleProxy *sender,
                        const std::string &fileName, uint32 requestId) = 0;
};
...
public:
  ...
  // A module ask to save a file in the backup repository
  void saveFile(NLNET::IModule *sender, const std::string &fileName,
                const NLNET::TBinBuffer &data);
  // A module ask to load a file
  void loadFile(NLNET::IModule *sender, const std::string &fileName,
                uint32 requestId);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &
  buildMessageFor_saveFile(NLNET::CMessage &__message,
                           const std::string &fileName,
                           const NLNET::TBinBuffer &data);
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_loadFile(NLNET::CMessage &__message,
                           const std::string &fileName, uint32 requestId);
};
...
private:
  typedef void (CBackupServiceClientSkel::*TMessageHandler)(
      NLNET::IModuleProxy *sender, const NLNET::CMessage &message);
  typedef std::map<std::string, TMessageHandler> TMessageHandlerMap;

  const TMessageHandlerMap &getMessageHandlers() const;

  void loadFileResult_skel(NLNET::IModuleProxy *sender,
                           const NLNET::CMessage &__message);

  void fileUpdate_skel(NLNET::IModuleProxy *sender,
                       const NLNET::CMessage &__message);
...
public:
  ...
  virtual void loadFileResult(NLNET::IModuleProxy *sender, uint32 requestId,
                              const std::string &fileName, uint32 fileTimeStamp,
                              const NLNET::TBinBuffer &data) = 0;
  // A file listened by the client have been changed, BS resend the file content
  virtual void fileUpdate(NLNET::IModuleProxy *sender,
                          const std::string &fileName,
...
public:
  ...
  // The BS return for a load file request
  void loadFileResult(NLNET::IModule *sender, uint32 requestId,
                      const std::string &fileName, uint32 fileTimeStamp,
                      const NLNET::TBinBuffer &data);
  // A file listened by the client have been changed, BS resend the file content
  void fileUpdate(NLNET::IModule *sender, const std::string &fileName,
                  const std::vector<std::string> &content);

  // Message serializer. Return the message received in reference for easier
  // integration
  static const NLNET::CMessage &buildMessageFor_loadFileResult(
  ...
  // integration
  static const NLNET::CMessage &
  buildMessageFor_fileUpdate(NLNET::CMessage &__message,
                             const std::string &fileName,
                             const std::vector<std::string> &content);
};

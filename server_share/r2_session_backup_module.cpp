...
/*
  ...
  The persistency of index is done via file. It is perhaps unecessary. Because
  we have a Database.
  ...
  -> Send a message to Dss to tell which sesion must be restored.
    ...
    | | |			--- notifyIfSavedFile   -->		|
  // Add entry to Dss Hibernating list |	        --- ( SU) ->
  createSession-->		|	// Wake-up Session |
  | |			<-- reportSavedSessions --	    |	// updated every
  30 seconds (DSS save sessions) |           <-- reportHibernatedSessions--  |
  // updated when necessary (a session has hibernated (after X minutes of wait)
                                                |	// Remove entries from
  Dss NotifyHibernating list
    --------------------------------------------------------------------------------------------
    CR2SessionBackupModule						DSS2
    |			--- notifyIfSavedFile   -->		|	// Add
  entry to Dss Hibernating list |	        --- SU -> createSession -->
  |   // Wake-up Session |			<-- reportSavedSessions --
  |	// updated every 30 seconds (DSS save sessions) |
  --- SU -> closeSession  -->		|   // Add entry to DSS closing list |
  <-- reportClosedSessions --		|	// (DSS delete sessions)

*/

class CR2SessionBackupModule
    : public CR2SessionBackupModuleItfSkel,
...
public:
  ...
  // From Su to Dss
  virtual void notifyIfSavedFile(NLNET::IModuleProxy *moduleProxy,
                                 TShardId shardId, TSessionId sessionId);

  // From Su
  // The DSS delete the session_*, the DBM remove entries from index
  virtual void reportDeletedSessions(NLNET::IModuleProxy *moduleProxy,
                                     const std::vector<TSessionId> &sessionId);
  // Same as reportSavedSessions but DSS remove local map
  virtual void reportHibernatedSessions(NLNET::IModuleProxy *moduleProxy,
                                        const std::vector<TSessionId> &session);
  // Update local index
  virtual void
  reportSavedSessions(NLNET::IModuleProxy *moduleProxy,
...
private:
  ...
  // Name of the file that contains override ring access (It is better to use
  // DB).
...
// NLMISC_VARIABLE( std::string, AdminModuleSavePath, "File where users
// adventure are stored." );

// const std::string CR2SessionBackupModule::_AdminModuleSaveFilename(
// "server_admin_module_data.xml");
...
public:
  ...
  void callback(const CFileDescription &fileDescription,
    ...
    nldebug("R2SBM: BSIcb COverrideRingAccessCallback::callback() for file: %s",
       ...
       last(entries.end());
        ...
        for (; first != last ; ++first)
        {
            ...
            if (firstLoop)
            {
                ...
                if (tokens.size() != 2 || tokens[0] != "Version" || version !=
       FileVersion)
                {
                    nlwarning("R2SBM: Obsolete File Discard '%s'",
       fileDescription.FileName.c_str()); return;
...
public:
       ...
       fileDescription.FileName.c_str()); return;
            ...
            else
            {
                ...
                else if (tokens.size() == 7)
                {
                    ...
                    sessions[sessionId] = TSessionInfos(timestamp,
       RSMGR::TSessionType(sessionType) ,positionX, positionY, orient, season);
                }
                else
                {
                    nlstop;
  ...
  void callback(const CFileDescription &fileDescription,
                NLMISC::IStream &dataStream) {
    nldebug(
        "R2SBM: BSIcb CHibernatingSessionListCallback::callback() for file: %s",
        fileDescription.FileName.c_str());
    CR2SessionBackupModule::TSessionInfos sessions;
...
public:
  ...
  void callback(const CFileDescription &fileDescription,
                NLMISC::IStream &dataStream) {
    nldebug("R2SBM: BSIcb CSaveSessionListCallback::callback() for file: %s",
            fileDescription.FileName.c_str());
    CR2SessionBackupModule::TSessionInfos sessions;
    parseStream(fileDescription, dataStream, sessions);
...
void CR2SessionBackupModule::registerDss(NLNET::IModuleProxy *moduleProxy,
                                         TShardId shardId) {
  nldebug("R2SBM : receive DSS registration from '%s'",
          moduleProxy->getModuleName().c_str());

  bool ok =
      _ServerEditionProxys.insert(std::make_pair(shardId, moduleProxy)).second;
  if (!ok) {
    nlwarning("R2SBM: a DSS module '%s' has tryed register to the same shardId "
              "'%d' than a other dss.",
              moduleProxy->getModuleName().c_str(), shardId);
    return;
  }
  ok = _ShardIds.insert(std::make_pair(moduleProxy, shardId)).second;
  if (!ok) {
    nlwarning("R2SBM: a DSS module '%s' has tryed to register 2 times.",
              moduleProxy->getModuleName().c_str());
    return;
  }
}
...
void CR2SessionBackupModule::onModuleDown(NLNET::IModuleProxy *moduleProxy) {
  ...
  if (moduleName == "ServerEditionModule") {
    ...
    if (found != _ShardIds.end()) {
      nldebug("R2SBM: Looks like a module '%s' was down before registering. "
              "The dss must have crash at startup?",
              moduleProxy->getModuleName().c_str());
      return;
    }

    TServerEditionProxys::iterator it =
        _ServerEditionProxys.find(found->second);
    nlassert(
        it !=
        _ServerEditionProxys.end()); // first check has been done in registerDss
    _ServerEditionProxys.erase(it);
    _ShardIds.erase(found);
  }
}

bool CR2SessionBackupModule::onProcessModuleMessage(
...
void CR2SessionBackupModule::onServiceUp(const std::string &serviceName,
                                         uint16 serviceId) {
  if (serviceName == "BS") {
...
void CR2SessionBackupModule::onModuleUpdate() {
  ...
  if (_BsGoingUp) {
    nldebug("BSI requesting files: '%s', '%s', '%s'",
            getOverrideRingAccessFilename().c_str(),
            getHibernatingSessionListFilename().c_str(),
            getSavedSessionListFilename().c_str());

    std::vector<CBackupFileClass> classes;
    // Obsolet system of right
    BsiGlobal.requestFile(getOverrideRingAccessFilename(),
                          new COverrideRingAccessCallback(this));
    BsiGlobal.requestFile(getHibernatingSessionListFilename(),
                          new CHibernatingSessionListCallback(this));
    BsiGlobal.requestFile(getSavedSessionListFilename(),
                          new CSaveSessionListCallback(this));
...
void CR2SessionBackupModule::updateSessionListFile() {
  /*
      ...
      if (_MustUpdateHibernatingFileList)
      {
          ...
          TSessionInfos::const_iterator first( _HibernatingSessions.begin() ),
     last( _HibernatingSessions.end() );
          ...
          {
              std::string tmp =
     NLMISC::toString("%u\t%s\t%u\t%lf\t%lf\t%lf\t%u\n", first->first.asInt(),
     first->second.SessionType.toString().c_str(),
     first->second.HibernationDate, first->second.PositionX,
     first->second.PositionY, first->second.Orient, first->second.Season); ss <<
     tmp;
          }
          std::string copy = ss.str();

          nldebug("UPDATING hibernating file list - BSI send file:
     %s",getHibernatingSessionListFilename().c_str()); CBackupMsgSaveFile msg(
     getHibernatingSessionListFilename(), CBackupMsgSaveFile::SaveFileCheck, Bsi
      ...
      {
          TSessions::const_iterator first( _Sessions.begin() ), last(
     _Sessions.end() );
          ...
          for (; first != last; ++first)
          {
              if (first->second->getLastSaveTime())
              {
                  ...
                  std::string basic = NLMISC::toString("%u\t%s\t%u\t",
     first->first.asInt(), first->second->getSessionType().toString().c_str(),
     first->second->getLastSaveTime()); std::string position =
     NLMISC::toString("%lf\t%lf\t%lf\t%u\n", x, y, orient, season); std::string
     tmp = toString("%s%s", basic.c_str(), position.c_str());
          ...
          nldebug("UPDATING saved session file list - BSI send file:
     %s",getSavedSessionListFilename().c_str()); CBackupMsgSaveFile msg(
     getSavedSessionListFilename(), CBackupMsgSaveFile::SaveFileCheck, Bsi );
          msg.DataMsg.serialBuffer((uint8*)copy.c_str(), copy.size());
          Bsi.sendFile( msg );
      }
     ...
     last(_ClosedSessions.end()); for (; first != last; ++first)
          {
              nldebug("CLEANING closed session - BSI delete file:
     %s",getSessionFilename(*first).c_str()); Bsi.deleteFile(
     getSessionFilename(*first), false);
          }
     ...
     first->second.c_str()); ss << tmp;

          }
          std::string copy = ss.str();

          nldebug("UPDATING ring access override file list - BSI send file:
     %s",getOverrideRingAccessFilename().c_str()); CBackupMsgSaveFile msg(
     getOverrideRingAccessFilename(), CBackupMsgSaveFile::SaveFileCheck, Bsi );
          msg.DataMsg.serialBuffer((uint8*)copy.c_str(), copy.size());
          Bsi.sendFile( msg );
      }
  */
...
void CR2SessionBackupModule::resetHibernatingSessionList(
    TSessionInfos &sessions) {
...
void CR2SessionBackupModule::addSavedSessionList(TSessionInfos &sessions) {
  //	_HibernatingSessions.swap(sessions);
  if (!sessions.empty()) {
    nlwarning("R2SBM: Warning: The server must have crashed. Because some "
              "saved file have not been synchronized. So we will define has "
              "hibernationg %u session",
              sessions.size());
  }
  ...
  for (; first != last; ++first) {
    ...
    nldebug("R2SBM: ADD HIBERNATE SESSION: "
            "CServerEditionModule::addSavedSessionList(): sessionId=%u ",
            first->first.asInt());
...
// The module send a message to a dss saying that the dss must wakeup a session
// from hibernation
void CR2SessionBackupModule::notifyIfSavedFile(NLNET::IModuleProxy *moduleProxy,
                                               TShardId shardId,
                                               TSessionId sessionId) {
  if (_WaitingForBS) {
    nlwarning(
        "R2SBM: try to send message to dss before data where loding from BS");
    return;
  }

  TServerEditionProxys::const_iterator found(
      _ServerEditionProxys.find(shardId));
  if (found == _ServerEditionProxys.end()) {
    nlwarning(
        "R2SBM: try to create a session on a DSS that do not exist yet...");
    return;
  }

  //<sql>
  TSessionInfos::const_iterator sessionFound =
...
// From Su
// The DSS delete the session_*, the DBM remove entries from index
void CR2SessionBackupModule::reportDeletedSessions(
    NLNET::IModuleProxy *moduleProxy,
  ...
  if (foundShard == _ShardIds.end()) {
    nlwarning("R2SBM: Message from an unregistered proxy '%s', "
              "moduleProxy->getModuleName().c_str()");
    return;
  }
  ...
  TActiveShards::iterator foundActiveShard =
      _ActiveShards.find(foundShard->second);
  std::vector<TSessionId>::const_iterator first(sessionIds.begin()),
      last(sessionIds.end());
  for (; first != last; ++first) {
    ...
    TSessionInfos::iterator hibernatingSession =
        _HibernatingSessions.find(sessionId);
...
// Same as reportSavedSessions but DSS remove local map
void CR2SessionBackupModule::reportHibernatedSessions(
    NLNET::IModuleProxy *moduleProxy,
    const std::vector<TSessionId> &sessionIds) {
  if (_WaitingForBS) {
    nlwarning(
  ...
  for (; first != last; ++first) {
    ...
    uint32 now = NLMISC::CTime::getSecondsSince1970();
    _HibernatingSessions[sessionId].LastDateUsed =
        now; // Update or create entry
  }
  //</sql>
}

// Update local index
void CR2SessionBackupModule::reportSavedSessions(
    NLNET::IModuleProxy *moduleProxy,
    const std::vector<TR2SbmSessionInfo> &sessionInfos) {
  if (_WaitingForBS) {
    nlwarning(
  ...
  /*
      ...
      TActiveShards::iterator
     foundActiveShard(_ActiveShards.find(foundShard->second));

      std::vector<std::pair<TSessionId, TSessionInfo> >::const_iterator
     first(sessionInfos.begin()), last(sessionInfos.end());
      foundActiveShard->second.clear();
      foundActiveShard->second.insert(first, last);

      for (; first != last ; ++first)
      {
          TSessionInfos::iterator
     found(_HibernatingSessions.find(first->first)); if (found !=
     _HibernatingSessions.end())
          {
...
void CR2SessionBackupModule::swapOverrideRingAccess(
    TOverrideRingAccess &access) {

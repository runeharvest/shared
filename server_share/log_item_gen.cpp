...
public:
  ...
  CItemDesc() : _NoContextCount(0) {
...

    {
      ...
      logDef.setLogText(
          "Cannot put the bought item in the character inventory");
...

    {
      ...
      logDef.setLogText(
          "An item was sold through the store and money is received");
...

    {
      ...
      logDef.setLogText("A player character enchants an item with a "
                        "crystallized spell phrase or recharges the sap load");
  ...
  void popContextVar_charId() { _charId.pop_back(); }
  ...
  void popContextVar_validatorCharId() { _validatorCharId.pop_back(); }
  ...
  void popContextVar_otherEntityId() { _otherEntityId.pop_back(); }

  void pushNoContext() { ++_NoContextCount; }
  void popNoContext() {
    nlassert(_NoContextCount > 0);
    --_NoContextCount;
  }

  uint32 getNoContextCount() { return _NoContextCount; }
};
...
TLogContext_Item_Exchange::TLogContext_Item_Exchange(
    const NLMISC::CEntityId &validatorCharId,
    const NLMISC::CEntityId &otherEntityId) {
  if (LGS::ILoggerServiceClient::isInitialized())
    LGS::ILoggerServiceClient::getInstance()->pushLogContext(_ContextName);
...
TLogNoContext_Item::TLogNoContext_Item() { ItemDesc.pushNoContext(); }

TLogNoContext_Item::~TLogNoContext_Item() { ItemDesc.popNoContext(); }

void _log_Item_Create(INVENTORIES::TItemId itemId,
                      const NLMISC::CSheetId &sheetId, uint32 quantity,
                      uint32 quality, const char *_filename_, uint _lineNo_) {
  ...
  if (!ItemDesc.getContextVar_charId(charId)) {
    // If this bomb is thrown, you need to add a log context (or otherwise a
    // 'noContext').
    STOP_IF(ItemDesc.getNoContextCount() == 0,
            _filename_ << "(" << _lineNo_
...
void _log_Item_UpdateQuantity(INVENTORIES::TItemId itemId, uint32 quantity,
                              uint32 oldQuantity, const char *_filename_,
                              uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {
...
void _log_Item_Move(INVENTORIES::TItemId itemId,
                    INVENTORIES::TInventory srcInventoryId,
                    INVENTORIES::TInventory dstInventoryId,
                    const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {
  ...
  logInfo.getParams()[2] =
      LGS::TParamValue(INVENTORIES::toString(srcInventoryId));

  logInfo.getParams()[3] =
      LGS::TParamValue(INVENTORIES::toString(dstInventoryId));
...
void _log_Item_PutInSaleStore(INVENTORIES::TItemId itemId,
                              const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {
...
void _log_Item_RemoveFromSaleStore(INVENTORIES::TItemId itemId,
                                   const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {
...
void _log_Item_Delete(INVENTORIES::TItemId itemId,
                      const NLMISC::CSheetId &sheetId, uint32 quantity,
                      uint32 quality, const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
...
void _log_Item_FailedAddBoughtItem(INVENTORIES::TItemId itemId,
                                   INVENTORIES::TInventory dstInventoryId,
                                   const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
...
void _log_Item_Money(uint64 moneyBefore, uint64 moneyAfter,
                     const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {

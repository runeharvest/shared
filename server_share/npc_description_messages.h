...
// The following serve for controlling verbose nature of logging - LOG is
// undefined at end of file
extern bool VerboseNpcDescriptionMsgLog;
#define LOG                                                                    \
  if (!(VerboseNpcDescriptionMsgLog && Mode == 4)) {                           \
  } else                                                                       \
    nlinfo

// build a man-readable description of a chat parameter set from component parts
std::string
buildChatDebugString(const std::vector<uint32> &ShopCategories,
...
public:
  CNpcChatProfile()
      : /*_guildCreator(false),*/ /*_dynamicMissionGiver(false),*/
        _FilterExplicitActionTradeByPlayerRace(false),
        _ExplicitActionSPType(EGSPD::CSPType::Unknown),
        _FilterExplicitActionTradeByBotRace(true), _DynamicMissionGiver(false),
        _Organization(0) {}
  ...
  const std::vector<RYMSG::TExplicitSale> getExplicitSales() const {
    return _ExplicitSales;
  }
  //	const std::vector<NLMISC::CSheetId>	&getShopItemTypes()	{ return
  //_shopItemTypes; } 	const std::vector<uint32>
  //&getShopItemQualities()	{ return _shopItemQualities; } 	const
  //std::vector<sint32>			&getShopItemPrices() const { return
  //_shopItemPrices; } 	const std::vector<uint8>
  //&getShopItemFactionTypes()	const { return _shopItemFactionTypes; } 	const
  //std::vector<uint32>			&getShopItemFactionPointPrices() const
  //{ return _shopItemFactionPointPrices; }
  const std::vector<uint32> &getMissions() const { return _Missions; }
  //	bool
  //getGuildCreator() const{ return _GuildCreator; }
  ...
  const std::vector<RYMSG::TContextOption> &getContextOptions() const {
    return _ContextOptions;
  };
  //	const std::vector<std::string>		&getContextOptionsTitles() const
  //{ return _ContextOptionsTitles;} 	const std::vector<std::string>
  //&getContextOptionsDetails() const  { return _ContextOptionsDetails;}
  const std::vector<std::string> &getOptionalProperties() const {
    return _OptionalProperties;
  }

  //	const std::vector<std::string>		&getShopNamedItemNames() const {
  //return _shopNamedItemNames; } 	const std::vector<uint32>
  //&getShopNamedItemPrices() const { return _shopNamedItemPrices; } 	const
  //std::vector<uint8>			&getShopNamedItemFactionTypes() const {
  //return _shopNamedItemFactionTypes; } 	const std::vector<uint32>
  //&getShopNamedItemFactionPointPrices() const { return
  //_shopNamedItemFactionPointPrices; }

  //	const std::vector<NLMISC::CSheetId>	&getExplicitActionTradeList()
  //const	{ return _ExplicitActionTradeList; }
...
protected:
  ...
  //	std::vector<NLMISC::CSheetId>	_shopItemTypes;			// list
  //of specific items to be found in shop 	std::vector<uint32>
  //_shopItemQualities;		// list of quality of specific item
  //	std::vector<sint32>				_shopItemPrices;
  //// list of specific item prices. -1 indicate "use standard price"
  //	std::vector<uint8>				_shopItemFactionTypes;
  //	std::vector<uint32>
  //_shopItemFactionPointPrices; 	std::vector<NLMISC::CSheetId>
  //_shopItemTypesNegators;	// field used when combining chat profiles
  //	std::vector<uint32>
  //_shopItemQualitiesNegators;	// list of quality of specific item
  std::vector<uint32> _ShopTypes; // categories of shop inventory to include
  //	std::vector<uint32>				_shopTypesNegators;
  //// field used when combining chat profiles

  //	std::vector<std::string>		_shopNamedItemNames;
  //	std::vector<uint32>				_shopNamedItemPrices;
  //	std::vector<uint8> _shopNamedItemFactionTypes; 	std::vector<uint32>
  //_shopNamedItemFactionPointPrices;

  //	std::vector<NLMISC::CSheetId>	_ExplicitActionTradeList;	// list
  //of specific actions to be found in shop
  ...
  //	std::vector<uint32>				_missionsNegators;
  //// field used when combining chat profiles
  ...
  //	bool							_GuildCreator;
  //// true if the bot proposes guild creation
  ...
  /// titles of the bot context options ContextOptionsDetails.size() ==
  /// ContextOptionsTitles.size()
  //	std::vector< std::string >		_ContextOptionsTitles;
  /// detail text of the option WARNING: we assert that
  /// ContextOptionsDetails.size() == ContextOptionsTitles.size()
...
struct CCharacterBotChatBeginEnd : public CMirrorTransportClass {
  // Container contains pair of Player character / bot which start or end
  // chating
...
struct CCustomElementId {
  CCustomElementId(uint32 alias = 0, const std::string &id = "")
      : PrimAlias(alias), Id(id) {}
  uint32 PrimAlias;
  std::string Id;
  void serial(NLMISC::IStream &f);
};

inline bool operator<(const CCustomElementId &leftHand,
                      const CCustomElementId &rightHand) {
...
// the key is a pair<primAlias, UserModelId>, the value is the content of the
// script
...
// key : pair<primAlias, customLootTableId>, value : all associated custom loot
// sets + money info

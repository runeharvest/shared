...
/** escape string using 'mysql_real_escape_string' and return a the escaped
 *string. Note that the function is internally optimized to quickly build the
 *return string with very little overhead.
 */
const std::string &escapeString(const std::string &str, CConnection &dbCnx);

class CConnection {
  friend const std::string &escapeString(const std::string &str,
                                         CConnection &dbCnx);
...
public:
  CConnection() : _MysqlContext(NULL), _Connected(false) {}
  ...
  bool connect(const std::string &hostName, const std::string &userName,
               const std::string &password, const std::string &defaultDatabase,
               uint port);
  ...
  uint32 getLastGeneratedId() { return uint32(mysql_insert_id(_MysqlContext)); }
...
protected:
  ...
  CResultBase(MYSQL_RES *result)
      : _CurrentRow(NULL), _FieldLength(NULL), _Result(result) {}

public:
  ~CResultBase() { mysql_free_result(_Result); }

  /// return the number of fields in the result set
  uint32 getNumFields() { return mysql_num_fields(_Result); }
...
class CUseResult : public CResultBase {
  ...
  CUseResult(MYSQL_RES *result) : CResultBase(result) {}
...
class CStoreResult : public CResultBase {
  ...
  CStoreResult(MYSQL_RES *result) : CResultBase(result) {}

public:
  /// Return the number of row in the result set
  uint32 getNumRows() { return uint32(mysql_num_rows(_Result)); }
...
enum TObjectState {
  ...
  os_clean,
  /// This is a persistent object, modified in transient space that need to be
  /// saved to db
  os_dirty,
  /// This is removed object, it should no more be used as it have been removed
  /// from db
  os_removed,
...
class CPersistentCache : public NLNET::IServiceUpdatable,
  ...
  CPersistentCache() {
    NLMISC::CCommandRegistry::getInstance().registerNamedCommandHandler(
        this, "CPersistentCache");
  }
...
public:
  ...
  void serviceLoopUpdate() {
    ...
    TUpdateFuncs::iterator first(_UpdateFuncs.begin()),
        last(_UpdateFuncs.end());
  ...
  /// Return the grand total of instance in memory (including in use and in
  /// cache objects)
  ...
  NLMISC_COMMAND_HANDLER_ADD(
      CPersistentCache, clearCache,
      "remove any unreferenced cached object from memory", "no params")
  NLMISC_COMMAND_HANDLER_ADD(CPersistentCache, dump, "dump cache status",
                             "no params")

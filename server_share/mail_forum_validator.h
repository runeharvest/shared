...
public:
  typedef void (*TMailNotification)(const std::string &username,
  ...
  static void validateUserEntry(uint32 homeMainlandId,
                                const std::string &userName,
                                const std::string &cookie);

  /// Unvalidate mail/forum user entry
  static void unvalidateUserEntry(uint32 homeMainlandId,
                                  const std::string &userName);

  /// Change player/guild name
  static void changeUserName(uint32 homeMainlandId, const std::string &oldname,
...
public:
  /// received new mail!
  static void cbMailNotification(NLNET::CMessage &msgin,
                                 const std::string &serviceName,
                                 NLNET::TServiceId serviceId);

  /**
   * Name translator
   * Takes an utf-8 string and returns an ascii string, so names can be
   * represented as filenames
   */
  static std::string nameToFile(const std::string &name);

  /**
   * Name translator
   * Takes an ascii string and returns an utf-8 string, so filenames can be
   * decoded into names
   */

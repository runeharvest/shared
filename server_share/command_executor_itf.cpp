...
const CCommandExecutorSkel::TMessageHandlerMap &
CCommandExecutorSkel::getMessageHandlers() const {
  ...
  if (!init) {
    ...
    res = handlers.insert(std::make_pair(
        std::string("CE_SC"), &CCommandExecutorSkel::sendCommand_skel));
...
bool CCommandExecutorSkel::fwdOnProcessModuleMessage(
...
void CCommandExecutorSkel::sendCommand_skel(NLNET::IModuleProxy *sender,
...
//
void CCommandExecutorProxy::sendCommand(NLNET::IModule *sender,
                                        const std::string &commandName,
                                        const NLMISC::CEntityId &senderEId,
                                        bool haveTarget,
                                        const NLMISC::CEntityId &targetEId,
                                        const std::string &arg) {
  if (_LocalModuleSkel && _LocalModule->isImmediateDispatchingSupported()) {
    // immediate local synchronous dispatching
    _LocalModuleSkel->sendCommand(
        _ModuleProxy->getModuleGateway()->getPluggedModuleProxy(sender),
        commandName, senderEId, haveTarget, targetEId, arg);
  } else {
    // send the message for remote dispatching and execution or local queing
    NLNET::CMessage __message;

    buildMessageFor_sendCommand(__message, commandName, senderEId, haveTarget,
                                targetEId, arg);
...
// integration
const NLNET::CMessage &CCommandExecutorProxy::buildMessageFor_sendCommand(
    NLNET::CMessage &__message, const std::string &commandName,
    const NLMISC::CEntityId &senderEId, bool haveTarget,
    const NLMISC::CEntityId &targetEId, const std::string &arg) {

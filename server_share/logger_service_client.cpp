...
CVariable<bool> VerboseLogger("LGS", "VerboseLogger",
                              "Activate verbose logging in serive output",
                              false, 0, true);
...
class CLoggerServiceClient
    : public ILoggerServiceClient,
      public CEmptyModuleServiceBehav<
          CEmptyModuleCommBehav<CEmptySocketBehav<CModuleBase>>>,
      IModuleTrackerCb {
...
public:
  CLoggerServiceClient()
      : _LoggerServices(TModuleClassPred("LoggerService")), _NbOpenContext(0) {
  ...
  void onModuleUpdate() {
    if (getGlobal_CommStarted()) {
      if (_NbOpenContext != 0) {
        WARN("LoggerClient : there are "
             << _NbOpenContext
             << " open context not closed !. Logs for this loop will be lost");
        // TODO : have a better handling of unpaired log context to not loast
        // all logs
        _LogInfos.clear();
      } else {
        // send the logs to all known logger services
        const TLoggerServices::TTrackedModules &loggers =
            _LoggerServices.getTrackedModules();

        TLoggerServices::TTrackedModules::iterator first(loggers.begin()),
            last(loggers.end());
  ...
  void registerWithLogger(IModuleProxy *loggerService) {
    ...
    TLogDefinitions::iterator first(getLogDefinitions().begin()),
        last(getLogDefinitions().end());
    for (; first != last; ++first) {
  ...
  static void addLogDefinitions(const std::vector<TLogDefinition> &logDefs) {
    BOMB_IF(getGlobal_CommStarted(),
            "Registering of log definition done AFTER comm started !", return);

    for (uint i = 0; i < logDefs.size(); ++i) {
      nlassert(getLogDefinitions().find(logDefs[i].getLogName()) ==
               getLogDefinitions().end());
      getLogDefinitions().insert(
          make_pair(logDefs[i].getLogName(), logDefs[i]));
    }
  }
  ...
  /// Send a log to the logger service (actual send granularity is handled by
  /// CLoggerServiceClient)
  void sendLog(const TLogInfo &logInfo) {
    TLogDefinitions::iterator it(
        getLogDefinitions().find(logInfo.getLogName()));
    BOMB_IF(it == getLogDefinitions().end(),
            "sendLog : Unknow log or log context named '"
                << logInfo.getLogName() << "'",
            return);
    BOMB_IF(it->second.getParams().size() != logInfo.getParams().size(),
            "sendLog : on log " << logInfo.getLogName()
                                << ", invalid number of log parameter. Need "
                                << it->second.getParams().size()
                                << ", received " << logInfo.getParams().size(),
            return);
    ...
    if (VerboseLogger) {
      ...
      char buffer[1024];
      int pos = sprintf(buffer, "LGS : log : %s : %s", ld.getLogName().c_str(),
                        ld.getLogText().c_str());
      for (uint i = 0; i < ld.getParams().size(); ++i) {
        pos += sprintf(buffer + pos, ", %s = %s",
                       ld.getParams()[i].getName().c_str(),
                       logInfo.getParams()[i].toString().c_str());
      }
      nlinfo("%s", buffer);
    }
  }

  /// Push a new log context (any following logs will be stored inside this
  /// context)
  void pushLogContext(const std::string &contextName) {
    ...
    BOMB_IF(it == getLogDefinitions().end(),
            "pushLogContext : Unknow log or log context named '" << contextName
                                                                 << "'",
            return);
    const TLogDefinition &ld = it->second;
    BOMB_IF(!ld.getContext(),
            "Push log context with name '" << contextName
                                           << "' is not a context name",
            return);
  ...
  void popLogContext(const std::string &contextName) {
    ...
    while (it != _LogInfos.rend() && it->getTimeStamp() != 0 &&
           it->getLogName() != contextName)
      ++it;

    BOMB_IF(it == _LogInfos.rend(),
            "popLogContext : Can't find opening context", return);
...
///////////////////////////////////////////////////////////////////////////
//	bool
//CLoggerServiceClient::_CommStarted = false;
...
/// Add a set of log description
void ILoggerServiceClient::addLogDefinitions(
    const std::vector<TLogDefinition> &logDefs) {
...
void ILoggerServiceClient::startLoggerComm() {
  ...
  if (CLoggerServiceClient::isInitialized()) {
    ...
    static_cast<CLoggerServiceClient *>(ILoggerServiceClient::getInstance())
        ->startLoggerComm();
  }
}

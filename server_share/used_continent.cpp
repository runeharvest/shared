...
CUsedContinent::CUsedContinent() {
  ...
  if (usedCont == NULL) {
    nlwarning(
        "UsedContinent: can't find the configuration var 'UsedContinenent' !");
  } else {
    ...
    for (uint i = 0; i < usedCont->size() / 2; ++i) {
      ...
      if (usedNumbers.find(ci.ContinentInstance) != usedNumbers.end()) {
        nlwarning("UsedContinent: error: continent '%s': instance number '%s' "
                  "invalid or already used, continent will not be available",
                  ci.ContinentName.c_str(),
                  usedCont->asString(i * 2 + 1).c_str());
      } else {
        nlinfo("UsedContinent: using continent '%s' with instance %u'",
               ci.ContinentName.c_str(), ci.ContinentInstance);
  ...
  if (translator == NULL) {
    nlwarning("Can't find the logical to physical continent name translation "
              "table !");
  } else {
...
bool CUsedContinent::isContinentUsed(const std::string &continentName) const {
  TUsedContinentCont::const_iterator it(find_if(
      _Continents.begin(), _Continents.end(), TFindContinent(continentName)));

  return it != _Continents.end();
}

uint32 CUsedContinent::getInstanceForContinent(
...
uint32 CUsedContinent::getInstanceForContinent(
  ...
  TUsedContinentCont::const_iterator it(
      find_if(_Continents.begin(), _Continents.end(),
              TFindContinentFromEnum(continentEnum)));
...
const std::string &
CUsedContinent::getContinentForInstance(uint32 instanceNumber) const {
  static const string emptyString;
  TUsedContinentCont::const_iterator it(find_if(
      _Continents.begin(), _Continents.end(), TFindInstance(instanceNumber)));
...
std::string
CUsedContinent::getPhysicalContinentName(const std::string &logicalName) const {
  std::map<std::string, std::string>::const_iterator it(
      _PhysicalNames.find(logicalName));

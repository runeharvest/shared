...
class CViewer; // a viewer entity record within a CVisionGroup (can migrate from
               // one vision group to another)

struct SUniverseEntity; // an entity record within a CUniverse
struct SInstanceEntity; // an entity record within a CInstance
struct SViewedEntity;   // an element of the vision descriptions of both
                        // CVisionGroup and CViewer

struct SDelayedPlayerPosition; // a teleport position for a player to be applied
                               // with a delay

class IVisionDeltaManager; // an interface class for defining a callback object
                           // that manages vision deltas

//-----------------------------------------------------------------------------------------------------------------
// constants for visibility / invisibility management
//-----------------------------------------------------------------------------------------------------------------

// the following invisibility levels are stored in the "WHO_SEES_ME" mirror
// property
enum TInvisibilityLevel {
  ...
  // presets defining who I can see and who sees me (who I can see * 16 + who
  // sees me)
  ...
  WHOSEESME_INVISIBLE_PLAYER =
      (VISIBLE << NUM_WHOSEESME_BITS) | INVISIBLE_PLAYER,
  ...
  WHOSEESME_INVISIBLE_SGM =
      (INVISIBLE_SGM << NUM_WHOSEESME_BITS) | INVISIBLE_SGM,

  WHOSEESME_VISIBLE_DEV = (INVISIBLE_DEV << NUM_WHOSEESME_BITS) | VISIBLE,
  WHOSEESME_INVISIBLE_DEV =
      (INVISIBLE_DEV << NUM_WHOSEESME_BITS) | INVISIBLE_DEV,
...
struct SInstanceEntity {
  ...
  TInvisibilityLevel
      InvisibilityLevel; // value used to determine which groups can see me
...
struct SViewedEntity {
  ...
  mutable uint32 VisionSlot; // this is mutable to allow for easy use in
                             // CViewer::updateVision()
  ...
  // ctor
  SViewedEntity() { VisionSlot = std::numeric_limits<uint32>::max(); }
};
...
inline uint64 buildWhoSeesMe(TInvisibilityLevel playerInvisibiltyLevel,
                             bool visibleToMobs) {
  return (visibleToMobs ? UINT64_CONSTANT(0xffffffff00000000)
                        : UINT64_CONSTANT(0x0000000000000000)) |
         (uint64)playerInvisibiltyLevel;
}
...
struct SUniverseEntity {
  ...
  uint32 AIInstance;      // the id of the instance that we're currently in
                          // (std::numeric_limits<uint32>::max() by default)
  mutable uint32
      InstanceIndex; // the index within the instance's _Entities vector
  NLMISC::CSmartPtr<CViewer>
      ViewerRecord; // pointer to the CViewer record for viewers (or NULL)
...
public:
  ...
  // - group id is arbitrary - generally use the id of the service responsible
  // for this instance BOMB_IF( the instance already exists )
  ...
  // if an entity already existed with the same id it's removed cleanly before
  // the add operation begins
  void addEntity(TDataSetRow dataSetRow, uint32 aiInstance, sint32 x, sint32 y,
                 TInvisibilityLevel invisibilityLevel, bool isViewer);
  ...
  void setEntityPositionDelayed(TDataSetRow dataSetRow, sint32 x, sint32 y,
                                uint32 ticks);

  // set the coordinates and change the aiInstance assignment for a pre-existing
  // entity
  void teleportEntity(TDataSetRow dataSetRow, uint32 aiInstance, sint32 x,
  ...
  // perform an update cycle, recalculate a sub-set of the universe's viewers'
  // visions
  void update();
  ...
  // clear out the vision for an entity for a certain number of iterations to
  // allow FES to clean vision buffers
  void forceResetVision(TDataSetRow dataSetRow);

private:
  // private implementations of above public routines - validity checks on
  // parameters are skipped as they are assumed to have been carried out in the
  // public interface
  void _addEntity(TDataSetRow dataSetRow, uint32 aiInstance, sint32 x, sint32 y,
                  TInvisibilityLevel invisibilityLevel, bool isViewer);
  void _removeEntity(TDataSetRow dataSetRow);
  void _setEntityPosition(TDataSetRow dataSetRow, sint32 x, sint32 y);
  void _teleportEntity(TDataSetRow dataSetRow, uint32 aiInstance, sint32 x,
                       sint32 y, TInvisibilityLevel invisibilityLevel);

private:
  std::vector<NLMISC::CSmartPtr<CInstance>>
      _Instances; // the instances in the service (vector index corresponds to
                  // AIInstance ids)
  std::vector<SUniverseEntity>
      _Entities; // basic info for all of the entities in the universe

  typedef std::list<SDelayedPlayerPosition> TDelayedPositions;
  TDelayedPositions _DelayedPositions;

  NLMISC::CRefPtr<IVisionDeltaManager>
      _VisionDeltaManager; // a callback object used to treat vision deltas
};
...
public:
  ...
  // add an entity to this instance (either a new entity or a move from another
  // instance) if the entity was attached to another instance on entry to this
  // routine it is detached cleanly
  void addEntity(TDataSetRow dataSetRow, uint32 x, uint32 y,
                 TInvisibilityLevel invisibilityLevel,
                 const SUniverseEntity &entity);

  // remove an entity from this instance (either to add to another instance or
  // to destroy)
  void removeEntity(SUniverseEntity &entity);

  // remove a viewer from their current vision group and create a new vision
  // group for them to use this method is used after viewer visionLevel changes
  ...
  void setEntityInvisibility(uint32 entityIndex,
                             TInvisibilityLevel invisibilityLevel);
...
private:
  ...
  TInstanceEntities
      _Entities; // the positions cache for all of the entities in the instance
  std::vector<NLMISC::CSmartPtr<CVisionGroup>>
      _VisionGroups; // all of the vision records in the instance
};
...
public:
  ...
  // release - called just before instance destruction to allow us to clear out
  // player visions
  ...
  // split the viewers from this group into 2, the second part going into the
  // 'other' CVisionGroup object
  ...
  // we use a distance from the origin to the centre of the vision unit as a
  // sort key for update() operations a few accessors...
...
public:
  ...
  // set the dataSetRow for the viewer - required for generating vision delta
  // packets
  ...
  void updateVision(const TVision &vision, CUniverse *theUniverse,
                    bool firstTime);
  ...
  // utility method to force vision reset at next update - used to re-initialise
  // vision after editing
...
public:
  // index variables manipulated by external classes to assist in rapid
  // management of deletion operations
  mutable uint32 VisionId; // the id of the Instance's vision group to which
                           // you're assigned
  mutable uint32
      VisionIndex; // the index within the vision group's _Viewers vector

private:
  // private methods called by updateVision()
  void _updateVisionBlind(CUniverse *theUniverse, CPlayerVisionDelta &result);
  void _updateVisionNormal(const TVision &vision, CUniverse *theUniverse,
...
private:
  ...
  uint32 _Y;
  TDataSetRow
      _ViewerId; // this is a datasetrow value used to compose vision messages
  uint32 _VisionResetCount; // flag meaning that at next update vision should be
                            // reset
  TInvisibilityLevel _VisionLevel; // value used to determine which entities are
...
struct SDelayedPlayerPosition {
  ...
  // ctor - default
  SDelayedPlayerPosition() { X = Y = 0; }

  // ctor - parameterised
  SDelayedPlayerPosition(NLMISC::TGameCycle gameCycle, TDataSetRow dataSetRow,
                         sint32 x, sint32 y) {
    X = x;
    Y = y;

...
public:
  ...
  void update() {
    ...
    try {
      ...
      nlwarning(
          "CLoginServiceWeb : Exception launch in callback server update");
    }
  }
  ...
  void loginResult(NLNET::TSockId dest, uint32 userId,
                   const std::string &cookie, uint32 resultCode,
  ...
  void logoutResult(NLNET::TSockId dest, uint32 errorCode,
  ...
  static void cb_login(NLNET::CMessage &message, NLNET::TSockId from,
                       NLNET::CCallbackNetBase &netbase) {
    H_AUTO(login_on_login);
#ifdef NL_DEBUG
    nldebug("CLoginServiceWeb::cb_login received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackServerAdaptor *adaptor =
        static_cast<ICallbackServerAdaptor *>(netbase.getUserData());

    CLoginServiceWebItf *callback =
        (CLoginServiceWebItf *)adaptor->getContainerClass();
  ...
  static void cb_logout(NLNET::CMessage &message, NLNET::TSockId from,
                        NLNET::CCallbackNetBase &netbase) {
    H_AUTO(logout_on_logout);
#ifdef NL_DEBUG
    nldebug("CLoginServiceWeb::cb_logout received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackServerAdaptor *adaptor =
        static_cast<ICallbackServerAdaptor *>(netbase.getUserData());
  ...
  virtual void on_login(NLNET::TSockId from, uint32 userId,
                        const std::string &ipAddress, uint32 domainId) = 0;
...
protected:
  ...
  static void _cbDisconnection(NLNET::TSockId from, void *arg) {
    CLoginServiceWebClientItf *_this =
        reinterpret_cast<CLoginServiceWebClientItf *>(arg);
...
public:
  ...
  static const std::string &getMessageName(const std::string &methodName) {
    ...
    if (!initialized) {
      messageNames.insert(
          std::make_pair(std::string("on_loginResult"), std::string("LGR")));
      messageNames.insert(
          std::make_pair(std::string("on_logoutResult"), std::string("LGOR")));
  ...
  virtual void update() {
    ...
    try {
      ...
      nlwarning(
          "CLoginServiceWeb : Exception launch in callback client update");
    }
  }
  ...
  static void cb_loginResult(NLNET::CMessage &message, NLNET::TSockId from,
                             NLNET::CCallbackNetBase &netbase) {
#ifdef NL_DEBUG
    nldebug("CLoginServiceWebClient::cb_loginResult received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackClientAdaptor *adaptor =
        static_cast<ICallbackClientAdaptor *>(netbase.getUserData());

    CLoginServiceWebClientItf *callback =
        (CLoginServiceWebClientItf *)adaptor->getContainerClass();
  ...
  static void cb_logoutResult(NLNET::CMessage &message, NLNET::TSockId from,
                              NLNET::CCallbackNetBase &netbase) {
#ifdef NL_DEBUG
    nldebug("CLoginServiceWebClient::cb_logoutResult received from class '%s'",
            typeid(netbase).name());
#endif
    ICallbackClientAdaptor *adaptor =
        static_cast<ICallbackClientAdaptor *>(netbase.getUserData());
...
#ifdef NL_DEBUG
    nldebug(
        "CLoginServiceWebClient::cb_logoutResult : calling on_logoutResult");
#endif
  ...
  //       2 - user already online, must relog
  //  errorString contain a stringified description in case of error
  virtual void on_loginResult(NLNET::TSockId from, uint32 userId,
                              const std::string &cookie, uint32 resultCode,
  ...
  //                 1 - invalid user
  //                 2 - user already offline
  virtual void on_logoutResult(NLNET::TSockId from, uint32 errorCode,

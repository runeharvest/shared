...
TLogNoContext_Ring::TLogNoContext_Ring() { RingDesc.pushNoContext(); }

TLogNoContext_Ring::~TLogNoContext_Ring() { RingDesc.popNoContext(); }

void _log_Ring_EnterSession(const NLMISC::CEntityId &charId, uint32 sessionId,
...
void _log_Ring_LeaveSession(const NLMISC::CEntityId &charId, uint32 sessionId,
                            const char *_filename_, uint _lineNo_) {
  static LGS::TLogInfo logInfo;
  static bool init = false;
  if (!init) {

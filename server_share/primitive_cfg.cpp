...
void CPrimitiveCfg::addPrimitive(std::vector<std::string> &vectorlist,
...
std::string CPrimitiveCfg::getContinentNameOf(const std::string &fileName) {
  ...
  std::map<std::string, std::vector<std::string>>::iterator first(
      _ContinentFiles.begin()),
      last(_ContinentFiles.end());
  for (; first != last; ++first)
    if (find(first->second.begin(), first->second.end(), name) !=
        first->second.end())
      return first->first;
...
// All leading and trailing ' ' and '\t' round keyword and tail characters are
// stripped If no keyword is found routine retuns false (keyword and tail retain
// previous content)
inline bool stringToWordAndTail(const std::string &input, std::string &word,
                                std::string &tail) {
  uint i = 0, j;

  // skip white space
  while (i < input.size() && isWhiteSpace(input[i]))
    ++i; // i points to start of word

  // look for the end of the word
  for (j = i; j < input.size() && !isWhiteSpace(input[j]);)
    ++j; // j points to next character after word
  ...
  for (i = (uint)input.size(); i > j && isWhiteSpace(input[i - 1]);)
    --i; // i points to character after end of tail text
...
void CPrimitiveCfg::readPrimitiveCfg(bool forceReload) {
  if (!forceReload && !_MapNames.empty())
    return;
  ...
  try {
    ...
    for (uint32 i = 0; i < var.size(); ++i) {
      ...
      // get the next string from the config file entry and make sure its not
      // empty
      ...
      //			// skip opening white space	and make sure
      //there's not just white space 			for (j=0;j<s.size() && (s[j]==' ' ||
      //s[j]=='\t');++j); 			if (j>=s.size()) 				continue;
      //			// separate the first word (we need it to know
      //what to do next) 			for (k=j;k<s.size() && s[k]!=' ' && s[k]!='\t';++k);
      //			std::string keyword=s.substr(j,k-j);
      //
      //			// separate out the tail, pruning trailing
      //blanks
      //			// ignore leading blanks
      //			for (j=k;j<s.size() && (s[j]==' ' ||
      //s[j]=='\t');++j);
      //			// ignore trailing spaces
      //			for (k=s.size()-1;s[k]==' '||s[k]=='\t';--k);
      //			// if we found some non-blank text take a copy
      //in a 'tail' variable 			std::string tail; 			if (k>=j) tail=s.substr(j,k-j+1);
      ...
      if (nlstricmp(keyword, "MAPEND") == 0) {
        ...
        //				if (j<=k)
        {
          ...
          //					else
          //						nlwarning("Not
          //Continents specified for %s",filename.c_str());
        }
        ...
        //				if (j<=k)
        {
          ...
          if (_Maps.find(includeName) == _Maps.end()) {
            nlwarning("PrimitiveCfg: Include %s failed, not defined.",
                      includeName.c_str());
          } else {
            ...
            for (uint32 primInd = 0; primInd < vectorlist.size(); primInd++) {
              ...
              if (_CurrentContinentFiles)
                ...
                nlwarning("Not Continents specified for %s",
                          includeName.c_str());
            }
          }
        }
    ...
    nlwarning("Error reading or parsing the primitive configuration file '%s'",
              filename.c_str());
  }
}

...
/**
 * A gameplay module. Implements the actions done by an element of a system (
 * e.g. : a player ) on an element of another system ( e.g. : a guild ) A module
 * is responsible of cleaning himself. \author Nicolas Brigand \author Nevrax
 * France \date 2004
 */
class IModule : public NLMISC::CRefCount {
public:
  /// ctor. Is in charge of registering the module on the parent and referenced
  /// core
  IModule(CModuleParent *parent, IModuleCore *referencedCore);
  virtual ~IModule() {}
  /// callback called when parent is deleted. It calls the
  /// onParentDestructionHandler virtual method
  ...
  template <class T> void getProxy(T &proxy) const {
    MODULE_AST(_Parent);

...
IModuleCore::~IModuleCore() {
  MODULE_INFO("Module core destructor : inform all referencing module and "
              "delete the contained module parent");
...
//----------------------------------------------------------------------------
IModuleCore::IModuleCore() { _ModulesCont = new CModuleParent(this); }
...
void IModuleCore::addReferencingModule(IModule *module) {
  ...
  std::vector<NLMISC::CRefPtr<IModule>>::iterator it(std::find(
      _ModulesReferencingMe.begin(), _ModulesReferencingMe.end(), ref));

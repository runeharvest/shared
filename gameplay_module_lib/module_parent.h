...
/**
 * A module parent is a class encapsulating the modules contained in a module
 * core It mainly provides accessors to the modules \author Nicolas Brigand
...
public:
  ...
  CModuleParent(::IModuleCore *owner) : _Owner(owner) { MODULE_AST(owner); };
  ...
  template <class ModuleClass> bool getModule(ModuleClass *&module) const {
    module = NULL;
    const uint size = (uint)_Modules.size();
    for (uint i = 0; i < size; i++) {
      ModuleClass *moduleChecked =
          dynamic_cast<ModuleClass *>((::IModule *)_Modules[i]);
      if (moduleChecked) {
        module = moduleChecked;
        // additional checks to ensure module uniqueness
#ifdef RY_MODULE_DEBUG
        for (uint j = i + 1; j < size; j++) {
          ModuleClass *moduleTest =
              dynamic_cast<ModuleClass *>((::IModule *)_Modules[j]);
          if (moduleTest)
            nlerror(
                "you required a unique module but there is more than 1!!!!");
        }
#endif
        return module != NULL;
      }
  ...
  /// add a new module to this container. Called by a module to register itself
  /// in the parent

...
CModuleParent::~CModuleParent() {
  MODULE_INFO("Module parent destruction : calling the Ondestruction handler "
              "for all modules");
  // as module could be removed from thi parent in IModule::onParentDestruction,
  // we have do remove modules with this strange way
...
void CModuleParent::removeChildModule(IModule *module) {
  ...
  std::vector<NLMISC::CRefPtr<IModule>>::iterator it(
      std::find(_Modules.begin(), _Modules.end(), ref));
  if (it == _Modules.end()) {

...
#define SAMPLE_OGG                                                             \
  "D:/source/kaetemi/toverhex/src/samples/music_stream/data/"                  \
  "aeon_1_10_mystic_river.ogg"
...
static void initSample() {
  ...
  // init with 128 tracks, EAX enabled, no ADPCM, and automatic sample bank
  // loading
  s_AudioMixer->init(8, true, false, NULL, true,
                     (UAudioMixer::TDriver)(selection - '0'));
  ...
  USource *source =
      s_AudioMixer->createSource(CStringMapper::map("default_stream"));
  nlassert(source);
  ...
  s_StreamSource->setFormat(s_AudioDecoder->getChannels(),
                            s_AudioDecoder->getBitsPerSample(),
                            (uint32)s_AudioDecoder->getSamplesPerSec());

...
CObjectDyn::CObjectDyn(double width, double depth, double height,
                       double orientation, const CVectorD &pos,
                       const CVectorD &speed, bool obstacle,
                       UMoveContainer &container, UScene &scene,
                       UMovePrimitive::TReaction reaction,
                       NLPACS::UMovePrimitive::TTrigger trigger,
                       uint8 worldImage, uint8 nbImage,
                       uint8 insertWorldImage) {
  // Create a box instance
  _Instance = scene.createInstance(CPath::lookup("rectangle.shape"));
  if (_Instance.empty()) {
    nlerror("Failed to load shape: %s",
            CPath::lookup("rectangle.shape").c_str());
  }
  ...
  if (_Instance.getNumMaterials()) {
    ...
    for (i = 0; i < _Instance.getNumMaterials(); i++) {
      ...
      if (trigger != UMovePrimitive::NotATrigger) {
        ...
        // material.setBlendFunc(UInstanceMaterial::srcalpha,
        // UInstanceMaterial::invsrcalpha);
...
CObjectDyn::CObjectDyn(double diameter, double height, const CVectorD &pos,
                       const CVectorD &speed, bool obstacle,
                       UMoveContainer &container, UScene &scene,
                       UMovePrimitive::TReaction reaction,
                       NLPACS::UMovePrimitive::TTrigger trigger,
  ...
  if (_Instance.empty()) {
    nlerror("Failed to load shape: %s",
            CPath::lookup("cylinder.shape").c_str());
  }
...
void CObjectDyn::tryMove(double deltaTime, UMoveContainer &container,
                         uint8 worldImage) {
  // New speed
  if ((_MovePrimitive->getSpeed(worldImage).norm() > 0.f) ||
      _Speed.norm() > 0.f) {
    // Brake
...
void CObjectDyn::setGlobalPos(UGlobalPosition &gpos, CVectorD &pos,
                              uint8 worldimage) {
...
void CObjectDyn::setSpeed(const CVectorD &speed) { _Speed = speed; }

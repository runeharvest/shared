...
// Get a random size
double randomSize() {
  return (SIZE_PRIMITIVE_MAX - SIZE_PRIMITIVE_MIN) *
...
CVectorD randomSpeed() {
  return CVectorD(
      2 * (double)INIT_SPEED * ((double)rand() / (double)RAND_MAX) - INIT_SPEED,
      2 * (double)INIT_SPEED * ((double)rand() / (double)RAND_MAX) - INIT_SPEED,
      0);
}
...
// Get a random pos
CVectorD randomPos() {
  return CVectorD(((double)ARENA_SIZE - 2 * BORDER_SIZE - SIZE_PRIMITIVE_MAX) *
                          ((double)rand() / (double)RAND_MAX) -
                      ARENA_SIZE / 2.0 + BORDER_SIZE + (SIZE_PRIMITIVE_MAX) / 2,
                  ((double)ARENA_SIZE - 2 * BORDER_SIZE - SIZE_PRIMITIVE_MAX) *
                          ((double)rand() / (double)RAND_MAX) -
                      ARENA_SIZE / 2.0 + BORDER_SIZE + (SIZE_PRIMITIVE_MAX) / 2,
                  0);
}
...
#endif // NL_OS_WINDOWS
{
  ...
  try {
    ...
    // Create a container
    // This container has 31 world images. The first is a static world image,
    // the others are dynamic. Each frame, the sample eval the collision in the
    // next dynamic world image. This is to show the multiple world images
    // functionality.
    UMoveContainer *container = UMoveContainer::createMoveContainer(
        -ARENA_SIZE / 2.0, -ARENA_SIZE / 2.0, ARENA_SIZE / 2.0,
        ARENA_SIZE / 2.0, NUM_CELL, NUM_CELL,
        (double)SIZE_PRIMITIVE_MAX * sqrt(2.0), MAX_WORLD_IMAGE);
    ...
    for (int cell = 0; cell < num_cell; cell++) {
      ...
      arrayArena.push_back(new CObjectDyn(
          SIZE_PRIMITIVE_MAX, SIZE_PRIMITIVE_MAX, 10 * SIZE_PRIMITIVE_MAX, 0,
          CVectorD(SIZE_PRIMITIVE_MAX * cell -
                       (ARENA_SIZE - SIZE_PRIMITIVE_MAX) / 2,
                   (ARENA_SIZE - SIZE_PRIMITIVE_MAX) / 2, 0),
          CVectorD(0, 0, 0), true, *container, *pScene,
          UMovePrimitive::DoNothing, UMovePrimitive::NotATrigger, 0, 1, 0));
      arrayArena.push_back(new CObjectDyn(
          SIZE_PRIMITIVE_MAX, SIZE_PRIMITIVE_MAX, 10 * SIZE_PRIMITIVE_MAX, 0,
          CVectorD(SIZE_PRIMITIVE_MAX * cell -
                       (ARENA_SIZE - SIZE_PRIMITIVE_MAX) / 2,
      ...
      arrayArena.push_back(new CObjectDyn(
          SIZE_PRIMITIVE_MAX, SIZE_PRIMITIVE_MAX, 10 * SIZE_PRIMITIVE_MAX, 0,
          CVectorD((ARENA_SIZE - SIZE_PRIMITIVE_MAX) / 2,
                   SIZE_PRIMITIVE_MAX * cell -
                       (ARENA_SIZE - SIZE_PRIMITIVE_MAX) / 2,
                   0),
          CVectorD(0, 0, 0), true, *container, *pScene,
          UMovePrimitive::DoNothing, UMovePrimitive::NotATrigger, 0, 1, 0));
    ...
    // Create one object
    arrayObj.push_back(new CObjectDyn(randomSize(), randomSize(), randomPos(),
                                      CVectorD(0, 0, 0), true, *container,
                                      *pScene, UMovePrimitive::Reflexion,
                                      UMovePrimitive::NotATrigger, 1, 30, 1));
    ...
    // Main loop
    while (pDriver->isActive() &&
           (!pDriver->AsyncListener.isKeyPushed(KeyESCAPE))) {
      ...
      if (pDriver->AsyncListener.isKeyDown(KeyINSERT)) {
        if (pDriver->AsyncListener.isKeyDown(KeyMENU)) {
          ...
          if (rand() & 1)
            arrayObj.push_back(new CObjectDyn(
                randomSize(), randomSize(), randomSize(), randomRot(),
                randomPos(), CVectorD(0, 0, 0), false, *container, *pScene,
                UMovePrimitive::DoNothing,
                (UMovePrimitive::TTrigger)(UMovePrimitive::EnterTrigger |
                                           UMovePrimitive::ExitTrigger |
                                           UMovePrimitive::OverlapTrigger),
                1, 30, worldImage));
          else
            arrayObj.push_back(new CObjectDyn(
                randomSize(), randomSize(), randomPos(), CVectorD(0, 0, 0),
                false, *container, *pScene, UMovePrimitive::DoNothing,
                (UMovePrimitive::TTrigger)(UMovePrimitive::EnterTrigger |
                                           UMovePrimitive::ExitTrigger |
                                           UMovePrimitive::OverlapTrigger),
                1, 30, worldImage));
          ...
          if (rand() & 1)
            arrayObj.push_back(new CObjectDyn(
                randomSize(), randomSize(), randomSize(), randomRot(),
                randomPos(), CVectorD(0, 0, 0), true, *container, *pScene,
                fixed ? UMovePrimitive::DoNothing : UMovePrimitive::Reflexion,
                UMovePrimitive::NotATrigger, 1, 30, worldImage));
          else
            arrayObj.push_back(new CObjectDyn(
                randomSize(), randomSize(), randomPos(), CVectorD(0, 0, 0),
                true, *container, *pScene,
                fixed ? UMovePrimitive::DoNothing : UMovePrimitive::Reflexion,
                UMovePrimitive::NotATrigger, 1, 30, worldImage));
        }
      }
      ...
      if (!arrayObj.empty()) {
        ...
        pCam.lookAt(plistener->getViewMatrix().getPos(),
                    arrayObj[selected]->getPos());
      }
      ...
      pDriver->drawQuad(0, 0.05f, 1.f * (float)inside / 50.f, 0.10f,
                        CRGBA::Green);
...
#ifdef NL_OS_WINDOWS
    ::MessageBox(NULL, e.what(), "Test collision move",

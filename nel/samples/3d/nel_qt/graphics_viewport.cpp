...
CGraphicsViewport::CGraphicsViewport(QWidget *parent)
    : QWidget(parent), m_GraphicsConfig(NULL), m_Driver(NULL),
      m_TextContext(NULL) {}
...
void CGraphicsViewport::init(CGraphicsConfig *graphicsConfig) {
  ...
  if (driver == "Direct3D")
    m_Direct3D = true; // m_Driver = Direct3D;
  else if (driver == "OpenGL")
    m_Direct3D = false; // m_Driver = OpenGL;
  else {
  ...
  connect(m_GraphicsConfig, SIGNAL(onBackgroundColor(NLMISC::CRGBA)), this,
          SLOT(cfcbBackgroundColor(NLMISC::CRGBA)));
  ...
  m_TextContext = m_Driver->createTextContext(
      CPath::lookup(m_GraphicsConfig->getFontName()));
  nlassert(m_TextContext);
  connect(m_GraphicsConfig, SIGNAL(onFontShadow(bool)), this,
          SLOT(cfcbFontShadow(bool)));
...
void CGraphicsViewport::release() {
  ...
  nlassert(m_TextContext);
  disconnect(m_GraphicsConfig, SIGNAL(onFontShadow(bool)), this,
             SLOT(cfcbFontShadow(bool)));
  ...
  // release driver
  nlassert(m_Driver);
  disconnect(m_GraphicsConfig, SIGNAL(onBackgroundColor(NLMISC::CRGBA)), this,
             SLOT(cfcbBackgroundColor(NLMISC::CRGBA)));
  m_Driver->release();
  delete m_Driver;
  m_Driver = NULL;
...
void CGraphicsViewport::updateInput() { m_Driver->EventServer.pump(); }
...
void CGraphicsViewport::saveScreenshot() {
  saveScreenshot(m_GraphicsConfig->getScreenshotName(),
                 m_GraphicsConfig->getScreenshotJPG(),
                 m_GraphicsConfig->getScreenshotPNG(),
                 m_GraphicsConfig->getScreenshotTGA());
}

void CGraphicsViewport::saveScreenshot(const string &name, bool jpg, bool png,
                                       bool tga) {
  // H_AUTO2
  ...
  string filename = std::string("./") + m_GraphicsConfig->getScreenshotPath() +
                    std::string("/") + name;
...
void CGraphicsViewport::resizeEvent(QResizeEvent *resizeEvent) {
  ...
  if (m_Driver && !m_Direct3D) {
    m_Driver->setMode(UDriver::CMode(resizeEvent->size().width(),
                                     resizeEvent->size().height(), 32));
  }

  // The OpenGL driver does not resize automatically.
  // The Direct3D driver breaks the window mode to include window borders when
  // calling setMode windowed.

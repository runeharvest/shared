...
CUndoRedoBinderButton::CUndoRedoBinderButton(QAbstractButton *abstractButton,
                                             QUndoStack *undoStack)
    : QObject(abstractButton), m_AbstractButton(abstractButton),
      m_UndoStack(undoStack) {
  nlassert(m_AbstractButton);
  bool c;
  m_Checked = m_AbstractButton->isChecked();
  c = connect(m_AbstractButton, SIGNAL(toggled(bool)), this,
              SLOT(abstractButtonToggled(bool)));
...
void CUndoRedoBinderButton::abstractButtonToggled(bool checked) {
  if (m_Enabled) {
    if (checked != m_Checked) {
      ...
      m_UndoStack->push(
          new CUndoRedoCommandButton(this, m_AbstractButton, undo));
    }
  } else {
    m_Checked = checked;
  }
...
CUndoRedoCommandButton::CUndoRedoCommandButton(CUndoRedoBinderButton *binder,
                                               QAbstractButton *abstractButton,
                                               bool undo /*, bool redo*/)
    : QUndoCommand(), m_Binder(binder), m_AbstractButton(abstractButton),
...
CUndoRedoBinderLineEdit::CUndoRedoBinderLineEdit(QLineEdit *lineEdit,
                                                 QUndoStack *undoStack)
  ...
  c = connect(m_LineEdit, SIGNAL(textChanged(const QString &)), this,
...
void CUndoRedoBinderLineEdit::lineEditTextChanged(const QString &text) {
  ...
  if (m_Enabled) {
    if (text != m_LastValue) {
      ...
      m_LastValue = redo;
      m_UndoStack->push(
          new CUndoRedoCommandLineEdit(this, m_LineEdit, undo, redo, m_Id));
    }
  } else {
    m_LastValue = text;
  }
...
CUndoRedoCommandLineEdit::CUndoRedoCommandLineEdit(
    CUndoRedoBinderLineEdit *binder, QLineEdit *lineEdit, const QString &undo,
    const QString &redo, int id)
    : QUndoCommand(), m_Binder(binder), m_LineEdit(lineEdit), m_Undo(undo),
      m_Redo(redo), m_Id(id) {
...
CUndoRedoBinderComboBox::CUndoRedoBinderComboBox(QComboBox *comboBox,
                                                 QUndoStack *undoStack)
    : QObject(comboBox), m_ComboBox(comboBox), m_UndoStack(undoStack) {
  nlassert(m_ComboBox);
  bool c;
  m_LastValue = m_ComboBox->currentIndex();
  c = connect(m_ComboBox, SIGNAL(currentIndexChanged(int)), this,
...
void CUndoRedoBinderComboBox::comboBoxCurrentIndexChanged(int index) {
  if (m_Enabled) {
    if (index != m_LastValue) {
      ...
      m_LastValue = redo;
      m_UndoStack->push(
          new CUndoRedoCommandComboBox(this, m_ComboBox, undo, redo));
    }
  } else {
    m_LastValue = index;
  }
...
CUndoRedoCommandComboBox::CUndoRedoCommandComboBox(
    CUndoRedoBinderComboBox *binder, QComboBox *comboBox, int undo, int redo)
    : QUndoCommand(), m_Binder(binder), m_ComboBox(comboBox), m_Undo(undo),
      m_Redo(redo) {
...
CUndoRedoBinderColorPicker::CUndoRedoBinderColorPicker(
    QtColorPicker *colorPicker, QUndoStack *undoStack)
  ...
  c = connect(m_ColorPicker, SIGNAL(colorChanged(const QColor &)), this,
              SLOT(colorPickerColorChanged(const QColor &)));
...
void CUndoRedoBinderColorPicker::colorPickerColorChanged(const QColor &col) {
  if (m_Enabled) {
    if (col != m_LastValue) {
      ...
      m_LastValue = redo;
      m_UndoStack->push(
          new CUndoRedoCommandColorPicker(this, m_ColorPicker, undo, redo));
    }
  } else {
    m_LastValue = col;
  }
...
CUndoRedoCommandColorPicker::CUndoRedoCommandColorPicker(
    CUndoRedoBinderColorPicker *binder, QtColorPicker *colorPicker,
    const QColor &undo, const QColor &redo)
    : QUndoCommand(), m_Binder(binder), m_ColorPicker(colorPicker),
      m_Undo(undo), m_Redo(redo) {

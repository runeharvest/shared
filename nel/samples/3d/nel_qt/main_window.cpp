...
CMainWindow::CMainWindow(const QMap<QString, QSize> &customSizeHints,
                         QWidget *parent, Qt::WindowFlags flags)
    : QMainWindow(parent, flags), m_UndoStack(NULL),
      m_IsGraphicsInitialized(false), m_IsGraphicsEnabled(false),
      m_IsSoundInitialized(false), m_IsSoundEnabled(false),
      m_GraphicsViewport(NULL), m_CommandLog(NULL), m_CommandLogDock(NULL),
      m_GraphicsConfig(NULL), m_GraphicsConfigScroll(NULL),
      m_GraphicsConfigDock(NULL), m_FileMenu(NULL), m_EditMenu(NULL),
      m_ViewportMenu(NULL), m_WidgetsMenu(NULL), m_HelpMenu(NULL),
      m_FileToolBar(NULL), m_EditToolBar(NULL), m_AboutAct(NULL),
      m_QuitAct(NULL), m_PrintDebugAct(NULL), m_UndoAct(NULL), m_RedoAct(NULL),
  ...
  m_Configuration.setAndCallback(
      "QtStyle", CConfigCallback(this, &CMainWindow::cfcbQtStyle));
  m_Configuration.setAndCallback(
      "QtPalette", CConfigCallback(this, &CMainWindow::cfcbQtPalette));

  m_Internationalization.init(&m_Configuration);
  m_Internationalization.enableCallback(
      CEmptyCallback(this, &CMainWindow::incbLanguageCode));
  ...
  connect(m_GraphicsConfig, SIGNAL(applyGraphicsConfig()), this,
          SLOT(applyGraphicsConfig()));
  m_Configuration.setAndCallback(
      "SoundEnabled", CConfigCallback(this, &CMainWindow::cfcbSoundEnabled));
}
...
void CMainWindow::createDockWindows() {
  ...
  // CommandLog (Console)
  {
    ...
    m_CommandLogDock->setAllowedAreas(Qt::TopDockWidgetArea |
  ...
  // GraphicsConfig (Graphics Configuration)
  {
    ...
    m_GraphicsConfigDock->setAllowedAreas(Qt::LeftDockWidgetArea |
                                          Qt::RightDockWidgetArea);
    m_GraphicsConfigScroll = new QScrollArea();
    m_GraphicsConfigScroll->setHorizontalScrollBarPolicy(
        Qt::ScrollBarAlwaysOff);
    m_GraphicsConfigScroll->setWidgetResizable(true);
    m_GraphicsConfig =
        new CGraphicsConfig(m_GraphicsConfigDock, &m_Configuration,
                            &m_Internationalization, m_UndoStack);
...
void CMainWindow::recalculateMinimumWidth() {
  if (m_GraphicsConfigScroll)
    m_GraphicsConfigScroll->setMinimumWidth(
        m_GraphicsConfig->minimumSizeHint().width() +
        m_GraphicsConfigScroll->minimumSizeHint().width());
}
...
void CMainWindow::printDebug() { nldebug("This is a debug message."); }

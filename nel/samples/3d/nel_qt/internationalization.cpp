...
void CInternationalization::init(CConfiguration *configuration) {
  ...
  m_Configuration->setAndCallback(
      "LanguageCode",
      CConfigCallback(this, &CInternationalization::cfcbLanguageCode));
}
...
void CInternationalization::saveLanguageCode(const std::string &language) {
  CConfigFile::CVar &var =
      m_Configuration->getConfigFile().getVar("LanguageCode");
...
void CInternationalization::disableCallback(CEmptyCallback incb) {
  std::vector<CEmptyCallback>::iterator it(m_Callbacks.begin()),
      end(m_Callbacks.end());
  for (; it != end; ++it)
    if (*it == incb) {

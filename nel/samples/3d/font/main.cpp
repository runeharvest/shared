...
#endif
{
  ...
  fontManager.computeString("cs Rotation", tc.getFontGenerator(),
                            CRGBA(255, 255, 255), 70, false, false,
                            CNELU::Driver, csRotation);

  CComputedString cs3d;
  fontManager.computeString("cs 3d", tc.getFontGenerator(), CRGBA(255, 127, 0),
                            75, false, false, CNELU::Driver, cs3d);

  // generate an Unicode string.
  ucstring ucs("cs Unicode String");

  CComputedString csUnicode;
  fontManager.computeString(ucs, tc.getFontGenerator(), CRGBA(32, 64, 127), 75,
                            false, false, CNELU::Driver, csUnicode);
  ...
  do {
    ...
    // (between (0.0 (left) and 1.0 (right)). third one is the y position
    // (between 0.0 (bottom) and 1.0 (top)).
    tc.setColor(CRGBA(255, 255, 255));
    tc.setFontSize(40);
    tc.setHotSpot(CComputedString::BottomLeft);
    tc.printAt(0.5f, 0.7f, string("printAt"));

    // the fourth param is the position of the hotspot, the text will be
    // displayed at x,y depending on the hotspot
    ...
    angle += 0.01f;
    csRotation.render2D(*CNELU::Driver, 0.2f, 0.7f,
                        CComputedString::MiddleMiddle, 1, 1, angle);

    csUnicode.render2D(*CNELU::Driver, 1.0f, 0.15f,
                       CComputedString::MiddleRight);

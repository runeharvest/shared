...
void QNelWidget::initializeGL() {
  ...
  //  Init NELU
  if (!NL3D::CNELU::init(width(), height(), viewport, 32, true, (void *)winId(),
                         false, false)) {
    return;
  }

  //_SceneRoot=
  //(NL3D::CTransform*)NL3D::CNELU::Scene->createModel(NL3D::TransformId);
  NL3D::CTransformShape *sphere =
      NL3D::CNELU::Scene->createInstance("sphere01.shape");

  ...
  try {
    ...
    NELRENDERER_CREATE_PROC createNelRenderer =
        reinterpret_cast<NELRENDERER_CREATE_PROC>(
            driverLib.getSymbolAddress(NELRENDERER_CREATE_PROC_NAME));

    //		CCeguiRendererNelLibrary *nelCeguiDriverLib =
    //dynamic_cast<CCeguiRendererNelLibrary
    //*>(driverLib.getNelLibraryInterface());
    ...
    try {
      ...
      // here we will use a StaticImage as the root, then we can use it to place
      // a background image
      Window *background = WindowManager::getSingleton().createWindow(
      ...
      initDemoEventWiring();
        ...
        CEGUI::Exception) { // catch to prevent exit (errors will be logged).
      ;
    }
...
bool handleSlider(const CEGUI::EventArgs &e) {
  ...
  float val =
      ((Slider *)((const WindowEventArgs &)e).window)->getCurrentValue();

  ((ProgressBar *)WindowManager::getSingleton().getWindow(
       "Demo7/Window2/Progbar1"))
      ->setProgress(val);
  ((ProgressBar *)WindowManager::getSingleton().getWindow(
       "Demo7/Window2/Progbar2"))
      ->setProgress(1.0f - val);
...
bool handleRadio(const CEGUI::EventArgs &e) {
  ...
  CEGUI::uint id = ((RadioButton *)((const WindowEventArgs &)e).window)
                       ->getSelectedButtonInGroup()
                       ->getID();

  if (id == 0) {
    WindowManager::getSingleton()
        .getWindow("Demo7/Window2/Image1")
        ->setProperty("Image", "set:WerewolfGuiImagery image:CeguiLogo");
  } else if (id == 1) {
    WindowManager::getSingleton()
        .getWindow("Demo7/Window2/Image1")
        ->setProperty("Image", "set:WerewolfGuiImagery image:NeLLogo");
  } else {
    WindowManager::getSingleton()
        .getWindow("Demo7/Window2/Image1")
        ->setProperty("Image", "set:WerewolfGuiImagery image:WerewolfLogo");
  }

...
void NeLDriver::init() {
  ...
  m_TextContext =
      m_Driver->createTextContext(NLMISC::CPath::lookup("n019003l.pfb"));
  if (m_TextContext == 0) {
  ...
  m_Scene->getCam().setPerspective(NLMISC::degToRad(90.0f), 1.33f,
                                   1.0f * GScale, 30000.0f * GScale);
...
double NeLDriver::getFps() {
  return m_DeltaTimeSmooth.getSmoothValue()
             ? 1.0 / m_DeltaTimeSmooth.getSmoothValue()
             : 0.0;
}
...
NL3D::UDriver &NeLDriver::getDriver() const { return *m_Driver; }

NL3D::UScene &NeLDriver::getScene() const { return *m_Scene; }

NL3D::UTextContext &NeLDriver::getTextContext() const { return *m_TextContext; }

...
int main(int argc, char **argv) {
  ...
  /* Try to connect to the login service and check the login, password of the
   * client. return an empty string if all go well
   */
  ...
  for (uint i = 0; i < CLoginClient::ShardList.size(); i++) {
    nlinfo("    ShardId %3d: %s (%d online players)",
           CLoginClient::ShardList[i].Id,
           CLoginClient::ShardList[i].Name.toUtf8().c_str(),
           CLoginClient::ShardList[i].NbPlayers);
  }

...
void cbPos(CMessage &msgin, const std::string &serviceName, TServiceId sid) {
  ...
  nlinfo("Received POS, Sending POS to FS (serial %.2fs, send %.2fs)",
         CTime::ticksToSecond(v2 - v1), CTime::ticksToSecond(v3 - v2));
}

void cbAskVision(CMessage &msgin, const std::string &serviceName,
                 TServiceId sid) {
  uint32 Value = '0ACE';
  ...
  */
  ...
  nlinfo("Sent Vision with %d values in %.2fms", NbId,
         CTime::ticksToSecond(v2 - v1) * 1000.0f);
}
...
TUnifiedCallbackItem CallbackArray[] = {
    {"PONG", cbPong}, {"POS", cbPos}, {"ASK_VISION", cbAskVision}};

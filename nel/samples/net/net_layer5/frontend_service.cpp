...
void cbPos(CMessage &msgin, const std::string &serviceName, TServiceId sid) {
  ...
  nlinfo("Received POS from %s (serial: %.2fs)", serviceName.c_str(),
         CTime::ticksToSecond(v2 - v1));
}
...
void cbVision(CMessage &msgin, const std::string &serviceName, TServiceId sid) {
  ...
  nlinfo("%dms of lag, Received Vision with %d values in %.2fms", (uint32)t,
         NbValue, CTime::ticksToSecond(v2 - v1) * 1000.0f);
  //	sendRequestVision();
...
NLNET::TUnifiedCallbackItem CallbackArray[] = {
    {"POS", cbPos}, {"PONG", cbPong}, {"VISION", cbVision}};
 ...
 */
NLNET_SERVICE_MAIN(CFrontEndService, "FS", "frontend_service", 37000,
                   CallbackArray, "", "")

...
struct CBufferizedPacket {
  CBufferizedPacket(CUdpSock *client, const uint8 *packet, uint32 packetSize,
...
void sendUDPNow(CUdpSock *client, const uint8 *packet, uint32 packetSize,
...
void setSimlagValues(sint32 lag, sint8 packetLoss, sint8 packetDuplication,
  ...
  if (packetDisordering != -1)
    PacketDisordering = packetDisordering;
}
...
void sendUDPNow(CUdpSock *client, const uint8 *packet, uint32 packetSize,
  ...
  //	nlinfo ("time %" NL_I64 "u sending now packet %5u", CTime::getLocalTime
  //(), packetNumber);
}

void sendUDP(CUdpSock *client, const uint8 *packet, uint32 packetSize,
             const CInetAddress *addr) {
  nlassert(client != NULL);
  nlassert(packet != NULL);
  nlassert(packetSize > 0);
  ...
  if ((float)rand() / (float)(RAND_MAX) * 100.0f >= PacketLoss) {
    ...
    if (lag > 100) {
      ...
      CBufferizedPacket *bp =
          new CBufferizedPacket(client, packet, packetSize, lag, addr);

      // duplicate the packet
      if ((float)rand() / (float)(RAND_MAX) * 100.0f < PacketDisordering &&
          BufferizedPackets.size() > 0) {

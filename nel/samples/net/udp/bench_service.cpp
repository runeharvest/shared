...
struct CClient {
  CClient(TSockId from, uint32 session, const string &cn)
      : From(from), Session(session), NextPingNumber(0), LastPongReceived(0),
        ConnectionName(cn), BlockNumber(0), FullMeanPongTime(0), FullNbPong(0),
        NbPing(0), NbPong(0), MeanPongTime(0), NbDuplicated(0),
        FirstWrite(true) {
  ...
  uint32 Session; // used to find the link between UDP and TCP connection at
                  // startup

  vector<pair<uint8, uint16>>
      PongReceived; // contains the number of pong receive for each message
                    // number and the time

  uint32 NextPingNumber, LastPongReceived;
  string ConnectionName;

  // this number is increase each time we filled the PongReceived array, the
  // goal is to avoid a very old packet to use as a new one
  uint32 BlockNumber;
  ...
  void updatePong(sint64 pingTime, sint64 pongTime, uint32 pongNumber,
                  uint32 blockNumber);
  void updateStat();
  void updateFullStat();
};
...
struct TInetAddressHash {
  ...
  inline bool operator()(const NLNET::CInetAddress &x1,
                         const NLNET::CInetAddress &x2) const {
    return x1 == x2;
  }
...
TClientMap
    ClientMap; // contains a quick access to the client using the udp address
...
void cbDisconnect(TSockId from, void *arg) {
  for (list<CClient>::iterator it = Clients.begin(); it != Clients.end();
       it++) {
    if ((*it).From == from) {
      // clear struct
      (*it).updateFullStat();
...
void CClient::updatePong(sint64 pingTime, sint64 pongTime, uint32 pongNumber,
                         uint32 blockNumber) {
  ...
  string fn =
      StatPathName + ConnectionName + "_" + ha + "_" + getDate() + ".pong";
  ...
  if (fp == NULL) {
    ...
    fprintf(fp, "%" NL_I64 "d\t%" NL_I64 "d\t%" NL_I64 "d\t%d\n", pongTime,
            pingTime, (pongTime - pingTime), pongNumber);
    fclose(fp);
  }
}
...
void CClient::updateFullStat() {
...

  {
    ...
    string fn =
        StatPathName + ConnectionName + "_" + ha + "_" + getDate() + ".stat";
    ...
    if (LastPongReceived > 0)
      line +=
          "(" + toString((float)NbLost / LastPongReceived * 100.0f) + "pc) ";
    line += "NbDuplicated " + toString(NbDup) + " ";
    if (LastPongReceived > 0)
      line += "(" + toString((float)NbDup / LastPongReceived * 100.0f) + "pc) ";
...

  {
    ...
    string fn =
        StatPathName + ConnectionName + "_" + ha + "_" + getDate() + ".ping";
...
void updateStat() {
  ...
  for (TClientMap::iterator it = ClientMap.begin(); it != ClientMap.end();
       it++) {
...
void removeClientByAddr(TClientMap::iterator iclient) {
  ...
  for (list<CClient>::iterator it = Clients.begin(); it != Clients.end();
    ...
    if ((*it).Address == (*iclient).first) {
      (*it).updateFullStat();
      nlinfo("Removing client %s",
             GETCLIENTA(iclient)->Address.asString().c_str());
      Clients.erase(it);
      break;
    }
  }
...
void handleReceivedPong(CClient *client, sint64 pongTime) {
  ...
  memcpy(msgin.bufferToFill(currentsize), CurrentInMsg->userDataR(),
         currentsize);
  ...
  if (mode == 0) {
    ...
    if (client == NULL) {
      ...
      for (it = Clients.begin(); it != Clients.end(); it++) {
        if ((*it).Session == session) {
          ...
          nlinfo("TCP-UDP linked TCP is %s, UDP is %s",
                 client->From->asString().c_str(),
                 client->Address.asString().c_str());
    ...
    //		nlinfo ("receive a pong from %s pongnb %d %" NL_I64 "d",
    //CurrentInMsg->AddrFrom.asString().c_str(), pongNumber, pongTime -
    //pingTime);
...
void sendPing() {
  ...
  for (TClientMap::iterator it = ClientMap.begin(); it != ClientMap.end();
    ...
    try {
      ...
      ReceiveTask->DataSock->sendTo(msgout.buffer(), size,
                                    GETCLIENTA(it)->Address);
    } catch (const Exception &e) {
      nlwarning("Can't send UDP packet to '%s' (%s)",
                GETCLIENTA(it)->Address.asString().c_str(), e.what());
    }
...
public:
  void init() {
    ...
    CallbackServer->addCallbackArray(
...
NLNET_SERVICE_MAIN(CBenchService, "BS", "bench_service", 45459,
                   EmptyCallbackArray, UDP_DIR, "")

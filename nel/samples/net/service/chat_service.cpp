...
//			nldebug ("debg flood %10d %s %d %d", val++,
bytesToHumanReadable(CSystemInfo::getAllocatedSystemMemory ()).c_str(),
UserSpeedLoop, NetSpeedLoop);
//			nlinfo ("info flood %10d %s %d %d", val++,
bytesToHumanReadable(CSystemInfo::getAllocatedSystemMemory ()).c_str(),
UserSpeedLoop, NetSpeedLoop);
//			nlwarning ("warn flood %10d %s %d %d", val++,
bytesToHumanReadable(CSystemInfo::getAllocatedSystemMemory ()).c_str(),
UserSpeedLoop, NetSpeedLoop);
      }
    */
    /////////
    //		fprintf (fp, "%d;%d;%d\n", CMemUtils::getAllocatedSystemMemory
    //(), UserSpeedLoop, NetSpeedLoop); 		fflush (fp);
    return true;
  }
...
static void cbChat(CMessage &msgin, const std::string &serviceName,
                   TServiceId sid) {
...
// this array contains all callback functions. it associates the callbackname
// (messagename), with a callback
TUnifiedCallbackItem CallbackArray[] = {{"CHAT", cbChat}};

// this macro is the "main". the first param is the class name inherited from
// IService. the second one is the name of the service used to register and find
// the service using the naming service. the third one is the port where the
// listen socket will be created. If you put 0, the system automatically finds a
// port.

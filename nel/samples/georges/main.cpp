...
struct TSampleConfig {
  /// Load a form, if necessary, and store its data in the members of this
  /// class.
  ...
  void readForm(const NLMISC::CSmartPtr<NLGEORGES::UForm> &form) {
    ...
    // The system has already loaded the form and parsed it using a loader. So
    // get the root node:
  ...
  /// This is used to make sure that changes in form/loader versions are
  /// correctly handled. This must be > 0.
...
int main(void) {
  ...
  try {
    ...
    if (!sampleConfigFile.empty()) {
      ...
      // this will hold the form we load - it must be contained in a smart
      // pointer.
      ...
      nlinfo("Retrieved Position Data (x,y,z): %d, %d, %d", xTest, yTest,
             zTest);
      ...
      if (testArray != NULL) {
        ...
        if (res == true)
          nlinfo("TestArray wasn't configured properly but was found, double "
                 "check the form.");
        else
      ...
      if (coolFiles != NULL) {
        ...
        for (uint idx = 0; idx < size; idx++) {
          ...
          // since we know the structure of the file, we know the array contains
          // structs retrieve the struct for use.
          ...
          nlinfo("Retrieved struct %d: |%s| - %s at %d (%d)", idx, name.c_str(),
                 shortname.c_str(), rank, toptenrank);
        }
      } else {
        if (res == true)
          nlinfo("CoolFilesInfo was found, but might have been entered "
                 "incorrectly, double check the form.");
        else
          nlinfo("CoolFilesInfo wasn't found or there was a syntax error "
                 "loading your form.");
      }
      ...
      nlinfo("Bool Backet 1: %s, Bool Bracket 2: %s", bbool1.c_str(),
             bbool2.c_str());

      // likewise you can access structures by index and dot notation. take the
      // CoolFilesInfo. the following should return 1, the ranking for the
      // Nevrax Presents element. and the 2nd one should return 'Crazy Car Jump'
      // from that element.
      uint rankTest;
      std::string nameTest;
      root.getValueByName(rankTest, ".CoolFilesInfo[2].Ranking");
      root.getValueByName(nameTest, ".CoolFilesInfo[1].ShortName");
      nlinfo("Retrieved .CoolFilesInfo[2].Ranking using bracket and dot "
             "notation: %d",
             rankTest);
      nlinfo("Retrieved .CoolFilesInfo[1].ShortName using bracket and dot "
             "notation: %s",
             nameTest.c_str());

      // if you set a value to a node name that doesn't exist, it's created.
      // after running this you will see it listed in the form:
      uint writeValue;
      ...
      // now set the value. created tells us if the value was creaed or just
      // updated.
      bool created;
      ...
      // if you're accessing a form read-only (not using set*) you can skip
      // this.
  ...
  ::loadForm("sample_config", "sample_configs.packed_sheets", MySampleConfigs,
             true, false);
  nlinfo("Number of sheets loaded: %d", MySampleConfigs.size());

  // Now demonstrate the packed sheet system using CSheetId's and the
  // sheet_id.bin file.
  nlinfo("Load packed sheets using CSheetId and the sheet_id.bin");
  ::loadForm("sample_config", "sample_configs_sheets.packed_sheets",
             MySampleConfigsSheets, true, false);
  nlinfo("Number of sheets loaded with CSheetId's: %d",
         MySampleConfigsSheets.size());
}

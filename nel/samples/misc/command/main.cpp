...
// (note that it is *not* a string). the second param is the help string that
// will be displayed when the user calls the help command. the third param is
// the usage of the command. it can the an empty string when the command doesn't
// need any parameters.
...
NLMISC_COMMAND(square, "display the square of the parameter", "<value>") {
  ...
  // check args, if there is not the right number of parameters, return bad
  // status.
  if (args.size() != 1)
    return false;
...
// this is the easiest way to set or get a variable in real time. you have a
// direct access to the variable so you can use this macro. first param is the
// type of the variable. second one is the variable name, and the third one is
// an help string that describes what the variable does. when the program is
// executed, you can type the variable name with no parameter to see the value
// of the variable or type the variable name with one parameter (the value you
// want to set) to change the variable contents.
...
// with this macro, you can get or set the variable in real time.
// this is an example of dynamic variable. the goal of this macro is to provide
// an easy way to get or set a variable when you don't have a direct access on
// this variable. first param is the type of the variable. second one is the
// name of the variable. the third one is the help string about this variable
// (what this variable does). when the program is executed, you can type the
// variable name with no parameter to see the value of the variable or type the
// variable name with one parameter (the value you want to set) to change the
// variable contents.
NLMISC_DYNVARIABLE(int, PrivVar, "dummy variable") {
  ...
  // if 'get' is true, you have to set '*pointer' with the value of your
  // variable. if 'get' is false, you have to set your variable with the value
  // of *pointer
  if (get)
    *pointer = dv.get();
  else

...
// create a displayer that displays in a file. The first parameter is the
// filename and the second one says that we want to clear the file before
// logging. If you put false (default value), then, new logs are append to the
// file.
...
int main(int /* argc */, char ** /* argv */) {
  ...
  // so, now, if we send something on the logger, it will be displayed on the 3
  // displayers (stdout, file and message box)
  ...
  // Before each display/displayNL, you have to set the position of where the
  // display occurs. If you don't do that, the position will not be displayed on
  // the displayers.
  CSetLogPosition(&logger, __LINE__, __FILE__)
      .log()
  ...
  // display the string without decoration and with a new line at the end of the
  // string.

...
template <class T> class CEmptyModuleServiceBehav : public T {
public:
...
template <class T> class CEmptyModuleCommBehav : public T {
public:
  std::string buildModuleManifest() const { return ""; }
...
template <class T> class CEmptySocketBehav : public T {
public:
  virtual void
  onModuleSocketEvent(IModuleSocket * /* moduleSocket */,
...
public:
  CInterceptorForwarder() : _Parent(NULL) {}
  ...
  ParentClass *getParent() { return _Parent; }
 ...
 *	predicate.
 *	A typical usage is to declare an instance of this class (specialized
 *with the appropriate predicate) as member of your module. You can also have
 *more than one tracker with different predicate if you are interested in
 *different modules.
 *
 *	NB : don't forget to init() each tracker in order to let it register
 *	in you module interceptor list (typically, call "_MyTracker.init(this,
 *this)" in your module's constructor).
 */
template <class ModulePredicate> class CModuleTracker {
public:
...
public:
  CModuleTracker(const ModulePredicate &pred)
      : _ModulePred(pred), _TrackerCallback(NULL) {}
   ...
   */
  const TTrackedModules &getTrackedModules() const { return _TrackedModules; }
...
private:
  ...
  void fwdOnModuleUp(NLNET::IModuleProxy *moduleProxy) {
    if (_ModulePred(moduleProxy)) {
      if (_TrackedModules.insert(moduleProxy).second &&
...
public:
  ;
  TModuleClassPred(const std::string &className) : _ClassName(className) {}

 ...
 * _UserKey is an uint32 generated by the login_service at each login password
 * verification _UserId is an uint32 uniq for each account (an account could
 * have more than one avatar)
 *
...
public:
  ...
  void serial(NLMISC::IStream &s) {
    ...
    if (!s.isReading() && !_Valid)
      nlwarning("LC: serialize a non valid cookie");
  ...
  std::string setToString() const {
    if (_Valid) {
      char cstr[8 * 3 + 2 + 1];
      NLMISC::smprintf(cstr, 8 * 3 + 2 + 1, "%08X|%08X|%08X", _UserAddr,
                       _UserKey, _UserId);
  ...
  void setFromString(const std::string &str) {
    ...
    // nlinfo ("LC: setFromString %s -> %s, isValid: %d", str.c_str (),
    // toString().c_str (), _Valid);
  }

  std::string toString() const {
    if (_Valid)
      return "'" + NLMISC::toString("%08X", (unsigned int)_UserAddr) + "|" +
             NLMISC::toString("%08X", (unsigned int)_UserKey) + "|" +
             NLMISC::toString("%08X", (unsigned int)_UserId) + "'";
    else

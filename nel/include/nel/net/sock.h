...
struct ESocket : public NLMISC::Exception {
   ...
   * the length of the address when displayed by asString() should no exceed
   * 256.
   */
 ...
 * One CSock object represents a communication between two hosts, the local one
 * and the remote one. This class implements layer 0 of the NeL Network Engine.
 * This class does not handle conversion between big endian and little endian ;
 * the provided buffers are sent raw.
 *
 * The "logging" boolean value is necessary because in this implementation we
 * always log to one single global CLog object : there is not one CLog object
 * per socket. Therefore we must prevent the socket used in CNetDisplayer from
 * logging itself... otherwise we would have an infinite recursion.
 *
 * The "connected" property may have a different meaning whether the socket is a
 * stream socket (e.g. using TCP) or it is a connectionless datagram socket
 * (e.g. using UDP). In the latter, "connected" only means that the local and
 * the remote addresses have been set.
 *
 * Important note: this class is thread-safe, meaning you can access to a CSock
 * object from multiple threads BUT the only things you are allow to do in
 * parallel are receive/send and read the connected() property.
 *
 * You must call CSock::initNetwork() before using any network class (even
 * CInetAddress). You must call CSock::releaseNetwork() at the end of your
 * program.
 *
 * By default, a socket is in blocking mode. Call setNonBlockingMode() to change
 * this behaviour. \author Olivier Cado \author Nevrax France \date 2000-2001
 */
class CSock {
public:
   ...
   * Note: This code is platform-dependant. On Unix, it is errno; on Windows it
   * is the Winsock error code. See also errorString()
   */
  ...
  /// Change the time out value used in getDataAvailable(), which is 0 by
  /// default
  ...
  /** Connection.
   * This method does not return a boolean, otherwise a programmer could ignore
   * the result and no exception would be thrown if connection fails :
   * - If addr is not valid, an exception ESocket is thrown
   * - If connect() fails for another reason, an exception
   * ESocketConnectionFailed is thrown
   */
  virtual void connect(const CInetHost &addrs);

  /** Sets the socket in nonblocking mode. Call this method *after* connect(),
   * otherwise you will get an "would block" error (10035 on Windows). In
   * nonblocking mode, use received() and sent() instead of receive() and send()
   */
   ...
   * - stop a select() in progress in another thread (in this case, just calling
   * the destructor is not enough)
   */
  virtual void close();
  ...
  /// Checks if there is some data to receive, waiting (blocking) at most for
  /// the time out value.
  bool dataAvailable();

  /** Receive a partial or an entire block of data, depending on nonblocking
   * mode.
   *
   * In blocking mode: the method waits until 'len' bytes have been received.
   *
   * In nonblocking mode: the method reads the bytes that have already been
   * received only, and resets 'len' to the number of bytes read. The actual
   * length may be smaller than the demanded length. In no data is available,
   * the return value is CSock::WouldBlock. If dataAvailable() returns true, you
   * are sure that receive() will not return CSock::WouldBlock.
   *
   * In case of graceful disconnection:
   * - connected() become false
   * - the return value is CSock::ConnectionClosed or an ESocketConnectionClosed
   * exception is thrown.
   *
   ...
   */
  CSock::TSockResult receive(uint8 *buffer, uint32 &len,
   ...
   *
   * In nonblocking mode : the method resets len to the actual number of bytes
   * sent. Even if less bytes than expected have been sent, it returns
   * CSock::Ok. The caller is expected to test the actual len to check if the
   * remaining data must be resent.
   *
   * \return CSock::Ok or CSock::Error (in case of failure).
   * When throw_exception is true, the method throws an ESocket exception in
   * case of failure.
   */
...
protected:
  /**
   * Constructor.
   * \param logging Disable logging if the server socket object is used by the
   * logging system, to avoid infinite recursion
   */

 ...
 *		to the connected unsecure route unless some module comming from
 *another transport (or a local module) will try to send a message to a module
 *on an unsecure route. In this case, the gateway will disclose the information
 *about this sender module (and only this one) to the unsecure route. In
 *firewall mode, the gateway keep a list of disclosed module for each route and
 *stop any message that is addressed to an undisclosed module (that can be
 *considered as a hacking attempt). Module that are disclosed by the unsecure
 *routes are normaly seen by all other module.
 *
...
public:
  ...
  /// Return a pointer on the named transport interface, or NULL if the
  /// transport is unknown.
  virtual IGatewayTransport *
  getGatewayTransport(const std::string &transportName) const = 0;
  ...
  /// Return the number of ping received. This is incremented by special
  /// "GW_PING" message for unit testing
  ...
  virtual void
  getModuleProxyList(std::vector<IModuleProxy *> &resultList) const = 0;
  //@}
  ...
  //&senderGateway, TModuleMessagePtr &message) =0;
 ...
 *
 *	At time of writing, NeL come with 2 transport : one based on layer 3
 *client, and one based on layer 3 server. In a short time, there will be
 *transport using layer 5.
 */
class IGatewayTransport {
protected:
...
public:
  ...
  IGatewayTransport(const TCtorParam &param)
      : Firewalled(false), PeerInvisible(false) {
...
protected:
#ifdef NL_DEBUG
  /// A debug flag that trigger an assert if true and something delete this
  /// object.
...
public:
  ...
  /// next message type, set to CModuleMessageHeaderCodec::mt_invalid when no
  /// module message are awaited
...
struct CAutoAssertSetter {
  ...
  CAutoAssertSetter(const CGatewayRoute &gwr) : GatewayRoute(gwr) {
    GatewayRoute._AssertOnDelete = true;
  }

  ~CAutoAssertSetter() { GatewayRoute._AssertOnDelete = false; }
};

#define NLNET_AUTO_DELTE_ASSERT                                                \
  CAutoAssertSetter __autoDeleteAssert(                                        \
      static_cast<const CGatewayRoute &>(*this))
#else
#define NLNET_AUTO_DELTE_ASSERT
#endif
...
public:
  ...
  CGatewaySecurity(const TCtorParam &params) : _Gateway(params.Gateway) {}
   ...
   *	check the data validity, and eventually, reject the one or mode data
   *block then it must update the proxy security with the new security data. If
   *the security plug-in reject one or more security data, it has the duty for
   *deleting the rejected object. Furthermore, it must free the existing
   *security data on the proxy if is replace them with the new one. Any other
   *combination is fine unless the security plug-in do a correct management of
   *freeing unused security data.
   */
  virtual void onNewSecurityData(CGatewayRoute *from, IModuleProxy *proxy,
                                 TSecurityData *firstSecurityData) = 0;

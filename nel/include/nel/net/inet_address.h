...
public:
  ...
  static void parseNameAndPort(std::string &hostname, uint16 &port,
  ...
  /// If you only care about the address, check getAddress().isValid() or
  /// isAddressValid()
  bool isValid() const;
  ...
  // Convert an IPv4 or the Any listener address and port to the sockaddr_in
  // structure
...
public:
  /// Returns the internal network address (it s the network address for example
  /// 192.168.0.0 for a C class) (DEPRECATED) // FIXME: IPv6
  uint32 internalNetAddress() const;

  /// Returns readable IP address. (ex: "195.68.21.195")
  std::string ipAddress() const;

  /// Returns readable IP address. (ex: "195.68.21.195") (previously host name)
  /// (DEPRECATED)
...
/// Take a internet dot string and convert it in an uint32 internal format for
/// example "128.64.32.16" -> 0xF0804020

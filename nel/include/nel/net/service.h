...
/**
 * The goal of this macro is to simplify the service creation, it creates the
 main body function.
 *
 * If you don't want to give a callback array, just put EmptyCallbackArray in
 the last argument
 *
 * Example:
 *\code
    ...
    // Create the main() function that create a test service instance and
 execute it.
    ...
    // the last 2 path are where is the config file is (directory) and where to
 log info (directory) NLNET_SERVICE_MAIN(CTestService, "TS", "test_service", 0,
 EmptyCallbackArray, "", "");
 *\endcode
 *
 * If you want the port to not be auto-assigned by the naming service, set the
 port to a number different than 0.
 *
 ...
 * -D followed by the client listening address of the frontend for the login
 system (FS only)
 ...
 * -Z[u] to just init the config file then return (used for test), use Zu to not
 release the service
 *
 *
 */
...
#if defined(NL_OS_WINDOWS) && defined(_WINDOWS)
#define NLNET_SERVICE_MAIN(__ServiceClassName, __ServiceShortName,             \
                           __ServiceLongName, __ServicePort,                   \
                           __ServiceCallbackArray, __ConfigDir, __LogDir)      \
                                                                               \
  int APIENTRY nltWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,        \
    ...
    scn->setCallbackArray(__ServiceCallbackArray,                              \
                          sizeof(__ServiceCallbackArray) /                     \
                              sizeof(__ServiceCallbackArray[0]));              \
    sint retval =                                                              \
        scn->main(__ServiceShortName, __ServiceLongName, __ServicePort,        \
                  __ConfigDir, __LogDir, __DATE__ " " __TIME__);               \
    delete scn;                                                                \
    return retval;                                                             \
  }
...
/// Callback where you can return true for direct clearance, or false for later
/// clearance.
...
class IService {
public:
  /// \name User overload methods. These methods can be overload by the user do
  /// handle init, update and release operation.
  // @{

  /** Called before the displayer is created, no displayer or network connection
     are built. Use this callback to check some args and perform some command
     line based stuff */
  ...
  /// This function is called every "frame" (you must call init() before). It
  /// returns false if the service is stopped.
  virtual bool update() { return true; }

  /// Finalization. Release the service. For example, this function frees all
  /// allocations made in the init() function.
  virtual void release() {}

  //@}

  /// \name get methods. These methods provide a way to read internal service
  /// variables.
  // @{
  ...
  /// Returns the current service unified name that is   alias/short-id or
  /// short-id if alias is empty
  ...
  /// Returns arguments of the program pass from the user to the program using
  /// parameters (ie: "myprog param1 param2")
  const NLMISC::CVectorSString &getArgs() const { return _Args; }

  /// Returns true if the argument if present in the command line (ie:
  /// haveArg('p') will return true if -p is in the command line)
   ...
   * getArg('p') will return C:\Documents and Settings\toto.tmp if
   * -p"C:\Documents and Settings\toto.tmp" is in the command line It'll thrown
   * an Exception if the argName is not found
   */
  std::string getArg(char argName) const;

  /// return true if named long arg is present on the commandline
  /// eg haveLongArg("toto") returns true if "--toto" or "--toto=xxx" can be
  /// found on commandline
  ...
  /// \name set methods. These methods provide a way to modify internal service
  /// variables.
  // @{

  /** Sets the status of the service, this status is return to the application.
   * EXIT_SUCCESS is the default status You can set it to EXIT_FAILURE or any
   * value you want. It's useful when you use the service in a script and you
   * want to know the return value of the application to do the appropriate
   * things.
   */
  void setExitStatus(sint exitStatus) { _ExitStatus = exitStatus; }

  /** Call this function if you want the service quits next loop. The code will
   * be returned outside of the application. \warning If you set the code to 0,
   * it ll NOT exit the service */
  void exit(sint code = 0x10);

  /** Selects timeout value in seconds for each update. You are absolutely
   * certain that your update() function will not be called before this amount
   * of time you set. If you set the update timeout value higher than 0, all
   * messages in queues will be process until the time greater than the timeout
   * user update(). If you set the update timeout value to 0, all messages in
   * queues will be process one time before calling the user update().
   *
   * The default value is 100 (100ms) or the value found in the config file
   * (UpdateTimeout)
   */
  void setUpdateTimeout(
      NLMISC::TTime
          timeout) { /*if (timeout>1.0) nlerror ("IServer::setUpdateTimeout is
                        now a double in SECOND and not ms");*/
  ...
  // use to display result of command (on file and windows displayer)
  // **without** filter
  NLMISC::CLog CommandLog;

  //@}

  /// \name private methods. These methods are used by internal system but can't
  /// be put in private, don't use them.
  // @{

  /// This main is called by the macro (service5 says if we have to use layer5
  /// or not)
  sint main(const char *serviceShortName, const char *serviceLongName,
            uint16 servicePort, const char *configDir, const char *logDir,
            const char *compilationDate);

  /// Sets the command line and init _Args variable. You must call this before
  /// calling main()
  ...
  void setCallbackArray(TUnifiedCallbackItem *array, uint nbelem) {
    _CallbackArray = array;
    _CallbackArraySize = nbelem;
  }
  ...
  /// Directory where to store files that the services will write but are the
  /// same for all shard instance (for example: packet_sheets) Use .toString()
  /// to access to the value
  NLMISC::CVariable<std::string> WriteFilesDirectory;

  /// Directory where to store files that the services will write during the
  /// exploitation of the game (for example: player backup, string cache) Use
  /// .toString() to access to the value
  NLMISC::CVariable<std::string> SaveFilesDirectory;

  /// If true (default), the provided SaveFilesDirectory will be converted to a
  /// full path (ex: "saves" -> "/home/dir/saves")
  NLMISC::CVariable<bool> ConvertSavesFilesDirectoryToFullPath;

  /** You can provide a callback interface (only one) that will be called if any
   * of the directory variables (WriteFilesDirectory, SaveFilesDirectory,
   * ConfigDirectory, LogDirectory, RunningDirectory) is changed (also called
   * for the first setting read from the .cfg file). Default is NULL.
   */
  void setDirectoryChangeCallback(NLMISC::IVariableChangedCallback *cbi) {
    _DirectoryChangedCBI = cbi;
  }
  ...
  // Warning: can take a moment to be received from the WS. The default value
  // (when not received yet) is DEFAULT_SHARD_ID.
  uint32 getShardId() const { return _ShardId; }

  const NLMISC::CCPUTimeStat &getCPUUsageStats() const {
    return _CPUUsageStats;
  }
  ...
  /**
   * If your service needs a delay when it is asked to quit, provide a callback
   * here (optional). Then, when the service will be asked to quit, this
   * callback will be called. Then you can either return true to allow immediate
   * closure, or false to delay the closure. The closure will then happen after
   * you call clearForClosure().
   *
   * If you don't provide a callback here, or if you call with NULL, the service
   * will exit immediately when asked to quit.
   */
  void setClosureClearanceCallback(TRequestClosureClearanceCallback cb) {
    _RequestClosureClearanceCallback = cb;
  }

  /**
   * If using clearance for closure (see setClosureClearanceCallback()), you can
   * call this method to allow the service to quit. If calling it while your
   * callback has not be called yet, the callback will be bypassed and the
   * service will quit immediately when asked to quit. If calling it after you
   * callback was called (and you returned false), the service will quit
   * shortly.
   */
  void clearForClosure() { _ClosureClearanceStatus = CCClearedForClosure; }

  /** Set the shard id (by the user code), when known before IService receives
   * it by the WS). If a non-default value is already set and different than
   * shardId => nlerror. If later IService receives a different value from the
   * WS => nlerror.
   */
...
private:
  ...
  /// Current service name sets by the actual service when declaring
  /// NLNET_SERVICE_MAIN
  std::string _ShortName; // ie: "NS"
  std::string _LongName;  // ie: "naming_service"
  std::string _AliasName; // this name is initialized by the admin executor
                          // service via the args
  ...
  /// the exit status of this service (the status is returned by the service at
  /// the release time)
  sint _ExitStatus;
  ...
  enum TClosureClearanceStatus {
    CCMustRequestClearance,
    CCWaitingForClearance,
    CCClearedForClosure,
    CCCallbackThenClose = 256
  };

  /// Closure clearance state (either CCMustRequestClearance,
  /// CCWaitingForClearance, CCClearedForClosure or CCCallbackThenClose + any
  /// other as a backup value)
  ...
  friend void serviceGetView(uint32 rid, const std::string &rawvarpath,
                             std::vector<std::string> &vara,
                             std::vector<std::string> &vala);
  friend void cbAESConnection(const std::string &serviceName, TServiceId sid,
                              void *arg);
  ...
  friend void cbReceiveShardId(NLNET::CMessage &msgin,
...
/** Interface class for object that need an update call during each service
 * loop.
 */
class IServiceUpdatable {
public:
  IServiceUpdatable() {
    if (IService::isServiceInitialized()) {
      ...
      nlwarning("IServiceUpdatable : IService is not initialized, IUpdatable "
                "will not be called");
    }
  }
...
inline IService *IService::getInstance() {
  if (_Instance == NULL) {
    ...
    nlassertex(NLMISC::INelContext::isContextInitialised(),
               ("You are trying to access a safe singleton without having "
                "initialized a NeL context. The simplest correction is to add "
                "'NLMISC::CApplicationContext myApplicationContext;' at the "
                "very beginning of your application."));
    // try to retrieve the safe singleton pointer
    _Instance = reinterpret_cast<IService *>(
        NLMISC::INelContext::getInstance().getSingletonPointer("IService"));
  }
  return _Instance;
}

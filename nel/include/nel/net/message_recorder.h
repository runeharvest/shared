...
/// Type of network events (if changed, don't forget to change EventToString()
/// and StringToEvent()
enum TNetworkEvent {
  Sending,
  Receiving,
...
struct TMessageRecord {
  ...
  TMessageRecord(bool input = false)
      : UpdateCounter(0), SockId(InvalidSockId), Message("", input) {}
  ...
  void serial(NLMISC::CMemStream &stream) {
    ...
    if (stream.isReading()) {
      ...
      len = Message.length();
      stream.serial(len);
      stream.serialBuffer(const_cast<uint8 *>(Message.buffer()),
                          len); // assumes the message contains plain text
    }
  }
...
public:
  ...
  /// Add a record
  void recordNext(sint64 updatecounter, TNetworkEvent event, TSockId sockid,
                  CMessage &message);
  ...
  /// Receive queue (corresponding to one update count). Use empty(), front(),
  /// pop().

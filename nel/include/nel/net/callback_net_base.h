 ...
 * from is the SockId of the connection, for a client, from is always the same
 * value
 */
typedef std::function<void(CMessage &msgin, TSockId from,
...
public:
   ...
   * On a server, you must provide a hostid. If you hostid = InvalidSockId, the
   * message will be sent to all connected client.
   */
  virtual void send(const CMessage &buffer, TSockId hostid = InvalidSockId,
                    bool log = true) = 0;
  ...
  virtual void displaySendQueueStat(NLMISC::CLog *log = NLMISC::InfoLog,
                                    TSockId destid = InvalidSockId) = 0;
   ...
   * If you provide a non-null pointer for nbBytesRemaining, the value will be
   * filled* will the number of bytes that still remain in the sending queue
   * after the non-blocking flush attempt.
   */
  virtual bool flush(TSockId hostid = InvalidSockId,
                     uint *nbBytesRemaining = NULL) = 0;

  /**	Appends callback array with the specified array. You can add callback
   * only *after* adding the server or the client. \param arraysize is the
   * number of callback items.
   */
  ...
  /// Set the pre dispatch callback. This callback is called before each message
  /// is dispatched
  void setPreDispatchCallback(TMsgCallback predispatchCallback) {
    _PreDispatchCallback = predispatchCallback;
  }
  ...
  /// returns the sockid of a connection. On a server, this function returns the
  /// parameter. On a client, it returns the connection.
  virtual TSockId getSockId(TSockId hostid = InvalidSockId) = 0;

  /** Sets the callback that you want the other side calls. If it didn't call
   * this callback, it will be disconnected If cb is NULL, we authorize *all*
   * callback. On a client, the hostid must be InvalidSockId (or ommited). On a
   * server, you must provide a hostid.
   */
...
protected:
  ...
  /// Constructor.
  CCallbackNetBase(TRecordingState rec = Off,
  ...
  /// On this layer, you can't call directly receive, It s the update() function
  /// that receive and call your callaback
...
private:
  ...
  friend void cbnbMessageAskAssociations(CMessage &msgin, TSockId from,
                                         CCallbackNetBase &netbase);
  friend void cbnbMessageRecvAssociations(CMessage &msgin, TSockId from,
                                          CCallbackNetBase &netbase);

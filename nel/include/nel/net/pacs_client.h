...
/**
 * Client side of Pacs Service. Allows to use PACS functionnality by the
 * networtk.
 *
 ...
 */
class CPacsClient {
  friend void cbPacsAnswer(CMessage &msgin, TSockId from,
...
public:
   ...
   * Send the message
   *
   * You must call this method after initMessage and others calls to setup
   * methods.
   */
  void sendMessage() {
    // Checks
   ...
   */
  void testMove(NLPACS::UMovePrimitive::TUserData id,
                const NLMISC::CVectorD &speed, double deltaTime) {
    // Append to the current message
    std::string name = "TS";
    bool nlTrue = true;
    _Message.serial(nlTrue, name, id, const_cast<NLMISC::CVectorD &>(speed),
                    deltaTime);
  }
   ...
   * No answer will be send by the service.
   *
   * \param absorption is the new attenuation factor for the primitive on the
   * service.
   */
  ...
  /**
   * Set the size of the current primitive on the service. Only for boxes
   * primitives.
   *
   * No answer will be send by the service.
   *
   * \param width is the new size on X axis factor for the primitive on the
   * service. \param depth is the new size on Y axis factor for the primitive on
   * the service.
   */
  ...
  /**
   * Set the height of the current primitive on the service. For boxes and
   * cylinders primitives.
   *
   * No answer will be send by the service.
   *
   * \param height is the new size on Z axis factor for the primitive on the
   * service.
   */
  ...
  /**
   * Set the radius of the current primitive on the service. For cylinders
   * primitives.
   *
   * No answer will be send by the service.
   *
  ...
  /**
   * Query the position and the speed of the primitive after an evalCollision
   * message.
   *
...
protected:
  ...
  /**
   * This call back is called when a message is coming. Used for
   * synchronisation.
   */
   ...
   * \param triggerInfo is an array of trigger descriptor. Each entry of the
   * array is a new trigger raised by evalCollision.
   */
  virtual void
  triggerCallback(uint32 evalId,
   ...
   */
  virtual void testMoveCallback(NLPACS::UMovePrimitive::TUserData id,
                                bool testResult) {}
   ...
   */
  virtual void getPositionSpeedCallback(NLPACS::UMovePrimitive::TUserData id,
...
static void cbPacsAnswer(CMessage &msgin, TSockId from,
                         CCallbackNetBase &netbase) {
...
static TCallbackItem PacsCallbackArray[] = {{"PACS_ASW", cbPacsAnswer}};
...
inline bool CPacsClient::connect() {
  ...
  if (_Server->connected()) {
    // Add callback array
    _Server->addCallbackArray(PacsCallbackArray,
                              sizeof(PacsCallbackArray) /
                                  sizeof(PacsCallbackArray[0]));

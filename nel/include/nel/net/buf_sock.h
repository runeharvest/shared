...
public:
  ...
  /// Little tricky but this string is used by Layer4 to know which callback is
  /// authorized. This is empty when all callback are authorized.
...
protected:
  ...
  /** Constructor
   * \param sock To provide an external socket. Set it to NULL to create it
   * internally.
   */
   ...
   * exceeds this calue, all data in the send queue is automatically sent (-1 to
  ...
  /** Pushes a disconnection message into bnb's receive queue, if it has not
   * already been done (returns true in this case). You can either specify a
   * sockid (for server) or InvalidSockId (for client)
   */
...
#endif
    return advertiseSystemEvent(bnb, sockid, _KnowConnected, true,
                                CBufNetBase::Disconnection);
  }

  /** Pushes a system message into bnb's receive queue, if the flags meets the
   * condition, then resets the flag and returns true. You can either specify a
   * sockid (for server) or InvalidSockId (for client).
   */
  bool advertiseSystemEvent(CBufNetBase *bnb, TSockId sockid, bool &flag,
                            bool condition, CBufNetBase::TEventType event) {
#ifdef NL_DEBUG
    if (sockid != InvalidSockId) {
      nlassert(sockid == this);
    }
   ...
   * or returns false if the socket is not physically connected the or an error
  ...
  bool pushBuffer(const NLMISC::CMemStream &buffer) {
    ...
    if (buffer.length() > biggerBufferSize) {
      ...
      LNETL1_DEBUG("LNETL1: new record! bigger network message pushed (sent) "
                   "is %u bytes",
                   biggerBufferSize);
    }
...
//		LNETL1_DEBUG( "LNETL1: Pushing buffer to %s", asString().c_str()
      ...
      if (buffer.size() > biggerBufferSize)
      {
          ...
          nlwarning ("LNETL1: new record! bigger network message pushed (sent)
is %u bytes", biggerBufferSize);
      }
  ...
  /// Connects to the specified addr; set connectedstate to true if no
  /// connection advertising is needed
  void connect(const CInetHost &addr, bool nodelay, bool connectedstate);

  /// Disconnects; set connectedstate to false if no disconnection advertising
  /// is needed
  void disconnect(bool connectedstate);

  /// Sets the "logically connected" state (changed when processing a
  /// connection/disconnection callback)
  void setConnectedState(bool connectedstate) {
    _ConnectedState = connectedstate;
  }

  /// Returns the "logically connected" state (changed when processing a
  /// connection/disconnection callback)
...
private:
  ...
  NLMISC::TTime _LastFlushTime; // updated only if time trigger is enabled
                                // (TriggerTime!=-1)
  ...
  // Connected state (from the user's point of view, i.e. changed when the
  // connection/disconnection event is at the front of the receive queue)
...
protected:
  ...
  /** Constructor
   * \param sock To provide an external socket. Set it to NULL to create it
   * internally. \maxExpectedBlockSize Default value: receiving limited to 10 M
   * per block)
   */
  CNonBlockingBufSock(CTcpSock *sock = NULL,
                      uint32 maxExpectedBlockSize = 10485760);

  /** Call this method after connecting (for a client connection) to set the
   * non-blocking mode. For a server connection, call it as soon as the object
   * is constructed
   */
   ...
   * \param nbExtraBytes Number of bytes to reserve for extra information such
   * as the event type \return True if the message has been completely received
   */
  bool receivePart(uint32 nbExtraBytes);

  /// Fill the event type byte at pos length()(for a client connection)
  void fillEventTypeOnly() {
    _ReceiveBuffer[_Length] = (uint8)CBufNetBase::User;
  }

  /** Return the length of the received block (call after receivePart() returns
   * true). The total size of received buffer is length() + nbExtraBytes (passed
   * to receivePart()).
   */
  ...
  const std::vector<uint8> receivedBuffer() const {
    nlnettrace("CServerBufSock::receivedBuffer");
...
private:
  ...
  // Counts the number of bytes read for the current element (length prefix or
  // buffer)
...
/**
 * CServerBufSock
 * A socket, its send buffer plus a nonblocking receiving system for a server
 * connection
 */
...
protected:
  ...
  /** Pushes a connection message into bnb's receive queue, if it has not
   * already been done (returns true in this case).
   */
  bool advertiseConnection(CBufServer *bnb) {
    return advertiseSystemEvent((CBufNetBase *)bnb, this, _KnowConnected, false,
                                CBufNetBase::Connection);
  }
...
private:
  /// True after a connection callback has been sent to the user, for this
  /// connection
  bool _Advertised;

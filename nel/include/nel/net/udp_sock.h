...
public:
  ...
  /** Binds the socket to the specified port. Call bind() for an unreliable
   * socket if the host acts as a server and expects to receive messages. If the
   * host acts as a client, call directly sendTo(), in this case you need not
   * bind the socket.
   */
  void bind(uint16 port);

  /// Same as bind(uint16) but binds on a specified address/port (useful when
   ...
   * The socket must have been bound before, by calling either bind() or
   * sendTo(). \param buffer [in] Address of buffer \param len [in/out]
   * Requested length of buffer, and actual number of bytes received \param addr
   * [out] Address of sender
   */

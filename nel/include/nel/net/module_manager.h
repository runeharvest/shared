...
public:
  ...
  /// Must be called before releasing the instance, will call
  /// IModule::onApplicationExit on all module instance
   ...
   */
  virtual void
  getAvailableModuleClassList(std::vector<std::string> &moduleClassList) = 0;
   ...
   *	TODO : make this method only available to CModuleSocket to prevent
   *dramatic ERROR
   */
  ...
  createModuleProxy(IModuleGateway *gateway, CGatewayRoute *route,
                    uint32 distance, IModule *localModule,
                    const std::string &moduleClassName,
                    const std::string &moduleFullyQualifiedName,
                    const std::string &moduleManifest,
                    //													const
                    //TModuleGatewayProxyPtr &foreignGateway,
...
typedef NLMISC::CFactoryIndirect<IModuleFactory, std::string>

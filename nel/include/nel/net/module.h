...
public:
   ...
   */
  virtual bool onProcessModuleMessage(IModuleProxy *senderModuleProxy,
                                      const CMessage &message) = 0;
...
public:
  ...
  /// An operation invocation has failed because of a bad return type from
  /// servant
  ...
  virtual void onServiceUp(const std::string &serviceName,
                           NLNET::TServiceId serviceId) = 0;
  /// A Nel layer 5 service has stopped.
  virtual void onServiceDown(const std::string &serviceName,
                             NLNET::TServiceId serviceId) = 0;
  /** Regular update from application.
   *	If the application is a service, then it call
   *CModuleManager::updateModules at each service loop. If the application is a
   *regular application, then you have to call manually
   ...
   */
  virtual void
  getPluggedSocketList(std::vector<IModuleSocket *> &resultList) = 0;
  //@}
   ...
   *	a gateway connection).
   */
  //		virtual void				onModuleUp(IModuleProxy
  //*moduleProxy) = 0;
  /** Called by a socket to inform this module that another
   *	module has been deleted OR has been no more accessible (due to
   *	some gateway disconnection).
   */
  //		virtual void
  //onModuleDown(IModuleProxy *moduleProxy) =0;
   ...
   */
  virtual void onReceiveModuleMessage(IModuleProxy *senderModuleProxy,
                                      const CMessage &message) = 0;
  /** Called internally by basic module imp to process the message by
   *	application code.
   */
  //		virtual void
  //onProcessModuleMessage(IModuleProxy *senderModuleProxy, const CMessage
  //&message) =0;
  /** Called by a socket to inform this module that the security
   *	data attached to a proxy have changed.
   */
  //		virtual void
  //onModuleSecurityChange(IModuleProxy *moduleProxy) =0;
   ...
   */
  virtual void invokeModuleOperation(IModuleProxy *destModule,
                                     const NLNET::CMessage &opMsg,
                                     NLNET::CMessage &resultMsg) = 0;
  ...
  virtual void onModuleSocketEvent(IModuleSocket *moduleSocket,
                                   TModuleSocketEvent eventType) = 0;
...
struct TSecurityData {
  ...
  struct TCtorParam {
    ...
    TCtorParam(uint8 dataTag) : DataTag(dataTag) {}
  };
  ...
  TSecurityData(const TCtorParam &params)
      : DataTag(params.DataTag), NextItem(NULL)
...
struct TUnknownSecurityData : public TSecurityData {
  ...
  TUnknownSecurityData(uint8 realDataTag, uint32 size)
      : TSecurityData(TSecurityData::TCtorParam(0xff)),
        RealDataTag(realDataTag), Data(size) {}
...
//	NLMISC_REGISTER_OBJECT(TSecurityData, TUnknownSecurityData, uint8,
//0xff);
...
public:
  ...
  void failInvoke() { _FailInvoke = true; }

  bool mustFailInvoke() { return _FailInvoke; }

  void resetFailInvoke() { _FailInvoke = false; }

  void processPendingMessage(class CModuleBase *module);
};

/// Template module task
template <class T> class TModuleTask : public CModuleTask {
public:
...
private:
  ...
  void run() {
    ...
    try {
      ...
      nlwarning("In module task '%s', exception '%e' thrown",
                typeid(this).name(), e.what());
    } catch (...) {
      nlwarning("In module task '%s', unknown exception thrown",
                typeid(this).name());
    }
...
public:
  TModuleTask(T *module, void (T::*taskMethod)())
      : CModuleTask(module), _Module(module), _TaskMethod(taskMethod) {}
};

// a special macro for easy module task startup
#define NLNET_START_MODULE_TASK(className, methodName)                         \
  {                                                                            \
    NLNET::TModuleTask<className> *task =                                      \
        new NLNET::TModuleTask<className>(this, &className::methodName);       \
...
template <class moduleClass> class CModuleFactory : public IModuleFactory {
public:
...
#define NLNET_REGISTER_MODULE_FACTORY(moduleClassName, registrationName)       \
  class moduleClassName##Factory                                               \
      : public NLNET::CModuleFactory<moduleClassName> {                        \
  public:                                                                      \
  ...
  NLMISC_REGISTER_OBJECT_INDIRECT(NLNET::IModuleFactory,                       \
                                  moduleClassName##Factory, std::string,       \
                                  registrationName)

// #define NLNET_MAKE_MODULE_FACTORY_TYPENAME(moduleClassName)
// moduleClassName##Factory
...
class CModuleBase : public IModule, public NLMISC::ICommandsHandler {
  ...
  /// list of coroutine to run for synchronous messaging (the first in the list
  /// is running)
...
protected:
  ...
  void invokeModuleOperation(IModuleProxy *destModule,
                             const NLNET::CMessage &opMsg,
                             NLNET::CMessage &resultMsg);
  void _onModuleUp(IModuleProxy *removedProxy);
  void _onModuleDown(IModuleProxy *removedProxy);

  bool _onProcessModuleMessage(IModuleProxy *senderModuleProxy,
                               const CMessage &message);

  /// base module command table
  NLMISC_COMMAND_HANDLER_TABLE_BEGIN(CModuleBase)
  NLMISC_COMMAND_HANDLER_ADD(CModuleBase, dump,
                             "display information about module instance status",
                             "no args")
  NLMISC_COMMAND_HANDLER_ADD(CModuleBase, plug,
                             "plug the module in a module socket",
                             "<socket_name>")
  NLMISC_COMMAND_HANDLER_ADD(CModuleBase, unplug,
                             "unplug the module out of a module socket",
                             "<socket_name>")
  NLMISC_COMMAND_HANDLER_ADD(
      CModuleBase, sendPing,
      "send a ping message to another module using the first available route",
      "<addresseeModuleName>")
...
class CModuleProxy : public IModuleProxy {
  ...
  CModuleProxy(TModulePtr localModule, TModuleId localModuleId,
               const std::string &moduleClassName,
               const std::string &fullyQualifiedModuleName,
               const std::string &moduleManifest);

public:
  const CGatewayRoute *getRoute() const { return _Route; }
 ...
 */
template <class PtrContainer> class TBroadcastModuleMessage {

  void sendMessage(IModule *sender, const PtrContainer &addresseeProxies,
                   const NLNET::CMessage &message) {
    typename PtrContainer::iterator first(addresseeProxies.begin()),
        last(addresseeProxies.end());

...
public:
  ...
  /// Returns if the host and port are valid, check isAddressValid if you only
  ...
  inline bool isAddressValid() const {
    return m_Addresses[0].getAddress().isValid();
  }

  /// Returns all local addresses
  static CInetHost localAddresses(uint16 port = 0, bool sort = true,
                                  bool loopback = true);

  /// Returns the list of addresses
  inline const std::vector<CInetAddress> &addresses() const {
    return m_Addresses;
  }
  ...
  /// Get a vector of this CInetHost separated into individual copies for each
  /// address. This is for compatibility purposes
  ...
  inline bool operator!=(const CInetHost &other) const {
    return !(*this == other);
  }

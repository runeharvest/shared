...
/**
 * Use this macro to register a class that can be transported in the init of
 * your program.
 */
...
/**
 * You have to inherit this class and implement description() and callback()
 * method. For an example of use, take a look at nel/samples/class_transport
 * sample. \author Vianney Lecroart \author Nevrax France \date 2002
 */
...
public:
  ...
  //		PropBool, PropFloat, PropDouble, PropString, PropDataSetRow,
  //PropEntityId, PropSheetId, PropUKN };

  //
  // Static methods
  //

  /// Init the transport class system (must be called one time, in the
  /// IService5::init() for example)
  static void init();

  /// Release the transport class system (must be called one time, in the
  /// IService5::release() for example)
  static void release();

  /** Call this function to register a new transport class.
   * \param instance A reference to a GLOBAL space of the instance of this
   * transport class. It will be used when receive this class from network.
   */
  ...
  /** You have to implement this function with the description of your class.
   This description
   * is used to send the class accross the network and read it. It must contains
   a class name and
   * a set properties.
   * Example:
   *\code
  ...
  /** This function will be call when we receive this class from the network. It
   * will use the instance given at the registration process. By default, it
   * does nothing.
   */
  virtual void callback(const std::string & /* name */,
   ...
   * \param defaultValue The value you want to be set when a message comes
   * without this property \param value Reference to the value where the
   * property will be read or write
   */
  template <class T>
  void property(const std::string &name, TProp type, T defaultValue, T &value) {
    switch (type) {
    case PropUInt8:
    case PropSInt8:
    case PropBool:
    ...
    case PropString:
      ...
      break;
      //		case PropEntityId: nlassert(sizeof(T) == sizeof
      //(NLMISC::CEntityId)); break;
    case PropSheetId:
    ...
    case PropUCString:
      ...
      break;
    default:
      nlerror("property %s have unknown type %d", name.c_str(), type);
    }
    ...
    } else if (Mode == 3) // register
    {
      ...
      TempRegisteredClass.Instance->Prop.push_back(
          new CRegisteredProp<T>(name, type, defaultValue, &value));
  ...
  template <class T>
  void propertyVector(const std::string &name, TProp type,
                      std::vector<T> &value) {
    if (Mode == 2) // write
    {
      // send only if needed
    ...
    } else if (Mode == 3) // register
    {
      ...
      nlassert(TempRegisteredClass.Instance != NULL);
      TempRegisteredClass.Instance->Prop.push_back(
          new CRegisteredPropCont<std::vector<T>>(name, type, &value));
    } else if (Mode == 4) // display
    {
      ...
      NETTC_DEBUG("NETTC:   prop %s %d: %d elements ( %s)", name.c_str(), type,
                  value.size(), val.c_str());
    } else {
      nlstop;
    }
  }
  ...
  template <class T>
  void propertyCont(const std::string &name, TProp type, T &value) {
    if (Mode == 2) // write
    {
      // send only if needed
      // todo manage unknown prop
    ...
    } else if (Mode == 3) // register
    {
      ...
      nlassert(TempRegisteredClass.Instance != NULL);
      TempRegisteredClass.Instance->Prop.push_back(
          new CRegisteredPropCont<T>(name, type, &value));
    } else if (Mode == 4) // display
    {
  ...
  // Read the header (first part of the transport class message, currently only
  // className)
...
protected:
  ...
  typedef std::vector<std::pair<std::string, std::vector<CRegisteredBaseProp>>>
  ...
  template <class T> struct CRegisteredProp : public CRegisteredBaseProp {
    ...
    CRegisteredProp(const std::string &name, TProp type, T defaultValue,
                    T *value = NULL)
        : CRegisteredBaseProp(name, type), DefaultValue(defaultValue),
          Value(value) {}
  ...
  static TRegisteredClass
      LocalRegisteredClass; // registered class that are in my program

  // The registered class that is currently filled (before put in
  ...
  static void registerOtherSideClass(NLNET::TServiceId sid,
  ...
  static void
  displayDifferentClass(NLNET::TServiceId sid, const std::string &className,
                        const std::vector<CRegisteredBaseProp> &otherClass,
                        const std::vector<CRegisteredBaseProp *> &myClass);

  //
  // Friends
  //

  friend void cbTCReceiveMessage(NLNET::CMessage &msgin,
                                 const std::string &name,
                                 NLNET::TServiceId sid);
  friend void cbTCUpService(const std::string &serviceName,
                            NLNET::TServiceId sid, void *arg);
  friend void cbTCReceiveOtherSideClass(NLNET::CMessage &msgin,
                                        const std::string &name,
                                        NLNET::TServiceId sid);
};
...
#if !FINAL_VERSION
  // Did the programmer forget to register the transport class? Forbid sending
  // then. nlassert( LocalRegisteredClass.find( className() ) !=
  // LocalRegisteredClass.end() ); Name is never set unless in Mode 3 on the
  // temporary registration object, so this nlassert is invalid! :(
#endif
...
inline bool CTransportClass::read(const std::string &name,
                                  NLNET::TServiceId sid) {
  nlassert(Init);
  nlassert(Mode == 0);

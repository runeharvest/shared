...
public:
	...
	void property(const std::string &name, TProp type, T defaultValue, T &value)
	{
		...
		case PropUInt8:
		case PropSInt8:
		case PropBool: nlassert(sizeof(T) == sizeof(uint8)); break;
		case PropUInt16:
		case PropSInt16: nlassert(sizeof(T) == sizeof(uint16)); break;
		case PropUInt32:
		case PropSInt32:
		case PropDataSetRow: nlassert(sizeof(T) == sizeof(uint32)); break;
		case PropUInt64:
		case PropSInt64: nlassert(sizeof(T) == sizeof(uint64)); break;
	...
	template <class T>
	void propertyVector(const std::string &name, TProp type, std::vector<T> &value)
	{
		if (Mode == 2) // write
		{
			// send only if needed
...
protected:
	...
	template <class T>
	struct CRegisteredProp : public CRegisteredBaseProp
	{
		CRegisteredProp()
		    : Value(NULL)
		{
	...
	template <class T>
	struct CRegisteredPropCont : public CRegisteredBaseProp
	{
		CRegisteredPropCont()
		    : Value(NULL)
		{

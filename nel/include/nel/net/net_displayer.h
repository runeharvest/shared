 ...
 * \ref log_howto
 * \bug When nlerror is called in a catch block, a connected NetDisplayer
 * becomes an IDisplayer => pure virtual call \author Olivier Cado \author
...
public:
  ...
  /** Sets logging server address. Call this method from outside only if you
   * want to use a LOGS not registered within the NS. It does nothing if the
   * displayer is already connected to a server.
   */
...
protected:
   ...
   * \warning If not connected, tries to connect to the logging server each
   * call. It can slow down your program a lot.
   */

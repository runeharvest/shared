...
public:
  /// Constructor
  CCallbackServer(TRecordingState rec = Off,
  ...
  /// Force to send all data pending in the send queue. See comment in
  /// CCallbackNetBase.
  bool flush(TSockId destid, uint *nbBytesRemaining = NULL) {
    nlassert(destid != InvalidSockId);
    return CBufServer::flush(destid, nbBytesRemaining);
  }
  ...
  /// Returns true if the connection is still connected. on server, we always
  /// "connected"
   ...
   * does nothing. Before disconnecting, any pending data is actually sent.
   */
  ...
  const CInetAddress &hostAddress(TSockId hostid) {
    ...
    return CBufServer::hostAddress(hostid);
  }
  ...
  void displayReceiveQueueStat(NLMISC::CLog *log = NLMISC::InfoLog) {
    CBufServer::displayReceiveQueueStat(log);
  }
  void displaySendQueueStat(NLMISC::CLog *log = NLMISC::InfoLog,
                            TSockId destid = InvalidSockId) {
    CBufServer::displaySendQueueStat(log, destid);
  }

  void displayThreadStat(NLMISC::CLog *log = NLMISC::InfoLog) {
    CBufServer::displayThreadStat(log);
  }

private:
  /// This function is public in the base class and put it private here because
  ...
  std::map<TSockId, TSockId>
      _MR_SockIds; // first=sockid in file; second=CBufSock*
#endif
  // ---------------------------------------
};

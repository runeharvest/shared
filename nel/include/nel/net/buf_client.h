...
public:
  /// Constructor (increments the reference to the object pointed to by the
  /// smart pointer sockid)
  CClientReceiveTask(CBufClient *client, CNonBlockingBufSock *bufsock)
      : NbLoop(0), _Client(client), _NBBufSock(bufsock) {
  ...
  CNonBlockingBufSock *bufSock() {
    return _NBBufSock;
  } // CHANGED: non-blocking client connection (previously, returned
    // _SockId->Sock)
 ...
 * dataAvailable(),                         bytesDownloaded(),
 * newBytesDownloaded() disconnection callback \endcode
 *
 * \author Olivier Cado
...
public:
  /** Constructor. Set nodelay to true to disable the Nagle buffering algorithm
   * (see CTcpSock documentation) initPipeForDataAvailable is for Linux only.
   * Set it to false if you provide an external pipe with
   ...
   * If this is a problem, call flush() multiple times until it returns 0 before
   * calling disconnect(). The disconnection callback will *not* be called. Do
   * not call if the socket is not connected.
   */
  void disconnect(bool quick = false);

  /** Sends a message to the remote host (in fact the message is buffered into
   * the send queue)
   */
  ...
  bool flush(uint *nbBytesRemaining = NULL) {
    return _BufSock->flush(nbBytesRemaining);
  }
  ...
  const CInetAddress &remoteAddress() const {
    return _BufSock->Sock->remoteAddr();
  }

  /// Returns the number of bytes downloaded (read or still in the receive
  /// buffer) since the latest connection
  ...
  /// Returns the number of bytes uploaded since the previous call to this
  /// method
  ...
  (mutexed on the receive queue) uint64	bytesReceived() {  }

 ...
 * By default, the size time trigger is disabled, the time trigger is set to 20
 * ms.
 *
 * Where do the methods take place:
 * \code
 * send(),	                           ->  send buffer   ->  update(),
 * flush() bytesSent(), newBytesSent()
 *
...
public:
   ...
   * documentation) initPipeForDataAvailable is for Linux only. Set it to false
   * if you provide an external pipe with setExternalPipeForDataAvailable().
   */
   ...
   * If hostid is not InvalidSockId and the socket is not connected, the method
   * does nothing. If quick is true, any pending data will not be sent before
   * disconnecting. If quick is false, a flush() will be called. In case of
   * network congestion, the entire pending data may not be flushed. If this is
   * a problem, call flush() multiple times until it returns 0 before calling
   * disconnect().
   */
  ...
  /** Send a message to the specified host, or to all hosts if hostid is
   * InvalidSockId
   */
  // void	send( const std::vector<uint8>& buffer, TSockId hostid );
  void send(const NLMISC::CMemStream &buffer, TSockId hostid);

  /** Checks if there is some data to receive. Returns false if the receive
   * queue is empty. This is where the connection/disconnection callbacks can be
   * called.
   */
  bool dataAvailable();

#ifdef NL_OS_UNIX
  /** Wait until the receive queue contains something to read (implemented with
   * a select()). This is where the connection/disconnection callbacks can be
   * called. If you use this method (blocking scheme), don't use dataAvailable()
   * (non-blocking scheme). \param usecMax Max time to wait in microsecond (up
   * to 1 sec)
   */
   ...
   * You must call dataAvailable() or sleepUntilDataAvailable() before every
   * call to receive()
   */
   ...
   * all data in the send queue is automatically sent (-1 to disable this
   * trigger)
   */
  void setTimeFlushTrigger(TSockId destid, sint32 ms);

  /** Sets the size flush trigger. When the size of the send queue reaches or
   * exceeds this value, all data in the send queue is automatically sent (-1 to
   * disable this trigger )
   */
  void setSizeFlushTrigger(TSockId destid, sint32 size);

  /** Force to send data pending in the send queue now. If all the data could
   * not be sent immediately, the returned nbBytesRemaining value is non-zero.
   * \param destid The identifier of the destination connection.
   * \param nbBytesRemaining If the pointer is not NULL, the method sets the
   * number of bytes still pending after the flush attempt. \returns False if an
   * error has occurred (e.g. the remote host is disconnected). To retrieve the
   * reason of the error, call CSock::getLastError() and/or CSock::errorString()
   */
  ...
  /*
  /// Returns the number of bytes pushed into the receive queue since the
  beginning (mutexed) uint32	bytesDownloaded()
  {
      NLMISC::CSynchronized<uint32>::CAccessor syncbpi ( &_BytesPushedIn );
      return syncbpi.value();
  }
  /// Returns the number of bytes downloaded since the previous call to this
  method uint32	newBytesDownloaded();
  */

  /// Returns the number of bytes popped by receive() since the beginning
  uint64 bytesReceived() const { return _BytesPoppedIn; }

  /// Returns the number of bytes popped by receive() since the previous call to
  /// this method
  ...
  /// Returns the number of bytes pushed by send() since the previous call to
  /// this method
  uint64 newBytesSent();
...
protected:
  ...
  /** Binds a new socket and send buffer to an existing or a new thread (that
   * starts) Note: this method is called in the listening thread.
   */
  void dispatchNewSocket(CServerBufSock *bufsock);

  /// Returns the receive task corresponding to a particular thread
  CServerReceiveTask *
  receiveTask(std::vector<NLMISC::IThread *>::iterator ipt) {
    return ((CServerReceiveTask *)((*ipt)->getRunnable()));
  }

  /// Pushes a buffer to the specified host's send queue and update (unless not
  /// connected)
  ...
  NLMISC::CSynchronized<uint32>&	syncBytesPushedIn() { return
  _BytesPushedIn; }
  */
...
private:
  ...
  /*
    /// Number of bytes pushed into the receive queue (by the receive threads)
    since the beginning. NLMISC::CSynchronized<uint32>	_BytesPushedIn;
...
public:
  ...
  /// Returns the number of connections handled by the thread (mutexed on
  /// _Connections)
   ...
   * Note: you must not call this method within a mutual exclusion on
   * _Connections, or there will be a deadlock (see clearClosedConnection())
   */
  void addToRemoveSet(TSockId sockid) {
    ...
    nlassert(sockid != InvalidSockId);
    {
      ...
      // - The set is locked by a receive thread which is removing the closed
      // connections.
      //   When the set gets unlocked, it is empty so the value is inserted. It
      //   means the value could be already in the set before it was cleared.
      //   Note: with a fonction such as tryAcquire(), we could avoid to enter
      //   the mutex when it is already locked
      ...
      // LNETL1_DEBUG( "LNETL1: ic: %p - RemoveSet.size(): %d", ic,
      // removesetsync.value().size() );
    }
#ifdef NL_OS_UNIX
    wakeUp();
#endif
  ...
  /// Delete all connections referenced in the remove list (mutexed on
  /// _RemoveSet and on _Connections)

...
struct TPrimitiveNamePredicate {
  TPrimitiveNamePredicate(const std::string &name) : Name(name) {}
...
struct TPrimitiveClassAndNamePredicate {
  TPrimitiveClassAndNamePredicate(const std::string &className,
                                  const std::string &name)
...
struct TPrimitivePropertyPredicate {
  TPrimitivePropertyPredicate(const std::string &propName,
 ...
 *	The primitive are tested recursively starting at an arbitrary node in
 *the primitive tree. Application code just need to call getNextMatch until it
 *return NULL indicating there is no more node that match the predicate.
 */
template <class Pred = TPrimitiveClassPredicate> class CPrimitiveEnumerator {
public:
  /** Construct a primitive enumerator.
   *	startPrim is the primitive where the enumeration start. Even if the
   *startPrimitive is not at the root of the primitive tree, the enumerator will
   *not try to parse the parent of startPrim. predicate is the functor
   *predicate. Each node is tested against the bool operator()(IPrimitive *)
   *	method of the predicate. If the predicate return true, then the node is
   *returned by getNextMatch.
   */
  CPrimitiveEnumerator(IPrimitive *startPrim, Pred &predicate)
      : _StartPrim(startPrim), _Predicate(predicate), _CurrentPrim(startPrim) {
 ...
 */
template <class Pred> class CPrimitiveSet {
public:
  void buildSet(IPrimitive *startPrimitive, Pred &predicate,
 ...
 */
template <class Pred> class CPrimitiveSetFilter {
public:
  void filterSet(const std::vector<IPrimitive *> &source, Pred &predicate,
                 TPrimitiveSet &result) {
    std::vector<IPrimitive *>::const_iterator first(source.begin()),
        last(source.end());
 ...
 */
inline bool loadXmlPrimitiveFile(CPrimitives &primDoc,
  ...
  try {
    ...
    // Read it
    return primDoc.read(xmlIn.getRootNode(),
                        NLMISC::CFile::getFilename(fileName).c_str(),
                        ligoConfig);
  } catch (const NLMISC::Exception &e) {
    nlwarning("Error reading input file '%s': '%s'", fileName.c_str(),
              e.what());
    return false;
  }
}
 ...
 */
inline bool saveXmlPrimitiveFile(CPrimitives &primDoc,
                                 const std::string &fileName) {
  try {
    ...
    nlwarning("Error writing output file '%s': '%s'", fileName.c_str(),
              e.what());
    return false;
  }
}
 ...
 */
template <class Pred>
void filterPrimitiveChilds(IPrimitive *parent, Pred &predicate,
                           TPrimitiveSet &result) {
  for (uint i = 0; i < parent->getNumChildren(); ++i) {
    IPrimitive *child;
    if (parent->getChild(child, i) && predicate(child))
...
void selectPrimByPath(IPrimitive *rootNode, const std::string &path,
                      TPrimitiveSet &result);

...
public:
  ...
  /** Build method
    ...
    * \param arrayTemplate is an array of ligo zone template pointer of size 9.
    If a pointer is NULL, checks will be done on non NULL pointer
    * but build will not be done.
    * \param arrayZone is an array of nel zone pointer of size 9. If a pointer
    is NULL, checks will be done but build will not be done.
    * \param config is the current lingo config file
    * \param errors is an array of error structure of size 9. One error
    structure by zone.
    * \return true if check success false if problem detected. Build is done if
    all the 18 pointers are not NULL.
    */
  bool build(const CMaterial &mat0, const CMaterial &mat1,
   ...
   * \param config is the current lingo config file
   * \param errors is an error handler filled with error code and message if the
   * method return false.
   *
   * \return true if check success false if problem detected. Errors are
   * reported in the error[0].
   */
  bool check(const CZoneTemplate &zoneTemplate, uint transitionNumber,
             const CLigoConfig &config, CLigoError &errors) const;

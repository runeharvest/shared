...
public:
  /// Build a edge zone
  bool build(const std::vector<NLMISC::CVector> &theEdge,
             const std::vector<uint32> &theId, uint rotation, sint32 offsetX,
             sint32 offsetY, const CLigoConfig &config, CLigoError &errors);
  ...
  /// Is the same edge ?
  bool isTheSame(const CZoneEdge &other, const CLigoConfig &config,
...
private:
  ...
  /// Rotation of the edge. Must be 0, 1, 2, 3. The rotation angle is Pi/2 *
  /// (double)_Rotation in CCW.
  uint32 _Rotation;

  /// X an Y offset of the edge. the position of the i-th vertex is rotate
  /// (_Rotation) * (theEdge[i], 0, 0) + (_OffsetX, _OffsetY, 0)
  sint32 _OffsetX;
  sint32 _OffsetY;
};

...
public:
  ...
  /** Load the config file. Can throw some exceptions if file doesn't exist or
   * is malformed.
   *
   * This file will try to open the file ligo class description file (XML) using
   * the LigoClass as file name. It will try first to load directly the file and
   * then to lookup the file in NLMISC::CPath.
   */
  bool readConfigFile(const std::string &fileName,
                      bool parsePrimitiveComboContent);

  /**
   * This file will read the file ligo class description file (XML) using the
   * LigoClass as file name. It will try first to load directly the file and
   * then to lookup the file in NLMISC::CPath.
   */
  bool readPrimitiveClass(const std::string &fileName,
                          bool parsePrimitiveComboContent);
  ...
  uint32 buildAlias(uint32 staticPart, uint32 dynamicPart,
                    bool warnIfOverload = true) const;
  /// register filename to static alias translation
  void registerFileToStaticAliasTranslation(const std::string &fileName,
                                            uint32 staticPart);
  ...
  getPrimitiveClass(const NLLIGO::IPrimitive &primitive) const;
  ...
  bool getPropertyString(std::string &result, const std::string &filename,
                         xmlNodePtr xmlNode, const std::string &propName);

  // Output error message
  void syntaxError(const std::string &filename, xmlNodePtr xmlNode,
                   const char *format, ...);
  ...
  const std::vector<CPrimitiveConfigurations> &
  getPrimitiveConfiguration() const {
    return _PrimitiveConfigurations;
  }

  // Update the DynamicAlias bit count that was previously defined in the config
  // file. The _StaticAliasFileMapping is updated in order that full alias stay
  // the same. All previous DynamicAlias must fit in the new
  // DynamicAliasBitCount

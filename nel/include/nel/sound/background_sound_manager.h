...
/// Number of background layer. Layer are identified in .prim by a letter
/// starting from 'a' (for layer 0)
 ...
 *
 * Sample bank patatoid are used to dynamicaly load/unload the sample banks will
 *the player walk the univers.
 *
...
public:
   ...
   *	deprecated
   */
  //	void		loadSamplesFromRegion(const NLLIGO::CPrimRegion
  //&region);
  ...
  /** Load background sound for a continent. It'll automatically unload the old
   * continent before loading the new one. This method load the 'audio' regions
   * (specifying the sounds), the 'effect' regions and the 'sample' regions.
   * Continent is for example "matis" or "fyros". It'll add .prim to the
   * continent name and lookup() to find zones. So, don't forget to add sound
   * .prim in the CPath system for the lookup With the new primitive file, this
   * method will try to load the .primitive file before attempting to load any
   * .prim file. If the .primitive is found, then no .prim are loaded.
   */
  ...
  /// Call this method to update the background sound (sub method of
  /// setListenerPosition)
...
private:
  ...
  // CAudioMixerUser will call private constructor and destructor, so, it is our
  // friend ;)
  ...
  void addSound(const std::string &soundName, uint layerId,
                const std::vector<NLLIGO::CPrimVector> &points, bool isPath);
  /// deprecated, Internal use only for loading.
  void addSound(const std::string &rawSoundName,
                const std::vector<NLLIGO::CPrimVector> &points, bool isPath);
  /// add a sample bank zone
  void addSampleBank(const std::vector<std::string> &bankNames,
                     const std::vector<NLLIGO::CPrimVector> &points);
  /// add a fx zone
  void addFxZone(const std::string &fxName,
                 const std::vector<NLLIGO::CPrimVector> &points);

  /// TODO : Utility... should be in NLMISC ?
  /*template <class CharType>
      std::vector<std::basic_string<CharType> >	split(const
  std::basic_string<CharType> &str, CharType splitTag)
  {
      std::vector<std::basic_string<CharType> >	splitted;
      std::basic_string<CharType>::size_type pos = 0, nextPos = 0, size = 0;

      while ((nextPos = str.find(splitTag, nextPos)) !=
  std::basic_string<CharType>::npos)
      {
          size = nextPos - pos;
          if (size > 0)
              splitted.push_back(std::basic_string<CharType>(str, pos, nextPos -
  pos));
          // skip the tag
          nextPos += 1;
          pos = nextPos;
  ...
  NLMISC::TTime
      _FilterFadesStart[UAudioMixer::TBackgroundFlags::NB_BACKGROUND_FLAGS];
  ...
  struct TSoundData {
    ...
    std::vector<NLMISC::CVector> Points;
    /// Flag telling if this sound is currently selected for play by bounding
    /// box.
    bool Selected;
  ...
  struct TSoundStatus {
    ...
    /** The relative gain of the source. This is used for patatoid
     * competition.when a smaller patatoid mute bigger one.
     */
    float Gain;
    ...
    bool Inside;
    /// Constructor.
    TSoundStatus(TSoundData &sd, NLMISC::CVector position, float gain,
                 float distance, bool inside)

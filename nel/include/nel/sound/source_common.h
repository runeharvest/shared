...
public:
  ...
  /// When groupController is NULL it will use the groupcontroller specified in
  /// the TSoundId. You should manually specify the groupController if this
  /// source is a child of another source, so that the parent source controller
  /// of the user-specified .sound file is the one that will be used.
  CSourceCommon(TSoundId id, bool spawn, TSpawnEndCallback cb,
  ...
  /// Called whenever the gain is changed trough setGain, setRelativeGain or the
  /// group controller's gain settings change.
  ...
  /// Tells this source not to call its callbacks when it ends. This is valid
  /// for spawned sources only.
  ...
  /// Get the final gain, including group controller changes. Use this when
  /// setting the physical source output gain.
  inline float getFinalGain() const {
    return _Gain * _GroupController->getFinalGain();
  }
  /// Get the pitch
  ...
  // void
  // set3DPositionVector( const NLMISC::CVector *pos )	{ _3DPosition = pos; }
  ...
  virtual void setFormat(uint8 /* channels */, uint8 /* bitsPerSample */,
                         uint32 /* frequency */) {
    nlassert(false);
  }
  /// Return the sample format information.
  virtual void getFormat(uint8 & /* channels */, uint8 & /* bitsPerSample */,
  ...
  /// over to hardware if needed. Set size to the number of bytes actually
  /// written to the buffer. Returns true if ok.
  virtual bool unlock(uint /* size */) {
    nlassert(false);
    return false;
  }
  /// Get the recommended buffer size to use with lock()/unlock()
  virtual void getRecommendedBufferSize(uint & /* samples */,
                                        uint & /* bytes */) const {
    nlassert(false);
  }

...
public:
  ...
  CSoundAnimation(std::string &name, TSoundAnimId id)
      : _Id(id), _Name(name), _Dirty(false) {}
  ...
  /** Returns whether the sound animation changed since the last save. (Editing
   * support) */
  ...
  /** Play the sounds of the markers that fall within the specified time
   * interval. */
  virtual void play(UAudioMixer *mixer, float lastTime, float curTime,

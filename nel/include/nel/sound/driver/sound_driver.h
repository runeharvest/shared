 ...
 *
 * The driver is a singleton. To access, only the pointer returned by
 * createDriver() is provided.
 *
 * \author Olivier Cado
...
public:
  ...
  enum TDriver {
    /// DriverAuto automatically picks the most awesome driver (picks first
    /// available in this order: FMod, OpenAl, XAudio2, DSound).
    DriverAuto = 0,
    /// DriverFMod is a driver that runs on FMod, nice quality, but not fully
    /// implemented.
    DriverFMod,
    /// DriverOpenAl is the recommended driver, especially if you have hardware
    /// sound acceleration.
    DriverOpenAl,
    /// DriverDSound is deprecated.
    DriverDSound,
    /// DriverXAudio2 runs on a fully software-based audio processing API
    /// without artificial limits.
    DriverXAudio2,
    NumDrivers
  };

  /// Driver creation flags, to configure behaviour, or check feature
  /// availability.
  enum TSoundOptions {
    /// Allow the user to use the ADPCM encoding. (verify availability with
    /// getOption)
     ...
     *		ISource::setAlpha() change the shape of attenuation (change the
     *curve) IListener::setRollOffFactor() will fail
     */
    OptionManualRolloff = 0x08,
    /// Enable local copy of buffer (used by OpenAL driver, required to build
    /// sample bank).
   ...
   * ESoundDriverNotFound, ESoundDriverCorrupted, ESoundDriverOldVersion,
   * ESoundDriverUnknownVersion
   */
  static ISoundDriver *createDriver(IStringMapperProvider *stringMapper,
                                    TDriver driverType = DriverAuto);
   ...
   * In case of failure, can throw one of these ESoundDriver exception objects:
   * ESoundDriverNotFound, ESoundDriverCorrupted, ESoundDriverOldVersion,
   *ESoundDriverUnknownVersion The driver instance should be deleted by the user
   *after init failure.
   *
   * You can request support for EAX. If EAX is requested and if there is
   *enougth hardware buffer replay, then only hardware buffer are created when
   *calling createBuffer. If the number of available hardware buffer is less
   *than 10, then EAX is ignored.
   *
   *	\param device If device.empty(), the default or most appropriate device
   *is used. \param options The features you want enabled. Verify after init()
   *with getOption() or getOptions().
   */
   ...
   */
  virtual bool getMusicInfo(const std::string & /* filepath */,
  ...
  /// implementation.
  virtual void
  getMusicExtensions(std::vector<std::string> &extensions) const = 0;
  /// Return if a music extension is supported by the driver's music channel.
  virtual bool
  isMusicExtensionSupported(const std::string &extension) const = 0;
  //@}
...
public:
  ESoundDriverCorrupted(const std::string &dllName)
      : ESoundDriver(
            std::string("Can't get NLSOUND_createISoundDriverInstance from ") +
            dllName + " (Bad dll?)") {}
};
...
/**
 * ESoundDriverNoEnvironmentEffects : ESoundDriverNotSupp : ESoundDriver :
 * NLMISC::Exception
 */
class ESoundDriverNoEnvironmentEffects : public ESoundDriverNotSupp {
public:
  ESoundDriverNoEnvironmentEffects()
      : ESoundDriverNotSupp("Environment effects are not supported by the "
...
public:
  ESoundDriverNoADPCM()
      : ESoundDriverNotSupp(
            "ADPCM is not supported by the current sound driver") {}
};

/**
 * ESoundDriverNoBufferStreaming : ESoundDriverNotSupp : ESoundDriver :
 * NLMISC::Exception
 */
...
public:
  ESoundDriverNoBufferStreaming()
      : ESoundDriverNotSupp(
            "Buffer streaming is not supported by the current sound driver") {}
};

/**
 * ESoundDriverNoManualRolloff : ESoundDriverNotSupp : ESoundDriver :
 * NLMISC::Exception
 */
class ESoundDriverNoManualRolloff : public ESoundDriverNotSupp {
public:
  ESoundDriverNoManualRolloff()
      : ESoundDriverNotSupp("Manual rolloff alpha is not supported by the "
                            "current sound driver") {}
};

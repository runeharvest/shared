...
public:
  ...
  ///// Add a buffer to the streaming queue.  A buffer of 100ms length is
  ///optimal for streaming. Buffers between 50ms and 100ms are optimal for
  ///streaming.
  ///// Should be called by a thread which checks countStreamingBuffers every
  ///50ms.Queue cleared on stop.
  ...
  virtual void
  setEffectFilter(TFilter filter,
                  float lowFrequency = NLSOUND_DEFAULT_FILTER_PASS_LF,
                  float highFrequency = NLSOUND_DEFAULT_FILTER_PASS_HF,
                  float passGain = NLSOUND_DEFAULT_FILTER_PASS_GAIN) = 0;
  ...
  virtual void setEffectFilterPassGain(
      float passGain = NLSOUND_DEFAULT_FILTER_PASS_GAIN) = 0;
...
protected:
  // common method used only with OptionManualRolloff. return the volume in
  // 1/100th DB  ( = mB)modified
  static sint32 computeManualRollOff(sint32 volumeMB, sint32 mbMin,
                                     sint32 mbMax, double alpha, float sqrdist,
                                     float distMin, float distMax);
  // common method used only with OptionManualRolloff. return the rolloff in
  // amplitude ratio (gain)
  static float computeManualRolloff(double alpha, float sqrdist, float distMin,
                                    float distMax);
};

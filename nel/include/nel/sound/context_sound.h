...
template <uint NbJoker, bool UseRandom, uint Shift = 5> struct CContextMatcher {
  // speudo constante
  enum {
    // Size of array : special case for 0 joker because we can't declare array
    // of 0 elements
    JOKER_ARRAY_SIZE = (NbJoker == 0 ? 1 : NbJoker)
  };

  CContextMatcher(uint32 *jokersValues, uint32 randomValue) : HashValue(0) {
    uint i;
    for (i = 0; i < NbJoker; ++i) {
  ...
  bool operator==(const CContextMatcher &other) const {
    if (HashValue != other.HashValue)
      ...
      return RandomValue == other.RandomValue &&
             memcmp(JokersValues, other.JokersValues,
                    sizeof(uint32) * NbJoker) == 0;
    else
      return memcmp(JokersValues, other.JokersValues,
                    sizeof(uint32) * NbJoker) == 0;
  }
  ...
  bool operator<(const CContextMatcher &other) const {
    ...
    int cmp =
        memcmp(JokersValues, other.JokersValues, sizeof(uint32) * NbJoker);
    if (cmp != 0)
      return cmp < 0;
  ...
  size_t getHashValue() const { return size_t(HashValue); }
...
#endif
  {
    ...
    bool operator()(const CContextMatcher &patternMatcher1,
                    const CContextMatcher &patternMatcher2) const {
...
public:
  ...
  virtual CSound *getSound(const CSoundContext &context,
                           uint32 randomValue) = 0;
  virtual void getSoundList(
...
class CContextSoundContainer : public IContextSoundContainer {
  ...
  typedef CHashMap<CContextMatcher<NbJoker, UseRandom, Shift>, CSound *,
                   typename CContextMatcher<NbJoker, UseRandom, Shift>::CHash>
      THashContextSound;

  virtual void init(uint *contextArgsIndex) {
    _MaxDist = 0;
    NLMISC::CFastMem::memcpy(_ContextArgsIndex, contextArgsIndex,
                             sizeof(uint) * NbJoker);
  }
  ...
  virtual void addSound(CSound *sound, const std::string &baseName) {
    const std::string &patternName =
        NLMISC::CStringMapper::unmap(sound->getName());
    ...
    std::string::const_iterator first(patternName.begin() + baseName.size()),
        last(patternName.end());
    //		std::string::const_iterator	first2(baseName.begin()),
    //last2(baseName.end());
    ...
    if (i != NbJoker)
      return;
    nlassertex(i == NbJoker,
               ("Error while adding sound '%s' into context sound container",
                NLMISC::CStringMapper::unmap(sound->getName()).c_str()));
    ...
    if (UseRandom) {
      ...
      if (!arg.empty()) {
        nlassertex(
            !ok, ("Error while adding sound '%s' into context sound container",
                  NLMISC::CStringMapper::unmap(sound->getName()).c_str()));
        // end of the argument.
        NLMISC::fromString(arg, randomValue);
        arg.clear();
    ...
    if (!ret.second) {
      ...
      nlassertex(it != _ContextSounds.end(),
                 ("Error wile adding soudn '%s' into context sound container",
                  NLMISC::CStringMapper::unmap(sound->getName()).c_str()));

      nlwarning("Sound %s has the same context matcher as the sound %s",
                NLMISC::CStringMapper::unmap(sound->getName()).c_str(),
                NLMISC::CStringMapper::unmap(it->second->getName()).c_str());
    }
  }
  ...
  void
  getSoundList(std::vector<std::pair<std::string, CSound *>> &subsounds) const {
    typename THashContextSound::const_iterator first(_ContextSounds.begin()),
        last(_ContextSounds.end());
    for (; first != last; ++first) {
      subsounds.push_back(
          std::make_pair(NLMISC::CStringMapper::unmap(first->second->getName()),
                         first->second));
    }
  }
...
private:
  ...
  /// The base name, that is the constante part of the name (before the first
  /// joker).

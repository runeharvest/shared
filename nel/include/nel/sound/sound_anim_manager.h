...
public:
  ...
  /** Start playing a sound animation. Returns an id number of this playback
   * instance or -1 if the animation was not found. \param name The id of the
   * animation to play.
   */
  virtual TSoundAnimPlayId playAnimation(TSoundAnimId id, float time,
                                         NL3D::CCluster *cluster,
                                         CSoundContext &context);

  /** Start playing a sound animation. Returns an id number of this playback
   * instance or -1 if the animation was not found. \param name The name of the
   * animation to play.
   */
  virtual TSoundAnimPlayId playAnimation(std::string &name, float time,
                                         NL3D::CCluster *cluster,
                                         CSoundContext &context);
  ...
  /** Play all the events of an animation in the interval between lastTime and
   * curTime. Both lastTime and curTime are measured relatively from the
   * beginning of the animation.
   */
  virtual void playAnimation(TSoundAnimId id, float lastTime, float curTime,
...
public:
    ESoundAnimationNotFound( const std::string filename ) :
      NLMISC::Exception( (std::string("Sound animation not found:
")+filename).c_str() ) {}
};
*/

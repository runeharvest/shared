 ...
 * TODO: Split IAudioDecoder into IAudioDecoder (actual decoding) and
 * IMediaDemuxer (stream splitter), and change the interface to make more sense.
...
public:
  ...
  /// Create a new music buffer, may return NULL if unknown type, destroy with
  /// delete. Filepath lookup done here. If async is true, it will stream from
  /// hd, else it will load in memory first.
  static IAudioDecoder *createAudioDecoder(const std::string &filepath,
                                           bool async, bool loop);

  /// Create a new music buffer from a stream, type is file extension like "ogg"
  /// etc.
  static IAudioDecoder *createAudioDecoder(const std::string &type,
                                           NLMISC::IStream *stream, bool loop);

  /// Get information on a music file (only artist and title at the moment).
  static bool getInfo(const std::string &filepath, std::string &artist,
                      std::string &title, float &length);

  /// Get audio/container extensions that are currently supported by the nel
  /// sound library.
  ...
  /// minimum if at end).
  virtual uint32 getNextBytes(uint8 *buffer, uint32 minimum,
                              uint32 maximum) = 0;

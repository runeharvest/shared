 ...
 *
 * The logical sources (_Sources) are the sources representing all entities in
 * the world, from the client's point of view. The tracks (_Tracks) are the
 * physical sources played by the sound driver. Their number is small.
 *
...
class CAudioMixerUser : public UAudioMixer,
                        public ISoundDriver::IStringMapperProvider,
                        public NLMISC::CManualSingleton<CAudioMixerUser> {
public:
  /// Constructor
  CAudioMixerUser();
  ...
  virtual void initDevice(const std::string &deviceName,
                          const CInitInfo &initInfo,
                          NLMISC::IProgressCallback *progressCallback = NULL);

  virtual void initClusteredSound(NL3D::UScene *uscene, float minGain,
                                  float maxDistance, float portalInterpolate);
   ...
   *	A priority channel can orverflow it's reserve value only if the low
   *water mark is not reach. In other word, when the number of played source
   *increase, you can control a 'smooth' cut in priority layer. The idea is to
   *try to keep some free track for the HighestPri source. By default, reserve
   *are set for each channel to the number of available tracks.
   */
  virtual void setPriorityReserve(TSoundPriority priorityChannel, uint reserve);
  /** Set the Low water mark.
   *	This value is use to mute sound source that try to play when there
   ...
   *default, the value is set to 0, witch mean no special treatment is done and
   *the mixer will mute sound with no user control at all. Note also that the
   *availability of a track is not guarantie if the sum of the priority reserve
   *(see setPriorityReserve) is grater than the number od available tracks
   *(witch is almos alwais the case). But this value will help the mixer make
   *it's best.
   */
  ...
  //	virtual void				loadEnvEffects( const char
  //*filename );

  void buildSampleBankList();
  bool useAPDCM() { return _UseADPCM; };
  /** Load buffers. Returns the number of buffers successfully loaded.
   * If you specify a non null notfoundfiles vector, it is filled with the names
   * of missing files if any. You can call this method several times, to load
   * several sound banks.
   */
  virtual uint32 loadSampleBank(bool async, const std::string &name,
  ...
  //	virtual void				loadSoundBank( const std::string
  //&path );

  // Load environment sounds ; treeRoot can be null if you don't want an access
  // to the envsounds
  //	virtual	void				loadEnvSounds( const char
  //*filename, UEnvSound **treeRoot=NULL );
  ...
  virtual USource *createSource(const NLMISC::TStringId &name,
                                ...
                                UGroupController *groupController = NULL);
  ...
  /** Delete a logical sound source. If you don't call it, the source will be
   * auto-deleted when deleting the audio mixer object
   */
  virtual void removeSource(CSourceCommon *source);

  /// Put source into a track
  //	void						giveTrack( CSimpleSource
  //*source );
  /// Release track
  //	void						releaseTrack(
  ...
  //	CEnvSoundUser				*getEnvSounds()
  //{ return _EnvSounds; }
  /// Return the listen pos vector
  const NLMISC::CVector &getListenPosVector() const { return _ListenPosition; }
  /** Same as removeSource() but does not delete the object (e.g. when not
   * allocated by new, as the CAmbiantSource channels)
   */
  //	void						removeMySource( USource
  //*source );
  /// Add ambiant sound pointer for later deletion
  //	void						addAmbiantSound( CSound
  //*sound )		{ _AmbSounds.insert( sound ); }
  // Allow to load sound files (nss) when the corresponding wave file is missing
  // (see CSound) static void
  // allowMissingWave( bool b )				{
  // CSound::allowMissingWave( b ); }
  ...
  CBackgroundSoundManager *getBackgroundSoundManager() {
    return _BackgroundSoundManager;
  }
  ...
  //	virtual void				loadBackgroundSoundFromRegion
  //(const NLLIGO::CPrimRegion &region); 	virtual void
  //loadBackgroundEffectsFromRegion (const NLLIGO::CPrimRegion &region); 	virtual
  //void				loadBackgroundSamplesFromRegion (const
  //NLLIGO::CPrimRegion &region);
  virtual void
  loadBackgroundAudioFromPrimitives(const NLLIGO::IPrimitive &audioRoot);
  ...
  //	virtual void				setBackgroundSoundDayNightRatio
  //(float ratio) { CBackgroundSoundManager::setDayNightRatio(ratio); }

  /// Return the sound driver.
  ISoundDriver *getSoundDriver();
  inline CSoundBank *getSoundBank() { return _SoundBank; }
  inline CSampleBankManager *getSampleBankManager() {
    return _SampleBankManager;
  }
  ...
  void
  setBackgroundFilterFades(const TBackgroundFilterFades &backgroundFilterFades);
  const TBackgroundFlags &getBackgroundFlags();
  const TBackgroundFilterFades &getBackgroundFilterFades();

  //	bool						setPlaying(CSimpleSource
  //*source); 	void
  //unsetPlaying(CSimpleSource *source);
  ...
  ///// Get a free track without a source! Steal one if if you want when no
  ///tracks are available! Used by music channel, etc.
  ...
  CMusicSoundManager *getBackgroundMusicManager() const {
    return _BackgroundMusicManager;
  }
  // Event music
  ...
  inline bool useOcclusionObstruction() const {
    return m_EnableOcclusionObstruction;
  }

  //@{
  //\name Reverb environment functions
  /// Add a reverb environment.
  void addEnvironment(const std::string &name,
  ...
  /// Set the current reverb environment.
  inline void setEnvironment(const std::string &environmentName,
                             float roomSize) {
    setEnvironment(NLMISC::CStringMapper::map(environmentName), roomSize);
  }
  /// Get a reverb environment
  const IReverbEffect::CEnvironment &
  getEnvironment(NLMISC::TStringId environmentName);
  /// Get a reverb environment
  inline const IReverbEffect::CEnvironment &
  getEnvironment(const std::string &environmentName) {
    return getEnvironment(NLMISC::CStringMapper::map(environmentName));
  }
  //@}

private:
  enum TMusicChannel { GeneralMusicChannel = 0, EventMusicChannel = 1 };
  bool playMusicChannel(TMusicChannel chan, const std::string &fileName,
...
public:
  ...
  void removeUserControledSource(CSourceCommon *source,
...
private:
  ...
  typedef CHashSet<IMixerUpdate *, THashPtr<IMixerUpdate *>>
      TMixerUpdateContainer;
  typedef CHashMap<IBuffer *, std::vector<class CSound *>, THashPtr<IBuffer *>>
      TBufferToSourceContainer;
  //	typedef std::multimap<NLMISC::TTime, IMixerEvent*>
  //TTimedEventContainer;
  typedef std::multimap<NLMISC::TTime, NLMISC::CDbgPtr<IMixerEvent>>
      TTimedEventContainer;
  typedef std::multimap<IMixerEvent *, TTimedEventContainer::iterator>
      TEventContainer;
...
protected:
  ...
  /// List of event ordered by event ptr with there respective multimap
  /// iterator.
  ...
  typedef CHashMap<NLMISC::TStringId, CControledSources,
...
private:
  ...
  /// The list of non discardable sound to play as soon as possible in order of
  /// arrival.
  ...
  /// Low water mark. After this number of free voice is reach, reserve can't be
  /// overloaded.
  ...
  std::string
      _BackgroundFilterShortNames[TBackgroundFlags::NB_BACKGROUND_FLAGS];
  ...
  typedef std::map<NLMISC::TStringId, IReverbEffect::CEnvironment>
      TEnvironments;
...
public:
  struct TSampleBankHeader {
    ...
    TSampleBankHeader() { Version = sample_bank_header_version; }

    void addSample(const std::string &name, uint32 frequency, uint32 nbSample,
                   uint32 sizeMono16, uint32 sizeAdpcm) {
      Name.push_back(name);
...
public: // Temp (EDIT)
  ...
  uint32 curTime() {
    return (uint32)(NLMISC::CTime::getLocalTime() - _StartTime);
  }

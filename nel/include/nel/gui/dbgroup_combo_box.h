...
#ifdef RYZOM_LUA_UCSTRING
  REFLECT_UCSTRING("view_text", getViewTextAsUtf16,
                   setViewTextAsUtf16) // Compatibility
#else
...
protected:
  ...
  bool _LinkedToDB; // if not linked to db, then _NotLinkedToDBSelection is used
                    // instead
  bool _Setuped;

...
//================================================================================
inline CLuaStackChecker::CLuaStackChecker(CLuaState *state,
...
#else
  nlassert((index != 0 && abs(index) <= getTop()) ||
           index == LUA_REGISTRYINDEX || index == LUA_GLOBALSINDEX);
#endif
}
...
inline void CLuaState::setTop(int index) {
  ...
  //  if index is strictly negative, this is a "pop". ensure that the user don't
  //  pop more than allowed
  if (index < 0) {
    checkIndex(index);
  }

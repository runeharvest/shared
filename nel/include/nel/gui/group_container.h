...
public:
  ...
  // Add a big delta so when the user is over the Resizer, always take it
  // whatever other controls under
  virtual uint getDeltaDepth() const { return 100; }

  // get real resizer pos : if parent has pop_min_w == pop_max_w, then
  // horizontal resizer will be discarded
  //                        if parent has pop_min_h == pop_max_h, then vertical
  //                        resizer will be discarded
 ...
 * If the container is popable, it will first pull it of the hierarchy, then it
 * becomes movable. It can also be used to change the position of a group
 * container that is inserted in the list of another container. \author Lionel
 * Berenguier \date 2003
 */
...
private:
  void setPoped(CGroupContainer *gc, sint32 x, sint32 y,
                const NLGUI::CEventDescriptorMouse &eventDesc);
  void setMovingInParent(CGroupContainer *gc, sint32 x, sint32 y,
                         const NLGUI::CEventDescriptorMouse &eventDesc);
...
public:
  // observer to know when children have moved. This can be used to keep
  // external datas in sync
  struct IChildrenObs {
    virtual void childrenMoved(uint srcIndex, uint destIndex,
                               CGroupContainer *children) = 0;
  };
...
public:
  ...
  virtual bool getViewsUnder(
      sint32 x, sint32 y, sint32 clipX, sint32 clipY, sint32 clipW,
      sint32 clipH,
      std::vector<CViewBase *> &vVB); // Return true if x,y under the group
  ...
  void setMovableInParentList(bool /* movable */) {
    _MovableInParentList = true;
  }
  bool isMovableInParentList() const { return _MovableInParentList; }

  // high light the border of the container
  void setHighLighted(bool hightlighted, uint8 alpha = 255) {
    _HighLighted = hightlighted;
...
#ifdef RYZOM_LUA_UCSTRING
  REFLECT_UCSTRING("uc_title_opened", getUCTitleOpened,
                   setUCTitleOpened); // Compatibility
  REFLECT_UCSTRING("uc_title_closed", getUCTitleClosed,
                   setUCTitleClosed);                   // Compatibility
  ...
  REFLECT_BOOL("right_button_enabled", getRightButtonEnabled,
  ...
  void popup();
  /** Popin the window and possibly put it back in its father container, using
   * the order defined in the list of the container. \param putBackInFather When
   * true, put the window back in its former father container, otherwise, the
   * container is unliked from the hierachy (parents are NULL) \param insertPos
   * If this is equal to -1, then the window is inserted at its previous
   * position. Otherwise it is inserted before the given position in the list
   */
  ...
  /** Force the container to blink (to tell the user that an event has
   * happened). This  uses the global color, so the container must use it This
   * state is automatically disabled if the container is opened \param numBlinks
   * 0 If the container should blink endlessly, the number of blink otherwise
   */
  ...
  // Get current father container (if any). If the container is popup, it gives
  ...
  // force the rollover alpha to its max value, depending on there's keyboard
  // focus or not
  ...
  static void validateCanDeactivate(bool validate) {
    _ValidateCanDeactivate = validate;
  }
  const std::string &getAHOnDeactiveCheck() const {
    return CAHManager::getInstance()->getAHName(_AHOnDeactiveCheck);
  }
  const std::string &getAHOnDeactiveCheckParams() const {
    return _AHOnDeactiveCheckParams;
  }
  //
  const std::string &getAHOnCloseButton() const {
    return CAHManager::getInstance()->getAHName(_AHOnCloseButton);
  }
  const std::string &getAHOnCloseButtonParams() const {
    return _AHOnCloseButtonParams;
  }
  //
  IActionHandler *getAHOnMovePtr() const { return _AHOnMove; }
  const std::string &getAHOnMove() const {
    return CAHManager::getInstance()->getAHName(_AHOnMove);
  }
  const std::string &getAHOnMoveParams() const { return _AHOnMoveParams; }
  //
  IActionHandler *getAHOnResizePtr() const { return _AHOnResize; }
  const std::string &getAHOnResize() const {
    return CAHManager::getInstance()->getAHName(_AHOnResize);
  }
  ...
  const std::string &getAHOnBeginMove() const {
    return CAHManager::getInstance()->getAHName(_AHOnBeginMove);
  }
  const std::string &getAHOnBeginMoveParams() const {
    return _AHOnBeginMoveParams;
  }

  //
  void setOnCloseButtonHandler(const std::string &h) {
    _AHOnCloseButton =
        CAHManager::getInstance()->getAH(h, _AHOnCloseButtonParams);
  }
  void setOnCloseButtonParams(const std::string &p) {
    _AHOnCloseButtonParams = p;
  }

  void setModalParentList(const std::string &name);
  bool checkIfModal(const NLGUI::CEventDescriptor
                        &event); // Return true if we can handle the event (and
                                 // prevent from selecting a window)
  bool isGrayed() const;
  bool blinkAllSons();
  ...
  // check if the container has been moved, resized, or popuped by the user (and
  // eventually clear that flag)
  ...
  // return true if the container has a modal parent window setuped => the whole
  // interface cannot switch desktop
  ...
  // set the help web page of this container. "" if none. NB: the help button is
  // not updated
...
protected:
  ...
  CViewBitmap *_ViewOpenState; // Arrow showing if we are opened or not (if we
                               // are openable)
  ...
  std::string _ModalParentNames; // Modal handling between container (container
                                 // can be linked together,
  std::vector<CGroupContainer *>
      _ModalSons; // when the son is active the parent is not active
  std::vector<CGroupContainer *>
      _ModalParents; // (but the rest of the interface is))
  ...
  // old position at which the window was popup, -1 values means that the window
  // hasn't been turned into a popup yet
  sint32 _PopupX;
  sint32 _PopupY;
  sint32 _PopupW;
  sint32 _PopupH;
  ...
  // action handler to test whether the windows can be deactivated (when the
  ...
  bool _OpenWhenPopup : 1; // Does the container must open when poped up ?
                           // (layer>0) and close when poped in...
  bool _OpenAtStart : 1;       // Mgt : to setup _Opened state at start
  bool _OpenedBeforePopup : 1; // Mgt : Is the container opened before poped up
                               // ? (layer>0)
  ...
  bool _Modal : 1; // the container is modal and prevent from switching virtual
                   // desktop
  //
  ...
  // Special Top Resizer Height (for Inventory and ChatGroup). <0 (default) =>
  // take default option value
...
private:
  ...
  void createResizer(uint index, THotSpot posRef, THotSpot type, sint32 offsetX,
                     sint32 offsetY, bool bMaxH);
  ...
  COptionsLayer *getContainerOptions(
      sint32 ls = -1); // Depends if overload by OptionsName or default used
...
public:
  ...
  CGroupList *getPreviousParentList() const {
    return _OldFatherContainer ? _OldFatherContainer->_List : NULL;
  }
  CCtrlScroll *getPreviousParentScrollBar() const {
    return _OldFatherContainer ? _OldFatherContainer->_ScrollBar : NULL;
  }

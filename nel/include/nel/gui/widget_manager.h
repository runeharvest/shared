...
public:
  /// Interface for event handlers that can be called when the screen is
  /// resized.
  ...
  /// Interface for event handlers that can be called when the widgets finished
  /// drawing.
  ...
  // Interface for event handlers that can be called when widgets are added or
  // moved
  ...
  public:
    ...
    virtual void onWidgetMoved(const std::string &oldid,
  ...
  struct SModalWndInfo {
    // Yoyo: store as CRefPtr in case they are deleted (can happen for instance
    // if menu right click on a guild memeber, and guild members are udpated
    // after)
  ...
  void addWindowToMasterGroup(const std::string &sMasterGroupName,
                              CInterfaceGroup *pIG);
  void removeWindowFromMasterGroup(const std::string &sMasterGroupName,
                                   CInterfaceGroup *pIG);
  ...
  CInterfaceElement *getElementFromId(const std::string &sStart,
                                      const std::string &sEltId);

  /**
   * get a window from its  Id of its group.
   *	NB: "ctrl_launch_modal" is a special Id which return the last ctrl which
   *has launch a modal. NULL if modal closed. \param groupId : the Id of the
   *window group
   */
  /// get an element from a define ID. shortcut for
  /// getElementFromId(getDefine(define))
  ...
  void enableModalWindow(CCtrlBase *ctrlLaunchingModal,
  ...
  void pushModalWindow(CCtrlBase *ctrlLaunchingModal,
                       const std::string &groupName);
  void popModalWindow();
  // pop all top modal windows with the given category (a string stored in the
  // modal)
  ...
  void setCurContextHelp(CCtrlBase *curContextHelp) {
    this->curContextHelp = curContextHelp;
  }
  ...
  const std::vector<CViewBase *> &getViewsUnderPointer() {
    return _ViewsUnderPointer;
  }
  const std::vector<CInterfaceGroup *> &getGroupsUnderPointer() {
    return _GroupsUnderPointer;
  }
  const std::vector<CCtrlBase *> &getCtrlsUnderPointer() {
    return _CtrlsUnderPointer;
  }
  ...
  // Internal : adjust a tooltip with respect to its parent. Returns the number
  // of coordinate that were clamped against the screen border
  uint adjustTooltipPosition(CCtrlBase *newCtrl, CInterfaceGroup *win,
                             THotSpot ttParentRef, THotSpot ttPosRef,
                             sint32 xParent, sint32 yParent, sint32 wParent,
                             sint32 hParent);

  void updateTooltipCoords();

  // Update tooltip coordinate if they need to be (getInvalidCoords() returns a
  // value != 0)
  ...
  void getNewWindowCoordToNewScreenSize(sint32 &x, sint32 &y, sint32 w,
                                        sint32 h, sint32 newW,
                                        sint32 newH) const;

  // move windows according to new screen size
  void moveAllWindowsToNewScreenSize(uint32 newScreenW, uint32 newScreenH,
                                     bool fixCurrentUI);
  ...
  // NB: setCaptureKeyboard(NULL) has not the same effect as
  // resetCaptureKeyboard(). it allows the capture to come back to the last
  // captured window (resetCaptureKeyboard() not)
  ...
  // register a view that wants to be notified at each frame (receive the msg
  // 'clocktick')
  ...
  NLMISC::CRGBA getGlobalColorForContent() const {
    return _GlobalColorForContent;
  }
  void setGlobalColorForContent(NLMISC::CRGBA col) {
    _GlobalColorForContent = col;
  }
   ...
   */
  const CInterfaceOptionValue &getSystemOption(TSystemOption o) const {
    return _SystemOptions[o];
  }

  // @}

  CInterfaceElement *getOverExtendViewText() { return _OverExtendViewText; }
  NLMISC::CRGBA &getOverExtendViewTextBackColor() {
    return _OverExtendViewTextBackColor;
  }

  // For single lined ViewText that are clipped: on over of viewText too big,
  // the text is drawn on top. A CRefPtr is kept
  ...
  uint8 getGlobalRolloverFactorContent() const {
    return _GlobalRolloverFactorContent;
  }
  uint8 getGlobalRolloverFactorContainer() const {
    return _GlobalRolloverFactorContainer;
  }
  ...
  void updateInterfaceTimes(const SInterfaceTimes &times) {
    interfaceTimes = times;
  }

  void setIngame(bool i) { inGame = i; }
  bool isIngame() const { return inGame; }

  void setScreenWH(uint32 w, uint32 h) {
    _ScreenW = w;
    _ScreenH = h;
  }
  ...
  // execute a procedure. give a list of parameters. NB: the first param is the
  // name of the proc (skipped)...
  void runProcedure(const std::string &procName, CCtrlBase *pCaller,
                    const std::vector<std::string> &paramList);
  // replace an action in a procedure (if possible)
  void setProcedureAction(const std::string &procName, uint actionIndex,
  ...
  CInterfaceElement *addWidgetToGroup(std::string &group,
...
private:
  ...
  NLMISC::CRefPtr<CInterfaceElement>
      draggedElement; // the element that we're currently dragging

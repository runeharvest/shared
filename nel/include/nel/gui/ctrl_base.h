...
public:
  ...
  enum TToolTipParentType {
    TTMouse =
        0, // The tooltip is displayed relatively to the mouse when it appears
    TTCtrl = 1, // The tooltip is displayed relatively to the ctrl it comes from
                // when it apeears
    TTWindow = 2, // The tooltip is displayed relatively to the window where the
                  // control lies.
    TTSpecialWindow =
        3, // The tooltip is displayed relatively to a special user window
...
public:
...
#endif
  /// Get the ContextHelp for this control, with tooltip specific code. Default
  /// behaviour is identical to getContextHelp.
  ...
  const std::string &getContextHelpActionHandler() const {
    return _OnContextHelp;
  }
  /// Get the ContextHelp ActionHandler Params
  const std::string &getContextHelpAHParams() const {
    return _OnContextHelpParams;
  }
  ...
  const std::string &getToolTipSpecialParent() const {
  ...
  void setToolTipSpecialParent(const std::string &parent) {
    _ToolTipSpecialParent = parent;
  }
  /// Get the ToolTip pos references (parent relevant only if
  /// getToolTipParent()!=TTMouse)
  ...
  /// Set the ToolTip pos references (parent relevant only if
  /// getToolTipParent()!=TTMouse)
  ...
  void setDefaultContextHelpAsUtf16(const ucstring &help) {
    _ContextHelp = help.toUtf8();
  } // Compatibility
#endif
  void setOnContextHelp(const std::string &help) { _OnContextHelp = help; }
  void setOnContextHelpAHParams(const std::string &p) {
    _OnContextHelpParams = p;
  }
  ...
  /** test if virtual desktop change is possible while this element is captured
   * by the mouse Useful for resizers
   */
...
#ifdef RYZOM_LUA_UCSTRING
  // 'tooltip' property expects string to be ucstring or latin1 which is not
  // possible from html page
  int luaSetTooltipUtf8(CLuaState &ls); // Compatibility
#endif

  REFLECT_EXPORT_START(CCtrlBase, CViewBase)
#ifdef RYZOM_LUA_UCSTRING
  REFLECT_UCSTRING("tooltip", getDefaultContextHelpAsUtf16,
                   setDefaultContextHelpAsUtf16);          // Compatibility
  ...
  // special for mouse over : return true and fill the name of the cursor to
  // display
...
protected:
  void convertTooltipHotSpot(const char *prop, THotSpot &parentHS,
                             THotSpot &childHS);

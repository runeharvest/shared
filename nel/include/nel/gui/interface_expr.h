...
public:
  ...
  void setRGBA(NLMISC::CRGBA value) {
    clean();
    _Type = RGBA;
    _RGBAValue =
        (uint32)(value.R + (value.G << 8) + (value.B << 16) + (value.A << 24));
  }
...
private:
  ...
  std::string _StringValue; // well, can't fit in union, unless we do some
                            // horrible hack..
private:
...
/**
 * Base class for user defined types that are use by the 'CInterfaceExprValue'
 * class Derivers should include the 'clone' method
 *
 ...
 *
 * - a string : 'toto', 'abcd', 'a\nbcd', 'a\\t', the escape sequences are the
 * one of C
 * - a integer 1, 2, 3
 * - a double  1.1, 2.2
 * - a database entry : @ui:interface:toto:truc. If the address is a leaf, it
 * returns the leaf value and put an observer on it. If not a leaf, it returns
 * 0, but put an observer on it.
 * - a database indirection @db:value[db:index] is replaced by @db:value0 if
 * db:index == 0 for example
 * - a user function call : fct(expr0, epxr1, ...).
 *
 * NB : The lua language has been integrated since then (2005), and should be
 * more suited for most of the tasks.
 *
 *
 * \author Nicolas Vizerie
 * \author Nevrax France
...
public:
   ...
   * It should return true if the result is meaningful. If not, the rest of the
...
public:
   ...
   * - This eventually fill a vector with a set of database entries it has
   * dependencies on \param expr The expression to evaluate \param result The
   * result value \param nodes If not NULL, will be filled with the database
   * nodes this expression depends on Node will only be inserted once, so we end
   * up with a set of node (not ordered) \param noFctCalls when set to true, the
   * terminal function calls will not be made, so the evaluation is only used to
   * see which database entries the expression depends on.
   */
  static bool eval(const std::string &expr, CInterfaceExprValue &result,
                   std::vector<NLMISC::ICDBNode *> *nodes = NULL,
                   bool noFctCalls = false);

  /** Build a tree from the given expression so that it can be evaluated
   * quickly. This is useful for a fixed expression that must be evaluated often
   */
...
private:
   ...
   */
  static const char *evalExpr(const char *expr, CInterfaceExprValue &result,
                              std::vector<NLMISC::ICDBNode *> *nodes,
                              bool noFctCalls);
  static const char *evalFct(const char *expr, CInterfaceExprValue &result,
                             std::vector<NLMISC::ICDBNode *> *nodes,
                             bool noFctCalls);
  static const char *evalDBEntry(const char *expr, CInterfaceExprValue &result,
                                 std::vector<NLMISC::ICDBNode *> *nodes);

public:
  static const char *unpackDBentry(const char *expr,
                                   std::vector<NLMISC::ICDBNode *> *nodes,
                                   std::string &dest,
   ...
   */
private:
  static const char *buildExprTree(const char *expr,
                                   CInterfaceExprNode *&result);
  static const char *buildFctNode(const char *expr,
                                  CInterfaceExprNode *&result);
  static const char *buildDBEntryNode(const char *expr,
                                      CInterfaceExprNode *&result);
};
...
#define REGISTER_INTERFACE_USER_FCT(name, fct)                                 \
  const struct __InterUserFctRegister__##fct {                                 \
    __InterUserFctRegister__##fct() {                                          \
      CInterfaceExpr::registerUserFct(name, fct);                              \
    }                                                                          \

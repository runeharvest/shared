...
// ***************************************************************************
/** Helper class to see if a stack is restored at its initial size (or with n
 * return results). Check that the stack size remains unchanged when the object
 * goes out of scope
 */
...
// **************************************************************************
/** Helper class to restore the lua stack to the desired size when this object
 * goes out of scope
 */
...
public:
  ...
  virtual ~ELuaError() NL_OVERRIDE {
    CLuaStackChecker::decrementExceptionContextCounter();
  }
  ELuaError(const std::string &reason) : Exception(reason) {
    CLuaStackChecker::incrementExceptionContextCounter();
  }
  // what(), plus append the Reason
  virtual std::string luaWhat() const throw() {
    return NLMISC::toString("LUAError: %s", what());
  }
};
...
public:
  ...
  // what(), plus append the Reason
  virtual std::string luaWhat() const throw() NL_OVERRIDE {
    return NLMISC::toString("ELuaParseError: %s", what());
  }
};

/** Exception thrown when something went wrong inside a wrapped function called
 * by lua
 */
...
public:
  ...
  // what(), plus append the Reason
  virtual std::string luaWhat() const throw() NL_OVERRIDE {
    return NLMISC::toString("ELuaExecuteError: %s", what());
  }
};
...
public:
  ...
  // what(), plus append the Reason
  virtual std::string luaWhat() const throw() {
    return NLMISC::toString("ELuaBadCast: %s", what());
  }
};
...
public:
  ...
  // what(), plus append the Reason
  virtual std::string luaWhat() const throw() {
    return NLMISC::toString("ELuaNotATable: %s", what());
  }
};
...
public:
   ...
   *	\throw ELuaParseError
   *	\param dbgSrc is a string for debug. Should be a filename (preceded with
   *'@'), or a short script.
   */
  void loadScript(const std::string &code, const std::string &dbgSrc);

  /** Execute a script from a string, possibly throwing an exception if there's
   *a parse error \throw ELuaParseError, ELuaExecuteError
   */
  void executeScript(const std::string &code, int numRet = 0);

  /** Execute a script from a string. If an errors occurs it is printed in the
   *log \return true if script execution was successful
   */
   ...
   *	It is different from doString() in such there is a cache (where the key
   *is the script itself) so that the second time this script is executed, there
   *is no parsing Note: I experienced optim with about 10 times faster than a
   *executeScript() on a simple "a= a+1;" script \throw ELuaParseError,
   *ELuaExecuteError
   */
  ...
  void
  pushValue(int index);   // copie nth element of stack to the top of the stack
  void remove(int index); // remove nth element of stack
  void insert(
      int index); // insert last element of the stack before the given position
  void replace(
      int index); // replace nth element of the stack with the top of the stack
  ...
  void
  toString(int index,
           std::string &str); // convert to a std::string, with a NULL check.
  ...
  /** Helper functions : get value of the wanted type in the top table after
   * conversion A default value is used if the stack entry is NULL. If
   * conversion fails then an exception is thrown (with optional msg)
   */
  ...
  const char *getTableStringValue(const char *name,
  ...
  void pushLightUserData(void *); // push a light user data (use newUserData to
                                  // push a full userdata)
  // metatables
  bool getMetaTable(int index = -1);
  bool setMetaTable(int index = -1); // set the metatable at top of stack to the
                                     // object at 'index' (usually -2), then pop
                                     // the metatable even if asignment failed
  // comparison
  ...
  void getTable(
      int index); // get value from a table at index 'index' (key is at top)
  void rawGet(int index);
  void setTable(int index); // set (key, value) from top of the stack into the
                            // given table both key and value are poped
  ...
  /** Helper : Execute a function by name. Lookup for the function is done in
   * the table at the index 'funcTableIndex' the behaviour is the same than with
   * call of pcall.
   */
  int pcallByNameGlobal(const char *functionName, int nargs, int nresults,
                        int errfunc = 0);
  int pcallByName(const char *functionName, int nargs, int nresults,
                  int funcTableIndex, int errfunc = 0);

  // push a C closure (pop n element from the stack and associate with the
  // function)
  ...
  /// \name Misc
  // @{
  /** Retrieve pointer to a CLuaState environment from its lua_State pointer, or
   * NULL if there no such environment
   */
  static CLuaState *fromStatePointer(lua_State *state);
  // Get state pointer. The state should not be closed (this object has
  // ownership)
  ...
  void setGCThreshold(
      int kb); // set max memory in KB (no-op with ref-counted version)

  // handle garbage collector for ref-counted version of lua (no-op with
  // standard version, in which case gc handling is automatic)
  void handleGC();
   ...
   *		2 ....
   *	NB: if called from a C function called from LUA, remember that
   *stackLevel 0 is the current function. Hence if you want to know what LUA
   *context called you, pass stackLevel=1! \param ret string cleared if any
   *error, else filled with formated FileName / LineNumber
   */
...
private:
  ...
  int _GCThreshold; // if refcounted gc is used, then garbage collector is
                    // handled manually
#endif
  // Small Script Cache
...
private:
  ...
  CLuaState(const CLuaState & /* other */) : NLMISC::CRefCount() {
    nlassert(0);
  }
  ...
  void executeScriptInternal(const std::string &code, const std::string &dbgSrc,
                             int numRet = 0);
  int pcallByNameInternal(const char *functionName, int nargs, int nresults,
                          int errfunc, int initialStackSize);
};

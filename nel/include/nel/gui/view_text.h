...
public:
  ...
  /// Constructor
  CViewText(const std::string &id, const std::string Text = "",
            sint FontSize = 12,
            NLMISC::CRGBA Color = NLMISC::CRGBA(255, 255, 255),
            bool Shadow = false, bool ShadowOutline = false);
  ...
  void setMultiLineClipEndSpace(
      bool state); // use it for multiline edit box for instance
  ...
  void getShadowOffset(sint &x, sint &y) {
    x = _ShadowX;
    y = _ShadowY;
  }
  ...
  /** Get position of the ith character, position are relative to the BR corner
   * of the text. \param lineEnd. When set to true, return the coordinate of the
   * previous line if the index is at the start of a line. When looking at
   * standard edit box, we see that if a line is split accross to line with no
   * This also returns the height of the line
   */
  void getCharacterPositionFromIndex(sint index, bool lineEnd, float &x,
                                     float &y, float &height) const;
  /** From a coordinate relative to the BR BR corner of the text, return the
   * index of a character. If no character is found at the given position, the
   * closest character is returned (first or last character, for the line or the
   * whole text)
   */
  void getCharacterIndexFromPosition(float x, float y, uint &index,
                                     bool &lineEnd) const;
  /** From a character index, get the index of the line it belongs to, or -1 if
   * the index is invalid \param cursorDisplayedAtEndOfPreviousLine true if the
   * cursor is displayed at the end of the previous line that match its index
   */
  sint getLineFromIndex(uint index,
                        bool cursorDisplayedAtEndOfPreviousLine = true) const;
  ...
  /// If localized, return localization key (ie "uiLanguage"), else return
  /// displayed text.
  std::string getHardText() const {
    return _HardText.empty() ? _Text : _HardText;
  }
  void
  setHardText(const std::string &ht); //< Localizes strings starting with "ui"
#ifdef RYZOM_LUA_UCSTRING
  ...
  /** Setup a Text with Format Tags. Text is store without color/format tags,
   * and special array is allocated for Format association
   */
...
#ifdef RYZOM_LUA_UCSTRING
  void
  setSingleLineTextFormatTagedAsUtf16(const ucstring &text); // Compatibility
#endif
  ...
  void setOverExtendViewTextUseParentRect(bool state) {
    _OverExtendViewTextUseParentRect = state;
  }
  bool getOverExtendViewTextUseParentRect() const {
    return _OverExtendViewTextUseParentRect;
  }
  ...
  REFLECT_STRING(
      ...
      setHardText); // Same as text, but localize is implicitly set true
  REFLECT_STRING("text", getText, setText);
  REFLECT_STRING("text_format", getText, setTextFormatTaged);
  REFLECT_STRING("text_single_line_format", getText,
                 setSingleLineTextFormatTaged);
#ifdef RYZOM_LUA_UCSTRING
  // REFLECT_UCSTRING("uc_text", getTextAsUtf16, setTextAsUtf16); // Deprecate
  // uc_ functions
  REFLECT_UCSTRING("uc_hardtext", getHardTextAsUtf16,
                   setHardTextAsUtf16); // Compatibility
  REFLECT_UCSTRING("uc_hardtext_format", getTextAsUtf16,
                   setTextFormatTagedAsUtf16); // Compatibility
  REFLECT_UCSTRING("uc_hardtext_single_line_format", getTextAsUtf16,
                   setSingleLineTextFormatTagedAsUtf16); // Compatibility
#endif
  ...
  REFLECT_BOOL("overExtendViewText", getOverExtendViewText,
               setOverExtendViewText);
  REFLECT_BOOL("overExtendViewTextUseParentRect",
               getOverExtendViewTextUseParentRect,
               setOverExtendViewTextUseParentRect);
...
protected:
  ...
  bool _Embolden;
  bool _Oblique;
  // width of the font in pixel. Just a Hint for tabing format (computed with
  // '_')
  float _FontWidth;
  ...
  /// Is the line (under p loop) should be considered at bottom (if false bottom
  /// is under p leg) maxw for the line/multiline
  sint32 _LineMaxW;
  ...
  struct CFormatInfo {
    ...
    bool operator==(const CFormatInfo &o) const {
      return Color == o.Color && TabX == o.TabX && IndexTt == o.IndexTt;
    }
  ...
  /// Get the current maxW for multiline, accordgin to parent and
...
private:
  ...
  public:
    ...
    void build(const std::string &text, NL3D::UTextContext &textContext,
               uint numSpaces = 0);
  };
  typedef std::vector<CWord> TWordVect;

  // A line of text (which is made of one word with space, or of several words
  ...
  public:
    ...
    // Add a new word (and its context) in the line + a number of spaces to
    // append at the end of the line
    void addWord(const std::string &word, uint numSpaces,
    ...
    // Get the number of chars in the line, not counting the end spaces, but
    // couting the spaces in words
    uint getNumChars() const { return _NumChars; }
    // Get the total number of spaces between words (not including those in
    // words, but there should not be if text is justified)
    ...
    float getWidth() const {
      return _WidthWithoutSpaces + _SpaceWidth * _NumSpaces;
    }
  ...
  private:
    ...
    uint _NumChars;
    uint _NumSpaces;
    float _SpaceWidth; // width of a space, in pixels (used with multispace
                       // alignment)
    float _StringLine;
    float _WidthWithoutSpaces; // width without space (see the Field NumSpaces
                               // in the CWord class). NB : space inserted
                               // inside a word are counted, however!
    uint _EndSpaces; // spaces at the end of the line
    bool _HasLF; // a linefeed is at end of line (no breaking due to line full)
  };
  /// NB : we keep pointers on lines (each line contains a vector, that we don't
  /// want to be copied, and this occurs as the vector of lines grows..)

  typedef NLMISC::CSmartPtr<CLine> TLineSPtr;
  typedef std::vector<TLineSPtr> TLinePtVect;

private:
  /** Data of the updated text for multiline. It is built from the _Text field
   * in the updateTextContext member function, and is used to perform the draw
   */
  TLinePtVect _Lines;

  /// if true, and if the view text is isSingleLineTextClamped(), then an over
  /// will be drawn, with the text
  bool _OverExtendViewText : 1;
  /// if true and _OverExtendViewText true too, use the parent rectangle to know
  /// if must display the over or not
...
private:
  ...
  void setStringSelectionSkipingSpace(uint stringId, const std::string &text,
  ...
  void flushWordInLine(std::string &ucCurrentWord, bool &linePushed,
  ...
  // used for "donctClipWord" case in updateTextContextMultiLineJustified().
  // currLine is reseted
  void addDontClipWordLine(std::vector<CWord> &currLine);

  // FormatTag build.
  static void buildFormatTagText(const std::string &text,
                                 std::string &textBuild,
                                 std::vector<CFormatTag> &formatTags,
                                 std::vector<std::string> &tooltips);
  // FormatTag parsing.
  bool isFormatTagChange(uint textIndex, uint ctIndex) const;
  void getFormatTagChange(uint textIndex, uint &ctIndex,

...
public:
  ...
  static std::string getParam(const std::string &Params,
                              const std::string &ParamName);

  static void
  getAllParams(const std::string &Params,
               std::vector<std::pair<std::string, std::string>> &AllParams);
};

/**
 interface for action handlers factory
 no release in this factory : a handler must be destroyed by the control that
 created it
 */
class CAHManager {
public:
  ...
  /// return the Action Handler 'name'. if name is of form 'ah:params', then
  /// params are filled (NB: else not changed)
   ...
   *	NB: if paramId is NULL, empty or does not exist in the xmlNode, then the
   *optional param in ahId (eg: "show:phrase_book") is taken NB: if none of the
   *optional param in ahId, or the specified param are filled/found, then params
   *is not changed
   */
  void parseAH(xmlNodePtr cur, const char *ahId, const char *paramId,
               IActionHandler *&ahRet, std::string &params);
  void parseAH(xmlNodePtr cur, const char *ahId, const char *paramId,
               IActionHandler *&ahRet, class CStringShared &params);

  /// Get the AH name from ptr
  const std::string &getAHName(IActionHandler *pAH) {
    return getActionHandlerName(pAH);
  }
...
#define REGISTER_ACTION_HANDLER(handler, name)                                 \
  ...
  public:                                                                      \
    handler##Factory() {                                                       \
      ...
      pAHFM->FactoryMap.insert(                                                \
          CAHManager::TFactoryMap::value_type(name, this));                    \

...
public:
  ...
  struct INodeAddedCallback {
     ...
     */
    virtual void nodeAdded(SNode *node,
                           CInterfaceElement *interfaceElement) = 0;
  };

  // Logic structure to initialize the group tree (root node is not displayed
  ...
  struct SNode : public CReflectableRefPtrTarget {
    ...
    bool Show;        // If false, the node is not displayed (true default, Root
                      // ignored)
    sint32 YDecal;
    // Text
    ...
    SNode *Father;
    // updated at display
    SNode *LastVisibleSon; // filled at build time, meaningfull only if son is
                           // shown and opened, undefined otherwise
    // Node added callback
    ...
    ucstring getTextAsUtf16() const {
      return ucstring::makeFromUtf8(Text);
    } // Compatibility
#endif
    ...
    void sort();
    // sort branch & sons alphabetically & by bitmap name (blank bitmap being
    // the first)
    void sortByBitmap();
    ...
    // get node from first parameter on lua stack and throw necessary exception
    // if not present
...
public:
  ...
  // Set root node and delete the last one the user must not delete all
  // allocated sub node. Nb: selection is reseted
  ...
  // Select a node by its line index (depends on opened nodes). no-op if not
  // possible (AH not here, line>size)
  ...
  // Get/Change the NavigateOneBrnahc option. if false, then perform a reset
  // before
  bool getNavigateOneBranch() const { return _NavigateOneBranch; }
  void changeNavigateOneBranch(bool newState);

  // should be called by action handler when thy want to cancel the selection of
  ...
  REFLECT_BOOL("navigate_one_branch", getNavigateOneBranch,
               changeNavigateOneBranch);
...
protected:
  ...
  // If a node is closed and a son of this node was selected, then this option
  // force the ancestor being the new selection
  bool _SelectAncestorOnClose;
  bool _NavigateOneBranch;
  bool _AvoidSelectNodeByIdIR;

  // when an action handler is run, it can call 'cancelSelectLine' if no
  // selection should be done for real
  ...
  CViewBitmap *createViewBitmap(uint line, const std::string &idPrefix,

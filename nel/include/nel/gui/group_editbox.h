...
public:
  ...
  public:
    ...
    virtual bool
    isComboKeyChat(const NLGUI::CEventDescriptorKey &edk) const = 0;
  };
  ...
  enum TEntryType {
    ...
    Filename,
    PlayerName
  }; // the type of entry this edit bot can deal with
  ...
  void setInputStringAsUtf32(const ::u32string &str) {
    setInputStringRef(str);
  } // Compatibility
  ::u32string getInputStringAsUtf32() const {
    return _InputString;
  } // Compatibility
#endif
  ...
  void stopParentBlink() {
    if (_Parent)
      _Parent->disableBlink();
  }
  ...
  void setCursorAtPreviousLineEnd(bool setCursor) {
    _CursorAtPreviousLineEnd = setCursor;
  }
  ...
  const ::u32string &getHistoric(uint32 index) const {
  ...
  void writeString(const std::string &str, bool replace = true,
                   bool atEnd = true);
  ...
  // ignore the next char/key event -> useful when a key set the focus on an
  // editbox (keydown is received, the focus, then keychar is received by the
  // editbox again, but is irrelevant)
  ...
  // set the input string as "default". will be reseted at first click (used for
  // user information)
  ...
  void setPositiveIntegerMinValue(uint32 minValue) {
    _PositiveIntegerMinValue = minValue;
  }
  void setPositiveIntegerMaxValue(uint32 maxValue) {
    _PositiveIntegerMaxValue = maxValue;
  }
...
#ifdef RYZOM_LUA_UCSTRING
  REFLECT_UCSTRING("uc_input_string", getInputStringAsUtf16,
                   setInputStringAsUtf16); // Compatibility
#endif
  REFLECT_EXPORT_END
...
protected:
  ...
  // undo / redo
  ::u32string _StartInputString; // value of the input string when focus was
                                 // acuired first
  ...
  bool _BlinkState : 1;
  bool _CursorAtPreviousLineEnd : 1; // force the cursor to be displayed at the
                                     // end of the previous line end (if END has
                                     // beeen pressed while in a string split
                                     // accross 2 lines)
  bool _LooseFocusOnEnter : 1;
  bool _ResetFocusOnHide : 1;
  bool _BackupFatherContainerPos : 1; // Backup father container position when
                                      // characters are typed. If the edit box
                                      // is at the bottom of the screen and if
                                      // it expands on y because of multiline,
                                      // thz parent container will be moved to
                                      // top The good position can be restored
                                      // by a press on enter then
  ...
  bool _DefaultInputString : 1; // Is the current input string the default one
...
public:
  static void setComboKeyHandler(IComboKeyHandler *handler) {
    comboKeyHandler = handler;
  }
};

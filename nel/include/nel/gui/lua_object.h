 ...
 * - lua 'pointers'
 *
 * Each reference object has an id giving its path in order to track bugs more
 * easily
 */
class CLuaObject {
public:
  ...
  // See if the obj
  bool isValid() const;
  // Pop a new value for this lua object from the top of the stack. The stack
  // must not be empty
  ...
  void setValue(const std::string &key, const CLuaObject &value) {
  ...
  // Enumeration of a table. If the object is not a table, an exception is
  // thrown.
   ...
   * TODO nico : version that takes destination object as a reference in its
   * parameter to avoid an object copy
   */
  CLuaObject operator[](double key) const;
  CLuaObject operator[](const char *key) const;
  CLuaObject operator[](const std::string &key) const {
    return operator[](key.c_str());
  }
  /** Checked access to a sub element of a table. An exception is thrown is the
   * element is not a table.
   */
  CLuaObject at(const char *key) const;
  CLuaObject at(const std::string &key) const { return at(key.c_str()); }

  // Test is that table has the given key. The object must be a table or an
  // exception is thrown
  bool hasKey(const char *key) const;

  /** debug : recursively get value (useful for table)
   * \param maxDepth (0 for no limit)
   * \param alreadySeen pointer to lua tables that have already been displayed
   * by the command (to avoid infinite recursion when a cycluic graph is
   * encountered)
   */
  std::string toStringRecurse(uint depth = 0, uint maxDepth = 20,
   ...
   * encountered)
   */
  void dump(uint maxDepth = 20,
            std::set<const void *> *alreadySeen = NULL) const;
  // concatenate identifiers, adding a dot between them if necessary. If right
  // is a number then brackets are added
  static std::string concatId(const std::string &left,
 ...
 * \param object A CLuaObject which must be a table, and on which enumeration is
 * done. An exception will be thrown as 'CLuaObject::enumerate' is called if
 * this is not the case \param enumerator The enumerator object
 */
#define ENUM_LUA_TABLE(object, enumerator)                                     \
  for (CLuaEnumeration enumerator = (object).enumerate();                      \
       enumerator.hasNext(); enumerator.next())
...
private:
  std::string _Str;
  mutable CLuaState::TRefPtr
      _LuaState; // ref ptr so that statics get rebuilt on lua restart
...
public:
  ...
  // hasher for lua string  -> they are unique pointers for each string, so just
  // hash a pointer instead of a string...
  size_t operator()(const char *value) const { return ((size_t)value) >> 3; }

  // equality for lua string for hash_map -> they are unique pointer -> compare
  // pointers instead of string content

...
public:
  ...
  enum EAlign { Bottom = 0, Top, Left, Right };
   ...
   */
  bool addChildAtIndex(CViewBase *child, uint index,
   ...
   */
  void addTextChild(const std::string &line, const NLMISC::CRGBA &textColor,
                    bool multiLine = true);
  ...
  /// Same as adding a text child but the text will be taken from the string
  /// manager
  ...
  bool delChild(CViewBase *child, bool noWarning = false,
                bool forceDontDelete = false);
   ...
   * \templ : a CViewText object. Only its font size, color and shadow are
   * required.
   */
   ...
   * required.
   */
  CViewText *getTextTemplatePtr() { return &_Templ; }
  ...
  // set the rank for the element at the given index (used to reinsert container
  // after they have been turned into popups)
...
protected:
  ...
  struct CElementInfo {
    uint Order; // Used to sort the window by their insertion order.
                // This is used to put back a window at the right place if it
                // was turned into a popup.
    CViewBase *Element;
  ...
  // (the element drawn are stored in _views, _contrlos or _childrengroups of
  // cinterfacegroup

...
public:
  ...
  public:
    ...
    enum ParsingStage {
      ...
      Unresolved = 1, /// module can parse in the first stage when styles,
                      /// templates, etc have not been resolved yet
  ...
  /// Interface for event handlers which can be called when setting up the
  /// options.
...
public:
   ...
   * \param isFilename true if xmlFileNames array contains the names of the xml
   * file, false, if each array is a script itself
   */
  bool parseInterface(const std::vector<std::string> &xmlFileNames, bool reload,
                      bool isFilename = true, bool checkInData = false);

  bool parseXMLDocument(xmlNodePtr root, bool reload);
  bool parseTemplateNode(xmlNodePtr node, xmlNodePtr instance,
                         xmlNodePtr templ);
  ...
  bool parseGroupChildren(xmlNodePtr cur, CInterfaceGroup *parentGroup,
                          bool reload);
  ...
  // Solve All define in a string. return false if some define not founs
  // (defError contains this define)
  bool solveDefine(const std::string &propVal, std::string &newPropVal,
                   std::string &defError);
  ...
  /// Association builders : associate an element of the interface with the
  /// string ID of another element used as reference for position values
  void addParentPositionAssociation(CInterfaceElement *element,
                                    const std::string &parentID);
  std::string getParentPosAssociation(CInterfaceElement *element) const;
  void addParentSizeAssociation(CInterfaceElement *element,
                                const std::string &parentID);
  std::string getParentSizeAssociation(CInterfaceElement *element) const;
  void addParentSizeMaxAssociation(CInterfaceElement *element,
                                   const std::string &parentID);
  std::string getParentSizeMaxAssociation(CInterfaceElement *element) const;

  /// LUA Class Association builder :  associate a lua script to a group (called
  /// for each group after every document parsed)
  void addLuaClassAssociation(CInterfaceGroup *group,
   ...
   */
  CInterfaceGroup *
  createGroupInstance(const std::string &templateName,
                      const std::string &parentID,
                      const std::pair<std::string, std::string> *templateParams,
                      uint numParams, bool updateLinks = true);
  CInterfaceGroup *createGroupInstance(
      const std::string &templateName, const std::string &parentID,
      std::vector<std::pair<std::string, std::string>> &templateParams,
      bool updateLinks = true) {
    if (!templateParams.empty())
      return createGroupInstance(templateName, parentID, &templateParams[0],
                                 (uint)templateParams.size(), updateLinks);
    else
   ...
   */
  CInterfaceElement *
  createUIElement(const std::string &templateName, const std::string &parentID,
                  const std::pair<std::string, std::string> *templateParams,
                  uint numParams, bool updateLinks /* = true */);
  CInterfaceElement *createUIElement(
      const std::string &templateName, const std::string &parentID,
      std::vector<std::pair<std::string, std::string>> &templateParams,
      bool updateLinks = true) {
    if (!templateParams.empty())
      return createUIElement(templateName, parentID, &templateParams[0],
                             (uint)templateParams.size(), updateLinks);
    else
  ...
  // search a "tree" node in the hierarchy that match node. may return root!
  // NULL if not found
  static xmlNodePtr searchTreeNodeInHierarchy(xmlNodePtr root,
                                              const char *node);
...
public:
  ...
  // return false if procedure not found, or if bad action index. return false
  // if has some param variable (@0...)
  bool getProcedureAction(const std::string &procName, uint actionIndex,
                          std::string &ah, std::string &params) const;
...
protected:
  ...
  /// vector storing parsed templates during init. At the end of init, only used
  /// template are kept
...
public:
  ...
  inline void addFeatureFlag(const std::string &flag) {
    m_FeatureFlags.insert(flag);
  }
...
public:
  ...
  /// Load A .lua. false if parse error. string 'error' contains the eventual
  /// error desc (but warning still displayed)
  ...
  void setSetupOptionsCallback(ISetupOptionCallbackClass *cb) {
    setupCallback = cb;
  }

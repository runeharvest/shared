 ...
 * done for each character preceded by $ $p -> expand the player name $P ->
 * expand the player name in uppercase $b -> expand the current bot name ( bot
 * with which the player is talking) $s -> expand the current short bot name
 * (with no specification/title in it) if "ui..." replace the format with CI18N
 */
...
public:
  ...
  public:
    ...
    virtual std::string formatString(const std::string &inputString,
  ...
  static std::string formatString(const std::string &inputString,
                                  const std::string &paramString);

  static void setFormatter(IViewTextFormatter *formatter) {

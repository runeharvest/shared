 ...
 * If the element is a control, then visitCtrl() is called, and then
 * visitElement() If the element is a view, then visitView() is called, and then
 * visitElement() If the element is a group, then visitGoup() is called, and
 * then visitElement()
 *
...
/**
 * class describing a localisable interface element, i.e. : an element with
 * coordinates \author Nicolas Brigand \author Nevrax France \date 2002
 */
class CInterfaceElement : public IInterfaceScaleWatcher,
...
public:
  ...
  enum EStrech { none = 0, width = 1, height = 2 };
   ...
   * NB Nico : if too slow, should use a CFastStream version instead, that is
   * designedto work in memory only
   */
  ...
  sint32 getW(bool bTestActive) const {
    return (bTestActive ? (_Active ? _W : 0) : _W);
  }

  sint32 getH() const { return (_Active ? _H : 0); }
  sint32 getH(bool bTestActive) const {
    return (bTestActive ? (_Active ? _H : 0) : _H);
  }
   ...
   *
   * The view must return the largest width its content can take if it will be
   * resized at maximum. Typical use : for a CTextView multiline, it returns the
   * size of the whole string.
   *
   * This method is used by the container CCGroupTable that need to know this
   * information about its children in its resizing algorithm.
   */
   ...
   *
   * The view must return the smallest width its content can take if it will be
   * resized at minimum. Typical use : for a CTextView multiline without word
   * clipping, it returns the size of the largest word.
   *
   * This method is used by the container CCGroupTable that need to know this
   * information about its children in its resizing algorithm.
   */
  ...
  void setXAndInvalidateCoords(sint32 x) {
    _X = x;
    invalidateCoords();
  }

  void setY(sint32 y) { _Y = y; }
  void setYAndInvalidateCoords(sint32 y) {
    _Y = y;
    invalidateCoords();
  }

  void setW(sint32 w);
  void setWAndInvalidateCoords(sint32 w) {
    setW(w);
    invalidateCoords();
  }

  void setH(sint32 h);
  void setHAndInvalidateCoords(sint32 h) {
    setH(h);
    invalidateCoords();
  }
   ...
   * \return NULL if the element is not on the window, otherwise returns a
   * pointer to the window
   */
  CInterfaceGroup *getRootWindow();

  /** get the element Depth, ie the number of parent he has (0 if _Parent==NULL)
   *	\warning slow test. don't take into account CCtrlBase::getDeltaDepth()
   *(since method not virtual)
   */
  uint getParentDepth() const;

  /**
   * true if the element and all its parents are active (recurs up to the root
   * window)
   */
  ...
  void relativeSInt64Read(CInterfaceProperty &rIP, const std::string &prop,
                          const char *val, const std::string &defVal);
  void relativeSInt32Read(CInterfaceProperty &rIP, const std::string &prop,
                          const char *val, const std::string &defVal);
  void relativeBoolRead(CInterfaceProperty &rIP, const std::string &prop,
                        const char *val, const std::string &defVal);
  void relativeRGBARead(CInterfaceProperty &rIP, const std::string &prop,
                        const char *val, const std::string &defVal);

  // Parse tools
  static std::string HotSpotToString(THotSpot spot);
  static std::string HotSpotCoupleToString(THotSpot parentPosRef,
                                           THotSpot posRef);
  static THotSpot convertHotSpot(const char *ptr); //
  static void convertHotSpotCouple(const char *ptr, THotSpot &parentPosRef,
                                   THotSpot &posRef);
   ...
   */
  static void convertPixelsOrRatio(const char *ptr, sint32 &pixels,
                                   float &ratio);

  // add an interface link to that element (kept in a smart ptr)
  void addLink(CInterfaceLink *link);

  // remove a link from that element; There's one less reference on the link
  // (which is referenced by a smart ptr)
  ...
  /** This allow to force opening an element. By default it just activate the
   * element. It allow to have different behaviour on more complex containers
   */
  ...
  // Options for views to be modulated by interface global color or not. Parsed
  // with "global_color". Default: true
  ...
  std::string getSizeRefAsString(const sint32 &sizeRef, const sint32 &sizeDivW,
                                 const sint32 &sizeDivH) const;
...
public:
  ...
  /* invalidate coords. set numPass==1 if you are SURE that only XReal/YReal
   *need to be updated Default 3 is needed for: 1: update _W/_H and
   *_WReal/_HReal according to Sons 2: update XReal/YReal (eg: according to
   *Scroll offset)
   */
  void invalidateCoords(uint8 numPass = 2);
  uint8 getInvalidCoords() const { return _InvalidCoords; }

  /* Invalidates the content of the window. This method invalidate the content
   * of the window (for CViewText or CGroupTable). It invalidates the content of
   * the parent too.
   */
  void invalidateContent();

  /* This call back is called when the content or the content of a child is been
   * invalidated.
   */
  ...
  /// Update the elements coords convert x,y,w,h (parentpos coord) to
  /// xreal,yreal,wreal,hreal (BL coord)
  ...
  enum TRenderWired { RenderView, RenderCtrl, RenderGroup };
  ...
  /** This is called before the config loading begins. This is the place to
   * restore default state for config info.
   */
  virtual void onLoadConfig() {}
  /** Tells whether that element wants to save info in a config stream. If this
   * returns true, then serialConfig is called.
   */
  virtual bool wantSerialConfig() const { return false; }
  // Serial config info about that element. This is called only if
  // wantSerialConfig() returns true
  ...
  /** When user is quitting the interface, this is called. Then the interface
   * config is saved This is where the element get the opportunity to do some
   * cleanup.
   */
  virtual void onQuit() {}

  /// Whent an element is added to a CInterfaceGroup via addCtrl, addGroup or
  /// addView, this is called after the add.
  virtual void onAddToGroup() {}

  /** typically used only in conjunction with CGroupInScene. Such groups move
   *every Frames. so this function is called on each children elements to move
   *the XReal/Yreal only (with a delta)
   */
...
protected:
  ...
  // position references e.g. : _PosRef=BL, _ParentPosref=MM : the bottom left
  // corner of the element will be placed on the center (middle middle) of the
  // parent window
  THotSpot _PosRef;
  THotSpot _ParentPosRef;
  NLMISC::CRefPtr<CInterfaceElement>
      _ParentPos; // RefPtr in case of group destroyed in a parent group with
                  // posref on it

  sint32 _SizeRef; // none == 0, w == 1, h == 2, wh == 3
  sint32 _SizeDivW, _SizeDivH;
  NLMISC::CRefPtr<CInterfaceElement>
      _ParentSize; // RefPtr in case of group destroyed in a parent group with
                   // posref on it
  ...
  void parseSizeRef(const char *sizeRefStr, sint32 &sizeref, sint32 &sizeDivW,
                    sint32 &sizeDivH);

private:
  /// Notifies the deletion watchers that this interface element is being
  /// deleted
...
public:
  CStringShared() { _Id = NLMISC::CStringMapper::emptyId(); }
  ...
  bool empty() const { return _Id == NLMISC::CStringMapper::emptyId(); }

  static CStringShared emptyString() { return CStringShared(); }
...
inline bool operator==(const CStringShared &lhs, const CStringShared &rhs) {

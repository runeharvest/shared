...
public:
  ...
  // Delete unknow type by name or ptr. NB: additionaly, if it's a group,
  // unmakeWindow() is called as necessary
  ...
  virtual bool isWindowUnder(sint32 x,
                             sint32 y); // Virtual for menu that is not square
  ...
  virtual bool getGroupsUnder(sint32 x, sint32 y, sint32 clipX, sint32 clipY,
                              sint32 clipW, sint32 clipH,
  ...
  /// Tool function to draw a single Element that should exist in the group
  /// (clipped by the group)
  ...
  void setAHOnEscape(const std::string &ah) {
    _AHOnEscape = CAHManager::getInstance()->getAH(ah, _AHOnEscapeParams);
  }
  const std::string &getAHOnEscape() const {
    return CAHManager::getInstance()->getAHName(_AHOnEscape);
  }
  ...
  // enterable
  void setAHOnEnter(const std::string &ah) {
    _AHOnEnter = CAHManager::getInstance()->getAH(ah, _AHOnEnterParams);
  }
  const std::string &getAHOnEnter() const {
    return CAHManager::getInstance()->getAHName(_AHOnEnter);
  }
  ...
  void setResizeFromChildWMargin(sint32 margin) {
    _ResizeFromChildWMargin = margin;
  }
  void setResizeFromChildHMargin(sint32 margin) {
    _ResizeFromChildHMargin = margin;
  }
  ...
  void evalChildrenBBox(bool resizeFromChildW, bool resizeFromChildH,
  ...
  void setLeftClickHandlerParams(const std::string &params) {
    _AHOnLeftClickParams = params;
  }
  void setRightClickHandlerParams(const std::string &params) {
    _AHOnRightClickParams = params;
  }
  void setOnActiveHandler(const std::string &h) {
    _AHOnActive = CAHManager::getInstance()->getAH(h, _AHOnActiveParams);
  }
  void setOnActiveParams(const std::string &p) { _AHOnActiveParams = p; }
  void setOnDeactiveHandler(const std::string &h) {
    _AHOnDeactive = CAHManager::getInstance()->getAH(h, _AHOnDeactiveParams);
  }
  void setOnDeactiveParams(const std::string &p) { _AHOnDeactiveParams = p; }

  const std::string &getLeftClickHandler() const {
    return CAHManager::getInstance()->getAHName(_AHOnLeftClick);
  }
  const std::string &getLeftClickHandlerParams() const {
    return _AHOnLeftClickParams;
  }
  const std::string &getRightClickHandler() const {
    return CAHManager::getInstance()->getAHName(_AHOnRightClick);
  }
  const std::string &getRightClickHandlerParams() const {
    return _AHOnRightClickParams;
  }
  const std::string &getOnActiveHandler() const {
    return CAHManager::getInstance()->getAHName(_AHOnActive);
  }
  const std::string &getOnActiveParams() const { return _AHOnActiveParams; }
  const std::string &getOnDeactiveHandler() const {
    return CAHManager::getInstance()->getAHName(_AHOnDeactive);
  }
  ...
  REFLECT_STRING_REF("left_click_params", getLeftClickHandlerParams,
                     setLeftClickHandlerParams);
  REFLECT_STRING_REF("right_click_params", getRightClickHandlerParams,
                     setRightClickHandlerParams);
  ...
  REFLECT_STRING_REF("on_deactive_params", getOnDeactiveParams,
                     setOnDeactiveParams);
  ...
  REFLECT_STRING_REF("on_escape_params", getAHOnEscapeParams,
                     setAHOnEscapeParams);
  ...
  REFLECT_SINT32("child_resize_wmargin", getResizeFromChildWMargin,
                 setResizeFromChildWMargin);
  REFLECT_BOOL("child_resize_h", getResizeFromChildH, setResizeFromChildH);
  REFLECT_SINT32("child_resize_hmargin", getResizeFromChildHMargin,
                 setResizeFromChildHMargin);
  REFLECT_SINT32("ofsy", getOfsY, setOfsY);
  ...
  /// Eval current clip coords. This is not incremental as with makeNewClip, and
  /// thus more slow. This also doesn't change the current clip window.
  ...
  virtual void renderWiredQuads(CInterfaceElement::TRenderWired type,
  ...
  // Set the LUA script to execute when a list of DB change (of forms:
  // "@DB1,@DB2" ....). The dbList is the key
  void addLuaScriptOnDBChange(const std::string &dbList,
...
protected:
  void makeNewClip(sint32 &oldClipX, sint32 &oldClipY, sint32 &oldClipW,
                   sint32 &oldClipH);
  void restoreClip(sint32 oldSciX, sint32 oldSciY, sint32 oldSciW,
                   sint32 oldSciH);

  // Compute clip contribution for current window. This doesn't change the clip
  // window in the driver.
  void computeClipContribution(sint32 &newX, sint32 &newY, sint32 &newW,
                               sint32 &newH) const;

  // Compute clip contribution for current window, and a previous clipping
  // rectangle. This doesn't change the clip window in the driver.
  void computeCurrentClipContribution(sint32 prevX, sint32 prevY, sint32 prevW,
                                      sint32 prevH, sint32 &newX, sint32 &newY,
  ...
  // notify children controls & groups that 'active' has been called on one of
  // their parent
  void
  notifyActiveCalled(const NLGUI::CEventDescriptorActiveCalledOnParent &desc);
...
protected:
  ...
  NLMISC::CRefPtr<CInterfaceElement>
      _ParentSizeMax; // RefPtr in case of group destroyed in a parent group
                      // with posref on it
  sint32 _MaxW, _MaxH;
...
private:
  ...
  /// \name LUA specific
  // @{
  // Lua Env Table created. Table is in the LUA_REGISTRYINDEX, with key as this
  // CInterfaceGroup* userdata
  bool _LUAEnvTableCreated;
  // The LUA script to be executed on Draw (checkCoords)
  CStringShared _LUAOnDraw;
  // The InterfaceLink created specialy for Lua Script to be executed at some DB
  // change
  typedef std::map<std::string, NLMISC::CSmartPtr<CInterfaceLink>>
      TLUAOnDbChange;

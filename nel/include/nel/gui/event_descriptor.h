...
public:
  enum EEventType { key = 0, mouse = 1, system = 2 };

  EEventType getType() const { return _EventType; }
...
public:
  ...
  CEventDescriptorKey()
      : _KeyEvent(unknown), _CtrlState(false), _ShiftState(false),
        _AltState(false), _Char(0) {
    _EventType = key;
  }
  ...
  // return the key that has been pressed. The key event type MUST be 'keydown'
  // or 'keyup', else => assert
  NLMISC::TKey getKey() const {
    nlassert(_KeyEvent == keydown || _KeyEvent == keyup);
    return _Key;
  }
  // return the char that has been pressed. The key event type MUST be
  // 'keychar', else => assert
  u32char getChar() const {
    nlassert(_KeyEvent == keychar);
    return _Char;
  }
  // return the string that has been sent. The key event type MUST be
  // 'keystring', else => assert
  ...
  bool isShiftDown() {
    return (_KeyEvent == CEventDescriptorKey::keydown &&
            (_Key == NLMISC::KeySHIFT || _ShiftState)) ||
           (_KeyEvent == CEventDescriptorKey::keyup &&
            (_Key != NLMISC::KeySHIFT && _ShiftState)) ||
  ...
  bool isCtrlDown() {
    return (_KeyEvent == CEventDescriptorKey::keydown &&
            (_Key == NLMISC::KeyCONTROL || _CtrlState)) ||
           (_KeyEvent == CEventDescriptorKey::keyup &&
            (_Key != NLMISC::KeyCONTROL && _CtrlState)) ||
...
public:
  ...
  sint32 getWheel() const { return _Wheel; }

  sint32 getEventTypeExtended() const { return _EventTypeExtended; }

  void setX(sint32 x) { _X = x; }

  void setY(sint32 y) { _Y = y; }

  void setWheel(sint32 w) { _Wheel = w; }

  void setEventTypeExtended(sint32 e) { _EventTypeExtended = e; }

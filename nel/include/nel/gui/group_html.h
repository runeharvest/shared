...
public:
  ICurlDownloadCB(const std::string &url) : url(url) {}
...
public:
  ...
  ICurlDownloadCB *
  addImageDownload(const std::string &url, CViewBase *img,
                   const CStyleParams &style = CStyleParams(),
                   const TImageType type = NormalImage,
                   const std::string &placeholder = "web_del.tga");
  ICurlDownloadCB *addTextureDownload(const std::string &url, sint32 &texId,
                                      CViewBase *view);
...
protected:
  ...
  void addImage(const std::string &id, const std::string &img,
                bool reloadImg = false,
                const CStyleParams &style = CStyleParams());

  // Add a text area in the current paragraph
  CInterfaceGroup *addTextArea(const std::string &templateName,
                               const char *name, uint rows, uint cols,
                               bool multiLine, const std::string &content,
                               uint maxlength);

  // Add a combo box in the current paragraph
  CDBGroupComboBox *addComboBox(const std::string &templateName,
                                const char *name);
  CGroupMenu *addSelectBox(const std::string &templateName, const char *name);

  // Add a button in the current paragraph. actionHandler, actionHandlerParams
  // and tooltip can be NULL.
  CCtrlButton *
  addButton(CCtrlButton::EType type, const std::string &name,
            const std::string &normalBitmap, const std::string &pushedBitmap,
            const std::string &overBitmap, const char *actionHandler,
            const char *actionHandlerParams, const std::string &tooltip,
            const CStyleParams &style = CStyleParams());
  ...
  // true if renderer is waiting for css files to finish downloading (link
  // rel=stylesheet)
  ...
  public:
    HTMLDListElement() : DT(false), DD(false) {}
  ...
  public:
    ...
    NLMISC::CRGBA getValueColor(const CHtmlElement &elm,
  ...
  public:
    HTMLProgressElement() : value(0.f), max(1.f) {}
  ...
  struct SFormSubmitButton {
    SFormSubmitButton(const std::string &form, const std::string &name,
  ...
  const std::string &selectTreeNodeRecurs(CGroupTree::SNode *node,
                                          const std::string &url);
  // search if the action / params match the url. look recurs into procedures
  bool actionLaunchUrlRecurs(const std::string &ah, const std::string &params,
                             const std::string &url);
  ...
  void doBrowseRemoteUrl(std::string url, const std::string &referer,
                         bool doPost = false,
                         const SFormFields &formfields = SFormFields());
...
private:
  ...
  public:
    CDataDownload(const std::string &u, const std::string &d)
        : ICurlDownloadCB(u), data(NULL), fp(NULL), dest(d), redirects(0),
  ...
  public:
    StylesheetDownloadCB(const std::string &url, const std::string &dest,
                         CGroupHTML *parent)
  ...
  public:
    ...
    ImageDownloadCB(const std::string &url, const std::string &dest,
                    CViewBase *img, const CStyleParams &style, TImageType type,
                    CGroupHTML *parent)
        : CDataDownload(url, dest), Parent(parent) {
  ...
  public:
    TextureDownloadCB(const std::string &url, const std::string &dest,
                      sint32 texId, CViewBase *view)
  ...
  public:
    BnpDownloadCB(const std::string &url, const std::string &dest,
                  const std::string md5sum, const std::string lua,
                  CGroupHTML *parent)
        : CDataDownload(url, dest), Parent(parent), m_md5sum(md5sum),
          m_lua(lua) {}
  ...
  void setImageSize(CViewBase *view,
                    const CStyleParams &style = CStyleParams());

  void setTextButtonStyle(CCtrlTextButton *ctrlButton,
  ...
  bool addBnpDownload(std::string url, const std::string &action,
                      const std::string &script, const std::string &md5sum);
  ...
  void dataDownloadFinished(bool success, const std::string &error,
                            CDataDownload *data);

  // HtmlType download finished
  void htmlDownloadFinished(const std::string &content, const std::string &type,
                            long code);

  // stylesheet finished downloading. if local file does not exist, then it
  // failed (404)
  ...
  void insertFormImageButton(const std::string &name,
                             const std::string &tooltip, const std::string &src,
                             const std::string &over, const std::string &formId,
                             const std::string &formAction = "",
                             uint32 minWidth = 0,
                             const std::string &templateName = "");

  void insertFormTextButton(const std::string &name, const std::string &tooltip,
                            const std::string &value, const std::string &formId,
                            const std::string &formAction = "",
                            uint32 minWidth = 0,
                            const std::string &templateName = "");

 ...
 * A link is an object that can read one or several values from the database,
 * that can evaluate an expression on these database entries (simple
 * computation, using the CInterfaceExpr class), and that can affect the result
 * to an interface property that has been exported by an interface element (the
 * export system uses reflect.h). The first time it is created, it places
 * observers on the database entries that are needed by the expression, so each
 ...
 *
 * Example of use : connecting a change in the db tree to the 'active' state of
 * a window
 *
...
public:
#ifdef NL_DEBUG
  // for debugging purposes : if this link is 'named' e.g is owner by
  // CInterfaceManager and was created by calling CInterfaceManager::addLink,
  // contains the name of this link
  std::string LinkName;
#endif
public:
  struct CTargetInfo {
  ...
  class CInterfaceLinkUpdater : public NLMISC::CCDBBranchObservingHandler::
...
public:
  CInterfaceLink();
  ~CInterfaceLink(); // this object should only be destroyed by a
                     // CInterfaceElement
  /** Make a link between the given interface element properties and a value
   * that depends on database entries. The link is automatically added in the
   * link list of the targets element (it calls CInterfaceElement::addLink), so
   * when all target elements are removed, the link is. If there are no target
   * element, the link is permanent (removed at exit) NB : The target is not
   * updated during this call.
   */
  bool init(const std::vector<CTargetInfo> &targets,
            const std::vector<CCDBTargetInfo> &cdbTargets,
            const std::string &expr, const std::string &actionHandler,
            const std::string &ahParams, const std::string &ahCond,
            CInterfaceGroup *parent);
  // force all the links that have been created to update their targets. This
  // can be called when the interface has been loaded, and when the databse
  // entries have been retrieved.
  ...
  // Get the i-th target
  CInterfaceElement *getTarget(uint index) const {
    return _Targets[index]._InterfaceElement;
  }

  static void removeAllLinks();

  static void setTargetProperty(const std::string &Target,
                                const CInterfaceExprValue &val);

  static bool isUpdatingAllLinks() { return _UpdateAllLinks; }

  /** From a target name of a link, retrieve the target element and its target
   * target property \return true if the target is valid
   */
  static bool splitLinkTarget(const std::string &target,
                              CInterfaceGroup *parentGroup,
                              std::string &propertyName,
                              CInterfaceElement *&targetElm);

  /** From several target names of a link (seprated by ','), retrieve the target
   * elements and their target properties \return true if all targets are valid
   */
  static bool
  splitLinkTargets(const std::string &targets, CInterfaceGroup *parentGroup,
                   std::vector<CInterfaceLink::CTargetInfo> &targetsVect);
  static bool splitLinkTargetsExt(
...
private:
  ...
  bool _On;
  static TLinkVect
      _LinksWithNoTarget; // there should be an owner for links with no targets
  static bool _UpdateAllLinks;
  ///\ name triggered link mgt
  //@{
  // next/previous link that was trigered. NULL means end or start of list
  // each ptr is duplicated because with manage 2 lists : one list in which
  // links are added, and one list in which we update links. This way one link
  // can trigger another with no prb
  ...
  // iterators in current list being updated : they're global so that deleting a
  // CInterfaceLink instance prevent them from becoming dangling pointers

...
/**
 * CGroupSubMenu describe an element of a contextual menu (contains text lines
 * and sub menu) \author Matthieu 'TrapII' Besson \author Nevrax France \date
 * 2002
 */
...
public:
  ...
  CViewTextMenu *
  addLineAtIndex(uint index, const std::string &name, const std::string &ah,
                 const std::string &params, const std::string &id = "",
                 const std::string &cond = std::string(),
                 const std::string &texture = "", bool checkable = false,
                 bool checked = false, bool formatted = false);
  ...
  void setRightClickHandlerParam(uint lineIndex,
   ...
   * 'ownership' tells whether this menu should remove the group when it is
   * deleted Setting a user group on a line with a separator is illegal
   */
  ...
  // if a menu isn't selectable, can't click on it, and there's no selection
  // when the mouse is over it (but can click on its widgets, such as a
  // usergroup)
  ...
  REFLECT_LUA_METHOD("getLineId",
                     luaGetLineId); // return the id of a line from its index
  REFLECT_LUA_METHOD(
      "getLineFromId",
      luaGetLineFromId); // return -1 if line with id is not found
  ...
  REFLECT_LUA_METHOD("addIconLine",
                     luaAddIconLine); // name, ah, ah_params, id, texture
  REFLECT_LUA_METHOD("addLineAtIndex",
                     luaAddLineAtIndex); // index, name, ah, ah_params, id
  ...
  REFLECT_LUA_METHOD("setUserGroupRight",
                     luaSetUserGroupRight); // line, group ptr
  REFLECT_LUA_METHOD("getUserGroupRight", luaGetUserGroupRight); // line
  REFLECT_LUA_METHOD("setUserGroupLeft",
                     luaSetUserGroupLeft); // line, group ptr
...
protected:
  struct SSubMenuEntry {
    ...
    CInterfaceGroup *UserGroupRight; // not for separator, inserted before
                                     // checkbox & submenu arrow
...
private:
   ...
   * otherwise, no copy is made, but this menu entries are appended to the
   * already created 'appendMenu' menu. NB : user groups are not duplicated
   */
  ...
  CViewBitmap *createIcon(CInterfaceElement *parentPos,
                          const std::string &texture);
...
public:
  ...
  void addLine(const std::string &name, const std::string &ah = "",
               const std::string &params = "",
               const std::string &id = std::string(),
               const std::string &cond = std::string(),
               const std::string &texture = "", bool checkable = false,
               bool checked = false);
  void addLineAtIndex(uint index, const std::string &name,
                      const std::string &ah = "",
                      const std::string &params = "",
                      const std::string &id = std::string(),
                      const std::string &cond = std::string(),
  ...
  // Highlight single line
  void setSelected(uint line) {
    if (_RootMenu)
      _RootMenu->setSelected(line);
  }
  void clearSelected() {
    if (_RootMenu)
      _RootMenu->clearSelected();
  }
  ...
  bool getCloseSubMenuUsingPopModal() const {
    return _CloseSubMenuUsingPopModal;
  }
  void setCloseSubMenuUsingPopModal(bool state) {
    _CloseSubMenuUsingPopModal = state;
  }
...
protected:
  ...
  NLMISC::CRGBA
      _ShadowColorOver;         // Color of the shadow when the mouse is over it

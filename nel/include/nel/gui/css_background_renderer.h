...
public:
  ...
  NLMISC::CRGBA getColor() const { return m_Background.color; }

  // override painting area to be at least the size of viewport (ie, <html>
  // background)
...
private:
  ...
  // if true, painting area returns area at least the size of viewport (ie,
  // <html> background)
  bool m_FillViewport;
  ...
  void getPaintingArea(const CSSBackground &bg, sint32 &areaX, sint32 &areaY,
                       sint32 &areaW, sint32 &areaH) const;

  // get positioning area based on background-origin
  void getPositioningArea(const CSSBackground &bg, sint32 &areaX, sint32 &areaY,
                          sint32 &areaW, sint32 &areaH) const;
  ...
  void calculatePosition(const CSSBackground &bg, sint32 &texX, sint32 &texY,
                         sint32 &texW, sint32 &texH) const;

  // calculate image tile position, size, count, and spacing based on
  // background-repeat
  void calculateTiles(const CSSBackground &bg, sint32 &texX, sint32 &texY,
                      sint32 &texW, sint32 &texH, sint32 &tilesX,
                      sint32 &tilesY, sint32 &spacingX, sint32 &spacingY) const;

  // position, size, and count for first tile to cover an area
  void getImageTile(sint32 &tilePos, sint32 &tileSize, sint32 &spacing,
                    sint32 &tiles, sint32 areaPos, sint32 areaSize,

 ...
 * \date 2002
 */
class CCtrlScroll : public CCtrlScrollBase,
...
public:
   ...
   *	Useful for finer controled group scrolling when the list is very big
   *(with mouseWheel or scroll buttons)
   */
  ...
  void setTextureMiddleTile(uint8 tile) {
    _TileM = tile;
  ...
  void setMinMax(sint32 nMin, sint32 nMax) {
    _Min = nMin;
    _Max = nMax;
  }
  ...
  void setTrackSize(sint32 /* trackSize */) {
    throw NLMISC::Exception("TrackSize is read-only");
  }
   ...
   * Example : Had we a list of items for which we want some item 'itemPtr' to
   * have its top position matching the middle of the list, we would do :
   * this->ensureVisible(itemPtr, Hotspot_Tx, Hotspot_Mx);
   *
   * The scrollbar will be moved accordingly.
   */
  void ensureVisible(CInterfaceElement *childElement, THotSpot childHotSpot,
                     THotSpot parentHotSpot);

protected:
  CInterfaceProperty
      _DBLink;   // If this is a value scroller we can link it with db
  ...
  bool _MouseDown : 1;
  bool _CallingAH : 1;
  bool _Cancelable : 1; // true if the slider may be cancelled when pressed on
  ...
  // For Value Scroller only: indicate the step the scroll bar has. 0 or 1 means
  // no step
  uint32 _StepValue;

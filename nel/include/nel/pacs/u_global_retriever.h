...
public:
  ...
  virtual bool testRaytrace(const NLMISC::CVectorD &v0,
                            const NLMISC::CVectorD &v1) = 0;
  ...
  getIdentifier(const UGlobalPosition &pos) const = 0;
   ...
   */
  virtual bool buildInstance(const std::string &id,
   ...
   * (double instead.), with specification of whether it is on landscape or in
   * interior
   */
   ...
   * Retrieves the position of an estimated point in the global retriever.
   */
  virtual UGlobalPosition
  retrievePosition(const NLMISC::CVector &estimated) const = 0;
   ...
   */
  virtual UGlobalPosition
  retrievePosition(const NLMISC::CVectorD &estimated) const = 0;

  /**
   * Retrieves the position of an estimated point in the global retriever. Uses
   * a snapping threshold.
   */
  virtual UGlobalPosition retrievePosition(const NLMISC::CVector &estimated,
                                           float threshold) const = 0;

  /**
   * Retrieves the position of an estimated point in the global retriever
   * (double instead.) with a snapping threshold
   */
  virtual UGlobalPosition retrievePosition(const NLMISC::CVectorD &estimated,
   ...
   */
  virtual bool isWaterPosition(const UGlobalPosition &pos,
                               float &waterHeight) const = 0;

  ///
  virtual float distanceToBorder(const UGlobalPosition &pos) const = 0;
  ///
  virtual void
  getBorders(const UGlobalPosition &pos,
             std::vector<std::pair<NLMISC::CLine, uint8>> &edges) = 0;
  virtual void
  getBorders(const NLMISC::CAABBox &box,
             std::vector<std::pair<NLMISC::CLine, uint8>> &edges) = 0;
   ...
   */
  //	virtual void
  //snapToInteriorGround(UGlobalPosition &pos) const = 0;

  /**
   * Converts a global position object into a 'human-readable' CVector.
   */
  virtual NLMISC::CVector
  getGlobalPosition(const UGlobalPosition &global) const = 0;

  /**
   * Converts a global position object into a 'human-readable' CVector (double
   * instead.)
   */
  virtual NLMISC::CVectorD
  getDoubleGlobalPosition(const UGlobalPosition &global) const = 0;

  /**
   * Refresh loaded retrievers around a position (one retriever is loaded at a
   * time)
   */
  virtual void refreshLrAround(const NLMISC::CVector &position,
                               float radius) = 0;

  /**
   * Refresh loaded retrievers around a position (all retrievers are updated at
   * this time -- used at startup)
   */
  virtual void refreshLrAroundNow(const NLMISC::CVector &position,
                                  float radius) = 0;

  /**
   * Create a global retriever.
   *
   * \param globalRetriver is the global retriever path file name. This method
   * use the CPath to find the file. \param retrieverBank is the global
   * retriever bank associated to the global retriever. \return the pointer on
   * the global retriever or NULL if the file is not found.
   */
  static UGlobalRetriever *
  createGlobalRetriever(const char *globalRetriever,

...
public:
  ...
  const IQuadNode *getChild(uint child) const {
    nlerror("Can't access child %d on the leaf!", child);
    return NULL;
  }
  bool check() const {
    if (!IQuadNode::check())
...
public:
  ...
  CQuadBranch(uint8 level = 0) : IQuadNode(level) {
    uint i;
    for (i = 0; i < 4; ++i)
      _Children[i] = NULL;
  }
  ...
  const IQuadNode *getChild(uint child) const {
    if (child > 3)
      nlerror("Can't access child %d on the branch", child);
    return _Children[child];
  }
  void setChild(uint child, IQuadNode *node) {
    if (child > 3)
      nlerror("Can't set child %d on the branch", child);
...
public:
  ...
  void clear();
  void init(float maxThickness, uint maxLevel, const NLMISC::CVector &center,

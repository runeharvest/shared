...
struct CCollisionFace {
  ...
  /// The number of the surface of which it is associated. -1 means exterior
  /// surface.
  sint32 Surface;
...
private:
  ...
  struct CEdgeInfo {
    ...
    CEdgeInfo(sint32 left = -1, sint32 leftEdge = -1, sint32 right = -1,
              sint32 rightEdge = -1)
        : Left(left), LeftEdge(leftEdge), Right(right), RightEdge(rightEdge) {}
  };
...
public:
  ...
  void link(bool linkInterior, std::vector<std::string> &errors) {
    ...
    for (i = 0; i < Faces.size(); ++i) {
      if ((Faces[i].Surface == CCollisionFace::ExteriorSurface &&
           linkInterior) ||
          (Faces[i].Surface >= CCollisionFace::InteriorSurfaceFirst &&
           !linkInterior))
        continue;
      ...
      for (j = 0; j < 3; ++j) {
        ...
        uint edge = (j + 2) % 3;
        uint32 va = Faces[i].V[j], vb = Faces[i].V[(j + 1) % 3];
        ...
        if ((it = relloc.find(CEdgeKey(va, vb))) != relloc.end()) {
          ...
          static char buf[512];
          sprintf(buf,
                  "Edge issue: (%.2f,%.2f,%.2f)-(%.2f,%.2f,%.2f) [left face "
                  "already found]\n"
                  "left.0:%d: v0:(%.2f,%.2f) v1:(%.2f,%.2f), v2:(%.2f,%.2f) "
                  "surf=%d material=%d\n"
                  "left.1:%d: v0:(%.2f,%.2f) v1:(%.2f,%.2f), v2:(%.2f,%.2f) "
                  "surf=%d material=%d",
                  eva.x, eva.y, eva.z, evb.x, evb.y, evb.z, (*it).second.Left,
                  Vertices[left0.V[0]].x, Vertices[left0.V[0]].y,
                  Vertices[left0.V[1]].x, Vertices[left0.V[1]].y,
                  Vertices[left0.V[2]].x, Vertices[left0.V[2]].y,
                  Faces[(*it).second.Left].Surface,
                  Faces[(*it).second.Left].Material, i, Vertices[left1.V[0]].x,
                  Vertices[left1.V[0]].y, Vertices[left1.V[1]].x,
                  Vertices[left1.V[1]].y, Vertices[left1.V[2]].x,
                  Vertices[left1.V[2]].y, Faces[i].Surface, Faces[i].Material);
          errors.push_back(std::string(buf));
          continue;
          /*					nlerror("On face %d, edge %d:
             left side of edge (%d,%d) already linked to face %d", i, edge, va,
             vb, (*it).second.Left);*/
          ...
          if ((*it).second.Right != -1) {
            ...
            sprintf(
                buf,
                "Edge issue: (%.2f,%.2f,%.2f)-(%.2f,%.2f,%.2f) [right face "
                "already found]\n"
                "right.0:%d: v0:(%.2f,%.2f) v1:(%.2f,%.2f), v2:(%.2f,%.2f)\n"
                "right.1:%d: v0:(%.2f,%.2f) v1:(%.2f,%.2f), v2:(%.2f,%.2f)",
                eva.x, eva.y, eva.z, evb.x, evb.y, evb.z, (*it).second.Right,
                Vertices[right0.V[0]].x, Vertices[right0.V[0]].y,
                Vertices[right0.V[1]].x, Vertices[right0.V[1]].y,
                Vertices[right0.V[2]].x, Vertices[right0.V[2]].y, i,
                Vertices[right1.V[0]].x, Vertices[right1.V[0]].y,
                Vertices[right1.V[1]].x, Vertices[right1.V[1]].y,
                Vertices[right1.V[2]].x, Vertices[right1.V[2]].y);
            errors.push_back(std::string(buf));
            continue;
            /*						nlerror("On face %d,
               edge %d: right side of edge (%d,%d) already linked to face %d",
                                            i, edge, vb, va,
               (*it).second.Right);*/
          }
          ...
          relloc.insert(
              std::make_pair(CEdgeKey(va, vb), CEdgeInfo(i, edge, -1, -1)));
        }
      }
    }

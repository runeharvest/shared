...
/**
 * A class that allows to retrieve surface in a large amount of zones (referred
 * as instances.) \author Benjamin Legros \author Nevrax France \date 2001
 */
...
public:
  ...
  public:
    ...
    CGlobalPosition(sint32 instanceId = -1,
                    const CLocalRetriever::CLocalPosition &localPosition =
                        CLocalRetriever::CLocalPosition()) {
    ...
    // void
    // serial(NLMISC::IStream &f) { f.serial(InstanceId, LocalPosition); }
  };
...
private:
  ...
  /// Used to retrieve the surface. Internal use only, to avoid large amount of
  /// new/delete
...
public:
   ...
   */
  CGlobalRetriever(const CRetrieverBank *bank = NULL) : _RetrieverBank(bank) {}
   ...
   * The selected instances are stored in
   * CCollisionSurfaceTemp.CollisionInstances
   */
  bool selectInstances(
      const NLMISC::CAABBox &bbox, CCollisionSurfaceTemp &cst,
      UGlobalPosition::TType type = UGlobalPosition::Unspecified) const;
  ...
  uint32 getMaterial(const UGlobalPosition &pos) const {
    ...
    if (!retriever.isLoaded() || pos.LocalPosition.Surface < 0 ||
        pos.LocalPosition.Surface >= (sint)retriever.getSurfaces().size())
      return 0xFFFFFFFF;
  ...
  bool isWaterPosition(const UGlobalPosition &pos, float &waterHeight) const {
    ...
    const CRetrievableSurface &surface =
        retriever.getSurface(pos.LocalPosition.Surface);

    waterHeight = surface.getWaterHeight();

    return (surface.getFlags() & (1 << CRetrievableSurface::IsUnderWaterBit)) !=
           0;
  }
  ...
  /// Retrieves the position of an estimated point in the global retriever.
  UGlobalPosition retrievePosition(const NLMISC::CVector &estimated,
                                   float threshold) const;

  /// Retrieves the position of an estimated point in the global retriever
  /// (double instead.)
  UGlobalPosition retrievePosition(const NLMISC::CVectorD &estimated,
  ...
  UGlobalPosition retrievePosition(const NLMISC::CVectorD &estimated,
                                   double threshold,
                                   UGlobalPosition::TType retrieveSpec) const;

  /// Retrieves the position of an estimated point in the global retriever
  /// (double instead.)
  UGlobalPosition retrievePosition(const NLMISC::CVectorD &estimated, uint h,
                                   sint &result) const;
  ...
  bool insurePosition(UGlobalPosition &pos) const {
    ...
    const CLocalRetriever &retriever =
        getRetriever(_Instances[pos.InstanceId].getRetrieverId());
  ...
  bool testPosition(UGlobalPosition &pos) const {
    ...
    if (!instance.getBBox().include(pos.LocalPosition.Estimation +
                                    instance.getOrigin()))
      return false;
  ...
  //	void
  //snapToInteriorGround(UGlobalPosition &position) const;
  ...
  /// Converts a global position object into a 'human-readable' CVector (double
  /// instead.)
  ...
  /// Creates an instance of local retriever at the origine position with the
  /// given orientation
  const CRetrieverInstance &makeInstance(uint32 retriever, uint8 orientation,
  ...
  ///
  void getBorders(const UGlobalPosition &pos,
                  std::vector<std::pair<NLMISC::CLine, uint8>> &edges);
  ///
  void getBorders(const NLMISC::CAABBox &sbox,
                  std::vector<std::pair<NLMISC::CLine, uint8>> &edges);
   ...
   * \param cst is the CCollisionSurfaceTemp object used as temp copmputing (one
   * per thread). \return list of collision against surface, ordered by
   * increasing time. this is a synonym for cst.CollisionDescs. NB: this array
   * may be modified by CGlobalRetriever on any collision call.
   */
  const TCollisionSurfaceDescVector *
  testCylinderMove(const UGlobalPosition &start, const NLMISC::CVector &delta,
   ...
   */
  const TCollisionSurfaceDescVector *
  testBBoxMove(const UGlobalPosition &start, const NLMISC::CVector &delta,
               const NLMISC::CVector &locI, const NLMISC::CVector &locJ,
               CCollisionSurfaceTemp &cst) const;
  /** apply a movement of a point against surface world. This should be called
   *after test???Move(). NB: It's up to you to give good t, relative to result
   *of test???Move(). Else, undefined results... NB: if you don't give same
   *start/delta as in preceding call to testMove(), and rebuildChains==false,
   *	start is returned (nlstop in debug).
   *
   * \param start is the start position of the movement. (must be same as passed
   *in test???Move()). \param delta is the requested movement (must be same as
   *passed in test???Move()). \param t must be in [0,1]. t*delta is the actual
   *requested movement. \param cst is the CCollisionSurfaceTemp object used as
   *temp computing (one per thread). (must be same as passed in test???Move()).
   * \param rebuildChains true if doMove() is not called just after the
   *testMove(). Then CGlobalRetriever must recompute some part of the data
   *needed to performing his task. \return new position of the entity.
   */
  UGlobalPosition doMove(const UGlobalPosition &start,
                         const NLMISC::CVector &delta, float t,
                         CCollisionSurfaceTemp &cst,
                         bool rebuildChains = false) const;
   ...
   * NB: this function is not perfect because a ContactSurface may appears 2+
   * times in the returned array. \param start is the center of the bbox. \param
   * locI is the new oriented I vector of the BBox.  I.norm()== Width/2. \param
   * locJ is the new oriented J vector of the BBox.  J.norm()== Height/2.  NB :
   * must have locI^locJ== aK (a>0) \param cst is the CCollisionSurfaceTemp
   * object used as temp copmputing (one per thread). \return list of collision
   * against surface (ContactTime and ContactNormal has no means). this is a
   * synonym for cst.CollisionDescs. NB: this array may be modified by
   * CGlobalRetriever on any collision call.
   */
  const TCollisionSurfaceDescVector &
  testBBoxRot(const CGlobalPosition &start, const NLMISC::CVector &locI,
              const NLMISC::CVector &locJ, CCollisionSurfaceTemp &cst) const;
  ...
  void findAStarPath(const UGlobalPosition &begin, const UGlobalPosition &end,
  ...
  void findPath(const UGlobalPosition &begin, const UGlobalPosition &end,
                CGlobalPath &path, uint32 forbidFlags = 0) const;
...
private:
  ...
  CRetrieverInstance::CAStarNodeInfo &
  getNode(CRetrieverInstance::CAStarNodeAccess &access) const {
  ...
  /** reset and fill cst.CollisionChains with possible collisions in
   * bboxMove+origin. result: collisionChains, computed localy to origin.
   */
  void findCollisionChains(CCollisionSurfaceTemp &cst,
                           const NLMISC::CAABBox &bboxMove,
                           const NLMISC::CVector &origin) const;
  /** reset and fill cst.CollisionDescs with effective collisions against
   * current cst.CollisionChains. result: new collisionDescs in cst.
   */
  void testCollisionWithCollisionChains(CCollisionSurfaceTemp &cst,
                                        const CVector2f &startCol,
                                        const CVector2f &deltaCol,
                                        CSurfaceIdent startSurface,
                                        float radius, const CVector2f bbox[4],
                                        TCollisionType colType) const;
  /** reset and fill cst.MoveDescs with effective collisions of a point movement
   * against current cst.CollisionChains. result: the surfaceIdent where we
   * stop. -1 if we traverse a Wall, which should not happen because of
   * collision test. NB: for precision pb, startCol and deltaCol should be
   * snapped on a grid of 1/1024 meters, using snapVector(). NB: for precision
   * pb (stop on edge etc....), return a "Precision problem ident", ie (-2,-2).
   * NB: when leaving an interior, return a surface (-3, -3) and restart is set
   * to the real restart position
   */
  CSurfaceIdent testMovementWithCollisionChains(CCollisionSurfaceTemp &cst,
                                                const CVector2f &startCol,
                                                const CVector2f &deltaCol,
                                                CSurfaceIdent startSurface,
   ...
   * current cst.CollisionChains. result: new collisionDescs in cst.
   */
  void testRotCollisionWithCollisionChains(CCollisionSurfaceTemp &cst,
                                           const CVector2f &startCol,
                                           CSurfaceIdent startSurface,
                                           const CVector2f bbox[4]) const;
  ...
  float getInteriorHeightAround(const UGlobalPosition &position,
                                float outsideTolerance) const;
  // @}

 ...
 *    v1 < v2 iff  v1.x < v2.x  ||  v1.x == v2.x && v1.y < v2.y  ||  v1.x ==
 * v2.x && v1.y == v2.y && v1.z < v2.z The vertices composing the chain are
 * actual CVector (12 bytes per vertex.) \author Benjamin Legros \author Nevrax
 * France \date 2001
 */
...
protected:
  ...
  /// Set if the chain should be read forward within the parent CChain (for
  /// sequential access to vertices.)
  bool _Forward;
...
public:
  ...
  /// Returns true if the chain should be accessed forward within the parent
  /// CChain (see _Forward.)
 ...
 * The vertices composing the chain are only 2 coordinates (x, y) wide, packed
 * on 16 bits each (which is 4 bytes per vertex.) This is the packed form of the
 * COrderedChain3f. (4 bytes per vertex.) \author Benjamin Legros \author Nevrax
 * France \date 2001
 */
class COrderedChain {
protected:
...
public:
  ...
  ///
  void traverse(sint from, sint to, bool forward,
                std::vector<CVector2s> &path) const;
...
protected:
  /// Build the whole surface from a vector of CVector and the left and right
  /// surfaces.
  void make(const std::vector<NLMISC::CVector> &vertices, sint32 left,
            sint32 right, std::vector<COrderedChain> &chains, uint16 thisId,
...
public:
  /// Constructor.
  CChain()
      : _Left(-1), _Right(-1), _StartTip(0xffff), _StopTip(0xffff),
        _Length(0.0f), _LeftLoop(0), _LeftLoopIndex(0), _RightLoop(0),
        _RightLoopIndex(0) {}
  ...
  /// Converts the surf id into the real index to the link (in the
  /// BorderChainLinks of the CRetrieverInstance.)

...
public:
   ...
   * NB: The outpout do not contains any redundant edge. A OChain appears only
   * one time in the result. \param bbox the area of interest. \param cst the
   * array of CEdgeChainEntry to fill. contain also OChainLUT, an array for
   * internal use. In: must be filled with 0xFFFF. Out: still filled with
   * 0xFFFF. \return number of edgechain found. stored in cst.EdgeChainEntries
   * (array cleared first).
   */
   ...
   * the array of CEdgeChainEntry to fill. contain also OChainLUT, an array for
   * internal use. In: must be filled with 0xFFFF. Out: still filled with
   * 0xFFFF. \return number of edgechain found. stored in cst.EdgeChainEntries
   * (array cleared first).
   */
...
private:
  void addEdgeToQuadNode(std::list<CEdgeChainEntry> &quadNode, sint ochainId,
                         sint edgeId);

...
public:
  ...
  enum TType {
    /**
     * This is a static 2d oriented bounding box. It can be oriented only on the
     * Z axis. It has a height. Collision can be performed only on its sides but
     * not on its top and bottom planes. It doesn't mode. Default value.
     */
    _2DOrientedBox = 0x0,

    /**
     * This is a movable 2d oriented cylinder. It can be oriented only on the Z
     * axis. It has a height. Collision can be performed only on its sides but
     * not on its top and bottom planes. It can move only with 3d translations.
     */
  ...
  enum TTrigger {
    ...
    /**
     * This is a one time trigger. This trigger is actived only when an object
     * enter in its volume.
     */
    EnterTrigger = 0x100,
    ...
    /**
     * This is a one time trigger. This trigger is actived only when an object
     * exit from its volume.
     */
    ExitTrigger = 0x200,

    /**
     * This is an overlap trigger. This trigger is actived each time the object
     * overlap the trigger.
     */
  ...
  /**
   * Set the attenuation of collision for this object. Default value is 1.
   * Should be between 0~1. 0, all the energy is attenuated by the collision. 1,
   * all the energy stay in the object. Used only with the flag Reflexion.
   *
   ...
   *
   * \param width is the new width size of the box. It the size of the sides
   * aligned on OX. \param depth is the new depth size of the box. It the size
   * of the sides aligned on OY.
   */
   ...
   *
   * \param height is the new height size of the box. It the size of the sides
   * aligned on OZ.
   */
   ...
   *
   * If you modify a noncollisionable primitive with this method, you must
   * evaluate in the world image where you have modify it before modify any
   * other dynamic properties in another world image.
   *
   * \param rot is the new OZ rotation in radian.
   * \param worldImage is the world image in which the primitive must be
   * oriented.
   */
   ...
   *
   * If you modify a noncollisionable primitive with this method, you must
   * evaluate in the world image where you have modified it before modify any
   * other dynamic properties in another world image.
   *
   * \param rot is the new OZ rotation in radian.
   * \param worldImage is the world image in which the primitive must be
   ...
   */
  virtual void getGlobalPosition(UGlobalPosition &pos,
                                 uint8 worldImage) const = 0;
   ...
   *
   * \param worldImage is the number of the world image where you want to insert
   * the primitive.
   */
   ...
   * This primitive must a collisionable primitive.
   *
   * \param worldImage is the number of the world image from where you want to
   ...
   * \param pos is the new global position of the primitive.
   */
  virtual void setGlobalPosition(
      const NLMISC::CVectorD &pos, uint8 worldImage,
      UGlobalPosition::TType type = UGlobalPosition::Unspecified) = 0;
   ...
   * \param pos is the new global position of the primitive.
   */
  virtual void setGlobalPosition(const UGlobalPosition &pos,
                                 uint8 worldImage) = 0;

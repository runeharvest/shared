...
public:
  ...
  void build(const CExteriorMesh &em, const CGlobalRetriever &global,
             CCollisionSurfaceTemp &cst, uint32 thisInstance);

  /** look in the quad to select a list of chain from a bbox.
   * NB: The outpout do not contains any redundant edge. An edge appears only
   * one time in the result. \param bbox the area of interest. \param cst the
   * array of CExteriorEdgeEntry to fill. contain also OChainLUT, an array for
   * internal use. In: must be filled with 0xFFFF. Out: still filled with
   * 0xFFFF. \return number of exterioredge found. stored in
   * cst.ExteriorEdgeEntries (array cleared first).
   */
  sint selectEdges(const NLMISC::CAABBox &bbox,
                   CCollisionSurfaceTemp &cst) const;

  /** look in the quad to select a list of chain from a line.
   * NB: The outpout do not contains any redundant edge. An edge appears only
   * one time in the result. \param start the starting point of the selection
   * segment. \param end the ending point of the selection segment. \param cst
   * the array of CExteriorEdgeEntry to fill. contain also OChainLUT, an array
   * for internal use. In: must be filled with 0xFFFF. Out: still filled with
   * 0xFFFF. \return number of exterioredge found. stored in
   * cst.ExteriorEdgeEntries (array cleared first).
   */
  sint selectEdges(CVector start, CVector end,
                   CCollisionSurfaceTemp &cst) const;
  ...
  // **************************
private:
  /** W*H pointers on array of CExteriorEdgeEntry indexes. NULL if no edge in
   * this quad. Each array is 1xuint16(LEN) + LEN*CExteriorEdgeEntryIndex(16
   * bits).
   */
...
private:
  ...
  void getGridBounds(sint32 &x0, sint32 &y0, sint32 &x1, sint32 &y1,

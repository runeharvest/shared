...
public:
  CEdgeCollideNode() { Next = 0xFFFFFFFF; }
};
...
public:
  /// First edgeCollideNode in the CCollisionSurfaceTemp allocator. 0xFFFFFFFF
  /// if none. This is a List of edgeCollide.
...
public:
  ...
  uint16 EdgeStart;
  /// the end edge of the ordered chain, found in this quad. "end edge" is
  /// lastEdge+1: numEdges= end-start.
  uint16 EdgeEnd;
};
...
public:
  ...
  void serial(NLMISC::IStream &f) {
    f.serial(EdgeId, ChainId, Interior, Exterior);
  }
};
...
public:
  ...
  CMoveSurfaceDesc(CRational64 t, CSurfaceIdent left, CSurfaceIdent right)
      : ContactTime(t), LeftSurface(left), RightSurface(right),
        ExteriorEdge(false) {}
...
public:
  ...
  CRotSurfaceDesc(CSurfaceIdent left, CSurfaceIdent right)
      : Tested(false), LeftSurface(left), RightSurface(right) {}
 ...
 * There should be one CCollisionSurfaceTemp per thread. This is a private
 * class, in essence. \author Lionel Berenguier \author Nevrax France \date 2001
 */
...
public:
  ...
  public:
    ...
    bool FoundCloseEdge;
    CDistanceSurface() {}
    CDistanceSurface(float distance, uint16 surface, uint16 instance,
                     bool foundCloseEdge)
        : Surface(surface), Instance(instance), Distance(distance),
          FoundCloseEdge(foundCloseEdge) {}

    bool operator()(const CDistanceSurface &a,
                    const CDistanceSurface &b) const {

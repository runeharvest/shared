...
protected:
  ...
  ///  Tells if retrievers should be read from rbank directly or streamed from
  ///  disk
  bool _LrInRBank;
...
public:
  ...
  CRetrieverBank(bool allLoaded = true)
      : _AllLoaded(allLoaded), _LrInRBank(true) {}
  ...
  const CLocalRetriever &getRetriever(uint n) const {
    ...
    /* if (!_Retrievers[n].isLoaded())
        nlwarning("Trying to access rbank '%s', retriever %d not loaded",
       _NamePrefix.c_str(), n); */
  ...
  /// Loads the retriever named 'filename' (using defined search paths) and adds
  /// it to the bank.
    ...
    */
    ...
    if (f.isReading()) {
      if (!_AllLoaded) {
        uint32 num = 0;
        f.serial(num);
        nlinfo("Presetting RetrieverBank '%s', %d retriever slots allocated",
               _NamePrefix.c_str(), num);
        ...
        uint i;
        for (i = 0; i < num; ++i) {
          std::string fname = NLMISC::CPath::lookup(
              _NamePrefix + "_" + NLMISC::toString(i) + ".lr", false, true);
          if (fname.empty())
            continue;
          ...
          try {
            ...
            nlwarning("Couldn't load retriever file '%s', %s", fname.c_str(),
                      e.what());
            _Retrievers[i].clear();
          }
        }
  ...
  void saveRetrievers(const std::string &path, const std::string &bankPrefix) {
    ...
    for (i = 0; i < _Retrievers.size(); ++i) {
      NLMISC::COFile f(NLMISC::CPath::standardizePath(path) + bankPrefix + "_" +
                       NLMISC::toString(i) + ".lr");
      f.serial(_Retrievers[i]);
    }
  }
  ...
  void saveShortBank(const std::string &path, const std::string &bankPrefix,
                     bool saveLr = true) {
    NLMISC::COFile f(NLMISC::CPath::standardizePath(path) + bankPrefix +
                     ".rbank");
  ...
  void diff(const std::set<uint> &newlr, std::set<uint> &in,
            std::set<uint> &out) {
  ...
  void loadRetriever(uint n, NLMISC::IStream &s) {
    if (_AllLoaded || n >= _Retrievers.size() || _Retrievers[n].isLoaded()) {
      nlwarning("RetrieverBank '%s' asked to load retriever %n whereas not "
                "needed, aborted",
                _NamePrefix.c_str(), n);
      return;
    }
  ...
  void setRetrieverAsLoaded(uint n) { _LoadedRetrievers.insert(n); }

  /// Unload nth retriever
  void unloadRetriever(uint n) {
    if (_AllLoaded || n >= _Retrievers.size() || !_Retrievers[n].isLoaded()) {
      nlwarning("RetrieverBank '%s' asked to unload retriever %n whereas not "
                "needed, aborted",
                _NamePrefix.c_str(), n);
      return;
    }

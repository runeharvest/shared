...
private:
...
#ifdef NL_DEBUG
    if (s > 32767 || s < -32768)
      nlerror("in CVector2s::safeCastSint16(sint64): value doesn't fit sint16 "
              "(value=%" NL_I64 "d)",
              s);
#endif
    return (sint16)s;
  ...
  static sint16 safeCastSint16(float f) {
    ...
    sint64 s = (sint64)f;
    if (s > 32767 || s < -32768)
      nlerror("in CVector2s::safeCastSint16(float): value doesn't fit sint16 "
              "(value=%f)",
              f);
    return (sint16)s;
#else
  ...
  static bool checkCastSint16(sint64 s) { return (s <= 32767 && s >= -32768); }
  ...
  static sint16 pack(float f) { return safeCastSint16(f * Vector2sAccuracy); }

  // unpack a fixed16 into a float
  static float unpack(sint16 s) { return (float)s / Vector2sAccuracy; }
...
public: // Methods.
  ...
  CVector2s operator+(const CVector2s &v) const {
    return CVector2s(safeCastSint16((sint64)x + (sint64)v.x),
                     safeCastSint16((sint64)y + (sint64)v.y));
  }
  CVector2s operator-(const CVector2s &v) const {
    return CVector2s(safeCastSint16((sint64)x - (sint64)v.x),
                     safeCastSint16((sint64)y - (sint64)v.y));
  ...
  CVector2s operator-() const {
    return CVector2s(safeCastSint16(-(sint64)x), safeCastSint16(-(sint64)y));
  }

  CVector2s &operator*=(float f) {
    x = safeCastSint16(f * x);
    y = safeCastSint16(f * y);
    return *this;
  }
  CVector2s &operator/=(float f) {
    x = safeCastSint16(f / x);
  ...
  CVector2s operator*(float f) const {
    return CVector2s(safeCastSint16(x * f), safeCastSint16(y * f));
  }
  CVector2s operator/(float f) const {
    return CVector2s(safeCastSint16(x / f), safeCastSint16(y / f));
  }
  ...
  float operator*(const CVector2s &v) const {
    return ((float)x * (float)v.x + (float)y * (float)v.y) /
           (Vector2sAccuracy * Vector2sAccuracy);
  }
  ...
  NLMISC::CVector2f unpack() const {
    return NLMISC::CVector2f(unpack(x), unpack(y));
  }
  NLMISC::CVector unpack3f(float hintz = 0.0f) const {
    return NLMISC::CVector(unpack(x), unpack(y), hintz);
  }

  /// Test if 2 segments intersects
  static bool intersect(const CVector2s &a0, const CVector2s &a1,
                        const CVector2s &b0, const CVector2s &b1,

...
public:
  ...
  public:
    /**
     * A chain tip. Contains the id of the chain and whether this tip is the
     * start of the chain. \author Benjamin Legros \author Nevrax France \date
     * 2001
     */
    struct CChainTip {
      /// The id of the chain.
  ...
  public:
    ...
    void translate(const NLMISC::CVector &translation) { Point += translation; }
  };

  /**
   * A topology. It's a set of surfaces which are definning a connex master
   * surface. \author Benjamin Legros \author Nevrax France \date 2001
   */
  ...
  public:
    /// Constructor.
    CLocalPosition(sint32 surface = -1,
    ...
    // void
    // serial(NLMISC::IStream &f) { f.serial(Surface, Estimation); }
  };
  ...
  public:
    void serial(NLMISC::IStream &f) {
      f.serial(Verts[0], Verts[1], Verts[2], Surface);
    }
  };

  /**
   * An iterator to go through chains without bothering about ordered chains and
   * those kind of f*cking stuffs \author Benjamin Legros \author Nevrax France
   * \date 2003
   */
  class CIterator {
  ...
  public:
    ...
    CIterator &operator++() {
      ...
      if (_OChainForward) {
        ++_IndexInOChain;
        if (((sint)_OChainIndex < (sint)chain.getSubChains().size() - 1 &&
             _IndexInOChain == _MaxIndexInOChain) ||
            _IndexInOChain > _MaxIndexInOChain) {
          ++_OChainIndex;
          setupIndex();
        }
    ...
    CVector2s operator*() const {
      if (end())
        return CVector2s();
      return _Retriever->getOrderedChain(
          _Retriever->getChain(_Chain).getSubChain(
              _OChainIndex))[_IndexInOChain];
    }

    CVector get3d() const {
      if (end())
        return CVector::Null;
      return _Retriever->getFullOrderedChain(
          _Retriever->getChain(_Chain).getSubChain(
              _OChainIndex))[_IndexInOChain];
    }
    ...
    bool end() const {
      return (_OChainIndex >=
              _Retriever->getChain(_Chain).getSubChains().size());
    }
  ...
  private:
    ...
    void setupIndex() {
      ...
      const COrderedChain &ochain =
          _Retriever->getOrderedChain(chain.getSubChain(_OChainIndex));
...
private:
  ...
  struct CIntersectionMarker {
    ...
    CIntersectionMarker(float position, uint16 ochain, uint16 edge, bool in)
        : Position(position), OChain(ochain), Edge(edge), In(in) {}
...
public:
  ...
  void
  buildInteriorSurfaceBBoxes(std::vector<NLMISC::CAABBox> &surfaceBBoxes) const;

  /// Builds the polygons (loops) of a given surface
  void buildSurfacePolygons(uint32 surface,
                            std::list<NLMISC::CPolygon> &polygons) const;

  /// Builds the polygons (loops) of all surfaces in the retriever
  void
  buildSurfacePolygons(std::list<std::list<NLMISC::CPolygon>> &polygons) const {
    uint i;
  ...
  /// Builds the polygons (loops) of a given surface
  void build3dSurfacePolygons(uint32 surface,
                              std::list<NLMISC::CPolygon> &polygons) const;

  /// Builds the polygons (loops) of all surfaces in the retriever
  void build3dSurfacePolygons(
      std::list<std::list<NLMISC::CPolygon>> &polygons) const {
    uint i;
    for (i = 0; i < _Surfaces.size(); ++i) {
      polygons.push_back(std::list<NLMISC::CPolygon>());
  ...
  /// Adds a surface to the local retriever, using its features. Returns the id
  /// of the newly created surface.
  sint32 addSurface(uint8 normalq, uint8 orientationq, uint8 mat, uint8 charact,
                    uint8 level, bool isUnderWater, float waterHeight,
                    bool clusterHint, const NLMISC::CVector &center,
                    const CSurfaceQuadTree &quad, sint8 quantHeight = 0);
   ...
   */
  sint32 addChain(const std::vector<NLMISC::CVector> &vertices, sint32 left,
                  sint32 right);
  ...
  /// \name  Collisions part.
  // @{
  /// compute the chain quad, used for collisions. the ChainQuad is serialised
  /// in serial(). _OrderedChains must be OK.
  void computeCollisionChainQuad();
  /** add possible collisions chains to the temp result.
   * \param cst the temp result to store collision chains. they are appened to
   * cst.CollisionChains. \param bboxMove the bbox which bounds the movement of
   * the entity. \param transBase the vector we use to translate local position
   * of edge.
   */
  void testCollision(CCollisionSurfaceTemp &cst,
                     const NLMISC::CAABBox &bboxMove,
                     const NLMISC::CVector2f &transBase) const;

  /**
   * Check all surfaces integrity
   */
  bool
  checkSurfacesIntegrity(NLMISC::CVector translation = NLMISC::CVector::Null,
   ...
   */
  bool
  checkSurfaceIntegrity(uint surf,
                        NLMISC::CVector translation = NLMISC::CVector::Null,
                        bool verbose = false) const;
  ...
  */
  ...
  /// Assures a position is really inside a surface (and independent from any
  /// accuracy issue) and returns true if the position was moved.
  bool insurePosition(ULocalPosition &local) const;

  /// Retrieves a position inside the retriever (from the local position),
  /// returns true if the position is close to a border
  void retrievePosition(NLMISC::CVector estimated,
                        CCollisionSurfaceTemp &cst) const;

  /// Retrieves a position inside the retriever (from the local position),
  /// returns true if the position is close to a border
  void retrieveAccuratePosition(CVector2s estimated, CCollisionSurfaceTemp &cst,
                                bool &onBorder) const;
  ...
  void findPath(const CLocalPosition &A, const CLocalPosition &B,
  ...
  /** Get the height of interior surface that matches the best this position.
   *Allows the position to be a bit outside the triangles. return 0 if not
   *loaded or not an interior surface. \param outsideTolerance how much are we
   *allowed to look outside the triangles
   */
  float getInteriorHeightAround(const ULocalPosition &position,
                                float outsideTolerance) const;

public:
  // Last minute fix... Some issues appeared when linking (trying to link 2
  // chains with only 1 in the neighbourhood)
  ...
  void replaceChain(uint32 chainId,
  ...
  void forceBorderChainId(uint32 chainId, uint32 borderId) {
    if (chainId >= _Chains.size()) {
      nlwarning("forceBorderChainId(): couldn't force border id %d for chain "
                "%d, doesn't exist",
                borderId, chainId);
      return;
    }
...
public:
  ...
  /*
      uint16
     getStartTip(uint32 chain, sint32 surface) const; uint16
     getStopTip(uint32 chain, sint32 surface) const;

      void
     setStartTip(uint32 chain, sint32 surface, uint16 startTip); void
     setStopTip(uint32 chain, sint32 surface, uint16 stopTip);
  */
  uint32 getPreviousChain(uint32 chain, sint32 surface) const;
  uint32 getNextChain(uint32 chain, sint32 surface) const;

public:
  void dumpSurface(uint surf,
                   const NLMISC::CVector &vect = NLMISC::CVector::Null) const;

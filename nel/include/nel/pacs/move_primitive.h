...
private:
  ...
  enum TStaticFlags {
    ...
    // Obstacle flag. This flag tells that this object is an obstacle for others
    // objects.
    ObstacleFlag = 0x1000,
...
public:
  /// Constructor
  CMovePrimitive(CMoveContainer *container, uint8 firstWorldImage,
                 uint8 numWorldImage);
  ...
  CPrimitiveWorldImage *getWorldImage(uint i) const {
    ...
    nlassert((i >= (uint)_FirstWorldImage) &&
             (i < (uint)_FirstWorldImage + (uint)_NumWorldImage));
  ...
  uint8 getNumWorldImage() const { return _NumWorldImage; }
   ...
   */
  void setCollisionMask(TCollisionMask mask) { _CollisionMask = mask; }
   ...
   */
  void setOcclusionMask(TCollisionMask mask) { _OcclusionMask = mask; }
  ...
  /// Get length
  float getLength(uint where) const { return _Length[where]; }

  /// Get height
  float getHeightInternal() const { return _Height; }

  /// Get length
  float getRadiusInternal() const { return _Length[0]; }

  /// Is an obstacle ?
  bool isObstacle() const { return (_StaticFlags & ObstacleFlag) != 0; }

  /// Is collisionable
  bool isCollisionable() const { return !isNonCollisionable(); }
  ...
  void
  setGlobalPosition(const NLMISC::CVectorD &pos, uint8 worldImage,
  ...
  // Test time. Return true if tetst can be perform, false if too many test have
  // been computed for this primitive
  bool checkTestTime(uint32 testTime, uint32 maxTestIteration) {
    ...
    if (testTime != _LastTestTime) {
      ...
      if (_IterationCount <= 0) {
        //				nlwarning("in checkTestTime(): Get to

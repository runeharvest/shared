...
public:
   ...
   */
  void setGlobalPosition(const UGlobalPosition &pos, CMoveContainer &container,
   ...
   */
  void
  setGlobalPosition(const NLMISC::CVectorD &pos, CMoveContainer &container,
                    CMovePrimitive &primitive, uint8 worldImage,
                    bool keepZ = false,
   ...
   */
  void setOrientation(double rot, CMoveContainer *container,
                      CMovePrimitive *primitive, uint8 worldImage) {
    // New position
   ...
   */
  void move(const NLMISC::CVectorD &speed, CMoveContainer &container,
            CMovePrimitive &primitive, uint8 worldImage);
   ...
   */
  void setSpeed(const NLMISC::CVectorD &speed, CMoveContainer *container,
                CMovePrimitive *primitive, uint8 worldImage) {
    // New time
    _Speed = speed;
  ...
  void linkInModifiedList(CMovePrimitive *next) { _NextModified = next; }

  /// Is in modified list ?
  bool isInWorldImageFlag() { return (_DynamicFlags & InWorldImageFlag) != 0; }
  ...
  CMovePrimitive *getNextModified() const { return _NextModified; }

  /**
   * Return min of the bounding box in X.
   */
  double getBBXMin() const { return _BBXMin; }

  /**
   * Return min of the bounding box in Y.
   */
  double getBBYMin() const { return _BBYMin; }

  /**
   * Return max of the bounding box in X.
   */
  double getBBXMax() const { return _BBXMax; }

  /**
   * Return max of the bounding box in Y.
   */
  double getBBYMax() const { return _BBYMax; }

  /**
   * Return the delta position
   */
  const NLMISC::CVectorD &getDeltaPosition() const { return _DeltaPosition; }

  /// Return the nieme MoveElement. The primitive can have 4 move elements. Can
  /// be NULL if the ineme elment is not in a list
  CMoveElement *getMoveElement(uint i) { return _MoveElement[i]; }
  ...
  void addMoveElement(CMoveCell &cell, uint16 x, uint16 y, double centerX,
                      double centerY, CMovePrimitive *primitive,
                      CMoveContainer &container, uint8 worldImage);

  /// Add the primitive in the cell at the end of the list and don't sort
  void addMoveElementendOfList(CMoveCell &cell, uint16 x, uint16 y,
  ...
  double getOrientation() const { return _OBData.Orientation; }

  // Dirt the position flag. Position has changed.
  void dirtPos(CMoveContainer *container, CMovePrimitive *primitive,
               uint8 worldImage) {
    _DynamicFlags |= DirtPosFlag;
  ...
  void dirtBB(CMoveContainer *container, CMovePrimitive *primitive,
              uint8 worldImage) {
  ...
  double getInitTime() const { return _InitTime; }
  ...
  */
   ...
   *
   * \return true if a collision has been detected in the time range, else
   * false.
   */
  bool evalCollision(CPrimitiveWorldImage &other, class CCollisionDesc &desc,
                     double timeMin, double timeMax, uint32 testTime,
                     uint32 maxTestIteration, double &firstContactTime,
                     double &lastContactTime, CMovePrimitive &primitive,
                     CMovePrimitive &otherPrimitive);

  // Box over box
  bool evalCollisionOBoverOB(CPrimitiveWorldImage &other, CCollisionDesc &desc,
  ...
  // Box over cylinder
  bool evalCollisionOBoverOC(CPrimitiveWorldImage &other, CCollisionDesc &desc,
                             double timeMin, double timeMax,
                             double &firstContactTime, double &lastContactTime,
                             CMovePrimitive &primitive,
                             CMovePrimitive &otherPrimitive);
  ...
  bool evalCollisionOCoverOC(CPrimitiveWorldImage &other, CCollisionDesc &desc,
                             double timeMin, double timeMax,
                             double &firstContactTime, double &lastContactTime,
                             CMovePrimitive &primitive,
                             CMovePrimitive &otherPrimitive);
  ...
  bool evalCollisionPoverS(CPrimitiveWorldImage &other, CCollisionDesc &desc,
                           uint numPoint, uint numSeg,
                           CMovePrimitive &primitive,
                           CMovePrimitive &otherPrimitive);

  // Point over cylinder
  bool evalCollisionPoverOC(CPrimitiveWorldImage &other, CCollisionDesc &desc,
                            uint numPoint, double &firstContactTime,
                            double &lastContactTime, CMovePrimitive &primitive,
                            CMovePrimitive &otherPrimitive);

  // Segment over cylinder
  bool evalCollisionSoverOC(CPrimitiveWorldImage &other, CCollisionDesc &desc,
                            uint numPoint, CMovePrimitive &primitive,
                            CMovePrimitive &otherPrimitive);
  ...
  evalCollision(CGlobalRetriever &retriever, CCollisionSurfaceTemp &surfaceTemp,
                uint32 testTime, uint32 maxTestIteration,
                CMovePrimitive &primitive);

  // Make a move with globalRetriever. Must be call after a free collision
  // evalCollision call.
  void doMove(CGlobalRetriever &retriever, CCollisionSurfaceTemp &surfaceTemp,
              double originalMax, double finalMax, bool keepZ = false);

  // Make a move wihtout globalRetriever.
  void doMove(double timeMax);

  // Reaction between two primitives. Return true if one object has been
  // modified.
  void reaction(CPrimitiveWorldImage &second, const CCollisionDesc &desc,
                CGlobalRetriever *retriver, CCollisionSurfaceTemp &surfaceTemp,
                bool collision, CMovePrimitive &primitive,
                CMovePrimitive &otherPrimitive, CMoveContainer *container,
                uint8 worldImage, uint8 secondWorldImage, bool secondConst);

  // Reaction with a static collision. Return true if one object has been
  // modified.
  void reaction(const CCollisionSurfaceDesc &surfaceDesc,
                const UGlobalPosition &globalPosition,
                CGlobalRetriever &retriever, double ratio, double dt,
                CMovePrimitive &primitive, CMoveContainer &container,
                uint8 worldImage);
...
private:
  ...
  enum TDynamicFlags {
    ...
    // The dirt bounding box. Precalculated data for the bounding box must be
    // recomputed.
    DirtBBFlag = 0x0002,
  ...
  // This position is the central bottom position for the box or for the
  // cylinder
  class CPosition {
  private:
    // 3d position
  ...
  public:
    ...
    const NLMISC::CVectorD &getPos() const { return _3dPosition; }

    // Set the 3d position
    void setPos(const NLMISC::CVectorD &newPos) { _3dPosition = newPos; }

    // Return the global position
    const UGlobalPosition &getGlobalPos() const { return _GlobalPosition; }
    ...
    void setGlobalPos(const UGlobalPosition &globalPosition,
      ...
      _GlobalPosition.LocalPosition.Estimation.z =
          globalRetriver.getMeanHeight(globalPosition);
    ...
    void setGlobalPosKeepZ(const UGlobalPosition &globalPosition,
      ...
      NLMISC::CVectorD dPos =
          globalRetriver.getDoubleGlobalPosition(globalPosition);
  ...
  // Time for valid getPos () position.  _Position.getPos () == _3dInitPosition
  // + _Speed * _InitTime;
  double _InitTime;
  ...
  // Sorted list of modified primitives. This pointer is managed by
  // CMoveContainer.
  CMovePrimitive *_NextModified;

  // Pointer into the 4 possibles sorted lists of movable primitives. Can be

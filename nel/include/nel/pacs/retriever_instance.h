...
/// Precision of Snap. 1/1024 meter. If you change this,
/// CEdgeCollide::testPointMove() won't work.
...
public:
  ...
  public:
    ...
    void serial(NLMISC::IStream &f) {
      f.serial(Instance, BorderChainId, ChainId, SurfaceId);
    }
  };
...
protected:
  ...
  struct CAStarNodeAccess {
    ...
    bool operator==(const CAStarNodeAccess &node) const {
      return InstanceId == node.InstanceId && NodeId == node.NodeId;
    }
    bool operator!=(const CAStarNodeAccess &node) const {
      return InstanceId != node.InstanceId || NodeId != node.NodeId;
    }
...
public:
  ...
  /// Resets links of the instance. This doesn't affect any neighboring
  /// instances...
  void resetLinks();
  ...
  void forceBorderChainLink(uint border, uint frontInstance, uint frontBorder,
                            uint frontChain, uint frontSurface) {
    if (border >= _BorderChainLinks.size()) {
      nlwarning("forceBorderChainLink(): couldn't force border %d on instance "
                "%d, doesn't exist!",
                border, _InstanceId);
      return;
    }
    ...
    if (_BorderChainLinks[border].Instance != 0xffff ||
      ...
      nlwarning("forceBorderChainLink(): forces border %d on instance %d "
                "whereas link is initialised yet, incoherences may appear!",
                border, _InstanceId);
    }
  ...
  /// Returns the number of the edge on the instance corresponding to the edge
  /// on the retriever.
  uint8 getInstanceEdge(uint8 retrieverEdge) const {
    return (retrieverEdge + _Orientation) % 4;
  }
  /// Returns the number of the edge on the retriever corresponding to the edge
  /// on the instance.
  uint8 getRetrieverEdge(uint8 instanceEdge) const {
    return (instanceEdge + 4 - _Orientation) % 4;
  }

  /// Returns the bbox of the instance (using the translation and orientation of
  /// the retriever)
  ...
  /** Inits the edgequad (only for Interior instances, and only after all the
   * landscape instances have been built.
   */
  ...
  void make(sint32 instanceId, sint32 retrieverId,
            const CLocalRetriever &retriever, uint8 orientation,
   ...
   * WARNING: the estimated position is a GLOBAL position, and the returned
   * position is a LOCAL position (to the retriever).
   */
  //	CLocalRetriever::CLocalPosition		retrievePosition(const
  //NLMISC::CVector &estimated, const CLocalRetriever &retriever,
  //CCollisionSurfaceTemp &cst) const;
  void retrievePosition(const NLMISC::CVector &estimated,
                        const CLocalRetriever &retriever,
                        CCollisionSurfaceTemp &cst,
                        bool sortByDistance = true) const;
  /**
   * Retrieves the position in the instance from an estimated position (double
   * instead.) WARNING: the estimated position is a GLOBAL position, and the
   * returned position is a LOCAL position (to the retriever).
   */
  void retrievePosition(const NLMISC::CVectorD &estimated,
                        const CLocalRetriever &retriever,
                        CCollisionSurfaceTemp &cst,
                        bool sortByDistance = true) const;
  /**
   ...
   */
  // void
  // retrieveAccuratePosition(const NLMISC::CVectorD &estimated, const
  // CLocalRetriever &retriever, CCollisionSurfaceTemp &cst, ) const;

  /// For the interior instances, snaps the point to the ground.
  void snapToInteriorGround(ULocalPosition &position,
                            const CLocalRetriever &retriever) const;
  ...
  /// Computes the position in the local axis of the retriever from a global
  /// position.
  NLMISC::CVector getLocalPosition(const NLMISC::CVector &globalPosition) const;
  /// Computes the position in the local axis of the retriever from a global
  /// position.
  NLMISC::CVector
  getLocalPosition(const NLMISC::CVectorD &globalPosition) const;
  /// Computes the position in the global axis from a local position (in the
  /// retriever axis).
  NLMISC::CVector getGlobalPosition(const NLMISC::CVector &localPosition) const;
  /// Computes the position (as double) in the global axis from a local position
  /// (in the retriever axis).
  NLMISC::CVectorD
  getDoubleGlobalPosition(const NLMISC::CVector &localPosition) const;

  /// Returns the bbox of the instance.
  //	NLMISC::CAABBox						getBBox() {
  //return _BBox; }

  /// \name Vector Snapping.
  // @{
  /** Snap a vector at 1mm (1/1024). v must be a local position (ie range from
   * -80 to +240). Doing this, we are sure we have precision of 9+10 bits, which
   * is enough for 24 bits float precision. NB: z is not snapped.
   */
  ...
  /** Snap a vector at 1mm (1/1024). v must be a local position (ie range from
   * -80 to +240). Doing this, we are sure we have precision of 9+10 bits, which
   * is enough for 24 bits float precision.
   */

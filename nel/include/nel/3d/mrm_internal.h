...
struct CLinearEquation {
  ...
  void clear() { NLMISC::contReset(Elts); }
};
...
// ***************************************************************************
/**
 * An internal mesh vertex attribute (UV, color, normal...) representation for
 * MRM building. \author Lionel Berenguier \author Nevrax France \date 2000
 */
...
struct CMRMEdgeFace : public CMRMEdge {
  ...
  CMRMEdgeFace(const CMRMEdge &e, CMRMFaceBuild *f) : CMRMEdge(e) { Face = f; }
};
...
public:
  ...
  sint getAssociatedEdge(const CMRMEdge &edge) const {
    sint v0 = edge.v0;
    sint v1 = edge.v1;
    if (Corner[0].Vertex == v0 && Corner[1].Vertex == v1)
      return 0;
    if (Corner[0].Vertex == v1 && Corner[1].Vertex == v0)
      return 0;
    if (Corner[1].Vertex == v0 && Corner[2].Vertex == v1)
      return 1;
    if (Corner[1].Vertex == v1 && Corner[2].Vertex == v0)
      return 1;
    if (Corner[0].Vertex == v0 && Corner[2].Vertex == v1)
      return 2;
    if (Corner[0].Vertex == v1 && Corner[2].Vertex == v0)
      return 2;
    return -1;
  }
  ...
  CMRMEdge getEdge(sint eId) const {
    if (eId == 0)
      return CMRMEdge(Corner[0].Vertex, Corner[1].Vertex);
    if (eId == 1)
      return CMRMEdge(Corner[1].Vertex, Corner[2].Vertex);
    if (eId == 2)
      return CMRMEdge(Corner[2].Vertex, Corner[0].Vertex);
    nlstop;
    return CMRMEdge(-1, -1);
  }
  ...
  bool hasVertex(sint numvertex) {
    return Corner[0].Vertex == numvertex || Corner[1].Vertex == numvertex ||
           Corner[2].Vertex == numvertex;
  }
  ...
  sint getAssociatedWedge(sint attribId, sint numvertex) {
    if (Corner[0].Vertex == numvertex)
      return Corner[0].Attributes[attribId];
    if (Corner[1].Vertex == numvertex)
      return Corner[1].Attributes[attribId];
    if (Corner[2].Vertex == numvertex)
      return Corner[2].Attributes[attribId];
    return -1;
  }
};
...
class CMRMSewingMesh {
  struct CLod {
    // A list of edge that must be collapsed for this Lod. NB: sorted from first
    // to collapse to last to collapse
...
public:
  ...
  /** >=0 if the lod has this edge to collapse. -1 else. NB: order of collapse
   *is returned. \param vertToCollapse is the vertex id which must be collapsed
   *to the other (ie the one which moves/dissapear)
   */
  sint mustCollapseEdge(uint lod, const CMRMEdge &edge,
                        uint &vertToCollapse) const;

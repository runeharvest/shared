...
public:
   ...
   *
   * If the tile is in the I-ne gourp, the flag 1<<I is set. There are 12
   * groups.
   */
...
private:
  // internal use
  static void intersect(const std::set<sint32> &setSrc1,
...
public:
  CTileSetTransition() { _Tile = -1; }
  sint32 getTile() const { return _Tile; }
  void serial(NLMISC::IStream &f);
...
public:
  ...
  sint32 getWidth() const { return _Width; }
  sint32 getHeight() const { return _Height; }
  void rotate();

  static bool allAlphaSet(const CTileBorder &border, TBorder where, int &pixel,
                          int &composante);
  static bool compare(const CTileBorder &border1, const CTileBorder &border2,
                      TBorder where1, TBorder where2, int &pixel,
                      int &composante);
...
public:
  ...
  void clearTransition(TTransition transition, CTile::TBitmap type,
                       CTileBank &bank);
  ...
  void setTile128(int indexInTileSet, const std::string &name,
                  CTile::TBitmap type, CTileBank &bank);
  void setTile256(int indexInTileSet, const std::string &name,
                  CTile::TBitmap type, CTileBank &bank);
  void setTileTransition(TTransition transition, const std::string &name,
                         CTile::TBitmap type, CTileBank &bank,
                         const CTileBorder &border);
  void setTileTransitionAlpha(TTransition transition, const std::string &name,
  ...
  // For edition: change the tileVegetableDesc. NB: only the
  // TileVegetableDescFileName is serialised.
  void setTileVegetableDesc(const CTileVegetableDesc &tvd);
  /** try to load the vegetable tile desc associated with the fileName (nlinfo()
   *if can't) lookup into CPath. no-op if string empty
   */
  ...
  TError checkTile256(CTile::TBitmap type, const CTileBorder &border,
                      int &pixel, int &composante);
  TError checkTileTransition(TTransition transition, CTile::TBitmap type,
                             const CTileBorder &border, int &indexError,
                             int &pixel, int &composante);

  // get
  const std::string &getName() const;
  bool getOriented() const { return _Oriented; }
  sint getNumTile128() const { return (sint)_Tile128.size(); }
  sint getNumTile256() const { return (sint)_Tile256.size(); }
  sint32 getTile128(sint index) const { return _Tile128[index]; }
  sint32 getTile256(sint index) const { return _Tile256[index]; }
  ...
  static TTransition getTransitionTile(TFlagBorder top, TFlagBorder bottom,
                                       TFlagBorder left, TFlagBorder right);
  TTransition getExistingTransitionTile(TFlagBorder _top, TFlagBorder _bottom,
...
private:
  ...
  // Delete 128 and 256 borders if no more valid texture file name for each
  // bitmap type.
...
public:
  ...
  // Get
  sint getLandCount() const { return (sint)_LandVector.size(); }
  const CTileLand *getLand(int landIndex) const {
    return &_LandVector[landIndex];
  }
  CTileLand *getLand(int landIndex) { return &_LandVector[landIndex]; }
  sint getTileSetCount() const { return (sint)_TileSetVector.size(); }
  const CTileSet *getTileSet(int tileIndex) const {
    return &_TileSetVector[tileIndex];
  }
  CTileSet *getTileSet(int tileIndex) { return &_TileSetVector[tileIndex]; }
  sint getTileCount() const { return (sint)_TileVector.size(); }
  const CTile *getTile(int tileIndex) const { return &_TileVector[tileIndex]; }
  CTile *getTile(int tileIndex) { return &_TileVector[tileIndex]; }
   ...
   * \param tileSet will receive the tile set number in which the tile is. -1 if
   * the tile is not used. \param number will receive the number of the tile in
   * the tileset. \param type is the type of tile.
   */
  ...
  /**
   *	you should call this method, after serialising the TileBank, and before
   *CLandscape::initTileBanks() You must call CLandscape::initTileBanks() after
   *calling this method for each tileSet call CTileSet::loadTileVegetableDesc()
   */
  void loadTileVegetableDescs();

  /**
   *	register all CVegetable to the Manager. called by
   *CLandscape::initTileBanks()
   */
  ...
  /// This method change ".tga" of texture filename, to ".dds". Do this only for
  /// Additive and Diffuse part (not alpha).
  void makeAllExtensionDDS();
  void setAbsPath(const std::string &newPath) { _AbsPath = newPath; }
  const std::string &getAbsPath() const { return _AbsPath; }
...
private:
  struct CTileXRef {
    CTileXRef() { _XRefTileType = undefined; }

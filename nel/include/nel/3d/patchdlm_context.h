...
public:
  // Diffuse Color of the Spot, pre-modulated with landscape
  // PointLightDiffuseMaterial. 0..255
  float R, G, B;
  // Is this a spot? NB: if false, cosMin/cosMax are still well computed for
  // correctLighting (cosMax=-1, cosMin= -2).
  bool IsSpot;
  ...
  // The estimated sphere which englobe the light. NB: approximated for
  // SpotLight
  NLMISC::CBSphere BSphere;
  // The BBox which englobe the light. NB: Sphere and Box are best fit to the
  // light, ie bbox may not englobe the sphere and vice versa
  NLMISC::CAABBox BBox;

public:
  // compile from a pointlight. NB: attenuation end is clamped to maxAttEnd
  // (must be >0)
  void compile(const CPointLight &pl, NLMISC::CRGBA landDiffMat,
...
public:
  ...
  /// texture coordinate bound in 8 bits. Important for Vegetable special DLM
  /// mapping
  uint8 MinU8, MaxU8;
  uint8 MinV8, MaxV8;

  /** Lighting Process: number of light contribution to this patch in last
   *rneder, and in cur Render. Modified by CPatch and CLandscape
   */
...
public:
  ...
  /** generate: link to patch and allocate texture space, compile vertices
   *etc... The context is also linked to the list, and removed automaticllay at
   *dtor Texture space is filled with black (RAM only) \return false if cannot
   *allocate texture space for this patch
   */
  bool generate(CPatch *patch, CTextureDLM *textureDLM,
   ...
   *	\param compType say if, before writing to the texture, the lightmap is
   *modulated with _Patch TileColor, _Patch textureFar (precomputed in the
   *context), modulate with a constant or not modulated at all \param
   *modulateCte used only if compType==ModulateConstant. this is the cte to be
   *modulate by lightmap before copy to texture
   */
...
private:
  ...
  struct CCluster {
    ...
    NLMISC::CBSphere BSphere;
    // If cluster not clipped, how many cluster to skip. NB: if NSkips==0, then
    // it is a leaf cluster.
    uint NSkips;
...
// If resolution of the lightmap is every 2x2 tiles, must bkup tileColors.
  ...
  /** The TextureFar (at tile level or less), which is always computed at
   *generate(). NB: Real compute is TextureFar*UserColor, so it need only to be
   *modulated by lightmap each frame.
   */
...
private:
  ...
  static const CRGBA *computeTileFarSrcDeltas(sint nRot, bool is256x256,
                                              uint8 uvOff,
                                              const CRGBA *srcPixel,
                                              sint &srcDeltaX, sint &srcDeltaY);
  static void copyTileToTexture(const CRGBA *srcPixel, sint srcDeltaX,
                                sint srcDeltaY, CRGBA *dstPixel,
                                uint dstStride);
  static void blendTileToTexture(const CRGBA *srcPixel, sint srcDeltaX,
                                 sint srcDeltaY, CRGBA *dstPixel,
                                 uint dstStride);
};

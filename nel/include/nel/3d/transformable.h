...
public:
  enum TTransformMode {
    ...
    RotEuler, // Matrix is computed from sperated composantes, with euler
              // rotation.
...
public:
   ...
   * This return true either if the matrix components (pos/rot etc...) are
   * touched, or if matrix is newer than caller date.
   */
  ...
  /// Work only in Rot* mode(nlassert).
  void setPos(float x, float y, float z) { setPos(CVector(x, y, z)); }
  ...
  /// Work only in Rot* mode (nlassert).
  void setScale(float scale) { setScale(CVector(scale, scale, scale)); }
  /// Work only in Rot* mode (nlassert).
  ...
  /// Work only in Rot* mode (nlassert).
  void setPivot(float x, float y, float z) { setPivot(CVector(x, y, z)); }
  ...
  TTransformMode getTransformMode() { return _Mode; }
  /// get the current rotorder (information vlaid only when RotEuler mode).
  CMatrix::TRotOrder getRotOrder() { return _RotOrder; }
  ...
  // @{
  /**
   * Setup Matrix by the lookAt method. Work only in DirectMatrix mode and
   * RotQuat mode (not euler...).
   *
  ...
  /// From IAnimatable. Deriver must implement this (channels may be detail-ed
  /// or not).

...
public:
  ...
  public:
    ...
    void serial(NLMISC::IStream &f) { f.serial(Pos, Normal); }
  };
...
public:
   ...
   *	\param triangleSelection. If not same size as triangles, not used. Else
   *texture info is filled only with triangles whose their
   *triangleSelection[triId]==true \param textureOverSample is rounded to the
   *best square (4,9,...). Prefer a srq(oddVal): 1,9,25,49 etc... NB:
   *overSamples are not averaged, but the nearest sample to the texel center is
   *taken.
   */
  void compile(const std::vector<bool> &triangleSelection,
...
// ***************************************************************************
/**
 * A very Small Shape with anims encoded as Key Meshes. Used for Lod of skinned
 *meshes NB: normals are not skinned (for anim size consideration). \author
 *Lionel Berenguier \author Nevrax France \date 2002
 */
...
public:
   ...
   *	NB: SkinWeights array tells for each vertex what bone to use (for alpha
   *test removing)
   */
  void buildMesh(const std::string &name,
                 const CLodCharacterShapeBuild &lodBuild);

  /** Add an animation. many nlAssert to verify array size etc... return false,
   *if same AnimName exist. NB: the entire animation is compressed to CVector3s
   *internally.
   */
  ...
  const std::vector<TLodCharacterIndexType> &getTriangleIndices() const {
   ...
   *	This system is used for example to remove weapon of a Lod, using
   *AlphaTesting (ie the polygons of the weapons are still drawn, but with
   *alpha==0 => invisible)
   */
  // @{

  /// init the process by resize-ing a tmp uint8 vector of getNumVertices()
  /// size, and reset to 0
   ...
   *	\param unPackScaleFactor return value is the scale factor which to
   *multiply \return NULL if animId is not valid
   */
  const CVector3s *getAnimKey(uint animId, TGlobalAnimationTime time,
                              bool wrapMode, CVector &unPackScaleFactor) const;

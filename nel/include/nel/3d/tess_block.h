...
public:
  /*
      Cache optim:
      Important to store them here for better Cache access in
     CPatch::MasterBlock (near too TessBlocks)
  */
  // FaceVectors.
...
private:
  ...
  // Clip result.
  bool Clipped;
  bool FullFar1;  // block Entirely IN transition. (all alpha of Far1 should be
                  // 255)
  bool EmptyFar1; // block Entirely OUT transition. (all alpha of Far1 should be
                  // 0)
...
public:
  ...
  // For memory optimisation, Info for Lightmap per block of 2x2 tiles are
  // stored here too.
  uint LightMapId;
...
public:
  ...
  void clip();
  // Clip Tile/Far.
  void clipFar(const CVector &refineCenter, float tileDistNear,
               float farTransition);
  ...
  // append/remove to modify list. This is for FaceVector update durint
  // refine(), addZone() ....
  bool isInModifyList() const { return _PrecToModify != NULL; }
  // append to modify list has the side effect od deleting ALL FaceVector.
  void appendToModifyListAndDeleteFaceVector(CTessBlock &root,

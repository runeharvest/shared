...
template <> inline const char *CPSAttribMaker<NLMISC::CRGBA>::getType() {
  return "CRGBA";
}

// Depending on the driver, the format of colors in vertex buffer may change. We
// don't want to change the format for each data that is (dynamically) in vertex
// buffer, so we modify gradients instead
inline NLMISC::CRGBA convertVBColor(NLMISC::CRGBA color,
                                    CVertexBuffer::TVertexColorType format) {
  return format == CVertexBuffer::TBGRA
             ? NLMISC::CRGBA(color.B, color.G, color.R, color.A)
             : color;
}

void convertVBColor(NLMISC::CRGBA *array, uint numColor,
                    CVertexBuffer::TVertexColorType format);
...
public:
  ...
  virtual void setValues(NLMISC::CRGBA startValue, NLMISC::CRGBA endValue) {
    CPSValueBlendFunc<NLMISC::CRGBA>::setValues(
        convertVBColor(startValue, _ColorType),
        convertVBColor(endValue, _ColorType));
  }
...
class CPSValueBlendSampleFuncRGBA
    : public CPSValueBlendSampleFunc<NLMISC::CRGBA, RGBA_BLENDER_NUM_VALUES> {
public:
  // ctor
  CPSValueBlendSampleFuncRGBA() : _ColorType(CVertexBuffer::TRGBA) {}
  //
  void getValues(NLMISC::CRGBA &startValue, NLMISC::CRGBA &endValue) const {
    CPSValueBlendSampleFunc<NLMISC::CRGBA, RGBA_BLENDER_NUM_VALUES>::getValues(
        startValue, endValue);
    startValue = convertVBColor(startValue, _ColorType);
    endValue = convertVBColor(endValue, _ColorType);
  }
  virtual void setValues(NLMISC::CRGBA startValue, NLMISC::CRGBA endValue) {
    CPSValueBlendSampleFunc<NLMISC::CRGBA, RGBA_BLENDER_NUM_VALUES>::setValues(
        convertVBColor(startValue, _ColorType),
        convertVBColor(endValue, _ColorType));
  }
  void serial(NLMISC::IStream &f) {
...
public:
  ...
  void setValues(const NLMISC::CRGBA *valueTab, uint32 numValues,
                 uint32 nbStages);
  void setValuesUnpacked(const NLMISC::CRGBA *valueTab, uint32 numValues,
                         uint32 nbStages);
  void serial(NLMISC::IStream &f) {
    setColorType(CVertexBuffer::TRGBA);
...
/// This is a int blender class. It just blend between 2 values. The blending is
/// exact, and thus slow...
class CPSColorBlenderExact : public CPSAttribMakerRGBA<CPSValueBlendFuncRGBA> {
public:
  NLMISC_DECLARE_CLASS(CPSColorBlenderExact);
  CPSColorBlenderExact(NLMISC::CRGBA startColor = NLMISC::CRGBA::White,
...
// an int blender class that perform 64 color sample between colors, it is
// faster than CPSColorBlenderExact
class CPSColorBlender : public CPSAttribMakerRGBA<CPSValueBlendSampleFuncRGBA> {
public:
  NLMISC_DECLARE_CLASS(CPSColorBlender);
  CPSColorBlender(NLMISC::CRGBA startColor = NLMISC::CRGBA::White,
                  NLMISC::CRGBA endColor = NLMISC::CRGBA::Black,
                  float nbCycles = 1.0f)
...
public:
  ...
  /**
   *	Construct the value gradient blender by passing a pointer to a color
  ...
  CPSColorGradient(const NLMISC::CRGBA *colorTab, uint32 nbValues,
                   uint32 nbStages, float nbCycles = 1.0f);
  static NLMISC::CRGBA _DefaultGradient[];
  CPSAttribMakerBase *clone() const { return new CPSColorGradient(*this); }
};

/** this memorize value by applying some function on the emitter. For a
...
public:
  CPSColorMemory() : _ColorType(CVertexBuffer::TRGBA) {

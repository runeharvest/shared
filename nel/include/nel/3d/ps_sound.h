...
/**
 * This is a goup of sound instances sound instance. sounds are produced during
 * the motion pass \author Nicolas Vizerie \author Nevrax France \date 2001
 */
...
public:
  ...
  const NLMISC::TStringId &getSoundName(void) const { return _SoundName; }
  ...
  /** Set a gain scheme. It must have been allocated by new, and is then owned
   * by this object
   */
  void setGainScheme(CPSAttribMaker<float> *gain);

  /// get the current gain scheme
  CPSAttribMaker<float> *getGainScheme(void) { return _GainScheme; }

  /// get the current gain scheme (const version)
  const CPSAttribMaker<float> *getGainScheme(void) const { return _GainScheme; }
  ...
  float getPitch(void) const { return _Pitch; }

  /** Set a pitch scheme. It must have been allocated by new, and is then owned
   * by this object
   */
  void setPitchScheme(CPSAttribMaker<float> *pitch);

  /// get the current pitch scheme
  CPSAttribMaker<float> *getPitchScheme(void) { return _PitchScheme; }
  ...
  /// set mute on/off. The default is off. WARNING : It is not saved by this
  /// object
  ...
  /** force the sound to use original pitch that has been entered in the .sound
   * sheet This remove any existing pitchScheme
   */

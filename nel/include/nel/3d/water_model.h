 ...
 * A water surface
 *
 * In order to get precise reflections, we tesselate the shape by projecting it
 * on screen, and by subdividing it by a fixed size grid. For each grid cell :
 * - if it is entirely inside the projected shape, it is displayed as it
 * - if is outside,  no-op
 * - when it intersect the projected shape border, we generate some triangles by
 * clipping the grid cell against the projected shape
 *
...
public:
  ...
  void init() { updateDiffuseMapMatrix(true); }
  ...
  /// inherited from UWaterInstance
  virtual float getAttenuatedHeight(const NLMISC::CVector2f &pos,
  ...
  // fill vertex buffer with this shape datas, and returns pointer to next free
  // location
  uint fillVB(void *dataStart, uint startTri, IDriver &drv);

  // setup vertex buffer before render
  static void setupVertexBuffer(CVertexBuffer &vb, uint numWantedVertices,
                                IDriver *drv);
...
protected:
  ...
  void setupMaterialNVertexShader(IDriver *drv, CWaterShape *shape,
                                  const NLMISC::CVector &obsPos, bool above,
                                  float zHeight);
  //
  void setupSimpleRender(CWaterShape *shape, const NLMISC::CVector &obsPos,
                         bool above);
...
private:
  // clipped tris of the shape after it has been projected on grid, all packed
  // in a single vector

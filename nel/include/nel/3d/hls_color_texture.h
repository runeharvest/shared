...
public:
   ...
   *	\param threshold used to know if a pixel mask value is or not an
   *intermediate (ie not 0 or 255)
   */
   ...
   *	\param colDelta array of delta to apply to the bitmap (must be of
   *numMasks entries) \param out a colorised bitmap with DXTC5/mipMaps generated
   */
  void buildColorVersion(const CHLSColorDelta *colDeltaList,
                         NLMISC::CBitmap &out);
...
private:
  struct CMask {
    ...
    /// Raw Data. First come the Mixt block (16*uint8). Then come the
    /// FullBlockIndex bits, then the MixtBlockIndex bits
...
private:
  ...
  // The DXTC5 data. Also contains bits from _BlockToCompressIndex to end
  // (rounded to uint32)
...
private:
  /// uncompress DXTC5 RGB only block, into a RGBA raw array. Alpha is setup
  /// with undefined values
  static void uncompressBlockRGB(const uint8 *srcDXTC, CRGBA *dstRGBA);

  /// used by compressBlockRGB()
  static void computeMinMax(sint *diffBlock, CVectorInt &v, sint mean[3],
                            sint rgb0[3], sint rgb1[3]);

  /// apply colDelta to the block. Alpha part is not modified. MMX with no EMMS
  /// called here !!!
  static void colorizeDXTCBlockRGB(const uint8 *srcPtr, uint8 *dstPtr,
                                   uint8 dHue, uint dLum, uint dSat);

public:
  /// compress DXTC5 RGB only block, from a RGBA raw array. dstDXTC Alpha part
  /// is not modified. srcRGBA->A are setup to 0!!

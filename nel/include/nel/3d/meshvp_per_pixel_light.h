...
/**
 * This vertex program is used to perform perpixel lighting with meshs. Its
 * outputs are :
 *
 ...
 *
 * This is designed to work with material that got a 'PerPixelLighting' shader.
 * Note that no bump is used with this shader.
 *
 * It requires that the input mesh has its tangent space first column vector
 * encoded in the 2nd texture coordinate. Other stages are not supported for
 * now.
 *
 * Note that this can be mixed with gouraud rendered light (the gouraud diffuse
 * and specular component are still computed)
 *
...
private:
  // this setup the material, and returns true if the v.p has been enabled /
  // disabled
  virtual bool setupForMaterial(const CMaterial &mat, IDriver *drv,
                                CScene *scene);
  bool _Enabled;
  bool _IsPointLight;
  //

 ...
 * NB: This is called after all 'transformName' calls in the ITransformName
 * callback
 */
...
public:
  ...
  /**
   * transformName is called at the addToScene(Async) moment to transform the
   * name of an instance to another name. This should be useful to replace
   * various shapes by others like in a shape-template. The function is called
   * with the name of the shape and the user must return the new name of the
   * shape. NB: This is called after the 'startAddingIG' calls in the
   * IIGAddBegin callback \param instanceName is the name of the instance
   * contained in the instance group (user string) \param shapeName is the name
   * of the instance contained in the instance group (user string) \return the
   * new name of the shape which must be loaded in place of the param
   */
  virtual std::string transformName(uint index, const std::string &instanceName,
                                    const std::string &shapeName) = 0;
};
...
public:
  ...
  /// set a callback to know when an instance has been added / removed from
  /// scene
  virtual void setAddRemoveInstanceCallback(IAddRemoveInstance *callback) = 0;

  /// Set a callback to know when an instance group is being created, and how
  /// many instances it contains
  virtual void setIGAddBeginCallback(IIGAddBegin *callback) = 0;

  /**
   * Add all the instances to the scene. By default, freezeHRC() those instances
   * and the root.
   *
   * \param scene is the scene in which you want to add the instance group.
   * \param driver is a pointer to a driver. If this pointer is not NULL, the
   * textures used by the shapes will be preloaded in this driver. If the
   * pointer is NULL (default), textures will ve loaded when the shape will be
   * used.
   */
  virtual void addToScene(class UScene &scene, UDriver *driver = NULL,
                          uint selectedTexture = 0) = 0;
  virtual void addToSceneAsync(class UScene &scene, UDriver *driver = NULL,
                               uint selectedTexture = 0) = 0;
  ...
  static void createInstanceGroupAsync(const std::string &instanceGroup,
                                       UInstanceGroup **pIG);
   ...
   */
  virtual void getInstanceMatrix(uint instanceNb,
                                 NLMISC::CMatrix &dest) const = 0;
  ...
  /**
   * Return an instance of the group. The instance lifeTime is the same of the
   * instangeGroup lifeTime
   *
   * \param instanceNb is the number of the instance.
   */
   ...
   */
  virtual void setBlendShapeFactor(const std::string &bsName,
                                   float rFactor) = 0;
   ...
   */
  virtual void
  setClusterSystemForInstances(class UInstanceGroup *pClusterSystem) = 0;
  ...
  /** return the parent cluster system of this IG. NULL if this is the
   * GlobalInstanceGroup. \return true if the link succeeded
   */
  ...
  /// see UTransform::freezeHRC(). Do it for all instances (not clusters), and
  /// for the root of the IG.
  virtual void freezeHRC() = 0;

  /// see UTransform::unfreezeHRC(). Do it for all instances (not clusters), and
  /// for the root of the IG.
  ...
  /** get the StaticLightSetup for a model at position
   *localPos/retrieverIdentifier/surfaceId. NB: all are get with help of
   *UGlobalPosition. localPos= gp.LocalPosition.Estimated NB:
   *retrieverIdentifier= globalRetriever->getLocalRetrieverId(gp);
   */
  virtual bool
  getStaticLightSetup(NLMISC::CRGBA sunAmbient, uint retrieverIdentifier,
                      sint surfaceId, const NLMISC::CVector &localPos,
  ...
  /// For debug purpose, display clusters as poly/lines. The matrixs and frustum
  /// should have been setuped
  virtual void displayDebugClusters(UDriver *drv,
                                    class UTextContext *txtCtx) = 0;

  /** return the DontCastShadowForInterior flag of an instance.
   *	Warning: if DontCastShadow=true, and DontCastShadowForInterior=false,
   ...
   *	Warning: if DontCastShadow=true, and DontCastShadowForExterior=false,
   *false is still returned
   */

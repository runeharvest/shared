...
// ***************************************************************************
/**
 * A pointLight with a name, and a default color setup. The current setup is the
 * setup in CPointLight. A CPointLightNamed influence can also be interpolated
 * with a CLightInfluenceInterpolator. \author Lionel Berenguier \author Nevrax
 * France \date 2001
 */
...
public:
  ...
  /** modulate the default color setup with nFactor, and set to the current
   *setup. NB: getUnAnimatedDiffuse() == getDiffuse()
   */
  void setLightFactor(NLMISC::CRGBA nFactor);

  /** modulate the default color setup with nFactor, and set to the current
   *setup. give 2 params the second is the "not animated" factor. used for
   *Landscape.
   */
  void setLightFactor(NLMISC::CRGBA nAnimatedFactor,
  ...
  // ******************
private:
  // For CLightInfluenceInterpolator to work. Only used by

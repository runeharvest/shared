...
protected:
  ...
  virtual void evalKey(const CKeyT *previous, const CKeyT *next,
    ...
    if (previous && next && !previous->Step) {
      ...
      cp0 =
          previous->Value + previous->OutTan * (dateNext - datePrevious) / 3.0f;
...
class CTrackKeyFramerBezier<CKeyBezierQuat, CQuat>
    : public ITrackKeyFramer<CKeyBezierQuat> {
public:
protected:
  /// \name From ITrackKeyFramer
  // @{
  ...
  /// evalKey (runtime).
  virtual void evalKey(const CKeyBezierQuat *previous,
                       const CKeyBezierQuat *next, TAnimationTime datePrevious,
    ...
    if (previous && next) {
      ...
      // quad slerp.
      resultVal.Value = CQuat::squad(previous->Value, previous->A, next->A,
                                     next->Value, date);
    } else {
      if (previous)
        copyToValue(resultVal.Value, previous->Value);

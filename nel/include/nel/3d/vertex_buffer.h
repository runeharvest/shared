 ...
 *
 * Before the vertex buffer is resident (IDriver::activeVertexBuffer), it is in
 * system memory. Once the vertex buffer is resident, the driver creates its
 * proprietary vertex buffer and release the internal vertex buffer. At this
 * moment the vertex buffer can be in VRAM, AGP or system memory.
 *
 * The vertex buffers resident in AGP and VRAM are writeonly, i-e, you can't
 * read them after a lock(). If you change the capacity of the format of a
 * writeonly buffer, the content is lost.
 *
 */
/* *** IMPORTANT ********************
...
public:
  ...
  enum TPreferredMemory {
    RAMPreferred = 0, // A block of driver RAM memory is allocated for this
                      // buffer. The buffer is read/write.
    AGPPreferred, // A block of driver AGP memory is allocated for this buffer.
                  // The buffer is writeonly.
    StaticPreferred, // The buffer will not be modified. A block of driver AGP
                     // or VRAM memory is allocated for this buffer. The buffer
                     // is writeonly.
    RAMVolatile, // A block of temporary driver RAM memory will be returned by
                 // lock(). The buffer must be entirely filled after each
                 // swapBuffers(). The buffer is writeonly.
    AGPVolatile, // A block of temporary driver AGP memory will be returned by
                 // lock(). The buffer must be entirely filled after each
                 // swapBuffers(). The buffer is writeonly.
    PreferredCount
  };
  ...
  /**
   * Value type, there is 13 kind of value type as in DirectX8 and
   * gl_vertex_program used in exteneded mode
   */
  enum TType {
...
public:
  ...
  /** Used by the driver implementation. The driver must first allocate its
   * internal buffer and fill DrvInfos. Then it has to call setLocation(true).
   *
   * If newLocation!=NotResident, setLocation() will copy the non resident
   * buffer in the choosed resident memory and untouch the buffer. If the buffer
   * preferres RAM or AGP memory, it will release the non resident memory.
   *
   * If newLocation==NotResident, setLocation() will realloc the non resident
   * buffer, copy the vertex data if the buffer was resident in RAM. Then it
   * will touch the buffer.*/
  void
  setLocation(TLocation newLocation); // The driver implementation must set the
                                      // location after activeVertexBuffer.
...
public:
   ...
   * Vertex color format is set to TRGBA.
   */
  CVertexBuffer(void);

  /**
   * Named vertex buffer. Make an empty vertex buffer. No value, no vertex.
   * Vertex color format is set to TRGBA.
   */
  ...
  /** Copy a vertex buffer, including vertices data. Destination vb is not
   * resident. May be slow if there's agp / vram readback. Initial content of
   * the destination vertex buffer is lost. Use this to retrieve content of a
   * write-only buffer. Using lock with CVertexBufferRead won't work (used as a
   * foolkeeper to prevent reading of a resident buffer that is not in ram) NB :
   * will assert when used with volatile vb
   */
  void copyVertices(CVertexBuffer &dest) const;
  /**
   * Set the buffer preferred memory. Default preferred memory is RAM.
   *
   * Preferre RAM if the buffer is changed several times in the same render
   *pass. Preferre AGP if the buffer is changed only one time in the same render
   *pass. Preferre Static if the buffer is changed only one time for
   *initialisation.
   *
   * If static memory is chosen, the driver will choose VRAM or AGP depending of
   *the user configuration.
   *
   * If static or RAM memory is preferred, the buffer won't be lost after a
   *driver reset.
   *
   * If the buffer preferres AGP memory, the buffer is lost after a driver
   *reset. When the buffer is lost, it returns in a non resident state. The
   *state must be tested at each pass with isResident(). If the buffer is in a
   * non resident state, the user must refill it.
   *
   * If VRAM memory allocation failed, the driver will try with AGP and then
   *with RAM. If AGP memory allocation failed, the driver will try with RAM. RAM
   *allocation should never failed.
   *
   *	Performance note:
   *	 - for RAM CVertexBuffer, you can read / write as you like.
   *	 - for AGP CVertexBuffer, you should write sequentially to take full
   *advantage of the write combiners. You can't read.
   *	 - for Static CVertexBuffer, you should write only one time, to init.
   *You can read. Each modification will be done in a RAM buffer. Then the
   *unlocked area will be copied into the VRAM or AGP memory.
   *
   * Volatile buffers must be completely filled at each pass. They are lost
   *after each swapBuffers(). They are writeonly. Volatile buffers must be
   *resized before the lock call. Only one lock per render must be done with
   *volatile buffers if keepLocalMemory is false.
   *
   * If keepLocalMemory is true, lock() will return a local memory pointer. The
   *local memory will copied in resident memory during the activation of the
   *buffer. The not all the buffer capacity is copied but only the used size.
   *
   ...
   * The vertex buffer is resident after a call to
   * IDriver::activeVertexBuffer().
   */
  bool isResident() const { return (_Location != NotResident) && DrvInfos; }

  /**
   * \name Standard values vertex buffer mgt.
   * \name Thoses methods manage the vertex buffer (position, normal, colors and
   * uv) with standard value.
   */
  // @{

  /**
   * Setup the vertex format using standard values. Do it before any other
   * standard methods.
   *
   * Use one or several flag between : PositionFlag, WeightFlag, NormalFlag,
   * PrimaryColorFlag, SecondaryColorFlag, FogFlag, TexCoord0Flag,
   * TexCoord1Flag, TexCoord2Flag, TexCoord3Flag, TexCoord4Flag, TexCoord5Flag,
   * TexCoord6Flag, TexCoord7Flag, PaletteSkinFlag
   *
   * If the buffer preferres AGP memory, the data are lost.
   ...
   *
   * If WeightFlag is specified, 4 float are used to setup the skinning value on
   * 4 bones.
   */
   ...
   *
   * Return one or several flags between : PositionFlag, WeightFlag, NormalFlag,
   * PrimaryColorFlag, SecondaryColorFlag, FogFlag, TexCoord0Flag,
   * TexCoord1Flag, TexCoord2Flag, TexCoord3Flag, TexCoord4Flag, TexCoord5Flag,
   * TexCoord6Flag, TexCoord7Flag, PaletteSkinFlag
   *
  ...
  /**
   * get the corresponding TValue according to the number of vertex attribute
   *wanted (v[0], v[1] ...) Warning!: (TValue)valueNumber !=
   *getValueIdByNumberEx(valueNumber).
   *
  ...
  /**
   * Clear all value in the vertex buffer. After this call, call addValue for
   * each value you want in your vertex buffer then call initEx() to init the
   * vertex buffer.
   *
   * If the buffer preferres AGP memory, the data are lost.
   * The vertex buffer is no more resident.
  ...
  /**
   * Add a value in the vertex buffer. After this call, call initEx() to init
   * the vertex buffer.
   *
   * If the buffer preferres AGP memory, the data are lost.
   * The vertex buffer is no more resident.
  ...
  sint getValueOffEx(TValue valueId) const {
    nlassert(_Flags & (1 << valueId));
    return _Offset[valueId];
  };
   ...
   * If the new size is bigger than capacity, reserve() will be called. see
   * reserve(). If the new size is smaller than capacity, the data are keeped,
   * the vertex buffer stay resident if it is resident.
   */
  ...
  /**
   * Reset all the vertices from memory (contReset()), so that capacity() ==
   * getNumVertices() == 0.
   *
   * If the buffer preferres AGP memory, the data are lost.
   * The vertex buffer is no more resident.
  ...
  /**
   * Reserve space for nVerts vertices. You are allowed to write your vertices
   * on this space.
   *
   * If the buffer preferres AGP memory, the data are lost.
   * The vertex buffer is no more resident.
  ...
  TType getValueType(uint value) const {
    nlassert(value < NumValue);
    return ((TType)_Type[value]);
  }
  ...
  /**
   * Access vertices. Multi lock is possible only if no regions are used. Each
   * lock need an accessor to be unlocked.
   *
   * Lock the vertex buffer and return and fill an accessor object. Once the
   * object is destroyed, the buffer in unlocked.
   *
   * \param accessor is the accessor object to fill
   * \param first is the first vertex to be accessed. Put 0 to select all the
   * vertices. What ever is this index, the indexices in the vertex buffer
   * remain the same. \param last is the last vertex to be accessed + 1. Put 0
   * to select all the vertices.
   */
  inline void lock(CVertexBufferReadWrite &accessor, uint first = 0,
                   uint last = 0);

  /**
   * Read only vertices access. Multi lock is possible only if no regions are
   * used. Each lock need an accessor to be unlocked.
   *
   * Lock the vertex buffer and return and fill an accessor object. Once the
   * object is destroyed, the buffer in unlocked.
   *
   ...
   * to select all the vertices.
   */
  inline void lock(CVertexBufferRead &accessor, uint first = 0,
                   uint last = 0) const;
  ...
  // @{
  /// just read/write the VertexFormat of the VB, number of vertices .... If
  /// read, VB is resized to numVertices.
  void serialHeader(NLMISC::IStream &f);
  /** just read/write a subset of the vertex buffer. NB: because a version is
   * serialised by subset, you must read same subset of vertices you have saved.
   * (can't write 0-10, 10-20, and after load 0-5, 5-20).
   */
  ...
  void setUVRouting(uint8 uvChannel, uint newUVRouting) {
    _UVRouting[uvChannel] = uint8(newUVRouting);
  }

  /**
   * Set the vertex color format. If the vertex buffer is not in this format,
   * the colors will be converted See getColorPointer()
   *
...
private:
  ...
  bool checkLockedBuffer() const {
    return _LockedBuffer || (!isResident() && _NonResidentVertices.empty());
  }
...
public:
   ...
   * The driver implementation must call setLocation (NotResident) if
...
public:
  ...
  CVertexBufferReadWrite() { _Parent = NULL; }
  ~CVertexBufferReadWrite() { unlock(); }
  ...
  inline void setValueFloat1Ex(CVertexBuffer::TValue valueId, uint idx,
                               float value);
  inline void setValueFloat2Ex(CVertexBuffer::TValue valueId, uint idx, float x,
                               float y);
  inline void setValueFloat3Ex(CVertexBuffer::TValue valueId, uint idx, float x,
                               float y, float z);
  inline void setValueFloat3Ex(CVertexBuffer::TValue valueId, uint idx,
                               const CVector &vector);
  inline void setValueFloat4Ex(CVertexBuffer::TValue valueId, uint idx, float x,
                               float y, float z, float w);
  inline void setValueUChar4Ex(CVertexBuffer::TValue valueId, uint idx,
                               CRGBA rgba);

  /** Get a pointer on a value.
   *
   *  For Color pointers :
   *  This method returns a (CRGBA*) or a (CBGRA*) regarding the vertex color
   *format returned by CVertexBufferReadWrite::getVertexColorFormat(). A call to
   *IDriver::activeVertexBuffer() will change this format to the format returned
   *by IDriver::getVertexColorFormat(). So, before each write of vertex color in
   *the vertex buffer, the vertex color format must be checked with
   *CVertexBuffer::getVertexColorFormat().
   */
  ...
  void *getValueEx(CVertexBuffer::TValue valueId, uint idx = 0) {
    ...
    nlassert(_Parent->_Flags & (1 << valueId));
    return (void *)(_Parent->_LockedBuffer + idx * _Parent->_VertexSize +
                    _Parent->getValueOffEx(valueId));
  }

  /** Touch the updated vertices. If the method is not call, the accessor update
   * all the vertices. \param first is the index of the first vertices to
   * update. \param last is the index of the last vertices to update + 1.
   */
...
public:
  ...
  CVertexBufferRead() { _Parent = NULL; }
  ~CVertexBufferRead() { unlock(); }
   ...
   *  For Color pointers :
   *  This method returns a (CRGBA*) or a (CBGRA*) regarding the vertex color
   *format returned by CVertexBufferRead::getVertexColorFormat(). A call to
   *IDriver::activeVertexBuffer() will change this format to the format returned
   *by IDriver::getVertexColorFormat(). So, before each write of vertex color in
   *the vertex buffer, the vertex color format must be checked with
   *CVertexBuffer::getVertexColorFormat().
  ...
  const void *getValueEx(CVertexBuffer::TValue valueId, uint idx = 0) const {
    nlassert(_Parent->_Flags & (1 << valueId));
    return (void *)(_Parent->_LockedBuffer + idx * _Parent->_VertexSize +
                    _Parent->getValueOffEx(valueId));
  }
...
inline void CVertexBufferReadWrite::setVertexCoord(uint idx, float x, float y,
                                                   float z) {
  float *ptr;
...
inline void CVertexBufferReadWrite::setColor(uint idx, CRGBA rgba) {
  ...
  nlassert(_Parent->_Type[CVertexBuffer::PrimaryColor] ==
...
inline void CVertexBufferReadWrite::setSpecular(uint idx, CRGBA rgba) {
  ...
  nlassert(_Parent->_Type[CVertexBuffer::SecondaryColor] ==
           CVertexBuffer::UChar4);
...
inline void CVertexBufferReadWrite::setTexCoord(uint idx, uint8 stage, float u,
                                                float v) {
  uint8 *ptr;
  float *ptrf;
  ...
  nlassert(_Parent->_Type[CVertexBuffer::TexCoord0 + stage] ==
...
inline void CVertexBufferReadWrite::setTexCoord(uint idx, uint8 stage,
                                                const CUV &uv) {
  uint8 *ptr;
  CUV *ptruv;
...
inline void CVertexBufferReadWrite::setPaletteSkin(uint idx, CPaletteSkin ps) {
  ...
  nlassert(_Parent->checkLockedBuffer());
  nlassert((_Parent->_Flags & CVertexBuffer::PaletteSkinFlag) ==
...
inline void
CVertexBufferReadWrite::setValueFloat1Ex(CVertexBuffer::TValue valueId,
                                         uint idx, float value) {
  uint8 *ptr;
  float *ptrf;
  ...
  nlassert((_Parent->_Type[valueId] == CVertexBuffer::Float4) ||
           (_Parent->_Type[valueId] == CVertexBuffer::Float3) ||
           (_Parent->_Type[valueId] == CVertexBuffer::Float2) ||
           (_Parent->_Type[valueId] == CVertexBuffer::Float1));
...
inline void
CVertexBufferReadWrite::setValueFloat2Ex(CVertexBuffer::TValue valueId,
                                         uint idx, float x, float y) {
  uint8 *ptr;
  float *ptrf;
...
inline void
CVertexBufferReadWrite::setValueFloat3Ex(CVertexBuffer::TValue valueId,
                                         uint idx, float x, float y, float z) {
  uint8 *ptr;
  float *ptrf;
...
inline void CVertexBufferReadWrite::setValueFloat3Ex(
    CVertexBuffer::TValue valueId, uint idx, const NLMISC::CVector &theVector) {
  uint8 *ptr;
  float *ptrf;
...
inline void
CVertexBufferReadWrite::setValueFloat4Ex(CVertexBuffer::TValue valueId,
                                         uint idx, float x, float y, float z,
                                         float w) {
  uint8 *ptr;
  float *ptrf;
...
inline void
CVertexBufferReadWrite::setValueUChar4Ex(CVertexBuffer::TValue valueId,
                                         uint idx, CRGBA rgba) {
  uint8 *ptr;
  CRGBA *ptrr;
...
inline void CVertexBuffer::lock(CVertexBufferReadWrite &accessor, uint first,
                                uint last) {
  accessor.unlock();
  ...
  if (_LockCounter == 0) {
    ...
    if (isResident() && !_KeepLocalMemory) {
      if (last == 0)
        last = _NbVerts;
      _LockedBuffer =
          DrvInfos->lock(first * _VertexSize, last * _VertexSize, false);
    } else {
...
inline void CVertexBuffer::lock(CVertexBufferRead &accessor, uint first,
                                uint last) const {
  accessor.unlock();
  accessor._Parent = this;
  ...
  if (_LockCounter == 0) {
    ...
    if (isResident() && !_KeepLocalMemory) {
      if (last == 0)
        last = _NbVerts;
      // Can read it ?
      nlassertex(_Location == RAMResident,
                 ("Try to read a write only vertex buffer"));
      _LockedBuffer =
          DrvInfos->lock(first * _VertexSize, last * _VertexSize, true);
    } else {
      if (_NonResidentVertices.empty())
        _LockedBuffer = NULL;
      else

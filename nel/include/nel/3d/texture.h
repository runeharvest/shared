...
// ITextureDrvInfos represent the real data of the texture, stored into the
// driver (eg: just a GLint for opengl).
...
// Many ITexture may point to the same ITextureDrvInfos, through
// CTextureDrvShare.
...
public:
  CTextureDrvShare(IDriver *drv, ItTexDrvSharePtrList it, ITexture *owner) {
    _Driver = drv;
    _DriverIterator = it;
    _OwnerTexture = owner;
  }
 ...
 *
 * Sharing System note: The deriver may implement sharing system by implement
 * supportSharing() and getShareName(). Such a texture may return a Unique Name
 * for sharing. If the driver already has this texture, it will reuse it. As a
 * direct impact, you cannot invalidate part of the textures with shared
 * texture. This is logic, since the Unique sharname of the texture must
 * represent all of it.
 *
 */
/* *** IMPORTANT ********************
...
public:
  ...
  ITexture();
  /// see operator=.
  ITexture(const ITexture &tex) : CBitmap(), CRefCount(), IStreamable() {
    operator=(tex);
  }
  /// Need a virtual dtor.
  virtual ~ITexture();
  /// The operator= do not copy drv info, and set touched=true. _Releasable,
  /// WrapMode and UploadFormat are copied.
  ...
  bool mipMapOff() const {
    return _MinFilter == NearestMipMapOff || _MinFilter == LinearMipMapOff;
  }
  bool mipMapOn() const { return !mipMapOff(); }
  // @}

  /**
   *  This method invalidates all the texture surface. When the driver calls
   * generate, the texture will rebuild all the texture and the driver will
   * update it.
   *
   * \see isAllInvalidated(), generate(), touchRect(), touched(),
   * _ListInvalidRect
   */
  void touch() {
  ...
  /**
   *  This method invalidates a rectangle of the texture surface. When the
   * driver calls generate, the texture could rebuild only this part of texture
   * and the driver will update only those rectangles.
   *
   *  This method is incompatible with textures which support sharing (see class
   * description). This method is incompatible with compressed textures. This
   * method is incompatible with cube textures.
   *
  ...
  void touchRect(const NLMISC::CRect &rect) {
    ...
    // Don't invalidate the rectangle if the full texture is already
    // invalidated.
    if (!isAllInvalidated()) {
  ...
  /**
   * Return whether texture can be released. If it returns true, the driver will
   * release the texture after generate it and upload it into the videomemory by
   * calling release(). If it returns false, the driver won't release the
   * texture.
   *
   ...
   * If it is true, the driver will release the texture after generating it and
   * upload it into the videomemory by calling release(). If it is false, the
   * driver won't release the texture.
   *
   ...
   *
   * This method is called by the driver when it needs to generate pixels of the
   * texture. If the texture is used for the first time or if it is touched, the
   * driver will call this method. For exemple, a texture file will load the
   * bitmap in this method.
   *
   * If the invalidate rect list is empty, generate() rebuild all the texture.
   * If the invalidate rect list is not empty, generate() rebuilds only the
   * invalidate rectangles in the list.
   *
   * Don't clear the touch flag or the invalid rectangle list until updating the
   * texture in generate(). It's the generate()'s caller jobs.
   *
   * After generation, if the texture is releasable, the driver will release the
   * texture by calling release().
   *
   * NB: a flag is maintained to see if the generated bitmap is coherent with
   * texture description (see touch*()). So if you do {generate(); generate();},
   * you only get 1 real bitmap generation...
   *
   * If, after the doGenerate, the bitmap format is compressed (DXTC) and no
   * mipmaps have been generated, the mipmap are disabled beacause the user
   * probably don't want the driver to unpacks the texture, generates the
   * mipmaps and repacks the dxtc texture (that takes a lot of CPU time).
   *
   * \param async tells the texture if the call is made asynchronously or not.
   *
   * \see isAllInvalidated(), touch(), touched(), touchRect(), clearTouched(),
   * _ListInvalidRect \see getReleasable(), setReleasable()
   */
  ...
  /**	Advanced. erase the _GoodGenerate=true. Special if you want to setup
   *directly the bitmap without using generate(). USE IT WITH CARE!! (used by
   *the CAsyncTextureManager)
   */
   ...
   */
  bool setupedIntoDriver() const { return TextureDrvShare != NULL; }
  ...
  /** Select a texture among several other (if this texture is a set of texture
   * such as CTextureMultiFile) The default does nothing
   */
  ...
  /** If this texture is selectable, build a non selectable version of this
   * texture that is setupped with the given slot. NB : If this texture is
   * selectable, you are ensured that the return pointer is not 'this'
   */
...
protected:
   ...
   * Generate the texture pixels.
   *
   * If the invalidate rect list is empty, generate() must rebuild all the
   * texture. If the invalidate rect list is not empty, generate() rebuilds only
   * the invalidate rectangles in the list.
   *
   * \see isAllInvalidated(), touch(), touched(), touchRect(), clearTouched(),
   * _ListInvalidRect, generate() \see getReleasable(), setReleasable()
   */
...
public:
  ...
  /**
   *  List of invalided rectangle. If the list is empty, generate() will rebuild
   * all the texture.
   *
  ...
  /**
   *  This method return the touched flag. If it is true, the driver will call
   * generate to rebuild the texture.
   *
   ...
   * If this is the case, the driver should resetup them for that texture (If
   * driver stores filter & wrap mode per texture (OpenGL) rather than globally
   * (D3D))
   */
   ...
   *
   * \see isAllInvalidated(), generate(), touch(), touched(), touchRect(),
   * _ListInvalidRect
   */
  ...
  void clearFilterOrWrapModeTouched() { _FilterOrWrapModeTouched = false; }
};

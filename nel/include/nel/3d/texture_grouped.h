...
/**
 * This kind texture is used for grouping several other textures. Each texture
 * must have the same size. The textures are copied into one single surface, so
 * animation can be performed only by UV shifting (if there's no wrapping). This
 * is useful when objects sorting (by texture) is too complex or cost too much
 * time (particles for examples...) \author Nicolas Vizerie \author Nevrax
 * France \date 2001
 */
...
public:
  ...
  /** Check if all the textures in a tab have the same size and the same pixel
   * format \param textureTab : pointer to a texture list* \param nbTex the
   * number of textures in the list (>0) \see setTextures()
   */
   ...
   *  WARNING : if you touch one of the textures in the tab later, you may need
   * to touch this one if it changed \param checkValid check that textures are
   * valid \param textureTab : pointer to a texture list \param nbTex the number
   * of textures in the list (>0) \see haveValidSizes()
   */
  ...
  /** Get the U-delta and V delta in the groupedTexture for one unit texture
   * (they all have the same size). return (0, 0) if no textures have been set
   */
  const CUV &getUVDelta(void) const { return _DeltaUV; }

  /// Get the origin UV for one texture. Its index is the same than in the tab
  /// that was sent to setTextures()
  const CUV &getUV(uint32 index) const { return _TexUVs[index].uv0; }
  ...
  // Get a tab of 4 UVs for a texture in the group : 0 = top-left, 1 =
...
protected:
  ...
  // cache sub bitmap size for each texture to avoid reloading of texture after
  // each serial
  static std::map<std::string, uint> _NameToSize;

  // Copy this class attributes from src; Used by the = operator and the copy
  // ctor
  void duplicate(const CTextureGrouped &src);

  // make textures as a group of dummy. This is used when textures formet are
  // incompatible
  ...
  void displayIncompatibleTextureWarning(CSmartPtr<ITexture> *textureTab,
                                         uint nbTex);

...
// ***************************************************************************
/**
 * A base mesh with just material manipulation. Used as an abstract base class
 * for CMesh or CMeshMRM. \author Lionel Berenguier \author Nevrax France \date
 * 2001
 */
...
public:
  /** If AutoCameraCol, meshs will build the cameracollision mesh only if the
   *mesh is lightmapped if NoCameraCol, the cameracollision won't be generated
   *	if ForceCameraCol, the cameracollision will be generated whether
   *lightmaped or not (as long as possible)
   */
  enum TCameraCollisionGenerate {
    AutoCameraCol = 0,
    NoCameraCol,
    ForceCameraCol
  };
...
public:
  ...
  struct CMeshBaseBuild {
    ...
    // Indicate how this mesh should build the collision mesh (for camera
    // ThirPerson collision)
...
public:
  ...
  uint getNbMaterial() const { return (uint)_Materials.size(); }

  /// Get a material
  CMaterial &getMaterial(uint id) { return _Materials[id]; }
  /// Get a material, const version
  const CMaterial &getMaterial(uint id) const { return _Materials[id]; }
  // @}
  ...
  /// Is this mesh lightable? true if one of his materials is not a lightmap.
  /// NB: result stored in file.
  ...
  // @{
  /** Tells that the model instanciated from this shape should be automatically
   * animated. If so the scene will search for an automatic anim that has the
   * same name than this shape and will bind it. This state is serialized. NB:
   * This is a clue to scene::createInstance, a call to createInstance of this
   * object won't make the animation binding.
   */
  ...
  /// get the lod texture info. see CSkeletonModel Lod system. NULL per default.
  /// NB: serialised
  const CLodCharacterTexture *getLodCharacterTexture() const {
    return _LodCharacterTexture;
  }
  // @}
  ...
  // Return the Default setup choosed for this shape (used for
  // UInstance::setOpacity())
  bool getDefaultOpacity() const { return _DefaultOpacity; }

  // Return the Default setup computed for this shape (used for
  // UInstance::setTransparency())
...
protected:
  ...
  /// Transform default tracks. Those default tracks are instancied, ie,
  /// CInstanceMesh will have the same and can't specialize it.
   ...
   *	We cannot use the meshgeom, since once the VBuffer/IndexBuffer are
   *resident (maybe AGP), they can't be locked without heavy performance penalty
   *	This is built by CMesh/CMeshMRM/CMeshMultiLod::buildSystemGeometry()
   *	And only if it has been flagged through
...
protected:
   ...
   */
  void applyMaterialUsageOptim(const std::vector<bool> &materialUsed,

...
public:
   ...
   * For lights, this update their pos, colors and attenuation distances in the
   * engine
   */
  ...
  // Get attribute maker that generate colors (or NULL if constant color is
  // used)
  ...
  // Set an attribute maker that generate the radius at which light attenuation
  // starts (or NULL to discard)
  void setAttenStartScheme(CPSAttribMaker<float> *scheme);
  // Get attribute maker that generate the radius for attenuation start (or NULL
  // if constant radius is used)
  CPSAttribMaker<float> *getAttenStartScheme() const {
    return _AttenStartScheme;
  }
  ...
  // Set an attribute maker that generate the radius at which light attenuation
  // ends (or NULL to discard)
  void setAttenEndScheme(CPSAttribMaker<float> *scheme);
  // Get attribute maker that generate the radius for attenuation end (or NULL
  // if constant radius is used)

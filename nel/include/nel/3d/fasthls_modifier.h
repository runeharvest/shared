...
public:
  ...
  /// convert a RGBA bitmap into another RGBA, with HLS decal (0..255,
  /// -255..+255, -255..+255).
  void convertRGBABitmap(NLMISC::CBitmap &dst, const NLMISC::CBitmap &src,
                         uint8 dh, sint dl, sint ds);

  /// convert a DDS bitmap into another DDS, with HLS decal (0..255, -255..+255,
  /// -255..+255).
  void convertDDSBitmap(NLMISC::CBitmap &dst, const NLMISC::CBitmap &src,
                        uint8 dh, sint dl, sint ds);

  /** convert a uint16 color into the uint16 with HLS decal.
   *	WARNING: this code use MMX when possible, but doesn't call emms!!! It is
   *User job to do it. dLum must be actually 0xFFFFFF00 + realDLum (where
   *realDLum== -255..+255) dSat must be actually 0xFFFFFF00 + realDSat (where
   *realDSat== -255..+255)
   */
...
private:
  ...
  void convertDDSBitmapDXTC1Or1A(NLMISC::CBitmap &dst,
                                 const NLMISC::CBitmap &src, uint8 dh,
                                 uint dLum, uint dSat);
  void convertDDSBitmapDXTC3Or5(NLMISC::CBitmap &dst,
                                const NLMISC::CBitmap &src, uint8 dh, uint dLum,
                                uint dSat);
};

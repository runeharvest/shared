...
public:
  /** Constructor
   *	\param keyDepth default is 32 bits, but if as example you're sure that
   *you just use 14 bits, you can set keyDepth=14. Clamped to 1,32. \param
   *digitDepth default is 8 bits. The sort will do ceil(keyDepth/digitDepth)
   *pass in Sort. But be aware that a CRadixSort object has a size of
   *(1<<digitDepth) * 8 bytes. (2K for digitDepth=8). Be aware too that values>8
   *is not a really good idea because of cache, and random access... And this
   *REALLY impact (eg: from 1 to 100 if you choose 16 instead than 8...) Clamped
   *to 1,min(16, keyDepth).
   */
  ...
  /**	Sort an array of T, in (keyDepth/digitSize)*O(size). Elements are
   *reordered in increasing order. \param array0 ptr on elements to be sorted.
  ...
  T *sort(T *array0, T *array1, uint size) {
    return doSort(array0, array1, size, true);
  }
  ...
  T *reverse_sort(T *array0, T *array1, uint size) {
    return doSort(array0, array1, size, false);
  }

...
public:
  ...
  /// set the zone tile color (if false tile are monochromed with the tile
  /// color)
...
private:
  // Zone Tile Color parameters to apply at load time : Mono = Monochrome,
  // Factor = Multiplier
...
public:
  ...
  CZoneLoadingTask(const std::string &sZoneName, TVolatileZonePtr *ppZone,
                   CVector &pos, bool monochrome, float factor);
  ~CZoneLoadingTask();

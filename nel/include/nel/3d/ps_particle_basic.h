...
public:
  ...
  /**
   * process one pass for the particles. The default behaviour shows the
   * particles
   */
  ...
  /** Returns true if the object can use global lighting color. (example :
   * 'lookat' particle do not have normals, so they use global lighting color
   * instead
   */
  ...
  void enableGlobalColorLighting(bool enabled) {
    _UsesGlobalColorLighting = enabled;
  }
  ...
  /// draw the particles for edition mode. The default behaviour just draw a
  /// wireframe model
  ...
  /// Force the Auto-LOD to be disbaled. When set to false, the default
  /// behaviour set in the system is used
  ...
  // Change z-bias of material. this must be redefined for all renderable
  // particles
...
protected:
   ...
   * This must be called when a geometric property of the particle has been
   * modified This needn't to be called during CPSParticle::resize overrides
   */
  /*
   ...
   * the index given index out of range -> nl_assert
   */
  virtual void deleteElement(uint32 index) = 0;

  /** Resize the bindable attributes containers. Size is the max number of
   * element to be contained. DERIVERS MUST CALL THEIR PARENT VERSION should not
   * be called directly. Call CPSLocated::resize instead
   */
  virtual void resize(uint32 size) = 0;

  /** System may have hand-tuned LOD, or auto LOD.
   * This compute the number of particles that must really be displayed, and the
   * src step that allow to go through the whole collection. The step in the
   * source is in a fixed point 16:16 format
   */
...
/// this class adds tunable color to a particle. Can be added using public
...
protected:
  ...
  /// Update the material and the vb and the like so that they match the color
  /// scheme
  virtual void updateMatAndVbForColor(void) = 0;

  void newColorElement(const CPSEmitterInfo &info) {
    if (_ColorScheme && _ColorScheme->hasMemory())
      _ColorScheme->newElement(info);
  }
  void deleteColorElement(uint32 index) {
    if (_ColorScheme && _ColorScheme->hasMemory())
      _ColorScheme->deleteElement(index);
  }
  void resizeColor(uint32 size) {
    nlassert(size < (1 << 16));
    if (_ColorScheme && _ColorScheme->hasMemory())
      _ColorScheme->resize(size, getColorOwner()->getSize());
  }
};
...
/// this class adds tunable size to a particle. Can be added using public
/// multiple inheritance
...
protected:
  ...
  void newSizeElement(const CPSEmitterInfo &info) {
    if (_SizeScheme && _SizeScheme->hasMemory())
      _SizeScheme->newElement(info);
  }
  void deleteSizeElement(uint32 index) {
    if (_SizeScheme && _SizeScheme->hasMemory())
      _SizeScheme->deleteElement(index);
  }
  void resizeSize(uint32 size) {
    nlassert(size < (1 << 16));
    if (_SizeScheme && _SizeScheme->hasMemory())
      _SizeScheme->resize(size, getSizeOwner()->getSize());
  }
};
...
/// this class adds tunable 2D rotation to a particle, it can be used by public
/// multiple inheritance
...
public:
  ...
  const CPSAttribMaker<float> *getAngle2DScheme(void) const {
    return _Angle2DScheme;
  }

  /** Set a constant angle for the particle. Angles range from  0.0f to 256.0f
   *(2 pi) This discrad any previous scheme \see setAngle2DScheme()
   */
   ...
   * the like for each angle, there are 4 float : 2 couple of float : a1, b1,
   * a2, b2 a1 * I + b1 * K = up left corner, a2 * I + b2 * K = up right corner,
   * This table must have been initialized with initRotTable
   */
  static inline const float *getRotTable(void) {
    nlassert(_InitializedRotTab); // must have called initRotTable at the start
                                  // of the apply
    return _RotTable;
  }
...
protected:
  ...
  void newAngle2DElement(const CPSEmitterInfo &info) {
    if (_Angle2DScheme && _Angle2DScheme->hasMemory())
      _Angle2DScheme->newElement(info);
  }
  void deleteAngle2DElement(uint32 index) {
    if (_Angle2DScheme && _Angle2DScheme->hasMemory())
      _Angle2DScheme->deleteElement(index);
  }
  void resizeAngle2D(uint32 size) {
    nlassert(size < (1 << 16));
    if (_Angle2DScheme && _Angle2DScheme->hasMemory())
      _Angle2DScheme->resize(size, getAngle2DOwner()->getSize());
  }
};
...
/** this class adds a texture to a particle. The texture can be animated or not.
 * it can be used by public multiple inheritance. The frame animation are all
 * stored in the same texture for optimisation so it's not suited for large
 * anim...
 */
...
public:
   ...
   *  It must have been allocated by new
   *  It will be deleted by this object
   *  a texture group must have been set before this, an assertion occurs
   * otherwise The integer is used as an index in a grouped texture. It tells
   * which frame to use
   */
  ...
  /// set a constant index for the current texture. not very useful, but
  /// available...
  ...
  /// get the texture group used. it discard any previous single texture. (if
  /// null, there's no texture animation)
  CTextureGrouped *getTextureGroup(void) { return _TexGroup; }

  /// get the texture group used if there's a texture scheme, const version. (if
  /// null, there's no texture animation)
  ...
  /// ctor : default have no texture. You must set it, otherwise you'll get an
  /// assertion when it's drawn
  CPSTexturedParticle();
...
protected:
  ...
  // a texture index. Most of the time, a scheme of index will be used instead
  // of that
  ...
  void newTextureIndexElement(const CPSEmitterInfo &info) {
    if (_TextureIndexScheme && _TextureIndexScheme->hasMemory())
      _TextureIndexScheme->newElement(info);
  }
  void deleteTextureIndexElement(uint32 index) {
    if (_TextureIndexScheme && _TextureIndexScheme->hasMemory())
      _TextureIndexScheme->deleteElement(index);
  }
  void resizeTextureIndex(uint32 size) {
    nlassert(size < (1 << 16));
    if (_TextureIndexScheme && _TextureIndexScheme->hasMemory())
      _TextureIndexScheme->resize(size, getTextureIndexOwner()->getSize());
  }
};
...
/** This class add multitexturing support to particles. It doesn't support
 * texture animation however. It adds a second texture that is combined with the
 * main texture by using a given operation. An alternate mode must be provided,
 * for gfx boards that doesn't support the op. For now, 2 stages only are
 * supported.
 */
...
public:
  ...
  bool isMultiTextureEnabled() const {
    return (_MultiTexState & (uint8)MultiTextureEnabled) != 0;
  }

  /// Set the main texture for multitexturing. Convert the texture to / from a
  /// bumpmap if needed (so you just provide its heightmap)
  ...
  /** Set the operation for the main texture. When EnvBumpMap is used,
   * setTexture2 must be called with a bump map, and the primary texture must be
   * convertible to rgba. Convert the texture to / from a bumpmap if needed
   */
  void setMainTexOp(TOperator op);

  TOperator getMainTexOp() const { return _MainOp; }

  // Enable the use of an alternate texture for multitexturing. When disabled,
  // this discard the textures that may have been set.
  void enableAlternateTex(bool enabled = true);
  bool isAlternateTexEnabled() const {
    return (_MultiTexState & (uint8)AlternateTextureEnabled) != 0;
  }

  /// Set the alternate texture for multitexturing. It is used when the main
  /// operator is not supported by the gfx board.
  // Convert the texture to / from a bumpmap if needed. (so you just provide its
  // heightmap)
  ...
  /// Set the operation for the alternate texture. Convert the texture to / from
  /// a bumpmap if needed.
  void setAlternateTexOp(TOperator op);

  TOperator getAlternateTexOp() const { return _AlternateOp; }
  ...
  /** this act as if the system had the most basic caps supported (no EMBM for
   * example...) Should be used only in edition mode for test
   */
...
protected:
  void setupMultiTexEnv(TOperator op, ITexture *tex1, ITexture *tex2,
                        CMaterial &mat, IDriver &drv);
  ...
  bool isEnvBumpMapUsed() const {
    return (_MultiTexState & EnvBumpMapUsed) != 0;
  }
  ...
  bool areBasicCapsForcedLocal() const {
    return (_MultiTexState & BasicCapsForced) != 0;
  }
  void forceBasicCapsLocal(bool force) {
    if (force)
      _MultiTexState |= BasicCapsForced;
    else
      _MultiTexState &= ~BasicCapsForced;
  }
  float _BumpFactor;
...
/** this class adds tunable 3D rotation to a PLANE particle, it can be used by
 * public multiple inheritance It must just produce 2 vectors that give the x
 * and y vector of the local basis.
 */
...
public:
  ...
  /// ctor : default have constant basis that map to the I & J vector (e.g
  /// identity)
...
protected:
  ...
  void newPlaneBasisElement(const CPSEmitterInfo &info) {
    if (_PlaneBasisScheme && _PlaneBasisScheme->hasMemory())
      _PlaneBasisScheme->newElement(info);
  }
  void deletePlaneBasisElement(uint32 index) {
    if (_PlaneBasisScheme && _PlaneBasisScheme->hasMemory())
      _PlaneBasisScheme->deleteElement(index);
  }
  void resizePlaneBasis(uint32 size) {
    nlassert(size < (1 << 16));
    if (_PlaneBasisScheme && _PlaneBasisScheme->hasMemory())
      _PlaneBasisScheme->resize(size, getPlaneBasisOwner()->getSize());
  }
};
...
/** This add a hint to rotated particle : only a few one are rotated, and the
 * other are duplcated
 *
 */
...
struct CPSHintParticleRotateTheSame {
  ...
  /** Tells that all particles are turning in the same manner, and only have a
  ...
  virtual void hintRotateTheSame(uint32 nbConfiguration,
                                 ...
                                 float maxAngularVelocity = NLMISC::Pi) = 0;
   ...
   *  \see hintRotateTheSame(), CPSRotated3dPlaneParticle
   */
  virtual uint32 checkHintRotateTheSame(float &minAngularVelocity,
                                        float &maxAngularVelocity) const = 0;
};
...
/** this contains material of a particle, this doesn't initiliaze anything, this
 * just give the abylity to change the blending mode
 */
class CPSMaterial {
public:
  ...
  /// serialization (not named 'serial' because it will be used via
  /// multiple-inheritance)
  ...
  /** Force the material to have one texture that is modulated by diffuse, and a
   * constant color and its diffuse color. This is not compatible with
   * multitextureing, however. \param force true to force constant color
   * modulation
   */
  void
  forceModulateConstantColor(bool force,
   ...
   * It can be used to do extra math with stages that have no textures (required
   * by the driver)
   */
...
inline void SetupModulatedStage(CMaterial &m, uint stage,

...
public:
  ...
  /**
   * The blend method. This method blend two animated values and store the
   * result in the object. The two first args can be reference on the object
   * itself. Idealy, it performs the operation
   * this->value=(this->value*blendFactor + value*(1.f-blendFactor))
   *
 ...
 * \date 2001
 */
template <class T> class CAnimatedValueBlendable : public IAnimatedValue {
public:
...
public:
  ...
  virtual void blend(const IAnimatedValue &value, float blendFactor) {
    ...
    // Cast.
    CAnimatedValueBlendable<NLMISC::CQuat> *pValue =
        (CAnimatedValueBlendable<NLMISC::CQuat> *)&value;

    // blend.
    // Yoyo: no makeClosest is done, because the result seems to be better when
    // done before: for all blend values, and not one after one.
...
public:
  ...
  virtual void blend(const IAnimatedValue &value, float blendFactor) {
    ...
    // Cast.
    CAnimatedValueBlendable<NLMISC::CRGBA> *pValue =
        (CAnimatedValueBlendable<NLMISC::CRGBA> *)&value;
 ...
 * \date 2001
 */
template <class T> class CAnimatedValueNotBlendable : public IAnimatedValue {
public:
  /// A default blend method. Doesn't work for all type.
  virtual void blend(const IAnimatedValue &value, float blendFactor) {
    // Check types of value. typeid is slow, assert only in debug
    ...
    // Cast
    CAnimatedValueNotBlendable<T> *pValue =
        (CAnimatedValueNotBlendable<T> *)&value;
...
// ***************************************************************************
/** This class must contain all the possible AnimatedValue, so the system can
 *work It is used at ITrack evaluation to fill one of these values
 */

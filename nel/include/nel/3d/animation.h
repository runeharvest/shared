...
public:
   ...
   *
   *	WARNING: slower if applyAnimHeaderCompression() has been called. try use
   *getIdTrackByChannelId() instead
   *
   * \param name is the name of the desired track.
   * \return CAnimation::NotFound if the track doesn't exist else the id of the
   *track.
   */
   ...
   * \return CAnimation::NotFound if the track doesn't exist (or anim header not
   *compressed) else return the id of the track.
   */
   ...
   *	Add manualy the shapes that can be spawned by the channel "spawn_script"
   *in this animation This add into a vector (avoid duplicates)
   *
   *	Then CAnimationSet::preloadSSSShapes() can be used after
   *CAnimationSet::build() to force loading into the ShapeBank/Texture of thoses
   *shapes, so there is no problem of shape loading during animation
   */
  ...
  // @{
  /** For each track that support it (CTrackSampled for instance), divide its
   * number of sampled keys, to lower the memory lod. Used typically by
   * CAnimationSet
   */
  void applySampleDivisor(uint sampleDivisor);

  /** For CTrackSampledQuat only, compress header. Used typically by
   * CAnimationSet NB: Animation cannot be serialized after this operation
   * (unserialisable tracks)
   */
   ...
   */
  void
  applyAnimHeaderCompression(CAnimationSet *animationSetOwner,
...
private:
  ...
  // Map to get a channel id with a name. EMPTY if applyAnimHeaderCompression()
  // called
  ...
  // Sorted array of ChannelId. Same size as _TrackVector. EMPTY if
  // applyAnimHeaderCompression() NOT called

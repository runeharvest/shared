...
// ***************************************************************************
/**
 * The track store an animation of an animated value. This animation can be
 * interpolated by several ways.
 *
...
public:
   ...
   *
   * The result is stored in CAnimatedValueBlock to simplify access at the
   * polymorphic values. The track choose which value to assign, and return the
   * one modified
   */
  virtual const IAnimatedValue &eval(const TAnimationTime &date,
                                     CAnimatedValueBlock &avBlock) = 0;
  ...
  /** typically used by CAnimation to lower the number of keys. not supported by
   * default
   */
   ...
   */
  virtual bool applyTrackQuatHeaderCompressionPass0(
      CTrackSampleCounter & /* quatCounter */) {
    return false;
  }
  virtual ITrack *
...
public:
  TAnimationTime getBeginTime() const { return 0.f; }
  TAnimationTime getEndTime() const { return 0.f; }
 ...
 * \date 2001
 */
template <class T> class CTrackDefaultBlendable : public ITrackDefault {
public:
  CTrackDefaultBlendable() {}
  CTrackDefaultBlendable(const T &val) { _Value = val; }

  /// set the default value.
  void setDefaultValue(const T &val) { _Value = val; }

  /// get the default value.
  const T &getDefaultValue() const { return _Value; }
 ...
 * \date 2001
 */
template <class T> class CTrackDefaultNotBlendable : public ITrackDefault {
public:
  CTrackDefaultNotBlendable() {}
  CTrackDefaultNotBlendable(const T &val) { _Value = val; }
...
public:
  NL3D_TRACKDEF_CTOR(CTrackDefaultString, CTrackDefaultNotBlendable,
                     std::string);

...
namespace NL3D {
const uint32 DefaultMaxLocatedInstance =
    1; // the default value for a located container
}
...
/// This structure helps to perform the collision step, by telling which
/// collisionner is the nearest if there are several candidate a distance of -1
/// indicates that no collisions occurred
struct CPSCollisionInfo {
  CPSCollisionInfo *Next;
  float
      Dist; // Distance to the nearest collider, or -1 if not collision occurred
  NLMISC::CVector NewPos;
  NLMISC::CVector NewSpeed; // The speed of particle after a collision occurred.
                            // After the updated of collision it is swapped with
                            // the post-collision speed
  CPSZone *CollisionZone;   // The zone on which the bounce occurred, can be
                          // useful to check the behaviour in case of collision
  uint32 Index;
  CPSCollisionInfo() { Dist = -1.f; }
  // update the collision info, and eventually link it in the list of active
  // collisions
 ...
 * A located don't do anything by itself. You must bind objects to it, such as a
 * particle, a force and so on. It is important to remember that a located holds
 * all instance of object of one type (force, emitter, particles or both...),
 * not only one. Not sharable accross systems
 */
...
public:
  ...
  /** Detach a bindable object from this located. Ownership is transferred to
   * the caller Any reference the object may have in the system is lost
   * (targets..) After that is may be inserted another system.
   */
  ...
  uint32 getNbBoundObjects(void) const {
    NL_PS_FUNC(getNbBoundObjects);
    return (uint32)_LocatedBoundCont.size();
  }
  ...
  /** Post a new Element to be created. This should be called by emitters only
   * (r.g in the sim loop) Calling this outside the sim loop will cause an
   * assert.
   */
  void postNewElement(const NLMISC::CVector &pos, const NLMISC::CVector &speed,
                      CPSLocated &emitterLocated, uint32 indexInEmitter,
                      TPSMatrixMode speedCoordSystem, TAnimationTime lifeTime);

  /**
   * Generate one more instance in a located.
   * The coordinates are given in the chosen basis for the located.
   * If the emitterLocated ptr is not null, then the coordinate are taken from
   * the emitterLocated basis and are expressed in this located basis. Will
   * succeed only if it hasn't reach the max number of allowed instances return
   * will be -1 if call failed or an index to the created object. Index is only
   * valid after creation. Any processing pass on the system will make it
   * invalid. It can be used with any attribute modification method of located
   * and located bindable \param indexInEmitter The index of the emitter (in the
   * emitterLocated object) \param basisConversionForSpeed : if false, the speed
   * vector is taken as if even if emitter and emittee basis are differents.
   ...
   * \param doEmitOnce : When the element is created, all emitters flagged as
   * 'EmitOnce' will be triggered
   */
  sint32 newElement(const NLMISC::CVector &pos, const NLMISC::CVector &speed,
                    CPSLocated *emitterLocated, uint32 indexInEmitter,
  ...
  /**  Shorcut to increase the particle counter (number of particle drawn, for
   * benchmark purpose ) should be called only by bound object that display
   * particles
   */
   ...
   * Valid only after the newElement method (overridable) of a LocatedBindable
   *is called : you get the index of the located being generated, if you need
   *its pos, speed, or mass.
   */
  ...
  /** Set a scheme (allocated by new, released by that object) that generate the
   * duration of locateds. Such a scheme can't own its memory. Any previous call
   * to setLastForever() is discarded Any previous scheme for lifetime is
   * discarded
   */
  ...
  /// get the life scheme of created particle, null if none (valid if they have
  /// a limited life time)
  ...
  /** Set a scheme (allocated by new, released by that object) that generate the
   * mass of locateds. Such a scheme can't own its memory. Any previous scheme
   * for Mass is discarded
   */
  ...
  /// Setup the driver model matrix. It is set accrodingly to the basis of the
  /// located
  void setupDriverModelMatrix(void);

  /** Compute a vector that will map to (1 0 0) after view and model transform.
   *  This allow to  have object that always faces the user, whatever basis they
   * are in
   */
  ...
  // Compute forces that apply on that located. Should be called only inside the
  // sim loop.
  void computeForces();

  // compute collisions
  void computeCollisions(uint firstInstanceIndex,
                         const NLMISC::CVector *posBefore,
                         const NLMISC::CVector *posAfter);

  // get a conversion matrix between 2 matrix modes
  static const NLMISC::CMatrix &getConversionMatrix(const CParticleSystem &ps,
                                                    TPSMatrixMode to,
                                                    TPSMatrixMode from);
   ...
   */
  static const NLMISC::CMatrix &getConversionMatrix(const CPSLocated *A,
                                                    const CPSLocated *B);
   ...
   *  Each observer will be called when this object dtor is called (call of
   * method notifyTargetRemoved() ) This allow for objects that hold this as a
   * target to know when it is suppressed (example : collision objects hold
   * located as targets) When an observer is detroyed, it MUST call
   * unregisterDtorObserver, The same observer can only register once,
  ...
  /** Enable the to force LOD degradation. This will suppress instances
   * immediately, (during the motion pass)  so that there won't be more than
   * maxNbInstance * dist / maxDist instances. This may not be desirable every
   * time since particle dissapear on screen, which may be noticeable.
   */
  ...
  // Inherited from CParticlesystemProcess. Change the coord system for thta
  // system.
   ...
   * needed in a few case only, and can only work if all the forces that apply
   * to the system are integrable. An assertion happens otherwise
   */
  ...
  /// inherited from CParticlesystemProcess perform parametric motion for this
  /// located to reach the given date
  virtual void performParametricMotion(TAnimationTime date);

  /// make the particle older of the given amount. Should not be called
  /// directly, as it is called by the system during its step method Dying
  /// particles are marked (removed in a later pass by called
  /// removeOldParticles)
  void updateLife();
  ...
  void integrateSingle(float startDate, float deltaT, uint numStep,
                       uint32 indexInLocated, NLMISC::CVector *destPos,
                       uint posStride = sizeof(NLMISC::CVector)) const;

  // compute position for a single element at the given date
  // NB : only works with object that have parametric trajectories
  inline void computeParametricPos(float date, uint indexInLocated,
                                   NLMISC::CVector &dest) const;

  /// Enable a trigger on death. It is used to create emission on an emitter
  /// with a given ID
  ...
  /** eval max duration of the located (if no scheme is used, this is the
   * lifetime) nb : return -1 if located last for ever
   */
...
protected:
  friend class CPSForce; // this is intended only for integrable forces that
                         // want to use registerIntegrableForce, and
                         // removeIntegrableForce
  ...
  // Keep vector of next positions during sim step (to avoid a copy after
  // position have been computed). Used only if there are collisions
  ...
  bool _ParametricMotion : 1; // When set to true, this tells the system to use
                              // parametric motion. Only parametric forces must
                              // have been applied.
  bool _TriggerOnDeath : 1;   // When set to true, then when any particle is
                              // destroyed, all particles with ID '_TriggerID'
                              // will be destroyed too
  bool _LastForever : 1;      // True if the located can't die.
  uint32 _TriggerID;
  /** Number of forces, (counts collision zones too). that are not integrable
   * over time. If this is not 0, then the trajectory cannot be computed at any
   * time. A force that is integrable must be in the same basis than the
   * located.
   */
  uint16 _NonIntegrableForceNbRefs;
  /// Number of forces that apply on that located that have the same basis that
  /// this one (required for parametric animation)
  ...
  // a container of masses. the inverse for mass are used in order to speed up
  // forces computation
...
public:
  /** WARNING : private use by forces only. This struct is used for parametric
   * trajectory. These kind of trajectory can only be computed in a few case,
  ...
  /** WARNING : private use by forces only. this vector is only used if
   * parametric motion is achievable and enabled, because of the extra storage
   * space
   *
   */
...
protected:
  ...
  /// notify the attached object that we have switch between parametric /
  /// incremental motion
...
public:
  /// PRIVATE USE: register a force that is integrable on this located. It must
  /// have been registered only once
  ...
  /// PRIVATE USE: says that an integrable force basis has changed, and says
  /// which is the right basis
  void integrableForceBasisChanged(TPSMatrixMode basis);

  /// PRIVATE USE: add a reference count that says that non-integrable forces
  /// have been added
  void addNonIntegrableForceRef(void);

  /// PRIVATE USE: decrease the reference count to say that a non-integrable
  ...
  /// PRIVATE USE :Should be only called by the sim loop when
  /// hasLODDegradation() returns true. /see forceLODDegradation
  void doLODDegradation();

private:
  // Special version for deleteElement, should be only called during the update
  // loop. If gives the amount of time that will ellapse until the end of the
  // simulation step, so that, if a particle is emitted 'On death' of its
  // emitter, it will have a correct pos at the next simultion step
  ...
  TAnimationTime
  computeDateFromCollisionToNextSimStep(uint particleIndex,
                                        float particleAgeInSeconds);
  // create a new element
  sint32 newElement(const CPSSpawnInfo &si, bool doEmitOnce,
...
public:
  ...
  /** this should be called before to delete any bindable inserted in a system,
   * but this is done by the system, so you should never need calling it. This
   * has been introduced because calls in dtor are not polymorphic to derived
   * class (which are already destroyed anyway), and some infos are needed in
   * some dtor. The default behaviour does nothing
   */
  virtual void finalize(void);
  /// dtor
  virtual ~CPSLocatedBindable();
  //@}
  /// Activate / Deactivate this object. When not active, the owning system
  /// won't try to call the 'step' method
  ...
  /** Can be used by located bindable that have located as targets (emitter,
   *collision zone, forces) to be notified that one of their target has been
   *removed. To do this : The object that focus the target must call
   *registerDTorObserver on the target, with himself as a parameter When the
   *target is removed, this target will call this method for all registered
   *CPSLocated The default behaviour remove this object as an observer
   *
  ...
  virtual void releaseAllRef();
  /***
   * The following is used to complete an aabbox that was computed using the
   * located positions You may not need to do anything with that, unless your
   * bindable has a space extents. For exAmple, with a particle which has a
   * radius of 2, you must enlarge the bbox to get the correct one. The default
   * behaviour does nothing \return true if you modified the bbox
   */
  virtual bool completeBBox(NLMISC::CAABBox & /* box */) const { return false; }
  /***
   * Override the following to say that you don't want to be part of a bbox
   * computation
   */
  ...
  inline NLMISC::CVector computeIWithZAxisAligned(void) const {
    return _Owner->computeIWithZAxisAligned();
  }
  /** Compute a vector that will map to (0 1 0) after view and model transform.
   *  This allow to  have object that always faces the user, whatever basis they
   * are in
  ...
  inline NLMISC::CVector computeKWithZAxisAligned(void) const {
    return _Owner->computeKWithZAxisAligned();
  }
  ...
  /** set the LODs that apply to that object (warning : it is based on the
   * position of the system, and don't act on a per instance basis ...) To have
   * per instance precision, you must use an attribute maker that has LOD as its
   * input
   */
  ...
  /** set the extern ID of this located bindable. 0 means no extern access. The
   * map of ID-locatedBindable. Is in th particle system, so this located
   * bindable must have been attached to a particle system, otherwise an
   * assertion is raised
   */
  void setExternID(uint32 id);
  /// get the extern ID of this located bindable
  uint32 getExternID(void) const { return _ExternID; }
  /** Called when the basis of the owner changed. the default behaviour does
   * nothing \param newBasis : True if in the system basis, false for the world
   * basis.
   */
  virtual void basisChanged(TPSMatrixMode /* systemBasis */) {}

  /// called when a located has switch between incrmental / parametric motion.
  /// The default does nothing
  virtual void motionTypeChanged(bool /* parametric */) {}

  // returns the number of sub-objects (including this one, that requires the
  // user matrix for its computations)
...
protected:
  ...
  /**	Generate a new element for this bindable. They are generated according
   * to the propertie of the class
   */
  virtual void newElement(const CPSEmitterInfo &info) = 0;

  // Delete element at the given index
  virtual void deleteElement(uint32 index) = 0;
  // Delete element at the given index. Gives the remaining time until the next
  // sim loop
  ...
  /** a bounce occurred, so some action could be done. The default behaviour
   * does nothing \param index the index of the element that bounced
   */
  virtual void bounceOccurred(uint32 /* index */,
                              TAnimationTime /* timeToNextsimStep */) {}

  /** show an drawing to represent the object, and in red if it is selected
   *  \param tab : a table of 2 * nbSeg vector. only the x and y coordinates are
   * used \param nbSeg : the number of segment \param scale  : the scale to use
   * for drawing
   */
...
protected:
  ...
  std::string _Name;
  //
  bool _Active; // Say if this bindable is active. If not active, the owning
                // system won't try to call 'step' on that object. True by
                // default
public:
  /** PRIVATE USE : called by the system when its date has been manually
   * changed. This his usually for object that expect time to be always
   * increasing, so that they can reset their datas
   */
  virtual void systemDateChanged() {}
};

/**
 * less operator on located bindable. They're sorted in decreasing priority
 * order
 */

inline bool operator<(const CPSLocatedBindable &lhs,
...
public:
  /** Add a new type of located for this to apply on. nlassert if already
   * present. You should only call this if this object and the target are
   * already inserted in a system. By overriding this and calling the
   * CPSTargetLocatedBindable version, you can also send some notificiation to
   * the object that's being attached.
   */
  virtual void attachTarget(CPSLocated *ptr);
  /** remove a target
   *  \see attachTarget
   */
  void detachTarget(CPSLocated *ptr) { notifyTargetRemoved(ptr); }
   ...
   *  can't call your releaseTargetRsc override in its destructor, it does it in
   * its finalize method, which is called by the particle system
   */
...
protected:
  friend class CPSLocated;
  /** Inherited from CPSLocatedBindable. A target has been remove If not present
   * -> assert This also call releaseTargetRsc for clean up
   */
  virtual void notifyTargetRemoved(CPSLocated *ptr);
...
// *****************************************************************************************************
inline const NLMISC::CMatrix &
CPSLocated::getConversionMatrix(const CPSLocated *A, const CPSLocated *B) {
  nlassert(A);
  nlassert(B);
  nlassert(
      A->_Owner ==
      B->_Owner); // conversion must be made between entity of the same system
...
inline TAnimationTime CPSLocated::getAgeInSeconds(uint elementIndex) const {
  ...
  if (_LastForever)
    return _Time[elementIndex];
  if (_LifeScheme)
    return _Time[elementIndex] / _TimeIncrement[elementIndex];
  return _Time[elementIndex] * _InitialLife;
}

// *****************************************************************************************************
inline void CPSLocated::computeParametricPos(float date, uint indexInLocated,
                                             NLMISC::CVector &dest) const {
...
// *****************************************************************************************************
inline const NLMISC::CMatrix &
CPSLocatedBindable::getLocalToWorldMatrix() const {
  nlassert(_Owner);

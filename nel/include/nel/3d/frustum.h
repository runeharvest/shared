...
// ***************************************************************************
/**
 * A Screen frustum, perspective or orthogonal. No default ctor. Useful for
 * camera. \author Lionel Berenguier \author Nevrax France \date 2000
 */
class CFrustum {
public:
  ...
  CFrustum() {}
  /// ctor.
  CFrustum(float left, float right, float bottom, float top, float znear,
           float zfar, bool perspective = true) {
    init(left, right, bottom, top, znear, zfar, perspective);
  }

  /// Init a frustum.
  void init(float left, float right, float bottom, float top, float znear,
            float zfar, bool perspective = true);
  ...
  void getValues(float &left, float &right, float &bottom, float &top,
                 float &znear, float &zfar) const;

  /** project a vector (x,y,z) onto frustum.
   * \param vec the point in 3D frustum space. Axis is NL3D axis: Xright,
   * Yfront, Ztop. \return the point in 2D: Xright, Ytop, Z=0.  Screen is mapped
   * to X:[0..1], Y: [0..1].
   */
   ...
   * \param vec the point in 3D frustum space. Axis is NL3D axis: Xright,
   * Yfront, Ztop. \return the point in 2D: Xright, Ytop, Z=0.  Screen is mapped
   * to X:[0..1], Y: [0..1] Z is a positive screen depth in world units.
   */
  CVector projectZ(const CVector &vec) const;

  /** unproject a point from screen to the frustum space.
   * \param vec the point on the screen, in Left hand axis (XRight, YTop,
   * ZFront). Z must be in [0..1] and hyperbolic. \return the point in the
   * frustum space (NL3D right hand axis).
   */
  CVector unProject(const CVector &vec) const;

  /** unproject a point from screen to the frustum space.
   * \param vec the point on the screen, in Left hand axis (XRight, YTop,
   * ZFront). Z is a positive depth in world unit. \return the point in the
   * frustum space (NL3D right hand axis).
   */
  CVector unProjectZ(const CVector &vec) const;

  /** return the aspect ration of this frustum. this is simply

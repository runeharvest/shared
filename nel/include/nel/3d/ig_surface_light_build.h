...
// ***************************************************************************
/**
 * Class used by NL3D::CInstanceLighter to build CIGSurfaceLight with PACS
 * surfaces \author Lionel Berenguier \author Nevrax France \date 2002
 */
...
public:
  ...
  struct CCellCorner {
    ...
    // Number of overSamples. At least one if InSurface. NB: Put here for
    // packing.
    ...
    // OverSamples. NB: there can be any number of overSamples (3, 4, 5,7 ...)
    // because some may be out of surface and thus disabled
...
public:
  ...
  void
  buildSunDebugMesh(CMesh::CMeshBuild &meshBuild,
                    CMeshBase::CMeshBaseBuild &meshBaseBuild,
                    const NLMISC::CVector &deltaPos = NLMISC::CVector::Null);

  /// Debug: build a colored Grid mesh of PointLight. R= pointLight1 id. G=
  /// PointLight2 id. B= The multiplier used to show Ids.
  void buildPLDebugMesh(CMesh::CMeshBuild &meshBuild,
                        CMeshBase::CMeshBaseBuild &meshBaseBuild,
                        const NLMISC::CVector &deltaPos,
                        const CInstanceGroup &igOut);

private:
  void addDebugMeshFaces(CMesh::CMeshBuild &meshBuild, CSurface &surface,
                         uint vId0, const std::vector<NLMISC::CRGBA> &colors);
};

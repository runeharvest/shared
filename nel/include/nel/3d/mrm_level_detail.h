...
// ***************************************************************************
/* This is used to have correct max Polygon count setuped. This suppose that
   MRM.NumLod==10 Hence, a mean value of 0.5*1/10 is used
*/
  ...
  float getNumTriangles(float dist) {
    ...
    if (ld <= 0)
      ...
      /* over-estimate the number of poly rendered
          because this is always the higher Lod which is rendered, geomorphing
         to the coarser one. NB: still need to over-estimate if ld==1, because
         getLevelDetailFromPolyCount() remove the shift...
      */
  ...
  float getLevelDetailFromPolyCount(float polygonCount) {
    ...
    if (MaxFaceUsed > MinFaceUsed) {
      ...
      /* remove the value added in getNumTriangles(). For the same reason:
          this is always the higher Lod which is rendered, geomorphing to the
         coarser one. Hence we must degrade a bit. NB: if
         polygonCount==MinFaceUsed, then we have here ld=
         -NL3D_MRM_LD_SHIFT_POLY_COUNT (ie -0.05f) but it is clamped below
      */
      ld -= NL3D_MRM_LD_SHIFT_POLY_COUNT;
      NLMISC::clamp(ld, 0, 1);
    } else

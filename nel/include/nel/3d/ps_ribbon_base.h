 ...
 * NB : Ribbons that don't herit from this are deprecated but may be kept for
 * compatibility. \author Nicolas Vizerie \author Nevrax France \date 2002
 */
...
public:
  ...
  enum TMatrixMode {
    ...
    IdentityMatrix,
    UserMatrix,
    FatherMatrix, // father located matrix (introduced for backward
                  // compatibility)
    MatrixModeCount
  };
  ...
  ///\name Geometry
  ///@{
  /// set the number of segments used with this particle. In this case, it can't
  /// be lower than 2
   ...
   * longer ribbons. Default is 0.02. It gives the sampling rate for each type
   * of ribbon
   */
  void setSegDuration(TAnimationTime ellapsedTime);
  TAnimationTime getSegDuration(void) const { return _SegDuration; }

  /** The length in meter of the ribbon. This is used only if the ribbon mode is
   * set to FixedSize. These kind of ribbon are usually slower than variable
   * size ribbons. The default is one metter.
   */
  void setRibbonLength(float length);
  float getRibbonLength() const { return _RibbonLength; }
  ///@}

  /** Allow degradation of ribbons with distance of the system (may not be
   * suited when theit paths have wicked angles) \param percent 1 mean no
   * degradation, 0 mean nothing will be draw when the system is at its max
   * dist. 1 is the default
   */
...
protected:
  ...
  typedef CPSVector<float>::V
      TFloatVect; // all positions for each ribbons packed in a single vector

  uint32 _NbSegs;
  TAnimationTime _SegDuration;
  bool _Parametric; // if this is set to true, then the owner has activated
                    // parametric motion.
  ...
  /// called when the motion type has changed, this allow us to draw smoother
  /// ribbons when parametric anim is used
   ...
   */
  void computeRibbon(uint index, NLMISC::CVector *dest,
                     uint stride = sizeof(NLMISC::CVector));

  /// Called each time the time of the system change in order to update the
  /// ribbons positions
  void updateGlobals();
...
private:
  ...
  /// point.
  void computeLinearRibbon(uint index, NLMISC::CVector *dest,
                           uint stride = sizeof(NLMISC::CVector));
  /// The same as compute linear ribbon but try to make its length constant
  void computeLinearCstSizeRibbon(uint index, NLMISC::CVector *dest,
                                  uint stride = sizeof(NLMISC::CVector));
  /// Compute the ribbon points using hermitte splines between each sampling
  /// point.
  void computeHermitteRibbon(uint index, NLMISC::CVector *dest,
                             uint stride = sizeof(NLMISC::CVector));

  /** Compute the ribbon points using hermitte splines between each sampling
   * point, and make a rough approximation to get a constant length
   */
  void computeHermitteCstSizeRibbon(uint index, NLMISC::CVector *dest,
                                    uint stride = sizeof(NLMISC::CVector));
...
CPSRibbonBase::getLocalToWorldTrailMatrix() const {
  NL_PS_FUNC(CPSRibbonBase_getLocalToWorldTrailMatrix)
#ifdef NL_DEBUG
  nlassert(_Owner);
  nlassert(_Owner->getOwner());
#endif
  return CPSLocated::getConversionMatrix(*_Owner->getOwner(), PSIdentityMatrix,
                                         convertMatrixMode());
}

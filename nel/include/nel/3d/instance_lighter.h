...
// ***************************************************************************
/**
 * A class to precompute "StaticSetup" lighting for instances in an
 *InstanceGroup. Class "inspired" from CZoneLighter :) \author Lionel Berenguier
...
public:
  ...
  public:
    ...
    // Disable Sun contribution: ie set 0 to all contributions of sun on
    // instances and surfaces?? false by default
    ...
    // Number of samples For sun shadowing only. Must be: 0 (disable), 2, 4, 8,
    // 16
    uint OverSampling;
...
public:
  /**	Tool method which take a single IG, and do all the god job to light this
   *one, with no other dependencies NB: it uses instLighter passed, init() ing
   *it. It use lightDesc.UserShapeMap or it load Shape in directory
   *lightDesc.ShapePath
   */
  static void lightIgSimple(CInstanceLighter &instLighter,
                            const CInstanceGroup &igIn, CInstanceGroup &igOut,
                            const CLightDesc &lightDesc, const char *igName);
...
public:
   ...
   *	igOut has different PointLights than igIn. eg: if a pointLight do not
   *light anything, then it is not present in igOut. NB: shapes are used to
   *retrieve useful info on them (center of AABBox ...) . They are taken from
   *	lightDesc.UserShapeMap, or loaded from lightDesc.ShapePath if not found.
   *	\param landscape if !NULL use this Landscape SunContribution, looking
   *landscape faces under each instance, for faster computing, and to get
   *influence of Sky. NB: this landscape does not have to be tesselated, but all
   *Zones that lies under igIn should be loaded in. \param igSurfaceLightBuild
   *if !NULL, light() will compute igOut.IGSurfaceLight, else it is just
   *cleared.
   */
  void light(const CInstanceGroup &igIn, CInstanceGroup &igOut,
             const CLightDesc &lightDesc, std::vector<CTriangle> &obstacles,
             CLandscape *landscape = NULL,
             CIGSurfaceLightBuild *igSurfaceLightBuild = NULL);

  // Add triangles from a landscape
  static void addTriangles(CLandscape &landscape, std::vector<uint> &listZone,
                           uint order, std::vector<CTriangle> &triangleArray);

  /** Add triangles from a transform shape. Work only for CMesh, CMultiMesh and
   *CMeshMRM all without skinning. Give An instanceId!=-1 only for instances of
   *the IG to compute. This is to avoid auto-shadowing.
   */
  static void addTriangles(const IShape &shape, const NLMISC::CMatrix &modelMT,
   ...
   *light() ). NB: you must append all PointLights of intersets, even ones from
   *the IG to compute.
   */
  ...
  // ********************************
private:
  static void addTriangles(const CMeshGeom &meshGeom,
                           const NLMISC::CMatrix &modelMT,
                           std::vector<CTriangle> &triangleArray,
                           sint instanceId);
  static void addTriangles(const CMeshMRMGeom &meshGeom,
                           const NLMISC::CMatrix &modelMT,
                           std::vector<CTriangle> &triangleArray,
                           sint instanceId);
  static void excludeAllPatchFromRefineAll(CLandscape &landscape,
                                           std::vector<uint> &listZone,
                                           bool exclude);

  // light / rayTrace against sun.
  void computeSunContribution(const CLightDesc &lightDesc,
...
private:
  ...
  struct CPointLightRT {
     ...
     *BSphere. If occluded or out of radius, return false, else return true. if
     *instanceComputed>=0, then skip obstacles with
     *InstanceId==instanceComputed. Also Skip if the light is an Ambient, and
...
private:
  /// \name Cell Iteration. ie iteration on _IGRetrieverGridMap cells and
  /// _IGSurfaceLightBuild cells
  // @{
  ...
  CIGSurfaceLightBuild::CCellCorner &getCurrentNeighborCellInfo(sint xnb,
                                                                sint ynb);

...
#define NL_MAX_SIZE_OF_TEXTURE_EDGE_SHIFT                                      \
  (NL_MAX_TILES_BY_PATCH_EDGE_SHIFT + NL_NUM_PIXELS_ON_FAR_TILE_EDGE_SHIFT)
#define NL_MAX_SIZE_OF_TEXTURE_EDGE                                            \
  (1 << NL_MAX_SIZE_OF_TEXTURE_EDGE_SHIFT) // Size max of a far texture edge in
                                           // pixel
...
const sint NbTileLightMapByTexture =
    NbTileLightMapByLine * NbTileLightMapByLine;
...
// ***************************************************************************
/**
 * A landscape. Use CZone to build zone, and use landscape to dynamically
 *add/remove them, for render.
 *
 *	Limits:
 ...
 *		- The value of Noise amplitude is global and cannot go over 1
 *meter (+- 1m). Sorry, this is a FIXED (for ever) value which should NEVER
 *change (because of Gfx database).
 *
 *	If you use the tiles mapped on the patches, load the near bank file
 *(.bank) and the far bank file (.farbank) by seralizing TileBank and
 *TileFarBank with those files. Then call initTileBanks.
 *
 * \author Lionel Berenguier
 * \author Nevrax France
 * \date 2000
...
public:
  // The bank of tiles information. Each time you change the bank, you should
  // call CLandscape::initTileBanks();
  CTileBank TileBank;
...
public:
   ...
   * CLandscape compile it. The contents of newZone are copied into the
   * landscape. \param newZone the new zone. \return true if OK, false
   * otherwise. As example, Fail if newZone is already connected.
   */
   ...
   * \return true if OK, false otherwise. As example, Fail if zone is not
   * connected.
   */
  ...
  void checkBinds();
  /// Verify the binding of patchs of one zone. throw EBadBind if error. nop if
  /// zone not loaded.
  ...
  // @{
  /// Set tile near distance. Default 50.f. maximized to length of Far alpha
  /// transition).
  void setTileNear(float tileNear);
  /// Get tile near distance.
  float getTileNear() const { return _TileDistNear; }
  /// Set threshold for subdivsion quality. The lower is threshold, the more the
  /// landscape is subdivided. Default: 0.001.
  ...
  /// Set Maximum Tile subdivision. Valid values must be in [0..4]  (assert).
  /// Default is 0 (for now :) ).
  void setTileMaxSubdivision(uint tileDiv);
  /// Get Maximum Tile subdivision.
  uint getTileMaxSubdivision();

  /// Enable the noise or not. NB: only new tesselation computed is modified, so
  /// you should call it only at init time.
  ...
  // @{
  /**
   *	A driver is needed for VB allocation. The VB will be allocated only the
   *first time the landscape is clipped. Because no VB are created for invisible
   *patchs. call setDriver() before any clip().
   */
   ...
   *	A more precise clip is made on TessBlocks. pyramid should be the same as
   *one passed to clip(). For optimisation, this pyramid should contains only
   *the Left/Right and Top/Bottom clip planes, in this order. \param
   *refineCenter should be the position of the camera \param frontVector should
   *be the J vector of the camera
   */
  void render(const CVector &refineCenter, const CVector &frontVector,
              const CPlane pyramid[NL3D_TESSBLOCK_NUM_CLIP_PLANE],
              bool doTileAddPass = false);

  /// Refine/Geomorph ALL the tesselation of the landscape, from the view point
  /// refineCenter. Even if !RefineMode.
  ...
  /** This is especially for Pacs. Each Vertex->EndPos which is not a corner of
   *a patch is set to the mean of its 2 shared Patchs. NB: Works with special
   *cases of rectangular patchs and binded patchs.
   */
  ...
  // @{
  /** Build the set of faces of landscape, which are IN a bbox. Useful for
   * collisions. The faces are builded at Tile level (2m*2m). \param bbox the
   * bbox where faces are searched. \param faces the result of the build. \param
   * faceSplit if true, Only faces which are IN or partialy IN the bbox are
   * returned. Else the clipping is done on patch level. Worst, but faster.
   */
  void buildCollideFaces(const CAABBoxExt &bbox, std::vector<CTriangle> &faces,
                         bool faceSplit);
  /** Build the set of faces of landscape, from a certain patch. Useful for
   * collisions. Triangles are built first in S then T order. There is two
   * triangles by tiles. So the number of triangles for a patch is
   * 2*OrderS*OrderT.
   */
  void buildCollideFaces(sint zoneId, sint patch,
                         std::vector<CTriangle> &faces);

  /** method relatively identical than buildCollideFaces(bbox....). Differences
   * below: NB: this method use first a quadgrid to locate patchs of interest,
   * then for each patch, it uses a convex hull subdivion to search in O(logn)
   * what part of the patch to insert. \param bbox the bbox to test against. NB:
   * you should modify your bbox according to heighfield. \param triangles array
   * to be filled (array first cleared, then elements added). \param
   * tileTessLevel 0,1 or 2  size of the triangles (2*2m, 1*1m or 0.5*0.5m).
   * Level of subdivision of a tile.
   */
  void buildTrianglesInBBox(const CAABBox &bbox,
                            std::vector<CTrianglePatch> &triangles,
                            uint8 tileTessLevel);

  /** Same as buildTrianglesInBBox(), but fill blockId instead of raw triangles.
   * NB: this method use first a quadgrid to locate patchs of interest, then for
   * each patch, it uses a convex hull subdivion to search in O(logn) what part
   * of the patch to insert. \param bbox the bbox to test against. NB: you
   * should modify your bbox according to heighfield. \param paBlockIds array to
   * be filled (no clear performed, elements added).
   */
  void buildPatchBlocksInBBox(const CAABBox &bbox,
  ...
  /** From the current tesselation of a patch of landscape, and a UV in this
   * patch, return tesselated position. NB: return Null if patch not found.
   */
   ...
   *	Warning: ptrs are only valid between 2 tesselation (2 calls of
   *Scene::render() or 2 calls of refine*() etc...)
   */
  void getTessellationLeaves(std::vector<const CTessFace *> &leaves) const;

  /** Get the camera 3rd person collision against the TileFaces (ie only under
   *approx 50 m) return a [0,1] value. 0 => collision at start. 1 => no
   *collision. \param radius is the radius of the 'cylinder' \param cone if
   *true, the object tested is a cone (radius goes to end)
   */
   ...
   * \return Return a zone pointer. NULL if the zone doesn't exist or isn't
   * loaded.
   */
  ...
  /// From an Id, return the name of the zone, in the form "150_EM", without
  /// extension.
  ...
  /** Force a range of tiles to be unloaded. You should call
   *changePatchTextureAndColor() on all patch or ensure that all zones are
   *deleted.
   */
  ...
  NLMISC::CSmartPtr<ITexture> getTileTexture(uint16 tileId,
                                             CTile::TBitmap bitmapType,
                                             CVector &uvScaleBias);

  /// Return the tile element for a patch at specific UV. Useful for getting
  /// surface data. Return NULL if not found.
   ...
   *  \param multiply is the multiply factor. Final color is
   * (diffuse*multiply*shading+ambiant*(1.0-shading))
   */
  void setupStaticLight(const CRGBA &diffuse, const CRGBA &ambiant,
                        float multiply);
  /**
   *  Get the light color by shading table.
   *
   *  \return a CRGBA[256] array. It give the static light color for a shading
   * value.
   */
  const CRGBA *getStaticLight() const { return _LightValue; }

  /**
   *  Setup the equivalent material diffuse component used for both Static and
   *Dynamic PointLights. Default is White.
   */
   ...
   *
   *  \param lightDir is the direction of the light vector used for the
   * lighting. NB: the vector is normalized.
   */
  ...
  /** This method remove all PointLights in all Zones, and hence reset
   * TileLightInfluences.
   */
  void removeAllPointLights();

  /// update the Light factor for all pointLights in All zones, according to
  /// scene LightGroups and AnimatedLights.
   ...
   * only one is possible. You should setup this heightfield around the zones
   * which will be loaded. It is applied only when a zone is loaded, so you
   * should setup it 2km around the user, each time you move too far from a
   * previous place (eg 160m from last setup).
   */
  ...
  /** enable the vegetable management in landscape. Valid only if VertexShader
   *is OK. if true, register TileBank vegetables Shape to manager.
   */
   ...
   */
  void setupVegetableLighting(const CRGBA &ambient, const CRGBA &diffuse,
   ...
   *
   *	\param windDir is the direction of the wind. NB: only XY direction is
   *kept. \param windFreq is the frequency for the animation (speed) \param
   *windPower is the power of the wind, and is a factor (0..1) of Bend \param
   *windBendMin is a value in (0..1) which indicate how much the vegetables are
   *bended at minimum (for very powerfull wind)
   */
   ...
   *	Out  CPrimitiveProfile::NTriangles displayed by vegetable part is
   *returned.
   */
  uint getNumVegetableFaceRendered() const;

  /** set the frequency of Vegetable lighting update. If freq==1, ALL lighted
   *igs are updated each second. e.g: if 1/20, then every 20 seconds, all Igs
   *are updated. If you set 0, no update will be done at all (this is the
   *default setup!!).
   */
  void setVegetableUpdateLightingFrequency(float freq);

  /** Debug purpose only : setup the colors of the patch of all the currently
   * loaded zones so that it shows which tiles have vegetable disabled, or are
   * above, below water. User provides a table with 4 colors for each state :
   * color 0 = above water
   * color 1 = underwater
  ...
  /** Set a density ratio [0, 1] to possibly reduce the amount of micro
   * vegetable drawn. Default to 1
   */
  ...
  /** Set the ModelPosition (for Precision ZBuffer purpose). NB: the model Pos
   *may be floor-ed (for greater float precision). Should be setup to the camera
   *position each frame.
   *
   *	NB: if vertexProgram is used, it is as faster as before (because of
   *geomorph done each frame, and because of VP MAD instruction). NB: if
   *vertexProgram is not used, it is a little slower, because of the
   *substraction.
   */
  ...
  /** set the frequency of lighting update. If freq==1, ALL patchs are updated
   *each second. e.g: if 1/20, then every 20 seconds, all patchs are updated. If
   *you set 0, no update will be done at all (this is the default setup!!).
   */
  void setUpdateLightingFrequency(float freq);

  /** update the lighting of ALL patch (slow method). NB: work even if
   *UpdateLightingFrequency==0 Additionaly, vegetables are also ALL updated.
   */
  ...
  /** For bench only. return approximate ammount of memory taken by dynamic
   *lightmap in RAM. This is sizeof(global Texture in RAM) + all
   *patchDLMContext. NB: not so slow, but do not call in final release.
   */
  uint getDynamicLightingMemoryLoad() const;

  /** Set PointLight Max Attenuation End landscape support. Every pointLight
   *AttEnd is clamped to this value. Default is 30.f.
   */
  ...
  /** For Vegetable Dynamic ligthing only: this is an approximate color of all
   *vegetables. Default is (180, 180, 180).
   */
  ...
  // @{
  void receiveShadowMap(IDriver *drv, CShadowMap *shadowMap,
                        const CVector &casterPos, const CMaterial &shadowMat,
                        const CVector &pzb);
  // @}
...
private:
  ...
  // This is not a valid TessBlock. But it is used as a Root for modification
  // list. Can't use a ptr, because of ~CTessBlock().
  CTessBlock _TessBlockModificationRoot;
  // @}

  // Return the render pass for a far texture here.
  CPatchRdrPass *getFarRenderPass(CPatch *pPatch, uint farIndex,
                                  float &far1UScale, float &far1VScale,
                                  float &far1UBias, float &far1VBias,
                                  bool &bRot);
  ...
  // Return the UvScaleBias for a tile Id. uv.z has the scale info. uv.x has the
  // BiasU, and uv.y has the BiasV. if bitmap type is CTile::alpha, Return also
  // the additionla rot for alpha (else 0).
  void getTileUvScaleBiasRot(uint16 tileId, CTile::TBitmap bitmapType,
                             CVector &uvScaleBias, uint8 &rotAlpha);

  // release Far render pass/reset Tile/Far render. Delete also VB, and
  // FaceVectors
  ...
  // update TheFaceVector for which the faces may have been modified during
  // refine(), refineAll() etc....
...
private:
  ...
  // For VertexProgram. true if change has occurred in threshold since the last
  // render().
  ...
  // Tiles Types.
  //=============
  // Texture Map. Use a RefPtr because TileTextureMap must not reference the
  // object, but the ptr.
  ...
  struct CTileInfo {
    // NB: CSmartPtr are not used for simplicity, and because of
    // TTileRdrPassSet... CPatchRdrPass::RefCount are excplictly
    // incremented/decremented... The rdrpass for diffuse+Alpha material.
  ...
  // The parrallel array of tile of those existing in TileBank. size of
  // NbTilesMax.
  ...
  bool eraseTileFarIfNotGood(uint tileNumber, uint sizeOrder0, uint sizeOrder1,
                             uint sizeOrder2);
...
private:
  ...
  // Tile LightMap mgt. NB: a lightmap is now a 2x2 tiles lightmap (10x10
  // pixels).
  // @{
  // Compute and get a lightmapId/lightmap renderpass.
  // lightmap returned is to be uses with getTileRenderPass(). The id returned
  // must be stored.
  uint getTileLightMap(CRGBA map[NL_TILE_LIGHTMAP_SIZE * NL_TILE_LIGHTMAP_SIZE],
  ...
  // refill a lightmap already computed. tileLightMapId must be the id returned
  // by getTileLightMap().
  void
  refillTileLightMap(uint tileLightMapId,
  ...
  struct CBezierPatchZ {
    ...
    float Vertices[4];
    /// The tangents ab, ba, bc, cb, cd, dc, da, ad. NB: tangents are points,
    /// not vectors.
    float Tangents[8];
   ...
   * of the patch to insert. \param bbox the bbox to test against. \param
   * triangles array to be filled (no clear performed, elements added). \param
   * tileTessLevel 0,1 or 2  size of the triangles (2*2m, 1*1m or 0.5*0.5m).
   * Level of subdivision of a tile.
   */
  void addTrianglesInBBox(const CPatchIdentEx &paIdEx, const CAABBox &bbox,
                          std::vector<CTrianglePatch> &triangles,
                          uint8 tileTessLevel) const;

  /** private version of buildPatchBlocksInBBox, searching only for one patch.
   */
  void addPatchBlocksInBBox(const CPatchIdentEx &paIdEx, const CAABBox &bbox,
                            std::vector<CPatchBlockIdent> &paBlockIds);
  // @}
  ...
  /** For a given tile Id, look into tileSet, and get the tile vegetable
   *descriptor \param tileId the tile [0..65535] to get the list of vegetable to
   *create.
   */
  ...
  /// The diffuse material of landscape, used for StaticPointLights and
  /// DynamicPointLights

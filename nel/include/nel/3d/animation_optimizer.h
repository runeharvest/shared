 ...
 *	Quaternion tracks are optimized. They are transformed to
 *CTrackSampledQuat or CTrackDefaultQuat. Vector tracks are optimized. They are
 *transformed to CTrackSampledVector or CTrackDefaultVector. \author Lionel
...
public:
   ...
   *	Default is 0.0001 and 0.000001. Above this value, 2 quaternions are said
   *different. NB: comparing 2 quaternions is made by multiplying the one with
   *the inverse of the other. the W component is then tested and must be
   *>1-threshold (modulo sign) to conclude to equality. Since
   *quat.W=cos(angle/2), you can deduce the angle threshold with
   *angleThre=acos(1-thre)*2
   *
   *	Give 2 values, one For Low precision and High precision. Default setup
   *use high precision. Use addLowPrecisionTrack() to drive low precision
   *tracks.
   */
  void setQuaternionThreshold(double lowPrecThre, double highPrecThre);

  /** Same principle as for setQuaternionThreshold(), but for vector tracks
   *(positions/scale). Default is 0.001 and 0.0001. Above this value, 2 vectors
   *are said different. NB: comparing 2 vectors is made by geting the norm of
   *the difference
   *
   *	Give 2 values, one For Low precision and High precision. Default setup
   *use high precision. Use addLowPrecisionTrack() to drive low precision
  ...
  /** see setQuaternionThreshold(). Any track which contains this name will be
   *considered as a low precision track. Default setup is empty, so all tracks
   *are "high precision" tracks. A good setup is for example
   *addLowPrecisionTrack("Finger") and addLowPrecisionTrack("Ponytail") Warning:
   *case sensitive
   */
   ...
   *	Default is 30 fps. NB: final numSamples must be <65535, else assert at
   *optimize() time.
   */
...
private:
  ...
  // return an optimized version of trackIn. Type returned should be different
  // (eg: CTrackSampledQuat)
  ...
  /** sample the track from beginTime to endTime, sample to numSamples, such
   *that key[0].Time==beginTime and key[numSamples-1].Time==endTime. NB:
   *quaternions are normalized, and are makeClosest()-ed from sample to the
   *next.
   */
  void sampleQuatTrack(const ITrack *trackIn, float beginTime, float endTime,
                       uint numSamples);
  ...
  /** sample the track from beginTime to endTime, sample to numSamples, such
   *that key[0].Time==beginTime and key[numSamples-1].Time==endTime.
   */
  void sampleVectorTrack(const ITrack *trackIn, float beginTime, float endTime,
                         uint numSamples);

...
public:
  /// \name Object
  // @{
  CDriverUser(uintptr_t windowIcon, UDriver::TDriver driver,
  ...
  virtual bool setDisplay(nlWindow wnd, const CMode &mode, bool show,
                          bool resizeable);
  ...
  virtual UTextureMem *createTextureMem(uint width, uint height,
  ...
  /** UScene ignore those function (use camera parameters instead), and do not
   * disturb this active Matrix context. (after a scene rendering, the Matrix
   * context for this interface is restored).
   */
  // @{
  ...
  virtual void drawTriangles(const std::vector<NLMISC::CTriangleColorUV> &tris,
                             UMaterial &mat);
  virtual void drawQuads(const NLMISC::CQuadColorUV *quads, uint32 nbQuads,
                         UMaterial &mat);
  ...
  // @{
  ///
  virtual UShapeBank *getShapeBank() { return &_ShapeBank; }
  // @}
  ...
  virtual bool stretchRect(UScene *scene, class UTexture &srcUText,
                           NLMISC::CRect &srcRect, class UTexture &destUText,
                           NLMISC::CRect &destRect);

  virtual bool setRenderTarget(class UTexture &uTex, uint32 x = 0, uint32 y = 0,
                               uint32 width = 0, uint32 height = 0,
                               uint32 mipmapLevel = 0, uint32 cubeFace = 0);

public:
  /// \name Accessor for CSeneUser.
  // @{
  IDriver *getDriver() { return _Driver; }
  void restoreMatrixContext() { setupMatrixContext(); }

 ...
 * Derivers should at least define the emit method which is called each time an
 * emission is needed. Not sharable accross systems.
 *
...
public:
  ...
  /// This is called inside the sim step, and triggers emitter that where
  /// flagged as 'emit once'
  void doEmitOnce(uint firstInstanceIndex);

  /// Display the emitter in edition mode
  virtual void showTool();

  /** Set the type of located to be emitted. The default is NULL which mean that
   * no emission will occur \return true if the operation could be performed. It
   * can fail when this cause the system the system to last forever, which is
   * incompatible with the 'BypassMaxNumIntegrationSteps' flag in
   * CParticleSystem
   */
  bool setEmittedType(CPSLocated *et);

  /** Inherited from CPSLocatedBindable
   *  We register to the emitted type (when setEmittedType is called), so, this,
   * this will be called when it is destroyed
   */
   ...
   *  regular     : means use Period, and generation number (the number of
   * particle to generate when an emission occurs) onDeath     : emit when the
   * emitter is destroyed once        : emit when the emitter is created
   *  onBounce    : emit when the emitter bounce
   *  externEmit  : emitted explicitly by the system user. A 4 letters ID must
   * be used to identify this kind of emitters the default ID is NONE
   */
  enum TEmissionType {
    regular = 0,
    onDeath = 1,
  ...
  /** Set the emission type. Please note that if the type is externEmit, this
   * located need to have been attached to the system (the system is holding the
   * ID-Located map) \return true if the operation could be performed. It can
   * fail when this cause the system the system to last forever, which is
   * incompatible with the 'BypassMaxNumIntegrationSteps' in CParticleSystem
   */
  ...
  /** Set a max. number of particle emission (0 means no limit and is the
   * default). Applies with regular emitter only. NB : the emitter should be
   * inserted in a system for this call to work \return true if the operation
   * could be performed. It can fail when this cause the system the system to
   * last forever, which is incompatible with the 'BypassMaxNumIntegrationSteps'
   * in CParticleSystem
   */
  bool setMaxEmissionCount(uint8 count);

  /// Get the max. number of particle emission (0 means no limit and is the
  /// default). Applies with regular emitter only.
  ...
  //@{
  /** Set a factor, to add the emitter speed to the emittee creation speed. this
   * can be < 0 The default is 0
   */
  void setSpeedInheritanceFactor(float fact) { _SpeedInheritanceFactor = fact; }
   ...
   * NB This also implies that the coord. system in which the speed vector is
   * expressed if the same than the one of the emitter so the calls to
   * enableUserMatrixModeForEmissionDirection() &
   * setUserMatrixModeForEmissionDirection() have no effects (but their value is
   * retained)
   */
  ...
  /** By default, the direction of emission is supposed to be expressed in the
   * same coordinate system than the one of the emitter. Enabling a user matrix
   * mode for the direction of emission allows to change that behaviour. example
   * of use : A fire p.s is linked to a torch, but the torch doesn't point to
   * the top. So particles are emitted in the axis aligned to the torch If
   * matrix mode for direction emission is set to PSIdentityMatrix, then the
   * direction is interpreted to be in world, and is thus independant from the
   * torch orientation : particles are always spawned in the +K direction.
   *
   * NB  : if isSpeedBasisEmissionEnabled() == true then this flag is
   * meaningless
   */
  void enableUserMatrixModeForEmissionDirection(bool enable = true);
  bool isUserMatrixModeForEmissionDirectionEnabled() const {
    return _UserMatrixModeForEmissionDirection;
  }
  /** Set the coord. system in with the direction is expressed. This value is
   * taken in account only if enableUserMatrixModeForEmissionDirection(true) has
   * been called. NB  : if isSpeedBasisEmissionEnabled() == true then this value
   * is meaningless
   */
  void setUserMatrixModeForEmissionDirection(TPSMatrixMode matrixMode);
  TPSMatrixMode getUserMatrixModeForEmissionDirection() const {
    return _UserDirectionMatrixMode;
  }
  //@}
  ...
  /** Enable consistent emission. The default is false. This try to keep the
   * number of emitted particle constant, by allowing more than one emission
   * cycle per iteration. This is useful to deal with poor frmerate. This has
   * several drawbacks though :
   * - collisions are not properly supported in this case (may be resolved
   * later). RESOLVED
   * - The motion is in straight lines.
   * - It assumes that emitter has no motion (for now). RESOLVED
   * NB nico : this is the default now ...
   * In fact, this should be used when there can't be collisions with the
   * emitted particles, and with main emitters only. NB : this has no effect if
   * the emission period is 0 (which mean emit at each frame)
   */
  ...
  /** For edition only : avoid that a call to CPSLocated::deleteElement() causes
   * emitters flagged with 'emitOnDeath' to emit
   */
  ...
  /** Test is the emitter will emit an infinite amount of particles (e.g it
   * doesn't stop after a while) NB : If the emitter isn't inserted in a
   * CPSLocated instance, an assertion will be reaised
   */
  ...
  // Set the emit trigger. At the next sim step, all particles will emit at the
  // start of the step.
  void setEmitTrigger() { _EmitTrigger = true; }
  // Update emit trigger. To Be called at the start of the sim step.
  // If emit trigger is set, all emitter emit a single time, and then the falg
  // is cleared
...
protected:
  ...
  /// This will call emit, and will add additionnal features (speed addition and
  /// so on). This must be called inside the sim loop.
  ...
  /// The same as processEmit, but can also add a time delta. This must be
  /// called inside the sim loop.
  void processEmitConsistent(const NLMISC::CVector &emitterPos,
                             uint32 emitterIndex, sint nbToGenerate,
   ...
   */
  void processRegularEmissionConsistent(uint firstInstanceIndex, float emitLOD,
  ...
  /** The particle system maintains a ref counter to see how many object
   * requires the user matrix for their computation (if it isn't required, a
   * significant amount of memory used for maintenance can be saved) This tool
   * function helps increasing / decreasing that count by seeing if the matrix
   * is still required or not
   */
  void updatePSRefCountForUserMatrixUsage(bool matrixIsNeededNow,
                                          bool matrixWasNeededBefore);

  /** This method is called each time one (and only one) located must be
   * emitted. DERIVERS MUST DEFINE THIS \param srcPos the source position of the
   * emitter (with eventually a correction) \param index the index of the
   * emitter in the tab that generated a located \param pos the resulting pos of
   * the particle, expressed in the emitter basis \param speed the reulting
   * speed of the emitter, expressed in the emitter basis
   */
  virtual void emit(const NLMISC::CVector &srcPos, uint32 index,
                    NLMISC::CVector &pos, NLMISC::CVector &speed) = 0;
   ...
   *  Attributes of the located that hold this bindable are still accessible for
   * of the index given index out of range -> nl_assert
   */
  ...
  /** Resize the bindable attributes containers. DERIVERS SHOULD CALL THEIR
   * PARENT VERSION should not be called directly. Call CPSLocated::resize
   * instead
   */
  ...
  /** The phase (  0 < phase  < period of emission). This is the time ellapsed
  ...
  bool
      _UserMatrixModeForEmissionDirection : 1; // true when the direction of
                                               // emission is expressed in a
                                               // user defined coordinate system
                                               // (otherwise it is expressed in
                                               // this object coordinate system,
                                               // as specified by setMatrixMode)
  bool _EmitTrigger : 1;
 ...
 *  It modulate the speed of emitted particle by a coeeficient produced by an
 * attribute maker
 */
...
public:
  ...
  /// dtor
  virtual ~CPSModulatedEmitter() { delete _EmitteeSpeedScheme; }
  ...
  void setEmitteeSpeedScheme(CPSAttribMaker<float> *scheme) {
    ...
    if (getModulatedEmitterOwner() && scheme->hasMemory())
      scheme->resize(getModulatedEmitterOwner()->getMaxSize(),
...
protected:
  // emitter must define this in order to allow this class to access the located
  // owner
  virtual CPSLocated *getModulatedEmitterOwner(void) = 0;

  void newEmitteeSpeedElement(const CPSEmitterInfo &info) {
    if (_EmitteeSpeedScheme && _EmitteeSpeedScheme->hasMemory())
      _EmitteeSpeedScheme->newElement(info);
  }

  void deleteEmitteeSpeedElement(uint32 index) {
    if (_EmitteeSpeedScheme && _EmitteeSpeedScheme->hasMemory())
      _EmitteeSpeedScheme->deleteElement(index);
  }

  void resizeEmitteeSpeed(uint32 capacity) {
    if (_EmitteeSpeedScheme && _EmitteeSpeedScheme->hasMemory())
      _EmitteeSpeedScheme->resize(capacity,
                                  getModulatedEmitterOwner()->getSize());
  }
...
class CPSEmitterDirectionnal : public CPSEmitter,
                               public CPSModulatedEmitter,
                               public CPSDirection {
...
/// ...)
class CPSEmitterRectangle : public CPSEmitter,
                            public CPSModulatedEmitter,
                            public IPSMover,
                            public CPSDirection {
public:
...
/// The same as a directionnel emitter, but you can also specify the radius for
/// emission
...
// A spherical emitter
class CPSSphericalEmitter : public CPSEmitter,
                            public CPSModulatedEmitter,
                            public IPSMover {
public:
  // Ctor
  ...
  NLMISC::CVector getScale(uint32 index) const {
    return NLMISC::CVector(_Radius[index], _Radius[index], _Radius[index]);
  }

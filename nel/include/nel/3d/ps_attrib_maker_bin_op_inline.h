 ...
 * We don't override the usual operators, because we may want behaviour such as
 * saturation, and this may be misleading with usual operators
 */

template <class T> inline T PSBinOpModulate(T arg1, T arg2) {
  return arg1 * arg2;
}
template <class T> inline T PSBinOpAdd(T arg1, T arg2) { return arg1 + arg2; }
template <class T> inline T PSBinOpSubtract(T arg1, T arg2) {
  return arg1 - arg2;
}

template <> inline CPlaneBasis PSBinOpModulate(CPlaneBasis p1, CPlaneBasis p2) {
  // we compute p1 * p2
...
template <> inline uint32 PSBinOpSubtract(uint32 lhs, uint32 rhs) {
...
// ***********************************************************************
template <> inline uint32 CPSAttribMakerBinOp<uint32>::getMinValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMinValue();
  case CPSBinOp::selectArg2:
    return _Arg[1]->getMinValue();
  case CPSBinOp::modulate:
    return _Arg[0]->getMinValue() * _Arg[1]->getMinValue();
  case CPSBinOp::add:
    return _Arg[0]->getMinValue() + _Arg[1]->getMinValue();
...
// ***********************************************************************
template <> inline uint32 CPSAttribMakerBinOp<uint32>::getMaxValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMaxValue();
  case CPSBinOp::selectArg2:
    return _Arg[1]->getMaxValue();
  case CPSBinOp::modulate:
    return _Arg[0]->getMaxValue() * _Arg[1]->getMaxValue();
  case CPSBinOp::add:
    return _Arg[0]->getMaxValue() + _Arg[1]->getMaxValue();
  case CPSBinOp::subtract: {
...
// ***********************************************************************
template <> inline sint32 CPSAttribMakerBinOp<sint32>::getMinValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMinValue();
  ...
  case CPSBinOp::add:
    return _Arg[0]->getMinValue() + _Arg[1]->getMinValue();
  case CPSBinOp::subtract:
    return _Arg[0]->getMinValue() - _Arg[1]->getMaxValue();
  default:
    nlassert(0);
    break;
  };
...
// ***********************************************************************
template <> inline sint32 CPSAttribMakerBinOp<sint32>::getMaxValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMaxValue();
  ...
  case CPSBinOp::add:
    return _Arg[0]->getMaxValue() + _Arg[1]->getMaxValue();
  case CPSBinOp::subtract:
    return _Arg[0]->getMaxValue() - _Arg[1]->getMinValue();
  default:
    nlassert(0);
    break;
  };
...
// ***********************************************************************
template <> inline float CPSAttribMakerBinOp<float>::getMinValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMinValue();
...
// ***********************************************************************
template <> inline float CPSAttribMakerBinOp<float>::getMaxValue(void) const {
  nlassert(_Arg[0] && _Arg[1]);
  switch (_Op) {
  case CPSBinOp::selectArg1:
    return _Arg[0]->getMaxValue();
...
inline CPSAttribMakerBinOp<T>::CPSAttribMakerBinOp(
    const CPSAttribMakerBinOp &other)
    : CPSAttribMaker<T>(other) // parent copy ctor
{
  CUniquePtr<CPSAttribMaker<T>> a0(
      NLMISC::safe_cast<CPSAttribMaker<T> *>(other._Arg[0]->clone())),
...
template <class T>
inline CPSAttribMakerBinOp<T>::CPSAttribMakerBinOp()
    : _Op(CPSBinOp::selectArg1), _Size(0), _MaxSize(0) {
  _Arg[0] = _Arg[1] = NULL;
  this->_HasMemory = true;
}

//=================================================================================================================
template <class T> inline void CPSAttribMakerBinOp<T>::clean(void) {
  delete _Arg[0];
  delete _Arg[1];
}

//=================================================================================================================
template <class T> inline CPSAttribMakerBinOp<T>::~CPSAttribMakerBinOp() {
  clean();
}
...
inline bool CPSAttribMakerBinOp<CPlaneBasis>::supportOp(CPSBinOp::BinOp op) {
  return (op == CPSBinOp::selectArg1 || op == CPSBinOp::selectArg2 ||
          op == CPSBinOp::modulate);
}
...
// for private use
template <class T>
inline void MakePrivate(uint8 *dest, const T *src1, const T *src2,
                        uint32 stride, uint32 numAttrib, CPSBinOp::BinOp op) {
...
//=================================================================================================================
template <class T>
inline void *CPSAttribMakerBinOp<T>::makePrivate(
    T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex, void *tab,
  ...
  while (leftToDo) {
    ...
    T *src1 = (T *)_Arg[0]->make(loc, startIndex + (numAttrib - leftToDo),
                                 &buf1[0], sizeof(T), toProcess, true, srcStep,
                                 forceClampEntry);
    T *src2 = (T *)_Arg[1]->make(loc, startIndex + (numAttrib - leftToDo),
                                 &buf2[0], sizeof(T), toProcess, true, srcStep,
                                 forceClampEntry);
...
//=================================================================================================================
template <class T>
inline void *CPSAttribMakerBinOp<T>::make(CPSLocated *loc, uint32 startIndex,
  ...
  return makePrivate((T *)&tab1[0], (T *)&tab2[0], loc, startIndex, tab, stride,
                     numAttrib, allowNoCopy, srcStep, forceClampEntry);
}
...
// for private use
template <class T>
inline void Make4Private(uint8 *dest, const T *src1, const T *src2,
                         uint32 stride, uint32 numAttrib, CPSBinOp::BinOp op) {
  const uint stride2 = stride << 1, stride3 = stride + stride2,
             stride4 = stride2 << 1;
...
//=================================================================================================================
template <class T>
inline void CPSAttribMakerBinOp<T>::make4Private(
    T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex, void *tab,
    uint32 stride, uint32 numAttrib, uint32 srcStep /* = (1 << 16)*/
) const {
  uint8 *dest = (uint8 *)tab;
  uint leftToDo = numAttrib, toProcess;
  nlassert(_Arg[0] && _Arg[1]);
...
//=================================================================================================================
template <class T>
inline void CPSAttribMakerBinOp<T>::make4(CPSLocated *loc, uint32 startIndex,
                                          void *tab, uint32 stride,
                                          uint32 numAttrib,
                                          uint32 srcStep /*= (1 << 16) */
) const

{
  ...
  uint8 tab1[PSBinOpBufSize * sizeof(T)];
  uint8 tab2[PSBinOpBufSize * sizeof(T)];
  make4Private((T *)&tab1[0], (T *)&tab2[0], loc, startIndex, tab, stride,
               numAttrib, srcStep);
}
...
// for private use
template <class T>
inline void MakeNPrivate(uint8 *dest, const T *src1, const T *src2,
                         uint32 stride, uint32 numAttrib, CPSBinOp::BinOp op,
                         uint nbReplicate, uint32 /* srcStep */ = (1 << 16)) {
  uint k;
...
//=================================================================================================================
template <class T>
inline void CPSAttribMakerBinOp<T>::makeNPrivate(
    T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex, void *tab,
...
//=================================================================================================================
template <class T>
inline void CPSAttribMakerBinOp<T>::makeN(CPSLocated *loc, uint32 startIndex,
                                          void *tab, uint32 stride,
                                          uint32 numAttrib, uint32 nbReplicate,
                                          uint32 /* srcStep = (1 << 16)*/
) const

{
  ...
  uint8 tab1[PSBinOpBufSize * sizeof(T)];
  uint8 tab2[PSBinOpBufSize * sizeof(T)];
  makeNPrivate((T *)&tab1[0], (T *)&tab2[0], loc, startIndex, tab, stride,
               numAttrib, nbReplicate);
}
...
inline void CPSAttribMakerBinOp<T>::deleteElement(uint32 index) {
  if (_Arg[0]->hasMemory())
    _Arg[0]->deleteElement(index);
  if (_Arg[1]->hasMemory())
    _Arg[1]->deleteElement(index);
  nlassert(_Size != 0);
  --_Size;
}
...
inline void CPSAttribMakerBinOp<T>::newElement(const CPSEmitterInfo &info) {
  if (_Arg[0]->hasMemory())
    _Arg[0]->newElement(info);
  if (_Arg[1]->hasMemory())
    _Arg[1]->newElement(info);
  if (_Size != _MaxSize) {
    ++_Size;
  }
}
...
//=================================================================================================================
template <class T>
inline void CPSAttribMakerBinOp<T>::resize(uint32 capacity,
                                           uint32 nbPresentElements) {
  nlassert(capacity < (1 << 16));
  _MaxSize = capacity;
  _Size = nbPresentElements;
  if (_Arg[0]->hasMemory())
    _Arg[0]->resize(capacity, nbPresentElements);
  if (_Arg[1]->hasMemory())
    _Arg[1]->resize(capacity, nbPresentElements);
}

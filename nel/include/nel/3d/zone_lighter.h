...
public:
  ...
  public:
    ...
    // Landscape ZBuffers size for all the landscape. There is one zbuffer like
    // this one per softshadow sample.
    uint ZBufferLandscapeSize;

    // Object ZBuffers size for all the landscape. This zbuffer is typically
    // finer. There is only one zbuffer like this.
    ...
    // Soft shadow jitter (0 ~ 1) to smooth softshadow aliasing when sampling
    // number is small
    ...
    /// Evaluation of the max height, in meters, of the vegetables. Needed when
    /// we compute whether a tile is below or above water.
  ...
  public:
    ...
    // Ctors
    CTriangle(const NLMISC::CTriangle &triangle, bool doubleSided,
              const NLMISC::CBitmap *texture, bool clampU, bool clampV,
              float *u, float *v, uint8 alphaTestThreshold) {
      // Copy the triangle
      Triangle = triangle;
...
#define MAX_CPU_PROCESS 10
  ...
  public:
    ...
    void build(std::vector<float> &heightField, const NLMISC::CVector &origine,
               float cellSeize, uint width, uint height);

    // Get max height on a direction
    float getMaxHeight(const NLMISC::CVector &position,
  ...
  // Light a zone
  void light(CLandscape &landscape, CZone &output, uint zoneToLight,
             const CLightDesc &description, std::vector<CTriangle> &obstacles,
             std::vector<uint> &listZone);

  // patch a zone, to compute only the tile water states.
  void computeTileFlagsOnly(CLandscape &landscape, CZone &output,
                            uint zoneToLight, const CLightDesc &description,
                            std::vector<uint> &listZone);

  // Add triangles from a landscape
  void addTriangles(CLandscape &landscape, std::vector<uint> &listZone,
                    uint order, std::vector<CTriangle> &triangleArray);

  // Add triangles from a transform shape. Work only for CMesh, CMultiMesh and
  // CMeshMRM all without skinning.
  void addTriangles(const IShape &shape, const NLMISC::CMatrix &modelMT,
  ...
  /// Add a water shape. This is needed to decide whether tiles are above /
  /// below water
  ...
  float attenuation(const CVector &pos,
                    const CZoneLighter::CLightDesc &description);

  /// \name Static PointLights mgt.
  //@{

  /// Append a static point light to compute. call at setup stage (before
  /// light() ).
...
private:
  ...
  void addTriangles(const CMeshBase &meshBase, const CMeshGeom &meshGeom,
                    const NLMISC::CMatrix &modelMT,
                    std::vector<CTriangle> &triangleArray);

  // Add triangles from a non skinned CMeshMRMGeom.
  void addTriangles(const CMeshBase &meshBase, const CMeshMRMGeom &meshGeom,
  ...
  void buildZoneInformation(CLandscape &landscape,
                            const std::vector<uint> &listZone,
                            const CLightDesc &lightDesc);

  // Exclude all the patch of a landscape from refine all
  void excludeAllPatchFromRefineAll(CLandscape &landscape,
                                    std::vector<uint> &listZone, bool exclude);

  // Get positions and normal from a patch
  void getPatchNormalAndPositions(std::vector<CLumelDescriptor> &lumels,
                                  CLandscape &landscape, uint zoneToLight,
                                  uint patch, CPatchUVLocator *locator,
                                  bool *binded);

  // Calc sky contribution. Used by getSkyContribution
  float calcSkyContribution(sint s, sint t, float height, float skyIntensity,
                            const CVector &normal) const;

  /// compute the sky contribution at the given position
  float getSkyContribution(const CVector &pos, const CVector &normal,
                           float SkyIntensity) const;

  // Get max height
  uint8 getMaxPhi(sint s, sint t, sint deltaS, sint deltaT,
                  float heightPos) const;

  // Eval a normal in the neighborhood
  void getNormal(const NL3D::CPatch *pPatch, sint16 lumelS, sint16 lumelT,
                 std::vector<NL3D::CPatchUVLocator> &locator,
                 const std::vector<NL3D::CPatch::CBindInfo> &bindInfo,
                 const std::vector<bool> &binded, std::set<uint64> &visited,
  ...
  bool isLumelOnEdgeMustBeOversample(
      uint patch, uint edge, sint s, sint t, const std::vector<bool> &binded,
      const std::vector<bool> &oversampleEdges,
      std::vector<CPatchUVLocator> &locator, uint8 shadowed,
  ...
  /// Process lighting for a set of lightable shapes. This is called by the
  /// threads created by lightShapes().
  void processLightableShapeCalc(uint process, TShapeVect *shapeToLit,
                                 uint firstShape, uint lastShape,
                                 const CLightDesc &description);

  /// Compute the lighting for a single lightable shape
  void lightSingleShape(CShapeInfo &lsi, const CLightDesc &description,
                        uint cpu);

  /// Compute the lighting for a water shape
  void lightWater(CWaterShape &ws, const CMatrix &MT,
                  const CLightDesc &description, uint cpu);

  /** Make a quad grid of all the water shapes that where registered by calling
   * addWaterShape() The vector of water shapes is released then \param bbox the
   * bbox of the zone containing the water shapes
   */
   ...
   */
  void computeTileFlagsForPositionTowardWater(
      const CLightDesc &lightDesc, std::vector<const CTessFace *> &tessFaces);

  /** If no water surface overlap the zone, so we set all the flags to
   * 'AboveWater", or don't change them if they were set to 'DisableVegetable'
   */
  void setTileFlagsToDefault(std::vector<const CTessFace *> &tessFaces);

  /** This copy the flags of the tiles from the source zone to a dest zone
   * (result of the lighting). This is needed beacuse these flags are updated to
   * say whether a given tile is above  / below water IMPORTANT : the source and
   * destination zones must match of course...
   */
  static void copyTileFlags(CZone &destZone, const CZone &srcZone);

  // Get texture from a material for alpha test
  bool getTexture(const CMaterial &material, NLMISC::CBitmap *&result,
                  bool &clampU, bool &clampV, uint8 &alphaTestThreshold,
                  bool &doubleSided);
...
private:
  ...
  struct CPointLightRT {
    ...
    // Faces that may occlude the light. Only Back Faces (from the light pov)
    // are inserted
    ...
    /** Tells if a point is visible from this light. NB: test first if in
     *BSphere If occluded or out of radius, return false, else return true. Also
     *Skip if the light is an Ambient, and skip if the light is a spot and if
     *the position is out of the cone
     */
  ...
  void compilePointLightRT(uint gridSize, float gridCellSize,
                           std::vector<CTriangle> &obstacles, bool doShadow);
  ...
  /** List of all the water shapes in the zone. We need them to check whether
   * the tiles are above / below water, or if theyr intersect water
   */

...
public:
  ...
  public:
    ...
    /// If true, then the shape is even not loaded/displayed (used for shadow
    /// casting?)
    bool DontAddToScene;

    /// If false, the shape is load but hid by default (NB: can be used for
    /// camera collision)
    bool Visible;

    /// Precomputed Lighting.
    // If true (false by default), then the instance don't cast shadow (used by
    // ig_lighter).
    bool DontCastShadow;
    // If true (false by default), then the instance's lighting will not be
    // precomputed.
    bool AvoidStaticLightPreCompute;
    // This part is precomputed.
    bool StaticLightEnabled; // If false, then the instance 's lighting is not
                             // pre-computed.
    uint8 SunContribution;   // Contribution of the Sun.
    // Ids of the lights. FF == disabled. if Lights[i]==FF, then all lights >=i
    // are considered disabled.
    uint8 Light[NumStaticLightPerInstance];
    /** Id of the ambiant Light to take for this instance. Ambient light are
     *stored too in ig->getPointLigths() If 0xFF => take Ambient of the sun.
     */
    uint8 LocalAmbientId;
    /** if true (false by default), the instance don't cast shadow, but ONLY FOR
     *ig_lighter (ig_lighter_lib) (zone_lighter and zone_ig_lighter ignore it).
     *	This is a special trick for the "Matis Serre" where the exterior mesh
     *cast shadow in the interior, but is not visible in the interior in
     *realTime because of cluster clipping.... omg :(
     */
    bool DontCastShadowForInterior;
    /** This is the opposite of DontCastShadowForInterior. zone_lighter or
     *zone_ig_lighter test it and if true, assume this instance dont cast shadow
     */
  ...
  /** Get the instance added to the scene. NULL if instanceNb too big, if
   *addToScene not called, or if instance is DontAddToScene
   */
  ...
  /** Retreive group information. NB: data may have changed, eg: order of
   * lights.
   */
  void retrieve(CVector &vGlobalPos, TInstanceArray &array,
                std::vector<CCluster> &Clusters, std::vector<CPortal> &Portals,
  ...
  /** For Debug Display of clusters. The view matrix and frustum should have
   *been setuped NB: the ModelMatrix is modified by this method
   */
...
public:
  ...
  /// Set the cluster system to test for instances that are not in a cluster of
  /// this IG
  ...
  /// link the root of the ig to a node. No-op if not added to scene. Pass NULL
  /// to reset by default
  ...
  /// see CTransform::freezeHRC(). Do it for all instances (not clusters), and
  /// for the root of the IG.
  void freezeHRC();

  /// see CTransform::unfreezeHRC(). Do it for all instances (not clusters), and
  /// for the root of the IG.
  void unfreezeHRC();
...
public:
  ...
  const std::vector<CPointLightNamed> &getPointLightList() const {
    return _PointLightArray.getPointLights();
  }

  /// Get the number of point lights
  uint getNumPointLights() const {
    return (uint)_PointLightArray.getPointLights().size();
  }
  ...
  bool getStaticLightSetup(NLMISC::CRGBA sunAmbient, uint retrieverIdentifier,
    ...
    return _IGSurfaceLight.getStaticLightSetup(
        sunAmbient, retrieverIdentifier, surfaceId, localPos, pointLightList,
   ...
   */
  CInstanceGroup *getParentClusterSystem() const {
    return _ParentClusterSystem;
  }
...
public:
  ...
  CClipTrav *_ClipTrav;
  // The cluster system used to link unclustered instances
...
private:
  ...
  /// Build the list of light. NB: sort by LightGroupName the array, and return
  /// index remap.
  ...
  // Yes this is ugly, but impossible otherwise with the actual user interface
  // system...

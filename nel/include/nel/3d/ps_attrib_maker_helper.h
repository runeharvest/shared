...
/** This template generate an attrib maker by defining the methods of the
 * CPSCAttribMaker class. You can derive your own class but it is a shortcut to
 * do the job \param T : the type to produce \param F : a class that override
 * the () operator, the input is chosen by the user (age, speed ...) , and the
 * output is the same type as T. Inline is preferable, as it will be called a
 * lot It can stores info that indicate how to build it
 */

template <typename T, class F>
class CPSAttribMakerT : public CPSAttribMaker<T> {
public:
  /// the functor object
  F _F;
  ...
  /** Fill tab with an attribute by using the given stride. It fills numAttrib
   * attributes, and use it to get the The particle life as an input
   */
  ...
  /** The same as make, but it replicate each attribute 4 times, thus filling
   * 4*numAttrib. Useful for facelookat and the like \see make()
   */
  ...
  CPSAttribMakerT(float nbCycles)
      : CPSAttribMaker<T>(nbCycles), _Clamp(false) {}

  /// dtor
  virtual ~CPSAttribMakerT() {}

  /** tells whether one may choose one attribute from a CPSLocated to use as an
   * input. If false, the input(s) is fixed For this class, it is supported
   */
  ...
  /** tells whether clamping is supported for the input (value can't go above
   * MaxInputValue)
   */
...
private:
  ...
  /** generate an attribute by using the given iterator. this allow to choose
   * the input of tha attribute maker \param canOverlapOne must be true if the
   * entry iterator can give values above 1 the attribute maker with no speed
   * penalty
   */

  template <typename It>
  void makeByIterator(It it, void *tab, uint32 stride, uint32 numAttrib,
                      bool canOverlapOne, bool forceClampEntry) const {
    uint8 *pt = (uint8 *)tab;
  ...
  /** The same as make, but it replicate each attribute 4 times, thus filling
   * 4*numAttrib. Useful for facelookat and the like \param canOverlapOne must
   * be true if the entry iterator can give values above 1 \see makeByIterator()
   */
  template <typename It>
  void make4ByIterator(It it, void *tab, uint32 stride, uint32 numAttrib,
                       bool canOverlapOne) const {

    uint8 *pt = (uint8 *)tab;

    // first precompute the various strides (stride * 2, 3 and 4)
    // const uint32 stride2 = stride << 1, stride3 = stride + stride2, stride4 =
    // stride2 << 1;
      ...
      if (!_Clamp) {
        if (this->_NbCycles == 1) {
          while (numAttrib--) {
            ...
            //*(T *)pt = *(T *)(pt + stride) = *(T *)(pt + stride2)  = *(T *)(pt
            //+ stride3) = _F(OptFastFractionnalPart(it.get()));
            *(T *)pt = _F(NLMISC::OptFastFractionnalPart(it.get()));
            *(T *)(pt + stride) = *(T *)pt;
            pt += stride;
            *(T *)(pt + stride) = *(T *)pt;
          ...
          while (numAttrib--) {
            ...
            // fill 4 attrib with the same value at once
            //*(T *)pt = *(T *)(pt + stride) = *(T *)(pt + stride2)  = *(T *)(pt
            //+ stride3) = _F(OptFastFractionnalPart(time));
            *(T *)pt = _F(NLMISC::OptFastFractionnalPart(time));
            *(T *)(pt + stride) = *(T *)pt;
            pt += stride;
            *(T *)(pt + stride) = *(T *)pt;
  ...
  /** The same as make4, but with n replication instead of 4
   *  \param canOverlapOne must be true if the entry iterator can give values
   * above 1 \see make4ByIterator
   */
  template <typename It>
  void makeNByIterator(It it, void *tab, uint32 stride, uint32 numAttrib,
  ...
  /* template <typename T, class F> */
  void /*CPSAttribMakerT<T, F>::*/ *
  make(CPSLocated *loc, uint32 startIndex, void *tab, uint32 stride,
       uint32 numAttrib, bool /* allowNoCopy */ /* = false */,
       uint32 srcStep /*= (1 << 16)*/, bool forceClampEntry /*= false*/
  ) const {
    ...
    if (srcStep == (1 << 16)) {
      ...
      case CPSInputType::attrDate: {
        CPSBaseIterator<TIteratorFloatStep1> it(
            TIteratorFloatStep1(loc->getTime().begin(), startIndex));
        makeByIterator(it, tab, stride, numAttrib, loc->getLastForever(),
                       forceClampEntry);
      } break;
      case CPSInputType::attrInverseMass: {
        CPSBaseIterator<TIteratorFloatStep1> it(
            TIteratorFloatStep1(loc->getInvMass().begin(), startIndex));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;
      case CPSInputType::attrSpeed: {
        CVectNormIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getSpeed().begin(), startIndex));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;

      case CPSInputType::attrPosition: {
        CVectNormIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getPos().begin(), startIndex));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;
      ...
      case CPSInputType::attrLOD: {
        ...
        CFDot3AddIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getPos().begin(), startIndex));
      ...
      case CPSInputType::attrSquareLOD: {
        ...
        CFSquareDot3AddIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getPos().begin(), startIndex));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
      case CPSInputType::attrClampedLOD: {
        ...
        CFClampDot3AddIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getPos().begin(), startIndex));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
      case CPSInputType::attrClampedSquareLOD: {
        ...
        CFClampSquareDot3AddIterator<TIteratorVectStep1> it(
            TIteratorVectStep1(loc->getPos().begin(), startIndex));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
    ...
    } else // fixed point steps
    {
      switch (_InputType.InputType) {
      case CPSInputType::attrDate: {
        CPSBaseIterator<TIteratorFloatStep1616> it(TIteratorFloatStep1616(
            loc->getTime().begin(), startIndex, srcStep));
        makeByIterator(it, tab, stride, numAttrib, loc->getLastForever(),
                       forceClampEntry);
      } break;
      case CPSInputType::attrInverseMass: {
        CPSBaseIterator<TIteratorFloatStep1616> it(TIteratorFloatStep1616(
            loc->getInvMass().begin(), startIndex, srcStep));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;
      case CPSInputType::attrSpeed: {
        CVectNormIterator<TIteratorVectStep1616> it(TIteratorVectStep1616(
            loc->getSpeed().begin(), startIndex, srcStep));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;

      case CPSInputType::attrPosition: {
        CVectNormIterator<TIteratorVectStep1616> it(
            TIteratorVectStep1616(loc->getPos().begin(), startIndex, srcStep));
        makeByIterator(it, tab, stride, numAttrib, true, forceClampEntry);
      } break;
      case CPSInputType::attrUniformRandom: {
        CRandomIterator it;
      ...
      case CPSInputType::attrLOD: {
        ...
        CFDot3AddIterator<TIteratorVectStep1616> it(
            TIteratorVectStep1616(loc->getPos().begin(), startIndex, srcStep));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
      case CPSInputType::attrSquareLOD: {
        ...
        CFSquareDot3AddIterator<TIteratorVectStep1616> it(
            TIteratorVectStep1616(loc->getPos().begin(), startIndex, srcStep));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
      case CPSInputType::attrClampedLOD: {
        ...
        CFClampDot3AddIterator<TIteratorVectStep1616> it(
            TIteratorVectStep1616(loc->getPos().begin(), startIndex, srcStep));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
      case CPSInputType::attrClampedSquareLOD: {
        ...
        CFClampSquareDot3AddIterator<TIteratorVectStep1616> it(
            TIteratorVectStep1616(loc->getPos().begin(), startIndex, srcStep));
        loc->getLODVect(it.V, it.Offset, loc->getMatrixMode());
        makeByIterator(it, tab, stride, numAttrib, false, forceClampEntry);
      } break;
  ...
  /* template <typename T, class F> */
  void /*CPSAttribMakerT<T, F>::*/ make4(CPSLocated *loc, uint32 startIndex,
                                         void *tab, uint32 stride,
                                         uint32 numAttrib,
                                         uint32 srcStep /*= (1 << 16)*/
  ) const {
  ...
  /* template <typename T, class F> */
  virtual void /*CPSAttribMakerT<T, F>::*/
  makeN(CPSLocated *loc, uint32 startIndex, void *tab, uint32 stride,
        uint32 numAttrib, uint32 nbReplicate, uint32 srcStep /* = (1 << 16)*/
  ) const {
    NLMISC::OptFastFloorBegin();
    nlassert(loc);
    if (srcStep == (1 << 16)) {
      ...
      case CPSInputType::attrDate: {
        CPSBaseIterator<TIteratorFloatStep1> it(
            TIteratorFloatStep1(loc->getTime().begin(), startIndex));
        makeNByIterator(it, tab, stride, numAttrib, nbReplicate,
                        loc->getLastForever());
      } break;
      case CPSInputType::attrInverseMass: {
        CPSBaseIterator<TIteratorFloatStep1> it(
  ...
  // Compute a single value from the input assuming that
  // NLMISC::OptFastFloorBegin() has been called
  virtual T getInternal(float input) {
    ...
    if (_Clamp) {
      ...
      return (input == MaxInputValue)
                 ? _F(MaxInputValue)
                 : _F(NLMISC::OptFastFractionnalPart(input));
    }
  }
};
...
T CPSAttribMakerT<T, F>::get(CPSLocated *loc, uint32 index) {
  ...
  case CPSInputType::attrLOD: {
    ...
    if (_Clamp) {
      ...
      result = (r == MaxInputValue) ? _F(MaxInputValue)
                                    : _F(NLMISC::OptFastFractionnalPart(r));
  } break;
  case CPSInputType::attrSquareLOD: {
    static NLMISC::CVector lodVect;
    float lodOffset;
...
/**  This class is an attribute maker that has memory, all what is does is to
 * duplicate its mem when 'make' is called It own an attribute maker that tells
 * how to produce the attribute from its emiter date, speed and so on ...
 */
template <typename T>
class CPSAttribMakerMemoryBase : public CPSAttribMaker<T> {
public:
   ...
   * for example, when you set this scheme to a LocatedBindable that does have a
   * least one instance in it
   *
   *  example :
   *      CPSDot *d = new CPSDot;
   *      CPSAttribMakerMemory<RGBA> *genAttribMaker = new
   * CPSAttribMakerMemory<RGBA>;
   *      genAttribMaker->setScheme(CPSColorBlender(CRGBA::White, CRGBA::Black)
   *      Now, if an emitter emit these particle, it'll start to emit white
   * ones, and then black ones d->setColorScheme(  genAttribMaker); now, suppose
   * that there were several dot instanciated before the setScheme is performed
   * : d->newElement(); no color has been memorized for this element, so when
   * setScheme is performed, it has to generate one There are no emitter that
   * provides it, so its taken from the default value Note : this should only be
   * useful in an editor, that allow the user to change the scheme with a
   * running system ...
   *
   */
  ...
  /** set the scheme used to store attribute. this MUST be called, otherwise an
   * assertion will be thrown later It must have been allocated by new, and it
  ...
  CPSAttribMakerMemoryBase(const CPSAttribMakerMemoryBase &src)
      : CPSAttribMaker<T>(src) // parent copy ctor
  {
    nlassert(src._Scheme);
    CUniquePtr<CPSAttribMaker<T>> s(
        NLMISC::safe_cast<CPSAttribMaker<T> *>(src._Scheme->clone()));
    this->_T = src._T;
  ...
  /// inherited from CPSAttribMaker
  virtual void *make(CPSLocated * /* loc */, uint32 startIndex, void *output,
                     uint32 stride, uint32 numAttrib, bool allowNoCopy = false,
                     uint32 srcStep = (1 << 16),
    ...
    if (!allowNoCopy || srcStep != (1 << 16) || sizeof(T) != stride) {
      ...
      if (srcStep == (1 << 16)) {
        typename CPSAttrib<T>::const_iterator it = _T.begin() + startIndex,
                                              endIt = _T.begin() + startIndex +
                                                      numAttrib;
        do {
          *(T *)tab = *it;
          ++it;
  ...
  /// inherited from CPSAttribMaker
  virtual void make4(CPSLocated * /* loc */, uint32 startIndex, void *tab,
                     uint32 stride, uint32 numAttrib,
  ...
  /// inherited from CPSAttribMaker
  virtual void makeN(CPSLocated * /* loc */, uint32 startIndex, void *tab,
                     uint32 stride, uint32 numAttrib, uint32 nbReplicate,
                     uint32 srcStep = (1 << 16)) const {
    // we just copy what we have memorized
    uint k;
    typename CPSAttrib<T>::const_iterator it = _T.begin() + startIndex,
  ...
  /// serialisation of the object. Derivers MUST call this, (if they use the
  /// attribute of this class at least)
  virtual void serial(NLMISC::IStream &f) {
  ...
  virtual void newElement(const CPSEmitterInfo &info) {
    ...
    if (info.Loc) {
      ...
      /** well a value may be returned without having to know the emitter
       * (random, user param ...) but this case is really useless anyway ...
       */
...
protected:
  ...
  /** this attribute maker tells us how to produce arguments from an emitter. as
   * an example, we may want to have a gradient of color : the emitter emit
   * green then blue particles, following a gradient. the color is produced by
   * _Scheme and _T stores it
   */
  CPSAttribMaker<T> *_Scheme;
};

/** Standard version for attrib maker memory : don't redefine the getMinValue &
 * getMaxValue methods -> meaningful for ordered sets only
 */
template <typename T>
class CPSAttribMakerMemory : public CPSAttribMakerMemoryBase<T> {
public:
  // default ctor
  CPSAttribMakerMemory() : CPSAttribMakerMemoryBase<T>() {}
  CPSAttribMakerMemory(const CPSAttribMakerMemory &other)
      : CPSAttribMakerMemoryBase<T>(other) {}
};

/** specializations for integral types : they have method getMin & getMax
 * We update the min & max value each time a new element is inserted so it is
 * just a minoration or a majoration of the real value. But as told in
 * CPSAttribMaker, we just need an approximation
 */
...
public:
  ...
  CPSAttribMakerMemory(const CPSAttribMakerMemory<uint32> &other)
      : CPSAttribMakerMemoryBase<uint32>(other) {
...
public:
  ...
  // copy ctor
  CPSAttribMakerMemory(const CPSAttribMakerMemory<sint32> &other)
      : CPSAttribMakerMemoryBase<sint32>(other) {
    _MinValue = other._MinValue;
    _MaxValue = other._MaxValue;
  }
  // serial. Should update min / max when reading
...
public:
  ...
  CPSAttribMakerMemory(const CPSAttribMakerMemory<float> &other)
      : CPSAttribMakerMemoryBase<float>(other) {
    _MinValue = other._MinValue;
    _MaxValue = other._MaxValue;
  }
  // serial. Should update min / max when reading

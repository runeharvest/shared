 ...
 * You can get this interface by using a dynamic_cast on a UInstance that is a
 * water instance \author Nicolas Vizerie \author Nevrax France \date 2001
 */
class UWaterInstance : public UInstance {
public:
  /** Get the ID of the water height map attached with this surface of water.
   * Once you got it, you can get an interface on it from the water height map
   * manager. NB : a water height map is usually shared between several wtare
   * instances
   */
  uint32 getWaterHeightMapID() const;

  /** When displaying the water height field, the value in the height field is
   * actually multiplied by a factor to get the height in world space. This
   * returns this factor
   */
  ...
  /** Get the attenuated height of the water in world space at the given
   * location. You must provide the viewer position This is useful if you want
   * to compute the position of a floating object
   */
  float getAttenuatedHeight(const NLMISC::CVector2f &pos,
  ...
  UWaterInstance(class CWaterModel *object) {
    _Object = (ITransformable *)object;
  };
  /// Attach an object to this proxy
  void attach(class CWaterModel *object) {
    _Object = (ITransformable *)object;
  };
  /// Detach the object
...
public:
  /// Retrieve a water heightmap from its ID. An assertion is raised if a wrong
  /// ID is provided
  static UWaterHeightMap &getWaterHeightMapFromID(uint32 ID);

  /** Blend factor for water surfaces that use CTextureBlend (day / night mgt
   * for example) The blend factor if clamped to [0, 1] NB : when transition has
   * finished, one should call releaseBlendTexture to eventually release
...
public:
   ...
   * \param strenght Strenght of the impulsion
   */
  virtual void perturbatePoint(const NLMISC::CVector2f &pos,
                               float strenght) = 0;

  /** Get the value of the height map at the given location. To get the height
   * in world coordinate, You should use getHeight
   */

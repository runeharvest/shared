...
// ***************************************************************************
/**
 * An material Reference for Animated reference. This object is stored in the
 * mesh, and is serialised. NB: formated for 3ds Max :). Emissive anim is a
 * float, with a constant RGB factor. \author Cyril 'Hulud' Corvazier \author
...
public:
   ...
   *
   * NB: for emissive part, emissive defaut track value is set to 1, and
   *emissive factor is set to the RGB emissive value stored into pMat.
   */
  ...
  /** Animated materials support Texture animation. This is the place where you
   * define your list of texture. This list of animated texture is serialised.
   * AnimatedMaterial animate texture with sint32 Tracks. If the id is not found
   * in CMaterialBase, then the CMaterial texture is left as before.
   *
   * NB: id 0x7FFFFFFF is a reserved id, used as default to indicate no valid
   * Animated texture.
   */
  // @{
  /// assign a specific texture for an id (a uint32). It is valid to give a NULL
  /// ptr (=> untextured). Sorted as a SmartPtr.
  void setAnimatedTexture(uint32 id, CSmartPtr<ITexture> pText);
  /// is this Id valid?
  bool validAnimatedTexture(uint32 id);
  /// return the texture for this Id. return NULL either if NULL texture for
  /// this id or if(!validAnimatedTexture()).
 ...
 * NB: formated for 3ds Max :). Emissive anim is a float, with a constant RGB
 * factor. Texture animation: see update(). \author Cyril 'Hulud' Corvazier
...
public:
  ...
  /** Check if the animated material is touched, and if necessary update the
   * stored material (if any). Texture animation: for now, texture animation is
   * possible only on stage 0. If TextureValue flag is touched (ie a texture
   * anim track is linked to the animated value), update() look into the
   * CMaterialBase what texture to set. If the track gives a bad Id for the
   * texture, no-op.
   */
  void update();
  ...
  /// number of animated values for each animated texture, taken from
  /// CMaterialBase
  enum {
    NumTexAnimatedValues = CMaterialBase::CTexAnimTracks::NumTexAnimatedValues
  };
  ...
  enum TAnimValues {
    ...
    AnimValueLast =
        TextureMatValues +
        NumTexAnimatedValues * IDRV_MAT_MAXTEXTURES /* texture matrix anim */
  };

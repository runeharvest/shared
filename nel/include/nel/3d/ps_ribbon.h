 ...
 * It replaces the old ribbon version. It has more limitations : no 2d rotations
 * and it has no propagation of size and color. It doesn't take as much memory,
 * and displays better  (the length is not dependent on framerate as it was the
 * case before)
 */
...
public:
   ...
   *  \param shape pointer to a shape that will be extruded along the ribbon. It
   * must have a unit size and be located in the x-y plane. This will be copied
   ...
   */
  virtual bool getColorFading(void) const { return _ColorFading; }
   ...
   *  \param points A list of points that define the shape of the slice of the
   * ribbon These points must be located in the x-y plane (z can be used for
   * effects). \param nbPointsInShape The number of points in the shape \param
   * braceMode In this mode, the ribbon mesh is a brace. The number of vertices
   * must be even. Each pair of vertices defines a segment whose V tex coord
   * goes from 0 to 1 When set to false, the list of vertices define a closed
   * loop shape
   */
   ...
   *  \param dest a table of cvector that has the right size, it will be filled
   * with vertices \see getNbVerticesInShape()
   */
...
private:
   ...
   * The number of slice is encoded in the upper word of the vb index (the int
   * used to lookup in the map) The number of vertices per slices is encoded in
   * the lower word
   */
  class CVBnPB {
  public:
  ...
  /// Get the number of vertices in each slices (depends on whether the ribbon
  /// is textured or not)

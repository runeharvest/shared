...
public:
  ...
  /** Load all Zones around a position. This is a blocking call.
   *	\zonesAdded array of name of the zones added, without extension (eg:
   *"150_EM").
   */
  virtual void loadAllZonesAround(const CVector &pos, float radius,
                                  std::vector<std::string> &zonesAdded) = 0;
  /// Delete old zones, or load new zones, around a position. new Zones are
  /// loaded async.
  virtual void refreshZonesAround(const CVector &pos, float radius) = 0;
  /** Delete old zones, or load new zones, around a position. new Zones are
   *loaded async. This method add or remove only one zone at a time.
   *	\zoneRemoved name of the zone removed, without extension (eg: "150_EM").
   *"" if none. \zoneAdded name of the zone added, without extension (eg:
   *"150_EM"). "" if none. \validZones subset of zones that can be loaded (NULL
   *for default set)
   */
  virtual void
  refreshZonesAround(const CVector &pos, float radius, std::string &zoneAdded,
                     std::string &zoneRemoved,
                     const std::vector<uint16> *validZoneIds = NULL) = 0;
  /// Delete old zones, or load new zones, around a position, until it is
  /// finished. This is a blocking call.
  virtual void
  refreshAllZonesAround(const CVector &pos, float radius,
   ...
   *	\zonesLoaded array of name of the zones added, without extension (eg:
   *"150_EM").
   */
   ...
   *  NB: This setup will be visible only for new texture far/near computed
   * (when player move or see dynamic lighting).
   *
   ...
   */
  virtual void setupStaticLight(const CRGBA &diffuse, const CRGBA &ambiant,
                                float multiply) = 0;

  /**	 Setup the equivalent material diffuse component used for both Static
   *and Dynamic PointLights. Default is White.
   */
   ...
   *WARNING!! If Scene Lighting is enabled (usual case), vegetable lighting
   *setup take last Sun setup at UScene::render(). Hence, you should force the
   *new lighting setup with ULandscape::setupVegetableLighting(), before calling
   *updateLightingAll().
   */
  ...
  /** if true, the refine Center is auto computed each frame from Camera
   *Position. Else must be given by setRefineCenterUser() Default to true.
   */
  ...
  /** enable the vegetable management in landscape. Valid only if Hardware
   * support VertexShader.
   */
   ...
   */
  virtual void setupVegetableLighting(const CRGBA &ambient,
                                      const CRGBA &diffuse,
                                      const CVector &directionalLight) = 0;
   ...
   */
  virtual void setVegetableWind(const CVector &windDir, float windFreq,
                                float windPower, float windBendMin) = 0;
   ...
   */
  virtual float getRayCollision(const NLMISC::CVector &start,
                                const NLMISC::CVector &end) = 0;
  // @}
};

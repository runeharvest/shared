...
public:
   ...
   * envmap used in this case. \param UpdateTime time in seconds needed to
   * update all 6 faces of the cube. 0 means the update is done at each frame
   */
  void init(uint cubeMapSize, uint projection2DSize,
            TGlobalAnimationTime updateTime, IDriver &driver);
  // Update content
  void update(TGlobalAnimationTime time, IDriver &driver);
  // Called when one face of the envmap must be computed. Should be defined by
  // derivers
...
private:
  ...
  static CVertexBuffer
      _FlattenVB; // vb to map cube map top hemisphere to a 2D map

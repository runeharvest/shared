...
public:
   ...
   */
  struct CBindInfo {
    /** The number of patchs on this edge. 0,1, 2 or 4.  0 means no neigbor on
     * this edge. \b 5 is a special code, which means the same thing than
     * NPatchs==1, but "I am one of the little patch connected to the bigger
     * neigbor". Like when NPatchs==1, ZoneId, Next[0] and Edge[0] must be
     * valid.
     */
    uint8 NPatchs;

    /// The neighbor zone of all neigbor patch. Often the same zone as the patch
    /// (but on zone border).
    uint16 ZoneId;
...
public:
  ...
  uint8 OrderS, OrderT;
  /// The Base Size*bumpiness of the patch (/2 at each subdivide). Set to 0, if
  /// you want CZone to compute it for you.
  float ErrorSize;
  /// The base corner vertices indices in the current zone. Used for patch
  /// connectivity.
  uint16 BaseVertices[4];

  /** "The don't smooth" flags. For each edge of the patch (0~3), the flag means
  that this patch mustn't be smoothed with its neightbor. The n-th edge links
  the vertices "n" and "(n+1)%4". The flag for the n-th edge is (1<<n). */
  uint8 Flags;
  ...
  /// setup NoiseSmooth flags: used for Noise geometry and lighting. NB:
  /// convention: corner0==A, corner1==B ...
  ...
  /** The Tile colors for this patch. There must be (OrderS+1)*(OrderT+1) tile
   * colors. Those are the colors at the corners of the tiles. They are stored
   * in line first order, from S=0 to 1, and T=0 to 1.
   */
  std::vector<CTileColor> TileColors;

  /** The Tile lumels for this patch. There must be (OrderS*4+1)*(OrderT*4+1)
   * tile lumels. Those are lumel value in tiles. There is 4x4 lumels by tiles
   * plus last lumels. They are stored in line first order, from S=0 to 1, and
   * T=0 to 1.
   */
  std::vector<uint8> Lumels;

  /** There is (OrderS/2+1) * (OrderT/2+1) tiles light influence.
   *	It indicates which static pointLight influence each corner of a
   *TessBlock (block of 2*2 tiles).
   *
   *	If empty(), suppose no light influence. but CZone::retrieve() always
   *return a size() == (OrderS/2+1) * (OrderT/2+1).
   *
   * They are stored in line first order, from S=0 to 1, and T=0 to 1.
   */
   ...
   * \return false if no neighbor has been found or tile ratio is not the same
   * than in this patch.
   */
  bool getNeighborTile(uint patchId, uint edge, sint position, uint &patchOut,
                       sint &sOut, sint &tOut,
                       const std::vector<CPatchInfo> &patchInfos) const;

  /** Adjusts a CPatchInfo array to get a symmetrized / rotated zone with
   * matching oriented tiles. This method only adjuste tile and vertex color
   * array, does'nt transform vertices.
   *
   * Transform an array of patchInfo by a symmetry on OY axis followed by a
   * 90deg CCW rotation (0, 1, 2, 3).
   *
   * The method doesn't transform vertices.
   * If symmetry, the method invert 0-3 and 1-2 vertices indexes to get CCW
   * oriented patches. It will fix bind information. The method fixes tile and
   * color vertex arrays. The method fixes tile rotation, 256 cases and tile
   * transistions.
   *
   * Return false if something wrong.
   */
  static bool transform(std::vector<CPatchInfo> &patchInfo,
                        NL3D::CZoneSymmetrisation &zoneSymmetry,
                        const NL3D::CTileBank &bank, bool symmetry, uint rotate,
                        float snapCell, float weldThreshold,
                        const NLMISC::CMatrix &toOriginalSpace);

  // Transform tile and 256 case with rotation and symmetry parameters
  static bool getTileSymmetryRotate(const NL3D::CTileBank &bank, uint tile,
                                    bool &symmetry, uint &rotate);
  static bool transformTile(const NL3D::CTileBank &bank, uint &tile,
                            uint &tileRotation, bool symmetry, uint rotate,
                            bool goofy);
  static void transform256Case(const NL3D::CTileBank &bank, uint8 &case256,
                               uint tileRotation, bool symmetry, uint rotate,
                               bool goofy);
...
struct CZoneInfo {
  ...
  /** borderVertices vertices connectivity for this zone. NB: borderVertices
   *must contains the connectivity across zones. It is VERY IMPORTANT to setup
   *zone corner connectivity too. A "corner borderVertex" may appear 3 times
   *here. One for each other zone of the corner.
   */
  std::vector<CBorderVertex> BorderVertices;

  /** List of PointLights that may influences Patchs and objects walking on
   *them. Must be of good size regarding to Patchs[i].TileLightInfluences data
   */
 ...
 *	- use build(). (then you can use serial to save the zone, don't need to
 *compile() the zone).
 *	- use serial() for loading this zone.
 *
 * Before a zone may be rendered, it must be compile()-ed, to compile and bind
 *patch, to make vertices etc...
 *
 * NB: you must call release() before deleting a compiled zone. (else assert in
 *destruction). \author Lionel Berenguier \author Nevrax France \date 2000
 */
class CZone {
public:
...
public:
   ...
   *	- compress Lumels.
   *	- sort PointLights by name, and hence remap TileLightInfluences
   *coordinates.
   *
   * NB: cannot build on a compiled zone. must release the zone before....
   *
   * \param numVertices maximize the numgber of vertices used by this zone with
   *this value.
   */
   ...
   */
  void build(uint16 zoneId, const std::vector<CPatchInfo> &patchs,
   ...
   * This method uncompress the patchs coordinates and all info into the patch
   * info/borderVertices. Warning!!! Due to compression, data won't be the same
   * as those given in build(). Same remark for TileLightInfluences, due to
   * light sorting.
   *
   */
   ...
   */
  void retrieve(std::vector<CPatchInfo> &patchs,
                std::vector<CBorderVertex> &borderVertices);
   ...
   *	- create/link the base vertices (internal..), according to present
   *neigbor zones.
   *	- compile() the patchs.
   *	- bind() the patchs.
   ...
   *	- destroy/unlink the base vertices (internal..), according to present
   *neigbor zones.
   *	- unbind() the patchs.
   *	- release() the patchs.
   *	- rebindBorder() on neighbor zones.
  ...
  /** Load/save a zone.
   * Save work even if zone is not compiled, but load must be done on a not
   * compiled zone...
   */
  void serial(NLMISC::IStream &f);
   ...
   * \param numPatch the index of patch in this zone which will receive his new
   * texture. assert if bad id. \param tiles the patch texture. assert if not of
   * good size (OrderS*OrderT). Can be NULL if you don't want to change the
   * patch texture. \param colors the patch texture. assert if not of good size
   * ((OrderS+1)*(OrderT+1)). Can be NULL if you don't want to change the patch
   * colors.
   */
  void changePatchTextureAndColor(sint numPatch,
                                  const std::vector<CTileElement> *tiles,
                                  const std::vector<CTileColor> *colors);

  /**
   * refresh the geometry (re-compute vertices).
   * Useful for Tile Noise edition. Do it after calling
   *changePatchTextureAndColor(). NB: a refreshTesselationGeometry() should be
   *done on All patchs, and all direct neighbors of this patch (including patchs
   *on corners). WARNING: specially coded for Tile edition. Result is not
   *perfect:
   *		- only EndPos of tesselation is modified. because Tile edition
   *always subdivide at max.
   *		- Pos=EndPos. because TileEdition always subdivide at max, and
   *don't refine...
   *		- TessBlocks BSphere may be too big. Clip is worse (too big),
   *but doesn't matter. \param numPatch the index of patch in this zone. assert
   *if bad id.
   */
   ...
   * \param numPatch the index of patch in this zone which will get his texture.
   * assert if bad id. \param \return The tiles the patch texture. The size
   * should be OrderS*OrderT. \see getPatch()
   */
   ...
   * \param numPatch the index of patch in this zone which will get his colors.
   * assert if bad id. \param \return The tiles the patch colors. The size
   * should be (OrderS+1)*(OrderT+1). \see getPatch()
   */
   ...
   * Set all tile colors of all patch of this zone to monochrome or not and
   * apply a multiplier factor convertion to monochrome is done by that formula
   * : newR = newG = newB = 0.28 * R + 0.59 * G + 0.13 * B; for monochrome the
   * factor is applied like for the color mode
   */
  void setTileColor(bool monochrome, float factor);

  /**
   *  Get the landscape in which is placed this zone. If no landscape, return
   *NULL.
   *
   *	\return the pointer of the landscape of the zone or NULL if the zone
   *hasn't be compiled.
   */
  CLandscape *getLandscape() const { return Landscape; }

  // NB: for all those function, CTessFace static rendering context must be
  // setup.
  /// Clip a zone. To know if must be rendered etc... A zone is IN if in BACK of
  /// at least one plane of the pyramid.
  ...
  /** apply a landscape heightfield on a zone (modification of Z control points
   * values). NB: this is done in Landscape addZone(), before compile(). \param
   * landScape the landscape which gives Z delta values, for a x,y point.
   */
  void applyHeightField(const CLandscape &landScape);

  /** Debug purpose only : setup the colors of the patch of this zone so that it
   * shows which tiles have vegetable disabled, or are above, below water. User
   * provides a table with 4 colors for each state : color 0 = above water color
   * 1 = underwater color 2 = intersect water color 3 = vegetable disabled
   */
  ...
  /** Get the Bounding spehere of a patch. Stored in Zone as an array and not in
     CPatch for Fast Memory access consideration during clipping. Work only when
     zone compiled.
  */
  ...
  bool isPatchRenderClipped(uint patch) const {
    return _PatchRenderClipped.get(patch);
  }
...
private:
  ...
  std::vector<CBorderVertex> BorderVertices;
  // NB: No problem on corners, since zones are compile()-ed with knowledge of
  // neighbors.

  // The patchs.
  std::vector<CPatch> Patchs;
  std::vector<CPatchConnect> PatchConnects;
  // Clipped States and BSphere stored here and not in CPatch for faster memory
  // access during clip
...
private:
  /**
   * Force border patchs (those who don't bind to current zone) to re bind()
   * them, using new neighborood. no-op if zone is not compiled.
   */
  ...
  void buildBindInfo(uint patchId, uint edge, CZone *neighborZone,

...
public:
   ...
   *	BendCenterZ: Center of bend is made on (vertx, verty, 0), perfect for
   *"comb-like" vegetation.
   */
  enum TBendCenterMode { BendCenterNull = 0, BendCenterZ, BendCenterModeCount };

  /** The standard input vertexBuffer
   *	If it has vertexColor, then BendWeight is read from color.R, and scale
   *to take MaxBendWeight at max. else, BendWeight is read from pos.z (clamped
   *to (0, maxZ), and scaled to take MaxBendWeight at max. If it has normal, and
   *Lighted==true, then it is lighted. Must have TexCoord0.
   */
  CVertexBuffer VB;
   ...
   */
  bool AlphaBlend;
  /** true If this shape must take one sided lighting. ie max(normal, -normal)
   *is computed during ligthing compute NB: valid ONLY if (PreComputeLighting)
   */
...
public:
  ...
  /** VertexBuffer of this Shape, ready to be transformed and copied into
   *vegetable manager Format is Pos/Normal/Tex0/Tex1 (no Normal if !Lighted).

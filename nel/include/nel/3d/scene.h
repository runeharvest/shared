 ...
 *
 * \b USER \b RULES:
 * - Before creating any CScene, call the cool method CScene::registerBasics(),
 *to register basic models.
 ...
 *
 * Coordinate System:  right hand cordinates with: X to the right, Y to the far,
 *Z to the top.
 *
 *           Z ^
 *             |      > Y
 ...
 *		- IShape  design the object that is instancied (a mesh as
 *example).
 *		- ITransformShape is the instance, which smart_point to a
 *IShape.
 *		- user can add shape manually in the scene CShapeBank with
 *CShapeBank::add (), or remove them with CShapeBank::release ().
 *		- user create instance of a shape with
 *CScene::createInstance(string shapeName); This create/load auto the shape if
 *needed (searching in CPath, shapename must be a valid file name), and then
 *create the instance, with help of IShape::createInstance().
 *
...
public:
   ...
   */
  static void registerModel(const NLMISC::CClassId &idModel,
  ...
  /** Release all relative to the scene (Models, traversals...)... Destroy the
   *Basic traversals too. The Lod Character Manager is reset() ed, but not
   *deleted (at dtor only).
   */
  void release();
  //@}
   ...
   *  This also update waiting instance that are loaded asynchronously (by
   * calling updateWaitingInstances) NB: no Driver clear buffers (color or
   * ZBuffer) are done.... This call t->traverse() function to registered render
   * traversal following their order given. NB: assert-crash if you are between
   * a beginPartRender() and a endPartRender() \param doHrcPass set it to false
   * to indicate that the CHrcTrav have not to be traversed. Useful to optimize
   * if you know that NONE of your models have moved (a good example is a shoot
   * of the scene from different cameras).
   */
   ...
   *(not fully tested) WARNING: assert-crash if beetween a
   ...
   *choose which part of the scene must be rendered WARNING: always must begin
   *rendering with at least UScene::RenderOpaque, else shadows won't work
   *	WARNING: assert-crash if a part in 'rp' has already been rendered since
   *the last beginPartRender()
   */
  void renderPart(UScene::TRenderPart rp, bool doHrcPass = true,
                  bool doTrav = true, bool keepTrav = false);
  ...
  void setViewport(const class CViewport &viewport) { _Viewport = viewport; }
   ...
   */
  CInstanceGroup *
  findCameraClusterSystemFromRay(CInstanceGroup *startClusterSystem,
   ...
   *
   * Model are deleted with the deleteModel() or with release() which delete all
   * models NB: Since CScene own the model, model MUST NOT be used with
   * SmartPtrs (but CRefPtr always work...). \param idModel the Unique Id of the
   * Model \return a valid model of the required type. NULL, if not found. \see
   * deleteModel()
   */
  ...
  /** Create a model, instance of the shape "shapename". If not present, try to
   * load "shapename" via the CPath. If fails, return NULL.
   */
  CTransformShape *createInstance(const std::string &shapeName);

  /** Create an instance, if the shape is not present, load the shape
   * asynchronously. The instance is really created when we process it in the
   * rendering.
   */
  void createInstanceAsync(const std::string &shapeName,
                           CTransformShape **pInstance,
                           const NLMISC::CVector &position,
                           uint selectedTexture);

  /** Delete an instance via his pointer. An instance is an entity which
   * reference a shape.
   */
  ...
  /// Add an IG for auto PointLight Factor animation. called by
  /// CInstanceGroup::addToScene()
  void addInstanceGroupForLightAnimation(CInstanceGroup *ig);
  /// Remove an IG for auto PointLight Factor animation. called by
  /// CInstanceGroup::removeFromScene()
  ...
  /// get the current time of the scene, in second. It start from 0 (at the firt
  /// call of animate)
  ...
  /** System time is a time that always run (independent from the animation time
   *that run only on animate()) It is updated at beginning of render()
   */
  ...
  uint getMaxSkeletonsInNotCLodForm() const {
    return _MaxSkeletonsInNotCLodForm;
  }
  ...
  //@{
  CCoarseMeshManager *getCoarseMeshManager() { return _CoarseMeshManager; }
  ...
  uint8 getCoarseMeshLightingUpdate() const {
    return _CoarseMeshLightingUpdate;
  }

  /// Get the LodCharacterManager. NULL if user did not set it. (done in
  /// DriverUser)
  ...
  uint getNumLightGroup() const { return (uint)_LightGroupColor.size(); }
  ...
  NLMISC::CRGBA getAnimatedLightFactor(sint animatedLightmap,
                                       uint lightGroup) const {
  ...
  /// Get a ref. to the particle system manager. You shouldn't call this (has
  /// methods for private processing)
  CParticleSystemManager &getParticleSystemManager();

  /// set the automatic animation set used by this scene. It is stored as a
  /// smart pointer
  ...
  /// \name Private
  // @{
  /// The scene owns a list of skeleton models. Added/Removed by CSkeletonModel
  /// intModel()/dtor
  ...
  ItSkeletonModelList getSkeletonModelListEnd() {
    return _SkeletonModelList.end();
  }

  /// The scene owns a list of CTransform enabled to Cast shadows. Added/Removed
  /// by createShadowMap()/deleteShadowMap()
  ...
  ItShadowCasterList getShadowCasterListBegin() {
    return _ShadowCasterList.begin();
  }
  ItShadowCasterList getShadowCasterListEnd() {
    return _ShadowCasterList.end();
  }
  ...
  void incrementProfileTriVBFormat(std::map<uint32, uint32> &formatToTri,
                                   uint32 vbFormat, uint32 numTris) {
   ...
   * simulate 'retinian persistence' based on the visibility in the current
   * scene. Several context allow to deals with a flare rendered from several
   * points of views. There's a limited number of contexts (MaxNumFlareContexts)
   */
  // @{
  ...
  void setFlareContext(uint context) {
    nlassert(context < MaxNumFlareContexts);
    _FlareContext = context;
  }
  ...
  uint getShadowMapMaxCasterInScreen() const {
    return _ShadowMapMaxCasterInScreen;
  }
  ...
  void setVisualCollisionManagerForShadow(CVisualCollisionManager *vcm) {
    _VisualCollisionManagerForShadow = vcm;
  }
  CVisualCollisionManager *getVisualCollisionManagerForShadow() const {
    return _VisualCollisionManagerForShadow;
  }
  // @}
  ...
  ILandscapePolyDrawingCallback *getLandscapePolyDrawingCallback() const {
    return _PolyDrawingCallback;
  }

  // Add a new occlusion query object that must be tested at the end of the
  // rendering
  ...
  // envmap may be shared accross several scenes.

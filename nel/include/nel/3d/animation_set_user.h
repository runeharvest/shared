...
public:
  ...
  CAnimationSetUser(CDriverUser *owner, NLMISC::IStream &f) {
    ...
    // create a smartptred animation set. DO NOT Allow header compression, cause
    // serial()
  ...
  uint addAnimation(const char *fileName, const char *animName,
                    bool displayMissingFileWarning = true) {
    ...
    NLMISC::CIFile file;
    std::string path =
        NLMISC::CPath::lookup(fileName, false, displayMissingFileWarning);
    if (path.empty())
      path = fileName;
    if (file.open(path)) {
  ...
  virtual uint addSkeletonWeight(const char *fileName, const char *skelName) {
    ...
    if (file.open(NLMISC::CPath::lookup(fileName))) {
      ...
      // Add the animation
      uint id =
          _AnimationSet->addSkeletonWeight(skelName, skeletonWeight.release());
  ...
  /**
   * Get an animation ID by name. If no animation is found, method returns
   * NotFound.
   */

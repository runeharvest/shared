 ...
 * \date 2001
 */
template <class CKeyT, class T, class TMapTimeCKey> class CTCBTools {
protected:
  ...
  void compileTCBEase(TMapTimeCKey &mapKey, bool loopMode) {
    ...
    for (; it != mapKey.end(); it++) {
      ...
      // loop mgt. must compute ease from last to first (useful if _RangeLock is
      // false).
  ...
  void computeTCBFactors(const CKeyT &key, float timeBefore, float time,
                         float timeAfter, float rangeDelta, bool firstKey,
                         bool endKey, bool isLoop, float &ksm, float &ksp,
                         float &kdm, float &kdp) {
    float fp, fn;
...
class CTrackKeyFramerTCB
    : public ITrackKeyFramer<CKeyT>,
      private CTCBTools<CKeyT, T, std::map<TAnimationTime, CKeyT>> {
public:
protected:
  ...
  virtual void compile() {
    ...
    typename std::map<TAnimationTime, CKeyT>::iterator it =
        this->_MapKey.begin(); // first key.
    typename std::map<TAnimationTime, CKeyT>::iterator itNext = it;
    itNext++; // second key.
    typename std::map<TAnimationTime, CKeyT>::iterator itPrev =
        this->_MapKey.end();
    itPrev--; // last key.
    ...
    if (nKeys == 2 && !this->getLoopMode()) {
      ...
      // rangeDelta is the length of effective Range - length of
      // LastKey-FirstKey. NB: if RangeLock, rangeDelta==0.
      float rangeDelta;
      // NB: _RangeDelta has just been compiled in
      // ITrackKeyFramer<CKeyT>::compile().
      ...
      for (; it != this->_MapKey.end();) {
        ...
        if (this->getLoopMode() ||
            (it != this->_MapKey.begin() && itNext != this->_MapKey.begin())) {
          computeTCBKey(itPrev->second, it->second, itNext->second,
                        itPrev->first, it->first, itNext->first, rangeDelta,
                        it == this->_MapKey.begin(),
                        itNext == this->_MapKey.begin(), this->getLoopMode());
        }
      ...
      // In not loop mode, compute first and last key, AFTER middle keys
      // computed.
      if (!this->getLoopMode()) {
        typename std::map<TAnimationTime, CKeyT>::iterator it0 =
            this->_MapKey.begin(); // first key.
        typename std::map<TAnimationTime, CKeyT>::iterator it1 = it0;
        it1++; // second key.
        typename std::map<TAnimationTime, CKeyT>::iterator itLast =
            this->_MapKey.end();
        itLast--; // last key.
        typename std::map<TAnimationTime, CKeyT>::iterator itLastPrev = itLast;
        itLastPrev--; // prev of last key.
  ...
  // *****************
private:
  void computeTCBKey(CKeyT &keyBefore, CKeyT &key, CKeyT &keyAfter,
                     float timeBefore, float time, float timeAfter,
                     float rangeDelta, bool firstKey, bool endKey,
                     bool isLoop) {
    float ksm, ksp, kdm, kdp;

    // compute tangents factors.
    this->computeTCBFactors(key, timeBefore, time, timeAfter, rangeDelta,
                            firstKey, endKey, isLoop, ksm, ksp, kdm, kdp);
  ...
  void computeFirstKey(CKeyT &keyFirst, CKeyT &keyAfter) {
    ...
    keyFirst.TanFrom =
        tm * ((keyAfter.Value - keyFirst.Value) * 3.0f - keyAfter.TanTo);
  }
  ...
  void computeLastKey(CKeyT &keyLast, CKeyT &keyBefore) {
    ...
    keyLast.TanTo =
        tm * ((keyLast.Value - keyBefore.Value) * 3.0f - keyBefore.TanFrom);
  }
};
...
class CTrackKeyFramerTCB<CKeyTCBQuat, NLMISC::CAngleAxis>
    : public ITrackKeyFramer<CKeyTCBQuat>,
      private CTCBTools<CKeyTCBQuat, NLMISC::CAngleAxis,
                        std::map<TAnimationTime, CKeyTCBQuat>> {
public:
  /// \name From ITrackKeyFramer
  // @{
  ...
  virtual void evalKey(const CKeyTCBQuat *previous, const CKeyTCBQuat *next,
    ...
    if (previous && next) {
      ...
      // quad slerp.
      resultVal.Value = CQuat::squadrev(next->LocalAngleAxis, previous->Quat,
                                        previous->A, next->B, next->Quat, date);
    } else {
      if (previous)
  ...
  virtual void compile() {
    ...
    for (; it != _MapKey.end();) {
      ...
      computeTCBKey(itPrev->second, it->second, itNext->second, itPrev->first,
                    it->first, itNext->first, rangeDelta, it == _MapKey.begin(),
                    itNext == _MapKey.begin(), getLoopMode());
  ...
  // *****************
private:
  void computeTCBKey(CKeyTCBQuat &keyBefore, CKeyTCBQuat &key,
                     CKeyTCBQuat &keyAfter, float timeBefore, float time,
                     float timeAfter, float rangeDelta, bool firstKey,
                     bool endKey, bool isLoop) {
    CQuat qp, qm;
    ...
    // compute tangents factors.
    float ksm, ksp, kdm, kdp;
    computeTCBFactors(key, timeBefore, time, timeAfter, rangeDelta, firstKey,
                      endKey, isLoop, ksm, ksp, kdm, kdp);

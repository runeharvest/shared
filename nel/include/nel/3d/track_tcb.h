...
protected:
	    ...
	    TAnimationTime date, IAnimatedValue &result)
	{
		...
		if (previous && next)
		{
			...
			copyToValue(resultVal.Value,
			    previous->Value * hb[0] + next->Value * hb[1] + previous->TanFrom * hb[2] + next->TanTo * hb[3]);
		}
		else
		{
			if (previous)
	...
	virtual void compile()
	{
		...
		itNext++; // second key.
		typename std::map<TAnimationTime, CKeyT>::iterator itPrev = this->_MapKey.end();
		itPrev--; // last key.
		...
		else
		{
			...
			if (!this->getLoopMode())
			{
				...
				it1++; // second key.
				typename std::map<TAnimationTime, CKeyT>::iterator itLast = this->_MapKey.end();
				itLast--; // last key.

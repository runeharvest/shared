 ...
 *
 * This mesh handle several meshes of any kind of shape (MRM, standard, coarse
 * meshes..) At run time, it chooses what LOD meshes it must render according to
 * its settings.
 *
...
public:
  ...
  public:
    ...
    public:
      CBuildSlot()
          : MeshGeom(NULL), DistMax(0.0f), BlendLength(0.0f), Flags(0) {}
      /**
       * Flags for the build of a slot
       *
       * BlendIn:	if this flag is specified, this mesh will blend before
       * be displayed. BlendOut:	if this flag is specified, this mesh
       * will blend before disapear. CoarseMesh: if this flag is specified, this
       * mesh is a coarse mesh.
       */
      enum {
        BlendIn = 0x01,
        BlendOut = 0x02,
      ...
      /**
       * A mesh base build to describe the mesh. Can't be NULL. The pointer is
       * owned by the CMeshMultiLod after the call.
       */
      IMeshGeom *MeshGeom;
    ...
    /// True if this mesh is a static lod (static means it doesn't move at each
    /// frame), else false for dynamic.
    bool StaticLod;
  ...
  /// Get slot mesh count.
  uint getNumSlotMesh() const { return (uint)_MeshVector.size(); }
  ...
  /// Is static mesh ?
  bool isStatic() const { return _StaticLod; }

  /** Change MRM Distance setup of the Lod 0 only. No op if the lod0 is not a
   *CMeshMRMGeom NB: no-op if distanceFinest<0, distanceMiddle<=distanceFinest
   *or if distanceCoarsest<=distanceMiddle. \param distanceFinest The MRM has
   *its max faces when dist<=distanceFinest. \param distanceMiddle The MRM has
   *50% of its faces at dist==distanceMiddle. \param distanceCoarsest The MRM
   *has faces/Divisor (ie near 0) when dist>=distanceCoarsest.
   */
  ...
  /// should not be called direclty as the intance of this shape will use
  /// 'getNumTrianglesWithCoarsestDist' themselves to get the correct distance.
  virtual float getNumTriangles(float distance) {
    return getNumTrianglesWithCoarsestDist(distance, -1);
  }

  // The same as getNumTriangles, but a coarsest dist is provided. When not -1,
  // it overrides the dist of the coarsest mesh
  float getNumTrianglesWithCoarsestDist(float distance,
                                        float coarsestMeshDist) const;
...
private:
  ...
  /// Static or dynamic load ? Yoyo: no more used, but leave for possible usage
  /// later...
  bool _StaticLod;
  ...
  void renderMeshGeom(uint slot, IDriver *drv, CMeshMultiLodInstance *trans,
                      float numPoylgons, uint32 rdrFlags, float alpha,
                      CCoarseMeshManager *manager);

  /// Render the CoarseMesh of a slot. must be a coarseMesh, and shoudl be
  /// called only in passOpaque.
  void renderCoarseMesh(uint slot, IDriver *drv, CMeshMultiLodInstance *trans,
                        CCoarseMeshManager *manager);

  /// Profile the MeshGeom of a slot, even if coarseMesh
  void profileMeshGeom(uint slot, CRenderTrav *rdrTrav,
                       CMeshMultiLodInstance *trans, float numPoylgons,
                       uint32 rdrFlags);

  /// compile misc things, like distMax, Coarse meshes, collision mesh at load
  /// time...
  void compileRunTime();

...
// class id for flares
const NLMISC::CClassId FlareModelClassId =
    NLMISC::CClassId(0x6d674c32, 0x53b961a0);
...
public:
   ...
   * \param index the index of the flare to set. Vaklue ranges from 0 to
   * MaxFlareNum - 1 \param tex the texture to set. NULL removes the texture
   */
   ...
   *  \param index the index of the flare to set. Value ranges from 0 to
   * MaxFlareNum - 1
   */
   ...
   * MaxFlareNum - 1
   */
  float getSize(uint index) const { return _Size[index]; }

  /** set the relative position of the nth flares. The default goes linearly
   * from 0 (which appear at the position of the flare) to 1 (which appears at
   * the center of the screen when the flare spaving is set to 1 \see
   * setFlareSpacing()
   */
  ...
  void setFlareSpacing(float spacing) { _Spacing = spacing; }

  /** Get the flares spacing : A spacing of 1.f means thta the last flare will
   * reach the center of the screen , a spacing of 0.5f means only the half way
   * to the middle of the screen will be reached
   */
  float getFlareSpacing(void) const { return _Spacing; }

  /// set the persistence of this shape, in second (the time it takes to fade
  /// from white to black)
   ...
   * \see setPersistence
   */
  TAnimationTime getPersistence(void) const { return _Persistence; }
  ...
  /// force the first flare to keep its real size (e.g the isze on screen
  /// doesn't remains constant)
  void setFirstFlareKeepSize(bool enable = true) {
    _FirstFlareKeepSize = enable;
  }
  ...
  /** Set Dazzle attenuation range. It is the same than with attenuationRange. 1
   * mean that the dazzle stops when the flare is at the border of screen. 0.5,
   * for the half way between center and border etc . \see enableDazzle()
   */
  void setDazzleAttenuationRange(float range) {
    _DazzleAttenuationRange = range;
  }

  /// get the attenuation range of Dazzle
  float getDazzleAttenuationRange(void) const {
    return _DazzleAttenuationRange;
  }
  ...
  /** set a distance ratio. when dist / maxViewDist is above this ratio, the
   * flares will start to fade The default is 0.9
   */
  ...
  /** The flare is considered to be at the infinite. This mean that it is always
   * drawn And that there's no attenuation with dist. The real flare must be
   * created far from the camera for this to work. The defualt is false
   */
  ...
  const std::string &getOcclusionTestMeshName() const {
    return _OcclusionTestMeshName;
  }
  ...
  // Tell whether the occlusion mesh inherit the rotation/scale part of the
  // model matrix
  void setOcclusionTestMeshInheritScaleRot(bool on) {
    _OcclusionTestMeshInheritScaleRot = on;
  }
  bool getOcclusionTestMeshInheritScaleRot() const {
    return _OcclusionTestMeshInheritScaleRot;
  }

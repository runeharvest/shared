...
public:
  ...
  // copy ctor. NB : Allocator is not shared from source, and bahave like when
  // reset(0) is called.
  COrderingTable(const COrderingTable<T> &other);

  // assignement operator. NB : Allocator is not shared from source, and bahave
  // like when reset(0) is called.
   ...
   *	\param maxElementToInsert prepare allocator for insert by setting
   *maximum insert() that will arise.
   */
  void reset(uint maxElementToInsert);

  /** Share allocator between 2 or more ordering tables. So that calling reset
   * will give the max number of insert for both tables. This is useful if
   * several table are used for sorting (example : sort by priority with one
   * table per possible priority) NB : the table of "source table" becomes the
   * used allocator
   */
  void shareAllocator(COrderingTable<T> &sourceTable) {
    _Allocator = sourceTable._Allocator;
  }

  /**
   * Insert an element in the ordering table
   *	NB: element is inserted in front of the list at nEntryPos (for optim
   *consideration) NB: nlassert in debug if num of insert() calls exceed value
   *passed in reset() NB: nlassert in debug if nEntryPos is => getSize()
   */
...
// ***************************************************************************
template <class T> COrderingTable<T>::~COrderingTable() { delete[] _Array; }
...
// ***************************************************************************
template <class T> uint32 COrderingTable<T>::getSize() { return _nNbElt; }
...
template <class T> void COrderingTable<T>::insert(uint32 nEntryPos, T *pValue) {
  ...
  nlassert(!_Allocator->NodePool.empty() &&
           _Allocator->CurAllocatedNode <
               (&_Allocator->NodePool[0]) + _Allocator->NodePool.size());
  ...
  // and replace head node with new data: consequence is pValue is insert in
  // front of the list
  headNode->val = pValue;
...
// ***************************************************************************
template <class T>
inline COrderingTable<T> &
COrderingTable<T>::operator=(const COrderingTable<T> &other) {
  _Allocator = new CAllocator;
  init(other._nNbElt);
  return *this;
}

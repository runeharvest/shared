 ...
 *
 * NB: ShapeCacheName is case-sensitive but shapeName are not (all entry are
 * lwrcased)
 *
...
public:
   ...
   * AsyncLoad_Shape : Asynchronous loading is currently loading the .shape
   * file, textures and lightmaps AsyncLoad_Texture : Asynchronous loading is
   * currently uploading textures and lightmaps to VRAM
   */
  enum TShapeState {
    NotPresent,
    Present,
  ...
  //@{
  /// Add a reference to a shape and return the instance created. NB:
  /// getPresentState()==Present must be tested first.
  IShape *addRef(const std::string &shapeName);

  /**
   * Release a reference to a shape by its instance. If the shape has no more
   * reference it is added to its own shape cache. When the shape cache is full
   ...
   *	\return NULL if shape not found or not loaded (if being async loaded
   *still return NULL)
   */
  ...
  /** Load the corresponding file from disk asynchronously and add it to the
   * bank. The driver passed to this function is used to know if we have to load
   * the textures.
   */
  void loadAsync(const std::string &shapeName, IDriver *pDriver,
                 const NLMISC::CVector &position, bool *bSignal,
  ...
  /// Setup the maximum number of bytes to upload for a frame (texture upload
  /// from RAM to VRAM)
  void setMaxBytesToUpload(uint32 MaxUploadPerFrame);

  /// Add directly a shape to the bank. If the shape name is already used do
  /// nothing.
  ...
  /**
   * Remove a ShapeCache. All shapes in the shape cache are deleted. All links
   * are redirected to the default ShapeCache
   */
  ...
  /**
   * Remove all ShapeCache and suppress all links (even the link to the default
   * cache are removed)
   */
  void reset();

  /// Set the shapeCache shapeCacheName the new size.(delete shapes if
  /// maxsize<shapeCacheSize).
  ...
  /// Link a shape to a ShapeCache. The ShapeCache must exist and must not
  /// contains the shape.
  void linkShapeToShapeCache(const std::string &shapeName,
   ...
   *	Shapes are Loaded if not present, assigned to the given cache, and fit
   *in the cache Size as max possible. NB: crash if you try to load a non shape
   *file (eg: a .dds etc...) \param shapeCacheName name of a shapeCache created
   *with addShapeCache()/setShapeCacheSize(). no-op if don't exist \param
   *fileList a list of file names. NB: CPath is used to load the shapes. \param
   *wildcard a filter string like: "*.shape", "??_HOM*.shape". NB: toLower-ed
   *internally \param flushTextures if true, then textures are flushed in the
   *driver drv
   */
  void preLoadShapes(const std::string &shapeCacheName,
                     ...
                     bool flushTextures = false, IDriver *drv = NULL);

  /** Mark this shape name as needing a buildSystemGeometry() (typically used
   *for selection) NB: this will take effect only for shapes on a subsequent
   *add(), load() or loadAsync()
   */
...
private:
  ...
  struct CWaitingShape {
    IShape
        *ShapePtr; // Do not work with this value that is shared between threads
    ...
    uint32
        UpTextProgress; // Upload Texture progress current texture or lightmap
...
private:
  ...
  bool processWSUploadTexture(CWaitingShape &rWS, uint32 &nTotalUploaded,
                              ITexture *pText);
...
private:
  ...
  CShapeCache *
  getShapeCachePtrFromShapeCacheName(const std::string &shapeCacheName);

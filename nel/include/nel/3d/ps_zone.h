...
/** This epsilon is in meter and give a thickness to surfaces for tests. It must
 * be above above 0 for the system to work correctly
 */
const float PSCollideEpsilon = 10E-3f;

/**
 * This class hold any entity that has an effect over located : a sink , a
 * bouncing zone etc This is a kind a specialized force, and it has an attached
 * list of the targets Not sharable accross systems.
 *
...
public:
   ...
   * The default behaviour call performMotion or show depending on the pass
   * being processed
   */
  ...
  /// Add a new type of located for this zone to apply on. nlassert if already
  /// present
  ...
  /** Inherited from CPSTargetLocatedBindable. It's called when one of the
   * targets has been detroyed or detached The default behaviour, release
   * collision infos from the located
   */

  virtual void releaseTargetRsc(CPSLocated *target);

  /// set the bounce factor. It has meaning only if the behaviour is set to
  /// bounce...
  void setBounceFactor(float bounceFactor) { _BounceFactor = bounceFactor; }

  /// get the bounce factor. It has meaning only if the behaviour is set to
  /// bounce...
  ...
  /** Compute collisions for the given target. This will update the collisions
   * infos. The caller must provide pointer to arrays positions before and after
   * time step.
   */
  virtual void computeCollisions(CPSLocated &target, uint firstInstanceIndex,
                                 const NLMISC::CVector *posBefore,
                                 const NLMISC::CVector *posAfter) = 0;
...
protected:
   ...
   * \param surfNormal  the normal of the surface at the collision point (this
   * must be a unit vector) \elasticity  1 = full bounce, 0 = no bounce
   * (contact) \ellapsedTime the time ellapsed
   */

  //	void bounce(uint32 locatedIndex, const NLMISC::CVector &bouncePoint,
  //const NLMISC::CVector &surfNormal, float elasticity, float ellapsedTime);
};
...
struct CRadiusPair {
  ...
  float R, R2;
  void serial(NLMISC::IStream &f) { f.serial(R, R2); }
};
...
/**
 *	The same as a plane, but with a rectangle. We don't encode the plane by
 *its normal, however...
 */

...
/**
 * This class holds a particle system. Most of the time it is used with a
 * particle system model. See particle_system_shape.h and
 * particle_system_model.h for more details. It can be used directly to create a
 * shape. If you plan to use this without a particle system model, make sure :
...
public:
   ...
   * \return true if the operation could be performed. It can fail when this
   * cause the system the system to last forever, which is incompatible with the
   * 'BypassMaxNumIntegrationSteps' in CParticleSystem.
   */
  bool merge(CParticleSystemShape *toMerge);

  /*** duplication method NOT SUPPORTED for now (duplication is using streams,
   * but it may not last) \param ch for private use, set to null by default
   */
  ...
  ///\name Sharing system
  //@{
  /** Enable/Disable sharing. When sharing is enabled, the state of a particle
   * system is the same for all the system that have the same shape. This allow
   * to gain memory. However, such system should not be built with LOD in mind
   * (for example, less emission with distance) LOD should be automatic for
   * these system (see Auto-Lod). This means that sharing is only useful for
   * system that have the same state, and if they are numerous : motion is
   * performed once, but only for one system with no LOD. LOD is done during
   * display only (when activated). The default for systems is to have no
   * sharing.
   */
  ...
  ///\name Scene setup
  //@{
  /** Set the scene in which the particle system is inserted. This is needed
   * when system must add objects to the scene (for particle that are mesh for
   * instance)
   */
   ...
   * This just duplicates the 'hiden' flag of matching transform (instance of
   * CParticleSystemModel) If the system goes from 'hide' to 'show', then no
   ...
   * NB: The previous matrix position is backuped during this call (used to
   * interpolate the system position during integration), so this should be
   * called only once per frame NB : pointer to the matrix should remains valid
   * as long as that particle system exists (no copy of the matrix is kept)
   */
  void setSysMat(const NLMISC::CMatrix *m);

  /** The same as 'setSysMat', but to set the matrix for elements with
   * matrixMode == PSUserMatrix NB : pointer to the matrix should remains valid
   * as long as that particle system exists (no copy of the matrix is kept)
   */
  ...
  const NLMISC::CMatrix &getSysMat() const {
    return _CoordSystemInfo.Matrix ? *_CoordSystemInfo.Matrix
                                   : NLMISC::CMatrix::Identity;
  }

  /// return the inverted matrix of the system
  const NLMISC::CMatrix &getInvertedSysMat() const {
    return _CoordSystemInfo.InvMatrix;
  }

  /** return the user matrix
   * NB : to save memory, the user matrix is actually saved when at least one
   * instance of CPSLocated that belongs to the system makes a reference on it.
   * This is usually the case when CPSLocated::setMatrixMode(PSUserMatrix) is
   * called. If no reference is made, then the fx matrix is returned instead
   */
  const NLMISC::CMatrix &getUserMatrix() const {
    NL_PS_FUNC_MAIN(getUserMatrix)
    return (_UserCoordSystemInfo &&
            _UserCoordSystemInfo->CoordSystemInfo.Matrix)
               ? *(_UserCoordSystemInfo->CoordSystemInfo.Matrix)
               : getSysMat();
  }
   ...
   * instance of CPSLocated that belongs to the system makes a reference on it.
   * This is usually the case when CPSLocated::setMatrixMode(PSUserMatrix) is
   * called. If no reference is made, then the inverted system matrix is
   * returned instead.
   */
  const NLMISC::CMatrix &getInvertedUserMatrix() const {
    return (_UserCoordSystemInfo &&
            _UserCoordSystemInfo->CoordSystemInfo.Matrix)
               ? _UserCoordSystemInfo->CoordSystemInfo.InvMatrix
               : getInvertedSysMat();
  }
  ...
  const NLMISC::CMatrix &getUserToFXMatrix() const {
    return (_UserCoordSystemInfo &&
            _UserCoordSystemInfo->CoordSystemInfo.Matrix)
               ? _UserCoordSystemInfo->UserBasisToFXBasis
               : NLMISC::CMatrix::Identity;
  }
  ...
  const NLMISC::CMatrix &getFXToUserMatrix() const {
    return (_UserCoordSystemInfo &&
            _UserCoordSystemInfo->CoordSystemInfo.Matrix)
               ? _UserCoordSystemInfo->FXBasisToUserBasis
               : NLMISC::CMatrix::Identity;
  }
  ...
  /// get the inverted view matrix . It is stored each time a new frame is
  /// processed
  const NLMISC::CMatrix &getInvertedViewMat(void) const {
    return _InvertedViewMat;
  }
  //@}
  ...
  /**
   * execute all the process of the system. It uses the driver that was set by a
   * call to setDriver. \param ellapsedTime The ellapsed time since the last
   * call \param pass the pass to be executed \see setDriver
   */
  virtual void step(TPass pass, TAnimationTime ellapsedTime,
                    CParticleSystemShape &shape, CParticleSystemModel &model);
  //@}
  ...
  /**\name Process attachment. Most process are located : set of objects of the
   * same type that have a position in space
   */

  //@{
  /** Attach a process (such as a located : see particle_system_process.h, and
   * ps_located.h) to the system. It is then owned by the process and will be
   * deleted by it. if already present -> nl assert \return true if the
   * operation could be performed. It can fail when this cause the system the
   * system to last forever, which is incompatible with the
   * 'BypassMaxNumIntegrationSteps' in CParticleSystem
   */
  ...
  /**\name User parameters. They may be or not used by the system. Their meaning
   * is defined during the construction of the system
   */

  //@{
  /** Set the value of a user parameter. It must range from 0 to 1. The user
   * value are not saved, and their default value is 0.f. The max number of user
   * param is MaxPSUserParam.
   */
   ...
   */
  void bindGlobalValueToUserParam(const std::string &globalValueName,
                                  uint userParamIndex);
  // Get name of a global value, or NULL if no global value is bound to that
  // user param
  ...
  /** Set a global vector value. Global vector values are set to (0, 0, 0) by
   * default Global vector values are used in some places. For example,
   * direction for a directionnal force.
   */
  static void setGlobalVectorValue(const std::string &name,
  ...
  public:
    ...
    const NLMISC::CVector &get() const {
      nlassert(_Value);
      return *_Value;
    }
    void set(const NLMISC::CVector &value) {
      nlassert(_Value);
      *_Value = value;
    }
  ...
  // Get a handle on a global value that provide a quick access on it (no map
  // lookup)
  static CGlobalVectorValueHandle
  getGlobalVectorValueHandle(const std::string &name);
  //@}
  ...
  // @{
  /** For edition purposes only : this allow to highlight in red the current
   * element being edited. \param located The located the current element
   * belongs to, or NULL if no element is selected. \index the index of the
   * element in the located. \lb the located bindable that is selected into a
   * located (NULL = all)
   */
  ...
  // @{
  /** This enable for more accurate integrations of movement. When this is
   * activated, integration is performed in a more accurate way when the
   * ellapsed time goes over a threshold, but it is more slow to perform.
   */
  void enableAccurateIntegration(bool enable = true) {
    _AccurateIntegration = enable;
  }
  bool isAccurateIntegrationEnabled(void) const { return _AccurateIntegration; }

  /** the time threshold and the max number of integration to perform, when
   * accurate integration is activated. The default is 0.15 for time threshold
   * and 2 for max NbIntegrations \param canSlowDown : Allow the system to slow
   * down in speed but to keep accuracy in its movement. It is useful for
   * critical situations where the framerate is very low. The default is true.
   */
  void setAccurateIntegrationParams(TAnimationTime threshold,
                                    uint32 maxNbIntegrations, bool canSlowDown,
  ...
  /** When activated, this bypass the limit on the max number of integration
   * steps This should NOT be used on FXs that are looping, because it would
   * slow endlessly Anyway if you try to do that an assertion will ocurrs
   * Typically, this is useful for spell fx because they are short, and it is
   * important that they don't slow down when framerate is too choppy
   */
  void setBypassMaxNumIntegrationSteps(bool bypass = true) {
    ...
    if (_BypassIntegrationStepLimit != bypass) {
      if (bypass) {
        if (!canFinish()) {
          nlwarning("<CParticleSystem::setBypassMaxNumIntegrationSteps> Can't "
                    "set flag to true. The system must have a finite duration");
  ...
  bool getBypassMaxNumIntegrationSteps() const {
    return _BypassIntegrationStepLimit;
  }

  /** Test if the system can finish (e.g it doesn't loop, doesn't have emitter
   * with illimited lifetime) NB : we assume that all emitters in the system are
   * accessible, e.g that the located graph is connex \param lastingForeverObj,
   * if not NULL, the pointer will be filled with the first object that last or
   * emit forever, or create a loop.
   */

  bool canFinish(CPSLocatedBindable **lastingForeverObj = NULL) const;

  /** Test if there are loops in the system. E.g A emit B emit A
   * NB : we assume that all emitters in the system are accessible, e.g that the
   * located graph is connex \param loopingObj, if not NULL, will be filled with
   * the first object that creates a loop.
   */
   ...
   *						- Hand tuned LOD (for emission,
   *color, size : this uses LOD as an input for attribute makers).
   *                     - Auto LOD : - With non-shared systems, it modulates
   *the emission period, quantity etc.. to get the desired result.
   *									 - With
   *shared systems : One version is animated with full LOD (no hand tuned LOD
   *should be applied !). All version are displayed with fewer particle than the
   *full LOD, depending on their distance. Visually, this is not as good as
   *hand-tuned system, or auto-LOD on non-shared systems, however ..
   */

  // @{

  /// set the max view distance for the system (in meters) . The default is 50
  /// meters.
  ...
  /// set a percentage that indicate where the 2nd LOD is located. Default is
  /// 0.5.
  void setLODRatio(float ratio) {
    nlassert(ratio > 0 && ratio <= 1.f);
    _LODRatio = ratio;
  }
   ...
   * You'll have for a given pos : pos * v + offset  = 0 at the nearest point,
   * and 1 when pos is at maxDist from the viewer. This is used by sub-component
   * of the system.
   */
  void getLODVect(NLMISC::CVector &v, float &offset, TPSMatrixMode matrixMode);

  /// get the current LOD of the system. It is based on the distance of the
  /// center of the system to the viewer
  ...
  float getOneMinusCurrentLODRatio(void) const {
    return _OneMinusCurrentLODRatio;
  }

  /** Enable / disbale auto-lod.
   * When auto-LOD is enabled, less particles are displayed when the system is
   * far. This apply to all particles in the systems (unless they override that
   * behaviour). The default is AutoLOD off.
   */
   ...
   * \param start A percentage of the max view dist that tells when the auto-lod
   * must start. \param strenght The degradation speed. It is interpreted as an
   * exponent.
   */
  ...
  /** when auto-lod on a non shared system is used, this set the degradation of
   * the system when it is far A value of 0 mean no more emissions at all. A
   * value of 0.1 means 10% of emission and so on. A value of 1 means there's no
   * LOD at all..
   */
  ...
  uint8 getAutoLODDegradationExponent() const {
    return _AutoLODDegradationExponent;
  }

  /** There are 2 modes for the auto-LOD (apply to shared systems only) :
   * - Particle are skip in the source container when display is performed (the
   * default)
   * - There are just less particles displayed, but this can lead to 'pulse
   * effect'. This is faster, though.
   */
  void setAutoLODMode(bool skipParticles) {
    _AutoLODSkipParticles = skipParticles;
  }
  bool getAutoLODMode() const { return _AutoLODSkipParticles; }

  /** Setup a color attenuation scheme with the distance from the viewer. This
   * doesn't act on a particle basis, the whole color of the system is changed
   * in an uniform way so it is fast (the same can be achieved on a particle
   * basis). This bypass the source of the scheme : it is set to 0 when the
   * system is on the user, and to 1 when it is at its max distance. \param
   * scheme A color scheme, that is then owned by this object. NULL disable
   * color attenuation. Any previous scheme is removed
   */
  ...
  CPSAttribMaker<NLMISC::CRGBA> *getColorAttenuationScheme() {
    return _ColorAttenuationScheme;
  }
   ...
   * Final color is the color due to attenuationByDistance (\see
   * getColorAttenuationScheme() modulated by the user color NB : that state is
   * not serialized
   */
  ...
  /** Get the current global color of the system. (It is updated just before
   * drawing...). It there's no color attenuation scheme it can be assumed to be
   * the same than the user color
   */
  ...
  void setForceGlobalColorLightingFlag(bool enable) {
    _ForceGlobalColorLighting = enable;
  }
  ...
  // \name Load balancing
  // @{
  // get an evaluation of how many tris are needed with the system for the given
  // distance
  float getWantedNumTris(float dist);

  /// set the number of tree the system may use. If not clled this will be the
  /// max
  void setNumTris(uint numFaces);

  /** Ask for the particle system to reevaluate the max number of faces it may
   * need. You don't usually need to call this
   */
  ...
  /** When this is set to false, the system will recompute his bbox each time it
   * is querried This may be needed for systems that move fast.
   */
  ...
  /** set a precomputed bbox (expressed in the system basis). This is allowed
   * only when setAutoComputeBBox is called with false (nlassert otherwise).
   */
  ...
  ///\name Invalidity flags (no direct effect, just indications for a third
  ///party, a model holding the system for example)
  // @{
  /** Tell the system that it is invalid when its out of range. The default is
   * false. This is only a indication flag and must be checked by third party (a
   * model holding the system for example)
   */
  ...
  bool getDestroyModelWhenOutOfRange(void) const {
    return _DestroyModelWhenOutOfRange;
  }
  ...
  /** when != to none, the Model hodling this sytem will be considered invalid
   * when dieCondition is met This is only an indication flag and must be
   * checked by third party (a model holding it for example) that must then use
   * the right methods \see hasEmitters \see hasParticles
   */
   ...
   * This may be necessary : the system could be destroyed because there are no
   * particles, but no particles were emitted yet
   *
   * This is an indication, and has no direct effect, and must be check by
   * calling isDestroyConditionVerified()
   *
   * If -1 is set (or a negative value), then the system will compute that delay
   * itself
   *
   * \see hasEmitters()
   * \see hasParticles()
  ...
  /** Must be called by a sub component of the system to tell that the duration
   * of the system may have changed. This can happen typically if :
   * - The system structure is changed (located added, merge ..)
   * - The lifetime of a located is changed
   * - Emitter parameters are modified
   * In this case, if the system must stop when there are no particle left, the
   * delay before that test must be recomputed
   */
  void systemDurationChanged();

  /** Get the a delay before to apply the death condition test
   * If the delay was set to -1 or a negative value, this will compute the
   * delay.
   */
  ...
  bool getAutoComputeDelayBeforeDeathConditionTest() const {
    return _AutoComputeDelayBeforeDeathTest;
  }

  /** tells the model holding this system that he become invalid when its out of
   * the view frustum. This is only an indication flag and must be checked by
   * third party (a model holding it for example) It has no direct effects \see
  ...
  bool doesDestroyWhenOutOfFrustum(void) const {
    return _DestroyWhenOutOfFrustum;
  }
  ...
  bool doesPerformMotionWhenOutOfFrustum(void) const {
    return _AnimType == AnimInCluster;
  }
  ...
  enum TPresetBehaviour {
    EnvironmentFX =
        0, // environment FX, not animated when not visible, persistent.
    RunningEnvironmentFX, /* an environment fx that should
                           ...
                           * it
                           */
    SpellFX, // always animated, not persistent, garanteed to match the good
             // frame even if framerate is low
    ...
    SpawnedEnvironmentFX, // environment fx, not animated when not visible, not
                          // persistent
    GroundFX, /** usually fx of foot steps (dust clouds etc.). Always animated,
               * persistents, duration of fxs is garanteed, but not velocity of
               * particle if framerate is too choppy (usually ok because
               * particle stay in place with those fxs)
               */
  ...
  ///\name sound managment
  // @{
  /// register a Sound server to this system. All systems share the same sound
  /// server.
  ...
  // *****************************************************************************************************
  ///\name external access to locatedBindable. PRIVATE PART (to avoid the use of
  ///friend)
  // @{
  /** register a locatedBindable, and allow it to be referenced by the given ID
   * this locatedBindable must belong to this system.
   * each pair <id, locatedBindable> must be unique, but there may be sevral LB
   * for the same key
   */
  void registerLocatedBindableExternID(uint32 id, CPSLocatedBindable *lb);

  /// unregister the given located bindable. An assertion is raised if it has
  ...
  const CPSLocatedBindable *getLocatedBindableByExternID(uint32 id,
                                                         uint index) const;
   ...
   * \warning As IDs are not internally stored in a vector, it is faster than
   * several calls to getID
   */
  ...
  // @{
  /** When using an emitter, it is allowed to have a period of '0'. This special
   * value means that the emitter should emit at each frame. This is deprecated
   * now (not framerate independent ..), but some previous systems may use it.
  ...
  // test if there are emitters in the system (e.g. derivers of CPSEmitter bound
  // to the system)
  bool hasEmittersTemplates() const;
  /** Eval a duration of the system (duration after which there are no particle
   * lefts). It is meaningful only if the system can finish. After the given
   * duration particle will have been spawn, and will possibly have finished
   * their life. The system may last longer (case where an emitter is triggered
   * when it bounce cannot be evaluated correclty without running the system) NB
   * : calling this is costly
   */
  float evalDuration() const;
  /** Enable/Disable auto-count mode. The default is disabled
   * In this mode, when a particle is spawned it is guaranteed to be created.
   * Particle array are resized if necessary for this. This helps to tune the
   * size of arrays that contains particle This is well adapted for edition, but
   * shouldn't be use at runtime because array are reallocated which doesn't
   * give the best performance. It is why that state isn't serialized. When the
   * system is modified by the user, he should call resetAutoCount, so that
   * array match the current number of particles. This is useful if the user
   * modifiy the system and that the number of particles decreases
   */
  ...
  void getTargeters(const CPSLocated *target,
                    std::vector<CPSTargetLocatedBindable *> &targeters);
  // allow to serialize identifiers. The default is false (to speedup
  // instanciation of ps by not creating unecessary strings)
  ...
  // Change z-bias of all materials. The z-bias value is given in world
  // coordinates
  ...
  /** get sorting of the system emitter precedence (it is topological sort of
   * the graph of emitters) The result vector contains the index of each process
   * (see getProcess) in order
   */
...
private:
  ...
  public:
    ...
    NLMISC::CVector
        CurrentDeltaPos; // current pos (for integration step) of the coord
                         // system relative to its final pos
  public:
    CCoordSystemInfo() { Matrix = NULL; }
  };
  ...
  public:
    ...
    NLMISC::CMatrix
        UserBasisToFXBasis; // conversion matrix : from user basis to FX Basis
    NLMISC::CMatrix
        FXBasisToUserBasis; // conversion matrix : from FX basis to user Basis
    uint16 NumRef; // number of objects in the system that use position of the
                   // user matrix because this is used rarely, we allocate
                   // memory to track the user matrix position only when needed
  public:
    CUserCoordSystemInfo() { NumRef = 0; }
  };

  CCoordSystemInfo _CoordSystemInfo; // coordinate system infos for this fx
  CUserCoordSystemInfo *_UserCoordSystemInfo; // coordinate system infos for an
                                              // hypothetic user matrix
  ...
  // number of rendered pass on the system, incremented each time the system is
  // redrawn
  uint64 _Date;

  /// Last update date of the system. Useful with sharing only, to avoid several
  /// motions.
  sint64 _LastUpdateDate;

  // current edited element located (edition purpose only)
  CPSLocated *_CurrEditedElementLocated;
  // current edited located bindable, NULL means all binadable of a located.
  // (edition purpose only)
   ...
   * the system may add objects to the scene (for particle that are meshs for
   * instance)
   */
  ...
  float _InvCurrentViewDist; // inverse of the current view dist. It can be the
                             // same than _InvMaxViewDist but when there's LOD,
                             // the view distance may be reduced
  ...
  const TGlobalValuesMap::value_type *
      *_UserParamGlobalValue; // usually set to NULL unless some user params
                              // mirror a global value, in this case this
                              // contains as many pointer into the global map as
                              // there are user params
  uint8 _BypassGlobalUserParam; // mask to bypass a global user param. This
  ...
  bool _ComputeBBox : 1; /// when set to true, the system will compute his BBox
                         /// every time computeBBox is called
  bool _BBoxTouched : 1;
  ...
  // The two following members have been moved after the bitfield to workaround
  // a MSVC 64-bit compiler bug (fixed in VS2013) For more info, see:
  // http://connect.microsoft.com/VisualStudio/feedback/details/777184/c-compiler-bug-vtable-pointer-put-at-wrong-offset-in-64-bit-mode
...
#endif
public:
  // For use by emitters only : This compute a delta of position of the fx
  // matrix to ensure that spawning position are correct when the system moves
  void interpolateFXPosDelta(NLMISC::CVector &dest, TAnimationTime deltaT);
  // For use by emitters only : This compute a delta of position of the user
  // matrix to ensure that spawning position are correct when the system moves
  void interpolateUserPosDelta(NLMISC::CVector &dest, TAnimationTime deltaT);
  // For use by emitters only : Get the current emit ratio when auto-LOD is
  // used. Valid only during the 'Emit' pass
  float getAutoLODEmitRatio() const { return _AutoLODEmitRatio; }
  // For private used by CPSLocated instances : should be called when the matrix
  // mode of a located has changed
  void matrixModeChanged(CParticleSystemProcess *proc, TPSMatrixMode oldMode,
                         TPSMatrixMode newMode);
  // FOR PRIVATE USE : called when one more object of the system needs the
  // _UserCoordSystemInfo field => so allocate it if needed.
  void addRefForUserSysCoordInfo(uint numRefs = 1);
  // FOR PRIVATE USE : called when one less object of the system needs the
  // _UserCoordSystemInfo field => deallocate it when there are no references
  // left
...
public:
  // spawn info. They are shared by all PS ! This is because there can be only
  // one PS processed at a time in the lib, so no need to store that per
  // instance
  ...
  static std::vector<NLMISC::CSmartPtr<CSpawnVect>>
      _Spawns; // for each process of the system (the array is ordered as
               // CParticleSystem::_ProcessVect is) , store a list of particles
               // to spawn. We can't spawn them directly, because ageing
               // particle must be processed and removed first to make room for
               // new particles and thus avoid a 'pulse' effect when framerate
               // is low or when spawning is fast  (see the update loop
               // CParticleSystem::step for details) NB : we use a smart pointer
               // to avoid resize of a vector of vector, which is baaad... This
               // is public but intended to be used by CPSLocated only.
  static std::vector<uint>
      _ParticleToRemove; // used during the update step, contains the indices of
                         // the particles to remove
  static std::vector<sint>
      _ParticleRemoveListIndex; // for each particle, -1 if it hasn't been
                                // removed, or else give the insertion number in
                                // _ParticleToRemove
  static std::vector<uint>
      _CollidingParticles; // index of particle that collided
  static std::vector<NLMISC::CVector>
      _SpawnPos; // spawn position of newly created particles
public:
 ...
 *  Because of cross referencement, an object of the system may need
 *referencment before it is created With map holding pointer to already created
 *object, we can duplicate the system safely for now it is for PRIVATE USE...

...
#define NL_PS_FUNC_MAIN(name)                                                  \
  CPSEnterLeave __psDebugHeader(#name);                                        \
  PSCurrName = _Name;
#else
 ...
 *  PSToolRender  : for edition purpose, show representations for forces,
 * emitters...
 */
enum TPSProcessPass { PSMotion, PSSolidRender, PSBlendRender, PSToolRender };

/** Objects of particle systems can be local to various matrixs defined by the
 * following enum
 */
enum TPSMatrixMode {
  PSFXWorldMatrix = 0,
...
/**
 *	A system particle process; A process is anything that can be called at
...
public:
  ...
  CParticleSystemProcess()
      : _Owner(NULL), _MatrixMode(PSFXWorldMatrix), _Index(0) {}
  ...
  /**
   * execute this process, telling how much time ellapsed must be used for
   * motion, and the real time ellapsed (for lifetime managment)
   */
  ...
  /// Shortcut to get a font generator if one was set, const version  (edition
  /// mode)
  ...
  /// Shortcut to get a font Manager if one was set, const version  (edition
  /// mode)
  ...
  /** Choose the basis for this process. NB: This won't change any existing
   * coordinate By default, all process are expressed in the world basis
   */

 ...
 *
 *	Because elements are duplicated all over cells and only one cell can be
 *selected at a time.
 *
 ...
 * \date 2002
 */
template <class T> class CStaticQuadGrid {
public:
  /// Constructor
  CStaticQuadGrid();
  /// dtor.
   ...
   *	Speed is in O(1), because the array of the cell is returned. NULL if
   *size==0.
   *
...
// ***************************************************************************
template <class T> CStaticQuadGrid<T>::CStaticQuadGrid() {
  _SizePower = 4;
  _Size = 1 << _SizePower;
  _EltSize = 1;
...
// ***************************************************************************
template <class T> CStaticQuadGrid<T>::~CStaticQuadGrid() { clear(); }

// ***************************************************************************
template <class T> void CStaticQuadGrid<T>::clear() {
...
// ***************************************************************************
template <class T> void CStaticQuadGrid<T>::build(CQuadGrid<T> &quadGrid) {
  clear();
...
// ***************************************************************************
template <class T>
const T *CStaticQuadGrid<T>::select(const NLMISC::CVector &pointIn,
                                    uint &numElts) {

...
/*
 *	In this file, we define several template that helps to create attributes
 *maker such as gradient (of float, int, vector etc) attributes maker are used
 *in the particle system to generate values, such as size, color etc. see
 *ps_attrib_maker.h for more information
 */

/** a blending function
 * it blends between t1 and t2 by the alpha amount
 * specializing this function may help with some types of data that don't have
 * the needed operator (NLMISC::CRGBA)
 */
...
inline NLMISC::CRGBA PSValueBlend(const NLMISC::CRGBA &t1,
                                  const NLMISC::CRGBA &t2, float alpha) {
  NLMISC::CRGBA result;
...
inline CPlaneBasis PSValueBlend(const CPlaneBasis &t1, const CPlaneBasis &t2,
                                float alpha) {
...
/// This produce Values
...
#endif
    return PSValueBlend(_StartValue, _EndValue,
                        time); // a cast to T is necessary, because
  ...
  ///
  T getMaxValue(void) const { return std::max((*this)(0), (*this)(1)); }
  T getMinValue(void) const { return std::min((*this)(0), (*this)(1)); }
  //@}
...
/** This is a Value blender class. The blending between value is not sampled
 * with this class. So it may be slow, but it is exact. It work with most type,
 * but some of them may need special  blending between value : if so you must
 * specialize the template function PSValueBlend defined in this file to do the
 * job... To use this, just derive a class, create a ctor, and declare it to the
 * class registry
 *
...
public:
  /** ctor
   *  With nbCycles, you can set the pattern frequency. It is usually one. See
   * ps_attrib_maker.h For further details
   */
...
/**
 * This temlate functor blend between 2 values by performing n samples (n =
 * template parameter) It may be faster that the CPSValueBlendFunc in some
 * cases. To accomplish blending, it use the template function PSValueBlend It
 * is used by CPSValueBlend \author Nicolas Vizerie \author Nevrax France \date
 * 2001 \see PSValueBlend
 */
...
/** This is a Values blender (sampled version, with n sample) class, that
 * operate on value of type T To use this, just derive a class from a
 * specialization of this template , create a ctor, and declare it to the class
 * registry in the ctor, you should call _F.setValue to init the functor object
 */

template <typename T, const uint n>
class CPSValueBlenderSample
    : public CPSAttribMakerT<T, CPSValueBlendSampleFunc<T, n>> {
public:
  /** ctor
   *  With nbCycles, you can set the pattern frequency. It is usually one. See
   * ps_attrib_maker.h For further details
...
/**
 * This functor blend between several Value. Intermediate value are sampled with
 * a given number of steps It is used by CPSValueGradient, that you can use to
 * have gradients with your own types \author Nicolas Vizerie \author Nevrax
 * France \date 2001
 */
...
/// this produce Values
  ...
  virtual void getValues(T *tab) const {
    ...
    T *pt = tab;
    uint32 src = 0;
    for (uint32 k = 0; k <= (_NbValues / _NbStages);
         ++k, src = src + _NbStages) {
      *pt++ = _Tab[src];
    }
  }
   ...
   *  \ValueTab a table containing the Values. Value will be blended, so you
   * must only provide keyframe Values \param nbStages The result is sampled
   * into a table by linearly interpolating values. This give the number of step
   * between each value WARNING : for integer types, some specilization exist
   * that ensure correct interpolation. see below
   */

  virtual void setValues(const T *ValueTab, uint32 numValues, uint32 nbStages);

  // the same, but value gradient has already been computed, so ValueTab must
  // contains numValues * nbStages + 1 values
  virtual void setValuesUnpacked(const T *ValueTab, uint32 numValues,
                                 uint32 nbStages);
  ...
  T getMaxValue(void) const { return _MaxValue; }

  T getMinValue(void) const { return _MinValue; }
 ...
 *  To use this, just derive a class from a specialization of this template ,
 * create a ctor, and declare it to the class registry in the ctor, you should
 * call _F.setValue to init the functor object
 */
...
template <typename T>
inline void computeGradient(const T *valueTab, uint32 numValues,
                            uint32 nbStages, typename CPSVector<T>::V &grad,
...
void computeGradient(const NLMISC::CRGBA *valueTab, uint32 numValues,
                     uint32 nbStages, CPSVector<CRGBA>::V &grad,
                     NLMISC::CRGBA &minValue, NLMISC::CRGBA &maxValue);

template <typename T>
void CPSValueGradientFunc<T>::setValues(const T *valueTab, uint32 numValues,
                                        uint32 nbStages) {
...
template <typename T>
void CPSValueGradientFunc<T>::setValuesUnpacked(const T *valueTab,
                                                uint32 numValues,
                                                uint32 nbStages) {
  _NbStages = nbStages;
...
#ifdef NL_COMP_VC14
  std::copy(valueTab, valueTab + _NbValues + 1,
            stdext::make_checked_array_iterator(&_Tab[0], _Tab.size()));
#else
  std::copy(valueTab, valueTab + _NbValues + 1, &_Tab[0]);
#endif
}

template <typename T> void CPSValueGradientFunc<T>::serial(NLMISC::IStream &f) {
  f.serialVersion(1);
  f.serial(_NbStages);
  if (f.isReading()) {
    ...
    if (NLMISC::CTraits<T>::HasTrivialCtor &&
        NLMISC::CTraits<T>::HasTrivialDtor && numVal < 256) {

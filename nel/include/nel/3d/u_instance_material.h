...
public:
  ...
  /// \name Texture UserColor. No effect if !isUserColor(). (getUserColor()
  ...
  /// Get the fileName used by the n-th texture file. (must be a texture file or
  /// an assertion is raised)
  std::string getTextureFileName(uint stage) const;
  // Empty the texture at the given stage
  void emptyTexture(uint stage = 0);
  /** Set the fileName used by the n-th texture file. (must be a texture file or
   *an assertion is raised) NB: if success and if
   *instanceOwner->getAsyncTextureMode()==true, then
   *instanceOwner->setAsyncTextureDirty(true) is called
   */
  void setTextureFileName(const std::string &fileName, uint stage = 0);
  /** Set the texture datas to be read from memory rather than from a file. This
   * erases any previous texture at that slot. The provided block can be a
   * memory image of a file, however \param stage The stage at which texture
   * must be set \param data Pointer of the file, or of the pixels datas \param
   * length length, in bytes, of the datas. \param isFile is true if the data
   * must be interpreted as a texture file. Otherwise, it is interpreted as the
   * raw datas of the texture, so the format and size of the texture must also
   * have been set to match the raw datas \param _delete Is true if the texture
   * has ownership on the texture datas \param texType relevant only when isFile
   * is set to false. Gives the format to expand the texture to when it is
   * generated.
   */
  void setTextureMem(uint stage, uint8 *data, uint32 length, bool _delete,
                     bool isFile = true, uint width = 0, uint height = 0,
                     NLMISC::CBitmap::TType texType = NLMISC::CBitmap::RGBA);
  ...
  /// Constructors
  UInstanceMaterial() { _Object = NULL; }
  UInstanceMaterial(class CMeshBaseInstance *mbi, class CMaterial *object,
                    class CAsyncTextureBlock *asyncTextureBlock) {
    _MBI = mbi;
    _Object = object;
    _AsyncTextureBlock = asyncTextureBlock;
  };
  /// Attach an object to this proxy
  void attach(class CMeshBaseInstance *mbi, class CMaterial *object,
              class CAsyncTextureBlock *asyncTextureBlock) {
    _MBI = mbi;
    _Object = object;
    _AsyncTextureBlock = asyncTextureBlock;

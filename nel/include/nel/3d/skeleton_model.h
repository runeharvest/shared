...
// ***************************************************************************
// ClassIds.
const NLMISC::CClassId SkeletonModelId =
    NLMISC::CClassId(0x7d4703b4, 0x43ad6ab1);
 ...
 *
 * Skeletons sons are added with bindSkin(), stickObject(). They are removed
 * auto at dtor.
 *
...
public:
   ...
   * NB: nlassert() if there is too many skins/sticked objects on this skeleton
   * (more than 255). NB: an object can't be skinned and sticked at same time :)
   * NB: replaced if already here.
   * NB: when a skin is binded, the command hide(), show(), ... have no effect
   * on it, until it is detachSkeletonSon()-ed NB: For Skins, all
   * Hrc/Clip/UpdateList link is done here \return false if mi is not skinnable,
   * true otherwise
   */
  bool bindSkin(CTransform *mi);
  /** parent a CTransform to a bone of the skeleton. NB: ~CTransform() calls
   * detachSkeletonSon(). NB: nlassert() if there is too many skins/sticked
   * objects on this skeleton (more than 255). NB: an object can't be skinned
   * and sticked at same time :) NB: replaced if already here. NB: mi is made
   * son of skeleton model in Traversals Hrc, and change are made at render()
   * for ClipTrav.
   */
  void stickObject(CTransform *mi, uint boneId);

  /** same method as stickObject(), but if you set forceCLod as true, then this
   *object will be visible even if the skeleton father is in CLod state (ie
   *displayed with a CLodCharacterShape) NB: if "mi" is a skeleton model,
   *forceCLod is considerer true, whatever the value passed in.
   */
  void stickObjectEx(CTransform *mi, uint boneId, bool forceCLod);

  /** unparent a CTransform from a bone of the skeleton, or unbind a skin. No-op
   * if not a son of this skeleton NB: mi is made son of Root in Traversals Hrc,
   * and change are made at render() for ClipTrav. NB: For Skins, all
   * Hrc/Clip/UpdateList link is done here
   */
  void detachSkeletonSon(CTransform *mi);

  /** Force the skeletonModel to recompute at next render which skins to render,
   *at which pass. If you call setOpacity()/setTransparency() on one of the
   *skins binded to the skeleton, you should call this method, else strange
   *result may occurs. NB: this is automatically called by
   *bindSkin()/detachSkeletonSon()
   */
   ...
   */
  const std::set<CTransform *> &getStickedObjects() const {
    return _StickedObjects;
  };
   ...
   *	set boneUsageType to UsageNormal if enable Skeleton Bone degradation
   *(skins) Forced usage are for Sticked objects
   */
  void incBoneUsage(uint i, TBoneUsageType boneUsageType);
  /// decrement the refCount of the ith bone. set forced to the same param
  /// passed when incBoneUsage()
  ...
  /// Tell if a bone has been computed in the last frame or not. false if boneId
  /// is invalid
  bool isBoneComputed(uint boneId) const;

  /** Force to compute a bone right now (useful if the skeleton is not visible
   * and the bone position is needed) no-op is bone not present or already
   * computed \return true if good indes & bone recomputed
   */
  bool forceComputeBone(uint boneId);

  /// return the number of bones currently animated/computed (because of
  /// bindSkin()/stickObject() / Lod system).
  uint getNumBoneComputed() const { return (uint)_BoneToCompute.size(); }

  /** change the Lod Bone interpolation distance (in meters). If 0,
   *interpolation is disabled. The smaller this value is, the more Lod skeleton
   *system will "pop". Default is 0.5 meters.
   */
  ...
  /** if Bones[boneId] is "Computed" (usage/lod), return
   *Bones[boneId].getBoneSkinMatrix() else return parent ones (recurs, but
   *precomputed)
   */
  const NLMISC::CMatrix &getActiveBoneSkinMatrix(uint boneId) const;

  /** Tool function, especially for animation bake. It updates all bones
   *(independent of bone usage, and lod interpolation), and take a user skeleton
   *worldMatrix as input. NB: no detail animation is performed here, just the
   *compute of bone hierarchy. NB: also, no special AnimCtrl (IK etc....) is
   *performed here
   */
  void computeAllBones(const CMatrix &modelWorldMatrix);

  /** Retrieve the current approx BBox around the skeleton, computed in the last
   *CSene::render(). for all computed bones, extend the bbox with their pos
   *	\param bbox return the bbox of the skinned skeleton, local to the
   *skeleton. If the skeleton was clipped, the bbox is not modified. \param
   *computeInWorld true if want to get the bbox in world. \return true if the
   *bbox is computed, false otherwise.
   */
  bool computeRenderedBBox(NLMISC::CAABBox &bbox, bool computeInWorld = false);

  /** same as computeRenderedBBox(), always in world, but use the bone max
   *sphere to enlarge the bbox NB: sticked objects don't influence the result
   */
  bool computeRenderedBBoxWithBoneSphere(NLMISC::CAABBox &bbox,
                                         bool computeInWorld = true);

  /** same as computeRenderedBBox() but force animation and compute of all bones
   *=> don't need render(), but slower. for all used bones, extend the bbox with
   *their pos NB: AnimCtrl are not evaluated by this method (since computed with
   *0 pos). \param bbox return the bbox of the skinned skeleton, local to the
   *skeleton. If the skeleton is not skinned/sticked at all, bbox is not
   *modified. \param forceCompute force evalution even if not skinned \param
   *computeInWorld true if want to get the bbox in world. \return true if the
   *bbox is computed, false otherwise.
   */
  ...
  /// SkeletonSpawnScript special: World Spawned objects are still relative to
  /// this position (default: Null)
  void setSSSWOPos(const CVector &pos) { _SSSWOPos = pos; }
  const CVector &getSSSWOPos() const { return _SSSWOPos; }
  /// SkeletonSpawnScript special: World Spawned objects are still relative to
  /// this direction (default: J)
  ...
  /** Internal. Tool method used by Skins, to remap their bones id to skeleton
   *ones \param bonesName input list of bones name \param boneId output list of
   *bone remapped (size==bonesName.size()). -1 if bone not found \param remap ==
   *boneId, but with 0 where boneId[i]==-1
   */
  void remapSkinBones(const std::vector<std::string> &bonesName,
                      std::vector<sint32> &bonesId, std::vector<uint> &remap);
  ...
  /// Change the Character Lod shape Id. set -1 if want to disable the feature
  /// (default)
  ...
  /** True if the skeleton model and his skins are to be displayed with a
   *CLodCharacterShape, instead of the std way This state is modified early
   *during the HRC Traversal. Because Clip traversal need this result.
   */
  bool isDisplayedAsLodCharacter() const { return _DisplayedAsLodCharacter; }

  /** This is the distance at which the skeleton use a CLodCharacterShape to
   *display himself if 0, never display the skeleton as a CLodCharacterShape
   */
   ...
   *	return 0 if CLod not enabled. Else return a priority>0 computed from
   *curDistance/LodCharacterDistance return 0 too if the skeleton is not visible
   *or if his ancestorSkeletonModel is not visible. If priority is >1 then the
   *skeleton must be displayed in CLod form
   */
   ...
   *	NB: Lod texturing is possible only in conjunction with
   *AsyncTextureManager. Hence, instances skinned to the skeleton should be in
   *AsyncTextureMode. For best result, you should wait that each of these
   *instances are isAsyncTextureReady() (texture loaded)
   */
  void computeLodTexture();

  /// Set the emissive of the skeleton model, when it is rendered in CLod form.
  /// Default to Black
  ...
  /** Special version for skins. NB: skins never follow their original MRM
   *distance setup, but follow this skeleton MRM setup. Default is to follow the
   *MAX of all skins binded (ie the finer). NB: Unlike
   *CMeshBaseInstance::changeMRMDistanceSetup(), this setup applies to the
   *SkeletonModel, not the shape. NB: no-op if distanceFinest<0,
   *distanceMiddle<=distanceFinest or if distanceCoarsest<=distanceMiddle.
   *	\param distanceFinest The MRM has its max faces when
   *dist<=distanceFinest. \param distanceMiddle The MRM has 50% of its faces at
   *dist==distanceMiddle. \param distanceCoarsest The MRM has faces/Divisor (ie
   *near 0) when dist>=distanceCoarsest.
   */
  void changeMRMDistanceSetup(float distanceFinest, float distanceMiddle,
  ...
  /**	If displayed as a CLod, render it, else render the skins binded to this
   * skeleton
   */
  ...
  /// \name AnimCtrl (IK...)
  // @{
  /** Set a special ctrl on a bone. see IAnimCtrl. NB: once an animCtrl is set
   *to a bone in a skeleton, his bones are always computed each frame. set to
   *NULL if you want to reset this bone AnimCtrl.
   */
  ...
  // TMP nico debug function : retrieve all max bone spheres in world
  // coordinates
...
private:
  ...
  struct CBoneUsage {
    ...
    uint16 Usage;
    /// Same as Usage, but must be animated/computed, even if Skeleton Lods say
    /// not (stickedObjects).
    uint16 ForcedUsage;
    /** Same as ForcedUsage, but must be animated/computed, even if the skeleton
     *is in CLod state ie displayed with a CLodCharacterShape. This is important
     *for skeletons which have skeletons sons sticked on them
     */
    uint16 CLodForcedUsage;
    /** The current state: which bones need to be computed. ie:
     *	(CLodForcedUsage) | ( ((Usage & currentLodUsage) | ForcedUsage) &
     *skeleton not in CLod state )
     */
    uint16 MustCompute;
  ...
  /// Flag set if the MRSkeleton lod change, or if a new bone Usage change (only
  /// if was 0 or become 0).
  ...
  /// According to Usage, ForedUsage and current skeleton Lod, update
  /// MustCompute and ValidBoneSkinMatrix
  ...
  /** True if the skeleton model and his skins have to be displayed with a
   *CLodCharacterShape, instead of the std way This state is modified early
   *during the HRC Traversal. Because Clip traversal need this result.
   */
  ...
  /** render a list of skin, no lighting setup etc..., but use where possible a
   * CVertexStreamManager
   */
  void renderSkinList(NLMISC::CObjectVector<CTransform *, false> &skinList,
                      float alphaMRM);
  // @}
...
// ***************************************************************************
/*
 *	Function used by CMeshMRM and CMeshMRMSkinned, to compute CMatrix3x4 of
 *used skeleton bones
 */
template <class TMatrixArray>
inline void computeBoneMatrixes3x4(TMatrixArray &boneMat3x4,
...
template <class TMatrixArray>
inline void computeBoneMatrixes3x4PreMul(TMatrixArray &boneMat3x4,

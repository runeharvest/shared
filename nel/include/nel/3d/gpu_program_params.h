...
public:
  ...
  // Set by name, it is recommended to use index when repeatedly setting an
  // element
  ...
  void set4i(const std::string &name, sint32 i0, sint32 i1, sint32 i2,
             sint32 i3);
  ...
  void set4ui(const std::string &name, uint32 ui0, uint32 ui1, uint32 ui2,
              uint32 ui3);
  ...
  size_t allocOffset(const std::string &name, uint size, uint count,
                     TType type);
  ...
  // Data access
  inline uint getSizeByOffset(size_t offset) const {
    return m_Meta[offset].Size;
  } // size of element (4 for float4)
  inline uint getCountByOffset(size_t offset) const {
    return m_Meta[offset].Count;
  } // number of elements (usually 1)
  inline uint getNbComponentsByOffset(size_t offset) const {
    return m_Meta[offset].Size * m_Meta[offset].Count;
  } // nb of components (size * count)
  ...
  inline TType getTypeByOffset(size_t offset) const {
    return m_Meta[offset].Type;
  }
  inline uint getIndexByOffset(size_t offset) const {
    return m_Meta[offset].Index;
  }
  const std::string &getNameByOffset(size_t offset) const {
    return m_Meta[offset].Name;
  };
  // @}

  // Utility
  static inline uint getNbRegistersByComponents(uint nbComponents) {
    return (nbComponents + 3) >> 2;
  } // vector register per 4 components

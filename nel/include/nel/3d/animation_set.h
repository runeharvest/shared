...
/**
 * An CAnimationSet is set of CAnimation. It manages global channel ID for a set
 * of animations with many tracks. An Id is associed with each track name.
 *
...
public:
  ...
  /**
   * \param headerOptim if true, the animationSet cannot be serialized
   *(nlassert), but could save lot of memory load: CTrackSampleQuat memory are
   *optimized (48 bytes to 12) and map<string, id> per animation is replaced
   *with a (IdChannelInAnimSet, IdTrackInAnim) association
   */
  CAnimationSet(bool headerOptim = false);
  ~CAnimationSet();

  /**
   * Get channel ID count. This number is the count of different track name in
   * the animation set.
   */
  ...
  /**
   * Get a channel ID with its name. If no channel is found, method returns
   * NotFound.
   */
  uint getChannelIdByName(const std::string &name) const {
    // Look for an id with this name
    std::map<std::string, uint32>::const_iterator ite =
        _ChannelIdByName.find(name);
  ...
  /**
   * Get a animation ID by name. If no channel is found, method returns
   * NotFound.
   */
  uint getAnimationIdByName(const std::string &name) const {
    // Look for an id with this name
    std::map<std::string, uint32>::const_iterator ite =
        _AnimationIdByName.find(name);
   ...
   */
  uint getNumAnimation() const { return (uint)_Animation.size(); }
   ...
   */
  uint getNumSkeletonWeight() const { return (uint)_SkeletonWeight.size(); }

  /**
   * Get a skeleton weight ID by name. If no skeleton weight is found, method
   * returns NotFound.
   */
  uint getSkeletonWeightIdByName(const std::string &name) const {
    // Look for an id with this name
    std::map<std::string, uint32>::const_iterator ite =
        _SkeletonWeightIdByName.find(name);
  ...
  /**
   * Add an animation to the set. The pointer of the animation must be allocated
   *with new. It is then handled by the animation set.
   *
   *	WARNING: it assert if you call addAnimation() after build(), while the
   *animation set is in HeadOptim mode
   *
  ...
  /**
   * Add a skeleton weight to the set. The pointer of the skeletonWeight must be
   * allocated with new. It is then handled by the animation set.
   *
   ...
   *
   * First, for each animation you want to add to the set, you must add the
  ...
  /**
   * Helper method.
   * Load an animation set from animation files in a directory, then call
   * build(). \param path Path to look at for animations \param recurse whether
   * to recurse to load animation in sub folders \param ext the extension for
   * animation files \param wantWarningMessage displays warning if some of the
   * files could not be loaded \return true if everything loaded ok
   */
  bool loadFromFiles(const std::string &path, bool recurse = true,
                     const char *ext = "anim", bool wantWarningMessage = true);

  /** Set the animation Set in "Low Memory" mode by skipping some keys
   * Each added animation will loose some keys for CTrackSampledQuat and
   *CTrackSampledVector \param sampleDivisor if set to 5 for instance, the
   *number of keys will be divided (ideally) by 5. if 0, set to 1. if 1 => no
   *key skip (default to 1)
   */

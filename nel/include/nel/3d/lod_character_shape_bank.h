...
public:
  ...
  /// Just add a shape to the bank. Use getShapeFullAcces() to fill it. return
  /// id of the shape
  uint32 addShape();
  ...
  /// Get a const ref on a shape. Ptr not valid after addShape(). NULL if not
  /// found
  ...
  /** re-compile the shape Bank. This must be called after changing a shape, or
   *after adding all your shapes. It return false if same names have been found,
   *but it is still correctly builded.
   */
  bool compile();

...
// ***************************************************************************
/**
 * This is a bone default value. This object is stored in the SkeletonShape, and
 * is serialised. For hierarchy of bones, it contains the id of his father.
...
public:
  ...
  /// The distance at which the bone is disabled in the skeleton. If 0, never
  /// disable.
...
// ***************************************************************************
/**
 * This is a bone, for skeleton animation, with information for result
 * WorldMatrix. \author Lionel Berenguier \author Nevrax France \date 2001
 */
...
public:
   ...
   * This ctor:
   *	- store a RefPtr on the bonebase (for getDefaultTracks() method). the
   *refptr is just for nlassert.
   *	- copy the bonebase default track value into Animated Values Pos/Rot
   *etc....
   */
  ...
  /** Compute the LocalSkeletonMatrix, the WorldMatrix, and the BoneSkinMatrix
   * (for skinning). NB: the result localSkeletonMatrix depends on
   * BoneBase::UnheritScale. \n NB: the result worldMatrix depends on
   * BoneBase::UnheritScale. \n NB: the result boneSkinMatrix depends on
   * BoneBase::InvBindPos. \n \param parent the parent of this bone (maybe NULL
   * if root). His WorldMatrix is used, so it should be computed before. \param
   * rootMatrix is used as father worldmatrix if parent==NULL. Useful for
   * computing WorldMatrix. \param skeletonForAnimCtrl if NULL, no AnimCtrl is
   * performed, else skeletonForAnimCtrl->getWorldMAtrix() should be == to
   * rootMatrix
   */
   ...
   *	NB: the interpolation is made on per-vector basis => bad matrix
   *interpolation.
   */
  ...
  /// enable the channels (lodEnable) associated to this bone in the
  /// channelMixer.
  void lodEnableChannels(CChannelMixer *chanMixer, bool enable);

  /** Force to eval the animation of that bone
   * Useful when a bone position is needed, and if the father skeleton has been
   * clipped (and thus not detail-animated) \param chanMixer the channel mixer
   * to which that bone has been registered
   */
  ...
  // *************************
public:
  // Private to SkeletonModel. You should not set this ptr directly. see
  // CSkeletonModel::setBoneAnimCtrl() The extra controller (IK...) on this bone
  IAnimCtrl *_AnimCtrl;

  // Private to SkeletonModel. This represent the max sphere for all skins
  // around this bone
...
private:
  ...
  // The bkuped channelIds for each channel of the bone. -1 if not registered
  // (or no tracks in animationSet).
  sint _PosChannelId;
...
inline void CBone::forceAnimate(CChannelMixer &chanMixer) {
  sint ids[] = {_PosChannelId, _RotEulerChannelId, _RotQuatChannelId,
                _ScaleChannelId, _PivotChannelId};

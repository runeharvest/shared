...
public:
   ...
   *	WARNING: the _GenerateShadowCasters list of shadow caster (added with
   *addShadowCaster() / selectShadowMapsToGenerate()) must have valid Ptrs. =>
   *Cannot leave this list non empty at end of CScene::render() because some
   *CTransform may be deleted between render()
   */
  void clearAllShadowCasters();

  /// Add a ShadowCaster that influence the scene this pass.
  void addShadowCaster(CTransform *model);
  /** From List of ShadowCaster, select a sub - part (so called Loding ShadowMap
   *Casters) NB: Beware that this clear the currentlist of shadowMap to generate
   *	NB: model->setGeneratingShadowMap(true); are called for each model
   *selected
   */
  ...
  /** render ShadowMap with ShadowCasters. Generate ShadowMap only (with the
   *AuxDriver). NB: current driver Frustum, ViewMatrix kept their initial state
   *but ModelMatrix not.
   */
  void renderGenerate(CScene *scene);

  /** project ShadowMaps onto receivers. NB: renderGenerate() must have been
   *called before. NB: current driver Frustum, ViewMatrix kept their initial
   *state but ModelMatrix not. call clearShadowCaster
   */
  ...
  // @{
  /// Allocate a texture for a shadowMap. NB: owned by a smartPtr. nlassert
  /// powerof2
...
private:
  ...
  // The Casters added to this pass that will compute their shadowMap this
  // frame. cleared at render
  ...
  void computeShadowColors(CScene *scene, CTransform *sc, CRGBA &ambient,
                           CRGBA &diffuse);

  void fillBlackBorder(IDriver *drv, uint numPassText, uint numTextW,
                       uint numTextH, uint baseTextureSize);

  void setBlackQuad(uint index, sint x, sint y, sint w, sint h);

  void updateBlurTexture(IDriver &drv, uint w, uint h);
  void copyScreenToBlurTexture(IDriver *drv, uint numPassText, uint numTextW,
                               uint numTextH, uint baseTextureSize);
  void applyFakeGaussianBlur(IDriver *drv, uint numPassText, uint numTextW,
                             uint numTextH, uint baseTextureSize,
                             uint blurSource);

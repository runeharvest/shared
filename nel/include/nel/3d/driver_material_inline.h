...
inline void CMaterial::setLighting(bool active, NLMISC::CRGBA emissive,
                                   NLMISC::CRGBA ambient, NLMISC::CRGBA diffuse,
                                   NLMISC::CRGBA specular, float shininess) {
  if (active) {
...
// ***************************************************************************
inline void CMaterial::texEnvArg0RGB(uint stage, TTexSource src,
                                     TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
...
// ***************************************************************************
inline void CMaterial::texEnvArg1RGB(uint stage, TTexSource src,
                                     TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
...
// ***************************************************************************
inline void CMaterial::texEnvArg2RGB(uint stage, TTexSource src,
                                     TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
...
// ***************************************************************************
inline void CMaterial::texEnvArg0Alpha(uint stage, TTexSource src,
                                       TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
...
// ***************************************************************************
inline void CMaterial::texEnvArg1Alpha(uint stage, TTexSource src,
                                       TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
  nlassert(oper == SrcAlpha || oper == InvSrcAlpha);
...
// ***************************************************************************
inline void CMaterial::texEnvArg2Alpha(uint stage, TTexSource src,
                                       TTexOperand oper) {
  nlassert(_ShaderType == CMaterial::Normal);
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
  nlassert(oper == SrcAlpha || oper == InvSrcAlpha);
...
inline void CMaterial::setUserColor(CRGBA userColor) {
  ...
  // setup stage 0 constant color (don't use texConstantColor() because of
  // assert).
...
inline CRGBA CMaterial::getUserColor() const {
  nlassert(_ShaderType == CMaterial::UserColor);
  // setup stage 0 constant color (don't use getTexConstantColor() because of
  // assert).
...
inline void CMaterial::enableUserTexMat(uint stage, bool enabled /*= true*/) {
  ...
  if (enabled) {
    if (!(_Flags &
          IDRV_MAT_USER_TEX_MAT_ALL)) // not usr tex mat setupped before?
    {
    ...
    if (!(_Flags & IDRV_MAT_USER_TEX_MAT_ALL))
...
inline void CMaterial::setTexCoordGenMode(uint stage, TTexCoordGenMode mode) {
  if (stage >= IDRV_MAT_MAXTEXTURES)
    return;
  _TexCoordGenMode &=
      ~(IDRV_MAT_TEX_GEN_MASK << (stage * IDRV_MAT_TEX_GEN_SHIFT));
  _TexCoordGenMode |=
      ((mode & IDRV_MAT_TEX_GEN_MASK) << (stage * IDRV_MAT_TEX_GEN_SHIFT));
  _Touched |= IDRV_TOUCHED_TEXGEN;
}

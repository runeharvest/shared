...
public:
  ...
  struct CMode {
    ...
    CMode(uint16 w, uint16 h, uint8 d, bool windowed = true, uint frequency = 0,
          sint8 aa = -1, const std::string &displayDevice = std::string()) {
...
public:
  ...
  /**
   * create the window. call activate(). Return true if mode activated, false if
   * it failed. \param show show or hide the window in window mode.
   */
  virtual bool setDisplay(const CMode &mode, bool show = true,
                          bool resizeable = true) = 0;
  virtual bool setDisplay(nlWindow wnd, const CMode &mode, bool show = true,
                          bool resizeable = true) = 0;
  ...
  /* Pass in dialog box mode. After having called this method, you can use
   *normal GUI. In fullscreen under direct3d, the main 3d window is minimized.
   *
   * \code
  ...
  /* Leave the dialog box mode. After having called this method, you can't use
   * normal GUI anymore. In fullscreen under direct3d, the main 3d window is
   * maximized.
   */
  virtual void endDialogMode() = 0;

  /// Release the window. All components are released (Texture, materials,
  /// scene, textcontexts).
  ...
  /// $ fog parameters. fog must enabled to see result. start and end are in
  /// [0,1] range.
  ...
  /// Create a new AnimationSet, load it from a file. Use CPath to search the
  /// animation set.  exception EPathNotFound if not found.
  virtual UAnimationSet *
  createAnimationSet(const std::string &animationSetFile) = 0;
  /// Delete a AnimationSet. NB: actually, this animation set is internally
  /// deleted only when no more UPlayList use it.
  ...
  /// Set a texture of specified size or of the size of the window as render
  /// target
  virtual void beginDefaultRenderTarget(uint32 width = 0,
                                        uint32 height = 0) = 0;
  ...
  createTextContext(const std::string fontFileName,
                    const std::string fontExFileName = "") = 0;
  ...
  /** Create a new texture file, searching in CPath. NB: by default a
   *textureFile created with this method has a setAllowDegradation() at false.
   * \param file filename, local to CPath paths.
   */
  virtual UTextureFile *createTextureFile(const std::string &file) = 0;
  /// Delete a texture file. This one will be really deleted in memory when no
  /// material point to it.
  virtual void deleteTextureFile(UTextureFile *textfile) = 0;
  /// Create a new Raw texture, to be filled by user.
  virtual UTextureMem *
  createTextureMem(uint width, uint height,
                   CBitmap::TType texType = CBitmap::RGBA) = 0;
  /// Delete a Raw texture. This one will be really deleted in memory when no
  /// material point to it.
  ...
  /** UScene ignore those function (use camera parameters instead, and UScene
   *viewport), and do not disturb this active Matrix context. (after a scene
   *rendering, the Matrix context for this interface is restored). Remarks are
   *nearly same for UTextContext, except for UTextContext::render3D() (see doc):
   *		- UTextContext use the setuped viewport of UDriver Matrix
   *context.
   *		- UTextContext use its own Matrix2D setup (own Frustum and own
   *ViewAMtrix/ ModelMatrix).
   *		- UTextContext restore ALL the matrix context, after any
   *rendering function.
   */
  // @{
  ...
  /** Set the active ModelMatrix for rendering. NB: UScene ignore this function
   * (use camera parameters instead).
   */
   ...
   * ModelMatrix is setup to identity. ViewMatrix is setup so that (x,y) of
   * vectors maps to x,y screen.
   */
  virtual void setMatrixMode2D(const CFrustum &frust) = 0;
  /// Tool function: same as setMatrixMode2D(), using a
  /// CFrustum(0,1,0,1,-1,1,false).
  void setMatrixMode2D11();
  /// Tool function: same as setMatrixMode2D(), using a
  /// CFrustum(0,4/3,0,1,-1,1,false).
  void setMatrixMode2D43();
  /** Tool function: Setup frustum/viewmatrix/modelmatrix for 3D, using
   * parameters of a UCamera. ModelMatrix setuped to identity. ViewMatrix
   * setuped to the inverse of camera 's LocalMatrix. Frustum setuped to UCamera
   * frustum.
   */
   ...
   * of vertices coordinates. Default range is [0, 1]. NB : znear should be
   * different from zfar or an assertion is raised
   */
  ...
  /// \name Interface 2D/3D.
  /** All of those render primitives are unlit! You must use UScene to render
   * lighted meshes. NB: If you set a texture to your material, the primitives
   * are textured, even if no Uvs are provided. \n NB: All rendering are done in
   * current viewport / current matrix context.
   */
  // @{
  ...
  virtual void drawTriangle(const NLMISC::CTriangleColor &tri,
                            UMaterial &mat) = 0;
  ...
  virtual void drawTriangle(const NLMISC::CTriangleColorUV &tri,
                            UMaterial &mat) = 0;
  ...
  virtual void drawQuads(const std::vector<NLMISC::CQuadColorUV> &quad,
                         UMaterial &mat) = 0;
  virtual void drawQuads(const std::vector<NLMISC::CQuadColorUV2> &quad,
                         UMaterial &mat) = 0;
  virtual void drawTriangles(const std::vector<NLMISC::CTriangleColorUV> &tris,
                             UMaterial &mat) = 0;
  virtual void drawQuads(const NLMISC::CQuadColorUV *qs, uint32 nbq,
                         UMaterial &mat) = 0;
  virtual void drawQuads(const NLMISC::CQuadColorUV2 *quads, uint32 nbQuads,
                         UMaterial &mat) = 0;
  virtual void drawTriangles(const NLMISC::CTriangleColorUV *tris,
                             uint32 nbTris, UMaterial &mat) = 0;

  // @}

  /// \name Tools for Interface 2D.
  /** For all those function, setMatrixMode2D*() should have been called (else
   * strange results!!).
   */
  // @{

  /// Draw a bitmap 2D. Warning: this is slow...
  virtual void drawBitmap(float x, float y, float width, float height,
                          class UTexture &texture, bool blend = true,
                          CRGBA col = CRGBA(255, 255, 255, 255)) = 0;
  ...
  virtual void drawTriangle(float x0, float y0, float x1, float y1, float x2,
                            float y2, CRGBA col) = 0;
  /// Draw a Quad in 2D. Warning: this is slow...
  virtual void drawQuad(float x0, float y0, float x1, float y1, CRGBA col) = 0;
  /// Draw a Quad in 2D. Warning: this is slow...
  virtual void drawQuad(float xcenter, float ycenter, float radius,
                        CRGBA col) = 0;
  /// Draw a Quad in 2D. Warning: this is slow...
  virtual void drawWiredQuad(float x0, float y0, float x1, float y1,
                             CRGBA col) = 0;
  /// Draw a Quad in 2D. Warning: this is slow...
  virtual void drawWiredQuad(float xcenter, float ycenter, float radius,
                             CRGBA col) = 0;
  ...
  /// Get the number of texture stage available, for multitexturing (Normal
  ...
  /** Return the amount of AGP memory allocated by initVertexArrayRange() to
   * store vertices.
   */
  virtual uint32 getAvailableVertexAGPMemory() = 0;

  /** Return the amount of video memory allocated by initVertexArrayRange() to
   * store vertices.
   */
   ...
   *to be DXTC5 compressed. Default is false. NB: this is done only on
   *TextureFile, with format Automatic
   */
   ...
   *	No-Op if already profiling
   *	NB: The results are the Locks in Chronogical time (since last
   *swapBuffers). Since multiple frame are summed, an "*" is marked againts the
   *VBHard name to show if it was not always this one (ptr test and not name
   *test) in the chronogical order. NB: if the driver does not support VBHard or
   *VBHard profiling (like ATI VBHard), result is empty. NB: ???? string is
  ...
  /** If the driver support it, stop profile index buffer locks, and "print"
   *result No-Op if already profiling NB: The results are the Locks in
   *Chronogical time (since last swapBuffers). Since multiple frame are summed,
   *an "*" is marked againts the index buffer name to show if it was not always
   *this one (ptr test and not name test) in the chronogical order. NB: if the
   *driver does not support index buffer profiling, result is empty. NB: ????
   *string is displayed if the index buffer has no name or if was just deleted.
   */
   ...
   *	\baseLevel When the texture is first added, it is loaded skipping the
   *baseLevel first mipmap \maxLevel During time, furhter mipmap are loaded,
   *according to instance position etc... maxLevel tells where to stop. If 0,
   *the texture will finally be entirely uploaded. Default is 3,1.
   */
  ...
  /// Setup max texture HLS Coloring per update() call (in bytes). Default to
  /// 20K.
  virtual void setupMaxHLSColoringPerFrame(uint maxCol) = 0;
  /** update the manager. New loaded texture are uploaded. Instances are updated
   *to know if all their pending textures have been uploaded.
   */
  virtual void updateAsyncTexture() = 0;

  /// get the async texture Size asked (ie maybe bigger than
  /// MaxTotalTextureSize).
  ...
  /// \name Water envmap
  // @{
  // Create a new water envmap. Such an envmap must then be set in a UScene for
  // the water surfaces of that scene to use it
...
public:
  /**
   *	This is the static function which build a UDriver, the root for all 3D
   *functions.
   */
  static UDriver *createDriver(uintptr_t windowIcon = 0, bool direct3d = false,
                               emptyProc exitFunc = 0);
  static UDriver *createDriver(uintptr_t windowIcon, TDriver driver,
                               emptyProc exitFunc = 0);

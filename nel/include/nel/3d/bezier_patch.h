...
public:
  ...
  CVector Vertices[4];
  /// The tangents ab, ba, bc, cb, cd, dc, da, ad. NB: tangents are points, not
  /// vectors.
  CVector Tangents[8];
...
public:
  ...
  /// complete computation of Vertices, Tangents, and Interiors, providing the
  ...
  /// \name Subdivision.
  // @{
  /** Subdivide the bezier patch in 2 bezier patches along s, at s (beetween
   * [0,1]). NB: left goes from 0 to s, right goes from s to 1.
   */
  void subdivideS(CBezierPatch &left, CBezierPatch &right,
                  float s = 0.5f) const;
  /** Subdivide the bezier patch in 2 bezier patches along t, at t (beetween
   * [0,1]). NB: top goes from 0 to t, bottom goes from t to 1.
   */
  void subdivideT(CBezierPatch &top, CBezierPatch &bottom,
                  float t = 0.5f) const;
  // @}

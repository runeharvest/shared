...
// ***************************************************************************
/**
 * Base interface for manipulating Movable Objects: camera, bones, lights,
 * instances etc... By default Transformmode is RotQuat. \author Lionel
...
public:
  ...
  void setRotEuler(float rx, float ry, float rz) {
    setRotEuler(CVector(rx, ry, rz));
  }
  /// Work only in RotQuat mode (nlassert).
  void setRotQuat(const CQuat &quat);
  /** Work only in RotQuat mode (nlassert).
   * Build a quaternion from a forward direction (a J vector). there is no
   * roll... jdir do not need to be noramlized.
   */
  void setRotQuat(const CVector &jdir);
  /** Work only in RotQuat mode (nlassert).
   * Build a quaternion from a forward direction (a J vector). the roll is
   * determined with help of the vector up vup... vectors do not need to be
   * noramlized.
   */

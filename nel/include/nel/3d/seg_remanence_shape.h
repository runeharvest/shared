 ...
 *
 * NB this doesn't support material with animated texture matrix (texture matrix
 * is used internally)
 *
...
public:
   ...
   * NB: ideally the bbox should be computed per model, but practically, a fixed
   * bbox suffice.
   */
  void setBBox(const NLMISC::CAABBox &bbox);

  /** Enable texture shifting on the u coordinates at the start of the
   * animation, so that the texture will 'unroll' until it is completly visible
  ...
  /** When stop() is called, the fx doesn't disappear, it must rollup (a texture
   * shifting is performed). This set the speed of rollup. 1 means it takes
   * sliceTime * numSlice to unroll 0.5 means it takes  2 * sliceTime * numSlice
   * to unroll and so on NB ratio must be > 0
   */

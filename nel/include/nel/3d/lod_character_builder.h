 ...
 *	Use it by first calling setShape(), then add animations to it, and
 *finally get result with getLodShape \author Lionel Berenguier \author Nevrax
 *France \date 2002
 */
...
public:
   ...
   *	NB: nlWarnings may occurs if don't find bones used by lodBuild, in the
   *skeletonShape. In this case, root bone 0 of the skeleton shape is used.
   *
   *	\param name is the name to give to the shape
   *	\param skeletonShape the skeleton used for animation. Ptr is handled by
   *the builder as a smartPtr. \param lodBuild the mesh information. Ptr is hold
   *by the builder, but not deleted
   */
  void setShape(const std::string &name, CSkeletonShape *skeletonShape,
                CLodCharacterShapeBuild *lodBuild);

  /** add an animation to the process
   *	\param animName is the name of the animation, used as a key to receover
   *animations in the CLodCharacterShape built \param animation is the animation
   *to bake/over sample. NB: the animation is deleted at the end of addAnim
   *	\param frameRate is the desired overSampling rate. eg: 20 frame per
   *second.
   */

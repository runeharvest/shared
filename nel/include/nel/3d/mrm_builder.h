...
public:
   ...
   */
  void compileMRM(const CMesh::CMeshBuild &mbuild,
                  std::vector<CMesh::CMeshBuild *> &bsList,
                  const CMRMParameters &params,
                  CMeshMRMGeom::CMeshBuildMRM &mrmMesh, uint numMaxMaterial);
...
private:
  ...
  bool _Skinned;
  /// If the current build is skinned, control the quality of the skinning
  /// redcution.
  ...
  float getDeltaFaceNormals(sint numvertex); // return a positive value of
                                             // Sum(|DeltaNormals|) / NNormals.
  ...
  // @{
  bool faceShareWedges(CMRMFaceBuild *face, sint attribId, sint numVertex1,
                       sint numVertex2);
  ...
  CMesh::CSkinWeight collapseSkinWeight(const CMesh::CSkinWeight &sw1,
  ...
  /// this is the root call to compute a single lodMesh and the coarserMesh from
  /// a baseMesh.
  void makeFromMesh(const CMRMMesh &baseMesh, CMRMMeshGeom &lodMesh,
                    CMRMMesh &coarserMesh, sint nWantedFaces);
  // @}

  void computeBsVerticesAttributes(std::vector<CMRMMesh> &srcBsMeshs,
  ...
  bool buildMRMSewingMeshes(const CMesh::CMeshBuild &mbuild, uint nWantedLods,
                            uint divisor);
...
private:
   ...
   */
  void buildBlendShapes(CMRMMesh &baseMesh,
                        std::vector<CMesh::CMeshBuild *> &bsList,
                        uint32 VertexFlags);

  /** build all LODs from a baseMesh. NB: the coarsestMesh is stored in
   * lodMeshs[0], and has no geomorph info since it is the coarsest mesh.
   * nWantedLods are created (including the coarsestMesh). \param lodMeshs array
   * created by the function (size of nWantedlods). \param nWantedLods number of
   * LODs wanted. \param divisor the coarsestMesh will have
   * baseMesh.Faces.size()/divisor  faces.
   */
  void buildAllLods(const CMRMMesh &baseMesh,
                    std::vector<CMRMMeshGeom> &lodMeshs, uint nWantedLods = 10,
                    uint divisor = 50);

  /** given a list of LODs, compress/reorganize data, and store in finalMRM
   * mesh.
   *
   */
  void buildFinalMRM(std::vector<CMRMMeshGeom> &lodMeshs,
...
private:
  ...
  sint findInsertAttributeInBaseMesh(CMRMMesh &baseMesh, sint attId,
                                     sint vertexId, const CVectorH &att);
  sint findInsertNormalInBaseMesh(CMRMMesh &baseMesh, sint attId, sint vertexId,
                                  const CVector &normal);
  sint findInsertColorInBaseMesh(CMRMMesh &baseMesh, sint attId, sint vertexId,
                                 CRGBA col);
  sint findInsertUvwInBaseMesh(CMRMMesh &baseMesh, sint attId, sint vertexId,
  ...
  /** from a final MRM Mesh representation, compute a CMeshBuildMRM. This is the
   *last stage of the algo. \param vbFlags the vertex format returned by earlier
   *call too buildMrmBaseMesh(). \param nbMats the number of materials of
   *original MeshBuild.
   */
  void buildMeshBuildMrm(const CMRMMeshFinal &finalMRM,
                         CMeshMRMGeom::CMeshBuildMRM &mbuild, uint32 vbFlags,
                         uint32 nbMats, const CMesh::CMeshBuild &mb);

...
public:
  ...
  /** build. NB: fails if too much vertices/triangles (>=65536) or if 0 vertices
   * / triangles \param vbForShadowRender: a RefPtr is kept on this VB for
   *shadow rendering
   */
  ...
  float getCameraCollision(const NLMISC::CMatrix &instanceMatrix,
                           class CCameraCol &camCol);

  /// compute the world bbox of an instance
  NLMISC::CAABBox computeWorldBBox(const NLMISC::CMatrix &instanceMatrix);

  /// receive a shadowMap. render in driver the triangles that intersect the
  /// shadow
  void receiveShadowMap(const NLMISC::CMatrix &instanceMatrix,
...
private:
  ...
  public:
    ...
    // return the list of elements intersected. NB: the vector is enlarged to
    // max, but real selection size is in the  return value
  ...
  private:
    ...
    // For Fast selection
    uint32 _ItSession;
    // For each element the session id. if same than _ItSession, then already
    // inserted

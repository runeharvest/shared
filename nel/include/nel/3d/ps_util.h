...
public:
  ...
  /// Draw a sphere
  static void displaySphere(NL3D::IDriver &driver, float radius,
                            const NLMISC::CVector &center, uint nbSubdiv = 4,
                            NLMISC::CRGBA color = NLMISC::CRGBA::White);

  /** Draw a disc (not filled)
   *  \param mat : a matrix, whose K vector is normal to the plane containing
   * the disc
   */
  static void displayDisc(NL3D::IDriver &driver, float radius,
                          const NLMISC::CVector &center,
                          const NLMISC::CMatrix &mat, uint nbSubdiv = 32,
                          NLMISC::CRGBA color = NLMISC::CRGBA::White);

  /** draw a cylinder (not filled)
   *  \param dim dimension of the cylinder along each axis, packed in a vector
   */
  static void displayCylinder(NL3D::IDriver &driver,
                              const NLMISC::CVector &center,
                              const NLMISC::CMatrix &mat,
                              const NLMISC::CVector &dim, uint nbSubdiv = 32,
                              NLMISC::CRGBA color = NLMISC::CRGBA::White);

  /// display a 3d quad in wireline, by using the 4 gicen corners
  static void display3DQuad(NL3D::IDriver &driver, const NLMISC::CVector &c1,
  ...
  /// display a basis using the given matrix. The model matrix must be restored
  /// after this call
  static void displayBasis(NL3D::IDriver *driver,
  ...
  static void displayArrow(NL3D::IDriver *driver, const NLMISC::CVector &start,
                           const NLMISC::CVector &v, float size,
                           NLMISC::CRGBA col1, NLMISC::CRGBA col2);

  /// display a string at the given world position. The model matrix must be
  /// restored after this call

  static void print(NL3D::IDriver *driver, const std::string &text,
                    CFontGenerator &fg, CFontManager &fm,
                    const NLMISC::CVector &pos, float size,
                    NLMISC::CRGBA col = NLMISC::CRGBA::White);

  /** build a basis from a vector using Schmidt orthogonalization method
   *  \param v K axis in the resulting basis
   *  \param dest The matrix containing the result. Only the rotation part is
   * modified
   */
  static void buildSchmidtBasis(const NLMISC::CVector &v,
                                NLMISC::CMatrix &dest);

  /** get a cosine from the fast cosine table (which must be have initialised
   * with initFastCosNSinTable). 256 <=> 2 Pi
   */
...
inline float CPSUtil::getInterpolatedNoise(const NLMISC::CVector &pos) {
  uint x = (uint)pos.x, y = (uint)pos.y, z = (uint)pos.z;

  // we want to avoid costly ctor call there...
  float fx = pos.x - x, fy = pos.y - y, fz = pos.z - z;
  ...
  const float v0 = getPerlinNoise(x, y, z), v1 = getPerlinNoise(x + 1, y, z),
  ...
  const float h1 = fx * v1 + (1.f - fx) * v0, h2 = fx * v3 + (1.f - fx) * v2,
              h3 = fx * v5 + (1.f - fx) * v4, h4 = fx * v7 + (1.f - fx) * v6;

  const float c1 = fy * h2 + (1.f - fy) * h1, c2 = fy * h4 + (1.f - fy) * h3;

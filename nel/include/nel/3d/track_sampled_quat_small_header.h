...
// ***************************************************************************
/** For minimum CTrackSampledQuat Header Overhead (44 bytes here). This is a
 *compressed version of CTrackSampledQuat. It works only with 1 TimeBlock (95%
 *of animations) and assumes that most of CTrackSampledCommon data (loop etc...)
 *is the same for all tracks of an animation The final size of this class is: 4
 *(vtable) 4		ptr on CTrackSamplePack 1+1+2	offset in
 *CTrackSamplePack
 */
class CTrackSampledQuatSmallHeader : public ITrack {
public:
  /// Constructor
  CTrackSampledQuatSmallHeader(CTrackSamplePack *pack, uint8 headerIndex,
                               uint8 numKeys, uint16 keyIndex);
...
protected:
  ...
  uint8 _NumKeys;
  // The index of starting key in _TrackSamplePack->Times and in
  // _TrackSamplePack->Keys
  uint16 _KeyIndex;

  // same code than CTrackSampledCommon
  enum TEvalType { EvalDiscard, EvalKey0, EvalInterpolate };
  TEvalType evalTime(const TAnimationTime &date, uint &keyId0, uint &keyId1,
                     float &interpValue);
};

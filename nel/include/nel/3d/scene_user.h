...
protected:
  ...
  struct CWaitingIG {
    ...
    CWaitingIG(UInstanceGroup **callerPtr, const NLMISC::CVector &pos,
               const NLMISC::CQuat &rot, uint selectedTexture,
               IAsyncLoadCallback *pCB)
        : IGToLoad(NULL), CallerPtr(callerPtr), Pos(pos), Rot(rot),
          SelectedTexture(selectedTexture), Callback(pCB) {}
  };
  //
...
public:
  ...
  virtual void renderPart(TRenderPart rp, bool doHrcPass = true,
  ...
  /*
     ...
     &instanceGroup); virtual	void			deleteInstanceGroup
     (UInstanceGroup	*group);
  */
  ...
  virtual void setLayersRenderingOrder(bool directOrder = true) {
    _Scene.setLayersRenderingOrder(directOrder);
  }
  virtual bool getLayersRenderingOrder() const {
    return _Scene.getLayersRenderingOrder();
  }
  //@}

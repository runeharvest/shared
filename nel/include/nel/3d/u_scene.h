...
public:
  ...
  /// Because the async loader set the pointer of the user after the whole
  /// loading (ig+shapes) what is passed through this method is the value that
  /// will be affected
 ...
 * A UScene create instances, camera lights etc...  Those objects are not
 * sharable through different scenes. You cannot use a camera create with
 * Scene1, for viewing Scene2.
 *
 * The scene is created with a default camera that you can retrieve with
 * getCam().
 *
...
public:
  ...
  /// \name Render
  //@{
  /** Render the scene from the CurrentCamera view (setCam()), and the current
   *Viewport. nlError if no current camera. NB: no Driver clear buffers (color
   *or ZBuffer) are done.... \n NB: The UDriver Matrix/Viewport context for
   *2D/3D interface is restored after this render. NB: nlerror if the current
   *camera has been deleted. NB: the UDriver Light setup (see
   *UDriver::setLight() / UDriver::setAmbientColor()) is modified. At the exit
   *of render(), all UDriver lights are disabled.
   */
  virtual void render(bool updateWaitingInstances = true,
                      bool restoreMatrixContextAfterRender = true) = 0;

  /** Begin Part Rendering
   *	During beginPartRender()/endPartRender(), you can ask other scene to
   *render their part, but you should avoid to create models or modify the scene
   *(not fully tested) WARNING: assert-crash if between a
   *beginPartRender()/endPartRender()
   */
  virtual void beginPartRender() = 0;

  /** Render a part (see render() for what it does)
   *	beginPartRender() must have been called
   * \param renderPart a combination of UScene::TRenderPart flags, allow to
   *choose which part of the scene must be rendered \param doHrcPass set it to
   *false to indicate that the CHrcTrav have not to be traversed. Useful to
   *optimize if you know that NONE of your models have moved (a good example is
   *a shoot of the scene from different cameras). \param doTrav set to false
   *when processing a second frame for stereo rending to avoid unnecessary
   *traversals. WARNING: always must begin rendering with at least
   *UScene::RenderOpaque, else shadows won't work WARNING: assert-crash if a
   *part in 'rp' has already been rendered since the last beginPartRender()
   */
  virtual void renderPart(UScene::TRenderPart rp, bool doHrcPass = true,
                          bool doTrav = true, bool keepTrav = false) = 0;

  /** End Part Rendering (commit model creation and deletion that were asked
   * during rendering)
   */
  virtual void endPartRender(bool updateWaitingInstances = true,
                             bool restoreMatrixContextAfterRender = true,
                             bool keepTrav = false) = 0;

  /** Update waiting instances and igs that are loaded asynchronously
   * NB: this is called by render()
   * \param systemTimeEllapsed : the time between 2 calls to
   * updateWaitingInstances, in seconds
   */
  ...
  /// \name Camera/Viewport.
  //@{
  /// Set the current camera for this scene. nlError if NULL, or if the camera
  /// was not created by this UScene.
   ...
   *	(clip through portals, cluster system hierarchy...), to find where could
   *lies the camera at End point. \param endPos may be modified to ensure that
   *it lies in a cluster of the found cluster system \return the found cluster
   *System
   */
  virtual UInstanceGroup *
  findCameraClusterSystemFromRay(UInstanceGroup *startClusterSystem,
                                 const NLMISC::CVector &startPos,
                                 NLMISC::CVector &endPos) = 0;
  //@}
  ...
  /** Create an instance of the shape "shapename". If not present, try to load
   * "shapename" via CPath. If fails (file not found), return NULL. nlerror if
   * the file is not a mesh file.
   */
   ...
   */
  virtual void createInstanceAsync(const std::string &shapeName,
  ...
  /** create an instance group asynchronously, and add it to a scene
   * asynchronously once it has been loaded. The instance groups that must be
   * added to the scene are checked at each render. The pIG pointer is filled
   * once the instance group has been loaded and add to the scene
   */
  virtual void createInstanceGroupAndAddToSceneAsync(
      const std::string &instanceGroup, UInstanceGroup **pIG,
      const NLMISC::CVector &pos, const NLMISC::CQuat &rot,
      uint selectedTexture, IAsyncLoadCallback *pCB = NULL) = 0;
  ...
  /** Create a skeleton instance of the skeleton shape "shapename". If not
   * present, try to load "shapename" via CPath. If fails (file not found),
   * return NULL. nlerror if the file is not a skeleton shape file.
   */
  ...
  /// Create a landscape. There could be many Landscape per Scene (Driver? :).
  /// But never tested :).
  ...
  // virtual	UInstanceGroup	*createInstanceGroup(const std::string
  // &instanceGroup) =0;
  /// Delete an instance group from this scene.
  // virtual	void			deleteInstanceGroup(UInstanceGroup
  // *group) =0;
  ...
  /** The mode of polygon balancing. NB: this apply to All LoadBalancingGroups,
   *but the "Default" group which is always considered as PolygonBalancingOff
   * PolygonBalancingOff => Models will be rendered with the number of faces
   *they want to render. PolygonBalancingOn  => Models will be rendered with the
   *number of faces the LoadBalancing want. PolygonBalancingClamp => Same as
   *PolygonBalancingOn, but factor <= 1, ie models won't be rendered with more
   *face they want to render.
   */
  enum TPolygonBalancingMode {
    PolygonBalancingOff = 0,
    PolygonBalancingOn,
   ...
   */
  virtual void setGroupLoadMaxPolygon(const std::string &group,
                                      uint nFaces) = 0;
  /** Get the number of faces wanted for a LoadBlancingGroup.
   *	The Group is created if did not exist.
   */
  virtual uint getGroupLoadMaxPolygon(const std::string &group) = 0;
  /** Get the last face count asked from the instances before reduction. only
   *for the given group return 0 if the Group does not exist.
   */
  virtual float getGroupNbFaceAsked(const std::string &group) const = 0;

  /** LoadBalancing for CLod and Skeletons.
   *	Setup the max number of skeletons displayed in std way (ie not CLod).
   *Default is 20.
   */
  ...
  /**
   *	Each coarseMesh lighting will be updated every "period" frame. clamped
   *to 1,255
   */
  ...
  /** Enable Scene Lighting system. For backward compatibility, false by
   *default. If false, all objects will take last driver 's light setup
   */
  ...
  /** setup the max number of point light that can influence a model. NB:
   *clamped by NL3D_MAX_LIGHT_CONTRIBUTION Default is 3. NB: the sun
  ...
  /** Advanced. When a model is influenced by more light than allowed, or when
   *it reach the limits of the light (attenuationEnd), the light can be darkened
   *according to some threshold. The resultLightColor begin to fade when
   *distModelToLight== attEnd- threshold*(attEnd-attBegin). when
   *distModelToLight== 0, resultLightColor==Black. By default, this value is
   *0.1f. Setting higher values will smooth transition but will generally darken
   *the global effects of lights. NB: clamp(value, 0, 1);
   */
  ...
  /// Set the current windDirection for all the scene. dir.z set to 0 and vector
  /// normalized.
  ...
  /** NB: for an historic reason, CLod interface is in UScene, BUT THERE IS ONLY
   * ONE LOD MANAGER PER UDriver!
   */
  //@{
   ...
   *	NB: a vector of ShapeBank is maintained internally, hence, not so many
   *shapeBank should be created at same Time. throw exception if failed to load
   *the file \param fileName is a .clodbank file, to be loaded. CPath::lookup is
   *used. \return	id of the shape Bank.
   */
   ...
   */
  virtual sint32 getCLodAnimIdByName(uint32 shapeId,
                                     const std::string &name) const = 0;

  //@}

  /** \name Flare contexts
   * The flare objects are designed to work with a single scene, because they
   * simulate 'retinia persistence' based on the visibility in the current
   * scene. Several context allow to deals with a flare rendered from several
   * points of views. There's a limited number of contexts
   */
  // @{
  // Get the max number of contexts
  virtual uint getNumFlareContexts() const = 0;
  // Set the current context for flares. context must be < to
  // MaxNumFlareContexts
  ...
  enum TRenderFilter {
    ...
    FilterCoarseMesh = 0x00001000, // Disable render of CoarseMesh (but not
                                   // management through Meshs!)
  ...
  /** Enable or disable some Models to be rendered (for profile). Default is ALL
   *enabled NB: filtering is made at clip pass. Hence, much of the render
   *processing is skipped (animDetail, light, lod, render)
   */
  ...
  /// The scene may have one special VisualCollisionManager which can manage
  /// some shadow receiving
  virtual void
  setVisualCollisionManagerForShadow(UVisualCollisionManager *vcm) = 0;
  // @}
  ...
  virtual void
  setLandscapePolyDrawingCallback(ILandscapePolyDrawingCallback *lpd) = 0;
  virtual ILandscapePolyDrawingCallback *
  getLandscapePolyDrawingCallback() const = 0;

  /** Setup transparency sorting
   * \param maxPriority Defines the valid range for priority in the [0, n]
   * interval. By default, there's no priority sorting (0 -> single priority,
   * 255 -> 256 possible priorities) Objects with higher priority are displayed
   * before any other object with lower priority, whatever their distance is.
   * \param NbDistanceEntries Defines the granularity for distance sorting. A
   * value of N with a view distance of D meters means that the sorting accuracy
   * will be of D / N meters at worst (when visible objects occupy the whole
   * distance range) NB : The memory allocated is a multiple of NumPriority *
   * NbDistanceEntries * 2 (2 if because of water ordering)
   */
  virtual void setupTransparencySorting(uint8 maxPriority = 0,
                                        uint NbDistanceEntries = 1024) = 0;

  /// \name Water envmaps
  // @{
  /** Set a water envmap to be used with water surfaces in that scene. Water
   * envmap may be shared across several scene. The envmap should have been
   * created from the same UDriver interface than the scene
   */
  ...
  /** Update water envmaps. Water textures that need to be updated includes
   * UWaterEnvMap textures & Day/Night textures (as defined in the water
   * material). Should be called at the beginning of the frame before anything
   * is rendered.
   */

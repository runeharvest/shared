...
#define NL3D_MESH_MRM_SKINNED_VERTEX_FORMAT                                    \
  (CVertexBuffer::PositionFlag | CVertexBuffer::NormalFlag |                   \
   CVertexBuffer::TexCoord0Flag | CVertexBuffer::PaletteSkinFlag)
...
public:
   ...
   */
  void build(CMesh::CMeshBuild &m, uint numMaxMaterial,
   ...
   */
  void getRdrPassPrimitiveBlock(uint lodId, uint renderingPassIndex,
                                CIndexBuffer &block) const {
  ...
  virtual uint
  getNumRdrPassesForInstance(CMeshBaseInstance * /* inst */) const {
    return 0;
  }
  ...
  virtual void activeInstance(CMeshGeomRenderContext & /* rdrCtx */,
                              ...
                              float /* polygonCount */, void * /* vbDst */) {}
  virtual void renderPass(CMeshGeomRenderContext & /* rdrCtx */,
                          CMeshBaseInstance * /* inst */,
                          float /* polygonCount */, uint /* rdrPass */) {}
  ...
  sint renderSkinGroupGeom(CMeshMRMSkinnedInstance *mi, float alphaMRM,
  ...
  void renderSkinGroupSpecularRdrPass(CMeshMRMSkinnedInstance *mi,
                                      uint rdrPassId);
  // @}
  ...
  sint renderShadowSkinGeom(CMeshMRMSkinnedInstance *mi, uint remainingVertices,
                            uint8 *vbDest);
  void renderShadowSkinPrimitives(CMeshMRMSkinnedInstance *mi,
  ...
  bool intersectSkin(CMeshMRMSkinnedInstance *mi, const CMatrix &toRaySpace,
...
private:
  ...
  public:
    ...
    // Get num triangle
    uint getNumTriangle() const { return (uint)PBlock.size() / 3; }
  };
  ...
  public:
    ...
    struct CPackedVertex {
      ...
      inline void getWeight(float &dest, uint index) const {
        dest =
            (float)Weights[index] * (1.f / NL3D_MESH_MRM_SKINNED_WEIGHT_FACTOR);
      }
    ...
    // Build it
    void build(const CVertexBuffer &buffer,
               const std::vector<CMesh::CSkinWeight> &skinWeight);

    // Clear it
    void clear() { _PackedBuffer.clear(); }

    // Reset
    void contReset() { NLMISC::contReset(_PackedBuffer); }
    ...
    // Acces it
    uint getNumVertices() const { return (uint)_PackedBuffer.size(); }
...
private:
  ...
  void applyGeomorphPosNormalUV0Int(std::vector<CMRMWedgeGeom> &geoms,
   ...
   */
  void applyRawSkinWithNormal(CLod &lod, CRawSkinnedNormalCache &rawSkinLod,
  ...
  // SkinGroup
  void updateShiftedTriangleCache(CMeshMRMSkinnedInstance *mi, sint curLodId,
                                  uint baseVertex);
...
private:
  ...
  void updateRawSkinNormal(bool enabled, CMeshMRMSkinnedInstance *mi,
                           sint curLodId);
  /// Increment the refCount, so instances RawSkins are no longer valid
  void dirtMeshDataId();

  // ApplySkin method
  void applyArrayRawSkinNormal1(CRawVertexNormalSkinned1 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal2(CRawVertexNormalSkinned2 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal3(CRawVertexNormalSkinned3 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal4(CRawVertexNormalSkinned4 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
...
public:
  ...
  /** Return true if the CMesh::CMeshBuild is compatible with the
   * CMeshMRMSkinned. If not, build a CMeshMRM. You must test this before call
   * build().
   */
  static bool isCompatible(const CMesh::CMeshBuild &m);

  /** Build a mesh, replacing old. WARNING: This has a side effect of deleting
   * AnimatedMaterials. Call isCompatible() before this method. this is much
   * slower than CMesh::build(), because it computes the MRM. \param params
   ...
   *and a previously builded CMeshMRMSkinnedGeom. WARNING: This has a side
   *effect of deleting AnimatedMaterials. this is much slower than
   *CMesh::build(), because it computes the MRM. \param params parameters of the
   *MRM build process.
   */
  ...
  /// used by All LODs.
  void getVertexBuffer(CVertexBuffer &output) const {
    _MeshMRMGeom.getVertexBuffer(output);
  }

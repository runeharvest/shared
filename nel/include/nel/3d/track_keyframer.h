 ...
 *
 * The ITrack and animated value types MUST match else assertions will be
 * raised.
 *
...
public:
  ...
  /// return true if Range is locked to first/last key. use getBeginTime and
  /// getEndTime to get the effective begin/end range times...
  bool isRangeLocked() const { return _RangeLock; }

  /// rangeDelta is (length of effective Range) - (length of LastKey-FirstKey).
  ...
  /// set LoopMode. 2 mode only: "constant" (<=>false), and "loop" (<=> true).
  /// same mode for in and out...
  ...
  /// From ITrack.
  virtual const IAnimatedValue &eval(const TAnimationTime &inDate,
  ...
  /** From UTrackKeyframer, retrieve the keys that are in the given range [t1,
   * t2] of the track \param result a vector that will be cleared, and filled
   * with the date ofthe keys
   */
  void getKeysInRange(TAnimationTime t1, TAnimationTime t2,
...
protected:
  ...
  /// This is for Deriver compile(), because _RangeDelta (getRangeDelta()) is
  /// himself computed in compile().
  float getCompiledRangeDelta() { return _RangeDelta; }

  /**
   * Precalc keyframe runtime infos for interpolation (OODTime...). All keys
   * should be processed. This is called by eval when necessary. Deriver should
   * call ITrackKeyFramer::compile() first, to compile basic Key runtime info.
   */
   ...
   *
   * i is the keyframe with the bigger time value that is inferior or equal than
   * date.
   *
...
public:
  ...
  virtual void evalKey(const CKeyT *previous, const CKeyT *next,
                       ...
                       TAnimationTime /* dateNext */, TAnimationTime /* date */,
                       IAnimatedValue &result) {
    // Const key.
    if (previous)
      copyToValue(static_cast<CAnimatedValueNotBlendable<T> &>(result).Value,
                  previous->Value);
    else if (next)
      copyToValue(static_cast<CAnimatedValueNotBlendable<T> &>(result).Value,
                  next->Value);
...
public:
  ...
  virtual void evalKey(const CKeyT *previous, const CKeyT *next,
    ...
    // Const key.
    if (previous)
      copyToValue(static_cast<CAnimatedValueBlendable<T> &>(result).Value,
                  previous->Value);
    else if (next)
      copyToValue(static_cast<CAnimatedValueBlendable<T> &>(result).Value,
                  next->Value);
...
public:
  ...
  virtual void evalKey(const CKeyT *previous, const CKeyT *next,
                       ...
                       TAnimationTime /* dateNext */, TAnimationTime date,
                       IAnimatedValue &result) {
    CAnimatedValueBlendable<T> &resultVal =
        static_cast<CAnimatedValueBlendable<T> &>(result);
    ...
    if (previous && next) {
      ...
      // NB: in case of <CKeyInt,sint32> important that second terme is a float,
      // so copyToValue(sint32, float) is used.
      copyToValue(resultVal.Value, previous->Value * (1.f - (float)date) +
                                       next->Value * (float)date);
    } else {
      if (previous)
...
class CTrackKeyFramerLinear<CKeyQuat, CQuat>
    : public ITrackKeyFramer<CKeyQuat> {
public:
  /// From ITrackKeyFramer
  virtual void evalKey(const CKeyQuat *previous, const CKeyQuat *next,
                       ...
                       TAnimationTime /* dateNext */, TAnimationTime date,
                       IAnimatedValue &result) {
    CAnimatedValueBlendable<CQuat> &resultVal =
        static_cast<CAnimatedValueBlendable<CQuat> &>(result);
...
class CTrackKeyFramerLinear<CKeyRGBA, NLMISC::CRGBA>
    : public ITrackKeyFramer<CKeyRGBA> {
public:
  /// From ITrackKeyFramer
  virtual void evalKey(const CKeyRGBA *previous, const CKeyRGBA *next,
                       ...
                       TAnimationTime /* dateNext */, TAnimationTime date,
                       IAnimatedValue &result) {
    CAnimatedValueBlendable<NLMISC::CRGBA> &resultVal =
        static_cast<CAnimatedValueBlendable<NLMISC::CRGBA> &>(result);
    ...
    if (previous && next) {
      ...
      // blend.
      resultVal.Value.blendFromui(previous->Value, next->Value,
                                  (uint)(date * 256));
    } else {
      if (previous)
        resultVal.Value = previous->Value;
...
class CTrackKeyFramerConstString
    : public CTrackKeyFramerConstNotBlendable<CKeyString, std::string> {
public:
  NLMISC_DECLARE_CLASS(CTrackKeyFramerConstString);
  NL3D_TRACKKEYF_CHOOSE(ValString)
};
class CTrackKeyFramerConstBool
    : public CTrackKeyFramerConstNotBlendable<CKeyBool, bool> {
public:
...
class CTrackKeyFramerLinearFloat
    : public CTrackKeyFramerLinear<CKeyFloat, float> {
public:
...
class CTrackKeyFramerLinearVector
    : public CTrackKeyFramerLinear<CKeyVector, CVector> {
public:
  NLMISC_DECLARE_CLASS(CTrackKeyFramerLinearVector);
  NL3D_TRACKKEYF_CHOOSE(ValVector)
};
class CTrackKeyFramerLinearQuat
    : public CTrackKeyFramerLinear<CKeyQuat, CQuat> {
public:
...
class CTrackKeyFramerLinearRGBA
    : public CTrackKeyFramerLinear<CKeyRGBA, NLMISC::CRGBA> {
public:
...
class CTrackKeyFramerTCBVector
    : public CTrackKeyFramerTCB<CKeyTCBVector, CVector> {
public:
  NLMISC_DECLARE_CLASS(CTrackKeyFramerTCBVector);
  NL3D_TRACKKEYF_CHOOSE(ValVector)
};
class CTrackKeyFramerTCBQuat
    : public CTrackKeyFramerTCB<CKeyTCBQuat, NLMISC::CAngleAxis> {
public:
...
class CTrackKeyFramerTCBRGBA
    : public CTrackKeyFramerTCB<CKeyTCBVector, NLMISC::CRGBA> {
public:
...
class CTrackKeyFramerBezierFloat
    : public CTrackKeyFramerBezier<CKeyBezierFloat, float> {
public:

...
namespace NL3D {
    ...
    !defined(NL_DONT_USE_EXTERNAL_CODE)
...
// the following macros, that redefines new & delete for classes of ps, is now
// empty NB : When using NL_MEMORY, we solve the redefinition of new errors as
// follow :
//
// PS_FAST_OBJ_ALLOC
#define PS_FAST_OBJ_ALLOC
// Partial specialization tips for vectors. For non-windows version it just
// fallback on the default allocator To define a vector of type T, one must do :
// CPSVector<T>::V MyVector;
...
// current contiguous block allocator to be used by the particle system
// allocator. std::allocator is used if no such allocator is provided
...
public:
  pointer allocate(size_type n, const void *hint) {
    ...
    if (PSBlockAllocator) {
      result = (TBlocAllocPtr *)PSBlockAllocator->alloc(
          n * sizeof(T) + sizeof(TBlocAllocPtr *));
      *result = PSBlockAllocator; // mark as a block from block allocator
    } else {
      result = (TBlocAllocPtr *)_Alloc.allocate(n * sizeof(T) +
                                                sizeof(TBlocAllocPtr *));
  ...
  void deallocate(pointer p, size_type n) {
    ...
    if (*(TBlocAllocPtr *)realAddress) {
      ...
      (*(TBlocAllocPtr *)realAddress)
          ->free((void *)realAddress, n * sizeof(T) + sizeof(TBlocAllocPtr *));
    } else {
...
// STLPort 4.6.x and 5.0.x version of allocator : written by Neverborn
// Entertainment
...
// allocation of objects of ps (to be used inside base class declaration,
// replaces operator new & delete)
#define PS_FAST_OBJ_ALLOC                                                      \

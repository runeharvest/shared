...
/** A cube map functor should return a color from a vector (with each coordinate
 * ranging from [-1..1]
 */
 ...
 * \param luminanceOnly When set to true, a luminance cube map is build. The
 * luminance is taken from the alpha component of the color produced by the
 * functor. Warning : this isn't supported anywhere. \param shareName a prefix
 * for sharename. If not empty this allow each face of the cube map to be
 * sharable (a number is appended to the given string)
 */

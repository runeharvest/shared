...
#define NL3D_LANDSCAPE_VEGETABLE_MAX_AGP_VERTEX_MAX                            \
  (max(NL3D_LANDSCAPE_VEGETABLE_MAX_AGP_VERTEX_UNLIT,                          \
...
// if NL_DLM_TILE_RES is defined, DLM texture precision are at Tile precision
// (ie 2m) If not defined then suppose TessBlock coarser precision (4m)
#define NL_DLM_TILE_RES

// ***************************************************************************
class CLandscapeGlobals {
public:
  // LANDSCAPE RENDERING CONTEXT.  Landscape must setup it at the beginning at
  // refine()/render(). The current date of LandScape for refine only.
  ...
  // The center of the landscape (near CameraCenter), for better ZBuffer
  // precision.
  ...
  // The tiles are not subdivided above this limit (but because of enforced
  // splits). Default: 4 => 50cm.
  ...
  // PATCH GLOBAL INTERFACE.  patch must setup them at the beginning at
  // refine()/render(). NO!!! REMIND: can't have any patch global, since a
  // propagated split()/updateErrorMetric() can arise. must use Patch pointer.

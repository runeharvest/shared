 ...
 *	- Snap logic entities position to Visual position (snap on landscape for
 *instance).
...
public:
   ...
   * manager and all entities must be cleared when the landscape is deleted.
   */
  void setLandscape(CLandscape *landscape);

  /** create an entity. NB: CVisualCollisionManager do not owns this ptr for
   * now, and you must delete it with deleteEntity(). NB: CVisualCollisionEntity
   * are no more valid when this manager is deleted.
   */
   ...
   */
  bool getRayCollision(const NLMISC::CVector &start, const NLMISC::CVector &end,
                       bool landscapeOnly = false);

  /** Add a Mesh Instance to the collision manager. For now it is used only for
   *Camera Collision \param mesh the collision mesh (keep a refptr on it) \param
   ...
   */
  uint addMeshInstanceCollision(CVisualCollisionMesh *mesh,
                                const CMatrix &instanceMatrix,
   ...
   *	NB: the MeshInstance Collision use a fake lighting to avoid backface
   *shadowing.
  ...
  void receiveShadowMap(IDriver *drv, CShadowMap *shadowMap,
                        const CVector &casterPos, CMaterial &shadowMat,
                        CShadowMapProjector &smp);

  // Get collision meshs instances that are inside the given box
  void getMeshs(const NLMISC::CAABBox &aabbox,
                std::vector<CMeshInstanceColInfo> &dest);

  // ***************************
private:
  /// The landscape used to generate tiles, and to snap position to tesselated
  /// ground.
  ...
  public:
    ...
    void
    receiveShadowMap(const CVisualCollisionMesh::CShadowContext &shadowContext);
  };

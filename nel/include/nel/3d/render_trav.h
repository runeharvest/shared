...
// ***************************************************************************
/* Skin Manager setup
 * For the moment, the skin manager vertex buffer routes all its UV channel to
 *UV0. See void	CVertexStreamManager::init(IDriver *driver, uint vertexFormat,
 *uint maxVertices) use 8 VBswap for minimal(=> no) lock() time. 2 is not enough
 *(saw 7 ms lost because of this)
 *	=> size of the manager is 1280 Ko
 *	NB: 5000 vertices max for a model is a strong limitation, but it's OK
 *for now...
 */
#define NL3D_MESH_SKIN_MANAGER_VERTEXFORMAT                                    \
 ...
 * NB: need much less Vertices because: 1/ foolish to do more. 2/ Only
 *position=> no UV/Normal discontinuities.
 *	=> size of the manager is 1.3 Mo
 *	use 8 VBswap for minimal(=> no) lock() time. 2 is not enough (saw 7 ms
...
public:
  ...
  struct CIdxLighted {
    ...
    uint DirOrPos[MaxLight]; // light 0, directional sun; light 1,2,3, omni
                             // point light
    uint EyePosition;
    uint DiffuseAlpha;
  };
  struct CFeaturesLighted {
    /// Number of point lights that this program is generated for, varies from 0
    /// to 3.
 ...
 * The purpose of this traversal is to render a list of models. This traversals
 *is tightly linked to the cliptraversal. The clipTraversals insert directly the
 *models with CRenderTrav::addRenderModel(m). The traverse() method should
...
public:
   ...
   * \param newRender true If scene render is beginning. Otherwise other parts
   * of the scene have already been rendered.
   */
  ...
  /** Add a model to the list of rendered models. \b DOESN'T \b CHECK if already
   * inserted.
   */
  ...
  /* This is for the rare case where objects are deleted during
   *CScene::render(). called by deleteModel() This method don't need to be
   *called by ~CTransform
   */
  ...
  // Yoyo: temporary. May be used later if we decide to support a PBuffer to
  // render texture for ShadowMap
  IDriver *getAuxDriver() { return Driver; }
  void setViewport(const CViewport &viewport) { _Viewport = viewport; }
  CViewport getViewport() const { return _Viewport; }
  ...
  void setLayersRenderingOrder(bool directOrder = true) {
    _LayersRenderingOrder = directOrder;
  }
   ...
   * interval. By default, there's no prioriy sorting (0 -> single priority, 255
   * -> 256 possible priorities) Objects with highers priority are displayed
   ...
   */
  void setupTransparencySorting(uint8 maxPriority = 0,
  ...
  const CShadowMapManager &getShadowMapManager() const {
    return _ShadowMapManager;
  }

  /// the MeshSkinManager for Shadow. Same Behaviour than std MeshSkinManager.
  /// NB: the Shadow MSM is inited with AuxDriver.
  void setShadowMeshSkinManager(CVertexStreamManager *msm);
  CVertexStreamManager *getShadowMeshSkinManager() const {
    return _ShadowMeshSkinManager;
  }
...
public:
   ...
   *	called at beginning of traverse(). Must be called by any model (before
   *and after rendering) that wish to use CDriver::setLight() instead of the
   *standard behavior with changeLightSetup()
   */
  void resetLightSetup();

  /** setup the driver to the given lightContribution.
   *	if lightContribution==NULL, then all currently enabled lights are
   *disabled. NB: lightContribution is cached, so if same than preceding, no-op.
   *	cache cleared at each frame with resetLightSetup().
   *	NB: models which are sticked or skinned on a skeleton have same
   *lightContribution because lightSetup is made on the skeleton only. Hence the
   *interest of this cache.
   *
   *	\param useLocalAttenuation if true, use Hardware Attenuation, else use
   *global one (attenuation with AttFactor)
   */
   ...
   *	Only 0..3 Light + SunLights are supported. The VP do NOT support
   *distance/Spot attenuation Also it does not handle World Matrix with non
   *uniform scale correctly since lighting is made in ObjectSpace
   *
   *	\param ctStart the program use ctes from ctStart to ctStart+NumCtes.
   *	\param supportSpecular asitsounds. PointLights and dirLight are
   *localViewer \param invObjectWM the inverse of object matrix: lights are mul
   *by this. Vp compute in object space.
   */
  void beginVPLightSetup(CVertexProgramLighted *program,
                         const CMatrix &invObjectWM);

  /** change the driver VP LightSetup constants which depends on material.
   *  \param excludeStrongest This remove the strongest light from the setup.
   * The typical use is to have it computed by using perpixel lighting.
   */
   ...
   *		- o[COL0] and o[COL1] are setuped. NB: BF0 and BF1 not
   *computed/setuped. Scratch:
   *		- R0, R1, R2, R3, R4
   *
   ...
   *		- 9:		material Diffuse Alpha copied to output. cte is:
   *{0,0, 1, alphaMat} TOTAL: 10 constants used. if supportSpecular:
   *		- 0:		AmbientColor.
   *		- 1..4:		DiffuseColor of 4 lights.
   *		- 5..8:		SpecularColor of 4 lights. NB:
   *SpecularColor[5].w get the specular exponent of the material
   *		- 9:		- (directional Light direction) in objectSpace
   *		- 10:		material Diffuse Alpha copied to output. cte is:
   *{0,0, 1, alphaMat}
   ...
   *
   *
   *	 NB: the number of active light does not change the number of constantes
   *used. But the VP code returned is modified accordingly.
   *
   *  \param numActivePoinLights tells how many point light from 0 to 3 this VP
   *must handle. NB: the Sun directionnal is not option NB:
   *nlassert(numActiveLights<=MaxVPLight-1).
   */
  static std::string getLightVPFragmentNeLVP(uint numActivePointLights,
                                             uint ctStart, bool supportSpecular,
                                             bool normalize);
  // TODO_VP_GLSL
  ...
  /// return an index to the current strongest settuped light (or -1 if there's
  /// none)
  sint getStrongestLightIndex() const;

  /** Get current color, diffuse and specular of the strongest light in the
   * scene. These values are modulated by the current material color, so these
   * values are valid only after changeVPLightSetupMaterial() has been called
   */
  ...
  // ReSetup the Driver Frustum/Camera. Called internally and by
  // ShadowMapManager only.
...
private:
  ...
  std::vector<CLayeredOrderingTable<CTransform>>
   ...
   *	The Landscape list is separated from std render List for optimisation
   *purpose. See Doc in traverse() method.
   */
  //@{
  // clear the list

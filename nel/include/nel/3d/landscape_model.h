...
// ***************************************************************************
const NLMISC::CClassId LandscapeModelId =
    NLMISC::CClassId(0x5a573b55, 0x6b395829);
...
public:
   ...
   */
  void setAdditive(float additive) { _Additive = additive; }

  /** Get additive value
   *
   * \return the additive value. [0, 1]
   */
  float getAdditive() const { return _Additive; }

  /** Set additive set
   *
   * \param enable is true to activbe additive, false to disactive it.
   */
  void enableAdditive(bool enable) { _ActiveAdditive = enable; }

  /** Get additive set
   *
   * \return true to if additive is actived, else false.
   */
  bool isAdditive() const { return _ActiveAdditive; }
  ...
  void setRefineCenterUser(const CVector &refineCenter) {
    _RefineCenterUser = refineCenter;
  }
  const CVector &getRefineCenterUser() const { return _RefineCenterUser; }

  /** Override CTransform::initModel(), to create CLandscape's
   * VegetableManager's BlendLayer models in the scene.
   */
  ...
  virtual void receiveShadowMap(CShadowMap *shadowMap, const CVector &casterPos,
...
private:
  ...
  // The current clustered pyramid. computed in clip(), and parsed in
  // traverseRender()
  std::vector<CPlane> ClusteredPyramid;

  // If ClusteredPyramid is already the WorldFrustumPyramid. NB: if false, it
  // may still be actually
  bool ClusteredPyramidIsFrustum;

  // This is The Last WorldMatrix used to render (not identity for ZBuffer
  // considerations).

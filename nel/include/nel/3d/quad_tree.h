 ...
 *
 *
 * This first implementation support real-time quad node split, but never merge
 *the quad node. The possibility to merge (delete) empty quads, when an element
 *erase occurs, will be added later.
 *
 * The quadtree is geometrically delimited. By default, his size is 1*1,
 *centered on (0,0,0). If an element which is out this zone is inserted, then it
 *will ALWAYS be considered selected in select*() methods. By default, the quad
 *tree is aligned on XZ.
 *
...
public:
  ...
  /// \name Initialization
  //@{
  /** Change the base matrix of the quad tree. For exemple this code init the
   * quad tree in the plane XY: \code CQuadTree			quadTree;
   * NLMISC::CMatrix		tmp;
  ...
  /** Init the container
   *
   * \param DepthMax is the max depth in the tree. The max cell count is
   * (1<<DepthMax)^2 \param center is the center of the quad tree \param size is
   * the width and the height of the initial quad tree.
   */
  ...
  //@{
  /// Clear the container. Elements are deleted, and the quadtree too (create()
  /// is undone)
  void clear();
  ...
  /** Insert a new element in the container. The bounding box of the element
   * MUST be included in the bounding box of the quadtree.
   *
   ...
   * of the element to insert with maximal coordinates. \param val is a
   * reference on the value to insert
   */
  ...
  /** Select element with multiple planes. Intersect with a polytope convex made
   * of planes. The normals of planes must be directed outwards the polytope.
   *
...
private: // Classes.
  ...
  public:
    ...
    CBaseNode *Prev, *Next;
    // for the quadnode list. A node MAY be pointed by 4 quad (each having the
    // same level).
  ...
  public:
    ...
    void clear()            // update links.
    {
      ...
      for (uint i = 0; i < 4; i++) {
        if (QuadPrevs[i]) {
          nlassert(QuadPrevs[i]->QuadNexts[i] == this);
          QuadPrevs[i]->QuadNexts[i] = QuadNexts[i];
        }
        if (QuadNexts[i]) {
          nlassert(QuadNexts[i]->QuadPrevs[i] == this);
          QuadNexts[i]->QuadPrevs[i] = QuadPrevs[i];
        }
        QuadPrevs[i] = NULL;
        QuadNexts[i] = NULL;
      }
  ...
  public:
    ...
    uint ListIndex;     // [0,3]. index of which list to follow in
                        // "Node.QuadNexts[]".
  ...
  public:
    ...
    void clear() {
      ...
      while ((p = RootNode.QuadNexts[ListIndex])) {
        p->clear(); // clear the links. => RootNode.QuadNexts[ListIndex] is
                    // implicitly modified
    ...
    void split() {
      ...
      // Middle compute.
      NLMISC::CVector MidLeft(0, 0, 0), MidRight(0, 0, 0), MidTop(0, 0, 0),
          MidBottom(0, 0, 0), Middle(0, 0, 0);
      MidLeft.x = BBoxMin.x;
      MidLeft.z = (BBoxMin.z + BBoxMax.z) / 2;
      MidRight.x = BBoxMax.x;
      MidRight.z = (BBoxMin.z + BBoxMax.z) / 2;
      MidTop.x = (BBoxMin.x + BBoxMax.x) / 2;
      MidTop.z = BBoxMin.z;
      MidBottom.x = (BBoxMin.x + BBoxMax.x) / 2;
      MidBottom.z = BBoxMax.z;
      Middle.x = MidTop.x;
      Middle.z = MidLeft.z;
      // Sons compute.
      // Don't care of Y.
      Sons[0]->BBoxMin = BBoxMin;
      Sons[0]->BBoxMax = Middle;
      Sons[1]->BBoxMin = MidTop;
      Sons[1]->BBoxMax = MidRight;
      Sons[2]->BBoxMin = MidLeft;
      Sons[2]->BBoxMax = MidBottom;
      Sons[3]->BBoxMin = Middle;
      Sons[3]->BBoxMax = BBoxMax;
    }

    // ============================================================================================
    // This is a quadtree, so those tests just test against x/z. (the base of
    // the box).
    bool includeBoxQuad(const NLMISC::CVector &boxmin,
    ...
    // ============================================================================================
    bool intersectBoxQuad(const NLMISC::CVector &boxmin,
                          const NLMISC::CVector &boxmax) {
      // inequality and equality is very important, to ensure that a element box
      // will not fit in too many quad boxes.
      if (boxmin.x > BBoxMax.x)
        return false;
      if (boxmin.z > BBoxMax.z)
        return false;
      if (boxmax.x <= BBoxMin.x)
        return false;
      if (boxmax.z <= BBoxMin.z)
        return false;
      return true;
    }
    // ============================================================================================
    bool intersectBox(const NLMISC::CVector &boxmin,
                      const NLMISC::CVector &boxmax) {
      // inequality and equality is very important, to ensure that a element box
      // will not fit in too many quad boxes.
      if (boxmin.x > BBoxMax.x)
        return false;
      if (boxmin.y > BBoxMax.y)
        return false;
      if (boxmin.z > BBoxMax.z)
        return false;
      if (boxmax.x <= BBoxMin.x)
        return false;
      if (boxmax.y <= BBoxMin.y)
        return false;
      if (boxmax.z <= BBoxMin.z)
        return false;
      return true;
    ...
    bool intersectBox(std::vector<NLMISC::CPlane> &BVolume) {
      ...
      for (sint i = 0; i < (int)BVolume.size(); i++) {
        ...
        // If only one of the box vertex is IN the plane, then all the box is IN
        // this plane.
        if (plane * NLMISC::CVector(b1.x, b1.y, b1.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b1.x, b1.y, b2.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b1.x, b2.y, b1.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b1.x, b2.y, b2.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b2.x, b1.y, b1.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b2.x, b1.y, b2.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b2.x, b2.y, b1.z) <= 0)
          continue;
        if (plane * NLMISC::CVector(b2.x, b2.y, b2.z) <= 0)
          continue;
        // If ALL box vertices are OUT of this plane, then the box is OUT of the
        // entire volume.
        return false;
      }
      // TODO. This is a simple box detection. The box is not really clipped and
      // sometimes, the box will said it intersect but it is not the case...
      // Here, We should test the real box volume, against BVolume. But this is
      // more expensive...
      return true;
    }
    ...
    void insert(const NLMISC::CVector &boxmin, const NLMISC::CVector &boxmax,
                uint wantdepth, CNode *newNode) {
      if (Level == 0) {
      ...
      // If we are inserting there or its children, we have to resize the Y-axe
      // BBox of quadnode.
      if (BBoxNeverRescale) {
        BBoxMin.y = boxmin.y;
        BBoxMax.y = boxmax.y;
      ...
      if (wantdepth == Level) {
        ...
        // If the quad is a leaf, we need to split it (because we are not yet at
        // the right level).
        if (isLeaf())
          split();
    ...
    // ============================================================================================
    void select(CBaseNode &selroot, const NLMISC::CVector &bboxmin,
...
public:
  ...
  public:
    ...
    T &operator*() const { return const_iterator::_Ptr->Value; }
    // Doesn't work...
    /*T*	operator->() const
        {return (&**this); }*/
    CIterator &operator++() {
      const_iterator::_Ptr = (CNode *)(const_iterator::_Ptr->Next);
      return (*this);
    }
    ...
    CIterator &operator--() {
      const_iterator::_Ptr = (CNode *)(const_iterator::_Ptr->Prev);
      return (*this);
    }
    CIterator operator--(int) {
      CIterator tmp = *this;
...
// ============================================================================================
template <class T>
void CQuadTree<T>::create(uint DepthMax, const NLMISC::CVector &center,
                          float size) {
...
// ============================================================================================
template <class T>
typename CQuadTree<T>::CIterator
CQuadTree<T>::insert(const NLMISC::CVector &bboxmin,
                     const NLMISC::CVector &bboxmax, const T &val) {
  NLMISC::CVector myboxmin2 = _ChangeBasis * bboxmin;
  NLMISC::CVector myboxmax2 = _ChangeBasis * bboxmax;
  NLMISC::CVector myboxmin(std::min(myboxmin2.x, myboxmax2.x),
                           std::min(myboxmin2.y, myboxmax2.y),
                           std::min(myboxmin2.z, myboxmax2.z));
  NLMISC::CVector myboxmax(std::max(myboxmin2.x, myboxmax2.x),
                           std::max(myboxmin2.y, myboxmax2.y),
                           std::max(myboxmin2.z, myboxmax2.z));
...
template <class T> void CQuadTree<T>::clearSelection() {
  ...
  while ((p = _Selection.Next)) {
    // We are removing this node from selection, which will implicitly modify
    // _Selection.Next.
    if (p->Prev)
      p->Prev->Next = p->Next;
    if (p->Next)
...
// ============================================================================================
template <class T>
void CQuadTree<T>::select(const NLMISC::CVector &bboxmin,
                          const NLMISC::CVector &bboxmax) {
  NLMISC::CVector myboxmin2 = _ChangeBasis * bboxmin;
  NLMISC::CVector myboxmax2 = _ChangeBasis * bboxmax;
  NLMISC::CVector bboxminCopy(std::min(myboxmin2.x, myboxmax2.x),
                              std::min(myboxmin2.y, myboxmax2.y),
                              std::min(myboxmin2.z, myboxmax2.z));
  NLMISC::CVector bboxmaxCopy(std::max(myboxmin2.x, myboxmax2.x),
                              std::max(myboxmin2.y, myboxmax2.y),
                              std::max(myboxmin2.z, myboxmax2.z));
...
// ============================================================================================
template <class T>
void CQuadTree<T>::select(const std::vector<NLMISC::CPlane> &BVolume) {
...
// ============================================================================================
template <class T>
void CQuadTree<T>::selectRay(const NLMISC::CVector &source,
                             const NLMISC::CVector &dir) {
  NLMISC::CMatrix mat;
  mat.identity();

  // Set a wrong matrix
  NLMISC::CVector vTmp = dir ^ ((fabs(vTmp * NLMISC::CVector(1, 0, 0)) > 0.f)
...
// ============================================================================================
template <class T>
void CQuadTree<T>::selectSegment(const NLMISC::CVector &source,
                                 const NLMISC::CVector &dest) {
  NLMISC::CMatrix mat;
  mat.identity();

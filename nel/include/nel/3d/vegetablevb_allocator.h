 ...
 *	Big difference is that here, we do not really matter about reallocation
 *because both software and hardware VB are present. Also, VertexProgram MUST be
 *supported by driver here. NB: unlike Landscape VBAllocator, the VertexProgram
 *is not managed by this class. \author Lionel Berenguier \author Nevrax France
 * \date 2001
 */
...
public:
   ...
   *	maxVertexInBufferHard is the number of AGP vertex allocated. if 0 =>
   *buffer soft only
   */
  void init(TVBType vbType, uint maxVertexInBufferHard);

  /** setup driver, and test for possible VBHard reallocation. if reallocation,
   *refill the VBHard to do anytime you're not sure of change of the
   *driver/vbHard state.
   *
  ...
  /** return number of vertices allocated with allocateVertex() (NB: do not
   *return the actual number of vertices allocated in VBuffer, but the number of
   *vertices asked to be allocated).
   */
  uint getNumUserVerticesAllocated() const;

  /// Allocate free vertices in VB. (RAM and AGP if possible). work with locked
  /// or unlocked buffer.
  uint allocateVertex();
  ...
  bool isBGRA() const {
    return _VBHard.getVertexColorFormat() == CVertexBuffer::TBGRA;
  }

  /** activate the VB or the VBHard in Driver setuped. nlassert if driver is
   * NULL or if buffer is locked.
   */
  void activate();
  // @}

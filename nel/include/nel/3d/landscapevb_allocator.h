...
public:
   ...
   *	The vertex buffer is reallocated and reallocationOccurs() return true
   *(see reallocationOccurs()). to do anytime you're not sure of change of the
   *driver/vbHard state.
   *
   *	Note: the vertexProgram is created/changed here, according to driver,
   *and TType.
   *
  ...
  // delete all VB, and free driver ressources (if RefPtr driver not deleted).
  // clear list too.
  void clear();
  ...
  // a buildVBInfo() should be done and ALL data are lost, so all VB must be
  // rewrited.
  ...
  // Allocate free vertices in VB. (AGP or RAM). work with locked or unlocked
  // buffer. NB: if reallocationOccurs(), then ALL data are lost.
  uint allocateVertex();
  ...
  /// \name Buffer access.
  // @{
  /** lock buffers Hard (if any). "slow call", so batch them. nlassert good
   *TType. return is the VB info. NB: if the buffer is locked while a
   *reallocation occurs, then the buffer is unlocked.
   */
  ...
  /** activate the VB or the VBHard in Driver setuped. nlassert if driver is
   * NULL or if buffer is locked. If vertexProgram possible, activate the
   * vertexProgram too. Give a vertexProgram Id to activate. Always 0, but 1 For
   * tile Lightmap Pass.
   */
  void activate(uint vpId);
  void activateVP(uint vpId);
  inline CVertexProgramLandscape *getVP(uint vpId) const {
    return _VertexProgram[vpId];
  }
  // @}
...
private:
  ...
  // tell if VBHard is possible. NB: for ATI, it is false because of slow
  // unlock.
  CVertexBuffer _VB;
  bool _BufferLocked;
...
public:
  ...
  CVertexProgramLandscape(CLandscapeVBAllocator::TType type,
                          bool lightMap = false);

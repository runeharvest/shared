...
public:
   ...
   * slot, use CAnimationPlaylist::empty.
   */
   ...
   *
   * \param slot is the id of the slot to set.
   * \return the animation number in use in this slot. Return
   * CAnimationPlaylist::empty if the slot is empty.
   */
   ...
   * empty the slot, use CAnimationPlaylist::empty. \param inverted is false if
   * the weights must be used as they are, true if the weights to used are 1.f -
   * weight.
   */
   ...
   * Default time is 0.f. \return the start weight used in the slot.
   */
   ...
   *
   * \param slot is the id of the slot to set.
   * \param time is the playlist time for which the end weight is set. Default
   * time is 0.f. \param endWeight is the factor to use in this slot.
   */
   ...
   * * the animation time according with the begin time of each slot, the speed
   * factor of each slot and the current time passed in parameter.
   * * the skeleton weight id.
   * * the weight interpolated with the start and end values. The weight value
   * are clamped before and after the animation.
   */
   ...
   */
  static float getWeightValue(TGlobalAnimationTime startWeightTime,
                              TGlobalAnimationTime endWeightTime,
                              TGlobalAnimationTime time, float startWeight,
                              float endWeight, float smoothness);
   ...
   */
  TAnimationTime getLocalTime(uint8 slot, TGlobalAnimationTime globalTime,
                              const CAnimationSet &animSet) const;

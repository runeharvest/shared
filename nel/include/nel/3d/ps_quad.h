...
/** This abstract class holds what is needed with quad particles (CPSFaceLookAt,
 * CPSFace) e.g Index and vertex buffer and method to setup them Material, and
 * method to setup them
 */
...
class CPSQuad : public CPSParticle,
                ...
                public CPSMaterial {
public:
  /** create the quad by giving a texture. This can't be a CTextureGrouped (for
   * animation) animation must be set later by using setTextureScheme
   */
...
protected:
   ...
   */
  void updateVbColNUVForRender(CVertexBuffer &vb, uint32 startIndex,
                               uint32 numQuad, uint32 srcStep, IDriver &drv);
  ...
  enum VBType {
    ...
    VBTexAnimated =
        0x0004,      // the texture coordinate can be animated (not precomputed)
    VBTex2 = 0x0008, // the vb has second texture coordinates
    VBTex2Animated = 0x0010, // the second texture coordinates can be animated
                             // (not precomputed)
    VBFullMask = 0x001f
  };
  ...
  /// used to get a pointer on the right vb dependant on its type (cf. values of
  /// VBType)
  static CVertexBuffer *const _VbTab[];

  // from CPSTexturedParticle / CPSMultiTexturedParticle  : gives us the
  // opportunity to update wrap mode for quad particles

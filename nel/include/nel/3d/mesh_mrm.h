...
public:
   ...
   */
  void build(CMesh::CMeshBuild &m, std::vector<CMesh::CMeshBuild *> &bsList,
             uint numMaxMaterial,
   ...
   *	NB: no-op if distanceFinest<0, distanceMiddle<=distanceFinest or if
   *distanceCoarsest<=distanceMiddle. \param distanceFinest The MRM has its max
   *faces when dist<=distanceFinest. \param distanceMiddle The MRM has 50% of
   *its faces at dist==distanceMiddle. \param distanceCoarsest The MRM has
   *faces/Divisor (ie near 0) when dist>=distanceCoarsest.
   */
  ...
  virtual void render(IDriver *drv, CTransformShape *trans, float polygonCount,
                      uint32 rdrFlags, float globalAlpha);

  /// render() this mesh as a skin
  virtual void renderSkin(CTransformShape *trans, float alphaMRM);

  /// get an approximation of the number of triangles this instance will render
  ...
  /// Scene profile
  void profileSceneRender(CRenderTrav *rdrTrav, CTransformShape *trans,
   ...
   *	\param f the input stream to read. NB: after load, f.getPos() return the
   *position of the second lod in the stream.
   */
  void loadFirstLod(NLMISC::IStream &f);

  /** Load next lod of a stream. use getNbLodLoaded() to know what Lod will be
   *loaded. NB: if getNbLodLoaded() == getNbLod(), no op. \param f the same
   *input stream passed to loadFirstLod(). NB: after load, f.getPos() is
   *"unedfined" (actually return the position of the next lod in the stream).
   */
  void loadNextLod(NLMISC::IStream &f);

  /** UnLoad Lod getNbLodLoaded()-1 from memory. use getNbLodLoaded()-1 to know
   * what Lod will be unloaded. NB: if getNbLodLoaded() <= 1, no op.
   */
  ...
  const NLMISC::CAABBoxExt &getBoundingBox() const { return _BBox; }

  /// get the vertex buffer used by the mrm mesh. NB: this VB store all Vertices
  /// used by All LODs.
  const CVertexBuffer &getVertexBuffer() const { return _VBufferFinal; }

  /// get the skinWeights used by the mrm mesh. NB: same size of the
  /// vertexBuffer. empty if !isSkinned()
  ...
  uint getNbRdrPass(uint lodId) const {
    return (uint)_Lods[lodId].RdrPass.size();
  }
   ...
   */
  const CIndexBuffer &getRdrPassPrimitiveBlock(uint lodId,
   ...
   *	\param lodId the id of the LOD.
   *	\param renderingPassIndex the index of the rendering pass in the matrix
   *block
   */
  ...
  uint getNbBlendShapes() const {
    return (uint)_MeshMorpher.BlendShapes.size();
  }

  /** Build a geometry copy for the given Lod. The VBuffer/IndexBuffer must not
   *be resident. false if bad lodId The process ensure there is no hole in the
   *resulting vertices array. Hence VB num Verts != vertices.size().
   */
  bool buildGeometryForLod(uint lodId, std::vector<CVector> &vertices,
  ...
  /// update Skeleton Usage. increment or decrement. computeBonesId must has
  /// been called before.
  void updateSkeletonUsage(CSkeletonModel *sm, bool increment);

  /// return array of bones used by the skin. computeBonesId must has been
  /// called before.
  ...
  /// computeBonesId must has been called before.
  const std::vector<NLMISC::CBSphere> &getSkinBoneSphere() const {
    return _BonesSphere;
  }

  /// Called for edition purpose (slow  O(NVertex) ). computeBonesId must has
  /// been called before.
  bool getSkinBoneBBox(CSkeletonModel *skeleton, NLMISC::CAABBox &bbox,
                       uint boneId) const;
  ...
  virtual void activeInstance(CMeshGeomRenderContext &rdrCtx,
                              CMeshBaseInstance *inst, float polygonCount,
                              void *vbDst);
  virtual void renderPass(CMeshGeomRenderContext &rdrCtx,
                          CMeshBaseInstance *inst, float polygonCount,
                          uint rdrPass);
  ...
  sint renderSkinGroupGeom(CMeshMRMInstance *mi, float alphaMRM,
                           uint remainingVertices, uint8 *vbDest);
  void renderSkinGroupPrimitives(
  ...
  virtual bool hasMeshVertexProgram() const {
    return _MeshVertexProgram != NULL;
  }

  /// \name ShadowMap Skin rendering
  // @{
  /// Setup the ShadowMesh
  void setShadowMesh(const std::vector<CShadowVertex> &shadowVertices,
  ...
  sint renderShadowSkinGeom(CMeshMRMInstance *mi, uint remainingVertices,
                            uint8 *vbDest);
  void renderShadowSkinPrimitives(CMeshMRMInstance *mi, CMaterial &castMat,
                                  IDriver *drv, uint baseVertex);

  /** Special use of skinning to compute intersection of a ray with it.
   *	Internaly Use same system than ShadowSkinning, see
   *CShadowSkin::getRayIntersection()
   */
  bool supportIntersectSkin() const { return supportShadowSkinGrouping(); }
  bool intersectSkin(CMeshMRMInstance *mi, const CMatrix &toRaySpace,
...
private:
  ...
  struct CVertexBlock {
    ...
    void serial(NLMISC::IStream &f) { f.serial(VertexStart, NVertices); }
  };
  ...
  public:
     ...
     *There is 4 array, 0th is for vertices which have only one matrix. 1st if
     *for vertices which have only 2 matrix ....
     */
    std::vector<uint32> InfluencedVertices[NL3D_MESH_SKINNING_MAX_MATRIX];
    /// Skinning: list of Matrix which influence this Lod. So we know what
    /// matrix to compute.
    std::vector<uint32> MatrixInfluences;
    /// Skinning: does the VBuffer part of this Lod contains original skin
    /// vertices.
    bool OriginalSkinRestored;
    /** Skinning: list of vertex blocks to copy from RAM to AGP, for this Lod
     *only. NB: it is constructed from InfluencedVertices. Only useful if
     *skinned.
     */
  ...
  /** A mesh information. NB: private. unlike CMesh::CMeshBuild, do not herit
   * from CMeshBase::CMeshBuild, because computed internally with CMRMBuilder,
   * and only geometry is of interest.
   */
  ...
  public:
    ...
    /// The last+1 new wedge this lod use. NB: Lod.NWedges==
    /// LodInfo.EndAddWedges.
    uint32 EndAddWedges;
    /// the absolute Lod offset in the last Stream which has been used to read
    /// this MRM.
    sint32 LodOffset;
...
private:
  ...
  bool _Skinned;
  /// Skinning: this is the list of vertices (mirror of VBuffer), at the bind
  /// Pos.
  ...
  /// This array give the index in the skeleton of the local bones used.
  /// computed at first computeBoneId()
  ...
  /// NB: HERE FOR PACKING ONLY. For clipping. Estimate if we must do a Precise
  /// clipping (ie with bboxes)
...
private:
  ...
  void applyGeomorphWithVBHardPtr(std::vector<CMRMWedgeGeom> &geoms,
                                  float alphaLod, uint8 *vertexDestPtr);

  /// Faster, but common geomorph apply
  void applyGeomorphPosNormalUV0(std::vector<CMRMWedgeGeom> &geoms,
                                 uint8 *vertexPtr, uint8 *vertexDestPtr,
  ...
  /** The same as apply skin, but with normal modified. Normal is not
   *normalized. 4 versions from slower to faster.
   */
  void applySkinWithNormal(CLod &lod, const CSkeletonModel *skeleton);
  void applyRawSkinWithNormal(CLod &lod, CRawSkinNormalCache &rawSkinLod,
                              const CSkeletonModel *skeleton, uint8 *vbHard,
                              float alphaLod);

  /** The same as apply skin with normal, but with a tangent space added
   *(encoded in a texture coordinate). The tangent space is modified, but not
   *normalized (must be done in a vertex program). 4 versions from slower to
   *faster.
   */
  void applySkinWithTangentSpace(CLod &lod, const CSkeletonModel *skeleton,
  ...
  // SkinGroup
  void updateShiftedTriangleCache(CMeshMRMInstance *mi, sint curLodId,
                                  uint baseVertex);
...
private:
  ...
  // ApplySkin method
  void applyArrayRawSkinNormal1(CRawVertexNormalSkin1 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal2(CRawVertexNormalSkin2 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal3(CRawVertexNormalSkin3 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
  void applyArrayRawSkinNormal4(CRawVertexNormalSkin4 *src,
                                uint8 *destVertexPtr, CMatrix3x4 *boneMat3x4,
                                uint nInf);
...
public:
  ...
  /** Build a mesh, replacing old. WARNING: This has a side effect of deleting
   * AnimatedMaterials. this is much slower than CMesh::build(), because it
   * computes the MRM. \param params parameters of the MRM build process.
   */
  void build(CMeshBase::CMeshBaseBuild &mBase, CMesh::CMeshBuild &m,
             std::vector<CMesh::CMeshBuild *> &listBS,
             const CMRMParameters &params = CMRMParameters());

  /** Build a mesh, replacing old. build from a CMeshBaseBuild (materials info)
   *and a previously builded CMeshMRMGeom. WARNING: This has a side effect of
   *deleting AnimatedMaterials. this is much slower than CMesh::build(), because
   *it computes the MRM. \param params parameters of the MRM build process.
   */
  void build(CMeshBase::CMeshBaseBuild &m, const CMeshMRMGeom &mgeom);

  /** Optimize material use. If a material in CMeshBase is not used by any
   *renderPasses, it is removed, and ids are updated. WARNING: This has a side
   *effect of deleting AnimatedMaterials. \param remap a remap material Id:
   *newId= remap[oldId]. -1 means "no more used"
   */
  ...
  virtual void getAABBox(NLMISC::CAABBox &bbox) const {
    bbox = getBoundingBox().getAABBox();
  }

  /// profiling
  virtual void profileSceneRender(CRenderTrav *rdrTrav, CTransformShape *trans,
                                  bool opaquePass);
  ...
  const CVertexBuffer &getVertexBuffer() const {
    return _MeshMRMGeom.getVertexBuffer();
  }
   ...
   */
  uint getNbRdrPass(uint lodId) const {
    return _MeshMRMGeom.getNbRdrPass(lodId);
  }
  ...
  // @{
  virtual IMeshGeom *supportMeshBlockRendering(CTransformShape *trans,

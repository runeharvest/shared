...
// ***************************************************************************
/**
 * A class used to render instances sorted by MeshGeom first, then per material,
 *where possible. This allow optimisation because less renderState swapping are
 *needed. WARNING: if you add MeshGeom to 2 different CMeshBlockManager at same
 *times, it won't work, and will certainly crashes (not checked/assert).
 *
 *	NB VBHeap part works, even if no User interface use it. It don't give
 *greate performance Add, but will may be used.
 *
...
public:
  ...
  /** Add an instance of a MeshGeom to render. Only CMeshBaseInstance can be
   *added. For now, only CMeshGeom and CMeshMRMGeom are known to work.
   */
  void addInstance(IMeshGeom *meshGeom, CMeshBaseInstance *inst,
                   float polygonCount);
  ...
  /** Add a Heap for a given vertexFormat. Any meshGeom added with addInstance()
   *which has this vertex Format may fit in this heap. return false and fail if
   *the heap can't be allocated or if the heap with same vertexFormat still
   *exist.
   */
...
private:
  ...
  struct CVBHeapBlock {
    /// List of instances. small realloc are performed, since same vector used
    /// each frame.
    std::vector<CInstanceInfo> RdrInstances;

    /// List of MeshGeom. small realloc are performed, since same vector used
    /// each frame.
  ...
  /** List of Heaps.
   *	NB: 0th heap is special: contains all meshs which can't fit in any
   *VBHeap.
   */
  ...
  void render(CVBHeapBlock *hb, IMeshGeom *meshGeom,
              std::vector<CInstanceInfo> &rdrInstances);

...
inline void SpawnedSourceEndedCallback(NLSOUND::USource *source,
                                       void *userParam);
...
public:
  ...
  void init(NLSOUND::USource *source, CPSSoundServImpl *soundServImp,
            bool spawned) {
    nlassert(source);
    _Source = source;
    _Spawned = spawned;
  ...
  virtual void setSoundParams(float gain, const NLMISC::CVector &pos,
                              const NLMISC::CVector &velocity, float pitch) {
    if (!_Source)
      return;
    if (gain < 0)
      gain = 0;
...
protected:
  friend inline void SpawnedSourceEndedCallback(NLSOUND::USource *source,
                                                void *userParam);
...
/**
 * This class implements PS sound server. It warps the calls to NEL sound.
 * Everything is in a .h file to avoid dependency \author Nicolas Vizerie
...
public:
  ...
  void init(NLSOUND::UAudioMixer *audioMixer) { _AudioMixer = audioMixer; }
  ...
  UPSSoundInstance *createSound(const NLMISC::TStringId &soundName,
                                bool spawned = true) {
    if (!_AudioMixer)
      return NULL;
    CPSSoundInstanceImpl *sound = new CPSSoundInstanceImpl;
    NLSOUND::USource *source = _AudioMixer->createSource(
        soundName, spawned, SpawnedSourceEndedCallback, sound);
    if (source) {
      /*
      if (spawned)
      {
...
/// this callback is called when a spawned source has ended, so that we know
/// that the pointer to it is invalid...
inline void SpawnedSourceEndedCallback(NLSOUND::USource *source,
                                       void *userParam) {

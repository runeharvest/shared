...
public:
  ...
  struct CStringInfo {
    ...
    float StringHeight;
    /** StringLine is the size from bottom of the whole string image to the
     *hotspot in pixels. for instance if the hotspot is bottomLeft the imaginary
     *line of the string "bpc" is under the b, under the loop of the p but over
     *the leg of the p. So StringLine is a positive value in this case. It may
     *be a negative value for the string "^" for example.
     */
    float StringLine;
    ...
    CStringInfo() {
      StringWidth = StringHeight = StringLine = 0;
      StringLength = 0;
    }
    CStringInfo(float w, float h, float l, size_t len) {
      StringWidth = w;
      StringHeight = h;
      StringLine = l;
      StringLength = len;
    ...
    /// convert size in pixels to size in 0-1 relative coordinates, according to
    /// Driver current size
...
public:
   ...
   * color. Default: (0,0,0,255) \param the shadow color
   */
  ...
  /** set to true if you want that the font manager look at Driver window size,
   * and resize fontSize so it keeps same size than if it was in 800x600...
   */
   ...
   *ViewAMtrix/ ModelMatrix). Exception: render3D() use UDriver Matrix context
   *for Frustum/ViewMatrix, but use its own modelmatrix.
   */
  // @{
  /**
  ...
  /**
   * set the letter selection of a string. Only letters in the range given are
   * displayed. Default is 0/0xFFFFFFFF
   */
  virtual void setStringSelection(uint32 i, uint32 selectStart,
                                  uint32 selectSize) = 0;
  /**
  ...
  /**
   * Get a string information from the list. return CStringInfo(0,0) if not
   *found. The returned string info is in pixel size per default.
   */
   ...
   * z : if the hotspot is bottom z is the position of the line of the string,
   * not the bottom of the string bounding box !
   */
  virtual void printAt(float x, float y, uint32 i) = 0;
  /** Same as printAt but special version for interface: clip and insert in a
   * temp buffer. z : if the hotspot is bottom z is the position of the line of
   * the string, not the bottom of the string bounding box !
   */
  virtual void printClipAt(URenderStringBuffer &renderBuffer, float x, float y,
                           uint32 i, float xmin, float ymin, float xmax,
                           float ymax) = 0;
  /** Same as printClipAt but special version for planar 3d interface: the final
   * vertices are unproject using a frustum. depth is the positive depth to used
   * to unproject the string worldSpaceMatrix is used to mul each vertex (when
   * in 0..1 coordinate for x/y and in meter for z coordinate)
   */
  virtual void printClipAtUnProjected(URenderStringBuffer &renderBuffer,
                                      class NL3D::CFrustum &frustum,
                                      const NLMISC::CMatrix &worldSpaceMatrix,
                                      float x, float y, float depth, uint32 i,
                                      float xmin, float ymin, float xmax,
                                      float ymax) = 0;
  // TEMP
  virtual void printClipAtOld(float x, float y, uint32 i, float xmin,
                              float ymin, float xmax, float ymax) = 0;
  /**
   ...
   *	render3D() use UDriver Matrix context for Frustum/ViewMatrix, but use
   *its own modelmatrix (mat).
   */
  virtual void render3D(const NLMISC::CMatrix &mat,
                        NLMISC::CUtfStringView sv) = 0;
  /**
   * compute and render a string at the location (3D method)
   *	render3D() use UDriver Matrix context for Frustum/ViewMatrix, but use
   *its own modelmatrix (mat).
   */
  virtual void render3D(const NLMISC::CMatrix &mat, const char *format,
                        ...) = 0;
  ...
  /// create a renderBuffer for printClipAt(). Must delete it with
  /// deleteRenderBuffer()
  virtual URenderStringBuffer *createRenderBuffer() = 0;
  virtual void deleteRenderBuffer(URenderStringBuffer *buffer) = 0;

  /// Flush the rendered string buffer. This method sets the driver matrix to a
  /// 2d11 matrix and sets ztest to always and disable z write.
  virtual void flushRenderBuffer(URenderStringBuffer *buffer) = 0;

  /// Flush the rendered string buffer. This method doesn't change the current
  /// matrices nor the material properties.
  virtual void flushRenderBufferUnProjected(URenderStringBuffer *buffer,
                                            bool zwrite) = 0;

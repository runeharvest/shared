...
public:
  struct SLetterKey {
    ...
    SLetterKey()
        : Char(0), Size(0), Embolden(false), Oblique(false),
          FontGenerator(NULL) {}
  ...
  struct SGlyphInfo : SLetterKey {
    ...
    SGlyphInfo()
        : CacheVersion(0), GlyphIndex(0), U0(0.f), V0(0.f), U1(0.f), V1(0.f),
...
private:
  ...
  void copyGlyphBitmap(uint8 *bitmap, uint32 bitmapW, uint32 bitmapH,
                       uint32 atlasX, uint32 atlasY);
  ...
  bool reserveAtlas(const uint32 width, const uint32 height, uint32 &x,
                    uint32 &y);

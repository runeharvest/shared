 ...
 *	A VegetableManager should be put into a CScene model which is Opaque (ie
 *rendered in Opaque pass), and call vegetableManager::render() at this time. a
 *good example is CLandscape.
 *
 *	Because during render(), it uses and setup special "Vegetable Blend
 *Layer models" to render transparents alpha blended vegetables. Toses models
 *are transparent so they are drawn during the transparent pass of the
 *renderTrav's CScene (so after the Opaque pass).
 *
...
public:
  /**
   * \param maxVertexVbHardUnlit maximum VertexCount in VBHard for Unlit (or
   * precomputed lighted) vegetables \param maxVertexVbHardLighted maximum
   * VertexCount in VBHard for Lighted vegetables \param nbBlendLayers for
   * ZSort/AlphaBlend rdrPass: number of layers of vegetables rendered
   * independently. \param blendLayerDistMax for ZSort/AlphaBlend rdrPass:
   * distance of the farest layer.
   */
  ...
  /** Before any render(), you must call this method (else nlassert). It creates
   *the necessary models in the scene, to manage AlphaBlending correctly. Those
   *models are deleted in the object dtor.
   */
  ...
  /// delete such a clipBlock. all sortBlocks and so all ig must be deleted
  /// before.
  void deleteClipBlock(CVegetableClipBlock *clipBlock);

  /** Create a SortBlock in a clipBlock where instance group (ig) will be
   *created. All AlphaBlend instances created in a SortBlock should have the
   *same vegetWaterState: AboveWater or UnderWater. Each time an instance is
   *added to the sortBlock it changes the _UnderWater state of the sortBlock.
   *	\param center you must give an approximate center for the sortBlock (for
   *sorting) \param radius you must give an approximate radius for the sortBlock
   *(for the system to know when you are IN the sortBlock, and then to sort in a
   *better way)
   */
  CVegetableSortBlock *createSortBlock(CVegetableClipBlock *clipBlock,
   ...
   *	Instances will be frustum-clipped by the clipBlock, and sorted (for the
   *ZSort rdrPass only) by sortBlock.
   */
   ...
   *	If the sortBlock has many Igs, you can do it after deleting all your
   *igs.
   */
   ...
   *
   *	More precisely:
   *		- For all vegetables shapes which will be created in an instance
   *group
   *			- reserveIgAddInstances(shape, waterState, number of
   *instances, vegetIgReserve)
   *		- call reserveIgCompile(ig, vegetIgReserve)
   *		- For all vegetables instances
   *			- addInstance(ig, ...)
   *
   *	After this setup, you CANNOT add anymore instances to the Ig (this is a
   *requirement for fast allocation).
   *
   *	After adding all your instances to an Ig, you must call
   *sortBlockOwnerOfTheIg->updateSortBlock() If the sortBlock has many Igs, you
   *can do it after filling all your igs.
   */
  // @{

  /**	reserve some instance space in an Ig.
   *	nothing is really done here, after doing this for all shapes of your ig,
   *you must call reserveIgCompile() \param vegetIgReserve the object where
   *space required for the ig is added
   */
  void reserveIgAddInstances(CVegetableInstanceGroupReserve &vegetIgReserve,
   ...
   */
  void reserveIgCompile(CVegetableInstanceGroup *ig,
                        const CVegetableInstanceGroupReserve &vegetIgReserve);

  /** add an instance to an ig, enlarging the associated clipBlock bbox.
   *	If the shape is not lighted, then only diffuseColor is used, to setup
   *color per vertex. Warning! Use OptFastFloor()! So call must be enclosed with
   *a OptFastFloorBegin()/OptFastFloorEnd().
   *
   *	Also, buffer must be locked.
   *
   *	ambientColor and diffuseColor should be in [0..1] (no clamp), else uint8
   *will wrap...
   *
   ...
   */
  void addInstance(CVegetableInstanceGroup *ig, CVegetableShape *shape,
                   const NLMISC::CMatrix &mat,
                   const NLMISC::CRGBAF &ambientColor,
                   const NLMISC::CRGBAF &diffuseColor, float bendFactor,
                   float bendPhase, float bendFreqFactor, float blendDistMax,
  ...
  void setDirectionalLight(const CRGBA &ambient, const CRGBA &diffuse,
                           const CVector &light);

  /** lock any AGP vertex buffers. Do it wisely (just one time before refine as
   *example). You MUST enclose calls to addInstance() (and so
   *CVegetable::generateInstance()) with lockBuffers() / unlockBuffers().
   */
  void lockBuffers();
  /// unlock any AGP vertex buffers
  void unlockBuffers();

  /** render the manager into a driver, with current viewMatrix/frustum/fog
   *setuped Buffers should be unlocked. \param	textureDLM is the dynamic
   *lightmap to use. can be NULL if don't want DLM
   */
  void render(const CVector &viewCenter, const CVector &frontVector,
              const std::vector<CPlane> &pyramid, ITexture *textureDLM,
              IDriver *driver);
   ...
   *	You MUST enclose calls to updateLighting() with lockBuffers() /
   *unlockBuffers().
   */
  void updateLighting();

  /** set the frequency of lighting update. If freq==1, ALL lighted igs are
   *updated each second. e.g: if 1/20, then every 20 seconds, all Igs are
   *updated. If you set 0, no update will be done at all (this is the default
   *setup!!).
   */
...
private:
  ...
  uint getRdrPassInfoForShape(CVegetableShape *shape,
                              TVegetableWater vegetWaterState,
                              bool &instanceLighted, bool &instanceDoubleSided,
                              bool &instanceZSort, bool &destLighted,
                              bool &precomputeLighting);

  /// Get the good allocator for the appropriate rdr pass.
  CVegetableVBAllocator &getVBAllocatorForRdrPassAndVBHardMode(uint rdrPass,
                                                               uint vbHardMode);
   ...
   *	vertices are allocated in other VBallocator, copied and freed in the old
   *VBallocator.
   */
   ...
   *	if all Ig is updated, return true and _ULCurrentIgRdrPass and
   *_ULCurrentIgInstance is updated.
   */
   ...
   */
  uint updateInstanceLighting(CVegetableInstanceGroup *ig, uint rdrPassId,
                              uint instanceId);

...
/// this struct has an enumeration of various binary operators available with
/// CPSAttribMakerBinOp
struct CPSBinOp {
  enum BinOp { selectArg1 = 0, selectArg2, modulate, add, subtract, last };
};

/// The size of the buffer use for intermediate operations with a binary
/// operator.
const uint PSBinOpBufSize = 1024;

/**
 * An attribute maker that compute an attribute in a particle system.
 * It takes 2 other attributes makers and perform a binary operation on them to
 * get the result This allow to have more complex behaviour with  particles :
 * random initial size that change with time and so on ... \author Nicolas
 * Vizerie \author Nevrax France \date 2001
 */
...
public:
   ...
   *  Which mean that an assertion will happen if get, make ... are called
   * before setArg is called
   */
  CPSAttribMakerBinOp();
   ...
   * \param arg The argument. Must have been allocated by new, and is then owned
   * by this object
   */
...
protected:
  void *makePrivate(T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex,
                    void *tab, uint32 stride, uint32 numAttrib,
                    bool allowNoCopy = false, uint32 srcStep = (1 << 16),
                    bool forceClampEntry = false) const;

  void make4Private(T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex,
                    void *tab, uint32 stride, uint32 numAttrib,
                    uint32 srcStep = (1 << 16)) const;

  void makeNPrivate(T *buf1, T *buf2, CPSLocated *loc, uint32 startIndex,
                    void *tab, uint32 stride, uint32 numAttrib,
                    uint32 nbReplicate, uint32 srcStep = (1 << 16)) const;

...
// ***************************************************************************
/**
 * UTexture implementation. This Texture must keep a SmartPtr on the real
 * ITexture. Because CMaterial attach texture with SmartPtr too!! \author Lionel
...
public:
  ...
  /// \name Object
  // @{
  /// This ctpor takes a freshly created texture!! UTexture will owns this
  /// texture (via smartptr).
  CTextureUser(ITexture *text) {
    ...
    nlassert((uint)UTexture::UploadFormatCount ==
             (uint)ITexture::UploadFormatCount);
  ...
  virtual void setFilterMode(TMagFilter magf, TMinFilter minf) {
    _Texture->setFilterMode((ITexture::TMagFilter)(uint32)magf,
                            (ITexture::TMinFilter)(uint32)minf);
  }
  ...
  virtual bool mipMapOff() const { return _Texture->mipMapOff(); }
  virtual bool mipMapOn() const { return _Texture->mipMapOn(); }
  ...
  virtual void releaseDatas() { _Texture->release(); }
};
...
public:
  ...
  virtual bool allowDegradation() const { return _Texture->allowDegradation(); }
  ...
  virtual void setEnlargeCanvasNonPOW2Tex(bool dontStretch) {
    ((CTextureFile *)(ITexture *)_Texture)
        ->setEnlargeCanvasNonPOW2Tex(dontStretch);
  }
...
public:
  CTextureMemUser(uint w, uint h, CBitmap::TType t) {
    ...
    if (t == CBitmap::RGBA)
      bpp = 32;
    else if (t == CBitmap::Luminance)
      bpp = 8;
    else if (t == CBitmap::Alpha)
      bpp = 8;
    else if (t == CBitmap::AlphaLuminance)
      bpp = 16;
    else if (t == CBitmap::DXTC1)
      bpp = 4;
    else if (t == CBitmap::DXTC1Alpha)
      bpp = 4;
    else if (t == CBitmap::DXTC3)
      bpp = 8;
    else if (t == CBitmap::DXTC5)
      bpp = 8;
    else if (t == CBitmap::DsDt)
      bpp = 16;
  ...
  void touch() { ((CTextureMem *)(ITexture *)_Texture)->touch(); }

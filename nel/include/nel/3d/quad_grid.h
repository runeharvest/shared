...
// ***************************************************************************
// Default Size of a block for allocation of elements and elements node list in
// the grid.
...
protected:
  ...
  static std::vector<uint>
      _AlreadySelected; // During some selection operations, mark the cells that
                        // have already been visited. may happen if the
                        // selection shape can overlap itself due to the grid
                        // vrapiing. A cell is known to be selected if its uint
                        // "timestamp" is equal to _SelectStamp.
  static uint _SelectStamp; // Incremented at each selection, value used to mark
                            // selected cells.
};

// ***************************************************************************
/**
 * This container is a simple grid, used to quickly find elements. His purpose
 * is similiar to CQuadTree, but it is a simple grid, so test are in O(1), not
 * in O(log n). It is perfect for local lookup (like in collisions). Use it if
 * you want to select small area, not large. Also, for best use, elements should
 * have approximatively the same size, and this size should be little smaller
 * than the size of a grid element...
 *
 * By default, the quad grid is aligned on XY. (unlike the quadtree!!!)
 *
 * Unlike the quadtree, the quadgrid is NOT geographicly delimited, ie, its
 * limits "tiles"!! This is why no "center" is required. As a direct
 * consequence, when you select something, you are REALLY not sure that what you
 * select is not a mile away from your selection :) ....
 *
 * Also, for memory optimisation, no bbox is stored in the quadgrid. Hence no
 * particular selection is made on the Z components...
 *
 * For maximum allocation speed Efficiency, it uses a CBlockMemory<CNode> to
 * allocate elements at insert(). DefaultBlockSize is 16, but you can change it
 * at construction.
 *
...
public:
  ...
  /// \name Initialization
  //@{
  /** Change the base matrix of the quad grid. For exemple this code init the
   * grid tree in the plane XZ: \code CQuadGrid			grid;
   * NLMISC::CMatrix		tmp;
   ...
   *
   * \param size is the width and the height of the initial quad tree, in number
   *of square. For performance view, this should be a power of 2, and <=32768.
   *(eg: 256,512, 8, 16 ...) \param eltSize is the width and height of an
   *element. Must be >0. Notice that the quadgrid MUST be square!!
   */
   ...
   *	Speed is in O(1 * L*H) where L*H is the number of squares surrounded by
   *the element
   *
   * \param it is the iterator to erase.
   * \return if element is currently selected, the next selected element is
   *returned, (or end()). if the element is not selected, end() is returned.
   */
   ...
   *the element
   *
   *	Warning! : bboxmin and bboxmax are multiplied by matrix setuped by
   *changeBase. This work for any matrix with 90deg rotations (min and max are
   *recomputed internally), but not with any rotation (43 degrees ...) because
   *of the nature of AABBox. To do this correclty you should compute the bbox
   *min and max in the basis given in changeBase, and insert() with multiplying
   *min and max with inverse of this basis. eg: CMatrix
   *base= getSomeBase(); CMatrix					invBase=
   *base.inverted();
   *		// create quadGrid.
   ...
   *		quadGrid.insert(invBase*bbox.getMin(), invBase*bbox.getMax(),
   *tri);
   *
   * \param bboxmin is the corner of the bounding box of the element to insert
   *with minimal coordinates. \param bboxmax is the corner of the bounding box
   *of the element to insert with maximal coordinates. \param val is a reference
   *on the value to insert.
   */
  CIterator insert(const NLMISC::CVector &bboxmin,
                   const NLMISC::CVector &bboxmax, const T &val);
  //@}
   ...
   *	Speed is in O(Nelts * L*H), where L*H is the number of squares
  ...
  void selectRay(const NLMISC::CVector &rayStart,
                 const NLMISC::CVector &rayEnd);

  /** Build a selection from a convex polygon. The resulting selection can then
   * be used for a subsequent call to 'select'
   */
  void buildSelectionShape(TSelectionShape &dest,
                           const NLMISC::CPolygon2D &poly) const;

  /** Select element intersecting a selection shape. Clear the selection first.
   */
  void select(const TSelectionShape &shape);

  /** Return the first iterator of the selected element list. begin and end are
   *valid till the next insert. Speed is in O(1)
   */
  CIterator begin();

  /** Return the end iterator of the selected element list. begin and end are
   *valid till the next insert. Speed is in O(1)
   */
  CIterator end();
  //@}
...
private: // Classes.
  ...
  /** An element inserted in the quadGrid. T + Link-list variables (CBaseNode
   * and QuadNodes)
   */
  class CNode : public CBaseNode {
  public:
    T Elt;
    // A node can be contained in L*H squares. This vector is a place holder for
    // each node of each quad list
...
private: // Methods.
  ...
  void initSelectStamps() const {
    ...
    ++_SelectStamp;
    if (_SelectStamp == 0) {
      std::fill(_AlreadySelected.begin(), _AlreadySelected.end(),
                std::numeric_limits<uint>::max());
    }
  }

  // return the coordinates on the grid of what include the bbox.
  void selectQuads(CVector bmin, CVector bmax, sint &x0, sint &x1, sint &y0,
                   sint &y1) {
    CVector bminp, bmaxp;
    bminp = bmin;
    bmaxp = bmax;
    ...
    // Very special case where the bbox.size==0 AND position is JUST on an edge
    // of a case.
    if (x0 == x1)
      x1++;
    if (y0 == y1)
...
public:
  ...
  public:
    ...
    const T &operator*() const { return _Ptr->Elt; }
    // Doesn't work...
    /*const T*	operator->() const
        {return (&**this); }*/
    const_iterator &operator++() {
      _Ptr = (CNode *)(_Ptr->Next);
      return (*this);
    }
    ...
    bool operator==(const const_iterator &x) const { return (_Ptr == x._Ptr); }
  ...
  public:
    ...
    T &operator*() const { return this->_Ptr->Elt; }
    // Doesn't work...
    /*T*	operator->() const
        {return (&**this); }*/
    CIterator &operator++() {
      this->_Ptr = (CNode *)(this->_Ptr->Next);
      return (*this);
    }
    ...
    CIterator &operator--() {
      this->_Ptr = (CNode *)(this->_Ptr->Prev);
      return (*this);
    }
    CIterator operator--(int) {
      CIterator tmp = *this;
...
// ***************************************************************************
template <class T>
CQuadGrid<T>::CQuadGrid(uint memoryBlockSize)
    : _NodeBlockMemory(memoryBlockSize) {
  initCons();
}
// ***************************************************************************
template <class T>
CQuadGrid<T>::CQuadGrid(const CQuadGrid<T> &o)
    : _NodeBlockMemory(o._NodeBlockMemory) {
...
// ***************************************************************************
template <class T>
CQuadGrid<T> &CQuadGrid<T>::operator=(const CQuadGrid<T> &o) {
  // this==o test
  if (this == &o)
    return *this;
  ...
  // NB: the order of nodes in CNode::QuadNodes is not important (may be
  // different from src to dst)
  for (uint i = 0; i < _Grid.size(); i++) {
    ...
    while (quadSrcCur != &quadSrcRoot) {
      ...
      CNode *dstNode = NULL;
      typename std::map<const CNode *, CNode *>::iterator it =
          srcNodeToDestNode.find(srcNode);
...
// ***************************************************************************
template <class T>
typename CQuadGrid<T>::CIterator
CQuadGrid<T>::erase(typename CQuadGrid<T>::CIterator it) {
  CNode *ptr = it._Ptr;
...
template <class T>
typename CQuadGrid<T>::CIterator
CQuadGrid<T>::insert(const NLMISC::CVector &bboxmin,
                     const NLMISC::CVector &bboxmax, const T &val) {
  CVector bmin, bmax;
  ...
  sint hn = y1 - y0;
  nlassert(wn > 0 && hn > 0);
  // NB: this allocation may be slow (don't use BlockMemory system). But STLPort
  // smallblock alloc works quite well (if <128 bytes, ie a block of 10 squares)
...
template <class T> void CQuadGrid<T>::selectAll() {
  // This is the opposite of clearSelection(). get all that are in
  // _UnSelectedList, and put them in _SelectedList
...
// ***************************************************************************
template <class T>
void CQuadGrid<T>::select(const NLMISC::CVector &bboxmin,
...
// ***************************************************************************
template <class T>
void CQuadGrid<T>::buildSelectionShape(TSelectionShape &dest,
                                       const NLMISC::CPolygon2D &poly) const {
  dest.clear();
  sint minY;
  ...
  for (uint k = 0; k < numVerts; ++k) {
    ...
    _ScaledPoly.Vertices[k].set(poly.Vertices[k].x * invScale,
                                poly.Vertices[k].y * invScale);
  }
  _ScaledPoly.computeOuterBorders(_PolyBorders, minY);
  if (_PolyBorders.empty())
    return;
  initSelectStamps();
  ...
  for (sint y = 0; y < numSegs; ++y) {
    ...
    for (sint x = _PolyBorders[y].first; x <= _PolyBorders[y].second; ++x) {
      ...
      if (_AlreadySelected[index] != _SelectStamp) {
        _AlreadySelected[index] =
            _SelectStamp; // update stamp, so that won't be selected twice if
                          // there's an overlap
...
template <class T> void CQuadGrid<T>::select(const TSelectionShape &shape) {
  clearSelection();
  for (TSelectionShape::const_iterator it = shape.begin(); it != shape.end();
       ++it) {
...
// ***************************************************************************
template <class T>
void CQuadGrid<T>::selectRay(const NLMISC::CVector &rayStart,
                             const NLMISC::CVector &rayEnd) {
  clearSelection();
  CVector localRayStart = _ChangeBasis * rayStart;
  CVector localRayEnd = _ChangeBasis * rayEnd;
  float invScale = 1.f / _EltSize;
  NLMISC::CVector2f localRayStart2f(localRayStart.x * invScale,
                                    localRayStart.y * invScale);
  NLMISC::CVector2f localRayEnd2f(localRayEnd.x * invScale,
                                  localRayEnd.y * invScale);
  ...
  do {
    ...
    if (_AlreadySelected[index] != _SelectStamp) {
      ...
      NLMISC::CGridTraversal::traverse(localRayStart2f, localRayDir, x, y));
}

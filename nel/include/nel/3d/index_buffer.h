 ...
 *
 * Before the index buffer is resident (IDriver::activeIndexBuffer), it is in
 * system memory. Once the index buffer is resident, the driver creates its
 * proprietary index buffer and release the internal index buffer. At this
 * moment the index buffer can be in VRAM, AGP or system memory.
 *
 * The index buffers resident in AGP and VRAM are writeonly, i-e, you can't read
 * them after a lock(). If you change the capacity of the format of a writeonly
 * buffer, the content is lost.
 *
 * Volatile buffers must be completely filled at each pass. They are lost after
 * each swapBuffers(). They are writeonly. Volatile buffers must be resized
 * before the lock call. Only one lock per render must be done with volatile
 * buffers.
 *
 */
/* *** IMPORTANT ********************
...
public:
  ...
  setLocation(TLocation newLocation); // The driver implementation must set the
                                      // location after activeIndexBuffer.
...
public:
  ...
  /**
   * Default constructor. Make an empty index buffer. No value, no index. Index
   * color format is set to TRGBA.
   */
  CIndexBuffer(void);

  /** Make an empty index buffer. No value, no index. Index color format is set
   * to TRGBA. Set its name to the given value
   */
   ...
   *	 - for VRAM CVertexBuffer, you should write only one time, to init. You
   *can't read.
   *
   ...
   * the index buffer stay resident if it is resident.
   */
  ...
  /**
   * Reset all the indexes from memory (contReset()), so that capacity() ==
   * getNumIndexes() == 0.
   *
  ...
  /**
   * Reserve space for nIndexes indexes. You are allowed to write your indexes
   * on this space.
   *
   * If the index buffer is resident in AGP or VRAM, the data are lost.
   * The index buffer is no more resident.
  ...
  /**
   * Access indexes. Multi lock is possible only if no regions are used. Each
   * lock need an accessor to be unlocked.
   *
   * Lock the index buffer and return and fill an accessor object. Once the
   * object is destroyed, the buffer in unlocked.
   *
   * \param accessor is the accessor object to fill
   * \param first is the first index to be accessed. Put 0 to select all the
   * indexes. What ever is this index, the indexices in the index buffer remain
   * the same. \param last is the last index to be accessed + 1. Put 0 to select
   * all the indexes.
   */
  inline void lock(CIndexBufferReadWrite &accessor, uint first = 0,
                   uint last = 0);

  /**
   * Read only indexes access. Multi lock is possible only if no regions are
   ...
   * all the indexes.
   */
  inline void lock(CIndexBufferRead &accessor, uint first = 0,
                   uint last = 0) const;
...
private:
  ...
  bool checkLockedBuffer() const {
    return _LockedBuffer || (!isResident() && _NonResidentIndexes.empty());
  }
...
public:
  ...
  IIBDrvInfos(IDriver *drv, ItIBDrvInfoPtrList it, CIndexBuffer *ib) {
    _Driver = drv;
    _DriverIterator = it;
    IndexBufferPtr = ib;
  }
  ...
  /** Unlock method.
   * \param first is the index of the first indexes to update. 0 to update all
   * the indexes. \param last is the index of the last indexes to update + 1. 0
  ...
  CIndexBuffer::TFormat getFormat() const {
    nlassert(IndexBufferPtr);
    return IndexBufferPtr->getFormat();
  }
  uint getIndexNumBytes() const {
    nlassert(IndexBufferPtr);
    return IndexBufferPtr->getIndexNumBytes();
  }
};
...
public:
  ...
  CIndexBufferReadWrite() { _Parent = NULL; }
  ~CIndexBufferReadWrite() { unlock(); }
  ...
  /** Touch the updated indexes. If the method is not call, the accessor update
   * all the indexes. \param first is the index of the first indexes to update.
  ...
  CIndexBuffer::TFormat getFormat() const {
    nlassert(_Parent);
    return _Parent->getFormat();
  }
  uint getIndexNumBytes() const {
    nlassert(_Parent);
    return _Parent->getIndexNumBytes();
  }
...
public:
  ...
  CIndexBufferRead() { _Parent = NULL; }
  ~CIndexBufferRead() { unlock(); }
...
inline void CIndexBuffer::lock(CIndexBufferReadWrite &accessor, uint first,
                               uint last) {
  accessor.unlock();
...
inline void CIndexBuffer::lock(CIndexBufferRead &accessor, uint first,
                               uint last) const {
  accessor.unlock();
  accessor._Parent = this;
  ...
  if (_LockCounter == 0) {
    ...
    if (isResident() && !_KeepLocalMemory) {
      ...
      nlassertex(_Location == RAMResident,
                 ("Try to read a write only index buffer"));
      ...
      if (_NonResidentIndexes.empty())
        ...
        _LockedBuffer =
            const_cast<void *>((const void *)&(_NonResidentIndexes[0]));
    }
  } else {
    nlassert((first == 0) && (last == 0));
  }

...
template <> inline const char *CPSAttribMaker<float>::getType() {
  return "float";
}
...
public:
  ...
  CPSFloatBlender(float startFloat = 0.1f, float endFloat = 1.f,
                  float nbCycles = 1.0f)
...
public:
  ...
  CPSFloatGradient(const float *floatTab, uint32 nbValues, uint32 nbStages,
                   float nbCycles = 1.0f);
...
/** this memorize float by applying some function on the emitter. For a
...
public:
  struct CCtrlPoint {
    CCtrlPoint() {}
    CCtrlPoint(float date, float value) : Date(date), Value(value) {
      nlassert(Date >= 0 && Date <= 1);
    }
    float Date;
    float Value;
    void serial(NLMISC::IStream &f) { f.serial(Date, Value); }
  };
...
/** This return a sampled value from the hermite curb. The more steps there are,
 * the more accurate it is You can also get an 'exact value'. This must be
 * called between beginFastFloor() and endFastFloor() statements \see getValue
 */
#ifdef NL_OS_WINDOWS
  __forceinline
#endif
...
inline bool operator<(const CPSFloatCurveFunctor::CCtrlPoint &lhs,

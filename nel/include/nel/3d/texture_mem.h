...
public:
   ...
   */
  CTextureMem()
      : _Data(NULL), _Delete(false), _AllowDegradation(false),
   ...
   * \param isFile is true if the data must be interpreted as a texture file.
   * Otherwise, it is interpreted as the raw datas of the texture, so the format
   * and size of the texture must also have been set to match the raw datas
   * \param width used only if isFile is set to false
   * \param height used only if isFile is set to false
   * \param texType relevant only when isFile is set to false. Gives the format
   * to expand the texture to when it is generated.
   */
  ...
  virtual bool supportSharing() const { return !_ShareName.empty(); }
  ...
  /// Make this texture sharable by assigning it a share name. An empty share
  /// name means it is not sharable, which is the default.
  void setShareName(const std::string &shareName) { _ShareName = shareName; }

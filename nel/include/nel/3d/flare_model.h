...
public:
  ...
  enum {
    OcclusionTestFrameDelay = 2
  }; // number of frame to wait before occlusion result is collected
  /// Constructor
  CFlareModel();
  // dtor
  ~CFlareModel();
  ...
  // Debugging aid : if an occlusion test mesh is used, display it using the
  // current material
...
private:
  ...
  CScene *_Scene;
  IOcclusionQuery
      *_OcclusionQuery[MaxNumContext]
                      [OcclusionTestFrameDelay]; // delay real test by a whole
                                                 // frame to avoid any stall
  IOcclusionQuery
      *_DrawQuery[MaxNumContext]
                 [OcclusionTestFrameDelay]; // querries to retrieve the surface
                                            // that would have been drawned if
                                            // there were no occlusion
  NLMISC::CRefPtr<IDriver> _LastDrv;        // last driver used for render
  uint64 _LastRenderIntervalBegin[MaxNumContext]; // Interval of frames during
                                                  // which this flare was
                                                  // traversed for render
  uint64 _LastRenderIntervalEnd[MaxNumContext];
  uint64 _NumFrameForOcclusionQuery[MaxNumContext]; // number of frames that
                                                    // were necessary to get the
...
public:
  ...
  /** Update state of occlusion query at the end of the frame. For private use
   * by the CScene class updateOcclusionQueryBegin() should have been called
   */
  void updateOcclusionQuery(IDriver *drv);
  /** Render all primitives of a mesh using the current material / metrix etc
   * ... NB : this does NOT activate the vertex buffer of the mesh it must be
   * activated prior to the call
   */

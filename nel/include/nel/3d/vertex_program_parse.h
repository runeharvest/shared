 ...
 * ---------------------------------
 *
 * To make your program compatible with D3D and OPENGL nel drivers, please
 * follow thoses directives to write your vertex programs
 *
 * - Use only v[0], v[1] etc.. syntax for input registers. Don't use v0, v1 or
 * v[OPOS] etc..
 * - Use only c[0], c[1] etc.. syntax for constant registers. Don't use c0, c1
 * etc..
 * - Use only o[HPOS], o[COL0] etc.. syntax for output registers. Don't use
 * oPos, oD0 etc..
 * - Use only uppercase for registers R1, R2 etc.. Don't use lowercase r1, r2
 * etc..
 ...
 * -> Direct3D driver implementation will have to modify the syntax on the fly
 * before the setup like this:
 ...
 *
 * Behaviour of LOG may change depending on implementation: You can only expect
 * to have dest.z = log2(abs(src.w)). LIT may or may not clamp the specular
 * exponent to [-128, 128] (not done when EXT_vertex_shader is used for example
 * ..)
 *
 * Depending on the implementation, some optimizations can be achieved by
 * masking the unused output values of instructions as LIT, EXPP ..
 *
...
struct CVPSwizzle {
  ...
  bool isScalar() const {
    return Comp[0] == Comp[1] && Comp[0] == Comp[2] && Comp[0] == Comp[3];
  }
  // Test if no swizzle is applied
  bool isIdentity() const {
    return Comp[0] == X && Comp[1] == Y && Comp[2] == Z && Comp[3] == W;
  }
};
...
struct CVPOperand {
  ...
  enum EOutputRegister {
    ...
    OPrimaryColor,
    OSecondaryColor,
    OBackFacePrimaryColor, // warning : backface colors are not supported on all
                           // implementations
  ...
  union {
    ...
    sint ConstantValue; // Index from 0 to 95, or -64 to +63 for constants with
                        // displacement
  } Value;
...
public:
   ...
   * \warning: Only syntax is checked. It doesn't check that a register has been
   * initialised before use. \param src The input text of a vertex program, in
   * OpenGL format. \param result The result program. \param errorOutput If
   * parsing failed, contains the reason \result true if the parsing succeeded
   */
  bool parse(const char *src, TProgram &result, std::string &errorOutput);

  /** Debugging purpose : This output a parsed vertex program in a string, with
   * the standard format. This can serve as a base for other format code
   * generation
   */
  static void dump(const TProgram &prg, std::string &dest);

  // test if a specific input is used by a vertex program
  static bool isInputUsed(const TProgram &prg,
...
private:
  bool parseOperand(CVPOperand &operand, bool outputOperand,
  ...
  //
  bool parseInstruction(CVPInstruction &instr, std::string &errorOutput,
                        bool &endEncountered);

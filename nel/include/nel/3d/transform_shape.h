...
// ***************************************************************************
// ClassIds.
const NLMISC::CClassId TransformShapeId =
    NLMISC::CClassId(0x1e6115e6, 0x63502517);
...
public:
  ...
  /** get an approximation of the number of triangles this instance want render
  ...
  /** get an approximation of the number of triangles this instance should
   * render. This method is valid only for IShape classes (in render()), after
   * LoadBalancing traversal is performed. NB: It is not guaranted that this
   * instance will render those number of triangles.
   */
  float getNumTrianglesAfterLoadBalancing() {
    return _NumTrianglesAfterLoadBalancing;
  }

  /// If the model support MRM, return the level detail setup. default is return
  /// NULL.
...
protected:
   ...
   *(as skeleton). Must call changeLightSetup() so change are effectively made
   *in driver
   */
  void setupCurrentLightContribution(CLightContribution *lightContrib,
                                     bool useLocalAtt);
...
private:
  /* The Activated lightContribution, and localAttenuation setup for this
     instance. This may be our lightContribution, or our ancestore skeleton
     contribution.
  */

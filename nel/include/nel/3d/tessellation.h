...
public:
  ...
  bool onBorder() const {
    return (S == 0 || S == 0x8000 || T == 0 || T == 0x8000);
  }
};
...
public:
  ...
  // Geomorph Information, for VertexProgram geomorph. NB: no RefineThreshold
  // dependency here.
  float MaxFaceSize;  // max(SizeFaceA, SizeFaceB)
  float MaxNearLimit; // max(NearLimitFaceA, NearLimitFaceB) with NearLimitFace=
                      // (1<<Patch->TileLimitLevel) / (1<<Level);
  ...
  // Compute the geomorphed position, with MaxFaceSize, MaxNearLimit ..., and
  // current globals.
  void computeGeomPos();
};
...
// ***************************************************************************
/** A tileface. There is one TileFace per pass (BUT Lightmap PASS!! same used
 * for RGB0 and LIGHTMAP).
 *
 */
...
struct CTileMaterial {
  ...
  //  3: additive pass.
  //	4: Cloud*Lightmap.
  // NB: BIG TRICK: the pass 0 has 2 Uvs: Uv0: RGB, and Uv1: Lightmap!!!! even,
  // if they are not used at the same pass :o). This simplifies, save memory,
  // and save from copies of v3f. NB: RENDER ORDER: CLOUD*LIGHTMAP is done
  // BEFORE ADDITIVE.

  // The list of faces, for each pass. NB: no faces for lightmaps!! since share
  // same face/vertices from RGB0. The only thing not shared is the renderpass
  // (Pass[0] and Pass[3] are different).
  ...
  // For fast Tile clipping, may add only tiles which are visibles in
  // RenderPass. (see preRender()).
  void appendTileToEachRenderPass(uint patchNumRenderableFaces);

  // Render of this tile. code in patch_render.cpp. use pass to acces both
  // Pass[] and TileFaceVectors[]. So don't work for lightmap.
...
public:
  ...
  /// \name Error metric.
  // @{
  // Size could be computed from TileSize: Size= Patch->BaseSize /
  // (1<<(TileSize-Patch->BaseTileSize)). (or optimized) But used by
  // updateErrorMetric(), and must be as fast as possible.
  ...
  float ErrorMetric;  // equal to projected size of face, but greater for the
                      // transition Far-Near.
...
public:
  ...
  bool hasVertex(CTessVertex *v) const {
    return VBase == v || VLeft == v || VRight == v;
  }
  ...
  // Fill all Tile Material infos (id, type...), according to CPatch tilemap,
  // and CTessFace paramcoord.
  void computeTileMaterial();
  // Release Tile Uvs.
  void releaseTileMaterial();

  // compute the NearLimit for this face. NearLimit= (1<<Patch->TileLimitLevel)
  // / (1<<Level)
  ...
  void refineAll();
  // update the refinement of the node, split if necessary (and then
  // updateRefine() his splitted sons), Then re-insert this face in the
  // appropriate CLandscape priority list.
  ...
  void forceMerge();
  // Used by CPatch::linkTessFaceWithEdge(). See it. NB: it assert if the link
  // is NULL.
  ...
  // Used by CPatch::forceMergeAtTileLevel(). force the merging of face (as
  // possible).
  void forceMergeAtTileLevel();

  // Used by CPatch::averageTesselationVertices(). See CLandscape doc. recurs
  // call.
...
private:
  ...
  static bool sameTile(const CTessFace *a, const CTessFace *b) {
    // WE CANNOT do this test, testing TileMaterial, because this one may be
    // releleased by releaseMaterial() !!
    return (a->Patch == b->Patch && a->TileId == b->TileId);
  }

  /// \name UV mgt.
  // @{
  // Allocate a CTessNearVertex "id" (base, left or right) for each not NULL
  // TileFace of "this" face. Init with good CTessVertex::Src, and then, insert
  // it into Patch RenderList.
  void allocTileUv(TTileUvId id);
  // delete a CTessNearVertex "id", removing it from Patch RenderList, for each
  // not NULL TileFace of "this" face.
  void deleteTileUv(TTileUvId id);
  // Just ptr-copy a CTessNearVertex "id" from another face/vertex id. Do this
  // for each not NULL TileFace.
  void copyTileUv(TTileUvId id, CTessFace *srcFace, TTileUvId srcId);

  // The Base NearVertex must be allocated before.
  // for each not NULL TileFace of "this" face, make the Base NearVertex the
  // middle of baseFace TileUvLeft, and baseFace TileUvRight.
  void heritTileUv(CTessFace *baseFace);

  // If patch !isRenderClipped(), and other state, create and fill VB for the
  // vertex id of all faces of this tileFace.
  void checkCreateFillTileVB(TTileUvId id);
  // If patch !isRenderClipped(), and other state, fill VB only for the vertex
  // id of all faces of this tileFace.
  void checkFillTileVB(TTileUvId id);

  // create The TileFaces, according to the TileMaterial passes. NO INSERTION IN
  // RENDERLIST!!
  void buildTileFaces();
  // delete The TileFaces, according to the TileMaterial passes. NO REMOVAL FROM
  // RENDERLIST!!
  ...
  // see updateRefine().
  float computeTileEMForUpdateRefine(float distSplitPoint, float distMinFace,
                                     float nearLimit);
  ...
  void initTileUvRGBA(sint pass, bool alpha, CParamCoord pointCoord,
                      CParamCoord middleCoord, CUV &uv);
  // The same, but for the lightmap pass.
  void initTileUvLightmap(CParamCoord pointCoord, CParamCoord middleCoord,
                          CUV &uv);
...
public:
  // The fake face which indicates a multiple patch face. Used in updateBind(),
  // for multiple bind.

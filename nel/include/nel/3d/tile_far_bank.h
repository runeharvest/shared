...
#define NL_NUM_PIXELS_ON_FAR_TILE_EDGE_SHIFT                                   \
  2 // a 128x128 far tile is a 4x4 bitmap in far 0
#define NL_NUM_PIXELS_ON_FAR_TILE_EDGE                                         \
  (1 << NL_NUM_PIXELS_ON_FAR_TILE_EDGE_SHIFT) // a 128x128 far tile is a 4x4
                                              // bitmap in far 0
...
public:
  ...
  public:
    ...
    /// Return the pointer on the pixels data. Call this method only if isFill
    /// () returns true.
    ...
    /// Return the pixel array size. Should be 0 for empty, 64 for a 128x128
    /// tile and 256 for a 256x256 tile.
    uint getSize(TFarType type, TFarOrder order) const {
      return (uint)_Pixels[type][order].size();
    }

    /// Set the pixel array of a far Tile
    void setPixels(TFarType type, TFarOrder order, NLMISC::CRGBA *pixels,
                   uint size);
  ...
  sint getNumTile() const { return (sint)_TileVector.size(); }

  /// Resize the tile bank
  void setNumTile(sint numTile) { _TileVector.resize(numTile); }
  ...
  /// Get the tile size
  static uint getFarTileEdgeSize(TFarOrder order) { return 4 >> (uint)order; }

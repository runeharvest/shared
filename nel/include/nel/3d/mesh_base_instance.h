...
// ***************************************************************************
// ClassIds.
const NLMISC::CClassId MeshBaseInstanceId =
    NLMISC::CClassId(0xef44331, 0x739f6bcf);

// ***************************************************************************
/**
 * An base class for instance of CMesh and CMeshMRM  (which derive from
 * CMeshBase). NB: this class is a model but is not designed to be instanciated
 * in CMOT. \author Lionel Berenguier \author Nevrax France \date 2001
 */
...
public:
   ...
   * Each CMeshBaseInstance has those materials, so they can be animated or
   * modified for each instance. By default, they are copied from the Mesh.
   */
   ...
   *	WARNING: once AsyncTextureMode is set, Material's Texture fields should
   *not be modified, else undefined results
   */
   ...
   *MeshMultiLod apply it only on Lod0. NB: This apply to the shape direclty!!
   *ie All instances using same shape will be affected NB: no-op if
   *distanceFinest<0, distanceMiddle<=distanceFinest or if
  ...
  /** If there are selectable texture in this mesh shape, this replace the
   *matching material instances with the right texture If
   *getAsyncTextureMode()==true, then this replace the AsyncTexture fileNames,
   *instead of the Materials file Names.
   */
   ...
   *	return always true if not in async texture mode, or if
   *startAsyncTextureLoading() has not been called since last
   ...
   *	NB: the id returned is the one in _CurrentAsyncTexture it the valid ones
   *(thoses loaded or being loaded) Can be used for some (non deleting) request
   *to the AsyncTextureManager
   */
...
public:
  ...
  bool _VPWindTreeFixed;  // Enable lighting for mesh vp wind tree material when
                          // no vp are available (else vertex color is used to
                          // tint the object ...)
};

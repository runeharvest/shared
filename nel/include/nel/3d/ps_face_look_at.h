...
public:
  /** create the face look at by giving a texture. This can't be a
   * CTextureGrouped (for animation) animation must be set later by using
   * setTextureScheme
   */
   ...
   *  \param coeff a coefficient for motion blur (too high value may give
   * unrealistic result) 0 deactivate the motion blur \param threshold : speed
   * will be clamped below this value
   */
  ...
  /** Setting this to true allows to have independant height and width for these
   * particles. The interface to manage the second size can be obtained from
   * getSecondSize(), which correspond to the height of particles. The default
   * is to not have independant sizes
   */
  ...
  // Force faces to be aligned on motion. This bypass motion blur / align on z
  // axis
...
protected:
  ...
  // in this struct we defines the getSizeOwner method, which is abstract in the
  // CPSSizedParticle clas

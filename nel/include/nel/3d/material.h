...
const uint32 IDRV_TOUCHED_TEX[IDRV_MAT_MAXTEXTURES] = {0x00010000, 0x00020000,
...
const uint32 IDRV_MAT_DEFMAT =
    0x00000040; // NB: Deprecated, but may still exist streams.
...
public:
  IMaterialDrvInfos(IDriver *drv, ItMatDrvInfoPtrList it) {
    _Driver = drv;
    _DriverIterator = it;
  }
...
public:
   ...
   *	blendConstant* enums are only valid if
   *driver->supportBlendConstantColor(). \see
   ...
   *	- not implemented yet.
   * UserColor:
   *	- UserColor (see setUserColor()) is blended with precomputed
   *texture/textureAlpha.
   *	- Alpha Blending ignore Alpha of texture (of course :) ), but use Alpha
   *diffuse (vertex/material color). LightMap:
   *	- Texture of stage 0 is blended with sum of lightmaps (see
   *setLightmap()). Vertex Color (or color, or lighting) doesn't affect the
   *final result (neither diffuse part nor specular part). Blending is special.
   *If enabled, Lightmap shader apply a standard transparency
   *srcalpha/invsrcalpha.
   *	- NB: if no texture in stage 0, undefined result.
   *	- UV0 is the UV for decal Texture. UV1 is the UVs for all the lightmaps.
   * Specular:
   *  - Texture of stage 0 is added to the multiplication of Texture Alpha of
   *stage 0 and Texture of stage 1
   *  - This is done in 2 passes
   * PerPixelLighting :
   *  - When not supported by the driver, this is equivalent to the normal
   *shader. This can be querried from the driver
   *  - When supported by the driver, the strongest light is rendered using per
   *pixel lighting. The last tex coordinate must be the S vector of the tangent
   *space basis (oriented in the direction where the s texture coords grows).
   *Other lights are rendered using gouraud shaing. The light setup is done in
   *the driver. PerPixelLighting : The same as PerPixelLighting but with no
   *specular Caustics: NOT IMPLEMENTED Cloud :
   * - Alpha of texture in stage 0 is blended with alpha of texture in stage 1.
   *Blend done with the alpha color of each stage and the whole is multiplied by
   *the alpha in color vertex [AT0*ADiffuseCol+AT1*(1-ADiffuseCol)]*AStage
   * - RGB still unchanged
   * Water :
   * - Water
   */
   ...
   * Interpolate*:	out= arg0*As + arg1*(1-As),  where As is taken from the
   *SrcAlpha of Texture/Previous/Diffuse/Constant, respectively if operator is
   ...
   *(see IDriver::supportMADOperator) EMBM : apply to both color and alpha : the
   *current texture, whose format is DSDT, is used to offset the texture in the
   *next stage, unless the EMBM unit is at the last stage, in which case it
   *operates on texture at first stage NB : for EMBM, this must be supported by
   *driver.
   */
  enum TTexOperator {
    Replace = 0,
   ...
   * Previous:	the arg is taken from the previous enabled stage. If stage 0,
   * Previous==Diffuse. Diffuse:		the arg is taken from the
   * primary color vertex. Constant:	the arg is taken from the constant color
   * setuped for this texture stage.
   */
  ...
  /** \name Texture Addressing Modes. They are valid only with the normal
   *texture shader. All modes are not supported everywhere, so you should check
   *for it in the driver. The modes are similar to those introduced with
   *DirectX 8.0 Pixel Shaders and OpenGL TEXTURE_SHADERS_NV
   */
  // @{
  enum TTexAddressingMode {
    TextureOff = 0 /* no texture */,
    FetchTexture,
    PassThrough,
    CullFragment,
    OffsetTexture,
   ...
   *	TexCoordGenObjectSpace: The UVW are generated from the XYZ defined in
   *ObjectSpace (before transformation) TexCoordGenEyeSpace: The UVW are
   *generated from the XYZ defined in EyeSpace (after ModelViewMatrix
   *transformation) NB: use the TextureMatrix for more control on the wanted
   *effect (eg: shadowMap projection etc...)
   */
  enum TTexCoordGenMode {
    TexCoordGenReflect = 0,
...
public:
  ...
  // @{
  /** ctor.
   * By default, shader is normal, SrcBlend is srcalpha, dstblend is
   * invsrcalpha, ZFunction is lessequal, ZBias is 0, Color is White:
   * (255,255,255,255), not double sided.
   */
  CMaterial();
  /// see operator=.
  CMaterial(const CMaterial &mat) : CRefCount() {
    _Touched = 0;
    _Flags = 0;
    operator=(mat);
  }
   ...
   *	- UserColor assert if stage!=0. (NB: internal only: UserColor setup
   *texture to stage 0 and 1).
   ...
   * Texture addressing modes should have been enabled otherwise an assertion is
   * raised
   */
  ...
  /// The bias is gived in world coordinate units. Positive bias give a lower z
  /// priority, negative bias give a higher bias priority.
  ...
  /// \name Color/Lighting..
  // @{
  /// The Color is used only if lighting is disabled. Also, color is replaced by
  /// per vertex color (if any).
  void setColor(CRGBA rgba);

  /// Batch setup lighting. Opacity is in diffuse.A.
  void setLighting(bool active, CRGBA emissive = CRGBA(0, 0, 0),
                   CRGBA ambient = CRGBA(0, 0, 0),
                   CRGBA diffuse = CRGBA(0, 0, 0),
                   CRGBA specular = CRGBA(0, 0, 0), float shininess = 10);

  /// Set the emissive part ot material. Useful only if setLighting(true) has
  /// been done.
  void setEmissive(CRGBA emissive = CRGBA(0, 0, 0));
  /// Set the Ambient part ot material. Useful only if setLighting(true) has
  /// been done.
  void setAmbient(CRGBA ambient = CRGBA(0, 0, 0));
  /// Set the Diffuse part ot material. Useful only if setLighting(true) has
  /// been done. NB: opacity is NOT copied from diffuse.A.
  void setDiffuse(CRGBA diffuse = CRGBA(0, 0, 0));
  /// Set the Opacity part ot material. Useful only if setLighting(true) has
  /// been done.
  void setOpacity(uint8 opa);
  /// Set the specular part ot material. Useful only if setLighting(true) has
  /// been done.
  void setSpecular(CRGBA specular = CRGBA(0, 0, 0));
  /// Set the shininess part ot material. Useful only if setLighting(true) has
  /// been done.
  void setShininess(float shininess);
  /// Set the color material flag. Used when the material is lighted. True to
  /// use the diffuse color of the material when lighted, false to use the color
  /// vertex.
  ...
  /// Return true if this material uses color material as diffuse when lighted,
  /// else return false if it uses color vertex.
  ...
  /** This part is valid for Normal shaders (nlassert). It maps the
   * EXT_texture_env_combine opengl extension. A stage is enabled if his texture
   * is !=NULL. By default, all stages are setup to Modulate style:
   *  AlphaOp=RGBOp= Modulate, RGBArg0= TextureSrcColor, RGBArg1=
   * PreviousSrcColor, AlphaArg0= TextureSrcAlpha, AlphaArg1= PreviousSrcAlpha.
   * ConstantColor default to White(255,255,255,255).
   *
   * For compatibility problems:
   * - no scaling is allowed (some cards do not implement this well).
   * - Texture can be the source only for Arg0 (DirectX restriction).
   * nlassert...
   *
   * NB: for Alpha Aguments, only operands SrcAlpha and InvSrcAlpha are valid
   * (nlassert..).
   */
  // @{
  ...
  // Get alpha operation at the given stage. Must use Normal shader or
  // nlassert(0)
  ...
  TTexCoordGenMode getTexCoordGenMode(uint stage) const {
    return (TTexCoordGenMode)((_TexCoordGenMode >>
                               (stage * IDRV_MAT_TEX_GEN_SHIFT)) &
                              IDRV_MAT_TEX_GEN_MASK);
  }

  // Enable a user texture matrix for the n-th stage. The initial matrix is set
  // to identity.
   ...
   * User texture matrix must be enabled for that stage, otherwise an assertion
   * is raised.
   */
  const NLMISC::CMatrix &getUserTexMat(uint stage) const;
  /// Decompose a user texture matrix, We assume that this is only a matrix for
  /// 2d texture.
  void decompUserTexMat(uint stage, float &uTrans, float &vTrans, float &wRot,
  ...
  /// Set the multiply x 2 mode to burn colors (used with lightmaps 8 bits)
  /// (def: false)
  void setLightMapMulx2(bool val) { _LightMapsMulx2 = val; }
  /// Used for LightMap Compression (LMC). Set the LMC color terms. default to
  /// black/white (no compression)
   ...
   * Default to: normal shader, no lighting, color to White(1,1,1,1), no
   * texture, ZBias=0, ZFunc= lessequal, ZWrite==true, no blend. All other
   * states are undefined (such as blend function, since blend is disabled).
   */
  void initUnlit();
  /** Init the material as default white lighted material. normal shader,
   * lighting .... Default to: normal shader, full black lighting, no texture,
   * ZBias=0, ZFunc= lessequal, ZWrite==true, no blend. All other states are
   * undefined (such as blend function, since blend is disabled).
   */
  void initLighted();
  // @}
   ...
   * This is useful only if textures of this material support selection of
   * course (such as CTextureMultiFile)
   */
  void selectTextureSet(uint index);
  // @}

  /** test if material a driver supports rendering of that material
   * \param  forceBaseCaps When true, the driver is considered to have the most
   * basic required caps (2 stages hardwares, no pixelShader, support for
   * constant color blend & multiply-add texture operator), so that any fancy
   * material will fail the test.
   */
...
public:
  ...
  struct CTexEnv {
    ...
    void setDefault() {
      ...
      // We cannot do better, because bit fields ordeinrg seems not to be
      // standardized, so we can not set Packed directly.
      Env.OpRGB = Modulate;
    ...
    inline uint getColorArg(uint index) {
      switch (index) {
      case 0:
        return Env.SrcArg0RGB;
      case 1:
        return Env.SrcArg1RGB;
      case 2:
        return Env.SrcArg2RGB;
      default:
        nlassert(0);
        break;
      }
    ...
    inline uint getAlphaArg(uint index) {
      switch (index) {
      case 0:
        return Env.SrcArg0Alpha;
      case 1:
        return Env.SrcArg1Alpha;
      case 2:
        return Env.SrcArg2Alpha;
      default:
        nlassert(0);
        break;
      }
    ...
    inline uint getColorOperand(uint index) {
      switch (index) {
      case 0:
        return Env.OpArg0RGB;
      case 1:
        return Env.OpArg1RGB;
      case 2:
        return Env.OpArg2RGB;
      default:
        nlassert(0);
        break;
      }
    ...
    inline uint getAlphaOperand(uint index) {
      switch (index) {
      case 0:
        return Env.OpArg0Alpha;
      case 1:
        return Env.OpArg1Alpha;
      case 2:
        return Env.OpArg2Alpha;
      default:
        nlassert(0);
        break;
      }
...
public:
  ...
  uint8 _TexAddrMode[IDRV_MAT_MAXTEXTURES]; // texture addressing enum packed as
                                            // bytes

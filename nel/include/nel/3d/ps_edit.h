...
/**
 * In this file, we define interfaces to interact more precisely with located in
 * a particle system It allows to manipulate individual elements of a located
...
/**
 * This interface allow to move a particle system element given its index in a
 * located \author Nicolas Vizerie \author Nevrax France \date 2001
 */
...
struct IPSMover {
  ...
  virtual bool supportUniformScaling(void) const {
    NL_PS_FUNC(supportUniformScaling);
    return false;
  }

  /** Send back true if non-uniform scaling can be applied
   *  If it sends false, non-uniform scaling leads to undefine results (default
   * has no effect)
   */
  virtual bool supportNonUniformScaling(void) const {
    NL_PS_FUNC(supportNonUniformScaling);
    return false;
  }
  ...
  virtual NLMISC::CVector getScale(uint32 /* index */) const {
    NL_PS_FUNC(getScale);
    return NLMISC::CVector(1.f, 1.f, 1.f);
  }

  /** some object may not store a whole matrix (e.g planes)
   *  this return true if only a normal is needed to set the orientation of the
   * object
   */
  virtual bool onlyStoreNormal(void) const {
    NL_PS_FUNC(onlyStoreNormal);
    return false;
  }

  ///  if the object only needs a normal, this return the normal. If not, is
  ///  return (0, 0, 0)
  virtual NLMISC::CVector getNormal(uint32 /* index */) {
    NL_PS_FUNC(getNormal);
    return NLMISC::CVector::Null;
  }

  /// if the object only stores a normal, this set the normal of the object.
  /// Otherwise it has no effect
  virtual void setNormal(uint32 /* index */, NLMISC::CVector /* n */) {
    NL_PS_FUNC(setNormal);
  }

...
/**
 * The same as an ordering table, but it allows to have several layers for the
 * display.
 * - Layer 0 acts as an ordering table.
 * - Layer 1 is a simple vector, with no sort.
 * - Layer 2 acts as an ordering table.
 * Typically, this can be used to sort objects that are above or below  a
 * surface (which fit in layer 1).
 */
template <class T> class CLayeredOrderingTable {
public:
  /// ctor
   ...
   * \param entry pos The position for the ordering tables. It is ignored when
   * the layer is 1.
   */
  void insert(uint layer, T *pValue, uint32 nEntryPos = 0);

  /** Share allocator between 2 or more layered ordering tables. So that calling
   * reset will give the max number of insert for both tables. This is useful if
   ...
   *	}
   *
   * \param forwardTraversal true to traverse from layer 0 to layer 2 and vice
   *versa.
   */
...
CLayeredOrderingTable<T>::CLayeredOrderingTable()
    : _IndexInLayer1(0), _CurrLayer(0), _ForwardTraversal(true) {
  // share allocator between layer 0 and 2
  _Layer2.shareAllocator(_Layer0);
}

//==================================================================
template <class T> void CLayeredOrderingTable<T>::init(uint32 nNbEntries) {
  _Layer0.init(nNbEntries);
  _Layer2.init(nNbEntries);
}

//==================================================================
template <class T> uint32 CLayeredOrderingTable<T>::getSize() {
...
//==================================================================
template <class T>
void CLayeredOrderingTable<T>::shareAllocator(
    CLayeredOrderingTable<T> &sourceTable) {
...
//==================================================================
template <class T>
void CLayeredOrderingTable<T>::insert(uint layer, T *pValue,
                                      uint32 nEntryPos /* = 0 */) {
  switch (layer) {
  case 0:
...
//==================================================================
template <class T> inline T *CLayeredOrderingTable<T>::get() {
  switch (_CurrLayer) {
  case 0:
    return _Layer0.get();
    break;
...
//==================================================================
template <class T> inline void CLayeredOrderingTable<T>::next() {

...
public:
  ...
  void init(const CVector &center, CVegetableManager *vegetManager,
            CVegetableClipBlock *vegetableClipBlock, CPatch *patch, uint ts,
            uint tt, CTessList<CLandscapeVegetableBlock> &list);

  /// release all IGs, reset the Vegetable block and remove from list. call
  /// before destruction.
  void release(CVegetableManager *vegeManager,
               CTessList<CLandscapeVegetableBlock> &list);

  /** update state of the vegetableBlock. If distance type change, then create /
   *delete any wanted Ig. Warning! IG creation Use OptFastFloor()! So call must
   *be enclosed with a OptFastFloorBegin()/OptFastFloorEnd().
   */
...
private:
  ...
  uint8 _Ts, _Tt;
  /// The current distance type. 0 means all IG are created.
  /// NL3D_VEGETABLE_BLOCK_NUMDIST means none.
  uint8 _CurDistType;

  /// The sortBlocks generated by this vegetable block, one for each
  /// tilematerial
  CVegetableSortBlock *_VegetableSortBlock[NL3D_TESSBLOCK_TILESIZE];

  /// The instance groups generated by this vegetable block, one for each
  /// distance type. and one per each tilematerial
  CVegetableInstanceGroup
      *_VegetableIG[NL3D_TESSBLOCK_TILESIZE][NL3D_VEGETABLE_BLOCK_NUMDIST];

  void createVegetableIGForDistType(

...
private:
  ...
  class CSlot {
    ...
    /// Is the slot empty ?
    bool isEmpty() { return _Animation == NULL; }
  ...
  private:
     ...
     * This weight can be given in any range because renormalisation is done in
     * final weight evaluation. If weight is 0.f, the final mix is not
     * influenced by the animation of this slot.
     */
    float _Weight;
  ...
  /**
   * An animated channel of the CChannelMixer. This class is used internaly in
   * the CChannelMixer.
   *
   * \author Cyril 'Hulud' Corvazier
   * \author Nevrax France
  ...
  private:
    ...
    /// Name of the channel in the channel mixer. Must be the same than the
    /// animated value name.
    std::string _ChannelName;

    /// A pointer on the IAnimatable object that handles the channel value. (ref
    /// ptr to ensure Animated value access)
    NLMISC::CRefPtr<IAnimatable> _Object;

    /// A pointer on the IAnimatedValue animated by this channel. If NULL, the
    /// channel is empty
    ...
    /// The id of the OwnerBit to touch the IAnimatable object. Useful for
    /// IAnimatable derivation.
    uint32 _OwnerValueId;

    /// The default track pointer used when track are missing in the animation.
    /// Can't be NULL.
    const ITrack *_DefaultTracks;

    /**
     * A track pointer on each slot CAnimation. Can't be NULL. If no track found
     * for this channel, the pointer is _DefaultTracks.
     */
    const ITrack *_Tracks[NumAnimationSlot];

    /**
     * A weight array for to blend each slot.
     * This value must be between 0.f and 1.f. If it is 0.f, the slot is not
     * used. If it is 1.f, the slot is used at 100%. This weight can be set
     * using a "skeleton template weight". Default value is 1.f.
     */
    float _Weights[NumAnimationSlot];

    /**
     * Pointer on the next channel selected for the animations selected in the
     * slots
     *
     * This list is used to only visit the channels animated by the animations
     * set in the slots of the mixer
     */
    CChannel *_Next;
  };
...
public:
   ...
   * The pointer is hold by the channel mixer until it changes. Return NULL if
   * no animationSet defined.
   */
   ...
   *
   * This is the main method. It evals animations selected in the slots for
   *listed channels.
   *
   * Only the channels that are animated by animations selected in the slots are
   *evaluated. They are stored in a linked list managed by the channel array.
   *
   * Others are initialized with the default channel value.
   *
   * \param detail true if eval the detail part of animation. (done after
   *clipping). \param evalDetailDate chann mixer store the last date of anim
   *detail evaluated. if same, do nothing, else if < or >, compute the anim.
   *ingored if detail is false.
   */
   ...
   * \param animatable is a pointer on the IAnimatable object in which the value
   *is stored. It will be kept by the CChannelMixer until it is removed from the
   *channel. \param value is a pointer on the value the channel works with. It
   *will be kept by the CChannelMixer until it is removed from the channel.
   * \param defaultValue is a track used by default if a track is not presents
   *in the animation for this channel. It will be kept by the CChannelMixer
   *until it is removed from the channel. \param valueId is the value ID in the
   *IAnimatable object. \param ownerId is the owner Bit of the animated vlaue,
   *in the IAnimatable object. touched when the animatedvalue is touched. \param
   *detail true if this channel must be evaluated in detail mode (see eval()).
   * \return -1 if the track was not found in the animationSet, else it return
   *the channelId as if returned by
   *CAnimationSet::getChannelIdByName(channelName).
   */
  sint addChannel(const std::string &channelName, IAnimatable *animatable,
                  IAnimatedValue *value, ITrack *defaultValue, uint32 valueId,
                  uint32 ownerValueId, bool detail);

  /// Reset the channel list if the mixer. All channels are removed from the
  /// mixer.
  void resetChannels();

  /** disabling a channel means it is no more modified during animation. Default
   *is enabled. NB: this channel must have been added (via addChannel()....).
  ...
  /** Same as enableChannel but for Animation Lod system. The channel is
   *animated only if both enableChannel() and lodEnableChannel() are true.
   *Default is enabled. NB: this channel must have been added (via
   *addChannel()....). \param channelId channelId get from
   *CAnimationSet::getChannelIdByName() or addChannel()
   */
   ...
   * be visited to check if they are used by the new animation. If they are,
   * they will be linked in the internal CChannel list.
   *
   * \param slot is the slot number to change the animation. Must be >= 0 and <
   * NumAnimationSlot. \param animation is the new animation index in the
   * animationSet use by this slot. \see CAnimationSet, CAnimation
   */
   ...
   *
   * \param slot is the slot number to change the time. Must be >= 0 and <
   * NumAnimationSlot. \param time is the new time to use in the slot. \see
   * TAnimationTime
   */
   ...
   * Each slot can have different weight. Calling this method won't dirt the
   *mixer.
   *
   *	By default the weight of the slot is 1.0f.
   *
   * \param slot is the slot number to change the weight.
   * \param weight is the new weight to use in the slot. No range for this
   *weight. If the weight == 0.f, the slot have no effect on the final mix.
   */
   ...
   *
   * Calling this method will dirt the mixer, ie, all the mixer's channels will
   * be visited to check if they are used by the old animation. If they are,
   * they will be linked in the internal CChannel list.
   *
   * Warning: this method will assign the default value in the animated value
   * that are removed from the active channel queue.
   *
   * \param slot is the slot number to empty. Must be >= 0 and <
   * NumAnimationSlot.
   */
   ...
   *
   * This method apply the weight of each node contains in skelWeight to the
   * channel's slot weight.
   *
   * \param slot is the slot number to empty. Must be >= 0 and <
   * NumAnimationSlot. \param skeleton is the index of a skeleton in the
   * animationSet. \param invert is true if the weights to attach to the
   * channels are the weights of the skeleton template. false if the weights to
   * attach to the channels are the 1.f-weights of the skeleton template.
   */
  ...
  /// reset to -1 the evalDetailDate. Hence next eval(true,..) will be forced to
  /// compute
...
private:
  ...
  // The first Global channel. If NULL, no channel to animate.  (animed in
  // eval(false))
  CChannel *_FirstChannelGlobal;

  // The first detail channel. If NULL, no channel to animate.  (animed in
  // eval(true))
   ...
   * \param activeSlot array of contiguous slots ids (there are 'numActiveSlots'
   * of them)
   */
  void evalSingleChannel(CChannel &chan, uint numActiveSlots,

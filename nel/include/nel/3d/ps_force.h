...
public:
  ...
  /// Override of CPSLocatedBindable::doesProduceBBox. forces usually are not
  /// part of the bbox
  ...
  /// Serial the force definition. MUST be called by deriver during their
  /// serialisation
  ...
  /// inherited from   CPSLocatedBindableTarget, we use that to tell whether
  /// this force is integrable or not
  virtual void attachTarget(CPSLocated *ptr);

  /// inherited from   CPSLocatedBindableTarget
  void releaseTargetRsc(CPSLocated *target);

  /** Integrate this force on the given located. If 'accumulate' is set to true,
   * it just add the effect of this force on position otherwise, it must also
   * integrate from the initial speed, and add this force effect. The first call
   * to this must be done with 'accumulate' set to false. NB : works only with
   * integrable forces
   */
  virtual void
  integrate(float /* date */, CPSLocated * /* src */, uint32 /* startIndex */,
            uint32 /* numObjects */, NLMISC::CVector * /* destPos */ = NULL,
            NLMISC::CVector * /* destSpeed */ = NULL,
            bool /* accumulate */ = false,
            uint /* posStride */ = sizeof(NLMISC::CVector),
            uint /* speedStride */ = sizeof(NLMISC::CVector)) const {
    nlassert(0); // not an integrable force
  }

  /** Compute a trajectory on several steps for a single object, rather than a
   * step for several object. If the start date is lower than the creation date,
   * the initial position is used NB : works only with integrable forces
   */
  virtual void
  integrateSingle(float /* startDate */, float /* deltaT */, uint /* numStep */,
...
protected:
  ...
  /// if this force has become integrable again, this method tells it to the
  /// target
  void renewIntegrable(void);

  /** inherited from	 CPSLocatedBindable. When we deal with integrable
   * forces, they must be in the same basis than their target. If this change,
   * we must notify the target of it.
   */
...
/// this is a class to set force instensity (acceleration for gravity, k
/// coefficient for springs...)
...
public:
  ...
  /// set a constant intensity for the force. this discard any previous call to
  /// setIntensityScheme
  ...
  // deriver have here the opportunity to setup the functor object. The default
  // does nothing
...
protected:
  ...
  void newForceIntensityElement(const CPSEmitterInfo &info) {
    if (_IntensityScheme && _IntensityScheme->hasMemory())
      _IntensityScheme->newElement(info);
  }
  void deleteForceIntensityElement(uint32 index) {
    if (_IntensityScheme && _IntensityScheme->hasMemory())
      _IntensityScheme->deleteElement(index);
  }
  void resizeForceIntensity(uint32 size) {
    if (_IntensityScheme && _IntensityScheme->hasMemory())
      _IntensityScheme->resize(size, getForceIntensityOwner()->getSize());
  }
};

/**
 * this class defines the newElement, deleteElement, and resize method of a
 * class that derives from CPSForceIntensity And that don't add per paerticle
 * attribute
 */
...
protected:
  ...
  virtual void newElement(const CPSEmitterInfo &info) {
    newForceIntensityElement(info);
  }
  virtual void deleteElement(uint32 index) {
    deleteForceIntensityElement(index);
  }
  virtual void resize(uint32 size) { resizeForceIntensity(size); }
};

/** a helper class to create isotropic force : they are independant of the
 *basis, and have no position (fluid friction for example) To use this class you
 *should provide to it a functor class that define the () operator with 3
 *parameters param1 = a const reference to the position of the particle param2 =
 *a reference to the position, that must be updated param3 =  a float giving the
 *inverse of the mass param4 = the ellapsed time, in second (has the
 *TAnimationTime type). Example of use : class MyForceFunctor
 *  {
 *    public:
 *      /// it is strongly recommended to have your operator inlined
 *      void operator() (const NLMISC::CVector &pos, NLMISC::CVector &speed,
 *float invMass , CanimationTime ellapsedTime)
 *      {
 *			// perform the speed update there
 *		}
 *
 *      // you can provide a serialization method. Note that that if the functor
 *parameters are set before each use,
 ...
 *
 *
 * because of the serialization process, you must proceed like the following.
 *(but you don't need to redefine serial, which will serilize the functor object
 *you passed for you
 *
 ...
 *
 *
 *  not that each functor may have its own parameter. the setupFunctor method
...
public:
  ...
  virtual void serial(NLMISC::IStream &f) {
    ...
    CPSForce::serial(f);
    f.serial(
        _F); // serial the functor object 5does nothing most of the time ...)
  }
...
template <class T> void CIsotropicForceT<T>::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < _Owner->getSize(); ++k) {
    setupFunctor(k);
    TPSAttribVector::iterator speedIt = target.getSpeed().begin(),
                              endSpeedIt = target.getSpeed().end();
...
/**
 *  a force that has the same direction everywhere. Mass is also taken in
 * account (which is not the case for gravity)
 */

class CPSDirectionnalForce : public CPSForceIntensityHelper,
                             public CPSDirection {
public:
  /// Compute the force on the targets
  virtual void computeForces(CPSLocated &target);
  ...
  /// Bind the direction to a global vaariable (e.g "WIND" ). The value can be
  /// changed in CParticleSystem::setGlobalVectorValue
  virtual void enableGlobalVectorValue(const std::string &name);
  // See if the direction is bound to a global variable. Return an empty string
  // if not
...
protected:
  ...
  CParticleSystem::CGlobalVectorValueHandle
      _GlobalValueHandle; // a global vector value may override the value of
                          // _Dir (example of use : global direction for wind)
};

/// a gravity class. Mass isn't taken in account (true with a uniform gravity
/// model, near earth )
...
public:
  ...
  virtual void integrate(float date, CPSLocated *src, uint32 startIndex,
                         uint32 numObjects, NLMISC::CVector *destPos = NULL,
                         NLMISC::CVector *destSpeed = NULL,
                         bool accumulate = false,
                         uint posStride = sizeof(NLMISC::CVector),
...
public:
...
#endif
      void
      operator()(const NLMISC::CVector & /* pos */, NLMISC::CVector &speed,
                 float invMass) {
  ...
  virtual void serial(NLMISC::IStream &f) {
    f.serialVersion(1);
    // we don't save intensity info : it is saved by the owning object (and set
    // before each use of this functor)
  }
...
/** the fluid friction force. We don't derive from CPSForceIntensityHelper
 * (which derives from CPSForce , because CIsotropicForceT also derives from
 * CPSForce, and we don't want to use virtual inheritance
 */

class CPSFluidFriction : public CIsotropicForceT<CPSFluidFrictionFunctor>,
                         public CPSForceIntensity {
public:
...
public:
  ...
  /** When used with parametric integration, this tells factor tells how fast
   * the force acts on particle (how fast it go through the lookup table in
   * fact)
   */
...
protected:
  ...
  float _ParametricFactor; // tells how fast this force act on a particle when
                           // parametric motion is used
  static NLMISC::CVector PrecomputedPos[]; // after the sequence we must be back
  ...
  __forceinline
#endif
      void
      operator()(const NLMISC::CVector & /* pos */,
                 NLMISC::CVector & /* speed */, float /* invMass */) {
    nlassert(0);
    ...
    /*	static const NLMISC::CVector v1(1.235f, - 45.32f, 157.5f);
        ...
        speed += ellapsedTime * _Intensity
               * NLMISC::CVector(2.f * (-0.5f + CPSUtil::buildPerlinNoise(_Scale
       * pos, _NumOctaves)) , 2.f * (-0.5f +  CPSUtil::buildPerlinNoise(_Scale *
       (pos +  v1) , _NumOctaves)) , 2.f * (-0.5f +
       CPSUtil::buildPerlinNoise(_Scale * (pos +  v2) , _NumOctaves))
                         );
                         */
  }
...
public:
  ...
  virtual void setupFunctor(uint32 index) {
    _F._Intensity =
        (_IntensityScheme ? _IntensityScheme->get(_Owner, index) : _K);
  }
...
public:
  ...
  CPSCylindricVortex(float intensity = 1.f)
      : _RadialViscosity(.1f), _TangentialViscosity(.1f) {
  ...
  virtual NLMISC::CVector getScale(uint32 k) const {
    return NLMISC::CVector(_Radius[k], _Radius[k], _Radius[k]);
  }
...
protected:
  ...
  // radial viscosity : when it is near of 1, if tends to set the radial
  // componenent of speed to 0
  float _RadialViscosity;

  // tangential viscosity : when set to 1, the tangential speed immediatly reach
  // what it would be in a real vortex (w = 1 / r2)

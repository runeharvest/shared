...
// SYSTEM IN A MOT MODEL // SEE also CParticleSystemshape, CParticleSystem
// //
/////////////////////////////////////////////////////////
...
public:
  ...
  /** Get the particle system contained in this transform shape. (NB: This is
   *shared by a smart ptr) \return pointer to the system, or NULL if no system
   *is currently hold by this model. this may happen when the system is not
   *visible and that it has been deleted
   */
  CParticleSystem *getPS(void) { return _ParticleSystem; }

  /** Get the particle system (NB : This is shared by a smart ptr) contained in
   * this transform shape. This may be null if the model is not visible.
   */
  const CParticleSystem *getPS(void) const { return _ParticleSystem; }

  /** Set the particle system for this transform shape after it has been
   * instanciated (from a memory stream, or by sharing) see CParticleSystemShape
   */
  ...
  //@{
  /**
   * test whether the system has become invalid. The condition for a system to
   * be invalid are encoded in the system itself (no more particles for
   * example). When a system has become invalid, you may want to remove it most
   * of the time
   */
  bool isInvalid(void) const { return _Invalidated; }

  /// interface for object that observe this model. They will be notified when
  /// it becomes invalid
  ...
  /** when called with true, this force the model to querry himself the ellapsed
   * time to the scene. This is the default. Otherwise, setEllapsedTime must be
   * called
   */
  void enableAutoGetEllapsedTime(bool enable = true) {
    _AutoGetEllapsedTime = enable;
  }
  /** This apply a ratio on the ellapsed time. This can be used to slow down a
   * system This must be in the >= 0. 1 means the system run at normal speed
   */
  ...
  /** force the edition mode : this will prevent the system from being removed
   * when it is out of range. When the model is first allocated, the system
   * resource are not allocated until it becomes visible. This also forces the
   * resources to be allocated. when there are no more particles in it etc.
   * (this also mean that you can safely keep a pointer on it) This flag is not
   * saved.
   */
  void setEditionMode(bool enable = true);
  /// test if edition mode is activated
  bool getEditionMode(void) const { return _EditionMode; }
  /// edition purpose : touch the system to tell that the transparency state of
  /// the system has changed (added/removed opaque/tansparent faces )
  void touchTransparencyState(void) { _TransparencyStateTouched = true; }
  /// edition purpose : touch the system to tell that the lightable state of the
  /// system has changed (added/removed lightable faces )
  void touchLightableState(void) { _LightableStateTouched = true; }
  //@}
  ...
  /** This update the infos about opacity (e.g are there solid faces and / or
   * transparent faces in the system). This must be called when the system is
   * instanciated, or when attributes have changed, such as the blending mode
   */
  ...
  /// inherited from CTransformShape. Returns the number of triangles wanted
  /// depeneding on the distance
  virtual float getNumTriangles(float distance);
  /// create an instance of this class
  static CTransform *creator() { return new CParticleSystemModel; }

  /// \name CTransform traverse specialisation
  // @{
  /** Very special clip for Particle System (because of the complexity of not
   * rendered, but still detail-animated...)
   */
   ...
   *	 - Detail animation for a particle system. It perform motion of the
   *particles (so, motion occurs only when the system has not be clipped)
   */
  ...
  // Set the user matrix, with instant update of the CParticleSystem pointed by
  // that model (if instanciated)
  ...
  // Set z-bias. Value is in world coordinates. Value remains even if ps isn't
  // present (rsc not allocated)
...
private:
  ...
  /// reallocated
  void reallocRsc();
  /// Called by the particle system manager to release this model resources (if
  /// it is too far for example)
  void refreshRscDeletion(const std::vector<CPlane> &worldFrustumPyramid,
                          const NLMISC::CVector &viewerPos);
  // Release the resources (attached system) of this model, but doesn't make it
  // invalid.
  void releaseRsc();
  // Mark this system model as invalid, delete the attached system, and calls
  // his observers
  void releaseRscAndInvalidate();
  /// Return true if the system is in the given world pyramid
  bool
  checkAgainstPyramid(const std::vector<CPlane> &worldFrustumPyramid) const;
  ...
  void insertInVisibleList() {
    ...
    if (!_InsertedInVisibleList) {
      ...
      if (_AncestorSkeletonModel == NULL) {
        ...
        // NB: don't insert if has an _AncestorSkeletonModel, because in this
        ...
        // NB: don't insert if has an _AncestorSkeletonModel, because in this
        // case, this ancestor will animDetail through the hierarchy...
...
private:
  CParticleSystemManager::TModelHandle
      _ModelHandle; /** a handle to say when the resources
                     * of the model (_ParticleSystem) are deleted
                     */
  CParticleSystemManager::TAlwaysAnimatedModelHandle
      _AnimatedModelHandle; // handle for permanenlty animated models
...
public:
  ...
  uint64 &Target;
  uint64 StartDate;
  CMiniTimer(uint64 &target) : Target(target) { SC.start(); }
  ~CMiniTimer() {
    SC.stop();

...
public:
  ...
  /** enable FOV animation. (default is false). see setPerspective(). znear and
   * zfar are kept from previous setup. NB: as setPerspective(), fov is the full
   * horizontal angle of camera (in radians).
   */
  ...
  /** enable Target/Roll animation. (default is false). TTransform mode is
   * forced to RotQuatMode. Roll is forced to 0. The camera builds the rot
   * matrix from the animated target/roll.
   */
   ...
   * NB: as setPerspective(), fov is the full horizontal angle of camera (in
   * radians).
   */
  static const char *getFovValueName() { return "fov"; }
  /// Return the name of the target track. This target is in the parent space of
  /// camera.
  ...
  /// Default Track Values for are identity (roll= 0, target= CVector::Null,
  /// fov=Pi/2).
  ...
  /// Compute corners of the camera Pyramid from current worldMatrix, and
  /// frustum

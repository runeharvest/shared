...
// class id for water
const NLMISC::CClassId WaterModelClassId =
    NLMISC::CClassId(0x41a0732e, 0x6c664506);

// class id for wave maker
const NLMISC::CClassId WaveMakerModelClassId =
    NLMISC::CClassId(0x16da3356, 0x7dec65fd);
 ...
 * -A bump map used to perturbate the bump map 1 (bump map 0). This simulate
 * local water motion -A color map whose both alpha and rgb are modulated with
 * the envmap to get transparency and color of the water. These maps can be
 * scaled, but not rotated. The bump maps can scroll at regular speed over time
 * The envmap, when set to a CTextureBlend, will automatically blend for all
 * shape when CWaterPoolManager::setBlend is called. This may be used to
 * simulate a night / day transition. The color / alpha map may have arbritrary
 * orientation over the surface \author Nicolas Vizerie \author Nevrax France
 * \date 2001
 */
...
public:
  ...
  /// inherited from IShape. Does nothing. A new traverseRender() was set for
  /// that
  ...
  ///\name Geometry setup
  //@{
  /// all water shape that have the same weter id will share the same height map
  /// for wave propagations
  ...
  /// set the size of borders. This is needed when water move is high, to avoid
  /// hole on the border of the screen
  ...
  // set a polygon that represent this shape. It must be a 2d polygon, with z
  // kept to 0 everywhere
  ...
  /// get the polygon this shape, in world space, according to its default
  /// position
  ...
  void getShapeInWorldSpace(NLMISC::CPolygon &poly,
                            const NLMISC::CMatrix &objMat) const;

  /// Set a factor that is applied to waves height when they are displayed.
  /// default is 1
  void setWaveHeightFactor(float f) { _WaveHeightFactor = f; }

  /// Get the factor that is applied to waves height when they are displayed.
  /// default is 1
  ...
  ///\name Texture setup
  //@{
  // set the environment reflected by water. Index should be 0 for the above
  // texture and 1 for the below texture
  void setEnvMap(uint index, ITexture *envMap);
  ITexture *getEnvMap(uint index) {
    nlassert(index < 2);
    return (ITexture *)_EnvMap[index];
  }
  const ITexture *getEnvMap(uint index) const {
    nlassert(index < 2);
    return (const ITexture *)_EnvMap[index];
  }

  /** Set a height map, used to generate a bumpmap (useful if supported by
   * hardware...) NB : not to be confused with the height map used to modify the
   * geometry, it  only modify texture
   */
  ...
  void setHeightMapSpeed(
      uint k, const NLMISC::CVector2f &speed); // speed given in texture units
  ...
  // set A 2x3 matrix used to compute position in colormap, from the x and y
  // coordinates in world space
  void setColorMapMat(const NLMISC::CVector2f &column0,
                      const NLMISC::CVector2f &column1,
                      const NLMISC::CVector2f &pos);
  void getColorMapMat(NLMISC::CVector2f &column0, NLMISC::CVector2f &column1,
  ...
  /// \name Lighmap
  // @{
  /// Tells this shape that it can replace its colormap by a lightmap during the
  /// zone lighting
  ...
  /// \name Misc flags
  // @{
  // Tells whether splashs are enabled (for client only, the flag in itself does
  // nothing, and is here for convenience)
  ...
  void setUseSceneWaterEnvMap(uint index, bool enable) {
    nlassert(index < 2);
    _UsesSceneWaterEnvMap[index] = enable;
  }
  bool getUseSceneWaterEnvMap(uint index) const {
    nlassert(index < 2);
    return _UsesSceneWaterEnvMap[index];
  }
  //@}
...
private:
  ...
  static uint32
      _MaxGridSize; // size with max rotation around y (without borders)
  ...
  static NLMISC::CSmartPtr<CVertexProgramWaterVPNoWave>
      _VertexProgramNoWaveDiffuse;
};

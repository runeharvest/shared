...
public:
  ...
  CTextureFile(const std::string &s) {
    /* ***********************************************
     *	WARNING: This Class/Method must be thread-safe (ctor/dtor/serial): no
     *static access for instance It can be loaded/called through
     *CAsyncFileManager for instance
     * ***********************************************/
  ...
  // User can disable the sharing system. Default is to be enabled. Not
  // serialized
  ...
  //// Used to fill a bitmap by reading a file, looking in CPath if necessary,
  ///and using user_color
  static void buildBitmapFromFile(NLMISC::CBitmap &dest,
                                  const std::string &fileName, bool asyncload,
                                  uint8 mipMapSkip = 0,
                                  bool enlargeCanvasNonPOW2Tex = false);

  /// If the file is a DDS texture with mipmap, skip the first skipLod mipmaps
  /// (0 by default) at loading
   ...
   *are snapped to the top-left corner of a power-of-2 sized texture Default is
   *false. Any texture that are non power of 2 are considered as a bug and thus
   *a DummyTexture is displayed NB: if true, CBitmap::getWidth() and
   *CBitmap::getHeight() will return the new enlarged size
   */
  void setEnlargeCanvasNonPOW2Tex(bool enlarge) {
    _EnlargeCanvasNonPOW2Tex = enlarge;
  }
   ...
   *	Default is false.
   */
  static void setSupportNonPowerOfTwoTextures(bool support) {
    _SupportNonPowerOfTwoTextures = support;
  }
  static bool supportNonPowerOfTwoTextures() {
    return _SupportNonPowerOfTwoTextures;
  }
...
private:
  ...
  bool _EnlargeCanvasNonPOW2Tex; // Non power of 2 textures are cropped. Default
                                 // is false

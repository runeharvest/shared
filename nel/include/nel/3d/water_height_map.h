...
/**
 * This class is a portion of water, it encodes its height, and simulates its
 *propagation. It contains datas about the eight around the user position
 *
 *
 *			size x 2
 *     |--------|
...
public:
   ...
   * Usually called by CWaterModel before the display, if this height map date
   * is not the same as the current date.
   */
  ...
  /** Set the userPos (relative to the height map coordinates). This is needed
   * because a height map can't be used with large surface (such as a sea). As a
   * consequence, the height map is only valid below the user (e.g from user.x -
   * 0.5 * size to user.x + 0.5 *size). When setPos is called, and if a move has
   * occurred, new area of the height field are set to 0 The pos set will be
   * taken in account when buffers have been swapped (e.g when the propagation
   * time as ellapsed)
   */
  void setUserPos(sint x, sint y);

  /** Retrieve the use pos
   * NB: this may be different from the params of a former call to setUserPos
   * (should have been taken in account)
   */
  ...
  /** Inherited from UWaterHeightMap. This version takes a location in world
   * space
   */
  virtual void perturbate(const NLMISC::CVector2f &pos, float strenght,
                          float radius);
  ...
  /** Inherited from UWaterHeightMap. Get the height of water at the given
   * location.
   */
  ...
  float *getPrevPointer(void) {
    return &(_Map[(_CurrMap + (NumWaterMap - 1)) % NumWaterMap][0]);
  }

  /// get the ratio between the previous and the current buffer
  float getBufferRatio() const {
    return _PropagationTime != 0 ? _PropagateEllapsedTime / _PropagationTime
                                 : 0.f;
  }
  ...
  /** Tells this height map the params to automatically generate waves. They are
   * generated as perturbation on the border of the field \param   intensity The
   * intensity of the waves. 0 disable waves \param period  the time ellapsed
   * between each waves \param radius  the radius od the impulsion of the waves
   * to be created \param border  true if waves should only be generated on the
   * border of height map, (actually, where waves can't be seen because of
   * distance, this avoid to see the impulsion)
   */
  ...
  /// Set the propagation time. This is the time needed to go from one unit to
  /// one other in the height map
...
private:
  ...
  /// swap the height maps. It must be called once propagation and filtering
  /// have been performed
  ...
  TAnimationTime
      _PropagationTime; // the time needed to perform a propagation, this allow
                        // split the propagation computation over time.
  ...
  // TFloat2Vect				   _Grad;   // used to store the
  // gradient
  uint8 _CurrMap;
  uint32 _Size;
  ...
  void makeCpy(uint buffer, uint dX, uint dY, uint sX, uint sY, uint width,
               uint height);

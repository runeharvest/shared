...
/** Micro-veget specific. Tells whether it is disabled, above water (the
 * default), under water, or if it intersects a water surface. This state is
 * represented encoded as an enum in CTileElement
 */
...
// ***************************************************************************
/**
 * An Element for CPatchTexture. Temporary! since CPatchTexture should be
 * compressed... NB: no default ctor => must init all fields. \author Lionel
...
public:
  ...
  /** The three tile ident. NL_TILE_ELM_LAYER_EMPTY means no Tile for this pass.
   * Tile[0] should be !=NL_TILE_ELM_LAYER_EMPTY. Else cross are drawn...
   */
  uint16 Tile[3];
   ...
   *
   * NB: During UV computing, orient is applied first, then tile256x256 uvOffset
   *(only if the tile is 256x256). \param is256x256 is this tile a part of a
   *256x256 \param uvOff the UV offset of tile 256x256. uvOff E [0,3]. Meanings:
   *	  ---------
   *	  | 0 | 3 |
   *	  |___|___|
   *	  |   |   |
  ...
  TVegetableInfo getVegetableState() const {
    return (TVegetableInfo)((Flags >> NL_TILE_ELM_OFFSET_VEGETABLE) &

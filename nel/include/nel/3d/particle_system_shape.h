...
// ***************************************************************************
// ClassIds.
const NLMISC::CClassId ParticleSystemModelId =
    NLMISC::CClassId(0x3a9b1dc3, 0x49627ff0);
...
public:
  ...
  /** build the shape from a 'raw' particle system. A prototype will be created
   * by copying the system in a memory stream NOTE : For now, prefer the
   * instanciation from a file, which do not need reallocation
   */
  ...
  // @{
  /** render() a particle system in a driver, with the specified TransformShape
   * information. CTransfromShape call this method in the render traversal.
   */
  virtual void render(NL3D::IDriver *drv, CTransformShape *trans,
                      bool passOpaque);
  // @}
  ...
  /// Always return a unit bounding box, unless the system has a precomputed
  /// bbox.
  virtual void getAABBox(NLMISC::CAABBox &bbox) const;

  /** this method is meaningless here : the traverseLoadBalancing() for particle
   * system compute the number of triangles from the Model, not the shape
   */
...
public:
   ...
   * A particle system may need to call this when a system is back in the
   * frustum An contiguous block allocator may be provided for fast alloc, init
   * will be called on such allocator with 0 if num bytes is unknown of with the
   * size needed otherwise.
   */
  CParticleSystem *
  instanciatePS(CScene &scene,
...
protected:
  /** A memory stream containing a particle system. Each system is instanciated
   * from this prototype Nevertheless, we store some more system infos which are
   * needed for its lifecycle mgt.
   */
  NLMISC::CMemStream _ParticleSystemProto;
  float
      _MaxViewDist; // the max view distance of the system, mirror the PS value
  ...
  /// For sharing, this tells us if there's a system already instanciated that
  /// we could use for sharing
  ...
  // keep smart pointer on textures for caching, so that when flushTextures is
  // called, subsequent
  std::vector<NLMISC::CSmartPtr<ITexture>> _CachedTex;

  // The amount of memory needed for instanciation or 0 if not known.
  // If the amount is known, a big block can be allocated for fast contiguous
  // allocations Given that a .ps can allocate numerous small block, this can be
  // slow indeed..
...
public:
  ...
  // In this case, only one CParticleSystem instance is created, even if there
  // are several models, and because the allocator must remains until the
  // instance is released, we must keep it in the shape
  //
  NLMISC::CContiguousBlockAllocator Allocator;
#endif
  // Order in which the element of the particle system must be processed during
  // the sim loop.

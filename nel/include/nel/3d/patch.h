...
#define NL_MAX_TILES_BY_PATCH_EDGE_SHIFT                                       \
  4 // max 16x16 tiles by patch (shift version)
#define NL_MAX_TILES_BY_PATCH_EDGE                                             \
  (1 << NL_MAX_TILES_BY_PATCH_EDGE_SHIFT) // max 16x16 tiles by patch
...
#define NL_PATCH_BLOCK_MAX_VERTEX                                              \
  (NL_PATCH_BLOCK_MAX_QUAD + 1) // Max vertex per CPatchQuadBlock.
...
#define NL3D_PATCH_VEGETABLE_NUM_TESSBLOCK_PER_CLIPBLOCK                       \
...
public:
  ...
  void serial(NLMISC::IStream &f) { f.serial(x, y, z); }
};
...
struct CPatchIdent {
  ...
  CPatchIdent(sint32 zoneId, uint16 patchId)
      : ZoneId(zoneId), PatchId(patchId) {}

public:
  bool operator<(const CPatchIdent &p) const {
    if (ZoneId != p.ZoneId)
      return ZoneId < p.ZoneId;
  ...
  bool operator!=(const CPatchIdent &p) const { return !(*this == p); }
};
...
public:
  ...
  bool operator==(const CPatchBlockIdent &pb) const {
    return PatchId == pb.PatchId && S0 == pb.S0 && S1 == pb.S1 && T0 == pb.T0 &&
           T1 == pb.T1;
  }
  bool operator!=(const CPatchBlockIdent &pb) const { return !(*this == pb); }
...
// ***************************************************************************
/**
 * A group of tiles faces in a patch. For speed/mem, max size of a
 * CPatchQuadBlock is fixed.
 *
 * \author Lionel Berenguier
 * \author Nevrax France
 * \date 2000
 ...
 *   B---> bc ---- cb <---C
 *
 * NB: Patch 1x1 or 1xX are illegal: lot of problem: rectangular geomoprh, Son0
 *and Son1 must be setup as tile at beginning ...
 *
 * NB: Edges number are:
 *	- 0: AB.
 *	- 1: BC.
...
public:
  struct CBindInfo {
    ...
    // The number of patchs on this edge. 0,1, 2 or 4. if MultipleBindNum>1,
    // NPatchs==1.
    sint NPatchs;

    // Special case: on this edge, we are a small patch connected to a bigger:
    // this is the X of 1/X (1,2 or 4). 0 if this is not the case.
    uint8 MultipleBindNum;
    // valid only if MultipleBindNum>1. this tells our place in this
    // MultipleBind: 0<=MultipleBindId<MultipleBindNum.
...
public:
  ...
  // There is OrderS/2+1 * OrderT/2+1 tiles light influence. CZone build it at
  // build() time.
  std::vector<CTileLightInfluence> TileLightInfluences;

  /// Noise Data.
  // @{
  /// The orientation of the NoiseMap. 0,1,2,3. This represent a CCW rotation of
  /// the NoiseMap.
  uint8 NoiseRotation;

  /** setup Smooth flags for Noise on corner: used for Noise geometry and for
   *lighting. NB: convention: corner0==A, corner1==B ...
   */
...
public:
   ...
   * Call this method before any other. Zone and Control points must be valid
   * before calling compile(). \n This is an \b ERROR to call compile() two
   * times. \n \param z zone where the patch must be binded. \param orderS the
   * Tile order in S direction: 2,4,8,16. \param orderT the Tile order in T
   * direction: 2,4,8,16. \param errorSize if 0, setup() compute himself the
   * errormetric of the patch. May be setup to surface of patch, modulated by
   * tangents and displacement map.
   */
  void compile(CZone *z, uint patchId, uint8 orderS, uint8 orderT,
               CTessVertex *baseVertices[4], float errorSize = 0);
  /// Un-compile a patch. Tesselation is deleted. if patch is not compiled, no -
  /// op.
  void release();
  ...
  /// Build the bbox of the patch, according to ctrl points, and displacement
  /// map max value.
  ...
  /**	Same as computeVertex, but special accurate version for
   *CVisualCollisionEntity. If on a corner of the patch (0,0), (0,1) ...., take
   *directly the BaseVertices[] result if on a border of a patch (ie s==0, s==1,
   *t==0 or t==1), also compute the vertex of the neighbor patch (if any), and
   *then average the result. This ensure that tesselation generated is perfectly
   *continous, even across patchs of same or different zones. This feature is
   *very important for CVisualCollisionEntity::snapToGround()
   */
  CVector computeContinousVertex(float s, float t) const;

  /** unbind the patch from All neighbors. neighbors patchs links are modified
   * too. The tesselation is forcemerged.
   */
  void unbind();

  /** bind the patch to 4 neighbors, given in this patch edge order (0,1,2,3).
   * Tesselation is reseted (patch unbound first). NB: this patch and his
   * neighborood must be compiled... NB: neighbor patchs must not be NULL (but
   * according to NPatchs).
   */
  ...
  // like preRender(), but update TextureFar only. (you should call preRender()
  // in this case).
  void updateTextureFarOnly(const NLMISC::CBSphere &patchSphere);
  /// Render this patch, if not clipped. Call
  /// PatchCurrentDriver->renderSimpleTriangles().
  // Global Setup setup it in CLandscape::render().
  void renderFar0();
  void renderFar1();
  // NB: renderTile() is now in CTileMaterial.
  // For All Far0/Far1/Tile etc..., compute Geomorph and Alpha in software (no
  // VertexShader).
  ...
  /**
   *  Unpack the lumels of the patches. Lumels are classes that describe the
   * lighting environnement at a given texel of the lightmap. It is used to
   * compute the shadow map of the patch, compress it and uncompress it. This
   * method uncompress the lumels stored in its Lumels member.
   *
   *  \param pShadow is a pointer on the destination lumel buffer. Size must be
   * ((OrderS*4/ratio)+1)*((OrderT*4/ratio)+1). \param ratio is the one over the
   * ratio of the texture destination. Must be 1 or 2. \see packShadowMap(),
   * resetCompressedLumels()
   */
  void unpackShadowMap(uint8 *pShadow);

  /**
   *  Pack the lumels of the patches. Lumels are classes that describe the
   * lighting environnement at a given texel of the lightmap. It is used to
   * compute the shadow map of the patch, compress it and uncompress it. This
   * method compress the lumels passed in parameter and stored them in its
   * Lumels member.
   *
   *  \param pShadow is a pointer on the destination lumel buffer. Size must be
   * (OrderS*4+1)*(OrderS*4+1). \see unpackShadowMap(), resetCompressedLumels()
   */
  void packShadowMap(const uint8 *pLumel);

  /**
   *  Rebuild the packed lumels without shadow. Only the interpolated color will
   * be used.
   *
  ...
  /** Debug purpose only : setup the colors of this patch so that it shows which
   * tiles have vegetable disabled, or are above, below water. User provides a
   * table with 4 colors for each state : color 0 = above water color 1 =
   * underwater color 2 = intersect water color 3 = vegetable disabled
   */
...
private:
   ...
   *  \see packShadowMap(), unpackShadowMap()
   */
  void packLumelBlock(uint8 *dest, const uint8 *source, uint8 alpha0,
                      uint8 alpha1);
   ...
   *  \see packShadowMap(), unpackShadowMap()
   */
  uint evalLumelBlock(const uint8 *original, const uint8 *unCompressed,
...
public:
  ...
  /**
   * Set the smooth flag for the n-th edge. flag is false if this edge must by
   * smoothed, true else.
   */
  ...
  /**
   * Get the smooth flag for the n-th edge. Return true if this edge must by
   * smoothed, false else.
   */
   ...
   * NB: this method use a convex hull subdivion to search in O(logn) what part
   * of the patch to insert. \param patchId the id of this patch, used to fill
   * triangles. \param bbox the bbox to test against. \param triangles array to
   * be filled (no clear performed, elements added). \param tileTessLevel 0,1 or
   * 2  size of the triangles (2*2m, 1*1m or 0.5*0.5m). Level of subdivision of
   * a tile.
   */
  void addTrianglesInBBox(CPatchIdent paId, const CAABBox &bbox,
   ...
   * NB: this method use a convex hull subdivion to search in O(logn) what part
   * of the patch to insert. \param patchId the id of this patch, used to fill
   * PatchBlockIdent. \param bbox the bbox to test against. \param paBlockIds
   * array to be filled (no clear performed, elements added).
   */
  void addPatchBlocksInBBox(CPatchIdent paId, const CAABBox &bbox,
                            std::vector<CPatchBlockIdent> &paBlockIds) const;

  /** From the current tesselation of this patch, and a UV in this patch, return
   * tesselated position.
   */
  CVector getTesselatedPos(CUV uv) const;

  /** From the current tesselation of this patch, append to the list of leaves
   * faces.
   */
   ...
   */
  void appendTileLightInfluences(
      const CUV &uv, std::vector<CPointLightInfluence> &pointLightList) const;

  /** For CTextureFar, compute current TLI Lightmap at tile level. array should
   * be allocated of at least sqr(NL_MAX_TILES_BY_PATCH_EDGE+1).
   */
...
public:
  ...
  // delete all VB allocated in VertexBuffers, according to Far0 and Far1. Do
  // not Test isRenderClipped() state. With VB, allocate to he faces array.
  void deleteVBAndFaceVector();

  // allocate all VB, according to Far0 and Far1. Do not Test isRenderClipped()
  // state. With VB, allocate to he faces array.
  void allocateVBAndFaceVector();

  // fill all VB, according to Far0, Far1 and CTessFace VBInfos. Do not Test
  // isRenderClipped() state. Do not fill a VB if reallocationOccurs().
  void fillVB();

  // if isRenderClipped()==false, fillVB().
  void fillVBIfVisible();

  // delete Far1 VB allocated in VertexBuffers. do it only if Far1==true. Do not
  // Test isRenderClipped() state. With VB, allocate to he faces array.
  void deleteVBAndFaceVectorFar1Only();

  // allocate Far1 VB, . do it only if Far1==true. Do not Test isRenderClipped()
  // state. With VB, allocate to he faces array.
  void allocateVBAndFaceVectorFar1Only();

  // fill Far0 VB according CTessFace VBInfos and Far0 (do not fill if !Far0).
  // Do not Test isRenderClipped() state. Do not fill a VB if
  // reallocationOccurs().
  void fillVBFar0Only();
  // same for Far1
  void fillVBFar1Only();

  // fill DLM Uv (ie UV1) for Far0 and Far1 VB only. NB: do not fill Far0 if
  // !Far0 (idem fro Far1). Do not Test isRenderClipped() state. Do not fill a
  // VB if reallocationOccurs().
  ...
  // Because of refine... tessBlock FaceVector may have been deleted, this
  // method do nothing if isRenderClipped(). If not, recreate FaceVector for
  // this tessBlock only, according to Far0 and Far1.
...
public:
  ...
  /// Recreate any vegetable block (as possible) in this patch. (useful for
  /// edition)
  ...
  // @{
  /** make a valid empty array of TileLightInfluences (ie resized to good size,
   *but with empty light influences
   */
  ...
  /// For lighting update, insert this before patchNext (CiruclarList). textNext
  /// must be !NULL
  void linkBeforeNearUL(CPatch *patchNext);
  /// For lighting update, unlink (CiruclarList)
  void unlinkNearUL();
  /// For lighting update, get Next (CiruclarList). If ==this, then list is
  /// empty
   ...
   *RefCount. Called by CLandscape.
   */
  void processDLMLight(CPatchDLMPointLight &pl);
  /** end Dynamic light Process, deleting the DLMContext if necessary.
   *	NB: _DLMContext->compileLighting() is not called, since done during
   *render phase. Called by CLandscape. _DLMContext must exist
   */
  void endDLMLighting();
  ...
  uint getTileMaterialRefCount() const {
    return MasterBlock.TileMaterialRefCount;
  }
...
private:
  ...
  // This is especially for Pacs. false by default, and used by
  // CZone::refineAll() and CZone::excludePatchFromRefineAll().
  bool ExcludeFromRefineAll;

  // For this patch, which level is required for a face to be inserted in the
  // secondary TessBlocks (and not the masterblock)??
  sint TessBlockLimitLevel;
  // For this patch, which level is required for a face to be a valid Tile??
  sint TileLimitLevel;
  // For this patch, which level is required for a face to be a "square" face
  // (not rectangular)??
  ...
  // Local info for CTessFace tiles. CPatch must setup them at the beginning at
  // refine()/render(). For Far Texture coordinates.
   ...
   *		4 flag for smooth edge. Same as CPatchInfo::CBindInfo shifted by
   *(<<NL_PATCH_SMOOTH_FLAG_SHIFT). See CPatchInfo::CBindInfo::Flags for
   *details.
   */
  uint8 Flags;
  ...
  /*
      Cache Optim Note:
      NB: for faster Cache Access during preRender(), you must leave those
     variables packed like this
      ...
      NB: Far0FaceVector and Far1FaceVector are accessed during renderFar*()
     (not preRender()) but they must be near to TessBlocks (for size() )
  */
  ...
  /// \name Block renders.
  // @{
  // Tells how many Renderable Face this Patch has. updated in
  // append/remove/FaceToRenderList()
  sint NumRenderableFaces;
  // The 2*2 block render. For memory optimisation, none is allocated when no
  // faces need it. There is (OrderT/2)*(OrderS/2) TessBlocks.
...
private:
  // The counter of faces which need TessBlocks (FarFaces, TileMaterial and
  // FarVertices). When 0, the vector is contReset()-ed.
  sint TessBlockRefCount;
  // @}

  /// \name Vegetables.
  // @{
  /// list of vegetable clipBlocks, created/destroyed at same time as
  /// TessBlocks.
...
private:
  ...
  // based on BaseVertices, recompute positions, and Make Face roots Son0 and
  // Son1.
  void makeRoots();
  ...
  // UGLY SIDE EFFECT: when refcount TessBlocks RefCount reach 0, tessblockas
  // are deleted. think of it in tesselation.cpp.
  ...
  // For rdr. Insert in the GOOD TessBlock the face (depending on level,
  // patchcoordinates etc...) call appendFaceToTileRenderList() to insert his
  // TileFaces into the good renderList.
  ...
  // For refreshTesselationGeometry() only. enlarge the TessBlock (if any) with
  // face->V*->EndPos.
  void extendTessBlockWithEndPos(CTessFace *face);

  // Set/Unset (to NULL) a TileMaterial from the TessBlocks. Material must exist
  // for both functions. And TileS/TileT must be OK.
  ...
  void appendNearVertexToRenderList(CTileMaterial *tileMat,
                                    CTessNearVertex *nv);
  void removeNearVertexFromRenderList(CTileMaterial *tileMat,
                                      CTessNearVertex *nv);
  ...
  /// \name Texture mgt.
  // @{
  // For CTessFace::computeMaterial(). Return the render pass for this material,
  // given the number of the tile, and the desired pass. NULL may be returned if
  // the pass is not present (eg: no additive for this tile...).
  CPatchRdrPass *getTileRenderPass(sint tileId, sint pass);
  // For CTessFace::computeMaterial(). Return the orient/scalebias for the tile
  // in the patchtexture, and the desired pass (and the desired stage:
  // RGB/Alpha).
  void getTileUvInfo(sint tileId, sint pass, bool alpha, uint8 &orient,
                     CVector &uvScaleBias, bool &is256x256, uint8 &uvOff);
  // @}

  // Tile LightMap mgt.
  // @{
  // for a given tile (accessed from the (ts,tt) coordinates), compute a
  // lightmap if necessary, and get a RenderPass.
  ...
  // Compute the Lightmap for 2x2 tiles. => 10x10 pixels. ts, tt is in
  // [0,OrderS], [0, OrderT].
  void computeNearBlockLightmap(uint ts, uint tt, NLMISC::CRGBA *lightText);
  void computeTileLightmapPixelAroundCorner(const NLMISC::CVector2f &stIn,
                                            NLMISC::CRGBA *dest,
                                            bool lookAround);

  // Compute a lightmap for a tile (ts,tt). 4x4 lumels are processed. NB:
  // result= lumel*userColor.
  void computeTileLightmap(uint ts, uint tt, NLMISC::CRGBA *dest, uint stride);
  // Compute a lightmap for an edge of a tile. 1x4 lumels. "edge" say what edge
  // of the tile to compute. pixels will be written in (dest+i*stride), where i
  // vary from 0 to 3 or 3 to 0 (according to "inverse").
  void computeTileLightmapEdge(uint ts, uint tt, uint edge, NLMISC::CRGBA *dest,
                               uint stride, bool inverse);
  // Compute a lightmap just for a pixel (s,t) of a tile (ts,tt). (s,t) E [0;3],
  // [0;3].
  void computeTileLightmapPixel(uint ts, uint tt, uint s, uint t,
                                NLMISC::CRGBA *dest);

  // Methods for automatic Lighting. NB: result= lumel only (no TileColors).
  void computeTileLightmapAutomatic(uint ts, uint tt, NLMISC::CRGBA *dest,
                                    uint stride);
  void computeTileLightmapEdgeAutomatic(uint ts, uint tt, uint edge,
                                        NLMISC::CRGBA *dest, uint stride,
                                        bool inverse);
  void computeTileLightmapPixelAutomatic(uint ts, uint tt, uint s, uint t,
                                         NLMISC::CRGBA *dest);
  // Methods for Precomputed Lighting. NB: result= lumel only (no TileColors).
  void computeTileLightmapPrecomputed(uint ts, uint tt, NLMISC::CRGBA *dest,
                                      uint stride);
  void computeTileLightmapEdgePrecomputed(uint ts, uint tt, uint edge,
                                          NLMISC::CRGBA *dest, uint stride,
                                          bool inverse);
  void computeTileLightmapPixelPrecomputed(uint ts, uint tt, uint s, uint t,
                                           NLMISC::CRGBA *dest);
  // Methods to modulate dest with TileColors. NB: A unmodified.
  void modulateTileLightmapWithTileColors(uint ts, uint tt, NLMISC::CRGBA *dest,
                                          uint stride);
  void modulateTileLightmapEdgeWithTileColors(uint ts, uint tt, uint edge,
                                              NLMISC::CRGBA *dest, uint stride,
                                              bool inverse);
  void modulateTileLightmapPixelWithTileColors(uint ts, uint tt, uint s, uint t,
                                               NLMISC::CRGBA *dest);
  // get the tileColors at the corners of the tile. corner order: 0,0; 1,0; 0,1;
  // 1,1. NB: A undefined.
  void getTileTileColors(uint ts, uint tt, NLMISC::CRGBA corners[4]);

  // get the current TLIColor given a TLI coordinate (in (0..OrderS/2+1,
  // 0..OrderT/2+1) ) NB: returned color is modulated by landscape material and
  // precomputed diffuse factor
  CRGBA getCurrentTLIColor(uint x, uint y) const;
  // get the current TLIColors at the corners of the tile (according to
  // pointLights current colors) corner order: 0,0; 1,0; 0,1; 1,1. NB: A
  // undefined.
  ...
  void addTileLightmapWithTLI(uint ts, uint tt, NLMISC::CRGBA *dest,
                              uint stride);
  void addTileLightmapEdgeWithTLI(uint ts, uint tt, uint edge,
                                  NLMISC::CRGBA *dest, uint stride,
                                  bool inverse);
  void addTileLightmapPixelWithTLI(uint ts, uint tt, uint s, uint t,
                                   NLMISC::CRGBA *dest);

  // @}

  // Recompute of new Far Values, according to globals. Don't erase Far0 and
  // Far1.
  void computeNewFar(const NLMISC::CBSphere &patchSphere, sint &newFar0,
                     sint &newFar1);

  // For Render. Those methods compute the vertices for Driver (in
  // CTessFace::Current*VB).
  ...
  void debugAllocationMarkIndicesFarList(CTessList<CTessFarVertex> &vertList,
                                         uint marker);
  void debugAllocationMarkIndicesNearList(CTessList<CTessNearVertex> &vertList,
                                          uint marker);
  ...
  // For Refine. Those methods do all the good job, and test if they can
  // allocate the VB.
  void checkCreateVertexVBFar(CTessFarVertex *pVert);
  void checkCreateVertexVBNear(CTessNearVertex *pVert);
  // For Refine. Those methods do all the good job, and test if they can fill
  // the VB.
  void checkFillVertexVBFar(CTessFarVertex *pVert);
  void checkFillVertexVBNear(CTessNearVertex *pVert);
  // For Refine. Those methods do all the good job, and test if they have to
  // delete the VB.
  ...
  void
  computeGeomorphAlphaFar1VertexListVB(CTessList<CTessFarVertex> &vertList);
   ...
   * s0, s1, t0, t1 represent the part of the bezier patch subdivided. At start,
   * s0=0, s1=OrderS, t0=0, t1=OrderT.
   */
  void addTrianglesInBBoxRecurs(CPatchIdent paId, const CAABBox &bbox,
                                std::vector<CTrianglePatch> &triangles,
                                uint8 tessLevel, const CBezierPatch &pa,
                                uint8 s0, uint8 s1, uint8 t0, uint8 t1) const;
  /** called by addTrianglesInBBoxRecurs(). effective fill the array of
   * triangles from 1 tile at tile coordinates s0,t0. depending of tessLevel
   * (0,1,2), 2, 8 or 32 triangles are added.  (2*2m, 1*1*m or 0.5*0.5m). NB:
   * only triangles of quad included in the bbox are added.
   */
  void addTileTrianglesInBBox(CPatchIdent paId, const CAABBox &bbox,
                              std::vector<CTrianglePatch> &triangles,
                              uint8 tessLevel, uint8 s0, uint8 t0) const;

  /** recurse method of addPatchBlocksInBBox.
   */
  void addPatchBlocksInBBoxRecurs(CPatchIdent paId, const CAABBox &bbox,
                                  std::vector<CPatchBlockIdent> &paBlockIds,
                                  const CBezierPatch &pa, uint8 s0, uint8 s1,
                                  uint8 t0, uint8 t1) const;
...
private:
  ...
  /// The 4 neighbors zone of this patch (setuped at bind() time). NB: NULL if
  /// zone not loaded, or if no patch near us.
  CZone *_BindZoneNeighbor[4];

  /** Used by bind(). Search in the tesselation the first face which own the
   *edge uv0-uv1. link it with linkTo, and return it. NULL if not found
   *(obviously because not so tesselated)
   */
  CTessFace *linkTessFaceWithEdge(const NLMISC::CVector2f &uv0,
  ...
  void computeDisplaceRawCoordinates(float sTile, float tTile, float s, float t,
                                     sint &ts, sint &tt, sint &ms,
                                     sint &mt) const;
  /** compute the displacement for s,t ([0;OrderS], [0;OrderT])
   *  (sTile, tTile) choose what NoiseMap to use, and (s,t) choose the
   *coordinate in the patch to compute this NoiseMap. Any rotation of the
   *NoiseMap is included in this method. NB: s,t does not have to be clamped to
   *([0;OrderS], [0;OrderT]).
   */
  float computeDisplaceRaw(float sTile, float tTile, float s, float t) const;
  /** useful only for computeDisplaceCornerSmooth().
   *	This method, if nessecary (ie sTile or tTile <0 or >Order), look on his
   *neighbor to compute the value.
   */
  float computeDisplaceRawOnNeighbor(float sTile, float tTile, float s,
                                     float t) const;
   ...
   */
  float computeDisplaceEdgeSmooth(float s, float t, sint8 smoothBorderX,
                                  sint8 smoothBorderY) const;
  /** compute the smoothed displacement for s,t ([0;OrderS], [0;OrderT]).
   * Special case on corner.
   */
  float computeDisplaceCornerSmooth(float s, float t, sint8 smoothBorderX,
                                    sint8 smoothBorderY) const;

  /** compute the smoothed normal for s,t ([0;OrderS], [0;OrderT]). Special case
   * on edge.
   */
  CVector computeNormalEdgeSmooth(float s, float t, sint8 smoothBorderX,
                                  sint8 smoothBorderY) const;
  /** compute the smoothed normal for s,t ([0;OrderS], [0;OrderT]). Special case
   * on corner.
   */
  CVector computeNormalCornerSmooth(float s, float t, sint8 smoothBorderX,
                                    sint8 smoothBorderY) const;
  /** same reasoning as in computeDisplaceRawOnNeighbor().
   */
  CVector computeNormalOnNeighbor(float s, float t, uint edgeExclude) const;

  /** compute the Final displacement for s,t ([0;1], [0;1]). This is the top
   *call. displace.norm() should be <= NL3D_NOISE_MAX.
   */
  void computeNoise(float s, float t, CVector &displace) const;

  // @}

  // From tile coordinates, return the tessBlockId, and the id of the material
  // in this tessBlock.
  ...
  /// Create / init the vegetableBlock in the corresponding TessBlock.
  /// TessBlocks must exist
  void createVegetableBlock(uint numTb, uint ts, uint tt);
  /// release the vegetableBlock in the corresponding TessBlock. TessBlocks must
  /// exist
  ...
  /*
      ...
      Warning! Use OptFastFloor()! So call must be enclosed with a
     OptFastFloorBegin()/OptFastFloorEnd().
  */
  void
  generateTileVegetable(CVegetableInstanceGroup *vegetIg, uint distType,
                        uint ts, uint tt,
                        CLandscapeVegetableBlockCreateContext &vbCreateCtx);

  // same as computeTileLightmapPrecomputed(), but brut result, not modified by
  // colorTable.
  void getTileLumelmapPrecomputed(uint ts, uint tt, uint8 *dest, uint stride);
  /** same as computeTileLightmapPixelPrecomputed, but brut result, not modified
   *by colorTable. Actually used for Lightmap get interface.
   */
  void getTileLumelmapPixelPrecomputed(uint ts, uint tt, uint s, uint t,
                                       uint8 &dest) const;
   ...
   *	created only when compiled, AND (when in Near OR (when in Far AND
   *touched by pointLight)) else NULL.
   */
  CPatchDLMContext *_DLMContext;

  /** The reference count for DLMContext. Each TileMaterial created add a
   *reference. Each pointLight which touch the patch too.
   */

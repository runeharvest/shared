...
public:
  ...
  /// render and make empty the render string buffer. see
  /// CComputedString::render2DClip()
  void flush(IDriver &driver, CMaterial *fontMat);

  /** render and make empty the render string buffer. see
   *CComputedString::render2DProjected() The driver view and model matrices have
   *to be setuped as material zbuffer flags by the user. This method only render
   *string quads.
   */
...
public:
  ...
  void clear() { _indexedColors.clear(); }

  bool empty() const { return _indexedColors.empty(); }

  uint size() const { return (uint)_indexedColors.size(); }
 ...
 * in a driver. It computes 4 vertices per char the renderer draw quads from
 * them.
 *
...
public:
   ...
   *the leg of the p. So StringLine is a positive value in this case. It may be
   *a negative value for the string "^" for example.
   */
  float StringLine;

  /// Optional: each render*() method can draw a subset of letters. Default is
  /// 0/FFFFFFFF
  uint32 SelectStart;
  uint32 SelectSize;
   ...
   * You should take care that for vertical hotspot, an imaginary line is
   * defined under letters with no leg (like m,b,c etc..) between the leg of p
   * and the loop of the p.
   */
  enum THotSpot {
    BottomLeft = 0,
    MiddleLeft,
  ...
  CComputedString(bool bSetupVB = true) {
    ...
    StringHeight = 0;
    if (bSetupVB) {
      Vertices.setVertexFormat(CVertexBuffer::PositionFlag |
   ...
   * \param useScreenAR43 if false then string is displayed with a pixel Ratio
   *1:1 (independent of window resolution). if true, the string is scaled
   *according to window width and height, to support 4:3 aspect ratio even on
   *weird screen resolution such as 640*240 (ie the char still look square, but
   *the pixel ratio is 2:1) \param roundToNearestPixel if true, snap the final
   *string position to the nearest pixel. if set to true, and if useScreenAR43=
   *false, you are sure that texels of the fonts fit exactly on centers of
   *pixels (no apparent bi-linear).
   */
  void render2D(IDriver &driver, float x, float z,
                THotSpot hotspot = BottomLeft, float scaleX = 1,
                float scaleZ = 1, float rotateY = 0, bool useScreenAR43 = false,
                bool roundToNearestPixel = true);

  /** Special for interface. same as render2D but clip the quads to
   *xmin,ymin/xmax,ymax. NB: behavior is same as render2D with: Hotspot =
   *bottomLeft, scaleX=1, scaleZ=1, rotateY=0, useScreenAR43= false,
   *roundToNearestPixel= false Additionnaly, this method doesn't render directly
   *to the driver but add primitives to a CRenderStringBuffer Use the method
   *CRenderStringBuffer::flush() to flush it all.
   */
  void render2DClip(IDriver &driver, CRenderStringBuffer &rdrBuffer, float x,
                    float z, float xmin = 0, float ymin = 0, float xmax = 1,
                    float ymax = 1);

  /** Special for interface. same as render2DClip but unproject the vertices
   *using a frustum and a scale matrix Use the method
   *CRenderStringBuffer::flush() to flush it all.
   */
  void render2DUnProjected(IDriver &driver, CRenderStringBuffer &rdrBuffer,
                           class NL3D::CFrustum &frustum,
                           const NLMISC::CMatrix &scaleMatrix, float x, float z,
                           float depth, float xmin = 0, float ymin = 0,
   ...
   * \param hotspot position of string origine
   */
  void render3D(IDriver &driver, const CMatrix &matrix,

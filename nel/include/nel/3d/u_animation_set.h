...
// ***************************************************************************
/**
 * An animation Set is a set of animation, loaded from file. It contains a set
 *of Skeleton TempalteWeight too.
 *
 *	NB: If the animation set is created with addAnimation() method (not with
 *createAnimationSet(animsetFile)) header optimisation is allowed => less memory
 *load.
 *
...
public:
  ...
  /**
   *  Add an animation in the animation set. After adding all your animations,
   *call build(). This method use CPath to search the animation file. WARNING:
   *it assert if you call addAnimation() after build()
   *
   * \param fileName is the animation filename
   * \param animName is the name of the animation in the animation set.
   * \param displayMissingFileWarning A warning will be displayed in the log if
   *the anim file is not found \return the id of the new animation or NotFound
   *if the file is not found.
   */
  virtual uint addAnimation(const char *fileName, const char *animName,
                            bool displayMissingFileWarning = true) = 0;
   ...
   */
  virtual uint addSkeletonWeight(const char *fileName,
                                 const char *skelName) = 0;
  ...
  /**
   * Get a animation ID by name. If no animation is found, method returns
   * NotFound.
   */
  ...
  /**
   * Get a SkeletonWeight ID by name. If no SkeletonWeight is found, method
   * returns NotFound.
   */

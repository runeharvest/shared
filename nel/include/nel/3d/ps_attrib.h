...
/** a container that is like a vector, but snapped to (1<<snapPower) byte memory
 * pages
 */
template <class T, const uint snapPower = 5> class CSnappedVector {
public:
  typedef T *iterator;
  ...
  void reserve(uint capacity) {
    ...
    try {
      ...
      T *newTab = (T *)((size_t)(newStart + (1 << snapPower)) &
                        ~((1 << snapPower) - 1)); // snap to a page

      for (iterator src = _Tab,
                    end = _Tab + (capacity < _Size ? capacity : _Size),
                    dest = newTab;
           src != end; ++src, ++dest) {
      ...
      // destroy previous objects. We assume that we can't have exceptions
      // raised from destructors
      for (iterator it = newTab /* old tab */, endIt = newTab + _Size;
           it != endIt; ++it) {
        it->~T();
      }
...
/**
 * This class is intended to store an attribute list in a located or in a
 * located bindable such as speed, color and so on. It is important to remember
 * that a located holds all instance of object of one type (force, emitter,
 * particles or both...). \author Nicolas Vizerie \author Nevrax France \date
 * 2001
 */
...
public:
  ...
  /// The type used by the container. Its is the type used to instanciate this
  /// template.
  typedef T value_type;
  ...
  //@{
  /** Resize the attributes tab. This tells what is the max number of element in
   * this tab, but don't add elements. The behaviour is much like
   * std::vector::reserve
   */
  ...
  const T &back() const { return _Tab.back(); }

  // get a reference on the last element
  T &back() { return _Tab.back(); }
...
template <typename T> CPSAttrib<T>::CPSAttrib() {
  _MaxSize = DefaultMaxLocatedInstance;
}

template <typename T> void CPSAttrib<T>::resize(uint32 nbInstances) {
  nlassert(nbInstances < (1 << 16));
  _Tab.reserve(nbInstances);
  _MaxSize = nbInstances;
}

template <typename T> sint32 CPSAttrib<T>::insert(const T &t) {
...
template <typename T> void CPSAttrib<T>::remove(uint32 index) {
  nlassert(index < _Tab.size());
...
template <typename T> void CPSAttrib<T>::serial(NLMISC::IStream &f) {
  ...
  if (ver == 1) {
    if (f.isReading()) {
      ...
      f.serial(size); // useless but, we were previously doing a serialCont...
  ...
  if (ver == 2) // this version didn't work well, it relied on the capacity of
                // the container to store the max number of instances
  {
    nlassert(0);
    /*	f.serial(_Tab);
        if (f.isReading())
...
template <typename T> void CPSAttrib<T>::swap(CPSAttrib<T> &other) {

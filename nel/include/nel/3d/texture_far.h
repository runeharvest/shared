...
/* NB: those Values work only if NL_MAX_TILES_BY_PATCH_EDGE is 16.
    asserted in the cpp.
    Else must change NL_MAX_FAR_PATCH_EDGE, NL_NUM_FAR_PATCH_EDGE_LEVEL, and
   NL_NUM_FAR_RECTANGLE_RATIO
*/

// The number of different Level to allocate. only possible to allocate 16, 8, 4
// or 2 tiles patch (1x? is impossible)
...
#define NL_MAX_FAR_PATCH_EDGE                                                  \
  (16 * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) // Max is 16x16 tiles for 4 pix/tiles.
...
#define NL_MAX_FAR_RECTANGLE_RATIO                                             \
  (1 << NL_MAX_RECTANGLE_RATIO_SHIFT) // Max is 16 tiles fo 2 tiles => 8.

// The number of Square of MaxFarEdge.
#define NL_NUM_FAR_BIGGEST_PATCH_PER_EDGE 8
#define NL_NUM_FAR_BIGGEST_PATCH_PER_TEXTURE                                   \
  (NL_NUM_FAR_BIGGEST_PATCH_PER_EDGE * NL_NUM_FAR_BIGGEST_PATCH_PER_EDGE)

// The size of the texture.  8*64 => textureFar of 512*512.
#define NL_FAR_TEXTURE_EDGE_SIZE                                               \
  (NL_MAX_FAR_PATCH_EDGE * NL_NUM_FAR_BIGGEST_PATCH_PER_EDGE)
...
/**
 * A CTextureFar is a set of texture used to map a whole patch when it is in far
 * Mode. (ie not in tile mode).
 * A CTextureFar handle several patch texture.\\
 *
 * TODO: keeping the far texture level1 alive when the tile pass in level0 (tile
 * mode), don't erase it.
...
public:
   ...
   *	return 0 if OK, and success to allocate a place without splitting any
   *square else return the size of the max space to split (eg if must a split a
   *32x32 to allocate a 16x8)
   */
  sint tryAllocatePatch(CPatch *pPatch, uint farIndex);

  /**
   *  Allocate a patch in the CTextureFar, according to its size. nlstop if
   * fails. Must call before tryAllocate()
   *
   *  \param pPatch is the pointer to the patch to add in the landscape
   *  \param farUVScale will receive the scale to use to compute the UV
   * coordinates \param farUBias will receive the U Bias to use to compute the
   * UV coordinates \param farVBias will receive the V Bias to use to compute
   * the UV coordinates \param bRot will receive true if the texture is rotated
   * of 90deg to the left or false. You should take care of this value to
   * compute UV coordinates.
   */
  void allocatePatch(CPatch *pPatch, uint farIndex, float &farUScale,
                     float &farVScale, float &farUBias, float &farVBias,
                     bool &bRot);
  ...
  /// For lighting update, insert this before textNext (CiruclarList). textNext
  /// must be !NULL
...
private:
  ...
  /** Lists of empty Space. One for each possible size (64x64, 64x8 etc, but not
   * 64x4 since not possible...)
   */
  std::list<CVector2s>
      _FreeSpaces[NL_NUM_FAR_PATCH_EDGE_LEVEL * NL_NUM_FAR_RECTANGLE_RATIO];
...
extern "C" void
NL3D_drawFarTileInFarTexture(const NL3D_CComputeTileFar *pTileFar);
extern "C" void
NL3D_drawFarTileInFarTextureAdditive(const NL3D_CComputeTileFar *pTileFar);
extern "C" void
NL3D_drawFarTileInFarTextureAlpha(const NL3D_CComputeTileFar *pTileFar);
extern "C" void
NL3D_drawFarTileInFarTextureAdditiveAlpha(const NL3D_CComputeTileFar *pTileFar);

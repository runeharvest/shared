...
public:
  ...
  struct CCorner {
    ...
    // Setup all to 0, but Color (to white)... Important for good corner
    // comparison. This is slow but doesn't matter since used at mesh
    // building....
    CCorner();
   ...
   * NB: if you don't use all matrix for this vertex, use at least the 0th
   * matrix, and simply set 0 on Weights you don't use.
   */
  struct CSkinWeight {
  ...
  struct CInterfaceLink {
    ...
    CInterfaceLink() { InterfaceId = -1; }
  };

  /// A mesh information.
  struct CMeshBuild {
    /** the IDRV_VF* flags which tells what vertices data are used. See
     * IDriver::setVertexFormat() for more information. NB: IDRV_VF_XYZ is
     * always considered to true.. Note that is some stage use 2 textures
     * coordinates instead of 3, then the extended vertex format must be used
     * isntead
     */
    sint32 VertexFlags;
    uint8 NumCoords[CVertexBuffer::MaxStage]; // tells for each uvw if is uses 2
                                              // or 3 coords
    uint8 UVRouting[CVertexBuffer::MaxStage]; // gives the uv routing table.
                                              // Each final UV channel can be
                                              // routed to any vertex uv

    // Vertices array
    std::vector<CVector> Vertices;

    // Palette Skinning Vertices array (same size as Vertices). NULL if no
    // skinning.
    std::vector<CSkinWeight> SkinWeights;

    // Bones name. Each matrix id used in SkinWeights must have a corresponding
    // string in the bone name array.
    ...
    NLMISC::CBitSet InterfaceVertexFlag; // each bit indicate if the vertex
...
public:
  ...
  /// Build a mesh from material info, and a builded MeshGeom. WARNING: This has
  /// a side effect of deleting AnimatedMaterials.
   ...
   *  \param matrixBlockIndex the index of the matrix block the rendering passes
   * belong to
   */
  uint getNbRdrPass(uint matrixBlockIndex) const;

  /** get the primitive block associated with a rendering pass of a matrix block
   *  \param matrixBlockIndex the index of the matrix block the renderin pass
   * belong to \param renderingPassIndex the index of the rendering pass in the
   * matrix block
   */
  const CIndexBuffer &getRdrPassPrimitiveBlock(uint matrixBlockIndex,
   ...
   * matrix block
   */
  uint32 getRdrPassMaterial(uint matrixBlockIndex,
                            uint renderingPassIndex) const;
...
public:
   ...
   */
  uint getNbRdrPass(uint matrixBlockIndex) const {
    return (uint)_MatrixBlocks[matrixBlockIndex].RdrPass.size();
  }
  ...
  uint32 getRdrPassMaterial(uint matrixBlockIndex,
                            uint renderingPassIndex) const {
    return _MatrixBlocks[matrixBlockIndex]
        .RdrPass[renderingPassIndex]
        .MaterialId;
  }
  ...
  /** Tool function to retrieve triangles geometry only of the mesh (of all
   *rdrpass). return false if the index buffer cannot be read (resident)
   */
  ...
  bool intersectSkin(CTransformShape *mi, const CMatrix &toRaySpace,
                     float &dist2D, float &distZ, bool computeDist2D);

  // @}

  /** render the mesh geometry with a single material. Render is said "Simple"
   *because no special features are used:
   ...
   *		- .....
   */
  void renderSimpleWithMaterial(IDriver *drv, const CMatrix &worldMatrix,
                                CMaterial &mat);
  ...
  IMeshVertexProgram *getMeshVertexProgram() const {
...
private:
   ...
   * NB: we must store palette info by corner (not by vertex) because Matrix
   * Block grouping may insert vertex discontinuities. eg: a vertex use
   * Matrix18. After Matrix grouping (16matrix max), Matrix18 could be Matrix2
   * for a group of face, but Matrix13 for another!!
   */
   ...
   */
  struct CBoneTmp {
    // How many ref points on it? (NB: a corner may have multiple (up to 4) on
    // it).
    uint RefCount;
    // Am i inserted into the current matrixblock?
    bool Inserted;
    // If I am inserted into the current matrixblock, to which local bone (0-15)
    // I am linked?
    uint MatrixIdInMB;
  ...
  struct CFaceTmp {
    ...
    void buildBoneUse(std::vector<uint> &boneUse,
...
private:
  ...
  void findVBId(TCornerSet &corners, const CCornerTmp *corn,
                sint &currentVBIndex, const CVector &vert,
        ...
        if (CCornerTmp::Flags & (CVertexBuffer::TexCoord0Flag << i)) {
          ...
          case 3:
            vba.setValueFloat3Ex(
                (CVertexBuffer::TValue)(CVertexBuffer::TexCoord0 + i), id,
                corn->Uvws[i].U, corn->Uvws[i].V, corn->Uvws[i].W);
            break;
  ...
  void computeSkinMatrixes(CSkeletonModel *skeleton, CMatrix3x4 *matrixes,
                           CMatrixBlock *prevBlock, CMatrixBlock &curBlock);
  void computeSoftwarePointSkinning(CMatrix3x4 *matrixes, CVector *srcVector,
                                    CPaletteSkin *srcPal, float *srcWgt,
                                    CVector *dstVector);
  void computeSoftwareVectorSkinning(CMatrix3x4 *matrixes, CVector *srcVector,
                                     CPaletteSkin *srcPal, float *srcWgt,
                                     CVector *dstVector);

...
template <> inline const char *CPSAttribMaker<CPlaneBasis>::getType() {
  return "CPlaneBasis";
}

/** these are some attribute makers for plane_basis
 * This is a plane basis class. It just blend between 2 plane by linearly
 * interpolating the normal (non sampled version)
 */
...
public:
  ...
  CPSPlaneBasisBlender(
      const CPlaneBasis &startBasis = CPlaneBasis(NLMISC::CVector::I),
...
public:
  ...
  /**
   *	Construct the value gradient blender by passing a pointer to a float
   *table. \param nbStages The result is sampled into a table by linearly
   *interpolating values. This give the number of step between each value \param
   *nbCycles : The nb of time the pattern is repeated during particle life. see
   *ps_attrib_maker.h
   */

  CPSPlaneBasisGradient(
      const CPlaneBasis *basisTab = CPSPlaneBasisGradient::DefaultPlaneBasisTab,
 ...
 * It set the plane basis to have its normal in the same direction than speed of
 * the located The cycle param has no effect o the direction
 */
...
public:
  CPSPlaneBasisFollowSpeed()
      : CPSAttribMaker<CPlaneBasis>(1), _ProjectionPlane(NoProjection) {}
   ...
   * index of the first element in the located
   */

  virtual void *make(CPSLocated *loc, uint32 startIndex, void *tab,
                     uint32 stride, uint32 numAttrib, bool enableNoCopy = false,
                     uint32 srcStep = (1 << 16),
  ...
  CPSAttribMakerBase *clone() const {
    return new CPSPlaneBasisFollowSpeed(*this);
  }
  //
...
/** this memorize value by applying some function based on the emitter. For a
 * particle's attribute, each particle has its own value memorized You MUST
 * called setScheme (from CPSAttribMakerMemory) to tell how the value will be
 * generated
 */
...
public:
  ...
  const CPlaneBasis &operator()(float date) const {
    return _PBTab[NLMISC::OptFastFloor(date * _NbSamples)];
  }
...
/// this is a spinner : this compute a basis by applying a rotation over the
/// given axis
// nb : default init is done with nb samples = 0, so must set a number of
// samples that is > 0 before use

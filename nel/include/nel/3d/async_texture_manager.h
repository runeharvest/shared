 ...
 *	Additionaly, store in RAM for each texture load a very low, DXTC1
 *compressed version of the texture. Used for some Lod systems. \author Lionel
 *Berenguier \author Nevrax France \date 2002
 */
class CAsyncTextureManager {
public:
  /// User is free to add bank to this manager. Other methods are used by the
  /// async manager
...
public:
   ...
   *baseLevel first mipmap \maxLevel During time, further mipmap are loaded,
  ...
  /// Setup max texture upload in driver per update() call (in bytes). Default
  /// to 64K
   ...
   *	Else if texture is found in the HLSTextureManager, it is builded (async)
   *from it, else Begin Async loading
   *
   *	Therefore, only CTextureFile are possible. Also note that the texture is
   *uploaded with mipmap by default, and UpLoadFormat is also default (say
   *ITexture::Auto)
   *
   *	If the texture file is not a DDS with mipmap, this is an error. But the
   *system doesn't fail and the file is entirely loaded and uploaded. The
   *problem is that upload is not cut according to maxUpLoadPerFrame, so some
   *freeze may occur.
   */
  uint addTextureRef(const std::string &textName, CMeshBaseInstance *instance,
                     const NLMISC::CVector &position);

  /// release a texture-instance tuple. The texture is released if no more
  /// instance use it.
   ...
   *	The bitmap returned has no mipmaps and should be in DXTC1 (not
   *guaranteed).
   */
  const NLMISC::CBitmap *getCoarseBitmap(uint id) const;

  /** update the manager. New loaded textures are uploaded. Instances are
   *updated to know if all their pending textures have been uploaded.
   */
...
private:
  ...
  public:
    ...
    bool Loaded;
    // true if the texture is loaded in the driver (at least the coarsest
    // level).
    bool UpLoaded;
...
private:
  ...
  // delete the texture and all references in map/array, instance refcount
  // etc...

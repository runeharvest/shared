...
/** We define a set of iterator object that can advance with a fixed point step
 * in the source container We have 2 version for each iterator : iterators that
 * advance with a step of 1, and iterators that advance with a fixed point (16 :
 * 16).
 */

template <typename TBaseIter> struct CPSBaseIterator {
  TBaseIter Iter;
...
/** This special iterator on a vector attributes enables to convert the speed to
 * its norm It is for private use only, and it has not all the functionnalities
 * of an iterator. The src datas can't be modified as we return the norm, and
 * not a reference on the value
 */
 ...
 *  It is for private use only, and it has not all the functionnalities of an
 * iterator.
 */

struct CRandomIterator {
  GET_INLINE float get() const {
    return float(rand() * (1 / double(RAND_MAX)));
  } // this may be optimized with a table...
  void advance() {}
...
template <class TBaseIter> struct CDistIterator : CPSBaseIterator<TBaseIter> {
  NLMISC::CVector V;
  float Offset;
  CDistIterator(const TBaseIter &it) : CPSBaseIterator<TBaseIter>(it) {}
};

/// this iterator perform a dot prod with a vector, add an offset and take the
/// fabs of the result
template <class TBaseIter> struct CFDot3AddIterator : CDistIterator<TBaseIter> {
  GET_INLINE
  float get() const {
...
/// this iterator perform a dot prod with a vector, add an offset. If it is
/// negatif it return MaxInputValue, and take the abs of the result
template <class TBaseIter>
...
/// this iterator perform a dot prod with a vector, add an offset. If it is
/// negatif it return MaxInputValue, and take the square of the result
template <class TBaseIter>

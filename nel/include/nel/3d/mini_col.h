...
public:
  ...
  /// Add a zone to the collision system. Zone must be loaded into the landscape
  /// before.
  void addZone(uint16 zoneId);

  /// Remove a zone from the collision system. Zone do not have to be loaded
  /// into the landscape.
  ...
  /** This function test if a move is OK, by snaping it to ground, and test if
   * angle is<45deg. If !OK, cur is set to prec, and false is returned.
   */
  bool testMove(const CVector &prec, CVector &cur);

  /** This function snap a position on the current set of faces.
   * hbot and hup are the margin where pos.z can't change. (the pos can't move
   * higher than +hup and lower than -hbot)
   */
   ...
   */
  bool getGroundNormal(const CVector &pos, CVector &normal, float hup = 0.5,
                       float hbot = 1000);
...
private:
  void addFaces(const std::vector<CTriangle> &faces, uint16 zoneId,
                uint16 patchId);
  void addLandscapePart(uint16 zoneId, uint16 patchId);
  void removeLandScapePart(uint16 zoneId, uint16 patchId,

...
// ***************************************************************************
/**
 *	A block of vegetable instances. Instaces are added to an IG, and deleted
 *when the ig is deleted. Internal to VegetableManager. Just an Handle for
 *public. \author Lionel Berenguier \author Nevrax France \date 2001
 */
...
public:
  ...
  /** The Vegetable Light Information. Contains list of PointLights which
   *influence the vegetIg. NB: PointLights ptrs are kept so the ig must be
   *deleted when the lights are deleted.
   */
...
private:
  ...
  struct CVegetableRdrPass {
    ...
    bool HardMode;
    // List of vertices used (used for deletion of the ig, and also for change
    // of HardMode).
  ...
  // For now, there is only 5 render pass: Lighted and Unlit combined with
  // 2Sided or not + the ZSort one.
  CVegetableRdrPass _RdrPass[NL3D_VEGETABLE_NRDRPASS];

  // list of triangles order, for quadrant ZSorting. only for
  // NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT rdrpass. this is why this don't
  // appear in CVegetableRdrPass
  ...
  // If the Igs contains some instance in
  // NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT rdrpass, this flag is true.
...
// ***************************************************************************
/**
 *	Mirror struct of CVegetableInstanceGroup, for reserveIg system in
 *CVegetableManager. Internal to VegetableManager. \author Lionel Berenguier

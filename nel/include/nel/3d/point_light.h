 ...
 *	With the special sunLight, this is the only light which can interact
 *with CTransform models in the standard lighting system.
 *
 *	Only Positionnal with or without attenuation are supported. no
 *directionnal. This restriction is for faster rendering, especially if
 *VertexProgram is used. New: Spot are managed but VertexProgrammed meshes won't
 *use localAttenuation. Special Ambiant are provided too but they are considered
 *like PointLight for dynamic light. They are used in a special way for static
 *light in Igs.
 *
...
public:
  ...
  /// Set the diffuse and specular color of the light to the same value. don't
  /// modify _Ambient.
   ...
   *	clamp(attenuationBegin,0 , +oo) and calmp(attenuationEnd,
   *attenuationBegin, +oo)
   */
  ...
  /** setup the spot AngleBegin and AngleEnd that define spot attenuation of the
   *light. Useful only if SpotLight NB: clamp(angleBegin, 0, PI);
   *clamp(angleEnd, angleBegin, PI); Default is PI/4, PI/2
   */
  ...
  /// Compute a linear attenuation from a point according to attenuation and
  /// spot setup. Return [0,1]
  float computeLinearAttenuation(const CVector &pos) const;

  /** Compute a linear attenuation from a point and precomputed distance
   *according to attenuation and spot setup. Return [0,1] \param modelRadius if
   *!0, suppose the point is a sphere, and compute the approximate Max
   *attenuation from every point on this sphere
   */
  float computeLinearAttenuation(const CVector &pos, float precomputedDist,
                                 float modelRadius = 0) const;

  /// setup the CLight with current pointLight state. factor is used to modulate
  /// the colors.
  void setupDriverLight(CLight &light, uint8 factor);

  /** setup the CLight with current pointLight state. Don't use driver
   *Attenuation and use software one setuped with an additional userAttenuation
   *	\param factor is used to modulate the colors. Should also integrate
   *light attenuation.
   */
...
private:
  ...
  // static	NLMISC::CBlockMemory<CTransform*, false>
  // _LightedModelListMemory; LightedModels. NB: do not contains models that
  // have this light in their FrozenStaticLightSetup

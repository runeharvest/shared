...
/**
 * CEvent3dMouseListener is a listener that handle a 3d matrix with mouse
 * events. This can be the view matrix, or the matrix of any object. \author
 * Cyril Corvazier \author Nevrax France \date 2000
 */
class CEvent3dMouseListener : public NLMISC::IEventListener,
                              public U3dMouseListener {
public:
  /**
   * Mouse mode.
  ...
  /**
   * Constructor.
   * You should call setMatrix, setFrustrum, setViewport, setHotStop and
   * setMouseMode to initialize the whole object. By default, the viewmatrix is
   * identity, the frustrum is (-1,1,-1,1,1,-1), the hot spot is (0,0,0) and the
   * viewport is fullscreen. The mouse mode is set to the NelStyle.
   */
   ...
   * \see getViewMatrix()
   */
  void setMatrix(const NLMISC::CMatrix &matrix) { _Matrix = matrix; }
   ...
   * getModelMatrix()
   */
  void setModelMatrix(const NLMISC::CMatrix &matrix) { _ModelMatrix = matrix; }

  /**
   * Set the current frustrum to use.
   * \param frustrum is the frustrum.
   */
  void setFrustrum(const CFrustum &frustrum) { _Frustrum = frustrum; }

  /**
   * Set the viewport in use in the window. By default, the viewport is
   * fullwindow. \param viewport is the viewport to use. All events outside the
   * viewport are ignored.
   */
  void setViewport(const NL3D::CViewport &viewport) { _Viewport = viewport; }

  /**
   * Set the current hot spot.
   * \param hotSpot is the target to use when the mouse move. It can be for
   * exemple the center. of the selected object. The hotspot is not modified by
   * mouse events. \see getViewMatrix()
   */
  void setHotSpot(const CVector &hotSpot) { _HotSpot = hotSpot; }
   ...
   * \see TMouseMode
   */
  void setMouseMode(TMouseMode mouseMode) { _MouseMode = mouseMode; }

  /// enable / disable model matrix edition mode. (the default deals with the
  /// with matrix)
  ...
  /** Each move of the modelMatrix is first transformed by this matrix before
   *applying. NB: rot are still done in local \transModelMove Trans part is
   *removed
   */
  ...
  /** if true, XY translation is made in XY World plane (instead of XY view
   *plane). false by default.
   */
   ...
   * \see setModelMatrix()
   */
  const NLMISC::CMatrix &getModelMatrix() { return _ModelMatrix; }

  /**
   * Get the current hot spot.
   * \return the target used when the mouse move. It can be for exemple the
   * center. of the selected object. The hotspot is not modified by mouse
   * events. \see getViewMatrix()
   */
  CVector getHotSpot() const { return _HotSpot; }

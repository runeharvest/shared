...
public:
  ...
  private:
    ...
    // Point to _TextureInstanceData. In this array, First come the name
    // (0-ended), then the CHLSColorDelta.
    uint32 _DataIndex;
  ...
  public:
    ...
    void buildAsKey(const char *ptr) { _DataPtr = (uint8 *)ptr; }
...
public:
   ...
   *	\param name name of the colored texture. NB: it is lowered in this
   *method \param colorTextureId index returned by addColorTexture() \param cols
   *must be same size of number of mask of the colorTextureId pointed (nlassert)
   */
  void addTextureInstance(const std::string &name, uint32 colorTextureId,

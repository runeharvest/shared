...
/**
 * This class list all the particle systems that have resources allocated at a
 * given time, so that we can remove the resource of those who are too far from
 * the viewer. This has been added because objects that are in a cluster that is
 * not visible are not parsed during traversals, so their resources couldn't
 * just be released Only a few systems are parsed each time (because they can be
 * numerous). Call to that class should be made by CScene, so you don't need to
 * use it \author Nicolas Vizerie \author Nevrax France \date 2002
 */
...
public:
  ...
  void refreshModels(const std::vector<NLMISC::CPlane> &worldFrustumPyramid,
                     const NLMISC::CVector &viewerPos);

  /// perform animation on systems that should be animated even if not parsed
  /// (temporary spells for example)
...
private:
  ...
  public:
    ...
    NLMISC::CMatrix
        OldAncestorMatOrRelPos; // last matrix of ancestor skeleton or relative
                                // matrix of ps to its ancestor (see flag below)
  ...
  addPermanentlyAnimatedSystem(CParticleSystemModel *);
...
private:
  TModelList::iterator
      _CurrListIterator; /// the current element being processed

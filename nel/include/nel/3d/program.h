...
// IProgramDrvInfos represent the real data of the GPU program, stored into the
...
// Features exposed by a program. Used to set builtin parameters on user
// provided shaders. This is only used for user provided shaders, not for
// builtin shaders, as it is a slow method which has to go through all of the
// options every time. Builtin shaders should set all flags to 0. Example:
...
// Stucture used to cache the indices of builtin parameters which are used by
// the drivers Not used for parameters of specific nl3d programs
...
public:
  enum TProfile {
    ...
    nelvp = 0x31010001, // VP supported by CVertexProgramParser, similar to
                        // arbvp1, can be translated to vs_1_1
    ...
    fp40 = 0x61020008,   // NV_fragment_program2, arbfp1 with "OPTION
...
public:
  ...
  // Add multiple sources using different profiles, the driver will use the
  ...
  // Get the idx of a parameter (ogl: uniform, d3d: constant, etcetera) by name.
  // Invalid name returns ~0
  inline uint getUniformIndex(const char *name) const {
    return m_DrvInfo->getUniformIndex(name);
  };
  inline uint getUniformIndex(const std::string &name) const {
    return m_DrvInfo->getUniformIndex(name.c_str());
  };
  inline uint getUniformIndex(CProgramIndex::TName name) const {
    return m_Index.Indices[name];
  }

  // Get feature information of the current program
  inline CSource *source() const { return m_Source; };
  inline const CProgramFeatures &features() const {
    return m_Source->Features;
  };
  inline TProfile profile() const { return m_Source->Profile; }

  // Build feature info, called automatically by the driver after compile
  // succeeds

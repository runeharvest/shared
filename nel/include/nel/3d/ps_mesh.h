...
const uint ConstraintMeshMaxNumVerts =
    512; // the maximum number of vertices for a constraint mesh
const uint ConstraintMeshBufSize =
    64; // number of meshs to be processed at once...
const uint ConstraintMeshMaxNumPrerotatedModels =
    32; // maximum number of meshs that can be prerotated

/** This class is for mesh handling. It operates with any mesh, but it must
 * insert them in the scene... It is not very adapted for lots of little meshs..
 *  To create the mesh basis, we use CPlaneBasis here. It give us the I and J
 * vector of the basis for each mesh and compute K ( K =  I ^ J)
 */
...
/** This class is for mesh that have very simple geometry. The constraint is
 * that they can only have one matrix block. They got a hint for constant
 * rotation scheme. With little meshs, this is the best to draw a maximum of
 * them
 */
...
public:
  ...
  enum TError {
    ShapeFileNotLoaded = -1,
    ShapeFileIsNotAMesh = -2,
    ShapeHasTooMuchVertices = -3
  };
...
public:
  ...
  /// Get the shape used for those particles. (must use no morphing or an
  /// assertion is raised)
   ...
   * NB : Morphing not supported with precomputed rotations. First mesh is used
   * instead \param shapesNames A tab of string containing the names of the
   * shapes \param numShapes
   */
  ...
  /// Use a constant value for morphing. This discard any scheme for the morph
  /// value. The value must range from 0 to numberOfShapes
  ...
  /** Tells that all meshs are turning in the same manner, and only have a
   *rotationnal bias This is a lot faster then other method. Any previous set
   *scheme for 3d rotation is kept. \param: the number of rotation configuration
   *we have. The more high it is, the slower it'll be If this is too low, a lot
   *of particles will have the same orientation If it is 0, then the hint is
   *disabled. This can't be higher than ConstraintMeshMaxNumPrerotatedModels
   *  \param  minAngularVelocity : the maximum angular velocity for particle
   *rotation \param  maxAngularVelocity : the maximum angular velocity for
   *particle rotation \see    CPSRotated3dPlaneParticle
   */
   ...
   * This allow to put colors on meshs that haven't got material that allow
   * them. \param stage The stage the modulation applies on. Range from 0 to
   * IDRV_MAT_MAXTEXTURES - 1. \param force True enable modulation, false
   * disable it.
   */
  void forceStageModulationByColor(uint stage, bool force);

  /// Test if the i-th stage of all materials is forced to be modulated with the
  /// mesh color
  ...
  void forceVertexColorLighting(bool force = true) {
    _VertexColorLightingForced = force;
  }

  /// test whether vertex color lighting is forced.
  bool isVertexColorLightingForced() const {
    return _VertexColorLightingForced;
  }

  /// Setup the buffers used with prerotated meshs. Must be called during
  /// initialization.
  ...
  /// Set the type of texture animation to use. None is the default. Setting a
  /// new value discard the previous change.
  ...
  //\name Global texture animation. Calls to these method are only valid if
  //texture animation is global.
  //@{
  ...
  /// Set the properties of texture animation for a texture stage. Global
  /// animation should have been activated.
  void setGlobalTexAnim(uint stage, const CGlobalTexAnim &properties);

  /// Get the properties of texture animation.Global animation should have been
  /// activated.
  const CGlobalTexAnim &getGlobalTexAnim(uint stage) const;

  /// Force the time counter for global anim to be reseted when a new mesh is
  /// created.
  void forceGlobalAnimTimeResetOnNewElement(bool force = true) {
    _ReinitGlobalAnimTimeOnNewElement = force;
  }
  bool isGlobalAnimTimeResetOnNewElementForced() const {
    return _ReinitGlobalAnimTimeOnNewElement != 0;
  }

  //@}

  /** test if mesh is correctly loaded and built (e.g all shape have been loaded
   * and have compatible format)
   * * NB : this force the meshs to be loaded
   */
  bool isValidBuild() const;

  /** get number of vertices for each mesh, or an error code if loading failed
   * (see TError enum) NB : this force the meshs to be reloaded
   */
...
protected:
   ...
   * \param opaque true if we are dealing with the opaque pass, false for
   * transparent faces
   */
   ...
   */
  void computeColors(CVertexBuffer &outVB, const CVertexBuffer &inVB,
                     uint startIndex, uint toProcess, uint32 srcStep,
                     IDriver &drv, CVertexBufferReadWrite &vba,
   ...
   * \param  numVerts, if not NULL, the dest vector will be filled with the
   * number of vertices of each mesh (or a TError enumerated value if loading
   * failed) \return true if the mesh could be found and match the requirement
   */
  ...
  /** A rendering pass. The primitive block contains several duplication of the
   * primitives of the original mesh, in order to draw several of them at once
   */
  class CRdrPass {
  public:
    CMaterial Mat;
  ...
  public:
    CMeshDisplay() { VB.setName("CPSConstraintMesh::CMeshDisplay"); }
  };

  void restoreMaterials();

  /// Setup a set of rendering passes (set good textures matrix / material
  /// colors)
  void setupRenderPasses(float date, TRdrPassSet &rdrPasses, bool opaque);

  /// Perform a set of rendering passes. The VB must have been activated in the
  /// driver before to call this
  void doRenderPasses(IDriver *driver, uint numObj, TRdrPassSet &rdrPasses,
                      bool opaque);
  ...
  public:
    /// ctor giving the max number of CMeshDisplay structures to be kept
    /// simultaneously.
    CMeshDisplayShare(uint maxNumMD) : _MaxNumMD(maxNumMD), _NumMD(0) {}

    // Retrieve a mesh display associated with the given mesh
    CMeshDisplay &getMeshDisplay(CMesh *mesh, const CVertexBuffer &meshVB,
                                 uint32 format);
  ...
  private:
    ...
    static void buildVB(CVertexBuffer &dest, const CVertexBuffer &meshVB,
                        uint32 format);
  };
   ...
   * If a mesh has been already setupped in the driver for another use than
   * particle system meshes. then locking it will return us an agp or vram
   * pointer, which is baaad because we are reading from it when writing each
   * instance to the final vb. This maps a mesh name, to its ram vb NB / TODO
   * Nico : would greatly benefits from real instancing using SetStreamFreq
   * (this code was originally written in 2001 ...) ...
   * TODO 2 : get rid of all theses statics !!! move then in scene at least
   * (ideally in a render context or something similar, because it may be shared
  ...
  /** Setup material so that global or per mesh color is taken in account.
   * Useful if material hasn't been setup correctly in the export
   */
  ...
  uint8 _Touched : 1;
  // flags that indicate whether the object has transparent faces. When the
  // 'touch' flag is set, it is undefined, until the next update() call.
  uint8 _HasTransparentFaces : 1;
  // flags that indicate whether the object has opaques faces. When the 'touch'
  // flag is set, it is undefined, until the next update() call.
...
private:
  CPSConstraintMesh(const CPSConstraintMesh &)
      : CPSParticle(), CPSSizedParticle(), CPSRotated3DPlaneParticle(),
        CPSHintParticleRotateTheSame(), CPSShapeParticle(),
        CPSColoredParticle() {

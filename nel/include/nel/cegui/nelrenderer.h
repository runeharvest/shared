...
private:
  ...
  static const int
      VERTEXBUFFER_CAPACITY; //!< capacity of the allocated vertex buffer

  ImageCodec *m_ImageCodec; //!< Holds a pointer to the image codec to use.
  DynamicModule
      *m_ImageCodecModule; //!< Holds a pointer to the image codec module. If
                           //!< d_imageCodecModule is 0 we are not owner of the
                           //!< image codec object
    ...
    virtual void operator()(const NLMISC::CEvent &event) {
      ...
      try {
        ...
        if (event == NLMISC::EventMouseDownId ||
            event == NLMISC::EventMouseUpId ||
            event == NLMISC::EventMouseMoveId ||
            event == NLMISC::EventMouseDblClkId ||
            event == NLMISC::EventMouseWheelId) {
          if (!m_MouseActive) {
            // we're not processing any mouse activity. The cursor isn't
            // captured maybe?
            return;
          }
          ...
          if (event == NLMISC::EventMouseDownId) {
            ...
            float delta_y =
                (float)((1.0f - mouseEvent->Y) - m_MouseY) * m_Height;

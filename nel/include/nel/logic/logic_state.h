...
public:
   ...
   *
   * \param msgs is the map associating all the message to send with their
   *destination id
   */
  void
  ...
  /**
   * Fill a map associating all the referenced var in the state with the id of
   * service managing them (debug purpose)
   */
  void fillVarMap(
      std::multimap<NLMISC::CEntityId, std::string> &stateMachineVariables);

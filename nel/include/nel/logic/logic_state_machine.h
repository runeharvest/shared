...
public:
  ...
  void addVariable(CLogicVariable var) {
    _Variables.insert(std::make_pair(var.getName(), var));
  }
  ...
  void addCounter(CLogicCounter counter) {
    _Counters.insert(std::make_pair(counter.getName(), counter));
  }
   ...
   */
  void
  getMessagesToSend(std::multimap<NLMISC::CEntityId, NLNET::CMessage> &msgs);
   ...
   * \param modifOperator can be one of these operators
   *:"SET"("set"),"ADD"("add"),"SUB"("sub"),"MUL"("mul"),"DIV"("div") \param
   *value is the value to use along with the modificator
   */
  void modifyVariable(std::string varName, std::string modifOperator,
                      sint64 value);

...
public:
   ...
   */
  virtual bool getEntryType(uint entry, TEntryType &type,
                            bool &array) const = 0;
  ...
  virtual bool getEntryIndexByName(uint &entry,
   ...
   * \return true if successed, false if the entry doesn't exist or is a virtual
   * DFN.
   */
   ...
   * \return true if successed, false if the entry doesn't exist or is not a
   * DFN.
   */

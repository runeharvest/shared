 ...
 * that contains this values (using serialCont). The next launch, the function
 will
 * only load the packed file and if some sheet have changed, it'll automatically
 regenerate
 * the packed file.
 *
 * To use the loadForm(), you first have to create a class that will contains
 values for one sheet.
 * This class must also implements 2 functions (readGeorges() and serial()) and
 1 static function (getVersion())
 *
 ...
 * Classical use (copy/paste this in your code):
    ...
    public:
        ...
        void readGeorges (const NLMISC::CSmartPtr<NLGEORGES::UForm> &form, const
 NLMISC::CSheetId &sheetId)
        {
            // the form was found so read the true values from George
            form->getRootNode ().getValueByName (WalkSpeed,
 "Basics.MovementSpeeds.WalkSpeed"); form->getRootNode ().getValueByName
 (RunSpeed, "Basics.MovementSpeeds.RunSpeed");
        }
        ...
        // This method is called when a sheet have been read from the packed
 sheet
        ...
        // return the version of this class, increments this value when the
 content of this class changed static uint getVersion () { return 1; }
    };

    // this structure is filled by the loadForm() function and will contain all
 you need std::map<NLMISC::CSheetId,CContainerEntry> Container;

    void init ()
    {
        // load the values using the george sheet or packed file and fill the
 container loadForm(".creature", "test.packed_sheets", Container);
    }

 * Now you can access the Container (using the CSheedId) to know the WalkSpeed
 and RunSpeed of all creatures.
 *
 */
...
// This Version may be used if you want to use the serialVersion() system in
// loadForm()
const uint32 PACKED_SHEET_VERSION_COMPATIBLE = 0;

// ***************************************************************************
/** This function is used to load values from georges sheet in a quick way.
 * \param sheetFilter a vector of string to filter the sheet in the case you
 * need more than one filter \param packedFilename the name of the file that
 * this function will generate (extension must be "packed_sheets") \param
...
void loadForm(const std::vector<std::string> &sheetFilters,
  ...
  // check the extension (i know that file like "foo.packed_sheetsbar" will be
  // accepted but this check is enough...)
  nlassert(packedFilename.find(".packed_sheets") != std::string::npos);

  std::string packedFilenamePath = NLMISC::CPath::lookup(
      NLMISC::CFile::getFilename(packedFilename), false, false);
  ...
  try {
    ...
    if (!ifile.open(packedFilenamePath)) {
      throw NLMISC::Exception("can't open PackedSheet %s",
                              packedFilenamePath.c_str());
    }
    // an exception will be launch if the file is not the good version or if the
    // file is not found
    ...
    ifile.serial(ver);
    if (ver != T::getVersion())
      throw NLMISC::Exception(
          "The packed sheet version in stream is different of the code");
    ifile.serialCont(container);
    ifile.close();
  } catch (const NLMISC::Exception &e) {
    // clear the container because it can contains partially loaded sheet so we
    // must clean it before continue
    container.clear();
    if (!updatePackedSheet) {
      if (errorIfPackedSheetNotGood)
        nlerror("loadForm(): Exception during reading the packed file and "
                "can't reconstruct them (%s)",
                e.what());
      else
        nlinfo("loadForm(): Exception during reading the packed file and can't "
               "reconstruct them (%s)",
               e.what());

      return;
    } else {
      nlinfo("loadForm(): Exception during reading the packed file, I'll "
             "reconstruct it (%s)",
             e.what());
    }
  }
  ...
  // set up the current sheet in container to remove sheet that are in the
  // container and not in the directory anymore
  std::map<NLMISC::CSheetId, bool> sheetToRemove;
  for (typename std::map<NLMISC::CSheetId, T>::iterator it = container.begin();
       it != container.end(); it++) {
    sheetToRemove.insert(std::make_pair((*it).first, true));
  }

  // check if we need to create a new .pitems or just read it
  uint32 packedFiledate =
      NLMISC::CFile::getFileModificationDate(packedFilenamePath);
      ...
      for (uint i = 0; i < depends.size(); ++i) {
        if (dependencyDates[depends[i]] > packedFiledate) {
          nldebug("Dependency on %s for %s not up to date !",
                  dictionnary[depends[i]].c_str(),
                  sheetIds[k].toString().c_str());
          NeededToRecompute.push_back(k);
          break;
        }
      }
  ...
  nlinfo("%d sheets checked, %d need to be recomputed", filenames.size(),
         NeededToRecompute.size());
  ...
  for (uint j = 0; j < NeededToRecompute.size(); j++) {
    if (NLMISC::CTime::getLocalTime() > last + 5000) {
      ...
      if (j > 0)
        nlinfo("%.0f%% completed (%d/%d), %d seconds remaining",
               (float)j * 100.0 / NeededToRecompute.size(), j,
               NeededToRecompute.size(),
               (NeededToRecompute.size() - j) * (last - start) / j / 1000);
    }

    // create the georges loader if necessary
    if (formLoader == NULL) {
      NLMISC::WarningLog->addNegativeFilter(
          "CFormLoader: Can't open the form file");
    ...
    form =
        formLoader->loadForm(sheetIds[NeededToRecompute[j]].toString().c_str());
    if (form) {
      // build the dependency data
      {
        ...
        nlassert(dependFiles.find(sheetIds[NeededToRecompute[j]].toString()) !=
                 dependFiles.end());
        // remove the sheet itself from the container
        dependFiles.erase(sheetIds[NeededToRecompute[j]].toString());

        std::set<std::string>::iterator first(dependFiles.begin()),
            last(dependFiles.end());
        for (; first != last; ++first) {
          const std::string filename = NLMISC::CFile::getFilename(*first);
          std::map<std::string, uint>::iterator findDicIt =
              dictionnaryIndex.find(filename);
          ...
          if (!p.empty()) {
            ...
            dictionnaryIndex.insert(
                std::make_pair(filename, (uint)dictionnary.size()));
      ...
      // add the new creature, it could be already loaded by the packed sheets
      // but will be overwritten with the new one
      typedef typename std::map<NLMISC::CSheetId, T>::iterator TType1;
      typedef typename std::pair<TType1, bool> TType2;
      TType2 res =
          container.insert(std::make_pair(sheetIds[NeededToRecompute[j]], T()));
  ...
  if (!NeededToRecompute.empty())
    nlinfo("%d seconds to recompute %d sheets",
           (uint32)(NLMISC::CTime::getLocalTime() - start) / 1000,
           NeededToRecompute.size());
  ...
  // we have now to remove sheets that are in the container and not exist
  // anymore in the sheet directories
  for (std::map<NLMISC::CSheetId, bool>::iterator it2 = sheetToRemove.begin();
       it2 != sheetToRemove.end(); it2++) {
    if ((*it2).second) {
      nlinfo("the sheet '%s' is not in the directory, remove it from container",
             (*it2).first.toString().c_str());
  ...
  try {
    if (containerChanged) {
      ...
      std::map<NLMISC::CSheetId, std::vector<uint32>>::iterator first(
          dependencies.begin()),
          last(dependencies.end());
      for (; first != last; ++first) {
      ...
      // Then get the dictionary + dependencies size, and write it back to
      // posBlockSize
    ...
    nlinfo("loadForm(): Exception during saving the packed file, it will be "
           "recreated next launch (%s)",
           e.what());
  }
 ...
 * \param packedFilename the name of the file that this function will generate
 * (extension must be "packed_sheets") \param container the map that will be
 * filled by this function
 */
template <class T>
void loadForm(const std::string &sheetFilter, const std::string &packedFilename,
              std::map<NLMISC::CSheetId, T> &container,
              bool updatePackedSheet = true,
              bool errorIfPackedSheetNotGood = true) {
  ...
  loadForm(vs, packedFilename, container, updatePackedSheet,
           errorIfPackedSheetNotGood);
}

// ***************************************************************************
// variant with smart pointers, maintain with function above
template <class T>
void loadForm2(const std::vector<std::string> &sheetFilters,
               const std::string &packedFilename,
  ...
  // set up the current sheet in container to remove sheets that are in the
  // container and not in the directory anymore
  std::map<NLMISC::CSheetId, bool> sheetToRemove;
  for (typename std::map<NLMISC::CSheetId, NLMISC::CSmartPtr<T>>::iterator it =
           container.begin();
       it != container.end(); it++) {
    sheetToRemove.insert(std::make_pair((*it).first, true));
  }
  ...
  for (uint j = 0; j < NeededToRecompute.size(); j++) {
    ...
    if (form) {
      // build the dependency data
      {
        ...
        for (; first != last; ++first) {
          ...
          if (!p.empty()) {
            ...
            // add a new dictionnary entry
            dicIndex = (uint)dictionnary.size();
            dictionnaryIndex.insert(std::make_pair(
                filename, (NLMISC::TSStringId)dictionnary.size()));
            dictionnary.push_back(filename);
      ...
      // but will be overwritten with the new one
      typedef
          typename std::map<NLMISC::CSheetId, NLMISC::CSmartPtr<T>>::iterator
              TType1;
      typedef typename std::pair<TType1, bool> TType2;
      TType2 res = container.insert(std::make_pair(
          sheetIds[NeededToRecompute[j]], NLMISC::CSmartPtr<T>(new T())));
  ...
  try {
    if (containerChanged) {
      ...
      // Then get the dicionary + dependencies size, and write it back to
      // posBlockSize
      sint32 endBlockSize = ofile.getPos();
      dependBlockSize = (endBlockSize - posBlockSize) - 4;
      ofile.seek(posBlockSize, NLMISC::IStream::begin);
...
// variant with smart pointers, maintain with function above
template <class T>
void loadForm2(const std::string &sheetFilter,
               const std::string &packedFilename,
               std::map<NLMISC::CSheetId, T> &container,
               bool updatePackedSheet = true,
               bool errorIfPackedSheetNotGood = true) {
  std::vector<std::string> vs;
  vs.push_back(sheetFilter);
  loadForm2(vs, packedFilename, container, updatePackedSheet,
            errorIfPackedSheetNotGood);
}
...
void loadForm(const std::vector<std::string> &sheetFilters,
  ...
  std::string packedFilenamePath =
      NLMISC::CPath::lookup(packedFilename, false, false);
  if (packedFilenamePath.empty()) {
    packedFilenamePath = packedFilename;
  }
  ...
  {
    std::vector<std::string>::const_iterator first(sheetFilters.begin()),
        last(sheetFilters.end());
    for (; first != last; ++first)
  ...
  {
    typename std::map<std::string, T>::iterator first(container.begin()),
        last(container.end());
    for (; first != last; ++first)
  ...
  for (uint k = 0; k < sheetNames.size(); k++) {
    ...
    if (d > packedFiledate ||
        container.find(sheetNames[k]) == container.end()) {
      NeededToRecompute.push_back(k);
    } else {
      // check the date of each parent
  ...
  nlinfo("%d sheets checked, %d need to be recomputed", sheetNames.size(),
         NeededToRecompute.size());
  ...
  for (uint j = 0; j < NeededToRecompute.size(); j++) {
    if (NLMISC::CTime::getLocalTime() > lastTime + 5000) {
      ...
      if (j > 0)
        nlinfo("%.0f%% completed (%d/%d), %d seconds remaining",
               (float)j * 100.0 / NeededToRecompute.size(), j,
               NeededToRecompute.size(),
               (NeededToRecompute.size() - j) * (lastTime - start) / j / 1000);
    }
    ...
    if (form) {
      // build the dependency data
      {
        ...
        std::set<std::string> dependFiles;
        form->getDependencies(dependFiles);
        nlassert(dependFiles.find(sheetNames[NeededToRecompute[j]]) !=
                 dependFiles.end());
        // remove the sheet itself from the container
        ...
        for (; first != last; ++first) {
          ...
          if (!p.empty()) {
            //						uint32 date =
            //NLMISC::CFile::getFileModificationDate(p);
      ...
      typedef typename std::pair<TType1, bool> TType2;
      TType2 res = container.insert(
          std::make_pair(sheetNames[NeededToRecompute[j]], T()));
  ...
  // we have now to remove sheet that are in the container and not exist anymore
  // in the sheet directories
  for (std::map<std::string, bool>::iterator it2 = sheetToRemove.begin();
       it2 != sheetToRemove.end(); it2++) {
    if (it2->second) {
  ...
  try {
    if (containerChanged) {
      ...
      uint32 depSize = (uint32)dependencies.size();
      ofile.serial(depSize);
      std::map<std::string, std::vector<uint32>>::iterator first(
          dependencies.begin()),
          last(dependencies.end());
      for (; first != last; ++first) {
...
// ***************************************************************************
template <class T>
void loadFormNoPackedSheet(const std::string &sheetFilter,
                           std::map<NLMISC::CSheetId, T> &container,
...
// variant with smart pointers, maintain with function above
template <class T>
void loadFormNoPackedSheet2(const std::string &sheetFilter,
                            std::map<NLMISC::CSheetId, T> &container,
                            const std::string &wildcardFilter) {
  std::vector<std::string> vs;
  vs.push_back(sheetFilter);
 ...
 * \param sheetFilters a vector of string to filter the sheet (by extension) in
 *the case you need more than one filter \param wildcardFilter an additional by
 *sheet filter (must include the extension) \param container the map that will
 *be filled by this function
 */
template <class T>
void loadFormNoPackedSheet(const std::vector<std::string> &sheetFilters,
                           std::map<NLMISC::CSheetId, T> &container,
                           const std::string &wildcardFilter) {
  // make sure the CSheetId singleton has been properly initialised
  ...
  for (uint k = 0; k < filenames.size(); k++) {
    ...
    if (p.empty())
      continue;
    // check if wildcardok
    if (!wildcardFilter.empty() && !NLMISC::testWildCard(p, wildcardFilter))
      continue;
...
// variant with smart pointers, maintain with function above
template <class T>
void loadFormNoPackedSheet2(
    const std::vector<std::string> &sheetFilters,
    std::map<NLMISC::CSheetId, NLMISC::CSmartPtr<T>> &container,
    const std::string &wildcardFilter) {
  // make sure the CSheetId singleton has been properly initialised
  NLMISC::CSheetId::init(false);

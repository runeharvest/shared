...
bool convertDfnFile(const std::string &oldFileName,
...
class CFormDfn : public UFormDfn {
  ...
  friend bool convertDfnFile(const std::string &oldFileName,
                             const std::string &newFileName);

public:
  // Default cstr
  CFormDfn() { Round = 0xffffffff; }
  ...
  // Get parent DFN
  void getParentDfn(std::vector<const CFormDfn *> &array,
...
private:
  ...
  void read(xmlNodePtr doc, CFormLoader &loader, bool forceLoad,

...
// For legacy targets, assume /volatile:ms is used, and volatile has acquire and
// release semantics In the future, enable /volatile:iso on all C++14 and up
// builds, once volatile usage is removed
...
// For legacy targets, treat volatile loads and stores as atomic with relaxed
// ordering semantics
...
// Define this to add test() operation to CAtomicFlag (may not be supported on
// all platforms) #define NL_ATOMIC_FLAG_TEST
...
public:
  ...
  NL_FORCE_INLINE bool
  test() const // get current value without changing, acq-rel
  {
...
public:
  ...
  test() const // get current value without changing, acq-rel
  {
    return __sync_fetch_and_add(const_cast<volatile int *>(&m_Flag),
                                0); // acq-rel
  }
#endif
#elif (!defined(NL_ATOMIC_FLAG_TEST) && defined(NL_ATOMIC_CPP14)) ||           \
    defined(NL_ATOMIC_CPP20)
private:
...
public:
  ...
  NL_FORCE_INLINE void clear() { m_Flag.clear(std::memory_order_release); }
  ...
  static_assert(sizeof(std::atomic_flag) <= sizeof(int) ||
                    sizeof(std::atomic_flag) <= sizeof(bool),
                "Atomic flag is larger than int and bool, it may be better to "
                "use a native implementation");
#elif defined(NL_ATOMIC_CPP14) && defined(ATOMIC_BOOL_LOCK_FREE) &&            \
...
public:
  ...
  static_assert(sizeof(std::atomic_bool) <= sizeof(bool),
                "Atomic bool is larger than bool, it may be better to use a "
...
public:
  ...
  test() const // get current value without changing, acq-rel
  {
    return _InterlockedExchangeAdd(const_cast<volatile long *>(&m_Flag), 0) !=
           0; // full barrier
  }
#endif
...
public:
  ...
  test() const // get current value without changing, acq-rel
  {
    // No idea if this is legal, but it's the only way to do so without changing
    // the flag value... OSSpinLock is normally just a 32-bit value!
    if (sizeof(OSSpinLock) == sizeof(uint64_t)) {
      return OSAtomicAdd64Barrier(0, const_cast<OSSpinLock *>(&m_SpinLock)) !=
             0; // acq-rel
    } else {
      return OSAtomicAdd32Barrier(0, const_cast<OSSpinLock *>(&m_SpinLock)) !=
             0; // acq-rel
    }
  }
#endif
  ...
  NL_FORCE_INLINE CAtomicFlag() {
    // when used as a global, create without clearing, as global memory is
    // already init to 0 otherwise, it might be cleared after being locked, due
    // to undefined initialization order
  }
...
public:
  ...
  NL_FORCE_INLINE int store(int value,
                            TMemoryOrder order = TMemoryOrderRelease) {
    m_Value.store(value, (std::memory_order)order);
    return value;
  }

  NL_FORCE_INLINE int fetchAdd(int value,
                               TMemoryOrder order = TMemoryOrderAcqRel) {
    return m_Value.fetch_add(value, (std::memory_order)order);
  }

  NL_FORCE_INLINE int exchange(int value,
                               TMemoryOrder order = TMemoryOrderAcqRel) {
    return m_Value.exchange(value, (std::memory_order)order);
  }

  static_assert(sizeof(std::atomic_int) <= sizeof(int),
                "Atomic int is larger than int, it may be better to use a "
                "native implementation");
#elif defined(NL_ATOMIC_WIN32)
private:
...
#else
    return _InterlockedExchangeAdd(const_cast<volatile long *>(&m_Value),
                                   0); // full barrier
#endif
  }
...
public:
  NL_FORCE_INLINE int load(TMemoryOrder order = TMemoryOrderAcquire) const {
    if (order == TMemoryOrderRelaxed)
      return m_Value;
    return __sync_fetch_and_add(const_cast<volatile int *>(&m_Value),
                                0); // acquire
  }
  ...
  NL_FORCE_INLINE int load(TMemoryOrder order = TMemoryOrderAcquire) const {
    if (order == TMemoryOrderRelaxed)
      ...
      return OSAtomicAdd32Barrier(
          0, const_cast<volatile int *>(&m_Value)); // acq-rel
  }
...
public:
  // Atomic operators
  NL_FORCE_INLINE CAtomicInt(int value,
                             TMemoryOrder order = TMemoryOrderRelease) {
    store(value, order);
  }

  NL_FORCE_INLINE CAtomicInt() {
    // when used as a global, you can create without initializing, as global
    // memory is already init to 0 otherwise, it might be cleared after being
    // locked, due to undefined initialization order
  }

  NL_FORCE_INLINE operator int() const { return load(); }

  NL_FORCE_INLINE int operator=(int value) { return store(value); }

  NL_FORCE_INLINE int operator+=(int value) { return fetchAdd(value) + value; }

  NL_FORCE_INLINE int operator-=(int value) { return fetchAdd(-value) - value; }

  NL_FORCE_INLINE int operator++() { return fetchAdd(1) + 1; }

  NL_FORCE_INLINE int operator++(int) { return fetchAdd(1); }

  NL_FORCE_INLINE int operator--() { return fetchAdd(-1) - 1; }

  NL_FORCE_INLINE int operator--(int) { return fetchAdd(-1); }

  NL_FORCE_INLINE CAtomicInt(const CAtomicInt &other) { store(other.load()); }
  ...
  // Compare operators
  NL_FORCE_INLINE bool operator==(int value) const { return load() == value; }
...
template <typename T> class CAtomicEnum {
#ifdef NL_CPP14
...
public:
  ...
  static_assert(sizeof(std::atomic<T>) <= sizeof(T),
                "Atomic enum is larger than enum type, it may be better to use "
...
public:
  ...
  NL_FORCE_INLINE CAtomicEnum(T value) { store(value); }
  ...
  NL_FORCE_INLINE operator T() const { return (T)load(); }

  NL_FORCE_INLINE T operator=(T value) { return store(value); }

  NL_FORCE_INLINE CAtomicEnum(const CAtomicEnum &other) { store(other.load()); }
  ...
  // Compare operators
  NL_FORCE_INLINE bool operator==(T value) const { return load() == value; }
...
public:
  NL_FORCE_INLINE CAtomicLockSpin(CAtomicFlag &flag) : m_Flag(flag) {
    while (m_Flag.testAndSet())
      ;
  }

  NL_FORCE_INLINE ~CAtomicLockSpin() { m_Flag.clear(); }
...
public:
  NL_FORCE_INLINE CAtomicLockYield(CAtomicFlag &flag) : m_Flag(flag) {
    while (m_Flag.testAndSet())
      nlYield();
  }

  NL_FORCE_INLINE ~CAtomicLockYield() { m_Flag.clear(); }
};
...
public:
  ...
  static NL_FORCE_INLINE void leave(CAtomicFlag &flag) { flag.clear(); }

  inline CAtomicLockFast(CAtomicFlag &flag) : m_Flag(flag) { enter(flag); }

  NL_FORCE_INLINE ~CAtomicLockFast() { leave(m_Flag); }
};
...
public:
  ...
  static NL_FORCE_INLINE void enter(CAtomicFlag &flag) {
    ...
    if (flag.testAndSet()) {
      ...
      int lastSpins = last;
      volatile int temp =
          17; // volatile so it's not optimized out, this is just to keep busy
  ...
  inline CAtomicLockFastMP(CAtomicFlag &flag) : m_Flag(flag) { enter(flag); }

  NL_FORCE_INLINE ~CAtomicLockFastMP() { leave(m_Flag); }
};

/// This class wraps an existing CAtomicFlag to have the same semantics as
/// NLMISC::CMutex and std::mutex It uses the CAtomicLockFast locking mechanism
class CFastMutexWrapper {
private:
  CAtomicFlag &m_Flag;

public:
  NL_FORCE_INLINE CFastMutexWrapper(CAtomicFlag &flag) : m_Flag(flag) {}

  inline void lock() { CAtomicLockFast::enter(m_Flag); }

  inline void enter() { CAtomicLockFast::enter(m_Flag); }

  NL_FORCE_INLINE void unlock() { CAtomicLockFast::leave(m_Flag); }

  NL_FORCE_INLINE void leave() { CAtomicLockFast::leave(m_Flag); }
};

...
/** This class allow simple mapping between string and other type (such as
 *integral types or enum) In fact this primarily intended to make a string /
 *enum correspondance Example of use :
 *
 * // An enumerated type
 ...
 *	static const CStringConversion conversion(stringTable,
 *sizeof(stringTable) / sizeof(stringTable[0]),  Unknown);
 *
 * // Some conversions request
 * TMyType value1 = conversion.fromString("foo");  // returns 'foo'
 * value1 = conversion.fromString("Foo");          // returns 'foo' (this is
 *case unsensitive by default) std::string str = conversion.toString(Bar)  //
 *returns "Bar"
 *
 * NB : Please note that that helpers macros are provided to build such a table
 *in an easy way \see NL_BEGIN_STRING_CONVERSION_TABLE \see
 *NL_END_STRING_CONVERSION_TABLE
 *
 *
 * NB: by default this class behaves in a case unsensitive way. To change this,
 *just change the 'Pred' template parameter to std::less<std::string>
 *
...
public:
  ...
  CStringConversion(const CPair *pairs, uint numPairs,
                    const DestType &notFoundValue);

  // add a pair in the array
  void insert(const char *str, TDestType value);

  // From a string, retrieve the associated value, or the 'notFoundValue'
  // provided to the ctor of this object if the string wasn't found
  const TDestType &fromString(const std::string &str) const;

  // From a value, retrieve the associated string, or an empty string if not
  // found
  ...
  // Check a value against the list a value, return true if the value exist in
  // the container
...
CStringConversion<DestType, Pred>::CStringConversion(
    const CPair *pairs, uint numPairs, const DestType &notFoundValue) {
...
//
template <class DestType, class Pred>
void CStringConversion<DestType, Pred>::insert(const char *str,
                                               TDestType value) {
...
template <class DestType, class Pred>
const DestType &
CStringConversion<DestType, Pred>::fromString(const std::string &str) const {
...
template <class DestType, class Pred>
const std::string &
CStringConversion<DestType, Pred>::toString(const DestType &value) const {

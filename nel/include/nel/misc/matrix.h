 ...
 * This is a column matrix, so operations like: \c v1=A*B*C*v0; applies C first
 * , then B, then A to vector v0. \n Since it is a column matrix, the first
 * column is the I vector of the base, 2nd is J, 3th is K. \n 4th column vector
 * is T, the translation vector.
 *
 * Angle orientation are: Xaxis: YtoZ. Yaxis: ZtoX. Zaxis: XtoY.
 *
 * This matrix keep a matrix state to improve Matrix, vector and plane computing
 * (matrix inversion, vector multiplication...). The internal matrix know if:
 * - matrix is identity
 * - matrix has a translation component
 * - matrix has a rotation component
 * - matrix has a uniform scale component (scale which is the same along the 3
 * axis)
 * - matrix has a non-uniform scale component
 * - matrix has a projection component (4th line of the matrix is not 0 0 0 1).
 *
 * An example of improvement is that CMatrix::operator*(const CVector &v) return
 * v if the matrix is identity.
 *
 * By default, a matrix is identity. But for a performance view, this is just a
 * StateBit=0... \author Lionel Berenguier \author Nevrax France \date 2000
 */
...
public:
   ...
   * Avoid it. It implies low compute since no check is done on base to see what
   * type of matrix it is (identity, rotation, scale, uniform scale...) \param i
   * The I vector of the Cartesian base. \param j The J vector of the Cartesian
   * base. \param k The K vector of the Cartesian base. \param hintNoScale set
   * it to true if you are sure that your rot matrix is a pure rot matrix with
   * no scale. If set to true and your rotation is not an orthonormal basis,
   * unpredictable result are excepted.
   */
  void setRot(const CVector &i, const CVector &j, const CVector &k,
              bool hintNoScale = false);
  /** Explicit setup the Rotation/Scale matrix (base).
   * Avoid it. It implies low compute since no check is done on m33 to see what
   * type of matrix it is (identity, rotation, scale, uniform scale) \param m33
   * the 3*3 column rotation matrix. (3x3 matrix stored in column-major order as
   * 9 consecutive values) \param hintNoScale set it to true if you are sure
   * that your rot matrix is a pure rot matrix with no scale. If set to true and
   * your rotation is not an orthonormal basis, unpredictable result are
   * excepted.
   */
  void setRot(const float m33[9], bool hintNoScale = false);
  /** Explicit setup the Rotation matrix (base) as a Euler rotation matrix.
   * \param v a vector of 3 angle (in radian), giving rotation around each axis
   * (x,y,z) \param ro the order of transformation applied. if ro==XYZ, then the
   * transform is M=M*Rx*Ry*Rz
   */
  ...
  /** Explicit setup the Rotation/Scale matrix (base) with the rotation part of
   * another matrix. \param matrix the matrix to copy rot part.
   */
  void setRot(const CMatrix &matrix);
  /** Explicit setup the Rotation/Scale matrix (base) with a scale (=> matrix
   *has no Rotation). 1 is tested to update bits accordingly \param scale the
   *scale to set
   */
  void setScale(float scale);
  /** Explicit setup the Rotation/Scale matrix (base) with a scale (=> matrix
   *has no Rotation). case where v.x==v.y==v.z is tested to set a uniform scale
   ...
   * \param proj the 4th line of the matrix. Set it to 0 0 0 1 to reset it to
   * default.
   */
   ...
   * Avoid it. It implies low compute since no check is done on rotation matrix
   * to see what type of matrix it is (identity, rotation, scale, uniform
   * scale). BUT check are made to see if it has translation or projection
   * components. \param m44 the 4*4 column matrix (4x4 matrix stored in
   * column-major order as 16 consecutive values)
   */
   ...
   *	\param j : line index.
   */
  void setCoefficient(float coeff, sint i, sint j) { M[(j << 2) + i] = coeff; }
  //@}
  /** Choose an arbitrary rotation matrix for the given direction. The matrix
   *will have I==idir \param idir: vector direction. MUST  be normalized.
   */

  void setArbitraryRotI(const CVector &idir);
  /** Choose an arbitrary rotation matrix for the given direction. The matrix
   *will have J==jdir \param jdir: vector direction. MUST  be normalized.
   */
  void setArbitraryRotJ(const CVector &jdir);
  /** Choose an arbitrary rotation matrix for the given direction. The matrix
   *will have K==kdir \param kdir: vector direction. MUST  be normalized.
   */
  ...
  /** Get the Rotation/Scale matrix (base).
   * \param m33 the matrix's 3*3 column rotation matrix. (3x3 matrix stored in
   * column-major order as 9 consecutive values)
   */
  ...
  CVector getK() const;
  /** Get 4*4 matrix.
   * \param m44 the matrix's 4*4 column matrix (4x4 matrix stored in
   * column-major order as 16 consecutive values)
   */
  void get(float m44[16]) const;
  /** Get 4*4 matrix.
   * \return the matrix's 4*4 column matrix (4x4 matrix stored in column-major
  ...
  //@{
  /** Matrix multiplication. Because of copy avoidance, this is the fastest
   *method Equivalent to *this= m1 * m2 \warning *this MUST NOT be the same as
   *m1 or m2, else it doesn't work (not checked/nlasserted)
   */
  ...
  /** Matrix multiplication assuming no projection at all in m1/m2 and Hence
   *this. Even Faster than setMulMatrix() Equivalent to *this= m1 * m2 NB: Also
   *always suppose m1 has a translation, for optimization consideration \warning
   **this MUST NOT be the same as m1 or m2, else it doesn't work (not
   *checked/nlasserted)
   */
  ...
  /** Normalize the matrix so that the rotation part is now an orthonormal
   * basis, ie a rotation with no scale. NB: projection part and translation
   * part are not modified. \param pref the preference axis order to normalize.
   * ZYX means that K direction will be kept, and the plane JK will be used to
   * lead the I vector. \return false if One of the vector basis is null. true
   * otherwise.
   */
  ...
  /// \ Vector Operations.
  //@{
  /// Multiply a normal. ie v.w=0 so the Translation component doesn't affect
  /// result. Projection doesn't affect result.
  CVector mulVector(const CVector &v) const;
  /// Multiply a point. ie v.w=1 so the Translation component do affect result.
  /// Projection doesn't affect result.
  CVector mulPoint(const CVector &v) const;
  /** Multiply a point. \sa mulPoint
   */
  CVector operator*(const CVector &v) const { return mulPoint(v); }
  ...
  /// return true if the matrix has a scale part (by scale(), by multiplication
  /// etc...)
  bool hasScalePart() const;
  /// return true if hasScalePart() and if if this scale is uniform.
  bool hasScaleUniform() const;
  /// return true the uniform scale. valid only if hasScaleUniform() is true,
  /// else 1 is returned.
  float getScaleUniform() const;
  /// return true if the matrix has a projection part (by setProj(), by
  /// multiplication etc...)
...
private:
  ...
  // access to M, in math conventions (mat(1,1) ... mat(4,4)). Indices from 0
  // to 3.
  float &mat(sint i, sint j) { return M[(j << 2) + i]; }
  // access to M, in math conventions (mat(1,1) ... mat(4,4)). Indices from 0
  // to 3.
  const float &mat(sint i, sint j) const { return M[(j << 2) + i]; }
  ...
  // Else they are not initialized but are supposed to represent identity and
  // Scale33==1.
  bool hasRot() const;
  // true if MAT_PROJ.
  // proj part is true means the bottom 1x4 projection part matrix is relevant.
  // Else it is not initialized but is supposed to represent the line vector
  // (0,0,0,1).
  bool hasProj() const;
  bool hasAll() const;

...
struct EFile : public EStream {
  ...
  EFile(const std::string &filename)
      : EStream("Unknown file error in '" + filename + "'"),
        Filename(filename) {}
  EFile(const std::string &filename, const std::string &text, bool)
      : EStream(text), Filename(filename) {}
...
struct EFileNotOpened : public EFile {
  EFileNotOpened(const std::string &filename)
      : EFile(filename, "File '" + filename + "' not opened", true) {}
};

struct EReadError : public EFile {
  EReadError(const std::string &filename)
      : EFile(filename, "Read error in file '" + filename + "' (End of file?)",
              true) {}
};

struct EWriteError : public EFile {
  EWriteError(const std::string &filename)
      : EFile(filename, "Write Error in file '" + filename + "'", true) {}
};
...
struct ERenameError : public EFile {
  ERenameError(const std::string &dest, const std::string &src)
      : EFile(dest,
              "Rename Error from the file '" + src + "' to the file '" + dest +
                  "'",
              true) {}
};
...
public: // Basic Usage.
  ...
  /// Set the cache file on open option (default behaviour is false (file is not
  /// cached at opening)
  void setCacheFileOnOpen(bool newState);

  /** If the file is opened with a big file, CacheFileOnOpen is replaced with
   *big file option. Except if allowBNPCacheFileOnOpen(false) is called. true is
   *default.
   */
...
public: // Advanced Usage.
  ...
  /// Get the number of file open from the beginning of the application. Files
  /// can be in a big file.
  ...
  /// Get the number of byte read from the file system since the application
  /// start.
...
public: // Basic Usage.
   ...
   *	If you open the file with the flag useTempFile, you MUST close
   *explicitly the file with close() if the writing as been successed.
   */

...
struct EReadError : public EFile
{
	EReadError(const std::string &filename)
	    : EFile(filename, "Read error in file '" + filename + "' (End of file?)", true)
	{
	}
};
...
{
	EDiskFullError(const std::string &filename)
	    : EWriteError(filename)
	{
	}
};
...
struct ERenameError : public EFile
{
	ERenameError(const std::string &dest, const std::string &src)
	    : EFile(dest, "Rename Error from the file '" + src + "' to the file '" + dest + "'", true)
	{
	}
};

...
inline CVector &CVector::operator/=(float f) { return *this *= (1.0f / f); }
...
inline CVector CVector::operator/(float f) const { return *this * (1.0f / f); }
...
inline float CVector::sqrnorm() const { return (float)(x * x + y * y + z * z); }
...
inline bool CVector::isNull() const { return *this == CVector::Null; }
inline bool CVector::operator<(const CVector &v) const {
  if (x != v.x)
    return x < v.x;
  if (y != v.y)
...
inline void CVector::cartesianToSpheric(float &r, float &theta,
                                        float &phi) const {
  CVector v;
...
inline void CVector::serial(IStream &f) { f.serial(x, y, z); }

...
public:
   ...
   */
  void init(xmlNodePtr node, class IProgressCallback &progressCallBack,
            bool mapBanks = false, CCDBBankHandler *bankHandler = NULL);
  ...
  virtual bool getNodeIndex(ICDBNode *node, uint &index) {
    index = 0;
    for (std::vector<ICDBNode *>::const_iterator it = _Nodes.begin();
         it != _Nodes.end(); it++) {
      if (*it == node)
        return true;
      index++;
  ...
  CCDBNodeLeaf *getLeaf(const std::string &id, bool bCreate) {
    return getLeaf(id.c_str(), bCreate);
  }
  ...
  virtual CCDBNodeBranch *getParent() { return _Parent; }

  // get the number of nodes
  uint16 getNbNodes() { return (uint16)_Nodes.size(); }
  ...
  // Add an observer to this branch. It will be notified of any change in the
  // sub-leaves

  /**
   * Add observer to all sub-leaves, except if a positive filter is set:
   * If positiveLeafNameFilter is non-empty, only changes to leaves having names
   * found in it will be notified (this is equivalent to creating a sub-branch
   * containing only the specified leaves and setting a branch observer on it,
   * except you don't need to change your database paths and update large
   * amounts of code!).
   */
  void
  addBranchObserver(ICDBDBBranchObserverHandle *handle,
   ...
   */
  void addBranchObserver(ICDBDBBranchObserverHandle *handle,
                         const char *dbPathFromThisNode,
                         const char **positiveLeafNameFilter = NULL,
                         uint positiveLeafNameFilterSize = 0);

  // Remove observer from all sub-leaves
  bool removeBranchObserver(IPropertyObserver *observer);

  /// Easy version of removeBranchObserver() (see above and see easy version of
  /// addBranchObserver())
  void removeBranchObserver(const char *dbPathFromThisNode,
                            ICDBNode::IPropertyObserver &observer);

  virtual bool isLeaf() const { return false; }

  // mark this branch and parent branch as 'modified'. This is usually called by
  // sub-leaves

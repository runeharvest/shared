 ...
 *\code
    ...
    CI18N::load ("fr");
    // display "Salut" that is the "hi" string in the selected language
 (French). nlinfo (CI18N::get("Hi").c_str ());
...
public:
  ...
  enum TLineFormat {
    // the text file is just loaded, no conversion or checks done on line
    // delimiters
    LINE_FMT_NO_CARE,
  ...
  struct ILoadProxy {
    virtual ~ILoadProxy() {}
    virtual void loadStringFile(const std::string &filename,
                                ucstring &text) = 0;
  };
  ...
  /// Return a vector with all language available. The vector contains the name
  /// of the language. The index in the vector is used in \c load() function
   ...
   *	As in getLanguageNames(), the index in the vector can be used to call
   *load()
   */
  ...
  /// Load a language file depending of the language code("en", "fr", ...). Code
  /// are ISO 639-2 compliant.
  static void load(const std::string &languageCode,
                   const std::string &fallbackLanguageCode = "");

  /** Load a language file from its filename
   * \param filename name of the language file to load, with its extension
   * \param reload The file is being reloaded so error message won't be issued
   * for strings that are overwritten
   */
   ...
   */
  static void readTextFile(const std::string &filename, ucstring &result,
                           bool fileLookup = true, bool preprocess = false,
  ...
  static ucstring makeMarkedString(ucchar openMark, ucchar closeMark,
                                   const ucstring &text);

  //@{
  //\name Parsing utility
  /** Skip the white space.
   *	You can optionally pass a ucstring pointer to receive any comments
   *string that build the white space. This is useful if you want to keep the
   *comments. NB : comments are appended to the comments string.
   */
  static void skipWhiteSpace(ucstring::const_iterator &it,
                             ucstring::const_iterator &last,
                             ucstring *storeComments = NULL,
                             bool newLineAsWhiteSpace = true);
  /// Parse a label
  static bool parseLabel(ucstring::const_iterator &it,
                         ucstring::const_iterator &last, std::string &label);
  /// Parse a marked string. NB : usually, we use [ and ] as string delimiters
  /// in translation files.
  static bool parseMarkedString(ucchar openMark, ucchar closeMark,
                                ucstring::const_iterator &it,
                                ucstring::const_iterator &last,
                                ucstring &result, uint32 *lineCounter = NULL,
   ...
   *	If found, the function return true and 'it' is advanced after the
   *matched token, Otherwise, the function return false and 'it' is unchanged.
   */
  static bool matchToken(const char *token, ucstring::const_iterator &it,
                         ucstring::const_iterator end);
  /// Advance iterator to the start of next line or to the end of string
  static void skipLine(ucstring::const_iterator &it,
                       ucstring::const_iterator end, uint32 &lineCounter);
  //@}
...
private:
  ...
  // the alternative language that will be used if the sentence is not found in
...
private:
  ...
  static bool loadFileIntoMap(const std::string &filename,
                              StrMapContainer &dest, StrMapContainer16 &dest16);

  /// The internal read function, it does the real job of readTextFile
  static void _readTextFile(const std::string &filename, ucstring &result,
                            bool fileLookup, bool preprocess,
                            TLineFormat lineFmt, bool warnIfIncludesNotFound,

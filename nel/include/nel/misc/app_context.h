...
public:
  ...
  virtual void releaseSingletonPointer(const std::string &singletonName,
                                       void *ptr) = 0;
  //@}
    ...
    if (_Instance == NULL) {                                                   \
      ...
      nlassertex(NLMISC::INelContext::isContextInitialised(),                  \
                 ("You are trying to access a safe singleton without having "  \
                  "initialized a NeL context. The simplest correction is to "  \
                  "add 'NLMISC::CApplicationContext myApplicationContext;' "   \
                  "at the very beginning of your application."));              \
      void *ptr =                                                              \
          NLMISC::INelContext::getInstance().getSingletonPointer(#className);  \
      if (ptr == NULL) {                                                       \
        /* allocate the singleton and register it */                           \
        _Instance = new className;                                             \
        NLMISC::INelContext::getInstance().setSingletonPointer(#className,     \
                                                               _Instance);     \
      } else {                                                                 \
...
public:                                                                        \
  static void releaseInstance() {                                              \
    if (_Instance) {                                                           \
      NLMISC::INelContext::getInstance().releaseSingletonPointer(#className,   \
                                                                 _Instance);   \
      delete _Instance;                                                        \
      _Instance = NULL;                                                        \
    }                                                                          \
 ...
 */
#define NLMISC_SAFE_SINGLETON_IMPL(className)                                  \
...
#define NLMISC_LIBRARY_ENTRY                                                   \
  void libraryEntryImp(NLMISC::INelContext &applicationContext) {              \
    nlassert(!NLMISC::INelContext::isContextInitialised() ||                   \
             &applicationContext == &(NLMISC::INelContext::getInstance()));    \
                                                                               \

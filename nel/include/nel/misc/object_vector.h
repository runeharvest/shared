...
// ***************************************************************************
/**
 * The purpose of this class is to copy most (but not all) of stl vector<>
 *features, without some of the speed/memory problems involved:
 *		- size of a vector<T> is 16 bytes typically. size of a
 *CObjectVector is 8 bytes (only a ptr and a size).
 *		- CObjectVector<T>::size() is faster than vector::size()
 *		- CObjectVector<T>::resize() is faster because it do not copy
 *from a default value, it just call the default constructor of the objects.
 ...
 *
 *	Of course some features are not implemented (for benefit of
 *speed/memory):
 *		- no reserve() scheme
 *
 *	Also, for now, not all vector<> features are implemented (iterator,
 *erase etc...).
 *
...
public:
   ...
   */
  const T &operator[](uint index) const { return _Ptr[index]; }
  /** Element accessor. no check is made on index. (No exception, no nlassert())
   */
  T &operator[](uint index) { return _Ptr[index]; }
  ...
  /** copy elements from an array ptr to this vector, beetween dstFirst element
   *(included) and dstLast element (not included). nlassert if last is too big.
   *copy(y, x, ...) where y>=x is valid, and nothing is copied.
   */
  ...
  /** fill elements with a value, beetween dstFirst element (included) and
   * dstLast element (not included).
   */
   ...
   *	NB: actually, the serial of a vector<> and the serial of a CObjectVector
   *is the same in the stream.
   */

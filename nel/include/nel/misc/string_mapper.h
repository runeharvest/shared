...
// const string *  as  uint (the TStringId returned by CStringMapper is a
// pointer to a string object) #ifdef HAVE_X86_64 typedef uint64 TStringId;
// #else
...
struct CStringIdHashMapTraits {
  ...
  bool operator()(const NLMISC::TStringId &strId1,
                  const NLMISC::TStringId &strId2) const {
...
class CStringMapper {
  ...
  public:
    CAutoFastMutex(CFastMutex *mtx) : m_Mutex(mtx) { m_Mutex->enter(); }
  ...
  CFastMutex m_Mutex; // Must be thread-safe (Called by CPortal/CCluster, each
                      // of them called by CInstanceGroup)
...
public:
  ...
  static TStringId map(const std::string &str) {
    return s_GlobalMapper.localMap(str);
  }
  /// Globaly map a string into a unique Id. ** This method IS Thread-Safe **
  ...
  static const std::string &unmap(const TStringId &stringId) {
    return s_GlobalMapper.localUnmap(stringId);
  }
  /// Globaly helper to serial a string id. ** This method IS Thread-Safe **
  static void serialString(NLMISC::IStream &f, TStringId &id) {
    s_GlobalMapper.localSerialString(f, id);
  }
  /// Return the global id for the empty string (helper function). NB: Works
  /// with every instance of CStringMapper
  ...
  */
  ...
  const std::string &localUnmap(const TStringId &stringId) const {
    return (stringId == NULL) ? *m_EmptyId : *((const std::string *)stringId);
  }
...
// linear from 0 (0 is empty string) (The TSStringId returned by
// CStaticStringMapper is an index in the vector and begin at 0)

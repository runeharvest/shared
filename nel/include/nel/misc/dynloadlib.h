...
#else
// other systems don't bother with this kind of detail, they export any 'extern'
// symbol (almost all functions)
...
void *nlGetSymbolAddress(NL_LIB_HANDLE libHandle,
...
// Utility macro to export a module entry point as a C pointer to a C++ class or
// function
#define NL_LIB_EXPORT_SYMBOL(symbolName, classOrFunctionName, instancePointer) \
  extern "C" {                                                                 \
  NL_LIB_EXPORT classOrFunctionName *symbolName =                              \
      (classOrFunctionName *)instancePointer;                                  \
  };
...
public:
  ...
  CLibrary(const std::string &libName, bool addNelDecoration, bool tryLibPath,
           bool ownership = true);
   ...
   *	If addNelDecoration is true, the standard Nel suffix and library
   *extention or prefix are added to the lib name (with is just a base name). If
   *tryLibPath is true, then the method will try to find the required library in
   *the added library files (in order of addition). Return true if the library
   *load ok.
   */
  bool loadLibrary(const std::string &libName, bool addNelDecoration,
                   bool tryLibPath, bool ownership = true);
  ...
  std::string getLibFileName() { return _LibFileName; }
  ...
  /** Build a NeL standard library name according to platform and compilation
   *mode setting. aka : adding decoration one base lib name. e.g : 'mylib'
   *become	'mylib_rd.dll' on Windows ReleaseDebug mode, 'libmylib.dylib'
   *under OS X or 'libmylib.so' on unix system.
   */
  static std::string makeLibName(const std::string &baseName);
  /** Remove NeL standard library name decoration according to platform and
   *compilation mode setting. e.g : 'mylib_rd.dll' on windows ReleaseDebug mode
   *become 'mylib'
   */
...
class INelLibrary {
  ...
  /// this counter keep tracks of how many time the library have been loaded in
  /// the same process.
...
public:
  INelLibrary() : _LoadingCounter(0), _LibContext(NULL) {}
...
#define NLMISC_DECL_PURE_LIB(className)                                        \
  ...
  NL_LIB_EXPORT_SYMBOL(NLMISC_PURE_LIB_ENTRY_POINT, NLMISC::INelLibrary,       \

 ...
 *	This heap manager is not designed for speed (because it stills use
 *standard heap allocation), but for use with special memory or special cases
 *where malloc/new cannot be used. \author Lionel Berenguier \author Nevrax
 *France \date 2001
 */
class CHeapMemory {
public:
  ...
  void reset();
  /** init the heap. this reset() the heap. heap ptr is stored in this class,
   *but no CHeapMemory methods write or read into. They use standard heap
   *instead (new / delete). \param heap the heap ptr. heap should be at least
   *N-bytes aligned, where N is the alignment you need (see param align, 4 by
   *default). \param align Any size given to allocate() will be rounded to match
   *this alignement. Valid values are 4,8,16, or 32. 4 by default.
   */
  ...
  /** allocate a block of size bytes. return NULL if not enough space or if
   *size==0. NB: for alignements consideration, allocation are aligned to 4
   *bytes.
   */
  void *allocate(uint size);
  /// free a block allocated with alloate(). no-op if NULL. nlstop() if don't
  /// find this block.

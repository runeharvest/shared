 ...
 * Add features to convert and assign \c ucstring to \c string and \c string to
 * \c ucstring.
 */
...
public:
  ...
  ucstring(const ucchar *begin, const ucchar *end) : ucstringbase(begin, end) {}

  ucstring(const std::string &str) : ucstringbase() { fromUtf8(str); }
...
struct CUCStringHashMapTraits {
  ...
  size_t operator()(const ucstring &id) const { return id.size(); }

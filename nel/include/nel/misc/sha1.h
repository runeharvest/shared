...
struct CHashKey {
  ...
  void serial(NLMISC::IStream &stream) { stream.serial(HashKeyString); }

  bool operator==(const CHashKey &v) const {
    return HashKeyString == v.HashKeyString;
  }

  bool operator!=(const CHashKey &v) const { return !(*this == v); }

  // this string is always 20 bytes long and is the code in binary format (can't
  // print it directly)
...
// This function get a filename (it works with big files) and returns his SHA
// hash key when forcePath is true, it doesn't use NLMISC::lookup
...
// This function get a buffer and key with size and returns his HMAC-SHA1 hash
// key
CHashKey getHMacSHA1(const uint8 *text, uint32 text_len, const uint8 *key,
                     uint32 key_len);

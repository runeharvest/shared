...
public:
   ...
   */
  uint16 get565() const {
    return ((uint16)(R & 0xf8) << 8) | ((uint16)(G & 0xfc) << 3) |
           (uint16)(B >> 3);
  }
   ...
   * The average is "correct": +1 is added to the four color, to make a "round"
   * like average.
   */
  ...
  /// \name RGBOnly methods. Same f() as their homonym, but don't modify A
  /// component.
  // @{

  /// see blendFromui()
  void blendFromuiRGBOnly(CRGBA c0, CRGBA c1,
                          uint coef) // coef must be in [0,256]
  {
    uint a1 = coef;
    uint a2 = 256 - a1;
    R = uint8((c0.R * a2 + c1.R * a1) >> 8);
  ...
  //@{
  /** Add a group of colors with saturation, using mmx instructions when
   * present. \param dest The destination color buffer, encoded as CRGBA's.
   ...
   * \param Dup the number of time the result must be duplicated in the
   * destination.
   */
  static void addColors(CRGBA *dest, const CRGBA *src1, const CRGBA *src2,
                        uint numColors, uint srcStride = sizeof(CRGBA),
                        uint destStride = sizeof(CRGBA), uint dup = 1);

  /** Modulate a group of colors with saturation, using mmx instructions when
   * present. \param dest The destination color buffer, encoded as CRGBA's.
   * \param src1 The first source color buffer, encoded as CRGBA's.
   * \param src2 The second source color buffer, encoded as CRGBA's.
   * \param numColors The number of colors to compute
   * \param Stride between each color.  It is the same for sources and
   * destination.
   */
  static void modulateColors(CRGBA *dest, const CRGBA *src1, const CRGBA *src2,
                             uint numColors, uint srcStride = sizeof(CRGBA),
                             uint destStride = sizeof(CRGBA), uint dup = 1);

  /** Subtract a group of colors with saturation (src1 - src2), using mmx
   * instructions when present. \param dest The destination color buffer,
   * encoded as CRGBA's. \param src1 The first source color buffer, encoded as
   * CRGBA's. \param src2 The second source color buffer, encoded as CRGBA's.
   * \param numColors The number of colors to compute
   * \param Stride between each color.  It is the same for sources and
   * destination.
   */
  static void subtractColors(CRGBA *dest, const CRGBA *src1, const CRGBA *src2,
                             uint numColors, uint srcStride = sizeof(CRGBA),
                             uint destStride = sizeof(CRGBA), uint dup = 1);
  //@}
   ...
   * There's no range for hue, (all hues colors range from 0 to 360, from 360 to
   * 720 and so on) \return true if the color is achromatic
   */
...
public:
   ...
   * \param a Alpha componant.
   */
  CBGRA(uint8 r, uint8 g, uint8 b, uint8 a = 255) : B(b), G(g), R(r), A(a) {}

  /**
   * Cast operator to CRGBA.
   */
  operator CRGBA() { return CRGBA(R, G, B, A); }
   ...
   * Comparison operator.
   */
  bool operator<(const CBGRA &c) const { return getPacked() < c.getPacked(); }
...
template <class U> inline CRGBA blend(CRGBA c0, CRGBA c1, U blendFactor) {
  CRGBA result;
...
public:
   ...
   * \return Return the result of the opertor
   */
  CRGBAF operator*(float f) const { return CRGBAF(R * f, G * f, B * f, A * f); }

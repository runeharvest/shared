...
/** Force v to be inside the interval [min,max]. Warning: implicit cast are made
 * if T,U or V are different.
 */
...
/** \c contReset take a container like std::vector or std::deque and put his
 * size to 0 like \c clear() but free all buffers. This function is useful
 * because \c resize(), \c clear(), \c erase() or \c reserve() methods never
 * realloc when the array size come down. \param a is the container to reset.
 */
 ...
 */
inline float degToRad(float deg) { return deg * (float)Pi / 180.0f; }

/** Converts from radians to degrees
 */
inline float radToDeg(float rad) { return rad * 180.0f / (float)Pi; }
...
int compareCaseInsensitive(const char *a, size_t lenA, const char *b,
                           size_t lenB);
inline int compareCaseInsensitive(const std::string &a, const std::string &b) {
  return compareCaseInsensitive(&a[0], a.size(), &b[0], b.size());
}
inline bool ltCaseInsensitive(const std::string &a, const std::string &b) {
  return compareCaseInsensitive(&a[0], a.size(), &b[0], b.size()) < 0;
}
std::string
toCaseInsensitive(const char *str); // UTF-8, case-insensitive toLower
std::string
toCaseInsensitive(const std::string &str); // UTF-8, case-insensitive toLower

/** ASCII to lowercase. Useful for internal identifiers.
 * Characters outside of the 7-bit ASCII space, and control characters, are
 * replaced.
 */
...
// Remove all the characters <= 32 (tab, space, new line, return, vertical tab
// etc..) at the beginning and at the end of a string
...
//////////////////////////////////////////////////////////////////////////
// ****  DEPRECATED *****: PLEASE DON'T USE THESE METHODS BUT FUNCTIONS ABOVE
// toLower() and toUpper()
//////////////////////////////////////////////////////////////////////////
...
inline int strnicmp(const char *lhs, const char *rhs, size_t n) {
...
inline sint nlstricmp(const std::string &lhs, const std::string &rhs) {
  return stricmp(lhs.c_str(), rhs.c_str());
}
inline sint nlstricmp(const std::string &lhs, const char *rhs) {
  return stricmp(lhs.c_str(), rhs);
}
inline sint nlstricmp(const char *lhs, const std::string &rhs) {
  return stricmp(lhs, rhs.c_str());
}
...
/// Returns a readable string from a vector of bytes. unprintable char are
/// replaced by '?'
std::string stringFromVector(const std::vector<uint8> &v, bool limited = true);

/// Convert a string into an sint64 (same as atoi() function but for 64 bits
/// intergers)
sint64 atoiInt64(const char *ident, sint64 base = 10);

/// Convert an sint64 into a string (same as itoa() function but for 64 bits
/// intergers)
...
/// Convert a time into a string that is easily readable by an human, for
/// example 3600 -> "1h"
...
/// Add digit grouping seperator to if value >= 10 000. Assumes input is
/// numerical string.
std::string formatThousands(const std::string &s);

/// This function executes a program in the background and returns instantly
/// (used for example to launch services in AES). The program will be launched
...
/// This function executes a program and wait for result (used for example for
/// crash report). The program will be launched in the current directory
...
/// This function replace all environment variables in a string by their
/// content. Environment variables names can use both Windows (%NAME%) and UNIX
/// syntax ($NAME) Authorized characters in names are A-Z, a-z, 0-9 and _
std::string expandEnvironmentVariables(const std::string &s);

/// Functions to convert a string with arguments to array or array to string
/// (will espace strings with spaces)
...
/// This function kills a program using his pid (on unix, it uses the kill()
/// POSIX function)
bool killProgram(uint32 pid);

/// This function kills a program using his pid with abort signal (on unix, it
/// uses the kill() POSIX function)
...
/** Explode a string (or ucstring) into a vector of string with *sep* as
 * separator. If sep can be more than 1 char, in this case, we find the entire
 * sep to separator (it s not a set of possible separator)
 *
 * \param skipEmpty if true, we don't put in the res vector empty string
 */
template <class T>
void explode(const T &src, const T &sep, std::vector<T> &res,
...
/** Join a string (or ucstring) from a vector of strings with *sep* as
 * separator. If sep can be more than 1 char, in this case, we find the entire
 * sep to separator (it s not a set of possible separator)
 */
 ...
 * (used by the toString() function). So we can use stringstream operator << and
 * >> with all NeL simple types (except for ucchar and ucstring)
 */
/*
#ifdef NL_OS_WINDOWS

#if _MSC_VER < 1300	// visual or older (on visual .NET, we don't need to do
that)
...
operator<<(std::basic_ostream<_CharT, _Traits>& __os, const uint64& __z)
{
    ...
    {
        std::basic_ostringstream<_CharT, _Traits, std::allocator<_CharT> >
__tmp; uint64	__z2 = __z; while (__z2 != 0)
        {
...
template <class T> inline T iavoid0(T x) {
  if (x == 0)
    return 1;
  return x;
}

 ...
 *	without copying the data buffer (thus making a CMemStrem copy almost
 *free). This class reference a TMemStreamBuffer object with a smart pointer,
 *	when some code call the getBufferWrite() method to obtain write access
 *	to the memory buffer, if the ref count is more than 1, then we make a
 *copy of the internal buffer for the current object.
 *
...
public:
  ...
  const TBuffer &getBuffer() const { return _SharedBuffer->_Buffer; }
 ...
 * The buffer size is increased by factor 2. It means the stream can be smaller
 * than the buffer size. The size of the stream is the current position in the
 * stream (given by lengthS() which is equal to getPos()), because data is
 * always written at the end (except when using poke()). About seek()
 * particularities: see comment of the seek() method.
 *
 * buffer() ----------------------------------- getPos() ---------------- size()
 ...
 * The stream is exactly the buffer (the size is given by lengthR()). Data is
 * read inside the stream, at the current position (given by getPos()). If you
 * try to read data while getPos() is equal to lengthR(), you'll get an
 * EStreamOverflow exception.
 *
 * buffer() ----------------------------------- getPos()
 * ------------------------- size() data already serialized in data not read yet
 * | length() = lengthR()
 *
...
public:
  ...
  CMemStream(bool inputStream = false, bool stringmode = false,
             uint32 defaultcapacity = 0)
      : NLMISC::IStream(inputStream), _StringMode(stringmode) {
    _DefaultCapacity = std::max((uint32)defaultcapacity,
                                (uint32)16); // prevent from no allocation
    _Buffer.getBufferWrite().resize(_DefaultCapacity);
    _Buffer.Pos = 0;
  }

  /// Copy constructor
  CMemStream(const CMemStream &other) : IStream(other) { operator=(other); }
  ...
  /** Returns a readable string to display it to the screen. It's only for
   * debugging purpose! Don't use it for anything else than to debugging, the
   * string format could change in the future. \param hexFormat If true, display
   * all bytes in hexadecimal, else display as chars (above 31, otherwise '.')
   */
   ...
   *
   * Warning: in output mode, seek(end) does not point to the end of the
   * serialized data, but on the end of the whole allocated buffer (see size()).
   * If you seek back and want to return to the end of the serialized data, you
   * have to store the position (a better way is to use reserve()/poke()).
   *
   * NB: If the stream doesn't support the seek fonctionnality, it throws
   ...
   */
  virtual sint32 getPos() const { return sint32(_Buffer.Pos); }

  /**
   * When writing, skip 'len' bytes and return the position of the blank space
   * for a future poke(). Warning: this has nothing to do with the semantics of
   * reserve() in std::vector!
   */
  ...
  /// Returns the size of the buffer (can be greater than the length, especially
  /// in output mode)
  uint32 size() const { return _Buffer.getBuffer().size(); }

  /** Returns a pointer to the message buffer (read only)
   * Returns NULL if the buffer is empty
   */
  virtual const uint8 *buffer() const { return _Buffer.getBuffer().getPtr(); }

  /**
   * When you fill the buffer externaly (using bufferAsVector) you have to reset
   * the BufPos calling this method
   *
   * If you are using the stream only in output mode, you can use this method as
   * a faster version of clear() *if you don't serialize pointers*.
   */
  ...
  /**
   * Resize the stream with the specified size, set the current position at the
   * beginning of the stream and return a pointer to the stream buffer.
   *
   * Precondition: the stream is an input stream.
   *
   * Suggested usage: construct an input stream, resize and get the buffer using
   * bufferToFillAndRead(), fill it with raw data using any filling function
   * (warning: don't fill more than 'msgsize' bytes!), then you are ready to
   * read, using serial(), the data you've just filled.
   */
   ...
   *
   * Precondition:
   * - If the stream is in input mode, it must not be empty (nothing filled),
   otherwise the position
   *   will be set to the end of the preallocated buffer (see DefaultCapacity).
   * Postcondition:
   * - Read->write, the position is set at the end of the stream, it is possible
   to add more data
   - - Write->Read, the position is set at the beginning of the stream
   */
  virtual void invert() {
    if (isReading()) {
      // In->Out: We want to write (serialize out) what we have read (serialized
      // in)
      ...
      // Out->In: We want to read (serialize in) what we have written
      // (serialized out)
      resetPtrTable();
      setInOut(true);
...
#endif
      void
      increaseBufferIfNecessary(uint32 len) {
  ...
  template <class T> void fastSerial(T &val) {
    ...
    if (isReading()) {
      ...
      if (lengthS() + sizeof(T) >
          length()) // calls virtual length (cf. sub messages)
  ...
  template <class T> void fastWrite(const T &value) {
    // nldebug( "MEMSTREAM: Writing %u-byte value in %p at pos %u",
    // sizeof(value), this, _BufPos - _Buffer.getPtr() );
    increaseBufferIfNecessary(sizeof(T));
  ...
  template <class T> void fastRead(T &value) {
    // nldebug( "MEMSTREAM: Reading %u-byte value in %p at pos %u",
    // sizeof(value), this, _BufPos - _Buffer.getPtr() );
    //  Check that we don't read more than there is to read
    if (lengthS() + sizeof(value) >
        length()) // calls virtual length (cf. sub messages)
    {
      throw EStreamOverflow();
    }
  ...
  /// Input: read len bytes at most from the stream until the next separator,
  /// and return the number of bytes read. The separator is then skipped.
...
protected:
   ...
   * Do not use directly, use length() instead in reading mode (which is
   * virtual)
   */
  uint32 lengthR() const { return size(); }
...
#define readnumber(dest, thetype, digits, convfunc)                            \
  ...
  uint realdigits =                                                            \
      serialSeparatedBufferIn((uint8 *)number_as_cstring, digits);             \
...
#define writenumber(src, format, digits)                                       \
  ...
  serialSeparatedBufferOut((uint8 *)number_as_cstring,                         \
                           (uint)strlen(number_as_cstring));
...
inline void CMemStream::serial(std::string &b) {
  if (_StringMode) {
    ...
    if (isReading()) {
      ...
      /*if (len>1000000)
          throw NLMISC::EInvalidDataStream( "CMemStream/str: Trying to read a
         string of %u bytes", len );
      */
      b.resize(len);
    } else {
      len = (uint32)b.size();
      if (len > 1000000)
        throw NLMISC::EInvalidDataStream(
            "CMemStream/str: Trying to write a string of %u bytes", len);
      serial(len);
    }
    ...
    if (isReading()) {
      if (!isXML()) {
        ...
        /*
        if (len>1000000)
            throw NLMISC::EInvalidDataStream( "CMemStream: Trying to read a
        string of %u bytes", len );
        */
        b.resize(len);
        if (len > 0) {
          // can serial all in a single call to serialBuffer, since sizeof(char)
          // == 1
...
inline void CMemStream::serial(ucstring &b) {
  if (_StringMode) {
    ...
    if (isReading()) {
      ...
      /*
      if (len>1000000)
          throw NLMISC::EInvalidDataStream( "CMemStream/str: Trying to read an
      ucstring of %u bytes", len );
      */
      b.resize(len);
    } else {
      len = (uint32)b.size();
      if (len > 1000000)
        throw NLMISC::EInvalidDataStream(
            "CMemStream/str: Trying to write an ucstring of %u bytes", len);
      serial(len);
    }
    // Read/Write the string.
    for (uint i = 0; i != len; ++i)

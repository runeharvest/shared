 ...
 * Example:
 * \code
    // I want to create a function that computes the square of the parameter and
 display the result NLMISC_COMMAND(square,"display the square of the
 parameter","<value>")
    {
 ...
 * \endcode
 *
 * Please use the same casing than for the function (first letter in lower case
 and after each word first letter in upper case)
 * ie: myFunction, step, orderByName, lookAtThis
 *
 * System extended by Sadge July 2004
 *  - NLMISC_CATEGORISED_COMMAND now takes a 4th 'category' parameter which is
 used by 'help' system to organise cmmands
 *  - All default commands added by NeL are categorised as "nel"
 *  - All commands created using the NLMISC_COMMAND macro are are categorised as
 "commands"
 *
...
#define NLMISC_COMMAND(__name, __help, __args)                                 \
  NLMISC_CATEGORISED_COMMAND(commands, __name, __help, __args)
#define NLMISC_CATEGORISED_COMMAND(__category, __name, __help, __args)         \
  struct __category##_##__name##Class : public NLMISC::ICommand {              \
    ...
    virtual bool execute(const std::string &rawCommandString,                  \
                         ...
                         NLMISC::CLog &log, bool quiet, bool human);           \
  };                                                                           \
  __category##_##__name##Class __category##_##__name##Instance;                \
  bool __category##_##__name##Class::execute(                                  \
      const std::string &rawCommandString,                                     \
      const std::vector<std::string> &args, NLMISC::CLog &log, bool quiet,     \
      bool human)

/** Helper to declare a command as friend of a class.
 *	Useful when you want to declare debug command that access private class
 *method or data.
 */
#define NLMISC_COMMAND_FRIEND(__name) friend struct commands_##__name##Class
#define NLMISC_CATEGORISED_COMMAND_FRIEND(__category, __name)                  \
  friend struct __category##_##__name##Class

/**
 * Create a function that can be call in realtime. Don't use this class directly
 * but use the macro NLMISC_COMMAND \author Vianney Lecroart \author Nevrax
 * France \date 2001
 */
class ICommand {
public:
  /// Constructor
  ICommand(const char *categoryName, const char *commandName,
  ...
  /// ..."
  static bool execute(const std::string &commandWithArgs, NLMISC::CLog &log,
                      bool quiet = false, bool human = true);
  ...
  /// if the string begin with an upper case, it s a variable, otherwise, it s a
  /// command
  ...
  /** declare a command to "enable control char". By default all commands
   *"enable control char"
   *
   *	eg: if enableControlCharForCommand("region", false) is called, then the
   *command:
   *
   ...
   *	hello;/"i/am/busy"/\never/disturb/me/please
   */
  static void enableControlCharForCommand(const std::string &commandName,
                                          bool state);
...
struct TCommandHandlerInfo {
  ...
  bool operator==(const TCommandHandlerInfo &other) const {
    return (CommandHelp == other.CommandHelp) &&
...
struct TCommandHandlerClassInfo {
  ...
  /// Constructor.
  TCommandHandlerClassInfo() : InstanceCount(0) {}
};
...
public:
   ...
   *	on the object class.
   */
  virtual void fillCommandsHandlerList(
      TCommandHandlerClassInfo::TCommandsInfo &commandList) = 0;
...
template <class T> struct TCommandHandler : public TCommandHandlerInfo {
  typedef bool (T::*TCommand)(const std::string &rawCommandString,
                              const std::vector<std::string> &args,
                              NLMISC::CLog &log, bool quiet, bool human);
 ...
 *			NLMISC_COMMAND_HANDLER_ADD(CMyClass, theCommand1,
 *"help", "args") NLMISC_COMMAND_HANDLER_ADD(CMyClass, theCommand2, "other
 *help", "other args") NLMISC_COMMAND_HANDLER_TABLE_END
 *
 ...
 *			NLMISC_COMMAND_HANDLER_ADD(CMyClass, addedCommand,
 *"help", "args") NLMISC_COMMAND_HANDLER_TABLE_END
 *
...
//{
// public:
//	/** Constructor, used to initialise the class name in the interface
//class */
////	CCommandsHandler()
...
//	 *	This method will be automatically implemented by the
//NLMISC_COMMAND_HANDLER_TABLE_BEGIN 	 *	macro utility.
//	 */
...
//	 */
////	bool execute(const std::string &rawCommandString, const std::string
///&commandName, const std::vector<std::string> &args, NLMISC::CLog &log, bool
///quiet, bool human = true) /	{ /		TCommand cmd =
///getCommandHandler(commandName); /		if (cmd != NULL) / { /
///T* tp = static_cast<T*>(this); /			return
///(tp->*cmd)(rawCommandString, args, log, quiet, human); /		} /
///else /		{ /			log.displayNL("Command on object
///'%s' : unknow command '%s'", /
///getCommandHandlerName().c_str(), /
///commandName.c_str()); /			return false; /		} /
///}
//};
...
#define NLMISC_COMMAND_HANDLER_TABLE_BEGIN(className)                          \
  ...
  typedef bool (className::*TCommand)(const std::string &rawCommandString,     \
                                      const std::vector<std::string> &args,    \
                                      NLMISC::CLog &log, bool quiet,           \
                                      bool human);                             \
                                                                               \
  /** Typedef for a container for command handler table. */                    \
  typedef std::map<std::string, NLMISC::TCommandHandler<className>>            \
      TCommandsTable;                                                          \
                                                                               \
  ...
  virtual bool execute(const std::string &rawCommandString,                    \
    ...
    if (cmd != NULL) {                                                         \
      ...
      log.displayNL("Command on object '%s' : unknow command '%s'",            \
                    getCommandHandlerName().c_str(), commandName.c_str());     \
      return false;                                                            \
    }                                                                          \
  }                                                                            \
  virtual void fillCommandsHandlerList(                                        \
      NLMISC::TCommandHandlerClassInfo::TCommandsInfo &commandList) {          \
    const TCommandsTable &commandTable =                                       \
        className##_getCommandsHandlerTable();                                 \
    TCommandsTable::const_iterator first(commandTable.begin()),                \
        last(commandTable.end());                                              \
 ...
 *
 */
#define NLMISC_COMMAND_HANDLER_ADD(className, theCommandName, theCommandHelp,  \
                                   theCommandArgs)                             \
  {                                                                            \
...
#define NLMISC_COMMAND_HANDLER_TABLE_EXTEND_BEGIN(className, baseClassName)    \
  ...
  virtual bool execute(const std::string &rawCommandString,                    \
    ...
    if (cmd != NULL) {                                                         \
      ...
      return baseClassName::execute(rawCommandString, commandName, args, log,  \
                                    quiet, human);                             \
    }                                                                          \
  }                                                                            \
  virtual void fillCommandsHandlerList(                                        \
...
#define NLMISC_CLASS_COMMAND_DECL(commandName)                                 \
  bool cmdHandler_##commandName(const std::string &rawCommandString,           \
                                const std::vector<std::string> &args,          \
                                NLMISC::CLog &log, bool quiet, bool human)

// A macro to implement the command method in a cpp (you still need to declare
// it in the class scope using the previous macro)
#define NLMISC_CLASS_COMMAND_IMPL(className, commandName)                      \
  bool className::cmdHandler_##commandName(                                    \
      const std::string &rawCommandString,                                     \
      const std::vector<std::string> &args, NLMISC::CLog &log, bool quiet,     \
      bool human)
...
#define NLMISC_CLASS_COMMAND_CALL_BASE(baseClassName, commandName)             \
  baseClassName::cmdHandler_##commandName(rawCommandString, args, log, quiet,  \
                                          human)
...
class CCommandRegistry {
  ...
  typedef std::map<std::string, TCommandHandlerClassInfo>
      TCommandsHandlersClass;
...
public:
  ...
  void registerNamedCommandHandler(ICommandsHandler *handler,
                                   const std::string &className);
  /// Called by command handlers to unregister themselves.
  void unregisterNamedCommandHandler(ICommandsHandler *handler,
                                     const std::string &className);

  /// Executes the command and display output to the log
  /// \param quiet true if you don't want to display the "executing the command
  /// ..."
  bool execute(const std::string &commandWithArgs, NLMISC::CLog &log,
               bool quiet = false, bool human = true);
  ...
  // initialisation for IVariable management (variable are an extension of
  // commands)
  void initVariables(NLMISC::CConfigFile &configFile);
};

/** This class is only used to serialize easily a command for the admin service
 * for example */

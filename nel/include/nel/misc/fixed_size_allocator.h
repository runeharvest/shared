 ...
 * NB: Unlike CBlockManager, when a chunk contains no allocated blocks, it will
 * be freed. If only one chunk remains, it isn't deleted, however. Another
 * motivation for that class is that it can be used for a memory arena style
 * allocator, because size isn't a fixed parameter of template. A fixed size
 * allocator implemented as a template for type T can be layered on this
 * implementation.
 *
...
private:
  ...
  public:
    ...
    CNode *Next; // points the next free block. Valid only if node is not
                 // allocated, otherwise, block datas overlap tht field.
    CNode *
        *Prev; // points the previous node 'Next' field. Valid only if node is
               // not allocated, otherwise, block datas overlap that field.
  ...
  public:
    ...
    // If all node of a chunk are unused, and it is not the last chunk of the
    // allocator, it is removed.
    void link();
  };
  class CChunk {
  public:
  ...
  //
  CNode *_FreeSpace;
  uint _NumChunks; // The number of created chunks. Once one of them is created,
                   // we manage to keep it alive in order to avoid chunk
                   // creation / destructions if only a few blocks are used

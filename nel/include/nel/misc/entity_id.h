...
public:
  ...
  /// The maximum number that we could generate without generate an overtaking
  /// exception.
  ...
  static CEntityId getNewEntityId(uint8 type) {
    nlassert(
        _NextEntityId !=
        Unknown); // type may be Unknown, so isUnknownId() would return true
  ...
  explicit CEntityId(const std::string &str) { fromString(str.c_str()); }
  ...
  uint64 getShortId() const { return DetailedId.Id; }

  /// Set the local entity number
  void setShortId(uint64 shortId) { DetailedId.Id = shortId; }

  /// Get the variable routing info
  uint8 getDynamicId() const { return DetailedId.DynamicId; }

  /// Set the variable routing info
  void setDynamicId(uint8 dynId) { DetailedId.DynamicId = dynId; }

  /// Get the persistent creator id
  uint8 getCreatorId() const { return DetailedId.CreatorId; }

  /// Set the persistent creator id
  void setCreatorId(uint8 creatorId) { DetailedId.CreatorId = creatorId; }

  /// Get the entity type
  uint8 getType() const { return (uint8)DetailedId.Type; }

  /// Set the entity type
  void setType(uint8 type) { DetailedId.Type = type; }

  /// Get the persistent part of the entity id (the dynamic part in the returned
  /// id is 0)
  ...
  bool isUnknownId() const { return DetailedId.Type == UNKNOWN_TYPE; }
  ...
  //	virtual bool operator == (const CEntityId &a) const
  {
    ...
    /*
    return (Id == a.DetailedId.Id && DetailedId.CreatorId ==
    a.DetailedId.CreatorId && DetailedId.Type == a.DetailedId.Type);
    */
  }
  bool operator!=(const CEntityId &a) const { return !((*this) == a); }
  ...
  // ---------------------------------------------------------------------------------
  // other methods...
  uint64 asUint64() const { return FullId; }
  ...
  */
  ...
  /// return a string in form "(a:b:c:d)" where a,b,c,d are components of entity
  /// id.
  ...
  /// Read from a debug string, use the same format as toString()
  /// (id:type:creator:dynamic) in hexadecimal
  ...
  //	virtual void	fromString(const char *str)
  {
    ...
    if (sscanf(str, "(%" NL_I64 "x:%x:%x:%x)", &id, &type, &creatorId,
               &dynamicId) != 4) {
      *this = Unknown;
      return;
    }
  ...
  */
  ...
  //	friend std::stringstream &operator << (std::stringstream &__os, const
  //CEntityId &__t);
};
...
/*inline std::stringstream &operator << (std::stringstream &__os, const
CEntityId &__t)
{
    __os << __t.toString ();
    return __os;

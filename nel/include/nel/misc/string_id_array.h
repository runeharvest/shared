...
public:
   ...
   * string will be added in _NeedToAskStringArray if ignoreAllUnknownId is
   * false and IgnoreIfUnknown is false too.
   */
    ...
    if (!_IgnoreAllUnknownId && !IgnoreIfUnknown) {
      ...
      if (_NeedToAskStringArray.find(str) == _NeedToAskStringArray.end()) {
        if (_AskedStringArray.find(str) == _AskedStringArray.end()) {
          // nldebug ("String '%s' not found, add it to _NeedToAskStringArray",
          // str.c_str ());
  ...
  void resize(TStringId size) { _StringArray.resize(size); }

  /// Returns the size of the _StringArray
  TStringId size() const { return (TStringId)_StringArray.size(); }
  ...
  void moveNeedToAskToAskedStringArray() {
    if (!_IgnoreAllUnknownId) {
      _AskedStringArray.insert(_NeedToAskStringArray.begin(),
                               _NeedToAskStringArray.end());
      _NeedToAskStringArray.clear();
    }
  }

  /// If set to true, when we ask a string with no id, we don't put it in the
  /// _NeedToAskStringArray array
  ...
  /// Displays all association of the array in a C style (useful to copy/paste
  /// in your C code)
  void display() {

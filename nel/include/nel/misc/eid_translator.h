...
public:
  ...
  struct CEntity {
    CEntity()
        : EntityNameStringId(0), EntitySlot(-1), ShardId(0),
          UId(std::numeric_limits<uint32>::max()), Online(false) {}

    CEntity(const ucstring &entityName, uint32 uid, const std::string &userName,
            sint8 entitySlot, uint32 shardId = 0)
        : EntityName(entityName), EntityNameStringId(0), EntitySlot(entitySlot),
          ShardId(shardId), UId(uid), UserName(userName), Online(false) {}
  ...
  void registerEntity(const CEntityId &eid, const ucstring &entityName,
                      sint8 entitySlot, uint32 uid, const std::string &userName,
                      uint32 shardId = 0);
  // register or update an entity in this manager
  void updateEntity(const CEntityId &eid, const ucstring &entityName,
                    sint8 entitySlot, uint32 uid, const std::string &userName,
                    uint32 shardId = 0);
  ...
  // set an association entityName / entityStringId, return true if association
  // has been set
  bool setEntityNameStringId(const ucstring &entityName, uint32 stringId);
  // set an association entityId / entityStringId, return true if association
  // has been set
  ...
  // check if parameters are coherent with the content of the class, if not, set
  // with the parameters and warn
  void checkEntity(const CEntityId &eid, const ucstring &entityName, uint32 uid,
                   const std::string &userName);

  // the first param is the file where are all entities information, the second
  // is a text file (one line per pattern using * and ?) with invalid entity
  // name
  ...
  void getEntityIdInfo(const CEntityId &eid, ucstring &entityName,
                       sint8 &entitySlot, uint32 &uid, std::string &userName,
                       bool &online, std::string *additional = NULL);

  // transform a username ucstring into a string that can be compared with
  // registered string
  std::string getRegisterableString(const ucstring &entityName);

  /// return a vector of invalid names
  const std::vector<std::string> &getInvalidNames() {
  ...
  /**
   * Callback called when getEntityIdInfo called, so service may add additional
   * info Format MUST be [InfoName InfoValue]* (e.g. a list of 2 strings, first
   * being name for the retrieved info, and second being the value of the info
   */
...
private:
  ...
  void getByUser(const std::string &userName,
                 std::vector<NLMISC::CEntityId> &res, bool exact = true);

  void getByEntity(const ucstring &entityName,
                   std::vector<NLMISC::CEntityId> &res, bool exact);
  ...
  // It means that there only alphabetic and numerical character and the name is
  // at least 3 characters long.
  bool isValidEntityName(const ucstring &entityName,

...
public:
  ...
  // NB: release the singleton, but assert if there is any pending loading
  // tasks. Each system that use the async file manager should ensure it has no
  // more pending task in it
  ...
  /**
   *	CCancelCallback is an interface that is used in call to
   *CAsyncFileManager::cancelLoad. This class contains one method that is called
   *for each task in the async file loader. If the method returns true, then the
   *task is removed and cancelLoad return. \author Boris Boucher \author Nevrax
   *France \date 10/2002
   */
...
private:
  //	CAsyncFileManager (); // Singleton mode -> access it with the
  ...
  public:
    CMultipleFileLoad(const std::vector<std::string> &vFileNames,
                      const std::vector<uint8 **> &vPtrs);
    void run(void);
    void getName(std::string &result) const;
  };

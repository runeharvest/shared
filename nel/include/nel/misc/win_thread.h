...
public:
  ...
  // Increment the suspend count, a suspend count >= 1 means the thread is
  // suspended
...
public:
  ...
  static bool enumProcessModules(uint32 processId,
                                 std::vector<std::string> &moduleNames);
  static uint32
  getProcessIdFromModuleFilename(const std::string &moduleFileName);
  static bool terminateProcess(uint32 processId, uint exitCode = 0);
  static bool terminateProcessFromModuleName(const std::string &moduleName,
                                             uint exitCode = 0);
...
public:
    CProcessWatch();
    ~CProcessWatch();
    // launch a process with the given name and arguments, return true on
success bool launch(const std::string &programName, const std::string
&arguments);

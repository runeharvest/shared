...
/**
 * Words dictionary: allows to search for keys and words in
 * <type>_words_<language>.txt Unicode files. All searches are case
 * not-sensitive.
 *
 * \author Olivier Cado
...
public:
   ...
   * - If inputStr is partially or completely found in the keys, all the
   * matching <key,words> are returned;
   * - If inputStr is partially or completely in the words, all the matching
   * <key, words> are returned. The following tags can modify the behaviour of
   * the search algorithm:
   * - ^mystring returns mystring only if it is at the beginning of a key or
   * word
   * - mystring$ returns mystring only if it is at the end of a key or word
   * All returned words are in UTF8 string or ANSI string, depending of the
   * config file.
   */
  ...
  /// Return the key contained in the provided string returned by lookup()
  /// (without extension)

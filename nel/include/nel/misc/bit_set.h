...
public:
  ...
  uint size() const { return NumBits; }
  /// Set a bit to 0 or 1.
  ...
  /// Get the value of a bit.
  bool operator[](sint bitNumber) const { return get(bitNumber); }
  /// Set a bit to 1.
   ...
   * The result BitSet is of size of \c *this. Any missing bits into bs will be
   * considered as 0.
   */
   ...
   * The bitset size is not changed. Any missing bits into bs will be considered
   * as 0.
   */
  CBitSet &operator&=(const CBitSet &bs);
  /**
  ...
  //@{
  /**
   * Compare two BitSet not necessarely of same size. The comparison is done on
   * N bits, where N=min(this->size(), bs.size()) \return true if the N common
   * bits of this and bs are the same. false otherwise.
   */
  ...
  /// Return a string representing the bitfield with 1 and 0 (from left to
  /// right)

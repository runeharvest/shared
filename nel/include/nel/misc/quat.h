...
struct CAngleAxis
{
	...
	CAngleAxis() { }
	CAngleAxis(const CVector &axis, float ang)
	    : Axis(axis)
	    , Angle(ang)
	{
	}
...
{
	if (fabs(x - a.x) < epsilon && fabs(y - a.y) < epsilon && fabs(z - a.z) < epsilon && fabs(w - a.w) < epsilon)
	{
		return true;
	}
	return false;
...
public:
	...
	CQuat(float X, float Y, float Z, float W)
	    : CQuatT<float>(X, Y, Z, W)
	{
	}
	/// ctor of a UNIT quaternion, from an angle axis.
	CQuat(const CVector &axis, float angle)
	    : CQuatT<float>(axis, angle)
	{
	}
	/// ctor of a UNIT quaternion, from an angle axis.
...
public:
	...
	CQuatD(double X, double Y, double Z, double W)
	    : CQuatT<double>(X, Y, Z, W)
	{
	}
	/// ctor of a UNIT quaternion, from an angle axis.
	CQuatD(const CVector &axis, float angle)
	    : CQuatT<double>(axis, angle)
	{
	}
	/// ctor of a UNIT quaternion, from an angle axis.

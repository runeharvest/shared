...
public:
  ...
  CQuatT operator+(const CQuatT &o) const {
    return CQuatT(x + o.x, y + o.y, z + o.z, w + o.w);
  }
  CQuatT operator-(const CQuatT &o) const {
    return CQuatT(x - o.x, y - o.y, z - o.z, w - o.w);
  }
  ...
  CQuatT operator/(T f) const {
    double oof = 1.0 / f;
    return CQuatT(x * oof, y * oof, z * oof, w * oof);
  }
  ...
  /// Invert this quaternion. If normalized, conjugate is faster and does same
  /// thing.
  void invert();
  ...
  CVector getAxis() const {
    CVector ret((float)x, (float)y, (float)z);
    return ret.normed();
  }
  ...
  void setAngleAxis(const CAngleAxis &angAxis) {
    setAngleAxis(angAxis.Axis, angAxis.Angle);
  }
  // @}
  ...
  CQuatT exp();
  /// ensure that *this and q are on same side of hypersphere, ie
  /// dotProduct(*this,q) is >0, modifying this if necessary.
  void makeClosest(const CQuatT &o);
  /// serial.
  void serial(IStream &f) { f.serial(x, y, z, w); }
  // @}
...
public:
  ...
  /** Quaternion spherical linear interpolation. when t==0, ret==q0, when t==1,
   * ret==q1. No hemisphere correction is made.
   */
  static CQuatT slerp(const CQuatT<T> &q0, const CQuatT<T> &q1, float t);
  /** Quaternion Quadratic spherical linear interpolation. when t==0, ret==q0,
   * when t==1, ret==q1. No hemisphere correction is made.
   */
  static CQuatT squad(const CQuatT<T> &q0, const CQuatT<T> &tgtQ0,
                      const CQuatT<T> &tgtQ1, const CQuatT<T> &q1, float t);
  /** Quaternion Quadratic spherical linear interpolation, with multi revision
   * support.
   */
  static CQuatT squadrev(const CAngleAxis &rot, const CQuatT<T> &q0,
                         const CQuatT<T> &tgtQ0, const CQuatT<T> &tgtQ1,
                         const CQuatT<T> &q1, float t);
...
/// f*quat operator
template <class T> inline CQuatT<T> operator*(T f, const CQuatT<T> &o) {
  return o * f;
}
...
inline bool CQuatT<T>::equal(const CQuatT<T> &a, float epsilon) const {
  if (fabs(x - a.x) < epsilon && fabs(y - a.y) < epsilon &&
...
inline CQuatT<T> CQuatT<T>::operator*(const CQuatT<T> &o) const {
  // wres= ww' - v.v'
  // vres= wv' + w'v + v^v' ]
  return CQuatT<T>((w * o.x) + (x * o.w) + (y * o.z) - (z * o.y),
...
// ***************************************************************************
template <class T> inline CQuatT<T> &CQuatT<T>::operator*=(const CQuatT<T> &o) {
  *this = *this * o;
  return *this;
}

// ***************************************************************************
template <class T> inline void CQuatT<T>::invert() {
...
// ***************************************************************************
template <class T> inline void CQuatT<T>::normalize() {
  T f = norm();
  if (f == 0)
    identity();
  else {
...
// ***************************************************************************
template <class T>
CQuatT<T> CQuatT<T>::squad(const CQuatT<T> &q0, const CQuatT<T> &tgtQ0,
                           const CQuatT<T> &tgtQ1, const CQuatT<T> &q1,
                           float t) {
  return CQuatT<T>::slerp(CQuatT<T>::slerp(q0, q1, t),
...
// ***************************************************************************
template <class T>
CQuatT<T> CQuatT<T>::squadrev(const CAngleAxis &rot, const CQuatT<T> &q0,
                              const CQuatT<T> &tgtQ0, const CQuatT<T> &tgtQ1,
                              const CQuatT<T> &q1, float t) {
  float s, v;
  ...
  // rotation of 180deg around rot.Axis.  (=> sin(a/2)==sin(Pi/2)==1, and
  // c(a/2)=0).
  qaxis.set(rot.Axis.x, rot.Axis.y, rot.Axis.z, 0);

  // the number of revisions (float!)
  nrevs = (float)(omega / Pi);
  // Angle>2Pi. squad from 0 to Pi, slerp from Pi to Angle-Pi, squad from
  // Angle-Pi to Angle.
  s = t * 2 * nrevs;

  // So for s, squad from 0 to 1, slerp from 1 to 2*nrevs-1, squad from
  // 2*nrevs-1 to 2*nrevs.
  if (s < 1.0f) {
    // first part.
    pp = q0 * qaxis;
...
// ***************************************************************************
template <class T> CQuatT<T> CQuatT<T>::log() {
  double len;
...
// ***************************************************************************
template <class T> CQuatT<T> CQuatT<T>::exp() {
  double len;
  len = sqrt(x * x + y * y + z * z);
...
// ***************************************************************************
template <class T> void CQuatT<T>::makeClosest(const CQuatT<T> &o) {
...
public:
  ...
  operator CQuat() const {
    return CQuat((float)x, (float)y, (float)z, (float)w);
  }
  // @}
};

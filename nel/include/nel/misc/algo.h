...
// NB: like 'toString' or 'swap', this function is intended to be specialised
// for other types (CRGBA ...)
...
// ***************************************************************************
// add a delta to a value until it reaches the wanted target. Value is clamped
// to the target
 ...
 */
template <class T, class U>
T computeBilinear(const T &v0, const T &v1, const T &v2, const T &v3,
                  const U &s, const U &t) {
  T h0 = t * v3 + ((U)1 - t) * v0;
  T h1 = t * v2 + ((U)1 - t) * v1;
 ...
 */
void drawFullLine(float x0, float y0, float x1, float y1,
...
// ***************************************************************************
/**	Search the lower_bound in a sorted array of Value, in growing order (0,
 *1, 2....). operator<= is used to perform the comparison. It return the first
 *element such that array[id]<=key If not possible, 0 is returned NB: but 0 may
 *still be a good value, so you must check whether or not 0 means "Not found",
 *or "Id 0".
 */
template <class T>
...
// ***************************************************************************
/**	Search the lower_bound in a sorted array of Value, in growing order (0,
 *1, 2....). operator<= is used to perform the comparison. It returns the first
 *element such that array[id]<=key If not possible, 0 is returned NB: but 0 may
 *still be a good value, so you must check whether or not 0 means "Not found",
 *or "Id 0".
 */
 ...
 *	eg: splitString("hello|bye|||bee", "|", list) return 3 string into list:
 *"hello", "bye" and "bee".
 */
void splitString(const std::string &str, const std::string &separator,
                 std::vector<std::string> &retList);

void splitUCString(const ucstring &ucstr, const ucstring &separator,
                   std::vector<ucstring> &retList);

// ***************************************************************************
/// In a string or ucstring, find a substr and replace it with another. return
/// true if replaced
template <class T, class U>
...
template <class T, class U> inline void setFlags(T &dest, U mask, bool on) {
  if (on) {
    dest = (T)(dest | (T)mask);
  } else {
    dest = (T)(dest & ~((T)mask));

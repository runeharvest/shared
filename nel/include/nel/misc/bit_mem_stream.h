...
#define serialAndLog1(v) _serialAndLog(#v, v);

#define serialAndLog2(v, s) _serialAndLog(#v, v, s);

#define serialBitAndLog(v) _serialBitAndLog(#v, v);
...
struct TBMSSerialInfo {
  ...
  TBMSSerialInfo(uint32 bitpos, uint32 bitsize, TSerialType type,
                 const char *symbol) {
...
struct TBMSDbgInfoData {
  /// Constructor
  TBMSDbgInfoData()
      : List(), CurrentBrowsedItem(0), NextSymbol(NULL),
        AddEventIsEnabled(true) {}

  /// Vector of serial items
  TBMSSerialInfoList List;

  /// Current browsed item in the list (valid only from beginEventBrowsing()
  /// until clear() or addSerial()/addPoke())
  uint32 CurrentBrowsedItem;

  /// Symbol of next event
  const char *NextSymbol;

  /// Flag to enable/disable addSerial() and addPoke() (because
  /// CBitMemStream::getSerialItem() must not add events in the list)
...
/*
 * Debug details about what was serialised (sizeof is only one pointer if not
...
public:
  ...
  /// Begin a browsing session of serial events, addSerial()/addPoke() is now
  /// disabled
  ...
  /// End a browsing session of serial events, and reenable
  /// addSerial()/addPoke()
  ...
  /// Return an eventId of serial event, or "" (and eventId -1) if nothing found
  /// at the specified bitpos
  std::string getEventIdAtBitPos(uint32 bitpos, sint32 *eventId) {
#ifdef NL_DEBUG
    if (_DbgData->CurrentBrowsedItem < _DbgData->List.size()) {
      if (bitpos == _DbgData->List[_DbgData->CurrentBrowsedItem]
                        .BitPos) // works only with a vector!
      {
        *eventId = (sint32)_DbgData->CurrentBrowsedItem;
        ++_DbgData->CurrentBrowsedItem;
        return toString("(%u)", _DbgData->CurrentBrowsedItem - 1);
      }
      // nlassert( bitpos < (*_List)[_CurrentBrowsedItem].BitPos ); // occurs if
      // stream overflow
    }
#else
    nlunreferenced(bitpos);
...
#ifdef NL_DEBUG
  /// Explicit init
  void init() { _DbgData = new TBMSDbgInfoData(); }
 ...
 * In CMemStream, _BufPos points to the end of the stream, where to write new
 * data. In CBitMemStream, _BufPos points to the last byte of the stream, where
 * to write new data. Then _FreeBits tells the number of bits not used in this
 * byte (i.e. (8-_FreeBits) is the number of bits already filled inside this
 * byte). The minimum buffer size is 1: when nothing has been written yet, the
 * position is at beginning and _FreeBits is 8.
 *
 * How input mode works:
 * Same as in CMemStream, but some bits may be free in the last byte (the
 * information about how many is not stored in the CBitMemStream object, the
 * reader needs to know the format of the data it reads).
 *
 * \author Olivier Cado
...
public:
  ...
  /**
   * Set the position at the beginning. In output mode, the method ensures the
   * buffer contains at least one blank byte to write to.
   *
  ...
  virtual void invert() {
    if (!isReading()) {
      //			++_BufPos; // write->read: extend to keep the
      //last byte inside the payload
      ++_Buffer.Pos; // write->read: extend to keep the last byte inside the
                     // payload
    }
    CMemStream::invert();
    if (!isReading()) {
...
#endif
      //			--_BufPos; // read->write: set the position on
      //the last byte, not at the end as in CMemStream::invert()
      --(_Buffer.Pos); // read->write: set the position on the last byte, not at
                       // the end as in CMemStream::invert()
    }
  ...
  /**
   * Same as CMemStream::reserve(). Warning, the return value is a byte pos (not
   * bitpos)! Consider using reserveBits() instead.
   */
  sint32 reserve(uint byteLen);

  /**
   * In a output bit stream, serialize nbits bits (no matter their value).
   * Works even if the number of bits to add is larger than 64. See also poke()
   * and pokeBits().
   */
  void reserveBits(uint nbits);

  /*
   * Rewrite the nbbits lowest bits of a value at the specified position bitpos
   * of the current output bit stream.
   *
   * Preconditions:
  ...
  /**
   * Rewrite the bitfield at the specified position bitpos of the current output
   * bit stream. The size of the bitfield is *not* written into the stream
   * (unlike serialCont()). Precondition: bitpos+bitfield.size() <= the current
   * length in bit of the stream. See also reserveBits().
   */
  ...
  /// Display the bits of the stream just before the specified bitpos (or
  /// current pos if -1)
  void displayLastBits(sint nbits, sint bitpos = -1,
  ...
  */
  ...
  void _serialAndLog(const char *argstr, sint8 &b) {
    serialAdaptAndLog(argstr, b, sint8);
  }
  void _serialAndLog(const char *argstr, uint16 &b) {
    serialAdaptAndLog(argstr, b, uint16);
  }
  void _serialAndLog(const char *argstr, sint16 &b) {
    serialAdaptAndLog(argstr, b, sint16);
  }
  void _serialAndLog(const char *argstr, uint32 &b) {
    serialAdaptAndLog(argstr, b, uint32);
  }
  void _serialAndLog(const char *argstr, sint32 &b) {
    serialAdaptAndLog(argstr, b, sint32);
  }
  void _serialAndLog(const char *argstr, uint64 &b) {
    serialAdapt64AndLog(argstr, b);
  }
  void _serialAndLog(const char *argstr, sint64 &b) {
    serialAdapt64AndLog(argstr, b);
  }
  void _serialAndLog(const char *argstr, float &b);
  void _serialAndLog(const char *argstr, double &b) {
    serialAdapt64AndLog(argstr, b);
  }
  void _serialAndLog(const char *argstr, bool &b) {
    _serialBitAndLog(argstr, b);
  }
#ifndef NL_OS_CYGWIN
...
protected:
   ...
   *
   * Preconditions
   * - The last written byte, at pos _BufPos, is fully written (but _FreeBits
  ...
  void increaseBufferIfNecessary() {
    ...
    if (_Buffer.Pos == _Buffer.getBuffer().size()) {
      ...
      //			_BufPos = _Buffer.getPtr() + bytepos; // don't
      //change the pos but update pointer (needed because the buffer may have
      //moved when reallocating)
    }
  }

  /**
   * Helper for poke(), to write a value inside an output stream (works because
   * reserveBits sets to 0) Warning: if _FreeBits == 8, increments _BufPos.
   */
  void serialPoke(uint32 value, uint nbits);

  /// Number of bits unused at the current pos. If 8, means the current pos if
  /// full and we need to increment the pos!
...
void displayBitStream(const CBitMemStream &msg, sint beginbitpos,
                      sint endbitpos, NLMISC::CLog *log = NLMISC::DebugLog);

/*
 * Return full info about a serial event, or "" if eventId is -1
 */
inline std::string CBMSDbgInfo::getEventLegendAtBitPos(CBitMemStream &bms,
                                                       sint32 eventId) {
#ifdef NL_DEBUG
  if (eventId != -1) {
    nlassert(eventId < (sint32)_DbgData->List.size());
    TBMSSerialInfo &serialItem =
        _DbgData->List[eventId]; // works only with a vector!
    return toString("(%d) BitPos %3u Type %s BitSize %2u Value %s %s\n",
                    eventId, serialItem.BitPos,
                    SerialTypeToCStr[serialItem.Type], serialItem.BitSize,
                    bms.getSerialItem(serialItem).c_str(),
                    (serialItem.Symbol != NULL) ? serialItem.Symbol : "");
  }
#else
  nlunreferenced(bms);

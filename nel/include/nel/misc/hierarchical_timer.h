...
#define H_AUTO(__name)                                                         \
  ...
  NLMISC::CAutoTimer __name##_auto(&__name##_timer);

// Same as H_AUTO but you don't have to give a name, it uses the function/line
#define H_AUTO2                                                                \
  static std::string __str_##__LINE__(                                         \
      NLMISC::toString("%s:%d", __FUNCTION__, __LINE__));                      \
  static NLMISC::CHTimer __timer_##__LINE__(__str_##__LINE__.c_str());         \
  NLMISC::CAutoTimer __auto_##__LINE__(&__timer_##__LINE__);

// If you want to bench a specific part of the code
#define H_BEFORE(__name)                                                       \
  static NLMISC::CHTimer __name##_timer(#__name);                              \
  __name##_timer.before();
#define H_AFTER(__name) __name##_timer.after();

// Display the timer info after each loop call
#define H_AUTO_INST(__name)                                                    \
  static NLMISC::CHTimer __name##_timer(#__name);                              \
  NLMISC::CAutoTimerInst __name##_auto(&__name##_timer);

// H_AUTO split in 2. The declaration of the static timer, and a CAutoTimer
// instance. Useful to group same timer bench in different functions for example
 ...
 *  \warning On Intel platform, processor cycles are counted, on other
 * platforms, CTime::getPerformanceTime is used instead.
 *
 * \sa CStopWatch
...
public:
   ...
   */
  static uint64 getStartStopNumTicks() { return _StartStopNumTicks; }
...
/**
 * Hierarchical timing system. Allows to accurately measure performance of
 routines, and displays results hierarchically.
 * To time a piece of code, just declare a static CHTimer object and encapsulate
 code between calls to before() and after() methods.
 * ex:
 *\code
 void myFunction()
 {
...
public:
  ...
  CHTimer(const char *name, bool isRoot = false)
      : _Name(name), _Parent(NULL), _IsRoot(isRoot) {}
   ...
   * \param wantStandardDeviation When true, benchmarks will report the standard
   * deviation of values. This require more memory, however, because each
   * samples must be kept. \param quick if true, quick compute the frequency of
   * the processor
   */
   ...
   */
  static void display(CLog *log = InfoLog, TSortCriterion criterion = TotalTime,
                      bool displayInline = true, bool displayEx = true);
   ...
   * \param displayEx	 true to display more detailed infos.
   */
  static void displayByExecutionPath(CLog *log = InfoLog,
   ...
   * \param labelNumChar
   */
  static void displayHierarchical(CLog *log = InfoLog, bool displayEx = true,
   ...
   * \param labelNumChar
   */
  static void displayHierarchicalByExecutionPath(CLog *log = InfoLog,
                                                 bool displayEx = true,
                                                 uint labelNumChar = 32,
                                                 uint indentationStep = 2);
   ...
   * \param labelNumChar
   */
  static void displayHierarchicalByExecutionPathSorted(
      CLog *log = InfoLog, TSortCriterion criterion = TotalTime,
      bool displayEx = true, uint labelNumChar = 32, uint indentationStep = 2);

  /** Hierarchical display, sorting is done in branches
   * \param displayEx	 true to display more detailed infos.
   * \param labelNumChar
   */
  static void displaySummary(CLog *log = InfoLog,
                             TSortCriterion criterion = TotalTime,
                             bool displayEx = true, uint labelNumChar = 32,
                             uint indentationStep = 2, uint maxDepth = 3);
...
private:
  ...
  struct CNode {
    ...
    uint64 SonsTotalTime; // maybe different from LastSonsTotalTime while
                          // benching the sons and if the display is called in a
                          // benched node
    TTimeVect Measures; // All time measures. Used only when standard deviation
                        // is wanted
    uint64 MinTime;     // the minimum time spent in that node
    uint64 MaxTime;     // the maximum time spent in that node
    uint64 NumVisits; // the number of time the execution has gone through this
                      // node
    // session max measure
...
private:
  ...
  /** This clock is used to measure the preamble of methods such as
   * CHTimer::before() This is static, but the Hierarchical Timer doesn't
   * support multi threading anyway..
   */
...
/**
 * An automatic measuring timer. Encapsulates calls to CHTimer, and avoids
 missuses of before() and after().
 * ex:
 *\code
 void myFunction()
 {

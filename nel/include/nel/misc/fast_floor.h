...
// init float CW.
inline void OptFastFloorBegin() { OptFastFloorPushCW(_RC_DOWN | _PC_53); }

// reset float CW.
inline void OptFastFloorEnd() { OptFastFloorPopCW(); }
...
inline void OptFastFloorBegin24() { OptFastFloorPushCW(_RC_DOWN | _PC_24); }

// reset float CW.
inline void OptFastFloorEnd24() { OptFastFloorPopCW(); }

// Force __stdcall to not pass parameters in registers.
/// Same method as OptFastFloor, but result are always positive and should never
/// be bigger than 2^23-1 Only used for float to byte color attributes
/// conversions
...
inline float OptFastFractionnalPart(float x) {
  ...
  return _mm_cvtss_f32(_mm_comilt_ss(a, b) ? _mm_sub_ss(b, a)
                                           : _mm_sub_ss(a, b));
#else

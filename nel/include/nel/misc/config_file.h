...
/**
 * CConfigFile class. Useful when you want to have a configuration file with
 *variables. It manages integers, real (double), and string basic types. A
 *variable can be an array of basic type. In this case, all elements of the
 *array must have the same type.
 *
 * If you setup the global callback before loading, it'll be called after the
 *load() function.
 *
 * Example:
 *\code
 ...
 *  cf.load ("test.txt");
 *
 *	// Attach a callback to the var1 variable. When the var1 will change,
 *this cvar1cb function will be called cf.setCallback ("var1", var1cb);
 *
 ...
 * var4 = 123.123 + 2;                   // var4  type:real, value:125.123 var5
 *= 123 + 2.1;                     // var5  type:int,         value:125
 *
 * var6 = (-112+1) * 3 - 14;             // var6  type:int,         value:-347
 *
 * var7 = var1 + 1;                      // var7  type:int,         value:124
 *
 * var8 = var2 + 10;                     // var8  type:string, value:456.2510
 *(convert 10 into a string and concat it) var9 = 10.15 + var2;  // var9
 *type:real,        value:466.4 (convert var2 into a real and add it)
 *
 * var10 = { 10.0, 51.1 };               // var10 type:realarray,
 *value:{10.0,51.1} var11 = { "str1", "str2", "str3" };   // var11
 *type:stringarray, value:{"str1", "str2", "str3"}
 *
 * var12 = { 10+var1, var1-var7 };       // var12 type:intarray, value:{133,-1}
 *\endcode
 *
 * Operators are '+', '-', '*', '/'.
 * You can't use operators on a array variable, for example, you can't do
 *\cvar13=var12+1. If you have 2 variables with the same name, the first value
 *will be remplaced by the second one.
 *
 * \bug if you terminate the config file with a comment without carriage returns
 *it'll generate an exception, add a carriage returns
 *
...
public:
  /**
   * CVar class. Used by CConfigFile. A CVar is returned when you want to have a
   * variable.
   *
  ...
  public:
    CVar()
        : Type(T_UNKNOWN), Root(false), Comp(false), FromLocalFile(true),
          SaveWrap(6) {}
    ...
    /// If the index is the size of the array, the value will be append at the
    /// end.
    //@{
    ...
    // Get the size of the variable. It's the number of element of the array or
    // 1 if it's not an array.
    uint size() const;
    ...
    bool FromLocalFile; // Used during cfg parsing. True if the var has been
                        // created from the currently parsed cfg
  ...
  /// Get a variable pointer with the variable name, without throwing exception.
  ...
  /// Clear all the variable array (including information on variable callback
  /// etc)
  void clear();
  ...
  /// set a callback function to a variable, it will be called when this
  /// variable is modified
  void setCallback(const std::string &VarName,
                   void (*cb)(CConfigFile::CVar &var));

  /// contains the variable names that getVar() and getVarPtr() tried to access
  /// but not present in the cfg
  ...
  /// \param timeout time in millisecond, if timeout=0, the check will be made
  /// each "frame"
...
struct EBadType : public EConfigFile {
  EBadType(const std::string &varName, int varType, int wantedType) {
    ...
    smprintf(str, NLMISC::MaxCStringSize,
             "Bad variable type, variable \"%s\" is a %s and not a %s",
             varName.c_str(), CConfigFile::CVar::TypeName[varType],
             CConfigFile::CVar::TypeName[wantedType]);
    _Reason = str;
...
struct EBadSize : public EConfigFile {
  EBadSize(const std::string &varName, int varSize, int varIndex) {
    static char str[NLMISC::MaxCStringSize];
    smprintf(
        str, NLMISC::MaxCStringSize,
        "Trying to access to the index %d but the variable \"%s\" size is %d",
        varIndex, varName.c_str(), varSize);
    _Reason = str;
    nlinfo("CF: Exception will be launched: %s", _Reason.c_str());
  }
};
...
struct EUnknownVar : public EConfigFile {
  EUnknownVar(const std::string &filename, const std::string &varName) {
    static char str[NLMISC::MaxCStringSize];
    smprintf(str, NLMISC::MaxCStringSize,
             "variable \"%s\" not found in file \"%s\"", varName.c_str(),
             filename.c_str());
    _Reason = str;
    nlinfo("CF: Exception will be launched: %s", _Reason.c_str());
  }
...
struct EParseError : public EConfigFile {
  EParseError(const std::string &fileName, int currentLine) {
    static char str[NLMISC::MaxCStringSize];
    smprintf(str, NLMISC::MaxCStringSize,
             "Parse error on the \"%s\" file, line %d", fileName.c_str(),
             currentLine);
    _Reason = str;
    nlinfo("CF: Exception will be launched: %s", _Reason.c_str());
  }
...
struct EFileNotFound : public EConfigFile {
  EFileNotFound(const std::string &fileName) {
    static char str[NLMISC::MaxCStringSize];
    smprintf(str, NLMISC::MaxCStringSize, "File \"%s\" not found",
             fileName.c_str());
    _Reason = str;
  }
};

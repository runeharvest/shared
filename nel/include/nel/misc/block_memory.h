 ...
 *
 * This memory manager is a fast memory allocator, doing same thing as
 *new/delete. It works by blocks. Blocks are always allocated, never deleted.
 *Allocation/free are in O(1).
 *
 * Elements with sizeof(T)<sizeof(void*) should not be used with this allocator,
 *because sizeEltInMemory= max(sizeof(T),sizeof(void*)).
 *
 * free() check invalid ptr in debug only, for extra cost of 8 octets per
 *element.
 *
 * NB: if template parameter __ctor_dtor__ is false, then ctor and dtor are not
 *called when an element is allocate()-ed or deallocate()-ed.
 *
 * \author Lionel Berenguier
 * \author Nevrax France
 * \date 2001
 */
template <class T, bool __ctor_dtor__ = true> class CBlockMemory {
public:
  /// Constructor
  ...
  /// delete an element allocated with this manager. dtor is called. NULL is
  /// tested.
  ...
  /** delete all blocks, freeing all memory. It is an error to purge() or delete
   *a CBlockMemory, while elements still remains!! You must free your elements
   *with free(). NB: you can disable this assert if you set
   *NL3D_BlockMemoryAssertOnPurge to false (good to quit a program quickly
   *without uninitialize).
   */
  void purge() {
    CAutoMutex<CFastMutex> lock(_Mutex);
  ...
  // ********************
public:
  // This is to be used with CSTLBlockAllocator only!!! It changes the size of
  // an element!!
  ...
  uint __stl_alloc_getEltSize() const { return _EltSize; }
...
private:
  void buildBlock(CBlock &block) {
    ...
    // by default, all elements are not allocated, build the list of free
    // elements.
    void *ptr = block.Data;
#ifdef NL_DEBUG
    // if debug, must decal for begin check.
  ...
  void releaseBlock(CBlock &block) { delete[] ((uint8 *)block.Data); }
};

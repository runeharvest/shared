...
public:
  ...
  ///	ctor
  CSString(const std::vector<NLMISC::CSString> &v,
  ...
  /// Return sub string up to but not including first instance of given
  /// character, starting at 'iterator' on exit 'iterator' indexes first
  /// character after extracted string segment
  ...
  /// '//'
  CSString splitToLineComment(bool truncateThis = false,
                              bool useSlashStringEscape = true);

  /// Return sub string from character following first instance of given
  /// character on
  ...
  /// Behave like a s strtok() routine, returning the sun string extracted from
  /// (and removed from) *this
  CSString
  strtok(const char *separators,
         bool useSmartExtensions = false, // if true then match brackets etc
                                          // (and refine with following args)
         bool useAngleBrace = false,      // - treat '<' and '>' as brackets
         bool useSlashStringEscape =
             true, // - treat '\' as escape char so "\"" == '"'
         bool useRepeatQuoteStringEscape = true); // - treat """" as '"')

  /// Return first word (blank separated) - can remove extracted word from
  /// source string
  ...
  /// Return first word or quote-encompassed sub-string - can remove extracted
  /// sub-string from source string
  ...
  /// A handy utility routine for knowing if a character is a white space
  /// character or not (' ','\t','\n','\r',26)
  static bool isWhiteSpace(char c);
  ///	Test whether character matches '{', '(','[' or '<' (the '<' test depends
  ///on the useAngleBrace parameter
  ...
  ///	Tests whether the character 'b' is the closing delimiter or string
  ///delimiter corresponding to character 'a'
  static bool isMatchingDelimiter(char a, char b);

  /// A handy utility routine for knowing if a character is a valid component of
  /// a file name
  static bool isValidFileNameChar(char c);
  /// A handy utility routine for knowing if a character is a valid first char
  /// for a keyword (a..z, '_')
  static bool isValidKeywordFirstChar(char c);
  /// A handy utility routine for knowing if a character is a valid subsequent
  /// char for a keyword (a..z, '_', '0'..'9')
  static bool isValidKeywordChar(char c);
  /// A handy utility routine for knowing if a character is printable
  /// (isValidFileNameChar + more basic punctuation)
  static bool isPrintable(char c);

  /// A handy utility routine for knowing if a character is a hex digit 0..9,
  /// a..f
  static bool isHexDigit(char c);
  /// A handy utility routine for converting a hex digit to a numeric value
  /// 0..15
  static char convertHexDigit(char c);

  // a handy routine that tests whether a given string contains binary
  // characters or not. Only characters>32 + isWhiteSpace() are valid
  bool isValidText();
  // a handy routine that tests whether a given string is a valid file name or
  // not
  ...
  // a second handy routine that tests whether a given string is a valid file
  // name or not equivalent to ('\"'+*this+'\"').isValidFileName()
  ...
  // a handy routine that tests whether or not a given string is quote
  // encapsulated
  bool
  isQuoted(bool useAngleBrace = false, // treat '<' and '>' as brackets
           bool useSlashStringEscape =
               true, // treat '\' as escape char so "\"" == '"'
           bool useRepeatQuoteStringEscape = true) const; // treat """" as '"'

  ///	Search for the closing delimiter matching the opening delimiter at
  ///position 'startPos' in the 'this' string
  /// on error returns startPos
  uint32 findMatchingDelimiterPos(bool useAngleBrace, bool useSlashStringEscape,
                                  bool useRepeatQuoteStringEscape,
                                  uint32 startPos = 0) const;

  ///	Extract a chunk from the 'this' string
  /// if first non-blank character is a string delimiter or an opening delimiter
  /// then extracts up to the matching closing delimiter in all other cases an
  /// empty string is returned the return string includes the opening blank
  /// characters (it isn't stripped)
  CSString
  matchDelimiters(bool truncateThis = false,
                  bool useAngleBrace = false, // treat '<' and '>' as brackets
                  bool useSlashStringEscape =
                      true, // treat '\' as escape char so "\"" == '"'
                  bool useRepeatQuoteStringEscape = true); // treat """" as '"'

  /// copy out section of string up to separator character, respecting quotes
  /// (but not brackets etc) on error tail after returned string doesn't begin
  /// with valid separator character
  CSString splitToStringSeparator(
      char separator, bool truncateThis = false,
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true, // treat """" as '"'
      bool truncateSeparatorCharacter =
          false); // if true tail begins after separator char

  /// copy out section of string up to separator character, respecting quotes,
  /// brackets, etc on error tail after returned string doesn't begin with valid
  /// separator character eg: splitToSeparator(','); - this might be used to
  /// split some sort of ',' separated input
  CSString
  splitToSeparator(char separator, bool truncateThis = false,
                   bool useAngleBrace = false, // treat '<' and '>' as brackets
                   bool useSlashStringEscape =
                       true, // treat '\' as escape char so "\"" == '"'
                   bool useRepeatQuoteStringEscape = true, // treat """" as '"'
  ...
  /// copy out section of string up to any of a given set of separator
  /// characters, respecting quotes, brackets, etc on error tail after returned
  /// string doesn't begin with valid separator character eg:
  /// splitToOneOfSeparators(",;",true,false,false,true); - this might be used
  /// to split a string read from a CSV file
  CSString splitToOneOfSeparators(
      const CSString &separators, bool truncateThis = false,
      bool useAngleBrace = false, // treat '<' and '>' as brackets
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true, // treat """" as '"'
      ...
      bool splitStringAtBrackets =
          true); // if true consider brackets as breaks in the string

  CSString splitToOneOfSeparators(
      const CSString &separators,
      bool useAngleBrace = false, // treat '<' and '>' as brackets
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true) const; // treat """" as '"'

  /// Return true if the string is a single block encompassed by a pair of
  /// delimiters eg: "((a)(b)(c))" or "(abc)" return true wheras "(a)(b)(c)" or
  /// "abc" return false
  bool isDelimitedMonoBlock(
      bool useAngleBrace = false, // treat '<' and '>' as brackets
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true // treat """" as '"'
  ...
  /// Return the sub string with leading and trailing delimiters ( such as '('
  /// and ')' or '[' and ']' ) removed if the string isn't a delimited monoblock
  /// then the complete string is returned eg "((a)b(c))" returns "(a)b(c)"
  /// whereas "(a)b(c)" returns the identical "(a)b(c)"
  CSString stripBlockDelimiters(
      bool useAngleBrace = false, // treat '<' and '>' as brackets
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true // treat """" as '"'
  ...
  /// Append the individual "wordOrWords" elements in the string to the result
  /// vector retuns true on success
  bool splitWordOrWords(CVectorSString &result,
                        bool useSlashStringEscape = true,
                        bool useRepeatQuoteStringEscape = true) const;
  ...
  /// retuns true on success
  bool
  splitBySeparator(char separator, CVectorSString &result,
                   bool useAngleBrace = false, // treat '<' and '>' as brackets
                   bool useSlashStringEscape =
                       true, // treat '\' as escape char so "\"" == '"'
                   bool useRepeatQuoteStringEscape = true, // treat """" as '"'
                   bool skipBlankEntries =
                       false // dont add blank entries to the result vector
  ) const;

  /// Append the separator-separated elements in the string to the result vector
  /// retuns true on success
  bool splitByOneOfSeparators(
      const CSString &separators, CVectorSString &result,
      bool useAngleBrace = false, // treat '<' and '>' as brackets
      bool useSlashStringEscape =
          true, // treat '\' as escape char so "\"" == '"'
      bool useRepeatQuoteStringEscape = true, // treat """" as '"'
      bool retainSeparators =
          false, // have the separators turn up in the result vector
      bool skipBlankEntries =
          false // dont add blank entries to the result vector
  ) const;

  /// join an array of strings to form a single string (appends to the existing
  /// content of this string) if this string is not empty then a separator is
  /// added between this string and the following
  const CSString &join(const std::vector<CSString> &strings,
                       const CSString &separator = "");
  ...
  /// if a string is not already encapsulated in quotes then return quote() else
  /// return *this
  CSString
  ...
  /// if a string is not a single word and is not already encapsulated in quotes
  /// then return quote() else return *this
  CSString
  quoteIfNotAtomic(bool useSlashStringEscape =
                       true, // treat '\' as escape char so "\"" == '"'
                   bool useRepeatQuoteStringEscape = true // treat """" as '"'
  ) const;
  ...
  /// the isParameter paramter is true if the string is to be used in an XML
  /// parameter block
  ...
  /// verifies whether a string contains sub-strings that correspond to xml
  /// special character codes
  bool isEncodedXML() const;

  /// verifies whether a string contains any XML incompatible characters
  /// in this case the string can be converted to xml compatible format via
  /// encodeXML() the isParameter paramter is true if the string is to be used
  /// in an XML parameter block
  ...
  /// Find index at which a sub-string starts (case not sensitive) - if
  /// sub-string not found then returns string::npos
  std::string::size_type find(const char *toFind,
                              std::string::size_type startLocation = 0) const;

  /// Find index at which a sub-string starts (case NOT sensitive) - if
  /// sub-string not found then returns string::npos
  std::string::size_type findNS(const char *toFind,
                                std::string::size_type startLocation = 0) const;
  ...
  template <class T> CSString &operator<<(const T &value) {
  ...
  /// Case insensitive string compare (useful for use as map keys, see
  /// less<CSString> below)
  ...
  /// Write a string to a text file
  // if the file already exists and its content is identicall to our own then it
  // is not overwritten returns true on success (including the case where the
  // file exists and is not overwritten), false on failure
...
/*CVectorSString &operator = (CVectorSString &left, const
std::vector<std::string> &right)
{
    left = reinterpret_cast<CVectorSString&>(right);
}
CVectorSString &operator = (CVectorSString &left, const std::vector<CSString>
&right)
{
    left = reinterpret_cast<CVectorSString&>(right);
}
...
//
//	// simple ctors
//	CVectorSString() {} 	CVectorSString( const CVectorSString& v )	{
//operator=(v); }
//
//	// ctors for building from different vetor types
//	CVectorSString( const std::vector<CSString>& v ):
//std::vector<CSString>(v)			{} 	CVectorSString( const
//std::vector<std::string>& v ):
//std::vector<CSString>(*(std::vector<CSString>*)&v) {}
//
//	// ctor for extracting sub_section of another vector
//	CVectorSString( const const_iterator& first, const const_iterator& last
//):	std::vector<CSString>(first,last)	{}
// };
...
inline CSString::CSString(const char *s) { *(std::string *)this = s; }

inline CSString::CSString(const std::string &s) { *(std::string *)this = s; }

inline CSString::CSString(char c) { *(std::string *)this = c; }
...
inline CSString::CSString(const std::vector<NLMISC::CSString> &v,
                          const std::string &separator) {
...
inline char CSString::back() const { return (*this)[size() - 1]; }
...
inline CSString CSString::left(uint32 count) const { return substr(0, count); }
...
inline bool CSString::isPrintable(char c) {
  ...
  if (c == '$')
    return true;
  if ((uint8)c == 156)
    return true; // Sterling Pound char causing error in gcc 4.1.2
  if (c == '^')
    return true;
  if (c == '~')
    return true;
...
inline bool CSString::isQuoted(bool useAngleBrace, bool useSlashStringEscape,
                               bool useRepeatQuoteStringEscape) const {
  return (left(1) == "\"") && (right(1) == "\"") &&
         isDelimitedMonoBlock(useAngleBrace, useSlashStringEscape,
...
inline std::string::const_reference
CSString::operator[](std::string::size_type idx) const {
  static char zero = 0;
  if (idx >= size())
    return zero;
  return data()[idx];
...
// *** The following was commented out by Sadge because there were strange
...
//	 */
//	template <>
//	struct less<NLMISC::CSString> : public
//std::binary_function<NLMISC::CSString, NLMISC::CSString, bool>
//	{
//		bool operator()(const NLMISC::CSString& x, const
//NLMISC::CSString& y) const { return x.icompare(y); }
//	};
//} // std
//_STLP_END_NAMESPACE
// namespace std
 ...
 *   std::map<NLMISC::CSString, CMyDataClass,
 * NLMISC::CUnsensitiveSStringLessPred> MyMap; Caution: a map declared without
 * CUnsensitiveSStringLessPred will behave as a standard string map.
 *

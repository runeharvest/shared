...
private:
  ...
  void resamplePicture32(const NLMISC::CRGBA *pSrc, NLMISC::CRGBA *pDest,
                         sint32 nSrcWidth, sint32 nSrcHeight, sint32 nDestWidth,
                         sint32 nDestHeight);
   ...
   * \param nDestHeight height after resample
   */
  void resamplePicture8(const uint8 *pSrc, uint8 *pDest, sint32 nSrcWidth,
   ...
   * \param nDestHeight height after resample
   */
  void resamplePicture8Fast(const uint8 *pSrc, uint8 *pDest, sint32 nSrcWidth,
                            sint32 nSrcHeight, sint32 nDestWidth,
                            sint32 nDestHeight);
   ...
   */
  float getColorInterp(float x, float y, float xy00, float xy01, float xy10,
                       float xy11) const;
...
public:
   ...
   * Bitmap supported are DDS (DXTC1, DXTC1 with Alpha, DXTC3, DXTC5), PNG, JPEG
   * and uncompressed TGA (24 and 32 bits). \param IStream The stream must be in
   * reading mode. \param mipMapSkip if the file is a DDS with mipMap.
   * N=mipMapSkip mipmaps are skipped. \return image depth (24 or 32), or 0 if
   * load failed \throw ESeekFailed : seek has failed
   */
  uint8 load(NLMISC::IStream &f, uint mipMapSkip = 0);

  /**
   * Determinate the bitmap size from a bitmap(TGA or DDS) from an IStream. load
   * just header of the file. Bitmap supported are DDS (DXTC1, DXTC1 with Alpha,
   * DXTC3, DXTC5), PNG, JPEG and uncompressed TGA (24 and 32 bits). NB: at the
   * end, f is seeked to begin. \param IStream The stream must be in reading
   * mode. \param width the width of the image. 0 if fails. \param height the
   * height of the image. 0 if fails. \throw ESeekFailed : seek has failed
   */
   ...
   * \see loadSize()
   */
  static void loadSize(const std::string &path, uint32 &retWidth,
                       uint32 &retHeight);
  ...
  /** Gain ownership of this texture datas. As a result, the bitmap is reseted
   * (put in the same state than when ctor is called, e.g a single mipmap with
   * null size) The CObjectVector objects that contains the bitmap (one per
   * mipmap) datas are 'swapped' with those in the array  provided by the
   * caller. NB : The user must provide at least min(getMipMapCount(),
   * maxMipMapCount) entries in the array \param mipmapArray Array of mipmap
   * that receive the bitmap datas \param maxMipMapCount Max number of mipmap to
   * be copied in the destination array.
   */
  void unattachPixels(CObjectVector<uint8> *mipmapDestArray,
  ...
  /**
   * Return the format of pixels stored at the present time in the object
   * buffer. \return Pixel format (rgba luminance alpha alphaLuminance dxtc1
   * dxtc1Alpha dxtc3 dxtc5)
   */
  TType getPixelFormat() const { return PixelFormat; }
   ...
   * \return number of mipmaps. 0 if no image at all. 1 if no mipmaping (for the
   * base level).
   */
  uint32 getMipMapCount() const { return _MipMapCount; }

  // Compute the number of mipmap needed for that bitmap (independently from the
  // current number of mipmaps that have been set)
   ...
   * \param newType is the new type of the bitmap. If don_t_know, keep the same
   * pixel format that before. \param resetTo0 by default the vector are filled
   * by 0. set false to gain performances.
   */
   ...
   * This is not a crop. Pixels are lost after resize.
   *	No validity check is made. It is the user responsabitility fo setup
   *correct mipmap size.
   *
   * \param numMipMap id of the mipmap
   * \param nNewWidth width after resize
   * \param nNewHeight height after resize
   * \param resetTo0 by default the vector are filled by 0. set false to gain
   *performances.
   */
  ...
  /**  ADVANCED USE
   *	To use in conjunction with resizeMipMap. Setup the correct total number
   *of mipmap No validity check is made. It is the user responsabitility fo
   *setup correct mipmap count.
   */
   ...
   *
   * \param loadAsAlpha is true to load grayscale bitmaps as alpha. false to
   * load grayscale bitmaps as luminance. default value is true.
   */
   ...
   *
   * \return true if the bitmap loads grayscale bitmaps as alpha, false if it
   * loads grayscale bitmaps as luminance.
   */
  bool isGrayscaleAsAlpha() const { return _LoadGrayscaleAsAlpha; }
   ...
   * \return true if the params were corrects and if the blit occurs. In debug
   * build there's an assertion
   */
   ...
   * IMPORTANT : copy to self is not handled correctly
   */
  void blit(const CBitmap &src, sint srcX, sint srcY, sint srcWidth,
            sint srcHeight, sint destX, sint destY);

  /**
   * Get the color in the bitmap for the first mipmap
   * The mipmaps must be built. If not just return the bilinear at the given
   * point. NB: coordinates are clamped. \param tiled If false coordinate are
   * clamped, else the bitmap is considered to tile
   */
   ...
   * \param factor The blend factor. 0 means the result is equal to Bm0, 256
   * means the result is equal to Bm1 \param inputBitmapIsMutable when true,
   * bitmap can be converted in place when needed (no copy done)
   */
  void blend(CBitmap &Bm0, CBitmap &Bm1, uint16 factor,

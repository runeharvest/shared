...
public:
  ...
  /**
   ...
   @param create Specifies if the node should be created if it doesn't exist
   yet.
  ...
  /**
   ...
   @param positiveLeafNameFilter A vector of strings containing the names of the
   leaves we want to observe

   */
  void
  addBranchObserver(const char *branchName,
   ...
   */
  void
  addBranchObserver(CCDBNodeBranch *branch,
                    ICDBNode::IPropertyObserver *observer,
                    const std::vector<std::string> &positiveLeafNameFilter =
                        std::vector<std::string>());
  ...
  /**
   ...
   @param observer   The observer we want to add
   @param positiveLeafNameFilter An array of strings containing the names of the
   leaves we want to observe
   @param positiveLeafNameFilterSize The size of the array

   */
  void addBranchObserver(const char *branchName, const char *dbPathFromThisNode,
                         ICDBNode::IPropertyObserver &observer,
   ...
   */
  void addBranchObserver(CCDBNodeBranch *branch, const char *dbPathFromThisNode,
                         ICDBNode::IPropertyObserver &observer,
   ...
   */
  void removeBranchObserver(const char *branchName,
                            ICDBNode::IPropertyObserver *observer);
   ...
   */
  void removeBranchObserver(CCDBNodeBranch *branch,
                            ICDBNode::IPropertyObserver *observer);

  /**
   Removes an observer from a branch in the database.
   @param branchName The name of the branch we start from
   @param dbPathFromThisNode The path to the branch we want to observe from the
   starting branch
   @param observer The observer we want to remove
  ...
  /**
   Adds a branch observer call flush observer. ( These are notified after the
   branch observers are notified )
   @param observer The observer

   */
  void addFlushObserver(

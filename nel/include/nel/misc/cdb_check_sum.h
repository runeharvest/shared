 ...
 * these check sum can be used to ensure that linked properties have all been
 * modified \author Nicolas Brigand \author Nevrax France \date 2002
 */
class CCDBCheckSum {
public:
  /// constructor
  ...
  template <class T> void add(const T &el) {
    T value = el;
  ...
  template <class T> void addVector(const std::vector<T> &vect) {
    for (typename std::vector<T>::const_iterator it = vect.begin();
         it != vect.end(); it++)
      add(*it);
  }

  uint32 getSum() { return _Sum; }

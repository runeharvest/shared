 ...
 * \date 2001
 */
template <class T> class CValueSmootherTemplate {
public:
  /// Constructor
  explicit CValueSmootherTemplate(uint n = 16) { init(n); }
  ...
  void addValue(T dt) {
    ...
    // update the frame sum. NB: see init(), at start, array is full of 0. so it
    // works even for un-inited values.
  ...
  uint getNumFrame() const { return _NumFrame; }

  uint getCurrentFrame() const { return _CurFrame; }

  const std::vector<T> &getLastFrames() const { return _LastFrames; }
 ...
 * \date 2003
 */
template <> class CValueSmootherTemplate<bool> {
public:
  /// Constructor
  explicit CValueSmootherTemplate(uint n = 1) { init(n); }
  ...
  uint getCurrentFrame() const { return 0; }

  const std::vector<bool> &getLastFrames() const { return _LastFrames; }

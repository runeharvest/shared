...
/**
 ...
 They are used for saving bandwidth, because the local CDBs are updated with
 deltas, that identify the updatable top-level branch with this id. The
 CCDBBankHandler manages the mapping of banks to their names, unified (node)
...
public:
  ...
  */
  ...
  /**
   @brief   Returns the number of bits used to store the number of nodes that
   belong to this bank.
   @param   bank The banks whose id bits we need.
   @return  Returns the number of bits used to store the number of nodes that
   belong to this bank.
  */
  uint getFirstLevelIdBits(uint bank) const {
  ...
  */
  ...
  /**
   @brief   Maps the specified bank name to a unified (node) index and vica
   versa.
  ...
  */
  ...
  /**
   @brief   Loads the known bank names from an array ( the order decides the
   bank Id ).
  ...
  */
  ...
  /// Calculates the number of bits used to store the number of nodes that
  /// belong to the banks.
  ...
  */
  uint getServerToClientUIDMapping(uint bank, uint index) const {
...
private:
  /// Mapping from server database index to client database index (first-level
  /// nodes)

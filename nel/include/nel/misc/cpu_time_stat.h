 ...
 * Allows accurate timing measures for both cpu and process (at least at OS
 * timing accuracy) Call peekMeasures() once in a while (once a second, for
 * instance, to avoid to much load) then call each getCPU... and getProcess...
 * methods to get instant CPU or Process load.
 *
...
public:
   ...
   */
  static bool getCPUTicks(uint64 &user, uint64 &nice, uint64 &system,
                          uint64 &idle, uint64 &iowait);

  /// Get absolute ticks values for a specific pid
  static bool getPIDTicks(uint64 &utime, uint64 &stime, uint64 &cutime,
                          uint64 &cstime, uint pid);
  ...
  float getCPULoad(TMeasureType type = Instant) const {
    return _CPUUser.get(type) + _CPUSystem.get(type) + _CPUNice.get(type) +
           _CPUIOWait.get(type);
  }
   ...
   * Please note that quadriprocessor machines will show maximum of 0.25 per
   * process (0.5 for a biprocessor). Children of a process are processes that
   * have been launched from this process (that is, this process is marked as
   * parent of the child process, see ppid, fork() and/or exec()) Unused but by
   * the AES (all launched services are children of the AES)
   */
  float getProcessLoad(TMeasureType type = Instant) const {
    return _PIDUTime.get(type) + _PIDSTime.get(type);
  }

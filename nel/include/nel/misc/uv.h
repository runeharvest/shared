...
public:
  ...
  // bin operators.
  CUV operator+(const CUV &v) const { return CUV(U + v.U, V + v.V); }
  // binary -
  CUV operator-(const CUV &v) const { return CUV(U - v.U, V - v.V); }
  // unary -
  CUV operator-() const { return CUV(-U, -V); }
  // = operators.
  ...
  /// This operator is here just for map/set insertion (no meaning). comparison
  /// order is U,V.
...
inline CUV operator*(float f, const CUV &uv) { return CUV(uv.U * f, uv.V * f); }

inline CUV operator*(const CUV &uv, float f) { return f * uv; }
...
public:
  ...
  CUVW operator*(float f) const { return CUVW(U * f, V * f, W * f); }
  // = operators.

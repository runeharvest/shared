 ...
 * Note: Center/HalfSize set to private, to have same manipulation for CAABBox
 * and CAABBoxExt. \author Lionel Berenguier \author Nevrax France \date 2000
 */
class CAABBox {
protected:
...
public:
   ...
   * Warning!! By default, a bbox is the vector 0,0,0. So set the first vertex
   * with setCenter() or else the bbox will be the extension of v and (0,0,0)...
   */
  ...
  /// clip the segment by the bbox. return false if don't intersect. a and b are
  /// modified.
  ...
  /**
   * Compute the union of 2 bboxs, that is the aabbox that contains the 2
   * others. Should end up in NLMISC
   */
   ...
   *	NB: this methods suppose the intersection exist, and doesn't check it
   *(use intersect() to check). If !intersect, *this is still modified and the
   *result bbox is big shit.
   */
  void computeIntersection(const CAABBox &b1, const CAABBox &b2);

  /** Apply a matrix on an aabbox
   *  \return an aabbox, bigger or equal to parameter, after the matrix
   * multiplication
   */
...
// ***************************************************************************
/**
 * An Extended Axis Aligned Bouding Box.  Sphere Min/Max Radius is stored for
 * improved clip test. \author Lionel Berenguier \author Nevrax France \date
 * 2000
 */
...
protected:
  ...
  void updateRadius() {
    ...
    RadiusMin = NLMISC::minof((float)fabs(HalfSize.x), (float)fabs(HalfSize.y),
                              (float)fabs(HalfSize.z));
  }

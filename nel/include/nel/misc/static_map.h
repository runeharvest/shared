 ...
 * Use it not like a map : begin by adding all your values with
 * add()/del()/fromMap() and then call endAdd() that performs a slow sort on the
 * vector and then call find() to find the element you want. If you have not
 * called endAdd() its done in find(), but take care that endAdd() is slow.
...
public:
  ...
  CStaticMap(const CStaticMap<Key, Typ, Comp> &__x)
      : _Data(__x._Data), _DataSorted(__x._DataSorted),
  ...
  iterator find(const key_type &__x) {
    ...
    iterator it = lower_bound((iterator)_Data.begin(), (iterator)_Data.end(),
                              __v, value_comp());
  ...
  const_iterator find(const key_type &__x) const {
    endAdd();
    value_type __v(__x, Typ());
    iterator it = lower_bound((const_iterator)_Data.begin(),
                              (const_iterator)_Data.end(), __v, value_comp());
    if ((it != end()) && (!value_comp()(*it, __v) && !value_comp()(__v, *it)))
      return it;
    else
      return end();

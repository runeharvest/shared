...
public:
  /**
   *	This is a function pointer that points on the best memcpy function
   *available depending of the OS and proc. In the best case, it will use
   *memcpySSE(), and in worst case, it'll use the libc memcpy() Simply use it
   *this way: CFastMem::memcpy(dst, src, size);
   */
  ...
  /**
   *	Fast memcpy using SSE instructions: prefetchnta and movntq. Can be
   *called only if SSE and MMX is supported NB: Copy per block of 4K through L1
   *cache Result is typically 420 Mo/s instead of 150 Mo/s.
   */
  static void *memcpySSE(void *dst, const void *src, size_t nbytes);

  /**
   *	Fast precaching of memory in L1 cache using MMX/SSE instructions: movq
   *and prefetchnta Result is typically 880 Mo/s (surely slower because of
   *overhead). nbytes should not override 4K
   */

...
public:
   ...
   * \warning Supports only Intel architectures for now. Returns 0 if not
   * implemented.
   */
  ...
  /** Helps to know whether the processor has streaming SIMD instructions (the
   * OS must supports it) This is initialized at started, so its fast (always
   * false on non 0x86 architecture ...)
   */
  ...
  /** Returns the space left on the hard drive that contains the filename in
   * bytes
   */

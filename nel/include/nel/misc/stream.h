...
#define NLMISC_BSWAP16(src)                                                    \
  (src) = (((src) >> 8) & 0xFF) | (((src) & 0xFF) << 8)
#if defined(NL_OS_WINDOWS) && !defined(NL_NO_ASM)
#define NLMISC_BSWAP32(src)                                                    \
  _asm mov eax, (src) _asm bswap eax _asm mov(src), eax
#else
#define NLMISC_BSWAP32(src)                                                    \
  (src) = (((src) >> 24) & 0xFF) | ((((src) >> 16) & 0xFF) << 8) |             \
          ((((src) >> 8) & 0xFF) << 16) | (((src) & 0xFF) << 24)
#endif
#define NLMISC_BSWAP64(src)                                                    \
  (src) = (((src) >> 56) & 0xFF) | ((((src) >> 48) & 0xFF) << 8) |             \
          ((((src) >> 40) & 0xFF) << 16) | ((((src) >> 32) & 0xFF) << 24) |    \
          ((((src) >> 24) & 0xFF) << 32) | ((((src) >> 16) & 0xFF) << 40) |    \
          ((((src) >> 8) & 0xFF) << 48) | (((src) & 0xFF) << 56)
...
/// This exception is raised when someone tries to serialize in more than there
/// is.
 ...
 * This is the base interface for stream objects. Differents kind of streams may
 be implemented,
 ...
 *
 * The deriver must:
 * - construct object specifying his type, see IStream(). A stream may be setup
 Input or Output at construction, but cannot
 ...
 * \b Client \b Use:
 *
 * An object which can be serialized, must provide a "void serial(IStream &)"
 method. In this method, he can use
 * any of the IStream method to help himself like:
 * - serial() with a base type (uint32, string, char...), or even with an object
 which provide "void serial(IStream &)"
 * - template serial(T0&, T1&, ...) to serialize multiple object/variables in
 one call (up to 6).
 * - serialCont() to serialize containers.
 * - serialVersion() to check/store a version number of his class.
 * - serialPtr() to use the ptr support of IStream (see serialPtr() for more
 information)
 ...
 * \code
 ...
 public:
    float	x;
    uint32	y;
    Class1	a;		// this class must provide a serial() method
 too... Base	*c,*d;	// Base must derive from IStreamable vector<Class2>
 tab;
 ...
 \endcode
 *
 * NB: \b YOU \b CANNOT use serial with a int / uint / sint type, since those
 type have unspecified length.
 * \author Lionel Berenguier
 * \author Vianney Lecroart
 * \author Nevrax France
 * \date 2000
...
public:
  /**
   * Set the behavior of IStream regarding input stream that are older/newer
   * than the class. If throwOnOlder==true, IStream throws a EOlderStream when
   * needed. If throwOnNewer==true, IStream throws a ENewerStream when needed.
   *
  ...
  /**
   * Get the behavior of IStream regarding input stream that are older/newer
   * than the class. \see serialVersion() setVersionException()
   */
...
public:
   ...
   * \param obj any object providing a "void serial(IStream&)" method. The
   * object doesn't have to derive from IStreamable.
   *
   * the VC++ error "error C2228: left of '.serial' must have class/struct/union
   * type" means you don't provide a serial() method to your object. Or you may
   * have use serial with a int / uint / sint type. REMEMBER YOU CANNOT do this,
   * since those type have unspecified length.
   */
  template <class T> void serial(T &obj) { obj.serial(*this); }

  // an utility template to unconst a type
  template <class T> static T &unconst(const T &t) {
...
// helper macro to serialize boolean encoded as 'bool foo : 1' (they can't be
// referenced)
#define nlSerialBitBool(_stream, _boolean)                                     \
  ...
  template <class T> void serialShortEnum(T &em) {
    uint8 i;
    if (isReading()) {
      serial(i);
      em = (T)i;
   ...
   * Unlike other serial method, The reading bitfield is returned!! If
   * !this->isReading(), bf is returned.
   *
   * MUST use it simply like this:   a= serialBitFieldX(a);		// where
   * X== 8, 16 or 32.
   *
   * NB: Performance warning: the data is stored as an uint8, uint16 or uint32,
   * according to the method you use.
   */
  //@{
   ...
   * map<>, multimap<> Support up to sint32 length containers. \see
  ...
  template <class T> void serialCont(std::set<T> &cont) { serialSTLCont(cont); }
   ...
   */
  template <class K, class T> void serialPtrCont(std::map<K, T> &cont) {
    serialPtrMap(cont);
  }
  ...
  /** \name standard STL containers serialisation. Elements must be pointers on
   * a base type (uint...) or on a object providing "void serial(IStream&)"
   * method. Known Supported containers: vector<>, list<>, deque<>, set<>,
   * multiset<> Support up to sint32 length containers. \see serialCont()
   * serialContPolyPtr()
   */
  template <class T, class Allocator>
  void serialContPtr(std::vector<T, Allocator> &cont) {
    serialVectorPtr(cont);
  }
  template <class T> void serialContPtr(std::list<T> &cont) {
    serialSTLContPtr(cont);
  }
  ...
  /** \name standard STL containers serialisation. Elements must be pointers on
   * a IStreamable object. Known Supported containers: vector<>, list<>,
   * deque<>, set<>, multiset<> Support up to sint32 length containers. \see
   * serialCont() serialContPtr()
   */
  template <class T, class Allocator>
  void serialContPolyPtr(std::vector<T, Allocator> &cont) {
    serialVectorPolyPtr(cont);
  }
  template <class T> void serialContPolyPtr(std::list<T> &cont) {
    serialSTLContPolyPtr(cont);
  }
  ...
  template <class K, class T> void serialContPolyPtr(std::map<K, T> &cont) {
    serialMapPolyPtr(cont);
  }

  /**
   * Serialize Non Polymorphic Objet Ptr.
   * Works with NULL pointers. If the same object is found mutliple time in the
   * stream, ONLY ONE instance is written! NB: The ptr is serialised as a uint64
   * (64 bit compliant). \param ptr a pointer on a base type or an object. \see
   * resetPtrTable()
   */
   ...
   * Works with NULL pointers. If the same object is found mutliple time in the
   * stream, ONLY ONE instance is written! NB: The ptr is serialised as a uint64
   * (64 bit compliant). \param ptr a pointer on a IStreamable object. \see
   * resetPtrTable()
   */
   ...
   * Then he can use the streamVersion returned to see how he should serialise
   * himself.
   *
   ...
   * \return the version of the stream. If the stream is an Output stream,
   * currentVersion is returned. \see setVersionException()
   * getVersionException()
   */
   ...
   * An object can stream a check value to check integrity or format of filed or
   * streamed data. Just call serial check with a const value. Write will serial
   * the value. Read will check the value is the same. If it is not, it will
   * throw EInvalidDataStream exception.
   *
   * NB: The type of the value must implement an operator == and must be
   * serializable. \param value the value used to the check. \see
   * EInvalidDataStream
   */
   ...
   *
   * NB: If the stream doesn't support the seek fonctionnality, it throw
   * ESeekNotSupported. Default implementation: { throw ESeekNotSupported; }
  ...
  /** \name XML user interface
    *
    * Those functions are used to add information in your stream to structure it
    like
    ...
    *
    ...
    \endcode
    *
    * Node header serials are the serialisations done between xmlPushBegin() and
    xmlPushEnd() call. There is some restrictions on them:
    * Node header serials are only available for basic types (numbers and
    strings).
    * xmlSetAttrib() must be called before node header serial.
    *
    * Note that XML documents only have ONE root node, so all serialisation must
    be done between a xmlPush() and a xmlPop() call.
    *
    * When a xml input stream will try to open a node, it will scan all currrent
    child nodes to find the good one.
   ...
   */
  template <class T> void xmlSerial(T &value0, const std::string &nodeName) {
    // Open the node
    xmlPush(nodeName);
  ...
  template <class T>
  void xmlSerial(T &value0, T &value1, T &value2, T &value3,
  ...
  /**
   * xmlPush() open recurcively a new node. You must call xmlPop to close this
   * node.
   *
   * \name is the name of the node to open
   * \return true if you can open the node, false if the stream is between a
   * xmlPushBegin() and a xmlPushEnd() call.
   */
  ...
  /**
   * xmlPushBegin() open recurcively a new node and open its header. You must
   * call xmlPushEnd() to close the header and xmlPop() to close this node.
   *
   * \name is the name of the node to open
   * \return true if you can open the node header, false if the stream is
   * between a xmlPushBegin() and a xmlPushEnd() call.
   */
   ...
   *
   * \return true if you can close the node header, false if no node header have
   * been opened with xmlPushBegin().
   */
  bool xmlPushEnd() {
    // XML Mode ?
    if (_XML) {
   ...
   *
   * \return true if you can close the node, false if the node can't be closed
   * (its header is still opened) or if there is no node to close.
   */
  bool xmlPop() {
    // XML Mode ?
    if (_XML) {
   ...
   * \return true if the attribute name have been set, false if the node header
   * is not open (the call is not between xmlPushBegin and xmlPushEnd)
   */
...
protected:
  ...
  virtual bool xmlSetAttribInternal(const std::string & /* name */) {
    return true;
  };
   ...
   * If Derived stream provide reset()-like methods, they must call this method
   * in their reset() methods. For example, CFile::close() must call it, so it
   * will work correctly with next serialPtr()
   */
  void resetPtrTable();

  /**
   * Change, in live, the state of the inputStream. This could be useful in
   * certain case. The deriver which would want to do such a thing must call
   * this method, and implement his own behavior. In certain case, it should
   * call resetPtrTable() if he want to reset the stream ptr info (maybe
   * always)...
   */
  void setInOut(bool inputStream);

  /** Get the size for this stream. return 0 by default. Only implemented for
   *input stream that know their size. Used internally to detect OverFlow with
   *vector<> for instance
   */
  virtual uint getDbgStreamSize() const { return 0; }

  /**
   * Elementarily check at least n bytes can be serialized from this stream (or
  ...
  void checkStreamSize(uint numBytes) const {
    ...
    if (ssize > 0 && ssize < numBytes)
      throw EStreamOverflow(
          "stream does not contain at least %u bytes for check", numBytes);
  }

public:
  //@{
  /** Method to be specified by the Deriver.
   * \warning Do not call these methods from outside, unless you really know
   * what you are doing. Using them instead of serial() can lead to
   * communication problems between different platforms !
   */
  virtual void serialBuffer(uint8 *buf, uint len) = 0;
  virtual void serialBit(bool &bit) = 0;
  //@}

  /// This method first serializes the size of the buffer and after the buffer
  /// itself, it enables the possibility to serial with a serialCont() on the
  /// other side.
...
private:
  ...
  // Ptr registry. We store 64 bit Id, to be compatible with future 64+ bits
  // pointers.
...
private:
  /**
   * standard STL containers serialization. Don't work with map<> and
   * multimap<>. Support up to sint32 length containers. serialize just len
  ...
  /**
   * standard STL containers serialisation. Don't work with map<> and
   *multimap<>. Support up to sint32 length containers.
   *
   * the object T must provide:
   *	\li typedef iterator;		(providing operator++() and operator*())
   *	\li typedef value_type;		(a base type (uint...), or an object
   *providing "void serial(IStream&)" method.) \li void clear(); \li size_type
   *size() const; \li iterator begin(); \li iterator end(); \li iterator
...
private:
  /**
   * standard STL containers serialisation. Don't work with map<> and
   * multimap<>.  Ptr version. Support up to sint32 length containers. serialize
   * just len  element of the container.
   */
  ...
  /**
   * standard STL containers serialisation. Don't work with map<> and
   * multimap<>.  Ptr version. Support up to sint32 length containers.
   */
...
private:
  /**
   * standard STL containers serialisation. Don't work with map<> and
   * multimap<>. PolyPtr version Support up to sint32 length containers.
   * serialize just len  element of the container.
   */
   ...
   * Support up to sint32 length containers. serialize just len  element of the
   * container.
   */
  template <class T> void serialMapContLenPolyPtr(T &cont, sint32 len) {
   ...
   * Support up to sint32 length containers.
   */
  template <class K, class T> void serialMapPolyPtr(std::map<K, T> &cont) {
    // Open a node header
    xmlPushBegin("MAP");
    // Attrib size
    xmlSetAttrib("size");
...
private:
  ...
  /**
   * STL map<>
   * Support up to sint32 length containers. Container must contain NeL smart
   *pointers.
   *
   * the object T must provide:
   *	\li typedef iterator;		(providing operator++() and operator*())
   ...
   * \param cont a STL map<> container.
   */
  template <class T> void serialPtrMap(T &cont) {
...
// ======================================================================================================
/**
 * An Object Streamable interface. Any polymorphic class which want to use
 * serial() in a polymorphic way, must derive from this interface. \author
 * Lionel Berenguier \author Vianney Lecroart \author Nevrax France \date 2000
 */

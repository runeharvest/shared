...
public:
  ...
  void list(const std::string &sBigFileName,
            std::vector<std::string> &vAllFiles);

  // Remove all big files added
  void removeAll();

  /** Signal that the current thread has ended : all file handles "permanently"
   * allocated for that thread can be released then, preventing them from
   * accumulating.
   */
  void currentThreadFinished();

  // Used by CIFile to get information about the files within the big file
  FILE *getFile(const std::string &sFileName, uint32 &rFileSize,
                uint32 &rBigFileOffset, bool &rCacheFileOnOpen,
                bool &rAlwaysOpened);

  // Used by Sound to get information for async loading of mp3 in .bnp. Return
  // false if file not found in registered bnps
  bool getFileInfo(const std::string &sFileName, uint32 &rFileSize,
                   uint32 &rBigFileOffset);

  // Used for CPath only for the moment !
  char *getFileNamePtr(const std::string &sFileName,
                       const std::string &sBigFileName);

  typedef CCallback<bool /* continue */, const std::string & /* filename */,
                    uint32 /* currentSize */, uint32 /* totalSize */>
      TUnpackProgressCallback;

  // Unpack all files in sBigFileName to sDestDir and send progress
  // notifications to optional callback
  static bool unpack(const std::string &sBigFileName,
                     const std::string &sDestDir,
                     TUnpackProgressCallback *callback = NULL);

  // A BNPFile header (filename is a char* pointing on FileNames and is always
  // lowercase)
  struct BNPFile {
  ...
  struct BNP {
    ...
    // FileName of the BNP. important to open it in getFile() (for other threads
    ...
    // Since many seek may be done on a FILE*, each thread should have its own
    // FILE opened.
    uint32 ThreadFileId;
    ...
    // Append BNP header to the big file BigFileName (to use after appendFile
    // calls)
    bool appendHeader();
    ...
    // Unpack BigFileName to sDestDir and send progress notifications to
    // optional callback
    bool unpack(const std::string &sDestDir,
                TUnpackProgressCallback *callback = NULL);
  };
...
private:
  ...
  bool getFileInternal(const std::string &sFileName, BNP *&zeBnp,
                       BNPFile *&zeBnpFile);
};

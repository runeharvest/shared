...
/**
 * Add a variable that can be modified in realtime. The variable must be global.
 If you must access the variable with
 * function, use NLMISC_DYNVARIABLE
 *
 * Example:
 * \code
    // I want to look and change the variable 'foobar' in realtime, so, first i
 create it: uint8 foobar;
    // and then, I add it
    NLMISC_VARIABLE(uint8, FooBar, "this is a dummy variable");
 * \endcode
 *
 * Please use the same casing than for the variable (first letter of each word
 in upper case)
...
#define NLMISC_VARIABLE(__type, __var, __help)                                 \
  NLMISC_CATEGORISED_VARIABLE(variables, __type, __var, __help)
#define NLMISC_CATEGORISED_VARIABLE(__category, __type, __var, __help)         \
  NLMISC::CVariablePtr<__type> __var##Instance(                                \
      #__category, #__var, __help " (" #__type ")", &__var)

/**
 * Add a variable that can be modified in realtime. The code profide the way to
 access to the variable in the read
 ...
 *	Useful when you want to declare variable that need to access private
 *data to act on or display internal state of the class
 */

#define NLMISC_DYNVARIABLE_FRIEND(__name)                                      \
  NLMISC_CATEGORISED_DYNVARIABLE_FRIEND(variables, __name)
#define NLMISC_CATEGORISED_DYNVARIABLE_FRIEND(__category, __name)              \
  friend class __name##Class
...
public:
  IVariable(const char *categoryName, const char *commandName,
            const char *commandHelp, const char *commandArgs = "[<value>]",
            bool useConfigFile = false, void (*cc)(IVariable &var) = NULL)
      : ICommand(categoryName, commandName, commandHelp, commandArgs),
        _UseConfigFile(useConfigFile), ChangeCallback(cc) {
    Type = Variable;
  }
  ...
  virtual bool execute(const std::string & /* rawCommandString */,
    ...
    if (quiet) {
      ...
      log.displayNL("Variable %s = %s", _CommandName.c_str(),
                    toString(human).c_str());
    }
    return true;
  }
...
template <class T> class CVariablePtr : public IVariable {
public:
  CVariablePtr(const char *categoryName, const char *commandName,
               const char *commandHelp, T *valueptr, bool useConfigFile = false,
               void (*cc)(IVariable &var) = NULL)
      : IVariable(categoryName, commandName, commandHelp, "[<value>]",
                  useConfigFile, cc),
        _ValuePtr(valueptr) {}
...
template <class T> class CVariable : public IVariable {
public:
  CVariable(const char *categoryName, const char *commandName,
            const char *commandHelp, const T &defaultValue,
            uint nbMeanValue = 0, bool useConfigFile = false,
            void (*cc)(IVariable &var) = NULL,
            bool executeCallbackForDefaultValue = false)
      : IVariable(categoryName, commandName, commandHelp,
                  "[<value>|stat|mean|min|max]", useConfigFile, cc),
        _Mean(nbMeanValue), _First(true) {
  ...
  void set(const T &val, bool executeCallback = true) {
    ...
    if (ChangeCallback && executeCallback)
      ChangeCallback(*this);
  }
...
public:
  CVariable(const char *categoryName, const char *commandName,
            const char *commandHelp, const std::string &defaultValue,
            uint /* nbMeanValue */ = 0, bool useConfigFile = false,
            void (*cc)(IVariable & /* var */) = NULL,
            bool executeCallbackForDefaultValue = false)
      : IVariable(categoryName, commandName, commandHelp, "[<value>]",
...
/// This class can provide a callback called when the value of a variable has
/// been changed

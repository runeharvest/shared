...
public:
  ...
  bool isIn(sint x, sint y) const {
    return x >= 0 && y >= 0 && x < (sint)_Width && y < (sint)_Height;
  }
   ...
   * Example : move(1, 0) will move the array of one column to the left. The
   * latest column is lost. The first column remains unchanged
   */
  void move(sint offsetX, sint offsetY);
  // Move a part of the array. Values are clamped as necessary
  void moveSubArray(sint dstX, sint dstY, sint srcX, sint srcY, sint width,
                    sint height);
  // Blit content from another array
  void blit(const CArray2D<T> &src, sint srcX, sint srcY, sint srcWidth,
            sint srcHeight, sint dstX, sint dstY);
   ...
   * displaced by the given offset (by a call to move for example). Example:
   * getUpdateRects(0, 1, result) will result the first row as a result
   */
  void getUpdateRects(sint moveOffsetX, sint moveOffsetY,
                      std::vector<NLMISC::CRect> &rectsToUpdate);
  // See which parts of array will be discarded if the array is displaced by the
  // given offset
  void getDiscardRects(sint moveOffsetX, sint moveOffsetY,
                       std::vector<NLMISC::CRect> &discardedRects);
  //
...
//*********************************************************************************
template <class T> void CArray2D<T>::clear() {
  _Array.clear();
  _Width = _Height = 0;
}

//*********************************************************************************
template <class T> void CArray2D<T>::serial(NLMISC::IStream &f) {
  f.serialCont(_Array);
  uint32 width = _Width;
...
void CArray2D<T>::getUpdateRects(sint moveOffsetX, sint moveOffsetY,
  ...
  if (moveOffsetX < 0) // moved right ?
  {
    ...
    if (height > 0) {
      ...
      rectsToUpdate.push_back(NLMISC::CRect((sint32)(_Width - width),
                                            (sint32)(std::max(-moveOffsetY, 0)),
  ...
  } else if (moveOffsetX > 0) // moved left ?
  {
    ...
    if (height > 0) {
      // complete column on the right
      rectsToUpdate.push_back(NLMISC::CRect(
          0, (sint32)std::max(-moveOffsetY, 0), (uint32)width, (uint32)height));
#ifdef NL_DEBUG
      checkRect(rectsToUpdate.back());
#endif
    }
...
//*********************************************************************************
template <class T>
void CArray2D<T>::getDiscardRects(sint moveOffsetX, sint moveOffsetY,
                                  std::vector<NLMISC::CRect> &discardedRects) {
...
//*********************************************************************************
template <class T>
void CArray2D<T>::moveSubArray(sint dstX, sint dstY, sint srcX, sint srcY,
                               sint width, sint height) {
  if (srcX >= (sint)getWidth())
    return;
  if (srcY >= (sint)getHeight())
    return;
  if (dstX >= (sint)getWidth())
    return;
  if (dstY >= (sint)getHeight())
    return;
  if (srcX < 0) {
    width += srcX;
    if (width <= 0)
...
//*********************************************************************************
template <class T>
void CArray2D<T>::blit(const CArray2D<T> &src, sint srcX, sint srcY,
                       sint srcWidth, sint srcHeight, sint dstX, sint dstY) {
  if (&src == this) {
...
//*********************************************************************************
template <class T> void CArray2D<T>::move(sint offsetX, sint offsetY) {
  moveSubArray(offsetX, offsetY, 0, 0, _Width, _Height);
}

//*********************************************************************************
template <class T> void CArray2D<T>::init(uint width, uint height) {

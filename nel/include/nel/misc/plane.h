 ...
 *
 * A "front segment" or a "front polygon" have all their vertices in front of
 * the plane. \author Lionel Berenguier \author Nevrax France \date 2000
 */
class CPlane {
public:
  float a, b, c, d;
...
public:
   ...
   * Since the normal of the plane may not be normalized (if setuped without
   * make()), the returned normal may NOT be normalized.
   */
   ...
   * Since the plane normal may not be normalized, distance() compute the
   * distance with the normalized normal of plane. If you are sure that your
   * plane has a normalized normal, it is much faster to do a \c fabs(p*v).
   */
   ...
   * The "back segment" is written in (p1,p2). If segment is entirely "front",
   * (p1,p2) is not modified. \return false if segment entirely front, or true.
   * \sa clipSegmentFront()
   */
  bool clipSegmentBack(CVector &p0, CVector &p1) const;
  /**
   * Clip a segment onto a plane.
   * The "front segment" is written in (p1,p2). If segment is entirely "back",
   * (p1,p2) is not modified. \return false if segment entirely back, or true.
   * \sa clipSegmentBack()
   */
   ...
   * \return number of vertices of out. 0 is returned if In polygon entirely
   * front, or if nIn<=2.
   */
  sint clipPolygonBack(CVector in[], CVector out[], sint nIn) const;
  // Clip a polygon with uvs by a plane
  sint clipPolygonBack(const CVector in[], const CUV inUV[], CVector out[],
                       CUV outUV[], sint nIn) const;
  ...
  //@{
  /// get the inverted version of this plane (same position, but inverted
  /// normal)
  ...
  /// \name Misc
  //@{
  void serial(IStream &f) { f.serial(a, b, c, d); }

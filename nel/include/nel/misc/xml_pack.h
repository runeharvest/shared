...
public:
  ...
  void list(const std::string &xmlPackFileName,
...
private:
  ...
  /** Try to match the specified text at current position. Return false if no
   *match return true and advance the it iterator if match is found.
   */
  bool matchString(std::string::iterator &it, std::string::iterator end,
                   const char *text);
  /// Advance up to the beginning of the next line, incrementing the in/out
  /// param lineCount
  void skipLine(std::string::iterator &it, std::string::iterator end,
                uint32 &lineCount);
  ///@}

...
/**
 * Interface to manage a database node, can contain a unique property or a set
 * of property \author Stephane Coutelas \author Nevrax France \date 2002
 */
...
public:
   ...
   */
  virtual void init(xmlNodePtr node, IProgressCallback &progressCallBack,
                    bool mapBanks = false,
                    CCDBBankHandler *bankHandler = NULL) = 0;
   ...
   * \param gc the server gameCycle of this update. Any outdated update are
   * aborted \param f : the stream.
   */
   ...
   */
  virtual void setParent(CCDBNodeBranch * /* parent */) {
    nlassertex(0, ("setParent() not overloaded for given node type!"));
  }
  ...
  virtual CCDBNodeBranch *getParent() {
    nlassertex(0, ("getParent() not overloaded for given node type!"));
    return NULL;
  }
  ...
  /// Set the atomic branch flag (when all the modified nodes of a branch should
  /// be tranmitted at the same time)
  ...
  static TStringId getStringId(const std::string &nodeName) {
    if (_DBSM == NULL)
      _DBSM = CStringMapper::createLocalMapper();
...
protected:
  ...
  // utility to build full name efficiently (without reallocating the string at
  // each parent level)
  void _buildFullName(CSString &fullName);

  /// Atomic flag: is the branch an atomic group, or is the leaf a member of an
  /// atomic group
  bool _AtomicFlag : 1;

...
public:
  ...
  inline bool buildSheetId(const char *sheetName) {
    return buildSheetId(sheetName, strlen(sheetName));
  }
  inline bool buildSheetId(const std::string &sheetName) {
    return buildSheetId(sheetName.c_str(), sheetName.size());
  }
   ...
   *	Operator==
   */
  inline bool operator==(const CSheetId &sheetRef) const {
    return (_Id.Id == sheetRef._Id.Id);
  }
   ...
   * - if 'ifNotFoundUseNumericId==false' the returned string is "<Sheet %d not
   * found in sheet_id.bin>" with the id in %d
   * - if 'ifNotFoundUseNumericId==tue'   the returned string is "#%u" with the
   * id in %u
   */
   ...
   *  This operation is non-destructive, the new entries are appended to the
   *result vector note: fileExtension *not* include the '.' eg "bla" and *not*
   *".bla"
   **/
...
private:
  ...
  class CCharComp {
  public:
    bool operator()(CChar x, CChar y) const { return strcmp(x.Ptr, y.Ptr) < 0; }
  };

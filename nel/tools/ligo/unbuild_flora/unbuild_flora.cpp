...
(Reference: prim_export, main.cpp)
...
std::string s_ReferenceDir; /* R:\pipeline\export\continents\r2_desert\zone_lighted_ig_land
                             */
std::string
    s_PrimitiveFile; /* R:\graphics\primitive\r2_desert\r2_desert.primitive */

/*

Debug arguments:

"R:\leveldesign\DFN" "R:\leveldesign\game_elem\plant\ecosystem"
"R:\reference\2008_july\data\r2_desert2"
"R:\pipeline\export\continents\r2_desert\zone_lighted_ig_land"
"R:\graphics\primitive\r2_desert\r2_desert.primitive" "R:\leveldesign\DFN"
"R:\leveldesign\game_elem\plant\ecosystem"
"R:\reference\2008_july\data\r2_forest2"
"R:\pipeline\export\continents\r2_forest\zone_lighted_ig_land"
"R:\graphics\primitive\r2_forest\r2_forest.primitive" "R:\leveldesign\DFN"
"R:\leveldesign\game_elem\plant\ecosystem"
"R:\reference\2008_july\data\r2_jungle2"
"R:\pipeline\export\continents\r2_jungle\zone_lighted_ig_land"
"R:\graphics\primitive\r2_jungle\r2_jungle.primitive" "R:\leveldesign\DFN"
"R:\leveldesign\game_elem\plant\ecosystem"
"R:\reference\2008_july\data\r2_lakes2"
"R:\pipeline\export\continents\r2_lakes\zone_lighted_ig_land"
"R:\graphics\primitive\r2_lakes\r2_lakes.primitive" "R:\leveldesign\DFN"
"R:\leveldesign\game_elem\plant\ecosystem"
"R:\reference\2008_july\data\r2_roots2"
"R:\pipeline\export\continents\r2_roots\zone_lighted_ig_land"
"R:\graphics\primitive\r2_roots\r2_roots.primitive"
...
struct CPoint {
  ...
  CVector Pos; /* Position, height not necessarily specified (X="26218.738281"
                  Y="-1092.078979" Z="0.000000") */
  ...
  float Radius;     /* Bounding radius (calculated from plant sheet and scale)
                       (0.450252) */
};
...
/*
instance fy_s2_savantree_c.shape : x = 23031.2, y = -1269.2, z = 75.8, sx = 0.5,
sy = 0.5, sz = 0.5 instance fy_s2_savantree_c.shape : x = 22906.6, y = -1148.1,
z = 79.6, sx = 0.6, sy = 0.6, sz = 0.6
*/
...
bool loadLeveldesign() {
  ...
  for (std::vector<std::string>::iterator it(plants.begin()), end(plants.end());
       it != end; ++it) {
    printf("%s\n", nlUtf8ToMbcs(*it));
    ...
    if (plant.Shape.empty()) {
      if (!form->getRootNode().getValueByName(plant.Shape,
                                              "3D.SpringFX.FXName"))
        continue;
    }
    if (plant.Shape.empty())
...
bool loadInstances() {
  ...
  for (std::vector<std::string>::iterator it(igs.begin()), end(igs.end());
       it != end; ++it) {
    ...
    for (ptrdiff_t i = 0; i < (ptrdiff_t)ig._InstancesInfos.size(); ++i) {
      ...
      std::map<std::string, CPlant>::iterator formIt =
          s_ShapeToForm.find(instance.Shape);
      if (formIt != s_ShapeToForm.end()) {
        instance.Form = formIt->second.Form;
        instance.Name = CFile::getFilenameWithoutExtension(instance.Form) +
                        nlstr("_") + zoneLwr + nlstr("_") + toString(i);
        instance.Radius = instance.Scale * formIt->second.Radius;
        printf(" = %f, %f, %f, %f, %f, '%s', '%s', %f\n", instance.Pos.x,
               instance.Pos.y, instance.Pos.z, instance.Angle, instance.Scale,
               nlUtf8ToMbcs(instance.Form), nlUtf8ToMbcs(instance.Name),
               instance.Radius);
        instance.Plant = true;
      } else {
...
bool eraseReference() {
  ...
  for (std::vector<std::string>::iterator it(igs.begin()), end(igs.end());
    ...
    for (ptrdiff_t i = 0; i < (ptrdiff_t)ig._InstancesInfos.size(); ++i) {
      ...
      printf("%s\n", nlUtf8ToMbcs(shape));
      bool erased = false;
      for (std::list<CPoint>::iterator it(s_Instances.begin()),
           end(s_Instances.end());
           it != end; ++it) {
        const CPoint &instance = *it;
        if (instance.Pos.x == info.Pos.x && instance.Pos.y == info.Pos.y &&
...
bool writeFlora() {
  ...
  fo << "        <NAME>name</NAME>\n";
  fo << "        <STRING>"
     << CFile::getFilenameWithoutExtension(s_PrimitiveFile)
     << "_flora</STRING>\n";
  fo << "      </PROPERTY>\n";
  for (std::list<CPoint>::iterator it(s_Instances.begin()),
       end(s_Instances.end());
    ...
    fo << "        <PT X=\"" << instance.Pos.x << "\" Y=\"" << instance.Pos.y
       << "\" Z=\"0.000000\"/>\n";
    ...
    fo << "          <NAME>form</NAME>\n";
    fo << "          <STRING>"
       << CFile::getFilenameWithoutExtension(instance.Form) << "</STRING>\n";
    fo << "        </PROPERTY>\n";
    fo << "        <PROPERTY TYPE=\"string\">\n";
    fo << "          <NAME>layer</NAME>\n";
    fo << "          <STRING>0</STRING>\n";
...
bool unbuildFlora() {
  ...
  return loadLeveldesign() && loadInstances() && eraseReference() &&
         eraseNonPlants() && writeFlora();
}

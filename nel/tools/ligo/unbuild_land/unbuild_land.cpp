...
bool saveLand() {
  try {
    ...
    if (fileOut.open(s_LandFileOut, false, true, false)) {
      ...
      nlwarning("Can't open the land file for writing: %s",
                s_LandFileOut.c_str());
      return false;
    }
...
float ratePoints(const std::vector<CVector> &zone,
                 const std::vector<CVector> &ref, float xx, float xy, float yx,
                 float yy) {
  // Rudimentary distance rating of vertices (not very reliable, but good
  // enough!)
  ...
  for (ptrdiff_t i = 0; i < zone.size(); ++i) {
    if (ref.size()) {
      int lowj = 0;
      float lowv = (CVector(ref[0].x * xx + ref[0].y * xy,
                            ref[0].x * yx + ref[0].y * yy, ref[0].z) -
                    zone[i])
                       .sqrnorm();
      for (ptrdiff_t j = 1; j < ref.size(); ++j) {
        float v = (CVector(ref[j].x * xx + ref[j].y * xy,
                           ref[j].x * yx + ref[j].y * yy, ref[j].z) -
                   zone[i])
                      .sqrnorm();
        if (v < lowv) {
          lowj = j;
...
void findBestBrick(
    std::string &brick, int &rotate, int &flip, float &es,
    std::vector<CVector> &zoneVertices,
    const std::map<std::string, std::vector<CVector>> &brickVertices) {
  float bestPoints = (float)(uint32)~0;
  for (std::map<std::string, std::vector<CVector>>::const_iterator
           it = brickVertices.begin(),
           end = brickVertices.end();
       it != end; ++it) {
    float rating;
...
bool unbuildLand() {
  ...
  for (std::vector<std::string>::const_iterator it = brickFiles.begin(),
                                                end = brickFiles.end();
       it != end; ++it) {
    if (CFile::getExtension(*it) != "zone")
      continue;

    if (NLMISC::startsWith(CFile::getFilename(*it),
                           "converted-")) // Not a real ligo
      continue;

    // if (NLMISC::startsWith(CFile::getFilename(*it), "foret-moor")) // Not
    // useful for r2_forest 	continue; if
    // (NLMISC::startsWith(CFile::getFilename(*it), "goo-")) // Not useful for
    // r2_forest 	continue; if
    // (NLMISC::endsWith(CFile::getFilenameWithoutExtension(*it), "_zc")) // Not
    // useful for r2_forest 	continue;
    ...
    brickVertices[CFile::getFilenameWithoutExtension(*it)] =
        std::vector<CVector>();
    ...
    for (ptrdiff_t i = 0; i < zonePatches.size(); ++i) {
      for (ptrdiff_t j = 0; j < 4; ++j) {
        ...
        if (rx <= -th || rx >= th || ry <= -th || ry >= th)
          goto SkipA;
      }
      for (ptrdiff_t j = 0; j < 4; ++j) {
        float rx = zonePatches[i].Patch.Vertices[j].x - off.x;
        float ry = zonePatches[i].Patch.Vertices[j].y - off.y;
        vec.push_back(CVector(
            rx, ry, s_Height ? zonePatches[i].Patch.Vertices[j].z : 0.0f));
      }
    SkipA:;
    }
  ...
  for (std::vector<std::string>::const_iterator it = runtimeFiles.begin(),
                                                end = runtimeFiles.end();
       it != end; ++it) {
    ...
    std::vector<CVector> vec;
    CVector off = CVector((float)x * s_CellSize, (float)y * s_CellSize, 0.0f) +
                  CVector(s_CellSize * 0.5f, s_CellSize * 0.5f, 0.0f);
    for (ptrdiff_t i = 0; i < zonePatches.size(); ++i) {
      for (ptrdiff_t j = 0; j < 4; ++j) {
        float rx = zonePatches[i].Patch.Vertices[j].x - off.x;
        float ry = zonePatches[i].Patch.Vertices[j].y - off.y;
    ...
    nlinfo("Zone: %s, brick: %s, rotate: %i; flip: %i, error score: %f",
           zoneName.c_str(), brick.c_str(), rotate, flip, es);
...
int main(int argc, char **argv) {
  ...
  if (!getXYFromZoneName(zoneMinX, zoneMinY,
                         args.getAdditionalArg("zonemin")[0]) ||
      !getXYFromZoneName(zoneMaxX, zoneMaxY,
                         args.getAdditionalArg("zonemax")[0])) {
    goto Fail;
  }

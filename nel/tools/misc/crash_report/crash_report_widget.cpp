...
CCrashReportWidget::CCrashReportWidget(QWidget *parent) : QWidget(parent) {
...
CCrashReportWidget::~CCrashReportWidget() { m_socket = NULL; }

void CCrashReportWidget::setup(
  ...
  if (m_developerMode) {
    ...
    QPushButton *alwaysIgnoreButton =
        new QPushButton(tr("Always Ignore"), this);
    ...
    connect(alwaysIgnoreButton, SIGNAL(clicked(bool)), this,
            SLOT(onAlwaysIgnoreClicked()));
    ...
    if (!m_socket->url().isEmpty() && !m_fileName.isEmpty()) {
      if (!m_forceSend) {
        QPushButton *cancelButton =
            new QPushButton(tr("Don't send report"), this);
...
void CCrashReportWidget::onCancelClicked() { removeAndQuit(); }
...
void CCrashReportWidget::onReportSent() {
  ...
  QMessageBox::information(this, tr("Report sent"),

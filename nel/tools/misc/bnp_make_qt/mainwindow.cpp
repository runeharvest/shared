...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent), uiMainWindow_(new Ui::MainWindow),
      processList_(new QProcess(this)), processSearch_(new QProcess(this)),
      processPack_(new QProcess(this)), processUnpack_(new QProcess(this)) {
  ...
  connect(uiMainWindow_->pushButtonListBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonListBrowseClicked()));
  connect(uiMainWindow_->pushButtonList, SIGNAL(clicked()), this,
          SLOT(onButtonListClicked()));
  connect(processList_, SIGNAL(finished(int)), this,
          SLOT(onProcessListComplete()));

  connect(uiMainWindow_->pushButtonSearchBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonSearchBrowseClicked()));
  connect(uiMainWindow_->pushButtonSearch, SIGNAL(clicked()), this,
          SLOT(onButtonSearchClicked()));
  connect(processSearch_, SIGNAL(finished(int)), this,
          SLOT(onProcessSearchComplete()));

  connect(uiMainWindow_->pushButtonPackBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonPackBrowseClicked()));
  connect(uiMainWindow_->pushButtonPack, SIGNAL(clicked()), this,
          SLOT(onButtonPackClicked()));
  connect(processPack_, SIGNAL(finished(int)), this,
          SLOT(onProcessPackComplete()));

  connect(uiMainWindow_->pushButtonUnpackBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonUnpackBrowseClicked()));
  connect(uiMainWindow_->pushButtonUnpack, SIGNAL(clicked()), this,
          SLOT(onButtonUnpackClicked()));
  connect(processUnpack_, SIGNAL(finished(int)), this,
          SLOT(onProcessUnpackComplete()));

  connect(uiMainWindow_->pushButtonBnpMakeBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonBnpMakeBrowseClicked()));
  connect(uiMainWindow_->pushButtonDataPathBrowse, SIGNAL(clicked()), this,
          SLOT(onButtonDataPathBrowseClicked()));
}

MainWindow::~MainWindow() { delete uiMainWindow_; }

void MainWindow::onButtonListBrowseClicked() {
  QString fileName;

  fileName = QFileDialog::getOpenFileName(this, "Choose a BNP file", dataPath_,
                                          "BNP file (*.bnp)");
...
void MainWindow::onButtonSearchBrowseClicked() {
  ...
  directory = QFileDialog::getExistingDirectory(
      this, tr("Choose a directory"), dataPath_,
...
void MainWindow::onProcessSearchComplete() {
  ...
  if (fileInfoListIndex_ < fileInfoList_.count()) {
    ...
    uiMainWindow_->textEditSearch->append(
        "===> " + uiMainWindow_->lineEditSearchPath->text() + "/" +
        fileInfo.fileName() + ":");
    arguments << "/l"
              << uiMainWindow_->lineEditSearchPath->text() + "/" +
                     fileInfo.fileName();
...
void MainWindow::onButtonPackBrowseClicked() {
  ...
  directory = QFileDialog::getExistingDirectory(
      this, tr("Choose the source directory"), dataPath_,
      QFileDialog::ShowDirsOnly | QFileDialog::DontResolveSymlinks);
...
void MainWindow::onButtonBnpMakeBrowseClicked() {
  ...
  fileName = QFileDialog::getOpenFileName(this, "Locate the bnp_make binary",
                                          dataPath_);

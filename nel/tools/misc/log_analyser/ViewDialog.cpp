...
afx_msg void CListCtrlEx::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) {
  ...
  ((CLog_analyserDlg *)(_ViewDialog->GetParent()))
...
void CListCtrlEx::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
  ...
  pDC->FillRect(rcHighlight, &CBrush(_ViewDialog->getBkColorForLine(
                                 nItem, bHighlight != 0)));
...
void CViewDialog::reload() {
  ...
  if (LogSessionStartDate.IsEmpty() ||
      (LogSessionStartDate == _T("Beginning"))) {
    SessionDatePassed = true;
  }

  ((CButton *)GetDlgItem(IDC_BUTTON1))->ShowWindow(SW_SHOW);
  ((CButton *)GetDlgItem(IDC_BUTTON2))->ShowWindow(SW_SHOW);
  m_Caption.Format(_T("%s (%u file%s) %u+ %u- (%s)"), Seriesname,
                   Filenames.size(), Filenames.size() > 1 ? "s" : "",
                   PosFilter.size(), NegFilter.size(),
                   LogSessionStartDate.IsEmpty()
                       ? "all"
                       : CString("session ") + LogSessionStartDate);
  UpdateData(false);
  clear();
  setRedraw(false);
...
/*
 * Reload with old filter to get bookmarks absolute line numbers (not called if
 * there's no bookmark)
 */
void CViewDialog::getBookmarksAbsoluteLines(
    vector<int> &bookmarksAbsoluteLines) {
  // Read the files
    ...
    if (ifs.open(NLMISC::tStrToUtf8(filename))) {
      ...
      while (!ifs.eof()) {
        ...
        if (SessionDatePassed) {
          ...
          if ((!LogSessionStartDate.IsEmpty()) &&
              (strstr(line, nlTStrToUtf8(LogDateString)))) {
            return;
          }
...
bool detectLineCorruption(const char *cLine) {
  ...
  if (!line.empty()) {
    ...
    // Some line may begin with no date, this is normal, we don't state them as
    // corruption

    // Search for year not at beginning. Ex: "2003/" (it does not work when the
    // year is different from the current one!)
    p = line.substr(1).find(timeToStr(CurrentTime).substr(0, 5));
    if (p != string::npos) {
      ++p; // because searched from pos 1

      // Search for date/time
      if ((line.size() > p + 20) && (line[p + 10] == ' ') &&
          (line[p + 13] == ':') && (line[p + 16] == ':') &&
          (line[p + 19] == ' ')) {
        // Search for the five next blank characters. The fifth is followed by
        // ": ". (Date Time LogType ThreadId Machine/Service SourceFile Line :
        // User-defined log line)
 ...
 *
 */
std::string CViewDialog::corruptedLinesString(
    const std::vector<unsigned int> &corruptedLines) {
  string res;
  if (!corruptedLines.empty()) {
    res = NLMISC::toString(" -> %u corrupted lines:",
                           (uint)corruptedLines.size());

    vector<unsigned int>::const_iterator ivc;
    for (ivc = corruptedLines.begin(); ivc != corruptedLines.end(); ++ivc) {
      res +=
          NLMISC::toString("\r\n   line %u : %s...", *ivc,
                           NLMISC::tStrToUtf8(Buffer[*ivc].Left(20)).c_str());
    }
...
void CViewDialog::loadFileOrSeries(const vector<int> &bookmarksAbsoluteLines) {
  ...
  bool corruptionDetectionEnabled =
      (((CButton *)(((CLog_analyserDlg *)GetParent())
                        ->GetDlgItem(IDC_DetectCorruptedLines)))
           ->GetCheck() == 1);
    ...
    if (!ifs.fail()) {
      ...
      while (!ifs.eof()) {
        ...
        if (SessionDatePassed) {
          ...
          if ((!LogSessionStartDate.IsEmpty()) &&
              (strstr(line, nlTStrToUtf8(LogDateString)))) {
            actualFilenames += NLMISC::tStrToUtf8(filename) +
          ...
          if (passFilter(line, PosFilter, NegFilter)) {
            ...
            if (!bookmarksAbsoluteLines.empty()) {
              ...
              ibal = find(bookmarksAbsoluteLines.begin(),
                          bookmarksAbsoluteLines.end(), currentAbsoluteLineNum);
...
At any time, press Ctrl-L in this edit box to return to this list of files and corrupted lines.";
  ...
  ((CLog_analyserDlg *)GetParent())
      ->displayCurrentLine(actualFilenames.c_str());
 ...
 */
void CViewDialog::setFilters(const std::vector<CString> &posFilter,
                             const std::vector<CString> &negFilter) {
 ...
 */
bool CViewDialog::passFilter(const char *text,
                             const std::vector<CString> &posFilter,
                             const std::vector<CString> &negFilter) const {
...
void CViewDialog::reloadTrace() {
  ...
  if (LogSessionStartDate.IsEmpty()) {
    ...
    if (PosFilter.empty())
      m_Caption =
          "Trace of " + Seriesname + " (session " + LogSessionStartDate + ")";
    else
      m_Caption =
          "Trace of " + PosFilter[0] + " (session " + LogSessionStartDate + ")";
  }
    ...
    while (!ifs.eof()) {
      ...
      if (SessionDatePassed) {
        ...
        if (pc != NULL) {
          ...
          if (PosFilter.empty() ||
              (strncmp(pc - PosFilter[0].GetLength(), tposFilter0.c_str(),
                       tposFilter0.size()) == 0)) {
 ...
 */
int CViewDialog::getNbLines() const { return (int)Buffer.size(); }
 ...
 */
int CViewDialog::getScrollIndex() const { return m_ListCtrl.GetTopIndex(); }

/*
 * Add the current scroll index to the bookmark list, or delete it if already
 * inside the list
 */
void CViewDialog::addBookmark() {
  ...
  if (ibk == Bookmarks.end()) {
    ...
    std::sort(Bookmarks.begin(),
              Bookmarks.end()); // not very fast but not many items
...
void CViewDialog::recallNextBookmark() {
  ...
  int origIndex = (CurrentBookmark == -1) ? getScrollIndex() : CurrentBookmark,
      destIndex;
  unsigned int i = 0;
  ...
  CurrentBookmark = destIndex; // because scrollTo does not scroll if we are at
...
void CViewDialog::OnButtonFilter() {
  if (((CLog_analyserDlg *)GetParent())->FilterDialog.DoModal() == IDOK) {
    setFilters(((CLog_analyserDlg *)GetParent())->FilterDialog.getPosFilter(),
               ((CLog_analyserDlg *)GetParent())->FilterDialog.getNegFilter());
...
COLORREF CViewDialog::getTextColorForLine(int index, bool selected) {
  if (selected)
    ...
    if (Buffer[index].Find(_T("DBG")) != -1)
      ...
      return RGB(0x80, 0, 0);
    else if ((Buffer[index].Find(_T("ERR")) != -1) ||
             (Buffer[index].Find(_T("AST")) != -1))
      return RGB(0xFF, 0, 0);
    else // INF and others
      return RGB(0, 0, 0);
  }
...
COLORREF CViewDialog::getBkColorForLine(int index, bool selected) {
  ...
  if ((i < Bookmarks.size()) && (index == Bookmarks[i]))
    if (selected)
      return (::GetSysColor(COLOR_HIGHLIGHT) + RGB(0xC0, 0x90, 0x90)) /
             2; // selected bookmark
    else
...
void CViewDialog::OnGetdispinfoList1(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  if (pItem->mask & LVIF_TEXT) // valid text buffer?
  {
    ...
    formatLogStr(str, ((CButton *)(((CLog_analyserDlg *)GetParent())
                                       ->GetDlgItem(IDC_DispLineHeaders)))
                              ->GetCheck() == 1);
...
void CViewDialog::OnButtonFind() {
  ...
  FindDialog->Create(true, FindStr, NULL,
                     frDown | frMatchCase | FR_HIDEWHOLEWORD, this);
}
...
afx_msg LRESULT CViewDialog::OnFindReplace(WPARAM wParam, LPARAM lParam) {
  ...
  if (FindDialog->FindNext()) {
    ...
    if (FindDialog->SearchDown()) {
      ...
      for (lineIndex = BeginFindIndex; lineIndex != (int)Buffer.size();
           ++lineIndex) {
        if (matchString(Buffer[lineIndex], FindStr,
                        FindDialog->MatchCase() != 0, matchPos)) {
          scrollTo(lineIndex);
          select(lineIndex);
          ...
          s.Format(_T("Found '%s' (downwards from line %d) at line %d:\r\n%s"),
                   FindStr, BeginFindIndex, lineIndex, Buffer[lineIndex]);
          ((CLog_analyserDlg *)GetParent())->displayCurrentLine(s);
          ((CLog_analyserDlg *)GetParent())
              ->selectText(1, matchPos, FindStr.GetLength());
    ...
    CString s;
    s.Format(_T("Not found (%s from line %d)"),
             FindDialog->SearchDown() ? _T("downwards") : _T("upwards"),
             BeginFindIndex);
    AfxMessageBox(s);
    // BeginFindIndex = 0;
    return 0;

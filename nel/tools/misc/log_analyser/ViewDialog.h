...
public:
  ...
  void setFilters(const std::vector<CString> &posFilter,
                  const std::vector<CString> &negFilter);

  /// Returns true if the string must be logged, according to the current
  /// filters
  bool passFilter(const char *text, const std::vector<CString> &posFilter,
  ...
  /// Scroll the listbox to the next found stored bookmkark (downwards from the
  ...
  ///
  std::string
  corruptedLinesString(const std::vector<unsigned int> &corruptedLines);

  /// Reload with old filter to get bookmarks absolute line numbers (not called
  /// if there's no bookmark)
  ...
  std::vector<CString> PosFilter, NegFilter, PreviousPosFilter,
      PreviousNegFilter;

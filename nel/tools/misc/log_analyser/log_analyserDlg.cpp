...
BOOL CLog_analyserDlg::OnInitDialog() {
  ...
  vector<CString> v;
  /*int pos = cmdLine.find_first_of(' '); // TODO: handle "" with blank
  characters while ( pos != string::npos )
  {
...
HCURSOR CLog_analyserDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
bool CLog_analyserDlg::addPlugIn(const std::string &dllName) {
  ...
  theApp.WriteProfileString(_T(""), nlUtf8ToTStr(pluginN),
                            nlUtf8ToTStr(dllName));
 ...
 *
 */
bool isNumberChar(char c) { return (c >= '0') && (c <= '9'); }
...
void CLog_analyserDlg::addView(std::vector<CString> &pathNames) {
  if (pathNames.empty()) {
    CFileDialog openDialog(true, NULL, _T("log.log"),
  ...
  if (pathNames.size() > 1) {
    ...
    for (i = 0; i != pathNames.size(); ++i) {
      // Ensure that a log file without number comes *after* the ones with a
      // number
      string name = NLMISC::tStrToUtf8(pathNames[i]);
      string::size_type dotpos = name.find_last_of('.');
      if ((dotpos != string::npos) && (dotpos > 2)) {
        if (!(isNumberChar(name[dotpos - 1]) &&
              isNumberChar(name[dotpos - 2]) &&
              isNumberChar(name[dotpos - 3]))) {
...
void CLog_analyserDlg::OnAddtraceview() {
  CFileDialog openDialog(true, NULL, _T("log.log"), OFN_HIDEREADONLY,
                         _T("Log files (*.log)|*.log|All files|*.*||"), this);
  if (openDialog.DoModal() == IDOK) {
    ...
    if (FilterDialog.DoModal() == IDOK) {
      view->setFilters(FilterDialog.getPosFilter(),
                       FilterDialog.getNegFilter());
    }
...
CViewDialog *CLog_analyserDlg::onAddCommon(const vector<CString> &filenames) {
  ...
  if (((CButton *)GetDlgItem(IDC_CheckSessions))->GetCheck() == 1) {
    ...
    case 0:
      ...
      break;
    case 1: {
      // 1 'Log Starting' => no choice if it's at the beginning (1st line, or
      // 2nd line with blank 1st)
 ...
 *
 */
void CLog_analyserDlg::getLogSeries(const CString &filenameStr,
  ...
  if (isLogSeriesEnabled()) {
    ...
    if (dotpos != string::npos) {
      ...
      bool anymore = true;
      while (anymore) {
        // If filename is my_service.log, try
        // my_service001.log..my_service999.log
        string npath = start;
...
void CLog_analyserDlg::OnComputeTraces() {
  ...
  m_ScrollBar.SetScrollRange(0, Views[0]->getNbLines() -
                                    Views[0]->getNbVisibleLines());
 ...
 *
 */
void CLog_analyserDlg::OnVScroll(UINT nSBCode, UINT nPos,
  ...
  if (Trace) {
    ...
    case SB_TOP:
      ...
      break;
    case SB_BOTTOM:
      index = Views[0]->getNbLines() - 1;
      break;
    case SB_ENDSCROLL:
      index = -1;
      break;
    case SB_LINEDOWN:
      index = Views[0]->getScrollIndex() + 1;
      break;
    case SB_LINEUP:
      index = Views[0]->getScrollIndex() - 1;
      break;
    case SB_PAGEDOWN:
      index = Views[0]->getScrollIndex() + Views[0]->getNbVisibleLines();
      break;
    case SB_PAGEUP:
      index = Views[0]->getScrollIndex() - Views[0]->getNbVisibleLines();
      break;
...
void CLog_analyserDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  if (::IsWindow(m_Edit)) {
    ...
    GetClientRect(&cltRect), m_Edit.GetWindowRect(&editRect);
...
void CLog_analyserDlg::OnHelpBtn() {
  ...
  s += "Simple Mode: open one or more log files using the button 'Add "
       "View...'.\n";
  s += "You can make a multiple selection, then the files will be sorted by "
       "log order.\n";
  s += "If the file(s) being opened contain(s) several log sessions, you can "
       "choose one or\n";
  s += "choose to display all sessions, if the checkbox 'Browse Log Sessions' "
       "is enabled. If the\n";
  s += "checkbox 'Browse All File Series' is checked and you choose "
       "my_service.log, all log\n";
  s += "files of the series beginning with my_service000.log up to the biggest "
       "number found,\n";
  s += "and ending with my_service.log, will be opened in the same view. If "
       "'Detect corrupted\n";
  s += "files' is checked, the possibly malformed lines will be reported.\n";
  s += "You can add some positive/negative filters. Finally, you may click a "
       "log line to display\n";
  s += "it in its entirety in the top field.\n";
  s += "Another way to open a file is to pass its filename as an argument. An "
       "alternative way to\n";
  s += "open one or more files is to drag & drop them onto the main window!.\n";
  s += "To actualize a file (which may have changed if a program is still "
       "writing into it), just\n";
  s += "click 'Filter...' and OK.\n";
  s += "Resizing a view is done by dragging its left border.\n";
  s += "Line bookmarks: set/remove = Ctrl+F2, recall = F2. They are kept when "
       "changing the filter.\n\n";
  s += "Trace Mode: open several log files in Trace Format (see below) using "
       "the button\n";
  s += "'Add Trace View...', you can limit the lines loaded to the ones "
       "containing a\n";
  s += "specific service shortname (see below). Then click the button 'Compute "
       "Traces'\n";
  s += "to display the matching lines. The lines are sorted using their "
       "gamecycle and\n";
  s += "blank lines are filled so that different parallel views have the same "
       "timeline.\n";
  s += "Use the right scrollbar to scroll all the views at the same time.\n";
  s += "The logs in Trace Format should contains some lines that have a "
       "substring sTRACE:n:\n";
  s += "where s is an optional service name (e.g. FS) and n is the gamecycle "
       "of the action\n";
  s += "(an integer).\n\n";
  s += "Plug-in system: You can provide DLLs to perform some processing or "
       "analysis on the current\n";
  s += "view. To register a new plug-in, drag-n-drop the DLL on the main "
       "window of the Log Analyser.\n";
  s += "To unregister a plug-in, see "
       "HKEY_CURRENT_USER\\Software\\Nevrax\\log_analyser.INI in the\n";
  s += "registry.";
  MessageBox(s);
}
...
void CLog_analyserDlg::OnAnalyse() {
  ...
  if (PlugInSelectorDialog.DoModal() == IDOK) {
    if (Views.empty()) {
      AfxMessageBox(
          _T("This plug-in needs to be applied on the first open view"));
      return;
    }
    ...
    PlugInSelectorDialog.AnalyseFunc(
        *(const std::vector<const char *> *)(void *)&(getCurrentView()->Buffer),
        resstr, logstr);
    if (!logstr.empty()) {
      vector<CString> pl;
    ...
    if (nEndChar != (int)resstr.size()) {
      CString s;
      s.Format(_T("Error: plug-in returned %u characters, only %d displayed"),
               (uint)resstr.size(), nEndChar + 1);
      AfxMessageBox(s);
    }
  }
}

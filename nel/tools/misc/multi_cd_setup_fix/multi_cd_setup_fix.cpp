...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE /* hPrevInstance */,
  ...
  // Windows
  HWND hwnd =
      CreateDialog(hInstance, MAKEINTRESOURCE(IDD_WAIT), NULL, MyDialogProc);
  RECT rect;
  RECT rectDesktop;
  ...
  SetWindowPos(
      ...
      (rectDesktop.bottom - rectDesktop.top - rect.bottom + rect.top) / 2, 0, 0,
      SWP_NOSIZE);
  ...
  if (GetTempPath(1024, tempPath)) {
    ...
    SetFileAttributes(setupFile,
                      GetFileAttributes(setupFile) & ~FILE_ATTRIBUTE_READONLY);
    BOOL deleted = DeleteFile(setupFile);
    if (!CopyFile(_T("setup.dat"), setupFile, FALSE) && deleted)
      MessageBox(NULL, _T("Not enough disk space"), _T("Setup"),
                 MB_OK | MB_ICONERROR);
    SetFileAttributes(setupFile,
                      GetFileAttributes(setupFile) & ~FILE_ATTRIBUTE_READONLY);
    ...
    SetFileAttributes(msiFile,
                      GetFileAttributes(msiFile) & ~FILE_ATTRIBUTE_READONLY);
    ...
    RegSetValueEx(hKey, _T("RyzomSetupClean"), 0, REG_SZ,
                  (const unsigned char *)batFileReg,
                  (DWORD)(_tcslen(batFileReg) + 1) * sizeof(TCHAR));
    ...
    if (CreateProcess(setupFile, option, NULL, NULL, FALSE, 0, NULL, tempPath,
                      &si, &pi))
      return 0;
  }

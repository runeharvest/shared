...
void usage() {
  printf("USAGE : \n");
  printf("   snp_make -p <directory_name> <package_file> <stream_directory> "
         "[option] ... [option]\n");
  printf("   option : \n");
  printf("      -if wildcard : add the file if it matches the wilcard (at "
         "least one 'if' conditions must be met for a file to be adding)\n");
  printf("      -ifnot wildcard : add the file if it doesn't match the wilcard "
         "(all the 'ifnot' conditions must be met for a file to be adding)\n");
...
void generateLZMA(const std::string &sourceFile,
...
#ifdef NL_OS_WINDOWS
  _spawnlp(_P_WAIT, "lzma.exe","lzma.exe", "e", sourceFile.c_str(),
outputFile.c_str(), NULL); #else // NL_OS_WINDOWS sint error = system
(cmd.c_str()); if (error) nlwarning("'%s' failed with error code %d",
cmd.c_str(), error); #endif // NL_OS_WINDOWS
  */
}
...
int main(int nNbArg, char **ppArgs) {
  ...
  if ((strcmp(ppArgs[1], "/p") == 0) || (strcmp(ppArgs[1], "/P") == 0) ||
    ...
    for (std::vector<std::string>::size_type i = 0; i < pathContent.size();
         ++i) {
    ...
    for (CStreamedPackage::TEntries::size_type i = 0;
         i < Package.Entries.size(); ++i) {
      const CStreamedPackage::CEntry &entry = Package.Entries[i];

      sint foundIndex = -1; // find index in found file list
      for (std::vector<std::string>::size_type j = 0; j < pathContent.size();
           ++j) {
    ...
    for (std::vector<std::string>::size_type i = 0; i < pathContent.size();
      ...
      std::string targetLzmaOld; // in case lzma wasn't made make sure it exists
                                 // a second run
      ...
      if (lastModified > entry.LastModified || fileSize != entry.Size ||
        ...
        std::string targetLzma; // in case lzma wasn't made make sure it exists
                                // a second run
  ...
  if ((strcmp(ppArgs[1], "/l") == 0) || (strcmp(ppArgs[1], "/L") == 0) ||
    ...
    if (!CFile::fileExists(PackageFileName)) {
      nlwarning("ERROR (snp_make) : package doesn't exist: '%s'",
    ...
    for (CStreamedPackage::TEntries::const_iterator it(Package.Entries.begin()),
         end(Package.Entries.end());
         it != end; ++it) {
      const CStreamedPackage::CEntry &entry = (*it);

      printf("List files in '%s'", PackageFileName.c_str());
      printf("%s { Hash: '%s', Size: '%u', LastModified: '%u' }",
             entry.Name.c_str(), entry.Hash.toString().c_str(), entry.Size,
             entry.LastModified);
    }

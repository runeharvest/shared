...
BOOL CData_mirrorDlg::OnInitDialog() {
  ...
  HIMAGELIST imageListSmall =
      (HIMAGELIST)SHGetFileInfo(TEXT("C:\\"), 0, &sfi, sizeof(SHFILEINFO),
...
void CData_mirrorDlg::updateList() {
  ...
  while (ite != entries.end()) {
    ...
    uint nItem = List.InsertItem(
        0, nlUtf8ToTStr(entry.Strings[CEntryFile::Path]), entry.Image);
    List.SetItemData(nItem,
                     DWORD_PTR(new std::list<CEntryFile>::iterator(ite)));
    ...
    // Add the sizes
    if (ModifiedFilter != Removed) {
      List.SetItemText(nItem, subString++,
                       nlUtf8ToTStr(entry.Strings[CEntryFile::NewSize]));
    }
    if (ModifiedFilter != Added) {
      List.SetItemText(nItem, subString++,
                       nlUtf8ToTStr(entry.Strings[CEntryFile::OldSize]));
    }

    // Add the dates
    if (ModifiedFilter != Removed) {
      List.SetItemText(nItem, subString++,
                       nlUtf8ToTStr(entry.Strings[CEntryFile::NewDate]));
    }
    if (ModifiedFilter != Added) {
      List.SetItemText(nItem, subString++,
                       nlUtf8ToTStr(entry.Strings[CEntryFile::OldDate]));
    }

    // Add the type
    List.SetItemText(nItem, subString++,
                     nlUtf8ToTStr(entry.Strings[CEntryFile::Type]));
...
HCURSOR CData_mirrorDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void CData_mirrorDlg::OnIgnore() {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ListView_GetCheckState(List, i)) {
      ...
      std::list<CEntryFile>::iterator *ite =
          (std::list<CEntryFile>::iterator *)List.GetItemData(i);
      nlassert(ite);
      entries.erase(*ite);
      delete ite;
...
bool setFileTime(const std::string &filename, const FILETIME &result) {
  HANDLE handle =
      CreateFile(nlUtf8ToTStr(filename), GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
...
void CData_mirrorDlg::OnOK() {
  ...
  uint totalCount = (uint)FilesToUpdate[Modified].size() +
                    (uint)FilesToUpdate[Added].size() +
                    (uint)FilesToUpdate[Removed].size();
  uint currentFile = 0;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!CopyFile(nlUtf8ToTStr(source), nlUtf8ToTStr(dest), FALSE)) {
      MessageBox(nlUtf8ToTStr("Can't copy file " + source + " in file " + dest),
  ...
  for (i = 0; i < count; i++) {
    ...
    if (!DeleteFile(nlUtf8ToTStr(dest))) {
      MessageBox(nlUtf8ToTStr("Can't delete the file " + dest),
                 _T("NeL Data Mirror"), MB_OK | MB_ICONEXCLAMATION);
      success = false;
    } else {
      if (!LogDirectory.empty()) {
...
void CData_mirrorDlg::resize() {
  ...
  if (ButtonXModifiedFromRight) {
    ...
    ModifiedFilterCtrl.SetWindowPos(NULL,
                                    client.right - ButtonXModifiedFromRight,
                                    child.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    AddedFilterCtrl.SetWindowPos(NULL, client.right - ButtonXAddedFromRight,
                                 child.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    RemovedFilterCtrl.SetWindowPos(NULL, client.right - ButtonXRemovedFromRight,
                                   child.top, 0, 0, SWP_NOSIZE | SWP_NOZORDER);
    List.SetWindowPos(NULL, 0, 0, client.right - ListRightFromRight,
                      client.bottom - ListBottomFromBottom,
...
bool getFileTime(const std::string &filename, FILETIME &result) {
  HANDLE handle =
      CreateFile(nlUtf8ToTStr(filename), GENERIC_READ, FILE_SHARE_READ, NULL,
                 OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (handle) {
    FILETIME res0;
    FILETIME res1;
...
void CData_mirrorDlg::buildSourceFiles() {
  ...
  CPath::getPathContent(MainDirectory + CurrentDir, true, false, true,
                        fileSource);
  uint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    // In the ignore list ?
    if (IgnoreFiles.find(str) == IgnoreFiles.end() &&
        (str != "ignore_list.txt")) {
      ...
      if (NLMISC::CFile::fileExists(mirrorFile)) {
        ...
        sint64 deltaInt = (((uint64)time1.dwHighDateTime) << 32 |
                           ((uint64)time1.dwLowDateTime)) -
                          (((uint64)time0.dwHighDateTime) << 32 |
                           ((uint64)time0.dwLowDateTime));
        ...
        if (deltaInSec > 2.0) {
          if (BinaryCompare) {
            ...
            // Check Files sizes
            if (NLMISC::CFile::getFileSize(mainFile) !=
                NLMISC::CFile::getFileSize(mirrorFile))
              bDiff = true;
  ...
  fileSource.clear();
  CPath::getPathContent(MirrorDirectory + CurrentDir, true, false, true,
                        fileSource);
...
void timeToString(string &dest, FILETIME time) {
  ...
  if (FileTimeToLocalFileTime(&time, &localFileTime)) {
    if (FileTimeToSystemTime(&localFileTime, &systemTime)) {
      char date[512];
      smprintf(date, 512, "%d/%d/%d %d:%02d", systemTime.wDay,
...
void CData_mirrorDlg::addEntry(uint where, const char *filename,
                               FILETIME &newDate, FILETIME &oldDate) {
  // Add an entry first
  ...
  if (ite == MapExtensions.end()) {
    ...
    SHGetFileInfo(winName, 0, &sfi, sizeof(SHFILEINFO),
    ...
    ite = MapExtensions
              .insert(std::map<string, CExtension>::value_type(ext, extension))
              .first;
  }
...
int CALLBACK CompareFunc(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort) {
  ...
  case CData_mirrorDlg::Modified: {
    ...
    case 0:
      result = strcmp(entry1.Strings[CEntryFile::Path].c_str(),
                      entry2.Strings[CEntryFile::Path].c_str());
      break;
    case 1:
      result = (entry1.NewSizeUI < entry2.NewSizeUI)    ? -1
               : (entry1.NewSizeUI == entry2.NewSizeUI) ? 0
                                                        : 1;
      break;
    case 2:
      result = (entry1.OldSizeUI < entry2.OldSizeUI)    ? -1
               : (entry1.OldSizeUI == entry2.OldSizeUI) ? 0
                                                        : 1;
      break;
    case 3:
    ...
    case 4:
      ...
      break;
    case 5:
      result = strcmp(entry1.Strings[CEntryFile::Type].c_str(),
                      entry2.Strings[CEntryFile::Type].c_str());
      break;
    }
    break;
  }

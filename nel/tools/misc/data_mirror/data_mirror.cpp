...
BOOL CData_mirrorApp::InitInstance() {
  ...
  try {
    ...
    string temp = "\"" + exePath;
    temp = temp + "\" \"%1\"";
    nlverify(RegisterDirectoryAppCommand("NeLDataMirror", "&Sync Mirror",
                                         temp.c_str()));
    ...
    if (NLMISC::CFile::isDirectory(CurrentDir)) {
      ...
      directory = false;
      CurrentDir = toLowerAscii(
          CPath::standardizePath(NLMISC::CFile::getPath(CurrentDir)));
    } else {
      MessageBox(NULL,
                 nlUtf8ToTStr(CurrentDir + " is not a directory nor a file."),
    ...
    if (CurrentDir.substr(0, MainDirectory.size()) == MainDirectory) {
      CurrentDir = CurrentDir.substr(MainDirectory.size(),
                                     CurrentDir.size() - MainDirectory.size());
    } else {
      if (CurrentDir.substr(0, MirrorDirectory.size()) == MirrorDirectory) {
        CurrentDir = CurrentDir.substr(
            MirrorDirectory.size(), CurrentDir.size() - MirrorDirectory.size());
      } else {
        MessageBox(NULL,
                   nlUtf8ToTStr(CurrentDir + " is not a sub directory of " +
                                MainDirectory + " or " + MirrorDirectory),
                   _T("NeL Data Mirror"), MB_OK | MB_ICONEXCLAMATION);
        return FALSE;
      }
    }
    ...
    if (CurrentDir.substr (0, MainDirectory.size ()) != MainDirectory)
    {
        MessageBox (NULL, (CurrentDir+" is not a sub directory of
    "+MainDirectory).c_str (), "NeL Data Mirror", MB_OK|MB_ICONEXCLAMATION);
        return FALSE;
    }
    CurrentDir = CurrentDir.substr (MainDirectory.size (), CurrentDir.size () -
    MainDirectory.size ());
    */
    ...
    MessageBox(NULL, nlUtf8ToTStr(e.what()), _T("NeL Data Mirror"),
...
bool RegisterDirectoryAppCommand(const char *appName, const char *command,
  ...
  // Create the app key
  HKEY hKey;
  if (RegCreateKey(HKEY_CLASSES_ROOT, _T("Directory"), &hKey) ==
      ERROR_SUCCESS) {
    // Create the icon
    char tmp[512];
    ...
    if (RegCreateKey(hKey, nlUtf8ToTStr(tmp), &hKey) == ERROR_SUCCESS) {
      ...
      RegSetValue(hKey, _T(""), REG_SZ, tcommand.c_str(),
                  (tcommand.size() + 1) * sizeof(TCHAR));
      if (RegCreateKey(hKey, _T("command"), &hKey) == ERROR_SUCCESS) {
        // Set the description
        tstring tapp = utf8ToTStr(app);
        RegSetValue(hKey, _T(""), REG_SZ, tapp.c_str(),
                    (tapp.size() + 1) * sizeof(TCHAR));
        return true;
      }
    }
  }

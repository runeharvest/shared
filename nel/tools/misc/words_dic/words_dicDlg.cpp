...
BOOL CWords_dicDlg::OnInitDialog() {
  ...
  SplashScreen->GetDlgItem(IDC_SplashText)
      ->SetWindowText(_T("Please wait while loading dictionary..."));
  if (!Dico.init())
    AfxMessageBox(_T("Can't init dictionary, see reason in log.log"));
  SplashScreen->DestroyWindow();
  delete SplashScreen;
  GetDlgItem(IDC_Status)
      ->SetWindowText(_T("Tip: ^ and $ can be used to represent the start and ")
                      _T("the end of string"));
...
HCURSOR CWords_dicDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
void CWords_dicDlg::lookUp(const CString &inputStr) {
  ...
  if (resultVec.empty()) {
    ...
    CString s;
    s.Format(_T("%u results found for \"%s\".%s"), resultVec.size(), inputStr,
             lvlRemoved ? _T(" Results containing \"lvl\" not shown") : _T(""));
 ...
 *
 */
void CWords_dicDlg::OnShowAll() { OnBtnFind(); }

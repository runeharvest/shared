...
bool i_comp(const string &s0, const string &s1) {
  return nlstricmp(CFile::getFilename(s0).c_str(),
                   CFile::getFilename(s1).c_str()) < 0;
}
...
bool packSubRecurse(const std::string &srcDirectory) {
  ...
  for (uint i = 1, len = pathContent.size(); i < len; ++i) {
    if (toLowerAscii(CFile::getFilename(pathContent[i - 1])) ==
        toLowerAscii(CFile::getFilename(pathContent[i]))) {
      nlwarning("File %s is not unique in BNP!",
                CFile::getFilename(pathContent[i]).c_str());
      return false;
    }
  }
...
int main(int argc, char **argv) {
  ...
  args.addArg("i", "if", "wildcard",
              "Add the file if it matches the wilcard (at least one 'if' "
              "conditions must be met for a file to be adding)",
              false);
  args.addArg("n", "ifnot", "wildcard",
              "Add the file if it doesn't match the wilcard (all the 'ifnot' "
              "conditions must be met for a file to be adding)",
              false);
  args.addArg("", "list-verbose", "",
              "List files using 'pos size name' format");
  args.addArg("", "extract", "name", "Extract file(s) from BNP into --output");
  args.addAdditionalArg("input",
                        "Input directory or BNP file depending on command");
  ...
  if (args.haveArg("p")) {
    ...
    if (!CFile::isDirectory(srcDirectory) || !CFile::isExists(srcDirectory)) {
      nlwarning("Error: %s doesn't exist or is not a directory!",
                srcDirectory.c_str());
    }
    ...
    if (args.haveArg("o")) {
      ...
      if (CFile::isDirectory(gBNPHeader.BigFileName)) {
        gBNPHeader.BigFileName =
            CPath::standardizePath(gBNPHeader.BigFileName) +
            CFile::getFilename(srcDirectory);
      }
    } else {
    ...
    if (!packSubRecurse(srcDirectory))
      return 1;
  ...
  if (args.haveLongArg("list-verbose")) {
    ...
    for (uint i = 0; i < gBNPHeader.SFiles.size(); ++i) {
      printf("%u %u %s\n", gBNPHeader.SFiles[i].Pos, gBNPHeader.SFiles[i].Size,
             gBNPHeader.SFiles[i].Name.c_str());
    }
  ...
  if (args.haveLongArg("extract") && !args.getLongArg("extract").empty()) {
    ...
    if (!inFile.open(CFile::getFilename(bnpName) + "@" + srcName)) {
      nlerror("Unable to open '%s' for reading",
              inFile.getStreamName().c_str());
    }

    COFile outFile;
    if (!outFile.open(dstName)) {
      nlerror("Unable to open '%s' for writing",
              outFile.getStreamName().c_str());
    }

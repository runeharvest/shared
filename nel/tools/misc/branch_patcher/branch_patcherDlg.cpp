...
BOOL CBranch_patcherDlg::OnInitDialog() {
  ...
  RECT cltRect;
  GetClientRect(&cltRect), m_Display = new CRichEditCtrl();
  m_Display->Create(
...
HCURSOR CBranch_patcherDlg::OnQueryDragIcon() { return (HCURSOR)m_hIcon; }
...
int CDirDialog::DoBrowse() { /////////////////////////////////////////
  ...
  if (!m_strInitDir.IsEmpty()) {
    ...
    if (SUCCEEDED(SHGetDesktopFolder(&pDesktopFolder))) {
      ...
      //
      hr = pDesktopFolder->ParseDisplayName(NULL, NULL, olePath, NULL, &pidl,
                                            &dwAttributes);
...
BOOL SendTextToClipboard(CString source) {
  ...
  if (OpenClipboard(NULL)) {
    ...
    clipbuffer = GlobalAlloc(GMEM_DDESHARE,
                             ((SIZE_T)source.GetLength() + 1) * sizeof(TCHAR));
    if (clipbuffer) {
...
void CBranch_patcherDlg::OnButtonPatch() {
  ...
  CString diffCmdLine;
  diffCmdLine.Format(
      _T("cvs.exe diff -c > %s 2> %s"), TEMP_DIFF_FILE,
      DIFF_ERRORS); // needs a valid cvs login before! and cvs.exe in the path
  CString text;
  text.Format(_T("Get diff from directory %s?\n\nCommand (choose No to copy ")
              _T("it into the clipboard):\n%s"),
              m_SrcDir, diffCmdLine);
  int result;
  if ((result = ::MessageBox(m_hWnd, text, _T("Confirmation"),
    ...
    if (_tchdir(m_SrcDir) == 0) {
      ...
      if ((m_Display->GetLineCount() == 0) ||
        ...
        displayMessage(
            ...
            "if the source directory is part of a CVS tree\r\n- check if "
            "cvs.exe is in your PATH\r\n- check if you are logged to the cvs "
            "server with 'cvs login' (set your home cvs directory in the HOME "
            "environment variable if needed)\r\n- check if C:\\ has enough "
            "free space and access rights to write a file.\n\nHere is the "
            "log:\n\n",
            true);
      } else {
...
static unsigned long CALLBACK MyStreamInCallback(DWORD_PTR dwCookie,
                                                 LPBYTE pbBuff, LONG cb,
                                                 LONG *pcb) {
...
static unsigned long CALLBACK MyStreamOutCallback(DWORD_PTR dwCookie,
                                                  LPBYTE pbBuff, LONG cb,
                                                  LONG *pcb) {
  CFile *pFile = (CFile *)dwCookie;
...
void CBranch_patcherDlg::OnDoPatch() {
  ...
  patchCmdLine.Format(_T("%spatch.exe -c -p%u --verbose < %s > %s 2> %s"),
                      PatchExeDir, CvsDiffDirLevel, TEMP_DIFF_FILE,
                      PATCH_RESULT,
                      PATCH_ERRORS); // needs patch.exe in the path
  concatOutput.Format(_T("copy %s+%s %s"), PATCH_RESULT, PATCH_ERRORS,
                      PATCH_RESULT);
  ...
  text.Format(_T("Patch diff to directory %s?\n\nCommand (choose No to copy ")
              _T("it into the clipboard):\n%s"),
              (LPCTSTR)m_DestDir, (LPCTSTR)patchCmdLine);
  int result;
  if ((result = ::MessageBox(m_hWnd, text, _T("Confirmation"),
                             MB_YESNOCANCEL | MB_ICONQUESTION)) == IDYES) {
    if (_tchdir(m_DestDir) == 0) {
      ...
      if ((m_Display->GetLineCount() == 0) ||
        ...
        CString s;
        s.Format(
            _T("Nothing was patched.\r\nIf this is not the expected ")
            _T("result:\r\n- check if the good patch.exe is in %s\r\n- check ")
            _T("if %s exists (generated by previous diff)\r\n- check if C:\\ ")
            _T("has enough free space and access rights to write a file."),
            TEMP_DIFF_FILE);
        displayMessage(s);
      } else {
...
void CBranch_patcherDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  if (m_Display) {
    ...
    CRect dispRect;
    m_Display->MoveWindow(20, 180, cltRect.right - 40, cltRect.bottom - 200,
                          true);
  }
}
...
void CBranch_patcherDlg::extractDirTokens() {
  ...
  c1 = 0;
  c2 = 0;
  while ((c1 < m_SrcDir.GetLength()) && (c2 < m_DestDir.GetLength()) &&
         (m_SrcDir[c1] == m_DestDir[c2])) {
    ++c1;
    ++c2;
  }
  ...
  /*if ( hasTokens() )
  {
      text.Format( "The two branch tokens '%s' and '%s' are now stored", Token1,
  Token2 );
      ::MessageBox( m_hWnd, text, "Tokens found", MB_OK | MB_ICONINFORMATION );
      return;
  }*/
  // notfound:
  //::MessageBox( m_hWnd, "Tokens not found in the directories", "Extracting
  //:tokens", MB_OK | MB_ICONEXCLAMATION );
}
...
void CBranch_patcherDlg::loadConfiguration() {
  ...
  CvsDiffDirLevel = theApp.GetProfileInt(
      _T(""), _T("CvsDiffDirLevel"),
      1); // 0 for old version of CVS, 1 for new version of CVS
}
...
void CBranch_patcherDlg::OnButtonExtractTokens() {
  if (!EnteringTokens) {
    ...
    ((CButton *)GetDlgItem(IDC_ButtonExtractTokens))
...
void CBranch_patcherDlg::displayTokens() {
  ((CButton *)GetDlgItem(IDC_ButtonClearTokens))
      ->EnableWindow(hasTokens() ? TRUE : FALSE);
  if (hasTokens()) {

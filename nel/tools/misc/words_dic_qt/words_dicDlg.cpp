...
void CWords_dicDlg::initDialog() {
  ...
  QPixmap brushPixmap =
      QPixmap(":/nel.png")
          .scaled(QSize(85, 60), Qt::KeepAspectRatio, Qt::SmoothTransformation);
  ...
  // Loading some items
  splash->showMessage("Please wait while loading dictionary...       ",
                      Qt::AlignVCenter | Qt::AlignRight);

  if (!Dico.init()) {
    QMessageBox::information(this, "Error Loading Dic",
                             "Can't init dictionary, see reason in log.log");
  }
  ...
  ui.statusLabel->setText(
      "Tip: ^ and $ can be used to represent the start and the end of string");
}
...
void CWords_dicDlg::lookUp(const std::string &inputStr, const bool showAll) {
  ...
  if (resultVec.empty()) {
    ...
    QStringList list;
    for (CVectorSString::const_iterator ivs = resultVec.begin();
         ivs != resultVec.end(); ++ivs) {
    ...
    QString s =
        QString("%1 results found for \"%2\".%3")
            .arg(list.size())
            .arg(QString(inputStr.c_str()))
            .arg(lvlRemoved ? " Results containing \"lvl\" not shown" : "");
...
void CWords_dicDlg::clear() { ui.resultsListWidget->clear(); }
...
void CWords_dicDlg::on_lookUpEdit_returnPressed() { on_findButton_clicked(); }
...
void CWords_dicDlg::on_fileListButton_clicked() { getFileList(); }

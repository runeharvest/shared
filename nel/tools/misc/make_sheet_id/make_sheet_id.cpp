...
// manageFile
// void manageFile( WIN32_FIND_DATA& findFileData, list<string>& dirs, string&
// currentDir );
...
void displayHelp() {
  ...
  printf("Usage: make_sheet_id -c<config file> -o<input/output file> [-k] [-e] "
         "[<start directory>] [<directory2>] ...\n");
  printf("-k : clean unwanted types from input\n");
  printf("-e : dump the list of extensions\n");
  printf("--remove-duplicates : WARNING! remove duplicate sheets, keep first "
         "on the list\n");
...
void readFormId(string &outputFileName) {
  ...
  // remove integer file extensions (created by CVS) and init FileTypeToId
  // (associates the form type to the form type id)
  ...
  for (itIF = IdToForm.begin(); itIF != IdToForm.end();) {
    ...
    if (isDuplicate && keepDuplicates) {
      nlwarning("Duplicate sheet (id:%d; name:'%s')", (*itIF).first.Id,
                (*itIF).second.c_str());
      isDuplicate = false;
    }

    if (isDuplicate || (*itIF).second.empty() || (*itIF).second == "." ||
        (*itIF).second == ".." || (*itIF).second[0] == '_' ||
        (*itIF).second.find(".#") == 0) {
      if (isDuplicate)
        nlwarning("Removed duplicate (id:%d; name:'%s')", (*itIF).first.Id,
                  (*itIF).second.c_str());
      ...
      if (getFileType((*itIF).second, fileType)) {
        ...
        nlwarning("Unknown file type for the file : %s",
                  (*itIF).second.c_str());
      }
      ++itIF;
    }
  ...
  // init TypeToLastId (associates the type id to the last index used for this
  // type)
  for (itIF = IdToForm.begin(); itIF != IdToForm.end(); ++itIF) {
...
strcmp("..",findFileData.cFileName) && strcmp("CVS",findFileData.cFileName))
    {
...
void addId(string fileName) {
  ...
  if (itFI == FormToId.end()) {
    ...
    map<string, string>::iterator itDupCheck =
        FormDupCheck.find(toLowerAscii(fileName));
    if (itDupCheck != FormDupCheck.end()) {
      // get mixed-case entry
      itFI = FormToId.find(itDupCheck->second);
      if (itFI != FormToId.end()) {
        TFormId formId = itFI->second;
        nlinfo("File renamed %d '%s' to '%s'", formId.Id,
               itDupCheck->second.c_str(), fileName.c_str());
  ...
  if (itFI == FormToId.end()) {
    // double check : if file not found we check with lower case version of
    // filename
    map<string, TFormId>::iterator itFILwr =
        FormToId.find(toLowerAscii(fileName));
    if (itFILwr != FormToId.end()) {
      nlwarning("Trying to add %s but the file %s is already known ! becareful "
                "with lower case and upper case.",
                fileName.c_str(), toLowerAscii(fileName).c_str());
      NbFilesDiscarded++;
      return;
    }
    ...
    if (getFileType(fileName, fileType)) {
      ...
      if (itFTI == FileTypeToId.end()) {
        ...
        if (firstFreeFileTypeId == -1) {
          ...
          nlinfo("Adding file type '%s' with id %d", fileType.c_str(),
                 firstFreeFileTypeId);
          NbTypesAdded++;
        }
      }
      ...
      nlinfo("Adding file '%s' id %d with type '%s' id %d", fileName.c_str(),
             fid.FormIDInfos.Id, fileType.c_str(), fid.FormIDInfos.Type);
      NbFilesAdded++;
    } else {
      // nlinfo("Unknown file type for the file : '%s' --> not
...
void display() {
  ...
  for (it1 = IdToForm.begin(); it1 != IdToForm.end(); ++it1) {
    nldebug("type: %d id: %d file: %s", (*it1).first.FormIDInfos.Type,
            (*it1).first.FormIDInfos.Id, (*it1).second.c_str());
  }
...
int main(int argc, char **argv) {
  ...
  if (!CFile::fileExists(configFileName)) {
    nlinfo("config file '%s' not found, working whithout filter",
  ...
  // get the current associations (read the sheet_id and fill the working
  // structures)
  readFormId(outputFileName);

  // output path
  std::string::size_type lastSeparator =
      CFile::getLastSeparator(outputFileName);
  string outputPath;
  ...
  map<TFormId, string>::iterator it1;
  for (it1 = IdToForm.begin(); it1 != IdToForm.end(); ++it1) {
    // string outputLine = "type: " + toString((*it1).first.FormIDInfos.Type) +"
    // id: " + toString((*it1).first.FormIDInfos.Id) + " file: " + (*it1).second
    // +"\n";
    string outputLine =
        " id: " + toString((*it1).first.Id) + " file: " + (*it1).second + "\n";
    output.serialBuffer((uint8 *)(const_cast<char *>(outputLine.data())),
                        (uint)outputLine.size());
  }

  nlinfo("------------- results ----------------");
  nlinfo("%d files added in '%s'", NbFilesAdded, outputFileName.c_str());
  nlinfo("%d files discarded because they are empty, begin with .# _ and so on",
         NbFilesDiscarded);
  ...
  for (map<string, uint8>::iterator it = FileTypeToId.begin();
       it != FileTypeToId.end(); ++it) {

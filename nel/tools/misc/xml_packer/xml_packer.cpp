...
enum TAction { pack, unpack, undefined };
...
int main(int argc, char *argv[]) {
  ...
  for (uint i = 0; i < uint(argc); ++i) {
    ...
    //				return -1;
    //			}
    //			// use the specified file archive instead of the
    //directory name 			filename = argv[++i];
    //		}
  }
  ...
  while (!dirStack.empty()) {
    ...
    dirStack.pop_back();
    string filename =
        dirName + "/" + getLastDirName(dirName) + "." + DefaultExt;
    switch (action) {
    case pack: {
      printf("Packing directory '%s'...\n", dirName.c_str());
      //				string packFileName =
      //dirName+"/tmp."+DefaultExt;
      ...
      if (CFile::fileExists(packFileName)) {
        breakable {
          ...
          if (!CFile::fileExists(indexFileName) ||
              CFile::getFileModificationDate(indexFileName) < packDate) {
      ...
      if (!validFiles.empty() && needRepack) {
        ...
        for (uint i = 0; i < validFiles.size(); ++i) {
          ...
          needFinalReturn =
              result != NULL ? buffer[strlen(buffer) - 1] != '\n' : true;
          while (result != 0) {
            fputs(buffer, fp);
            result = fgets(buffer, MaxLineSize, subFp);
            needFinalReturn = result != NULL
    ...
    case unpack: {
      ...
      // open the pack file
      //				FILE *fp =
      //nlfopen(dirName+"/tmp."+DefaultExt, "rt");
      ...
      do {
        ...
        if (!fgets(buffer, MaxLineSize, fp)) {
          fclose(fp);
          printf("Error : invalid pack file '%s' at line %u", filename.c_str(),
                 linecount);
          return -1;
        }
        ...
        CSString subFileName =
            parser.leftCrop(sizeof("	<nel:xml_file name=") - 1);
        ...
        if (output == NULL) {
          printf("Error : can not open output file '%s' from pack file '%s'",
                 subFileName.c_str(), filename.c_str());
          exit(-1);
        }

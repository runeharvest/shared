...
class CDfnField {
public:
  explicit CDfnField(const std::string &name)
      : _isAnArray(false), _name(name) {}

  CDfnField(const std::string &name, const bool &isAnArray)
      : _isAnArray(isAnArray), _name(name) {}

  virtual ~CDfnField() {}

  bool operator<(const CDfnField &other) const { return _name < other._name; }

  bool operator==(const CDfnField &other) const { return _name == other._name; }

  const std::string &getName() const { return _name; }

  const bool &isAnArray() const { return _isAnArray; }
...
void setOutputFile(const CSString &filename) {
  ...
  if (Outf == NULL) {
    fprintf(stderr, "Can't open output file '%s' ! aborting.",
            filename.c_str());
    getchar();
    exit(1);
  }
...
void buildFileVector(std::vector<std::string> &filenames,
  ...
  while (!filters.empty()) {
    ...
    switch (filter[0]) {
    case '+':
      in.push_back(filter.leftCrop(1));
      break;
      break;
    case '-':
      out.push_back(filter.leftCrop(1));
      break;
      break;
    default:
  ...
  /*	for (i=0;i<filespec.size();)
      {
          for (j=i;j<filespec.size() && filespec[j]!=' ' &&
     filespec[j]!='\t';j++) {} switch(filespec[i])
          {
          case '+':
     ...
     '-'\n",&(filespec[i])); getchar(); exit(1);
          ...
          i=j;
          while (i<filespec.size() && (filespec[i]==' ' || filespec[i]=='\t'))
     i++; // skip white space
      }
  */
...
void setErrorString(std::string &valueString, const UFormElm::TEval &evaluated,
                    const UFormElm::TWhereIsValue &where) {
  if (evaluated == UFormElm::NoEval) {
    switch (where) {
    case UFormElm::ValueForm:
      valueString = "ValueForm";
      break;
    case UFormElm::ValueParentForm:
      valueString = "ValueParentForm";
      break;
    case UFormElm::ValueDefaultDfn:
      valueString = "ValueDefaultDfn";
      break;
    case UFormElm::ValueDefaultType:
      valueString = "ValueDefaultType";
      break;
    default:
      valueString = "ERR";
    }
...
void scanFiles(const CSString &filespec) {
  ...
  for (uint j = 0; j < filenames.size(); j++) {
    if (NLMISC::CTime::getLocalTime() > last + 5000) {
      ...
      if (j > 0) {
        nlinfo("%.0f%% completed (%d/%d), %d seconds remaining",
               (float)j * 100.0 / filenames.size(), j, filenames.size(),
               (filenames.size() - j) * (last - start) / j / 1000);
      }
    }
    ...
    if (form) {
      ...
      //			std::string s;
      fprintf(Outf, "%s",
              CFile::getFilenameWithoutExtension(filenames[j]).c_str());
      for (uint i = 0; i < fields.size(); i++) {
        ...
        if (fieldForm) {
          if (fieldForm->isArray()) //	if its an array
          {
            ...
            while (arrayIndex < arraySize) {
              if (fieldForm->getArrayValue(valueString, arrayIndex,
                                           fields[i]._evaluated, &where))
            ...
            if (form->getRootNode().getValueByName(
                    valueString, fields[i]._name, fields[i]._evaluated,
                    &where)) // fieldForm->getValue(valueString,fields[i]._evaluated))
              ;              // addQuotesRoundString	(valueString);
            else
              setErrorString(valueString, fields[i]._evaluated, where);
          }
        ...
        //				UFormElm::TWhereIsValue where;
        //
        //				bool result=form->getRootNode
        //().getValueByName(s,fields[i]._name, fields[i]._evaluated,&where); 				if
        //(!result)
        //				{
        ...
        //						{
        //						case
        //UFormElm::ValueForm: s="ValueForm"; break; 						case
        //UFormElm::ValueParentForm: s="ValueParentForm"; break; 						case
        //UFormElm::ValueDefaultDfn: s="ValueDefaultDfn"; break; 						case
        //UFormElm::ValueDefaultType: s="ValueDefaultType"; break; 						default:
        //s="ERR";
        //						}
        //
        //					}
...
void loadSheetPath() {
  ...
  for (uint i = 0; i < inputSheetPaths.size(); ++i) {
    ...
    for (vector<string>::const_iterator ip = pathsToAdd.begin();
         ip != pathsToAdd.end(); ++ip) {
 ...
 *
 */
void fillFromDFN(UFormLoader *formLoader, set<CDfnField> &dfnFields,
                 UFormDfn *formDfn, const string &rootName,
                 const string &dfnFilename) {
  uint i;
  for (i = 0; i != formDfn->getNumEntry(); ++i) {
    ...
    case UFormDfn::EntryVirtualDfn: {
      CSmartPtr<UFormDfn> subFormDfn =
          formLoader->loadFormDfn((entryName + ".dfn").c_str());
      if (!subFormDfn)
        nlwarning("Can't load virtual DFN %s", entryName.c_str());
      else
        fillFromDFN(formLoader, dfnFields, subFormDfn, rootBase + entryName,
                    entryName + ".dfn");
      break;
    }
    ...
    case UFormDfn::EntryType: {
      ...
      // nlinfo( "DFN entry: %s (in %s)", (rootBase + entryName).c_str(),
      // dfnFilename.c_str() );
      break;
    }
    }
 ...
 * - Ensure there is no non-ascii char (such as Excel's special blank crap), set
 * them to ' '.
 */
 ...
 * CSV -> Georges
 */
void convertCsvFile(const string &file, bool generate,
                    const string &sheetType) {
    ...
    for (i = 1; i != fields.size(); ++i) {
      ...
      if (fields[i].empty()) {
        ...
        if (ist == dfnFields.end()) {
          nlinfo("Skipping field #%u (%s, not found in %s DFN)", i,
                 fields[i].c_str(), sheetType.c_str());
          activeFields[i] = false;
        }
      }
    }
  ...
  if (generate) {
    ...
    try {
      ...
      if (letterIndex1 && letterIndex1->asInt() > 0) {
        ...
        nlinfo("Mapping letter #%u (%s) of sheet name to directory",
               dirmapLetterIndex + 1,
               dirmapLetterBackward ? "backward" : "forward");
      }
      ...
      nlinfo("Write empty properties mode: %s",
             WriteEmptyProperties ? "ON" : "OFF");
      ...
      nlinfo("Force insert parents mode: %s",
             ForceInsertParents ? "ON" : "OFF");
  ...
  uint nbNewSheets = 0, nbModifiedSheets = 0, nbUnchangedSheets = 0,
       nbWritten = 0;
  while (!feof(s)) {
    lineBuffer[0] = '\0';
    if (!fgets(lineBuffer, BUFFER_SIZE, s)) {
    ...
    //		map<string, string>::iterator it = inputSheetPathContent.find(
    //CFile::getFilenameWithoutExtension( filebase ) );
    map<string, string>::iterator it =
        inputSheetPathContent.find(CFile::getFilename(filebase));
    ...
    if (it == inputSheetPathContent.end()) {
      ...
      if (!generate) {
        ...
        form = (CForm *)formLoader->loadForm(
            (string("_empty.") + sheetType).c_str());
        if (form == NULL) {
        ...
        if (dirmapLetterIndex != std::numeric_limits<uint>::max()) {
          if (dirmapLetterIndex < filebase.size()) {
            ...
            char c;
            if (dirmapLetterBackward)
              letterIndex = (uint)(filebase.size() - 1 -
                                   (CFile::getExtension(filebase).size() + 1)) -
                            dirmapLetterIndex;
            else
            ...
            if (idm == dirmapDirs.end()) {
              nlinfo("Directory mapping not found for %s (index %u)",
                     filebase.c_str(), letterIndex);
              dirbase.clear(); // put into root
            }
          } else {
            nlerror("Can't map directory with letter #%u, greater than size of "
                    "%s + code",
                    dirmapLetterIndex, filebase.c_str());
          }
        }
    ...
    for (i = 1; i < args.size() && i < fields.size(); ++i) {
      ...
      if (var == "parent") // already case-lowered
      {
        ...
        if ((isNewSheet || ForceInsertParents) && (!parentVals.empty())) {
          // This is slow. Opti: insertParent() should have an option to do it
          ...
          for (uint p = 0; p != parentVals.size(); ++p) {
            string localExtension =
                (parentVals[p].find(addExtension) == string::npos)
                    ? addExtension
                    : "";
            string parentName = parentVals[p] + localExtension;

            CSmartPtr<CForm> parentForm = (CForm *)formLoader->loadForm(
                CFile::getFilename(parentName.c_str()).c_str());
            if (!parentForm) {
      ...
      if (rootForm.getNodeByName(&fieldForm, var)) {
        ...
        while (memberIndex < memberVals.size()) {
          ...
          if (!memberVal.empty()) {
            ...
            if (memberVal == "ValueForm" || memberVal == "ValueParentForm" ||
                memberVal == "ValueDefaultDfn" ||
                memberVal == "ValueDefaultType" || memberVal == "ERR")
              continue;
          }

          //				nlassert(fieldDfn);
          //				virtual bool getEntryFilenameExt (uint
          //entry, std::string &name) const = 0; 				virtual bool getEntryFilename
          //(uint entry, std::string &name) const = 0;
          if (dfnForm) {
            string fileName;
            string fileNameExt;
            bool toto = false;
                                             ...
                                             var)) //	field exists.
            {
              ...
              if (fileName == filenameTyp) {
                ...
                if (!fileNameExt.empty() && fileNameExt != "*.*") {
                  ...
                  if (memberVal.find(extension) ==
                      string::npos) // extension not found.
                  {
                    memberVal = NLMISC::toString("%s.%s", memberVal.c_str(),
                                                 extension.c_str());
                  }
                }
              }
          ...
          if (dfnForm->isAnArrayEntryByName(var)) {
            if (!isNewSheet && fieldForm != NULL) {
              uint arraySize;
              const UFormElm *arrayNode = NULL;
              if (fieldForm->isArray() && fieldForm->getArraySize(arraySize) &&
                  arraySize == memberVals.size()) {
                string test;
            ...
            // nldebug( "%s: %s '%s'", args[0].c_str(), var.c_str(),
            // memberVal.c_str() );
            //  need to put the value at the correct index.
            const std::string fieldName =
                NLMISC::toString("%s[%u]", var.c_str(), currentMemberIndex)
                    .c_str();
            const_cast<UFormElm &>(rootForm).setValueByName(memberVal,
                                                            fieldName);
            isModified = true;
            displayed = true;
          } else {
            if (!isNewSheet) {
              string test;
              if (rootForm.getValueByName(test, var) && test == memberVal) {
                continue;
              }
            }
            // nldebug( "%s: %s '%s'", args[0].c_str(), var.c_str(),
    ...
    if (isNewSheet || displayed) {
      if (WriteSheetsToDisk) {
        ...
        string ext =
            (filename.find(addExtension) == string::npos) ? addExtension : "";
        ...
        if (!output.isOpen()) {
          nlinfo("ERROR! cannot create file path: %s",
                 absoluteFileName.c_str());
        } else {
          form->write(output);
          output.close();
  ...
  nlinfo("%u sheets processed (%u new, %u modified, %u unchanged - %u written)",
         nbNewSheets + nbModifiedSheets + nbUnchangedSheets, nbNewSheets,
         nbModifiedSheets, nbUnchangedSheets, nbWritten);
  UFormLoader::releaseLoader(formLoader);
}
...
void usage(char *argv0, FILE *out) {
  fprintf(out, "\n");
  fprintf(out,
          "Syntax: %s [-p <sheet path>] [-s <field_separator>] [-g <sheet "
          "type>] [-o <output path>] [<script file name> | <csv file name>]",
          argv0);
  fprintf(out, "(-g = generate sheet files, needs template sheet _empty.<sheet "
               "type> and <sheet type>_dirmap.cfg in the current folder");
  fprintf(out, "\n");

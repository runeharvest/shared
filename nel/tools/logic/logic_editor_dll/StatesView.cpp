...
#ifdef _DEBUG
void CStatesView::AssertValid() const { CTreeView::AssertValid(); }

void CStatesView::Dump(CDumpContext &dc) const { CTreeView::Dump(dc); }
#endif //_DEBUG
...
void CStatesView::OnUpdate(CView *pSender, LPARAM lHint, CObject *pHint) {
  ...
  while (pos != NULL) {
    ...
    // if this is the selected state (and there was no selected event), select
    // it)
...
void CStatesView::OnSelchanged(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  CStatePage *pStatePage =
      static_cast<CStatePage *>(pFormView->m_pPropertySheet->GetPage(3));

  if (pDoc->m_states.Lookup(stateName, (void *&)m_pSelectedState)) {
    pFormView->m_pPropertySheet->SetActivePage(pStatePage);

    if (this->m_pSelectedEvent == NULL ||

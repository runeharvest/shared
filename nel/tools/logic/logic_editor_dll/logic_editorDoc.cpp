...
#ifdef _DEBUG
void CLogic_editorDoc::AssertValid() const { CDocument::AssertValid(); }

void CLogic_editorDoc::Dump(CDumpContext &dc) const { CDocument::Dump(dc); }
#endif //_DEBUG
...
BOOL CLogic_editorDoc::changeVarName(CString old, const CString &newName) {
  ...
  // TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO DO TO
  // DO TO DO TO DO TO DO TO DO
  void *p;
...
BOOL CLogic_editorDoc::changeConditionName(CString old,
...
file.close();
  ...
  for (itVar = logicStateMachine.getVariables().begin();
       itVar != logicStateMachine.getVariables().end(); ++itVar) {
    m_variables.AddTail(CString((*itVar).first.c_str()));
  }

  // init the counters
  map<string, CLogicCounter>::const_iterator itCounter;
  for (itCounter = logicStateMachine.getCounters().begin();
       itCounter != logicStateMachine.getCounters().end(); ++itCounter) {
  ...
  for (itCondition = logicStateMachine.getConditions().begin();
       itCondition != logicStateMachine.getConditions().end(); ++itCondition) {
  ...
  for (itState = logicStateMachine.getStates().begin();
       itState != logicStateMachine.getStates().end(); ++itState) {
  ...
  /*
      ...
      // form view
      CEditorFormView *pFormView = static_cast<CEditorFormView *> (
     pChild->m_wndSplitter.GetPane(0,1) ); ASSERT_VALID(pFormView);

...
const CString &CConditionNode::getNodeAsString() const {
  ...
  {
    m_sNodeString.Format(_T("%s %s %g"), LPCTSTR(m_sVariableName),
                         LPCTSTR(m_sOperator), m_dComparand);
  }

  return m_sNodeString;
}

void CConditionNode::changeConditionName(const CString &old,
...
void CCondition::changeConditionName(CString old,
...
//
//-----------------------------------------------------
void cConditionNodeToCLogicConditionNode(
    CConditionNode *conditionNode, CLogicConditionNode *logicConditionNode) {
  ...
  // this node is a logic node
  {
    ...
    case CConditionNode::COMPARISON: {
      logicConditionNode->LogicBlock.Type =
          CLogicConditionLogicBlock::COMPARISON;

      logicConditionNode->LogicBlock.ComparisonBlock.VariableName =
          NLMISC::tStrToUtf8(conditionNode->m_sVariableName);
      logicConditionNode->LogicBlock.ComparisonBlock.Operator =
          NLMISC::tStrToUtf8(conditionNode->m_sOperator);
      logicConditionNode->LogicBlock.ComparisonBlock.Comparand =
          (sint64)conditionNode->m_dComparand;
    } break;

    case CConditionNode::SUB_CONDITION: {
      logicConditionNode->LogicBlock.Type =
          CLogicConditionLogicBlock::SUB_CONDITION;

      logicConditionNode->LogicBlock.SubCondition =
          NLMISC::tStrToUtf8(conditionNode->m_sConditionName);
    } break;
    }
    ...
    for (pos = conditionNode->m_ctSubTree.GetHeadPosition(); pos != NULL;) {
      ...
      cConditionNodeToCLogicConditionNode(pConditionNode,
                                          logicConditionNodeTmp);
...
//
//-----------------------------------------------------
void cConditionToCLogicCondition(CCondition &condition,
...
//
//-----------------------------------------------------
void cLogicConditionNodeToCConditionNode(
    const CLogicConditionNode *logicConditionNode, CConditionNode *node) {
  // terminator node
  ...
  // logic block with condition sub tree
    ...
    case CLogicConditionLogicBlock::COMPARISON: {
      ...
      node->m_sVariableName = CString(
          logicConditionNode->LogicBlock.ComparisonBlock.VariableName.c_str());
      node->m_sOperator = CString(
          logicConditionNode->LogicBlock.ComparisonBlock.Operator.c_str());
      node->m_dComparand =
          (double)logicConditionNode->LogicBlock.ComparisonBlock.Comparand;
    }; break;

    case CLogicConditionLogicBlock::SUB_CONDITION: {
      node->m_type = CConditionNode::SUB_CONDITION;
      node->m_sConditionName =
          CString(logicConditionNode->LogicBlock.SubCondition.c_str());
    }; break;
    ...
    for (itNode = logicConditionNode->_Nodes.begin();
         itNode != logicConditionNode->_Nodes.end(); ++itNode) {
...
//
//-----------------------------------------------
void cLogicConditionToCCondition(const CLogicCondition &logicCondition,
  ...
  // condition tree
  vector<CLogicConditionNode>::const_iterator itNode;
  for (itNode = logicCondition.Nodes.begin();
       itNode != logicCondition.Nodes.end(); ++itNode) {
    // convert the node

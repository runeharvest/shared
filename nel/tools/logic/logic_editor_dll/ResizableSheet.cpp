...
//		get rid if the "resizable" stuff, just kept the auto arrange of
//controls
//
...
CResizableSheet::CResizableSheet() { Construct(); }

CResizableSheet::CResizableSheet(UINT nIDCaption, CWnd *pParentWnd,
                                 UINT iSelectPage)
    : CPropertySheet(nIDCaption, pParentWnd, iSelectPage) {
  Construct();
}

CResizableSheet::CResizableSheet(LPCTSTR pszCaption, CWnd *pParentWnd,
                                 UINT iSelectPage)
...
int CResizableSheet::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  /*********************************/
  // MODIFIED 14/09/2000 Fleury David, I don't want the user to be able to
  // resize the sheet
...
static UINT _propButtons[] = {IDOK,       IDCANCEL,   ID_APPLY_NOW, IDHELP,
...
void CResizableSheet::ArrangeLayout() {
  ...
  if (m_psh.dwFlags & PSH_WIZARD) // wizard mode
  {
    ...
    // add the control
    DeferWindowPos(hdwp, *pWnd, NULL, objrc.left, objrc.top, objrc.Width(),
                   objrc.Height(), SWP_NOZORDER | SWP_NOACTIVATE);
  } else // tabbed mode
  {
    // get tab control's bottom-right corner
    pWnd = GetTabControl();
    ...
    DeferWindowPos(hdwp, *pWnd, NULL, 0, 0, objrc.Width(), objrc.Height(),
  ...
  for (int i = 0; i < 7; i++) {
    ...
    DeferWindowPos(hdwp, *pWnd, NULL, objrc.left, objrc.top, 0, 0,
...
void CResizableSheet::EnableSaveRestore(LPCTSTR pszSection, LPCTSTR pszEntry,
                                        BOOL bWithPage) {
...
void CResizableSheet::SaveWindowRect() {
  ...
  // always save page
  data.Format(PROFILE_FMT, rc.left, rc.top, rc.right, rc.bottom, wp.showCmd,
              wp.flags, page);
...
void CResizableSheet::LoadWindowRect() {
  ...
  if (_stscanf_s(data, PROFILE_FMT, &rc.left, &rc.top, &rc.right, &rc.bottom,
                 &wp.showCmd, &wp.flags, &page) == 7) {

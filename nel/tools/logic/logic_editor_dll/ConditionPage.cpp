...
void CConditionPage::Update() {
  ...
  // list of variables
  CComboBox *comboBox =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_NODE_VAR_NAME));
...
//
//--------------------------------------------------------
void CConditionPage::addCondition(CLogic_editorDoc *pDoc,
...
void CConditionPage::OnButtonDeleteNode() {
  if (m_pSelectedConditionNode != NULL) {
    ...
    if (parentNode != NULL) {
      ...
      POSITION pos = m_pSelectedCondition->m_ctConditionTree.Find(

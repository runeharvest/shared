...
#ifdef _DEBUG
void CLogicTreeView::AssertValid() const { CView::AssertValid(); }

void CLogicTreeView::Dump(CDumpContext &dc) const { CView::Dump(dc); }
#endif //_DEBUG
...
int CLogicTreeView::OnCreate(LPCREATESTRUCT lpCreateStruct) {
  ...
  // Create two views
  if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CStatesView), CSize(0, 250),
                                pContext)) {
    TRACE0("Failed to create states view in splitter window in LogicTreeView");
    return -1;
  }

  if (!m_wndSplitter.CreateView(1, 0, RUNTIME_CLASS(CConditionsView),
                                CSize(0, 0), pContext)) {
    TRACE0(
        "Failed to create conditions view in splitter window in LogicTreeView");
    return -1;
  }
...
void CLogicTreeView::OnSize(UINT nType, int cx, int cy) {
  ...
  m_wndSplitter.MoveWindow(-xborderWidth, -yborderWidth,
                           cx + (xborderWidth * 2), cy + (yborderWidth * 2));
}

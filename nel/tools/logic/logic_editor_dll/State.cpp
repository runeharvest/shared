...
void cEventToCLogicEvent(CEvent &event, CLogicEvent &logicEvent) {
  ...
  if (logicEvent.EventAction.IsStateChange) {
    ...
    logicEvent.EventAction.StateChange =
        NLMISC::tStrToUtf8(event.m_sStateChange);
  } else {
    /// message destination
    logicEvent.EventAction.EventMessage.Destination =
        NLMISC::tStrToUtf8(event.m_sMessageDestination);

    /// message id
    logicEvent.EventAction.EventMessage.MessageId =
        "LOGIC"; // string( (LPCSTR)event.m_sMessageID ); //TEMP!!!

    /// message arguments
    logicEvent.EventAction.EventMessage.Arguments =
        NLMISC::tStrToUtf8(event.m_sArguments);
  }
...
CState::CState(const CString &name) : m_sName(name) {}
...
BOOL CState::removeEvent(CEvent *event) {
  // go through all the events and remove the first event equal to the one in
  // param
  POSITION pos = m_evEvents.GetHeadPosition();
  POSITION oldpos;
...
void cLogicStateToCState(const CLogicState &logicState, CState &state) {
  ...
  for (itMsg = logicState._EntryMessages.begin();
       itMsg != logicState._EntryMessages.end(); ++itMsg) {
    // TODO
  }
  for (itMsg = logicState._ExitMessages.begin();
       itMsg != logicState._ExitMessages.end(); ++itMsg) {
    // TODO
  }

  vector<CLogicEvent>::const_iterator itEvt;
  for (itEvt = logicState._Events.begin(); itEvt != logicState._Events.end();
       ++itEvt) {
    ...
    event->m_sMessageDestination =
        CString((*itEvt).EventAction.EventMessage.Destination.c_str());
    event->m_sMessageID =
        CString((*itEvt).EventAction.EventMessage.MessageId.c_str());
    event->m_sArguments =
        CString((*itEvt).EventAction.EventMessage.Arguments.c_str());

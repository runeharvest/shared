...
inline void CResizablePage::Construct() { m_bInitDone = FALSE; }

CResizablePage::CResizablePage() { Construct(); }
...
void CResizablePage::AddAnchor(HWND wnd, CSize tl_type, CSize br_type) {
  ...
  // add to the list
  m_plLayoutList.AddTail(new Layout(wnd, tl_type, tl_margin, br_type, br_margin,
                                    hscroll, refresh));
}

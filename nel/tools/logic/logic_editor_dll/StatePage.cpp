...
void CStatePage::Update() {
  ...
  // list of conditions
  CComboBox *comboBox =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_STATE_COND_NAMES));
...
void CStatePage::addState(CLogic_editorDoc *pDoc, CState *state) {
  ...
  CComboBox *comboBox =
      static_cast<CComboBox *>(GetDlgItem(IDC_COMBO_STATE_CHANGE));
  ...
  if (pDoc->m_states.Lookup(state->m_sName, pState) == FALSE) {
    // add the new state to the states tree, and to the vector of states in the
    // document
...
void CStatePage::OnButtonAddState() {
  ...
  /// \toto Malkav : check if a state with this name does not already exist in
  /// the document
  CLogic_editorDoc *pDoc =
      static_cast<CLogic_editorDoc *>(pChild->GetActiveDocument());
  ASSERT_VALID(pDoc);
...
void CStatePage::OnButtonStateRemove() {
  // get selected state
  if (m_pSelectedState != NULL) {
    if (AfxMessageBox(_T("Your are about to permanently delete this ")
                      _T("state.\nDoing so will invalidate all references to ")
...
void CStatePage::OnButtonStateApply() {
  ...
  if (m_pSelectedState != NULL) {
    if (m_sStateName.IsEmpty()) {
      AfxMessageBox(
          _T("State name cannot be empty, please enter a valid name"));
      return;
    }

    if (AfxMessageBox(_T("Your are about to change this state name.\nDoing so ")
                      _T("will change all occurence of the old name to the ")
                      _T("new one.\nDo you want to continue anyway ?"),
                      MB_OKCANCEL | MB_DEFBUTTON2) == IDOK) {
      CMainFrame *pFrame = (CMainFrame *)AfxGetApp()->m_pMainWnd;
      CChildFrame *pChild = (CChildFrame *)pFrame->GetActiveFrame();

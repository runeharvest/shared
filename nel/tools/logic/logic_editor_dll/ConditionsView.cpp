...
// The method CreateDragImage is used during drag'n drop to create a drag image.
// The problem is that it only works for a CTreeCtrl with images. The following
// method, CreateDragImageEx, checks whether the CTreeCtrl has a valid normal
// CImageList. If so, it just calls the standard CreateDragImage method and
// returns. If, on the other hand, no valid CImageList is found, a bitmap is
// created based on the size of item rect, and the item text is drawn into it.
// Then a CImageList is created and the bitmap is added to it.
CImageList *CConditionsView::CreateDragImageEx(HTREEITEM hItem) {
  ...
  memDC.FillSolidRect(&rect,
                      RGB(0, 255, 0)); // Here green is used as mask color
...
#ifdef _DEBUG
void CConditionsView::AssertValid() const { CTreeView::AssertValid(); }

void CConditionsView::Dump(CDumpContext &dc) const { CTreeView::Dump(dc); }
#endif //_DEBUG
...
void CConditionsView::OnSelchanged(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  // get the selected item and update the good frame (open the condition edition
  // page in the form too)
  ...
  CEditorFormView *pFormView =
      static_cast<CEditorFormView *>(pChild->m_wndSplitter.GetPane(0, 1));
  CConditionPage *pCondPage =
      static_cast<CConditionPage *>(pFormView->m_pPropertySheet->GetPage(2));

  if (pDoc->m_conditions.Lookup(condName, (void *&)m_pSelectedCondition)) {
    pFormView->m_pPropertySheet->SetActivePage(pCondPage);

    if (pCondPage->m_pSelectedConditionNode != m_pSelectedNode ||
        m_pSelectedNode == NULL) {
...
void CConditionsView::OnBeginrdrag(NMHDR *pNMHDR, LRESULT *pResult) {
  ...
  *pResult = 0;
  ...
  m_hitemDrop = NULL;
  m_pDragImage =
      CreateDragImageEx(m_hitemDrag); // get the image list for dragging
...
void CConditionsView::OnRButtonUp(UINT nFlags, CPoint point) {
  ...
  if (m_bRDragging) {
    ...
    // if the target is a condition and the dragged item is a node, do not allow
    // '..as sibling' operations
    CConditionNode *pNode;
    if (m_mapItemToNode.Lookup(m_hitemDrag, pNode) &&
        !m_mapItemToNode.Lookup(m_hitemDrop, pNode)) {

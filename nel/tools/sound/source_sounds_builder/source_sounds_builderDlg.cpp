 ...
 *
 */
void CSource_sounds_builderDlg::OnBeginlabeleditTree1(NMHDR *pNMHDR,
                                                      LRESULT *pResult) {
 ...
 *
 */
void CSource_sounds_builderDlg::OnEndlabeleditTree1(NMHDR *pNMHDR,
                                                    LRESULT *pResult) {
  TV_DISPINFO *pTVDispInfo = (TV_DISPINFO *)pNMHDR;
 ...
 *
 */
void CSource_sounds_builderDlg::OnSelchangedTree1(NMHDR *pNMHDR,
                                                  LRESULT *pResult) {
  ...
  if ((pNMTreeView->itemNew.hItem != NULL) &&
      (pNMTreeView->itemNew.hItem != m_Tree.GetRootItem())) {
 ...
 *
 */
void CSource_sounds_builderDlg::OnDeleteitemTree1(NMHDR *pNMHDR,
                                                  LRESULT *pResult) {
  NM_TREEVIEW *pNMTreeView = (NM_TREEVIEW *)pNMHDR;
...
void CSource_sounds_builderDlg::OnSave() {
  ...
  // Prompt filename
  CFileDialog savedlg(false, "nss", _Filename, OFN_OVERWRITEPROMPT,
                      "NeL Source Sounds (*.nss)|*.nss||", this);
  if (savedlg.DoModal() == IDOK) {
    ...
    try {
      ...
      if (!duplicates.empty()) {
        CString s;
        s.Format("Warning: the following names are duplicates. The first "
                 "occurence of each one was not written in the output file. "
                 "Correct the names and save again:\n\n%s",
                 duplicates.c_str());
      ...
      if (!blanksounds.empty()) {
        CString s;
        s.Format(
            "Warning: the following sounds have no wave file specified:\n\n%s",
            blanksounds.c_str());
        AfxMessageBox(s, MB_ICONWARNING);
      }
...
void CSource_sounds_builderDlg::OnLoad() {
  // Prompt filename
  CFileDialog opendlg(true, "nss", "", OFN_HIDEREADONLY,
                      "NeL Source Sounds (*.nss)|*.nss||", this);
  if (opendlg.DoModal() == IDOK) {
    ...
    TSoundMap soundmap;
    try {
      // Loading works even if the wave file are missing because we have called
      // allowMissingWave() before
      ...
      AfxMessageBox("Cannot load: the file does not match the current format "
                    "or version !",
                    MB_ICONSTOP);
    }
...
void CSource_sounds_builderDlg::UpdateTree() {
  ...
  for (i = 0; i != _Sounds.size(); i++) {
    CString s;
    s.Format("%s (%s)", _Sounds[i]->getName().c_str(),
             _Sounds[i]->getFilename().c_str());
...
void CSource_sounds_builderDlg::OnImport() {
  // Prompt filename
  CFileDialog opendlg(true, "nsn", "", OFN_HIDEREADONLY,
                      "NeL Sounds Names (*.nsn; *.txt)|*.nsn; *.txt||", this);
  if (opendlg.DoModal() == IDOK) {
    CWaitCursor waitcursor;
    ...
    while (!fs.eof()) {
      ...
      // Add new name if not already existing (useful for new versions of the
      // names file)
...
void CSource_sounds_builderDlg::OnOK() {
  ...
  _SoundPage->apply();
  //_SoundPage->SetFocus(); // to Exit from label editing if one label in the
  //tree control is in editing mode (in apply())
}
...
void CSource_sounds_builderDlg::OnCancel() {
  ...
  if (!_EditingName) {
    ...
    {
      switch (AfxMessageBox("Save before exiting ?",
...
void CSource_sounds_builderDlg::OnImpDir() {
  // Prompt filename (CMultiFileDialog is a workaround class derived from
  // CFileDialog)
  CMultiFileDialog opendlg(
      true, "wav", "", OFN_ALLOWMULTISELECT | OFN_EXPLORER | OFN_HIDEREADONLY,
  ...
  if (opendlg.DoModal() == IDOK) {
    ...
    if (err != 0) {
      CString s;
      s.Format("%u files on %u could not be loaded. Check the sounds for which "
               "'Mono' or 'Stereo' is not visible",
               err, total);
      AfxMessageBox(s, MB_ICONWARNING);
    }
...
uint CSource_sounds_builderDlg::addSoundAndFile(const string &name) {
  // Add new name if not already existing (useful for new versions of the names
  // file)
 ...
 *
 */
void CSource_sounds_builderDlg::OnKeydownTree1(NMHDR *pNMHDR,
                                               LRESULT *pResult) {

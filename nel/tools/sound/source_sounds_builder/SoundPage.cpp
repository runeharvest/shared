...
const char *PriorityStr[NbSoundPriorities] = {"Highest", "High", "Medium",
                                              "Low"};
...
BOOL CSoundPage::OnInitDialog() {
  ...
  try {
    ...
    CString s;
    s.Format("No sound driver: %s\n\nSound playback will be unavailable",
             e.what());
    AfxMessageBox(s);
    _AudioMixer = NULL;
  }
  ...
  // Cone drawing: make it work with normal and big fonts (depending on system
  // settings)
...
void CSoundPage::getPropertiesFromSound() {
  ...
  ((CSliderCtrl *)GetDlgItem(IDC_SliderGain))
      ->SetPos(ConvertLogScaleToLinearSliderPosTo(m_Gain * 100.0f));
  ((CSliderCtrl *)GetDlgItem(IDC_SliderPitch))
      ->SetPos(ConvertLogScaleToLinearSliderPosTo(m_Pitch * 100.0f));
  UpdateStereo();
  ...
  ((CSliderCtrl *)GetDlgItem(IDC_SliderOuterGain))
      ->SetPos(ConvertLogScaleToLinearSliderPosTo(m_OuterGain * 100.0f));
  UpdateData(false);
...
void CSoundPage::UpdateCurrentSound() {
  ...
  if (!m_Pos3D) {
    _CurrentSound->setProperties(
        string(name), string(m_Filename), m_Gain, m_Pitch,
        (TSoundPriority)(m_Priority.GetCurSel()), m_Looping != 0, m_Pos3D != 0);
  } else {
    _CurrentSound->setProperties(
        string(name), string(m_Filename), m_Gain, m_Pitch,
        (TSoundPriority)(m_Priority.GetCurSel()), m_Looping != 0, m_Pos3D != 0,
        m_MinDist, m_MaxDist, degToRad((float)m_InnerAngleDeg),
        degToRad((float)m_OuterAngleDeg), m_OuterGain);
  }
...
void CSoundPage::apply() {
  if (_CurrentSound) {
    ...
    if (!m_Filename.empty()) {
      CString s = ((CSource_sounds_builderDlg *)GetOwner())->SoundName(_HItem) +
                  " (" + m_Filename + ")";
 ...
 * Reverts the changes
 */
void CSoundPage::OnCancel() { getPropertiesFromSound(); }
...
void CSoundPage::OnChooseFile() {
  // Prompt filename
  CFileDialog opendlg(true, "wav", "", OFN_OVERWRITEPROMPT,
 ...
 *
 */
void CSoundPage::OnRemove() { removeSound(); }

/*
 *
 */
void CSoundPage::removeSound() {
  /*if ( AfxMessageBox( "Are you sure to remove the current sound from the list
...
void CSoundPage::DrawCones() {
  ...
  CRect rect(XCenter - Radius, YCenter - Radius, XCenter + Radius + 1,
             YCenter + Radius + 1);
...
void CSoundPage::OnPaint() {
  ...
  if (m_Pos3D) {
    ...
    uint8 c =
        (uint)(255.0f * (1.0f - (float)(ConvertLogScaleToLinearSliderPosTo(
                                    m_OuterGain * 100.0f)) /
                                    40.0f)); // linear, not logarithmic
    ...
    dc.Pie(XCenter - Radius, YCenter - Radius, XCenter + Radius + 1,
           YCenter + Radius + 1, XCenter - dx, y, XCenter + dx, y);

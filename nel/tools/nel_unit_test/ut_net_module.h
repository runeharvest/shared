...
public:
  ...
  CModuleType0() : PingCount(0), ResponseReceived(0) {
    ModuleUpCalled = 0;
    ModuleDownCalled = 0;
  ...
  std::string buildModuleManifest() const { return "CModuleType0"; }
  ...
  bool onProcessModuleMessage(NLNET::IModuleProxy *senderModuleProxy,
  ...
  void onModuleSocketEvent(NLNET::IModuleSocket *moduleSocket,
...
struct TSecurityType1 : public NLNET::TSecurityData {
  ...
  TSecurityType1(const TCtorParam &param) : NLNET::TSecurityData(param) {}

  void serial(NLMISC::CMemStream &s) { s.serial(SecurityGatewayName); }
};

NLMISC_REGISTER_OBJECT(NLNET::TSecurityData, TSecurityType1, uint8, tst_type1);

// security type 2 data : contains host gateway name and a predefined integer
// value
struct TSecurityType2 : public NLNET::TSecurityData {
  string SecurityGatewayName;
  uint32 IntegerValue;

  TSecurityType2(const TCtorParam &param) : NLNET::TSecurityData(param) {
...
struct TSecurityType3 : public NLNET::TSecurityData {
  ...
  TSecurityType3(const TCtorParam &param) : NLNET::TSecurityData(param) {}
...
struct TSecurityType4 : public NLNET::TSecurityData {
  ...
  TSecurityType4(const TCtorParam &param) : NLNET::TSecurityData(param) {}
...
public:
  CTestSecurity1(const TCtorParam &params) : NLNET::CGatewaySecurity(params) {}
  ...
  void onNewSecurityData(NLNET::CGatewayRoute *from, NLNET::IModuleProxy *proxy,
                         NLNET::TSecurityData *firstSecurityData) {
...
NLMISC_REGISTER_OBJECT(NLNET::CGatewaySecurity, CTestSecurity1, std::string,
                       "TestSecurity1");

/** a sample security plug-in that add type 3 and type 4 security data to local
 * modules,
 */
class CTestSecurity2 : public NLNET::CGatewaySecurity {
public:
  CTestSecurity2(const TCtorParam &params) : NLNET::CGatewaySecurity(params) {}
...
NLMISC_REGISTER_OBJECT(NLNET::CGatewaySecurity, CTestSecurity2, std::string,
                       "TestSecurity2");
...
public:
  ...
  virtual std::string buildModuleManifest() const { return Name; }
...
public:
  ...
  bool lookForModuleProxy(const vector<NLNET::IModuleProxy *> proxList,
                          const std::string &modName) {
    for (uint i = 0; i < proxList.size(); ++i) {
      if (proxList[i]->getModuleName().find(modName) ==
          (proxList[i]->getModuleName().size() - modName.size()))
        return true;
    }
  ...
  NLNET::IModuleProxy *retrieveModuleProxy(NLNET::IModuleGateway *gw,
                                           const std::string &modName) {
  ...
  void tear_down() { NLMISC::CPath::setCurrentPath(_RestorePath.c_str()); }
  ...
  void layer3Autoconnect() {
    ...
    // down/up
    //
    //	We create two gateway, gw1 and gw2, plugged in themselves, then we
    //create 	a layer 3 client on gw1, update the network, then we create the
    //layer 3 server 	on gw2, update the network then check that module are
    //connected.
    //
    //	Then we close the L3 server on gw2, update the network, check that
    //module 	are diconnected and reopen the L3 server and recheck that module
    //are connected.
    //
    //
    ...
    cr.execute("gw1.transportCmd l3c(connect addr=localhost:8062)",
               NLMISC::InfoLog());
    ...
    // update the network (give more time because we must cover the Layer3
    // client reconnection timer)
    for (uint i = 0; i < 40; ++i) {
      mm.updateModules();
      NLMISC::nlSleep(50);
    }
    ...
    cr.execute("gw1.sendPing " + gw2->getModuleFullyQualifiedName(),
               NLMISC::InfoLog());
    cr.execute("gw2.sendPing " + gw1->getModuleFullyQualifiedName(),
               NLMISC::InfoLog());
  ...
  void securityPlugin() {
    ...
    for (uint retry = 0; retry < 2; ++retry) {
      ...
      TEST_ASSERT(st2->SecurityGatewayName ==
                  gw2->getModuleFullyQualifiedName());
      ...
      TEST_ASSERT(st1->SecurityGatewayName ==
                  gw2->getModuleFullyQualifiedName());
  ...
  void distanceAndConnectionLoop() {
    ...
    //
    //                     /---<optional>---\
		//      (l3s) gw1 (l3c)                  (l3s) gw2 (l3c) ------ (l3s) gw3
    //      (l3c) ------ (l3s) gw4
    //	      |            \----------------/ | 	      | | 	      | |
    //         \------------------<optional>-----------------------------------/
    //
  ...
  void firewalling() {
    // check that, with firewall mode enabled, unsafe root can only see
    // protected modules if they initiate the dialog first (i.e only a protected
    // module can send a message to an unsafe module).
    //
    // for this test, we have the following context :
    // 'master' : a gateway that accesses connection on two transports, one
    // 'firewalled', the other one normal 'peer1' : gateway connected to gateway
    // 'master' on a firewalled transport 'peer2' : gateway connected to gateway
    // 'master' on a firewalled transport 'other' : gateway connected to gateway
    // 'master' on a classic transport
    //
    //	peer1 (l3c)-----\
		//			         >-|<- (l3s1/Firewalled) master (l3s2) -----
    //(l3c) other 	peer2 (l3c)-----/
    //
    //  'peer1' and 'peer2' must not see any module except modules that try to
    ...
    // now send the debug 'PING' message from 'other' to 'peer1', and a message
    // from 'master' to 'peer2'
    {
      NLNET::CMessage ping("DEBUG_MOD_PING");
    ...
    cmd = "master.transportOptions l3s1(Firewalled)";
    TEST_THROWS(cr.execute(cmd, NLMISC::InfoLog()),
                NLNET::IModuleGateway::EGatewayFirewallBreak);
  ...
  void peerInvisible() {
    // check that, with peer invisible enable, the peer modules are effectively
    // invisible, and, also, check that other modules, on other route are
    // visible. for this test, we have the following context : 'master' : a
    // gateway that acces connection on to transport, on 'peer invisible', the
    // other normal 'peer1' : gateway connected to gateway 'master' on a peer
    // invisible transport 'peer2' : gateway connected to gateway 'master' on a
    // peer invisible transport 'other' : gateway connected to gateway 'master'
    // on a classic transport
    //
    //	peer1 (l3c)-----\
		//			         >-- (l3s1/PeerInvisible) master (l3s2) -----
    //(l3c) other 	peer2 (l3c)-----/
    //
    ...
    //
    //	When switching the PeerInvisible option to OFF, peer1 and peer2 must see
    //each other
  ...
  void moduleMessaging() {
    ...
    // create two gateway an connect them, plug the gateway on themselves and
    // send a message
    NLNET::IModule *mods = mm.createModule("StandardGateway", "gws", "");
    TEST_ASSERT(mods != NULL);
    NLNET::IModuleGateway *gws = dynamic_cast<NLNET::IModuleGateway *>(mods);
    ...
    // send with ISocket
    socketGws->sendModuleMessage(mods, proxiesS[1]->getModuleProxyId(),
                                 aMessage);
    // update the gateways...
    for (uint i = 0; i < 4; ++i) {
      mm.updateModules();
  ...
  void moduleDisclosure() {
    ...
    TEST_ASSERT(proxies.size() == 1);
    TEST_ASSERT(proxies[0]->getGatewayRoute() != NULL);
    TEST_ASSERT(proxies[0]->getModuleName().find("gws") ==
                proxies[0]->getModuleName().size() - 3);
    ...
    TEST_ASSERT(proxies[1]->getModuleName().find("gwc") ==
                proxies[1]->getModuleName().size() - 3);
    ...
    TEST_ASSERT(proxies.size() == 1);
    TEST_ASSERT(proxies[0]->getGatewayRoute() != NULL);
    TEST_ASSERT(proxies[0]->getModuleName().find("gwc") ==
                proxies[0]->getModuleName().size() - 3);
  ...
  void connectGateways() {
    ...
    // add transport for server mode
    string cmd = "gws.transportAdd L3Server l3s";
    TEST_ASSERT(NLMISC::CCommandRegistry::getInstance().execute(
        cmd, NLMISC::InfoLog()));
    cmd = "gws.transportCmd l3s(open port=6185)";
    TEST_ASSERT(NLMISC::CCommandRegistry::getInstance().execute(
        cmd, NLMISC::InfoLog()));
  ...
  /*	void moduleManagerCommands()
      {
          string cmd;
          // load a library
          cmd = "moduleManager.loadLibrary
     net_module_lib_test/net_module_lib_test";
          TEST_ASSERT(NLMISC::CCommandRegistry::getInstance().execute(cmd,
     NLMISC::InfoLog()));
  ...
  void createLocalGateway() {
    ...
    NLNET::IModule *gateway =
        mm.createModule("LocalGateway", "localGateway", "");
  ...
  /*	void unloadModuleLib()
      {
          ...
          CRefPtr<IModule> module1 = mm.createModule("ModuleType1",
     "TheModule2", "the args"); TEST_ASSERT(module1 != NULL);
          ...
          TModulePtr module2 = mm.createModule("ModuleType1",
     "TheModuleThatCantBeCreated", "the args"); TEST_ASSERT(module2 == NULL);
      }*/

  void failedInit() {
    NLNET::IModuleManager &mm = NLNET::IModuleManager::getInstance();

    NLNET::IModule *module =
        mm.createModule("ModuleType0", "FailingInit", "FAIL");
  ...
  /*	void deleteModule()
      {
          ...
          IModule *module = mm.createModule("ModuleType1", "TheModuleToDelete",
     "the args"); TEST_ASSERT(module != NULL);
  ...
  /*	void createModule()
      {
          ...
          TModulePtr module = mm.createModule("ModuleType1", "TheModule", "the
     args"); TEST_ASSERT(module != NULL);
  ...
  void testModuleInitInfoQuering() {
    ...
    TEST_ASSERT(mif.getParam("sub.subsub.g") ==
                mif.SubParams[2]->SubParams[2]->SubParams[0]);
  }
  ...
  void testModuleInitInfoParsing() {
    ...
    TEST_ASSERT(
        mif.SubParams[2]->SubParams[2]->SubParams[0]->SubParams.size() == 0);
    TEST_ASSERT(mif.SubParams[2]->SubParams[2]->SubParams[0]->ParamName == "g");
    TEST_ASSERT(mif.SubParams[2]->SubParams[2]->SubParams[0]->ParamValue ==
                "bean in box");

    TEST_ASSERT(
        mif.SubParams[2]->SubParams[2]->SubParams[1]->SubParams.size() == 0);
    TEST_ASSERT(mif.SubParams[2]->SubParams[2]->SubParams[1]->ParamName == "z");
    TEST_ASSERT(mif.SubParams[2]->SubParams[2]->SubParams[1]->ParamValue ==
                "2");
  }
};

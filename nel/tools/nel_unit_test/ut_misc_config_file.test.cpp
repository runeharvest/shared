...
TEST_F(CUTMiscConfigFileTest, configWithOptional) {
  ...
  ASSERT_NO_THROW({
    configFile.load(NEL_UNIT_BASE "ut_misc_files/cfg_with_optional.cfg");
  });
...
TEST_F(CUTMiscConfigFileTest, configWithDefine) {
  ...
  ASSERT_NO_THROW(
      { configFile.load(NEL_UNIT_BASE "ut_misc_files/cfg_with_define.cfg"); });
...
TEST_F(CUTMiscConfigFileTest, configWithBadTest) {
  ...
  string fullName = NLMISC::CPath::getFullPath(
      NEL_UNIT_BASE "ut_misc_files/cfg_with_bad_test.cfg", false);
  ...
  EXPECT_THAT(
      warningLogs,
      Contains(StrEq(string("Preprocess: In file ") + fullName +
                     "(6) : Error unrecognized preprocessor command\n")));
  EXPECT_THAT(
      warningLogs,
      Contains(StrEq(string("Preprocess: In file ") + fullName +
                     "(9) : Error found '#endif' without matching #if\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(12) : Error parsing include file command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(13) : Error parsing include file command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(14) : Error parsing optional file command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(15) : Error parsing optional file command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(16) : Error parsing #define command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StrEq(string("Preprocess: In file ") + fullName +
                             "(17) : Error parsing #ifdef command\n")));
  EXPECT_THAT(warningLogs,
              Contains(StartsWith(
                  "Preprocess: Missing 1 closing #endif after parsing")));
}
...
TEST_F(CUTMiscConfigFileTest, configIncludeAndOptional) {
  ...
  string fullName = NLMISC::CPath::getFullPath(
      NEL_UNIT_BASE "ut_misc_files/cfg_with_include_and_optional.cfg", false);
  ...
  EXPECT_THAT(
      warningLogs,
      Contains(StrEq(string("Preprocess: In file ") + fullName +
                     "(2) : Cannot include file 'a_missing_file.cfg'\n")));
}
...
TEST_F(CUTMiscConfigFileTest, reportErrorInSubFiles) {
  ...
  string fullName = NLMISC::CPath::getFullPath(
      NEL_UNIT_BASE "ut_misc_files/cfg_with_error_main.cfg", false);
  string subfullName = NLMISC::CPath::getFullPath(
      NEL_UNIT_BASE "ut_misc_files/cfg_with_error.cfg", false);
  ...
  EXPECT_THAT(
      warningLogs,
      Contains(StrEq(string("CF: Parsing error in file ") + subfullName +
                     " line 18, look in 'debug_cfg_with_error_main.cfg' for a "

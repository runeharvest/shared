...
void cbTest(NLNET::CMessage &msgin, NLNET::TSockId from,
  ...
  if (msgin.getName() == "TEST_50") {
    while (NLMISC::CTime::getLocalTime() - before < 49)
      ; // wait
    maxDuration = 70;
  } else
    maxDuration = 10;
  ...
  if (actualDuration > maxDuration)
    nlerror(
        "The callback cbTest takes too long (%u) for %s, please fix the test",
        (uint)actualDuration, msgin.getName().c_str());
}

static NLNET::TCallbackItem CallbackArray[] = {{"TEST_0", cbTest},
                                               {"TEST_50", cbTest}};
...
public:
  ...
  void sendReceiveUpdate() {
    ...
    msgoutExpectingAnswer0.setType(
        "TEST_0"); // could be passed to the constructor
    ...
    _Server->addCallbackArray(CallbackArray, sizeof(CallbackArray) /
                                                 sizeof(NLNET::TCallbackItem));
    _Client = new NLNET::CCallbackClient();
    _Client->connect(NLNET::CInetAddress("localhost", TestPort1));
    _Client->addCallbackArray(CallbackArray, sizeof(CallbackArray) /
                                                 sizeof(NLNET::TCallbackItem));
    ...
    NbTestReceived = 0;
    for (uint i = 0; i != 20;
         ++i) // send 20 messages that will trigger a time-consuming callback

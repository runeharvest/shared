...
const char *referenceResult[] = {
    "Task1 : 0", "Task2 : 0", "Main : 0",

    "Task1 : 1", "Task2 : 1", "Main : 1",
...
public:
  CTask1(vector<string> &output = result) : Output(output) {}
...
public:
  CTask2(vector<string> &output = result) : Output(output) {}
...
public:
  ...
  void runTasks() {
    ...
    // start the two task (can be done in the loop, but we want the task output
    // first)
    t1.resume();
    t2.resume();

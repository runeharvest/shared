 ...
 */
static float getHeightMapZ(float x, float y, const CZoneLimits &zl,
                           const SExportOptions &options, CBitmap *heightMap1,
                           CBitmap *heightMap2) {
  ...
  if (heightMap1 != NULL) {
    float xc =
        (x - options.CellSize * zl._ZoneMinX) / (options.CellSize * SizeX);
    float yc = 1.0f - ((y - options.CellSize * zl._ZoneMinY) /
                       (options.CellSize * SizeY));
...
int main(int nNbArg, char **ppArgs) {
  ...
  if (nNbArg != 2) {
    ...
    printf("ZFactor1 = 1.0;\n");
    printf(
        "HeightMapFile2 = \"R:/graphics/landscape/ligo/jungle/noise.tga\";\n");
    printf("ZFactor2 = 0.5;\n");
  ...
  // Get all ig files in the input directory and elevate to the z of the double
  // heightmap
  ...
  if (!options.HeightMapFile1.empty()) {
    ...
    try {
      ...
      if (inFile.open(options.HeightMapFile1)) {
        ...
        outString(toString("Couldn't not open %s: heightmap 1 map ignored",
                           options.HeightMapFile1.c_str()));
        delete HeightMap1;
        HeightMap1 = NULL;
      }
    } catch (const Exception &e) {
      outString(toString("Cant load height map : %s : %s",
                         options.HeightMapFile1.c_str(), e.what()));
      delete HeightMap1;
      HeightMap1 = NULL;
    }
  }
  ...
  if (!options.HeightMapFile2.empty()) {
    ...
    try {
      ...
      if (inFile.open(options.HeightMapFile2)) {
        ...
        outString(toString("Couldn't not open %s: heightmap 2 map ignored",
                           options.HeightMapFile2.c_str()));
        delete HeightMap2;
        HeightMap2 = NULL;
      }
    } catch (const Exception &e) {
      outString(string("Cant load height map : ") + options.HeightMapFile2 +
                " : " + e.what() + "\n");
      delete HeightMap2;
      HeightMap1 = NULL;
    }
  ...
  CPath::getPathContent(options.InputIGDir, false, false, true,
                        vAllFilesUnfiltered);
  ...
  for (uint32 i = 0; i < vAllFiles.size(); ++i) {
    ...
    if (pIG != NULL) {
      ...
      if (IA.empty() && PLN.empty() && Portals.empty() && Clusters.empty())
        continue;
      ...
      for (k = 0; k < IA.size(); ++k) {
        ...
        IA[k].Pos.z += getHeightMapZ(instancePos.x, instancePos.y, zl, options,
      ...
      for (k = 0; k < PLN.size(); ++k) {
        ...
        PLN[k].setPosition(PLN[k].getPosition() +
                           getHeightMapZ(lightPos.x, lightPos.y, zl, options,
                                         HeightMap1, HeightMap2) *
                               CVector::K);
      }
      ...
      for (k = 0; k < Portals.size(); ++k) {
        ...
        meanPos += vGlobalPos;
        float z = getHeightMapZ(meanPos.x, meanPos.y, zl, options, HeightMap1,
                                HeightMap2);
        for (l = 0; l < portal.size(); ++l) {
          portal[l].z += z;
        }
      ...
      for (k = 0; k < Clusters.size(); ++k) {
        ...
        float z = getHeightMapZ(clusterPos.x, clusterPos.y, zl, options,
                                HeightMap1, HeightMap2);
      ...
      std::string outFilePath =
          CPath::standardizePath(options.OutputIGDir, true) +
          CFile::getFilename(vAllFiles[i]);

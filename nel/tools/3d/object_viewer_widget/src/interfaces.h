...
public:
  ...
  virtual void saveScreenshot(const std::string &nameFile, bool jpg, bool png,
                              bool tga) = 0;
  virtual bool loadMesh(const std::string &meshFileName,
                        const std::string &skelFileName) = 0;
  ...
  virtual void setCamera(NL3D::UScene *scene, NLMISC::CAABBox &bbox,
                         NL3D::UTransform &entity, bool high_z) = 0;
  virtual bool setupLight(const NLMISC::CVector &position,
                          const NLMISC::CVector &direction) = 0;

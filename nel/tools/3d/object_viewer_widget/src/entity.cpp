...
CEntity::CEntity(void)
    : _Name("<Unknown>"), _FileNameShape(""), _FileNameSkeleton(""),
      _inPlace(false), _incPos(false), _Instance(NULL), _Skeleton(NULL),
...
CEntity::~CEntity(void) {
  if (_PlayList != NULL) {
    ...
    CObjectViewerWidget::objViewWid().getPlayListManager()->deletePlayList(
        _PlayList);
    _PlayList = NULL;
  }
  if (_AnimationSet != NULL) {
    CObjectViewerWidget::objViewWid().getDriver()->deleteAnimationSet(
        _AnimationSet);
    _AnimationSet = NULL;
  }
  if (!_Skeleton.empty()) {
...
void CEntity::loadAnimation(std::string &fileName) {
  uint id = _AnimationSet->addAnimation(
      fileName.c_str(), CFile::getFilenameWithoutExtension(fileName).c_str());
...
void CEntity::loadSWT(std::string &fileName) {
  uint id = _AnimationSet->addSkeletonWeight(
      fileName.c_str(), CFile::getFilenameWithoutExtension(fileName).c_str());
...
void CEntity::addTransformation(CMatrix &current, UAnimation *anim, float begin,
                                float end, UTrack *posTrack,
                                UTrack *rotquatTrack, UTrack *nextPosTrack,
                                UTrack *nextRotquatTrack, bool removeLast) {
  // In place ?
  if (_inPlace) {
    // Just identity
    current.identity();
...
void CEntity::animatePlayList(NL3D::TAnimationTime time) {
  if (!_PlayListAnimation.empty()) {
    // Animation index
    uint id =
        _AnimationSet->getAnimationIdByName(_PlayListAnimation[0].c_str());
    ...
    while (time >= endTime) {
      ...
      if (index < _PlayListAnimation.size()) {
        id = _AnimationSet->getAnimationIdByName(
            _PlayListAnimation[index].c_str());
...
void CEntity::animateChannelMixer() {
  for (uint i = 0; i < NL3D::CChannelMixer::NumAnimationSlot; i++) {
    if (_SlotInfo[i].Enable) {
      ...
      uint skelId =
          _AnimationSet->getSkeletonWeightIdByName(_SlotInfo[i].Skeleton);
      ...
      _PlayList->setStartWeight(i, _SlotInfo[i].StartBlend,
                                _SlotInfo[i].StartTime);

...
public:
  CSlotInfo()
      : Animation("empty"), Skeleton("empty"), Offset(0), StartTime(0),
        EndTime(0), StartBlend(1), EndBlend(1), Smoothness(1), SpeedFactor(1),
        ClampMode(0), SkeletonInverted(false), Enable(true){};
...
public:
  ...
  struct SAnimationStatus {
    ...
    SAnimationStatus()
        : LoopAnim(false), PlayAnim(false), CurrentTimeAnim(0), StartAnim(0),
...
private:
  ...
  void addTransformation(NLMISC::CMatrix &current, NL3D::UAnimation *anim,
                         float begin, float end, NL3D::UTrack *posTrack,
                         NL3D::UTrack *rotquatTrack, NL3D::UTrack *nextPosTrack,
                         NL3D::UTrack *nextRotquatTrack, bool removeLast);

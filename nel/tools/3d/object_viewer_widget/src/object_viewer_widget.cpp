...
typedef bool (*winProc)(NL3D::IDriver *driver, HWND hWnd, UINT message,
...
CObjectViewerWidget::CObjectViewerWidget(QWidget *parent)
    : _isGraphicsInitialized(false), _isGraphicsEnabled(false), _Driver(NULL),
      _Light(0), _phi(0), _psi(0), _dist(2), _CameraFocal(75),
      _CurrentInstance(""), _BloomEffect(false), _Scene(0), QWidget(parent) {
...
CObjectViewerWidget::~CObjectViewerWidget() { release(); }
...
void CObjectViewerWidget::init() {
  ...
  // NLMISC::CConfigFile::CVar v =
  // Modules::config().getConfigFile().getVar("BackgroundColor");
  //_BackgroundColor = CRGBA(v.asInt(0), v.asInt(1), v.asInt(2));
  _BackgroundColor = CRGBA(255, 255, 255);

  // set graphics driver from config
  // NLMISC::CConfigFile::CVar v2 =
  // Modules::config().getConfigFile().getVar("GraphicsDriver"); Choose driver
  // opengl to work correctly under Linux example
...
void CObjectViewerWidget::updateRender() {
  ...
  // QModelIndex index =
  // _dirModel->setRootPath("D:/Dev/Ryzom/code/ryzom/common/data_leveldesign/leveldesign");
  ...
  if (isVisible()) {
    ...
    // 06. Update Entities (movement, do after possible tp from incoming
    // messages etc)
    //      - Move other entities
    //      - Update self entity
    //      - Move bullets
    ...
    //      - Update entities
    //      - Update move container (swap with Update entities? todo: check
    //      code!)
    //      - Update bullets
    // ...
...
void CObjectViewerWidget::updateInitialization(bool visible) {
  ...
  do {
    ...
    // bool wantLandscape = wantGraphics && m_IsGraphicsInitialized &&
    // isLandscapeEnabled;

    // .. stuff that depends on other stuff goes on top to prioritize
    // deinitialization
...
void CObjectViewerWidget::saveScreenshot(const std::string &nameFile, bool jpg,
                                         bool png, bool tga) {
  // H_AUTO2
...
bool CObjectViewerWidget::loadMesh(const std::string &meshFileName,
...
void CObjectViewerWidget::setBackgroundColor(NLMISC::CRGBA backgroundColor) {
  ...
  // Modules::config().getConfigFile().getVar("BackgroundColor").setAsInt(_BackgroundColor.R,
  // 0);
  // Modules::config().getConfigFile().getVar("BackgroundColor").setAsInt(_BackgroundColor.G,
  // 1);
  // Modules::config().getConfigFile().getVar("BackgroundColor").setAsInt(_BackgroundColor.B,
  // 2);
}
...
void CObjectViewerWidget::setGraphicsDriver(bool Direct3D) {
  ...
  // if (_Direct3D)
  // Modules::config().getConfigFile().getVar("GraphicsDriver").setAsString("Direct3D");
  // else
  // Modules::config().getConfigFile().getVar("GraphicsDriver").setAsString("OpenGL");
}

void CObjectViewerWidget::setSizeViewport(uint16 w, uint16 h) {
  _Scene->getCam().setPerspective(_CameraFocal * (float)Pi / 180.f,
                                  (float)w / h, 0.1f, 1000);
}

void CObjectViewerWidget::setCurrentObject(const std::string &name) {
  if ((_Entities.count(name) != 0) || (name.empty()))
    _CurrentInstance = name;
  else
    nlerror("Entity %s not found", name.c_str());
...
void CObjectViewerWidget::setCamera(NL3D::UScene *scene, CAABBox &bbox,
                                    UTransform &entity, bool high_z) {
  ...
  // scene->getCam().setPerspective(_CameraFocal * (float)Pi/180.f, (float)w/h,
  // 0.1f, 1000);
...
bool CObjectViewerWidget::setupLight(const CVector &position,
  ...
  _Light->setupPointLight(light_ambiant, light_diffuse, light_specular,
                          position, direction + light_direction);
...
QIcon *CObjectViewerWidget::saveOneImage(string shapename) {
  ...
  Scene->getCam().setPerspective(_CameraFocal * (float)Pi / 180.f,
                                 (float)output_width / output_height, 0.1f,
                                 1000);

  string filename =
      CPath::standardizePath("") + toString("%s.%s", shapename.c_str(), "png");
...
bool CObjectViewerWidget::nativeEvent(const QByteArray &eventType,
                                      void *message, long *result) {
  if (getDriver() && getDriver()->isActive()) {
    NL3D::IDriver *driver =
        dynamic_cast<NL3D::CDriverUser *>(getDriver())->getDriver();
    if (driver) {

...
const CIndexBuffer *getRdrPassPrimitiveBlock(const CMeshGeom *mesh, uint lodId,
                                             uint renderPass) {
  return &(mesh->getRdrPassPrimitiveBlock(lodId, renderPass));
}

const CIndexBuffer *getRdrPassPrimitiveBlock(const CMeshMRMGeom *mesh,
                                             uint lodId, uint renderPass) {
  return &(mesh->getRdrPassPrimitiveBlock(lodId, renderPass));
}

// ***************************************************************************

const CIndexBuffer *getRdrPassPrimitiveBlock(const CMeshMRMSkinnedGeom *mesh,
                                             uint lodId, uint renderPass) {
...
int main(int argc, char *argv[]) {
  ...
  if (ProcessMeshMRMSkinned(filename, shapeMesh))
    return 0;
  if (ProcessMeshMRM(filename, shapeMesh))
    return 0;
  if (ProcessMesh(filename, shapeMesh))
    return 0;
...
bool ProcessMeshMRMSkinned(const std::string &filename, IShape *shapeMesh) {
  ...
  // **** First, for the best lod indicate what vertex is used or not. Also
  // index geomorphs to know what real vertex is used
  ...
  // use a map to remove duplicates (because of UV/normal discontinuities
  // before!!)
  ...
  for (i = 0; i < meshIn->getNbRdrPass(lodId); ++i) {
    ...
    for (size_t pass = 0; pass < shadowTriangles.size(); pass += 3) {
      ofs << "f " << shadowTriangles[pass] + 1 << "/"
          << shadowTriangles[pass] + 1 << "/" << shadowTriangles[pass] + 1
          << " ";
      ofs << shadowTriangles[pass + 1] + 1 << "/"
          << shadowTriangles[pass + 1] + 1 << "/"
          << shadowTriangles[pass + 1] + 1 << " ";
      ofs << shadowTriangles[pass + 2] + 1 << "/"
          << shadowTriangles[pass + 2] + 1 << "/"
          << shadowTriangles[pass + 2] + 1 << endl;
    }
...
bool ProcessMesh(const std::string &filename, IShape *shapeMesh) {
  ...
  for (i = 0; i < meshIn->getNbRdrPass(0); ++i) {
    ...
    for (size_t pass = 0; pass < shadowTriangles.size(); pass += 3) {
      ofs << "f " << shadowToWelded[shadowTriangles[pass]] + 1 << "/"
          << shadowTriangles[pass] + 1 << "/" << shadowTriangles[pass] + 1
          << " ";
      ofs << shadowToWelded[shadowTriangles[pass + 1]] + 1 << "/"
          << shadowTriangles[pass + 1] + 1 << "/"
          << shadowTriangles[pass + 1] + 1 << " ";
      ofs << shadowToWelded[shadowTriangles[pass + 2]] + 1 << "/"
          << shadowTriangles[pass + 2] + 1 << "/"
          << shadowTriangles[pass + 2] + 1 << endl;
    }

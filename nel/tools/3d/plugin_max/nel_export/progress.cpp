...
INT_PTR CALLBACK CalculatingDialogCallback(
  ...
  CProgressBar *pClass =
      (CProgressBar *)GetWindowLongPtr(hwndDlg, GWLP_USERDATA);
  ...
  case WM_PAINT: {
    ...
    SendMessage(GetDlgItem(hwndDlg, IDC_PROGRESS1), PBM_SETPOS,
                (int)(pClass->rRatioCalculated * 100), 0);

    if (pClass->rRatioCalculated > 0.0) {
      double TimeLeft =
          (TimeCurrent - pClass->rTimeBegin) * (1.0 - pClass->rRatioCalculated);
      _stprintf(temp, _T("Time remaining : %02d h %02d m %02d s"),
                ((uint32)TimeLeft) / 3600, (((uint32)TimeLeft) / 60) % 60,
                (((uint32)TimeLeft)) % 60);
    ...
    SendMessage(GetDlgItem(hwndDlg, IDC_STATICINFO), WM_SETTEXT, 0,
                (LPARAM)MaxTStrFromUtf8(all).data());
  } break;
...
// -----------------------------------------------------------------------------------------------
CProgressBar::CProgressBar() { hWndProgress = NULL; }
...
void CProgressBar::initProgressBar(sint32 nNbMesh, Interface &ip) {
  ...
  hWndProgress = CreateDialogParam(CNelExportDesc.HInstance(),
                                   ...
                                   NULL, // ip.GetMAXHWnd(),
                                   CalculatingDialogCallback, (LPARAM)this);
}
...
// -----------------------------------------------------------------------------------------------
bool CProgressBar::isCanceledProgressBar() { return bCancelCalculation; }

...
// --------------------
// OLD VERSION
// This version handled a group of object at one time to optimize the
// precalculation of the accelerated world raytrace structure
...
// #define RATIOLIGHTMAP	0.25		// 0.25 meters in real world
// represent 1 Lumel in UV world
...
struct SLMPlane {
  ...
  vector<uint8>
      msk; // 0 - No pixel
           ...
           // 3 - Pixel is exterior in this plane but interior in another of the
           // same smooth group 4 - Pixel is exterior and is extrapolated
...
struct SCubeGrid {
  ...
  // =======================================================================
  void project(CTriangle &tri, CPlane pyr[4], CPlane &gridPlane, sint32 nGridNb,
               SCubeGridCell &cell) {
  ...
  void select(CVector &v) {
    CPlane gp;
    // Get the plane
    if ((-v.z <= v.x) && (v.x <= v.z) && (-v.z <= v.y) && (v.y <= v.z) &&
        (0.0f <= v.z)) {
      nSelGrid = kUp;
      gp.make(CVector(0, 0, 1), CVector(0, 0, 0.5));
    }
    ...
    if ((v.x <= 0.0f) && (v.x <= v.y) && (v.y <= -v.x) && (v.x <= v.z) &&
        (v.z <= -v.x)) {
      nSelGrid = kLeft;
      gp.make(CVector(-1, 0, 0), CVector(-0.5, 0, 0));
    }
    if ((0.0f <= v.x) && (-v.x <= v.y) && (v.y <= v.x) && (-v.x <= v.z) &&
        (v.z <= v.x)) {
      nSelGrid = kRight;
      gp.make(CVector(1, 0, 0), CVector(0.5, 0, 0));
    }
    if ((v.y <= v.x) && (v.x <= -v.y) && (v.y <= 0.0f) && (v.y <= v.z) &&
        (v.z <= -v.y)) {
      nSelGrid = kFront;
      gp.make(CVector(0, -1, 0), CVector(0, -0.5, 0));
    }
    if ((-v.y <= v.x) && (v.x <= v.y) && (0.0f <= v.y) && (-v.y <= v.z) &&
        (v.z <= v.y)) {
      nSelGrid = kBack;
      gp.make(CVector(0, 1, 0), CVector(0, 0.5, 0));
    }
  ...
  SCubeGridCell getSel() { return *itSel; }

  void nextSel() { ++itSel; }

  bool isEndSel() { return (itSel == grids[nSelGrid].end()); }
};
...
// -----------------------------------------------------------------------------------------------
void SortFaceByTextureName(vector<CMesh::CFace *> &AllFaces,
                           CMesh::CMeshBuild *pMB) {
  int i, j;
...
// -----------------------------------------------------------------------------------------------
// TextureNames is an array which indicates the number of faces that follows
// which have the same texture name
void ComputeAreaOfTextureName(vector<sint32> &TextureNames,
...
// -----------------------------------------------------------------------------------------------
void SortFaceByMaterialId(vector<sint32> &FaceGroup,
                          vector<CMesh::CFace *>::iterator ItFaces,
                          sint32 nNbFace) {
  int i, j;
  sint32 nMatID;
...
// -----------------------------------------------------------------------------------------------
// Test if the 2 faces are continuous (same vertex, same normal (if wanted),
// same uv (if wanted))
bool FaceContinuous(CMesh::CFace *pF1, CMesh::CFace *pF2, bool bTestUV = true,
                    bool bTestNormal = true) {
  sint32 i, j;
  sint32 F1c[2] = {-1, -1};
  sint32 F2c[2] = {-1, -1};
  ...
  for (j = 0; j < 3; ++j)
    for (i = 0; i < 3; ++i) {
      if ((pF1->Corner[j].Vertex == pF2->Corner[i].Vertex) &&
          (pF1->Corner[(j + 1) % 3].Vertex ==
           pF2->Corner[(i + 1) % 3].Vertex)) {
        F1c[0] = j;
        F1c[1] = (j + 1) % 3;
        F2c[0] = i;
        F2c[1] = (i + 1) % 3;
  ...
  if (F1c[0] == -1)
    return false;
  // Here we get the vertex continuity between F1c[0] and F2c[0], and, F1c[1]
  // and F2c[1] Is there a normal continuity
  if (bTestNormal)
    for (i = 0; i < 2; ++i) {
      ...
      // is n1 equal to n2 ?
      double epsilon =
          1.0 - (n1 * n2); // theorically n1*n2 equal to 1.0 but epsilon error
      if (epsilon > 0.001)
        return false;
    }
  ...
  if (bTestUV)
    for (i = 0; i < 2; ++i) {
      if ((fabs(pF1->Corner[F1c[i]].Uvws[1].U - pF2->Corner[F2c[i]].Uvws[1].U) >
           0.001) ||
          (fabs(pF1->Corner[F1c[i]].Uvws[1].V - pF2->Corner[F2c[i]].Uvws[1].V) >
           0.001))
        return false;
    }
  return true;
...
// -----------------------------------------------------------------------------------------------
void SortFaceBySMoothGroup(vector<sint32> &FaceGroup,
                           vector<CMesh::CFace *>::iterator ItFaces,
                           sint32 nNbFace) {
...
// -----------------------------------------------------------------------------------------------
void SortFaceByTextureSurface(int offset, int nNbFace,
                              vector<CMesh::CFace *> &AllFaces) {
  int i, j;
  ...
  for (i = 0; i < nNbFace - 1; ++i)
    for (j = i + 1; j < nNbFace; ++j)
      if (AllFaces[i]->MaterialId == AllFaces[j]->MaterialId) {
        ...
        double surfacei = 0.5 * fabs((AllFaces[i]->Corner[1].Uvws[1].U -
                                      AllFaces[i]->Corner[0].Uvws[1].U) *
                                         (AllFaces[i]->Corner[2].Uvws[1].V -
                                          AllFaces[i]->Corner[0].Uvws[1].V) -
                                     (AllFaces[i]->Corner[1].Uvws[1].V -
                                      AllFaces[i]->Corner[0].Uvws[1].V) *
                                         (AllFaces[i]->Corner[2].Uvws[1].U -
                                          AllFaces[i]->Corner[0].Uvws[1].U));
        double surfacej = 0.5 * fabs((AllFaces[j]->Corner[1].Uvws[1].U -
                                      AllFaces[j]->Corner[0].Uvws[1].U) *
                                         (AllFaces[j]->Corner[2].Uvws[1].V -
                                          AllFaces[j]->Corner[0].Uvws[1].V) -
                                     (AllFaces[j]->Corner[1].Uvws[1].V -
                                      AllFaces[j]->Corner[0].Uvws[1].V) *
                                         (AllFaces[j]->Corner[2].Uvws[1].U -
                                          AllFaces[j]->Corner[0].Uvws[1].U));
...
// -----------------------------------------------------------------------------------------------
bool isInTriangleOrEdge(double x, double y, double xt1, double yt1, double xt2,
...
// -----------------------------------------------------------------------------------------------
bool isInTriangle(double x, double y, double xt1, double yt1, double xt2,
                  double yt2, double xt3, double yt3) {
  // Test vector T1X and T1T2
  double sign1 = ((xt2 - xt1) * (y - yt1) - (yt2 - yt1) * (x - xt1));
  // Test vector T2X and T2T3
...
// -----------------------------------------------------------------------------------------------
bool segmentIntersection(double x1, double y1, double x2, double y2, double x3,
...
bool intersectionTriangleSphere(CTriangle &t, CBSphere &s) {
  ...
  CPlane p2;
  p2.make(t.V0, t.V1, t.V0 + n);
  p2.normalize();
  f = p2 * newCenter;
  p2.make(t.V1, t.V2, t.V1 + n);
  p2.normalize();
  float f2 = p2 * newCenter;
  p2.make(t.V2, t.V0, t.V2 + n);
  p2.normalize();
...
// -----------------------------------------------------------------------------------------------
void CreatePiece(vector<uint8> &Piece, sint32 &nSizeX, sint32 &nSizeY,
                 sint32 &nPosX, sint32 &nPosY, float lumx1, float lumy1,
                 float lumx2, float lumy2, float lumx3, float lumy3,
                 uint8 nCol) {
  double minx, miny;
  double maxx, maxy;
  int j, k;
  ...
  // The square interact with the triangle if an edge of the square is cut by an
  // edge of the triangle Or the square is in the triangle

  for (j = 0; j < nSizeY - 1; ++j)
    for (k = 0; k < nSizeX - 1; ++k) {
      // Is the square (j,k) is interacting with the triangle
      // This means : The square contains a point of the triangle (can be done
      // for the 3 points)
      //              The triangle contains a point of the square
      // If so then we have to turn on all the 4 pixels of the square
      if (isInTriangleOrEdge(k + 0.5, j + 0.5, lumx1, lumy1, lumx2, lumy2,
                             lumx3, lumy3) ||
          isInTriangleOrEdge(k + 1.5, j + 0.5, lumx1, lumy1, lumx2, lumy2,
                             lumx3, lumy3) ||
          isInTriangleOrEdge(k + 0.5, j + 1.5, lumx1, lumy1, lumx2, lumy2,
                             lumx3, lumy3) ||
          isInTriangleOrEdge(k + 1.5, j + 1.5, lumx1, lumy1, lumx2, lumy2,
                             lumx3, lumy3)) {
      ...
      if (segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx1, lumy1,
                              lumx2, lumy2) ||
          segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx2, lumy2,
                              lumx3, lumy3) ||
          segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx3, lumy3,
                              lumx1, lumy1) ||

          segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx1, lumy1,
                              lumx2, lumy2) ||
          segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx2, lumy2,
                              lumx3, lumy3) ||
          segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx3, lumy3,
                              lumx1, lumy1) ||

          segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx1, lumy1,
                              lumx2, lumy2) ||
          segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx2, lumy2,
                              lumx3, lumy3) ||
          segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx3, lumy3,
                              lumx1, lumy1) ||

          segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx1, lumy1,
                              lumx2, lumy2) ||
          segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx2, lumy2,
                              lumx3, lumy3) ||
          segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx3, lumy3,
                              lumx1, lumy1)) {
        Piece[k + j * nSizeX] = nCol;
        Piece[1 + k + j * nSizeX] = nCol;
        Piece[k + (1 + j) * nSizeX] = nCol;
  ...
  Piece[((sint32)(lumx1 - 0.5)) + (1 + ((sint32)(lumy1 - 0.5))) * nSizeX] =
      nCol;
  Piece[1 + ((sint32)(lumx1 - 0.5)) + (1 + ((sint32)(lumy1 - 0.5))) * nSizeX] =
      nCol;

  Piece[((sint32)(lumx2 - 0.5)) + ((sint32)(lumy2 - 0.5)) * nSizeX] = nCol;
  Piece[1 + ((sint32)(lumx2 - 0.5)) + ((sint32)(lumy2 - 0.5)) * nSizeX] = nCol;
  Piece[((sint32)(lumx2 - 0.5)) + (1 + ((sint32)(lumy2 - 0.5))) * nSizeX] =
      nCol;
  Piece[1 + ((sint32)(lumx2 - 0.5)) + (1 + ((sint32)(lumy2 - 0.5))) * nSizeX] =
      nCol;

  Piece[((sint32)(lumx3 - 0.5)) + ((sint32)(lumy3 - 0.5)) * nSizeX] = nCol;
  Piece[1 + ((sint32)(lumx3 - 0.5)) + ((sint32)(lumy3 - 0.5)) * nSizeX] = nCol;
  Piece[((sint32)(lumx3 - 0.5)) + (1 + ((sint32)(lumy3 - 0.5))) * nSizeX] =
      nCol;
  Piece[1 + ((sint32)(lumx3 - 0.5)) + (1 + ((sint32)(lumy3 - 0.5))) * nSizeX] =
      nCol;
}

// -----------------------------------------------------------------------------------------------
void ResizeBitmap(vector<uint8> &vBitmap, sint32 &nSizeX, sint32 &nSizeY,
                  sint32 nNewSizeX, sint32 nNewSizeY) {
...
void ResizeBitmap32(CBitmap *pImage, sint32 nNewSizeX, sint32 nNewSizeY) {
  ...
  for (j = 0; j < min(nCurSizeY, nNewSizeY); ++j)
    for (i = 0; i < min(nCurSizeX, nNewSizeX); ++i) {
      vImgTemp[4 * (i + j * nNewSizeX) + 0] =
          vBitmap[4 * (i + j * pImage->getWidth()) + 0];
      vImgTemp[4 * (i + j * nNewSizeX) + 1] =
          vBitmap[4 * (i + j * pImage->getWidth()) + 1];
      vImgTemp[4 * (i + j * nNewSizeX) + 2] =
          vBitmap[4 * (i + j * pImage->getWidth()) + 2];
      vImgTemp[4 * (i + j * nNewSizeX) + 3] =
          vBitmap[4 * (i + j * pImage->getWidth()) + 3];
    }
...
// -----------------------------------------------------------------------------------------------
bool PutPieceInLightMap(vector<uint8> &Piece, sint32 nPieceSizeX,
                        sint32 nPieceSizeY, vector<uint8> &LightMap,
                        sint32 nLightMapSizeX, sint32 nLightMapSizeY,
                        sint32 &nNewPosX, sint32 &nNewPosY) {
  sint32 i, j, a, b;
  bool bGoodPosition;

  if (nPieceSizeX > nLightMapSizeX)
    return false;
  if (nPieceSizeY > nLightMapSizeY)
    return false;
    ...
    for (i = 0; i < (nLightMapSizeX - nPieceSizeX); ++i) {
      ...
      if (bGoodPosition) {
        ...
        for (b = 0; b < nPieceSizeY; ++b) {
          for (a = 0; a < nPieceSizeX; ++a) {
            if (Piece[a + b * nPieceSizeX] != 0)
              LightMap[(i + a) + (j + b) * nLightMapSizeX] =
                  Piece[a + b * nPieceSizeX];
          }
        }
        nNewPosX = i;
        nNewPosY = j;
...
void MapFace(CMesh::CFace *pFace, vector<CVector> &Vertices, float rRatio) {
  CVector V01 =
      Vertices[pFace->Corner[1].Vertex] - Vertices[pFace->Corner[0].Vertex];
  CVector V02 =
      Vertices[pFace->Corner[2].Vertex] - Vertices[pFace->Corner[0].Vertex];
  CVector n = V01 ^ V02;
  n.normalize();

  // Quantize the normal

  // Table of unitary vector with relevant direction to map The I vector
  // represent the plane normal and the J,K vector the U,V vector
  CMatrix QuantizationTbl[3];
  QuantizationTbl[0].identity();
  QuantizationTbl[1].identity();
  QuantizationTbl[1].rotateZ((float)(Pi / 2.0));
  QuantizationTbl[2].identity();
  QuantizationTbl[2].rotateY((float)(Pi / 2.0));
  ...
  // defined by the quantized vector (as the plane normal) and the origin (as a
  // point in the plane)
...
float getUVDist(CUV &UV1, CUV &UV2) {
  return sqrtf((UV2.U - UV1.U) * (UV2.U - UV1.U) +
               (UV2.V - UV1.V) * (UV2.V - UV1.V));
}

// -----------------------------------------------------------------------------------------------
void getLightBuildList(std::vector<SLightBuild> &vectLight, TimeValue tvTime,
                       Interface &ip, INode *node = NULL) {
  if (node == NULL)
    node = ip.GetRootNode();
  ...
  if (obj) {
    ...
    if (obj->SuperClassID() == LIGHT_CLASS_ID) {
      ...
      Modifier *pModifier = CExportNel::getModifier(
          node, Class_ID(NEL_LIGHT_CLASS_ID_A, NEL_LIGHT_CLASS_ID_B));
      if (pModifier != NULL) {
        int bDynamic;
        string sGroup;
        // Get the value of the parameters
        CExportNel::getValueByNameUsingParamBlock2(
            *pModifier, "bDynamic", (ParamType2)TYPE_BOOL, &bDynamic, 0);
        CExportNel::getValueByNameUsingParamBlock2(
            *pModifier, "sGroup", (ParamType2)TYPE_STRING, &sGroup, 0);
        if (bDynamic) {
      ...
      if (maxLight->EvalLightState(tvTime, valid, &ls) == REF_SUCCEED) {
        ...
        */
        ...
        nelLight.setCutoff
        ((float)(NLMISC::Pi*maxLight.GetFallsize(tvTime)/180.f/2.f));
        ...
        if (maxLight.GetUseAtten())
            nelLight.setupAttenuation (maxLight.GetAtten (tvTime, ATTEN_START),
        maxLight.GetAtten (tvTime, ATTEN_END)); else nelLight.setNoAttenuation
        ();
        */

        nelLight.rHotspot =
            (float)(Pi * maxLight->GetHotspot(tvTime) / (2.0 * 180.0));
        nelLight.rFallof =
            (float)(Pi * maxLight->GetFallsize(tvTime) / (2.0 * 180.0));
...
// -----------------------------------------------------------------------------------------------
void getLightBuilds(vector<SLightBuild> &lights, TimeValue tvTime,
                    Interface &ip) {
  SLightBuild amb;
...
// -----------------------------------------------------------------------------------------------
void MoveFaceUV1(vector<CMesh::CFace *>::iterator ItFace, sint32 nNbFace,
                 double rOffsU, double rOffsV) {
  sint32 i, j;
  for (i = 0; i < nNbFace; ++i) {
...
// -----------------------------------------------------------------------------------------------
void MultiplyFaceUV1(vector<CMesh::CFace *>::iterator ItFace, sint32 nNbFace,
                     double rFactor) {
  sint32 i, j;
  for (i = 0; i < nNbFace; ++i) {
    CMesh::CFace *pF = (*ItFace);
...
bool PutFaceUV1InLumelCoord(double rRatioLightMap, vector<CVector> &Vertices,
                            vector<CMesh::CFace *>::iterator ItFace,
                            sint32 nNbFace) {
  sint32 i, j;
  ...
  for (i = 0; i < nNbFace; ++i) {
    ...
    p1.x = pF->Corner[0].Uvws[1].U;
    p1.y = pF->Corner[0].Uvws[1].V;
    p1.z = 0.0f;
    p2.x = pF->Corner[1].Uvws[1].U;
    p2.y = pF->Corner[1].Uvws[1].V;
    p2.z = 0.0f;
    p3.x = pF->Corner[2].Uvws[1].U;
    p3.y = pF->Corner[2].Uvws[1].V;
    p3.z = 0.0f;
  ...
  for (j = 0; j < 3; ++j)
    for (i = 0; i < 3; ++i)
      if (segmentIntersection(
              pF1->Corner[i].Uvws[1].U, pF1->Corner[i].Uvws[1].V,
              pF1->Corner[(i + 1) % 3].Uvws[1].U,
              pF1->Corner[(i + 1) % 3].Uvws[1].V, pF2->Corner[j].Uvws[1].U,
              pF2->Corner[j].Uvws[1].V, pF2->Corner[(j + 1) % 3].Uvws[1].U,
              pF2->Corner[(j + 1) % 3].Uvws[1].V))
        return true;
...
// -----------------------------------------------------------------------------------------------
void SortFaceByPlane(vector<sint32> &FaceGroup,
...
/* // To keep
void SortPlanesBySurface( vector<sint32> &PlaneGroup,
vector<CMesh::CFace*>::iterator ItFace, sint32 nNbFace )
{
    sint32 i, j, k;
    double rMinU = 1000000.0, rMaxU = -1000000.0, rMinV = 1000000.0, rMaxV =
-1000000.0;
    ...
    for( i = 0; i < sizes.size(); ++i )
    {
        ...
        for( j = 0; j < PlaneGroup[i]; ++j )
        {
            ...
            for( k = 0; k < 3; ++k )
            {
                if( rMinU > pF->Corner[k].Uvws[1].U ) rMinU =
pF->Corner[k].Uvws[1].U; if( rMaxU < pF->Corner[k].Uvws[1].U ) rMaxU =
pF->Corner[k].Uvws[1].U; if( rMinV > pF->Corner[k].Uvws[1].V ) rMinV =
pF->Corner[k].Uvws[1].V; if( rMaxV < pF->Corner[k].Uvws[1].V ) rMaxV =
pF->Corner[k].Uvws[1].V;
            }
            ++ItParseI;
        }
        sizes[i].first = i;
...
void CopyPlaneColToBitmap32(CBitmap *pImage, SLMPlane &Plane, sint32 nLayerNb) {
  if ((pImage->getWidth() != Plane.w) || (pImage->getHeight() != Plane.h))
...
void ResizeLMPlane(SLMPlane &Dst, sint32 nNewSizeX, sint32 nNewSizeY) {
  ...
  for (i = 0; i < nNewSizeX * nNewSizeY; ++i)
    for (j = 0; j < 8; ++j) {
      vImgTemp2[i].p[j].R = vImgTemp2[i].p[j].G = vImgTemp2[i].p[j].B =
          vImgTemp2[i].p[j].A = 0.0f;
    }
...
// -----------------------------------------------------------------------------------------------
// Stretch a plane by a given factor 4.0 -> multiply its size by 4 and 0.5 ->
// halves its size
  ...
  if (osFactor > 1.0) // Up
  {
    ...
    for (j = 0; j < nNewSizeY; ++j) {
      ...
      for (i = 0; i < nNewSizeX; ++i) {
        ...
        for (k = 0; k < 8; ++k) {
          vImgTemp2[i + j * nNewSizeX].p[k].R +=
              pPlane->col[((sint32)x) + ((sint32)y) * pPlane->w].p[k].R;
          vImgTemp2[i + j * nNewSizeX].p[k].G +=
              pPlane->col[((sint32)x) + ((sint32)y) * pPlane->w].p[k].G;
          vImgTemp2[i + j * nNewSizeX].p[k].B +=
              pPlane->col[((sint32)x) + ((sint32)y) * pPlane->w].p[k].B;
  ...
  } else // Down
  {
    ...
    for (j = 0; j < pPlane->h; ++j) {
      ...
      for (i = 0; i < pPlane->w; ++i) {
        ...
        for (k = 0; k < 8; ++k) {
          vImgTemp2[((sint32)x) + ((sint32)y) * nNewSizeX].p[k].R +=
              pPlane->col[i + j * pPlane->w].p[k].R;
          vImgTemp2[((sint32)x) + ((sint32)y) * nNewSizeX].p[k].G +=
              pPlane->col[i + j * pPlane->w].p[k].G;
          vImgTemp2[((sint32)x) + ((sint32)y) * nNewSizeX].p[k].B +=
              pPlane->col[i + j * pPlane->w].p[k].B;
...
void CreateLMPlaneFromFace(SLMPlane &Out, CMesh::CFace *pF) {
  ...
  ResizeLMPlane(Out,
                1 + ((sint32)floor(maxx + 0.5)) - ((sint32)floor(minx - 0.5)),
  ...
  Out.msk[1 + ((sint32)(lumx1 - 0.5)) + (1 + ((sint32)(lumy1 - 0.5))) * Out.w] =
      1;
  ...
  Out.msk[1 + ((sint32)(lumx2 - 0.5)) + (1 + ((sint32)(lumy2 - 0.5))) * Out.w] =
      1;
  ...
  Out.msk[1 + ((sint32)(lumx3 - 0.5)) + (1 + ((sint32)(lumy3 - 0.5))) * Out.w] =
      1;
}

// -----------------------------------------------------------------------------------------------
// Warning : modify the Faces Uvws[1]
void CreateLMPlaneFromFaceGroup(SLMPlane &Plane,
                                vector<CMesh::CFace *>::iterator ItFace,
                                sint32 nNbFace) {
  sint32 i, j;
  double rMinU = 1000000.0, rMaxU = -1000000.0, rMinV = 1000000.0,
         rMaxV = -1000000.0;
  vector<CMesh::CFace *>::iterator ItParseI = ItFace;
  CMesh::CFace *pF;
  ...
  for (i = 0; i < nNbFace; ++i) {
    pF = *ItParseI;
    for (j = 0; j < 3; ++j) {
      if (rMinU > pF->Corner[j].Uvws[1].U)
        rMinU = pF->Corner[j].Uvws[1].U;
      if (rMaxU < pF->Corner[j].Uvws[1].U)
        rMaxU = pF->Corner[j].Uvws[1].U;
      if (rMinV > pF->Corner[j].Uvws[1].V)
        rMinV = pF->Corner[j].Uvws[1].V;
      if (rMaxV < pF->Corner[j].Uvws[1].V)
        rMaxV = pF->Corner[j].Uvws[1].V;
    }
    Plane.faces[i] = pF;
    ++ItParseI;
  }
...
// -----------------------------------------------------------------------------------------------
void CalculateGradient(SGradient &g, CMesh::CFace *pF,
                       vector<CVector> &vVertices, CVector &n1, CVector &n2,
                       CVector &n3) {
  double u1 = pF->Corner[0].Uvws[1].U, v1 = pF->Corner[0].Uvws[1].V,
         u2 = pF->Corner[1].Uvws[1].U, v2 = pF->Corner[1].Uvws[1].V,
         u3 = pF->Corner[2].Uvws[1].U, v3 = pF->Corner[2].Uvws[1].V;
...
CVector CalculateInterpolatedVertex(SGradient &g, double u, double v) {
  CVector vRet;
  vRet.x =
      (float)(g.GraduPx * (u - g.InitU) + g.GradvPx * (v - g.InitV) + g.InitPx);
  vRet.y =
      (float)(g.GraduPy * (u - g.InitU) + g.GradvPy * (v - g.InitV) + g.InitPy);
  vRet.z =
      (float)(g.GraduPz * (u - g.InitU) + g.GradvPz * (v - g.InitV) + g.InitPz);
  return vRet;
}
...
CVector CalculateInterpolatedNormal(SGradient &g, double u, double v) {
  CVector vRet;
  vRet.x =
      (float)(g.GraduNx * (u - g.InitU) + g.GradvNx * (v - g.InitV) + g.InitNx);
  vRet.y =
      (float)(g.GraduNy * (u - g.InitU) + g.GradvNy * (v - g.InitV) + g.InitNy);
  vRet.z =
      (float)(g.GraduNz * (u - g.InitU) + g.GradvNz * (v - g.InitV) + g.InitNz);
  vRet.normalize();
  return vRet;
}

// -----------------------------------------------------------------------------------------------
float TestRay(CVector &vLightPos, CVector &vVertexPos, SWorldRT &wrt,
              sint32 nLightNb) {
  ...
  for( sint32 i = 0; i < Meshes.size(); ++i )
  {
      ...
      if( segmentIntersectBSphere( rayP1, rayP2, Meshes[i].second ) )
      {
          ...
          {
              CVector hit;
              CTriangle t(MBMatrix *
  pMB->Vertices[pMB->Faces[j].Corner[0].Vertex], MBMatrix *
  pMB->Vertices[pMB->Faces[j].Corner[1].Vertex], MBMatrix *
  pMB->Vertices[pMB->Faces[j].Corner[2].Vertex] ); CPlane plane; plane.make(
  t.V0, t.V1, t.V2 );
...
// -----------------------------------------------------------------------------------------------
float RayTraceAVertex(CVector &p, SWorldRT &wrt, sint32 nLightNb,
                      SLightBuild &rLight) {
  double rFactor = 0.0;
  ...
  case SLightBuild::LightPoint: {
    ...
    light_p_distance =
        light_p_distance -
        (0.01 + (0.05 * light_p_distance / 100.0)); // Substract n centimeter
    light_p.normalize();
    light_p *= light_p_distance;
    rFactor =
        TestRay(rLight.Position, rLight.Position + light_p, wrt, nLightNb);
    nLightForFactor++;
  } break;
...
CRGBAF LightAVertex(CVector &pRT, CVector &p, CVector &n,
  ...
  for (uint32 nLight = 0; nLight < vLights.size(); ++nLight) {
    ...
    case SLightBuild::LightPoint: {
      ...
      else if (p_light_distance > rLight.rRadiusMax)
        light_intensity = 0.0f;
      else
        light_intensity = 1.0f - (p_light_distance - rLight.rRadiusMin) /
                                     (rLight.rRadiusMax - rLight.rRadiusMin);
      p_light.normalize();
    ...
    case SLightBuild::LightSpot: {
      ...
      if (ang > rLight.rFallof)
        light_intensity = 0.0f;
      else if (ang > rLight.rHotspot)
        light_intensity *=
            1.0f - (ang - rLight.rHotspot) / (rLight.rFallof - rLight.rHotspot);
...
// -----------------------------------------------------------------------------------------------
CVector CalcInterpolatedVertexInFace(SGradient &g, double Uin, double Vin,
                                     CMesh::CFace *pF) {
  double Uout, Vout;
  double Utmp, Vtmp;
  double u1 = pF->Corner[0].Uvws[1].U, v1 = pF->Corner[0].Uvws[1].V;
  ...
  factor = ((Uin - u1) * (u2 - u1) + (Vin - v1) * (v2 - v1)) /
           ((u2 - u1) * (u2 - u1) + (v2 - v1) * (v2 - v1));
  if ((factor >= 0.0) && (factor <= 1.0)) {
    Utmp = u1 + (u2 - u1) * factor;
    Vtmp = v1 + (v2 - v1) * factor;
  ...
  factor = ((Uin - u2) * (u3 - u2) + (Vin - v2) * (v3 - v2)) /
           ((u3 - u2) * (u3 - u2) + (v3 - v2) * (v3 - v2));
  if ((factor >= 0.0) && (factor <= 1.0)) {
    Utmp = u2 + (u3 - u2) * factor;
    Vtmp = v2 + (v3 - v2) * factor;
    rDistTmp = sqrt((Uin - Utmp) * (Uin - Utmp) + (Vin - Vtmp) * (Vin - Vtmp));
    if (rDistTmp < rDist) {
      rDist = rDistTmp;
      Uout = Utmp;
  ...
  factor = ((Uin - u3) * (u1 - u3) + (Vin - v3) * (v1 - v3)) /
           ((u1 - u3) * (u1 - u3) + (v1 - v3) * (v1 - v3));
  if ((factor >= 0.0) && (factor <= 1.0)) {
    Utmp = u3 + (u1 - u3) * factor;
    Vtmp = v3 + (v1 - v3) * factor;
    rDistTmp = sqrt((Uin - Utmp) * (Uin - Utmp) + (Vin - Vtmp) * (Vin - Vtmp));
    if (rDistTmp < rDist) {
      rDist = rDistTmp;
      Uout = Utmp;
...
// -----------------------------------------------------------------------------------------------
void FirstLight(CMesh::CMeshBuild *pMB, SLMPlane &Plane,
  ...
  for (i = 0; i < nNbFace; ++i) {
    ...
    for (k = nPosMinV; k <= nPosMaxV; ++k)
      for (j = nPosMinU; j <= nPosMaxU; ++j) {
        if (isInTriangleOrEdge(j + 0.5, k + 0.5, pF->Corner[0].Uvws[1].U,
                               pF->Corner[0].Uvws[1].V, pF->Corner[1].Uvws[1].U,
                               pF->Corner[1].Uvws[1].V, pF->Corner[2].Uvws[1].U,
                               pF->Corner[2].Uvws[1].V)) {
          CVector p = CalculateInterpolatedVertex(g, j + 0.5, k + 0.5);
          CVector n = CalculateInterpolatedNormal(g, j + 0.5, k + 0.5);
          CRGBAF col =
              LightAVertex(p, p, n, vLights, AllLights, wrt, doubleSided);
          Plane.col[j - Plane.x + (k - Plane.y) * Plane.w].p[nLayerNb].R =
              col.R;
          Plane.col[j - Plane.x + (k - Plane.y) * Plane.w].p[nLayerNb].G =
              col.G;
          Plane.col[j - Plane.x + (k - Plane.y) * Plane.w].p[nLayerNb].B =
              col.B;
...
// -----------------------------------------------------------------------------------------------
void SecondLight(CMesh::CMeshBuild *pMB, vector<SLMPlane *>::iterator ItPlanes,
                 sint32 nNbPlanes, vector<CVector> &vVertices,
                 CMatrix &ToWorldMat, vector<sint32> &vLights,
                 vector<SLightBuild> &AllLights, sint32 nLayerNb,
                 SWorldRT &wrt) {
  // Fill interiors
  sint32 i, j, k;
  sint32 nPosMinU, nPosMaxU, nPosMinV, nPosMaxV;
  ...
  for (sint32 nPlanes1 = 0; nPlanes1 < nNbPlanes; ++nPlanes1) {
    ...
    for (i = 0; i < nNbFace1; ++i) {
      ...
      // Select bounding square of the triangle
      for (j = 0; j < 3; ++j) {
        if (rMinU > pF1->Corner[j].Uvws[1].U)
          rMinU = pF1->Corner[j].Uvws[1].U;
        if (rMaxU < pF1->Corner[j].Uvws[1].U)
          rMaxU = pF1->Corner[j].Uvws[1].U;
        if (rMinV > pF1->Corner[j].Uvws[1].V)
          rMinV = pF1->Corner[j].Uvws[1].V;
        if (rMaxV < pF1->Corner[j].Uvws[1].V)
          rMaxV = pF1->Corner[j].Uvws[1].V;
      }
      nPosMaxU = ((sint32)floor(rMaxU + 0.5));
      nPosMaxV = ((sint32)floor(rMaxV + 0.5));
      nPosMinU = ((sint32)floor(rMinU - 0.5));
      ...
      for (k = nPosMinV; k < nPosMaxV; ++k)
        for (j = nPosMinU; j < nPosMaxU; ++j)
          if ((pPlane1->msk[j - pPlane1->x + (k - pPlane1->y) * pPlane1->w] ==
               1) ||
              (pPlane1->msk[1 + j - pPlane1->x +
                            (k - pPlane1->y) * pPlane1->w] == 1) ||
              (pPlane1->msk[1 + j - pPlane1->x +
                            (1 + k - pPlane1->y) * pPlane1->w] == 1) ||
              (pPlane1->msk[j - pPlane1->x +
                            (1 + k - pPlane1->y) * pPlane1->w] == 1))
            if (segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx1,
                                    lumy1, lumx2, lumy2) ||
                segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx2,
                                    lumy2, lumx3, lumy3) ||
                segmentIntersection(j + 0.5, k + 0.5, j + 1.5, k + 0.5, lumx3,
                                    lumy3, lumx1, lumy1) ||

                segmentIntersection(j + 0.5, k + 0.5, j + 0.5, k + 1.5, lumx1,
                                    lumy1, lumx2, lumy2) ||
                segmentIntersection(j + 0.5, k + 0.5, j + 0.5, k + 1.5, lumx2,
                                    lumy2, lumx3, lumy3) ||
                segmentIntersection(j + 0.5, k + 0.5, j + 0.5, k + 1.5, lumx3,
                                    lumy3, lumx1, lumy1) ||

                segmentIntersection(j + 1.5, k + 1.5, j + 1.5, k + 0.5, lumx1,
                                    lumy1, lumx2, lumy2) ||
                segmentIntersection(j + 1.5, k + 1.5, j + 1.5, k + 0.5, lumx2,
                                    lumy2, lumx3, lumy3) ||
                segmentIntersection(j + 1.5, k + 1.5, j + 1.5, k + 0.5, lumx3,
                                    lumy3, lumx1, lumy1) ||

                segmentIntersection(j + 1.5, k + 1.5, j + 0.5, k + 1.5, lumx1,
                                    lumy1, lumx2, lumy2) ||
                segmentIntersection(j + 1.5, k + 1.5, j + 0.5, k + 1.5, lumx2,
                                    lumy2, lumx3, lumy3) ||
                segmentIntersection(j + 1.5, k + 1.5, j + 0.5, k + 1.5, lumx3,
                                    lumy3, lumx1, lumy1)) {
              // If all segment of the current face are linked with a face in
              // this plane, no need to continue
              vector<CMesh::CFace *>::iterator ItParseM =
                  pPlane1->faces.begin();
              sint32 nNbSeg = 0;
              uint32 m, n;
                  ...
                  for (n = 0; n < pPlane2->faces.size(); ++n) {
                    ...
                    if (FaceContinuous(pF1, pF2)) {
                      for (sint32 o = 0; o < 4; ++o) {
                        ...
                        if ((pPlane1->msk[nAbsX - pPlane1->x +
                                          (nAbsY - pPlane1->y) * pPlane1->w] ==
                             1) &&
                            (nAbsX >= pPlane2->x) &&
                            (nAbsX < (pPlane2->x + pPlane2->w)) &&
                            (nAbsY >= pPlane2->y) &&
                            (nAbsY < (pPlane2->y + pPlane2->h))) {
                          // Is it an interior calculated pixel ?
                          if (pPlane2->msk[nAbsX - pPlane2->x +
                                           (nAbsY - pPlane2->y) * pPlane2->w] ==
                              2) { // Yes -> ok so get it
                            pPlane1
                                ->col[nAbsX - pPlane1->x +
                                      (nAbsY - pPlane1->y) * pPlane1->w]
                                ...
                                pPlane2
                                    ->col[nAbsX - pPlane2->x +
                                          (nAbsY - pPlane2->y) * pPlane2->w]
                                    .p[nLayerNb];
                            pPlane1->msk[nAbsX - pPlane1->x +
                                         (nAbsY - pPlane1->y) * pPlane1->w] = 3;
                          } else if (pPlane2->msk[nAbsX - pPlane2->x +
                                                  (nAbsY - pPlane2->y) *
                                                      pPlane2->w] ==
                                     1) { // No -> Add extrapolated value
                            CVector iv = CalculateInterpolatedVertex(
                                g, ((double)nAbsX) + 0.5,
                                ((double)nAbsY) + 0.5);
                            CVector in = CalculateInterpolatedNormal(
                                g, ((double)nAbsX) + 0.5,
                                ((double)nAbsY) + 0.5);
                            CVector rv = CalcInterpolatedVertexInFace(
                                g, ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5,
                                pF1);
                            CRGBAF col =
                                LightAVertex(rv, iv, in, vLights, AllLights,
                                             wrt, doubleSided);
                            // float f = 1.0f;
                            pPlane2
                                ->col[nAbsX - pPlane2->x +
              ...
              for (sint32 o = 0; o < 4; ++o) {
                sint32 nAbsX = j + (o / 2), nAbsY = k + (o % 2);
                if (pPlane1->msk[nAbsX - pPlane1->x +
                                 (nAbsY - pPlane1->y) * pPlane1->w] == 1) {
                  CVector iv = CalculateInterpolatedVertex(
                      g, ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5);
                  CVector in = CalculateInterpolatedNormal(
                      g, ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5);
                  CVector rv = CalcInterpolatedVertexInFace(
                      g, ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5, pF1);
                  CRGBAF col = LightAVertex(rv, iv, in, vLights, AllLights, wrt,
                                            doubleSided);
                  // float f = 1.0f;
                  pPlane1
                      ->col[nAbsX - pPlane1->x +
  ...
  // All planes are done so now we have to average the value of lumels grouping
  // severals normals
  ItPlanes1 = ItPlanes;
...
bool isInteractionLightMeshWithoutAmbient(SLightBuild &rSLB,
                                          CMesh::CMeshBuild &rMB) {
  CAABBox meshBox;
...
void getLightInteract(CMesh::CMeshBuild *pMB, vector<SLightBuild> &AllLights,
  ...
  for (i = 0; i < AllLights.size(); ++i) {
    if (isInteractionLightMesh(AllLights[i], *pMB)) {
      ...
      for (j = 0; j < nNbGroup; ++j)
        if (AllLights[vvLights[j].operator[](0)].GroupName ==
            AllLights[i].GroupName)
          break;
      ...
      if ((j == nNbGroup) && (nNbGroup < 8)) {
        ...
        ++nNbGroup;
      } else { // The light name already exist or there is not enought groups
        if (j == nNbGroup)
          j = nNbGroup - 1;
...
// -----------------------------------------------------------------------------------------------
void GetAllNodeInScene(vector<CMesh::CMeshBuild *> &Meshes,
                       vector<SLightBuild> &AllLights, INode *pNode = NULL) {
  if (pNode == NULL)
  ...
  if (!RPO::isZone(*pNode, tvTime))
    if (CExportNel::isMesh(*pNode, tvTime)) {
      ...
      // If the mesh has no interaction with one of the light selected we do not
      // need it
...
void buildWorldRT(SWorldRT &wrt, vector<SLightBuild> &AllLights) {
    ...
    case SLightBuild::LightPoint: {
      for (j = 0; j < wrt.vMB.size(); ++j) {
        for (k = 0; k < wrt.vMB[j]->Faces.size(); ++k) {
          ...
          cell.pMB = wrt.vMB[j];
          CTriangle tri =
              CTriangle(cell.pMB->Vertices[cell.pF->Corner[0].Vertex] -
                            AllLights[i].Position,
                        cell.pMB->Vertices[cell.pF->Corner[1].Vertex] -
                            AllLights[i].Position,
                        cell.pMB->Vertices[cell.pF->Corner[2].Vertex] -
                            AllLights[i].Position);
          if (intersectionTriangleSphere(
                  tri, CBSphere(CVector(0, 0, 0), AllLights[i].rRadiusMax)))
  ...
  for( i = 0; i < wrt.vMB.size(); ++i )
  {
      wrt.qtAccel[i].create( 4, wrt.bbBoxes[i].getCenter(),
  max(wrt.bbBoxes[i].getSize().x, wrt.bbBoxes[i].getSize().y) );
...
bool isBoxCanCastShadowOnBoxWithLight(CAABBox &b1, CAABBox &b2,
                                      SLightBuild &l) {
...
// -----------------------------------------------------------------------------------------------
void supprLightNoInteractOne(vector<SLightBuild> &vLights,
                             CMesh::CMeshBuild *pMB) {
  uint32 i, j;
...
void buildWorldRTAccel(SWorldRT &wrt, CMesh::CMeshBuild *pMB,
                       vector<vector<sint32>> &vvLights) {
  sint32 i, j, k;
  // Determine all meshes that can cast shadows on the current mesh with current
  // lights
  /*
  ...
  for( j = 0; j < wrt.vMB.size(); ++j )
  {
      for( k = 0; k < vvLights.size(); ++k )
      {
          ...
          for( i = 0; i < vLights.size(); ++i )
          {
              if( isBoxCanCastShadowOnBoxWithLight( wrt.bbBoxes[j] , curBBox,
  *vLights[i] ) )
              {
                  allmesh.push_back( j );
                  break;
  ...
  wrt.qtAccel.create( 9, bbWorld.getCenter(),
  max(bbWorld.getSize().x,bbWorld.getSize().y) );
...
int CALLBACK CalculatingDialogCallback(HWND hwndDlg,  // handle to dialog box
                                       UINT uMsg,     // message
  ...
  case WM_PAINT: {
    char temp[256];
    SendMessage(GetDlgItem(hwndDlg, IDC_PROGRESS1), PBM_SETPOS,
                gRatioCalculated * 100, 0);

    if (gRatioCalculated > 0.0) {
      DWORD TimeLeft = ((TimeCurrent - gTimeBegin) / gRatioCalculated) *
                       (1.0 - gRatioCalculated);
      sprintf(temp, "Time remaining : %02d h %02d m %02d s", TimeLeft / 3600000,
              (TimeLeft / 60000) % 60, (TimeLeft / 1000) % 60);

      SendMessage(GetDlgItem(hwndDlg, IDC_STATICTIMELEFT), WM_SETTEXT, 0,
                  (long)temp);
...
// -----------------------------------------------------------------------------------------------
void AddLightInfo(CMesh::CMeshBuild *pMB, string &LightName, uint8 nMatNb,
                  uint8 nStageNb) {
  CMesh::CMatStage ms;
  ms.nMatNb = nMatNb;
  ...
  if (itMap == pMB->LightInfoMap.end()) {
    ...
    pMB->LightInfoMap.insert(
        pair<string, CMesh::CLightInfoMapList>(LightName, listTemp));
  } else {
...
void CNelExport::deleteLM(INode &ZeNode) {
  ...
  for (i = 0; i < 8; ++i) {
    ...
    sSaveName += tmp;
    sSaveName += ".tga";
    FILE *file = nlfopen(sSaveName, "rb") if (file) {
      fclose(file);
...
// bool CNelExport::exportMeshLM(const char *sPath, INode& ZeNode, Interface&
// ip, TimeValue tvTime)
bool CNelExport::calculateLM(CMesh::CMeshBuild *pZeMeshBuild, INode &ZeNode,
                             Interface &ip, TimeValue tvTime,
                             bool absolutePath) {
  ...
  */
  ...
  // for( nNode=0; nNode < nNbMesh; ++nNode )
  {
    ...
    for (uint32 nMat = 0; nMat < FaceGroupByMat.size(); ++nMat) {
      ...
      SortFaceBySMoothGroup(FaceGroupBySmooth, AllFaces.begin() + offsetMat,
                            FaceGroupByMat[nMat]);

      offsetSmooth = offsetMat;
      for (uint32 nSmoothNb = 0; nSmoothNb < FaceGroupBySmooth.size();
           ++nSmoothNb) {
        uint32 nPlaneNb, nLight;
        vector<sint32> FaceGroupByPlane;

        if (!PutFaceUV1InLumelCoord(
                theExportSceneStruct.rLumelSize, AllVertices,
                AllFaces.begin() + offsetSmooth, FaceGroupBySmooth[nSmoothNb]))
          continue;

        SortFaceByPlane(FaceGroupByPlane, AllFaces.begin() + offsetSmooth,
                        FaceGroupBySmooth[nSmoothNb]);
        // AllPlanes.resize( FaceGroupByPlane.size() );

        // SortPlanesBySurface( FaceGroupByPlane, AllFaces.begin()+offsetSmooth,
        // FaceGroupBySmooth[nSmoothNb] );
        ...
        for (nPlaneNb = 0; nPlaneNb < FaceGroupByPlane.size(); ++nPlaneNb) {
          ...
          AllPlanes[AllPlanesPrevSize + nPlaneNb]->nNbLayerUsed =
              vvLights.size();
          // Fill planes (part of lightmap)
          CreateLMPlaneFromFaceGroup(*AllPlanes[AllPlanesPrevSize + nPlaneNb],
                                     AllFaces.begin() + offsetPlane,
                                     FaceGroupByPlane[nPlaneNb]);
          // Next group of face with the same plane in the same smooth group of
          // the same material
        ...
        for (nLight = 0; nLight < vvLights.size(); ++nLight) {
          ...
          SecondLight(pMB, AllPlanes.begin() + AllPlanesPrevSize,
                      FaceGroupByPlane.size(), AllVertices, MBMatrix,
                      vvLights[nLight], AllLights, nLight, WorldRT);
        }
        if (theExportSceneStruct.nOverSampling > 1) {
          for (nPlaneNb = 0; nPlaneNb < FaceGroupByPlane.size(); ++nPlaneNb)
            ModifyLMPlaneWithOverSampling(
                AllPlanes[AllPlanesPrevSize + nPlaneNb],
    ...
    for (j = 0; j < LightMap.nNbLayerUsed; ++j)
      if (!isAllBlack(LightMap, j)) {
        ...
        for (i = 0; i < pMB->Materials.size(); ++i) {
          ...
          AddLightInfo(pMB, AllLights[vvLights[j].operator[](0)].GroupName, i,
                       nLightMapNb);
...
bool CNelExport::exportScene(std::vector<INode *> &vectNode) {
  // All inputs are transfered throught the global theExportSceneStruct
  // structure
  ...
  if (theExportSceneStruct.bExportShapes &&
      (!theExportSceneStruct.bExportLighting)) {
    // Get time
    TimeValue tvTime = theCNelExport.ip->GetTime();
    ...
    for (int nNode = 0; nNode < nNumSelNode; nNode++) {
      ...
      // Try to export a mesh
        ...
        if (theExportSceneStruct
                .sExportShapes[theExportSceneStruct.sExportShapes.size() - 1] !=
            '\\')
          strcat(sSavePath, "\\");
        ...
        // Export the mesh
        if (!theCNelExport.exportMesh(sSavePath, *pNode, *theCNelExport.ip,
                                      tvTime)) {
          // Error message
          char sErrorMsg[512];
          sprintf(sErrorMsg, "Error exporting the mesh %s in the file\n%s",
                  pNode->GetName(), sSavePath);
          MessageBox(theCNelExport.ip->GetMAXHWnd(), sErrorMsg,
                     "NeL export scene", MB_OK | MB_ICONEXCLAMATION);
        }
      }
    }
  }
  ...
  if (theExportSceneStruct.bExportLighting) {
    ...
    bCancelCalculation = true;
    HWND hwndCalculating = CreateDialog(
        CNelExportDesc.HInstance(), MAKEINTRESOURCE(IDD_CALCULATING),
        theCNelExport.ip->GetMAXHWnd(), CalculatingDialogCallback);
    ...
    nNbMesh = 0;
    // Create all the MeshBuild used to place lightmaps and to raytrace
    // lightmaps
    for (nNode = 0; nNode < nNumSelNode; ++nNode) {
      INode *pNode = vectNode[nNode];
      if (!RPO::isZone(*pNode, tvTime))
        if (CExportNel::isMesh(*pNode, tvTime)) {
          ...
          AllMeshBuilds[nNbMesh].first =
              CExportNel::createMeshBuild(*pNode, tvTime);
          nNbMesh++;
        }
    }
    ...
    // --------------------
    SWorldRT WorldRT; // The static world for raytrace
    //		vector< pair < CMesh::CMeshBuild*, CBSphere > >
    //MeshBuildForRaytrace;
    vector<SLightBuild> AllLights;
    ...
    for (nNode = 0; nNode < nNbMesh; ++nNode) {
      ...
      for (uint32 nMat = 0; nMat < FaceGroupByMat.size(); ++nMat) {
        ...
        for (uint32 nSmoothNb = 0; nSmoothNb < FaceGroupBySmooth.size();
          ...
          if (!PutFaceUV1InLumelCoord(theExportSceneStruct.rLumelSize,
                                      AllVertices,
                                      AllFaces.begin() + offsetSmooth,
                                      FaceGroupBySmooth[nSmoothNb]))
            continue;
          ...
          // SortPlanesBySurface( FaceGroupByPlane,
          // AllFaces.begin()+offsetSmooth, FaceGroupBySmooth[nSmoothNb] );
          ...
          for (nPlaneNb = 0; nPlaneNb < FaceGroupByPlane.size(); ++nPlaneNb) {
            ...
            CreateLMPlaneFromFaceGroup(*AllPlanes[AllPlanesPrevSize + nPlaneNb],
                                       AllFaces.begin() + offsetPlane,
                                       FaceGroupByPlane[nPlaneNb]);
            // Next group of face with the same plane in the same smooth group
      ...
      zeTime2 = CTime::getPerformanceTime();
      // Update UV coords to Texture space
      PutFaceUV1InTextureCoord(LightMap.w, LightMap.h, AllFaces.begin(),
                               nNbFace);
      sint32 nLightMapNb = 0;
      for (j = 0; j < LightMap.nNbLayerUsed; ++j)
        if (!isAllBlack(LightMap, j)) {

...
// Copyright (C) 2010-2022  Jan BOON (Kaetemi) <jan.boon@kaetemi.be>
//
...
public:
  ...
  bool exportAnim(const std::string &sPath, std::vector<INode *> &vectNode,
                  TimeValue time, bool scene);
  bool exportSWT(const std::string &sPath, std::vector<INode *> &vectNode);

  bool exportInstanceGroup(std::string filename,
                           std::vector<INode *> &vectNode);
  bool exportSkeleton(const std::string &sPath, INode *pNode, TimeValue time);

  bool exportCollision(const std::string &sPath, std::vector<INode *> &nodes,
                       TimeValue time);

  bool exportPACSPrimitives(const std::string &sPath,
                            std::vector<INode *> &nodes, TimeValue time);

  bool exportVegetable(const std::string &sPath, INode &node, TimeValue time);

  bool exportLodCharacter(const std::string &sPath, INode &node,
                          TimeValue time);

  void viewMesh(TimeValue time);

  static void
  initOptions(); // read the CNelExportSceneStruct from disk or init it
  static void deleteLM(INode &ZeNode); // the export scene struct MUST be
                                       // initialized before calling this fn
  void OnNodeProperties(const std::set<INode *> &listNode);

  ULONG SelectFileForSave(HWND Parent, const TCHAR *Title, const TCHAR *Mask,
                          std::string &FileName);

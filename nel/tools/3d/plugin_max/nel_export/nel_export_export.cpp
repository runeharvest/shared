...
bool CNelExport::exportMesh(const std::string &sPath, INode &node,
                            TimeValue time) {
  // Result to return
  bool bRet = false;
  ...
  try {
    ...
    NLMISC::CFile::getTemporaryOutputFilename(
        tempPathBuffer + "_nel_export_mesh_", tempFileName);
    ...
    if (os.obj) {
      ...
      if (CExportNel::isSkin(node)) {
        ...
        if (skeletonRoot) {
          ...
          _ExportNel->buildSkeletonShape(*skeletonShape, *skeletonRoot,
                                         &boneBindPos, mapId, time);
      ...
      if (pShape) {
        ...
        try {
          ...
          nlwarning(
              "Failed to delete pShape pointer! Something might be wrong.");
          CFile::deleteFile(tempFileName);
          bRet = false;
        }
...
bool CNelExport::exportVegetable(const std::string &sPath, INode &node,
                                 TimeValue time) {
  bool bRet = false;
...
bool CNelExport::exportAnim(const std::string &sPath,
                            std::vector<INode *> &vectNode, TimeValue time,
                            bool scene) {
  // Result to return
  bool bRet = false;
  std::string tempFileName;
  ...
  try {
    ...
    for (uint n = 0; n < vectNode.size(); n++) {
      ...
      int prefixe = CExportNel::getScriptAppData(
          vectNode[n], NEL3D_APPDATA_EXPORT_ANIMATION_PREFIXE_NAME, 0);

      // Set the name only if it is a scene animation
      if (scene || prefixe) {
        // try to get the prefix from the appData if present. If not, takes it
        // from the node name
        nodeName = CExportNel::getScriptAppData(
            vectNode[n], NEL3D_APPDATA_INSTANCE_NAME, "");
    ...
    if (vectNode.size()) {
      ...
      if (file.open(tempFileName)) {
        try {
          ...
          if (_ErrorInDialog)
            MessageBoxA(NULL, e.what(), "NeL export",
        ...
        if (_ErrorInDialog)
          MessageBox(NULL, _T("Can't open the file for writing."),
                     _T("NeL export"), MB_OK | MB_ICONEXCLAMATION);
        else
          nlwarning("ERROR : Can't open the file (%s) for writing",
                    tempFileName.c_str());
        if (_TerminateOnFileOpenIssues)
          nelExportTerminateProcess();
      }
...
bool CNelExport::exportSkeleton(const std::string &sPath, INode *pNode,
                                TimeValue time) {
  // Result to return
  bool bRet = false;
...
bool CNelExport::exportLodCharacter(const std::string &sPath, INode &node,
                                    TimeValue time) {
  // Result to return
  bool bRet = false;

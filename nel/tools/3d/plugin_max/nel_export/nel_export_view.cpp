...
public:
  ...
  void initMaxLighter(Interface &_Ip) { ProgressBar.initProgressBar(100, _Ip); }
  void closeMaxLighter() { ProgressBar.uninitProgressBar(); }
...
void regsiterOVPath() {
// must test it first, because NL_DEBUG_FAST and NL_DEBUG are declared at same
// time. #ifdef NL_DEBUG_FAST
...
//	HMODULE hModule = GetModuleHandle("object_viewer_dll_rd.dll");
  ...
  if (!hModule) {
    ::MessageBox(
        NULL,
        _T("'hModule' failed at '") __FUNCTION__ _T("' in file '") __FILE__
                                                 _T(" on ")
                                                 _T("line ") NL_MACRO_TO_STR(
                                                     __LINE__),
        _T("NeL Export"), MB_OK | MB_ICONERROR);
    return;
  }
  ...
  if (!res) {
    ::MessageBox(
        NULL,
        _T("'res' failed at '") __FUNCTION__ _T("' in file '") __FILE__
                                             _T(" on line ") NL_MACRO_TO_STR(
                                                 __LINE__),
        _T("NeL Export"), MB_OK | MB_ICONERROR);
    return;
  }
  ...
  std::string modulePath =
      NLMISC::CFile::getPath(MCharStrToUtf8(sModulePath)) + "object_viewer.cfg";
...
void CNelExport::viewMesh(TimeValue time) {
  ...
  if (view->isInstanceRunning()) {
    ::MessageBox(NULL,
                 _T("An instance of the viewer is currently running, please ")
                 _T("close it :)"),
  ...
  if (view) {
    ...
    if (!view->initUI()) {
      ::MessageBox(NULL,
                   _T("Failed to initialize object viewer ui, this may be a ")
                   _T("driver init issue, check your log.log files"),
                   _T("NeL Export"), MB_OK | MB_ICONEXCLAMATION);
      ...
      // Try to export a mesh
        ...
        if (CExportNel::isSkin(*pNode)) {
          ...
          if (skeletonRoot) {
            ...
            mapRootMapBoneBindPos::iterator iteSkeleton =
                skeletonMap.find(skeletonRoot);

            // Not found ?
            if (iteSkeleton == skeletonMap.end()) {
              // Insert one
              skeletonMap.insert(mapRootMapBoneBindPos::value_type(
                  skeletonRoot, CExportNel::mapBoneBindPos()));
      ...
      // Try to export a mesh
        ...
        if (CExportNel::isSkin(*pNode)) {
          ...
          if (skeletonRoot) {
            ...
            mapRootMapBoneBindPos::iterator iteSkeleton =
                skeletonMap.find(skeletonRoot);
            std::map<INode *, CSkeletonDesc>::iterator skelBindPod =
                mapSkeletonShape.find(skeletonRoot);
            ...
            if (skelBindPod == mapSkeletonShape.end()) {
              ...
              _ExportNel->buildSkeletonShape(*skelShape, *skeletonRoot,
                                             &(iteSkeleton->second), mapId,
                                             time);

              // Add the shape in the view
              uint instance = view->addSkel(
                  skelShape, MCharStrToUtf8(skeletonRoot->GetName()));
              ...
              // Insert in the map
              mapSkeletonShape.insert(
                  std::map<INode *, CSkeletonDesc>::value_type(
                      skeletonRoot, CSkeletonDesc(instance, mapId)));
            }
          }
        }
      }
    ...
    for (nNode = 0; nNode < nNumSelNode; nNode++) {
      ...
      // Is it a automatic light ? if yes add tracks from nel_light (color
      // controller)
      int bAnimated =
          CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_LM_ANIMATED, 0);
      if (bAnimated)
      ...
      // Try to export a mesh
        ...
        if (CExportNel::isSkin(*pNode)) {
          ...
          if (skeletonRoot) {
            ...
            mapRootMapBoneBindPos::iterator iteSkel =
                skeletonMap.find(skeletonRoot);
            std::map<INode *, CSkeletonDesc>::iterator iteSkelShape =
                mapSkeletonShape.find(skeletonRoot);
            ...
            // Export the shape
            IShape *pShape;
            pShape = _ExportNel->buildShape(*pNode, time,
                                            &iteSkelShape->second.MapId, true);

            // Build succesful ?
            if (pShape) {
              // Add the shape in the view
              uint instance =
                  view->addMesh(pShape, MCharStrToUtf8(pNode->GetName()),
                                iteSkelShape->second.SkeletonInstance);
        ...
        if (!skined) {
          ...
          if (pShape) {
            ...
            view->addMesh(pShape, nelObjectNameLwr.c_str(), 0xffffffff, NULL,
                          false);
    ...
    NL3D::CInstanceGroup *ig =
        _ExportNel->buildInstanceGroup(igVectNode, resultInstanceNode, time);
    if (ig) {
      // If ExportLighting
      if (theExportSceneStruct.bExportLighting) {
        // Light the ig.
        ...
        lightDesc.OverSampling =
            NLMISC::raiseToNextPowerOf2(theExportSceneStruct.nOverSampling);
        ...
        if (theExportSceneStruct.bTestSurfaceLighting) {
          ...
          _ExportNel->computeCollisionRetrieverFromScene(
              time, slInfo.RetrieverBank, slInfo.GlobalRetriever,
              slInfo.ColIdentifierPrefix.c_str(),
              slInfo.ColIdentifierSuffix.c_str(), slInfo.IgFileName);
        }

        // Light Ig.
        CIgLighterLib::lightIg(maxInstanceLighter, *ig, *igOut, lightDesc,
                               slInfo, "");
      ...
      for (i = 0; i < ig->getNumInstance(); i++) {
        // Set the single animation
        view->setSingleAnimation(igAnim[resultInstanceNode[i]],
                                 "3dsmax current animation", firstInstance + i);
    ...
    if (theExportSceneStruct.bExportLighting) {
      ...
      // lighting
      view->setupSceneLightingSystem(true, igSunDirection, sunAmb, igSunColor,
                                     igSunColor);
      ...
      if (!vectLight.empty())
      {
          ...
          view->setupSceneLightingSystem(false, igSunDirection, CRGBA::Black,
      igSunColor, igSunColor);

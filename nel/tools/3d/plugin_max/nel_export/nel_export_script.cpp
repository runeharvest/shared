...
def_visible_primitive(force_quit_on_msg_displayer,
...
MCHAR *sExportShapeExErrorMsg =
    _M("NeLExportShapeEx [Object] [Filename.shape] [bShadow] [bExportLighting] "
       "[sLightmapPath] [nLightingLimit] [fLumelSize] [nOverSampling] "
       "[bExcludeNonSelected] [bShowLumel]");
MCHAR *sExportAnimationErrorMsg = _M(
    "NelExportAnimation [node array] [Filename.anim] [bool_scene_animation]");
MCHAR *sExportCollisionErrorMsg =
    _M("NelExportCollision [node array] [output directory]");
MCHAR *sExportPACSPrimitivesErrorMsg =
    _M("NelExportPACSPrimitves [node array] [output filename]");
...
Value *export_shape_cf(Value **arg_list, int count) {
  ...
  try {
    ...
    int bCol = CExportNel::getScriptAppData(node, NEL3D_APPDATA_COLLISION,
                                            BST_UNCHECKED);
    if (bCol == BST_CHECKED)
      return ret;
...
Value *export_shape_ex_cf(Value **arg_list, int count) {
  ...
  theExportSceneStruct.sExportLighting =
      MCharStrToUtf8(arg_list[4]->to_string());
...
Value *export_skeleton_cf(Value **arg_list, int count) {
  ...
  type_check(arg_list[0], MAXNode,
             _M("NelExportSkeleton [root node] [Filename]"));
...
Value *export_ig_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], Array,
             _M("NelExportInstanceGroup [Object array] [Filename]"));
  type_check(arg_list[1], String,
             _M("NelExportInstanceGroup [Object array] [Filename]"));
  ...
  try {
    if (is_array(arg_list[0])) {
      ...
      uint i;
      for (i = 0; i < (uint)array->size; i++)
        type_check(array->get(i + 1), MAXNode,
                   _M("NelExportInstanceGroup [Object array] [Filename]"));
...
Value *export_skeleton_weight_cf(Value **arg_list, int count) {
  ...
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], Array,
             _M("NelExportSkeletonWeight [Object array] [Filename]"));
  type_check(arg_list[1], String,
             _M("NelExportSkeletonWeight [Object array] [Filename]"));
...
Value *test_file_date_cf(Value **arg_list, int count) {
  ...
  // Open it
  FILE *file =
      nlfopen(ucstring((const ucchar *)file0.data()).toUtf8().c_str(), "r");
  if (file == NULL)
    return &true_value;
  ...
  // Create first file
  HANDLE h0 = CreateFileW((LPCWSTR)file0.data(), GENERIC_READ, FILE_SHARE_READ,
                          NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (h0 != INVALID_HANDLE_VALUE) {
    HANDLE h1 =
        CreateFileW((LPCWSTR)file1.data(), GENERIC_READ, FILE_SHARE_READ, NULL,
    ...
    if (h1 != INVALID_HANDLE_VALUE) {
      ...
      if (GetFileTime(h0, &creationTime0, &lastAccessTime0, &lastWriteTime0)) {
        if (GetFileTime(h1, &creationTime1, &lastAccessTime1,
                        &lastWriteTime1)) {
          // Compare date
...
Value *node_properties_cf(Value **arg_list, int count) {
  ...
  for (i = 0; i < (uint)array->size; i++) {
    type_check(array->get(i + 1), MAXNode,
               _M("NelNodeProperties [node_array] [dialog error]"));
...
Value *mirror_physique_cf(Value **arg_list, int count) {
  ...
  // do it
  theCNelExport._ExportNel->mirrorPhysiqueSelection(*node, time, vertIn,
                                                    threshold);
...
Value *get_file_modification_date_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  MCHAR *message = _M("date NeLGetFileModificationDate [filename] - If an "
                      "error occurred, returns undefined.");
  ...
  string result;
  HANDLE file =
      CreateFileW((LPCWSTR)sPath.data(), GENERIC_READ, FILE_SHARE_READ, NULL,
                  OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (file) {
    FILETIME lastWriteTime;
    if (GetFileTime(file, NULL, NULL, &lastWriteTime)) {
      char number[512];
      sprintf(number, "%08x%08x", lastWriteTime.dwHighDateTime,
...
Value *set_file_modification_date_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  MCHAR *message = _M("bool NeLSetFileModificationDate [filename] [date] - If "
  ...
  // get vertices indices
  string result;
  HANDLE file = CreateFileW(sPath.data(), GENERIC_WRITE, FILE_SHARE_WRITE, NULL,
                            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
  if (file) {
    FILETIME lastWriteTime;
    if (swscanf(sDate.data(), L"%08x%08x", &lastWriteTime.dwHighDateTime,
                &lastWriteTime.dwLowDateTime) == 2) {
...
Value *force_quit_on_msg_displayer_cf(Value **arg_list, int count) {
  ...
  // disable the Windows popup telling that the application aborted and disable
  // the dr watson report.
  ...
  if (NLMISC::DefaultMsgBoxDisplayer ||
      INelContext::getInstance().getDefaultMsgBoxDisplayer()) {
    if (!NLMISC::DefaultMsgBoxDisplayer)
      NLMISC::DefaultMsgBoxDisplayer =
          INelContext::getInstance().getDefaultMsgBoxDisplayer();
    nldebug("Disable NeL report msg displayer");
    INelContext::getInstance().getAssertLog()->removeDisplayer(
        NLMISC::DefaultMsgBoxDisplayer);
    INelContext::getInstance().getErrorLog()->removeDisplayer(
        NLMISC::DefaultMsgBoxDisplayer);
  ...
  INelContext::getInstance().setDefaultMsgBoxDisplayer(
      NLMISC::DefaultMsgBoxDisplayer);

...
bool CNelExport::exportZone(const std::string &sPath, INode &node,
                            TimeValue time) {
  // Result to return
  bool bRet = false;
  ...
  if (os.obj) {
    ...
    RPO *pPatchObject =
        (RPO *)os.obj->ConvertToType(time, RYKOLPATCHOBJ_CLASS_ID);
    ...
    if (pPatchObject) {
      ...
      if (pPatchObject->rpatch->exportZone(&node, &pPatchObject->patch, zone,
                                           zoneSymmetry, 0, 160, 1, false)) {
        // Open a file
        COFile file;
        if (file.open(sPath)) {
          try {

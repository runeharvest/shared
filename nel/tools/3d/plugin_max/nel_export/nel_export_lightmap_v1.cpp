...
// -----------------------------------------------------------------------------------------------
// -----------------------------------------------------------------------------------------------
void CNelExport::exportInstanceGroup(string filename,
  ...
  for (i = 0; i < (sint)vectNode.size(); ++i, ++it) {
    ...
    if (pParent->IsRootNode() ==
        0) // The pNode doesn't have the root node for parent
    {
...
void SortFaceByMaterialId(int offset, int nNbFace,
                          vector<CMesh::CFace *> &AllFaces) {
  int i, j;
...
void CreatePiece(vector<uint8> &Piece, sint32 &nSizeX, sint32 &nSizeY,
  ...
  for (j = 0; j < nSizeY - 1; ++j)
    for (k = 0; k < nSizeX - 1; ++k) {
      ...
      if (isInTriangle(k + 0.5, j + 0.5, lumx1, lumy1, lumx2, lumy2, lumx3,
                       lumy3) ||
          isInTriangle(k + 1.5, j + 0.5, lumx1, lumy1, lumx2, lumy2, lumx3,
                       lumy3) ||
          isInTriangle(k + 0.5, j + 1.5, lumx1, lumy1, lumx2, lumy2, lumx3,
                       lumy3) ||
          isInTriangle(k + 1.5, j + 1.5, lumx1, lumy1, lumx2, lumy2, lumx3,
                       lumy3)) {
...
void MapFace(CMesh::CFace *pFace, vector<CVector> &Vertices, double rRatio) {
  ...
  QuantizationTbl[1].rotateZ(Pi / 2);
  QuantizationTbl[2].identity();
  QuantizationTbl[2].rotateY(Pi / 2);
...
float getUVDist(CUV &UV1, CUV &UV2) {
  return sqrt((UV2.U - UV1.U) * (UV2.U - UV1.U) +
...
// *******************
// #define RATIOLIGHTMAP	0.25		// 0.5 meters in real world
...
#define MAXLIGHTMAPSIZE 1024
  if (theExportSceneStruct.bExportLighting) {
    ...
    // 2 -> Place lightmap for all MeshBuild (this operation does not need
    // interaction between MeshBuilds)
    for (nNode = 0; nNode < nNbMesh; ++nNode) {
      ...
      vector<sint32>
          AreaTextureNames; // Number of faces with the same texture name
      ...
      for (i = 0; i < nNbFace; ++i) {
        CVector v = AllVertices[AllFaces[i]->Corner[1].Vertex] -
                    AllVertices[AllFaces[i]->Corner[0].Vertex];
        SpaceDist += v.norm();
        v = AllVertices[AllFaces[i]->Corner[2].Vertex] -
            AllVertices[AllFaces[i]->Corner[1].Vertex];
        SpaceDist += v.norm();
        v = AllVertices[AllFaces[i]->Corner[0].Vertex] -
            AllVertices[AllFaces[i]->Corner[2].Vertex];
        SpaceDist += v.norm();
        TextureDist += getUVDist(AllFaces[i]->Corner[1].Uvws[1],
                                 AllFaces[i]->Corner[0].Uvws[1]);
        TextureDist += getUVDist(AllFaces[i]->Corner[2].Uvws[1],
                                 AllFaces[i]->Corner[1].Uvws[1]);
        TextureDist += getUVDist(AllFaces[i]->Corner[0].Uvws[1],
                                 AllFaces[i]->Corner[2].Uvws[1]);
      }
      ...
      for (i = 0; i < nNbFace; ++i) {
        ...
        for (j = 0; j < 3; ++j) { // For each corner
          AllFaces[i]->Corner[j].Uvws[1].U -= nNewPosX;
          AllFaces[i]->Corner[j].Uvws[1].V -= nNewPosY;
        }

        while (true) {
          if (!PutPieceInLightMap(Piece, nPieceSizeX, nPieceSizeY, TheLightMap,
                                  nLightMapSizeX, nLightMapSizeY, nNewPosX,
                                  nNewPosY)) {
            if ((nLightMapSizeX < MAXLIGHTMAPSIZE) ||
                (nLightMapSizeY < MAXLIGHTMAPSIZE)) {
              if (nLightMapSizeX < nLightMapSizeY)
                ResizeBitmap(TheLightMap, nLightMapSizeX, nLightMapSizeY,
                             nLightMapSizeX * 2, nLightMapSizeY);
              else
                ResizeBitmap(TheLightMap, nLightMapSizeX, nLightMapSizeY,
                             nLightMapSizeX, nLightMapSizeY * 2);
            } else {
              // ERROR: we reached the maximum texture size
              nlstop;
            }
          } else {
            // We found a position for this piece, update texture coord in lumel
            // format
            for (j = 0; j < 3; ++j) {
              AllFaces[i]->Corner[j].Uvws[1].U += nNewPosX;
              AllFaces[i]->Corner[j].Uvws[1].V += nNewPosY;
      ...
      // All faces have been put in the lightmap so update All texture coord
      // with the size of the lightmap Convert all faces from lumels to
      // lumels/sizeofLightMap to get uv from 0.0 to 1.0
      for (i = 0; i < nNbFace; ++i)
        for (j = 0; j < 3; ++j) {
        ...
        for (j = 0; j < nLightMapSizeY; ++j)
          for (i = 0; i < nLightMapSizeX; ++i)
            if (TheLightMap[i + j * nLightMapSizeX] != 0) {
              rText[(i + j * nLightMapSizeX) * 4 + 0] =
                  63 + ((TheLightMap[i + j * nLightMapSizeX] * 15) % 190);
              rText[(i + j * nLightMapSizeX) * 4 + 1] =
                  63 + ((TheLightMap[i + j * nLightMapSizeX] * 27 + 63) % 190);
              rText[(i + j * nLightMapSizeX) * 4 + 2] =
                  63 + ((TheLightMap[i + j * nLightMapSizeX] * 13 + 127) % 190);
    ...
    // All nodes are mapped correctly with a lightmap and lightmap is created on
    // disk
    // --------
    // RAYTRACE the scene
    // --------
    ...
    for (nNode = 0; nNode < nNbMesh; ++nNode) {
      ...
      for (i = 0; i < nNbFace; ++i) {
        ...
        ITexture *pText =
            pMB->Materials[pMB->Faces[i].MaterialId].getTexture(1);
        ...
        // Get the mask
        CreatePiece(Piece, nPieceSizeX, nPieceSizeY, nPiecePosX, nPiecePosY, u1,
                    v1, u2, v2, u3, v3, 1);
        // Parse the mask
        for (k = 0; k < nPieceSizeY; ++k)
          for (j = 0; j < nPieceSizeX; ++j)
            ...
            {                                    // Yes
              CRGBAF theColor;
              // Convert the (u,v) to world space coord (use gradient of x along
              // u and v) (same for y,z...)
              double theU = nPiecePosX + j + 0.5;
              double theV = nPiecePosY + k + 0.5;
              CVector p, n; // p is the point in the world space

              double GradU =
                  ((p3.x - p1.x) * (v2 - v1) - (p2.x - p1.x) * (v3 - v1)) *
                  GradDen;
              double GradV =
                  ((p2.x - p1.x) * (u3 - u1) - (p3.x - p1.x) * (u2 - u1)) *
                  GradDen;
              p.x = GradU * (theU - u1) + GradV * (theV - v1) + p1.x;

              GradU = ((p3.y - p1.y) * (v2 - v1) - (p2.y - p1.y) * (v3 - v1)) *
                      GradDen;
              GradV = ((p2.y - p1.y) * (u3 - u1) - (p3.y - p1.y) * (u2 - u1)) *
                      GradDen;
              p.y = GradU * (theU - u1) + GradV * (theV - v1) + p1.y;

              GradU = ((p3.z - p1.z) * (v2 - v1) - (p2.z - p1.z) * (v3 - v1)) *
                      GradDen;
              GradV = ((p2.z - p1.z) * (u3 - u1) - (p3.z - p1.z) * (u2 - u1)) *
                      GradDen;
              p.z = GradU * (theU - u1) + GradV * (theV - v1) + p1.z;

              // The same for the normal

              GradU = ((n3.x - n1.x) * (v2 - v1) - (n2.x - n1.x) * (v3 - v1)) *
                      GradDen;
              GradV = ((n2.x - n1.x) * (u3 - u1) - (n3.x - n1.x) * (u2 - u1)) *
                      GradDen;
              n.x = GradU * (theU - u1) + GradV * (theV - v1) + n1.x;

              GradU = ((n3.y - n1.y) * (v2 - v1) - (n2.y - n1.y) * (v3 - v1)) *
                      GradDen;
              GradV = ((n2.y - n1.y) * (u3 - u1) - (n3.y - n1.y) * (u2 - u1)) *
                      GradDen;
              n.y = GradU * (theU - u1) + GradV * (theV - v1) + n1.y;

              GradU = ((n3.z - n1.z) * (v2 - v1) - (n2.z - n1.z) * (v3 - v1)) *
                      GradDen;
              GradV = ((n2.z - n1.z) * (u3 - u1) - (n3.z - n1.z) * (u2 - u1)) *
                      GradDen;
              ...
              for (sint32 nLight = 0; nLight < vLights.size(); ++nLight) {
                ...
                case SLightBuild::LightPoint: {
                  ...
                  if (p_light_distance < rLight.rRadiusMin)
                    ...
                    light_intensity =
                        1.0 - (p_light_distance - rLight.rRadiusMin) /
                                  (rLight.rRadiusMax - rLight.rRadiusMin);
                  p_light.normalize();

                  // ??? light_intensity *= light_intensity * light_intensity;
                  lightDiffCol.R = light_intensity * rLight.Diffuse.R *
                                   max(0.0f, n * p_light) / 255.0;
                  lightDiffCol.G = light_intensity * rLight.Diffuse.G *
                                   max(0.0f, n * p_light) / 255.0;
                  lightDiffCol.B = light_intensity * rLight.Diffuse.B *
                                   max(0.0f, n * p_light) / 255.0;
                  lightDiffCol.A = light_intensity * rLight.Diffuse.A *
                                   max(0.0f, n * p_light) / 255.0;
                } break;
                case SLightBuild::LightDir: {
                  ...
                  light.normalize();
                  lightDiffCol.R =
                      rLight.Diffuse.R * max(0.0f, n * light) / 255.0;
                  lightDiffCol.G =
                      rLight.Diffuse.G * max(0.0f, n * light) / 255.0;
                  lightDiffCol.B =
                      rLight.Diffuse.B * max(0.0f, n * light) / 255.0;
                  lightDiffCol.A =
                      rLight.Diffuse.A * max(0.0f, n * light) / 255.0;
                } break;
                ...
                theColor.R +=
                    lightAmbiCol.R + lightDiffCol.R; // * matDiffCol.R;
                if (theColor.R > 1.0)
                  theColor.R = 1.0;
                theColor.G +=
                    lightAmbiCol.G + lightDiffCol.G; // * matDiffCol.G;
                if (theColor.G > 1.0)
                  theColor.G = 1.0;
                theColor.B +=
                    lightAmbiCol.B + lightDiffCol.B; // * matDiffCol.B;
                if (theColor.B > 1.0)
                  theColor.B = 1.0;
                theColor.A +=
                    lightAmbiCol.A + lightDiffCol.A; // * matDiffCol.A;
                if (theColor.A > 1.0)
                  theColor.A = 1.0;
              }
              // Affect the color
              ...
              pixel[(j + nPiecePosX +
                     (k + nPiecePosY) * pLightMap->getWidth()) *
                        4 +
                    0] = theColor.R * 255;
              pixel[(j + nPiecePosX +
                     (k + nPiecePosY) * pLightMap->getWidth()) *

...
public:
  virtual int IsPublic() NL_OVERRIDE { return 0; }
  virtual void *Create(BOOL loading = FALSE) NL_OVERRIDE { return new RPO; }
  virtual const MCHAR *ClassName() NL_OVERRIDE { return _M("RklPatch"); }
  ...
  virtual Class_ID ClassID() NL_OVERRIDE { return RYKOLPATCHOBJ_CLASS_ID; }
  virtual const MCHAR *Category() NL_OVERRIDE { return _M("Rykol Tools"); }
};
...
ClassDesc *GetRPODesc() { return &RPODesc; }
...
void RPO::EndEditParams(IObjParam *ip, ULONG flags, Animatable *next) {
  // TODO: Save plugin parameter values into class variables, if they are not
  // hosted in ParamBlocks.
...
public:
  int proc(ViewExp *vpt, int msg, int point, int flags, IPoint2 m,
           Matrix3 &mat);
  void SetObj(RPO *obj) { ob = obj; }
};

// ------------------------------------------------------------------------------------------------------------------------------------------------

int RPOCreateCallBack::proc(ViewExp *vpt, int msg, int point, int flags,
...
int RPO::HitTest(TimeValue t, INode *inode, int type, int crossing, int flags,
...
void RPO::Snap(TimeValue t, INode *inode, SnapInfo *snap, IPoint2 *p,
               ViewExp *vpt) {
...
Mesh *RPO::GetRenderMesh(TimeValue t, INode *inode, View &view,
                         BOOL &needDelete) {
...
/*
  ...
  If this returns a pointer other than itself, it must be deleted by whatever
  called it.
*/
Object *RPO::ConvertToType(TimeValue t, Class_ID cid) {
  ...
  //{
  //   --- uncomment this if you want to do conversion from rkl back to max
  //   patch (you will lose rkl data in the zone)
  ...
  if (cid == triObjectClassID) {
    ...
    rpatch->InvalidateChannels(PART_TOPO | PART_GEOM | PART_TEXMAP |
                               PART_SELECT | PART_DISPLAY);
  ...
  for (int i = 0; i < NUM_OBJ_CHANS; i++) {
    if (channels & chMask[i]) {
      ...
      case TEXMAP_CHAN_NUM:
        texmapValid.SetEmpty();
        break;
      case SELECT_CHAN_NUM:
        selectValid.SetEmpty();
        break;

...
public:
  ...
  uint16 nPatch; // # of the patch on which the vertex is binded. Valid only if
                 // bBinded==true.
  uint16 nEdge;  // # of the edge in the patch on which the vertex is binded.
  ...
  uint16 fnslmq;
  // float			fWhere;				// Where on the
  // edge the vertex is binded. Value must be 0.25f, 0.5f or 0.75f.
  //  Valid only if bBinded==true.
};
...
public:
  tileDesc() { setEmpty(); }
  void setTile(int num, int ncase, int displace, tileIndex tile0,
               tileIndex tile1, tileIndex tile2) {
    _Num = num;
    _MatIDTab[0] = tile0;
    _MatIDTab[1] = tile1;
    _MatIDTab[2] = tile2;
  ...
  tileIndex &getLayer(int num) { return _MatIDTab[num]; }
  const tileIndex &getLayer(int num) const { return _MatIDTab[num]; }
  int getNumLayer() const { return _Num; }
  void setEmpty() {
    _Num = 0;
    _Flags = 0;
  }
  ...
  int getCase() const { return _Flags & CASE_MASK; }
...
public:
  ...
  void Init() { Binding.bBinded = false; }
};
...
private:
  ...
  // Tabl for color on tile ( size: ((2^NbTilesU)+1) * ((2^NbTilesV)+1) )
  // default, all 0xffffffff. Color in 32 bits ARGB.
  uint Colors[17 * 17];
...
public:
  ...
  uint getTileSize() const { return (1 << NbTilesU) * (1 << NbTilesV); }
  ...
  // Get edge
  CEdgeInfo &getEdge(uint edge) { return _Edges[edge]; }

  // Get edge
  const CEdgeInfo &getEdge(uint edge) const { return _Edges[edge]; }

public:
  void Init(int nU = RPO_DEFAULT_TESSEL, int nV = RPO_DEFAULT_TESSEL,
            bool bKeep = false) {
  ...
  const NL3D::CTileBank &getBank(std::string &path = GetBankPathName()) {
    if (path != _lastPath) {
      try {
        ...
        MessageBox(NULL, MaxTStrFromUtf8(excp.what()).data(), _T("Load error"),
...
private:
  ...
  void resizeMapHit(uint size) { _Data._MapHitToTileIndex->resize(size); }
  ...
  void resizeUIPatch(uint size) { _Data._UIPatch->resize(size); }

  // Resize the user info size
  void resizeUIVertex(uint size) { _Data._UIVertex->resize(size); }

public:
  // Get map hit size
  uint getMapHitSize() const { return (uint)_Data._MapHitToTileIndex->size(); }
  ...
  uint getUIPatchSize() const { return (uint)_Data._UIPatch->size(); }
  ...
  uint getUIVertexSize() const { return (uint)_Data._UIVertex->size(); }
...
private:
  ...
  // after
  void BindingVertex(int nVertex, int nPatch, int nEdge, int nPrimary,
                     typeBind nType);
  ...
  void FindPatch(PatchMesh *patch, int nEdge, int &WhichEdge, int &nPatch,
                 int nFirstPatch);
...
public:
  ...
  // Check the validity of the RPatchMesh's data with the RPO's data (debug
  // stuff) BIND SAFE
  ...
  void Subdivide(int nPatch, int nV0, int nV1, int nV2, int nV3, int nCenter,
                 int nFirstPatch, PatchMesh &patch);

  // Subdivide edge 1 and 3 BIND SAFE
  void SubdivideU(int nPatch, int nV0, int nV1, int nFirstPatch,
                  PatchMesh &patch);

  // Subdivide edge 0 and 2 BIND SAFE
  void SubdivideV(int nPatch, int nV0, int nV1, int nFirstPatch,
                  PatchMesh &patch);
  ...
  void DeleteAndSweep(const BitArray &remapVerts, const BitArray &remapPatches,
                      PatchMesh &patch);
  ...
  int GetSelLevel() { return selLevel; }
  ...
  Matrix3 GetSelTileTm(PatchMesh &patch, TimeValue t, INode *node,
                       bool &bHasSel) const;

  // Get the center of the selected tiles
  Point3 GetSelTileCenter(PatchMesh &patch, TimeValue t, INode *node,
                          bool &bHasSel) const;

  // Hittest method
  BOOL SubObjectHitTest(GraphicsWindow *gw, Material *ma, HitRegion *hr,
                        DWORD flags, SubPatchHitList &hitList, TimeValue t,
                        PatchMesh &patch);
  ...
  bool exportZone(INode *pNode, PatchMesh *pPM, NL3D::CZone &zone,
                  NL3D::CZoneSymmetrisation &sym, int zoneId, float snapCell,
                  float weldThreshold, bool forceBuildZoneSymmetry);
  ...
  void getVertexColor(int patch, int s, int t, NLMISC::CRGBA &dest) {
    // Get the color
    uint encodedColor = getUIPatch(patch).getColor(
        t * ((1 << getUIPatch(patch).NbTilesU) + 1) + s);
  ...
  void setVertexColor(int patch, int s, int t, const NLMISC::CRGBA &newColor) {
    // Get the color
    uint encodedColor = (newColor.A << 24) | (newColor.R << 16) |
                        (newColor.G << 8) | newColor.B;

    // Store the color
    getUIPatch(patch).setColor(t * ((1 << getUIPatch(patch).NbTilesU) + 1) + s,
                               encodedColor);
  }
};

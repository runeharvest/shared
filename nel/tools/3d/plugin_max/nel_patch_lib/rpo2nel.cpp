...
static int getCommonVertex(PatchMesh *pPM, int ipatch1, int ipatch2,
                           int *pordervtx = NULL) {
  Patch *patch1;
  Patch *patch2;
...
static int getOtherBindedVertex(RPatchMesh *pRPM, PatchMesh *pPM, int ipatch1,
                                int ipatch2, int iOtherVertex) {
  Patch *patch1;
  Patch *patch2;
...
bool RPatchMesh::exportZone(INode *pNode, PatchMesh *pPM, NL3D::CZone &zone,
                            CZoneSymmetrisation &zoneSymmetry, int zoneId,
                            float snapCell, float weldThreshold,
                            bool forceBuildZoneSymmetry) {
  Matrix3 TM;
  CPatchInfo pi;
  ...
  if (symmetry || rotate || forceBuildZoneSymmetry) {
    ...
    if (!sName.empty()) {
      ...
      if (file.open(sName)) {
        try {
          ...
          MessageBox(NULL, MaxTStrFromUtf8(e.what()).data(), _T("Error"),
    ...
    if (loaded == false) {
      nlwarning(
          "Can't load any tile bank. Select on with the tile_utility plug-in");
      mprintf(_T("Can't load any tile bank. Select on with the tile_utility ")
              _T("plug-in"));
      return false;
    }
  }
  ...
  for (uint patch = 0; patch < (uint)pPM->numPatches; patch++) {
    ...
    for (uint edge = 0; edge < 4; edge++) {
      ...
      if (ite == edgeSet.end())
        ite = edgeSet
                  .insert(pair<pair<uint, uint>, uint>(
                      pair<uint, uint>(min(v1, v2), max(v1, v2)), 1))
                  .first;
      else {
        // Add a ref
        ite->second++;
  ...
  for (i = 0; i < pPM->numPatches; i++) {
    ...
    for (v = 0; v < pi.OrderT + 1; v++)
      for (u = 0; u < pi.OrderS + 1; u++) {
        ...
        // Create a rgba value
        CRGBA rgba((color & 0xff0000) >> 16, (color & 0x00ff00) >> 8,
                   color & 0xff);
    ...
    for (int e = 0; e < 4; e++) {
      ...
      if (edge.patch2 >= 0) {
        ...
        if (edge.patch2 != i) {
          ...
          patchinfo[i].BindEdges[e].Edge[0] =
              getCommonEdge(pPM, pPatch->edge[e], pPM->patches[edge.patch2]);
        } else {
          patchinfo[i].BindEdges[e].Next[0] = edge.patch1;
          patchinfo[i].BindEdges[e].Edge[0] =
              getCommonEdge(pPM, pPatch->edge[e], pPM->patches[edge.patch1]);
        }
      }
      ...
      if (edge.patches.Count() > 1) {
        ...
        if (edge.patches[1] != i) {
          ...
          patchinfo[i].BindEdges[e].Edge[0] = getCommonEdge(
              pPM, pPatch->edge[e], pPM->patches[edge.patches[1]]);
        } else {
          patchinfo[i].BindEdges[e].Next[0] = edge.patches[0];
          patchinfo[i].BindEdges[e].Edge[0] = getCommonEdge(
              pPM, pPatch->edge[e], pPM->patches[edge.patches[0]]);
        }
      }
  ...
  // Tileset of the tile and rotation is important but tile number, rotation and
  // case will be ajusted later For each patches
  for (i = 0; i < pPM->numPatches; i++) {
      ...
      for (u = 0; u < patchInfo.OrderS; u++) {
        ...
        for (int l = 0; l < 3; l++) {
          if (l >= desc.getNumLayer()) {
            ...
            // this check was intended to avoid nel patch paint crash, but it
            // breaks zone export if (tile >= (uint)bank.getTileCount())
            //{
            ...
            //	MessageBoxA(NULL, error.c_str(), "RPO2NEL", MB_OK |
            //MB_ICONERROR); 	return false;
            //}

            // Set the tile
            patchInfo.Tiles[u + v * patchInfo.OrderS].Tile[l] = tile;
            patchInfo.Tiles[u + v * patchInfo.OrderS].setTileOrient(
                l, (uint8)tileRotation);
          }
        }
        if (patchInfo.Tiles[u + v * patchInfo.OrderS].Tile[0] == 0xffff)
          ...
          if (desc.getCase() == 0)
            ...
            patchInfo.Tiles[u + v * patchInfo.OrderS].setTile256Info(true,
                                                                     case256);
          }
        }
        patchInfo.Tiles[u + v * patchInfo.OrderS].setTileSubNoise(
            desc.getDisplace());

        // Default VegetableState: AboveWater. Important: must not be
        // VegetableDisabled
        patchInfo.Tiles[u + v * patchInfo.OrderS].setVegetableState(
            CTileElement::AboveWater);
      }
  }
  ...
  if (symmetry || rotate) {
    ...
    sym *= rot;
    sym.invert();
    if (!CPatchInfo::transform(patchinfo, zoneSymmetry, bank, symmetry, rotate,
                               snapCell, weldThreshold, sym)) {
...
void RPatchMesh::importZone(PatchMesh *pPM, NL3D::CZone &zone, int &zoneId) {
  ...
  for (uint patch = 0; patch < patchs.size(); patch++) {
    ...
    uiRef.Init(getPowerOf2(patchs[patch].OrderS),
               getPowerOf2(patchs[patch].OrderT));
    ...
    for (v = 0; v < patchs[patch].OrderT; v++)
      for (u = 0; u < patchs[patch].OrderS; u++) {
        ...
        int numLayer =
            (patchs[patch].Tiles[tileindex].Tile[0] == NL_TILE_ELM_LAYER_EMPTY)
                ? 0
            : (patchs[patch].Tiles[tileindex].Tile[1] ==
               NL_TILE_ELM_LAYER_EMPTY)
                ? 1
            : (patchs[patch].Tiles[tileindex].Tile[2] ==
               NL_TILE_ELM_LAYER_EMPTY)
                ? 2
                : 3;
        ...
        // Set the tile
        desc.setTile(
            numLayer, is256x256 ? uvOff + 1 : 0, tileElement.getTileSubNoise(),
    ...
    for (v = 0; v < (uint)(patchs[patch].OrderT + 1); v++)
      for (u = 0; u < (uint)(patchs[patch].OrderS + 1); u++) {
        CRGBA color;
        color.set565(patchs[patch]
                         .TileColors[u + v * (patchs[patch].OrderS + 1)]
                         .Color565);
        uiRef.setColor(u + v * (patchs[patch].OrderS + 1),

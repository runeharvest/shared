...
public:
  ...
  virtual int Display(TimeValue t, INode *inode, ViewExp *vpt,
                      int flags) NL_OVERRIDE;
  virtual int HitTest(TimeValue t, INode *inode, int type, int crossing,
                      int flags, IPoint2 *p, ViewExp *vpt) NL_OVERRIDE;
  virtual void Snap(TimeValue t, INode *inode, SnapInfo *snap, IPoint2 *p,
                    ViewExp *vpt) NL_OVERRIDE;
  // TODO: Return the name that will appear in the history browser (modifier
  // stack)
  virtual GET_OBJECT_NAME_CONST MCHAR *
  GetObjectName(NL_GET_OBJECT_NAME_PARAMS) NL_GET_CLASS_NAME_CONST NL_OVERRIDE {
    return _M("Rykol Patch Object");
  }

  virtual void GetWorldBoundBox(TimeValue t, INode *mat, ViewExp *vpt,
                                Box3 &box) NL_OVERRIDE;
  virtual void GetLocalBoundBox(TimeValue t, INode *mat, ViewExp *vpt,
                                Box3 &box) NL_OVERRIDE;

  virtual void GetDeformBBox(TimeValue t, Box3 &box, Matrix3 *tm,
                             BOOL useSel) NL_OVERRIDE;
  // TODO: Return the default name of the node when it is created.
  virtual void InitNodeName(TSTR &s) NL_OVERRIDE {
    s = _M("Rykol Patch Object");
  }
  ...
  virtual void GetCollapseTypes(Tab<Class_ID> &clist,
                                Tab<TSTR *> &nlist) NL_OVERRIDE;
  virtual int IntersectRay(TimeValue t, Ray &ray, float &at,
                           Point3 &norm) NL_OVERRIDE;
  ...
  virtual Interval ObjectValidity(TimeValue t) NL_OVERRIDE {
    ...
    if (!(validBits & chMask[SUBSEL_TYPE_CHAN_NUM]))
      iv.SetEmpty();
    if (!(validBits & chMask[DISP_ATTRIB_CHAN_NUM]))
      iv.SetEmpty();
    return iv;
  }
  ...
  virtual Interval ChannelValidity(TimeValue t, int nchan) NL_OVERRIDE {
    ...
    case TOPO_CHAN_NUM:
      return topoValid;
      break;
    case TEXMAP_CHAN_NUM:
      return texmapValid;
      break;
    case SELECT_CHAN_NUM:
      return selectValid;
      break;
    default:
...
#define copyFlags(dest, source, mask)                                          \
  ...
  // From Animatable
  virtual void BeginEditParams(IObjParam *ip, ULONG flags,
                               Animatable *prev) NL_OVERRIDE;
  virtual void EndEditParams(IObjParam *ip, ULONG flags,
                             Animatable *next) NL_OVERRIDE;

  // From GeomObject
  virtual Mesh *GetRenderMesh(TimeValue t, INode *inode, View &view,
                              BOOL &needDelete) NL_OVERRIDE;
  ...
  virtual BOOL IsSubClassOf(Class_ID classID) NL_OVERRIDE {
    return classID == ClassID() ? true : PatchObject::IsSubClassOf(classID);
  }
  ...
  /*int	NumParamBlocks()
  {
      ...
      return 1;
  }					// return number of ParamBlocks in this
  instance IParamBlock2* GetParamBlock(int i)
  {

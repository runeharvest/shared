...
float bindWhere[BIND_COUNT] = {0.25f, 0.75f, 0.5f, 0.5f};
...
std::string GetBankPathName() {
  HKEY hKey;
  if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_TILEDIT, 0, KEY_READ, &hKey) ==
      ERROR_SUCCESS) {
    TCHAR path[256];
    DWORD len = 256 * sizeof(TCHAR);
    DWORD type;
    if (RegQueryValueEx(hKey, _T("Bank Path"), 0, &type, (LPBYTE)path, &len) ==
        ERROR_SUCCESS) {
      RegCloseKey(hKey);
...
int GetBankTileSetSet() {
  ...
  if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_TILEDIT, 0, KEY_READ, &hKey) ==
    ...
    DWORD len = 256;
    DWORD type;
    if (RegQueryValueEx(hKey, _T("Tileset Set"), 0, &type, (LPBYTE)&tileSetSet,
                        &len) == ERROR_SUCCESS) {
      RegCloseKey(hKey);
      return tileSetSet;
    }
...
void SetBankPathName(const std::string &path) {
  ...
  if (RegCreateKey(HKEY_CURRENT_USER, REGKEY_TILEDIT, &hKey) == ERROR_SUCCESS) {
    ...
    RegSetValueEx(hKey, _T("Bank Path"), 0, REG_SZ, (LPBYTE)buffer,
...
void SetBankTileSetSet(int tileSetSet) {
  HKEY hKey;
  if (RegCreateKey(HKEY_CURRENT_USER, REGKEY_TILEDIT, &hKey) == ERROR_SUCCESS) {
    RegSetValueEx(hKey, _T("Tileset Set"), 0, REG_DWORD, (LPBYTE)&tileSetSet,
                  4);
    RegCloseKey(hKey);
  }
}
...
int IsVerticesJoined(int nVert0, int nVert1, const CVertexNeighborhood &tab,
...
// Check if two vertex are joined by a edge
bool IsVerticesJoined2(int nVert0, int nVert1, int &nVert2, int &nEdge0,
                       int &nEdge1, const CVertexNeighborhood &tab,
                       const PatchMesh &patch) {
  int nCount = 0;
  ...
  for (uint n = 0; n < listSize; n++) {
    ...
    int nNextVert = (patch.edges[pList[n]].v1 == nVert0)
                        ? patch.edges[pList[n]].v2
                        : patch.edges[pList[n]].v1;
    if (nNextVert != -1) {
...
// Check binding
int CheckBind(int nVert, int nSeg, int &v0, int &v1, int &v2, int &v3,
              const CVertexNeighborhood &tab, const PatchMesh &patch,
              bool bAssert, bool bCreate) {
  int config = -1;
    ...
    if (v1 == -1) {
...
#endif // (MAX_RELEASE < 4000)
      {
        ...
        if ((nSeg0 != -1) && (nSeg1 != -1)) {
          // additionnal check.
#if (MAX_RELEASE < 4000)
          if ((patch.edges[nSeg0].patch2 == -1) &&
              (patch.edges[nSeg1].patch2 == -1)) {
            if (!IsVertexInPatch(nVert, patch.edges[nSeg].patch1, patch))
              config = 0;
          }
#else  // (MAX_RELEASE < 4000)
          if ((patch.edges[nSeg0].patches.Count() < 2) &&
              (patch.edges[nSeg1].patches.Count() < 2)) {
        ...
        if (config == -1) {
          ...
          if (IsVerticesJoined2(v0, nVert, v2, nEdge0, nEdge1, tab, patch) &&
            ...
            // additionnal check
            if ((v2 != v3) && (nEdge0 != nEdge1) && (nEdge0 != nEdge2) &&
                (nEdge0 != nEdge3) && (nEdge1 != nEdge2) &&
                (nEdge1 != nEdge3) && (nEdge2 != nEdge3) &&
#if (MAX_RELEASE < 4000)
                ...
                (!IsVertexInPatch(nVert, patch.edges[nSeg].patches[0],
                                  patch)) &&
...
bool RPatchMesh::Validity(const PatchMesh &patch, bool bAssert) {
  ...
  for (int nVertex = 0; nVertex < (int)getUIVertexSize(); nVertex++) {
    ...
    if (getUIVertex(nVertex).Binding.bBinded) {
      ...
      if ((type != BIND_25) && (type != BIND_50) && (type != BIND_75) &&
          (type != BIND_SINGLE)) {
        if (bAssert)
      ...
      if ((nPrim < 0) || (nPrim >= patch.numVerts)) {
        ...
        if (nPrim != nVertex) {
          ...
          if ((getUIVertex(nPrim).Binding.nPrimVert == BIND_50) ||
              (getUIVertex(nPrim).Binding.nPrimVert == BIND_SINGLE)) {
            if (bAssert)
              nlassert(0);
            bRet = false;
          }
      ...
      // Check geom
      int v0, v1, v2, v3;
      switch (CheckBind(nPrim, patch.patches[nPatchNumber].edge[nEdgeNumber],
                        v0, v1, v2, v3, tab, patch, bAssert, false)) {
      case 0:
        if (type != BIND_SINGLE) {
          if (bAssert)
            nlassert(0);
...
static Point3 InterpCenter(Point3 e1, Point3 i1, Point3 i2, Point3 e2,
                           Point3 *v1 = NULL, Point3 *v2 = NULL,
                           Point3 *v3 = NULL, Point3 *v4 = NULL) {
...
void RPatchMesh::UpdateBindingPos(PatchMesh &patch) {
  ...
  for (uint nV = 0; nV < vertSize; nV++) {
    ...
    if (pVertex->Binding.bBinded) {
      ...
      int nEdge =
          patch.patches[pVertex->Binding.nPatch].edge[pVertex->Binding.nEdge];
      ...
      case BIND_25: {
        ...
        v2 = InterpCenter(patch.verts[edge.v1].p, patch.vecs[edge.vec12].p,
                          patch.vecs[edge.vec21].p, patch.verts[edge.v2].p, &v0,
                          &v1, &v3, &v4);

        // Final interpolation
        patch.verts[nV].p =
            InterpCenter(patch.verts[edge.v1].p, v0, v1, v2,
                         &patch.vecs[pVertex->Binding.nBefore2].p,
                         &patch.vecs[pVertex->Binding.nBefore].p,
                         &patch.vecs[pVertex->Binding.nAfter].p,
                         &patch.vecs[pVertex->Binding.nAfter2].p);
      } break;
      case BIND_50:
        patch.verts[nV].p =
            InterpCenter(patch.verts[edge.v1].p, patch.vecs[edge.vec12].p,
                         patch.vecs[edge.vec21].p, patch.verts[edge.v2].p, NULL,
                         NULL, NULL, NULL);
        break;
      case BIND_75: {
        Point3 v0, v1, v2, v3, v4;
        ...
        // Final interpolation
        patch.verts[nV].p =
            InterpCenter(v2, v3, v4, patch.verts[edge.v2].p,
                         &patch.vecs[pVertex->Binding.nBefore2].p,
                         &patch.vecs[pVertex->Binding.nBefore].p,
                         &patch.vecs[pVertex->Binding.nAfter].p,
                         &patch.vecs[pVertex->Binding.nAfter2].p);
      ...
      case BIND_SINGLE:
        patch.verts[nV].p =
            InterpCenter(patch.verts[edge.v1].p, patch.vecs[edge.vec12].p,
                         patch.vecs[edge.vec21].p, patch.verts[edge.v2].p,
                         &patch.vecs[pVertex->Binding.nBefore2].p,
                         &patch.vecs[pVertex->Binding.nBefore].p,
                         &patch.vecs[pVertex->Binding.nAfter].p,
                         &patch.vecs[pVertex->Binding.nAfter2].p);
...
void RPatchMesh::UpdateBindingInfo(PatchMesh &patch) {
  ...
  for (int n = 0; n < patch.numVerts; n++) {
    if (getUIVertex(n).Binding.bBinded) {
      int v0 = patch
                   .edges[patch.patches[getUIVertex(n).Binding.nPatch]
                              .edge[getUIVertex(n).Binding.nEdge]]
                   .v1;
      int v1 = patch
                   .edges[patch.patches[getUIVertex(n).Binding.nPatch]
                              .edge[getUIVertex(n).Binding.nEdge]]
      ...
      for (uint nn = 0; nn < listSize; nn++) {
        if (getUIVertex(n).Binding.nType == BIND_25) {
          if (IsVertexInEdge(v0, pList[nn], patch)) {
            nlassert(vB == -1);
            vB = pList[nn];
          } else if (IsVertexInEdge(getUIVertex(n).Binding.nPrimVert, pList[nn],
                                    patch)) {
            nlassert(vA == -1);
            vA = pList[nn];
          } else {
          ...
          if (IsVertexInEdge(getUIVertex(n).Binding.nPrimVert, pList[nn],
                             patch)) {
            nlassert(vB == -1);
            vB = pList[nn];
      ...
      nlassert(vA != -1);
      nlassert(vT != -1);
      getUIVertex(n).Binding.nBefore = (patch.edges[vB].v1 == n)
                                           ? patch.edges[vB].vec12
                                           : patch.edges[vB].vec21;
      getUIVertex(n).Binding.nAfter = (patch.edges[vA].v1 == n)
                                          ? patch.edges[vA].vec12
                                          : patch.edges[vA].vec21;
      getUIVertex(n).Binding.nT = (patch.edges[vT].v1 == n)
                                      ? patch.edges[vT].vec12
                                      : patch.edges[vT].vec21;

      getUIVertex(n).Binding.nBefore2 = (patch.edges[vB].v1 == n)
                                            ? patch.edges[vB].vec21
                                            : patch.edges[vB].vec12;
      getUIVertex(n).Binding.nAfter2 = (patch.edges[vA].v1 == n)
                                           ? patch.edges[vA].vec21
                                           : patch.edges[vA].vec12;
    }
  }
...
// Fill the binding info for a vertex. Don't forget to call UpdateBindingInfo
// after
void RPatchMesh::BindingVertex(int nVertex, int nPatch, int nEdge, int nPrimary,
                               typeBind nType) {
  // Some check
  nlassert(nVertex >= 0);
  ...
  nlassert(nEdge >= 0);
  nlassert(nEdge < 4);
  nlassert((nType == BIND_25) || (nType == BIND_50) || (nType == BIND_75) ||
           (nType == BIND_SINGLE));
...
// Subdivide both way
void RPatchMesh::Subdivide(int nPatch, int nV0, int nV1, int nV2, int nV3,
                           int nCenter, int nFirstPatch, PatchMesh &patch) {
  // Subdivide patch
  int nOldTileCount =
      (1 << getUIPatch(nPatch).NbTilesU) * (1 << getUIPatch(nPatch).NbTilesV);
  int nOldVertexCount = ((1 << getUIPatch(nPatch).NbTilesU) + 1) *
                        ((1 << getUIPatch(nPatch).NbTilesV) + 1);
  ...
  for (v = 0; v < nTileCountV; v++)
    for (int u = 0; u < nTileCountU; u++) {
      ...
      pPatch->Tile[nTile].Rotate=(getUIPatch
      (nPatch).Tile[nOldTile].Rotate+3)&3;*/
      pPatch->getTileDesc(nTile) = getUIPatch(nPatch).getTileDesc(nOldTile);
  ...
  for (v = 0; v < nTileCountV; v++)
    for (int u = 0; u < nTileCountU; u++) {
      ...
      pPatch->getTileDesc(nTile) = getUIPatch(nPatch).getTileDesc(nOldTile);
      // pPatch->Tile[nTile].Rotate=(getUIPatch
      // (nPatch).Tile[nOldTile].Rotate+2)&3;
  ...
  for (v = 0; v < nTileCountV + 1; v++)
    for (int u = 0; u < nTileCountU + 1; u++) {
      ...
      nlassert(nVertex < (int)pPatch->getColorSize());
      int nOldVertex =
          nTileCountU + u + ((2 * nTileCountU + 1) * (nTileCountV + v));
      nOldVertex %= nOldVertexCount;
      nlassert(nOldVertex < (int)getUIPatch(nPatch).getColorSize());
      pPatch->setColor(nVertex, getUIPatch(nPatch).getColor(nOldVertex));
    }
  ...
  for (v = 0; v < nTileCountV; v++)
    for (int u = 0; u < nTileCountU; u++) {
      ...
      pPatch->getTileDesc(nTile) = getUIPatch(nPatch).getTileDesc(nOldTile);
      // pPatch->Tile[nTile].Rotate=(getUIPatch
      // (nPatch).Tile[nOldTile].Rotate+1)&3;
...
void RPatchMesh::SubdivideU(int nPatch, int nV0, int nV1, int nFirstPatch,
                            PatchMesh &patch) {
  // Subdivide patch
  int nOldTileCount =
      (1 << getUIPatch(nPatch).NbTilesU) * (1 << getUIPatch(nPatch).NbTilesV);
  ...
  for (int nNewPatch = nFirstPatch; nNewPatch < nFirstPatch + 2; nNewPatch++) {
    ...
    for (v = 0; v < nTileCountV; v++)
      for (int u = 0; u < nTileCountU; u++) {
        ...
        nlassert(nTile < (int)pPatch->getTileSize());
        int nOldTile =
            v + nTileCountV * nU + (nTileCountU - 1 - u) * nTileCountV * 2;
        nOldTile %= nOldTileCount;
        nlassert(nOldTile < (int)getUIPatch(nPatch).getTileSize());
        pPatch->getTileDesc(nTile) = getUIPatch(nPatch).getTileDesc(nOldTile);
        // pPatch->Tile[nTile].Rotate=(getUIPatch
    ...
    for (v = 0; v < nTileCountV + 1; v++)
      for (int u = 0; u < nTileCountU + 1; u++) {
        ...
        nlassert(nVertex < (int)pPatch->getColorSize());
        int nOldVertex =
            v + nTileCountV * nU + (nTileCountU - u) * (nTileCountV * 2 + 1);
        nOldVertex %= nOldVertexCount;
        nlassert(nOldVertex < (int)getUIPatch(nPatch).getColorSize());
        pPatch->setColor(nVertex, getUIPatch(nPatch).getColor(nOldVertex));
      }
  ...
  for (int i = 0; i < (int)getUIVertexSize(); i++) {
    if ((getUIVertex(i).Binding.bBinded) &&
        ((int)getUIVertex(i).Binding.nPatch == nPatch)) {
...
void RPatchMesh::SubdivideV(int nPatch, int nV0, int nV1, int nFirstPatch,
                            PatchMesh &patch) {
  // Subdivide patch
  int nOldTileCount =
      (1 << getUIPatch(nPatch).NbTilesU) * (1 << getUIPatch(nPatch).NbTilesV);
...
void RPatchMesh::AddHook(int nVert, int nSeg, PatchMesh &patch) {
  ...
  int patch1 =
      patch.edges[nSeg].patches.Count() > 0 ? patch.edges[nSeg].patches[0] : -1;
#endif // (MAX_RELEASE < 4000)
...
// AddHook
void RPatchMesh::AddHook(int nVert0, int nVert1, int nVert2, int nSeg,
                         PatchMesh &patch) {
#if (MAX_RELEASE < 4000)
  // Une side of the edge must be cleared
  nlassert(patch.edges[nSeg].patch2 == -1);
...
#else  // (MAX_RELEASE < 4000)
  if ((pMesh->edges[nEdge].patches.Count() > 0 ? pMesh->edges[nEdge].patches[0]
                                               : -1) == nMe)
    return (pMesh->edges[nEdge].patches.Count() > 1
                ? pMesh->edges[nEdge].patches[1]
                : -1);
  else {
    nlassert((pMesh->edges[nEdge].patches.Count() > 1
                  ? pMesh->edges[nEdge].patches[1]
                  : -1) == nMe);
    return (pMesh->edges[nEdge].patches.Count() > 0
                ? pMesh->edges[nEdge].patches[0]
                : -1);
  }
#endif // (MAX_RELEASE < 4000)
}
  ...
  for (int i = 0; i < nOldPolyCount; i++) {
    ...
    if (patch->patchSel[i]) {
      ...
      for (int nEdge=0; nEdge<nEdgeCount; nEdge++)
      {
          ...
          // Find the new face
          FindPatch (patch, nE, nWhichEdge[nEdge], nNewFaces[nEdge],
      nOldPolyCount);
      }
...
void RPatchMesh::FindPatch(PatchMesh *patch, int nEdge, int &WhichEdge,
                           int &nPatch, int nFirstPatch) {
...
// Weld -> Bind safe
void RPatchMesh::Weld(PatchMesh *patch) { ResolveTopoChanges(patch, false); }
      ...
      } else // RPATCHMESH_SERIALIZE_VERSION_3
      {
        ...
        for (int j = 0; j < nSize2; j++) {
          ...
          if (nVersion >= RPATCHMESH_SERIALIZE_VERSION_5)
            iload->Read(&getUIPatch(i).getTileDesc(j)._Flags, sizeof(USHORT),
                        &nb);
          else
          ...
          for (int k = 0; k < 3; k++) {
            ...
            // Not used anymore getUIPatch (i).getTileDesc
            // (j)._MatIDTab[k].Invert=invert;
...
IOResult RPatchMesh::Save(ISave *isave) {
  ...
  for (int i = 0; i < nSize; i++) {
    ...
    for (j = 0; j < nSize2; j++) {
      ...
      for (int k = 0; k < 3; k++) {
        ...
        int tile;
        int rotate;
        // Not used anymore invert=(getUIPatch (i).getTileDesc
        // (j)._MatIDTab[k].Invert!=0);
  ...
  for (i = 0; i < nSize; i++) {
    ...
    uint /* typeBind */ nType =
        (uint /* typeBind */)getUIVertex(i).Binding.nType;
...
Matrix3 RPatchMesh::GetSelTileTm(PatchMesh &patch, TimeValue t, INode *node,
                                 bool &bHasSel) const {
  Interval valid;
  Box3 box;
...
Point3 RPatchMesh::GetSelTileCenter(PatchMesh &patch, TimeValue t, INode *node,
                                    bool &bHasSel) const {
  Interval valid;
  Box3 box;
...
// Hittest method
BOOL RPatchMesh::SubObjectHitTest(GraphicsWindow *gw, Material *ma,
                                  HitRegion *hr, DWORD flags,
                                  SubPatchHitList &hitList, TimeValue t,
                                  PatchMesh &patch) {
  // Return value
  BOOL bRet = FALSE;
...
// Return the patch adjacent at nPatch in the edge nEdge. If no patch adjacent,
// return nPatch
...
#else  // (MAX_RELEASE < 4000)
  nlassert((pEdge->patches.Count() > 0 && pEdge->patches[0] == nPatch) ||
           (pEdge->patches.Count() > 1 && pEdge->patches[1] == nPatch));
  if (pEdge->patches[0] == nPatch)
    return (pEdge->patches.Count() > 1 && pEdge->patches[1] != -1)
               ? pEdge->patches[1]
               : nPatch;
  else
    return (pEdge->patches.Count() > 0 && pEdge->patches[0] != -1)
               ? pEdge->patches[0]
               : nPatch;
#endif // (MAX_RELEASE < 4000)
}
...
void RPatchMesh::BuildMesh(TimeValue t, PatchMesh &patch, Mesh *pMesh) {
  ...
  for (nChannel = 0; nChannel < MAX_MESHMAPS; nChannel++) {
    if ((patch.getNumMapVerts(nChannel) > 0) ||
        ((!rTess.KeepMapping) && (nChannel == 1))) {
  ...
  if (Build & (PART_GEOM | PART_GEOM | PART_SELECT | PART_TEXMAP)) {
    ...
    // Corner coord
    Point3 vTt[4][5] = {
        {Point3(0, 1, 0), Point3(0, 1, 0), Point3(0.f, 0.5f, 0.f),
         Point3(0.5f, 0.5f, 0.f), Point3(0.5f, 1.f, 0.f)},
        {Point3(0, 0, 0), Point3(0.f, 0.5f, 0.f), Point3(0, 0, 0),
         Point3(0.5f, 0.f, 0.f), Point3(0.5f, 0.5f, 0.f)},
        {Point3(1, 0, 0), Point3(0.5f, 0.5f, 0.f), Point3(0.5f, 0.f, 0.f),
         Point3(1.f, 0.f, 0.f), Point3(1.f, 0.5f, 0.f)},
        {Point3(1, 1, 0), Point3(0.5f, 1.f, 0.f), Point3(0.5f, 0.5f, 0.f),
         Point3(1.f, 0.5f, 0.f), Point3(1.f, 1.f, 0.f)}};
    for (int p = 0; p < patch.getNumPatches(); p++) {
      ...
      if (patch.patches[p].type == PATCH_QUAD) {
        ...
        if (Build & (PART_GEOM | PART_SELECT)) {
          ...
          if (Build & PART_SELECT) {
            ...
            pMesh->vertSel.Set(offset + nbdivsV * (nbdivsU + 1),
                               patch.vertSel[patch.patches[p].v[1]]);
            pMesh->vertSel.Set(offset + nbdivsV * (nbdivsU + 1) + nbdivsU,
                               patch.vertSel[patch.patches[p].v[2]]);
            pMesh->vertSel.Set(offset + nbdivsU,
                               patch.vertSel[patch.patches[p].v[3]]);
          }
        }
          ...
          if (Build & PART_SELECT) {
            ...
            if (GetSelLevel() == EP_PATCH) {
              ...
              if (bPatchSel || patch.patchSel[GetAdjacentPatch(p, 2, &patch)]) {
                // Right
                for (int ee = 0; ee < nbdivsV; ee++)
                  pMesh->edgeSel.Set(ne + 6 * nbdivsU * ee + 4 +
                                     6 * (nbdivsU - 1));
              }
          ...
          for (y = 0; y < nbdivsV; y++) {
            for (x = 0; x < nbdivsU; x++) {
              ...
              if (Build & PART_TEXMAP) {
                ...
                uint nTileNumber = 0;
                tileDesc &desc =
                    getUIPatch(p).getTileDesc(uTile + vTile * nTileCountU);
                if (!desc.isEmpty()) {
                  nTileNumber =
                      desc.getLayer(std::min(desc.getNumLayer() - 1,
                                             rTess.TransitionType - 1))
                          .Tile +
                      1;
                }
                ...
                if (rTess.KeepMapping || !bTextured || (nChannel != 1)) {
                  ...
                  if (rTess.KeepMapping || (nChannel != 1)) {
                    for (int ii = 0; ii < 4; ii++)
                      vT[ii] = patch.getMapVert(
                          nChannel, patch.tvPatches[nChannel][p].tv[ii]);
                  } else {
                    for (int ii = 0; ii < 4; ii++)
                      vT[ii] = Point3(0, 0, 0);
                  }
                  ...
                  for (int v = 0; v < nVCount; v++) {
                    ...
                    for (int u = 0; u < nUCount; u++) {
                      Point3 vRight =
                          (vT[(2) & 3] - vT[(3) & 3]) * fV + vT[(3) & 3];
                      Point3 vLeft =
                          (vT[(1) & 3] - vT[(0) & 3]) * fV + vT[(0) & 3];
                      Point3 vPoint = (vRight - vLeft) * fU + vLeft;
                      pMesh->setMapVert(nChannel, offsetcopy + u + v * nUCount,
                                        vPoint);
                      fU += fUd;
                    }
                    fV += fVd;
                  ...
                  for (int vTile = 0; vTile < nTileCountV; vTile++) {
                    for (int uTile = 0; uTile < nTileCountU; uTile++) {
                      ...
                      for (int v = 0; v < nVertexTileCount; v++) {
                        float fV = (float)((v + 1) >> 1) /
                                   (float)(nVertexTileCount >> 1);
                        for (int u = 0; u < nVertexTileCount; u++) {
                          // rotate
                          tileDesc &desc = getUIPatch(p).getTileDesc(
                              uTile + vTile * nTileCountU);
                          int nRotate =
                              4 -
                              desc.getLayer(std::min(desc.getNumLayer() - 1,
                                                     rTess.TransitionType - 1))
                          ...
                          float fU = (float)((u + 1) >> 1) /
                                     (float)(nVertexTileCount >> 1);
                          int nIndex = mpcopy +
                                       vTile * nVertexTileCount *
                                           nVertexTileCount * nTileCountU +
                                       v * nVertexTileCount * nTileCountU +
                                       uTile * nVertexTileCount + u;
                          Point3 vRight = (vTt[(2 + nRotate) & 3][nCase] -
                                           vTt[(3 + nRotate) & 3][nCase]) *
                                              fV +
                                          vTt[(3 + nRotate) & 3][nCase];
                          Point3 vLeft = (vTt[(1 + nRotate) & 3][nCase] -
                                          vTt[(0 + nRotate) & 3][nCase]) *
                                             fV +
                                         vTt[(0 + nRotate) & 3][nCase];
                          Point3 vPoint = (vRight - vLeft) * fU + vLeft;
...
void RPatchMesh::GetPatchTess(int nPatch, int &nUTess, int &nVTess) {
  ...
  // Return tessel level depending tessel of the pathmesh and count of tile in
  // the patch
...
// Display
int RPatchMesh::Display(TimeValue t, INode *inode, ViewExp *vpt, int flags,
                        PatchMesh &patch) {
  ...
  if ((GetSelLevel() == EP_VERTEX) && (inode->Selected())) {
    ...
    patch.render(gw, inode->Mtls(),
                 (flags & USE_DAMAGE_RECT) ? &vpt->GetDammageRect() : NULL,
                 COMP_ALL |
                     ((flags & DISP_SHOWSUBOBJECT) ? COMP_OBJSELECTED : 0),
                 inode->NumMtls());
    ...
    if (patch->patchSel[p]) {
      ...
      for (v = 0; v < nOldV; v++)
        for (u = 0; u < nOldU; u++) {
          int newU = nOldV - v - 1;
          int newV = u;
          getUIPatch(p).getTileDesc(newU + newV * nOldV) =
              old.getTileDesc(u + v * nOldU);
      ...
      for (v = 0; v < nOldV; v++)
        for (u = 0; u < nOldU; u++) {
          int newU = nOldV - v - 1;
          int newV = u;
          getUIPatch(p).setColor(newU + newV * nOldV,
                                 old.getColor(u + v * nOldU));
        }
    }
  }
...
/*void RPatchMesh::flipTilesUpDown (PatchMesh *patch)
{
    ...
    for (int p=0; p<patch->numPatches; p++)
    {
        ...
        {
            // Rotate each layer
            getUIPatch (p).getTileDesc (u+v*nU) = old.getTileDesc
(u+(nV-1-v)*nU); getUIPatch (p).getTileDesc (u+v*nU).flipUpDown ();
        }
    }
}*/
...
void RPatchMesh::operator=(const RPatchMesh &fromOb) {
  DeepCopy(const_cast<RPatchMesh *>(&fromOb),
           PART_GEOM | SELECT_CHANNEL | PART_SUBSEL_TYPE | PART_DISPLAY |
               PART_TOPO | TEXMAP_CHANNEL);
}
#endif // USE_CACHE

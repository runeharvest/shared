...
public:
  ...
  const MCHAR *InternalName() {
    return _M("NeL tile bank utility");
...
static INT_PTR CALLBACK Tile_utilityDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                            LPARAM lParam) {
  switch (msg) {
  case WM_INITDIALOG: {
  ...
  case WM_COMMAND: {
    ...
    case IDC_BANK_PATH: {
      ...
      ofn.hwndOwner = NULL;
      ofn.lpstrFilter =
          _T("Rykol bank files (*.bank)\0*.bank\0All Files (*.*)\0*.*\0");
    ...
    case IDC_SETUP: {
      if (!theTile_utility.SetupMaterial())
        MessageBox(NULL, _T("Select some nel patch object.."), _T("Rykol tile"),
...
void Tile_utility::BeginEditParams(Interface *ip, IUtil *iu) {
  this->iu = iu;
  this->ip = ip;
  hPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_PANEL),
                             Tile_utilityDlgProc, GetString(IDS_PARAMS), 0);
  SetupUI();
}
...
void Tile_utility::Load(const std::string &path) {
  try {
    ...
    if (!file.open(path)) {
      ...
      MessageBox(NULL, MaxTStrFromUtf8(tmp).data(), _T("Error.."),
                 MB_OK | MB_ICONEXCLAMATION);
    } else {
      Bank.clear();
      Bank.serial(file);
    ...
    std::string tmp =
        toString("Error while loading %s:\n\n%s", path.c_str(), stream.what());
    MessageBox(NULL, MaxTStrFromUtf8(tmp).data(), _T("Error.."),
               MB_OK | MB_ICONEXCLAMATION);
  }
}
...
void Tile_utility::SetupUI() {
  ...
  for (int nLand = 0; nLand < Bank.getLandCount(); nLand++) {
    ...
    if (hCombo) {
      SendMessage(hCombo, CB_INSERTSTRING, -1,
                  (LPARAM)MaxTStrFromUtf8(name).data());
    }
  }
  ...
  if (hPanel) {
    ...
    if (Bank.getLandCount()) {
      // Button text
      std::string name =
          toLower(NLMISC::CFile::getFilenameWithoutExtension(Path));
      ...
      // Static text
      TCHAR sTmp[256];
      _stprintf(sTmp, _T("%d diffuse tiles."),
                Bank.getNumBitmap(CTile::diffuse));
      SetWindowText(hwndStatic1, sTmp);
      _stprintf(sTmp, _T("%d additive tiles."),
                Bank.getNumBitmap(CTile::additive));
  ...
  for (i = 0; i < Bank.getTileCount(); i++) {
    ...
    if (!tile->isFree()) {
      ...
      if (!tile->getRelativeFileName(CTile::diffuse).empty() ||
          !tile->getRelativeFileName(CTile::additive).empty()) {
        bActive = true;
        ...
        if (!tile->getRelativeFileName(CTile::diffuse).empty()) {
          ...
          tex->SetMapName(
              MaxTStrFromUtf8(Bank.getAbsPath() +
                              tile->getRelativeFileName(CTile::diffuse))
                  .data());
        ...
        if (!tile->getRelativeFileName(CTile::additive).empty()) {
          ...
          tex->SetMapName(
              MaxTStrFromUtf8(Bank.getAbsPath() +
                              tile->getRelativeFileName(CTile::additive))
                  .data());

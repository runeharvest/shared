...
public:
  ...
  const TCHAR *ClassName() {
    return GetString(IDS_DS_RGBMULT_CDESC);
  } // mjm - 2.3.99
#if (MAX_VERSION_MAJOR >= 24)
  virtual const TCHAR *NonLocalizedClassName() NL_OVERRIDE {
    return _M("RGB Additive");
  }
#endif
  SClass_ID SuperClassID() { return TEXMAP_CLASS_ID; }
  Class_ID ClassID() { return RGBAddClassID; }
...
enum {
  ...
  RGBAdd_map2,
  RGBAdd_map1_on,
  RGBAdd_map2_on, // main grad params
  RGBAdd_type,
};

static ParamBlockDesc2 RGBAdd_param_blk(
    RGBAdd_params, _T("parameters"), 0, &maskCD, P_AUTO_CONSTRUCT + P_AUTO_UI,
    0,
    // rollout
    IDD_RGBMULT, _T("RGB Additif Parameters"), 0, 0, NULL,
    // params
    RGBAdd_color1, _T("color1"), TYPE_RGBA, P_ANIMATABLE, IDS_DS_COLOR1,
    p_default, Color(0, 0, 0), p_ui, TYPE_COLORSWATCH, IDC_MULT_COL1, nl_p_end,
    RGBAdd_color2, _T("color2"), TYPE_RGBA, P_ANIMATABLE, IDS_DS_COLOR2,
    p_default, Color(0.5, 0.5, 0.5), p_ui, TYPE_COLORSWATCH, IDC_MULT_COL2,
    nl_p_end, RGBAdd_map1, _T("map1"), TYPE_TEXMAP, P_OWNERS_REF, IDS_JW_MAP1,
    p_refno, 1, p_subtexno, 0, p_ui, TYPE_TEXMAPBUTTON, IDC_MULT_TEX1, nl_p_end,
    RGBAdd_map2, _T("map2"), TYPE_TEXMAP, P_OWNERS_REF, IDS_JW_MAP2, p_refno, 2,
    p_subtexno, 1, p_ui, TYPE_TEXMAPBUTTON, IDC_MULT_TEX2, nl_p_end,
    RGBAdd_map1_on, _T("map1Enabled"), TYPE_BOOL, 0, IDS_JW_MAP1ENABLE,
    p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_MAPON1, nl_p_end,
    RGBAdd_map2_on, _T("map2Enabled"), TYPE_BOOL, 0, IDS_JW_MAP2ENABLE,
    p_default, TRUE, p_ui, TYPE_SINGLECHEKBOX, IDC_MAPON2, nl_p_end,
    RGBAdd_type, _T("alphaFrom"), TYPE_INT, 0, IDS_PW_ALPHAFROM, p_default, 2,
    p_range, 0, 2, p_ui, TYPE_RADIO, 3, IDC_MULT_ALPHA1, IDC_MULT_ALPHA2,
    IDC_MULT_ALPHA3, nl_p_end,
...
Point3 RGBAdd::EvalNormalPerturb(ShadeContext &sc) {
  if (gbufID)
    sc.SetGBufferID(gbufID);
  Point3 p0 = subTex[0] && mapOn[0] ? subTex[0]->EvalNormalPerturb(sc)
                                    : Point3(0.0f, 0.0f, 0.0f);
  Point3 p1 = subTex[1] && mapOn[1] ? subTex[1]->EvalNormalPerturb(sc)
                                    : Point3(0.0f, 0.0f, 0.0f);
  return p0 + p1;
}
...
#endif
{
  ...
  case 0:
    if (localized)
      return TSTR(GetString(IDS_DS_COLOR1));
    else
      return _T("Color 1");
  case 1:
    if (localized)
      return TSTR(GetString(IDS_DS_COLOR2));
    else
      return _T("Color 2");
  default:
    return TSTR(_T(""));
...
RefResult RGBAdd::NotifyRefChanged(NOTIFY_REF_PARAMS) {
  ...
  case REFMSG_CHANGE:
    ...
    if (hTarget == pblock) {
      ...
      // if so, limit rollout update to the changing item and update any active
      // viewport texture
...
IOResult RGBAdd::Load(ILoad *iload) {
  ...
  ParamBlock2PLCB *plcb =
      new ParamBlock2PLCB(versions, 1, &RGBAdd_param_blk, this, 0);

...
NL3D::IShape *CExportNel::buildRemanence(INode &node, TimeValue time) {
  ...
  uint numSlices =
      getScriptAppData(&node, NEL3D_APPDATA_REMANENCE_SLICE_NUMBER, 2);
  float samplingPeriod =
      getScriptAppData(&node, NEL3D_APPDATA_REMANENCE_SAMPLING_PERIOD, 0.02f);
  float rollupRatio =
      getScriptAppData(&node, NEL3D_APPDATA_REMANENCE_ROLLUP_RATIO, 1.f);

  if (samplingPeriod <= 0.f)
    samplingPeriod = 0.02f;
  if (numSlices <= 2)
    numSlices = 2;
  if (rollupRatio <= 0)
    rollupRatio = 1.f;
  ...
  if (materials.size() != 1) {
    buildRemanenceError(
        this, node, "The remanent segment %s should have a single material");
    return NULL;
  }
  ...
  if (
    ...
    buildRemanenceError(this, node,
                        "Remanence export : %s should only contain one curve "
  ...
  srs->setTextureShifting(
      CExportNel::getScriptAppData(
          &node, NEL3D_APPDATA_REMANENCE_SHIFTING_TEXTURE, 0) != 0);
  ...
  0)); bool deleteIt=false; if (obj != tri) deleteIt = true; Mesh *pMesh =
  &tri->mesh; if (pMesh && pMesh->numVerts > 1)
  {

...
struct CMeshInterfaceVertex {
  ...
  // test wether the given vertex can be welded to one of this interface
  // vertices
  ...
  bool snapVert(NLMISC::CVector &vert, NLMISC::CVector &normal,
                const NLMISC::CMatrix &toWorldMat, float threshold) {
    if (canWeld(toWorldMat * vert, threshold)) {
      // snap to pos and normal
      // The pos of the interface vertex is in world space, so we put it back in
      // object space
...
struct CMeshInterface {
  ...
  bool snapVert(NLMISC::CVector &vert, NLMISC::CVector &normal,
    ...
    for (uint k = 0; k < Verts.size(); ++k) {
      if (Verts[k].snapVert(vert, normal, toWorldMat, threshold))
        return true;
    }
    return false;
  }

  // test wether a vertex can be welded to that interface. snapTo get the index
  // of the interface vertex snapped to
  bool canWeld(const NLMISC::CVector &pos, float threshold,
               uint &snapTo) const {
    for (uint k = 0; k < Verts.size(); ++k) {
  ...
  // build this Interface from a max mesh (usually a polygon converted to a
  // mesh)
...
bool CMeshInterface::buildFromMaxMesh(INode &node, TimeValue tvTime) {
  ...
  if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) {
    ...
    for (k = 0; k < numVerts; ++k) {
      CVector prevNorm =
          (Verts[k].Pos - Verts[(k + numVerts - 1) % numVerts].Pos) ^
          polyNormal;
      CVector nextNorm =
          (Verts[(k + 1) % numVerts].Pos - Verts[k].Pos) ^ polyNormal;
...
static void ApplyMeshInterfacesForMRM(std::vector<CMeshInterface> &interfaces,
  ...
  for (uint m = 0; m < interfaces.size(); ++m) {
    ...
    for (uint k = 0; k < mbuild.Interfaces[m].Vertices.size(); ++k) {
      ...
      mbuild.Interfaces[m].Vertices[k].Pos =
          toObjectMat * interfaces[m].Verts[k].Pos;

      mbuild.Interfaces[m].Vertices[k].Normal =
          toObjectMatNormal * interfaces[m].Verts[k].Normal;
  ...
  for (uint k = 0; k < mbuild.Vertices.size(); ++k) {
    ...
    for (uint m = 0; m < interfaces.size(); ++m) {
      uint snapTo;
      if (interfaces[m].canWeld(toWorldMat * mbuild.Vertices[k], threshold,
                                snapTo)) {
        ...
        vertex.setPos(mbuild.Vertices[k],
                      NL3D_MESH_MRM_SKINNED_DEFAULT_POS_SCALE);
        vertex.getPos(mbuild.Vertices[k],
                      NL3D_MESH_MRM_SKINNED_DEFAULT_POS_SCALE);
...
// private func to apply a set of mesh Interface to a meshbuild
static void ApplyMeshInterfacesUsingInterfaceNormals(
    std::vector<CMeshInterface> &interfaces, CMesh::CMeshBuild &mbuild,
    const NLMISC::CMatrix &toWorldMat, float threshold) {
  // for every faces
  for (uint k = 0; k < mbuild.Faces.size(); ++k) {
    ...
    for (uint l = 0; l < 3; ++l) {
      // against each Interface
      for (uint m = 0; m < interfaces.size(); ++m) {
        interfaces[m].snapVert(
            mbuild.Vertices[mbuild.Faces[k].Corner[l].Vertex],
            mbuild.Faces[k].Corner[l].Normal, toWorldMat, threshold);
      }
    }
  }
}
 ...
 */
static void AddNodeToQuadGrid(const NLMISC::CAABBox &delimiter,
                              TNodeFaceQG &destQuadGrid, INode &node,
                              TimeValue time) {
  CAABBox nodeBBox;
  if (CExportNel::buildMeshAABBox(node, nodeBBox, time)) {
    if (delimiter.intersect(nodeBBox)) {
      nldebug("Adding %s to mesh interface quad grid",
              MCharStrToUtf8(node.GetName()).c_str());
      // add this node tris
      ...
      if (obj) {
        if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) {
          ...
          TriObject *tri = (TriObject *)obj->ConvertToType(
              time, Class_ID(TRIOBJ_CLASS_ID, 0));
          if (tri) {
            // Note that the TriObject should only be deleted
            // if the pointer to it is not equal to the object
            // pointer that called ConvertToType()
            ...
            for (sint l = 0; l < mesh.getNumFaces(); ++l) {
              ...
              if (faceBBox.intersect(delimiter)) {
                ...
                destQuadGrid.insert(faceBBox.getMin(), faceBBox.getMax(),
                                    nodeFace);
                ++numFaceAdded;
              }
            }
...
/** Build a quadgrid of all the faces in a node and its sons that are inside the
 * given BBox The quad grid is arbitrarily oriented in the X-Z plane (has this
 * will mainly be used with characters)
 */
static void BuildNodeFacesQuadGrid(const NLMISC::CAABBox &delimiter,
                                   TNodeFaceQG &destQuadGrid, INode &baseNode,
                                   TimeValue time) {
  const uint numQuads = 16;
  NLMISC::CMatrix qgBasis;
  qgBasis.identity();
  qgBasis.setRot(NLMISC::CVector::I, NLMISC::CVector::K, -NLMISC::CVector::J,
                 true);
...
/// Build a normal from a list of node faces. It is assumes that all faces share
/// at least one smoothing group
static void
BuildNormalFromNodeFaces(const std::vector<const CNodeFace *> &faces,
                         NLMISC::CVector &dest) {
...
static void ApplyMeshInterfacesUsingSceneNormals(
    INode &sceneBaseNode, std::vector<CMeshInterface> &interfaces,
    CMesh::CMeshBuild &mbuild, const NLMISC::CMatrix &toWorldMat,
    float threshold, TimeValue time) {
  ...
  for (uint k = 0; k < interfaces.size(); ++k) {
    ...
    iBBox.extend(iBBox.getMax() +
                 NLMISC::CVector(threshold, threshold, threshold));
    sceneQG.clear();
    ...
    for (uint l = 0; l < mbuild.Faces.size(); ++l) {
      for (uint m = 0; m < 3; ++m) {
        ...
        const CVector &vert =
            toWorldMat * mbuild.Vertices[mbuild.Faces[l].Corner[m].Vertex];
        if (interfaces[k].canWeld(vert, threshold)) {
          ...
          while (faceIt != sceneQG.end()) {
            ...
            // the face must have at least a smoothing group in common with this
            // one
          ...
          if (!candidateFaces.empty()) {
            ::BuildNormalFromNodeFaces(candidateFaces,
                                       mbuild.Faces[l].Corner[m].Normal);
            mbuild.Faces[l].Corner[m].Normal =
                toWorldMatInv.mulVector(mbuild.Faces[l].Corner[m].Normal);
            ++numWelds;
          }
        }
      }
 ...
 */
static bool
SelectVerticesInMeshFromInterfaces(const std::vector<CMeshInterface> &inters,
                                   float threshold, INode &node,
                                   TimeValue tvTime) {
  ObjectState os = node.EvalWorldState(tvTime);
  Object *obj = os.obj;
  // Check if there is an object
 ...
 */
static bool BuildMeshInterfaces(const char *cMaxFileName,
                                std::vector<CMeshInterface> &meshInterfaces,
                                CExportNel &exportNel, TimeValue tvTime) {
  ...
  /** don't know why, but a call to Interface::MergeFromFile freeze the
   * application, so we use a call to maxscript instead
   */
  ...
  // Before merging, rename the materials if in non-echec export mode and backup
  // there original names in a map
  ...
  std::string command("mergeMAXFile \"" + maxFileNameWithSlash +
                      "\" #noRedraw #mergeDups");
  if (CExportNel::scriptEvaluate(command.c_str(), NULL, scriptNothing) ==
      false) {
    nlwarning("Unable to merge %s", maxFileName.c_str());
    CExportNel::scriptEvaluate(("print \"Failed to load mesh interfaces " +
                                maxFileNameWithSlash + "\"")
                                   .c_str(),
                               NULL, scriptNothing);
    mergeSuccess = false;
  ...
  std::set_difference(allNodes.begin(), allNodes.end(), firstNodes.begin(),
                      firstNodes.end(),
  ...
  for (it = mergedNodes.begin(); it != mergedNodes.end(); ++it) {
    ...
    if (meshInterface.buildFromMaxMesh(**it, tvTime)) {
      if (!meshInterface.Verts.empty()) {
        nldebug("adding interface %d from %s", meshInterfaces.size(),
                exportNel.getNelObjectName(**it).c_str());
        // well we could avoid a vector copy
        meshInterfaces.push_back(meshInterface);
      }
    } else {
      nlwarning("unable to build interface from %s",
                exportNel.getNelObjectName(**it).c_str());
    }
  }
...
void CExportNel::applyInterfaceToMeshBuild(INode &node,
                                           CMesh::CMeshBuild &mbuild,
                                           const NLMISC::CMatrix &toWorldMat,
                                           TimeValue time) {
  nldebug("===============================================");
  ...
  // get the threshold
  float threshold = CExportNel::getScriptAppData(
      &node, NEL3D_APPDATA_INTERFACE_THRESHOLD, -1.f);

  if (threshold < 0.f) {
    nlwarning("Invalid threshold used for interface merging, in node %s",
              CExportNel::getNelObjectName(node).c_str());
    return;
  }

  std::vector<CMeshInterface> meshInterface;
  if (!::BuildMeshInterfaces(interfaceFile.c_str(), meshInterface, *this,
                             time)) {
    return;
  }
  ...
  // store interface info in mesh for MRM, & mark vertices that are on an
  // interface
  ...
  if (!useSceneNodeNormals) {
    ::ApplyMeshInterfacesUsingInterfaceNormals(meshInterface, mbuild,
                                               toWorldMat, threshold);
  } else {
    ::ApplyMeshInterfacesUsingSceneNormals(*CExportNel::getRootNode(),
                                           meshInterface, mbuild, toWorldMat,
                                           threshold, time);
  }
}
...
bool CExportNel::selectInterfaceVertices(INode &node, TimeValue time) {
  std::string interfaceFile =
      CExportNel::getScriptAppData(&node, NEL3D_APPDATA_INTERFACE_FILE, "");
  if (interfaceFile.empty()) {
    nlwarning("SelectInterfaceVertices : This node has no mesh interface : %s",
              this->getNelObjectName(node).c_str());
    return false;
  }
  ...
  std::vector<CMeshInterface> meshInterfaces;
  if (!::BuildMeshInterfaces(interfaceFile.c_str(), meshInterfaces, *this,
                             time)) {
    return false;
  }

  return ::SelectVerticesInMeshFromInterfaces(meshInterfaces, threshold, node,
                                              time);
}

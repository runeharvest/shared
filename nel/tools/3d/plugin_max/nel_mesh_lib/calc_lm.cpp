...
void SLightBuild::convertFromMaxLight(INode *node, TimeValue tvTime) {
  ...
  string sTmp = CExportNel::getScriptAppData(
      node, NEL3D_APPDATA_SOFTSHADOW_RADIUS,
      toString(NEL3D_APPDATA_SOFTSHADOW_RADIUS_DEFAULT));
  NLMISC::fromString(sTmp, this->rSoftShadowRadius);
  sTmp = CExportNel::getScriptAppData(
      node, NEL3D_APPDATA_SOFTSHADOW_CONELENGTH,
      toString(NEL3D_APPDATA_SOFTSHADOW_CONELENGTH_DEFAULT));
...
struct SGradient {
  ...
  // -----------------------------------------------------------------------------------------------
  void init(CMesh::CFace *pF, vector<CVector> &vVertices, CVector &n1,
            CVector &n2, CVector &n3) {
    ...
    CRGBA c1 = pF->Corner[0].Color, c2 = pF->Corner[1].Color,
    ...
    this->GraduPx =
        ((p3.x - p1.x) * (v2 - v1) - (p2.x - p1.x) * (v3 - v1)) * GradDen;
    this->GradvPx =
        ((p2.x - p1.x) * (u3 - u1) - (p3.x - p1.x) * (u2 - u1)) * GradDen;

    this->GraduPy =
        ((p3.y - p1.y) * (v2 - v1) - (p2.y - p1.y) * (v3 - v1)) * GradDen;
    this->GradvPy =
        ((p2.y - p1.y) * (u3 - u1) - (p3.y - p1.y) * (u2 - u1)) * GradDen;

    this->GraduPz =
        ((p3.z - p1.z) * (v2 - v1) - (p2.z - p1.z) * (v3 - v1)) * GradDen;
    this->GradvPz =
        ((p2.z - p1.z) * (u3 - u1) - (p3.z - p1.z) * (u2 - u1)) * GradDen;

    // The same for the normal

    this->GraduNx =
        ((n3.x - n1.x) * (v2 - v1) - (n2.x - n1.x) * (v3 - v1)) * GradDen;
    this->GradvNx =
        ((n2.x - n1.x) * (u3 - u1) - (n3.x - n1.x) * (u2 - u1)) * GradDen;

    this->GraduNy =
        ((n3.y - n1.y) * (v2 - v1) - (n2.y - n1.y) * (v3 - v1)) * GradDen;
    this->GradvNy =
        ((n2.y - n1.y) * (u3 - u1) - (n3.y - n1.y) * (u2 - u1)) * GradDen;

    this->GraduNz =
        ((n3.z - n1.z) * (v2 - v1) - (n2.z - n1.z) * (v3 - v1)) * GradDen;
    this->GradvNz =
        ((n2.z - n1.z) * (u3 - u1) - (n3.z - n1.z) * (u2 - u1)) * GradDen;

    // The same for the color

    this->GraduR =
        ((c3.R - c1.R) * (v2 - v1) - (c2.R - c1.R) * (v3 - v1)) * GradDen;
    this->GradvR =
        ((c2.R - c1.R) * (u3 - u1) - (c3.R - c1.R) * (u2 - u1)) * GradDen;

    this->GraduG =
        ((c3.G - c1.G) * (v2 - v1) - (c2.G - c1.G) * (v3 - v1)) * GradDen;
    this->GradvG =
        ((c2.G - c1.G) * (u3 - u1) - (c3.G - c1.G) * (u2 - u1)) * GradDen;

    this->GraduB =
        ((c3.B - c1.B) * (v2 - v1) - (c2.B - c1.B) * (v3 - v1)) * GradDen;
    this->GradvB =
        ((c2.B - c1.B) * (u3 - u1) - (c3.B - c1.B) * (u2 - u1)) * GradDen;
  }

  // -----------------------------------------------------------------------------------------------
  CVector getInterpolatedVertex(double u, double v) {
    CVector vRet;
    vRet.x = (float)(this->GraduPx * (u - this->InitU) +
                     this->GradvPx * (v - this->InitV) + this->InitPx);
    vRet.y = (float)(this->GraduPy * (u - this->InitU) +
                     this->GradvPy * (v - this->InitV) + this->InitPy);
    vRet.z = (float)(this->GraduPz * (u - this->InitU) +
                     this->GradvPz * (v - this->InitV) + this->InitPz);
    return vRet;
  }
  ...
  CVector getInterpolatedNormal(double u, double v) {
    CVector vRet;
    vRet.x = (float)(this->GraduNx * (u - this->InitU) +
                     this->GradvNx * (v - this->InitV) + this->InitNx);
    vRet.y = (float)(this->GraduNy * (u - this->InitU) +
                     this->GradvNy * (v - this->InitV) + this->InitNy);
    vRet.z = (float)(this->GraduNz * (u - this->InitU) +
                     this->GradvNz * (v - this->InitV) + this->InitNz);
    vRet.normalize();
    return vRet;
  }
  ...
  CRGBAF getInterpolatedColor(double u, double v) {
    CRGBAF vRet;
    vRet.R = (float)(this->GraduR * (u - this->InitU) +
                     this->GradvR * (v - this->InitV) + this->InitR);
    vRet.G = (float)(this->GraduG * (u - this->InitU) +
                     this->GradvG * (v - this->InitV) + this->InitG);
    vRet.B = (float)(this->GraduB * (u - this->InitU) +
                     this->GradvB * (v - this->InitV) + this->InitB);
  ...
  CVector getInterpolatedVertexInFace(double Uin, double Vin,
                                      CMesh::CFace *pF) {
    double Uout, Vout;
    double Utmp, Vtmp;
    double u1 = pF->Corner[0].Uvws[1].U, v1 = pF->Corner[0].Uvws[1].V;
    ...
    if ((factor >= 0.0) && (factor <= 1.0)) {
      ...
      rDistTmp =
          sqrt((Uin - Utmp) * (Uin - Utmp) + (Vin - Vtmp) * (Vin - Vtmp));
      if (rDistTmp < rDist) {
        rDist = rDistTmp;
        Uout = Utmp;
        Vout = Vtmp;
...
// -----------------------------------------------------------------------------------------------
/*
void SortFaceByTextureName(vector<CMesh::CFace*> &AllFaces, CMesh::CMeshBuild
*pMB, CMeshBase::CMeshBaseBuild *pMBB)
{
    int i, j;
...
which have the same texture name void ComputeAreaOfTextureName(vector<sint32>
&TextureNames, vector<CMesh::CFace*> &AllFaces, CMesh::CMeshBuild *pMB,
...
*/
// -----------------------------------------------------------------------------------------------
void ClearFaceWithNoLM(CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB,
                       vector<CMesh::CFace *> &ZeFaces) {
  sint32 i;
      ...
      for (nCurSG_NbFaceLeft = 1; nCurSG_NbFaceLeft > 0; --nCurSG_NbFaceLeft) {
        for (k = 0; k < FacesFinal[i].cont.size(); ++k)
          if (FacesNext[FacesFinal[i].cont[k]].added == false) {
            ...
            FacesFinal[i + nCurSG_NbFaceLeft] =
                FacesNext[FacesFinal[i].cont[k]];
            ++nCurSG_NbFace;
            ++nCurSG_NbFaceLeft;
          }
        ++i;
...
// -----------------------------------------------------------------------------------------------
bool vertexInSquare(double xs, double ys, double ws, double hs, double xv,
                    double yv) {
...
// -----------------------------------------------------------------------------------------------
void getLightNodeList(std::vector<INode *> &vectLightNode, TimeValue tvTime,
                      Interface &ip, bool visibleOnly, INode *node = NULL) {
  if (node == NULL)
    node = ip.GetRootNode();
  ...
  if (nelLight.canConvertFromMaxLight(node, tvTime)) {
    ...
    if (!visibleOnly || !node->IsHidden()) {
      ...
      int nLMExport = CExportNel::getScriptAppData(
  ...
  for (int i = 0; i < node->NumberOfChildren(); i++)
    getLightNodeList(vectLightNode, tvTime, ip, visibleOnly,
                     node->GetChildNode(i));
}

// -----------------------------------------------------------------------------------------------
void getLightmapLightBuilds(vector<SLightBuild> &lights, TimeValue tvTime,
                            Interface &ip, bool visibleOnly) {
  ...
  for (uint32 nLight = 0; nLight < vLights.size(); ++nLight) {
    ...
    case SLightBuild::LightSpot: {
      ...
      if ((rLight.ProjBitmap.getHeight() != 0) && (light_intensity > 0.0f)) {
        ...
        // float dotSize = gOptions.rLumelSize * (rLight.Position-inter).norm()
        // / (rLight.Position-p).norm();
    ...
    if (light_intensity > 0.0f) {
      if (bRcvShadows && rLight.bCastShadow && gOptions.bShadow)
        RTFactor = wrt.raytrace(pRT, vLights[nLight], rtVal,
                                gOptions.nExportLighting == 1);
      else
...
// -----------------------------------------------------------------------------------------------
void FirstLight(CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB,
                SLMPlane &Plane, vector<CVector> &vVertices,
                CMatrix &ToWorldMat, vector<sint32> &vLights,
                vector<SLightBuild> &AllLights, uint32 nLayerNb,
                CRTWorld &wrt) {
  // Fill interiors
    ...
    for (k = nPosMinV; k <= nPosMaxV; ++k)
      for (j = nPosMinU; j <= nPosMaxU; ++j)
        if (Plane.msk[j - Plane.x + (k - Plane.y) * Plane.w] == 1) {
          if (isInTriangleOrEdge(
                  j + 0.5, k + 0.5, pF->Corner[0].Uvws[1].U,
            ...
            CRGBAF col = LightAVertex(rtVal, p, p, n, vLights, AllLights, wrt,
                                      doubleSided, pMBB->bRcvShadows);
            Plane.ray[j - Plane.x + (k - Plane.y) * Plane.w] = rtVal;
            Plane
                .col[j - Plane.x + (k - Plane.y) * Plane.w +
                     Plane.w * Plane.h * nLayerNb]
                .R = col.R * (vl.R / 255.0f) * (matDiff.R / 255.0f);
            Plane
                .col[j - Plane.x + (k - Plane.y) * Plane.w +
                     Plane.w * Plane.h * nLayerNb]
                .G = col.G * (vl.G / 255.0f) * (matDiff.G / 255.0f);
            Plane
                .col[j - Plane.x + (k - Plane.y) * Plane.w +
                     Plane.w * Plane.h * nLayerNb]
                .B = col.B * (vl.B / 255.0f) * (matDiff.B / 255.0f);
            Plane
                .col[j - Plane.x + (k - Plane.y) * Plane.w +
                     Plane.w * Plane.h * nLayerNb]
                .A = 1.0f;
...

            {
                  ...
                  for (n = 0; n < pP2->faces.size(); ++n) {
                    ...
                    if (FaceContinuous(pF1, pF2)) {
                      for (uint32 o = 0; o < 4; ++o) {
                        ...
                        if ((pP1->msk[nAbsX - pP1->x +
                                      (nAbsY - pP1->y) * pP1->w] == 1) &&
                            (nAbsX >= pP2->x) &&
                            (nAbsX < (pP2->x + (sint32)pP2->w)) &&
                            (nAbsY >= pP2->y) &&
                            (nAbsY < (pP2->y + (sint32)pP2->h))) {
                          // Is it an interior calculated pixel ?
                          if (pP2->msk[nAbsX - pP2->x +
                                       (nAbsY - pP2->y) * pP2->w] ==
                              2) { // Yes -> ok so get it
                            pP1->col[nAbsX - pP1->x +
                                     (nAbsY - pP1->y) * pP1->w +
                                     pP1->w * pP1->h * nLayerNb] =
                                pP2->col[nAbsX - pP2->x +
                                         (nAbsY - pP2->y) * pP2->w +
                                         pP2->w * pP2->h * nLayerNb];
                            pP1->msk[nAbsX - pP1->x +
                                     (nAbsY - pP1->y) * pP1->w] = 3;
                            pP1->ray[nAbsX - pP1->x +
                                     (nAbsY - pP1->y) * pP1->w] =
                                pP2->ray[nAbsX - pP2->x +
                                         (nAbsY - pP2->y) * pP2->w];
                          } else if (pP2->msk[nAbsX - pP2->x +
                                              (nAbsY - pP2->y) * pP2->w] ==
                                     1) { // No -> Add extrapolated value
                            CVector iv = g.getInterpolatedVertex(
                                ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5);
                            CVector in = g.getInterpolatedNormal(
                                ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5);
                            CRGBAF vl =
                                g.getInterpolatedColor(j + 0.5, k + 0.5);
                            CVector rv = g.getInterpolatedVertexInFace(
                                ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5,
                                pF1);
                            uint8 rtVal = pP2->ray[nAbsX - pP2->x +
                                                   (nAbsY - pP2->y) * pP2->w];
                            CRGBAF col = LightAVertex(
                                rtVal, rv, iv, in, vLights, AllLights, wrt,
                                doubleSided, pMBB->bRcvShadows);
                            pP2->ray[nAbsX - pP2->x +
                                     (nAbsY - pP2->y) * pP2->w] = rtVal;
                            // float f = 1.0f;
                            pP2->col[nAbsX - pP2->x +
                                     (nAbsY - pP2->y) * pP2->w +
                                     pP2->w * pP2->h * nLayerNb]
                                .R +=
                                col.R * (vl.R / 255.0f) * (matDiff.R / 255.0f);
                            pP2->col[nAbsX - pP2->x +
                                     (nAbsY - pP2->y) * pP2->w +
                                     pP2->w * pP2->h * nLayerNb]
                                .G +=
                                col.G * (vl.G / 255.0f) * (matDiff.G / 255.0f);
                            pP2->col[nAbsX - pP2->x +
                                     (nAbsY - pP2->y) * pP2->w +
                                     pP2->w * pP2->h * nLayerNb]
                                .B +=
                                col.B * (vl.B / 255.0f) * (matDiff.B / 255.0f);
                            pP2->col[nAbsX - pP2->x +
                                     (nAbsY - pP2->y) * pP2->w +
                                     pP2->w * pP2->h * nLayerNb]
                                .A += 1.0f;
              ...
              for (sint32 o = 0; o < 4; ++o) {
                ...
                if (pP1->msk[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w] == 1) {
                  CVector iv = g.getInterpolatedVertex(((double)nAbsX) + 0.5,
                                                       ((double)nAbsY) + 0.5);
                  CVector in = g.getInterpolatedNormal(((double)nAbsX) + 0.5,
                                                       ((double)nAbsY) + 0.5);
                  CRGBAF vl = g.getInterpolatedColor(j + 0.5, k + 0.5);
                  CVector rv = g.getInterpolatedVertexInFace(
                      ((double)nAbsX) + 0.5, ((double)nAbsY) + 0.5, pF1);
                  uint8 rtVal =
                      pP1->ray[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w];
                  CRGBAF col =
                      LightAVertex(rtVal, rv, iv, in, vLights, AllLights, wrt,
                                   doubleSided, pMBB->bRcvShadows);
                  pP1->ray[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w] = rtVal;
                  // float f = 1.0f;
                  pP1->col[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w +
                           pP1->w * pP1->h * nLayerNb]
                      .R += col.R * (vl.R / 255.0f) * (matDiff.R / 255.0f);
                  pP1->col[nAbsX - pP1->x + (nAbsY - pP1->y) * pP1->w +
                           pP1->w * pP1->h * nLayerNb]
    ...
    for (k = 0; k < pP1->h; ++k)
      for (j = 0; j < pP1->w; ++j) {
        if (pP1->msk[j + k * pP1->w] == 1) {
          sint32 nNbNormals =
              (sint32)pP1->col[j + k * pP1->w + pP1->w * pP1->h * nLayerNb].A;

          // Avoid divid by zero
          if (nNbNormals) {
            pP1->col[j + k * pP1->w + pP1->w * pP1->h * nLayerNb].R /=
                nNbNormals;
            pP1->col[j + k * pP1->w + pP1->w * pP1->h * nLayerNb].G /=
                nNbNormals;
            pP1->col[j + k * pP1->w + pP1->w * pP1->h * nLayerNb].B /=
                nNbNormals;
          }
...
// -----------------------------------------------------------------------------------------------
CAABBox getMeshBBox(CMesh::CMeshBuild &rMB, CMeshBase::CMeshBaseBuild &rMBB,
                    bool bNeedToTransform) {
  CAABBox meshBox;
  if (bNeedToTransform) {
...
void getLightInteract(CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB,
  ...
  for (i = 0; i < AllLights.size(); ++i) {
    if (isInteractionLightMesh(AllLights[i], *pMB, *pMBB)) {
      ...
      for (j = 0; j < nNbGroup; ++j)
        if ((AllLights[vvLights[j].operator[](0)].AnimatedLight ==
             AllLights[i].AnimatedLight) &&
            (AllLights[vvLights[j].operator[](0)].LightGroup ==
             AllLights[i].LightGroup))
          break;
...
// -----------------------------------------------------------------------------------------------
void convertToWorldCoordinate(CMesh::CMeshBuild *pMB,
                              CMeshBase::CMeshBaseBuild *pMBB,
                              CVector &translation) {
  uint32 j, k;
...
void supprLightNoInteractOne(vector<SLightBuild> &vLights,
                             ...
                             CMeshBase::CMeshBaseBuild *pMBB, INode &node) {
  uint32 i;
  ...
  for (i = 0; i < vLights.size(); ++i) {
    ...
    if (vLights[i].setExclusion.find(MCharStrToUtf8(node.GetName())) !=
        vLights[i].setExclusion.end()) {
      bInteract = false;
    } else {
...
// -----------------------------------------------------------------------------------------------
// Add information for ont mesh to reference all the lights that interact with
// him
void addLightInfo(CMesh::CMeshBuild *pMB, CMeshBase::CMeshBaseBuild *pMBB,
                  string &animatedLight, uint lightGroup, uint8 nMatNb,
                  uint8 nStageNb) {
  /* Search in the light mesh info the good light group. Add the material stage
   * if it exists. Else, add a new entry in the light group. */

  const uint count = pMBB->LightInfoMap.size();
  uint i;
  for (i = 0; i < count; i++) {
    CMeshBase::CLightMapInfoList &info = pMBB->LightInfoMap[i];
    if ((info.AnimatedLight == animatedLight) &&
...
/*
void sans_majuscule_au_debut_LinkToObjectAround (CMesh::CMeshBuild *pMB,
CMeshBase::CMeshBaseBuild *pMBB, vector<CVector>& vertices, SWorldRT& wrt)
{
    ...
    // Here objs contains the objects of the worldRT structure that touch the
reference mesh
    // And it contains the pointer to the face that must be rendered to give
continuity to our object
}
*/

void createLightmapLog(COFile &outputLog, const char *outputDirectory,
                       const char *projectName, const char *objectName) {
...
void appendLightmapLog(COFile &outputLog, const char *lightmapName,
                       const vector<sint32> lightIndexes,
                       const vector<SLightBuild> &lights) {
  try {
    string text;
    text = lightmapName;
    text += " :\n";
    ...
    for (i = 0; i < lightIndexes.size(); i++) {
      ...
      text += lights[lightIndexes[i]].Name +
              toString(" (group %d, animation \"%s\")",
                       lights[lightIndexes[i]].LightGroup,
                       lights[lightIndexes[i]].AnimatedLight.c_str());
      text += "\n";
    }
...
// absolutePath tell this code to put the name of the lightmap in absolute or
// relative path this is very usefull for viewer inside MAX
bool CExportNel::calculateLM(CMesh::CMeshBuild *pZeMeshBuild,
                             CMeshBase::CMeshBaseBuild *pZeMeshBaseBuild,
                             INode &ZeNode, TimeValue tvTime,
                             uint firstMaterial, bool outputLightmapLog) {
  ...
  string sLumelSizeMul =
      CExportNel::getScriptAppData(&ZeNode, NEL3D_APPDATA_LUMELSIZEMUL, "1.0");
  float rLumelSizeMul;
  NLMISC::fromString(sLumelSizeMul, rLumelSizeMul);
  // 8Bits LightMap Compression
  bool lmcEnabled =
      CExportNel::getScriptAppData(&ZeNode, NEL3D_APPDATA_EXPORT_LMC_ENABLED,
                                   BST_UNCHECKED) == BST_CHECKED;
  enum { NumLightGroup = 3 };
  CRGBA lmcAmbient[NumLightGroup];
  CRGBA lmcDiffuse[NumLightGroup];
  for (i = 0; i < NumLightGroup; i++) {
    lmcAmbient[i] = CExportNel::getScriptAppData(
        &ZeNode, NEL3D_APPDATA_EXPORT_LMC_AMBIENT_START + i, CRGBA::Black);
    lmcDiffuse[i] = CExportNel::getScriptAppData(
        &ZeNode, NEL3D_APPDATA_EXPORT_LMC_DIFFUSE_START + i, CRGBA::White);
  }
  ...
  //	return calculateLMRad (pZeMeshBuild, pZeMeshBaseBuild, ZeNode, ip,
  //tvTime, absolutePath, gOptions);
  ...
  if (gOptions.FeedBack != NULL) {
    ...
    if (gOptions.bShadow)
      ...
      sTmp += ",Shadow Off";
    sTmp += ",OverSampling = " +
            toString(gOptions.nOverSampling * gOptions.nOverSampling);
    gOptions.FeedBack->setLine(1, sTmp);
  }

  CRTWorld WorldRT(_ErrorInDialog, _View, _AbsolutePath, _Ip, _ErrorTitle,
                   this); // The static world for raytrace
  vector<SLightBuild> AllLights;
  ...
  WorldRT.build(AllLights, -vGlobalPos, gOptions.bExcludeNonSelected,
                lodListToExclude, lodListToInclude);
  ...
  // for( nNode=0; nNode < nNbMesh; ++nNode )
  {
    ...
    for (uint32 nMat = 0; nMat < FaceGroupByMat.size(); ++nMat) {
      ...
      if (gOptions.FeedBack != NULL) {
        string sTmp = "Material (" + toString(1 + nMat) + "/" +
                      toString(FaceGroupByMat.size()) + ")";
      ...
      for (uint32 nSmoothNb = 0; nSmoothNb < FaceGroupBySmooth.size();
        ...
        if (gOptions.FeedBack != NULL) {
          string sTmp = "SmoothGroup (" + toString(1 + nSmoothNb) + "/" +
                        toString(FaceGroupBySmooth.size()) + ")";
          gOptions.FeedBack->setLine(4, sTmp);
          gOptions.FeedBack->update();
        }
        ...
        if (!PutFaceUV1InLumelCoord(gOptions.rLumelSize, AllVertices,
          ...
          if (gOptions.FeedBack != NULL) {
            ...
            sTmp = "Mat(" + toString(1 + nMat) +
                   ") at least one group with texture";
        ...
        for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
             ++nPlaneNb) {
          ...
          AllPlanes[AllPlanesPrevSize + nPlaneNb]->createFromFaceGroup(
        ...
        for (nLight = 0; nLight < (sint)vvLights.size(); ++nLight) {
          ...
          for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
            ...
            TempPlanes[nPlaneNb]->createFromPlane(
                *AllPlanes[AllPlanesPrevSize + nPlaneNb]);
          }
          for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
               ++nPlaneNb) {
            // Light the LightMap for the plane (interior only)
            FirstLight(pMB, pMBB, *TempPlanes[nPlaneNb], AllVertices, MBMatrix,
                       vvLights[nLight], AllLights, 0, WorldRT);
          }
          // Make extoriors
          SecondLight(pMB, pMBB, TempPlanes.begin(), FaceGroupByPlane.size(),
                      AllVertices, MBMatrix, vvLights[nLight], AllLights, 0,
                      WorldRT);
          ...
          if (gOptions.nOverSampling > 1) {
            ...
            for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
                 ++nPlaneNb)
              FirstLight(pMB, pMBB, *TempPlanes[nPlaneNb], AllVertices,
                         MBMatrix, vvLights[nLight], AllLights, 0, WorldRT);
            SecondLight(pMB, pMBB, TempPlanes.begin(), FaceGroupByPlane.size(),
                        AllVertices, MBMatrix, vvLights[nLight], AllLights, 0,
                        WorldRT);
            ...
            for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
              ...
              ModifyLMPlaneWithOverSampling(
                  TempPlanes[nPlaneNb], 1.0 / ((double)gOptions.nOverSampling),
                  false);
          }

          for (nPlaneNb = 0; nPlaneNb < (sint)FaceGroupByPlane.size();
               ++nPlaneNb)
            TempPlanes[nPlaneNb]->copyFirstLayerTo(
                *AllPlanes[AllPlanesPrevSize + nPlaneNb], (uint8)nLight);
    ...
    for (i = 0; i < AllPlanes.size(); ++i) {
      if (gOptions.FeedBack != NULL) {
        string sTmp =
            "Plane (" + toString(i) + "/" + toString(AllPlanes.size()) + ")";
        gOptions.FeedBack->setLine(4, sTmp);
        gOptions.FeedBack->update();
      }
    ...
    _wsplitpath(WStr(_Ip->GetCurFileName()), NULL, NULL, (wchar_t *)projectName,
                NULL);

    // Add lightmap information in the lightmap log
    COFile outputLog;
    if (outputLightmapLog)
      createLightmapLog(outputLog, gOptions.sExportLighting.c_str(),
                        ucstring(projectName).toUtf8().c_str(),
                        MaxTStrToUtf8(ZeNode.GetName()).c_str());
    ...
    for (j = 0; j < LightMap.nNbLayerUsed; ++j) {
      ...
      // if lightmap compression enabled, must not skip this layer if the
      // ambient term is not black!
      if (!LightMap.isAllBlack((uint8)j) || lmcLayerAmbient != CRGBA::Black) {
        ...
        if (lmcEnabled) {
          // compress to 8 bits
          LightMap.copyColToBitmap8x2(pLightMap, j, lmcLayerAmbient,
                                      lmcLayerDiffuse);
        } else
        ...
        try {
          ...
          if (gOptions.FeedBack != NULL) {
            std::string message = toString("Can't write the file %s : %s",
                                           sSaveName.c_str(), e.what());
        ...
        // Add lightmap information in the lightmap log
        if (outputLightmapLog)
          appendLightmapLog(outputLog, sSaveName.c_str(), vvLights[j],
                            AllLights);
        ...
        if (gOptions.bShowLumel)
          pLightMap->setFilterMode(ITexture::Nearest,
          ...
          if (pMBB->Materials[i].getShader() == CMaterial::LightMap) {
            ...
            if (!vvLights.empty() && !vvLights[j].empty()) {
              ...
              if (lmcEnabled) {
                ...
                pMBB->Materials[i].setLMCColors(nLightMapNb, mA,
                                                lmcLayerDiffuse);
                pMBB->Materials[i].setLightMapMulx2(true);
              }

              addLightInfo(pMB, pMBB,
                           AllLights[vvLights[j].operator[](0)].AnimatedLight,
                           AllLights[vvLights[j].operator[](0)].LightGroup,

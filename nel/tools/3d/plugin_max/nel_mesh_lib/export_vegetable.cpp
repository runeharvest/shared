...
// ***************************************************************************
bool CExportNel::buildVegetableShape(NL3D::CVegetableShape &skeletonShape,
    ...
    if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) {
      ...
      if (tri) {
        ...
        if ((buildMesh.VertexFlags & CVertexBuffer::TexCoord0Flag) == 0) {
          // Error
          outputErrorMessage(
              "Can't build vegetable mesh: need UV1 coordinates");
        } else {
          // Build a mesh
          CMesh mesh;
          ...
          if (mesh.getNbMatrixBlock() != 1) {
            ...
            if (mesh.getNbRdrPass(0) != 1) {
              // Error
              outputErrorMessage(
                  "The object must have less than one material!");
            } else {
              ...
              // Get the appdata
              vegetableBuild.AlphaBlend =
                  CExportNel::getScriptAppData(
                      &node, NEL3D_APPDATA_VEGETABLE_ALPHA_BLEND, 0) == 0;
              ...
              if (vegetableBuild.AlphaBlend) {
                ...
                // Precompute light ?
                vegetableBuild.PreComputeLighting =
                    CExportNel::getScriptAppData(
                        &node, NEL3D_APPDATA_VEGETABLE_ALPHA_BLEND_OFF_LIGHTED,
                        0) == 0;

                // Double sided ?
                vegetableBuild.DoubleSided =
                    CExportNel::getScriptAppData(
                        &node,
                        NEL3D_APPDATA_VEGETABLE_ALPHA_BLEND_OFF_DOUBLE_SIDED,
                        0) != BST_UNCHECKED;
              }
              ...
              if (vegetableBuild.PreComputeLighting) {
                ...
                vegetableBuild.BestSidedPreComputeLighting =
                    CExportNel::getScriptAppData(
                        &node,
                        NEL3D_APPDATA_VEGETABLE_FORCE_BEST_SIDED_LIGHTING,
                        0) != BST_UNCHECKED;
              }

              // Max bend weight
              vegetableBuild.MaxBendWeight = CExportNel::getScriptAppData(
                  &node, NEL3D_APPDATA_BEND_FACTOR,
                  NEL3D_APPDATA_BEND_FACTOR_DEFAULT);

              // BendMode
              vegetableBuild.BendCenterMode =
                  (CVegetableShapeBuild::TBendCenterMode)
                      CExportNel::getScriptAppData(
                          &node, NEL3D_APPDATA_BEND_CENTER, 0);

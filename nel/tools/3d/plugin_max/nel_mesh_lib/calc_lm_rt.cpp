...
std::map<INode *, std::pair<NL3D::CMesh::CMeshBuild *,
                            NL3D::CMeshBase::CMeshBaseBuild *>>
    CRTWorld::s_WorldCache;
...
// -----------------------------------------------------------------------------------------------
CRTWorld::CRTWorld(bool errorInDialog, bool view, bool absolutePath,
                   Interface *ip, std::string errorTitle, CExportNel *exp) {
  _Ip = ip;
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::build(vector<SLightBuild> &AllLights, CVector &trans,
                     bool bExcludeNonSelected, const set<INode *> &excludeNode,
                     const set<INode *> &includeNode) {
  uint32 i, j, k;
  ...
  for (i = 0; i < AllLights.size(); ++i) {
    ...
    case SLightBuild::LightPoint: {
      ...
      for (j = 0; j < vMB.size(); ++j) {
        if (rLight.setExclusion.find(MCharStrToUtf8(vINode[j]->GetName())) !=
            rLight.setExclusion.end())
          continue;
        ...
        for (k = 0; k < vMB[j]->Faces.size(); ++k) {
          ...
          if (intersectionTriangleSphere(
                  tri, CBSphere(CVector(0, 0, 0), rLight.rRadiusMax)))
    ...
    case SLightBuild::LightDir: {
      ...
      for (j = 0; j < vMB.size(); ++j) {
        ...
        for (k = 0; k < vMB[j]->Faces.size(); ++k) {
          ...
          if (intersectionSphereCylinder(s, rLight.Position, rLight.Direction,
                                         rLight.rDirRadius)) {
            float t = ((s.Center - rLight.Position) * rLight.Direction) /
                      (rLight.Direction * rLight.Direction);
...
// -----------------------------------------------------------------------------------------------
CRGBAF CRTWorld::raytrace(NLMISC::CVector &vVertex, sint32 nLightNb,
                          uint8 &rtVal, bool bSoftShadow) {
  ...
  case SLightBuild::LightPoint: {
    ...
    light_p_distance =
        light_p_distance -
        (0.01f + (0.05f * light_p_distance / 100.0f)); // Substract n centimeter
    light_p.normalize();
  ...
  case SLightBuild::LightDir: {
    float lightdist = ((vVertex - rLight.Position) * rLight.Direction) /
                      (rLight.Direction.norm());
    ...
    vVertexPos =
        vVertex - (0.01f + (0.05f * lightdist / 100.0f)) * rLight.Direction;
    vSelect = vVertexPos;
  } break;
  default:
    break;
  ...
  if ((bSoftShadow) && (rLight.Type == SLightBuild::LightDir)) {
    ...
    RayOfLight.initDirectionnal(8, vVertex, rLight.Direction, rSoftShadowRadius,
                                rLight.rSoftShadowConeLength);
    ...
    float rVal =
        RayOfLight.getArea() / rAreaInit; // rVal = AreaNotShadowed / AreaInit
  ...
  } else // Normal shadowing
  {
    ...
    while (!vLightAccel[nLightNb]->isEndSel()) {
      ...
      if ((Factor.R == 0.0f) && (Factor.G == 0.0f) && (Factor.B == 0.0f))
        return Factor;
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::testCell(CRGBAF &retValue, SGridCell &cell, CVector &vLightPos,
                        CVector &vVertexPos, uint8 &rtVal) {
  CVector hit;
  ...
  if (t.intersect(vLightPos, vVertexPos, hit, plane)) {
    if (cell.pMBB->Materials[cell.pF->MaterialId].getBlend() ||
        cell.pMBB->Materials[cell.pF->MaterialId]
            .getAlphaTest()) { // This is a transparent face we have to look in
                               // the texture
      ...
      if (pT == NULL) {
        retValue *=
            1.0f -
            (cell.pMBB->Materials[cell.pF->MaterialId].getOpacity() / 255.0f);
      ...
      cPixMap.A *=
          cell.pMBB->Materials[cell.pF->MaterialId].getOpacity() / 255.0f;
      cPixMap.R *=
          cell.pMBB->Materials[cell.pF->MaterialId].getDiffuse().R / 255.0f;
      cPixMap.G *=
          cell.pMBB->Materials[cell.pF->MaterialId].getDiffuse().G / 255.0f;
      cPixMap.B *=
          cell.pMBB->Materials[cell.pF->MaterialId].getDiffuse().B / 255.0f;
      ...
      rtVal = 255;
    } else { // This is not a transparent face so if we intersect we get shadow
      if (rtVal < 255)
        rtVal += 1;
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::addNode(INode *pNode, vector<CMesh::CMeshBuild *> &Meshes,
                       vector<CMeshBase::CMeshBaseBuild *> &MeshesBase,
                       vector<INode *> &INodes, vector<SLightBuild> &AllLights,
                       const set<INode *> &excludeNode, TimeValue tvTime) {
  if (!RPO::isZone(*pNode, tvTime) && CExportNel::isMesh(*pNode, tvTime)) {
    // Nel export
    nlassert(_Ip->GetRootNode() == s_WorldCacheRoot);
    CAABBox aabbox;
    std::map<INode *, CAABBox>::iterator aabboxIt =
        s_WorldCacheAABBox.find(pNode);
    if (aabboxIt == s_WorldCacheAABBox.end()) {
      ...
      // FIXME: This breaks repeated exports while the file is open and being
      // changed!!! Not an issue for now
    ...
    if (excludeNode.find(pNode) == excludeNode.end()) {
      for (uint32 i = 0; i < AllLights.size(); ++i) {
        if (isInteractionWithLight(
                AllLights[i],
                aabbox)) // FIXME: This is always true with sunlight!
        {
          bInteract = true;
          break;
        }
    ...
    if (bInteract) {
      int nAccelType =
          CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_ACCEL, 32);
      if ((nAccelType & 3) == 0) // If not an accelerator
      {
        ...
        std::map<INode *,
                 std::pair<NL3D::CMesh::CMeshBuild *,
                           NL3D::CMeshBase::CMeshBaseBuild *>>::iterator
            buildIt = s_WorldCache.find(pNode);
        if (buildIt == s_WorldCache.end()) {
          ...
          s_WorldCache[pNode] = std::make_pair(
              pMB ? new CMesh::CMeshBuild(*pMB) : NULL,
              pMBB ? new CMeshBase::CMeshBaseBuild(*pMBB) : NULL);
        } else {
          // FIXME: This breaks repeated exports while the file is open and
          // being changed!!! Not an issue for now
          pMB = buildIt->second.first
                    ? new CMesh::CMeshBuild(*buildIt->second.first)
                    : NULL;
          pMBB = buildIt->second.second
                     ? new CMeshBase::CMeshBaseBuild(*buildIt->second.second)
                     : NULL;
        }
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::getAllSelectedNode(
    vector<CMesh::CMeshBuild *> &Meshes,
    vector<CMeshBase::CMeshBaseBuild *> &MeshesBase, vector<INode *> &INodes,
    vector<SLightBuild> &AllLights, const set<INode *> &excludeNode,
    const set<INode *> &includeNode) {
  if (_Ip->GetRootNode() != s_WorldCacheRoot ||
      MaxTStrToUtf8(_Ip->GetCurFileName()) !=
          s_WorldCacheFile) // FIXME: This breaks repeated exports while the
                            // file is open and being changed!!! Not an issue
                            // for now
  {
    ...
    for (std::map<INode *,
                  std::pair<NL3D::CMesh::CMeshBuild *,
                            NL3D::CMeshBase::CMeshBaseBuild *>>::iterator
             it(s_WorldCache.begin()),
         end(s_WorldCache.end());
         it != end; ++it) {
  ...
  for (int nNode = 0; nNode < nNumSelNode; nNode++) {
    ...
    if (includeNode.find(pNode) == includeNode.end()) {
      // Add the node
      addNode(pNode, Meshes, MeshesBase, INodes, AllLights, excludeNode,
              tvTime);
    }
  }
...
// -----------------------------------------------------------------------------------------------
void CRTWorld::getAllNodeInScene(
    vector<CMesh::CMeshBuild *> &Meshes,
    vector<CMeshBase::CMeshBaseBuild *> &BaseMeshes, vector<INode *> &INodes,
    vector<SLightBuild> &AllLights, const set<INode *> &excludeNode,
    INode *pNode) {
  if (_Ip->GetRootNode() != s_WorldCacheRoot ||
      MaxTStrToUtf8(_Ip->GetCurFileName()) !=
  ...
  for (sint32 i = 0; i < pNode->NumberOfChildren(); ++i)
    getAllNodeInScene(Meshes, BaseMeshes, INodes, AllLights, excludeNode,
...
// -----------------------------------------------------------------------------------------------
bool CRTWorld::intersectionSphereCylinder(CBSphere &s, CVector &cyCenter,
                                          CVector &cyDir, float cyRadius) {
...
bool CRTWorld::isInteractionWithLight(SLightBuild &rSLB, CAABBox &meshBox) {
  ...
  case SLightBuild::LightDir: {
    ...
    if (intersectionSphereCylinder(s, rSLB.Position, rSLB.Direction,
                                   rSLB.rDirRadius))
      return true;
  } break;
  }
...
// -----------------------------------------------------------------------------------------------
bool CRTWorld::isInteractionLightMesh(SLightBuild &rSLB,
                                      NL3D::CMesh::CMeshBuild &rMB,
                                      NL3D::CMeshBase::CMeshBaseBuild &rMBB) {
...
// -----------------------------------------------------------------------------------------------
void CRTRay::initDirectionnal(uint32 nNbSide, const NLMISC::CVector &vVertex,
                              const NLMISC::CVector &lightDir, float rRadius,
                              float rDistCyl) {
  // Construct the matrix with K= vertex to light direction
  CVector vDirection = -lightDir;
  vDirection.normalize();
  CVector I = (fabs(vDirection * CVector(1.f, 0, 0)) > 0.99)
                  ? CVector(0.f, 1.f, 0.f)
                  : CVector(1.f, 0.f, 0.f);
  ...
  // Cone pyramid: perspective and capped at near (for precision and z division)
  // and far (where start the cylinder)
  ...
  // Cylinder pyramid: orthogonal and capped at near (ie rDistCyl, where start
  // the cylinder) only
  ...
  for (uint32 i = 0; i < rShp.Vertices.size(); ++i) {
    rShp.Vertices[i].U =
        rRadius * cosf(i * 2.0f * (float)Pi / rShp.Vertices.size());
    rShp.Vertices[i].V =
        rRadius * sinf(i * 2.0f * (float)Pi / rShp.Vertices.size());
  }
  _DistCyl = rDistCyl;
}
...
// -----------------------------------------------------------------------------------------------
void CRTRay::clipProjected(const NLMISC::CVector &v0, const NLMISC::CVector &v1,
                           const NLMISC::CVector &v2) {
  CUV tri[3];
  ...
  // Backface swapping
  if (((tri[1].U - tri[0].U) * (tri[2].V - tri[0].V) -
       (tri[1].V - tri[0].V) * (tri[2].U - tri[0].U)) < 0.0f) {
    swap(tri[0], tri[1]);
  }
  ...
  for (uint32 i = 0; i < Shapes.size(); ++i) {
    ...
    } else // The shape is not clipped by the triangle so must be add it
           // directly
    {
...
// -----------------------------------------------------------------------------------------------
bool CRTRay::isShapeMustBeClippedByTriangle(CRTRay::SConvexShape &scs,
                                            CUV tri[3]) {
  uint32 i, j;
  bool bAllVertExt;
  ...
  for (i = 0; i < 3; ++i) {
    ...
    for (j = 0; j < scs.Vertices.size(); ++j)
      ...
      if (((p1.U - p0.U) * (scs.Vertices[j].V - p0.V) -
           (p1.V - p0.V) * (scs.Vertices[j].U - p0.U)) > 0.0f) {
  ...
  for (i = 0; i < scs.Vertices.size(); ++i) {
    ...
    for (j = 0; j < 3; ++j)
      // Does the vertex j is interior ?
      if ((p1.U - p0.U) * (tri[j].V - p0.V) -
              (p1.V - p0.V) * (tri[j].U - p0.U) >
          0.0f) {
        bAllVertExt = false; // Yes so all vertex are NOT exterior
        break;
      }
...
// -----------------------------------------------------------------------------------------------
void CRTRay::clipShape(CRTRay::SConvexShape &ShapeIn, CUV Tri[3],
                       vector<CRTRay::SConvexShape> &ShapesOut) {
  uint32 i, j, lastj;
  ...
  for (i = 0; i < 3; ++i) {
    ...
    // Front is the polygon exterior to the side (which does not contain the
    // triangle)
    ...
    bool previousExt =
        ((sideV1.U - sideV0.U) * (BackInput.Vertices[lastj].V - sideV0.V) -
             (sideV1.V - sideV0.V) * (BackInput.Vertices[lastj].U - sideV0.U) <
         0.0f);
    // BackInput -> Back, Front   split from the current edge
    for (j = 0; j < BackInput.Vertices.size(); ++j) {
      bool currentExt =
          ((sideV1.U - sideV0.U) * (BackInput.Vertices[j].V - sideV0.V) -
               (sideV1.V - sideV0.V) * (BackInput.Vertices[j].U - sideV0.U) <
           0.0f);
      if (currentExt) {
        // Vertex j exterior
        ...
        {
          // Ok, intersect
          CUV tmp = getLineIntersection(BackInput.Vertices[lastj],
                                        BackInput.Vertices[j], sideV0, sideV1);
...
void CRTRay::weldVertices(CRTRay::SConvexShape &Shp) {
  ...
  for (uint32 j = 0; j < Shp.Vertices.size(); ++j) {
    ...
    if (((p1.U - p0.U) * (p1.U - p0.U) + (p1.V - p0.V) * (p1.V - p0.V)) >
        0.0001f) {
...
// -----------------------------------------------------------------------------------------------
CUV CRTRay::getLineIntersection(const CUV &l1p1, const CUV &l1p2,
                                const CUV &l2p1, const CUV &l2p2) {
  float denominator = (l1p2.U - l1p1.U) * (l2p2.V - l2p1.V) -
                      (l1p2.V - l1p1.V) * (l2p2.U - l2p1.U);
  if (denominator == 0.0)
    return l1p1; // The segment are colinear
  float k = ((l2p1.U - l1p1.U) * (l2p2.V - l2p1.V) -
             (l2p1.V - l1p1.V) * (l2p2.U - l2p1.U)) /
            denominator;
  if (k <= 0.0f)
    return l1p1;
  if (k >= 1.0f)
...
// -----------------------------------------------------------------------------------------------
SGridCell CRTLightAccelDir::getSel() { return *itSel; }

// -----------------------------------------------------------------------------------------------
void CRTLightAccelDir::nextSel() { ++itSel; }

// -----------------------------------------------------------------------------------------------
bool CRTLightAccelDir::isEndSel() { return (itSel == grid.end()); }
...
// -----------------------------------------------------------------------------------------------
SGridCell CRTLightAccelPoint::getSel() { return *_Selection[_CurSel].itSel; }
...
// -----------------------------------------------------------------------------------------------
bool CRTLightAccelPoint::isEndSel() { return (_CurSel == _Selection.size()); }

// -----------------------------------------------------------------------------------------------
void CRTLightAccelPoint::project(CTriangle &tri, CPlane pyr[4],
                                 CPlane &gridPlane, sint32 nGridNb,
                                 SGridCell &cell) {

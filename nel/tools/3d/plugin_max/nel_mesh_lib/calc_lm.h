...
#define NEL3D_APPDATA_LM_ANIMATED_LIGHT                                        \
  (NEL3D_APPDATA_LM + 1) // The name of the animation used by light
#define NEL3D_APPDATA_LM_ANIMATED                                              \
  (NEL3D_APPDATA_LM + 2) // 1 if this light give an animation, 0 else
#define NEL3D_APPDATA_LM_LIGHT_GROUP                                           \
  (NEL3D_APPDATA_LM +                                                          \
   3) // The light group. 0 : global, 1 : day, 2 : night etc..

#define NEL3D_APPDATA_LM_ANIMATED_LIGHT_DEFAULT ""
#define NEL3D_APPDATA_LM_LIGHT_GROUP_DEFAULT 0

// ***********************************************************************************************
extern void convertToWorldCoordinate(
    NL3D::CMesh::CMeshBuild *pMB, NL3D::CMeshBase::CMeshBaseBuild *pMBB,
    NLMISC::CVector &translation = NLMISC::CVector(0.0f, 0.0f, 0.0f));

extern NLMISC::CMatrix
getObjectToWorldMatrix(NL3D::CMesh::CMeshBuild *pMB,
...
struct SLightBuild {
  ...
  bool bCastShadow;
  bool bAmbientOnly; // true if LightPoint|LightSpot|LightDir with
                     // GetAmbientOnly().
  float rMult;
  ...
  std::set<std::string>
      setExclusion; // List of object name excluded by this light

  float rSoftShadowRadius; // The radius max used when calculating soft shadows
  float rSoftShadowConeLength; // The distance between vertex and cylinder
                               // beginning
 ...
 *	/param visibleOnly if false, all lights are retrieved, else only light
 *not hidden are retrieved
 */
void getLightmapLightBuilds(std::vector<SLightBuild> &lights, TimeValue tvTime,
                            Interface &ip, bool visibleOnly = false);

...
bool CExportNel::buildLodCharacter(NL3D::CLodCharacterShapeBuild &lodBuild,
      ...
      if (tri) {
        ...
        // build the shape
        {
          ...
          /* Build a mesh. (duplicate UV/normals)
              Use a CMeshMRM for an historic reason: simpler because don't use
             "matrix blocks".
          */
          CMRMParameters mrmParams;
          CMeshMRM meshMRM;
          // To avoid heavy MRM compute, ask for only one lod (=> no poly
          // reduction)
          mrmParams.Divisor = 1;
          mrmParams.NLods = 1;
          meshMRM.build(buildBaseMesh, buildMesh,
                        std::vector<CMesh::CMeshBuild *>(), mrmParams);
          ...
          if ((format & CVertexBuffer::PositionFlag) &&
              meshMRMGeom.isSkinned()) {
            uint i;
            ...
            for (i = 0; i < numVerts; i++)
              lodBuild.Vertices[i] =
                  *(const CVector *)vba.getVertexCoordPointer(i);
            // copy skinWeights
            ...
            if (format & CVertexBuffer::NormalFlag) {
              for (i = 0; i < numVerts; i++)
                lodBuild.Normals[i] =
                    *(const CVector *)vba.getNormalCoordPointer(i);
            }
            ...
            for (i = 0; i < meshMRMGeom.getNbRdrPass(0); i++) {
              const CIndexBuffer &pb =
                  meshMRMGeom.getRdrPassPrimitiveBlock(0, i);
              CIndexBufferRead iba;
              pb.lock(iba);
              nlassert(dstTriIdx + pb.getNumIndexes() <=
                       lodBuild.TriangleIndices.size());
              ...
              // if the material of this pass is the 0th material, flag tris for
              // TextureInfo selection
              if (meshMRMGeom.getRdrPassMaterial(0, i) == 0) {
                for (uint tri = dstTriIdx / 3;
                     tri < dstTriIdx / 3 + pb.getNumIndexes() / 3; tri++)

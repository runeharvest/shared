...
public:
  ...
  // if not empty, compile all bone scripts, and add to the animation of the
  // skeleton
...
public:
  enum { NoError = 0, VertexWithoutWeight, InvalidSkeleton, CodeCount };
  ...
  /// Constructor
  CExportNel(bool errorInDialog, bool view, bool absolutePath, Interface *ip,
             std::string errorTitle, CExportNelOptions *opt);
   ...
   */
  NL3D::IShape *buildShape(INode &node, TimeValue time,
                           const TInodePtrInt *nodeMap, bool buildLods);
   ...
   */
  NL3D::CMesh::CMeshBuild *
  createMeshBuild(INode &node, TimeValue tvTime,
                  NL3D::CMesh::CMeshBaseBuild *&baseBuild,
  ...
  /** Use interface mesh from a max file to unify normal at extremities of a
   * mesh Example : a character is sliced in slots, each parts being stored in a
   * file. The normals at the junction part are incorrect. An "interface" is a
   * polygon that is used to unify normal between various meshs IMPORTANT : the
   * meshbuild should have been exported in WORLD SPACE Note : the name of the
   * max file that contains the name of the interface is stored in an app data
   * attached to this node \param node the node from which datas must be
   * retrieved (name of the .max file containing the interfaces) \param
   * meshBuildToModify The mesh build whose normal will be modified \param
   * toWorldMat a matrix to put the meshbuild vertices into worldspace \param
   * tvTime time aty which evaluate the mesh
   */
  void applyInterfaceToMeshBuild(INode &node,
                                 NL3D::CMesh::CMeshBuild &meshBuildToModify,
                                 const NLMISC::CMatrix &toWorldMat,
                                 TimeValue tvTime);

  /** This takes a max mesh, and select the vertices that match vertices of a
   * mesh interface This has no effect if the mesh has no app datas specifying a
   * mesh interface \see applyInterfaceToMeshBuild \return true if the operation
   * succeed
   */
   ...
   */
  NL3D::CInstanceGroup *
  buildInstanceGroup(const std::vector<INode *> &vectNode,
                     std::vector<INode *> &resultInstanceNode,
                     TimeValue tvTime);
   ...
   * \param landscape is a pointer ona landscape created with the scene. Can be
   * NULL if you dan't want to build landscape zones. \param progress is the
   * progress bar to use to display build progression. Can be NULL if no
   * prgoress bar is needed \param buildHidden If it is true, build hidden nodes
   * \param onlySelected If it is true, build only selected nodes
   * \param buildLods If it is true, build lod of objects
   */
  void buildScene(NL3D::CScene &scene, NL3D::CShapeBank &shapeBank,
                  NL3D::IDriver &driver, TimeValue tvTime,
                  NL3D::CLandscape *landscape, IProgress *progress,
                  bool buildHidden, bool onlySelected, bool buildLods);
   ...
   * skeletonShape must be NULL if no bones.
   *
   *	if excludeCollision then return false if the mesh is a collision
   *(NL3D_APPDATA_COLLISION) else don't test NL3D_APPDATA_COLLISION.
   */
   ...
   */
  static bool buildMeshAABBox(INode &node, NLMISC::CAABBox &dest,
                              TimeValue time);
  ...
  bool calculateLM(NL3D::CMesh::CMeshBuild *pZeMeshBuild,
                   ...
                   INode &ZeNode, TimeValue tvTime, uint firstMaterial,
                   bool outputLightmapLog);
  ...
  // Add animation track of this node into the animation object pass in
  // parameter.
  void addAnimation(NL3D::CAnimation &animation, INode &node,
                    const char *sBaseName, bool root);

  // Build a NeL track with a 3dsmax node and a controller.
  NL3D::ITrack *buildATrack(NL3D::CAnimation &animation, Control &c,
                            TNelValueType type, Animatable &node,
                            const CExportDesc &desc,
                            CAnimationBuildCtx *animBuildCtx,
                            bool bodyBiped = false);

  // Build a Nel bool track from a On/Off max Controller (doesn't work with
  // buildATRack, which require a keyframer interface , which isn't provided by
  // an on / off controller)
  static NL3D::CTrackKeyFramerConstBool *buildOnOffTrack(Control &c);

  // Add tracks for particle systems
  void addParticleSystemTracks(NL3D::CAnimation &animation, INode &node,
                               const char *parentName);

  // Add tracks for the bone and its children (recursive)
  void addBoneTracks(NL3D::CAnimation &animation, INode &node,
                     const char *parentName, CAnimationBuildCtx *animBuildCtx,
                     bool root, CSSSBuild &ssBuilder);

  // Add biped tracks
  void addBipedNodeTracks(NL3D::CAnimation &animation, INode &node,
                          const char *parentName,
  ...
  // Add a SkeletonSpawnScript track. It takes the first note track of the
  // object
  ...
  static void buildNelKey(NL3D::CKeyFloat &nelKey, ILinFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyInt &nelKey, ILinFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBool &nelKey, ILinFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyVector &nelKey, ILinPoint3Key &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyRGBA &nelKey, ILinPoint3Key &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyQuat &nelKey, ILinRotKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyVector &nelKey, ILinScaleKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);

  static void buildNelKey(NL3D::CKeyBezierFloat &nelKey, IBezFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBool &nelKey, IBezFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBezierVector &nelKey, IBezPoint3Key &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBezierQuat &nelKey, IBezQuatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBezierVector &nelKey, IBezScaleKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);

  static void buildNelKey(NL3D::CKeyTCBFloat &nelKey, ITCBFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyBool &nelKey, ITCBFloatKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyTCBVector &nelKey, ITCBPoint3Key &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyTCBQuat &nelKey, ITCBRotKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);
  static void buildNelKey(NL3D::CKeyTCBVector &nelKey, ITCBScaleKey &maxKey,
                          float ticksPerSecond, const CExportDesc &desc,
                          Control &c);

  // Create the transform matrix tracks
  void createBipedKeyFramer(NL3D::ITrack *&nelRot, NL3D::ITrack *&nelPos,
                            bool isRot, bool isPos, float ticksPerSecond,
                            const Interval &range, int oRT,
                            const CExportDesc &desc, INode &node,
                            CAnimationBuildCtx *animBuildCtx);
   ...
   *
   * mapBindPos is the pointer of the map of bind pos by bone. Can be NULL if
   * the skeleton is already in the bind pos.
   */
  void buildSkeletonShape(NL3D::CSkeletonShape &skeletonShape, INode &node,
                          mapBoneBindPos *mapBindPos, TInodePtrInt &mapId,
                          TimeValue time);

  // Build an array of CBoneBase
  void buildSkeleton(std::vector<NL3D::CBoneBase> &bonesArray, INode &node,
                     mapBoneBindPos *mapBindPos, TInodePtrInt &mapId,
                     std::set<std::string> &nameSet, TimeValue time,
                     sint32 &idCount, sint32 father = -1);

  /*
   * Add Skinning data into the build structure.
   * You must call first isSkin. If isSkin return true, then you can call this
   * method. Can return the following errors:
   *
   * skeletonShape must be NULL if no bones.
   *
   * NoError
   ...
   * InvalidSkeleton
   */
  static uint buildSkinning(NL3D::CMesh::CMeshBuild &buildMesh,
                            const TInodePtrInt &skeletonShape, INode &node);

  // Return true if the mesh is a skin, else return false.
  static bool isSkin(INode &node);

  // Return the root node of the skeleton attached to the node. Return NULL if
  // no skeleton.
  ...
  /// return true if the bone must unherit his father scale. Special cases for
  /// biped, biped father etc...
  bool getNELUnHeritFatherScale(INode &node);

  /**	This method do the same thing as getLocalMatrix() mut manages complexs
     cases of Scale unheritance, strange Biped node Scale stuff, etc....
   */
  void getNELBoneLocalTM(INode &node, TimeValue time, NLMISC::CVector &nelScale,
                         NLMISC::CQuat &nelQuat, NLMISC::CVector &nelPos);

  /// Physique Mirroring
  bool mirrorPhysiqueSelection(INode &node, TimeValue time,
                               const std::vector<uint> &vertIn,
                               float threshold);
  ...
  void addChildLodNode(std::set<INode *> &lodListToExclude,
                       INode *current = NULL);
  void addParentLodNode(INode &node, std::set<INode *> &lodListToExclude,
                        INode *current = NULL);
  ...
  createCollisionMeshBuild(std::vector<INode *> &nodes, TimeValue tvTime);

  /** Export a list of CCollisionMeshBuild from a list of node, grouped by
   *igName. User must delete the meshBuilds. meshBuildList is a vector of tuple
   *igName-Cmb NB: check NEL3D_APPDATA_COLLISION. if not set, the node is not
   *exported
   */
  bool createCollisionMeshBuildList(
      std::vector<INode *> &nodes, TimeValue time,
      std::vector<std::pair<std::string, NLPACS::CCollisionMeshBuild *>>
          &meshBuildList);

  /**
   *	Parse all the scene and build retrieverBank and globalRetriever. NULL if
   *no collisions mesh found. \param retIgName eg: if IgName of a collisionMesh
   *"col_pipo_1" is found in the scene, and if igNamePrefix=="col_" and
   *igNameSuffix=="_", then retIgName returned is "pipo". If different igName
   *may match, result is undefined (random).
   */
  void computeCollisionRetrieverFromScene(
      TimeValue time, NLPACS::CRetrieverBank *&retrieverBank,
      NLPACS::CGlobalRetriever *&globalRetriever, const char *igNamePrefix,
      const char *igNameSuffix, std::string &retIgName);

  /**
   * Retrieve the Z rotation in radian of an object with its I matrix vector,
   * assuming that the K matrix vector is near CVector::K
   */
  static float getZRot(const NLMISC::CVector &i);

  /**
   * Build a primtive block with an array of inode. Return false if one of the
   * object is not a PACS primitive object.
   */
  bool buildPrimitiveBlock(TimeValue time, std::vector<INode *> objects,
  ...
  static void convertMatrix(NLMISC::CMatrix &nelMatrix,
                            const Matrix3 &maxMatrix);

  // Convert a NeL matrix in 3dsmax matrix
  static void convertMatrix(Matrix3 &maxMatrix,
                            const NLMISC::CMatrix &nelMatrix);

  /// Convert a 3dsmax uv matrix to a nel uv matrix
  static void uvMatrix2NelUVMatrix(const Matrix3 &uvMatrix,
                                   NLMISC::CMatrix &dest);

  // Convert a 3dsmax vector in NeL vector
  static void convertVector(NLMISC::CVector &nelVector,
                            const Point3 &maxVector);
  ...
  static void decompMatrix(NLMISC::CVector &nelScale, NLMISC::CQuat &nelRot,
                           NLMISC::CVector &nelPos, const Matrix3 &maxMatrix);
  // Convert a 3dsmax color in NeL color
  static void convertColor(NLMISC::CRGBA &nelColor, const Color &maxColor);

  // Return true if the node as the classId classid or is derived from a class
  // with classId class id
  static bool isClassIdCompatible(Animatable &node, Class_ID &classId);

  // Return the pointer on ther subanim with the name sName of the node. If it
  ...
  // Get the NEL node name. ie either NEL3D_APPDATA_INSTANCE_SHAPE appData or
  // just GetName().
  static std::string getNelObjectName(INode &node);

  // Get lights
  void getLights(std::vector<NL3D::CLight> &vectLight, TimeValue time,
                 INode *node = NULL);

  // Get the root node of the scene
  INode *getRootNode() const;
  // Get All node (objects only) of a hierarchy. NULL => all the scene
  void getObjectNodes(std::vector<INode *> &vectNode, TimeValue time,
                      INode *node = NULL);

  // *** Paramblock2 access

  // Return the pointer on the subanim with the name sName of the node. If it
  // doesn't exist, return NULL.
  static Control *getControlerByName(Animatable &node, const char *sName);

  // Return the pointer on the subanim with the name sName of the node. If it
  // doesn't exist, return NULL. Support only those type: TYPE_FLOAT TYPE_INT
  // TYPE_POINT3
  // TYPE_BOOL
  static bool getValueByNameUsingParamBlock2(Animatable &node,
                                             const char *sName, ParamType2 type,
                                             void *pValue, TimeValue time,
                                             bool verbose = true);
  ...
  static bool scriptEvaluate(const char *script, void *out,
                             TNelScriptValueType type);
  ...
  // Get an appData integer
  static std::string getScriptAppData(Animatable *node, uint32 id,
                                      const std::string &def);

  // Set an appData integer
  static void setScriptAppData(Animatable *node, uint32 id,
                               const std::string &value);

  // Get an appData RGBA
  static NLMISC::CRGBA getScriptAppData(Animatable *node, uint32 id,
                                        NLMISC::CRGBA def);
  ...
  // Get an appData VertexProgram WindTree (ensure same default values for all
  // retrieve).
  static void getScriptAppDataVPWT(Animatable *node, CVPWindTreeAppData &apd);

  // Set an appData VertexProgram WindTree.
  static void setScriptAppDataVPWT(Animatable *node,
                                   const CVPWindTreeAppData &apd);

  /** private func : this convert a polygon expressed as a max mesh into a list
   * of ordered vectors. This also gives an average normal by averaging faces
   * normals.
   */
  static void maxPolygonMeshToOrderedPoly(Mesh &mesh,
                                          std::vector<NLMISC::CVector> &dest,
                                          const NLMISC::CMatrix &basis,
                                          NLMISC::CVector &avgNormal);
   ...
   * the skeleton is already in the bind pos.
   */
  bool buildVegetableShape(NL3D::CVegetableShape &skeletonShape, INode &node,
                           TimeValue time);
   ...
   */
  bool buildLodCharacter(NL3D::CLodCharacterShapeBuild &lodBuild, INode &node,
                         TimeValue time, const TInodePtrInt *nodeMap);
  ...
  /// Return true if error must be in dialog, false if error must not stop the
  /// process.
...
private:
  ...
  public:
    ...
    // Default constructor init _IndexInMaxMaterial to MISSING_VALUE, _UVMatrix
    // to indentity and clamp to 0, 0, 1, 1
    CMaterialDesc() {
  ...
  public:
     ...
     * (UVRouting[i] == i)		UV channel i is present in the vertex
     * buffer (UVRouting[i] != i)		UV channel i is not present in
     * the vertex buffer. It is routed to another UV channel.*/
  ...
  // Can return an interger in [0; MAX_MESHMAPS-1] or on of the following value:
  // UVGEN_REFLEXION, UVGEN_MISSING
  static int getVertMapChannel(Texmap &texmap, Matrix3 &channelMatrix,
                               TimeValue time);

  // Build a CLight
  static bool buildLight(GenLight &maxLight, NL3D::CLight &nelLight,
                         INode &node, TimeValue time);
   ...
   */
  void buildBaseMeshInterface(NL3D::CMeshBase::CMeshBaseBuild &buildMesh,
                              CMaxMeshBaseBuild &maxBaseBuild, INode &node,
   ...
   * if isMorphTarget is true, no "mrm/normal mesh interface" is built
   */
  void buildMeshInterface(
      TriObject &tri, NL3D::CMesh::CMeshBuild &buildMesh,
  ...
  // Build a water shape. The given node must have a water materiel, an
  ...
  /** Test wether the given max node has a water material. A water object should
   * only have one material, and must have planar, convex geometry. Morevover,
   * the mapping should only have scale and offsets, no rotation
   */
  static bool hasWaterMaterial(INode &node, TimeValue time);

  // Build an array of NeL material corresponding with max material at this
  // node.
  void buildMaterials(std::vector<NL3D::CMaterial> &Materials,
                      CMaxMeshBaseBuild &maxBaseBuild, INode &node,
                      TimeValue time);

  // Build a NeL material corresponding with a max material.
  void buildAMaterial(NL3D::CMaterial &material, CMaxMaterialInfo &materialInfo,
                      Mtl &mtl, TimeValue time);

  // Build a NeL texture corresponding with a max Texmap.
  NL3D::ITexture *buildATexture(Texmap &texmap,
                                CMaterialDesc &remap3dsTexChannel,
                                TimeValue time, bool forceCubic = false);

  /// Build a NeL texture cube from a Reflect/refract map containing 6 textures.
  NL3D::CTextureCube *buildTextureCubeFromReflectRefract(Texmap &texmap,
                                                         TimeValue time);

  /// Build a NeL texture cube from a Composite map containing 6 textures.
  /// (note: no re-ordering is done, because it didn't reorder composite cube in
  /// previous version either).
  NL3D::CTextureCube *buildTextureCubeFromComposite(Texmap &texmap,
                                                    TimeValue time);

  /// Build a NeL texture cube from a single texture.
  NL3D::CTextureCube *buildTextureCubeFromTexture(Texmap &texmap,
                                                  TimeValue time);

public:
  /** Return true if a mesh has a material whose shader requires a specific
   * vertex shader to work (for example, per-pixel lighting). This also stores
   * the result in shader
   */
  static bool hasMaterialWithShaderForVP(INode &node, TimeValue time,
                                         NL3D::CMaterial::TShader &shader);

  /// Test wether the given material need a specific vertex program to work
  /// correctly
  static bool needVP(Mtl &mat, TimeValue time,
                     NL3D::CMaterial::TShader &shader);

private:
  /** Build a mesh vertex program associated with the given shader
   * For now there can be only one vertex program per mesh, so you must specify
   * the shader this v.p must be built for. The meshBuild may be modfied by this
   * operation (to add tangent space infos for example) \return The vertex
   * program or NULL if the op failed, or if there's no need for a v.p.
   */
  static NL3D::IMeshVertexProgram *
  buildMeshMaterialShaderVP(NL3D::CMaterial::TShader shader,
  ...
  void addNodeTracks(NL3D::CAnimation &animation, INode &node,
                     const char *parentName, CAnimationBuildCtx *animBuildCtx,
                     bool root, CSSSBuild &ssBuilder, bool bodyBiped = false);

  // Add tracks for the light
  void addLightTracks(NL3D::CAnimation &animation, INode &node,
                      const char *parentName);

  // Add tracks for the morphing
  void addMorphTracks(NL3D::CAnimation &animation, INode &node,
                      const char *parentName);

  // Add tracks for the object
  void addObjTracks(NL3D::CAnimation &animation, Object &obj,
                    const char *parentName);

  // Add tracks for the material
  void addMtlTracks(NL3D::CAnimation &animation, Mtl &mtl,
                    const char *parentName);

  // Add tracks for the texture
  void addTexTracks(NL3D::CAnimation &animation, Texmap &tex, uint stage,
                    const char *parentName);

  // Get a biped key parameter using script
  bool getBipedKeyInfo(const char *nodeName, const char *paramName, uint key,
                       float &res);
  ...
  public:
    ...
    // The animation range for Biped Nodes. BipedRangeMin and BipedRangeMax are
    // inclusive
  ...
  public:
    ...
    /* return true if this node must export Track Position. It returns true for
       bodyBiped and LClavicle and RClavicle limb for now. NB: We use script to
       now if the node is a clavicle (ie not hack-tested by node name) But this
       slow test is done in compileBiped().
    */
  ...
  //
  void overSampleBipedAnimation(CAnimationBuildCtx &animBuildCtx,
                                uint overSampleValue);
  ...
  void getNELBoneLocalScale(INode &node, TimeValue time,

...
CExportNel::CExportNel(bool errorInDialog, bool view, bool absolutePath,
                       Interface *ip, std::string errorTitle,
...
Matrix3 CExportNel::viewMatrix2CameraMatrix(const Matrix3 &viewMatrix) {
  ...
  // This matrix is used to compute the coordinates to the viewport space. Take
  // ths invert.
...
bool getValueByNameUsingParamBlock2Internal(Animatable &node, const char *sName,
                                            ParamType2 type, void *pValue,
                                            TimeValue tvTime) {
  // for all parameters block in this node
  for (int nBlock = 0; nBlock < node.NumParamBlocks(); nBlock++) {
    // Get the paramblock2 pointer
    ...
    for (int nSub = 0; nSub < param->NumParams(); nSub++) {
      ...
      if (strcmp(MCharStrToUtf8(paramDef.int_name).c_str(), sName) == 0) {
        ...
        case TYPE_STRING_TAB:
          typeOk = (paramType == TYPE_FILENAME_TAB) ||
                   (paramType == TYPE_STRING_TAB);
          break;
        default:
        ...
        if (typeOk) {
          ...
          nlwarning("Invalid type specified for pblock2 value with name '%s', "
                    "given type '%u', found '%u'",
  ...
  for (uint s = 0; s < numSubAnim; s++) {
    ...
    if (node.SubAnim(s)) {
      ...
      // Get the ctrl for sub anim
      if (getValueByNameUsingParamBlock2Internal(*node.SubAnim(s), sName, type,
                                                 pValue, tvTime))
        return true;
    }
  }
...
bool CExportNel::getValueByNameUsingParamBlock2(Animatable &node,
                                                ...
                                                TimeValue tvTime,
                                                bool verbose) {
  if (getValueByNameUsingParamBlock2Internal(node, sName, type, pValue,
                                             tvTime)) {
    return true;
  } else {
...
Modifier *CExportNel::getModifier(INode *pNode, Class_ID modCID) {
  ...
  if (os.obj && (os.obj->SuperClassID() != GEOMOBJECT_CLASS_ID) &&
      (os.obj->SuperClassID() != LIGHT_CLASS_ID)) {
    return NULL;
  }
...
std::string CExportNel::getNelObjectName(INode &node) {
  // Workaround for FX (don't know why, but the AppData are not copied when FX
  // are duplicated, so try to get the name in another way) If this is a
  // particle system, try to get the name of the shape.from the param blocks
  ...
  if (obj) {
    ...
    if (clid.PartA() == NEL_PARTICLE_SYSTEM_CLASS_ID) {
      ...
      if (CExportNel::getValueByNameUsingParamBlock2(
              node, "ps_file_name", (ParamType2)TYPE_STRING, &shapeName, 0)) {
  ...
  AppDataChunk *ad =
      node.GetAppDataChunk(MAXSCRIPT_UTILITY_CLASS_ID, UTILITY_CLASS_ID,
                           NEL3D_APPDATA_INSTANCE_SHAPE);
  if (ad && ad->data) {
    ...
    Object *obj = os.obj;
    if (obj) {
      ad = obj->GetAppDataChunk(MAXSCRIPT_UTILITY_CLASS_ID, UTILITY_CLASS_ID,
                                NEL3D_APPDATA_INSTANCE_SHAPE);
      if (ad && ad->data) {
        if (_tcslen((const TCHAR *)ad->data) != 0) {
          // get file name only
          return NLMISC::CFile::getFilename(
              MCharStrToUtf8((const MCHAR *)ad->data));
        } else {
          return MCharStrToUtf8(node.GetName());
        }
      } else {
...
void CExportNel::decompMatrix(NLMISC::CVector &nelScale, NLMISC::CQuat &nelRot,
...
void CExportNel::getLocalMatrix(Matrix3 &localMatrix, INode &node,
                                TimeValue time) {
  INode *parent;
...
bool CExportNel::hasLightMap(INode &node, TimeValue time) {
  ...
  if (nMaterialCount > 0) {
    ...
    for (int nSub = 0; nSub < nMaterialCount; nSub++) {
      ...
      CExportNel::getValueByNameUsingParamBlock2(
          *pSub, "bLightMap", (ParamType2)TYPE_BOOL, &bLightMap, 0);
      if (bLightMap)
        return true;
    }
  }
  // Else check only this material
    ...
    int bLightMap = 0; // false
    CExportNel::getValueByNameUsingParamBlock2(
        *pNodeMat, "bLightMap", (ParamType2)TYPE_BOOL, &bLightMap, 0);
    if (bLightMap)
      return true;
  }
  return false;
...
void CExportNel::outputErrorMessage(const std::string &message) {
  if (_ErrorInDialog) {
    MessageBoxW(_Ip->GetMAXHWnd(), nlUtf8ToWide(message),
                nlUtf8ToWide(_ErrorTitle), MB_OK | MB_ICONEXCLAMATION);
  }
...
bool CExportNel::isVegetable(INode &node, TimeValue time) {
  return CExportNel::getScriptAppData(&node, NEL3D_APPDATA_VEGETABLE,
...
bool CExportNel::isLodCharacter(INode &node, TimeValue time) {
  return CExportNel::getScriptAppData(&node, NEL3D_APPDATA_CHARACTER_LOD,
                                      BST_UNCHECKED) != BST_UNCHECKED;
}

// --------------------------------------------------

void CExportNel::addChildLodNode(std::set<INode *> &lodListToExclude,
                                 INode *current) {
  // First node ?
  if (current == NULL)
  ...
  for (uint lod = 0; lod < lodCount; lod++) {
    // Get lod name
    std::string lodName =
        getScriptAppData(current, NEL3D_APPDATA_LOD_NAME + lod, "");
    if (lodName != "") {
      // Get the lod by name
...
void CExportNel::addParentLodNode(INode &child,
  ...
  for (uint lod = 0; lod < lodCount; lod++) {
    // Get lod name
    std::string lodName =
        getScriptAppData(parent, NEL3D_APPDATA_LOD_NAME + lod, "");
    if (lodName != "") {
      // Get the lod by name
      INode *lodNode = _Ip->GetINodeByName(MaxTStrFromUtf8(lodName));
      if (lodNode == &child) {
...
void CExportNel::uvMatrix2NelUVMatrix(const Matrix3 &maxMatrix,
                                      NLMISC::CMatrix &dest) {
  // Basis vector
  CVector I, J, K, P;
  ...
  dest = convert * dest *
         convert; // exported v are already inverted therefore the conversion
}

// --------------------------------------------------
void CExportNel::getObjectNodes(std::vector<INode *> &vectNode, TimeValue time,
                                INode *node) {
  // Get the root node
  if (node == NULL)
...
// --------------------------------------------------
INode *CExportNel::getRootNode() const { return _Ip->GetRootNode(); }
...
uint CExportNel::getLightGroup(INode *node) {
  return CExportNel::getScriptAppData(node, NEL3D_APPDATA_LM_LIGHT_GROUP,
                                      NEL3D_APPDATA_LM_LIGHT_GROUP_DEFAULT);
}
...
struct CMaxMeshSeg {
  ...
  CMaxMeshSeg(uint v0, uint v1) : V0(v0), V1(v1) {}
};

// private : predicate to ordonate segments (used by
// maxPolygonMeshToOrderedPoly)
struct CPredNextSegOf {
  uint Prev;
  CPredNextSegOf(uint prev) : Prev(prev) {}
  bool operator()(const CMaxMeshSeg &value) const {
    return value.V0 == Prev || value.V1 == Prev;
  }
};

/// Get normal of a max triangle in nel format
static NLMISC::CVector
getMaxFaceNormal(const Mesh &m, const NLMISC::CMatrix &basis, uint faceIndex) {
  CVector corner[3];
  for (uint k = 0; k < 3; ++k) {
...
void CExportNel::maxPolygonMeshToOrderedPoly(Mesh &mesh,
                                             std::vector<NLMISC::CVector> &dest,
                                             const NLMISC::CMatrix &basis,
                                             NLMISC::CVector &avgNormal) {
  /// We use a very simple (but slow) algo : examine for each segment how many
  /// tris share it. If it is one then it is a border seg Then, just order
  /// segments
  ...
  for (;;) {
    TSegList::iterator nextSeg = std::find_if(
        borderSegs.begin(), borderSegs.end(), CPredNextSegOf(nextToFind));
    if (nextSeg == borderSegs.end())
      return;
    CExportNel::convertVector(
        pos, mesh.verts[nextSeg->V0 == nextToFind ? nextSeg->V0 : nextSeg->V1]);
    dest.push_back(basis * pos);
...
float toFloatMax(const TCHAR *src) {
  float result = 0.f;
  if (toFloatMax(MCharStrToUtf8(src), result))
    return result;
  return 0.f;
}
...
///=======================================================================
void CExportNel::buildCamera(NL3D::CCameraInfo &cameraInfo, INode &node,
                             TimeValue time) {
  // Eval the object a time
  ObjectState os = node.EvalWorldState(time);
  ...
  if (os.obj) {
    // Object can convert itself to NeL patchmesh ?
    GenCamera *genCamera = (GenCamera *)os.obj->ConvertToType(
        time, Class_ID(LOOKAT_CAM_CLASS_ID, 0));
    if (genCamera) {
...
bool CExportNel::isErrorInDialog() const { return _ErrorInDialog; }

...
void CRadialVertices::init(INode *node, Mesh *mesh, TimeValue time,
                           Interface &ip, CExportNel &nelExport) {
  // Reset the mask
  ...
  // For each appdata
  uint app;
  for (app = NEL3D_RADIAL_FIRST_SM;
       app < NEL3D_RADIAL_FIRST_SM + NEL3D_RADIAL_NORMAL_COUNT; app++) {
    // Get the appvalue
    string pivotName = CExportNel::getScriptAppData(
        _NodePtr, NEL3D_APPDATA_RADIAL_NORMAL_SM + app - NEL3D_RADIAL_FIRST_SM,
        "");
    ...
    if (!pivotName.empty()) {
      ...
      if (pivotNode) {
        ...
        char msg[512];
        smprintf(msg, 512, "Can't find pivot node named '%s' of length %u",
                 pivotName.c_str(), (uint)pivotName.length());
    ...
    for (app = NEL3D_RADIAL_FIRST_SM;
      ...
      if (_SmoothingGroupMask & (1 << app)) {
        ...
        if ((uint32)_MeshPtr->faces[face].smGroup & (1 << app)) {
          ...
          CVector vertex(_MeshPtr->verts[vertex].x, _MeshPtr->verts[vertex].y,
                         _MeshPtr->verts[vertex].z);

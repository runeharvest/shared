...
/** Test wether the given max material is a water material. A water object
 * should only have one material, and must have planar, convex geometry.
...
bool CExportNel::hasWaterMaterial(INode &node, TimeValue time) {
  ...
  if (pNodeMat == NULL)
    return false;
  if (pNodeMat->NumSubMtls() != 0)
    return 0; // subMaterials not supported for water

  int bWater = 0; // false
  CExportNel::getValueByNameUsingParamBlock2(
      *pNodeMat, "bWater", (ParamType2)TYPE_BOOL, &bWater, time);
  return bWater != 0;
}

///===================================================================================================
bool CExportNel::hasMaterialWithShaderForVP(INode &node, TimeValue time,
...
///===================================================================================================
bool CExportNel::needVP(Mtl &mat, TimeValue time,
                        NL3D::CMaterial::TShader &shader) {
  if (mat.NumSubMtls() != 0) // look at sub materials
  {
    for (uint k = 0; k < (uint)mat.NumSubMtls(); ++k) {
      if (CExportNel::needVP(*(mat.GetSubMtl(k)), time, shader))
        return true;
    }
    return false;
  }
  ...
  CExportNel::getValueByNameUsingParamBlock2(
      mat, "iShaderType", (ParamType2)TYPE_INT, &shaderType, time);
  switch (shaderType) {
...
static NL3D::CMeshVPPerPixelLight *
BuildPerPixelLightingVP(NL3D::CMesh::CMeshBuild *mb, bool wantSpecular) {
  // We need at least one texture set to be able to build the tangent space
  // information. In this version, we support only one texture coordinate for
  // the input. If there are more the result is undefined.
...
///===================================================================================================
IMeshVertexProgram *
CExportNel::buildMeshMaterialShaderVP(NL3D::CMaterial::TShader shader,
  ...
  case CMaterial::PerPixelLightingNoSpec:
    ...
    break;
  default:
    return NULL; // no need for a vp
  }
}

// Build an array of NeL material corresponding with max material at this node.
// Return the number of material exported. Fill an array to remap the 3ds
// vertexMap channels for each materials. maxBaseBuild.RemapChannel.size() must
// be == to materials.size(). maxBaseBuild.RemapChannel[mat].size() is the final
// count of NeL vertexMap channels used for the material nb mat. For each NeL
// channel of a material, copy the 3ds channel
// maxBaseBuild.RemapChannel[nelChannel]._IndexInMaxMaterial using the
// transformation matrix maxBaseBuild.RemapChannel[nelChannel]._UVMatrix.
// maxBaseBuild.NeedVertexColor will be true if at least one material need
// vertex color. Forced to true if lightmaps are used.
// maxBaseBuild.AlphaVertex[mat] will be true if the material use per vertex
// alpha. maxBaseBuild.AlphaVertexChannel[mat] will be the channel to use to get
// the alpha if the material use per vertex alpha. This method append the node
// material to the vector passed.
void CExportNel::buildMaterials(std::vector<NL3D::CMaterial> &materials,
  ...
  if (pNodeMat != NULL) {
    ...
    if (nMaterialCount > 0) {
      ...
      for (int nSub = 0; nSub < nMaterialCount; nSub++) {
        ...
        // Export it
        buildAMaterial(materials[maxBaseBuild.FirstMaterial + nSub],
                       maxBaseBuild.MaterialInfo[nSub], *pSub, time);

        // Need vertex color ?
        maxBaseBuild.NeedVertexColor |=
            maxBaseBuild.MaterialInfo[nSub].AlphaVertex |
            maxBaseBuild.MaterialInfo[nSub].ColorVertex;
      }
    }
    // Else export only this material, so, count is 1
      ...
      buildAMaterial(materials[maxBaseBuild.FirstMaterial],
                     maxBaseBuild.MaterialInfo[0], *pNodeMat, time);

      // Need vertex color ?
      maxBaseBuild.NeedVertexColor |= maxBaseBuild.MaterialInfo[0].AlphaVertex |
                                      maxBaseBuild.MaterialInfo[0].ColorVertex;
    }
  }
  ...
  if (nMaterialCount == 0) {
    ...
    // Default mat
    materials[maxBaseBuild.FirstMaterial].setLighting(
        true, CRGBA::Black, CRGBA::White, CRGBA::White, CRGBA::Black);
...
void CExportNel::buildAMaterial(NL3D::CMaterial &material,
                                CMaxMaterialInfo &materialInfo, Mtl &mtl,
                                TimeValue time) {
  ...
  if (isClassIdCompatible(mtl, Class_ID(NEL_MTL_A, NEL_MTL_B))) {
    ...
    int iShaderType = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "iShaderType", (ParamType2)TYPE_INT, &iShaderType, time);
    ...
    // Water or lightmap ??
    bool isWaterOrLightmap =
        iShaderType == SHADER_WATER || iShaderType == SHADER_LIGHTMAP;
    ...
    int bUnlighted = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bUnlighted", (ParamType2)TYPE_BOOL, &bUnlighted, 0);
    ...
    int bStainGlass = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bStainedGlassWindow", (ParamType2)TYPE_BOOL, &bStainGlass, time);
    ...
    int bAlphaBlend = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bAlphaBlend", (ParamType2)TYPE_BOOL, &bAlphaBlend, time);
    ...
    int blendFunc = 1;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "iBlendSrcFunc", (ParamType2)TYPE_INT, &blendFunc, time);
    material.setSrcBlend((CMaterial::TBlend)(blendFunc - 1));
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "iBlendDestFunc", (ParamType2)TYPE_INT, &blendFunc, time);
    ...
    int bAlphaTest = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bAlphaTest", (ParamType2)TYPE_BOOL, &bAlphaTest, time);
    material.setAlphaTest(bAlphaTest != 0);

    // Vertex alpha
    int bAlphaVertex = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bAlphaVertex", (ParamType2)TYPE_BOOL, &bAlphaVertex, time);
    // if water or lightMap, disable alphaVertex
    if (isWaterOrLightmap)
      bAlphaVertex = 0;

    // Active vertex alpha if in alpha test or alpha blend mode
    materialInfo.AlphaVertex =
        (bAlphaBlend || bAlphaTest) && (bAlphaVertex != 0);

    // Get channel to use for alpha vertex
    materialInfo.AlphaVertexChannel = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "iAlphaVertexChannel", (ParamType2)TYPE_INT,
        &materialInfo.AlphaVertexChannel, time);
    ...
    float zBias = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "fZBias", (ParamType2)TYPE_FLOAT, &zBias, time);
    ...
    if (bAlphaBlend) {
      ...
      int bForceZWrite = 0;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "bForceZWrite", (ParamType2)TYPE_BOOL, &bForceZWrite, time);
      material.setZWrite(bForceZWrite != 0);
    } else {
      // Force no z write ?
      int bForceNoZWrite = 0; // false
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "bForceNoZWrite", (ParamType2)TYPE_BOOL, &bForceNoZWrite, time);
    ...
    int bColorVertex = 0;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bColorVertex", (ParamType2)TYPE_BOOL, &bColorVertex, time);
    ...
    CRGBA nelDiffuse;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "cDiffuse", (ParamType2)TYPE_RGBA, &maxDiffuse, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "diffuse", (ParamType2)TYPE_RGBA, &maxDiffuse, time);
    ...
    float fOp = 0.0f;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "pOpacity", (ParamType2)TYPE_PCNT_FRAC, &fOp, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "opacity", (ParamType2)TYPE_PCNT_FRAC, &fOp, time);
    ...
    Point3 maxSelfIllum;
    float fTemp;
    CExportNel::getValueByNameUsingParamBlock2(mtl, "bUseSelfIllumColor",
                                               (ParamType2)TYPE_BOOL,
                                               &bSelfIllumColorOn, time) ||
        CExportNel::getValueByNameUsingParamBlock2(mtl, "useSelfIllumColor",
                                                   (ParamType2)TYPE_BOOL,
                                                   &bSelfIllumColorOn, time);
    if (bSelfIllumColorOn) {
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "cSelfIllumColor", (ParamType2)TYPE_RGBA, &maxSelfIllum, time) ||
          CExportNel::getValueByNameUsingParamBlock2(mtl, "selfIllumColor",
                                                     (ParamType2)TYPE_RGBA,
                                                     &maxSelfIllum, time);
    } else {
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "pSelfIllumAmount", (ParamType2)TYPE_PCNT_FRAC, &fTemp, time) ||
          CExportNel::getValueByNameUsingParamBlock2(
              mtl, "selfIllumAmount", (ParamType2)TYPE_PCNT_FRAC, &fTemp, time);
    ...
    Point3 maxAmbient;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "cAmbient", (ParamType2)TYPE_RGBA, &maxAmbient, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "ambient", (ParamType2)TYPE_RGBA, &maxAmbient, time);
    ...
    Point3 maxSpecular;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "cSpecular", (ParamType2)TYPE_RGBA, &maxSpecular, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "specular", (ParamType2)TYPE_RGBA, &maxSpecular, time);
    convertColor(nelSpecular, maxSpecular);

    // Get specular level
    float shininess;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "pSpecularLevel", (ParamType2)TYPE_PCNT_FRAC, &shininess, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "specularLevel", (ParamType2)TYPE_PCNT_FRAC, &shininess, time);
    ...
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "pGlossiness", (ParamType2)TYPE_PCNT_FRAC, &shininess, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "glossiness", (ParamType2)TYPE_PCNT_FRAC, &shininess, time);
    ...
    if (iShaderType == SHADER_USER_COLOR) {
      ...
      Point3 maxUserColor;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "cUserColor", (ParamType2)TYPE_RGBA, &maxUserColor, time);
    ...
    int bDoubleSided;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bTwoSided", (ParamType2)TYPE_BOOL, &bDoubleSided, time) ||
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, "twoSided", (ParamType2)TYPE_BOOL, &bDoubleSided, time);
    ...
    CExportNel::getValueByNameUsingParamBlock2(mtl, "bExportTextureMatrix",
                                               (ParamType2)TYPE_BOOL,
                                               &bExportTexMatAnim, 0);
    ...
    for (i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
      ...
      if ((iShaderType == SHADER_USER_COLOR) && (i > 0))
        break;
      if ((iShaderType == SHADER_LIGHTMAP) && (i > 0))
        break;
      if ((iShaderType == SHADER_SPECULAR) && (i > 1))
        break;
      ...
      int bEnableSlot = 0;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, enableSlotName, (ParamType2)TYPE_BOOL, &bEnableSlot, time);
      if (bEnableSlot) {
        ...
        Texmap *pTexmap = NULL;
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, textureName, (ParamType2)TYPE_TEXMAP, &pTexmap, time);
        if (pTexmap) {
          ...
          pTexture =
              buildATexture(*pTexmap, materialDesc, time,
                            (i == 1) && (iShaderType == SHADER_SPECULAR));
          ...
          int bTexGen = 0;
          CExportNel::getValueByNameUsingParamBlock2(
              mtl, texGenName, (ParamType2)TYPE_BOOL, &bTexGen, time);
          ...
          if (materialDesc._IndexInMaxMaterial >= 0) {
            ...
            for (j = 0; j < i; j++) {
              // Same UV channel ?
              if (materialInfo.RemapChannel[j]._IndexInMaxMaterial ==
                  materialDesc._IndexInMaxMaterial)
                break;
            }
    ...
    if (iShaderType == SHADER_LIGHTMAP) // lightmap enabled ?
    {
      ...
      CExportNel::getValueByNameUsingParamBlock2(mtl, "iLightMapChannel",
                                                 (ParamType2)TYPE_INT,
                                                 &iLightMapChannel, time);
      ...
      materialInfo.RemapChannel[size]._IndexInMaxMaterialAlternative =
          materialInfo.RemapChannel[0]._IndexInMaxMaterial;
    }

    // For each slot
    uint stage;
    for (stage = 0;
         stage < std::min((uint)MAT_SLOTS, (uint)IDRV_MAT_MAXTEXTURES);
         stage++) {
      // Make a post fixe
      char postfixC[10];
      ...
      if (iShaderType == SHADER_NORMAL) {
        ...
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbOperation" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGB, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbBlendSource" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBBlend, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg0" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg0, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg1" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg1, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg2" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg2, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg0Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg0Operand, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg1Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg1Operand, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iRgbArg2Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opRGBArg2Operand, time);
        // Setup the value
        if (opRGB < 5)
          ...
          if (opRGB == 6) {
            ...
            material.texEnvOpRGB(stage,
                                 (CMaterial::TTexOperator)(opRGBBlend + 3));
          }
        }
        material.texEnvArg0RGB(stage, (CMaterial::TTexSource)(opRGBArg0 - 1),
                               (CMaterial::TTexOperand)(opRGBArg0Operand - 1));
        if (opRGBArg1 == 4) {
          material.texEnvArg1RGB(
              stage, CMaterial::Texture,
              (CMaterial::TTexOperand)(opRGBArg1Operand - 1));
        } else {
          material.texEnvArg1RGB(
              stage, (CMaterial::TTexSource)(opRGBArg1),
              (CMaterial::TTexOperand)(opRGBArg1Operand - 1));
        }
        if (opRGBArg2 == 4) {
          material.texEnvArg2RGB(
              stage, CMaterial::Texture,
              (CMaterial::TTexOperand)(opRGBArg2Operand - 1));
        } else {
          material.texEnvArg2RGB(
              stage, (CMaterial::TTexSource)(opRGBArg2),
              (CMaterial::TTexOperand)(opRGBArg2Operand - 1));
        }
        ...
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaOperation" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlpha, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaBlendSource" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaBlend, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg0" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg0, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg1" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg1, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg2" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg2, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg0Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg0Operand, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg1Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg1Operand, time);
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iAlphaArg2Operand" + postfix).c_str(), (ParamType2)TYPE_INT,
            &opAlphaArg2Operand, time);
        ...
        if (opAlpha < 5)
          ...
          if (opAlpha == 6) {
            ...
            material.texEnvOpAlpha(stage,
                                   (CMaterial::TTexOperator)(opAlphaBlend + 3));
          }
        }
        material.texEnvArg0Alpha(
            stage, (CMaterial::TTexSource)(opAlphaArg0 - 1),
            (CMaterial::TTexOperand)(opAlphaArg0Operand - 1));
        if (opAlphaArg1 == 4) {
          material.texEnvArg1Alpha(
              stage, CMaterial::Texture,
              (CMaterial::TTexOperand)(opAlphaArg1Operand - 1));
        } else {
          material.texEnvArg1Alpha(
              stage, (CMaterial::TTexSource)(opAlphaArg1),
              (CMaterial::TTexOperand)(opAlphaArg1Operand - 1));
        }
        if (opAlphaArg2 == 4) {
          material.texEnvArg2Alpha(
              stage, CMaterial::Texture,
              (CMaterial::TTexOperand)(opAlphaArg2Operand - 1));
        } else {
          material.texEnvArg2Alpha(
              stage, (CMaterial::TTexSource)(opAlphaArg2),
              (CMaterial::TTexOperand)(opAlphaArg2Operand - 1));
        }
        ...
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("cConstant" + postfix).c_str(), (ParamType2)TYPE_RGBA,
            &constantColor, time);
        ...
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iConstantAlpha" + postfix).c_str(), (ParamType2)TYPE_INT,
            &constantAlpha, time);
        ...
        int texEnvMode = 0;
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, ("iTextureShader" + postfix).c_str(), (ParamType2)TYPE_INT,
            &texEnvMode, time);
        if (texEnvMode > 1) {
          // Enable and setup it
          material.enableTexAddrMode();
          material.setTexAddressingMode(
              stage, (CMaterial::TTexAddressingMode)(texEnvMode - 2));
        }
      }
    }
    ...
    /// TODO: Only one texture for the time. Add multitexture support, and
    /// shaders support.

    // Look for a diffuse texmap
    vector<bool> mapEnables;
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "mapEnables", (ParamType2)TYPE_BOOL_TAB, &mapEnables, time, false);
    ...
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bForceZWrite", (ParamType2)TYPE_BOOL, &bForceZWrite, time, false);
    CExportNel::getValueByNameUsingParamBlock2(mtl, "bForceNoZWrite",
                                               (ParamType2)TYPE_BOOL,
                                               &bForceNoZWrite, time, false);

    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bAlphaTest", (ParamType2)TYPE_BOOL, &bAlphaTest, time, false);

    if (pSpeTexmap != NULL) {
      material.setShader(CMaterial::Specular);
    } else {
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "bLightMap", (ParamType2)TYPE_BOOL, &bLightMap, time, false);
      if (bLightMap) {
    ...
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bStainedGlassWindow", (ParamType2)TYPE_BOOL, &bStainedGlassWindow,
        time, false);
    ...
    if (pDifTexmap) {
      ...
      if (isClassIdCompatible(*pDifTexmap, Class_ID(BMTEX_CLASS_ID, 0)) ||
          isClassIdCompatible(*pDifTexmap,
                              Class_ID(NEL_BITMAP_TEXTURE_CLASS_ID_A,
                                       NEL_BITMAP_TEXTURE_CLASS_ID_B))) {
        ...
        if (pOpaTexmap != NULL) {
          if (bAlphaTest) { // If Alpha Test enabled no blend required just
                            // check if we are forced to NOT write in the
                            // ZBuffer
          ...
          } else { // No Alpha Test so we have to blend and check if we are
                   // forced to write in the ZBuffer
        ...
        if (bLightMap) // lightmap enabled ?
        {
          ...
          materialInfo.RemapChannel[1]._IndexInMaxMaterialAlternative =
              materialInfo.RemapChannel[0]._IndexInMaxMaterial;
    ...
    if (pSpeTexmap) {
      ...
      } else if (isClassIdCompatible(*pSpeTexmap,
                                     Class_ID(COMPOSITE_CLASS_ID, 0))) {
    ...
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "opacityType", (ParamType2)TYPE_INT, &opacityType, time, false);
    ...
    if (isClassIdCompatible(mtl, Class_ID(DMTL_CLASS_ID, 0)) ||
      ...
      CRGBA nelDiffuse;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "diffuse", (ParamType2)TYPE_RGBA, &maxDiffuse, time, false);
      ...
      float fOp = 0.0f;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "opacity", (ParamType2)TYPE_PCNT_FRAC, &fOp, time, false);
      ...
      if (fOp < 0.99f) {
        if (bAlphaTest) { // If Alpha Test enabled no blend required just check
                          // if we are forced to NOT write in the ZBuffer
          material.setAlphaTest(true);
          if (bForceNoZWrite)
            material.setZWrite(false);
          else
            material.setZWrite(true);
        } else { // No Alpha Test so we have to blend and check if we are forced
                 // to write in the ZBuffer
          material.setBlend(true);
          if (bForceZWrite)
            material.setZWrite(true);
          else
      ...
      // else
      //	convertColor (emissiveColor, stdmat->GetDiffuse
      //(time)*stdmat->GetSelfIllum (time));
      int bSelfIllumColorOn;
      Point3 maxSelfIllum;
      float fTemp;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "useSelfIllumColor", (ParamType2)TYPE_BOOL, &bSelfIllumColorOn,
          time, false);
      if (bSelfIllumColorOn) {
        CExportNel::getValueByNameUsingParamBlock2(mtl, "selfIllumColor",
                                                   (ParamType2)TYPE_RGBA,
                                                   &maxSelfIllum, time, false);
      } else {
        CExportNel::getValueByNameUsingParamBlock2(mtl, "selfIllumAmount",
                                                   (ParamType2)TYPE_PCNT_FRAC,
                                                   &fTemp, time, false);
        maxSelfIllum = maxDiffuse * fTemp;
      }
      convertColor(nelEmissive, maxSelfIllum);
      ...
      Point3 maxAmbient;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "ambient", (ParamType2)TYPE_RGBA, &maxAmbient, time, false);
      convertColor(nelAmbient, maxAmbient);

      Point3 maxSpecular;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "specular", (ParamType2)TYPE_RGBA, &maxSpecular, time, false);
      convertColor(nelSpecular, maxSpecular);

      // Specular level
      float shininess; //=stdmat->GetShinStr(time);
      CExportNel::getValueByNameUsingParamBlock2(mtl, "specularLevel",
                                                 (ParamType2)TYPE_PCNT_FRAC,
                                                 &shininess, time, false);
      CRGBAF fColor = nelSpecular;
      fColor *= shininess;
      nelSpecular = fColor;

      // Shininess
      CExportNel::getValueByNameUsingParamBlock2(mtl, "glossiness",
                                                 (ParamType2)TYPE_PCNT_FRAC,
                                                 &shininess, time, false);
      ...
      // Light parameters
      material.setLighting(true, nelEmissive, nelAmbient, nelDiffuse,
                           nelSpecular, shininess);

      // Double sided
      int bDoubleSided;
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "twoSided", (ParamType2)TYPE_BOOL, &bDoubleSided, time, false);
    ...
    if (!bLightMap) {
      ...
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "bUnlighted", (ParamType2)TYPE_BOOL, &bUnlighted, 0, false);
      if (bUnlighted) {
    ...
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bAlphaVertex", (ParamType2)TYPE_BOOL, &bAlphaVertex, time, false);
    ...
    if (bAlphaVertex) {
      ...
      if (bAlphaTest) {
        // If Alpha Test enabled no blend required just check if we are forced
        // to NOT write in the ZBuffer
        material.setAlphaTest(true);
        if (bForceNoZWrite)
          material.setZWrite(false);
        else
      ...
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, "iAlphaVertexChannel", (ParamType2)TYPE_INT,
          &materialInfo.AlphaVertexChannel, time, false);
    }
    ...
    // Get the scripted value
    CExportNel::getValueByNameUsingParamBlock2(
        mtl, "bColorVertex", (ParamType2)TYPE_BOOL, &bColorVertex, time, false);
...
int CExportNel::getVertMapChannel(Texmap &texmap, Matrix3 &channelMatrix,
  ...
  // Set to identity because deafult implementation of texmapGetUVTransform make
  // nothing
...
ITexture *CExportNel::buildATexture(Texmap &texmap,
                                    ...
                                    TimeValue time, bool forceCubic) {
  ...
  if (isClassIdCompatible(texmap, Class_ID(BMTEX_CLASS_ID, 0)) ||
      isClassIdCompatible(texmap, Class_ID(NEL_BITMAP_TEXTURE_CLASS_ID_A,
                                           NEL_BITMAP_TEXTURE_CLASS_ID_B))) {
    // Cast the pointer
    ...
    int bApply;
    bool bRes = getValueByNameUsingParamBlock2(
        texmap, BMTEX_CROP_APPLY, (ParamType2)TYPE_BOOL, &bApply, time);
    nlassert(bRes);

    // If a crop is applyed
    if (bApply) {
      // Get the crop value U
      bRes = getValueByNameUsingParamBlock2(texmap, BMTEX_CROP_U_NAME,
                                            (ParamType2)TYPE_FLOAT,
                                            &(remap3dsTexChannel._CropU), time);
      nlassert(bRes);

      // Get the crop value V
      bRes = getValueByNameUsingParamBlock2(texmap, BMTEX_CROP_V_NAME,
                                            (ParamType2)TYPE_FLOAT,
                                            &(remap3dsTexChannel._CropV), time);
      nlassert(bRes);

      // Get the crop value W
      bRes = getValueByNameUsingParamBlock2(texmap, BMTEX_CROP_W_NAME,
                                            (ParamType2)TYPE_FLOAT,
                                            &(remap3dsTexChannel._CropW), time);
      nlassert(bRes);

      // Get the crop value H
      bRes = getValueByNameUsingParamBlock2(texmap, BMTEX_CROP_H_NAME,
                                            (ParamType2)TYPE_FLOAT,
                                            &(remap3dsTexChannel._CropH), time);
      nlassert(bRes);
    }

    //  1) build a texture that can either be a file texture or a multifile
    //  texture

    ITexture *srcTex;
    const uint numNelTextureSlots = 8;
    static const char *fileNamesTab[] = {"bitmap1FileName", "bitmap2FileName",
                                         "bitmap3FileName", "bitmap4FileName",
                                         "bitmap5FileName", "bitmap6FileName",
                                         "bitmap7FileName", "bitmap8FileName"};

    if (isClassIdCompatible(
            texmap,
            Class_ID(
                NEL_BITMAP_TEXTURE_CLASS_ID_A,
                NEL_BITMAP_TEXTURE_CLASS_ID_B))) // is it a set of textures ?
    {
      uint k;
      ...
      for (k = 0; k < numNelTextureSlots; ++k) // copy each texture of the set
      {
        ...
        bRes = getValueByNameUsingParamBlock2(texmap, fileNamesTab[k],
                                              (ParamType2)TYPE_STRING,
                                              &fileName[k], time);
        nlassert(bRes);
      }
      ...
      if (l == 1 && !fileName[0].empty()) {
        ...
        static_cast<CTextureFile *>(srcTex)->setFileName(
            ConvertTexFileName(fileName[0], _AbsolutePath));
      } else {
        ...
        for (k = 0; k < numUsedSlots; ++k) // copy each texture of the set
        {
          if (!fileName[k].empty()) {
            ...
            std::string convertMultiTex =
                ConvertTexFileName(fileName[k], _AbsolutePath);
            static_cast<CTextureMultiFile *>(srcTex)->setFileName(
                k, convertMultiTex.c_str());
          }
        }
      }
    } else // standard texture
    {
      ...
      static_cast<CTextureFile *>(srcTex)->setFileName(
          ConvertTexFileName(mapName, _AbsolutePath));
    }

    // 2) Use this texture 'as it', or duplicate it to create the faces of a
    // cube map
    ...
    if (forceCubic) {
      ...
      for (uint side = 0; side < 6; side++) {
        ...
        if (dynamic_cast<CTextureMultiFile *>(srcTex)) {
          pTextureCube->setTexture(
              tfNewOrder[side],
              new CTextureMultiFile(*static_cast<CTextureMultiFile *>(srcTex)));
        } else {
          pTextureCube->setTexture(
              tfNewOrder[side],
              new CTextureFile(*static_cast<CTextureFile *>(srcTex)));
        }
      }
  ...
  {
    if (!(texmap.GetTextureTiling() & U_WRAP))
      pTexture->setWrapS(ITexture::Clamp);
    if (!(texmap.GetTextureTiling() & V_WRAP))
      pTexture->setWrapT(ITexture::Clamp);
  }
...
NL3D::CTextureCube *
CExportNel::buildTextureCubeFromReflectRefract(Texmap &texmap, TimeValue time) {
  if (isClassIdCompatible(texmap, Class_ID(ACUBIC_CLASS_ID, 0))) {
    ...
    vector<string> names;
    CExportNel::getValueByNameUsingParamBlock2(
        texmap, "bitmapName", (ParamType2)TYPE_STRING_TAB, &names, time);
...
/// Build a NeL texture cube from a Composite map containing 6 textures (note:
/// no re-ordering is done, because it didn't reorder composite cube in previous
/// version either).
NL3D::CTextureCube *CExportNel::buildTextureCubeFromComposite(Texmap &texmap,
                                                              TimeValue time) {
  if (isClassIdCompatible(texmap, Class_ID(COMPOSITE_CLASS_ID, 0))) {
    ...
    for (uint i = 0; i < nNbSubMap; ++i) {
      ...
      if (pSubMap != NULL &&
          isClassIdCompatible(*pSubMap, Class_ID(BMTEX_CLASS_ID, 0))) {
        CMaterialDesc _3dsTexChannel;
...
NL3D::CTextureCube *CExportNel::buildTextureCubeFromTexture(Texmap &texmap,
                                                            TimeValue time) {
  if (isClassIdCompatible(texmap, Class_ID(BMTEX_CLASS_ID, 0))) {
    // texture cube
    CTextureCube *pTextureCube = new CTextureCube();

...
struct SLMPlane {
  ...
  vector<CRGBAF> col; // 32 bits value for each pixel of each layer.The layers
                      // are contiguous. (0.0f -> 1.0f)
  ...
  void copyColToBitmap8x2(CBitmap *pImage, uint32 nLayerNb, CRGBAF refAmbient,
                          CRGBAF refDiffuse);
  ...
  void createFromFaceGroup(vector<CMesh::CFace *>::iterator ItFace,
                           uint32 nNbFace);

...
void CExportNel::addAnimation(CAnimation &animation, INode &node,
  ...
  // If it is a biped, over Samples now along the animation, and get Keys
  // values.
  ...
  if (transform && (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID)) {
    ...
    int exportNodeMaterial = CExportNel::getScriptAppData(
        &node, NEL3D_APPDATA_EXPORT_ANIMATED_MATERIALS, 0);
    {
  ...
  int exportNoteTrack =
      CExportNel::getScriptAppData(&node, NEL3D_APPDATA_EXPORT_NOTE_TRACK, 0);
  if (exportNoteTrack) {
...
// --------------------------------------------------
void CExportNel::addParticleSystemTracks(CAnimation &animation, INode &node,
  ...
  for (uint k = 0; k < 4; ++k) {
    ...
    if (ctrl) {
      ITrack *pTrack =
          buildATrack(animation, *ctrl, typeFloat, node, desc, NULL);
      if (pTrack) {
        std::string name =
            parentName + std::string(NL3D::CParticleSystemModel::getPSParamName(
                             (uint)NL3D::CParticleSystemModel::PSParam0 + k));
...
void CExportNel::addNodeTracks(CAnimation &animation, INode &node,
                               const char *parentName,
                               CAnimationBuildCtx *animBuildCtx, bool root,
                               CSSSBuild &ssBuilder, bool bodyBiped) {
  // Tmp track*
  ITrack *pTrack;
  ...
  if (transform) {
    ...
    if (c) {
      // Build the track
      pTrack = buildATrack(animation, *c, typeScale, node, desc, animBuildCtx,
                           bodyBiped);
      if (pTrack) {
    ...
    if (c) {
      pTrack = buildATrack(animation, *c, typeRotation, node, desc,
                           animBuildCtx, bodyBiped);
      if (pTrack) {
    ...
    if (c) {
      pTrack = buildATrack(animation, *c, typePos, node, desc, animBuildCtx,
                           bodyBiped);
      if (pTrack) {
        // Choose the good name for this track
      ...
      pTrack = buildATrack(animation, *transform, typePos, node, desc,
                           animBuildCtx, bodyBiped);
      if (pTrack) {
        // Choose the good name for this track
        name = parentName + std::string(ITransformable::getPosValueName());
    ...
    if (isCamera(node, 0)) {
      ...
      if (c) {
        pTrack = buildATrack(animation, *c, typeFloat, node, desc, animBuildCtx,
                             bodyBiped);
        if (pTrack) {
      ...
      if (target) {
        ...
        if (targetC) {
          pTrack = buildATrack(animation, *targetC, typePos, *target, desc,
                               animBuildCtx, bodyBiped);
          if (pTrack) {
            // Choose the good name for this track
  ...
  int exportSSS =
      CExportNel::getScriptAppData(&node, NEL3D_APPDATA_EXPORT_SSS_TRACK, 0);
  if (exportSSS) {
...
void CExportNel::addBipedNodeTracks(CAnimation &animation, INode &node,
                                    const char *parentName,
                                    CAnimationBuildCtx *animBuildCtx, bool root,
                                    CSSSBuild &ssBuilder) {
  ...
  if ((transform->ClassID() == BIPSLAVE_CONTROL_CLASS_ID) ||
    ...
    // Export keyframes
    addNodeTracks(animation, node, name.c_str(), animBuildCtx, root, ssBuilder,
                  bodyBiped);
...
void CExportNel::addBoneTracks(NL3D::CAnimation &animation, INode &node,
                               const char *parentName,
                               CAnimationBuildCtx *animBuildCtx, bool root,
                               CSSSBuild &ssBuilder) {
  // Create a track name
...
void CExportNel::addLightTracks(NL3D::CAnimation &animation, INode &node,
                                const char *parentName) {
  CExportDesc desc;
  ...
  int bAnimated =
      CExportNel::getScriptAppData(&node, NEL3D_APPDATA_LM_ANIMATED, 0);
  if (bAnimated) {
...
void CExportNel::addMorphTracks(NL3D::CAnimation &animation, INode &node,
                                const char *parentName) {
  CExportDesc desc;
...
void CExportNel::addObjTracks(CAnimation &animation, Object &obj,
                              const char *parentName) {
...
void CExportNel::addMtlTracks(CAnimation &animation, Mtl &mtl,
                              const char *parentName) {
  ...
  if (bExportTexMatAnim != 0) {
    for (uint i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
      ...
      CExportNel::getValueByNameUsingParamBlock2(
          mtl, enableSlotName, (ParamType2)TYPE_BOOL, &bEnableSlot, 0);
      if (bEnableSlot) {
        ...
        Texmap *pTexmap = NULL;
        CExportNel::getValueByNameUsingParamBlock2(
            mtl, textureName, (ParamType2)TYPE_TEXMAP, &pTexmap, 0);
        if (pTexmap) {
...
void CExportNel::addTexTracks(CAnimation &animation, Texmap &tex, uint stage,
                              const char *parentName) {
  // Texmap name
  ...
  if (c) {
    ...
    if (pTrack) {
      name =
          parentName + std::string(CAnimatedMaterial::getTexMatWRotName(stage));
      if (animation.getTrackByName(name.c_str())) {
        delete pTrack;
        pTrack = NULL;
      } else {
...
// Build nel keys
void CExportNel::buildNelKey(CKeyFloat &nelKey, ILinFloatKey &maxKey,
...
void CExportNel::buildNelKey(CKeyInt &nelKey, ILinFloatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(CKeyBool &nelKey, ILinFloatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(NL3D::CKeyVector &nelKey, ILinPoint3Key &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(NL3D::CKeyRGBA &nelKey, ILinPoint3Key &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(NL3D::CKeyQuat &nelKey, ILinRotKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(NL3D::CKeyVector &nelKey, ILinScaleKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  // Make a scale matrix
...
void CExportNel::buildNelKey(NL3D::CKeyBezierFloat &nelKey,
                             IBezFloatKey &maxKey, float ticksPerSecond,
...
void CExportNel::buildNelKey(NL3D::CKeyBool &nelKey, IBezFloatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  nelKey.Value = fabs(maxKey.val) <= FLOAT_EPSILON;
}
...
void CExportNel::buildNelKey(NL3D::CKeyBezierVector &nelKey,
                             IBezPoint3Key &maxKey, float ticksPerSecond,
                             const CExportDesc &desc, Control &c) {
  nelKey.Value.x = maxKey.val.x;
  nelKey.Value.y = maxKey.val.y;
  nelKey.Value.z = maxKey.val.z;
...
void CExportNel::buildNelKey(NL3D::CKeyBezierQuat &nelKey, IBezQuatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  nelKey.Value.w = -maxKey.val.w;
  nelKey.Value.x = maxKey.val.x;
...
void CExportNel::buildNelKey(NL3D::CKeyBezierVector &nelKey,
                             IBezScaleKey &maxKey, float ticksPerSecond,
                             const CExportDesc &desc, Control &c) {
  // Make a scale matrix
  Matrix3 srtm, stm, mat;
  maxKey.val.q.MakeMatrix(srtm);
...
void CExportNel::buildNelKey(NL3D::CKeyTCBFloat &nelKey, ITCBFloatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  nelKey.Value = maxKey.val;
...
void CExportNel::buildNelKey(NL3D::CKeyBool &nelKey, ITCBFloatKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  nelKey.Value = fabs(maxKey.val) <= FLOAT_EPSILON;
}
...
void CExportNel::buildNelKey(NL3D::CKeyTCBVector &nelKey, ITCBPoint3Key &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  nelKey.Value.x = maxKey.val.x;
  nelKey.Value.y = maxKey.val.y;
...
void CExportNel::buildNelKey(NL3D::CKeyTCBQuat &nelKey, ITCBRotKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
...
void CExportNel::buildNelKey(NL3D::CKeyTCBVector &nelKey, ITCBScaleKey &maxKey,
                             float ticksPerSecond, const CExportDesc &desc,
                             Control &c) {
  // Make a scale matrix
  Matrix3 srtm, stm, mat;
...
// buildATRack, which require a keyframer interface , which isn't provided by an
// on / off controller)
NL3D::CTrackKeyFramerConstBool *CExportNel::buildOnOffTrack(Control &c) {
  ...
  if ((!(FOREVER == range)) && (!(NEVER == range)))
    track->unlockRange(CExportNel::convertTime(range.Start()),
                       CExportNel::convertTime(range.End()));
...
ITrack *createKeyFramer(IKeyControl *ikeys, TTracker *, TKey *, TMaxKey *,
                        float ticksPerSecond, const Interval &range, int oRT,
                        const CExportDesc &desc, Control &c) {
  ...
  // Set the range
  if ((!(FOREVER == range)) && (!(NEVER == range)))
    pLinTrack->unlockRange(CExportNel::convertTime(range.Start()),
                           CExportNel::convertTime(range.End()));
...
void CExportNel::createBipedKeyFramer(ITrack *&nelRot, ITrack *&nelPos,
                                      bool isRot, bool isPos,
  ...
  /*
      Yoyo:
      The New version of this exporter over sample All frame along the
     animation. A CTrackSampleQuat/CTrackSampleVector will be used late in
     process (at compression time).
  */
  ...
  CAnimationBuildCtx::CBipedNode *bipedNodeInfo =
      animBuildCtx->getBipedNodeInfo(&node);
  ...
  if (numKeys != 0) {
    ...
    if ((!(FOREVER == range)) && (!(NEVER == range))) {
      if (isRot)
        ((TMRot *)nelRot)
            ->unlockRange(CExportNel::convertTime(range.Start()),
                          CExportNel::convertTime(range.End()));
      if (isPos)
        ((TMPos *)nelPos)
            ->unlockRange(CExportNel::convertTime(range.Start()),
...
ITrack *CExportNel::buildATrack(CAnimation &animation, Control &c,
  ...
  if (ikeys) {
    if (ikeys->GetNumKeys() > 0) {
      ...
      if (c.ClassID() == Class_ID(LININTERP_FLOAT_CLASS_ID, 0)) {
        // For float
        if (type == typeFloat)
          pTrack = createKeyFramer<CTrackKeyFramerLinearFloat, CKeyFloat,
                                   ILinFloatKey>(
              ikeys, (CTrackKeyFramerLinearFloat *)NULL, (CKeyFloat *)NULL,
              (ILinFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For int
        if (type == typeInt)
          pTrack =
              createKeyFramer<CTrackKeyFramerLinearInt, CKeyInt, ILinFloatKey>(
                  ikeys, (CTrackKeyFramerLinearInt *)NULL, (CKeyInt *)NULL,
                  (ILinFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For bool
        if (type == typeBoolean)
          pTrack =
              createKeyFramer<CTrackKeyFramerConstBool, CKeyBool, ILinFloatKey>(
                  ikeys, (CTrackKeyFramerConstBool *)NULL, (CKeyBool *)NULL,
                  (ILinFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(LININTERP_POSITION_CLASS_ID, 0)) {
        // For vector
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerLinearVector, CKeyVector,
                                   ILinPoint3Key>(
              ikeys, (CTrackKeyFramerLinearVector *)NULL, (CKeyVector *)NULL,
              (ILinPoint3Key *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For color
        if (type == typeColor)
          pTrack = createKeyFramer<CTrackKeyFramerLinearRGBA, CKeyRGBA,
                                   ILinPoint3Key>(
              ikeys, (CTrackKeyFramerLinearRGBA *)NULL, (CKeyRGBA *)NULL,
              (ILinPoint3Key *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(LININTERP_ROTATION_CLASS_ID, 0)) {
        ...
        if (type == typeRotation)
          pTrack =
              createKeyFramer<CTrackKeyFramerLinearQuat, CKeyQuat, ILinRotKey>(
                  ikeys, (CTrackKeyFramerLinearQuat *)NULL, (CKeyQuat *)NULL,
                  (ILinRotKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(LININTERP_SCALE_CLASS_ID, 0)) {
        ...
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerLinearVector, CKeyVector,
                                   ILinScaleKey>(
              ikeys, (CTrackKeyFramerLinearVector *)NULL, (CKeyVector *)NULL,
              (ILinScaleKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      ...
      if (c.ClassID() == Class_ID(HYBRIDINTERP_FLOAT_CLASS_ID, 0)) {
        // For float
        if (type == typeFloat)
          pTrack = createKeyFramer<CTrackKeyFramerBezierFloat, CKeyBezierFloat,
                                   IBezFloatKey>(
              ikeys, (CTrackKeyFramerBezierFloat *)NULL,
              (CKeyBezierFloat *)NULL, (IBezFloatKey *)NULL, ticksPerSecond,
              range, oRT, desc, c);

        // For int
        if (type == typeInt)
          pTrack = createKeyFramer<CTrackKeyFramerBezierInt, CKeyBezierFloat,
                                   IBezFloatKey>(
              ikeys, (CTrackKeyFramerBezierInt *)NULL, (CKeyBezierFloat *)NULL,
              (IBezFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For bool
        if (type == typeBoolean)
          pTrack =
              createKeyFramer<CTrackKeyFramerConstBool, CKeyBool, IBezFloatKey>(
                  ikeys, (CTrackKeyFramerConstBool *)NULL, (CKeyBool *)NULL,
                  (IBezFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if ((c.ClassID() == Class_ID(HYBRIDINTERP_POSITION_CLASS_ID, 0)) ||
        ...
        // For vector
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerBezierVector,
                                   CKeyBezierVector, IBezPoint3Key>(
              ikeys, (CTrackKeyFramerBezierVector *)NULL,
              (CKeyBezierVector *)NULL, (IBezPoint3Key *)NULL, ticksPerSecond,
              range, oRT, desc, c);

        // For color
        if (type == typeColor)
          pTrack = createKeyFramer<CTrackKeyFramerBezierRGBA, CKeyBezierVector,
                                   IBezPoint3Key>(
              ikeys, (CTrackKeyFramerBezierRGBA *)NULL,
              (CKeyBezierVector *)NULL, (IBezPoint3Key *)NULL, ticksPerSecond,
              range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(HYBRIDINTERP_ROTATION_CLASS_ID, 0)) {
        // For quaternion
        if (type == typeRotation)
          pTrack = createKeyFramer<CTrackKeyFramerBezierQuat, CKeyBezierQuat,
                                   IBezQuatKey>(
              ikeys, (CTrackKeyFramerBezierQuat *)NULL, (CKeyBezierQuat *)NULL,
              (IBezQuatKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(HYBRIDINTERP_SCALE_CLASS_ID, 0)) {
        // For scale
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerBezierVector,
                                   CKeyBezierVector, IBezScaleKey>(
              ikeys, (CTrackKeyFramerBezierVector *)NULL,
              (CKeyBezierVector *)NULL, (IBezScaleKey *)NULL, ticksPerSecond,
              range, oRT, desc, c);
      }
      ...
      if (c.ClassID() == Class_ID(TCBINTERP_FLOAT_CLASS_ID, 0)) {
        // For float
        if (type == typeFloat)
          pTrack = createKeyFramer<CTrackKeyFramerTCBFloat, CKeyTCBFloat,
                                   ITCBFloatKey>(
              ikeys, (CTrackKeyFramerTCBFloat *)NULL, (CKeyTCBFloat *)NULL,
              (ITCBFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For int
        if (type == typeInt)
          pTrack = createKeyFramer<CTrackKeyFramerTCBInt, CKeyTCBFloat,
                                   ITCBFloatKey>(
              ikeys, (CTrackKeyFramerTCBInt *)NULL, (CKeyTCBFloat *)NULL,
              (ITCBFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For bool
        if (type == typeBoolean)
          pTrack =
              createKeyFramer<CTrackKeyFramerConstBool, CKeyBool, ITCBFloatKey>(
                  ikeys, (CTrackKeyFramerConstBool *)NULL, (CKeyBool *)NULL,
                  (ITCBFloatKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if ((c.ClassID() == Class_ID(TCBINTERP_POSITION_CLASS_ID, 0)) ||
        ...
        // For vector
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerTCBVector, CKeyTCBVector,
                                   ITCBPoint3Key>(
              ikeys, (CTrackKeyFramerTCBVector *)NULL, (CKeyTCBVector *)NULL,
              (ITCBPoint3Key *)NULL, ticksPerSecond, range, oRT, desc, c);

        // For color
        if (type == typeColor)
          pTrack = createKeyFramer<CTrackKeyFramerTCBRGBA, CKeyTCBVector,
                                   ITCBPoint3Key>(
              ikeys, (CTrackKeyFramerTCBRGBA *)NULL, (CKeyTCBVector *)NULL,
              (ITCBPoint3Key *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(TCBINTERP_ROTATION_CLASS_ID, 0)) {
        ...
        if (type == typeRotation)
          pTrack =
              createKeyFramer<CTrackKeyFramerTCBQuat, CKeyTCBQuat, ITCBRotKey>(
                  ikeys, (CTrackKeyFramerTCBQuat *)NULL, (CKeyTCBQuat *)NULL,
                  (ITCBRotKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
      if (c.ClassID() == Class_ID(TCBINTERP_SCALE_CLASS_ID, 0)) {
        ...
        if ((type == typePos) || (type == typeScale))
          pTrack = createKeyFramer<CTrackKeyFramerTCBVector, CKeyTCBVector,
                                   ITCBScaleKey>(
              ikeys, (CTrackKeyFramerTCBVector *)NULL, (CKeyTCBVector *)NULL,
              (ITCBScaleKey *)NULL, ticksPerSecond, range, oRT, desc, c);
      }
    }
  }

  // No keyframer controler
  if ((c.ClassID() == BIPSLAVE_CONTROL_CLASS_ID) || bodyBiped) {
    NL3D::ITrack *doomy;

    // For rotation
    if (type == typeRotation) {
      if (animBuildCtx) {
        createBipedKeyFramer(pTrack, doomy, true, false, ticksPerSecond, range,
                             oRT, desc, *(INode *)&node, animBuildCtx);
      }
    }
  }
  ...
  if (bodyBiped ||
      (animBuildCtx && animBuildCtx->mustExportBipedBonePos((INode *)&node))) {
    NL3D::ITrack *doomy;
    ...
    if (type == typePos) {
      ...
      // Create key set
      if (animBuildCtx) {
        createBipedKeyFramer(doomy, pTrack, false, true, ticksPerSecond, range,
                             oRT, desc, *(INode *)&node, animBuildCtx);
      } else {
        MessageBox(NULL, _T("Warning: no pos track exported!"), _T("Tmp NEL"),
...
bool CExportNel::getBipedKeyInfo(const char *nodeName, const char *paramName,
                                 uint key, float &res) {
  // Make the script sequence
  char script[512];
  _snprintf(script, 512, "(biped.getKey $'%s'.controller %d).%s", nodeName,
            key + 1, paramName);
...
bool CExportNel::getBipedInplaceMode(const char *nodeName,
                                     const char *inplaceFunction, bool &res) {
  // Make the script sequence
  char script[512];
...
bool CExportNel::setBipedInplaceMode(const char *nodeName,
                                     const char *inplaceFunction, bool onOff) {
  // Make the script sequence
  char script[512];
  _snprintf(script, 512, "$'%s'.controller.%s = %s", nodeName, inplaceFunction,
            onOff ? "true" : "false");
...
// ***************************************************************************
void CExportNel::buildBipedInformation(CAnimationBuildCtx &animBuildCtx,
                                       INode &node) {
  ...
  // Is it a biped node ?
  if (transform && (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID ||
                    transform->ClassID() == BIPSLAVE_CONTROL_CLASS_ID)) {
    // Yes, Add the node
    //===============
    ...
    if (ikeys && ikeys->GetNumKeys() > 0) {
      ...
      // Get the first key
      ikeys->GetKey(0, &key);
      animBuildCtx.BipedRangeMin =
          std::min(animBuildCtx.BipedRangeMin, key.time);

      // Get the last key
      ikeys->GetKey(ikeys->GetNumKeys() - 1, &key);
      animBuildCtx.BipedRangeMax =
          std::max(animBuildCtx.BipedRangeMax, key.time);
    }
  }
...
// ***************************************************************************
void CExportNel::overSampleBipedAnimation(CAnimationBuildCtx &animBuildCtx,
  ...
  for (i = 0; i < animBuildCtx.BipedNodes.size(); i++) {
    ...
    if (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID) {
      ...
      bool res = getBipedInplaceMode(getName(*node).c_str(), "inPlaceYMode",
                                     pm.InPlaceYMode);
      nlassert(res);
      res = getBipedInplaceMode(getName(*node).c_str(), "inPlaceXMode",
                                pm.InPlaceXMode);
      nlassert(res);
      res = getBipedInplaceMode(getName(*node).c_str(), "inPlaceMode",
                                pm.InPlaceMode);
      nlassert(res);
    ...
    IBipedExport *BipIface =
        (IBipedExport *)transform->GetInterface(I_BIPINTERFACE);
    nlassert(BipIface);
  ...
  int interpolationStep =
      (overSampleValue * GetTicksPerFrame()) / GetFrameRate();

  // get the number of keys to create. (ceil() and +1)
  uint numKeys = 1 + (interpolationStep - 1 + animBuildCtx.BipedRangeMax -
                      animBuildCtx.BipedRangeMin) /
                         interpolationStep;
  ...
  for (i = 0; i < animBuildCtx.BipedNodes.size(); i++) {
    bpExportPos[i] =
        animBuildCtx.mustExportBipedBonePos(animBuildCtx.BipedNodes[i].Node);
  }

  // For all time Values usefull for the Biped.
  TimeValue time;
  for (time = animBuildCtx.BipedRangeMin; time <= animBuildCtx.BipedRangeMax;
       time += interpolationStep) {
    time = std::min(animBuildCtx.BipedRangeMax, time);

    /* Do this in this way, so the Biped will be evaluated in Max only once per
     * frame.
     */
    ...
    for (i = 0; i < animBuildCtx.BipedNodes.size(); i++) {
      ...
      if (!bpExportPos[i]) {
        // can use it because RotQuat is the same with this method or with
        // getNELBoneLocalTM()
        Matrix3 localTM(TRUE);
        getLocalMatrix(localTM, *bpNode.Node, time);
        // NB: with this method the NEL local pos and local scale are false, but
        // doesn't matter since not used...
        decompMatrix(bpKey.Scale, bpKey.Quat, bpKey.Pos, localTM);
        bpKey.Quat.normalize();
      } else {
        // Get the NEL local TM, with correct position
        getNELBoneLocalTM(*bpNode.Node, time, bpKey.Scale, bpKey.Quat,
                          bpKey.Pos);
      }
  ...
  for (i = 0; i < nodeToResetPlaceMode.size(); i++) {
    ...
    setBipedInplaceMode(getName(*node).c_str(), "inPlaceYMode",
                        pm.InPlaceYMode);
    setBipedInplaceMode(getName(*node).c_str(), "inPlaceXMode",
                        pm.InPlaceXMode);
  ...
  //=========================
  // NB: 	for all node, you must _ExportBipedBonePosSet.insert(node) in
  // compileBiped() to test it correctly
  /*for(i=0; i<animBuildCtx.BipedNodes.size(); i++)
  {
      CAnimationBuildCtx::CBipedNode		&bpNode=
  animBuildCtx.BipedNodes[i]; if(bpNode.Keys.size()==0) return;
...
void CExportNel::CAnimationBuildCtx::compileBiped() {
  ...
  for (i = 0; i < BipedNodes.size(); i++) {
    ...
    if (transform->ClassID() == BIPBODY_CONTROL_CLASS_ID) {
      ...
      // Must add some important limbs of this biped too (Biped stuff =>
      // sometimes pos are animated).
...
// ***************************************************************************
void CExportNel::CAnimationBuildCtx::addLimbNodeToExportPos(
    INode *rootNode, const char *limbId) {
  char script[512];
  INode *limbNode = NULL;

  // evaluate script.
  _snprintf(script, 512, "biped.getNode $'%s' %s",
            CExportNel::getName(*rootNode).c_str(), limbId);
  // If script is ok.
...
// ***************************************************************************
CExportNel::CAnimationBuildCtx::CBipedNode *
CExportNel::CAnimationBuildCtx::getBipedNodeInfo(INode *node) {
...
static void commitSSSKey(NL3D::TAnimationTime keyTime,
                         std::map<CSpawnCmd, CSpawnObject> &objects,
                         CTrackKeyFramerConstString *finalTrack,
                         bool &insertedAt0) {
  CKeyString keyValue;
  ...
  for (it = objects.begin(); it != objects.end(); it++) {
    ...
    if (it->second.Spawned) {
      if (it->first.Cmd == "lspawn") {
        keyValue.Value +=
            toString("objl %d %s %s\n", it->second.FinalId,
                     it->first.Shape.c_str(), it->first.Bone.c_str());
      } else if (it->first.Cmd == "wspawn") {
        keyValue.Value += toString("objw %d %s\n", it->second.FinalId,
                                   it->first.Shape.c_str());
      }
    }
  }
...
void CSSSBuild::compile(NL3D::CAnimation &dest, const char *sBaseName) {
  ...
  /*	Take each script at each bone, and generate only ONE script (the global
     script), that will be attached to the animation
  */
  ...
  for (i = 0; i < Bones.size(); i++) {
    ...
    for (uint j = 0; j < bone.Track.size(); j++) {
      ...
      for (uint k = 0; k < lines.size(); k++) {
        ...
        // remove any '\r'
#ifndef NL_CPP17
        line.resize(std::remove_if(line.begin(), line.end(),
        ...
        if (words.size() >= 2) {
          ...
          if (words[0] == "wspawn")
            ok = true;
          else if (words[0] == "lspawn")
            ok = true;
          else if (words[0] == "wkill")
            ok = true;
          else if (words[0] == "lkill")
            ok = true;
  ...
  for (kit = keys.begin(); kit != keys.end(); kit++) {
    ...
    if (keyValue.Cmd == "wspawn" || keyValue.Cmd == "lspawn")
      ...
      if (keyValue.Cmd == "wkill")
        keyValue.Cmd = "wspawn";
      if (keyValue.Cmd == "lkill")
        keyValue.Cmd = "lspawn";
    }
  ...
  if (finalTrack) {
    // ---- add the track
    std::string name = std::string(sBaseName) +
                       NL3D::CSkeletonModel::getSpawnScriptValueName();

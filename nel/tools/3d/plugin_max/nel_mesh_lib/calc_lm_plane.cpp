...
// -----------------------------------------------------------------------------------------------
void SLMPlane::copyColToBitmap8x2(CBitmap *pImage, uint32 nLayerNb, CRGBAF lA,
                                  CRGBAF lD) {
  if (nLayerNb >= nNbLayerUsed)
    return;

  if ((pImage->getWidth() != w) || (pImage->getHeight() != h)) {
    pImage->resize(w, h);
  }

  /*
      We want to compress the RGBA lightmap L32 into a (lA,lD,L8) lightmap,
     where lA and lD are constants.

      lA and lD are given, so we have to compute L8.

      In the standard 32 bits Lightmap shader, for a single lightmap, the final
     formula is: O= L32*F*T where: O:		output L32:	the lightmap
     (actually in 16 bits RGB) F:		the lightmap factor (for light
     animation or activation) T:		the diffuse texture

      In the "8bits *2" Lightmap shader, for a single lightmap, the final
     formula is: O= (mA+L8*mD)*F*T*2 where: L8:		the 8 bits lightmap mA:
     Material Lightmap Ambient term. equal to lA/2, for convenience, and avoid
     too much "ambient clamping" mD:		Material Lightmap Diffuse term.
     equal to lD. *2:		the pixel shader multiplyBy2, to allow some
     TextureDiffuse burn.
...
void SLMPlane::copyColToBitmap32(CBitmap *pImage, uint32 nLayerNb) {
  ...
  for (uint32 i = 0; i < w * h; ++i) {
    // if we are in multiply x2 we have to set the following value to 127.0 else
    // set to 255.0 By default we are in multiply x2 in Lightmap 8 bits mode but
    // not in 32 bits mode
  ...
  for (b = 0; b < this->h; ++b)
    for (a = 0; a < this->w; ++a)
      if (this->msk[a + b * this->w] != 0) {
        Dst.msk[(this->x + a) + (this->y + b) * Dst.w] =
            this->msk[a + b * this->w];
        if (bMaskOnly == false)
          for (c = 0; c < this->nNbLayerUsed; ++c)
            Dst.col[(this->x + a) + (this->y + b) * Dst.w + Dst.w * Dst.h * c] =
                this->col[a + b * this->w + w * h * c];
      }
}
...
void SLMPlane::resize(uint32 nNewSizeX, uint32 nNewSizeY) {
  ...
  for (k = 0; k < nNbLayerUsed; ++k)
    for (j = 0; j < min(this->h, nNewSizeY); ++j)
      for (i = 0; i < min(this->w, nNewSizeX); ++i)
        vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k] =
            this->col[i + j * this->w + w * h * k];
...
// halves its size Take care the decalage is affected by the scaling factor
// (like homothetie)
  ...
  if (osFactor > 1.0) // Enlarge the image
  {
    ...
    for (j = 0; j < nNewSizeY; ++j) {
      ...
      for (i = 0; i < nNewSizeX; ++i) {
        ...
        for (k = 0; k < nNbLayerUsed; ++k) {
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].R +=
              col[((sint32)x) + ((sint32)y) * w + w * h * k].R;
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].G +=
              col[((sint32)x) + ((sint32)y) * w + w * h * k].G;
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].B +=
              col[((sint32)x) + ((sint32)y) * w + w * h * k].B;
  ...
  } else // Reduce image
  {
    ...
    for (j = 0; j < this->h; ++j) {
      ...
      for (i = 0; i < this->w; ++i) {
        ...
        for (k = 0; k < nNbLayerUsed; ++k) {
          vImgTemp2[((sint32)x) + ((sint32)y) * nNewSizeX +
                    nNewSizeX * nNewSizeY * k]
              .R += col[i + j * w + w * h * k].R;
          vImgTemp2[((sint32)x) + ((sint32)y) * nNewSizeX +
                    nNewSizeX * nNewSizeY * k]
              .G += col[i + j * w + w * h * k].G;
    ...
    for (j = 0; j < nNewSizeY; ++j)
      for (i = 0; i < nNewSizeX; ++i)
        if (vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].A > 1.0f) {
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].R /=
              vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].A;
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].G /=
              vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].A;
          vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].B /=
              vImgTemp2[i + j * nNewSizeX + nNewSizeX * nNewSizeY * k].A;
...
// -----------------------------------------------------------------------------------------------
bool SLMPlane::isInTriangleOrEdge(double x, double y, double xt1, double yt1,
                                  double xt2, double yt2, double xt3,
                                  double yt3) {
...
// -----------------------------------------------------------------------------------------------
bool SLMPlane::segmentIntersection(double x1, double y1, double x2, double y2,
  ...
  for (j = decalY; j < (decalY + sizeY - 1); ++j)
    for (k = decalX; k < (decalX + sizeX - 1); ++k) {
      ...
      if (isInTriangleOrEdge(k + 0.5, j + 0.5, lumx1, lumy1, lumx2, lumy2,
        ...
        msk[k + (1 + j) * w] = 1;
        msk[1 + k + (1 + j) * w] = 1;
      } else if (segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx1,
                                     lumy1, lumx2, lumy2) ||
                 segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx2,
                                     lumy2, lumx3, lumy3) ||
                 segmentIntersection(k + 0.5, j + 0.5, k + 1.5, j + 0.5, lumx3,
                                     lumy3, lumx1, lumy1) ||

                 segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx1,
                                     lumy1, lumx2, lumy2) ||
                 segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx2,
                                     lumy2, lumx3, lumy3) ||
                 segmentIntersection(k + 0.5, j + 0.5, k + 0.5, j + 1.5, lumx3,
                                     lumy3, lumx1, lumy1) ||

                 segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx1,
                                     lumy1, lumx2, lumy2) ||
                 segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx2,
                                     lumy2, lumx3, lumy3) ||
                 segmentIntersection(k + 1.5, j + 1.5, k + 1.5, j + 0.5, lumx3,
                                     lumy3, lumx1, lumy1) ||

                 segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx1,
                                     lumy1, lumx2, lumy2) ||
                 segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx2,
                                     lumy2, lumx3, lumy3) ||
                 segmentIntersection(k + 1.5, j + 1.5, k + 0.5, j + 1.5, lumx3,
                                     lumy3, lumx1, lumy1)) {
        msk[k + j * w] = 1;
        msk[1 + k + j * w] = 1;
        msk[k + (1 + j) * w] = 1;
...
// -----------------------------------------------------------------------------------------------
void SLMPlane::createFromFaceGroup(vector<CMesh::CFace *>::iterator ItFace,
                                   uint32 nNbFace) {
  uint32 i, j;

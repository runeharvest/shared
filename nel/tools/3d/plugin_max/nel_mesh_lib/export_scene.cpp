...
CInstanceGroup *
CExportNel::buildInstanceGroup(const vector<INode *> &vectNode,
    ...
    if ((nAccelType & 3) == 0) // If not an accelerator
      if (!RPO::isZone(*pNode, tvTime))
        if (CExportNel::isMesh(*pNode, tvTime) ||
            CExportNel::isDummy(*pNode, tvTime)) {
          ++nNbInstance;
        }
  }
  ...
  for (i = 0; i < (sint)vectNode.size(); ++i, ++it) {
    ...
    if ((nAccelType & 3) == 0) // If not an accelerator
      if (!RPO::isZone(*pNode, tvTime))
        if (CExportNel::isMesh(*pNode, tvTime) ||
            CExportNel::isDummy(*pNode, tvTime)) {
          aIGArray[nNumIG].DontAddToScene =
              CExportNel::getScriptAppData(pNode,
                                           NEL3D_APPDATA_DONT_ADD_TO_SCENE, 0)
                  ? true
                  : false;
          aIGArray[nNumIG].InstanceName = CExportNel::getScriptAppData(
              pNode, NEL3D_APPDATA_INSTANCE_NAME, "");
          resultInstanceNode[nNumIG] = pNode;
          if (aIGArray[nNumIG].InstanceName ==
              "") // no instance name was set, takes the node name instead
          {
          ...
          sint appDataCameraCol = CExportNel::getScriptAppData(
              pNode, NEL3D_APPDATA_CAMERA_COLLISION_MESH_GENERATION, 0);
          ...
          aIGArray[nNumIG].DontCastShadowForInterior =
              CExportNel::getScriptAppData(
                  pNode, NEL3D_APPDATA_LIGHT_DONT_CAST_SHADOW_INTERIOR,
                  BST_UNCHECKED)
                  ? true
                  : false;
          aIGArray[nNumIG].DontCastShadowForExterior =
              CExportNel::getScriptAppData(
          ...
          if (pParent->IsRootNode() == 0) {
            ...
            for (j = 0; j < vectNode.size(); ++j) {
              ...
              int nAccelType2 =
                  CExportNel::getScriptAppData(pNode2, NEL3D_APPDATA_ACCEL, 32);
  ...
  */
  ...
  for (i = 0; i < (sint)vectNode.size(); ++i, ++it) {
    ...
    int nAccelType = CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_ACCEL,
                                                  NEL3D_APPDATA_ACCEL_DEFAULT);
    bool bFatherVisible =
        nAccelType & NEL3D_APPDATA_ACCEL_FATHER_VISIBLE ? true : false;
    bool bVisibleFromFather =
        nAccelType & NEL3D_APPDATA_ACCEL_VISIBLE_FROM_FATHER ? true : false;
    bool bAudibleLikeVisible =
        (nAccelType & NEL3D_APPDATA_ACCEL_AUDIBLE_NOT_LIKE_VISIBLE) ? false
                                                                    : true;
    bool bFatherAudible = bAudibleLikeVisible ? bFatherVisible
                          : nAccelType & NEL3D_APPDATA_ACCEL_FATHER_AUDIBLE
                              ? true
                              : false;
    bool bAudibleFromFather =
        bAudibleLikeVisible ? bVisibleFromFather
        : nAccelType & NEL3D_APPDATA_ACCEL_AUDIBLE_FROM_FATHER ? true
                                                               : false;

    if ((nAccelType & NEL3D_APPDATA_ACCEL_TYPE) ==
        NEL3D_APPDATA_ACCEL_CLUSTER) // If cluster
      if (!RPO::isZone(*pNode, tvTime))
        if (CExportNel::isMesh(*pNode, tvTime)) {
          CCluster clusterTemp;
          std::string temp;

          temp = CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_SOUND_GROUP,
                                              "no sound");
          clusterTemp.setSoundGroup(temp != "no sound" ? temp : "");
          temp = CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_ENV_FX,
                                              "no fx");
          ...
          for (j = 0; j < pMB->Faces.size(); ++j) {
            if (!clusterTemp.makeVolume(
                    pMB->Vertices[pMB->Faces[j].Corner[0].Vertex],
                    pMB->Vertices[pMB->Faces[j].Corner[1].Vertex],
                    pMB->Vertices[pMB->Faces[j].Corner[2].Vertex])) {
              // ERROR : The volume is not convex !!!
              nlwarning("ERROR: The cluster %s is not convex.",
                        MCharStrToUtf8(vectNode[i]->GetName()).c_str());
            }
          }
    ...
    if ((nAccelType & 3) == 1) // If Portal
      if (!RPO::isZone(*pNode, tvTime))
        if (CExportNel::isMesh(*pNode, tvTime)) {
          ...
          temp = CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_OCC_MODEL,
                                              "no occlusion");
          portalTemp.setOcclusionModel(temp != "no occlusion" ? temp : "");
          temp = CExportNel::getScriptAppData(
              ...
              for (k = 0; k < 3; ++k) {
                for (m = 0; m < poly.size(); ++m) {
                  if ((pMB->Faces[j].Corner[k].Vertex == poly[m]) &&
                      (pMB->Faces[j].Corner[(k + 1) % 3].Vertex ==
                       poly[(m + 1) % poly.size()])) {
                    found = true;
                    break;
                  }
                  if ((pMB->Faces[j].Corner[(k + 1) % 3].Vertex == poly[m]) &&
                      (pMB->Faces[j].Corner[k].Vertex ==
                       poly[(m + 1) % poly.size()])) {
                    found = true;
                    break;
                  }
          ...
          if (!portalTemp.setPoly(polyv)) {
            ...
            nlwarning("ERROR: The portal %s is not convex.",
                      MCharStrToUtf8(vectNode[i]->GetName()).c_str());
          }

          if (nAccelType & 16) // is dynamic portal ?
          {
            string InstanceName = CExportNel::getScriptAppData(
                pNode, NEL3D_APPDATA_INSTANCE_NAME, "");
          ...
          if (nNbCluster != 2) {
            // ERROR
            nlwarning("ERROR: The portal %s has not 2 clusters but %d",
                      MCharStrToUtf8(vectNode[i]->GetName()).c_str(),
                      nNbCluster);
          }
        ...
        if (CExportNel::isMesh(*pNode, tvTime) ||
          ...
          if (nAccelType & 32) // Is the flag clusterize set ?
          {
            ...
            std::vector<NLMISC::CVector>
                FXVertices; // Used only if the obj is a fx. It contains the
                            // corners of the bbox.
            ...
            if (obj) {
              ...
              if (clid.PartA() == NEL_PARTICLE_SYSTEM_CLASS_ID) {
                ...
                if (!CExportNel::getValueByNameUsingParamBlock2(
                        *pNode, "ps_file_name", (ParamType2)TYPE_STRING,
                        &psFilePath, 0)) {
                ...
                if (!psFilePath.empty()) {
                  ...
                  if (iF.open(psFilePath.c_str())) {
                    try {
                      iF.serial(ss);
                      NL3D::CParticleSystemShape *pss =
                          dynamic_cast<NL3D::CParticleSystemShape *>(
                              ss.getShapePointer());
                      if (!pss) {
                        nlwarning("ERROR: Node %s shape is not a FX",
                                  CExportNel::getName(*pNode).c_str());
                      } else {
                        NLMISC::CAABBox bbox;
                        pss->getAABBox(bbox);
                        // transform in world
                        ...
                        for (uint k = 0; k < 8; ++k) {
                          FXVertices.push_back(CVector(
                              ((k & 1) ? 1 : -1) * bbox.getHalfSize().x +
                                  bbox.getCenter().x,
                              ((k & 2) ? 1 : -1) * bbox.getHalfSize().y +
                                  bbox.getCenter().y,
                              ((k & 4) ? 1 : -1) * bbox.getHalfSize().z +
                                  bbox.getCenter().z));
                        }
                        //
                  ...
                  if (buildMeshBBox) {
                    nlwarning("ERROR: Can't get bbox of a particle system from "
                              "its shape, using helper bbox instead");
                  }
                }
              }
            }
            ...
            if (!vClusters.empty())
              if (aIGArray[nNumIG].Clusters.empty()) {
                nlwarning("ERROR: Object %s is not attached to any "
                          "cluster\nbut his flag clusterize is set",
  ...
  for (i = 0; i < (sint)vectNode.size(); ++i) {
    ...
    if (sLightBuild.canConvertFromMaxLight(pNode, tvTime)) {
      ...
      int nRTExport = CExportNel::getScriptAppData(
          pNode, NEL3D_APPDATA_EXPORT_REALTIME_LIGHT, BST_CHECKED);
      if (nRTExport == BST_CHECKED) {
        ...
        if (sLightBuild.Type != SLightBuild::LightDir) {
          ...
          // Attenuation
          plNamed.setupAttenuation(sLightBuild.rRadiusMin,
                                   sLightBuild.rRadiusMax);
          // Colors
          ...
          // Which light type??
          if (sLightBuild.bAmbientOnly ||
              sLightBuild.Type == SLightBuild::LightAmbient) {
            plNamed.setType(CPointLight::AmbientLight);
            // Special ambient info
            int nRTAmbAdd = CExportNel::getScriptAppData(
                pNode, NEL3D_APPDATA_REALTIME_AMBIENT_ADD_SUN, BST_UNCHECKED);
      ...
      int nExportSun = CExportNel::getScriptAppData(
          pNode, NEL3D_APPDATA_EXPORT_AS_SUN_LIGHT, BST_UNCHECKED);
...
void CExportNel::buildScene(NL3D::CScene &scene, NL3D::CShapeBank &shapeBank,
                            IDriver &driver, TimeValue tvTime,
                            NL3D::CLandscape *landscape, IProgress *progress,
                            bool buildHidden, bool onlySelected,
                            bool buildLods) {
  // Register classes
  ...
  for (nNode = 0; nNode < nNumNode; nNode++) {
    ...
    if ((!pNode->IsHidden() || buildHidden) &&
        (pNode->Selected() || !onlySelected)) {
      ...
      if (RPO::isZone(*pNode, tvTime) && landscape) {
        ...
        if (os.obj) {
          ...
          RPO *tri = (RPO *)os.obj->ConvertToType(_Ip->GetTime(),
                                                  RYKOLPATCHOBJ_CLASS_ID);
          if (tri) {
            CZone zone;
            CZoneSymmetrisation sym;
            static int zoneId = 0;
            if (tri->rpatch->exportZone(pNode, &tri->patch, zone, sym, zoneId++,
                                        160, 1, false))
      ...
      // Try to export a mesh
        ...
        if (!skined) {
          ...
          if ((CExportNel::getScriptAppData(pNode, NEL3D_APPDATA_ACCEL, 32) &
               3) == 0) {
            // Export the shape
            IShape *pShape = NULL;
            ...
            // Export successful ?
            if (pShape) {
              // get the nelObjectName, as used in building of the
              // instanceGroup.
              std::string nelObjectName = CExportNel::getNelObjectName(*pNode);

              // ugly way to verify the shape is really added to the sahepBank:
              // use a refPtr :)
              ...
              if (!(nelObjectName.find(".shape") != std::string::npos ||
                    nelObjectName.find(".ps") != std::string::npos)) {
              ...
              shapeBank.add(nelObjectNameLwr.c_str(),
                            CSmartPtr<IShape>(pShape));
              scene.createInstance(nelObjectNameLwr.c_str());

              // If the shape is not destroyed in addMesh() (with smarPtr), then
              // add it to the shape map.
              if (prefShape) {
  ...
  NL3D::CInstanceGroup *ig =
      buildInstanceGroup(igVectNode, resultInstanceNode, tvTime);
  if (ig) {
    // If ExportLighting
    if (_Options.bExportLighting) {
      // Light the ig.
      ...
      lightDesc.OverSampling =
          NLMISC::raiseToNextPowerOf2(_Options.nOverSampling);
  ...
  if (_Options.bExportLighting) {
    ...
    // setup lighting and sun, if any light added. Else use std OpenGL front
    // lighting

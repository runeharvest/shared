...
bool CExportNel::scriptEvaluate(const char *script, void *out,
                                TNelScriptValueType type) {
  BOOL result = TRUE;
  init_thread_locals();

  four_typed_value_locals(Parser * parser, Value * code, Value * result,
                          StringStream * source);
...
void CExportNel::setScriptAppData(Animatable *node, uint32 id, int value) {
  ...
  // Add data
  node->AddAppDataChunk(MAXSCRIPT_UTILITY_CLASS_ID, UTILITY_CLASS_ID, id,
                        block.size() + 1, copy);
}
...
std::string CExportNel::getScriptAppData(Animatable *node, uint32 id,
...
void CExportNel::setScriptAppData(Animatable *node, uint32 id,
...
NLMISC::CRGBA CExportNel::getScriptAppData(Animatable *node, uint32 id,
                                           NLMISC::CRGBA def) {
  // Get the chunk
  AppDataChunk *ap =
      node->GetAppDataChunk(MAXSCRIPT_UTILITY_CLASS_ID, UTILITY_CLASS_ID, id);
...
void CExportNel::setScriptAppData(Animatable *node, uint32 id,
  ...
  // RGBA to string
  std::string value =
      NLMISC::toString("%d %d %d %d", val.R, val.G, val.B, val.A);
...
// ***************************************************************************
void CExportNel::getScriptAppDataVPWT(Animatable *node,
  ...
  apd.SpecularLighting =
      getScriptAppData(node, NEL3D_APPDATA_VPWT_USE_SPEC, BST_UNCHECKED);
  ...
  for (uint i = 0; i < CVPWindTreeAppData::HrcDepth; i++) {
    ...
    // get appData.
    apd.Frequency[i] =
        getScriptAppData(node, NEL3D_AppDataVPWTFreq[i], defFreq);
    apd.FrequencyWindFactor[i] =
        getScriptAppData(node, NEL3D_AppDataVPWTFreqWD[i], 0);
    apd.DistXY[i] =
        getScriptAppData(node, NEL3D_AppDataVPWTDistXY[i], defDistXY);
    apd.DistZ[i] = getScriptAppData(node, NEL3D_AppDataVPWTDistZ[i], 0);
    apd.Bias[i] = getScriptAppData(node, NEL3D_AppDataVPWTBias[i],
                                   CVPWindTreeAppData::NumTicks / 2);
  }
}

// ***************************************************************************
void CExportNel::setScriptAppDataVPWT(Animatable *node,
  ...
  for (uint i = 0; i < CVPWindTreeAppData::HrcDepth; i++) {
    ...
    setScriptAppData(node, NEL3D_AppDataVPWTFreqWD[i],
                     apd.FrequencyWindFactor[i]);

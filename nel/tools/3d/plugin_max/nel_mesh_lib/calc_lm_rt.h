...
// ***********************************************************************************************
/** A ray is a cone/cylinder coming from the vertex to evaluate against a
 *directional light The cone start at vertex, and end at a certain distance,
 *then start the cylinder. This is to allow fast grid lookup acceleration (a
 *"cone only" shape may produce a to big radius for grid lookup with large
 *scene)
 */
class CRTRay {

public:
  void initDirectionnal(uint32 nNbSide, const NLMISC::CVector &vVertex,
                        const NLMISC::CVector &lightDir, float rRadius,
                        float rDistCyl);

  void clip(const NLMISC::CTriangle
                &t); // Warning : t is transformed with the _InvVertexMat matrix
...
private:
  void clipProjected(const NLMISC::CVector &v0, const NLMISC::CVector &v1,
                     const NLMISC::CVector &v2);

  bool isShapeMustBeClippedByTriangle(SConvexShape &scs, NLMISC::CUV tri[3]);
  void clipShape(SConvexShape &ShapeIn, NLMISC::CUV Tri[3],
                 std::vector<SConvexShape> &ShapesOut);
  NLMISC::CUV getLineIntersection(const NLMISC::CUV &l1p1,
                                  const NLMISC::CUV &l1p2,
                                  const NLMISC::CUV &l2p1,
                                  const NLMISC::CUV &l2p2);
...
class CRTWorld {
  ...
  static std::map<INode *, std::pair<NL3D::CMesh::CMeshBuild *,
...
public:
  CRTWorld(bool errorInDialog, bool view, bool absolutePath, Interface *ip,
           std::string errorTitle, CExportNel *exp);
  ~CRTWorld();
  // Build meshes from lights (if a mesh interact with at least one light except
  // ambient)
  void build(std::vector<SLightBuild> &AllLights, NLMISC::CVector &vGlobalTrans,
             ...
             const std::set<INode *> &includeNode);

  // Raytrace the vertex vVertex from light nLightNb
  NLMISC::CRGBAF raytrace(NLMISC::CVector &vVertex, sint32 nLightNb,
                          uint8 &rtVal, bool bSoftShadow);

private:
  // All we need for the raytrace

  void testCell(NLMISC::CRGBAF &retValue, SGridCell &cell,
                NLMISC::CVector &vLightPos, NLMISC::CVector &vVertexPos,
                uint8 &rtVal);
  ...
  void addNode(INode *pNode, std::vector<NL3D::CMesh::CMeshBuild *> &Meshes,
               ...
               std::vector<SLightBuild> &AllLights,
               const std::set<INode *> &excludeNode, TimeValue tvTime);

  void
  getAllSelectedNode(std::vector<NL3D::CMesh::CMeshBuild *> &Meshes,
                     std::vector<NL3D::CMeshBase::CMeshBaseBuild *> &MeshesBase,
                     std::vector<INode *> &INodes,
                     std::vector<SLightBuild> &AllLights,
  ...
  void
  getAllNodeInScene(std::vector<NL3D::CMesh::CMeshBuild *> &Meshes,
  ...
  bool intersectionSphereCylinder(NLMISC::CBSphere &s,
                                  NLMISC::CVector &cyCenter,
                                  NLMISC::CVector &cyDir, float cyRadius);

  bool isInteractionWithLight(SLightBuild &rSLB, NLMISC::CAABBox &meshBox);

  bool isInteractionLightMesh(SLightBuild &rSLB, NL3D::CMesh::CMeshBuild &rMB,
...
public:
  float rMin,
      rMax; // distance min and max from the light position to clip all rays
...
private:
  void project(NLMISC::CTriangle &tri, NLMISC::CPlane pyr[4],
               NLMISC::CPlane &gridPlane, sint32 nGridNb, SGridCell &cell);
};

...
void CExportNel::buildSkeletonShape(CSkeletonShape &skeletonShape, INode &node,
...
bool CExportNel::getNELUnHeritFatherScale(INode &node) {
  ...
  /* If my parent exist, and if my parent is a BipedNode, always suppose I must
     unherit scale. This is because Biped NodeTM always have a Scale 1,1,1.
     Hence Sons bones do not herit scale.
  */
...
INode *CExportNel::getNELScaleReferenceNode(INode &node) {
  ...
  bool exportScale = getScriptAppData(&node, NEL3D_APPDATA_EXPORT_BONE_SCALE,
                                      BST_UNCHECKED) == BST_CHECKED;
  // Get the reference node
  if (exportScale) {
    std::string boneScaleNameExt =
        getScriptAppData(&node, NEL3D_APPDATA_EXPORT_BONE_SCALE_NAME_EXT, "");
...
// ***************************************************************************
void CExportNel::getNELBoneLocalScale(INode &node, TimeValue time,
                                      NLMISC::CVector &nelScale) {
  // To get the correct local Scale, we use an other bone as reference (if
  // present)
  ...
  // get the scale from std way.
    ...
    /* If this node do not inherit scale (ie must unherit), then we must not
       take the localScale computed with getLocalMatrix. In this case, the local
       Scale is simply the NodeTM scale.
    */
...
void CExportNel::getNELBoneLocalTM(INode &node, TimeValue time,
  ...
  /* The following is important To have correct NEL position
      Let me explain:
          - Biped bones NodeTM have correct World Position, but NodeTM.Scale ==
     1,1,1 (always) We can get the bone local scale from OffsetScale, this why
     we need a Reference Node, to perform the difference (because the Reference
     Node ofsset scale is not a 1,1,1 scale)
          - Nel bones Pos are the LOCAL pos, and they INHERIT parent scale (like
     in Maya). Nel UnheritScale is used to unherit the scale from father to the
     rotation/scale only part of the son. The problem is that if we export
     Position default pos from the current scaled "node", we will have the
      "already scaled from father Local Pos" from Biped. Instead we want the
     "not scaled from father Local Pos" because NEL will do the "scaled from
     father" job in RealTime.

      So to get the correct local Position, we must remove the fahter Nel Local
     Scale.
  */

  // If my father is a biped bone, then do special stuff.
  INode *parentNode = node.GetParentNode();
  // NB: don't need to test If I must unherit scale, because sons of Biped nodes
  // always unherit scale...
  if (parentNode && isBipedNode(*parentNode)) {
    /* In this case, I must remove my father Nel scale, because the biped NodeTM
       has a scale 1,1,1. Hence the Local position, computed with
       getLocalMatrix(), we have is false.
    */
    CVector fatherScale;
    // NB: avoid calling getNELBoneLocalTM() for father else Recursive call
    // untill to the root !!! We need just the scale here.
...
void CExportNel::buildSkeleton(std::vector<CBoneBase> &bonesArray, INode &node,
                               mapBoneBindPos *mapBindPos, TInodePtrInt &mapId,
                               std::set<std::string> &nameSet, TimeValue time,
                               sint32 &idCount, sint32 father) {
  ...
  // Root must be exported with Identity because path are setuped interactively
  // in the root of the skeleton
  if (id == 0) {
  ...
  if (!matrixComputed) {
    // if we have a reference node, ie the one with the good original
    // pos/rot/scale used for bindPos, must use it.
    INode *referenceNode = getNELScaleReferenceNode(node);
    if (referenceNode) {
  ...
  bone.LodDisableDistance =
      getScriptAppData(&node, NEL3D_APPDATA_BONE_LOD_DISTANCE, 0.f);
  ...
  for (int children = 0; children < numChildren; children++)
    buildSkeleton(bonesArray, *node.GetChildNode(children), mapBindPos, mapId,
                  nameSet, time, ++idCount, id);
}
...
bool CExportNel::isSkin(INode &node) {
  ...
  if (skin) {
    ...
    // if (skin->IsEnabled())
    {
      ...
      if (comSkinInterface) {
        // Get local data
        ISkinContextData *localData =
            comSkinInterface->GetContextInterface(&node);
  ...
  if (!ok) {
    ...
    if (skin) {
      ...
      {
        // Get a com_skin2 interface
        IPhysiqueExport *physiqueInterface =
            (IPhysiqueExport *)skin->GetInterface(I_PHYINTERFACE);
...
uint CExportNel::buildSkinning(CMesh::CMeshBuild &buildMesh,
  ...
  if (skin) {
    ...
    if (comSkinInterface) {
      ...
      if (localData) {
        ...
        // Ctrl we have the same number of vertices in the mesh and in the
        // modifier.
        if (buildMesh.Vertices.size() != vertCount) {
          ok = InvalidSkeleton;
        } else {
          // If not the same count, return false (perhaps, the modifier is not
          // in the good place in the stack..)
          if (buildMesh.Vertices.size() == vertCount) {
            // Rebuild the array
            ...
            for (uint vert = 0; vert < vertCount; vert++) {
              ...
              for (uint bone = 0; bone < boneCount; bone++) {
                ...
                // Insert in the map
                weightMap.insert(
                    std::map<float, uint>::value_type(weight, boneId));
              }
              ...
              while (ite != weightMap.begin()) {
                ...
                // Get the bones ID
                sint32 matrixId = -1;
                TInodePtrInt::const_iterator itId =
                    skeletonShape.find(comSkinInterface->GetBone(ite->second));
    ...
    if (physiqueInterface) {
      // Get local data
      IPhyContextExport *localData =
          physiqueInterface->GetContextInterface(&node);
        ...
        if (buildMesh.Vertices.size() != vertCount) {
          ...
          if (buildMesh.Vertices.size() == vertCount) {
            ...
            for (uint vert = 0; vert < vertCount; vert++) {
              ...
              IPhyVertexExport *vertexInterface =
                  localData->GetVertexInterface(vert);
              ...
              for (uint bone = 0; bone < boneCount; bone++) {
                if (blendedInterface) {
                  ...
                  if (node == NULL) {
                    ...
                    // Insert in the map
                    weightMap.insert(
                        std::map<float, INode *>::value_type(weight, node));
                  }
                } else {
                  // Get node
                  ...
                  // Insert in the map
                  weightMap.insert(
                      std::map<float, INode *>::value_type(1.f, node));
                }
              }
    ...
    if (comSkinInterface) {
      ...
      if (localData) {
        ...
        for (uint vtx = 0; vtx < (uint)localData->GetNumPoints(); vtx++) {
          // For each bones
          uint bone;
          for (bone = 0; bone < (uint)localData->GetNumAssignedBones(vtx);
               bone++) {
            // Get the bone pointer
            INode *newBone = comSkinInterface->GetBone(
                localData->GetAssignedBone(vtx, bone));
      ...
      if (physiqueInterface) {
        ...
        if (localData) {
          ...
          for (uint vtx = 0; vtx < numVert; vtx++) {
            ...
            // Get a vertex interface
            IPhyVertexExport *vertexInterface =
                localData->GetVertexInterface(vtx);
            ...
            if (vertexInterface) {
              ...
              if (type == RIGID_TYPE) {
                // this is a rigid vertex
                IPhyRigidVertex *rigidInterface =
                    (IPhyRigidVertex *)vertexInterface;
                ...
                // It must be a blendable vertex
                nlassert(type == RIGID_BLENDED_TYPE);
                IPhyBlendedRigidVertex *blendedInterface =
                    (IPhyBlendedRigidVertex *)vertexInterface;
...
void CExportNel::addSkeletonBindPos(INode &skinedNode,
                                    mapBoneBindPos &boneBindPos) {
  // Return success
  uint ok = NoError;
  ...
  if (skin) {
    ...
    if (comSkinInterface) {
      // Get local data
      ISkinContextData *localData =
          comSkinInterface->GetContextInterface(&skinedNode);
    ...
    if (skin) {
      ...
      if (physiqueInterface) {
        // Get local data
        IPhyContextExport *localData =
            physiqueInterface->GetContextInterface(&skinedNode);
        ...
        if (localData) {
          ...
          for (uint vert = 0; vert < vertCount; vert++) {
            ...
            if (type == RIGID_TYPE) {
              ...
              // Add an entry inthe map
              if (boneBindPos.insert(mapBoneBindPos::value_type(bone, bindPos))
                      .second) {
#ifdef NL_DEBUG
                // *** Debug info
              ...
              for (boneIndex = 0; boneIndex < count; boneIndex++) {
                ...
                if (bone == NULL) {
                  nlwarning("bone == NULL; boneIndex = %i / %i", boneIndex,
                            count);
                } else {
                  // Get the bind matrix of the bone
                  Matrix3 bindPos;
                  ...
                  if (res != MATRIX_RETURNED) {
                    nlwarning(
                        "res != MATRIX_RETURNED; res = %d; boneIndex = %u / %u",
                        res, boneIndex, count);
                  ...
                  // Add an entry inthe map
                  if (boneBindPos
                          .insert(mapBoneBindPos::value_type(bone, bindPos))
                          .second) {
#ifdef NL_DEBUG
                    // *** Debug info
                    ...
                    offsetPosTM.SetTrans(bone->GetObjOffsetPos());
                    Matrix3 offsetTM =
                        offsetScaleTM * offsetRotTM * offsetPosTM;
...
struct CSortVertex {
  uint Index;
  float SqrDist;
  bool operator<(const CSortVertex &o) const { return SqrDist < o.SqrDist; }
};
...
// From a bone, retrieve the bone mirror (by name: R / L). NB: if not found,
// return same (eg: important for mirror on spine).
static INode *getMirrorBone(const std::vector<INode *> &skeletonNodes,
                            INode *bone) {
...
bool CExportNel::mirrorPhysiqueSelection(INode &node, TimeValue tvTime,
  ...
  bbox.setHalfSize(bbox.getHalfSize() +
                   CVector(threshold, threshold, threshold));
  ...
  if (skin) {
    // Get a com_skin2 interface
    IPhysiqueImport *physiqueInterface =
        (IPhysiqueImport *)skin->GetInterface(I_PHYIMPORT);

    // Found com_skin2 ?
    if (physiqueInterface) {
      // Get local data
      IPhyContextImport *localData =
          physiqueInterface->GetContextInterface(&node);
      ...
      if (localData) {
        ...
        for (uint i = 0; i < tempVertex.size(); i++) {
          ...
          if (sv.Mirrored) {
            ...
            blendedInterface =
                (IPhyBlendedRigidVertexImport *)localData->SetVertexInterface(
                    i, RIGID_BLENDED_TYPE);

            if (blendedInterface) {
              // set the vertex data
              for (uint bone = 0; bone < sv.NumWeight; bone++) {
                blendedInterface->SetWeightedNode(sv.Bone[bone],
                                                  sv.Weight[bone], bone == 0);
              }

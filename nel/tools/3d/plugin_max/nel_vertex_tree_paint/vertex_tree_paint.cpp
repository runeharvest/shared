...
static void LoadImages() {
  ...
  hButtonImages = ImageList_Create(15, 14, ILC_MASK, 2,
                                   0); // 17 is kluge to center square. -SA
...
static INT_PTR CALLBACK VertexPaintDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                           LPARAM lParam) {
  int numPoints;
  VertexPaint *mod = (VertexPaint *)GetWindowLongPtr(hWnd, GWLP_USERDATA);
  if (!mod && msg != WM_INITDIALOG)
    return FALSE;
  int comboResult;
  ...
  case WM_INITDIALOG:
    ...
    mod->iPaintButton->SetCheck(
        mod->ip->GetCommandMode()->ID() == CID_PAINT &&
        !((PaintMouseProc *)mod->ip->GetCommandMode()->MouseProc(&numPoints))
             ->GetPickMode());
    ...
    mod->iColorGradient[0] =
        GetIColorSwatch(GetDlgItem(hWnd, IDC_PALETTE_GRAD0));
    mod->iColorGradient[1] =
        GetIColorSwatch(GetDlgItem(hWnd, IDC_PALETTE_GRAD1));
    mod->iColorGradient[0]->SetColor(mod->lastGradientColor[0]);
    mod->iColorGradient[1]->SetColor(mod->lastGradientColor[1]);

    // Init comboBox
    SendDlgItemMessage(hWnd, IDC_COMBO_TYPE, CB_ADDSTRING, 0,
                       (LPARAM) _T("Tree Weight"));
    SendDlgItemMessage(hWnd, IDC_COMBO_TYPE, CB_ADDSTRING, 0,
                       (LPARAM) _T("Phase Level 1"));
    SendDlgItemMessage(hWnd, IDC_COMBO_TYPE, CB_ADDSTRING, 0,
                       (LPARAM) _T("Phase Level 2"));
    SendDlgItemMessage(hWnd, IDC_COMBO_TYPE, CB_SETCURSEL,
                       mod->getEditionType(), 0);
  ...
  case CC_COLOR_CHANGE:
    if (LOWORD(wParam) == IDC_COLOR) {
      ...
      case 0:
        mod->lastWeightColor = iCol->GetColor();
        break;
      case 1:
      case 2:
  ...
  case WM_COMMAND:
    ...
    case IDC_COMBO_TYPE:
      // Init default type.
      comboResult =
          SendDlgItemMessage(hWnd, IDC_COMBO_TYPE, CB_GETCURSEL, 0, 0);
...
// Subclass procedure
LRESULT APIENTRY colorSwatchSubclassWndProc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                            LPARAM lParam) {
  switch (uMsg) {
  case WM_LBUTTONDOWN:
  case WM_LBUTTONUP:
  ...
  case WM_DESTROY:
    ...
    // Fallthrough...
  default:
    return CallWindowProc(colorSwatchOriginalWndProc, hwnd, uMsg, wParam,
                          lParam);
    break;
  }
  return 0;
...
COLORREF VertexPaint::palColors[] = {
    // RGB(32,  32,  32),	RGB(  96,96,96),	RGB(  160,160,160),
    // RGB(224,224,224) };
...
VertexPaint::VertexPaint()
    : iTint(NULL), fTint(1.0f), iGradientBend(NULL), fGradientBend(0.0f) {
  flags = 0x0;
  _EditType = 0;
...
Interval VertexPaint::LocalValidity(TimeValue t) { return FOREVER; }

BOOL VertexPaint::DependOnTopology(ModContext &mc) { return TRUE; }
...
void VertexPaint::NotifyInputChanged(Interval changeInt, PartID partID,
...
void VertexPaint::ModifyObject(TimeValue t, ModContext &mc, ObjectState *os,
                               INode *node) {
  if (!os->obj->IsSubClassOf(triObjectClassID))
    return;

  os->obj->ReadyChannelsForMod(GEOM_CHANNEL | TOPO_CHANNEL | VERTCOLOR_CHANNEL |
                               TEXMAP_CHANNEL);
  ...
  if (mesh) {
    ...
    if (!d)
      mc.localData = d = new VertexPaintData(tobj->GetMesh());
    if (!d->GetMesh())
      d->SetCache(*mesh);

    {
      ...
      // If the incoming Mesh had no vertex colors, this will add a default map
      // to start with. The default map has the same topology as the Mesh (so
      // one color per vertex), with all colors set to white.
      if (!mesh->mapSupport(0))
        md.AddVertexColors();
      // if (cache && !cache->mapSupport(0)) mdc.AddVertexColors ();

      // We used two routines -- VCreate to add new map vertices, and FRemap to
      // make the existing map faces use the new verts.  frFlags tell FRemap
      // which vertices on a face should be "remapped", and the ww array
      // contains the new locations.
      VertColor nvc;
      int j;
...
void VertexPaint::BeginEditParams(IObjParam *ip, ULONG flags,
                                  Animatable *prev) {

  this->ip = ip;
  editMod = this;
  if (!hParams) {
    hParams = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_PANEL),
                                VertexPaintDlgProc, GetString(IDS_PARAMS),
                                (LPARAM)this);
    ...
    for (i = 0; i < NUMPALETTES; i++) {
      colorSwatchOriginalWndProc = (WNDPROC)SetWindowLongPtr(
          hPaletteWnd[i], GWLP_WNDPROC, (LONG_PTR)colorSwatchSubclassWndProc);
    }
  ...
  iTint = SetupIntSpinner(hParams, IDC_TINT_SPIN, IDC_TINT, 0, 100,
                          (int)(fTint * 100.0f));

  // Init Gradient Bend spinner
  iGradientBend = SetupIntSpinner(hParams, IDC_BEND_SPIN, IDC_BEND, 0, 100,
                                  (int)(fGradientBend * 100.0f));
...
int VertexPaint::NumRefs() { return 0; }

RefTargetHandle VertexPaint::GetReference(int i) { return NULL; }

void VertexPaint::SetReference(int i, RefTargetHandle rtarg) {}

int VertexPaint::NumSubs() { return 0; }

Animatable *VertexPaint::SubAnim(int i) { return NULL; }
...
void VertexPaint::backupCurrentColor() {
  ...
  case 0:
    lastWeightColor = iColor->GetColor();
    break;
  case 1:
  case 2:
    lastPhaseColor = iColor->GetColor();
    break;
  }
}
void VertexPaint::reloadBkupColor() {
  ...
  switch (getEditionType()) {
  case 0:
    iColor->SetColor(lastWeightColor);
    break;
  case 1:
  case 2:
    iColor->SetColor(lastPhaseColor);
    break;
  }
}
...
void VertexPaint::fillSelectionColor() {
  ...
  for (mci = 0; mci < modContexts.Count(); mci++) {
    ...
    if (mc && mc->localData)
      theHold.Put(
          new VertexPaintRestore((VertexPaintData *)mc->localData, this));
  }
  ...
  switch (getEditionType()) {
  case 0:
    whichComponent = VertexPaintData::Red;
    break;
  case 1:
    whichComponent = VertexPaintData::Green;
    break;
  case 2:
    whichComponent = VertexPaintData::Blue;
    break;
  }
...
VertexPaintData::VertexPaintData(Mesh &m)
    : mesh(NULL), colordata(NULL), nverts(NULL), nvcverts(NULL), numColors(0),
      numnverts(0), numnvcverts(0) {
  SetCache(m);
}
...
Mesh *VertexPaintData::GetMesh() { return mesh; }
...
void VertexPaintData::SetColor(int i, float bary, COLORREF c,
                               TComponent whichComp) {
...
int VertexPaintData::GetNumColors() { return numColors; }
...
void VertexPaintData::AllocColorData(int numcols) {
  ...
  if (numColors > 0) {
    // If the new number of colors is bigger than what we have in the colordata
    // array
...
VertexPaintRestore::VertexPaintRestore(VertexPaintData *pLocalData,
                                       VertexPaint *pVPaint)

...
class VertexPaint : public Modifier {
  ...
  friend INT_PTR CALLBACK VertexPaintDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                             LPARAM lParam);
...
public:
  ...
  ChannelMask ChannelsUsed() {
    return GEOM_CHANNEL | TOPO_CHANNEL | VERTCOLOR_CHANNEL | PART_SUBSEL_TYPE |
           SELECT_CHANNEL | PART_DISPLAY | TEXMAP_CHANNEL;
  }
  ChannelMask ChannelsChanged() {
    return VERTCOLOR_CHANNEL | TOPO_CHANNEL | GEOM_CHANNEL;
  }
...
private:
  ...
  void SetFlag(DWORD fl, bool set) {
    if (set)
      SetFlag(fl);
    else
      ClearFlag(fl);
...
public:
  int proc(HWND hWnd, int msg, int point, int flags,
           IPoint2 m); // Mouse callback
...
public:
  ...
  void SetModifier(VertexPaint *pMod) {
    pModifier = pMod;
    mouseProc.SetModifier(pMod);
  }

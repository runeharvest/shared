...
BOOL VertexPaint::ActivatePaint(BOOL bOnOff, BOOL bPick) {
  ...
  if (bOnOff) {
    ...
    if (!hDropperCursor) {
      hDropperCursor =
          LoadCursor(hInstance, MAKEINTRESOURCE(IDC_DROPPER_CURSOR));
    }
      ...
      if (mesh->IntersectRay(ray, at, norm, fi, bary)) {
        if (mesh->vertCol) {
          ...
          for (int i = 0; i < 3; i++) {
            ...
            // Apply painting
            d->SetColor(f->v[i], opacity, pModifier->GetActiveColor(),
                        whichComponent);
          }
...
void PaintMouseProc::MaybeStartHold() {
  ...
  for (int i = 0; i < modContexts.Count(); i++) {
    ...
    if (mc && mc->localData)
      theHold.Put(
          new VertexPaintRestore((VertexPaintData *)mc->localData, pModifier));
  }
}
...
PaintCommandMode::PaintCommandMode() { iInterface = NULL; }

int PaintCommandMode::Class() { return PICK_COMMAND; }

int PaintCommandMode::ID() { return CID_PAINT; }
...
BOOL PaintCommandMode::ChangeFG(CommandMode *oldMode) { return FALSE; }
...
void VertexPaint::EnterMode() {
  int numPoints;
  iPaintButton->SetCheck(
      ip->GetCommandMode()->ID() == CID_PAINT &&
      !((PaintMouseProc *)ip->GetCommandMode()->MouseProc(&numPoints))
           ->GetPickMode());
  iPickButton->SetCheck(
      ip->GetCommandMode()->ID() == CID_PAINT &&
      ((PaintMouseProc *)ip->GetCommandMode()->MouseProc(&numPoints))
          ->GetPickMode());
...
COLORREF VertexPaint::GetActiveColor() { return iColor->GetColor(); }
...
TriObject *GetTriObjectFromNode(INode *node, TimeValue t, int &deleteIt) {
  ...
  if (obj->CanConvertToType(Class_ID(TRIOBJ_CLASS_ID, 0))) {
    TriObject *tri =
        (TriObject *)obj->ConvertToType(0, Class_ID(TRIOBJ_CLASS_ID, 0));

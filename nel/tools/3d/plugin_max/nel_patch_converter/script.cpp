...
void errorMessage(const MCHAR *msg, const TCHAR *title, Interface &it,
                  bool dialog) {
  // Text or dialog ?
  if (dialog) {
    // Dialog message
...
Value *export_zone_cf(Value **arg_list, int count) {
  ...
  if (os.obj) {
    ...
    if (tri) {
      ...
      if (tri->rpatch->exportZone(node, &tri->patch, zone, zoneSymmetry, nZone,
                                  160, 1, false)) {
        // Export path
...
Value *import_zone_cf(Value **arg_list, int count) {
  ...
  if (input.open(filename)) {
    try {
      ...
      RPO *rpo =
          (RPO *)CreateInstance(GEOMOBJECT_CLASS_ID, RYKOLPATCHOBJ_CLASS_ID);
      ...
      // Error message
      std::string msg = toString("Error when loading file %s: %s",
    ...
    // Error message
    std::string msg =
        toString("Can't open the file %s for reading.", filename.c_str());
    errorMessage(MaxTStrFromUtf8(msg), _M("NeL import zone"), *ip, dialog);
  }
...
Value *get_selected_tile_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_selected_patch_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_selected_vertex_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_tile_mode_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_compute_interior_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_interior_mode_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
    ...
    if (os.obj->CanConvertToType(RYKOLPATCHOBJ_CLASS_ID)) {
      ...
      if (tri) {
        if (arg_list[1]->to_int() <= tri->patch.numPatches)
          tri->patch.patches[arg_list[1]->to_int() - 1].SetAuto(
              arg_list[2]->to_int());
      }
      // Note that the TriObject should only be deleted
      // if the pointer to it is not equal to the object
...
Value *set_vertex_count_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_vector_count_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_vertex_pos_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_vector_pos_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_vertex_pos_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_vector_pos_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_edge_vect1_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_edge_vect2_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_edge_vert1_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_edge_vert2_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_sel_edge_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *set_steps_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
  ...
  if (os.obj) {
    ...
    if (os.obj->CanConvertToType(Class_ID(PATCHOBJ_CLASS_ID, 0))) {
      bRet = true;
      PatchObject *tri = (PatchObject *)os.obj->ConvertToType(
          ip->GetTime(), Class_ID(PATCHOBJ_CLASS_ID, 0));
      if (tri)
...
Value *set_tile_steps_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_tile_count_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
  check_arg_count(get_tile_count, 2, count);

  // Check to see if the arguments match up to what we expect
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], MAXNode,
             _M("GetRykolTileCount [Zone] [PatchNumber]"));
  // type_check(arg_list[1], Integer, "SetRykolPatchSteps [Object]");
...
Value *get_patch_count_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
...
Value *get_tile_tile_number_cf(Value **arg_list, int count) {
  ...
  type_check(
      arg_list[0], MAXNode,
      _M("NelGetTileTileNumber [Zone] [PatchNumber] [TileNumber] [Layer]"));
  type_check(
      arg_list[1], Integer,
      _M("NelGetTileTileNumber [Zone] [PatchNumber] [TileNumber] [Layer]"));
  type_check(
      ...
      if (tri) {
        ...
        if (nPatch >= tri->rpatch->getUIPatchSize()) {
          ...
          if (tile >= nPatchCount) {
            ...
            // Tile index
            if (layer < (uint)tri->rpatch->getUIPatch(nPatch)
                            .getTileDesc(tile)
                            .getNumLayer()) {
              nRet = tri->rpatch->getUIPatch(nPatch)
                         .getTileDesc(tile)
                         .getLayer(layer)
                         .Tile +
                     1;
...
Value *get_tile_noise_number_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (4)
  check_arg_count(get_tile_count, 3, count);
  ...
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], MAXNode,
             _M("NelGetTileTileNumber [Zone] [PatchNumber] [TileNumber]"));
  type_check(arg_list[1], Integer,
             _M("NelGetTileTileNumber [Zone] [PatchNumber] [TileNumber]"));
  type_check(arg_list[2], Integer,
             _M("NelGetTileTileNumber [Zone] [PatchNumber] [TileNumber]"));
    ...
    if (tri) {
      ...
      if (nPatch >= tri->rpatch->getUIPatchSize()) {
        ...
        if (tile >= nPatchCount) {
          ...
          // Tile index
          nRet =
              tri->rpatch->getUIPatch(nPatch).getTileDesc(tile).getDisplace() +
              1;
        }
      }
...
Value *set_tile_noise_number_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (4)
  check_arg_count(get_tile_count, 4, count);
  ...
  type_check(
      arg_list[0], MAXNode,
      _M("NelGetTileNoiseNumber [Zone] [PatchNumber] [TileNumber] [noise]"));
  type_check(
      arg_list[1], Integer,
      _M("NelGetTileNoiseNumber [Zone] [PatchNumber] [TileNumber] [noise]"));
  type_check(
      ...
      if (tri) {
        ...
        if (nPatch >= tri->rpatch->getUIPatchSize()) {
          ...
          if (tile >= nPatchCount) {
            ...
            // Tile index
            tri->rpatch->getUIPatch(nPatch).getTileDesc(tile).setDisplace(
                noise);
            bRet = true;
          }
        }
...
Value *load_bank_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (4)
...
Value *get_tile_set_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (4)
...
void DoAttach(INode *node, PatchMesh *patch, RPatchMesh *rpatch, INode *attNode,
              PatchMesh *attPatch, RPatchMesh *rattPatch, Interface *ip) {
      ...
      if (attPatch->patches[i].type == PATCH_QUAD) {
        ...
        for (nU = 0; nU < ctU; nU++) {
          for (int nV = 0; nV < ctV; nV++) {
            rattPatch->getUIPatch(i).getTileDesc(nU + nV * ctU) =
                rpatch.getTileDesc(ctU - 1 - nU + (ctV - 1 - nV) * ctU);
          }
        }
        for (nU = 0; nU < ctU + 1; nU++) {
          for (int nV = 0; nV < ctV + 1; nV++) {
            rattPatch->getUIPatch(i).setColor(
                nU + nV * (ctU + 1),
                rpatch.getColor(ctU - nU + (ctV - nV) * ctU));
          }
        }
      }
  ...
  if (m1 && m2 && (m1 != m2)) {
    ...
    // This should be revisited in the future so we don't have to use the
    // SYSSET_CLEAR_UNDO. theHold.Suspend();
...
Value *attach_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
  ...
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], MAXNode,
             _M("NeLAttachPatchMesh [RykolPatchMeshSrc] [RykolPatchMeshDest]"));
  type_check(arg_list[1], MAXNode,
             _M("NeLAttachPatchMesh [RykolPatchMeshSrc] [RykolPatchMeshDest]"));
  ...
  if (osSrc.obj && osDest.obj) {
    ...
    if ((osSrc.obj->CanConvertToType(RYKOLPATCHOBJ_CLASS_ID)) &&
        (osDest.obj->CanConvertToType(RYKOLPATCHOBJ_CLASS_ID))) {
      ...
      RPO *triDest = (RPO *)osDest.obj->ConvertToType(ip->GetTime(),
                                                      RYKOLPATCHOBJ_CLASS_ID);
      if (triSrc && triDest) {
        DoAttach(nodeDest, &triDest->patch, triDest->rpatch, nodeSrc,
                 &triSrc->patch, triSrc->rpatch, ip);
        bRet = true;
      }
      // Note that the TriObject should only be deleted
      // if the pointer to it is not equal to the object
...
Value *weld_cf(Value **arg_list, int count) {
  // Make sure we have the correct number of arguments (2)
  check_arg_count(attach, 2, count);
  ...
  // We want to use 'TurnAllTexturesOn <object to use>'
  type_check(arg_list[0], MAXNode,
             _M("NeLWeldPatchMesh [RykolPatchMeshSrc] [threshold]"));
  type_check(arg_list[1], Float,
             _M("NeLWeldPatchMesh [RykolPatchMeshSrc] [threshold]"));

...
public:
  ...
  const MCHAR *GetObjectName(bool localized) const {
    if (localized)
      ...
      return _T("Rykol Patchmesh Converter");
  }
#endif
  ...
  ChannelMask ChannelsUsed() {
    return (PART_GEOM | SELECT_CHANNEL | PART_SUBSEL_TYPE | PART_DISPLAY |
  ...
  void GetClassName(TSTR &s, bool localized) const {
    if (localized)
      ...
      s = _T("Rykol Patchmesh Converter");
  }
#endif
...
public:
  ...
  const MCHAR *InternalName() {
    return _M("PatchObjectToNelPatchObject");

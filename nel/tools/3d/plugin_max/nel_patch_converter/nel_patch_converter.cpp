...
static ParamBlockDesc2
    po2rpo_param_blk(po2rpo_params, _T("params"), 0, &PO2RPODesc,
                     P_AUTO_CONSTRUCT + P_AUTO_UI, PBLOCK_REF,
                     // rollout
                     IDD_PANEL, IDS_PARAMS, 0, 0, NULL,
                     // params
                     pb_spin, _T("spin"), TYPE_FLOAT, P_ANIMATABLE, IDS_SPIN,
                     p_default, 0.1f, p_range, 0.0f, 1000.0f, p_ui,
                     TYPE_SPINNER, EDITTYPE_FLOAT, IDC_EDIT, IDC_SPIN, 0.01f,
                     nl_p_end, nl_p_end);
...
PO2RPO::PO2RPO() : pblock(NULL) { PO2RPODesc.MakeAutoParamBlocks(this); }
...
static int done = 0;
void PO2RPO::ModifyObject(TimeValue t, ModContext &mc, ObjectState *os,
                          INode *node) {
  /*
      if (!done)
      {
  ...
  */
  // Note the default implementation: a class is considered to also be a
  // subclass of itself.
...
INT_PTR CALLBACK DlgProc_Panel(HWND hWnd, UINT message, WPARAM wParam,
                               LPARAM lParam) {
  switch (message) {
  // -----
  case WM_INITDIALOG: {
...
void PO2RPO::BeginEditParams(IObjParam *ip, ULONG flags, Animatable *prev) {
  this->ip = ip;
  hRollup = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_PANEL),
                              DlgProc_Panel, _T("PO2RPO"), (LPARAM)this);

...
BOOL PickPatchAttach::HitTest(IObjParam *ip, HWND hWnd, ViewExp *vpt, IPoint2 m,
                              int flags) {
...
BOOL PickPatchAttach::Pick(IObjParam *ip, ViewExp *vpt) {
  ...
  nlassert(node);
  GeomObject *object =
      (GeomObject *)node->GetObjectRef()->Eval(ip->GetTime()).obj;
  if (object->CanConvertToType(RYKOLPATCHOBJ_CLASS_ID)) {
    RPO *attPatch =
        (RPO *)object->ConvertToType(ip->GetTime(), RYKOLPATCHOBJ_CLASS_ID);
    if (attPatch) {
      ...
      BOOL res = TRUE;
      if (nodes[0]->GetMtl() && node->GetMtl() &&
          (nodes[0]->GetMtl() != node->GetMtl()))

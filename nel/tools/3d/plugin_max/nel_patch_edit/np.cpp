 ...
 *<
    ...
    IMPORTANT USAGE NOTE:
        ...
        When you do an operation in edit patch which will change the topology,
 the form of the code should look like this code, taken from the vertex
 deletion:
        ...
        continue;
          ...
          -->		patchData->RecordTopologyTags(patch);
            ...
            // If this is the first edit, then the delta arrays will be
 allocated patchData->BeginEdit(t);
 ...
 "DoVertDelete"));
        ...
        The key elements in the "changed topology" case are the calls noted by
 arrows. These record special tags inside the object so that after the topology
 is changed by the modifier code, the UpdateChanges code can make a new mapping
 from the old object topology to the new.

        If the operation doesn't change the topology, then the three topology
 tag calls aren't needed and the UpdateChanges call becomes:
...
// Forward references
INT_PTR CALLBACK PatchSelectDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                    LPARAM lParam);
INT_PTR CALLBACK PatchOpsDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                 LPARAM lParam);
INT_PTR CALLBACK PatchObjSurfDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                     LPARAM lParam);
INT_PTR CALLBACK PatchSurfDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                  LPARAM lParam);
INT_PTR CALLBACK PatchTileDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                  LPARAM lParam);
INT_PTR CALLBACK PatchEdgeDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                  LPARAM lParam);
...
void SetPatchHitOverride(int value) { patchHitOverride = value; }

void ClearPatchHitOverride() { patchHitOverride = 0; }
...
int EditPatchMod::Display(TimeValue t, INode *inode, ViewExp *vpt, int flags,
                          ModContext *mc) {
  return 0;
}

void EditPatchMod::GetWorldBoundBox(TimeValue t, INode *inode, ViewExp *vpt,
...
public:
  ...
  void Redo() { *(setList->sets[index]) = newset; }

  TSTR Description() { return TSTR(_T("Change Named Sel Set")); }
};

// Selection set, misc fixup utility function
// This depends on PatchMesh::RecordTopologyTags being called prior to the topo
// changes
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    for (set = 0; set < patchData->vselSet.Count(); ++set) {
      ...
      for (int vert = 0; vert < patch->numVerts; ++vert) {
        // Get the knot's previous location, then copy that selection into the
        // new set
  ...
  case EP_VERTEX: {
    ...
    for (int i = 0; i < mcList.Count(); i++) {
      ...
      if (patchData->tempData &&
          patchData->TempData(this)->PatchCached(ip->GetTime())) {
        RPatchMesh *rpatch;
        PatchMesh *patch =
...
void EditPatchMod::DoVertWeld() {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (patch->vertSel.NumberSet() > 1) {
      ...
      if (theHold.Holding())
        theHold.Put(
            new PatchRestore(patchData, this, patch, rpatch, _T("DoVertWeld")));
...
void ResetVert(PatchMesh *patch) {
  ...
  for (int nV = 0; nV < patch->numVerts; nV++) {
    ...
    if (patch->vertSel[nV]) {
      ...
      for (nn = 0; nn < listSize; nn++) {
        Point3 vect2 = patch
                           ->verts[(patch->edges[pList[nn]].v1 == nV)
                                       ? patch->edges[pList[nn]].v2
                                       : patch->edges[pList[nn]].v1]
                           .p;
        vect2 -= vert;
        vect2 /= 3.f;
        ...
        tmp1 = Normalize(tmp1);
        int nTang = (patch->edges[pList[nn]].v1 == nV)
                        ? patch->edges[pList[nn]].vec12
                        : patch->edges[pList[nn]].vec21;
      ...
      {
          int
      nTang=(patch->edges[pList[nn]].v1==nV)?patch->edges[pList[nn]].vec12:patch->edges[pList[nn]].vec21;
...
Value *turn_patch_cf(Value **arg_list, int count) {
  ...
  type_check(arg_list[0], MAXNode,
             _M("RykolTurnPatch [Node] [Modifier] [Patch]"));
  type_check(arg_list[1], MAXModifier,
             _M("RykolTurnPatch [Node] [Modifier] [Patch]"));

...
int EditPatchMod::GetSubobjectLevel() { return selLevel; }
...
void EditPatchMod::RefreshSelType() {
  ...
  if (hOpsPanel) {
    ...
    {
    hSurfPanel = ip->AddRollupPage(hInstance,
    MAKEINTRESOURCE(IDD_EDPATCH_SURF_OBJ), PatchObjSurfDlgProc,
    GetString(IDS_TH_SURFACEPROPERTIES), (LPARAM) this, rsSurf ? 0 :
    APPENDROLL_CLOSED);
    }
    else
    */
    if (selLevel == EP_PATCH) {
      hSurfPanel = ip->AddRollupPage(
          hInstance, MAKEINTRESOURCE(IDD_EDPATCH_SURF), PatchSurfDlgProc,
          GetString(IDS_TH_SURFACEPROPERTIES), (LPARAM)this,
          rsSurf ? 0 : APPENDROLL_CLOSED);
    }
    if (selLevel == EP_TILE) {
      hTilePanel = ip->AddRollupPage(
          hInstance, MAKEINTRESOURCE(IDD_EDPATCH_TILE), PatchTileDlgProc,
          _M("Tile Properties"), (LPARAM)this, rsTile ? 0 : APPENDROLL_CLOSED);
    }
    if (selLevel == EP_EDGE) {
      hEdgePanel = ip->AddRollupPage(
          hInstance, MAKEINTRESOURCE(IDD_EDPATCH_EDGE), PatchEdgeDlgProc,
          _M("Edge Properties"), (LPARAM)this, rsEdge ? 0 : APPENDROLL_CLOSED);
    }
    SetSurfDlgEnables();
    SetTileDlgEnables();
    SetEdgeDlgEnables();
  ...
  ICustToolbar *iToolbar =
      GetICustToolbar(GetDlgItem(hSelectPanel, IDC_SELTYPE));
  ICustButton *but;
  for (int i = 1; i < 5; i++) {
      ...
      for (int i = 0; i < mcList.Count(); i++) {
        ...
        case EP_VERTEX:
          for (dataSet = 0; dataSet < patchData->vselSet.Count(); ++dataSet) {
            if (*(patchData->vselSet.names[dataSet]) ==
                *namedSel[sublevel][set]) {
        ...
        case EP_EDGE:
          for (dataSet = 0; dataSet < patchData->eselSet.Count(); ++dataSet) {
            if (*(patchData->eselSet.names[dataSet]) ==
                *namedSel[sublevel][set]) {
        ...
        case EP_PATCH:
          for (dataSet = 0; dataSet < patchData->pselSet.Count(); ++dataSet) {
            if (*(patchData->pselSet.names[dataSet]) ==
                *namedSel[sublevel][set]) {
...
// Select a subcomponent within our object(s).  WARNING! Because the HitRecord
// list can indicate any of the objects contained within the group of patches
// being edited, we need to watch for control breaks in the patchData pointer
// within the HitRecord!

void EditPatchMod::SelectSubComponent(HitRecord *hitRec, BOOL selected,
...
void EditPatchMod::SelectAll(int selLevel) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (theHold.Holding()) {
      theHold.Put(
          new PatchRestore(patchData, this, patch, rpatch, _T("SelectAll")));
    }
...
void EditPatchMod::ActivateSubobjSel(int level, XFormModes &modes) {
  ...
  case EP_PATCH:
    modes = XFormModes(moveMode, rotMode, nuscaleMode, uscaleMode, squashMode,
                       selectMode);
    break;
...
void EditPatchMod::ActivateSubSelSet(TSTR &setName) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (set) {
      ...
      BitArray *psel = GetLevelSelectionSet(
          patch, rpatch); // Get the appropriate selection set

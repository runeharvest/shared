...
INT_PTR CALLBACK SelectByMatDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                    LPARAM lParam) {
  static int *param;
  switch (msg) {
  case WM_INITDIALOG:
    ...
    /*SetupIntSpinner(hWnd, IDC_TESS_U_SPIN, IDC_TESS_U2, 1, RPO_DEFAULT_TESSEL,
    param[2]); SetupIntSpinner(hWnd, IDC_TESS_V_SPIN, IDC_TESS_V2, 1,
    RPO_DEFAULT_TESSEL, param[3]);*/
...
void SetSmoothButtonState(HWND hWnd, DWORD bits, DWORD invalid,
                          DWORD unused = 0) {
...
INT_PTR CALLBACK SelectBySmoothDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) {
  static DWORD *param;
  switch (msg) {
  case WM_INITDIALOG:
...
INT_PTR CALLBACK PatchEdgeDlgProc(HWND hDlg, UINT message, WPARAM wParam,
  ...
  case WM_COMMAND:
    ...
    case IDC_NO_SMOOTH:
      ...
      // Set the smooth flag for selected edges if state is checked or
      // indeterminate
      ep->setSmoothFlags(IsDlgButtonChecked(hDlg, IDC_NO_SMOOTH) ==
                         BST_CHECKED);
      break;
    }
    break;
...
INT_PTR CALLBACK PatchSurfDlgProc(HWND hDlg, UINT message, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    ...
    ep->tessUSpin = SetupIntSpinner(hDlg, IDC_TESS_U_SPIN, IDC_TESS_U2, 1, 4,
                                    RPO_DEFAULT_TESSEL);
    ep->tessVSpin = SetupIntSpinner(hDlg, IDC_TESS_V_SPIN, IDC_TESS_V2, 1, 4,
                                    RPO_DEFAULT_TESSEL);
  ...
  case WM_COMMAND:
    ...
    case IDC_SELECT_BYID: {
      if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_SELECTBYMAT),
                         ep->ip->GetMAXHWnd(), SelectByMatDlgProc,
                         (LPARAM)sbmParams)) {
    ...
    case IDC_SELECTBYSMOOTH: {
      ...
      if (DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_EM_SELECTBYSMOOTH),
                         ep->ip->GetMAXHWnd(), SelectBySmoothDlgProc,
                         (LPARAM)sbsParams)) {

...
extern void DeletePatchParts(PatchMesh *patch, RPatchMesh *rpatch,
                             BitArray &delVerts, BitArray &delPatches);
...
BOOL ClearPVertSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                               int reRecord) {
  if (reRecord)
    sel = patch->vertSel;
...
BOOL SetPVertSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord)
    sel = patch->vertSel;
...
BOOL InvertPVertSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                                int reRecord) {
...
BOOL ClearPEdgeSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                               int reRecord) {
  if (reRecord)
    sel = patch->edgeSel;
...
BOOL SetPEdgeSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord)
    sel = patch->edgeSel;
...
BOOL InvertPEdgeSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                                int reRecord) {
...
BOOL ClearPatchSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                               int reRecord) {
  if (reRecord)
    sel = patch->patchSel;
...
BOOL SetPatchSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord)
    sel = patch->patchSel;
...
BOOL InvertPatchSelRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                                int reRecord) {
...
BOOL PVertDeleteRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord) {
    oldPatch = *patch;
    roldPatch = *rpatch;
...
BOOL PVertChangeRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord) {
    oldPatch = *patch;
    roldPatch = *rpatch;
...
BOOL PatchDetachRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord && !copy) {
    oldPatch = *patch;
    roldPatch = *rpatch;
...
BOOL PatchChangeRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
...
IOResult PatchAddRecord::Load(ILoad *iload) {
  ...
  while (IO_OK == (res = iload->OpenChunk())) {
    ...
    case PADDR_TYPE_CHUNK:
      ...
      //				res = oldPatch.Load(iload);
      //				break;
      // If the following chunk is present, it's a MAX 2.0 file and a
      // post-addition weld is to be performed
...
// Compute midpoint division for patch vectors -- Provide patchmesh, patch
// number, 4 bez points returns 2 new vectors
/*
static Point3 InterpPoint(PatchMesh *patch, int index, float pct, int e1, int
i1, int i2, int e2, Point3 *v1 = NULL, Point3 *v2 = NULL, Point3 *v3 = NULL,
Point3 *v4 = NULL)
{
...
static Point3 InterpPoint(float pct, Point3 e1, Point3 i1, Point3 i2, Point3 e2,
Point3 *v1 = NULL, Point3 *v2 = NULL, Point3 *v3 = NULL, Point3 *v4 = NULL)
{
...
static Point3 InterpDegree3(Point3 a, Point3 b, Point3 c, Point3 d, float
interp)
{
    Point3 ab = a +(b - a) * interp;
    Point3 bc = b +(c - b) * interp;
...
*/
extern void SubdividePatch(int type, BOOL propagate, PatchMesh *patch,
                           RPatchMesh *rpatch);

BOOL EdgeSubdivideRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                               int reRecord) {
  if (reRecord) {
    oldPatch = *patch;
    roldPatch = *rpatch;
...
BOOL PatchSubdivideRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                                int reRecord) {
  if (reRecord) {
    oldPatch = *patch;
    roldPatch = *rpatch;

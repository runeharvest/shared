...
void EditPatchData::Apply(TimeValue t, RPO *patchOb, int selLevel) {
  ...
  if (!GetFlag(EPD_UPDATING_CACHE) && tempData && tempData->PatchCached(t)) {
    ...
    patchOb->patch.DeepCopy(patch, PART_GEOM | SELECT_CHANNEL |
    ...
    *patchOb->rpatch = *rpatch;
    ...
    // For old files, which contain exhaustive data to reconstruct the editing
    // process of patches, we'll have data in the 'changes' table.  If it's
    // there, go ahead and replay the edits, then store the alterations in our
    // new delta format and discard the change table!
    ...
    if (count) {
      ...
      for (int i = 0; i < count; ++i) {
        ...
        // If we hit one that didn't play back OK, we need to flush the
        // remainder
        if (!result) {
...
void EditPatchData::UpdateChanges(PatchMesh *patch, RPatchMesh *rpatch,
                                  BOOL checkTopology) {
  if (theHold.Holding()) {
...
IOResult EditPatchData::Save(ISave *isave) {
  ...
  //	isave->Write(&meshAdaptive,sizeof(BOOL),&nb);	// Future use (Not used
  //now)
  isave->EndChunk();
...
IOResult EditPatchData::Load(ILoad *iload) {
  ...
  while (IO_OK == (res = iload->OpenChunk())) {
    ...
    case MESH_ATTRIB_CHUNK:
      ...
      res = iload->Read(&meshAdaptive, sizeof(BOOL),
                        &nb); // Future use (Not used now)
      break;
      // 3-18-99 to suport render steps and removal of the mental tesselator

...
static void SetVertFilter() {
  patchHitLevel[EP_VERTEX] = (filterVerts ? SUBHIT_PATCH_VERTS : 0) |
                             (filterVecs ? SUBHIT_PATCH_VECS : 0);
}
...
INT_PTR CALLBACK PatchSelectDlgProc(HWND hDlg, UINT message, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    ...
    iToolbar->AddTool(
        ToolButtonItem(CTB_CHECKBUTTON, 0, 4, 0, 4, 24, 23, 24, 23, EP_VERTEX));
    iToolbar->AddTool(
        ToolButtonItem(CTB_CHECKBUTTON, 1, 5, 1, 5, 24, 23, 24, 23, EP_EDGE));
    iToolbar->AddTool(
        ToolButtonItem(CTB_CHECKBUTTON, 2, 6, 2, 6, 24, 23, 24, 23, EP_PATCH));
    iToolbar->AddTool(
        ToolButtonItem(CTB_CHECKBUTTON, 3, 7, 3, 7, 24, 23, 24, 23, EP_TILE));
    ReleaseICustToolbar(iToolbar);
    ep->RefreshSelType();
    CheckDlgButton(hDlg, IDC_DISPLATTICE, ep->displayLattice);
    //			CheckDlgButton( hDlg, IDC_DISPSURFACE,
    //ep->displaySurface);
  ...
  case WM_COMMAND: {
    ...
    case EP_VERTEX:
      if (ep->GetSubobjectLevel() == EP_VERTEX)
        ...
        ep->ip->SetSubObjectLevel(EP_VERTEX);
      needRedraw = TRUE;
      break;
    ...
    case EP_EDGE:
      if (ep->GetSubobjectLevel() == EP_EDGE)
        ...
        ep->ip->SetSubObjectLevel(EP_EDGE);
      needRedraw = TRUE;
      break;
    ...
    case EP_PATCH:
      if (ep->GetSubobjectLevel() == EP_PATCH)
        ...
        ep->ip->SetSubObjectLevel(EP_PATCH);
      needRedraw = TRUE;
      break;
    ...
    case EP_TILE:
      if (ep->GetSubobjectLevel() == EP_TILE)
        ...
        ep->ip->SetSubObjectLevel(EP_TILE);
      needRedraw = TRUE;
      break;
...
INT_PTR CALLBACK PatchOpsDlgProc(HWND hDlg, UINT message, WPARAM wParam,
  ...
  case WM_INITDIALOG: {
    ...
    ep->transitionSpin->LinkToEdit(GetDlgItem(hDlg, IDC_TRANSITION),
                                   EDITTYPE_INT);
    ...
    ep->stepsRenderSpin->LinkToEdit(GetDlgItem(hDlg, IDC_STEPS_RENDER),
                                    EDITTYPE_POS_INT);
    ...
    // Old
    EnableWindow(GetDlgItem(hDlg, IDC_STEPS),
                 !IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_STEPSSPINNER),
                 !IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_STEPS_RENDER),
                 !IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_STEPSRENDERSPINNER),
                 !IsDlgButtonChecked(hDlg, IDC_TILE_MODE));

    // New
    EnableWindow(GetDlgItem(hDlg, IDC_TILESTEPS),
                 IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_TILESTEPSSPINNER),
                 IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_KEEP_MAPPING),
                 IsDlgButtonChecked(hDlg, IDC_TILE_MODE));

    // New
    EnableWindow(GetDlgItem(hDlg, IDC_TRANSITION),
                 IsDlgButtonChecked(hDlg, IDC_TILE_MODE));
    EnableWindow(GetDlgItem(hDlg, IDC_TRANSITIONSPINNER),
                 IsDlgButtonChecked(hDlg, IDC_TILE_MODE));

    ep->weldSpin = GetISpinner(GetDlgItem(hDlg, IDC_THRESHSPINNER));
    ep->weldSpin->SetLimits(0, 999999, FALSE);
    ep->weldSpin->LinkToEdit(GetDlgItem(hDlg, IDC_WELDTHRESH),
                             EDITTYPE_UNIVERSE);
    ...
    ep->matSpin =
        SetupIntSpinner(hDlg, IDC_MAT_IDSPIN, IDC_MAT_ID, 1, MAX_MATID, 0);
    ep->tessUSpin = SetupIntSpinner(hDlg, IDC_TESS_U_SPIN, IDC_TESS_U2, 1,
                                    RPO_DEFAULT_TESSEL, 0);
    ep->tessVSpin = SetupIntSpinner(hDlg, IDC_TESS_V_SPIN, IDC_TESS_V2, 1,
                                    RPO_DEFAULT_TESSEL, 0);
    ep->tileNum =
        SetupIntSpinner(hDlg, IDC_TILE_MAT_SPIN, IDC_TILE_MAT, 0, 65535, 0);
    ep->tileRot =
        SetupIntSpinner(hDlg, IDC_TILE_ROT_SPIN, IDC_TILE_ROT, 0, 3, 0);
    ep->SetOpsDlgEnables();
  ...
  case WM_COMMAND:
    ...
    case IDC_SHOW_INTERIOR_FACES:
      ...
      //						ep->InvalidateMesh();
      //						ep->NotifyDependents(FOREVER,
      //PART_TOPO, REFMSG_CHANGE); 						ep->ip->RedrawViews
      //(ep->ip->GetTime(),REDRAW_END);
      break;
    case IDC_TILE_MODE: {
    ...
    case IDC_BIND:
      ...
      if (ep->ip->GetCommandMode() == ep->bindMode)
        ...
        ep->ip->SetCommandMode(ep->bindMode);
      break;
    case IDC_UNBIND:
      ep->DoRemoveHook();
      break;
    ...
    case IDC_EP_EXTRUDE:
      if (ep->ip->GetCommandMode() == ep->extrudeMode)
        ...
        ep->ip->SetCommandMode(ep->extrudeMode);
      break;
    case IDC_EP_BEVEL:
      if (ep->ip->GetCommandMode() == ep->bevelMode)
        ep->ip->SetStdCommandMode(CID_OBJMOVE);
      else
        ep->ip->SetCommandMode(ep->bevelMode);
      break;

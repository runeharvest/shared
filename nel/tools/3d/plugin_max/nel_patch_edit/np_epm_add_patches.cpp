...
static void AddPatches(int type, PatchMesh *patch, RPatchMesh *rpatch,
                       BOOL postWeld) {
  ...
  if (type == PATCH_TRI)
    MessageBox(NULL, _T("Rykol tools"), _T("C'est pas cool les tripatches..."),
               MB_OK | MB_ICONEXCLAMATION);

  // Add a patch of the desired type to each selected edge that doesn't have two
  // patches atatched!
  for (int i = 0; i < edges; ++i) {
    if (patch->edgeSel[i]) {
      ...
      int nFirstPatch = edge.patches.Count() > 0 ? edge.patches[0] : -1;
      if (edge.patches.Count() < 2 ||
          edge.patches[1] < 0) // less than 2 patches
#endif // (MAX_RELEASE < 4000)
      {
        ...
        patch->setNumVerts(verts + type - 2,
                           TRUE); // Add the appropriate number of verts
        rpatch->SetNumVerts(verts + type -
                            2); // And the appropriate vector count
        patch->setNumVecs(vecs + (type - 1) * 2 + type,
                          TRUE); // And the appropriate vector count
        ...
        Patch &spatch = patch->patches[nFirstPatch];
        Patch &dpatch = patch->patches[patches];
        switch (type) {
        case PATCH_TRI:
          dpatch.setVerts(edge.v2, edge.v1, verts);
          dpatch.setVecs(edge.vec21, edge.vec12, vecs, vecs + 1, vecs + 2,
                         vecs + 3);
          ...
          case PATCH_QUAD: { // Tri from Quad
            ...
            // Compute the new vert position
            Point3 otherCenter = (patch->verts[spatch.v[opposite1]].p +
                                  patch->verts[spatch.v[opposite2]].p) /
                                 2.0f;
...
void EditPatchMod::DoPatchAdd(int type) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (patch->edgeSel.NumberSet()) {
      ...
      if (theHold.Holding())
        theHold.Put(
            new PatchRestore(patchData, this, patch, rpatch, _T("DoPatchAdd")));

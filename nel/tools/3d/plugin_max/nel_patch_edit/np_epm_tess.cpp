...
INT_PTR CALLBACK AdvParametersDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam,
                                         LPARAM lParam);
...
void EditPatchMod::SetTessUI(HWND hDlg, TessApprox *tess) {
  ...
  if (tess->showInteriorFaces)
    ...
    CheckDlgButton(hDlg, IDC_SHOW_INTERIOR_FACES, FALSE);
  if (tileMode) {
  ...
  case TESS_SPATIAL:
    ...
    ShowWindow(GetDlgItem(hDlg, IDC_TESS_VIEW_DEP),
               settingViewportTess ? SW_HIDE : SW_SHOW);
...
BOOL CALLBACK PatchObjSurfDlgProc(HWND hDlg, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
  ...
  case WM_INITDIALOG: {
    ...
    if (!ep->settingViewportTess && ep->settingDisp &&
        ep->GetProdTess().type == TESS_SET)
    ...
    ep->uSpin =
        SetupIntSpinner(hDlg, IDC_TESS_U_SPINNER, IDC_TESS_U, 1, 100, t.u);
    ep->vSpin =
        SetupIntSpinner(hDlg, IDC_TESS_V_SPINNER, IDC_TESS_V, 1, 100, t.v);
#define MAX_F 1000.0f
    ep->edgeSpin = SetupFloatSpinner(hDlg, IDC_TESS_EDGE_SPINNER, IDC_TESS_EDGE,
                                     0.0f, MAX_F, t.edge);
    ep->distSpin = SetupFloatSpinner(hDlg, IDC_TESS_DIST_SPINNER, IDC_TESS_DIST,
                                     0.0f, MAX_F, t.dist);
    ep->angSpin = SetupFloatSpinner(hDlg, IDC_TESS_ANG_SPINNER, IDC_TESS_ANG,
                                    0.0f, MAX_F, t.ang);
    ep->mergeSpin = SetupFloatSpinner(hDlg, IDC_MERGE_SPINNER, IDC_MERGE,
                                      0.000f, MAX_F, t.merge);
    ep->SetTessUI(hDlg, &t);
  ...
  case WM_COMMAND:
    ...
    case IDC_TESS_VIEW: {
      ...
      ep->SetTessUI(hDlg, &t);
      EnableWindow(GetDlgItem(hDlg, IDC_TESS_VIEW_DEP),
                   FALSE); // always off here
      break;
    }
    ...
    case IDC_TESS_LDA: {
      ...
      switch (LOWORD(wParam)) {
        // watje 12-10-98
      case IDC_SHOW_INTERIOR_FACES:
        tess.showInteriorFaces =
            IsDlgButtonChecked(hDlg, IDC_SHOW_INTERIOR_FACES);
        break;
      case IDC_TESS_SET:
        tess.type = TESS_SET;
        EnableWindow(GetDlgItem(hDlg, IDC_SHOW_INTERIOR_FACES), TRUE);
    ...
    case IDC_ADVANCED_PARAMETERS: {
      ...
      int retval = DialogBox(hInstance, MAKEINTRESOURCE(IDD_SURF_APPROX_ADV),
...
#define MAX_SUBDIV 7
static BOOL initing =
    FALSE; // this is a hack but CenterWindow causes bad commands
INT_PTR CALLBACK AdvParametersDialogProc(HWND hDlg, UINT uMsg, WPARAM wParam,
                                         LPARAM lParam) {
  switch (uMsg) {
  case WM_INITDIALOG: {
    initing = TRUE;
    ...
    initing = FALSE;
    psMinSpin =
        SetupIntSpinner(hDlg, IDC_TESS_MIN_REC_SPINNER, IDC_TESS_MIN_REC, 0,
                        sParams.mMax, sParams.mMin);
    psMaxSpin =
        SetupIntSpinner(hDlg, IDC_TESS_MAX_REC_SPINNER, IDC_TESS_MAX_REC,
                        sParams.mMin, MAX_SUBDIV, sParams.mMax);
    psMaxTrisSpin =
        SetupIntSpinner(hDlg, IDC_TESS_MAX_TRIS_SPINNER, IDC_TESS_MAX_TRIS, 0,
                        2000000, sParams.mTris);

...
static BOOL CALLBACK DetachDialogProc(HWND hDlg, UINT message, WPARAM wParam,
                                      LPARAM lParam) {
  TCHAR tempName[256];
  switch (message) {
  case WM_INITDIALOG:
...
static int GetDetachOptions(IObjParam *ip, TSTR &newName) {
  ...
  if (DialogBox(hInstance, MAKEINTRESOURCE(IDD_DETACH), ip->GetMAXHWnd(),
                (DLGPROC)DetachDialogProc) == 1) {
    newName = detachName;
    return 1;
  }
  return 0;
...
void EditPatchMod::DoPatchDetach(int copy, int reorient) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (patch->patchSel.NumberSet()) {
      ...
      // We've deleted everything that wasn't selected -- Now add this to the
      // patch object accumulator

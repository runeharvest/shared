...
static int namedSetLevel[] = {EP_NS_VERTEX, EP_NS_VERTEX, EP_NS_EDGE,
                              EP_NS_PATCH};
static int namedClipLevel[] = {CLIP_P_VERT, CLIP_P_VERT, CLIP_P_EDGE,
                               CLIP_P_PATCH};
...
public:
  EPM_BindMouseProc(EditPatchMod *spl, IObjParam *i) {
    pobj = spl;
    ip = i;
  }
...
struct EPM_Mesh {
  EPM_Mesh(PatchMesh *pmesh, RPatchMesh *rmesh, class EditPatchData *patchData,
           INode *node, ModContext *mod, int mcListIndex) {
    PMesh = pmesh;
    RMesh = rmesh;
    PatchData = patchData;
    Node = node;
...
struct EPM_PaintTile {
  ...
  bool intersect(const Ray &ray, std::vector<EPM_Mesh> &vectMesh, TimeValue t,
                 NLMISC::CVector &hit);
  sint32 patch;
  sint32 tile;
  sint16 Mesh;
...
public:
  // Ctor
  CBackupValue() {}
  CBackupValue(const tileDesc &desc, uint mesh, uint tile)
      : Desc(desc), Mesh(mesh), Tile(tile) {}
...
protected:
  // Paint algorithm
  void SetTile(int mesh, int tile, const tileDesc &desc,
               std::vector<EPM_Mesh> &vectMesh, CLandscape *land,
               CNelPatchChanger &nelPatchChg,
               std::vector<CBackupValue> *backupStack, bool undo = true,
               bool updateDisplace = false);

  // Get a tile
  void GetTile(int mesh, int tile, tileDesc &desc,
               std::vector<EPM_Mesh> &vectMesh, CLandscape *land);

  bool PutATile(EPM_PaintTile *pTile, int tileSet, int curRotation,
                const NL3D::CTileBank &bank, bool selectCycle,
                std::set<EPM_PaintTile *> &visited,
                std::vector<EPM_Mesh> &vectMesh, NL3D::CLandscape *land,
  ...
  BOOL PutDisplace(int tile, int mesh, const CTileBank &bank,
                   std::vector<EPM_Mesh> &vectMesh, CLandscape *land,
                   int recurs, std::set<EPM_PaintTile *> &alreadyRecursed,
                   CNelPatchChanger &nelPatchChg);

  int selectTile(uint tileSet, bool selectCycle, bool _256, uint group,
                 const CTileBank &bank);
  bool GetBorderDesc(EPM_PaintTile *tile, tileSetIndex *pVoisinCorner,
                     NL3D::CTileSet::TFlagBorder pBorder[4][3],
                     tileDesc *pVoisinIndex, const NL3D::CTileBank &bank,
                     std::vector<EPM_Mesh> &vectMesh,
                     CNelPatchChanger &nelPatchChg);

  const NL3D::CTileSetTransition *
  FindTransition(int nTileSet, int nRotate,
                 const NL3D::CTileSet::TFlagBorder *border,
                 const NL3D::CTileBank &bank);
  bool PropagateBorder(EPM_PaintTile *tile, int curRotation, int curTileSet,
                       std::set<EPM_PaintTile *> &visited,
                       const NL3D::CTileBank &bank,
                       std::vector<EPM_Mesh> &vectMesh, NL3D::CLandscape *land,
                       CNelPatchChanger &nelPatchChg,
                       std::vector<CBackupValue> &backupStack,
                       bool recurseNoDiff = true);

  // Calc rotate path
  uint8 CalcRotPath(EPM_PaintTile *from, EPM_PaintTile *to, int depth,
                    int rotate, int &deltaX, int &deltaY, int &cost);

  // Just put a tile
  BOOL PutTile(int tile, int mesh, bool first, const NL3D::CTileBank &bank,
               int tileSet, std::vector<EPM_Mesh> &vectMesh,
               NL3D::CLandscape *land, int recurs,
               std::set<EPM_PaintTile *> &alreadyRecursed,
               CNelPatchChanger &nelPatchChg, bool _256);

  void RecursTile(EPM_PaintTile *pTile, const CTileBank &bank, int tileSet,
                  std::vector<EPM_Mesh> &vectMesh, CLandscape *land, int recurs,
                  std::set<EPM_PaintTile *> &alreadyRecursed, bool first,
                  int rotation, CNelPatchChanger &nelPatchChg, bool _256);

  BOOL HitATile(ViewExp *vpt, IPoint2 *p, int *tile, int *mesh, TimeValue t,
                std::vector<EPM_Mesh> &vectMesh, NLMISC::CVector &hit);
  BOOL HitATile(const CViewport &viewport, const CCamera &camera, float x,
                float y, int *tile, int *mesh, TimeValue t,
                std::vector<EPM_Mesh> &vectMesh, NLMISC::CVector &hit);
  BOOL AnyHits(ViewExp *vpt) { return vpt->NumSubObjHits(); }
...
public:
  EPM_ExtrudeMouseProc(EditPatchMod *o, IObjParam *i) : moveTrans(i) {
    po = o;
    ip = i;
  }
  int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m);
...
public:
  EPM_ExtrudeSelectionProcessor(EPM_ExtrudeMouseProc *mc, EditPatchMod *o,
                                IObjParam *i)
...
public:
  EPM_ExtrudeCMode(EditPatchMod *o, IObjParam *i)
      : fgProc((ReferenceTarget *)o), mouseProc(&eproc, o, i), eproc(o, i) {
    po = o;
  }
  int Class() { return MODIFY_COMMAND; }
...
public:
  EPM_BevelMouseProc(EditPatchMod *o, IObjParam *i) : moveTrans(i) {
    po = o;
    ip = i;
  }
  int proc(HWND hwnd, int msg, int point, int flags, IPoint2 m);
...
public:
  EPM_BevelSelectionProcessor(EPM_BevelMouseProc *mc, EditPatchMod *o,
                              IObjParam *i)
      : GenModSelectionProcessor(mc, (BaseObject *)o, i) {}
};
...
public:
  ...
  static BOOL rsSel, rsOps, rsSurf, rsTile,
      rsEdge; // rollup states (FALSE = rolled up)
  static IObjParam *ip;
  ...
  static BOOL
      settingDisp; // if we're doign renderer is it mesh or displacmenent
  ...
  void NotifyInputChanged(Interval changeInt, PartID partID, RefMessage message,
                          ModContext *mc);
  ...
  virtual void
  GetClassName(NL_GET_CLASS_NAME_PARAMS) NL_GET_CLASS_NAME_CONST NL_OVERRIDE {
    s = TSTR(_T("EditPatchNeLMod"));
  }
  ...
  void XFormVerts(XFormProc *xproc, TimeValue t, Matrix3 &partm,
                  Matrix3 &tmAxis);

  // Specialized xform for bezier handles
  void XFormHandles(XFormProc *xproc, TimeValue t, Matrix3 &partm,
                    Matrix3 &tmAxis, int object, int handleIndex);

  // Affine transform methods
  void Move(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis, Point3 &val,
            BOOL localOrigin = FALSE);
  void Rotate(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis, Quat &val,
              BOOL localOrigin = FALSE);
  void Scale(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis, Point3 &val,
             BOOL localOrigin = FALSE);

  // The following is called before the first Move(), Rotate() or Scale() call
  void TransformStart(TimeValue t);

  // The following is called after the user has completed the Move, Rotate or
  // Scale operation and the undo object has been accepted.
  void TransformFinish(TimeValue t);

  // The following is called when the transform operation is cancelled by a
  // right-click and the undo has been cancelled.
  ...
  void DoAddHook(PatchMesh *pMesh, int vert0, int vert1, int vert2, int seg,
                 int config);
  void DoRemoveHook();
  ...
  GET_OBJECT_NAME_CONST MCHAR *GetObjectName() {
    return GetString(IDS_TH_EDITPATCH);
  }
#else
  ...
  // Get the commonality of material index for the selection (-1 indicates no
  // commonality)
  int GetSelMatIndex();
  ...
  void BalanceSelPatch(int patch, int size, bool balanceU,
                       std::set<int> &visitedU, std::set<int> &visitedV,
                       RPatchMesh *rpatch, PatchMesh *);
  ...
  int DoAttach(INode *node, PatchMesh *attPatch, RPatchMesh *rattPatch,
               bool &canUndo);

  // Store current topology in the PatchObject
  void RecordTopologyTags();

  // Re-match named selection sets, etc. with changed topology (Call
  // RecordTopologyTags before making the changes to the shape, then call this)
...
class PickPatchAttach : public PickModeCallback, public PickNodeCallback {
public:
  EditPatchMod *ep;
...
const DWORD patchLevelDispFlags[] = {0, DISP_VERTTICKS | DISP_SELVERTS,
                                     DISP_SELEDGES, DISP_SELPATCHES, 0};

// For hit testing...
static int patchHitLevel[] = {0, SUBHIT_PATCH_VERTS | SUBHIT_PATCH_VECS,
                              SUBHIT_PATCH_EDGES, SUBHIT_PATCH_PATCHES, 0};
...
// General-purpose patch point table -- Maintains point table for each of n
// polygons
class PatchPointTab {
public:
...
public:
  ...
  PatchVertexDelta &operator=(PatchVertexDelta &from) {
    dtab = from.dtab;
    return *this;
  }
...
// Class for recording changes -- This is used to reconstruct an object from the
// original whenever the modifier is re-entered or whenever the system needs to
// reconstruct an object's cache.  This may be slow if a lot of changes have
// been recorded, but it's about the only way to properly reconstruct an object
// because the output of one operation becomes the input of the next.

// These are used as follows:
// When a user makes a modification to an object, a StartChangeGroup call needs
// to be made to the EditPatchData object.  Then a change record needs to be
// added for each sub-operation that makes up the modification.  These records
// are owned by the EditPatchData object, but they should also be referenced by
// the undo object for that operation.  If an undo is done, ownership of the
// modification record transfers to the undo/redo object and the record is
// REMOVED (NOT DELETED) from the EditPatchData object.  This keeps the record
// around for a redo operation but removes it from the list of records for the
// modifier.  If the undo is redone, ownership transfers back to the modifier,
// when it is re-added to the modification record list.

// Note that this class contains load and save methods, necessary because the
// modifier needs to be able to save and load them.  When you subclass off of
// this, be sure your load and save methods call the base class's first!
...
public:
  PatchVertexDelta
      delta; // Position changes for each vertex (Wasteful!  Change later?)
  BOOL Redo(PatchMesh *patch, RPatchMesh *rpatch, int reRecord);
  IOResult Load(ILoad *iload);
};
...
public:
  ...
  EPMapVert(int v, Point3 &o, Point3 &d) {
    vert = v;
    original = o;
    delta = d;
    originalStored = TRUE;
...
public:
  ...
  // Update the deltas we have stored, if necessary and apply to output patch
  // mesh. This is in response to the original shape changing
...
public:
  ...
  void SetHandleFlag(int vert) {
    handleVert = vert;
    handleFlag = TRUE;
  }
  BOOL DoingHandles() { return handleFlag; }
  void ApplyHandlesAndZero(PatchMesh &patch) {
    vdelta.ApplyHandlesAndZero(patch, handleVert);
  }
  void RescaleWorldUnits(float f);

  // MAXr3: New recording system
  void RecordTopologyTags(PatchMesh *patch);
  void UpdateChanges(PatchMesh *patch, RPatchMesh *rpatch,
                     BOOL checkTopology = TRUE);

  // Named selection set access
  GenericNamedSelSetList &
  GetSelSet(EditPatchMod
                *mod); // Get the one for the current subobject selection level
  GenericNamedSelSetList &GetSelSet(
      int level); // Get the one for the specified subobject selection level
...
public:
  ...
  PatchRestore(EditPatchData *pd, EditPatchMod *mod, PatchMesh *patch,
               RPatchMesh *rpatch, TCHAR *id = _T(""));

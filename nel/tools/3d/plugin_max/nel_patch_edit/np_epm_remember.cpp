...
int EditPatchMod::RememberPatchThere(HWND hWnd, IPoint2 m) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    patch->SubObjectHitTest(gw, gw->getMaterial(), &hr,
                            SUBHIT_PATCH_PATCHES /* | HIT_ABORTONHIT*/,
                            hitList);
    ...
    if (hit) {
      ...
      while (hit) {
        if (patch->patchSel[hit->index]) {
          if (patch->SelPatchesSameType()) {
            rememberedPatch = NULL;
            rememberedData =
                patch->patches[hit->index].flags & (~PATCH_INTERIOR_MASK);
            goto finish;
          }
      ...
      rememberedData =
          patch->patches[rememberedIndex].flags & (~PATCH_INTERIOR_MASK);
    }
...
int EditPatchMod::RememberVertThere(HWND hWnd, IPoint2 m) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    gw->setTransform(mat);
    patch->SubObjectHitTest(gw, gw->getMaterial(), &hr,
                            SUBHIT_PATCH_VERTS /* | HIT_ABORTONHIT*/, hitList);
    PatchSubHitRec *hit = hitList.First();
    if (hit) {
      result = 1;
      // Go thru the list and see if we have one that's selected
      ...
      while (hit) {
        if (patch->vertSel[hit->index]) {
          if (patch->SelVertsSameType()) {
            rememberedPatch = NULL;
            rememberedData =
                patch->verts[hit->index].flags & (~PVERT_TYPE_MASK);
            goto finish;
          }

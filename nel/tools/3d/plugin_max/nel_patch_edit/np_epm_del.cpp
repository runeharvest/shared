...
BOOL PatchDeleteRecord::Redo(PatchMesh *patch, RPatchMesh *rpatch,
                             int reRecord) {
  if (reRecord) {
    oldPatch = *patch;
    roldPatch = *rpatch;
      ...
      for (int i = 0; i < edges; ++i) {
        if (patch->edgeSel[i]) {
          ...
          for (int pn = 0; pn < patch->edges[i].patches.Count(); ++pn)
            delPatches.Set(patch->edges[i].patches[pn]);
...
// Deletes any vertices tagged, also any patches tagged.  Automatically deletes
// the vectors that are deleted as a result of the patch deletion and sweeps any
// vertices floating in space.
...
void DeletePatchParts(PatchMesh *patch, RPatchMesh *rpatch, BitArray &delVerts,
  ...
  // We treat vectors specially in order to clean up after welds.  First, we tag
  // 'em all, then untag only those on unselected patches so that any dangling
  ...
  for (int chan = -NUM_HIDDENMAPS; chan < patch->getNumMaps(); ++chan) {
    ...
    if (tverts && patch->mapPatches(chan)) {
      ...
      if (newTVerts)
        memcpy(patch->mapVerts(chan), newTVertArray,
               newTVerts * sizeof(PatchTVert));
      ...
      if (newTVPatches)
        memcpy(patch->mapPatches(chan), newArray,
               newTVPatches * sizeof(TVPatch));
      delete[] newArray;
    }
  }

...
void EditPatchMod::SetNamedSelSetName(int index, TSTR &newName) {
  ...
  if (theHold.Holding())
    theHold.Put(
        new EPSelSetNameRestore(this, namedSel[selLevel - 1][index], newName));
...
static INT_PTR CALLBACK PickSetNameDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                           LPARAM lParam) {
  static TSTR *name;
...
BOOL EditPatchMod::GetUniqueSetName(TSTR &name) {
  while (1) {
    ...
    if (!DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_PASTE_NAMEDSET),
                        ip->GetMAXHWnd(), PickSetNameDlgProc, (LPARAM)&name))
      return FALSE;
  }
  return TRUE;
}

static INT_PTR CALLBACK PickSetDlgProc(HWND hWnd, UINT msg, WPARAM wParam,
                                       LPARAM lParam) {
  switch (msg) {
  case WM_INITDIALOG: {
...
int EditPatchMod::SelectNamedSet() {
  ...
  return DialogBoxParam(hInstance, MAKEINTRESOURCE(IDD_SEL_NAMEDSET),
                        ip->GetMAXHWnd(), PickSetDlgProc, (LPARAM)&names);
}
...
// Old MAX files (pre-r3) have EditPatchData named selections without names
// assigned.  This assigns them their proper names for r3 and later code.  If no
// fixup is required, this does nothing.
void EditPatchMod::MaybeFixupNamedSels() {
  int i;
  if (!ip)
    return;

  // Go thru the modifier contexts, and stuff the named selection names into the
  // EditPatchData
  ModContextList mcList;
  INodeTab nodes;
  ...
  for (i = 0; i < mcList.Count(); i++) {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      GenericNamedSelSetList &pdSel = patchData->GetSelSet(j);
      // Some old files may have improper counts in the EditPatchData.  Limit
      // the counter
      int mc = mSel.Count();
      int pdc = pdSel.Count();
...
void EditPatchMod::XFormHandles(XFormProc *xproc, TimeValue t, Matrix3 &partm,
  ...
  // Create a change record for this object and store a pointer to its delta
  // info in this EditPatchData
  if (!TestAFlag(A_HELD)) {
    patchData->vdelta.SetSize(*patch, FALSE);
    if (theHold.Holding()) {
      theHold.Put(
          new PatchRestore(patchData, this, patch, rpatch, _T("XFormHandles")));
    }
  ...
  if (primaryKnot >= 0) {
    ...
    if (lockedHandles) {
      ...
      if ((vert.flags & PVERT_COPLANAR) && (vert.vectors.Count() > 2) &&
          !(newpt == theKnot)) {
      locked_handles:
          ...
          if (newNorm == -oldNorm) {
            for (int v = 0; v < vectors; ++v) {
              ...
              if (theVec != handleIndex) {
                Point3 testVec =
                    patch->vecs[theVec].p - pDeltas[theVec] - theKnot;
...
void EditPatchMod::XFormVerts(XFormProc *xproc, TimeValue t, Matrix3 &partm,
                              Matrix3 &tmAxis) {
  ModContextList mcList;
  INodeTab nodes;
  Matrix3 mat, imat;
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (!TestAFlag(A_HELD)) {
      patchData->vdelta.SetSize(*patch, FALSE);
      if (theHold.Holding()) {
        // Hulud: here, i pass a NULL pointer because rpatch are not modified by
        // xform
        theHold.Put(
            new PatchRestore(patchData, this, patch, NULL, _T("XFormVerts")));
      }
      patchData->vdelta.Zero(); // Reset all deltas
      patchData->ClearHandleFlag();
      wasBroken = FALSE;
      ...
      if (wasBroken)
        shiftPressed = TRUE;
      if (patchData->DoingHandles())
        patchData->ApplyHandlesAndZero(
            *patch); // Reapply the slave handle deltas
      else
        patchData->vdelta.Zero();
    }
    ...
    if (numAxis == NUMAXIS_INDIVIDUAL) {
      switch (selLevel) {
      case EP_VERTEX: {
        // Selected vertices - either directly or indirectly through selected
        // faces or edges.
        ...
        patch->computeInteriors(); // Kind of broad-spectrum -- only need to
                                   // recompute affected patches
      } break;
      case EP_EDGE:
      case EP_PATCH: {
        // Selected vertices - either directly or indirectly through selected
    ...
    patchData->UpdateChanges(patch,
                             NULL); // Hulud: here, i pass a NULL pointer
                                    // because rpatch are not modified by xform
    patchData->TempData(this)->Invalidate(PART_GEOM);
    patchData->SetFlag(EPD_BEENDONE, TRUE);
  }
...
void EditPatchMod::Move(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis,
                        Point3 &val, BOOL localOrigin) {
  MoveXForm proc(val);
  XFormVerts(&proc, t, partm, tmAxis);
}

void EditPatchMod::Rotate(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis,
                          Quat &val, BOOL localOrigin) {
  RotateXForm proc(val);
  XFormVerts(&proc, t, partm, tmAxis);
}

void EditPatchMod::Scale(TimeValue t, Matrix3 &partm, Matrix3 &tmAxis,
                         Point3 &val, BOOL localOrigin) {
  ScaleXForm proc(val);
  XFormVerts(&proc, t, partm, tmAxis);
}
...
void EditPatchMod::TransformFinish(TimeValue t) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    RPatchMesh *rpatch;
    PatchMesh *patch =
        patchData->TempData(this)->GetPatch(ip->GetTime(), rpatch);
    if (!patch)
      continue;
...
void EditPatchMod::ModifyObject(TimeValue t, ModContext &mc, ObjectState *os,
                                INode *node) {
  ...
  if (!mc.localData) {
    ...
    // 3-18-99 to suport render steps and removal of the mental tesselator
    meshStepsRender = patchData->meshStepsRender =
        patchOb->GetMeshStepsRender();
    showInterior = patchData->showInterior = patchOb->GetShowInterior();

    //		meshAdaptive = patchData->meshAdaptive = patchOb->GetAdaptive();
    //// Future use (Not used now)
    tileLevel = patchData->tileLevel = patchOb->rpatch->rTess.TileTesselLevel;
    transitionType = patchData->transitionType =
        patchOb->rpatch->rTess.TransitionType;
    ...
    mViewTessNormals = patchData->mViewTessNormals =
        patchOb->GetViewTessNormals();
    mProdTessNormals = patchData->mProdTessNormals =
        patchOb->GetProdTessNormals();
...
void EditPatchMod::NotifyInputChanged(Interval changeInt, PartID partID,
...
void EditPatchMod::SetPropagate(BOOL sw) { propagate = sw; }
...
void EditPatchMod::GetSubObjectTMs(SubObjAxisCallback *cb, TimeValue t,
  ...
  if (mc->localData) {
    ...
    RPatchMesh *rpatch;
    PatchMesh *patch = patchData->TempData(this)->GetPatch(t, rpatch);
    // Watch out -- The system can call us even if we didn't get a valid patch
    // object
    if (!patch)
      return;
...
void EditPatchMod::GetSubObjectCenters(SubObjAxisCallback *cb, TimeValue t,
                                       INode *node, ModContext *mc) {
  Interval valid;

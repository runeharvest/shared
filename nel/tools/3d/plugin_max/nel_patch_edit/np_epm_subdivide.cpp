...
void DeletePatchParts(PatchMesh *patch, RPatchMesh *rpatch, BitArray &delVerts,
                      BitArray &delPatches);
...
static Point3 GetOuterInside(Point3 a, Point3 b, Point3 c, Point3 d, Point3 e,
                             Point3 f) {
...
static Point3 GetNewEdgeVec(Point3 a, Point3 b, Point3 c, Point3 d, Point3 e,
                            Point3 f, Point3 g, Point3 h) {
  return a * _1_16 + b * _1_16 + c * _3_16 + d * _3_16 + e * _1_16 + f * _1_16 +
         g * _3_16 + h * _3_16;
}

// --------------------------------------------------------------------------------

static Point3 GetCentralInterior(Point3 a, Point3 b, Point3 c, Point3 d,
                                 Point3 e, Point3 f, Point3 g, Point3 h,
                                 Point3 i, Point3 j) {
  return a * _1_16 + b * _1_8 + c * _1_16 + d * _1_16 + e * _1_16 + f * _1_16 +
         g * _1_16 + h * _3_16 + i * _3_16 + j * _1_8;
}

// --------------------------------------------------------------------------------

static Point3 GetNewEdgeCenter(Point3 a, Point3 b, Point3 c, Point3 d, Point3 e,
                               Point3 f, Point3 g, Point3 h, Point3 i) {
  return a * _1_16 + b * _1_8 + c * _1_16 + d * _1_8 + e * _1_16 + f * _1_16 +
         g * _1_4 + h * _1_8 + i * _1_8;
}
...
static Point3 InterpCenter(PatchMesh *patch, int index, int e1, int i1, int i2,
                           int e2, Point3 *v1 = NULL, Point3 *v2 = NULL,
...
static Point3 InterpCenter(PatchMesh *patch, int index, int e1, int i1, int e2,
...
static Point3 InterpCenter(Point3 e1, Point3 i1, Point3 e2, Point3 *v1 = NULL,
                           Point3 *v2 = NULL) {
  Point3 a = (e1 + i1) / 2.0f;
  Point3 b = (i1 + e2) / 2.0f;
  if (v1)
...
static Point3 InterpEdge(PatchMesh *patch, int index, float pct, int c1, int e1,
                         int e2, int c2, Point3 *v1 = NULL, Point3 *v2 = NULL,
                         Point3 *v3 = NULL, Point3 *v4 = NULL) {
...
static void FindNewTriEdge(PatchMesh *patch, Patch &p, int vert, Point3 &e1,
                           Point3 &e2, Point3 &e3) {
  int a = vert;
  int b = vert * 3;
  int c = b + 1;
...
static void FindNewOuterTriInteriors(PatchMesh *patch, Patch &p, int vert,
                                     Point3 &i1, Point3 &i2, Point3 &i3) {
  int a = vert;
  int b = vert * 3;
  int c = (b + 8) % 9;
  int d = (b + 7) % 9;
...
static void FindNewInnerTriInteriors(PatchMesh *patch, Patch &p, Point3 &i1,
                                     Point3 &i2, Point3 &i3) {
  Point3 pa = p.aux[0];
  Point3 pb = p.aux[1];
  Point3 pc = p.aux[2];
...
// This is a first shot at a degree reducer which turns a degree-4 curve into a
// degree-3 curve, it probably won't give very good results unless the curve was
// converted from degree 3 to degree 4 returns just the vector points
static void CubicFromQuartic(Point3 q1, Point3 q2, Point3 q3, Point3 q4,
                             Point3 q5, Point3 &c2, Point3 &c3) {
...
void SubdividePatch(int type, BOOL propagate, PatchMesh *patch,
                    RPatchMesh *rpatch) {
  int i;
  ...
  // Tag the edges that are on tagged patches but aren't tagged (only happens in
  // propagate=0) And set up a table with useful division info
  ...
  int newVecs = divEdges * 4 +
                newInteriors; // 4 new vectors per edge + new interior verts
  ...
  for (i = 0; i < edges; ++i) {
    ...
    DebugPrint("Old edge: %d  New edge: %d (%d %d) %d (%d %d) %d\n", e.oldEdge,
               e.v1, e.vec12, e.vec21, e.v2, e.vec23, e.vec32, e.v3);
  }
#endif
      ...
      if (p.type == PATCH_TRI) {
        ...
        for (chan = 0; chan < patch->getNumMaps(); ++chan) {
          if (patch->tvPatches[chan]) {
            ...
            tp4.tv[2] = tvc;
#if MAX_RELEASE <= 3100
            patch->tVerts[chan][tva] = (patch->tVerts[chan][tp.tv[0]] +
                                        patch->tVerts[chan][tp.tv[1]]) /
                                       2.0f;
            patch->tVerts[chan][tvb] = (patch->tVerts[chan][tp.tv[1]] +
                                        patch->tVerts[chan][tp.tv[2]]) /
                                       2.0f;
            patch->tVerts[chan][tvc] = (patch->tVerts[chan][tp.tv[2]] +
                                        patch->tVerts[chan][tp.tv[0]]) /
                                       2.0f;
#else
            patch->tVerts[chan][tva] =
                ((UVVert &)patch->tVerts[chan][tp.tv[0]] +
                 (UVVert &)patch->tVerts[chan][tp.tv[1]]) /
                2.0f;
            patch->tVerts[chan][tvb] =
                ((UVVert &)patch->tVerts[chan][tp.tv[1]] +
                 (UVVert &)patch->tVerts[chan][tp.tv[2]]) /
                2.0f;
            patch->tVerts[chan][tvc] =
                ((UVVert &)patch->tVerts[chan][tp.tv[2]] +
                 (UVVert &)patch->tVerts[chan][tp.tv[0]]) /
                2.0f;
#endif
        ...
        // Now we'll compute the vectors for the three new edges being created
        // inside this patch These come back as degree 4's, and we need to
        // reduce them to degree 3 for use in our edges -- This is a bit risky
        // because we aren't guaranteed a perfect fit.
        ...
        CubicFromQuartic(patch->verts[e2.v2].p, i1, i2, i3,
                         patch->verts[e0.v2].p, v1, v2);
        CubicFromQuartic(patch->verts[e0.v2].p, i4, i5, i6,
                         patch->verts[e1.v2].p, v3, v4);
        CubicFromQuartic(patch->verts[e1.v2].p, i7, i8, i9,
                         patch->verts[e2.v2].p, v5, v6);
        ...
        // Now compute the interior vectors for the new patches if the one we're
        // dividing isn't automatic Must compute vectors for this patch's
        // divided edges
        if (!(p.flags & PATCH_AUTO)) {
          ...
          FindNewOuterTriInteriors(patch, p, 1, patch->vecs[p1.interior[1]].p,
                                   patch->vecs[p1.interior[2]].p,
                                   patch->vecs[p1.interior[0]].p);
          FindNewOuterTriInteriors(patch, p, 2, patch->vecs[p2.interior[1]].p,
                                   patch->vecs[p2.interior[2]].p,
                                   patch->vecs[p2.interior[0]].p);
          FindNewInnerTriInteriors(patch, p, patch->vecs[p3.interior[0]].p,
                                   patch->vecs[p3.interior[1]].p,
                                   patch->vecs[p3.interior[2]].p);
          FindNewOuterTriInteriors(patch, p, 0, patch->vecs[p4.interior[0]].p,
                                   patch->vecs[p4.interior[1]].p,
                                   patch->vecs[p4.interior[2]].p);
        }
      } else { // Quad patch
        ...
        if (pi.div02 && pi.div13) {
          ...
          InterpCenter(new0, w1, w2, new2, &patch->vecs[newev1].p,
                       &patch->vecs[newev2].p, &patch->vecs[newev6].p,
                       &patch->vecs[newev5].p);
          InterpCenter(new1, w3, w4, new3, &patch->vecs[newev3].p,
                       &patch->vecs[newev4].p, &patch->vecs[newev8].p,
                       &patch->vecs[newev7].p);
...
#define RPO_REMAR_V2(a, b, c) (a)
          ...
          for (chan = 0; chan < patch->getNumMaps(); ++chan) {
            if (patch->tvPatches[chan]) {
              ...
              patch->tVerts[chan][tvb] = (patch->tVerts[chan][tp.tv[1]] +
                                          patch->tVerts[chan][tp.tv[2]]) /
                                         2.0f;
              patch->tVerts[chan][tvc] = (patch->tVerts[chan][tp.tv[3]] +
                                          patch->tVerts[chan][tp.tv[0]]) /
                                         2.0f;
              patch->tVerts[chan][tve] = (patch->tVerts[chan][tp.tv[0]] +
                                          patch->tVerts[chan][tp.tv[1]] +
                                          patch->tVerts[chan][tp.tv[2]] +
                                          patch->tVerts[chan][tp.tv[3]]) /
                                         4.0f;
#else
              patch->tVerts[chan][tva] =
                  ((UVVert &)patch->tVerts[chan][tp.tv[0]] +
              ...
              patch->tVerts[chan][tvb] =
                  ...
                  2.0f;
              patch->tVerts[chan][tvc] =
                  ((UVVert &)patch->tVerts[chan][tp.tv[3]] +
                   (UVVert &)patch->tVerts[chan][tp.tv[0]]) /
                  2.0f;
              patch->tVerts[chan][tve] =
                  ((UVVert &)patch->tVerts[chan][tp.tv[0]] +
                   (UVVert &)patch->tVerts[chan][tp.tv[1]] +
                   patch->tVerts[chan][tp.tv[2]] +
                   patch->tVerts[chan][tp.tv[3]]) /
          ...
          rpatch->Subdivide(i, e0.v2, e1.v2, e2.v2, e3.v2, center, pat - 4,
                            *patch);
        } else if (pi.div02) {
          // Divide edges 0 & 2
          ...
          for (chan = 0; chan < patch->getNumMaps(); ++chan) {
            if (patch->tvPatches[chan]) {
              ...
              patch->tVerts[chan][tva] = (patch->tVerts[chan][tp.tv[0]] +
                                          patch->tVerts[chan][tp.tv[1]]) /
                                         2.0f;
              patch->tVerts[chan][tvb] = (patch->tVerts[chan][tp.tv[2]] +
                                          patch->tVerts[chan][tp.tv[3]]) /
                                         2.0f;
#else
              patch->tVerts[chan][tva] =
...
#if MAX_RELEASE <= 3100
              patch->tVerts[chan][tva] = (patch->tVerts[chan][tp.tv[1]] +
                                          patch->tVerts[chan][tp.tv[2]]) /
                                         2.0f;
              patch->tVerts[chan][tvb] = (patch->tVerts[chan][tp.tv[0]] +
                                          patch->tVerts[chan][tp.tv[3]]) /
                                         2.0f;
#else
              patch->tVerts[chan][tva] =

...
BOOL EPM_BindMouseProc::HitTest(ViewExp *vpt, IPoint2 *p, int type, int flags,
                                int subType) {
...
int EPM_BindMouseProc::proc(HWND hwnd, int msg, int point, int flags,
                            IPoint2 m) {
  ...
  case MOUSE_POINT:
    ...
    case 1:
      ...
      //					if(HitAnEndpoint(vpt, &m,
      //shape1, poly1, vert1, NULL, &poly2, &vert2)) 						ss->DoVertConnect(vpt,
      //shape1, poly1, vert1, poly2, vert2);
      if (HitASegment(vpt, &m, &seg1)) {
        ...
        int v0, v1, v2, v3;
        switch (
            CheckBind(vert1, seg1, v0, v1, v2, v3, tab, *pMesh, false, true)) {
        case 0:
  ...
  case MOUSE_MOVE:
    ...
    if (HitASegment(vpt, &m, &seg1)) {
      ...
      int v0, v1, v2, v3;
      if (CheckBind(vert1, seg1, v0, v1, v2, v3, tab, *pMesh, false, true) !=
          -1)
        SetCursor(LoadCursor(hInstance, MAKEINTRESOURCE(IDC_TH_SELCURSOR)));
      else
  ...
  case MOUSE_FREEMOVE:
    ...
    /*
       ...
       MAKEINTRESOURCE(IDC_TH_SELCURSOR)));
                        }
                    }
                SetCursor(LoadCursor(NULL, IDC_ARROW));
    */

...
int EditPatchMod::DoAttach(INode *node, PatchMesh *attPatch,
                           RPatchMesh *rattPatch, bool &canUndo) {
      ...
      for (int chan = -NUM_HIDDENMAPS; chan < patch->getNumMaps(); ++chan) {
        if (chan < attPatch->getNumMaps() && attPatch->mapPatches(chan)) {
          ...
          for (j = 0; j < ct; j++) {
            ...
            attPatch->mapPatches(chan)[i].interiors[j] =
                tvInteriors[(ct) - (j)-1];
            int index = ct - j - 1;
            int a;
            int b;
  ...
  if (theHold.Holding())
    theHold.Put(
        new PatchRestore(patchData, this, patch, rpatch, _T("DoAttach")));

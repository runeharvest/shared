...
void PatchRightMenu::Init(RightClickMenuManager *manager, HWND hWnd,
                          IPoint2 m) {
  ...
  case EP_VERTEX:
    if (ep->RememberVertThere(hWnd, m)) {
      ...
      manager->AddMenu(this, flags1, PVERT_COPLANAR,
                       GetString(IDS_TH_COPLANAR));
  ...
  case EP_PATCH:
    if (ep->RememberPatchThere(hWnd, m)) {
      ...
      manager->AddMenu(this, MF_SEPARATOR, 0, NULL);
      manager->AddMenu(this, flags1, PATCH_AUTO,
                       GetString(IDS_TH_AUTOINTERIOR));
...
int EditPatchMod::HitTest(TimeValue t, INode *inode, int type, int crossing,
  ...
  if (mc->localData) {
    ...
    if (selLevel != EP_TILE) {
      res = patch->SubObjectHitTest(
          gw, gw->getMaterial(), &hr,
          flags | ((patchHitOverride) ? patchHitLevel[patchHitOverride]
                                      : patchHitLevel[selLevel]),
          hitList);
    } else {
      res = rpatch->SubObjectHitTest(
          gw, gw->getMaterial(), &hr,
          flags | ((patchHitOverride) ? patchHitLevel[patchHitOverride]
                                      : patchHitLevel[selLevel]),
          hitList, t, *patch);
    ...
    rec = hitList.First();
    while (rec) {
      vpt->LogHit(inode, mc, rec->dist, 123456,
                  new PatchHitData(rec->patch, rec->index, rec->type));
      rec = rec->Next();
    }
  }

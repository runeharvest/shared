...
void EditPatchMod::BeginEditParams(IObjParam *ip, ULONG flags,
                                   Animatable *prev) {
  this->ip = ip;
  ...
  hSelectPanel = ip->AddRollupPage(
      hInstance, MAKEINTRESOURCE(IDD_EDPATCH_SELECT), PatchSelectDlgProc,
      GetString(IDS_TH_SELECTION), (LPARAM)this, rsSel ? 0 : APPENDROLL_CLOSED);
  hOpsPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_EDPATCH_OPS),
                                PatchOpsDlgProc, GetString(IDS_TH_GEOMETRY),
                                (LPARAM)this, rsOps ? 0 : APPENDROLL_CLOSED);
  /* watje 3 - 18 - 99
...
*/
  if (selLevel == EP_PATCH) {
    hSurfPanel =
        ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_EDPATCH_SURF),
                          PatchSurfDlgProc, GetString(IDS_TH_SURFACEPROPERTIES),
                          (LPARAM)this, rsSurf ? 0 : APPENDROLL_CLOSED);
  } else
    hSurfPanel = NULL;
...
void EditPatchMod::EndEditParams(IObjParam *ip, ULONG flags, Animatable *next) {
  ...
  //	if ( ip->GetCommandMode()->ID() == CID_EP_EXTRUDE )
  //ip->SetStdCommandMode( CID_OBJMOVE ); 	if ( ip->GetCommandMode()->ID() ==
  //CID_EP_BEVEL ) ip->SetStdCommandMode( CID_OBJMOVE );
...
int EditPatchMod::NumSubObjTypes() { return 4; }
...
EditPatchMod::~EditPatchMod() { ClearSetNames(); }

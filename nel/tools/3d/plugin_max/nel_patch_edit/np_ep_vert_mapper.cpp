...
void EPVertMapper::RecomputeDeltas(PatchMesh &patch) {
  ...
  for (i = 0; i < verts; ++i) {
    ...
    if (map.vert >= 0 && map.originalStored) {
      ...
      if (map.delta != oldDelta)
        DebugPrint(
            "Vert %d delta changed from %.2f %.2f %.2f to %.2 %.2f %.2f\n", i,
            oldDelta.x, oldDelta.y, oldDelta.z, map.delta.x, map.delta.y,
            map.delta.z);
#endif
    }
  }
  for (i = 0; i < vecs; ++i) {
    ...
    if (map.vert >= 0 && map.originalStored) {
      ...
      if (map.delta != oldDelta)
        DebugPrint(
            "Vec %d delta changed from %.2f %.2f %.2f to %.2 %.2f %.2f\n", i,
            oldDelta.x, oldDelta.y, oldDelta.z, map.delta.x, map.delta.y,
            map.delta.z);
#endif
    }
...
void EPVertMapper::UpdateAndApplyDeltas(PatchMesh &inPatch,
                                        PatchMesh &outPatch) {
  ...
  for (i = 0; i < verts; ++i) {
    ...
    if (pv.vert >= 0 && pv.originalStored) {
      ...
      // watje 4-27-99 instead just throwing an nlassert it pops a message box
      // up and troes to recover
      if (!(pv.vert >= 0 && pv.vert < outPatch.numVerts)) {
        ...
        MessageBox(GetCOREInterface()->GetMAXHWnd(), warning, title,
                   MB_OK | MB_APPLMODAL);
      }

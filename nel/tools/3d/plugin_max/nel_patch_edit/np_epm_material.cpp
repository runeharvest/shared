...
void EditPatchMod::SetSelTess(int nU, int nV) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    if (theHold.Holding()) {
      theHold.Put(
          new PatchRestore(patchData, this, patch, rpatch, _T("SetSelTess")));
    }
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    for (int j = 0; j < patch->getNumEdges(); j++) {
      ...
      if (patch->edgeSel[j]) {
        ...
        bool edgeFlag = ((p0 != -1) && (rpatch->getUIPatch(p0).getEdgeFlag(
                                           WhereIsTheEdge(p0, j, *patch)))) ||
                        ((p1 != -1) && (rpatch->getUIPatch(p1).getEdgeFlag(
                                           WhereIsTheEdge(p1, j, *patch))));
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    for (int j = 0; j < patch->getNumEdges(); j++) {
      ...
      if (patch->edgeSel[j]) {
        ...
        if (p0 != -1) {
          // Set the flag
          rpatch->getUIPatch(p0).setEdgeFlag(WhereIsTheEdge(p0, j, *patch),
                                             smooth);
        ...
        if (p1 != -1) {
          // Set the flag
          rpatch->getUIPatch(p1).setEdgeFlag(WhereIsTheEdge(p1, j, *patch),
                                             smooth);
        ...
        for (int pe = 0; pe < patch->edges[j].patches.Count(); ++pe) {
          ...
          // Set the flag
          rpatch->getUIPatch(pn).setEdgeFlag(WhereIsTheEdge(pn, j, *patch),
                                             smooth);
...
void EditPatchMod::BalanceSelPatch(int patch, int size, bool balanceU,
                                   std::set<int> &visitedU,
                                   std::set<int> &visitedV, RPatchMesh *rpatch,
                                   PatchMesh *ppatch) {
  // already visited ?
  if ((balanceU && (visitedU.find(patch) != visitedU.end())) ||
      ((!balanceU) && (visitedV.find(patch) != visitedV.end())))
    return;
...
#if (MAX_RELEASE < 4000)
    int nOtherPatch = (ppatch->edges[nEdge].patch1 == patch)
                          ? ppatch->edges[nEdge].patch2
                          : ppatch->edges[nEdge].patch1;
#else  // (MAX_RELEASE < 4000)
    int nOtherPatch = (ppatch->edges[nEdge].patches.Count() > 0 &&
                       ppatch->edges[nEdge].patches[0] == patch)
                          ? (ppatch->edges[nEdge].patches.Count() > 1
                                 ? ppatch->edges[nEdge].patches[1]
                                 : -1)
                          : (ppatch->edges[nEdge].patches.Count() > 0
                                 ? ppatch->edges[nEdge].patches[0]
                                 : -1);
#endif // (MAX_RELEASE < 4000)
    if (nOtherPatch >= 0) {
      int nNeiborEdge = WhereIsTheEdge(nOtherPatch, nEdge, *ppatch);
      BalanceSelPatch(nOtherPatch, (i & 1) ? nU : nV, (nNeiborEdge & 1) != 0,
                      visitedU, visitedV, rpatch, ppatch);
    }
  }
}
...
#if (MAX_RELEASE < 4000)
          int nOtherPatch = (patch->edges[nEdge].patch1 == j)
                                ? patch->edges[nEdge].patch2
                                : patch->edges[nEdge].patch1;
#else  // (MAX_RELEASE < 4000)
          int nOtherPatch = (patch->edges[nEdge].patches.Count() > 0 &&
                             patch->edges[nEdge].patches[0] == j)
                                ? (patch->edges[nEdge].patches.Count() > 1
                                       ? patch->edges[nEdge].patches[1]
                                       : -1)
                                : (patch->edges[nEdge].patches.Count() > 0
                                       ? patch->edges[nEdge].patches[0]
                                       : -1);
#endif // (MAX_RELEASE < 4000)
          if (nOtherPatch >= 0) {
            int nNeiborEdge = WhereIsTheEdge(nOtherPatch, nEdge, *patch);
            BalanceSelPatch(nOtherPatch, (i & 1) ? nU : nV,
                            (nNeiborEdge & 1) != 0, visitedU, visitedV, rpatch,
                            patch);
          }
        }
      }

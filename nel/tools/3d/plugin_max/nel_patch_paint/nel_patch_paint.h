...
inline int getScriptAppDataPatchMesh(Animatable *node, uint32 id, int def) {
  // Get the chunk
  AppDataChunk *ap =
      node->GetAppDataChunk(MAXSCRIPT_UTILITY_CLASS_ID, UTILITY_CLASS_ID, id);
...
struct EPM_Mesh {
  EPM_Mesh(PatchMesh *pmesh, RPatchMesh *rmesh, class PaintPatchData *patchData,
           INode *node, ModContext *mod, int mcListIndex, bool original) {
    PMesh = pmesh;
    RMesh = rmesh;
    PatchData = patchData;
    Node = node;
    ...
    Symmetry =
        getScriptAppDataPatchMesh(node, NEL3D_APPDATA_ZONE_SYMMETRY, 0) != 0;
  }
  PatchMesh *PMesh;
  RPatchMesh *RMesh;
  INode *Node;
...
struct EPM_PaintTile {
  ...
  bool intersect(const Ray &ray, std::vector<EPM_Mesh> &vectMesh, TimeValue t,
                 NLMISC::CVector &hit, CVector &topVector);
  sint32 patch;
  sint32 tile;
  sint16 Mesh;
  uint8 u;
...
protected:
  ...
  bool isLockedEx(PaintPatchMod *pobj, EPM_PaintTile *pTile,
                  std::vector<EPM_Mesh> &vectMesh, CLandscape *land);

  inline static bool isLocked(PaintPatchMod *pobj, EPM_PaintTile *pTile,
                              uint8 mask = 0xff);
  ...
  bool ClearATile(EPM_PaintTile *pTile, std::vector<EPM_Mesh> &vectMesh,
                  NL3D::CLandscape *land, CNelPatchChanger &nelPatchChg,
                  bool _256, bool _force128 = false);
  ...
  int getLayer(EPM_PaintTile *tile, int border, int tileSet, int rotate,
               std::vector<EPM_Mesh> &vectMesh, NL3D::CLandscape *land);
...
public:
  ...
  virtual GET_OBJECT_NAME_CONST MCHAR *
  GetObjectName(NL_GET_OBJECT_NAME_PARAMS) NL_GET_CLASS_NAME_CONST NL_OVERRIDE {
    return _M("NeL Patch Painter");
  }
...
public:
  ...
  PatchRestore(PaintPatchData *pd, PaintPatchMod *mod, PatchMesh *patch,
...
inline bool EPM_PaintMouseProc::isLocked(PaintPatchMod *pobj,
                                         EPM_PaintTile *pTile, uint8 mask) {
  // Lock border ?
  if (pobj->lockBorders) {

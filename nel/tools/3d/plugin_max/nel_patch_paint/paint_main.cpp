...
void PaintPatchMod::BeginEditParams(IObjParam *ip, ULONG flags,
                                    Animatable *prev) {
  this->ip = ip;
  ...
  hOpsPanel = ip->AddRollupPage(hInstance, MAKEINTRESOURCE(IDD_EDPATCH_OPS),
                                PatchOpsDlgProc, _M("Geometry"), (LPARAM)this,
                                rsOps ? 0 : APPENDROLL_CLOSED);
...
void PaintPatchMod::EndEditParams(IObjParam *ip, ULONG flags,
                                  Animatable *next) {
  if (hOpsPanel) {

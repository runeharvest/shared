...
public:
  ...
  CUndoStruct(uint16 patch, uint8 s, uint8 t, const CRGBA &color,
              uint16 blend) {
    // Build data
    _Patch = patch;
    _S = s;
...
public:
  // Constructor
  CUndoElement(int mesh, int tile, const CUndoStruct &old,
               const CUndoStruct &_new)
      : _Old(old), _New(_new) {
    _Action = UndoTile;
    _Mesh = mesh;
    _Tile = tile;
  ...
  // Constructor
  CUndoElement(int mesh, const CUndoStruct &old, const CUndoStruct &_new)
      : _Old(old), _New(_new) {
    _Action = UndoColor;
    _Mesh = mesh;
  };
...
public:
  ...
  // Undo
  void undo(EPM_PaintMouseProc &mouseProc, std::vector<EPM_Mesh> &vectMesh,
            CLandscape *land, PaintPatchMod *pobj, CPaintColor &paintColor);

  // Redo
  void redo(EPM_PaintMouseProc &mouseProc, std::vector<EPM_Mesh> &vectMesh,
            CLandscape *land, PaintPatchMod *pobj, CPaintColor &paintColor);

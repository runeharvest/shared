...
// Undo
void CTileUndo::undo(EPM_PaintMouseProc &mouseProc,
                     std::vector<EPM_Mesh> &vectMesh, CLandscape *land,
                     PaintPatchMod *pobj, CPaintColor &paintColor) {
  // List to undo
  getUndoList();
  ...
  if (_Result.size() != 0) {
    ...
    do {
      ...
      if (ite->_Action == UndoTile)
        // Set the tile
        mouseProc.SetTile(ite->_Mesh, ite->_Tile, ite->_Old._Desc, vectMesh,
                          land, nelPatchChg, false, false);
      else if (ite->_Action == UndoColor)
        // Set the color
        paintColor.setVertexColor(
            ite->_Mesh, ite->_Old._Patch, ite->_Old._S, ite->_Old._T,
            ite->_Old._Color, ite->_Old._Blend, vectMesh, nelPatchChg, false);
...
// Redo
void CTileUndo::redo(EPM_PaintMouseProc &mouseProc,
                     std::vector<EPM_Mesh> &vectMesh, CLandscape *land,
                     PaintPatchMod *pobj, CPaintColor &paintColor) {
  // List to undo
  getRedoList();
  ...
  while (ite != _Result.end()) {
    ...
    if (ite->_Action == UndoTile)
      // Set the tile
      mouseProc.SetTile(ite->_Mesh, ite->_Tile, ite->_New._Desc, vectMesh, land,
                        nelPatchChg, false, false);
    else if (ite->_Action == UndoColor)
      // Set the color
      paintColor.setVertexColor(ite->_Mesh, ite->_New._Patch, ite->_New._S,
                                ite->_New._T, ite->_New._Color,
                                ite->_New._Blend, vectMesh, nelPatchChg, false);

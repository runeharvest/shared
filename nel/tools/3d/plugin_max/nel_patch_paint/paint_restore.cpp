...
PatchRestore::PatchRestore(PaintPatchData *pd, PaintPatchMod *mod,
                           PatchMesh *patch, RPatchMesh *rpatch, TCHAR *id) {
  gotRedo = FALSE;
  epd = pd;
  this->mod = mod;
  oldPatch = *patch;
...
void PatchRestore::Restore(int isUndo) {
  ...
  mod->NotifyDependents(FOREVER, PART_GEOM | PART_TOPO | PART_SELECT,
                        REFMSG_CHANGE);
}

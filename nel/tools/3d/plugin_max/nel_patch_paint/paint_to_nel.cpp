...
std::vector<CTileElement> *CNelPatchChanger::getTileArray(int mesh, int patch) {
  ...
  if (ite == _MapNeLPatchInfo.end())
    ite = (_MapNeLPatchInfo.insert(CNelPatchMap::value_type(
               CNelPatchKey(mesh, patch), CNelPatchValue())))
              .first;
...
void CNelPatchChanger::applyChanges(bool displace) {
      ...
      for (uint edge = 0; edge < 4; edge++) {
        ...
        if (neighborEdge.Zone) {
          ...
          for (uint i = 0; i < (uint)neighborEdge.NPatchs; i++) {
            // Add new patch
            setNewPatch.insert(
                std::pair<uint, uint>(neighborEdge.Zone->getZoneId(),
                                      neighborEdge.Next[i]->getPatchId()));
          }
        }
      }
  ...
  while (ite != _MapNeLPatchInfo.end()) {
    ...
    zone->changePatchTextureAndColor(ite->first.second, ite->second.Tiles,
                                     ite->second.TileColors);
...
// Clear the container
void CNelPatchChanger::clear() { _MapNeLPatchInfo.clear(); }

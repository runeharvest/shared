...
float distance(PaintPatchMod *pobj) {
  return COLOR_BRUSH_MIN + (COLOR_BRUSH_MAX - COLOR_BRUSH_MIN) *
                               (float)pobj->ColorBushSize /
                               (float)COLOR_BRUSH_STEP;
}

/*-------------------------------------------------------------------*/

void CPaintColor::paint(int mesh, int tile, const CVector &hit,
                        const CVector &topVector,
  ...
  if (_Landscape) {
    ...
    paintATile(&_MouseProc->metaTile[mesh][tile], visited, hit, vectMesh,
               nelPatchChg, frozenVertices);
...
bool CPaintColor::getVertexInNeighbor(EPM_PaintTile *pTile, int curU, int curV,
                                      int neighbor, int &finalMesh,
                                      int &finalPatch, int &finalS,
                                      int &finalT) {
  // Get the vertex id
  /*
  0     3
  ...
  */
  ...
  int neighborVertexId =
      (((vertexId == neighbor) ? vertexId - 1 : vertexId + 1) +
       pTile->rotate[neighbor]) &
      3;
  ...
  // Final s coordinate
  finalS = pTile->voisins[neighbor]->u +
           (((neighborVertexId == 2) || (neighborVertexId == 3)) ? 1 : 0);

  // Final t coordinate
  finalT = pTile->voisins[neighbor]->v +
           (((neighborVertexId == 1) || (neighborVertexId == 2)) ? 1 : 0);
...
void CPaintColor::paintATile(EPM_PaintTile *pTile,
                             std::set<EPM_PaintTile *> &visited,
                             const CVector &hit,
                             std::vector<EPM_Mesh> &vectMesh,
                             CNelPatchChanger &nelPatchChg,
                             std::vector<CVertexColorFrozed> &frozenVertices) {
  // Check some args
  ...
  if ((vectMesh[pTile->Mesh].PMesh->selLevel == EP_PATCH) &&
      (!vectMesh[pTile->Mesh].PMesh->patchSel[pTile->patch]))
    return;
  ...
  if (!frozen) {
    ...
    bool bottomFrozen =
        (pTile->voisins[1] != NULL) && pTile->voisins[1]->frozen;
    ...
    bool leftLocked =
        ((pTile->voisins[0] == NULL) || leftFrozen) && _PObj->lockBorders;
    bool bottomLocked =
        ((pTile->voisins[1] == NULL) || bottomFrozen) && _PObj->lockBorders;
    bool rightLocked =
        ((pTile->voisins[2] == NULL) || rightFrozen) && _PObj->lockBorders;
    bool topLocked =
        ((pTile->voisins[3] == NULL) || topFrozen) && _PObj->lockBorders;
    ...
    if (!leftLocked && !topLocked) {
      ...
      paintAVertex(pTile->Mesh, pTile->patch, pTile->u, pTile->v, hit, vectMesh,
                   nelPatchChg);
    }
    ...
    bool borderRight =
        pTile->u ==
        (1 << vectMesh[pTile->Mesh].RMesh->getUIPatch(pTile->patch).NbTilesU) -
            1;
    bool borderBottom =
        pTile->v ==
        (1 << vectMesh[pTile->Mesh].RMesh->getUIPatch(pTile->patch).NbTilesV) -
            1;
    if (borderRight) {
      // Not locked ?
      if (!rightLocked && !topLocked) {
        ...
        if (rightFrozen || topFrozen)
          frozenVertices.push_back(
              CVertexColorFrozed(pTile, pTile->u + 1, pTile->v));

        // Set the vertex color
        paintAVertex(pTile->Mesh, pTile->patch, pTile->u + 1, pTile->v, hit,
                     vectMesh, nelPatchChg);
      }
    }
    if (borderBottom) {
      // Not locked ?
      if (!leftLocked && !bottomLocked) {
        ...
        if (leftFrozen || bottomFrozen)
          frozenVertices.push_back(
              CVertexColorFrozed(pTile, pTile->u, pTile->v + 1));

        // Set the vertex color
        paintAVertex(pTile->Mesh, pTile->patch, pTile->u, pTile->v + 1, hit,
                     vectMesh, nelPatchChg);
      }
    }
    if (borderRight && borderBottom) {
      ...
      if (!rightLocked && !bottomLocked) {
        ...
        if (rightFrozen || bottomFrozen)
          frozenVertices.push_back(
              CVertexColorFrozed(pTile, pTile->u + 1, pTile->v + 1));

        // Set the vertex color
        paintAVertex(pTile->Mesh, pTile->patch, pTile->u + 1, pTile->v + 1, hit,
                     vectMesh, nelPatchChg);
      }
    }
  }
  ...
  for (int neightbor = 0; neightbor < 4; neightbor++) {
    ...
    if (pTile->voisins[neightbor])
      // Ok, recusive call
      paintATile(pTile->voisins[neightbor], visited, hit, vectMesh, nelPatchChg,
                 frozenVertices);
  }
}
...
void CPaintColor::paintAVertex(int mesh, int patch, int s, int t,
                               const CVector &hit,
                               std::vector<EPM_Mesh> &vectMesh,
                               CNelPatchChanger &nelPatchChg) {
  // Check some args
  ...
  CVector vertexPos =
      pPatch->computeVertex((float)s / (float)pPatch->getOrderS(),
                            (float)t / (float)pPatch->getOrderT());
  ...
  if (distBrush <= brushSize) {
    ...
    // Set the vertex color
    setVertexColor(mesh, patch, s, t, theColor, blend, vectMesh, nelPatchChg,
                   true);
  }
}
...
void CPaintColor::pickVertexColor(int mesh, int patch, int s, int t,
                                  CVector &pos, CRGBA &color,
                                  std::vector<EPM_Mesh> &vectMesh) {
  // Check some args
  nlassert(_PObj->brushSize >= 0);
  nlassert(_PObj->brushSize < 3);
  ...
  // Eval the vertex position
  pos = pPatch->computeVertex((float)s / (float)pPatch->getOrderS(),
                              (float)t / (float)pPatch->getOrderT());
...
void CPaintColor::setVertexColor(int mesh, int patch, int s, int t,
                                 const CRGBA &newColor, uint16 blend,
                                 std::vector<EPM_Mesh> &vectMesh,
  ...
  if (undo) {
    ...
    CUndoElement elmt(mesh, CUndoStruct(patch, s, t, old, 256),
                      CUndoStruct(patch, s, t, newColor, blend));
...
bool CPaintColor::loadBrush(const std::string &brushFileName) {
  ...
  try {
    ...
    if (inputFile.open(brushFileName)) {
      ...
      // Error message
      std::string msg =
          toString("Can't open the file %s.", brushFileName.c_str());
      MessageBox((HWND)CNELU::Driver->getDisplay(), MaxTStrFromUtf8(msg).data(),
                 _T("NeL Painter"), MB_OK | MB_ICONEXCLAMATION);
    ...
    // Error message
    MessageBox((HWND)CNELU::Driver->getDisplay(),
               MaxTStrFromUtf8(e.what()).data(), _T("NeL Painter"),
...
void CPaintColor::forceFrozen(
    const std::vector<CVertexColorFrozed> &frozenVertices,
    CNelPatchChanger &nelPatchChg, std::vector<EPM_Mesh> &vectMesh) {
  ...
  for (vertex = 0; vertex < frozenVertices.size(); vertex++) {
    ...
    for (mesh = 0; mesh < vectMesh.size(); mesh++) {
      ...
      if ((vectMesh[mesh].RMesh == vectMesh[vertexFrozen.Tile->Mesh].RMesh) &&
          (mesh != (uint)vertexFrozen.Tile->Mesh)) {
        // Reference tile pointer
        EPM_PaintTile *refTile =
            &_MouseProc->metaTile[mesh][vertexFrozen.Tile->tile];
        ...
        for (n = 0; n < 4; n++)
          // Flag
          neighborNotFrozen[n] =
              (refTile->voisins[n] != NULL) && (!refTile->voisins[n]->frozen);

        // Get the vertex id
        static const int remap[4] = {0, 3, 1, 2};
        uint vertexId =
            (vertexFrozen.S - refTile->u) + (vertexFrozen.T - refTile->v) * 2;
        nlassert(vertexId < 4);
        vertexId = remap[vertexId];
        ...
        if (neighbor != 0xffffffff) {
          ...
          CRGBA color;
          vectMesh[mesh].RMesh->getVertexColor(refTile->patch, vertexFrozen.S,
                                               vertexFrozen.T, color);

          // Get the corresponding vertex
          int finalMesh, finalPatch, finalS, finalT;
          nlverify(getVertexInNeighbor(refTile, vertexFrozen.S, vertexFrozen.T,
                                       neighbor, finalMesh, finalPatch, finalS,
                                       finalT));

          // Set the vertex color
          setVertexColor(finalMesh, finalPatch, finalS, finalT, color, 255,
                         vectMesh, nelPatchChg, true);
        }
      }
    }
  }
  ...
  for (vertex = 0; vertex < frozenVertices.size(); vertex++) {
    ...
    for (n = 0; n < 4; n++)
      // Flag
      neighborFrozen[n] = (vertexFrozen.Tile->voisins[n] != NULL) &&
                          vertexFrozen.Tile->voisins[n]->frozen;

    // Get the vertex id
    static const int remap[4] = {0, 3, 1, 2};
    uint vertexId = (vertexFrozen.S - vertexFrozen.Tile->u) +
                    (vertexFrozen.T - vertexFrozen.Tile->v) * 2;
    nlassert(vertexId < 4);
    vertexId = remap[vertexId];
    ...
    nlverify(getVertexInNeighbor(vertexFrozen.Tile, vertexFrozen.S,
                                 vertexFrozen.T, neighbor, sourceMesh,
                                 sourcePatch, sourceS, sourceT));

    // Get the source color
    CRGBA color;
    vectMesh[sourceMesh].RMesh->getVertexColor(sourcePatch, sourceS, sourceT,
                                               color);

    // Set the vertex color
    setVertexColor(vertexFrozen.Tile->Mesh, vertexFrozen.Tile->patch,
                   vertexFrozen.S, vertexFrozen.T, color, 255, vectMesh,
                   nelPatchChg, true);
  }
}

...
CVector maxToNel(const Point3 &p) { return CVector(p.x, p.y, p.z); }

CRGBA maxToNel(COLORREF ref) {
  return CRGBA((uint8)(ref & 0xff), (uint8)((ref >> 8) & 0xff),
               (uint8)((ref >> 16) & 0xff));
}

COLORREF nelToMax(CRGBA ref) { return RGB(ref.R, ref.G, ref.B); }

void WarningInvalidTileSet() {
  if (!bWarningInvalidTileSet) {
    bWarningInvalidTileSet = true;
    MessageBox(NULL,
               _T("The tile bank is not compatible with your zone.\nPlease ")
               _T("use the good bank or erase and repaint the zone."),
               _T("Tile paint"), MB_OK | MB_ICONEXCLAMATION);
  }
}

int brushValue[BRUSH_COUNT] = {0, 4, 8};
...
void enterPainter(CTileBank &banktoLoad) {
  ...
  HKEY hKey;
  if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_EDIT_PATCH, 0, KEY_READ, &hKey) ==
      ERROR_SUCCESS) {
    DWORD len = 4;
    DWORD type;
    RegQueryValueEx(hKey, _T("Background"), 0, &type, (LPBYTE)&backGround,
                    &len);
...
void exitPainter() {
  ...
  HKEY hKey;
  if (RegCreateKey(HKEY_CURRENT_USER, REGKEY_EDIT_PATCH, &hKey) ==
      ERROR_SUCCESS) {
...
// Draw GUI
void drawVertexColorBox(float x0, float y0, float x1, float y1, CRGBA color,
                        float opacity, float hardness, IDriver &driver) {
  ...
  empty.A =
      (uint8)(255.f *
              std::max(
                  std::min(opacity * (1.f + 1.414213f * (hardness - 1.f)), 1.f),
                  0.f));
  ...
  CDRU::drawQuad(x0, y0, xc, yc, empty, medium, full, medium, driver,
                 CViewport());
  CDRU::drawQuad(xc, y0, x1, yc, fullFull, fullFull, fullFull, fullFull, driver,
                 CViewport());
  CDRU::drawQuad(xc, yc, x1, y1, fullFull, fullFull, fullFull, fullFull, driver,
                 CViewport());
  CDRU::drawQuad(x0, yc, xc, y1, medium, full, medium, empty, driver,
                 CViewport());
}

// Draw GUI
void drawInterface(TModeMouse select, TModePaint mode, PaintPatchMod *pobj,
                   IDriver &driver, CLandscapeModel &landscape,
  ...
  if (select == ModeSelect) {
    ...
    if (mode == ModeTile) {
      ...
      for (uint t = 0; t < num; t++) {
        ...
        if (bank.getTileSet(tileSet)->getNumTile128() > 0) {
          ...
          if (texture)
            CDRU::drawBitmap((float)((t + 1) % MOD_WIDTH) / (float)MOD_WIDTH,
                             (float)((t + 1) / MOD_WIDTH) / (float)MOD_HEIGHT,
                             1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                             *texture, driver, CViewport(), false);
        }
      }
    }
    // Select a tileSet
      ...
      if (pobj->DisplaceTileSet != -1) {
        ...
        for (uint t = 0; t < CTileSet::CountDisplace; t++) {
          // Get the texture
          ITexture *texture =
              bankCont->TileSet[pobj->DisplaceTileSet].DisplaceBitmap[t];

          // Draw the texture
          if (texture)
            CDRU::drawBitmap((float)(t % MOD_WIDTH) / (float)MOD_WIDTH,
                             (float)(t / MOD_WIDTH) / (float)MOD_HEIGHT,
                             1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                             *texture, driver, CViewport(), false);
          else
            CDRU::drawQuad((float)(t % MOD_WIDTH) / (float)MOD_WIDTH,
                           (float)(t / MOD_WIDTH) / (float)MOD_HEIGHT,
                           (float)(t % MOD_WIDTH) / (float)MOD_WIDTH +
                               1.f / (float)MOD_WIDTH,
                           (float)(t / MOD_WIDTH) / (float)MOD_HEIGHT +
                               1.f / (float)MOD_HEIGHT,
                           driver, CRGBA(128, 128, 128), CViewport());
        }
      }
    }
  } else {
    ...
    if (pobj->automaticLighting) {
      ...
      // Draw the bitmap
      CDRU::drawBitmap((float)(MOD_WIDTH - 1) / (float)MOD_WIDTH,
                       (float)(MOD_HEIGHT - 1) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT, *tex,
                       driver, CViewport(), true);
    }
    ...
    if (pobj->lockBorders) {
      ...
      // Draw the bitmap
      CDRU::drawBitmap((float)(MOD_WIDTH - 1) / (float)MOD_WIDTH,
                       (float)(MOD_HEIGHT - 2) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT, *tex,
                       driver, CViewport(), true);
    }
    ...
    if ((pobj->CurrentTileSet != -1) && (mode == ModeTile)) {
      ...
      if (bank.getTileSet(pobj->CurrentTileSet)->getOriented()) {
        ...
        // Draw the bitmap
        CDRU::drawBitmap((float)(MOD_WIDTH - 1) / (float)MOD_WIDTH,
                         (float)(0) / (float)MOD_HEIGHT, 1.f / (float)MOD_WIDTH,
                         1.f / (float)MOD_HEIGHT, *tex, driver, CViewport(),
                         true);
      }
      ...
      if (pobj->TileGroup == 0) {
        ...
        if (tex)
          CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 1) / (float)MOD_HEIGHT,
                           1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                           *tex, driver, CViewport(), true);
      } else {
        bool full = false;

        // 128 or 256 ?
        if (pobj->tileSize == 0) {
          if (bankCont->TileSet[pobj->CurrentTileSet]
                  .GroupTile128[pobj->TileGroup - 1]
                  .size() != 0)
            full = true;
        } else {
          if (bankCont->TileSet[pobj->CurrentTileSet]
                  .GroupTile256[pobj->TileGroup - 1]
                  .size() != 0)
            full = true;
        }
        ...
        if (full) {
          // Get pointer
          ITexture *tex = bankCont->TileSet[pobj->CurrentTileSet]
                              .GroupBitmap[pobj->TileGroup - 1];
    ...
    if (mode == ModeDisplace) {
      if (pobj->DisplaceTileSet != -1) {
        // Get pointer
        ITexture *tex = bankCont->TileSet[pobj->DisplaceTileSet]
                            .DisplaceBitmap[pobj->DisplaceTile];
        ...
        if (tex)
          ...
          CDRU::drawQuad(x0, y0, x1, y1, driver, CRGBA(128, 128, 128),
                         CViewport());
        }
      }
    }
...
#define BORDER_COLOR (0.2f)
      ...
      // Draw second color
      drawVertexColorBox(x0 + (x1 - x0) * BORDER_COLOR, y0, x1,
                         y1 + (y0 - y1) * BORDER_COLOR, maxToNel(color2), opa2,
                         hard2, driver);

      // Draw first color
      drawVertexColorBox(x0, y0 + (y1 - y0) * BORDER_COLOR,
                         x1 + (x0 - x1) * BORDER_COLOR, y1, maxToNel(color1),
                         opa1, hard1, driver);
    }

    // *** Draw the current bursh size
    if ((mode == ModeTile) || (mode == ModeDisplace)) {
      ITexture *burshSize[BRUSH_COUNT] = {bankCont->_smallBitmap,
                                          bankCont->mediumBitmap,
                                          bankCont->largeBitmap};

      // Draw brush icon
      CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 3) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                       *(burshSize[pobj->brushSize]), driver, CViewport(),
                       true);
      ...
      // Scale factor
      float scaleFactor =
          0.9f * (float)PaintPatchMod::ColorBushSize / (float)COLOR_BRUSH_STEP +
          0.1f;
      ...
      // Draw brush icon
      CDRU::drawBitmap(x0, y0, x1 - x0, y1 - y0, *bankCont->largeBitmap, driver,
                       CViewport(), true);
    }
    ...
    if (mode == ModeTile) {
      ...
      // Draw brush icon
      CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 2) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                       *(size[pobj->tileSize]), driver, CViewport(), true);
    }

    if ((pobj->CurrentTileSet != -1) && (mode == ModeTile)) {
      // *** Draw the current group
      ITexture *group[NL3D_CTILE_NUM_GROUP + 1] = {
          bankCont->allBitmap, bankCont->_0Bitmap, bankCont->_1Bitmap,
          bankCont->_2Bitmap,  bankCont->_3Bitmap, bankCont->_4Bitmap,
          bankCont->_5Bitmap,  bankCont->_6Bitmap, bankCont->_7Bitmap,
          bankCont->_8Bitmap,  bankCont->_9Bitmap, bankCont->_10Bitmap,
          bankCont->_11Bitmap};

      // Draw group icon
      CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 4) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                       *(group[pobj->TileGroup]), driver, CViewport(), true);
    }
    ...
    if (paintColor.getBrushMode() && (mode == ModeColor)) {
      ...
      CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 3) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                       paintColor.getBrush(), driver, CViewport(), false);
    }

    // For color only
    if (pobj->ShowCurrentState && ((int)pobj->CurrentState < 3) &&
        ((int)pobj->CurrentState >= 0)) {
      // Draw brush icon
      ITexture *bitmaps[3] = {bankCont->nothingBitmap, bankCont->regularBitmap,
                              bankCont->goofyBitmap};
      CDRU::drawBitmap(0, (float)(MOD_HEIGHT - 5) / (float)MOD_HEIGHT,
                       1.f / (float)MOD_WIDTH, 1.f / (float)MOD_HEIGHT,
                       *(bitmaps[(int)pobj->CurrentState]), driver, CViewport(),
                       true);
    }
  }
}
...
void addNeLZone(Object *object, INode *current, std::vector<EPM_Mesh> &vectMesh,
                PatchMesh *patch, RPatchMesh *rpatch, PaintPatchData *patchData,
                ModContext *mod, INode *original, uint &nelIndex) {
  // Not the same
  if ((current->GetObjectRef() == object) && (!current->IsHidden())) {
    vectMesh.push_back(EPM_Mesh(patch, rpatch, patchData, current, mod,
                                nelIndex++, original == current));
  }

  // Call to child
  for (uint child = 0; child < (uint)current->NumberOfChildren(); child++)
    addNeLZone(object, current->GetChildNode(child), vectMesh, patch, rpatch,
               patchData, mod, original, nelIndex);
}

// Tile painting algorithm. watch "doc/3d/tile_algorithm.doc" for details

void makeVectMesh(std::vector<EPM_Mesh> &vectMesh, INodeTab &nodes,
                  ModContextList &mcList, PaintPatchMod *pobj, TimeValue t) {
  ...
  for (int i = 0; i < mcList.Count(); i++) {
    ...
    addNeLZone(nodes[i]->GetObjectRef(), pobj->ip->GetRootNode(), vectMesh,
               patch, rpatch, patchData, mcList[i], nodes[i], nelIndex);
  }
}

void transformDesc(tileDesc &desc, bool symmetry, uint rotate, uint mesh,
                   uint tile, std::vector<EPM_Mesh> &vectMesh) {
  ...
  if (desc.getCase() != 0) {
    ...
    CPatchInfo::getTileSymmetryRotate(bank, desc.getLayer(0).Tile, tileSymmetry,
                                      tileRotate);
    ...
    if (symmetry) {
      if (bank.getTileSet(tileSet)->getOriented()) {
        if ((symVector[mesh].getOrientedTileBorderState(patch, symTile) !=
             CZoneSymmetrisation::Nothing))
          goofy = (symVector[mesh].getOrientedTileBorderState(patch, symTile) ==
                   CZoneSymmetrisation::Goofy);
        else
          goofy = (symVector[mesh].getTileState(patch, symTile, 0) ==
                   CZoneSymmetrisation::Goofy);
      } else {
        if ((symVector[mesh].getTileBorderState(patch, symTile) !=
             CZoneSymmetrisation::Nothing))
          goofy = (symVector[mesh].getTileBorderState(patch, symTile) ==
                   CZoneSymmetrisation::Goofy);
        else
          goofy = (symVector[mesh].getTileState(patch, symTile, 0) ==
                   CZoneSymmetrisation::Goofy);
    ...
    CPatchInfo::transform256Case(bank, case256, tileRotation, tileSymmetry,
                                 tileRotate, goofy);
  ...
  for (int l = 0; l < desc.getNumLayer(); l++) {
    ...
    if (symmetry) {
      if (bank.getTileSet(tileSet)->getOriented()) {
        if ((symVector[mesh].getOrientedTileBorderState(patch, symTile) !=
          ...
          goofy = (symVector[mesh].getTileState(patch, symTile, l) ==
                   CZoneSymmetrisation::Goofy);
      } else {
        if ((symVector[mesh].getTileBorderState(patch, symTile) !=
             CZoneSymmetrisation::Nothing))
    ...
    // Transform the transfo
    if (CPatchInfo::getTileSymmetryRotate(bank, tile, tileSymmetry,
                                          tileRotate)) {
      // Transform it
      if (CPatchInfo::transformTile(bank, tile, tileRotation, tileSymmetry,
                                    tileRotate, goofy)) {
...
void transformInvDesc(tileDesc &desc, bool symmetry, uint rotate, uint mesh,
                      uint tile, std::vector<EPM_Mesh> &vectMesh) {
  transformDesc(desc, false, 4 - rotate, mesh, tile, vectMesh);
  transformDesc(desc, symmetry, 0, mesh, tile, vectMesh);
}

void EPM_PaintMouseProc::SetTile(int mesh, int tile, const tileDesc &desc,
                                 std::vector<EPM_Mesh> &vectMesh,
                                 CLandscape *land,
                                 CNelPatchChanger &nelPatchChg,
                                 std::vector<CBackupValue> *backupStack,
                                 bool undo, bool updateDisplace) {
  // Undo: get old value
  tileDesc oldDesc;
  ...
  if (land) {
    ...
    for (uint i = 0; i < vectMesh.size(); i++) {
      ...
      if (vectMesh[i].RMesh == vectMesh[mesh].RMesh) {
        ...
        // Get the patch
        std::vector<CTileElement> &copyZone =
            *nelPatchChg.getTileArray(i, patch);
        ...
        transformInvDesc(nelDesc, vectMesh[mesh].Symmetry,
                         4 - vectMesh[mesh].Rotate, mesh, tile, vectMesh);
        ...
        for (int l = 0; l < 3; l++) {
          if (l >= nelDesc.getNumLayer()) {
            ...
            copyZone[u + v * OrderS].setTileOrient(l,
                                                   nelDesc.getLayer(l).Rotate);
          }
        }
        if (copyZone[u + v * OrderS].Tile[0] == 0xffff)
          ...
          if (nelDesc.getCase() == 0)
            ...
            copyZone[u + v * OrderS].setTile256Info(true,
                                                    nelDesc.getCase() - 1);
        }
        ...
        // Undo: push new value
        if (undo) {
          CUndoElement undoEmlt(i, tile, CUndoStruct(oldDesc),
                                CUndoStruct(desc));
...
void EPM_PaintMouseProc::GetTile(int mesh, int tile, tileDesc &desc,
                                 std::vector<EPM_Mesh> &vectMesh,
                                 CLandscape *land) {
  // NeL update
  if (land) {
...
int EPM_PaintMouseProc::selectTile(uint tileSet, bool selectCycle, bool _256,
                                   uint group, const CTileBank &bank) {
  // TileSet
  ...
  if (_256) {
    ...
    if (group == 0) {
      ...
      const std::vector<uint> &groupArray =
          bankCont->TileSet[tileSet].GroupTile256[group - 1];
      ...
      if (groupArray.size() == 0)
        ...
        // No, select in the global list
        nTile =
            TileSet->getTile256(groupArray[index % (uint32)groupArray.size()]);
    }
  } else {
    // Select in a group ?
    if (group == 0) {
      ...
      // Ref on the group tile array
      const std::vector<uint> &groupArray =
          bankCont->TileSet[tileSet].GroupTile128[group - 1];
      ...
      if (groupArray.size() == 0)
        ...
        // No, select in the global list
        nTile =
            TileSet->getTile128(groupArray[index % (uint32)groupArray.size()]);
    }
  }
...
bool EPM_PaintMouseProc::isLockedEx(PaintPatchMod *pobj, EPM_PaintTile *pTile,
                                    std::vector<EPM_Mesh> &vectMesh,
                                    CLandscape *land) {
  // Lock border ?
  if (pobj->lockBorders) {
    ...
    if (backup.getCase() > 0) {
      ...
      return (pTile->locked != 0) ||
             (pTile->getRight256(0, nRot)->locked != 0) ||
             (pTile->getBottom256(0, nRot)->locked != 0) ||
             (pTile->getRightBottom256(0, nRot)->locked != 0);
    } else
      // Return lock status
      return (pTile->locked != 0);
  }
...
bool EPM_PaintMouseProc::isLocked256(PaintPatchMod *pobj,
                                     EPM_PaintTile *pTile) {
  // Lock border ?
  if (pobj->lockBorders) {
    // Align on the grid
    ...
    // Return status
    return (pTile->locked != 0) || (pTile->voisins[2]->locked != 0) ||
           (pTile->voisins[1]->locked != 0) ||
           (pTile->voisins[2]->voisins[1]->locked != 0);
  }
...
bool EPM_PaintMouseProc::ClearATile(EPM_PaintTile *pTile,
                                    std::vector<EPM_Mesh> &vectMesh,
                                    CLandscape *land,
  ...
  if (_256) {
    ...
    // Get right tile
    int nRot;
    EPM_PaintTile *neighbor[4] = {pTile, pTile->getRight256(0, nRot),
                                  pTile->getBottom256(0, nRot),
                                  pTile->getRightBottom256(0, nRot)};
    ...
    for (n = 0; n < 4; n++) {
      ...
      SetTile(neighbor[n]->Mesh, neighbor[n]->tile, desc, vectMesh, land,
              nelPatchChg, NULL);
    }
  } else {
    // Check locked
...
bool EPM_PaintMouseProc::PutATile(EPM_PaintTile *pTile, int tileSet,
                                  int curRotation, const CTileBank &bank,
                                  bool selectCycle,
  ...
  if ((vectMesh[pTile->Mesh].PMesh->selLevel == EP_PATCH) &&
      (!vectMesh[pTile->Mesh].PMesh->patchSel[patch]))
    return false;
  ...
  if (tileSet == -1) {
    ...
    if (_256) {
      // Main tile
      tileDesc desc;
      desc.setTile(1, 1 + ((-curRotation) & 3), 0,
                   tileIndex(nTile, curRotation), tileIndex(0, 0),
                   tileIndex(0, 0));

      // Main tile
      SetTile(pTile->Mesh, pTile->tile, desc, vectMesh, land, nelPatchChg,
              &backupStack);

      // get right
      int nRot;
      EPM_PaintTile *other = pTile->getRight256(0, nRot);
      desc.setTile(1, 1 + ((-curRotation - 1) & 3), 0,
                   tileIndex(nTile, (curRotation - nRot) & 3), tileIndex(0, 0),
                   tileIndex(0, 0));
      nlassert(other);
      SetTile(other->Mesh, other->tile, desc, vectMesh, land, nelPatchChg,
              &backupStack);
      ...
      // get corner
      other = pTile->getRightBottom256(0, nRot);
      desc.setTile(1, 1 + ((-curRotation + 2) & 3), 0,
                   tileIndex(nTile, (curRotation - nRot) & 3), tileIndex(0, 0),
                   tileIndex(0, 0));
      nlassert(other);
      nlassert(other == pTile->getBottomRight256(0, nRot));
      ...
      tileDesc desc;
      desc.setTile(1, 0, 0, tileIndex(nTile, curRotation), tileIndex(0, 0),
                   tileIndex(0, 0));
      SetTile(pTile->Mesh, pTile->tile, desc, vectMesh, land, nelPatchChg,
              &backupStack);
    }
  ...
  if (_256) {
    ...
    for (int n = 0; n < 4; n++) {
      ...
      case 0:
        // Main
        if (pTile->voisins[3])
          if (!PropagateBorder(pTile->voisins[3],
                               (pTile->rotate[3] + curRotation) & 3, tileSet,
                               visited, bank, vectMesh, land, nelPatchChg,
                               backupStack)) {
            bContinue = false;
            goto zap;
          }
        if (pTile->voisins[0])
          if (!PropagateBorder(pTile->voisins[0],
                               (pTile->rotate[0] + curRotation) & 3, tileSet,
                               visited, bank, vectMesh, land, nelPatchChg,
                               backupStack)) {
            bContinue = false;
            goto zap;
      ...
      case 1:
        ...
        if (other->voisins[(0 - nRot) & 3])
          if (!PropagateBorder(
                  other->voisins[(0 - nRot) & 3],
                  (other->rotate[(0 - nRot) & 3] + curRotation) & 3, tileSet,
                  visited, bank, vectMesh, land, nelPatchChg, backupStack)) {
            bContinue = false;
            goto zap;
          }
        if (other->voisins[(1 - nRot) & 3])
          if (!PropagateBorder(
                  other->voisins[(1 - nRot) & 3],
                  (other->rotate[(1 - nRot) & 3] + curRotation) & 3, tileSet,
                  visited, bank, vectMesh, land, nelPatchChg, backupStack)) {
            bContinue = false;
            goto zap;
          }
      ...
      case 2:
        ...
        if (other->voisins[(2 - nRot) & 3])
          if (!PropagateBorder(
                  other->voisins[(2 - nRot) & 3],
                  (other->rotate[(2 - nRot) & 3] + curRotation) & 3, tileSet,
                  visited, bank, vectMesh, land, nelPatchChg, backupStack)) {
            bContinue = false;
            goto zap;
          }
      ...
      case 3:
        ...
        if (other->voisins[(3 - nRot) & 3])
          if (!PropagateBorder(
                  other->voisins[(3 - nRot) & 3],
                  (other->rotate[(3 - nRot) & 3] + curRotation) & 3, tileSet,
                  visited, bank, vectMesh, land, nelPatchChg, backupStack)) {
            bContinue = false;
            goto zap;
          }
  ...
  zap:;
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (pTile->voisins[ii]) {
        if (!PropagateBorder(pTile->voisins[ii],
                             (pTile->rotate[ii] + curRotation) & 3, tileSet,
                             visited, bank, vectMesh, land, nelPatchChg,
                             backupStack)) {
          bContinue = false;
          break;
  ...
  if (!bContinue) {
    ...
    for (int back = backupStack.size() - 1; back >= 0; back--) {
      // Yo!
      SetTile(backupStack[back].Mesh, backupStack[back].Tile,
              backupStack[back].Desc, vectMesh, land, nelPatchChg, NULL);
    }
    ...
    if (!_256 && !backup256) {
      ...
      for (uint edge = 0; edge < 4; edge++) {
        ...
        if (pTile->voisins[edge]) {
          ...
          // Tile is filled ?
          if (GetBorderDesc(pTile->voisins[edge], pVoisinCorner, pBorder,
                            &pVoisinIndex, bank, vectMesh, nelPatchChg, land)) {
            // Neighbor edge
            ...
            for (uint subTile = 0; subTile < 2; subTile++) {
              ...
              int slot =
                  getLayer(pTile, edge,
                           pVoisinCorner[(neigborEdge + subTile) & 3].TileSet,
                           (pVoisinCorner[(neigborEdge + subTile) & 3].Rotate -
                            pTile->rotate[edge]) &
                               3,
                           vectMesh, land);
              ...
              // Get the border
              borderEdges[edge][1 - subTile] =
                  CTileSet::getInvertBorder(pBorder[neigborEdge][slot]);
            }
          }
        }
      }
      ...
      for (int l = 0; l < (int)setIndex.size(); l++) {
        if (l == 0) {
          ...
          finalIndex[l].Tile =
              selectTile(setIndex[l].TileSet, false, false, 0, bank);
          ...
          for (uint e = 0; e < 4; e++) {
            ...
            if (bFilled[e] && bFilled[(e + 1) & 3])
              ...
              if (pTile->voisins[e]) {
                ...
                // Tile is filled ?
                if (GetBorderDesc(pTile->voisins[e], pVoisinCorner, pBorder,
                                  &pVoisinIndex, bank, vectMesh, nelPatchChg,
                                  land)) {
                  // Neighbor edge
                  int neigborEdge = (2 + e + pTile->rotate[e]) & 3;

                  // Get the slot
                  int slot = getLayer(pTile, e, setIndex[l].TileSet,
                                      setIndex[l].Rotate, vectMesh, land);

                  if ((slot != -1) &&
                      (pBorder[neigborEdge][slot] != CTileSet::dontcare)) {
                    // Invert the border
                    border[e] =
                        CTileSet::getInvertBorder(pBorder[neigborEdge][slot]);
          ...
          const CTileSetTransition *tileTrans = FindTransition(
              setIndex[l].TileSet, setIndex[l].Rotate, border, bank);
          if (!tileTrans) {
            return false;
          }
          finalIndex[l].Rotate = (setIndex[l].Rotate & 3);
      ...
      case 1:
        desc.setTile(1, 0, desc.getDisplace(), finalIndex[0], tileIndex(0, 0),
                     tileIndex(0, 0));
        break;
      case 2:
        desc.setTile(2, 0, desc.getDisplace(), finalIndex[0], finalIndex[1],
                     tileIndex(0, 0));
        break;
      case 3:
        desc.setTile(3, 0, desc.getDisplace(), finalIndex[0], finalIndex[1],
                     finalIndex[2]);
        break;
      default:
        nlassert(0); // no!
...
void EPM_PaintMouseProc::PutADisplacetile(EPM_PaintTile *pTile,
...
bool EPM_PaintMouseProc::GetBorderDesc(
    EPM_PaintTile *tile, tileSetIndex *pVoisinCorner,
    CTileSet::TFlagBorder pBorder[4][3], tileDesc *pVoisinIndex,
  ...
  for (int nL = 0; nL < nLayer; nL++) {
    ...
    for (int i = 0; i < 4; i++) {
      // Border type
      CTileSet::TBorder toBorder[4] = {CTileSet::left, CTileSet::bottom,
                                       CTileSet::right, CTileSet::top};
      CTileSet::TFlagBorder border = CTileSet::_1111;
      if (type == CTileBank::transition)
        border = CTileSet::getOrientedBorder(
            toBorder[i],
            CTileSet::getEdgeType((CTileSet::TTransition)number, toBorder[i]));
      ...
      case CTileSet::_1000:
        ...
        pVoisinCorner[(i + backup.getLayer(nL).Rotate) & 3].Rotate =
            index.Rotate;
        break;
      }
  ...
  pVoisinIndex->setTile(nLayer, backup.getCase(), backup.getDisplace(),
                        pIndexx[0], pIndexx[1], pIndexx[2]);
...
const CTileSetTransition *
EPM_PaintMouseProc::FindTransition(int nTileSet, int nRotate,
                                   const CTileSet::TFlagBorder *border,
                                   const CTileBank &bank) {
  ...
  for (int i = 0; i < 4; i++) {
    CTileSet::TBorder toBorder[4] = {CTileSet::left, CTileSet::bottom,
                                     CTileSet::right, CTileSet::top};
    pBorderConverted[i] =
        CTileSet::getOrientedBorder(toBorder[i], border[(i + nRotate) & 3]);
  }

  // Look for good tile..
  CTileSet::TTransition nTransition =
      CTileSet::getTransitionTile(pBorderConverted[3], pBorderConverted[1],
                                  pBorderConverted[0], pBorderConverted[2]);
...
int EPM_PaintMouseProc::getLayer(EPM_PaintTile *tile, int border, int tileSet,
                                 int rotate, std::vector<EPM_Mesh> &vectMesh,
                                 CLandscape *land) {
  int nLayer = -1;
  tileDesc desc;
  GetTile(tile->voisins[border]->Mesh, tile->voisins[border]->tile, desc,
          vectMesh, land);
...
bool EPM_PaintMouseProc::PropagateBorder(
    EPM_PaintTile *tile, int curRotation, int curTileSet,
    std::set<EPM_PaintTile *> &visited, const CTileBank &bank,
    std::vector<EPM_Mesh> &vectMesh, CLandscape *land,
    CNelPatchChanger &nelPatchChg, std::vector<CBackupValue> &backupStack,
    bool recurseNoDiff) {
  // 1) Already visited
  ...
  // Check if we are in patch subobject and if this patch is selected
  if ((vectMesh[tile->Mesh].PMesh->selLevel == EP_PATCH) &&
      (!vectMesh[tile->Mesh].PMesh->patchSel[patch]))
    return false;
  ...
  tileDesc pIndex;
  bool bFill = GetBorderDesc(tile, nCorner, nBorder, &pIndex, bank, vectMesh,
                             nelPatchChg, land);
  ...
  for (v = 0; v < 4; v++) {
    ...
    if (tile->voisins[v]) {
      ...
      CTileSet::TFlagBorder pBorder[4][3];
      tileDesc pVoisinIndex;
      bFill = GetBorderDesc(tile->voisins[v], pVoisinCorner, pBorder,
                            &pVoisinIndex, bank, vectMesh, nelPatchChg, land);
      if (bFill) {
        ...
        if ((visited.find(tile->voisins[v]) != visited.end()) /*|| (vectMesh[tile->voisins[v]->Mesh].Node->IsFrozen())*/)
          ...
          if (bTouched[v]) {
            if (nCorner[v] != pVoisinCorner[(edge + 1) & 3]) {
              ...
              int delta =
                  (pVoisinCorner[(edge + 1) & 3].Rotate - nCorner[v].Rotate) &
                  3;
              if (delta != 2) {
                if (nCorner[v] < pVoisinCorner[(edge + 1) & 3]) {
          ...
          if (bTouched[nNextCorner]) {
            if (nCorner[nNextCorner] != pVoisinCorner[edge]) {
              // Check if it is a valid corner
              int delta =
                  (pVoisinCorner[edge].Rotate - nCorner[nNextCorner].Rotate) &
                  3;
              if (delta != 2) {
                if (nCorner[nNextCorner] < pVoisinCorner[edge]) {
  ...
  if (_256 && !tile->frozen) {
    ...
    if (other) {
      ...
      if (!desc1.isEmpty()) {
        if ((desc1.isEmpty()) || (desc1.getCase() != (1 + ((nCase + 1) & 3))) ||
            (desc1.getLayer(0).Tile != backup.getLayer(0).Tile) ||
            (desc1.getLayer(0).Rotate !=
             ((backup.getLayer(0).Rotate - rot) & 3)))
          bDiff = true;
      } else
        bDiff = true;
    ...
    if (other) {
      ...
      GetTile(other->Mesh, other->tile, desc1, vectMesh, land);
      if (!desc1.isEmpty()) {
        if ((desc1.isEmpty()) || (desc1.getCase() != (1 + ((nCase + 3) & 3))) ||
            (desc1.getLayer(0).Tile != backup.getLayer(0).Tile) ||
            (desc1.getLayer(0).Rotate !=
             ((backup.getLayer(0).Rotate - rot) & 3)))
          bDiff = true;
  ...
  for (v = 0; v < 4; v++) {
    if (tile->voisins[v] == NULL)
      ...
      tileDesc desc;
      GetTile(tile->voisins[v]->Mesh, tile->voisins[v]->tile, desc, vectMesh,
              land);
      if (bModified[v] || bModified[(v + 1) & 3] || (desc.isEmpty()))
        bSameEdge[v] = false;
    }
  }
          ...
          if (bFilled[(c + 1) & 3])
            ...
            // If 3 mat, use a 3/4 border
              ...
              if (tile->voisins[c]) {
                ...
                CTileSet::TFlagBorder pBorder[4][3];
                tileDesc pVoisinIndex;
                if (GetBorderDesc(tile->voisins[c], pVoisinCorner, pBorder,
                                  &pVoisinIndex, bank, vectMesh, nelPatchChg,
                                  land)) {
                  ...
                  if (nLayer != -1) {
                    ...
                    if (pBorder[edge][nLayer] ==
                        CTileSet::getInvertBorder(invWanted)) {
                      ...
                      const CTileSetTransition *pTile = FindTransition(
                          ite->TileSet, ite->Rotate + tile->rotate[c],
                          newBorder, bank);
                      if (!pTile) {
                        WarningInvalidTileSet();
                        return false;
                      ...
                      // Set the tile..
                      SetTile(tile->voisins[c]->Mesh, tile->voisins[c]->tile,
                              pVoisinIndex, vectMesh, land, nelPatchChg,
                              &backupStack);
                    }
            ...
            // Normal,
              ...
              if (tile->voisins[c] &&
                  ((visited.find(tile->voisins[c]) != visited.end()) /*||
 (vectMesh[tile->voisins[c]->Mesh].Node->IsFrozen())*/
                ...
                CTileSet::TFlagBorder pBorder[4][3];
                tileDesc pVoisinIndex;
                bool bOk = GetBorderDesc(tile->voisins[c], pVoisinCorner,
                                         pBorder, &pVoisinIndex, bank, vectMesh,
                                         nelPatchChg, land);
                ...
                int nLayer = getLayer(tile, c, ite->TileSet, ite->Rotate,
                                      vectMesh, land);
                if (nLayer != -1) {
                  nlassert((pBorder[edge][nLayer] == CTileSet::_0111) ||
                           (pBorder[edge][nLayer] == CTileSet::_0001));
                }
                ...
                if ((bVisited[c] || !recurseNoDiff) && bSameEdge[c]) {
                  ...
                  tileDesc pVoisinIndex;
                  bool bOk = GetBorderDesc(tile->voisins[c], pVoisinCorner,
                                           pBorder, &pVoisinIndex, bank,
                                           vectMesh, nelPatchChg, land);
                  ...
                  if (bOk) {
                    ...
                    // transition ok?
                    if ((pBorder[edge][nLayer] == CTileSet::_0111) ||
                        (pBorder[edge][nLayer] == CTileSet::_0001)) {
                      // copy!
                      border[c] =
                          CTileSet::getInvertBorder(pBorder[edge][nLayer]);
                      bSameEdge[c] = true;
                      bComputed = true;
                    }
                  }
          ...
          if (!bFilled[nNextCorner])
            ...
            // If extraordinary vertex and the small edge
            if ((extraOrdinary[nNextCorner]) &&
                (extraOrdinarySmallEdge[nNextCorner] == c)) {
            ...
            // If 3 mat, use a 3/4 border
              ...
              // If voisin already visited, force his transition to 3/4
              if (tile->voisins[c]) //&&visited.find
                                    //(tile->voisins[c])!=visited.end())
              {
                tileSetIndex pVoisinCorner[4];
                CTileSet::TFlagBorder pBorder[4][3];
                tileDesc pVoisinIndex;
            ...
            // Normal,
              ...
              // Voisin visited ?
              if ((tile->voisins[c]) && ((visited
                                              .find(
                                                  tile->voisins
                                                      [c]) != visited.end()) /*|| (vectMesh[tile->voisins[c]->Mesh].Node->IsFrozen())*/)) {
                // Yes, visited. Copy the border
                tileSetIndex pVoisinCorner[4];
                CTileSet::TFlagBorder pBorder[4][3];
                tileDesc pVoisinIndex;
                ...
                int nLayer = getLayer(tile, c, ite->TileSet, ite->Rotate,
                                      vectMesh, land);
                if (nLayer != -1) {
                  nlassert((pBorder[edge][nLayer] == CTileSet::_1110) ||
                           (pBorder[edge][nLayer] == CTileSet::_1000));
                }
                ...
                if ((bVisited[c] || !recurseNoDiff) && bSameEdge[c]) {
                  ...
                  if (bOk) {
                    ...
                    // transition ok?
                    if ((pBorder[edge][nLayer] == CTileSet::_1110) ||
                        (pBorder[edge][nLayer] == CTileSet::_1000)) {
                      // copy!
                      border[c] =
                          CTileSet::getInvertBorder(pBorder[edge][nLayer]);
      ...
      const CTileSetTransition *pTile =
          FindTransition(ite->TileSet, ite->Rotate, border, bank);
      if (!pTile) {
        WarningInvalidTileSet();
        return false;
      }
  ...
  SetTile(tile->Mesh, tile->tile, desc, vectMesh, land, nelPatchChg,
          &backupStack);
  ...
  for (v = 0; v < 4; v++) {
    ...
    if ((tile->voisins[v]) && (visited.find(tile->voisins[v]) == visited
                                                                     .end()) /* && (vectMesh[tile->voisins[v]->Mesh].Node->IsFrozen()==0)*/) {
      // ok.. not visited, border with modified corner ?
      if (bModified[v] || bModified[(v + 1) & 3] || (!bSameEdge[v]))
        if (!PropagateBorder(tile->voisins[v],
                             (tile->rotate[v] + curRotation) & 3, curTileSet,
                             visited, bank, vectMesh, land, nelPatchChg,
                             backupStack, false)) {
          bContinue = false;
          break;
...
uint8 EPM_PaintMouseProc::CalcRotPath(EPM_PaintTile *from, EPM_PaintTile *to,
                                      int depth, int rotate, int &deltaX,
                                      int &deltaY, int &cost) {
  ...
  if (depth > 0) {
    ...
    for (int i = 0; i < 4; i++) {
      if (from->voisins[i]) {
        ...
        int myDeltaY;
        int myCost;
        int myRet = CalcRotPath(from->voisins[i], to, depth - 1,
                                (from->rotate[i] + rotate) & 3, myDeltaX,
                                myDeltaY, myCost);
        if (myRet != 0xff) {
...
void EPM_PaintMouseProc::RecursTile(EPM_PaintTile *pTile, const CTileBank &bank,
                                    int tileSet,
                                    std::vector<EPM_Mesh> &vectMesh,
                                    CLandscape *land, int recurs,
                                    std::set<EPM_PaintTile *> &alreadyRecursed,
                                    bool first, int rotation,
                                    CNelPatchChanger &nelPatchChg, bool _256) {
  if (alreadyRecursed.find(pTile) == alreadyRecursed.end()) {
    ...
    if (nModeTexture == ModeDisplace)
      ...
      PutATile(pTile, tileSet, rotation, bank, first, visited, vectMesh, land,
               nelPatchChg, _256);
  }
    ...
    for (int i = 0; i < 4; i++) {
      if (_256) {
        if (pTile->get2Voisin(i))
          RecursTile(pTile->get2Voisin(i), bank, tileSet, vectMesh, land,
                     recurs - 2, alreadyRecursed, false,
                     (rotation + pTile->get2VoisinRotate(i)) & 3, nelPatchChg,
                     true);
      } else {
        if (pTile->voisins[i])
          RecursTile(pTile->voisins[i], bank, tileSet, vectMesh, land,
                     recurs - 1, alreadyRecursed, false,
                     (rotation + pTile->rotate[i]) & 3, nelPatchChg, false);
      }
    }
  }
}

BOOL EPM_PaintMouseProc::PutDisplace(int tile, int mesh, const CTileBank &bank,
                                     std::vector<EPM_Mesh> &vectMesh,
                                     CLandscape *land, int recurs,
                                     std::set<EPM_PaintTile *> &alreadyRecursed,
                                     CNelPatchChanger &nelPatchChg) {
  ...
  // Ok recurse tiles
  RecursTile(pTile, bank, 0, vectMesh, land,
             brushValue[PaintPatchMod::brushSize], alreadyRecursed, true,
...
BOOL EPM_PaintMouseProc::PutTile(int tile, int mesh, bool first,
                                 const CTileBank &bank, int tileSet,
                                 std::vector<EPM_Mesh> &vectMesh,
                                 CLandscape *land, int recurs,
                                 std::set<EPM_PaintTile *> &alreadyRecursed,
                                 CNelPatchChanger &nelPatchChg, bool _256) {
  ...
  if (first) {
    ...
    int cost;
    uint8 deltaRot =
        CalcRotPath(pOldTile, pTile, DEPTH_SEARCH_MAX, 0, deltaX, deltaY, cost);
  ...
  if (!lost) {
    {
      ...
      // Ok recurse tiles
      RecursTile(pTile, bank, tileSet, vectMesh, land,
                 brushValue[PaintPatchMod::brushSize], alreadyRecursed, first,
...
/*int offset[4]={ 0, (nV-1)*MAX_TILE_IN_PATCH, (nV-1)*MAX_TILE_IN_PATCH+nU-1,
nU-1 }; int offsetOther[4]={ 0, (nVOther-1)*MAX_TILE_IN_PATCH,
(nVOther-1)*MAX_TILE_IN_PATCH+nUOther-1, nUOther-1 };*/
...
void mainproc(CScene &scene, CEventListenerAsync &AsyncListener,
              CEvent3dMouseListener &mouseListener, CLandscapeModel &landscape,
              IDriver &driver, callThread *pData, CPaintColor &paintColor) {
  ...
  // Choose a color
  if ((AsyncListener.isKeyPushed((TKey)PainterKeys[Select])) &&
  ...
  if (AsyncListener.isKeyPushed((TKey)PainterKeys[ToggleArrows])) {
    ...
    if (pData->pobj->additiveTile) {
      ...
      for (i = 0; i < landscape.Landscape.TileBank.getTileCount(); i++) {
        landscape.Landscape.TileBank.getTile(i)->setFileName(CTile::additive,
                                                             "arrow.tga");
  ...
  if (AsyncListener.isKeyPushed((TKey)PainterKeys[SelectColorBrush])) {
    ...
    static TCHAR szFilter[] = _T("Targa Files (*.tga)\0*.tga\0")
                              _T("All Files (*.*)\0*.*\0\0");
    ...
    openFile.Flags = OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT | OFN_ENABLESIZING |
                     OFN_EXPLORER;
  ...
  // Clear buffers
  CNELU::clearBuffers(CRGBA((uint8)(backGround & 0xff),
                            (uint8)((backGround >> 8) & 0xff),
                            (uint8)((backGround >> 16) & 0xff)));
...
// Speed.
#define NSPEED 5
    static float Speed[NSPEED] = {
        1.66f, 3.33f, 8.33f, 40.0f,
        200.0f}; // 6km/h, 12km/h, 30km/h, 144km/h, 700km/h.
    static sint idSpeed = 2;
    float speed = Speed[idSpeed];
    float rotSpeed = 80 * (float)Pi / 180;

    // Rot.
    if (AsyncListener.isKeyDown(KeyLEFT))
      CameraRot.z += rotSpeed * dt;
    if (AsyncListener.isKeyDown(KeyRIGHT))
      CameraRot.z -= rotSpeed * dt;
    ...
    if (AsyncListener.isKeyDown(KeyUP))
      CameraPos += dir;
    if (AsyncListener.isKeyDown(KeyDOWN))
      CameraPos -= dir;
...
public:
  MouseListener(IObjParam *ip, CCamera *camera, CViewport *viewport,
                PaintPatchMod *pobj, EPM_PaintMouseProc *eproc,
                CLandscape *land, CEventListenerAsync *async,
                CEvent3dMouseListener *mouseList,
                std::vector<EPM_Mesh> &vectMesh, TimeValue t)
      : PaintColor(pobj, land, &bankCont->Undo, eproc),
        _FillTile(pobj, land, &bankCont->Undo, eproc), _VectMesh(vectMesh) {
    _Ip = ip;
    _Camera = camera;
    _Viewport = viewport;
    _Pobj = pobj;
...
private:
  ...
  CZoneSymmetrisation::TState
  MouseListener::getState(int mesh, int tile, const CVector &hit,
                          std::vector<EPM_Mesh> &vectMesh);
    ...
    if (modeSelect != ModeSelect) {
      ...
      if (event == EventMouseDownId) {
        if (pressed &&
            ((mouse->Button & rightButton) &&
             ((mouse->Button & (ctrlButton | shiftButton | altButton)) == 0))) {
          pressed = false;
        ...
        if (mouse->Button == leftButton) {
          CVector hotSpot;
          CVector topVector;
          if (_Eproc->HitATile(*_Viewport, *_Camera, mouse->X, mouse->Y, &tile1,
                               &mesh1, _T, _VectMesh, hotSpot, topVector)) {
            ...
            if (modeSelect == ModePaint) {
              if ((nModeTexture == ModeTile) ||
                  (nModeTexture == ModeDisplace)) {
                // Check if we are in patch subobject and if this patch is
                // selected
                if ((_VectMesh[mesh1].PMesh->selLevel != EP_PATCH) ||
                    (_VectMesh[mesh1].PMesh->patchSel[patch])) {
                  std::set<EPM_PaintTile *> alreadyRecursed;
                  ...
                  if (nModeTexture == ModeTile) {
                    // Put the tile
                    _Eproc->PutTile(
                        tile1, mesh1, true, bank, _Pobj->CurrentTileSet,
                        _VectMesh, _Land, brushValue[PaintPatchMod::brushSize],
                        alreadyRecursed, nelPatchChg, _Pobj->tileSize != 0);
                  } else // (nModeTexture==ModeDisplace)
                  {
                    // Put the tile
                    _Eproc->PutDisplace(tile1, mesh1, bank, _VectMesh, _Land,
                                        brushValue[PaintPatchMod::brushSize],
                                        alreadyRecursed, nelPatchChg);
                  }

                  // Flush nel chgt
                  nelPatchChg.applyChanges((nModeTexture == ModeDisplace) &&
                                           (_Pobj->automaticLighting));
                }
              } else if (nModeTexture == ModeColor) {
                // Paint
              ...
              CZoneSymmetrisation::TState state =
                  getState(mesh1, tile1, hotSpot, _VectMesh);
              ...
              if (nModeTexture == ModeTile)
                ...
                _FillTile.fillTile(mesh1, patch, _VectMesh,
                                   _Pobj->CurrentTileSet,
                                   _Pobj->TileFillRotation, _Pobj->TileGroup,
                                   _Pobj->tileSize != 0, bank);
              else if (nModeTexture == ModeColor)
                // Fill this patch with the current color
                _FillTile.fillColor(mesh1, patch, _VectMesh, maxToNel(color1),
                                    (uint16)(256.f * opa1), PaintColor);
              ...
              for (int pp = 0; pp < np; ++pp) {
                ...
                // Fill default color
                _FillTile.fillColor(mesh1, pp, _VectMesh, CRGBA(255, 255, 255),
                                    256, PaintColor);
      ...
      if (event == EventMouseMoveId) {
        if ((pressed) && (mouse->Button == leftButton)) {
          CVector hotSpot;
          CVector topVector;
          if (_Eproc->HitATile(*_Viewport, *_Camera, mouse->X, mouse->Y, &tile2,
                               &mesh2, _T, _VectMesh, hotSpot, topVector)) {
            _3dMouseListener->setHotSpot(hotSpot);
            if ((tile1 != tile2) || (mesh1 != mesh2)) {
              ...
              if ((nModeTexture == ModeTile) ||
                ...
                // selected
                if ((_VectMesh[mesh1].PMesh->selLevel != EP_PATCH) ||
                    (_VectMesh[mesh2].PMesh->patchSel[patch])) {
                  std::set<EPM_PaintTile *> alreadyRecursed;
                  ...
                  if (nModeTexture == ModeTile) {
                    // Put the tile
                    _Eproc->PutTile(
                        tile2, mesh2, false, bank, _Pobj->CurrentTileSet,
                        _VectMesh, _Land, brushValue[PaintPatchMod::brushSize],
                        alreadyRecursed, nelPatchChg, _Pobj->tileSize != 0);
                  } else // (nModeTexture==ModeDisplace)
                  {
                    // Put the tile
                    _Eproc->PutDisplace(tile2, mesh2, bank, _VectMesh, _Land,
                                        brushValue[PaintPatchMod::brushSize],
                                        alreadyRecursed, nelPatchChg);
                  }
...
// Pick a tile
void MouseListener::pick(int mesh, int tile, const CVector &hit,
                         TModePaint mode) {
  // Pick tile
  if ((mode == ModeTile) || (mode == ModeDisplace)) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      CVector pos;
      CRGBA color;
      PaintColor.pickVertexColor(mesh, pTile->patch, u[i], v[i], pos, color,
                                 _VectMesh);
...
int getBindedEdge(int nPatch, int nVertInPatch, const PatchMesh &patch,
                  const RPatchMesh &rpatch) {
  ...
  case BIND_SINGLE:
    ...
    if (patch.patches[rpatch.getUIVertex(nVertInMesh).Binding.nPatch]
            .v[rpatch.getUIVertex(nVertInMesh).Binding.nEdge] ==
...
#endif // NL_DEBUG
      nlassert(
          patch.patches[rpatch.getUIVertex(nVertInMesh).Binding.nPatch]
              .v[(rpatch.getUIVertex(nVertInMesh).Binding.nEdge + 1) & 3] ==
          patch.patches[nPatch].v[(nVertInPatch - 1) & 3]);
  ...
  case BIND_25:
    ...
    if ((rpatch.getUIVertex(nVertexBefore).Binding.bBinded) &&
        (rpatch.getUIVertex(nVertexBefore).Binding.nPatch ==
         rpatch.getUIVertex(nVertInMesh).Binding.nPatch) &&
        (rpatch.getUIVertex(nVertexBefore).Binding.nEdge ==
         rpatch.getUIVertex(nVertInMesh).Binding.nEdge) &&
        (rpatch.getUIVertex(nVertexBefore).Binding.nType == BIND_50)) {
      return (nVertInPatch - 1) & 3;
    } else {
      nlassert(patch.patches[rpatch.getUIVertex(nVertInMesh).Binding.nPatch]
                   .v[rpatch.getUIVertex(nVertInMesh).Binding.nEdge] ==
               patch.patches[nPatch].v[(nVertInPatch + 1) & 3]);
      return nVertInPatch;
    }
  ...
  case BIND_50:
    ...
    if ((rpatch.getUIVertex(nVertexBefore).Binding.bBinded) &&
      ...
      nlassert(rpatch.getUIVertex(nVertexAfter).Binding.nPatch ==
               rpatch.getUIVertex(nVertInMesh).Binding.nPatch);
      nlassert(rpatch.getUIVertex(nVertexAfter).Binding.nEdge ==
               rpatch.getUIVertex(nVertInMesh).Binding.nEdge);
  ...
  case BIND_75:
    ...
    if ((rpatch.getUIVertex(nVertexAfter).Binding.bBinded) &&
        (rpatch.getUIVertex(nVertexAfter).Binding.nPatch ==
         rpatch.getUIVertex(nVertInMesh).Binding.nPatch) &&
        (rpatch.getUIVertex(nVertexAfter).Binding.nEdge ==
         rpatch.getUIVertex(nVertInMesh).Binding.nEdge) &&
  ...
  if (pobj->hOpsPanel) {
    ...
    for (i = 0; i < (int)vectMesh.size(); i++) {
      ...
      for (int p = 0; p < patch->numPatches; p++) {
        ...
        for (int u = 0; u <= nU; u++)
          for (int v = 0; v <= nV; v++) {
            Point3 pos = patch->patches[p].interp(patch, (float)u / (float)(nU),
                                                  (float)v / (float)(nV));
    ...
    CVector center((fMinX + fMaxX) / 2.f, (fMinY + fMaxY) / 2.f,
                   (fMinZ + fMaxZ) / 2.f);
    CVector lookFrom(fMaxX, fMaxY, fMaxZ);
    eproc.quadTreeSelect.create(
        8, center,
        (float)std::max(std::max(fMaxX - fMinX, fMaxY - fMinY), fMaxZ - fMinZ));
      ...
      for (int p = 0; p < patch->numPatches; p++) {
        ...
        for (int u = 0; u < nU; u++)
          for (int v = 0; v < nV; v++) {
            EPM_PaintTile *pTile =
                &eproc
                     .metaTile[i][p * NUM_TILE_SEL + v * MAX_TILE_IN_PATCH + u];
            pTile->Mesh = i;
            pTile->patch = p;
            pTile->tile = p * NUM_TILE_SEL + v * MAX_TILE_IN_PATCH + u;
            pTile->voisins[3] =
                v == 0 ? NULL
                       : &eproc.metaTile[i][p * NUM_TILE_SEL +
                                            (v - 1) * MAX_TILE_IN_PATCH + u];
            pTile->voisins[1] =
                v == (nV - 1)
                    ? NULL
                    : &eproc.metaTile[i][p * NUM_TILE_SEL +
            ...
            Point3 pos[4];
            pos[0] = patch->patches[p].interp(patch, (float)u / (float)(nU),
                                              (float)v / (float)(nV));
            pos[0] = pos[0] * (vectMesh[i].Node->GetObjectTM(t));
            pos[1] = patch->patches[p].interp(
                patch, (float)(u + 1) / (float)(nU), (float)v / (float)(nV));
            pos[1] = pos[1] * (vectMesh[i].Node->GetObjectTM(t));
            pos[2] =
                patch->patches[p].interp(patch, (float)(u + 1) / (float)(nU),
                                         (float)(v + 1) / (float)(nV));
            pos[2] = pos[2] * (vectMesh[i].Node->GetObjectTM(t));
            pos[3] = patch->patches[p].interp(patch, (float)u / (float)(nU),
                                              (float)(v + 1) / (float)(nV));
            pos[3] = pos[3] * (vectMesh[i].Node->GetObjectTM(t));

            // Store its center
            pTile->Center = (maxToNel(pos[0]) + maxToNel(pos[1]) +
                             maxToNel(pos[2]) + maxToNel(pos[3])) /
                            4.f;

            // Store its radius
            pTile->Radius =
                std::max(std::max((maxToNel(pos[0]) - pTile->Center).norm(),
                                  (maxToNel(pos[1]) - pTile->Center).norm()),
                         std::max((maxToNel(pos[2]) - pTile->Center).norm(),
                                  (maxToNel(pos[3]) - pTile->Center).norm()));
            ...
            eproc.quadTreeSelect.insert(CVector(fMinX, fMinY, fMinZ),
                                        CVector(fMaxX, fMaxY, fMaxZ), pTile);
          }
      }
    }
        ...
        for (int e = 0; e < 4; e++) {
          ...
          if (mYedge == -1) {
            std::string error = NLMISC::toString(
                "Invalid edge '%i' with value '%i' in patch '%i' in PatchMesh",
                p, mYedge, e);
            nlwarning(error.c_str());
            MessageBox(NULL, MaxTStrFromUtf8(error).data(),
                       _T("NeL Patch Painter"), MB_OK | MB_ICONSTOP);
            return;
          }
#if (MAX_RELEASE < 4000)
          int otherPatch = (patch->edges[mYedge].patch1 == p)
                               ? patch->edges[mYedge].patch2
                               : patch->edges[mYedge].patch1;
          ...
          if (otherPatch != -1) {
            ...
            if (rpatch->getUIVertex(patch->patches[p].v[e]).Binding.bBinded)
              vertBinded = e;
            if (rpatch->getUIVertex(patch->patches[p].v[(e + 1) & 3])
                    .Binding.bBinded)
              vertBinded = (e + 1) & 3;
            if ((vertBinded != -1) &&
                (getBindedEdge(p, vertBinded, *patch, *rpatch) == e)) {
            ...
            } else // Look in the neighborhood for a voisin
            {
              ...
              Point3 vB1 =
                  patch->verts[patch->patches[p].v[(e + 1) & 3]].p * m1;
                ...
                if (ii != i) {
                  ...
                  for (int ee = 0; ee < patch2->numEdges; ee++) {
#if (MAX_RELEASE < 4000)
                    if ((patch2->edges[ee].patch1 == -1) ||
                        (patch2->edges[ee].patch2 == -1)) {
                      //
                      int pp = patch2->edges[ee].patch1 == -1
                                   ? patch2->edges[ee].patch2
                                   : patch2->edges[ee].patch1;
#else  // (MAX_RELEASE < 4000)
                    if (patch2->edges[ee].patches.Count() > 0) {
                      ...
                      Point3 vA2 =
                          patch2->verts[patch2->patches[pp].v[edge]].p * m2;
                      Point3 vB2 =
                          patch2->verts[patch2->patches[pp].v[(edge + 1) & 3]]
                              .p *
                          m2;
                      ...
                      if (((vA1 - vB2).Length() < WELD_THRESOLD) &&
                          ((vA2 - vB1).Length() < WELD_THRESOLD)) {
                        // The same!!
          ...
          if (patchVoisin.patch != -1) {
            ...
            std::string second =
                MCharStrToUtf8(vectMesh[patchVoisin.Mesh].Node->GetName());
            int rot = (2 - ((vectMesh[i].Symmetry) ? (2 - e) : e) +
                       ((vectMesh[patchVoisin.Mesh].Symmetry) ? (2 - edgeVoisin)
                                                              : edgeVoisin)) &
                      3;
            int nU = 1 << rpatch->getUIPatch(p).NbTilesU;
            int nV = 1 << rpatch->getUIPatch(p).NbTilesV;
            int nUOther = 1 << vectMesh[patchVoisin.Mesh]
                                   .RMesh->getUIPatch(patchVoisin.patch)
                                   .NbTilesU;
            int nVOther = 1 << vectMesh[patchVoisin.Mesh]
                                   .RMesh->getUIPatch(patchVoisin.patch)
                                   .NbTilesV;
            int nTile = (e & 1) ? nU : nV;
            ...
            if (nTile == (nTile2 >> dividEdge)) {
              ...
              int offsetOther = getOffset(edgeVoisin, nUOther, nVOther,
                                          vectMesh[patchVoisin.Mesh].Symmetry);
              static int delta[4] = {MAX_TILE_IN_PATCH, 1, -MAX_TILE_IN_PATCH,
                                     -1};

              int symIndex = vectMesh[i].Symmetry ? -1 : 1;
              int symIndexOther = vectMesh[patchVoisin.Mesh].Symmetry ? -1 : 1;

              EPM_PaintTile *pTile1 =
                  &eproc.metaTile[i][p * NUM_TILE_SEL + offset];
              EPM_PaintTile *pTile2 =
                  &eproc.metaTile[nMeshIndex][patchVoisin.patch * NUM_TILE_SEL +
                                              offsetOther];
              pTile2 += (nTile2 - 1) * delta[edgeVoisin] * symIndexOther;
              pTile2 -=
                  (delta[edgeVoisin] * symIndexOther * nTile2 * offsetEdge) >>
                  2;
              ...
              for (int end = 0; end < nTile; end++) {
                ...
                // tile src..
                nlassert((pTile2->voisins[edgeVoisin] == NULL) ||
    ...
    if (!sName.empty()) {
      ...
      if (file.open(sName)) {
        try {
          ...
          MessageBox(NULL, MaxTStrFromUtf8(stream.what()).data(), _T("Error"),
  ...
  if (hModule) {
    ...
    if (res) {
      ...
      std::string modulePath =
          NLMISC::CFile::getPath(MCharStrToUtf8(sModulePath)) + "ligoscape.cfg";
...
DWORD WINAPI myThread(LPVOID vData) {
  ...
  try {
    ...
    try {
      ...
      TheLand->Landscape.setupStaticLight(LightDiffuse, LightAmbiant,
                                          LightMultiply);

      // *******************
      CExportNel export_(true, true, true, pData->eproc->ip,
                         "NeL Patch Painter", NULL);
        ...
        for (uint nNode = 0; nNode < (uint)root->NumberOfChildren(); nNode++) {
          ...
          // Try to export a mesh
            ...
            if (pShape) {
              ...
              // Create an instance
              CTransformShape *tShape =
                  CNELU::Scene->createInstance(CExportNel::getName(*pNode));
      ...
      CNELU::Camera->setPerspective(75.f * (float)Pi / 180.f /*vp->GetFOV()*/,
                                    1.33f, 0.1f, 10000.f);
      ...
      for (i = 0; i < (int)pData->VectMesh.size(); i++) {
        ...
        // Create the zone..
        CZone zone;
        if (rpatch->exportZone(pData->VectMesh[i].Node, patch, zone,
                               symVector[i], i, config.CellSize, config.Snap,
                               true)) {
          // Smooth corner
          ...
          std::string message = toString(
              "Can't build the zone named %s",
              MCharStrToUtf8(pData->VectMesh[i].Node->GetName()).c_str());
          MessageBox(pData->eproc->ip->GetMAXHWnd(),
                     MaxTStrFromUtf8(message).data(), _T("NeL Painter"),
                     MB_OK | MB_ICONEXCLAMATION);
        }
      }
      ...
      MouseListener listener(pData->eproc->ip, CNELU::Camera, &viewport,
                             pData->pobj, pData->eproc, &TheLand->Landscape,
                             &CNELU::AsyncListener, &mouseListener,
                             pData->VectMesh, pData->T);
      ...
      if (pData->pobj->preloadTiles) {
        ...
        for (sint tss = 0; tss < (sint)tileSetSelector.getTileCount(); tss++) {
          ...
          for (tl = 0; tl < tileSet->getNumTile128(); tl++)
            TheLand->Landscape.flushTiles(CNELU::Scene->getDriver(),
          ...
          for (tl = 0; tl < CTileSet::count; tl++)
            TheLand->Landscape.flushTiles(
                CNELU::Scene->getDriver(),
                (uint16)tileSet->getTransition(tl)->getTile(), 1);
        }
      }
      ...
      do {
        ...
        // Call the proc
        mainproc(*CNELU::Scene, CNELU::AsyncListener, mouseListener, *TheLand,
                 *CNELU::Scene->getDriver(), pData, listener.PaintColor);
      } while (!CNELU::AsyncListener.isKeyPushed(KeyESCAPE) &&
               listener.WindowActive);
      ...
      MessageBox(NULL, MaxTStrFromUtf8(druExcept.what()).data(),
                 _T("NeL driver utility"), MB_OK | MB_ICONEXCLAMATION);
    }

    delete pData;
  } catch (const Exception &e) {
    MessageBox(NULL, MaxTStrFromUtf8(e.what()).data(), _T("NeL Painter"),
               MB_OK | MB_ICONEXCLAMATION);
  }
...
BOOL EPM_PaintMouseProc::HitATile(ViewExp *vpt, IPoint2 *p, int *tile,
                                  int *mesh, TimeValue t,
                                  std::vector<EPM_Mesh> &vectMesh, CVector &hit,
                                  CVector &topVector) {
  ...
  quadTreeSelect.selectRay(CVector(ray.p.x, ray.p.y, ray.p.z),
                           CVector(ray.dir.x, ray.dir.y, ray.dir.z));
...
BOOL EPM_PaintMouseProc::HitATile(const CViewport &viewport,
                                  const CCamera &camera, float x, float y,
                                  int *tile, int *mesh, TimeValue t,
                                  std::vector<EPM_Mesh> &vectMesh,
                                  NLMISC::CVector &hit,
                                  NLMISC::CVector &topVector) {
  // Get a world ray with the mouse 2d point
  CVector pos, dir;
  viewport.getRayWithPoint(x, y, pos, dir, camera.getMatrix(),
                           camera.getFrustum());
...
bool CheckTri(const Point3 &pos0, const Point3 &pos1, const Point3 &pos2,
              const Ray &ray, CVector &hit) {
...
bool EPM_PaintTile::intersect(const Ray &ray, std::vector<EPM_Mesh> &vectMesh,
                              TimeValue t, CVector &hit, CVector &topVector) {
  ...
  // 4 corners
  Point3 pos[4];
  pos[0] = patchPtr->patches[patch].interp(patchPtr, (float)u / (float)(nU),
                                           (float)v / (float)(nV));
  pos[0] = pos[0] * (node->GetObjectTM(t));
  pos[1] = patchPtr->patches[patch].interp(patchPtr, (float)u / (float)(nU),
                                           (float)(v + 1) / (float)(nV));
  pos[1] = pos[1] * (node->GetObjectTM(t));
  pos[2] = patchPtr->patches[patch].interp(
      patchPtr, (float)(u + 1) / (float)(nU), (float)(v + 1) / (float)(nV));
  pos[2] = pos[2] * (node->GetObjectTM(t));
  pos[3] = patchPtr->patches[patch].interp(
      patchPtr, (float)(u + 1) / (float)(nU), (float)v / (float)(nV));
...
int EPM_PaintMouseProc::proc(HWND hwnd, int msg, int point, int flags,
                             IPoint2 m) {
#if MAX_VERSION_MAJOR >= 19

...
void CFillPatch::fillTile(int mesh, int patch, std::vector<EPM_Mesh> &vectMesh,
                          int tileSet, int rot, int group, bool _256,
  ...
  if (_Landscape) {
    ...
    tileDesc descClear;
    descClear.setTile(0, 0, 0, tileIndex(0, 0), tileIndex(0, 0),
                      tileIndex(0, 0));

    // Fill tile description
    tileDesc descFill; // Four fill descriptor LeftTop, LeftBottom, RightTop,
                       // RightBottom

    // Fill with clear ?
    if (tileSet == -1)
      descFill.setTile(0, 0, 0, tileIndex(0, 0), tileIndex(0, 0),
                       tileIndex(0, 0));
    ...
    for (uint v = 0; v < numV; v += (1 << (int)_256))
      for (uint u = 0; u < numU; u += (1 << (int)_256)) {
        // Yes, if we are not in clear mode and not in 256 or if it is a top
        // 256x256 tile
        int nTile = 0;
        ...
        for (vv = 0; vv < (uint)_256 + 1; vv++)
          for (uu = 0; uu < (uint)_256 + 1; uu++) {
            ...
            EPM_PaintTile *pTile =
                &_MouseProc
                     ->metaTile[mesh][patch * NUM_TILE_SEL +
                                      (vv + v) * MAX_TILE_IN_PATCH + uu + u];
            ...
            for (n = 0; n < 4; n++) {
              ...
              if (pTile->voisins[n]) {
                if (_256) {
                  ...
                  // Neighbor locked ?
                  if (_MouseProc->isLockedEx(_PObj, pTile->voisins[n], vectMesh,
                                             _Landscape)) {
                    nearLocked = true;
                  }
                }
          ...
          if (nearLocked) {
            ...
            for (vv = 0; vv < (uint)_256 + 1; vv++)
              for (uu = 0; uu < (uint)_256 + 1; uu++) {
                ...
                EPM_PaintTile *pTile =
                    &_MouseProc->metaTile[mesh][patch * NUM_TILE_SEL +
                                                (vv + v) * MAX_TILE_IN_PATCH +
                                                uu + u];
                nlassert(pTile->Mesh == mesh);
                nlassert(pTile->patch == patch);

                // Clear the tile
                _MouseProc->ClearATile(pTile, vectMesh, _Landscape, nelPatchChg,
                                       _256, !_256);
              }
              ...
              for (vv = 0; vv < (uint)_256 + 1; vv++)
                for (uu = 0; uu < (uint)_256 + 1; uu++) {
                  ...
                  for (n = 0; n < 4; n++) {
                    ...
                    if ((pTile->voisins[n]) &&
                        ((pTile->voisins[n]->Mesh != mesh) ||
                         (pTile->voisins[n]->patch != patch))) {
                      // Neigborhood not compatible ?
                      tileDesc descNei;
                      _MouseProc->GetTile(pTile->voisins[n]->Mesh,
                                          pTile->voisins[n]->tile, descNei,
                                          vectMesh, _Landscape);
                      ...
                      if (descNei.getNumLayer() == 1) {
                        // Check rotation
                        if (descNei.getLayer(0).Rotate ==
                            ((pTile->rotate[n] + rot) & 3)) {
                          // Get the tile set
                          int neiTileSet;
                          int number;
                          CTileBank::TTileType type;
                          bank.getTileXRef(descNei.getLayer(0).Tile, neiTileSet,
                                           number, type);
                ...
                for (uu = 0; uu < (uint)_256 + 1; uu++) {
                  ...
                  if (tileSet != -1) {
                    ...
                    if (_256) {
                      ...
                      case 0:
                        // LeftTop
                        descFill.setTile(1, ((0 - rot) & 3) + 1, 0,
                                         tileIndex(nTile, rot), tileIndex(0, 0),
                                         tileIndex(0, 0));
                        break;
                      case 1:
                        // LeftBottom
                        descFill.setTile(1, ((1 - rot) & 3) + 1, 0,
                                         tileIndex(nTile, rot), tileIndex(0, 0),
                                         tileIndex(0, 0));
                        break;
                      case 2:
                        // RightTop
                        descFill.setTile(1, ((3 - rot) & 3) + 1, 0,
                                         tileIndex(nTile, rot), tileIndex(0, 0),
                                         tileIndex(0, 0));
                        break;
                      case 3:
                        // RightBottom
                        descFill.setTile(1, ((2 - rot) & 3) + 1, 0,
                                         tileIndex(nTile, rot), tileIndex(0, 0),
                                         tileIndex(0, 0));
                        break;
                      default:
                      ...
                      // Set the tile
                      descFill.setTile(1, 0, 0, tileIndex(nTile, rot),
                                       tileIndex(0, 0), tileIndex(0, 0));
                  }

                  // Get the displace index
                  tileDesc descOrig;
                  _MouseProc->GetTile(mesh, pTile->tile, descOrig, vectMesh,
                                      _Landscape);

                  // Fill the current tile with the current tileset and rotation
                  descFill.setDisplace(descOrig.getDisplace());
                  _MouseProc->SetTile(mesh, pTile->tile, descFill, vectMesh,
                                      _Landscape, nelPatchChg, NULL);
                }
            } else {
              // For all tiles in this tile
              for (vv = 0; vv < (uint)_256 + 1; vv++)
                for (uu = 0; uu < (uint)_256 + 1; uu++) {
                  ...
                  // Clear the tile
                  _MouseProc->ClearATile(pTile, vectMesh, _Landscape,
                                         nelPatchChg, _256, !_256);
                }
            }
          }
        }
...
void CFillPatch::fillColor(int mesh, int patch, std::vector<EPM_Mesh> &vectMesh,
                           const CRGBA &color, uint16 blend,
  ...
  if (_Landscape) {
    ...
    for (uint v = 0; v < numV; v++)
      for (uint u = 0; u < numU; u++) {
        ...
        paintColor.setVertexColor(mesh, patch, u, v, color, blend, vectMesh,
                                  nelPatchChg, true);
      }
...
void CFillPatch::fillDisplace(int mesh, int patch,
  ...
  if (_Landscape) {
    ...
    for (uint v = 0; v < numV; v++)
      for (uint u = 0; u < numU; u++) {
        ...
        EPM_PaintTile *pTile =
            &_MouseProc->metaTile[mesh][patch * NUM_TILE_SEL +
                                        (v)*MAX_TILE_IN_PATCH + u];
        nlassert(pTile->Mesh == mesh);
        nlassert(pTile->patch == patch);

        // Set the displace
        _MouseProc->PutADisplacetile(pTile, bank, vectMesh, _Landscape,
                                     nelPatchChg);
      }

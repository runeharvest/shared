...
void CPaintLight::setup(NL3D::CLandscape &landscape, NL3D::CScene &scene) {
  ...
  for (uint light = 0; light < _Lights.size(); light++) {
    ...
    // Attenuation
    pointLightModel->PointLight.setupAttenuation(_Lights[light].rRadiusMin,
                                                 _Lights[light].rRadiusMax);
    ...
    // Which light type??
    if (_Lights[light].bAmbientOnly ||
        _Lights[light].Type == SLightBuild::LightAmbient)
      ...
      // Export Spot infos.
      pointLightModel->lookAt(_Lights[light].Position,
                              _Lights[light].Position +
                                  _Lights[light].Direction);
      pointLightModel->PointLight.setupSpotAngle(_Lights[light].rHotspot,
                                                 _Lights[light].rFallof);
    } else {
      // What???
      nlstop;
    }

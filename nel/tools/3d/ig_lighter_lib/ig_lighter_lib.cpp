...
// ***************************************************************************
void CIgLighterLib::overSampleCell(CIGSurfaceLightBuild::CCellCorner &cell,
                                   uint nSampleReq,
                                   const CLocalRetriever &localRetriever,
                                   CGlobalRetriever &globalRetriever,
                                   sint retrieverInstanceId,
                                   const ULocalPosition &localPos,
                                   float cellSize, float cellRaytraceDeltaZ) {
  uint sample;

  nlassert(nSampleReq == 2 || nSampleReq == 4 || nSampleReq == 8 ||
           nSampleReq == 16);
  ...
  if (nSampleReq == 2) {
    ...
    /*
        For all 16 samples, but skip ones such that we have this pattern (X ==
       sample). X . X . . X . X X . X . . X . X
    */
    for (sample = 0; sample < 16; sample++) {
      ...
      if ((x + y) & 1)
        continue;
      // Start at BL corner (-s2), get middle of sample (+s8) and there is 4
      // sample by line/column (*s4) NB: index= sample/2 because only 8 samples
      // are computed.
...
void CIgLighterLib::lightIg(CInstanceLighter &instanceLighter,
  ...
  if (lightDesc.Shadow) {
    ...
    for (i = 0; i < (sint)igIn.getNumInstance(); i++) {
      // progress
      instanceLighter.progress("Loading Shapes obstacles",
                               float(i) / igIn.getNumInstance());

      // Skip it?? IgLighterLib use the DontCastShadowForInterior flag. See doc
      // of this flag
      if (igIn.getInstance(i).DontCastShadow ||
          igIn.getInstance(i).DontCastShadowForInterior)
        continue;
  ...
  if (retrieverBank && globalRetriever) {
    ...
    // col Identifier.
    string colIdent = slInfo.ColIdentifierPrefix + slInfo.IgFileName +
      ...
      if (instance.getType() == CLocalRetriever::Interior) {
        ...
        if (retIdent.find(colIdent) != string::npos) {
          ...
          if (itRgm != igSurfaceLightBuild->RetrieverGridMap.end()) {
            nlwarning("ERROR Found 2 different collision retriever with same "
                      "identifier: '%s'. The 2nd is discared\n",
                      retIdent.c_str());
          } else {
            // Append CRetrieverLightGrid.
            itRgm = igSurfaceLightBuild->RetrieverGridMap
                        .insert(make_pair(
                            localRetrieverId,
                            CIGSurfaceLightBuild::CRetrieverLightGrid()))
                        .first;
            ...
            // Compute the bbox for all surfaces. (NB: local to the
            // localRetriever).
            ...
            for (uint surfaceId = 0; surfaceId < numSurfaces; surfaceId++) {
              ...
              surfDst.Origin.x =
                  floorf(surfDst.Origin.x / cellSurfaceLightSize) *
                  cellSurfaceLightSize;
              surfDst.Origin.y =
                  floorf(surfDst.Origin.y / cellSurfaceLightSize) *
                  cellSurfaceLightSize;

              // Snap Width / Height on cellSize.
              float sizex =
                  surfaceBBoxes[surfaceId].getMax().x - surfDst.Origin.x;
              float sizey =
                  surfaceBBoxes[surfaceId].getMax().y - surfDst.Origin.y;
              ...
              for (sint yCell = 0; yCell < (sint)surfDst.Height; yCell++) {
                for (sint xCell = 0; xCell < (sint)surfDst.Width; xCell++) {
                  ...
                  localPos.Estimation.x =
                      surfDst.Origin.x + xCell * cellSurfaceLightSize;
                  localPos.Estimation.y =
                      surfDst.Origin.y + yCell * cellSurfaceLightSize;
                  ...
                  if (snapped) {
                    ...
                    cell.CenterPos =
                        globalRetriever->getGlobalPosition(globalPos);
                    ...
                    if (lightDesc.OverSampling == 0) {
                      ...
                      // OverSample.
                      overSampleCell(cell, lightDesc.OverSampling,
                                     localRetriever, *globalRetriever,
                                     instanceId, localPos, cellSurfaceLightSize,
                                     slInfo.CellRaytraceDeltaZ);
                      // it is possible that no samples lies in surfaces (small
                      // surface). In this case, just copy CenterPos into
                      // samples.
  ...
  // Run.
  //=======
  instanceLighter.light(igIn, igOut, lightDesc, obstacles, NULL,
                        igSurfaceLightBuild);

  // Output a debug mesh??
  if (igSurfaceLightBuild && slInfo.BuildDebugSurfaceShape &&
      !igSurfaceLightBuild->RetrieverGridMap.empty()) {
    ...
    for (uint i = 0; i < 2; i++) {
      ...
      // What kind of debug?
      if (i == 0)
        igSurfaceLightBuild->buildSunDebugMesh(meshBuild, meshBaseBuild,
                                               deltaPos);
      else
        igSurfaceLightBuild->buildPLDebugMesh(meshBuild, meshBaseBuild,
                                              deltaPos, igOut);

...
void displayZones() {
  ...
  try {
    ...
    string tmp = string("Cant load bankfile ") + ViewerCfg.BanksPath + "/" +
                 ViewerCfg.Bank;
    nlerror(tmp.c_str());
  }
  ...
  try {
    ...
    string tmp =
        string("Cant load bankfile ") + ViewerCfg.BanksPath + "/" + farBank;
    nlerror(tmp.c_str());
  }
  ...
  Landscape->Landscape.setupStaticLight(ViewerCfg.LandDiffuse,
                                        ViewerCfg.LandAmbient, 1.1f);
  ...
  for (int ts = 0; ts < Landscape->Landscape.TileBank.getTileSetCount(); ts++) {
    ...
    for (tl = 0; tl < tileSet->getNumTile128(); tl++)
      Landscape->Landscape.flushTiles(CNELU::Scene->getDriver(),
    ...
    for (tl = 0; tl < CTileSet::count; tl++)
      Landscape->Landscape.flushTiles(
  ...
  for (i = 0; i < ViewerCfg.Zones.size(); i++) {
    ...
    try {
      CIFile file(
          CPath::lookup(ViewerCfg.ZonesPath + "/" + ViewerCfg.Zones[i]));
      zone.serial(file);
      file.close();
  ...
  // Init collision Manager.
  CNELU::clearBuffers(CRGBA(0, 0, 0));
  ViewerCfg.TextContext.printfAt(0.5f, 0.5f,
  ...
  MoveListener.init(CNELU::Scene, ViewerCfg.Width, ViewerCfg.Height,
                    *CNELU::Camera);
  ...
  do {
    ...
    if (ViewerCfg.EyesHeight.z < 0.1f)
      ViewerCfg.EyesHeight.z = 0.1f;
    if (eh < 0.1f)
      eh = 0.1f;
    ...
    if (showInfos) {
      ...
      // black top quad
      CDRU::drawQuad(0, 0.97f, 1.0f, 1.0f, *CNELU::Driver, CRGBA(0, 0, 0),
                     CNELU::Scene->getViewport());

      // black bottom quad
      CDRU::drawQuad(0, 0, 1.0f, 0.03f, *CNELU::Driver, CRGBA(0, 0, 0),
                     CNELU::Scene->getViewport());
      ...
      // Display Tile Near
      ViewerCfg.TextContext.printfAt(0.75f, 0.98f, "Tile Near : %.1f",
                                     tileNear);
      ...
      // Display Threshold
      ViewerCfg.TextContext.printfAt(0.3f, 0.98f, "Threshold : %.3f",
                                     threshold);
      ...
      ViewerCfg.TextContext.printfAt(0.3f, 0.01f, "Zone : %s",
                                     zoneName.c_str());

      // Position
      ViewerCfg.TextContext.printfAt(0.1f, 0.01f, "Position : %.2f %.2f %.2f",
                                     pos.x, pos.y, pos.z);

      // Eyes height
      ViewerCfg.TextContext.printfAt(0.7f, 0.01f, "Eyes : %.2f m",
                                     ViewerCfg.EyesHeight.z);

      // Display speed in km/h
      ViewerCfg.TextContext.setColor(CRGBA(255, 0, 0));
      ViewerCfg.TextContext.printfAt(0.5f, 0.01f, "Speed : %d km/h",
                                     (sint)(MoveListener.getSpeed() * 3.6f));
      ViewerCfg.TextContext.setColor(CRGBA(255, 255, 255));
      ...
      if (heading < 0)
        heading += 360;
      ViewerCfg.TextContext.printfAt(0.9f, 0.01f, "Heading : %d degrees",
                                     heading);
...
void writeConfigFile(const char *configFileName) {
  ...
  fprintf(f, "Height = %d;\n", ViewerCfg.Height);
  fprintf(f, "Depth = %d;\n", ViewerCfg.Depth);
  fprintf(f, "Position = { %f, %f, %f };\n", ViewerCfg.Position.x,
          ViewerCfg.Position.y, ViewerCfg.Position.z);
  fprintf(f, "EyesHeight = %f;\n", ViewerCfg.EyesHeight.z);
  fprintf(f, "Background = { %d, %d, %d };\n", ViewerCfg.Background.R,
          ViewerCfg.Background.G, ViewerCfg.Background.B);
  fprintf(f, "ZFar = %f;\n", ViewerCfg.ZFar);

  fprintf(f, "AutoLight = %d;\n", ViewerCfg.AutoLight ? 1 : 0);
  fprintf(f, "LightDir = { %f, %f, %f };\n", ViewerCfg.LightDir.x,
          ViewerCfg.LightDir.y, ViewerCfg.LightDir.z);
  ...
  fprintf(f, "LandAmbient = { %d, %d, %d };\n", ViewerCfg.LandAmbient.R,
          ViewerCfg.LandAmbient.G, ViewerCfg.LandAmbient.B);
  fprintf(f, "LandDiffuse = { %d, %d, %d };\n", ViewerCfg.LandDiffuse.R,
          ViewerCfg.LandDiffuse.G, ViewerCfg.LandDiffuse.B);
...
\****************************************************************/
#ifdef NL_OS_WINDOWS
int APIENTRY nltWinMain(HINSTANCE /* hInstance */,
...
#endif // NL_OS_WINDOWS
{
  try {
    ...
    // Init NELU
    NL3D::CNELU::init(ViewerCfg.Width, ViewerCfg.Height, CViewport(),
                      ViewerCfg.Depth, ViewerCfg.Windowed, EmptyWindow, false,
                      false);

...
public:
  ...
  virtual void run() {
    while (Running) {
      ...
      m_ColorModifier.convertBitmap(DestBitmap, ColorBitmap, MaskBitmap,
                                    retDeltaHue);
      BitmapMutex.leave();
  ...
  CColorThread()
      : PanoplyPreview(NULL), BitmapsOk(false), Hue(0), Lightness(0),
        Saturation(0), Luminosity(0), Contrast(0), Process(false),
        Running(true) {}
...
void CPanoplyPreview::displayBitmap(
    const CBitmap &bitmap) // Called from thread!
{
  ...
  if (bitmap.getWidth() != m_Image->width() ||
      bitmap.getHeight() != m_Image->height()) {
    QImage *image = m_Image;
    m_Image =
        new QImage(bitmap.getWidth(), bitmap.getHeight(), QImage::Format_RGB32);
    delete image;
  }
  ...
  for (uint32 y = 0; y < bitmap.getHeight(); ++y) {
    ...
    const uint8 *src =
        (const uint8 *)&buffer[y * bitmap.getWidth() * sizeof(uint32)];
    ...
    // memcpy(m_Image->scanLine(y), &buffer[y * bitmap.getWidth() *
    // sizeof(uint32)], sizeof(uint32) * bitmap.getWidth());
  }
...
void CPanoplyPreview::colorEdited(const QString &text) { m_ColorFile = text; }

void CPanoplyPreview::maskEdited(const QString &text) { m_MaskFile = text; }
  ...
  try {
    {
      ...
      if (!is.open(m_ColorFile.toLocal8Bit().data()))
        throw NLMISC::Exception("Cannot open file '%s'",
                                m_ColorFile.toLocal8Bit().data());
      uint32 depth = m_ColorThread->ColorBitmap.load(is);
      if (depth == 0 || m_ColorThread->ColorBitmap.getPixels().empty())
        throw NLMISC::Exception("Failed to load bitmap '%s'",
                                m_ColorFile.toLocal8Bit().data());
...
CSliderTextEdit::CSliderTextEdit(QWidget *parent, QLineEdit *lineEdit,
                                 float scale)
    : QSlider(Qt::Horizontal, parent), m_LineEdit(lineEdit), m_Updating(false),
      m_Scale(scale) {
  ...
  connect(lineEdit, SIGNAL(textEdited(const QString &)), this,
          SLOT(lineEditTextEdited(const QString &)));
}
...
void CPanoplyPreview::createDockWindows(CMainWindow *mainWindow) {
  ...
  // Color Modifier
  {
    ...
    // Input File Paths
    {
      ...
      colorFile->setText("W:\\database\\stuff\\fyros\\agents\\_"
                         "textures\\actors\\fy_hof_armor00_arm01_c1.png");
      groupLayout->addWidget(colorFile, 0, 1);

      m_ColorFile = colorFile->text();
      connect(colorFile, SIGNAL(textEdited(const QString &)), this,
              SLOT(colorEdited(const QString &)));
      ...
      maskFile->setText(
          "W:\\database\\stuff\\fyros\\agents\\_textures\\actors\\mask\\fy_hof_"
          "armor00_arm01_c1_skin.png");
      groupLayout->addWidget(maskFile, 1, 1);

      m_MaskFile = maskFile->text();
      connect(maskFile, SIGNAL(textEdited(const QString &)), this,
              SLOT(maskEdited(const QString &)));

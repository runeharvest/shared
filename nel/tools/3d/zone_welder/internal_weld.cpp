...
struct CPatchVertexInfo {
  uint ZoneIndex; // this zone index, 0 for the midlle zone, and from 1 to 8 for
                  // the zones around
  ...
  CVector Pos;
  CPatchVertexInfo() {}
  CPatchVertexInfo(uint zoneIndex, uint patchIndex, uint patchVertex,
                   const CVector &pos)
      : ZoneIndex(zoneIndex), PatchIndex(patchIndex), PatchVertex(patchVertex),
        Pos(pos) {}
};
...
void bind_1_1(std::vector<CPatchInfo> &zoneInfos, uint patch0, uint edge0,
              uint patch1, uint edge1, uint zoneId) {
  // Bind type
...
void bind_1_2(std::vector<CPatchInfo> &zoneInfos, uint patch, uint edge,
              uint patch0, uint edge0, uint patch1, uint edge1, uint zoneId) {
  // Bind type
...
void bind_1_4(std::vector<CPatchInfo> &zoneInfos, uint patch, uint edge,
              uint patch0, uint edge0, uint patch1, uint edge1, uint patch2,
              uint edge2, uint patch3, uint edge3, uint zoneId) {
  // Bind type
...
static inline bool CanWeld(const CVector &v1, const CVector &v2,
                           float weldThreshold) {
...
uint getOtherCountAndPos(const std::vector<CPatchInfo> &zoneInfo, uint patch,
                         uint edge, uint &otherPos) {
  // Must be a multiple patch bind
  if (zoneInfo[patch].BindEdges[edge].NPatchs == 5) {
    uint i;
    const CPatchInfo &otherPatchRef =
        zoneInfo[zoneInfo[patch].BindEdges[edge].Next[0]];
    uint otherEdge = zoneInfo[patch].BindEdges[edge].Edge[0];
    for (i = 0; i < otherPatchRef.BindEdges[otherEdge].NPatchs; i++) {
      if ((otherPatchRef.BindEdges[otherEdge].Next[i] == patch) &&
          (otherPatchRef.BindEdges[otherEdge].Edge[i] == edge)) {
        otherPos = i;
...
static void GetCandidateVertices(const CVector &pos, TPVQuadGrid &qg,
                                 TPVVect &dest, uint patchToExclude,
                                 uint patchToExcludeZone, float weldThreshold,
                                 bool exclude) {
  dest.clear();
  ...
  for (TPVQuadGrid::CIterator it = qg.begin(); it != qg.end(); ++it) {
    if (::CanWeld((*it).Pos, pos, weldThreshold)) {
      if ((!exclude) || (!((*it).ZoneIndex == patchToExcludeZone &&
                           (*it).PatchIndex == patchToExclude))) {
        // Final distance test
...
bool isBinded(std::vector<CPatchInfo> &zoneInfos, uint patch0, uint edge0,
              uint patch1, uint edge1) {
  // Binded ?
  if ((zoneInfos[patch0].BindEdges[edge0].NPatchs != 0) &&
      (zoneInfos[patch1].BindEdges[edge1].NPatchs != 0)) {
    // Binded ?
...
CVector evalPatchEdge(CPatchInfo &patch, uint edge, float lambda) {
  ...
  return patch.Patch.eval((1.f - lambda) * indexToST[vIndex[0]][0] +
                              lambda * indexToST[vIndex[1]][0],
                          (1.f - lambda) * indexToST[vIndex[0]][1] +
                              lambda * indexToST[vIndex[1]][1]);
}
...
void CleanZone(std::vector<CPatchInfo> &zoneInfos, uint zoneId,
  ...
  // fill the quad grid
  float zoneSize =
      2.f * weldThreshold + std::max(zoneBBox.getMax().x - zoneBBox.getMin().x,
    ...
    for (l = 0; l < zoneInfos.size(); ++l) {
      ...
      for (m = 0; m < 4; ++m) {
        ...
        if (patch.BindEdges[m].NPatchs == 0) {
          ...
          // Get vertices from other patch that could be welded with this patch
          // boder's vertices.
          for (q = 0; q < 2; ++q) {
            ::GetCandidateVertices(patch.Patch.Vertices[vIndex[q]], qg,
                                   verts[q], l, zoneId, weldThreshold, true);
          }
          ...
          for (bindCount = 1; bindCount < 5; bindCount <<= 1) {
            ...
            // Compute points along the border and found list of vertex binded
            // to it.
            float lambda = middle;
            for (n = 1; n < bindCount; ++n) {
              CVector borderPos = evalPatchEdge(patch, m, lambda);
              ::GetCandidateVertices(borderPos, qg, binded4[n], l, zoneId,
                                     weldThreshold, true);
              lambda += middle;
            }
            ...
            for (q = 0; q < bindCount; q++) {
              for (n = 0; n < binded4[q].size(); n++) {
                for (p = 0; p < binded4[q + 1].size(); p++) {
                  ...
                  if (pv0.ZoneIndex == pv1.ZoneIndex &&
                      pv0.PatchIndex == pv1.PatchIndex) {
                    // Direct edge ?
            ...
            if (q == bindCount) {
              ...
              for (q = 0; q < bindCount - 1; q++) {
                if (!isBinded(zoneInfos, neighborPatches[q],
                              (neighborEdges[q] - 1) & 3,
                              neighborPatches[q + 1],
                              (neighborEdges[q + 1] + 1) & 3))
                  break;
              }
              ...
              if (q == (bindCount - 1)) {
                // Bind it
                if (bindCount == 1) {
                  bind_1_1(zoneInfos, l, m, neighborPatches[0],
                           neighborEdges[0], zoneId);
                  bind1Count++;
                } else if (bindCount == 2) {
                  bind_1_2(zoneInfos, l, m, neighborPatches[0],
                           neighborEdges[0], neighborPatches[1],
                           neighborEdges[1], zoneId);
                  bind2Count++;
                } else {
                  bind_1_4(zoneInfos, l, m, neighborPatches[0],
                           neighborEdges[0], neighborPatches[1],
                           neighborEdges[1], neighborPatches[2],
                           neighborEdges[2], neighborPatches[3],
                           neighborEdges[3], zoneId);
                  bind4Count++;
                }
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if ((bindCount == 2) || (bindCount == 4)) {
        ...
        for (vert = 1; vert < bindCount; vert++) {
          ...
          // yes, insert it in the tree
          const CVector half(weldThreshold, weldThreshold, weldThreshold);
          qg.insert(borderPos - half, borderPos + half,
                    CPatchVertexInfo(zoneId, l, 5, borderPos));
  ...
  for (l = 0; l < zoneInfos.size(); ++l) {
    ...
    for (vert = 0; vert < 4; ++vert) {
      ...
      if ((patch.BindEdges[vert].NPatchs != 0) &&
          (patch.BindEdges[(vert - 1) & 3].NPatchs != 0)) {
        // Welded ?
        bool welded = false;
        ...
        ::GetCandidateVertices(pos, qg, toWeld, l, zoneId, weldThreshold,
                               false);
        ...
        if (welded) {
          ...
          for (w = 0; w < toWeld.size(); w++) {
            if (toWeld[w]->PatchVertex != 5) {
              ...
              zoneInfos[toWeld[w]->PatchIndex]
                  .Patch.Vertices[toWeld[w]->PatchVertex] = average;
            }
          }
        }
      }
  ...
  for (l = 0; l < zoneInfos.size(); ++l) {
    ...
    for (edge = 0; edge < 4; ++edge) {
      ...
      if (/*(bindCount == 1) || */ (bindCount == 5)) {
        ...
        nlassert(
            ((bindCount == 1) && (otherCount == 1)) ||
            ((bindCount == 5) && ((otherCount == 2) || (otherCount == 4))));
        ...
        for (tang = 0; tang < 2; tang++) {
          ...
          // Next offset
          float dist =
              (patch.Patch.Tangents[2 * edge + tang] - tangVect).norm();
          if ((patch.Patch.Tangents[2 * edge + tang] - tangVect).sqrnorm() >
              0.0001)
            weldCount++;

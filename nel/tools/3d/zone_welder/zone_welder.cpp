...
/*******************************************************************\
                        findPatchIndex()
\*******************************************************************/
bool getPatchAndEdge(const std::vector<CPatchInfo> &patchs, uint16 baseVertex1,
                     uint16 baseVertex2, uint16 &patchIndex, uint8 &edgeIndex) {
  uint ptch;
    ...
    for (i = 0; i < 4; i++) {
      if (patchs[ptch].BaseVertices[i] == baseVertex1) {
        ...
        fprintf(fdbg, "patch %d continent bv %d : %d %d %d %d\n", ptch,
                baseVertex1, patchs[ptch].BaseVertices[0],
                patchs[ptch].BaseVertices[1], patchs[ptch].BaseVertices[2],
...
void CleanZone(std::vector<CPatchInfo> &zoneInfos, uint zoneId,
               const CAABBoxExt &zoneBBox, float weldThreshold);
...
void weldZones(const char *center) {
  ...
  if (centerZoneId == 0) {
    ...
    for (itptch = centerZonePatchs.begin(); itptch != centerZonePatchs.end();
         itptch++) {
      for (j = 0; j < 4; j++) {
    ...
    for (itbv = centerZoneBorderVertices.begin();
         itbv < centerZoneBorderVertices.end(); itbv++) {
  ...
  //================================================
  // NB: do it only for edges sharing 2 patchs of centerZone. (don't care
  // adjacent zones). smoothing with adjacent zones is done with a better
  // smoothing tool: CZoneTgtSmoother, see below, after the weld of the zone.
  {
  ...
  for (i = 0; i < 8; i++) {
    ...
    try {
      ...
      if (f.open(ss)) {
        ...
        adjZones[i].serial(f);
        adjZones[i].retrieve(adjZoneInfos[i].Patchs,
                             adjZoneInfos[i].BorderVertices);
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (adjZonesId[i] == 0) {
      ...
      // edge neighbour : current zone
      for (itptch = adjZonePatchs.begin(); itptch != adjZonePatchs.end();
           itptch++) {
        for (j = 0; j < 4; j++) {
      ...
      // border vertices neighbour : current zone
      for (itbv = adjZoneBorderVertices.begin();
           itbv != adjZoneBorderVertices.end(); itbv++) {
    ...
    fprintf(fdbg, "(before) zone %u bordervertices size : %u\n", i,
            (uint)adjZoneBorderVertices.size());

    // delete border vertices of the adjacent zone if their neighbour zoneId
    // is equal to current zone zoneId
    std::vector<CBorderVertex>::iterator itborder =
        adjZoneBorderVertices.begin();
    ...
    for (ptch = 0; ptch < adjZonePatchs.size(); ptch++) {
      for (j = 0; j < 4; j++) {
        ...
        wvinf.PatchVertex = j;
        if (adjBaseVertexIndexSet.find(wvinf.IndexInZone) ==
            adjBaseVertexIndexSet.end()) {
    ...
    for (ptch = 0; ptch < centerZonePatchs.size();
         ptch++) // for all patchs in center zone
    {
      ...
      // for every points in center patch we look for close points in adjacent
      // patch
      for (j = 0; j < 4; j++) // 4 patch vertices (in center zone)
      {
        ...
        CWeldableVertexInfos wvinf;
        float minDistance =
            weldRadius + 1; // rq: we weld only if we found a distance
        ...
        while (itqdt != quadTrees[i].end()) {
          CVector vadj;
          vadj.x = adjZonePatchs[(*itqdt).PatchIndex]
                       .Patch.Vertices[(*itqdt).PatchVertex]
                       .x;
          vadj.y = adjZonePatchs[(*itqdt).PatchIndex]
                       .Patch.Vertices[(*itqdt).PatchVertex]
                       .y;
          vadj.z = adjZonePatchs[(*itqdt).PatchIndex]
                       .Patch.Vertices[(*itqdt).PatchVertex]
                       .z;
...
#ifdef NL_DEBUG_WELD
            nlverify(!isTheSame(centerZonePatchs[ptch].Patch.Vertices[j],
                                NL_DEBUG_WELD_V0));
            nlverify(!isTheSame(centerZonePatchs[ptch].Patch.Vertices[j],
                                NL_DEBUG_WELD_V1));
#endif // NL_DEBUG_WELD
        ...
        if (minDistance < weldRadius) // i.e if we have found 2 vertices to weld
        {
          ...
          if (centerBaseVertexIndexSet.find(
                  centerZonePatchs[ptch].BaseVertices[j]) !=
              centerBaseVertexIndexSet.end()) {
            if (currentAdjBaseVertexIndexSet.find(wvinf.IndexInZone) ==
                currentAdjBaseVertexIndexSet.end()) {
              ...
              fprintf(
                  fdbg,
                  "%d) weld vertices : zone%d.(patch%d.vertex%d).baseVertex%d "
                  "to centerZone.(patch%d.vertex%d).baseVertex%d\n",
                  weldCount, i, wvinf.PatchIndex, wvinf.PatchVertex,
                  wvinf.IndexInZone, ptch, j,
                  centerZonePatchs[ptch].BaseVertices[j]);
#endif
            }
          }
      ...
      for (j = 0; j < 4; j++) {
#ifdef NL_DEBUG_WELD
        if ((isTheSame(centerZonePatchs[ptch].Patch.Vertices[j],
                       NL_DEBUG_WELD_V0) ||
             isTheSame(centerZonePatchs[ptch].Patch.Vertices[(j + 1) % 4],
                       NL_DEBUG_WELD_V0)) &&
            (isTheSame(centerZonePatchs[ptch].Patch.Vertices[j],
                       NL_DEBUG_WELD_V1) ||
             isTheSame(centerZonePatchs[ptch].Patch.Vertices[(j + 1) % 4],
                       NL_DEBUG_WELD_V1)))
          nlstop;
       ...
       // if vertex has been welded...
        if (toWeld[j] == false)
          continue;
        // ...we look if next vertex(i.e if the edge) in center zone has to be
        // welded
        if (toWeld[(j + 1) % 4] == false)
          continue;
        ...
        uint8 edgeIndex;
        uint16 patchIndex;
        if (!getPatchAndEdge(adjZonePatchs, nearVertexInfos[j].IndexInZone,
                             nearVertexInfos[(j + 1) % 4].IndexInZone,
                             patchIndex, edgeIndex)) {
#if WELD_LOG
          fprintf(fdbg,
                  "* Error * : Can't find patch containing the following edge "
                  ": %d - %d\n",
                  nearVertexInfos[j].IndexInZone,
                  nearVertexInfos[(j + 1) % 4].IndexInZone);
#endif
          nlwarning("ERROR : zone_welder : Can't find patch containing the "
                    "following edge : %d - %d\n",
                    nearVertexInfos[j].IndexInZone,
                    nearVertexInfos[(j + 1) % 4].IndexInZone);
          continue;
...
#if WELD_LOG
        fprintf(fdbg,
                "weld edges : zone%d.patch%d.edge%d(%d-%d) to "
                "centerZone.patch%d.edge%d(%d-%d)\n",
                i, patchIndex, edgeIndex, nearVertexInfos[j].IndexInZone,
                nearVertexInfos[(j + 1) % 4].IndexInZone, ptch, j,
                centerZonePatchs[ptch].BaseVertices[j],
        ...
        if (adjZonePatchs[patchIndex].BindEdges[edgeIndex].NPatchs != 0) {
          ...
          getZoneNameByCoord(centerZoneId & 0xff, (centerZoneId >> 8) + 1,
                             nameCenter);
          getZoneNameByCoord(adjZonesId[i] & 0xff, (adjZonesId[i] >> 8) + 1,
                             nameAdj);

          // Main message
          smprintf(buf, 2048,
                   "Bind Error: try to bind the patch n %d in zone n %s with "
                   "patch n %d in zone %s\n"
                   "This patch is already binded with the following patches : ",
                   ptch + 1, nameAdj.c_str(), patchIndex + 1,
                   nameCenter.c_str());
          sserror << buf;

          // Sub message
          for (uint i = 0;
               i < adjZonePatchs[patchIndex].BindEdges[edgeIndex].NPatchs;
               i++) {
            // Last patch ?
            bool last =
                (i ==
                 (uint)(adjZonePatchs[patchIndex].BindEdges[edgeIndex].NPatchs -
                        1));

            // Sub message
            smprintf(buf, 2048, "patch n %d%s",
                     adjZonePatchs[patchIndex].BindEdges[edgeIndex].Next[i] + 1,
                     last ? "\n" : ",");
          ...
          CVector middle0 =
              (centerZonePatchs[ptch].Patch.Tangents[2 * j] +
               adjZonePatchs[patchIndex].Patch.Tangents[2 * edgeIndex + 1]) /
              2;
          CVector middle1 =
              (centerZonePatchs[ptch].Patch.Tangents[2 * j + 1] +
               adjZonePatchs[patchIndex].Patch.Tangents[2 * edgeIndex]) /
              2;
  ...
  //====================================
  // do it before "make coplanar beetween zones", because CZoneTgtSmoother use
  // tangents and vertices to smooth.
  {
    ...
    // For all borderVertices of centerZone, choose the good vertex, add
    // neighbor influence
    uint bv;
    for (bv = 0; bv < centerZoneBorderVertices.size(); bv++) {
      ...
      if (adjVertices.find(centerBV.NeighborZoneId) != adjVertices.end()) {
        outVertices[centerVert].Vertex +=
            adjVertices[centerBV.NeighborZoneId][centerBV.NeighborVertex]
                .Vertex;
      ...
      for (ptch = 0; ptch < adjZonePatchs.size(); ptch++) {
        ...
        for (uint corner = 0; corner < 4; corner++) {
          ...
          if (verts[idVert].OnBorder) {
            pa.Patch.Vertices[corner] =
                outVertices[verts[idVert].IdOnCenterZone].Vertex;
          }
        }
      }
    }
  ...
  adjZoneInfos[i].Patchs; std::vector<CBorderVertex>	&adjZoneBorderVertices=
  adjZoneInfos[i].BorderVertices;

          adjZones[i].build(adjZonesId[i], adjZonePatchs,
  adjZoneBorderVertices); land.addZone(adjZones[i]);
      ...
      if(centerZone)
      {
          ...
          // get result from the compiled zone, and copy in the uncompiled one
  (ie in centerZonePatchs). for(i=0;i<centerZonePatchs.size();i++)
          {
              const CPatch	&paSrc= *((const
  CZone*)centerZone)->getPatch(i); CPatchInfo		&paDst=
  centerZonePatchs[i]; for(uint corner=0; corner<4; corner++)
                  paDst.setCornerSmoothFlag(corner,
  paSrc.getCornerSmoothFlag(corner));
          }
      }
  }*/
  ...
  if (errorMessage.empty()) {
    ...
    // Main message
    nlwarning("ERROR weld failed. Correct errors below: (indices are MAX "
              "indices (+1))\n");

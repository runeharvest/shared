...
std::string
    s_ReferenceDir; /* R:\pipeline\export\continents\r2_desert\zone_weld */
...
// unbuild_elevation --land "R:\graphics\landscape\ligo\desert\r2_desert.land"
// "Y:\temp\r2_desert_elevation.Py" "R:\reference\2008_july\data\r2_desert"
// "R:\pipeline\export\continents\r2_desert\zone_weld"
// --land "R:\graphics\landscape\ligo\jungle\zorai.land" --referenceext zonenhw
// "X:\wsl\big_zorai.py" "R:\reference\2008_july\data\zorai_zones"
// "R:\pipeline\export\continents\zorai\zone_weld"

// --land "R:\graphics\landscape\ligo\desert\r2_desert.land"
// "X:\wsl\big_r2_desert.py" "R:\reference\2008_july\data\r2_desert"
// "R:\pipeline\export\continents\r2_desert\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\jungle\r2_jungle.land"
// "X:\wsl\big_r2_jungle.py" "R:\reference\2008_july\data\r2_jungle"
// "R:\pipeline\export\continents\r2_jungle\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\jungle\r2_forest.land"
// "X:\wsl\big_r2_forest.py" "R:\reference\2008_july\data\r2_forest"
// "R:\pipeline\export\continents\r2_forest\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\lacustre\r2_lakes.land"
// "X:\wsl\big_r2_lakes.py" "R:\reference\2008_july\data\r2_lakes"
// "R:\pipeline\export\continents\r2_lakes\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\primes_racines\r2_roots.land"
// "X:\wsl\big_r2_roots.py" "R:\reference\2008_july\data\r2_roots"
// "R:\pipeline\export\continents\r2_roots\zone_weld" --extendcoords

// --land "R:\graphics\landscape\ligo\desert\r2_desert.land" --referenceext
// zonew "X:\wsl\check_r2_desert.py" "R:\reference\2008_july\data\r2_desert"
// "R:\pipeline\export\continents\r2_desert\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\jungle\r2_jungle.land" --referenceext
// zonew "X:\wsl\check_r2_jungle.py" "R:\reference\2008_july\data\r2_jungle"
// "R:\pipeline\export\continents\r2_jungle\zone_weld" --extendcoords
// --land "R:\graphics\landscape\ligo\primes_racines\r2_roots.land"
// --referenceext zonew "X:\wsl\check_r2_roots.py"
// "R:\reference\2008_july\data\r2_roots"
// "R:\pipeline\export\continents\r2_roots\zone_weld" --extendcoords
...
bool processZone(std::vector<NLMISC::CVector> &output,
  ...
  if (sourceVertices.size() != referenceVertices.size()) {
    nlwarning("Mismatching vertex count in zone %s, source vertices: %i, "
              "reference vertices: %i",
              zone.c_str(), (int)sourceVertices.size(),
              (int)referenceVertices.size());
    ++s_Warnings;
  }
  ...
  for (ptrdiff_t i = 0; i < (ptrdiff_t)referenceVertices.size(); ++i) {
    ...
    if (!sourceHeightDiv) {
      nlwarning("No matching vertices in source for zone %s, x: %f, y: %f",
                zone.c_str(), referenceVertices[i].x, referenceVertices[i].y);
      ++s_Warnings;
      continue;
    }

    if (referenceHeightDiv != sourceHeightDiv) {
      nlwarning("Mismatching vertex count for zone %s, x: %f, y: %f "
                "(reference: %i, source: %i)",
                zone.c_str(), referenceVertices[i].x, referenceVertices[i].y,
                referenceHeightDiv, sourceHeightDiv);
      ++s_Warnings;
      continue;
    }
    ...
    output.push_back(NLMISC::CVector(referenceVertices[i].x,
                                     referenceVertices[i].y,
...
bool unbuildElevation() {
  ...
  for (std::vector<std::string>::iterator it(referenceZones.begin()),
       end(referenceZones.end());
       it != end; ++it) {
    if (CFile::getExtension(*it) != s_ReferenceExt)
      continue;

    std::string zone = CFile::getFilenameWithoutExtension(*it);
    std::string sourceZone = s_SourceDir + "/" +
...

  {
    ...
    fprintf(fo, "import naturalneighbor # "
                "https://github.com/innolitics/natural-neighbor-interpolation "
                "- pip3 install naturalneighbor\n");
    ...
    fprintf(fo, "nn_interpolated_values = naturalneighbor.griddata(points, "
                "values, grid_ranges)\n");
    fprintf(fo, "img = []\n");
    fprintf(fo, "for y in range(0, %i):\n", h);
    fprintf(fo, "\tline = []\n");
    fprintf(fo, "\tfor x in range(0, %i):\n", w);
    fprintf(fo, "\t\tline.append(np.round(nn_interpolated_values[x][y][0])."
                "astype(int) + 127)\n");
...
bool unbuildElevation(NLMISC::CCmdArgs &args) {
  ...
  if (args.haveLongArg("zonemin") && args.haveLongArg("zonemax")) {
    ...
    nlwarning(
        "Must have either both 'zonemin' and 'zonemax', or 'land' specified");
    return false;
  }
...
int main(int argc, char **argv) {
  ...
  args.addAdditionalArg("source",
                        "Input folder with zones at the right height");
  args.addAdditionalArg("reference",
                        "Input folder with zones at the wrong height");

  args.addArg("", "sourceext", "extension",
              "Source zone extension (default: zonel)");
  args.addArg("", "referenceext", "extension",
              "Reference zone extension (default: zonew)");

  args.addArg("", "land", "land",
              "Ligo land file (either specify this or the boundaries)");
  ...
  args.addArg("", "extendcoords", "flag",
              "Extend coordinates to edge of bitmap pixels");

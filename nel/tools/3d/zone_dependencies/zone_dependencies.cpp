...
static void computeZoneIGBBox(const char *zoneName, CLightingBBox &result,
                              TShapeMap &shapeMap,
                              const TString2LightingBBox &additionnalIG);
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 5) {
    // Help message
    printf("zone_dependencies [properties.cfg] [firstZone.zone] "
           "[lastzone.zone] [output_dependencies.cfg]\n");
  } else {
    ...
    try {
      ...
      lightDirection.set(sun_direction.asFloat(0), sun_direction.asFloat(1),
                         sun_direction.asFloat(2));
      ...
      /*
                  ...
                  CConfigFile::CVar &shapes_path = properties.getVar
         ("shapes_path"); NLMISC::CPath::addSearchPath(shapes_path.asString(),
         true, true);
      */
      CConfigFile::CVar &compute_dependencies_with_igs =
          properties.getVar("compute_dependencies_with_igs");
      bool computeDependenciesWithIgs =
          compute_dependencies_with_igs.asInt() != 0;
        ...
        if (getZoneCoordByName(lastName.c_str(), lastX, lastY)) {
          ...
          for (y = firstY; y <= lastY; y++)
            for (x = firstX; x <= lastX; x++) {
              ...
              // Progress
              progress("Build bounding boxes",
                       (float)(x + y * lastX) / (float)(lastX * lastY));
              ...
              if (file.open(dir + zoneName + ext)) {
                ...
                try {
                  ...
                  if (computeDependenciesWithIgs) {
                    computeZoneIGBBox(zoneName.c_str(), igBBox, shapeMap,
                                      villagesBBox);
                  }
                  ...
                  zoneLBox.OccludingBox = zoneLBox.ReceivingBox =
                      zoneBox; // can't be void
                  ...
                  if (!zoneLBox.OccludingBox.IsVoid) {
                    quadGrid.insert(zoneLBox.ReceivingBox.Box.getMin(),
                                    zoneLBox.ReceivingBox.Box.getMax(),
                                    zoneDesc);
                  }
                  ...
                  // Index
                  uint index =
                      (x - firstX) + (y - firstY) * (lastX - firstX + 1);
                  ...
                  // Error handling
                  nlwarning("ERROR in file %s, %s",
                            (dir + zoneName + ext).c_str(), e.what());
                }
              }
            }
          ...
          for (y = firstY; y <= lastY; y++)
            for (x = firstX; x <= lastX; x++) {
              ...
              if (dependencies[index].Loaded) {
                ...
                // Make a corner array
                CVector corners[4] = {CVector(vMin.x, vMin.y, vMax.z),
                                      CVector(vMax.x, vMin.y, vMax.z),
                                      CVector(vMax.x, vMax.y, vMax.z),
                                      CVector(vMin.x, vMax.y, vMax.z)};
                ...
                for (corner = 0; corner < 4; corner++) {
                  ...
                  if (lightDirection.z != 0) {
                    ...
                    target = corners[corner] +
                             (lightDirection *
                              ((minZ - corners[corner].z) / lightDirection.z));
                  } else {
                ...
                while (it != quadGrid.end()) {
                  // Index
                  uint targetIndex = ((*it).X - firstX) +
                                     ((*it).Y - firstY) * (lastX - firstX + 1);

                  // Not the same
                  if (targetIndex != index) {
                    // Target min z
                    float targetMinZ =
                        dependencies[targetIndex].BBox.getMin().z;
                    if (targetMinZ < vMax.z) {
                      // Min z inf to max z ?
                      // Target optimized bbox
                      CAABBox bBoxOptimized =
                          dependencies[index].BBox.getAABBox();
                      ...
                      if ((*it).BBox.intersect(bBox)) {
                        // Insert in the set
                        dependencies[targetIndex].Dependences.insert(
                            CZoneDependenciesValue(x, y));
                      }
                    }
                  }
            ...
            for (x = firstX; x <= lastX; x++) {
              ...
              if (dependencies[index].Loaded) {
                ...
                if ((outputFile = nlfopen(toLowerAscii(outputFileName), "w"))) {
                  ...
                  // Add dependent zones
                  set<CZoneDependenciesValue>::iterator ite =
                      dependencies[index].Dependences.begin();
 ...
 * instance group \return true if the computed bbox is valid
 */
static void computeIGBBox(const NL3D::CInstanceGroup &ig, CLightingBBox &result,
                          TShapeMap &shapeMap) {
  result = CLightingBBox(); // starts with void result
  bool firstBBox = true;
  /// now, compute the union of all bboxs
  for (CInstanceGroup::TInstanceArray::const_iterator it =
           ig._InstancesInfos.begin();
       it != ig._InstancesInfos.end(); ++it) {
    ...
    } else // must load the shape to get its bbox
    {
      ...
      if (getExt(toLoad).empty())
        toLoad += ".shape";
      shapePathName = NLMISC::CPath::lookup(toLoad, false, false);

      if (shapePathName.empty()) {
        nlwarning("Unable to find shape '%s'", it->Name.c_str());
      } else if (toLowerAscii(CFile::getExtension(shapePathName)) ==
                 "pacs_prim") {
        nlwarning("EXPORT BUG: Can't read %s (not a shape), should not be part "
                  "of .ig!",
                  shapePathName.c_str());
      } else {
        ...
        if (shapeInputFile.open(shapePathName.c_str())) {
          ...
          try {
            ...
            // this prevent the case where a huge surface of water will cause
            // the zone it is attached to (the 'Zone' field in the villages
            // sheets) to load all the zones that the water surface cover. (This
            // caused an 'out of memory error' in the zone lighter due to too
            // many zone being loaded)

            // FIXME : test for water case hardcoded for now
            CWaterShape *ws =
                dynamic_cast<CWaterShape *>(shapeStream.getShapePointer());
            if (ws) {
              CAABBox bbox;
              shapeStream.getShapePointer()->getAABBox(bbox);
              currBBox.OccludingBox = CPotentialBBox(
                  bbox); // occluding box is used, though the water shape
                         // doesn't cast shadow -> the tiles flag ('above',
                         // 'intersect', 'below water') are updated inside the
                         // zone_lighter
              currBBox.ReceivingBox.IsVoid =
                  true; // no lighted by the zone lighter !!!
              currBBox.removeVoid();
          ...
          nlwarning("Unable to open shape file %s to get its bbox",
                    it->Name.c_str());
        }
      }
    }
 ...
 * \param additionnalIG a map that gives an additionnal ig for a zone from its
 * name (ryzom specific : used to compute village bbox) \return true if the
 * computed bbox is valid
 */
static void computeZoneIGBBox(const char *zoneName, CLightingBBox &result,
                              TShapeMap &shapeMap,
                              const TString2LightingBBox &additionnalIG) {
...
static void computeBBoxFromVillage(const NLGEORGES::UFormElm *villageItem,
                                   ...
                                   uint villageIndex, TShapeMap &shapeMap,
  ...
  if (!(villageItem->getNodeByName(&igNamesItem, "IgList") && igNamesItem)) {
    nlwarning("No list of IGs was found in the continent form %s, village #%d",
              continentName.c_str(), villageIndex);
    return;
  }
  ...
  for (uint l = 0; l < numIgs; ++l) {
    if (!(igNamesItem->getArrayNode(&currIg, l) && currIg)) {
      nlwarning("Couldn't get ig #%d in the continent form %s, in village #%d",
                l, continentName.c_str(), villageIndex);
      continue;
    }
    ...
    if (!igNameItem->getValue(igName)) {
      nlwarning("Couldn't get ig name of ig #%d in the continent form %s, in "
                "village #%d",
                l, continentName.c_str(), villageIndex);
      continue;
    }
    if (igName.empty()) {
      nlwarning("Ig name of ig #%d in the continent form %s, in village #%d is "
                "an empty string",
                l, continentName.c_str(), villageIndex);
      continue;
    }
 ...
 * \param parameter a config file that contains the name of the continent
 * containing the zones we are processing \param zone2bbox This will be filled
 * with the name of a zone and the bbox of the village it contains \param a map
 * of shape \param a vector that will be filled with a zone name and the bbox of
...
static void computeIGBBoxFromContinent(NLMISC::CConfigFile &parameter,
  ...
  try {
    ...
    CConfigFile::CVar &level_design_directory =
        parameter.getVar("level_design_directory");
    CConfigFile::CVar &level_design_world_directory =
        parameter.getVar("level_design_world_directory");
    CConfigFile::CVar &level_design_dfn_directory =
        parameter.getVar("level_design_dfn_directory");
    ...
    if (villageForm != NULL) {
      ...
      if (!(rootItem.getNodeByName(&villagesItem, "Villages") &&
            villagesItem)) {
      ...
      for (uint k = 0; k < numVillage; ++k) {
        ...
        if (!(villagesItem->getArrayNode(&currVillage, k) && currVillage)) {
          nlwarning("Couldn't get village %d in continent %s",
                    continentName.c_str(), k);
          continue;
        }
        // check that this village is in the dependency zones
        NLGEORGES::UFormElm *zoneNameItem;
        if (!currVillage->getNodeByName(&zoneNameItem, "Zone") &&
            zoneNameItem) {
          nlwarning("Couldn't get zone item of village %d in continent %s",
                    continentName.c_str(), k);
          continue;
        }
        std::string zoneName;
        if (!zoneNameItem->getValue(zoneName)) {
          nlwarning("Couldn't get zone name of village %d in continent %s",
                    continentName.c_str(), k);
          continue;
        }
        zoneName =
            NLMISC::toLowerAscii(CFile::getFilenameWithoutExtension(zoneName));
        CLightingBBox result;

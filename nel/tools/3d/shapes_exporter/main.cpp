...
// function to split a string into several substrings delimited by specified
// characters
...
sint APIENTRY WinMain(HINSTANCE /* hInstance */, HINSTANCE /* hPrevInstance */,
                      LPSTR cmdline, int /* nCmdShow */) {
  // we can specify several shapes on command line
  // so we need to process it
  std::vector<std::string> argv;

  // argv[0] is the fullpath to the executable but since it's not used, we can
  // let it empty
  argv.push_back("");
...
sint main(int argc, char **argv) {
  ...
  if (argc > 1) {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      exporter.exportShape(
          argv[i], CPath::standardizePath(exporter.settings.output_path));
    }
  } else {
    std::vector<std::string> filenames;

    // search all .max files
    CPath::getPathContent(exporter.settings.input_path, true, false, true,
                          filenames);
    //		CPath::getFileList("max", filenames);
    // search .max files corresponding to a filter
    // fix bad textures
    CPath::remapFile("ma_hof_armor_00_tibia_c1.tga",
                     "ma_hof_armor00_tibia_c1.png");
    CPath::remapFile("ma_hof_armor_00_foot_c1.tga",
                     "ma_hof_armor00_foot_c1.png");
    CPath::remapFile("ma_hof_armor_01_botte_c1.tga",
                     "ma_hof_armor01_botte_c1.png");
    CPath::remapFile("ma_hof_armor_01_pied_c1.tga",
                     "ma_hof_armor01_pied_c1.png");

    CPath::remapFile("ma_hom_armor_01_botte_c1.tga",
                     "ma_hom_armor01_botte_c1.png");
    CPath::remapFile("ma_hom_armor_01_pied_c1.tga",
                     "ma_hom_armor01_pied_c1.png");

    CPath::remapFile("hair_spec.tga", "spec_hair.png");

    CPath::remapFile("zo_hof_armor_00_mollet_c1.tga",
                     "zo_hof_armor00_mollet_c1.png");
    CPath::remapFile("zo_hof_armor_00_pied_c1.tga",
                     "zo_hof_armor00_pied_c1.png");

    CPath::remapFile("zo_hom_armor_00_mollet_c1.tga",
                     "zo_hom_armor00_mollet_c1.png");
    CPath::remapFile("zo_hom_armor_00_pied_c1.tga",
                     "zo_hom_armor00_pied_c1.png");
    ...
    //		CPath::getFileListByName("ps", "dustdoor", filenames); // trop
    //court 		CPath::getFileListByName("ps", "aura_recept", filenames);
    ...
    for (size_t i = 0; i < filenames.size(); ++i) {
      ...
      std::string baseFilename =
          toLowerAscii(CFile::getFilenameWithoutExtension(filenames[i]));
      ...
      std::string output_path =
          exporter.settings.output_path + md5.substr(0, 2) + "/" + md5;

      // file is an animation
      std::string animation; // CPath::lookup(baseFilename + ".anim", false,
                             // false, false);
      // file is a skeleton
      std::string skeleton; // CPath::lookup(baseFilename + ".skel", false,
                            // false, false);
      // file is a shape
      std::string shape =
          CPath::lookup(baseFilename + ".shape", false, false, false);
      ...
      if (!shape.empty() && false) {
        ...
        if (!CFile::isExists(output_path) &&
            !CFile::createDirectoryTree(output_path)) {
          nlwarning("can't create %s", output_path.c_str());
          continue;
        }

        if (in.open(shape) &&
            out.open(output_path + "/" + baseFilename + ".shape")) {
      ...
      if (!animation.empty()) {
        ...
        //				skeleton =
        //ShapesExporter::findSkeleton(shape);
        // render shape
        ...
        continue;
        /*
                        // create a temporary list with shapes which could
           correspond to .max file for(size_t j = 0; j < shapes.size(); ++j)
                        {

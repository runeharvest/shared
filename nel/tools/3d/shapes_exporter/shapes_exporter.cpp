...
bool ShapesExporter::init() {
  ...
  Driver->setDisplay(UDriver::CMode(
      (uint16)(settings.output_width * settings.output_antialiasing),
      (uint16)(settings.output_height * settings.output_antialiasing), 32,
      true));
...
bool ShapesExporter::parseConfigFile(const string &filename) {
  ...
  try {
    settings.input_path =
        CPath::standardizePath(cf.getVar("input_path").asString());
  ...
  try {
    settings.output_path =
        CPath::standardizePath(cf.getVar("output_path").asString());
  } catch (const EUnknownVar &) {
  }
  ...
  try {
    ...
    for (uint i = 0; i < (uint)recursive_search_pathes.size(); ++i)
      CPath::addSearchPath(
          CPath::standardizePath(recursive_search_pathes.asString(i)), true,
          false);
  } catch (const EUnknownVar &) {
  }
  ...
  try {
    ...
    settings.light_direction =
        CVector(var.asFloat(0), var.asFloat(1), var.asFloat(2));
  } catch (const EUnknownVar &) {
  ...
  try {
    settings.output_antialiasing =
        (uint8)cf.getVar("output_antialiasing").asInt();
  } catch (const NLMISC::EUnknownVar &) {
...
bool ShapesExporter::setupLight(const CVector &position,
  ...
  //	Light->setupDirectional(settings.light_ambiant, settings.light_diffuse,
  //settings.light_specular, settings.light_direction);
  Light->setupPointLight(settings.light_ambiant, settings.light_diffuse,
                         settings.light_specular, position,
                         direction + settings.light_direction);
    ...
    for (uint s = 0; s < 5; ++s) {
      ...
      for (uint i = 0; i < numMat; ++i) {
        ...
        for (sint j = 0; j <= mat.getLastTextureStage(); ++j) {
          ...
          if (mat.isTextureFile(j) &&
              mat.getTextureFileName(j) == "CTextureMultiFile:Dummy")
            allGood = false;
        }
      }
...
bool ShapesExporter::exportShape(const string &filename,
  ...
  if (CFile::getExtension(filename) == "ps") {
    UParticleSystemInstance *psi =
        static_cast<UParticleSystemInstance *>(&Entity);
    if (psi) {
...
bool ShapesExporter::exportSkeleton(const string &skeleton,
...
/*
bool ShapesExporter::exportAnimation(const std::string &animation, const
std::string &skeleton, const std::vector<std::string> &parts, const std::string
&output_path)
{
...
//	playlist->setWeightSmoothness(newSlot, 1.0f);
    ...
    Camera.lookAt(CVector(center.x + dist - radius, center.y, center.z),
center);
...
bool ShapesExporter::renderShape(UTransform &entity,
                                 const string &output_path) {
  CQuat quat(0.f, 0.f, 0.f, 0.f);
  ...
  for (uint step_z = 0; step_z < settings.output_steps_z; ++step_z) {
    CQuat z(axis1,
            orientation1 * (float)step_z *
                ((float)NLMISC::Pi * 2.f / (float)settings.output_steps_z));

    for (uint step_x = 0; step_x < settings.output_steps_x; ++step_x) {
      CQuat x(axis2,
              orientation2 * (float)step_x *
                  ((float)NLMISC::Pi * 2.f / (float)settings.output_steps_x));

      entity.setRotQuat(quat * z * x);

      string filename = CPath::standardizePath(output_path) +
                        toString("%03d_%03d.%s", step_z, step_x,
                                 settings.output_format.c_str());
...
bool ShapesExporter::renderPS(UInstance &entity, const string &output_path,
                              double &duration, CAABBox &bbox) {
  ...
  while (true) {
    ...
    if (duration > 0.0f) {
      string filename =
          CPath::standardizePath(output_path) +
          toString("000_%03d.%s", step, settings.output_format.c_str());
      step++;
      if (!saveOneImage(filename))
        return false;
    }
    if (time >= 30.0 || (nbparticle == 0 && startTime > 0.0f) ||
  ...
  if (duration > 0.0f) {
    if (step > 0) {
      FILE *fp =
          nlfopen(CPath::standardizePath(output_path) + "nb_steps.txt", "w");
      if (fp) {
        fprintf(fp, "%d", step);
        fclose(fp);
      }
    }
    nlinfo("PS duration %f after %f with nothing with %d steps, dt %f",
           duration, startTime, step, deltaTime);
  }
...
bool ShapesExporter::createThumbnail(const string &filename,
                                     const string &path) {
  ...
  if (CFile::getExtension(filename) == "ps") {
    FILE *fp =
        nlfopen(CPath::standardizePath(output_path) + "nb_steps.txt", "r");
    if (fp) {
      char str[100];
      if (!fgets(str, 100, fp))
        strcpy(str, "0");
  ...
  if (in.open(output_path + toString("/000_%03d.", selectedFrame) +
    ...
    if (out.open(output_path + "/" +
                 CFile::getFilenameWithoutExtension(filename) + "." +
                 settings.preview_format)) {
...
std::string ShapesExporter::findSkeleton(const std::string &shape) {
  ...
  // work in 60% of cases
  std::string skeleton =
      CPath::lookup(baseFilename + ".skel", false, false, false);
  ...
  if (pos != std::string::npos) {
    skeleton = CPath::lookup(baseFilename.substr(0, pos) + ".skel", false,
                             false, false);

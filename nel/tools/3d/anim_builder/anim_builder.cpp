...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    ...
    try {
      ...
      try {
        CConfigFile::CVar &anim_low_precision_tracks =
            parameter.getVar("anim_low_precision_tracks");
        uint lpt;
        for (lpt = 0; lpt < (uint)anim_low_precision_tracks.size(); lpt++) {
          animationOptimizer.addLowPrecisionTrack(
              anim_low_precision_tracks.asString(lpt));
        }
      } catch (const EUnknownVar &) {
        nlwarning("\"anim_low_precision_tracks\" not found in the parameter "
                  "file. Add \"Finger\" and \"Ponytail\" by default");
      ...
      // Sample Rate.
      try {
        CConfigFile::CVar &anim_sample_rate =
            parameter.getVar("anim_sample_rate");
        ...
        nlwarning("\"anim_sample_rate\" not found in the parameter file. Use "
                  "Default of 30 fps.");
        animationOptimizer.setSampleFrameRate(30);
      }
      ...
      for (uint iFile = 0; iFile < listFile.size(); iFile++) {
        ...
        if (CFile::getExtension(igFile) == "anim") {
          ...
          // progress
          printf("Anim builded: %4d. Anim Skipped: %4d\r", numBuilded,
                 numSkipped);
        }
      }

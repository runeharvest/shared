...
void GetCWD(int length, char *dir) { GetCurrentDirectoryA(length, dir); }

bool ChDir(const char *path) { return SetCurrentDirectoryA(path); }
void dir(const std::string &sFilter, std::vector<std::string> &sAllFiles,
         bool bFullPath) {
...
void GetCWD(int length, char *directory) { getcwd(directory, length); }
bool ChDir(const char *path) { return (chdir(path) == 0 ? true : false); }
void dir(const string &sFilter, vector<string> &sAllFiles, bool bFullPath) {
  ...
  if ((dp = opendir(sCurDir)) == NULL) {
    string sTmp = string("ERROR :  Can't open the dir : \"") + string(sCurDir) +
                  string("\"");
    outString(sTmp);
    return;
  }
  ...
  std:
    string sFileName = std::string(dirp->d_name);
    if (sFileName
            .substr((sFileName.length() - sFilter.length()), sFilter.length())
...
bool tryAllPos(NLMISC::CBitmap *pSrc, NLMISC::CBitmap *pDst, sint32 &x,
               sint32 &y) {
  uint32 i, j;
  ...
  if (nSrcWidth > pDst->getWidth())
    return false;
  if (nSrcHeight > pDst->getHeight())
    return false;
  ...
  for (b = 0; b < pSrc->getHeight(); ++b)
    for (a = 0; a < pSrc->getWidth(); ++a)
      if (rSrcPix[4 * (a + b * pSrc->getWidth()) + 3] != 0) {
        if (rDstPix[4 * ((x + a) + (y + b) * pDst->getWidth()) + 3] != 0)
          return false;
        rDstPix[4 * ((x + a) + (y + b) * pDst->getWidth()) + 0] =
            rSrcPix[4 * (a + b * pSrc->getWidth()) + 0];
        rDstPix[4 * ((x + a) + (y + b) * pDst->getWidth()) + 1] =
            rSrcPix[4 * (a + b * pSrc->getWidth()) + 1];
        rDstPix[4 * ((x + a) + (y + b) * pDst->getWidth()) + 2] =
            rSrcPix[4 * (a + b * pSrc->getWidth()) + 2];
        rDstPix[4 * ((x + a) + (y + b) * pDst->getWidth()) + 3] =
            rSrcPix[4 * (a + b * pSrc->getWidth()) + 3];
      }
  return true;
}
...
// ---------------------------------------------------------------------------
void FlagVertices(CMeshGeom &mg, uint InMatID,
                  vector<bool> &verticesNeedRemap) {
  CVertexBuffer &vertexBuffer =
      const_cast<CVertexBuffer &>(mg.getVertexBuffer());
  ...
  for (matrixBlock = 0; matrixBlock < nbMatrixBlock; matrixBlock++) {
    ...
    for (renderPass = 0; renderPass < numRenderPass; renderPass++) {
      ...
      {
        // Get primitives
        const CIndexBuffer &primitiveBlock =
            mg.getRdrPassPrimitiveBlock(matrixBlock, renderPass);
...
void FlagVerticesMRM(CMeshMRMGeom &mg, uint InMatID,
                     vector<bool> &verticesNeedRemap) {
  CVertexBuffer &vertexBuffer =
      const_cast<CVertexBuffer &>(mg.getVertexBuffer());
...
int main(int nNbArg, char **ppArgs) {
  ...
  if (!ChDir(ppArgs[1])) {
    outString(string("ERROR : directory ") + ppArgs[1] +
              " do not exists or access is denied\n");
    return -1;
  }
  ...
  if (!ChDir(ppArgs[2])) {
    outString(string("ERROR : directory ") + ppArgs[2] +
              " do not exists or access is denied\n");
    return -1;
  }
  ...
  for (uint32 nShp = 0; nShp < AllShapeNames.size(); ++nShp) {
    try {
      ...
      outString(string("ERROR: shape not found ") + AllShapeNames[nShp] +
                " - " + e.what());
      return -1;
    }
  }
  ...
  // **** Parse all mesh loaded, to flag each lightmap if 8 bit or not (NB: all
  // layers should be same mode)
  ...
  for (uint32 lmc8bitMode = 0; lmc8bitMode < 2; ++lmc8bitMode)
    for (uint32 nNbLayer = 0; nNbLayer < 256; ++nNbLayer) {
      ...
      for (i = 0; i < (sint32)AllLightmapNames.size(); ++i) {
        bool lm8Bit =
            setLM8Bit.find(NLMISC::toLowerAscii(AllLightmapNames[i])) !=
            setLM8Bit.end();
        // if same mode
      ...
      */
      ...
      for (uint k = 0; k < AllLightmapNames.size(); ++k) {
        ...
        if (AllLightmapTags[k] == -1) {
          nlinfo(NLMISC::toString("Lightmap %s has no tag",
                                  AllLightmapNames[k].c_str())
                     .c_str());
        } else {
          nlinfo(NLMISC::toString(
                     "Lightmap %s has tag %d : %s", AllLightmapNames[k].c_str(),
                     (int)AllLightmapTags[k], tags[AllLightmapTags[k]].c_str())
                     .c_str());
        }
      }
      ...
      if (nNbLayer > 0)
        for (i = 0; i < (sint32)AllLightmapNames.size(); ++i) {
          ...
          for (k = 1; k <= (sint32)nNbLayer; ++k) {
            string sTmp3 =
                getBaseName(AllLightmapNames[i]) + NLMISC::toString(k) + ".tga";
          ...
          // Should delete all layers of this lightmap (in fact in lightmapnames
          // list we have only the name of the current layer)
          if (bFound) {
            sTmp2 = getBaseName(AllLightmapNames[i]);
            outString(string("ERROR: lightmaps ") + sTmp2 +
                      "*.tga not all the same size\n");
            for (k = 0; k < (sint32)AllLightmapNames.size(); ++k) {
              if (strnicmp(AllLightmapNames[k].c_str(), sTmp2.c_str(),
                           sTmp2.size()) == 0) {
      ...
      for (i = 0; i < (sint32)(AllLightmaps.size() - 1); ++i)
        for (j = i + 1; j < (sint32)AllLightmaps.size(); ++j) {
          ...
          if ((pBI->getWidth() * pBI->getHeight()) <
              (pBJ->getWidth() * pBJ->getHeight())) {
      ...
      for (i = 0; i < (sint32)AllLightmapNames.size(); ++i) {
        outString(NLMISC::toString("%d / %d\n", (int)i,
                                   (int)AllLightmapNames.size()));
        bool bAssigned = false;
        for (j = 0; j < i; ++j) {
          // Tags of both textures must match. We don't want to spread lightmap
          // chunk in bitmap whose other part aren't used by current ig
          // lightmaps (this wastes vram for nothing)
          if (AllLightmapTags[i] != AllLightmapTags[j])
            continue;

          // Try to place the texture i into the texture j
          // This can be done only if texture was exported from the same zone.
          ...
          if (tryAllPos(pBI, pBJ, x, y)) {
            ...
            if (!putIn(pBI, pBJ, x, y)) {
              outString(string("ERROR : cannot put reference lightmap ") +
                        AllLightmapNames[i] + " in " + AllLightmapNames[j]);
              return -1;
            }
            // Put texture i into texture j for all layers of the lightmap !
            for (k = 0; k <= (sint32)nNbLayer; ++k) {
              string sTexNameI = getBaseName(AllLightmapNames[i]) +
                                 NLMISC::toString(k) + ".tga";
              string sTexNameJ = getBaseName(AllLightmapNames[j]) +
                                 NLMISC::toString(k) + ".tga";
              NLMISC::CBitmap BitmapI;
              NLMISC::CBitmap BitmapJ;
              NLMISC::CIFile inFile;

              outString(NLMISC::toString(
                            "INFO : Transfering %s (tag = %d) in %s (tag = %d)",
                            sTexNameI.c_str(), (int)AllLightmapTags[i],
                            sTexNameJ.c_str(), (int)AllLightmapTags[j]) +
                        " at (" + NLMISC::toString(x) + "," +
                        NLMISC::toString(y) + ")\n");
              ...
              if (!putIn(&BitmapI, &BitmapJ, x, y)) {
                outString(string("ERROR : cannot put lightmap ") + sTexNameI +
                          " in " + sTexNameJ + "\n");
                return -1;
              }
            ...
            for (k = 0; k < (sint32)AllShapes.size(); ++k) {
              ...
              if (pMesh != NULL) {
                ...
                rVNR.resize(
                    pMesh->getMeshGeom().getVertexBuffer().getNumVertices(),
                    false);
              } else if (pMeshMRM != NULL) {
                VerticesNeedRemap.resize(1); // Only one meshmrmgeom
                vector<bool> &rVNR = VerticesNeedRemap[0];
                rVNR.resize(
                    pMeshMRM->getMeshGeom().getVertexBuffer().getNumVertices(),
                    false);
                ...
                for (m = 0; m < nNumSlot; ++m) {
                  ...
                  const CMeshGeom *pMG =
                      dynamic_cast<const CMeshGeom *>(&pMeshML->getMeshGeom(m));
                  if (pMG != NULL)
              ...
              for (m = 0; m < (sint32)nNbMat; ++m) {
                ...
                if (rMat.getShader() == CMaterial::LightMap) {
                  ...
                  while (rMat.getLightMap(stage) != NULL) {
                    ...
                    if (pTF != NULL) {
                      string sTexName =
                          NLMISC::toLowerAscii(getBaseName(pTF->getFileName()));
                      string sTexNameMoved = NLMISC::toLowerAscii(
                          getBaseName(AllLightmapNames[i]));
                      if (sTexName == sTexNameMoved) {
                        // We must remap the name and indicate to remap uvs
                        bMustRemapUV = true;
                        // string sNewTexName =
                        // NLMISC::toLowerAscii(getBaseName(AllLightmapNames[j]));
                        // sNewTexName +=
                        // NLMISC::toString(getLayerNb(pTF->getFileName())) +
                        // ".tga"; pTF->setFileName (sNewTexName);
                      }
                    }
                    ++stage;
                  }
                ...
                if (bMustRemapUV) // Flaggage of the vertices to remap
                {
                  if (pMesh != NULL) {
                    // Flag all vertices linked to face with material m
                    FlagVertices(const_cast<CMeshGeom &>(pMesh->getMeshGeom()),
                                 m, VerticesNeedRemap[0]);
                  } else if (pMeshMRM != NULL) {
                    FlagVerticesMRM(
                        const_cast<CMeshMRMGeom &>(pMeshMRM->getMeshGeom()), m,
                        VerticesNeedRemap[0]);
                  } else if (pMeshML != NULL) {
                    sint32 nNumSlot = pMeshML->getNumSlotMesh();
                    for (n = 0; n < nNumSlot; ++n) {
                      // Get the mesh geom
                      CMeshGeom *pMG = const_cast<CMeshGeom *>(
                          dynamic_cast<const CMeshGeom *>(
                              &pMeshML->getMeshGeom(n)));
                      if (pMG) {
                        // Flag the vertices
                        FlagVertices(*pMG, m, VerticesNeedRemap[n]);
                      } else {
                        // Get the mesh MRM geom
                        CMeshMRMGeom *pMMRMG = const_cast<CMeshMRMGeom *>(
                  ...
                  while (rMat.getLightMap(stage) != NULL) {
                    ...
                    if (pTF != NULL) {
                      ...
                      string sTexNameMoved = NLMISC::toLowerAscii(
                          getBaseName(AllLightmapNames[i]));
                      if (sTexName == sTexNameMoved) {
                        string sNewTexName = NLMISC::toLowerAscii(
                            getBaseName(AllLightmapNames[j]));
                        sNewTexName +=
                            NLMISC::toString(getLayerNb(pTF->getFileName())) +
                            ".tga";
              ...
              // We have now the list of vertices to remap for all material that
              // have been changed So parse this list and apply the
              // transformation : (uv * TexSizeI + decalXY) / TexSizeJ
              for (m = 0; m < (sint32)VerticesNeedRemap.size(); ++m) {
                CVertexBuffer *pVB;
                if (pMesh != NULL) {
                  pVB = const_cast<CVertexBuffer *>(
                      &pMesh->getMeshGeom().getVertexBuffer());
                } else if (pMeshMRM != NULL) {
                  pVB = const_cast<CVertexBuffer *>(
                      &pMeshMRM->getMeshGeom().getVertexBuffer());
                } else if (pMeshML != NULL) {
                  const CMeshGeom *pMG =
                      dynamic_cast<const CMeshGeom *>(&pMeshML->getMeshGeom(m));
              ...
              if (bMustSave) {
                try {
                  ...
                  outString(string("ERROR: cannot save shape ") +
                            AllShapeNames[k] + " - " + e.what());
                  return -1;
                }
              }
            }
  ...
  // **** Additionally, output or clear a "flag file" in a dir to info if a 8bit
  // lihgtmap or not

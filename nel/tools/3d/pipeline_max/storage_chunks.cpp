...
CStorageChunks::CStorageChunks(NLMISC::IStream &stream, sint64 size)
    : m_Stream(stream), m_Is64Bit(false) {
  if (size >= 2147483647L)
...
sint32 CStorageChunks::leaveChunk() {
  if (m_Stream.isReading()) {
    ...
    m_Stream.seek(
        sizePos,
        NLMISC::IStream::begin); // hopefully this correctly overwrites!!!
    uint32 sizeField = (uint32)sizeWithHeader | (uint32)currentChunk()->Id
                                                    << 31; // add container flag

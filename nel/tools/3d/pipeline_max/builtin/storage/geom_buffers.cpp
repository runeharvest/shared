...
CGeomPolyFaceInfo::CGeomPolyFaceInfo()
    : I1(0), Material(0), SmoothingGroups(0) {}
...
void CGeomPolyFaceInfo::serial(NLMISC::IStream &stream) {
  ...
  if (bitfield & 0x0001) { /*nldebug("i1");*/
    stream.serial(I1);
    nlassert(I1);
    bitfield &= ~0x0001;
  } else
    I1 = 0;
  if (bitfield & 0x0008) { /*nldebug("material");*/
    stream.serial(Material);
    nlassert(Material);
    bitfield &= ~0x0008;
  } else
    Material = 0;
  if (bitfield & 0x0010) { /*nldebug("smoothing");*/
    stream.serial(SmoothingGroups);
  ...
  if (bitfield & 0x0020) {
    /*nldebug("triangles");*/
    if (stream.isReading())
      Triangulation.resize(Vertices.size() - 3);
    else
      nlassert(Triangulation.size() == Vertices.size() - 3);
    for (std::vector<std::pair<uint32, uint32>>::size_type i = 0;
         i < Triangulation.size(); ++i) {
  ...
  if (bitfield)
    nlerror("Remaining bitfield value 0x%x, please debug and implement",
            (uint32)bitfield);
}
...
std::string CGeomPolyFaceInfo::toString() const {
  ...
  ss << ")";
  if (I1)
    ss << ", I1: " << "0x" << NLMISC::toString("%x", I1);
  if (Material)
    ss << ", M: " << Material;
  if (SmoothingGroups)
    ss << ", S: " << SmoothingGroups;
  if (Triangulation.size()) {
    ss << ", ( ";
    for (std::vector<std::pair<uint32, uint32>>::size_type i = 0;
         i < Triangulation.size(); ++i) {
...
std::string CGeomBuffers::className() const { return "GeomBuffers"; }

void CGeomBuffers::toString(std::ostream &ostream,
...
IStorageObject *CGeomBuffers::createChunkById(uint16 id, bool container) {
  ...
  case PMBS_GEOM_BUFFERS_TRI_C_INDEX_CHUNK_ID:
    nlassert(!container);
    return new CStorageArray<
        CGeomTriIndex>(); // nb: parse should check if the sizes match with
                          // tri_a size (which is the master index buffer)

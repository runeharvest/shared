...
public:
  ...
  // public // TODO: Simplify using templates and returning a specialized
  // storage object, auto-converted to the requested type.
  /*/// Gets a pointer to an appdata chunk buffer. Returns NULL if it does not
  exist. Size is returned in the size parameter. const uint8
  *read(NLMISC::CClassId classId, TSClassId superClassId, uint32 subId, uint32
  &size) const;
  /// Locks a pointer to an appdata chunk buffer for writing to with specified
  capacity. May return NULL if this chunk is unparsable or no memory can be
  allocated. uint8 *lock(NLMISC::CClassId classId, TSClassId superClassId,
  uint32 subId, uint32 capacity);
  /// Unlocks a pointer to an appdata chunk buffer, setting the final written
  size. void unlock(NLMISC::CClassId classId, TSClassId superClassId, uint32
  subId, uint32 size);
  /// Fills an appdata chunk buffer with specified data, which will be copied.
  void fill(NLMISC::CClassId classId, TSClassId superClassId, uint32 subId,
  ...
  template <typename T>
  T *getOrCreate(NLMISC::CClassId classId, TSClassId superClassId,
                 uint32 subId);
  /// Erases an appdata chunk.
  void erase(NLMISC::CClassId classId, TSClassId superClassId, uint32 subId);
...
template <typename T> T *CAppDataEntry::value() {
  if (m_Value) {
    ...
    if (result) {
      ...
      nlwarning(
          "AppData value has already been cast to another type, recasting, the "
          "previous chunk becomes invalid, this may not be intended");
...
template <typename T>
T *CAppData::get(NLMISC::CClassId classId, TSClassId superClassId,
                 uint32 subId) {
  ...
  if (it == m_Entries.end()) {
    nldebug("Trying to read non-existant key, this is allowed, returning NULL");
    return NULL;
  }
...
template <typename T>
T *CAppData::getOrCreate(NLMISC::CClassId classId, TSClassId superClassId,
                         uint32 subId) {
  if (m_ChunksOwnsPointers) {
    nlwarning("Not parsed");
    return NULL;

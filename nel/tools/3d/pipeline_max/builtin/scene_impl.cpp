...
const NLMISC::CClassId CSceneImpl::ClassId =
    NLMISC::CClassId(0x00002222, 0x00000000);
...
void CSceneImpl::parse(uint16 version, uint filter) {
  ...
  if (version > Version3)
    nlassert(m_TrackSetList);
}

void CSceneImpl::clean() { CReferenceMaker::clean(); }

void CSceneImpl::build(uint16 version, uint filter) {
  CReferenceMaker::build(version);
}

void CSceneImpl::disown() { CReferenceMaker::disown(); }

void CSceneImpl::init() { CReferenceMaker::init(); }
...
void CSceneImpl::toStringLocal(std::ostream &ostream, const std::string &pad,
                               uint filter) const {
...
void CSceneImpl::setReference(uint index, CReferenceMaker *reference) {
  ...
  case 11:
    ...
    break;
  default:
    nlerror("Unknown reference index %i entry <ptr=0x%x> (%s, 0x%x)", index,
            (uint32)(uint64)(void *)reference,
            reference->classDesc()->classId().toString().c_str(),
            reference->classDesc()->superClassId());
    break;
  }
}

uint CSceneImpl::nbReferences() const { return m_TrackSetList ? 12 : 11; }

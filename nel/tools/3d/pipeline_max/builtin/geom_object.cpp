...
CGeomObject::CGeomObject(CScene *scene)
    : CObject(scene), m_Unknown0900(NULL), m_GeomBuffers(NULL) {}
...
const NLMISC::CClassId CGeomObject::ClassId =
...
void CGeomObject::parse(uint16 version, uint filter) {
  if (filter == 0) {
    ...
    if (!m_ChunksOwnsPointers) {
      ...
      m_GeomBuffers = static_cast<STORAGE::CGeomBuffers *>(
          getChunk(PMB_GEOM_BUFFERS_CHUNK_ID));
    }
  }
}

void CGeomObject::clean() { CObject::clean(); }
...
void CGeomObject::build(uint16 version, uint filter) {
  if (filter == 0) {
    ...
    if (m_Unknown0900)
      putChunk(PMB_GEOM_UNKNOWN0900_CHUNK_ID, m_Unknown0900);
    if (m_GeomBuffers)
      putChunk(PMB_GEOM_BUFFERS_CHUNK_ID, m_GeomBuffers);
  }
}
...
void CGeomObject::init() { CObject::init(); }
...
void CGeomObject::toStringLocal(std::ostream &ostream, const std::string &pad,
                                uint filter) const {
  if (filter == 0) {
...
void CGeomObject::triangulatePolyFace(
    std::vector<STORAGE::CGeomTriIndex> &triangles,
  ...
  for (uint i = 0; i < nbCuts; ++i) {
    // Can travel both ways over cuts, but the first direction is handled
    // directly!
    // from_to[polyFace.Triangulation[i].first][polyFace.Triangulation[i].second]
    // = true;
    from_to[polyFace.Triangulation[i].second][polyFace.Triangulation[i].first] =
        true;
  }
  ...
  for (uint i = 0; i < nbCuts; ++i) {
    ...
    // from_to[polyFace.Triangulation[i].first][polyFace.Triangulation[i].second]
    // = false; // handled! Try to find a path that works

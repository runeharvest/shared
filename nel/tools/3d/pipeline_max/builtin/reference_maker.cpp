...
CReferenceMaker::CReferenceMaker(CScene *scene)
    : CAnimatable(scene), m_ReferenceMap(false), m_204B_Equals_2E(NULL),
      m_References2035Value0(0), m_Unknown2045(NULL), m_Unknown2047(NULL),
      m_Unknown21B0(NULL) {}
...
const NLMISC::CClassId CReferenceMaker::ClassId =
...
const CReferenceMakerSuperClassDesc
...
void CReferenceMaker::parse(uint16 version, uint filter) {
  ...
  if (!m_ChunksOwnsPointers) {
    CStorageArray<sint32> *references2034 =
        static_cast<CStorageArray<sint32> *>(
            getChunk(PMB_REFERENCES_2034_CHUNK_ID));
    CStorageArray<sint32> *references2035 =
        static_cast<CStorageArray<sint32> *>(
            getChunk(PMB_REFERENCES_2035_CHUNK_ID));
    if (references2034)
      nlassert(references2035 == NULL); // Apparently, there can be only one.
    if (references2035)
      nlassert(references2034 == NULL);
    m_204B_Equals_2E = static_cast<CStorageValue<uint8> *>(
        getChunk(PMB_204B_EQUALS_2E_CHUNK_ID));
    if (m_204B_Equals_2E)
      nlassert(m_204B_Equals_2E->Value ==
               0x2e); // Really, let me know when it has another value.

    // Parse contents
    if (references2034) {
      m_ReferenceMap = false; // NOTE: Plugins may check after parse if they
                              // parsed with the correct type, to find a pattern
      m_ArchivedChunks.push_back(references2034);
      for (std::vector<sint32>::size_type i = 0;
           i < references2034->Value.size(); ++i) {
        if (references2034->Value[i] > 0) {
          CReferenceMaker *referenceMaker = dynamic_cast<CReferenceMaker *>(
              container()->getByStorageIndex(references2034->Value[i]));
          if (!referenceMaker)
            nlerror("Reference maker %s %s, 0x%x is not a reference maker",
                    ucstring(container()
                                 ->getByStorageIndex(references2034->Value[i])
                                 ->classDesc()
                                 ->displayName())
                        .toUtf8()
                        .c_str(),
    ...
    if (references2035) {
      ...
      while (it != end) {
        ...
        if (!referenceMaker)
          nlerror("Reference maker %s, 0x%x is not a reference maker",
                  container()
                      ->getByStorageIndex(referenceindex)
                      ->classDesc()
                      ->classId()
                      .toString()
                      .c_str(),
    ...
    m_Unknown2045 = static_cast<CStorageRaw *>(
        getChunk(PMB_UNKNOWN2045_CHUNK_ID)); // not sure if this is part of
                                             // maker or target
    m_Unknown2047 = static_cast<CStorageRaw *>(
        getChunk(PMB_UNKNOWN2047_CHUNK_ID)); // not sure if this is part of
                                             // maker or target
    m_Unknown21B0 = static_cast<CStorageRaw *>(
        getChunk(PMB_UNKNOWN21B0_CHUNK_ID)); // not sure if this is part of
                                             // maker or target
  }
}

void CReferenceMaker::clean() {
  CAnimatable::clean(); // Nothing to do here, m_ArchivedChunks is cleaned
                        // (deleted) for us!
}
...
void CReferenceMaker::build(uint16 version, uint filter) {
  ...
  // TODO: Build contents
  // if (m_References2034) putChunk(PMB_REFERENCES_2034_CHUNK_ID,
  // m_References2034); if (m_References2035)
  // putChunk(PMB_REFERENCES_2035_CHUNK_ID, m_References2035);
  if (!m_ReferenceMap) {
    ...
    for (uint i = 0; i < nb; ++i) {
      ...
      if (referenceMaker) {
        ...
        references2035->Value.push_back(
            container()->getOrCreateStorageIndex(referenceMaker));
      }
    }
  ...
  if (m_204B_Equals_2E)
    putChunk(PMB_204B_EQUALS_2E_CHUNK_ID, m_204B_Equals_2E);
  if (m_Unknown2045)
    putChunk(PMB_UNKNOWN2045_CHUNK_ID, m_Unknown2045);
  if (m_Unknown2047)
    putChunk(PMB_UNKNOWN2047_CHUNK_ID, m_Unknown2047);
  if (m_Unknown21B0)
    putChunk(PMB_UNKNOWN21B0_CHUNK_ID, m_Unknown21B0);
}
...
void CReferenceMaker::init() { CAnimatable::init(); }
...
void CReferenceMaker::toStringLocal(std::ostream &ostream,
  ...
  if (nb) {
    ...
    for (uint i = 0; i < nb; ++i) {
      ...
      if (referenceMaker) {
        ...
        ostream << "> ";
        ostream << "("
                << ucstring(referenceMaker->classDesc()->displayName()).toUtf8()
                << ", " << referenceMaker->classDesc()->classId().toString()
                << ") ";
      }
    }
    ostream << "} ";
...
uint CReferenceMaker::nbReferences() const { return m_References.size(); }

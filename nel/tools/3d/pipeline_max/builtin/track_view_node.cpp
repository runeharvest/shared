...
CTrackViewNode::CTrackViewNode(CScene *scene)
    : CReferenceTarget(scene), m_Empty0140(NULL), m_Empty0150(NULL) {}
...
const NLMISC::CClassId CTrackViewNode::ClassId =
    NLMISC::CClassId(0x8d73b8aa, 0x90f2ee71);
...
void CTrackViewNode::parse(uint16 version, uint filter) {
  ...
  if (!m_ChunksOwnsPointers) {
    // Read unknown chunks
    m_Empty0140 =
        static_cast<CStorageRaw *>(getChunk(PMB_TVNODE_EMPTY0140_CHUNK_ID));
    if (m_Empty0140)
      nlassert(m_Empty0140->Value.empty());
    m_Empty0150 =
        static_cast<CStorageRaw *>(getChunk(PMB_TVNODE_EMPTY0150_CHUNK_ID));
    if (m_Empty0150)
      nlassert(m_Empty0140->Value.empty());

    // Read child nodes
    for (std::vector<TChild>::size_type i = 0; i < m_Children.size(); ++i) {
      m_Children[i].DisplayName =
          getChunkValue<ucstring>(PMB_TVNODE_DISPLAYNAME_CHUNK_ID);
      m_Children[i].Identifier =
          getChunkValue<NLMISC::CClassId>(PMB_TVNODE_IDENTIFIER_CHUNK_ID);
      m_Children[i].IsNotAnotherNode =
          getChunkValue<sint32>(PMB_TVNODE_ISNOTNODE_CHUNK_ID);
    }
  }
}

void CTrackViewNode::clean() { CReferenceTarget::clean(); }
...
void CTrackViewNode::build(uint16 version, uint filter) {
  ...
  if (m_Empty0140)
    putChunk(PMB_TVNODE_EMPTY0140_CHUNK_ID, m_Empty0140);
  if (m_Empty0150)
    putChunk(PMB_TVNODE_EMPTY0150_CHUNK_ID, m_Empty0150);
  ...
  for (std::vector<TChild>::size_type i = 0; i < m_Children.size(); ++i) {
    ...
    putChunkValue(PMB_TVNODE_ISNOTNODE_CHUNK_ID,
                  m_Children[i].IsNotAnotherNode);
  }
}
...
void CTrackViewNode::init() { CReferenceTarget::init(); }
...
void CTrackViewNode::toStringLocal(std::ostream &ostream,
  ...
  for (std::vector<TChild>::size_type i = 0; i < m_Children.size(); ++i) {
    ...
    ostream << "("
            ...
            << ") ";
    ostream << "(" << m_Children[i].DisplayName.toUtf8() << ", "
            << m_Children[i].Identifier.toString() << ", "
            << (m_Children[i].IsNotAnotherNode ? "ENTRY" : "TVNODE") << ") ";
  }
}
...
uint CTrackViewNode::nbReferences() const { return m_Children.size(); }

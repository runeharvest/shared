...
bool IStorageObject::getSize(sint32 &size) const { return false; }

bool IStorageObject::isContainer() const { return false; }
...
std::string
CStorageContainer::className() const // why is this not const in IClassable?
{
...
#endif
      CStorageChunks chunks(stream,
                            stream.isReading() ? storageStream->size() : 0);
      serial(chunks);
      return;
    }
  }
...
void CStorageContainer::toString(std::ostream &ostream,
  ...
  sint i = 0;
  for (TStorageObjectContainer::const_iterator it = m_Chunks.begin(),
                                               end = m_Chunks.end();
       it != end; ++it) {
    std::stringstream ss;
...
void CStorageContainer::parse(uint16 version, uint filter) {
  nlassert(m_ChunksOwnsPointers); // Can only use this when m_Chunks still has
                                  // ownership.
  for (TStorageObjectContainer::const_iterator it = m_Chunks.begin(),
                                               end = m_Chunks.end();
       it != end; ++it) {
...
void CStorageContainer::clean() {
  nlassert(m_ChunksOwnsPointers); // Can only use the default when m_Chunks
                                  // retains ownership.
  for (TStorageObjectContainer::const_iterator it = m_Chunks.begin(),
                                               end = m_Chunks.end();
       it != end; ++it) {
...
void CStorageContainer::disown() {
  nlassert(m_ChunksOwnsPointers); // Can only use this when m_Chunks has been
                                  // given ownership.
  for (TStorageObjectContainer::const_iterator it = m_Chunks.begin(),
                                               end = m_Chunks.end();
       it != end; ++it) {
...
IStorageObject *CStorageContainer::findLastStorageObject(uint16 id) const {
  for (TStorageObjectContainer::const_reverse_iterator it = m_Chunks.rbegin(),
                                                       end = m_Chunks.rend();
       it != end; ++it) {
    if (it->first == id)
      return it->second;
...
bool CStorageContainer::isContainer() const { return true; }
  ...
  if (chunks.stream().isReading()) {
    ...
    while (chunks.enterChunk()) {
      ...
      if (storageObject->isContainer())
        static_cast<CStorageContainer *>(storageObject)->serial(chunks);
      else
        storageObject->serial(chunks.stream());
...
std::string CStorageRaw::className() const { return "StorageRaw"; }

void CStorageRaw::serial(NLMISC::IStream &stream) {
  stream.serialBuffer(&Value[0], Value.size());
}

void CStorageRaw::toString(std::ostream &ostream,
                           const std::string &pad) const {
  // note: only use pad when multi-lining
  // like Blah: (Something) "SingleValue"
  //      Blahblah: (Container) {
...
void CStorageRaw::setSize(sint32 size) { Value.resize(size); }

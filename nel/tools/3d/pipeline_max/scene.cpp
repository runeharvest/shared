...
CScene::CScene(const CSceneClassRegistry *sceneClassRegistry,
               CDllDirectory *dllDirectory, CClassDirectory3 *classDirectory3)
    : m_SceneClassRegistry(sceneClassRegistry), m_DllDirectory(dllDirectory),
      m_ClassDirectory3(classDirectory3) {}

CScene::~CScene() {}

std::string CScene::className() const { return "Scene"; }
...
void CScene::clean() { CStorageContainer::clean(); }
...
void CScene::disown() { CStorageContainer::disown(); }
...
IStorageObject *CScene::createChunkById(uint16 id, bool container) {
  if (container) {
    ...
    return new CSceneClassContainer(this, m_SceneClassRegistry, m_DllDirectory,
                                    m_ClassDirectory3);
  }
...
CSceneClassContainer::CSceneClassContainer(
    CScene *scene, const CSceneClassRegistry *sceneClassRegistry,
    CDllDirectory *dllDirectory, CClassDirectory3 *classDirectory3)
    : m_Scene(scene), m_SceneClassRegistry(sceneClassRegistry),
      m_DllDirectory(dllDirectory), m_ClassDirectory3(classDirectory3),
      m_BuiltinScene(NULL) {}
...
void CSceneClassContainer::toString(std::ostream &ostream,
...
void CSceneClassContainer::parse(uint16 version, uint filter) {
  ...
  for (std::vector<CSceneClass *>::size_type i = 0;
       i < m_StorageObjectByIndex.size(); ++i) {
    m_StorageObjectByIndex[i] = static_cast<CSceneClass *>(it->second);
    ++it;
  }
  CSceneClass *builtinScene =
      m_StorageObjectByIndex[m_StorageObjectByIndex.size() - 1];
...
CSceneClass *CSceneClassContainer::getByStorageIndex(uint32 index) const {
  ...
  if (index >= m_StorageObjectByIndex.size()) {
    nlerror("Index %i is outside size %i", index,
            m_StorageObjectByIndex.size());
    return NULL;
  }
...
uint32
CSceneClassContainer::getOrCreateStorageIndex(CSceneClass *storageObject) {
  // Temporary 'readonly' implementation, not modifying m_Chunks!
  return m_StorageObjectToIndex[storageObject];
}

IStorageObject *CSceneClassContainer::createChunkById(uint16 id,
                                                      bool container) {
  // nldebug("Scene class id %x (%i)", (uint32)id, (uint32)id);
  switch (id) {
    // Known unknown special identifiers...
  case 0x2032:
    return m_SceneClassRegistry->createUnknown(
        m_Scene, 0x0, NLMISC::CClassId(0x29263a68, 0x405f22f5),
        ucstring("OSM Derived"), ucstring("Internal"), ucstring("Internal"));
  case 0x2033:
    return m_SceneClassRegistry->createUnknown(
        m_Scene, 0x0, NLMISC::CClassId(0x4ec13906, 0x5578130e),
        ucstring("WSM Derived"), ucstring("Internal"), ucstring("Internal"));
    // return new CSceneClass(m_Scene); // TODO: Make dummy dllentry and
    // classentry for these... return static_cast<IStorageObject *>(new
    // CSceneClassUnknown<CSceneClass>(dllEntry, classEntry));
  }
  const CClassEntry *classEntry = m_ClassDirectory3->get(id);
  CSceneClass *sceneClass = m_SceneClassRegistry->create(
      m_Scene, classEntry->superClassId(), classEntry->classId());
  if (sceneClass) {
    return static_cast<IStorageObject *>(sceneClass);
  } else {
    const CDllEntry *dllEntry = m_DllDirectory->get(classEntry->dllIndex());
    sceneClass = m_SceneClassRegistry->createUnknown(
        m_Scene, classEntry->superClassId(), classEntry->classId(),
        classEntry->displayName(), dllEntry->dllFilename(),
        dllEntry->dllDescription());
    if (sceneClass) {
      return static_cast<IStorageObject *>(sceneClass);
    } else {
      // Create an invalid unknown scene class
      return static_cast<IStorageObject *>(new CSceneClassUnknown<CSceneClass>(
          m_Scene, classEntry->classId(), classEntry->superClassId(),
          classEntry->displayName(), "SceneClassUnknown",

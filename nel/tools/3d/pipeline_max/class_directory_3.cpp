...
CClassDirectory3::~CClassDirectory3() {
  ...
  if (!m_ChunksOwnsPointers) {
    for (TStorageObjectContainer::iterator it = m_ChunkCache.begin(),
                                           end = m_ChunkCache.end();
         it != end; ++it) {
      delete it->second;
    }
    for (std::vector<CClassEntry *>::iterator subit = m_Entries.begin(),
...
std::string CClassDirectory3::className() const { return "ClassDirectory3"; }

// Parallel to CDllDirectory
void CClassDirectory3::toString(std::ostream &ostream,
  ...
  if (m_ChunksOwnsPointers) {
    ...
    sint i = 0;
    for (TStorageObjectContainer::const_iterator it = m_ChunkCache.begin(),
                                                 end = m_ChunkCache.end();
         it != end; ++it) {
      uint16 id = it->first;
      switch (id) {
      ...
      {
        uint subi = 0;
        for (std::vector<CClassEntry *>::const_iterator
                 subit = m_Entries.begin(),
                 subend = m_Entries.end();
             subit != subend; ++subit) {
          ostream << "\n" << pad << "Entries[" << subi << "]: ";
...
void CClassDirectory3::parse(uint16 version, uint filter) {
  ...
  // Parse chunks
  for (TStorageObjectContainer::iterator it = m_Chunks.begin(),
                                         end = m_Chunks.end();
       it != end; ++it) {
    uint16 id = it->first;
    switch (id) {
    case 0x2040: // ClassEntry
    {
      ...
      if (!parsedDllEntry) {
        m_ChunkCache.push_back(
            TStorageObjectWithId(id, NULL)); // Dummy entry to know the location
        lastCached = id;
        parsedDllEntry = true;
      }
...
void CClassDirectory3::build(uint16 version, uint filter) {
  ...
  // Set up the m_Chunks list, when (CClassEntry::ID, NULL) is found write out
  // all of the entries.
  for (TStorageObjectContainer::iterator it = m_ChunkCache.begin(),
                                         end = m_ChunkCache.end();
       it != end; ++it) {
...
const CClassEntry *CClassDirectory3::get(uint16 index) const {
  ...
  if (index >= m_Entries.size()) {
    nlerror("Index 0x%x is above the number of entries %i", (uint32)index,
            (uint32)m_Entries.size());
    return NULL;
  }
...
// Parallel to CDllDirectory
uint16
CClassDirectory3::getOrCreateIndex(const ISceneClassDesc *sceneClassDesc) {
  nlassert(!m_ChunksOwnsPointers);
  std::map<NLMISC::CClassId, uint16>::iterator it =
      m_ClassIdToIndex.find(sceneClassDesc->classId());
...
CClassEntry::CClassEntry(CDllDirectory *dllDirectory,
                         const ISceneClassDesc *sceneClassDesc)
    : m_Header(new CClassEntryHeader()), m_Name(new CStorageValue<ucstring>()) {
  m_Chunks.push_back(TStorageObjectWithId(0x2060, m_Header));
  m_Chunks.push_back(TStorageObjectWithId(0x2042, m_Name));
  m_Header->DllIndex =
      dllDirectory->getOrCreateIndex(sceneClassDesc->dllPluginDesc());
...
std::string CClassEntry::className() const { return "ClassEntry"; }

void CClassEntry::toString(std::ostream &ostream,
                           const std::string &pad) const {
  if (m_Header && m_Name) {
    ostream << "(" << className() << ") [" << m_Chunks.size() << "] PARSED { ";
    std::string padpad = pad + "\t";
...
std::string CClassEntryHeader::className() const { return "ClassEntryHeader"; }
...
void CClassEntryHeader::toString(std::ostream &ostream,
                                 const std::string &pad) const {
  ostream << "(" << className() << ") { ";

...
enum TParseLevel {
  ...
  // PARSE_NELDATA = 0x00000004, // Parse all structures related to nel specific
  // data (nel material, node properties, etcetera) PARSE_NEL3D = 0x00000008, //
  // Parse classes to initialize their nel3d equivalent classes
};

// IStorageObject : exposes serial(CStorageStream &stream) and dump(const
// std::string &pad)
...
public:
  ...
  virtual std::string getClassName() {
    return className();
  } // inherited from NLMISC::IClassable through NLMISC::IStreamable
  virtual std::string className() const = 0; // renamed for constness
  // virtual void serial(NLMISC::IStream &stream); // inherited from
  // NLMISC::IStreamable
  std::string toString();
...
public:
  ...
  virtual void serial(NLMISC::IStream &stream); // only used to wrap a container
  ...
  void serial(NLMISC::IStream &stream,
              uint size); // without wrapping, known size

  // virtual
  // Parse this class with given version and parse level filter
  virtual void parse(uint16 version, uint filter = 0);
  // Clean up built data or duplicate unparsed source data, call after
  // serializing build and after parse
  virtual void clean();
  // Build the storage structure needed to store the parsed data back
  virtual void build(uint16 version, uint filter = 0);
  // Give ownership of the chunks back to the m_Chunks, must call build first,
  // call instead of clean, reduces the parse level back to 0
  virtual void disown();
...
public:
  ...
  findStorageObject(uint16 id,
                    uint nb = 0) const; // find storage object with given id, nb
                                        // count in case there are more

...
CDllDirectory::~CDllDirectory() {
  ...
  if (!m_ChunksOwnsPointers) {
    ...
    for (std::vector<CDllEntry *>::iterator subit = m_Entries.begin(),
...
std::string CDllDirectory::className() const { return "DllDirectory"; }

// Parallel to CClassDirectory3
void CDllDirectory::toString(std::ostream &ostream,
  ...
  if (m_ChunksOwnsPointers) {
    ...
    for (TStorageObjectContainer::const_iterator it = m_ChunkCache.begin(),
      ...
      {
        uint subi = 0;
        for (std::vector<CDllEntry *>::const_iterator subit = m_Entries.begin(),
                                                      subend = m_Entries.end();
             subit != subend; ++subit) {
          ostream << "\n" << pad << "Entries[" << subi << "]: ";
...
void CDllDirectory::parse(uint16 version, uint filter) {
  ...
  for (TStorageObjectContainer::iterator it = m_Chunks.begin(),
    ...
    case 0x2038: // DllEntry
    {
      ...
      m_InternalNameToIndex[NLMISC::toLower(dllEntry->dllFilename())] =
          m_Entries.size();
...
void CDllDirectory::build(uint16 version, uint filter) {
  ...
  // Set up the m_Chunks list, when (CDllEntry::ID, NULL) is found write out all
  // of the entries.
  for (TStorageObjectContainer::iterator it = m_ChunkCache.begin(),
                                         end = m_ChunkCache.end();
       it != end; ++it) {
...
// Parallel to CClassDirectory3
sint32
CDllDirectory::getOrCreateIndex(const IDllPluginDescInternal *dllPluginDesc) {
  nlassert(!m_ChunksOwnsPointers);
  std::map<ucstring, sint32>::iterator it = m_InternalNameToIndex.find(
      NLMISC::toLower(ucstring(dllPluginDesc->internalName())));
...
void CDllDirectory::addInternalIndices() {
  m_InternalNameToIndex[NLMISC::toLower(
      ucstring(DllPluginDescBuiltin.internalName()))] = -1;
  m_InternalNameToIndex[NLMISC::toLower(
      ucstring(DllPluginDescScript.internalName()))] = -2;
}
...
std::string CDllEntry::className() const { return "DllEntry"; }

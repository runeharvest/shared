...
CSceneClass::~CSceneClass() {
  if (!m_ChunksOwnsPointers) {
    for (TStorageObjectContainer::iterator it = m_OrphanedChunks.begin(),
                                           end = m_OrphanedChunks.end();
         it != end; ++it)
      delete it->second;
    m_OrphanedChunks.clear();
    for (std::vector<IStorageObject *>::iterator it = m_ArchivedChunks.begin(),
                                                 end = m_ArchivedChunks.end();
         it != end; ++it)
      delete (*it);
...
void CSceneClass::toString(std::ostream &ostream,
  ...
  if (m_ChunksOwnsPointers) {
    ...
    ostream << "> ";
    ostream << "(" << className() << ": "
            << ucstring(classDesc()->displayName()).toUtf8() << ", "
            << classDesc()->classId().toString() << ", 0x";
    {
      std::stringstream ss;
      ss << std::hex << std::setfill('0');
    ...
    ostream << ", "
            << ucstring(classDesc()->dllPluginDesc()->internalName()).toUtf8()
            << ") [" << m_OrphanedChunks.size() << "] { ";
    toStringLocal(ostream, pad);
    // Append orphans
    std::string padpad = pad + "\t";
    sint i = 0;
    for (TStorageObjectContainer::const_iterator it = m_OrphanedChunks.begin(),
                                                 end = m_OrphanedChunks.end();
         it != end; ++it) {
      std::stringstream ss;
      ss << std::hex << std::setfill('0');
...
void CSceneClass::parse(uint16 version, uint filter) {
  ...
  m_OrphanedChunks.insert(m_OrphanedChunks.end(), m_Chunks.begin(),
                          m_Chunks.end());
...
void CSceneClass::clean() {
  ...
  if (m_Chunks.size() == 0 && m_OrphanedChunks.size() != 0) {
    ...
    return;
  } // Already cleaned, should not call twice, not reliable because not all
    // chunks have child chunks
...
void CSceneClass::build(uint16 version, uint filter) {
  ...
  if (m_Chunks.size() != 0) {
    nldebug("Not parsed, or disowned");
    return;
  } // Don't have local ownership, parsing may have failed, the built version is
    // implicitly up to date

  // Store orphan chunks
  m_Chunks.insert(m_Chunks.end(), m_OrphanedChunks.begin(),
                  m_OrphanedChunks.end());
...
void CSceneClass::disown() {
  ...
  if (!m_ChunksOwnsPointers && (m_Chunks.size() < m_OrphanedChunks.size())) {
    nlerror("Not built");
    return;
  } // If chunks is not the owner, built chunks must match the parsed data. This
...
const NLMISC::CClassId CSceneClass::ClassId =
    NLMISC::CClassId::Null;                         // This class is invalid
...
static const CSceneClassDesc<CSceneClass> SceneClassDesc(
    static_cast<const IDllPluginDescInternal *>(&DllPluginDescBuiltin));
} /* anonymous namespace */
...
void CSceneClass::toStringLocal(std::ostream &ostream, const std::string &pad,
                                uint filter) const {
  // Nothing to do here...
}
...
IStorageObject *CSceneClass::getChunk(uint16 id) {
  if (m_OrphanedChunks.begin()->first == id) {
    ...
    for (TStorageObjectContainer::iterator it = m_OrphanedChunks.begin(),
         ...
         it != end; ++it) {
      if (it->first == id) {
        nlwarning("Try to get chunk with 0x%x id, but found 0x%x instead. "
                  "Found the correct chunk at a different position. Unknown "
                  "chunks, or chunks out of order",
                  (uint32)id, (uint32)m_OrphanedChunks.begin()->first);
  ...
  // nldebug("Chunk 0x%x not found, this is allowed, returning NULL",
  // (uint32)id);
  return NULL;
}
...
void CSceneClass::putChunk(uint16 id, IStorageObject *storageObject) {
  if (storageObject->isContainer()) {
    static_cast<CStorageContainer *>(storageObject)
        ->build(VersionUnknown); // FIXME
  }
  m_OrphanedChunks.insert(m_PutChunkInsert,
                          TStorageObjectWithId(id, storageObject));
}

...
struct CObject {
  ...
  void display() {
    nlinfo("Object %s processed with %u vertices and %u faces", name.c_str(),
           (uint)textureCoords.size(), (uint)faces.size());
  }
};
...
bool fillPoint(NLMISC::CBitmap &bitmap, sint width, CPoints &points) {
  ...
  if (c == NLMISC::CRGBA::White) {
    ...
    if (p.y > 0)
      points.push_back(p + Up);
    if (p.y < h - 1)
      points.push_back(p + Down);
    if (p.x > 0)
...
void drawEdge(NLMISC::CBitmap &bitmap, const CObject &object, const CFace &face,
  ...
  for (uint j = 0, jlen = pixels.size(); j < jlen; ++j) {
    bitmap.setPixelColor(pixels[j].first, pixels[j].second,
                         NLMISC::CRGBA::Black);
  }
}
...
int main(int argc, char **argv) {
  ...
  args.addArg("c", "colorize", "color",
              "Colorize textures using a color (in HTML hexdecimal format like "
              "#rrggbb)");
  args.addArg("f", "fill", "color or image",
              "Fill background part with color or image");
  args.addArg("u", "uvmap", "", "Generate a UV Map texture from OBJ file");
  args.addArg("w", "width", "width of border",
              "Width of the border to fill (default 0)");
  args.addArg("s", "size", "size of output bitmap",
              "Width and height of generated bitmap (default 4096)");
  args.addArg("b", "background", "background color",
  ...
  if (args.haveArg("f")) {
    ...
    // will use a copy 1024x1024 texture map on a 4096x4096 UV Map preserving
    // the different areas

    std::string foregroundColorOrFilename = args.getArg("f").front();

    NLMISC::CRGBA foregroundColor = NLMISC::CRGBA::Black,
    ...
    // we can't have more than width * height points, so allocate memory for all
    // of them
    ...
    for (sint y = 0, h = inBitmap.getHeight(); y < h; ++y) {
      for (sint x = 0, w = inBitmap.getWidth(); x < w; ++x) {
        if (inBitmap.getPixelColor(x, y) != DiscardColor) {
          ...
          outBitmap.setPixelColor(
              x, y,
              useTexture
                  ? textureBitmap.getPixelColor(x % textureBitmap.getWidth(),
                                                y % textureBitmap.getHeight())
                  : foregroundColor);
        } else {
    ...
    while (!objFile.eof()) {
      ...
      if (line.substr(0, 3) == "vt ") {
        ...
        if (tokens.size() == 3) {
          ...
          object.textureCoords.push_back(NLMISC::CUV(
              u * (float)TextureSize, (1.f - v) * (float)TextureSize));
        } else {

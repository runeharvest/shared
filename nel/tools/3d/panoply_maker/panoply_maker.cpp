...
// Each part is defined by a mask. The red component of it is considered as an
// alpha value (not the alpha, because it is faster to create a grey texture
// with photoshop..) The result is serialized in png or tga files.
// ========================================================================================================
// why this tool ? : it is useful to create various colored cloth and skin
// textures Not all hardware allow it to manage that at runtime (lack for
// palettized textures or pixel shaders...)
//=========================================================================================================
...
struct CBuildInfo {
  ...
  std::vector<std::string>
      BitmapExtensions;     // the supported extension for bitmaps
...
///
static void
BuildColoredVersionForOneBitmap(const CBuildInfo &bi,
 ...
 */
static bool CheckIfNeedRebuildColoredVersionForOneBitmap(
    const CBuildInfo &bi, const std::string &fileNameWithExtension,
    bool mustDivideBy2);
...
int main(int argc, char *argv[]) {
  ...
  if (!strcmp(argv[2], "gtm") || !strcmp(argv[2], "cgi")) {
    ...
    try {
      ...
      /// look paths
      try {
        NLMISC::CConfigFile::CVar &additionnal_paths =
            cf.getVar("additionnal_paths");
        for (uint k = 0; k < (uint)additionnal_paths.size(); ++k) {
          NLMISC::CPath::addSearchPath(
              NLMISC::CPath::standardizePath(additionnal_paths.asString(k)),
              true, false);
        }
      ...
      try {
        _Path_Input_TexBases = NLMISC::CPath::standardizePath(
            cf.getVar("input_path_texbase").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }

      /// repertory of masks (original)
      try {
        _Path_Input_Masks = NLMISC::CPath::standardizePath(
            cf.getVar("input_path_mask").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }

      /// optimized masks output directory created
      try {
        _Path_Output_MasksOptimized = NLMISC::CPath::standardizePath(
            cf.getVar("output_path_mask_optimized").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }

      /// file of infos about colorization average for the client
      try {
        _Path_Output_Cgi = NLMISC::CPath::standardizePath(
            cf.getVar("output_path_cgi").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }

      /// file of infos about multiplexing texture for the client
      try {
        _Path_Output_Gtm = NLMISC::CPath::standardizePath(
            cf.getVar("output_path_gtm").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }
    ...
    {
      CInfoMaskGeneration infoMaskGen(_Path_Input_TexBases, _Path_Input_Masks,
                                      _Path_Output_MasksOptimized,
                                      _Path_Output_Gtm, argv[3], 1);
      infoMaskGen.init();
      infoMaskGen.process();
    } else if (!strcmp(argv[2], "cgi")) /// colorized information
    {
      CInfoColorGeneration infoColor(_Path_Input_TexBases, _Path_Input_Masks,
                                     _Path_Output_Cgi, argv[3], 1);
      infoColor.init();
      infoColor.process();
    }
  } else {
    ...
    try {
      ...
      try {
        bi.InputPath =
            NLMISC::CPath::standardizePath(cf.getVar("input_path").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }
      ...
      try {
        bi.OutputPath =
            NLMISC::CPath::standardizePath(cf.getVar("output_path").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }

      /// hls info path
      try {
        bi.HlsInfoPath = NLMISC::CPath::standardizePath(
            cf.getVar("hls_info_path").asString());
      } catch (const NLMISC::EUnknownVar &) {
      ...
      try {
        bi.CachePath =
            NLMISC::CPath::standardizePath(cf.getVar("cache_path").asString());
      } catch (const NLMISC::EUnknownVar &) {
      }
      ...
      try {
        NLMISC::CConfigFile::CVar &bitmap_extensions =
            cf.getVar("bitmap_extensions");
        for (uint k = 0; k < (uint)bitmap_extensions.size(); ++k) {
          std::string ext =
              "." + NLMISC::toLowerAscii(bitmap_extensions.asString(k));
          if (std::find(bi.BitmapExtensions.begin(), bi.BitmapExtensions.end(),
                        ext) == bi.BitmapExtensions.end()) {
...

{
  ...
  for (uint k = 0; k < (uint)mask_extensions.size(); ++k) {
    ...
    NLMISC::CConfigFile::CVar &luminosities =
        cf.getVar(colorMasks[k].MaskExt + "_luminosities");
    NLMISC::CConfigFile::CVar &contrasts =
        cf.getVar(colorMasks[k].MaskExt + "_constrasts");
    NLMISC::CConfigFile::CVar &hues =
        cf.getVar(colorMasks[k].MaskExt + "_hues");
    NLMISC::CConfigFile::CVar &lightness =
        cf.getVar(colorMasks[k].MaskExt + "_lightness");
    NLMISC::CConfigFile::CVar &saturation =
        cf.getVar(colorMasks[k].MaskExt + "_saturations");
    NLMISC::CConfigFile::CVar &colorIDs =
        cf.getVar(colorMasks[k].MaskExt + "_color_id");
    ...
    if (luminosities.size() != contrasts.size() ||
      ...
      throw NLMISC::Exception(
          "All color descriptors must have the same number of arguments");
    }
...
static void BuildColoredVersions(const CBuildInfo &bi) {
  ...
  for (uint sizeVersion = 0; sizeVersion < 2; sizeVersion++) {
    ...
    if (sizeVersion == 0)
      ...
      NLMISC::CPath::getPathContent(bi.InputPath + DivideBy2Dir, false, false,
                                    true, files);
    ...
    for (uint k = 0; k < files.size(); ++k) {
      for (uint l = 0; l < bi.BitmapExtensions.size(); ++l) {
        std::string fileExt =
            "." + NLMISC::toLowerAscii(NLMISC::CFile::getExtension(files[k]));
        if (fileExt == bi.BitmapExtensions[l]) {
          // nlwarning("Processing : %s ", files[k].c_str());
          try {
            if (CheckIfNeedRebuildColoredVersionForOneBitmap(
                    bi, NLMISC::CFile::getFilename(files[k]),
                    sizeVersion == 1)) {
              BuildColoredVersionForOneBitmap(
                  bi, NLMISC::CFile::getFilename(files[k]), sizeVersion == 1);
            } else {
              // nlwarning(("No need to rebuild " +
              // NLMISC::CFile::getFilename(files[k])).c_str());
            }
          } catch (const std::exception &e) {
...
static bool CheckIfNeedRebuildColoredVersionForOneBitmap(
    const CBuildInfo &bi, const std::string &fileNameWithExtension,
    bool mustDivideBy2) {
  if (bi.CachePath.empty())
    return true;
  uint32 srcDate = (uint32)NLMISC::CFile::getFileModificationDate(
      replaceSlashes(bi.InputPath + fileNameWithExtension));
  ...
  for (uint k = 0; k < bi.ColorMasks.size(); ++k) {
    std::string maskName =
        fileName + "_" + bi.ColorMasks[k].MaskExt + "." + fileExt;
    ...
    if (!maskFileName.empty()) // found the mask ?
    {
      ...
      if (NLMISC::CFile::fileExists(maskFileName)) {
        srcDate =
            std::max(srcDate, (uint32)NLMISC::CFile::getFileModificationDate(
                                  replaceSlashes(maskFileName)));
        masks.push_back(li);
      }
    }
  }
  ...
  if (!NLMISC::CFile::fileExists(cacheHLSInfo.c_str()))
    ...
    if (!NLMISC::CFile::moveFile(outputHLSInfo, cacheHLSInfo)) {
      nlerror("Couldn't move %s to %s", cacheHLSInfo.c_str(),
              outputHLSInfo.c_str());
      return true;
    }
  }
  ...
  for (;;) {
    ...
    for (l = 0; l < masks.size(); ++l) {
      ...
      outputFileName +=
          bi.DefaultSeparator + bi.ColorMasks[maskID].CMs[colorID].ColID;
    }
...
BuildColoredVersionForOneBitmap(const CBuildInfo &bi,
    ...
    try {
      if (is.open(fullInputBitmapPath)) {
        ...
        if (bi.OptimizeTextures > 0 && depth == 32 &&
            srcBitmap.isAlphaUniform(&value) && value == 255) {
          nlwarning("Texture %s can be optimized, run textures_optimizer",
                    fullInputBitmapPath.c_str());
        }
        ...
        nlerror("Unable to open %s. Processing next",
                fullInputBitmapPath.c_str());
        return;
      }
    } catch (const NLMISC::Exception &e) {
      nlerror("File or format error with %s (%s). Processing next...",
              fullInputBitmapPath.c_str(), e.what());
      return;
    }
  }
      ...
      try {
        ...
        if (is.open(maskFileName)) {
          ...
          if (li.Mask.getPixelFormat() == CBitmap::RGBA &&
              bi.OptimizeTextures > 0 && !li.Mask.isGrayscale()) {
            nlwarning("Mask %s is using colors, results may by incorrect! Run "
                      "textures_optimizer to fix it.",
                      maskFileName.c_str());
          }
          ...
          if (li.Mask.getWidth() != srcBitmap.getWidth() ||
            ...
            throw NLMISC::Exception(
                "Bitmap and mask do not have the same size");
          }
        ...
        nlerror("Error with %s: %s. Aborting this bitmap processing",
                maskFileName.c_str(), e.what());
        return;
      }
    }
  }
  ...
  for (;;) {
    ...
    CHLSBankTextureInfo::CTextureInstance &hlsTextInstance =
        hlsInfo.Instances[instId];
    ...
    /// Save the result. We let propagate exceptions (if there's no more space
    /// disk it useless to continue...)
    {
      ...
      try {
        ...
        if (os.open(fullOutputPath)) {
          ...
          if (mustDivideBy2)
            resultBitmap.resample((resultBitmap.getWidth() + 1) / 2,
                                  (resultBitmap.getHeight() + 1) / 2);
          // write the file

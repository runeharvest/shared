...
void _Edge::CreateH(TileInfo *tile) {
  size = tile->BmpInfo.bmiHeader.biWidth -
         2; // on n'inclut pas les coins dans les bordures
  line = new char[3 * size];
  for (int i = 0; i < size; i++) {
    line[i * 3] =
        ((char *)tile->Bits)[(i + 1) * 3 +
                             (tile->BmpInfo.bmiHeader.biWidth) *
                                 (tile->BmpInfo.bmiHeader.biHeight - 1) * 3];
    line[i * 3 + 1] =
        ((char *)tile->Bits)[(i + 1) * 3 + 1 +
                             (tile->BmpInfo.bmiHeader.biWidth) *
                                 (tile->BmpInfo.bmiHeader.biHeight - 1) * 3];
    line[i * 3 + 2] =
        ((char *)tile->Bits)[(i + 1) * 3 + 2 +
                             (tile->BmpInfo.bmiHeader.biWidth) *
                                 (tile->BmpInfo.bmiHeader.biHeight - 1) * 3];
  }
}
...
void _Edge::CreateG(TileInfo *tile) {
  size = tile->BmpInfo.bmiHeader.biHeight -
         2; // on n'inclut pas les coins dans les bordures
  line = new char[3 * size];
  for (int i = 0; i < size; i++) {
    line[i * 3] =
        ((char *)tile->Bits)[(i + 1) * 3 * tile->BmpInfo.bmiHeader.biWidth];
    line[i * 3 + 1] =
        ((char *)tile->Bits)[(i + 1) * 3 * tile->BmpInfo.bmiHeader.biWidth + 1];
    line[i * 3 + 2] =
        ((char *)tile->Bits)[(i + 1) * 3 * tile->BmpInfo.bmiHeader.biWidth + 2];
  }
}
...
void _Edge::CreateD(TileInfo *tile) {
  ...
  for (int i = 0; i < size; i++) {
    line[i * 3] =
        ((char *)tile->Bits)[(i + 1) * 3 * tile->BmpInfo.bmiHeader.biWidth +
                             (tile->BmpInfo.bmiHeader.biWidth - 1) * 3];
    line[i * 3 + 1] =
        ((char *)tile->Bits)[(i + 1) * 3 * tile->BmpInfo.bmiHeader.biWidth + 1 +
                             (tile->BmpInfo.bmiHeader.biWidth - 1) * 3];
    line[i * 3 + 2] =
...
void TileList::Reload(CDC *pDC, tilelist::iterator iFirst,
                      int n) // recharge en memoire une tranche de tiles
{
  ...
  for (int i=0;i<n;i++)
  {
      if (!(*p)->Loaded && _LoadBitmap((*p)->path,&(*p)->BmpInfo,&(*p)->Bits))
      {
          (*p)->Loaded=1;
          (*p)->DibSection =
  CreateDIBSection(pDC->m_hDC,&(*p)->BmpInfo,DIB_RGB_COLORS,(void**)&buffer_bidon,0,0);
      }
      p++;
  }*/
}
...
void TileCtrl::Init() {
  ...
  char name[256];
  char *defautpath = ((SelectionTerritoire *)GetParent()->GetParent())
  ...
  {
      int
  size=TileCroix.BmpInfo.bmiHeader.biHeight*TileCroix.BmpInfo.bmiHeader.biWidth*TileCroix.BmpInfo.bmiHeader.biBitCount/8;
      char *temp = new char[size];
      TileCroix.DibSection =
  CreateDIBSection(GetDC()->m_hDC,&TileCroix.BmpInfo,DIB_RGB_COLORS,(void**)&temp,0,0);
  }*/
  count_ = 1;
}
...
void TileCtrl::GetVisibility(int &First,
                             int &Last) // retourne l'indice du premier et du
                                        // dernier item visible dans la fenetre
{
  RECT rect;
  int i = GetNbTileLine();
  ...
  if (First < 0)
    ...
    First *= i;
  Last = First + i * (j + 1);
  if (InfoList.theList.size() > 0 && Last >= InfoList.theList.size())
    Last = InfoList.theList.size() - 1;
}

int TileCtrl::GetIndex(LPPOINT pt) // retourne l'index d'un incone a partir de
                                   // sa position dans le fenetre
// si le curseur n'est pas sur un icon, retourne -1
{
  int i = GetNbTileLine();
  if ((pt->y + scrollpos) < spacing_y)
    return -1;
...
void TileCtrl::DeleteTile(int i) {
  /*	for (tilelist::iterator
     p=InfoList.theList.begin();p!=InfoList.theList.end();++p)
      {
  ...
  // this->DeleteItem(i); //temp, should first delete the bitmap file and the
  // DIB section
...
void TileCtrl::DrawTile(int i, CDC *pDC) {
  ...
  tilelist::iterator p = InfoList.theList.begin();
  for (int k = 0; k < i && p != InfoList.theList.end(); k++)
    p++;
  if (p == InfoList.theList.end())
    return;
  bmp = *p;
  if (bmp) {
    POINT pt;
    pt = GetPos(i);
    if (!bmp->Loaded) {
      StretchDIBits(pDC->m_hDC, pt.x, pt.y, sizeicon_x, sizeicon_y, 0, 0,
                    TileCroix.BmpInfo.bmiHeader.biWidth,
                    TileCroix.BmpInfo.bmiHeader.biHeight, TileCroix.Bits,
                    &TileCroix.BmpInfo, DIB_RGB_COLORS, SRCCOPY);
    } else {
      RECT rect;
      GetClientRect(&rect);
      StretchDIBits(pDC->m_hDC, pt.x, pt.y, rect.right - rect.left,
                    rect.bottom - rect.top, 0, 0,
                    bmp->BmpInfo.bmiHeader.biWidth,
                    bmp->BmpInfo.bmiHeader.biHeight, bmp->Bits, &bmp->BmpInfo,
...
void TileCtrl::OnPaint() {
  ...
  CFont font;
  font.CreateFont(-10, 0, 0, 0, FW_THIN, false, false, false, DEFAULT_CHARSET,
                  OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                  FIXED_PITCH, NULL);
  CFont *oldFont = dc.SelectObject(&font);
  for (int i = iFV; i <= iLV; i++)
    DrawTile(i, &dc);
  ::ReleaseDC(*this, dc);
...
LRESULT TileCtrl::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_COMMAND) // The user had selected an item on popup menu
  {
    ...
    if (id == 3) {
      CFileDialog *load = new CFileDialog(
          true, "bmp", NULL, OFN_ALLOWMULTISELECT | OFN_ENABLESIZING,
          "*.bmp ||", this->GetParent());
      ...
      CFileDialog *load = new CFileDialog(true, "bmp", NULL, OFN_ENABLESIZING,
                                          "*.bmp ||", this->GetParent());
      if (load->DoModal() == IDOK) {
        POSITION p = load->GetStartPosition(); // la doc dit que p=NULL quand il
                                               // n'y a pas de selection : c'est
...
void TileCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (!ViewTileMode) {
    popup.AppendMenu(GetSelectedCount() == 1 ? MF_STRING
                                             : MF_STRING | MF_GRAYED,
                     0, "Remplacer...");
    popup.AppendMenu(GetSelectedCount() > 0 ? MF_STRING : MF_STRING | MF_GRAYED,
                     1, "Supprimer");

...
LRESULT Browse::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_MOUSEMOVE) {
    ...
    if (lbutton) // on dessine le carre de selection
    {
      ...
      if (current.top > current.bottom) {
        int temp = current.bottom;
        current.bottom = current.top;
        current.top = temp;
      }

      CDC *pDC = GetDC();
      m_ctrl.DrawDragRect(pDC, NULL, size, &last_sel,
                          size); // on efface l'ancien carre

      m_ctrl.UpdateSelection(&current, (int)wParam,
                             m_128x128); // on affiche les modifes

      m_ctrl.DrawDragRect(pDC, &current, size, NULL,
                          size); // on affiche le nouveau carre
      ::ReleaseDC(*this, *pDC);
  ...
  if (message == WM_LBUTTONDOWN) {
    ...
    RECT p, rect;
    p.left = m_ctrl.MousePos.x;
    p.top = m_ctrl.MousePos.y;
    ClientToScreen(&p);
    ...
    if (pt.x >= rect.left && pt.x < rect.right && pt.y >= rect.top &&
      ...
      if (index != -1 && !(wParam & MK_SHIFT) /* && !(wParam&MK_CONTROL)*/) {
        ...
        if (p != m_ctrl.InfoList.GetLast(m_128x128)) {
          ...
          CDC *pDC = NULL;
          int indexx = 0;
          for (p = m_ctrl.InfoList.GetFirst(m_128x128);
               p != m_ctrl.InfoList.GetLast(m_128x128); ++p, indexx++) {
            if (p != pp && p->Selected) {
              if (!(wParam & MK_CONTROL)) {
                p->Selected = 0;
                if (pDC == NULL)
  ...
  if (message == WM_LBUTTONUP || message == WM_NCLBUTTONUP) {
    ...
    if (!selection && index != -1) {
      ...
      ::ReleaseDC(*this, *pDC);
      int index = 0;
      for (tilelist::iterator p = m_ctrl.InfoList.GetFirst(m_128x128);
           p != m_ctrl.InfoList.GetLast(m_128x128); ++p, index++) {
        if (p->Selected & 3) {
          p->Selected = 2;
        } else
          p->Selected = 0;
  ...
  if (message == WM_SIZE && m_ctrl.count_) {
    ...
    int hview = (m_ctrl.InfoList.GetSize(m_128x128) / i + 1) *
                    (m_ctrl.sizeicon_y + m_ctrl.spacing_y) +
                m_ctrl.spacing_y;
  ...
  if (message == WM_VSCROLL || message == WM_MOUSEWHEEL) {
    ...
    int hview = (m_ctrl.InfoList.GetSize(m_128x128) / i + 2) *
                    (m_ctrl.sizeicon_y + m_ctrl.spacing_y) +
                m_ctrl.spacing_y;
    ...
    if (message == WM_MOUSEWHEEL) {
      ...
      pos = inf.nPos -
            inc * (((m_ctrl.sizeicon_y + m_ctrl.spacing_y) * SCROLL_MAX) /
                   (hview - m_ctrl.spacing_y));
    } else {
      ...
      case SB_LINEUP:
        pos =
            inf.nPos - (((m_ctrl.sizeicon_y + m_ctrl.spacing_y) * SCROLL_MAX) /
                        (hview - m_ctrl.spacing_y));
        break;
      case SB_LINEDOWN:
        pos =
    ...
    if (iFV > old_iLV || iLV < old_iFV || scrollcode == SB_PAGEDOWN ||
      ...
      if (scroll_pixel) {
        ...
        if (abs(scroll_pixel) > (rect_clip.bottom - rect_clip.top))
          scroll_pixel = 0;
        else
          pDC->ScrollDC(0, scroll_pixel, &rect_scroll, &rect_clip, NULL, NULL);
...
unsigned long Browse::MyControllingFunction(void *pParam) {
  ...
  for (int i = 0; i < br->m_ctrl.InfoList.GetSize(br->m_128x128); i++) {
    ...
    case 0:
      path =
          tileBank2
              .getTile(tileBank2.getTileSet(br->m_ctrl.InfoList._tileSet)
                           ->getTile128(i))
    ...
    case 1:
      ...
      break;
    case 2: {
      int index = tileBank2.getTileSet(br->m_ctrl.InfoList._tileSet)
                      ->getTransition(i)
                      ->getTile();
      if (index != -1) {
        path = tileBank2.getTile(index)->getRelativeFileName(
            (CTile::TBitmap)(br->m_ctrl.Texture - 1));
    ...
    case 3:
      ...
      path = tileBank2.getDisplacementMap(
          tileBank2.getTileSet(br->m_ctrl.InfoList._tileSet)
              ->getDisplacementTile((CTileSet::TDisplacement)i));
      break;
    }
    ...
    if (!path.empty() &&
        _LoadBitmap(tileBank2.getAbsPath() + path, pBmp, *bits, pAlpha, rot)) {
      *ld = 1;
      int iFV, iLV;
      br->m_ctrl.GetVisibility(iFV, iLV, br->m_128x128);
      if (i <= iLV && i >= iFV) {
...
void Browse::Init() {
  ...
  if (RegOpenKey(HKEY_CURRENT_USER, REGKEY_TILEDIT, &regkey) == ERROR_SUCCESS) {
    value = 256;
    type = REG_SZ;
    if (RegQueryValueEx(regkey, REGKEY_WNDPL, 0, &type,
                        (unsigned char *)&sWindowpl, &value) == ERROR_SUCCESS) {
      WINDOWPLACEMENT wndpl;
      _stscanf(sWindowpl, _T("%d %d %d %d %d %d %d %d %d %d"), &wndpl.flags,
               &wndpl.ptMaxPosition.x, &wndpl.ptMaxPosition.y,
               &wndpl.ptMinPosition.x, &wndpl.ptMinPosition.y,
               &wndpl.rcNormalPosition.bottom, &wndpl.rcNormalPosition.left,
               &wndpl.rcNormalPosition.right, &wndpl.rcNormalPosition.top,
               &wndpl.showCmd);
    ...
    value = 256;
    type = REG_SZ;
    if (RegQueryValueEx(regkey, REGKEY_LASTPATH, 0, &type,
                        (unsigned char *)&sWindowpl, &value) != ERROR_SUCCESS)
    ...
    value = 4;
    type = REG_DWORD;
    if (RegQueryValueEx(regkey, REGKEY_BUTTONZOOM, 0, &type,
                        (unsigned char *)&m_ctrl.Zoom, &value) != ERROR_SUCCESS)
      m_ctrl.Zoom = 3;
    value = 4;
    type = REG_DWORD;
    if (RegQueryValueEx(regkey, REGKEY_BUTTONVARIETY, 0, &type,
                        (unsigned char *)&m_128x128, &value) != ERROR_SUCCESS)
      m_128x128 = 0;
    value = 4;
    type = REG_DWORD;
    if (RegQueryValueEx(regkey, REGKEY_BUTTONTEXTURE, 0, &type,
                        (unsigned char *)&m_ctrl.Texture,
                        &value) != ERROR_SUCCESS)
      m_ctrl.Texture = 1;
    value = 4;
    type = REG_DWORD;
    if (RegQueryValueEx(regkey, REGKEY_BUTTONTEXTINFO, 0, &type,
                        (unsigned char *)&m_ctrl.InfoTexte,
                        &value) != ERROR_SUCCESS)
      m_ctrl.InfoTexte = 1;
    RegCloseKey(regkey);
  }
  ...
  button->SetCheck(1);
  if (cx != -1 && cy != -1 && x != -1 && y != -1)
    SetWindowPos(0, x, y, cx, cy, 0);
  ...
  SendMessage(WM_SIZE, rect.right - rect.left,
              rect.bottom - rect.top); // force resize
...
void Browse::OnCancel() {
  ...
  if (::MessageBox(NULL, _T("Are you sure you want to cancel?"), _T("Cancel"),
                   MB_OK | MB_ICONQUESTION | MB_YESNO) == IDYES) {
...
void Browse::OnDestroy() {
  ...
  char sWindowpl[256];
  sprintf(sWindowpl, "%d %d %d %d %d %d %d %d %d %d", wndpl.flags,
          wndpl.ptMaxPosition.x, wndpl.ptMaxPosition.y, wndpl.ptMinPosition.x,
          wndpl.ptMinPosition.y, wndpl.rcNormalPosition.bottom,
          wndpl.rcNormalPosition.left, wndpl.rcNormalPosition.right,
          wndpl.rcNormalPosition.top, wndpl.showCmd);
  if (RegCreateKey(HKEY_CURRENT_USER, REGKEY_TILEDIT, &regkey) ==
      ERROR_SUCCESS) {
    ...
    RegSetValueEx(regkey, REGKEY_WNDPL, 0, REG_SZ,
                  (const BYTE *)tWindowpl.c_str(),
                  (tWindowpl.size() + 1) * sizeof(TCHAR));
    RegSetValueEx(regkey, REGKEY_LASTPATH, 0, REG_SZ,
                  (const BYTE *)tLastPath.c_str(),
                  (tLastPath.size() + 1) * sizeof(TCHAR));
    RegSetValueEx(regkey, REGKEY_BUTTONZOOM, 0, REG_DWORD,
                  (const unsigned char *)&m_ctrl.Zoom, 4);
    RegSetValueEx(regkey, REGKEY_BUTTONVARIETY, 0, REG_DWORD,
                  (const unsigned char *)&m_128x128, 4);
    RegSetValueEx(regkey, REGKEY_BUTTONTEXTURE, 0, REG_DWORD,
                  (const unsigned char *)&m_ctrl.Texture, 4);
    RegSetValueEx(regkey, REGKEY_BUTTONSORT, 0, REG_DWORD,
                  (const unsigned char *)&m_ctrl.Sort, 4);
    RegSetValueEx(regkey, REGKEY_BUTTONTEXTINFO, 0, REG_DWORD,
                  (const unsigned char *)&m_ctrl.InfoTexte, 4);
    RegCloseKey(regkey);
  }
}
...
void Browse::OnBatchLoad() {
  ...
  if (sFile.DoModal() == IDOK) {
    ...
    if (::MessageBox(NULL,
                     _T("Do you want to use rotation to reuse alpha tiles ?"),
                     _T("Import rotated tiles"),
                     MB_OK | MB_ICONQUESTION | MB_YESNO) == IDYES)
      rotate = true;
    ...
    for (int i = 0; i < CTileSet::count; i++) {
      if (m_ctrl.Texture == 3) {
        ...
        // Transition to patch
        CTileSetTransition *trans =
            tileBank2.getTileSet(land)->getTransition(transition);
        if (tileBank2.getTile(trans->getTile())
          ...
          for (int rot = 0; rot < 4; rot++) {
            ...
            std::string sFinal = path + toString("%s%02d.%s", filename.c_str(),
                                                 (int)transition, ext.c_str());
            ...
            if (pFile) {
              fclose(pFile);
              ok = m_ctrl.InfoList.setTileTransitionAlpha(i, sFinal,
                                                          (4 - rot) % 4);
        ...
        CTileSetTransition *trans =
            tileBank2.getTileSet(land)->getTransition(transition);
        if (tileBank2.getTile(trans->getTile())
                ->getRelativeFileName(m_ctrl.Texture == 1 ? CTile::diffuse
                                                          : CTile::additive)
                .empty()) {
          // Try to load a tile with a file name like /tiletransition0.tga
          std::string sFinal = path + toString("%s%02d.%s", filename.c_str(),
          ...
          if (pFile) {
            fclose(pFile);
            if (!m_ctrl.InfoList.setTileTransition(
                    i, sFinal,
                    m_ctrl.Texture == 1 ? CTile::diffuse : CTile::additive))
              break;
          }
        }
      }
...
void Browse::OnExportBorder() {
  ...
  if (sFile.DoModal() == IDOK) {
    ...
    // 256 or 128 ?
    int width, height;
    tileBank2.getTileSet(land)
        ->getBorder128(m_ctrl.Texture == 1 ? CTile::diffuse : CTile::additive)
    ...
    if (width && height) {
      ...
      if (error) {
        // Error message
        std::string tmp =
            toString("Can't write bitmap %s", tStrToUtf8(pathName).c_str());
        MessageBox(nlUtf8ToTStr(tmp), _T("Export border"),
...
void Browse::OnImportBorder() {
  ...
  if (sFile.DoModal() == IDOK) {
    ...
    if (error) {
      // Error message
      std::string tmp =
          toString("Can't read bitmap %s", tStrToUtf8(pathName).c_str());
      MessageBox(nlUtf8ToTStr(tmp), _T("Import border"),
                 MB_OK | MB_ICONEXCLAMATION);
    }
    ...
    if ((bitmap.getWidth() == 128) && (bitmap.getHeight() == 128)) {
      ...
      // Error message
      std::string tmp = toString("The bitmap must have a size of 128x128 (%s)",
                                 tStrToUtf8(pathName).c_str());
      MessageBox(nlUtf8ToTStr(tmp), _T("Import border"),
                 MB_OK | MB_ICONEXCLAMATION);
    }
    ...
    tileBank2.getTileSet(land)->setBorder(
        m_ctrl.Texture == 1 ? CTile::diffuse : CTile::additive, border);

    // Message
    MessageBox(_T("The border has been changed."), _T("Import border"),

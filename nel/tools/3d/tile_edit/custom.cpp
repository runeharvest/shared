...
Custom::Custom(CWnd *pParent /*=NULL*/) : CDialog(Custom::IDD, pParent) {
...
LRESULT Custom::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_INITDIALOG) {
    ...
    if (nButton > 4)
      SetWindowPos(0, 0, 0, client.right - client.left,
                   15 * 2 + (button.bottom - button.top + 10) * nButton + 10,
                   SWP_NOMOVE);
    ...
    for (int i = 0; i < nButton; i++) {
      ...
      /*			RECT st = button; st.left+=20; st.right =
         client.right - 90; st.top -= 3;

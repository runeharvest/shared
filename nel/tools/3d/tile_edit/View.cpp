...
__int64 flagGroupSort = 0;
int showNULL = 0;
// ViewPopup *theViewPopup = 0;

bool zouille() { return (GetAsyncKeyState(VK_F2) & (1 << 15)) != 0; };

// Rotate a buffer
void rotateBuffer(uint &Width, uint &Height,
...
static bool loadPic(const string &path, std::vector<NLMISC::CBGRA> &tampon,
...
int _LoadBitmap(const std::string &path, LPBITMAPINFO BitmapInfo,
                std::vector<NLMISC::CBGRA> &Tampon,
...
int TileList::setTile128(int tile, const std::string &name,
  ...
  if (RemovePath(troncated, tileBank2.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBank2.getAbsPath() + troncated, tampon, Width, Height)) {
      ...
      return (int)(MessageBox(NULL, nlUtf8ToTStr(msg), _T("Can't load bitmap."),
      ...
      if ((error != CTileSet::ok) && (error != CTileSet::addFirstA128128) &&
        ...
        std::string msg = NLMISC::toString("%s\nPixel: %d (%s)\nContinue ?",
                                           CTileSet::getErrorMessage(error),
                                           pixel, comp[composante]);

        return (int)(MessageBox(NULL, nlUtf8ToTStr(msg), _T("Can't add tile"),
                                MB_YESNO | MB_ICONEXCLAMATION) == IDYES);
      } else {
        if (error == CTileSet::addFirstA128128)
          tileBank2.getTileSet(_tileSet)->setBorder(type, border);

        tileBank2.getTileSet(_tileSet)->setTile128(tile, troncated, type,
                                                   tileBank2);
        switch (type) {
        case CTile::diffuse:
    ...
    std::string msg = NLMISC::toString(
        "The bitmap %s is not in the absolute path %s.\nContinue ?",
        name.c_str(), tileBank2.getAbsPath().c_str());
    return (int)(MessageBox(NULL, nlUtf8ToTStr(msg), _T("Load error"),
                            MB_YESNO | MB_ICONEXCLAMATION) == IDYES);
  }
...
int TileList::setTile256(int tile, const std::string &name,
                         NL3D::CTile::TBitmap type) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBank2.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBank2.getAbsPath() + troncated, tampon, Width, Height)) {
      ...
      if (type == CTile::additive)
        ...
        error = tileBank2.getTileSet(_tileSet)->checkTile256(type, border,
                                                             pixel, composante);
      if ((error != CTileSet::ok) && !zouille()) {
        ...
        tileBank2.getTileSet(_tileSet)->setTile256(tile, troncated, type,
                                                   tileBank2);
        switch (type) {
        case CTile::diffuse:
...
int TileList::setTileTransition(int tile, const std::string &name,
                                NL3D::CTile::TBitmap type) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBank2.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBank2.getAbsPath() + troncated, tampon, Width, Height)) {
      ...
      if ((error != CTileSet::ok) && (error != CTileSet::addFirstA128128) &&
        ...
        std::string msg = NLMISC::toString("%s\nPixel: %d (%s)\nContinue ?",
                                           CTileSet::getErrorMessage(error),
                                           pixel, comp[composante]);
        return MessageBox(NULL, nlUtf8ToTStr(msg), _T("Can't add tile"),
                          MB_YESNO | MB_ICONEXCLAMATION) == IDYES;
      } else {
        if (error == CTileSet::addFirstA128128)
          tileBank2.getTileSet(_tileSet)->setBorder(type, border);
        tileBank2.getTileSet(_tileSet)->setTileTransition(
            (CTileSet::TTransition)tile, troncated, type, tileBank2, border);
        switch (type) {
        case CTile::diffuse:
        ...
        theListTransition[tile].Load(
            tileBank2.getTileSet(_tileSet)->getTransition(tile)->getTile(),
    ...
    // Error: bitmap not in the absolute path..
    std::string msg =
        NLMISC::toString("The bitmap %s is not in the absolute path %s.",
                         name.c_str(), tileBank2.getAbsPath().c_str());
    MessageBox(NULL, nlUtf8ToTStr(msg), _T("Load error"),
...
int TileList::setDisplacement(int tile, const std::string &name) {
  ...
  if (RemovePath(troncated, tileBank2.getAbsPath().c_str())) {
    ...
    if (!troncated.empty()) {
      ...
      if (!_LoadBitmap(tileBank2.getAbsPath() + troncated,
                       &theListDisplacement[tile].BmpInfo,
                       theListDisplacement[tile].Bits, NULL, 0)) {
        std::string msg = tileBank2.getAbsPath() + troncated;
        MessageBox(NULL, nlUtf8ToTStr(msg), _T("Can't load file"),
                   MB_OK | MB_ICONEXCLAMATION);
      } else {
        // Check the size
        if ((theListDisplacement[tile].BmpInfo.bmiHeader.biWidth != 32) ||
            (-theListDisplacement[tile].BmpInfo.bmiHeader.biHeight != 32)) {
          // Error message
          MessageBox(NULL,
                     _T("Invalid size: displacement map must be 32x32 8 bits."),
                     nlUtf8ToTStr(troncated), MB_OK | MB_ICONEXCLAMATION);
          ...
          tileBank2.getTileSet(_tileSet)->setDisplacement(
              (CTileSet::TDisplacement)tile, troncated, tileBank2);
        }
      }
    }
  } else {
...
int TileList::setTileTransitionAlpha(int tile, const std::string &name,
                                     int rot) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBank2.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBank2.getAbsPath() + troncated, tampon, Width, Height)) {
      std::string msg = tileBank2.getAbsPath() + troncated + "\nContinue ?";
      return MessageBox(NULL, nlUtf8ToTStr(msg), _T("Can't load bitmap."),
                        MB_YESNO | MB_ICONEXCLAMATION) == IDYES;
    } else {
      CTileBorder border;
      border.set(Width, Height, tampon);
      ...
      int pixel = -1;
      int composante = 4;
      if (((error = tileBank2.getTileSet(_tileSet)->checkTileTransition(
                (CTileSet::TTransition)tile, CTile::alpha, border, indexError,
                pixel, composante)) != CTileSet::ok) &&
          !zouille()) {
        std::string msg;
        if ((error == CTileSet::topInterfaceProblem) ||
            (error == CTileSet::bottomInterfaceProblem) ||
            (error == CTileSet::leftInterfaceProblem) ||
            (error == CTileSet::rightInterfaceProblem) ||
            (error == CTileSet::topBottomNotTheSame) ||
            (error == CTileSet::rightLeftNotTheSame) ||
            (error == CTileSet::topInterfaceProblem)) {
          static const char *comp[] = {"Red", "Green", "Blue", "Alpha", ""};
          if (indexError != -1)
            msg = NLMISC::toString(
                "%s\nIncompatible with tile nb %d\nPixel: %d (%s)",
                CTileSet::getErrorMessage(error), indexError, pixel,
                comp[composante]);
          else
            msg = NLMISC::toString(
                "%s\nIncompatible with the 128x128 tile\nPixel: %d (%s)",
                CTileSet::getErrorMessage(error), pixel, comp[composante]);
        } else {
        ...
        tileBank2.getTileSet(_tileSet)->setTileTransitionAlpha(
            (CTileSet::TTransition)tile, troncated, tileBank2, border, rot);
        theListTransition[tile].alphaLoaded = 0;
        theListTransition[tile].Load(
            tileBank2.getTileSet(_tileSet)->getTransition(tile)->getTile(),
            NULL);
    ...
    // Error: bitmap not in the absolute path..
    TCHAR msg[512];
    _stprintf(msg, _T("The bitmap %s is not in the absolute path %s."),
              nlUtf8ToTStr(name), nlUtf8ToTStr(tileBank2.getAbsPath()));
...
void TileList::clearTransition(int index, CTile::TBitmap bitmap) {
  ...
  tileBank2.getTileSet(_tileSet)->clearTransition((CTileSet::TTransition)index,
                                                  bitmap, tileBank2);
}

void TileList::clearDisplacement(int index) {
  // Clear the displacement map filename
  tileBank2.getTileSet(_tileSet)->clearDisplacement(
      (CTileSet::TDisplacement)index, tileBank2);
...
const std::string &TileInfo::getRelativeFileName(CTile::TBitmap type,
                                                 int index) {
...
bool TileInfo::Load(int index, std::vector<NLMISC::CBGRA> *Alpha) {
  ...
  if (!loaded && !getRelativeFileName(CTile::diffuse, index).empty()) {
    if (!_LoadBitmap(tileBank2.getAbsPath() +
                         getRelativeFileName(CTile::diffuse, index),
                     &BmpInfo, Bits, Alpha, 0)) {
      bRes = false;
      MessageBox(NULL,
                 nlUtf8ToTStr(tileBank2.getAbsPath() +
                              getRelativeFileName(CTile::diffuse, index)),
                 _T("Can't load file"), MB_OK | MB_ICONEXCLAMATION);
    } else
      loaded = 1;
  }
  if (!nightLoaded && !getRelativeFileName(CTile::additive, index).empty()) {
    if (!_LoadBitmap(tileBank2.getAbsPath() +
                         getRelativeFileName(CTile::additive, index),
                     &nightBmpInfo, nightBits, Alpha, 0)) {
      bRes = false;
      MessageBox(NULL,
                 nlUtf8ToTStr(tileBank2.getAbsPath() +
                              getRelativeFileName(CTile::additive, index)),
  ...
  if (!alphaLoaded && !getRelativeFileName(CTile::alpha, index).empty()) {
    if (!_LoadBitmap(tileBank2.getAbsPath() +
                         getRelativeFileName(CTile::alpha, index),
                     &alphaBmpInfo, alphaBits, NULL,
...
void TileList::Reload(int first, int count,
...
// CTView
CTView::CTView() {
  sizetile_x = SIZE_SMALL;
  sizetile_y = SIZE_SMALL;
  sizetext_y = 14;
  spacing_x = SPACING_SMALL_X;
  spacing_y = SPACING_SMALL_Y;
  Zoom = 3;
  Texture = 1;
  Sort = 1;
  InfoTexte = 1;
...
void CTView::Init(int _land, int n) {
  ...
  TCHAR *defautpath = ((SelectionTerritoire *)GetParent()->GetParent())
                          ->DefautPath.GetBuffer(256);
  count_ = 1;
}
...
int CTView::GetNbTileLine(void) {
  RECT rect;
  GetClientRect(&rect);
  return (max(1, int(rect.right - rect.left - spacing_x) /
                     (sizeicon_x + spacing_x)));
}
...
int CTView::GetNbTileColumn(void) {
  ...
  GetClientRect(&rect);
  int deb;
  if (scrollpos < spacing_y)
    deb = -scrollpos;
  else
    deb = (scrollpos - spacing_y) % (sizeicon_y + spacing_y);
  if (deb > sizeicon_y)
    deb -= sizeicon_y + spacing_y;
  int ret = ((rect.bottom - rect.top /*- spacing_y*/ + deb) /
             (sizeicon_y + spacing_y)) +
            1;
  return max(1, ret);
}

void CTView::GetVisibility(int &First, int &Last,
                           int n) // retourne l'indice du premier et du dernier
                                  // item visible dans la fenetre
{
  int y;
  int i = GetNbTileLine();
  ...
  if (scrollpos < spacing_y)
    ...
    y = (scrollpos - spacing_y) % (sizeicon_y + spacing_y);
  First = ((scrollpos - spacing_y) / (sizeicon_y + spacing_y));
  if (y > sizeicon_y)
    First++;
  First *= i;
  Last = First + i * j - 1;
  if (InfoList.GetSize(n) > 0 && Last >= InfoList.GetSize(n))
    Last = InfoList.GetSize(n) - 1;
  if (First > Last)
    First = Last;
}

int CTView::GetIndex(LPPOINT pt, int n) // retourne l'index d'un incone a partir
                                        // de sa position dans le fenetre
// si le curseur n'est pas sur un icon, retourne -1
{
  ...
  GetClientRect(&rect);
  int i = GetNbTileLine();
  int lf = (rect.right - rect.left) -
           (GetNbTileLine() * (spacing_x + sizeicon_x) + spacing_x);
  if (pts.x > (rect.right - rect.left - lf))
    return -1;
  if (pts.x < 0)
    return -1;
  if (pts.y < spacing_y)
  ...
  int ret = ic + il * i;
  if (ret < InfoList.GetSize(n) && ret >= 0)
    return ret;
  else
    return -1;
}
...
void CTView::UpdateSelection(LPRECT rect_, int mode,
                             int n) // rect : coordonnees du rectangle de
                                    // selection dans la fenetre parent
{
  RECT client, *rect;
  ...
  if (rect->top < scrollpos)
    rect->top = scrollpos;
  int lf = (client.right - client.left) -
           (GetNbTileLine() * (spacing_x + sizeicon_x) + spacing_x);
  if (rect->right > (client.right - client.left - spacing_x - lf)) {
    rect->right = client.right - client.left - spacing_x - lf;
  }
  if ((rect->bottom - scrollpos) > (client.bottom - client.top))
    rect->bottom = client.bottom - client.top + scrollpos;

  if (rect->left < spacing_x)
    rect->left = spacing_x;
  if ((rect->top - scrollpos) < spacing_y)
    rect->top = spacing_y + scrollpos;
  int rx = (rect->left - spacing_x) % (spacing_x + sizeicon_x);
  rect->left -= rx;
  if (rx >= sizeicon_x) {
    rect->left += sizeicon_x + spacing_x;
    if (rect->left >= (client.right - client.left))
      rect->left = (client.right - client.left) - spacing_x;
  }
  int ry = (rect->top - spacing_y) % (spacing_y + sizeicon_y);
  rect->top -= ry;
  if (ry >= sizeicon_y) {
    rect->top += sizeicon_y + spacing_y;
    if ((rect->top - scrollpos) >= (client.bottom - client.top))
      rect->top = (client.bottom - client.top) - spacing_y + scrollpos;
  }

  if (rect->right < spacing_x || (rect->bottom - scrollpos) < spacing_y)
    return;
...
int CTView::IsSelected(int i) { return 0; }

void CTView::UpdateSize(int n) {
  spacing_tile_text = 3;
  if (Zoom == 1) {
    sizetile_x = sizetile_y = SIZE_SMALL;
    spacing_x = SPACING_SMALL_X;
    spacing_y = SPACING_SMALL_Y;
  }
  if (Zoom == 2) {
    sizetile_x = sizetile_y = SIZE_NORMAL;
    spacing_x = SPACING_NORMAL_X;
    spacing_y = SPACING_NORMAL_Y;
  }
  if (Zoom == 3) {
    sizetile_x = sizetile_y = SIZE_BIG;
    spacing_x = SPACING_BIG_X;
    spacing_y = SPACING_BIG_Y;
  }
  if (n == 1) {
    sizetile_x *= 2;
    sizetile_y *= 2;
  }
  sizeicon_x = sizetile_x;
  sizeicon_y = sizetile_y + sizetext_y + spacing_tile_text;
}
...
void CTView::OnPaint() {
  ...
  if (InfoList.GetSize(parent->m_128x128) == 0)
    return;
  ...
  if (!normal_font) {
    ...
    normal_font->CreateFont(-10, 0, 0, 0, FW_THIN, false, false, false,
...
void CTView::OnDropFiles(HDROP hDropInfo) {
  ...
  if (!lockInsertion) {
    Browse *parent = (Browse *)this->GetParent();
    TCHAR FileName[256];
    int count = DragQueryFile(hDropInfo, 0xffffffff, FileName,
                              256); // count = files number in drop queue
    ...
    for (int i = 0; i < count; i++) {
      ...
      case 0: {
        ...
        if (InfoList.setTile128(index, NLMISC::tStrToUtf8(FileName),
                                Texture == 1 ? CTile::diffuse
                                             : (Texture == 2 ? CTile::additive
                                                             : CTile::alpha))) {
      ...
      case 1: {
        ...
        if (InfoList.setTile256(index, NLMISC::tStrToUtf8(FileName),
                                Texture == 1 ? CTile::diffuse
                                             : (Texture == 2 ? CTile::additive
                                                             : CTile::alpha))) {
          tilelist::iterator it = InfoList.GetLast(parent->m_128x128);
...
void CTView::DrawTile(tilelist::iterator i, CDC *pDC, int clear, int n) {
  ...
  switch (n) {
  case 0:
    pth = i->getRelativeFileName(
        (CTile::TBitmap)(Texture - 1),
        tileBank2.getTileSet(InfoList._tileSet)->getTile128(i->id));
    break;
  case 1:
    pth = i->getRelativeFileName(
        (CTile::TBitmap)(Texture - 1),
        tileBank2.getTileSet(InfoList._tileSet)->getTile256(i->id));
    break;
  case 2: {
    int index = tileBank2.getTileSet(InfoList._tileSet)
                    ->getTransition(i->id)
                    ->getTile();
    if (index != -1)
  ...
  case 3:
    pth = tileBank2.getDisplacementMap(
        tileBank2.getTileSet(InfoList._tileSet)
            ->getDisplacementTile((CTileSet::TDisplacement)i->id));
    break;
  }
  ...
  rect_txt.left = pt.x;
  rect_txt.right = pt.x + sizeicon_x + spacing_x;
  ...
  if (!loaded) {
    pDC->FillSolidRect(pt.x, pt.y, sizetile_x, sizetile_y,
    ...
    StretchDIBits(pDC->m_hDC, pt.x, pt.y, sizetile_x, sizetile_y, 0, 0,
                  bmpinf->bmiHeader.biWidth, -bmpinf->bmiHeader.biHeight,
  ...
  pDC->DrawText((LPCTSTR)tmp.c_str(), (int)tmp.length(), &rect_txt,
  ...
  if (i->Selected & 3) {
    ...
    rc.left = pt.x;
    rc.top = pt.y;
    rc.right = rc.left + sizetile_x;
    rc.bottom = rc.top + sizetile_y;
    ShadeRect(pDC, rc);
  }
...
void CTView::DrawDragRect(CDC *pDC, LPCRECT lpRect, SIZE size,
                          LPCRECT lpRectLast, SIZE sizeLast, CBrush *pBrush,
                          CBrush *pBrushLast) {
  ...
  if (lpRectLast != NULL /* && lpRect!=NULL*/) {
    ...
    if (lpRect == NULL)
      rgnOutside.CreateRectRgnIndirect(lpRectLast);
    else
  ...
  if (!lpRect ||
      (pBrush->m_hObject != pBrushLast->m_hObject && lpRectLast != NULL)) {
...
void CTView::RemoveSelection(int n) {
  for (tilelist::iterator p = InfoList.theList[n].begin();
       p != InfoList.theList[n].end(); ++p) {
    p->Selected = 0;
  }
}
...
LRESULT CTView::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_COMMAND) // The user had selected an item on popup menu
  {
    ...
    if (id == ID_MENU_SUPR_TILE) {
      ...
      CFileDialog load(true, NULL, nlUtf8ToTStr(LastPath),
                       OFN_ENABLESIZING | OFN_ALLOWMULTISELECT,
                       _T("PNG Bitmap (*.png)|*.png|Targa bitmap ")
                       _T("(*.tga)|*.tga|All files (*.*)|*.*||"),
                       NULL);
      ...
      if (load.DoModal() == IDOK) {
        ...
        // AfxMessageBox ("toto");
        POSITION p = load.GetStartPosition(); // la doc dit que p=NULL quand il
                                              // n'y a pas de selection : c'est
                                              // faux, genial les MFC
        while (p) {
          ...
          if (!str.IsEmpty()) {
            ...
            if (id == ID_MENU_ADD) {
              ...
              // For each tile
              for (int index = 0; index < InfoList.GetSize(parent->m_128x128);
                   index++) {
                // If selected
                if (InfoList.Get(index, parent->m_128x128)->Selected) {
                  ...
                  case 0:
                    // Set the 128 tile
                    ok = InfoList.setTile128(
                        index, pathname,
                        Texture == 1
                            ? CTile::diffuse
                            : (Texture == 2 ? CTile::additive : CTile::alpha));
                    break;
                  case 1:
                    // Set the 256 tile
                    ok = InfoList.setTile256(
                        index, pathname,
                        Texture == 1
                            ? CTile::diffuse
                            : (Texture == 2 ? CTile::additive : CTile::alpha));
                  ...
                  case 2:
                    ...
                    // Alpha!
                      ...
                      if (selectRotation.DoModal() == IDOK) {
                        ...
                        ok = InfoList.setTileTransitionAlpha(
                            index, pathname, selectRotation.RotSelected);
                      }
                    }
                    break;
                  case 3:
...
void CTView::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (!lockInsertion) {
    ...
    int c = 0;
    for (tilelist::iterator p = InfoList.GetFirst(parent->m_128x128);
         p != InfoList.GetLast(parent->m_128x128); ++p) {
      if (p->Selected)
        c++;
    }

    if (!ViewTileMode) {
      popup.AppendMenu(parent->m_128x128 < 2 ? MF_STRING
                                             : MF_STRING | MF_GRAYED,
                       ID_MENU_ADD, _T("Add..."));
      popup.AppendMenu(c > 0 ? MF_STRING : MF_STRING | MF_GRAYED,
                       ID_MENU_REPLACE, _T("Replace..."));
      popup.AppendMenu(c > 0 ? MF_STRING : MF_STRING | MF_GRAYED,
                       ID_MENU_SUPR_BITMAP, _T("Del bitmap"));
      popup.AppendMenu((c > 0 && parent->m_128x128 < 2) ? MF_STRING
                                                        : MF_STRING | MF_GRAYED,
                       ID_MENU_SUPR_TILE, _T("Del tile"));
    } else {
    }
    bPopup = 1;
    popup.TrackPopupMenu(TPM_LEFTALIGN, MousePos.x + wndpos.left,
                         MousePos.y + wndpos.top, GetParent(), NULL);
  }

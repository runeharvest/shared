...
public:
  EditTerr(int land) { _land = land; }

private:
  virtual void OnInit() {
    UpdateData();
    TCHAR sTitle[512];
    _stprintf(sTitle, _T("Tile sets use by %s"),
              nlUtf8ToTStr(tileBank.getLand(_land)->getName()));
    SetWindowText(sTitle);
    for (int i = 0; i < tileBank.getTileSetCount(); i++) {
      m_ctrlCombo.InsertString(-1,
                               nlUtf8ToTStr(tileBank.getTileSet(i)->getName()));
      if (tileBank.getLand(_land)->isTileSet(tileBank.getTileSet(i)->getName()))
        m_ctrlList.InsertString(
            -1, nlUtf8ToTStr(tileBank.getTileSet(i)->getName()));
    }
    UpdateData(FALSE);
  }
  virtual void OnOk() {
...
void SelectionTerritoire::OnRemoveTerritoire() {
  ...
  if (nindex != LB_ERR) {
    ...
    MessageBox(_T("No tilesset selected"),
               _T("Chcrois kca va pas etreuu possibleuuu"), MB_ICONERROR);
  }
}
...
public:
  EditTileSet(int tileSet) { _tileSet = tileSet; }

private:
  virtual void OnInit() {
    UpdateData();
    TCHAR sTitle[512];
    _stprintf(sTitle, _T("Children of the tile set %s"),
              nlUtf8ToTStr(tileBank.getTileSet(_tileSet)->getName()));
    SetWindowText(sTitle);
    for (int i = 0; i < tileBank.getTileSetCount(); i++) {
      if (i != _tileSet)
        m_ctrlCombo.InsertString(
            -1, nlUtf8ToTStr(tileBank.getTileSet(i)->getName()));

      if (tileBank.getTileSet(_tileSet)->isChild(
  ...
  virtual void OnOk() {
    ...
    for (i = 0; i < tileBank.getTileSetCount(); i++) {
      // remove tile set
      tileBank.getTileSet(_tileSet)->removeChild(
          tileBank.getTileSet(i)->getName());
    }
    for (i = 0; i < m_ctrlList.GetCount(); i++) {
      CString rString;
...
void SelectionTerritoire::OnSelect() {
  ...
  static TCHAR BASED_CODE szFilter[] =
      _T("NeL tile bank files (*.bank)|*.bank|All Files (*.*)|*.*||");
...
LRESULT SelectionTerritoire::WindowProc(UINT message, WPARAM wParam,
                                        LPARAM lParam) {
...
void SelectionTerritoire::OnOK() {
  ...
  if (::MessageBox(NULL, _T("Are you sure you want to quit?"), _T("Quit"),
...
void SelectionTerritoire::OnSaveAs() {
  ...
  static TCHAR BASED_CODE szFilter[] =
      _T("NeL tile bank files (*.bank)|*.bank|All Files (*.*)|*.*||");
  CFileDialog sFile(false, _T("bank"), DefautPath + MainFileName, 0, szFilter,
                    this);
  if (sFile.DoModal() == IDOK) {
...
bool CheckPath(const std::string &path,
               const std::string &absolutePathToRemplace) {
  // Look for absolute path in path
  if (strnicmp(path.c_str(), absolutePathToRemplace.c_str(),
               absolutePathToRemplace.length()) == 0)
    return true;
...
void SelectionTerritoire::OnPath() {
  ...
  if (list) {
    ...
    TCHAR msg[512];
    _stprintf(msg,
              _T("Do you really want to set %s as base path of the bank ?"),
              path);
    ...
    if (MessageBox(msg, _T("TileEdit"), MB_YESNO | MB_ICONQUESTION) == IDYES) {
      ...
      if ((*oldPath) == 0) {
            ...
            for (type = CTile::diffuse; type < CTile::bitmapCount; type++) {
              ...
              if (!bitmapPath.empty()) {
                ...
                if (CheckPath(bitmapPath, tStrToUtf8(path)) == false) {
                  ...
                  _stprintf(
                      msg,
                      _T("Path '%s' can't be found in bitmap '%s'. Continue ?"),
                      path, nlUtf8ToTStr(bitmapPath));

                  // Message
                  if (MessageBox(msg, _T("TileEdit"),
        ...
        for (uint noise = 1; noise < tileBank.getDisplacementMapCount();
          ...
          if (strcmp(bitmapPath, "") != 0) {
            ...
            if (CheckPath(bitmapPath, tStrToUtf8(path)) == false) {
              ...
              // Message
              if (MessageBox(msg, _T("TileEdit"), MB_YESNO | MB_ICONQUESTION) ==
                  IDNO)
                break;
            }
          }
        ...
        if (goodPath) {
          ...
          // Info message
          MessageBox(_T("Can't set the path."), _T("TileEdit"),
...
void SelectionTerritoire::OnExport() {
  ...
  CFileDialog sFile(false, _T("*.smallbank"), DefautPath + _T("*.smallbank"), 0,
                    szFilter, this);
...
void SelectionTerritoire::OnChooseVeget() {
  ...
  int index = list->GetCurSel();
  if (index != LB_ERR) {
    CChooseVegetSet chooseVeget(
        this, tileBank.getTileSet(index)->getTileVegetableDescFileName());
    if (chooseVeget.DoModal() == IDOK) {
      tileBank.getTileSet(index)->setTileVegetableDescFileName(
          chooseVeget.FileName);
    }
  } else {
    MessageBox(_T("No tilesset selected"), _T("Error"), MB_ICONERROR);
  }

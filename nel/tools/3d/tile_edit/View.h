...
public:
  ...
  // !)
  int loaded, nightLoaded,
      alphaLoaded; // tells if the tile was already loaded or not
...
public:
  ...
  // int Add (const char *path,const char *pathNight,const char *pathBump, int
  // hh, int bb, int gg, int dd, unsigned int flags);
  ...
  int setTileTransition(int tile, const std::string &name,
  ...
  void Reload(int first, int count,
...
public:
  ...
  void DrawDragRect(CDC *pDC, LPCRECT lpRect, SIZE size, LPCRECT lpRectLast,
                    SIZE sizeLast, CBrush *pBrush = NULL,

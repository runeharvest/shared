...
void displayMeshBase(FILE *logStream, CMeshBase *meshBase) {
  ...
  if (nLms) {
    ...
    for (uint i = 0; i < nLms; i++) {
      ...
      fprintf(logStream, "  LightGroup=%d; AnimatedLight='%s'; mat/stage: ", lg,
              al.c_str());
      std::list<CMeshBase::CLightMapInfoList::CMatStage>::iterator it =
          meshBase->_LightInfos[i].StageList.begin();
...
void displayInfoFileInStream(FILE *logStream, const char *fileName,
    ...
    if (strstr(fileName, ".zone")) {
      ...
      if (vl) {
        ...
        for (k = 0; k < zoneInfo.PointLights.size(); ++k) {
          ...
          fprintf(logStream,
                  "    light group = %d, anim = \"%s\" x=%.1f, y=%.1f, z=%.1f, "
                  "r=%d, g=%d, b=%d, dr=%d, dg=%d, db=%d\n",
                  pl.LightGroup, pl.AnimatedLight.c_str(), pl.getPosition().x,
                  pl.getPosition().y, pl.getPosition().z, diffuse.R, diffuse.G,
                  diffuse.B, defaultDiffuse.R, defaultDiffuse.G,
                  defaultDiffuse.B);
        }
      }
      if (vp) {
        ...
        for (k = 0; k < zoneInfo.Patchs.size(); ++k) {
          fprintf(logStream,
                  "   Patch %d, S %d, T %d, smooth flags %d %d %d %d, corner "
                  "flags %d %d %d %d\n",
                  k, zoneInfo.Patchs[k].OrderS, zoneInfo.Patchs[k].OrderT,
          ...
          for (l = 0; l < 4; l++) {
            fprintf(logStream,
                    "    Bind edge %d, NPatchs %d, ZoneId %d, Next %d %d %d "
                    "%d, Edge %d %d %d %d\n",
                    l, zoneInfo.Patchs[k].BindEdges[l].NPatchs,
      ...
      if (vi) {
        ...
        for (k = 0; k < ig._InstancesInfos.size(); ++k) {
          fprintf(logStream,
                  "    Instance %3d: shape = %s, name = %s, x = %.1f, y = "
                  "%.1f, z = %.1f, sx = %.1f, sy = %.1f, sz = %.1f\n",
                  k, ig._InstancesInfos[k].Name.c_str(),
                  ig._InstancesInfos[k].InstanceName.c_str(),
                  ig._InstancesInfos[k].Pos.x + gpos.x,
                  ig._InstancesInfos[k].Pos.y + gpos.y,
                  ig._InstancesInfos[k].Pos.z + gpos.z,
                  ig._InstancesInfos[k].Scale.x, ig._InstancesInfos[k].Scale.y,
                  ig._InstancesInfos[k].Scale.z);
        }
      }
      if (vl) {
        fprintf(logStream, "  Lights:\n");
        ...
        for (k = 0; k < ig.getNumPointLights(); ++k) {
          ...
          CRGBA defaultDiffuse = pl.getDefaultDiffuse();
          fprintf(logStream,
                  "    Light %3d: Light group = %d, anim = \"%s\" x=%.1f, "
                  "y=%.1f, z=%.1f, r=%d, g=%d, b=%d, dr=%d, dg=%d, db=%d\n",
                  k, pl.LightGroup, pl.AnimatedLight.c_str(),
                  pl.getPosition().x + gpos.x, pl.getPosition().y + gpos.y,
                  pl.getPosition().z + gpos.z, diffuse.R, diffuse.G, diffuse.B,
                  defaultDiffuse.R, defaultDiffuse.G, defaultDiffuse.B);
        }
      }
      if (veil) {
        fprintf(logStream, "  Instances Bound To Lights:\n");
        fprintf(logStream, "    WordList:\n");
        fprintf(logStream, "    'StaticLight Not Computed' means the instance "
                           "has a ASP flag or the ig is not yet lighted\n");
        fprintf(logStream, "    If lighted, for each instance, the format is "
                           "'SunContribution(8Bit) - idLight0;idLight1 (or "
                           "NOLIGHT) - LocalAmbientId (or GLOBAL_AMBIENT)' \n");
        fprintf(logStream, "    DCS means the instance don't cast shadow (used "
                           "in the lighter)\n");
        fprintf(logStream,
                "    DCSINT Same but very special for ig_lighter only\n");
        fprintf(logStream, "    DCSEXT Same but very special for zone_lighter "
                           "and zone_ig_lighter only\n");
        fprintf(logStream,
                "    ASP means the instance AvoidStaticLightPreCompute (used "
                "in the lighter)\n");
        fprintf(
            logStream,
            "  "
      ...
      file.serial(shapeStream);
      CSkeletonShape *skel =
          dynamic_cast<CSkeletonShape *>(shapeStream.getShapePointer());
      ...
      CMesh *mesh = dynamic_cast<CMesh *>(shapeStream.getShapePointer());
      CMeshMRM *meshMRM =
          dynamic_cast<CMeshMRM *>(shapeStream.getShapePointer());
      CMeshMRMSkinned *meshMRMSkinned =
          dynamic_cast<CMeshMRMSkinned *>(shapeStream.getShapePointer());
      CMeshMultiLod *meshMulti =
          dynamic_cast<CMeshMultiLod *>(shapeStream.getShapePointer());

      // Material infos
      CMeshBase *meshBase =
          dynamic_cast<CMeshBase *>(shapeStream.getShapePointer());
      if (meshBase) {
      ...
      // MultiLod??
        ...
        if (numSlots) {
          const CMeshGeom *meshGeom =
              dynamic_cast<const CMeshGeom *>(&(meshMulti->getMeshGeom(0)));
          const CMeshMRMGeom *meshMRMGeom =
              dynamic_cast<const CMeshMRMGeom *>(&(meshMulti->getMeshGeom(0)));
          if (meshGeom)
                ...
                for (i = 0; i < keys.size(); i++) {
                  if (track->interpolate(keys[i], cvalue))
                    fprintf(logStream,
                            ...
                            keys[i], cvalue.R, cvalue.G, cvalue.B, cvalue.A);
                }
                ...
                for (i = 0; i < keys.size(); i++) {
                  if (track->interpolate(keys[i], qvalue))
                    fprintf(logStream,
                            ...
                            keys[i], qvalue.x, qvalue.y, qvalue.z, qvalue.w);
                }
...
void displayInfoFile(FILE *logStream, const char *fileName,
                     const set<string> &options, bool displayShortFileName) {
  // Display on screen.
...
int main(int argc, const char *argv[]) {
  ...
  if (argc < 2) {
    ...
    puts("    [opt] can get: ");
    puts("    -ms display only a Warning if file is a .shape and is a Mesh, "
         "skinned, but without MRM");
  ...
  if (CFile::isDirectory(fileName)) {
    ...
    if (logStream)
      fprintf(logStream,
              "\n\n ************** \n I HAVE FOUND AT MAX %d LIGHTMAPS IN A "
              "SHAPE\n",
              MaxNumLightMap);
  } else {

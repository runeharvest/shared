...
void buildFaces(CLandscape &landscape, sint zoneId, sint patch,
...
int main(int argc, char *argv[]) {
  try {
    ...
    if (argc != 4) {
      ...
      printf("\t\tVertices are CCW, in a right hand basis with Z axis to the "
             "top\n");
    } else {
            ...
            for (int y = yMin; y <= yMax; y++)
              for (int x = xMin; x <= xMax; x++) {
                ...
                if (input.open(path + name + ext)) {
                  ...
                  for (uint patch = 0; patch < (uint)zone.getNumPatchs();
                       patch++) {
                    // vector of triangle
                    std::vector<CTriangle> faces;

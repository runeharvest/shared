...
//**************************************************************************************************************************
CParticleDlg::CParticleDlg(class CObjectViewer *main, CWnd *pParent,
                           CMainFrame *mainFrame, CAnimationDlg *animDLG)
    : CDialog(CParticleDlg::IDD, pParent), MainFrame(mainFrame),
      CurrentRightPane(NULL), _ActiveNode(NULL), _ObjView(main),
      _EmptyBBox(true), _AutoUpdateBBox(false), _PW(NULL)

{
  ...
  NL3D::CParticleSystem::setSerializeIdentifierFlag(
      true); // serialize identifiers for edition
...
CParticleDlg::~CParticleDlg() {
  ...
  delete StartStopDlg;
  if (_PW)
    _PW->setModificationCallback(NULL);
  delete _PW;
}
...
BOOL CParticleDlg::OnInitDialog() {
  ...
  _StatusBar.SetPaneInfo(0, ID_PS_EDITOR_STATUS, SBPS_NORMAL,
...
void CParticleDlg::OnSize(UINT nType, int cx, int cy) {
  ...
  if (ParticleTreeCtrl->m_hWnd && this->m_hWnd) {
    ...
    if (CurrentRightPane) {
      CurrentRightPane->MoveWindow(r.right + 10, r.top,
                                   r.right + CurrRightPaneWidth + 10,
                                   r.top + CurrRightPaneHeight);
    }
...
LRESULT CParticleDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) {
  ...
  if (message == WM_GETMINMAXINFO) {
    ...
    if (CurrentRightPane) {
      ...
      if (cy < (CurrRightPaneHeight + 20))
        cy = CurrRightPaneHeight + 20;
    }
...
void CParticleDlg::goPostRender() {
  NL3D::CParticleSystem *currPS =
      _ActiveNode ? _ActiveNode->getPSPointer() : NULL;
  if (!currPS)
    return;
  if (StartStopDlg->isBBoxDisplayEnabled() && currPS) {
    ...
    NL3D::CPSUtil::displayBBox(NL3D::CNELU::Driver, _CurrBBox,
                               currPS->getAutoComputeBBox() ? CRGBA::White
                                                            : CRGBA::Red);
  }
...
void CParticleDlg::stickPSToSkeleton(CParticleWorkspace::CNode *node,
                                     NL3D::CSkeletonModel *skel, uint bone,
...
//**************************************************************************************************************************
bool CParticleDlg::savePS(HWND parent, CParticleWorkspace::CNode &psNode,
                          bool askToContinueWhenError) {
  return savePSAs(parent, psNode, psNode.getFullPath(), askToContinueWhenError);
}

//**************************************************************************************************************************
bool CParticleDlg::savePSAs(HWND parent, CParticleWorkspace::CNode &psNode,
                            const std::string &fullPath,
                            bool askToContinueWhenError) {
  nlassert(psNode.getPSPointer());
  if (psNode.getResetAutoCountFlag() &&
      psNode.getPSPointer()->getAutoCountFlag()) {
    MessageBox(psNode.getFilename().c_str() + getStrRsc(IDS_AUTO_COUNT_ERROR),
  ...
  try {
    ...
    if (askToContinueWhenError) {
      int result = ::MessageBox(
          parent, CString(e.what()) + getStrRsc(IDS_CONTINUE_SAVING),
          getStrRsc(IDS_ERROR), MB_ICONEXCLAMATION);
      return result == MB_OK;
    } else {
      ::MessageBox(parent, nlUtf8ToTStr(e.what()), getStrRsc(IDS_ERROR),
                   MB_ICONEXCLAMATION);
      return false;
    }
  }
...
//**************************************************************************************************************************
bool CParticleDlg::loadPS(HWND parent, CParticleWorkspace::CNode &psNode,
                          TLoadPSBehav behav) {
  bool loadingOK = true;
  try {
    if (!psNode.loadPS()) {
      ...
      case ReportError:
        ::MessageBox(parent,
                     (LPCTSTR)(CString(psNode.getFilename().c_str()) + " : " +
                               getStrRsc(IDS_COULDNT_INSTANCIATE_PS)),
                     getStrRsc(IDS_ERROR), MB_OK | MB_ICONEXCLAMATION);
        return true;
        break;
      case ReportErrorSkippable: {
        CSkippableMessageBox mb(getStrRsc(IDS_ERROR),
                                CString(psNode.getFilename().c_str()) + " : " +
                                    getStrRsc(IDS_COULDNT_INSTANCIATE_PS),
      ...
      setStatusBarText(CString(psNode.getFullPath().c_str()) + " " +
                       getStrRsc(IDS_LOADED));
    }
  } catch (const NLMISC::Exception &e) {
    switch (behav) {
    case Silent:
    ...
    case ReportError:
      ...
      break;
    case ReportErrorSkippable: {
      CSkippableMessageBox mb(getStrRsc(IDS_ERROR), CString(e.what()),
                              CWnd::FromHandle(parent));
      mb.DoModal();
      return !mb.getBypassFlag();
    } break;
  ...
  if (psNode.getPSPointer()->hasLoop()) {
    localizedMessageBox(parent, IDS_FX_HAS_LOOP, IDS_WARNING,
                        MB_OK | MB_ICONEXCLAMATION);
  }
  return behav != Silent;
}
    ...
    if (_PW->isModified()) {
      int result =
          localizedMessageBox(*this, IDS_PS_WORKSPACE_MODIFIED,
                              IDS_PARTICLE_EDITOR, MB_YESNO | MB_ICONQUESTION);
      if (result == IDYES) {
...
void CParticleDlg::OnCreateNewPsWorkspace() {
  ...
  if (result = IDOK) {
    if (cf.touchFile()) {
      ...
      newPW->init(_ObjView, cf.getFullPath(), _ObjView->getFontManager(),
                  _ObjView->getFontGenerator());
...
void CParticleDlg::loadWorkspace(const std::string &fullPath) {
  ...
  newPW->init(_ObjView, fullPath, _ObjView->getFontManager(),
              _ObjView->getFontGenerator());
  newPW->setModificationCallback(ParticleTreeCtrl);
  // save empty workspace
  try {
    newPW->load();
    setStatusBarText(CString(newPW->getFilename().c_str()) + " " +
                     getStrRsc(IDS_LOADED));
  } catch (const NLMISC::EStream &e) {
    MessageBox(nlUtf8ToTStr(e.what()), getStrRsc(IDS_ERROR),
               MB_ICONEXCLAMATION);
  ...
  for (uint k = 0; k < newPW->getNumNode(); ++k) {
    ...
    displayErrorMsg = loadPS(*this, *newPW->getNode(k),
                             displayErrorMsg ? ReportErrorSkippable : Silent);
...
void CParticleDlg::saveWorkspaceStructure() {
  ...
  try {
    _PW->save();
    setStatusBarText(CString(_PW->getFilename().c_str()) + " " +
                     getStrRsc(IDS_SAVED));
  } catch (const NLMISC::EStream &e) {
    localizedMessageBox(*this, nlUtf8ToTStr(e.what()), IDS_ERROR,
                        MB_ICONEXCLAMATION);
    setStatusBarText(CString(e.what()));
  }
}
...
void CParticleDlg::saveWorkspaceContent(bool askToSaveModifiedPS) {
  ...
  for (uint k = 0; k < _PW->getNumNode(); ++k) {
    if (_PW->getNode(k)->isModified()) {
      if (saveAll) {
        bool keepSaving =
            savePS(*this, *_PW->getNode(k), k != _PW->getNumNode());
        if (!keepSaving)
          break;
      } else {
        // ask if the user wants to save the ps, or save all ps
        CString mess;
        mess = CString(_PW->getNode(k)->getFilename().c_str()) +
               getStrRsc(IDS_SAVE_MODIFIED_PS);
        CSaveOptionsDlg sop(getStrRsc(IDS_SAVE_FILE), mess, this);
        sop.DoModal();
        bool saveThisFile = false;
        bool stop = false;
        switch (sop.getChoice()) {
        case CSaveOptionsDlg::Yes:
          saveThisFile = true;
          break;
        case CSaveOptionsDlg::No:
          saveThisFile = false;
          break;
        case CSaveOptionsDlg::SaveAll:
          saveAll = true;
          break;
        case CSaveOptionsDlg::Stop:
          stop = true;
          break;
        default:
          nlassert(0);
...
void CParticleDlg::setActiveNode(CParticleWorkspace::CNode *node) {
  ...
  if (MainFrame->isMoveFX()) {
    if (node) {
      _ObjView->getMouseListener().setModelMatrix(
          node->getPSModel()->getMatrix());
    } else {
      _ObjView->getMouseListener().setModelMatrix(NLMISC::CMatrix::Identity);
    }
  } else if (MainFrame->isMoveFXUserMatrix()) {
    if (node) {
      _ObjView->getMouseListener().setModelMatrix(
          node->getPSModel()->getUserMatrix());
    } else {
      _ObjView->getMouseListener().setModelMatrix(NLMISC::CMatrix::Identity);
    }
  }
...
CParticleDlg::getModelFromPS(NL3D::CParticleSystem *ps) const {
  if (!ps)
    return NULL;
  if (!_PW)
    return NULL;
...
uint CParticleDlg::computeStatusBarWidth() const {
  ...
  if (!CurrentRightPane)
    return (uint)std::max((sint)tcRect.Width() - 16, (sint)0);
  return (uint)std::max((sint)tcRect.Width() - 4, (sint)0);
}

//**************************************************************************************************************************
void CParticleDlg::OnViewPsFilename() {
  ParticleTreeCtrl->setViewFilenameFlag(
      !ParticleTreeCtrl->getViewFilenameFlag());
  updateMenu();
}
...
void CParticleDlg::updateMenu() {
  ...
  menu->CheckMenuItem(
      ...
      MF_BYCOMMAND |
          (ParticleTreeCtrl->getViewFilenameFlag() ? MF_CHECKED : 0));
}

...
/** construct a dialog that allow to edit a binary operator that produce
 * argument of a particle system
 */
template <class T> class CBinOpDlgT : public CBinOpDlg {
public:
  /// ctruct the given dialog from the given scheme that owns memory
  CBinOpDlgT(NL3D::CPSAttribMakerBinOp<T> *editedScheme,
             CAttribDlgT<T> **attrbDlg, IPopupNotify *pn,
             HBITMAP bitmapToDisplay)
      : CBinOpDlg(pn), _BitmapToDisplay(bitmapToDisplay),
        _EditedScheme(editedScheme) {
    for (uint k = 0; k < 2; ++k) {
      nlassert(attrbDlg);
  ...
  void init(CWnd *pParent) {
    ...
    static const TCHAR *ops[] = {_T("selectArg1"), _T("selectArg2"),
                                 _T("modulate"), _T("add"), _T("subtract")};

...
#if !defined(                                                                  \
    AFX_LOCATED_PROPERTIES_H__772D6C3B_6CFD_47B5_A132_A8D2352EACF9__INCLUDED_)
...
class CLocatedProperties : public CDialog,
  ...
  // Construction
public:
  CLocatedProperties(CParticleWorkspace::CNode *node, NL3D::CPSLocated *loc,
                     CParticleDlg *pdlg); // standard constructor
...
protected:
  ...
  struct CMaxNbParticlesWrapper : public IPSWrapperUInt {
    ...
    void set(const uint32 &v) {
      // if the max new size is lower than the current number of instance, we
      // must suppress item in the CParticleTreeCtrl

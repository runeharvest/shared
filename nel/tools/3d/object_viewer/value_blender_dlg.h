...
#if !defined(                                                                  \
    AFX_VALUEBLENDERDLG_H__4242C860_C538_45BD_8348_C6DF314D688A__INCLUDED_)
...
// user of the dialog must provide an implementation of this struct to provide
// indivual values edition
struct IValueBlenderDlgClient {
  ...
  createDialog(uint index, CParticleWorkspace::CNode *ownerNode) = 0;
...
public:
  /** Create the dialog.
   * \param createInterface interface that allows to create a dialog to edit one
   * of the 2 values used for the blend. \param destroyInterface true if this
   * object must take care to call 'delete' on the 'createInterface' pointer
   */
  CValueBlenderDlg(
      IValueBlenderDlgClient *createInterface, bool destroyInterface,
      CWnd *pParent, IPopupNotify *pn,

...
void CSceneDlgMouseListener::operator()(const CEvent &event) {
  if (event == EventMouseDownId) {
    ...
    if (mouseEvent->Button == rightButton) {
      ...
      menu.CheckMenuItem(ID_ENABLE_ELEMENT_XROTATE,
                         ml.getModelMatrixRotationAxis() ==
                                 CEvent3dMouseListener::xAxis
                             ? MF_CHECKED
                             : MF_UNCHECKED);
      menu.CheckMenuItem(ID_ENABLE_ELEMENT_YROTATE,
                         ml.getModelMatrixRotationAxis() ==
                                 CEvent3dMouseListener::yAxis
                             ? MF_CHECKED
                             : MF_UNCHECKED);
      menu.CheckMenuItem(ID_ENABLE_ELEMENT_ZROTATE,
                         ml.getModelMatrixRotationAxis() ==
                                 CEvent3dMouseListener::zAxis
                             ? MF_CHECKED
                             : MF_UNCHECKED);
      ...
      ::TrackPopupMenu(subMenu->m_hMenu, TPM_LEFTALIGN | TPM_RIGHTBUTTON, x, y,
                       0, SceneDlg->m_hWnd, NULL);
    }
  }
}
...
ON_UPDATE_COMMAND_UI(iDM_SHOW_OCCLUSION_TEST_MESHS,
                     OnUpdateShowOcclusionTestMeshs)
ON_WM_CREATE()
ON_WM_ERASEBKGND()
...
ON_UPDATE_COMMAND_UI(ID_WINDOW_CHOOSE_FRAME_DELAY,
                     OnUpdateWindowChooseFrameDelay)
...
ON_COMMAND_RANGE(IDM_ACTIVATE_TEXTURE_SET_1, IDM_ACTIVATE_TEXTURE_SET_8,
                 OnActivateTextureSet)
...
ON_UPDATE_COMMAND_UI(ID_EDIT_MOVE_OBJECT_LIGHT_TEST,
                     OnUpdateEditMoveObjectLightTest)
...
ON_UPDATE_COMMAND_UI_RANGE(ID_SCENE_CAMERA_FIRST, ID_SCENE_CAMERA_LAST,
                           OnUpdateSceneCamera)
END_MESSAGE_MAP()
...
void CMainFrame::update() {
  ...
  ObjView->_ChooseFrameDelayDlg->ShowWindow(ChooseFrameDelayWindow ? SW_SHOW
                                                                   : SW_HIDE);
  ObjView->_ChooseBGColorDlg->ShowWindow(ChooseBGColorWindow ? SW_SHOW
                                                             : SW_HIDE);
  ObjView->_ChooseSunColorDlg->ShowWindow(ChooseSunColorWindow ? SW_SHOW
                                                               : SW_HIDE);
  ObjView->_SkeletonScaleDlg->ShowWindow(SkeletonScaleWindow ? SW_SHOW
                                                             : SW_HIDE);
...
void CMainFrame::registerValue(bool read) {
  if (read) {
    ...
    HKEY hKey;
    if (RegOpenKeyEx(HKEY_CURRENT_USER, REGKEY_OBJ_VIEW_SCENE_DLG, 0, KEY_READ,
                     &hKey) == ERROR_SUCCESS) {
      DWORD len = sizeof(BOOL);
      DWORD type;
      NLMISC::CRGBA bgCol;
      RegQueryValueEx(hKey, _T("ViewAnimation"), 0, &type,
                      (LPBYTE)&AnimationWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewAnimationSet"), 0, &type,
                      (LPBYTE)&AnimationSetWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewSlots"), 0, &type,
                      (LPBYTE)&MixerSlotsWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewParticles"), 0, &type,
                      (LPBYTE)&ParticlesWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewDayNight"), 0, &type,
                      (LPBYTE)&DayNightWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewWaterPool"), 0, &type,
                      (LPBYTE)&WaterPoolWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewVegetable"), 0, &type,
                      (LPBYTE)&VegetableWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewGlobalWind"), 0, &type,
                      (LPBYTE)&GlobalWindWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewSoundAnimWind"), 0, &type,
                      (LPBYTE)&GlobalWindWindow, &len);
      len = sizeof(float);
      RegQueryValueEx(hKey, _T("MoveSpeed"), 0, &type, (LPBYTE)&MoveSpeed,
                      &len);
      len = sizeof(uint);
      RegQueryValueEx(hKey, _T("ObjectMode"), 0, &type, (LPBYTE)&MoveMode,
                      &len);
      len = sizeof(NLMISC::CRGBA);
      RegQueryValueEx(hKey, _T("BackGroundColor"), 0, &type, (LPBYTE)&BgColor,
                      &len);
      len = sizeof(float);
      RegQueryValueEx(hKey, _T("GlobalWindPower"), 0, &type,
                      (LPBYTE)&GlobalWindPower, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewChooseFrameDelay"), 0, &type,
                      (LPBYTE)&ChooseFrameDelayWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewChooseBGColor"), 0, &type,
                      (LPBYTE)&ChooseBGColorWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewChooseSunColor"), 0, &type,
                      (LPBYTE)&ChooseSunColorWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewSkeletonScaleWindow"), 0, &type,
                      (LPBYTE)&SkeletonScaleWindow, &len);
      len = sizeof(BOOL);
      RegQueryValueEx(hKey, _T("ViewTuneMRMWindow"), 0, &type,
                      (LPBYTE)&TuneMRMWindow, &len);
    }
  } else {
    HKEY hKey;
    if (RegCreateKey(HKEY_CURRENT_USER, REGKEY_OBJ_VIEW_SCENE_DLG, &hKey) ==
        ERROR_SUCCESS) {
      RegSetValueEx(hKey, _T("ViewAnimation"), 0, REG_BINARY,
                    (LPBYTE)&AnimationWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewAnimationSet"), 0, REG_BINARY,
                    (LPBYTE)&AnimationSetWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewSlots"), 0, REG_BINARY,
                    (LPBYTE)&MixerSlotsWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewParticles"), 0, REG_BINARY,
                    (LPBYTE)&ParticlesWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewDayNight"), 0, REG_BINARY,
                    (LPBYTE)&DayNightWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewWaterPool"), 0, REG_BINARY,
                    (LPBYTE)&WaterPoolWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewDayNight"), 0, REG_BINARY,
                    (LPBYTE)&DayNightWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewVegetable"), 0, REG_BINARY,
                    (LPBYTE)&VegetableWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewGlobalWind"), 0, REG_BINARY,
                    (LPBYTE)&GlobalWindWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewSoundAnimWind"), 0, REG_BINARY,
                    (LPBYTE)&SoundAnimWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewLightGroupWind"), 0, REG_BINARY,
                    (LPBYTE)&LightGroupWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewChooseFrameDelay"), 0, REG_BINARY,
                    (LPBYTE)&ChooseFrameDelayWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewChooseBGColor"), 0, REG_BINARY,
                    (LPBYTE)&ChooseBGColorWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewChooseSunColor"), 0, REG_BINARY,
                    (LPBYTE)&ChooseSunColorWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("MoveSpeed"), 0, REG_BINARY, (LPBYTE)&MoveSpeed,
                    sizeof(float));
      RegSetValueEx(hKey, _T("ObjectMode"), 0, REG_BINARY, (LPBYTE)&MoveMode,
                    sizeof(uint));
      RegSetValueEx(hKey, _T("BackGroundColor"), 0, REG_BINARY,
                    (LPBYTE)&BgColor, sizeof(NLMISC::CRGBA));
      RegSetValueEx(hKey, _T("GlobalWindPower"), 0, REG_BINARY,
                    (LPBYTE)&GlobalWindPower, sizeof(float));
      RegSetValueEx(hKey, _T("ViewSkeletonScaleWindow"), 0, REG_BINARY,
                    (LPBYTE)&SkeletonScaleWindow, sizeof(bool));
      RegSetValueEx(hKey, _T("ViewTuneMRMWindow"), 0, REG_BINARY,
                    (LPBYTE)&TuneMRMWindow, sizeof(bool));
    }
  }
}
...
void CMainFrame::OnResetCamera() {
  ...
  for (m = 0; m < ObjView->_ListInstance.size(); m++) {
    CTransformShape *pTransform = dynamic_cast<CTransformShape *>(
        ObjView->_ListInstance[m]->TransformShape);
    if (pTransform) {
      ...
      if (!pSkelModel) {
        ...
        radius =
            pTransform->getMatrix().mulVector(CVector(radius, 0, 0)).norm();
        found = true;
        m++;
        break;
      } else {
        ...
        if (pSkelModel->Bones.size()) {
          ...
          // Reset the radius
          radius = boundingBox.getRadius();
          radius = pSkelModel->Bones[0]
                       .getMatrix()
                       .mulVector(CVector(radius, 0, 0))
                       .norm();
          found = true;
          m++;
          break;
  ...
  for (; m < ObjView->_ListInstance.size(); m++) {
    ...
    if (pTransform) {
      ...
      if (!pSkelModel) {
        ...
        radius2 =
            pTransform->getMatrix().mulVector(CVector(radius2, 0, 0)).norm();
        ...
        if (pSkelModel->Bones.size()) {
          ...
          // Get the radius
          radius2 = boundingBox.getRadius();
          radius2 = pSkelModel->Bones[0]
                        .getMatrix()
                        .mulVector(CVector(radius2, 0, 0))
                        .norm();
  ...
  // Setup camera
  CNELU::Camera->lookAt(
      hotSpot + CVector(0.57735f, 0.57735f, 0.57735f) * radius, hotSpot);
...
void CMainFrame::OnEditX() {
  X ^= true;
  ObjView->getMouseListener().enableModelTranslationAxis(
      CEvent3dMouseListener::xAxis, X != 0);
}

void CMainFrame::OnEditY() {
  Y ^= true;
  ObjView->getMouseListener().enableModelTranslationAxis(
      CEvent3dMouseListener::yAxis, Y != 0);
}

void CMainFrame::OnEditZ() {
  Z ^= true;
  ObjView->getMouseListener().enableModelTranslationAxis(
      CEvent3dMouseListener::zAxis, Z != 0);
}

void CMainFrame::OnEnableElementXrotate() {
  ObjView->getMouseListener().setModelMatrixRotationAxis(
      CEvent3dMouseListener::xAxis);
}
...
void CMainFrame::OnFileExit() { DestroyWindow(); }
...
void CMainFrame::OnFileLoadconfig() {
  ...
  static TCHAR BASED_CODE szFilter[] =
      _T("NeL Object viewer config (*.ovcgf)|*.ovcgf|All Files (*.*)|*.*||");
  CFileDialog fileDlg(TRUE, _T(".ovcgf"), _T("*.ovcgf"),
  ...
  if (fileDlg.DoModal() == IDOK) {
    ...
    if (file.open(tStrToUtf8(fileDlg.GetPathName()))) {
      try {
        ...
        MessageBox(nlUtf8ToTStr(e.what()), _T("NeL object viewer"),
                   MB_OK | MB_ICONEXCLAMATION);
      }
    } else {
      // Create a message
      CString msg;
      msg.Format(_T("Can't open the file %s for reading."),
...
void CMainFrame::OnFileOpen() {
  ...
  if (GetOpenFileName(&openFile)) {
    ...
    if (!meshFilename.empty()) {
      ...
      static TCHAR BASED_CODE szFilter2[] =
          _T("NeL Skeleton Files (*.skel)|*.skel|All Files (*.*)|*.*||");
      CFileDialog fileDlg2(TRUE, _T(".skel"), _T("*.skel"),
                           OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter2);
      if (fileDlg2.DoModal() == IDOK) {
        // Load the shape with a skeleton
        if (ObjView->loadMesh(meshFilename,
                              tStrToUtf8(fileDlg2.GetPathName()))) {
          // Reset the camera
          OnResetCamera();
...
void CMainFrame::OnFileSaveconfig() {
  ...
  static TCHAR BASED_CODE szFilter[] =
      _T("NeL Object viewer config (*.ovcgf)|*.ovcgf|All Files (*.*)|*.*||");
  CFileDialog fileDlg(FALSE, _T(".ovcgf"), _T("*.ovcgf"),
                      OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
  if (fileDlg.DoModal() == IDOK) {
    ObjView->ParticleWorkspaceFilename.clear();
    CParticleWorkspace *pw =
        ObjView->getParticleDialog()->getParticleWorkspace();
    if (pw && pw->getNumNode() != 0) {
      if (localizedMessageBox(*this, IDS_INCLUDE_PARTICLE_WORKSPACE_INFOS,
                              IDS_OBJECT_VIEWER,
    ...
    if (file.open(tStrToUtf8(fileDlg.GetPathName()))) {
      ...
      // Create a message
      CString msg;
      msg.Format(_T("Can't open the file %s for writing"),
                 (LPCTSTR)fileDlg.GetPathName());
      MessageBox(msg, _T("NeL object viewer"), MB_OK | MB_ICONEXCLAMATION);
    }
  }
...
void CMainFrame::OnViewSetbackground() {
  static COLORREF colTab[16] = {0,        0xff0000, 0x00ff00, 0xffff00,
                                0x0000ff, 0xff00ff, 0x00ffff, 0xffffff,
                                0x7f7f7f, 0xff7f7f, 0x7fff7f, 0xffff7f,
                                0x7f7fff, 0xff7fff, 0x7fffff, 0xff7f00};
...
void CMainFrame::OnActivateFog() {
  ...
  CMenu *menu = GetMenu();
  menu->CheckMenuItem(IDM_ACTIVATE_FOG,
                      MF_BYCOMMAND | (FogActive ? MF_CHECKED : MF_UNCHECKED));
}
...
void CMainFrame::OnShowOcclusionTestMeshs() {
  ObjView->setOcclusionTestMeshsVisible(
      !ObjView->getOcclusionTestMeshsVisible());
}
...
void CMainFrame::OnUpdateEditX(CCmdUI *pCmdUI) { pCmdUI->SetCheck(X); }

void CMainFrame::OnUpdateEditY(CCmdUI *pCmdUI) { pCmdUI->SetCheck(Y); }

void CMainFrame::OnUpdateEditZ(CCmdUI *pCmdUI) { pCmdUI->SetCheck(Z); }
...
void CMainFrame::OnActivateTextureSet(UINT nID) {
  /// convert ID to index
  static const uint convIndex[] = {0, 3, 1, 2, 4, 5, 6, 7};

  ObjView->activateTextureSet(convIndex[nID - IDM_ACTIVATE_TEXTURE_SET_1]);
}

void CMainFrame::OnShuffleTextureSet() { ObjView->shuffleTextureSet(); }
...
void CMainFrame::OnEditMoveelement() {
  ...
  ObjView->getMouseListener().enableModelMatrixEdition(true);
  ObjView->getMouseListener().enableTranslateXYInWorld(false);
  ObjView->getMouseListener().setModelMatrix(
      ObjView->getParticleDialog()->getElementMatrix());
...
void CMainFrame::OnEditMoveFX() {
  ...
  ObjView->getMouseListener().enableTranslateXYInWorld(false);
  ObjView->getMouseListener().setModelMatrix(
      ObjView->getParticleDialog()->getPSWorldMatrix());
...
void CMainFrame::OnEditMoveObjectLightTest() {
  ...
  // Each move must be multiplied by inverese of scene root matrix.
  ObjView->getMouseListener().setModelMatrixTransformMove(
      ObjView->_SceneRoot->getMatrix().inverted());
}
...
void CMainFrame::OnShootScene() { ObjView->shootScene(); }
...
void CMainFrame::OnUpdateSceneCamera(CCmdUI *pCmdUI) {
  bool checked = ObjView->getCurrentCamera() ==
                 (sint)(pCmdUI->m_nID - ID_SCENE_CAMERA_FIRST);
  bool exist =
      (pCmdUI->m_nID - ID_SCENE_CAMERA_FIRST) < ObjView->getNumCamera();
  pCmdUI->SetCheck((checked && exist) ? TRUE : FALSE);
  pCmdUI->Enable(exist ? TRUE : FALSE);
  if (exist) {
    CInstanceInfo *instance = ObjView->getInstance(
        ObjView->getCameraInstance(pCmdUI->m_nID - ID_SCENE_CAMERA_FIRST));
    nlassert(instance->Camera);
    std::string text =
        NLMISC::toString("Camera %s", instance->Saved.ShapeFilename.c_str());

...
CWaterPoolEditor::CWaterPoolEditor(NL3D::CWaterPoolManager *wpm,
                                   CWnd *pParent /*= NULL*/)
...
BOOL CWaterPoolEditor::OnInitDialog() {
  ...
  ScreenToClient(&r);
  _ImpulsionStrenghtDlg =
      new CEditableRangeFloat("WAVE_IMPULSION_STRENGHT", NULL, 0.2f, 3.0f);
  ...
  ScreenToClient(&r);
  _WaveImpulsionRadiusDlg =
      new CEditableRangeUInt("WAVE_IMPULSION_RADIUS", NULL, 2, 5);
...
int CWaterPoolEditor::addPool(uint32 ID) {
  std::string poolId =
      NLMISC::toString("%d (%s)", ID, _Wpm->getPoolByID(ID).getName().c_str());
...
uint32 CWaterPoolEditor::getCurrentPoolID() {
  UpdateData();
  nlassert(m_PoolList.GetCurSel() !=
           LB_ERR); // must always have something selected
...
void CWaterPoolEditor::OnAutomaticWavesGeneration() {
  UpdateData();
  getCurrentPool().enableWaves(
      m_AutomaticWavesGeneration ? true : false /* VC++ warning */);
  updateWaveParams();
}
...
void CWaterPoolEditor::OnBordersOnly() {
  ...
  ;
  NL3D::CWaterHeightMap &whm = getCurrentPool();
  whm.setWaves(whm.getWaveIntensity(), whm.getWavePeriod(),
               whm.getWaveImpulsionRadius(),
               m_BordersOnly ? true : false /* VC++ Warning */);
}
...
void CWaterPoolEditor::OnLoadPool() {
  static TCHAR BASED_CODE szFilter[] =
      _T("NeL Water Pool Files (*.wpf)|*.wpf||");
  CFileDialog fileDlg(TRUE, _T(".wpf"), _T("*.wpf"),
    ...
    try {
      ...
      if (iF.open(NLMISC::tStrToUtf8(fileDlg.GetPathName()))) {
        if (iXml.init(iF)) {
          ...
          iF.close();
          MessageBox(nlUtf8ToTStr(NLMISC::toString(
                         "Unable to init xml stream from file: %s",
                         NLMISC::tStrToUtf8(fileDlg.GetPathName()).c_str())),
                     _T("NeL object viewer"), MB_OK | MB_ICONEXCLAMATION);
        }
      } else {
        MessageBox(nlUtf8ToTStr(NLMISC::toString(

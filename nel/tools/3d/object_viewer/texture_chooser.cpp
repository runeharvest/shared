...
CTextureChooser::CTextureChooser(NL3D::CPSMultiTexturedParticle *mtp /*= NULL*/,
    ...
    : _CurrBitmap(0), _Wrapper(NULL), _Texture(NULL),
      _EnableRemoveButton(false), _MTP(mtp), _Node(ownerNode),
      _MultiTexDlg(NULL)
...
BOOL CTextureChooser::EnableWindow(BOOL bEnable) {
  ...
  if (_MTP) {
    ...
    GetDlgItem(IDC_EDIT_MULTITEXTURING)
        ->EnableWindow((bEnable ? true : false /* VC WARNING */) &
                       _MTP->isMultiTextureEnabled());
  }
...
void CTextureChooser::textureToBitmap() {
  ...
  NL3D::CTextureBump *tb =
      dynamic_cast<NL3D::CTextureBump *>((NL3D::ITexture *)_Texture);
...
void CTextureChooser::OnBrowseTexture() {
  ...
  if (dynamic_cast<NL3D::CTextureFile *>(_Wrapper->get())) {
    texName =
        (static_cast<NL3D::CTextureFile *>(_Wrapper->get()))->getFileName();
  }
  ...
  if (fd.DoModal() == IDOK) {
    ...
    try {
      NL3D::CTextureFile *tf =
          new NL3D::CTextureFile(NLMISC::tStrToUtf8(fd.GetFileName()));
...
void CTextureChooser::updateMultiTexCtrl() {
  GetDlgItem(IDC_EDIT_MULTITEXTURING)
      ->EnableWindow(_MTP->isMultiTextureEnabled());
}
...
void CTextureChooser::OnEnableMultitexturing() {
  _MTP->enableMultiTexture(m_MultiTexCtrl.GetCheck() ? true
                                                     : false /* VC warning */);
  updateMultiTexCtrl();
}

...
static const uint IconIDs[] = {IDB_FORCE,
                               IDB_PARTICLE,
                               IDB_EMITTER,
                               IDB_LIGHT,
                               IDB_COLLISION_ZONE,
...
//****************************************************************************************************************
CParticleTreeCtrl::~CParticleTreeCtrl() { reset(); }
...
void CParticleTreeCtrl::reset() {
  ...
  for (std::vector<CNodeType *>::iterator it = _NodeTypes.begin();
       it != _NodeTypes.end(); ++it) {
    delete *it;
  }
  _NodeTypes.clear();
}
...
//****************************************************************************************************************
void CParticleTreeCtrl::rebuildLocatedInstance(
  ...
  while (currLocated) {
    ...
    for (uint32 k = 0; k < loc->getSize(); ++k) {
      ...
      InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
                 _T("instance"), PSIconLocatedInstance, PSIconLocatedInstance,
                 0, 0, (LPARAM)newNt, currLocated, TVI_LAST);
    }
...
//=====================================================================================================
void CParticleTreeCtrl::suppressLocatedInstanceNbItem(
    CParticleWorkspace::CNode &node, uint32 newSize) {
  HTREEITEM currPS = getTreeItem(&node);
  nlassert(currPS);
...
//=====================================================================================================
HTREEITEM
CParticleTreeCtrl::buildTreeFromPS(CParticleWorkspace::CNode &node,
                                   HTREEITEM rootHandle,
                                   HTREEITEM prevSibling /*= TVI_LAST*/) {
  ...
  if (node.isLoaded()) {
    ...
    HTREEITEM psRoot =
        InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT,
                   nlUtf8ToTStr(computeCaption(node)), PSIconParticleSystem,
                   PSIconParticleSystem, 0, 0, NULL, rootHandle, prevSibling);
    ...
    for (uint k = 0; k < node.getPSPointer()->getNbProcess(); k++) {
      CPSLocated *loc =
          dynamic_cast<CPSLocated *>(node.getPSPointer()->getProcess(k));
      if (loc)
        createNodeFromLocated(loc, psRoot);
    }
    rebuildLocatedInstance(node);
    return psRoot;
  } else {
    // bind a bitmap that say that the PS hasn't been loaded
    HTREEITEM psRoot = InsertItem(
        TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT,
        nlUtf8ToTStr(computeCaption(node)), PSIconParticleSystemNotLoaded,
...
void CParticleTreeCtrl::buildTreeFromWorkSpace(CParticleWorkspace &ws) {
  ...
  HTREEITEM rootHandle =
      InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_TEXT,
                 nlUtf8ToTStr(computeCaption(ws)), PSIconWorkspace,
                 PSIconWorkspace, 0, 0, NULL, NULL, TVI_LAST);
  // set the param (doesn't seems to work during first creation)
...
//=====================================================================================================
void CParticleTreeCtrl::createNodeFromLocated(NL3D::CPSLocated *loc,
                                              HTREEITEM rootHandle) {
  // insert an item for the located
  CNodeType *nt = new CNodeType(loc);
  _NodeTypes.push_back(nt);
  // bind located icon
  HTREEITEM nodeHandle =
      InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
                 nlUtf8ToTStr(loc->getName()), PSIconLocated, PSIconLocated, 0,
                 0, (LPARAM)nt, rootHandle, TVI_LAST);
...
//=====================================================================================================
void CParticleTreeCtrl::createNodeFromLocatedBindable(
    NL3D::CPSLocatedBindable *lb, HTREEITEM rootHandle) {
  // we ordered the image so that they match the type for a located bindable
  // (force,  particles,  collision zones...)
  CNodeType *nt = new CNodeType(lb);
  _NodeTypes.push_back(nt);
  InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
             nlUtf8ToTStr(lb->getName()), lb->getType(), lb->getType(),
             PSIconForce, PSIconForce, (LPARAM)nt, rootHandle, TVI_LAST);
}
...
void CParticleTreeCtrl::updateRightPane(CNodeType &nt) {
  ...
  case CNodeType::located: {
    ...
    CLocatedProperties *lp =
        new CLocatedProperties(getOwnerNode(&nt), nt.Loc, _ParticleDlg);
    lp->init(0, 0);
  ...
  case CNodeType::locatedBindable: {
    if (dynamic_cast<NL3D::CPSEmitter *>(nt.Bind)) {
      CEmitterDlg *ed = new CEmitterDlg(
          getOwnerNode(&nt), static_cast<NL3D::CPSEmitter *>(nt.Bind),
          _ParticleDlg);
      ed->init(_ParticleDlg);
      _ParticleDlg->setRightPane(ed);
    } else if (dynamic_cast<NL3D::CPSTargetLocatedBindable *>(nt.Bind)) {
      CLocatedTargetDlg *ltd = new CLocatedTargetDlg(
          getOwnerNode(&nt),
          static_cast<NL3D::CPSTargetLocatedBindable *>(nt.Bind), _ParticleDlg);
      ltd->init(_ParticleDlg);
      _ParticleDlg->setRightPane(ltd);
    } else if (dynamic_cast<NL3D::CPSSound *>(nt.Bind)) {
      nlassert(getOwnerNode(&nt));
      CEditPSSound *epss = new CEditPSSound(
          getOwnerNode(&nt), static_cast<NL3D::CPSSound *>(nt.Bind));
      epss->init(_ParticleDlg);
      _ParticleDlg->setRightPane(epss);
    } else if (dynamic_cast<NL3D::CPSLight *>(nt.Bind)) {
      nlassert(getOwnerNode(&nt));
      CEditPSLight *epsl = new CEditPSLight(
          getOwnerNode(&nt), static_cast<NL3D::CPSLight *>(nt.Bind));
      epsl->init(_ParticleDlg);
      _ParticleDlg->setRightPane(epsl);
    } else {
      nlassert(getOwnerNode(&nt));
      CLocatedBindableDialog *lbd =
          new CLocatedBindableDialog(getOwnerNode(&nt), nt.Bind);
  ...
  case CNodeType::locatedInstance: {
    ...
    _LastClickedPS = ps;
    CPSMoverDlg *moverDlg =
        new CPSMoverDlg(getOwnerNode(&nt), this,
                        &_ParticleDlg->MainFrame->ObjView->getMouseListener(),
                        nt.Loc, nt.LocatedInstanceIndex);
...
void CParticleTreeCtrl::OnRButtonDown(UINT nFlags, CPoint point) {
  ...
  if (item) {
    ...
    if (node) {
      if (!node->isStateMemorized() &&
          _ParticleDlg->StartStopDlg->getState() !=
              CStartStopParticleSystem::RunningMultiple) {
    ...
    case CNodeType::locatedBindable:
      ...
      menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD1N2,
                                        MF_UNCHECKED | MF_BYCOMMAND);
      menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD1,
                                        MF_UNCHECKED | MF_BYCOMMAND);
      menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD2,
                                        MF_UNCHECKED | MF_BYCOMMAND);
      // check the menu to tell which lod is used for this located bindable
      if (nt->Bind->getLOD() == NL3D::PSLod1n2)
        menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD1N2,
                                          MF_CHECKED | MF_BYCOMMAND);
      if (nt->Bind->getLOD() == NL3D::PSLod1)
        menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD1,
                                          MF_CHECKED | MF_BYCOMMAND);
      if (nt->Bind->getLOD() == NL3D::PSLod2)
        menu.GetSubMenu(0)->CheckMenuItem(IDM_LB_LOD2,
                                          MF_CHECKED | MF_BYCOMMAND);
    ...
    nlassert(subMenu);
    subMenu->TrackPopupMenu(TPM_LEFTALIGN | TPM_RIGHTBUTTON, r.left + point.x,
                            r.top + point.y, this);
  }
}
...
void CParticleTreeCtrl::deleteSelection() {
  ...
  case CNodeType::located: {
    ...
    // if the system is running,  we must destroy initial infos about the
    // located, as they won't need to be restored when the stop button will be
    // pressed
  ...
  case CNodeType::locatedBindable: {
    ...
    // that what saved about the located bindable,  when the start button was
    // pressed,  as they won't need to be restored
...
//****************************************************************************************************************
BOOL CParticleTreeCtrl::OnCmdMsg(UINT nID, int nCode, void *pExtra,
                                 AFX_CMDHANDLERINFO *pHandlerInfo) {
  if (nCode != 0)
    return CTreeCtrl::OnCmdMsg(nID, nCode, pExtra, pHandlerInfo);
  CPSLocatedBindable *toCreate = NULL;
  CNodeType *nt = (CNodeType *)GetItemData(GetSelectedItem());
  bool createLocAndBindable = false; // when set to true,  must create a located
                                     // and a simultaneously a located bindable
  switch (nID) {
    ///////////////
  // workspace //
  ///////////////
  ...
  case IDM_PS_SORT_BY_FILENAME: {
    ...
    struct CSortByFilename : public CParticleWorkspace::ISort {
      bool less(const CParticleWorkspace::CNode &lhs,
                const CParticleWorkspace::CNode &rhs) const {
  ...
  case IDM_FACE:
    ...
    break;
  case IDM_SHOCKWAVE_LOC:
    createLocAndBindable = true;
  case IDM_SHOCKWAVE:
    toCreate = new NL3D::CPSShockWave;
    break;
  case IDM_RIBBON_LOOK_AT_LOC:
    createLocAndBindable = true;
  ...
  case ID_INSTANCIATE_LOCATED: {
    ...
    sint32 objIndex =
        nt->Loc->newElement(NLMISC::CVector::Null, NLMISC::CVector::Null, NULL,
                            0, nt->Loc->getMatrixMode(), 0.f);
    ...
    HTREEITEM root =
        InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
                   _T("instance"), PSIconLocatedInstance, PSIconLocatedInstance,
                   0, 0, (LPARAM)nt, GetSelectedItem(), TVI_LAST);
  ...
  case IDM_COPY_LOCATED:
    ...
    nlassert(nt->Loc);
    _LocatedCopy.reset(
        NLMISC::safe_cast<NL3D::CPSLocated *>(::DupPSLocated(nt->Loc)));
    break;
  ...
  case IDM_PASTE_LOCATED: {
    ...
    _ParticleDlg->StartStopDlg->resetAutoCount(getOwnerNode(nt));
    CPSLocated *copy =
        dynamic_cast<CPSLocated *>(::DupPSLocated(_LocatedCopy.get()));
    if (!copy)
      break;
  ...
  case IDM_PASTE_BINDABLE: {
    ...
    getOwnerNode(nt)->setModified(true);
    _ParticleDlg->StartStopDlg->resetAutoCount(getOwnerNode(nt));
    CPSLocatedBindable *copy =
        ::DupPSLocatedBindable(_LocatedBindableCopy.get());
    if (!copy)
      break;
    if (nt->Loc->bind(copy)) {
  ...
  case IDM_MERGE_PS: {
    ...
    static TCHAR BASED_CODE szFilter[] =
        _T("ps & shapes files(*.ps;*.shape)|*.ps; *.shape||");
    ...
    if (fd.DoModal() == IDOK) {
      ...
      NLMISC::CPath::addSearchPath(
          NLMISC::CFile::getPath(NLMISC::tStrToUtf8(fd.GetPathName())));
      ...
      try {
        ...
        std::string shapeName =
            NLMISC::CFile::getFilename(NLMISC::tStrToUtf8(fd.GetPathName()));
        ...
        if (!trs) {
          localizedMessageBox(*this, IDS_COULDNT_INSTANCIATE_PS, IDS_ERROR,
        ...
        MessageBox(nlUtf8ToTStr(e.what()), getStrRsc(IDS_ERROR),
                   MB_OK | MB_ICONEXCLAMATION);
        return TRUE;
      }
      ...
      bool mergeOK = nt->PS->getPSPointer()->merge(
          NLMISC::safe_cast<NL3D::CParticleSystemShape *>(
              (NL3D::IShape *)psm->Shape));
      NL3D::CShapeBank *oldSB = CNELU::Scene->getShapeBank();
      CNELU::Scene->setShapeBank(sb.get());
      CNELU::Scene->deleteInstance(psm);
      CNELU::Scene->setShapeBank(oldSB);
      if (!mergeOK) {
        localizedMessageBox(*this, IDS_PS_NO_FINITE_DURATION, IDS_ERROR,
                            MB_ICONEXCLAMATION);
        return TRUE;
      }
      if (wasActiveNode) {
  ...
  case ID_MENU_SAVE_PS:
    ...
    break;
  case IDM_SAVE_PS_AS: {
    if (nt->PS->getResetAutoCountFlag() &&
        nt->PS->getPSPointer()->getAutoCountFlag()) {
      MessageBox(nt->PS->getFilename().c_str() +
                     getStrRsc(IDS_AUTO_COUNT_ERROR),
                 getStrRsc(IDS_WARNING), MB_ICONEXCLAMATION);
    } else {
      _ParticleDlg->StartStopDlg->stop();
      std::string fileName = nt->PS->getFilename();
      static TCHAR BASED_CODE szFilter[] =
          _T("ps & shapes files(*.ps;*.shape)|*.ps; *.shape||");
      CFileDialog fd(FALSE, _T(".ps"), nlUtf8ToTStr(fileName),
                     OFN_OVERWRITEPROMPT, szFilter, this);
      if (fd.DoModal() == IDOK) {
        _ParticleDlg->savePSAs(*this, *nt->PS,
                               NLMISC::tStrToUtf8(fd.GetPathName()), false);
      }
    }
  } break;
  case IDM_CLEAR_PS_CONTENT: {
    if (localizedMessageBox(*this, IDS_CLEAR_CONTENT, IDS_PARTICLE_EDITOR,
                            MB_YESNO) == IDYES) {
      CParticleWorkspace::CNode *ownerNode = getOwnerNode(nt);
      nlassert(ownerNode);
      ownerNode->setResetAutoCountFlag(false);
  ...
  if (toCreate) {
    ...
    if (createLocAndBindable) {
      ...
      std::pair<CParticleTreeCtrl::CNodeType *, HTREEITEM> p =
          createLocated(nt->PS->getPSPointer(), GetSelectedItem());
      nt = p.first;
      son = 0;
      father = p.second;
      ...
      if (getOwnerNode(nt)->getPSPointer()->getBypassMaxNumIntegrationSteps()) {
        if (toCreate->getType() == NL3D::PSParticle ||
            toCreate->getType() == NL3D::PSEmitter) {
    ...
    if (!nt->Loc->bind(toCreate)) {
      MessageBox(
          _T("The system is flagged with 'No max Nb steps',  or uses the ")
          _T("preset 'Spell FX'. System must have finite duration. Can't add ")
          _T("object. To solve this,  set a limited life time for the father."),
    ...
    if (!createLocAndBindable) {
      if (!son) {
        ...
        while (son != NULL) {
          if (((CNodeType *)GetItemData(son))->Type ==
              CNodeType::locatedInstance) {
            break;
          }
          lastSon = son;
    ...
    InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
               nlUtf8ToTStr(toCreate->getName()), toCreate->getType(),
               toCreate->getType(), 0, 0, (LPARAM)newNt, father, lastSon);
    touchPSState(nt);
    Invalidate();
    _ParticleDlg->StartStopDlg->resetAutoCount(getOwnerNode(nt));
  }
...
//****************************************************************************************************************
std::pair<CParticleTreeCtrl::CNodeType *, HTREEITEM>
CParticleTreeCtrl::createLocated(NL3D::CParticleSystem *ps,
                                 HTREEITEM headItem) {
  std::string name;
  char num[128];
  ...
  _NodeTypes.push_back(newNt);
  // insert item in tree
  HTREEITEM insertedItem =
      InsertItem(TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_PARAM | TVIF_TEXT,
                 nlUtf8ToTStr(name), PSIconLocated, PSIconLocated, 0, 0,
                 (LPARAM)newNt, headItem, TVI_LAST);
  touchPSState(newNt);
  ...
  *pResult = 0;
  if (info->item.pszText) {
    ...
    case CNodeType::workspace: {
      ...
      workspaceModifiedFlagChanged(
          *nt->WS); // change name (this may be called twice because of the
                    // modification callback, but this doesn't matter)
    } break;
    case CNodeType::particleSystem: {
      if (!nt->PS->isLoaded()) {
        localizedMessageBox(*this, IDS_CANT_CHANGE_PS_NAME, IDS_ERROR,
                            MB_OK | MB_ICONEXCLAMATION);
      } else {
        nt->PS->getPSPointer()->setName(NLMISC::tStrToUtf8(info->item.pszText));
        nt->PS->setModified(true);
      }
      this->SetItemText(info->item.hItem,
                        nlUtf8ToTStr(computeCaption(*nt->PS)));
    } break;
    case CNodeType::located: {
      nlassert(getOwnerNode(nt));
      getOwnerNode(nt)->setModified(true);
...
void CParticleTreeCtrl::moveElement(const NLMISC::CMatrix &m) {
  ...
  if (m.getPos() == mat.getPos() && m.getI() == mat.getI() &&
      m.getJ() == mat.getJ() && m.getK() == mat.getK())
    return;
...
CParticleSystem *CParticleTreeCtrl::CNodeType::getOwnerPS() const {
  ...
  case located:
    return Loc->getOwner();
  case particleSystem:
    return PS->getPSPointer();
  case locatedBindable:
    return Bind->getOwner()->getOwner();
  case locatedInstance:
    return Loc->getOwner();
  case workspace:
    return NULL;
  default:
...
CParticleTreeCtrl::getOwnerNode(CNodeType *nt) const {
  if (!nt)
    return NULL;
...
void CParticleTreeCtrl::updateCaption(CParticleWorkspace::CNode &node) {
  ...
  if (!item)
    return;
  // update name of ps to dipslay a star in front of it (this tells that the ps
  // has been modified)
  SetItemText(item, nlUtf8ToTStr(computeCaption(node)));
}

//****************************************************************************************************************
std::string CParticleTreeCtrl::computeCaption(const std::string &path,
...
std::string CParticleTreeCtrl::computeCaption(CParticleWorkspace::CNode &node) {
  ...
  if (node.isLoaded()) {
    baseCaption =
        computeCaption(node.getRelativePath(), node.getPSPointer()->getName(),
                       node.isModified());
  } else {
...
std::string CParticleTreeCtrl::computeCaption(CParticleWorkspace &workspace) {
  return computeCaption(workspace.getFilename(), workspace.getName(),
...
void CParticleTreeCtrl::insertNewPS(CParticleWorkspace &pws) {
  static const TCHAR BASED_CODE szFilter[] =
      _T("NeL Particle systems (*.ps)|*.ps||");
  CFileDialog fd(TRUE, _T(".ps"), _T("*.ps"),
                 OFN_ALLOWMULTISELECT | OFN_FILEMUSTEXIST, szFilter, this);
  ...
  if (fd.DoModal() == IDOK) {
    CParticleWorkspace::IModificationCallback *oldCallback =
        pws.getModificationCallback();
    ...
    while (pos) {
      ...
      if (!node) {
        if (diplayNodeAlreadyInserted) {
          if (pos) {
            CSkippableMessageBox smb(path + getStrRsc(IDS_PS_ALREADY_INSERTED),
                                     getStrRsc(IDS_ERROR), this);
            smb.DoModal();
            diplayNodeAlreadyInserted = !smb.getBypassFlag();
          } else {
            MessageBox(CString(nlUtf8ToTStr(NLMISC::CFile::getFilename(
                           NLMISC::tStrToUtf8(path)))) +
                           getStrRsc(IDS_PS_ALREADY_INSERTED),
                       getStrRsc(IDS_ERROR), MB_OK | MB_ICONEXCLAMATION);
          }
        }
        continue;
      }
      if (diplayLoadingError) {
        diplayLoadingError =
            _ParticleDlg->loadPS(*this, *node,
                                 pos ? CParticleDlg::ReportErrorSkippable
                                     : CParticleDlg::ReportError);
      } else {
...
void CParticleTreeCtrl::createNewPS(CParticleWorkspace &pws) {
  CCreateFileDlg cfd(getStrRsc(IDS_CREATE_NEW_PS),
                     NLMISC::CPath::standardizeDosPath(pws.getPath()), "ps",
                     this);
  if (cfd.DoModal() == IDOK) {
    if (pws.containsFile(cfd.getFileName())) {
      MessageBox(
          (LPCTSTR)(CString(
                        NLMISC::CFile::getFilename(cfd.getFileName()).c_str()) +
                    getStrRsc(IDS_PS_ALREADY_INSERTED)),
          getStrRsc(IDS_ERROR), MB_ICONEXCLAMATION);
    }
    if (cfd.touchFile()) {
      CParticleWorkspace::CNode *node = pws.addNode(cfd.getFullPath());
      nlassert(node); // should always succeed because we tested if file already
                      // exists
      node->createEmptyPS();
...
HTREEITEM CParticleTreeCtrl::getTreeItem(CNodeType *nt) const {
  ...
  while (!leftToTraverse.empty()) {
    ...
    if (GetChildItem(curr))
      leftToTraverse.push(GetChildItem(curr));
    if (GetNextSiblingItem(curr))
      leftToTraverse.push(GetNextSiblingItem(curr));
  }
  return NULL;
}
...
//****************************************************************************************************************
HTREEITEM
CParticleTreeCtrl::getTreeItem(CParticleWorkspace::CNode *node) const {
  if (!GetRootItem())
    return NULL;
  std::stack<HTREEITEM> leftToTraverse;
  leftToTraverse.push(GetRootItem());
...
//****************************************************************************************************************
void CParticleTreeCtrl::nodeModifiedFlagChanged(
    CParticleWorkspace::CNode &node) {
  updateCaption(node);
}
...
void CParticleTreeCtrl::setActiveNode(CParticleWorkspace::CNode *node) {
  ...
  if (_LastActiveNode)
    SetItemState(_LastActiveNode, 0, TVIS_BOLD);
  if (newItem) {
...
//****************************************************************************************************************
void CParticleTreeCtrl::sortWorkspace(CParticleWorkspace &ws,
...
void CParticleTreeCtrl::removeAllPS(CParticleWorkspace &ws) {
  if (localizedMessageBox(*this, IDS_REMOVE_ALL_PS, IDS_PARTICLE_SYSTEM_EDITOR,

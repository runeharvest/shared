...
CVegetableNoiseValueDlg::CVegetableNoiseValueDlg(
    const std::string &noiseValueName)
...
void CVegetableNoiseValueDlg::setDefaultRangeAbs(float defRangeMin,
                                                 float defRangeMax) {
  _DefAbsRangeMin = defRangeMin;
  _DefAbsRangeMax = defRangeMax;
}
void CVegetableNoiseValueDlg::setDefaultRangeRand(float defRangeMin,
                                                  float defRangeMax) {
  _DefRandRangeMin = defRangeMin;
  _DefRandRangeMax = defRangeMax;
}
void CVegetableNoiseValueDlg::setDefaultRangeFreq(float defRangeMin,
                                                  float defRangeMax) {
...
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SLIDER_VEGETABLE_SCALE_NOISE,
          OnReleasedcaptureSliderVegetableScaleNoise)
ON_WM_VSCROLL()
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CVegetableNoiseValueDlg::OnInitDialog() {
  ...
  _AbsValue = new CDirectEditableRangeFloat(
      _TitleName + "_VEGET_" + std::string("AbsValue"), _DefAbsRangeMin,
      _DefAbsRangeMax, "AbsValue:");
  _AbsValue->init(x, y, this);
  y += dy;

  // Init RandValue Slider.
  _RandValue = new CDirectEditableRangeFloat(
      _TitleName + "_VEGET_" + std::string("RandValue"), _DefRandRangeMin,
      _DefRandRangeMax, "RandValue:");
  _RandValue->init(x, y, this);
  y += dy;

  // Init Frequency Slider.
  _Frequency = new CDirectEditableRangeFloat(
      _TitleName + "_VEGET_" + std::string("Frequency"), _DefFreqRangeMin,
      _DefFreqRangeMax, "Frequency:");
...
void CVegetableNoiseValueDlg::setNoiseValue(NLMISC::CNoiseValue *nv,
                                            IVegetableRefresh *vegetRefresh) {
  _NoiseValue = nv;
  _VegetableRefresh = vegetRefresh;

  // Set pointer to edit value.
  if (_AbsValue)
    _AbsValue->setFloat(&nv->Abs, vegetRefresh);
  if (_RandValue)
    _RandValue->setFloat(&nv->Rand, vegetRefresh);
  if (_Frequency)
    _Frequency->setFloat(&nv->Frequency, vegetRefresh);
}

void CVegetableNoiseValueDlg::OnReleasedcaptureSliderVegetableScaleNoise(
...
void CVegetableNoiseValueDlg::OnVScroll(UINT nSBCode, UINT nPos,
                                        CScrollBar *pScrollBar) {
  CSliderCtrl *sliderCtrl = (CSliderCtrl *)pScrollBar;
  if (sliderCtrl == &SliderNoiseValue && nSBCode == SB_THUMBPOSITION ||
...
void CVegetableNoiseValueDlg::applyScaleSlider(sint scrollValue) {
  ...
  float scale = (NL_VEGETABLE_EDIT_SLIDER_NVS_SIZE / 2 - scrollValue) /
                (float)(NL_VEGETABLE_EDIT_SLIDER_NVS_SIZE / 2);
  float factor;
  ...
  // Apply to noiseValue. NB: don't do a IPSWrapper::set(), because this cause a
  // refreshVegetableDisplay And we don't want it during sliding (too slow).
  ...
  // update marker text
  StaticScaleMarker.SetWindowText(
      nlUtf8ToTStr(NLMISC::toString("%d%%", (sint)(factor * 100))));
}

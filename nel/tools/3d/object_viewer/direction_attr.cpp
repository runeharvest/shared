...
CDirectionAttr::CDirectionAttr(const std::string &id)
    : _DirectionDlg(NULL), _Wrapper(NULL), _DirectionWrapper(NULL) {
...
BOOL CDirectionAttr::EnableWindow(BOOL bEnable) {
  ...
  if (_DirectionWrapper && _DirectionWrapper->supportGlobalVectorValue() &&
      !_DirectionWrapper->getGlobalVectorValueName().empty()) {
...
void CDirectionAttr::OnGlobalDirection() {
  ...
  CChooseName chooseName(
      nlUtf8ToTStr(_DirectionWrapper->getGlobalVectorValueName()));

  if (chooseName.DoModal() == IDOK) {
    _DirectionWrapper->enableGlobalVectorValue(chooseName.getName());
    if (!chooseName.getName().empty()) {
      NL3D::CParticleSystem::setGlobalVectorValue(
          chooseName.getName(),
          NLMISC::CVector::I); // take a non NULL value for the direction
    }
  }
  EnableWindow(TRUE);
}

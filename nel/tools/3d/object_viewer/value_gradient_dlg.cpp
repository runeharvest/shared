...
CValueGradientDlg::CValueGradientDlg(IValueGradientDlgClient *clientInterface,
    ...
    : CDialog(CValueGradientDlg::IDD, pParent),
      _ClientInterface(clientInterface), _Node(ownerNode),
      _DestroyClientInterface(destroyClientInterface), _EditValueDlg(NULL),
      _CanTuneNbStages(canTuneNbStages), _MinSize(minSize), _PN(pn) {
  //{{AFX_DATA_INIT(CValueGradientDlg)
  //}}AFX_DATA_INIT
  _NbStepDlg = new CEditableRangeUInt(std::string("GRADIENT NB STEP"),
                                      ownerNode, 1, 255);
}

CValueGradientDlg::~CValueGradientDlg() {
  if (_DestroyClientInterface)
...
void CValueGradientDlg::OnInsertValue() {
  ...
  ++_Size;
  _ClientInterface->modifyGradient(IValueGradientDlgClient::Insert,
                                   m_GradientList.GetCurSel());
...
void CValueGradientDlg::OnRemoveValue() {
  ...
  uint oldIndex = m_GradientList.GetCurSel();
  --_Size;
  _ClientInterface->modifyGradient(IValueGradientDlgClient::Delete,
                                   m_GradientList.GetCurSel());
...
void CValueGradientDlg::OnValueDown() {
  ...
  if (currIndex == (uint)(m_GradientList.GetCount() - 1))
    return;
...
void CValueGradientDlg::OnSelchangeGradientList() {
  ...
  delete _EditValueDlg;
  _EditValueDlg =
      _ClientInterface->createDialog(m_GradientList.GetCurSel(), this, _Node);

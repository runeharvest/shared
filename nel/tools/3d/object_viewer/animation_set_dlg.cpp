...
CAnimationSetDlg::CAnimationSetDlg(CObjectViewer *objView,
                                   CWnd *pParent /*=NULL*/)
...
void CAnimationSetDlg::OnAddSkelWt() {
  ...
  if (instance != CB_ERR) {
    ...
    static TCHAR BASED_CODE szFilter[] =
        ...
        _T("(*.*)|*.*||");
    CFileDialog fileDlg(TRUE, _T(".swt"), _T("*.swt"),
...
void CAnimationSetDlg::refresh(BOOL update) {
  if (update) {
    ...
    for (i = 0; i < _ObjView->getNumInstance(); i++) {
      std::string name = NLMISC::CFile::getFilenameWithoutExtension(
          _ObjView->getInstance(i)->Saved.ShapeFilename);
    ...
    if (instance != 0xffffffff) {
      ...
      for (i = 0; i < object->Saved.AnimationFileName.size(); i++) {
        ...
        std::string name = NLMISC::CFile::getFilenameWithoutExtension(
            object->Saved.AnimationFileName[i]);

        // Get the animation pointer
        CAnimation *anim = object->AnimationSet.getAnimation(
            object->AnimationSet.getAnimationIdByName(name));
        ...
        while (ite != setString.end()) {
          ...
          if (keyTrack) {
            ...
            keyTrack->getKeysInRange(track->getBeginTime() - 1,
                                     track->getEndTime() + 1, keys);

            // Print track info
            name = toString("%s (%f - %f) %u keys", typeid(*track).name(),
                            track->getBeginTime(), track->getEndTime(),
                            (uint32)keys.size());
          } else {
            // Print track info
            name = toString("%s (%f - %f)", typeid(*track).name(),
                            track->getBeginTime(), track->getEndTime());
          }
      ...
      for (i = 0; i < object->Saved.SWTFileName.size(); i++) {
        // Get the animation name
        std::string name = NLMISC::CFile::getFilenameWithoutExtension(
            object->Saved.SWTFileName[i]);

        // Get the animation pointer
        CSkeletonWeight *swt = object->AnimationSet.getSkeletonWeight(
            object->AnimationSet.getSkeletonWeightIdByName(name));
        ...
        for (uint n = 0; n < numNode; n++) {
          std::string percent =
              toString("%s (%f%%)", swt->getNodeName(n).c_str(),
                       swt->getNodeWeight(n) * 100);
      ...
      for (i = 0; i < object->Saved.PlayList.size(); i++) {
        // Insert an intem
        int item =
            PlayList.InsertString(-1, nlUtf8ToTStr(object->Saved.PlayList[i]));
...
void CAnimationSetDlg::OnSetAnimLength() {
  ...
  // For each instance
  for (uint instance = 0; instance < _ObjView->_ListInstance.size();
       instance++) {
    // Ref on the instance
    ...
    if (inst.Saved.PlayList.size()) {
      ...
      for (uint i = 0; i < (uint)inst.Saved.PlayList.size(); i++) {
        // Get the animation
        CAnimation *anim = inst.AnimationSet.getAnimation(
            inst.AnimationSet.getAnimationIdByName(inst.Saved.PlayList[i]));

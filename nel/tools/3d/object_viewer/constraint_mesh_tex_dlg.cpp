...
CConstraintMeshTexDlg::CConstraintMeshTexDlg(NL3D::CPSConstraintMesh *cm,
                                             CWnd *pParent /*=NULL*/)
...
CConstraintMeshTexDlg::~CConstraintMeshTexDlg() { RemoveDlg(_CurrDlg); }
...
ON_BN_CLICKED(IDC_REINIT_WHEN_NEW_ELEMENT_IS_CREATED,
              OnReinitWhenNewElementIsCreated)
//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CConstraintMeshTexDlg::setupDlg() {
  if (_CM->getTexAnimType() == NL3D::CPSConstraintMesh::GlobalAnim) {
    ...
    ((CButton *)GetDlgItem(IDC_REINIT_WHEN_NEW_ELEMENT_IS_CREATED))
        ->SetCheck(_CM->isGlobalAnimTimeResetOnNewElementForced() ? 1 : 0);
  } else {
...
void CConstraintMeshTexDlg::OnSelchangeCurrentStage() {
  UpdateData();
  RemoveDlg(_CurrDlg);
  createGlobalAnimationDlg(
      (uint)((CComboBox *)GetDlgItem(IDC_CURRENT_STAGE))->GetCurSel());
}
...
void CConstraintMeshTexDlg::OnReinitWhenNewElementIsCreated() {
  _CM->forceGlobalAnimTimeResetOnNewElement(
      ((CButton *)GetDlgItem(IDC_REINIT_WHEN_NEW_ELEMENT_IS_CREATED))
              ->GetCheck()
          ? true
          : false);

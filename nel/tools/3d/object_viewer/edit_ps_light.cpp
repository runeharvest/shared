...
//***************************************************************************************
CEditPSLight::CEditPSLight(CParticleWorkspace::CNode *ownerNode,
                           NL3D::CPSLight *light)
    : _Node(ownerNode), _ColorDlg(NULL), _AttenStartDlg(NULL),
      _AttenEndDlg(NULL) {
...
void CEditPSLight::init(CWnd *pParent /*=NULL*/) {
  ...
  bmh = LoadBitmap(::AfxGetInstanceHandle(),
                   MAKEINTRESOURCE(IDB_LIGHT_ATTEN_START));
  ...
  ScreenToClient(&r);
  bmh = LoadBitmap(::AfxGetInstanceHandle(),
                   MAKEINTRESOURCE(IDB_LIGHT_ATTEN_END));

...
CEmitterDlg::CEmitterDlg(CParticleWorkspace::CNode *ownerNode,
                         NL3D::CPSEmitter *emitter, CParticleDlg *particleDlg)
    : _Node(ownerNode), _Emitter(emitter), _PeriodDlg(NULL), _GenNbDlg(NULL),
      _StrenghtModulateDlg(NULL), _SpeedInheritanceFactorDlg(NULL),
...
void CEmitterDlg::OnSelchangeEmittedType() {
  ...
  if (!_Emitter->setEmittedType(_LocatedList[k])) {
    if (_Emitter->getOwner()->getOwner()->getBehaviourType() ==
            NL3D::CParticleSystem::SpellFX ||
        _Emitter->getOwner()->getOwner()->getBypassMaxNumIntegrationSteps()) {
      MessageBox(_T("Can't perform operation : the system is flagged with 'No ")
                 _T("max nb steps' or uses the preset 'Spell FX', and thus, ")
                 _T("should have a finite duration. This operation create a ")
                 _T("loop in the system, and so is forbidden."),
                 _T("Error"), MB_ICONEXCLAMATION);
    } else {
      MessageBox(_T("Loops with emitters are forbidden."), _T("Error"),
                 MB_ICONEXCLAMATION);
    }
    initEmittedType();
  }
...
void CEmitterDlg::OnSelchangeTypeOfEmission() {
  UpdateData();
  if (!_Emitter->setEmissionType(
          (NL3D::CPSEmitter::TEmissionType)m_EmissionTypeCtrl.GetCurSel())) {
    CString mess;
...
BOOL CEmitterDlg::OnInitDialog() {
  ...
  ScreenToClient(&r);
  _SpeedInheritanceFactorDlg =
      new CEditableRangeFloat("SPEED_INHERITANCE_FACTOR", _Node, -1.f, 1.f);
  ...
  ScreenToClient(&r);
  _DelayedEmissionDlg =
      new CEditableRangeFloat("DELAYED_EMISSION", _Node, 0.f, 10.f);
  ...
  _MaxEmissionCountDlg =
      new CEditableRangeUInt("MAX_EMISSION_COUNT", _Node, 0, 100);
  ...
  HBITMAP bmh = LoadBitmap(::AfxGetInstanceHandle(),
                           MAKEINTRESOURCE(IDB_EMISSION_PERIOD));
  _PeriodDlg->init(bmh, posX, posY, this);
  posY += 120;

  // setup the dialog that helps tuning the number of particle being emitted at
  // a time
  ...
  bmh = LoadBitmap(::AfxGetInstanceHandle(),
                   MAKEINTRESOURCE(IDB_EMISSION_QUANTITY));
  ...
  if (dynamic_cast<NL3D::CPSEmitterConic *>(_Emitter)) {
    CEditableRangeFloat *ecr = new CEditableRangeFloat(
        std::string("CONIC EMITTER RADIUS"), _Node, 0.1f, 2.1f);
    pushWnd(ecr);
    _ConicEmitterRadiusWrapper.E =
        dynamic_cast<NL3D::CPSEmitterConic *>(_Emitter);
    ecr->setWrapper(&_ConicEmitterRadiusWrapper);
    ecr->init(posX + 80, posY, this);

    CStatic *s = new CStatic;
    pushWnd(s);
    s->Create(_T("Radius :"), SS_LEFT,
              CRect(posX, posY + 10, posX + 70, posY + 32), this);
  ...
  } else if (_Emitter->getUserMatrixModeForEmissionDirection() ==
             NL3D::PSFXWorldMatrix) {
    m_DirectionModeCtrl.SetCurSel((int)LocalToSystem);
  } else if (_Emitter->getUserMatrixModeForEmissionDirection() ==
             NL3D::PSIdentityMatrix) {
...
void CEmitterDlg::OnConsistentEmission() {
  UpdateData();
  _Emitter->enableConsistenEmission(
      m_ConsistentEmission != 0 ? true : false /* VC6 warning */);
  updateModifiedFlag();
  UpdateData(TRUE);
}

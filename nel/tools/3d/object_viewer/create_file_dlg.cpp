...
CCreateFileDlg::CCreateFileDlg(const CString &title,
...
void CCreateFileDlg::OnOK() {
  ...
  if (_Filename.empty()) {
    localizedMessageBox(*this, IDS_FILENAME_PATH, IDS_ERROR,
                        MB_ICONEXCLAMATION);
    return;
  }
  // check that filename is valid (should be a single file name)
  if (_Filename != NLMISC::CFile::getFilename(_Filename)) {
    localizedMessageBox(*this, IDS_INVALID_FILENAME, IDS_ERROR,
                        MB_ICONEXCLAMATION);
    return;
  }
  ...
  if (!NLMISC::CFile::isExists(_Path)) {
    ...
    if (!result) {
      MessageBox(
          (LPCTSTR)(getStrRsc(IDS_COULDNT_CREATE_DIRECTORY) + _Path.c_str()),
  ...
  if (NLMISC::nlstricmp(NLMISC::CFile::getExtension(_Filename), _Extension) !=
      0) {
...
bool CCreateFileDlg::touchFile() {
  ...
  if (NLMISC::CFile::isExists(fullPath)) {
    int result = MessageBox(
        (LPCTSTR)(CString(filename.c_str()) + getStrRsc(IDS_OVERWRITE_FILE)),
  ...
  if (!testFile.open(fullPath)) {
    localizedMessageBox(*this, IDS_CANNOT_CREATE_FILE, IDS_ERROR,
                        MB_ICONEXCLAMATION);
    return false;
  }
  testFile.close();

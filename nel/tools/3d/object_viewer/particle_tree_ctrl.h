...
#if !defined(                                                                  \
    AFX_PARTICLE_TREE_CTRL_H__9A55D046_3E95_49CC_99AC_8A4F268EDD33__INCLUDED_)
...
class CParticleTreeCtrl : public CTreeCtrl,
                          public CParticleWorkspace::IModificationCallback {
...
public:
   ...
   */
  HTREEITEM buildTreeFromPS(CParticleWorkspace::CNode &node,
  ...
  void createNodeFromLocatedBindable(NL3D::CPSLocatedBindable *lb,
                                     HTREEITEM rootHandle);
  ...
  /// suppress located instance item, so that they don't have higher index than
  /// the new size
  void suppressLocatedInstanceNbItem(CParticleWorkspace::CNode &node,
                                     uint32 newSize);
  //
  void init(void);
  // move the current element by using the given matrix
  void moveElement(const NLMISC::CMatrix &mat);
  // get the matrix of the current element being selected, or identity if
  // there's none
...
public:
  ...
  struct CNodeType {
    ...
    CNodeType(CParticleWorkspace *ws) {
      nlassert(ws);
      WS = ws;
      Type = workspace;
    }
    // build node for a located
    CNodeType(NL3D::CPSLocated *loc) {
      nlassert(loc);
      Loc = loc;
      Type = located;
    }
    ...
    CNodeType(NL3D::CPSLocatedBindable *lb) {
      Bind = lb;
      Type = locatedBindable;
    }
...
private:
  // instanciate a located in the given system , and return its nodetype and
  // htreeitem
  std::pair<CNodeType *, HTREEITEM> createLocated(NL3D::CParticleSystem *ps,
                                                  HTREEITEM headItem);
  ...
  std::string computeCaption(const std::string &path,
                             const std::string &userName, bool modified);

  // Allow user to insert multiple PS in the workspace (prompt a file dialog to
  // chose them)
  ...
  // remove part of the tree and the associated CNodeType objects. IMPORTANT :
  // DOES NOT update the matching elements in the ps that is being edited.
...
public:
  // Update currently active node. Its node in the tree is displayed with bold
  // characters.

...
CSchemeManager::~CSchemeManager() {
  for (TSchemeMap::iterator it = _SchemeMap.begin(); it != _SchemeMap.end();
       ++it) {
    delete it->second.second;
  }
}

void CSchemeManager::insertScheme(const std::string &name,
...
void CSchemeManager::getSchemes(const std::string &type,
                                std::vector<TSchemeInfo> &dest) {
  TSchemeMap::const_iterator lbd = _SchemeMap.lower_bound(type),
                             ubd = _SchemeMap.upper_bound(type);
  dest.clear();
...
void CSchemeManager::serial(NLMISC::IStream &f) {
  ...
  if (!f.isReading()) {
    ...
    f.serial(size);
    for (TSchemeMap::iterator smIt = _SchemeMap.begin();
         smIt != _SchemeMap.end(); ++smIt) {
...
void CSchemeManager::rename(NL3D::CPSAttribMakerBase *am,

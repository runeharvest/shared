...
CEditFollowPath::CEditFollowPath(NL3D::CPSPlaneBasisFollowSpeed *pbfs,
...
BOOL CEditFollowPath::OnInitDialog() {
  ...
  ((CComboBox *)GetDlgItem(IDC_PROJECTION_MODE))
      ->SetCurSel((int)_FollowPath->getProjectionPlane());
...
void CEditFollowPath::OnSelchangeProjectionMode() {
  ...
  _FollowPath->setProjectionPlane(
      (NL3D::CPSPlaneBasisFollowSpeed::TProjectionPlane)index);
  if (_Node)

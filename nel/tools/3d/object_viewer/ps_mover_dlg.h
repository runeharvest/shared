...
#if !defined(                                                                  \
    AFX_PS_MOVER_DLG_H__C1C4348E_3384_4557_B99E_CBE4E5492C0C__INCLUDED_)
...
class CPSMoverDlg : public CDialog {
  // Construction
public:
  // construct the object with a pointer to the item being edited, and to the
  // mouse listener to update its model matrix
  CPSMoverDlg(CParticleWorkspace::CNode *ownerNode, CWnd *parent,
              NL3D::CEvent3dMouseListener *ml, NL3D::CPSLocated *editedLocated,
...
public:
  ...
  // get the current moving interface, or NULL, if the selected object has no
  // IPSMover interface
...
protected:
  ...
  // update the mouse listener position when the user entered a value with the
  // keyboard

...
CurveEdit::CurveEdit(NL3D::CPSFloatCurveFunctor *curve,
                     CParticleWorkspace::CNode *ownerNode, IPopupNotify *pn,
                     CWnd *pParent /*=NULL*/)
    : CDialog(CurveEdit::IDD, pParent), _Node(ownerNode), _PN(pn), Curve(curve),
      _State(Create), _X(10), _Y(10), _Width(350), _Height(200),
      _SelectedCtrlPoint(-1), _NumSamplesDlg(NULL)
...
void CurveEdit::OnMouseMove(UINT nFlags, CPoint point) {
  ...
  case Moving: {
    ...
    _SelectedCtrlPoint = intersectCtrlPoint(
        point.x,
        point.y); // the index of the point we are moving may have changed
    if (_SelectedCtrlPoint == -1)
      _State = Move;
    Invalidate();
  } break;
  }
...
void CurveEdit::OnLButtonDown(UINT nFlags, CPoint point) {
  ...
  case Remove: {
    if (_SelectedCtrlPoint == -1 || Curve->getNumCtrlPoints() == 2)
      return;
...
sint CurveEdit::intersectCtrlPoint(sint x, sint y) {
  ...
  for (uint k = 0; k < numPoints; ++k) {
    ...
    if (x >= (sint)(p.x - CtrlPointSize) && x <= (sint)(p.x + CtrlPointSize) &&
        y >= (sint)(p.y - CtrlPointSize) && y <= (sint)(p.y + CtrlPointSize)) {
      return k;
    }
  }
  return -1;
...
CurveEdit::CCtrlPoint CurveEdit::coordsFromScreen(sint x, sint y) const {
  ...
  CCtrlPoint pos(date, _Origin + (_Y + _Height - y) / (_Scale * _Height));
  return pos;
}
...
void CurveEdit::drawCtrlPoints(CDC &dc) {
  ...
  uint numPoints = Curve->getNumCtrlPoints();
  for (uint k = 0; k < numPoints; ++k) {
    CGdiObject *oldPen = dc.SelectObject(
        &pens[k == (uint)_SelectedCtrlPoint
                  ? 1
                  : 0]); // slect the red pen if thi ctrl point is selected
    const CCtrlPoint &cp = Curve->getControlPoint(k);
    POINT p = makePoint(cp.Date, cp.Value);
...
BOOL CurveEdit::OnInitDialog() {
  ...
  _NumSamplesDlg = new CEditableRangeUInt(std::string("FLOAT_CURVE_NB_SAMPLE"),
                                          _Node, 1, 512);
...
void CurveEdit::OnDisplayInterpolation() { Invalidate(); }
...
void CurveEdit::OnDestroy() { CDialog::OnDestroy(); }

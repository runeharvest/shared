...
CAutoLODDlg::CAutoLODDlg(CParticleWorkspace::CNode *ownerNode,
                         NL3D::CParticleSystem *ps, IPopupNotify *pn,
                         CWnd *pParent /* = NULL*/)
    : _Node(ownerNode), _PS(ps), _PN(pn), CDialog(CAutoLODDlg::IDD, pParent) {
  nlassert(ps);
...
BOOL CAutoLODDlg::OnInitDialog() {
  ...
  CEditableRangeFloat *erf =
      new CEditableRangeFloat("AUTO_LOD_DIST_RATIO", _Node, 0.f, 0.99f);
  ...
  // For non-shared systems only : Set the LOD bias at the max distance, so that
  // some particles are still displayed
  ...
  ((CComboBox *)GetDlgItem(IDC_DEGRADATION_EXPONENT))
      ->SetCurSel(std::min(4, (sint)_PS->getAutoLODDegradationExponent()) - 1);
  ((CButton *)GetDlgItem(IDC_SKIP_PARTICLES))
      ->SetCheck(_PS->getAutoLODMode() ? 1 : 0);
...
void CAutoLODDlg::OnSelchangeDegradationExponent() {
  _PS->setupAutoLOD(
      _PS->getAutoLODStartDistPercent(),
      ((CComboBox *)GetDlgItem(IDC_DEGRADATION_EXPONENT))->GetCurSel() + 1);
}

void CAutoLODDlg::OnSkipParticles() {
  _PS->setAutoLODMode(((CButton *)GetDlgItem(IDC_SKIP_PARTICLES))->GetCheck() !=
                      0);
}

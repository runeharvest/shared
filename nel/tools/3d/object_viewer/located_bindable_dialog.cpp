...
CLocatedBindableDialog::CLocatedBindableDialog(
    CParticleWorkspace::CNode *ownerNode, NL3D::CPSLocatedBindable *bindable)
...
void CLocatedBindableDialog::init(CParticleDlg *pParent) {
  ...
  if (ps->isSharingEnabled()) {
    ...
    if (ps->isAutoLODEnabled() == false) {
      ...
      ((CButton *)GetDlgItem(IDC_NO_AUTO_LOD))
          ->SetCheck(NLMISC::safe_cast<NL3D::CPSParticle *>(_Bindable)
                         ->isAutoLODDisabled());
    }
  } else {
  ...
  // control at the top of the sheet are not available for meshs & constraint
  // meshes, so use that extra space
  if (dynamic_cast<NL3D::CPSMesh *>(_Bindable) ||
      dynamic_cast<NL3D::CPSConstraintMesh *>(_Bindable)) {
    yPos = 0;
  }
  ...
  if (dynamic_cast<NL3D::CPSMaterial *>(_Bindable)) {
    ...
    // z-test
    ((CButton *)GetDlgItem(IDC_ZTEST))
        ->SetCheck(material->isZTestEnabled() ? BST_CHECKED : BST_UNCHECKED);
    // z-bias
    GetDlgItem(IDC_ZBIAS)->SetWindowText(
        nlUtf8ToTStr(NLMISC::toString("%.2f", -material->getZBias())));
  } else {
  ...
  if (dynamic_cast<NL3D::CPSParticle *>(_Bindable)) {
    ...
    if (p->supportGlobalColorLighting()) {
      ...
      // if global color lighting is forced for all objects, don't allow to
      // modify
      GetDlgItem(ID_GLOBAL_COLOR_LIGHTING)
          ->EnableWindow(ps->getForceGlobalColorLightingFlag() ? FALSE : TRUE);
      ((CButton *)GetDlgItem(ID_GLOBAL_COLOR_LIGHTING))
          ->SetCheck(p->usesGlobalColorLighting() ? 1 : 0);
    } else {
    ...
    if (dynamic_cast<NL3D::CPSColoredParticle *>(_Bindable)) {
      ...
      HBITMAP bmh = LoadBitmap(::AfxGetInstanceHandle(),
                               MAKEINTRESOURCE(IDB_PARTICLE_COLOR));
    ...
    if (dynamic_cast<NL3D::CPSRotated2DParticle *>(_Bindable)) {
      ...
      CAttribDlgFloat *ad =
          new CAttribDlgFloat("PARTICLE_ANGLE2D", _Node, 0.f, 256.f);
      pushWnd(ad);
      ...
      HBITMAP bmh = LoadBitmap(::AfxGetInstanceHandle(),
                               MAKEINTRESOURCE(IDB_PARTICLE_ANGLE));
      ad->init(bmh, xPos, yPos, this);
      ad->GetClientRect(&rect);
      yPos += rect.bottom + 3;
    }
    ...
    if (dynamic_cast<NL3D::CPSRotated3DPlaneParticle *>(_Bindable)) {
      ...
      pushWnd(pb);
      _PlaneBasisWrapper.S =
          dynamic_cast<NL3D::CPSRotated3DPlaneParticle *>(_Bindable);
    ...
    if (dynamic_cast<NL3D::CPSHintParticleRotateTheSame *>(_Bindable)) {
      CPrecomputedRotationsDlg *pr = new CPrecomputedRotationsDlg(
          _Node, dynamic_cast<NL3D::CPSHintParticleRotateTheSame *>(_Bindable),
          pb);
      pushWnd(pr);
    ...
    if (dynamic_cast<NL3D::CPSFaceLookAt *>(_Bindable)) {
      ...
      CEditableRangeFloat *mbc = new CEditableRangeFloat(
          std::string("MOTION_BLUR_COEFF"), _Node, 0, 5);
      pushWnd(mbc);
      ...
      s->Create(_T("Fake motion blur coeff."), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
      mbc->GetClientRect(&rect);
      yPos += rect.bottom + 3;
      mbc = new CEditableRangeFloat(std::string("MOTION_BLUR_THRESHOLD"), _Node,
                                    0, 5);
      pushWnd(mbc);
      _MotionBlurWnd.push_back(mbc);
      ...
      pushWnd(s);
      _MotionBlurWnd.push_back(s);
      s->Create(_T("Fake motion blur threshold."), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
      mbc->GetClientRect(&rect);
      ...
      ((CButton *)GetDlgItem(IDC_ALIGN_ON_MOTION))
          ->SetCheck(fla->getAlignOnMotion());
      ((CButton *)GetDlgItem(IDC_ZALIGN))->SetCheck(fla->getAlignOnZAxis());
      updateValidWndForAlignOnMotion(fla->getAlignOnMotion());
    }

    // if we're dealing with a shockwave, we add dlg for the radius cut, and the
    // number of segments
    if (dynamic_cast<NL3D::CPSShockWave *>(_Bindable)) {
      NL3D::CPSShockWave *sw = static_cast<NL3D::CPSShockWave *>(_Bindable);
      CEditableRangeFloat *rc =
          new CEditableRangeFloat(std::string("RADIUS CUT"), _Node, 0, 1);
      pushWnd(rc);
      ...
      CStatic *s = new CStatic;
      pushWnd(s);
      s->Create(_T("Radius cut."), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
      ...
      CEditableRangeUInt *snbs = new CEditableRangeUInt(
          std::string("SHOCK WAVE NB SEG"), _Node, 3, 24);
      pushWnd(snbs);
      ...
      CEditableRangeFloat *uvd =
          new CEditableRangeFloat(std::string("TEX UFACTOR"), _Node, 0, 5);
      pushWnd(uvd);
      ...
      s = new CStatic;
      pushWnd(s);
      s->Create(_T("Texture U factor :"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->ShowWindow(SW_SHOW);
    ...
    if (dynamic_cast<NL3D::CPSFanLight *>(_Bindable)) {
      CEditableRangeUInt *nbf =
          new CEditableRangeUInt(std::string("NB_FANS"), _Node, 3, 127);
      pushWnd(nbf);
      ...
      CStatic *s = new CStatic;
      pushWnd(s);
      s->Create(_T("Nb fan lights :"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
      ...
      nbf =
          new CEditableRangeUInt(std::string("PHASE_SMOOTHNESS"), _Node, 0, 31);
      pushWnd(nbf);
      nbf->enableUpperBound(32, true);
      _FanLightSmoothnessWrapper.P =
          static_cast<NL3D::CPSFanLight *>(_Bindable);
      nbf->setWrapper(&_FanLightSmoothnessWrapper);
      nbf->init(xPos + 140, yPos, this);
      s = new CStatic;
      pushWnd(s);
      s->Create(_T("Phase smoothnes:"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->ShowWindow(SW_SHOW);
      ...
      CEditableRangeFloat *nbfp = new CEditableRangeFloat(
          std::string("FAN_LIGHT_PHASE"), _Node, 0, 4.f);
      pushWnd(nbfp);
      ...
      s = new CStatic;
      pushWnd(s);
      s->Create(_T("Fan light speed :"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->ShowWindow(SW_SHOW);
      ...
      nbfp = new CEditableRangeFloat(std::string("FAN_LIGHT_INTENSITY"), _Node,
                                     0, 4.f);
      pushWnd(nbfp);
      ...
      s = new CStatic;
      pushWnd(s);
      s->Create(_T("Fan light intensity:"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->ShowWindow(SW_SHOW);
    ...
    if (dynamic_cast<NL3D::CPSTailParticle *>(_Bindable)) {
      ...
      if (!dynamic_cast<NL3D::CPSRibbonLookAt *>(_Bindable)) {
        nbs =
            new CEditableRangeUInt(std::string("TAIL_NB_SEGS_"), _Node, 2, 16);
        nbs->enableLowerBound(1, true);
      } else {
        nbs = new CEditableRangeUInt(std::string("LOOKAT_RIBBON_TAIL_NB_SEGS_"),
                                     _Node, 2, 16);
        nbs->enableLowerBound(1, true);
      }
      ...
      CTailParticleDlg *tpd = new CTailParticleDlg(
          _Node, dynamic_cast<NL3D::CPSTailParticle *>(_Bindable));
      pushWnd(tpd);
    ...
    if (dynamic_cast<NL3D::CPSShapeParticle *>(_Bindable)) {
      CMeshDlg *md =
          new CMeshDlg(_Node, dynamic_cast<NL3D::CPSShapeParticle *>(_Bindable),
                       _ParticleDlg);
    ...
    if (dynamic_cast<NL3D::CPSConstraintMesh *>(_Bindable)) {
      CConstraintMeshDlg *cmd = new CConstraintMeshDlg(
          static_cast<NL3D::CPSConstraintMesh *>(_Bindable));
      cmd->init(xPos, yPos, this);
      cmd->GetClientRect(&rect);
      yPos += rect.bottom + 3;
      CConstraintMeshTexDlg *cmtd = new CConstraintMeshTexDlg(
          static_cast<NL3D::CPSConstraintMesh *>(_Bindable), this);
    ...
    if (dynamic_cast<NL3D::CPSTexturedParticle *>(_Bindable)) {
      CTextureAnimDlg *td = new CTextureAnimDlg(
          _Node, dynamic_cast<NL3D::CPSTexturedParticle *>(_Bindable),
    ...
    if (dynamic_cast<NL3D::CPSTexturedParticleNoAnim *>(_Bindable)) {
      NL3D::CPSTexturedParticleNoAnim *tp =
          dynamic_cast<NL3D::CPSTexturedParticleNoAnim *>(_Bindable);
      _TextureNoAnimWrapper.TP = tp;
      CTextureChooser *tc = new CTextureChooser(
          dynamic_cast<NL3D::CPSMultiTexturedParticle *>(_Bindable), _Node);
    ...
    if (dynamic_cast<NL3D::CPSRibbon *>(_Bindable)) {
      ...
      CEditableRangeFloat *uvd =
          new CEditableRangeFloat(std::string("RIBBON UFACTOR"), _Node, 0, 5);
      pushWnd(uvd);
      ...
      s = new CStatic;
      pushWnd(s);
      s->Create(_T("Texture V factor :"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
      uvd->GetClientRect(&rect);
    ...
    if (dynamic_cast<NL3D::CPSRibbonBase *>(_Bindable)) {
      ...
      // add dialog for uv tuning with ribbon
      CEditableRangeFloat *sd = new CEditableRangeFloat(
          std::string("SEGMENT DURATION"), _Node, 0.05f, 0.5f);
      ...
      CStatic *s = new CStatic;
      pushWnd(s);
      s->Create(_T("Seg Duration :"), SS_LEFT,
                CRect(xPos, yPos + 16, xPos + 139, yPos + 48), this);
      s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
      s->ShowWindow(SW_SHOW);
    ...
    if (dynamic_cast<NL3D::CPSRibbonBase *>(_Bindable)) {
      CRibbonDlg *rd = new CRibbonDlg(
          _Node, static_cast<NL3D::CPSRibbonBase *>(_Bindable), this);
...
void CLocatedBindableDialog::OnSelchangeBlendingMode() {
  ...
  nlassert(m);
  m->setBlendingMode(
      (NL3D::CPSMaterial::TBlendingMode)m_BlendingMode.GetCurSel());
  touchPSState();
  updateModifiedFlag();
}
...
uint CLocatedBindableDialog::updateSizeControl() {
  ...
  if (fla && fla->hasIndependantSizes()) {
    ...
    _SizeCtrl = new CAttribDlgFloat(
        editWidth ? "PARTICLE_WIDTH" : "PARTICLE_HEIGHT", _Node, 0.f, 1.f);
    ...
    bmh = LoadBitmap(
        ...
        MAKEINTRESOURCE(editWidth ? IDB_PARTICLE_WIDTH : IDB_PARTICLE_HEIGHT));
  } else // general case. Wrap to the size interface and the appropriate dialog
  {
    ...
    bmh = LoadBitmap(::AfxGetInstanceHandle(),
                     MAKEINTRESOURCE(IDB_PARTICLE_SIZE));
  }
  RECT rect;
...
//***********************************************************************************
void CLocatedBindableDialog::OnSizeWidth() { updateSizeControl(); }

//***********************************************************************************
void CLocatedBindableDialog::OnSizeHeight() { updateSizeControl(); }
...
void CLocatedBindableDialog::OnGlobalColorLighting() {
  NL3D::CPSParticle *p = NLMISC::safe_cast<NL3D::CPSParticle *>(_Bindable);
  p->enableGlobalColorLighting(
      ((CButton *)GetDlgItem(ID_GLOBAL_COLOR_LIGHTING))->GetCheck() == 1);
  touchPSState();
  updateModifiedFlag();
}
...
void CLocatedBindableDialog::OnAlignOnMotion() {
  ...
  NL3D::CPSFaceLookAt *fla =
      NLMISC::safe_cast<NL3D::CPSFaceLookAt *>(_Bindable);
...
void CLocatedBindableDialog::OnZtest() {
  ...
  nlassert(mat);
  mat->enableZTest(((CButton *)GetDlgItem(IDC_ZTEST))->GetCheck() !=
                   BST_UNCHECKED);
  updateModifiedFlag();
}
...
void CLocatedBindableDialog::OnChangeZbias() {
  UpdateData();
  // Trick to track "Enter" keypress: CEdit are multiline. If GetLineCount()>1,
  // then user has press enter.
...
//***************************************************************************************************************************
void CLocatedBindableDialog::OnKillfocusZbias() { updateZBias(); }

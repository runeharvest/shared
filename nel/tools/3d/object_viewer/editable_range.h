...
#if !defined(                                                                  \
    AFX_EDITABLE_RANGE_H__0B1EFF2B_FA0E_4AC8_88B8_416605043BF9__INCLUDED_)
...
public:
  /**
   * Construct the dialog by giving it an Id. It will be used to save the user
   * preference. Each dialog of this type must have its own id in the app.
   */
  CEditableRange(const std::string &id,
                 CParticleWorkspace::CNode *node); // standard constructor
  // Dialog Data
...
public:
  /// for derivers : this must querries the value to the desired manager and set
  /// the dialog values
  virtual void updateRange(void) = 0;
  /** for derivers : value update from a linked object (reader). See examples in
   * subclasses
   */
  virtual void updateValueFromReader(void) = 0;
  // empty the values and the slider
  void emptyDialog(void);
  // validate the lower an upper bound of a range from their string
  // representation
  virtual bool editableRangeValueValidator(const CString &lo,
                                           const CString &up) = 0;
  // update the dialog display
  void update();
  // Implementation
protected:
  /** for derivers : value update : this is call with a float ranging from 0.f
   * to 1.f (from the slider) And it must convert it to the desired value,
   * changing the value of this dialog
   */
  ...
  // the range tune button was pressed. It muist show a dialog that allows the
  // user to choose the range he wants
...
/** here, we define a template for editable ranges EditableRanges, that help to
 * instanciate it Derivers have just to specialize 2 static methods :
 * value2CString and string2value
 */
...
public:
  ...
  CEditableRangeT(const std::string &id, CParticleWorkspace::CNode *node,
                  T defaultMin, T defaultMax)
      : CEditableRange(id, node), _Range(defaultMin, defaultMax),
        _Wrapper(NULL) {}

  /** set an interface of a wrapper  to read / write values in the particle
   * system NB : The 'OwnerNode' field of the wrapper if set to the value given
   * when to 'node' when this object was constructed
   */
...
public:
  ...
  // SPECIALIZE THAT. convert a CString into a value, return NULL if ok, or a
...
protected:
  void updateValueFromText(void) {
    ...
    if (!message) {
      const TCHAR *mess = validateUpperBound(value),
  ...
  void setValue(T value) {
    ...
    if (value < _Range.first) {
      ...
      if (_Range.second != _Range.first) {
        m_SliderPos = (uint)((double)(value - _Range.first) /
                                 (_Range.second - _Range.first) *
                                 (m_SliderCtrl.GetRangeMax() -
  ...
  virtual bool editableRangeValueValidator(const CString &lo,
    ...
    const TCHAR *mess = validateUpperBound(loT),
    ...
    if (upT <= loT) {
      ::MessageBox(NULL,
                   _T("upper bound must be strictly greater than lower bound"),
...
CEditableRangeT<float>::CEditableRangeT(const std::string &id,
...
inline const TCHAR *CEditableRangeT<float>::string2value(const CString &value,
                                                         float &result) {
...
CEditableRangeT<uint32>::CEditableRangeT(const std::string &id,
                                         CParticleWorkspace::CNode *node,
                                         uint32 defaultMin, uint32 defaultMax)
    : CEditableRange(id, node), _Range(defaultMin, defaultMax), _Wrapper(NULL) {
}

inline void CEditableRangeT<uint32>::value2CString(uint32 value,
                                                   CString &dest) {
  dest.Format(_T("%d"), value);
}

inline const TCHAR *CEditableRangeT<uint32>::string2value(const CString &value,
                                                          uint32 &result) {
  sint32 tmp;
...
CEditableRangeT<sint32>::CEditableRangeT(const std::string &id,
                                         CParticleWorkspace::CNode *node,
                                         sint32 defaultMin, sint32 defaultMax)
    : CEditableRange(id, node), _Range(defaultMin, defaultMax), _Wrapper(NULL) {
}

inline void CEditableRangeT<sint32>::value2CString(sint32 value,
                                                   CString &dest) {
  dest.Format(_T("%d"), value);
}

inline const TCHAR *CEditableRangeT<sint32>::string2value(const CString &value,
                                                          sint32 &result) {
  sint32 tmp;
  if (NLMISC::fromString(NLMISC::tStrToUtf8(value), tmp)) {
    result = tmp;

...
/////////////////////////////////////////////////////////////////////
static float NbCyclesReader(void *lParam) {
  return ((CAttribDlg *)lParam)->getSchemeNbCycles();
}
static void NbCyclesWriter(float value, void *lParam) {
  ((CAttribDlg *)lParam)->setSchemeNbCycles(value);
}
...
public:
  std::string Id; // the Id of each of the dialog (it will be followed by %1 or
                  // %2) must be filled by the user
...
protected:
  ...
  virtual CEditAttribDlg *newDialog(const std::string &id,
                                    IPSWrapper<T> *wrapper) = 0;
...
/** This template generate an interface that is used with the gradient edition
 * dialog T is the type to be edited (color, floet, etc..)
 */

template <typename T>
class CValueGradientDlgClientT : public IValueGradientDlgClient,
                                 public IPSWrapper<T> {
public:
  std::string Id; // the Id of each of the dialog (it will be followed by %1 or
                  // %2) must be filled by the user
  ...
  /** must provide a dialog for the edition of one value (only ONE exist at a
   * time) \param index the index of the value in the dialog \grad the dlg that
   * called this method (deriver can ask a redraw then)
   */
  ...
  virtual void modifyGradient(TAction action, uint index) {
    ...
    T *tab =
        new T[Scheme->getNumValues() + 1]; // +1 is for the add / insert case
    Scheme->getValues(tab);

    switch (action) {
    case IValueGradientDlgClient::Add:
      tab[Scheme->getNumValues()] = DefaultValue;
      Scheme->setValues(tab, Scheme->getNumValues() + 1,
                        Scheme->getNumStages());
      break;
    case IValueGradientDlgClient::Insert:
      ::memmove(tab + (index + 1), tab + index,
                sizeof(T) * (Scheme->getNumValues() - index));
      tab[index] = DefaultValue;
      Scheme->setValues(tab, Scheme->getNumValues() + 1,
                        Scheme->getNumStages());
      break;
    case IValueGradientDlgClient::Delete:
      ::memmove(tab + index, tab + index + 1,
                sizeof(T) * (Scheme->getNumValues() - index - 1));
      Scheme->setValues(tab, Scheme->getNumValues() - 1,
                        Scheme->getNumStages());
      break;
  ...
  uint32 getNbSteps(void) const { return Scheme->getNumStages(); }

  // set the number of interpolation steps
  void setNbSteps(uint32 value) { Scheme->setNumStages(value); }
...
//*************************************************************************************************************
CAttribDlg::CAttribDlg(const std::string &valueID,
                       CParticleWorkspace::CNode *ownerNode,
                       bool enableConstantValue /* = true*/)
    : _CstValueDlg(NULL), _Node(ownerNode), _FirstDrawing(true),
      _EnableConstantValue(enableConstantValue), _DisableMemoryScheme(false),
      _SchemeEditionDlg(NULL), _NbCycleEnabled(true), _NbCyclesDlg(NULL),
      _ValueID(valueID), _SrcInputEnabled(true) {
...
//*************************************************************************************************************
void CAttribDlg::closeEditWindow() { childPopupClosed(NULL); }
...
void CAttribDlg::init(HBITMAP bitmap, sint x, sint y, CWnd *pParent) {
  ...
  if (_NbCycleEnabled) {
    _NbCyclesDlg = new CEditableRangeFloat(_ValueID + "%%NB_CYCLE_INFO", _Node,
                                           0.1f, 10.1f);
...
void CAttribDlg::inputValueUpdate(void) {
  if (useScheme() &&
      getSchemeInput().InputType == NL3D::CPSInputType::attrUserParam) {
...
public:
  CEditAttribDlg *newDialog(const std::string &id, IPSWrapperFloat *wrapper) {
    CEditableRangeFloat *erf =
        new CEditableRangeFloat(id, wrapper->OwnerNode, MinRange, MaxRange);
...
//*************************************************************************************************************
CAttribDlgFloat::CAttribDlgFloat(const std::string &valueID,
                                 CParticleWorkspace::CNode *node,
                                 float minRange, float maxRange)
    : CAttribDlgT<float>(valueID, node), _MinRange(minRange),
      _MaxRange(maxRange) {
  _CstValueId = valueID;
}
...
CEditAttribDlg *CAttribDlgFloat::createConstantValueDlg() {
  CEditableRangeFloat *erf =
      new CEditableRangeFloat(_CstValueId, _Node, _MinRange, _MaxRange);
...
std::string CAttribDlgFloat::getSchemeName(uint index) const {
  const char *types[] = {"value blender", "values gradient", "curve",
                         "value computed from emitter", "binary operator"};
  nlassert(index < 5);
...
CWnd *CAttribDlgFloat::editScheme(void) {
  ...
  if (dynamic_cast<NL3D::CPSFloatBlender *>(scheme)) {
    ...
    myInterface->SchemeFunc =
        &((NL3D::CPSValueBlenderSample<float, 64> *)scheme)->_F;
    CValueBlenderDlg *vb =
        new CValueBlenderDlg(myInterface, true, this, this, _Node);
    vb->init(this);
    return vb;
  }
  if (dynamic_cast<NL3D::CPSFloatGradient *>(scheme)) {
    ...
    wrapper->Scheme =
        &(((NL3D::CPSFloatGradient *)(_SchemeWrapper->getScheme()))->_F);
    CValueGradientDlg *gd =
        new CValueGradientDlg(wrapper, _Node, true, this, this);
    wrapper->GradDlg = gd;
  ...
  if (dynamic_cast<NL3D::CPSFloatMemory *>(scheme)) {
    CAttribDlgFloat *adf =
        new CAttribDlgFloat(_CstValueId, _Node, _MinRange, _MaxRange);
    this->duplicateBoundChecker(*adf);
    CValueFromEmitterDlgT<float> *vfe = new CValueFromEmitterDlgT<float>(
        (NL3D::CPSFloatMemory *)(scheme), adf, this, m_AttrBitmap.GetBitmap());
    vfe->init(this);
    return vfe;
  }
  if (dynamic_cast<NL3D::CPSFloatBinOp *>(scheme)) {
    ...
    CBinOpDlgT<float> *bod = new CBinOpDlgT<float>(
        (NL3D::CPSFloatBinOp *)(scheme), (CAttribDlgT<float> **)ad, this,
  ...
  if (dynamic_cast<NL3D::CPSFloatCurve *>(scheme)) {
    CurveEdit *ce = new CurveEdit(
        &(dynamic_cast<NL3D::CPSFloatCurve *>(scheme)->_F), _Node, this, this);
    ce->init(this);
    return ce;
  }
...
void CAttribDlgFloat::setCurrentScheme(uint index) {
  ...
  case 3:
    ...
    ((NL3D::CPSAttribMakerMemory<float> *)scheme)
        ->setScheme(new NL3D::CPSFloatBlender(_MinRange, _MaxRange));
    break;
  case 4:
...
public:
  CEditAttribDlg *newDialog(const std::string &id, IPSWrapperUInt *wrapper) {
    CEditableRangeUInt *eru =
        new CEditableRangeUInt(id, wrapper->OwnerNode, MinRange, MaxRange);
...
//*************************************************************************************************************
CAttribDlgUInt::CAttribDlgUInt(const std::string &valueID,
                               CParticleWorkspace::CNode *node, uint32 minRange,
                               uint32 maxRange)
    : CAttribDlgT<uint32>(valueID, node), _MinRange(minRange),
      _MaxRange(maxRange) {
  _CstValueId = valueID;
}
...
CEditAttribDlg *CAttribDlgUInt::createConstantValueDlg() {
  CEditableRangeUInt *erf =
      new CEditableRangeUInt(_CstValueId, _Node, _MinRange, _MaxRange);
  erf->setWrapper(_Wrapper);
  duplicateBoundChecker(*erf);
  return erf;
}
...
std::string CAttribDlgUInt::getSchemeName(uint index) const {
  const char *types[] = {"value blender", "values gradient",
                         "values computed from emitter", "binary operator"};
  nlassert(index < 4);
  return std::string(types[index]);
}
...
CWnd *CAttribDlgUInt::editScheme(void) {
  ...
  if (dynamic_cast<const NL3D::CPSUIntBlender *>(scheme)) {
    ...
    myInterface->SchemeFunc =
        &((NL3D::CPSValueBlenderSample<uint32, 64> *)scheme)->_F;
  ...
  if (dynamic_cast<const NL3D::CPSUIntGradient *>(scheme)) {
    ...
    wrapper->MaxRange = _MaxRange;
    wrapper->Scheme =
        &(((NL3D::CPSUIntGradient *)(_SchemeWrapper->getScheme()))->_F);
    CValueGradientDlg *gd =
        new CValueGradientDlg(wrapper, _Node, true, this, this);
    wrapper->GradDlg = gd;
  ...
  if (dynamic_cast<const NL3D::CPSUIntMemory *>(scheme)) {
    ...
    CAttribDlgUInt *adu =
        new CAttribDlgUInt(_CstValueId, _Node, _MinRange, _MaxRange);
    this->duplicateBoundChecker(*adu);
    CValueFromEmitterDlgT<uint32> *vfe = new CValueFromEmitterDlgT<uint32>(
        (NL3D::CPSUIntMemory *)(scheme), adu, this, m_AttrBitmap.GetBitmap());
    vfe->init(this);
    return vfe;
  }
  if (dynamic_cast<const NL3D::CPSUIntBinOp *>(scheme)) {
    ...
    CBinOpDlgT<uint32> *bod = new CBinOpDlgT<uint32>(
        (NL3D::CPSUIntBinOp *)(scheme), (CAttribDlgT<uint32> **)ad, this,
        m_AttrBitmap.GetBitmap());
    bod->init(this);
    return bod;
  }
...
void CAttribDlgUInt::setCurrentScheme(uint index) {
  ...
  case 2:
    ...
    ((NL3D::CPSAttribMakerMemory<uint32> *)scheme)
        ->setScheme(new NL3D::CPSUIntBlender(_MinRange, _MaxRange));
    break;
  case 3:
...
public:
  CEditAttribDlg *newDialog(const std::string &id,
                            IPSWrapper<sint32> *wrapper) {
    CEditableRangeInt *eri =
        new CEditableRangeInt(id, wrapper->OwnerNode, MinRange, MaxRange);
...
CAttribDlgInt::CAttribDlgInt(const std::string &valueID,
                             CParticleWorkspace::CNode *node, sint32 minRange,
                             sint32 maxRange)
    : CAttribDlgT<sint32>(valueID, node), _MinRange(minRange),
      _MaxRange(maxRange) {
  _CstValueId = valueID;
}
...
CEditAttribDlg *CAttribDlgInt::createConstantValueDlg() {
  CEditableRangeInt *erf =
      new CEditableRangeInt(_CstValueId, _Node, _MinRange, _MaxRange);
  erf->setWrapper(_Wrapper);
  duplicateBoundChecker(*erf);
  return erf;
}
...
std::string CAttribDlgInt::getSchemeName(uint index) const {
  ...
  const char *types[] = {"value exact blender", "values gradient",
                         "values computed from emitter", "binary operator"};
  nlassert(index < 4);
  return std::string(types[index]);
}
...
CWnd *CAttribDlgInt::editScheme(void) {
  ...
  if (dynamic_cast<const NL3D::CPSIntBlender *>(scheme)) {
    ...
    myInterface->SchemeFunc =
        &((NL3D::CPSValueBlenderSample<sint32, 64> *)scheme)->_F;
  ...
  if (dynamic_cast<const NL3D::CPSIntGradient *>(scheme)) {
    ...
    wrapper->MaxRange = _MaxRange;
    wrapper->Scheme =
        &(((NL3D::CPSIntGradient *)(_SchemeWrapper->getScheme()))->_F);
    CValueGradientDlg *gd =
        new CValueGradientDlg(wrapper, _Node, true, this, this);
    wrapper->GradDlg = gd;
    wrapper->DefaultValue = 0;
  ...
  if (dynamic_cast<const NL3D::CPSIntMemory *>(scheme)) {
    CAttribDlgInt *adi =
        new CAttribDlgInt(_CstValueId, _Node, _MinRange, _MaxRange);
    this->duplicateBoundChecker(*adi);
    CValueFromEmitterDlgT<sint32> *vfe = new CValueFromEmitterDlgT<sint32>(
        (NL3D::CPSIntMemory *)_SchemeWrapper->getScheme(), adi, this,
        m_AttrBitmap.GetBitmap());
    vfe->init(this);
    return vfe;
  }
  /*	if (dynamic_cast<const NL3D::CPSIntMemory *>(scheme))
      {
          CValueFromEmitterDlgT<float, CAttribDlgFloat> vfe(
     (NL3D::CPSFloatMemory *)(scheme), std::string("UINT SCHEME"),
     m_AttrBitmap.GetBitmap()); vfe.DoModal();
      }*/
  if (dynamic_cast<const NL3D::CPSIntBinOp *>(scheme)) {
    ...
    CBinOpDlgT<sint32> *bod = new CBinOpDlgT<sint32>(
        (NL3D::CPSIntBinOp *)(scheme), (CAttribDlgT<sint32> **)ad, this,
        m_AttrBitmap.GetBitmap());
    bod->init(this);
    return bod;
  }
...
void CAttribDlgInt::setCurrentScheme(uint index) {
  ...
  case 2:
    ...
    ((NL3D::CPSAttribMakerMemory<sint32> *)scheme)
        ->setScheme(new NL3D::CPSIntBlender(_MinRange, _MaxRange));
    break;
  case 3:
...
public:
  CEditAttribDlg *newDialog(const std::string &id,
                            IPSWrapper<NLMISC::CRGBA> *wrapper) {
...
//*************************************************************************************************************
CAttribDlgRGBA::CAttribDlgRGBA(const std::string &valueID,
                               CParticleWorkspace::CNode *node)
    : CAttribDlgT<CRGBA>(valueID, node) {}
...
std::string CAttribDlgRGBA::getSchemeName(uint index) const {
  const char *types[] = {"color sampled blender", "color gradient",
                         "color exact blender", "values computed from emitter",
                         "binary operator"};
  nlassert(index < 5);
  return std::string(types[index]);
}
...
CWnd *CAttribDlgRGBA::editScheme(void) {
  ...
  if (dynamic_cast<const NL3D::CPSColorBlender *>(scheme)) {
    ...
    myInterface->SchemeFunc =
        &((NL3D::CPSValueBlenderSample<CRGBA, 64> *)scheme)->_F;
    CValueBlenderDlg *vb =
        new CValueBlenderDlg(myInterface, true, this, this, _Node);
    vb->init(this);
    return vb;
  ...
  if (dynamic_cast<const NL3D::CPSColorGradient *>(scheme)) {
    ...
    wrapper->Scheme =
        &(((NL3D::CPSColorGradient *)(_SchemeWrapper->getScheme()))->_F);
    CValueGradientDlg *gd =
        new CValueGradientDlg(wrapper, _Node, true, this, this);
    wrapper->GradDlg = gd;
  ...
  if (dynamic_cast<const NL3D::CPSColorMemory *>(scheme)) {
    ...
    CValueFromEmitterDlgT<CRGBA> *vfe = new CValueFromEmitterDlgT<CRGBA>(
        (NL3D::CPSColorMemory *)(scheme), ad, this, m_AttrBitmap.GetBitmap());
    vfe->init(this);
    return vfe;
  }
  ...
  if (dynamic_cast<const NL3D::CPSColorBinOp *>(scheme)) {
    ...
    CBinOpDlgT<CRGBA> *bod = new CBinOpDlgT<CRGBA>(
        (NL3D::CPSColorBinOp *)(scheme), (CAttribDlgT<CRGBA> **)ad, this,
        m_AttrBitmap.GetBitmap());
    bod->init(this);
    return bod;
  }
...
void CAttribDlgRGBA::setCurrentScheme(uint index) {
  ...
  case 1:
    scheme = new NL3D::CPSColorGradient(
        NL3D::CPSColorGradient::_DefaultGradient, 2, 16, 1.f);
    break;
  case 2:
  ...
  case 3:
    ...
    ((NL3D::CPSAttribMakerMemory<CRGBA> *)scheme)
...
class CPlaneBasisGradientDlgWrapper
    : public CValueGradientDlgClientT<NL3D::CPlaneBasis> {
public:
  /// a function that can display a value in a gradient, with the given offset.
  /// Deriver must define this
  void displayValue(CDC *dc, uint index, sint x, sint y) {

    NLMISC::CRGBA c1[] = {NLMISC::CRGBA::Black, NLMISC::CRGBA::Black,
                          NLMISC::CRGBA::Black};
    NLMISC::CRGBA c2[] = {NLMISC::CRGBA::Green, NLMISC::CRGBA::Green,
                          NLMISC::CRGBA::Red};
  ...
  CEditAttribDlg *newDialog(const std::string &id,
                            IPSWrapper<NL3D::CPlaneBasis> *wrapper) {
...
CAttribDlgPlaneBasis::CAttribDlgPlaneBasis(const std::string &valueID,
                                           CParticleWorkspace::CNode *node)
    : CAttribDlgT<NL3D::CPlaneBasis>(valueID, node) {}
...
std::string CAttribDlgPlaneBasis::getSchemeName(uint index) const {
  const char *types[] = {"basis gradient", "follow path", "spinner",
                         "values computed from emitter", "binary operator"};
  nlassert(index < 5);
  return std::string(types[index]);
}
...
CWnd *CAttribDlgPlaneBasis::editScheme(void) {
  ...
  if (dynamic_cast<NL3D::CPSPlaneBasisGradient *>(scheme)) {
    ...
    wrapper->Scheme =
        &(((NL3D::CPSPlaneBasisGradient *)(_SchemeWrapper->getScheme()))->_F);
    CValueGradientDlg *gd =
        new CValueGradientDlg(wrapper, _Node, true, this, this);
    wrapper->GradDlg = gd;
  ...
  if (dynamic_cast<NL3D::CPSPlaneBasisFollowSpeed *>(scheme)) {
    CEditFollowPath *efp = new CEditFollowPath(
        (NL3D::CPSPlaneBasisFollowSpeed *)scheme, _Node, this, this);
    efp->init(this);
    return efp;
  }
  ...
  if (dynamic_cast<NL3D::CPSPlaneBasisMemory *>(scheme)) {
    ...
    CValueFromEmitterDlgT<NL3D::CPlaneBasis> *vfe =
        new CValueFromEmitterDlgT<NL3D::CPlaneBasis>(
            (NL3D::CPSPlaneBasisMemory *)(scheme), ad, this,
            m_AttrBitmap.GetBitmap());
    vfe->init(this);
    return vfe;
  }
  ...
  if (dynamic_cast<NL3D::CPSPlaneBasisBinOp *>(scheme)) {
    ...
    CBinOpDlgT<NL3D::CPlaneBasis> *bod = new CBinOpDlgT<NL3D::CPlaneBasis>(
        (NL3D::CPSPlaneBasisBinOp *)(scheme),
        (CAttribDlgT<NL3D::CPlaneBasis> **)ad, this, m_AttrBitmap.GetBitmap());
    bod->init(this);
    return bod;
  }

  if (dynamic_cast<NL3D::CPSBasisSpinner *>(scheme)) {
    CEditSpinner *es = new CEditSpinner(
        static_cast<NL3D::CPSBasisSpinner *>(scheme), _Node, this, this);
    es->init(this);
    return es;
  }
  return NULL;
...
void CAttribDlgPlaneBasis::setCurrentScheme(uint index) {
  ...
  case 3:
    ...
    ((NL3D::CPSAttribMakerMemory<NL3D::CPlaneBasis> *)scheme)
...
sint CAttribDlgPlaneBasis::getCurrentScheme(void) const {
  const NL3D::CPSAttribMaker<NL3D::CPlaneBasis> *scheme =
      _SchemeWrapper->getScheme();
...
//*************************************************************************************************************
void CAttribDlg::OnClose() { CDialog::OnClose(); }

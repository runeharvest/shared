...
static void DrawBasis(CDC &dc, sint x, sint y, sint size, const char *xStr,
                      const char *yStr) {
...
static void DrawVector(CDC &dc, float vx, float vy, sint x, sint y, sint size) {
  ...
  dc.LineTo(int(x + size / 2 + vx * 0.9f * size / 2),
            int(y + size / 2 - vy * 0.9f * size / 2));
...
/// private get back a screen coord (pox, py) to a part of a vector. The basis
/// of click is (x, y) - (x + size, y + size)
static void ScreenToVect(float &vx, float &vy, sint px, sint py, sint x, sint y,
                         sint size) {
  vx = (px - (x + size / 2)) / 0.9f;
  vy = ((y + size / 2) - py) / 0.9f;
}
...
void CDirectionEdit::OnPaint() {
  ...
  DrawBasis(dc, CornerDist, CornerDist + BasisGap + BasisSize, BasisSize, "Y",
            "Z");
  DrawVector(dc, v.y, v.z, CornerDist, CornerDist + BasisGap + BasisSize,
             BasisSize);
}
...
void CDirectionEdit::selectNewVect(const CPoint &point) {
  ...
  NLMISC::CVector v = _Wrapper->get();
  if (point.x > CornerDist && point.y > CornerDist &&
      point.x < (CornerDist + BasisSize) &&
      point.y < (CornerDist + BasisSize)) {
  ...
  if (point.x > CornerDist && point.y > (BasisGap + BasisSize + CornerDist) &&
      point.x < (CornerDist + BasisSize) &&
      point.y < (CornerDist + BasisGap + 2 * BasisSize)) {
    ScreenToVect(v.y, v.z, point.x, point.y, CornerDist,
                 CornerDist + BasisGap + BasisSize, BasisSize);

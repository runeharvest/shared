...
void CGraph::renderGraph() {
  ...
  float ih = 1.f / h;
  NL3D::CViewport vp;
  CDRU::drawQuad(X * iw, Y * ih, (X + Width) * iw, (Y + Height) * ih,
                 *CNELU::Driver, BackColor, vp);
  ...
  for (deque<float>::reverse_iterator it = Values.rbegin(); it != Values.rend();
    ...
    CDRU::drawLine(vect1.x * iw, vect1.y * ih, pos * iw, PrevY * ih,
                   *CNELU::Driver, lineCol);
  ...
  CDRU::drawLine(X * iw, peakval * ih, (X + Width) * iw, peakval * ih,
                 *CNELU::Driver, frontCol);
  ...
  CDRU::drawLine(X * iw, avrval * ih, (X + Width) * iw, avrval * ih,
                 *CNELU::Driver, frontCol);
}
...
void CGraph::addValue(float value) {
  TTime currentTime = (uint64)(1000 * NLMISC::CTime::ticksToSecond(

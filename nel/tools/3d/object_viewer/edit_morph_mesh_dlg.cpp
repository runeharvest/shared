...
CEditMorphMeshDlg::CEditMorphMeshDlg(CParticleWorkspace::CNode *ownerNode,
                                     NL3D::CPSConstraintMesh *cm, CWnd *pParent,
                                     CParticleDlg *particleDlg,
                                     IPopupNotify *pn /*= NULL*/)
    : _Node(ownerNode), _PN(pn), _CM(cm),
      CDialog(CEditMorphMeshDlg::IDD, pParent), _ParticleDlg(particleDlg) {
  nlassert(cm);
...
bool CEditMorphMeshDlg::getShapeNameFromDlg(std::string &name) {
  ...
  if (fd.DoModal() == IDOK) {
    ...
    (NLMISC::CFile::getPath(tStrToUtf8(fd.GetPathName()));
    */
...
void CEditMorphMeshDlg::OnDown() {
  sint selItem = m_MeshList.GetCurSel();
  if (selItem == (sint)(_CM->getNumShapes() - 1))
    return;
  std::vector<std::string> shapeNames;
  shapeNames.resize(_CM->getNumShapes());
  _CM->getShapesNames(&shapeNames[0]);
...
//====================================================================
CEditMorphMeshDlg::CMorphSchemeWrapper::scheme_type *
CEditMorphMeshDlg::CMorphSchemeWrapper::getScheme(void) const {
  nlassert(CM);
...
BOOL CEditMorphMeshDlg::OnInitDialog() {
  ...
  ScreenToClient(&r);
  HBITMAP bmh =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_MORPH_SCHEME));
...
//====================================================================
std::string CEditMorphMeshDlg::getShapeDescStr(uint shapeIndex,
                                               sint numVerts) const {
  if (numVerts >= 0) {
    CString verts;
    verts.LoadString(IDS_VERTICES);
    std::string msg = _CM->getShape(shapeIndex) + " (" +
                      NLMISC::toString(numVerts) + " " +
                      NLMISC::tStrToUtf8(verts) + ")";
    return msg;
  } else {
    std::string result =
        _CM->getShape(shapeIndex) + " (" +
        NLMISC::tStrToUtf8(CMeshDlg::getShapeErrorString(numVerts)) + ")";
    return result;
  }
}

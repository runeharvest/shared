...
CTuneMrmDlg::CTuneMrmDlg(CObjectViewer *viewer, NL3D::CScene *scene,
                         CWnd *pParent /*=NULL*/)
...
void CTuneMrmDlg::OnReleasedcaptureTmdSliderMax(NMHDR *pNMHDR,
                                                LRESULT *pResult) {
  // Get value from slider
  applySlider(FaceSlider.GetPos(), MaxValueSlider.GetPos());
  *pResult = 0;
...
void CTuneMrmDlg::applySlider(UINT sliderCur, UINT sliderMax) {
  nlctassert(NL_TMD_SLIDER_SIZE * NL_TMD_SLIDER_MAX_STEP *
                 NL_TMD_SLIDER_STEP_SIZE <
             ((uint)1 << 31));
  // compute actual max polygon
  uint actualValue =
      sliderCur * sliderMax * NL_TMD_SLIDER_STEP_SIZE / NL_TMD_SLIDER_SIZE;
...
void CTuneMrmDlg::OnClose() { _ObjViewer->getMainFrame()->OnWindowTuneMRM(); }

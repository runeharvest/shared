...
float CSoundAnimView::_ZoomValue[] = {0.1f, 0.25f, 0.5f, 1.0f,
                                      2.0f, 4.0f,  10.0f};
...
bool CSoundAnimView::registerClass() {
  ...
  _WndClass = AfxRegisterWndClass(CS_VREDRAW | CS_HREDRAW,
...
void CSoundAnimView::Create(CObjectViewer *objView, CAnimationDlg *animDlg,
                            CSoundAnimDlg *sndDlg, const RECT &rect) {
  registerClass();
  ...
  CWnd::Create((LPCTSTR)_WndClass, _T("Sound Animation"), WS_CHILD | WS_VISIBLE,
               rect, (CWnd *)sndDlg, ++_WndId);
}
...
void CSoundAnimView::mark() { insertMarkerAt(_AnimationDlg->getTime()); }
    ...
    if (anim->isDirty()) {
      ...
      if (filename.empty()) {
        ...
        TCHAR BASED_CODE szFilter[] =
            ...
            _T("(*.*)|*.*||");
        CFileDialog fileDlg(FALSE, _T(".sound_anim"), nlUtf8ToTStr(filename),
...
void CSoundAnimView::refresh(BOOL update) {
  ...
  if (!instanceInfo->Saved.PlayList.empty()) {
    ...
    for (uint index = 0; index < instanceInfo->Saved.PlayList.size(); index++) {
      ...
      CAnimation *anim = instanceInfo->AnimationSet.getAnimation(
          instanceInfo->AnimationSet.getAnimationIdByName(name));
      ...
      if (soundAnim == 0) {
        ...
        try {
          ...
          nlwarning("Couldn't find sound animation <%s>: %s", name.c_str(),
                    e.what());
          needCreate = true;
        }
        if (needCreate) {
...
void CSoundAnimView::insertMarkerAt(float time) {
  ...
  if (getAnimationAt(holder, time)) {
    ...
    CSoundAnimMarker *marker =
        new CSoundAnimMarker((float)(time - holder._AnimStart));
...
void CSoundAnimView::OnPaint() {
  ...
  for (iter = _Animations.begin(); iter != _Animations.end(); iter++) {
    ...
    dc->DrawText(_StringBuffer.c_str(), &r,
    ...
    for (uint32 i = 0; i < nmarkers; i++) {
      ...
      dc->FillRect(&r,
                   (_SelectedMarker == marker) ? &_SelectBrush : &_MarkerBrush);
    }
  }
...
void CSoundAnimView::AssertValid() const { CWnd::AssertValid(); }

// ***************************************************************************

void CSoundAnimView::Dump(CDumpContext &dc) const { CWnd::Dump(dc); }

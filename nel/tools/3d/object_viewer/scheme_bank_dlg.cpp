...
void CSchemeBankDlg::OnSelchangeSchemeList() {
  ...
  _CurrScheme = (NL3D::CPSAttribMakerBase *)m_SchemeList.GetItemData(
      m_SchemeList.GetCurSel());
}
...
void CSchemeBankDlg::buildList() {
  ...
  for (TSchemeVect::const_iterator it = schemes.begin(); it != schemes.end();
       ++it) {
...
void CSchemeBankDlg::OnSaveBank() {
  ...
  if (fd.DoModal() == IDOK) {
    ...
    try {
      ...
      std::string message =
          NLMISC::toString("Error saving scheme bank : %s", e.what());
      MessageBox(nlUtf8ToTStr(message), _T("Object viewer"),
...
void CSchemeBankDlg::OnLoadBank() {
  ...
  if (fd.DoModal() == IDOK) {
    ...
    try {
      ...
      std::string message =
          NLMISC::toString("Error loading scheme bank : %s", e.what());
      MessageBox(nlUtf8ToTStr(message), _T("Object viewer"),
                 MB_ICONEXCLAMATION | MB_OK);
      return;
    }
...
void CSchemeBankDlg::OnRemove() {
  ...
  if (m_SchemeList.GetCurSel() == LB_ERR)
    return;
  SchemeManager.remove((NL3D::CPSAttribMakerBase *)m_SchemeList.GetItemData(
      m_SchemeList.GetCurSel()));
...
void CSchemeBankDlg::OnRename() {
  ...
  if (cn.DoModal() == IDOK) {
    NL3D::CPSAttribMakerBase *scheme =
        (NL3D::CPSAttribMakerBase *)m_SchemeList.GetItemData(
            m_SchemeList.GetCurSel());
    SchemeManager.rename(scheme, cn.getName());
    int curSel = m_SchemeList.GetCurSel();
    m_SchemeList.DeleteString(curSel);
    int insertedPos =
        m_SchemeList.InsertString(curSel, nlUtf8ToTStr(cn.getName()));

...
#if !defined(                                                                  \
    AFX_TEXTURE_ANIM_DLG_H__4A689FB0_93B9_4F5A_8075_8006D8FD19B2__INCLUDED_)
...
public:
  CTextureAnimDlg(
      CParticleWorkspace::CNode *ownerNode, NL3D::CPSTexturedParticle *p,
      NL3D::CPSMultiTexturedParticle *mtp = NULL); // standard constructor
  ~CTextureAnimDlg();
...
protected:
  ...
  void cleanCtrl(void);
  // create the right controls, depending on the fact that a an,imated texture
  // is used or not
  void setupCtrl(void);
  //////////////////////////////////
  ...
  struct CTextureIndexWrapper : public IPSWrapper<sint32>,
                                IPSSchemeWrapper<sint32> {
    NL3D::CPSTexturedParticle *P;
  ...
  /**
   * the implementation of this struct tells the gradient dialog bow how to edit
   * a texture list
   */

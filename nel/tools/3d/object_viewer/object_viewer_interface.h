...
public:
  ...
  // Go. It shouldn't be called if there's an instance of the viewer that is
  // running.
  virtual void go() = 0;
  ...
  // Add a mesh
  virtual uint addMesh(NL3D::IShape *pMeshShape, const std::string &meshName,
                       uint skelIndex, const char *bindSkelName = NULL,
                       bool createInstance = true) = 0;

  // Add a skel
  virtual uint addSkel(NL3D::IShape *pSkelShape,
                       const std::string &skelName) = 0;

  // Add a camera
  virtual uint addCamera(const NL3D::CCameraInfo &cameraInfo,
  ...
  // Load a mesh
  virtual bool loadMesh(std::vector<std::string> &meshFilename,
                        const std::string &skeleton) = 0;

  // Load a shape
  virtual void resetCamera() = 0;

  // Set single animation
  virtual void setSingleAnimation(NL3D::CAnimation *pAnim,
                                  const std::string &name, uint instance) = 0;
  ...
  /** set the water pool manager used by the object viewer. Must be the same
   * than tyhe one of the dll which created the models (because the 3d lib is
   * duplicated : one version in the viewer, and one version in the exporter)
   */
  ...
  virtual void setupSceneLightingSystem(bool enable,
                                        ...
                                        NLMISC::CRGBA sunSpecular) = 0;

  /** Scene lighting System: enable dynamic object testing. Give the ig the
   *shape must be light_tested against globalRetriever is not deleted by the
   *ObjectViewer.
   */
  virtual void
  enableDynamicObjectLightingTest(NLPACS::CGlobalRetriever *globalRetriever,
                                  NL3D::CInstanceGroup *ig) = 0;

  // Get instance
  static OBJECT_VIEWER_EXPORT IObjectViewer *
  getInterface(int version = OBJECT_VIEWER_VERSION);

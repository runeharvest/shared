...
public:
   ...
   *  If the ID wasn't present, a new entry is created, and the given values are
   * returned
   */
  static std::pair<T, T> GetRange(const std::string &id, const T &minRange,
                                  const T &maxRange) {
    nlassert(id != "");
  ...
  /** the same but no default values are provided. An assertion  occurs if not
   * present. In release build, the entrie is created, but with uninitialized
   * values ...
   */
  ...
  static void SetRange(const std::string &id, const T &minRange,
                       const T &maxRange) {
    nlassert(id != "");
    _RangeMap[id] = std::pair<T, T>(minRange, maxRange);
  }
  ...
  static void serial(NLMISC::IStream &f) {
    ...
    if (!f.isReading()) {
      size = _RangeMap.size();
      f.serial(size);
      for (TRangeMap::const_iterator it = _RangeMap.begin();
           it != _RangeMap.end(); ++it) {

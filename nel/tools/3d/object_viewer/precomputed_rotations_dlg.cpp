...
CPrecomputedRotationsDlg::CPrecomputedRotationsDlg(
    CParticleWorkspace::CNode *ownerNode,
    NL3D::CPSHintParticleRotateTheSame *prts, CAttribDlg *toDisable)
...
void CPrecomputedRotationsDlg::updateFromReader(void) {
  ...
  if (m_PrecomputedRotations) {
    ...
    sprintf(out, "%d",
            _RotatedParticle->checkHintRotateTheSame(minVelocity, maxVelocity));
    m_NbModels = out;
...
void CPrecomputedRotationsDlg::OnUpdateNbModels() {
  ...
  sint32 newNbModels;
  bool valid =
      (NLMISC::fromString(NLMISC::tStrToUtf8(m_NbModels), newNbModels) &&
       newNbModels > 0);
  ...
  if (valid) {
    _RotatedParticle->checkHintRotateTheSame(valueMin, valueMax);
    _RotatedParticle->hintRotateTheSame((uint32)newNbModels, valueMin,
                                        valueMax);
    updateModifiedFlag();
  } else {
    MessageBox(_T("invalid value !!"));

...
CPSMoverDlg::CPSMoverDlg(CParticleWorkspace::CNode *ownerNode, CWnd *parent,
                         NL3D::CEvent3dMouseListener *ml,
                         NL3D::CPSLocated *editedLocated,
                         uint32 editedLocatedIndex) // standard constructor
    : CDialog(CPSMoverDlg::IDD, parent), _Node(ownerNode),
      _EditedLocated(editedLocated), _EditedLocatedIndex(editedLocatedIndex),
      _DirectionDlg(NULL), _MouseListener(ml), _Scale(NULL), _XScale(NULL),
      _YScale(NULL), _ZScale(NULL), _ScaleText(NULL), _XScaleText(NULL),
      _YScaleText(NULL), _ZScaleText(NULL) {
...
BOOL CPSMoverDlg::OnInitDialog() {
  ...
  for (uint k = 0; k < numBound; ++k) {
    if (dynamic_cast<NL3D::IPSMover *>(_EditedLocated->getBoundObject(k))) {
      uint insertedLine = m_SubComponentCtrl.AddString(
          nlUtf8ToTStr(_EditedLocated->getBoundObject(k)->getName()));
      m_SubComponentCtrl.SetItemData(
          insertedLine, (DWORD_PTR)_EditedLocated->getBoundObject(k));
      ++nbCandidates;
    }
  }
  ...
  if (nbCandidates > 0) {
    ...
    _EditedLocated->getOwner()->setCurrentEditedElement(
        _EditedLocated, _EditedLocatedIndex,
        (NL3D::CPSLocatedBindable *)m_SubComponentCtrl.GetItemData(0));
  }
...
void CPSMoverDlg::createScaleControls(void) {
  ...
  if (m->supportUniformScaling() && !m->supportNonUniformScaling()) {
    ...
    _ScaleText->Create(_T("Scale : "), SS_LEFT,
                       CRect(xPos, yPos + 10, xPos + 60, yPos + 32), this);
    ...
    _XScaleText->Create(_T("x Scale : "), SS_LEFT,
                        CRect(xPos, yPos + 10, xPos + 60, yPos + 32), this);
    ...
    _YScaleText->Create(_T("y Scale : "), SS_LEFT,
                        CRect(xPos, yPos + 10, xPos + 60, yPos + 32), this);
    ...
    _ZScaleText->Create(_T("z Scale : "), SS_LEFT,
                        CRect(xPos, yPos + 10, xPos + 60, yPos + 32), this);
...
NL3D::IPSMover *CPSMoverDlg::getMoverInterface(void) {
  ...
  if (currIndex == -1)
    return NULL;
  return dynamic_cast<NL3D::IPSMover *>(
      (NL3D::CPSLocatedBindable *)(m_SubComponentCtrl.GetItemData(currIndex)));
}

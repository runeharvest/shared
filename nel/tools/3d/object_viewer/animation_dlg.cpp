...
void CAnimationDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) {
  ...
  if (pScrollBar == GetDlgItem(IDC_TIME_LINE)) {
    ...
    // Setup current pos
    CurrentFrame =
        (float)TimeLineCtrl.GetPos() * (End - Start) / 65535.f + Start;
...
void CAnimationDlg::handle() {
  ...
  // Forward or rewind ?
  bool forward =
      (((CButton *)GetDlgItem(IDC_FFW))->GetState() & BST_PUSHED) != 0;
  bool rewind =
      (((CButton *)GetDlgItem(IDC_FRW))->GetState() & BST_PUSHED) != 0;
  ...
  if (forward || rewind || Playing) {
    ...
    if (forward) {
      // Fast forward
      CurrentFrame +=
          (float)((float)deltaTime * SPEED_FOREWARD * Speed / 1000.0);
    } else if (rewind) {
      // Fast rewind
      CurrentFrame -=
          (float)((float)deltaTime * SPEED_FOREWARD * Speed / 1000.0);

...
public:
  ...
  public:
    ...
    // Serial node information into workspace stream. This does not save the
    // particle system shape, only a reference to its file
     ...
     * \return true if loading succeed (false means that loading was ok, but
     * this is not a particle system). Other cases throw an exception.
     */
    bool loadPS();
    ...
    // Memorize current position of object in the system. Useful to play the
    // system because instances can be created / deleted
    void memorizeState();
    // Restore state previously memorize. Is usually called when the user stops
    // a particle system
    void restoreState();
    // Test if state is currenlty memorized
    bool isStateMemorized() const;
    // For edition : If the state of the system has been memorized, keep it on
    // par with the system when it is modified
  ...
  private:
    ...
    NL3D::CShapeBank
        *_ShapeBank; // Keep a shape bank per node because we want the whole
                     // path to identify the ps, not just its filename (shape
                     // bank keeps the filename only)
    std::string _RelativePath; // relative path from which the ps was inserted
                               // relative path is also a unique identifier for
                               // this ps in the workspace
    CPSInitialPos
        _InitialPos; // initial pos of system. Allow to restore the initial
                     // instances of the system  when doing start / stop
    bool _Modified;
  ...
  struct IModificationCallback {
    ...
    virtual void
    nodeSkelParentChanged(CNode &node) = 0; // called when fx has been linked /
                                            // unlinked from a skeleton parent
  };
  // sorting predicate
  struct ISort {
  ...
  void init(CObjectViewer *ov, const std::string &filename,
   ...
   * NB : 2 node with the same name re not allowed, even if their path is
   * different
   */
  bool containsFile(std::string filename) const;

  /** Add a node in the workspace. Will succeed only if fx filename does not
   * already exist in the workspace. The node is in the 'unloaded' state, so
   * caller must load it afterward. NB : no lookup is done, full path must be
   * provided.
   *
  ...
  /** Save the workspace structure. The target file is the one given when this
   * object was created NB : ps shape are not saved, only the structure is. To
   * save the shapes, call CNode::save()
   */
  void save();
  /** Load the workspace structure. The target file is the one given when this
   * object was created All nodes are in the 'unloaded" state, so it is to the
   * caller to load them by calling load() on their node
   */
  void load();
  // Test whether the structure of the workspace has been modified (does not
  // test if ps inside the workspace have been modified)
  bool isModified() const { return _Modified; }

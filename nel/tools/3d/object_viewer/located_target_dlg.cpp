...
CLocatedTargetDlg::CLocatedTargetDlg(CParticleWorkspace::CNode *ownerNode,
    ...
    : _Node(ownerNode), _LBTarget(lbTarget), _ParticleDlg(particleDlg) {
...
void CLocatedTargetDlg::OnAddTarget() {
  ...
  if (dynamic_cast<NL3D::CPSForce *>(_LBTarget)) {
    ...
    for (int k = 0; k < selCount; ++k) {
      NL3D::CPSLocated *loc =
          (NL3D::CPSLocated *)m_AvailableTargets.GetItemData(indexs[k] - k);
      nlassert(loc);
    ...
    if (forEverLastingTarget) {
      ...
      if (MessageBox((LPCTSTR)messStr, (LPCTSTR)warningStr, MB_OKCANCEL) !=
          IDOK) {
        return;
      }
    }
...
void CLocatedTargetDlg::OnRemoveTarget() {
  ...
  for (int k = 0; k < selCount; ++k) {
    NL3D::CPSLocated *loc =
        (NL3D::CPSLocated *)m_Targets.GetItemData(indexs[k] - k);
    nlassert(loc);
...
BOOL CLocatedTargetDlg::OnInitDialog() {
  ...
  if (dynamic_cast<NL3D::CPSZone *>(_LBTarget)) {
    CCollisionZoneDlg *czd = new CCollisionZoneDlg(
        _Node, dynamic_cast<NL3D::CPSZone *>(_LBTarget), _ParticleDlg);
    pushWnd(czd);
  ...
  if (dynamic_cast<NL3D::CPSForceIntensity *>(_LBTarget)) {
    _ForceIntensityWrapper.F =
        dynamic_cast<NL3D::CPSForceIntensity *>(_LBTarget);
    CAttribDlgFloat *fi =
        new CAttribDlgFloat(std::string("FORCE INTENSITY"), _Node, 0, 100);
    pushWnd(fi);
    ...
    HBITMAP bmh = LoadBitmap(::AfxGetInstanceHandle(),
                             MAKEINTRESOURCE(IDB_FORCE_INTENSITY));
  ...
  if (dynamic_cast<NL3D::CPSCylindricVortex *>(_LBTarget)) {
    CEditableRangeFloat *rv =
        new CEditableRangeFloat(std::string("RADIAL_VISCOSITY"), _Node, 0, 1);
    pushWnd(rv);
    _RadialViscosityWrapper.V =
        dynamic_cast<NL3D::CPSCylindricVortex *>(_LBTarget);
    rv->setWrapper(&_RadialViscosityWrapper);
    rv->init(posX + 140, posY, this);
    CStatic *s = new CStatic;
    pushWnd(s);
    s->Create(_T("Radial viscosity : "), SS_LEFT,
              CRect(posX, posY, posX + 139, posY + 32), this);
    ...
    CEditableRangeFloat *tv = new CEditableRangeFloat(
        std::string("TANGENTIAL_VISCOSITY"), _Node, 0, 1);
    pushWnd(tv);
    _TangentialViscosityWrapper.V =
        dynamic_cast<NL3D::CPSCylindricVortex *>(_LBTarget);
    ...
    s = new CStatic;
    pushWnd(s);
    s->Create(_T("Tangential Viscosity : "), SS_LEFT,
              CRect(posX, posY, posX + 139, posY + 32), this);
    s->ShowWindow(SW_SHOW);
  ...
  if (dynamic_cast<NL3D::CPSBrownianForce *>(_LBTarget)) {
    CEditableRangeFloat *rv =
        new CEditableRangeFloat(std::string("PARAMETRIC_FACTOR"), _Node, 0, 64);
    pushWnd(rv);
    ...
    CStatic *s = new CStatic;
    pushWnd(s);
    s->Create(_T("Parametric factor : "), SS_LEFT,
              CRect(posX, posY, posX + 139, posY + 40), this);
    s->SetFont(CFont::FromHandle((HFONT)GetStockObject(DEFAULT_GUI_FONT)));
    s->ShowWindow(SW_SHOW);

...
#if !defined(                                                                  \
    AFX_VALUE_FROM_EMITTER_DLG_H__D3416DBF_1735_4FBB_A1FD_7E8DCC133825__INCLUDED_)
...
class CValueFromEmitterDlg : public CDialog {
  // Construction
public:
  CValueFromEmitterDlg(IPopupNotify *pn,
...
/** construct a dialog that allow to edit a scheme used for initial attribute
 * generation in a particle
 */
template <class T> class CValueFromEmitterDlgT : public CValueFromEmitterDlg {
public:
  CValueFromEmitterDlgT(NL3D::CPSAttribMakerMemory<T> *editedScheme,
                        CAttribDlgT<T> *srcDlg, IPopupNotify *pn,
                        HBITMAP bitmapToDisplay)
      : CValueFromEmitterDlg(pn), _BitmapToDisplay(bitmapToDisplay),
        _AttrbDlg(srcDlg) {
    nlassert(srcDlg);

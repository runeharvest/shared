...
///==================================================================
CMeshDlg::CMeshDlg(CParticleWorkspace::CNode *ownerNode,
                   NL3D::CPSShapeParticle *sp, CParticleDlg *particleDlg)
    : _Node(ownerNode), _ShapeParticle(sp), _EMMD(NULL),
...
BOOL CMeshDlg::OnInitDialog() {
  ...
  if (!dynamic_cast<NL3D::CPSConstraintMesh *>(_ShapeParticle)) {
    ...
    NL3D::CPSConstraintMesh *cm =
        NLMISC::safe_cast<NL3D::CPSConstraintMesh *>(_ShapeParticle);
...
void CMeshDlg::updateMeshErrorString() {
  ...
  NL3D::CPSConstraintMesh *cm =
      dynamic_cast<NL3D::CPSConstraintMesh *>(_ShapeParticle);
  if (!cm)
    return;
  ...
  if (numVerts.empty())
    return;
  if (numVerts.size() == 1) {
    GetDlgItem(IDC_MESH_ERROR)
        ->SetWindowText((LPCTSTR)getShapeErrorString(numVerts[0]));
  } else {
...
CString CMeshDlg::getShapeErrorString(sint errorCode) {
  ...
  case NL3D::CPSConstraintMesh::ShapeFileIsNotAMesh:
    str.LoadString(IDS_SHAPE_FILE_NOT_MESH);
    break;
  case NL3D::CPSConstraintMesh::ShapeFileNotLoaded:
    str.LoadString(IDS_SHAPE_NOT_LOADED);
    break;
  case NL3D::CPSConstraintMesh::ShapeHasTooMuchVertices:
    str.LoadString(IDS_TOO_MUCH_VERTICES);
    break;
  default:
    break;
  };

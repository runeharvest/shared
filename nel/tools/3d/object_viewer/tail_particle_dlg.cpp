...
CTailParticleDlg::CTailParticleDlg(CParticleWorkspace::CNode *ownerNode,
                                   NL3D::CPSTailParticle *tp) {
  nlassert(tp);
  _TailParticle = tp;
  _Node = ownerNode;
...
void CTailParticleDlg::init(CWnd *pParent, sint x, sint y) {
  ...
  if (!dynamic_cast<NL3D::CPSRibbon *>(_TailParticle)) {
    ...
    ///		m_TailPersistAfterDeath = (dynamic_cast<NL3D::CPSRibbon
    ///*>(_TailParticle))->getPersistAfterDeath();
  }
  NL3D::CPSRibbon *ribbon = dynamic_cast<NL3D::CPSRibbon *>(_TailParticle);
  if (ribbon) {
    ((CComboBox *)GetDlgItem(IDC_RIBBON_ORIENTATION))
        ->SetCurSel(ribbon->getOrientation());
  } else {
...
void CTailParticleDlg::OnTailPersistAfterDeath() {
  ...
  ///*>(_TailParticle))->setPersistAfterDeath(m_TailPersistAfterDeath ? true :
  ///false);
  _Node->setModified(true);
  UpdateData(FALSE);
}
...
void CTailParticleDlg::OnSelchangeTailShape() {
  ...
  case 0: // triangle
    r->setShape(NL3D::CPSRibbon::Triangle,
  ...
  case 1: // quad
    ...
    break;
  case 2: // octogon
    r->setShape(NL3D::CPSRibbon::HeightSides,
                NL3D::CPSRibbon::NbVerticesInHeightSide);
    break;
  case 3: // pentagram
    r->setShape(NL3D::CPSRibbon::Pentagram,
                NL3D::CPSRibbon::NbVerticesInPentagram);
    break;
  case 4: // simple segment x
    r->setShape(NL3D::CPSRibbon::SimpleSegmentX,
                NL3D::CPSRibbon::NbVerticesInSimpleSegmentX, true);
    break;
  case 5: // simple segment y
    r->setShape(NL3D::CPSRibbon::SimpleSegmentY,
                NL3D::CPSRibbon::NbVerticesInSimpleSegmentY, true);
    break;
  case 6: // simple segment z
    r->setShape(NL3D::CPSRibbon::SimpleSegmentZ,
                NL3D::CPSRibbon::NbVerticesInSimpleSegmentZ, true);
    break;
  case 7: // simple brace
    r->setShape(NL3D::CPSRibbon::SimpleBrace,
                NL3D::CPSRibbon::NbVerticesInSimpleBrace, true);
    break;
  }
  _Node->setModified(true);
  Invalidate();
  ...
  if (r) {
    ...
    if (r->getBraceMode()) {
      for (uint k = 0; k < verts.size() / 2; ++k) {
        dc.MoveTo((int)(x + (size / 2) * (1 + verts[2 * k].x)),
                  (int)(y + (size / 2) * (1 - verts[2 * k].y)));
        dc.LineTo((int)(x + (size / 2) * (1 + verts[2 * k + 1].x)),
                  (int)(y + (size / 2) * (1 - verts[2 * k + 1].y)));
      }
    } else {
      dc.MoveTo((int)(x + (size / 2) * (1 + verts[0].x)),
                (int)(y + (size / 2) * (1 - verts[0].y)));
      for (std::vector<NLMISC::CVector>::const_iterator it = verts.begin();
           it != verts.end(); ++it) {
        dc.LineTo((int)(x + (size / 2) * (1 + it->x)),
                  (int)(y + (size / 2) * (1 - it->y)));
      }
      dc.LineTo((int)(x + (size / 2) * (1 + verts[0].x)),
                (int)(y + (size / 2) * (1 - verts[0].y)));
    }
    dc.SelectObject(old);
  }
...
void CTailParticleDlg::OnSelchangeRibbonOrientation() {
  NL3D::CPSRibbon *r = dynamic_cast<NL3D::CPSRibbon *>(_TailParticle);
  nlassert(r);
  r->setOrientation((NL3D::CPSRibbon::TOrientation)(
                        (CComboBox *)GetDlgItem(IDC_RIBBON_ORIENTATION))
                        ->GetCurSel());
  _Node->setModified(true);
}

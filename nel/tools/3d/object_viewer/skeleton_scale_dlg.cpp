...
CSkeletonScaleDlg::CSkeletonScaleDlg(CObjectViewer *viewer,
                                     CWnd *pParent /*=NULL*/)
...
ON_WM_DESTROY()
ON_WM_VSCROLL()
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_BONE_SX,
          OnReleasedcaptureSsdSliderBoneSx)
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_BONE_SY,
          OnReleasedcaptureSsdSliderBoneSy)
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_BONE_SZ,
          OnReleasedcaptureSsdSliderBoneSz)
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_SKIN_SX,
          OnReleasedcaptureSsdSliderSkinSx)
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_SKIN_SY,
          OnReleasedcaptureSsdSliderSkinSy)
ON_NOTIFY(NM_RELEASEDCAPTURE, IDC_SSD_SLIDER_SKIN_SZ,
...
// ***************************************************************************
void CSkeletonScaleDlg::setSkeletonToEdit(NL3D::CSkeletonModel *skel,
...
// ***************************************************************************
void CSkeletonScaleDlg::OnVScroll(UINT nSBCode, UINT nPos,
                                  CScrollBar *pScrollBar) {
  CSliderCtrl *sliderCtrl = (CSliderCtrl *)pScrollBar;
  TScaleId sliderId = getScaleIdFromSliderCtrl(sliderCtrl);
  if (sliderId != SidNone && nSBCode == SB_THUMBPOSITION ||
      nSBCode == SB_THUMBTRACK) {
    // If the user press ALT-Tab while dragging an old slider, the old slider is
    // not released. ThereFore, release the old one now
    ...
    if (_SliderEdited == SidNone) {
      ...
      // Bkup all scales (dont bother selected bones or which scale is
      // edited...)
      _BkupBones = _Bones;
    }
...
// ***************************************************************************
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderBoneSx(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
  onSliderReleased(SidBoneX);
  *pResult = 0;
}
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderBoneSy(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
  onSliderReleased(SidBoneY);
  *pResult = 0;
}
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderBoneSz(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
  onSliderReleased(SidBoneZ);
  *pResult = 0;
}
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderSkinSx(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
  onSliderReleased(SidSkinX);
  *pResult = 0;
}
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderSkinSy(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
  onSliderReleased(SidSkinY);
  *pResult = 0;
}
void CSkeletonScaleDlg::OnReleasedcaptureSsdSliderSkinSz(NMHDR *pNMHDR,
                                                         LRESULT *pResult) {
...
void CSkeletonScaleDlg::onSliderReleased(TScaleId sid) {
  ...
  // push an undo/redo only at release of slide. push the value of scale before
  // slide
...
void CSkeletonScaleDlg::applyScaleSlider(sint scrollValue) {
  ...
  float scale =
      (NL_SSD_SLIDER_SIZE / 2 - scrollValue) / (float)(NL_SSD_SLIDER_SIZE / 2);
  ...
  for (uint i = 0; i < _Bones.size(); i++) {
    ...
    if (bone.Selected) {
      ...
      case SidBoneX:
        bone.BoneScale.x = bkup.BoneScale.x * factor;
        break;
      case SidBoneY:
        bone.BoneScale.y = bkup.BoneScale.y * factor;
        break;
      case SidBoneZ:
        bone.BoneScale.z = bkup.BoneScale.z * factor;
        break;
      case SidSkinX:
        bone.SkinScale.x = bkup.SkinScale.x * factor;
        break;
      case SidSkinY:
        bone.SkinScale.y = bkup.SkinScale.y * factor;
        break;
      case SidSkinZ:
        bone.SkinScale.z = bkup.SkinScale.z * factor;
        break;
      };
      // round result
...
void CSkeletonScaleDlg::applyMirrorToSkeleton() {
  if (_SkeletonModel) {
    ...
    for (uint i = 0; i < _Bones.size(); i++) {
      ...
      _SkeletonModel->Bones[i].setScale(_Bones[i].BoneScale /
                                        NL_SSD_SCALE_PRECISION);
      _SkeletonModel->Bones[i].setSkinScale(_Bones[i].SkinScale /
                                            NL_SSD_SCALE_PRECISION);
    }
  }
}
...
void CSkeletonScaleDlg::applySkeletonToMirror() {
  if (_SkeletonModel) {
    ...
    for (uint i = 0; i < _SkeletonModel->Bones.size(); i++) {
      ...
      _Bones[i].SkinScale =
          _SkeletonModel->Bones[i].getSkinScale() * NL_SSD_SCALE_PRECISION;
      _Bones[i].BoneScale =
          _SkeletonModel->Bones[i].getScale() * NL_SSD_SCALE_PRECISION;
  ...
  for (uint i = 0; i < _Bones.size(); i++) {
    ...
    if (bone.Selected) {
      if (!someSelected) {
        ...
        if (boneScaleAll.x != bone.BoneScale.x)
          boneScaleDiff.x = 1.f;
        if (boneScaleAll.y != bone.BoneScale.y)
          boneScaleDiff.y = 1.f;
        if (boneScaleAll.z != bone.BoneScale.z)
          boneScaleDiff.z = 1.f;
        if (skinScaleAll.x != bone.SkinScale.x)
          skinScaleDiff.x = 1.f;
        if (skinScaleAll.y != bone.SkinScale.y)
          skinScaleDiff.y = 1.f;
        if (skinScaleAll.z != bone.SkinScale.z)
          skinScaleDiff.z = 1.f;
      }
    }
  }
...
// ***************************************************************************
void CSkeletonScaleDlg::refreshTextViewWithScale(TScaleId sid, float scale,
                                                 float diff) {
...
// ***************************************************************************
CSkeletonScaleDlg::TScaleId
CSkeletonScaleDlg::getScaleIdFromSliderCtrl(CSliderCtrl *sliderCtrl) const {
...
// ***************************************************************************
CSkeletonScaleDlg::TScaleId
CSkeletonScaleDlg::getScaleIdFromEditId(UINT ctrlId) const {
  nlctassert(SidCount == 6);
  if (ctrlId == IDC_SSD_EDIT_BONE_SX)
    return SidBoneX;
  if (ctrlId == IDC_SSD_EDIT_BONE_SY)
    return SidBoneY;
  if (ctrlId == IDC_SSD_EDIT_BONE_SZ)
    return SidBoneZ;
  if (ctrlId == IDC_SSD_EDIT_SKIN_SX)
    return SidSkinX;
  if (ctrlId == IDC_SSD_EDIT_SKIN_SY)
    return SidSkinY;
  if (ctrlId == IDC_SSD_EDIT_SKIN_SZ)
    return SidSkinZ;
...
// ***************************************************************************
void CSkeletonScaleDlg::OnSsdButtonUndo() { undo(); }

void CSkeletonScaleDlg::OnSsdButtonRedo() { redo(); }
...
void CSkeletonScaleDlg::OnSsdButtonSaveas() {
  ...
  // choose the file
  CFileDialog fd(FALSE, _T("skel"), nlUtf8ToTStr(_SkeletonFileName),
                 OFN_OVERWRITEPROMPT,
...
bool CSkeletonScaleDlg::saveCurrentInStream(NLMISC::IStream &f) {
  try {
    ...
    (NLMISC::safe_cast<NL3D::CSkeletonShape *>(
         (NL3D::IShape *)_SkeletonModel->Shape))
        ->retrieve(boneBases);
...
// ***************************************************************************
void CSkeletonScaleDlg::pushUndoState(const TBoneMirrorArray &precState,
                                      bool dirtSave) {
  // **** test if real change
...
void CSkeletonScaleDlg::drawSelection() {
      ...
      for (it = skins.begin(); it != skins.end(); it++) {
        ...
        if (skin->getSkinBoneBBox(skinBoneBBox, i)) {
          ...
          if (empty) {
            ...
            boneBBox =
                NLMISC::CAABBox::computeAABBoxUnion(boneBBox, skinBoneBBox);
          }
        }
      }

      // if there is no skin influence on this bone, still display a tiny bbox,
      // to see the scale
  ...
  for (uint i = 0; i < _SkeletonModel->Bones.size(); i++) {
    ...
    // scale with skin scale, because the localskeleton and world matrix do not
    // have this scale
    ...
    matBBox.setRot(NLMISC::CVector::I * bbox.getSize().x,
    ...
    NLMISC::CVector corner = finalMat.getPos() - finalMat.getI() / 2 -
                             finalMat.getJ() / 2 - finalMat.getK() / 2;
    NL3D::IDriver *driver = NL3D::CNELU::Driver;
    driver->setupModelMatrix(NLMISC::CMatrix::Identity);
    NL3D::CDRU::drawWiredBox(corner, finalMat.getI(), finalMat.getJ(),
                             finalMat.getK(), NLMISC::CRGBA::White, *driver);
  }
}
...
void CSkeletonScaleDlg::OnSsdButtonSaveScale() {
  ...
  CFileDialog fd(
      FALSE, _T("scale"), nlUtf8ToTStr(defaultFileName), OFN_OVERWRITEPROMPT,
...
void CSkeletonScaleDlg::OnSsdButtonLoadScale() {
  ...
  CFileDialog fd(TRUE, _T("scale"), nlUtf8ToTStr(defaultFileName), 0,
                 _T("SkelScaleFiles (*.scale)|*.scale|All Files (*.*)|*.*||"),
                 this);

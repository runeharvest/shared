...
CLocatedProperties::CLocatedProperties(CParticleWorkspace::CNode *node,
                                       NL3D::CPSLocated *loc,
                                       CParticleDlg *pdlg)
    : CDialog(CLocatedProperties::IDD, pdlg), _Node(node), _Located(loc),
      _ParticleDlg(pdlg), _MassDialog(NULL), _LifeDialog(NULL) {
...
void CLocatedProperties::init(uint32 x, uint32 y) {
  ...
  HBITMAP bmh =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_LIFE_TIME));
  ...
  if (_Located->getOwner()) {
    _MaxNbParticles->EnableWindow(
        _Located->getOwner()->getAutoCountFlag() ? FALSE : TRUE);
  }
...
void CLocatedProperties::updateTriggerOnDeath(void) {
  ...
  m_EditTriggerOnDeath.EnableWindow(
      ((enable ? true : false /* MSVC6 warning */) &&
...
void CLocatedProperties::OnDisgradeWithLod() {
  UpdateData();
  _Located->forceLODDegradation(
      m_DisgradeWithLOD ? true : false /* to avoid warning from MSVC */);
  touchPSState();
}
...
void CLocatedProperties::OnTriggerOnDeath() {
  UpdateData();
  _Located->enableTriggerOnDeath(m_TriggerOnDeath ? true
                                                  : false /* MSVC6 wraning */);
  updateTriggerOnDeath();
  touchPSState();
}
...
void CLocatedProperties::goPostRender() {
  ...
  GetDlgItem(IDC_CURR_NUM_PARTS)
      ->SetWindowText(nlUtf8ToTStr(NLMISC::toString(_Located->getSize())));
}

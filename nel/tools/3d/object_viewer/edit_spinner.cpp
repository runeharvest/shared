...
CEditSpinner::CEditSpinner(NL3D::CPSBasisSpinner *sf,
                           CParticleWorkspace::CNode *ownerNode, CWnd *pParent,
                           IPopupNotify *pn)
    : CDialog(CEditSpinner::IDD, pParent), _Node(ownerNode), _DirDlg(NULL),
      _NbSamplesDlg(NULL), _Spinner(sf), _PN(pn) {
...
CEditSpinner::~CEditSpinner() {
  if (_DirDlg) {
    _DirDlg->DestroyWindow();
    delete _DirDlg;
  }
  if (_NbSamplesDlg) {
    _NbSamplesDlg->DestroyWindow();
    delete _NbSamplesDlg;
  }
}

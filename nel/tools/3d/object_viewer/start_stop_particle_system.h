...
#if !defined(                                                                  \
    AFX_STAR_STOP_PARTICLE_SYSTEM_H__291E2631_42A1_4598_86AB_D6CE30C64DAF__INCLUDED_)
...
public:
  ...
  CStartStopParticleSystem(CParticleDlg *particleDlg,
                           CAnimationDlg *animationDLG); // standard constructor
  // dtor
  ...
  bool isRunning() const {
    return _State == RunningSingle || _State == RunningMultiple;
  }
...
private:
  ...
  // From CObjectViewer::IMainLoopCallBack : update display of number of
  // particles
  virtual void goPreRender();
  virtual void goPostRender() {}
  NL3D::CParticleSystem *getCurrPS() const {
    return _ActiveNode ? _ActiveNode->getPSPointer() : NULL;
  }
  NL3D::CParticleSystemModel *getCurrPSModel() const {
   ...
   * \return True if the node could be played. May not be the case if the node
   * has loops
   */

...
#if !defined(                                                                  \
    AFX_PARTICLE_DLG_H__AD58E337_952E_4C0D_A6D8_F87AFFEA3A24__INCLUDED_)
...
public:
  ...
  // get the matrix of the current selected element selected, or identity if
  // there's none
  ...
  void stickPSToSkeleton(
      CParticleWorkspace::CNode *node, NL3D::CSkeletonModel *skel, uint bone,
  ...
  bool isPSStickedToSkeleton() const {
    return _ActiveNode != NULL ? _ActiveNode->getParentSkel() != NULL : false;
  }
  ...
  // Check if the current workspace has been modified, and prompt the user to
  // save it if so
  void checkModifiedWorkSpace();
  // Load a new particle workspace (without asking if current workspace has been
  // modified)
...
public:
   ...
   * \param askToContinueWhenError If an error occurs, the user will be asked if
   * loading must continue (if there are other ps to save) \return true if
   * loading should continue for remaining ps, false if loading failed and if
   * user didn't want to continue loading
   */
  bool savePS(HWND parent, CParticleWorkspace::CNode &ps,
              bool askToContinueWhenError);
  bool savePSAs(HWND parent, CParticleWorkspace::CNode &ps,
                const std::string &fullPath, bool askToContinueWhenError);
  // Enum to describe the behaviour of loadPS
  enum TLoadPSBehav {
    Silent,              // loading error are not prompted
    ReportError,         // report error
    ReportErrorSkippable // report error and ask if the user want errors to be
                         // reported again
  };
  /** Try to load a particle system. Prompt msg as necessary for error loading
   * \param behav Behaviour in case of error
   * \return true if the user want see further error (e.g he hasn't checked the
   * 'don't show egain' message box.
   */

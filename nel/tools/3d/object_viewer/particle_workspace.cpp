...
//**************************************************************************************************************************
void CParticleWorkspace::CNode::stickPSToSkeleton(
    NL3D::CSkeletonModel *skel, uint bone, const std::string &parentSkelName,
...
//***********************************************************************************************
void CParticleWorkspace::CNode::removeLocatedBindable(
...
void CParticleWorkspace::CNode::createEmptyPS() {
  ...
  NL3D::CParticleSystemModel *psm =
      NLMISC::safe_cast<NL3D::CParticleSystemModel *>(
          NL3D::CNELU::Scene->createInstance(shapeName));
  nlassert(psm);
  NL3D::CNELU::Scene->setShapeBank(oldSB);
  setup(*psm);
  unload();
...
//***********************************************************************************************
void CParticleWorkspace::CNode::setRelativePath(
...
//***********************************************************************************************
void CParticleWorkspace::CNode::savePS() { savePSAs(getFullPath()); }
...
bool CParticleWorkspace::CNode::loadPS() {
  ...
  NL3D::CParticleSystemModel *psm =
      dynamic_cast<NL3D::CParticleSystemModel *>(trs);
  if (!psm) {
...
//***********************************************************************************************
void CParticleWorkspace::init(CObjectViewer *ov, const std::string &filename,
...
CParticleWorkspace::addNode(const std::string &filenameWithFullPath) {
  nlassert(_OV);
  ...
  for (uint k = 0; k < _Nodes.size(); ++k) {
    if (NLMISC::nlstricmp(_Nodes[k]->getFilename(), fileName) == 0)
      return NULL;
  }
  ...
  if (!PathRelativePathTo(resultPath, nlUtf8ToTStr(dosPath),
                          FILE_ATTRIBUTE_DIRECTORY,
                          nlUtf8ToTStr(filenameWithFullPath), 0)) {
...
bool CParticleWorkspace::containsFile(std::string filename) const {
  for (uint k = 0; k < _Nodes.size(); ++k) {
    if (NLMISC::nlstricmp(filename, _Nodes[k]->getFilename()) == 0)
      return true;
  }
  return false;
}
...
//***********************************************************************************************
CParticleWorkspace::CNode *
CParticleWorkspace::getNodeFromPS(NL3D::CParticleSystem *ps) const {
  for (uint k = 0; k < _Nodes.size(); ++k) {
    if (_Nodes[k]->getPSPointer() == ps)
      return _Nodes[k];
  }
  return NULL;
}
...
void CParticleWorkspace::setModifiedFlag(bool modified) {
  ...
  _Modified = modified;
  if (_ModificationCallback)
    _ModificationCallback->workspaceModifiedFlagChanged(*this);
}
...
public:
  ...
  bool operator()(const NLMISC::CSmartPtr<CParticleWorkspace::CNode> &lhs,
                  const NLMISC::CSmartPtr<CParticleWorkspace::CNode> &rhs) {
      ...
      for (uint l = 0; l < ov->getNumInstance(); ++l) {
        ...
        if (ii->TransformShape && ii->Saved.ShapeFilename == parentSkelName) {
          NL3D::CSkeletonModel *skel =
              dynamic_cast<NL3D::CSkeletonModel *>(ii->TransformShape);
          if (skel) {
            sint boneID = skel->getBoneIdByName(parentBoneName);
            if (boneID != -1) {
              _Nodes[k]->stickPSToSkeleton(skel, (uint)boneID, parentSkelName,
                                           parentBoneName);
              break;
            }
          }

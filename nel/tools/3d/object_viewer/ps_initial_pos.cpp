...
// private : predicate to remove located from a
// CPSInitialPos::TInitialLocatedSizeVect vector
...
// private : predicate to remove located from a
// PSInitialPos::CInitPSInstanceInfo vector
struct CRemoveLocatedFromInitPSInstanceInfoVectPred
...
// private : predicate to remove located from a PSInitialPos::CRotScaleInfo
// vector
...
// private : predicate to remove located bindable pointers in a
// TRotScaleInfoVect vect
...
void CPSInitialPos::removeLocated(NL3D::CPSLocated *loc) {
  // in each container, we delete every element that has a pointer over lthe
  // located loc , by using the dedicated predicate.

  CRemoveLocatedFromLocatedSizePred p;
  p.Loc = loc;
  _InitialSizeVect.erase(
      std::remove_if(_InitialSizeVect.begin(), _InitialSizeVect.end(), p),
      _InitialSizeVect.end());

  CRemoveLocatedFromInitPSInstanceInfoVectPred p2;
  p2.Loc = loc;
  _InitInfoVect.erase(
      std::remove_if(_InitInfoVect.begin(), _InitInfoVect.end(), p2),
      _InitInfoVect.end());

  CRemoveLocatedFromRotScaleInfoVectPred p3;
  p3.Loc = loc;
  _RotScaleInfoVect.erase(
      std::remove_if(_RotScaleInfoVect.begin(), _RotScaleInfoVect.end(), p3),
...
void CPSInitialPos::removeLocatedBindable(NL3D::CPSLocatedBindable *lb) {
  ...
  p.LB = lb;
  _RotScaleInfoVect.erase(
      std::remove_if(_RotScaleInfoVect.begin(), _RotScaleInfoVect.end(), p),
      _RotScaleInfoVect.end());
}
...
void CPSInitialPos::restoreSystem() {
  ...
  for (uint k = 0; k < _PS->getNbProcess(); ++k) {
    NL3D::CPSLocated *loc =
        dynamic_cast<NL3D::CPSLocated *>(_PS->getProcess(k));
    if (loc) {
      while (loc->getSize()) {
        loc->deleteElement(0);
      }
  ...
  for (TInitialLocatedSizeVect ::iterator itSize = _InitialSizeVect.begin();
       itSize != _InitialSizeVect.end(); ++itSize) {
    //	nlassert(itSize->first->getSize() == 0)
    for (uint l = 0; l < itSize->second; ++l) {
      itSize->first->newElement(NLMISC::CVector::Null, NLMISC::CVector::Null,
                                NULL, 0, itSize->first->getMatrixMode(), 0.f);
    }
  ...
  for (TInitInfoVect::iterator it = _InitInfoVect.begin();
       it != _InitInfoVect.end(); ++it) {
  ...
  for (TRotScaleInfoVect::iterator it2 = _RotScaleInfoVect.begin();
       it2 != _RotScaleInfoVect.end(); ++it2) {

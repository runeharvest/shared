...
CSlotDlg::CSlotDlg(CWnd *pParent /*=NULL*/) : CDialog(CSlotDlg::IDD, pParent) {
...
void CSlotDlg::OnPaint() {
  ...
  ScreenToClient(&rect);
  Blend.OnPaint(rect, pDc, StartBlend, EndBlend, (float)StartTime,
                (float)EndTime, Smoothness, StartAnimTime, EndAnimTime,
                !isEmpty());
...
void CSlotDlg::updateScrollBar() {
  ...
  if (fabs(EndAnimTime - StartAnimTime) < 0.00001f) {
    ...
    info.nPage = (int)(10000.f * (AnimationLength / SpeedFactor) /
                       (EndAnimTime - StartAnimTime));
    info.nPos = (int)(10000.f * ((float)Offset - StartAnimTime) /
                      (EndAnimTime - StartAnimTime));
  }
...
void CSlotDlg::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) {
  ...
  if (pScrollBar == GetDlgItem(IDC_SCROLLBAR)) {
    ...
    if (nSBCode == SB_THUMBTRACK) {
      ...
      UpdateData();
      Offset = (int)((EndAnimTime - StartAnimTime) * (float)nPos / 10000.f +
                     StartAnimTime);
...
void CSlotDlg::OnClamp() { refresh(FALSE); }

// ***************************************************************************

void CSlotDlg::OnRepeat() { refresh(FALSE); }

// ***************************************************************************

void CSlotDlg::OnDisable() { refresh(FALSE); }
...
void CSlotDlg::computeLength() {
  if (getAnimationPointer())
    AnimationLength = (getAnimationPointer()->getEndTime() -
                       getAnimationPointer()->getBeginTime()) *
                      MainDlg->getFrameRate();
}
...
const CSkeletonWeight *CSlotDlg::getSkeletonPointer() const {
  ...
  if (instance) {
    ...
    CSlotInfo *slot = getSlotInformation();
    uint animId =
        instance->AnimationSet.getSkeletonWeightIdByName(slot->Skeleton);
    if (animId != CAnimationSet::NotFound) {
...
void CSlotDlg::OnEnable() { refresh(FALSE); }

// ***************************************************************************

void CSlotDlg::OnInvertSkeletonWeight() { refresh(FALSE); }

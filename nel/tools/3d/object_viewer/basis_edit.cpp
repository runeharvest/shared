...
NLMISC::CMatrix BuildEulerMatrix(float psi, float theta, float phi) {
  float ca = cosf(psi), sa = sinf(psi), cb = cosf(theta), sb = sinf(theta),
        cc = cosf(phi), sc = sinf(phi);
  NLMISC::CMatrix m;
  m.identity();
  m.setRot(
      NLMISC::CVector(ca * cb * cc - sa * sc, -cc * sa - ca * cb * sc, ca * sb),
...
NLMISC::CVector GetEulerAngles(const NLMISC::CMatrix &mat) {
  ...
  NLMISC::CVector sol[] = {
      NLMISC::CVector(b0, a0, c0), NLMISC::CVector(b0, a2, c0),
      NLMISC::CVector(b0, a0, c2), NLMISC::CVector(b0, a2, c2),
      NLMISC::CVector(b1, a1, c1), NLMISC::CVector(b1, a3, c1),
      NLMISC::CVector(b1, a1, c3), NLMISC::CVector(b1, a3, c3)};
  ...
  for (uint k = 0; k < 8; ++k) {
    float ca = cosf(sol[k].x), sa = sinf(sol[k].x), cb = cosf(sol[k].y),
          sb = sinf(sol[k].y), cc = cosf(sol[k].z), sc = sinf(sol[k].z);
...
void CBasisEdit::updateAnglesFromReader() {
  ...
  m_ThetaCtrl.SetScrollPos(
      (uint)(360.f * angles.y / (2.f * (float)NLMISC::Pi)));
...
static CPoint BasisVectXForm(NLMISC::CVector &v, float size) {
  ...
  return CPoint((int)(size * (sq * (v.x + v.y))),
                (int)(size * (-v.z + sq * (v.x - v.y))));
}
...
void CBasisEdit::OnPaint() {
  ...
  NLMISC::CRGBA c1[] = {NLMISC::CRGBA::White, NLMISC::CRGBA::White,
                        NLMISC::CRGBA::White};
...
void CBasisEdit::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar *pScrollBar) {
  ...
  if (nSBCode == SB_THUMBPOSITION || nSBCode == SB_THUMBTRACK) {
    NLMISC::CVector angles(
        2.f * (float)NLMISC::Pi * m_PsiCtrl.GetScrollPos() / 360.f,

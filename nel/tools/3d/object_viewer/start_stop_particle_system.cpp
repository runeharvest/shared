...
CStartStopParticleSystem::CStartStopParticleSystem(CParticleDlg *particleDlg,
    ...
    : CDialog(CStartStopParticleSystem::IDD, particleDlg),
      _ParticleDlg(particleDlg), _State(Stopped), _LastCurrNumParticles(-1),
      _LastMaxNumParticles(-1), _LastNumWantedFaces(-1), _LastSystemDate(-1.f),
      _AutoRepeat(false), _AnimationDLG(animationDLG), _LastSceneAnimFrame(0.f),
      _ActiveNode(NULL) {
...
BOOL CStartStopParticleSystem::OnInitDialog() {
  ...
  HBITMAP bm[4];
  bm[0] =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_START_SYSTEM));
  bm[1] =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_STOP_SYSTEM));
  bm[2] =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_PAUSE_SYSTEM));
  bm[3] = LoadBitmap(::AfxGetInstanceHandle(),
                     MAKEINTRESOURCE(IDB_START_MULTIPLE_SYSTEM));
...
//******************************************************************************************************
void CStartStopParticleSystem::OnStartSystem() { start(); }

//******************************************************************************************************
void CStartStopParticleSystem::OnStartMultipleSystem() { startMultiple(); }

//******************************************************************************************************
void CStartStopParticleSystem::OnStopSystem() { stop(); }

//******************************************************************************************************
void CStartStopParticleSystem::OnPause() { pause(); }
...
void CStartStopParticleSystem::updateUIFromState() {
  ...
  if (!getCurrPS()) {
    ...
    if (!_ActiveNode->getParentSkelName().empty()) {
      GetDlgItem(IDC_STICK_BONE)
          ->SetWindowText(nlUtf8ToTStr(_ActiveNode->getParentBoneName() + "." +
                                       _ActiveNode->getParentBoneName()));
    } else {
      GetDlgItem(IDC_STICK_BONE)->SetWindowText(_T(""));
    }
    GetDlgItem(IDC_ACTIVE_PS)
        ->SetWindowText(nlUtf8ToTStr(_ActiveNode->getFilename()));
    GetDlgItem(IDC_ENABLE_AUTO_COUNT)->EnableWindow(TRUE);
    ((CButton *)GetDlgItem(IDC_ENABLE_AUTO_COUNT))
        ->SetCheck(getCurrPS()->getAutoCountFlag() ? 1 : 0);
    GetDlgItem(IDC_RESET_COUNT)
        ->EnableWindow((_ActiveNode->getPSPointer()->getAutoCountFlag() &&
                        !_ActiveNode->getResetAutoCountFlag())
                           ? TRUE
                           : FALSE);
    ...
    GetDlgItem(IDC_CLEAR_ANIM)
        ->EnableWindow(!_ActiveNode->getTriggerAnim().empty());
...
void CStartStopParticleSystem::start() {
  ...
  updateUIFromState();
  if (m_LinkPlayToScenePlay) // is scene animation subordinated to the fx
                             // animation
  {
...
//******************************************************************************************************
void CStartStopParticleSystem::OnReleasedcaptureAnimSpeed(NMHDR *pNMHDR,
                                                          LRESULT *pResult) {
  UpdateData();
  CSliderCtrl *sl = (CSliderCtrl *)GetDlgItem(IDC_ANIM_SPEED);
...
void CStartStopParticleSystem::OnEnableAutoCount() {
  ...
  stop();
  bool autoCount =
      ((CButton *)GetDlgItem(IDC_ENABLE_AUTO_COUNT))->GetCheck() != 0;
  if (autoCount) {
    CString caption;
    CString mess;
...
//******************************************************************************************************
void CStartStopParticleSystem::resetAutoCount(CParticleWorkspace::CNode *node,
                                              bool reset /* = true */) {
  if (!node)
    return;
  if (node->getResetAutoCountFlag() == reset)
    return;
  node->setResetAutoCountFlag(reset);
  if (node == _ActiveNode) {
    GetDlgItem(IDC_RESET_COUNT)
        ->EnableWindow(
            (_ActiveNode->getPSPointer()->getAutoCountFlag() && !reset)
                ? TRUE
                : FALSE);
  }
...

{
  if (!_ActiveNode)
    return;
  if (enable == _ActiveNode->getPSPointer()->getAutoCountFlag())
    return;
  nlassert(getCurrPS());
  ...
  CLocatedProperties *lp =
      dynamic_cast<CLocatedProperties *>(_ParticleDlg->getRightPane());
  if (lp) {
...
void CStartStopParticleSystem::goPreRender() {
                             ...
                             // animation
  {
    ...
    if (_AnimationDLG && isRunning()) {
      if (_LastSceneAnimFrame >
          _AnimationDLG->CurrentFrame) // did animation restart ?
      {
        restartAllFX();
      }
      _LastSceneAnimFrame = _AnimationDLG->CurrentFrame;
    }
  } else if (_AutoRepeat && !m_LinkPlayToScenePlay) // auto repeat feature
  {
    if (isRunning()) {
      ...
      for (uint k = 0; k < _PlayingNodes.size(); ++k) {
        if (_PlayingNodes[k]) {
          if (isRunning(_PlayingNodes[k])) {
            fxStarted = true;
            if (_PlayingNodes[k]->getPSPointer()->getSystemDate() <=
                _PlayingNodes[k]->getPSPointer()->evalDuration()) {
              allFXFinished = false;
              break;
            } else {
              if (_PlayingNodes[k]->getPSPointer()->getCurrNumParticles() !=
                  0) {
                allFXFinished = false;
                break;
              }
  ...
  if (_State == RunningMultiple || _State == PausedMultiple) {
    ...
    for (uint k = 0; k < _PlayingNodes.size(); ++k) {
      if (_PlayingNodes[k]) {
        if (!isRunning(_PlayingNodes[k]) ||
            !_PlayingNodes[k]->getPSModel()->hasActiveEmitters()) {
          // see if chosen anim is currently running
          if (_PlayingNodes[k]->getTriggerAnim().empty() ||
              currAnims.count(_PlayingNodes[k]->getTriggerAnim())) {
            ...
            // yes -> trigger the fx
            play(*_PlayingNodes[k]);
          } else if (!_PlayingNodes[k]
                          ->getPSPointer()
                          ->hasParticles()) // fx is being shut down, stop it
                                            // when necessary
          {
          ...
          if (!_PlayingNodes[k]->getTriggerAnim().empty()) {
            if (_PlayingNodes[k]->getPSModel()->hasActiveEmitters()) {
              // see if anim if already playing. If this is not the case then
              // shutdown the emitters
  ...
  if (_ActiveNode) {
    ...
    if (currNumParticles != _LastCurrNumParticles ||
        maxNumParticles != _LastMaxNumParticles) {
      CString numParts;
      numParts.LoadString(IDS_NUM_PARTICLES);
      numParts += CString(NLMISC::toString("%d / %d", (int)currNumParticles,
                                           (int)maxNumParticles)
                              .c_str());
    ...
    sint numWantedFaces = (uint)ps->getWantedNumTris(
        (ps->getSysMat().getPos() - camMat.getPos()).norm());
      ...
      for (uint k = 0; k < pws->getNumNode(); ++k) {
        if (pws->getNode(k)->isLoaded()) {
          if (pws->getNode(k) == _ActiveNode) {
            pws->getNode(k)->getPSModel()->enableDisplayTools(
                !isRunning(pws->getNode(k)) || m_DisplayHelpers);
          } else {
...
void CStartStopParticleSystem::OnLinkPlayToScenePlay() {
  ...
  //  When this method is called, the 'play' button of the scene is controlled
  //  by the 'play' button of the particle editor and thus is not accessible.
...
void CStartStopParticleSystem::OnLinkToSkeleton() {
  ...
  if (ov->chooseBone(NLMISC::tStrToUtf8(chooseBoneForPS), skel, boneIndex,
                     &parentSkelName, &parentBoneName)) {
...
//******************************************************************************************************
void CStartStopParticleSystem::setActiveNode(
    CParticleWorkspace::CNode *activeNode) {
...
//******************************************************************************************************
void CStartStopParticleSystem::forceActiveNode(
    CParticleWorkspace::CNode *activeNode) {
  UpdateData();
...
bool CStartStopParticleSystem::checkHasLoop(CParticleWorkspace::CNode &node) {
  ...
  if (!node.getPSPointer()->hasLoop())
    return false;
  MessageBox(CString(node.getFilename().c_str()) + " : " +
                 getStrRsc(IDS_FX_HAS_LOOP),
...
void CStartStopParticleSystem::OnBrowseAnim() {
  ...
  */
  ...
  std::vector<std::string> animList(animSet.begin(), animSet.end());
  CSelectString st(animList,
                   NLMISC::tStrToUtf8(getStrRsc(IDS_SELECT_ANIMATION)), this,
                   false);
...
void CStartStopParticleSystem::OnRestickAll() {
  if (_ParticleDlg->getParticleWorkspace()) {
    _ParticleDlg->getParticleWorkspace()->restickAllObjects(
        _ParticleDlg->getObjectViewer());
  }
}

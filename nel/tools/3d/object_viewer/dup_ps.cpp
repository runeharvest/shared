...
/** This can duplicate any serializable type by using a serialization policy
 * (polymorphic, non polymorphic ..) The serialization policy must have a method
 * to serial a pointer on the object (see example below) NB : of course this is
 * slow (but convenient) way of performing a copy
...
struct CDupObjPolicy {
  template <typename T> static void serial(T *&obj, NLMISC::IStream &dest) {
    dest.serialPtr(obj);
    /*if (dest.isReading())
    {
...
NL3D::CParticleSystemProcess *DupPSLocated(const CParticleSystemProcess *in) {
  ...
  try {
    ...
    if (in->getOwner() == NULL) {
       ...
       * We can't duplicate the object direclty (it may be referencing other
       * objects in the system, so these objects will be copied too...)
       */
      CUniquePtr<CParticleSystem> newPS(
          DupSerializable<CDupObjPolicy>(in->getOwner()));
      // scene pointer is not serialised, but 'detach' may need the scene to be
      // specified
...
NL3D::CPSLocatedBindable *DupPSLocatedBindable(CPSLocatedBindable *in) {
  ...
  try {
    ...
    if (in->getOwner() == NULL) {
      ...
      newPS->setScene(
          in->getOwner()
              ->getScene()); // 'unbind' require the scene to be attached
      CPSLocated *loc =
          NLMISC::safe_cast<CPSLocated *>(newPS->getProcess(index));

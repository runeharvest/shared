...
/// This class implement bound checking for values. Can be used in an edition
/// dialog
template <class T> class CBoundChecker {
public:
  /** ctor
  ...
  bool isUpperBoundExcluded(void) const { return _UpperBoundExcluded; }
  ...
  bool isLowerBoundExcluded(void) const { return _LowerBoundExcluded; }
  ...
  const TCHAR *validateUpperBound(T v) {
    if (!_UpperBoundEnabled)
      return NULL;
    if (_UpperBoundExcluded && v < _UpperBound)
      return NULL;
    if (!_UpperBoundExcluded && v <= _UpperBound)
      return NULL;
  ...
  const TCHAR *validateLowerBound(T v) {
    if (!_LowerBoundEnabled)
      return NULL;
    if (_LowerBoundExcluded && v > _LowerBound)
      return NULL;
    if (!_LowerBoundExcluded && v >= _LowerBound)
      return NULL;

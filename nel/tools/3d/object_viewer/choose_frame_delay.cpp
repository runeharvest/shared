...
uint32 CChooseFrameDelay::CCFDWrapper::get(void) const {
  return OV->getFrameDelay();
}
void CChooseFrameDelay::CCFDWrapper::set(const uint32 &value) {
  OV->setFrameDelay(value);
}
...
void CChooseFrameDelay::goPreRender() {
  ...
  if (_LockToPS && ov->getParticleDialog()->getActiveNode()) {
    CParticleWorkspace::CNode *activeNode =
        ov->getParticleDialog()->getActiveNode();

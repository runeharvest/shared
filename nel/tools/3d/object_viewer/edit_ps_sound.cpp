...
CEditPSSound::CEditPSSound(CParticleWorkspace::CNode *ownerNode,
                           NL3D::CPSSound *sound)
    : _Node(ownerNode), _Sound(sound), _GainDlg(NULL), _PitchDlg(NULL),
      _PercentDlg(NULL) {
  nlassert(sound);
}
...
void CEditPSSound::init(CWnd *pParent /*= NULL*/) {
  ...
  _PercentDlg = new CEditableRangeFloat(std::string("SOUND_EMISSION_PERCENT"),
                                        _Node, 0, 1);
  ...
  HBITMAP bmh =
      LoadBitmap(::AfxGetInstanceHandle(), MAKEINTRESOURCE(IDB_SOUND_VOLUME));
...
void CEditPSSound::OnChangeSoundName() {
  ...
  UpdateData();
  _Sound->setSoundName(
      NLMISC::CStringMapper::map(NLMISC::tStrToUtf8(m_SoundName)));
  updateModifiedFlag();
}
...
void CEditPSSound::OnKeepOriginalPitch() {
  ...
  _Sound->setUseOriginalPitchFlag(
      m_KeepOriginalPitch ? true : false /* to avoid VCC warning*/);
  nlassert(_PitchDlg);

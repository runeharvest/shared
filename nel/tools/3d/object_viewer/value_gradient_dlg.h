...
#if !defined(                                                                  \
    AFX_VALUE_GRADIENT_DLG_H__45A21D97_D65B_494E_B171_D53F71F4AFC7__INCLUDED_)
...
struct IValueGradientDlgClient {
   ...
   *  ONLY ONE DIALOG WILL BE QUERRIED AT A TIME. the previous dialog is
   * detroyed before this is called again \param index the index of the value in
   * the dialog \grad the dlg that called this method (deriver can ask a redraw
   * then)
   */
  virtual CEditAttribDlg *
  createDialog(uint index, CValueGradientDlg *grad,
               CParticleWorkspace::CNode *ownerNode) = 0;
...
public:
  /** construct the dialog. The user must provides an interface of type
   * IValueGradientDlgClient and a pointer to the parent window \param
   * destroyClientInterface When set to true, dlete will be called on the client
   * interface when this obj dtor is called. \param canTuneNbStage The gradient
   * is sampled, and the number of intermediate values can be tuned. When this
   * is set to false, this is disabled. \param minSize The minimum number of
   * elements that the gradient must have.
   */

  CValueGradientDlg(IValueGradientDlgClient *clientInterface,
                    CParticleWorkspace::CNode *ownerNode,
                    bool destroyClientInterface, CWnd *pParent,
                    IPopupNotify *pn, bool canTuneNbStages = true,
                    uint minSize = 2);
...
protected:
  ...
  uint _MinSize;
  // false to disable the dialog that control the number of stages between each
  // value

...
public:
   ...
   */
  CAttribDlg(const std::string &valueID, CParticleWorkspace::CNode *ownerNode,
             bool enableConstantValue = true); // standard constructor
...
public:
  ...
  /** Disable the possibility to choose a scheme that has memory. (for example,
   * a scheme for lifetime of a located has no sense because located have
   * already some memory to store it)
   */
...
public:
  // private usage (not private because accessed by a static function) : return
  // the nbCycles parameter of the scheme (e.g the input multiplier).
  virtual float getSchemeNbCycles(void) const = 0;

  // private usage (not private because accessed by a static function) : set the
  // nbCycles parameter of the scheme (e.g the input multiplier)
...
protected:
  ...
  // get the current scheme, -1 if the scheme is unknow (created outside the
  // editor ?)
  ...
  // true when created, it is set to false once a constant, or scheme dialog has
  // bee shown
  bool _FirstDrawing;
...
/**
 * a template class that helps to specialize the attrib maker edition dialog
 * with various types
 */
template <typename T> class CAttribDlgT : public CAttribDlg {
public:
  CAttribDlgT(const std::string &valueID, CParticleWorkspace::CNode *node)
      : CAttribDlg(valueID, node), _Wrapper(NULL), _SchemeWrapper(NULL) {}
  ...
  void setSchemeWrapper(IPSSchemeWrapper<T> *schemeWrapper) {
    ...
    _SchemeWrapper->OwnerNode = _Node;
  }
  ...
  virtual void setCurrentSchemePtr(NL3D::CPSAttribMakerBase *s) {
    _SchemeWrapper->setSchemeAndUpdateModifiedFlag(
        NLMISC::safe_cast<NL3D::CPSAttribMaker<T> *>(s));
  }
...
public:
   ...
   *  \param minValue : the min value for the editable range dlg(for constant
   * value) \param maxValue : the min value for the editable range dlg (for
   * constant value)
   */
  CAttribDlgFloat(const std::string &valueID, CParticleWorkspace::CNode *node,
...
public:
   ...
   * constant value)
   */
  CAttribDlgUInt(const std::string &valueID, CParticleWorkspace::CNode *node,
                 uint32 minValue = 0, uint32 maxValue = 10);
...
public:
   ...
   * constant value)
   */
  CAttribDlgInt(const std::string &valueID, CParticleWorkspace::CNode *node,
                sint32 minValue = 0, sint32 maxValue = 10);

...
CTextureAnimDlg::CTextureAnimDlg(CParticleWorkspace::CNode *ownerNode,
    ...
    : _Node(ownerNode), _EditedParticle(p), _TextureChooser(NULL),
      _TextureIndexDialog(NULL), _MTP(mtp), _MultiTexDlg(NULL) {
  nlassert(p);
...
BOOL CTextureAnimDlg::EnableWindow(BOOL bEnable) {
  if (_TextureChooser)
    _TextureChooser->EnableWindow(bEnable);
  ...
  if (_MTP) {
    ...
    GetDlgItem(IDC_EDIT_MULTITEX)
        ->EnableWindow(bEnable & (_MTP->isMultiTextureEnabled() ? 1 : 0));
  }
...
void CTextureAnimDlg::setupCtrl(void) {
  ...
  if (_EditedParticle->getTextureGroup()) {
    if (!_TextureIndexDialog) {
      ...
      _TextureIndexDialog = new CAttribDlgInt(
          "TEXTURE_INDEX", _Node, 0,
          _EditedParticle->getTextureGroup()->getNbTextures() - 1);
      ...
      HBITMAP bmh = LoadBitmap(::AfxGetInstanceHandle(),
                               MAKEINTRESOURCE(IDB_ANIM_SEQUENCE));
...
void CTextureAnimDlg::OnEnableTextureAnim() {
  ...
  if (!m_EnableTextureAnim) {
    ...
    NLMISC::CSmartPtr<NL3D::ITexture> tex =
        (NL3D::ITexture *)new NL3D::CTextureFile(std::string(""));
...
CEditAttribDlg *CTextureAnimDlg::CGradientInterface::createDialog(
    uint index, CValueGradientDlg *grad, CParticleWorkspace::CNode *ownerNode) {
...
void CTextureAnimDlg::CGradientInterface::modifyGradient(TAction action,
                                                         uint index) {
  nlassert(P);
  ...
  case IValueGradientDlgClient::Add: {
    ...
    NLMISC::CSmartPtr<NL3D::ITexture> lastTex =
        textureList[textureList.size() - 1];
...
void CTextureAnimDlg::CGradientInterface::displayValue(CDC *dc, uint index,
                                                       sint x, sint y) {
  const uint tSize = 20;
  NLMISC::CSmartPtr<NL3D::ITexture> tex =
      P->getTextureGroup()->getTexture(index);
  tex->generate();
...
uint32 CTextureAnimDlg::CGradientInterface::getNbSteps(void) const { return 1; }
...
void CTextureAnimDlg::CGradientInterface::CTextureWrapper::set(
    NL3D::ITexture *t) {
  nlassert(P);
  nlassert(P->getTextureGroup());
  ...
  if (P->getTextureGroup()->getNbTextures() > 1) {
    ...
    if (t->getWidth() != tex->getWidth() ||
        t->getHeight() != tex->getHeight()) {
      ::MessageBox(NULL, _T("All textures must have the same size !"),
                   _T("error"), MB_OK);
      return;
    }

    if (t->PixelFormat != tex->PixelFormat) {
      ::MessageBox(NULL, _T("All textures must have the same pixel format !"),
                   _T("error"), MB_OK);
      return;
    }
  }

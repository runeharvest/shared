...
//=====================================================
CParticleSystemEdit::CParticleSystemEdit(CParticleWorkspace::CNode *ownerNode,
                                         CParticleTreeCtrl *ptc)
    : _Node(ownerNode), _TimeThresholdDlg(NULL), _MaxIntegrationStepDlg(NULL),
      _MaxViewDistDlg(NULL), _LODRatioDlg(NULL), _AutoLODDlg(NULL),
      _GlobalColorDlg(NULL), _ParticleTreeCtrl(ptc) {
  nlassert(ptc);
...
void CParticleSystemEdit::init(CWnd *pParent) // standard constructor
{
  ...
  _MaxViewDistDlg =
      new CEditableRangeFloat(std::string("MAX VIEW DIST"), _Node, 0, 100.f);
  ...
  _LODRatioDlg =
      new CEditableRangeFloat(std::string("LOD RATIO"), _Node, 0, 1.f);
  ...
  _TimeThresholdDlg = new CEditableRangeFloat(std::string("TIME THRESHOLD"),
                                              _Node, 0.005f, 0.3f);
  ...
  _MaxIntegrationStepDlg =
      new CEditableRangeUInt(std::string("MAX INTEGRATION STEPS"), _Node, 0, 4);
  ...
  for (uint k = 0; k < 4; ++k) {
    ...
    CEditableRangeFloat *erf = new CEditableRangeFloat(
        std::string("USER PARAM") + (char)(k + 65), NULL, 0, 1.0f);
  ...
  m_PrecomputeBBoxCtrl.SetCheck(
      _Node->getPSPointer()->getAutoComputeBBox() ? 0 : 1);
  ((CButton *)GetDlgItem(IDC_AUTO_BBOX))
      ->SetCheck(_ParticleTreeCtrl->getParticleDlg()->getAutoBBox() ? 1 : 0);

  m_EnableSlowDown = csd;
  ((CButton *)GetDlgItem(IDC_SHARABLE))
      ->SetCheck(_Node->getPSPointer()->isSharingEnabled());
  ...
  /// global color
  int bGlobalColor =
      _Node->getPSPointer()->getColorAttenuationScheme() != NULL ? 1 : 0;
  ...
  _MaxIntegrationStepDlg->EnableWindow(
      _Node->getPSPointer()->getBypassMaxNumIntegrationSteps() ? FALSE : TRUE);
...
void CParticleSystemEdit::updateDieOnEventParams() {
  BOOL ew = _Node->getPSPointer()->getDestroyCondition() ==
                    NL3D::CParticleSystem::none
                ? FALSE
                : TRUE;
  GetDlgItem(IDC_AUTO_DELAY)->EnableWindow(ew);
  bool autoDelay =
      _Node->getPSPointer()->getAutoComputeDelayBeforeDeathConditionTest();
  if (autoDelay) {
    ew = FALSE;
  }
...
void CParticleSystemEdit::OnPrecomputeBbox() {
  UpdateData();
  _Node->getPSPointer()->setAutoComputeBBox(
      !_Node->getPSPointer()->getAutoComputeBBox() ? true : false);
...
void CParticleSystemEdit::updatePrecomputedBBoxParams() {
  ...
  BOOL ew = !_ParticleTreeCtrl->getParticleDlg()->getAutoBBox() &&
            !_Node->getPSPointer()->getAutoComputeBBox();

  m_BBoxXCtrl.EnableWindow(ew);
  m_BBoxYCtrl.EnableWindow(ew);
  m_BBoxZCtrl.EnableWindow(ew);
  GetDlgItem(IDC_DEC_BBOX)
      ->EnableWindow(!_Node->getPSPointer()->getAutoComputeBBox());
  GetDlgItem(IDC_INC_BBOX)
      ->EnableWindow(!_Node->getPSPointer()->getAutoComputeBBox());
  ...
  if (!ew) {
    ...
    char out[128];
    sprintf(out, "%.3g", b.getHalfSize().x);
    m_BBoxX = out;
    sprintf(out, "%.3g", b.getHalfSize().y);
    m_BBoxY = out;
    sprintf(out, "%.3g", b.getHalfSize().z);
    m_BBoxZ = out;
  }

  GetDlgItem(IDC_RESET_BBOX)
      ->EnableWindow(_ParticleTreeCtrl->getParticleDlg()->getAutoBBox() &&
                     !_Node->getPSPointer()->getAutoComputeBBox());
...
void CParticleSystemEdit::OnSelchangePsDieOnEvent() {
  UpdateData();
  _Node->getPSPointer()->setDestroyCondition(
      (NL3D::CParticleSystem::TDieCondition)m_DieOnEvent.GetCurSel());
...
void CParticleSystemEdit::OnAccurateIntegration() {
  UpdateData();
  _Node->getPSPointer()->enableAccurateIntegration(
      m_AccurateIntegration ? true : false);
  updateIntegrationParams();
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnEnableSlowDown() {
  ...
  bool klt;
  _Node->getPSPointer()->getAccurateIntegrationParams(t, max, csd, klt);
  _Node->getPSPointer()->setAccurateIntegrationParams(
      t, max, m_EnableSlowDown ? true : false, klt);
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnDieWhenOutOfRange() {
  UpdateData();
  _Node->getPSPointer()->setDestroyModelWhenOutOfRange(
      m_DieWhenOutOfRange ? true : false);
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnDieWhenOutOfFrustum() {
  UpdateData();
  _Node->getPSPointer()->destroyWhenOutOfFrustum(m_DieWhenOutOfFrustum ? true
                                                                       : false);
...
void CParticleSystemEdit::OnChangeApplyAfterDelay() {
  if (_Node->getPSPointer()->getAutoComputeDelayBeforeDeathConditionTest())
    return;
...
void CParticleSystemEdit::OnSelchangeLifeMgtPresets() {
  ...
  _Node->getPSPointer()->activatePresetBehaviour(
      (NL3D::CParticleSystem::TPresetBehaviour)m_PresetCtrl.GetCurSel());
  updateLifeMgtPresets();
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnSelchangeAnimTypeCtrl() {
  UpdateData(TRUE);
  _Node->getPSPointer()->setAnimType(
      (NL3D::CParticleSystem::TAnimType)m_AnimTypeCtrl.GetCurSel());
  updateModifiedFlag();
}
...
void CParticleSystemEdit::updateLifeMgtPresets() {
  ...
  m_BypassMaxNumSteps =
      _Node->getPSPointer()->getBypassMaxNumIntegrationSteps();
  ...
  BOOL bEnable = _Node->getPSPointer()->getBehaviourType() ==
                         NL3D::CParticleSystem::UserBehaviour
                     ? TRUE
                     : FALSE;
...
void CParticleSystemEdit::OnEnableAutoLod() {
  ...
  _Node->getPSPointer()->enableAutoLOD(
      bEnable ? true : false /* performance warning */);
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnGlobalColor() {
  ...
  if (_Node->getPSPointer()->getColorAttenuationScheme() == NULL) {
    static const NLMISC::CRGBA grad[] = {NLMISC::CRGBA::White,
                                         NLMISC::CRGBA::Black};
    _Node->getPSPointer()->setColorAttenuationScheme(
        new NL3D::CPSColorGradient(grad, 2, 64, 1.f));
    GetDlgItem(IDC_EDIT_GLOBAL_COLOR)->EnableWindow(TRUE);
  } else {
...
//=====================================================
float CUserParamWrapper::get(void) const { return PS->getUserParam(Index); }

//=====================================================
void CUserParamWrapper::set(const float &v) { PS->setUserParam(Index, v); }

//=====================================================
float CMaxViewDistWrapper::get(void) const { return PS->getMaxViewDist(); }

//=====================================================
void CMaxViewDistWrapper::set(const float &d) { PS->setMaxViewDist(d); }

//=====================================================
float CLODRatioWrapper::get(void) const { return PS->getLODRatio(); }

//=====================================================
void CLODRatioWrapper::set(const float &v) { PS->setLODRatio(v); }
...
void CParticleSystemEdit::OnEnableLoadBalancing() {
  ...
  if (m_EnableLoadBalancing == FALSE) {
    int result = MessageBox(_T("Are you sure ?"), _T("Load balancing on/off"),
                            MB_OKCANCEL);
    if (result == IDOK) {
...
//=====================================================
static void chooseGlobalUserParam(uint userParam, NL3D::CParticleSystem *ps,
                                  CWnd *parent) {
  nlassert(ps);
  CChooseName cn(!ps->getGlobalValueName(userParam).empty()
                     ? ps->getGlobalValueName(userParam).c_str()
                     : "",
                 parent);
  if (cn.DoModal() == IDOK) {
...
void CParticleSystemEdit::OnBypassMaxNumSteps() {
  ...
  if (m_BypassMaxNumSteps && !_Node->getPSPointer()->canFinish()) {
    MessageBox(_T("The system must have a finite duration for this setting! ")
               _T("Please check that."),
               _T("error"), MB_ICONEXCLAMATION);
    return;
  }
  _Node->getPSPointer()->setBypassMaxNumIntegrationSteps(m_BypassMaxNumSteps !=
                                                         FALSE);
  _MaxIntegrationStepDlg->EnableWindow(
      _Node->getPSPointer()->getBypassMaxNumIntegrationSteps() ? FALSE : TRUE);
  updateModifiedFlag();
...
void CParticleSystemEdit::OnAutoBbox() {
  nlassert(_ParticleTreeCtrl);
  _ParticleTreeCtrl->getParticleDlg()->setAutoBBox(
      !_ParticleTreeCtrl->getParticleDlg()->getAutoBBox());
  updatePrecomputedBBoxParams();
  updateModifiedFlag();
}
...
void CParticleSystemEdit::OnLockFrameDelay() {
  bool locked = !_ParticleTreeCtrl->getParticleDlg()
                     ->getObjectViewer()
                     ->getFrameDelayDlg()
                     ->isLockedToPS();
  _ParticleTreeCtrl->getParticleDlg()
      ->getObjectViewer()
      ->getFrameDelayDlg()
      ->lockToPS(locked);
  CString buttonStr;

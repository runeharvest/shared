...
CRibbonDlg::CRibbonDlg(CParticleWorkspace::CNode *ownerNode,
                       NL3D::CPSRibbonBase *ribbon, CWnd *pParent /* = NULL*/)
    : CDialog(CRibbonDlg::IDD, pParent), _Node(ownerNode), _Ribbon(ribbon),
      _RibbonLengthDlg(NULL) {
  nlassert(ribbon);
  //{{AFX_DATA_INIT(CRibbonDlg)
  m_UseHermitteInterpolation =
      _Ribbon->getInterpolationMode() == NL3D::CPSRibbonBase::Hermitte;
...
void CRibbonDlg::OnConstantLength() {
  UpdateData();
  _Ribbon->setRibbonMode(m_ConstantLength ? NL3D::CPSRibbonBase::FixedSize
...
BOOL CRibbonDlg::OnInitDialog() {
  ...
  CEditableRangeFloat *erf =
      new CEditableRangeFloat("RIBBON_LENGTH", _Node, 0.1f, 10.1f);
  ...
  // Coord system
  ((CComboBox *)GetDlgItem(IDC_TRAIL_COORD_SYSTEM))
      ->SetCurSel((int)_Ribbon->getMatrixMode());
...
void CRibbonDlg::updateState() {
  _RibbonLengthDlg->EnableWindow(_Ribbon->getRibbonMode() ==
...
void CRibbonDlg::OnSelchangeTrailCoordSystem() {
  _Ribbon->setMatrixMode((NL3D::CPSRibbonBase::TMatrixMode)(
                             (CComboBox *)GetDlgItem(IDC_TRAIL_COORD_SYSTEM))
                             ->GetCurSel());

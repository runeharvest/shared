...
CVegetableDlg::CVegetableDlg(CObjectViewer *viewer, CWnd *pParent /*=NULL*/)
    : CDialog(CVegetableDlg::IDD, pParent), _ObjView(viewer),
      _PropertySheet(NULL), _VegetableDensityPage(NULL),
      _VegetableApperancePage(NULL), _VegetableScalePage(NULL),
      _VegetableRotatePage(NULL), _VegetableWindDlg(NULL) {
  nlassert(viewer);
...
void CVegetableDlg::doRefreshVegetableDisplay() {
  ...
  // first build the vegetSet, but don't keep <default> shapeName. and skip
  // Hiden vegetables too
...
// ***************************************************************************
void CVegetableDlg::CVegetableDesc::initVegetable(
    const NL3D::CVegetable &vegetable) {
...
void CVegetableDlg::CVegetableDesc::updateVegetableName() {
  ...
  if (Vegetable->ShapeName.empty()) {
    ...
    VegetableName += str;
    // NB: if you add info with other parameters, you must use
    // updateCurSelVegetableName() if they change
  }
}
...
// ***************************************************************************
bool CVegetableDlg::loadVegetableSet(NL3D::CTileVegetableDesc &vegetSet,
                                     const TCHAR *title) {
  vegetSet.clear();
  bool ok = false;
...
// ***************************************************************************
void CVegetableDlg::buildVegetableSet(NL3D::CTileVegetableDesc &vegetSet,
...
void CVegetableDlg::appendVegetableSet(NL3D::CTileVegetableDesc &vegetSet) {
  ...
  for (uint distType = 0; distType < NL3D_VEGETABLE_BLOCK_NUMDIST; distType++) {
    ...
    const std::vector<NL3D::CVegetable> &vegetList =
        vegetSet.getVegetableList(distType);
...
void CVegetableDlg::OnButtonVegetableClear() {
  ...
  if (MessageBox(_T("Clear all the list?"), _T("Clear List"),
...
void CVegetableDlg::OnButtonVegetableSaveDesc() {
  ...
  if (id != LB_ERR) {
    ...
    CFileDialog fd(
        FALSE, _T("vegetdesc"), nlUtf8ToTStr(fileName), OFN_OVERWRITEPROMPT,
        _T("VegetDescFiles (*.vegetdesc)|*.vegetdesc|All Files (*.*)|*.*||"),
        this);
...
void CVegetableDlg::OnButtonVegetableSaveSet() {
  ...
  // Then try to save it.
  CFileDialog fd(
      FALSE, _T("vegetset"), nlUtf8ToTStr(_LastVegetSetName),
      OFN_OVERWRITEPROMPT,
        ...
        if (!dlg.SubsetCopy) {
          ...
          if (dlg.AngleSetup) {
            ...
            case NL3D::CVegetable::AngleWall:
              vegetDst.setAngleWall(vegetSrc.getCosAngleMin(),
                                    vegetSrc.getCosAngleMax());
              break;
            }
          }
          // Appearance
...
// ***************************************************************************
void CVegetableDlg::setShowHideVegetable(uint id, bool visible,
                                         bool refreshDisplay) {
  if (id >= _Vegetables.size())
    return;

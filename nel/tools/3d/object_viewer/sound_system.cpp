...
// TSoundAnimId			CSoundSystem::_CurrentAnimation =
// CSoundAnimation::NoId; sint32
// CSoundSystem::_CurrentPlayback = -1;
...
void CSoundSystem::setListenerMatrix(const NLMISC::CMatrix &m) {
  if (_AudioMixer) {
    ...
    if (m.getPos() != oldMatrix.getPos() || m.getJ() != oldMatrix.getJ() ||
        m.getK() != oldMatrix.getK()) {
...
void CSoundSystem::initSoundSystem() {
  ...
  try {
    // TODO : boris : Hum, as far as I know, throwing exeption in constructor is
    // a very BAD idea...
...
void CSoundSystem::play(const string &soundName) {
  if (_AudioMixer) {
    NLSOUND::USource *src =
        _AudioMixer->createSource(CStringMapper::map(soundName), true);
    if (src) {
      ...
      MessageBox(NULL,
                 _T("Can't play the sound (perhaps it's contextual sound)"),
...
USource *CSoundSystem::create(const std::string &soundName) {
  if (_AudioMixer) {
    NLSOUND::USource *src =
        _AudioMixer->createSource(CStringMapper::map(soundName), false);
    if (src) {
      src->setLooping(false);
      const CVector &pos = _AudioMixer->getListener()->getPos();
      src->setPos(pos);

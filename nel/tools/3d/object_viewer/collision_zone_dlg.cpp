...
CCollisionZoneDlg::CCollisionZoneDlg(CParticleWorkspace::CNode *ownerNode,
                                     NL3D::CPSZone *zone,
                                     CParticleDlg *particleDlg)
    : _Zone(zone), _Node(ownerNode), _ParticleDlg(particleDlg) {
...
void CCollisionZoneDlg::init(sint x, sint y, CWnd *pParent) {
  ...
  _BounceFactorDlg =
      new CEditableRangeFloat(std::string("BOUNCE_FACTOR"), _Node, 0.f, 1.f);
...
void CCollisionZoneDlg::OnSelchangeCollisionBehaviour() {
  UpdateData();
  _Zone->setCollisionBehaviour(
      (NL3D::CPSZone::TCollisionBehaviour)m_CollisionBehaviour.GetCurSel());
  _BounceFactorDlg->EnableWindow(
      _Zone->getCollisionBehaviour() == NL3D::CPSZone::bounce ? TRUE : FALSE);

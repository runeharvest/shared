...
std::string CObjectViewer::getModulePath() const {
  ...
  if (SPath.empty()) {
    ...
    if (len)
      SPath =
          NLMISC::CPath::standardizePath(NLMISC::wideToUtf8(buf, len), true) +
          ".nel/tools/";
  }
  // Check project
  if (SPath.empty()) {
    ...
    std::string cwd =
        NLMISC::CPath::standardizePath(NLMISC::wideToUtf8(buf, len), false);
  ...
  if (SPath.empty()) {
    ...
    int res = GetModuleFileNameW(hModule, buf, MAX_PATH);
    nlassert(res);
...
void CObjectViewer::loadConfigFile() {
  ...
  try {
    ...
    try {
      ...
      CConfigFile::CVar &recursive_search_pathes =
          cf.getVar("recursive_search_pathes");
    ...
    try {
      CConfigFile::CVar &extensions_remapping =
          cf.getVar("extensions_remapping");
      if (extensions_remapping.size() % 2 != 0) {
        nlwarning("extensions_remapping must have a multiple of 2 entries (ex: "
                  "extensions_remapping={\"dds\",\"tga\"};)");
      } else {
        for (uint i = 0; i < (uint)extensions_remapping.size(); i += 2)
          CPath::remapExtension(extensions_remapping.asString(i),
                                extensions_remapping.asString(i + 1), true);
      }
    } catch (const EUnknownVar &) {
    }
    ...
    try {
      /*			CConfigFile::CVar &var =
         cf.getVar("sound_path"); string soundPath = var.asString();
      ...
      */
      ...
      //::MessageBox(NULL, "warning : 'sample_path' or 'packed_sheet_path'
      //:variable not defined.\nSound will not work properly.",
      //:"Objectviewer.cfg", MB_OK|MB_ICONEXCLAMATION);
    }
    ...
    try {
      ...
      //
      if (!loadingOk) {
        //::MessageBox(NULL, "Warning : Unable to load all automatic animation",
        //:"Error", MB_OK | MB_ICONEXCLAMATION);
        nlwarning("Unable to load all automatic animation");
      }
      CNELU::Scene->setAutomaticAnimationSet(as.release());
    } catch (const EUnknownVar &) {
      //::MessageBox(NULL, "No automatic animation path specified, please set
      //:'automatic_animation_path'", "warning", MB_OK);
    ...
    if (var)
      _FogColor = CRGBA((uint8)(var->asInt(0)), (uint8)(var->asInt(1)),
                        (uint8)(var->asInt(2)));
    ...
    ::MessageBox(NULL, nlUtf8ToTStr(e.what()), _T("Objectviewer.cfg"),
...
void CObjectViewer::initCamera() {
  ...
  frustrum.initPerspective(_CameraFocal * (float)Pi / 180.f,
                           height != 0 ? (float)width / (float)height : 0.f,
                           0.1f, 1000.f);
  ...
  for (i = 0; i < _Cameras.size(); i++) {
    frustrum.initPerspective(_ListInstance[_Cameras[i]]->Camera->getFov(),
                             height != 0 ? (float)width / (float)height : 0.f,
                             0.1f, 1000.f);
...
bool CObjectViewer::initUI(HWND parent) {
  ...
  HICON hIcon = (HICON)LoadImage(
      theApp.m_hInstance, MAKEINTRESOURCE(IDI_APP_ICON), IMAGE_ICON, 16, 16, 0);
  ...
  IDriver *driver =
      _Direct3d ? CDRU::createD3DDriver() : CDRU::createGlDriver();
  ...
  // Create the window
  _MainFrame->CFrameWnd::Create(
      AfxRegisterWndClass(0, 0, NULL, hIcon), _T("NeL object viewer"),
      0x00cfc000, /*WS_OVERLAPPEDWINDOW,*/ CFrameWnd::rectDefault, parentWndPtr,
      MAKEINTRESOURCE(IDR_OBJECT_VIEWER_MENU),
  ...
  // Init NELU
  if (!CNELU::init(viewportRect.right, viewportRect.bottom, viewport, 32, true,
                   view->m_hWnd, false, _Direct3d)) {
    return false;
  }
  ...
  setupSceneLightingSystem(_SceneLightEnabled, _SceneLightSunDir,
                           _SceneLightSunAmbiant, _SceneLightSunDiffuse,
  ...
  getRegisterWindowState(_AnimationSetDlg, REGKEY_OBJ_VIEW_ANIMATION_SET_DLG,
                         false);
  ...
  getRegisterWindowState(_ChooseFrameDelayDlg, REGKEY_CHOOSE_FRAME_DELAY_DLG,
                         false);

  // Set backgroupnd color
  setBackGroundColor(_MainFrame->BgColor);

  // Create bg color window (must create after the background color has been
  // set)
  ...
  getRegisterWindowState(_ChooseSunColorDlg, REGKEY_CHOOSE_SUN_COLOR_DLG,
                         false);
  ...
  if (iF.open(path)) {
    try {
      ...
      std::string msg = toString(
          "Unable to load the default scheme bank file : %s", e.what());
      ::MessageBox(NULL, nlUtf8ToTStr(msg), _T("Object Viewer"),
                   MB_ICONEXCLAMATION);
    }
  }
  iF.close();

  // try to load a default config file for the viewer (for anitmation and
  // particle edition setup)
  ...
  if (iF.open(path)) {
    try {
      ...
      std::string msg =
          toString("Error while loading default.ovcgf : %s", e.what());
      ::MessageBox(NULL, nlUtf8ToTStr(msg), _T("NeL object viewer"),
                   MB_OK | MB_ICONEXCLAMATION);
    }
  }
...
void CObjectViewer::addTransformation(CMatrix &current, CAnimation *anim,
                                      float begin, float end, ITrack *posTrack,
                                      ITrack *rotquatTrack,
                                      ITrack *nextPosTrack,
                                      ITrack *nextRotquatTrack,
                                      bool removeLast) {
  // In place ?
...
void CObjectViewer::setupPlaylist(float time) {
  ...
  for (i = 0; i < _ListInstance.size(); i++) {
    ...
    if (_AnimationSetDlg->UseMixer) {
      ...
      // A playlist
      _ListInstance[i]->Playlist.setupMixer(_ListInstance[i]->ChannelMixer,
                                            _AnimationDlg->getTime());
    } else {
      ...
      if (_ListInstance[i]->Saved.PlayList.size() > 0) {
        ...
        anim = _ListInstance[i]->AnimationSet.getAnimation(
            _ListInstance[i]->AnimationSet.getAnimationIdByName(
                _ListInstance[i]->Saved.PlayList[0]));
        if (anim) {
        ...
        while (time >= endTime) {
          ...
          if (index < _ListInstance[i]->Saved.PlayList.size()) {
            ...
            CAnimation *newAnim = _ListInstance[i]->AnimationSet.getAnimation(
                _ListInstance[i]->AnimationSet.getAnimationIdByName(
                    _ListInstance[i]->Saved.PlayList[index]));
            ITrack *newPosTrack = (ITrack *)newAnim->getTrackByName("pos");
            ITrack *newRotquatTrack =
                (ITrack *)newAnim->getTrackByName("rotquat");

            // Add the transformation
            addTransformation(current, anim, newAnim->getBeginTime(),
                              anim->getEndTime(), posTrack, rotquatTrack,
                              newPosTrack, newRotquatTrack, true);
            ...
            // Add the transformation
            addTransformation(current, anim, 0, anim->getEndTime(), posTrack,
                              rotquatTrack, NULL, NULL, false);
        ...
        if (index >= _ListInstance[i]->Saved.PlayList.size()) {
          ...
          // Good index
          choosedIndex = _ListInstance[i]->AnimationSet.getAnimationIdByName(
              _ListInstance[i]->Saved.PlayList[index]);
          ...
          // Add the transformation
          addTransformation(current, anim, 0,
                            anim->getBeginTime() + time - startTime, posTrack,
                            rotquatTrack, NULL, NULL, false);
        ...
        if (there) {
          ...
          CSkeletonModel *skelModel =
              dynamic_cast<CSkeletonModel *>(_ListInstance[i]->TransformShape);
          // If a  skeleton model
          if (skelModel) {
        ...
        if (_ListInstance[i]->TransformShape) {
          CMeshBase *meshBase = dynamic_cast<CMeshBase *>(
              (IShape *)_ListInstance[i]->TransformShape->Shape);
          if (meshBase) {
            _ListInstance[i]->TransformShape->setPos(
                meshBase->getDefaultPos()->getDefaultValue());
            _ListInstance[i]->TransformShape->setRotQuat(
                meshBase->getDefaultRotQuat()->getDefaultValue());
            _ListInstance[i]->TransformShape->setScale(
                meshBase->getDefaultScale()->getDefaultValue());
          } else {
        ...
        if (_ListInstance[i]->Camera) {
          _ListInstance[i]->Camera->setPos(
              _ListInstance[i]->Camera->getDefaultPos()->getDefaultValue());
          _ListInstance[i]->Camera->setTargetPos(
              _ListInstance[i]
                  ->Camera->getDefaultTargetPos()
                  ->getDefaultValue());
        }
      }
    }
...
void CObjectViewer::go() {
  nlassert(!_InstanceRunning); // this shouldn't be called if an instance of the
                               // viewer is running.
  AFX_MANAGE_STATE(AfxGetStaticModuleState());
  ...
  do {
    ...
    if (isParentWnd(_MainFrame->m_hWnd, GetForegroundWindow())) {
      ...
      // CNELU::Scene->animate( (float) + NLMISC::CTime::ticksToSecond(
      ...
      {
        std::vector<IMainLoopCallBack *> copyVect(_CallBackList.begin(),
                                                  _CallBackList.end());
        for (std::vector<IMainLoopCallBack *>::iterator it =
                 _CallBackList.begin();
      ...
      if (_MainFrame->MoveMode == CMainFrame::ObjectMode) {
        ...
        CDRU::drawLine(_MouseListener.getHotSpot() + CVector(radius, 0, 0),
                       _MouseListener.getHotSpot() + CVector(-radius, 0, 0),
                       _HotSpotColor, *CNELU::Driver);
        CDRU::drawLine(_MouseListener.getHotSpot() + CVector(0, radius, 0),
                       _MouseListener.getHotSpot() + CVector(0, -radius, 0),
                       _HotSpotColor, *CNELU::Driver);
        CDRU::drawLine(_MouseListener.getHotSpot() + CVector(0, 0, radius),
                       _MouseListener.getHotSpot() + CVector(0, 0, -radius),
                       _HotSpotColor, *CNELU::Driver);
      }
      ...
      if (_FXMatrixVisible)
        drawFXMatrix();
      if (_FXUserMatrixVisible)
        drawFXUserMatrix();
      if (_SceneMatrixVisible)
        drawSceneMatrix();
      ...
      float fps =
          timeDiff > 0
              ? (float)(1.0 / NLMISC::CTime::ticksToSecond(newTime - lastTime))
              : 1000.0f;
      lastTime = newTime;
      ...
      // Display std info.
      std::string msgBar = toString(
          "%s - Nb tri: %u - Texture used (MiB): %5.2f - Texture allocated "
          "(MiB): %5.2f - Distance: %5.0f - Sounds: %u/%u - Fps: %03.1f",
          _Direct3d ? "Direct3d" : "OpenGL",
          in.NLines + in.NPoints + in.NQuads * 2 + in.NTriangles +
              in.NTriangleStrips,
          (float)CNELU::Driver->getUsedTextureMemory() / (float)(1024 * 1024),
          (float)CNELU::Driver->profileAllocatedTextureMemory() /
              (float)(1024 * 1024),
          (_SceneCenter - CNELU::Camera->getMatrix().getPos()).norm(),
          nbPlayingSources, nbSources, fps);
      ...
      if (_VegetableDlg != NULL) {
        if (_VegetableLandscape != NULL) {
          ...
          vegetMsgBar.Format(
              _T("%u"),
              _VegetableLandscape->Landscape.getNumVegetableFaceRendered());
      ...
      if (_MainFrame->isMoveElement()) {
        // for now we apply a transform on the selected object in the particle
        // system
      ...
      if (_ObjectLightTest && _GlobalRetriever) {
        ...
        UGlobalPosition gPos =
            _GlobalRetriever->retrievePosition(_ObjectLightTestMatrix.getPos());
...
void CObjectViewer::reinitChannels() {
  ...
  for (uint i = 0; i < _ListInstance.size(); i++) {
    ...
    // Setup animation set
    _ListInstance[i]->ChannelMixer.setAnimationSet(
        &(_ListInstance[i]->AnimationSet));

    // Register the transform (but not if it has automatic animation, as a
    // channel mixer has been created for us)
    bool autoAnim = false;
    if (dynamic_cast<CMeshBaseInstance *>(_ListInstance[i]->TransformShape)) {
      CMeshBase *mb = NLMISC::safe_cast<CMeshBase *>(
          (IShape *)static_cast<CMeshBaseInstance *>(
              _ListInstance[i]->TransformShape)
              ->Shape);
    ...
    if (!autoAnim) {
      if (_ListInstance[i]->TransformShape)
        _ListInstance[i]->TransformShape->registerToChannelMixer(
            &(_ListInstance[i]->ChannelMixer), "");
      if (_ListInstance[i]->Camera)
        _ListInstance[i]->Camera->registerToChannelMixer(
            &(_ListInstance[i]->ChannelMixer), "");
    }
  }
...
void CObjectViewer::enableChannels() {
  ...
  for (uint i = 0; i < _ListInstance.size(); i++) {
    ...
    uint rotQuatId =
        _ListInstance[i]->AnimationSet.getChannelIdByName("rotquat");
    uint rotEulerId =
        _ListInstance[i]->AnimationSet.getChannelIdByName("roteuler");
...
float CObjectViewer::getFrameRate() { return _AnimationDlg->Speed; }
    ...
    if (f.isReading()) {
      ...
      for (uint i = 0; i < readed.size(); i++) {
        try {
          ...
          if (readed[i].Camera) {
            instance = addCamera(readed[i].CameraInfo,
                                 readed[i].ShapeFilename.c_str());
          } else {
            // Load the shape
            CIFile input;
            ...
            if (input.open(path)) {
              ...
              if (readed[i].IsSkeleton) {
                // Add the skel
                instance = addSkel(serialShape.getShapePointer(),
                                   readed[i].ShapeFilename.c_str());
                SkeletonUsedForSound = instance;
              } else {
                // Add the mesh
                if (readed[i].SkeletonId != 0xffffffff)
                  instance = addMesh(serialShape.getShapePointer(),
                                     readed[i].ShapeFilename.c_str(),
                                     readed[i].SkeletonId + firstInstance,
                                     (readed[i].BindBoneName.empty())
                                         ? NULL
                                         : readed[i].BindBoneName.c_str());
                else
                  instance =
                      addMesh(serialShape.getShapePointer(),
                              readed[i].ShapeFilename.c_str(), 0xffffffff,
                              (readed[i].BindBoneName.empty())
                                  ? NULL
                                  : readed[i].BindBoneName.c_str());
              }
              ...
              // Error message
              std::string message = toString("File not found %s",
                                             readed[i].ShapeFilename.c_str());
              _MainFrame->MessageBox(nlUtf8ToTStr(message),
          ...
          // Load animations
          for (uint anim = 0; anim < readed[i].AnimationFileName.size();
               anim++) {
          ...
          for (uint slot = 0; slot < NL3D::CChannelMixer::NumAnimationSlot;
               slot++)
            _ListInstance[instance]->Saved.SlotInfo[slot] =
                readed[i].SlotInfo[slot];
        } catch (const Exception &e) {
          // Error message
          std::string message =
              toString("Error loading shape %s: %s",
                       readed[i].ShapeFilename.c_str(), e.what());
          _MainFrame->MessageBox(nlUtf8ToTStr(message), _T("NeL object viewer"),
                                 MB_OK | MB_ICONEXCLAMATION);
...
bool CObjectViewer::loadInstanceGroup(const std::string &igFilename) {
  ...
  if (file.open(igFilename)) {
    ...
    try {
      ...
      // clean
      delete ig;
      _MainFrame->MessageBox(nlUtf8ToTStr(e.what()), _T("NeL object viewer"),
                             MB_OK | MB_ICONEXCLAMATION);
      return false;
    }
  } else {
    // Create a message
    std::string msg =
        toString("Can't open the file %s for reading.", igFilename.c_str());
    _MainFrame->MessageBox(nlUtf8ToTStr(msg), _T("NeL object viewer"),
                           MB_OK | MB_ICONEXCLAMATION);
    return false;
  }
...
bool CObjectViewer::loadMesh(std::vector<std::string> &meshFilename,
  ...
  if (!skeleton.empty()) {
    ...
    if (file.open(skeleton)) {
      ...
      // Create a message
      std::string msg = NLMISC::toString("Can't open the file %s for reading.",
                                         skeleton.c_str());
      _MainFrame->MessageBox(nlUtf8ToTStr(msg), _T("NeL object viewer"),
                             MB_OK | MB_ICONEXCLAMATION);
  ...
  for (uint i = 0; i < meshFilename.size(); i++) {
    ...
    if (shapeSkel && (!skelUsed)) {
      ...
      if (skelIndex != 0xffffffff) {
        skelUsed = true;
        transformSkel = dynamic_cast<CSkeletonModel *>(
            _ListInstance[skelIndex]->TransformShape);
...
uint CObjectViewer::addMesh(NL3D::IShape *pMeshShape,
                            const std::string &meshName, uint skelIndex,
                            const char *bindSkelName, bool createInstance) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState());
  ...
  if (createInstance) {
    ...
    // Get a mesh instance
    CMeshBaseInstance *meshInstance =
        dynamic_cast<CMeshBaseInstance *>(pTrShape);

    // Bind the mesh
    if (skelIndex != 0xffffffff) {
      // Get the skeleton
      NL3D::CSkeletonModel *transformSkel = dynamic_cast<CSkeletonModel *>(
          _ListInstance[skelIndex]->TransformShape);
      nlassert(transformSkel);
      ...
      CMeshMRMSkinned *meshMrmSkinned =
          dynamic_cast<CMeshMRMSkinned *>(pMeshShape);
      if ((mesh && mesh->getMeshGeom().isSkinned()) ||
          (meshMrm && meshMrm->getMeshGeom().isSkinned()) || meshMrmSkinned) {
        // Bind to skeleton
        ...
        if (bindBone == 0xffffffff) {
          ...
          CSelectString dialogSelect(listBones, message.c_str(), _MainFrame,
                                     false);
...
// ***************************************************************************
bool CObjectViewer::chooseBone(const std::string &caption,
                               NL3D::CSkeletonModel *&skel, uint &boneIndex,
                               std::string *skelName /*= NULL*/,
                               std::string *boneName /*= NULL*/) {
  for (uint k = 0; k < _ListInstance.size(); ++k) {
    NL3D::CSkeletonModel *transformSkel =
        dynamic_cast<CSkeletonModel *>(_ListInstance[k]->TransformShape);
    if (transformSkel) {
      // Make a list of bones
      vector<string> listBones;
      uint bone;
      ...
      if (dialogSelect.DoModal() == IDOK) {
        ...
        if (boneName) {
          *boneName = safe_cast<NL3D::CSkeletonModel *>(
                          _ListInstance[k]->TransformShape)
                          ->Bones[boneIndex]
                          .getBoneName();
        }
        return true;
...
uint CObjectViewer::addCamera(const NL3D::CCameraInfo &cameraInfo,
...
uint CObjectViewer::addSkel(NL3D::IShape *pSkelShape,
...
IObjectViewer *IObjectViewer::getInterface(int version) {
  ...
  if (version != OBJECT_VIEWER_VERSION) {
    MessageBox(NULL, _T("Bad version of object_viewer.dll."),
               _T("NeL object viewer"), MB_ICONEXCLAMATION | MB_OK);
    return NULL;
  } else
...
void CObjectViewer::setSingleAnimation(NL3D::CAnimation *pAnim,
  ...
  if (instance < _ListInstance.size()) {
    ...
    _ListInstance[instance]->Saved.SlotInfo[0].StartTime =
        (int)(pAnim->getBeginTime() * _AnimationDlg->Speed);
    _ListInstance[instance]->Saved.SlotInfo[0].EndTime =
        (int)(pAnim->getEndTime() * _AnimationDlg->Speed);
...
void CObjectViewer::registerMainLoopCallBack(IMainLoopCallBack *i) {
  //	nlassert(std::find(_CallBackList.begin(), _CallBackList.end(), i) ==
  //_CallBackList.begin()); // the object was register twice !!
...
void CObjectViewer::removeMainLoopCallBack(IMainLoopCallBack *i) {
  std::vector<IMainLoopCallBack *>::iterator it =
      std::find(_CallBackList.begin(), _CallBackList.end(), i);
...
void CObjectViewer::shuffleTextureSet() {
  ...
  for (it = _ListInstance.begin(); it != _ListInstance.end(); ++it) {
    NL3D::CTransformShape *trShape = (*it)->TransformShape;
    if (dynamic_cast<NL3D::CMeshBaseInstance *>(trShape)) {
      static_cast<NL3D::CMeshBaseInstance *>(trShape)->selectTextureSet(rand() %
                                                                        8);
    }
  }
}
...
void CObjectViewer::enableFXs(bool enabled) {
  // Stand alone fxs
  for (uint instance = 0; instance < _ListInstance.size(); instance++) {
    NL3D::CSegRemanence *sr = dynamic_cast<NL3D::CSegRemanence *>(
        _ListInstance[instance]->TransformShape);
    if (sr) {
      if (enabled)
        sr->start();
      else
  ...
  for (uint igId = 0; igId < _ListIG.size(); ++igId) {
    for (uint k = 0; k < _ListIG[igId]->_Instances.size(); ++k) {
      NL3D::CSegRemanence *sr =
          dynamic_cast<NL3D::CSegRemanence *>(_ListIG[igId]->_Instances[k]);
      if (sr) {
        if (enabled)
          sr->start();
        else
...
void CObjectViewer::evalSoundTrack(float lastTime, float currentTime) {
  ...
  for (uint i = 0; i < _ListInstance.size(); i++) {
    ...
    if (!_ListInstance[i]->Saved.PlayList.empty()) {
      ...
      for (uint index = 0; index < _ListInstance[i]->Saved.PlayList.size();
           index++) {
        // Pointer on the animation
        string &name = _ListInstance[i]->Saved.PlayList[index];
        CAnimation *anim = _ListInstance[i]->AnimationSet.getAnimation(
            _ListInstance[i]->AnimationSet.getAnimationIdByName(name));
        ...
        if ((startTime <= currentTime) && (currentTime < endTime)) {
          ...
          if (SkeletonUsedForSound != 0xFFFFFFFF) {
            const CMatrix &m = _ListInstance[SkeletonUsedForSound]
                                   ->TransformShape->getMatrix();
            SoundContext.Position = m.getPos();
          } else {
            SoundContext.Position = CVector::Null;
          }

          CSoundSystem::playAnimation(name, lastTime - startTime,
                                      currentTime - startTime, SoundContext);
        }
      }
    }
  }
...
void CObjectViewer::evalSoundTrack (float lastTime, float currentTime)
{
    if (lastTime!=currentTime)
    {
        ...
        for (uint instance=0; instance<_ListInstance.size(); instance++)
        {
            ...
            {
                // Anim id
                uint animId=_ListInstance[instance]->Playlist.getAnimation
(slot);
                ...
                if (animId!=CAnimationPlaylist::empty)
                {
                    // Get the animation
                    CAnimation
*anim=_ListInstance[instance]->AnimationSet.getAnimation (animId); nlassert
(anim);
...
dynamic_cast<UTrackKeyframer *>(track); if (soundTrackKF)
                        {
                            // Sound keys
                            std::vector<TAnimationTime> result;

                            // Get local begin and endTime
                            TAnimationTime localLastTime =
_ListInstance[instance]->Playlist.getLocalTime (slot, lastTime,
_ListInstance[instance]->AnimationSet); TAnimationTime localCurrentTime =
_ListInstance[instance]->Playlist.getLocalTime (slot, currentTime,
_ListInstance[instance]->AnimationSet);

                            // Good interval
                            if (localLastTime<=localCurrentTime)
                            {
                                // Get keys in this interval
                                soundTrackKF->getKeysInRange(localLastTime,
localCurrentTime, result);
                            ...
                            else
                            {
                                ...
                                // Get keys to the end
                                soundTrackKF->getKeysInRange(localCurrentTime,
endTime, result);

                                // Get keys at the beginning
                                soundTrackKF->getKeysInRange(beginTime,
localLastTime, result);
                            }

                            // Process sounds
                            NLSOUND::UAudioMixer *audioMixer =
CSoundSystem::getAudioMixer (); if( audioMixer )
                            {
                                vector<TAnimationTime>::iterator itResult;
                                for( itResult = result.begin(); itResult !=
result.end(); ++itResult )
                                {
                                    string soundName;
                                    double keyTime = *itResult;
                                    nlinfo("keyTime = %f  result size :
%d",*itResult,result.size());

                                    if( !track->interpolate( *itResult,
soundName) )
                                    {
                                        nlwarning("The key at offset %f is not a
string",*itResult);
                                    }
                                    else
                                    {
                                        ...
                                        if( soundName == "step" )
                                        {
                                            // need to spawn a sound linked to
the anim string dummySound = "PAShommecourseappartdur1a"; USource *source =
audioMixer->createSource (dummySound.c_str() , true ); if (source)
                                            {
                                                source->setPos (CVector::Null);
                                                source->play ();
                                                nlinfo ("launching dummy sound
%s for the step event", dummySound.c_str());
                                            }
                                            else
                                            {
                                                nlwarning ("sound not found for
the step event: '%s'", dummySound.c_str());
                                            }
                                        }
                                        else if (soundName.find ("snd_") !=
string::npos)
                                        {
                                            // need to spawn a sound linked to
the anim USource *source = audioMixer->createSource ( soundName.c_str(), true );
                                            if (source)
                                            {
                                                source->setPos (CVector::Null);
                                                source->play ();
                                                nlinfo ("launching sound for
anim event from notetrack '%s'", soundName.c_str());
                                            }
                                            else
                                            {
                                                nlwarning ("sound not found:
'%s'", soundName.c_str());
                                            }
                                        }
                                        else
                                        {
                                            nlwarning ("unknown notetrack event:
'%s'", soundName.c_str());
                                        }
                                    }
                                }
...
// ***************************************************************************
void CObjectViewer::setupSceneLightingSystem(bool enable,
...
// ***************************************************************************
void CObjectViewer::enableDynamicObjectLightingTest(
    NLPACS::CGlobalRetriever *globalRetriever, NL3D::CInstanceGroup *ig) {
  AFX_MANAGE_STATE(AfxGetStaticModuleState());
  ...
  if (_GlobalRetriever) {
    ...
    if (_ObjectLightTest != NULL) {
      ...
      if (!_ObjectLightTestShape.empty()) {
        string str = string("Path not found for Light Test Shape: ") +
                     _ObjectLightTestShape;
        ::MessageBox(NULL, nlUtf8ToTStr(str.c_str()),
                     _T("Dynamic Object Light Test"),
                     MB_OK | MB_ICONEXCLAMATION);
      }
      // disable.
      _ObjectLightTest = NULL;
...
// ***************************************************************************
void CObjectViewer::COVLogicInfo::getStaticLightSetup(
    NLMISC::CRGBA sunAmbient, std::vector<CPointLightInfluence> &pointLightList,
    uint8 &sunContribution, CRGBA &ambient) {
  Ig->getStaticLightSetup(
      sunAmbient, GlobalRetriever->getLocalRetrieverId(GPos),
      GPos.LocalPosition.Surface, GPos.LocalPosition.Estimation, pointLightList,
      sunContribution, ambient);
}
...
bool CObjectViewer::createVegetableLandscape() {
  ...
  if (!_VegetableLandscape) {
    ...
    _VegetableLandscape = static_cast<CLandscapeModel *>(
        CNELU::Scene->createModel(LandscapeModelId));
    ...
    try {
      ...
      if (CNELU::Driver) {
        ...
        sint nbTextTotal = 0;
        for (ts = 0;
             ts < _VegetableLandscape->Landscape.TileBank.getTileSetCount();
             ts++) {
          CTileSet *tileSet =
              _VegetableLandscape->Landscape.TileBank.getTileSet(ts);
        ...
        for (ts = 0;
          ...
          for (tl = 0; tl < tileSet->getNumTile128(); tl++, nbTextDone++) {
            _VegetableLandscape->Landscape.flushTiles(
                CNELU::Driver, (uint16)tileSet->getTile128(tl), 1);
            dlgProgress.ProgressBar.SetPos(nbTextDone * 100 / nbTextTotal);
          }
          for (tl = 0; tl < tileSet->getNumTile256(); tl++, nbTextDone++) {
            _VegetableLandscape->Landscape.flushTiles(
                CNELU::Driver, (uint16)tileSet->getTile256(tl), 1);
            dlgProgress.ProgressBar.SetPos(nbTextDone * 100 / nbTextTotal);
          }
          for (tl = 0; tl < CTileSet::count; tl++, nbTextDone++) {
            _VegetableLandscape->Landscape.flushTiles(
                CNELU::Driver, (uint16)tileSet->getTransition(tl)->getTile(),
                1);
            dlgProgress.ProgressBar.SetPos(nbTextDone * 100 / nbTextTotal);
          }
        }
      ...
      _VegetableLandscape->Landscape.setupStaticLight(
          _VegetableLandscapeDiffuse, _VegetableLandscapeAmbient, 1);
      _VegetableLandscape->Landscape.loadVegetableTexture(_VegetableTexture);
      _VegetableLandscape->Landscape.setupVegetableLighting(
          _VegetableAmbient, _VegetableDiffuse, _VegetableLightDir);
      _VegetableLandscape->Landscape.setVegetableWind(
          _VegetableWindDir, _VegetableWindFreq, _VegetableWindPower,
      ...
      MessageBox(_MainFrame->m_hWnd, nlUtf8ToTStr(e.what()),
                 _T("Failed to Load landscape"), MB_OK | MB_APPLMODAL);
...
// ***************************************************************************
void CObjectViewer::refreshVegetableLandscape(
    const NL3D::CTileVegetableDesc &tvdesc) {
  ...
  if (_VegetableLandscape) {
    ...
    for (sint ts = 0;
         ts < _VegetableLandscape->Landscape.TileBank.getTileSetCount(); ts++) {
      CTileSet *tileSet =
          _VegetableLandscape->Landscape.TileBank.getTileSet(ts);
...
// ***************************************************************************
uint CObjectViewer::getEditedObject() { return _SelectedObject; }
...
void CInstanceInfo::setAnimationPlaylist(float frameRate) {
  for (uint id = 0; id < NL3D::CChannelMixer::NumAnimationSlot; id++) {
    if (Saved.SlotInfo[id].Enable) {
      // Set the animation
      uint animId =
          AnimationSet.getAnimationIdByName(Saved.SlotInfo[id].Animation);
      ...
      uint skelId =
          AnimationSet.getSkeletonWeightIdByName(Saved.SlotInfo[id].Skeleton);
      if (skelId == CAnimationSet::NotFound)
        Playlist.setSkeletonWeight(id, CAnimationPlaylist::empty, false);
      else
        Playlist.setSkeletonWeight(id, skelId,
                                   Saved.SlotInfo[id].SkeletonInverted);

      // Set others values
      Playlist.setTimeOrigin(id, Saved.SlotInfo[id].Offset / frameRate);
      Playlist.setSpeedFactor(id, Saved.SlotInfo[id].SpeedFactor);
      Playlist.setStartWeight(id, Saved.SlotInfo[id].StartBlend,
                              Saved.SlotInfo[id].StartTime / frameRate);
      Playlist.setEndWeight(id, Saved.SlotInfo[id].EndBlend,
                            Saved.SlotInfo[id].EndTime / frameRate);
...
// ***************************************************************************
void CObjectViewer::addAnimation(NL3D::CAnimation *anim,
                                 const std::string &filename,
                                 const std::string &name, uint instance) {
  // Add an animation
  uint id =
      _ListInstance[instance]->AnimationSet.addAnimation(name.c_str(), anim);
  ...
  if (CNELU::Driver && CNELU::ShapeBank) {
    _ListInstance[instance]->AnimationSet.preloadSSSShapes(*CNELU::Driver,
                                                           *CNELU::ShapeBank);
  }
...
// ***************************************************************************
CMainDlg *CObjectViewer::getSlotDlg() { return _SlotDlg; }
...
void CObjectViewer::shootScene() {
  static const TCHAR BASED_CODE szFilter[] =
      _T("PNG Files (*.png)|*.png|Targa Files (*.tga)|*.tga|Jpeg Files ")
      _T("(*.jpg)|*.jpg|All Files (*.*)|*.*||");
  CFileDialog fileDlg(FALSE, _T(".tga"), _T("*.tga"),
  ...
  if (fileDlg.DoModal() == IDOK) {
    ...
    if (movieSize.DoModal() == IDOK) {
      ...
      _MainFrame->SetWindowPos(NULL, 0, 0, window.right - window.left,
      ...
      // The file name
      string filename = NLMISC::CFile::getFilenameWithoutExtension(
          tStrToUtf8(fileDlg.GetPathName()));
      string extension =
          NLMISC::CFile::getExtension(tStrToUtf8(fileDlg.GetPathName()));
        ...
        try {
          ...
          if (output.open(filenamefinal)) {
            ...
            std::string message = toString(
                "Can't open the file %s for writing.", filenamefinal.c_str());
            _MainFrame->MessageBox(nlUtf8ToTStr(message),
                                   _T("NeL object viewer"),
                                   MB_OK | MB_ICONEXCLAMATION);
            break;
          ...
          std::string message =
              toString("Error during writing of the file %s: %s",
                       filenamefinal.c_str(), e.what());
          _MainFrame->MessageBox(nlUtf8ToTStr(message), _T("NeL object viewer"),
                                 MB_OK | MB_ICONEXCLAMATION);
          break;
        }
...

{
  ...
  drawNamedMatrix(getFXUserMatrix(), fxUserMatrixStr, NLMISC::CRGBA::Red, 0.2f,
                  10.f);
}
...
void CObjectViewer::drawFXMatrix() {
  ...
  drawNamedMatrix(_ParticleDlg->getPSWorldMatrix(), fxStr, NLMISC::CRGBA::Blue,
                  -0.2f, 10.f);
}
...
void CObjectViewer::drawSceneMatrix() {
  ...
  drawNamedMatrix(_SceneRoot->getMatrix(), sceneMatrixStr, NLMISC::CRGBA::White,
                  0.f, 10.f);
}

void CObjectViewer::drawNamedMatrix(const NLMISC::CMatrix &matrix,
                                    const std::string &name,
                                    NLMISC::CRGBA color, float textZOffset,
                                    float testSize) {
  CPSUtil::displayBasis(CNELU::Driver, matrix, NLMISC::CMatrix::Identity, 1.f,
                        *_FontGenerator, _FontManager);
  CPSUtil::print(CNELU::Driver, name, *_FontGenerator, _FontManager,
                 matrix.getPos() + NLMISC::CVector(0.f, 0.f, textZOffset),
                 testSize, color);
}

sint CObjectViewer::getCurrentCamera() const { return _CurrentCamera; }
...
uint CObjectViewer::getNumCamera() const { return (uint)_Cameras.size(); }
...
int localizedMessageBox(HWND parentWindow, int messageStringID,
  ...
  // TODO : replace older call to ::MessageBox in the object viewer with that
  // function
}

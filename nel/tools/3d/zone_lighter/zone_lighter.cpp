...
static void loadIGFromVillage(const NLGEORGES::UFormElm *villageItem,
                              const std::string &continentName,
                              uint villageIndex,
                              std::list<CInstanceGroupRef> &instanceGroups,
  ...
  for (uint l = 0; l < numIgs; ++l) {
    ...
    // verify that the ig is not already added (case of tr_water.ig in
    // additional_igs)
    for (uint igAdd = 0; igAdd < (uint)additionalIgNames.size(); igAdd++) {
      if (toLowerAscii(additionalIgNames.asString()) == toLower(igName)) {
        nlwarning(
            "Skipping Village Ig %s, cause already exist in additional ig",
            igName.c_str());
        continue;
      }
    }
...
static void loadIGFromContinent(NLMISC::CConfigFile &parameter,
                                std::list<CInstanceGroupRef> &instanceGroups,
  ...
  try {
    ...
    //
    std::string pathName =
        CPath::lookup(continentName); // level_design_world_directory.asString()
                                      // + "/" + continentName;
    if (pathName.empty()) {
    ...
    if (villageForm != NULL) {
      ...
      for (uint k = 0; k < numVillage; ++k) {
        ...
        for (uint l = 0; l < zoneNameArray.size(); ++l) {
          if (NLMISC::nlstricmp(
                  CFile::getFilenameWithoutExtension(zoneNameArray[l]),
                  zoneName) == 0) {
  ...
  if (argc < 5) {
    ...
    if (inputFile.open(argv[1])) {
      ...
      try {
        ...
        // Grid size
        CConfigFile::CVar &quad_grid_cell_size =
            parameter.getVar("quad_grid_cell_size");
        lighterDesc.GridCellSize = quad_grid_cell_size.asFloat();

        // Heightfield cell size
        CConfigFile::CVar &global_illumination_cell_size =
            parameter.getVar("global_illumination_cell_size");
        lighterDesc.HeightfieldCellSize =
            global_illumination_cell_size.asFloat();

        // Heightfield size
        CConfigFile::CVar &global_illumination_length =
            parameter.getVar("global_illumination_length");
        ...
        CConfigFile::CVar &zbuffer_landscape_size =
            parameter.getVar("zbuffer_landscape_size");
        lighterDesc.ZBufferLandscapeSize = zbuffer_landscape_size.asInt();

        // ZBuffer object size
        CConfigFile::CVar &zbuffer_object_size =
            parameter.getVar("zbuffer_object_size");
        lighterDesc.ZBufferObjectSize = zbuffer_object_size.asInt();

        // Soft shadow samples sqrt
        CConfigFile::CVar &soft_shadow_samples_sqrt =
            parameter.getVar("soft_shadow_samples_sqrt");
        lighterDesc.SoftShadowSamplesSqrt = soft_shadow_samples_sqrt.asInt();

        // Soft shadow jitter
        CConfigFile::CVar &soft_shadow_jitter =
            parameter.getVar("soft_shadow_jitter");
        ...
        CConfigFile::CVar &modulate_water_color =
            parameter.getVar("modulate_water_color");
        lighterDesc.ModulateWaterColor = modulate_water_color.asInt() != 0;

        CConfigFile::CVar &sky_contribution_for_water =
            parameter.getVar("sky_contribution_for_water");
        lighterDesc.SkyContributionForWater =
            sky_contribution_for_water.asInt() != 0;
        ...
        // Sun contribution
        CConfigFile::CVar &sun_contribution =
            parameter.getVar("sun_contribution");
        ...
        // Sky contribution
        CConfigFile::CVar &sky_contribution =
            parameter.getVar("sky_contribution");
        ...
        // Vegetable Height
        CConfigFile::CVar &vegetable_height =
            parameter.getVar("vegetable_height");
        ...
        // A landscape allocated with new: it is not delete because destruction
        // take 3 secondes more!
        ...
        if (lighterDesc.Shadow) {
          CConfigFile::CVar &dependant_zones =
              dependency.getVar("dependencies");
        ...
        while (ite != instanceGroup.end()) {
          ...
          if (lighterDesc.Shadow) {
            // For each instance
            for (uint instance = 0; instance < group->getNumInstance();
                 instance++) {
              // Get the instance shape name
              string name = group->getShapeName(instance);
              if (!name.empty()) {
                // Skip it?? use the DontCastShadowForExterior flag. See doc of
                // this flag
                if (group->getInstance(instance).DontCastShadow ||
                    group->getInstance(instance).DontCastShadowForExterior)
                  continue;

                if (toLowerAscii(CFile::getExtension(name)) == "pacs_prim") {
                  nlwarning("EXPORT BUG: Can't read %s (not a shape), should "
                            "not be part of .ig!",
                            name.c_str());
                  continue;
                }
                ...
                if (iteMap != shapeMap.end()) {
                  ...
                  /** If it is a lightable shape and we are dealing with the ig
                   * of the main zone, add it to the lightable shape list
                   */
                  IShape *shape = iteMap->second;
                  if (ite == instanceGroup
                                 .begin() /* are we dealing with main zone */
                      && zoneIgLoaded /* ig of the main zone loaded successfully
                                         (so its indeed the ig of the first
                                         zone) ? */
                  ...
                  /** If it is a water shape, add it to the lighter, so that it
                   * can check which tiles are above / below water for this
                   * zone. The result is saved in the flags of tiles. A tile
                   * that have their flags set to VegetableDisabled won't get
                   * setupped
                   */
                  if (dynamic_cast<NL3D::CWaterShape *>(shape)) {
                    lighter.addWaterShape(
                        static_cast<NL3D::CWaterShape *>(shape), mt);
                  }
                }
              }
            }
        ...
        if (continu) {
          ...
          if (!tileWaterPatchOnly) {
            // Light the zone
            lighter.light(*landscape, output, zone.getZoneId(), lighterDesc,
                          vectorTriangle, listZoneId);
          } else {
            // Load the zonel.
            CIFile zonelFile;
            ...
            if (zonelFile.open(argv[2])) {
              ...
              throw Exception(
                  "ERROR Can't open the zonel for tile water patching. abort");
            }
            zonelFile.close();
            ...
            COFile outputFile;
            string bkupFile =
                tileWaterPatchBkupDir + "/" + CFile::getFilename(argv[2]);
            ...
            // patch water flags
            lighter.computeTileFlagsOnly(*landscape, output, zone.getZoneId(),
                                         lighterDesc, listZoneId);
          }

...
#define myinfo                                                                 \
  NLMISC::createDebug(), NLMISC::InfoLog->setPosition(__LINE__, __FILE__),     \
      NLMISC::InfoLog->displayRawNL

// ***************************************************************************
void filterRyzomBug(const char *dirSrc, const char *dirDst,
                    uint patchVersionWanted, const string specialFilter) {
    ...
    if (f.open(fileFullPath, true)) {
      ...
      while (!f.eof()) {
        ...
        if (str.compare(0, userIdTok.size(), userIdTok) == 0) {
          numUserId++;
          if (precUserId) {
            nlwarning("Don't find a PatchVersion for all UserId in %s",
                      fileFullPath.c_str());
            ok = false;
            break;
          } else
        ...
        if (str.compare(0, patchVersionTok.size(), patchVersionTok) == 0) {
          ...
          if (version != (sint)patchVersionWanted) {
            nlwarning("The Log %s contains a PatchVersion different: %d",
                      fileFullPath.c_str(), version);
            ok = false;
            break;
          }
        }
        if (!specialFilter.empty() &&
            str.compare(0, specialFilter.size(), specialFilter) == 0)
          filterFound = true;
      }
      f.close();
...
void statRyzomBug(const char *dirSrc) {
  ...
  for (uint i = 0; i < fileList.size(); i++) {
    ...
    if (f.open(fileFullPath, true)) {
      ...
      sint precShardId = -1;
      sint precTimeInGame =
          -1; // 0 means "never in game", 1 means < 10 min, 2 means more
      ...
      while (!f.eof()) {
        ...
        if (str.compare(0, senderIdTok.size(), senderIdTok) == 0) {
          ...
          sscanf(posStr.c_str(), "%f%f%f", &precUserPos.x, &precUserPos.y,
                 &precUserPos.z);
        } else if (str.compare(0, timeInGameIdTok.size(), timeInGameIdTok) ==
                   0) {
        ...
        } else if (str.compare(0, patchVersionIdTok.size(),
                               patchVersionIdTok) == 0) {
          ...
          if (timeStr.size() < 19)
            ...
            precLocalTime =
                (((precLocalTime * 24) + hour) * 60 + minute) * 60 + sec;
          }
          ...
          if (absTime < 0)
            absTime = -absTime;
          if (precSenderId != precSenderId2 || absTime > sint64(60)) {
  ...
  // **** display Stats
  // general stats
  myinfo("**** Total: %d Crashs (%d with duplicates)", totalCrash,
         totalCrashDuplicate);
  myinfo(
      "NB: 'duplicates' means: crashs that are removed because suppose the "
      "player click 'ignore' (same sender/same Localtime, within about 1 min)");
  ...
  for (it = timeInGameMap.begin(); it != timeInGameMap.end(); it++) {
    myinfo("**** %d Crashs for TimeInGame %s", it->second.Val,
           it->first == 0 ? "0h 0min 0sec"
                          : (it->first == 1   ? "<=5 min"
                             : it->first == 2 ? "> 5 min"
                                              : "??? Bad parse ???"));
  }
  ...
  for (it2 = resortSender.begin(); it2 != resortSender.end(); it2++) {
    ...
    if (crashFileMap.empty()) {
      ...
      for (TStatStrMap::iterator it = crashFileMap.begin();
           it != crashFileMap.end(); it++) {
  ...
  for (TStatStrStrMap::iterator itStrStr = crashFileToSenderMap.begin();
       itStrStr != crashFileToSenderMap.end(); itStrStr++) {
    // count total crash instance
    uint numCrash = 0;
    TStatStrMap &userIdMap = itStrStr->second;
    for (TStatStrMap::iterator it = userIdMap.begin(); it != userIdMap.end();
         it++)
  ...
  for (it2 = resortCrashLog.begin(); it2 != resortCrashLog.end(); it2++) {
    ...
    if (userIdMap.empty()) {
      myinfo("    Error parsing Crashs for CrashFile %s (?????)",
             crashLog.c_str());
    } else {
  ...
  // RAW userPos
  myinfo("");
  myinfo("**** RAW Crashs Pos (copy in excel, and use "
         "insert/chart/(X/Y)Scatter):");
  for (uint i = 0; i < userPosArray.size(); i++) {
    myinfo("%.2f\t%.2f\t%.2f", userPosArray[i].x, userPosArray[i].y,
           userPosArray[i].z);
  }
}

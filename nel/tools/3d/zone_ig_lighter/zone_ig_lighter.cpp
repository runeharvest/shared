  ...
  if (argc < 5) {
    ...
    if (inputFile.open(argv[1])) {
      ...
      try {
        ...
        // OverSampling
        CConfigFile::CVar &ig_oversampling =
            parameter.getVar("ig_oversampling");
        lighterDesc.OverSampling = ig_oversampling.asInt();
        // validate value: 0, 2, 4, 8, 16
        lighterDesc.OverSampling =
            raiseToNextPowerOf2(lighterDesc.OverSampling);
        ...
        if (loadInstanceGroup) {
          ...
          try {
            CConfigFile::CVar &additionnal_ig =
                parameter.getVar("additionnal_ig");
        ...
        while (ite != instanceGroup.end()) {
          ...
          for (uint instance = 0; instance < group->getNumInstance();
            ...
            // Skip it?? use the DontCastShadowForExterior flag. See doc of this
            // flag
            ...
            if (toLowerAscii(CFile::getExtension(name)) == "pacs_prim") {
              nlwarning("EXPORT BUG: Can't read %s (not a shape), should not "
                        "be part of .ig!",
                        name.c_str());
              continue;
            }
            ...
            if (!name.empty()) {
              ...
              if (iteMap != shapeMap.end()) {
                ...
                // If centerInstanceGroup, take good instanceId, to avoid
                // selfShadowing
                sint instanceId;
        ...
        if (continu) {
          ...
          // Light the zone
          lighter.light(*centerInstanceGroup, output, lighterDesc,
                        vectorTriangle, landscape);

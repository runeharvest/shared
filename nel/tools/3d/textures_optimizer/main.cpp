...
void writeInstructions() {
  ...
  std::cout << std::endl;
  std::cout
      << "  Try to optimize TGA or PNG textures by removing useless alpha "
         "channel or converting a RGB with black and white values to grayscale"
      << std::endl;
  std::cout << "  By default, it only make checks and display if texture can "
               "optimized or not"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "-m : Apply mask optimizations (convert to grayscale using red "
               "value and remove alpha)"
            << std::endl;
  std::cout << std::endl;
    ...
    if (option.length() > 0) {
      ...
      // Filename
        ...
        if (ext == "png" || ext == "tga") {
          ...
          nlwarning("Only PNG and TGA files supported, %s won't be processed",
                    option.c_str());
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  for (uint i = 0; i < InputFilenames.size(); ++i) {
    std::string ext =
        NLMISC::toLowerAscii(NLMISC::CFile::getExtension(InputFilenames[i]));
    ...
    if (MaskOptimizations && (!isGrayscale || hasAlpha)) {
      std::cout << InputFilenames[i] << " (mask with wrong format)"
                << std::endl;
      ...
      if (!isGrayscale && bitmap.isGrayscale()) {
        std::cout << InputFilenames[i] << " (grayscale image with RGB colors)"
                  << std::endl;

        if (FixGrayscale) {
          if (!bitmap.convertToType(hasAlpha ? NLMISC::CBitmap::AlphaLuminance
      ...
      if (hasAlpha && bitmap.isAlphaUniform(&alpha)) {
        std::cout << InputFilenames[i] << " (image with uniform alpha channel "
                  << (sint)alpha << ")" << std::endl;

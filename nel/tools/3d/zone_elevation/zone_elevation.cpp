...
NLMISC::CVector getHeightNormal(float x, float y) {
  ...
  // compute a good delta to compute tangents of the heightmap: 1/10 of a pixel,
...
// ***************************************************************************
// void CExport::computeSubdividedTangents(uint numBinds, const
// NL3D::CBezierPatch &patch, uint edge, NLMISC::CVector subTangents[8])
void computeSubdividedTangents(uint numBinds, const NL3D::CBezierPatch &patch,
                               uint edge, NLMISC::CVector subTangents[8]) {
  // Subdivide the Bezier patch to get the correct tangents to apply to
  // neighbors
  ...
  // Subdivide one time.
  if (subDivideOnS)
    patch.subdivideS(subPatchs1_2[0], subPatchs1_2[1]);
  else
    patch.subdivideT(subPatchs1_2[0], subPatchs1_2[1]);
...
// ***************************************************************************
// bool CExport::applyVertexBind(NL3D::CPatchInfo &pa, NL3D::CPatchInfo &oldPa,
// uint edgeToModify, bool startEdge, ...
bool applyVertexBind(NL3D::CPatchInfo &pa, NL3D::CPatchInfo &oldPa,
                     uint edgeToModify, bool startEdge,
                     const NLMISC::CMatrix &oldTgSpace,
                     const NLMISC::CMatrix &newTgSpace,
                     const NLMISC::CVector &bindedPos,
                     const NLMISC::CVector &bindedTangent) {
  ...
  if (pa.Patch.Vertices[vertexToModify] == bindedPos)
    ...
    tgToModify &= 7;
    /* To keep the same continuity aspect around the vertex, we compute the
    original tangent in a special space: the Binded Patch Tangent Space. Once we
    have the original tangent in the original patch TgSpace, we reapply it in
    the transformed patch TgSpace, to get the transformed tangent
    */
    pa.Patch.Tangents[tgToModify] =
        newTgSpace * (oldTgSpace.inverted() * oldPa.Patch.Tangents[tgToModify]);

    // Do the same to the associated interior.
    pa.Patch.Interiors[vertexToModify] =
        newTgSpace *
        (oldTgSpace.inverted() * oldPa.Patch.Interiors[vertexToModify]);
...
void applyZoneHeightmap() {
  ...
  // Apply the Heighmap to all vertices/tangents/interiors (see Land Export
  // tool.)
  for (size_t i = 0; i < zonePatches.size(); ++i) {
    ...
    for (size_t j = 0; j < 4; ++j) {
      ...
      float height =
          getHeight(rPI.Patch.Vertices[j].x, rPI.Patch.Vertices[j].y);
      rPI.Patch.Vertices[j].z += height;
    }

    // Interior and tangent are rotated to follow the heightmap normal, avoiding
    // the "Stair Effect". Compute the matrix to apply to interiors and
    // tangents.
    CMatrix tgMatrix[4];
    for (size_t j = 0; j < 4; ++j) {
      // compute the normal of the heightmap.
      CVector hmapNormal =
          getHeightNormal(rPI.Patch.Vertices[j].x, rPI.Patch.Vertices[j].y);

      // Compute the rotation which transforms the original normal: (0,0,1), to
      // this normal.
  ...
  // See `void CExport::transformZone (CZone &zeZone, sint32 nPosX, sint32
  // nPosY, uint8 nRot, uint8 nFlip, bool computeHeightmap)`
  // 3. For all binds, reset the position of near vertices/tangents/interiors.
  // Must do it at last
  // --------
  bool bindVertexModified = true;
  // Since this is a recursive problem (binded patchs may bind other patchs), do
  // it unitl all vertices no more move :)
      ...
      for (size_t j = 0; j < 4; ++j) {
        ...
        if (numBinds == 2 || numBinds == 4) {
          ...
          for (uint vb = 0; vb < numBinds - 1; vb++) {
            ...
            // The first patch (CCW) must change the vertex which starts on the
            // edge (CCW)
            ...
            bindVertexModified |= applyVertexBind(
                zonePatches[ngbId], oldPatchInfos[ngbId], edgeToModify, true,
                oldTgSpace, newTgSpace, bindedPos, bindedTangent);

            // The second patch (CCW) must change the vertex which ends on the
            // edge (CCW)
            ...
            bindVertexModified |= applyVertexBind(
                zonePatches[ngbId], oldPatchInfos[ngbId], edgeToModify, false,
                oldTgSpace, newTgSpace, bindedPos, bindedTangent);
          }
        }
      }
...
int main(int argc, char **argv) {
  ...
  // TODO: args.addArg("", "batch", "", "Process all zones in input (specify
  // input as C:/folder/.zonenhw)");
  ...
  if (args.haveLongArg("heightmap")) {
    ...
    try {
      ...
      if (inFile.open(args.getLongArg("heightmap")[0])) {
        ...
        nldebug("Cant load height map: %s",
                args.getLongArg("heightmap")[0].c_str());
        delete s_HeightMap;
        s_HeightMap = NULL;
      }
  ...
  if (args.haveLongArg("heightmap2")) {
    ...
    try {
      ...
      if (inFile.open(args.getLongArg("heightmap2")[0])) {
        ...
        nldebug("Cant load height map: %s",
                args.getLongArg("heightmap2")[0].c_str());
        delete s_HeightMap2;
        s_HeightMap2 = NULL;
      }
    } catch (const Exception &) {

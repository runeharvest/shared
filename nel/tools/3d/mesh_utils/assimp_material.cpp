...
void assimpMaterial(NL3D::CMaterial &mat, CMeshUtilsContext &context,
  ...
  if (am->Get(AI_MATKEY_BLEND_FUNC, i) == aiReturn_SUCCESS)
    switch ((aiBlendMode)i) {
  ...
  if (texCount > IDRV_MAT_MAXTEXTURES) {
    tlwarning(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Material '%s' has more than %i textures (%i textures found)",
              amname.C_Str(), IDRV_MAT_MAXTEXTURES, texCount);
  ...
  for (unsigned int ti = 0; ti < texCount; ++ti) {
    ...
    if (am->GetTexture(aiTextureType_DIFFUSE, ti, &path, &mapping, &uvindex,
                       &blend, &op, &mapmode) != aiReturn_SUCCESS) {
    ...
    std::string fileName =
        CFile::getFilename(CPath::standardizePath(path.C_Str(), false));
    std::string knownPath = CPath::lookup(fileName, false, false, false);
    if (knownPath.empty()) {
      tlwarning(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
                "Texture '%s' referenced in material '%s' but not found in the "
                "database search paths",
                fileName.c_str(), amname.C_Str());
    }

    // NeL supports bitmap and cubemap, but we import only basic bitmap here.
    // Cubemap can be inserted from the mesh editor tool NeL also has fancy
    // multi-bitmap thing to switch between summer and winter and so on. Same
    // story
    CSmartPtr<CTextureFile> tex = new CTextureFile();
    tex->setFileName(fileName);
    tex->setWrapS(mapmode == aiTextureMapMode_Clamp ? ITexture::Clamp
                                                    : ITexture::Repeat);
    tex->setWrapT(mapmode == aiTextureMapMode_Clamp ? ITexture::Clamp
                                                    : ITexture::Repeat);
    mat.setTexture(ti, tex);

    // TODO uvindex for uv routing (probably necessary during shape import - if
    // so also need to also ask the uv channel in the editor and store in meta)
    ...
    mat.texEnvArg1Alpha(ti, ti == 0 ? CMaterial::Diffuse : CMaterial::Previous,
                        CMaterial::SrcAlpha);
    mat.texEnvArg1RGB(ti, ti == 0 ? CMaterial::Diffuse : CMaterial::Previous,
                      CMaterial::SrcColor);
    switch (op) {
...
CSmartPtr<CMaterial> assimpMaterial(CMeshUtilsContext &context,
                                    const aiMaterial *am) {

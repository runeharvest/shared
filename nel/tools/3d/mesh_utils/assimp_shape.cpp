...
inline CUVW convUvw(const aiVector3D &av) {
  return CUVW(av.x, -av.y,
              av.z); // UH OH COORDINATE CONVERSION ?! ONLY FOR TEXTURES !!
}
...
void assimpBuildBaseMesh(CMeshBase::CMeshBaseBuild &buildBaseMesh,
  ...
  for (unsigned int mi = 0; mi < node->mNumMeshes; ++mi) {
    ...
    const aiMaterial *am =
        context.InternalScene->mMaterials[mesh->mMaterialIndex];

    aiString amname;
    if (am->Get(AI_MATKEY_NAME, amname) != aiReturn_SUCCESS) {
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Material used by node '%s' has no name",
              node->mName.C_Str()); // TODO: Maybe autogen names by index in
                                    // mesh or node if this is actually a thing
      assimpMaterial(buildBaseMesh.Materials[mi], context, am);
    } else {
      buildBaseMesh.Materials[mi] =
          *context.SceneMeta.Materials[amname.C_Str()];
    }
  }

  // Positioning
  const aiMatrix4x4 &root = context.InternalScene->mRootNode->mTransformation;
  const aiMatrix4x4 &tf =
      nodeContext.InternalNode->mTransformation; // COORDINATE CONVERSION HERE
  ...
  if (buildBaseMesh.DefaultScale.x != 1.0f ||
      buildBaseMesh.DefaultScale.y != 1.0f ||
      buildBaseMesh.DefaultScale.z != 1.0f) {
    tlmessage(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Node '%s' has a scaled transformation. This may be a mistake",
              node->mName.C_Str());
  }
...
bool assimpBuildMesh(CMesh::CMeshBuild &buildMesh,
  ...
  for (unsigned int mi = 0; mi < node->mNumMeshes; ++mi) {
    // TODO: Maybe needs to be the same count too for all meshes, so compare
    // with mesh 0
    const aiMesh *mesh = context.InternalScene->mMeshes[node->mMeshes[mi]];
    ...
    if (mesh->GetNumUVChannels() > CVertexBuffer::MaxStage) {
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "(%s) mesh->GetNumUVChannels() > CVertexBuffer::MaxStage",
              node->mName.C_Str());
      return false;
    }
    if (!mesh->HasNormals()) {
  ...
  // Meshes in assimp are separated per material, so we need to re-merge them
  // for the mesh build process This process also deduplicates vertices
  bool cleanupMesh = true;
  sint32 numVertices = 0;
  for (unsigned int mi = 0; mi < node->mNumMeshes; ++mi)
    numVertices +=
        context.InternalScene->mMeshes[node->mMeshes[mi]]->mNumVertices;
  ...
  for (unsigned int mi = 0; mi < node->mNumMeshes; ++mi) {
    ...
    for (unsigned int vi = 0; vi < mesh->mNumVertices; ++vi) {
      ...
      if (vecit == vertexIdentifiers.end()) {
        ...
        if (cleanupMesh)
          vertexIdentifiers[vec] = numVertices; // Don't remap if we don't wan't
                                                // to lose vertex indices
  ...
  numFaces = 0;
  unsigned int refNumColorChannels =
      context.InternalScene->mMeshes[node->mMeshes[0]]->GetNumColorChannels();
  unsigned int refNumUVChannels =
      context.InternalScene->mMeshes[node->mMeshes[0]]->GetNumUVChannels();
  for (unsigned int mi = 0; mi < node->mNumMeshes; ++mi) {
    const aiMesh *mesh = context.InternalScene->mMeshes[node->mMeshes[mi]];
    ...
    if (numColorChannels > 2) {
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Shape '%s' has too many color channels in mesh %i (%i channels "
              "found)",
              node->mName.C_Str(), mi, numColorChannels);
    }
    ...
    if (numUVChannels > CVertexBuffer::MaxStage) {
      tlerror(
          context.ToolLogger, context.Settings.SourceFilePath.c_str(),
          "Shape '%s' has too many uv channels in mesh %i (%i channels found)",
          node->mName.C_Str(), mi, numUVChannels);
    ...
    for (unsigned int ui = 0; ui < numUVChannels; ++ui)
      buildMesh.VertexFlags |= (CVertexBuffer::TexCoord0Flag
                                << ui); // TODO: Coord UV tex stage rerouting

    // TODO: Channels do in fact differ between submeshes, so we need to
    // correctly recount and reroute the materials properly
    if (numColorChannels != refNumColorChannels)
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Shape '%s' mismatch of nb color channel in mesh '%i', please "
              "contact developer",
              node->mName.C_Str(), mi);
    if (numUVChannels != refNumUVChannels)
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Shape '%s' mismatch of nb uv channel in mesh '%i', please "
              "contact developer",
              node->mName.C_Str(), mi);
    ...
    for (unsigned int fi = 0; fi < mesh->mNumFaces; ++fi) {
      ...
      if (af.mNumIndices != 3) {
        tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
                "(%s) Face %i on mesh %i has %i faces", node->mName.C_Str(), fi,
                mi, af.mNumIndices);
        continue; // return false; Keep going, just drop the face for better
                  // user experience
      }
      if (cleanupMesh) {
        if (vertexRemapping[mi][af.mIndices[0]] ==
                vertexRemapping[mi][af.mIndices[1]] ||
            vertexRemapping[mi][af.mIndices[1]] ==
                vertexRemapping[mi][af.mIndices[2]] ||
            vertexRemapping[mi][af.mIndices[2]] ==
                vertexRemapping[mi][af.mIndices[0]])
      ...
      // TODO: If we want normal maps, we need to add tangent vectors to CFace
      // and build process UV channels
      for (unsigned int ui = 0; ui < numUVChannels; ++ui) // TODO: UV Rerouting
      {
        face.Corner[0].Uvws[ui] =
            convUvw(mesh->mTextureCoords[ui][af.mIndices[0]]);
        face.Corner[1].Uvws[ui] =
            convUvw(mesh->mTextureCoords[ui][af.mIndices[1]]);
        face.Corner[2].Uvws[ui] =
            convUvw(mesh->mTextureCoords[ui][af.mIndices[2]]);
      }
      for (unsigned int ui = numUVChannels; ui < CVertexBuffer::MaxStage;
           ++ui) {
  ...
  if (numFaces != buildMesh.Faces.size()) {
    tlmessage(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Removed %u degenerate faces in shape '%s'",
              (uint32)(buildMesh.Faces.size() - numFaces), node->mName.C_Str());
    buildMesh.Faces.resize(numFaces);
  }

...
void importShapes(CMeshUtilsContext &context, const aiNode *node) {
  if (node != context.InternalScene->mRootNode) {
    ...
    if (nodeMeta.ExportMesh == TMeshShape && nodeMeta.InstanceName.empty()) {
      if (node->mNumMeshes) {
        nldebug("Shape '%s' found containing '%u' meshes", node->mName.C_Str(),
                node->mNumMeshes);
...
void flagMetaBones(CMeshUtilsContext &context) {
  for (TNodeContextMap::iterator it(context.Nodes.begin()),
       end(context.Nodes.end());
       it != end; ++it) {
...
void flagLocalParentBones(CMeshUtilsContext &context,
...
bool hasIndirectParentBone(CMeshUtilsContext &context,
                           CNodeContext &nodeContext) {
  const aiNode *parent = nodeContext.InternalNode;
  while (parent = parent->mParent)
    if (parent->mName.length && parent != context.InternalScene->mRootNode)
      if (context.Nodes[parent->mName.C_Str()].IsBone)
        return true;
  return false;
}
  ...
  for (TNodeContextMap::iterator it(context.Nodes.begin()),
    ...
    if (nodeContext.Shape) {
      std::string shapePath =
          NLMISC::CPath::standardizePath(
              context.Settings.DestinationDirectoryPath, true) +
          it->first + ".shape";
      ...
      if (NL3D::CMeshBase *mesh =
              dynamic_cast<NL3D::CMeshBase *>(nodeContext.Shape.getPtr())) {
        for (uint mi = 0; mi < mesh->getNbMaterial(); ++mi) {
          ...
          for (uint ti = 0; ti < NL3D::IDRV_MAT_MAXTEXTURES; ++ti) {
            if (NL3D::ITexture *itex = mat.getTexture(ti)) {
              if (NL3D::CTextureFile *tex =
                      dynamic_cast<NL3D::CTextureFile *>(itex)) {
                std::string fileName = tex->getFileName();
                std::string knownPath =
                    NLMISC::CPath::lookup(fileName, false, false, false);
                if (!knownPath.empty()) {
                  context.ToolLogger.writeDepend(NLPIPELINE::RUNTIME,
                                                 shapePath.c_str(),
                                                 knownPath.c_str());
                } else {
                  // TODO: Move this warning into nelmeta serialization so it's
                  // shown before export
                  tlwarning(context.ToolLogger,
                            context.Settings.SourceFilePath.c_str(),
                            "Texture '%s' referenced in material but not found "
                            "in the database search paths",
                            fileName.c_str());
                }
              }
            }
...
int exportScene(const CMeshUtilsSettings &settings) {
  ...
  context.ToolLogger.writeDepend(
      NLPIPELINE::BUILD, "*",
      NLMISC::CPath::standardizePath(context.Settings.SourceFilePath, false)
          .c_str()); // Base input file

  // Apply database configuration
  if (!NLPIPELINE::CProjectConfig::init(
          settings.SourceFilePath,
          NLPIPELINE::CProjectConfig::DatabaseTextureSearchPaths, true)) {
    tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
            "Unable to find database.cfg in input path or any of its parents.");
    // return EXIT_FAILURE; We can continue but the output will not be
    // guaranteed...
  }
  ...
  const aiScene *scene = importer.ReadFile(
      settings.SourceFilePath,
      0 | aiProcess_Triangulate | aiProcess_ValidateDataStructure |
          aiProcess_GenNormals // Or GenSmoothNormals? TODO: Validate smoothness
                               // between material boundaries!
  ); // aiProcess_SplitLargeMeshes | aiProcess_LimitBoneWeights
  if (!scene) {
    const char *errs = importer.GetErrorString();
    if (errs)
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
              "Assimp failed to load the scene: '%s'", errs);
    else
      tlerror(context.ToolLogger, context.Settings.SourceFilePath.c_str(),
  ...
  // aiProcess_RemoveRedundantMaterials: Not used because we may override
  // materials with NeL Material from meta aiProcess_ImproveCacheLocality: TODO:
  // Verify this does not modify vertex indices scene->mRootNode->mMetaData

  context.InternalScene = scene;
  if (context.SceneMeta.load(context.Settings.SourceFilePath))
    context.ToolLogger.writeDepend(
        NLPIPELINE::BUILD, "*",
        context.SceneMeta.metaFilePath().c_str()); // Meta input file
  ...
  // For each shape test if all the bones have the same root bones for their
  // skeleton 1) Iterate each until a different is found 2) When a different
  // root is found, connect the two to the nearest common bone
  // ]
  // -- SKEL FLAG --

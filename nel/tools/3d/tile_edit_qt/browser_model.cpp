...
// Rotate a buffer
void rotateBuffer(std::vector<NLMISC::CBGRA> &Buffer, uint &Width,
                  uint &Height) {
  // Make a copy
...
int loadPixmapBuffer(const std::string &path,
                     std::vector<NLMISC::CBGRA> &Buffer,
...
bool TileInfo::Load(int index, std::vector<NLMISC::CBGRA> *Alpha) {
  ...
  if (!loaded && !getRelativeFileName(Diffuse, index).empty()) {
    path = fixPath(tileBankBrowser.getAbsPath() +
                   getRelativeFileName(Diffuse, index));

    if (!loadPixmapBuffer(path, Bits, Alpha, 0)) {
      bRes = false;
      QMessageBox::information(NULL, QObject::tr("Can't load Diffuse file"),
                               QString((path.c_str())));
    } else
      loaded = 1;
  }
  if (!nightLoaded && !getRelativeFileName(Additive, index).empty()) {
    nightPath = fixPath(tileBankBrowser.getAbsPath() +
                        getRelativeFileName(Additive, index));
    if (!loadPixmapBuffer(nightPath, nightBits, Alpha, 0)) {
      bRes = false;
      QMessageBox::information(NULL, QObject::tr("Can't load Additive file"),
                               QString(nightPath.c_str()));
    } else
      nightLoaded = 1;
  }
  if (!alphaLoaded && !getRelativeFileName(::Alpha, index).empty()) {
    alphaPath = fixPath(tileBankBrowser.getAbsPath() +
                        getRelativeFileName(::Alpha, index));
    if (!loadPixmapBuffer(alphaPath, alphaBits, NULL,
                          tileBankBrowser.getTile(index)->getRotAlpha())) {
      bRes = false;
      QMessageBox::information(NULL, QObject::tr("Can't load Alpha file"),
                               QString(alphaPath.c_str()));
    } else
      alphaLoaded = 1;
  }
...
const std::string TileInfo::getRelativeFileName(TileTexture texture,
                                                int index) {
  nlassert(this->tileType != UnSet);

  std::string currentPath;
  if (tileType != Displace) {
    currentPath = tileBankBrowser.getTile(index)->getRelativeFileName(
        (CTile::TBitmap)texture);
  } else {
    if (texture == Diffuse) {
      currentPath = tileBankBrowser.getDisplacementMap(index);

      // TODO titegus: remove MAGIC STRING "EmptyDisplacementMap" in Nel
      // \tile_bank.cpp\void CTileNoise::setEmpty () or add an IsEmpty() method
      // !!!!
...
bool TileList::setTile128(int tile, const std::string &name,
  ...
  if (RemovePath(troncated, tileBankBrowser.getAbsPath().c_str())) {
    ...
    uint Width;
    uint Height;
    if (!loadPic(tileBankBrowser.getAbsPath() + troncated, tampon, Width,
                 Height)) {
      return (QMessageBox::Yes ==
              QMessageBox::question(
                  NULL, QObject::tr("Can't load bitmap."),
                  QString(((tileBankBrowser.getAbsPath() + troncated) +
                           "\nContinue ?")
                              .c_str()),
      ...
      if (type == CTile::additive)
        ...
        error = tileBankBrowser.getTileSet(_tileSet)->checkTile128(
            type, border, pixel, composante);

      if ((error != CTileSet::ok) && (error != CTileSet::addFirstA128128)) {
        QString pixelMessage = QObject::tr("%1\nPixel: %2(%3).\nContinue ?")
                                   .arg(CTileSet::getErrorMessage(error))
                                   .arg(pixel)
                                   .arg(comp[composante]);
        return (QMessageBox::Yes ==
                QMessageBox::question(NULL, QObject::tr("Can't set Bitmap"),
                                      pixelMessage,
                                      QMessageBox::Yes | QMessageBox::No));
      } else {
        ...
        tileBankBrowser.getTileSet(_tileSet)->setTile128(tile, troncated, type,
                                                         tileBankBrowser);
        switch (type) {
        case CTile::diffuse:
    ...
    QString notInAbsolutePathMessage =
        QObject::tr("The bitmap %1 is not in the absolute path %2.\nContinue ?")
            .arg(name.c_str())
            .arg(tileBankBrowser.getAbsPath().c_str());
    return (QMessageBox::Yes ==
            QMessageBox::question(NULL, QObject::tr("Load error"),
                                  notInAbsolutePathMessage,
                                  QMessageBox::Yes | QMessageBox::No));
  }
...
bool TileList::setTile256(int tile, const std::string &name,
                          NL3D::CTile::TBitmap type) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBankBrowser.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBankBrowser.getAbsPath() + troncated, tampon, Width,
      ...
      if (type == CTile::additive)
        ...
        error = tileBankBrowser.getTileSet(_tileSet)->checkTile256(
            type, border, pixel, composante);
      if ((error != CTileSet::ok)) {
        ...
        tileBankBrowser.getTileSet(_tileSet)->setTile256(tile, troncated, type,
                                                         tileBankBrowser);
        switch (type) {
        case CTile::diffuse:
...
bool TileList::setTileTransition(int tile, const std::string &name,
                                 NL3D::CTile::TBitmap type) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBankBrowser.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBankBrowser.getAbsPath() + troncated, tampon, Width,
      ...
      if ((error != CTileSet::ok) && (error != CTileSet::addFirstA128128)) {
        ...
        if (error == CTileSet::addFirstA128128)
          tileBankBrowser.getTileSet(_tileSet)->setBorder(type, border);
        tileBankBrowser.getTileSet(_tileSet)->setTileTransition(
            (CTileSet::TTransition)tile, troncated, type, tileBankBrowser,
            border);
        switch (type) {
        case CTile::diffuse:
...
bool TileList::setDisplacement(int tile, const std::string &name,
                               NL3D::CTile::TBitmap type) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBankBrowser.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBankBrowser.getAbsPath() + troncated, tampon, Width,
      ...
      if ((Width != 32) || (Height != 32)) {
        ...
        return (QMessageBox::Yes ==
                QMessageBox::question(
                    NULL, QObject::tr("Can't set Bitmap"),
                    QString((troncated + "\nInvalid size: displacement map "
                                         "must be 32x32 8 bits.\nContinue ?")
                                .c_str()),
                    QMessageBox::Yes | QMessageBox::No));
      } else {
        // change the file name of the displacement map
        tileBankBrowser.getTileSet(_tileSet)->setDisplacement(
            (CTileSet::TDisplacement)tile, troncated, tileBankBrowser);
...
bool TileList::setTileTransitionAlpha(int tile, const std::string &name,
                                      int rot) {
  // Remove the absolute path from the path name
  std::string troncated = name;
  if (RemovePath(troncated, tileBankBrowser.getAbsPath().c_str())) {
    ...
    if (!loadPic(tileBankBrowser.getAbsPath() + troncated, tampon, Width,
      ...
      int pixel = -1;
      int composante = 4;
      if (((error = tileBankBrowser.getTileSet(_tileSet)->checkTileTransition(
        ...
        tileBankBrowser.getTileSet(_tileSet)->setTileTransitionAlpha(
            (CTileSet::TTransition)tile, troncated, tileBankBrowser, border,
            rot);
        theListTransition[tile].alphaLoaded = 0;
        theListTransition[tile].Load(tileBankBrowser.getTileSet(_tileSet)
                                         ->getTransition(tile)
                                         ->getTile(),
                                     NULL);
      }
...
void TileList::clearTile128(int index, CTile::TBitmap bitmap) {
  ...
  tileBankBrowser.getTileSet(_tileSet)->clearTile128(index, bitmap,
                                                     tileBankBrowser);
}
...
void TileList::clearTile256(int index, CTile::TBitmap bitmap) {
  ...
  tileBankBrowser.getTileSet(_tileSet)->clearTile256(index, bitmap,
                                                     tileBankBrowser);
}
...
void TileList::clearTransition(int index, CTile::TBitmap bitmap) {
  ...
  tileBankBrowser.getTileSet(_tileSet)->clearTransition(
      (CTileSet::TTransition)index, bitmap, tileBankBrowser);
}
...
void TileList::clearDisplacement(int index, CTile::TBitmap bitmap) {
  ...
  tileBankBrowser.getTileSet(_tileSet)->clearDisplacement(
      (CTileSet::TDisplacement)index, tileBankBrowser);
}

tilelist::iterator TileList::GetFirst(int n) { return theList[n].begin(); }

tilelist::iterator TileList::GetLast(int n) { return theList[n].end(); }

tilelist::iterator TileList::Get(int i, int n) {
  return theList[n].begin() + i;
}

int TileList::GetSize(int n) { return (int)theList[n].size(); }

void TileList::Reload(int first, int last,
                      TileType n) // recharge en memoire une tranche de tiles
{
  for (int i = first; i < last; i++) {
    switch (n) {
    case _128x128: {
      theList[n][i].Load(tileBankBrowser.getTileSet(_tileSet)->getTile128(i),
                         NULL);
      break;
    }
    case _256x256: {
      theList[n][i].Load(tileBankBrowser.getTileSet(_tileSet)->getTile256(i),
                         NULL);
      break;
    }
    case Transition: {
      int index =
          tileBankBrowser.getTileSet(_tileSet)->getTransition(i)->getTile();
      if (index != -1)
        theList[n][i].Load(index, &theListTransition[i].alphaBits);
      break;
    }
    case Displace: {
      int index = tileBankBrowser.getTileSet(_tileSet)->getDisplacementTile(
          (CTileSet::TDisplacement)i);
      if (index != -1)

...
TileModel::TileModel()
    : pixmapSide(128), tileLabel(QObject::tr("Right-Click to select Bitmap")),
      index(-1) {}

TileModel::TileModel(const QPixmap &pixmap, QString tileLabel, int index)
    : pixmap(pixmap), tileLabel(tileLabel), index(index) {
  pixmapSide = pixmap.width();
}

TileModel::TileModel(int pixmapSide, QString tileLabel, int index)
    : pixmapSide(pixmapSide), index(index) {
...
tiles_model::tiles_model(QObject *parent) : QAbstractListModel(parent) {}
...
QVariant tiles_model::data(const QModelIndex &index, int role) const {
  ...
  if (role == Qt::DecorationRole || role == Qt::UserRole) {
    CTile_Widget wiwi;
    wiwi.initWidget(tiles.value(index.row()).getPixmap(),
                    tiles.value(index.row()).getPixmapSide(),
                    tiles.value(index.row()).getTileLabel());
#ifdef USE_QT5

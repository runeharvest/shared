...
void CTile_browser_dlg::initDialog(const int &tileSetIndex) {
  ...
  tileLabelButtonGroup->addButton(ui.indexRadioButton,
                                  CTile_browser_dlg::Index);
  tileLabelButtonGroup->addButton(ui.fileNameRadioButton,
                                  CTile_browser_dlg::FileName);
  ...
  tileZoomButtonGroup->addButton(ui.normalRadioButton,
                                 CTile_browser_dlg::Normal);
  ...
  if (settings.contains("browser/TileType"))
    tileTypeButtonGroup->button(settings.value("browser/TileType").toInt())
        ->setChecked(true);

  if (settings.contains("browser/TileTexture"))
    tileTextureButtonGroup
        ->button(settings.value("browser/TileTexture").toInt())
        ->setChecked(true);

  if (settings.contains("browser/TileLabel"))
    tileLabelButtonGroup->button(settings.value("browser/TileLabel").toInt())
        ->setChecked(true);

  if (settings.contains("browser/TileZoom"))
    tileZoomButtonGroup->button(settings.value("browser/TileZoom").toInt())
        ->setChecked(true);

  // GroupBox checkedIdChanged subscription
  connect(tileTypeButtonGroup, SIGNAL(buttonClicked(int)), this,
          SLOT(on_tileTypeButtonGroup_clicked(int)));
  connect(tileTextureButtonGroup, SIGNAL(buttonClicked(int)), this,
          SLOT(on_tileTextureButtonGroup_clicked(int)));
  connect(tileLabelButtonGroup, SIGNAL(buttonClicked(int)), this,
          SLOT(on_tileLabelButtonGroup_clicked(int)));
  connect(tileZoomButtonGroup, SIGNAL(buttonClicked(int)), this,
          SLOT(on_tileZoomButtonGroup_clicked(int)));

  // Tile View Model
  tileViewModel = new tiles_model(this);
  ui.tileBrowserListView->setModel(tileViewModel);
  connect(
      ui.tileBrowserListView->selectionModel(),
      SIGNAL(selectionChanged(const QItemSelection &, const QItemSelection &)),
      this,
      SLOT(on_tiles_model_selectionChanged(const QItemSelection &,
...
void CTile_browser_dlg::on_tiles_model_selectionChanged(
    const QItemSelection &s, const QItemSelection &d) {
  if (!ui.tileBrowserListView->selectionModel()->selectedRows().empty()) {
    ...
    if (tileTypeButtonGroup->checkedId() == _128x128 ||
        tileTypeButtonGroup->checkedId() == _256x256) {
...
void CTile_browser_dlg::on_actionAddTile_triggered(bool checked) {
  ...
  QStringList fileNames = QFileDialog::getOpenFileNames(
      this, "Choose Bitmap", QString(tileBankBrowser.getAbsPath().c_str()),
      "All supported files (*.png;*.tga);;PNG Bitmap (*.png);;Targa "
      "Bitmap(*.tga);;All Files (*.*);;",
  ...
  if (!fileNames.isEmpty()) {
    ...
    case _128x128:
      for (int i = 0; i < fileNames.size(); i++) {
        ...
        if (!browserModel.setTile128(
                tileId, fileName.toUtf8().constData(),
                (CTile::TBitmap)tileTextureButtonGroup->checkedId())) {
...
void CTile_browser_dlg::on_actionDeleteTile_triggered(bool checked) {
  ...
  case _128x128:
    for (int i = 0;
         i < ui.tileBrowserListView->selectionModel()->selectedRows().count();
         i++) {
      int tileId =
          (ui.tileBrowserListView->selectionModel()->selectedRows().at(i).data(
               Qt::UserRole + 1))
              .toInt();
      browserModel.removeTile128(tileId);
    }
  ...
  if (!fileName.isEmpty()) {
    ...
    case _128x128:
      for (int i = 0;
           i < ui.tileBrowserListView->selectionModel()->selectedRows().count();
           i++) {
        int tileId = (ui.tileBrowserListView->selectionModel()
                          ->selectedRows()
                          .at(i)
                          .data(Qt::UserRole + 1))
                         .toInt();
    ...
    case Transition:
      for (int i = 0;
        ...
        int tileId = (ui.tileBrowserListView->selectionModel()
                         ...
                         .toInt();
        if (tileTextureButtonGroup->checkedId() != Alpha) {
          if (!browserModel.setTileTransition(
                  tileId, fileName.toUtf8().constData(),
                  (CTile::TBitmap)tileTextureButtonGroup->checkedId()))
            break;
        } else {
          ...
          if (rotationOk) {
            if (!browserModel.setTileTransitionAlpha(
                    tileId, fileName.toUtf8().constData(), rot))
              break;
          }
        }
      }
    ...
    case Displace:
      for (int i = 0;
        ...
        int tileId = (ui.tileBrowserListView->selectionModel()
                          ...
                          .data(Qt::UserRole + 1))
                         .toInt();
        if (!browserModel.setDisplacement(
                tileId, fileName.toUtf8().constData(),
                (CTile::TBitmap)tileTextureButtonGroup->checkedId()))
          break;
      }
    ...
    for (int i = 0;
      ...
      int tileId =
          (ui.tileBrowserListView->selectionModel()->selectedRows().at(i).data(
               Qt::UserRole + 1))
              .toInt();
      browserModel.clearTile128(
          tileId, (CTile::TBitmap)tileTextureButtonGroup->checkedId());
    }
    break;
  case _256x256:
    for (int i = 0;
    ...
    for (int i = 0;
      ...
      int tileId =
          (ui.tileBrowserListView->selectionModel()->selectedRows().at(i).data(
               Qt::UserRole + 1))
              .toInt();
      browserModel.clearDisplacement(
          tileId, (CTile::TBitmap)tileTextureButtonGroup->checkedId());
    }
    break;
  default:
...
void CTile_browser_dlg::on_refreshPushButton_clicked() { LoadInThread(); }

void CTile_browser_dlg::on_batchLoadPushButton_clicked() {
  QFileDialog::Options options;
  QString selectedFilter;
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Choose Bitmap"), QString(tileBankBrowser.getAbsPath().c_str()),
      tr("All supported files (*.png;*.tga);;PNG Bitmap (*.png);;Targa Bitmap "
         "(*.tga);;All Files (*.*);;"),
      &selectedFilter, options);
  ...
  if (!fileName.isEmpty()) {
    ...
    // ???
    bool rotate = (QMessageBox::Yes ==
                   QMessageBox::question(
                       this, tr("Import rotated tiles"),
                       tr("Do you want to use rotation to reuse alpha tiles?"),
    ...
    for (int i = 0; i < CTileSet::count; i++) {
      if (tileTextureButtonGroup->checkedId() == Alpha) {
        ...
        CTileSetTransition *trans =
            tileBankBrowser.getTileSet(tileSetIndex)->getTransition(transition);
        if (tileBankBrowser.getTile(trans->getTile())
          ...
          for (int rot = 0; rot < 4; rot++) {
            ...
            QString nextFileName = QDir::toNativeSeparators(fi.absolutePath()) +
                                   QDir::separator() + nextBaseName +
                                   QString(".") + fi.suffix();
            ...
            if (pFile) {
              fclose(pFile);
              ok = browserModel.setTileTransitionAlpha(
                  i, nextFileName.toUtf8().constData(), (4 - rot) % 4);
        ...
        // Transition to patch
        // CTileSetTransition* trans=tileBankBrowser.getTileSet
        // (tileSetIndex)->getTransition (transition); if
        // (tileBankBrowser.getTile (trans->getTile())->getRelativeFileName
        // ((CTile::TBitmap)tileTextureButtonGroup->checkedId()).empty())
        //{
        //	// Try to load a tile with a file name like /tiletransition0.tga
        //	char sName2[256];
        //	char sFinal[256];
        ...
        //	{
        //		fclose (pFile);
        //		if (!infoList.setTileTransition (i, sFinal,
        //(CTile::TBitmap) tileTextureButtonGroup->checkedId())) 			break;
        //	}
        //}
      }
    }
...
// TODO titegus: replace that by 4 buttons Export128Diffuse, Export128Additive,
// Export256Diffuse, Export256Diffuse ?
void CTile_browser_dlg::on_exportBorderPushButton_clicked() {
  ...
  if (!fileName.isEmpty()) {
    ...
    tileBankBrowser.getTileSet(tileSetIndex)
        ->getBorder128((CTile::TBitmap)tileTextureButtonGroup->checkedId())
...
// TODO titegus: What's the point in Importing a new border if there is no Pixel
// Compatibility check ?
void CTile_browser_dlg::on_importBorderPushButton_clicked() {
  ...
  if (!fileName.isEmpty()) {
    ...
    if ((bitmap.getWidth() == 128) && (bitmap.getHeight() == 128)) {
      ...
      // Error message
      QString s =
          tr("The bitmap must have a size of 128x128 (%1)").arg(fileName);
      QMessageBox::information(this, tr("Import border"), s);
    }
    ...
    tileBankBrowser.getTileSet(tileSetIndex)
        ->setBorder((CTile::TBitmap)tileTextureButtonGroup->checkedId(),
                    border);

    // Message
    QMessageBox::information(this, tr("Import border"),
                             tr("The border has been changed."));
  }
}
...
void CTile_browser_dlg::EnableBrowserInteractions() {
  ...
  if (tileTextureButtonGroup->checkedId() == Alpha) {
    ...
    // TODO titegus:Makes no sense, ExportBorder should not be linked to the
    // selected radio button , Replace exportBorder By ExportDiffuse and
    // ExportAdditive??
...
void CTile_browser_dlg::RefreshView() {
  ...
  tilelist::iterator p =
      browserModel.GetFirst(tileTypeButtonGroup->checkedId());
  for (p = browserModel.GetFirst(tileTypeButtonGroup->checkedId());
       p != browserModel.GetLast(tileTypeButtonGroup->checkedId()); p++) {
    ...
    if (tileLoaded) {
      ...
      for (int colIndex = 0; colIndex < bufferHeight; colIndex++) {
        for (int lineIndex = 0; lineIndex < bufferWidth; lineIndex++) {
          image.setPixel(lineIndex, colIndex,
                         qRgb((*bits)[itBufferPixel].R,
      ...
      image = image.scaled(
          tileZoomButtonGroup->checkedId() *
              (tileTypeButtonGroup->checkedId() == _256x256 ? 2 : 1),
          tileZoomButtonGroup->checkedId() *
              (tileTypeButtonGroup->checkedId() == _256x256 ? 2 : 1));
      ...
      TileModel tile =
          TileModel(tileZoomButtonGroup->checkedId() *
                        (tileTypeButtonGroup->checkedId() == _256x256 ? 2 : 1),
                    fileInfo, p->getId());
      tileViewModel->addTile(tile);
    }
...
void CTile_browser_dlg::closeEvent(QCloseEvent *event) {
  int reply = QMessageBox::question(
      this, tr("Quit"),
      tr("Are you sure you want to Quit TileSet Edition without Saving?"),
      QMessageBox::Yes | QMessageBox::No);
...
void CTile_browser_dlg::accept() {
  ...
  settings.setValue("browser/TileTexture",
                    this->tileTextureButtonGroup->checkedId());
  settings.setValue("browser/TileLabel",
                    this->tileLabelButtonGroup->checkedId());

...
bool CheckPath(const std::string &path, const char *absolutePathToRemplace) {
  ...
  if (strnicmp(path.c_str(), absolutePathToRemplace,
               strlen(absolutePathToRemplace)) == 0)
    return true;
  else
    return false;
}
...
void CTile_edit_dlg::initDialog() {
  ui.surfaceDataLineEdit->setValidator(
      new QIntValidator(ui.surfaceDataLineEdit));
}
...
void CTile_edit_dlg::on_addLandPushButton_clicked() {
  ...
  QString text = QInputDialog::getText(
      this, tr("Add Land"), tr("Enter land name:"), QLineEdit::Normal, "", &ok);
  if (ok && !text.isEmpty()) {
    if (ui.landListWidget->findItems(text, Qt::MatchExactly).count() > 0) {
      QMessageBox::information(this, tr("Error Adding Land"),
...
void CTile_edit_dlg::on_editLandPushButton_clicked() {
  ...
  if (nindex != -1) {
    ...
    for (int i = 0; i < tileBank.getTileSetCount(); i++) {
      if (tileBank.getLand(nindex)->isTileSet(
              tileBank.getTileSet(i)->getName()))
        landTileSetList.append(
            QString(tileBank.getTileSet(i)->getName().c_str()));
      else
        availableTileSetList.append(
            QString(tileBank.getTileSet(i)->getName().c_str()));
    }

    bool ok = false;
    QStringList items = CItems_edit_dlg::getItems(
        this, tr("Edit Land"), ui.landListWidget->item(nindex)->text(),
        availableTileSetList, landTileSetList, &ok);

    if (ok) {
      for (int i = 0; i < tileBank.getTileSetCount(); i++) {
        // remove tile set
        tileBank.getLand(nindex)->removeTileSet(
            tileBank.getTileSet(i)->getName());
      }
    ...
    QMessageBox::information(this, tr("No Land Selected"),
                             tr("Please, select the Land to edit first ..."));
  }
}
...
void CTile_edit_dlg::on_deleteLandPushButton_clicked() {
  ...
  if (nindex != -1) {
    ...
    QMessageBox::information(this, tr("No Land Selected"),
                             tr("Please, select the Land to delete first ..."));
  }
}

void CTile_edit_dlg::on_addTileSetPushButton_clicked() {
  bool ok;
  QString text = QInputDialog::getText(this, tr("Add Tile Set"),
                                       tr("Enter Tile Set name:"),
                                       QLineEdit::Normal, "", &ok);
  if (ok && !text.isEmpty()) {
    if (ui.tileSetListWidget->findItems(text, Qt::MatchExactly).count() > 0) {
      QMessageBox::information(this, tr("Error Adding Tile Set"),
                               tr("This name already exists"));
    } else {
...
void CTile_edit_dlg::on_editTileSetPushButton_clicked() {
  ...
  if (nindex != -1) {
    ...
    CTile_browser_dlg *tileBrowser =
        new CTile_browser_dlg((QWidget *)this, (Qt::WindowFlags)0);
    tileBrowser->initDialog(nindex);
    if (tileBrowser->exec()) {
      tileBank = tileBankBrowser;
    }
  } else {
    QMessageBox::information(this, tr("No Tile Set Selected"),
                             tr("Please, select a Tile Set to edit first ..."));
  }
}
void CTile_edit_dlg::on_deleteTileSetPushButton_clicked() {
  ...
  if (nindex != -1) {
    ...
    // WORKAROUND : Qt 4.4 Behaviour (when removing at row 0 and currentRow
    // equals to 0, currentRow is set to 1, it mess with the selectionChanged
    // event
    if (nindex == 0) {
    ...
    QMessageBox::information(
        this, tr("No Tile Set Selected"),
        tr("Please, select a Tile Set to delete first ..."));
  }
}
void CTile_edit_dlg::on_chooseVegetPushButton_clicked() {
  ...
  if (nindex != -1) {
    ...
    QString fileName = QFileDialog::getOpenFileName(
        this, tr("Choose Veget Set"), ui.chooseVegetPushButton->text(),
        tr("NeL VegetSet Files (*.vegetset);;All Files (*.*);;"),
    ...
    QMessageBox::information(this, tr("No Tile Set Selected"),
                             tr("Please, select a Tile Set first ..."));
  }
}
...
void CTile_edit_dlg::on_orientedCheckBox_stateChanged(int state) {
  if (!tileSetSelectionChanging) {
    int nindex = ui.tileSetListWidget->currentRow();
    if (nindex != -1) {
      tileBank.getTileSet(nindex)->setOriented(
          ui.orientedCheckBox->isChecked());
    } else {
      QMessageBox::information(this, tr("No Tile Set Selected"),
                               tr("Please, select a Tile Set first ..."));
    }
...
void CTile_edit_dlg::on_tileSetListWidget_itemSelectionChanged() {
  ...
  if (nindex != -1) {
    if (!tileBank.getTileSet(nindex)->getTileVegetableDescFileName().empty())
      ui.chooseVegetPushButton->setText(QString(
          tileBank.getTileSet(nindex)->getTileVegetableDescFileName().c_str()));
    else
      ui.chooseVegetPushButton->setText("...");

    ui.surfaceDataLineEdit->setText(
        QString::number(tileBank.getTileSet(nindex)->SurfaceData));
...
void CTile_edit_dlg::on_quitPushButton_clicked() { close(); }
void CTile_edit_dlg::on_loadPushButton_clicked() {
  QFileDialog::Options options;
  QString selectedFilter;
  QString fileName = QFileDialog::getOpenFileName(
      this, tr("Open Bank"), ui.absolutePathPushButton->text(),
      tr("NeL tile bank files (*.bank);;All Files (*.*);;"), &selectedFilter,
      options);
...
void CTile_edit_dlg::on_savePushButton_clicked() {
  ...
  QMessageBox::information(this, tr("Error"),
                           tr("Can't Save Bank, check the path"));
}
...
void CTile_edit_dlg::on_saveAsPushButton_clicked() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Save Bank"), this->mainFile.absoluteFilePath(),
      tr("NeL tile bank files (*.bank);;All Files (*.*);;"), &selectedFilter,
      options);
  if (!fileName.isEmpty()) {
    // Set MainFile
...
void CTile_edit_dlg::on_exportPushButton_clicked() {
  ...
  QString fileName = QFileDialog::getSaveFileName(
      this, tr("Export Bank"),
      this->mainFile.absolutePath() + QDir::separator() + tr("*.smallbank"),
      tr("NeL tile small bank files (*.smallbank);;All Files (*.*);;"),
      &selectedFilter, options);
  if (!fileName.isEmpty()) {
    // Copy the bank
    ...
    QMessageBox::information(this, tr("Error"),
                             tr("Can't Export the Bank, check the path"));
  }
}
...
void CTile_edit_dlg::on_absolutePathPushButton_clicked() {
  ...
  QString directory = QFileDialog::getExistingDirectory(
      this, tr("Select the absolute base path of the bank"),
      ui.absolutePathPushButton->text(), options);
  ...
  if (!directory.isEmpty()) {
    ...
    QString confirmMessage =
        tr("Do you really want to set %1 as base path of the bank?").arg(path);
    reply = QMessageBox::question(this, tr("Confirm Path"), confirmMessage,
    ...
    if (reply == QMessageBox::Yes) {
      ...
      if ((*oldPath) == 0) {
          ...
          if (tileSet != -1) {
            ...
            for (type = CTile::diffuse; type < CTile::bitmapCount; type++) {
              ...
              const std::string &bitmapPath =
                  tileBank.getTile(tiles)->getRelativeFileName(
                      (CTile::TBitmap)type);
              ...
              if (!bitmapPath.empty()) {
                ...
                if (CheckPath(bitmapPath, path.toUtf8()) == false) {
                  ...
                  QString continueMessage =
                      tr("Path '%1' can't be found in bitmap '%2'. Continue ?")
                          .arg(path)
                          .arg(QString(bitmapPath.c_str()));
                  reply = QMessageBox::question(
                      this, tr("Continue"), continueMessage,
                      QMessageBox::Yes | QMessageBox::No);
        ...
        // For all tiles, check we can change the path
        for (uint noise = 1; noise < tileBank.getDisplacementMapCount();
             noise++) {
          // Bitmap string
          ...
          if (strcmp(bitmapPath, "") != 0) {
            ...
            if (CheckPath(bitmapPath, path.toUtf8()) == false) {
              ...
              QString continueMessage =
                  tr("Path '%1' can't be found in bitmap '%2'. Continue ?")
                      .arg(path)
                      .arg(QString(bitmapPath));
              reply =
                  QMessageBox::question(this, tr("Continue"), continueMessage,
                                        QMessageBox::Yes | QMessageBox::No);
              if (reply == QMessageBox::No)
                break;
            }
        ...
        if (goodPath) {
          ...
          for (int tiles = 0; tiles < tileBank.getTileCount(); tiles++) {
            ...
            if (tileSet != -1) {
              // 3 types of bitmaps
              for (int type = CTile::diffuse; type < CTile::bitmapCount;
                   type++) {
                // Bitmap string
                ...
                if (!bitmapPath.empty()) {
                  ...
                  // Set the bitmap
                  tileBank.getTile(tiles)->setFileName((CTile::TBitmap)type,
                                                       bitmapPath);
                }
              }
            }
...
void CTile_edit_dlg::closeEvent(QCloseEvent *event) {
  int reply = QMessageBox::question(this, tr("Quit"),

...
// def_visible_primitive( export_transition_zone,
// "NeLLigoExportTransitionZone");
...
def_visible_primitive(check_zone_with_transition,
                      "NeLLigoCheckZoneWithTransition");
...
bool MakeSnapShot(NLMISC::CBitmap &snapshot, const NL3D::CTileBank &tileBank,
                  const NL3D::CTileFarBank &tileFarBank, sint xmin, sint xmax,
                  sint ymin, sint ymax, const CLigoConfig &config,
                  bool errorInDialog);

bool MakeSnapShot(NLMISC::CBitmap &snapshot, const NL3D::CTileBank &tileBank,
                  const NL3D::CTileFarBank &tileFarBank, sint xmax, sint ymax,
                  const CLigoConfig &config, bool errorInDialog);
...
Value *export_material_cf(Value **arg_list, int count) {
  ...
  TCHAR *message = _T("NeLLigoExportMaterial [Object] [Filename] [CheckOnly] ")
                   _T("[Error in dialog]");
  ...
  if (os.obj) {
    ...
    RPO *tri =
        (RPO *)os.obj->ConvertToType(ip->GetTime(), RYKOLPATCHOBJ_CLASS_ID);
    if (tri) {
      // Load the ligofile
      CLigoConfig config;
      if (!CMaxToLigo::loadLigoConfigFile(config, *MAXScript_interface,
                                          errorInDialog)) {
        // Output an error
        CMaxToLigo::errorMessage(
            "Error: can't load the config file ligoscape.cfg",
        ...
        bool res = CMaxToLigo::buildZoneTemplate(node, tri->patch, zoneTemplate,
                                                 config, ScriptErrors[0],
                                                 ip->GetTime());
        ...
        if (res) {
          ...
          if (tri->rpatch->exportZone(node, &tri->patch, zone, zoneSymmetry, 0,
                                      config.CellSize, config.Snap, false)) {
            // Build a material
            ...
            if (res = material.build(zoneTemplate, config, ScriptErrors[0])) {
              ...
              if (!checkOnly) {
                ...
                std::string path =
                    ...
                    CFile::getFilenameWithoutExtension(fileName) + ".zone";
                ...
                try {
                  ...
                  if (outputfile.open(fileName)) {
                    ...
                    if (outputfile2.open(path)) {
                      ...
                      // Error message
                      char tmp[512];
                      smprintf(tmp, 512, "Can't open the file %s for writing.",
                               path);
                      CMaxToLigo::errorMessage(tmp, "NeL Ligo export material",
                                               *MAXScript_interface,
                                               errorInDialog);
                      ok = false;
                    }
                  ...
                  // Error message
                  char tmp[512];
                  smprintf(tmp, 512, "Error while save the file %s : %s",
                           fileName, e.what());
                  CMaxToLigo::errorMessage(tmp, "NeL Ligo export material",
            ...
            CMaxToLigo::errorMessage(
                "Error: can't export the Nel zone, check bind errors.",
                "NeL Ligo export material", *MAXScript_interface,
                errorInDialog);
          }
        }
      ...
      // Output an error
      CMaxToLigo::errorMessage(
          "Error: can't convert the object in 3ds NeL patch mesh object",
          "NeL Ligo export material", *MAXScript_interface, errorInDialog);
    }
  }
...
Value *export_transition_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoExportTransition [Object array (count=9)] ")
                   _T("[Output filename] [First material filename] [Second ")
                   _T("material filename] [CheckOnly] [Error in dialog]");
  ...
  if (!CMaxToLigo::loadLigoConfigFile(config, *MAXScript_interface,
                                      errorInDialog)) {
    // Output an error
    CMaxToLigo::errorMessage("Error: can't load the config file ligoscape.cfg",
                             "NeL Ligo export transition", *MAXScript_interface,
                             errorInDialog);
  } else {
    // CTransition::TransitionZoneCount elements in the array ?
    if (nodes->size == CTransition::TransitionZoneCount) {
      // Build a node array
      std::vector<CZoneTemplate> arrayTemplate(
          CTransition::TransitionZoneCount);
      std::vector<const CZoneTemplate *> arrayTemplatePtr(
          CTransition::TransitionZoneCount, NULL);
      ...
      for (uint i = 0; i < (uint)nodes->size; i++) {
        ...
        if (nodes->get(i + 1) != &undefined) {
          ...
          if (os.obj) {
            // Convert in 3ds NeL patch mesh
            arrayTri[i] = (RPO *)os.obj->ConvertToType(ip->GetTime(),
            ...
            if (arrayTri[i]) {
              ...
              // Build the zone template
              if (CMaxToLigo::buildZoneTemplate(
                      arrayNode[i], arrayTri[i]->patch, arrayTemplate[i],
                      config, ScriptErrors[i], ip->GetTime())) {
      ...
      if (ok) {
        ...
        for (uint mat = 0; mat < 2; mat++) {
          ...
          if (input.open(matFilename[mat])) {
            ...
            try {
              ...
              // Error message
              char tmp[2048];
              smprintf(tmp, 2048, "Error while loading material file %s : %s",
                       matFilename[mat], e.what());
              CMaxToLigo::errorMessage(tmp, "NeL Ligo export transition",
                                       *MAXScript_interface, errorInDialog);
              ok = false;
            }
          } else {
            // Error message
            char tmp[512];
            smprintf(tmp, 512, "Can't open the file %s for reading.",
                     matFilename[mat]);
            CMaxToLigo::errorMessage(tmp, "NeL Ligo export transition",
                                     *MAXScript_interface, errorInDialog);
            ok = false;
        ...
        if (ok) {
          ...
          if (ok = transition.build(
                  materials[0], materials[1], arrayTemplatePtr, config,
                  ScriptErrors,
                  ScriptErrors[CTransition::TransitionZoneCount])) {
            // Export ?
            if (!checkOnly) {
              ...
              if (allPresent) {
                ...
                uint zone;
                for (zone = 0; zone < CTransition::TransitionZoneCount;
                     zone++) {
                  // Build the zone
                  CZoneSymmetrisation zoneSymmetry;
                  if (!arrayTri[zone]->rpatch->exportZone(
                          arrayNode[zone], &arrayTri[zone]->patch, zones[zone],
                          zoneSymmetry, 0, config.CellSize, config.Snap,
                          false)) {
                    // Error, zone can't be exported
                    CMaxToLigo::errorMessage(
                        "Error: can't export the Nel zone, check bind errors.",
                    ...
                    if (outputfile.open(fileName)) {
                      ...
                      for (zone = 0; zone < CTransition::TransitionZoneCount;
                        ...
                        if (outputfile2.open(path)) {
                          ...
                          char tmp[512];
                          smprintf(tmp, 512,
                                   "Can't open the file %s for writing.", path);
                          CMaxToLigo::errorMessage(
                              tmp, "NeL Ligo export transition",
                              *MAXScript_interface, errorInDialog);
                          ok = false;
                        }
                      }
                ...
                char tmp[512];
                smprintf(tmp, 512,
                         "All transition are not present. Can't export..");
                CMaxToLigo::errorMessage(tmp, "NeL Ligo export transition",
                                         *MAXScript_interface, errorInDialog);
                ok = false;
              }
      ...
      // Output an error
      CMaxToLigo::errorMessage("Error: must have 9 cell in the node array",
                               "NeL Ligo export transition",
                               *MAXScript_interface, errorInDialog);
    }
  }
...
Value *get_error_zone_template_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoGetErrorZoneTemplate [Array error codes] [Array ")
                   _T("vertex id] [Array messages] [Error index]");
  type_check(arg_list[0], Array, message);
...
Value *check_zone_with_material_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoCheckZoneWithMaterial [Object] [Material ")
                   _T("filename] [Error in dialog]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], String, message);
  type_check(arg_list[2], Boolean, message);
          ...
          if (fileName != "") {
            ...
            try {
              ...
              if (inputfile.open(fileName)) {
                ...
                // Get the zone edges
                const std::vector<CZoneEdge> &zoneEdges =
                    zoneTemplate.getEdges();
                ...
                for (uint edge = 0; edge < zoneEdges.size(); edge++) {
                  // Check it
                  if (!material.getEdge().isTheSame(zoneEdges[edge], config,
                                                    ScriptErrors[0]))
                    res = false;
                }
              } else {
                ...
                char tmp[512];
                smprintf(tmp, 512,
                         "Can't open the zone template file %s for reading.",
                         fileName);
                CMaxToLigo::errorMessage(tmp, "NeL Ligo check zone",
                                         *MAXScript_interface, errorInDialog);
              }
            } catch (const Exception &e) {
              // Error message
              char tmp[512];
              smprintf(tmp, 512, "Error while loading the file %s : %s",
                       fileName, e.what());
              CMaxToLigo::errorMessage(tmp, "NeL Ligo check zone",
                                       *MAXScript_interface, errorInDialog);
            }
...
Value *check_zone_with_transition_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoCheckZoneWithTransition [Object] [Transition ")
                   _T("filename] [Transition number: 0~8] [Error in dialog]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], String, message);
        ...
        if (res) {
          ...
          try {
            ...
            if (inputfile.open(fileName)) {
              ...
              // Check it
              res = transition.check(zoneTemplate, transitionNumber, config,
                                     ScriptErrors[0]);
            } else {
              // Error message
              char tmp[512];
              smprintf(
                  tmp, 512,
                  "Can't open the transition template file %s for reading.",
                  fileName);
              CMaxToLigo::errorMessage(tmp, "NeL Ligo check zone",
                                       *MAXScript_interface, errorInDialog);
            }
            ...
            // Error message
            char tmp[512];
            smprintf(tmp, 512, "Error while loading the file %s : %s", fileName,
                     e.what());
            CMaxToLigo::errorMessage(tmp, "NeL Ligo check zone",
                                     *MAXScript_interface, errorInDialog);
          }
...
// Get the zone mask
void getSquareMask(CZone &zone, std::vector<bool> &mask, uint &width,
                   uint &height, float cellSize) {
  CZoneInfo zoneInfo;
  ...
  for (i = 0; i < zoneInfo.Patchs.size(); i++) {
    ...
    for (v = 0; v < 4; v++) {
      ...
      sint positionX =
          (uint)((patch.Patch.Vertices[v].x + cellSize / 2) / cellSize);
      sint positionY =
          (uint)((patch.Patch.Vertices[v].y + cellSize / 2) / cellSize);
...
Value *export_zone_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoExportZone [Object] [Ligozone filename] ")
                   _T("[Category Array] [Error in dialog] [Snapshot]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], String, message);
  type_check(arg_list[2], Array, message);
  type_check(arg_list[3], Boolean, message);
  ...
  if (!CMaxToLigo::loadLigoConfigFile(config, *MAXScript_interface,
    ...
    // Output an error
    CMaxToLigo::errorMessage("Error: can't load the config file ligoscape.cfg",
                             "NeL Ligo check zone", *MAXScript_interface,
                             errorInDialog);
  } else {
    ...
    for (i = 0; i < (uint)array->size; i++) {
      ...
      if (cell->size != 2) {
        // Output an error
        CMaxToLigo::errorMessage("Error: category arguments are not 2 strings",
                                 "NeL Ligo export zone", *MAXScript_interface,
                                 errorInDialog);
        return &false_value;
      }
    ...
    if (os.obj) {
      ...
      if (tri) {
        ...
        if (!CMaxToLigo::loadLigoConfigFile(config, *MAXScript_interface,
          ...
          std::string outputFilenameSnapShot =
              path + "zonesBitmaps/" + name + ".tga";

          // Build the ligozone filename
          std::string outputFilenameLigozone =
              path + "zoneLigos/" + name + ".ligozone";
          ...
          if (!tri->rpatch->exportZone(node, &tri->patch, zone, zoneSymmetry, 0,
            ...
            bool useBoundingBox =
                CExportNel::getScriptAppData(
                    node, NEL3D_APPDATA_LIGO_USE_BOUNDINGBOX, 0) != 0;
            bool maskOk = false;
            ...
            if (useBoundingBox) {
              ...
              // Build the zone template
              bool res = CMaxToLigo::buildZoneTemplate(
                  node, tri->patch, zoneTemplate, config, ScriptErrors[0],
                  ip->GetTime());
              ...
              try {
                ...
                if (tileBank == NULL) {
                  ...
                  if (fileBank.open(GetBankPathName())) {
                    ...
                    char tmp[512];
                    smprintf(tmp, 512,
                             "Can't open the bank file %s for reading.",
                             GetBankPathName().c_str());
                    CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                             *MAXScript_interface,
                                             errorInDialog);
                  }
                }
                ...
                if (tileBank != NULL) {
                  ...
                  if (weWantToMakeASnapshot) {
                    ...
                    std::string path =
                        NLMISC::CFile::getPath(GetBankPathName());
                    std::string name =
                    ...
                    if (fileFarBank.open(farBankPathName)) {
                      ...
                      CBitmap snapshot;
                      if (MakeSnapShot(snapshot, *tileBank, *tileFarBank, width,
                                       height, config, errorInDialog)) {
                        ...
                        char tmp[512];
                        smprintf(tmp, 512,
                                 "Can't open the tga file %s for writing.",
                                 outputFilenameSnapShot);
                        CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                                 *MAXScript_interface,
                                                 errorInDialog);
                        cont = false;
                      ...
                      char tmp[512];
                      smprintf(tmp, 512,
                               "Can't open the farbank file %s for reading.",
                               farBankPathName);
                      CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                               *MAXScript_interface,
                                               errorInDialog);
                  ...
                  if (cont) {
                    {
                      ...
                      if (outputZone.open(outputFilenameZone)) {
                        ...
                        char tmp[512];
                        smprintf(tmp, 512,
                                 "Can't open the NeL zone file %s for writing.",
                                 outputFilenameZone);
                        CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                                 *MAXScript_interface,
                                                 errorInDialog);
                    ...
                    // The .LIGOZONE
                    {
                      ...
                      if (j >= mask.size()) {
                        categories.push_back(
                            pair<string, string>("filled", "yes"));
                      } else {
                        categories.push_back(
                            pair<string, string>("filled", "no"));
                      }

                      // Add the zone categorie
                      if (width == height) {
                        categories.push_back(
                            pair<string, string>("square", "yes"));
                      } else {
                        categories.push_back(
                            pair<string, string>("square", "no"));
                      }
                      ...
                      for (j = 0; j < categories.size(); j++) {
                        bankElm.addCategory(strlwr(categories[j].first),
                                            strlwr(categories[j].second));
                      }
                      ...
                      try {
                        ...
                        if (outputLigoZone.open(outputFilenameLigozone)) {
                          ...
                          smprintf(
                              tmp, 512,
                              "Can't open the ligozone file %s for writing.",
                              outputFilenameLigozone);
                          CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                                   *MAXScript_interface,
                                                   errorInDialog);
                        }
                        ...
                        char tmp[512];
                        smprintf(tmp, 512,
                                 "Error while writing the file %s : %s",
                                 outputFilenameLigozone, e.what());
                        CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                                 *MAXScript_interface,
                                                 errorInDialog);
                      }
                ...
                // Error message
                char tmp[512];
                smprintf(tmp, 512, "Error while loading the file bank %s : %s",
                         GetBankPathName().c_str(), e.what());
                CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                         *MAXScript_interface, errorInDialog);
              }
            }
...
Value *get_error_string_cf(Value **arg_list, int count) {
  ...
  // Error code
  return new String(MaxTStrFromUtf8(
      CLigoError::getStringError((CLigoError::TError)errorCode)));
}
...
Value *get_zone_mask_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoGetZoneMask [Object] [Mask Array] [Width Array] ")
                   _T("[Height Array] [Error in dialog]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], Array, message);
  type_check(arg_list[2], Array, message);
...
Value *get_zone_size_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoGetZoneMask [Object] [minx Array] [maxy Array] ")
                   _T("[miny Array] [maxy Array] [Error in dialog]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], Array, message);
  type_check(arg_list[2], Array, message);
  type_check(arg_list[3], Array, message);
...
bool MakeSnapShot(NLMISC::CBitmap &snapshot, const NL3D::CTileBank &tileBank,
                  const NL3D::CTileFarBank &tileFarBank, sint xmax, sint ymax,
                  const CLigoConfig &config, bool errorInDialog) {
  return MakeSnapShot(snapshot, tileBank, tileFarBank, 0, xmax, 0, ymax, config,
                      errorInDialog);
}
...
bool MakeSnapShot(NLMISC::CBitmap &snapshot, const NL3D::CTileBank &tileBank,
  ...
  try {
    ...
    // Use NELU
    if (CNELU::init(oversampledWidth, oversampledHeight, CViewport(), 32, true,
                    NULL, false, true)) // FIXME: OpenGL not working correctly,
                                        // offscreen not available in Direct3D
    {
      // Setup the camera
      ...
      CExportNel export_(errorInDialog, false, true, MAXScript_interface,
                         "Snapshot ligozone", &options);
      export_.buildScene(*CNELU::Scene, *CNELU::ShapeBank, *CNELU::Driver, 0,
                         &theLand->Landscape, NULL, false, false, false);
      ...
      // theLand->Landscape.setupStaticLight(CRGBA(255, 255, 255), CRGBA(0, 0,
      // 0), 1.0f); theLand->Landscape.setThreshold(0.0005);
      ...
      // Output an error
      CMaxToLigo::errorMessage("Can't initialise opengl offscreen renderer",
                               "NeL Ligo check zone", *MAXScript_interface,
                               errorInDialog);
    }
  } catch (const Exception &e) {
    ...
    // Output an error
    CMaxToLigo::errorMessage(tmp, "NeL Ligo check zone", *MAXScript_interface,
                             errorInDialog);
  }
...
Value *make_snapshot_cf(Value **arg_list, int count) {
  ...
  // Check to see if the arguments match up to what we expect
  TCHAR *message = _T("NeLLigoMakeSnapShot [Object] [Snapshot filename] ")
                   _T("[xMin] [xMax] [yMin] [yMax] [Error in dialog]");
  type_check(arg_list[0], MAXNode, message);
  type_check(arg_list[1], String, message);
  type_check(arg_list[2], Integer, message);
  type_check(arg_list[3], Integer, message);
          ...
          try {
            if (tileBank == NULL) {
              ...
              if (fileBank.open(GetBankPathName())) {
                ...
                // Error message
                char tmp[512];
                smprintf(tmp, 512, "Can't open the bank file %s for writing.",
                         GetBankPathName().c_str());
                CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                         *MAXScript_interface, errorInDialog);
              }
            ...
            if (tileBank != NULL) {
              CIFile fileFarBank;
              std::string name =
                  CFile::getFilenameWithoutExtension(GetBankPathName());
              std::string farBankPathName =
                  CFile::getPath(GetBankPathName()) + name + ".farbank";
              ...
              if (fileFarBank.open(farBankPathName)) {
                ...
                CBitmap snapshot;
                if (MakeSnapShot(snapshot, *tileBank, *tileFarBank, xMin, xMax,
                                 yMin, yMax, config, errorInDialog)) {
                  // Build the snap shot filename
                  std::string outputFilenameSnapShot =
                      pathtga + nametga + ".tga";
                  ...
                  if (outputSnapShot.open(outputFilenameSnapShot)) {
                    ...
                    categories.push_back(
                        pair<string, string>("material", "fyros"));
                    ...
                    char tmp[512];
                    smprintf(tmp, 512,
                             "Can't open the NeL snapshot file %s for writing.",
                             outputFilenameSnapShot);
                    CMaxToLigo::errorMessage(tmp, "NeL Ligo export zone",
                                             *MAXScript_interface,
                                             errorInDialog);

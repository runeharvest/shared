...
int main(int argc, char **argv) {
  ...
  try {
    ...
    for (k = 0; k < ig._InstancesInfos.size(); ++k) {
      printf("instance %s : x = %.1f, y = %.1f, z = %.1f, sx = %.1f, sy = "
             "%.1f, sz = %.1f, a = %.1f\n",
             ig._InstancesInfos[k].Name.c_str(),
             ...
             ig._InstancesInfos[k].Rot.getAngle());
    }
    printf("\n");
    printf("Lights\n");
    printf("---------\n");
    for (k = 0; k < ig.getNumPointLights(); ++k) {
      ...
      printf("light group = %d, anim = \"%s\" x = %.1f, y = %.1f, z = %.1f\n",
             pl.LightGroup, pl.AnimatedLight.c_str(),
             pl.getPosition().x + gpos.x, pl.getPosition().y + gpos.y,
             pl.getPosition().z + gpos.z);
    }
    printf("---------\n");
    ...
    printf("---------\n");
    // IGSurfaceLight info.
    const CIGSurfaceLight::TRetrieverGridMap &rgm =
        ig.getIGSurfaceLight().getRetrieverGridMap();
    ...
    for (; it != rgm.end(); it++) {
      for (uint i = 0; i < it->second.Grids.size(); i++) {
        printf("grid(%d, %d): %dx%d\n", rgmInst, i, it->second.Grids[i].Width,
               it->second.Grids[i].Height);

...
bool dataCheck(const std::string &sFileNameSrc, const std::string &FileNameDest,
               uint8 algo);
...
bool dataCheck(const std::string &sFileNameSrc,
               const std::string &sFileNameDest, uint8 &algo, bool wantMipMap) {
...
void dividSize(CBitmap &bitmap) {
  ...
  for (y = 0; y < newHeight; y++)
    for (x = 0; x < newWidth; x++) {
      ...
      uint i;
      for (i = 0; i < 4; i++) {
        pixelDest[offsetDest + i] =
            ((uint)pixelSrc[offsetSrc + i] + (uint)pixelSrc[offsetSrc + 4 + i] +
             (uint)pixelSrc[offsetSrc + 4 * width + i] +
             (uint)pixelSrc[offsetSrc + 4 * width + 4 + i]) >>
            2;
      }
    }
}
...
const int bayerDiv8R[4][4] = {
    {7, 3, 6, 2}, {1, 5, 0, 4}, {6, 2, 7, 3}, {0, 4, 1, 5}};

const int bayerDiv8G[4][4] = {
    {0, 4, 1, 5}, {6, 2, 7, 3}, {1, 5, 0, 4}, {7, 3, 6, 2}};

const int bayerDiv8B[4][4] = {
    {5, 1, 4, 0}, {3, 7, 2, 6}, {4, 0, 5, 1}, {2, 6, 3, 7}};
...
int main(int argc, char **argv) {
  ...
  args.setDescription(
      "Convert TGA or PNG image file to DDS compressed file using DXTC "
      "compression (DXTC1, DXTC1 with alpha, DXTC3, or DXTC5).\n"
      "  The program looks for possible user color files and load them "
      "automatically, a user color file must have the same name that the "
      "original tga file, plus the extension \"_usercolor\"\n"
      "Eg.: pic.tga, the associated user color file must be: "
      "pic_usercolor.tga\n");
  args.addArg("o", "output", "output.dds", "Output DDS filename or directory");
  args.addArg("a", "algo", "algo",
  ...
  args.addArg("g", "grayscale", "",
              "Don't load grayscape images as alpha but as grayscale");
  args.addArg("m", "mipmap", "", "Create MipMap");
  args.addArg("r", "reduce", "FACTOR",
              "Reduce the bitmap size before compressing\n  FACTOR is 0, 1, 2, "
              "3, 4, 5, 6, 7 or 8");
  ...
  if (args.haveArg("a")) {
    ...
    if (strAlgo == "1")
      ...
      OptAlgo = DXT5;
    else if (strAlgo == "tga8")
      OptAlgo = TGA8;
    else if (strAlgo == "tga16")
      OptAlgo = TGA16;
    else if (strAlgo == "png8")
      OptAlgo = PNG8;
    else if (strAlgo == "png16")
      OptAlgo = PNG16;
    else {
  ...
  for (uint i = 0; i < inputFileNames.size(); ++i) {
    ...
    if (OptGrayscale)
      picTga.loadGrayscaleAsAlpha(false);
    ...
    if (imageDepth != 16 && imageDepth != 24 && imageDepth != 32 &&
        imageDepth != 8) {
    ...
    if (!OptOutputFileName.empty()) {
      ...
      if (CFile::isDirectory(OptOutputFileName)) {
        outputFileName = CPath::standardizePath(OptOutputFileName) +
                         CFile::getFilename(getOutputFileName(inputFileName));
      } else {
        outputFileName = OptOutputFileName;

        if (inputFileNames.size() > 1) {
          cerr << "WARNING! Several files to convert to the same output "
                  "filename! Use an output directory instead."
               << endl;
          return 1;
        }
      }
    ...
    */
    ...
    if (pos == std::string::npos) {
      ...
      userColorFileName = inputFileName.substr(0, pos) + "_usercolor" +
                          inputFileName.substr(pos);
    }

    // Reading second Tga for user color, don't complain if _usercolor is
    // missing
    ...
    if (CPath::exists(userColorFileName) && input2.open(userColorFileName)) {
      ...
      for (uint32 i = 0; i < width * height; i++) {
        // If no UserColor, must take same RGB, and keep same Alpha from src1
        // !!! So texture can have both alpha userColor and other alpha usage.
        if (pRGBASrc2[i].A == 255) {
          ...
          // Old code.
          /*uint8 F = (uint8) ((float)pRGBASrc[i].R*0.3 +
          (float)pRGBASrc[i].G*0.56 + (float)pRGBASrc[i].B*0.14); uint8 Frgb;
    ...
    if ((algo == TGA16) || (algo == TGA8) || (algo == PNG16) ||
        (algo == PNG8)) {
      ...
      cout << "compressing (" << algostr << ") " << inputFileName << " to "
           << outputFileName << endl;

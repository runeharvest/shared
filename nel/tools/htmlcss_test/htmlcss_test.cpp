...
// ***************************************************************************
void checkRuleset(CHtmlElement &elm, CCssStyle &style, TStyleVec testset,
                  bool exists) {
  bool verbose = false;
    ...
    if (failed) {
      ...
      if (it.first == "font-size") {
        printf("[%s]: font-size: %d; expected '%s'\n", existsMessage.c_str(),
               style.Current.FontSize, it.second.c_str());
        printf("     (%s)\n", elm.toString().c_str());
        failed2 = false;
      } else if (it.first == "background-color") {
        printf("[%s]: background-color: '%s'; expected '%s'\n",
               existsMessage.c_str(),
               style.Current.Background.color.toString().c_str(),
               it.second.c_str());
        printf("     (%s)\n", elm.toString().c_str());
        failed2 = false;
      }
...
void recursiveHtmlRender(CHtmlElement &elm, CCssStyle &style) {
  ...
  if (elm.Type == CHtmlElement::TEXT_NODE) {
    ...
    if (elm.hasAttribute("data-ruleset")) {
      TStyleVec testset =
          CCssParser::parseDecls(elm.getAttribute("data-ruleset"));
      checkRuleset(elm, style, testset, true);
    }

    if (elm.hasAttribute("data-ruleunset")) {
      TStyleVec testset =
          CCssParser::parseDecls(elm.getAttribute("data-ruleunset"));
      checkRuleset(elm, style, testset, false);
    }

    if (elm.hasAttribute("data-ruleset-before")) {
      TStyleVec testset =
          CCssParser::parseDecls(elm.getAttribute("data-ruleset-before"));
    }
...
int main(int argc, const char *argv[]) {
  ...
  if (argc == 2) {
    ...
    CPath::getPathContent("tests/", true /*recursive*/, false /*wantDir*/,
                          true /*wantFile*/, result, NULL /*callback*/,
                          true /*showEverything*/);
    printf(":: got %ld files\n", result.size());

...
void CListenSock::init(uint16 port) {
  // Use any address
  CInetAddress localaddr; // By default, INETADDR_ANY (useful for gateways that
  ...
  LNETL0_DEBUG("LNETL0: Socket %d listen socket is at %s", _Sock,
...
/*
 * Prepares to receive connections on a specified address/port (useful when the
...
void CListenSock::init(const CInetAddress &addr) {
  ...
  if (addr.getAddress().isAny()) {
    LNETL0_DEBUG("LNETL0: Binding listen socket to any address (%s), port %hu",
                 addr.getAddress().toString().c_str(), addr.port());
  }
  ...
  if (::bind(_Sock, (sockaddr *)&sockAddr, sizeOfSockAddr(sockAddr)) != 0) {
    if (_AddressFamily == AF_INET6 && addr.getAddress().isAny() &&
      ...
      nlwarning("Failed to bind to dual stack IPv6 Any addres, binding to IPv4 "
                "Any address instead");
  ...
  // Listen
  if (::listen(_Sock, _BackLog) !=
      0) // SOMAXCONN = maximum length of the queue of pending connections
  {
    throw ESocket("Unable to listen on specified port");
  }
  //	LNETL0_DEBUG( "LNETL0: Socket %d listening at %s", _Sock,
  //_LocalAddr.asString().c_str() );
}
...
CTcpSock *CListenSock::accept() {
  ...
  LNETL0_DEBUG("LNETL0: Socket %d accepted an incoming connection from %s, "
               "opening socket %d",
               _Sock, addr.asString().c_str(), newsock);
  CTcpSock *connection = new CTcpSock(newsock, addr);
  return connection;
}

/*
 * Sets the number of the pending connections queue. -1 for the maximum possible
 * value.
 */
void CListenSock::setBacklog(sint backlog) {
  if (backlog == -1) {
    _BackLog = SOMAXCONN; // SOMAXCONN = maximum length of the queue of pending
                          // connections
  } else {
    _BackLog = backlog;
  }
  if (_Bound) {
    if (::listen(_Sock, _BackLog) != 0) {
      throw ESocket(
          "Unable to listen on specified port, while changing backlog");
    }
  }
}

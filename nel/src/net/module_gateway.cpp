...
struct TSecurityDataDesc {
  ...
  TSecurityDataDesc() : SecurityData(NULL) {}
    ...
    if (!s.isReading()) {
      ...
      for (uint i = 0; i < nbSecBlock; ++i) {
        ...
        try {
          ...
          sd = NLMISC_GET_FACTORY(TSecurityData, uint8)
                   .createObject(dataTag, param);
          ...
          // FAILED to read the security block, rewind to old pos and serial as
          // unknow
          nlwarning("Error while reading stream for security data type %u",
                    dataTag);
...
struct TModuleDescCodec {
  ...
  TModuleDescCodec(IModuleProxy *proxy) {
    ...
    SecDesc.SecurityData =
        const_cast<TSecurityData *>(proxy->getFirstSecurityData());
  }
...
struct TModuleRemMsg {
  ...
  void serial(NLMISC::IStream &s) { s.serialCont(RemovedModules); }
};
 ...
 *	across process.
 */
class CStandardGateway : public CModuleBase,
...
public:
  CStandardGateway() : _SecurityPlugin(NULL), _PingCounter(0) {}
   ...
   ** Gateway methods
   ***********************************************************/
  virtual const std::string &getGatewayName() const { return getModuleName(); }
  ...
  virtual void createTransport(const std::string &transportClass,
                               const std::string &instanceName) {
    if (_Transports.find(instanceName) != _Transports.end()) {
      nlwarning("A transport with the name '%s' already exist in this gateway",
                instanceName.c_str());
      return;
    }
    ...
    param.Gateway = this;
    IGatewayTransport *transport =
        NLMISC_GET_FACTORY(IGatewayTransport, std::string)
            .createObject(transportClass, param);

    if (transport == NULL) {
      nlwarning("Failed to create a transport with the class '%s'",
                transportClass.c_str());
      return;
    }
    ...
    nldebug("NETL6: Gateway transport %s (%s) created", instanceName.c_str(),
            transportClass.c_str());
  }
  ...
  virtual void deleteTransport(const std::string &instanceName) {
    ...
    //			nlassert(_TransportPtrIdx.find(transport) !=
    //_TransportPtrIdx.end()); 			_TransportPtrIdx.erase(transport);
    delete transport;
    //			delete it->second;
  ...
  virtual void
  setTransportPeerInvisible(const std::string &transportInstanceName,
                            bool peerInvisible) {
    ...
    nldebug("NETL6: Gateway transport %s peer invisible mode %s",
            transportInstanceName.c_str(), peerInvisible ? "ON" : "OFF");

    // For each route of this transport, we need to disclose/undisclose peer
    // modules
    ...
    for (; first != last; ++first) {
      ...
      if (route->getTransport() == transport) {
        ...
        TModuleProxies::iterator first(_ModuleProxies.begin()),
            last(_ModuleProxies.end());
        for (; first != last; ++first) {
          ...
          if (proxy->getGatewayRoute() != NULL &&
            ...
            // this module is on the same transport, but another route,
            // remove/add it from the route
            if (peerInvisible)
  ...
  virtual void
  setTransportFirewallMode(const std::string &transportInstanceName,
                           bool firewalled) {
    TTransportList::iterator it(_Transports.find(transportInstanceName));
    if (it == _Transports.end()) {
      nlwarning("Unknown transport named '%s'", transportInstanceName.c_str());
    ...
    nldebug("NETL6: Gateway transport %s firewall mode %s",
            transportInstanceName.c_str(), firewalled ? "ON" : "OFF");
        ...
        if (route->getTransport() == transport) {
          ...
          for (; first != last; ++first) {
            ...
            if (proxy->getGatewayRoute() == NULL ||
              ...
              if (route->FirewallDisclosed.find(proxy->getModuleProxyId()) ==
                  route->FirewallDisclosed.end())
                discloseModuleToRoute(route, proxy);
            }
          }
        }
  ...
  virtual void transportCommand(const TParsedCommandLine &commandLine) {
    for (uint i = 1; i < commandLine.SubParams.size(); ++i) {
      ...
      if (it == _Transports.end()) {
        nlwarning("Unknown transport named '%s', ignoring command.",
                  transportName.c_str());
      } else if (subParam->SubParams.empty()) {
        nlwarning("Can't find sub param list for transport '%s' command",
                  transportName.c_str());
      } else {
        nldebug("NETL6: Gateway transport %s, sending command '%s'",
                transportName.c_str(), commandLine.toString().c_str());
  ...
  getGatewayTransport(const std::string &transportName) const {
    TTransportList::const_iterator it(_Transports.find(transportName));
  ...
  virtual uint32 getRouteCount() const { return (uint32)_Routes.size(); }

  virtual uint32 getReceivedPingCount() const { return _PingCounter; }
  ...
  virtual void onRouteRemoved(CGatewayRoute *route) {
    ...
    //			CGatewayRoute::TForeignToLocalIdx::TAToBMap::const_iterator
    //first(route->ForeignToLocalIdx.getAToBMap().begin()),
    //last(route->ForeignToLocalIdx.getAToBMap().end()); 			for (; first != last;
    //++first)
    while (!route->ForeignToLocalIdx.getAToBMap().empty()) {
      removeForeignModule(route,
                          route->ForeignToLocalIdx.getAToBMap().begin()->first);
      //				TModuleId localProxyId = first->second;
      //				TModuleProxies::iterator
      //it(_ModuleProxies.find(localProxyId)); 				nlassert(it !=
      //_ModuleProxies.end());
      //
      ...
      //
      //				// remove proxy record from the proxy
      //list 				_ModuleProxies.erase(it); 				_NameToProxyIdx.removeWithB(modProx);
      //
  ...
  virtual void createSecurityPlugin(const std::string &className) {
    if (_SecurityPlugin != NULL) {
      nlwarning("NLNETL5 : CStandardGateway::createSecurityPlugin : plug-in "
                "already created ");
      return;
    }
    ...
    params.Gateway = this;
    CGatewaySecurity *gs = NLMISC_GET_FACTORY(CGatewaySecurity, std::string)
                               .createObject(className, params);
    if (gs == NULL) {
      nlwarning("NLNETL5 : CStandardGateway::createSecurityPlugin : can't "
                "create a security plug-in for class '%s'",
                className.c_str());
      return;
    }
  ...
  virtual void sendSecurityCommand(const TParsedCommandLine &command) {
    if (_SecurityPlugin == NULL) {
      nlwarning("NLNETL5 : CStandardGateway::sendSecurityCommand : plug-in NOT "
                "created ");
      return;
    }
  ...
  virtual void removeSecurityPlugin() {
    if (_SecurityPlugin == NULL) {
      nlwarning("NLNETL5 : CStandardGateway::removeSecurityPlugin : plug-in "
                "not created");
      return;
    }
  ...
  void replaceAllSecurityDatas(IModuleProxy *proxy,
                               TSecurityData *securityData) {
    nlassert(proxy->getModuleGateway() == this);
  ...
  void onReceiveModuleMessageFromGateway(CGatewayRoute *from,
    ...
    if (it == _ModuleProxies.end()) {
      nlwarning("Can't dispatch the module message, sender proxy %u is not in "
                "this gateway",
    ...
    if (it == _ModuleProxies.end()) {
      nlwarning("Can't dispatch the module message '%s', sender proxy %u is "
                "not in this gateway",
                msgin.getName().c_str(), from->NextAddresseeProxyId);
      return;
    }
    addresseeProxy = it->second;

    // give the message to the gateway (either for local dispatch or for
    // forwarding)
    sendModuleProxyMessage(senderProxy, addresseeProxy, msgin);
  }

  // A gateway receive a module message header
  void onReceiveModuleMessageHeader(CGatewayRoute *from,
                                    const CMessage &msgin) {
    H_AUTO(CModuleGetaway_onReceiveModuleMessageHeader);
    if (from->NextMessageType != CModuleMessageHeaderCodec::mt_invalid) {
      // juste warn (but that is VERY BAD)
      nlwarning("Receiving a new module message header without having received "
                "the previous module message !");
    }

    // store the message information in the route
    CModuleMessageHeaderCodec::decode(msgin, from->NextMessageType,
                                      from->NextSenderProxyId,
                                      from->NextAddresseeProxyId);

    // translate sender id
    const TModuleId *pmoduleId =
        from->ForeignToLocalIdx.getB(from->NextSenderProxyId);
    if (pmoduleId == NULL) {
      nlwarning("The sender proxy %u is unknown in the translation table, "
                "can't dispatch the message !",
                from->NextSenderProxyId);
      from->NextMessageType = CModuleMessageHeaderCodec::mt_invalid;
      return;
    }
  ...
  void onReceiveModuleAdd(CGatewayRoute *from, const CMessage &msgin) {
    ...
    nldebug(
        "Gateway '%s' : store module info for '%s' (foreign ID %u) @ %u hop",
        getGatewayName().c_str(), modDesc.ModuleFullName.c_str(),
        modDesc.ModuleProxyId, modDesc.ModuleDistance);
    ...
    if (_NameToProxyIdx.getB(modNameId) != NULL) {
      ...
      //					from->ForeignToLocalIdx.insert(make_pair(modDesc.ModuleProxyId,
      //modProx->getModuleProxyId()));
      from->ForeignToLocalIdx.add(modDesc.ModuleProxyId,
                                  modProx->getModuleProxyId());
      ...
      if (modDesc.SecDesc.SecurityData != NULL) {
        CModuleProxy *proxy = static_cast<CModuleProxy *>(modProx);
        if (_SecurityPlugin != NULL) {
          _SecurityPlugin->onNewSecurityData(from, proxy,
                                             modDesc.SecDesc.SecurityData);
        } else {
      ...
      IModuleProxy *modProx = IModuleManager::getInstance().createModuleProxy(
          this, from, modDesc.ModuleDistance, NULL, modDesc.ModuleClass,
          modDesc.ModuleFullName, modDesc.ModuleManifest,
  ...
  void onReceiveModuleDistanceUpdate(CGatewayRoute *from,
                                     const CMessage &msgin) {
    ...
    if (pModuleId == NULL) {
      nlwarning("Receive a module distance update for foreign module %u, but "
    ...
    for (; range.first != range.second; ++range.first) {
      ...
      if (kmi.Route == from) {
        ...
        nldebug("Gateway '%s' : updating distance from %u to %u hop for module "
                "'%s'",
                getGatewayName().c_str(), kmi.ModuleDistance, newDistance,
      ...
      if (proxy->_Distance < newDistance) {
        ...
        for (; range.first != range.second; ++range.first) {
          ...
          if (kmi.ModuleDistance < proxy->_Distance) {
            nldebug(
                "Gateway '%s' : proxy '%s' use a new path from %u to %u hop",
                getGatewayName().c_str(), proxy->getModuleName().c_str(),
        ...
        if (range.first == range.second) {
          ...
          nldebug("Gateway '%s' : proxy '%s' path distance changed from %u to "
                  "%u hop",
                  getGatewayName().c_str(), proxy->getModuleName().c_str(),
        ...
        nldebug(
            "Gateway '%s' : proxy '%s' path distance reduced from %u to %u hop",
            getGatewayName().c_str(), proxy->getModuleName().c_str(),
            proxy->_Distance, newDistance);
  ...
  void onReceiveModuleSecurityUpdate(CGatewayRoute *from,
                                     const CMessage &msgin) {
    ...
    if (pModuleId == NULL) {
      nlwarning("LNETL6 : receive module security update for unknown module "
                "foreign proxy %u",
                secChg.ModuleId);
      return;
    }
    ...
    if (_SecurityPlugin != NULL) {
      ...
      _SecurityPlugin->onNewSecurityData(from, modProx,
                                         secChg.SecDesc.SecurityData);
    } else {
    ...
    {
      TPluggedModules::TAToBMap::const_iterator first(
          _PluggedModules.getAToBMap().begin()),
          last(_PluggedModules.getAToBMap().end());
      for (; first != last; ++first) {
  ...
  virtual IModuleProxy *getPluggedModuleProxy(IModule *pluggedModule) {
    TLocalModuleIndex::iterator it(
        _LocalModuleIndex.find(pluggedModule->getModuleId()));
  ...
  virtual uint32 getProxyCount() const { return (uint32)_ModuleProxies.size(); }

  /// Fill a vector with the list of proxies managed here. The module are filled
  /// in ascending proxy id order.
  virtual void
  getModuleProxyList(std::vector<IModuleProxy *> &resultList) const {
    TModuleProxies::const_iterator first(_ModuleProxies.begin()),
        last(_ModuleProxies.end());
    for (; first != last; ++first) {
  ...
  virtual void sendModuleProxyMessage(IModuleProxy *senderProxy,
                                      IModuleProxy *addresseeProxy,
    ...
    if (addresseeProxy->getGatewayRoute() &&
      ...
      if (route->FirewallDisclosed.find(senderProxy->getModuleProxyId()) ==
          route->FirewallDisclosed.end()) {
    ...
    if (!isModuleProxyVisible(addresseeProxy, senderProxy->getGatewayRoute())) {
      nlwarning("Module %u '%s' try to send message to %u '%s' but addressee "
                "is not visible, message discarded",
    ...
    if (addresseeProxy->getGatewayRoute() == NULL) {
      ...
      nlassert(_ModuleProxies.find(senderProxy->getModuleProxyId()) !=
               _ModuleProxies.end());
      ...
      if (adrcp == NULL) {
        nlwarning("sendModuleMessage : can't find addressee module %u that is "
                  "not plugged here !",
                  addresseeModId);
        return;
      }
      ...
      if (!addreseeMod->isImmediateDispatchingSupported()) {
        ...
        // this provide a coherent behavior between local and distant module
        // message exchange
        ...
        nldebug("NETL6 : gateway '%s' : queuing local message '%s' from proxy "
                "%u to proxy %u",
                getModuleName().c_str(), message.getName().c_str(),
                lm.SenderProxyId, lm.AddresseProxyId);
      ...
      CModuleMessageHeaderCodec::encode(msgHeader,
  ...
  virtual void dispatchModuleMessage(IModuleProxy *senderProxy,
                                     IModuleProxy *addresseeProxy,
    ...
    // As the addressee is local, the foreign proxy id is the local module id (a
    // bit triky...)
    TModuleId addresseeModId = addresseeProxy->getForeignModuleId();

    const TModulePtr *adrcp = _PluggedModules.getB(addresseeModId);
    if (adrcp == NULL) {
      nlwarning("dispatchModuleMessage : dispatching a message to module %u "
                "that is not plugged here !",
                addresseeModId);
      return;
    }
    ...
    //			addreseeMod->onProcessModuleMessage(senderProxy,
    //message);
    try {
      addreseeMod->onReceiveModuleMessage(senderProxy, message);
    } catch (...) {
      nlwarning("An exception was thrown while dispatching message '%s' from "
                "'%s' to '%s'",
                message.getName().c_str(), senderProxy->getModuleName().c_str(),
                addresseeProxy->getModuleName().c_str());
  ...
  std::string buildModuleManifest() const { return string(); }

  void onServiceUp(const std::string & /* serviceName */,
                   NLNET::TServiceId /* serviceId */) {}
  void onServiceDown(const std::string & /* serviceName */,
                     NLNET::TServiceId /* serviceId */) {}
  void onModuleUpdate() {
    ...
    while (!_LocalMessages.empty()) {
      ...
      if (senderProx == NULL) {
        nlwarning(
            "CStandardGateway : local message dispatching : Failed to retrieve "
            "proxy for sender module %u while dispatching message '%s' to %u",
            lm.SenderProxyId, lm.Message.getName().c_str(), lm.AddresseProxyId);
      } else if (addresseeProx == NULL) {
        nlwarning("CStandardGateway : local message dispatching : Failed to "
                  "retrieve proxy for addressee module %u while dispatching "
                  "message '%s' from %u",
                  lm.AddresseProxyId, lm.Message.getName().c_str(),
                  lm.SenderProxyId);
      } else {
    ...
    {
      TTransportList::iterator first(_Transports.begin()),
          last(_Transports.end());
      for (; first != last; ++first) {
  ...
  bool onProcessModuleMessage(IModuleProxy * /* senderModuleProxy */,
                              const CMessage &message) {
  ...
  void onModuleSocketEvent(IModuleSocket * /* moduleSocket */,
  ...
  const std::string &getSocketName() { return getModuleName(); }
  ...
  void _sendModuleMessage(IModule *senderModule, TModuleId destModuleProxyId,
    ...
    TLocalModuleIndex::iterator it(
        _LocalModuleIndex.find(senderModule->getModuleId()));
  ...
  void onModulePlugged(IModule *pluggedModule) {
    nldebug("NETL6: Gateway %s : plugging module '%s' id=%u",
            getModuleName().c_str(), pluggedModule->getModuleName().c_str(),
    ...
    _LocalModuleIndex.insert(
        make_pair(pluggedModule->getModuleId(), modProx->getModuleProxyId()));
    ...
    //
    // second, disclose already known proxies in the gateway to the plugged
    // module
    {
      TModuleProxies::iterator first(_ModuleProxies.begin()),
          last(_ModuleProxies.end());
      for (; first != last; ++first) {
        ...
        if (modProx->getGatewayRoute() != NULL ||
            modProx->getForeignModuleId() != pluggedModule->getModuleId()) {
  ...
  void onModuleUnplugged(IModule *unpluggedModule) {
    nldebug("NETL6: Gateway %s : unplugging module '%s' id=%u",
            getModuleName().c_str(), unpluggedModule->getModuleName().c_str(),
            unpluggedModule->getModuleId());

    // remove the proxy info
    TLocalModuleIndex::iterator it(
        _LocalModuleIndex.find(unpluggedModule->getModuleId()));
    nlassert(it != _LocalModuleIndex.end());
    TModuleProxies::iterator it2(_ModuleProxies.find(it->second));
    nlassert(it2 != _ModuleProxies.end());
    ...
    // warn the unplugged module that all proxies in this gateway become
    // unavailable
    {
      TModuleProxies::iterator first(_ModuleProxies.begin()),
          last(_ModuleProxies.end());
    ...
    // this module
    TLocalMessageList::iterator first(_LocalMessages.begin()),
        last(_LocalMessages.end());
    for (; first != last; ++first) {
      ...
      if (lm.AddresseProxyId == localProxyId ||
        ...
        // erase this message !
        nlwarning(
            "CStandardGateway : while unplugging module %u from the gateway, "
            "locale message '%s' from proxy %u to proxy %u is lost",
            unpluggedModule->getModuleId(), lm.Message.getName().c_str(),
            lm.SenderProxyId, lm.AddresseProxyId);
  ...
  void removeForeignModule(CGatewayRoute *route, TModuleId foreignModuleId) {
    ...
    if (pModuleId == NULL) {
      // oups !
      nlwarning("removeForeignModule : unknown foreign module id %u",
                foreignModuleId);
      return;
    }
    ...
    if (it2 == _ModuleProxies.end()) {
      // oups !
      nlwarning("Gateway '%s' : removeForeignModule : can't find proxy for id "
                "%u coming from foreign id %u",
                getGatewayName().c_str(), proxyId, foreignModuleId);
    ...
    nlassert(found == true);
    // NB : stl debug mode don't allow to test with range.first when range;first
    // is erased.
    //				nlassert(range.first != range.second);
    ...
    if (range.first != range.second) {
      ...
      if (modProx->_Route != kmi.Route) {
        ...
        nldebug(
            "Gateway '%s' : use a new route for module '%s' from %u to %u hop",
            getGatewayName().c_str(), modProx->getModuleName().c_str(),
        ...
        if (modProx->_Distance != kmi.ModuleDistance) {
          // the distance has changed, update and send the new distance to other
          // gateway
          modProx->_Distance = kmi.ModuleDistance;
  ...
  void sendModuleDistanceUpdate(IModuleProxy *proxy) {
    ...
    for (; first != last; ++first) {
      ...
      if (isModuleProxyVisible(proxy, route)) {
        ...
        //					// TODO : optimize by batch
        //sending 					TModuleDistanceChangeMsg mdu;
        //
        //					mdu.ModuleId =
        //proxy->getModuleProxyId(); 					mdu.NewDistance =
        //proxy->getModuleDistance()+1;
        //
  ...
  bool isModuleProxyVisible(IModuleProxy *proxy, CGatewayRoute *route) {
    ...
    // if the module is on the same route, it can't be seen (it is seen by the
    // route outbound)
    ...
    if (proxy->getGatewayRoute()->getTransport() != transport) {
      ...
      //				CGatewayRoute::TForeignToLocalIdx::iterator
      //it(route->ForeignToLocalIdx.find(proxy->getForeignModuleId()));
  ...
  void updateModuleSecurityDataToRoute(CGatewayRoute *route,
                                       IModuleProxy *proxy) {
    CGatewayRoute::TPendingEvent pe;
  ...
  void sendPendingModuleUpdate(CGatewayRoute *route) {
    ...
    //
    //				std::set<IModuleProxy*>::iterator
    //first(route->PendingDisclosure.begin()),
    //last(route->PendingDisclosure.end()); 				for (uint i=0; first != last; ++i,
    //++first)
    //				{
    //					TModuleDescMsg &modDesc =
    //message.Modules[i]; 					IModuleProxy *addedModule = *first;
    //
    //					modDesc.ModuleProxyId =
    //addedModule->getModuleProxyId(); 					modDesc.ModuleClass	 =
    //addedModule->getModuleClassName(); 					modDesc.ModuleFullName =
    //addedModule->getModuleName(); 					modDesc.ModuleDistance =
    //addedModule->getModuleDistance()+1;
    //				}
    ...
    //				std::copy(route->PendingUndisclosure.begin(),
    //route->PendingUndisclosure.end(), back_insert_iterator<vector<TModuleId>
    //>(message.RemovedModules)); 				route->PendingUndisclosure.clear();
    //
  ...
  NLMISC_COMMAND_HANDLER_ADD(
      CStandardGateway, dump,
      "dump various information about the gateway statue", "")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, transportListAvailableClass,
                             "list the available transport class", "no param")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, transportAdd,
                             "add a new transport to this gateway",
                             "<transportClass> <instanceName>")
  NLMISC_COMMAND_HANDLER_ADD(
      CStandardGateway, transportOptions,
      "set a gateway level option on a transport",
      "<transportClass> ( [PeerInvisible] [Firewalled] )")
  NLMISC_COMMAND_HANDLER_ADD(
      CStandardGateway, transportCmd, "send a command to a transport",
      "[<transportName> ( <cmd specific to transport> )]*")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, transportRemove,
                             "remove an existing transport instance",
                             "<transportName>")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, securityListAvailableClass,
                             "list the available security class", "no param")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, securityCreate,
                             "create a security plug-in", "<securityClassName>")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, securityCommand,
                             "send a command to the security plug-in",
                             "<cmd specific to plug-in>")
  NLMISC_COMMAND_HANDLER_ADD(CStandardGateway, securityRemove,
  ...
  NLMISC_CLASS_COMMAND_DECL(securityCreate) {
    ...
    if (_SecurityPlugin != NULL) {
      log.displayNL(
          "The gateway already have a security plug-in ! Remove it first");
      return true;
    }

    log.displayNL("Creating a security plug-in '%s' in gateway '%s'",
                  args[0].c_str(), getModuleName().c_str());
  ...
  NLMISC_CLASS_COMMAND_DECL(transportOptions) {
    ...
    //			IGatewayTransport *transport =
    //_Transports.find(transName)->second;
  ...
  NLMISC_CLASS_COMMAND_DECL(dump) {
    ...
    log.displayNL("The gateway has %u locally plugged module :",
                  _PluggedModules.getAToBMap().size());
    {
      TPluggedModules::TAToBMap::const_iterator first(
          _PluggedModules.getAToBMap().begin()),
          last(_PluggedModules.getAToBMap().end());
      for (; first != last; ++first) {
        ...
        log.displayNL("    ID:%5u : \tName = '%s' \tclass = '%s'",
                      module->getModuleId(), module->getModuleName().c_str(),
                      module->getModuleClassName().c_str());
      }
    }

    log.displayNL("The gateway as %u transport activated :",
                  _Transports.size());
    {
      TTransportList::iterator first(_Transports.begin()),
          last(_Transports.end());
      for (; first != last; ++first) {
        ...
        log.displayNL("Transport '%s' (transport class is '%s') :",
                      name.c_str(), transport->getClassName().c_str());
        log.displayNL("  * %s", transport->PeerInvisible
                                    ? "Peer module are NON visible"
                                    : "Peer modules are visible");
        log.displayNL("  * %s",
                      transport->Firewalled ? "Firewall ON" : "Firewall OFF");
        transport->dump(log);
      }
    }
 ...
 *	existing one.
 */
void CGatewaySecurity::setSecurityData(IModuleProxy *proxy,
                                       TSecurityData *securityData) {
 ...
 */
void CGatewaySecurity::replaceAllSecurityDatas(IModuleProxy *proxy,
                                               TSecurityData *securityData) {
  // forward the call to standard gateway
  CStandardGateway *sg = static_cast<CStandardGateway *>(_Gateway);

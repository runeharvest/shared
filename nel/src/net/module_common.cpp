...
TParsedCommandLine::~TParsedCommandLine() { clear(); };

void TParsedCommandLine::clear() {
  for (std::vector<TParsedCommandLine *>::iterator it = SubParams.begin();
       it != SubParams.end(); ++it) {
    delete (*it);
  }
  SubParams.clear();
...
bool TParsedCommandLine::_parseParamList(const std::string &rawParamString) {
  ...
  for (CSString part = parsedString.strtok(" \t", true, false); !part.empty();
       part = parsedString.strtok(" \t", true, false)) {
    if (part[0] == '(') {
      // this is a sub parameter list
      if (SubParams.empty() || SubParams.back()->ParamName.empty()) {
        nlwarning("While parsing param string '%s', missing param header",
                  rawParamString.c_str());
        return false;
      }
      if (!SubParams.back()->ParamValue.empty()) {
        nlwarning("While parsing param string '%s', Invalid sub param header "
                  "'%s' for sub part '%s', must not define value",
                  rawParamString.c_str(), SubParams.back()->ParamName.c_str(),
                  part.c_str());
      ...
      if (part[part.size() - 1] != ')') {
        nlwarning("While parsing param string '%s', Invalid sub param value "
                  "'%s' missing closing ')'",
                  rawParamString.c_str(), part.c_str());
      ...
      if (!SubParams.back()->_parseParamList(part)) {
        nlwarning("Error parsing sub param list for header '%s' in '%s'",
                  SubParams.back()->ParamName.c_str(), rawParamString.c_str());
        return false;
      }
    } else if (part[part.size() - 1] == ')') {
      nlwarning("While parsing param string '%s', Invalid param value '%s' : "
                "missing openning '('",
                rawParamString.c_str(), part.c_str());
      ...
      if (SubParams.empty() || !SubParams.back()->ParamValue.empty()) {
        nlwarning("While parsing param string '%s', param '%s' already have "
                  "the value '%s'",
                  rawParamString.c_str(), SubParams.back()->ParamName.c_str(),
      ...
      if (name.empty()) {
        nlwarning(
            "Can't find param name for value '%s' in the param string '%s'",
            part.c_str(), rawParamString.c_str());
        return false;
      }
...
const TParsedCommandLine *
TParsedCommandLine::getParam(const std::string &name) const {
  vector<string> parts;
...
void TParsedCommandLine::setParam(const std::string &name,
...
const TParsedCommandLine *
TParsedCommandLine::_getParam(std::vector<std::string>::iterator it,

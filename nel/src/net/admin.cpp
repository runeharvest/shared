...
struct CRequest {
  CRequest(uint32 id, TServiceId sid)
      : Id(id), NbWaiting(0), NbReceived(0), SId(sid) {
...
static void cbInfo(CMessage &msgin, const std::string & /* serviceName */,
                   TServiceId /* sid */) {
...
static void cbServGetView(CMessage &msgin,
...
// AES wants to know if i'm not dead, I have to answer faster as possible or
// i'll be killed
static void cbAdminPing(CMessage & /* msgin */,
                        const std::string & /* serviceName */, TServiceId sid) {
  // Send back a pong to say to the AES that I'm alive
  CMessage msgout("ADMIN_PONG");
  CUnifiedNetwork::getInstance()->send(sid, msgout);
}

static void cbStopService(CMessage & /* msgin */,
                          const std::string &serviceName, TServiceId sid) {
  nlinfo("ADMIN: Receive a stop from service %s-%hu, need to quit",
         serviceName.c_str(), sid.get());
  IService::getInstance()->exit(0xFFFF);
}

void cbAESConnection(const string & /* serviceName */, TServiceId /* sid */,
                     void * /* arg */) {
  // established a connection to the AES, identify myself

  //
  // Sends the identification message with the name of the service and all
  // commands available on this service
  //

  nlinfo("cbAESConnection: Identifying self as: AliasName='%s' LongName='%s' "
         "PId=%u",
         IService::getInstance()->_AliasName.c_str(),
         IService::getInstance()->_LongName.c_str(), getpid());
  CMessage msgout("SID");
  uint32 pid = getpid();
  msgout.serial(IService::getInstance()->_AliasName,
                IService::getInstance()->_LongName, pid);
...
static void cbAESDisconnection(const std::string &serviceName, TServiceId sid,
                               void * /* arg */) {
...
static void addRequestWaitingNb(uint32 rid) {
  for (uint i = 0; i < Requests.size(); i++) {
    if (Requests[i].Id == rid) {
      ...
      nldebug("ADMIN: ++ i %d rid %d NbWaiting+ %d NbReceived %d", i,
              Requests[i].Id, Requests[i].NbWaiting, Requests[i].NbReceived);
...
static void subRequestWaitingNb (uint32 rid)
{
    for (uint i = 0 ; i < Requests.size (); i++)
    {
        if (Requests[i].Id == rid)
        {
            Requests[i].NbWaiting--;
            nldebug ("ADMIN: ++ i %d rid %d NbWaiting- %d NbReceived %d", i,
Requests[i].Id, Requests[i].NbWaiting, Requests[i].NbReceived); return;
        }
    }
...
void addRequestAnswer(uint32 rid, const TAdminViewVarNames &varNames,
...
static void cleanRequest() {
  ...
  for (uint i = 0; i < Requests.size();) {
    ...
    if (currentTime >= Requests[i].Time + RequestTimeout) {
      nlwarning("ADMIN: **** i %d rid %d -> Requests[i].NbWaiting (%d) != "
                "Requests[i].NbReceived (%d)",
                i, Requests[i].Id, Requests[i].NbWaiting,
                Requests[i].NbReceived);
    ...
    if (Requests[i].NbWaiting <= Requests[i].NbReceived) {
      ...
      if (Requests[i].SId.get() == 0) {
        nlinfo(
            "ADMIN: Receive an answer for the fake request %d with %d answers",
            Requests[i].Id, Requests[i].Answers.size());
        for (uint j = 0; j < Requests[i].Answers.size(); j++) {
          uint k;
          for (k = 0; k < Requests[i].Answers[j].VarNames.size(); k++) {
            InfoLog->displayRaw("%-10s",
                                Requests[i].Answers[j].VarNames[k].c_str());
          }
          InfoLog->displayRawNL("");
          for (k = 0; k < Requests[i].Answers[j].Values.size(); k++) {
            InfoLog->displayRaw("%-10s",
                                Requests[i].Answers[j].Values[k].c_str());
          }
          InfoLog->displayRawNL("");
          InfoLog->displayRawNL("-------------------------");
        }
      ...
      nldebug("ADMIN: ++ i %d rid %d NbWaiting0 %d NbReceived %d", i,
              Requests[i].Id, Requests[i].NbWaiting, Requests[i].NbReceived);
    }
...
void serviceGetView(uint32 rid, const string &rawvarpath,
                    TAdminViewResult &answer, bool async) {
  string str;
  CLog logDisplayVars;
  ...
  if (CCommandRegistry::getInstance().isNamedCommandHandler(
          varpath.Destination[0].first)) {
  ...
  if (varpath.isFinal()) {
    ...
    for (uint j = 0; j < varpath.Destination.size(); j++) {
      ...
      if (ICommand::isCommand(cmd)) {
        ...
        nlinfo("ADMIN: Add to result view '%s' = '%s'",
               varpath.Destination[j].first.c_str(), str.c_str());
      }
      ...
      for (uint j = 0; j < subvarpath.Destination.size(); j++) {
        ...
        if (isRemoteCommand(cmd)) {
          if (async && RemoteClientCallback != 0) {
            // ok we have to send the request to another side, just send and
            // wait
            addRequestWaitingNb(rid);
          ...
          for (uint k = 0; k < strs.size(); k++) {
            ...
            if (!async) {
              ...
              if (y == answer.size()) {
                ...
                // todo when we work on entity, we don't need service name and
                // server so we should remove them and collapse all var for the
                // same entity
                varNames->push_back("service");
            ...
            nlinfo("ADMIN: Add to result view for entity '%s', '%s' = '%s'",
                   varpath.Destination[i].first.c_str(),
                   subvarpath.Destination[j].first.c_str(), str.c_str());
          }
          mdDisplayVars.unlockStrings();
        }
      }
...
void initAdmin(bool dontUseAES) {
  if (!dontUseAES) {
    ...
    CUnifiedNetwork::getInstance()->setServiceDownCallback(
        "AES", cbAESDisconnection, NULL);
    CUnifiedNetwork::getInstance()->addService("AES",
                                               CInetHost("localhost:43997"));
  }
  CUnifiedNetwork::getInstance()->addCallbackArray(
      CallbackArray, sizeof(CallbackArray) / sizeof(CallbackArray[0]));
}
  ...
  if (CurrentTime >= lastGraphUpdateCheck + 1) {
    ...
    for (uint j = 0; j < GraphUpdates.size(); j++) {
      if (CurrentTime >= GraphUpdates[j].LastUpdate + GraphUpdates[j].Update) {
        ...
        sint32 val;
        if (strs.size() != 1) {
          nlwarning("ADMIN: The graph update command execution not return "
                    "exactly 1 line but %d",
                    strs.size());
  ...
  if (CurrentTime >= lastAlarmsCheck + AlarmCheckDelay) {
    ...
    for (uint i = 0; i < Alarms.size();) {
      ...
      if (str == "???") {
        ...
        nlwarning("ADMIN: Alarm problem: variable '%s' returns ??? instead of "
                  "a good value",
                  Alarms[i].Name.c_str());
        ...
        if (Alarms[i].GT && val >= err) {
          if (!Alarms[i].Activated) {
            nlinfo("ADMIN: VARIABLE TOO BIG '%s' %u >= %u",
                   Alarms[i].Name.c_str(), val, err);
            Alarms[i].Activated = true;
            sendAdminEmail("Alarm: Variable %s is %u that is greater or equal "
                           "than the limit %u",
                           Alarms[i].Name.c_str(), val, err);
          }
        } else if (!Alarms[i].GT && val <= err) {
          if (!Alarms[i].Activated) {
            nlinfo("ADMIN: VARIABLE TOO LOW '%s' %u <= %u",
                   Alarms[i].Name.c_str(), val, err);
            Alarms[i].Activated = true;
            sendAdminEmail("Alarm: Variable %s is %u that is lower or equal "
                           "than the limit %u",
                           Alarms[i].Name.c_str(), val, err);
          }
        } else {
          if (Alarms[i].Activated) {
            nlinfo("ADMIN: variable is ok '%s' %u %s %u",
                   Alarms[i].Name.c_str(), val, (Alarms[i].GT ? "<" : ">"),
                   err);
...
void setInformation(const vector<string> &alarms,
  ...
  for (i = 0; i < alarms.size(); i += 3) {
    ...
    if (shardvarpath.Destination.empty() ||
        shardvarpath.Destination[0].second.empty())
      continue;
    CVarPath servervarpath(shardvarpath.Destination[0].second);
    if (servervarpath.Destination.empty() ||
        servervarpath.Destination[0].second.empty())
      continue;
    CVarPath servicevarpath(servervarpath.Destination[0].second);
    if (servicevarpath.Destination.empty() ||
        servicevarpath.Destination[0].second.empty())
      continue;

    string name = servicevarpath.Destination[0].second;

    if (IService::getInstance()->getServiceUnifiedName().find(
            servicevarpath.Destination[0].first) != string::npos &&
        ICommand::exists(name)) {
      fromString(alarms[i + 1], tmp);
      nlinfo("ADMIN: Adding alarm '%s' limit %d order %s (varpath '%s')",
             name.c_str(), tmp, alarms[i + 2].c_str(), alarms[i].c_str());
      Alarms.push_back(CAlarm(name, tmp, alarms[i + 2] == "gt"));
    } else {
      if (IService::getInstance()->getServiceUnifiedName().find(
              servicevarpath.Destination[0].first) == string::npos) {
        nlinfo("ADMIN: Skipping alarm '%s' limit %d order %s (varpath '%s') "
               "(not for my service, i'm '%s')",
               name.c_str(), fromString(alarms[i + 1], tmp) ? tmp : tmp,
               alarms[i + 2].c_str(), alarms[i].c_str(),
  ...
  for (i = 0; i < graphupdate.size(); i += 2) {
    ...
    if (ICommand::exists(VarName) &&
      ...
      nlinfo("ADMIN: Adding graphupdate '%s' update %d (varpath '%s')",
             VarName.c_str(), tmp, graphupdate[i].c_str());
      GraphUpdates.push_back(CGraphUpdate(VarName, tmp));
    } else {
      if (IService::getInstance()->getServiceShortName() != ServiceName) {
        nlinfo("ADMIN: Skipping graphupdate '%s' limit %d (varpath '%s') (not "
               "for my service, i'm '%s')",
               VarName.c_str(), fromString(graphupdate[i + 1], tmp) ? tmp : tmp,
               graphupdate[i].c_str(),
               IService::getInstance()->getServiceUnifiedName().c_str());
      } else {
        nlinfo("ADMIN: Skipping graphupdate '%s' limit %d (varpath '%s') (var "
               "not exist)",
               VarName.c_str(), fromString(graphupdate[i + 1], tmp) ? tmp : tmp,
               graphupdate[i].c_str());
      }
...
NLMISC_CATEGORISED_COMMAND(nel, displayInformation,
  ...
  for (i = 0; i < Alarms.size(); i++) {
    log.displayNL(" %d %s %d %s %s", i, Alarms[i].Name.c_str(), Alarms[i].Limit,
                  (Alarms[i].GT ? "gt" : "lt"),
                  (Alarms[i].Activated ? "on" : "off"));
  }
  log.displayNL("There're %d graphupdate:", GraphUpdates.size());
  for (i = 0; i < GraphUpdates.size(); i++) {
    log.displayNL(" %d %s %d %d", i, GraphUpdates[i].Name.c_str(),
                  GraphUpdates[i].Update, GraphUpdates[i].LastUpdate);
  }
  return true;
}

NLMISC_CATEGORISED_COMMAND(nel, getView,
  ...
  for (uint i = 0; i < answer.size(); i++) {
    ...
    for (uint j = 0; j < answer[i].VarNames.size(); j++) {
      log.displayNL("    %s -> %s", answer[i].VarNames[j].c_str(),
                    answer[i].Values[j].c_str());
    }
  }

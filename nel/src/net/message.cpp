 ...
 * Constructor by name
 */
CMessage::CMessage(const std::string &name, bool inputStream,
                   TStreamFormat streamformat, uint32 defaultCapacity)
    : NLMISC::CMemStream(inputStream, false, defaultCapacity), _Type(OneWay),
      _SubMessagePosR(0), _LengthR(0), _HeaderSize(0xFFFFFFFF),
      _TypeSet(false) {
...
CMessage::CMessage(NLMISC::CMemStream &memstr)
    : NLMISC::CMemStream(memstr), _Type(OneWay), _SubMessagePosR(0),
      _LengthR(0), _HeaderSize(0xFFFFFFFF), _TypeSet(false) {
  sint32 pos = getPos();
 ...
 * Copy constructor
 */
CMessage::CMessage(const CMessage &other) : CMemStream(), _TypeSet(false) {
  operator=(other);
}
...
CMessage &CMessage::operator=(const CMessage &other) {
  //	nlassertex( (!other.isReading()) || (!other.hasLockedSubMessage()),
  //("Storing %s", LockedSubMessageError) );
  nlassertex((!isReading()) || (!hasLockedSubMessage()),
             ("Assigning %s", LockedSubMessageError));
...
/**
 * Similar to operator=, but makes the current message contain *only* the locked
 * sub message in msgin or the whole msgin if it is not locked
 *
 * Preconditions:
 ...
 * contains only the locked sub message in msgin, otherwise the current message
 * is exactly msgin
...
void CMessage::setType(const std::string &name, TMessageType type) {
  ...
  if (!isReading()) {
    ...
    // debug features, we number all packet to be sure that they are all sent
    // and received \todo remove this debug feature when ok this value will be
    // fill after in the callback function
...
/*
 * The message was filled with an CMemStream, Now, we'll get the message type on
 * this buffer
 */
void CMessage::readType() {
  nlassert(isReading());

  // debug features, we number all packet to be sure that they are all sent and
  // received \todo remove this debug feature when ok
...
bool CMessage::typeIsSet() const { return _TypeSet; }

// Clear the message. With this function, you can reuse a message to create
// another message
void CMessage::clear() {
  nlassertex((!isReading()) || (!hasLockedSubMessage()),
             ("Clearing %s", LockedSubMessageError));
...
/*
 * Returns the type name in string if available. Be sure that the message have
...
std::string CMessage::getName() const {
  if (hasLockedSubMessage()) {
    ...
    notconstMsg.seek(
        subPosSaved,
        begin); // not really const... but removing the const from getName()
                // would need too many const changes elsewhere
...
/*
 * Return an input stream containing the stream beginning in the message at the
 * specified pos
 */

 ...
 * Constructor
 */
CCallbackServer::CCallbackServer(TRecordingState rec, const string &recfilename,
                                 bool recordall, bool initPipeForDataAvailable)
    : CCallbackNetBase(rec, recfilename, recordall),
      CBufServer(DEFAULT_STRATEGY, DEFAULT_MAX_THREADS,
                 DEFAULT_MAX_SOCKETS_PER_THREADS, true, rec == Replay,
                 initPipeForDataAvailable),
      _ConnectionCallback(NULL), _ConnectionCbArg(NULL) {
#ifndef USE_MESSAGE_RECORDER
  nlassertex(rec == Off, ("LNETL3S: The message recorder is disabled at "
                          "compilation time ; switch the recording state Off"));
#endif
 ...
 * Replayed : MAYBE
 */
void CCallbackServer::send(const CMessage &buffer, TSockId hostid,
                           bool /* log */) {
  nlassert(connected());
  ...
  //	if (log)
  {
    //		LNETL3_DEBUG ("LNETL3S: Server: send(%s, %s)",
  ...
  if (_MR_RecordingState != Replay) {
    ...
    if (_MR_RecordingState == Record) {
      // Record sent message
      _MR_Recorder.recordNext(_MR_UpdateCounter, Sending, hostid,
                              const_cast<CMessage &>(buffer));
    }
  }
#endif
}
...
void CCallbackServer::receive(CMessage &buffer, TSockId *hostid) {
  ...
  if (_MR_RecordingState != Replay) {
    ...
    if (_MR_RecordingState == Record) {
      ...
      _MR_Recorder.recordNext(_MR_UpdateCounter, Receiving, *hostid,
                              const_cast<CMessage &>(buffer));
    }
  } else {
 ...
 * nothing. Before disconnecting, any pending data is actually sent. Recorded :
 * YES in noticeDisconnection called in the disconnection callback Replayed :
 * YES in noticeDisconnection called in the disconnection callback
 */
void CCallbackServer::disconnect(TSockId hostid) {
  ...
  // else, no need to manually replay the disconnection, such as in
  // CCallbackClient, it will be replayed during the next update()
#endif
}
...
bool CCallbackServer::replaySystemCallbacks() {
  do {
    if (_MR_Recorder.ReceivedMessages.empty()) {
      ...
      TSockId sockid;
      std::map<TSockId, TSockId>::iterator isi =
          _MR_SockIds.find(_MR_Recorder.ReceivedMessages.front().SockId);
      ...
      case Accepting: {
        ...
        _MR_SockIds.insert(
            make_pair(_MR_Recorder.ReceivedMessages.front().SockId, sockid));

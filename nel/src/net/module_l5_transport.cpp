...
public:
  ...
  CL5Route(IGatewayTransport *transport) : CGatewayRoute(transport) {}
...
private:
  ...
  CTransportIdAllocator() {
    ...
    for (TL5TransportId i = 0;
         i < TL5TransportId(UINT64_CONSTANT(0xffffffffffffffff)); ++i) {
...
public:
  ...
  CGatewayL5Transport(const IGatewayTransport::TCtorParam &param)
      : IGatewayTransport(param), _Open(false) {
  ...
  ~CGatewayL5Transport() {
    ...
    nlassert(_TransportDispatcher.find(_TransportId) !=
             _TransportDispatcher.end());
  ...
  void dump(NLMISC::CLog &log) const {
    ...
    if (!_Open) {
      ...
      for (; first != last; ++first) {
        ...
        log.displayNL(
            "    + route to service %hu('%s'), %u entries in the proxy "
            "translation table :",
            sid.get(),
  ...
  bool onCommand(const TParsedCommandLine &command) {
    ...
    if (commandName == "open") {
      ...
      const TParsedCommandLine *netName =
          command.SubParams[0]->getParam("SubNet");
      if (netName != NULL) {
  ...
  void open(const std::string &subNetName) {
    ...
    if (!callbackRegistered) {
      ...
      un->addCallbackArray(L5TransportCallback,
    ...
    while (!uniqueService.empty()) {
      ...
      if (un->isConnectionConnected(sid)) {
        ...
        // the Connection is not established right now. We wait for the
        // ServiceUp callback
      }
    }
  ...
  void onDispatchMessage(const CMessage &msgin, TServiceId sid) {
    ...
    LNETL6_DEBUG("LNETL6: L5 transport onDispatchMessage from service %hu",
                 sid.get());
    /// retrieve the route for dispatching
    TRouteMap::iterator it(_Routes.find(sid));
    if (it == _Routes.end()) {
      nlwarning("Gateway '%s' : Can't find route for service %hu for "
                "dispatching, message is discarded",
                _Gateway->getGatewayName().c_str(), sid.get());
      return;
    }
  ...
  void onAddTransport(TServiceId sid, TTransportDesc &desc) {
    ...
    LNETL6_DEBUG("LNETL6: L5 transport onAddTransport from service %hu",
                 sid.get());
    ...
    if (_Routes.find(sid) != _Routes.end()) {
      LNETL6_DEBUG("LNETL6: L5 transport onAddTransport a route for this "
                   "service alredy exist");
      return;
    }
  ...
  void onRemoveTransport(TServiceId sid, TTransportDesc &desc) {
    ...
    LNETL6_DEBUG("LNETL6: L5 transport onRemoveTransport from service %hu",
                 sid.get());
    // Remove the route
    TRouteMap::iterator it(_Routes.find(sid));
    if (it == _Routes.end()) {
      nlwarning("onRemoveTransport : can't find a route to the transport %hu "
                "on service %u",
                desc.TransportId, sid.get());
      return;
    }
  ...
  static void cbL5AddTransport(CMessage &msgin,
                               const std::string & /* serviceName */,
                               TServiceId sid) {
    LNETL6_DEBUG("LNETL6: L5 transport cbL5AddTransport from service %hu",
                 sid.get());
    ...
    TTransportDispatcher::iterator first(_TransportDispatcher.begin()),
        last(_TransportDispatcher.end());
    for (; first != last; ++first) {
      CGatewayL5Transport *transport = first->second;
      if (transport->_Open && transport->_SubNetName == desc.SubNetName &&
  ...
  static void cbL5RemoveTransport(CMessage &msgin,
                                  const std::string & /* serviceName */,
                                  TServiceId sid) {
    LNETL6_DEBUG("LNETL6: L5 transport cbL5RemoveTransport from service %hu",
                 sid.get());
  ...
  static void cbDispatchL5Message(CMessage &msgin,
                                  const std::string &serviceName,
                                  TServiceId sid) {
    LNETL6_DEBUG("LNETL6: L5 transport cbDispatch called, receiving from %s",
                 serviceName.c_str());
    ...
    if (it == _TransportDispatcher.end()) {
      nlwarning("ReceiveL5Message, can't find transport id %u for dispatching, "
  ...
  static void cbOnServiceUp(const std::string &serviceName, TServiceId sid,
                            void * /* arg */) {
    LNETL6_DEBUG("LNETL6: L5 transport cbOnServiceUp called, service up for %s",
                 serviceName.c_str());
  ...
  static void cbOnServiceDown(const std::string &serviceName, TServiceId sid,
                              void * /* arg */) {
    LNETL6_DEBUG(
        "LNETL6: L5 transport cbOnServicedown called, service down for %s",
        serviceName.c_str());
...
CGatewayL5Transport::TTransportDispatcher
    CGatewayL5Transport::_TransportDispatcher;

// register this class in the transport factory
NLMISC_REGISTER_OBJECT(IGatewayTransport, CGatewayL5Transport, std::string,

...
public:
  ...
  CL3ServerRoute(IGatewayTransport *transport)
      : CGatewayRoute(transport), LastCommTime(CTime::getSecondsSince1970()) {}
...
public:
  ...
  void dump(NLMISC::CLog &log) const {
    ...
    if (_CallbackServer.get() == NULL) {
      ...
      for (; first != last; ++first) {
        ...
        CL3ServerRoute *route = first->second;
        log.displayNL(
            "    + route to '%s', %u entries in the proxy translation table :",
  ...
  /// Open the server by starting listing for incoming connection on the
  /// specified port
  void openServer(uint16 port) {
    ...
    cbs->setConnectionCallback(cbConnection,
                               static_cast<IGatewayTransport *>(this));
    cbs->setDisconnectionCallback(cbDisconnection,
                                  static_cast<IGatewayTransport *>(this));
  ...
  void onDispatchMessage(const CMessage &msgin, TSockId from,
  ...
  static void cbConnection(TSockId from, void *arg) {
    nlassert(arg != NULL);
    CGatewayL3ServerTransport *transport =
        dynamic_cast<CGatewayL3ServerTransport *>(
            static_cast<IGatewayTransport *>(arg));
  ...
  // Forward to the real method, do the dispatching to the correct
  // CGatewayL3ServerTransport instance
  static void cbDispatchMessage(CMessage &msgin, TSockId from,
                                CCallbackNetBase &netbase) {
...
CGatewayL3ServerTransport::TDispatcherIndex
    CGatewayL3ServerTransport::_DispatcherIndex;

// register this class in the transport factory
NLMISC_REGISTER_OBJECT(IGatewayTransport, CGatewayL3ServerTransport,
                       std::string, string(LAYER3_SERVER_CLASS_NAME));

void CL3ServerRoute::sendMessage(const CMessage &message) const {
  H_AUTO(L3SRoute_sendMessage);
  NLNET_AUTO_DELTE_ASSERT;

  CGatewayL3ServerTransport *trpt =
      static_cast<CGatewayL3ServerTransport *>(_Transport);
...
public:
  ...
  CL3ClientRoute(IGatewayTransport *transport, CInetHost serverAddr,
                 uint32 connId)
      : CGatewayRoute(transport), ServerAddr(serverAddr),
        LastCommTime(CTime::getSecondsSince1970()), LastConnectionRetry(0),
        ConnId(connId) {}
...
public:
  ...
  CGatewayL3ClientTransport(const IGatewayTransport::TCtorParam &param)
      : IGatewayTransport(param), _RetryInterval(RETRY_INTERVAL) {}
    ...
    for (; first != last; ++first) {
      ...
      if (!route->CallbackClient.connected()) {
        ...
        if (route->LastConnectionRetry + _RetryInterval < now) {
          ...
          try {
            nldebug("Connecting to %s...",
                    route->ServerAddr.toStringLong().c_str());
            route->CallbackClient.connect(route->ServerAddr);
            nldebug("Connected to %s",
                    route->ServerAddr.toStringLong().c_str());
            _Gateway->onRouteAdded(route);
          } catch (...) {
            nlinfo("Server %s still not available for connection",
                   route->ServerAddr.toStringLong().c_str());
          }
        }
      } else {
        ...
        // avoid cross checking of client and server. If server is alive, then
        // we receive the server keep alive packet a little before we need to
        // send the client one, thus reseting the keep alive timer.
  ...
  void dump(NLMISC::CLog &log) const {
    ...
    for (; first != last; ++first) {
      CL3ClientRoute *route = first->second;
      log.displayNL("    + route to '%s', %s, %u entries in the proxy "
                    "translation table :",
                    route->ServerAddr.toStringLong().c_str(),
                    route->CallbackClient.connected() ? "connected"
                                                      : "NOT CONNECTED",
                    route->ForeignToLocalIdx.getAToBMap().size());
      {
        CGatewayRoute::TForeignToLocalIdx::TAToBMap::const_iterator first(
        ...
        for (; first != last; ++first) {
          ...
          log.displayNL(
              "      - Proxy '%s' : local proxy id %u => foreign module id %u",
              modProx != NULL ? modProx->getModuleName().c_str()
                              : "ERROR, invalid module",
              first->second, first->first);
        }
  ...
  bool onCommand(const TParsedCommandLine &command) {
    ...
    if (commandName == "connect") {
      ...
      nldebug("CGatewayL3ClientTransport : setting retry interval to %u",
              _RetryInterval);
    } else
      return false;
  ...
  void connect(CInetHost &addr) {
    ...
    // set the callbacks
    route->CallbackClient.setDisconnectionCallback(
        cbDisconnection, static_cast<IGatewayTransport *>(this));
    route->CallbackClient.setDefaultCallback(cbDispatchMessage);

    try {
      nldebug("CGatewayL3ClientTransport : Connecting to %s...",
              addr.toStringLong().c_str());
      route->LastConnectionRetry = CTime::getSecondsSince1970();
      // connect to the server
      route->CallbackClient.connect(addr);
      nldebug("CGatewayL3ClientTransport : Connected to %s with connId %u",
              addr.toStringLong().c_str(), connId);
    } catch (const ESocketConnectionFailed &) {
      nlinfo("CGatewayL3ClientTransport : Failed to connect to server %s, "
             "retrying in %u seconds",
             addr.toStringLong().c_str(), _RetryInterval);
    }
  ...
  void close(uint32 connId) {
    ...
    if (connId >= _RouteIds.size()) {
      nlwarning(
          "CGatewayL3ClientTransport : Invalid connectionId %u, max is %u",
          connId, _RouteIds.size() - 1);
      return;
    }

    if (_RouteIds[connId] == NULL) {
      nlwarning("CGatewayL3ClientTransport : Invalid connectionId %u, the "
                "connection is unused now.",
                connId);
      return;
    }
    ...
    nldebug("CGatewayL3ClientTransport : Closing connection %u to %s", connId,
            route->ServerAddr.toStringLong().c_str());
  ...
  void onDisconnection(TSockId from) {
    ...
    nldebug("CGatewayL3ClientTransport : Disconnection from %s",
            it->second->ServerAddr.toStringLong().c_str());
  ...
  static void cbDisconnection(TSockId from, void *arg) {
    nlassert(arg != NULL);
    CGatewayL3ClientTransport *transport =
...
CGatewayL3ClientTransport::TDispatcherIndex
    CGatewayL3ClientTransport::_DispatcherIndex;

// register this class in the transport factory
NLMISC_REGISTER_OBJECT(IGatewayTransport, CGatewayL3ClientTransport,
                       std::string, string(LAYER3_CLIENT_CLASS_NAME));

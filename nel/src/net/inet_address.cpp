...
void CInetAddress::parseNameAndPort(std::string &hostname, uint16 &port,
  ...
  if (pos != string::npos &&
      ((pos6end == string::npos && pos == hostnameAndPort.find(':')) ||
       pos > pos6end) &&
 ...
 * Sets port
 */
void CInetAddress::setPort(uint16 port) { m_Port = port; }
...
void CInetAddress::fromSockAddrStorage(const TSockAddrStorage *saddr) {
  if (saddr->ss_family == AF_INET) {
    ...
    if (saddr->ss_family != AF_UNSPEC) {
      nlwarning(
          "CInetAddress::fromSockAddrStorage(): unknown address family %d",
          saddr->ss_family);
    }
    m_Address.setNull();
    m_Port = 0;
 ...
 */
string CInetAddress::ipAddress() const { return m_Address.toString(); }

/*
 * Returns host name. (ex: "www.nevrax.org")
 */
string CInetAddress::hostName() const { return ipAddress(); }

/*
 * Returns port
 */
uint16 CInetAddress::port() const { return m_Port; }

/*
 * Returns hostname and port as a string. (ex: "www.nevrax.org:80
 * (195.68.21.195)")
 */
std::string CInetAddress::asString() const { return asIPString(); }

/*
 * Returns IP address and port as a string. (ex: "195.68.21.195:80")
 */
std::string CInetAddress::asIPString() const {
  return ((m_Address.isValid() && !m_Address.isIPv4()) ? "[" + ipAddress() + "]"
                                                       : ipAddress()) +
         ":" + NLMISC::toString(port());
}

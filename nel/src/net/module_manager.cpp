...
NLMISC_CATEGORISED_COMMAND(net, initModuleManager,
                           "force the initialisation of the module manager",
                           "") {
...
public:
  static void releaseInstance() {
    if (_Instance) {
      NLMISC::INelContext::getInstance().releaseSingletonPointer(
          "CModuleManager", _Instance);
      delete _Instance;
      _Instance = NULL;
    }
  }
  ...
  struct TModuleLibraryInfo : public CRefCount {
    ...
    /// The short name of the library, i.e not path, no decoration (.dll or  .so
    /// and NeL compilation mode tag)
  ...
  CModuleManager() : IModuleManager(), ICommandsHandler(), _LastGeneratedId(0) {
  ...
  virtual void applicationExit() {
    TModuleInstances::TAToBMap::const_iterator first(
        _ModuleInstances.getAToBMap().begin()),
        last(_ModuleInstances.getAToBMap().end());
  ...
  void
  addModuleFactoryRegistry(TLocalModuleFactoryRegistry &moduleFactoryRegistry) {
    ...
    for (; first != last; ++first) {
      ...
      if (_ModuleFactoryRegistry.find(*first) != _ModuleFactoryRegistry.end()) {
        ...
        nlinfo("CModuleManger : add module factory : module class '%s' is "
               "already registered; ignoring new factory @%p",
               className.c_str(), factory);
      } else {
        // store the factory
  ...
  virtual bool loadModuleLibrary(const std::string &libraryName) {
    ...
    string fullName =
        NLMISC::CPath::standardizePath(path) + CLibrary::makeLibName(shortName);
    ...
    if (!mli->LibraryHandler.loadLibrary(fullName, false, true, true)) {
      nlwarning("CModuleManager : failed to load the library '%s' in '%s'",
                shortName.c_str(), fullName.c_str());
      return false;
    }
    ...
    CNelModuleLibrary *modLib = dynamic_cast<CNelModuleLibrary *>(
        mli->LibraryHandler.getNelLibraryInterface());
    if (modLib == NULL) {
      nlwarning(
          "CModuleManager : the library '%s' is not a pure Nel Module library",
          shortName.c_str());
      return false;
    }
    ...
    pair<TModuleLibraryInfos::iterator, bool> ret =
        _ModuleLibraryRegistry.insert(make_pair(shortName, mli.release()));
    if (!ret.second) {
      nlwarning(
          "CModuleManager : failed to store module library information !");
      return false;
    }
  ...
  virtual bool unloadModuleLibrary(const std::string &libraryName) {
    ...
    if (it == _ModuleLibraryRegistry.end()) {
      nlwarning("CModuleManager : failed to unload library '%s' : unknown or "
                "not loaded library",
                shorName.c_str());
      return false;
    }

    // by erasing this entry, we delete the CLibrary object that hold the
    // library module, thus destroying any factory the belong into and deleting
    // the instantiated module ! Wow, that's pretty much for a single line ;)
   ...
   */
  //		virtual void registerModuleFactory(class IModuleFactory
  //*moduleFactory)
  //		{
  //			nlstop;
  //		}
  ...
  virtual void unregisterModuleFactory(class IModuleFactory *moduleFactory) {
    ...
    TModuleFactoryRegistry::iterator it(
        _ModuleFactoryRegistry.find(moduleFactory->getModuleClassName()));

    if (it == _ModuleFactoryRegistry.end()) {
      nlwarning("The module factory for class '%s' in not registered.",
                moduleFactory->getModuleClassName().c_str());
      return;
    } else if (it->second != moduleFactory) {
      nlinfo("The module factory @%p for class '%s' is not the registered one, "
             "ignoring.",
             moduleFactory, moduleFactory->getModuleClassName().c_str());
      return;
    }

    nlinfo("ModuleManager : unregistering factory for module class '%s'",
           moduleFactory->getModuleClassName().c_str());
   ...
   */
  virtual void
  getAvailableModuleClassList(std::vector<std::string> &moduleClassList) {
    TModuleFactoryRegistry::iterator first(_ModuleFactoryRegistry.begin()),
        last(_ModuleFactoryRegistry.end());
    for (; first != last; ++first) {
   ...
   */
  virtual IModule *createModule(const std::string &className,
    ...
    if (moduleName.empty()) {
      ...
      if (_ModuleInstances.getB(moduleName) != NULL) {
        nlwarning("createModule : the name '%s' is already used by another "
                  "module, can't instantiate the module",
                  moduleName.c_str());
        return NULL;
      }
    }
    ...
    if (module.get() == NULL) {
      nlwarning("createModule : factory failed to create a module instance for "
                "class '%s'",
                className.c_str());
    ...
    if (modBase == NULL) {
      nlwarning("Invalid module returned by factory for class '%s'",
                className.c_str());
      return NULL;
    }
    ...
    if (initResult) {
      ...
      nlwarning("Create module : the new module '%s' of class '%s' has failed "
                "to initilize properly.",
  ...
  virtual IModule *getLocalModule(const std::string &moduleName) {
    TModuleInstances::TAToBMap::const_iterator it(
        _ModuleInstances.getAToBMap().find(moduleName));
  ...
  virtual void updateModules() {
    ...
    TModuleIds::TAToBMap::const_iterator first(_ModuleIds.getAToBMap().begin()),
        last(_ModuleIds.getAToBMap().end());
    for (; first != last; ++first) {
      ...
      if (modBase != NULL) {
        ...
        while (!modBase->_ModuleTasks.empty()) {
          ...
          if (task->isFinished()) {
            LNETL6_DEBUG("NLNETL6: updateModule : task %p is finished, delete "
                         "and remove from task list",
                         task);
  ...
  virtual void registerModuleSocket(IModuleSocket *moduleSocket) {
    ...
    TModuleSockets::iterator it(
        _ModuleSocketsRegistry.find(moduleSocket->getSocketName()));
    nlassert(it == _ModuleSocketsRegistry.end());

    nldebug("Registering module socket '%s'",
            moduleSocket->getSocketName().c_str());
    _ModuleSocketsRegistry.insert(
        make_pair(moduleSocket->getSocketName(), moduleSocket));
  }
  ...
  virtual void unregisterModuleSocket(IModuleSocket *moduleSocket) {
    ...
    nldebug("Unregistering module socket '%s'",
            moduleSocket->getSocketName().c_str());
  ...
  virtual void registerModuleGateway(IModuleGateway *moduleGateway) {
    ...
    TModuleGateways::iterator it(
        _ModuleGatewaysRegistry.find(moduleGateway->getGatewayName()));
    nlassert(it == _ModuleGatewaysRegistry.end());

    nldebug("Registering module gateway '%s'",
            moduleGateway->getGatewayName().c_str());
    _ModuleGatewaysRegistry.insert(
        make_pair(moduleGateway->getGatewayName(), moduleGateway));
  }
  /** Unregister a socket in the manager.
   */
  virtual void unregisterModuleGateway(IModuleGateway *moduleGateway) {
    ...
    nldebug("Unregistering module gateway '%s'",
            moduleGateway->getGatewayName().c_str());
  ...
  virtual IModuleProxy *createModuleProxy(
      IModuleGateway *gateway, CGatewayRoute *route, uint32 distance,
      IModule *localModule, const std::string &moduleClassName,
      const std::string &moduleFullyQualifiedName,
      const std::string &moduleManifest, TModuleId foreignModuleId) {
    CUniquePtr<CModuleProxy> modProx(
        new CModuleProxy(localModule, ++_LastGeneratedId, moduleClassName,
                         moduleFullyQualifiedName, moduleManifest));
    modProx->_Gateway = gateway;
    modProx->_Route = route;
    modProx->_Distance = distance;
    modProx->_ForeignModuleId = foreignModuleId;

    nldebug("Creating module proxy (ID : %u, foreign ID : %u, name : '%s', "
            "class : '%s' at %u hop",
            modProx->getModuleProxyId(), modProx->getForeignModuleId(),
            modProx->getModuleName().c_str(),
            modProx->getModuleClassName().c_str(), modProx->_Distance);

    //			_ModuleProxyInstances.add(moduleFullyQualifiedName,
    //TModuleProxyPtr(modProx.get()));
    _ModuleProxyIds.add(modProx->getModuleProxyId(),
                        TModuleProxyPtr(modProx.get()));
  ...
  virtual void releaseModuleProxy(TModuleId moduleProxyId) {
    TModuleProxyIds::TAToBMap::const_iterator it(
        _ModuleProxyIds.getAToBMap().find(moduleProxyId));
    ...
    nldebug("Releasing module proxy ('%s', ID : %u)",
            it->second->getModuleName().c_str(), moduleProxyId);
    ...
    nlassertex(
        sanityCheck == NULL,
        ("Someone has kept a smart pointer on the proxy '%s' of class '%s'",
         sanityCheck->getModuleName().c_str(),
         sanityCheck->getModuleClassName().c_str()));
  }
  ...
  NLMISC_COMMAND_HANDLER_ADD(
      CModuleManager, dump,
      "dump various information about module manager state", "");
  NLMISC_COMMAND_HANDLER_ADD(CModuleManager, loadLibrary,
                             "load a pure nel library module (give the path if "
                             "needed and only the undecorated lib name)",
                             "[path]<undecoratedLibName>");
  NLMISC_COMMAND_HANDLER_ADD(CModuleManager, unloadLibrary,
                             "unload a pure nel library module (give the "
                             "undecorated name, any path will be removed",
                             "<undecoratedLibName>");
  NLMISC_COMMAND_HANDLER_ADD(CModuleManager, createModule,
                             "create a new module instance",
                             "<moduleClass> <instanceName> [*<moduleArg>]");
  NLMISC_COMMAND_HANDLER_ADD(CModuleManager, deleteModule,
                             "delete a module instance", "<instanceName>");
  ...
  NLMISC_CLASS_COMMAND_DECL(createModule) {
    ...
    nlinfo("Creating module '%s' of class '%s' with params '%s'",
           moduleName.c_str(), moduleClass.c_str(), moduleArgs.c_str());
  ...
  NLMISC_CLASS_COMMAND_DECL(dump) {
    ...
    log.displayNL("Dumping CModuleManager internal states :");
    {
      ...
      for (uint i = 0; i < moduleList.size(); ++i) {
        if (_ModuleFactoryRegistry.find(moduleList[i]) ==
            _ModuleFactoryRegistry.end()) {
          log.displayNL("    %s : UNAVAILABLE !", moduleList[i].c_str());
        } else {
          IModuleFactory *modFact =
              TLocalModuleFactoryRegistry::instance().getFactory(moduleList[i]);
          log.displayNL("    %s : OK\tInit params : '%s'",
                        moduleList[i].c_str(),
                        modFact->getInitStringHelp().c_str());
        }
      }
    }
    {
      log.displayNL(" List of %u loaded module libraries :",
                    _ModuleLibraryRegistry.size());
      TModuleLibraryInfos::iterator first(_ModuleLibraryRegistry.begin()),
          last(_ModuleLibraryRegistry.end());
      for (; first != last; ++first) {
        TModuleLibraryInfo &mli = *(first->second);
        log.displayNL(
            "  Library '%s' loaded from '%s', contains %u modules classes:",
            first->first.c_str(), mli.FullPathLibraryName.c_str(),
            mli.ModuleFactoryList.size());
        {
          vector<string>::iterator f2(mli.ModuleFactoryList.begin()),
              l2(mli.ModuleFactoryList.end());
          for (uint i = 0; i < mli.ModuleFactoryList.size(); ++i) {
            if (_ModuleFactoryRegistry.find(mli.ModuleFactoryList[i]) ==
                _ModuleFactoryRegistry.end()) {
              log.displayNL(
                  "    %s : UNAVAILABLE, this class is already registered !",
                  mli.ModuleFactoryList[i].c_str());
            } else {
    ...
    {
      log.displayNL(" List of %u module instances :",
                    _ModuleInstances.getAToBMap().size());
      TModuleInstances::TAToBMap::const_iterator first(
          _ModuleInstances.getAToBMap().begin()),
          last(_ModuleInstances.getAToBMap().end());
      for (; first != last; ++first) {
    ...
    {
      log.displayNL(" List of %u module proxies :",
                    _ModuleProxyIds.getAToBMap().size());
      TModuleProxyIds::TAToBMap::const_iterator first(
          _ModuleProxyIds.getAToBMap().begin()),
          last(_ModuleProxyIds.getAToBMap().end());
      for (; first != last; ++first) {
        IModuleProxy *modProx = first->second;
        if (modProx->getGatewayRoute() != NULL) {
          log.displayNL(
              "    ID:%5u (Foreign ID : %u) : \tname = '%s' \tclass = '%s'",
              modProx->getModuleProxyId(), modProx->getForeignModuleId(),
              modProx->getModuleName().c_str(),
              modProx->getModuleClassName().c_str());
        } else {
          log.displayNL("    ID:%5u (Local proxy for module ID : %u) : \tname "
                        "= '%s' \tclass = '%s'",
...
bool IModuleManager::isInitialized() { return CModuleManager::isInitialized(); }

IModuleManager &IModuleManager::getInstance() {
  return CModuleManager::getInstance();
}

void IModuleManager::releaseInstance() { CModuleManager::releaseInstance(); }

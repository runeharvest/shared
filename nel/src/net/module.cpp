...
//////////////////////////////////////
IModuleInterceptable::IModuleInterceptable() : _Registrar(NULL) {}
...
void IModuleInterceptable::registerInterceptor(
    IInterceptorRegistrar *registrar) {
...
void IModuleInterceptable::interceptorUnregistered(
    IInterceptorRegistrar *registrar) {
...
IModuleFactory::~IModuleFactory() {
  ...
  while (!_ModuleInstances.empty()) {
    ...
    nlassertex(sanityCheck == NULL,
               ("Some code have kept pointer on module '%s'",
...
//////////////////////////////////////
CModuleTask::CModuleTask(class CModuleBase * /*module */) : _FailInvoke(false) {
...
CModuleBase::CModuleBase()
    : _CurrentSender(NULL), _CurrentMessage(NULL), _CurrentMessageFailed(false),
      _MessageDispatchTask(NULL), _ModuleFactory(NULL),
...
void CModuleBase::registerInterceptor(IModuleInterceptable *interceptor) {
  ...
  nlassert(find(_ModuleInterceptors.begin(), _ModuleInterceptors.end(),
                interceptor) == _ModuleInterceptors.end());
...
void CModuleBase::unregisterInterceptor(IModuleInterceptable *interceptor) {
  TInterceptors::iterator it =
      find(_ModuleInterceptors.begin(), _ModuleInterceptors.end(), interceptor);
...
TModuleId CModuleBase::getModuleId() const { return _ModuleId; }

const std::string &CModuleBase::getModuleName() const { return _ModuleName; }
...
const std::string &CModuleBase::getModuleFullyQualifiedName() const {
  if (_FullyQualifedModuleName.empty()) {
    nlassertex(
        !_ModuleName.empty(),
        ("Call to CModuleBase::getModuleFullyQualifiedName before module name "
         "have been set (did you call from module constructor ?)"));
    // build the name
    string hostName;
...
std::string CModuleBase::getModuleManifest() const {
  ...
  TInterceptors::const_iterator first(_ModuleInterceptors.begin()),
      last(_ModuleInterceptors.end());
...
void CModuleBase::onReceiveModuleMessage(IModuleProxy *senderModuleProxy,
...
void CModuleBase::_receiveModuleMessageTask() {
  ...
  while (!_MessageDispatchTask->isTerminationRequested()) {
    ...
    try {
      ...
      nlwarning("In module task '%s' (cotask message receiver), exception '%e' "
                "thrown",
                typeid(this).name(), e.what());
      // an exception have been thrown
      _CurrentMessageFailed = true;
    } catch (...) {
      nlwarning("In module task '%s' (cotask message receiver), unknown "
                "exception thrown",
                typeid(this).name());
      // an exception have been thrown
      _CurrentMessageFailed = true;
...
bool CModuleBase::initModule(const TParsedCommandLine &initInfo) {
  ...
  if (initInfo.getParam("base.useCoTaskDispatch")) {
    ...
    //			NLNET_START_MODULE_TASK(CModuleBase,
    //_receiveModuleMessageTask); 	TModuleTask<className> *task = new
    //TModuleTask<className>(this, &className::methodName);
    _MessageDispatchTask = new TModuleTask<CModuleBase>(
        this, TModuleTask<CModuleBase>::TMethodPtr(
                  &CModuleBase::_receiveModuleMessageTask));
  }
...
void CModuleBase::getPluggedSocketList(
    std::vector<IModuleSocket *> &resultList) {
  TModuleSockets::iterator first(_ModuleSockets.begin()),
      last(_ModuleSockets.end());
  for (; first != last; ++first) {
 ...
 */
void CModuleBase::invokeModuleOperation(IModuleProxy *destModule,
  ...
  for (;;) {
    ...
    while (!_SyncMessages.empty()) {
      ...
      if (msg.getType() == CMessage::Response) {
        ...
        //					{
        //						nlwarning("Some
        //exception where throw will dispatching message '%s' from '%s' to
        //'%s'", 							msg.getName().c_str(), 							proxy->getModuleName().c_str(),
        ...
        //						{
        //							// send back an
        //exception message 							CMessage except; 							except.setType("EXCEPT",
        //CMessage::Except); 							proxy->sendModuleMessage(this, except);
        //						}
        //
        //					}
...
void CModuleBase::_onModuleUp(IModuleProxy *removedProxy) {
  ...
  TInterceptors::iterator first(_ModuleInterceptors.begin()),
      last(_ModuleInterceptors.end());
  for (; first != last; ++first) {
    IModuleInterceptable *interceptor = *first;
...
void CModuleBase::_onModuleDown(IModuleProxy *removedProxy) {
  ...
  {
    TMessageList::iterator first(_SyncMessages.begin()),
        last(_SyncMessages.end());
    for (; first != last; ++first) {
  ...
  {
    TInvokeStack::iterator first(_InvokeStack.begin()),
        last(_InvokeStack.end());
    for (; first != last; ++first) {
      if (*first == removedProxy) {
        // at least, we need either a running task or the default dispatch task
        // activated
        nlassert(!_ModuleTasks.empty() || _MessageDispatchTask != NULL);

        // gasp, we lost one of the module needed to managed the invocation
        // stack! make each call generate an exception
        while (first != _InvokeStack.end()) {
          ...
          CModuleTask *task = !_ModuleTasks.empty() ? _ModuleTasks.front()
                                                    : _MessageDispatchTask;
          task->failInvoke();
...
bool CModuleBase::_onProcessModuleMessage(IModuleProxy *senderModuleProxy,
                                          const CMessage &message) {
  ...
  try {
    ...
    nlwarning("Some exception where throw will dispatching message '%s' from "
              "'%s' to '%s'",
...
IModuleFactory *CModuleBase::getFactory() { return _ModuleFactory; }
...
NLMISC_CLASS_COMMAND_IMPL(CModuleBase, plug) {
  ...
  IModuleSocket *socket =
      IModuleManager::getInstance().getModuleSocket(args[0]);
  ...
  if (_ModuleSockets.find(socket) == _ModuleSockets.end()) {
    log.displayNL("Failed to plug the module '%s' into the socket '%s'",
                  getModuleName().c_str(), socket->getSocketName().c_str());
  } else
...
NLMISC_CLASS_COMMAND_IMPL(CModuleBase, sendPing) {
  ...
  log.displayNL("Can't find a route to send message to module '%s'",
                modName.c_str());
...
NLMISC_CLASS_COMMAND_IMPL(CModuleBase, dump) {
  ...
  log.displayNL("  Module full name : '%s'",
                getModuleFullyQualifiedName().c_str());
  log.displayNL("  Module class     : '%s'",
                _ModuleFactory->getModuleClassName().c_str());
  log.displayNL("  Module ID        : %u", _ModuleId);
  log.displayNL("  The module is plugged into %u sockets :",
                _ModuleSockets.size());
  {
    TModuleSockets::iterator first(_ModuleSockets.begin()),
        last(_ModuleSockets.end());
    for (; first != last; ++first) {
      ...
      log.displayNL("    Socket '%s', %u modules reachable :",
                    ps->getSocketName().c_str(), proxies.size() - 1);
      ...
      for (uint i = 0; i < proxies.size(); ++i) {
        ...
        if (name != getModuleFullyQualifiedName()) {
          log.displayNL(
              "      Module '%s' (Module Proxy ID : %u, class : '%s')",
...
CModuleProxy::CModuleProxy(TModulePtr localModule, TModuleId localModuleId,
                           const std::string &moduleClassName,
                           const std::string &fullyQualifiedModuleName,
                           const std::string &moduleManifest)
    : _ModuleProxyId(localModuleId), _ForeignModuleId(INVALID_MODULE_ID),
      ...
      _Manifest(moduleManifest), _SecurityData(NULL) {}

TModuleId CModuleProxy::getModuleProxyId() const { return _ModuleProxyId; }

TModuleId CModuleProxy::getForeignModuleId() const { return _ForeignModuleId; }

uint32 CModuleProxy::getModuleDistance() const { return _Distance; }

IModule *CModuleProxy::getLocalModule() const { return _LocalModule; }

CGatewayRoute *CModuleProxy::getGatewayRoute() const { return _Route; }
...
const std::string &CModuleProxy::getModuleManifest() const { return _Manifest; }

IModuleGateway *CModuleProxy::getModuleGateway() const { return _Gateway; }

void CModuleProxy::sendModuleMessage(IModule *senderModule,

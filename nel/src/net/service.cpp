...
static const sint Signal[] = {SIGABRT, SIGFPE,  SIGILL,
                              SIGINT,  SIGSEGV, SIGTERM};

static const char *SignalName[] = {"SIGABRT", "SIGFPE",  "SIGILL",
                                   "SIGINT",  "SIGSEGV", "SIGTERM"};

static const char *NegFiltersNames[] = {"NegFiltersDebug",   "NegFiltersInfo",
                                        "NegFiltersWarning", "NegFiltersAssert",
                                        "NegFiltersError",   0};
...
// services stat
CVariable<sint32> UserSpeedLoop("nel", "UserSpeedLoop",
                                "duration of the last user loop (in ms)", 10,
                                false);
CVariable<sint32> NetSpeedLoop("nel", "NetSpeedLoop",
                               "duration of the last network loop (in ms)", 10,
                               false);
/// The time passed in callback during the loop
CVariable<uint32> L5CallbackTime(
    "nel", "L5CallbackTime",
    "Time passed in the L5 callback function in the last loop (in ms)", 0, 100);
/// The number of L5 callback treated
CVariable<uint32>
    L5CallbackCount("nel", "L5CallbackCount",
                    "The number of layer 5 callback received in the last loop",
                    0, 100);
...
static void UpdateAssertionThreadTimeoutCB(IVariable &var) {
  uint32 timeOut;
  fromString(var.toString(), timeOut);
  MyTAT.timeout(timeOut);
}
static CVariable<uint32> UpdateAssertionThreadTimeout(
    "nel", "UpdateAssertionThreadTimeout",
    "in millisecond, timeout before thread assertion", 0, 0, true,
    UpdateAssertionThreadTimeoutCB);

// Flag to enable/disable the flushing of the sending queues when the service is
// shut down Default: false (matches the former behavior) Set it to true in
// services that need to send data on exit (for instance in their release()
// method)
CVariable<bool>
    FlushSendingQueuesOnExit("nel", "FlushSendingQueuesOnExit",
                             "Flag to enable/disable the flushing of the "
                             "sending queues when the service is shut down",
                             false, 0, true);

// If FlushSendingQueuesOnExit is on, only the sending queues to these specified
// services will be flushed Format: service short names separated by ':'
// Default: "" (all will be flushed if FlushSendingQueuesOnExit is on, none if
// it is off)
CVariable<string> NamesOfOnlyServiceToFlushSending(
    "nel", "NamesOfOnlyServiceToFlushSending",
    "If FlushSendingQueuesOnExit is on, only the sending queues to these "
    "specified services will be flushed (ex: \"WS:LS\"; all will be flushed if "
    "empty string)",
    "", 0, true);

bool wasExitSignalAsked() {
  if (IService::isServiceInitialized() &&
      IService::getInstance()->WindowDisplayer != NULL) {
  ...
  for (int i = 0; i < (int)(sizeof(Signal) / sizeof(Signal[0])); i++) {
    if (Sig == Signal[i]) {
      if (getThreadId() != SignalisedThread) {
        nldebug("SERVICE: Not the main thread (%u, %u) received the signal "
                "(%s, %d), ignore it",
                getThreadId(), SignalisedThread, SignalName[i], Sig);
        return;
      } else {
        ...
        case SIGSEGV:
        case SIGTERM:
          // you should not call a function and system function like printf in a
          // SigHandle because signal-handler routines are usually called
          // asynchronously when an interrupt occurs.
...
void cbDirectoryChanged(IVariable &var) {
  ...
  // (warning: ConvertSavesFilesDirectoryToFullPath, read from the config file,
  // won't be ready for the initial variable assigments done before the config
  // file has been loaded)
  string vp = var.toString();
  if ((var.getName() != "SaveFilesDirectory") ||
      instance->ConvertSavesFilesDirectoryToFullPath.get()) {
...
void cbReceiveShardId(CMessage &msgin, const string &serviceName,
  ...
  if (serviceName != "WS") {
    nlwarning("SERVICE: received unauthorized R_SH_ID callback from service "
              "%s-%uh asking to set ShardId to %d",
              serviceName.c_str(), serviceId.get(), shardId);
    return;
  }
...
void IService::anticipateShardId(uint32 shardId) {
  if (!((_ShardId == DEFAULT_SHARD_ID) || (shardId == _ShardId)))
    nlerror("IService::anticipateShardId() overwrites %u with %u", _ShardId,
            shardId);
  _ShardId = shardId;
}
...
void IService::setShardId(uint32 shardId) {
  if (!((_ShardId == DEFAULT_SHARD_ID) || (shardId == _ShardId)))
    nlwarning("SERVICE: The shardId from the WS (%u) is different from the "
              "anticipated shardId (%u)",
              shardId, _ShardId);
  _ShardId = shardId;
}
...
IService::IService()
    : WindowDisplayer(0),
      WriteFilesDirectory("nel", "WriteFilesDirectory",
                          "directory where to save generic shard information "
                          "(packed_sheets for example)",
                          ".", 0, true, cbDirectoryChanged),
      SaveFilesDirectory("nel", "SaveFilesDirectory",
                         "directory where to save specific shard information "
                         "(shard time for example)",
                         ".", 0, true, cbDirectoryChanged),
      ConvertSavesFilesDirectoryToFullPath(
          "nel", "ConvertSaveFilesDirectoryToFullPath",
          "If true (default), the provided SaveFilesDirectory will be "
          "converted to a full path (ex: saves -> /home/dir/saves)",
          true, 0, true),
      ListeningPort("nel", "ListeningPort", "listening port for this service",
                    0, 0, true),
      _RecordingState(CCallbackNetBase::Off), _UpdateTimeout(100), _SId(0),
      _ExitStatus(0), _Initialized(false),
      ConfigDirectory("nel", "ConfigDirectory",
                      "directory where config files are", ".", 0, true,
                      cbDirectoryChanged),
      LogDirectory("nel", "LogDirectory",
                   "directory where the service is logging", ".", 0, true,
                   cbDirectoryChanged),
      RunningDirectory("nel", "RunningDirectory",
                       "directory where the service is running on", ".", 0,
                       true, cbDirectoryChanged),
      Version("nel", "Version", "Version of the shard", ""), _CallbackArray(0),
      _CallbackArraySize(0), _DontUseNS(false), _DontUseAES(false),
      _ResetMeasures(false), _ShardId(0),
      _ClosureClearanceStatus(CCMustRequestClearance),
      _RequestClosureClearanceCallback(NULL), _DirectoryChangedCBI(NULL) {
  // Singleton
  _Instance = this;
    ...
    if (_Args[i].size() >= 2 && _Args[i][0] == '-') {
      if (_Args[i][1] == argName) {
        ...
        */
        ...
        if (_Args[i].size() < 3)
          return "";
        // throw Exception ("Parameter '-%c' is malformed, missing content",
        // argName);
...
bool IService::haveLongArg(const char *argName) const {
  for (uint32 i = 0; i < _Args.size(); i++) {
    if (_Args[i].left(2) == "--" &&
        _Args[i].leftCrop(2).splitTo('=') == argName) {
      return true;
    }
  }
  return false;
...
void IService::setArgs(const char *args) {
  ...
  do {
    ...
    string::difference_type length =
        (pos2 != string::npos) ? pos2 - pos1 : string::npos;
...
void cbLogFilter(CConfigFile::CVar &var) {
  ...
  CConfigFile::CVar &oldvar =
      IService::getInstance()->ConfigFile.getVar(var.Name);
...
sint IService::main(const char *serviceShortName, const char *serviceLongName,
                    uint16 servicePort, const char *configDir,
                    const char *logDir, const char *compilationDate) {
  ...
  try {
    ...
    if (!CFile::fileExists(ConfigDirectory.c_str() + _LongName + ".cfg")) {
      ...
      if (!CFile::fileExists(ConfigDirectory.c_str() + _LongName +
                             "_default.cfg")) {
        nlerror("SERVICE: Neither the config file '%s' nor the default one can "
                "be found, can't launch the service",
                cfn.c_str());
      } else {
    ...
    if (ConfigFile.exists("DefaultEmailSMTP") &&
        ConfigFile.exists("DefaultEmailTo"))
    ...
    // we have to call this again because the config file can changed this
    // variable but the cmd line is more prioritary
    if (haveArg('A'))
      RunningDirectory = CPath::standardizePath(getArg('A'));

    //
    // Init debug/log stuffs (must be first things otherwise we can't log if
    // errors)
    //
    ...
    bool noLog = (ConfigFile.exists("DontLog")) &&
                 (ConfigFile.getVar("DontLog").asInt() == 1);
    noLog |= haveLongArg("nolog");
    if (!noLog) {
      // we create the log with service name filename ("test_service_ALIAS.log"
      // for example)
    ...
    bool dontUseStdIn = (ConfigFile.exists("DontUseStdIn")) &&
                        (ConfigFile.getVar("DontUseStdIn").asInt() == 1);
    if (!dontUseStdIn) {
    ...
    if ((var = ConfigFile.getVarPtr("WindowStyle")) != NULL) {
      ...
      if (WindowDisplayer == NULL && disp != "NONE") {
        nlinfo("SERVICE: Unknown value for the WindowStyle (should be GTK, WIN "
               "or NONE), use no window displayer");
      }
    }

    vector<pair<string, uint>> displayedVariables;
    // uint speedNetLabel, speedUsrLabel, rcvLabel, sndLabel, rcvQLabel,
    // sndQLabel, scrollLabel;
    if (WindowDisplayer != NULL) {
      ...
      if ((var = ConfigFile.getVarPtr("XWinParam")) != NULL)
        x = var->asInt();
      if ((var = ConfigFile.getVarPtr("YWinParam")) != NULL)
        y = var->asInt();
      if ((var = ConfigFile.getVarPtr("WWinParam")) != NULL)
        w = var->asInt();
      if ((var = ConfigFile.getVarPtr("HWinParam")) != NULL)
        h = var->asInt();
      if ((var = ConfigFile.getVarPtr("HistoryWinParam")) != NULL)
        history = var->asInt();
      if ((var = ConfigFile.getVarPtr("IWinParam")) != NULL)
        iconified = var->asInt() == 1;
      if ((var = ConfigFile.getVarPtr("FontSize")) != NULL)
        fs = var->asInt();
      if ((var = ConfigFile.getVarPtr("FontName")) != NULL)
        fn = var->asString();
      if ((var = ConfigFile.getVarPtr("WordWrap")) != NULL)
        ww = var->asInt() == 1;

      if (haveArg('I'))
        iconified = true;

      WindowDisplayer->create(string("*INIT* ") + _ShortName + " " + _LongName,
                              iconified, x, y, w, h, history, fs, fn, ww,
                              &CommandLog);
      ...
      displayedVariables.push_back(
          make_pair(string("NetLop|NetSpeedLoop"),
                    WindowDisplayer->createLabel("NetLop")));
      displayedVariables.push_back(
          make_pair(string("UsrLop|UserSpeedLoop"),
                    WindowDisplayer->createLabel("UsrLop")));
      displayedVariables.push_back(make_pair(
          string("|Scroller"), WindowDisplayer->createLabel("NeL Rulez")));

      CConfigFile::CVar *v = ConfigFile.getVarPtr("DisplayedVariables");
      if (v != NULL) {
        for (uint i = 0; i < v->size(); i++) {
          displayedVariables.push_back(
              make_pair(v->asString(i),
                        WindowDisplayer->createLabel(v->asString(i).c_str())));
        }
      }
    }

    nlinfo("SERVICE: Starting Service '%s' using NeL (" __DATE__ " " __TIME__
           ") compiled %s",
           _ShortName.c_str(), CompilationDate.c_str());
    ...
    if ((var = ConfigFile.getVarPtr("SId")) != NULL) {
      ...
      if (sid <= 0 || sid > 255) {
        nlwarning("SERVICE: Bad SId value in the config file, %d is not in "
                  "[0;255] range",
                  sid);
        _SId.set(0);
      } else {
    ...
    //
    // Register all network associations (must be before the
    // CUnifiedNetwork::getInstance()->init)
    //

    if ((var = ConfigFile.getVarPtr("Networks")) != NULL) {
      for (uint8 i = 0; i < var->size(); i++)
        CUnifiedNetwork::getInstance()->addNetworkAssociation(var->asString(i),
                                                              i);
    }
    ...
    if (!_DontUseNS) {
      ...
      while (!ok && !wasExitSignalAsked()) {
        ...
        if (haveArg('B')) {
          // if the naming service address is set on the command line, get it
          // (overwrite the cfg)
          LSAddr = getArg('B');
        } else {
        ...
        try {
          ...
          if (CUnifiedNetwork::getInstance()->init(
                  &loc, _RecordingState, _ShortName, ListeningPort, _SId)) {
            ok = true;
          } else {
            ...
            IStdinMonitorSingleton::getInstance()
                ->release(); // does nothing if not initialized
          ...
          nlinfo("SERVICE: Could not connect to the Naming Service (%s). "
                 "Retrying in a few seconds...",
                 loc.toStringLong().c_str());
          nlSleep(5000);
        }
      }
    } else {
      CUnifiedNetwork::getInstance()->init(NULL, _RecordingState, _ShortName,
                                           ListeningPort, _SId);
    }
    ...
    // If it's 0, it means that we don't use NS and we left the other side
    // server to find a sid for your connection
    ...
    NLNET::CUnifiedNetwork::getInstance()->addCallbackArray(
        builtinServiceCallbacks,
        sizeof(builtinServiceCallbacks) / sizeof(builtinServiceCallbacks[0]));

    // add callback set in the NLNET_SERVICE_MAIN macro
    NLNET::CUnifiedNetwork::getInstance()->addCallbackArray(_CallbackArray,
                                                            _CallbackArraySize);
    ...
    if ((var = ConfigFile.getVarPtr("Paths")) != NULL) {
      for (uint i = 0; i < var->size(); i++) {
        CPath::addSearchPath(
            NLMISC::expandEnvironmentVariables(var->asString(i)), true, false);
      }
    }
    ...
    if ((var = ConfigFile.getVarPtr("PathsNoRecurse")) != NULL) {
      for (uint i = 0; i < var->size(); i++) {
        CPath::addSearchPath(
            NLMISC::expandEnvironmentVariables(var->asString(i)), false, false);
      }
    }

    // if we can, try to setup where to save files
    if (IService::getInstance()->haveArg('W')) {
      // use the command line param if set (must be done after the config file
      // has been loaded)
    ...
    //
    // Call the user service update each loop and check files and network
    // activity
    //
    ...
    for (;;) {
      ...
      // if the launching mode is 'quit after the first update' we set the exit
      // signal
      if (haveArg('Q')) {
        // we use -100 so that the final return code ends as 0
        // (100+ExitSignalAsked) because otherwise we can't launch services with
        // -Q in a makefile
        ExitSignalAsked = -100;
      }
      NbUserUpdate++;
      ...
      if (WindowDisplayer != NULL) {
        ...
        if (!WindowDisplayer->update()) {
          nlinfo(
              "SERVICE: The window displayer was closed by user, need to quit");
          ExitSignalAsked = 1;
        }
      }
      ...
      if (ExitSignalAsked != 0) {
        if (_RequestClosureClearanceCallback) {
          if (_ClosureClearanceStatus == CCClearedForClosure) {
            ...
            // Always direct closure, because we don't have a connection to the
            // naming service anymore But still call the callback
      ...
      set<IServiceUpdatable *>::iterator first(_Updatables.begin()),
          last(_Updatables.end());
      ...
      if (_RequestClosureClearanceCallback) {
        if (!CNamingClient::connected()) {
          if (_ClosureClearanceStatus < CCCallbackThenClose)
            _ClosureClearanceStatus +=
                CCCallbackThenClose; // change status but backup old value
        } else {
          if (_ClosureClearanceStatus >= CCCallbackThenClose)
            _ClosureClearanceStatus -=
                CCCallbackThenClose; // set the closure state back if the NS
                                     // comes back
        }
      }
      ...
      // nldebug ("SYNC: updatetimeout must be %d and is %d, sleep the rest of
      // the time", _UpdateTimeout, delta);

      CHTimer::endBench();

      // Resetting the hierarchical timer must be done outside the top-level
      // timer
      if (_ResetMeasures) {
        CHTimer::clear();
    ...
    // Somebody call nlerror, so we have to quit now, the message already
    // display so we don't have to to anything
    setExitStatus(EXIT_FAILURE);
  } catch (const ESocket &e) {
    // Catch NeL network exception to release the system cleanly
    // setExitStatus (EXIT_FAILURE);
    ErrorLog->displayNL("NeL Exception in \"%s\": %s", _ShortName.c_str(),
                        e.what());
  ...
  try {
    ...
    explode(NamesOfOnlyServiceToFlushSending.get(), string(":"),
            namesOfOnlyServiceToFlushSendingV, true);
  ...
  CHTimer::displayHierarchicalByExecutionPathSorted(
      &CommandLog, CHTimer::TotalTime, true, 64);
...
void IService::setCurrentStatus(const std::string &status) {
  ...
  _ServiceStatusStack.erase(std::remove(_ServiceStatusStack.begin(),
                                        _ServiceStatusStack.end(), status),
                            _ServiceStatusStack.end());

  // insert the status on top of the stack
  _ServiceStatusStack.push_back(status);
}

/// Remove a status from the status stack. If this status is at top of stack,
/// the next status become the current status
...
std::string IService::getFullStatus() const {
  ...
  set<string>::const_iterator first(_ServiveStatusTags.begin()),
      last(_ServiveStatusTags.end());
  for (; first != last; ++first) {
 ...
 */
void IService::requireResetMeasures() { _ResetMeasures = true; }
...
/*
 * Returns the date of launch of the service. Unit: see
 * CTime::getSecondsSince1970()
 */
uint32 IService::getLaunchingDate() const { return LaunchingDate; }
...
NLMISC_CATEGORISED_DYNVARIABLE(nel, string, LaunchingDate,
                               "date of the launching of the program") {
  nlunreferenced(human);
  if (get)
    *pointer = asctime(localtime((time_t *)&LaunchingDate));
}

NLMISC_CATEGORISED_DYNVARIABLE(nel, string, Uptime,
...
NLMISC_CATEGORISED_VARIABLE(nel, string, CompilationDate,
                            "date of the compilation");
NLMISC_CATEGORISED_VARIABLE(nel, string, CompilationMode,
                            "mode of the compilation");

NLMISC_CATEGORISED_VARIABLE(
    nel, uint32, NbUserUpdate,
    "number of time the user IService::update() called");

NLMISC_CATEGORISED_DYNVARIABLE(nel, string, Scroller,
  ...
  if (get) {
    ...
    static string foo =
        "Welcome to NeL Service! This scroll is used to see the update "
        "frequency of the main function and to see if the service is frozen or "
        "not. Have a nice day and hope you'll like NeL!!! "
        "Welcome to NeL Service! This scroll is used to see the update "
        "frequency of the main function and to see if the service is frozen or "
        "not. Have a nice day and hope you'll like NeL!!! ";
    static int pos = 0;
...
NLMISC_CATEGORISED_COMMAND(nel, mutex, "display mutex values", "") {
  ...
  for (im = acquiretimes.begin(); im != acquiretimes.end(); ++im) {
    log.displayNL("%d %p %s: %.0f %.0f, called %u times th(%d, %d wait)%s",
                  (*im).second.MutexNum, (*im).first,
                  (*im).second.MutexName.c_str(),
                  CTime::cpuCycleToSecond((*im).second.TimeToEnter) * 1000.0,
                  CTime::cpuCycleToSecond((*im).second.TimeInMutex) * 1000.0,
                  (*im).second.Nb, (*im).second.ThreadHavingTheMutex,
                  (*im).second.WaitingMutex, (*im).second.Dead ? " DEAD" : "");
  }
...
NLMISC_CATEGORISED_COMMAND(nel, serviceInfo,
  ...
  log.displayNL("Service %s '%s' using NeL (" __DATE__ " " __TIME__ ")",
                IService::getInstance()->getServiceLongName().c_str(),
                IService::getInstance()->getServiceUnifiedName().c_str());
  log.displayNL("Service listening port: %d",
                IService::getInstance()->ListeningPort.get());
  log.displayNL("Service running directory: '%s'",
                IService::getInstance()->RunningDirectory.c_str());
  log.displayNL("Service log directory: '%s'",
                IService::getInstance()->LogDirectory.c_str());
  log.displayNL("Service save files directory: '%s'",
                IService::getInstance()->SaveFilesDirectory.c_str());
  log.displayNL("Service write files directory: '%s'",
                IService::getInstance()->WriteFilesDirectory.c_str());
  log.displayNL("Service config directory: '%s' config filename: '%s.cfg'",
                IService::getInstance()->ConfigDirectory.c_str(),
                IService::getInstance()->_LongName.c_str());
  log.displayNL("Service id: %hu", IService::getInstance()->_SId.get());
  log.displayNL("Service update timeout: %dms",
                IService::getInstance()->_UpdateTimeout);
  log.displayNL("Service %suse naming service",
                IService::getInstance()->_DontUseNS ? "don't " : "");
  log.displayNL("Service %suse admin executor service",
                IService::getInstance()->_DontUseAES ? "don't " : "");
  log.displayNL("NeL is compiled in %s mode", CompilationMode.c_str());

  log.displayNL("Services arguments: %d args",
                IService::getInstance()->_Args.size());
  for (uint i = 0; i < IService::getInstance()->_Args.size(); i++) {
    log.displayNL("  argv[%d] = '%s'", i,
                  IService::getInstance()->_Args[i].c_str());
  }
...
NLMISC_CATEGORISED_COMMAND(
  ...
  log.displayNL("Window Displayer : XWinParam = %d; YWinParam = %d; WWinParam "
                "= %d; HWinParam = %d;",
                x, y, w, h);
  return true;
}

NLMISC_CATEGORISED_COMMAND(nel, displayConfigFile,
...
NLMISC_CATEGORISED_COMMAND(
    nel, getUnknownConfigFileVariables,
  ...
  log.displayNL("%d Variables not found in the configfile '%s'",
                IService::getInstance()->ConfigFile.UnknownVariables.size(),
                IService::getInstance()->ConfigFile.getFilename().c_str());
  for (uint i = 0;
       i < IService::getInstance()->ConfigFile.UnknownVariables.size(); i++) {
    log.displayNL(
        "  %s",
        IService::getInstance()->ConfigFile.UnknownVariables[i].c_str());
  }
  return true;
}
...
NLMISC_CATEGORISED_DYNVARIABLE(
  ...
  if (get) {
    ...
    if (IService::getInstance()->getServiceShortName() == "AES" ||
        IService::getInstance()->getServiceShortName() == "AS") {
      nlinfo("SERVICE: I can't set State=0 because I'm the admin and I should "
             "never quit");
    } else if (*pointer == "0" || *pointer == "2") {
      // ok, we want to set the value to false, just quit
      nlinfo("SERVICE: User ask me with a command to quit using the State "
             "variable");
  ...
  // whether reading or writing, the internal value of the state variable should
  // end up as the result of getFullStatus()
  IService *srv = IService::getInstance();
  if (srv == NULL) {
    return;
  }
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, CPULoad,
                               "Get instant CPU load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getCPULoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, ProcessLoad,
                               "Get instant CPU load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getProcessLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, CPUUserLoad,
                               "Get instant CPU user load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getCPUUserLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, CPUSytemLoad,
                               "Get instant CPU system load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getCPUSystemLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(
    cpu, float, CPUNiceLoad,
    "Get instant CPU nice processes load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getCPUNiceLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, CPUIOWaitLoad,
                               "Get instant CPU IO wait load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getCPUIOWaitLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(
    cpu, float, ProcessUserLoad,
    "Get instant CPU user load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getProcessUserLoad();
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(
    cpu, float, ProcessSystemLoad,
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, MeanCPULoad,
                               "Get instant CPU load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, MeanProcessLoad,
                               "Get instant CPU load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, MeanCPUUserLoad,
                               "Get instant CPU user load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, MeanCPUSytemLoad,
                               "Get instant CPU system load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, MeanCPUIOWaitLoad,
                               "Get instant CPU IO wait load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(
    cpu, float, MeanProcessSystemLoad,
    "Get instant CPU system load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
    *pointer = IService::getInstance()->getCPUUsageStats().getProcessSystemLoad(
        CCPUTimeStat::Mean);
  }
}

NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, PeakCPULoad,
                               "Get instant CPU load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, PeakProcessLoad,
                               "Get instant CPU load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, PeakCPUUserLoad,
                               "Get instant CPU user load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, PeakCPUSytemLoad,
                               "Get instant CPU system load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(cpu, float, PeakCPUIOWaitLoad,
                               "Get instant CPU IO wait load of the server") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {
...
NLMISC_CATEGORISED_DYNVARIABLE(
    cpu, float, PeakProcessSystemLoad,
    "Get instant CPU system load of the process/service") {
  nlunreferenced(human);
  // read or write the variable
  if (get) {

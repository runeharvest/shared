...
public:
  ...
  // interface for adding commands, retrieving commands and verifying whether
  // there are commands waiting
...
CStdinMonitorThread::CStdinMonitorThread() { _CommandWaiting = false; }
...
void CStdinMonitorThread::pushCommand(std::string nextCommand) {
  ...
  // set the _CommandWaiting flag, to allow reader thread to treat the new
  // command
...
bool CStdinMonitorThread::commandWaiting() const { return _CommandWaiting; }
...
void CStdinMonitorSingleton::init() {
  ...
  _StdinMonitorThreadHandle =
      NLMISC::IThread::create(_StdinMonitorThreadInstance, 1024 * 4 * 4);
...
void CStdinMonitorSingleton::update() {
  ...
  // if there's a command waiting then treat it (not more than one command per
  // visit)
...
void CStdinMonitorSingleton::release() {
  // if we've never been initialised or we've already been released thent
  ...
  // destroy the thread object instance and reset the pointer to NULL to mark as
  // 'uninitialised'

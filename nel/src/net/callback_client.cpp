 ...
 * Constructor
 */
CCallbackClient::CCallbackClient(TRecordingState rec,
                                 const std::string &recfilename, bool recordall,
...
CCallbackClient::~CCallbackClient() { nlassert(!LockDeletion); }
 ...
 * Replayed : MAYBE
 */
void CCallbackClient::send(const CMessage &buffer, TSockId hostid,
                           bool /* log */) {
  ...
  //	if (log)
  {
    //		nldebug ("LNETL3C: Client: send(%s)",
    //buffer.toString().c_str()); 		nldebug ("send message number %u",
    //SendNextValue);
  }
...
void CCallbackClient::receive(CMessage &buffer, TSockId *hostid) {
  ...
  *hostid = InvalidSockId;
  ...
  if (_MR_RecordingState != Replay) {
    ...
    // and received \todo remove this debug feature when ok
#ifdef NL_BIG_ENDIAN
  ...
  if (_MR_RecordingState != Replay) {
    try {
      ...
      if (_MR_RecordingState == Record) {
        ...
        _MR_Recorder.recordNext(_MR_UpdateCounter, Connecting, _BufSock,
                                addrmsg);
      }
      ...
      if (_MR_RecordingState == Record) {
        ...
        CMessage addrmsg;
        addrmsg.serial(const_cast<CInetHost &>(addrs));
        _MR_Recorder.recordNext(_MR_UpdateCounter, ConnFailing, _BufSock,
                                addrmsg);
      }
      throw;
    }
    ...
    default:
      nlwarning(
          "LNETL3C: No connection event in replay data, at update #%" NL_I64
          "u",
          _MR_UpdateCounter);
    }
  }
  ...
  if (_BufSock->connectedState()) {
    ...
    if (_MR_RecordingState != Replay) {
      ...
      if (!(_MR_Recorder.checkNextOne(_MR_UpdateCounter) == Disconnecting)) {
        nlwarning("LNETL3C: No disconnection event in the replay data, at "
                  "update #%" NL_I64 "u",
                  _MR_UpdateCounter);
      }
    }
    // Record or replay disconnection (because disconnect() in the client does
    // not push a disc. event)

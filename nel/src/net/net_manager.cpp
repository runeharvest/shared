...
static void nmNewDisconnection(TSockId from, void *arg) {
  ...
  nldebug("HNETL4: nmNewDisconnection() from service '%s'",
          basest->Name.c_str());

  // call the client callback if necessary
  if (basest->DisconnectionCallback != NULL)
    basest->DisconnectionCallback(basest->Name, from,
                                  basest->DisconnectionCbArg);
}

// find a not connected callbackclient or create a new one and connect to the
// Addr
void CNetManager::createConnection(CBaseStruct &Base, const CInetAddress &Addr,
                                   const string &name) {
  uint i;
  ...
  if (i == Base.NetBase.size()) {
    CCallbackClient *cc =
        new CCallbackClient(_RecordingState, name + string(".nmr"));
    Base.NetBase.push_back(cc);
  }

  CCallbackClient *cc = dynamic_cast<CCallbackClient *>(Base.NetBase[i]);

  cc->CCallbackNetBase::setDisconnectionCallback(nmNewDisconnection,
                                                 (void *)&Base);
...
void RegistrationBroadcast(const std::string &name, TServiceId sid,
                           const vector<CInetAddress> &addr) {
  nldebug("HNETL4: RegistrationBroadcast() of service %s-%hu", name.c_str(),
          (uint16)sid.get());

  // find if this new service is interesting
  for (CNetManager::ItBaseMap itbm = CNetManager::_BaseMap.begin();
       itbm != CNetManager::_BaseMap.end(); itbm++) {
    if ((*itbm).second.Type == CBaseStruct::Client &&
        !(*itbm).second.NetBase[0]->connected()) {
      if (name == (*itbm).first) {
...
static void UnregistrationBroadcast(const std::string &name, TServiceId sid,
                                    const vector<CInetAddress> &addr) {
  nldebug("HNETL4: UnregistrationBroadcast() of service %s-%hu", name.c_str(),
          (uint16)sid.get());
}
...
void CNetManager::addServer(const std::string &serviceName, uint16 servicePort,
  ...
  CCallbackServer *cs =
      new CCallbackServer(_RecordingState, serviceName + string(".nmr"));
  ...
  cs->CCallbackNetBase::setDisconnectionCallback(nmNewDisconnection,
                                                 (void *)&((*itbm).second));
  ...
  // register the server to the naming service if we are connected to Naming
  // Service
  ...
  nlinfo("HNETL4: Server '%s' added, registered and listen to port %hu",
         serviceName.c_str(), servicePort);
}

void CNetManager::addClient(const std::string &serviceName,
                            const std::string &addr, bool autoRetry) {
  nldebug("HNETL4: Adding client '%s' with addr '%s' in CNetManager",
          serviceName.c_str(), addr.c_str());
  ItBaseMap itbm = find(serviceName);
...
void CNetManager::addClient(const std::string &serviceName) {
  ...
  CCallbackClient *cc = new CCallbackClient(
      _RecordingState,
      serviceName + string(".nmr")); // ? - would not work if several clients
                                     // with the same name
  ...
  if (CNamingClient::lookupAndConnect(serviceName, *cc)) {
    ...
    if ((*itbm).second.ConnectionCallback != NULL)
      (*itbm).second.ConnectionCallback(serviceName, cc->getSockId(),
                                        (*itbm).second.ConnectionCbArg);
  }
}

void CNetManager::addGroup(const std::string &groupName,
                           const std::string &serviceName) {
  nlassert(CNamingClient::connected());
  nldebug("HNETL4: Adding '%s' to group '%s' in CNetManager",
  ...
  vector<string>::iterator it =
      std::find((*itbm).second.ServiceNames.begin(),
                (*itbm).second.ServiceNames.end(), serviceName);
...
void CNetManager::addCallbackArray(const std::string &serviceName,
                                   const TCallbackItem *callbackarray,
                                   CStringIdArray::TStringId arraysize) {
  nldebug("HNETL4: addingCallabckArray() for service '%s'",
          serviceName.c_str());
  ItBaseMap itbm = find(serviceName);
  for (uint32 i = 0; i < (*itbm).second.NetBase.size(); i++) {
...
void CNetManager::update(TTime timeout) {
        ...
        if ((*itbm).second.NetBase[i]->connected()) {
          ...
          if (CTime::getLocalTime() - lastTime > 5000) {
            ...
            if ((*itbm).second.Type == CBaseStruct::ClientWithAddr &&
                (*itbm).second.AutoRetry) {
              CCallbackClient *cc =
                  dynamic_cast<CCallbackClient *>((*itbm).second.NetBase[i]);
              try {
                nlassert((*itbm).second.ServiceNames.size() == 1);
                cc->connect(CInetAddress((*itbm).second.ServiceNames[0]));

                if ((*itbm).second.ConnectionCallback != NULL)
                  (*itbm).second.ConnectionCallback(
                      (*itbm).second.Name, cc->getSockId(),
                      (*itbm).second.ConnectionCbArg);
              } catch (ESocketConnectionFailed &e) {
                // can't connect now, try later
                nlinfo("HNETL4: can't connect now to %s (reason: %s)",
                       (*itbm).second.ServiceNames[0].c_str(), e.what());
              }
            }
          }
        }
  ...
  //	nlinfo("time : %f %f %f %d", CTime::ticksToSecond(p2-p1),
  //CTime::ticksToSecond(p3-p2), CTime::ticksToSecond(p4-p3), timeout);
}

void CNetManager::send(const std::string &serviceName, const CMessage &buffer,
                       TSockId hostid) {
  nldebug("HNETL4: send for service '%s' message %s to %s", serviceName.c_str(),
          buffer.toString().c_str(), hostid->asString().c_str());
  ItBaseMap itbm = find(serviceName);
  for (uint32 i = 0; i < (*itbm).second.NetBase.size(); i++) {
    if ((*itbm).second.NetBase[i]->connected())
...
void CNetManager::setConnectionCallback(const std::string &serviceName,
                                        TNetManagerCallback cb, void *arg) {
  nldebug("HNETL4: setConnectionCallback() for service '%s'",
          serviceName.c_str());
  ItBaseMap itbm = find(serviceName);
...
void CNetManager::setDisconnectionCallback(const std::string &serviceName,
                                           TNetManagerCallback cb, void *arg) {
  nldebug("HNETL4: setDisconnectionCallback() for service '%s'",
          serviceName.c_str());
  ItBaseMap itbm = find(serviceName);

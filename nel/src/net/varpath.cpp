...
string CVarPath::getToken() {
  ...
  case ' ':
    break;
  default:
    while (TokenPos < RawVarPath.size() && RawVarPath[TokenPos] != '.' &&
           RawVarPath[TokenPos] != '[' && RawVarPath[TokenPos] != ']' &&
           RawVarPath[TokenPos] != ',' && RawVarPath[TokenPos] != '=' &&
           RawVarPath[TokenPos] != ' ') {
      res += RawVarPath[TokenPos++];
    }
...
void CVarPath::decode() {
  ...
  if (val == "[") {
    for (;;) {
      ...
      for (;;) {
        ...
        // end of token
        if (val.empty()) {
          nlwarning("VP: Bad VarPath '%s', suppose it s an empty varpath",
                    RawVarPath.c_str());
          Destination.clear();
          return;
        }
    ...
    nlwarning("VP: Malformated VarPath '%s' before position %d",
              RawVarPath.c_str(), TokenPos);
    return;
  }
  ...
  val = getToken();
  if (val == " ") {
    // special case, there s a space that means that everything after is not
    // really part of the varpath.
...
void CVarPath::display() {
  ...
  for (uint i = 0; i < Destination.size(); i++) {
    nlinfo("VP:  > '%s' '%s'", Destination[i].first.c_str(),
           Destination[i].second.c_str());
  }
}

NLMISC_CATEGORISED_COMMAND(nel, varPath, "Test a varpath (for debug purpose)",
                           "<rawvarpath>") {
  ...
  for (uint i = 0; i < vp.Destination.size(); i++) {
    log.displayNL("Dest '%s' value '%s'", vp.Destination[i].first.c_str(),
                  vp.Destination[i].second.c_str());
  }

...
CBufClient::CBufClient(bool nodelay, bool replaymode, bool)
    : CBufNetBase(),
#endif
      _NoDelay(nodelay), _PrevBytesDownloaded(0), _PrevBytesUploaded(0),
      _RecvTask(NULL), _RecvThread(NULL)
...
_PrevBytesSent( 0 )*/
{
  ...
  if (replaymode) {
    _BufSock = new CNonBlockingBufSock(
        new CDummyTcpSock(), CBufNetBase::DefaultMaxExpectedBlockSize);
  } else {

    _BufSock =
        new CNonBlockingBufSock(NULL, CBufNetBase::DefaultMaxExpectedBlockSize);
...
bool CBufClient::dataAvailable() {
  // slow down the layer H_AUTO (CBufClient_dataAvailable);
  {
     ...
     * disconnection event (rare), call the callback and loop
     */
    while (dataAvailableFlag()) {
      ...
      default: // should not occur
      {
        ...
        LNETL1_INFO("LNETL1: Invalid block type: %hu (should be = %hu)",
...
void CBufClient::receive(NLMISC::CMemStream &buffer) {
  ...
  // commented for optimisation LNETL1_DEBUG( "LNETL1: Client read buffer (%d+%d
  // B)", buffer.size(), sizeof(TSockId)+1 );
...
void CBufClient::disconnect(bool quick) {
  ...
  // When the NS tells us to remove this connection AND the connection has
  // physically disconnected but not yet logically (i.e. disconnection event not
  // processed yet), skip flushing and physical active disconnection
  if (_BufSock->Sock->connected()) {
...
/*
 * Returns the number of bytes pushed by send() since the previous call to this
 * method
 */
...
void CClientReceiveTask::run() {
  ...
  //						can generate a 60 s wait on
  //destruction of the CBufSock 						By the way, checking every 1s is not a time
  //consuming
    ...
    try {
      ...
      while (!_NBBufSock->Sock->dataAvailable()) {
        if (!_NBBufSock->Sock->connected()) {
          LNETL1_DEBUG("LNETL1: Client connection %s closed",
      ...
      {
        // commented out for optimisation: LNETL1_DEBUG( "LNETL1: Client %s
        // received buffer (%u bytes)", _SockId->asString().c_str(),
        // buffer.size()/*, stringFromVector(buffer).c_str()*/ );
        //  Add event type
      ...
      LNETL1_DEBUG("LNETL1: Client connection %s broken",
                   sockId()->asString().c_str());
...
NLMISC_CATEGORISED_VARIABLE(nel, uint32, NbClientReceiveTask,

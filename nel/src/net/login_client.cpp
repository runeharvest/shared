...
static void cbShardValidate(CMessage &msgin, TSockId from,
...
static void cbVerifyLoginPassword(CMessage &msgin, TSockId from,
                                  CCallbackNetBase &netbase) {
  //
...
static void cbShardChooseShard(CMessage &msgin, TSockId from,
                               CCallbackNetBase &netbase) {
  //
...
string CLoginClient::authenticate(const string &loginServiceAddr,
                                  const ucstring &login,
                                  const string &cpassword,
                                  const string &application) {
  string result =
      authenticateBegin(loginServiceAddr, login, cpassword, application);
  if (!result.empty())
    return result;
...
string CLoginClient::authenticateBegin(const string &loginServiceAddr,
                                       const ucstring &login,
                                       const string &cpassword,
                                       const string &application) {
  VerifyLoginPasswordReason.clear();
  ...
  try {
    if (_LSCallbackClient == 0) {
      ...
      _LSCallbackClient->addCallbackArray(LSCallbackArray,
                                          sizeof(LSCallbackArray) /
                                              sizeof(LSCallbackArray[0]));
    }
    ...
    _LSCallbackClient = 0;
    nlwarning("Connection refused to LS (addr:%s): %s",
              loginServiceAddr.c_str(), e.what());
    return toString("Connection refused to LS (addr:%s): %s",
                    loginServiceAddr.c_str(), e.what());
  }
...
string CLoginClient::connectToShard(CLoginCookie &lc, const std::string &addr,
                                    CCallbackClient &cnx) {
  ...
  try {
    ...
    cnx.addCallbackArray(FESCallbackArray, sizeof(FESCallbackArray) /
                                               sizeof(FESCallbackArray[0]));
...
string CLoginClient::connectToShard(const std::string &addr, CUdpSock &cnx) {
  ...
  try {
    ...
    //
    // If a personal firewall such as ZoneAlarm is installed and permission not
    // granted yet, the connect blocks until the user makes a choice. If the
    // user denies the connection, the exception ESocket is thrown. Other
    // firewalls such as Kerio make the send() fail instead.
    //
    cnx.connect(CInetHost(addr));
  } catch (const ESocket &e) {
    return string("FES refused the connection (") + e.what() + ")";
...
string CLoginClient::confirmConnection(sint32 shardId) {
  ...
  nlinfo("addr: '%s' cookie: %s", ShardChooseShardAddr.c_str(),
         ShardChooseShardCookie.c_str());

  return "";
}

string CLoginClient::wantToConnectToShard(sint32 shardId, string &ip,
                                          string &cookie) {
...
bool CLoginClient::selectShardUpdate(string &error, string &ip,
                                     string &cookie) {
  if (!_LSCallbackClient) {
...
CLoginClient::CShardEntry *CLoginClient::getShard(sint32 shardId) {
  for (TShardList::iterator it = ShardList.begin(); it != ShardList.end();
       it++) {
    if ((*it).Id == shardId)
      return &(*it);
  }

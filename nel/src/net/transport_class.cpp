...
NLMISC::CVariable<bool>
    VerboseNETTC("nel", "VerboseNETTC",
                 "Enable verbose logging in CTransportClass operations", true,
                 0, true);
...
map<string, CTransportClass::CRegisteredClass>
    CTransportClass::LocalRegisteredClass; // registered class that are in my
                                           // program
...
string typeToString(CTransportClass::TProp type) {
  string conv[] = {
      ...
      "PropDataSetRow", "PropSheetId", "PropUCString", "PropUKN"};
  //		"PropBool", "PropFloat", "PropDouble", "PropString",
  //"PropDataSetRow", "PropEntityId", "PropSheetId", "PropUKN" };
...
void CTransportClass::displayDifferentClass(
    TServiceId sid, const string &className,
    const vector<CRegisteredBaseProp> &otherClass,
    const vector<CRegisteredBaseProp *> &myClass) {
  NETTC_INFO("NETTC: Service with sid %hu send me the TransportClass '%s' with "
             "differents properties:",
             sid.get(), className.c_str());
  NETTC_INFO("NETTC:  My local TransportClass is:");
  for (uint i = 0; i < myClass.size(); i++) {
    NETTC_INFO("NETTC:    Property: %d Name: '%s' type: '%s'", i,
               myClass[i]->Name.c_str(),
               typeToString(myClass[i]->Type).c_str());
  }

  NETTC_INFO("NETTC:  The other side TransportClass is:");
  for (uint i = 0; i < otherClass.size(); i++) {
    NETTC_INFO("NETTC:    Property: %d Name: '%s' type: '%s'", i,
               otherClass[i].Name.c_str(),
               typeToString(otherClass[i].Type).c_str());
  }
}

void CTransportClass::registerOtherSideClass(TServiceId sid,
                                             TOtherSideRegisteredClass &osrc) {
  for (TOtherSideRegisteredClass::iterator it = osrc.begin(); it != osrc.end();
       it++) {
    ...
    if (res == LocalRegisteredClass.end()) {
      ...
      // there was previously an nlwarning here but that was wrong because it is
      // quite normal for this to happen when one service ueses different
      // transport classes to communicate with several different services, so
      // the message has been changed to an nldebug
      NETTC_DEBUG("NETTC: the other side class '%s' declared from service %d "
                  "is not registered in my system, skip it",
                  (*it).first.c_str(), (uint32)sid.get());
      continue;
    }
    ...
    for (sint j = 0; j < (sint)(*it).second.size(); j++) {
      ...
      for (k = 0; k < (*res).second.Instance->Prop.size(); k++) {
        if ((*it).second[j].Name == (*res).second.Instance->Prop[k]->Name) {
          if ((*it).second[j].Type != (*res).second.Instance->Prop[k]->Type) {
            nlwarning(
                "NETTC: Property '%s' of the class '%s' have not the same type "
                "in the 2 sides (%s %s)",
                (*it).second[j].Name.c_str(), (*it).first.c_str(),
                typeToString((*it).second[j].Type).c_str(),
                typeToString((*res).second.Instance->Prop[k]->Type).c_str());
          }
          break;
        }
      }
      if (k == (*res).second.Instance->Prop.size()) {
        // not found, put -1
        (*res).second.Instance->States[sid.get()].push_back(
            make_pair(-1, (*it).second[j].Type));
      } else {
    ...
    if ((*it).second.size() != (*res).second.Instance->Prop.size()) {
      // 2 class don't have the same number of prop => different class =>
      // display class
      displayDifferentClass(sid, (*it).first.c_str(), (*it).second,
                            (*res).second.Instance->Prop);
    } else {
      ...
      for (uint i = 0; i < (*res).second.Instance->Prop.size(); i++) {
        if ((*res).second.Instance->Prop[i]->Name != (*it).second[i].Name) {
          ...
          displayDifferentClass(sid, (*it).first.c_str(), (*it).second,
                                (*res).second.Instance->Prop);
          break;
        } else if ((*res).second.Instance->Prop[i]->Type !=
                   (*it).second[i].Type) {
...
void CTransportClass::registerClass(CTransportClass &instance) {
  ...
  LocalRegisteredClass[TempRegisteredClass.Instance->Name] =
      TempRegisteredClass;
...
void CTransportClass::unregisterClass() {
  for (TRegisteredClass::iterator it = LocalRegisteredClass.begin();
       it != LocalRegisteredClass.end(); it++) {
...
void CTransportClass::displayLocalRegisteredClass(CRegisteredClass &c) {
  ...
  for (uint j = 0; j < c.Instance->Prop.size(); j++) {
    NETTC_DEBUG("NETTC:    > %s %s", c.Instance->Prop[j]->Name.c_str(),
                typeToString(c.Instance->Prop[j]->Type).c_str());
  }
  ...
  for (uint l = 0; l < c.Instance->States.size(); l++) {
    if (c.Instance->States[l].size() != 0) {
      ...
      for (uint k = 0; k < c.Instance->States[l].size(); k++) {
        NETTC_DEBUG("NETTC:      - %d type : %s",
                    c.Instance->States[l][k].first,
                    typeToString(c.Instance->States[l][k].second).c_str());
      }
    }
  }
}
...
void cbTCReceiveMessage(CMessage &msgin, const string &name, TServiceId sid) {
  ...
  CTransportClass::TRegisteredClass::iterator it =
      CTransportClass::LocalRegisteredClass.find(className);
  if (it == CTransportClass::LocalRegisteredClass.end()) {
    nlwarning(
        "NETTC: Receive unknown transport class '%s' received from %s-%hu",
        className.c_str(), name.c_str(), sid.get());
    return;
  }
  ...
  if (!(*it).second.Instance->read(name, sid)) {
    nlwarning("NETTC: Can't read the transportclass '%s' received from %s-%hu "
              "(probably not registered on sender service)",
              className.c_str(), name.c_str(), sid.get());
  }
}

void cbTCReceiveOtherSideClass(CMessage &msgin, const string & /* name */,
                               TServiceId sid) {
  ...
  for (uint i = 0; i < nbClass; i++) {
    ...
    osrc.push_back(
        make_pair(className, vector<CTransportClass::CRegisteredBaseProp>()));
    ...
    for (uint j = 0; j < nbProp; j++) {
      ...
      NETTC_DEBUG("NETTC:     %s %s", prop.Name.c_str(),
                  typeToString(prop.Type).c_str());
...
void cbTCUpService(const std::string &serviceName, TServiceId sid,
                   void * /* arg */) {
  NETTC_DEBUG("NETTC: CTransportClass Service %s %hu is up",
...
void CTransportClass::init() {
  ...
  nlassert(PropUInt8 < PropUKN);
  DummyProp[PropUInt8] = new CTransportClass::CRegisteredProp<uint8>;
  nlassert(PropUInt16 < PropUKN);
  DummyProp[PropUInt16] = new CTransportClass::CRegisteredProp<uint16>;
  nlassert(PropUInt32 < PropUKN);
  DummyProp[PropUInt32] = new CTransportClass::CRegisteredProp<uint32>;
  nlassert(PropUInt64 < PropUKN);
  DummyProp[PropUInt64] = new CTransportClass::CRegisteredProp<uint64>;
  nlassert(PropSInt8 < PropUKN);
  DummyProp[PropSInt8] = new CTransportClass::CRegisteredProp<sint8>;
  nlassert(PropSInt16 < PropUKN);
  DummyProp[PropSInt16] = new CTransportClass::CRegisteredProp<sint16>;
  nlassert(PropSInt32 < PropUKN);
  DummyProp[PropSInt32] = new CTransportClass::CRegisteredProp<sint32>;
  nlassert(PropSInt64 < PropUKN);
  DummyProp[PropSInt64] = new CTransportClass::CRegisteredProp<sint64>;
  nlassert(PropBool < PropUKN);
  DummyProp[PropBool] = new CTransportClass::CRegisteredProp<bool>;
  nlassert(PropFloat < PropUKN);
  DummyProp[PropFloat] = new CTransportClass::CRegisteredProp<float>;
  nlassert(PropDouble < PropUKN);
  DummyProp[PropDouble] = new CTransportClass::CRegisteredProp<double>;
  nlassert(PropString < PropUKN);
  DummyProp[PropString] = new CTransportClass::CRegisteredProp<string>;
  //	nlassert (PropDataSetRow < PropUKN); DummyProp[PropDataSetRow] = new
  //CTransportClass::CRegisteredProp<TDataSetRow>; 	nlassert (PropEntityId <
  //PropUKN); DummyProp[PropEntityId] = new
  //CTransportClass::CRegisteredProp<CEntityId>;
  nlassert(PropSheetId < PropUKN);
  DummyProp[PropSheetId] = new CTransportClass::CRegisteredProp<CSheetId>;
  nlassert(PropUCString < PropUKN);
  DummyProp[PropUCString] = new CTransportClass::CRegisteredProp<ucstring>;

  // we have to know when a service comes, so add callback (put the callback
  // before all other one because we have to send this message first)
  CUnifiedNetwork::getInstance()->setServiceUpCallback("*", cbTCUpService, NULL,
                                                       false);
...
/*
 * Get the name of message (for displaying), or extract the class name if it is
 * a transport class.
 *
 * Preconditions:
 ...
 * - msgName contains "msg %s" or "transport class %s" where %s is the name of
 * message, or the name transport class is the message is a CT_MSG.
 */
void getNameOfMessageOrTransportClass(NLNET::CMessage &msgin,
                                      std::string &msgName) {

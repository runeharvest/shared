...
/*
 * On Linux, the default limit of descriptors is usually 1024, you can increase
 * it with ulimit
 */
 ...
 * Constructor
 */
CBufServer::CBufServer(TThreadStategy strategy, uint16 max_threads,
                       uint16 max_sockets_per_thread, bool nodelay,
                       bool replaymode, bool initPipeForDataAvailable)
    :
#ifdef NL_OS_UNIX
      ...
      CBufNetBase(),
#endif
      _ThreadStrategy(strategy), _MaxThreads(max_threads),
      _MaxSocketsPerThread(max_sockets_per_thread), _ListenTask(NULL),
      _ListenThread(NULL), _ThreadPool("CBufServer::_ThreadPool"),
      _ConnectionCallback(NULL), _ConnectionCbArg(NULL), _BytesPushedOut(0),
      _BytesPoppedIn(0), _PrevBytesPoppedIn(0), _PrevBytesPushedOut(0),
      _NbConnections(0), _NoDelay(nodelay), _ReplayMode(replaymode) {
...
void CBufServer::init(uint16 port) {
  ...
  if (!_ReplayMode) {
    ...
    LNETL1_DEBUG("LNETL1: Binding listen socket to any address, port %hu",
                 port);
  }
}
...
CBufServer::~CBufServer() {
  ...
  if (!_ReplayMode) {
    ...
    CThreadPool::iterator ipt;
    {
      ...
      for (ipt = poolsync.value().begin(); ipt != poolsync.value().end();
           ++ipt) {
        ...
        {
          CSynchronized<CConnections>::CAccessor connectionssync(
              &task->_Connections);
          for (ipb = connectionssync.value().begin();
               ipb != connectionssync.value().end(); ++ipb) {
            (*ipb)->Sock->disconnect();
          }
        }
#endif
 ...
 * If hostid is not null and the socket is not connected, the method does
 * nothing. If quick is true, any pending data will not be sent before
 * disconnecting.
 */
void CBufServer::disconnect(TSockId hostid, bool quick) {
  ...
  if (hostid != InvalidSockId) {
    ...
    if (hostid->Sock->connected()) {
      ...
      hostid->Sock->disconnect(); // the connection will be removed by the next
                                  // call of update()
    }
  } else {
    // Disconnect all
...
void CBufServer::send(const CMemStream &buffer, TSockId hostid) {
  ...
  nlassertex(buffer.length() <= maxSentBlockSize(),
...
bool CBufServer::dataAvailable() {
  ...
  {
    /* If no data available, enter the 'while' loop and return false (1 volatile
     * test) If there are user data available, enter the 'while' and return true
     * immediately (1 volatile test + 1 short locking) If there is a
     * connection/disconnection event (rare), call the callback and loop
     */
    while (dataAvailableFlag()) {
      // Because _DataAvailable is true, the receive queue is not empty at this
      // point
      vector<uint8> buffer;
      uint8 val;
      {
      ...
      case CBufNetBase::Disconnection: {
        ...
        LNETL1_DEBUG("LNETL1: Disconnection event for %p %s", sockid,
                     sockid->asString().c_str());
      ...
      case CBufNetBase::Connection: {
        TSockId sockid = *((TSockId *)(&*buffer.begin()));
        LNETL1_DEBUG("LNETL1: Connection event for %p %s", sockid,
                     sockid->asString().c_str());
      ...
      default: // should not occur
        LNETL1_INFO("LNETL1: Invalid block type: %hu (should be = to %hu",
                    (uint16)(buffer[buffer.size() - 1]), (uint16)(val));
        LNETL1_INFO("LNETL1: Buffer (%d B): [%s]", buffer.size(),
                    stringFromVector(buffer).c_str());
...
#ifdef NL_OS_UNIX
/* Wait until the receive queue contains something to read (implemented with a
 * select()). This is where the connection/disconnection callbacks can be
 * called. \param usecMax Max time to wait in microsecond (up to 1 sec)
 */
void CBufServer::sleepUntilDataAvailable(uint usecMax) {
  // Prevent looping infinitely if the system time was changed
  if (usecMax > 999999) // limit not told in Linux man but here:
  ...
  do {
    ...
    tv.tv_sec = 0;
    tv.tv_usec = usecMax;
    int res = ::select(_DataAvailablePipeHandle[PipeRead] + 1, &readers, NULL,
                       NULL, &tv);
    if (res == -1)
      nlerror("LNETL1: Select failed in sleepUntilDataAvailable (code %u)",
              CSock::getLastError());
  } while (!dataAvailable()); // will loop if only a connection/disconnection
                              // event was read
}
#endif

/*
 * Receives next block of data in the specified. The length and hostid are
 * output arguments. Precond: dataAvailable() has returned true, phostid not
 * null
 */
void CBufServer::receive(CMemStream &buffer, TSockId *phostid) {
  ...
  *phostid =
      *((TSockId *)&(buffer.buffer()[buffer.size() - sizeof(TSockId) - 1]));
  ...
  // commented for optimisation LNETL1_DEBUG( "LNETL1: Read buffer (%d+%d B)
  // from %s", buffer.size(), sizeof(TSockId)+1,
  // /*stringFromVector(buffer).c_str(), */(*phostid)->asString().c_str() );
      ...
      CConnections::iterator ipb;
      {
        ...
        for (ipb = connectionssync.value().begin();
          ...
          if (!((*ipb)->Sock->connected() && (*ipb)->update())) {
            ...
            LNETL1_DEBUG("LNETL1: Socket %s is disconnected",
                         (*ipb)->asString().c_str());
            // Disconnection event if disconnected (known either from flush (in
            // update) or when receiving data)
...
void CBufServer::displayThreadStat(NLMISC::CLog *log) {
  ...
  log->displayNL("server listen thread %p nbloop %d", _ListenTask,
                 _ListenTask->NbLoop);
}
...
void CListenTask::run() {
...
#ifdef NL_OS_UNIX
  SOCKET descmax;
  descmax = _ListenSock.descriptor() > _WakeUpPipeHandle[PipeRead]
                ? _ListenSock.descriptor()
                : _WakeUpPipeHandle[PipeRead];
#endif
  ...
  while (!exitRequired()) {
    try {
      ...
      int res = ::select(descmax + 1, &readers, NULL, NULL,
      ...
      case -1:
        ...
        if (CSock::getLastError() == 4) {
          LNETL1_DEBUG("LNETL1: Select failed (in listen thread): %s (code %u) "
                       "but IGNORED",
                       CSock::errorString(CSock::getLastError()).c_str(),
                       CSock::getLastError());
          continue;
        }
        nlerror("LNETL1: Select failed (in listen thread): %s (code %u)",
                CSock::errorString(CSock::getLastError()).c_str(),
                CSock::getLastError());
      }
      ...
      if (FD_ISSET(_WakeUpPipeHandle[PipeRead], &readers)) {
        uint8 b;
        if (read(_WakeUpPipeHandle[PipeRead], &b, 1) ==
            -1) // we were woken-up by the wake-up pipe
        {
      ...
      if (newSock != NULL) {
        ...
        LNETL1_DEBUG("LNETL1: New connection : %s",
                     bufsock->asString().c_str());
...
void CBufServer::dispatchNewSocket(CServerBufSock *bufsock) {
  ...
  if (_ThreadStrategy == SpreadSockets) {
    ...
    for (iptmin = iptmax = ipt = poolsync.value().begin();
         ipt != poolsync.value().end(); ++ipt) {
    ...
    // Check if we make the pool of threads grow (if we have not found vacant
    // room and if it is allowed to)
    if ((poolsync.value().empty()) ||
      ...
      LNETL1_DEBUG("LNETL1: New socket dispatched to thread %d",
                   iptmin - poolsync.value().begin());
    }
  } else // _ThreadStrategy == FillThreads
  {
    ...
    // Check if we have to make the thread pool grow (if we have not found
    // vacant room)
 ...
 */
void CBufServer::addNewThread(CThreadPool &threadpool,
  ...
  IThread *thr = IThread::create(task, 1024 * 4 * 4);
  {
    ...
    thr->start();
    LNETL1_DEBUG("LNETL1: Added a new thread; pool size is %d",
                 threadpool.size());
    LNETL1_DEBUG("LNETL1: New socket dispatched to thread %d",
                 threadpool.size() - 1);
  }
...
void CServerReceiveTask::run() {
  ...
  while (!exitRequired()) {
    ...
    CConnections::iterator ipb;
    {
      ...
      for (ipb = connectionssync.value().begin();
           ipb != connectionssync.value().end(); ++ipb) {
        if ((*ipb)->Sock->connected()) // exclude disconnected sockets that are
                                       // not deleted
        {
...
#ifdef NL_OS_WINDOWS
    TIMEVAL tv;
    tv.tv_sec = 0; // short time because the newly added connections can't be
                   // added to the select fd_set
    tv.tv_usec = 10000;
...
#ifdef NL_OS_WINDOWS
    case 0:
      continue; // time-out expired, no results
#endif
    case -1:
      // we'll ignore message (Interrupted system call) caused by a CTRL-C
      /*if (CSock::getLastError() == 4)
      {
          LNETL1_DEBUG ("LNETL1: Select failed (in receive thread): %s (code %u)
      but IGNORED", CSock::errorString( CSock::getLastError() ).c_str(),
      CSock::getLastError()); continue;
      }*/
      // nlerror( "LNETL1: Select failed (in receive thread): %s (code %u)",
      // CSock::errorString( CSock::getLastError() ).c_str(),
      // CSock::getLastError() );
      LNETL1_DEBUG("LNETL1: Select failed (in receive thread): %s (code %u)",
                   CSock::errorString(CSock::getLastError()).c_str(),
    ...
    for (ic = connections_copy.begin(); ic != connections_copy.end(); ++ic) {
      if (FD_ISSET((*ic)->Sock->descriptor(), &readers) != 0) {
        CServerBufSock *serverbufsock =
            static_cast<CServerBufSock *>(static_cast<CBufSock *>(*ic));
        try {
          // 4. Receive data
          if (serverbufsock->receivePart(sizeof(TSockId) +
                                         1)) // +1 for the event type
          {
            ...
            _Server->pushMessageIntoReceiveQueue(
                serverbufsock->receivedBuffer());
            ...
            // Statistics
            {
                CSynchronized<uint32>::CAccessor syncbpi (
            &_Server->syncBytesPushedIn() ); syncbpi.value() += bufsize;
            }
            */
          }
        }
        ...
        //				{
        //					LNETL1_DEBUG( "LNETL1:
        //Connection %s closed", serverbufsock->asString().c_str() );
        //					// The socket went to
        //_Connected=false when throwing the exception
        //				}
        catch (const ESocket &) {
          LNETL1_DEBUG("LNETL1: Connection %s broken",
                       serverbufsock->asString().c_str());
          (*ic)->Sock->disconnect();
        }
        /*
        #ifdef NL_OS_UNIX
                        skip = true; // don't check _WakeUpPipeHandle (yes,
        check it to read any written byte) #endif
  ...
  CConnections::iterator ic;
  {
    NLMISC::CSynchronized<CConnections>::CAccessor removesetsync(&_RemoveSet);
    {
      if (!removesetsync.value().empty()) {
        ...
        NLMISC::CSynchronized<CConnections>::CAccessor connectionssync(
            &_Connections);
        for (ic = removesetsync.value().begin();
             ic != removesetsync.value().end(); ++ic) {
...
NLMISC_CATEGORISED_VARIABLE(nel, uint32, NbServerListenTask,
                            "Number of server listen thread");
NLMISC_CATEGORISED_VARIABLE(nel, uint32, NbServerReceiveTask,

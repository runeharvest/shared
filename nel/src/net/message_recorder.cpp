...
CMessageRecorder::CMessageRecorder() : _RecordAll(true) {
  ...
  nlerror("The message recorder works only with plain text messages. Please "
 ...
 * Start recording
 */
bool CMessageRecorder::startRecord(const std::string &filename,
                                   bool recordall) {
  _Filename = filename;
...
/*
 * Same as stringFromVector() but assumes the vector contains only printable
 * characters
 */
 ...
 * Add a record
 */
void CMessageRecorder::recordNext(sint64 updatecounter, TNetworkEvent event,
                                  TSockId sockid, CMessage &message) {
  nlassert(_File.is_open());

  if ((_RecordAll) || (event != Sending)) {
    // Serial to stream
    TMessageRecord rec(event, sockid, message,
                       updatecounter /*CTime::getLocalTime()*/);
    ...
    int len = (int)(stream.length() - 2); // not the null character (and its
                                          // separator) at the end of the buffer
    _File << "* ";
    _File << len; // if we put the expression directly, it makes an access
                  // violation ! Weird.
    _File << " ";
...
bool CMessageRecorder::loadNext(TMessageRecord &record) {
  // WARNING!!! This features doesn't work anymore becaues bufferAsVector() is
  // not available with new CMemStream
  nlstop;
  return false;
...
bool CMessageRecorder::getNext(TMessageRecord &record, sint64 updatecounter) {
  if (!_PreloadedRecords.empty()) {
    if (_PreloadedRecords.front().UpdateCounter == updatecounter) {
      ...
      nlassert(updatecounter <
               _PreloadedRecords.front().UpdateCounter); // not >
      return false;
    }
  } else {
    if (loadNext(record)) {
...
/*
 * Returns true and the event type if the counter of the next data is
 * updatecounter
 */
TNetworkEvent CMessageRecorder::checkNextOne(sint64 updatecounter) {
  TMessageRecord record;
  if (getNext(record, updatecounter)) {
    nldebug("MR: Check next one: %s at update %" NL_I64 "u",
            EventToString(record.Event).c_str(), updatecounter);
    return record.Event;
  } else {
    return Error;
  }
...
TNetworkEvent CMessageRecorder::replayConnectionAttempt(const CInetHost &addr) {
  ...
  if (!_ConnectionAttempts.empty()) {
    ...
    for (ipr = _ConnectionAttempts.begin(); ipr != _ConnectionAttempts.end();
         ++ipr) {
      ...
      if (stored_addr == addr) {
        // Found
        event = (*ipr).Event;
        nldebug("MR: Connection attempt found at update %" NL_I64 "u",
                (*ipr).UpdateCounter);

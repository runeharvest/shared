...
/// Yield method
CVariable<uint32>
    UseYieldMethod("nel", "UseYieldMethod",
                   "0=select 1=usleep 2=nanosleep 3=sched_yield 4=none", 0, 0,
                   true);
#endif

/// Reduce sending lag
CVariable<bool> FlushSendsBeforeSleep(
    "nel", "FlushSendsBeforeSleep",
    "If true, send buffers will be flushed before sleep, not in next update",
    true, 0, true);

/// Network congestion monitoring
CVariable<uint> L5TotalBytesInLowLevelSendQueues(
    "nel", "L5TotalBytesInLowLevelSendQueues",
    "Number of bytes pending in send queues (postponed by non-blocking send()) "
    "for network congestion monitoring. N/A if FlushSendsBeforeSleep disabled)",
    0, 0, true);

/// Receiving size limit
CVariablePtr<uint32>
    DefaultMaxExpectedBlockSize("nel", "DefaultMaxExpectedBlockSize",
                                "If receiving more than this value in bytes, "
                                "the connection will be dropped",
                                &CBufNetBase::DefaultMaxExpectedBlockSize,
                                true);

/// Sending size limit
CVariablePtr<uint32> DefaultMaxSentBlockSize(
    "nel", "DefaultMaxSentBlockSize",
    "If sending more than this value in bytes, the program may be stopped",
    &CBufNetBase::DefaultMaxSentBlockSize, true);

#define AUTOCHECK_DISPLAY nlwarning
// #define AUTOCHECK_DISPLAY
// CUnifiedNetwork::getInstance()->displayInternalTables (), nlerror

const TServiceId TServiceId::InvalidId =
    TServiceId(std::numeric_limits<uint16>::max());
...
void uNetRegistrationBroadcast(const string &name, TServiceId sid,
                               const vector<CInetAddress> &addr) {
  nldebug("HNETL5: + naming %s-%u '%s'", name.c_str(), sid.get(),
          vectorCInetAddressToString(addr).c_str());
  ...
  if (uni->_IdCnx[sid.get()].State ==
      CUnifiedNetwork::CUnifiedConnection::NotUsed) {
    uni->_IdCnx[sid.get()] =
        CUnifiedNetwork::CUnifiedConnection(name, sid, false);
    uni->_UsedConnection.push_back(sid);
  }

  if (uni->_IdCnx[sid.get()].ExtAddress.isValid())
    AUTOCHECK_DISPLAY("HNETL5: %s-%u already inserted in the table with '%s'",
                      name.c_str(), sid.get(),
                      uni->_IdCnx[sid.get()].ExtAddress.toStringLong().c_str());
  ...
  uni->_IdCnx[sid.get()].setupNetworkAssociation(uni->_NetworkAssociations,
                                                 uni->_DefaultNetwork);
}

// when a service unregisters
void uNetUnregistrationBroadcast(const string &name, TServiceId sid,
                                 const vector<CInetAddress> &addr) {
  nldebug("HNETL5: - naming %s-%u '%s'", name.c_str(), sid.get(),
          vectorCInetAddressToString(addr).c_str());
  ...
  if (uc == 0)
    return; // should never happen, the getUnifiedConnection() will generate a
            // AUTOCHECK_DISPLAY
  ...
  if (!uc->Connections.empty()) {
    // set all connection to dead, now, all messages received on this socket
    ...
    //
    // It's the first connection that added the _NamedCnx so if there's no
    // connection, no need to remove entry in _NamedCnx
    //
  ...
  bool found = false;
  for (vector<TServiceId>::iterator it = uni->_UsedConnection.begin();
       it != uni->_UsedConnection.end(); it++) {
    if (*it == uc->ServiceId) {
      found = true;
  ...
  if (!found)
    AUTOCHECK_DISPLAY("HNETL5: can't find the sid %hu in the _UsedConnection",
                      uc->ServiceId.get());
...
void uncbDisconnection(TSockId from, void * /* arg */) {
  if (from->appId() == AppIdDeadConnection) {
    ...
    CUnifiedNetwork::CUnifiedConnection *uc = uni->getUnifiedConnection(sid);
    if (uc == 0) {
      nlinfo("HNETL5: - connec '%s' sid %hu", from->asString().c_str(),
             sid.get());
    } else {
      nlinfo("HNETL5: - connec '%s' %s-%hu", from->asString().c_str(),
             uc->ServiceName.c_str(), sid.get());

      if (uc->IsExternal) {
        if (!uc->AutoRetry) {
          // If it s a external service with no auto retry, remove the
          // connection
        ...
        for (i = 0; i < uc->Connections.size(); i++) {
          if (uc->Connections[i].valid() &&
              uc->Connections[i].CbNetBase->getSockId(
                  uc->Connections[i].HostId) == from) {
            if (uc->Connections[i].IsServerConnection) {
              // we have to remove the stuffs now because HostId will not be
              // accessible later
              uc->Connections[i].reset();
            } else {
              // if it s a client, we can't delete now because the callback
              // client is currently in use only disconnect
              if (uc->Connections[i].CbNetBase->connected()) {
                uc->Connections[i].CbNetBase->disconnect(
                    uc->Connections[i].HostId);
              }
            }
            break;
          }
        ...
        if (i == uc->Connections.size()) {
          AUTOCHECK_DISPLAY("HNETL5: received a disconnection from a service "
                            "but the connection is not in my list!");
        }
      }
    }
...
void uncbServiceIdentification(CMessage &msgin, TSockId from,
  ...
  if (from->appId() != AppIdDeadConnection)
    AUTOCHECK_DISPLAY(
        "HNETL5: received a connect ident from an unknown connection 0x%" NL_I64
        "X",
        from->appId());
  ...
  nlinfo("HNETL5: + connect ident '%s' %s-%hu pos %hu ext %d",
         from->asString().c_str(), inSName.c_str(), inSid.get(), (uint16)pos,
         (uint8)isExternal);
  ...
  if (inSid.get() == 0) {
    if (isExternal) {
      ...
      CUnifiedNetwork::getInstance()->_ExtSId.set(
          CUnifiedNetwork::getInstance()->_ExtSId.get() + 1);
      // the following was an nlwarning but this is in fact desired behavior and
      // not an error so we have changed to nlinfo
      nlinfo("HNETL5: Received a connection from a service with a SId 0, we "
             "give him the SId %d",
             inSid.get());
    } else {
      nlwarning("HNETL5: Received a connection from a service with a SId 0 and "
                "wasn't external, disconnecting it");
      netbase.disconnect();
      return;
    }
  }
  ...
  case CUnifiedNetwork::CUnifiedConnection::NotUsed: // add the new unified
                                                     // connection
    uni->_IdCnx[inSid.get()] =
        CUnifiedNetwork::CUnifiedConnection(inSName, inSid, isExternal);
  ...
  if (uni->_IdCnx[inSid.get()].IsExternal != isExternal) {
    AUTOCHECK_DISPLAY(
        "HNETL5: Receive a connection that is not totally external %d %d",
        uni->_IdCnx[inSid.get()].IsExternal, isExternal);
    return;
  }
  ...
  uni->_IdCnx[inSid.get()].Connections[pos] =
      CUnifiedNetwork::CUnifiedConnection::TConnection(&netbase, from);

  // If the connection is external, we'll never receive the ExtAddress by the
  // naming service, so add it manually
  if (isExternal) {
    uni->_IdCnx[inSid.get()].ExtAddress =
        CInetHost(netbase.hostAddress(from), true);
    uni->_IdCnx[inSid.get()].setupNetworkAssociation(uni->_NetworkAssociations,
                                                     uni->_DefaultNetwork);
  }
...
void uncbMsgProcessing(CMessage &msgin, TSockId from,
                       CCallbackNetBase & /* netbase */) {
  if (from->appId() == AppIdDeadConnection) {
  ...
  if (itcb == uni->_Callbacks.end()) {
    ...
    nlwarning("HNETL5: Can't find callback '%s' called by service %hu",
              msgin.getName().c_str(), sid.get());
  } else {
    CUnifiedNetwork::CUnifiedConnection *uc = uni->getUnifiedConnection(sid);

    if (uc == 0) {
      nlwarning("HNETL5: Received a message from a service %hu that is not "
                "ready (bad appid? 0x%" NL_I64 "X)",
                sid.get(), from->appId());
      return;
    }
    if ((*itcb).second == 0) {
      nlwarning("HNETL5: Received message %s from a service %hu but the "
                "associated callback is NULL",
                msgin.getName().c_str(), sid.get());
      return;
    }
...
TCallbackItem unServerCbArray[] = {{"UN_SIDENT", uncbServiceIdentification}};
...
public:
  ...
  struct CCheckAddress {
    CCheckAddress()
        : ConnectionId(0xDEAD), NeedCheck(false), AddressValid(false) {}
    CInetHost Address;
  ...
  void checkService(const CInetHost &address, uint connectionId,
                    uint connectionIndex, const std::string &service,
                    TServiceId id);
};
...
#if FINAL_VERSION
          nlinfo("HNETL5: can't connect to %s-%hu now (%s)",
                 CheckList[i].ServiceName.c_str(), CheckList[i].ServiceId.get(),
                 e.what());
#else
          nlwarning("HNETL5: can't connect to %s-%hu now (%s)",
                    CheckList[i].ServiceName.c_str(),
                    CheckList[i].ServiceId.get(), e.what());
#endif
        }
      }
    }
...
void CAliveCheck::checkService(const CInetHost &address, uint connectionId,
                               uint connectionIndex, const std::string &service,
                               TServiceId id) {
  uint i;
  for (i = 0; i < sizeof(CheckList) / sizeof(CheckList[0]); ++i) {
...
bool CUnifiedNetwork::init(const CInetHost *addr,
                           CCallbackNetBase::TRecordingState rec,
                           const string &shortName, uint16 port,
                           TServiceId &sid) {
  ...
  if (_NamingServiceAddr.isValid()) {
    ...
    CNamingClient::setUnregistrationBroadcastCallback(
        uNetUnregistrationBroadcast);

    // connect to the naming service (may generate a ESocketConnectionFailed
    // exception)
  ...
  // setup the server callback only if server port != 0, otherwise there's no
  // server callback
  _ServerPort = port;
  ...
  if (_ServerPort != 0) {
    ...
    _CbServer =
        new CCallbackServer(CCallbackNetBase::Off, "", true,
                            false); // don't init one pipe per connection
#if defined(NL_OS_UNIX)
    _CbServer->setExternalPipeForDataAvailable(
        _MainDataAvailablePipe); // the main pipe is shared for all connections
    ...
    do {
      ...
      try {
        ...
        nlwarning("Failed to init the listen socket on port %u, is the service "
                  "already running ?",
                  port);
    ...
    _CbServer->addCallbackArray(unServerCbArray,
                                1); // the service ident callback
    _CbServer->setDefaultCallback(
        uncbMsgProcessing); // the default callback wrapper
  ...
  if (CNamingClient::connected()) {
    ...
    nlinfo("HNETL5: Server '%s' added, registered and listen to port %hu",
           _Name.c_str(), _ServerPort);
  }
...
void CUnifiedNetwork::connect() {
  nlassertex(
      _Initialised == true,
      ("Try to CUnifiedNetwork::connect() whereas it is not initialised yet"));

  if (ThreadCreator != NLMISC::getThreadId())
    nlwarning("HNETL5: Multithread access but this class is not thread safe "
              "thread creator = %u thread used = %u",
              ThreadCreator, NLMISC::getThreadId());

  if (CNamingClient::connected()) {
    // get the services list
    const list<CNamingClient::CServiceEntry> &services =
    ...
    for (its = services.begin(); its != services.end(); ++its) {
      if (_SId != (*its).SId) {
        // add service with name, address, ident, not external, service id, and
        // not autoretry (obsolete) we put the last true because the name
        // callback should already inserted it by uNetRegistrationBroadcast()
        addService((*its).Name, CInetHost((*its).Addr, true), true, false,
                   (*its).SId, false, true);
      } else {
        // don't process services received after mine because they'll connect to
        // me
        break;
      }
    }
...
void CUnifiedNetwork::release(
    bool mustFlushSendQueues,
    const std::vector<std::string> &namesOfOnlyServiceToFlushSending) {
  if (!_Initialised)
    return;
  ...
  if (mustFlushSendQueues) {
    ...
    while ((bytesRemaining =
                tryFlushAllQueues(namesOfOnlyServiceToFlushSending)) != 0) {
      if (i == 0)
        totalBytes = (float)bytesRemaining;
      if (i % 20 == 0) {
        nldebug("%.1f%% of %.3f KB flushed so far", // display without HNETL5 to
                                                    // bypass filters!
                ((float)(bytesRemaining - totalBytes)) / totalBytes,
                totalBytes / 1024.0f);
      }
  ...
  for (uint i = 0; i < _IdCnx.size(); ++i) {
    if (_IdCnx[i].State != CUnifiedNetwork::CUnifiedConnection::NotUsed) {
      for (uint j = 0; j < _IdCnx[i].Connections.size(); j++) {
        if (_IdCnx[i].Connections[j].valid() &&
            !_IdCnx[i].Connections[j].IsServerConnection) {
...
void CUnifiedNetwork::addService(const string &name, const CInetHost &addr,
                                 bool sendId, bool external, TServiceId sid,
                                 bool autoRetry, bool shouldBeAlreayInserted) {
  nlassertex(_Initialised == true, ("Try to CUnifiedNetwork::addService() "
                                    "whereas it is not initialised yet"));
  ...
  nlinfo("HNETL5: addService %s-%hu '%s'", name.c_str(), sid.get(),
         vectorCInetAddressToString(addr).c_str());
  ...
  // at this point it s possible that the service already added in the _IdCnx by
  // the uNetRegistrationBroadcast()

  if (shouldBeAlreayInserted &&
      _IdCnx[sid.get()].State == CUnifiedNetwork::CUnifiedConnection::NotUsed)
    AUTOCHECK_DISPLAY("HNETL5: the unified connection should already set by "
                      "the naming reg broadcast and is not (%hu)",
                      sid.get());
  if (!shouldBeAlreayInserted &&
      _IdCnx[sid.get()].State == CUnifiedNetwork::CUnifiedConnection::Ready)
    AUTOCHECK_DISPLAY(
        "HNETL5: the unified connection should not already set but is (%hu)",
        sid.get());
  ...
  if (_IdCnx[sid.get()].State == CUnifiedNetwork::CUnifiedConnection::NotUsed) {
    *uc = CUnifiedConnection(name, sid, external);
    ...
    if (name != uc->ServiceName)
      AUTOCHECK_DISPLAY("HNETL5: name are different in addService %s %s",
                        name.c_str(), uc->ServiceName.c_str());
    if (sid != uc->ServiceId)
      AUTOCHECK_DISPLAY("HNETL5: sid are different in addService %hu %hu",
                        sid.get(), uc->ServiceId.get());
    if (addr != uc->ExtAddress)
      AUTOCHECK_DISPLAY(
          "HNETL5: external addr are different in addService '%s' '%s'",
          addr.toStringLong().c_str(), uc->ExtAddress.toStringLong().c_str());
  }
  ...
  CCallbackClient *cbc =
      new CCallbackClient(CCallbackNetBase::Off, "", true,
                          false); // don't init one pipe per connection
#if defined(NL_OS_UNIX)
  cbc->setExternalPipeForDataAvailable(
      _MainDataAvailablePipe); // the main pipe is shared for all connections
  ...
  try {
    ...
    nlwarning("HNETL5: can't connect to %s (sid %u) now (%s) '%s'",
              name.c_str(), sid.get(), e.what(), addr.toStringLong().c_str());
  ...
  if (connectSuccess && sendId) {
    ...
    if (uc->IsExternal) {
      // in the case that the service is external, we can't send our sid because
      // the external service can have other connection with the same sid (for
      // example, LS can have 2 WS with same sid => sid = 0 and leave the other
      // side to find a good number
      ssid.set(0);
    }
  ...
  if (cntok) {
    ...
    callServiceUpCallback(
        name,
        sid); // global callback ("*") will be called even for external service
  }
...
void CUnifiedNetwork::update(TTime timeout) {
  ...
  nlassertex(
      _Initialised == true,
      ("Try to CUnifiedNetwork::update() whereas it is not initialised yet"));
  ...
  if (_NamingServiceAddr.isValid()) {
    if (CNamingClient::connected()) {
      ...
      try {
        ...
        nlwarning("HNETL5: Could not connect to the Naming Service (%s). "
                  "Retrying in a few seconds...",
                  _NamingServiceAddr.toStringLong().c_str());
      }
    }
  }
  ...
  for (;;) {
    ...
    if (CAliveCheck::Thread) {
      uint i;
      for (i = 0; i < sizeof(CAliveCheck::Thread->CheckList) /
                          sizeof(CAliveCheck::Thread->CheckList[0]);
           ++i) {
        ...
        if (address.NeedCheck && address.AddressValid) {
          ...
          if (uc.ServiceName == address.ServiceName &&
              uc.ServiceId == address.ServiceId && uc.ValidRequested) {
    ...
    for (uint k = 0; k < _UsedConnection.size(); ++k) {
      ...
      nlassert(_IdCnx[_UsedConnection[k].get()].State ==
               CUnifiedNetwork::CUnifiedConnection::Ready);
      for (uint j = 0; j < _IdCnx[_UsedConnection[k].get()].Connections.size();
           j++) {
        ...
        CUnifiedConnection::TConnection &conn =
            _IdCnx[_UsedConnection[k].get()].Connections[j];
        ...
        if (conn.CbNetBase->connected()) {
          ...
          if (uc.ValidExternal) {
            ...
            CAliveCheck::Thread->checkService(uc.ExtAddress,
                                              _UsedConnection[k].get(), j,
                                              uc.ServiceName, uc.ServiceId);
          }
        }
      }
    }
    ...
    for (uint i = 0; i < _ConnectionToReset.size(); ++i) {
      // remove the _UsedConnection
      bool found = false;
      for (vector<TServiceId>::iterator it = _UsedConnection.begin();
           it != _UsedConnection.end(); it++) {
      ...
      if (!found)
        AUTOCHECK_DISPLAY(
            "HNETL5: can't find the sid %hu in the _UsedConnection",
            _IdCnx[_ConnectionToReset[i].get()].ServiceId.get());
    ...
    // If it's the end (or if the Unix system time was changed forwards), don't
    // sleep (currentTime > t0 + timeout)
    if (remainingTime <= 0)
      break;

    // If the Unix system time was changed backwards, don't wait more than
    // requested and don't provide an erroneous time to the sleep function that
    // would fail (currentTime < previousTime)
    if (remainingTime > prevRemainingTime) {
      // Restart at previousTime
      nldebug("Backward time sync detected (at least -%" NL_I64 "d ms)",
    ...
    case 0:
      sleepUntilDataAvailable(remainingTime);
    ...
    case 2:
      nlSleep(1);
      break; // 10 ms (by nanosleep, but 20 ms measured on kernel 2.4.20)
    case 3:
      ::sched_yield();
      break; // makes all slow (at least on kernel 2.4.20) !
    default:
      break; // don't sleep at all, makes all slow!
    }
 ...
 * Auto-reconnect
 */
void CUnifiedNetwork::autoReconnect(CUnifiedConnection &uc,
                                    uint connectionIndex) {
  H_AUTO(L5AutoReconnect);
  nlassert(connectionIndex == 0);
  try {
    CCallbackClient *cbc =
        (CCallbackClient *)uc.Connections[connectionIndex].CbNetBase;
    cbc->connect(uc.ExtAddress);
    uc.Connections[connectionIndex].CbNetBase->getSockId()->setAppId(
        uc.ServiceId.get());
    nlinfo("HNETL5: reconnection to %s-%hu success", uc.ServiceName.c_str(),
           uc.ServiceId.get());
    ...
    if (uc.SendId) {
      ...
      TServiceId ssid = _SId;
      if (uc.IsExternal) {
        // in the case that the service is external, we can't send our sid
        // because the external service can have other connection with the same
        // sid (for example, LS can have 2 WS with same sid => sid = 0 and leave
      ...
      msg.serial(pos); // send the position in the connection table
      msg.serial(uc.IsExternal);
      uc.Connections[connectionIndex].CbNetBase->send(
          msg, uc.Connections[connectionIndex].HostId);
    }
...
#if FINAL_VERSION
    nlinfo("HNETL5: can't connect to %s-%hu now (%s)", uc.ServiceName.c_str(),
           uc.ServiceId.get(), e.what());
#else
    nlwarning("HNETL5: can't connect to %s-%hu now (%s)",
              uc.ServiceName.c_str(), uc.ServiceId.get(), e.what());
#endif
  }
}
...
void CUnifiedNetwork::sleepUntilDataAvailable(TTime msecMax) {
  ...
  if (msecMax > 999) // limit not told in Linux man but here:
                     // http://docs.hp.com/en/B9106-90009/select.2.html
    msecMax = 999;
...
bool CUnifiedNetwork::isConnectionConnected(TServiceId sid) const {
  // a Connected connection is a connection that is Ready but that is not yet
  // connected (serverUp will be called latter via L5).
...
uint8 CUnifiedNetwork::findConnectionId(TServiceId sid, uint8 nid) {
  if (_IdCnx[sid.get()].Connections.size() == 0) {
    nlwarning(
        "HNETL5: Can't send message to %s because no connection are available",
        _IdCnx[sid.get()].ServiceName.c_str());
    return 0xFF;
  }
  ...
  if (nid == 0xFF) {
    // default network
    // nldebug ("HNETL5: nid %hu, will use the default connection %hu",
    // (uint16)nid, (uint16)connectionId);
  } else if (nid >= _IdCnx[sid.get()].NetworkConnectionAssociations.size()) {
    nlwarning("HNETL5: No net association for nid %hu, use the default "
              "connection %hu",
              (uint16)nid, (uint16)connectionId);
  } else {
    if (_IdCnx[sid.get()].NetworkConnectionAssociations[nid] >=
        _IdCnx[sid.get()].Connections.size()) {
      nlwarning(
          "HNETL5: Can't send message to %s because nid %d point on a bad "
          "connection (%d and only have %d cnx), use default connection",
          _IdCnx[sid.get()].ServiceName.c_str(), nid, connectionId,
          _IdCnx[sid.get()].Connections.size());
    } else {
  ...
  if (connectionId >= _IdCnx[sid.get()].Connections.size() ||
      !_IdCnx[sid.get()].Connections[connectionId].valid() ||
      !_IdCnx[sid.get()].Connections[connectionId].CbNetBase->connected()) {
    if (nid != 0xFF) {
      // not a default network. There's a problem with the selected
      // connectionID, so try to find a valid one
      nlwarning("HNETL5: Can't find selected connection id %hu to send message "
                "to %s because connection is not valid or connected, find a "
                "valid connection id",
                (uint16)connectionId, _IdCnx[sid.get()].ServiceName.c_str());
    }

    for (connectionId = 0; connectionId < _IdCnx[sid.get()].Connections.size();
         connectionId++) {
      if (_IdCnx[sid.get()].Connections[connectionId].valid() &&
          _IdCnx[sid.get()].Connections[connectionId].CbNetBase->connected()) {
        // we found one at last, use this one
        // nldebug ("HNETL5: Ok, we found a valid connectionid, use %hu",
        // (uint16)connectionId);
        if (nid < _IdCnx[sid.get()].NetworkConnectionAssociations.size()) {
          _IdCnx[sid.get()].NetworkConnectionAssociations[nid] =
              connectionId; // we set the preferred networkConnectionAssociation
        } else {
          if (nid == 0xFF) {
        ...
        nlwarning("HNETL5: selected connection id %hu from network %hu to send "
                  "message to %s",
                  (uint16)connectionId, (uint16)nid,
                  _IdCnx[sid.get()].ServiceName.c_str());
        break;
      }
    }
    ...
    if (connectionId == _IdCnx[sid.get()].Connections.size()) {
      nlwarning("HNETL5: Can't send message to %s because default connection "
                "is not exist, valid or connected",
                _IdCnx[sid.get()].ServiceName.c_str());
      return 0xFF;
    }
  }
...
uint CUnifiedNetwork::send(const string &serviceName, const CMessage &msgout,
                           bool warnIfNotFound, uint8 nid) {
  nlassertex(_Initialised == true,
             ("Try to CUnifiedNetwork::send(const string&, const CMessage&) "
              "whereas it is not initialised yet"));
  ...
  pair<TNameMappedConnection::const_iterator,
  ...
  if (range.first != _NamedCnx.end()) {
    for (it = range.first; it != range.second; ++it) {
      ...
      if (sid.get() >= _IdCnx.size() ||
          _IdCnx[sid.get()].State !=
              CUnifiedNetwork::CUnifiedConnection::Ready) {
        // It often happen when the service is down (connection broke and the
        // naming not already say that it s down) In this case, just warn
        nlwarning(
            "HNETL5: Can't send %s to the service '%s' because it was in the "
            "_NamedCnx but not in _IdCnx (means that the service is down)",
            msgout.getName().c_str(), serviceName.c_str());
        return false;
      }
      ...
      {
        nlwarning("HNETL5: Can't send %s message to %hu because no connection "
                  "available",
                  msgout.getName().c_str(), sid.get());
        continue;
      }

      _IdCnx[sid.get()].Connections[connectionId].CbNetBase->send(
          msgout, _IdCnx[sid.get()].Connections[connectionId].HostId);
    }
  }

  if (!found && warnIfNotFound)
    nlwarning("HNETL5: can't find service %s to send message %s",
              serviceName.c_str(), msgout.getName().c_str());
...
bool CUnifiedNetwork::send(TServiceId sid, const CMessage &msgout, uint8 nid) {
  nlassertex(_Initialised == true,
             ("Try to CUnifiedNetwork::send(TServiceId, const CMessage&) "
              "whereas it is not initialised yet"));
  ...
  if (sid.get() >= _IdCnx.size() ||
    ...
    nlwarning(
        "HNETL5: Can't send %s to the service '%hu' because not in _IdCnx",
        msgout.getName().c_str(), sid.get());
    return false;
  }
  ...
  if (connectionId == 0xff) // failed
  {
    nlwarning("HNETL5: Can't send %s to the service '%hu' because no "
...
void CUnifiedNetwork::sendAll(const CMessage &msgout, uint8 nid) {
  nlassertex(_Initialised == true,
             ("Try to CUnifiedNetwork::send(const CMessage&) whereas it is not "
              "initialised yet"));
  ...
  for (TServiceId::size_type i = 0; i < _IdCnx.size(); ++i) {
    if (_IdCnx[i].State == CUnifiedNetwork::CUnifiedConnection::Ready) {
      ...
      {
        nlwarning(
            "HNETL5: Can't send message to %u because no connection available",
            i);
        continue;
      }

      _IdCnx[i].Connections[connectionId].CbNetBase->send(
          msgout, _IdCnx[i].Connections[connectionId].HostId);
    }
  }
}
 ...
 * \param namesOfOnlyServiceToFlushSending If not empty, only the send queues to
 * the services specified (by short name) will be flushed.
 */
uint CUnifiedNetwork::tryFlushAllQueues(
  ...
  for (uint k = 0; k < _UsedConnection.size(); ++k) {
    ...
    // Skip the connection if it is not found in the 'only' list (except if the
    // list is empty)
    if ((!namesOfOnlyServiceToFlushSending.empty()) &&
        (std::find(namesOfOnlyServiceToFlushSending.begin(),
                   namesOfOnlyServiceToFlushSending.end(),
                   uc.ServiceName) == namesOfOnlyServiceToFlushSending.end()))
      continue;
    ...
    for (uint j = 0; j < uc.Connections.size(); j++) {
      ...
      if (uc.Connections[j].CbNetBase->connected()) {
        ...
        uc.Connections[j].CbNetBase->flush(uc.Connections[j].HostId,
                                           &bytesRemainingLocal);
...
void CUnifiedNetwork::addCallbackArray(
    const TUnifiedCallbackItem *callbackarray, sint arraysize) {
  uint i;

  for (i = 0; i < (uint)arraysize; ++i)
    _Callbacks.insert(
        make_pair(string(callbackarray[i].Key), callbackarray[i].Callback));
}

void CUnifiedNetwork::setServiceUpCallback(const string &serviceName,
                                           TUnifiedNetCallback cb, void *arg,
                                           bool back) {
  nlassert(cb != NULL);
  if (serviceName == "*") {
    if (back)
    ...
    if (back)
      ...
      _UpCallbacks[serviceName].insert(_UpCallbacks[serviceName].begin(),
                                       make_pair(cb, arg));
  }
}

void CUnifiedNetwork::removeServiceUpCallback(const string &serviceName,
  ...
  if (serviceName == "*") {
    ...
    if (_UpCallbacks.find(serviceName) != _UpCallbacks.end()) {
      ...
      std::list<TCallbackArgItem>::iterator first(list.begin()),
          last(list.end());
...
void CUnifiedNetwork::setServiceDownCallback(const string &serviceName,
                                             TUnifiedNetCallback cb, void *arg,
                                             bool back) {
  nlassert(cb != NULL);
  if (serviceName == "*") {
    ...
    if (back)
      ...
      _DownCallbacks[serviceName].insert(_DownCallbacks[serviceName].begin(),
                                         make_pair(cb, arg));
  }
}

void CUnifiedNetwork::removeServiceDownCallback(const string &serviceName,
                                                TUnifiedNetCallback cb,
                                                void *arg) {
  if (serviceName == "*") {
    uint i;
    for (i = 0; i < _DownUniCallback.size(); ++i) {
      if (_DownUniCallback[i].first == cb &&
          _DownUniCallback[i].second == arg) {
        // we found it
  ...
  for (vector<TServiceId>::iterator it = _UsedConnection.begin();
    ...
    if (_IdCnx[it->get()].State == CUnifiedNetwork::CUnifiedConnection::Ready)
      for (j = 0; j < _IdCnx[it->get()].Connections.size(); ++j)
        if (_IdCnx[it->get()].Connections[j].valid() &&
            !_IdCnx[it->get()].Connections[j].IsServerConnection)
    ...
    if (_IdCnx[it->get()].State == CUnifiedNetwork::CUnifiedConnection::Ready)
      for (j = 0; j < _IdCnx[it->get()].Connections.size(); ++j)
        if (_IdCnx[it->get()].Connections[j].valid() &&
            !_IdCnx[it->get()].Connections[j].IsServerConnection)
          received +=
              _IdCnx[it->get()].Connections[j].CbNetBase->getBytesReceived();
  }
    ...
    if (_IdCnx[it->get()].State == CUnifiedNetwork::CUnifiedConnection::Ready)
      for (j = 0; j < _IdCnx[it->get()].Connections.size(); ++j)
        if (_IdCnx[it->get()].Connections[j].valid() &&
            !_IdCnx[it->get()].Connections[j].IsServerConnection)
          sent +=
              _IdCnx[it->get()].Connections[j].CbNetBase->getSendQueueSize();
  }
    ...
    if (_IdCnx[it->get()].State == CUnifiedNetwork::CUnifiedConnection::Ready)
      for (j = 0; j < _IdCnx[it->get()].Connections.size(); ++j)
        if (_IdCnx[it->get()].Connections[j].valid() &&
            !_IdCnx[it->get()].Connections[j].IsServerConnection)
          received +=
              _IdCnx[it->get()].Connections[j].CbNetBase->getReceiveQueueSize();
  }
...
CCallbackNetBase *CUnifiedNetwork::getNetBase(const std::string &name,
                                              TSockId &host, uint8 nid) {
  nlassertex(_Initialised == true, ("Try to CUnifiedNetwork::getNetBase() "
                                    "whereas it is not initialised yet"));
  ...
  if (count <= 0) {
    ...
    nlwarning("HNETL5: %d services %s to getNetBase, returns the first valid",
              count, name.c_str());
  }
  ...
  if (connectionId == 0xff) // failed
  {
    nlwarning("HNETL5: Can't getNetBase %s because no connection available",
              name.c_str());
    host = InvalidSockId;
    return NULL;
  }
...
CCallbackNetBase *CUnifiedNetwork::getNetBase(TServiceId sid, TSockId &host,
  ...
  if (sid.get() >= _IdCnx.size() ||
      _IdCnx[sid.get()].State != CUnifiedNetwork::CUnifiedConnection::Ready) {
    nlwarning(
        "HNETL5: Can't get net base to the service '%hu' because not in _IdCnx",
        sid.get());
    host = InvalidSockId;
    return NULL;
  }
  ...
  if (connectionId == 0xff) // failed
  {
    nlwarning("HNETL5: Can't getNetBase %hu because no connection available",
              sid.get());
    host = InvalidSockId;
    return NULL;
  }
...
TUnifiedMsgCallback
CUnifiedNetwork::findCallback(const std::string &callbackName) {
...
bool CUnifiedNetwork::isServiceLocal(TServiceId sid) {
  ...
  for (uint i = 0; i < laddr.size(); i++) {
    for (uint j = 0; j < _IdCnx[sid.get()].ExtAddress.size(); j++) {
      ...
      if (_IdCnx[sid.get()].ExtAddress.addresses()[j].getAddress() ==
          laddr[i].getAddress())
        return true;
    }
  }
...
/*
 * Return a string identifying the service, using the format "NAME-sid" (or
 * "sid" only if not found)
 */
...
bool CUnifiedNetwork::isUsed() { return (_Instance != NULL); }

//
//
//

CUnifiedNetwork::CUnifiedConnection *
CUnifiedNetwork::getUnifiedConnection(TServiceId sid, bool warn) {
  if (sid.get() < _IdCnx.size() &&
      _IdCnx[sid.get()].State == CUnifiedConnection::Ready) {
    if (sid != _IdCnx[sid.get()].ServiceId) {
      AUTOCHECK_DISPLAY(
          "HNETL5: Sid index %hu is not the same that in the entry %hu",
          sid.get(), _IdCnx[sid.get()].ServiceId.get());
      return NULL;
    }
    return &_IdCnx[sid.get()];
  } else {
    if (warn)
      nlwarning("HNETL5: Try to get a bad unified connection (sid %hu is not "
                "in the table)",
                sid.get());
    return NULL;
  }
...
void CUnifiedNetwork::autoCheck() {
  ...
  for (itn = _NamedCnx.begin(); itn != _NamedCnx.end(); ++itn) {
    if ((*itn).first != _IdCnx[itn->second.get()].ServiceName)
      AUTOCHECK_DISPLAY("HLNET5: problem with name synchro between _NameCnx "
                        "'%s' and _IdCnx '%s' '%hd'",
                        (*itn).first.c_str(),
    ...
    if (_IdCnx[itn->second.get()].AutoCheck > 1)
      AUTOCHECK_DISPLAY(
          "HLNET5: problem with name synchro between _NameCnx '%s' and _IdCnx "
          "'%s' '%hd' more than one entry is named with the same name",
          (*itn).first.c_str(), _IdCnx[itn->second.get()].ServiceName.c_str(),
          itn->second.get());
    _IdCnx[itn->second.get()].AutoCheck++;
  }

  for (i = 0; i < _UsedConnection.size(); i++) {
    if (_IdCnx[_UsedConnection[i].get()].State !=
        CUnifiedNetwork::CUnifiedConnection::Ready)
      AUTOCHECK_DISPLAY("HLNET5: problem with the _UsedConnection syncro sid "
                        "%d is not used in _IdCnx",
                        _UsedConnection[i].get());
  }

  for (i = 0; i < _IdCnx.size(); i++) {
    if (_IdCnx[i].State == CUnifiedNetwork::CUnifiedConnection::Ready) {
      for (j = 0; j < _UsedConnection.size(); j++) {
        if (_UsedConnection[j].get() == i)
          break;
      }
      if (j == _UsedConnection.size())
        AUTOCHECK_DISPLAY("HLNET5: problem with the _UsedConnection syncro sid "
                          "%d is not in _UsedConnection",
                          i);
    }
  }
  ...
  for (i = 0; i < _IdCnx.size(); i++) {
    if (_IdCnx[i].State == CUnifiedNetwork::CUnifiedConnection::NotUsed) {
      if (_IdCnx[i].ServiceName != "DEAD")
        AUTOCHECK_DISPLAY("HLNET5: sid %d name should be DEAD and is '%s'", i,
                          _IdCnx[i].ServiceName.c_str());
      if (_IdCnx[i].ServiceId.get() != 0xDEAD)
        AUTOCHECK_DISPLAY("HLNET5: sid %d sid should be 0xDEAD and is 0x%X", i,
                          _IdCnx[i].ServiceId.get());
      if (!_IdCnx[i].Connections.empty())
        AUTOCHECK_DISPLAY(
            "HLNET5: sid %d connection size should be 0 and is %d", i,
            _IdCnx[i].Connections.size());
      if (_IdCnx[i].ExtAddress.isValid())
        AUTOCHECK_DISPLAY("HLNET5: sid %d ext addr size should be 0 and is %d",
                          i, _IdCnx[i].ExtAddress.size());
      if (_IdCnx[i].AutoCheck != 0)
        AUTOCHECK_DISPLAY("HLNET5: sid %d prob with syncro with _NamedCnx", i);
    } else if (_IdCnx[i].State == CUnifiedNetwork::CUnifiedConnection::Ready) {
      if (_IdCnx[i].ServiceId.get() != i)
        AUTOCHECK_DISPLAY("HNETL5: Bad syncro sid index sid entry for %d %d", i,
                          _IdCnx[i].ServiceId.get());

      if (_IdCnx[i].ServiceName == "DEAD")
        AUTOCHECK_DISPLAY("HLNET5: sid %d name should not be DEAD and is '%s'",
                          i, _IdCnx[i].ServiceName.c_str());
      if (_IdCnx[i].ServiceId.get() == 0xDEAD)
        AUTOCHECK_DISPLAY("HLNET5: sid %d sid should not be 0xDEAD and is 0x%X",
                          i, _IdCnx[i].ServiceId.get());
      if (_IdCnx[i].ExtAddress.isValid() &&
          _IdCnx[i].Connections.size() > _IdCnx[i].ExtAddress.size())
        AUTOCHECK_DISPLAY(
            "HLNET5: sid %d ext addr size should not be 0 and is %d", i,
            _IdCnx[i].ExtAddress.size());

      if (_IdCnx[i].AutoRetry == true && _IdCnx[i].Connections.size() > 1)
        AUTOCHECK_DISPLAY(
            "HLNET5: sid %d auto retry with more than one connection %d", i,
            _IdCnx[i].Connections.size());
      if (_IdCnx[i].AutoRetry == true && _IdCnx[i].IsExternal == false)
        AUTOCHECK_DISPLAY("HLNET5: sid %d auto retry with internal connection",
                          i);
      if (_IdCnx[i].AutoRetry == true &&
          _IdCnx[i].Connections[0].valid() == false)
        AUTOCHECK_DISPLAY("HLNET5: sid %d auto retry with invalid connection",
                          i);

      for (j = 0; j < _IdCnx[i].Connections.size(); j++) {
        if (_IdCnx[i].Connections[j].valid() &&
            !_IdCnx[i].Connections[j].IsServerConnection &&
            _IdCnx[i].Connections[j].CbNetBase->connected() &&
            _IdCnx[i].Connections[j].getAppId() != i)
          AUTOCHECK_DISPLAY("HLNET5: sid %d bad appid %" NL_I64 "X", i,
                            _IdCnx[i].Connections[j].getAppId());
      }

      for (j = 0; j < _IdCnx[i].NetworkConnectionAssociations.size(); j++) {
        if (_IdCnx[i].NetworkConnectionAssociations[j] != 0) {
          if (_NetworkAssociations[j] !=
              _IdCnx[i]
                  .ExtAddress
                  .addresses()[_IdCnx[i].NetworkConnectionAssociations[j]]
                  .internalNetAddress())
            AUTOCHECK_DISPLAY(
                "HLNET5: sid %d nid %d have address 0x%08x and is not the good "
                "connection net 0x%08x",
                i, j, _NetworkAssociations[j],
                _IdCnx[i]
                    .ExtAddress
                    .addresses()[_IdCnx[i].NetworkConnectionAssociations[j]]
                    .internalNetAddress());
...
void CUnifiedNetwork::displayInternalTables(NLMISC::CLog *log) {
  ...
  for (TNameMappedConnection::iterator it = _NamedCnx.begin();
       it != _NamedCnx.end(); it++) {
  ...
  for (i = 0; i < _IdCnx.size(); i++) {
    if (_IdCnx[i].State != CUnifiedNetwork::CUnifiedConnection::NotUsed) {
      ...
      for (j = 0; j < _IdCnx[i].NetworkConnectionAssociations.size(); j++) {
        log->displayNL("     * nid %d -> cnxn %hu", j,
                       (uint16)_IdCnx[i].NetworkConnectionAssociations[j]);
      }
    }
  }
  ...
  for (i = 0; i < _NetworkAssociations.size(); i++) {
    log->displayNL("> 0x%08x -> '%s'", _NetworkAssociations[i],
                   internalIPAddressToString(_NetworkAssociations[i]).c_str());
  }
}

bool CUnifiedNetwork::haveNamedCnx(const std::string &name, TServiceId sid) {
  CUnifiedNetwork::TNameMappedConnection::iterator it;
  pair<CUnifiedNetwork::TNameMappedConnection::iterator,
...
void CUnifiedNetwork::addNamedCnx(const std::string &name, TServiceId sid) {
  ...
  if (range.first != range.second) {
    ...
    if (it != range.second) {
      AUTOCHECK_DISPLAY("HNETL5: Try to add 2 times the same connection %s-%hu",
                        name.c_str(), sid.get());
      return;
    }
  }
...
void CUnifiedNetwork::removeNamedCnx(const std::string &name, TServiceId sid) {
  ...
  if (range.first == range.second) {
    AUTOCHECK_DISPLAY(
        "HNETL5: The unified connection %s-%hu wasn't on the _NamedCnx",
        name.c_str(), sid.get());
    return;
  }
...
void CUnifiedNetwork::addNetworkAssociation(const string &networkName,
                                            uint8 nid) {
  ...
  nlinfo("HNETL5: Associate network '%s' 0x%08x '%s' to nid %hu",
         networkName.c_str(), _NetworkAssociations[nid],
         internalIPAddressToString(_NetworkAssociations[nid]).c_str(),
         (uint16)nid);
}

void CUnifiedNetwork::callServiceUpCallback(const std::string &serviceName,
                                            TServiceId sid,
  ...
  if (_NotifiedUpCallbacks.find(pss) != _NotifiedUpCallbacks.end()) {
    nlwarning(
        "HNETL5: Attempt to call service UP callback twice for '%s', ignored!",
        serviceName.c_str());
    return;
  }
  ...
  CUnifiedNetwork::TNameMappedCallback::iterator it =
      _UpCallbacks.find(serviceName);
  if (it != _UpCallbacks.end()) {
    // call it
    for (list<TCallbackArgItem>::iterator it2 = (*it).second.begin();
         it2 != (*it).second.end(); it2++) {
      TUnifiedNetCallback cb = (*it2).first;
      if (cb)
        cb(serviceName, sid, (*it2).second);
      else
        nlwarning("HNETL5: User set an empty callback for '%s' service up",
                  serviceName.c_str());
    }
  }
...
void CUnifiedNetwork::callServiceDownCallback(const std::string &serviceName,
                                              TServiceId sid,
                                              bool callGlobalCallback) {
  std::pair<std::string, TServiceId> pss = std::make_pair(serviceName, sid);
  if (_NotifiedUpCallbacks.find(pss) == _NotifiedUpCallbacks.end()) {
    nlwarning("HNETL5: Attempt to call service DOWN callback twice for '%s', "
              "ignored!",
              serviceName.c_str());
    return;
  }
  ...
  // now we warn the user
  CUnifiedNetwork::TNameMappedCallback::iterator it =
      _DownCallbacks.find(serviceName);
  if (it != _DownCallbacks.end()) {
    ...
    for (list<TCallbackArgItem>::iterator it2 = (*it).second.begin();
      ...
      if (cb)
        ...
        nlwarning("HNETL5: User set an empty callback for '%s' service down",
                  serviceName.c_str());
    }
  }
...
void CUnifiedNetwork::CUnifiedConnection::display(bool full, CLog *log) {
  log->displayNL(
      "> %s-%hu %s %s %s (%d ExtAddr %d Cnx) TotalCb %d", ServiceName.c_str(),
      ServiceId.get(), IsExternal ? "External" : "NotExternal",
      AutoRetry ? "AutoRetry" : "NoAutoRetry", SendId ? "SendId" : "NoSendId",
      ExtAddress.size(), Connections.size(), TotalCallbackCalled);
  ...
  for (uint j = 0; j < maxc; j++) {
    ...
    if (j < Connections.size() && Connections[j].valid()) {
      ...
      ext += Connections[j]
                 .CbNetBase->getSockId(Connections[j].HostId)
                 ->asString();
...
bool createMessage(CMessage &msgout, const vector<string> &args, CLog &log) {
  for (uint i = 2; i < args.size(); i += 2) {
    ...
    if (type == "s8") {
      ...
      CEntityId e;
      e.fromString(value.c_str());
      msgout.serial(e);
    } else {
      log.displayNL("type '%s' is not a valid type", type.c_str());
      return false;
    }
  }
  return true;
...
NLMISC_CATEGORISED_VARIABLE(nel, uint32, TotalCallbackCalled,
                            "Total callback called number on layer 5");

NLMISC_CATEGORISED_DYNVARIABLE(nel, uint64, SendQueueSize,
...
NLMISC_CATEGORISED_DYNVARIABLE(nel, uint64, ReceiveQueueSize,
...
NLMISC_CATEGORISED_DYNVARIABLE(nel, uint64, ReceivedBytes,
...
NLMISC_CATEGORISED_DYNVARIABLE(nel, uint64, SentBytes,
 ...
 * for stl containers, you have first to put a u32 type that is the size of the
 * container and after all elements (ex: if you want to put a vector<uint16>
 * that have 3 elements: u32 3 u16 10 u16 11 u16 12)
 *
 * ex: msgin 128 REGISTER u32 10 u32 541 u32 45
 * You'll receive a fake message REGISTER that seems to come from the service
 * number 128 with 3 uint32.
 *
 */

NLMISC_CATEGORISED_COMMAND(
    nel, msgin,
    "Simulate an input message from another service (ex: msgin 128 REGISTER "
    "u32 10 b 1 f 1.5)",
  ...
  if (!CUnifiedNetwork::isUsed()) {
    log.displayNL(
        "Can't do that because the service doesn't use CUnifiedNetwork");
    return false;
  }
  ...
  if (cb == NULL) {
    log.displayNL("Callback for message '%s' is not found",
                  messageName.c_str());
  } else {
...
NLMISC_CATEGORISED_COMMAND(
    nel, msgout,
    "Send a message to a specified service (ex: msgout 128 REGISTER u32 10 b 1 "
    "f 1.5)",
    "<ServiceName>|<ServiceId> <MessageName> [<ParamType> <Param>]*") {
  nlunreferenced(rawCommandString);
  nlunreferenced(quiet);
  ...
  if (cnb == NULL) {
    log.displayNL("'%s' is a bad <ServiceId> or <ServiceName>",
                  args[0].c_str());
    return false;
  }
...
NLMISC_CATEGORISED_COMMAND(nel, l5QueuesStats,
  ...
  log.displayNL("%u Unified Connections:",
                CUnifiedNetwork::getInstance()->_IdCnx.size());
  for (uint i = 0; i < CUnifiedNetwork::getInstance()->_IdCnx.size(); i++) {
    if (CUnifiedNetwork::getInstance()->_IdCnx[i].State !=
        CUnifiedNetwork::CUnifiedConnection::NotUsed) {
...
NLMISC_CATEGORISED_COMMAND(nel, l5InternalTables,
...
NLMISC_CATEGORISED_COMMAND(nel, l5Callback,
  ...
  log.displayNL("There're %d registered callbacks:",
                CUnifiedNetwork::getInstance()->_Callbacks.size());
  uint i = 0;
  for (CUnifiedNetwork::TMsgMappedCallback::iterator it =
           CUnifiedNetwork::getInstance()->_Callbacks.begin();
       it != CUnifiedNetwork::getInstance()->_Callbacks.end(); it++) {
    log.displayNL(" %d '%s' %s", i++, (*it).first.c_str(),
                  ((*it).second == NULL ? "have a NULL address" : ""));
  }
...
NLMISC_CATEGORISED_COMMAND(
  ...
  TServiceId sid(nId);
  if (sid.get() > 0) {
    log.displayNL("Service %s-%hu and sid %s are %son the same computer",
                  CUnifiedNetwork::getInstance()->_Name.c_str(),
                  CUnifiedNetwork::getInstance()->_SId.get(), args[0].c_str(),
                  CUnifiedNetwork::getInstance()->isServiceLocal(sid) ? ""
                                                                      : "not ");
  } else {
    log.displayNL(
        "Service %s-%hu and %s are %son the same computer",
        CUnifiedNetwork::getInstance()->_Name.c_str(),
        CUnifiedNetwork::getInstance()->_SId.get(), args[0].c_str(),
        CUnifiedNetwork::getInstance()->isServiceLocal(args[0]) ? "" : "not ");
  }
...
NLMISC_CLASS_COMMAND_IMPL(CUnifiedNetwork, addService) {
  ...
  // syntax is as follow :
  // <serviceName> ( address=<address:port> [sid=<serviceId>] [sendId]
  // [external] [autoRetry] )
  ...
  ;
  if (!ia.isValid()) {
    log.displayNL("Can't parse internet address in '%s'",
                  address->ParamValue.c_str());
    return false;
  }
  ...
  log.displayNL("Adding service '%s' as sid %u with [sendId = %s], [external = "
                "%s], [autoRetry = %s]",
                serviceInfo->ParamName.c_str(), sid, sendId ? "YES" : "NO",
                external ? "YES" : "NO", autoRetry ? "YES" : "NO");

  addService(serviceInfo->ParamName, ia, sendId, external, serviceId, autoRetry,
             false);

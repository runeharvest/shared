...
#if defined(NL_OS_WINDOWS) && defined(GetNameInfo)
    res = GetNameInfoW((SOCKADDR *)&storage, sizeof(TSockAddrIn), host,
                       NI_MAXHOST, NULL, 0, NI_NAMEREQD);
#else
    res = getnameinfo((struct sockaddr *)&storage, sizeof(TSockAddrIn), host,
                      NI_MAXHOST, NULL, 0, NI_NAMEREQD);
#endif
  } else if (storage.ss_family == AF_INET6) {
    ((TSockAddrIn6 *)&storage)->sin6_port = 0;
#if defined(NL_OS_WINDOWS) && defined(GetNameInfo)
    res = GetNameInfoW((SOCKADDR *)&storage, sizeof(TSockAddrIn6), host,
                       NI_MAXHOST, NULL, 0, NI_NAMEREQD);
#else
    res = getnameinfo((struct sockaddr *)&storage, sizeof(TSockAddrIn6), host,
                      NI_MAXHOST, NULL, 0, NI_NAMEREQD);
#endif
  }
...
CInetHost::CInetHost() { m_Addresses.push_back(CInetAddress(false)); }
...
void CInetHost::set(const std::string &hostname, uint16 port) {
  ...
  if (status) {
    ...
    LNETL0_DEBUG(
        "LNETL0: Network error: resolution of hostname '%s' failed: %s",
        hostname.c_str(), gai_strerror(status));
    throw ESocket(
        (std::string("Hostname resolution failed for ") + hostname).c_str());
  }
...
void CInetHost::set(const CInetAddress &address, bool lookup) {
  ...
  if (lookup && m_Hostname.empty()) {
    ...
    if (std::find(localhost.addresses().begin(), localhost.addresses().end(),
                  address) != localhost.addresses().end()) {
      // It's me!
  ...
  if (m_Hostname.empty()) {
    ...
    {
      m_Hostname =
          nlstr("invalid.invalid"); // "invalid.invalid" invalid hostname
    }
  ...
  if (std::find(m_Addresses.begin(), m_Addresses.end(), address) ==
      m_Addresses.end()) {
...
CInetHost CInetHost::localAddresses(uint16 port, bool sort, bool loopback) {
  ...
  // Local hostname might be mapped to only return a loopback address, try
  // interface addresses as well
  ...
  if (!host.m_Addresses.size() && skipLoopback.size()) {
    nlwarning("The local hostname only resolves to %i loopback addresses, "
              "using them anyway",
              (int)skipLoopback.size());
  ...
  if (sort) {
    ...
    for (size_t i = 0; i < host.m_Addresses.size(); ++i) {
      ...
      if (!host.m_Addresses[i].getAddress().isIPv4() &&
          host.m_Addresses[i].getAddress().isValid())
    ...
    if (sorting.size() < host.m_Addresses.size()) {
      nlwarning("Discarded %i local host addresses (phase 1)",
                (int)(host.m_Addresses.size() - sorting.size()));
    }
    ...
    for (size_t i = 0; i < sorting.size(); ++i) {
      // Filter only IPv4 for loopback first, since [::1] appears to have
      // throttling on connect on Ubuntu
      if (sorting[i].getAddress().getType() == CIPv6Address::Loopback &&
          sorting[i].getAddress().isIPv4()) {
    ...
    if (host.m_Addresses.size() < sorting.size()) {
      // nlwarning("Discarded %i local host addresses (phase 2)",
      // (int)(sorting.size() - host.m_Addresses.size()));
    }
  }

  if (host.m_Addresses.empty()) {
    throw ESocket(
        NLMISC::toString("No network card detected for %s", localhost).c_str(),
        false);
  }
...
void CInetHost::serial(NLMISC::IStream &s) {
  ...
  // When hostname is empty, try to reverse lookup starting from the *last
  // *address, which is most likely a public one
  std::string hostname =
      m_Hostname == nlstr("localhost") ? std::string() : m_Hostname;
  s.serial(hostname);
...
std::string CInetHost::toString() const {
  std::string hostname =
      m_Hostname.empty() ? nlstr("invalid.invalid") : m_Hostname;

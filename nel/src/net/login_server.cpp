...
struct CPendingUser {
  CPendingUser(const CLoginCookie &cookie, const string &un, const string &up,
               const string &ux, uint32 instanceId, uint32 charSlot)
      : Cookie(cookie), UserName(un), UserPriv(up), UserExtended(ux),
        InstanceId(instanceId), CharSlot(charSlot) {
...
void CLoginServer::refreshPendingList() {
  ...
  while (it != PendingUsers.end()) {
    if ((*it).Time < Time - TimeBeforeEraseCookie) {
      nlinfo("LS: Removing cookie '%s' because too old",
             (*it).Cookie.toString().c_str());
...
void cbWSChooseShard(CMessage &msgin, const std::string & /* serviceName */,
                     TServiceId /* sid */) {
  ...
  for (it = PendingUsers.begin(); it != PendingUsers.end(); it++) {
    ...
    if (pu.Cookie == cookie) {
      ...
      nlwarning("LS: Cookie %s is already in the pending user list",
  ...
  if (it == PendingUsers.end()) {
    ...
    nlinfo("LS: New cookie %s (name '%s' priv '%s' extended '%s' instance %u "
           "slot %u) inserted in the pending user list (awaiting new client)",
           cookie.toString().c_str(), userName.c_str(), userPriv.c_str(),
           userExtended.c_str(), instanceId, charSlot);
    PendingUsers.push_back(CPendingUser(cookie, userName, userPriv,
                                        userExtended, instanceId, charSlot));
    reason.clear();
...
void cbWSDisconnectClient(CMessage &msgin, const std::string &serviceName,
                          TServiceId /* sid */) {
...
void cbShardValidation(CMessage &msgin, TSockId from,
  ...
  if (!reason.empty()) {
    nlwarning("LS: User (%s) is not in the pending user list (cookie:%s)",
              netbase.hostAddress(from).asString().c_str(),
              cookie.toString().c_str());
    // disconnect him
...
void CLoginServer::setListenAddress(const string &la) {
  ...
  if (ListenAddr.empty()) {
    ListenHost.clear();
    nlerror("FATAL : listen address in invalid, it should be either set via "
            "ListenAddress variable or with -D argument");
    nlstop;
  }

  nlinfo("LS: Listen Address that will be sent to the client is now '%s'",
         ListenAddr.c_str());
  ListenHost = CInetHost(ListenAddr);
}

uint32 CLoginServer::getNbPendingUsers() { return (uint32)PendingUsers.size(); }
...
void cfcbAcceptInvalidCookie(CConfigFile::CVar &var) {
  ...
  nlinfo("LS: This service %saccept invalid cookie",
         AcceptInvalidCookie ? "" : "doesn't ");
}
...
void cfcbTimeBeforeEraseCookie(CConfigFile::CVar &var) {
  ...
  nlinfo("LS: This service will remove cookie after %d seconds",
...
void CLoginServer::init(const string &listenAddress) {
  ...
  try {
    cfcbDefaultUserPriv(
        IService::getInstance()->ConfigFile.getVar("DefaultUserPriv"));
    IService::getInstance()->ConfigFile.setCallback("DefaultUserPriv",
                                                    cfcbDefaultUserPriv);
  } catch (const Exception &) {
  }

  try {
    cfcbAcceptInvalidCookie(
        IService::getInstance()->ConfigFile.getVar("AcceptInvalidCookie"));
    IService::getInstance()->ConfigFile.setCallback("AcceptInvalidCookie",
                                                    cfcbAcceptInvalidCookie);
  } catch (const Exception &) {
  }

  try {
    cfcbTimeBeforeEraseCookie(
        IService::getInstance()->ConfigFile.getVar("TimeBeforeEraseCookie"));
    IService::getInstance()->ConfigFile.setCallback("TimeBeforeEraseCookie",
                                                    cfcbTimeBeforeEraseCookie);
  } catch (const Exception &) {
  }
  ...
  setListenAddress(la);
  IService::getInstance()->ConfigFile.setCallback("ListenAddress",
                                                  cfcbListenAddress);
}
...
void CLoginServer::init(CCallbackServer &server, TNewClientCallback ncl) {
  ...
  server.addCallbackArray(ClientCallbackArray,
                          sizeof(ClientCallbackArray) /
                              sizeof(ClientCallbackArray[0]));
...
void CLoginServer::init(const std::string &listenAddr,
...
string CLoginServer::isValidCookie(const CLoginCookie &lc, string &userName,
                                   string &userPriv, string &userExtended,
                                   uint32 &instanceId, uint32 &charSlot) {
  userName.clear();
  userPriv.clear();
  ...
  for (it = PendingUsers.begin(); it != PendingUsers.end(); it++) {
    CPendingUser &pu = *it;
    if (pu.Cookie == lc) {
      nlinfo("LS: Cookie '%s' is valid and pending (user %s), send the client "
             "connection to the WS",
             lc.toString().c_str(), pu.UserName.c_str());
...
void CLoginServer::connectToWS() {
  CUnifiedNetwork::getInstance()->addCallbackArray(
      WSCallbackArray, sizeof(WSCallbackArray) / sizeof(WSCallbackArray[0]));
}
...
const std::string &CLoginServer::getListenAddress() { return ListenAddr; }

/// Call this method to retrieve the listen address
const CInetHost &CLoginServer::getListenHost() { return ListenHost; }
bool CLoginServer::acceptsInvalidCookie() { return AcceptInvalidCookie; }

//
// Commands
//

NLMISC_CATEGORISED_COMMAND(nel, lsUsers,
  ...
  if (ModeTcp) {
    log.displayNL("Display the %d connected users :",
                  UserIdSockAssociations.size());
    for (map<uint32, TSockId>::iterator it = UserIdSockAssociations.begin();
         it != UserIdSockAssociations.end(); it++) {
...
NLMISC_CATEGORISED_COMMAND(nel, lsPending,
  ...
  for (list<CPendingUser>::iterator it = PendingUsers.begin();
       it != PendingUsers.end(); it++) {
    log.displayNL("> %s %s", (*it).Cookie.toString().c_str(),
                  (*it).UserName.c_str());
  }
  log.displayNL("End of the list");
  ...
  if (get) {
    *pointer = ListenAddr;
    ...
    if ((*pointer).find(":") == string::npos) {
      nlwarning("LS: You must set the address + port (ie: "
                "\"itsalive.nevrax.org:38000\")");
      return;
    ...
    ListenHost = CInetHost(ListenAddr);
    nlinfo("LS: Listen Address that will be send to client is '%s'",
           ListenAddr.c_str());
  }
}

NLMISC_CATEGORISED_VARIABLE(
    nel, string, DefaultUserPriv,

...
const char *_CUniTime::getStringUniTime(TTime ut) {
  ...
  smprintf(str, 512, "%02d/%02d/%04d (week %d) %02d:%02d:%02d.%03d", day + 1,
           month + 1, year + 1, week + 1, h, m, s, ms);
  return str;
}
...
/*
static void cbGetUniversalTime (CMessage &msgin, TSockId from, CCallbackNetBase
&netbase)
{
    nlstop;
...
void _CUniTime::syncUniTimeFromService(
    CCallbackNetBase::TRecordingState /* rec */, const CInetHost * /* addr */) {
  nlstop;
  /***************************************************************/
  /******************* THE FOLLOWING CODE IS COMMENTED OUT *******/
  /***************************************************************
      ...
      server.addCallbackArray (UniTimeCallbackArray, sizeof
  (UniTimeCallbackArray) / sizeof (UniTimeCallbackArray[0]));
      ...
      nlinfo ("_CUniTime::syncUniTimeFromService(): ping:%" NL_I64 "dms,
  time:%ds, unitime:%" NL_I64 "dms", delta, GetUniversalTimeSecondsSince1970,
  GetUniversalTimeUniTime);

  // <-- from here to the "-->" comment, the block must be executed in less than
  one second or an infinite loop occurs

      // get the second
      firstsecond = CTime::getSecondsSince1970 ();
      nextsecond = firstsecond+1;

      // wait the next start of the second (take 100% of CPU to be more
  accurate) while (nextsecond != CTime::getSecondsSince1970 ()) nlassert
  (CTime::getSecondsSince1970 () <= nextsecond);
  ...
  // -->
      ...
      if ( ! _Simulate )
      {
          if (abs((sint32)((TTime)nextsecond -
  (TTime)GetUniversalTimeSecondsSince1970)) > 10)
          {
              nlerror ("the time delta (between me and the Time Service) is too
  big (more than 10s), servers aren't NTP synchronized"); goto error;
          }

          // compute the delta between the other side and our side number of
  second since 1970 deltaAdjust = ((TTime) nextsecond - (TTime)
  GetUniversalTimeSecondsSince1970) * 1000;
  ...
  NL_I64 "ds, rlocaltime:%" NL_I64 "d, deltaAjust:%" NL_I64 "dms", nextsecond,
  GetUniversalTimeUniTime, lt, deltaAdjust);
      }
      else
      {
          nlinfo ("_CUniTime::syncUniTimeFromService(): runitime:%" NL_I64 "ds,
  rlocaltime:%" NL_I64 "d", GetUniversalTimeUniTime, lt);
      }
  ...
  error:
      nlerror ("Time Service is not found, lost or can't synchronize universal
  time");
  ***************************************************************/
}
...
static void cbServerAskUniversalTime(CMessage & /* msgin */, TSockId from,
                                     CCallbackNetBase &netbase) {
  nlstop;
  TTime ut = _CUniTime::getUniTime();

  // afficher l adresse de celui qui demande
  nlinfo("UT: Send the universal time %" NL_I64 "d to '%s'", ut,
...
void _CUniTime::installServer(CCallbackServer *server) {
  ...
  server->addCallbackArray(ServerTimeServiceCallbackArray,
                           sizeof(ServerTimeServiceCallbackArray) /
                               sizeof(ServerTimeServiceCallbackArray[0]));
...
/*
static void cbClientGetUniversalTime (CMessage &msgin, TSockId from,
CCallbackNetBase &netbase)
{
    nlstop;
    // get the association between a date and unitime
...
void _CUniTime::syncUniTimeFromServer(CCallbackClient * /* client */) {
  ...
  /***************************************************************
      ...
      {
          client->addCallbackArray (ClientUniTimeCallbackArray, sizeof
  (ClientUniTimeCallbackArray) / sizeof (ClientUniTimeCallbackArray[0]));
          alreadyAddedCallback = true;
      }
      ...
      while (attempt < 10)
      {
          ...
          if (delta < 10 || delta < bestdelta)
          {
              ...
              _CUniTime::setUniTime (GetClientUniversalTimeUniTime,
  (before+after)/2);
      ...
      client->disconnect ();
      nlinfo ("Universal time is %" NL_I64 "dms with a mean error of %" NL_I64
  "dms", _CUniTime::getUniTime(), bestdelta/2); return; error: nlwarning
  ("there's no connection or lost or can't synchronize universal time");
  ***************************************************************/
}
...
NLMISC_CATEGORISED_COMMAND(nel, time, "displays the universal time", "")
{
    ...
    if ( _CUniTime::Sync )
    {
        log.displayNL ("CTime::getLocalTime(): %" NL_I64 "dms,
_CUniTime::getUniTime(): %" NL_I64 "dms", CTime::getLocalTime (),
_CUniTime::getUniTime ()); log.displayNL ("_CUniTime::getStringUniTime(): '%s'",
_CUniTime::getStringUniTime());
    }
    else
    {
        log.displayNL ("CTime::getLocalTime(): %" NL_I64 "dms <Universal time
not sync>", CTime::getLocalTime ());
    }

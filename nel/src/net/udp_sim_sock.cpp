...
struct CBufferizedOutPacket {
  CBufferizedOutPacket(CUdpSock *client, const uint8 *packet, uint32 packetSize,
                       uint32 delay, const CInetAddress *addr)
      : Client(client), PacketSize(packetSize),
...
void CUdpSimSock::sendUDPNow(const uint8 *buffer, uint32 len,
...
void CUdpSimSock::sendUDP(const uint8 *buffer, uint32 &len,
                          const CInetAddress *addr) {
  ...
  if ((float)rand() / (float)(RAND_MAX) * 100.0f >= _OutPacketLoss) {
    ...
    if (lag > 100) {
      ...
      CBufferizedOutPacket *bp =
          new CBufferizedOutPacket(&UdpSock, buffer, len, lag, addr);

      // duplicate the packet
      if ((float)rand() / (float)(RAND_MAX) * 100.0f < _OutPacketDisordering &&
          !_BufferizedOutPackets.empty()) {
...
void cbSimVar(CConfigFile::CVar &var) {
  if (var.Name == "SimInLag")
    CUdpSimSock::_InLag = var.asInt();
  else if (var.Name == "SimInPacketLost")
    CUdpSimSock::_InPacketLoss = uint8(var.asInt());
  else if (var.Name == "SimOutLag")
    CUdpSimSock::_OutLag = var.asInt();
  else if (var.Name == "SimOutPacketLost")
    CUdpSimSock::_OutPacketLoss = uint8(var.asInt());
  else if (var.Name == "SimOutPacketDuplication")
    CUdpSimSock::_OutPacketDuplication = uint8(var.asInt());
  else if (var.Name == "SimOutPacketDisordering")
    CUdpSimSock::_OutPacketDisordering = uint8(var.asInt());
  else
    nlstop;
}
...
void CUdpSimSock::connect(const CInetHost &addrs) { UdpSock.connect(addrs); }

void CUdpSimSock::close() { UdpSock.close(); }
  ...
  if (_InLag > 0) {
    while (UdpSock.dataAvailable()) {
      ...
      if ((float)rand() / (float)(RAND_MAX) * 100.0f >= _InPacketLoss) {
        CBufferizedOutPacket *bp =
            new CBufferizedOutPacket(&UdpSock, buffer, len, _InLag, &addr);
    ...
    if (!_BufferizedInPackets.empty() &&
        _BufferizedInPackets.front()->Time <= ct)
      return true;
    else
      return false;
  } else {
...
CSock::TSockResult CUdpSimSock::send(const uint8 *buffer, uint32 &len,
                                     bool /* throw_exception */) {
  sendUDP(buffer, len);
  return CSock::Ok;
}

void CUdpSimSock::sendTo(const uint8 *buffer, uint32 &len,
                         const CInetAddress &addr) {
  sendUDP(buffer, len, &addr);
}

bool CUdpSimSock::connected() { return UdpSock.connected(); }

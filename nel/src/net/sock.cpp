...
ESocket::ESocket(const char *reason, bool systemerror, CInetHost *addr) {
  ...
  if (addr != NULL) {
    ...
    smprintf(str, 256, reason,
             addr->toStringLong().c_str()); // reason *must* contain "%s"
    _Reason += str;
  } else {
...
/*
 * Initializes the network engine if it is not already done (under Windows,
 * calls WSAStartup()).
 */
void CSock::initNetwork() {
  ...
  if (!s_Initialized.load(NLMISC::TMemoryOrderRelaxed)) {
    ...
    s_Initialized = true;
    // Ok if it gets initialized more than once due to multiple threads reaching
    // here, just need to release multiple times!
    ++s_WsaInitCount;
  }
#else
  if (!s_Initialized.load(NLMISC::TMemoryOrderRelaxed)) {
 ...
 * Note: This code is platform-dependant. On Unix, it is errno; on Windows it is
 * the Winsock error code. See also errorString()
 */
uint CSock::getLastError() { return (uint)ERROR_NUM; }
...
std::string CSock::errorString(uint errorcode) {
  ...
  case WSAEINVAL /*10022*/:
    return "Invalid socket (maybe not bound) or argument";
  case WSAEMFILE /*10024*/:
    return "Too many open sockets";
  case WSAENOTSOCK /*10038*/:
    return "Socket operation on nonsocket (maybe invalid select descriptor)";
  case WSAEMSGSIZE /*10040*/:
    return "Message too long";
  case WSAEADDRINUSE /*10048*/:
    return "Address already in use (is this service already running in this "
           "computer?)";
  ...
  case WSAENOBUFS /*10055*/:
    return "No buffer space available; please close applications or reboot";
  case WSAESHUTDOWN /*10058*/:
    return "Cannot send/receive after socket shutdown";
  ...
  case WSANOTINITIALISED /*093*/:
    return "'Windows Sockets' not initialized";
  default:
    return "";
  }
#elif defined NL_OS_UNIX
...
CSock::CSock(bool logging)
    : _Sock(INVALID_SOCKET), _LocalAddr(false), _RemoteAddr(false),
      _Logging(logging), _NonBlocking(false), _BytesReceived(0), _BytesSent(0),
      _TimeoutS(0), _TimeoutUs(0), _MaxReceiveTime(0), _MaxSendTime(0),
      _Blocking(false), _AddressFamily(AF_UNSPEC) {
...
/*
 * Construct a CSock object using an existing connected socket descriptor and
 * its associated remote address
 */
CSock::CSock(SOCKET sock, const CInetAddress &remoteaddr)
    : _Sock(sock), _RemoteAddr(remoteaddr), _Logging(true), _NonBlocking(false),
      _BytesReceived(0), _BytesSent(0), _MaxReceiveTime(0), _MaxSendTime(0),
  ...
  // fcntl should be more portable but not tested fcntl(_Sock, F_SETFD,
  // FD_CLOEXEC);
#endif
}

static SOCKET createSocketInternal(int &family, int familyA, int familyB,
  ...
  if (res == INVALID_SOCKET) {
    // Fallback to IPv4
    nlwarning(
        "Could not create a socket of the preferred socket family %i, create a "
        "socket with family %i, the system may lack IPv6 support",
        familyA, familyB);
...
void CSock::createSocket(int type, int protocol) {
  ...
  _Sock =
      createSocketInternal(_AddressFamily, AF_INET6, AF_INET, type, protocol);

  if (_AddressFamily == AF_INET6) {
    // Ensure this is dual stack IPv6 and IPv4
    // Disable IPv6 ONLY flag
    int no = 0;
    if (setsockopt(_Sock, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&no, sizeof(no)) !=
        0) {
      nlwarning("Could not disable IPV6_V6ONLY flag, this means we're running "
                "on a legacy OS, and IPv4 is preferred");
#ifdef NL_OS_WINDOWS
      closesocket(_Sock);
#elif defined NL_OS_UNIX
      ::close(_Sock);
      ...
      _Sock = createSocketInternal(_AddressFamily, AF_INET, AF_INET6, type,
                                   protocol);
    }
  }
...
void CSock::close() {
  if (_Logging) {
    LNETL0_DEBUG("LNETL0: Socket %d closing for %s at %s", _Sock,
                 _RemoteAddr.asString().c_str(), _LocalAddr.asString().c_str());
  }
 ...
 */
CSock::~CSock() {
  // nlinfo( "Report for %s socket %s: Max send time: %u Max recv time: %u",
  // _NonBlocking?"non-blocking":"blocking", remoteAddr().asString().c_str(),
  // _MaxSendTime, _MaxReceiveTime ); nlinfo( "Max send time: %u",
  // _MaxSendTime);
  if (_Sock != INVALID_SOCKET) {
    if (_Logging) {
      LNETL0_DEBUG("LNETL0: Socket %d closing for %s at %s", _Sock,
                   _RemoteAddr.asString().c_str(),
                   _LocalAddr.asString().c_str());
    }
...
void CSock::connect(const CInetHost &addrs) {
  ...
  for (size_t ai = 0; ai < addrs.size(); ++ai) {
    ...
    LNETL0_DEBUG("LNETL0: Socket %d connecting to %s...", _Sock,
                 addrs.toStringLong(ai).c_str());
    ...
    attempted = true;
    // Connection (when _Sock is a datagram socket, connect establishes a
    // default destination address)
    if (::connect(_Sock, (const sockaddr *)(&sockAddr),
                  sizeOfSockAddr(sockAddr)) != 0) {
      /*		if ( _Logging )
              {
      #ifdef NL_OS_WINDOWS
                  nldebug( "Impossible to connect socket %d to %s %s (%d)",
      _Sock, addr.hostName().c_str(), addr.asIPString().c_str(), ERROR_NUM );
      #elif defined NL_OS_UNIX
                  nldebug( "Impossible to connect socket %d to %s %s (%d:%s)",
      _Sock, addr.hostName().c_str(), addr.asIPString().c_str(), ERROR_NUM,
      strerror(ERROR_NUM) ); #endif
              }
      */
      LNETL0_DEBUG("LNETL0: Socket %d failed to connect to %s", _Sock,
                   addrs.toStringLong(ai).c_str());
      continue;
    }
    setLocalAddress();
    if (_Logging) {
      LNETL0_DEBUG("LNETL0: Socket %d connected to %s (local %s)", _Sock,
                   addrs.toStringLong(ai).c_str(),
                   _LocalAddr.asString().c_str());
    }
    _RemoteAddr = addr;
...
bool CSock::dataAvailable() {
  ...
  case 0:
    return false;
  case -1:
    throw ESocket("CSock::dataAvailable(): select failed");
  default:
    return true;
  }
}
 ...
 */
CSock::TSockResult CSock::send(const uint8 *buffer, uint32 &len,
                               bool throw_exception) {
  TTicks before = CTime::getPerformanceTime();
  len = ::send(_Sock, (const char *)buffer, len, 0);
  _MaxSendTime =
      max((uint32)(CTime::ticksToSecond(CTime::getPerformanceTime() - before) *
                   1000.0f),
          _MaxSendTime);

  //	nldebug ("CSock::send(): Sent %d bytes to %d (%d)", len, _Sock,
  //ERROR_NUM);

  if (_Logging) {
    //		LNETL0_DEBUG ("LNETL0: CSock::send(): Sent %d bytes to %d res:
    //%d (%d)", realLen, _Sock, len, ERROR_NUM);
  }
  ...
  if (((int)len) == SOCKET_ERROR) {
    if (ERROR_NUM == ERROR_WOULDBLOCK) {
      ...
      // nlSleep(10);
      if (!_Blocking) {
        // nldebug("SendWouldBlock - %s / %s Entering snooze
        // mode",_LocalAddr.asString().c_str(),_RemoteAddr.asString().c_str());
        _Blocking = true;
      }
      return Ok;
    }
...
#ifdef NL_OS_WINDOWS
      throw ESocket(
          NLMISC::toString("Unable to send data: error %u", GetLastError())
              .c_str());
#else
  ...
  if (_Blocking) {
    // nldebug("SendWouldBlock - %s / %s Leaving snooze
    // mode",_LocalAddr.asString().c_str(),_RemoteAddr.asString().c_str());
    _Blocking = false;
  }
  return Ok;
 ...
 * Receives data
 */
CSock::TSockResult CSock::receive(uint8 *buffer, uint32 &len,
                                  bool throw_exception) {
  if (_NonBlocking) {
    ...
    // nlinfo ("CSock::receive(): NBM Received %d bytes to %d res: %d (%d)",
    // realLen, _Sock, len, ERROR_NUM);

    if (_Logging) {
      //			LNETL0_DEBUG ("LNETL0: CSock::receive(): NBM
      //Received %d bytes to %d res: %d (%d)", realLen, _Sock, len, ERROR_NUM);
    }

    _MaxReceiveTime = max(
        (uint32)(CTime::ticksToSecond(CTime::getPerformanceTime() - before) *
                 1000.0f),
        _MaxReceiveTime);
  ...
  } else // Blocking Mode
  {
    ...
    while (total < len) {
      ...
      //			nlinfo ("CSock::receive(): BM Received %d bytes
      //to %d res: %d (%d) total %d", len, _Sock, brecvd, ERROR_NUM, total);
...
void CSock::setSendBufferSize(sint32 size) {
  setsockopt(_Sock, SOL_SOCKET, SO_SNDBUF, (char *)(&size),
             (socklen_t)sizeof(size));
}

 ...
 */
class CLocalGateway : public CModuleBase,
...
public:
  ...
  virtual uint32 getTransportCount() const { return 0; }

  virtual uint32 getRouteCount() const { return 0; }

  virtual uint32 getReceivedPingCount() const { return 0; }
  ...
  virtual void onReceiveMessage(CGatewayRoute * /* from */,
  ...
  //		}
  //		virtual void
  //getGatewayClientList(std::vector<TModuleGatewayProxyPtr> gatewayList)
  //		{
  //			return;
  //		}
  ...
  //		{
  //		}
  //		virtual TModuleGatewayConstant
  //onClientGatewayConnect(TModuleGatewayProxyPtr &clientGateway)
  //		{
  //			return mgc_reject_connection;
  //		}
  //		virtual void onClientGatewayDisconnect(TModuleGatewayProxyPtr
  //&clientGateway)
  //		{
  //		}
  //		virtual void
  //getGatewayServerList(std::vector<TModuleGatewayProxyPtr> serverList)
  //		{
  //			return;
  //		}
  ...
  //			nlstop;
  //		}
  //		virtual void disconnectGateway(TModuleGatewayProxyPtr
  //&serverGateway)
  //		{
  //			nlstop;
  //		}
  //		virtual void onGatewayConnection(const TModuleGatewayProxyPtr
  //&serverGateway, TModuleGatewayConstant connectionResult)
  //		{
  //			nlstop;
  //		}
  //		virtual void onGatewayDisconnection(const TModuleGatewayProxyPtr
  //&serverGateway)
  //		{
  //			nlstop;
  //		}
  ...
  virtual void discloseModule(IModuleProxy *moduleProxy) {
    ...
    //			CModuleProxy *modProx = dynamic_cast<CModuleProxy
    //*>(moduleProxy); 			nlassert(modProx != NULL);
  ...
  getModuleProxyList(std::vector<IModuleProxy *> &resultList) const {
    ...
    {
      TModuleProxies::TAToBMap::const_iterator first(
          _ModuleProxies.getAToBMap().begin()),
          last(_ModuleProxies.getAToBMap().end());
      for (; first != last; ++first) {
    ...
    map<TModuleId, IModuleProxy *>::iterator first(index.begin()),
        last(index.end());
    for (; first != last; ++first) {
  ...
  //		virtual void onReceiveModuleMessage(TModuleGatewayProxyPtr
  //&senderGateway, TModuleMessagePtr &message)
  //		{
  //		}
  ...
  virtual void dispatchModuleMessage(IModuleProxy * /* senderProxy */,
    ...
    //			TModuleProxies::TAToBMap::const_iterator
    //firstSource(_ModuleProxies.getAToBMap().begin()),
    //lastSource(_ModuleProxies.getAToBMap().end()); 			for (; firstSource !=
    //lastSource && firstSource->first->getForeignModuleId() != sourceId;
    //++firstSource) {} 			nlassert(  firstSource != lastSource );
    //
    //			TPluggedModules::iterator
    //first(_PluggedModules.begin()), last(_PluggedModules.end()); 			TModuleId
    //destId = message->getAddresseeModuleProxyId(); 			for (; first != last &&
    //(*first)->getModuleId() != destId; ++first) {} 			if (first != last)
    //			{
    //				(*first)->onProcessModuleMessage(firstSource->first,
    //message);
    //			}
  }
  /***********************************************************
  ...
  void _sendModuleMessage(IModule *senderModule, TModuleId destModuleProxyId,
    ...
    TModuleProxies::TAToBMap::const_iterator first(
        ...
        last(_ModuleProxies.getAToBMap().end());
    for (;
         first != last && first->first->getModuleProxyId() != destModuleProxyId;
         ++first) {
    }
    if (first != last) {
      first->first->sendModuleMessage(senderModule, message);
      return;
    }
  ...
  void onModuleUnplugged(IModule *unpluggedModule) {
    ...
    TModuleProxies::TBToAMap::const_iterator it(
        _ModuleProxies.getBToAMap().find(CStringMapper::map(
            getGatewayName() + "/" +
            unpluggedModule->getModuleFullyQualifiedName())));

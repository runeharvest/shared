 ...
 * Constructor
 */
CUdpSock::CUdpSock(bool logging) : CSock(logging), _Bound(false) {
  // Socket creation
  createSocket(SOCK_DGRAM, IPPROTO_UDP);
}

/** Binds the socket to the specified port. Call bind() for an unreliable socket
 * if the host acts as a server and waits for messages. If the host acts as a
 * client, call sendTo(), there is no need to bind the socket.
 */
void CUdpSock::bind(uint16 port) {
  CInetAddress addr; // any IP address
  addr.setPort(port);
  bind(addr);
  setLocalAddress(); // will not set the address if the host is multihomed, use
                     // bind(CInetAddress) instead
}

/*
 * Same as bind(uint16) but binds on a specified address/port (useful when the
...
void CUdpSock::bind(const CInetAddress &addr) {
  ...
  if (::bind(_Sock, (sockaddr *)(&sockAddr), sizeOfSockAddr(sockAddr)) != 0) {
    if (_AddressFamily == AF_INET6 && addr.getAddress().isAny() &&
        !addr.getAddress().isIPv4()) {
      // Try to bind to IPv4 Any address if a dual stack listen (default) was
  ...
  _Bound = true;
  if (_Logging) {
    LNETL0_DEBUG("LNETL0: Socket %d bound at %s", _Sock,
...
void CUdpSock::sendTo(const uint8 *buffer, uint len, const CInetAddress &addr) {
  ...
  //  Send
  if (::sendto(_Sock, (const char *)buffer, len, 0, (sockaddr *)(&sockAddr),
               sizeOfSockAddr(sockAddr)) != (sint32)len) {
    throw ESocket("Unable to send datagram");
  }
  _BytesSent += len;

  if (_Logging) {
    LNETL0_DEBUG("LNETL0: Socket %d sent %d bytes to %s", _Sock, len,
                 addr.asString().c_str());
  }
...
bool CUdpSock::receive(uint8 *buffer, uint32 &len, bool throw_exception) {
  ...
  _BytesReceived += len;
  if (_Logging) {
    LNETL0_DEBUG("LNETL0: Socket %d received %d bytes from peer %s", _Sock, len,
...
bool CUdpSock::receivedFrom(uint8 *buffer, uint &len, CInetAddress &addr,
  ...
  len = ::recvfrom(_Sock, (char *)buffer, len, 0, (sockaddr *)(&sockAddr),
                   &saddrlen);

  // If an error occurs, the saddr is not valid
  // When the remote socket is closed, get sender's address to know who is
  // quitting
  ...
  _BytesReceived += len;
  if (_Logging) {
    LNETL0_DEBUG("LNETL0: Socket %d received %d bytes from %s", _Sock, len,
                 addr.asString().c_str());
  }
  return true;
}

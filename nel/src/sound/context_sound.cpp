...
/// Constructor
CContextSound::CContextSound() : _ContextSounds(0) {}
/// Destructor
...
void CContextSound::importForm(const std::string &filename,
...
bool CContextSound::isDetailed() const { return false; }
...
uint32 CContextSound::getDuration() {
  ...
  std::vector<std::pair<std::string, CSound *>>::iterator first(sounds.begin()),
      last(sounds.end());
...
void CContextSound::getSubSoundList(
    ...
    if (parseArg) {
      if (isdigit(int(*first))) {
        ...
        if (useRandom) {
          ...
          nlassertex(nbJoker < SoundContextNbArgs,
                     ("Error will trying to play ContextSound '%s'",
...
#define LM_CASE_CONTAINER_CREATOR(size)                                        \
  ...
  default:
    nlwarning("Unsuported number of context argument in context sound '%s'!",
              CStringMapper::unmap(_Name).c_str());
    return;
  }
  // cleanup macro
#undef LM_CASE_CONTAINER_CREATOR
  ...
  // ok, we have the container, now fill it with the sound
  {
    ...
    std::vector<NLMISC::TStringId>::iterator first(allSounds.begin()),
        last(allSounds.end());
    for (; first != last; ++first) {
      ...
      if (soundName.size() > _BaseName.size()) {
        ...
        if (i == _BaseName.size()) {
          // The base name is ok, check that the next char is a digit (avoid
          // conflit if some sound have a longeur base name with same beginning)
          if (soundName[i] >= '0' && soundName[i] <= '9')
            _ContextSounds->addSound(
                CAudioMixerUser::instance()->getSoundId(*first), _BaseName);
        }
      }
    }
  }

...
void IReverbEffect::CEnvironment::resize(float roomSize) {
  ...
  if (flags & NLSOUND_ENVIRONMENT_LATE_REVERB_SCALE) {
    ...
    // The value of 20.0f used in the scaling comes from the relationship
    // between the decibel (dB) scale and the linear scale used to represent the
    // intensity of sound.
    float lateReverbAdjust =
        ((flags & NLSOUND_ENVIRONMENT_DECAY_TIME_SCALE) ? 20.0f : 30.0f) *
        scaleFactorLog10;
...
// public domain worldwide.
// This software is distributed without any warranty.
CFilterParameters::CFilterParameters(float occlusion, float occlusionLfRatio,
  ...
  float obstructionGain =
      decibelsToAmplitudeRatio(obstruction * occlusionLfRatio);

...
#define NLSOUND_DISTANCE(_from, _to, _period)                                  \
  (((_to) > (_from)) ? (_to) - (_from) : (_period) + (_to) - (_from))
...
uint32 getWritePosAndSpace(uint32 &nextWritePos, uint32 playPos,
                           uint32 writePos, uint32 bufferSize);

// ******************************************************************

CSourceDSound::CSourceDSound(uint sourcename)
    : ISource(), _SourceName(sourcename) {
#if EAX_AVAILABLE == 1
  _EAXSource = 0;
#endif
  _Sample = 0;
...
void CSourceDSound::init(LPDIRECTSOUND directSound, bool useEax) {
  ...
  if (driver->countHw3DBuffers() > 0) {
    ...
    nldebug("Failed to create a 3D Hardware DirectX secondary buffer. Try 3D "
            "software one");

    if (useEax) {
      throw ESoundDriver(
          "No 3d hardware sound buffer, but EAX support requested");
    }
    // nldebug("Source: Allocating 3D buffer in software");
    desc.dwFlags = DSBCAPS_GLOBALFOCUS | DSBCAPS_LOCSOFTWARE |
                   DSBCAPS_GETCURRENTPOSITION2 | DSBCAPS_CTRL3D |
                   DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLFREQUENCY |
                   DSBCAPS_MUTE3DATMAXDISTANCE;
  ...
  if (FAILED(directSound->CreateSoundBuffer(&desc, &_SecondaryBuffer, NULL))) {
    if (useEax) {
      throw ESoundDriver("Failed to create a 3d hardware sound buffer, but EAX "
                         "support requested");
    }
    ...
    if (FAILED(
            directSound->CreateSoundBuffer(&desc, &_SecondaryBuffer, NULL))) {
  ...
  if (FAILED(_SecondaryBuffer->Lock(0, 0, &ptr, &bytes, NULL, NULL,
  ...
  if (FAILED(_SecondaryBuffer->QueryInterface(IID_IDirectSound3DBuffer,
                                              (LPVOID *)&_3DBuffer))) {
...
/// Add a buffer to the streaming queue.  A buffer of 100ms length is optimal
/// for streaming. Should be called by a thread which checks
/// countStreamingBuffers every 100ms.
void CSourceDSound::submitStreamingBuffer(IBuffer * /* buffer */) {
  throw ESoundDriverNoBufferStreaming();
}

/// Return the amount of buffers in the queue (playing and waiting). 3 buffers
/// is optimal.
...
void CSourceDSound::getCursors(TCursors &cursors) {
  _SecondaryBuffer->GetCurrentPosition((DWORD *)&cursors.PlayCursor,
                                       (DWORD *)&cursors.WriteCursor);
  ...
  */
  ...
  //	cursors.WriteSize = std::min(_UpdateCopySize, cursors.PlayCursor +
  //_SecondaryBufferSize - cursors.WriteCursor);
  cursors.WriteSize = std::min(
      _UpdateCopySize, (cursors.PlayCursor - cursors.WriteCursor) & _SizeMask);
}
...
void CSourceDSound::fillData(sint16 *dst, uint nbSample) {
  ...
  if (_Sample != 0) {
    ...
    // nldebug("Filling from %p to %p (%p sample, %p bytes)", dst, dst+nbSample,
    // nbSample, nbSample*2);
    ...
    case Mono8:
      data8 = (sint8 *)data;
      data8 += _SampleOffset;
      //	nldebug(" with data (Mono8) from %p to %p (sample : base = %p,
      //size = %p)", data8, data8+nbSample, data, _SampleSize);
    ...
    case Mono16ADPCM:
      ...
      // nldebug("Filling ADPCM : %p => %p with %p to %p", dst, dst+nbSample,
      // dataAdpcm - (uint8*)data, dataAdpcm + (nbSample/2)-(uint8*)data);
      // nldebug(" with data (Mono16ADPCM) from %p to %p (sample : base = %p,
      // size = %p)", dataAdpcm, dataAdpcm+(nbSample/2), data, _SampleSize*2);
    ...
    case Mono16:
      ...
      // nldebug("Filling Mono16 : %p => %p with %p to %p", dst, dst+nbSample,
      // data16 - (sint16*)data, data16 + (nbSample)-(sint16*)data);
      //	nldebug(" with data (Mono16) from %p to %p (sample : base = %p,
      //size = %p)", data16, data16+nbSample, data, _SampleSize/2);
    ...
    case Stereo8:
      ...
      for (i = 0; i < nbSample; ++i) {
        dst[i] =
            (sint16(data8[i * 2]) * 128) + (sint16(data8[i * 2 + 1]) * 128);
      }
    ...
    */
    ...
    nldebug("Filling : NO DATA from %p to %p (%p sample, %p bytes)", dst,
            dst + nbSample, nbSample, nbSample * 2);
...
void CSourceDSound::fillSilence(const TLockedBufferInfo &lbi, int nbSample) {
  ...
  uint tmp = size;
  sint16 *ptr = lbi.Ptr1;
  //	nldebug("Silencing from %p to %p (%p sample, %p bytes)", ptr, ptr+size,
  //size, size*2);
...
bool CSourceDSound::play() {
  ...
  if (lock(_FillOffset, cursors.WriteSize, lbi)) {
    ...
    uint dataToFill =
        std::min(uint(cursors.WriteSize / 2), _SampleSize - _SampleOffset);
    ...
    if (_Loop) {
      ...
      if (_SampleOffset == _SampleSize) {
        // begin to write silence, but stil in play state until all sample are
        // played
        //				_State = source_silencing;
...
void CSourceDSound::setLooping(bool l) { _Loop = l; }

// ******************************************************************

bool CSourceDSound::getLooping() const { return _Loop; }
...
bool CSourceDSound::play()
{
    ...
    DBGPOS(("[%p] PLAY: Enter, buffer state = %u", this,
_SecondaryBufferState)); switch (_SecondaryBufferState)
    {
...
bool CSourceDSound::needsUpdate() {
  return _State == source_silencing || _State == source_playing ||
         _State == source_swap_pending;
}
...
bool CSourceDSound::update() {
  ...
  //	if (_FillOffset < cursors.WriteCursor && _FillOffset
  //>cursors.PlayCursor)
  if (fillSize + margin > _SecondaryBufferSize) {
    // arg !
    nlwarning(
        "FillOffset is between play and write cursor (P = %p F = %p W = %p!",
        cursors.PlayCursor, _FillOffset, cursors.WriteCursor);
    _FillOffset = cursors.WriteCursor;
    _FillOffset = (_FillOffset + 3) & 0xfffffffC;
    _SilenceWriten = 0;
    nlassert(_FillOffset < _SecondaryBufferSize);
  ...
  /*	nldebug("Play = %p, Write = %p, Fill = %p, FillSize = %p, Margin = %p,
     Advance = %p", cursors.PlayCursor, cursors.WriteCursor, _FillOffset,
     fillSize, margin, advance);
  */
  ...
  */
  //	nldebug("Cursors : Play = %p, Write = %p, Fill = %p",
  //cursors.PlayCursor, cursors.WriteCursor, _FillOffset);
  ...
  if (writeSize >= _UpdateCopySize) {
    ...
    if (lock(_FillOffset, writeSize, lbi)) {
      ...
      if (_State == source_playing) {
        nlassert(_SampleOffset <= _SampleSize);
        uint32 updateSize =
            min(_SampleSize - _SampleOffset, uint(writeSize / 2));
        ...
        // the sample has been changed but not replayed yet ? so we 'stop' the
        // old buffer
        ...
        uint32 updateSize =
            min(writeSize, _SecondaryBufferSize - _SilenceWriten);
    ...
    case NL_DSOUND_SILENCED:
        {
            ...
            space = getWritePosAndSpace(_NextWritePos, playPos, writePos,
_SecondaryBufferSize);
...
void CSourceDSound::setPos(const NLMISC::CVector &pos, bool deferred) {
  ...
  if (_3DBuffer != NULL) {
    if (_3DBuffer->SetPosition(pos.x, pos.z, pos.y,
                               deferred ? DS3D_DEFERRED : DS3D_IMMEDIATE) !=
        DS_OK) {
      nlwarning("SetPosition failed");
    } else {
      // nlwarning ("%p set source NEL(p:%.2f/%.2f/%.2f) DS(p:%.2f/%.2f/%.2f)",
      // this, pos.x, pos.y, pos.z, pos.x, pos.z, pos.y);
    }
  }
}
...
const NLMISC::CVector &CSourceDSound::getPos() const { return _Pos; }

// ******************************************************************

void CSourceDSound::setVelocity(const NLMISC::CVector &vel, bool deferred) {
  if (_3DBuffer != NULL) {
    if (_3DBuffer->SetVelocity(vel.x, vel.z, vel.y,
                               deferred ? DS3D_DEFERRED : DS3D_IMMEDIATE) !=
        DS_OK) {
...
void CSourceDSound::setDirection(const NLMISC::CVector &dir) {
  if (_3DBuffer != 0) {
    if (_3DBuffer->SetConeOrientation(dir.x, dir.z, dir.y, DS3D_DEFERRED) !=
        DS_OK) {
      nlwarning("SetConeOrientation failed (x=%.2f, y=%.2f, z=%.2f)", dir.x,
                dir.y, dir.z);
    } else {
      // nlwarning ("NLSOUND: %p set source direction NEL(p:%.2f/%.2f/%.2f)
      // DS(p:%.2f/%.2f/%.2f)", this, dir.x, dir.y, dir.z, dir.x, dir.z, dir.y);
    }
  }
}
...
void CSourceDSound::setGain(float gain) {
  ...
  /*
  if ((_SecondaryBuffer != 0) && (_SecondaryBuffer->SetVolume(_Volume) !=
  DS_OK))
  {
 ...
 * Get the gain
 */
float CSourceDSound::getGain() const { return _Gain; }
...
void CSourceDSound::setPitch(float coeff) {
  ...
  if ((_Sample != 0) && (_SecondaryBuffer != 0)) {
    ...
    if (_SecondaryBuffer->SetFrequency(_SampleRate) != DS_OK) {
      //			nlwarning("SetFrequency failed (buffer freq=%d,
      //NeL freq=%.5f, DSound freq=%d)", freq, coeff, newfreq);
...
bool CSourceDSound::getSourceRelativeMode() const { return _PosRelative; }

// ******************************************************************
void CSourceDSound::setMinMaxDistances(float mindist, float maxdist,
                                       bool deferred) {
  if (_3DBuffer != 0) {
    if (_3DBuffer->SetMinDistance(std::max(DS3D_DEFAULTMINDISTANCE, mindist),
                                  deferred ? DS3D_DEFERRED : DS3D_IMMEDIATE) !=
        DS_OK) {
      nlwarning("SetMinDistance (%f) failed", mindist);
    }
    if (_3DBuffer->SetMaxDistance(std::min(DS3D_DEFAULTMAXDISTANCE, maxdist),
                                  deferred ? DS3D_DEFERRED : DS3D_IMMEDIATE) !=
        DS_OK) {
...
void CSourceDSound::updateVolume(const NLMISC::CVector &listener) {
  if (!CSoundDriverDSound::instance()->getOption(
  ...
  } else // manual rolloff
  {
    ...
    sint32 volumeDB =
        ISource::computeManualRollOff(_Volume, DSBVOLUME_MIN, DSBVOLUME_MAX,
                                      _Alpha, sqrdist, mindist, maxdist);
...
void CSourceDSound::setCone(float innerAngle, float outerAngle,
                            float outerGain) {
  if (_3DBuffer != 0) {
    // Set the cone angles
...
void CSourceDSound::getCone(float &innerAngle, float &outerAngle,
...
//	{
//		_EAXSource =
//CSoundDriverDSound::instance()->createPropertySet(this);
//	}
//	if ( _EAXSource != NULL )
//	{
//		H_AUTO(NLSOUND_EAXPropertySet_Set)
//		HRESULT res = _EAXSource->Set( DSPROPSETID_EAX_BufferProperties,
//prop, NULL, 0, value, valuesize ); 		if (res != S_OK)
//		{
 ...
 *  alpha = 1.0: the volume will decrease linearly between 1.0 and 0.0 (linear
 *scale) alpha = -1.0: the volume will decrease inversely with the distance
 *(1/dist). This is the default used by DirectSound/OpenAL
 *
 *  For any other value of alpha, an interpolation is be done between the two
 *  adjacent curves. For example, if alpha equals 0.5, the volume will be
 *halfway between the linear dB curve and the linear amplitude curve.
 */
void CSourceDSound::setAlpha(double a) { _Alpha = a; }
...
bool CSourceDSound::getDirect() const { return true; }
...
bool CSourceDSound::isDirectFilterEnabled() const { return false; }

/// Set the filter parameters for the direct channel
void CSourceDSound::setDirectFilter(TFilter /*filterType*/,
                                    float /*lowFrequency*/,
                                    float /*highFrequency*/,
                                    float /*passGain*/) {}

/// Get the filter parameters for the direct channel
void CSourceDSound::getDirectFilter(TFilter &filterType, float &lowFrequency,
                                    float &highFrequency,
                                    float &passGain) const {
...
float CSourceDSound::getDirectFilterPassGain() const { return 0.0f; }

/// Set the effect send for this source, NULL to disable. [IEffect], default:
/// NULL
void CSourceDSound::setEffect(IReverbEffect * /* reverbEffect */) {}

/// Get the effect send for this source
IEffect *CSourceDSound::getEffect() const { return NULL; }
...
bool CSourceDSound::isEffectFilterEnabled() const { return false; }

/// Set the filter parameters for the effect channel
void CSourceDSound::setEffectFilter(TFilter /*filterType*/,
                                    float /*lowFrequency*/,
                                    float /*highFrequency*/,
                                    float /*passGain*/) {}

/// Get the filter parameters for the effect channel
void CSourceDSound::getEffectFilter(TFilter &filterType, float &lowFrequency,
                                    float &highFrequency,
                                    float &passGain) const {
  filterType = FilterLowPass;
  lowFrequency = NLSOUND_DEFAULT_FILTER_PASS_LF;
...
float CSourceDSound::getEffectFilterPassGain() const { return 0.0f; }

// ******************************************************************

IBuffer *CSourceDSound::getBuffer() { return _Sample; }

// ******************************************************************

bool CSourceDSound::lock(uint32 offset, uint32 size, TLockedBufferInfo &lbi) {
  HRESULT hr = _SecondaryBuffer->Lock(offset, size, (LPVOID *)&lbi.Ptr1,
                                      (DWORD *)&lbi.Size1, (LPVOID *)&lbi.Ptr2,
                                      (DWORD *)&lbi.Size2, 0);
  ...
  if (hr == DSERR_BUFFERLOST) {
    ...
    if (FAILED(_SecondaryBuffer->Lock(offset, size, (LPVOID *)&lbi.Ptr1,
                                      (DWORD *)&lbi.Size1, (LPVOID *)&lbi.Ptr2,
                                      (DWORD *)&lbi.Size2, 0))) {
...
bool CSourceDSound::lock(uint32 writePos, uint32 size, uint8* &ptr1, DWORD
&bytes1, uint8* &ptr2, DWORD &bytes2)
{
    HRESULT hr = _SecondaryBuffer->Lock(writePos, size, (LPVOID*) &ptr1,
&bytes1, (LPVOID*) &ptr2, &bytes2, 0);
    ...
    if (hr == DSERR_BUFFERLOST)
    {
        ...
        if (FAILED(_SecondaryBuffer->Lock(_NextWritePos, _UpdateCopySize,
(LPVOID*) &ptr1, &bytes1, (LPVOID*) &ptr2, &bytes2, 0)))
        {
            nlwarning("Lock failed (2)");
            return false;
        }
...
*/
/*
void CSourceDSound::copySampleTo16BitsTrack(void *dst, void *src, uint nbSample,
...
************************************************************************/
/*
uint32 getWritePosAndSpace(uint32 &nextWritePos, uint32 playPos, uint32
writePos, uint32 bufferSize)
{
    uint32 space;
    if (playPos < writePos) //_NextWritePos)
    {
        ...
        {
            // We have a problem here because our write pointer is in the
forbiden zone
...
bool CSourceDSound::fill()
{
    ...
    space = getWritePosAndSpace(nextWritePos, playPos, writePos,
_SecondaryBufferSize);
    ...
    uint32 available = (_BytesWritten < _BufferSize) ? _BufferSize -
_BytesWritten : 0;
...
//		CFastMem::memcpy(ptr1, data + _BytesWritten, bytes1);
        copySampleTo16BitsTrack(ptr1, data + _BytesWritten, bytes1/2,
sampleFormat); _BytesWritten += bytes1; bytes -= bytes1;

        if (ptr2)
        {
            if (bytes > 0)
            {
//				CFastMem::memcpy(ptr2, data + _BytesWritten,
bytes); copySampleTo16BitsTrack(ptr2, data + _BytesWritten, bytes/2,
sampleFormat); _BytesWritten += bytes;
            }
            ...
            if (bytes < bytes2)
            {
                if (_Loop)
                {
                    ...
                    copySampleTo16BitsTrack(ptr2 + bytes, data, (bytes2 -
bytes)/2, sampleFormat); _BytesWritten = bytes2 - bytes;
                }
                else
                {
...
//			CFastMem::memcpy(ptr1, data + _BytesWritten, bytes);
            copySampleTo16BitsTrack(ptr1, data + _BytesWritten, bytes/2,
sampleFormat); _BytesWritten += bytes;
        }
...
//			CFastMem::memcpy(ptr1 + bytes, data, bytes1 - bytes);
            copySampleTo16BitsTrack(ptr1 + bytes, data, (bytes1 - bytes)/2,
sampleFormat); _BytesWritten = bytes1 - bytes;

            if (ptr2)
            {
//				CFastMem::memcpy(ptr2, data + _BytesWritten,
bytes2); copySampleTo16BitsTrack(ptr2, data + _BytesWritten, bytes2 / 2,
sampleFormat); _BytesWritten += bytes2;
            }
    ...
    if (_BytesWritten == _BufferSize)
    {
        ...
        else
        {
            ...
            _EndState = (playPos > _EndPosition)? NL_DSOUND_TAIL1 :
NL_DSOUND_TAIL2;

            DBGPOS(("[%p] FILL: SILENCING", this));
            DBGPOS(("[%p] FILL: ENDSTATE=%d, E=%d, P=%d", this, (int) _EndState,
_EndPosition, playPos));
        }
    }
    ...
    DBGPOS(("[%p] FILL: P=%d, W=%d, NW=%d, SZ=%d, BW=%d, S=%d, B=%d", this,
playPos, writePos, _NextWritePos, _BufferSize, _BytesWritten, _SilenceWritten,
bytes1 + bytes2));
    ...
    _UnlockTime += CTime::ticksToSecond(CTime::getPerformanceTime() -
startUnlock); _CopyCount++; #endif
...
*/
/*	// Don't bother if the number of samples that can be written is too
small. if (space < _UpdateCopySize)
    {
        return false;
    }
    ...
    {
        // The play cursor wrapped around the buffer and is now before the end
position _EndState = NL_DSOUND_TAIL2; DBGPOS(("[%p] FILL: ENDSTATE=%d, E=%d,
P=%d", this, (int) _EndState, _EndPosition, playPos));
    }
    ...
    DBGPOS(("[%p] SLNC: P=%d, W=%d, NW=%d, SZ=%d, BW=%d, S=%d, B=%d", this,
playPos, writePos, _NextWritePos, _BufferSize, _BytesWritten, _SilenceWritten,
bytes1 + bytes2));
...
/**
 *	Calculate the size of the crossfade and set the pointer in the sample
 *buffer to the sample that comes after the write cursor in the DirectSound
 *buffer. This method also returns the number of samples that have been written
 *after the write cursor.
 *
 */
/*
void CSourceDSound::getFadeOutSize(uint32 writePos, uint32 &xfadeSize, sint16*
&in1, uint32 &writtenTooMuch)
{
    // The number of samples over which we will do the crossfade
    xfadeSize = _XFadeSize;


    // The tricky part of this method is to figger out how many samples of the
old
    // buffer were written after the write cursor and figger our with what
position
    // in the old buffer the write cursor corresponds. We have to consider the
following
    // cases:
    //
    // - the old buffer just looped,
    // - the old buffer is finished, but stil has samples in the DirectSound
buffer
    // - the default case, i.e. the old buffer is playing somewhere in the
middle.
    //

    // How many bytes did we write after the write position?

    writtenTooMuch = NLSOUND_DISTANCE(writePos, _NextWritePos,
_SecondaryBufferSize);
    ...
    if (_SilenceWritten > 0)
    {
        ...
        DBGPOS(("[%p] FADE: END, TOO=%d, S=%d, F=%d", this, writtenTooMuch,
_SilenceWritten, xfadeSize));
    }
    ...
    else if (writtenTooMuch >= _BytesWritten)
    {
        ...
        DBGPOS(("[%p] FADE: LOOPED, TOO=%d, F=%d", this, writtenTooMuch,
xfadeSize));

    }

    // This is the default case. Simply fade from the previous to the next
buffer.
    ...
    else
    {
        ...
        DBGPOS(("[%p] FADE: STD, TOO=%d, F=%d", this, writtenTooMuch,
xfadeSize));
    }
}
*/
...
void CSourceDSound::crossFade()
{
    ...
    if (FAILED(_SecondaryBuffer->Lock(writePos, bytes, (LPVOID*) &ptr1, &bytes1,
(LPVOID*) &ptr2, &bytes2, 0)))
    {
        LeaveCriticalSection(&_CriticalSection);
        throw ESoundDriver("Failed to lock the DirectSound secondary buffer");
    }
...
#define MIX 1
    ...
    // In the first case, the cross fade is completely contained in the first
buffer
    // pointed to by ptr1.
...
#if MIX
//		CFastMem::memcpy(ptr1 + xfadeByteSize, data2 + xfadeByteSize,
bytes1 - xfadeByteSize); copySampleTo16BitsTrack(ptr1 + xfadeByteSize, data2 +
xfadeByteSize, (bytes1 - xfadeByteSize)/2, sampleFormat);

        _BytesWritten = bytes1;
#else
//		CFastMem::memcpy(ptr1 + xfadeByteSize, data2, bytes1 -
xfadeByteSize); copySampleTo16BitsTrack(ptr1 + xfadeByteSize, data2, (bytes1 -
xfadeByteSize)/2, sampleFormat); _BytesWritten = bytes1 - xfadeByteSize; #endif

        if (ptr2)
        {
            //CFastMem::memcpy(ptr2, data2 + _BytesWritten, bytes2);
            copySampleTo16BitsTrack(ptr2, data2 + _BytesWritten, bytes2/2,
sampleFormat); _BytesWritten += bytes2;
        }

    }

    // In the second case, the cross fade stretches over the first and the
second buffers. else
    {
        ...
        if (ptr2)
        {
            ...
            // Copy remaining samples
#if MIX
//			CFastMem::memcpy(ptr2 + 2 * k, data2 + _BytesWritten + 2
* k, bytes2 - 2 * k); copySampleTo16BitsTrack(ptr2 + 2 * k, data2 +
_BytesWritten + 2 * k, (bytes2 - 2 * k)/2, sampleFormat); _BytesWritten +=
bytes2; #else
            //CFastMem::memcpy(ptr2 + 2 * k, data2, bytes2 - 2 * k);
            copySampleTo16BitsTrack(ptr2 + 2 * k, data2, (bytes2 - 2 * k)/2,
sampleFormat); _BytesWritten = bytes2 - 2 * k; #endif
        }
    }
    ...
    DBGPOS(("[%p] XFADE", this));
    DBGPOS(("[%p] P=%d, W=%d, NW=%d, SZ=%d, BW=%d, B=%d", this, playPos,
writePos, _NextWritePos, _BufferSize, _BytesWritten, bytes1 + bytes2));
    ...
    _MaxSwapTime = (_LastSwapTime > _MaxSwapTime) ? _LastSwapTime :
_MaxSwapTime; _MinSwapTime = (_LastSwapTime < _MinSwapTime) ? _LastSwapTime :
_MinSwapTime; _SwapCount++; #endif
...
void CSourceDSound::fadeOut()
{
    ...
    if (FAILED(_SecondaryBuffer->Lock(writePos, _SwapCopySize, (LPVOID*) &ptr1,
&bytes1, (LPVOID*) &ptr2, &bytes2, 0)))
    {
        throw ESoundDriver("Failed to lock the DirectSound secondary buffer");
    }
    ...
    // In the first case, the fade out is completely contained in the first
buffer
    // pointed to by ptr1.
    if (xfadeByteSize < bytes1)
    {
    ...
    // In the second case, the fade out stretches over the first and the second
buffers. else
    {
    ...
    DBGPOS(("[%p] FDOU: ENDSTATE=%d, E=%d, P=%d", this, (int) _EndState,
_EndPosition, playPos));


    DBGPOS(("[%p] FDOU: P=%d, W=%d, NW=%d, SZ=%d, BW=%d, S=%d, B=%d", this,
playPos, writePos, _NextWritePos, _BufferSize, _BytesWritten, _SilenceWritten,
bytes1 + bytes2));
...
//	setPitch(_SecondaryBuffer->GetFrequency());
    ...
    uint8* data = ((CBufferDSound*) _Buffer)->getData();
    uint32 available = (_BytesWritten < _BufferSize) ? _BufferSize -
_BytesWritten : 0; uint32 bytes = NLSOUND_MIN(_SwapCopySize, available);
...
sampleFormat); _BytesWritten += bytes1; bytes -= bytes1;
        ...
        if (ptr2)
        {
            if (bytes > 0)
            {
                ...
                copySampleTo16BitsTrack(ptr2, data + _BytesWritten, bytes/2,
sampleFormat); _BytesWritten += bytes;
            }
...
sampleFormat); _BytesWritten = bytes1 - bytes;
            ...
            if (ptr2)
            {
                ...
                copySampleTo16BitsTrack(ptr2, data + _BytesWritten, bytes2/2,
sampleFormat); _BytesWritten += bytes2;
            }
        }
        else
...
NL_DSOUND_TAIL2;
            ...
            DBGPOS(("[%p] FDIN: ENDSTATE=%d, E=%d, P=%d", this, (int) _EndState,
_EndPosition, playPos));
        }
    }
    ...
    DBGPOS(("[%p] FDIN: P=%d, W=%d, NW=%d, SZ=%d, BW=%d, S=%d, B=%d", this,
playPos, writePos, _NextWritePos, _BufferSize, _BytesWritten, _SilenceWritten,
bytes1 + bytes2));

 ...
 *   - the buffer still contains samples but silence is being written
 * (silencing),
...
/*enum TSourceDSoundBufferState
{
    ...
    NL_DSOUND_FILLING,
    /// The buffer still contains samples but silence is being written
(silencing), NL_DSOUND_SILENCING,
    /// The buffer contains no samples but only silence (silenced)
    NL_DSOUND_SILENCED
} ;
/*

/** The state of the source as experienced by the user: playing, paused, and
stopped. */
/*
...
/** To figger out whether the sound device has played all the samples in the
 * buffer, the position of the play cursor is traced relatively to the position
 * of the last sample in the buffer.
 */
/*
...
public:
   ...
   * relative mode is on, otherwise the source is considered as a 3D source. Use
   * submitStreamingBuffer for streaming.
   */
  virtual void setStaticBuffer(IBuffer *buffer);
  /// Return the buffer, or NULL if streaming is used. Not available for
  /// streaming.
  ...
  //@{
  /// Set looping on/off for future playbacks (default: off), not available for
  /// streaming
   ...
   * 3D mode -> 3D position
   * st mode -> x is the pan value (from left (-1) to right (1)), set y and z to
   * 0
   */
   ...
   * pitch shift of one octave. 0 is not a legal value.
   */
  virtual void setPitch(float pitch);
  /// Get the pitch
  virtual float getPitch() const;
  /// Set the source relative mode. If true, positions are interpreted relative
  ...
  /// Set the cone angles (in radian) and gain (in [0 , 1]) (default: 2PI, 2PI,
  /// 0)
  virtual void setCone(float innerAngle, float outerAngle, float outerGain);
  /// Get the cone angles (in radian)
  virtual void getCone(float &innerAngle, float &outerAngle,
  ...
  virtual void setDirectFilter(TFilter filter, float lowFrequency,
                               float highFrequency, float passGain);
  /// Get the filter parameters for the direct channel
  virtual void getDirectFilter(TFilter &filterType, float &lowFrequency,
  ...
  virtual void setEffectFilter(TFilter filter, float lowFrequency,
                               float highFrequency, float passGain);
  /// Get the filter parameters for the effect channel
  virtual void getEffectFilter(TFilter &filterType, float &lowFrequency,
                               float &highFrequency, float &passGain) const;
...
private:
  void copySampleTo16BitsTrack(void *dst, void *src, uint nbSample,
  ...
  // Utility function that locks the DirectSound buffer and restores it if it
  // was lost.
  //	bool					lock(uint32 writePos, uint32
  //size, uint8* &ptr1, DWORD &bytes1, uint8* &ptr2, DWORD &bytes2);
  bool lock(uint32 writePos, uint32 size, TLockedBufferInfo &lockedInfo);

  // Utility function that unlocks the DirectSound buffer
  //	bool					unlock(uint8* ptr1, DWORD
  ...
  //	void					getFadeOutSize(uint32 writePos,
  //uint32& xfadeSize, sint16* &in1, uint32 &writtenTooMuch);

  // Fill the buffer with fresh samples. Should be called inside the critical
  // zone.
  bool fill();
  // Fill the buffer with sparkling silence. Should be called inside the
  // critical zone.
  bool silence();
  // Do a cross fade between the current buffer and the buffer stored in the
  // _SwapBuffer variable. Call inside the critical zone.
  void crossFade();
  ...
  // The amount of silence written (in bytes) after the end of the current sound
  // buffer
  ...
  // The state of the buffer to reach the end of the audio samples. To flag the
  // buffer as STOPPED, we have to make sure all the samples in the buffer are
  // played. The play cursor in the DirectSound buffer is inspected and when it
  // crosses the position of the last sample written (_EndPosition) the buffer
  // is flagged as stopped. The _EndState field is used to keep a trace of the
  // whereabouts of the play cursor.
...
public:
  ...
  static double getTestAverage() {
    return (_SwapCount > 0) ? 1000.0f * _TotalSwapTime / _SwapCount : 0.0;
  };

  static double getAveragePosTime() {
    return (_CopyCount > 0) ? 1000.0f * _PosTime / _CopyCount : 0.0;
  };
  static double getAverageLockTime() {
    return (_CopyCount > 0) ? 1000.0f * _LockTime / _CopyCount : 0.0;
  };
  static double getAverageCopyTime() {
    return (_CopyCount > 0) ? 1000.0f * _CopyTime / _CopyCount : 0.0;
  };
  static double getAverageUnlockTime() {
    return (_CopyCount > 0) ? 1000.0f * _UnlockTime / _CopyCount : 0.0;
  };
  static double getAverageCumulTime() {
    return (_CopyCount > 0)
               ? 1000.0f * (_PosTime + _LockTime + _CopyTime + _UnlockTime) /
                     _CopyCount
               : 0.0;
  };
  static uint getAverageUpdateSize() {
    return (_CopyCount > 0) ? (uint)(_TotalUpdateSize / _CopyCount) : 0;
  };
  ...
  static double getAverageUpdateTime() {
    return (_UpdateCount > 0) ? 1000.0f * _TotalUpdateTime / _UpdateCount : 0.0;
  };

  static double getTotalUpdateTime() { return 1000.0f * _TotalUpdateTime; };
  static double getUpdateBytesPerMsec() {
    return (_UpdateCount > 0) ? _TotalUpdateSize / _TotalUpdateTime / 1000.0
                              : 0.0;
  }

...
LRESULT CALLBACK CSoundDriverCreateWindowProc(HWND hWnd, UINT message,
...
__declspec(dllexport) ISoundDriver *NLSOUND_createISoundDriverInstance
  ...
  CSoundDriverWnd =
      CreateWindow((LPSTR) "CSoundDriver", (LPSTR) "CSoundDriver",
                   WS_OVERLAPPEDWINDOW, CW_USEDEFAULT, CW_USEDEFAULT, 400, 300,
                   (HWND)NULL, (HMENU)NULL, CSoundDriverDllHandle, (LPSTR)NULL);
  ...
  /*	// install the NeL Io routine
      LPMMIOPROC ret = mmioInstallIOProc(mmioStringToFOURCC("NEL_", 0),
     (LPMMIOPROC)NelIOProc, MMIO_INSTALLPROC); nlassert(ret != 0);
  */
...
LPKSPROPERTYSET CSoundDriverDSound::createPropertySet(CSourceDSound *source) {
  ...
  // some checking code
  {
    if (propertySet != 0) {
      ...
      uint i;
      for (i = DSPROPERTY_EAXLISTENER_NONE; i <= DSPROPERTY_EAXLISTENER_FLAGS;
           ++i) {
        ULONG ulSupport = 0;
        propertySet->QuerySupport(DSPROPSETID_EAX_ListenerProperties, i,
                                  &ulSupport);
        if ((ulSupport & (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) !=
            (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) {
          //					nlwarning("CSoundDriverDSound::createPropertySet
          //: listener property %s not supported", listenerProperties[i]);
        }
      }

      char *bufferProperties[] = {"DSPROPERTY_EAXBUFFER_NONE",
      ...
      for (i = DSPROPERTY_EAXBUFFER_NONE; i <= DSPROPERTY_EAXBUFFER_FLAGS;
           ++i) {
        ULONG ulSupport = 0;
        propertySet->QuerySupport(DSPROPSETID_EAX_BufferProperties, i,
                                  &ulSupport);
        if ((ulSupport & (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) !=
            (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) {
          //					nlwarning("CSoundDriverDSound::createPropertySet
          //: buffer property %s not supported", bufferProperties[i]);
        }
      }
    } else {
      nlwarning("CSoundDriverDSound::createPropertySet : propertie set not "
                "available !");
    }
  }
...
void CSoundDriverDSound::initDevice(const std::string &device,
...
#if EAX_AVAILABLE
                                | OptionReverbEffect | OptionFilterEffect
#endif
                                | OptionSoftwareBuffer | OptionManualRolloff |
  ...
  // set the options
  _Options =
      (TSoundOptions)(((sint)options & supportedOptions) | forcedOptions);
  ...
  if (getOption(OptionReverbEffect) || getOption(OptionFilterEffect)) {
    if (EAXDirectSoundCreate8(NULL, &_DirectSound, NULL) != DS_OK) {
      throw ESoundDriver(
          "Failed to create the DirectSound object from EAX proxy funtion");
    }
  } else
#endif
  {
  ...
  if (_DirectSound->SetCooperativeLevel(CSoundDriverWnd, DSSCL_PRIORITY) !=
      DS_OK) {
  ...
  if (res != DS_OK && res != DS_NO_VIRTUALIZATION) {
    ...
    nlwarning(
        "Primary buffer: Failed to create a buffer with 3D capabilities.");
    ...
    if (countHw2DBuffers() > 0) {
      ...
      desc.dwFlags =
          DSBCAPS_PRIMARYBUFFER | DSBCAPS_LOCHARDWARE | DSBCAPS_CTRLVOLUME;
    } else {
      nldebug("Primary buffer: Allocating 2D buffer in software");
      desc.dwFlags =
          DSBCAPS_PRIMARYBUFFER | DSBCAPS_LOCSOFTWARE | DSBCAPS_CTRLVOLUME;
    }

    if (_DirectSound->CreateSoundBuffer(&desc, &_PrimaryBuffer, NULL) !=
        DS_OK) {
  ...
  if ((_Caps.dwMinSecondarySampleRate > 22050) &&
      (22050 > _Caps.dwMaxSecondarySampleRate)) {
  ...
  /*	LPVOID ptr;
      ...
      HRESULT hr = _PrimaryBuffer->Lock(0, 0, &ptr, &bytes, NULL, NULL,
     DSBLOCK_ENTIREBUFFER); if (FAILED(hr))
      {
          switch (hr)
          {
          case DSERR_BUFFERLOST:
              throw ESoundDriver("Failed to lock the DirectSound primary buffer:
     DSERR_BUFFERLOST"); case DSERR_INVALIDCALL: throw ESoundDriver("Failed to
     lock the DirectSound primary buffer: DSERR_INVALIDCALL"); case
     DSERR_INVALIDPARAM: throw ESoundDriver("Failed to lock the DirectSound
     primary buffer: DSERR_INVALIDPARAM"); case DSERR_PRIOLEVELNEEDED: throw
     ESoundDriver("Failed to lock the DirectSound primary buffer:
     DSERR_PRIOLEVELNEEDED"); default: throw ESoundDriver("Failed to lock the
     DirectSound primary buffer: unknown error");
  ...
  */
  ...
  catch (const ESoundDriver& e)
  {
      ...
      // The caps told me there were 31 buffers available. In reality, there
  were
      // only 30 available because WinAmp was using one. Somehow DirectSound
  didn't
      // notice. So when creating buffer 31, an exception was thrown.
      // If something like this happens, don't bother and go on with the buffers
  we've
...
*/
  ...
  _TimerID =
      timeSetEvent(_TimerPeriod, 0, &CSoundDriverDSound::TimerCallback,
                   (DWORD_PTR)this, TIME_CALLBACK_FUNCTION | TIME_PERIODIC);
...
/// Return options that are enabled (including those that cannot be disabled on
/// this driver).
...
void CSoundDriverDSound::writeProfile(string &out) {
  ...
  out +=
      "\tBuffer size: " + toString((int)CSourceDSound::_SecondaryBufferSize) +
      "\n";
  ...
  out +=
      "\t3d hw buffers: " + toString((uint32)caps.dwMaxHw3DAllBuffers) + "\n";
  ...
  out += "\tUpdate time source --- " +
         toString(CSourceDSound::getAverageUpdateTime()) + "\n";
  ...
  out += "\tUpdate num sources --- " +
         toString((int)getAverageUpdateSources()) + "\n";
  out += "\tUpdate byte size --- " +
         toString(CSourceDSound::getAverageUpdateSize()) + "\n";
  out += "\tSwap time --- " + toString(CSourceDSound::getTestAverage()) + "\n";
  out += "\tSrc --- " + toString(countPlayingSources()) + "\n";
#endif
}

void CALLBACK CSoundDriverDSound::TimerCallback(UINT /* uID */, UINT /* uMsg */,
                                                DWORD_PTR dwUser,
                                                DWORD_PTR /* dw1 */,
                                                DWORD_PTR /* dw2 */) {
  ...
  if (now - lastUpdate > _TimerPeriod * 2) {
    //		nlwarning("CSoundDriverDSound::TimerCallback : no update since
    //%u millisec (nominal update = %u", uint32(now-lastUpdate),
    //uint32(_TimerPeriod));
  } else {
  ...
  */
  {
    ...
    if (last > _TimerPeriod / 2) {
      nlwarning("CSoundDriverDSound::TimerCallback : update took %u millisec",
                (uint32)last);
    }
  }
...
#if NLSOUND_PROFILE
  _TotalUpdateTime +=
      1000.0 * CTime::ticksToSecond(CTime::getPerformanceTime() - tnow);
  _UpdateCount++;
#endif
}
...
IListener *CSoundDriverDSound::createListener() {
  ...
  if (FAILED(_PrimaryBuffer->QueryInterface(IID_IDirectSound3DListener,
                                            (LPVOID *)&dsoundListener))) {
...
//// ******************************************************************
// bool CSoundDriverDSound::readWavBuffer( IBuffer *destbuffer, const
// std::string &name, uint8 *wavData, uint dataSize)
//{
//	return ((CBufferDSound*) destbuffer)->readWavBuffer(name, wavData,
//dataSize);
// }
//
// bool CSoundDriverDSound::readRawBuffer( IBuffer *destbuffer, const
// std::string &name, uint8 *rawData, uint dataSize, TSampleFormat format,
// uint32 frequency)
//{
//	return ((CBufferDSound*) destbuffer)->readRawBuffer(name, rawData,
//dataSize, format, frequency);
// }
...
uint CSoundDriverDSound::countTimerIntervals() { return 1024; }
...
void CSoundDriverDSound::printDriverInfo(FILE *fp) {
  ...
  fprintf(fp,
          ...
          "copy size: %d\n"
          "swap size: %d\n",
          CSourceDSound::_SecondaryBufferSize, CSourceDSound::_UpdateCopySize,
...
void CSoundDriverDSound::startBench() { NLMISC::CHTimer::startBench(); }
void CSoundDriverDSound::endBench() { NLMISC::CHTimer::endBench(); }

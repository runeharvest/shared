...
public:
   ...
   * not implemented. If the song has no name, result is filled with the
   * filename. \param filepath path to file, CPath::lookup done by driver \param
   * artist returns the song artist (empty if not available) \param title
   * returns the title (empty if not available)
   */
...
private:
  ...
  static void CALLBACK TimerCallback(UINT uID, UINT uMsg, DWORD_PTR dwUser,
  ...
  /// The EAX support is requested and accepted (ie, there is enougth hardware
  /// 3D buffer)
  bool _UseEAX;
...
public:
  ...
  double getAverageUpdateTime() {
    return (_UpdateCount) ? _TotalUpdateTime / _UpdateCount : 0.0;
  }
  uint32 getAverageUpdateSources() {
    return (_UpdateExec) ? _UpdateSources / _UpdateExec : 0;
  }
  double getUpdatePercentage() {
    return (_UpdateCount) ? (double)_UpdateExec / (double)_UpdateCount : 0;
  }
#endif
};

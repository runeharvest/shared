...
CListenerDSound::CListenerDSound(
  ...
  if (_Instance == NULL) {
    ...
    if (CSoundDriverDSound::instance()->getOption(
      ...
      if (!_Listener || FAILED(_Listener->SetRolloffFactor(
                            DS3D_MINROLLOFFFACTOR, DS3D_DEFERRED))) {
...
void CListenerDSound::setPos(const NLMISC::CVector &pos) {
  ...
  if (_Listener != NULL) {
    if (FAILED(_Listener->SetPosition(pos.x, pos.z, pos.y, DS3D_DEFERRED))) {
      ...
      // nlwarning ("set listener NEL(p:%.2f/%.2f/%.2f) DS(p:%.2f/%.2f/%.2f)",
 ...
 */
void CListenerDSound::setOrientation(const NLMISC::CVector &front,
                                     const NLMISC::CVector &up) {
  if (_Listener != NULL) {
    if (FAILED(_Listener->SetOrientation(front.x, front.z, front.y, up.x, up.z,
                                         up.y, DS3D_DEFERRED))) {
      nlwarning("SetOrientation failed");
    } else {
      // nlwarning ("NLSOUND: set listener orientation NEL(f:%.2f/%.2f/%.2f
      // up:%.2f/%.2f/%.2f) DS(f:%.2f/%.2f/%.2f up:%.2f/%.2f/%.2f)", front.x,
      // front.y, front.z, up.x, up.y, up.z, front.x, front.z, front.y, up.x,
      // up.z, up.y);
    }
  }
}
 ...
 */
void CListenerDSound::getOrientation(NLMISC::CVector &front,
                                     NLMISC::CVector &up) const {
  if (_Listener != NULL) {
...
void CListenerDSound::setRolloffFactor(float f) {
  ...
  nlassert(!CSoundDriverDSound::instance()->getOption(
      ISoundDriver::OptionManualRolloff));
...
float CListenerDSound::getRolloffFactor() {
  // Works only in API rolloff mode
  nlassert(!CSoundDriverDSound::instance()->instance()->getOption(
      ISoundDriver::OptionManualRolloff));

  if (!CSoundDriverDSound::instance()->instance()->getOption(
          ISoundDriver::OptionManualRolloff)) {
    if (_Listener != NULL) {
      float f;
...
/*
 * Set DSPROPERTY_EAXLISTENER_ENVIRONMENT and
 * DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE if EAX available (see EAX listener
 * properties)
 */
#if EAX_AVAILABLE == 1
void CListenerDSound::setEnvironment(uint env, float size) {
  ...
  if (_EAXListener != NULL) {
    HRESULT res = _EAXListener->Set(DSPROPSETID_EAX_ListenerProperties,
                                    DSPROPERTY_EAXLISTENER_ENVIRONMENT, NULL, 0,
                                    &env, sizeof(unsigned long));
    if (res != S_OK)
      nlwarning("Setting EAX environment #%u fail : %x", env, res);
    res = _EAXListener->Set(DSPROPSETID_EAX_ListenerProperties,
                            DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE, NULL, 0,
                            &size, sizeof(float));
    if (res != S_OK)
...
void CListenerDSound::setEAXProperty(uint prop, void *value, uint valuesize) {
  ...
  if (_EAXListener != NULL) {
    HRESULT res = _EAXListener->Set(DSPROPSETID_EAX_ListenerProperties, prop,
                                    NULL, 0, value, valuesize);
    if (res != S_OK)
      nlwarning("Setting EAX listener prop #%d fail : %x", prop, res);
  }
#else
void CListenerDSound::setEAXProperty(uint /* prop */, void * /* value */,
                                     uint /* valuesize */) {
#endif
}

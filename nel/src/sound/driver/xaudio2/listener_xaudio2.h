 ...
 * CListenerXAudio2 is an implementation of the IListener interface to run on
 * XAudio2.
 * TODO: For occlusion reverb output gain must be controllable per voice (source
 * send with lower gain to reverb submix).
 */
class CListenerXAudio2 : public IListener,
                         public NLMISC::CManualSingleton<CListenerXAudio2> {
protected:
  // outside pointers
...
public:
  ...
  //@{
  /// Set the doppler factor (default: 1) to exaggerate or not the doppler
  /// effect
  virtual void setDopplerFactor(float f);
  /// Set the rolloff factor (default: 1) to scale the distance attenuation
  /// effect

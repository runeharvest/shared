...
CListenerXAudio2::CListenerXAudio2(CSoundDriverXAudio2 *soundDriver)
    : _DryVoice(NULL), _FilterVoice(NULL), _ListenerOk(false),
      _SoundDriver(soundDriver), _DopplerScaler(1.0f), _Pos(0.0f, 0.0f, 0.0f),
      _RolloffScaler(1.0f) {
  ...
  if (FAILED(hr = soundDriver->getXAudio2()->CreateSubmixVoice(
                 &_DryVoice, voice_details.InputChannels,
                 voice_details.InputSampleRate, 0, 9010, NULL, NULL))) {
    release();
    _SoundDriver = NULL;
    throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX
                       "FAILED CreateSubmixVoice _DryVoice!");
    return;
  }

  if (FAILED(hr = soundDriver->getXAudio2()->CreateSubmixVoice(
                 &_FilterVoice, voice_details.InputChannels,
                 voice_details.InputSampleRate, 0, 9000, NULL, NULL))) {
    release();
    _SoundDriver = NULL;
    throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX
...
CListenerXAudio2::~CListenerXAudio2() {
  ...
  release();
  if (soundDriver)
    soundDriver->removeListener(this);
}
 ...
 */
const NLMISC::CVector &CListenerXAudio2::getPos() const { return _Pos; }
...
/// Set the orientation vectors (3D mode only, ignored in stereo mode) (default:
/// (0,1,0), (0,0,-1))
void CListenerXAudio2::setOrientation(const NLMISC::CVector &front,
                                      const NLMISC::CVector &up) {
  // nldebug("--- orientation --- %s --- %s ---", front.toString().c_str(),
  // up.toString().c_str());
...
void CListenerXAudio2::getOrientation(NLMISC::CVector &front,
 ...
 */
void CListenerXAudio2::setGain(float gain) { _DryVoice->SetVolume(gain); }

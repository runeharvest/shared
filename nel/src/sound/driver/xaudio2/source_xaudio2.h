 ...
 * CSourceXAudio2 is an implementation of the ISource interface to run on
 * XAudio2.
 */
...
private:
  ...
  NLMISC::CVector _Pos;
  /// Source relative to listener position (listener + source = actual source
  /// location).
  bool _Relative;
  ...
  // -- Output path vars --
  /// Voices
  IXAudio2Voice *_DirectDryVoice, *_DirectFilterVoice, *_EffectDryVoice,
      *_EffectFilterVoice;
  /// Path enabled settings
  bool _DirectDryEnabled, _DirectFilterEnabled, _EffectDryEnabled,
...
public:
  ...
  /// yet.
  bool initFormat(IBuffer::TBufferFormat bufferFormat, uint8 channels,
                  uint8 bitsPerSample);
  ...
  /// (Internal) Prepare to play. Stop the currently playing buffers, and set
  /// the correct voice settings.
  bool preparePlay(IBuffer::TBufferFormat bufferFormat, uint8 channels,

...
// Copyright (C) 2008-2023  Jan BOON (Kaetemi) <jan.boon@kaetemi.be>
//
...
CAdpcmXAudio2::CAdpcmXAudio2(bool loop)
    : _SourceVoice(NULL), _SourceData(NULL), _SourceSize(0), _SampleRate(0),
      _Loop(loop), _BufferNext(0), _SampleNb(0), _AdpcmSize(0), _AdpcmIndex(0),
...
void CAdpcmXAudio2::submitSourceBuffer(CBufferXAudio2 *buffer) {
  ...
  if (_SourceData)
    nlerror(NLSOUND_XAUDIO2_PREFIX
            "Only 1 ADPCM buffer can be submitted at a time! Call "
            "flushSourceBuffers first in CSourceXAudio2 stop().");
...
void CAdpcmXAudio2::processBuffers() {
  if (_SourceData) {
    ...
    while (voice_state.BuffersQueued < _BufferNb) {
      ...
      // nldebug("yeah, i'm decoding adpcm! %s, the size is %u and I'm at index
      // %u, run %u, do %u bytes, source data %s!!!", _Loop ? "it's LOOPING" :
      // "it's NOT looping", (uint32)_SourceSize, (uint32)_AdpcmIndex,
      // (uint32)0, (uint32)inbytes, _SourceData ? "EXISTS" : "does NOT exist");

      if (inbytes > 0) {
        IBuffer::decodeADPCM(_SourceData + _AdpcmIndex, _Buffer[_BufferNext],
                             inbytes * 2, _State);
...
void CAdpcmXAudio2::OnBufferStart(void *pBufferContext) {
  ...
  // nlwarning("No valid buffer context available for: %u",
  // (uint32)pBufferContext);
}
...
void CAdpcmXAudio2::OnVoiceError(void * /* pBufferContext */,
                                 HRESULT /* Error */) {}

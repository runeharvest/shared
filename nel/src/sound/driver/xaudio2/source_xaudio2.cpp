...
CSourceXAudio2::CSourceXAudio2(CSoundDriverXAudio2 *soundDriver)
    : _SoundDriver(soundDriver), _SourceVoice(NULL), _StaticBuffer(NULL),
      ...
      _DirectDryVoice(NULL), _DirectFilterVoice(NULL), _EffectDryVoice(NULL),
      _EffectFilterVoice(NULL), _DirectDryEnabled(false),
      _DirectFilterEnabled(false), _EffectDryEnabled(false),
      _EffectFilterEnabled(false), _DirectGain(NLSOUND_DEFAULT_DIRECT_GAIN),
      ...
      _Gain(1.0f), _MinDistance(1.0f),
      _MaxDistance(sqrt(numeric_limits<float>::max())), _AdpcmUtility(NULL),
...
CSourceXAudio2::~CSourceXAudio2() {
  ...
  release();
  if (soundDriver)
    soundDriver->removeSource(this);
}

void CSourceXAudio2::release() // called by driver or destructor, whichever is
                               // first
{
  if (_AdpcmUtility) {
...
void CSourceXAudio2::commit3DChanges() {
  ...
  // Only mono buffers get 3d sound, multi-channel buffers go directly to the
  // speakers without any distance rolloff.
  if (_Channels > 1) {
    ...
    // nlerror(NLSOUND_XAUDIO2_PREFIX "Stereo16 and Stereo8 not fully
    ...
    // // nldebug(NLSOUND_XAUDIO2_PREFIX "_SampleVoice->getBuffer() %u",
    // (uint32)_SampleVoice->getBuffer());
    ...
    X3DAUDIO_DISTANCE_CURVE curve = {
        (X3DAUDIO_DISTANCE_CURVE_POINT *)&curve_points[0], 2};
    ...
    if (_SoundDriver->getOption(ISoundDriver::OptionManualRolloff)) {
      ...
      float rolloff = ISource::computeManualRolloff(_Alpha, sqrdist,
      ...
      // divide min distance (distance from where to start attenuation) with
      // rolloff scaler (factor to get faster attenuation)
      _Emitter.CurveDistanceScaler =
          _MinDistance / _SoundDriver->getListener()->getRolloffScaler();
      // _MaxDistance not implemented (basically should cut off sound beyond
      // maxdistance)
    }

    X3DAudioCalculate(
        _SoundDriver->getX3DAudio(),
        _Relative ? _SoundDriver
                        ->getEmptyListener() // position is relative to listener
                                             // (we use 0pos listener)
                  : _SoundDriver->getListener()
                        ->getListener(), // position is absolute
        &_Emitter, X3DAUDIO_CALCULATE_MATRIX | X3DAUDIO_CALCULATE_DOPPLER,
        &dspSettings);
    ...
    if (_DirectDryEnabled) {
      ...
      _SourceVoice->SetOutputMatrix(_DirectDryVoice, 1,
                                    dspSettings.DstChannelCount, outputMatrix,
                                    _OperationSet);
      if (_DirectFilterEnabled) {
        ...
        _SourceVoice->SetOutputMatrix(_DirectFilterVoice, 1,
                                      dspSettings.DstChannelCount, outputMatrix,
                                      _OperationSet);
      }
    }
    ...
    if (_EffectDryEnabled) {
      ...
      _SourceVoice->SetOutputMatrix(_EffectDryVoice, 1, 1, &outputSingle,
                                    _OperationSet);
      if (_EffectFilterEnabled) {
        float effectFilterGain = _EffectGain - effectDryGain;
        outputSingle = monoRolloff * effectFilterGain;
        _SourceVoice->SetOutputMatrix(_EffectFilterVoice, 1, 1, &outputSingle,
                                      _OperationSet);
      }
    }

    // nldebug(NLSOUND_XAUDIO2_PREFIX "left: %f, right %f",
    // _SoundDriver->getDSPSettings()->pMatrixCoefficients[0],
    // _SoundDriver->getDSPSettings()->pMatrixCoefficients[1]);
...
void CSourceXAudio2::submitBuffer(CBufferXAudio2 *ibuffer) {
  ...
  if (_AdpcmUtility) {
    ...
    buffer.LoopBegin = 0;
    buffer.LoopCount =
        (_IsLooping && !_BufferStreaming) ? XAUDIO2_LOOP_INFINITE : 0;
    buffer.LoopLength = 0;
...
void CSourceXAudio2::setupVoiceSends() {
  ...
  _SoundDriver->getXAudio2()->CommitChanges(
      _OperationSet); // SetOutputVoices does not support OperationSet
  _SourceVoice->SetOutputVoices(
      &voiceSends); // SetOutputVoices does not support OperationSet
  setupDirectFilter();
  setupEffectFilter();
  _SoundDriver->getXAudio2()->CommitChanges(
      _OperationSet); // SetOutputVoices does not support OperationSet
    ...
    if (!voice_state.BuffersQueued) {
      if (!_IsLooping)
        nlwarning(NLSOUND_XAUDIO2_PREFIX
                  "Switched streaming mode while buffer still queued!?! Flush");
      _SoundDriver->getXAudio2()->CommitChanges(_OperationSet);
...
void CSourceXAudio2::setStaticBuffer(IBuffer *buffer) {
  ...
  // if (buffer) // nldebug(NLSOUND_XAUDIO2_PREFIX "setStaticBuffer %s",
  // _SoundDriver->getStringMapper()->unmap(buffer->getName()).c_str()); else //
  // nldebug(NLSOUND_XAUDIO2_PREFIX "setStaticBuffer NULL");

  // if (_IsPlaying) nlwarning(NLSOUND_XAUDIO2_PREFIX "Called
  // setStaticBuffer(IBuffer *buffer) while _IsPlaying == true!");
...
IBuffer *CSourceXAudio2::getStaticBuffer() {
  nlassert(!_BufferStreaming); // can be implemented trough
    ...
    if (_AdpcmUtility) {
      ...
      if (_SourceVoice) {
        if (_IsPlaying) {
          ...
          _SourceVoice->GetState(&voice_state);
          if (voice_state.BuffersQueued) {
            // nlwarning(NLSOUND_XAUDIO2_PREFIX "Not playing but buffer already
            // queued while switching loop mode!?! Flush and requeue");
            if (FAILED(_SourceVoice->FlushSourceBuffers()))
              nlwarning(NLSOUND_XAUDIO2_PREFIX "FAILED FlushSourceBuffers");
...
/// (Internal) Initialize voice with this format, if no voice has been created
/// yet.
bool CSourceXAudio2::initFormat(IBuffer::TBufferFormat bufferFormat,
                                uint8 channels, uint8 bitsPerSample) {
  // nlwarning(NLSOUND_XAUDIO2_PREFIX "New voice with format %u!",
  // (uint32)_StaticBuffer->getFormat());

  nlassert(!_SourceVoice);
  nlassert(!_AdpcmUtility);
  // create adpcm utility callback if needed
  if (bufferFormat == IBuffer::FormatDviAdpcm)
    _AdpcmUtility = new CAdpcmXAudio2(_IsLooping);
  // create voice with adpcm utility callback or NULL callback
  _SourceVoice = _SoundDriver->createSourceVoice(bufferFormat, channels,
                                                 bitsPerSample, _AdpcmUtility);
  if (_AdpcmUtility)
    _AdpcmUtility->setSourceVoice(_SourceVoice);
  if (!_SourceVoice)
    return false; // fail
  ...
  // nlinfo("voice_details.InputSampleRate: '%u'",
  // voice_details.InputSampleRate);
  //_FilterParameters[0].Frequency = XAudio2CutoffFrequencyToRadians(5000,
  //voice_details.InputSampleRate);
  // nlinfo("_FilterParameters[0].Frequency: '%f'",
  // _FilterParameters[0].Frequency);
  //_FilterParameters[0].OneOverQ = 1.0f;
  // nlinfo("_FilterParameters[0].OneOverQ: '%f'",
  // _FilterParameters[0].OneOverQ);
  //_SourceVoice->SetOutputFilterParameters(_SendDescriptors[0].pOutputVoice,
  //&_FilterParameters[0]);
  // test
...
/// (Internal) Prepare to play. Stop the currently playing buffers, and set the
/// correct voice settings.
bool CSourceXAudio2::preparePlay(IBuffer::TBufferFormat bufferFormat,
                                 uint8 channels, uint8 bitsPerSample,
                                 uint32 frequency) {
  if (_IsPlaying) {
    // nlwarning(NLSOUND_XAUDIO2_PREFIX "Called play() while _IsPlaying ==
    // true!"); stop the currently playing voice if it's of the same type as we
    // need
    if (bufferFormat == _Format && channels == _Channels &&
        bitsPerSample == _BitsPerSample)
      // cannot call stop directly before destroy voice, ms bug in xaudio2, see
      // msdn docs
      stop(); // sets _IsPlaying = false;
  }
  if (_SourceVoice && (bufferFormat != _Format || channels != _Channels ||
                       bitsPerSample != _BitsPerSample)) {
    // nlwarning(NLSOUND_XAUDIO2_PREFIX "Switching format %u to %u!",
    // (uint32)_Format, (uint32)bufferFormat); destroy existing voice
    _SoundDriver->getXAudio2()->CommitChanges(_OperationSet);
    _SoundDriver->destroySourceVoice(_SourceVoice);
    _SourceVoice = NULL;
  ...
  if (!_SourceVoice) {
    ...
    _SourceVoice->SetSourceSampleRate(frequency);
    if (!_BufferStreaming) {
      // notify other sources about this format, so they can make a voice in
      // advance we know that there is a very high chance that all sources use
      // same format so this gives better results at runtime (avoids sound
      // clicks etc)
  ...
  //_SourceVoice->GetVoiceDetails(&voice_details);
  // nlinfo("voice_details.InputSampleRate: '%u'",
  // voice_details.InputSampleRate); test
...
bool CSourceXAudio2::play() {
  ...
  if (_IsPaused) {
    if (SUCCEEDED(_SourceVoice->Start(0)))
      _IsPaused = false;
    else
      nlwarning(NLSOUND_XAUDIO2_PREFIX "FAILED Play");
    return !_IsPaused;
  } else {
    ...
    if (_BufferStreaming) {
      ...
      if (SUCCEEDED(_SourceVoice->Start(0)))
        ...
        nlwarning(NLSOUND_XAUDIO2_PREFIX "FAILED Play (_BufferStreaming)");
    } else {
      if (_StaticBuffer) {
        preparePlay(_StaticBuffer->getFormat(), _StaticBuffer->getChannels(),
...
void CSourceXAudio2::pause() {
  ...
  if (_IsPaused)
    nlwarning(NLSOUND_XAUDIO2_PREFIX "Called pause() while _IsPaused == true!");
  ...
  if (_IsPlaying) {
    ...
    nlwarning(NLSOUND_XAUDIO2_PREFIX
              "Called pause() while _IsPlaying == false!");
}
...
void CSourceXAudio2::setPos(
    const NLMISC::CVector &pos,
    bool /* deffered */) // note: deferred with a different spelling
{
  ...
  // !! todo if threaded: if (!deffered) { /* nlwarning(NLSOUND_XAUDIO2_PREFIX
  // "!deffered"); */ commit3DChanges(); }
}

/// Get the position vector.
const NLMISC::CVector &CSourceXAudio2::getPos() const { return _Pos; }

/// Set the velocity vector (3D mode only, ignored in stereo mode) (default:
/// (0,0,0))
void CSourceXAudio2::setVelocity(const NLMISC::CVector &vel,
                                 bool /* deferred */) {
  // nldebug(NLSOUND_XAUDIO2_PREFIX "setVelocity %f %f %f", vel.x, vel.y,
  // vel.z);

  NLSOUND_XAUDIO2_X3DAUDIO_VECTOR_FROM_VECTOR(_Emitter.Velocity, vel);

  // !! todo if threaded: if (!deferred) { /* nlwarning(NLSOUND_XAUDIO2_PREFIX
  // "!deferred"); */ commit3DChanges(); }
}
...
/// Set the direction vector (3D mode only, ignored in stereo mode) (default:
/// (0,0,0) as non-directional)
void CSourceXAudio2::setDirection(const NLMISC::CVector &dir) {
  // nldebug(NLSOUND_XAUDIO2_PREFIX "setDirection %f %f %f", dir.x, dir.y,
  // dir.z);
...
/// Get the gain
float CSourceXAudio2::getGain() const { return _Gain; }
...
void CSourceXAudio2::setPitch(float pitch) {
  ...
  if (_SourceVoice) {
    if (_Channels > 1)
      ...
      _SourceVoice->SetFrequencyRatio(_Pitch * _Doppler, _OperationSet);
  }
}

/// Get the pitch
float CSourceXAudio2::getPitch() const { return _Pitch; }

/// Set the source relative mode. If true, positions are interpreted relative to
...
bool CSourceXAudio2::getSourceRelativeMode() const { return _Relative; }

/// Set the min and max distances (default: 1, MAX_FLOAT) (3D mode only)
void CSourceXAudio2::setMinMaxDistances(float mindist, float maxdist,
                                        bool /* deferred */) {
  // nldebug(NLSOUND_XAUDIO2_PREFIX "setMinMaxDistances %f, %f", mindist,
  // maxdist);

  static float maxSqrt = sqrt(std::numeric_limits<float>::max());
  if (maxdist >= maxSqrt) {
    nlwarning(
        "SOUND_DEV (XAudio2): Ridiculously high max distance set on source");
    maxdist = maxSqrt;
  }
...
void CSourceXAudio2::setCone(float innerAngle, float outerAngle,
                             float outerGain) {
  // nldebug(NLSOUND_XAUDIO2_PREFIX "setCone %f, %f ,%f", innerAngle,
...
void CSourceXAudio2::getCone(float &innerAngle, float &outerAngle,
...
bool CSourceXAudio2::getDirect() const { return _DirectDryEnabled; }
...
float CSourceXAudio2::getDirectGain() const { return _DirectGain; }
...
void CSourceXAudio2::setupDirectFilter() {
  // todo: this sort of works and sounds sort of similar to the openal way, but
  // still needs work
  if (_DirectDryEnabled && _DirectFilterEnabled) {
    switch (_DirectFilter.Type) {
    case LowPassFilter:
      _DirectFilter.Frequency = XAudio2CutoffFrequencyToRadians(
          _DirectFilterHighFrequency, _Frequency);
      _DirectFilter.OneOverQ = 1.0f;
      break;
    case BandPassFilter: {
      float centerFrequency =
          sqrt(_DirectFilterLowFrequency *
               _DirectFilterHighFrequency); // geometric mean
      float bandwidth = _DirectFilterHighFrequency - _DirectFilterLowFrequency;
      float q = centerFrequency / bandwidth;
      _DirectFilter.Frequency =
          XAudio2CutoffFrequencyToRadians(centerFrequency, _Frequency);
      _DirectFilter.OneOverQ = 1.0f / q;
      break;
    }
    case HighPassFilter:
      _DirectFilter.Frequency = XAudio2CutoffFrequencyToRadians(
          _DirectFilterLowFrequency, _Frequency);
      _DirectFilter.OneOverQ = 1.0f;
      break;
    default:
    ...
    _SourceVoice->SetOutputFilterParameters(_DirectFilterVoice, &_DirectFilter,
                                            _OperationSet);
  }
}

/// Set the filter parameters for the direct channel
void CSourceXAudio2::setDirectFilter(TFilter filterType, float lowFrequency,
                                     float highFrequency, float passGain) {
  switch (filterType) {
...
void CSourceXAudio2::getDirectFilter(TFilter &filterType, float &lowFrequency,
                                     float &highFrequency,
...
void CSourceXAudio2::setDirectFilterPassGain(float passGain) {
  _DirectFilterPassGain =
      min(max(passGain, NLSOUND_MIN_GAIN), NLSOUND_MAX_GAIN);
}
...
void CSourceXAudio2::setEffect(IReverbEffect *reverbEffect) {
  setEffect(reverbEffect
                ? static_cast<CEffectXAudio2 *>(
                      static_cast<CReverbEffectXAudio2 *>(reverbEffect))
                : NULL);
}

/// Get the effect send for this source
IEffect *CSourceXAudio2::getEffect() const { return NULL; }
...
/// Get the gain for the direct path
float CSourceXAudio2::getEffectGain() const { return _EffectGain; }
...
void CSourceXAudio2::setupEffectFilter() {
  ...
  if (_EffectDryEnabled && _EffectFilterEnabled) {
    ...
    case LowPassFilter:
      _EffectFilter.Frequency = XAudio2CutoffFrequencyToRadians(
          _EffectFilterHighFrequency, _Frequency);
      _EffectFilter.OneOverQ = 1.0f;
      break;
    case BandPassFilter: {
      float centerFrequency =
          sqrt(_EffectFilterLowFrequency *
               _EffectFilterHighFrequency); // geometric mean
      float bandwidth = _EffectFilterHighFrequency - _EffectFilterLowFrequency;
      float q = centerFrequency / bandwidth;
      _EffectFilter.Frequency =
          XAudio2CutoffFrequencyToRadians(centerFrequency, _Frequency);
    ...
    case HighPassFilter:
      _EffectFilter.Frequency = XAudio2CutoffFrequencyToRadians(
          _EffectFilterLowFrequency, _Frequency);
      _EffectFilter.OneOverQ = 1.0f;
      break;
    default:
    ...
    _SourceVoice->SetOutputFilterParameters(_EffectFilterVoice, &_EffectFilter,
                                            _OperationSet);
  }
}

/// Set the filter parameters for the direct channel
void CSourceXAudio2::setEffectFilter(TFilter filterType, float lowFrequency,
                                     float highFrequency, float passGain) {
  switch (filterType) {
  case ISource::FilterLowPass:
    _EffectFilter.Type = LowPassFilter;
...
/// Get the filter parameters for the direct channel
void CSourceXAudio2::getEffectFilter(TFilter &filterType, float &lowFrequency,
                                     float &highFrequency,
                                     float &passGain) const {
  switch (_EffectFilter.Type) {
  case LowPassFilter:

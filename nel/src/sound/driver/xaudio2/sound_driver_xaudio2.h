...
#ifndef NLSOUND_SOUND_DRIVER_XAUDIO2_H // todo: support MAKEINTRESOURCE for
                                       // music files?
 ...
 * CSoundDriverXAudio2 is an implementation of the ISoundDriver interface to run
 * on XAudio2.
 */
class CSoundDriverXAudio2
    : public ISoundDriver,
      public NLMISC::CManualSingleton<CSoundDriverXAudio2> {
protected:
  // pointers
  /// Pointer to XAudio2.
  IXAudio2 *_XAudio2;
...
public:
  ...
  inline void performanceRegisterBuffer(IBuffer::TBufferFormat bufferFormat,
                                        uint size) {
    switch (bufferFormat) {
    case IBuffer::FormatPcm:
      _PerformancePCMBufferSize += size;
      break;
    case IBuffer::FormatDviAdpcm:
      _PerformanceADPCMBufferSize += size;
      break;
    }
  }
  /// (Internal) Remove a data buffer from the performance counters.
  inline void performanceUnregisterBuffer(IBuffer::TBufferFormat bufferFormat,
                                          uint size) {
    switch (bufferFormat) {
    case IBuffer::FormatPcm:
      _PerformancePCMBufferSize -= size;
  ...
  inline void performanceIncreaseSourcePlayCounter() {
    ++_PerformanceSourcePlayCounter;
  }
  /// (Internal) Increase the commit 3d counter by one.
  inline void performanceIncreaseCommit3DCounter() {
    ++_PerformanceCommit3DCounter;
  }

  /// (Internal) Initialize uninitialized sources with this format (so xaudio2
  /// voices don't need to be created at runtime)
  void initSourcesFormat(IBuffer::TBufferFormat bufferFormat, uint8 channels,
                         uint8 bitsPerSample);
  ...
  IXAudio2SourceVoice *createSourceVoice(IBuffer::TBufferFormat bufferFormat,
  ...
  uint getDeviceIndex(const std::string &device,
  ...
  /// Does not create a sound loader... that's really awesome but what does it
  /// do?
  ...
  /// (Internal) Remove the listener (should be called by the destructor of the
  /// listener class)
  inline void removeListener(CListenerXAudio2 *listener) {
    nlassert(_Listener == listener);
    _Listener = NULL;
  }

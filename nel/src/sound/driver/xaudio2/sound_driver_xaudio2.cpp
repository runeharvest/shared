...
static XAUDIO2_DEBUG_CONFIGURATION
    NLSOUND_XAUDIO2_DEBUG_CONFIGURATION_DISABLED = {0,    0,    true,
                                                    true, true, true};
...
NLMISC_CATEGORISED_COMMAND(nel, xa2DebugDisable, "", "") {
  ...
  args;
  rawCommandString;
  CSoundDriverXAudio2::getInstance()->getXAudio2()->SetDebugConfiguration(
      &NLSOUND_XAUDIO2_DEBUG_CONFIGURATION_DISABLED);
  return true;
}

static XAUDIO2_DEBUG_CONFIGURATION NLSOUND_XAUDIO2_DEBUG_CONFIGURATION_HEAVY = {
    (UINT32)(~XAUDIO2_LOG_FUNC_CALLS & ~XAUDIO2_LOG_LOCKS &
             ~XAUDIO2_LOG_MEMORY),
    0,
    true,
    true,
...
NLMISC_CATEGORISED_COMMAND(nel, xa2DebugHeavy, "", "") {
  ...
  rawCommandString;
  CSoundDriverXAudio2::getInstance()->getXAudio2()->SetDebugConfiguration(
      &NLSOUND_XAUDIO2_DEBUG_CONFIGURATION_HEAVY);
  return true;
}
...
CSoundDriverXAudio2::CSoundDriverXAudio2(
  ...
  if (hr == RPC_E_CHANGED_MODE) {
    nlwarning(NLSOUND_XAUDIO2_PREFIX "CoInitializeEx COINIT_APARTMENTTHREADED");
    hr = CoInitializeEx(NULL, COINIT_APARTMENTTHREADED);
  }
  _CoInitOk = (hr == S_OK) || (hr == S_FALSE);
  if (!_CoInitOk) {
    release();
    throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX "FAILED CoInitializeEx");
    return;
  }
#endif
...
//	flags |= XAUDIO2_DEBUG_ENGINE; // comment when done using this :)
  ...
  if (FAILED(hr = XAudio2Create(&_XAudio2, flags, XAUDIO2_DEFAULT_PROCESSOR))) {
    ...
    throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX
                       "XAudio2 failed to initialize. Please install the "
                       "latest version of the DirectX End-User Runtimes.");
    return;
  }
}
...
#define NLSOUND_XAUDIO2_RELEASE(pointer)                                       \
  if (_SoundDriverOk)                                                          \
    nlassert(pointer);                                                         \
...
void CSoundDriverXAudio2::release() {
  ...
  if (!_Sources.empty()) {
    nlwarning(NLSOUND_XAUDIO2_PREFIX "_Sources.size(): '%u'",
  ...
  if (!_Buffers.empty()) {
    nlwarning(NLSOUND_XAUDIO2_PREFIX "_Buffers.size(): '%u'",
              (uint32)_Buffers.size());
  ...
  if (!_Effects.empty()) {
    nlwarning(NLSOUND_XAUDIO2_PREFIX "_Effects.size(): '%u'",
  ...
  if (_Listener) {
    nlwarning(NLSOUND_XAUDIO2_PREFIX "_Listener: !NULL");
    _Listener->release();
    _Listener = NULL;
  }
...
uint CSoundDriverXAudio2::getDeviceIndex(
    const std::string &device, XAUDIO2_DEVICE_DETAILS *deviceDetails) {
  if (device.empty()) {
...
void CSoundDriverXAudio2::initDevice(const std::string &device,
  ...
  const sint supportedOptions = OptionAllowADPCM | OptionSoftwareBuffer |
                                OptionManualRolloff | OptionLocalBufferCopy |
                                OptionHasBufferStreaming | OptionReverbEffect |
                                OptionFilterEffect;

  // list of forced options in this driver
  // always use software buffer, always have local copy
  const sint forcedOptions =
      OptionSoftwareBuffer | OptionManualRolloff | OptionLocalBufferCopy;
  ...
  if (FAILED(hr = _XAudio2->CreateMasteringVoice(
                 &_MasteringVoice, 0, NLSOUND_XAUDIO2_MASTER_SAMPLE_RATE, 0,
                 deviceIndex, NULL))) {
    release();
    throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX
                       "FAILED CreateMasteringVoice _MasteringVoice!");
    return;
  }

  // X3DAudio
  // speed of sound in meters per second for dry air at approximately 20C, used
  // with X3DAudioInitialize #define X3DAUDIO_SPEED_OF_SOUND 343.5f
  X3DAudioInitialize(deviceDetails.OutputFormat.dwChannelMask,
                     X3DAUDIO_SPEED_OF_SOUND, _X3DAudioHandle);
...
void CSoundDriverXAudio2::initSourcesFormat(IBuffer::TBufferFormat bufferFormat,
                                            uint8 channels,
                                            uint8 bitsPerSample) {
  std::set<CSourceXAudio2 *>::iterator it(_Sources.begin()),
      end(_Sources.end());
  for (; it != end; ++it) {
    if (!(*it)->getSourceVoice()) {
      (*it)->initFormat(bufferFormat, channels, bitsPerSample);
    }
  }
}
...
CSoundDriverXAudio2::createSourceVoice(IBuffer::TBufferFormat bufferFormat,
  ...
  // TODO: Set callback (in CSourceXAudio2 maybe) for when error happens on
  // voice, so we can restart it!
  IXAudio2SourceVoice *source_voice = NULL;

  if (FAILED(hr = _XAudio2->CreateSourceVoice(&source_voice, &wfe, 0,
                                              NLSOUND_MAX_PITCH, callback, NULL,
                                              NULL))) {
    if (source_voice)
      source_voice->DestroyVoice();
    nlerror(NLSOUND_XAUDIO2_PREFIX "FAILED CreateSourceVoice");
    return NULL;
  }
...
IListener *CSoundDriverXAudio2::createListener() {
  if (!_Listener)
    _Listener = new CListenerXAudio2(this);
...
void CSoundDriverXAudio2::writeProfile(std::string &out) {
  ...
  out = toString(NLSOUND_XAUDIO2_NAME) +
        ...
        toString(performance.AudioCyclesSinceLastQuery) +
        "\n\tTotalCyclesSinceLastQuery: " +
        toString(performance.TotalCyclesSinceLastQuery) +
        "\n\tMinimumCyclesPerQuantum: " +
        toString(performance.MinimumCyclesPerQuantum) +
        "\n\tMaximumCyclesPerQuantum: " +
        toString(performance.MaximumCyclesPerQuantum) +
        "\n\tMemoryUsageInBytes: " + toString(performance.MemoryUsageInBytes) +
        "\n\tCurrentLatencyInSamples: " +
        toString(performance.CurrentLatencyInSamples) +
        "\n\tGlitchesSinceEngineStarted: " +
        toString(performance.GlitchesSinceEngineStarted) +
        "\n\tActiveSourceVoiceCount: " +
        toString(performance.ActiveSourceVoiceCount) +
        "\n\tTotalSourceVoiceCount: " +
        toString(performance.TotalSourceVoiceCount) +
        "\n\tActiveSubmixVoiceCount: " +
        toString(performance.ActiveSubmixVoiceCount) +
        "\n\tActiveXmaSourceVoices: " +
        toString(performance.ActiveXmaSourceVoices) +
...
void CSoundDriverXAudio2::startBench() { NLMISC::CHTimer::startBench(); }

void CSoundDriverXAudio2::endBench() { NLMISC::CHTimer::endBench(); }
...
/// Remove a buffer (should be called by the friend destructor of the buffer
/// class)
void CSoundDriverXAudio2::removeBuffer(CBufferXAudio2 *buffer) {
  if (_Buffers.find(buffer) != _Buffers.end())
    _Buffers.erase(buffer);
  else
    nlwarning("removeBuffer already called");
}

/// Remove a source (should be called by the friend destructor of the source
/// class)
void CSoundDriverXAudio2::removeSource(CSourceXAudio2 *source) {
  if (_Sources.find(source) != _Sources.end())
    _Sources.erase(source);
  else
    nlwarning("removeSource already called");
}

/// (Internal) Remove an effect (should be called by the destructor of the
/// effect class)
void CSoundDriverXAudio2::removeEffect(CEffectXAudio2 *effect) {
  if (_Effects.find(effect) != _Effects.end())
    _Effects.erase(effect);
  else
    nlwarning("removeEffect already called");
}

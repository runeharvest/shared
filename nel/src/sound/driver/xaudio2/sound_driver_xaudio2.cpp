...
CSoundDriverXAudio2::CSoundDriverXAudio2(ISoundDriver::IStringMapperProvider * /* stringMapper */)
    ...
    , _CoInitOk(false)
    , _OperationSetCounter(65536)
    , _PerformanceCommit3DCounter(0)
    , _PerformanceADPCMBufferSize(0)
    , _PerformancePCMBufferSize(0)
...
//	flags |= XAUDIO2_DEBUG_ENGINE; // comment when done using this :)
	...
	{
		release();
		throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX "XAudio2 failed to initialize. Please install the latest version of the DirectX End-User Runtimes.");
		return;
	}
}
...
void CSoundDriverXAudio2::initDevice(const std::string &device, TSoundOptions options)
{
	...
	const sint forcedOptions = OptionSoftwareBuffer
	    | OptionManualRolloff
	    | OptionLocalBufferCopy;
	...
	{
		release();
		throw ESoundDriver(NLSOUND_XAUDIO2_PREFIX "FAILED CreateMasteringVoice _MasteringVoice!");
		return;
	}
...
void CSoundDriverXAudio2::initSourcesFormat(IBuffer::TBufferFormat bufferFormat, uint8 channels, uint8 bitsPerSample)
{
	...
	for (; it != end; ++it)
	{
		if (!(*it)->getSourceVoice()) { (*it)->initFormat(bufferFormat, channels, bitsPerSample); }
	}
}

...
CEffectXAudio2::CEffectXAudio2(CSoundDriverXAudio2 *soundDriver, uint channels)
    : _SoundDriver(soundDriver), _DryVoice(NULL), _FilterVoice(NULL),
      _Effect(NULL) {
  HRESULT hr;
  ...
  // Mix voice
  {
    if (FAILED(hr = soundDriver->getXAudio2()->CreateSubmixVoice(
                   &_DryVoice, channels,
                   min(max(20000, (int)voice_details.InputSampleRate), 48000),
                   0, 4510, NULL, NULL))) {
      release();
      _SoundDriver = NULL;
    ...
    if (channels == 1) {
      ...
      for (uint32 i = 0; i < voice_details.InputChannels; ++i)
        outputMatrix[i] = 1.0f;
      _DryVoice->SetOutputMatrix(sendDescriptor.pOutputVoice, 1,
                                 voice_details.InputChannels, outputMatrix);
    }
  }
  ...
  {
    if (FAILED(hr = soundDriver->getXAudio2()->CreateSubmixVoice(
                   &_FilterVoice, channels, voice_details.InputSampleRate, 0,
                   4500, NULL, NULL))) {
      release();
      _SoundDriver = NULL;
      nlwarning(NLSOUND_XAUDIO2_PREFIX
                "FAILED CreateSubmixVoice _FilterVoice!");
      return;
    }
...
CReverbEffectXAudio2::CReverbEffectXAudio2(CSoundDriverXAudio2 *soundDriver)
  ...
  // FIXME: Reverb must be within [20000, 48000] Hz and has limited input and
...
void CReverbEffectXAudio2::setEnvironment(const CEnvironment &environment,
  ...
  _ReverbParams.RearDelay =
      (BYTE)((float)XAUDIO2FX_REVERB_DEFAULT_REAR_DELAY * invScaleRate);
  ...
  clamp(_ReverbParams.RoomFilterFreq, XAUDIO2FX_REVERB_MIN_ROOM_FILTER_FREQ,
  ...
  clamp(_ReverbParams.RoomSize, XAUDIO2FX_REVERB_MIN_ROOM_SIZE,
  ...
  if (env.DecayHFRatio >= 1.0f) {
    ...
    clamp(index, XAUDIO2FX_REVERB_MIN_LOW_EQ_GAIN,
          XAUDIO2FX_REVERB_MAX_LOW_EQ_GAIN);
    _ReverbParams.LowEQGain = (BYTE)index;
    _ReverbParams.DecayTime =
        std::max(0.1f, env.DecayTime * env.DecayHFRatio * invScaleRate);
  } else {
    _ReverbParams.LowEQGain = XAUDIO2FX_REVERB_DEFAULT_LOW_EQ_GAIN;
    sint32 index = (sint32)(log10f(env.DecayHFRatio) * 4.0f) + 8;
    clamp(index, XAUDIO2FX_REVERB_MIN_HIGH_EQ_GAIN,
  ...
  sint32 reflections_delay =
      (sint32)(env.ReflectionsDelay * invScaleRate * 1000.0f);
  clamp(reflections_delay, XAUDIO2FX_REVERB_MIN_REFLECTIONS_DELAY + 1,
        XAUDIO2FX_REVERB_MAX_REFLECTIONS_DELAY - 1);
  ...
  clamp(reverb_delay, XAUDIO2FX_REVERB_MIN_REVERB_DELAY + 1,
        XAUDIO2FX_REVERB_MAX_REVERB_DELAY - 1);

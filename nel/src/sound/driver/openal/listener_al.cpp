...
const NLMISC::CVector &CListenerAL::getPos() const {
  ...
  // versions ! alGetListener3f() is part of the spec, though.
  /*
  #ifdef NL_OS_WINDOWS
      // Coordsys conversion
      float glposz;
 ...
 */
void CListenerAL::setOrientation(const NLMISC::CVector &front,
 ...
 */
void CListenerAL::getOrientation(NLMISC::CVector &front,
...
void CListenerAL::setEnvironment(uint env, float size) {
  if (AlExtEax) {
    eaxSet(&DSPROPSETID_EAX_ListenerProperties,
           DSPROPERTY_EAXLISTENER_ENVIRONMENT, 0, &env, sizeof(unsigned long));
    eaxSet(&DSPROPSETID_EAX_ListenerProperties,
           DSPROPERTY_EAXLISTENER_ENVIRONMENTSIZE, 0, &size, sizeof(float));
  }
#else
void CListenerAL::setEnvironment(uint /* env */, float /* size */) {
#endif
...
#else
void CListenerAL::setEAXProperty(uint /* prop */, void * /* value */,
                                 uint /* valuesize */) {
#endif
}

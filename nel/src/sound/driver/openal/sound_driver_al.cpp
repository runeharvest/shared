...
// Currently, the OpenAL headers are different between Windows and Linux
// versions ! AL_INVALID_XXXX are part of the spec, though.
/*#ifdef NL_OS_UNIX
...
void alTestError() {
  ...
  default:
    nlerror("OpenAL: Unknown error %x", errcode);
  }
}
#endif
...
uint32 NLSOUND_interfaceVersion() { return ISoundDriver::InterfaceVersion; }
...
CSoundDriverAL::~CSoundDriverAL() {
  ...
  if (!_Sources.empty()) {
    ...
    for (; it != end; ++it)
      (*it)->release(); // CSourceAL will be deleted by user
    _Sources.clear();
  }
  if (!_Buffers.empty())
    alDeleteBuffers(compactAliveNames(_Buffers, alIsBuffer),
                    &*_Buffers.begin());
  ...
  if (!_Effects.empty()) {
    ...
    for (; it != end; ++it)
      (*it)->release(); // CEffectAL will be deleted by user
    _Effects.clear();
  }
...
void CSoundDriverAL::initDevice(const std::string &device,
  ...
  const sint supportedOptions =
      ...
      OptionHasBufferStreaming | OptionReverbEffect | OptionFilterEffect;
  ...
  nldebug("AL: ALC_DEVICE_SPECIFIER: '%s'",
          alcGetString(_AlDevice, ALC_DEVICE_SPECIFIER));
  ...
  alTestError();
  nldebug("AL: AL_VERSION: '%s', AL_RENDERER: '%s', AL_VENDOR: '%s'", alversion,
          alrenderer, alvendor);
  ...
  nldebug("AL: Max. sources: %u, Max. effects: %u", (uint32)countMaxSources(),
          (uint32)countMaxEffects());
  ...
  allocateNewItems(alGenBuffers, alIsBuffer, _Buffers, _NbExpBuffers,
                   INITIAL_BUFFERS);
  alTestError();
}
...
/// this driver).
ISoundDriver::TSoundOptions CSoundDriverAL::getOptions() { return _Options; }
 ...
 */
void CSoundDriverAL::allocateNewItems(TGenFunctionAL algenfunc,
                                      TTestFunctionAL altestfunc,
                                      vector<ALuint> &names, uint index,
                                      uint nb) {
 ...
 */
void CSoundDriverAL::generateItems(TGenFunctionAL algenfunc,
                                   TTestFunctionAL altestfunc, uint nb,
                                   ALuint *array) {
...
IBuffer *CSoundDriverAL::createBuffer() {
  CBufferAL *buffer = new CBufferAL(createItem(
      alGenBuffers, alIsBuffer, _Buffers, _NbExpBuffers, BUFFER_ALLOC_RATE));
  return buffer;
}
...
IReverbEffect *CSoundDriverAL::createReverbEffect() {
  ...
  if (alGetError() != AL_NO_ERROR) {
    nlinfo("AL: Creative Reverb Effect not supported, falling back to standard "
           "Reverb Effect");
  } else {
    ...
    CCreativeReverbEffectAL *eff =
        new CCreativeReverbEffectAL(this, effect, slot);
  ...
  if (alGetError() != AL_NO_ERROR) {
    ...
    alAuxiliaryEffectSloti(slot, AL_EFFECTSLOT_AUXILIARY_SEND_AUTO, AL_TRUE);
    alTestError(); // auto only for reverb!
    CStandardReverbEffectAL *eff =
        new CStandardReverbEffectAL(this, effect, slot);
    ieffect = static_cast<IReverbEffect *>(eff);
    effectal = static_cast<CEffectAL *>(eff);
    nlassert(ieffect);
    nlassert(effectal);
...
/// Return the maximum number of effects that can be created, which is only 1 in
/// openal software mode :(
 ...
 */
ALuint CSoundDriverAL::createItem(TGenFunctionAL algenfunc,
                                  TTestFunctionAL altestfunc,
                                  vector<ALuint> &names, uint &index,
                                  uint allocrate) {
  nlassert(index <= names.size());
  if (index == names.size()) {
    ...
    if (nbalive == names.size()) {
      ...
      // FIXME assumption about inner workings of std::vector;
      // &(names[...]) only works with "names.size() - nbalive == 1"
      generateItems(algenfunc, altestfunc, (uint)names.size() - nbalive,
                    &(names[nbalive]));
    }
  }
 ...
 */
uint CSoundDriverAL::compactAliveNames(vector<ALuint> &names,
                                       TTestFunctionAL altestfunc) {
  vector<ALuint>::iterator iball, ibcompacted;
  for (iball = names.begin(), ibcompacted = names.begin(); iball != names.end();
       ++iball) {
...
void CSoundDriverAL::commit3DChanges() {
  ...
  if (getOption(OptionManualRolloff)) {
    for (std::set<CSourceAL *>::iterator it(_Sources.begin()),
         end(_Sources.end());
         it != end; ++it)
...
void CSoundDriverAL::writeProfile(std::string &out) {
  ...
  // TODO: write other useful information like OpenAL version and supported
  // extensions
}

// Does not create a sound loader .. what does it do then?
void CSoundDriverAL::startBench() { NLMISC::CHTimer::startBench(); }

void CSoundDriverAL::endBench() { NLMISC::CHTimer::endBench(); }
...
void CSoundDriverAL::removeSource(CSourceAL *source) {
  if (_Sources.find(source) != _Sources.end())
    ...
    nlwarning("AL: removeSource already called");
}
...
void CSoundDriverAL::removeEffect(CEffectAL *effect) {
  if (_Effects.find(effect) != _Effects.end())
    ...
    nlwarning("AL: removeEffect already called");
}

/// Delete a buffer or a source
bool CSoundDriverAL::deleteItem(ALuint name, TDeleteFunctionAL aldeletefunc,
                                vector<ALuint> &names) {

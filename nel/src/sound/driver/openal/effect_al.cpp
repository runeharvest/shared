...
CEffectAL::CEffectAL(CSoundDriverAL *soundDriver, ALuint alEffect,
                     ALuint alAuxEffectSlot)
    : _SoundDriver(soundDriver), _AlEffect(alEffect),
...
CEffectAL::~CEffectAL() {
  ...
  release();
  if (soundDriver)
    soundDriver->removeEffect(this);
}

void CEffectAL::release() {
  if (_AlAuxEffectSlot != AL_EFFECTSLOT_NULL) {
    alDeleteAuxiliaryEffectSlots(1, &_AlAuxEffectSlot);
    _AlAuxEffectSlot = AL_EFFECTSLOT_NULL;
  }
  if (_AlEffect != AL_EFFECT_NULL) {
    alDeleteEffects(1, &_AlEffect);
    _AlEffect = AL_EFFECT_NULL;
  }
  _SoundDriver = NULL;
}
...
CStandardReverbEffectAL::CStandardReverbEffectAL(CSoundDriverAL *soundDriver,
                                                 ALuint alEffect,
                                                 ALuint alAuxEffectSlot)
    : CEffectAL(soundDriver, alEffect, alAuxEffectSlot) {
...
void CStandardReverbEffectAL::setEnvironment(const CEnvironment &environment,
  ...
  alEffectf(_AlEffect, AL_REVERB_DENSITY, env.Density / 100.0f);
  alTestWarning("AL_REVERB_DENSITY");
  alEffectf(_AlEffect, AL_REVERB_DIFFUSION, env.Diffusion / 100.0f);
  alTestWarning("AL_REVERB_DIFFUSION");
  alEffectf(_AlEffect, AL_REVERB_GAIN,
            decibelsToAmplitudeRatio(env.RoomFilter));
  alTestWarning("AL_REVERB_GAIN");
  alEffectf(_AlEffect, AL_REVERB_GAINHF,
            decibelsToAmplitudeRatio(env.RoomFilterHF));
  alTestWarning("AL_REVERB_GAINHF");
  alEffectf(_AlEffect, AL_REVERB_DECAY_TIME, env.DecayTime);
  alTestWarning("AL_REVERB_DECAY_TIME");
  alEffectf(_AlEffect, AL_REVERB_DECAY_HFRATIO, env.DecayHFRatio);
  alTestWarning("AL_REVERB_DECAY_HFRATIO");
  alEffectf(_AlEffect, AL_REVERB_REFLECTIONS_GAIN,
            decibelsToAmplitudeRatio(env.Reflections));
  alTestWarning("AL_REVERB_REFLECTIONS_GAIN");
  alEffectf(_AlEffect, AL_REVERB_REFLECTIONS_DELAY, env.ReflectionsDelay);
  alTestWarning("AL_REVERB_REFLECTIONS_DELAY");
  alEffectf(_AlEffect, AL_REVERB_LATE_REVERB_GAIN,
            decibelsToAmplitudeRatio(env.LateReverb));
  alTestWarning("AL_REVERB_LATE_REVERB_GAIN");
  alEffectf(_AlEffect, AL_REVERB_LATE_REVERB_DELAY, env.LateReverbDelay);
  ...
  alEffecti(_AlEffect, AL_REVERB_DECAY_HFLIMIT,
            env.Flags & NLSOUND_ENVIRONMENT_DECAY_HF_LIMIT);
  alTestWarning("AL_REVERB_DECAY_HFLIMIT");
}
...
CCreativeReverbEffectAL::CCreativeReverbEffectAL(CSoundDriverAL *soundDriver,
                                                 ALuint alEffect,
                                                 ALuint alAuxEffectSlot)
    : CEffectAL(soundDriver, alEffect, alAuxEffectSlot) {
  // set default environment
...
void CCreativeReverbEffectAL::setEnvironment(const CEnvironment &environment,
  ...
  alEffectf(_AlEffect, AL_EAXREVERB_DECAY_HFRATIO,
            efxcreativereverb.flDecayHFRatio);
  alTestWarning("AL_EAXREVERB_DECAY_HFRATIO");
  alEffectf(_AlEffect, AL_EAXREVERB_DECAY_LFRATIO,
            efxcreativereverb.flDecayLFRatio);
  alTestWarning("AL_EAXREVERB_DECAY_LFRATIO");
  alEffectf(_AlEffect, AL_EAXREVERB_REFLECTIONS_GAIN,
            efxcreativereverb.flReflectionsGain);
  alTestWarning("AL_EAXREVERB_REFLECTIONS_GAIN");
  alEffectf(_AlEffect, AL_EAXREVERB_REFLECTIONS_DELAY,
            efxcreativereverb.flReflectionsDelay);
  alTestWarning("AL_EAXREVERB_REFLECTIONS_DELAY");
  alEffectfv(_AlEffect, AL_EAXREVERB_REFLECTIONS_PAN,
             efxcreativereverb.flReflectionsPan);
  alTestWarning("AL_EAXREVERB_REFLECTIONS_PAN");
  alEffectf(_AlEffect, AL_EAXREVERB_LATE_REVERB_GAIN,
            efxcreativereverb.flLateReverbGain);
  alTestWarning("AL_EAXREVERB_LATE_REVERB_GAIN");
  alEffectf(_AlEffect, AL_EAXREVERB_LATE_REVERB_DELAY,
            efxcreativereverb.flLateReverbDelay);
  alTestWarning("AL_EAXREVERB_LATE_REVERB_DELAY");
  alEffectfv(_AlEffect, AL_EAXREVERB_LATE_REVERB_PAN,
             efxcreativereverb.flLateReverbPan);
  ...
  alEffectf(_AlEffect, AL_EAXREVERB_MODULATION_TIME,
            efxcreativereverb.flModulationTime);
  alTestWarning("AL_EAXREVERB_MODULATION_TIME");
  alEffectf(_AlEffect, AL_EAXREVERB_MODULATION_DEPTH,
            efxcreativereverb.flModulationDepth);
  alTestWarning("AL_EAXREVERB_MODULATION_DEPTH");
  alEffectf(_AlEffect, AL_EAXREVERB_AIR_ABSORPTION_GAINHF,
            efxcreativereverb.flAirAbsorptionGainHF);
  alTestWarning("AL_EAXREVERB_AIR_ABSORPTION_GAINHF");
  alEffectf(_AlEffect, AL_EAXREVERB_HFREFERENCE,
            efxcreativereverb.flHFReference);
  alTestWarning("AL_EAXREVERB_HFREFERENCE");
  alEffectf(_AlEffect, AL_EAXREVERB_LFREFERENCE,
            efxcreativereverb.flLFReference);
  alTestWarning("AL_EAXREVERB_LFREFERENCE");
  alEffectf(_AlEffect, AL_EAXREVERB_ROOM_ROLLOFF_FACTOR,
            efxcreativereverb.flRoomRolloffFactor);
  alTestWarning("AL_EAXREVERB_ROOM_ROLLOFF_FACTOR");
  alEffecti(_AlEffect, AL_EAXREVERB_DECAY_HFLIMIT,
            efxcreativereverb.iDecayHFLimit);
  alTestWarning("AL_EAXREVERB_DECAY_HFLIMIT"); // note: spec says
                                               // AL_EAXREVERB_DECAYHF_LIMIT
}

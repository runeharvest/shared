...
CBufferAL::CBufferAL(ALuint buffername)
    : IBuffer(), _BufferName(buffername), _Name(NULL),
      _SampleFormat(AL_INVALID), _Frequency(0), _DataAligned(NULL),
      _DataPtr(NULL), _Capacity(0), _Size(0),
      _StorageMode(IBuffer::StorageAuto), _IsLoaded(false) {}
 ...
 */
void CBufferAL::setName(NLMISC::TStringId bufferName) { _Name = bufferName; }

/// Set the sample format. (channels = 1, 2, ...; bitsPerSample = 8, 16;
/// frequency = samples per second, 44100, ...)
void CBufferAL::setFormat(TBufferFormat format, uint8 channels,
  ...
  switch (sampleFormat) {
  case Mono8:
    _SampleFormat = AL_FORMAT_MONO8;
    break;
  case Mono16:
    _SampleFormat = AL_FORMAT_MONO16;
    break;
  case Stereo8:
    _SampleFormat = AL_FORMAT_STEREO8;
    break;
  case Stereo16:
    _SampleFormat = AL_FORMAT_STEREO16;
    break;
  default:
    nlstop;
...
bool CBufferAL::unlock(uint size) {
  ...
  if (_StorageMode != IBuffer::StorageSoftware &&
      !CSoundDriverAL::getInstance()->getOption(
...
bool CBufferAL::fill(const uint8 *src, uint size) {
  ...
  bool localBufferCopy = CSoundDriverAL::getInstance()->getOption(
      ISoundDriver::OptionLocalBufferCopy);
...
void CBufferAL::getFormat(TBufferFormat &format, uint8 &channels,
...
float CBufferAL::getDuration() const {
  ...
  return (float)(getSize()) * 1000.0f / (float)_Frequency /
...
bool CBufferAL::isStereo() const {
  return (_SampleFormat == AL_FORMAT_STEREO8) ||
         (_SampleFormat == AL_FORMAT_STEREO16);
}

/// Return the name of this buffer
NLMISC::TStringId CBufferAL::getName() const { return _Name; }

bool CBufferAL::isBufferLoaded() const { return _IsLoaded; }
...
IBuffer::TStorageMode CBufferAL::getStorageMode() { return _StorageMode; }

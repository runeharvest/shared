...
CSourceAL::CSourceAL(CSoundDriverAL *soundDriver)
    : _SoundDriver(NULL), _Buffer(NULL), _Source(AL_NONE),
      ...
      _RelativeMode(false), _Pos(0.0f, 0.0f, 0.0f), _Gain(NLSOUND_DEFAULT_GAIN),
      _Alpha(1.0), _MinDistance(1.0f),
      _MaxDistance(sqrt(numeric_limits<float>::max())), _Effect(NULL),
      _Direct(true), _DirectGain(NLSOUND_DEFAULT_DIRECT_GAIN),
      _EffectGain(NLSOUND_DEFAULT_EFFECT_GAIN),
      _DirectFilterType(ISource::FilterLowPass),
      _EffectFilterType(ISource::FilterLowPass), _DirectFilterEnabled(false),
  ...
  if (soundDriver->getOption(ISoundDriver::OptionFilterEffect)) {
    ...
    alFilterf(_DirectFilter, AL_LOWPASS_GAINHF,
              NLSOUND_DEFAULT_FILTER_PASS_GAIN);
    alTestError();
    ...
    alFilterf(_EffectFilter, AL_LOWPASS_GAINHF,
              NLSOUND_DEFAULT_FILTER_PASS_GAIN);
    alTestError();
  }
...
void CSourceAL::updateManualRolloff() {
  CVector distanceVector =
      _RelativeMode ? _Pos : (_Pos - CListenerAL::getInstance()->getPos());
  float distanceSquare = distanceVector.sqrnorm();
  float rolloff = ISource::computeManualRolloff(_Alpha, distanceSquare,
  ...
  nlwarning("Called updateManualRolloff(), physical gain is %f, configured "
            "gain is %f, distanceSquare is %f, rolloff is %f",
            gain, _Gain, distanceSquare, rolloff);
  alTestError();
#endif
}
 ...
 * If the buffer is stereo, the source mode becomes stereo and the source
 * relative mode is on, otherwise the source is considered as a 3D source.
 */
...
IBuffer *CSourceAL::getStaticBuffer() { return _Buffer; }
...
void CSourceAL::submitStreamingBuffer(IBuffer *buffer) {
  ...
  if (!bufferAL->isBufferLoaded()) {
    nlwarning("AL: MUSICBUG: Streaming buffer was not loaded, skipping buffer. "
  ...
  // Resume playback if the internal OpenAL source stopped due to buffer
  // underrun.
  ALint srcstate;
...
uint CSourceAL::countStreamingBuffers() const {
  // a bit ugly here, but makes a much easier/simpler implementation on both
  // drivers
...
bool CSourceAL::play() {
  ...
  if (_Buffer) {
    ...
    if (!_IsPlaying) // set start time if not playing yet
      _StartTime = CTime::getLocalTime();
    _IsPlaying = true; // this play always virtually succeed but may not
                       // actually be playing
    if (!_QueuedBuffers.empty()) // ensure buffers have actually queued
    {
      alSourcePlay(_Source);
      if (alGetError() != AL_NO_ERROR) {
        nlwarning("AL: MUSICBUG: Unknown error while trying to play streaming "
                  "source.");
      }
    } else {
    ...
    _IsPlaying = (alGetError() == AL_NO_ERROR);
    if (_IsPlaying)
        _StartTime = CTime::getLocalTime(); // TODO: Played time should freeze
    when buffering fails, and be calculated based on the number of buffers
    played plus passed time. This is necessary for synchronizing animation with
    sound. return _IsPlaying;
    */
    // Streaming mode
...
void CSourceAL::pause() {
  if (_Buffer != NULL) {
    if (_IsPaused)
      nlwarning("AL: Called pause() while _IsPaused == true!");
...
const NLMISC::CVector &CSourceAL::getPos() const { return _Pos; }
...
void CSourceAL::setGain(float gain) {
  ...
  if (!_SoundDriver->getOption(ISoundDriver::OptionManualRolloff)) {
    ...
    nlwarning("Called setGain(), manual rolloff, commit deferred to play or "
              "update, physical gain is %f, configured gain is %f",
              gain, _Gain);
  }
#endif
}
...
float CSourceAL::getGain() const {
  ...
  ALfloat gain;
  alGetSourcef(_Source, AL_GAIN, &gain);
  nlwarning("Called getGain(), physical gain is %f, configured gain is %f",
            gain, _Gain);
  alTestError();
#endif
  return _Gain;
...
void CSourceAL::setPitch(float pitch) {
  alSourcef(_Source, AL_PITCH,
            std::min(std::max(pitch, NLSOUND_MIN_PITCH), NLSOUND_MAX_PITCH));
  alTestError();
}
...
void CSourceAL::setMinMaxDistances(float mindist, float maxdist,
  ...
  if (maxdist >= maxSqrt) {
    nlwarning(
        "SOUND_DEV (OpenAL): Ridiculously high max distance set on source");
    maxdist = maxSqrt;
  }
...
void CSourceAL::getCone(float &innerAngle, float &outerAngle,
...
bool CSourceAL::getDirect() const { return _Direct; }
...
float CSourceAL::getDirectGain() const { return _DirectGain; }
...
bool CSourceAL::isDirectFilterEnabled() const { return _DirectFilterEnabled; }

/// Set the filter parameters for the direct channel
void CSourceAL::setDirectFilter(TFilter filterType, float /* lowFrequency */,
...
void CSourceAL::getDirectFilter(TFilter &filterType, float &lowFrequency,
                                float &highFrequency, float &passGain) const {
...
void CSourceAL::setupEffectFilter() {
  if (_Effect && _EffectGain > 0) {
    if (_EffectGain < 1 ||
        (_EffectFilterPassGain < 1 && _EffectFilterEnabled)) {
      ...
      alSource3i(_Source, AL_AUXILIARY_SEND_FILTER, _Effect->getAuxEffectSlot(),
                 0, _EffectFilter);
    } else {
      // no filtering
      alSource3i(_Source, AL_AUXILIARY_SEND_FILTER, _Effect->getAuxEffectSlot(),
    ...
    // mute
    alSource3i(_Source, AL_AUXILIARY_SEND_FILTER, AL_EFFECTSLOT_NULL, 0,
               AL_FILTER_NULL);
  }
  alTestError();
}
...
float CSourceAL::getEffectGain() const { return _EffectGain; }
...
bool CSourceAL::isEffectFilterEnabled() const { return _EffectFilterEnabled; }

/// Set the filter parameters for the effect channel
void CSourceAL::setEffectFilter(TFilter filterType, float /* lowFrequency */,
                                float /* highFrequency */, float passGain) {
...
void CSourceAL::getEffectFilter(TFilter &filterType, float &lowFrequency,
                                float &highFrequency, float &passGain) const {

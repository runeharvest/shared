...
void IBuffer::sampleFormatToBufferFormat(TSampleFormat sampleFormat,
                                         TBufferFormat &bufferFormat,
                                         uint8 &channels,
                                         uint8 &bitsPerSample) {
  switch (sampleFormat) {
...
void IBuffer::bufferFormatToSampleFormat(TBufferFormat bufferFormat,
...
uint IBuffer::getPCMSizeFromDuration(float duration, uint8 channels,
                                     uint8 bitsPerSample, uint32 frequency) {
  return (uint)(duration * ((float)frequency) *
                (((float)bitsPerSample) / 8.0f) * ((float)channels));
}

float IBuffer::getDurationFromPCMSize(uint size, uint8 channels,
                                      uint8 bitsPerSample, uint32 frequency) {
  return ((float)size) / ((float)channels) / (((float)bitsPerSample) / 8.0f) /
         ((float)frequency);
}
...
void IBuffer::encodeADPCM(const sint16 *indata, uint8 *outdata, uint nbSample,
                          TADPCMState &state) {
...
void IBuffer::decodeADPCM(const uint8 *indata, sint16 *outdata, uint nbSample,
                          TADPCMState &state) {
...
static bool readHeader(const uint8 *header, const char *fourcc, uint32 &size,
                       const uint8 *&data) {
  memcpy(&size, header + 4, sizeof(uint32));
  data = header + 8;
  return (header[0] == fourcc[0] && header[1] == fourcc[1] &&
          header[2] == fourcc[2] && header[3] == fourcc[3]);
}

static bool findChunk(const uint8 *src, uint32 srcSize, const char *fourcc,
...
bool IBuffer::readWav(const uint8 *wav, uint size, std::vector<uint8> &result,
  ...
  if (dataData + dataSize > wav + size) {
    ...
    nlwarning("WAV: Oversize 'data' chunk with dataSize %u and wav size %u, "
              "cutting %u bytes",
              (uint32)dataSize, (uint32)size, (uint32)cut);
    dataSize -= cut;
  }
...
bool IBuffer::writeWav(const uint8 *buffer, uint size,
                       TBufferFormat bufferFormat, uint8 channels,
                       uint8 bitsPerSample, uint32 frequency,
                       NLMISC::IStream &out) {
  ...
  out.serialBuffer(const_cast<uint8 *>(static_cast<const uint8 *>(
...
bool IBuffer::convertToMono16PCM(const uint8 *buffer, uint size,
                                 std::vector<sint16> &result,
                                 TBufferFormat bufferFormat, uint8 channels,
                                 uint8 bitsPerSample) {
  if (size == 0 || channels == 0 || bitsPerSample == 0)
    return false;

  switch (bufferFormat) {
  case FormatPcm: {
    result.resize((std::vector<sint16>::size_type)(
        ((uint64)size / (uint64)channels) * 8UL / (uint64)bitsPerSample));
...
bool IBuffer::convertMono16PCMToMonoADPCM(const sint16 *buffer, uint samples,

...
sint32 ISource::computeManualRollOff(sint32 volumeMB, sint32 mbMin,
  ...
  if (sqrdist < distMin * distMin) {
    ...
    if (alpha == 0.0) {
      ...
      double amp2 =
          0.0001 +
          0.9999 * (distMax - dist) /
              (distMax - distMin);       // linear amp between 0.00001 and 1.0
...
float ISource::computeManualRolloff(double alpha, float sqrdist, float distMin,
                                    float distMax) {
  /*static const sint mbMin = -10000;
  static const sint mbMax = 0;
  sint32 rolloffMb = ISource::computeManualRollOff(mbMax, mbMin, mbMax, alpha,
  sqrdist, distMin, distMax); float rolloffGain = (float)pow(10.0,
  (double)rolloffMb / 2000.0); clamp(rolloffGain, 0.0f, 1.0f); return
  rolloffGain;*/

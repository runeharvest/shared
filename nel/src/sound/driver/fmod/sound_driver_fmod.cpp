...
CSoundDriverFMod::CSoundDriverFMod(
    ISoundDriver::IStringMapperProvider *stringMapper)
    : _StringMapper(stringMapper), _FModOk(false), _MasterGain(1.f),
...
CSoundDriverFMod::~CSoundDriverFMod() {
  ...
  {
    set<CMusicChannelFMod *>::iterator it(_MusicChannels.begin()),
        end(_MusicChannels.end());
    for (; it != end; ++it) {
  ...
  // Assure that the remaining sources have released all their channels before
  // closing
  ...
  // Assure that the listener has released all resources before closing down
  // FMod
...
void CSoundDriverFMod::initDevice(const std::string &device,
  ...
  nlinfo("FMod Hardware Support: %d 2D channels, %d 3D channels, %d Total "
         "Channels",
         num2D, num3D, numTotal);
}

/// Return options that are enabled (including those that cannot be disabled on
/// this driver).
ISoundDriver::TSoundOptions CSoundDriverFMod::getOptions() { return _Options; }
...
void CSoundDriverFMod::commit3DChanges() {
  ...
  // We handle the "SourceRelative state" ourselves. Updates sources according
  // to current listener position/velocity
  set<CSourceFMod *>::iterator iter;
  for (iter = _Sources.begin(); iter != _Sources.end(); iter++) {
...
float CSoundDriverFMod::getGain() { return _MasterGain; }

// ***************************************************************************
void CSoundDriverFMod::startBench() { NLMISC::CHTimer::startBench(); }
void CSoundDriverFMod::endBench() { NLMISC::CHTimer::endBench(); }
 ...
 * not implemented. \param filepath full path to file \param artist returns the
 * song artist (empty if not available) \param title returns the title (empty if
 * not available)
 */
bool CSoundDriverFMod::getMusicInfo(const std::string &filepath,
  ...
  FSOUND_STREAM *stream = FSOUND_Stream_Open(filepath.c_str(), FSOUND_2D,
                                             (sint)fileOffset, (sint)fileSize);
  if (stream) {
...
void CSoundDriverFMod::markMusicChannelEnded(void *stream,
                                             CMusicChannelFMod *musicChannel) {
  // verify if it exists
  set<CMusicChannelFMod *>::iterator it(_MusicChannels.find(musicChannel));
  if (it != _MusicChannels.end())
    musicChannel->markMusicChannelEnded(stream);
}
...
bool CSoundDriverFMod::isMusicExtensionSupported(
    const std::string &extension) const {
  return (extension == "ogg") || (extension == "mp3") || (extension == "mp2") ||
         (extension == "mp1") || (extension == "wav") || (extension == "raw");
}

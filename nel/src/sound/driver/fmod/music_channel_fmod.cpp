...
signed char F_CALLBACKAPI streamEndCallBack(FSOUND_STREAM *stream,
                                            void * /* buff */, int /* len */,
                                            void *userdata) {
  // Avoid any problem, check that the sound driver is still allocated
  if (!CSoundDriverFMod::getInstance())
    return false;

  // mark this fader as music ended
  CSoundDriverFMod::getInstance()->markMusicChannelEnded(
      stream, static_cast<CMusicChannelFMod *>(userdata));

  return true;
}

CMusicChannelFMod::CMusicChannelFMod(CSoundDriverFMod *soundDriver)
    : _Gain(1.0f), _MusicStream(NULL), _MusicBuffer(NULL), _MusicChannel(-1),
      _CallBackEnded(false), _SoundDriver(soundDriver) {}
...
/// else just path to file with fileSize 0.
bool CMusicChannelFMod::playAsync(const std::string &filepath, bool loop,
                                  uint fileOffset, uint fileSize) {
  nlassert(!_MusicBuffer);

  // open fmod stream async
  _MusicStream = FSOUND_Stream_Open(
      (const char *)filepath.c_str(),
      FSOUND_2D | (loop ? FSOUND_LOOP_NORMAL : FSOUND_LOOP_OFF) |
          FSOUND_NONBLOCKING,
      fileOffset, fileSize);
  nlassert(_MusicStream);

  // with FSOUND_NONBLOCKING, the file is surely not ready, but still try now
  // (will retry to replay at each updateMusic())
  playStream();
...
bool CMusicChannelFMod::playSync(const std::string &filepath, bool loop) {
  ...
  // open FMOD stream
  _MusicStream =
      FSOUND_Stream_Open((const char *)_MusicBuffer,
                         FSOUND_2D | FSOUND_LOADMEMORY |
                             (loop ? FSOUND_LOOP_NORMAL : FSOUND_LOOP_OFF),
                         0, fs);
  if (!_MusicStream) {
    nlwarning("NLSOUND FMod Driver: Error while creating the FMOD stream for "
              "music file");
    delete[] _MusicBuffer;
    _MusicBuffer = NULL;
    return false;
  ...
  if (!playStream()) {
    nlwarning(
        "NLSOUND FMod Driver: Error While trying to play sync music file");
...
bool CMusicChannelFMod::playStream() {
  if (FSOUND_Stream_GetOpenState(_MusicStream) == -3) {
    nlwarning("NLSOUND FMod Driver: stream failed to open. (file not found, "
  ...
  // Start playing
  if ((_MusicChannel =
           FSOUND_Stream_PlayEx(FSOUND_FREE, _MusicStream, NULL, true)) == -1)
    return false;
  ...
  FSOUND_Stream_SetEndCallback(_MusicStream, streamEndCallBack,
...
void CMusicChannelFMod::update() {
  // if this channel is playing an async music, may retry to start the music
  // each frame
 ...
 */
bool CMusicChannelFMod::play(const std::string &filepath, bool async,
                             bool loop) {
  // stop
  stop();
  ...
  if (pathName.empty()) {
    nlwarning("NLSOUND FMod Driver: Music file %s not found!",
              filepath.c_str());
    return false;
  }
  if (async) {
    ...
    if (pathName.find('@') != string::npos) {
      ...
      if (CBigFile::getInstance().getFileInfo(pathName, fileSize, fileOffset)) {
        ...
        return playAsync(CPath::lookup(bnpName, false), loop, fileOffset,
                         fileSize);
      } else {
...
void CMusicChannelFMod::stop() {
  if (_MusicStream) {
    /* just append this channel for closing. We have to maintain such a list
       because in case of async playing, FMod FSOUND_Stream_Stop() and
       FSOUND_Stream_Close() calls fail if the file is not ready (hapens if
       music stopped in the 50 ms after the play for instance)
    */
    _WaitingForClose.push_back(_MusicStream);

    // force stop now (succeed in 99% of case, and if not succeed, it means the
    // play has not yet begun, so no problem)
    updateWaitingForClose();
...
bool CMusicChannelFMod::isEnded() {
  if (_MusicStream && _MusicChannel != -1) {
    ...
    if ((int)FSOUND_Stream_GetPosition(_MusicStream) ==
        FSOUND_Stream_GetLength(_MusicStream))
      return true;

    // NB: the preceding code don't work with .ogg vorbis encoded mp3. Thus test
    // also the end with a callback
    if (_CallBackEnded)
      return true;
  }
  // if playing, but not starting because of async, not ended (because not even
  // really started)
  else if (_MusicStream)
    return false;

...
CSourceFMod::CSourceFMod(uint sourcename) : ISource(), _SourceName(sourcename) {
  _Sample = NULL;
  _NextSample = NULL;
...
uint32 CSourceFMod::getTime() {
  ...
  return uint32(1000.0f * FSOUND_GetCurrentPosition(_FModChannel) /
                (float)freq);
}
...
bool CSourceFMod::play() {
  ...
  if (_Sample) {
    ...
    if (bufFMod->_FModSample)
      _FModChannel =
          FSOUND_PlaySoundEx(FSOUND_FREE, bufFMod->_FModSample, NULL, true);

    // Update all setup for this channel
    if (_FModChannel != -1) {
      FSOUND_SetLoopMode(_FModChannel,
                         _Loop ? FSOUND_LOOP_NORMAL : FSOUND_LOOP_OFF);
      ...
      if (!CSoundDriverFMod::getInstance()->getOption(
...
bool CSourceFMod::getLooping() const { return _Loop; }
...
const NLMISC::CVector &CSourceFMod::getPos() const { return _Pos; }
...
void CSourceFMod::getVelocity(NLMISC::CVector &vel) const { vel = _Vel; }
...
void CSourceFMod::getDirection(NLMISC::CVector &dir) const { dir = _Front; }
...
// ***************************************************************************
float CSourceFMod::getGain() const { return _Gain; }
...
// ******************************************************************
float CSourceFMod::getPitch() const { return _Pitch; }
...
bool CSourceFMod::getSourceRelativeMode() const { return _PosRelative; }

// ******************************************************************
void CSourceFMod::setMinMaxDistances(float mindist, float maxdist,
                                     bool /* deferred */) {
...
void CSourceFMod::updateVolume(const NLMISC::CVector &listener) {
  nlassert(CSoundDriverFMod::getInstance()->getOption(
      ISoundDriver::OptionManualRolloff));
  ...
  // sint32 volumeDB= sint32(floor(2000.0 * log10(_Gain))); // convert to
  // 1/100th decibels const	sint32	dbMin= -10000; const	sint32	dbMax=
  // 0; clamp(volumeDB, dbMin, dbMax);

  //// attenuate the volume according to distance and alpha
  // volumeDB= ISource::computeManualRollOff(volumeDB, dbMin, dbMax, _Alpha,
  // sqrdist);
  ...
  float rolloff =
      ISource::computeManualRolloff(_Alpha, sqrdist, _MinDist, _MaxDist);
...
void CSourceFMod::setCone(float /* innerAngle */, float /* outerAngle */,
                          float /* outerGain */) {
  // TODO_SOURCE_DIR
}
...
void CSourceFMod::getCone(float & /* innerAngle */, float & /* outerAngle */,
 ...
 */
void CSourceFMod::setAlpha(double a) { _Alpha = a; }
...
bool CSourceFMod::getDirect() const { return true; }

/// Set the gain for the direct path
void CSourceFMod::setDirectGain(float /* gain */) {}

/// Get the gain for the direct path
float CSourceFMod::getDirectGain() const { return NLSOUND_DEFAULT_DIRECT_GAIN; }
...
bool CSourceFMod::isDirectFilterEnabled() const { return false; }

/// Set the filter parameters for the direct channel
void CSourceFMod::setDirectFilter(TFilter /*filterType*/,
                                  float /*lowFrequency*/,
                                  float /*highFrequency*/, float /*passGain*/) {
}

/// Get the filter parameters for the direct channel
void CSourceFMod::getDirectFilter(TFilter &filterType, float &lowFrequency,
...
float CSourceFMod::getDirectFilterPassGain() const { return 0.0f; }
...
IEffect *CSourceFMod::getEffect() const { return NULL; }

/// Set the gain for the effect path
void CSourceFMod::setEffectGain(float /* gain */) {}

/// Get the gain for the effect path
float CSourceFMod::getEffectGain() const { return NLSOUND_DEFAULT_EFFECT_GAIN; }
...
bool CSourceFMod::isEffectFilterEnabled() const { return false; }

/// Set the filter parameters for the effect channel
void CSourceFMod::setEffectFilter(TFilter /*filterType*/,
                                  float /*lowFrequency*/,
                                  float /*highFrequency*/, float /*passGain*/) {
}

/// Get the filter parameters for the effect channel
void CSourceFMod::getEffectFilter(TFilter &filterType, float &lowFrequency,
                                  float &highFrequency, float &passGain) const {
  filterType = FilterLowPass;
  lowFrequency = NLSOUND_DEFAULT_FILTER_PASS_LF;
  highFrequency = NLSOUND_DEFAULT_FILTER_PASS_HF;
...
float CSourceFMod::getEffectFilterPassGain() const { return 0.0f; }

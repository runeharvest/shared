...
// ***************************************************************************
void CBufferFMod::setName(NLMISC::TStringId bufferName) { _Name = bufferName; }

// ***************************************************************************
/// Set the sample format. (channels = 1, 2, ...; bitsPerSample = 8, 16;
/// frequency = samples per second, 44100, ...)
void CBufferFMod::setFormat(TBufferFormat format, uint8 channels,
                            uint8 bitsPerSample, uint frequency) {
...
void CBufferFMod::getFormat(TBufferFormat &format, uint8 &channels,
                            uint8 &bitsPerSample, uint &frequency) const {
  sampleFormatToBufferFormat(_Format, format, channels, bitsPerSample);
  frequency = _Freq;
}

/// Return the size of the buffer, in bytes.
uint CBufferFMod::getSize() const { return _Size; }
...
NLMISC::TStringId CBufferFMod::getName() const { return _Name; }

/// Return true if the buffer is loaded. Used for async load/unload.
bool CBufferFMod::isBufferLoaded() const { return _FModSample != 0; }
...
void CBufferFMod::loadDataToFMod(const uint8 *data) {
  ...
  if (data) {
    ...
    // create FMod sample
    _FModSample = FSOUND_Sample_Load(FSOUND_FREE, (const char *)uploadData,
                                     commonType | type, 0, _Size);

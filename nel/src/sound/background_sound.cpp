...
CBackgroundSound::CBackgroundSound() : _Duration(0), _DurationValid(false) {}
...
void CBackgroundSound::importForm(const std::string &filename,
    ...
    if (psoundList != 0 && psoundList->isArray()) {
      ...
      for (uint i = 0; i < size; ++i) {
        ...
        if (psoundItem != NULL) {
          ...
          for (uint j = 0;
               j < UAudioMixer::TBackgroundFlags::NB_BACKGROUND_FLAGS; j++) {
            char tmp[200];
...
uint32 CBackgroundSound::getDuration() {
  ...
  std::vector<TSoundInfo>::const_iterator first(_Sounds.begin()),
      last(_Sounds.end());
...
void CBackgroundSound::getSubSoundList(
  ...
  for (; first != last; ++first) {
    CSound *sound = mixer->getSoundId(first->SoundName);
    subsounds.push_back(
        make_pair(CStringMapper::unmap(first->SoundName), sound));
  }
}

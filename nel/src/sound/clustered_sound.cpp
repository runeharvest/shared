...
public:
  ...
  void readGeorges(const NLMISC::CSmartPtr<NLGEORGES::UForm> &form,
    ...
    try {
      ...
      for (uint i = 0; i < size; ++i) {
        ...
        _SoundGroupAssoc.push_back(make_pair(CStringMapper::map(soundGroup),
  ...
  /** called by GEORGE::loadForm when a sheet read from the packed sheet is no
   *more in the directories.
   */
  void removed() {
    // nothing to do
  }
...
CClusteredSound::CClusteredSound()
    : _Scene(0), _RootCluster(0), _LastEnv(CStringMapper::emptyId()),
      _LastEnvSize(-1.0f), // size goes from 0.0f to 100.0f
      m_EnableOcclusionObstruction(false), m_EnableReverb(false) {}

void CClusteredSound::buildSheets(const std::string &packedSheetPath) {
  std::map<std::string, CSoundGroupSerializer> container;
  ::loadForm("sound_group", packedSheetPath + "sound_groups.packed_sheets",
             container, true, false);
}

void CClusteredSound::init(NL3D::CScene *scene, float portalInterpolate,
                           float maxEarDist, float minGain,
                           bool enableOcclusionObstruction, bool enableReverb) {
  ...
  m_FilterMaterials[CStringMapper::map("wood door")] =
      m_FilterMaterials[CStringMapper::map("THICKDOOR")];
  m_FilterMaterials[CStringMapper::map("brick door")] =
      m_FilterMaterials[CStringMapper::map("BRICKWALL")];
  ...
  std::map<std::string, CSoundGroupSerializer> container;
  ::loadForm("sound_group",
             CAudioMixerUser::instance()->getPackedSheetPath() +
                 "sound_groups.packed_sheets",
             container, CAudioMixerUser::instance()->getPackedSheetUpdate(),
             false);

  // copy the container data into internal structure
  std::map<std::string, CSoundGroupSerializer>::iterator first(
      container.begin()),
      last(container.end());
  for (; first != last; ++first) {
    _SoundGroupToSound.insert(first->second._SoundGroupAssoc.begin(),
                              first->second._SoundGroupAssoc.end());
  }
...
void CClusteredSound::update(const CVector &listenerPos,
                             const CVector & /* view */,
                             const CVector & /* up */) {
  ...
  // and start the cluster traversal to find out what cluster is audible and how
  // we ear it
...

  {
    ...
    //		std::map<std::string, CClusterSound>::iterator
    //first(_Sources.begin()), last(_Sources.end());
  ...
  TClusterStatusMap::const_iterator first(_AudibleClusters.begin()),
      last(_AudibleClusters.end());
  for (; first != last; ++first) {
    ...
    if (soundGroup != NO_SOUND_GROUP) {
      ...
      if (it != _Sources.end()) {
        ...
        if (cs.Distance >= css.Dist) {
          ...
          cs.Source->setPos(listenerPos + css.Direction * css.Dist +
                            CVector(0, 0, 2));
          if (css.DistFactor < 1.0f)
            cs.Source->setRelativeGain(
                css.Gain * (1.0f - (css.DistFactor * css.DistFactor *
        ...
        //				nldebug("Searching sound assoc for group
        //[%s]", CStringMapper::unmap(soundGroup).c_str());
        ...
        if (it2 != _SoundGroupToSound.end()) {
          ...
          //					nldebug("Found the sound [%s]
          //for sound group [%s]", CStringMapper::unmap(soundName).c_str(),
          //CStringMapper::unmap(soundGroup).c_str());

          cs.Distance = css.Dist;
          cs.Source = CAudioMixerUser::instance()->createSource(
              soundName, false, NULL, NULL, cluster);
          if (cs.Source != 0) {
            cs.Source->setPos(listenerPos + css.Direction * css.Dist +
                              CVector(0, 0, 2));
            if (css.DistFactor < 1.0f)
              cs.Source->setRelativeGain(
                  css.Gain *
                  (1.0f - (css.DistFactor *
                           css.DistFactor /**css.DistFactor*css.DistFactor*/)));
            else
              cs.Source->setRelativeGain(css.Gain);
  ...
  // check for source to stop
  {
    ...
    TClusterSoundCont::iterator first(newSources.begin()),
        last(newSources.end());
    for (; first != last; ++first) {
  ...
  if (mixer->useReverb() && !vCluster.empty()) {
    ...
    if (fxId != _LastEnv || size != _LastEnvSize) {
      nldebug("AM: CClusteredSound => setEnvironment %s %f",
              CStringMapper::unmap(fxId).c_str(), size);
...
const CClusteredSound::CClusterSoundStatus *
CClusteredSound::getClusterSoundStatus(NL3D::CCluster *cluster) {
...
void CClusteredSound::soundTraverse(const std::vector<CCluster *> &clusters,
  ...
  for (uint i = 0; i < clusters.size(); ++i) {
    bool valid = true;
    // eliminate cluster when listener is behind their portals AND inside the
    // other cluster
    for (uint j = 0; j < clusters[i]->getNbPortals(); j++) {
      ...
      if (poly.size() < 3) {
        ...
        if (warned.find(clusters[i]->Name) == warned.end()) {
          nlwarning(
              "Cluster [%s] contains a portal [%s] with less than 3 vertex !",
              clusters[i]->Name.c_str(),
              portal->getName().empty() ? "no name"
                                        : portal->getName().c_str());
      ...
      if ((dist < 0 && dist2 > 0) || (dist > 0 && dist2 < 0)) {
        if (portal->getCluster(0) == clusters[i]) {
          if (find(clusters.begin(), clusters.end(), portal->getCluster(1)) !=
              clusters.end()) {
            valid = false;
            continue;
          }
        } else if (find(clusters.begin(), clusters.end(),
                        portal->getCluster(0)) != clusters.end()) {
          valid = false;
          continue;
        }
      }
  ...
  do {
    ...
    std::copy(_NextTraversalStep.begin(), _NextTraversalStep.end(),
              std::back_inserter(curClusters));
    ...
    while (!curClusters.empty()) {
      ...
      for (i = 0; i < cluster->getNbPortals(); ++i) {
        ...
        if (otherCluster &&
            travContext.PreviousCluster !=
                otherCluster) // && (!travContext.FilterUnvisibleChild ||
                              // otherCluster->AudibleFromFather))
        {
          ...
          if (poly.size() < 3) {
            ...
            if (warned.find(cluster->Name) == warned.end()) {
              nlwarning("Cluster [%s] contains a portal [%s] with less than 3 "
                        "vertex !",
                        cluster->Name.c_str(),
                        portal->getName().empty() ? "no name"
                                                  : portal->getName().c_str());
            ...
            if (surface > 340 /* && otherCluster->isIn(travContext.ListenerPos, travContext.MaxDist-travContext.Dist)*/)
              ...
              minDist =
                  getAABoxNearestPos(box, travContext.ListenerPos, nearPos);
              ...
              if (travContext.Dist + minDist < _MaxEarDistance) {
                ...
                css.Direction = interpolateSourceDirection(
                    travContext, css.Dist + travContext.Dist, nearPos,
                    travContext.ListenerPos /*realListener*/, d1, d2, alpha);
                ...
                if (addAudibleCluster(otherCluster, css)) {
                  //								debugLines.push_back(CLine(travContext.ListenerPos,
                  //nearPos));
                  ...
                  stc.Alpha = alpha;
                  stc.PreviousVector =
                      (nearPos - travContext.ListenerPos).normed();
                  addNextTraverse(otherCluster, stc);
                  _AudioPath.push_back(
                      make_pair(travContext.ListenerPos, nearPos));
                }
              }
            } else {
              // find the nearest point of this portal (either on of the
              // perimeter vertex or a point on the portal surface)
              float minDist;
              CVector nearPos;
              ...
              if (travContext.Dist + minDist < _MaxEarDistance) {
                ...
                TFilterMaterialsMap::iterator it =
                    m_FilterMaterials.find(occId);

                if (m_EnableOcclusionObstruction) {
                  if (it != m_FilterMaterials.end()) {
                    // found an occlusion material for this portal
                    css.Occlusion =
                        max(NLSOUND_MIN_OCCLUSION,
                            travContext.Occlusion + it->second.Occlusion);
                    css.OcclusionLFFactor = travContext.OcclusionLFFactor *
                                            it->second.OcclusionLFFactor;
                    css.OcclusionRoomRatio = travContext.OcclusionRoomRatio *
                                             it->second.OcclusionRoomRatio;
                    css.DirectCutoffFrequency =
                        sqrtf(travContext.DirectCutoffFrequency *
                              it->second.DirectCutoffFrequency);
                    css.EffectCutoffFrequency =
                        sqrtf(travContext.EffectCutoffFrequency *
                              it->second.EffectCutoffFrequency);
                  } else {
                    ...
                    css.OcclusionRoomRatio = travContext.OcclusionRoomRatio;
                    css.DirectCutoffFrequency =
                        travContext.DirectCutoffFrequency;
                    css.EffectCutoffFrequency =
                        travContext.EffectCutoffFrequency;
                  }
                } else {
                  if (it != m_FilterMaterials.end()) {
                    // found an occlusion material for this portal
                ...
                if (travContext.NbPortal >= 1) {
                  ...
                  if (h < 0) {
                    // obst = float(2000 + asinf(-(soundDir ^
                    // travContext.PreviousVector).norm()) / (Pi/2) * 2000);
                    obst =
                        40.0f -
                        (soundDir ^ travContext.PreviousVector).norm() * 20.0f;
                  } else {
                    // obst = float(asinf((soundDir ^
                    // travContext.PreviousVector).norm()) / (Pi/2) * 2000);
                    obst =
                        (soundDir ^ travContext.PreviousVector).norm() * 20.0f;
                  }
                  ...
                  if (m_EnableOcclusionObstruction) {
                    css.Obstruction = max(NLSOUND_MIN_OBSTRUCTION,
                                          travContext.Obstruction - obst);
                    css.OcclusionLFFactor =
                        0.50f * travContext.OcclusionLFFactor;
                  } else {
                ...
                //							css.Dist
                //= travContext.Dist + float(sqrt(minDist));
                css.Dist = travContext.Dist + minDist;
                css.DistFactor = css.Dist / _MaxEarDistance;
                ...
                if (addAudibleCluster(otherCluster, css)) {
                  //								debugLines.push_back(CLine(travContext.ListenerPos,
                  //nearPoint));
                  CSoundTravContext tc(nearPos,
                                       travContext.FilterUnvisibleChild,
                                       !cluster->AudibleFromFather);
                  tc.Dist = css.Dist;
                  tc.Gain = css.Gain;
      ...
      for (i = 0; i < cluster->Children.size(); ++i) {
        ...
        if (c != travContext.PreviousCluster) {
          // clip on distance.
          if (c->AudibleFromFather &&
              c->isIn(travContext.ListenerPos,
                      _MaxEarDistance - travContext.Dist)) {
            float minDist;
            CVector nearPos;
            ...
            if (travContext.Dist + minDist < _MaxEarDistance) {
              ...
              css.EffectCutoffFrequency = travContext.EffectCutoffFrequency;
              /*							if
                 (travContext.NbPortal == 0) css.Direction = (nearPos -
                 travContext.ListenerPos).normed(); else css.Direction =
                 travContext.Direction1;
              */
              float alpha = travContext.Alpha;
              CVector d1(travContext.Direction1), d2;
      ...
      if (cluster->Father && cluster->Father != travContext.PreviousCluster &&
          cluster->FatherAudible) {
        //				if (!travContext.FilterUnvisibleFather
        //|| ((1.0f-travContext.Alpha) > travContext.MinGain))
        {
          ...
          if (c != _RootCluster)
            ...
            // special case for root cluster coz it have a zero sized box and a
            // zero position.
          ...
          css.Gain = travContext.Gain;
          /*					if
             (travContext.FilterUnvisibleFather)
                              {
                                  // compute a gain
                                  float alpha = 1-(travContext.Dist /
             _PortalInterpolate); alpha = alpha * alpha * alpha; css.Gain =
             max(0.0f, alpha);
                              }
                              else
                                  css.Gain = travContext.Gain;
...
void CClusteredSound::addNextTraverse(CCluster *cluster,
                                      CSoundTravContext &travContext) {
  std::map<CCluster *, CSoundTravContext>::iterator it =
      _NextTraversalStep.find(cluster);
...
bool CClusteredSound::addAudibleCluster(CCluster *cluster,
...
CVector CClusteredSound::interpolateSourceDirection(
    const CClusteredSound::CSoundTravContext &context, float portalDist,
    const CVector &nearPoint, const CVector &realListener, CVector &d1,
    CVector & /* d2 */, float &alpha) {
  ...
  if (portalDist > _PortalInterpolate || alpha >= 1.0f) {
    ...
    //			factor = factor*factor*factor;
                direction = (nearPoint-realListener).normed();
                direction = d1 = (direction * factor + d1 *
    (1-factor)).normed();
  ...
  //			d1 = d1+d2;
              d2 = (nearPoint-context.ListenerPos).normed() * (1-alpha);
  //			direction = d1 + d2 +
  (nearPoint-context.ListenerPos).normed() * (1-alpha); direction = d1 + d2;
              direction.normalize();
          }
      }
      else
  ...
  //		alpha = 0.0f
          ...
          {
              d2 = (nearPoint-context.ListenerPos).normed() * (1-alpha);
  //			d2 = d1; //(nearPoint-context.ListenerPos).normed(); //
  * (1-alpha); direction = d1+d2; direction.normalize();
          }
          else
          {
  //			d2 = (nearPoint-context.ListenerPos).normed() *
  (1-alpha); d1 = d1+d2;
  //			d2 = (nearPoint-realListener).normed();
  //			direction =
  d1+d2+(nearPoint-context.ListenerPos).normed() * (1-alpha);
              direction.normalize();
          }
      }
...
float CClusteredSound::getPolyNearestPos(const std::vector<CVector> &poly,
                                         const CVector &pos,
                                         CVector &nearPoint) {
  CPlane plane;
  ...
  for (uint j = 0; j < nbVertex; ++j) {
    ...
    //		if (projIn /*&& j<poly.size()-1*/)
    {
      // check each segment
      if (plane.getNormal() *
              ((poly[(j + 1) % nbVertex] - poly[j]) ^ (proj - poly[j])) <
          0) {
...
float CClusteredSound::getAABoxNearestPos(const CAABBox &box,
                                          const CVector &pos,
                                          CVector &nearPos) {
  CVector vMin, vMax;
  box.getMin(vMin);

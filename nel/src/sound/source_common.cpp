...
CSourceCommon::CSourceCommon(TSoundId id, bool spawn, TSpawnEndCallback cb,
                             void *cbUserParam, NL3D::CCluster *cluster,
                             CGroupController *groupController)
    : _Priority(MidPri), _Playing(false), _Looping(false),
      _Position(CVector::Null), _Velocity(CVector::Null),
      _Direction(CVector::Null), _Gain(1.0f), _Pitch(1.0f),
      _RelativeMode(false), _InitialGain(1.0f), _PlayStart(0), _Spawn(spawn),
      _SpawnEndCb(cb), _CbUserParam(cbUserParam), _Cluster(cluster),
      _UserVarControler(id->getUserVarControler()),
      _GroupController(groupController ? groupController
                                       : id->getGroupController()) {
 ...
 */
void CSourceCommon::setLooping(bool l) { _Looping = l; }

/*
 * Return the looping state
 */
bool CSourceCommon::getLooping() const { return _Looping; }
...
void CSourceCommon::play() {
  ...
  if (_UserVarControler != CStringMapper::emptyId())
    CAudioMixerUser::instance()->addUserControledSource(this,
                                                        _UserVarControler);
}
...
void CSourceCommon::stop() {
  ...
  if (_UserVarControler != CStringMapper::emptyId())
    CAudioMixerUser::instance()->removeUserControledSource(this,
                                                           _UserVarControler);
}
 ...
 */
void CSourceCommon::setPos(const NLMISC::CVector &pos) { _Position = pos; }

/* Get the position vector.
 * If the source is stereo, return the position vector which reference was
 * passed to set3DPositionVector()
 */
 ...
 * (0,0,0))
 */
void CSourceCommon::setVelocity(const NLMISC::CVector &vel) { _Velocity = vel; }
...
/*
 * Return the relative gain (see setRelativeGain()), or the absolute gain if
 * getSource() is null.
 */
 ...
 */
void CSourceCommon::setSourceRelativeMode(bool mode) { _RelativeMode = mode; }

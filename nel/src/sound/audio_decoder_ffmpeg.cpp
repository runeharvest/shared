...
int avio_read_packet(void *opaque, uint8 *buf, int buf_size) {
  NLSOUND::CAudioDecoderFfmpeg *decoder =
      static_cast<NLSOUND::CAudioDecoderFfmpeg *>(opaque);
...
CAudioDecoderFfmpeg::CAudioDecoderFfmpeg(NLMISC::IStream *stream, bool loop)
    : IAudioDecoder(), _Stream(stream), _Loop(loop), _IsMusicEnded(false),
      _StreamSize(0), _IsSupported(false), _AvioContext(NULL),
      _FormatContext(NULL), _AudioContext(NULL), _AudioStreamIndex(0),
      _SwrContext(NULL) {
  ...
  try {
    ...
    // avio_ctx_buffer can be reallocated by ffmpeg and assigned to
    // avio_ctx->buffer
    ...
    _AvioContext =
        avio_alloc_context(avio_ctx_buffer, avio_ctx_buffer_size, 0, this,
                           &avio_read_packet, NULL, &avio_seek);
    if (!_AvioContext)
    ...
    if (avformat_find_stream_info(_FormatContext, NULL) >= 0) {
      _AudioStreamIndex = av_find_best_stream(
          _FormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, NULL, 0);
...
CAudioDecoderFfmpeg::~CAudioDecoderFfmpeg() { release(); }
...
bool CAudioDecoderFfmpeg::isFormatSupported() const { return _IsSupported; }

/// Get information on a music file.
bool CAudioDecoderFfmpeg::getInfo(NLMISC::IStream *stream, std::string &artist,
  ...
  while ((tag = av_dict_get(ffmpeg._FormatContext->metadata, "", tag,
...
uint32 CAudioDecoderFfmpeg::getNextBytes(uint8 *buffer, uint32 minimum,
                                         uint32 maximum) {
  if (_IsMusicEnded)
    return 0;
  nlassert(minimum <= maximum); // can't have this..

  // TODO: CStreamFileSource::play() will stall when there is no frames on
  // warmup supported can be set false if there is an issue creating converter
  if (!_IsSupported) {
    _IsMusicEnded = true;
    return 1;
  }
  ...
  if (!_SwrContext) {
    sint64 in_channel_layout =
        av_get_default_channel_layout(_AudioContext->channels);
    _SwrContext = swr_alloc_set_opts(
        NULL,
        // output
        FFMPEG_CHANNEL_LAYOUT, FFMPEG_SAMPLE_FORMAT, FFMPEG_SAMPLE_RATE,
        // input
        in_channel_layout, _AudioContext->sample_fmt,
  ...
  while (bytes_read < minimum) {
    ...
    if (packet.stream_index == _AudioStreamIndex) {
      ...
      do {
        ...
        if (ret < 0) {
          nlwarning("FFMPEG: error decoding audio frame: %s",
        ...
        if (got_frame) {
          uint32 out_bps =
              av_get_bytes_per_sample(FFMPEG_SAMPLE_FORMAT) * FFMPEG_CHANNELS;
          uint32 max_samples = (maximum - bytes_read) / out_bps;

          uint32 out_samples = av_rescale_rnd(
              swr_get_delay(_SwrContext, _AudioContext->sample_rate) +
                  frame.nb_samples,
          ...
          uint32 converted = swr_convert(_SwrContext, &buffer, max_samples,
                                         (const uint8 **)frame.extended_data,
                                         frame.nb_samples);
...
uint8 CAudioDecoderFfmpeg::getChannels() { return FFMPEG_CHANNELS; }

uint CAudioDecoderFfmpeg::getSamplesPerSec() { return FFMPEG_SAMPLE_RATE; }

uint8 CAudioDecoderFfmpeg::getBitsPerSample() { return FFMPEG_BITS_PER_SAMPLE; }

bool CAudioDecoderFfmpeg::isMusicEnded() { return _IsMusicEnded; }
...
float CAudioDecoderFfmpeg::getLength() {
  ...
  } else if (_FormatContext->streams[_AudioStreamIndex]->duration !=
             AV_NOPTS_VALUE) {
    length = _FormatContext->streams[_AudioStreamIndex]->duration *
             av_q2d(_FormatContext->streams[_AudioStreamIndex]->time_base);
  }
  return length;
}

void CAudioDecoderFfmpeg::setLooping(bool loop) { _Loop = loop; }

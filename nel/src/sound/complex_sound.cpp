...
bool CComplexSound::isDetailed() const { return false; }

void CComplexSound::parseSequence(const std::string &str,
                                  std::vector<uint32> &seq, uint scale) {
  seq.clear();
...
void CComplexSound::getSubSoundList(
    std::vector<std::pair<std::string, CSound *>> &subsounds) const {
  CAudioMixerUser *mixer = CAudioMixerUser::instance();
  std::vector<NLMISC::TStringId>::const_iterator first(_Sounds.begin()),
      last(_Sounds.end());
  for (; first != last; ++first) {
...
uint32 CComplexSound::getDuration() {
  ...
  std::vector<NLMISC::TStringId>::iterator first(_Sounds.begin()),
      last(_Sounds.end());
  for (; first != last; ++first) {
    CSound *sound = mixer->getSoundId(*first);
    if (sound != NULL) {
  ...
  switch (_PatternMode) {
  case MODE_CHAINED: {
    // sum the duration minus the xfade time (this is an aproximation if sample
    // are shorter than 2 xfade time)
    vector<uint32>::iterator first(_SoundSeq.begin()), last(_SoundSeq.end()),
        prev;
    for (; first != last; ++first) {
      if (first != _SoundSeq.begin() && !durations.empty()) {
        // remove a xfade value
        _Duration -= minof<uint32>(uint32(_XFadeLength / _TicksPerSeconds),
                                   durations[*first % durations.size()] / 2,
                                   durations[*prev % durations.size()] / 2);
      }
      if (!durations.empty())
        _Duration += durations[*first % durations.size()];
      prev = first;
    }
    //			_Duration -= max(sint(0), sint(_XFadeLength *
    //(_SoundSeq.size()-2)  ));
  } break;
  case MODE_SPARSE: {
    if (_SoundSeq.empty())
      ...
      std::vector<uint32>::iterator first(_DelaySeq.begin()),
          last(_DelaySeq.end());
...
CComplexSound::CComplexSound()
    : _PatternMode(CComplexSound::MODE_UNDEFINED), _TicksPerSeconds(1.0f),
      _XFadeLength(3000), // default to 3000 sec.
      _DoFadeIn(true), _DoFadeOut(true), _MaxDistValid(false), _Duration(0),
...
float CComplexSound::getMaxDistance() const {
  if (!_MaxDistValid) {
    ...
    // Hum, getMaxDistance is const, but we must compute the real max dist and
    // update it !
...
void CComplexSound::importForm(const std::string &filename,

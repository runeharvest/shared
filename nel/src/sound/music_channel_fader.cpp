...
CMusicChannelFader::CMusicChannelFader()
    : _SoundDriver(NULL), _ActiveMusicFader(0), _Gain(1.0f), _LastTime(0) {}
...
void CMusicChannelFader::updateVolume() {
  for (uint i = 0; i < _MaxMusicFader; ++i)
    if (_MusicFader[i].Playing)
      _MusicFader[i].MusicChannel->setVolume(_MusicFader[i].XFadeVolume *
                                             _Gain);
}
 ...
 */
bool CMusicChannelFader::play(const std::string &filepath, uint xFadeTime,
  ...
  if (xFadeTime && !stopped)
    fader.fadeIn(xFadeTime); // only fade in when fading out
  else
    fader.XFadeVolume = 1.0f;
  fader.Playing = true;

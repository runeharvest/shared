...
int vorbisSeekFunc(void *datasource, ogg_int64_t offset, int whence) {
  if (whence == SEEK_CUR && offset == 0) {
    // nlwarning(NLSOUND_XAUDIO2_PREFIX "This seek call doesn't do a damn thing,
    // wtf.");
  ...
  if (audio_decoder_vorbis->getStream()->seek(
          SEEK_SET ? audio_decoder_vorbis->getStreamOffset() + (sint32)offset
                   : (sint32)offset,
          origin))
    return 0;
  else
...
//{
//	//CAudioDecoderVorbis *audio_decoder_vorbis = (CAudioDecoderVorbis
//*)datasource;
// }

long vorbisTellFunc(void *datasource) {
  CAudioDecoderVorbis *audio_decoder_vorbis = (CAudioDecoderVorbis *)datasource;
  return (long)(audio_decoder_vorbis->getStream()->getPos() -
                audio_decoder_vorbis->getStreamOffset());
}
...
CAudioDecoderVorbis::~CAudioDecoderVorbis() { ov_clear(&_OggVorbisFile); }

/// Get information on a music file (only artist and title at the moment).
bool CAudioDecoderVorbis::getInfo(NLMISC::IStream *stream, std::string &artist,
                                  std::string &title, float &length) {
  CAudioDecoderVorbis mbv(
      stream, false); // just opens and closes the oggvorbisfile thing :)
...
uint32 CAudioDecoderVorbis::getNextBytes(uint8 *buffer, uint32 minimum,
  ...
  do {
    ...
    sint br = ov_read(
        &_OggVorbisFile, (char *)&buffer[bytes_read], maximum - bytes_read,
        endianness, // Specifies big or little endian byte packing. 0 for little
                    // endian, 1 for b ig endian. Typical value is 0.
        getBitsPerSample() == 8 ? 1 : 2,
        getBitsPerSample() == 8 ? 0 : 1, // Signed or unsigned data. 0 for
                                         // unsigned, 1 for signed. Typically 1.
        &current_section);
...
uint8 CAudioDecoderVorbis::getBitsPerSample() { return 16; }

bool CAudioDecoderVorbis::isMusicEnded() { return _IsMusicEnded; }

float CAudioDecoderVorbis::getLength() {
  return (float)ov_time_total(&_OggVorbisFile, -1);
}

void CAudioDecoderVorbis::setLooping(bool loop) { _Loop = loop; }

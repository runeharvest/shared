...
CComplexSource::CComplexSource(CComplexSound *soundPattern, bool spawn,
    ...
    : CSourceCommon(soundPattern, spawn, cb, cbUserParam, cluster,
                    groupController),
      _Source1(NULL), _Source2(NULL), _Muted(false) {
...
CComplexSource::~CComplexSource() {
  ...
  std::vector<USource *>::iterator first(_AllSources.begin()),
      last(_AllSources.end());
...
TSoundId CComplexSource::getSound() { return _PatternSound; }
/*
...
bool CComplexSource::isPlaying() { return _Playing; }
...
void CComplexSource::playStuf() {
  ...
  case CComplexSound::MODE_CHAINED: {
    ...
    if (!soundSeq.empty()) {
      CSound *sound = mixer->getSoundId(
          _PatternSound->getSound(soundSeq[_SoundSeqIndex++]));
      ...
      if (_PatternSound->doFadeIn())
        _FadeLength =
            min(uint32(_PatternSound->getFadeLength() / _TickPerSecond),
                sound->getDuration() / 2);
      else
        _FadeLength = 0;

      _Source2 = mixer->createSource(sound, false, 0, 0, _Cluster, NULL,
                                     _GroupController);
      if (_Source2 == NULL)
        return;
  ...
  case CComplexSound::MODE_SPARSE: {
    ...
    if (!delaySeq.empty() && delaySeq[_DelaySeqIndex] != 0) {
      ...
      // begin with a delay
      mixer->addEvent(
          this, uint64(now + delaySeq[_DelaySeqIndex++] / _TickPerSecond));
    } else {
      if (!delaySeq.empty())
        _DelaySeqIndex = 1;
      const vector<uint32> &soundSeq = _PatternSound->getSoundSeq();
      if (!soundSeq.empty()) {
        ...
        _Source1 = mixer->createSource(sound, false, 0, 0, _Cluster, NULL,
                                       _GroupController);
        if (_Source1 == NULL)
          return;
        ...
        if (!delaySeq.empty() && _DelaySeqIndex < delaySeq.size()) {
          ...
          mixer->addEvent(this,
                          uint64(now + sound->getDuration() +
                                 delaySeq[_DelaySeqIndex++] / _TickPerSecond));
  ...
  case CComplexSound::MODE_ALL_IN_ONE: {
    ...
    std::vector<NLMISC::TStringId>::const_iterator first(sounds.begin()),
        last(sounds.end());
    ...
    if (!_Looping) {
      ...
      mixer->addEvent(this, NLMISC::CTime::getLocalTime() +
...
void CComplexSource::onUpdate() {
  ...
  //	- when max reach, stop the update, swap the sound, delete sound1 and set
  //event for next fade.
  ...
  if (_FadeLength > 0) {
    _FadeFactor =
        float((double(now) - double(_StartTime2)) / double(_FadeLength));
  ...
  if (_FadeFactor >= 1.0) {
    ...
    if (_Source2) {
      ...
      _Source2 = NULL;
      // if there is a next sound available, program an event for the next
      // xfade.
      CSound *sound2 = NULL;
      //			_SoundSeqIndex++;
      const vector<uint32> &soundSeq = _PatternSound->getSoundSeq();
      if (_SoundSeqIndex < soundSeq.size()) {
        sound2 = mixer->getSoundId(
            _PatternSound->getSound(soundSeq[_SoundSeqIndex++]));
      } else if (_Looping) {
      ...
      if (sound2 != NULL) {
        // nldebug("CS : Chaining to sound %s",
        // CStringMapper::unmap(sound2->getName()).c_str());
        CAudioMixerUser *mixer = CAudioMixerUser::instance();

        // determine the XFade length (if next sound is too short.
        _FadeLength = minof<uint32>(
            uint32(_PatternSound->getFadeLength() / _TickPerSecond),
            (sound2->getDuration()) / 2,
            (_Source1->getSound()->getDuration()) / 2);
        _Source2 = mixer->createSource(sound2, false, 0, 0, _Cluster, NULL,
                                       _GroupController);
        if (_Source2) {
          _Source2->setPriority(_Priority);
          // there is a next sound, add event for xfade.
          // nldebug("Seting event for sound %s in %u millisec (XFade = %u).",
          // CStringMapper::unmap(_Source1->getSound()->getName()).c_str(),
          // _Source1->getSound()->getDuration()-_FadeLength, _FadeLength);
          mixer->addEvent(this, _StartTime1 +
                                    _Source1->getSound()->getDuration() -
                                    _FadeLength);
        }
      } else {
        // no sound after, just set an event at end of current sound to stop the
        // complex sound.
        nldebug("Setting last event for sound %s in %u millisec.",
                CStringMapper::unmap(_Source1->getSound()->getName()).c_str(),
                _Source1->getSound()->getDuration());
        if (_PatternSound->doFadeOut()) {
          ...
          mixer->addEvent(this, _StartTime1 +
                                    _Source1->getSound()->getDuration() -
                                    _PatternSound->getFadeLength());
        } else {
          // set the event at end of sound.
          mixer->addEvent(this,
                          _StartTime1 + _Source1->getSound()->getDuration());
        }
      }
    } else {
      if (_PatternSound->doFadeOut()) {
    ...
    //		nldebug("XFade : %4.3f <=> %4.3f (Fade Len = %6.3f",
    //(1.0f-_FadeFactor)*_Gain, _FadeFactor*_Gain, _FadeLength/1000.0f);
    ...
    if (_Source2) {
      // lower the sound 1.
      //			_Source2->setRelativeGain(float(sqrt(_FadeFactor))
      //* _Gain);
...
if (!soundSeq.empty() && !_LastSparseEvent) {
  ...
  CSound *sound =
      mixer->getSoundId(_PatternSound->getSound(soundSeq[_SoundSeqIndex++]));

  _Source1 =
      mixer->createSource(sound, false, 0, 0, _Cluster, NULL, _GroupController);
  if (_Source1 == NULL) {
    stop();
    return;
  }
  ...
  if (!delaySeq.empty() && _DelaySeqIndex < delaySeq.size()) {
    // event for next sound.
    mixer->addEvent(this, uint64(now + sound->getDuration() +
                                 delaySeq[_DelaySeqIndex++] / _TickPerSecond));
    if (_DelaySeqIndex == delaySeq.size() && !_Looping)
      _LastSparseEvent = true;
  } else {
...
void CComplexSource::checkup() {
  ...
  if (_Source1 != NULL && _Source1->getSound()->getLooping() &&
      !_Source1->isPlaying())
    _Source1->play();
  if (_Source2 != NULL && _Source2->getSound()->getLooping() &&
      !_Source2->isPlaying())
    _Source2->play();

...
CSoundAnimManager::CSoundAnimManager(NLSOUND::UAudioMixer *mixer)
    : _Mixer(mixer) {
  if (_Instance != 0) {
...
void CSoundAnimManager::saveAnimation(CSoundAnimation *anim,
                                      std::string &filename) {
  nlassert(anim);
...
// ********************************************************
/*
TSoundAnimPlayId CSoundAnimManager::playAnimation(TSoundAnimId id, float time,
CVector* position)
{
    ...
    _PlayerId++;
    CSoundAnimPlayer* player = new CSoundAnimPlayer(anim, time, position,
_Mixer, _PlayerId); nlassert(player);
...
std::string CSoundAnimManager::idToName(TSoundAnimId id) {
  static string empty;
  TSoundAnimMap::iterator it(
      std::find_if(_IdMap.begin(), _IdMap.end(), TFindId(id)));
...
void CSoundAnimManager::playAnimation(TSoundAnimId id, float lastTime,
                                      float curTime, NL3D::CCluster *cluster,
...
// ********************************************************
TSoundAnimPlayId
CSoundAnimManager::playAnimation(TSoundAnimId /* id */, float /* time */,
...
// ********************************************************
TSoundAnimPlayId
CSoundAnimManager::playAnimation(string & /* name */, float /* time */,
                                 NL3D::CCluster * /* cluster */,
                                 CSoundContext & /* context */) {
  /*	nlassert(position);

...
CSound *CSound::createSound(const std::string &filename,
 ...
 */
CSound::CSound()
    : _Gain(1.0f), _Pitch(1.0f), _Priority(MidPri), _ConeInnerAngle(6.283185f),
      _ConeOuterAngle(6.283185f), _ConeOuterGain(1.0f), _Looping(false),
      _MinDist(1.0f), _MaxDist(1000000.0f),
      _UserVarControler(CStringMapper::emptyId()), _GroupController(NULL) {}
...
void CSound::serial(NLMISC::IStream &s) {
  ...
  if (s.isReading())
    _GroupController = CGroupControllerRoot::getInstance()->getGroupController(
        NLSOUND_SHEET_V1_DEFAULT_SOUND_GROUP_CONTROLLER);
#else
  if (s.isReading()) {
 ...
 */
void CSound::importForm(const std::string &filename,

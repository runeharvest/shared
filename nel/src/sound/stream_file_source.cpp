...
CStreamFileSource::CStreamFileSource(CStreamFileSound *streamFileSound,
                                     bool spawn, TSpawnEndCallback cb,
                                     void *cbUserParam, NL3D::CCluster *cluster,
                                     CGroupController *groupController)
    : CStreamSource(streamFileSound, spawn, cb, cbUserParam, cluster,
                    groupController),
      m_AudioDecoder(NULL), m_Paused(false), m_DecodingEnded(false) {
  ...
  if (m_WaitingForPlay) {
    if (m_Thread->isRunning()) {
      if (m_NextBuffer || !m_FreeBuffers) {
        ...
        nldebug("play waiting, play stream %s",
                getStreamFileSound()->getFilePath().c_str());
#endif
        CStreamSource::play();
        if (!_Playing && !m_WaitingForPlay) {
          nldebug("Stream file source playback not possible or necessary for "
                  "some reason");
        }
      } else {
#ifdef NLSOUND_STREAM_FILE_DEBUG
        nldebug("play waiting, hop onto waiting list %s",
                getStreamFileSound()->getFilePath().c_str());
#endif
      ...
      // thread went kaboom while not started playing yet, probably the
      // audiodecoder cannot be started don't play
    ...
    if (!getStreamFileSound()->getAsync()) {
      ...
      if (m_WaitingForPlay && m_Thread->isRunning()) {
        CStreamSource::play();
        if (!_Playing) {
          nlwarning("Failed to synchronously start playing a file stream "
                    "source. This happens when all physical tracks are in use. "
                    "Use a Highest Priority sound");
        }
      }
    } else {
      CAudioMixerUser *mixer = CAudioMixerUser::instance();
...
bool CStreamFileSource::isEnded() {
  return m_DecodingEnded || (!m_Thread->isRunning() && !_Playing &&
                             !m_WaitingForPlay && !m_Paused);
}

float CStreamFileSource::getLength() { return m_AudioDecoder->getLength(); }

bool CStreamFileSource::isLoadingAsync() { return m_WaitingForPlay; }

bool CStreamFileSource::prepareDecoder() {
  // creates a new decoder or keeps going with the current decoder if the stream
  // was paused

  if (m_Paused) {
    // handle paused!
    m_Paused = false;
  } else if (m_AudioDecoder) // audio decoder should normally not exist when not
                             // paused and starting the thread
  {
    nlwarning(
        "CAudioDecoder already exists, possible thread race bug with pause");
    delete m_AudioDecoder;
    m_AudioDecoder = NULL;
  }
  if (!m_AudioDecoder) {
    ...
    m_AudioDecoder = IAudioDecoder::createAudioDecoder(
        m_LookupPath, getStreamFileSound()->getAsync(),
        getStreamFileSound()->getLooping());
    if (!m_AudioDecoder) {
      nlwarning("Failed to create IAudioDecoder, likely invalid format");
      return false;
    }
    this->setFormat(m_AudioDecoder->getChannels(),
                    m_AudioDecoder->getBitsPerSample(),
                    (uint32)m_AudioDecoder->getSamplesPerSec());
  }
  uint samples, bytes;
...
inline bool CStreamFileSource::bufferMore(
    uint bytes) // buffer from bytes (minimum) to bytes * 2 (maximum)
{
...
void CStreamFileSource::run() {
  ...
  while (_Playing || m_WaitingForPlay) {
    if (!m_AudioDecoder->isMusicEnded()) {
      ...
      ++dumpI;
      if (!(dumpI % 100)) {
        nldebug("buffer %s %s %s", _Playing ? "PLAYING" : "NP",
                m_WaitingForPlay ? "WAITING" : "NW",
                getStreamFileSound()->getFilePath().c_str());
        nldebug("gain %f",
                hasPhysicalSource() ? getPhysicalSource()->getGain() : -1.0f);
      }
#endif
      ...
      if (bufferMore(bytes))
        recSleep = doSleep = this->getRecommendedSleepTime();
      else
        doSleep = recSleep >> 2; // /4
      NLMISC::nlSleep(doSleep);
    } else {
      // wait until done playing buffers
      while (this->hasFilledBuffersAvailable() &&
             (_Playing || m_WaitingForPlay)) {
#ifdef NLSOUND_STREAM_FILE_DEBUG
        nldebug("music ended, wait until done %s",
                getStreamFileSound()->getFilePath().c_str());
#endif
        NLMISC::nlSleep(40);
      }
  ...
  if (m_Paused) {
    ...
    delete m_AudioDecoder;
    m_AudioDecoder = NULL;
    // _Playing cannot be used to detect play state because its required in
    // cleanup Using m_AudioDecoder in isEnded() may result race condition
    // (decoder is only created after thread is started)

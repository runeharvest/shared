...
CSimpleSource::CSimpleSource(CSimpleSound *simpleSound, bool spawn,
    ...
    : CSourceCommon(simpleSound, spawn, cb, cbUserParam, cluster,
                    groupController),
      _SimpleSound(simpleSound), _Track(NULL), _PlayMuted(false),
...
CSimpleSource::~CSimpleSource() {
  if (_Playing)
    stop();
  // Yoyo: security. with prec stop(), should not be needed, but a crash still
  // raise in "currentEvent->onEvent();" in audio_mixer_user.cpp [KAETEMI TODO:
  // Take a look at previous comment.]
...
IBuffer *CSimpleSource::getBuffer() { return _SimpleSound->getBuffer(); }
...
void CSimpleSource::play() {
  ...
  if (_SimpleSound->getBuffer() == 0 ||
      ...
      //|| (mixer->getListenPosVector() - _Position).sqrnorm() >
      //_SimpleSound->getMaxDistance() * _SimpleSound->getMaxDistance())
      || (_RelativeMode ? getPos().sqrnorm()
    ...
    // nldebug("CSimpleSource %p : play FAILED !",
    // (CAudioMixerUser::IMixerEvent*)this);
    return;
  }

  // -- Here we can play the source, either in a real track or as a muted
  // source.
  ...
  if (hasPhysicalSource()) {
    ...
    if (!_SimpleSound->getBuffer()->isStereo()) {
      pSource->setMinMaxDistances(_SimpleSound->getMinDistance(),
                                  _SimpleSound->getMaxDistance(), false);
    ...
    if (!play)
      nlwarning(
          "Failed to play physical sound source. This is a serious error");
#endif

    // nldebug("CSimpleSource %p : REAL play done",
    // (CAudioMixerUser::IMixerEvent*)this);
  } else {
    ...
    // nldebug("CSimpleSource %p : MUTED play done",
    // (CAudioMixerUser::IMixerEvent*)this);
  }
...
void CSimpleSource::onEvent() {
  // nldebug("CSimpleSource %p : stop EVENT",
  // (CAudioMixerUser::IMixerEvent*)this);
  ...
  if (hasPhysicalSource()) {
    if (!_SimpleSound->getBuffer()->isStereo()) {
      ...
      if (dir.isNull()) // workaround
      {
        ...
        //				if (!coneset)
        {
          getPhysicalSource()->setCone(_SimpleSound->getConeInnerAngle(),
                                       _SimpleSound->getConeOuterAngle(),
                                       _SimpleSound->getConeOuterGain());
          coneset = true;
        }
 ...
 * the listener position (default: false)
 */
 ...
 */
bool CSimpleSource::isPlaying() { return _Playing; }

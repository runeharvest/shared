...
CStreamSource::CStreamSource(CStreamSound *streamSound, bool spawn,
    ...
    : CSourceCommon(streamSound, spawn, cb, cbUserParam, cluster,
                    groupController),
      m_StreamSound(streamSound), m_Alpha(0.0f), m_Track(NULL),
      m_FreeBuffers(3), m_NextBuffer(0), m_LastSize(0), m_BytesPerSecond(0),
      m_WaitingForPlay(false), m_PitchInv(1.0f) {
...
bool CStreamSource::isPlaying() { return _Playing; }
...
CVector CStreamSource::getVirtualPos() const {
  if (getCluster() != 0) {
    ...
    const CClusteredSound::CClusterSoundStatus *css =
        CAudioMixerUser::instance()->getClusteredSound()->getClusterSoundStatus(
            getCluster());
    if (css != 0) {
      // there is some data here, update the virtual position of the sound.
      float dist = (css->Position - getPos()).norm();
      CVector vpos(CAudioMixerUser::instance()->getListenPosVector() +
...
void CStreamSource::play() {
...

  {
    ...
    // if ((mixer->getListenPosVector() - _Position).sqrnorm() >
    // m_StreamSound->getMaxDistance() * m_StreamSound->getMaxDistance())
    if ((_RelativeMode ? getPos().sqrnorm()
                       : (mixer->getListenPosVector() - getPos()).sqrnorm()) >
        m_StreamSound->getMaxDistance() * m_StreamSound->getMaxDistance()) {
      ...
      nldebug("CStreamSource %p : play FAILED, source is too far away !",
              (CAudioMixerUser::IMixerEvent *)this);
#endif
      return;
    }
    ...
    if (hasPhysicalSource()) {
      ...
      pSource->setMinMaxDistances(m_StreamSound->getMinDistance(),
                                  m_StreamSound->getMaxDistance(), false);
      ...
      play = pSource->play();
      // nldebug("CStreamSource %p : REAL play done",
      // (CAudioMixerUser::IMixerEvent*)this);
    } else {
    ...
    if (play) {
      ...
      nlwarning(" * getLooping: %s",
                getPhysicalSource()->getLooping() ? "YES" : "NO");
      nlwarning(" * isPlaying: %s",
                getPhysicalSource()->isPlaying() ? "YES" : "NO");
      nlwarning(" * isStopped: %s",
                getPhysicalSource()->isStopped() ? "YES" : "NO");
      nlwarning(" * isPaused: %s",
                getPhysicalSource()->isPaused() ? "YES" : "NO");
      nlwarning(" * getPos: %f, %f, %f", getPhysicalSource()->getPos().x,
      ...
      nlwarning(" * getSourceRelativeMode: %s",
                getPhysicalSource()->getSourceRelativeMode() ? "YES" : "NO");
      float a, b, c;
      ...
      nlwarning(" * getDirect: %s",
                getPhysicalSource()->getDirect() ? "YES" : "NO");
      nlwarning(" * getDirectGain: %f", getPhysicalSource()->getDirectGain());
      nlwarning(" * isDirectFilterEnabled: %s",
                getPhysicalSource()->isDirectFilterEnabled() ? "YES" : "NO");
      nlwarning(" * getEffect: %s",
                getPhysicalSource()->getEffect() ? "YES" : "NO");
      nlwarning(" * getEffectGain: %f", getPhysicalSource()->getEffectGain());
      nlwarning(" * isEffectFilterEnabled: %s",
                getPhysicalSource()->isEffectFilterEnabled() ? "YES" : "NO");
#endif
    }
  }
...
void CStreamSource::setDirection(const NLMISC::CVector &dir) {
  ...
  if (hasPhysicalSource()) {
    if (!m_Buffers[0]->isStereo()) {
      ...
      {
        getPhysicalSource()->setCone(
            float(Pi * 2), float(Pi * 2),
            1.0f); // because the direction with 0 is not enough for a
                   // non-directional source!
        getPhysicalSource()->setDirection(
            CVector::I); // Don't send a 0 vector, DSound will complain. Send
                         // (1,0,0), it's omnidirectional anyway.
        coneset = false;
      } else {
        //				if (!coneset)
        {
          getPhysicalSource()->setCone(m_StreamSound->getConeInnerAngle(),
                                       m_StreamSound->getConeOuterAngle(),
                                       m_StreamSound->getConeOuterGain());
          coneset = true;
        }
...
void CStreamSource::setFormat(uint8 channels, uint8 bitsPerSample,
                              uint32 frequency) {
  nlassert(!_Playing);

  m_Buffers[0]->setFormat(IBuffer::FormatPcm, channels, bitsPerSample,
                          frequency);
  m_Buffers[1]->setFormat(IBuffer::FormatPcm, channels, bitsPerSample,
                          frequency);
  m_Buffers[2]->setFormat(IBuffer::FormatPcm, channels, bitsPerSample,
                          frequency);

  m_BytesPerSecond =
      ((uint)bitsPerSample * (uint)frequency * (uint)channels) / 8;
}

/// Return the sample format information.
void CStreamSource::getFormat(uint8 &channels, uint8 &bitsPerSample,
...
/// Get a writable pointer to the buffer of specified size. Use capacity to
/// specify the required bytes. Returns NULL when all the buffer space is
/// already filled. Call setFormat() first.
...
/// Notify that you are done writing to the locked buffer, so it can be copied
/// over to hardware if needed. Set size to the number of bytes actually written
/// to the buffer. Returns true if ok.
...
/// Get the recommended sleep time based on the size of the last submitted
/// buffer and the available buffer space
uint32 CStreamSource::getRecommendedSleepTime() const {
  if (m_FreeBuffers > 0)
    return 0;
  uint32 sleepTime =
      (uint32)((1000.0f * ((float)m_LastSize) / (float)m_BytesPerSecond) *
               m_PitchInv);

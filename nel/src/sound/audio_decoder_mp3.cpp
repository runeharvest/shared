...
// callback for drmp3
static size_t drmp3_read(void *pUserData, void *pBufferOut,
                         size_t bytesToRead) {
  NLSOUND::CAudioDecoderMP3 *decoder =
      static_cast<NLSOUND::CAudioDecoderMP3 *>(pUserData);
...
// callback for drmp3
static drmp3_bool32 drmp3_seek(void *pUserData, int offset,
...
CAudioDecoderMP3::CAudioDecoderMP3(NLMISC::IStream *stream, bool loop)
    : IAudioDecoder(), _Stream(stream), _Loop(loop), _IsMusicEnded(false),
      _StreamSize(0), _IsSupported(false), _PCMFrameCount(0) {
...
bool CAudioDecoderMP3::isFormatSupported() const { return _IsSupported; }

/// Get information on a music file.
bool CAudioDecoderMP3::getInfo(NLMISC::IStream *stream, std::string &artist,
...
uint32 CAudioDecoderMP3::getNextBytes(uint8 *buffer, uint32 minimum,
                                      uint32 maximum) {
  if (_IsMusicEnded)
    return 0;
  ...
  while (framesToRead > 0) {
    ...
    drmp3dec_f32_to_s16(tempBuffer, pFrameBufferOut,
                        tempFrames * _Decoder.channels);
...
uint8 CAudioDecoderMP3::getChannels() { return _Decoder.channels; }

uint CAudioDecoderMP3::getSamplesPerSec() { return _Decoder.sampleRate; }

uint8 CAudioDecoderMP3::getBitsPerSample() { return MP3_BITS_PER_SAMPLE; }

bool CAudioDecoderMP3::isMusicEnded() { return _IsMusicEnded; }
...
void CAudioDecoderMP3::setLooping(bool loop) { _Loop = loop; }

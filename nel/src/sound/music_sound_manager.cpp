...
void CMusicSoundManager::update() {
  ...
  if (_TimeConstraintEnabled && _CurrentMusicPlaying &&
      currentTime <=
          _PlayStartTime + _CurrentMusicPlaying->getMinimumPlayTime())
  ...
  for (; it != _Sources.end(); it++) {
    ...
    if (_AlreadyPlayedSources.find(src) != _AlreadyPlayedSources.end())
      continue;
    // verify that this sound can be played again from the last time it has been
    // played
    if (_TimeConstraintEnabled && snd->LastStopTime > INT_MIN &&
        currentTime <= snd->LastStopTime + snd->getTimeBeforeCanReplay())
      continue;
    ...
    // else compare sound
      ...
      // take the higher priority (priority value is inversed: 0 is the best
      // priority)
      ...
      // else, other criteria
        ...
        /* if the new sound is not looping and the best is, consider the new
           sound as an "instant sound" which is prioritary
        */
        if (!snd->getLooping() && bestSound->getLooping()) {
          bestSound = snd;
          bestSource = src;
        } else if (snd->getLooping() == bestSound->getLooping()) {
          // if the bestSound is the current sound played, prefer to not change
          // the sound
          if (bestSound != _CurrentMusicPlaying) {
            /* NB: here, bestSound can be != from _CurrentMusicPlaying in the
               following cases:
                - _CurrentMusicPlaying= NULL
                - bestSound was assigned to a higher priority sound than
               _CurrentMusicPlaying thereFore snd should be different from
               _CurrentMusicPlaying, since this one is of lower priority...
             */
  ...
  // else, no new music found => if the music is currently playing
    ...
    if (_CurrentMusicPlaying->getLooping() == false && mixer->isMusicEnded()) {
      ...
      // verify that a source with this sound still exist. If not, we have to
      // cut this sound too
      bool found = false;
      std::set<CMusicSource *>::iterator it = _Sources.begin();
      for (; it != _Sources.end(); it++) {
...
void CMusicSoundManager::startMusic(CMusicSound *newMs, CMusicSource *newSrc) {
  ...
  CAudioMixerUser::instance()->playMusic(
      CStringMapper::unmap(newMs->getFileName()), uint(xFade), true,
      newMs->getLooping());
  ...
  // The source is played this time. Avoid replay it for infinite time if the
  // player stay in the zone
  if (!newMs->getLooping())
...
void CMusicSoundManager::stopMusic(bool allowFade) {
  if (_CurrentMusicPlaying) {
    ...
    if (allowFade)
      CAudioMixerUser::instance()->stopMusic(
          _CurrentMusicPlaying->getFadeOutLength());
    else

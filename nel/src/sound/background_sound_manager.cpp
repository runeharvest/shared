...
CBackgroundSoundManager::CBackgroundSoundManager()
  ...
  for (uint i = 0; i < UAudioMixer::TBackgroundFlags::NB_BACKGROUND_FLAGS;
       ++i) {
...
CBackgroundSoundManager::~CBackgroundSoundManager() { unload(); }

const UAudioMixer::TBackgroundFlags &
CBackgroundSoundManager::getBackgroundFlags() {
  return _BackgroundFlags;
}

void CBackgroundSoundManager::setBackgroundFilterFades(
    const UAudioMixer::TBackgroundFilterFades &backgroundFilterFades) {
...
void CBackgroundSoundManager::addSound(
    const std::string &soundName, uint layerId,
    const std::vector<NLLIGO::CPrimVector> &points, bool isPath) {
  ...
  if (sd.Sound != 0) {
    ...
    nlwarning("The sound '%s' can't be loaded",
              CStringMapper::unmap(sd.SoundName).c_str());
  }
}

void CBackgroundSoundManager::addSound(
    const std::string &rawSoundName,
    const std::vector<NLLIGO::CPrimVector> &points, bool isPath) {
  uint layerId = 0;
  uint n = 0;
  string name;
  ...
  if (n == 2) {
    ...
    if (pos1 == string::npos) {
      nlwarning("zone have the malformated name '%s' missing -name-",
                rawSoundName.c_str());
      return;
    }
    pos1++;
    ...
    if (pos1 == string::npos || pos2 == string::npos) {
      nlwarning(
          "zone have the malformated name '%s' missing -layerId- or -name-",
          rawSoundName.c_str());
      return;
    }
    pos1++;
  ...
  /*
      ...
      if (sd.Sound != 0)
      {
          ...
          vector<CVector>::iterator first(sd.Points.begin()),
     last(sd.Points.end()); for (; first != last; ++first)
          {
              vmin.x = min(first->x, vmin.x);
              vmin.y = min(first->y, vmin.y);
              vmax.x = max(first->x, vmax.x);
...
void CBackgroundSoundManager::loadAudioFromPrimitives(
...
void CBackgroundSoundManager::loadSoundsFromPrimitives(
    const NLLIGO::IPrimitive &soundRoot) {
  std::string className;
        ...
        if (child->getPropertyByName("class", className)) {
          ...
          if (className == "sound_zone") {
            if (child->getNumVector() > 2) {
              addSound(soundName, layerId,
                       static_cast<const CPrimZone *>(child)->VPoints, false);
            } else {
              nlwarning(
                  "A background sound patatoid have less than 3 points '%s'",
                  primName.c_str());
            }
          } else if (className == "sound_path") {
            if (child->getNumVector() > 1) {
              addSound(soundName, layerId,
                       static_cast<const CPrimPath *>(child)->VPoints, true);
            } else {
              nlwarning("A background sound path have less than 2 points '%s'",
                        primName.c_str());
            }
...
void CBackgroundSoundManager::loadSamplesFromPrimitives(
    const NLLIGO::IPrimitive &sampleRoot) {
  std::string className;
  _Banks.clear();
        ...
        if (child->getPropertyByName("class", className)) {
          ...
          if (className == "sample_bank_zone") {
            ...
            if (child->getPropertyByName("bank_names", names)) {
              addSampleBank(*names,
                            static_cast<const CPrimZone *>(child)->VPoints);
            }
          }
        }
      }
...
void CBackgroundSoundManager::loadEffectsFromPrimitives(
    const NLLIGO::IPrimitive &fxRoot) {
  std::string className;
  _FxZones.clear();
...
void CBackgroundSoundManager::addFxZone(
    const std::string &fxName, const std::vector<NLLIGO::CPrimVector> &points) {
  TFxZone fxZone;
  ...
  vector<CVector>::iterator first(fxZone.Points.begin()),
      last(fxZone.Points.end());
  for (; first != last; ++first) {
    vmin.x = min(first->x, vmin.x);
    vmin.y = min(first->y, vmin.y);
    vmax.x = max(first->x, vmax.x);
...
void CBackgroundSoundManager::addSampleBank(
...
// void CBackgroundSoundManager::loadSamplesFromRegion(const NLLIGO::CPrimRegion
// &region)
//{
//	_Banks.clear();
//
...
//split(region.VZones[i].Name, '-'); 			std::vector<std::string>	bankNames;
//
//			if (splitted.size() > 2)
//			{
...
//				}
//
//				addSampleBank(bankNames,
//region.VZones[i].VPoints);
//			}
//			else
//			{
//				nlwarning ("A sample bank patatoid name did'nt
//contains banks name '%s'", region.VZones[i].Name.c_str());
//			}
//		}
//		else
//		{
//			nlwarning ("A sample bank patatoid have less than 3
//points '%s'", region.VZones[i].Name.c_str());
//		}
//	}
// }

// void CBackgroundSoundManager::loadEffecsFromRegion(const NLLIGO::CPrimRegion
// &region)
//{
// }
...
//		if(region.VZones[i].VPoints.size()>2)
//		{
//			addSound(region.VZones[i].Name,
//region.VZones[i].VPoints, false);
//		}
//		else
//		{
//			nlwarning ("A background sound patatoid have less than 3
//points '%s'", region.VZones[i].Name.c_str());
//		}
//	}
//
...
//		{
//			addSound(region.VPaths[i].Name,
//region.VPaths[i].VPoints, true);
//		}
//		else
//		{
//			nlwarning ("A background sound path have less than 2
//points '%s'", region.VPaths[i].Name.c_str());
//		}
//	}
...
void CBackgroundSoundManager::load(const string &continent,
  ...
  // First, try to load from a .primitive file (contain everythink)
  {
    ...
    if (!path.empty() && file.open(path)) {
      // first, try to load the binary version (if up to date)
      {
        ...
        if (!binPath.empty() && (CFile::getFileModificationDate(binPath) >
      ...
      if (mixer->getPackedSheetUpdate()) {
        ...
        string filename = mixer->getPackedSheetPath() + "/" + continent +
  ...
  //			file.close ();
  //
  //			nlinfo ("Region '%s' contains %d zones for the
  //background sounds", continent.c_str(), region.VZones.size());
  //
  ...
  //
  //			nlinfo ("Region '%s' contains %d zones for the
  //background effetcs", continent.c_str(), region.VZones.size());
  //
  ...
  //
  //			nlinfo ("Region '%s' contains %d zones for the
  //background samples banks", continent.c_str(), region.VZones.size());
  //
...
void CBackgroundSoundManager::stop() {
  ...
  for (uint i = 0; i < BACKGROUND_LAYER; ++i) {
    ...
    std::vector<TSoundData>::iterator first(_Layers[i].begin()),
        last(_Layers[i].end());
    for (; first != last; ++first) {
...
void CBackgroundSoundManager::setListenerPosition(
    const CVector &listenerPosition) {
...
void CBackgroundSoundManager::updateBackgroundStatus() {
  ...
  if (clusteredSound != 0) {
    const CClusteredSound::CClusterSoundStatus *css =
        clusteredSound->getClusterSoundStatus(clusteredSound->getRootCluster());
    if (css != 0) {
  ...
  if (mixer->useReverb()) {
    ...
    std::vector<TFxZone>::iterator first(_FxZones.begin()),
        last(_FxZones.end());
    for (; first != last; ++first) {
  ...
  // compute the list of load/unload banks.
  {
    ...
    /*		{
                nldebug("-----------------------------");
                nldebug("Loaded sample banks (%u elements):",
       _LoadedBanks.size()); set<string>::iterator first(_LoadedBanks.begin()),
       last(_LoadedBanks.end()); for (; first != last; ++first)
                {
            ...
            }
            {
                ...
                set<string>::iterator first(newBanks.begin()),
       last(newBanks.end()); for (; first != last; ++first)
                {
                    const string &str = *first;
                    nldebug("  %s", first->c_str());
                }
    ...
    */
    // ok, now compute to set : the set of bank to load, and the set of banks to
    // unload.
    std::set<std::string> noChange;
    std::set_intersection(_LoadedBanks.begin(), _LoadedBanks.end(),
                          newBanks.begin(), newBanks.end(),
                          std::inserter(noChange, noChange.end()));

    std::set<std::string> loadList;
    std::set_difference(newBanks.begin(), newBanks.end(), noChange.begin(),
                        noChange.end(),
                        std::inserter(loadList, loadList.end()));

    std::set<std::string> unloadList;
    std::set_difference(_LoadedBanks.begin(), _LoadedBanks.end(),
                        newBanks.begin(), newBanks.end(),
                        std::inserter(unloadList, unloadList.end()));

    // and now, load and unload....
    {
      std::set<std::string>::iterator first(loadList.begin()),
          last(loadList.end());
      for (; first != last; ++first) {
        //				nldebug("Trying to load sample bank %s",
        //first->c_str());
    ...
    {
      std::set<std::string>::iterator first(unloadList.begin()),
          last(unloadList.end());
      for (; first != last; ++first) {
        //				nldebug("Trying to unload sample bank
        //%s", first->c_str());
        if (mixer->unloadSampleBank(*first)) {
          // ok, the bank is unloaded
          _LoadedBanks.erase(*first);
        } else if (mixer->getSampleBankManager()->findSampleBank(
                       CStringMapper::map(*first)) == 0) {
  ...
  for (uint i = 0; i < BACKGROUND_LAYER; ++i) {
    ...
    for (uint count = 0; first != last; ++first, ++count) {
      if (listener.x >= first->MinBox.x && listener.x <= first->MaxBox.x &&
          listener.y >= first->MinBox.y && listener.y <= first->MaxBox.y
          //				&& listener.z >= first->MinBox.z &&
          //listener.z <= first->MaxBox.z
      ) {
        //				nldebug("patat %u is selected by box
        //(%s)", count, first->SoundName.c_str());
        selectedIndex.push_back(count);
      } else {
        //				nldebug("patat %u is rejected  by box
        //(%s)", count, first->SoundName.c_str());
    ...
    // Compute new source mixing in this layer
    {
      ...
      // first loop to compute selected sound gain and position and order the
      // result by surface..
      {
        vector<uint>::iterator first(selectedIndex.begin()),
            last(selectedIndex.end());
        for (; first != last; ++first) {
          TSoundData &sd = layer[*first];
          CVector pos;
          float gain = 1.0f;
          ...
          if (CPrimZone::contains(listener, sd.Points, distance, pos,
                                  sd.IsPath)) {
            inside = true;
            pos = _LastPosition; // use the real listener position, not the 0 z
                                 // centered
            gain = 1.0f;
            //						nlinfo ("inside patate
            //%d name '%s' ", *first, sd.SoundName.c_str());
          } else {
            if (sd.MaxDist > 0 && distance < sd.MaxDist) {
              // compute the gain.
              //							gain =
              //(sd.MaxDist - distance) / sd.MaxDist;
            } else {
              // too far
              gain = 0;
            }
            // nlinfo ("near patate %d name '%s' from %f ", *first,
            // sd.SoundName.c_str(), distance);
          }

          // store the status.
          status.push_back(make_pair(
              sd.Surface, TSoundStatus(sd, pos, gain, distance, inside)));
        }
      }
      // second loop thrue the surface ordered selected sound.
      {
        ...
        list<pair<float, TSoundStatus>>::iterator first(status.begin()),
            last(status.end());
        for (; first != last; ++first) {
          TSoundStatus &ss = first->second;

          // special algo for music sound (don't influence / use maskFactor
          // strategy)
          bool musicSound =
              ss.SoundData.Sound &&
              ss.SoundData.Sound->getSoundType() == CSound::SOUND_MUSIC;

          // ---- music sound special case (music competition is managed
          // specially in the CMusicSoundManager)
          if (musicSound) {
            if (ss.Gain > 0) {
              ...
              if (ss.SoundData.Source == 0) {
                ...
                ss.SoundData.Source =
                    static_cast<CSourceCommon *>(mixer->createSource(
                        ss.SoundData.Sound, false, 0, 0, rootCluster));
              }
              if (ss.SoundData.Source != 0) {
                // update the position (not used I think, but maybe
                // important...)
            ...
            } else if (ss.SoundData.Source != 0 &&
                       ss.SoundData.Source->isPlaying()) {
          ...
          // ---- standard sound case
            ...
            if (maskFactor > 0.0f && ss.Gain > 0) {
              ...
              //							if
              //(ss.Gain == 1) 							if (ss.Distance == 0)
              if (ss.Inside) {
                // inside a pattate, then decrease the mask factor will we are
                // more inside the patate
              ...
              if (ss.SoundData.Source != 0) {
                ...
                //								nldebug("Setting
                //source %s at %f", ss.SoundData.SoundName.c_str(), gain);
                if (!ss.SoundData.Source->isPlaying()) {
                  // start the sound is needed.
                  ss.SoundData.Source->play();
                } else if (ss.SoundData.Source->getType() !=
                           CSourceCommon::SOURCE_SIMPLE)
      ...
      if (_FilterFadesStart[i] != 0) {
        ...
        if (_BackgroundFlags.Flags[i]) {
          ...
          if (delta > _BackgroundFilterFades.FadeOuts[i]) {
            ...
            _FilterFadeValues[i] =
                1 - (float(delta) / _BackgroundFilterFades.FadeOuts[i]);
          ...
          if (delta > _BackgroundFilterFades.FadeIns[i]) {
            ...
            _FilterFadeValues[i] =
                float(delta) / _BackgroundFilterFades.FadeIns[i];
            // continue to fade (at least for this filter.
            _DoFade |= true;
          }
        }
    ...
    for (i = 0; i < BACKGROUND_LAYER; ++i) {
      ...
      for (; first != last; ++first) {
        if (first->Selected) {
          ...
          if (first->Source != 0 &&
              first->Source->getType() == CSourceCommon::SOURCE_BACKGROUND)
            static_cast<CBackgroundSource *>(first->Source)
...
void CBackgroundSoundManager::setBackgroundFlags(
    const UAudioMixer::TBackgroundFlags &backgroundFlags) {
  for (uint i = 0; i < UAudioMixer::TBackgroundFlags::NB_BACKGROUND_FLAGS;
       ++i) {
    if (_BackgroundFlags.Flags[i] != backgroundFlags.Flags[i]) {
      ...
      if (backgroundFlags.Flags[i]) {
        ...
        _FilterFadesStart[i] = uint64(NLMISC::CTime::getLocalTime() -
                                      (1 - _FilterFadeValues[i]) *
                                          _BackgroundFilterFades.FadeOuts[i]);
        _DoFade = true;
      } else {
        // the filter is cleared, do a fade in
        _FilterFadesStart[i] =
            uint64(NLMISC::CTime::getLocalTime() -
                   (_FilterFadeValues[i]) * _BackgroundFilterFades.FadeIns[i]);
        _DoFade = true;
      }
    }
...
void CBackgroundSoundManager::onUpdate() { updateBackgroundStatus(); }
...
void CBackgroundSoundManager::TSoundData::serial(NLMISC::IStream &s) {
  ...
  if (s.isReading()) {
    ...
    s.serial(
        const_cast<std::string &>(NLMISC::CStringMapper::unmap(SoundName)));
  }
  s.serial(MinBox);
  s.serial(MaxBox);
  s.serial(Surface);

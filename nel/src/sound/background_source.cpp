...
CBackgroundSource::CBackgroundSource(CBackgroundSound *backgroundSound,
...
TSoundId CBackgroundSource::getSound() { return NULL; }

void CBackgroundSource::setGain(float gain) {
  CSourceCommon::setGain(gain);

  std::vector<TSubSource>::iterator first(_Sources.begin()),
      last(_Sources.end());
...
void CBackgroundSource::play() {
  ...
  const vector<CBackgroundSound::TSoundInfo> &sounds =
      _BackgroundSound->getSounds();
  vector<CBackgroundSound::TSoundInfo>::const_iterator first(sounds.begin()),
      last(sounds.end());

  for (; first != last; ++first) {
    TSubSource subSource;
    subSource.Source = mixer->createSource(first->SoundName, false, 0, 0,
                                           _Cluster, NULL, _GroupController);
  ...
  for (; first != last; ++first) {
    ...
    if (ss.Source != 0) {
      ...
      if (gain == 0 && ss.Status != SUB_STATUS_STOP) {
        ...
        // is closer
        ss.Status =
            ss.Source->isPlaying() ? SUB_STATUS_PLAY : SUB_STATUS_PLAY_FAIL;

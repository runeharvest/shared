...
IAudioDecoder *IAudioDecoder::createAudioDecoder(const std::string &filepath,
...
IAudioDecoder *IAudioDecoder::createAudioDecoder(const std::string &type,
...
bool IAudioDecoder::getInfo(const std::string &filepath, std::string &artist,
...
/// Get audio/container extensions that are currently supported by the nel sound
/// library.
void IAudioDecoder::getMusicExtensions(std::vector<std::string> &extensions) {
  ...
  if (std::find(extensions.begin(), extensions.end(), "ogg") ==
      extensions.end()) {
    extensions.push_back("ogg");
  }
  if (std::find(extensions.begin(), extensions.end(), "mp3") ==
      extensions.end()) {
...
#ifdef FFMPEG_ENABLED
  return (extension == "ogg" || extension == "mp3" || extension == "flac" ||
          extension == "aac");
#else

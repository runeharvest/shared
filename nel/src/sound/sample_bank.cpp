...
CSampleBank::CSampleBank(NLMISC::TStringId name,
                         CSampleBankManager *sampleBankManager)
    : _SampleBankManager(sampleBankManager), _Name(name), _Loaded(false),
      _LoadingDone(true), _ByteSize(0) {
...
CSampleBank::~CSampleBank() {
  ...
  for (CSampleBankManager::TSampleBankContainer::iterator
       ...
       end(_SampleBankManager->m_Banks.end());
       it != end; ++it) {
    if (it->second == this) {
...
void CSampleBank::load(bool async) {
  ...
  CSampleBankManager::TVirtualBankCont::iterator it(
      _SampleBankManager->m_VirtualBanks.find(_Name));
  if (it != _SampleBankManager->m_VirtualBanks.end()) {
    // this is a virtual sample bank !
    nlinfo("Loading virtual sample bank %s",
           CStringMapper::unmap(_Name).c_str());

    const CAudioMixerUser::TBackgroundFlags &flags =
        _SampleBankManager->m_AudioMixer->getBackgroundFlags();

    for (uint i = 0; i < it->second.size(); ++i) {
      if (flags.Flags[it->second[i].Filter]) {
        CSampleBank *bank =
            _SampleBankManager->findSampleBank(it->second[i].BankName);
        if (bank)
          bank->load(async);
      }
    }
  ...
  // nlinfo("Loading sample bank %s %", CStringMapper::unmap(_Name).c_str(),
  ...
  if (_Loaded) {
    nlwarning("Trying to load an already loaded bank : %s",
              CStringMapper::unmap(_Name).c_str());
    return;
  }
  ...
  try {
    ...
    for (i = 0; i < sbh.Name.size(); ++i) {
      ...
      TStringId nameId =
          CStringMapper::map(CFile::getFilenameWithoutExtension(sbh.Name[i]));
      ...
      /*		{
                  ...
                  {
                      IBuffer::decodeADPCM(data+count/2, data16+count, 1024,
         state);
                  }
                  IBuffer::decodeADPCM(data+count/2, data16+count,
         sbh.NbSample[i]-count, state);
                  ...
                  _SoundDriver->readRawBuffer(ibuffer, sbh.Name[i],
         (uint8*)data16, sbh.NbSample[i]*2, Mono16, sbh.Freq[i]); delete []
         data16; delete [] data16_2;
              }
      */
      ...
      CAudioMixerUser::getInstance()->getSoundBank()->bufferLoaded(nameId,
                                                                   ibuffer);
    }
    free(data);
    ...
    // loading failed !
    nlwarning("Exception %s during loading of sample bank %s", e.what(),
              filename.c_str());

    if (_SampleBankManager->m_AudioMixer->getPackedSheetUpdate()) {
      nlinfo(
          "Deleting offending sound bank, you need to restart to recreate it!");
  ...
  /*
      ...
      std::string list =
  CPath::lookup(CStringMapper::unmap(_Name)+CAudioMixerUser::SampleBankListExt,
  false); if (list.empty())
      {
          nlwarning("File %s not found to load sample bank %s",
  (CStringMapper::unmap(_Name)+CAudioMixerUser::SampleBankListExt).c_str(),
  CStringMapper::unmap(_Name).c_str()); return;
      }
  ...
  //			std::string
  sampleName(CFile::getFilenameWithoutExtension(*iter)); NLMISC::TStringId
  sampleName(CStringMapper::map(CFile::getFilenameWithoutExtension(*iter)));

              if (async)
              {
                  ibuffer->presetName(sampleName);
                  nldebug("Preloading sample [%s]",
  CStringMapper::unmap(sampleName).c_str());
              }
              else
              {
                  ...
                  if (!fullName.empty())
                  {
                      ...
                      _SoundDriver->readWavBuffer(ibuffer, fullName, buffer,
  size); _ByteSize += ibuffer->getSize();
          ...
          catch (const ESoundDriver &e)
          {
              ...
              nlwarning("Problem with file '%s': %s", (*iter).c_str(),
  e.what());
          }
      }
      ...
      else
      {
          ...
          {
              CAsyncFileManagerSound::getInstance().loadWavFile(_LoadList.front().first,
  CStringMapper::unmap(_LoadList.front().second)+".wav"); _LoadList.pop_front();
          }
...
void CSampleBank::onUpdate() {
  if (_SplitLoadDone) {
    ...
    if (_LoadList.empty()) {
      ...
      _SampleBankManager->m_AudioMixer->getBackgroundSoundManager()
      ...
      _SplitLoadDone = false;
      for (uint i = 0; i < ASYNC_LOADING_SPLIT && !_LoadList.empty(); ++i) {
        CAsyncFileManagerSound::getInstance().loadWavFile(
            _LoadList.front().first,
            CStringMapper::unmap(_LoadList.front().second) + ".wav");
        _LoadList.pop_front();
      }
      // add a end loading event...
      CAsyncFileManagerSound::getInstance().signal(&_SplitLoadDone);
...
bool CSampleBank::unload() {
  ...
  if (!_Loaded) {
    nlwarning("Trying to unload an already unloaded bank : %s",
              CStringMapper::unmap(_Name).c_str());
    return true;
  }
...
bool CSampleBank::isLoaded() { return _Loaded; }
...
uint CSampleBank::countSamples() { return (uint)_Samples.size(); }

...
void CSoundBank::bufferUnloaded(const NLMISC::TStringId &bufferName) {
  ...
  if (it != _BufferAssoc.end()) {
    ...
    // update all sounds.
    TSimpleSoundContainer::iterator first(it->second.begin()),
        last(it->second.end());
...
void CSoundBank::bufferLoaded(const NLMISC::TStringId & /* bufferName */,
                              IBuffer *buffer) {
  //	std::map<std::string, std::vector<TBufferAssoc> >::iterator
  //it(_BufferAssoc.find(buffer->getName()));
...
void CSoundBank::load(const std::string &packedSheetDir,
  ...
  // this structure is fill by the loadForm() function and will contain all you
  // need
  std::map<std::string, CSoundSerializer> Container;
  nlassert(!_Loaded);
  // Just call the GEORGE::loadFrom method to read all available sounds
  ::loadForm("sound", packedSheetDir + "sounds.packed_sheets", Container,
             packedSheetUpdate, false);
  _Loaded = true;

  // add all the loaded sound in the sound banks
  std::map<std::string, CSoundSerializer>::iterator first(Container.begin()),
      last(Container.end());
  for (; first != last; ++first) {
...
void CSoundBank::unload() {
  ...
  /*	vector<CSound*> vec;
      ...
      {
          // We can't delete directly second because the map is based on
     second->getName() vec.push_back( (*map_iter).second );
      }
 ...
 */
bool CSoundBank::isLoaded() { return _Loaded; }
 ...
 */
uint CSoundBank::countSounds() { return (uint)_Sounds.size(); }

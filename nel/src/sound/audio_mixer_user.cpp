...
UAudioMixer *UAudioMixer::createAudioMixer() { return new CAudioMixerUser(); }

// ******************************************************************

CAudioMixerUser::CAudioMixerUser()
    : _AutoLoadSample(false), _UseADPCM(true), _SoundDriver(NULL),
      _SoundBank(NULL), _SampleBankManager(NULL), _BackgroundSoundManager(NULL),
      _ClusteredSound(0), _ReverbEffect(NULL), _ListenPosition(CVector::Null),
      _BackgroundMusicManager(NULL), _PlayingSources(0),
      _PlayingSourcesMuted(0), _Leaving(false) {
#if NL_PROFILE_MIXER
  _UpdateTime = 0.0;
  _CreateTime = 0.0;
  _UpdateCount = 0;
...
std::vector<UAudioMixer::TDriverInfo> UAudioMixer::getDrivers() {
  ...
  for (uint i = 0; i < drv.size(); ++i) {
    ...
    case ISoundDriver::DriverAuto:
      drivers.push_back(
          TDriverInfo(DriverAuto, ISoundDriver::getDriverName(drv[i])));
      break;
    case ISoundDriver::DriverFMod:
      drivers.push_back(
          TDriverInfo(DriverFMod, ISoundDriver::getDriverName(drv[i])));
      break;
    case ISoundDriver::DriverDSound:
      drivers.push_back(
          TDriverInfo(DriverDSound, ISoundDriver::getDriverName(drv[i])));
      break;
    case ISoundDriver::DriverOpenAl:
      drivers.push_back(
          TDriverInfo(DriverOpenAl, ISoundDriver::getDriverName(drv[i])));
      break;
    case ISoundDriver::DriverXAudio2:
      drivers.push_back(
          TDriverInfo(DriverXAudio2, ISoundDriver::getDriverName(drv[i])));
      break;
    default:
      nlwarning("Unknown driver id %d, name '%s'", drv[i],
                ISoundDriver::getDriverName(drv[i]));
      break;
    }
  }
...
void CAudioMixerUser::initClusteredSound(NL3D::UScene *uscene, float minGain,
                                         float maxDistance,
...
void CAudioMixerUser::initClusteredSound(NL3D::CScene *scene, float minGain,
                                         float maxDistance,
  ...
  _ClusteredSound->init(scene, portalInterpolate, maxDistance, minGain,
                        m_EnableOcclusionObstruction, m_EnableReverb);
}

void CAudioMixerUser::setPriorityReserve(TSoundPriority priorityChannel,
                                         uint reserve) {
  _PriorityReserve[priorityChannel] =
      (uint32)min((uint)_Tracks.size(), reserve);
}
...
void CAudioMixerUser::writeProfile(std::string &out) {
  ...
  */
  ...
  out += "\tPlaying simple sources: " + toString(countPlayingSimpleSources()) +
         " / " + toString(countSimpleSources()) + " \n";
  out += "\tAvailable tracks: " + toString(getAvailableTracksCount()) + " \n";
  out += "\tUsed tracks: " + toString(getUsedTracksCount()) + " \n";
  //	out << "\tMuted sources: " << nb << " \n";
  //	out << "\tMuted sources: " << max(0,
  //sint(_PlayingSources.size())-sint(_NbTracks)) << " \n"; 	out << "\tMuted
  //sources: " << max(0, sint(_PlayingSources)-sint(_NbTracks)) << " \n";
  out += "\tMuted sources: " + toString((int)_PlayingSourcesMuted) + "\n";
  out +=
      "\tSources waiting for play: " + toString(_SourceWaitingForPlay.size()) +
      " \n";
  out += "\tHighestPri: " + toString((int)_ReserveUsage[HighestPri]) + " / " +
         toString((int)_PriorityReserve[HighestPri]) + " \n";
  out += "\tHighPri:    " + toString((int)_ReserveUsage[HighPri]) + " / " +
         toString((int)_PriorityReserve[HighPri]) + "\n";
  out += "\tMidPri:     " + toString((int)_ReserveUsage[MidPri]) + " / " +
         toString((int)_PriorityReserve[MidPri]) + " \n";
  out += "\tLowPri:     " + toString((int)_ReserveUsage[LowPri]) + " / " +
         toString((int)_PriorityReserve[LowPri]) + " \n";
  out += "\tFreeTracks: " + toString(_FreeTracks.size()) + " / " +
         toString(_Tracks.size()) + " \n";
  out += "\tAverage update time: " +
         toString(1000.0 * _UpdateTime / iavoid0(_UpdateCount)) + " msec\n";
  out += "\tAverage create time: " +
         toString(1000.0 * _CreateTime / iavoid0(_CreateCount)) + " msec\n";
  out += "\tEstimated CPU: " +
         toString((100.0 * 1000.0 * (_UpdateTime + _CreateTime) / curTime())) +
         "%%\n";
...
void CAudioMixerUser::removeSourceWaitingForPlay(CSourceCommon *source) {
  std::list<CSourceCommon *>::iterator it =
      find(_SourceWaitingForPlay.begin(), _SourceWaitingForPlay.end(), source);
...
void CAudioMixerUser::reset() {
  ...
  while (!_Sources.empty()) {
    // removeSource( _Sources.begin(), true ); // 3D sources, the envsounds were
    // removed above
...
void CAudioMixerUser::setPackedSheetOption(const std::string &path,
                                           bool update) {
...
void CAudioMixerUser::setSamplePaths(const std::string &wavAssetPath,
                                     const std::string &bankBuildPath) {
...
void CAudioMixerUser::init(uint maxTrack, bool useEax, bool useADPCM,
                           IProgressCallback *progressCallBack,
                           bool autoLoadSample, TDriver driverType,
                           bool forceSoftwareBuffer, bool manualRolloff) {
  nlctassert(NumDrivers == UAudioMixer::TDriver(ISoundDriver::NumDrivers));
  initDriver(
      NLSOUND::ISoundDriver::getDriverName((ISoundDriver::TDriver)driverType));
  CInitInfo initInfo;
...
void CAudioMixerUser::initDriver(const std::string &driverName) {
  ...
  case NumDrivers:
    driverType = ISoundDriver::DriverAuto;
    nlwarning("AM: driverName value '%s' ('%s') is not available.",
              driverName.c_str(), dn.c_str());
    break;
  }
...
/// Initialize the selected device on the currently initialized driver. Leave
/// deviceName empty to select the default device.
void CAudioMixerUser::initDevice(const std::string &deviceName,
                                 const CInitInfo &initInfo,
  ...
  try {
    ...
    if (m_EnableReverb)
      driverOptions |= ISoundDriver::OptionReverbEffect;
    if (m_EnableOcclusionObstruction)
      driverOptions |= ISoundDriver::OptionFilterEffect;
    if (_UseADPCM)
      driverOptions |= ISoundDriver::OptionAllowADPCM;
    if (forceSoftware)
      driverOptions |= ISoundDriver::OptionSoftwareBuffer;
    if (manualRolloff)
      driverOptions |= ISoundDriver::OptionManualRolloff;
    if (_AutoLoadSample)
      driverOptions |= ISoundDriver::OptionLocalBufferCopy;

    // init the driver with selected device and needed options
    _SoundDriver->initDevice(deviceName,
                             (ISoundDriver::TSoundOptions)driverOptions);

    // verify the options, OptionHasBufferStreaming not checked
    if (m_EnableReverb &&
        !_SoundDriver->getOption(ISoundDriver::OptionReverbEffect)) {
      nlwarning("AM: OptionReverbEffect not available, m_EnableReverb = false");
      m_EnableReverb = false;
    }
    if (m_EnableOcclusionObstruction &&
        !_SoundDriver->getOption(ISoundDriver::OptionFilterEffect)) {
      nlwarning("AM: OptionFilterEffect not available, "
    ...
    if (_AutoLoadSample &&
        !_SoundDriver->getOption(ISoundDriver::OptionLocalBufferCopy)) {
      nlwarning(
          "AM: OptionLocalBufferCopy not available, _AutoLoadSample = false");
      _AutoLoadSample = false;
    }
    if (forceSoftware &&
        !_SoundDriver->getOption(ISoundDriver::OptionSoftwareBuffer)) {
      nlwarning("AM: OptionSoftwareBuffer not available, forceSoftwareBuffer = "
                "false");
      forceSoftware = false; // not really needed, but set anyway in case this
                             // is still used later in this function
    }
    if (manualRolloff &&
      ...
      manualRolloff = false; // not really needed, but set anyway in case this
                             // is still used later in this function
    }
  } catch (const ESoundDriver &e) {
    nlwarning(e.what());
  ...
  if (m_EnableReverb) {
    ...
    } else // createEffect succeeded, add environments
    {
      ...
      // todo: loading this data from a file or something would be neat (check
      // https://wiki.thedarkmod.com/index.php?title=Setting_Reverb_Data_of_Rooms_(EAX)
      // for reference) also: check if this should go into clustered_sound
      // (background_sound_manager also uses this stuff at one point, though)
      // effect presets (based on I3DL2 specification/guidelines, see 3dl2help.h
      // and efx-presets.h) // TODO: Consolidate differences in density and
      // diffusion parameter
      addEnvironment("GENERIC",
                     IReverbEffect::CEnvironment(
                         0, 7.5f, -10.00f, -1.00f, 1.49f, 0.83f, -26.02f,
                         0.007f, 2.00f, 0.011f, 100.0f, 100.00f, 0x3f));
      addEnvironment("PADDEDCELL", IReverbEffect::CEnvironment(
                                       1, 1.4f, -10.00f, -60.00f, 0.17f, 0.10f,
                                       -12.04f, 0.001f, 2.07f, 0.002f, 100.0f,
                                       100.00f, 0x3f)); // TODO: Density
      addEnvironment("ROOM", IReverbEffect::CEnvironment(
                                 2, 1.9f, -10.00f, -4.54f, 0.40f, 0.83f,
                                 -16.46f, 0.002f, 0.53f, 0.003f, 100.0f,
                                 100.00f, 0x3f)); // TODO: Density
      addEnvironment("BATHROOM", IReverbEffect::CEnvironment(
                                     3, 1.4f, -10.00f, -12.00f, 1.49f, 0.54f,
                                     -3.70f, 0.007f, 10.30f, 0.011f, 100.0f,
                                     60.00f, 0x3f)); // TODO: Density
      addEnvironment("LIVINGROOM", IReverbEffect::CEnvironment(
                                       4, 2.5f, -10.00f, -60.00f, 0.50f, 0.10f,
                                       -13.76f, 0.003f, -11.04f, 0.004f, 100.0f,
                                       100.00f, 0x3f)); // TODO: Density
      addEnvironment("STONEROOM",
                     IReverbEffect::CEnvironment(
                         5, 11.6f, -10.00f, -3.00f, 2.31f, 0.64f, -7.11f,
                         0.012f, 0.83f, 0.017f, 100.0f, 100.00f, 0x3f));
      addEnvironment("AUDITORIUM",
                     IReverbEffect::CEnvironment(
                         6, 21.6f, -10.00f, -4.76f, 4.32f, 0.59f, -7.89f,
                         0.020f, -2.89f, 0.030f, 100.0f, 100.00f, 0x3f));
      addEnvironment("CONCERTHALL",
                     IReverbEffect::CEnvironment(
                         7, 19.6f, -10.00f, -5.00f, 3.92f, 0.70f, -12.30f,
                         0.020f, -0.02f, 0.029f, 100.0f, 100.00f, 0x3f));
      addEnvironment(
          "CAVE", IReverbEffect::CEnvironment(8, 14.6f, -10.00f, 0.00f, 2.91f,
                                              1.30f, -6.02f, 0.015f, -3.02f,
                                              0.022f, 100.0f, 100.00f, 0x1f));
      addEnvironment(
          "ARENA", IReverbEffect::CEnvironment(9, 36.2f, -10.00f, -6.98f, 7.24f,
                                               0.33f, -11.66f, 0.020f, 0.16f,
                                               0.030f, 100.0f, 100.00f, 0x3f));
      addEnvironment("HANGAR",
                     IReverbEffect::CEnvironment(
                         10, 50.3f, -10.00f, -10.00f, 10.05f, 0.23f, -6.02f,
                         0.020f, 1.98f, 0.030f, 100.0f, 100.00f, 0x3f));
      addEnvironment(
          "CARPETEDHALLWAY",
          IReverbEffect::CEnvironment(11, 1.9f, -10.00f, -40.00f, 0.30f, 0.10f,
                                      -18.31f, 0.002f, -16.30f, 0.030f, 100.0f,
                                      100.00f, 0x3f)); // TODO: Density
      addEnvironment("HALLWAY", IReverbEffect::CEnvironment(
                                    12, 1.8f, -10.00f, -3.00f, 1.49f, 0.59f,
                                    -12.19f, 0.007f, 4.41f, 0.011f, 100.0f,
                                    100.00f, 0x3f)); // TODO: Density
      addEnvironment("STONECORRIDOR",
                     IReverbEffect::CEnvironment(
                         13, 13.5f, -10.00f, -2.37f, 2.70f, 0.79f, -12.14f,
                         0.013f, 3.95f, 0.020f, 100.0f, 100.00f, 0x3f));
      addEnvironment(
          "ALLEY", IReverbEffect::CEnvironment(14, 7.5f, -10.00f, -2.70f, 1.49f,
                                               0.86f, -12.04f, 0.007f, -0.04f,
                                               0.011f, 100.0f, 100.00f, 0x3f));
      addEnvironment("FOREST",
                     IReverbEffect::CEnvironment(
                         15, 38.0f, -10.00f, -33.00f, 1.49f, 0.54f, -25.60f,
                         0.162f, -6.13f, 0.088f, 79.0f, 100.00f, 0x3f));
      addEnvironment(
          "CITY", IReverbEffect::CEnvironment(16, 7.5f, -10.00f, -8.00f, 1.49f,
                                              0.67f, -22.73f, 0.007f, -22.17f,
                                              0.011f, 50.0f, 100.00f, 0x3f));
      addEnvironment("MOUNTAINS",
                     IReverbEffect::CEnvironment(
                         17, 100.0f, -10.00f, -25.00f, 1.49f, 0.21f, -27.80f,
                         0.300f, -20.14f, 0.100f, 27.0f, 100.00f, 0x1f));
      addEnvironment("QUARRY",
                     IReverbEffect::CEnvironment(
                         18, 17.5f, -10.00f, -10.00f, 1.49f, 0.83f, -100.00f,
                         0.061f, 5.00f, 0.025f, 100.0f, 100.00f, 0x3f));
      addEnvironment("PLAIN",
                     IReverbEffect::CEnvironment(
                         19, 42.5f, -10.00f, -20.00f, 1.49f, 0.50f, -24.66f,
                         0.179f, -25.14f, 0.100f, 21.0f, 100.00f, 0x3f));
      addEnvironment("PARKINGLOT",
                     IReverbEffect::CEnvironment(
                         20, 8.3f, -10.00f, 0.00f, 1.65f, 1.50f, -13.63f,
                         0.008f, -11.53f, 0.012f, 100.0f, 100.00f, 0x1f));
      addEnvironment("SEWERPIPE", IReverbEffect::CEnvironment(
                                      21, 1.7f, -10.00f, -10.00f, 2.81f, 0.14f,
                                      4.29f, 0.014f, 6.48f, 0.021f, 80.0f,
                                      60.00f, 0x3f)); // TODO: Density
      addEnvironment("UNDERWATER", IReverbEffect::CEnvironment(
                                       22, 1.8f, -10.00f, -40.00f, 1.49f, 0.10f,
                                       -4.49f, 0.007f, 17.00f, 0.011f, 100.0f,
                                       100.00f, 0x3f)); // TODO: Density
      addEnvironment("DRUGGED",
                     IReverbEffect::CEnvironment(
                         23, 1.9f, -10.00f, 0.00f, 8.39f, 1.39f, -1.15f, 0.002f,
                         9.85f, 0.030f, 50.0f, 42.87f, 0x1f)); // TODO: Density
      addEnvironment("DIZZY", IReverbEffect::CEnvironment(
                                  24, 1.8f, -10.00f, -4.00f, 17.23f, 0.56f,
                                  -17.13f, 0.020f, -6.13f, 0.030f, 60.0f,
                                  36.45f, 0x1f)); // TODO: Density
      addEnvironment("PSYCHOTIC", IReverbEffect::CEnvironment(
                                      25, 1.0f, -10.00f, -1.51f, 7.56f, 0.91f,
                                      -6.26f, 0.020f, 7.74f, 0.030f, 50.0f,
                                      6.25f, 0x1f)); // TODO: Density
      // FIXME: Don't have accurate room size of the following environments, are
      // we using these?
      addEnvironment("SMALLROOM",
                     IReverbEffect::CEnvironment(
                         26, 7.5f, -10.00f, -6.00f, 1.10f, 0.83f, -4.00f,
                         0.005f, 5.00f, 0.010f, 100.0f, 100.00f, 0x20));
      addEnvironment("MEDIUMROOM",
                     IReverbEffect::CEnvironment(
                         26, 7.5f, -10.00f, -6.00f, 1.30f, 0.83f, -10.00f,
                         0.010f, -2.00f, 0.020f, 100.0f, 100.00f, 0x20));
      addEnvironment("LARGEROOM",
                     IReverbEffect::CEnvironment(
                         26, 7.5f, -10.00f, -6.00f, 1.50f, 0.83f, -16.00f,
                         0.020f, -10.00f, 0.040f, 100.0f, 100.00f, 0x20));
      addEnvironment("MEDIUMHALL",
                     IReverbEffect::CEnvironment(
                         26, 7.5f, -10.00f, -6.00f, 1.80f, 0.70f, -13.00f,
                         0.015f, -8.00f, 0.030f, 100.0f, 100.00f, 0x20));
      addEnvironment("LARGEHALL",
                     IReverbEffect::CEnvironment(
                         26, 7.5f, -10.00f, -6.00f, 1.80f, 0.70f, -20.00f,
                         0.030f, -14.00f, 0.060f, 100.0f, 100.00f, 0x20));
      addEnvironment(
          "PLATE", IReverbEffect::CEnvironment(26, 7.5f, -10.00f, -2.00f, 1.30f,
                                               0.90f, 0.00f, 0.002f, 0.00f,
                                               0.010f, 100.0f, 75.00f, 0x20));
      // these are the default environment settings in case no environment data
      // is available (you'll hear this one)
      _DefaultEnvironment = getEnvironment("PLAIN");
      _DefaultRoomSize =
          _DefaultEnvironment.RoomSize; // changed from 7.5f, since 42.5f is the
                                        // real default for PLAIN
  ...
  nlinfo("AM: Initialized audio mixer with %u voices, %s, %s, and %s.",
         ...
         m_EnableOcclusionObstruction ? "with occlusion and obstruction support"
                                      : "WITHOUT occlusion and obstruction",
         _UseADPCM ? "with ADPCM sample source"
                   : "with 16 bits PCM sample source");
  ...
  try {
    ...
    if (!mixerConfigFile.empty()) {
      ...
      if (bgFlags != NULL) {
        ...
        uint i;
        for (i = 0;
             i < min(size, (uint)(TBackgroundFlags::NB_BACKGROUND_FLAGS));
             ++i) {
        ...
        for (; i < TBackgroundFlags::NB_BACKGROUND_FLAGS; ++i) {
          ...
          root.getValueByName(fadeIn, ".BackgroundFlags[0].FadeIn",
                              NLGEORGES::UFormElm::Eval, &where);
          root.getValueByName(fadeOut, ".BackgroundFlags[0].FadeOut",
                              NLGEORGES::UFormElm::Eval, &where);
          root.getValueByName(flags.Flags[i],
                              ".BackgroundFlags[0].InitialState",
                              NLGEORGES::UFormElm::Eval, &where);
...
/// Build a sample bank from a directory containing .wav files, and return the
/// path to the written file.
std::string UAudioMixer::buildSampleBank(const std::string &wavDir,
...
/// Build a sample bank from a list of .wav files, and return the path to the
/// written file.
std::string
UAudioMixer::buildSampleBank(const std::vector<std::string> &sampleList,
                             const std::string &bankDir,
                             const std::string &bankName) {
  ...
  for (uint j = 0; j < sampleList.size(); ++j) {
    nldebug("  Adding sample [%s] into bank",
            CFile::getFilename(sampleList[j]).c_str());
    ...
    if (!IBuffer::readWav(&buffer[0], size, result, bufferFormat, channels,
                          bitsPerSample, frequency)) {
      nlwarning("    IBuffer::readWav returned false");
      continue;
    }

    vector<sint16> mono16Data;
    if (!IBuffer::convertToMono16PCM(&result[0], (uint)result.size(),
                                     mono16Data, bufferFormat, channels,
                                     bitsPerSample)) {
    ...
    if (!IBuffer::convertMono16PCMToMonoADPCM(
            &mono16Data[0], (uint)mono16Data.size(), adpcmData)) {
    ...
    if (mono16Data.size() & 1)
      nlwarning("Uneven sample numbers, ADPCM will miss a sample. File: %s, "
                "Samples: %i, ADPCM Size: %i",
                sampleList[j].c_str(), (int)mono16Data.size(),
                (int)adpcmData.size());
    ...
    hdr.addSample(
        CFile::getFilename(sampleList[j]), frequency, (uint32)mono16Data.size(),
        (uint32)mono16Buffers[j].size() * 2, (uint32)adpcmBuffers[j].size());
  }

  // write the sample bank (if any sample available)
  if (!hdr.Name.empty()) {
    string filename =
        CPath::standardizePath(bankDir, true) + bankName + ".sample_bank";
    COFile sbf(filename);
    sbf.serial(hdr);
    // nldebug("Header seeking = %i", sbf.getPos());
    nlassert(mono16Buffers.size() == adpcmBuffers.size());
    for (uint j = 0; j < mono16Buffers.size(); ++j) {
      sbf.serialBuffer((uint8 *)(&mono16Buffers[j][0]),
                       (uint)mono16Buffers[j].size() * 2);
      sbf.serialBuffer((uint8 *)(&adpcmBuffers[j][0]),
                       (uint)adpcmBuffers[j].size());
    }
...
void CAudioMixerUser::buildSampleBankList() {
  ...
  for (i = 0; i < bankDir.size(); ++i) {
    ...
    if (i >= bankFile.size() ||
        CFile::getFilenameWithoutExtension(bankFile[i]) > bankname) {
      nlinfo("Compiling sample bank [%s]", bankname.c_str());
      std::string filename = buildSampleBank(bankDir[i], sbp, bankname);
      if (bankFile.size() < i + 1)
        bankFile.resize(i + 1);
      else
        bankFile.insert(bankFile.begin() + i, std::string());
      bankFile[i] = filename;
      ...
      try {
        ...
        if (sampleList.size() == hdr.Name.size()) {
          for (uint j = 0; j < sampleList.size(); ++j) {
            ...
            if (CFile::getFileModificationDate(sampleList[j]) >=
                CFile::getFileModificationDate(bankFile[i])) {
              upToDate = false;
              break;
            }
          }
  ...
  /*
      // clear the exisiting list file
      {
          ...
          {
              if (fileList[i].find(".sample_bank_list") == fileList[i].size() -
     17)
              {
...
void CAudioMixerUser::setBackgroundFlagName(uint flagIndex,
                                            const std::string &flagName) {
  if (flagIndex < TBackgroundFlags::NB_BACKGROUND_FLAGS)
    _BackgroundFilterNames[flagIndex] = flagName;
}
void CAudioMixerUser::setBackgroundFlagShortName(
    uint flagIndex, const std::string &flagShortName) {
  if (flagIndex < TBackgroundFlags::NB_BACKGROUND_FLAGS)
...
const UAudioMixer::TBackgroundFilterFades &
...
public:
  ...
  void serial(NLMISC::IStream &s) { s.serialCont(Controlers); }
...
void CAudioMixerUser::initUserVar() {
  ...
  ::loadForm("user_var_binding",
             _PackedSheetPath + "user_var_binding.packed_sheets", Container,
             _UpdatePackedSheet, false);
  // fill the real container.
  std::map<std::string, CUserVarSerializer>::iterator first(Container.begin()),
      last(Container.end());
  for (; first != last; ++first) {
    for (uint i = 0; i < first->second.Controlers.size(); ++i) {
      _UserVarControls.insert(make_pair(first->second.Controlers[i].Name,
                                        first->second.Controlers[i]));
    }
  }

  // update all the sounds to refer to the controler.
  {
    TUserVarControlsContainer::iterator first(_UserVarControls.begin()),
        last(_UserVarControls.end());
    for (; first != last; ++first) {
      std::vector<NLMISC::TStringId>::iterator first2(
          first->second.SoundNames.begin()),
          last2(first->second.SoundNames.end());
...
/// Build the sound bank packed sheets file from georges sound sheet files with
/// .sound extension in the search path, and return the path to the written
/// file.
...
std::string
UAudioMixer::buildClusteredSoundGroupSheets(const std::string &packedSheetDir) {
  std::string dir = CPath::standardizePath(packedSheetDir, true);
...
std::string
UAudioMixer::buildUserVarBindingSheets(const std::string &packedSheetDir) {
  std::string dir = CPath::standardizePath(packedSheetDir, true);
  std::map<std::string, CUserVarSerializer> container;
  ::loadForm("user_var_binding", dir + "user_var_binding.packed_sheets",
...
void CAudioMixerUser::setUserVar(NLMISC::TStringId varName, float value) {
  ...
  if (it != _UserVarControls.end()) {
    ...
    //		if (it->second.Value != value)
    {
      ...
      // update all sources
      std::set<CSourceCommon *>::iterator first(it->second.Sources.begin()),
          last(it->second.Sources.end());
      for (; first != last; ++first) {
...
void CAudioMixerUser::addUserControledSource(CSourceCommon *source,
...
void CAudioMixerUser::removeUserControledSource(CSourceCommon *source,
                                                NLMISC::TStringId varName) {
  TUserVarControlsContainer::iterator it(_UserVarControls.find(varName));
  if (it != _UserVarControls.end()) {
...
void CAudioMixerUser::enable(bool /* b */) {
  ...
  //
  nlassert(_PlayingSources.find(_Tracks[i]->getSource()) !=
  _PlayingSources.end());
...
ISoundDriver *CAudioMixerUser::getSoundDriver() { return _SoundDriver; }

// ******************************************************************

void CAudioMixerUser::getFreeTracks(uint nb, CTrack **tracks) {
  std::vector<CTrack *>::iterator first(_FreeTracks.begin()),
      last(_FreeTracks.end());
...
//		if (_UseEax) free_track->getPhysicalSource()->setEffect(NULL);
//// no reverb! 		return free_track;
//	}
...
//			if (_FreeTracks.empty())
//			{
//				nlwarning("No free track after cutting a playing
//sound source !");
//			}
//			else
//			{
...
//				++_ReserveUsage[HighestPri];
//				if (_UseEax)
//free_track->getPhysicalSource()->setEffect(NULL); // no reverb! 				return
//free_track;
//			}
//		}
//	}
...
CTrack *CAudioMixerUser::getFreeTrack(CSourceCommon *source) {
  ...
  if (!_FreeTracks.empty()) {
    ...
    if (_FreeTracks.size() > _LowWaterMark ||
        _ReserveUsage[source->getPriority()] <
            _PriorityReserve[source->getPriority()]) {
      ...
      if (source->getPriority() == HighestPri ||
        ...
        _ReserveUsage[source->getPriority()]++;
        //				nldebug("Track %p assign to source %p",
        //ret, ret->getSource());
        return ret;
      }
    }
  ...
  // try to find a track with a source cuttable
  {
    ...
    float d1, d2, t1, t2;
    d1 = source->getSourceRelativeMode()
             ? source->getPos().norm()
             : (source->getPos() - _ListenPosition).norm();
    ...
    for (uint i = 0; i < _Tracks.size(); ++i) {
      ...
      if (src2) {
        ...
        d2 = src2->getSourceRelativeMode()
                 ? src2->getPos().norm()
                 : (src2->getPos() - _ListenPosition).norm();
        t2 = max(0.0f,
                 1.0f - ((d2 - src2MinDist) / (src2MaxDist - src2MinDist)));

        const float tfactor = 1.3f;
        if (t1 > t2 * tfactor)
        //				if (d1 < d2)
        {
          //					nldebug("Cutting source %p with
          //source %p (%f > %f*%f)", src2, source, t1, tfactor, t2);
          ...
          if (_FreeTracks.empty()) {
            ...
            ret->setLogicalSource(source);
            _ReserveUsage[source->getPriority()]++;
            //						nldebug("Track %p assign
            //to source %p", ret, ret->getSource());
            return ret;
          }
        }
      }
...
//	nlassert(track);
//
//	if (_UseEax) track->getPhysicalSource()->setEffect(_ReverbEffect); //
//return reverb!
//	--_ReserveUsage[HighestPri];
...
void CAudioMixerUser::getPlayingSoundsPos(
    bool virtualPos, std::vector<std::pair<bool, NLMISC::CVector>> &pos) {
  int nbplay = 0;
  int nbmute = 0;
  int nbsrc = 0;
  ...
  for (; first != last; ++first) {
    ...
    if (ps->getType() == CSourceCommon::SOURCE_SIMPLE) {
      ...
      if (source->isPlaying()) {
        if (virtualPos)
          pos.push_back(
              make_pair(source->getTrack() == 0, source->getVirtualPos()));
        else
        ...
        if (source->getTrack() == 0)
          ...
          //					nldebug ("Source %p playing on
          //track %p", source, source->getTrack());
          nbplay++;
        }
      }
  ...
  //	nldebug("Total source : %d, playing : %d, muted : %d", nbsrc, nbplay,
  //nbmute);
}
...
*/
  ...
  // update the object.
  {
    ...
    {
      std::vector<std::pair<IMixerUpdate *, bool>>::iterator first(
          _UpdateEventList.begin()),
          last(_UpdateEventList.end());
      for (; first != last; ++first) {
        if (first->second) {
          //					nldebug("Inserting update %p",
          //first->first);
          _UpdateList.insert(first->first);
        } else {
          //					nldebug("Removing update %p",
          //first->first);
    ...
    // 2nd, do the update
    {
      TMixerUpdateContainer::iterator first(_UpdateList.begin()),
          last(_UpdateList.end());
      for (; first != last; ++first) {
        if (*first == 0) {
  ...
  // send the event.
  {
    ...
    // **** 1st, update the event list
    {
      std::list<std::pair<NLMISC::TTime, IMixerEvent *>>::iterator first(
          _EventListUpdate.begin()),
          last(_EventListUpdate.end());
      for (; first != last; ++first) {
        // add an event
        //				nldebug ("Add event %p", first->second);
        TTimedEventContainer::iterator it(_EventList.insert(make_pair(
            first->first, NLMISC::CDbgPtr<IMixerEvent>(first->second))));
    ...
    while (!_EventList.empty() && _EventList.begin()->first <= now) {
      ...
      TEventContainer::iterator it(
          _Events.lower_bound(_EventList.begin()->second));
      ...
      // now, run the event
      //			nldebug("Sending Event %p",
      //_EventList.begin()->second);
  ...
  for (i = 0; i < _Tracks.size(); ++i) {
    if (!_Tracks[i]->isPlaying()) {
      ...
      if (!_SourceWaitingForPlay.empty() && nbWaitingSources) {
        ...
        //				nldebug("Before POP Sources waiting :
        //%u", _SourceWaitingForPlay.size());
        _SourceWaitingForPlay.pop_front();
        --nbWaitingSources;
        //				nldebug("After POP Sources waiting :
        //%u", _SourceWaitingForPlay.size());
      }
    }
  }
      ...
      if (_Tracks[i]->isPlaying()) {
        if (_Tracks[i]->getLogicalSource() != 0) {
          ...
          if (source->getCluster() != 0) {
            ...
            const CClusteredSound::CClusterSoundStatus *css =
                _ClusteredSound->getClusterSoundStatus(source->getCluster());
            if (css != 0) {
              // there is some data here, update the virtual position of the
              // sound.
              float dist = (css->Position - source->getPos()).norm();
              CVector vpos(_ListenPosition +
                           css->Direction * (css->Dist + dist));
              //							_Tracks[i]->DrvSource->setPos(source->getPos()
              //* (1-css->PosAlpha) + css->Position*(css->PosAlpha));
              isource->setPos(source->getPos() * (1 - css->PosAlpha) +
                              vpos * (css->PosAlpha));
              ...
              if (m_EnableOcclusionObstruction) {
                ...
                CFilterParameters params(css->Occlusion, css->OcclusionLFFactor,
                                         css->OcclusionRoomRatio,
                                         css->Obstruction);

                isource->setDirectGain(params.DirectGain);
                isource->setDirectFilter(
                    ISource::TFilter::FilterLowPass, css->DirectCutoffFrequency,
                    css->DirectCutoffFrequency, params.DirectGainPass);
                isource->enableDirectFilter(true);

                if (m_EnableReverb) {
                  isource->setEffectGain(params.EffectGain);
                  isource->setEffectFilter(ISource::TFilter::FilterLowPass,
                                           css->EffectCutoffFrequency,
                                           css->EffectCutoffFrequency,
                                           params.EffectGainPass);
  ...
  for ( i=0; i!=_NbTracks; i++ )
  {
      ...
      if ( su = _Tracks[i]->getSource() )
      {
          nldebug( "%u: %p %s %s %s %s, vol %u",
                  i, &_Tracks[i]->DrvSource,
  _Tracks[i]->isAvailable()?"FREE":"USED",
...
void CAudioMixerUser::addSource(CSourceCommon *source) {
  ...
  //	_profile(( "AM: ADDSOURCE, SOUND: %d, TRACK: %p, NAME=%s",
  //source->getSound(), source->getTrack(), 			source->getSound() &&
  //(!source->getSound()->getName().empty()) ?
  //source->getSound()->getName().c_str() : "" ));
}

static bool checkSound(CSound *sound,
                       const vector<pair<string, CSound *>> &subsounds,
                       vector<string> &missingFiles) {
  vector<pair<string, CSound *>>::const_iterator first(subsounds.begin()),
      last(subsounds.end());
...
bool CAudioMixerUser::tryToLoadSampleBank(const std::string &sampleName) {
  ...
  if (!path.empty()) {
    ...
    nlwarning("tryToLoadSoundBank : can't find sample bank for '%s'",
              sampleName.c_str());
    return false;
  }
}
...
UGroupController *CAudioMixerUser::getGroupController(const std::string &path) {
  return static_cast<UGroupController *>(
      _GroupController.getGroupController(path));
}

// ******************************************************************

USource *CAudioMixerUser::createSource(TSoundId id, bool spawn,
                                       TSpawnEndCallback cb, void *userParam,
  ...
  _profile((
      "AM: [%u]---------------------------------------------------------------",
      curTime()));
  _profile(("AM: CREATESOURCE: SOUND=%p, NAME=%s, TIME=%d", id,
            id->getName().c_str(), curTime()));
  _profile(("AM: SOURCES: %d, PLAYING: %d, TRACKS: %d", getSourcesNumber(),
  ...
  if (_AutoLoadSample) {
    if (id->getSoundType() == CSound::SOUND_SIMPLE) {
      ...
      if (ss->getBuffer() == NULL) {
        const string sampleName =
            CStringMapper::unmap(ss->getBuffername()) + ".wav";
    ...
    retrySound:
      ...
      if (invalid) {
        nlwarning("The sound %s contain an infinite recursion !",
                  CStringMapper::unmap(id->getName()).c_str());
        return NULL;
      }
      ...
      if (!missingFiles.empty() /* && count <= missingFiles.size()*/) {
        ...
        for (uint i = 0; i < missingFiles.size(); ++i) {
          if (missingFiles[i].find(" (sample)") != string::npos) {
            ...
            string sample =
                missingFiles[i].substr(0, missingFiles[i].find(" (sample)")) +
                ".wav";
  ...
  case CSound::SOUND_SIMPLE: {
    ...
    if (simpleSound->getBuffer() == NULL) {
      ...
      const std::string &name =
          CStringMapper::unmap(simpleSound->getBuffername());
    ...
    // Create source
    CSimpleSource *source =
        new CSimpleSource(simpleSound, spawn, cb, userParam, cluster,
                          static_cast<CGroupController *>(groupController));
  ...
  case CSound::SOUND_STREAM_FILE: {
    ...
    ret =
        new CStreamFileSource(streamFileSound, spawn, cb, userParam, cluster,
                              static_cast<CGroupController *>(groupController));
  } break;
  ...
  case CSound::SOUND_BACKGROUND: {
    ...
    ret =
        new CBackgroundSource(bgSound, spawn, cb, userParam, cluster,
                              static_cast<CGroupController *>(groupController));
  } break;
  ...
  case CSound::SOUND_CONTEXT: {
    ...
    if (sound != 0) {
      ret = createSource(sound, spawn, cb, userParam, cluster, NULL,
                         static_cast<CGroupController *>(groupController));
...
USource *CAudioMixerUser::createSource(const NLMISC::TStringId &name,
                                       bool spawn, TSpawnEndCallback cb,
                                       void *userParam, NL3D::CCluster *cluster,
                                       CSoundContext *context,
                                       UGroupController *groupController) {
...
/*
void				CAudioMixerUser::loadEnvEffects( const char
*filename )
{
...
uint32
CAudioMixerUser::loadSampleBank(bool async, const std::string &name,
                                std::vector<std::string> *notfoundfiles) {
  //	nlassert( filename != NULL );
...
bool CAudioMixerUser::unloadSampleBank(const std::string &name) {
  ...
  CSampleBank *pbank =
      _SampleBankManager->findSampleBank(CStringMapper::map(name));
...
void CAudioMixerUser::getSoundNames(
    std::vector<NLMISC::TStringId> &names) const {
  _SoundBank->getNames(names);
}

// ******************************************************************

uint CAudioMixerUser::getPlayingSourcesCount() const { return _PlayingSources; }

// ******************************************************************

uint CAudioMixerUser::countPlayingSimpleSources() const {
  uint count = 0;
  for (TSourceContainer::const_iterator it(_Sources.begin()),
       end(_Sources.end());
       it != end; ++it) {
...
// ******************************************************************
/*
void			CAudioMixerUser::loadEnvSounds( const char *filename,
UEnvSound **treeRoot )
{
    nlassert( filename != NULL );
...
void CAudioMixerUser::getLoadedSampleBankInfo(
...
NLMISC_CATEGORISED_COMMAND(nel, displaySoundInfo,
  ...
  log.displayNL(
      "%d tracks, MAX_TRACKS = %d, contains:",
      CAudioMixerUser::instance()->_Tracks.size(),
      CAudioMixerUser::instance()->getSoundDriver()->countMaxSources());
  ...
  for (uint i = 0; i < CAudioMixerUser::instance()->_Tracks.size(); i++) {
    if (CAudioMixerUser::instance()->_Tracks[i] == NULL) {
      ...
      log.displayNL(
          "Track %d %s available and %s playing.", i,
          (CAudioMixerUser::instance()->_Tracks[i]->isAvailable() ? "is"
                                                                  : "is not"),
          (CAudioMixerUser::instance()->_Tracks[i]->isPlaying() ? "is"
                                                                : "is not"));
      if (CAudioMixerUser::instance()->_Tracks[i]->getLogicalSource() == NULL) {
        log.displayNL("    CSourceCommon is NULL");
      } else {
        const CVector &pos = CAudioMixerUser::instance()
                                 ->_Tracks[i]
                                 ->getLogicalSource()
                                 ->getPos();
        string bufname;
        CSourceCommon *src =
            CAudioMixerUser::instance()->_Tracks[i]->getLogicalSource();
        switch (src->getType()) {
        case CSourceCommon::SOURCE_SIMPLE: {
          CSimpleSource *simpleSrc = static_cast<CSimpleSource *>(src);
          if (simpleSrc->getBuffer())
            bufname = CStringMapper::unmap(simpleSrc->getBuffer()->getName());
          log.displayNL("    CSourceCommon is CSimpleSource is id %d buffer "
                        "name '%s' pos %f %f %f",
                        simpleSrc->getSound(), bufname.c_str(), pos.x, pos.y,
                        pos.z);
        } break;
        default:
          log.displayNL("    CSourceCommon is id %d pos %f %f %f",
                        src->getSound(), pos.x, pos.y, pos.z);
          break;
        }
      }
    }
...
void CAudioMixerUser::unregisterBufferAssoc(CSound *sound, IBuffer *buffer) {
  ...
  if (it != _BufferToSources.end()) {
    std::vector<CSound *>::iterator first(it->second.begin()),
        last(it->second.end());
...
void CAudioMixerUser::registerUpdate(
    CAudioMixerUser::IMixerUpdate *pmixerUpdate) {
...
void CAudioMixerUser::unregisterUpdate(
    CAudioMixerUser::IMixerUpdate *pmixerUpdate) {
...
void CAudioMixerUser::addEvent(CAudioMixerUser::IMixerEvent *pmixerEvent,
...
void CAudioMixerUser::removeEvents(CAudioMixerUser::IMixerEvent *pmixerEvent) {
  ...
  // Slow O(N) but _EventListUpdate should be small, cause cleared each frame
  // and not so many events are added/removed.
  ...
  pair<TEventContainer::iterator, TEventContainer::iterator> range =
      _Events.equal_range(pmixerEvent);
...
void CAudioMixerUser::setBackgroundFlags(
    const TBackgroundFlags &backgroundFlags) {
  _BackgroundSoundManager->setBackgroundFlags(backgroundFlags);
}

void CAudioMixerUser::setBackgroundFilterFades(
    const TBackgroundFilterFades &backgroundFilterFades) {
  _BackgroundSoundManager->setBackgroundFilterFades(backgroundFilterFades);
}

// void CAudioMixerUser::loadBackgroundSoundFromRegion (const
// NLLIGO::CPrimRegion &region)
//{
//	_BackgroundSoundManager->loadSoundsFromRegion(region);
// }

// void CAudioMixerUser::loadBackgroundEffectsFromRegion (const
// NLLIGO::CPrimRegion &region)
//{
//	_BackgroundSoundManager->loadEffecsFromRegion(region);
// }
// void CAudioMixerUser::loadBackgroundSamplesFromRegion (const
// NLLIGO::CPrimRegion &region)
//{
//	_BackgroundSoundManager->loadSamplesFromRegion(region);
// }

void CAudioMixerUser::loadBackgroundAudioFromPrimitives(
    const NLLIGO::IPrimitive &audioRoot) {
  _BackgroundSoundManager->loadAudioFromPrimitives(audioRoot);
}

void CAudioMixerUser::playBackgroundSound() { _BackgroundSoundManager->play(); }

void CAudioMixerUser::stopBackgroundSound() { _BackgroundSoundManager->stop(); }
...
void CAudioMixerUser::changeMaxTrack(uint maxTrack) {
  ...
  if (maxTrack != max_track_old)
    nlwarning("AM: MaxTrack limited from %u to %u", (uint32)max_track_old,
              (uint32)maxTrack);
  ...
  if (maxTrack > prev_track_nb) {
    ...
    try {
      for (i = prev_track_nb; i < maxTrack; ++i) {
        ...
        if (m_EnableReverb)
          _Tracks[i]->getPhysicalSource()->setEffect(_ReverbEffect);
      ...
      delete _Tracks[i];
      // If the source generation failed, keep only the generated number of
      // sources
      maxTrack = i;
      _Tracks.resize(maxTrack);
      nlwarning("AM: Failed to create another track, MaxTrack is %u now",
                (uint32)maxTrack);
    }
  }
  // **** else must delete some tracks
    ...
    while (_Tracks.size() + non_erasable.size() > maxTrack &&
           !_Tracks.empty()) {
      ...
      if (track->isAvailable()) {
        ...
        if (track->getLogicalSource()) {
          ...
          _FreeTracks.erase(
              find(_FreeTracks.begin(), _FreeTracks.end(), track));
          delete track;
        }
    ...
    if (maxTrack != _Tracks.size())
      nlwarning("AM: Failed to reduce number of tracks; MaxTrack is now %u "
                "instead of the requested %u",
                (uint32)_Tracks.size(), (uint32)maxTrack);
  }
}
...
void CAudioMixerUser::debugLogEvent(const char *reason) {
  ...
  for (itEv = _Events.begin(); itEv != _Events.end(); itEv++) {
    nlinfo("\t: %x - (%d,%x)", itEv->first, (uint32)itEv->second->first,
           itEv->second->second.ptr());
  }
}

// ***************************************************************************
bool CAudioMixerUser::playMusicChannel(TMusicChannel chan,
                                       const std::string &fileName,
                                       uint xFadeTime, bool async, bool loop) {
...
// ***************************************************************************
bool CAudioMixerUser::playMusic(const std::string &fileName, uint xFadeTime,
                                bool async, bool loop) {
  return playMusicChannel(GeneralMusicChannel, fileName, xFadeTime, async,
                          loop);
}
...
// ***************************************************************************
bool CAudioMixerUser::getSongTitle(const std::string &filename,
                                   std::string &result, float &length) {
  if (_SoundDriver) {
    std::string artist;
    std::string title;
...
// ***************************************************************************
bool CAudioMixerUser::playEventMusic(const std::string &fileName,
                                     uint xFadeTime, bool async, bool loop) {
...
/// Get audio/container extensions that are currently supported by nel or the
...
void CAudioMixerUser::addEnvironment(
    const std::string &environmentName,
    const IReverbEffect::CEnvironment &environment) {
  if (_ReverbEffect) {
    TStringId environment_name = CStringMapper::map(environmentName);

    if (_Environments.find(environment_name) != _Environments.end())
      nlwarning("Reverb environment %s already exists, replacing with new one",
                CStringMapper::unmap(environment_name).c_str());
...
void CAudioMixerUser::setEnvironment(NLMISC::TStringId environmentName,
                                     float roomSize) {
  if (_ReverbEffect) {
...
CAudioMixerUser::getEnvironment(NLMISC::TStringId environmentName) {
  ...
  if (it == _Environments.end()) {
    nlwarning("Reverb environment '%s' does not exist, returning default",
              CStringMapper::unmap(environmentName).c_str());
...
NLMISC_CATEGORISED_COMMAND(nel, displaySoundProfile,

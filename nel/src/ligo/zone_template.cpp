...
inline bool CZoneTemplate::snapOnGrid(float &value, float resolution,
                                      float snap) {
  // Calc the floor
...
inline bool CZoneTemplate::isSnapedOnGrid(float value, float resolution,
                                          float snap) {
  // Snapped
  float snapped = value;
...
inline sint32 CZoneTemplate::getSnappedIndex(float value, float resolution,
                                             float snap) {
  // Snapped
  float snapped = value;
...
bool CZoneTemplate::build(const std::vector<NLMISC::CVector> &vertices,
  ...
  for (edge = 0; edge < edgeCount; edge++) {
    ...
    if (boundaryFlags[theEdge.first] && boundaryFlags[theEdge.second]) {
      // Common coordinates
      uint common =
          boundaryFlags[theEdge.first] & boundaryFlags[theEdge.second];
      ...
      if (common) {
        ...
        if (keep) {
          ...
          bool second =
              edgePairReverse.find(theEdge.second) != edgePairReverse.end();
          ...
          if (first || second) {
            ...
            if (first)
              errors.pushVertexError(CLigoError::VertexAlreadyUsed,
                                     theEdge.first, 0);
          ...
          if ((!first) && (!second)) {
            // Add to the map
            edgePair.insert(
                map<uint, uint>::value_type(theEdge.first, theEdge.second));
            edgePairReverse.insert(
                map<uint, uint>::value_type(theEdge.second, theEdge.first));
          }
        }
      }
    }
  ...
  if (ok) {
    ...
    while (vertIte != listVert.end()) {
      ...
      if (commonFlags & SnappedXFlag) {
        // Get x index
        sint32 prevIndex =
            getSnappedIndex(vertices[previous].x, config.CellSize, config.Snap);
        sint32 nextIndex =
            getSnappedIndex(vertices[next].x, config.CellSize, config.Snap);
    ...
    if (errored.empty()) {
      ...
      while (firstIte != listVert.end()) {
        // Corner ?
        if ((boundaryFlags[*firstIte] & (SnappedXFlag | SnappedYFlag)) ==
            (SnappedXFlag | SnappedYFlag))
          // Yes, exit
          break;
      ...
      for (;;) {
        ...
        // Corner ?
        if ((boundaryFlags[*vertIte] & (SnappedXFlag | SnappedYFlag)) ==
            (SnappedXFlag | SnappedYFlag)) {
          // Get the index of start and end of the edge
          sint32 startX = getSnappedIndex(vertices[edge[0]].x, config.CellSize,
                                          config.Snap);
          sint32 startY = getSnappedIndex(vertices[edge[0]].y, config.CellSize,
                                          config.Snap);
          sint32 endX = getSnappedIndex(vertices[edge[edge.size() - 1]].x,
                                        config.CellSize, config.Snap);
          sint32 endY = getSnappedIndex(vertices[edge[edge.size() - 1]].y,
                                        config.CellSize, config.Snap);
          ...
          if ((startX == endX) && (startY == endY)) {
            ...
            errors.pushVertexError(CLigoError::TwoCornerVertices,
                                   edge[edge.size() - 1], 0);
          ...
          if ((abs(startX - endX) > 1) || (abs(startY - endY) > 1)) {
            ...
            errors.pushVertexError(CLigoError::CornerIsMissing,
                                   edge[edge.size() - 1], 0);
          ...
          CLigoError errorBis;
          if (!_Edges[edgeIndex].build(vertexArray, edge, rotation, startX,
                                       startY, config, errorBis)) {
            // Flat zone
...
void CZoneTemplate::getMask(std::vector<bool> &mask, uint &width,
                            uint &height) {
  // Some constantes

...
// Copyright (C) 2010-2013  Matt RAYKOWSKI (sfb) <matt.raykowski@gmail.com>
...
// ---------------------------------------------------------------------------
CZoneBankElement::CZoneBankElement() { _SizeX = _SizeY = 0; }

// ---------------------------------------------------------------------------
void CZoneBankElement::addCategory(const std::string &CatType,
                                   const std::string &CatValue) {
  _CategoriesMap.insert(pair<string, string>(CatType, CatValue));
}

// ---------------------------------------------------------------------------
const string &CZoneBankElement::getName() { return getCategory("zone"); }

// ---------------------------------------------------------------------------
const string &CZoneBankElement::getSize() { return getCategory("size"); }
...
// ---------------------------------------------------------------------------
void CZoneBankElement::setMask(const std::vector<bool> &mask, uint8 sizeX,
                               uint8 sizeY) {
  _SizeX = sizeX;
  _SizeY = sizeY;
  _Mask = mask;
...
// ---------------------------------------------------------------------------
void CZoneBank::debugSaveInit(CZoneBankElement &zbeTmp,
...
bool CZoneBank::addElement(const std::string &elementName, std::string &error) {
  try {
    ...
    if (fileIn.open(elementName)) {
      ...
      zbeTmp.serial(input);
      _ElementsMap.insert(
          pair<string, CZoneBankElement>(zbeTmp.getName(), zbeTmp));
      return true;
    } else {
...
// ---------------------------------------------------------------------------
void CZoneBank::getCategoryValues(const std::string &CategoryType,
...
// ---------------------------------------------------------------------------
void CZoneBank::resetSelection() { _Selection.clear(); }

// ---------------------------------------------------------------------------
void CZoneBank::addOrSwitch(const std::string &CategoryType,
...
// ---------------------------------------------------------------------------
void CZoneBank::addAndSwitch(const std::string &CategoryType,
                             const std::string &CategoryValue) {
  uint32 i, j;
  // And the selection with some constraints
  // All elements of the selection must have a catType and catValue equal to
  // those given in parameters
...
// ---------------------------------------------------------------------------
void CZoneBank::getSelection(
    std::vector<CZoneBankElement *> &SelectedElements) {

...
bool CTransition::build(const CMaterial &mat0, const CMaterial &mat1,
    ...
    for (i = 0; i < TransitionZoneCount; i++) {
      ...
      if (arrayTemplate[i]) {
        ...
        for (uint j = 0; j < 4; j++) {
          ...
          if (edge < 0) {
            // The same edge ?
            if (!invertedEdges[-edge - 1].isTheSame(
                    arrayTemplate[i]->getEdges()[j], config, errors[i])) {
              ok = false;
            }
          } else {
            // The same edge ?
            if (!_EdgeZone[edge - 1].isTheSame(arrayTemplate[i]->getEdges()[j],
                                               config, errors[i])) {
              ok = false;
            }
          }
...
bool CTransition::check(const CZoneTemplate &zoneTemplate,
                        uint transitionNumber, const CLigoConfig &config,
  ...
  for (uint j = 0; j < 4; j++) {
    ...
    if (edge < 0) {
      ...
      // The same edge ?
      if (!invertedEdges.isTheSame(zoneTemplate.getEdges()[j], config,
                                   errors)) {
        ok = false;
      }
    } else {
      // The same edge ?
      if (!_EdgeZone[edge - 1].isTheSame(zoneTemplate.getEdges()[j], config,
                                         errors)) {
        ok = false;
      }
    }

...
void XMLError(xmlNodePtr xmlNode, const std::string &filename,
              const char *format, ...) {
  va_list args;
  va_start(args, format);
  char buffer[1024];
  ...
  Error(filename, "node (%s), line (%d) : %s", xmlNode->name, xmlNode->line,
        buffer);
}

// ***************************************************************************

xmlNodePtr GetFirstChildNode(xmlNodePtr xmlNode, const std::string &filename,
  ...
  XMLError(xmlNode, filename.c_str(), "Can't find XML node named (%s)",
           childName.c_str());
  return NULL;
}
...
bool GetPropertyString(string &result, const std::string &filename,
                       xmlNodePtr xmlNode, const std::string &propName) {
  // Call the CIXml version
  if (!CIXml::getPropertyString(result, xmlNode, propName)) {
    // Output a formated error
    XMLError(xmlNode, filename, "Can't find XML node property (%s)",
             propName.c_str());
    return false;
  }
  return true;
...
bool ReadInt(const std::string &propName, int &result,
             const std::string &filename, xmlNodePtr xmlNode) {
  string value;
...
void WriteInt(const std::string &propName, int value, xmlNodePtr xmlNode) {
  // Set properties
  xmlSetProp(xmlNode, (const xmlChar *)propName.c_str(),
             (const xmlChar *)(toString(value).c_str()));
}

// ***************************************************************************

bool ReadUInt(const std::string &propName, uint &result,
              const std::string &filename, xmlNodePtr xmlNode) {
  string value;
  if (GetPropertyString(value, filename, xmlNode, propName)) {
...
bool ReadFloat(const std::string &propName, float &result,
               const std::string &filename, xmlNodePtr xmlNode) {
  string value;
  if (GetPropertyString(value, filename, xmlNode, propName)) {
...
bool ReadVector(CPrimVector &point, const std::string &filename,
                xmlNodePtr xmlNode) {
  CPrimVector pos;
...
void WriteVector(const CPrimVector &point, xmlNodePtr xmlNode) {
  // Set properties
  xmlSetProp(xmlNode, (const xmlChar *)"X",
             (const xmlChar *)(toString(point.x).c_str()));
  xmlSetProp(xmlNode, (const xmlChar *)"Y",
             (const xmlChar *)(toString(point.y).c_str()));
  xmlSetProp(xmlNode, (const xmlChar *)"Z",
             (const xmlChar *)(toString(point.z).c_str()));
  if (point.Selected)
...
bool GetNodeString(string &result, const std::string &filename,
                   xmlNodePtr xmlNode, const std::string &nodeName) {
  // Look for the node
  xmlNodePtr node = CIXml::getFirstChildNode(xmlNode, nodeName);
  if (!node) {
    XMLError(xmlNode, filename, "Can't find XML node named (%s)",
             nodeName.c_str());
    return false;
  }
  ...
  if (!CIXml::getContentString(result, node)) {
    XMLError(xmlNode, filename, "Can't find any text in the node named (%s)",
             nodeName.c_str());
    return false;
  }
...
bool GetContentString(string &result, const std::string &filename,
                      xmlNodePtr xmlNode) {
  // Get the node string
...
CPropertyString::CPropertyString(const std::string &str) { String = str; }
...
CPropertyStringArray::CPropertyStringArray(
    const std::vector<std::string> &stringArray, bool _default) {
  StringArray = stringArray;
  Default = _default;
}
...
bool CPrimZone::contains(const NLMISC::CVector &v,
...
bool CPrimNode::read(xmlNodePtr xmlNode, const std::string &filename,
...
uint CPrimNode::getNumVector() const { return 0; }

// ***************************************************************************

const CPrimVector *CPrimNode::getPrimVector() const { return NULL; }

// ***************************************************************************

CPrimVector *CPrimNode::getPrimVector() { return NULL; }

// ***************************************************************************

NLLIGO::IPrimitive *CPrimNode::copy() const { return new CPrimNode(*this); }
...
uint CPrimPoint::getNumVector() const { return 1; }

// ***************************************************************************

const CPrimVector *CPrimPoint::getPrimVector() const { return &Point; }

// ***************************************************************************

CPrimVector *CPrimPoint::getPrimVector() { return &Point; }

// ***************************************************************************

NLLIGO::IPrimitive *CPrimPoint::copy() const { return new CPrimPoint(*this); }

// ***************************************************************************

bool CPrimPoint::read(xmlNodePtr xmlNode, const std::string &filename,
                      uint version, CLigoConfig &config) {
  // Read points
...
void CPrimPoint::write(xmlNodePtr xmlNode, const std::string &filename) const {
  ...
  if (Angle != 0) {
    xmlNodePtr ptNode =
        xmlNewChild(xmlNode, NULL, (const xmlChar *)"ANGLE", NULL);
...
uint CPrimPath::getNumVector() const { return (uint)VPoints.size(); }
...
NLLIGO::IPrimitive *CPrimPath::copy() const { return new CPrimPath(*this); }
...
bool CPrimPath::read(xmlNodePtr xmlNode, const std::string &filename,
                     uint version, CLigoConfig &config) {
  // Read points
  VPoints.clear();
...
uint CPrimZone::getNumVector() const { return (uint)VPoints.size(); }
...
NLLIGO::IPrimitive *CPrimZone::copy() const { return new CPrimZone(*this); }
...
bool CPrimZone::read(xmlNodePtr xmlNode, const std::string &filename,
                     uint version, CLigoConfig &config) {
  // Read points
  VPoints.clear();
  VPoints.reserve(CIXml::countChildren(xmlNode, "PT"));
...
bool CPrimZone::contains(const NLMISC::CVector &v,
                         const std::vector<CPrimVector> &points,
                         float &distance, NLMISC::CVector &nearPos,
                         bool isPath) {
  ...
  if ((v.x < vMin.x) || (v.y < vMin.y) || (v.x > vMax.x) || (v.y > vMax.y)) {
    ...
    for (uint i = 0; i < points.size(); ++i) {
      float dist =
          getSegmentDist(v, points[i], points[(i + 1) % points.size()], pos);
...
bool CPrimZone::contains(const NLMISC::CVector &v,
                         const std::vector<CVector> &points, float &distance,
...
float CPrimZone::getSegmentDist(const NLMISC::CVector v,
...
// ***************************************************************************
void CPrimZone::getAABox(NLMISC::CVector &cornerMin,
                         NLMISC::CVector &cornerMax) const {
  cornerMin.x = FLT_MAX;
  cornerMin.y = FLT_MAX;
  cornerMin.z = 0;
  cornerMax.x = -FLT_MAX;
...
IPrimitive::IPrimitive() { _Parent = NULL; }
...
void IPrimitive::serial(NLMISC::IStream &f) {
  ...
  if (f.isReading()) {
    ...
    f.serial(size);
    std::map<std::string, IProperty *>::iterator first(_Properties.begin()),
        last(_Properties.end());
  ...
  if (f.isReading()) {
    ...
    uint index = 0;
    for (std::vector<IPrimitive *>::iterator it = children.begin();
         it != children.end(); ++it, ++index) {
  ...
  if (f.isReading()) {
    // reloc child link
    vector<IPrimitive *>::iterator first(_Children.begin()),
        last(_Children.end());
    for (; first != last; ++first) {
      if (*first)
        (*first)->_Parent = this;
...
void IPrimitive::branchLink() {
  onBranchLink();
  std::vector<IPrimitive *>::iterator first(_Children.begin()),
      last(_Children.end());
  for (; first != last; ++first) {
    (*first)->branchLink();
  }
...
void IPrimitive::operator=(const IPrimitive &node) {
  ...
  // Copy properties
  std::map<std::string, IProperty *>::const_iterator ite =
      node._Properties.begin();
  while (ite != node._Properties.end()) {
    ...
    if (propString) {
      ...
      *newProp = *propString;
      _Properties.insert(
          std::map<std::string, IProperty *>::value_type(ite->first, newProp));
    } else {
      CPropertyStringArray *propStringArray =
          dynamic_cast<CPropertyStringArray *>(ite->second);
      if (propStringArray) {
        // New property
        CPropertyStringArray *newProp = new CPropertyStringArray();
        *newProp = *propStringArray;
        _Properties.insert(std::map<std::string, IProperty *>::value_type(
            ite->first, newProp));
      } else {
...
const IPrimitive *
IPrimitive::getPrimitive(const std::string &absoluteOrRelativePath) const {
...
bool IPrimitive::getProperty(uint index, std::string &property_name,
  ...
  nlwarning("NLLIGO::IPrimitive::getProperty : invalid index (index : %u, size "
            ": %u).",
            index, (uint)_Properties.size());
  return false;
}
...
bool IPrimitive::getPropertyByName(const std::string &property_name,
                                   const IProperty *&result) const {
  // Look for the property
  std::map<std::string, IProperty *>::const_iterator ite =
      _Properties.find(property_name);
...
bool IPrimitive::getPropertyByName(const std::string &property_name,
  ...
  if (getPropertyByName(property_name, prop)) {
    ...
    if (strProp) {
      ...
      nlwarning("NLLIGO::IPrimitive::getPropertyByName : property (%s) in not "
                "a string.",
                property_name.c_str());
    }
  }
...
bool IPrimitive::getPropertyByName(const std::string &property_name,
  ...
  if (getPropertyByName(property_name, prop)) {
    const CPropertyString *strProp =
        dynamic_cast<const CPropertyString *>(prop);
    if (strProp) {
      result = strProp->String;
      return true;
    } else {
...
bool IPrimitive::getPropertyByName(
    const std::string &property_name,
    const std::vector<std::string> *&result) const {
  // Get the property
  IProperty *prop;
  if (getPropertyByName(property_name, prop)) {
    const CPropertyStringArray *strProp =
        dynamic_cast<const CPropertyStringArray *>(prop);
    if (strProp) {
      result = &(strProp->StringArray);
      return true;
    } else {
...
bool IPrimitive::getPropertyByName(const std::string &property_name,
  ...
  IProperty *prop;
  if (getPropertyByName(property_name, prop)) {
    const CPropertyColor *colorProp =
        dynamic_cast<const CPropertyColor *>(prop);
    if (colorProp) {
...
bool IPrimitive::removeProperty(uint index) {
  ...
  nlwarning("NLLIGO::IPrimitive::removeProperty : invalid index (index : %u, "
            "size : %u).",
            index, (uint)_Properties.size());
  return false;
}
...
bool IPrimitive::removePropertyByName(const std::string &property_name) {
  // Look for the property
  std::map<std::string, IProperty *>::iterator ite =
      _Properties.find(property_name);
  if (ite != _Properties.end()) {
    _Properties.erase(ite);
    return true;
...
bool IPrimitive::getChild(const IPrimitive *&result, uint childId) const {
  if (childId < _Children.size()) {
    ...
    nlwarning(
        "NLLIGO::IPrimitive::getChild : invalid index (index : %u, size %u).",
        childId, (uint)_Children.size());
  }
  return false;
}
...
bool IPrimitive::removeChild(IPrimitive *child) {
  ...
  if (getChildId(childId, child)) {
    ...
    nlwarning("NLLIGO::IPrimitive::removeChild : invalid child, can't remove "
              "(child : %p)",
              child);
  }
  return false;
...
bool IPrimitive::removeChild(uint childId) {
  if (childId < _Children.size()) {
    ...
    nlwarning("NLLIGO::IPrimitive::removeChild : invalid index (index : %u, "
              "size %u).",
              childId, (uint)_Children.size());
  }
  return false;
...
bool IPrimitive::unlinkChild(IPrimitive *child) {
  ...
  if (getChildId(childId, child)) {
    ...
    nlwarning("NLLIGO::IPrimitive::unlinkChild : invalid child, can't unlink "
              "(child : %p)",
              child);
  }
  return false;
...
bool IPrimitive::addPropertyByName(const std::string &property_name,
                                   IProperty *result) {
  bool inserted = _Properties
                      .insert(std::map<std::string, IProperty *>::value_type(
...
bool IPrimitive::read(xmlNodePtr xmlNode, const std::string &filename,
                      uint version, CLigoConfig &config) {
      ...
      if (GetNodeString(name, filename, propNode, "NAME")) {
        ...
        if (GetPropertyString(type, filename, propNode, "TYPE")) {
          ...
          if (type == "string") {
            ...
            // Read it
            if (!GetNodeString(propertyString->String, filename, propNode,
                               "STRING")) {
              return false;
            }
          } else if (type == "string_array") {
            // Create a new property
            CPropertyStringArray *propertyStringArray =
                new CPropertyStringArray;
            property = propertyStringArray;

            // Read strings
            xmlNodePtr stringNode;
            propertyStringArray->StringArray.reserve(
                CIXml::countChildren(propNode, "STRING"));
          ...
          // Property found ?
          if (property == NULL) {
            XMLError(propNode, filename,
                     "IPrimitive::read : Unknown property type (%s)",
                     type.c_str());
            return false;
          }
          ...
          // Add it
          _Properties.insert(
              std::map<std::string, IProperty *>::value_type(name, property));
        } else {
          return false;
        }
      } else {
    ...
    do {
      ...
      if (GetPropertyString(type, filename, childNode, "TYPE")) {
        ...
        IPrimitive *primitive =
            static_cast<IPrimitive *>(CClassRegistry::create(type));

        // Primitive type not found ?
        if (primitive == NULL) {
          XMLError(childNode, filename,
                   "IPrimitive::read : Unknown primitive type (%s)",
                   type.c_str());
          return false;
        }
...
void IPrimitive::initDefaultValues(CLigoConfig &config) {
  ...
  if (primitiveClass) {
    ...
    uint i;
    for (i = 0; i < count; i++) {
      const CPrimitiveClass::CParameter &parameter =
          primitiveClass->Parameters[i];
    ...
    for (i = 0; i < count; i++) {
      ...
      if (pString) {
        ...
        if (pStringArray->StringArray.empty()) {
          ...
          parameter.getDefaultValue(pStringArray->StringArray, *this,
                                    *primitiveClass);
        }
      }
    }
...
void IPrimitive::write(xmlNodePtr xmlNode, const std::string &filename) const {
  ...
  //	if (!Expanded)
  //		xmlSetProp (xmlNode, (const xmlChar*)"EXPANDED", (const
  //xmlChar*)"false");

  // Set the type
  xmlSetProp(xmlNode, (const xmlChar *)"TYPE",
             (const xmlChar
                  *)(const_cast<IPrimitive *>(this)->getClassName().c_str()));

  // Save the unparsed property
  if (!_UnparsedProperties.empty()) {
    xmlNodePtr commentNode =
        xmlNewComment((const xmlChar *)(_UnparsedProperties.c_str()));
  ...
  while (ite != _Properties.end()) {
    ...
    if (!ite->second->Default) {
      // Create new nodes
      xmlNodePtr propNode =
          xmlNewChild(xmlNode, NULL, (const xmlChar *)"PROPERTY", NULL);
      xmlNodePtr nameNode =
          xmlNewChild(propNode, NULL, (const xmlChar *)"NAME", NULL);
      xmlNodePtr textNode = xmlNewText((const xmlChar *)(ite->first.c_str()));
      xmlAddChild(nameNode, textNode);

      // Type
      const CPropertyString *str =
          dynamic_cast<const CPropertyString *>(ite->second);
      if (str) {
        // Set the type
        xmlSetProp(propNode, (const xmlChar *)"TYPE",
                   (const xmlChar *)"string");

        // Create new nodes
        xmlNodePtr stringNode =
            xmlNewChild(propNode, NULL, (const xmlChar *)"STRING", NULL);
        xmlNodePtr textNode =
            xmlNewText((const xmlChar *)(str->String.c_str()));
        xmlAddChild(stringNode, textNode);
      } else {
        // Should be an array
        const CPropertyStringArray *array =
            dynamic_cast<const CPropertyStringArray *>(ite->second);
        if (array) {
          // Set the type
          xmlSetProp(propNode, (const xmlChar *)"TYPE",
                     (const xmlChar *)"string_array");
          ...
          for (uint i = 0; i < array->StringArray.size(); i++) {
            ...
            xmlNodePtr stringNode =
                xmlNewChild(propNode, NULL, (const xmlChar *)"STRING", NULL);
            xmlNodePtr textNode =
                xmlNewText((const xmlChar *)(array->StringArray[i].c_str()));
            xmlAddChild(stringNode, textNode);
          }
        } else {
          // Should be a color
          const CPropertyColor *color =
              safe_cast<const CPropertyColor *>(ite->second);
          ...
          // Create new nodes
          xmlNodePtr colorNode =
              xmlNewChild(propNode, NULL, (const xmlChar *)"COLOR", NULL);
          xmlSetProp(colorNode, (const xmlChar *)"R",
                     (const xmlChar *)toString(color->Color.R).c_str());
          xmlSetProp(colorNode, (const xmlChar *)"G",
                     (const xmlChar *)toString(color->Color.G).c_str());
          xmlSetProp(colorNode, (const xmlChar *)"B",
                     (const xmlChar *)toString(color->Color.B).c_str());
          xmlSetProp(colorNode, (const xmlChar *)"A",
                     (const xmlChar *)toString(color->Color.A).c_str());
        }
      }
    }
  ...
  for (uint i = 0; i < _Children.size(); i++) {
    // New node
    xmlNodePtr childNode =
        xmlNewChild(xmlNode, NULL, (const xmlChar *)"CHILD", NULL);
...
uint IPrimitive::getNumProperty() const { return (uint)_Properties.size(); }
...
void IPrimitive::setUnparsedProperties(
    const std::string &unparsedProperties) const {
...
CPrimAlias::CPrimAlias() : _Alias(0), _Container(NULL) {}

CPrimAlias::CPrimAlias(const CPrimAlias &other) : IPrimitive(other) {
...
void CPrimAlias::onBranchLink() {
  ...
  // generate a new alias, eventually keeping the current one if any and if
  // still available
...
uint32 CPrimAlias::getAlias() const { return _Alias; }
...
// Read the primitive
bool CPrimAlias::read(xmlNodePtr xmlNode, const std::string &filename,
                      uint version, CLigoConfig &config) {
  // Read alias
  ...
  if (ptNode) {
    ...
    if (ReadInt("VALUE", val, filename, ptNode)) {
      ...
      //			// set to null, it will be rewrited by
      //onBranchLink callback
      ////			_Container = NULL;
    } else {
      // error in format !
      nlwarning("CPrimAlias::read: Can't find xml property 'VALUE' in element "
                "<ALIAS>");
      return false;
    }
  } else {
...
void CPrimAlias::write(xmlNodePtr xmlNode, const std::string &filename) const {
  // Write alias
  xmlNodePtr ptNode =
      xmlNewChild(xmlNode, NULL, (const xmlChar *)"ALIAS", NULL);
...
CPrimitives::CPrimitives() : _LigoConfig(NULL) {
...
CPrimitives::~CPrimitives() { delete RootNode; }

// ***************************************************************************

uint32 CPrimitives::getAliasStaticPart() { return _AliasStaticPart; }
...
uint32 CPrimitives::genAlias(IPrimitive *prim, uint32 preferedAlias) {
  ...
  if (preferedAlias != 0) {
    ...
    nlassert(preferedAlias ==
             (preferedAlias & _LigoConfig->getDynamicAliasMask()));
    ...
    if (it == _AliasInUse.end()) {
      ...
      //			nldebug("Alias: added alias %u, %u alias used",
      //preferedAlias, _AliasInUse.size()+1);
      ...
      if (it->second == prim) {
        ...
        //				nldebug("Alias: using alias %u, %u alias
        //used", preferedAlias, _AliasInUse.size()+1);
        return preferedAlias;
      }
    }
  }
  ...
  // insert the alias
  //	nldebug("Alias: added alias %u, %u alias in use", ret,
  //_AliasInUse.size()+1);
...
//	nlassert(_LigoConfig);
//	// only dynamic part allowed here
//	nlassert(dynamicAlias == (dynamicAlias &
//_LigoConfig->getDynamicAliasMask())); 	std::set<uint32>::iterator
//it(_AliasInUse.find(dynamicAlias));
//	// warn if already found
//	if (it != _AliasInUse.end())
//	{
//		const string &fileName =
//_LigoConfig->getFileNameForStaticAlias(_AliasStaticPart); 		if
//(fileName.empty()) 			nlwarning("Dynamic Alias %u is already in use"); 		else
//			nlwarning("Dynamic Alias %u is already in use in file
//'%s'", 				dynamicAlias, 				fileName.c_str()); 		return;
//	}
//
//	// insert the alias
//	nldebug("Alias: added alias %u, %u alias in use", dynamicAlias,
//_AliasInUse.size()+1); 	_AliasInUse.insert(dynamicAlias);
// }
//
void CPrimitives::releaseAlias(IPrimitive *prim, uint32 alias) {
  ...
  if (it->second != prim) {
    nlwarning(
        "CPrimitives::releaseAlias: The alias %u is own by another primitive !",
        alias);
    return;
  }

  // remove this alias
  //	nldebug("Alias: remove alias %u, %u alias left", it->first,
  //_AliasInUse.size()-1);
...
void CPrimitives::forceAlias(CPrimAlias *prim, uint32 alias) {
  ...
  if (it != _AliasInUse.end() && it->second != prim) {
    ...
    CPrimAlias *pa =
        static_cast<CPrimAlias *>(const_cast<IPrimitive *>(it->second));
...
uint32 CPrimitives::getLastGeneratedAlias() { return _LastGeneratedAlias; }
...
IPrimitive *CPrimitives::getPrimitiveByAlias(uint32 primAlias) {
  ...
  std::map<uint32, IPrimitive *>::const_iterator it(
      _AliasInUse.find(primAlias));
...
void CPrimitives::buildPrimitiveWithAliasList(
    std::map<uint32, IPrimitive *> &result) {
  nlassert(_LigoConfig != NULL);

  std::map<uint32, IPrimitive *>::iterator first(_AliasInUse.begin()),
      last(_AliasInUse.end());
  for (; first != last; ++first) {
    result.insert(
        make_pair(_LigoConfig->buildAlias(_AliasStaticPart, first->first),
                  first->second->getParent()));
  }
}
...
bool CPrimitives::read(xmlNodePtr xmlNode, const std::string &filename,
                       CLigoConfig &config) {
  nlassert(xmlNode);
  ...
  if (_LigoConfig) {
    ...
    _AliasStaticPart =
        _LigoConfig->getFileStaticAliasMapping(CFile::getFilename(filename));
  }
  ...
  if (strcmp((const char *)xmlNode->name, "PRIMITIVES") == 0) {
    ...
    if (GetPropertyString(versionName, filename, xmlNode, "VERSION")) {
      ...
      if (version <= NLLIGO_PRIMITIVE_VERSION) {
        ...
        Error(filename, "CPrimitives::read : Unknown file version (%u)",
              version);
        return false;
      }
    } else {
...
void CPrimitives::write(xmlDocPtr doc, const std::string &filename) const {
  ...
  // Primitive node
  xmlNodePtr primNode =
      xmlNewDocNode(doc, NULL, (const xmlChar *)"PRIMITIVES", NULL);
...
void CPrimitives::write(xmlNodePtr root, const std::string &filename) const {
  ...
  xmlSetProp(root, (const xmlChar *)"VERSION",
             (const xmlChar *)toString(NLLIGO_PRIMITIVE_VERSION).c_str());

  // The primitive root node
  xmlNodePtr nameNode =
      xmlNewChild(root, NULL, (const xmlChar *)"ROOT_PRIMITIVE", NULL);
  xmlNodePtr subNode =
      xmlNewChild(nameNode, NULL, (const xmlChar *)"ALIAS", NULL);
...
void CPrimitives::convertAddPrimitive(IPrimitive *child, const IPrimitive *prim,
                                      bool hidden) {
  // The primitve
  ...
  if (oldPoint) {
    // Create a primitive
    CPrimPoint *point =
        static_cast<CPrimPoint *>(CClassRegistry::create("CPrimPoint"));
    primitive = point;
    ...
    *point = *oldPoint;
    ...
    if (oldPath) {
      // Create a primitive
      CPrimPath *path =
          static_cast<CPrimPath *>(CClassRegistry::create("CPrimPath"));
      primitive = path;
      ...
      *path = *oldPath;
      ...
      if (oldZone) {
        // Create a primitive
        CPrimZone *zone =
            static_cast<CPrimZone *>(CClassRegistry::create("CPrimZone"));
        primitive = zone;
...
*/			}
  ...
  if (j == numChildren) {
    // Create a node
    CPrimNode *primNode =
        static_cast<CPrimNode *>(CClassRegistry::create("CPrimNode"));
...
CPrimitiveContext::CPrimitiveContext()
    : CurrentLigoConfig(NULL), CurrentPrimitive(NULL) {}

...
bool CPrimitiveConfigurations::read(xmlNodePtr configurationNode,
  ...
  xmlNodePtr matchGroups =
      CIXml::getFirstChildNode(configurationNode, "MATCH_GROUP");
  if (matchGroups) {
    do {
      // Add a pair
      ...
      if (match) {
        do {
          ...
          if (config.getPropertyString(name, filename, match, "NAME")) {
            ...
            config.syntaxError(filename, match,
                               "Missing match name in configuration (%s)",
                               name.c_str());
            return false;
          }

...
// ***************************************************************************
CLigoConfig::CLigoConfig() : _DynamicAliasBitCount(32) {}

// ***************************************************************************

bool CLigoConfig::readConfigFile(const std::string &fileName,
  ...
  CConfigFile::CVar &primitiveClassFilename =
      cf.getVar("primitive_class_filename");
...
bool CLigoConfig::readPrimitiveClass(const std::string &_fileName,
                                     bool parsePrimitiveComboContent) {
  // File exist ?
  ...
  if (file.open(filename)) {
    try {
      ...
      if (strcmp((const char *)root->name, "NEL_LIGO_PRIMITIVE_CLASS") == 0) {
        // ALIAS_DYNAMIC_BITS
        xmlNodePtr aliasBits =
            CIXml::getFirstChildNode(root, "ALIAS_DYNAMIC_BITS");
        if (aliasBits) {
          string bits;
          if (getPropertyString(bits, filename.c_str(), aliasBits,
                                "BIT_COUNT")) {
            uint32 uBits;
        ...
        xmlNodePtr indexFileNameNode =
            CIXml::getFirstChildNode(root, "ALIAS_STATIC_FILE_ID");
        if (indexFileNameNode) {
          string indexFileName;
          if (getPropertyString(indexFileName, filename.c_str(),
                                indexFileNameNode, "FILE_NAME")) {
            ...
            nlwarning("Can't find XML element <FILE_NAME>, no file index "
                      "available for alias");
        } else {
          // by default, set the size of dynamic alias to 32 bits
          _DynamicAliasBitCount = 32;
        ...
        if (primitive) {
          do {
            ...
            std::string name;
            if (getPropertyString(name, filename.c_str(), primitive,
                                  "CLASS_NAME")) {
              // Add the primitive
              pair<std::map<std::string, CPrimitiveClass>::iterator, bool>
                  insertResult = _PrimitiveClasses.insert(
                      std::map<std::string, CPrimitiveClass>::value_type(
                          name, CPrimitiveClass()));
              if (insertResult.second) {
                if (!insertResult.first->second.read(
                        primitive, filename.c_str(), name.c_str(),
                        contextStrings, _ContextFilesLookup, *this,
                        parsePrimitiveComboContent))
                  return false;
              } else {
                syntaxError(filename.c_str(), root,
                            "Class (%s) already defined", name.c_str());
              }
            } else
              return false;
        ...
        _PrimitiveConfigurations.reserve(
            _PrimitiveConfigurations.size() +
            CIXml::countChildren(root, "CONFIGURATION"));
        xmlNodePtr configuration =
            CIXml::getFirstChildNode(root, "CONFIGURATION");
        if (configuration) {
          do {
            // Get the configuration name
            std::string name;
            if (getPropertyString(name, filename.c_str(), configuration,
                                  "NAME")) {
              // Add the configuration
              _PrimitiveConfigurations.resize(_PrimitiveConfigurations.size() +
                                              1);
              if (!_PrimitiveConfigurations.back().read(
                      configuration, filename.c_str(), name.c_str(), *this))
                return false;
            } else
              return false;

            configuration =
                CIXml::getNextChildNode(configuration, "CONFIGURATION");
          } while (configuration);
        }
...
bool CLigoConfig::reloadIndexFile(const std::string &indexFileName) {
  if (_IndexFileName.empty() && indexFileName.empty()) {
    nlwarning("CLigoConfig::reloadIndexFile: no file name specified and index "
              "file not previously loaded, can't load anything");
    return false;
  }

  if (!_IndexFileName.empty() && !indexFileName.empty() &&
      _IndexFileName != indexFileName) {
    nlwarning("CLigoConfig::reloadIndexFile: index file already loaded as "
              "'%s', can't load another file '%s'!",
              _IndexFileName.c_str(), indexFileName.c_str());
    return false;
  }
  ...
  if (pathName.empty()) {
    nlwarning("Can't find index file '%s' in search path, no file index "
              "available for alias",
              indexFileName.c_str());
    return false;
  }
    ...
    for (uint i = 0; i < files->size() / 2; ++i) {
      ...
      if (isFileStaticAliasMapped(fileName)) {
        ...
        if (getFileStaticAliasMapping(fileName) != index) {
          nlwarning(
              "CLigoConfig::reloadIndexFile: the mapping for the file '%s' as "
              "changed from %u to %u in the config file, the change is ignored",
              fileName.c_str(), index, getFileStaticAliasMapping(fileName));
        }
      } else {
...
NLMISC::CRGBA
CLigoConfig::getPrimitiveColor(const NLLIGO::IPrimitive &primitive) {
  // Get the class
  string className;
  if (primitive.getPropertyByName("class", className)) {
    // Get the class
    std::map<std::string, CPrimitiveClass>::iterator ite =
        _PrimitiveClasses.find(className);
...
CLigoConfig::getLinkedPrimitive(const NLLIGO::IPrimitive &primitive) const {
  // Get the parent
  ...
  if (parent) {
    ...
    if (parent->getChildId(childId, &primitive)) {
      ...
      if (primitive.getPropertyByName("class", className)) {
        // Get the class
        std::map<std::string, CPrimitiveClass>::const_iterator ite =
            _PrimitiveClasses.find(className);
        if (ite != _PrimitiveClasses.end()) {
...
const NLLIGO::IPrimitive *CLigoConfig::getPreviousLinkedPrimitive(
    const NLLIGO::IPrimitive &primitive) const {
  // Get the parent
  const IPrimitive *parent = primitive.getParent();
  if (parent) {
    uint childId;
...
bool CLigoConfig::canBeChild(const NLLIGO::IPrimitive &child,
  ...
  if (child.getPropertyByName("class", childClassName)) {
    ...
    return ((parent.getParent() == NULL) ||
            (!parent.getPropertyByName("class", parentClassName)));
  }
}
...
bool CLigoConfig::getPropertyString(std::string &result,
  ...
  if (!CIXml::getPropertyString(result, xmlNode, propName)) {
    ...
    syntaxError(filename, xmlNode, "Missing XML node property (%s)",
                propName.c_str());
    return false;
  }
  return true;
...
void CLigoConfig::syntaxError(const std::string &filename, xmlNodePtr xmlNode,
                              const char *format, ...) {
  va_list args;
  ...
  errorMessage("(%s), node (%s), line (%d) :\n%s", filename.c_str(),
               xmlNode->name, (sint)xmlNode->line, buffer);
}
...
CLigoConfig::getPrimitiveClass(const IPrimitive &primitive) const {
...
const CPrimitiveClass *
CLigoConfig::getPrimitiveClass(const std::string &className) const {
  std::map<std::string, CPrimitiveClass>::const_iterator ite =
      _PrimitiveClasses.find(className);
  if (ite != _PrimitiveClasses.end()) {
    return &(ite->second);
...
bool CLigoConfig::isStaticChild(const NLLIGO::IPrimitive &primitive) {
  ...
  if (parent) {
    ...
    string className;
    string name;
    if (parentClass && primitive.getPropertyByName("class", className) &&
...
uint32 CLigoConfig::getDynamicAliasMask() const {
  ...
  // The generated code lead to no shift at all and return 0 instead of
  // 0xffffffff
...
uint32 CLigoConfig::buildAlias(uint32 staticPart, uint32 dynamicPart,
                               bool warnIfOverload) const {
  if (warnIfOverload) {
    if (staticPart !=
        (staticPart & (getStaticAliasMask() >> getDynamicAliasSize()))) {
      nlwarning(
          "CLigoConfig::buildAlias: staticPart 0x%x is outside the mask 0x%x",
          staticPart, getStaticAliasMask() >> getDynamicAliasSize());
    }
    if (dynamicPart != (dynamicPart & getDynamicAliasMask())) {
      nlwarning(
          "CLigoConfig::buildAlias: dynamicPart 0x%x is outside the mask 0x%x",
          dynamicPart, getDynamicAliasMask());
    }
  }
...
void CLigoConfig::registerFileToStaticAliasTranslation(
    const std::string &fileName, uint32 staticPart) {
  // check the existing mapping
  std::map<std::string, uint32>::iterator first(
      _StaticAliasFileMapping.begin()),
      last(_StaticAliasFileMapping.end());
  for (; first != last; ++first) {
    if (first->second == staticPart) {
      nlassertex(false, ("While registering static alias %u to file '%s', the "
                         "alias is already assigned to file '%s'",
                         staticPart, fileName.c_str(), first->first.c_str()));
    }
  }
  if ((staticPart << getDynamicAliasSize()) !=
      ((staticPart << getDynamicAliasSize()) & getStaticAliasMask())) {
    nlwarning(
        "CLigoConfig::registerStaticAliasTranslation: staticPart 0x%x(%u) is "
        "outside the mask 0x%x, the staticPart will be clipped to 0x%x(%u)",
        staticPart, staticPart, getStaticAliasMask(),
        staticPart & getStaticAliasMask(), staticPart & getStaticAliasMask());
...
const std::string &
CLigoConfig::getFileNameForStaticAlias(uint32 staticAlias) const {
  std::map<std::string, uint32>::const_iterator first(
      _StaticAliasFileMapping.begin()),
      last(_StaticAliasFileMapping.end());
...
uint32
CLigoConfig::getFileStaticAliasMapping(const std::string &fileName) const {
  std::map<std::string, uint32>::const_iterator it(
      _StaticAliasFileMapping.find(fileName));
...
uint32 CLigoConfig::aliasFromString(const std::string &fullAlias) {
  ...
  return ((staticPart << getDynamicAliasSize()) & getStaticAliasMask()) |
         (dynPart & getDynamicAliasMask());
}

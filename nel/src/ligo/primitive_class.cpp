    ...
    bool parsePrimitiveComboContent)
{
	     ...
	     paramNode = CIXml::getNextChildNode(paramNode, "PARAMETER"))
	{
		     ...
		     comboValueNode = CIXml::getNextChildNode(comboValueNode, "COMBO_VALUES"))
		{
			...
			// Add a combo value
			pair<std::map<std::string, CParameter::CConstStringValue>::iterator, bool> insertResult = parameter.ComboValues.insert(std::map<std::string, CParameter::CConstStringValue>::value_type(type, CParameter::CConstStringValue()));
...
{
	return (Type == other.Type) && (Name == other.Name) && (Visible == other.Visible) && (Filename == other.Filename) && (ComboValues == other.ComboValues) && (DefaultValue == other.DefaultValue);
}

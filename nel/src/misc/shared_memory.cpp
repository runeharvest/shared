...
void *CSharedMemory::createSharedMemory(TSharedMemId sharedMemId, uint32 size) {
  ...
  // Create a file mapping backed by the virtual memory swap file (not a data
  // file)
  HANDLE hMapFile = CreateFileMappingA(INVALID_HANDLE_VALUE, NULL,
                                       PAGE_READWRITE, 0, size, sharedMemId);
  if ((hMapFile == NULL) || (GetLastError() == ERROR_ALREADY_EXISTS)) {
    nlwarning("SHDMEM: Cannot create file mapping for smid %s: error %u%s, "
              "mapFile %p",
              sharedMemId, GetLastError(),
              (GetLastError() == ERROR_ALREADY_EXISTS) ? " (already exists) "
                                                       : "",
              hMapFile);
    return NULL;
  }
  // else
  //	nldebug( "SHDMEM: Creating smid %s --> mapFile %p", sharedMemId,
  //hMapFile );
...
bool CSharedMemory::closeSharedMemory(void *accessAddress) {
  ...
  map<void *, HANDLE>::iterator im =
      AccessAddressesToHandles.find(accessAddress);
  if (im != AccessAddressesToHandles.end()) {
    // nldebug( "SHDMEM: CloseHandle mapFile %u", (*im).second );
    if (!CloseHandle((*im).second))
      nlwarning("SHDMEM: CloseHandle failed: error %u, mapFile %u",
                GetLastError(), (*im).second);
...
/*
 * Destroy a shared memory segment (to call only by the process that created the
 * segment) Note: does nothing under Windows, it is automatic. "Rescue feature":
 * set "force" to true if a segment was created and left out of control (meaning
 * a new createSharedMemory() with the same sharedMemId fails), but before, make
 * sure the segment really belongs to you!
 *
 ...
 * segment is still accessible by another process until it calls
 * closeSharedMemory().
 */

...
CTaskManager::CTaskManager()
    : _RunningTask(""), _TaskQueue(""), _DoneTaskQueue("") {
...
bool CTaskManager::deleteTask(IRunnable *r) {
  CSynchronized<list<CWaitingTask>>::CAccessor acces(&_TaskQueue);
  for (list<CWaitingTask>::iterator it = acces.value().begin();
       it != acces.value().end(); it++) {
    if (it->Task == r) {
...
void CTaskManager::changeTaskPriority(
    CSynchronized<std::list<CWaitingTask>>::CAccessor &acces) {

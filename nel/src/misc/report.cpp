...
// Set to 1 if you want command line report tool even when the debugger is
// present
...
inline const char *getReportPostURL() {
  ...
  buf[0] = '\0';
  int res =
      GetEnvironmentVariableA(NL_REPORT_POST_URL_ENVVAR, buf, sizeof(buf));
  if (res <= 0 || res > 511)
    return NULL;
  if (buf[0] == '\0')
    return NULL;
...
TReportResult report(const std::string &title, const std::string &subject,
  ...
  if (!body.empty()) {
    std::string reportFile =
        getLogDirectory() +
        NLMISC::toString("nel_report_%u.log", (uint)time(NULL));
    ...
    if (!f) {
      ...
      if (written != body.length()) {
        nlwarning("Unable to write %u bytes to %s, only %u written",
                  (uint)body.length(), reportPath.c_str(), (uint)written);
      }
  ...
  if (((INelContext::isContextInitialised() &&
    ...
    if (synchronous) {
      TReportResult result =
          (TReportResult)NLMISC::launchProgramAndWaitForResult(
              NL_CRASH_REPORT_TOOL, params);

      if (result != ReportAlwaysIgnore && result != ReportIgnore &&
          result != ReportAbort && result != ReportBreak) {
#if NL_DEBUG_REPORT
        if (INelContext::isContextInitialised())
      ...
      NLMISC::launchProgram(
          NL_CRASH_REPORT_TOOL, params,
          NL_DEBUG_REPORT ? INelContext::isContextInitialised()
                          : false); // Only log if required, avoid infinite loop
      return defaultResult;
    }
  } else {
#if NL_DEBUG_REPORT
    ...
    if (IsDebuggerPresent()) {
      ...
      for (;;) {
        printf("\n");
        printf(
            "Always Ignore (S), Ignore (I), Abort (A), Break (B)?\n"); // S for
                                                                       // Surpress
        printf("> ");
        int c = getchar();
        getchar();

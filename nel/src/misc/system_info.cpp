...
static string getCpuInfo(const string &colname) {
  ...
  nlwarning("SI: Can't find the colname '%s' in /proc/cpuinfo",
            colname.c_str());
  return "";
}
...
static uint64 getSystemMemory(const string &colname) {
  ...
  nlwarning("SI: Can't find the colname '%s' in /proc/meminfo",
            colname.c_str());
  return 0;
}
#endif // NL_OS_UNIX
...
string CSystemInfo::getOS() {
  ...
  PGNSI pGNSI = (PGNSI)GetProcAddress(GetModuleHandleA("kernel32.dll"),
                                      "GetNativeSystemInfo");
  ...
  if (VER_PLATFORM_WIN32_NT == osvi.dwPlatformId && osvi.dwMajorVersion > 4) {
    ...
    if (pGetProductInfo &&
        pGetProductInfo(osvi.dwMajorVersion, osvi.dwMinorVersion,
                        osvi.wServicePackMajor, osvi.wServicePackMinor,
                        &dwType)) {
      // Test for the specific product family.
      // see
      // https://msdn.microsoft.com/fr-fr/library/windows/desktop/ms724358(v=vs.85).aspx
      switch (dwType) {
  ...
  // Windows 3.1 with Win32s
    ...
    OSString = toString("Microsoft Windows %d.%d + Win32s", osvi.dwMajorVersion,
                        osvi.dwMinorVersion);
  }
  // Unknown Windows version
  else {
    OSString = toString("Microsoft Windows %d.%d", osvi.dwMajorVersion,
                        osvi.dwMinorVersion);
  }

  OSString += toString(" (%d.%d %d)", osvi.dwMajorVersion, osvi.dwMinorVersion,
...
string CSystemInfo::getProc() {
  ...
  if (result == ERROR_SUCCESS) {
    ...
    valueSize = 1024;
    result = ::RegQueryValueExA(hKey, "Identifier", NULL, NULL, (LPBYTE)value,
                                &valueSize);
    if (result == ERROR_SUCCESS)
      ProcString += value;
    else
    ...
    valueSize = 1024;
    result = ::RegQueryValueExA(hKey, "VendorIdentifier", NULL, NULL,
                                (LPBYTE)value, &valueSize);
    if (result == ERROR_SUCCESS)
      ProcString += value;
    else
    ...
    result =
        ::RegQueryValueExA(hKey, "~MHz", NULL, NULL, (LPBYTE)value, &valueSize);
    if (result == ERROR_SUCCESS) {
  ...
  uint numProc = 1;
  for (uint i = 1; i < 8; i++) {
    string tmp = string("Hardware\\Description\\System\\CentralProcessor\\") +
                 toString(i);

    // try to open the key
    result = ::RegOpenKeyExA(HKEY_LOCAL_MACHINE, tmp.c_str(), 0,
                             KEY_QUERY_VALUE, &hKey);
    // Make sure to close the reg key
    RegCloseKey(hKey);
  ...
  uint processors = 0;
  if (fromString(getCpuInfo("processor"), processors))
    ++processors;
...
uint64 CSystemInfo::getProcessorFrequency(bool quick) {
  ...
  if (!quick) {
    ...
    freq = (uint64)((double)bestNumCycles * 1 /
                    CTime::ticksToSecond(bestNumTicks));
  } else {
  ...
  nlinfo("SI: CSystemInfo: Processor frequency is %.0f MHz",
...
/*
 *	Note: Not used in NeL probably in Ryzom closed source. Not translated in
 *AT&T asm, I don't understand the aim of this method Returns true if the CPU
 *has HT,  even if it is disabled. Maybe shoud count how many (virtual) core
 *there is.
 */
...
#elif defined NL_OS_UNIX
  return getSystemMemory("MemFree:") + getSystemMemory("Buffers:") +
         getSystemMemory("Cached:");
#else
  return 0;
#endif
}
...
NLMISC_CATEGORISED_DYNVARIABLE(nel, string, AvailableHDSpace,
                               "Hard drive space left in bytes") {
  // ace: it's a little bit tricky, if you don't understand how it works, don't
  // touch!
...
NLMISC_CATEGORISED_DYNVARIABLE(
    nel, string, AvailablePhysicalMemory,
    "Physical memory available on this computer in bytes") {
  if (get)
    *pointer = bytesToHumanReadable(CSystemInfo::availablePhysicalMemory());
}

NLMISC_CATEGORISED_DYNVARIABLE(
    nel, string, TotalPhysicalMemory,
    "Total physical memory on this computer in bytes") {
  if (get)
    *pointer = bytesToHumanReadable(CSystemInfo::totalPhysicalMemory());
}

NLMISC_CATEGORISED_DYNVARIABLE(nel, string, ProcessUsedMemory,
                               "Memory used by this process in bytes") {
  if (get)
    *pointer = bytesToHumanReadable(CSystemInfo::getAllocatedSystemMemory());
}

NLMISC_CATEGORISED_DYNVARIABLE(nel, string, OS, "OS used") {
  if (get)
    *pointer = CSystemInfo::getOS();
}
...
bool CSystemInfo::getVideoInfo(std::string &deviceName, uint64 &driverVersion) {
   ...
   *
   * Follow the recommendations in the news group
   * comp.os.ms-windows.programmer.nt.kernel-mode : "Get Video Driver ... Need
   * Version"
   */
  ...
  if (hm) {
    ...
    (LPCTSTR lpDevice, DWORD iDevNum, PDISPLAY_DEVICE lpDisplayDevice,
     DWORD dwFlags) = NULL;
    *(FARPROC *)&EnumDisplayDevices = GetProcAddress(hm, "EnumDisplayDevicesA");
    if (EnumDisplayDevices) {
      ...
      bool found = false;
      while (EnumDisplayDevices(NULL, device, (DISPLAY_DEVICE *)&DisplayDevice,
                                0)) {
        // Main board ?
        if ((DisplayDevice.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) &&
          ...
          ver.dwOSVersionInfoSize = sizeof(OSVERSIONINFO);
          GetVersionEx(&ver);
          bool atleastNT4 = (ver.dwMajorVersion > 3) &&
          ...
          if (toLowerAscii(keyPath).find("\\registry\\machine") == 0) {
            keyPath = "HKEY_LOCAL_MACHINE" +
                      keyPath.substr(strlen("\\registry\\machine"));
          }
          ...
          // * Read the registry
          HKEY baseKey;
          if (RegOpenKeyExA(keyRoot, keyPath.c_str(), 0, KEY_READ, &baseKey) ==
              ERROR_SUCCESS) {
            DWORD valueType;
            char value[512];
            DWORD size = 512;
            string driverName;
            if (RegQueryValueExA(baseKey, keyName.c_str(), NULL, &valueType,
                                 (unsigned char *)value,
                                 &size) == ERROR_SUCCESS) {
              // Null ?
              if (value[0] != 0) {
                bool ok = !winXP;
                if (winXP) {
                  // In Windows'XP we got service name -> not real driver name,
                  // so
                  string xpKey =
                      string("System\\CurrentControlSet\\Services\\") + value;
                  if (RegOpenKeyExA(HKEY_LOCAL_MACHINE, xpKey.c_str(), 0,
                                    KEY_READ, &baseKey) == ERROR_SUCCESS) {
                    size = 512;
                    if (RegQueryValueExA(baseKey, "ImagePath", NULL, &valueType,
                                         (unsigned char *)value,
                                         &size) == ERROR_SUCCESS) {
                      if (value[0] != 0) {
                        static const std::string s_systemRoot =
                            "\\SystemRoot\\";

                        driverName = value;

                        if (driverName.substr(0, s_systemRoot.length()) ==
                            s_systemRoot) {
                        ...
                        nlwarning("CSystemInfo::getVideoInfo : empty value "
                                  "ImagePath in key %s",
                                  xpKey.c_str());
                    } else
                      nlwarning("CSystemInfo::getVideoInfo : can't query "
                                "ImagePath in key %s",
                                xpKey.c_str());
                  } else
                    nlwarning("CSystemInfo::getVideoInfo : can't open key %s",
                              xpKey.c_str());
                }
                ...
                if (hmVersion) {
                  ...
                  (LPSTR, DWORD, DWORD, LPVOID) = NULL;
                  DWORD(WINAPI * _GetFileVersionInfoSize)
                  (LPSTR, LPDWORD) = NULL;
                  BOOL(WINAPI * _VerQueryValue)
                  (const LPVOID, LPSTR, LPVOID *, PUINT) = NULL;
                  *(FARPROC *)&_GetFileVersionInfo =
                      GetProcAddress(hmVersion, "GetFileVersionInfoA");
                  *(FARPROC *)&_GetFileVersionInfoSize =
                      GetProcAddress(hmVersion, "GetFileVersionInfoSizeA");
                  *(FARPROC *)&_VerQueryValue =
                      GetProcAddress(hmVersion, "VerQueryValueA");
                  if (_VerQueryValue && _GetFileVersionInfoSize &&
                      _GetFileVersionInfo) {
                    ...
                    DWORD dwHandle;
                    DWORD size = _GetFileVersionInfoSize(
                        (char *)driverName.c_str(), &dwHandle);
                    if (size) {
                      vector<uint8> buffer;
                      buffer.resize(size);
                      if (_GetFileVersionInfo((char *)driverName.c_str(),
                                              dwHandle, size, &buffer[0])) {
                        VS_FIXEDFILEINFO *info;
                        UINT len;
                        char bslash[] = {'\\', 0x00};
                        if (_VerQueryValue(&buffer[0], bslash, (VOID **)&info,
                                           &len)) {
                          driverVersion =
                              (((uint64)info->dwFileVersionMS) << 32) |
                              info->dwFileVersionLS;
                          return true;
                        } else
                          nlwarning("CSystemInfo::getVideoInfo : VerQueryValue "
                                    "fails (%s)",
                                    driverName.c_str());
                      } else
                        nlwarning("CSystemInfo::getVideoInfo : "
                                  "GetFileVersionInfo fails (%s)",
                                  driverName.c_str());
                    } else
                      nlwarning("CSystemInfo::getVideoInfo : "
                                "GetFileVersionInfoSize == 0 (%s)",
                                driverName.c_str());
                  } else
                    nlwarning(
                        "CSystemInfo::getVideoInfo : No VerQuery, "
                        "GetFileVersionInfoSize, GetFileVersionInfo functions");
                } else
                  nlwarning("CSystemInfo::getVideoInfo : No version dll");
              } else
                nlwarning(
                    "CSystemInfo::getVideoInfo : empty value %s in key %s",
                    keyName.c_str(), keyPath.c_str());
            } else
              nlwarning(
                  "CSystemInfo::getVideoInfo : can't query value %s in key %s",
                  keyName.c_str(), keyPath.c_str());
          } else
            nlwarning("CSystemInfo::getVideoInfo : can't open key %s",
                      keyPath.c_str());
      ...
      if (!found)
        nlwarning(
            "CSystemInfo::getVideoInfo : No primary display device found");
    } else

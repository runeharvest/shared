...
bool CSystemInfo::hasHyperThreading()
{
	...
	if (hasCPUID())
	{
		...
		if ((((CPUInfo[0] & 0xf00) == 0xf00) || (CPUInfo[0] & 0xf00000)) && strcmp(vendor_id, "GenuineIntel") == 0)
...
bool CSystemInfo::getVideoInfo(std::string &deviceName, uint64 &driverVersion)
{
	...
	if (hm)
	{
		...
		(LPCTSTR lpDevice, DWORD iDevNum, PDISPLAY_DEVICE lpDisplayDevice, DWORD dwFlags) = NULL;
		*(FARPROC *)&EnumDisplayDevices = GetProcAddress(hm, "EnumDisplayDevicesA");
		if (EnumDisplayDevices)
		{
			...
			{
				// Main board ?
				if ((DisplayDevice.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) && (DisplayDevice.StateFlags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP) && ((DisplayDevice.StateFlags & DISPLAY_DEVICE_MIRRORING_DRIVER) == 0) && (DisplayDevice.DeviceKey[0] != 0))
				{
					found = true;

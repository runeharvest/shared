...
#define MAT_PROJ 16
// Validity bits. These means that the part may be yet identity, but is valid in
// the floats. NB: MAT_VALIDTRANS no more used for faster Pos access
...
bool CMatrix::hasProjectionPart() const { return (StateBit & MAT_PROJ) != 0; }
...
inline bool CMatrix::hasTrans() const { return (StateBit & MAT_TRANS) != 0; }
inline bool CMatrix::hasProj() const { return (StateBit & MAT_PROJ) != 0; }
...
void CMatrix::setMulMatrixNoProj(const CMatrix &m1, const CMatrix &m2) {
  /*
  For a fast MulMatrix, it appears to be better to not take State bits into
  account (no test/if() overhead) Just do heavy mul all the time (common case,
  and not so slow)
  */
  ...
  // Setup no proj at all, and force valid rot (still may be identity, but 0/1
  // are filled)
  StateBit =
      (m1.StateBit | m2.StateBit | MAT_VALIDROT) & ~(MAT_PROJ | MAT_VALIDPROJ);

  // Modify Scale. This test is important because Scale33 may be a #NAN if
  // SCALEANY => avoid very slow mul.
  if (hasScaleUniform())
...
void CMatrix::setMulMatrix(const CMatrix &m1, const CMatrix &m2) {
  ...
  bool MGeneralCase =
      !M1Identity && !M2Identity && !M1ScaleOnly && !M2ScaleOnly;

  // Manage the most common general case first (optim the if ): blending of two
  // rotations.
  if (MGeneralCase) {
    a11 = m1.a11 * m2.a11 + m1.a12 * m2.a21 + m1.a13 * m2.a31;
    a12 = m1.a11 * m2.a12 + m1.a12 * m2.a22 + m1.a13 * m2.a32;
...
// ======================================================================================================
void CMatrix::invert() { *this = inverted(); }
...
void CMatrix::transpose() {
  ...
  // NB: if no Trans or no Proj, do nothing, so don't need to modify VALIDTRANS
  // and VALIDPROJ too.
}
...
bool CMatrix::fastInvert33(CMatrix &ret) const {
  ...
  if (StateBit & MAT_SCALEUNI) {
    ...
    // Must divide the matrix by 1/Scale 2 times, to set unit, and to have a
    // Scale=1/Scale.
    S = 1.0 / Scale33;
    ret.Scale33 = (float)S;
    s = S * S;
...
bool CMatrix::slowInvert33(CMatrix &ret) const {
  ...
  s = 1.0 / s;
  ret.a11 = (float)(invi.x * s);
  ret.a12 = (float)(invi.y * s);
  ret.a13 = (float)(invi.z * s);
  ret.a21 = (float)(invj.x * s);
  ret.a22 = (float)(invj.y * s);
  ret.a23 = (float)(invj.z * s);
  ret.a31 = (float)(invk.x * s);
  ret.a32 = (float)(invk.y * s);
  ret.a33 = (float)(invk.z * s);
...
bool CMatrix::slowInvert44(CMatrix &ret) const {
  ...
  // Compute determinant.
  //=====================
  s = ret.mat(0, 0) * mat(0, 0) + ret.mat(0, 1) * mat(0, 1) +
      ret.mat(0, 2) * mat(0, 2) + ret.mat(0, 3) * mat(0, 3);
  if (s == 0)
    return false;
...
void CMatrix::getRot(CQuat &quat) const {
  ...
  if (tr > 0.0) {
    ...
    i = 0;
    if (pmat->a22 > pmat->a11)
      i = 1;
    if (pmat->a33 > pmat->mat(i, i))
      i = 2;
    j = nxt[i];
    k = nxt[j];

    s = (float)sqrt((pmat->mat(i, i) - (pmat->mat(j, j) + pmat->mat(k, k))) +
                    1.0);
...
// ======================================================================================================
void CMatrix::setScale(float scale) { setScaleUni(scale); }

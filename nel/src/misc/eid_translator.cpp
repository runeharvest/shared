...
void CEntityIdTranslator::getByUser(uint32 uid, vector<CEntityId> &res) {
  ...
  for (TEntityCont::iterator it = RegisteredEntities.begin();
       it != RegisteredEntities.end(); it++) {
...
void CEntityIdTranslator::getByUser(const string &userName,
                                    vector<CEntityId> &res, bool exact) {
...
const ucstring &CEntityIdTranslator::getByEntity(const CEntityId &eid) {
  ...
  // we have to remove the crea and dyna because it can changed dynamically and
  // will not be found in the storage array
  CEntityId reid(eid);
  reid.setCreatorId(0);
  reid.setDynamicId(0);
...
void CEntityIdTranslator::getByEntity(const ucstring &entityName,
                                      vector<CEntityId> &res, bool exact) {
...
bool CEntityIdTranslator::isValidEntityName(const ucstring &entityName,
  ...
  if (entityName.size() < 3) {
    log->displayNL("Bad entity name '%s' (less than 3 char)",
  ...
  if (entityName.size() > 15) {
    ...
    if (entityName.find(ucstring("(")) > 15 ||
        entityName[entityName.size() - 1] != ucchar(')')) {
      log->displayNL("EIT: Bad entity name '%s' (more than 15 char)",
                     entityName.toString().c_str());
      return false;
    }
  }
  ...
  for (uint i = 0; i < entityName.size(); i++) {
    ...
    // only accept name with alphabetic and numeric value [a-zA-Z] and
    // parenthesis
    if (!allowNumeric && !isalpha(entityName[i]) && entityName[i] != '(' &&
        entityName[i] != ')') {
      log->displayNL("Bad entity name '%s' (only char and num)",
                     entityName.toString().c_str());
      return false;
    }
  }
  ...
  for (uint i = 0; i < InvalidEntityNames.size(); i++) {
    if (testWildCard(en, InvalidEntityNames[i])) {
      log->displayNL(
          "Bad entity name '%s' (match the invalid entity name pattern '%s')",
          entityName.toString().c_str(), InvalidEntityNames[i].c_str());
      return false;
    }
  }
...
bool CEntityIdTranslator::checkEntityName(const ucstring &entityName) {
  ...
  if (!isValidEntityName(entityName, NLMISC::InfoLog))
    return false;
...
bool CEntityIdTranslator::entityNameExists(const ucstring &entityName) {
  ...
  /*	for (TEntityCont::iterator it = RegisteredEntities.begin(); it !=
     RegisteredEntities.end(); it++)
      {
...
void CEntityIdTranslator::registerEntity(const CEntityId &eid,
                                         const ucstring &entityName,
                                         sint8 entitySlot, uint32 uid,
  ...
  if (RegisteredEntities.find(reid) != RegisteredEntities.end()) {
    nlwarning("EIT: Can't register EId %s EntityName '%s' UId %d UserName '%s' "
              "because EId is already in the map",
              reid.toString().c_str(), entityName.toString().c_str(), uid,
              userName.c_str());
    return;
  }
  ...
  if (!checkEntityName(entityName)) {
    if (isValidEntityName(entityName))
      nlwarning("EIT: Can't register EId %s EntityName '%s' UId %d UserName "
                "'%s' because EntityName is already in the map",
                reid.toString().c_str(), entityName.toString().c_str(), uid,
                userName.c_str());
    else
      nlwarning("EIT: Can't register EId %s EntityName '%s' UId %d UserName "
                "'%s' because EntityName is invalid",
                reid.toString().c_str(), entityName.toString().c_str(), uid,
                userName.c_str());
    return;
  }

  nlinfo("EIT: Register EId %s EntityName '%s' UId %d UserName '%s'",
         reid.toString().c_str(), entityName.toString().c_str(), uid,
         userName.c_str());
  RegisteredEntities.insert(
      make_pair(reid, CEntityIdTranslator::CEntity(entityName, uid, userName,
                                                   entitySlot, shardId)));
  NameIndex.insert(make_pair(toLower(entityName), reid));
}

void CEntityIdTranslator::updateEntity(const CEntityId &eid,
                                       const ucstring &entityName,
                                       sint8 entitySlot, uint32 uid,
  ...
  if (it == RegisteredEntities.end()) {
    ...
    if (entity.EntityName != entityName) {
      if (!checkEntityName(entityName)) {
        nlwarning(
            "EIT: Can't update EId %s EntityName '%s' UId %d UserName '%s' "
            "with new name '%s' because EntityName is already in the map",
            reid.toString().c_str(), entity.EntityName.toString().c_str(), uid,
            userName.c_str(), entityName.toString().c_str());
        return;
      }
...
void CEntityIdTranslator::unregisterEntity(const CEntityId &eid) {
  ...
  if (it == RegisteredEntities.end()) {
    nlwarning("EIT: Can't unregister EId %s because EId is not in the map",
              reid.toString().c_str());
    return;
  }
  ...
  nldebug("EIT: Unregister EId %s EntityName '%s' UId %d UserName '%s'",
          reid.toString().c_str(), entity.EntityName.toString().c_str(),
          entity.UId, entity.UserName.c_str());
  NameIndex.erase(toLower(entity.EntityName));
...
void CEntityIdTranslator::checkEntity(const CEntityId &eid,
                                      const ucstring &entityName, uint32 uid,
                                      const string &userName) {
  ...
  map<CEntityId, CEntityIdTranslator::CEntity>::iterator it =
      RegisteredEntities.find(reid);

  nlinfo("EIT: Checking EId %s EntityName '%s' UId %d UserName '%s'",
         reid.toString().c_str(), entityName.toString().c_str(), uid,
         userName.c_str());

  if (it == RegisteredEntities.end()) {
    nlwarning("EIT: Check failed because EId is not in the CEntityIdTranslator "
              "map for EId %s EntityName '%s' UId %d UserName '%s'",
              reid.toString().c_str(), entityName.toString().c_str(), uid,
              userName.c_str());

    if (checkEntityName(entityName)) {
      nlwarning("EIT: Check failed because entity name already exist '%s' for "
                "EId %s EntityName '%s' UId %d UserName '%s'",
                getByEntity(entityName).toString().c_str(),
                reid.toString().c_str(), entityName.toString().c_str(), uid,
                userName.c_str());
    }
  } else {
    CEntity &entity = it->second;
    if (entity.EntityName != entityName) {
      nlwarning("EIT: Check failed because entity name not identical '%s' in "
                "the CEntityIdTranslator map for EId %s EntityName '%s' UId %d "
                "UserName '%s'",
                entity.EntityName.toString().c_str(), reid.toString().c_str(),
                entityName.toString().c_str(), uid, userName.c_str());
      if (!entityName.empty()) {
        entity.EntityName = entityName;
      }
    }
    if (entity.UId != uid) {
      nlwarning("EIT: Check failed because uid not identical (%d) in the "
                "CEntityIdTranslator map for EId %s EntityName '%s' UId %d "
                "UserName '%s'",
                entity.UId, reid.toString().c_str(),
                entityName.toString().c_str(), uid, userName.c_str());
      if (uid != 0) {
        entity.UId = uid;
      }
    ...
    if (entity.UserName != userName) {
      nlwarning("EIT: Check failed because user name not identical '%s' in the "
                "CEntityIdTranslator map for EId %s EntityName '%s' UId %d "
                "UserName '%s'",
                entity.UserName.c_str(), reid.toString().c_str(),
                entityName.toString().c_str(), uid, userName.c_str());
      if (!userName.empty()) {
        entity.UserName = userName;
      }
...
void cbInvalidEntityNamesFilename(
    const std::string &invalidEntityNamesFilename) {
  CEntityIdTranslator::getInstance()->InvalidEntityNames.clear();

  string fn = CPath::lookup(invalidEntityNamesFilename, false);

  if (fn.empty()) {
    nlwarning("EIT: Can't load filename '%s' for invalid entity names filename "
              "(not found)",
              invalidEntityNamesFilename.c_str());
    return;
  }
...
void CEntityIdTranslator::load(const string &fileName,
                               const string &invalidEntityNamesFilename) {
  ...
  if (!FileName.empty()) {
    nlwarning("EIT: Can't load file '%s' for EntityIdTranslator because we "
              "already load the file '%s'",
  ...
  if (CFile::fileExists(FileName)) {
    ...
    if (ifile.open(FileName)) {
      ...
      NameIndex.clear();
      TEntityCont::iterator first(RegisteredEntities.begin()),
          last(RegisteredEntities.end());
      for (; first != last; ++first) {
        NameIndex.insert(
            make_pair(toLower(first->second.EntityName), first->first));
      }
    } else {
      nlwarning("EIT: Can't load filename '%s' for EntityIdTranslator",
                FileName.c_str());
    }
  }
  ...
  NLMISC::CFile::addFileChangeCallback(invalidEntityNamesFilename,
...
void CEntityIdTranslator::save() {
  ...
  if (FileName.empty()) {
    nlwarning("EIT: Can't save empty filename for EntityIdTranslator (you "
              "forgot to load() it before?)");
    return;
  }
  ...
  if (ofile.open(FileName)) {
    ...
    nlwarning("EIT: Can't save filename '%s' for EntityIdTranslator",
              FileName.c_str());
  }
}
...
uint32 CEntityIdTranslator::getUId(const string &userName) {
  ...
  for (TEntityCont::iterator it = RegisteredEntities.begin(); it != itEnd;
       ++it) {
    CEntity &entity = it->second;
    if (entity.UserName == userName) {
      return entity.UId;
...
void CEntityIdTranslator::getEntityIdInfo(const CEntityId &eid,
  ...
  TEntityCont::iterator it = RegisteredEntities.find(reid);
  if (it == RegisteredEntities.end()) {
    nlwarning("EIT: %s is not registered in CEntityIdTranslator",
              reid.toString().c_str());
    entityName.clear();
    entitySlot = -1;
...
bool CEntityIdTranslator::setEntityNameStringId(const ucstring &entityName,
                                                uint32 stringId) {
  const TEntityCont::iterator itEnd = RegisteredEntities.end();
  for (TEntityCont::iterator it = RegisteredEntities.begin(); it != itEnd;
       ++it) {
...
bool CEntityIdTranslator::setEntityNameStringId(const CEntityId &eid,
                                                uint32 stringId) {
...
std::string
CEntityIdTranslator::getRegisterableString(const ucstring &entityName) {
...
NLMISC_CATEGORISED_COMMAND(nel, findEIdByUser,
  ...
  log.displayNL("User Name '%s' (uid=%d) has %d entities:", userName.c_str(),
                uid, res.size());
  for (uint i = 0; i < res.size(); i++) {
    log.displayNL(
        ">  %s '%s'", res[i].toString().c_str(),
        CEntityIdTranslator::getInstance()->getByEntity(res[i]).c_str());
  }

  return true;
}

NLMISC_CATEGORISED_COMMAND(nel, findEIdByEntity,
  ...
  CEntityIdTranslator::getInstance()->getEntityIdInfo(
      eid, entityName, entitySlot, uid, userName, online, &extinf);

  log.displayNL(
      "UId %d UserName '%s' EId %s EntityName '%s' EntitySlot %hd %s%s%s", uid,
      userName.c_str(), eid.toString().c_str(), entityName.toString().c_str(),
      (sint16)entitySlot, (extinf.c_str()), (extinf.empty() ? "" : " "),
      (online ? "Online" : "Offline"));

  return true;
}

NLMISC_CATEGORISED_COMMAND(nel, entityNameValid,
                           "Tell if an entity name is valid or not using "
                           "CEntityIdTranslator validation rulez",
                           "<entityname>") {
  if (args.size() != 1)
    return false;
  ...
  if (!CEntityIdTranslator::getInstance()->isValidEntityName(args[0], &log)) {
    ...
    if (CEntityIdTranslator::getInstance()->checkEntityName(args[0])) {
      log.displayNL("Entity name '%s' is already used by another player",
                    args[0].c_str());
    } else {
...
NLMISC_CATEGORISED_COMMAND(
    nel, playerInfo,
    "Get information about a player or all players in CEntityIdTranslator",
    "[<entityname>|<eid>|<username>|<uid>]") {
  if (args.size() == 0) {
    const map<CEntityId, CEntityIdTranslator::CEntity> &res =
        CEntityIdTranslator::getInstance()->getRegisteredEntities();
    log.displayNL("%d result(s) for 'all players information'", res.size());
    for (map<CEntityId, CEntityIdTranslator::CEntity>::const_iterator it =
             res.begin();
         it != res.end(); it++) {
      const CEntityIdTranslator::CEntity &entity = it->second;
      log.displayNL(
          "UId %d UserName '%s' EId %s EntityName '%s' EntitySlot %hd %s",
          entity.UId, entity.UserName.c_str(), it->first.toString().c_str(),
          entity.EntityName.toString().c_str(), (sint16)(entity.EntitySlot),
    ...
    if (eid != CEntityId::Unknown) {
      // we have to remove the crea and dyna because it can changed dynamically
      // and will not be found in the storage array
      eid.setCreatorId(0);
      eid.setDynamicId(0);
    ...
    for (uint i = 0; i < res.size(); i++) {
      ...
      std::string extinf;
      CEntityIdTranslator::getInstance()->getEntityIdInfo(
          res[i], entityName, entitySlot, uid2, userName, online, &extinf);

      log.displayNL(
          "UId %d UserName '%s' EId %s EntityName '%s' EntitySlot %hd %s%s%s",
          uid2, userName.c_str(), res[i].toString().c_str(),
          entityName.toString().c_str(), (sint16)entitySlot, (extinf.c_str()),
          (extinf.empty() ? "" : " "), (online ? "Online" : "Offline"));
    }

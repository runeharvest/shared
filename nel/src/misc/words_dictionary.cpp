...
/* Load the config file and the related words files. Return false in case of
 * failure. Config file variables:
...
bool CWordsDictionary::init(const string &configFileName) {
  ...
  if (cfFound) {
    ...
    if (v) {
      ...
      if (v->size() != additionalFiles.size()) {
        nlwarning("AdditionalFiles and AdditionalFileColumnTitles have "
                  "different size, ignoring second one");
  ...
  for (vector<string>::const_iterator ifl = fileList.begin();
       ifl != fileList.end(); ++ifl) {
    ...
    for (iAdditionalFile = 0; iAdditionalFile != additionalFiles.size();
         ++iAdditionalFile) {
      if ((p = filename.find(additionalFiles[iAdditionalFile])) !=
          string::npos) {
    ...
    if (isAdditionalFile || ((p = filename.find(pattern)) != string::npos)) {
      ...
      const string wordType =
          isAdditionalFile
              ? ""
              : truncFilename.substr(0, p - (origSize - truncFilename.size()));
      const string colTitle = isAdditionalFile
                                  ? additionalFileColumnTitles[iAdditionalFile]
                                  : DefaultColTitle;
      ...
      uint ck, cw = 0;
      if (worksheet.findId(ck) &&
          worksheet.findCol(ucstring(colTitle), cw)) // =>
      {
        for (std::vector<STRING_MANAGER::TWorksheet::TRow>::iterator ip =
                 worksheet.begin();
             ip != worksheet.end(); ++ip) {
        ...
        nlwarning("WD: %s ID or %s not found in %s", wordType.c_str(),
                  colTitle.c_str(), filename.c_str());
    }
  }

  if (_Keys.empty()) {
    if (wordsPath.empty())
      nlwarning("WD: WordsPath missing in config file %s",
                configFileName.c_str());
    nlwarning("WD: %s_words_%s.txt not found", filter.c_str(),
              languageCode.c_str());
    return false;
  } else
    return true;
 ...
 * - If inputStr is partially or completely found in the keys, all the matching
 * <key,words> are returned;
 * - If inputStr is partially or completely in the words, all the matching <key,
 * words> are returned. The following tags can modify the behaviour of the
 * search algorithm:
 ...
 */
void CWordsDictionary::lookup(const CSString &inputStr,
                              CVectorSString &resultVec) const {
  ...
  // Search
  for (CVectorSString::const_iterator ivs = _Keys.begin(); ivs != _Keys.end();
       ++ivs) {
    const CSString &key = *ivs;
    string::size_type p;
    if ((p = key.findNS(searchStr.c_str())) != string::npos) {
      if (((!findAtBeginning) || (p == 0)) &&
          ((!findAtEnd) || (p == key.size() - searchStr.size())))
        resultVec.push_back(makeResult(key, _Words[ivs - _Keys.begin()]));
    }
  }
  for (CVectorSString::const_iterator ivs = _Words.begin(); ivs != _Words.end();
       ++ivs) {
    const CSString &word = *ivs;
    string::size_type p;
    if ((p = word.findNS(searchStr.c_str())) != string::npos) {
      if (((!findAtBeginning) || (p == 0)) &&
          ((!findAtEnd) || (p == word.size() - searchStr.size())))
 ...
 */
void CWordsDictionary::exactLookupByKey(const CSString &key,
 ...
 */
inline CSString CWordsDictionary::makeResult(const CSString &key,
                                             const CSString &word) {
  return key + ": " + word.c_str();
}

/*
 * Return the key contained in the provided string returned by lookup() (without
 * extension)
 */

...
std::string winWideToCp(const wchar_t *str, size_t len, UINT cp)
{
	...
	if (!tmp)
		return std::string();
	int tmpLen = WideCharToMultiByte(cp, 0, str, (int)(len + 1), tmp,
	    (int)((len + 1) * 4), NULL, NULL);
	if (tmpLen <= 1)
	{
		_freea(tmp);
		return std::string();
...
std::string winCpToCp(const char *str, size_t len, UINT srcCp, UINT dstCp)
{
	...
	int tmpLen = MultiByteToWideChar(
	    srcCp, 0, str, (int)(len + 1), /* include null-termination */
	    tmp, (int)((len + 1) * 2));
	if (tmpLen <= 1)
	{
		_freea(tmp);
	...
	std::string res = winWideToCp(tmp, (size_t)tmpLen - 1,
	    dstCp); /* tmpLen includes null-term */
	_freea(tmp);
	return res;
}
...
std::wstring winCpToWide(const char *str, size_t len, UINT cp)
{
	...
	if (!tmp)
		return std::wstring();
	int tmpLen = MultiByteToWideChar(
	    cp, 0, str, (int)(len + 1), /* include null-termination */
	    tmp, (int)((len + 1) * 2));
	if (tmpLen <= 1)
	{
		_freea(tmp);

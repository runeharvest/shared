...
// Copyright (C) 2012-2013  Jan BOON (Kaetemi) <jan.boon@kaetemi.be>
//
...
static void *ProxyFunc(void *arg) {
...

  {
    ...
    if (parent->_ThreadHandle == thread_self)
      ...
      throw EThread(
          "Thread ended after being detached, this should not happen");
  }
...
CPThread::CPThread(IRunnable *runnable, uint32 stackSize)
    : Runnable(runnable), _State(ThreadStateNone), _StackSize(stackSize) {}
...
CPThread::~CPThread() {
  ...
  if (_State != ThreadStateNone)
    pthread_detach(
        _ThreadHandle); // free allocated resources only if it was created
}
...
void CPThread::start() {
  ...
  if (_State != ThreadStateNone) {
    if (_State == ThreadStateRunning) {
      // I don't know if this behaviour is allowed, but neither thread
      // implementations check the start function, and both simply let the
      // existing running thread for what it is... From now on, this is not
      // allowed.
      throw EThread("Starting a thread that is already started, existing "
                    "thread will continue running, this should not happen");
    }
    detach_old_thread = true;
  }

  if (pthread_create(&_ThreadHandle, _StackSize != 0 ? &tattr : NULL, ProxyFunc,
                     this) != 0) {
  ...
  if (detach_old_thread) {
    // Docs don't say anything about what happens when pthread_create is called
...
bool CPThread::isRunning() { return _State == ThreadStateRunning; }
...
bool CPThread::setCPUMask(uint64 cpuMask) {
  ...
  sint res = pthread_setaffinity_np(_ThreadHandle, sizeof(uint64),
...
uint64 CPThread::getCPUMask() {
  ...
  sint res = pthread_getaffinity_np(_ThreadHandle, sizeof(uint64),
                                    (cpu_set_t *)&cpuMask);
...
uint64 CPProcess::getCPUMask() {
  ...
  if (res) {
    nlwarning("sched_getaffinity() returned %d, errno = %d: %s", res, errno,
              strerror(errno));
    return 0;
  }
...
bool CPProcess::setCPUMask(uint64 cpuMask) {
  ...
  sint res =
      sched_setaffinity(getpid(), sizeof(uint64), (const cpu_set_t *)&cpuMask);

  if (res) {
    nlwarning("sched_setaffinity() returned %d, errno = %d: %s", res, errno,
              strerror(errno));
    return false;
  }

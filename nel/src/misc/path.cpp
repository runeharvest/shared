...
void CPath::releaseInstance() {
  if (_Instance) {
    NLMISC::INelContext::getInstance().releaseSingletonPointer("CPath",
                                                               _Instance);
    delete _Instance;
    _Instance = NULL;
  }
...
void CPath::getFileList(const std::string &extension,
                        std::vector<std::string> &filenames) {
  getInstance()->_FileContainer.getFileList(extension, filenames);
}

void CFileContainer::getFileList(const std::string &extension,
                                 std::vector<std::string> &filenames) {
  if (!_MemoryCompressed) {
    ...
    std::vector<CFileContainer::CMCFileEntry>::iterator first(_MCFiles.begin()),
        last(_MCFiles.end());
...
void CPath::getFileListByName(const std::string &extension,
                              const std::string &name,
                              std::vector<std::string> &filenames) {
  getInstance()->_FileContainer.getFileListByName(extension, name, filenames);
}

void CFileContainer::getFileListByName(const std::string &extension,
                                       const std::string &name,
                                       std::vector<std::string> &filenames) {
  if (!_MemoryCompressed) {
    TFiles::iterator first(_Files.begin()), last(_Files.end());
    ...
    if (!name.empty()) {
      for (; first != last; ++first) {
        string ext = SSMext.get(first->second.idExt);
        if (first->first.find(name) != string::npos &&
            (ext == extension || extension.empty())) {
          filenames.push_back(first->first);
        }
      }
    }
    ...
    if (!name.empty()) {
      for (; first != last; ++first) {
        string ext = SSMext.get(first->idExt);
        if (strstr(first->Name, name.c_str()) != NULL &&
            (ext == extension || extension.empty())) {
          filenames.push_back(first->Name);
        }
      }
...
void CPath::getFileListByPath(const std::string &extension,
                              const std::string &path,
                              std::vector<std::string> &filenames) {
  getInstance()->_FileContainer.getFileListByPath(extension, path, filenames);
}

void CFileContainer::getFileListByPath(const std::string &extension,
                                       const std::string &path,
                                       std::vector<std::string> &filenames) {
  if (!_MemoryCompressed) {
    TFiles::iterator first(_Files.begin()), last(_Files.end());
    ...
    if (!path.empty()) {
      for (; first != last; ++first) {
        ...
        if (strstr(p.c_str(), path.c_str()) != NULL &&
            (ext == extension || extension.empty())) {
          filenames.push_back(first->Name);
        }
      }
...
void CPath::clearMap() { getInstance()->_FileContainer.clearMap(); }
...
CFileContainer::MCfind(const std::string &filename) {
  nlassert(_MemoryCompressed);
  vector<CMCFileEntry>::iterator it;
  CMCFileEntry temp_cmc_file;
  temp_cmc_file.Name = (char *)filename.c_str();
  it = lower_bound(_MCFiles.begin(), _MCFiles.end(), temp_cmc_file,
                   CMCFileComp());
  if (it != _MCFiles.end()) {
...
void CPath::remapExtension(const string &ext1, const string &ext2,
                           bool substitute) {
  getInstance()->_FileContainer.remapExtension(ext1, ext2, substitute);
}

void CFileContainer::remapExtension(const string &ext1, const string &ext2,
                                    bool substitute) {
  nlassert(!_MemoryCompressed);
  ...
  if (ext1lwr.empty() || ext2lwr.empty()) {
    nlwarning(
        "PATH: CPath::remapExtension(%s, %s, %d): can't remap empty extension",
        ext1lwr.c_str(), ext2lwr.c_str(), substitute);
  }

  if (ext1lwr == "bnp" || ext2lwr == "bnp" || ext1lwr == "snp" ||
      ext2lwr == "snp") {
    nlwarning(
        "PATH: CPath::remapExtension(%s, %s, %d): you can't remap a big file",
        ext1lwr.c_str(), ext2lwr.c_str(), substitute);
  }
  ...
  if (!substitute) {
    ...
    NL_DISPLAY_PATH(
        "PATH: CPath::remapExtension(%s, %s, %d): extension removed",
        ext1lwr.c_str(), ext2lwr.c_str(), substitute);
  } else {
    sint n = findExtension(ext1lwr, ext2lwr);
    if (n != -1) {
      nlwarning(
          "PATH: CPath::remapExtension(%s, %s, %d): remapping already set",
          ext1lwr.c_str(), ext2lwr.c_str(), substitute);
      return;
    }
    ...
    while (it != _Files.end()) {
      ...
      if (!(*it).second.Remapped && ext == ext1lwr) {
        ...
        if (pos != string::npos) {
          ...
          // TODO perhaps a problem because I insert in the current map that I
          // process
    ...
    NL_DISPLAY_PATH("PATH: CPath::remapExtension(%s, %s, %d): extension added",
                    ext1lwr.c_str(), ext2lwr.c_str(), substitute);
  }
}
...
void CFileContainer::remapFile(const std::string &file1,
...
string CPath::lookup(const string &filename, bool throwException,
                     bool displayWarning, bool lookupInLocalDirectory) {
  return getInstance()->_FileContainer.lookup(
      filename, throwException, displayWarning, lookupInLocalDirectory);
}

string CFileContainer::lookup(const string &filename, bool throwException,
                              bool displayWarning,
   ...
   * ***********************************************/
  /*
      NB: CPath access static instance getInstance() of course, so user must
     ensure that no mutator is called while async loading
  */

  // If the file already contains a @, it means that a lookup already proceed
  // and returning a big file, do nothing
  if (filename.find("@") != string::npos) {
    NL_DISPLAY_PATH("PATH: CPath::lookup(%s):	already found",
                    filename.c_str());
    return filename;
  }
  ...
  if (_MemoryCompressed) {
    ...
    if (pMCFE != NULL) {
      ...
      if (pMCFE->Remapped)
        fname = CFile::getFilenameWithoutExtension(pMCFE->Name) + "." +
                SSMext.get(pMCFE->idExt);
      else
        fname = pMCFE->Name;

      NL_DISPLAY_PATH(
          "PATH: CPath::lookup(%s): found in the map directory: '%s'",
  ...
  } else // NOT memory compressed
  {
    ...
    if (it != _Files.end()) {
      ...
      if (it->second.Remapped)
        fname = CFile::getFilenameWithoutExtension((*it).second.Name) + "." +
                SSMext.get((*it).second.idExt);
      else
  ...
  for (uint i = 0; i < _AlternativePaths.size(); i++) {
    ...
    if (CFile::fileExists(s)) {
      NL_DISPLAY_PATH(
          "PATH: CPath::lookup(%s): found in the alternative directory: '%s'",
          str.c_str(), s.c_str());
      return s;
    }
    ...
    for (uint j = 0; j < _Extensions.size(); j++) {
      if (toLowerAscii(CFile::getExtension(str)) == _Extensions[j].second) {
        string rs = _AlternativePaths[i] +
                    CFile::getFilenameWithoutExtension(filename) + "." +
                    _Extensions[j].first;
        if (CFile::fileExists(rs)) {
          NL_DISPLAY_PATH("PATH: CPath::lookup(%s): found in the alternative "
                          "directory: '%s'",
                          str.c_str(), rs.c_str());
          return rs;
        }
  ...
  if (lookupInLocalDirectory && CFile::fileExists(filename)) {
    NL_DISPLAY_PATH(
        "PATH: CPath::lookup(%s): found in the current directory: '%s'",
  ...
  if (displayWarning) {
    if (filename.empty())
      ...
      nlwarning("PATH: File (%s) not found (%s)", str.c_str(),
                filename.c_str());
  }
...
bool CFileContainer::setCurrentPath(const std::string &path) {
  int res;
  // nldebug("Change current path to '%s' (current path is '%s')", path.c_str(),
...
#endif
  if (res != 0)
    nlwarning("Cannot change current path to '%s' (current path is '%s') res: "
              "%d errno: %d (%s)",
              path.c_str(), getCurrentPath().c_str(), res, errno,
              strerror(errno));
  return res == 0;
}
...
std::string CFileContainer::getFullPath(const std::string &path,
                                        bool addFinalSlash) {
...
dirent *readdir(DIR *dir) {
  // FIX : call to SetCurrentDirectory() and SetCurrentDirectory() removed to
  // improve speed
  nlassert(!sDir.empty());

  // first visit in this directory : FindFirstFile()
  if (hFind == NULL) {
    hFind = FindFirstFileW(nlUtf8ToWide(CPath::standardizePath(sDir) + "*"),
                           &findData);
  }
...
bool isdirectory(dirent *de) {
  ...
  return ((de->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) != 0) &&
         ((de->dwFileAttributes & FILE_ATTRIBUTE_SYSTEM) == 0);
#else
...
void CPath::getPathContent(const string &path, bool recurse, bool wantDir,
                           bool wantFile, vector<string> &result,
                           class IProgressCallback *progressCallBack,
                           bool showEverything) {
  getInstance()->_FileContainer.getPathContent(path, recurse, wantDir, wantFile,
...
void CFileContainer::getPathContent(const string &path, bool recurse,
                                    bool wantDir, bool wantFile,
  ...
  if (dir == NULL) {
    NL_DISPLAY_PATH("PATH: CPath::getPathContent(%s, %d, %d, %d): could not "
                    "open the directory",
                    path.c_str(), recurse, wantDir, wantFile);
    return;
  }
  ...
  for (;;) {
    ...
    if (de == NULL) {
      NL_DISPLAY_PATH(
          "PATH: CPath::getPathContent(%s, %d, %d, %d): end of directory",
          path.c_str(), recurse, wantDir, wantFile);
      break;
    }
    ...
    if (isdirectory(de)) {
      ...
      if ((!showEverything) &&
          (fn == "CVS" || fn == ".svn" || fn == ".hg" || fn == ".git")) {
        NL_DISPLAY_PATH(
            "PATH: CPath::getPathContent(%s, %d, %d, %d): skip '%s' directory",
            path.c_str(), recurse, wantDir, wantFile, fn.c_str());
        continue;
      }

      string stdName = standardizePath(standardizePath(path) + fn);
      if (recurse) {
        NL_DISPLAY_PATH("PATH: CPath::getPathContent(%s, %d, %d, %d): need to "
                        "recurse into '%s'",
                        path.c_str(), recurse, wantDir, wantFile,
                        stdName.c_str());
      ...
      if (wantDir) {
        NL_DISPLAY_PATH(
            "PATH: CPath::getPathContent(%s, %d, %d, %d): adding path '%s'",
            path.c_str(), recurse, wantDir, wantFile, stdName.c_str());
    ...
    if (wantFile && isfile(de)) {
      if ((!showEverything) &&
          (fn.size() >= 4 && fn.substr(fn.size() - 4) == ".log")) {
        NL_DISPLAY_PATH("PATH: CPath::getPathContent(%s, %d, %d, %d): skip "
                        "*.log files (%s)",
                        path.c_str(), recurse, wantDir, wantFile, fn.c_str());
        continue;
      }
      ...
      NL_DISPLAY_PATH(
          "PATH: CPath::getPathContent(%s, %d, %d, %d): adding file '%s'",
          path.c_str(), recurse, wantDir, wantFile, stdName.c_str());
      result.push_back(stdName);
    }
  }
  ...
  for (uint i = 0; i < recursPath.size(); i++) {
    ...
    if (progressCallBack) {
      ...
      progressCallBack->pushCropedValues((float)i / (float)recursPath.size(),
                                         (float)(i + 1) /
                                             (float)recursPath.size());
    }

    getPathContent(recursPath[i], recurse, wantDir, wantFile, result,
                   progressCallBack, showEverything);
...
void CPath::addSearchPath(const string &path, bool recurse, bool alternative,
                          class IProgressCallback *progressCallBack) {
  getInstance()->_FileContainer.addSearchPath(path, recurse, alternative,
                                              progressCallBack);
}

void CFileContainer::addSearchPath(const string &path, bool recurse,
                                   bool alternative,
                                   class IProgressCallback *progressCallBack) {
  ...
  if (path.empty()) {
    nlwarning("PATH: CPath::addSearchPath(%s, %s, %s): can't add empty "
              "directory, skip it",
              path.c_str(), recurse ? "recursive" : "not recursive",
  ...
  if (!CFile::isDirectory(path)) {
    nlinfo("PATH: CPath::addSearchPath(%s, %s, %s): '%s' is not a directory, "
           "I'll call addSearchFile()",
           path.c_str(), recurse ? "recursive" : "not recursive",
           alternative ? "alternative" : "not alternative", path.c_str());
  ...
  if (!CFile::isExists(newPath)) {
    nlwarning(
        "PATH: CPath::addSearchPath(%s, %s, %s): '%s' is not found, skip it",
        path.c_str(), recurse ? "recursive" : "not recursive",
        alternative ? "alternative" : "not alternative", newPath.c_str());
    return;
  }

  nlinfo("PATH: CPath::addSearchPath(%s, %d, %d): adding the path '%s'",
         path.c_str(), recurse, alternative, newPath.c_str());

  NL_DISPLAY_PATH("PATH: CPath::addSearchPath(%s, %d, %d): try to add '%s'",
                  path.c_str(), recurse, alternative, newPath.c_str());
  ...
  if (alternative) {
    ...
    for (uint p = 0; p < pathsToProcess.size(); p++) {
      ...
      if (i == _AlternativePaths.size()) {
        ...
        NL_DISPLAY_PATH(
            "PATH: CPath::addSearchPath(%s, %s, %s): path '%s' added",
            newPath.c_str(), recurse ? "recursive" : "not recursive",
            alternative ? "alternative" : "not alternative",
            pathsToProcess[p].c_str());
      } else {
        nlwarning(
            "PATH: CPath::addSearchPath(%s, %s, %s): path '%s' already added",
            newPath.c_str(), recurse ? "recursive" : "not recursive",
            alternative ? "alternative" : "not alternative",
            pathsToProcess[p].c_str());
      }
    ...
    for (uint f = 0; f < filesToProcess.size(); f++) {
      ...
      //			insertFileInMap (filename, filepath, false,
...
void CPath::addSearchFile(const string &file, bool remap,
                          const string &virtual_ext,
                          NLMISC::IProgressCallback *progressCallBack) {
  getInstance()->_FileContainer.addSearchFile(file, remap, virtual_ext,
                                              progressCallBack);
}

void CFileContainer::addSearchFile(
    const string &file, bool remap, const string &virtual_ext,
    NLMISC::IProgressCallback *progressCallBack) {
  nlassert(!_MemoryCompressed);
  ...
  // check empty file
  if (newFile.empty()) {
    nlwarning("PATH: CPath::addSearchFile(%s, %d, '%s'): can't add empty file, "
              "skip it",
              file.c_str(), remap, virtual_ext.c_str());
    return;
  }

  // check if the file exists
  if (!CFile::isExists(newFile)) {
    nlwarning("PATH: CPath::addSearchFile(%s, %d, '%s'): '%s' is not found, "
              "skip it (current dir is '%s'",
              file.c_str(), remap, virtual_ext.c_str(), newFile.c_str(),
  ...
  if (CFile::isDirectory(newFile)) {
    nlwarning(
        "PATH: CPath::addSearchFile(%s, %d, '%s'): '%s' is not a file, skip it",
        file.c_str(), remap, virtual_ext.c_str(), newFile.c_str());
    return;
  }
  ...
  if (fileExtension == "bnp") {
    NL_DISPLAY_PATH(
        "PATH: CPath::addSearchFile(%s, %d, '%s'): '%s' is a big file, add it",
        file.c_str(), remap, virtual_ext.c_str(), newFile.c_str());
  ...
  if (fileExtension == "snp") {
    NL_DISPLAY_PATH("PATH: CPath::addSearchStreamedPackage(%s, %d, '%s'): '%s' "
                    "is a streamed package, add it",
                    file.c_str(), remap, virtual_ext.c_str(), newFile.c_str());
  ...
  if (fileExtension == "xml_pack") {
    NL_DISPLAY_PATH("PATH: CPath::addSearchFile(%s, %d, '%s'): '%s' is an xml "
                    "pack file, add it",
                    file.c_str(), remap, virtual_ext.c_str(), newFile.c_str());
...
void CPath::addSearchListFile(const string &filename, bool recurse,
                              bool alternative) {
  getInstance()->_FileContainer.addSearchListFile(filename, recurse,
                                                  alternative);
}

void CFileContainer::addSearchListFile(const string &filename, bool recurse,
                                       bool alternative) {
  // check empty file
  if (filename.empty()) {
    nlwarning("PATH: CPath::addSearchListFile(%s, %d, %d): can't add empty "
              "file, skip it",
              filename.c_str(), recurse, alternative);
    return;
  }

  // check if the file exists
  if (!CFile::isExists(filename)) {
    nlwarning("PATH: CPath::addSearchListFile(%s, %d, %d): '%s' is not found, "
              "skip it",
              filename.c_str(), recurse, alternative, filename.c_str());
    return;
  }

  // check if it s a file
  if (CFile::isDirectory(filename)) {
    nlwarning("PATH: CPath::addSearchListFile(%s, %d, %d): '%s' is not a file, "
              "skip it",
              filename.c_str(), recurse, alternative, filename.c_str());
    return;
  }
...
void CPath::addSearchBigFile(const string &sBigFilename, bool recurse,
                             bool alternative,
                             NLMISC::IProgressCallback *progressCallBack) {
  getInstance()->_FileContainer.addSearchBigFile(sBigFilename, recurse,
                                                 alternative, progressCallBack);
}

void CFileContainer::addSearchBigFile(
    const string &sBigFilename, bool recurse, bool alternative,
    NLMISC::IProgressCallback *progressCallBack) {
  // Check if filename is not empty
  if (sBigFilename.empty()) {
    nlwarning("PATH: CPath::addSearchBigFile(%s, %d, %d): can't add empty "
              "file, skip it",
              sBigFilename.c_str(), recurse, alternative);
    return;
  }
  // Check if the file exists
  if (!CFile::isExists(sBigFilename)) {
    nlwarning(
        "PATH: CPath::addSearchBigFile(%s, %d, %d): '%s' is not found, skip it",
        sBigFilename.c_str(), recurse, alternative, sBigFilename.c_str());
    return;
  }
  // Check if it s a file
  if (CFile::isDirectory(sBigFilename)) {
    nlwarning("PATH: CPath::addSearchBigFile(%s, %d, %d): '%s' is not a file, "
              "skip it",
              sBigFilename.c_str(), recurse, alternative, sBigFilename.c_str());
    return;
  }
  ...
  if (Handle == NULL) {
    nlwarning(
        "PATH: CPath::addSearchBigFile(%s, %d, %d): can't open file, skip it",
        sBigFilename.c_str(), recurse, alternative);
    return;
  }

  // add the link with the CBigFile singleton
  if (CBigFile::getInstance().add(sBigFilename,
                                  BF_ALWAYS_OPENED | BF_CACHE_FILE_ON_OPEN)) {
    // also add the bigfile name in the map to retrieve the full path of a .bnp
    // when we want modification date of the bnp for example
    insertFileInMap(CFile::getFilename(sBigFilename), sBigFilename, false,
                    CFile::getExtension(sBigFilename));
    ...
    for (uint32 i = 0; i < nNbFile; ++i) {
      ...
      if (progressCallBack) {
        ...
        progressCallBack->pushCropedValues((float)i / (float)nNbFile,
                                           (float)(i + 1) / (float)nNbFile);
      }
    ...
    nlwarning(
        "PATH: CPath::addSearchBigFile(%s, %d, %d): can't add the big file",
        sBigFilename.c_str(), recurse, alternative);
  }
...
void CPath::addSearchStreamedPackage(
    const string &filename, bool recurse, bool alternative,
    NLMISC::IProgressCallback *progressCallBack) {
  getInstance()->_FileContainer.addSearchBigFile(filename, recurse, alternative,
                                                 progressCallBack);
}

void CFileContainer::addSearchStreamedPackage(
    const string &filename, bool recurse, bool alternative,
    NLMISC::IProgressCallback *progressCallBack) {
  // Check if filename is not empty
  if (filename.empty()) {
    nlwarning("PATH: CPath::addSearchStreamedPackage(%s, %d, %d): can't add "
              "empty file, skip it",
              filename.c_str(), recurse, alternative);
    return;
  }
  // Check if the file exists
  if (!CFile::isExists(filename)) {
    nlwarning("PATH: CPath::addSearchStreamedPackage(%s, %d, %d): '%s' is not "
              "found, skip it",
              filename.c_str(), recurse, alternative, filename.c_str());
    return;
  }
  ...
  if (CStreamedPackageManager::getInstance().loadPackage(filename)) {
    ...
    for (std::vector<std::string>::iterator it(fileNames.begin()),
      ...
      for (uint j = 0; j < _Extensions.size(); j++) {
        if (_Extensions[j].first == ext) {
          // Need to remap
          insertFileInMap(CFile::getFilenameWithoutExtension(*it) + "." +
                              _Extensions[j].second,
...
// WARNING : recurse is not used
void CPath::addSearchXmlpackFile(const string &sXmlpackFilename, bool recurse,
                                 bool alternative,
                                 NLMISC::IProgressCallback *progressCallBack) {
  getInstance()->_FileContainer.addSearchXmlpackFile(
      sXmlpackFilename, recurse, alternative, progressCallBack);
}

void CFileContainer::addSearchXmlpackFile(
    const string &sXmlpackFilename, bool recurse, bool alternative,
    NLMISC::IProgressCallback *progressCallBack) {
  // Check if filename is not empty
  if (sXmlpackFilename.empty()) {
    nlwarning("PATH: CPath::addSearchXmlpackFile(%s, %d, %d): can't add empty "
              "file, skip it",
              sXmlpackFilename.c_str(), recurse, alternative);
    return;
  }
  // Check if the file exists
  if (!CFile::isExists(sXmlpackFilename)) {
    nlwarning("PATH: CPath::addSearchXmlpackFile(%s, %d, %d): '%s' is not "
              "found, skip it",
              sXmlpackFilename.c_str(), recurse, alternative,
              sXmlpackFilename.c_str());
    return;
  }
  // Check if it s a file
  if (CFile::isDirectory(sXmlpackFilename)) {
    nlwarning("PATH: CPath::addSearchXmlpackFile(%s, %d, %d): '%s' is not a "
              "file, skip it",
              sXmlpackFilename.c_str(), recurse, alternative,
              sXmlpackFilename.c_str());
    return;
  ...
  if (Handle == NULL) {
    nlwarning("PATH: CPath::addSearchXmlpackFile(%s, %d, %d): can't open file, "
              "skip it",
              sXmlpackFilename.c_str(), recurse, alternative);
    return;
  }
  ...
  if (CXMLPack::getInstance().add(sXmlpackFilename)) {
    // also add the xmlpack file name in the map to retrieve the full path of a
    // .xml_pack when we want modification date of the xml_pack for example
    insertFileInMap(sXmlpackFilename, sXmlpackFilename, false,
                    CFile::getExtension(sXmlpackFilename));
    ...
    for (uint i = 0; i < filenames.size(); ++i) {
      ...
      if (progressCallBack) {
        ...
        progressCallBack->pushCropedValues((float)i / (float)filenames.size(),
                                           (float)(i + 1) /
      ...
      insertFileInMap(filenames[i], packfilenamealone + "@@" + filenames[i],
                      false, ext);
      ...
      for (uint j = 0; j < _Extensions.size(); j++) {
        if (_Extensions[j].first == ext) {
          // need to remap
          insertFileInMap(filenamewoext + "." + _Extensions[j].second,
                          packfilenamealone + "@@" + filenames[i], true,
                          _Extensions[j].first);
        }
      }
    ...
    nlwarning("PATH: CPath::addSearchXmlpackFile(%s, %d, %d): can't add the "
              "xml pack file",
              sXmlpackFilename.c_str(), recurse, alternative);
  }
...
void CFileContainer::insertFileInMap(const string &filename,
                                     const string &filepath, bool remap,
  ...
  if (it != _Files.end()) {
    ...
    if (path.find("@") != string::npos && filepath.find("@") == string::npos) {
      // if there's a file in a big file and a file in a path, the file in path
      // wins replace with the new one
      nlinfo("PATH: CPath::insertFileInMap(%s, %s, %d, %s): already inserted "
             "from '%s' but special case so override it",
             filename.c_str(), filepath.c_str(), remap, extension.c_str(),
             path.c_str());
      ...
      if (path2 == sPathOnly)
        return;
      nlwarning("PATH: CPath::insertFileInMap(%s, %s, %d, %s): already "
                "inserted from '%s', skip it",
                filename.c_str(), filepath.c_str(), remap, extension.c_str(),
                path2.c_str());
    }
  } else {
    ...
    NL_DISPLAY_PATH("PATH: CPath::insertFileInMap(%s, %s, %d, %s): added",
                    toLowerAscii(filename).c_str(), filepath.c_str(), remap,
                    toLowerAscii(extension).c_str());
  }
}

void CPath::display() { getInstance()->_FileContainer.display(); }
...
void CFileContainer::display() {
  ...
  if (_MemoryCompressed) {
    for (uint i = 0; i < _MCFiles.size(); ++i) {
      ...
      nlinfo("PATH: %-25s %-5s %-5d %s", fe.Name, ext.c_str(), fe.Remapped,
             path.c_str());
    }
  } else {
    for (TFiles::iterator it = _Files.begin(); it != _Files.end(); it++) {
      string ext = SSMext.get((*it).second.idExt);
      string path = SSMpath.get((*it).second.idPath);
      nlinfo("PATH: %-25s %-5s %-5d %s", (*it).first.c_str(), ext.c_str(),
             (*it).second.Remapped, path.c_str());
    }
  }
  nlinfo("PATH: ");
  ...
  for (uint j = 0; j < _Extensions.size(); j++) {
    nlinfo("PATH: '%s' -> '%s'", _Extensions[j].first.c_str(),
           _Extensions[j].second.c_str());
  }
...
void CFileContainer::removeBigFiles(
    const std::vector<std::string> &bnpFilenames) {
...
void CPath::memoryCompress() { getInstance()->_FileContainer.memoryCompress(); }
...
void CFileContainer::memoryCompress() {
  ...
  nlinfo("PATH: Number of file: %d, extension: %d, path: %d", nDbg, nDbg2,
         nDbg3);
  ...
  while (it != _Files.end()) {
    ...
    if ((sTmp.find("@@") == string::npos) && (sTmp.find('@') != string::npos) &&
        (sTmp.find("snp@") == string::npos) && !it->second.Remapped) {
      // This is a file included in a bigfile (so the name is in the bigfile
      // manager)
    } else {
  ...
  while (it != _Files.end()) {
    ...
    if ((sTmp.find("@") == string::npos) || (sTmp.find("@@") != string::npos) ||
        (sTmp.find("snp@") != string::npos) || rFE.Remapped) {
      ...
      _MCFiles[nNb].Name =
          CBigFile::getInstance().getFileNamePtr(rFE.Name, sTmp);
      if (_MCFiles[nNb].Name == NULL) {
        nlerror("memoryCompress: failed to find named file in big file: %s",
                SSMpath.get(rFE.idPath));
      }
    }
...
void CFileContainer::memoryUncompress() {
  ...
  for (std::vector<CMCFileEntry>::iterator it = _MCFiles.begin();
       it != _MCFiles.end(); ++it) {
    CFileEntry fe;
    fe.Name = it->Name;
    fe.idExt = it->idExt;
...
bool CFile::isDirectory(const string &filename) {
  ...
  if (res == INVALID_FILE_ATTRIBUTES) {
    // nlwarning ("PATH: '%s' is not a valid file or directory name",
    // filename.c_str ());
    return false;
  }
  ...
  if (res == -1) {
    // There was previously a warning message here but that was incorrect as it
    // is defined that isDirectory returns false if the directory doesn't exist
    // nlwarning ("PATH: can't stat '%s' error %d '%s'", filename.c_str(),
...
uint32 CFile::getFileSize(const std::string &filename) {
  ...
  if (filename.find("@@") != string::npos) {
    ...
    if (pos > 3 && filename[pos - 3] == 's' && filename[pos - 2] == 'n' &&
        filename[pos - 1] == 'p') {
      uint32 fs = 0;
      CStreamedPackageManager::getInstance().getFileSize(
          fs, filename.substr(pos + 1));
      return fs;
    } else {
      uint32 fs = 0, bfo;
      bool c, d;
...
uint32 CFile::getFileModificationDate(const std::string &filename) {
  ...
  // time, the file date reported by _stat may (not always!) be adjusted by
  // 3600s This is a bad behavior because file time should always be reported as
  // UTC time value
  ...
  if (h == INVALID_HANDLE_VALUE) {
    nlwarning("Can't get modification date on file '%s' : %s", fn.c_str(),
              NLMISC::formatErrorMessage(NLMISC::getLastError()).c_str());
    return 0;
  }
  ...
  // win32 file times are in 10th of micro sec (100ns resolution), starting at
  // jan 1, 1601 hey Mr Gates, why 1601 ?
...
bool CFile::setFileModificationDate(const std::string &filename,
                                    uint32 modTime) {
  string::size_type pos;
  string fn;
  if ((pos = filename.find('@')) != string::npos) {
  ...
  // create a file handle (this does not open the file)
  HANDLE h = CreateFileW(nlUtf8ToWide(fn), GENERIC_WRITE | GENERIC_READ,
                         FILE_SHARE_WRITE | FILE_SHARE_READ, NULL,
                         OPEN_EXISTING, 0, 0);
  if (h == INVALID_HANDLE_VALUE) {
    nlwarning(
        "Can't set modification date on file '%s' (error accessing file) : %s",
        fn.c_str(), NLMISC::formatErrorMessage(NLMISC::getLastError()).c_str());
    return false;
  }
  ...
  if (GetFileTime(h, &creationFileTime, &accessFileTime, &modFileTime) == 0) {
    nlwarning("Can't set modification date on file '%s' : %s", fn.c_str(),
...
void CFile::removeFileChangeCallback(const std::string &filename) {
  ...
  for (uint i = 0; i < FileToCheck.size(); i++) {
    if (FileToCheck[i].FileName == fn) {
      nlinfo("PATH: CFile::removeFileChangeCallback: '%s' is removed from "
             "checked files modification",
             fn.c_str());
...
void CFile::addFileChangeCallback(const std::string &filename,
                                  void (*cb)(const string &filename)) {
  string fn = CPath::lookup(filename, false, false);
  if (fn.empty()) {
    fn = filename;
  }
  nlinfo("PATH: CFile::addFileChangeCallback: I'll check the modification date "
         "for this file '%s'",
         fn.c_str());
...
void CFile::checkFileChange(TTime frequency) {
  ...
  if (CTime::getLocalTime() > lastChecked + frequency) {
    for (uint i = 0; i < FileToCheck.size(); i++) {
      if (CFile::getFileModificationDate(FileToCheck[i].FileName) !=
          FileToCheck[i].LastModified) {
        // need to reload it
        if (FileToCheck[i].Callback != NULL)
          FileToCheck[i].Callback(FileToCheck[i].FileName);

        FileToCheck[i].LastModified =
            CFile::getFileModificationDate(FileToCheck[i].FileName);
      }
    }
...
static bool CopyMoveFile(const std::string &dest, const std::string &src,
                         bool copyFile, bool failIfExists = false,
  ...
  if (copyFile) {
    ...
    if (fp1 == NULL) {
      nlwarning("PATH: CopyMoveFile error: can't fopen in read mode '%s'",
                ssrc.c_str());
      return false;
    }
    FILE *fp2 = nlfopen(sdest, "wb");
    if (fp2 == NULL) {
      nlwarning("PATH: CopyMoveFile error: can't fopen in read write mode '%s'",
                sdest.c_str());
      return false;
    }
    ...
    while (s != 0) {
      ...
      if (ws != 1) {
        nlwarning(
            "Error copying '%s' to '%s', trying to write %u bytes failed.",
            ssrc.c_str(), sdest.c_str(), s);
        fclose(fp1);
        fclose(fp2);
    ...
    if (MoveFileW(nlUtf8ToWide(ssrc), nlUtf8ToWide(sdest)) == 0) {
      ...
      nlwarning("PATH: CopyMoveFile error: can't link/move '%s' into '%s', "
                "error %u (%s)",
                ssrc.c_str(), sdest.c_str(), lastError,
    ...
    if (rename(ssrc.c_str(), sdest.c_str()) == -1) {
      ...
      if (errno == EXDEV) {
        ...
        if (!CopyMoveFile(dest, src, true, failIfExists, progress))
          return false;
        ...
        if (!CFile::deleteFile(src))
          return false;

        // set same modification time
        if (!CFile::setFileModificationDate(dest, modificationTime)) {
          nlwarning("Unable to set modification time %s (%u) for %s",
                    timestampToHumanReadable(modificationTime).c_str(),
                    modificationTime, dest.c_str());
        }
      } else {
        nlwarning(
            "PATH: CopyMoveFile error: can't rename '%s' into '%s', error %u",
            ssrc.c_str(), sdest.c_str(), errno);
...
bool CFile::copyFile(const std::string &dest, const std::string &src,
                     bool failIfExists /*=false*/,
                     IProgressCallback *progress) {
  return CopyMoveFile(dest, src, true, failIfExists, progress);
}

bool CFile::quickFileCompare(const std::string &fileName0,
...
bool CFile::thoroughFileCompare(const std::string &fileName0,
                                const std::string &fileName1,
                                uint32 maxBufSize) {
  // make sure the files both exist
  if (!fileExists(fileName0) || !fileExists(fileName1))
  ...
  for (uint32 i = 0; i < fileSize; i += bufSize) {
    // for the last block in the file reduce buf size to represent the amount of
    // data left in file
...
bool CFile::createDirectory(const std::string &filename) {
  ...
  return mkdir(filename.c_str(), S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP |
...
bool CFile::createDirectoryTree(const std::string &filename) {
  ...
  while (i < filename.size()) {
    ...
    // if the file name ended with a '/' then there's no extra directory to
    // create
    if (i == filename.size())
      break;
...
bool CPath::makePathRelative(const std::string &basePath,
  ...
  for (;;) {
    ...
    if (strncmp(tmp.c_str(), src.c_str(), tmp.length()) == 0) {
      ...
      relativePath =
          prefix + relativePath.substr(size, relativePath.length() - size);
      return true;
    }
...
std::string CPath::makePathAbsolute(const std::string &relativePath,
  ...
  //
  else if (isalpha(relativePath[0]) && (relativePath[1] == ':') &&
...
bool CFile::setRWAccess(const std::string &filename) {
  ...
  if (_waccess(wideFile.c_str(), 00) == 0 &&
      _waccess(wideFile.c_str(), 06) == -1) {
    ...
    if (_wchmod(wideFile.c_str(), _S_IREAD | _S_IWRITE) == -1) {
      if (INelContext::getInstance().getAlreadyCreateSharedAmongThreads()) {
        nlwarning("PATH: Can't set RW access to file '%s': %d %s",
...
bool CFile::setExecutable(const std::string &filename) {
  ...
  if (stat(filename.c_str(), &buf) == 0) {
    ...
    if (chmod(filename.c_str(), mode) == -1) {
      if (INelContext::getInstance().getAlreadyCreateSharedAmongThreads()) {
        nlwarning("PATH: Can't set +x flag on file '%s': %d %s",
                  filename.c_str(), errno, strerror(errno));
      }
      return false;
    }
    ...
    if (INelContext::getInstance().getAlreadyCreateSharedAmongThreads()) {
      nlwarning("PATH: Can't access file '%s': %d %s", filename.c_str(), errno,
                strerror(errno));
    }
    return false;
  }
...
bool CFile::deleteFile(const std::string &filename) {
  ...
  if (res == -1) {
    if (INelContext::getInstance().getAlreadyCreateSharedAmongThreads()) {
      nlwarning("PATH: Can't delete file '%s': (errno %d) %s", filename.c_str(),
                errno, strerror(errno));
    }
    return false;
  }
...
#endif
  if (res == -1) {
    nlwarning("PATH: Can't delete directory '%s': (errno %d) %s",
              filename.c_str(), errno, strerror(errno));
    return false;
  }
  return true;
...
void CFile::getTemporaryOutputFilename(const std::string &originalFilename,

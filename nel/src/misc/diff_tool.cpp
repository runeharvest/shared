...
bool loadStringFile(const std::string filename,
                    vector<TStringInfo> &stringInfos, bool forceRehash,
                    ucchar openMark, ucchar closeMark, bool specialCase) {
  /*	uint8 *buffer = 0;
      uint	size;
  ...
  */
  ...
  while (first != last) {
    ...
    if (!CI18N::parseLabel(first, last, si.Identifier)) {
      ...
      nlwarning("DT: Fatal : In '%s', line %u: Invalid label after '%s'",
                filename.c_str(), line, lastLabel.c_str());
      return false;
    }
    ...
    if (specialCase) {
      ...
      if (!CI18N::parseMarkedString(openMark, closeMark, first, last,
                                    si.Text2)) {
        uint32 line = countLine(text, first);
    ...
    if (forceRehash || !parseHashFromComment(si.Comments, si.HashValue)) {
      ...
      //			nldebug("Generating hash for %s as %s",
      //si.Identifier.c_str(), CI18N::hashToString(si.HashValue).c_str());
    } else {
      //			nldebug("Comment = [%s]",
      //si.Comments.toString().c_str()); 			nldebug("Retrieving hash for %s as %s",
      //si.Identifier.c_str(), CI18N::hashToString(si.HashValue).c_str());
    }
    stringInfos.push_back(si);
  }
  ...
  // check identifier uniqueness
  {
    ...
    for (uint i = 0; i < stringInfos.size(); ++i) {
      ...
      if (it != unik.end()) {
        nlwarning("DT: loadStringFile : identifier '%s' exist twice",
                  stringInfos[i].Identifier.c_str());
        error = true;
      } else
...
ucstring prepareStringFile(const vector<TStringInfo> &strings,
                           bool removeDiffComments, bool noDiffInfo) {
  string diff;

  vector<TStringInfo>::const_iterator first(strings.begin()),
      last(strings.end());
  for (; first != last; ++first) {
    ...
    if (!si.Identifier.empty() || !si.Text.empty()) {
      ...
      //			if (si.Comments.find(ucstring("// HASH_VALUE "))
      //== ucstring::npos)
      if (!noDiffInfo) {
...
bool readPhraseFile(const std::string &filename, vector<TPhrase> &phrases,
                    bool forceRehash) {
  ucstring doc;
...
bool readPhraseFileFromString(ucstring const &doc, const std::string &filename,
                              vector<TPhrase> &phrases, bool forceRehash) {
  ...
  while (first != last) {
    ...
    if (!CI18N::parseLabel(first, last, phrase.Identifier)) {
      ...
      nlwarning(
          "DT: In '%s' line %u: Error parsing phrase identifier after %s\n",
    ...
    if (!CI18N::parseMarkedString('(', ')', first, last, phrase.Parameters)) {
      uint32 line = countLine(doc, first);
      nlwarning(
          "DT: in '%s', line %u: Error parsing parameter list for phrase %s\n",
          filename.c_str(), line, phrase.Identifier.c_str());
      return false;
    }
    CI18N::skipWhiteSpace(first, last, &phrase.Comments);
    if (first == last || *first != '{') {
      uint32 line = countLine(doc, first);
      nlwarning(
          "DT: In '%s', line %u: Error parsing block opening '{' in phase %s\n",
          filename.c_str(), line, phrase.Identifier.c_str());
      return false;
    }
    ++first;
    ...
    while (first != last && *first != '}') {
      ...
      while (first != last && *first == '(') {
        if (!CI18N::parseMarkedString('(', ')', first, last, cond)) {
          uint32 line = countLine(doc, first);
          nlwarning("DT: In '%s' line %u: Error parsing conditional expression "
                    "in phrase %s, clause %u\n",
                    filename.c_str(), line, phrase.Identifier.c_str(),
      ...
      if (first == last) {
        nlwarning("DT: in '%s': Found end of file in non closed block for "
                  "phrase %s\n",
                  filename.c_str(), phrase.Identifier.c_str());
        return false;
      }
      ...
      if (CI18N::parseMarkedString('[', ']', first, last, clause.Text)) {
        ...
        uint32 line = countLine(doc, first);
        nlwarning("DT: in '%s' line %u: Error reading text for clause %u (%s) "
                  "in  phrase %s\n",
                  filename.c_str(), line, phrase.Clauses.size() + 1,
                  clause.Identifier.c_str(), phrase.Identifier.c_str());
        return false;
      }
    ...
    if (first == last || *first != '}') {
      uint32 line = countLine(doc, first);
      nlwarning(
          "DT: in '%s' line %u: Missing block closing tag '}' in phrase %s\n",
          filename.c_str(), line, phrase.Identifier.c_str());
      return false;
    }
    ++first;
    ...
    // handle hash value.
    if (forceRehash ||
        !parseHashFromComment(phrase.Comments, phrase.HashValue)) {
      ...
      if (forceRehash) {
        ...
        ucstring::size_type pos =
            phrase.Comments.find(ucstring("// HASH_VALUE"));
  ...
  // check identifier uniqueness
  {
    ...
    for (uint i = 0; i < phrases.size(); ++i) {
      ...
      if (it != unik.end()) {
        nlwarning("DT: readPhraseFile : identifier '%s' exist twice",
                  phrases[i].Identifier.c_str());
        error = true;
      } else
...
ucstring preparePhraseFile(const vector<TPhrase> &phrases,
  ...
  for (; first != last; ++first) {
    ...
    if (!p.Identifier.empty() || !p.Clauses.empty()) {
      if (p.Comments.find(ucstring("// HASH_VALUE ")) == ucstring::npos) {
        // add the hash value.
        ret +=
            ucstring("// HASH_VALUE ") + CI18N::hashToString(p.HashValue) + nl;
      }
...
bool loadExcelSheet(const string filename, TWorksheet &worksheet,
                    bool checkUnique) {
...
bool readExcelSheet(const ucstring &str, TWorksheet &worksheet,
                    bool checkUnique) {
  if (str.empty())
    return true;
  ...
  // **** Build array of lines. just point to strArray, and fill 0 where
  // appropriated
  vector<ucchar *> lines;
  lines.reserve(500);
  ...
  while ((pos = str.find(nl, lastPos)) != ucstring::npos) {
    if (pos > lastPos) {
      strArray[pos] = 0;
      //			nldebug("Found line : [%s]",
      //ucstring(&strArray[lastPos]).toString().c_str());
  ...
  for (i = 0; i < lines.size(); ++i) {
    ...
    if (newColCount != max(newColCount, numCells)) {
      ...
      nldebug("At line %u, numCol changed to %u", i, newColCount);
    }
  }
  ...
  for (i = 0; i < lines.size(); ++i) {
    ...
    worksheet.setData(startLine + i, numCells, cell);
    numCells++;
    nlassertex(numCells <= newColCount,
               ("readExcelSheet: bad row format: at line %u, the row has %u "
                "cell, max is %u",
                i, numCells, newColCount));
  ...
  if (checkUnique) {
    if (worksheet.size() > 0) {
      ...
      uint nameCol = 0;
      while (nameCol < worksheet.ColCount &&
             (*worksheet.getData(0, nameCol).begin() == uint16('*') ||
              worksheet.getData(0, nameCol) == ucstring("DIFF_CMD")))
        ++nameCol;
      ...
      if (nameCol < worksheet.ColCount) {
        ...
        for (uint j = 0; j < worksheet.size(); ++j) {
          ...
          if (it != unik.end()) {
            nlwarning("DT: readExcelSheet : identifier '%s' exist twice",
                      worksheet.getData(j, nameCol).toString().c_str());
            error = true;
          } else
...
void makeHashCode(TWorksheet &sheet, bool forceRehash) {
  if (!sheet.Data.empty()) {
    TWorksheet::TRow::iterator it = find(
        sheet.Data[0].begin(), sheet.Data[0].end(), ucstring("*HASH_VALUE"));
    if (forceRehash || it == sheet.Data[0].end()) {
      ...
      for (uint k = 1; k < sheet.ColCount; ++k) {
        if (sheet.Data[0][k].find(ucstring("*")) != 0 &&
            sheet.Data[0][k].find(ucstring("DIFF ")) != 0) {
          columnOk[k] = true;
        }
      }
...
ucstring prepareExcelSheet(const TWorksheet &worksheet) {
  ...
  for (uint i = 0; i < worksheet.Data.size(); ++i) {
    for (uint j = 0; j < worksheet.Data[i].size(); ++j) {
      if (i > 0 && hashValue[j] &&
          (!worksheet.Data[i][j].empty() && worksheet.Data[i][j][0] != '_'))
        text += "_";

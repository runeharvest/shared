...
CEvalNumExpr::TReturnState CEvalNumExpr::getNextToken(TToken &token) {
  ...
  if (currentChar <= 128) {
    ...
    // Is a number ?
      ...
      // Is an octal value ?
      else if ((currentChar == '0') && (secondChar >= '0') &&
               (secondChar <= '9')) {
        // Go to the number
        _ExprPtr++;
        currentChar = *_ExprPtr;
...
CEvalNumExpr::TReturnState CEvalNumExpr::evalExpression(const char *expression,
                                                        double &result,
                                                        int *errorIndex,
                                                        uint32 userData) {
  // Init the ptr
...
CEvalNumExpr::TReturnState CEvalNumExpr::evalExpression(double &finalResult,
                                                        TToken &nextToken,
                                                        uint32 userData) {
  // Array of result
  ...
  for (;;) {
    ...
    // init table
    for (uint i = 0; i < (uint)InternalOperator; ++i)
      resultUnaryOp[i] = NotOperator;
              ...
              if (error == NoError) {
                ...
                if (nextToken == Close) {
                  ...
                  case Pow:
                    ...
                    break;
                  case Rand:
                    value = arg0 + (arg1 - arg0) * (double)rand() /
                                       ((double)RAND_MAX + 1.0);
                    break;
                  default:
    ...
    for (i = unaryOpCount - 1; i >= 0; i--) {
      switch ((i < InternalOperator) ? resultUnaryOp[i]
                                     : resultUnaryOpSup[i - InternalOperator]) {
      case Not:
  ...
  while (exprCount != 1) {
    // Reduct ?
    TOperator before = (((index - 1) < InternalOperator)
                            ? resultOp[index - 1]
                            : resultOpSup[index - 1 - InternalOperator]);
    TOperator after = (index < opCount)
                          ? (((index) < InternalOperator)
                                 ? resultOp[index]
                                 : resultOpSup[index - InternalOperator])
                          : NotOperator;
    if ((index == opCount) ||
        (_OperatorPrecedence[before] <= _OperatorPrecedence[after])) {
      // Eval the value
      double &v0 = ((index - 1) < InternalOperator)
                       ? result[index - 1]
                       : resultSup[index - 1 - InternalOperator];
      double &v1 = ((index) < InternalOperator)
                       ? result[index]
                       : resultSup[index - InternalOperator];
      ...
      case Xor:
        ...
        break;
      case LogicalAnd:
        v0 = (double)(uint)((floor(v0 + 0.5) != 0.0) &&
                            (floor(v1 + 0.5) != 0.0));
        break;
      case LogicalOr:
        v0 = (double)(uint)((floor(v0 + 0.5) != 0.0) ||
      ...
      for (; i < exprCount - 1; i++) {
        ...
        if (i < InternalOperator)
          result[i] = (i + 1 < InternalOperator)
                          ? result[i + 1]
                          : resultSup[i + 1 - InternalOperator];
        else
          resultSup[i - InternalOperator] =
              (i + 1 < InternalOperator) ? result[i + 1]
                                         : resultSup[i + 1 - InternalOperator];
      }
      exprCount--;
      ...
      for (; i < opCount - 1; i++) {
        // Copy
        if (i < InternalOperator)
          resultOp[i] = (i + 1 < InternalOperator)
                            ? resultOp[i + 1]
                            : resultOpSup[i + 1 - InternalOperator];
        else
          resultOpSup[i - InternalOperator] =
              (i + 1 < InternalOperator)
                  ? resultOp[i + 1]
                  : resultOpSup[i + 1 - InternalOperator];
      }
      opCount--;
...
const CEvalNumExpr::TOperator CEvalNumExpr::_OperatorArray[128] = {
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, ExtOperator, NotOperator,
    NotOperator, NotOperator, Remainder,   ExtOperator, NotOperator,
    NotOperator, NotOperator, Mul,         Plus,        NotOperator,
    ExtOperator, NotOperator, Div,         NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    ...
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, ExtOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
    NotOperator, NotOperator, NotOperator, NotOperator, NotOperator,
...
CEvalNumExpr::TReturnState
CEvalNumExpr::evalValue(const char *value, double &result, uint32 userData) {
  return UnknownValue;
}
...
CEvalNumExpr::TReturnState
CEvalNumExpr::evalFunction(const char *funcName, double arg0, double &result) {
  return UnknownFunction;
}

// ***************************************************************************

CEvalNumExpr::TReturnState CEvalNumExpr::evalFunction(const char *funcName,

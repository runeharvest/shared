...
void CSystemUtils::setWindow(nlWindow window) { s_window = window; }

bool CSystemUtils::updateProgressBar(uint value, uint total) {
#ifdef TASKBAR_PROGRESS
  if (s_window == NULL) {
    nldebug("No window has be set with CSystemUtils::setWindow(), progress bar "
            "can't be displayed");
    return false;
  }
  ...
  // error can be ignored because Windows versions before Windows 7 doesn't
  // support it
  ...
  if (total) {
    ...
    hr = pTaskbarList->SetProgressValue(s_window, (ULONGLONG)value,
                                        (ULONGLONG)total);
  } else {
    // don't update anymore the progress
    hr = pTaskbarList->SetProgressState(
        s_window, value == 0 ? TBPF_INDETERMINATE : TBPF_NOPROGRESS);
  }
...
bool CSystemUtils::copyTextToClipboard(const std::string &text) {
  ...
  if (OpenClipboard(NULL)) {
    ...
    HGLOBAL mem =
        GlobalAlloc(GHND | GMEM_DDESHARE,
                    isUnicode ? ((textWide.size() + 1) * sizeof(wchar_t))
                              : (textMbcs.size() + 1));
...
bool CSystemUtils::pasteTextFromClipboard(std::string &text) {
  ...
  if (OpenClipboard(NULL)) {
    ...
    // warning, this code can't be debuggued in VC++ IDE, hObj will be always
    // NULL
      ...
      if (hLock != NULL) {
        ...
        if (isUnicode) {
          ...
          if (!text.size() && str[0])
            ...
            text =
                CUtfStringView(text).toUtf8(true); // Sanitize UTF-8 user input
        } else {
...
bool CSystemUtils::isScreensaverEnabled() {
  ...
  LSTATUS lReturn = RegOpenKeyExA(HKEY_CURRENT_USER, "Control Panel\\Desktop",
  ...
  if (lReturn == ERROR_SUCCESS) {
    ...
    lReturn = RegQueryValueExA(hKeyScreenSaver, "SCRNSAVE.EXE", NULL, &dwType,
                               NULL, &dwSize);
...
bool CSystemUtils::enableScreensaver(bool screensaver) {
  bool res = false;
#ifdef NL_OS_WINDOWS
  res = (SystemParametersInfoA(SPI_SETSCREENSAVEACTIVE,
                               screensaver ? TRUE : FALSE, NULL, 0) == TRUE);
#endif
  return res;
}

std::string CSystemUtils::getRootKey() { return RootKey; }

void CSystemUtils::setRootKey(const std::string &root) { RootKey = root; }
...
string CSystemUtils::getRegKey(const string &entry) {
  ...
  if (RegOpenKeyExW(HKEY_CURRENT_USER, nlUtf8ToWide(RootKey), 0, KEY_READ,
                    &hkey) == ERROR_SUCCESS) {
    DWORD dwType = 0L;
    DWORD dwSize = KeyMaxLength;
    wchar_t buffer[KeyMaxLength];

    if (RegQueryValueExW(hkey, nlUtf8ToWide(entry), NULL, &dwType,
...
bool CSystemUtils::setRegKey(const string &valueName, const string &value) {
  ...
  if (RegCreateKeyExW(HKEY_CURRENT_USER, nlUtf8ToWide(RootKey), 0, NULL,
    ...
    if (RegSetValueExW(hkey, nlUtf8ToWide(valueName), 0, REG_SZ,
                       (const BYTE *)wvalue.c_str(),
                       (wvalue.size() + 1) * sizeof(WCHAR)) == ERROR_SUCCESS)
      res = true;
    RegCloseKey(hkey);
  } else {
    nlwarning("Can't set the reg key '%s' '%s'", valueName.c_str(),
              value.c_str());
  }
#endif
...
/// Detect whether the current process is a windowed application. Return true if
/// definitely yes, false if unknown
  ...
  for (uint index = 0;; ++index) {
    ...
    if (SUCCEEDED(pAdapter->GetDesc(&desc))) {
      ...
      nldebug("DXGI Adapter: %u - %s - DedicatedVideoMemory: %d KiB", index,
              adapter.name.c_str(), adapter.memory);
...
sint CSystemUtils::getTotalVideoMemory() {
  ...
  if (hDXGI) {
    ...
    if (!pCreateDXGIFactory) {
      pCreateDXGIFactory =
          (LPCREATEDXGIFACTORY)GetProcAddress(hDXGI, "CreateDXGIFactory");
    }
  ...
  if (res == -1) {
    ...
    if (hInstDDraw) {
      ...
      LPDIRECTDRAWENUMERATEEXA pDirectDrawEnumerateEx =
          (LPDIRECTDRAWENUMERATEEXA)GetProcAddress(hInstDDraw,
      ...
      if (pDirectDrawEnumerateEx && pDDCreate) {
        HRESULT hr = pDirectDrawEnumerateEx(DDEnumCallbackEx, (VOID *)&adapter,
        ...
        if (SUCCEEDED(hr) && adapter.found) {
          ...
          if (SUCCEEDED(hr)) {
            ...
            if (SUCCEEDED(hr)) {
              ...
              hr = pDDraw7->GetAvailableVidMem(&ddscaps, &pdwAvailableVidMem,
                                               NULL);

              if (SUCCEEDED(hr)) {
                res = (sint)pdwAvailableVidMem / 1024;
                nlinfo("DirectDraw Adapter: %s - DedicatedVideoMemory: %d KiB",
                       adapter.name.c_str(), adapter.memory);
              } else {
                nlwarning("Unable to get DirectDraw available video memory: %s",
                          FormatError(hr).c_str());
              }

              SAFE_RELEASE(pDDraw7);
            } else {
              nlwarning("Unable to query IDirectDraw7 interface: %s",
                        FormatError(hr).c_str());
            }
          } else {
            nlwarning("Unable to call DirectDrawCreate: %s",
                      FormatError(hr).c_str());
          }
        } else {
          nlwarning("Unable to enumerate DirectDraw adapters (%s): %s",
                    (adapter.found ? "found" : "not found"),
                    FormatError(hr).c_str());
        }
      } else {
        nlwarning("Unable to get pointer on DirectDraw functions "
                  "(DirectDrawEnumerateExA %p, DirectDrawCreate %p)",
                  pDirectDrawEnumerateEx, pDDCreate);
      }
      ...
      for (uint i = 0; i < lines.size(); ++i) {
        ...
        if (NLMISC::fromString(memory, res)) {
          if (units == "MB") {
            ...
            nlwarning("nvidia-smi reported %d %s as wrong video memory units",
                      res, units.c_str());
            break;
          }
    ...
    if (file.open(logFile, true)) {
      ...
      while (!file.eof()) {
        ...
        if (pos != std::string::npos) {
          ...
          if (!NLMISC::fromString(videoMemory, res))
            continue;
      ...
      if (deviceId.empty()) {
        ...
        if (out.empty()) {
          ...
          for (uint i = 0; i < lines.size(); ++i) {
            ...
            sint tmpSize;
            if (!NLMISC::fromString(sizeStr, tmpSize))
              continue;

...
CWinDisplayer::CWinDisplayer(const char *displayerName)
    : CWindowDisplayer(displayerName), Exit(false) {
  needSlashR = true;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam,
  ...
  case WM_ACTIVATE: {
    if (LOWORD(wParam) != WA_INACTIVE) {
      CWinDisplayer *cwd =
          (CWinDisplayer *)GetWindowLongPtrW(hWnd, GWLP_USERDATA);
      if (cwd != NULL)
  ...
  case WM_SIZE: {
    ...
    if (cwd != NULL) {
      ...
      //				SetWindowPos (cwd->_HEdit, NULL, 0,
      //cwd->_ToolBarHeight, w, h-cwd->_ToolBarHeight-cwd->_InputEditHeight,
      //SWP_NOZORDER | SWP_NOACTIVATE );
      SetWindowPos(cwd->_HInputEdit, NULL, 0, h - cwd->_InputEditHeight, w,
                   cwd->_InputEditHeight, SWP_NOZORDER | SWP_NOACTIVATE);
      cwd->resizeLabels();
    }
  } break;
  case WM_DESTROY:
  ...
  case WM_COMMAND: {
    if (HIWORD(wParam) == BN_CLICKED) {
      ...
      CSynchronized<std::vector<CWindowDisplayer::CLabelEntry>>::CAccessor
          access(&cwd->_Labels);
      for (uint i = 0; i < access.value().size(); i++) {
        if (access.value()[i].Hwnd == (HWND)lParam) {
          if (access.value()[i].Value == "@Clear|CLEAR") {
            // special commands because the clear must be called by the display
            // thread and not main thread
            cwd->clear();
          } else {
            // the button was found, add the command in the command stack
            CSynchronized<std::vector<std::string>>::CAccessor accessCommands(
                &cwd->_CommandsToExecute);
            string str;
    ...
    case EN_MSGFILTER:
      ...
      if (pmf->msg == WM_CHAR) {
        if (pmf->wParam == VK_RETURN) {
          ...
          string TextSend;
          CWinDisplayer *cwd =
              (CWinDisplayer *)GetWindowLongPtr(hWnd, GWLP_USERDATA);
          ...
          while (*pos2 != '\0') {
            ...
            if (!str.empty()) {
              {
                CSynchronized<std::vector<std::string>>::CAccessor access(
                    &cwd->_CommandsToExecute);
          ...
          SendMessageA(cwd->_HInputEdit, EM_SETSEL, wcslen(wText),
                       wcslen(wText));
        ...
        if (pmf->wParam == VK_UP) {
          CWinDisplayer *cwd =
              (CWinDisplayer *)GetWindowLongPtrA(hWnd, GWLP_USERDATA);
          ...
          if (!cwd->_History.empty()) {
            ...
            SendMessageA(cwd->_HInputEdit, EM_SETSEL, (WPARAM)ucs.size(),
                         (LPARAM)ucs.size());
          }
      ...
      if (access.value()[i].NeedUpdate && !access.value()[i].Value.empty()) {
        if (access.value()[i].Hwnd == NULL) {
          ...
          if (access.value()[i].Value[0] == '@') {
            access.value()[i].Hwnd = CreateWindowA(
                "BUTTON", "", WS_CHILD | WS_VISIBLE | BS_DEFPUSHBUTTON, 0, 0, 0,
                0, _HWnd, (HMENU)NULL,
                (HINSTANCE)GetWindowLongPtrA(_HWnd, GWLP_HINSTANCE), NULL);
          } else {
            access.value()[i].Hwnd = CreateWindowA(
                "STATIC", "", WS_CHILD | WS_VISIBLE | SS_SIMPLE, 0, 0, 0, 0,
                _HWnd, (HMENU)NULL,
                (HINSTANCE)GetWindowLongPtrA(_HWnd, GWLP_HINSTANCE), NULL);
          }
          SendMessageA((HWND)access.value()[i].Hwnd, WM_SETFONT, (WPARAM)_HFont,
                       TRUE);
          needResize = true;
        }
        ...
        // do this tricks to be sure that windows will clear what is after the
        // number
        if (access.value()[i].Value[0] != '@')
        ...
        SendMessageW((HWND)access.value()[i].Hwnd, WM_SETTEXT, 0,
                     (LPARAM)nlUtf8ToWide(n));
...
void CWinDisplayer::resizeLabels() {
  {
    ...
    for (;;) {
      nb = 0;
      while (i + nb != access.value().size() &&
             !access.value()[i + nb].Value.empty())
        nb++;
      ...
      for (uint j = 0; j < nb; j++) {
        if ((HWND)access.value()[i + j].Hwnd != NULL)
          SetWindowPos((HWND)access.value()[i + j].Hwnd, NULL, j * delta,
                       y * _ToolBarHeight, delta, _ToolBarHeight,
                       SWP_NOZORDER | SWP_NOACTIVATE);
      }
      i += nb + 1;
      y++;
    ...
    SetWindowPos(_HEdit, NULL, 0, nby * _ToolBarHeight, Rect.right,
                 Rect.bottom - nby * _ToolBarHeight - _InputEditHeight,
                 SWP_NOZORDER | SWP_NOACTIVATE);
  }
}
...
void CWinDisplayer::open(string titleBar, bool iconified, sint x, sint y,
                         sint w, sint h, sint hs, sint fs,
                         const std::string &fn, bool ww, CLog *log) {
  if (w == -1)
    w = 700;
  if (h == -1)
    h = 300;
  ...
  // create the window
  _HWnd = CreateWindowW(L"NLDisplayerClass", L"", WndFlags, CW_USEDEFAULT,
                        CW_USEDEFAULT, WndRect.right, WndRect.bottom, NULL,
  ...
  _HFont = CreateFontW(-MulDiv(rfs, GetDeviceCaps(GetDC(0), LOGPIXELSY), 72), 0,
                       0, 0, FW_DONTCARE, FALSE, FALSE, FALSE, DEFAULT_CHARSET,
                       OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
                       DEFAULT_PITCH | FF_DONTCARE,
                       (LPCWSTR)ucstring::makeFromUtf8(rfn).c_str());

  // create the edit control
  DWORD dwStyle = WS_HSCROLL | WS_CHILD | WS_VISIBLE | WS_VSCROLL |
  ...
  _HEdit = CreateWindowExW(
      WS_EX_OVERLAPPEDWINDOW, RICHEDIT_CLASSW, L"", dwStyle, 0, _ToolBarHeight,
      w, h - _ToolBarHeight - _InputEditHeight, _HWnd, (HMENU)NULL,
  ...
  _HInputEdit =
      CreateWindowExW(WS_EX_OVERLAPPEDWINDOW, RICHEDIT_CLASSW, L"",
                      WS_CHILD |
                          WS_VISIBLE
                          /*| ES_MULTILINE*/
                          | ES_WANTRETURN | ES_NOHIDESEL | ES_AUTOHSCROLL,
                      0, h - _InputEditHeight, w, _InputEditHeight, _HWnd, NULL,
                      (HINSTANCE)GetWindowLongPtr(_HWnd, GWLP_HINSTANCE), NULL);
  SendMessageW(_HInputEdit, WM_SETFONT, (WPARAM)_HFont, TRUE);

  LRESULT dwEvent =
      SendMessageW(_HInputEdit, EM_GETEVENTMASK, (WPARAM)0, (LPARAM)0);
...
void CWinDisplayer::display_main() {
  ...
  while (_Continue) {
    ...
    {
      CSynchronized<std::list<std::pair<uint32, std::string>>>::CAccessor
          access(&_Buffer);
      ...
      if (vecSize > 0) {
        ...
        if (GetScrollInfo(_HEdit, SB_VERT, &info) != 0)
          bottom =
              (info.nPage == 0) || (info.nMax <= (info.nPos + (int)info.nPage));

        // look if we have the focus
        bool focus = (GetFocus() == _HEdit);
        if (focus) {
          SendMessageA(_HEdit, EM_SETOPTIONS, ECOOP_AND,
                       (LPARAM)~ECO_AUTOVSCROLL);
          SendMessageA(_HEdit, EM_SETOPTIONS, ECOOP_AND,
                       (LPARAM)~ECO_AUTOHSCROLL);
        }
        ...
        // find how many lines we have to remove in the current output to add
        // new lines
        ...
        if (_HistorySize > 0 && nLine + vecSize > _HistorySize) {
          ...
          if (nblineremove == _HistorySize) {
            ...
            LRESULT oldIndex2 =
                SendMessageW(_HEdit, EM_LINEINDEX, nblineremove, 0);
        ...
        for (it = access.value().begin(); it != access.value().end();) {
          ...
          if ((col >> 24) == 0) {
            ...
            SendMessageA(_HEdit, EM_SETCHARFORMAT, (WPARAM)SCF_SELECTION,
                         (LPARAM)&CharFormat);
          }
        ...
        if (focus) {
          SendMessageA(_HEdit, EM_SETOPTIONS, ECOOP_OR,
                       (LPARAM)ECO_AUTOVSCROLL);
          SendMessageA(_HEdit, EM_SETOPTIONS, ECOOP_OR,
                       (LPARAM)ECO_AUTOHSCROLL);
        }

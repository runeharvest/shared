...
void CBigFile::releaseInstance() {
  if (_Instance) {
    NLMISC::INelContext::getInstance().releaseSingletonPointer("CBigFile",
                                                               _Instance);
    delete _Instance;
    _Instance = NULL;
  }
...
// ***************************************************************************
CBigFile::CThreadFileArray::CThreadFileArray() { _CurrentId = 0; }
...
// ***************************************************************************
uint32 CBigFile::CThreadFileArray::allocate() { return _CurrentId++; }
...
CBigFile::CHandleFile &CBigFile::CThreadFileArray::get(uint32 index) {
  ...
  // if the vector is not allocated, allocate it (empty entries filled with NULL
  // => not opened FILE* in this thread)
  if (index >= ptr->size()) {
...
bool CBigFile::add(const std::string &sBigFileName, uint32 nOptions) {
  ...
  if (_BNPs.find(bigfilenamealone) != _BNPs.end()) {
    nlwarning("CBigFile::add : bigfile %s already added.",
              bigfilenamealone.c_str());
    return false;
  }
  ...
  // nldebug("BigFile : added bnp '%s' to the collection",
  // bigfilenamealone.c_str());
...
CBigFile::BNP::BNP()
    : FileNames(NULL), ThreadFileId(0), CacheFileOnOpen(false),
      AlwaysOpened(false), InternalUse(false), OffsetFromBeginning(0) {}
...
bool CBigFile::BNP::readHeader() {
  // Only external use
  if (InternalUse || BigFileName.empty())
    return false;
...
// ***************************************************************************
bool CBigFile::BNP::unpack(const std::string &sDestDir,
                           TUnpackProgressCallback *callback) {
  // Only external use
  if (InternalUse || BigFileName.empty())
    return false;
  ...
  for (uint32 i = 0; i < SFiles.size(); ++i) {
    ...
    if (out != NULL) {
      ...
      if (readError) {
        nlwarning("%s read error errno = %d: %s", filename.c_str(), errno,
                  strerror(errno));
      }
      bool writeError = fwrite(ptr, rBNPFile.Size, 1, out) != 1;
      if (writeError) {
        nlwarning("%s write error errno = %d: %s", filename.c_str(), errno,
                  strerror(errno));
      }
...
// ***************************************************************************
void CBigFile::list(const std::string &sBigFileName,
  ...
  while (it != rbnp.Files.end()) {
    vAllFiles.push_back(
        string(it->Name)); // Add the name of the file to the return vector
    ++it;
  }
}
...
// ***************************************************************************
bool CBigFile::getFileInternal(const std::string &sFileName, BNP *&zeBnp,
                               BNPFile *&zeBnpFile) {
  ...
  itNBPFile = lower_bound(rbnp.Files.begin(), rbnp.Files.end(), temp_bnp_file,
                          CBNPFileComp());
...
FILE *CBigFile::getFile(const std::string &sFileName, uint32 &rFileSize,
                        uint32 &rBigFileOffset, bool &rCacheFileOnOpen,
                        bool &rAlwaysOpened) {
  BNP *bnp = NULL;
  ...
  /* If not opened, open it now. There is 2 reason for it to be not opened:
      rbnp.AlwaysOpened==false, or it is a new thread which use it for the first
     time.
  */
  if (handle.File == NULL) {
    handle.File = nlfopen(bnp->BigFileName, "rb");
    if (handle.File == NULL) {
      nlwarning("bnp: can't fopen big file '%s' error %d '%s'",
                bnp->BigFileName.c_str(), errno, strerror(errno));
      return NULL;
    }
  }
...
// ***************************************************************************
bool CBigFile::getFileInfo(const std::string &sFileName, uint32 &rFileSize,
...
// ***************************************************************************
char *CBigFile::getFileNamePtr(const std::string &sFileName,
                               const std::string &sBigFileName) {
  string bigfilenamealone = CFile::getFilename(sBigFileName);
  if (_BNPs.find(bigfilenamealone) != _BNPs.end()) {
...
void CBigFile::getBigFilePaths(std::vector<std::string> &bigFilePaths) {
  bigFilePaths.clear();
  for (std::map<std::string, BNP>::iterator it = _BNPs.begin();
       it != _BNPs.end(); ++it) {
...
// ***************************************************************************
bool CBigFile::unpack(const std::string &sBigFileName,

...
TMouseButton CWinEventEmitter::getButtons() const {
  uint result = (_CtrlButton ? ctrlButton : 0) | (_AltButton ? altButton : 0) |
...
bool CWinEventEmitter::processMessage(HWND hWnd, uint32 msg, WPARAM wParam,
                                      LPARAM lParam, CEventServer *server) {
  if (!server)
    server = &_InternalServer;

  /// Process IME messages
  /*if ( _IMEEventsEnabled && (ImmIsUIMessage( ImmGetDefaultIMEWnd((HWND)_HWnd),
  msg, wParam, lParam) == TRUE) )
  {
      server->postEvent( new CEventIME(msg, wParam, lParam, this) );
      return true; // trap message (however DefWindowProc will still be called
  in some instances by the event listener)
  }*/
    ...
    if (_KeyboardEventsEnabled) {
      ...
      if (wParam < KeyCount)
        server->postEvent(new CEventKeyDown(
            (NLMISC::TKey)wParam,
            getKeyButton(_AltButton, _ShiftButton, _CtrlButton),
            (((int)lParam) & (1 << 30)) == 0, this));
    }
    break;
  ...
  case WM_KEYUP:
    if (_KeyboardEventsEnabled) {
      ...
      // As Print Screen button does not trigger a WM_KEYDOWN msg, simulate it
      // here
      if ((int)wParam == VK_SNAPSHOT) {
        if (wParam < KeyCount)
          server->postEvent(new CEventKeyDown(
              (NLMISC::TKey)wParam,
              getKeyButton(_AltButton, _ShiftButton, _CtrlButton), true, this));
      }

      // Post the message
      if (wParam < KeyCount)
        server->postEvent(new CEventKeyUp(
            (NLMISC::TKey)wParam,
            getKeyButton(_AltButton, _ShiftButton, _CtrlButton), this));
    }
    break;
#ifdef WM_UNICHAR
  case WM_UNICHAR:
    if (wParam != UNICODE_NOCHAR && _KeyboardEventsEnabled) {
      ...
      server->postEvent(new CEventChar(
          (u32char)wParam, getKeyButton(_AltButton, _ShiftButton, _CtrlButton),
          this));
    }
    break;
#endif
  case WM_CHAR:
    if (_KeyboardEventsEnabled) {
      ...
      if ((c & 0xFC00) == 0xD800) {
        ...
        for (CUtfStringView::iterator it(cv.begin()), end(cv.end()); it != end;
             ++it)
          server->postEvent(new CEventChar(
              *it, getKeyButton(_AltButton, _ShiftButton, _CtrlButton), this));
      } else {
        if (_Utf16Pair) {
          server->postEvent(new CEventChar(
              _Utf16Pair, getKeyButton(_AltButton, _ShiftButton, _CtrlButton),
              this));
          _Utf16Pair = 0;
        }
        server->postEvent(new CEventChar(
            c, getKeyButton(_AltButton, _ShiftButton, _CtrlButton), this));
      }
    }
    break;
  /*case WM_IME_CHAR:
    ...
    if (_MouseEventsEnabled) {
      ...
      case WM_RBUTTONDOWN:
        ...
        server->postEvent(new CEventMouseDown(
            fX, fY,
            (TMouseButton)(rightButton | (button & ~(leftButton | middleButton |
                                                     rightButton))),
            this));
        break;
      case WM_MBUTTONDOWN:
      ...
      case WM_LBUTTONDOWN:
        ...
        server->postEvent(new CEventMouseDown(
            fX, fY,
            (TMouseButton)(leftButton | (button & ~(leftButton | middleButton |
                                                    rightButton))),
            this));
        break;
      ...
      case WM_RBUTTONUP:
        ...
        server->postEvent(new CEventMouseUp(
            fX, fY,
            (TMouseButton)(rightButton | (button & ~(leftButton | middleButton |
                                                     rightButton))),
            this));
      ...
      case WM_RBUTTONDBLCLK:
        server->postEvent(new CEventMouseDblClk(
            fX, fY,
            (TMouseButton)(rightButton | (button & ~(leftButton | middleButton |
                                                     rightButton))),
            this));
  ...
  case WM_DISPLAYCHANGE:
    server->postEvent(new CEventDisplayChange(LOWORD(lParam), HIWORD(lParam),
                                              (uint)wParam, this));
    break;
  case WM_MOUSEWHEEL:
    if (_MouseEventsEnabled) {
      ...
      server->postEvent(new CEventMouseWheel(fX, fY, button,
                                             (short)HIWORD(wParam) >= 0, this));
      break;
    }
  ...
  case WM_INPUTLANGCHANGE:
    if (_IMEEventsEnabled) {
      ...
      server->postEvent(
          new CEventIME(msg, (uint32)wParam, (uint32)lParam, this));
...
TMouseButton CWinEventEmitter::buildFlags() const {
  uint flags = (_CtrlButton ? ctrlButton : 0) |
               (_ShiftButton ? shiftButton : 0) | (_AltButton ? altButton : 0) |
               (_MouseButtons[0] ? leftButton : 0) |
               (_MouseButtons[1] ? rightButton : 0) |
               (_MouseButtons[2] ? middleButton : 0);

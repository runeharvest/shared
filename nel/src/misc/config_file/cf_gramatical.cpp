...
// WARNING!!!! DEBUG_PRINTF are commented using // so IT MUST HAVE NO
// INSTRUCTION AFTER A DEBUG_PRINTF OR THEY LL BE COMMENTED
/*
...
#if YYDEBUG != 0
static const short yyprhs[] = {0,  0,  2,  3,  6,  8,  12, 17, 22, 24,
                               28, 33, 36, 38, 42, 44, 48, 52, 54, 58,
                               62, 65, 68, 72, 74, 76, 78, 80};

static const short yyrhs[] = {
    21, 0,  0,  21, 22, 0,  22, 0,  19, 6,  17, 0,  5,  4,  23, 7,  0,
    5,  3,  23, 7,  0,  25, 0,  15, 24, 14, 0,  15, 24, 16, 14, 0,  15,
    14, 0,  25, 0,  24, 16, 25, 0,  26, 0,  25, 8,  26, 0,  25, 9,  26,
    0,  27, 0,  26, 10, 27, 0,  26, 11, 27, 0,  8,  27, 0,  9,  27, 0,
    13, 23, 12, 0,  17, 0,  18, 0,  6,  0,  28, 0,  5,  0};

#endif

#if YYDEBUG != 0
static const short yyrline[] = {
    0,   104, 104, 107, 108, 111, 123, 185, 245, 246, 247, 248, 251, 252,
    255, 256, 257, 260, 261, 262, 265, 266, 267, 268, 269, 270, 271, 274};
#endif

#if YYDEBUG != 0 || defined(YYERROR_VERBOSE)

static const char *const yytname[] = {
    "$",        "error",    "$undefined.", "ADD_ASSIGN", "ASSIGN",
    "VARIABLE", "STRING",   "SEMICOLON",   "PLUS",       "MINUS",
    "MULT",     "DIVIDE",   "RPAREN",      "LPAREN",     "RBRACE",
    "LBRACE",   "COMMA",    "INTEGER",     "REAL",       "FILELINE",
    "ROOT",     "instlist", "inst",        "expression", "exprbrace",
    "expr2",    "expr3",    "expr4",       "variable",   NULL};
#endif

static const short yyr1[] = {0,  20, 20, 21, 21, 22, 22, 22, 23, 23,
                             23, 23, 24, 24, 25, 25, 25, 26, 26, 26,
                             27, 27, 27, 27, 27, 27, 27, 28};

static const short yyr2[] = {0, 1, 0, 2, 1, 3, 4, 4, 1, 3, 4, 2, 1, 3,
                             1, 3, 3, 1, 3, 3, 2, 2, 3, 1, 1, 1, 1, 1};

static const short yydefact[] = {2,  0,  0, 1,  4,  0,  0,  0,  3,  27, 25, 0,
                                 0,  0,  0, 23, 24, 0,  8,  14, 17, 26, 0,  5,
                                 20, 21, 0, 11, 0,  12, 7,  0,  0,  0,  0,  6,
                                 22, 9,  0, 15, 16, 18, 19, 10, 13, 0,  0,  0};

static const short yydefgoto[] = {45, 3, 4, 17, 28, 18, 19, 20, 21};

static const short yypact[] = {
    -4, 17,     2,      -4,     -32768, 1,      1,  -15, -32768, -32768, -32768,
    50, 50,     1,      22,     -32768, -32768, 10, 14,  23,     -32768, -32768,
    25, -32768, -32768, -32768, 31,     -32768, -3, 14,  -32768, 50,     50,
    50, 50,     -32768, -32768, -32768, 36,     23, 23,  -32768, -32768, -32768,
    14, 29,     46,     -32768};

static const short yypgoto[] = {-32768, -32768, 44, -1,    -32768,
                                -14,    6,      -8, -32768};

#define YYLAST 68

static const short yytable[] = {
    29, 1,  23, 24, 25, 22, 9,  10, 7,  11, 12, 37, 26, 38, 13, 2,  14, 30,
    15, 16, 5,  6,  31, 32, 44, 41, 42, 9,  10, 46, 11, 12, 35, 33, 34, 13,
    27, 39, 40, 15, 16, 9,  10, 36, 11, 12, 47, 8,  0,  13, 43, 0,  0,  15,
    16, 9,  10, 0,  11, 12, 0,  0,  0,  13, 0,  0,  0,  15, 16};

static const short yycheck[] = {
    14, 5,  17, 11, 12, 6, 5,  6,  6,  8,  9,  14, 13, 16, 13, 19, 15, 7,
    17, 18, 3,  4,  8,  9, 38, 33, 34, 5,  6,  0,  8,  9,  7,  10, 11, 13,
    14, 31, 32, 17, 18, 5, 6,  12, 8,  9,  0,  3,  -1, 13, 14, -1, -1, 17,
    18, 5,  6,  -1, 8,  9, -1, -1, -1, 13, -1, -1, -1, 17, 18};
...
#line 124 "cf_gramatical.yxx"
  {
    ...
    for (i = 0;
         i < (int)((*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))
                       .size());
         i++) {
      if ((*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i].Name ==
          yyvsp[-3].Val.String) {
        if (cf_OverwriteExistingVariable ||
            (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i].Root ||
            !strcmp(yyvsp[-3].Val.String, "RootConfigFilename")) {
    ...
    if (i == (int)((*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))
                       .size())) {
      // nouvelle variable
      ...
      (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM))).push_back(Var);
               ...
               (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i]
                   .Root ||
               !strcmp(yyvsp[-3].Val.String, "RootConfigFilename")) {
      // reaffectation d'une variable
      Var.Callback =
          (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i].Callback;
      DEBUG_PRINTF("yacc: reassign var name '%s' type %d\n", Var.Name.c_str(),
                   Var.Type);
      if (Var != (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i] &&
          Var.Callback != NULL)
        (Var.Callback)(Var);
      (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i] = Var;
    } else {
      DEBUG_PRINTF(
          "yacc: don't reassign var '%s' because the variable already exists\n",
          yyvsp[-3].Val.String);
    }
...
#line 186 "cf_gramatical.yxx"
  {
    ...
    if (i == (int)((*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))
      ...
      if ((*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i]
              .FromLocalFile) {
        // this var was created in the current cfg, append the new value at the
        // end
        ...
        // this var has been created in a parent Cfg, append at the beginning of
        // the array
...
#line 251 "cf_gramatical.yxx"
  {
    yyval.Val = yyvsp[0].Val; /*cf_CurrentVar.Type = $1.Type;*/
    cf_setVar(cf_CurrentVar, yyvsp[0].Val);
    ;
    break;
  }
  case 13:
#line 252 "cf_gramatical.yxx"
  {
    yyval.Val = yyvsp[0].Val; /*cf_CurrentVar.Type = $3.Type;*/
    cf_setVar(cf_CurrentVar, yyvsp[0].Val);
    ;
    break;
  }
...
#line 266 "cf_gramatical.yxx"
  {
    cf_value v;
    v.Type =
        NLMISC::CConfigFile::CVar::T_INT; /* just to avoid a warning, I affect
                                             'v' with a dummy value */
    yyval.Val = cf_op(yyvsp[0].Val, v, OP_NEG);
    ;
    break;
  }
  case 22:
...
#line 275 "cf_gramatical.yxx"
  {
    ...
    if (ok) {
      cf_value Var;
      Var.Type =
          (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i].Type;
      DEBUG_PRINTF("vart %d\n", Var.Type);
      switch (Var.Type) {
      case NLMISC::CConfigFile::CVar::T_INT:
        Var.Int = (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i]
                      .IntValues[0];
        break;
      case NLMISC::CConfigFile::CVar::T_REAL:
        Var.Real = (*((vector<NLMISC::CConfigFile::CVar> *)(YYPARSE_PARAM)))[i]
                       .RealValues[0];
        break;
...
cf_value cf_op(cf_value a, cf_value b, cf_operation op) {
  ...
  case OP_PLUS: //  ++++++++++++++++++++++++
    ...
    case NLMISC::CConfigFile::CVar::T_STRING:
      ...
      case NLMISC::CConfigFile::CVar::T_INT: {
        char str2[60];
        NLMISC::smprintf(str2, 60, "%d", b.Int);
        strcat(a.String, str2);
        break;
      }
      case NLMISC::CConfigFile::CVar::T_REAL: {
        char str2[60];
        NLMISC::smprintf(str2, 60, "%f", b.Real);
        strcat(a.String, str2);
        break;
      }
      case NLMISC::CConfigFile::CVar::T_STRING:
...
void cf_setVar(NLMISC::CConfigFile::CVar &Var, cf_value Val) {
  DEBUG_PRINTF(
      "Set var (type %d var name '%s') with new var type %d with value : ",
      Var.Type, Var.Name.c_str(), Val.Type);
  cf_print(Val);
  DEBUG_PRINTF("\n");
  Var.Root = LoadRoot;
  if (Var.Type == NLMISC::CConfigFile::CVar::T_UNKNOWN ||
    ...
    switch (Val.Type) {
    case NLMISC::CConfigFile::CVar::T_INT:
      Var.IntValues.push_back(Val.Int);
      break;
    case NLMISC::CConfigFile::CVar::T_REAL:
      Var.RealValues.push_back(Val.Real);
      break;
    case NLMISC::CConfigFile::CVar::T_STRING:
      Var.StrValues.push_back(Val.String);
      break;
    default:
      break;
    }
    ...
    case NLMISC::CConfigFile::CVar::T_INT:
      switch (Val.Type) {
      case NLMISC::CConfigFile::CVar::T_REAL:
        Var.IntValues.push_back((int)Val.Real);
        break;
      case NLMISC::CConfigFile::CVar::T_STRING: {
        int val = 0;
        NLMISC::fromString(Val.String, val);
        Var.IntValues.push_back(val);
        break;
      }
      default:
        break;
    ...
    case NLMISC::CConfigFile::CVar::T_REAL:
      switch (Val.Type) {
      case NLMISC::CConfigFile::CVar::T_INT:
        Var.RealValues.push_back((double)Val.Int);
        break;
      case NLMISC::CConfigFile::CVar::T_STRING: {
        double val = 0.0;
        NLMISC::fromString(Val.String, val);
        Var.RealValues.push_back(val);
        break;
      }
      default:
        break;
    ...
    case NLMISC::CConfigFile::CVar::T_STRING:
      switch (Val.Type) {
      case NLMISC::CConfigFile::CVar::T_INT:
        Var.StrValues.push_back(toString(Val.Int));
        break;
      case NLMISC::CConfigFile::CVar::T_REAL:
        Var.StrValues.push_back(toString(Val.Real));
        break;
      default:
        break;
      }

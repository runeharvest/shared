...
// put true if you want that the config file class check type when you call
// asFunctions (for example, check when you call asInt() that the variable is an
// int). when it's false, the function will convert to the wanted type (if he
// can)
...
int CConfigFile::CVar::asInt(int index) const {
  if (CheckType && Type != T_INT)
    throw EBadType(Name, Type, T_INT);
  switch (Type) {
  case T_STRING: {
    if (index >= (int)StrValues.size() || index < 0)
      throw EBadSize(Name, (int)StrValues.size(), index);
    int ret = 0;
    NLMISC::fromString(StrValues[index], ret);
    return ret;
  }
  case T_REAL:
    if (index >= (int)RealValues.size() || index < 0)
      throw EBadSize(Name, (int)RealValues.size(), index);
    return (int)RealValues[index];
  default:
    if (index >= (int)IntValues.size() || index < 0)
      throw EBadSize(Name, (int)IntValues.size(), index);
    return IntValues[index];
  }
}

double CConfigFile::CVar::asDouble(int index) const {
  if (CheckType && Type != T_REAL)
    throw EBadType(Name, Type, T_REAL);
  switch (Type) {
  case T_INT:
    if (index >= (int)IntValues.size() || index < 0)
      throw EBadSize(Name, (int)IntValues.size(), index);
...
std::string CConfigFile::CVar::asString(int index) const {
  if (CheckType && Type != T_STRING)
    throw EBadType(Name, Type, T_STRING);
  switch (Type) {
  case T_INT:
    if (index >= (int)IntValues.size() || index < 0)
      throw EBadSize(Name, (int)IntValues.size(), index);
...
void CConfigFile::CVar::setAsInt(int val, int index) {
  if (Type != T_INT)
    throw EBadType(Name, Type, T_INT);
  else if (index > (int)IntValues.size() || index < 0)
    throw EBadSize(Name, (int)IntValues.size(), index);
  else if (index == (int)IntValues.size())
...
void CConfigFile::CVar::setAsDouble(double val, int index) {
  if (Type != T_REAL)
    throw EBadType(Name, Type, T_REAL);
  else if (index > (int)RealValues.size() || index < 0)
    throw EBadSize(Name, (int)RealValues.size(), index);
  else if (index == (int)RealValues.size())
    RealValues.push_back(val);
  else
    RealValues[index] = val;
  Root = false;
}
...
void CConfigFile::CVar::setAsString(const std::string &val, int index) {
  if (Type != T_STRING)
    throw EBadType(Name, Type, T_STRING);
  else if (index > (int)StrValues.size() || index < 0)
    throw EBadSize(Name, (int)StrValues.size(), index);
  else if (index == (int)StrValues.size())
...
bool CConfigFile::CVar::operator==(const CVar &var) const {
  if (Type == var.Type) {
    switch (Type) {
    case T_INT:
      return IntValues == var.IntValues;
      break;
    case T_REAL:
      return RealValues == var.RealValues;
      break;
    case T_STRING:
      return StrValues == var.StrValues;
      break;
    default:
      break;
    }
...
void CConfigFile::CVar::add(const CVar &var) {
  if (Type == var.Type) {
    switch (Type) {
    case T_INT:
      IntValues.insert(IntValues.end(), var.IntValues.begin(),
                       var.IntValues.end());
      break;
    case T_REAL:
      RealValues.insert(RealValues.end(), var.RealValues.begin(),
                        var.RealValues.end());
      break;
    case T_STRING:
      StrValues.insert(StrValues.end(), var.StrValues.begin(),
                       var.StrValues.end());
      break;
    default:
      break;
...
uint CConfigFile::CVar::size() const {
  switch (Type) {
  case T_INT:
    return (uint)IntValues.size();
  case T_REAL:
    return (uint)RealValues.size();
  case T_STRING:
    return (uint)StrValues.size();
  default:
    return 0;
  }
}
...
CConfigFile::~CConfigFile() {
  ...
  vector<CConfigFile *>::iterator it =
      find((*_ConfigFiles).begin(), (*_ConfigFiles).end(), this);
...
void CConfigFile::load(const string &fileName, bool lookupPaths) {
  ...
  if (!locale || strcmp(locale, "C")) {
    nlerror("Numeric locale not defined to C, an external library possibly "
            "redefined it!");
  }
  ...
  /* 	_FileName.clear ();
      ...
      // If we find a linked config file, load it but don't overload already
     existing variable CVar *var = getVarPtr ("RootConfigFilename"); if (var)
      {
          string RootConfigFilename = var->asString();
          nlinfo ("RootConfigFilename variable found in the '%s' config file,
     parse it (%s)", fileName.c_str(), RootConfigFilename.c_str());
...
bool CConfigFile::loaded() { return !CConfigFile::FileNames.empty(); }

uint32 CConfigFile::getVarCount() { return (uint32)_Vars.size(); }
  ...
  while (!fn.empty()) {
    ...
    if (!CPath::lookup(fn, false).empty()) {
      ...
      if (!parsingOK) {
        ...
        nlwarning("CF: Parsing error in file %s line %d, look in '%s' for a "
                  "preprocessed version of the config file",
                  cf_CurrentFile, cf_CurrentLine, debugFileName.c_str());
      ...
      // reset all 'FromLocalFile' flag on created vars before reading next root
      // cfg
      for (uint i = 0; i < _Vars.size(); ++i) {
        _Vars[i].FromLocalFile = false;
      }
    } else {
      nlwarning("CF: Config file '%s' not found in the path '%s'", fn.c_str(),
    ...
    if (var) {
      ...
      if (!NLMISC::CFile::fileExists(RootConfigFilename)) {
        ...
        string path = NLMISC::CPath::standardizePath(
            NLMISC::CFile::getPath(FileNames[0]));
        RootConfigFilename = path + RootConfigFilename;
      }

      RootConfigFilename =
          NLMISC::CPath::getFullPath(RootConfigFilename, false);

      if (RootConfigFilename != fn) {
        nlinfo("CF: RootConfigFilename variable found in the '%s' config file, "
               "parse the root config file '%s'",
               fn.c_str(), RootConfigFilename.c_str());
  ...
  /*	if (filename == NULL)
      {
          ...
          {
              // if we clear all the array, we'll lost the callback on variable
     and all information
              //		_Vars.clear();
              cfrestart (NULL);
              cf_CurrentLine = 1;
              ...
              if (!parsingOK)
              {
                  nlwarning ("Parsing error in file %s line %d",
     _FileName.c_str(), cf_CurrentLine); throw EParseError (_FileName,
     cf_CurrentLine);
              }
          }
          else
          {
              nlwarning ("ConfigFile '%s' not found in the path '%s'",
     _FileName.c_str(), CPath::getCurrentPath().c_str()); throw EFileNotFound
     (_FileName);
          }
      }
      else
      {
          ...
          if (cf_ifile.open (filename))
          {
              ...
              if (!parsingOK)
              {
                  nlwarning ("Parsing error in file %s line %d", filename,
     cf_CurrentLine); throw EParseError (filename, cf_CurrentLine);
              }
          }
          else
          {
              nlwarning ("RootConfigFilename '%s' not found",
     _FileName.c_str());
          }
      }
...
CConfigFile::CVar *CConfigFile::getVarPtr(const std::string &varName) {
  ...
  for (i = 0; i < _Vars.size(); i++) {
    // the type could be T_UNKNOWN if we add a callback on this name but this
    // var is not in the config file
    if (_Vars[i].Name == varName &&
        (_Vars[i].Type != CVar::T_UNKNOWN || _Vars[i].Comp))
      return &(_Vars[i]);
  }
      ...
      if (_Vars[i].Comp) {
        ...
        case CConfigFile::CVar::T_INT: {
          for (int it = 0; it < (int)_Vars[i].IntValues.size(); it++) {
            ...
            fprintf(fp, "%d%s", _Vars[i].IntValues[it],
                    it < (int)_Vars[i].IntValues.size() - 1 ? ", " : " ");
          }
          break;
        }
        case CConfigFile::CVar::T_STRING: {
          for (int st = 0; st < (int)_Vars[i].StrValues.size(); st++) {
            ...
            fprintf(fp, "\"%s\"%s", _Vars[i].StrValues[st].c_str(),
                    st < (int)_Vars[i].StrValues.size() - 1 ? ", " : " ");
          }
          break;
        }
        case CConfigFile::CVar::T_REAL: {
          for (int rt = 0; rt < (int)_Vars[i].RealValues.size(); rt++) {
            ...
            fprintf(fp, "%.10f%s", _Vars[i].RealValues[rt],
                    rt < (int)_Vars[i].RealValues.size() - 1 ? ", " : " ");
          }
          break;
        }
        default:
        ...
        case CConfigFile::CVar::T_STRING:
          fprintf(fp, "%-20s = \"%s\";\n", _Vars[i].Name.c_str(),
                  _Vars[i].StrValues[0].c_str());
          break;
        case CConfigFile::CVar::T_REAL:
          fprintf(fp, "%-20s = %.10f;\n", _Vars[i].Name.c_str(),
                  _Vars[i].RealValues[0]);
          break;
        default:
          break;
        }
...
void CConfigFile::display() const { display(InfoLog); }

void CConfigFile::display(CLog *log) const {
  createDebug();

  log->displayRawNL("Config file %s have %d variables and %d root config file:",
                    getFilename().c_str(), _Vars.size(), FileNames.size() - 1);
  ...
  for (int i = 0; i < (int)_Vars.size(); i++) {
    ...
    if (_Vars[i].Comp) {
      ...
      default: {
        log->displayRawNL("%-20s <default case comp> (%d)",
                          _Vars[i].Name.c_str(), _Vars[i].Type);
        break;
      }
      }
    } else {
      switch (_Vars[i].Type) {
      case CConfigFile::CVar::T_INT:
        log->displayRawNL("%-20s '%d'", _Vars[i].Name.c_str(),
                          _Vars[i].IntValues[0]);
        break;
      case CConfigFile::CVar::T_STRING:
        log->displayRawNL("%-20s \"%s\"", _Vars[i].Name.c_str(),
                          _Vars[i].StrValues[0].c_str());
        break;
      case CConfigFile::CVar::T_REAL:
        log->displayRawNL("%-20s `%f`", _Vars[i].Name.c_str(),
                          _Vars[i].RealValues[0]);
        break;
      case CConfigFile::CVar::T_UNKNOWN:
        ...
        break;
      default: {
        log->displayRawNL("%-20s <default case> (%d)", _Vars[i].Name.c_str(),
                          _Vars[i].Type);
        break;
      }
      }
...
void CConfigFile::setCallback(void (*cb)()) {
  ...
  if (!FileNames.empty())
    nlinfo(
        "CF: Setting callback to reload the file '%s' when modified externally",
        getFilename().c_str());
}

void CConfigFile::setCallback(const string &VarName,
                              void (*cb)(CConfigFile::CVar &var)) {
  for (vector<CVar>::iterator it = _Vars.begin(); it != _Vars.end(); it++) {
    if (VarName == (*it).Name) {
      (*it).Callback = cb;
      // nldebug("CF: Setting callback to reload the variable '%s' in the file
      // '%s' when modified externally", VarName.c_str(),
      // getFilename().c_str());
      return;
    }
  }
  ...
  Var.Comp = false;
  _Vars.push_back(Var);
  // nldebug("CF: Setting callback to reload the variable '%s' in the file '%s'
  // when modified externally (currently unknown)", VarName.c_str(),
  // getFilename().c_str());
}
...
void CConfigFile::checkConfigFiles() {
  ...
  if (_Timeout > 0 &&
      (float)(time(NULL) - LastCheckTime) * 1000.0f < (float)_Timeout)
    return;

  LastCheckTime = time(NULL);

  bool needReparse;
  for (vector<CConfigFile *>::iterator it = (*_ConfigFiles).begin();
       it != (*_ConfigFiles).end(); it++) {
    needReparse = false;
    nlassert((*it)->FileNames.size() == (*it)->LastModified.size());
    for (uint i = 0; i < (*it)->FileNames.size(); i++) {
      if ((*it)->LastModified[i] !=
          CFile::getFileModificationDate((*it)->FileNames[i])) {
        needReparse = true;
        (*it)->LastModified[i] =
            CFile::getFileModificationDate((*it)->FileNames[i]);
      }
    ...
    if (needReparse) {
      try {
        ...
        nlwarning("CF: Exception will re-read modified config file '%s': %s",
                  (*it)->getFilename().c_str(), e.what());
      }
    }
  }
}

void CConfigFile::setTimeout(uint32 timeout) { _Timeout = timeout; }

void CConfigFile::clear() { _Vars.clear(); }
...
uint CConfigFile::getNumVar() const { return (uint)_Vars.size(); }

CConfigFile::CVar *CConfigFile::getVar(uint varId) { return &(_Vars[varId]); }

CConfigFile::CVar *CConfigFile::insertVar(const std::string &varName,

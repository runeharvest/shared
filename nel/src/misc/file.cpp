...
typedef std::list<uint64> TFileAccessTimes; // list of times at which a given
                                            // file is opened for reading
typedef CHashMap<std::string, TFileAccessTimes>
    TFileAccessLog; // map from file name to read access times
...
bool CIFile::open(const std::string &path, bool text) {
  ...
  if (IFileAccessLoggingEnabled) {
    ...
    TSynchronizedFileAccessLog::CAccessor synchronizedFileAccesLog(
        &IFileAccessLog);
    TFileAccessLog &fileAccessLog = synchronizedFileAccesLog.value();

    // add the current time to the container entry for the given path (creating
    // a new container entry if required)
  ...
  if ((pos = path.find('@')) != string::npos) {
    ...
    if (pos + 1 < path.size() && path[pos + 1] == '@') {
      ...
      if (_AllowBNPCacheFileOnOpen) {
        _F = CXMLPack::getInstance().getFile(path, _FileSize, _BigFileOffset,
                                             _CacheFileOnOpen, _AlwaysOpened);
      } else {
        bool dummy;
        _F = CXMLPack::getInstance().getFile(path, _FileSize, _BigFileOffset,
                                             dummy, _AlwaysOpened);
      }
    } else if (pos > 3 && path[pos - 3] == 's' && path[pos - 2] == 'n' &&
               path[pos - 1] == 'p') {
      ...
      _AlwaysOpened = false;
      std::string filePath;
      if (CStreamedPackageManager::getInstance().getFile(
              filePath, path.substr(pos + 1))) {
        ...
        if (_F != NULL) {
          ...
          nlwarning("Failed to open file '%s', error %u : %s", path.c_str(),
      ...
      _IsInBigFile = true;
      if (_AllowBNPCacheFileOnOpen) {
        _F = CBigFile::getInstance().getFile(path, _FileSize, _BigFileOffset,
                                             _CacheFileOnOpen, _AlwaysOpened);
      } else {
        bool dummy;
        _F = CBigFile::getInstance().getFile(path, _FileSize, _BigFileOffset,
  ...
  if (_F == NULL) {
    ...
    if (_F != NULL) {
      ...
      */
      ...
      nlwarning("Failed to open file '%s', error %u : %s", path.c_str(), errno,
                strerror(errno));
      _FileSize = 0;
    }
...
// ======================================================================================================
void CIFile::setCacheFileOnOpen(bool newState) { _CacheFileOnOpen = newState; }

// ======================================================================================================
void CIFile::setAsyncLoading(bool newState) { _IsAsyncLoading = true; }
...
// ======================================================================================================
sint32 CIFile::getPos() const { return _ReadPos; }
...
// ======================================================================================================
uint CIFile::getDbgStreamSize() const { return getFileSize(); }
...
// ======================================================================================================
COFile::COFile() : IStream(false) { _F = NULL; }

// ======================================================================================================
COFile::COFile(const std::string &path, bool append, bool text,
               bool useTempFile)
    : IStream(false) {
  _F = NULL;
...
// ======================================================================================================
COFile::~COFile() { internalClose(false); }
// ======================================================================================================
bool COFile::open(const std::string &path, bool append, bool text,
  ...
  // if appending to file and using a temporary file, copycat temporary file
  // from original...
...
// ======================================================================================================
void COFile::close() { internalClose(true); }
// ======================================================================================================
...
// ======================================================================================================
std::string COFile::getStreamName() const { return _FileName; }
...
// ======================================================================================================
NLMISC_CATEGORISED_COMMAND(nel, iFileAccessLogStart,
...
// ======================================================================================================
NLMISC_CATEGORISED_COMMAND(nel, iFileAccessLogDisplay,
  ...
  while (it != itEnd) {
    ...
    if (fileName.contains("@")) {
      log.display("%d,%s,%s,", numTimes, fileName.splitTo("@").c_str(),
                  fileName.splitFrom("@").c_str());
    } else {

...
CStopWatch::CStopWatch(uint queueLength)
    : _BeginTime(0), _ElapsedTicks(0), _SumTicks(0), _MeasurementNumber(0),
      _Queue(), _QLength(queueLength) {}
 ...
 * Resume
 */
void CStopWatch::resume() { _BeginTime = CTime::getPerformanceTime(); }

/*
 * Add time (in TTicks unit) to the current measurement
 */
void CStopWatch::addTime(TTickDuration t) { _ElapsedTicks += t; }
...
/*
 * Average of the queueLength last durations (using the queueLength argument
...
TMsDuration CStopWatch::getPartialAverage() const {
  if (_Queue.empty())
    ...
    return (TMsDuration)(CTime::ticksToSecond(
                             accumulate(_Queue.begin(), _Queue.end(), 0) /
                             _Queue.size()) *
                         1000.0);
}
...
TMsDuration CStopWatch::getAverageDuration() const {
  return (TMsDuration)(CTime::ticksToSecond(_SumTicks / _MeasurementNumber) *
                       1000.0);
}

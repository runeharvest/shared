...
CBufFIFO::CBufFIFO()
    : _Buffer(NULL), _BufferSize(0), _Empty(true), _Head(NULL), _Tail(NULL),
      _Rewinder(NULL) {
#ifdef BUFFIFO_TRACK_ALL_BUFFERS
...
void CBufFIFO::push(const std::vector<uint8> &buffer1,
                    const std::vector<uint8> &buffer2) {
  push(&buffer1[0], buffer1.size(), &buffer2[0], buffer2.size());
}

void CBufFIFO::push(const uint8 *buffer1, uint32 size1, const uint8 *buffer2,
                    uint32 size2) {
#if STAT_FIFO
  TTicks before = CTime::getPerformanceTime();
#endif
  ...
  nlassert((size1 + size2) > 0 &&
           (size1 + size2) <
               pow(2.0, static_cast<double>(sizeof(TFifoSize) * 8)));
...
#if DEBUG_FIFO
  nldebug("%p frontLast() returns %d ", this, s,
          *(tail + sizeof(TFifoSize) + size - 1));
#endif
...
void CBufFIFO::resize(uint32 s) {
  ...
  if (s < _BufferSize && UsedSize > s) {
    ...
    nlwarning("BF: Can't resize the FIFO because there's not enough room in "
              "the new wanted buffer (%d bytes needed at least)",
              UsedSize);
    return;
  }
...
void CBufFIFO::displayStats(CLog *log) {
  log->displayNL("%p CurrentQueueSize: %d, TotalQueueSize: %d", this, size(),
                 _BufferSize);
  log->displayNL("%p InQueue: %d", this, _Pushed - _Fronted);

  log->displayNL("%p BiggestBlock: %d, SmallestBlock: %d", this, _BiggestBlock,
                 _SmallestBlock);
  log->displayNL("%p BiggestBuffer: %d, SmallestBuffer: %d", this,
                 _BiggestBuffer, _SmallestBuffer);
  log->displayNL(
      "%p Pushed: %d, PushedTime: total %" NL_I64 "d ticks, mean %f ticks",
      this, _Pushed, _PushedTime,
      (_Pushed > 0 ? (double)(sint64)_PushedTime / (double)_Pushed : 0.0));
  log->displayNL(
      "%p Fronted: %d, FrontedTime: total %" NL_I64 "d ticks, mean %f ticks",
      this, _Fronted, _FrontedTime,
      (_Fronted > 0 ? (double)(sint64)_FrontedTime / (double)_Fronted : 0.0));
  log->displayNL(
      "%p Resized: %d, ResizedTime: total %" NL_I64 "d ticks, mean %f ticks",
      this, _Resized, _ResizedTime,
      (_Resized > 0 ? (double)(sint64)_ResizedTime / (double)_Resized : 0.0));
}
...
void CBufFIFO::display() {
  ...
  smprintf(str, 1024, "%p %p (%5d %5d) %p %p %p ", this, _Buffer, _BufferSize,
           CBufFIFO::size(), _Rewinder, _Tail, _Head);
  ...
  for (i = 0; i < (sint32)_BufferSize; i += gran) {
    ...
    if (_Tail >= pos && _Tail < pos + gran) {
      ...
      strncat(str, "H", 1);
    } else if (_Rewinder != NULL && _Rewinder >= pos &&
               _Rewinder < pos + gran) {
      strncat(str, "R", 1);
    } else {
      if (strlen(str) < 1023) {
...
#ifdef BUFFIFO_TRACK_ALL_BUFFERS
NLMISC_CATEGORISED_COMMAND(misc, dumpAllBuffers, "Dump all the fifo buffer",
                           "no args") {
  log.displayNL("Dumping %u FIFO buffers :", CBufFIFO::_AllBuffers.size());

  CBufFIFO::TAllBuffers::iterator first(CBufFIFO::_AllBuffers.begin()),
      last(CBufFIFO::_AllBuffers.end());

...
gint ButtonClicked(GtkWidget *Widget, gpointer *Data) {
  ...
  CSynchronized<std::vector<CGtkDisplayer::CLabelEntry>>::CAccessor access(
      &(disp->_Labels));
      ...
      if (access.value()[i].NeedUpdate && !access.value()[i].Value.empty()) {
        ...
        if (access.value()[i].Hwnd == NULL) {
          ...
          if (access.value()[i].Value[0] == '@') {
            ...
            gtk_signal_connect(GTK_OBJECT(access.value()[i].Hwnd), "clicked",
                               GTK_SIGNAL_FUNC(ButtonClicked), (gpointer)this);
            GtkLabel *label =
                GTK_LABEL(gtk_bin_get_child(GTK_BIN(access.value()[i].Hwnd)));
            gtk_label_set_justify(label, GTK_JUSTIFY_LEFT);
            gtk_label_set_line_wrap(label, FALSE);
            gtk_widget_show(GTK_WIDGET(access.value()[i].Hwnd));
            gtk_box_pack_start(GTK_BOX(hrootbox),
                               GTK_WIDGET(access.value()[i].Hwnd), TRUE, TRUE,
                               0);
          } else {
            access.value()[i].Hwnd = gtk_label_new("");
            gtk_label_set_justify(GTK_LABEL(access.value()[i].Hwnd),
                                  GTK_JUSTIFY_LEFT);
...
gint KeyIn(GtkWidget *Widget, GdkEventKey *Event, gpointer *Data) {
  ...
  case GDK_Up:
    if (CommandHistoryPos > 0) {
      CommandHistoryPos--;
      gtk_entry_set_text(GTK_ENTRY(Widget),
                         CommandHistory[CommandHistoryPos].c_str());
    }
    break;
  case GDK_Down:
...
void updateInput() { gtk_widget_grab_focus(InputText); }
...
void CGtkDisplayer::open(std::string titleBar, bool iconified, sint x, sint y,
  ...
  /*	// Clear button
     ...
     (ButtonClear), (gpointer) this); gtk_box_pack_start (GTK_BOX (hrootbox),
  ...
  */
  ...
  gtk_signal_connect(GTK_OBJECT(OutputText), "key_press_event",
                     GTK_SIGNAL_FUNC(KeyOut), NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(InputText), "activate",
                     GTK_SIGNAL_FUNC(cbValidateCommand), NULL);
  gtk_signal_connect(GTK_OBJECT(InputText), "key_press_event",
                     GTK_SIGNAL_FUNC(KeyIn), NULL);
...
gint updateInterf(gpointer data) {
  ...
  bool Bottom =
      (Adj->value >= Adj->upper - Adj->page_size - Adj->step_increment);
  ...
  {
    CSynchronized<std::list<std::pair<uint32, std::string>>>::CAccessor access(
        &disp->_Buffer);
    ...
    for (it = access.value().begin(); it != access.value().end(); it++) {
      ...
      gtk_text_buffer_insert_with_tags(buffer, &end_iter, (*it).second.c_str(),
                                       -1, tag, NULL);
      textChanged = true;
    }
  ...
  if (Bottom && textChanged) {
    ...
    gtk_text_view_scroll_to_mark(GTK_TEXT_VIEW(OutputText), mark, 0.0, true,
                                 0.0, 1.0);
  }

...
public:
  CUpdateThread(CWindowDisplayer *disp, string windowNameEx, bool iconified,
                sint x, sint y, sint w, sint h, sint hs, sint fs,
                const std::string &fn, bool ww, CLog *log)
      : Disp(disp), WindowNameEx(windowNameEx), X(x), Y(y), W(w), H(h), HS(hs),
        Iconified(iconified), FS(fs), FN(fn), WW(ww), Log(log) {}
...
void CWindowDisplayer::create(string windowNameEx, bool iconified, sint x,
                              sint y, sint w, sint h, sint hs, sint fs,
                              const std::string &fn, bool ww, CLog *log) {
  nlassert(_Thread == NULL);
  _Thread = IThread::create(new CUpdateThread(this, windowNameEx, iconified, x,
                                              y, w, h, hs, fs, fn, ww, log));
...
std::string
CWindowDisplayer::stringifyMessage(const NLMISC::CLog::TDisplayInfo &args,
                                   const char *message, bool needSlashR) {
...
void CWindowDisplayer::doDisplay(const NLMISC::CLog::TDisplayInfo &args,
                                 const char *message) {

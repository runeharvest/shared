...
const char *SerialTypeToCStr[TBMSSerialInfo::NbSerialTypes] = {
...
CBitMemStream::CBitMemStream(bool inputStream, uint32 defaultcapacity)
    : CMemStream(inputStream, false, defaultcapacity), _FreeBits(0) {
  resetBufPos();
}
...
CBitMemStream::CBitMemStream(const CBitMemStream &other)
    : CMemStream(other), _FreeBits(other._FreeBits), _DbgInfo(other._DbgInfo) {}
...
void CBitMemStream::_serialAndLog(const char *argstr, uint32 &value,
                                  uint nbits) {
  _DbgInfo.setSymbolOfNextSerialEvent(argstr);
  sint32 bitpos = getPosInBit();
  serial(value, nbits);
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: %s: %u bits at bitpos %d (%u)", this,
            isReading() ? "I" : "O", argstr, nbits, bitpos, value);
}

void CBitMemStream::_serialAndLog(const char *argstr, uint64 &value,
                                  uint nbits) {
  _DbgInfo.setSymbolOfNextSerialEvent(argstr);
  sint32 bitpos = getPosInBit();
  serial(value, nbits);
...
void CBitMemStream::_serialBitAndLog(const char *argstr, bool &bit) {
  ...
  serialBit(bit);
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: %s: 1 bit at bitpos %d (%hu)", this,
            isReading() ? "I" : "O", argstr, bitpos, (uint16)bit);
}

NLMISC_CATEGORISED_COMMAND(nel, verboseAllTraffic,
  ...
  if (args.size() == 1) {
    ...
    else if (args[0] == string("off") || args[0] == string("OFF") ||
...
void CBitMemStream::internalSerial(uint32 &value, uint nbits, bool resetvalue) {
  if (nbits == 0)
    return;
  if (nbits > 32)
    throw EMemStream(string("trying to serial ") + NLMISC::toString(nbits) +
                     string(" bits"));
  ...
  if (isReading()) {
    ...
    if (nbits > _FreeBits) {
      // nldebug( "Reading byte %u from %u free bits (%u remaining bits)",
      // lengthS(), _FreeBits, nbits );
      ...
      _FreeBits = 8;
      internalSerial(value, nbits - readbits,
                     false); // read without resetting value
    } else {
      // nlinfo( "Reading last byte %u from %u free bits (%u remaining bits)",
      // lengthS(), _FreeBits, nbits ); displayByteBits( *_BufPos, 8,
      // _FreeBits-1, false );
    ...
    if (nbits > _FreeBits) {
      ...
      // nldebug( "Writing byte %u into %u free bits (%u remaining bits)",
      // lengthS(), _FreeBits, nbits ); displayDwordBits( value, 32, nbits-1,
      // false );
      ...
      *(buffer + _Buffer.Pos) |= (v >> (nbits - _FreeBits));
      ...
      // Shorter or equal
      // nldebug( "Writing last byte %u into %u free bits (%u remaining bits)",
      // lengthS(), _FreeBits, nbits ); displayByteBits( *_BufPos, 8, 7, false
      // );
...
#ifdef LOG_ALL_TRAFFIC
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: Reserving %u bits at bitpos %d", this,
            isReading() ? "I" : "O", nbits, getPosInBit());
#endif
...
void CBitMemStream::serialPoke(uint32 value, uint nbits) {
  ...
  if (nbits > _FreeBits) {
    ...
    *(buffer + _Buffer.Pos) |= (v << (_FreeBits - nbits));
    ...
    if (_FreeBits == 0) {
      pointNextByte(); // do not set next byte to 0!
      //			nlassert( _BufPos < _Buffer.getPtr() +
      //_Buffer.size() );
      nlassert(_Buffer.Pos < _Buffer.getBuffer().size());
    }
  }
...
/*
 * Rewrite the nbbits lowest bits of a value at the specified position bitpos of
 * the current output bit stream.
 *
 * Preconditions:
...
void CBitMemStream::poke(uint32 value, uint bitpos, uint nbits) {
  ...
  // Save the current pointers of the stream, and make them point to the
  // required position
 ...
 * bit stream. The size of the bitfield is *not* written into stream (unlike
 * serialCont()). Precondition: bitpos+bitfield.size() <= the current length in
 * bit of the stream. See also reserveBits().
 */
...
#ifdef LOG_ALL_TRAFFIC
    if (VerboseAllTraffic)
      nldebug("TRAFFIC/%p/%s: Reading %u bits bitfield at bitpos %d", this,
              isReading() ? "I" : "O", len, getPosInBit());
#endif
    uint i = 0;
    uint32 v;
    while (len > 32) {
  ...
  if (isReading()) {
    ...
    if (len > length() - (uint32)getPos())
      throw NLMISC::EInvalidDataStream(
          "BMS: Trying to read a string of %u bytes, past stream size", len);
    b.resize(len);
  } else {
    len = (uint32)b.size();
    if (len > 1000000)
      throw NLMISC::EInvalidDataStream(
          "BMS: Trying to write a string of %u bytes", len);
    serial(len);
  }
...
#ifdef LOG_ALL_TRAFFIC
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: String (size 32+%u*8 bits) at bitpos %d", this,
            isReading() ? "I" : "O", len, bitpos);
#endif
}
  ...
  if (_StringMode) {
    ...
    if (isReading()) {
      ...
      if (len > (uint32)(sint32(length()) - sint32(getPos())))
        throw NLMISC::EInvalidDataStream(
            "BMS: Trying to read an ucstring of %u bytes, past stream size",
            len);
      b.resize(len);
    } else {
      len = (uint32)b.size();
      if (len > 1000000)
        throw NLMISC::EInvalidDataStream(
            "BMS: Trying to write an ucstring of %u bytes", len);
      serial(len);
    }
...
#ifdef LOG_ALL_TRAFFIC
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: Ucstring at bitpos %d", this,
            isReading() ? "I" : "O", bitpos);
#endif
}
...
void CBitMemStream::append(const CBitMemStream &newBits) {
  ...
  uint32 lastByte =
      (uint32)(*(newBits.buffer() + newBits.getPos())) >> newBits._FreeBits;
...
void CBitMemStream::serialMemStream(CMemStream &b) {
  ...
  if (isReading()) {
    ...
    if (len > length() - getPos())
      throw NLMISC::EInvalidDataStream(
          "BMS: Trying to read a BMS of %u bytes, past stream size", len);
...
#ifdef LOG_ALL_TRAFFIC
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: Sub-bitmemstream (size 32+%u*8 bits) at bitpos %d",
            this, isReading() ? "I" : "O", len, bitpos);
#endif
}
...
void CBitMemStream::serialCont(std::vector<bool> &cont) {
  ...
  if (isReading()) {
    ...
    if (len / 8 > (sint32)(length() - getPos())) {
      throw NLMISC::EInvalidDataStream(
          "BMS: Trying to read a vec<bool> of %u bytes, past stream size",
          len / 8);
    }
...
#ifdef LOG_ALL_TRAFFIC
  if (VerboseAllTraffic)
    nldebug("TRAFFIC/%p/%s: Container (header: 32 bits) at bitpos %d", this,
            isReading() ? "I" : "O", bitpos);
#endif
}
 ...
 */
void CBitMemStream::displayLastBits(sint nbits, sint bitpos,
                                    NLMISC::CLog *log) {
  if (bitpos == -1)
    bitpos = getPosInBit();
...
void displayBitStream(const CBitMemStream &msg, sint beginbitpos,
                      sint endbitpos, NLMISC::CLog *log) {
  sint beginpos = beginbitpos / 8;
  sint endpos = endbitpos / 8;
  nlinfo("BMS: beginpos %d endpos %d beginbitpos %d endbitpos %d", beginpos,
         endpos, beginbitpos, endbitpos);
  displayByteBits(*(msg.buffer() + beginpos), 8,
                  8 - (beginbitpos - beginpos * 8), true, log);
  const uint8 *p;
...
void CBitMemStream::displayStream(const char *title, CLog *log) {
  //	nlassert( (_BufPos >= _Buffer.getPtr()) && (_BufPos <= _Buffer.getPtr()
  //+ _Buffer.size()) );
  nlassert(_Buffer.Pos <= _Buffer.getBuffer().size());

  // Display title and information
  string s = (isReading() ? string("I") : string("O")) + string("BMS ") +
             string(title) + ": ";
  string sLegend;
  ...
  //	s += NLMISC::toString( "BitPos=%d Pos=%u FreeBits=%u Size=%u ",
  //getPosInBit(), (uint32)(_BufPos-_Buffer.getPtr()), _FreeBits, _Buffer.size()
  //);
  s += NLMISC::toString("BitPos=%d Pos=%u FreeBits=%u Size=%u ", getPosInBit(),
                        _Buffer.Pos, _FreeBits, _Buffer.getBuffer().size());
  ...
  uint32 bitpos = 0;
  const uint8 *p;
  //	uint8 *endPos = isReading() ? (_Buffer.getPtr() + _Buffer.size()) :
  //(_BufPos+1);
  const uint8 *endPos =
      isReading() ? (_Buffer.getBuffer().getPtr() + _Buffer.getBuffer().size())
                  : (_Buffer.getBuffer().getPtr() + _Buffer.Pos + 1);

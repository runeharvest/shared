...
void CUtfStringView::append(std::string &str, u32char c) { appendUtf8(str, c); }
...
std::string CUtfStringView::toUtf8(bool reEncode) const {
  ...
  if (m_Iterator == utf8Iterator && !reEncode)
    return std::string((const char *)m_Str,
                       (const char *)((ptrdiff_t)m_Str + m_Size));
  std::string res;
  res.reserve(m_Size);
...
ucstring CUtfStringView::toUtf16(bool reEncode) const {
  if (m_Iterator == utf16Iterator && !reEncode)
    return ucstring((const ucchar *)m_Str,
                    (const ucchar *)((ptrdiff_t)m_Str + m_Size));
  ucstring res;
...
::u32string CUtfStringView::toUtf32() const {
  ...
  if (m_Iterator == utf32Iterator)
    return ::u32string((const u32char *)m_Str,
                       (const u32char *)((ptrdiff_t)m_Str + m_Size));
  ::u32string res;
...
std::wstring CUtfStringView::toWide() const {
  ...
  if (m_Iterator == utf16Iterator)
    return std::wstring((const wchar_t *)m_Str,
                        (const wchar_t *)((ptrdiff_t)m_Str + m_Size));
  std::wstring res;
  res.reserve(m_Size << 1);
  for (iterator it(begin()), end(this->end()); it != end; ++it) {
    u32char c = *it;
...
u32char CUtfStringView::utf8Iterator(const void **addr) {
  // Decode UTF-8
  // This implementation makes no attempt at fixing bad encoding, except for bad
  // UTF-16 surrogate pairs Invalid characters are replaced with the replacement
  // character
          ...
          if (c0 & 0x10000) {
            ...
            if ((cx & 0xC0) == 0x80) {
              ...
              return 0xFFFD;
                     ...
                     0xD800) // Higher bits of nutcase UTF-16 encoded as UTF-8
          {
            uint8 cy;
            if ((*pp)[0] == 0xED && ((cx = (*pp)[1]) & 0xF0) == 0xB0 &&
                ((cy = (*pp)[2]) & 0xC0) == 0x80) {
              ...
              return 0xFFFD;
                     ...
                     0xDC00) // Lower bits of nutcase UTF-16 encoded as UTF-8
          {
            // Replacement character �
            return 0xFFFD;
          } else if (c0 < 0x0800) {

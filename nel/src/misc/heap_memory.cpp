...
void CHeapMemory::removeEmptySpace(CEmptySpace &space) {
  ...
  // remove from the list of EmptySpaces. NB: must fo it after all, because
  // "space" may be deleted.
...
void CHeapMemory::freeBlock(void *ptr) {
  ...
  // if next exist.
  if (itNext != _EmptySpaces.end()) {
    // If Previous is not just after allocated ptr, it means that there is some
    // allocated blocks beetween, so it is not a valid empty space to concat.
    if (itNext->second.Ptr != (uint8 *)ptr + size)
      itNext = _EmptySpaces.end();
  }
  // if previous exist.
  if (itPrevious != _EmptySpaces.end()) {
    // If Previous is not just before allocated ptr, it means that there is some
    // allocated blocks beetween, so it is not a valid empty space to concat.
    if (itPrevious->second.Ptr + itPrevious->second.Size != (uint8 *)ptr)
      itPrevious = _EmptySpaces.end();
  }

  // According to configuration, build the new empty space, mreging previous and
  // next, and remove old ones.
  //==========================

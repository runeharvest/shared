...
public:
    ...
    for (uint z = 0; z < NL3D_NOISE_GRID_SIZE; z++) {
      for (uint y = 0; y < NL3D_NOISE_GRID_SIZE; y++) {
        for (uint x = 0; x < NL3D_NOISE_GRID_SIZE; x++) {
          uint id = x + (y << NL3D_NOISE_GRID_SIZE_SHIFT) +
                    (z << (NL3D_NOISE_GRID_SIZE_SHIFT * 2));
  ...
  static inline float getLevelSize(uint level) { return _Sizes[level]; }
  ...
  // **************
private:
  static uint8 _Texture3d[NL3D_NOISE_GRID_SIZE * NL3D_NOISE_GRID_SIZE *
  ...
  static inline float lookup(uint ux, uint uy, uint uz) {
    uint id = ux + (uy << NL3D_NOISE_GRID_SIZE_SHIFT) +
              (uz << (NL3D_NOISE_GRID_SIZE_SHIFT * 2));
...
uint8 CRandomGrid3D::_Texture3d[NL3D_NOISE_GRID_SIZE * NL3D_NOISE_GRID_SIZE *
                                NL3D_NOISE_GRID_SIZE];
...
float CNoiseValue::noise(const CVector &pos) const {
  ...
  turb = CRandomGrid3D::getLevelSize(0) * CRandomGrid3D::evalBiLinear(pos);
  // level 1
  turb +=
...
// ***************************************************************************
void CNoiseColorGradient::eval(const CVector &posInWorld,
                               CRGBAF &result) const {

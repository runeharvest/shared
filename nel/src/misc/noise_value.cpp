...
float CNoiseValue::noise(const CVector &pos) const
{
	...
	for (uint level = 0; level < NL3D_NOISE_LEVEL; level++)
	{
		...
		turb += CRandomGrid3D::getLevelSize(level) * CRandomGrid3D::evalBiLinear(vd + CRandomGrid3D::getLevelPhase(level));
	...
	// level 1
	turb += CRandomGrid3D::getLevelSize(1) * CRandomGrid3D::evalBiLinear(pos * 2 + CRandomGrid3D::getLevelPhase(1));
	// level 2
	turb += CRandomGrid3D::getLevelSize(2) * CRandomGrid3D::evalBiLinear(pos * 4 + CRandomGrid3D::getLevelPhase(2));
#endif

...
static jpeg_source_mgr jpgSourceManager = {NULL,
                                           0,
                                           jpgDecompressInit,
                                           jpgDecompressFill,
                                           jpgDecompressSkip,
...
uint8 CBitmap::readJPG(NLMISC::IStream &f) {
  ...
  if (cinfo.jpeg_color_space == JCS_GRAYSCALE) {
    ...
    resize(cinfo.image_width, cinfo.image_height,
           _LoadGrayscaleAsAlpha ? Alpha : Luminance);
  } else {
...
static void jpgCompressTerm(j_compress_ptr cinfo) {
  if (JPGBufferSize - cinfo->dest->free_in_buffer > 0)
    JPGStream->serialBuffer(
        (uint8 *)JPGBuffer,
        (uint)(JPGBufferSize - cinfo->dest->free_in_buffer));
}

static jpeg_destination_mgr jpgDestinationManager = {
    0, 0, jpgCompressInit, jpgCompressEmpty, jpgCompressTerm};
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    ...
    for (i = 0; i < _Width; ++i) {
      for (j = 0; j < dstChannels; ++j) {
        row_pointer[0][i * dstChannels + j] =
            (uint8)_Data[0][offset + i * srcChannels + j];
      }
    }

...
// Flag to use thread instead of coroutine primitives (i.e windows fibers or gcc
// context)
...
#endif // NL_USE_THREAD_COTASK
{
  ...
  TCoTaskData(CCoTask *task)
      : _TaskThread(NULL), _CoTask(task), _ResumeTask(false),
        _TaskHasYield(false) {}

  virtual ~TCoTaskData() {
    NL_CT_DEBUG("CoTaskData : ~TCoTaskData %p : deleting cotask data", this);
    if (_TaskThread != NULL) {
      NL_CT_DEBUG("CoTask : ~TCoTaskData (%p) waiting for thread termination",
                  this);
  ...
  static void NL_WIN_CALLBACK startFunc(void *param) {
    ...
    try {
      ...
      nlwarning("CCoTask::startFunc : the task has generated an unhandled "
                "exeption and will terminate");
      NLMISC_BREAKPOINT;
    }

    task->_Finished = true;

    NL_CT_DEBUG("CoTask : task %p finished, entering infinite yield loop "
                "(waiting destruction)",
                task);
...
CCoTask::CCoTask(uint stackSize)
    : _Started(false), _TerminationRequested(false), _Finished(false) {
...
void CCoTask::start() {
  ...
  for (;;) {
    ...
    if (!_PImpl->_TaskHasYield) {
      // not finished
      NL_CT_DEBUG("CoTask : start() task %p has not yield, leaving mutex",
                  this);
      // leave the mutex
      _PImpl->_TaskMutex.leave();
    } else {
  ...
  if (mainFiber == NULL) {
    ...
    if (mainFiber == NULL) {
      ...
      if (dw == ERROR_ALREADY_FIBER)
        nlerror(
            "ConvertThreadToFiber ERROR_ALREADY_FIBER: "
            "If you are using nel in dynamic libraries, you should have a "
            "'pure "
  ...
  _PImpl->_Fiber =
      CreateFiber(NL_TASK_STACK_SIZE, TCoTaskData::startFunc, this);
  ...
  makecontext(&_PImpl->_Ctx,
              reinterpret_cast<void (*)()>(TCoTaskData::startFunc), 1, this);
#endif
  resume();
#endif // NL_USE_THREAD_COTASK
}
...
void CCoTask::yield() {
  ...
  for (;;) {
    ...
    if (!_PImpl->_ResumeTask) {
      ...
      NL_CT_DEBUG("CoTask : yield() task %p not time to resume, leaving mutex",
                  this);
      _PImpl->_TaskMutex.leave();
      //				nlSleep(0);
    } else
...
void CCoTask::resume() {
  ...
  if (!_Started)
    ...
    for (;;) {
      ...
      if (!_PImpl->_TaskHasYield) {
        NL_CT_DEBUG(
            "CoTask : resume() task %p still not yielding, leaving mutex",
            this);
        _PImpl->_TaskMutex.leave();
...
void TCoTaskData::run() {
  ...
  _TaskMutex.enter();
  NL_CT_DEBUG("CoTask : TCoTaskData::run mutex aquired, calling "
              "'_CoTask->run()' for task %p",
              _CoTask);
...
void CCoTask::requestTerminate() { _TerminationRequested = true; }
...
void CCoTask::sleep(uint milliseconds) {
  ...
  while (!isTerminationRequested()) {
    ...
    if (currTime - startTime >= milliseconds)
      break;
    yield();
  }
}

void CCoTask::releaseInstance() { CCurrentCoTask::releaseInstance(); }

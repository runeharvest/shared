...
bool CCPUTimeStat::getCPUTicks(uint64 &user, uint64 &nice, uint64 &system,
  ...
  if (fscanf(f,
             "cpu  %" NL_I64 "u %" NL_I64 "u %" NL_I64 "u %" NL_I64 "u %" NL_I64
             "u",
...
bool CCPUTimeStat::getPIDTicks(uint64 &utime, uint64 &stime, uint64 &cutime,
  ...
  // /proc/<pid>/stat
  // pid com sta ppi pgi ses tty tpg fla mif maf cmi cma [utime]    [stime]
  // [cutime]   [cstime] ...
  if (fscanf(f,
             "%*d %*s %*c %*d %*d %*d %*d %*d %*u %*u %*u %*u %*u %" NL_I64
             "u %" NL_I64 "u %" NL_I64 "u %" NL_I64 "u",
...
CCPUTimeStat::CCPUTimeStat()
    : _PID(0), _FirstTime(true), _LastCPUTicks(0), _LastPIDTicks(0) {
#ifdef NL_OS_UNIX
...
void CCPUTimeStat::peekMeasures() {
  ...
  if (getCPUTicks(u, n, s, i, io) && getPIDTicks(ut, st, cut, cst, _PID)) {
    ...
    if (!_FirstTime) {
      uint32 cpuTotal = _CPUUser.Diff + _CPUNice.Diff + _CPUSystem.Diff +
                        _CPUIdle.Diff + _CPUIOWait.Diff;
      ...
      uint32 pidTotal =
          _PIDUTime.Diff + _PIDSTime.Diff + _PIDCUTime.Diff + _PIDCSTime.Diff;

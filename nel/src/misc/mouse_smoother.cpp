...
static inline void BuildHermiteVector(const NLMISC::CVector2f &P0,
                                      ...
                                      NLMISC::CVector2f &dest, float lambda) {
...
// *******************************************************************************************
NLMISC::CVector2f CMouseSmoother::samplePos(const CVector2f &wantedPos,
                                            double date) {
  if (!_Init) {
    _Sample[0] = _Sample[1] = _Sample[2] = _Sample[3] =
        CSample(date, wantedPos);
    _Init = true;
  } else {
  ...
  if (_Sample[2].Date != _Sample[0].Date) {
    t0 = (float)dt * (_Sample[2].Pos - _Sample[0].Pos) /
         (float)(_Sample[2].Date - _Sample[0].Date);
  } else {
    t0 = NLMISC::CVector::Null;
  }
  CVector2f t1;
  if (_Sample[3].Date != _Sample[1].Date) {
    t1 = (float)dt * (_Sample[3].Pos - _Sample[1].Pos) /
         (float)(_Sample[3].Date - _Sample[1].Date);
  } else {
  ...
  if (dt == 0)
    return _Sample[2].Pos;
  BuildHermiteVector(_Sample[1].Pos, _Sample[2].Pos, t0, t1, result,
                     (float)((evalDate - _Sample[1].Date) / dt));
  return result;
}

// *******************************************************************************************
void CMouseSmoother::reset() { _Init = false; }

...
//-----------------------------------------------
static /*inline*/ void
addNode(ICDBNode *newNode, std::string newName, CCDBNodeBranch *parent,
        std::vector<ICDBNode *> &nodes, std::vector<ICDBNode *> &nodesSorted,
        xmlNodePtr &child, const string &bankName, bool atomBranch,
        bool clientOnly, IProgressCallback &progressCallBack, bool mapBanks,
        CCDBBankHandler *bankHandler = NULL) {
  ...
  if (mapBanks && (parent->getParent() == NULL)) {
    if (!bankName.empty()) {
      ...
      // nldebug( "CDB: Mapping %s for %s (node %u)", newName.c_str(),
      // bankName.c_str(), nodes.size()-1 );
    } else {
...
void CCDBNodeBranch::init(xmlNodePtr node, IProgressCallback &progressCallBack,
                          bool mapBanks, CCDBBankHandler *bankHandler) {
  xmlNodePtr child;

  _Sorted = false;
  // look for other branches within this branch
  uint countNode =
      CIXml::countChildren(node, "branch") + CIXml::countChildren(node, "leaf");
  uint nodeId = 0;
  for (child = CIXml::getFirstChildNode(node, "branch"); child;
       child = CIXml::getNextChildNode(child, "branch")) {
    // Progress bar
    progressCallBack.progress((float)nodeId / (float)countNode);
    progressCallBack.pushCropedValues((float)nodeId / (float)countNode,
                                      (float)(nodeId + 1) / (float)countNode);
    ...
    CXMLAutoPtr clientonly(
        (const char *)xmlGetProp(child, (xmlChar *)"clientonly"));

    string sBank, sAtom, sClientonly;
    if (bank)
      sBank = bank.getDatas();
    if (atom)
      sAtom = (const char *)atom;
    if (clientonly)
    ...
    if ((const char *)count != NULL) {
      ...
      for (uint i = 0; i < countAsInt; i++) {
        ...
        progressCallBack.pushCropedValues((float)i / (float)countAsInt,
                                          (float)(i + 1) / (float)countAsInt);

        //				nlinfo("+ %s%d",name,i);
        string newName = string(name.getDatas()) + toString(i);
        addNode(new CCDBNodeBranch(newName), newName, this, _Nodes,
                _NodesByName, child, sBank, sAtom == "1", sClientonly == "1",
                progressCallBack, mapBanks, bankHandler);
        //				nlinfo("-");
      ...
      addNode(new CCDBNodeBranch(newName), newName, this, _Nodes, _NodesByName,
              child, sBank, sAtom == "1", sClientonly == "1", progressCallBack,
              mapBanks, bankHandler);
      //			nlinfo("-");
    }
  ...
  for (child = CIXml::getFirstChildNode(node, "leaf"); child;
       child = CIXml::getNextChildNode(child, "leaf")) {
    // Progress bar
    progressCallBack.progress((float)nodeId / (float)countNode);
    progressCallBack.pushCropedValues((float)nodeId / (float)countNode,
                                      (float)(nodeId + 1) / (float)countNode);
    ...
    if ((const char *)count != NULL) {
      ...
      for (uint i = 0; i < countAsInt; i++) {
        ...
        //				nlinfo("  %s%d",name,i);
        string newName = string(name.getDatas()) + toString(i);
        addNode(new CCDBNodeLeaf(newName), newName, this, _Nodes, _NodesByName,
                child, sBank, false, false, progressCallBack, mapBanks,
                bankHandler);
  ...
  if ((mapBanks) && (getParent() == NULL)) {
    ...
    nlassertex(bankHandler->getUnifiedIndexToBankSize() == countNode,
               ("Mapped: %u Nodes: %u",
                bankHandler->getUnifiedIndexToBankSize(), countNode));
...
bool CCDBNodeBranch::setProp(CTextId &id, sint64 value) {
  ...
  // Property not found.
  if (pNode == NULL) {
    nlwarning("Property %s not found in %s", str.c_str(),
              id.toString().c_str());
    return false;
  }
...
/*
 * Update the database from the delta, but map the first level with the bank
 * mapping (see _CDBBankToUnifiedIndexMapping)
 */
void CCDBNodeBranch::readAndMapDelta(TGameCycle gc, CBitMemStream &s, uint bank,
  ...
  if (verboseDatabase) {
    ...
    // CInterfaceManager::getInstance()->getChatOutput()->addTextChild(
    // ucstring( displayStr ),CRGBA(255,255,255,255));
    nlinfo("CDB: %s%s %u/%d", (!getParent()) ? "[root] " : "-",
           displayStr.c_str(), idx, _IdBits);
  }
...
void CCDBNodeBranch::readDelta(TGameCycle gc, CBitMemStream &f) {
  if (isAtomic()) {
    ...
    if (!bitfield.getVector().empty()) {
      if (verboseDatabase) {
        nldebug("CDB/ATOM: Bitfield: %s LastBits:",
                bitfield.toString().c_str());
    ...
    for (uint i = 0; i != bitfield.size(); ++i) {
      if (bitfield[i]) {
        ...
        if (leaf)
          ...
          nlwarning("CDB: Can't find leaf with index %u in atom branch %s", i,
...
void CCDBNodeBranch::clear() {
  ...
  _Nodes.clear();
  _NodesByName.clear();
  // must remove all branch observers, to avoid any problem in subsequent
  // flushObserversCalls()
  removeAllBranchObserver();
} // clear //

/*
 * Find the leaf which count is specified (if found, the returned value is
 * non-null and count is 0)
 */
...
void CCDBNodeBranch::removeNode(const CTextId &id) {
  ...
  uint indexSorted;
  for (indexSorted = 0; indexSorted < pParent->_NodesByName.size();
       ++indexSorted)
...
bool CCDBNodeBranch::addObserver(IPropertyObserver *observer, CTextId &id) {
  // test if this node is the desired one, if yes, add the observer to all the
  // children nodes
  if (id.getCurrentIndex() == id.size()) {
...
bool CCDBNodeBranch::removeObserver(IPropertyObserver *observer, CTextId &id) {
  // test if this node is the desired one, if yes, remove the observer to all
  // the children nodes
  if (id.getCurrentIndex() == id.size()) {
    for (uint i = 0; i < _Nodes.size(); ++i) {
...
//-----------------------------------------------
void CCDBNodeBranch::addBranchObserver(
    ICDBDBBranchObserverHandle *handle,
...
//-----------------------------------------------
void CCDBNodeBranch::addBranchObserver(ICDBDBBranchObserverHandle *handle,
  ...
  if (dbPathFromThisNode[0] == '\0') // empty string
  {
    ...
    branchNode = safe_cast<CCDBNodeBranch *>(
        getNode(ICDBNode::CTextId(dbPathFromThisNode), false));
    if (branchNode == NULL) {
...
//-----------------------------------------------
void CCDBNodeBranch::removeBranchObserver(
    const char *dbPathFromThisNode, ICDBNode::IPropertyObserver &observer) {
  CCDBNodeBranch *branchNode = safe_cast<CCDBNodeBranch *>(
      getNode(ICDBNode::CTextId(dbPathFromThisNode), false));
  if (branchNode == NULL) {
    std::string msg = *getName();
    msg += ":";
...
ICDBNode *CCDBNodeBranch::find(const std::string &nodeName) {
  ...
  vector<ICDBNode *>::iterator it = lower_bound(
      _NodesByName.begin(), _NodesByName.end(), &tmp, CCDBNodeBranchComp());

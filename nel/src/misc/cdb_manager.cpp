...
CCDBManager::CCDBManager(const char *rootNodeName, uint maxBanks)
...
CCDBNodeLeaf *CCDBManager::getDbLeaf(const std::string &name, bool create) {
  ...
  leaf = dynamic_cast<CCDBNodeLeaf *>(
      _Database->getNode(ICDBNode::CTextId(name), create));
  return leaf;
}
...
CCDBNodeBranch *CCDBManager::getDbBranch(const std::string &name) {
  ...
  branch = dynamic_cast<CCDBNodeBranch *>(
      _Database->getNode(ICDBNode::CTextId(name), false));
  return branch;
}
...
void CCDBManager::addBranchObserver(
    const char *branchName, ICDBNode::IPropertyObserver *observer,
    const std::vector<std::string> &positiveLeafNameFilter) {
  CCDBNodeBranch *b = dynamic_cast<CCDBNodeBranch *>(
      _Database->getNode(ICDBNode::CTextId(std::string(branchName)), false));
  if (b == NULL)
    return;
...
void CCDBManager::addBranchObserver(
  ...
  if (branch == NULL)
    return;
  branchObservingHandler.addBranchObserver(branch, observer,
                                           positiveLeafNameFilter);
}

void CCDBManager::addBranchObserver(const char *branchName,
  ...
  if (b == NULL)
    return;
  branchObservingHandler.addBranchObserver(b, dbPathFromThisNode, observer,
                                           positiveLeafNameFilter,
                                           positiveLeafNameFilterSize);
}

void CCDBManager::addBranchObserver(CCDBNodeBranch *branch,
                                    const char *dbPathFromThisNode,
                                    ICDBNode::IPropertyObserver &observer,
                                    const char **positiveLeafNameFilter,
                                    uint positiveLeafNameFilterSize) {
  if (branch == NULL)
    return;
  branchObservingHandler.addBranchObserver(branch, dbPathFromThisNode, observer,
                                           positiveLeafNameFilter,
                                           positiveLeafNameFilterSize);
}

void CCDBManager::removeBranchObserver(const char *branchName,
                                       ICDBNode::IPropertyObserver *observer) {
  CCDBNodeBranch *b = dynamic_cast<CCDBNodeBranch *>(
      _Database->getNode(ICDBNode::CTextId(std::string(branchName)), false));
  if (b == NULL)
...
void CCDBManager::removeBranchObserver(CCDBNodeBranch *branch,
                                       ICDBNode::IPropertyObserver *observer) {
  if (branch == NULL)
    return;
...
void CCDBManager::removeBranchObserver(CCDBNodeBranch *branch,
  ...
  if (branch == NULL)
    return;
  branchObservingHandler.removeBranchObserver(branch, dbPathFromThisNode,
                                              observer);
}

void CCDBManager::addFlushObserver(
    CCDBBranchObservingHandler::IBranchObserverCallFlushObserver *observer) {
  if (observer == NULL)
    return;
...
void CCDBManager::resizeBanks(uint newSize) { bankHandler.resize(newSize); }

...
/// Try to match the specified text at current position. Return false of no
/// match
bool CXMLPack::matchString(string::iterator &it, string::iterator end,
                           const char *text) {
...
/// Advance up to the beginning of the next line, incrementing the in/out param
/// lineCount
void CXMLPack::skipLine(string::iterator &it, string::iterator end,
                        uint32 &lineCount) {
...
bool CXMLPack::add(const std::string &xmlPackFileName) {
  ...
  if (packIt != _XMLPacks.end()) {
    nlwarning(
        "CXMLPack::add : can't add xml_pack file '%s' because already added",
        xmlPackFileName.c_str());
    return false;
  }
  ...
  if (!matchString(it, end, "<nel:packed_xml>")) {
    nlwarning("Error : invalid pack file '%s', invalid header",
              xmlPackFileName.c_str());
    return false;
  }
  ...
  for (;;) {
    ...
    if (!matchString(it, end, "<nel:xml_file")) {
      nlwarning("Error : invalid pack file content at line %u in '%s'",
                lineCount, xmlPackFileName.c_str());
      return false;
    }

    // ok, extract the file name from the header, match 'name' then '=' then '"'
    if (!matchString(it, end, "name") || !matchString(it, end, "=") ||
        !matchString(it, end, "\"")) {
      nlwarning("Error : invalid pack file sub header at line %u in '%s', "
                "can't found attribute 'name'",
                lineCount, xmlPackFileName.c_str());
      return false;
    }
    ...
    if (it == end) {
      nlwarning("Error : invalid pack file sub header at line %u in '%s', "
                "can't found attribute closing quote for name",
                lineCount, xmlPackFileName.c_str());
      return false;
    }
    ...
    if (it == end) {
      nlwarning("Error : invalid sub file at line %u in '%s', reach end of "
                "file without closing file and pack elements",
                lineCount, xmlPackFileName.c_str());
      return false;
    }
  ...
  nldebug("XMLPack : xml_pack '%s' added to the collection with %u files",
          xmlPackFileName.c_str(), packInfo._XMLFiles.size());
  // ok, parsing ended
  return true;
}

// List all files in an xml_pack file
void CXMLPack::list(const std::string &xmlPackFileName,
  ...
  if (it != _XMLPacks.end()) {
    ...
    TXMLPackInfo::TFileList::const_iterator first(packInfo._XMLFiles.begin()),
        last(packInfo._XMLFiles.end());
...
FILE *CXMLPack::getFile(const std::string &sFileName, uint32 &rFileSize,
                        uint32 &rFileOffset, bool &rCacheFileOnOpen,
                        bool &rAlwaysOpened) {
  ...
  if (parts.size() != 2) {
    nlwarning("CXMLPack::getFile : Can't extract pack and filename from '%s', "
              "found %u part instead of 2 when spliting apart from '@@'",
              sFileName.c_str(), parts.size());
    return NULL;
  }
  ...
  if (packIt == _XMLPacks.end()) {
    nlwarning(
        "CXMLPack::getFile : Can't find xml pack file named '%s' to open '%s'",
        parts[0].c_str(), sFileName.c_str());
    return NULL;
  }

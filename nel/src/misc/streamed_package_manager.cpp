...
bool CStreamedPackageManager::loadPackage(const std::string &package) {
  ...
  std::map<std::string, CStreamedPackage>::iterator it =
      m_Packages.find(packname);
  ...
  for (CStreamedPackage::TEntries::const_iterator it(p.Entries.begin()),
       end(p.Entries.end());
       it != end; ++it) {
...
void CStreamedPackageManager::list(std::vector<std::string> &fileNames,
                                   const std::string &package) {
  // nldebug("List package '%s'", package.c_str());

  std::map<std::string, CStreamedPackage>::iterator it =
      m_Packages.find(package);
  CStreamedPackage &p = it->second;
...
bool CStreamedPackageManager::getFile(std::string &filePath,
...
bool CStreamedPackageManager::getFileSize(uint32 &fileSize,
                                          const std::string &fileName) {

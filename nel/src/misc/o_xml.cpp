...
#if defined(NL_OS_WINDOWS) && defined(NL_COMP_VC_VERSION) &&                   \
    NL_COMP_VC_VERSION >= 80
  ...
  if (_InternalStream) {
    ...
    if (_CurrentNode) {
      ...
      if (_PushBegin) {
        ...
        if (_AttribPresent) {
          // Set the attribute
          xmlSetProp(_CurrentNode, (const xmlChar *)_AttribName.c_str(),
          ...
          // * - Between xmlPushBegin() and xmlPushEnd(), before each serial,
          // you must set the attribute name with xmlSetAttrib.
          // * - Between xmlPushBegin() and xmlPushEnd(), you must serial only
      ...
      // * Check that your serial is initialy made between a xmlPushBegin and
      // xmlPushEnd calls.
      nlerror("Error, the stream don't use XML streaming properly");
    }
  } else {
...
void COXml::serial(sint8 &b) { writenumber((sint16)b, "%hd", 4); }

// ***************************************************************************

void COXml::serial(uint16 &b) { writenumber(b, "%hu", 5); }

// ***************************************************************************

void COXml::serial(sint16 &b) { writenumber(b, "%hd", 6); }

// ***************************************************************************

void COXml::serial(uint32 &b) { writenumber(b, "%u", 10); }

// ***************************************************************************

void COXml::serial(sint32 &b) { writenumber(b, "%d", 11); }

// ***************************************************************************

void COXml::serial(uint64 &b) { writenumber(b, "%" NL_I64 "u", 20); }

// ***************************************************************************

void COXml::serial(sint64 &b) { writenumber(b, "%" NL_I64 "d", 20); }

// ***************************************************************************

void COXml::serial(float &b) { writenumber((double)b, "%f", 128); }

// ***************************************************************************

void COXml::serial(double &b) { writenumber(b, "%f", 128); }

// ***************************************************************************

void COXml::serial(bool &b) { serialBit(b); }
  ...
  if (_InternalStream) {
    ...
    if (!_PushBegin) {
      ...
      if (_CurrentNode == NULL) {
        ...
        _CurrentNode = xmlNewDocNode(_Document, NULL,
                                     (const xmlChar *)nodeName.c_str(), NULL);
        ...
        // Create a new node
        _CurrentNode = xmlNewChild(_CurrentNode, NULL,
                                   (const xmlChar *)nodeName.c_str(), NULL);
      ...
      nlwarning("XML: You must close your xmlPushBegin - xmlPushEnd before "
                "calling a new xmlPushBegin.");
      return false;
    }
  } else {
...
bool COXml::xmlSetAttribInternal(const std::string &attribName) {
  ...
  if (_InternalStream) {
    ...
    if (_PushBegin) {
      ...
      nlwarning("XML: You must call xmlSetAttrib between xmlPushBegin and "
                "xmlPushEnd calls.");
      return false;
    }
  } else {
...
bool COXml::xmlCommentInternal(const std::string &comment) {
  ...
  if (_InternalStream) {
    ...
    if (_CurrentNode != NULL) {
      ...
      nlwarning("XML: You must call xmlCommentInternal between xmlPushBegin "
                "and xmlPushEnd.");
      return false;
    }
  } else {
...
void COXml::flush() {
  if (_Document) {
    ...
    xmlOutputBufferPtr outputBuffer = xmlOutputBufferCreateIO(
        xmlOutputWriteCallbackForNeL, xmlOutputCloseCallbackForNeL, this, NULL);

    // Save the file
    int res = xmlSaveFormatFileTo(outputBuffer, _Document, NULL, 1);

    // No error should be returned because, exception should be raised by the
    // internal stream
    nlassert(res != -1);

...
CSynchronized<CInterWindowMsgQueue::TMessageQueueMap>
    CInterWindowMsgQueue::_MessageQueueMap(
        "CInterWindowMsgQueue::_MessageQueueMap");
...
CInterWindowMsgQueue::CProtagonist::CProtagonist()
    : _Id(0), _Wnd(0), _SharedMemMutex(0), _SharedWndHandle(NULL) {}

// **************************************************************************************************
CInterWindowMsgQueue::CProtagonist::~CProtagonist() { release(); }
...
bool CInterWindowMsgQueue::CProtagonist::init(uint32 id) {
  ...
  _SharedMemMutex =
      CreateMutexA(NULL, FALSE, toString("NL_MUTEX_%d", (int)id).c_str());
  if (!_SharedMemMutex)
    return false;
  _Id = id;
  return true;
...
void CInterWindowMsgQueue::CProtagonist::setWnd(HWND wnd) {
  ...
  nlassert(
      _SharedWndHandle ==
      NULL); // setWnd was called before ?
             // setWnd should be called once for the 'local' window at beginning
  acquireSMMutex();
  _SharedWndHandle =
      CSharedMemory::createSharedMemory(toSharedMemId((int)_Id), sizeof(HWND));
  if (_SharedWndHandle) {
...
HWND CInterWindowMsgQueue::CProtagonist::getWnd() {
  ...
  // access shared memory just for the time of the query 	(this allow to
  // see if foreign window is still alive)
  nlassert(!_SharedWndHandle);
  HWND result = 0;
  acquireSMMutex();
...
// **************************************************************************************************
CInterWindowMsgQueue::CSendTask::CSendTask(CInterWindowMsgQueue *parent)
    : _StopAsked(false) {
  nlassert(parent);
  _Parent = parent;
}
...
void CInterWindowMsgQueue::CSendTask::run() {
  while (!_StopAsked) {
    ...
    if (targetWindow == 0) {
      // there is no receiver, so message queue has become irrelevant, so just
      // clear it
      ...
      CMemStream outMsg;
      {
        CSynchronized<TMsgList>::CAccessor outMessageQueue(
            &_Parent->_OutMessageQueue);
      ...
      if (!nestedMsgs.empty()) {
        ...
        for (;;) {
          LRESULT result = ::SendMessageA(
              targetWindow, WM_COPYDATA, (WPARAM)_Parent->_LocalWindow.getWnd(),
              (LPARAM)&cds);
          if (result)
            break;
          // retry ...
          ...
          if (!_Parent->_ForeignWindow.getWnd()) {
            nlwarning("CInterWindowMsgQueue : tried to send message, but "
                      "destination window has been closed");
            break;
          }
        }
      }
...
// **************************************************************************************************
void CInterWindowMsgQueue::CSendTask::stop() { _StopAsked = true; }
...
CInterWindowMsgQueue::CInterWindowMsgQueue()
    : _SendTask(NULL), _SendThread(NULL),
      _OutMessageQueue("CInterWindowMsgQueue::_OutMessageQueue") {}

// **************************************************************************************************
bool CInterWindowMsgQueue::init(HWND ownerWindow, uint32 localId,
                                uint32 foreignId) {
  return initInternal(NULL, ownerWindow, localId, foreignId);
}

// **************************************************************************************************
bool CInterWindowMsgQueue::init(HINSTANCE hInstance, uint32 localId,
                                uint32 foreignId) {
  return initInternal(hInstance, NULL, localId, foreignId);
}

// **************************************************************************************************
bool CInterWindowMsgQueue::initInternal(HINSTANCE hInstance, HWND ownerWindow,
                                        uint32 localId, uint32 foreignId) {
  if (!ownerWindow) {
    // see if
    nlassert(hInstance);
  ...
  nlassert(localId != foreignId);
  {
    ...
    // NB : use a 'new' instead of an automatic object here, because I got an
    // 'INTERNAL COMPILER ERROR' compiler file 'msc1.cpp', line 1794 else, this
    // is one of the way recommended by microsoft to solve the problem.
    ...
    if (messageQueueMap->value().count(msgQueueIdent)) {
      nlassert(!_DummyWindow.getWnd()); // invisible window has just been
                                        // created, it can't be in the map now!
    ...
    if (ownerWindow != _DummyWindow.getWnd()) {
      ...
      ++refCount;
      if (refCount == 1) {
        nlassert(oldWinProc != listenerProc); // first registration so the
                                              // winproc must be different
    ...
    // this way we are sure that the new win proc has been installed, and can
...
void CInterWindowMsgQueue::release() {
  if (_LocalWindow.getWnd() != 0) {
    if (IsWindow(
            _LocalWindow.getWnd())) // handle gracefully case where the window
                                    // has been destroyed before this manager
    {

      if (_LocalWindow.getWnd() != _DummyWindow.getWnd()) {
        WNDPROC currWinProc =
            (WNDPROC)GetWindowLongPtr(_LocalWindow.getWnd(), GWLP_WNDPROC);
        if (currWinProc != listenerProc) {
          nlassert(0); // IF THIS ASSERT FIRES :
                       ...
                       // Hook (performed at init) and unhook (performed here at
                       // release) should be done in reverse order ! We got no
                       // way to unhook correclty, else.
        }
      }
    }
    if (_LocalWindow.getWnd() != _DummyWindow.getWnd()) {
      ...
      if (it->second.RefCount == 0) {
        if (IsWindow(_LocalWindow.getWnd())) {
          SetWindowLongPtr(_LocalWindow.getWnd(), GWLP_WNDPROC,
                           (LONG_PTR)it->second.OldWinProc);
        }
  ...
  if (_LocalWindow.getId() != 0) {
    ...
    // is one of the way recommended by microsoft to solve the problem.
    CUniquePtr<TAccessor> messageQueueMap(new TAccessor(&_MessageQueueMap));
    TMessageQueueMap::iterator it = messageQueueMap->value().find(
        CMsgQueueIdent(_LocalWindow.getWnd(), _LocalWindow.getId(),
                       _ForeignWindow.getId()));
...
LRESULT CInterWindowMsgQueue::handleWMCopyData(HWND hwnd, COPYDATASTRUCT *cds) {
  ...
  if (cds->lpData) {
    ...
    try {
      ...
      {
        typedef CSynchronized<TMessageQueueMap>::CAccessor TAccessor;
        // NB : use a 'new' instead of an automatic object here, because I got
        // an 'INTERNAL COMPILER ERROR' compiler file 'msc1.cpp', line 1794
        // else, this is one of the way recommended by microsoft to solve the
        // problem.
        CUniquePtr<TAccessor> messageQueueMap(new TAccessor(&_MessageQueueMap));
        TMessageQueueMap::iterator it =
            messageQueueMap->value().find(CMsgQueueIdent(hwnd, toId, fromId));
        if (it != messageQueueMap->value().end()) {
          ...
          nlwarning("CInterWindowMsgQueue : Received inter window message from "
                    "'%x' to '%x', but there's no associated message queue",
                    (int)fromId, (int)toId);
        }
      }
    } catch (const EStream &) {
      nlwarning("CInterWindowMsgQueue : Bad message format in inter window "
                "communication");
    }
  } else {
...
//=====================================================
LRESULT CALLBACK CInterWindowMsgQueue::listenerProc(HWND hwnd, UINT uMsg,
                                                    WPARAM wParam,
                                                    LPARAM lParam) {
  if (uMsg == WM_COPYDATA) // WM_COPYDATA messages are sent by the other window
...
//=====================================================
LRESULT CALLBACK CInterWindowMsgQueue::invisibleWindowListenerProc(
...
// **************************************************************************************************
CInterWindowMsgQueue::~CInterWindowMsgQueue() { release(); }
...
uint CInterWindowMsgQueue::getSendQueueSize() const {
  CSynchronized<TMsgList>::CAccessor outMessageQueue(
      &const_cast<CSynchronized<TMsgList> &>(_OutMessageQueue));

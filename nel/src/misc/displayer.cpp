...
CVariable<bool> StdDisplayerColor("nel", "StdDisplayerColor",
                                  "Enable colors in std displayer", true, 0,
                                  true);

static const char *LogTypeToString[][8] = {
    {"", "ERR", "WRN", "INF", "DBG", "STT", "AST", "UKN"},
    {"", "Error", "Warning", "Information", "Debug", "Statistic", "Assert",
     "Unknown"},
    {"", "A fatal error occurs. The program must quit", "", "", "", "",
     "A failed assertion occurs", ""},
};

const char *IDisplayer::logTypeToString(CLog::TLogType logType,
                                        bool longFormat) {
...
IDisplayer::~IDisplayer() { delete _Mutex; }
...
void CStdDisplayer::doDisplay(const CLog::TDisplayInfo &args,
                              const char *message) {
  ...
  if (!consoleModeTest) {
    HANDLE handle = CreateFileA("CONOUT$", GENERIC_READ | GENERIC_WRITE,
  ...
  if (consoleMode) {
#ifndef NL_OS_WINDOWS
    // we don't use cout because sometimes, it crashs because cout isn't already
    // init, printf doesn t crash.
    if (!str.empty())
      printf("%s", str.c_str());
...
#ifdef NL_OS_WINDOWS
  // display the string in the debugger is the application is started with the
  // debugger
  if (IsDebuggerPresent()) {
    ...
    // OutputDebugString is a big shit, we can't display big string in one time,
    // we need to split
    uint32 pos = 0;
    string splited;
    for (;;) {
...
CFileDisplayer::CFileDisplayer(const std::string &filename, bool eraseLastLog,
                               const char *displayerName, bool raw)
    : IDisplayer(displayerName), _NeedHeader(true), _LastLogSizeChecked(0),
      _Raw(raw) {
  _FilePointer = (FILE *)1;
...
void CFileDisplayer::setParam(const std::string &filename, bool eraseLastLog) {
  ...
  if (filename.empty()) {
    ...
    printf(
        "CFileDisplayer::setParam(): Can't create file with empty filename\n");
    return;
  }
  ...
  if (eraseLastLog) {
    /*		ofstream ofs (filename.c_str(), ios::out | ios::trunc);
            ...
            {
                // can't do nlwarning or infinite recurs
                printf ("CFileDisplayer::setParam(): Can't open and clear the
       log file '%s'\n", filename.c_str());
            }*/
    ...
    bool fileExist = true;
    while (fileExist) {
      string fileToDelete = CFile::getPath(filename) +
...
// <FileName> <Line> : <Msg>"
void CFileDisplayer::doDisplay(const CLog::TDisplayInfo &args,
                               const char *message) {
  bool needSpace = false;
  // stringstream ss;
  string str;
  ...
  if (_FilePointer > (FILE *)1) {
    // if the file is too big (>5mb), rename it and create another one (check
    // only after 20 lines to speed up)
...
#ifndef NL_OS_WINDOWS
        if (rename(_FileName.c_str(), CFile::findNewFile(_FileName).c_str()) !=
            0)
          printf("Can't rename the log file '%s' to '%s'", _FileName.c_str(),
                 CFile::findNewFile(_FileName).c_str());
#else
        if (_wrename(nlUtf8ToWide(_FileName), nlUtf8ToWide(newFileName)) != 0)
          wprintf(L"Can't rename the log file '%s' to '%s'",
                  nlUtf8ToWide(_FileName), nlUtf8ToWide(newFileName));
#endif
        _FilePointer = (FILE *)1;
  ...
  if (_FilePointer != 0) {
    ...
    if (!args.CallstackAndLog.empty()) {
      if (fwrite(args.CallstackAndLog.c_str(), args.CallstackAndLog.size(), 1,
                 _FilePointer) != 1) {
#ifndef NL_OS_WINDOWS
...
// <FileName> <Line>: <Msg>" Log format on the screen: in debug   "<ProcessName>
// <FileName> <Line>: <Msg>"
//                           in release "<Msg>"
void CMsgBoxDisplayer::doDisplay(const CLog::TDisplayInfo &args,
                                 const char *message) {
  bool needSpace = false;
  string str;
  ...
  */
  {
    ...
    // procname is host/service_name-sid we only want the service_name to avoid
    // redondant mail
    string procname;
    ...
    subject += procname + " NeL " + toString(LogTypeToString[0][args.LogType]) +
               " " + (args.FileName ? string(args.FileName) : "") + " " +
               toString(args.Line) + " " +
               (args.FuncName ? string(args.FuncName) : "");
    ...
    if (getenv("NEL_IGNORE_ASSERT") == NULL) {
      ...
      TReportResult reportResult =
          report(args.ProcessName + " NeL " +
                     toString(logTypeToString(args.LogType, true)),
                 subject, body, filename, NL_REPORT_SYNCHRONOUS,
      ...
      case ReportAbort:
        ...
        // disable the Windows popup telling that the application aborted and
        // disable the dr watson report.

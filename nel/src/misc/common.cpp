...
void operator delete(void *p) noexcept { aligned_free(p); }
...
/*
 * Portable Sleep() function that suspends the execution of the calling thread
 * for a number of milliseconds. Note: the resolution of the timer is
 * system-dependant and may be more than 1 millisecond.
 */
void nlSleep(uint32 ms) {
  ...
  do {
    res = nanosleep(&ts,
                    &ts); // resolution: 10 ms (with common scheduling policy)
...
std::string bytesToHumanReadableUnits(uint64 bytes,
...
uint32 humanReadableToBytes(const string &str) {
  ...
  if (str[str.size() - 1] == 'B') {
    ...
    case 'k':
      res *= 1000;
      break; // kilo symbol should be a lowercase K
    case 'K':
      res *= 1000;
      break;
    case 'i': {
...
NLMISC_CATEGORISED_COMMAND(
    nel, btohr, "Convert a bytes number into an human readable number",
    "<int>") {
  ...
  log.displayNL("%s -> %s", args[0].c_str(),
                bytesToHumanReadable(args[0]).c_str());

  return true;
}

NLMISC_CATEGORISED_COMMAND(
    nel, hrtob, "Convert a human readable number into a bytes number", "<hr>") {
  nlunreferenced(rawCommandString);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
uint32 fromHumanReadable(const std::string &str) {
  ...
  case 's':
    return val; // second
  case 'n':
    return val * 60; // minutes (mn)
  case 'h':
    return val * 60 * 60; // hour
  case 'd':
    return val * 60 * 60 * 24; // day
  case 'b':                    // bytes
    switch (str[str.size() - 2]) {
    case 'k':
      return val * 1024;
...
NLMISC_CATEGORISED_COMMAND(
    nel, stohr, "Convert a second number into an human readable time",
    "<int>") {
  nlunreferenced(rawCommandString);
  nlunreferenced(quiet);
  nlunreferenced(human);
  ...
  log.displayNL("%s -> %s", args[0].c_str(),
                secondsToHumanReadable(seconds).c_str());

  return true;
}

NLMISC_CATEGORISED_COMMAND(nel, toLower, "Convert a string to lowercase",
                           "<string>") {
  nlunreferenced(args);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
NLMISC_CATEGORISED_COMMAND(nel, toUpper, "Convert a string to uppercase",
                           "<string>") {
  nlunreferenced(args);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
NLMISC_CATEGORISED_COMMAND(nel, toLowerAscii,
                           "Convert a string's ascii-characters to lowercase",
                           "<string>") {
  nlunreferenced(args);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
NLMISC_CATEGORISED_COMMAND(nel, toUpperAscii,
                           "Convert a string's ascii-characters to uppercase",
                           "<string>") {
  nlunreferenced(args);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
std::string toHexa(const uint8 &b) { return toString("%02hhx", b); }
...
std::string encodeURIComponent(const std::string &in) {
  ...
  static const std::vector<char> notEscaped(
      makeCharLookupTable("ABCDEFGHIJKLMNOPQRSTUVWXYZ"
...
std::string formatThousands(const std::string &s) {
  ...
  do {
    for (i = remaining, k = 0; i >= 0 && k < 3; --i, ++k) {
      ...
      if (i > 0 && k == 2)
        ns = separator + ns; // j > 0 means still more digits
    }
...
/*#elif defined(NL_OS_WINDOWS)
        // it doesn't work because pid != handle and i don't know how to kill a
   pid or know the real handle of another service (not -1) int res =
   TerminateProcess((HANDLE)pid, 888); nlwarning("Failed to kill '%d' err %d:
   '%s'", pid, GetLastError (), lpMsgBuf); return res != 0;
*/
#else
...
static bool createProcess(const std::string &programName,
                          const std::string &arguments, bool log,
  ...
  // a .bat file must have first parameter to NULL and use 2nd parameter to pass
  // filename
  ...
  // or 0 for a window
  BOOL res =
      CreateProcessW(sProgramName, (LPWSTR)nlUtf8ToWide(args), NULL, NULL,
                     FALSE, CREATE_DEFAULT_ERROR_MODE | CREATE_NO_WINDOW, NULL,
                     NULL /* current dir */, &si, &pi);
  ...
  if (!res) {
    if (log) {
      ...
      nlwarning("LAUNCH: Failed launched '%s' with arg '%s' err %d: '%s'",
                programName.c_str(), arguments.c_str(), lastError,
...
bool launchProgram(const std::string &programName, const std::string &arguments,
  ...
  if (!createProcess(programName, arguments, log, pi))
    return false;

  // nldebug("LAUNCH: Successful launch '%s' with arg '%s'",
  // programName.c_str(), arguments.c_str());
  CloseHandle(pi.hProcess);
  CloseHandle(pi.hThread);
  return true;
#else
  ...
  if (toLowerAscii(CFile::getExtension(programName)) == ".app") {
    ...
    if (log) {
      nlwarning("LAUNCH: Failed launched '%s' with arg '%s' return code %d",
                programName.c_str(), arguments.c_str(), res);
    }
  ...
  if (firstLaunchProgram) {
    ...
    // From "man signal":
    //------
    // According to POSIX (3.3.1.3) it is unspecified what happens when SIGCHLD
    // is set to SIG_IGN.   Here the  BSD  and  SYSV  behaviours  differ,
    // causing BSD software that sets the action for SIGCHLD to SIG_IGN to fail
    // on Linux.
    //------
    //
    // But it works fine on my GNU/Linux so I do this because it's easier :) and
    // I don't know exactly what to do to be portable.
  ...
  if (status == -1) {
    ...
    if (log)
      nlwarning("LAUNCH: Failed launched '%s' with arg '%s' err %d: '%s'",
                programName.c_str(), arguments.c_str(), errno, err);
  } else if (status == 0) {
...
bool launchProgramArray(const std::string &programName,
                        const std::vector<std::string> &arguments, bool log) {
#ifdef NL_OS_WINDOWS
  PROCESS_INFORMATION pi;

  std::string argumentsJoined = joinArguments(arguments);

  if (!createProcess(programName, argumentsJoined, log, pi))
    return false;
  ...
  if (toLowerAscii(CFile::getExtension(programName)) == "app") {
    ...
    if (log) {
      nlwarning("LAUNCH: Failed launched '%s' with arg '%s' return code %d",
                programName.c_str(), argumentsJoined.c_str(), res);
    }
  ...
  if (status == -1) {
    ...
    if (log)
      nlwarning("LAUNCH: Failed launched '%s' with arg '%s' err %d: '%s'",
                programName.c_str(), joinArguments(arguments).c_str(), errno,
                err);
  } else if (status == 0) {
    // Exec (the only allowed instruction after vfork)
    status = execvp(programName.c_str(), &argv.front());
...
sint launchProgramAndWaitForResult(const std::string &programName,
                                   const std::string &arguments, bool log) {
#ifdef NL_OS_WINDOWS
  PROCESS_INFORMATION pi;
  ...
  if (log) {
    ...
    nlwarning("LAUNCH: Failed launched '%s' with arg '%s' and error: %s",
              programName.c_str(), arguments.c_str(), error.c_str());
  }
...
std::string getCommandOutput(const std::string &command) {
  ...
  while (!feof(pipe)) {
    if (fgets(buffer, MAX_LINE_WIDTH, pipe) != NULL)
      result += buffer;
  }
...
std::string expandEnvironmentVariables(const std::string &s) {
  ...
  while (pos2 < len &&
         (pos1 = s.find_first_of("%$", pos2)) != std::string::npos) {
    ...
    while (pos2 < len && (isalnum(s[pos2]) || s[pos2] == '_'))
      ++pos2;
    ...
    if (found) {
      ...
      if (value) {
        ...
        // value not found
        found = false;
        nlwarning("Environment variable '%s' not found, won't be replaced",
                  name.c_str());
      }
    }
...
bool explodeArguments(const std::string &str, std::vector<std::string> &args) {
  ...
  do {
    ...
    std::string::difference_type length =
        (pos2 != std::string::npos) ? pos2 - pos1 : std::string::npos;
...
std::string escapeArgument(const std::string &arg) {
  ...
  return arg;
#else
  // characters to escape, only " and $ (to prevent a $something replaced by an
  ...
  while ((pos = arg.find_first_of(s_charsToEscape, lastPos)) !=
         std::string::npos) {
    // add previous part
...
/*
 * Display the bits (with 0 and 1) composing a number (uint32) (from right to
 * left)
 */
void displayDwordBits(uint32 b, uint nbits, sint beginpos, bool displayBegin,
                      NLMISC::CLog *log) {
  string s1, s2;
  sint i;
  for (i = nbits - 1; i != -1; --i) {
...
int nlfseek64(FILE *stream, sint64 offset, int origin) {
  ...
  // This code doesn't work under windows : fseek() implementation uses a signed
  // 32 bits offset. What ever we do, it can't seek more than 2 Go. For the
  // moment, i don't know if it works under linux for seek of more than 2 Go.

  nlassert((offset < SINT64_CONSTANT(2147483647)) &&
           (offset > SINT64_CONSTANT(-2147483648)));
...
NLMISC_CATEGORISED_COMMAND(
    nel, sleep, "Freeze the service for N seconds (for debug purpose)", "<N>") {
  nlunreferenced(rawCommandString);
  nlunreferenced(quiet);
  nlunreferenced(human);
...
NLMISC_CATEGORISED_COMMAND(nel, system,
                           "Execute the command line using system() function "
                           "call (wait until the end of the command)",
                           "<commandline>") {
  nlunreferenced(rawCommandString);
  nlunreferenced(quiet);
  nlunreferenced(human);
  ...
  if (error) {
    log.displayNL("Execution of '%s' failed with error code %d", cmd.c_str(),
                  error);
  } else {
...
NLMISC_CATEGORISED_COMMAND(
    nel, launchProgram,
    "Execute the command line using launcProgram() function call (launch in "
    "background task without waiting the end of the execution)",
  ...
  log.displayNL("End of Execution of '%s' with argument '%s'", cmd.c_str(),
                arg.c_str());
  return true;
}
...
static bool openDocWithExtension(const std::string &document,
                                 const std::string &ext) {
#ifdef NL_OS_WINDOWS
  // First try ShellExecute()
  HINSTANCE result = ShellExecuteW(NULL, L"open", nlUtf8ToWide(document), NULL,
                                   NULL, SW_SHOWDEFAULT);
  ...
  if ((uintptr_t)result <= HINSTANCE_ERROR) {
    ...
    if (GetRegKey(HKEY_CLASSES_ROOT, nlUtf8ToWide("." + ext), key) ==
        ERROR_SUCCESS) {
...
#elif defined(NL_OS_MAC)
  CFURLRef url =
      CFURLCreateWithBytes(NULL, (const UInt8 *)document.c_str(),
                           document.length(), kCFStringEncodingUTF8, NULL);

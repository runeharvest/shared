...
CStaticMap<CSheetId::CChar, uint32, CSheetId::CCharComp>
...
CSheetId::CSheetId(const string &sheetName) {
  if (!buildSheetId(sheetName.c_str(), sheetName.size())) {
    if (sheetName.empty())
      nlwarning("SHEETID: Try to create an CSheetId with empty name. TODO: "
                "check why.");
    else
      nlwarning("SHEETID: The sheet '%s' is not in sheet_id.bin, setting it to "
                "Unknown",
                sheetName.c_str());
    // std::string stack;
...
CSheetId::CSheetId(const std::string &sheetName,
  ...
  if (sheetName.rfind('.') == std::string::npos) {
    ...
    *this = CSheetId(withType);
    // nldebug("SHEETID: Constructing CSheetId from name '%s' without explicit
    // type, defaulting as '%s' to '%s'", sheetName.c_str(),
    // defaultType.c_str(), withType.c_str());
  } else {
...
bool CSheetId::buildSheetId(const char *sheetName, size_t sheetNameLen) {
  ...
  if (_DontHaveSheetKnowledge) {
    ...
    if (it == _DevSheetNameToId.end()) {
      ...
      _Id.IdInfos.Id = _DevSheetIdToName[typeId].size() - 1;
      // nldebug("SHEETID: Type %i, id %i, sheetid %i", _Id.IdInfos.Type,
      // _Id.IdInfos.Id, _Id.Id);
...
#pragma warning(pop)
  ...
  // we failed to find the sheet name in the sheetname map so see if the string
  // is numeric
...
void CSheetId::loadSheetId() {
  ...
  if (!path.empty() && file.open(path)) {
    ...
    if (_RemoveUnknownSheet) {
      ...
      for (itStr2 = tempMap.begin(); itStr2 != tempMap.end();) {
        if (CPath::exists((*itStr2).second)) {
          ...
          // nldebug ("Removing file '%s' from CSheetId because the file not
          // exists", (*olditStr).second.c_str ());
          itStr2++;
          tempMap.erase(olditStr);
          removednbfiles++;
        }
      }
      nlinfo("SHEETID: Removed %d files on %d from CSheetId because these "
             "files don't exist",
             removednbfiles, nbfiles);
    }
    ...
    // Convert the map to one big string and 1 static map (id to name)
    {
      ...
      // Make the big string (composed of all strings) and a vector referencing
      // each string
      tempVec.resize(nNb);
      ...
      // The vector of all small string is not needed anymore we have all the
      // info in the static map and with the pointer AllStrings referencing the
      // beginning.
    }
    ...
    // Build the invert map (Name to Id) & file extension vector
    {
      ...
      for (itStr = _SheetIdToName.begin(); itStr != _SheetIdToName.end();
           ++itStr) {
        ...
        if (_FileExtensions[type].empty()) {
          ...
          _FileExtensions[type] =
              toLowerAscii(CFile::getExtension((*itStr).second.Ptr));
        }
        nSize--;
      }
  ...
  nldebug("Finished loading sheet_id.bin: %u entries read",
          _SheetIdToName.size());
}
...
void CSheetId::init(bool removeUnknownSheet) {
  // allow multiple calls to init in case libraries depending on sheetid call
  // this init from their own
  if (_Initialised) {
    if (_DontHaveSheetKnowledge)
...
string CSheetId::toString(bool ifNotFoundUseNumericId) const {
  ...
  if (_DontHaveSheetKnowledge) {
    if (_Id.IdInfos.Type < _DevSheetIdToName.size() &&
      ...
      if (ifNotFoundUseNumericId) {
        ...
        return NLMISC::toString("<Sheet %u not found in loaded sheets>",
                                _Id.Id);
      }
    }
  }
  ...
  if (itStr != _SheetIdToName.end()) {
    ...
    // This nlwarning is commented out because the loggers are mutexed,
    // therefore you couldn't use toString() within a nlwarning().
    // nlwarning("<CSheetId::toString> The sheet %08x is not in
    // sheet_id.bin",_Id.Id);
    if (ifNotFoundUseNumericId) {
      return NLMISC::toString("#%u", _Id.Id);
    } else {
...
void CSheetId::serialString(NLMISC::IStream &f,
                            const std::string &defaultType) {
  nlassert(_Initialised);
...
void CSheetId::display() {
  ...
  for (itStr = _SheetIdToName.begin(); itStr != _SheetIdToName.end(); ++itStr) {
    ...
    nlinfo("SHEETID: (%08x %d) %s", (*itStr).first, (*itStr).first,
           (*itStr).second.Ptr);
  }

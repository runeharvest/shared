...
// If you don't want to add default displayer, put 0 instead of 1. In this case,
// you have to manage yourself displayer (in final release for example, we have
// to put 0) Alternatively, you can use --without-logging when using configure
// to set it to 0.
...
NLMISC::CVariablePtr<bool>
    _DisableNLDebug("nel", "DisableNLDebug",
                    "Disables generation and output of nldebug logs (no code "
                    "associated with the log generation is executed)",
                    &DisableNLDebug, true);
...
// ***************************************************************************
CImposterLog::CImposterLog(TAccessor accessor) : _Accessor(accessor) {}
...
CLog &CImposterLog::operator()() { return *(operator CLog *()); }
...
bool isCrashAlreadyReported() { return CrashAlreadyReported; }
void setCrashAlreadyReported(bool state) { CrashAlreadyReported = state; }

void setAssert(bool assert) { INelContext::getInstance().setNoAssert(!assert); }

void nlFatalError(const char *format, ...) {
  char *str;
  NLMISC_CONVERT_VARGS(str, format, 256 /*NLMISC::MaxCStringSize*/);

  INelContext::getInstance().setDebugNeedAssert(
      NLMISC::DefaultMsgBoxDisplayer == NULL);
...
// the default behavior is to display all in standard output and to a file named
// "log.log";
...
static void initDebug2(bool logInFile) {
  ...
  INelContext::getInstance().getDebugLog()->addDisplayer(DefaultMemDisplayer,
                                                         true);
  INelContext::getInstance().getInfoLog()->addDisplayer(DefaultMemDisplayer,
                                                        true);
  INelContext::getInstance().getWarningLog()->addDisplayer(DefaultMemDisplayer,
                                                           true);
  INelContext::getInstance().getAssertLog()->addDisplayer(DefaultMemDisplayer,
                                                          true);
  INelContext::getInstance().getErrorLog()->addDisplayer(DefaultMemDisplayer,
                                                         true);
  ...
  if (DefaultMsgBoxDisplayer) {
    INelContext::getInstance().getAssertLog()->addDisplayer(
        DefaultMsgBoxDisplayer);
    INelContext::getInstance().getErrorLog()->addDisplayer(
        DefaultMsgBoxDisplayer);
  }
...
void _assertex_stop_0(bool &ignoreNextTime, sint line, const char *file,
...
bool _assert_stop(bool &ignoreNextTime, sint line, const char *file,
                  const char *funcName, const char *exp) {
...
typedef struct _NEL_MINIDUMP_EXCEPTION_INFORMATION {
  DWORD ThreadId;
...
static void DumpMiniDump(PEXCEPTION_POINTERS excpInfo) {
  HANDLE file = CreateFileA(NL_CRASH_DUMP_FILE, GENERIC_WRITE, 0, NULL,
  ...
  if (file) {
    ...
    if (hm) {
      ...
      (HANDLE hProcess, DWORD ProcessId, HANDLE hFile,
       ...
       PNEL_MINIDUMP_EXCEPTION_INFORMATION CallbackParam) = NULL;
      *(FARPROC *)&MiniDumpWriteDump = GetProcAddress(hm, "MiniDumpWriteDump");
      if (MiniDumpWriteDump) {
        ...
        MiniDumpWriteDump(GetCurrentProcess(), GetCurrentProcessId(), file,
                          MiniDumpNormal, excpInfo ? &eInfo : NULL, NULL, NULL);
      } else {
...
public:
  ...
  EDebug(EXCEPTION_POINTERS *pexp) : m_pexp(pexp) {
    nlassert(pexp != 0);
    createWhat();
  }
  ...
  void createWhat() {
    ...
    if (m_pexp == NULL) {
      ...
      case EXCEPTION_ACCESS_VIOLATION:
        ...
        longExc = "The thread attempted to read from or write to a virtual "
                  "address for which it does not have the appropriate access";
        ext = ", thread attempts to ";
        ext +=
            m_pexp->ExceptionRecord->ExceptionInformation[0] ? "write" : "read";
        if (m_pexp->ExceptionRecord->ExceptionInformation[1])
          ext += toString(" at 0x%X",
                          m_pexp->ExceptionRecord->ExceptionInformation[1]);
        else
          ext += " at <NULL>";
        break;
      case EXCEPTION_DATATYPE_MISALIGNMENT:
        shortExc = "Datatype Misalignment";
        longExc = "The thread attempted to read or write data that is "
                  "misaligned on hardware that does not provide alignment. For "
                  "example, 16-bit values must be aligned on 2-byte "
                  "boundaries, 32-bit values on 4-byte boundaries, and so on";
        break;
      case EXCEPTION_BREAKPOINT:
        shortExc = "Breakpoint";
        longExc = "A breakpoint was encountered";
        break;
      case EXCEPTION_SINGLE_STEP:
        shortExc = "Single Step";
        longExc = "A trace trap or other single-instruction mechanism signaled "
                  "that one instruction has been executed";
        break;
      case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
        shortExc = "Array Bounds Exceeded";
        longExc =
            "The thread attempted to access an array element that is out of "
            "bounds, and the underlying hardware supports bounds checking";
        break;
      case EXCEPTION_FLT_DENORMAL_OPERAND:
        shortExc = "Float Denormal Operand";
        longExc = "One of the operands in a floating-point operation is "
                  "denormal. A denormal value is one that is too small to "
                  "represent as a standard floating-point value";
        break;
      case EXCEPTION_FLT_DIVIDE_BY_ZERO:
        shortExc = "Float Divide By Zero";
        longExc = "The thread attempted to divide a floating-point value by a "
                  "floating-point divisor of zero";
        break;
      case EXCEPTION_FLT_INEXACT_RESULT:
        shortExc = "Float Inexact Result";
        longExc = "The result of a floating-point operation cannot be "
                  "represented exactly as a decimal fraction";
        break;
      case EXCEPTION_FLT_INVALID_OPERATION:
        shortExc = "Float Invalid Operation";
        longExc = "This exception represents any floating-point exception not "
                  "included in this list";
        break;
      case EXCEPTION_FLT_OVERFLOW:
        shortExc = "Float Overflow";
        longExc = "The exponent of a floating-point operation is greater than "
                  "the magnitude allowed by the corresponding type";
        break;
      case EXCEPTION_FLT_STACK_CHECK:
        shortExc = "Float Stack Check";
        longExc = "The stack overflowed or underflowed as the result of a "
                  "floating-point operation";
        break;
      case EXCEPTION_FLT_UNDERFLOW:
        shortExc = "Float Underflow";
        longExc = "The exponent of a floating-point operation is less than the "
                  "magnitude allowed by the corresponding type";
        break;
      case EXCEPTION_INT_DIVIDE_BY_ZERO:
        shortExc = "Integer Divide By Zero";
        longExc = "The thread attempted to divide an integer value by an "
                  "integer divisor of zero";
        break;
      case EXCEPTION_INT_OVERFLOW:
        shortExc = "Integer Overflow";
        longExc = "The result of an integer operation caused a carry out of "
                  "the most significant bit of the result";
        break;
      case EXCEPTION_PRIV_INSTRUCTION:
        shortExc = "Privileged Instruction";
        longExc = "The thread attempted to execute an instruction whose "
                  "operation is not allowed in the current machine mode";
        break;
      case EXCEPTION_IN_PAGE_ERROR:
        shortExc = "In Page Error";
        longExc =
            "The thread tried to access a page that was not present, and the "
            "system was unable to load the page. -ie. the program or memory "
            "mapped file couldn't be paged in because it isn't accessable any "
            "more. Device drivers can return this exception if something went "
            "wrong with the read (i.e hardware problems)";
        break;
      case EXCEPTION_ILLEGAL_INSTRUCTION:
        shortExc = "Illegal Instruction";
        longExc =
            "The thread tried to execute an invalid instruction -such as MMX "
            "opcodes on a non MMX system. Branching to an invalid location can "
            "cause this -something stack corruption often causes";
        break;
      case EXCEPTION_NONCONTINUABLE_EXCEPTION:
        shortExc = "Noncontinuable Exception";
        longExc = "The thread attempted to continue execution after a "
                  "noncontinuable exception occurred";
        break;
      case EXCEPTION_STACK_OVERFLOW:
        shortExc = "Stack Overflow";
        longExc = "Stack overflow. Can occur during errant recursion, or when "
                  "a function creates a particularly large array on the stack";
        break;
      case EXCEPTION_INVALID_DISPOSITION:
        shortExc = "Invalid Disposition";
        longExc = "Whatever number the exception filter returned, it wasn't a "
                  "value the OS knows about";
        break;
      case EXCEPTION_GUARD_PAGE:
        shortExc = "Guard Page";
        longExc = "Memory Allocated as PAGE_GUARD by VirtualAlloc() has been "
                  "accessed";
        break;
      case EXCEPTION_INVALID_HANDLE:
        shortExc = "Invalid Handle";
        longExc.clear();
        break;
      case CONTROL_C_EXIT:
        shortExc = "Control-C";
        longExc = "Lets the debugger know the user hit Ctrl-C. Seemingly for "
                  "console apps only";
        break;
      case STATUS_NO_MEMORY:
        shortExc = "No Memory";
        longExc =
            "Called by HeapAlloc() if you specify HEAP_GENERATE_EXCEPTIONS and "
            "there is no memory or heap corruption";
        ext = ", unable to allocate ";
        ext += toString("%d bytes",
                        m_pexp->ExceptionRecord->ExceptionInformation[0]);
        break;
      case STATUS_WAIT_0:
        shortExc = "Wait 0";
        longExc.clear();
      ...
      case STATUS_ABANDONED_WAIT_0:
        shortExc = "Abandoned Wait 0";
        longExc.clear();
        break;
      case STATUS_USER_APC:
        shortExc = "User APC";
        longExc = "A user APC was delivered to the current thread before the "
                  "specified Timeout interval expired";
        break;
      case STATUS_TIMEOUT:
        shortExc = "Timeout";
        longExc.clear();
      ...
      case STATUS_SEGMENT_NOTIFICATION:
        shortExc = "Segment Notification";
        longExc.clear();
        break;
      case STATUS_FLOAT_MULTIPLE_FAULTS:
        shortExc = "Float Multiple Faults";
        longExc.clear();
        break;
      case STATUS_FLOAT_MULTIPLE_TRAPS:
        shortExc = "Float Multiple Traps";
        longExc.clear();
        break;
#ifdef NL_COMP_VC6
      case STATUS_ILLEGAL_VLM_REFERENCE:
        shortExc = "Illegal VLM Reference";
        longExc.clear();
        break;
#endif
      case 0xE06D7363:
        shortExc = "Microsoft C++ Exception";
        longExc = "Microsoft C++ Exception";
        break; // cpp exception
      case 0xACE0ACE:
        shortExc.clear();
        longExc.clear();
      ...
      default:
        shortExc = "Unknown Exception";
        longExc = "Unknown Exception " +
                  toString("0x%X", m_pexp->ExceptionRecord->ExceptionCode);
        break;
      };
      ...
      if (m_pexp->ExceptionRecord != NULL) {
        if (m_pexp->ExceptionRecord->ExceptionAddress)
          addr =
              toString(" at 0x%X", m_pexp->ExceptionRecord->ExceptionAddress);
        else
          addr = " at <NULL>";
      }
      ...
      if (_Reason.empty()) {
        if (!shortExc.empty())
          _Reason += shortExc + " exception generated" + addr + ext + ".\n";
        if (!longExc.empty())
          _Reason += longExc + ".\n";
      }
      ...
      if (!shortExc.empty() || !longExc.empty()) {
        // yoyo: allow only to send the crash report once. Because users usually
        // click ignore, which create noise into list of bugs (once a player
        // crash, it will surely continues to do it).
        report(progname + shortExc, subject, _Reason, NL_CRASH_DUMP_FILE, true,
               !isCrashAlreadyReported(), ReportAbort);
        // TODO: Does this need to be synchronous? Why does this not handle the
        // report result?
...
/*		skipNFirst = 0;
                ...
                nlverify (SymInitialize(getProcessHandle(), NULL, FALSE) ==
   TRUE);
                ...
                {
                    SetLastError(0);
                    BOOL res = StackWalk (IMAGE_FILE_MACHINE_I386,
   getProcessHandle(), GetCurrentThread(), &callStack, m_pexp->ContextRecord,
   NULL, FunctionTableAccess, GetModuleBase, NULL);
                    ...
                    if (i >= skipNFirst)
                    {
                        ...
                        symInfo = getFuncInfo (callStack.AddrPC.Offset,
   callStack.AddrFrame.Offset); _Reason += srcInfo + ": " + symInfo + "\n";
                    }
                }
    ...
    // 			if(looping)
    // 			{
    // 				_Reason += "******* WARNING: crashed in the user
    // crash callback *******\n"; 				looping = false;
    // 			}
    // 			else
    // 			{
  ...
  string getSourceInfo(DWORD_TYPE addr) {
    ...
    // ACE: removed the next code because "SymGetLineFromAddr" is not available
    // on windows 98
    bool ok = false;
    ...
    while (!SymGetLineFromAddr (getProcessHandle(), addr - displacement,
    (DWORD*)&resdisp, &line))
    {
        if (100 == ++displacement)
        {
    ...
    */
    ...
    // change the displacement if I had to search backward to find the source
    // line.
    if (displacement)
  ...
  HANDLE getProcessHandle() {
    return CSystemInfo::isNT() ? GetCurrentProcess()
                               : (HANDLE)(uintptr_t)GetCurrentProcessId();
  }

  // return true if found
  bool findAndErase(string &str, const char *token,
  ...
  void cleanType(string &rawType, string &displayType) {
    ...
    while (findAndErase(
        rawType,
        "classbasic_string<char,classchar_traits<char>,classallocator<char>>",
        "string"))
      ;
    while (findAndErase(displayType,
                        "class basic_string<char,class char_traits<char>,class "
                        "allocator<char> >",
                        "string"))
      ;
    while (findAndErase(
        rawType,
        "classvector<char,class char_traits<char>,class allocator<char> >",
        "string"))
      ;
  }
    ...
    if (i != string::npos) {
      ...
      for (i++; i < parse.size(); i++) {
        ...
        if (stop == 0 && (parse[i] == ',' || parse[i] == ')')) {
          ...
          } else if (type == "string") // we assume a string is always passed by
                                       // reference (i.e. addr is a string**)
          {
...
#define WORKAROUND_VCPP_SYNCHRONOUS_EXCEPTION                                  \
  if (global_force_exception_flag)                                             \
    force_exception_frame();
...
#if FINAL_VERSION
  // In final version, throw EDebug to display a smart dialog box with callstack
  // & log when crashing
#pragma message("Smart crash enabled")
  DumpMiniDump(pexp);
  throw EDebug(pexp);
#else
  // In debug version, let the program crash and use a debugger (clicking
  // "Cancel") Ace: 'if' not activated because we can't debug if enabled:
  // keeping only 0xACE0ACE for nlstop... if (!TrapCrashInDebugger &&
  // IsDebuggerPresent ())
  {
...
void getCallStackAndLog(string &result, sint skipNFirst) {
  ...
  //	try
  //	{
  //		WORKAROUND_VCPP_SYNCHRONOUS_EXCEPTION // force to install a
  //exception frame
  //
  //		DWORD array[1];
  //		array[0] = skipNFirst;
...
std::string getLogDirectory() { return LogPath; }

// You should not call this, unless you know what you're trying to do (it kills
// debug/log)! Destroys debug environment, to clear up the memleak log. NeL
// context must be deleted immediately after debug destroyed, or there will be
// various issues when static destructors call nldebug etc...
void destroyDebug() {
  delete sd;
  sd = NULL;
  ...
  if (INelContext::isContextInitialised()) {
    ...
    context.setErrorLog(NULL);
    delete log;
    log = NULL;
    log = context.getWarningLog();
    context.setWarningLog(NULL);
    delete log;
    log = NULL;
    log = context.getInfoLog();
    context.setInfoLog(NULL);
    delete log;
    log = NULL;
    log = context.getDebugLog();
    context.setDebugLog(NULL);
    delete log;
    log = NULL;
    log = context.getAssertLog();
    context.setAssertLog(NULL);
    delete log;
    log = NULL;
...
void createDebug(const char *logPath, bool logInFile, bool eraseLastLog) {
  ...
  if (!INelContext::getInstance().getAlreadyCreateSharedAmongThreads()) {
    ...
    {
      // Use an environment variable to share the value among the EXE and its
      // child DLLs (otherwise there would be one distinct bool by module, and
      // the last _set_se_translator would overwrite the previous ones)
      const char *SE_TRANSLATOR_IN_MAIN_MODULE = "NEL_SE_TRANS";
      char envBuf[2];
      if (GetEnvironmentVariableA(SE_TRANSLATOR_IN_MAIN_MODULE, envBuf, 2) ==
          0) {
        _set_se_translator(exceptionTranslator);
...
TInstanceCounterData::TInstanceCounterData(const char *className)
    : _InstanceCounter(0), _DeltaCounter(0), _ClassName(className),
      _Touched(false) {
...
void CInstanceCounterManager::registerInstaceCounterLocalManager(
    CInstanceCounterLocalManager *localMgr) {
  _InstanceCounterMgrs.insert(localMgr);
}

void CInstanceCounterManager::unregisterInstaceCounterLocalManager(
    CInstanceCounterLocalManager *localMgr) {
...
std::string CInstanceCounterManager::displayCounters() const {
...

  {
    ...
    std::set<CInstanceCounterLocalManager *>::const_iterator first2(
        _InstanceCounterMgrs.begin()),
        last2(_InstanceCounterMgrs.end());
    for (; first2 != last2; ++first2) {
      // iterate over managers
      const CInstanceCounterLocalManager *mgr = *first2;
      {
        std::set<TInstanceCounterData *>::const_iterator first(
            mgr->_InstanceCounters.begin()),
            last(mgr->_InstanceCounters.end());
        for (; first != last; ++first) {
          ...
          if (counters.find(icd->_ClassName) == counters.end()) {
            // insert a new item
            counters.insert(
                make_pair(string(icd->_ClassName), TInstanceCounterData(*icd)));
          } else {
            // accumulate the counter with the existing counter
            TInstanceCounterData &icddest =
                counters.find(icd->_ClassName)->second;
  ...
  map<string, TInstanceCounterData>::iterator first(counters.begin()),
      last(counters.end());
  for (; first != last; ++first) {
    TInstanceCounterData &icd = first->second;
    ret += toString("  Class '%-20s', \t%10d instances, \t%10d delta\n",
                    icd._ClassName, icd._InstanceCounter,
...
void CInstanceCounterManager::resetDeltaCounter() {
  std::set<CInstanceCounterLocalManager *>::iterator first2(
      _InstanceCounterMgrs.begin()),
      last2(_InstanceCounterMgrs.end());
  for (; first2 != last2; ++first2) {
    // iterate over managers
    ...
    {
      std::set<TInstanceCounterData *>::iterator first(
          mgr->_InstanceCounters.begin()),
          last(mgr->_InstanceCounters.end());
      for (; first != last; ++first) {
...
uint32 CInstanceCounterManager::getInstanceCounter(
...
sint32 CInstanceCounterManager::getInstanceCounterDelta(
    const std::string &className) const {
  sint32 result = 0;
  std::set<CInstanceCounterLocalManager *>::const_iterator first2(
      _InstanceCounterMgrs.begin()),
...
void CInstanceCounterLocalManager::unregisterInstanceCounter(
    TInstanceCounterData *counter) {
...
std::string formatErrorMessage(int errorCode) {
  ...
  if (len == 0)
    return toString("FormatMessage returned error %d", getLastError());
...
NLMISC_CATEGORISED_COMMAND(nel, displayInstanceCounter,
                           "display the instance counters", "[<filter>]") {
  string className;
  if (args.size() == 1)
    className = args[0];
  if (args.size() > 1)
...
NLMISC_CATEGORISED_COMMAND(nel, resetInstanceCounterDelta,
...
NLMISC_CATEGORISED_COMMAND(nel, displayMemlog,
  ...
  if (args.empty())
    ...
    NLMISC::fromString(args[0], nbLines);
  else
    return false;
...
NLMISC_CATEGORISED_COMMAND(nel, resetFilters,
...
NLMISC_CATEGORISED_COMMAND(nel, addPositiveFilterDebug,
                           "add a positive filter on DebugLog", "<filterstr>") {
  if (args.size() != 1)
    return false;
...
NLMISC_CATEGORISED_COMMAND(nel, addNegativeFilterDebug,
                           "add a negative filter on DebugLog", "<filterstr>") {
  if (args.size() != 1)
    return false;
...
NLMISC_CATEGORISED_COMMAND(nel, removeFilterDebug,
                           "remove a filter on DebugLog", "[<filterstr>]") {
  if (args.empty())
...
NLMISC_CATEGORISED_COMMAND(nel, displayFilterDebug,
...
NLMISC_CATEGORISED_COMMAND(nel, addPositiveFilterInfo,
                           "add a positive filter on InfoLog", "<filterstr>") {
  if (args.size() != 1)
    return false;
...
NLMISC_CATEGORISED_COMMAND(nel, addNegativeFilterInfo,
                           "add a negative filter on InfoLog", "<filterstr>") {
  if (args.size() != 1)
    return false;
...
NLMISC_CATEGORISED_COMMAND(nel, removeFilterInfo, "remove a filter on InfoLog",
                           "[<filterstr>]") {
  if (args.empty())
...
NLMISC_CATEGORISED_COMMAND(nel, displayFilterInfo, "display filter on InfoLog",
                           "[d|i|w|e]") {
  if (args.size() > 1)
    return false;
  if (args.size() == 1) {
...
NLMISC_CATEGORISED_COMMAND(nel, addPositiveFilterWarning,
...
NLMISC_CATEGORISED_COMMAND(nel, addNegativeFilterWarning,
...
NLMISC_CATEGORISED_COMMAND(nel, removeFilterWarning,
                           "remove a filter on WarningLog", "[<filterstr>]") {
  if (args.empty())
...
NLMISC_CATEGORISED_COMMAND(nel, displayFilterWarning,
...
NLMISC_CATEGORISED_COMMAND(nel, writeaccess,
...
NLMISC_CATEGORISED_COMMAND(nel, readaccess,

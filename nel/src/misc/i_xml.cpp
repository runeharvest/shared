      ...
      if (_PushBegin) {
        ...
        if (_AttribPresent) {
          ...
          xmlChar *attributeValue =
              xmlGetProp(_CurrentElement, (const xmlChar *)_AttribName.c_str());
          ...
          if (attributeValue) {
            ...
            char tmp[512];
            smprintf(tmp, 512,
                     "NeL XML Syntax error in block line %d\nAttribute \"%s\" "
                     "is missing in node \"%s\"",
                     (int)_CurrentElement->line, _AttribName.c_str(),
        ...
        if (_ContentStringIndex < length) {
          ...
          if (checkSeparator) {
            ...
            while (_ContentStringIndex < length) {
              // Not a separator ?
              if ((_ContentString[_ContentStringIndex] == SEPARATOR) ||
                  (_ContentString[_ContentStringIndex] == '\n')) {
                _ContentStringIndex++;
                break;
              }
          ...
          smprintf(tmp, 512,
                   "NeL XML Syntax error in block line %d \nMissing keywords "
                   "in text child node in the node %s",
      ...
      if (!_PushBegin) {
        ...
        if (_CurrentNode == NULL) {
          ...
          if (_CurrentNode) {
            ...
            if ((_CurrentNode->type != XML_ELEMENT_NODE) ||
                ((const char *)_CurrentNode->name != nodeName)) {
              // Make an error message
              char tmp[512];
              smprintf(tmp, 512,
                       "NeL XML Syntax error : root node has the wrong name : "
                       "\"%s\" should have \"%s\"",
        ...
        if (_CurrentNode == NULL) {
          ...
          smprintf(tmp, 512,
                   "NeL XML Syntax error in block line %d \nCan't open the "
                   "node named %s in node named %s",
                   (int)_CurrentElement->line, nodeName.c_str(),
                   _CurrentElement->name);
          throw EXmlParsingError(tmp);
        }
        ...
        nlerror("You must close your xmlPushBegin - xmlPushEnd before calling "
                "a new xmlPushBegin.");
        return false;
      }
    } else {
  ...
  if (_BinaryStream) {
    ...
    if (_Parser) {
      ...
      if (_PushBegin) {
        ...
        nlerror("You must call xmlSetAttrib between xmlPushBegin and "
                "xmlPushEnd calls.");
        return false;
      }
    } else {
...
xmlNodePtr CIXml::getFirstChildNode(xmlNodePtr parent,
...
xmlNodePtr CIXml::getNextChildNode(xmlNodePtr last,
                                   const std::string &childName) {
  last = last->next;
  while (last) {
...
bool CIXml::getPropertyString(std::string &result, xmlNodePtr node,
                              const std::string &property) {
  // Get the value
  const char *value =
      (const char *)xmlGetProp(node, (xmlChar *)property.c_str());
  if (value) {
    // Active value
    result = value;
...
int CIXml::getIntProperty(xmlNodePtr node, const std::string &property,
                          int defaultValue) {
  std::string s;
...
double CIXml::getFloatProperty(xmlNodePtr node, const std::string &property,
                               float defaultValue) {
  std::string s;
...
std::string CIXml::getStringProperty(xmlNodePtr node,
                                     const std::string &property,
...
std::string CIXml::getErrorString() { return _ErrorString; }

...
// *****************************************************************************************************************
CFixedSizeAllocator::CFixedSizeAllocator(uint numBytesPerBlock,
...
CFixedSizeAllocator::~CFixedSizeAllocator() {
  if (_NumAlloc != 0) {
    ...
    if (_NumChunks > 0) {
      ...
      // delete the left chunk. This should force all the left nodes to be
      // removed from the empty list
...
#define aligned_offsetof(s, m)                                                 \
  ((offsetof(s, m) + (NL_DEFAULT_MEMORY_ALIGNMENT - 1)) &                      \
   ~(NL_DEFAULT_MEMORY_ALIGNMENT - 1))
...
void CFixedSizeAllocator::CChunk::init(CFixedSizeAllocator *alloc) {
  ...
  if (Allocator->_SpareMem) {
    ...
    Mem = (uint8 *)aligned_malloc(
        getBlockSizeWithOverhead() * alloc->getNumBlockPerChunk(),
        NL_DEFAULT_MEMORY_ALIGNMENT); // new uint8[getBlockSizeWithOverhead() *
                                      // alloc->getNumBlockPerChunk()];
  }
  //
  ...
  if (alloc->_FreeSpace) {
    alloc->_FreeSpace->Prev = &getNode(NumFreeObjs - 1).Next;
  }
...
void *CFixedSizeAllocator::CNode::unlink() {
  ...
  *Prev = Next;
  ...
  return (void *)((uintptr_t)(this) +
                  aligned_offsetof(CNode, Next)); //(void *) &Next;
}

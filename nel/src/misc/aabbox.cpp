...
bool CAABBox::intersect(const CAABBox &box) const
{
	...
	return !(mina.x > maxb.x || mina.y > maxb.y || mina.z > maxb.z || minb.x > maxa.x || minb.y > maxa.y || minb.z > maxa.z);
}
...
CAABBox CAABBox::computeAABBoxUnion(const CAABBox &b1, const CAABBox &b2)
{
	CAABBox result;
	CVector min, max;
	CVector min1 = b1.getMin(), max1 = b1.getMax(), min2 = b2.getMin(), max2 = b2.getMax();
	max.maxof(max1, max2);
	min.minof(min1, min2);

...
void CCmdArgs::addArg(const TArg &arg) { _Args.push_back(arg); }

void CCmdArgs::addArg(const std::string &shortName, const std::string &longName,
                      const std::string &helpName,
                      const std::string &helpDescription, bool onlyOnce) {
  TArg arg;
...
void CCmdArgs::addAdditionalArg(const std::string &helpName,
                                const std::string &helpDescription,
                                bool onlyOnce, bool required) {
  TArg arg;
  arg.helpName = helpName;
  arg.helpDescription = helpDescription;
  arg.onlyOnce = onlyOnce;
...
std::vector<std::string> CCmdArgs::getArg(const std::string &argName) const {
  ...
  for (uint i = 0; i < _Args.size(); ++i) {
    ...
    if (arg.shortName == argName && arg.found)
      return arg.values;
  }
...
CCmdArgs::getLongArg(const std::string &argName) const {
  // process each argument
  for (uint i = 0; i < _Args.size(); ++i) {
    const TArg &arg = _Args[i];

    // return values if long arg found
    if (arg.longName == argName && arg.found)
      return arg.values;
  }
...
bool CCmdArgs::needAdditionalArg() const {
  ...
  for (uint i = 0; i < _Args.size(); ++i) {
    ...
    if (arg.shortName.empty() && arg.longName.empty() &&
        !arg.helpName.empty() && arg.required && !arg.found)
      return true;
  }
...
bool CCmdArgs::haveAdditionalArg(const std::string &name) const {
  ...
  for (uint i = 0; i < _Args.size(); ++i) {
    ...
    // they don't have any short or long name, but need a name in help
    if (arg.shortName.empty() && arg.longName.empty() &&
        !arg.helpName.empty() && arg.helpName == name && arg.found)
      return true;
  }
...
std::vector<std::string>
CCmdArgs::getAdditionalArg(const std::string &name) const {
  // process each argument
  for (uint i = 0; i < _Args.size(); ++i) {
    const TArg &arg = _Args[i];
    ...
    // they don't have any short or long name, but need a name in help
    if (arg.shortName.empty() && arg.longName.empty() &&
        !arg.helpName.empty() && arg.helpName == name)
      return arg.values;
  }
...
bool CCmdArgs::parse(const std::vector<std::string> &argv) {
  ...
  _ProgramPath = CPath::makePathAbsolute(
      CPath::standardizePath(CFile::getPath(argv.front())),
  ...
  for (uint i = 1; i < argc; i++) {
...
#endif
    {
      ...
      for (uint j = 0; j < _Args.size(); ++j) {
        ...
        if ((useLongName && name != arg.longName) ||
            (!useLongName && name != arg.shortName))
          continue;
      ...
      for (uint j = 0, len = _Args.size(); j < len; ++j) {
        ...
        // in fact, if there are more than one required arguments, all arguments
        // are added in first one to simplify
...
void CCmdArgs::displayHelp() {
  ...
  for (uint i = 0; i < _Args.size(); ++i) {
    ...
    if (!arg.shortName.empty()) {
      ...
      if (!arg.helpName.empty()) {
        syntaxes.push_back(
            toString("-%s <%s>", arg.shortName.c_str(), arg.helpName.c_str()));
      } else {
    ...
    if (!arg.longName.empty()) {
      if (!arg.helpName.empty()) {
        ...
        syntaxes.push_back(
            toString("--%s <%s>", arg.longName.c_str(), arg.helpName.c_str()));
      } else {
  ...
  for (uint i = 0; i < _Args.size(); ++i) {
    ...
    if (arg.shortName.empty() && arg.longName.empty() &&
        !arg.helpName.empty() && !arg.helpDescription.empty()) {
...
#ifdef BUILD_DATE
  printf("%s %s (built on %s)\n%s\n", _ProgramName.c_str(), _Version.c_str(),
         BUILD_DATE, COPYRIGHT);
#endif
}

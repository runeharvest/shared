...
//============================================================
sint CPlane::clipPolygonBack(const CVector in[], const CUV inUV[],
                             CVector out[], CUV outUV[], sint nIn) const {
  sint nOut = 0, s, p, i;
  if (nIn <= 2)
    return 0;
  ...
  for (i = 0; i < nIn; i++) {
    ...
    if (dp3Curr < 0) {
      if (dp3Prev >= 0) {
        float lambda = favoid0(
            (float)((double)dp3Prev / ((double)dp3Prev - (double)dp3Curr)));
...
//============================================================
CPlane CPlane::inverted() const { return CPlane(-a, -b, -c, -d); }

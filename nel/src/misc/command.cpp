...
ICommand::ICommand(const char *categoryName, const char *commandName,
                   const char *commandHelp, const char *commandArgs) {
  // self registration
  ...
  if (comm != LocalCommands->end()) {
    ...
    nlstopex(("There are 2 commands that have the same name in the project "
              "(command name '%s'), skip the second definition",
              commandName));
  } else {
...
ICommand::~ICommand() {
  ...
  for (TCommand::iterator comm = LocalCommands->begin();
       comm != LocalCommands->end(); comm++) {
    if ((*comm).second == this) {
      ...
      // Yoyo: if no nlinfo()/nlwarning() (thus no createDebug(), thus no new
      // CApplicationContext) done in the .dll, it is possible that the nel
      // context is never initialized
      if (INelContext::isContextInitialised()) {
...
void CCommandRegistry::registerCommand(ICommand *command) {
  if (_Commands.find(command->getName()) != _Commands.end()) {
    //		nlwarning("There are 2 commands that have the same name in the
    //project (command name '%s'), skip the second definition",
...
void CCommandRegistry::unregisterCommand(ICommand *command) {
  ...
  // nlwarning("CCommandRegistry::unregisterCommand : the command '%s' is not
  // registered", command->getName().c_str());
}

void CCommandRegistry::registerNamedCommandHandler(
    ICommandsHandler *handler, const std::string &className) {
  const std::string &name = handler->getCommandHandlerName();
  if (_CommandsHandlers.getB(name) != NULL) {
    nlwarning("CCommandRegistry : a commands handler with the name '%s' "
              "already exist, ignoring new candidat",
              name.c_str());
    return;
  }
  ...
  if (it == _CommandsHandlersClass.end()) {
    nlinfo("CCommandRegistry : adding commands handler for class '%s'",
           className.c_str());
  }
...
void CCommandRegistry::unregisterNamedCommandHandler(
    ICommandsHandler *handler, const std::string &className) {
  ...
  if (it != _CommandsHandlersClass.end()) {
    ...
    if (it->second.InstanceCount == 0) {
      nlinfo("CCommandRegistry : removing commands handler for class '% s'",
             className.c_str());
...
bool ICommand::execute(const std::string &commandWithArgs, CLog &log,
                       bool quiet, bool human) {
  try {
    return CCommandRegistry::getInstance().execute(commandWithArgs, log, quiet,
                                                   human);
  } catch (const exception &e) {
    log.displayNL("Command '%s' thrown an exception :",
                  commandWithArgs.c_str());
...
bool CCommandRegistry::execute(const std::string &commandWithArgs, CLog &log,
                               bool quiet, bool human) {
  if (!quiet) {
    log.displayNL("Executing command : '%s'", commandWithArgs.c_str());
  }

  // true to indicate that '"', ';' and '\' are special character sequence
  // control
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (commandWithArgs[i] != ' ' && commandWithArgs[i] != '\t' &&
          commandWithArgs[i] != '\n' && commandWithArgs[i] != '\r') {
        break;
      }
      i++;
    ...
    if (allowControlChar && commandWithArgs[i] == '\"') {
      ...
      for (;;) {
        ...
        if (commandWithArgs[i] == '\\') {
          ...
          if (i == commandWithArgs.size()) {
            if (!quiet)
              log.displayNL(
                  "Missing character after the backslash \\ character");
            return false;
          }
      ...
      for (;;) {
        ...
        if (i == commandWithArgs.size() || commandWithArgs[i] == ' ' ||
            commandWithArgs[i] == '\t' || commandWithArgs[i] == '\n' ||
            commandWithArgs[i] == '\r') {
          break;
        }
      }
    ...
    // separator
    if (i < commandWithArgs.size() && allowControlChar &&
        commandWithArgs[i] == ';') {
      // store the raw command
      if (!commands.empty() && commands.back().RawCommandString.empty())
        commands.back().RawCommandString =
            string(commandWithArgs.begin() + commandBegin,
                   commandWithArgs.begin() + i);
      firstArg = true;
      i++;
      commandBegin = i;
    }
  ...
  if (!commands.empty() && commands.back().RawCommandString.empty())
    commands.back().RawCommandString = string(
        commandWithArgs.begin() + commandBegin, commandWithArgs.begin() + i);
  ...
  for (uint u = 0; u < commands.size(); u++) {
    ...
    if (pos != string::npos) {
      ...
      if (ppch != NULL) {
        ...
        ret =
            ret && (*ppch)->execute(commands[u].RawCommandString, commandName,
                                    commands[u].CommandArgs, log, quiet, human);
      } else {
        if (!quiet)
          log.displayNL("Command '%s' : can't found object named '%s'",
                        cp.CommandName.c_str(), objectName.c_str());
      }
    } else {
      ...
      if (comm == _Commands.end()) {
        ...
        ret = false;
        if (!quiet)
          log.displayNL("Command '%s' not found, try 'help'",
                        commands[u].CommandName.c_str());
      } else {
        bool res =
            comm->second->execute(commands[u].RawCommandString,
                                  commands[u].CommandArgs, log, quiet, human);
        ret = ret & res;
        if (!res) {
          if (!quiet)
            log.displayNL("Bad command usage, try 'help %s'",
                          commands[u].CommandName.c_str());
        }
      }
    }
 ...
 * Case-sensitive. Displays the list after two calls with the same non-unique
 * completion. Completes commands used with prefixes (such as "help " for
 * example) as well.
 */
...
void CCommandRegistry::expand(std::string &commandName, NLMISC::CLog &log) {
  ...
  vector<string> matchingnames;
  {
    if (objectName.empty()) {
      ...
      for (TCommandsHandlers::TAToBMap::const_iterator it(
               _CommandsHandlers.getAToBMap().begin());
           it != _CommandsHandlers.getAToBMap().end(); ++it) {
      ...
      if (pch != NULL) {
        ...
        TCommandsHandlersClass::iterator it =
            _CommandsHandlersClass.find((*pch)->getCommandHandlerClassName());

        // list of class commands
        if (it != _CommandsHandlersClass.end()) {
          TCommandHandlerClassInfo &chci = it->second;

          for (TCommandHandlerClassInfo::TCommandsInfo::iterator it(
                   chci._Commands.begin());
               it != chci._Commands.end(); ++it) {
            string first = toLowerAscii(it->first);
            if (first.find(lowerCommandName) == 0) {
              matchingnames.push_back(it->first);
            }
  ...
  if (matchingnames.size() == 1) {
    if (_CommandsHandlers.getAToBMap().find(matchingnames.front()) !=
        _CommandsHandlers.getAToBMap().end()) {
  ...
  // Try to complete to the common part if there are several results
  {
    ...
    while (true) {
      ...
      for (imn = matchingnames.begin(); imn != matchingnames.end(); ++imn) {
        // Return common string if the next letter is not the same in all
        // matching names
        if (((*imn).size() == i) || ((letter != 0) && ((*imn)[i] != letter))) {
          ...
          if (lastCommandName == commandName) {
            ...
            for (imn2 = matchingnames.begin(); imn2 != matchingnames.end();
                 ++imn2) {
              // ss << " " << (*imn2);
              str += " " + (*imn2);
            }
        ...
        // Add the next letter to the common string if it is the same in all
        // matching names
        else if (letter == 0) {
          letter = (*imn)[i];
        }
...
NLMISC_CATEGORISED_COMMAND(nel, help,
                           "display help on a specific variable/commands or on "
  ...
  if (args.empty()) {
    ...
    for (CCommandRegistry::TCategorySet::iterator it = cr._Categories.begin();
         it != cr._Categories.end(); ++it) {
  ...
  if (args[0] == "all") {
    ...
    log.displayNL("Displaying all %d variables and commands: ",
                  cr._Commands.size());
    uint i = 0;
    for (TCommand::iterator comm = cr._Commands.begin();
         comm != cr._Commands.end(); ++comm, i++) {
      log.displayNL("%2d %-15s: %s", i, comm->first.c_str(),
                    comm->second->HelpString.c_str());
    }

    // display the class commands
    {
      CCommandRegistry::TCommandsHandlersClass::iterator first(
          cr._CommandsHandlersClass.begin()),
          last(cr._CommandsHandlersClass.end());
      for (; first != last; ++first) {
        log.displayNL("%-15s :", first->first.c_str());
        TCommandHandlerClassInfo &chci = first->second;
        {
          TCommandHandlerClassInfo::TCommandsInfo::iterator first(
              chci._Commands.begin()),
              last(chci._Commands.end());
          for (; first != last; ++first) {
            log.displayNL("  %-15s: %s", first->first.c_str(),
    ...
    // display the named instance instance
    {
      ...
      CCommandRegistry::TCommandsHandlers::TAToBMap::const_iterator first(
          cr._CommandsHandlers.getAToBMap().begin()),
          last(cr._CommandsHandlers.getAToBMap().end());
      for (; first != last; ++first) {
        log.displayNL(" %-15s: %s", first->first.c_str(),
  ...
  // treat the case where the supplied parameter is a category name
  {
    if (cr._Categories.find(args[0]) != cr._Categories.end()) {
      log.displayNL("Displaying commands and variables from category: %s",
                    args[0].c_str());
      uint i = 0;
      for (TCommand::iterator comm = cr._Commands.begin();
  ...
  // treat the case where the supplied parameter is a class name
  {
    ...
    if (cr._CommandsHandlersClass.find(className) !=
        cr._CommandsHandlersClass.end()) {
      ...
      if (className != args[0]) {
        ...
        for (; first != last; ++first) {
          if (first->first == cmdName) {
            ...
            log.displayNL("usage: <instanceName>.%s %s : %s", cmdName.c_str(),
                          first->second.CommandArgs.c_str(),
                          first->second.CommandHelp.c_str());
            //						log.displayNL("
            //%s::%-15s: %s", className.c_str(), cmdName.c_str(),
            //first->second.CommandHelp.c_str());
            return true;
          }
        }
        ...
        log.displayNL(" Here is a list of the %u named instance of this class",
                      chci.InstanceCount);
        for (CCommandRegistry::TCommandsHandlers::TAToBMap::const_iterator it =
                 cr._CommandsHandlers.getAToBMap().begin();
             it != cr._CommandsHandlers.getAToBMap().end(); ++it) {
  ...
  // treat the case where the supplied parameter is an object name
  {
    ...
    if (cr._CommandsHandlers.getB(objName) != NULL) {
      const string &className =
          (*(cr._CommandsHandlers.getB(objName)))->getCommandHandlerClassName();
      if (cr._CommandsHandlersClass.find(className) !=
          cr._CommandsHandlersClass.end()) {
        TCommandHandlerClassInfo &chci = cr._CommandsHandlersClass[className];
        if (objName != args[0]) {
          ...
          for (; first != last; ++first) {
            if (first->first == cmdName) {
              ...
              log.displayNL("usage: %s.%s %s : %s", objName.c_str(),
                            cmdName.c_str(), first->second.CommandArgs.c_str(),
                            first->second.CommandHelp.c_str());
              //							log.displayNL("
              //%s.%-15s: %s", className.c_str(), cmdName.c_str(),
              //first->second.CommandHelp.c_str());
  ...
  if (args[0].find('*') != std::string::npos ||
    ...
    log.displayNL(
        "Displaying commands, variables and objects matching wildcard: '%s'",
        args[0].c_str());
    ...
    // display the class commands that match
    {
      ...
      for (; first != last; ++first) {
        ...
        TCommandHandlerClassInfo &chci = first->second;
        {
          ...
          for (; first != last; ++first) {
            if (testWildCard(first->first, args[0])) {
              log.displayNL("  %s::%-15s: %s", className.c_str(),
                            first->first.c_str(),
                            first->second.CommandHelp.c_str());
            }
          }
  ...
  // treat the case where we're looking at help on a given command
  {
    ...
    if (cr._Commands.find(args[0]) != cr._Commands.end()) {
      ...
      log.displayNL("usage: %s %s", comm->first.c_str(),
    ...
    // look in the class commands
    {
      ...
      for (; first != last; ++first) {
        TCommandHandlerClassInfo &chci = first->second;
        {
          TCommandHandlerClassInfo::TCommandsInfo::iterator it =
              chci._Commands.find(args[0]);
          if (it != chci._Commands.end()) {
            ...
            log.displayNL("usage: %s %s", it->first.c_str(),
                          commandArgs.empty() ? ""
                                              : commandArgs.front().c_str());
  ...
  // we've failed to find a case that works so display an error message and
  // prompt the player
  log.displayNL("'%s' is not a command, category or wildcard. Type 'help' for "
                "more information",
                args[0].c_str());
  return true;
}
...
// ***************************************************************************
void ICommand::enableControlCharForCommand(const std::string &commandName,
                                           bool state) {
  CCommandRegistry::getInstance().enableControlCharForCommand(commandName,
                                                              state);
}
void CCommandRegistry::enableControlCharForCommand(
    const std::string &commandName, bool state) {
  if (state)
...
bool ICommand::isControlCharForCommandEnabled(const std::string &commandName) {
  return CCommandRegistry::getInstance().isControlCharForCommandEnabled(
      commandName);
}
bool CCommandRegistry::isControlCharForCommandEnabled(
    const std::string &commandName) {
  // true if not in the set
  return _CommandsDisablingControlChar.find(commandName) ==
         _CommandsDisablingControlChar.end();
}

ICommandsHandler::ICommandsHandler() : _ClassName(NULL) {}
...
void ICommandsHandler::registerCommandsHandler() {
  if (_ClassName == NULL) {
    ...
    CCommandRegistry::getInstance().registerNamedCommandHandler(this,
                                                                *_ClassName);
  }
}
...
void ICommandsHandler::unregisterCommandsHandler() {
  if (_ClassName != NULL) {
    CCommandRegistry::getInstance().unregisterNamedCommandHandler(this,
                                                                  *_ClassName);
    _ClassName = NULL;
  }
}

ICommandsHandler::~ICommandsHandler() { unregisterCommandsHandler(); }

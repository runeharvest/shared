...
// ***************************************************************************
bool CPolygon::toConvexPolygonsEdgeIntersect(const CVector2f &a0,
                                             const CVector2f &a1,
                                             const CVector2f &b0,
                                             const CVector2f &b1) {
  // both vertical?
  ...
  // In it ?
  return (((a0 - intersection) * (a1 - intersection) < 0) &&
          ((b0 - intersection) * (b1 - intersection) < 0));
}
...
public:
  ...
  void insert(CBSPNode2v *node) {
    ...
    if (p0Front && p1Front) {
      ...
      // New node
      CBSPNode2v *newNode =
          new CBSPNode2v(node->Plane, node->P0, newVertex, node->V0, node->V1);
  ...
  bool intersect(const CVector &p0, const CVector &p1, uint v0, uint v1) const {
    ...
    if (p0Front != p1Front)
      if ((v0 != V0) && (v0 != V1) && (v1 != V0) && (v1 != V1))
        if (CPolygon::toConvexPolygonsEdgeIntersect(
                (CVector2f)P0, (CVector2f)P1, (CVector2f)p0, (CVector2f)p1))
          return true;
...
bool CPolygon::toConvexPolygonsLeft(const std::vector<CVector> &vertex, uint a,
                                    uint b, uint c) {
  return ((vertex[b].x - vertex[a].x) * (vertex[c].y - vertex[a].y) -
          (vertex[c].x - vertex[a].x) * (vertex[b].y - vertex[a].y)) < 0;
}

// ***************************************************************************

bool CPolygon::toConvexPolygonsLeftOn(const std::vector<CVector> &vertex,
...
bool CPolygon::toConvexPolygonsInCone(const std::vector<CVector> &vertex,
                                      uint a, uint b) {
  // Prev and next
  uint a0 = a + 1;
  if (a0 == vertex.size())
  ...
  if (toConvexPolygonsLeftOn(vertex, a, a1, a0)) {
    return toConvexPolygonsLeft(vertex, a, b, a0) &&
           toConvexPolygonsLeft(vertex, b, a, a1);
  } else {
    return !(toConvexPolygonsLeft(vertex, a, b, a1) &&
             toConvexPolygonsLeft(vertex, b, a, a0));
  }
}

// ***************************************************************************

bool CPolygon::toConvexPolygonsDiagonal(const std::vector<CVector> &vertex,
                                        const CBSPNode2v &bsp, uint a, uint b) {
  ...
  // Check visibility
  if (toConvexPolygonsInCone(vertex, a, b) &&
      toConvexPolygonsInCone(vertex, b, a)) {
    // Intersection ?
...
void CPolygon::toConvexPolygonsLocalAndBSP(std::vector<CVector> &localVertices,
                                           CBSPNode2v &root,
                                           const CMatrix &basis) const {
  // Invert matrix
  ...
  for (; i < Vertices.size(); i++) {
    ...
    // Build the BSP root
    root.insert(
        new CBSPNode2v(clipPlane, localVertices[i], localVertices[j], i, j));
...
// ***************************************************************************
bool CPolygon::toConvexPolygons(std::list<CPolygon> &outputPolygons,
                                const CMatrix &basis) const {
  // Some vertices ?
  if (Vertices.size() > 2) {
    // Local vertices
    ...
    again:;
      ...
      while (last != current) {
        // Is a diagonal ?
        if ((toConvexPolygonsDiagonal(localVertices, root,
                                      *lastPreviousPrevious, *last)) &&
            (toConvexPolygonsDiagonal(localVertices, root, *currentNext,
                                      *last)) &&
      ...
      if (last == current) {
        ...
        while (current != first) {
          // Is a diagonal ?
          if ((toConvexPolygonsDiagonal(localVertices, root, *firstNextNext,
                                        *first)) &&
              (toConvexPolygonsDiagonal(localVertices, root, *lastPrevious,
                                        *first)) &&
...
bool CPolygon::chain(const std::vector<CPolygon> &other, const CMatrix &basis) {
  ...
  for (o = 0; o < other.size(); o++) {
    // Build the local array and the BSP
    other[o].toConvexPolygonsLocalAndBSP(localVerticesOther[o], rootOther[o],
                                         basis);
  }
  ...
  for (o = 0; o < other.size(); o++) {
    ...
    for (i = 0; i < thisCount; i++) {
      for (j = 0; j < otherCount; j++) {
        // Test this segement
        if (!root.intersect(localVertices[i], localVerticesOther[o][j], i,
                            0xffffffff)) {
          ...
          for (otherO = 0; otherO < other.size(); otherO++) {
            ...
            if (rootOther[otherO].intersect(
                    localVertices[i], localVerticesOther[o][j], 0xffffffff,
                    (otherO == o) ? j : 0xffffffff))
              break;
          }
          ...
          if (otherO == other.size()) {
            // Insert new vertices
            mainCopy.Vertices.insert(mainCopy.Vertices.begin() + i,
                                     2 + otherCount, CVector());
    ...
    if (i == thisCount) {
      ...
      for (otherToCheck = o + 1; otherToCheck < other.size(); otherToCheck++) {
        ...
        for (i = 0; i < otherToCheckCount; i++) {
          for (j = 0; j < otherCount; j++) {
            // Test this segement
            if (!rootOther[otherToCheck].intersect(
                    localVerticesOther[otherToCheck][i],
                    localVerticesOther[o][j], i, 0xffffffff)) {
              // Test each other polygons
              uint otherO;
              for (otherO = 0; otherO < other.size(); otherO++) {
                // Intersect ?
                if (rootOther[otherO].intersect(
                        localVerticesOther[otherToCheck][i],
                        localVerticesOther[o][j],
                        (otherToCheck == otherO) ? i : 0xffffffff,
                        (otherO == o) ? j : 0xffffffff))
                  break;
              }
              ...
              if (otherO == other.size()) {
                // Insert new vertices
                copy[otherToCheck].Vertices.insert(
                    copy[otherToCheck].Vertices.begin() + i, 2 + otherCount,
                    CVector());

                // Copy the first vertex
                copy[otherToCheck].Vertices[i] =
                    copy[otherToCheck].Vertices[i + otherCount + 2];
                ...
                for (k = 0; k < otherCount; k++) {
                  ...
                  if (index >= otherCount)
                    index -= otherCount;
                  copy[otherToCheck].Vertices[i + k + 1] =
                      copy[otherO].Vertices[index];
                }

                // Copy the last one
                copy[otherToCheck].Vertices[i + otherCount + 1] =
                    copy[otherO].Vertices[j];
                break;
              }
            }
          }
...
// ***************************************************************************
void CPolygon2D::fromPolygon(const CPolygon &src,
...
bool CPolygon2D::isConvex() {
  ...
  // we apply a dummy algo for now : check whether every vertex is in the same
  // side of every plane defined by a segment of this poly
  uint numVerts = (uint)Vertices.size();
  if (numVerts < 3)
    return true;
  CVector segStart, segEnd;
  CPlane clipPlane;
  for (TVec2fVect::const_iterator it = Vertices.begin(); it != Vertices.end();
       ++it) {
    ...
    if (n != 0) {
      clipPlane.make(
          segStart, segEnd,
          (n > 10 ? n : 10) * CVector::K +
              segStart); // make a plane, with this segment and the poly normal
      // check each other vertices against this plane
      for (TVec2fVect::const_iterator it2 = Vertices.begin();
           it2 != Vertices.end(); ++it2) {
        if (it2 != it &&
            it2 != (it + 1)) // the vertices must not be part of the test plane
...
void CPolygon2D::buildConvexHull(CPolygon2D &dest) const {
    ...
    if (k != pIndex) {
      for (l = 0; l < numVerts; ++l) {
        if (l != pIndex && l != k) {
          ...
          // CVector cp = CVector(seg1.x, seg1.y, 0) ^ CVector(seg2.x, seg2.y,
          // 0); float n = fabsf(cp.z);
          float n = seg1 * seg2;
          if (n < bestCP) {
            p1Index = l;
            p2Index = k;
  ...
  // start from the given triplet, and complete the poly until we reach the
  // first point
  pCurr = p2Index;
  pPrev = pIndex;
  ...
  for (;;) {
    ...
    for (TIndexSet::iterator it = leftIndex.begin(); it != leftIndex.end();
         ++it) {
      if (step == 0 && *it == p1Index)
        continue;
      ...
      if (i != j) {
        for (k = 0; k < numVerts; ++k) {
          if (k != i && k != j) {
            ...
            float area =
                fabsf((CVector(v0.x, v0.y, 0) ^ CVector(v1.x, v1.y, 0)).norm());
            if (area > bestArea) {
              bestArea = area;
              index0 = i;
              index1 = j;
...
static void ScanEdge(CPolygon2D::TRasterVect &outputVect, sint topY,
                     const CVector2f &v1, const CVector2f &v2,
                     bool rightEdge = true) {
  ...
  CPolygon2D::TRasterVect::iterator outputIt =
      outputVect.begin() + (ceilY1 - topY);
...
// *******************************************************************************
// This function alow to cycle forward through a vertex vector like if it was a
// circular list
static inline CPolygon2D::TVec2fVect::const_iterator
Next(const CPolygon2D::TVec2fVect::const_iterator &it,
...
// *******************************************************************************
// This function alow to cycle backward through a (non null) vertex vector like
// if it was a circular list
static inline CPolygon2D::TVec2fVect::const_iterator
Prev(const CPolygon2D::TVec2fVect::const_iterator &it,
     const CPolygon2D::TVec2fVect &cont) {
  nlassert(cont.size() != 0);
  if (it == cont.begin())
    return cont.end() - 1;
...
bool CPolygon2D::isCCWOriented() const {
  ...
  if (fLowest == fHighest) {
    ...
    while (pHighestRight->x == pHighest->x) {
      ...
      if (pHighestRight == pHighest)
        return false; // the poly is reduced to a point, returns an abritrary
                      // value
    }
  ...
  // iterator to the first vertex after pHighestRight, that has the same y than
  // the highest vertex
...
static void ScanOuterEdgeRight(CPolygon2D::TRaster *r, float x1, float y1,
                               float x2, float y2, sint minY) {
  ...
  if (deltaX >= 0.f) {
    if (height == 1) {
      ...
      currRaster = r + ((sint)floorf(y1) - minY);
      iposx =
          (sint32)(65536.0 * (x1 + inverseSlope *
                                       (ceilf(y1) - y1))); // sub-pixel accuracy
      if (ceilf(y1) == y1) {
        iposx += iInverseSlope;
      }
      do {
    ...
    iposx =
        (sint32)(65536.0 *
                 (x1 + inverseSlope * (ceilf(y1) - y1))); // sub-pixel accuracy
    if (ceilf(y1) == y1) {
      iposx += iInverseSlope;
    }
    while (--height) {
...
static void ScanOuterEdgeLeft(CPolygon2D::TRaster *r, float x1, float y1,
                              float x2, float y2, sint minY) {
  CPolygon2D::TRaster *currRaster;
  float deltaX, deltaY;
  float inverseSlope;
...
// *******************************************************************************
void CPolygon2D::computeOuterBorders(TRasterVect &borders,
                                     sint &minimumY) const {
#ifdef NL_DEBUG
  checkValidBorders();
#endif
  ...
  for (;;) {
    ...
    if (pHighestRightNext == last)
      pHighestRightNext = first;
    if (pHighestRightNext->y != pHighestRight->y)
      break;
  ...
  if (!ccw) {
    ...
    do {
      ...
      if (next == last)
        next = first;
      ScanOuterEdgeRight(&borders[0], curr->x, curr->y, next->x, next->y,
                         minimumY);
      curr = next;
    } while (curr != plowest);
    do {
      ...
      if (next == last)
        next = first;
      ScanOuterEdgeLeft(&borders[0], next->x, next->y, curr->x, curr->y,
                        minimumY);
      curr = next;
    ...
    do {
      ...
      if (next == last)
        next = first;
      ScanOuterEdgeLeft(&borders[0], curr->x, curr->y, next->x, next->y,
                        minimumY);
      curr = next;
    } while (curr != plowest);
    do {
      ...
      if (next == last)
        next = first;
      ScanOuterEdgeRight(&borders[0], next->x, next->y, curr->x, curr->y,
                         minimumY);
      curr = next;
...
static void ScanInnerEdge(CPolygon2D::TRaster *r, float x1, float y1, float x2,
                          float y2, sint minY, bool rightEdge) {
  const uint rol16 = 65536;
  CPolygon2D::TRaster *currRaster;
  float deltaX, deltaY;
  float inverseSlope;
  ...
  currRaster = r + ((sint)floorf(y1) - minY);
  //
  iposx = (sint32)(rol16 * (x1 + inverseSlope *
                                     (ceilf(y1) - y1))); // sub-pixel accuracy
  if (rightEdge) {
    iposx -= rol16 - 1;
    if (deltaX >= 0.f) {
...
// *******************************************************************************
void CPolygon2D::computeInnerBorders(TRasterVect &borders,
                                     sint &minimumY) const {
#ifdef NL_DEBUG
  checkValidBorders();
#endif
  ...
  // compute highest (with lowest y) and lowest (with highest y) points of the
  // poly
  do {
    fright = std::max(fright, curr->x);
    fleft = std::min(fleft, curr->x);
  ...
  if (!ccw) {
    ...
    do {
      ...
      if (next == last)
        next = first;
      ScanInnerEdge(&borders[0], curr->x, curr->y, next->x, next->y, minimumY,
                    true);
      curr = next;
    } while (curr != plowest);
    do {
      ...
      if (next == last)
        next = first;
      ScanInnerEdge(&borders[0], next->x, next->y, curr->x, curr->y, minimumY,
                    false);
      curr = next;
    } while (curr != pHighestLeft);
  } else {
...
// *******************************************************************************
void CPolygon2D::getLineEquation(uint index, float &a, float &b,
                                 float &c) const {
...
bool CPolygon2D::intersect(const CPolygon2D &other) const {
  ...
  if (getNonNullSeg(nonNullSegIndex)) {
    ...
    for (uint k = 0; k < Vertices.size(); ++k) {
      ...
      if ((getSegRef0(k) - getSegRef1(k)).sqrnorm() == 0.f)
        continue;
...
// *******************************************************************************
bool CPolygon2D::contains(const CVector2f &p,
                          bool hintIsConvex /*= true*/) const {
  if (hintIsConvex) {
    uint numVerts = (uint)Vertices.size();
...
CPolygon2D::CPolygon2D(const CTriangle &tri, const CMatrix &projMat) {
  Vertices.resize(3);
  NLMISC::CVector proj[3] = {projMat * tri.V0, projMat * tri.V1,
                             projMat * tri.V2};
...
// *******************************************************************************
void CPolygon2D::getBoundingRect(CVector2f &minCorner,
...
bool operator==(const CPolygon2D &lhs, const CPolygon2D &rhs) {
  if (lhs.Vertices.size() != rhs.Vertices.size())
    return false;
  return std::equal(lhs.Vertices.begin(), lhs.Vertices.end(),
                    rhs.Vertices.begin());
}
...
bool operator<(const CPolygon2D &lhs, const CPolygon2D &rhs) {
  ...
  for (uint k = 0; k < lhs.Vertices.size(); ++k) {
    if (lhs.Vertices[k] != rhs.Vertices[k])
      return lhs.Vertices[k] < rhs.Vertices[k];
  }
  return false;
}

// *******************************************************************************
static inline bool testSegmentIntersection(const CVector2f &a,
                                           const CVector2f &b,
                                           const CVector2f &c,
                                           const CVector2f &d) {
...
bool CPolygon2D::selfIntersect() const {
  ...
  for (uint k = 0; k < numEdges; ++k) {
    // test intersection with all other edges that don't share a vertex with
    // this one
    const CVector2f &p0 = getSegRef0(k);
    const CVector2f &p1 = getSegRef1(k);
    for (uint l = 0; l < k; ++l) {
      ...
      if (v0 == p0 || v0 == p1 || v1 == p0 || v1 == p1)
        continue;
      //
      if (testSegmentIntersection(p0, p1, v0, v1))
        return true;
    }
  }
  return false;

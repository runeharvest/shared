...
static void readPNGData(png_structp png_ptr, png_bytep data,
                        png_size_t length) {
...
static void writePNGData(png_structp png_ptr, png_bytep data,
                         png_size_t length) {
  IStream *stream = static_cast<IStream *>(png_get_io_ptr(png_ptr));
  if (stream)
    stream->serialBuffer((uint8 *)data, (uint)length);
...
uint8 CBitmap::readPNG(NLMISC::IStream &f) {
  ...
  png_struct *png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL,
                                               setPNGError, setPNGWarning);
  ...
  png_get_IHDR(png_ptr, info_ptr, &width, &height, &iBitDepth, &iColorType,
               NULL, NULL, NULL);
  ...
  if (iBitDepth < 8 || iColorType == PNG_COLOR_TYPE_PALETTE ||
  ...
  default:
    imageDepth = iBitDepth * 4;
    nlwarning("Unable to determine PNG color type: %d, consider it as RGBA",
              iColorType);
    break;
  }
  ...
  png_bytepp row_pointers =
      (png_bytepp)png_malloc(png_ptr, height * sizeof(png_bytep));
  ...
  if (iColorType == PNG_COLOR_TYPE_RGBA || iColorType == PNG_COLOR_TYPE_RGB ||
      iColorType == PNG_COLOR_TYPE_PALETTE) {
    // get all channels
    dstChannels = 4;
    firstChannel = 0;
    lastChannel = 3;
...
bool CBitmap::writePNG(NLMISC::IStream &f, uint32 d) {
  ...
  if (PixelFormat > AlphaLuminance)
    return false;
  if (!_Width || !_Height)
    return false;

  if (d == 0)
    d = bitPerPixels[PixelFormat];

  if (d != 32 && d != 24 && d != 16 && d != 8)
    return false;

  // create image write structure
  png_struct *png_ptr = png_create_write_struct(PNG_LIBPNG_VER_STRING, NULL,
                                                setPNGError, setPNGWarning);
  ...
  for (y = 0; y < height; ++y) {
    for (x = 0; x < width; ++x) {
      ...
      if (dstChannels <= 2 && srcChannels == 4) {
        ...
        row_pointers[y][dstOffset] =
            CRGBA(_Data[0][srcOffset + 0], _Data[0][srcOffset + 1],
                  _Data[0][srcOffset + 2])
                .toGray();
        ...
        if (sig_bit.alpha) {
          // copy alpha value
          row_pointers[y][dstOffset + 3] =
              PixelFormat == Luminance ? 255 : _Data[0][srcOffset + 1];
        }
      } else {
        // copy all values

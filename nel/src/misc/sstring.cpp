...
CSString CSString::strtok(
    const char *separators,
    bool useSmartExtensions, // if true then match brackets etc (and refine with
                             // following args)
    ...
    bool useRepeatQuoteStringEscape) // - treat """" as '"')
{
  if (useSmartExtensions) {
    ...
    while (!empty() && token.empty())
      token = splitToOneOfSeparators(separators, true, useAngleBrace,
...
CSString CSString::splitToOneOfSeparators(
    const CSString &separators, bool truncateThis,
    bool useAngleBrace,              // treat '<' and '>' as brackets
    bool useSlashStringEscape,       // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape, // treat """" as '"'
  ...
  for (i = 0; i < size(); ++i) {
    ...
    // if we've found the separator character then all's cool so break out of
    // the loop
    if (separators.contains(thisChar))
      break;

    // if we have a bracket or quote of any type then match to it's matching
    // bracket, quote or whatever
    if (isOpeningDelimiter(thisChar, useAngleBrace) ||
        isStringDelimiter(thisChar)) {
      if (i != 0) {
        // we are not at beginning of the string, delimiter is considered as
        // separator
        if (splitStringAtBrackets)
          break;
      }
      uint32 j = i;
      i = findMatchingDelimiterPos(useAngleBrace, useSlashStringEscape,
                                   useRepeatQuoteStringEscape, i);
...
CSString CSString::splitToLineComment(bool truncateThis,
  ...
  for (uint32 i = 0; i < size(); ++i) {
    ...
    else if (!quoteOpen && i < size() - 1 && operator[](i) == '/' &&
                                             operator[](i + 1) == '/') {
...
bool CSString::isValidText() {
  // setup a handy static lookup table for differentiating valid and invalid
  // text characters
  ...
  while (i--) {
    if (!tbl[(uint8) operator[](i)]) {
      nldebug("string is not valid text due to character: %u at index: %u",
              (uint8) operator[](i), i);
      return false;
    }
  }
...
uint32 CSString::findMatchingDelimiterPos(bool useAngleBrace,
  ...
  if (isOpeningDelimiter(openingDelimiter, useAngleBrace)) {
    ...
    while (i < size()) {
      ...
      if (isOpeningDelimiter((*this)[i], useAngleBrace) ||
      ...
      if (isClosingDelimiter((*this)[i], useAngleBrace)) {
        // we've found a closing delimiter that doesn't match our opening
        // delimiter so give up
        return startPos;
      }
    }
    ...
    while (i < size()) {
      ...
      if ((*this)[i] == openingDelimiter) {
        if (useRepeatQuoteStringEscape && (*this)[i + 1] == openingDelimiter) {
          // we've found a "" pair and we're treating it as \" equivalent so
      ...
      if (useSlashStringEscape && (*this)[i] == '\\') {
        // we've found a '\' character so skip the next character, whatever it
        // is
        ++i;
        continue;
      }
...
CSString CSString::matchDelimiters(
    bool truncateThis,
    bool useAngleBrace,              // treat '<' and '>' as brackets
    bool useSlashStringEscape,       // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape) // treat """" as '"'
{
  // skip white space
  uint32 startPos;
  for (startPos = 0; startPos < size() && isWhiteSpace((*this)[startPos]);
       ++startPos) {
  }

  // locate the matching brace
  uint32 matchPos =
      findMatchingDelimiterPos(useAngleBrace, useSlashStringEscape,
                               useRepeatQuoteStringEscape, startPos);
...
CSString CSString::splitToStringSeparator(
    char separator, bool truncateThis,
    bool useSlashStringEscape,       // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape, // treat """" as '"'
    bool truncateSeparatorCharacter) // if true tail begins after separator char
{
  ...
  for (i = 0; i < size(); ++i) {
    ...
    if (isStringDelimiter(thisChar)) {
      uint32 j = i;
      i = findMatchingDelimiterPos(false, useSlashStringEscape,
                                   useRepeatQuoteStringEscape, i);
      // if there was a problem then break here
      if (j == i)
        break;
...
CSString CSString::splitToSeparator(
    char separator,
    bool useAngleBrace,        // treat '<' and '>' as brackets
    bool useSlashStringEscape, // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape) const // treat """" as '"'
{
  return const_cast<CSString *>(this)->splitToSeparator(
      separator, false, useAngleBrace, useSlashStringEscape,
      useRepeatQuoteStringEscape, false);
}
...
CSString CSString::splitToOneOfSeparators(
    ...
    bool useRepeatQuoteStringEscape) const // treat """" as '"'
{
  return const_cast<CSString *>(this)->splitToOneOfSeparators(
      separators, false, useAngleBrace, useSlashStringEscape,
      useRepeatQuoteStringEscape, false);
}

bool CSString::isDelimitedMonoBlock(
    bool useAngleBrace,             // treat '<' and '>' as brackets
    bool useSlashStringEscape,      // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape // treat """" as '"'
) const {
  if (empty())
    return false;
  uint32 matchPos = findMatchingDelimiterPos(
      useAngleBrace, useSlashStringEscape, useRepeatQuoteStringEscape);
  return (matchPos == size() - 1 &&
          isMatchingDelimiter((*this)[0], (*this)[matchPos]));
}

CSString CSString::stripBlockDelimiters(
    bool useAngleBrace,             // treat '<' and '>' as brackets
    bool useSlashStringEscape,      // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape // treat """" as '"'
) const {
  if (isDelimitedMonoBlock(useAngleBrace, useSlashStringEscape,
...
bool CSString::splitWordOrWords(CVectorSString &result,
                                bool useSlashStringEscape,
                                bool useRepeatQuoteStringEscape) const {
  CSString s = *this;
  while (!s.empty()) {
    uint pre = (uint)s.size();
    result.push_back(s.firstWordOrWords(true, useSlashStringEscape,
                                        useRepeatQuoteStringEscape));
    uint post = (uint)s.size();
    if (post >= pre)
      return false;
...
bool CSString::splitBySeparator(
    char separator, CVectorSString &result,
    bool useAngleBrace,              // treat '<' and '>' as brackets
    bool useSlashStringEscape,       // treat '\' as escape char so "\"" == '"'
    bool useRepeatQuoteStringEscape, // treat """" as '"'
  ...
  while (!s.empty()) {
    uint pre = (uint)s.size();
    result.push_back(s.splitToSeparator(separator, true, useAngleBrace,
                                        useSlashStringEscape,
                                        useRepeatQuoteStringEscape, true));
...
bool CSString::splitByOneOfSeparators(
  ...
  CSString s = *this;
  while (!s.empty() && skipBlankEntries && !retainSeparators &&
         separators.contains(s[0]))
    s = s.leftCrop(1);

  while (!s.empty()) {
    uint pre = (uint)s.size();
    result.push_back(s.splitToOneOfSeparators(
        separators, true, useAngleBrace, useSlashStringEscape,
        useRepeatQuoteStringEscape, !retainSeparators));

    // if we failed to extract a string segment then we must be looking at a
    // separator
...
const CSString &CSString::join(const std::vector<CSString> &strings,
...
CSString CSString::splitTo(const char *s, bool truncateThis) {
  ...
  // we didn't find the separator string so we're returning a copy of the whole
  // string
  if (truncateThis)
    clear();
  return result;
...
CSString CSString::firstWordOrWords(bool truncateThis,
                                    bool useSlashStringEscape,
                                    bool useRepeatQuoteStringEscape) {
  uint32 startPos;
  ...
  if (isStringDelimiter((*this)[startPos])) {
    uint32 endPos = findMatchingDelimiterPos(
        false, useSlashStringEscape, useRepeatQuoteStringEscape, startPos);
...
unsigned CSString::countWordOrWords(bool useSlashStringEscape,
                                    bool useRepeatQuoteStringEscape) const {
  unsigned count = 0;
  CSString hold = strip();
  while (!hold.empty()) {
    hold = hold.tailFromFirstWordOrWords(useSlashStringEscape,
                                         useRepeatQuoteStringEscape)
               .strip();
    ++count;
  }
...
CSString CSString::wordOrWords(uint32 idx, bool useSlashStringEscape,
                               bool useRepeatQuoteStringEscape) const {
  CSString hold = strip();
  ...
  return hold.firstWordOrWords(useSlashStringEscape,
                               useRepeatQuoteStringEscape);
}
...
CSString CSString::quote(bool useSlashStringEscape,
                         bool useRepeatQuoteStringEscape) const {
  CSString result;
...
CSString CSString::quoteIfNotQuoted(bool useSlashStringEscape,
                                    bool useRepeatQuoteStringEscape) const {
  if (empty() || (*this)[0] != '\"' ||
      !isDelimitedMonoBlock(false, useSlashStringEscape,
                            useRepeatQuoteStringEscape))
...
CSString CSString::quoteIfNotAtomic(bool useSlashStringEscape,
                                    bool useRepeatQuoteStringEscape) const {
  if (empty())
    return "\"\"";
...
CSString CSString::unquote(bool useSlashStringEscape,
                           bool useRepeatQuoteStringEscape) const {
  CSString result = stripBlockDelimiters(false, useSlashStringEscape,
                                         useRepeatQuoteStringEscape);
  uint32 i, j;
  for (i = 0, j = 0; i < result.size(); ++i, ++j) {
    ...
    } else if (useRepeatQuoteStringEscape && (i + 1 < result.size()) &&
               result[i] == '\"' && result[i + 1] == '\"')
      ++i;
    if (i < result.size())
      result[j] = result[i];
...
CSString CSString::unquoteIfQuoted(bool useSlashStringEscape,
                                   bool useRepeatQuoteStringEscape) const {
  if (isQuoted())
    ...
    breakable {
      if ((*this)[i] == '&') {
        ...
        if ((size() - i) >= 5) {
          ...
          if (j == i + 3)
            break;
          // make sure have the terminating ';' to complete the token:
          // "&#x"[0-9|a-f]*";"
          if (j >= size() || (*this)[j] != ';')
            break;
          // make sure that the value we have is only one or 2 hex digits
          if (j >= i + 6)
            nlwarning("Truncating extended char code '%s",
                      (substr(i, j - i + 1) + "' => using " + substr(j - 2, 2))
                          .c_str());
          ...
          result += c;
          // move 'i' forward to point at the ';' .. the for(...) will increment
          // i to point to next char
          i = j;
          continue;
        }
      }
...
bool CSString::isEncodedXML() const {
  ...
  for (uint i = (uint)size(); i--;) {
    ...
    case ';':
      ...
      if (i >= 5 && substr(i - 5, 5) == "&quot") {
        foundToken = true;
        i -= 5;
        break;
      }
      if (i >= 4 && substr(i - 4, 4) == "&amp") {
        foundToken = true;
        i -= 4;
        break;
      }
      if (i >= 3 && substr(i - 3, 3) == "&lt") {
        foundToken = true;
        i -= 3;
        break;
      }
      if (i >= 3 && substr(i - 3, 3) == "&gt") {
        foundToken = true;
        i -= 3;
        break;
      }
      ...
      if (i >= 3 && isHexDigit((*this)[i - 1])) {
        ...
        if ((*this)[j + 2] != 'x')
          continue;
        // ensure that the remainder between the leading '&#x' and trailing ';'
        // are hex digits
        for (k = j + 3; k < i; ++k)
  ...
  for (i = 0; i < size();) {
    ...
    for (j = 0; toFind[j]; ++j)
      if ((*this)[i + j] != toFind[j])
        break;
    // if strings were identical then j reffers to ASCIIZ terminator at end of
    // 'toFind'
    if (toFind[j] == 0) {
      if (replacement != NULL)
        result += replacement;
...
CSString::find(const char *toFind, std::string::size_type startLocation) const {
...
/// Find index at which a sub-string starts (case NOT sensitive) - if sub-string
/// not found then returns string::npos
std::string::size_type
CSString::findNS(const char *toFind,
                 std::string::size_type startLocation) const {
  const char *constStr = c_str();
...
static const uint32 MaxNegSint32LastDigit =
    MaxNegSint32 - (MaxNegSint32 / 10) * 10;
static const uint32 MaxNegSint32PreLastDigit = (MaxNegSint32 / 10);

static const uint32 MaxPosSint32 = ~0u / 2;
static const uint32 MaxPosSint32LastDigit =
    MaxPosSint32 - (MaxPosSint32 / 10) * 10;
...
sint32 CSString::atosi() const {
  ...
  for (const_iterator it = begin() + 1; it != end(); ++it) {
    ...
    if (result >= MaxPosSint32PreLastDigit /*~0u/20*/) {
      if (result > MaxPosSint32PreLastDigit ||
          offset > (neg ? MaxNegSint32LastDigit : MaxPosSint32LastDigit))
        return 0;
    }
    result = 10 * result + offset;
  }
...
static const uint64 MaxNegSint64LastDigit =
    MaxNegSint64 - (MaxNegSint64 / 10) * 10;
static const uint64 MaxNegSint64PreLastDigit = (MaxNegSint64 / 10);

static const uint64 MaxPosSint64 = ((uint64)0 - (uint64)1) / 2;
static const uint64 MaxPosSint64LastDigit =
    MaxPosSint64 - (MaxPosSint64 / 10) * 10;
...
sint64 CSString::atosi64() const {
  ...
  for (const_iterator it = begin() + 1; it != end(); ++it) {
    ...
    if (result >= MaxPosSint64PreLastDigit /*~0u/20*/) {
      if (result > MaxPosSint64PreLastDigit ||
          offset > (neg ? MaxNegSint64LastDigit : MaxPosSint64LastDigit))
        return 0;
    }
    result = 10 * result + offset;
  }
...
bool CSString::readFromFile(const CSString &fileName) {
  ...
  if (file == NULL) {
    clear();
    // There was previously a warning displayed here but that was incorrect as
    // it is defined that refaFromFile returns an empty result if the file is
    // not found nlwarning("Failed to open file for reading:
    // %s",fileName.c_str());
    return false;
  }
  ...
  if (bytesRead != size()) {
    resize(bytesRead);
    nlwarning("Failed to read file contents (requested %u bytes but fread "
              "returned %u) for file:%s",
              size(), bytesRead, fileName.c_str());
    return false;
  }
  return true;
}
...
bool CSString::writeToFile(const CSString &fileName) const {
  ...
  uint32 recordsWritten =
      (uint32)fwrite(const_cast<char *>(data()), size(), 1, file);
  fclose(file);
  if (recordsWritten != 1) {
    nlwarning("Failed to write file contents (requested %u bytes but fwrite "
              "returned %u) for file:%s",
              size(), recordsWritten, fileName.c_str());
    return false;
  }
  ...
  if (NLMISC::CFile::fileExists(fileName)) {
    ...
    if (NLMISC::CFile::getFileSize(fileName) == size()) {
      ...
      if (hold.size() == size() && memcmp(&hold[0], &(*this)[0], size()) == 0) {
        ...
        nldebug("CSSWTF Request to write data to file %s IGNORED because file "
                "already contains correct data",
                fileName.c_str());
        return true;
      }
    }

...
public:
  ...
  CNel7ZipInStream(NLMISC::IStream *s) : _Stream(s) {
    Read = readFunc;
    Seek = seekFunc;
  }

  // the read function called by 7zip to read data
  static SRes readFunc(const ISeekInStream *object, void *buffer,
                       size_t *size) {
    try {
  ...
  static SRes seekFunc(const ISeekInStream *object, Int64 *pos,
                       ESzSeek origin) {
    try {
      CNel7ZipInStream *me = (CNel7ZipInStream *)object;
      sint32 offset = (sint32)*pos;
      bool ret =
          me->_Stream->seek(offset, (NLMISC::IStream::TSeekOrigin)origin);
...
bool unpack7Zip(const std::string &sevenZipFile,
                const std::string &destFileName) {
  nlinfo("Uncompressing 7zip archive '%s' to '%s'", sevenZipFile.c_str(),
         destFileName.c_str());
  ...
  if (processedSize != outSizeProcessed) {
    nlerror("Failed to write %u char to output file '%s'",
            outSizeProcessed - processedSize, destFileName.c_str());
    return false;
  }
...
bool unpackLZMA(const std::string &lzmaFile, const std::string &destFileName) {
  nldebug("unpackLZMA: decompress LZMA file '%s' to '%s", lzmaFile.c_str(),
          destFileName.c_str());
  ...
  if (inSize < LZMA_PROPS_SIZE + 8) {
    nlwarning("unpackLZMA: Invalid file size, too small file '%s'",
              lzmaFile.c_str());
    return false;
  }
  ...
  try {
    ...
    nlwarning("unpackLZMA: Error while reading '%s': %s", lzmaFile.c_str(),
              e.what());
    return false;
  }
  ...
  sint res = LzmaUncompress(&outBuffer[0], &outProcessed, &inBuffer[0],
                            &inProcessed, &propsBuffer[0], LZMA_PROPS_SIZE);

  if (res != 0 || outProcessed != fileSize) {
    nlwarning("unpackLZMA: Failed to decode lzma file '%s' with status %d",
              lzmaFile.c_str(), res);
    return false;
  }
  ...
  try {
    ...
    nlwarning("unpackLZMA: Error while writing '%s': %s", destFileName.c_str(),
              e.what());
...
bool packLZMA(const std::string &srcFileName, const std::string &lzmaFileName) {
  nldebug("packLZMA: compress '%s' to LZMA file '%s", srcFileName.c_str(),
          lzmaFileName.c_str());
  ...
  try {
    ...
    nlwarning("packLZMA: Error while reading '%s': %s", srcFileName.c_str(),
              e.what());
    return false;
  }
  ...
  sint res =
      LzmaCompress(&outBuffer[0], &outSize, &inBuffer[0], inSize, &outProps[0],
                   &outPropsSize, 9, 1 << 27, -1, -1, -1, -1, 1);
  ...
  case SZ_OK: {
    ...
    try {
      ...
      nlwarning("packLZMA: Error while writing '%s': %s", lzmaFileName.c_str(),
                e.what());
  ...
  case SZ_ERROR_MEM:
    nlwarning("packLZMA: Memory allocation error while compressing '%s' (input "
              "buffer size: %u, output buffer size: %u)",
              srcFileName.c_str(), (uint)inSize, (uint)outSize);
    break;

  case SZ_ERROR_PARAM:
    nlwarning("packLZMA: Incorrect parameter while compressing '%s'",
              srcFileName.c_str());
    break;

  case SZ_ERROR_OUTPUT_EOF:
    nlwarning("packLZMA: Output buffer overflow while compressing '%s' (input "
              "buffer size: %u, output buffer size: %u)",
              srcFileName.c_str(), (uint)inSize, (uint)outSize);
    break;
  ...
  case SZ_ERROR_THREAD:
    nlwarning(
        "packLZMA: Errors in multithreading functions (only for Mt version)");
    break;

  default:
    nlwarning("packLZMA: Unknown error (%d) while compressing '%s'", res,
              srcFileName.c_str());
  }

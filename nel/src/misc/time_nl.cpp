...
bool hasMonotonicClock() {
  if (!a_CheckedMonotonicClock) {
    ...
    if ((clock_gettime(CLOCK_MONOTONIC, &tv) == 0) &&
      ...
      //			nldebug( "Monotonic local time supported
      //(resolution %.6f ms)", ((float)tv.tv_sec)*1000.0f +
      //((float)tv.tv_nsec)/1000000.0f );
...
void CTime::probeTimerInfo(CTime::CTimerInfo &result) {
  breakable {
    ...
    if (winPerfFreq.QuadPart == 1000) {
      nldebug(
          "Higher precision timer not available, OS defaulted to GetTickCount");
      result.IsHighPrecisionAvailable = false;
    }
    ...
    uint64 threadMask =
        IThread::getCurrentThread()
            ->getCPUMask(); // broken on linux, don't expect it to work anywhere
#else
    ...
    uint backwards = 0;    // Happens when the timers are not always in sync and
    ...
    // Cycle 32 times trough all cores, and verify if the timing remains
    // consistent.
    for (uint i = 32; i; --i) {
      uint64 currentBit = 1;
      for (uint j = 64; j; --j) {
          ...
          */
          ...
          if (result.IsHighPrecisionAvailable) {
            ...
            if (winPerfCountN.QuadPart == winPerfCount.QuadPart)
              ++identical;
            if (winPerfCountN.QuadPart < winPerfCount.QuadPart ||
                winPerfCountN.QuadPart - winPerfCount.QuadPart < 0)
              ++backwards;
            if (winPerfCountN.QuadPart - winPerfCount.QuadPart >
                winPerfFreq.QuadPart / 20) // 50ms skipping check
              ++skipping;
          ...
          if (hasMonotonicClock()) {
            ...
            if (monoClock.tv_sec == monoClockN.tv_sec &&
                monoClock.tv_nsec == monoClockN.tv_nsec)
              ++identical;
            if (monoClockN.tv_sec < monoClock.tv_sec ||
                (monoClock.tv_sec == monoClockN.tv_sec &&
                 monoClockN.tv_nsec < monoClock.tv_nsec))
              ++backwards;
            if (monoClock.tv_sec == monoClockN.tv_sec &&
                (monoClockN.tv_nsec - monoClock.tv_nsec > 50000000L))
              ++skipping;
            else if ((monoClock.tv_sec == monoClockN.tv_sec &&
                      monoClock.tv_nsec < monoClockN.tv_nsec) ||
                     monoClock.tv_sec < monoClockN.tv_sec)
              ++regular;
            monoClock.tv_sec = monoClockN.tv_sec;
    ...
    nldebug("Time identical: %i, backwards: %i, regular: %i, skipping: %i, "
            "frequency bug: %i",
            identical, backwards, regular, skipping, frequencybug);
    if (identical > regular)
      nlwarning("The system timer is of relatively low resolution, you may "
                "experience issues");
    if (backwards > 0 || frequencybug > 0) {
      nlwarning(
          "The current system timer is not reliable across multiple cpu cores");
      result.RequiresSingleCore = true;
    } else
      result.RequiresSingleCore = false;
 ...
 * This values is the same on all computer if computers are synchronized (with
 * NTP for example).
 */
uint32 CTime::getSecondsSince1970() { return uint32(time(NULL)); }

/** Return the number of second since midnight (00:00:00), January 1, 1970,
 * coordinated universal time, according to the system clock.
 * The time returned is UTC (aka GMT+0), ie it does not have the local time
 * ajustement nor it have the daylight saving ajustement. This values is the
 * same on all computer if computers are synchronized (with NTP for example).
 */
 ...
 * Use it only to measure time difference, the absolute value does not mean
 * anything. On Unix, getLocalTime() will try to use the Monotonic Clock if
 * available, otherwise the value can jump backwards if the system time is
 * changed by a user or a NTP time sync process. The value is different on 2
 * different computers; use the CUniTime class to get a universal time that is
 * the same on all computers. \warning On Win32, the value is on 32 bits only.
 * It wraps around to 0 every about 49.71 days.
 */
TTime CTime::getLocalTime() {
  ...
  /* Retrieve time is ms
   * Why do we prefer getPerformanceTime() to timeGetTime() ? Because on one
   * dual-processor Win2k PC, we have noticed that timeGetTime() slows down when
   * the client is running !!!
   */
  /* Now we have noticed that on all WinNT4 PC the getPerformanceTime can give
   * us value that are less than previous
   */
  ...
  //  return timeGetTime(); // Only this was left active before it was
  //  commented.
  //}
  ...
  if (hasMonotonicClock()) {
    ...
    return (TTime)tv.tv_sec * (TTime)1000 +
           (TTime)((tv.tv_nsec /*+500*/) / 1000000);
  }
 ...
 * \warning The speed of tick increase can vary (especially on laptops or CPUs
 * with power management), so profiling several times and computing the average
...
TTicks CTime::getPerformanceTime() {
  ...
  if (firstWarn) {
    nlwarning("TTicks CTime::getPerformanceTime () is not implemented for your "
              "processor, returning 0");
    firstWarn = false;
  }
  return 0;
#endif // HAVE_X86
...
4(%esi)"); \ asm volatile ("pop %edx\n\t" "pop %eax\n\t" "pop %esi");
*/

/* Convert a ticks count into second. If the performance time is not supported
 * on this hardware, it returns 0.0.
 */
...
std::string CTime::getHumanRelativeTime(sint32 nbSeconds) {
  ...
  const sint32 oneMonth =
      oneDay * 30; // aprox, a more precise value is 30.416666... but no matter
  const sint32 oneYear =
      oneDay * 365; // aprox, a more precise value is 365.26.. who care?
...
uint64 CTime::getWindowsToUnixBaseTimeOffset() {
  ...
  if (!init) {
    // compute the offset to convert windows base time into unix time (aka
    // epoch) build a WIN32 system time for jan 1, 1970
    SYSTEMTIME baseTime;
    baseTime.wYear = 1970;
    baseTime.wMonth = 1;
    ...
    offset = baseFileTime.dwLowDateTime |
             (uint64(baseFileTime.dwHighDateTime) << 32);

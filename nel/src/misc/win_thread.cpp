...
CWinThread::CWinThread(void *threadHandle, uint32 threadId) {
  ...
  if (GetCurrentThreadId() == threadId) {
    ...
    nlassert(0); // WARNING: following code has not tested! don't know if it
                 // work fo real ... This is just a suggestion of a possible
                 // solution, should this situation one day occur ...
    // Ensure that this thread don't get deleted, or we could suspend the main
    // thread
    s_CS.enter();
    // the 2 following statement must be executed atomicaly among the threads of
    // the current process !
...
void CWinThread::incSuspendCount() {
  ...
  nlassert(newSuspendCount ==
           _SuspendCount +
               1); // is this assert fire , then 'SuspendThread' or
                   // 'ResumeThread' have been called outside of this object
                   // interface! (on this thread handle ...)
...
void CWinThread::start() {
  if (isRunning())
    throw EThread("Starting a thread that is already started, existing thread "
                  "will continue running, this should not happen");

  //	ThreadHandle = (void *) ::CreateThread (NULL, _StackSize, ProxyFunc,
  //this, 0, (DWORD *)&ThreadId);
  ThreadHandle =
      (void *)::CreateThread(NULL, 0, ProxyFunc, this, 0, (DWORD *)&ThreadId);
  //	nldebug("NLMISC: thread %x started for runnable '%x'", typeid( Runnable
  //).name()); 	OutputDebugString(toString(NL_LOC_MSG " NLMISC: thread %x started
...
IProcess *IProcess::getCurrentProcess() { return &CurrentProcess; }
...
uint64 CWinProcess::getCPUMask() {
  ...
  if (GetProcessAffinityMask((HANDLE)_ProcessHandle, &processAffinityMask,
                             &systemAffinityMask)) {
    // Return the CPU mask
...
bool CWinProcess::setCPUMask(uint64 mask) {
  ...
  return SetProcessAffinityMask((HANDLE)_ProcessHandle, processAffinityMask) !=
         0;
}
...
class CPSAPILib {
public:
  typedef BOOL(WINAPI *EnumProcessesFunPtr)(DWORD *lpidProcess, DWORD cb,
                                            DWORD *cbNeeded);
  typedef DWORD(WINAPI *GetModuleFileNameExWFunPtr)(HANDLE hProcess,
                                                    HMODULE hModule,
                                                    LPWSTR lpFilename,
                                                    DWORD nSize);
  typedef BOOL(WINAPI *EnumProcessModulesFunPtr)(HANDLE hProcess,
                                                 HMODULE *lphModule, DWORD cb,
                                                 LPDWORD lpcbNeeded);
...
bool CPSAPILib::init() {
  ...
  if (!_PSAPILibHandle) {
    ...
    EnumProcesses =
        (EnumProcessesFunPtr)GetProcAddress(_PSAPILibHandle, "EnumProcesses");
    GetModuleFileNameExW = (GetModuleFileNameExWFunPtr)GetProcAddress(
        _PSAPILibHandle, "GetModuleFileNameExW");
    EnumProcessModules = (EnumProcessModulesFunPtr)GetProcAddress(
        _PSAPILibHandle, "EnumProcessModules");
...
bool CWinProcess::enumProcessesId(std::vector<uint32> &processesId) {
  ...
  for (;;) {
    DWORD cbNeeded;
    if (!PSAPILib.EnumProcesses((DWORD *)&prcIds[0],
                                (DWORD)(prcIds.size() * sizeof(DWORD)),
                                &cbNeeded)) {
...
// ****************************************************************************************************************
bool CWinProcess::enumProcessModules(uint32 processId,
  ...
  for (;;) {
    DWORD cbNeeded;
    if (!PSAPILib.EnumProcessModules(
            hProcess, (HMODULE *)&prcModules[0],
            (DWORD)(prcModules.size() * sizeof(HMODULE)), &cbNeeded)) {
  ...
  for (uint m = 0; m < prcModules.size(); ++m) {
    if (PSAPILib.GetModuleFileNameExW(hProcess, prcModules[m], moduleName,
                                      MAX_PATH)) {
...
// ****************************************************************************************************************
uint32
CWinProcess::getProcessIdFromModuleFilename(const std::string &moduleFileName) {
  std::vector<uint32> processesId;
  if (!enumProcessesId(processesId))
    return false;
  std::vector<std::string> moduleNames;
  for (uint prc = 0; prc < processesId.size(); ++prc) {
    if (enumProcessModules(processesId[prc], moduleNames)) {
      for (uint m = 0; m < moduleNames.size(); ++m) {
        if (nlstricmp(CFile::getFilename(moduleNames[m]), moduleFileName) ==
            0) {
...
// ****************************************************************************************************************
bool CWinProcess::terminateProcessFromModuleName(const std::string &moduleName,
                                                 uint exitCode) {
...
public:
    ...
    bool launch(const std::string &programName, const std::string &arguments)
    {
        ...
        if (CreateProcessW(programName.c_str(),
const_cast<LPTSTR>(arguments.c_str()), NULL, NULL, FALSE, 0, NULL, NULL,
...
bool CProcessWatch::launch(const std::string &programName, const std::string
&arguments)
{

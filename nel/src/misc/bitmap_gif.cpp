...
uint8 CBitmap::readGIF(NLMISC::IStream &f) {
...

  {
    ...
    if (width * height > 4000 * 4000) {
      nlwarning("GIF image size is too big (width=%d, height=%d)", width,
                height);
      return 0;
    }
    ...
    if (curFrame->ExtensionBlockCount > 0) {
      for (sint e = 0; e < curFrame->ExtensionBlockCount; e++) {
        ...
        if (ext->Function == GRAPHICS_EXT_FUNC_CODE) {
          ...
          transparency = (flag & GIF_TRANSPARENT_MASK) ? ext->Bytes[3]
                                                       : GIF_NOT_TRANSPARENT;
    ...
    if (curFrame->ImageDesc.Interlace) {
      ...
      for (uint8 pass = 0; pass < 4; pass++) {
        ...
        for (uint32 y = 0; y < height; y++) {
          ...
          uint32 dstOffset = (y + offset_y) * gif->SWidth * dstChannels +
                             offset_x * dstChannels;
      ...
      for (uint32 y = 0; y < height; y++) {
        ...
        uint32 dstOffset =
            (y + offset_y) * gif->SWidth * dstChannels + offset_x * dstChannels;
        for (uint32 x = 0; x < width; x++) {
          uint32 index = curFrame->RasterBits[srcOffset];

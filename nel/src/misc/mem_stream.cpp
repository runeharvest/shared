  ...
  if (isReading()) {
    ...
    if (pos + len > total) // calls virtual length (cf. sub messages)
    {
      throw EStreamOverflow(
          "CMemStream serialBuffer overflow: Read past %u bytes", total);
    }
 ...
 * When serializing, increment the pointer if its value exceeds its previous
 * value (to prevent from an "inside serial" to increment it). Then a seek(end)
 * would get back to the pointer.
 */
...
/*
 * Input: read from the stream until the next separator, and return the number
 * of bytes read. The separator is then skipped.
 */
uint CMemStream::serialSeparatedBufferIn(uint8 *buf, uint len) {
  ...
  if ((_Buffer.Pos == _Buffer.getBuffer().size()) || // we are at the end
      ((lengthS() + len + SEP_SIZE > length()) &&
       (_Buffer.getBuffer()[_Buffer.getBuffer().size() - 1] !=
        SEPARATOR))) // we are before the end // calls virtual length (cf. sub
                     // messages)
  {
    throw EStreamOverflow();
  }

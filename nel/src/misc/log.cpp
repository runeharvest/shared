...
CLog::CLog(TLogType logType)
    : _LogType(logType), _FileName(NULL), _Line(-1), _FuncName(NULL),
...
void CLog::setDefaultProcessName() {
  if (_ProcessName == NULL) {
    _ProcessName = (string *)INelContext::getInstance().getSingletonPointer(
        "NLMISC::CLog::_ProcessName");
    if (_ProcessName == NULL) {
      _ProcessName = new string;
      INelContext::getInstance().setSingletonPointer(
          "NLMISC::CLog::_ProcessName", _ProcessName);
    }
  }
...
/// Symetric to setPosition(). Automatically called by display...(). Do not call
/// if noDisplayer().
...
void CLog::addDisplayer(IDisplayer *displayer, bool bypassFilter) {
  ...
  if (bypassFilter) {
    CDisplayers::iterator idi =
        std::find(_BypassFilterDisplayers.begin(),
                  _BypassFilterDisplayers.end(), displayer);
    ...
    CDisplayers::iterator idi =
        std::find(_Displayers.begin(), _Displayers.end(), displayer);
...
void CLog::removeDisplayer(IDisplayer *displayer) {
  ...
  idi = std::find(_BypassFilterDisplayers.begin(),
                  _BypassFilterDisplayers.end(), displayer);
  if (idi != _BypassFilterDisplayers.end()) {
...
void CLog::removeDisplayer(const char *displayerName) {
  ...
  for (idi = _BypassFilterDisplayers.begin();
       idi != _BypassFilterDisplayers.end();) {
    if ((*idi)->DisplayerName == displayerName) {
...
IDisplayer *CLog::getDisplayer(const char *displayerName) {
  ...
  for (idi = _BypassFilterDisplayers.begin();
       idi != _BypassFilterDisplayers.end(); idi++) {
    if ((*idi)->DisplayerName == displayerName) {
      return *idi;
    }
  }
...
bool CLog::attached(IDisplayer *displayer) const {
  return (find(_Displayers.begin(), _Displayers.end(), displayer) !=
          _Displayers.end()) ||
         (find(_BypassFilterDisplayers.begin(), _BypassFilterDisplayers.end(),
               displayer) != _BypassFilterDisplayers.end());
}
...
void CLog::displayString(const char *str) {
  ...
  for (CDisplayers::iterator idi = _BypassFilterDisplayers.begin();
       idi != _BypassFilterDisplayers.end(); idi++) {
  ...
  if (passFilter(disp)) {
    ...
    for (CDisplayers::iterator idi = _Displayers.begin();
         idi != _Displayers.end(); idi++) {
      (*idi)->display(*args, disp);
    }
  }
  TempString.clear();
...
/*
 * Display the string with decoration and final new line to all attached
 * displayers
 */
...
/// Do not call this unless you know why you're doing it, it kills the debug/log
/// system!
void CLog::releaseProcessName() {
  if (INelContext::isContextInitialised()) {
    INelContext::getInstance().releaseSingletonPointer(
        "NLMISC::CLog::_ProcessName", _ProcessName);
  }

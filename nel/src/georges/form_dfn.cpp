...
void CFormDfn::write(xmlDocPtr doc, const std::string &filename) {
  ...
  for (parent = 0; parent < Parents.size(); parent++) {
    ...
    if (!Parents[parent].ParentFilename.empty()) {
      ...
      xmlNodePtr parentNode =
          xmlNewChild(node, NULL, (const xmlChar *)"PARENT", NULL);

      // Save parent
      xmlSetProp(parentNode, (const xmlChar *)"Name",
                 (const xmlChar *)Parents[parent].ParentFilename.c_str());
    }
  }
  ...
  for (elm = 0; elm < Entries.size(); elm++) {
    // Add a node
    xmlNodePtr elmPtr =
        xmlNewChild(node, NULL, (const xmlChar *)"ELEMENT", NULL);
    xmlSetProp(elmPtr, (const xmlChar *)"Name",
               (const xmlChar *)Entries[elm].Name.c_str());

    // What kind of element
    switch (Entries[elm].TypeElement) {
    case UFormDfn::EntryType:
      xmlSetProp(elmPtr, (const xmlChar *)"Type", (const xmlChar *)"Type");
      xmlSetProp(elmPtr, (const xmlChar *)"Filename",
                 (const xmlChar *)Entries[elm].Filename.c_str());
      if ((!Entries[elm].FilenameExt.empty()) &&
          Entries[elm].FilenameExt != "*.*")
        xmlSetProp(elmPtr, (const xmlChar *)"FilenameExt",
                   (const xmlChar *)Entries[elm].FilenameExt.c_str());
      break;
    ...
    case UFormDfn::EntryVirtualDfn:
      xmlSetProp(elmPtr, (const xmlChar *)"Type",
                 (const xmlChar *)"DfnPointer");
      break;
    }
    ...
    if ((Entries[elm].TypeElement == UFormDfn::EntryType) &&
        (!Entries[elm].Default.empty()))
      xmlSetProp(elmPtr, (const xmlChar *)"Default",
                 (const xmlChar *)Entries[elm].Default.c_str());
  }
...
void CFormDfn::read(xmlNodePtr root, CFormLoader &loader, bool forceLoad,
  ...
  if (((const char *)root->name == NULL) ||
      (strcmp((const char *)root->name, "DFN") != 0)) {
    // Throw exception
    warning(true, "read",
            "XML Syntax error in block line %d, node (%s) should be DFN.",
  ...
  while (parentNumber < parentCount) {
    ...
    const char *parentFilename =
        (const char *)xmlGetProp(parent, (xmlChar *)"Name");
    if (parentFilename) {
      ...
      // Load the parent
      Parents[parentNumber].Parent = loader.loadFormDfn(
          Parents[parentNumber].ParentFilename.c_str(), forceLoad);
      if ((Parents[parentNumber].Parent == NULL) && !forceLoad) {
        // Throw exception
        warning(true, "read", "Can't load parent DFN file (%s).",
                Parents[parentNumber].ParentFilename.c_str());
      }
    } else {
      // Throw exception
      warning(
          true, "read",
          "XML Syntax error in block (%s) line %d, aguments Name not found.",
  ...
  while (childNumber < childCount) {
    ...
    if (value) {
      ...
      const char *filename =
          (const char *)xmlGetProp(child, (xmlChar *)"Filename");
      ...
      const char *filenameExt =
          (const char *)xmlGetProp(child, (xmlChar *)"FilenameExt");
      if (filenameExt) {
      ...
      if (typeName) {
        ...
        if (stricmp(typeName, "Type") == 0) {
          ...
          if (!Entries[childNumber].Filename.empty()) {
            Entries[childNumber].Type =
                loader.loadType(Entries[childNumber].Filename.c_str());
            if ((Entries[childNumber].Type == NULL) && !forceLoad) {
              // Throw exception
              warning(true, "read",
                      "In XML block (%s) line %d, file not found %s.",
                      child->name, (sint)child->line,
                      Entries[childNumber].Filename.c_str());
            }

            // Read the default value
            const char *defaultName =
                (const char *)xmlGetProp(child, (xmlChar *)"Default");
            if (defaultName) {
            ...
            // Throw exception
            warning(true, "read",
                    "XML In block (%s) line %d, no filename found for the .typ "
                    "file.",
                    child->name, (sint)child->line);
          }
          ...
          if (!Entries[childNumber].Filename.empty()) {
            // Load the filename
            Entries[childNumber].Dfn = loader.loadFormDfn(
                Entries[childNumber].Filename.c_str(), forceLoad);
            if ((Entries[childNumber].Dfn == NULL) && !forceLoad) {
              // Throw exception
              warning(true, "read",
                      "XML In block (%s) line %d, file not found %s.",
                      child->name, (sint)child->line,
                      Entries[childNumber].Filename.c_str());
            }
          } else {
          ...
          // Throw exception
          warning(true, "read",
                  "XML Syntax error in block (%s) line %d, element has not a "
                  "valid type name attribut \"Type = %s\".",
        ...
        // Throw exception
        warning(true, "read",
                "XML Syntax error in block (%s) line %d, element has no type "
                "name attribut \"Type = [Type][Dfn][DfnPointer]\".",
                child->name, (sint)child->line);
      }

      // Get the array attrib
      Entries[childNumber].Array = false;
      const char *arrayFlag =
          (const char *)xmlGetProp(child, (xmlChar *)"Array");
      if (arrayFlag) {
...
void CFormDfn::getParentDfn(std::vector<const CFormDfn *> &array,
                            uint32 round) const {
  // Checkout recursive calls
  if (round > NLGEORGES_MAX_RECURSION) {
    // Turn around..
...
uint CFormDfn::getNumParent() const { return (uint)Parents.size(); }
...
uint CFormDfn::getNumEntry() const { return (uint)Entries.size(); }

// ***************************************************************************

void CFormDfn::setNumEntry(uint size) { Entries.resize(size); }
...
CFormDfn::CEntry &CFormDfn::getEntry(uint entry) { return Entries[entry]; }

// ***************************************************************************

void CFormDfn::setNumParent(uint size) { Parents.resize(size); }

// ***************************************************************************

void CFormDfn::setParent(uint parent, CFormLoader &loader,
                         const std::string &filename) {
  if (filename.empty())
...
void CFormDfn::CEntry::setType(CFormLoader &loader,
                               const std::string &filename) {
...
void CFormDfn::CEntry::setType(TEntryType type) { TypeElement = type; }

// ***************************************************************************

void CFormDfn::CEntry::setDfn(CFormLoader &loader,
                              const std::string &filename) {
...
const std::string &CFormDfn::CEntry::getName() const { return Name; }

// ***************************************************************************

void CFormDfn::CEntry::setName(const std::string &name) { Name = name; }

// ***************************************************************************

const std::string &CFormDfn::CEntry::getDefault() const { return Default; }

// ***************************************************************************

void CFormDfn::CEntry::setDefault(const std::string &def) { Default = def; }

// ***************************************************************************

void CFormDfn::CEntry::setArrayFlag(bool flag) { Array = flag; }

// ***************************************************************************

bool CFormDfn::CEntry::getArrayFlag() const { return Array; }

// ***************************************************************************

UFormDfn::TEntryType CFormDfn::CEntry::getType() const { return TypeElement; }

// ***************************************************************************

const std::string &CFormDfn::CEntry::getFilename() const { return Filename; }

// ***************************************************************************

void CFormDfn::CEntry::setFilename(const std::string &def) { Filename = def; }

// ***************************************************************************

CType *CFormDfn::CEntry::getTypePtr() { return Type; }

// ***************************************************************************

CFormDfn *CFormDfn::CEntry::getDfnPtr() { return Dfn; }

// ***************************************************************************

const CType *CFormDfn::CEntry::getTypePtr() const { return Type; }

// ***************************************************************************

const CFormDfn *CFormDfn::CEntry::getDfnPtr() const { return Dfn; }
...
bool CFormDfn::getEntryByName(const std::string &name,
...
uint CFormDfn::getNumParents() const { return (uint)Parents.size(); }
...
const std::string &CFormDfn::getComment() const { return Header.Comments; }
...
void CFormDfn::warning(bool exception, const std::string &function,
  ...
  // Set the warning
  NLGEORGES::warning(exception, "(CFormDfn::%s) in form DFN (%s) : %s",
                     function.c_str(), _Filename.c_str(), buffer);
}
  ...
  if (dependencies.insert(toLowerAscii(CFile::getFilename(_Filename))).second) {
    ...
    for (i = 0; i < Entries.size(); i++) {
      ...
      if (Entries[i].getTypePtr()) {
        dependencies.insert(
            toLowerAscii(CFile::getFilename(Entries[i].getFilename())));
      }
    }
  }
}

...
void CType::write(xmlDocPtr doc) const {
  ...
  // Default valid
  if (!Default.empty()) {
    xmlSetProp(node, (const xmlChar *)"Default",
               (const xmlChar *)Default.c_str());
  }
  ...
  // Increment valid
  if (!Increment.empty()) {
    xmlSetProp(node, (const xmlChar *)"Increment",
               (const xmlChar *)Increment.c_str());
  }
  ...
  for (def = 0; def < Definitions.size(); def++) {
    xmlNodePtr defNode =
        xmlNewChild(node, NULL, (const xmlChar *)"DEFINITION", NULL);
    xmlSetProp(defNode, (const xmlChar *)"Label",
               (const xmlChar *)Definitions[def].Label.c_str());
    xmlSetProp(defNode, (const xmlChar *)"Value",
               (const xmlChar *)Definitions[def].Value.c_str());
  }
...
void CType::read(xmlNodePtr root) {
  ...
  if (((const char *)root->name == NULL) ||
      (strcmp((const char *)root->name, "TYPE") != 0)) {
    // Throw exception
    warning2(true, "read",
             "XML Syntax error in block line %d, node (%s) should be TYPE.",
  ...
  if (value) {
    ...
    if (type != TypeCount)
      ...
      // Throw exception
      warning2(true, "read",
               "XML Syntax error in TYPE block line %d, the Type value is "
               "unknown (%s).",
    ...
    // Throw exception
    warning2(true, "read",
             "XML Syntax error in TYPE block line %d, the Type argument was "
             "not found.",
             (sint)root->line);
  }
  ...
  while (child < childrenCount) {
    ...
    if (label) {
      ...
      if (value) {
        ...
        // Throw exception
        warning2(true, "read",
                 "XML Syntax error in DEFINITION block line %d, the Value "
      ...
      // Throw exception
      warning2(true, "read",
               "XML Syntax error in DEFINITION block line %d, the Label "
               "argument was not found.",
               (sint)childPtr->line);
    }
...
const char *CType::TypeNames[TypeCount] = {
    "UnsignedInt", "SignedInt", "Double", "String", "Color",
};

// ***************************************************************************

const char *CType::UITypeNames[UITypeCount] = {
    "Edit",        "EditSpin", "NonEditableCombo",
    "FileBrowser", "BigEdit",  "ColorEdit",
};

// ***************************************************************************

const char *CType::getTypeName(TType type) { return TypeNames[type]; }

// ***************************************************************************

const char *CType::getUIName(TUI type) { return UITypeNames[type]; }
...
public:
  ...
  virtual CEvalNumExpr::TReturnState evalValue(const char *value,
                                               double &result, uint32 round) {
    ...
    if (Form) {
      ...
      if (strcmp(value, "$filename") == 0) {
        ...
        const string filename =
            CFile::getFilenameWithoutExtension(Form->getFilename());
        ...
        for (uint i = 0; i < Type->Definitions.size(); i++) {
          if (!nlstricmp(Type->Definitions[i].Label.c_str(), value)) {
            ...
            sint index;
            return expr.evalExpression(Type->Definitions[i].Value.c_str(),
                                       result, &index, round + 1);
          }
        }
        ...
        // Search for the node
        if (((const CFormElm &)Form->getRootNode())
                .getNodeByName(value, &parentDfn, parentIndex, &nodeDfn,
                               &nodeType, &node, type, array, parentVDfnArray,
                               false, round + 1)) {
          ...
          if (type == UFormDfn::EntryType) {
            ...
            nlassert(nodeType);
            string res;
            if (nodeType->getValue(res, Form, atom, *parentDfn, parentIndex,
                                   UFormElm::Eval, NULL, round + 1, value)) {
              // Request exist ?
              if (requestExist) {
                // Doesn't exist
                result = res.empty() ? 0 : 1;
                return CEvalNumExpr::NoError;
              } else if (((const CFormElm &)Form->getRootNode())
                             .convertValue(result, res)) {
                return CEvalNumExpr::NoError;
              }
            }
...
uint getNextToken(const char *startString, string &token, uint &offset) {
  ...
  if ((startString[offset] == '$') &&
      (strncmp(startString + offset + 1, "filename", 8) == 0)) {
    offset += 9;
...
bool CType::getValue(string &result, const CForm *form,
                     const CFormElmAtom *node, const CFormDfn &parentDfn,
                     uint parentIndex, UFormElm::TEval evaluate, uint32 *where,
                     uint32 round, const std::string &formName) const {
  if (round > NLGEORGES_MAX_RECURSION) {
    // Turn around..
    warning2(false, "getDefinition",
             "Recurcive call on the same DFN, look for loop inheritances.");
    return false;
  }
  ...
  if (node && !node->Value.empty()) {
    if (where)
      *where = (node->Form == form) ? CFormElm::ValueForm
                                    : CFormElm::ValueParentForm;
    result = node->Value;
  }
  ...
  if (evaluate == UFormElm::Formula) {
    ...
    if ((Type == Double) || (Type == SignedInt) || (Type == UnsignedInt)) {
      ...
      int offset;
      CEvalNumExpr::TReturnState error =
          expr.evalExpression(result.c_str(), value, &offset, round + 1);
      if (error == CEvalNumExpr::NoError) {
        ...
        warning(false, formName, form->getFilename().c_str(), "getValue",
      ...
      while (offset < result.size()) {
        ...
        if (tokenType == NL_TOKEN_OPEN_BRACKET) {
          ...
          if (nextEnd == 0xffffffff) {
            ...
            // Format code ?
            if (((nextEnd - offset) >= 3) && (result[offset] == '$') &&
                (result[offset + 1] == 'z') && (result[offset + 2] <= '9') &&
                (result[offset + 2] >= '0')) {
              // Save padding
            ...
            CMyEvalNumExpr expr(form, this);
            int offsetExpr;
            CEvalNumExpr::TReturnState error = expr.evalExpression(
                valueName.c_str(), value, &offsetExpr, round + 1);
            if (error == CEvalNumExpr::NoError) {
              // To string
              char format[200];
              char result[200];
          ...
          if (nextEnd == 0xffffffff) {
            ...
            // Search for the node
            if (((const CFormElm &)form->getRootNode())
                    .getNodeByName(valueName, &parentDfn, parentIndex, &nodeDfn,
                                   &nodeType, &node, type, array,
                                   parentVDfnArray, false, round + 1)) {
              // End, return the current index
              if (type == UFormDfn::EntryType) {
                // The atom
                const CFormElmAtom *atom =
                ...
                nlassert(nodeType);
                string result2;
                if (nodeType->getValue(result2, form, atom, *parentDfn,
                                       parentIndex, UFormElm::Eval, NULL,
                                       round + 1, valueName.c_str())) {
                  dest += result2;
                }
              } else {
                char msg[512];
                buildError(msg, offset);
                warning(false, formName, form->getFilename().c_str(),
                        "getValue", "Node is not an atom (%s)\n%s\n%s",
                        valueName.c_str(), result.c_str(), msg);
                return false;
              }
            } else
              return false;
...
bool CType::uiCompatible(TType type, TUI ui) {
  ...
  case String:
    return (ui == Edit) || (ui == NonEditableCombo) || (ui == FileBrowser) ||
           (ui == BigEdit);
  case Color:
    return (ui == ColorEdit);
  default:
...
void CType::warning(bool exception, const std::string &formName,
  ...
  // Set the warning
  NLGEORGES::warning(exception, "(CType::%s) In form (%s) in node (%s) : %s",
                     function.c_str(), formFilename.c_str(), formName.c_str(),
                     buffer);
}

// ***************************************************************************

void CType::warning2(bool exception, const std::string &function,
...
UType::TType CType::getType() const { return Type; }

// ***************************************************************************

const string &CType::getDefault() const { return Default; }

// ***************************************************************************

const string &CType::getMin() const { return Min; }

// ***************************************************************************

const string &CType::getMax() const { return Max; }

// ***************************************************************************

const string &CType::getIncrement() const { return Increment; }

// ***************************************************************************

uint CType::getNumDefinition() const { return (uint)Definitions.size(); }

// ***************************************************************************

bool CType::getDefinition(uint index, std::string &label,
  ...
  warning2(false, "getDefinition", "Index out of bounds (%d >= %d)", index,
           Definitions.size());
  return false;
}
...
const string &CType::getComment() const { return Header.Comments; }

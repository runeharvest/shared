...
UFormElm &CForm::getRootNode() { return Elements; }

// ***************************************************************************

const UFormElm &CForm::getRootNode() const { return Elements; }
...
void CForm::write(xmlDocPtr doc, const std::string &filename) {
  ...
  for (uint parent = 0; parent < ParentList.size(); parent++) {
    ...
    if (!(ParentList[parent].ParentFilename.empty())) {
      ...
      xmlSetProp(parentNode, (const xmlChar *)"Filename",
                 (const xmlChar *)ParentList[parent].ParentFilename.c_str());
    }
  }
...
void CForm::readParent(const char *parent, CFormLoader &loader) {
  ...
  if (theParent != NULL) {
    ...
    if (!insertParent(getParentCount(), parent, theParent)) {
      ...
      // Throw exception
      warning(true, "readParent",
              "Can't set the parent FORM named (%s). Check if it is the same "
              "form or if it use a differnt formDfn.",
              parentName.c_str());
    }
  } else {
    // Make an error message
    ...
    // Throw exception
    warning(true, "readParent", "Can't load the parent FORM named (%s).",
            parentName.c_str());
  }
}
...
void CForm::read(xmlNodePtr node, CFormLoader &loader, CFormDfn *dfn,
  ...
  // Check node name
  if (((const char *)node->name == NULL) ||
      (strcmp((const char *)node->name, "FORM") != 0)) {
    // Make an error message
    warning(true, "read",
            "XML Syntax error in block line %d, node (%s) should be FORM.",
  ...
  if (child == NULL) {
    // Throw exception
    warning(true, "read",
            "Syntax error in block line %d, node (%s) should have a STRUCT "
            "child node.",
            (sint)node->line, node->name);
  }
...
const std::string &CForm::getComment() const { return Header.Comments; }
...
bool CForm::insertParent(uint before, const std::string &filename,
                         CForm *parent) {
  // Set or reset ?
  nlassert(parent);
  ...
  if (parent->Elements.FormDfn == Elements.FormDfn) {
    // Set members
    std::vector<CParent>::iterator ite =
        ParentList.insert(ParentList.begin() + before, CParent());
    ite->Parent = parent;
    ...
    // Output an error
    warning(false, "insertParent",
            "Can't insert parent form (%s) that has not the same DFN.",
            filename.c_str());
  }
...
CForm *CForm::getParent(uint parent) const { return ParentList[parent].Parent; }
...
uint CForm::getParentCount() const { return (uint)ParentList.size(); }

// ***************************************************************************

void CForm::clean() { clearParents(); }

// ***************************************************************************

void CForm::clearParents() { ParentList.clear(); }

// ***************************************************************************

const std::string &CForm::getFilename() const { return _Filename; }

// ***************************************************************************

void CForm::warning(bool exception, const std::string &function,
  ...
  // Set the warning
  NLGEORGES::warning(exception, "(CForm::%s) in form (%s) : %s",
...
uint CForm::getNumParent() const { return getParentCount(); }

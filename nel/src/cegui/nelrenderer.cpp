...
extern "C" {
CEGUI::Renderer *createNeLRendererInstance(NL3D::UDriver *driver,
                                           bool withRP = true) {
  return new CEGUI::NeLRenderer(driver, withRP);
}
} // extern "C"
...
NeLRenderer::NeLRenderer(NL3D::UDriver *driver, bool withRP,
  ...
  m_InputDriver.setScreenMode(d_display_area.getWidth(),
                              d_display_area.getHeight(), (float)mode.Depth);
...
//	void NeLRenderer::addSearchPath(const std::string &path, bool recurse,
//bool alternative, class NLMISC::IProgressCallback *progressCallBack) {
//		NLMISC::CPath::addSearchPath(path.c_str(),recurse,alternative,progressCallBack);
//	}

void NeLRenderer::addQuad(const Rect &dest_rect, float z, const Texture *tex,
  ...
  // set quad position, flipping y co-ordinates, and applying appropriate texel
  // origin offset
...
void NeLRenderer::renderQuad(NLMISC::CQuadColorUV quad, NL3D::UMaterial mat) {
  ...
  m_Driver->setFrustum(NL3D::CFrustum(0, d_display_area.getWidth(),
                                      d_display_area.getHeight(), 0, -1, 1,
                                      false));
...
void NeLRenderer::doRender(void) {
  ...
  for (QuadVector::iterator itr = d_quadlist.begin(); itr != d_quadlist.end();
       itr++) {
...
void NeLRenderer::clearRenderList(void) { d_quadlist.clear(); }
...
void NeLRenderer::setQueueingEnabled(bool setting) { d_queueing = setting; }

bool NeLRenderer::isQueueingEnabled(void) const { return d_queueing; }

NLMISC::CRGBA NeLRenderer::colorToNeL(CEGUI::colour color) {
  NLMISC::CRGBA ctmp;
  ctmp.set((uint8)(color.getRed() * 255), (uint8)(color.getGreen() * 255),
           (uint8)(color.getBlue() * 255), (uint8)(color.getAlpha() * 255));
  return ctmp;
}
...
ImageCodec &NeLRenderer::getImageCodec(void) { return *m_ImageCodec; }
...
void NeLRenderer::setupImageCodec(const String &codecName) {
  ...
  ImageCodec *(*createFunc)(void) =
      (ImageCodec * (*)(void))
          m_ImageCodecModule->getSymbolAddress("createImageCodec");
  m_ImageCodec = createFunc();
}

void NeLRenderer::cleanupImageCodec() {
  if (m_ImageCodec && m_ImageCodecModule) {
    void (*deleteFunc)(ImageCodec *) = (void (*)(
        ImageCodec *))m_ImageCodecModule->getSymbolAddress("destroyImageCodec");

...
void NeLTexture::loadFromFile(const String &filename,
                              const String &resourceGroup) {
  String file = NLMISC::CPath::lookup(filename.c_str()).c_str();

  // this object can only contain one texture at a time, free the old one if it
  // exists.
  freeNeLTexture();
  ...
  m_Material.setBlendFunc(NL3D::UMaterial::srcalpha,
...
void NeLTexture::loadFromMemory(const void *buffPtr, uint buffWidth,
  ...
  // create the texture
  m_TextureMem =
      new NL3D::CTextureMem(pTmpBuf, buffWidth * buffHeight * size, true, false,
                            buffWidth, buffHeight, NLMISC::CBitmap::RGBA);
  m_TextureMem->setWrapS(NL3D::ITexture::Clamp);
  m_TextureMem->setWrapT(NL3D::ITexture::Clamp);
  m_TextureMem->setFilterMode(NL3D::ITexture::Linear,
                              NL3D::ITexture::LinearMipMapOff);
  ...
  /**
   * Configure the material. This is a little more complicated than loading a
   * texture from a file, since CTextureFileUser and CTextureFile take care of
   * prepping all of the necessary alpha settings.
   */
  ...
  m_Material.texEnvArg0RGB(0, NL3D::UMaterial::Texture,
                           NL3D::UMaterial::SrcColor);
  m_Material.texEnvArg1RGB(0, NL3D::UMaterial::Diffuse,
                           NL3D::UMaterial::SrcColor);
  m_Material.texEnvOpAlpha(0, NL3D::UMaterial::Modulate);
  m_Material.texEnvArg0Alpha(0, NL3D::UMaterial::Texture,
                             NL3D::UMaterial::SrcAlpha);
  m_Material.texEnvArg1Alpha(0, NL3D::UMaterial::Diffuse,
                             NL3D::UMaterial::SrcAlpha);
  m_Material.setZFunc(NL3D::UMaterial::always);
...
NL3D::UMaterial NeLTexture::getNeLTexture(void) { return m_Material; }
...
void NeLTexture::freeNeLTexture(void) {
  ...
  if (m_UsingFile == true) {
    ...
    nlwarning("Something has gone horribly wrong, unable to free any type of "
              "texture.");
  }
}

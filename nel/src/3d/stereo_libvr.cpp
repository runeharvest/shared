...
CStereoLibVR::CStereoLibVR(const CStereoLibVRDeviceHandle *handle)
    : m_Stage(0), m_SubStage(0), m_OrientationCached(false), m_Driver(NULL),
      m_BarrelTexU(NULL), m_PixelProgram(NULL),
  ...
  if (m_DevicePtr->HMDDevice) {
    ...
    nldebug("LibVR: HScreenSize: %f, VScreenSize: %f",
            m_DevicePtr->HMDInfo.h_screen_size,
            m_DevicePtr->HMDInfo.v_screen_size);
    nldebug("LibVR: VScreenCenter: %f", m_DevicePtr->HMDInfo.v_center);
    nldebug("LibVR: EyeToScreenDistance: %f",
            m_DevicePtr->HMDInfo.eye_to_screen[0]);
    nldebug("LibVR: LensSeparationDistance: %f",
            m_DevicePtr->HMDInfo.lens_separation);
    nldebug("LibVR: HResolution: %i, VResolution: %i",
            m_DevicePtr->HMDInfo.h_resolution,
            m_DevicePtr->HMDInfo.v_resolution);
    nldebug("LibVR: DistortionK[0]: %f, DistortionK[1]: %f",
            m_DevicePtr->HMDInfo.distortion_k[0],
            m_DevicePtr->HMDInfo.distortion_k[1]);
    nldebug("LibVR: DistortionK[2]: %f, DistortionK[3]: %f",
            m_DevicePtr->HMDInfo.distortion_k[2],
            m_DevicePtr->HMDInfo.distortion_k[3]);
...
void CStereoLibVR::setDriver(NL3D::UDriver *driver) {
  ...
  if (drvInternal->supportPixelProgram(CPixelProgram::fp40) &&
      drvInternal->supportBloomEffect() &&
      drvInternal->supportNonPowerOfTwoTextures()) {
    nldebug("VR: fp40");
    m_PixelProgram = new CPixelProgram(g_StereoOVR_fp40);
  } else if (drvInternal->supportPixelProgram(CPixelProgram::arbfp1) &&
             drvInternal->supportBloomEffect() &&
             drvInternal->supportNonPowerOfTwoTextures()) {
    nldebug("VR: arbfp1");
  ...
  if (m_PixelProgram) {
    ...
    m_BarrelTex->resize(m_DevicePtr->HMDInfo.h_resolution,
                        m_DevicePtr->HMDInfo.v_resolution);
...
void CStereoLibVR::initCamera(uint cid, const NL3D::UCamera *camera) {
  ...
  float projectionCenterOffset =
      st_conf.proj.projection_offset * 0.5 *
      (m_LeftFrustum[cid].Right - m_LeftFrustum[cid].Left);
...
void CStereoLibVR::getCurrentMatrix(uint cid, NL3D::UCamera *camera) const {
  CMatrix translate;
  if (m_Stage % 2)
    translate.translate(CVector(
        (m_DevicePtr->InterpupillaryDistance * m_Scale) * -0.5f, 0.f, 0.f));
  else
    translate.translate(CVector(
        (m_DevicePtr->InterpupillaryDistance * m_Scale) * 0.5f, 0.f, 0.f));
...
bool CStereoLibVR::beginRenderTarget() {
  ...
  if (m_Driver && m_Stage == 1 &&
    ...
    static_cast<CDriverUser *>(m_Driver)->setRenderTarget(*m_BarrelTexU, 0, 0,
                                                          0, 0);
    return true;
  }
  return false;
...
bool CStereoLibVR::endRenderTarget() {
       ...
       UDriver::Filled)) // set to 4 to turn off distortion of 2d gui
  {
    ...
    drvInternal->setPixelProgramConstant(1, screenCenterX, screenCenterY, 0.f,
                                         0.f);
...
/// Get GUI shift
void CStereoLibVR::getInterface2DShift(uint cid, float &x, float &y,
  ...
  NLMISC::CVector ipd;
  if (m_Stage % 2)
    ipd = CVector((m_DevicePtr->InterpupillaryDistance * m_Scale) * -0.5f, 0.f,
                  0.f);
  else
    ipd = CVector((m_DevicePtr->InterpupillaryDistance * m_Scale) * 0.5f, 0.f,
                  0.f);
  ...
  CVector proj =
      CStereoLibVR::getCurrentFrustum(cid).project(vec + ipd + rotshift);
...
void CStereoLibVR::releaseLibrary() { nlassert(s_DeviceCounter == 0); }

bool CStereoLibVR::isDeviceCreated() { return m_DevicePtr->HMDDevice != NULL; }

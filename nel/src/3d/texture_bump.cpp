...
// Map that give the normalization factor for each map from its sharename. This
// avoid to generate several time the maps to get the normalization factor if a
// bumpmap is shared by severals CTextureBump instances;
struct CNormalizationInfo {
  uint NumRefs;
  float NormalizationFactor;
};
typedef std::map<std::string, CNormalizationInfo>
    TNameToNI; // sharename to the normalization factor
class CNameToNFStatic {
public:
  TNameToNI Map;
  bool Initialized;
  CNameToNFStatic() : Initialized(true) {}
  ~CNameToNFStatic() { Initialized = false; }
};
...
#define GET_HGT(x, y)                                                          \
  ((sint)((src[(uint)(x) % width + ((uint)(y) % height) * width] &             \
           0x00ff00) >>                                                        \
          8))
...
/// create a rgba gradient texture from a height map (red component of a rgba
/// bitmap)
static void BuildDsDtAsRGBA(uint32 *src, sint width, sint height,
                            uint32 *dest) {
#define GET_HGT(x, y)                                                          \
  ((sint)((src[(uint)(x) % width + ((uint)(y) % height) * width] &             \
           0x00ff00) >>                                                        \
  ...
  for (x = 0; x < width; ++x) {
    for (y = 0; y < height; ++y) {
      ...
      sint16 ds = (sint16)(GET_HGT(x + 1, y) - GET_HGT(x - 1, y));
      sint16 dt = (sint16)(GET_HGT(x, y + 1) - GET_HGT(x, y - 1));
      dest[off] = 0xff000000 | (uint32)((ds + 0x80) & 0xff) |
                  (uint32)((dt + 0x80) << 8);
    }
  }
}

/// Normalize a DsDt texture after it has been built, and return the
...
static float NormalizeDsDt(uint16 *src, sint width, sint height) {
  ...
  for (k = 0; k < size; ++k) {
    highestDelta =
        std::max(highestDelta, (uint)::abs((sint)(sint8)(src[k] & 255)));
    highestDelta =
        std::max(highestDelta, (uint)::abs((sint)(sint8)(src[k] >> 8)));
  }
...
static float NormalizeDsDtAsRGBA(uint32 *src, sint width, sint height) {
  ...
  for (k = 0; k < size; ++k) {
    highestDelta =
        std::max(highestDelta, (uint)abs((sint8)((src[k] & 0xff) - 0x80)));
    highestDelta = std::max(highestDelta,
                            (uint)abs((sint8)(((src[k] >> 8) & 0xff) - 0x80)));
  }
...
CTextureBump::CTextureBump()
    : _NormalizationFactor(NULL), _DisableSharing(false),
...
void CTextureBump::setFilterMode(TMagFilter magf, TMinFilter minf) {
  nlstop; // set filter mode not allowed with bump textures (not supported by
          // some GPUs)
}
...
void CTextureBump::serial(NLMISC::IStream &f) {
  ...
  if (ver >= 1) {
    bool oldFlag = false;
    f.serial(oldFlag); // backaward compatibility : serial the old
                       // "forceAbsoluteOffset" flag
  }
  if (ver >= 2) {
...
void CTextureBump::doGenerate(bool async) {
  ...
  if (getUploadFormat() == RGBA8888) {
    CBitmap::resize(_HeightMap->getWidth(), _HeightMap->getHeight(),
                    CBitmap::RGBA);
  } else {
    CBitmap::resize(_HeightMap->getWidth(), _HeightMap->getHeight(),
                    CBitmap::DsDt);
  ...
  // build the DsDt map
  if (getUploadFormat() == RGBA8888) {
    BuildDsDtAsRGBA((uint32 *)&(_HeightMap->getPixels()[0]), width, height,
                    (uint32 *)&(getPixels()[0]));
  } else {
    BuildDsDt((uint32 *)&(_HeightMap->getPixels()[0]), width, height,
              (uint16 *)&(getPixels()[0]));
  }
  ...
  if (_ForceNormalize) {
    if (getUploadFormat() == RGBA8888) {
      normalizationFactor =
          NormalizeDsDtAsRGBA((uint32 *)&(getPixels()[0]), width, height);
    } else {
      normalizationFactor =
          NormalizeDsDt((uint16 *)&(getPixels()[0]), width, height);
    }
  }
  ...
  if (it == s_NameToNF.Map.end()) {
    ...
    std::pair<TNameToNI::iterator, bool> pb =
        s_NameToNF.Map.insert(TNameToNI::value_type(shareName, ni));
...
///==============================================================================================
CTextureBump::~CTextureBump() {
  if (s_NameToNF.Initialized && _NormalizationFactor &&
      !s_NameToNF.Map.empty()) {

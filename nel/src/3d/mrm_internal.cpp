...
// ***************************************************************************
sint CMRMSewingMesh::mustCollapseEdge(uint lod, const CMRMEdge &edge,
...
// ***************************************************************************
void CMRMSewingMesh::build(const CMesh::CInterface &meshInt, uint nWantedLods,
                           uint divisor) {
  ...
  */
  ...
  // build how many edges the coarsest lod will have. At least 3 edge for a
  ...
  // be the coarsest mesh (no collapse)
  for (uint lod = nWantedLods - 1; lod > 0; lod--) {
    // Linear.
    sint nCurEdges = (sint)floor(0.5f + nBaseEdges +
                                 (nMaxEdges - nBaseEdges) * (float)(lod - 1) /
                                     (nWantedLods - 1));
    ...
    while (nCurEdges < (sint)edgeList.size()) {
      ...
      for (uint j = 0; j < edgeList.size(); j++) {
        ...
        CVector edgeDelta = (meshInt.Vertices[edgeList[j].v1].Pos -
                             meshInt.Vertices[edgeList[j].v0].Pos);
        ...
        CVector precEdgeDir = (meshInt.Vertices[edgeList[precEdgeId].v1].Pos -
                               meshInt.Vertices[edgeList[precEdgeId].v0].Pos);
        ...
        /* The more the edge is linear to the previous edge, the more we can
           collapse it. If totaly linear (curEdgeDir*precEdgeDir==1), it should
           be a good idea to collapse it now. But the fact is that this is an
           interface and we don't know what kind of mesh share those edges (I
           mean it is possible that neighbors faces are absolutely not
           coplanar). So don't add to much importance on Angular Part
        */
      ...
      // changes vert ids of the prec edge. eg: edge(12) is deleted=> 01
      // 12 23... becomes 02 23 (NB: 1 is collapsed to 2)
      uint precEdgeId =
          (uint)((bestEdgeId + edgeList.size() - 1) % edgeList.size());

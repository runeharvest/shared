...
// ------------------------------------------------------------------------------------------------
void CCloud::init(uint32 nVoxelW, uint32 nVoxelH, uint32 nVoxelD,
                  float rBaseFreq, uint32 nNbOctave) {
  if (_UStart != NULL)
    return;
...
void CCloud::generate(CNoise3d &noise) {
  ...
  _Driver->setRenderTarget(_CloudTexTmp->Tex, 0, 0, _Width * _NbW,
                           _Height * _NbH);
  _Driver->setFrustum(0, (float)_CloudTexTmp->Tex->getWidth(), 0,
                      (float)_CloudTexTmp->Tex->getHeight(), -1, 1, false);
  ...
  uint32 nVSize = rVB.getVertexSize();
  {
    ...
    *pVertices = CVector(0.0f, 0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)_NbW * _Width, 0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)_NbW * _Width, (float)_NbH * _Height, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
  ...
  for (nOct = 0; nOct < _NbOctave; ++nOct) {
    ...
    noise.renderGrid(_NbW, _NbH, _Width, _Height, (float)_UStart[nOct],
                     (float)_VStart[nOct], (float)_WStart[nOct], dU, dV, dW,
                     1.0f / (2 << nOct));
    ...
    for (j = 0; j < _NbH; ++j)
    {
        for (i = 0; i < _NbW; ++i)
        {
            qc.V0 = CVector((float)i*_Width,		(float)j*_Height,
    0.0f); qc.V1 = CVector((float)(i+1)*_Width,	(float)j*_Height,
    0.0f); qc.V2 = CVector((float)(i+1)*_Width,	(float)(j+1)*_Height,	0.0f);
            qc.V3 = CVector((float)i*_Width,		(float)(j+1)*_Height,
    0.0f); noise.render (qc,
    (float)_WStart[nOct]+dW*(i+(float)j*_NbW)/(((float)_NbW)*_NbH), 1.0f/(2<<nOct));
        }
    }*/
    noise.flush();
  }

  // Apply attenuation texture (not needed to resetup position again (done when
  // clearing to black))
  {
    CVertexBufferReadWrite vba;
    rVB.lock(vba);
  ...
  uint8 colpow =
      (uint8)(255 -
              (((uint32)CloudPower * (255 - (uint32)CloudDistAtt)) / 255));
  ...
  _CloudTexTmp->Tex->setFilterMode(ITexture::Nearest,
...
void CCloud::light() {
  ...
  // Destination position for lighting accumulation buffer from (0, 0) size
  // (_Width, _Height)
  CVertexBuffer &rVB = _CloudScape->_VertexBuffer;
  uint32 nVSize = rVB.getVertexSize();
  {
    ...
    rVB.lock(vba);
    CVector *pVertices = vba.getVertexCoordPointer(0);
    *pVertices = CVector((float)0.0f, (float)0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)1.f, (float)0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)1.f, (float)1.f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
  ...
  for (j = 0; j < _NbH; ++j) {
    for (i = 0; i < _NbW; ++i) {
      ...
      if ((i + j) > 0) {
        {
          ...
          pUV->U = previ * oneOverNbW;
          pUV->V = prevj * oneOverNbH;
          pUV = (CUV *)(((uint8 *)pUV) + nVSize);
          pUV->U = (previ + 1) * oneOverNbW;
          pUV->V = prevj * oneOverNbH;
          pUV = (CUV *)(((uint8 *)pUV) + nVSize);
          pUV->U = (previ + 1) * oneOverNbW;
...

      {
        ...
        pUV->V = j * oneOverNbH;
        pUV = (CUV *)(((uint8 *)pUV) + nVSize);
        pUV->U = (i + 1) * oneOverNbW;
        pUV->V = j * oneOverNbH;
        pUV = (CUV *)(((uint8 *)pUV) + nVSize);
        pUV->U = (i + 1) * oneOverNbW;
        pUV->V = (j + 1) * oneOverNbH;
        pUV = (CUV *)(((uint8 *)pUV) + nVSize);
        pUV->U = i * oneOverNbW;
        pUV->V = (j + 1) * oneOverNbH;
      }
      ...
      /* The copy target trick. If not available, render the temporary texture
       * in Tex2 */
      if (!_Driver->copyTargetToTexture(_CloudTexTmp->Tex2, i * _Width,
                                        j * _Height, 0, 0, 0, 0, 0)) {
        _Driver->setRenderTarget(_CloudTexTmp->Tex2, i * _Width, j * _Height,
                                 _Width, _Height);
        viewport.init((float)i * oneOverNbW, (float)j * oneOverNbH, oneOverNbW,
                      oneOverNbH);
        _Driver->setupViewport(viewport);
...
void CCloud::reset(NL3D::CCamera *pViewer) {
  if (_BillSize != 4) {
    ...
    _TexBill = new CTextureMem(_MemBill, 4 * _BillSize * _BillSize, true, false,
                               _BillSize, _BillSize);
  ...
  if (_OldBillSize != 4) {
    ...
    _TexOldBill = new CTextureMem(_MemOldBill, 4 * _OldBillSize * _OldBillSize,
                                  true, false, _OldBillSize, _OldBillSize);
  ...
  CVertexBuffer &rVB = _CloudScape->_VertexBuffer;
  {
    ...
    *pVertices = CVector(0.0f, 0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector(5.0f, 0.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector(5.0f, 5.0f, 0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
  ...
  CVector Center =
      CVector(_Pos.x + _Size.x / 2, _Pos.y + _Size.y / 2, _Pos.z + _Size.z / 2);
  CVector Size = _Size;
  CVector I, J, K;
...
void CCloud::dispXYZ(CMaterial *pMat) {
  ...
  for (j = 0; j < _NbH; ++j) {
    for (i = 0; i < _NbW; ++i) {
...

      {
        ...
        *pVertices =
            CVector(_Pos.x, _Pos.y,
                    _Pos.z + _Size.z * (_NbW * _NbH - d) * oneOverNbWNbH);
        pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
        *pVertices =
            CVector(_Pos.x + _Size.x, _Pos.y,
                    _Pos.z + _Size.z * (_NbW * _NbH - d) * oneOverNbWNbH);
        pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
        *pVertices =
            CVector(_Pos.x + _Size.x, _Pos.y + _Size.y,
                    _Pos.z + _Size.z * (_NbW * _NbH - d) * oneOverNbWNbH);
        pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
        *pVertices =
            CVector(_Pos.x, _Pos.y + _Size.y,
                    _Pos.z + _Size.z * (_NbW * _NbH - d) * oneOverNbWNbH);
...

  {
    ...
    rVB.lock(vba);
    CVector *pVertices = vba.getVertexCoordPointer(0);
    *pVertices = CVector((float)0.25f, 0, (float)0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)0.75f, 0, (float)0.0f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
    *pVertices = CVector((float)0.75f, 0, (float)0.75f);
    pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
...
void CCloud::calcBill(const CVector &Viewer, const CVector &Center,
                      const CVector &Size, CVector &I, CVector &J, CVector &K,
...
void CCloud::genBill(CCamera *pCam, uint32 nBillSize) {
  ...
  viewport.init(0.0f, 0.0f, ((float)_BillSize + 1) / ((float)textureW),
                ((float)_BillSize + 1) / ((float)textureH));
  _Driver->setupViewport(viewport);
  ...
  uint32 nVSize = rVB.getVertexSize();
  {
    ...
    rVB.lock(vba);
    {
      ...
      *pVertices = CVector(0.0f, 0.0f, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
      *pVertices = CVector(1.0f, 0.0f, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
      *pVertices = CVector(1.0f, 0.0f, 1.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
...
void CCloud::dispBill(CCamera *pCam) {
  ...
  /*		Center= _BillCenter*((float)(Trans)/(float)TransTotal) +
      ...
      calcBill (Viewer, Center, Size, I, J, K, Left, Right, Top, Bottom, Near,
     Far);
  ...
  _CloudScape->_MatBill.setColor(
      CRGBA(255, 255, 255, (uint8)(255 * ((float)Time / (float)FuturTime))));
  CVertexBuffer &rVB = _CloudScape->_VertexBuffer;
  {
    CVertexBufferReadWrite vba;
    rVB.lock(vba);

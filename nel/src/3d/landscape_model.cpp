...
void CLandscapeModel::registerBasic() {
  CScene::registerModel(LandscapeModelId, TransformId,
...
void CLandscapeModel::traverseHrc() {
  ...
  /* Compile Animate PointLight here.
      NB: Animated PointLight are important for Dynamic Object lighted by
     landscape lights. The landscape and vegetable are not influenced by thoses
     animation (as if the light were not animated) NB: important to do before
     the LightTrav. Also even if landscape is hidden, still must animate those
     light?
  */
...
void CLandscapeModel::traverseClip() {
  ...
  /// if already clipped in this frame, This means that we are visible from 2
  /// CCluster.
  if (_ClipDate == clipTrav.CurrentDate) {
    ...
    if (!ClusteredPyramidIsFrustum) {
      /* If the current pyramid is different from the last stored pyramid, this
       mean per exemple that the player is in a room and look the landscape from
       2 windows through portals. The other case is that the camera is in 2
       clusters, so the clusterSystem start from this 2, and so we arrive here 2
       times (but through the same portal => same pyramid obviously...)
       */
      if (ClusteredPyramid != clipTrav.WorldPyramid) {
        // in this rare case take the Whole frustum pyramid (a more correct code
        // is to make an Union of the 2 pyramids...)
        ClusteredPyramid = clipTrav.WorldFrustumPyramid;
        ClusteredPyramidIsFrustum = true;
      }
    }
  } else {
    // If the camera is in Rootcluster or visible through a protal from one Room
    // Cluster only, use the clustered pyramid.
    ClusteredPyramid = clipTrav.WorldPyramid;
    // We are not sure we are the FrustumPyramid
    ClusteredPyramidIsFrustum = false;
  }

  // Call normal CTransform traverseClip to insert this model in render list
  // etc... NB: CTransform::traverseClip() update _ClipDate...
...
void CLandscapeModel::clipAndRenderLandscape() {
  /*
      NB: For best Optimisation, it is important that the Clip act at same time
     as the Render. Why? For complex GPU/CPU synchronisation.

      Since the landscape is rendered at end of the opaque pass, the next clip
     of the next frame will arise too early (if we assume no transparent or
     out-scene rendering), RESULTING IN A LOCK => A CPU STALL!!

      In a "landscape only" program, this is not easily avoidable. But in common
     programs with other opaque meshs, skinning etc... we'll have lot of CPU/GPU
     Work between this clip and the Landscape Render. And since the clip() lock
     the VB, and stall, all those tasks won't be parralelized.

      I have already seen such a stall (3 ms).

      Therefore, all Landscape VB Lock arise at the same time=> The GPU will
     render the landscape while the CPU will prepare render and do skinning of
     the next opaque frame for instance.
  */
  ...
  // Yes, this is ugly, but the clip pass is finished in render(), for clipping
  // TessBlocks. This saves another Landscape patch traversal, so this is
  // faster... Order them in order which clip faster (first horizontal, then
  // vertical). NB: TessBlock are ALWAYS clipped with the frustum pyramid, not
  // the clustered one (faster clip for most common cases).
  ...
  // Change the landscape center. All Geomorphed pos (in VertexBuffer only or
  // during VertexProgram) subtract this position.
  Landscape.setPZBModelPosition(renderTrav.CamPos);

  /* setup the model matrix
      ZBuffer Precision: set the modelMatrix to the current landscape
     PZBModelPosition. NB: don't use renderTrav.CamPos directly because
     setPZBModelPosition() may modify the position
  */
  ...
  nlassert(scene);
  {
    ...
    if (scene->isLightingSystemEnabled()) {
      ...
      Landscape.setupVegetableLighting(scene->getSunAmbient(),
                                       scene->getSunDiffuse(),
                                       scene->getSunDirection());

      // current visible Dynamic light list are registered in
      // LightTrav::LightingManager
      CLightTrav &lightTrav = scene->getLightTrav();
      // Get all dynamic light list, and light landscape with it.
      Landscape.computeDynamicLighting(
          lightTrav.LightingManager.getAllDynamicLightList());
    }
  }
  ...
  Landscape.render(refineCenter, renderTrav.CamLook, CurrentPyramid,
                   isAdditive());
...
void CLandscapeModel::traverseRender() {
  ...
  // No-Op. But delay the clip and render to the end of Opaque Rendering. For
  // VBLock optim
...
// ***************************************************************************
void CLandscapeModel::profileRender() { Landscape.profileRender(); }
...
void CLandscapeModel::getReceiverBBox(CAABBox &bbox) {
  ...
  float hs =
      Landscape.getTileNear() * 1.5f; // Enlarge a little because may have some
                                      // triangles outside this dist.
  bbox.setHalfSize(CVector(hs, hs, hs));
}

// ***************************************************************************
void CLandscapeModel::receiveShadowMap(CShadowMap *shadowMap,
  ...
  // render.
  Landscape.receiveShadowMap(drv, shadowMap, casterPos, shadowMat,
                             Landscape.getPZBModelPosition());
}

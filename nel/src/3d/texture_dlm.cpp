...
CTextureDLM::CTextureDLM(uint width, uint height) {
  ...
  // NB: this is simplier like that, and this is not a problem, since actually
  // only 256Ko is allocated :o)
  setReleasable(false);
  // create the bitmap.
  ...
  // Since NL_DLM_BLOCK_SIZE is 10 or 18 (a factor of prime number 5 or 3
  // respectively), we are sure there is at least one pixel which is not used by
  // blcks. The last pixel is filled with black (see CTextureDLm doc)
...
// ***************************************************************************
void CTextureDLM::modulateAndfillRect565(uint x, uint y, uint w, uint h,
                                         CRGBA *textMap, uint16 *modColor) {
...
// ***************************************************************************
void CTextureDLM::modulateAndfillRect8888(uint x, uint y, uint w, uint h,
...
// ***************************************************************************
void CTextureDLM::modulateConstantAndfillRect(uint x, uint y, uint w, uint h,
                                              CRGBA *textMap, CRGBA mc) {
  // compute start dst to copy.
  CRGBA *dst = (CRGBA *)getPixels().getPtr();
  dst += y * getWidth() + x;
...
void CTextureDLM::releaseLightMap(uint x, uint y) {
  ...
  // If wasFull and now it is empty (nLMapPerBlock==1 case), just append to
  // EmptyBlocks.
  if (wasFull && isEmpty) {
  ...
  // if was not full but now it is empty, must remove from free list and insert
  // into EmptyBlocks
  else if (!wasFull && isEmpty) {

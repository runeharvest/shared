...
CMeshInstance::CMeshInstance() {
  ...
  // LoadBalancing is not useful for Mesh, because meshs cannot be reduced in
  // faces. Override CTransformShape state.
...
void CMeshInstance::registerBasic() {
  CScene::registerModel(MeshInstanceId, MeshBaseInstanceId,
...
// ***************************************************************************
bool CMeshInstance::intersectSkin(const CMatrix &toRaySpace, float &dist2D,
...
void CMeshInstance::generateShadowMap(const CVector &lightDir) {
  ...
  _ShadowMap->buildCasterCameraMatrix(lightDir, localPosMatrix, bbShape,
                                      cameraMatrix);
  ...
  driver->activeVertexBuffer(
      const_cast<CVertexBuffer &>(mesh->getVertexBuffer()));
  ...
  // Use the 3/4 bottom of the BBox minus the light direction in XY. NB: little
  // hack: suppose no Rotate (but Z) and no scale
  CVector ldir = lightDir;
  ldir.z = 0;
  ldir.normalize();
  float lenXY =
      (CVector(bbShape.getHalfSize().x, bbShape.getHalfSize().y, 0)).norm();
...
// ***************************************************************************
CShadowMap *CMeshInstance::getShadowMap() { return _ShadowMap; }
...
// ***************************************************************************
void CMeshInstance::renderIntoSkeletonShadowMap(CSkeletonModel *rootSkeleton,
                                                CMaterial &castMat) {
  /*
      Yoyo: Not Very Robuts here:
          - suppose the MeshInstance don't have sons.
          - suppose that the VertexBuffer doesn't have VertexColor (else
     castMat.color unused).
  */

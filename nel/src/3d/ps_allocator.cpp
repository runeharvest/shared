...
struct CPSAllocInfo {
  ...
  TBlocAllocPtr
      BlocAllocator; // may be NULL if was allocated from stl allocator
};
//
void *PSFastMemAlloc(uint numBytes) {
  ...
  if (PSBlockAllocator) {
    result = (CPSAllocInfo *)PSBlockAllocator->alloc(numBytes +
                                                     sizeof(CPSAllocInfo));
    result->BlocAllocator =
        PSBlockAllocator; // mark as a block from block allocator
  } else {
    result = (CPSAllocInfo *)PSStdAllocator.allocate(numBytes +
                                                     sizeof(CPSAllocInfo));
...
void PSFastMemFree(void *block) {
  ...
  if (ai->BlocAllocator) {
    ...
    ai->BlocAllocator->free((void *)realAddress,
                            ai->NumAllocatedBytes + sizeof(CPSAllocInfo));
  } else {

...
IShape *CShapeBank::addRef(const string &shapeNameNotLwr) {
  ...
  TShapeInfoMap::iterator scfpmIt =
      ShapePtrToShapeInfo.find(getShapePtrFromShapeName(shapeName));
...
void CShapeBank::release(IShape *pShp) {
  ...
  if (str == NULL) {
    nlwarning(
        "Trying to release a mesh that have not be added to the shape bank");
  } else {
...
void CShapeBank::processWaitingShapes() {
  ...
  while (wsmmIt != WaitingShapes.end()) {
    ...
    IShape *pShp = rWS.ShapePtr; // Take care this value is shared between
                                 // thread so copy it in a local variable first
      ...
      if (pMesh != NULL) {
        ...
        for (i = 0; i < nNbMat; ++i) {
            ...
            if (CurrentProgress >= rWS.UpTextProgress) {
              if (rMat.texturePresent(j)) {
                if ((!_pDriver->isTextureExist(*rMat.getTexture(j))) ||
                  ...
                  if (!processWSUploadTexture(rWS, nTotalUploaded,
                                              rMat.getTexture(j)))
                    break;
                } else {
                  /*
                      Must release texture data because of the following
                     scenario (common!)
                      - An IG with 2 different meshs is async loaded
                      - the 2 meshs access the same texture "pipo.tga"
                      - in CAsyncFileManager3D::loadMesh(), if this case arise,
                     the textures are loaded twice in RAM!!!! (is managed per
                     mesh, but not thourhg meshs)
                      - hence the first mesh setup and upload above the texture
                     in VRAM but the second still have data in RAM!! (because
                     2nd will say that isTextureExist() ....

                      Note: you could say that the cool stuff would be to not
                     load the 2 :) but it appears that this case arise rarely
                     because of the following statement:
                          - while the second mesh is async loaded, the first
                     upload in VRAM so there is good change that the second
                     don't load the texture (isTextureExist() return true in the
                     thread)
                  */
          ...
          if (rMat.getShader() == CMaterial::LightMap) {
            uint j = 0;
            ITexture *pText = rMat.getLightMap(j);
            while (pText != NULL) {
              if (CurrentProgress >= rWS.UpTextProgress) {
    ...
    case AsyncLoad_Ready:
      ...
      // The delete operation can take several frames to complete but this is
      // not a problem

      // For error do the same as delete but let the flag to error if a shape is
      // asked just after the error was found
          ...
          if (bSignal != NULL) {
            ...
            while (wsmmIt2 != WaitingShapes.end()) {
              ...
              if (shapeName2 != shapeName) {
                std::set<bool *>::iterator ite2 =
                    wsmmIt2->second.Signal.begin();
...
// ***************************************************************************
bool CShapeBank::processWSUploadTexture(CWaitingShape &rWS,
  ...
  for (; rWS.UpTextMipMap < nMipMap;) {
    ...
    if ((nTotalUploaded + nWeight) > _MaxUploadPerFrame) {
      ...
      uint32 nLineWeight = (pText->getWidth(nMM) *
                            CBitmap::bitPerPixels[pText->getPixelFormat()]) /
                           8;
      // NB: nLineWeight can be 0 in case of 1x1 in DXTC1. Estimate 1 byte
      ...
      zeRect.set(0, rWS.UpTextLine, pText->getWidth(nMM),
                 pText->getHeight(nMM));
      rWS.UpTextLine = 0;
      rWS.UpTextMipMap += 1;
    }
...
CShapeBank::TShapeState
CShapeBank::getPresentState(const string &shapeNameNotLwr) {
  string shapeName = toLowerAscii(shapeNameNotLwr);
...
void CShapeBank::load(const string &shapeNameNotLwr) {
  ...
  TShapeMap::iterator smIt = ShapeMap.find(shapeName);
  if (smIt == ShapeMap.end()) {
    // If we are loading it asynchronously so we do not have to try to load it
    // in sync mode
    TWaitingShapesMap::iterator wsmmIt = WaitingShapes.find(shapeName);
    if (wsmmIt != WaitingShapes.end())
      return;
...
void CShapeBank::loadAsync(const std::string &shapeNameNotLwr, IDriver *pDriver,
                           const CVector &position, bool *bSignal,
  ...
  if (firstTime)
    wsmmIt =
        WaitingShapes
            .insert(TWaitingShapesMap::value_type(shapeName, CWaitingShape()))
            .first;
  ...
  if (firstTime)
    CAsyncFileManager3D::getInstance().loadMesh(
        shapeName, &(wsmmIt->second.ShapePtr), pDriver, position,
        selectedTexture);
}
...
void CShapeBank::cancelLoadAsync(const std::string &shapeNameNotLwr) {
  ...
  if (wsmmIt != WaitingShapes.end()) {
    ...
    if (wsmmIt->second.RefCnt == 0) {
      ...
      // If the state is not Delete, or Error, then the mesh has not been added
      // to the map.
      if (wsmmIt->second.State != AsyncLoad_Delete &&
          wsmmIt->second.State != AsyncLoad_Error) {
        /* but it can still have been loaded:
            - just ended in above cancelLoadMesh() because it was the current
           task
            - ended in async far before this cancelLoadAsync() call, but
           processWaintingShape() still not called on it In this case we have to
           delete this shape, or even cancel texture upload! NB: don't forget
           that the load can still be a fail too....
        */
        IShape *shape = wsmmIt->second.ShapePtr;
        if (shape != NULL && shape != (IShape *)-1) {
          // this ptr should not be added to the map
          nlassert(ShapePtrToShapeInfo.find(shape) ==
                   ShapePtrToShapeInfo.end());

          // Before deleting this shape, we must ensure it is not currently
          // uploading texture
          if (wsmmIt->second.State == AsyncLoad_Texture) {
            /* if it was uploading a texture, then force him to end, else may
               have a bug in this scenario (very improbable, but possible I
               think):
                    - a mesh is loaded asynchronously, and reference a texture
               "pipo.tga"
                    - mesh load async is ended, and the texture is not found in
               the driver
                        => texture generate()-d too
                    - mesh state is AsyncLoad_Texture, and begin (but doesn't
               end) to upload the texture
                    - another mesh is created syncrhonously using also this
               texture (thus found in driver, and so just referencing it, no
               generate)
                    - the async mesh is then canceled, while the texture has not
               end to load!
                    - the texture is still in memory (the sync mesh still point
               to it), but with partialy uploaded data!
            */
            // \todo yoyo: should be very rare, and don't know if really
            // happens. must do tests forceEndUpLoadTexture(wsmmIt->second);
          }
...
bool CShapeBank::isShapeWaiting() { return !WaitingShapes.empty(); }
...
void CShapeBank::add(const string &shapeNameNotLwr, IShape *pShp) {
  ...
  if (pShp && _ShapeNeedingSystemGeometryCopy.find(shapeName) !=
                  _ShapeNeedingSystemGeometryCopy.end()) {
...
void CShapeBank::addShapeCache(const string &shapeCacheName) {
  TShapeCacheMap::iterator scmIt =
      ShapeCacheNameToShapeCache.find(shapeCacheName);
  if (scmIt == ShapeCacheNameToShapeCache.end()) {
    // Not found so add it
    ShapeCacheNameToShapeCache.insert(
        TShapeCacheMap::value_type(shapeCacheName, CShapeCache()));
  }
}
...
void CShapeBank::setShapeCacheSize(const string &shapeCacheName,
                                   sint32 maxSize) {
  TShapeCacheMap::iterator scmIt =
      ShapeCacheNameToShapeCache.find(shapeCacheName);
  if (scmIt != ShapeCacheNameToShapeCache.end()) {
...
// ***************************************************************************
sint CShapeBank::getShapeCacheFreeSpace(
    const std::string &shapeCacheName) const {
  TShapeCacheMap::const_iterator scmIt =
      ShapeCacheNameToShapeCache.find(shapeCacheName);
  if (scmIt != ShapeCacheNameToShapeCache.end()) {
...
void CShapeBank::linkShapeToShapeCache(const string &shapeNameNotLwr,
                                       const string &shapeCacheName) {
  string shapeName = toLowerAscii(shapeNameNotLwr);
...
CShapeBank::CShapeCache *
CShapeBank::getShapeCachePtrFromShapePtr(IShape *pShp) {
  TShapeInfoMap::iterator scfpmIt = ShapePtrToShapeInfo.find(pShp);
  if (scfpmIt != ShapePtrToShapeInfo.end()) {
...
CShapeBank::CShapeCache *
CShapeBank::getShapeCachePtrFromShapeCacheName(const string &shapeCacheName) {
  TShapeCacheMap::iterator scmIt =
      ShapeCacheNameToShapeCache.find(shapeCacheName);
  if (scmIt != ShapeCacheNameToShapeCache.end()) {
    return &(scmIt->second);
...
CShapeBank::CShapeCache *
CShapeBank::getShapeCachePtrFromShapeName(const std::string &shapeName) {
  TShapeCacheNameMap::iterator scnIt =
      ShapeNameToShapeCacheName.find(shapeName);
...
bool CShapeBank::isShapeCache(const std::string &shapeCacheName) const {
  return ShapeCacheNameToShapeCache.find(shapeCacheName) !=
         ShapeCacheNameToShapeCache.end();
}
      ...
      if (getPresentState(fileName) == CShapeBank::NotPresent) {
        ...
        if (getShapeCacheFreeSpace(shapeCacheName) > 0) {
          ...
          if (getPresentState(fileName) == CShapeBank::Present) {
            ...
            if (shp) {
              // nlinfo("Loading %s", CPath::lookup(fileName.c_str(), false,
              // false).c_str());

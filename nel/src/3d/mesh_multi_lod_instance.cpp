...
void CMeshMultiLodInstance::registerBasic() {
  CScene::registerModel(MeshMultiLodInstanceId, MeshBaseInstanceId,
...
CRGBA CMeshMultiLodInstance::getCoarseMeshLighting() {
  ...
  sunContrib.modulateFromuiRGBOnly(sunContrib,
                                   getLightContribution().SunContribution / 2);
  // Add Ambient
...
void CMeshMultiLodInstance::traverseLoadBalancing() {
  ...
  // If this is the second pass of LoadBalancing, choose the Lods, according to
  ...
  if (loadTrav.getLoadPass() == 1) {
    ...
    if (startBlend < distance) {
      ...
      if (nextSlot && shape->_MeshVector[Lod0 + 1].MeshGeom &&
          (nextSlot->Flags & CMeshMultiLod::CMeshSlot::BlendIn)) {
        // Render the geom mesh with alpha blending with
        // nextSlot->BeginPolygonCount
...
// ***************************************************************************
void CMeshMultiLodInstance::changeMRMDistanceSetup(float distanceFinest,
...
// ***************************************************************************
void CMeshMultiLodInstance::setUVCoarseMesh(CMeshGeom &geom, uint vtDstSize,
                                            uint dstUvOff) {
  ...
  nlassert(vbSrc.getVertexFormat() &
...
// ***************************************************************************
void CMeshMultiLodInstance::setPosCoarseMesh(CMeshGeom &geom,
...
// ***************************************************************************
void CMeshMultiLodInstance::setColorCoarseMesh(CRGBA color, uint vtDstSize,
                                               uint dstColorOff) {

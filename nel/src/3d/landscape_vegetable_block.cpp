...
// ***************************************************************************
void CLandscapeVegetableBlock::init(
    const CVector &center, CVegetableManager *vegetManager,
    CVegetableClipBlock *vegetableClipBlock, CPatch *patch, uint ts, uint tt,
  ...
  for (tmt = _Tt; tmt < _Tt + 2; tmt++) {
    for (tms = _Ts; tms < _Ts + 2; tms++) {
      ...
      _VegetableSortBlock[(tmt - _Tt) * 2 + (tms - _Ts)] =
          vegetManager->createSortBlock(_VegetableClipBlock, center,
...
// ***************************************************************************
void CLandscapeVegetableBlock::release(
...
// ***************************************************************************
void CLandscapeVegetableBlock::update(const CVector &viewCenter,
                                      CVegetableManager *vegeManager) {
  ...
  while (newDistType < NL3D_VEGETABLE_BLOCK_NUMDIST &&
         sqrDist > CLVBSqrDistLUT::Array[newDistType + 1]) {
    newDistType++;
  }
  /*
...
// ***************************************************************************
void CLandscapeVegetableBlock::createVegetableIGForDistType(
    uint i, CVegetableManager *vegeManager,
  ...
  for (tmt = _Tt; tmt < _Tt + 2; tmt++) {
    for (tms = _Ts; tms < _Ts + 2; tms++) {
      ...
      CVegetableInstanceGroup *vegetIg =
          vegeManager->createIg(_VegetableSortBlock[tileId]);
      _VegetableIG[tileId][i] = vegetIg;

      // generate
      _Patch->generateTileVegetable(vegetIg, i, tms, tmt, vbCreateCtx);

      // If the ig is empty, delete him. This optimize rendering because no
      // useless ig are tested for rendering. This speed up some 1/10 of ms...
      if (vegetIg->isEmpty()) {
        vegeManager->deleteIg(vegetIg);
        _VegetableIG[tileId][i] = NULL;
      }

      // NB: vegtable SortBlock is updated in CLandscapeVegetableBlock::update()
      // after
    }
  }
}
...
// ***************************************************************************
void CLandscapeVegetableBlockCreateContext::init(CPatch *patch, uint ts,
                                                 uint tt) {
  nlassert(patch);
  _Empty = true;
  _Patch = patch;
...
// ***************************************************************************
void CLandscapeVegetableBlockCreateContext::eval(uint ts, uint tt, float x,
                                                 float y, CVector &pos) {
  nlassert(NL3D_TESSBLOCK_TILESIZE == 4);
  ...
  if (_Empty) {
    ...
    for (uint j = 0; j < 3; j++) {
      ...
      for (uint i = 0; i < 3; i++) {
        ...
        // eval position.
        // use computeVertex() and not bpatch->eval() because vegetables must
        // follow the noise (at least at tile precision...). It is slower but
        // necessary.

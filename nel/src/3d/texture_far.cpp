...
CRGBA CTextureFar::_LightmapExpanded[NL_NUM_PIXELS_ON_FAR_TILE_EDGE *
                                     NL_MAX_TILES_BY_PATCH_EDGE *
                                     NL_NUM_PIXELS_ON_FAR_TILE_EDGE *
                                     NL_MAX_TILES_BY_PATCH_EDGE];
uint8 CTextureFar::_LumelExpanded
    [(NL_MAX_TILES_BY_PATCH_EDGE * NL_LUMEL_BY_TILE + 1) *
     (NL_MAX_TILES_BY_PATCH_EDGE * NL_LUMEL_BY_TILE + 1)];
CRGBA CTextureFar::_TileTLIColors[(NL_MAX_TILES_BY_PATCH_EDGE + 1) *
                                  (NL_MAX_TILES_BY_PATCH_EDGE + 1)];
...
CTextureFar::CTextureFar() {
  ...
  */
  ...
  // This texture is releasable. It doesn't stays in standard memory after been
  // uploaded into video memory.
  setReleasable(true);
...
// ***************************************************************************
void CTextureFar::allocatePatch(CPatch *pPatch, uint farIndex, float &farUScale,
                                float &farVScale, float &farUBias,
                                float &farVBias, bool &bRot) {
  // get the size of the subtexture to allocate
  uint width =
      (pPatch->getOrderS() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >> (farIndex - 1);
  uint height =
...
uint CTextureFar::touchPatchULAndNext() {
  ...
  if (_ItULPatch != _PatchToPosMap.end()) {
    ...
    uint width = (pPatch->getOrderS() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >>
                 (farIndex - 1);
    uint height = (pPatch->getOrderT() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >>
                  (farIndex - 1);
    if (width < height)
      std::swap(width, height);
...
// ***************************************************************************
void CTextureFar::startPatchULTouch() { _ItULPatch = _PatchToPosMap.begin(); }
...
void CTextureFar::doGenerate(bool async) {
  ...
  CBitmap::resize(NL_FAR_TEXTURE_EDGE_SIZE, NL_FAR_TEXTURE_EDGE_SIZE, RGBA,
                  false);
...
void CTextureFar::rebuildPatch(const CVector2s texturePos,
  ...
  uint subTextWidth =
      (nS * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >> (pid.FarIndex - 1);
  ...
  // Delta to add to the destination offset when walk for a pixel to the right
  // in the source tile
  sint dstDeltaX;

  // Delta to add to the destination offset when walk for a pixel to the bottom
  // in the source tile
  sint dstDeltaY;
  ...
  // higher than larger (goofy), the patch is stored with a rotation of 1 (to
  // the left of course)
  else {
  ...
  lightMap.StaticLightColor =
      patch->getZone()->getLandscape()->getStaticLight();
  ...
  for (uint t = 0; t < nT; t++) {
    ...
    for (uint s = 0; s < nS; s++) {
      ...
      // Lightmap pointer
      TileFar.SrcLightingPixels =
          _LightmapExpanded + (s * tileSize) + (t * nS * tileSize * tileSize);
      ...
      for (sint l = 0; l < 3; l++) {
        ...
        // Is the last layer ?
        bool lastLayer =
            ((l == 2) || (tileElm.Tile[l + 1] == NL_TILE_ELM_LAYER_EMPTY));
        ...
        if (tile != NL_TILE_ELM_LAYER_EMPTY) {
          ...
          if (pTile) {
            ...
            if (pTile->isFill(CTileFarBank::diffuse)) {
              ...
              // Source pointer
              const CRGBA *pSrcDiffusePixels =
                  pTile->getPixels(CTileFarBank::diffuse, orderX);
              ...
              if (pTile->isFill(CTileFarBank::additive)) {
                ...
                pSrcAdditivePixels =
                    pTile->getPixels(CTileFarBank::additive, orderX);
              }
              ...
              case 2: {
                // Destination pointer
                uint newOffset =
                    sourceOffset + (tileSize - 1) * sourceSize + tileSize - 1;
                TileFar.SrcDiffusePixels = pSrcDiffusePixels + newOffset;
                TileFar.SrcAdditivePixels = pSrcAdditivePixels + newOffset;
 ...
 */
inline void NL3D_asmExpandLineColor565(const uint16 *src, CRGBA *dst, uint du,
                                       uint len) {
 ...
 *	NB: start to write at pixel 1.
 */
inline void NL3D_asmExpandLineColor8888(const CRGBA *src, CRGBA *dst, uint du,
                                        uint len) {
  static uint64 blank = 0;
  if (len == 0)
    return;
 ...
 */
inline void NL3D_asmBlendLines(CRGBA *dst, const CRGBA *src0, const CRGBA *src1,
                               uint index, uint len) {
  static uint64 blank = 0;
  if (len == 0)
    return;
 ...
 */
static void NL3D_asmAssembleShading1x1(const uint8 *lumels,
                                       const CRGBA *colorMap,
                                       const CRGBA *srcTLIs,
                                       const CRGBA *srcUSCs, CRGBA *dst,
                                       uint lineWidth, uint nbTexel) {
  static uint64 blank = 0;
  if (nbTexel == 0)
    return;
 ...
 */
static void NL3D_asmAssembleShading2x2(const uint8 *lumels,
                                       const CRGBA *colorMap,
                                       const CRGBA *srcTLIs,
                                       const CRGBA *srcUSCs, CRGBA *dst,
                                       uint lineWidth, uint nbTexel) {
...
// ***************************************************************************
#pragma warning(disable : 4731) // frame pointer register 'ebp' modified by
                                // inline assembly code
/**	Lightmap Combining for Far level 0 (nearest)
 *	read 1 lumel, and deals with UserColor and TLI
 */
static void NL3D_asmAssembleShading4x4(const uint8 *lumels,
                                       const CRGBA *colorMap,
                                       const CRGBA *srcTLIs,
                                       const CRGBA *srcUSCs, CRGBA *dst,
                                       uint nbTexel) {
...
#pragma warning(default : 4731) // frame pointer register 'ebp' modified by
                                // inline assembly code
...
extern "C" void NL3D_expandLightmap(const NL3D_CExpandLightmap *pLightmap) {
  ...
  static CRGBA
      expandedUserColor[(NL_MAX_TILES_BY_PATCH_EDGE + 1) * NL_LUMEL_BY_TILE *
                        (NL_MAX_TILES_BY_PATCH_EDGE + 1) * NL_LUMEL_BY_TILE];
  static CRGBA
      expandedTLIColor[(NL_MAX_TILES_BY_PATCH_EDGE + 1) * NL_LUMEL_BY_TILE *
                       (NL_MAX_TILES_BY_PATCH_EDGE + 1) * NL_LUMEL_BY_TILE];
  ...
  for (v = 0; v < pLightmap->Height; v++) {
    ...
    //-------------
    if (asmMMX) {
      NL3D_asmExpandLineColor565(&colorTilePtr->Color565,
                                 expandedUserColorLinePtr, expandFactor,
                                 dstWidth - 2);
      NL3D_asmExpandLineColor8888(colorTLIPtr, expandedTLIColorLinePtr,
                                  expandFactor, dstWidth - 2);
    }
    // C implementation
    //-------------
    else {
      ...
      for (u = 1; u < dstWidth - 1; u++) {
        // Check
        nlassert((u + v * dstWidth) <
                 (sizeof(expandedUserColorLine) / sizeof(CRGBA)));
        ...
        expandedUserColorLinePtr[u].blendFromui(color0, color1,
                                                srcIndexPixel & 0xff);
        // Compute current TLI color
        color0 = colorTLIPtr[srcIndex];
        color1 = colorTLIPtr[srcIndex + 1];
        expandedTLIColorLinePtr[u].blendFromui(color0, color1,
                                               srcIndexPixel & 0xff);
    ...
    // Last pixel
    expandedUserColorLinePtr[dstWidth - 1].set565(
        colorTilePtr[pLightmap->Width - 1].Color565);
  ...
  // Copy first row
  memcpy(expandedUserColorPtr, expandedUserColorLinePtr,
         dstWidth * sizeof(CRGBA));
  memcpy(expandedTLIColorPtr, expandedTLIColorLinePtr,
         dstWidth * sizeof(CRGBA));
  ...
  for (v = 1; v < dstHeight - 1; v++) {
    ...
    //-------------
    if (asmMMX) {
      NL3D_asmBlendLines(expandedUserColorPtr, colorTilePtr0, colorTilePtr1,
                         indexPixel, dstWidth);
      NL3D_asmBlendLines(expandedTLIColorPtr, colorTLIPtr0, colorTLIPtr1,
                         indexPixel, dstWidth);
    }
    // C implementation
    //-------------
      ...
      for (u = 0; u < dstWidth; u++) {
        expandedUserColorPtr[u].blendFromui(colorTilePtr0[u], colorTilePtr1[u],
                                            indexPixel & 0xff);
        expandedTLIColorPtr[u].blendFromui(colorTLIPtr0[u], colorTLIPtr1[u],
                                           indexPixel & 0xff);
      }
    }
  ...
  expandedUserColorLinePtr =
      expandedUserColorLine + dstWidth * (pLightmap->Height - 1);
  expandedTLIColorLinePtr =
      expandedTLIColorLine + dstWidth * (pLightmap->Height - 1);
  ...
  case 1: {
    ...
    for (v = 0; v < dstHeight; v++) {
      ...
      //-------------
      if (asmMMX) {
        NL3D_asmAssembleShading1x1(lineLumelPtr, pLightmap->StaticLightColor,
                                   lineTLIPtr, lineUSCPtr, lineDestPtr,
                                   lineWidth, dstWidth);
      }
      // C implementation
      //-------------
        ...
        for (u = 0; u < dstWidth; u++) {
          ...
          // Shading is filtred
          uint shading = ((uint)lineLumelPtr[lumelIndex] +
                          (uint)lineLumelPtr[lumelIndex + 1] +
                          (uint)lineLumelPtr[lumelIndex + 2] +
                          (uint)lineLumelPtr[lumelIndex + 3] +
                          (uint)lineLumelPtr[lumelIndex + lineWidth] +
                          (uint)lineLumelPtr[lumelIndex + 1 + lineWidth] +
                          (uint)lineLumelPtr[lumelIndex + 2 + lineWidth] +
                          (uint)lineLumelPtr[lumelIndex + 3 + lineWidth] +
                          (uint)lineLumelPtr[lumelIndex + lineWidthx2] +
                          (uint)lineLumelPtr[lumelIndex + 1 + lineWidthx2] +
                          (uint)lineLumelPtr[lumelIndex + 2 + lineWidthx2] +
                          (uint)lineLumelPtr[lumelIndex + 3 + lineWidthx2] +
                          (uint)lineLumelPtr[lumelIndex + lineWidthx3] +
                          (uint)lineLumelPtr[lumelIndex + 1 + lineWidthx3] +
                          (uint)lineLumelPtr[lumelIndex + 2 + lineWidthx3] +
                          (uint)lineLumelPtr[lumelIndex + 3 + lineWidthx3]) >>
                         4;
  ...
  case 2: {
    ...
    for (v = 0; v < dstHeight; v++) {
      ...
      //-------------
      if (asmMMX) {
        NL3D_asmAssembleShading2x2(lineLumelPtr, pLightmap->StaticLightColor,
                                   lineTLIPtr, lineUSCPtr, lineDestPtr,
                                   lineWidth, dstWidth);
      }
      // C implementation
  ...
  case 4:
    ...
    //-------------
    if (asmMMX) {
      NL3D_asmAssembleShading4x4(lineLumelPtr, pLightmap->StaticLightColor,
                                 lineTLIPtr, lineUSCPtr, lineDestPtr, nbTexel);
    }
    // C implementation
    //-------------
    else {
...
// ***************************************************************************
inline void NL3D_asmModulateLineColors(CRGBA *dst, const CRGBA *src0,
                                       const CRGBA *src1, uint len,
                                       uint src0DeltaX, uint dstDeltaX) {
  static uint64 blank = 0;
  if (len == 0)
    return;
...
// ***************************************************************************
inline void NL3D_asmModulateAndBlendLineColors(CRGBA *dst, const CRGBA *src0,
                                               const CRGBA *src1, uint len,
                                               uint src0DeltaX,
                                               uint dstDeltaX) {
  static uint64 blank = 0;
  ...
  for (y = 0; y < pTileFar->Size; y++) {
    ...
    if (pTileFar->AsmMMX) {
      NL3D_asmModulateLineColors(pDstPixels, pSrcPixels, pSrcLightPixels,
                                 pTileFar->Size, pTileFar->SrcDeltaX,
                                 pTileFar->DstDeltaX);
    }
    // C Implementation.
    //---------
      ...
      for (x = 0; x < pTileFar->Size; x++) {
        ...
        pDstLine->R =
            (uint8)(((uint)pSrcLine->R * (uint)pSrcLightingLine->R) >> 8);
        pDstLine->G =
            (uint8)(((uint)pSrcLine->G * (uint)pSrcLightingLine->G) >> 8);
        pDstLine->B =
            (uint8)(((uint)pSrcLine->B * (uint)pSrcLightingLine->B) >> 8);
...
void NL3D_drawFarTileInFarTextureAlpha(const NL3D_CComputeTileFar *pTileFar) {
  ...
  for (y = 0; y < pTileFar->Size; y++) {
    ...
    //---------
    if (pTileFar->AsmMMX) {
      NL3D_asmModulateAndBlendLineColors(
          pDstPixels, pSrcPixels, pSrcLightPixels, pTileFar->Size,
          pTileFar->SrcDeltaX, pTileFar->DstDeltaX);
    }
    // C Implementation.
    //---------
    else {
      ...
      for (x = 0; x < pTileFar->Size; x++) {
        ...
        pDstLine->R =
            (uint8)(((((uint)pSrcLine->R * (uint)pSrcLightingLine->R) >> 8) *
                         alpha +
                     (uint)pDstLine->R * oneLessAlpha) >>
                    8);
        pDstLine->G =
            (uint8)(((((uint)pSrcLine->G * (uint)pSrcLightingLine->G) >> 8) *
                         alpha +
                     (uint)pDstLine->G * oneLessAlpha) >>
                    8);
        pDstLine->B =
            (uint8)(((((uint)pSrcLine->B * (uint)pSrcLightingLine->B) >> 8) *
                         alpha +
                     (uint)pDstLine->B * oneLessAlpha) >>
                    8);
...
// #ifdef NL_NO_ASM
void NL3D_drawFarTileInFarTextureAdditive(
    const NL3D_CComputeTileFar *pTileFar) {
  // Pointer of the Src diffuse pixels
  const CRGBA *pSrcPixels = pTileFar->SrcDiffusePixels;
  ...
  for (y = 0; y < pTileFar->Size; y++) {
    ...
    for (x = 0; x < pTileFar->Size; x++) {
      // Read and write a pixel
      uint nTmp = (((uint)pSrcLine->R * (uint)pSrcLightingLine->R) >> 8) +
                  (uint)pSrcAddLine->R;
      if (nTmp > 255)
        nTmp = 255;
      pDstLine->R = (uint8)nTmp;
      nTmp = (((uint)pSrcLine->G * (uint)pSrcLightingLine->G) >> 8) +
             (uint)pSrcAddLine->G;
      if (nTmp > 255)
        nTmp = 255;
      pDstLine->G = (uint8)nTmp;
      nTmp = (((uint)pSrcLine->B * (uint)pSrcLightingLine->B) >> 8) +
             (uint)pSrcAddLine->B;
      if (nTmp > 255)
        nTmp = 255;
...
// TODO: asm implementation of this function \\//
// #ifdef NL_NO_ASM
void NL3D_drawFarTileInFarTextureAdditiveAlpha(
    const NL3D_CComputeTileFar *pTileFar) {
  // Pointer of the Src pixels
  const CRGBA *pSrcPixels = pTileFar->SrcDiffusePixels;

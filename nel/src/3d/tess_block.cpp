...
// ***************************************************************************
void CTessBlock::init(CPatch *patch) { _Patch = patch; }
// ***************************************************************************
CPatch *CTessBlock::getPatch() { return _Patch; }
...
// ***************************************************************************
void CTessBlock::forceClip() { Clipped = true; }
...
// ***************************************************************************
void CTessBlock::clipFar(const CVector &refineCenter, float tileDistNear,
                         float farTransition) {
  ...
  for (uint tileId = 0; tileId < NL3D_TESSBLOCK_TILESIZE; tileId++) {
    ...
    if (RdrTileRoot[tileId]) {
      ...
      for (uint facePass = 0; facePass < NL3D_MAX_TILE_FACE; facePass++) {
        CTessList<CTileFace> &faceList =
            RdrTileRoot[tileId]->TileFaceList[facePass];
        TLandscapeIndexType *faceVector =
            RdrTileRoot[tileId]->TileFaceVectors[facePass];
        ...
        if (faceList.size() > 0) {
          ...
          for (pFace = faceList.begin(); pFace;
               pFace = (CTileFace *)pFace->Next) {
#if defined(NL_DEBUG) && defined(NL_LANDSCAPE_INDEX16)
...
#endif
            *(dest++) =
                (TLandscapeIndexType)pFace->V[CTessFace::IdUvBase]->Index;
            *(dest++) =
                (TLandscapeIndexType)pFace->V[CTessFace::IdUvLeft]->Index;
            *(dest++) =
                (TLandscapeIndexType)pFace->V[CTessFace::IdUvRight]->Index;
          }
        }
      }
    }
...
void CTessBlock::createFaceVectorTile(CLandscapeFaceVectorManager &mgr) {
  // For all tiles existing, and for all facePass existing, create the
  // faceVector.
  for (uint tileId = 0; tileId < NL3D_TESSBLOCK_TILESIZE; tileId++) {
    // if tile exist.
    if (RdrTileRoot[tileId]) {
...
void CTessBlock::deleteFaceVectorTile(CLandscapeFaceVectorManager &mgr) {
  // For all tiles existing, and for all facePass existing, delete the
  // faceVector.
  for (uint tileId = 0; tileId < NL3D_TESSBLOCK_TILESIZE; tileId++) {
    // if tile exist.
    if (RdrTileRoot[tileId]) {
...
// ***************************************************************************
void CTessBlock::appendToModifyListAndDeleteFaceVector(
    CTessBlock &root, CLandscapeFaceVectorManager &mgr) {

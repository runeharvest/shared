...
// ***************************************************************************
// NLMISC::CBlockMemory<CTransform*, false>
// CPointLight::_LightedModelListMemory(NL3D_LIGHTED_MODEL_ALLOC_BLOCKSIZE);
...
// ***************************************************************************
CPointLight::CPointLight(const CPointLight &o)
...
// ***************************************************************************
CPointLight::~CPointLight() { resetLightedModels(); }

// ***************************************************************************
void CPointLight::setType(TType type) { _Type = type; }
CPointLight::TType CPointLight::getType() const { return _Type; }

// ***************************************************************************
void CPointLight::setupAttenuation(float attenuationBegin,
...
// ***************************************************************************
float CPointLight::computeLinearAttenuation(const CVector &pos, float dist,
  ...
  if (_Type == SpotLight) {
    ...
    if (modelRadius > 0) {
      ...
      if (modelRadius > dist)
        ...
        // compute the angle of the cone made by the model sphere and the
        // pointLightCenter.
        float cosAngleSphere =
            modelRadius / sqrtf(sqr(dist) + sqr(modelRadius));
        /* If this one is smaller than cosAngleDirSpot, it's mean that the angle
           of this cone is greater than the angleDirSpot, hence a part of the
           sphere "ps" exist such that _SportDirection*(ps-_Position).normed()
           == 1
            => no spotAttenuation
        */
        if (cosAngleSphere < cosAngleDirSpot)
          ...
          float cosDelta = cosAngleSphere * cosAngleDirSpot +
                           sinAngleSphere * sinAngleDirSpot;
...
void CPointLight::setupDriverLight(CLight &light, uint8 factor) {
  ...
  if (_Type == SpotLight) {
    light.setupSpotLight(ambient, diffuse, specular, _Position, _SpotDirection,
                         ...
                         _ConstantAttenuation, _LinearAttenuation,
...
void CPointLight::setupDriverLightUserAttenuation(CLight &light, uint8 factor) {
  ...
  light.setupPointLight(ambient, diffuse, specular, _Position, CVector::Null, 1,
                        0, 0);
}
...
void CPointLight::resetLightedModels() {
  ...
  while (_LightedModels.begin() != _LightedModels.end()) {
    ...
    nlassert(_LightedModels.begin() == _LightedModels.end() ||
             *_LightedModels.begin() != model);
  }
}

// ***************************************************************************
CPointLight::ItTransformList
CPointLight::appendLightedModel(CTransform *model) {

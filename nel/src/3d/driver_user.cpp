...
// ***************************************************************************
UDriver *UDriver::createDriver(uintptr_t windowIcon, bool direct3d,
                               emptyProc exitFunc) {
  return new CDriverUser(windowIcon,
                         direct3d ? CDriverUser::Direct3d : CDriverUser::OpenGl,
                         exitFunc);
}

// ***************************************************************************
UDriver *UDriver::createDriver(uintptr_t windowIcon, TDriver driver,
                               emptyProc exitFunc) {
...
// ***************************************************************************
CDriverUser::CDriverUser(uintptr_t windowIcon, TDriver driver,
                         emptyProc exitFunc) {
  ...
  _VBColorUv.setVertexFormat(CVertexBuffer::PositionFlag |
                             CVertexBuffer::PrimaryColorFlag |
                             CVertexBuffer::TexCoord0Flag);
  _VBQuadsColUv.setVertexFormat(CVertexBuffer::PositionFlag |
                                CVertexBuffer::PrimaryColorFlag |
                                CVertexBuffer::TexCoord0Flag);
  _VBTrisColUv.setVertexFormat(CVertexBuffer::PositionFlag |
                               CVertexBuffer::PrimaryColorFlag |
                               CVertexBuffer::TexCoord0Flag);
  _VBQuadsColUv2.setVertexFormat(
      CVertexBuffer::PositionFlag | CVertexBuffer::PrimaryColorFlag |
...
// ***************************************************************************
bool CDriverUser::setDisplay(nlWindow wnd, const CMode &mode, bool show,
                             bool resizeable) {
  NL3D_HAUTO_UI_DRIVER;

  // window init.
  if (_Driver->setDisplay(wnd,
                          GfxMode(mode.Width, mode.Height, mode.Depth,
                                  mode.Windowed, false, mode.Frequency,
                                  mode.AntiAlias, mode.DisplayDevice),
                          show, resizeable)) {
    // Always true
    nlverify(activate());
    ...
    NL3D::CTextureFile::setSupportNonPowerOfTwoTextures(
        _Driver->supportNonPowerOfTwoTextures());
...
bool CDriverUser::setMode(const CMode &mode) {
  return _Driver->setMode(GfxMode(mode.Width, mode.Height, mode.Depth,
                                  mode.Windowed, false, mode.Frequency,
                                  mode.AntiAlias, mode.DisplayDevice));
}
...
// ----------------------------------------------------------------------------
struct CModeSorter {
  bool operator()(const UDriver::CMode &mode1,
                  const UDriver::CMode &mode2) const {
...
bool CDriverUser::getModes(std::vector<CMode> &modes) {
  ...
  for (uint i = 0; i < vTmp.size(); ++i)
    modes.push_back(CMode(vTmp[i].Width, vTmp[i].Height, vTmp[i].Depth,
                          vTmp[i].Windowed, vTmp[i].Frequency,
                          vTmp[i].AntiAlias, vTmp[i].DisplayDevice));
...
// ***************************************************************************
void CDriverUser::setColorMask(bool bRed, bool bGreen, bool bBlue,
                               bool bAlpha) {
...
// ***************************************************************************
void CDriverUser::drawTriangle(const NLMISC::CTriangleColor &shp,
                               UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawTriangle(const NLMISC::CTriangleColorUV &shp,
                               UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawQuads(const std::vector<NLMISC::CQuadColorUV> &q,
                            UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawQuads(const std::vector<NLMISC::CQuadColorUV2> &q,
                            UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawQuads(const NLMISC::CQuadColorUV *quads, uint32 nbQuads,
                            UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawQuads(const NLMISC::CQuadColorUV2 *quads, uint32 nbQuads,
                            UMaterial &mat) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawTriangles(
    const std::vector<NLMISC::CTriangleColorUV> &tris, UMaterial &mat) {
  drawTriangles(&tris[0], (uint32)tris.size(), mat);
}

// ***************************************************************************
void CDriverUser::drawTriangles(const NLMISC::CTriangleColorUV *tris,
...
// ***************************************************************************
void CDriverUser::drawBitmap(float x, float y, float width, float height,
                             class UTexture &texture, bool blend, CRGBA col) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawTriangle(float x0, float y0, float x1, float y1, float x2,
                               float y2, CRGBA col) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawQuad(float xcenter, float ycenter, float radius,
                           CRGBA col) {
  H_AUTO2;

  drawQuad(xcenter - radius, ycenter - radius, xcenter + radius,
           ycenter + radius, col);
}
// ***************************************************************************
void CDriverUser::drawWiredQuad(float x0, float y0, float x1, float y1,
                                CRGBA col) {
  H_AUTO2;
...
// ***************************************************************************
void CDriverUser::drawWiredQuad(float xcenter, float ycenter, float radius,
                                CRGBA col) {
  H_AUTO2;

  drawWiredQuad(xcenter - radius, ycenter - radius, xcenter + radius,
                ycenter + radius, col);
}
...
// ***************************************************************************
UDriver::TMessageBoxId CDriverUser::systemMessageBox(const char *message,
                                                     const char *title,
...
// ***************************************************************************
CMaterial &CDriverUser::convMat(UMaterial &mat) { return *mat.getObjectPtr(); }
...
bool CDriverUser::setMonitorColorProperties(
...
void CDriverUser::getZBufferPart(std::vector<float> &zbuffer,
                                 NLMISC::CRect &rect) {
  NL3D_HAUTO_UI_DRIVER;
...
void CDriverUser::addCursor(const std::string &name,
...
void CDriverUser::setCursor(const std::string &name, NLMISC::CRGBA col,
                            uint8 rot, sint hotSpotX, sint hotSpotY,
                            bool forceRebuild) {
  NL3D_HAUTO_UI_DRIVER;
...
void CDriverUser::startBench(bool wantStandardDeviation, bool quick,
                             bool reset) {
...
void CDriverUser::endBench() { _Driver->endBench(); }
...
// ***************************************************************************
void CDriverUser::stencilOp(TStencilOp fail, TStencilOp zfail,
                            TStencilOp zpass) {
  NL3D_HAUTO_UI_DRIVER
...
bool CDriverUser::stretchRect(UScene *scene, class UTexture &srcUText,
  ...
  ITexture *destText =
      (dynamic_cast<CTextureUser *>(&destUText))->getITexture();
  ...
  if (!_Driver->stretchRect(srcText, srcRect, destText, destRect)) {
    ...
    float newU =
        _Driver->isTextureRectangle(srcText) ? (float)srcRect.Width : 1.f;
    float newV =
        _Driver->isTextureRectangle(srcText) ? (float)srcRect.Height : 1.f;
...
// ***************************************************************************
bool CDriverUser::setRenderTarget(class UTexture &uTex, uint32 x, uint32 y,
  ...
  bool result =
      _Driver->setRenderTarget(tex, x, y, width, height, mipmapLevel, cubeFace);

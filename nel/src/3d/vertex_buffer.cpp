...
const uint CVertexBuffer::SizeType[NumType] = {
    1 * sizeof(double), 1 * sizeof(float), 1 * sizeof(short),
    2 * sizeof(double), 2 * sizeof(float), 2 * sizeof(short),
    3 * sizeof(double), 3 * sizeof(float), 3 * sizeof(short),
    4 * sizeof(double), 4 * sizeof(float), 4 * sizeof(short),
    4 * sizeof(char),
};

const uint CVertexBuffer::NumComponentsType[NumType] = {1, 1, 1, 2, 2, 2, 3,
                                                        3, 3, 4, 4, 4, 4};
...
CVertexBuffer::CVertexBuffer() { construct(); }
...
CVertexBuffer::~CVertexBuffer() {
  ...
  if (DrvInfos)
    DrvInfos->VertexBufferPtr =
        NULL; // Tell the driver info to not restore memory when it will die
...
void CVertexBuffer::copyVertices(CVertexBuffer &dest) const {
  ...
  NLMISC::CFastMem::memcpy(destDatas.getVertexCoordPointer(),
                           srcDatas.getVertexCoordPointer(),
                           getVertexSize() * getNumVertices());
}
...
CVertexBuffer::TValue CVertexBuffer::getValueIdByNumberEx(uint valueNumber) {
  // See NV_vertex_program spec, or driver_opengl_vertex.cpp::
  // GLVertexAttribIndex.
  static TValue lut[16] = {
      Position,       Weight,    Normal,      PrimaryColor,
      SecondaryColor, Fog,       PaletteSkin, Empty,
...
void CVertexBuffer::addValueEx(TValue valueId, TType type) {
  ...
  // unfortunately, some vertex program implementations don't allow any type for
  // any value
  switch (valueId) {
  case Position:
    nlassert(numComp >= 2);
...
uint16 CVertexBuffer::remapV2Flags(uint32 oldFlags, uint &weightCount) {
  ...
  const uint32 OLD_IDRV_VF_PALETTE_SKIN = 0x00010000 | OLD_IDRV_VF_W0 |
                                          OLD_IDRV_VF_W1 | OLD_IDRV_VF_W2 |
                                          OLD_IDRV_VF_W3;
...
void CVertexBuffer::serialOldV1Minus(NLMISC::IStream &f, sint ver) {
  ...
  */
  ...
  // All other infos (but _NonResidentVertices) are computed by
  // setVertexFormat() and setNumVertices().
  ...
  for (sint id = 0; id < (sint)_NbVerts; id++) {
    ...
    for (i = 0; i < weightCount; i++) {
      ...
      float &w =
          *(float *)(pointer + stridedId + _Offset[Weight] + i * sizeof(float));
      f.serial(w);
    }
    // CPaletteSkin (version 1+ only).
    if ((ver >= 1) &&
        ((_Flags & PaletteSkinFlag) == CVertexBuffer::PaletteSkinFlag)) {
      CPaletteSkin &ps =
          *(CPaletteSkin *)(pointer + stridedId + _Offset[PaletteSkin]);
      f.serial(ps);
    }
  }
...
void CVertexBuffer::serialHeader(NLMISC::IStream &f) {
  ...
  */
  ...
  // All other infos (but _NonResidentVertices) are computed by initEx() and
  // setNumVertices().
...
void CVertexBuffer::serialSubset(NLMISC::IStream &f, uint vertexStart,
                                 uint vertexEnd) {
  /* ***********************************************
   *	WARNING: This Class/Method must be thread-safe (ctor/dtor/serial): no
   *static access for instance It can be loaded/called through CAsyncFileManager
  ...
  for (uint id = vertexStart; id < vertexEnd; id++) {
    ...
    for (uint value = 0; value < NumValue; value++) {
      ...
      if (_Flags & (1 << value)) {
        ...
        void *ptr = (void *)((&(*_NonResidentVertices.begin())) +
                             id * _VertexSize + getValueOffEx((TValue)value));
...
bool CVertexBuffer::setVertexColorFormat(TVertexColorType format) {
  ...
  if ((TVertexColorType)_VertexColorFormat != format) {
    ...
    if (_Flags & (PrimaryColorFlag | SecondaryColorFlag)) {
      uint i;
      uint32 *ptr0 =
          (_Flags & PrimaryColorFlag)
              ? (uint32 *)&(_NonResidentVertices[_Offset[PrimaryColor]])
              : NULL;
      uint32 *ptr1 =
          (_Flags & SecondaryColorFlag)
              ? (uint32 *)&(_NonResidentVertices[_Offset[SecondaryColor]])
              : NULL;
      for (i = 0; i < _NbVerts; i++) {
        if (ptr0) {
          const NL_REGISTER uint32 value = *ptr0;
#ifdef NL_LITTLE_ENDIAN
          *ptr0 = (value & 0xff00ff00) | ((value & 0xff) << 16) |
                  ((value & 0xff0000) >> 16);
#else  // NL_LITTLE_ENDIAN
          *ptr0 = (value & 0x00ff00ff) | ((value & 0xff00) << 16) |
                  ((value & 0xff000000) >> 16);
...
#ifdef NL_LITTLE_ENDIAN
          *ptr1 = (value & 0xff00ff00) | ((value & 0xff) << 16) |
                  ((value & 0xff0000) >> 16);
#else  // NL_LITTLE_ENDIAN
          *ptr1 = (value & 0x00ff00ff) | ((value & 0xff00) << 16) |
                  ((value & 0xff000000) >> 16);
#endif // NL_LITTLE_ENDIAN
...
void CVertexBuffer::setLocation(TLocation newLocation) {
  ...
  if (newLocation != NotResident) {
    ...
    const uint size =
        ((_PreferredMemory == RAMVolatile) || (_PreferredMemory == AGPVolatile))
            ? _NbVerts * _VertexSize
            : _Capacity * _VertexSize;
    ...
    nlassert(
        size <=
        _NonResidentVertices.size()); // Internal buffer must have the good size
    memcpy(dest, &(_NonResidentVertices[0]), size);
    DrvInfos->unlock(0, 0);

    // Reset the non resident container if not a static preferred memory and not
    // put in RAM
    if ((_PreferredMemory != StaticPreferred) && (_Location != RAMResident) &&
        !_KeepLocalMemory)
      contReset(_NonResidentVertices);
    ...
    if ((_Location == RAMResident) && (_PreferredMemory != RAMVolatile) &&
        (_PreferredMemory != AGPVolatile) && !_KeepLocalMemory) {
      // The driver must have setuped the driver info
      nlassert(DrvInfos);
      ...
      if (!_NonResidentVertices.empty())
        memcpy(&(_NonResidentVertices[0]), src,
               std::min(size, (uint)_ResidentSize));
      DrvInfos->unlock(0, 0);
    }
...
IVBDrvInfos::~IVBDrvInfos() { _Driver->removeVBDrvInfoPtr(_DriverIterator); }
...
CPaletteSkin *CVertexBufferReadWrite::getPaletteSkinPointer(uint idx) {
  ...
  if ((_Parent->_Flags & CVertexBuffer::PaletteSkinFlag) !=
...
CVertexBufferRead::getVertexCoordPointer(uint idx) const {
  nlassert(_Parent->checkLockedBuffer());
  const uint8 *ptr;
...
const NLMISC::CVector *
CVertexBufferRead::getNormalCoordPointer(uint idx) const {
  nlassert(_Parent->checkLockedBuffer());
  const uint8 *ptr;
...
const NLMISC::CUV *CVertexBufferRead::getTexCoordPointer(uint idx,
                                                         uint8 stage) const {
  nlassert(_Parent->checkLockedBuffer());
  const uint8 *ptr;

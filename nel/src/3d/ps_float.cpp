...
CPSFloatGradient::CPSFloatGradient(const float *floatTab, uint32 nbValues,
...
///=======================================================================================
const CPSFloatCurveFunctor::CCtrlPoint &
CPSFloatCurveFunctor::getControlPoint(uint index) const {
  return _CtrlPoints[index];
}

///=======================================================================================
void CPSFloatCurveFunctor::setCtrlPoint(uint index,
...
float CPSFloatCurveFunctor::getValue(float date) const {
  ...
  while (it != _CtrlPoints.end() && it->Date <= date)
    ++it;

  if (it == _CtrlPoints.begin())
    return _CtrlPoints[0].Value;
  if (it == _CtrlPoints.end())
    return _CtrlPoints[_CtrlPoints.size() - 1].Value;
  CPSVector<CCtrlPoint>::V::const_iterator precIt = it - 1;
  if (precIt->Date == it->Date)
    return 0.5f * (precIt->Value + it->Value);
...
float CPSFloatCurveFunctor::getSlope(uint index) const {
  ...
  if (index == 0) {
    return _CtrlPoints[1].Date != _CtrlPoints[0].Date
               ? (_CtrlPoints[1].Value - _CtrlPoints[0].Value) /
  ...
  if (index == _CtrlPoints.size() - 1) {
    return _CtrlPoints[index].Date != _CtrlPoints[index - 1].Date
               ? (_CtrlPoints[index].Value - _CtrlPoints[index - 1].Value) /
                     (_CtrlPoints[index].Date - _CtrlPoints[index - 1].Date)
               : 1e6f;
  }

  // tangent for other points
  return _CtrlPoints[index + 1].Date != _CtrlPoints[index - 1].Date
             ? (_CtrlPoints[index + 1].Value - _CtrlPoints[index - 1].Value) /

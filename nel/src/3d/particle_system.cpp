...
// bool FilterPS[10] = { false, false, false, false, false, false, false, false,
// false, false };
...
static const float PS_MIN_TIMEOUT =
    1.f; // the test that check if there are no particles left
...
CParticleSystem::CParticleSystem()
    : _Driver(NULL), _FontGenerator(NULL), _FontManager(NULL),
      _UserCoordSystemInfo(NULL), _Date(0), _LastUpdateDate(-1),
      _CurrEditedElementLocated(NULL), _CurrEditedElementLocatedBindable(NULL),
      _CurrEditedElementIndex(0), _Scene(NULL), _TimeThreshold(0.15f),
      _SystemDate(0.f), _MaxNbIntegrations(2), _LODRatio(0.5f),
      _OneMinusCurrentLODRatio(0), _MaxViewDist(PSDefaultMaxViewDist),
      _MaxDistLODBias(0.05f), _InvMaxViewDist(1.f / PSDefaultMaxViewDist),
      _InvCurrentViewDist(1.f / PSDefaultMaxViewDist), _AutoLODEmitRatio(0.f),
      _DieCondition(none), _DelayBeforeDieTest(-1.f), _NumWantedTris(0),
      _AnimType(AnimInCluster), _UserParamGlobalValue(NULL),
      _BypassGlobalUserParam(0), _PresetBehaviour(UserBehaviour),
      ...
      _ComputeBBox(true), _BBoxTouched(true), _AccurateIntegration(true),
      _CanSlowDown(true), _DestroyModelWhenOutOfRange(false),
      _DestroyWhenOutOfFrustum(false), _Sharing(false), _AutoLOD(false),
      ...
      _AutoLODStartDistPercent(0.1f), _AutoLODDegradationExponent(1) {
...
std::vector<NLMISC::CSmartPtr<CParticleSystem::CSpawnVect>>
    CParticleSystem::_Spawns; // spawns for the current system being processed
...
void CParticleSystem::notifyMaxNumFacesChanged(void)
{
    ...
    for (TProcessVect::iterator it = _ProcessVect.begin(); it !=
_ProcessVect.end(); ++it)
    {
...
void CParticleSystem::updateNumWantedTris() {
  ...
  _NumWantedTris = 0;
  for (TProcessVect::iterator it = _ProcessVect.begin();
       it != _ProcessVect.end(); ++it) {
...
float CParticleSystem::getWantedNumTris(float dist) {
  ...
  if (dist > _MaxViewDist)
    return 0;
  float retValue = ((1.f - dist * _InvMaxViewDist) * _NumWantedTris);
  /// nlassertex(retValue >= 0 && retValue < 10000, ("dist = %f, _MaxViewDist =
  /// %f, _MaxNumFacesWanted = %d, retValue = %f",  dist, _MaxViewDist,
  /// _MaxNumFacesWanted, retValue));
  return retValue;
}
...
void CParticleSystem::setNumTris(uint numFaces) {
  ...
  if (_EnableLoadBalancing) {
    ...
    if (numFaces >= wantedNumTri || wantedNumTri == 0 || _NumWantedTris == 0 ||
        modelDist < epsilon) {
      _InvCurrentViewDist = _InvMaxViewDist;
    } else {

      _InvCurrentViewDist =
          (_NumWantedTris - numFaces) / (_NumWantedTris * modelDist);
    }
  } else {
...
bool CParticleSystem::hasEmitters() const {
  ...
  for (TProcessVect::const_iterator it = _ProcessVect.begin();
       it != _ProcessVect.end(); ++it) {
    if ((*it)->hasEmitters())
      return true;
  }
...
#endif
    float
    CParticleSystem::getDistFromViewer() const {
...
#endif
    float
    CParticleSystem::updateLODRatio() {
...
void CParticleSystem::step(TPass pass, TAnimationTime ellapsedTime,
  ...
  case Anim: {
    if (ellapsedTime <= 0.f)
      return;
    // update user param from global value if needed, unless this behaviour is
    // bypassed has indicated by a flag in _BypassGlobalUserParam
    if (_UserParamGlobalValue) {
      nlctassert(MaxPSUserParam < 8); // there should be less than 8 parameters
                                      // because of mask stored in a byte
      uint8 bypassMask = 1;
      for (uint k = 0; k < MaxPSUserParam; ++k) {
        if (_UserParamGlobalValue[k] &&
            !(_BypassGlobalUserParam &
              bypassMask)) // if there is a global value for this param and if
                           // the update is not bypassed
        {
        ...
        if (!_BypassIntegrationStepLimit && nbPass > _MaxNbIntegrations) {
          ...
          if (_CanSlowDown) {
            ...
            InverseTotalEllapsedTime =
                ellapsedTime != 0 ? 1.f / ellapsedTime : 0.f;
          }
        } else {
          EllapsedTime = ellapsedTime / nbPass;
          InverseTotalEllapsedTime =
    ...
    updateLODRatio();
    {
      ...
      if (_AutoLOD && !_Sharing) {
        ...
        if (currLODRatio <= _AutoLODStartDistPercent) {
          ...
          float lodValue =
              (currLODRatio - 1.f) / (_AutoLODStartDistPercent - 1.f);
          ...
          _AutoLODEmitRatio =
              (1.f - _MaxDistLODBias) * finalValue + _MaxDistLODBias;
          if (_AutoLODEmitRatio < 0.f)
    ...
    {
      MINI_TIMER(PSAnim4)
      // set start position. Used by emitters that emit from Local basis to
      // world
      if (!_HiddenAtPreviousFrame && !_HiddenAtCurrentFrame) {
        _CoordSystemInfo.CurrentDeltaPos =
      ...
      // displaySysPos(_Driver, _CurrentDeltaPos + _OldSysMat.getPos(),
      // CRGBA::Red);
      //  process passes
    }
    RealEllapsedTime =
        _KeepEllapsedTimeForLifeUpdate ? (ellapsedTime / nbPass) : EllapsedTime;
    ...
    // Sort by emission depth. We assume that the ps is a directed acyclic graph
    // (so no loop will be encountered)
    ...
    do {
      {
        ...
        _CoordSystemInfo.CurrentDeltaPos +=
            (_CoordSystemInfo.Matrix->getPos() - _CoordSystemInfo.OldPos) *
            (EllapsedTime * InverseTotalEllapsedTime);
        if (_UserCoordSystemInfo) {
          CCoordSystemInfo &csi = _UserCoordSystemInfo->CoordSystemInfo;
          csi.CurrentDeltaPos += (csi.Matrix->getPos() - csi.OldPos) *
                                 (EllapsedTime * InverseTotalEllapsedTime);
        }
      }
      for (uint k = 0; k < shape._ProcessOrder.size(); ++k) {
        if (!_ProcessVect[shape._ProcessOrder[k]]->isLocated())
          continue;
        CPSLocated *loc =
            static_cast<CPSLocated *>(_ProcessVect[shape._ProcessOrder[k]]);
        ...
        if (loc->getSize() != 0) {
          ...
          if (!loc->isParametricMotionEnabled())
            loc->computeMotion();
          // Update life and mark particles that must be removed
          loc->updateLife();
          // Spawn particles. Emitters date is updated only after so we check in
          // CPSLocated::postNewElement if the emitter was still alive at this
          // date, otherwise we discard the post
          loc->computeSpawns(0, false);
          if (loc->hasCollisionInfos())
            loc->updateCollisions();
        ...
        if (!_Spawns[shape._ProcessOrder[k]]->SpawnInfos.empty()) {
          uint insertionIndex =
              loc->getSize(); // index at which new particles will be inserted
          // add new particles that where posted by ancestor emitters, and also
          // mark those that must already be deleted
          loc->addNewlySpawnedParticles();
          if (insertionIndex != loc->getSize()) {
            // Compute motion for spawned particles. This is useful only if
            // particle can collide because
            if (loc->hasCollisionInfos()) {
              loc->resetCollisions(loc->getSize());
    ...
    } while (--nbPass);
    {
      ...
      for (TProcessVect::iterator it = _ProcessVect.begin();
           it != _ProcessVect.end(); ++it) {
        if ((*it)->isParametricMotionEnabled())
          (*it)->performParametricMotion(_SystemDate);
      }
    }
...
void CParticleSystem::serial(NLMISC::IStream &f) {
  ...
  // version 8: Replaced the attribute '_PerformMotionWhenOutOfFrustum' by a
  // _AnimType field which allow more precise control

  // f.serial(_ViewMat);

  // patch for old fx : force to recompute duration when fx is saved to avoid
  // prbs
  if (!f.isReading()) {
  ...
  if (version > 3) // tell whether the system must compute his bbox, hold a
                   // precomputed bbox
  {
  ...
  {
    bool destroyModelWhenOutOfRange =
        _DestroyModelWhenOutOfRange; // read from bitfield
  ...
  if (version > 5) {
    bool destroyWhenOutOfFrustum =
        _DestroyWhenOutOfFrustum; // read from bitfield
  ...
  if (version >= 12) {
    ...
    nlctassert(MaxPSUserParam < 8); // In this version mask of used global user
                                    // params are stored in a byte..
    if (f.isReading()) {
      uint8 mask;
      f.serial(mask);
      if (mask) {
  ...
  if (version >= 15) {
    bool bypassIntegrationStepLimit =
        _BypassIntegrationStepLimit; // read from bitfield
    f.serial(bypassIntegrationStepLimit);
    _BypassIntegrationStepLimit = bypassIntegrationStepLimit;
  }

  if (version >= 17) {
    bool forceGlobalColorLighting =
        _ForceGlobalColorLighting; // read from bitfield
    f.serial(forceGlobalColorLighting);
    _ForceGlobalColorLighting = forceGlobalColorLighting;
  }

  if (version >= 18) {
    bool autoComputeDelayBeforeDeathTest =
        _AutoComputeDelayBeforeDeathTest; // read from bitfield
    f.serial(autoComputeDelayBeforeDeathTest);
    _AutoComputeDelayBeforeDeathTest = autoComputeDelayBeforeDeathTest;
  } else {
    nlassert(f.isReading());
    // for all previously created system, force to eval the system duration in
    // an automatyic way
...
bool CParticleSystem::attach(CParticleSystemProcess *ptr) {
  ...
  nlassert(ptr);
  nlassert(std::find(_ProcessVect.begin(), _ProcessVect.end(), ptr) ==
           _ProcessVect.end()); // can't attach twice
  ...
  if (getBypassMaxNumIntegrationSteps()) {
    if (!canFinish()) {
      remove(ptr);
      nlwarning("<void CParticleSystem::attach> Can't attach object : this "
                "causes the system to last forever, and it has been flagged "
                "with 'BypassMaxNumIntegrationSteps'. Object is not attached");
      return false;
    }
  }
...
void CParticleSystem::remove(CParticleSystemProcess *ptr) {
  ...
  TProcessVect::iterator it =
      std::find(_ProcessVect.begin(), _ProcessVect.end(), ptr);
...
void CParticleSystem::forceComputeBBox(NLMISC::CAABBox &aabbox) {
  ...
  for (TProcessVect::const_iterator it = _ProcessVect.begin();
    ...
    if ((*it)->computeBBox(tmpBox)) {
      ...
      const CMatrix &convMat = CPSLocated::getConversionMatrix(
          *this, PSFXWorldMatrix, (*it)->getMatrixMode());
...
void CParticleSystem::setUserMatrix(const NLMISC::CMatrix *m) {
  ...
  if (_SystemDate == 0.f) {
    csi.OldPos =
        m ? m->getPos()
          : getSysMat()
                .getPos(); // _CoordSystemInfo.Matrix is relevant if at least
                           // one call to setSysMat has been performed before
  }
  if (!m)
    return;
  ...
  _UserCoordSystemInfo->UserBasisToFXBasis =
      _CoordSystemInfo.InvMatrix * *(csi.Matrix);
    ...
    if ((*it)->isLocated()) {
      ...
      for (uint k = 0; k < loc->getNbBoundObjects(); ++k) {
        ...
        if (lb->getType() == PSParticle) {
          if (((CPSParticle *)lb)->hasOpaqueFaces())
            return true;
        }
      }
    }
  ...
  for (TProcessVect::const_iterator it = _ProcessVect.begin();
    ...
    if ((*it)->isLocated()) {
      ...
      for (uint k = 0; k < loc->getNbBoundObjects(); ++k) {
        CPSLocatedBindable *lb = loc->getBoundObject(k);
        if (lb->getType() == PSParticle) {
          if (((CPSParticle *)lb)->hasTransparentFaces())
            return true;
        }
      }
    }
  ...
  for (TProcessVect::const_iterator it = _ProcessVect.begin();
    ...
    if ((*it)->isLocated()) {
      ...
      for (uint k = 0; k < loc->getNbBoundObjects(); ++k) {
        CPSLocatedBindable *lb = loc->getBoundObject(k);
        if (lb->getType() == PSParticle) {
          if (((CPSParticle *)lb)->hasLightableFaces())
            return true;
          if (((CPSParticle *)lb)->usesGlobalColorLighting())
            return true;
        }
      }
    }
...
///=======================================================================================
void CParticleSystem::getLODVect(NLMISC::CVector &v, float &offset,
  ...
  case PSUserMatrix: {
    const CVector tv =
        getInvertedUserMatrix().mulVector(_InvertedViewMat.getJ());
...
TPSLod CParticleSystem::getLOD(void) const {
  ...
  const float dist = fabsf(_InvCurrentViewDist *
                           (getSysMat().getPos() - _InvertedViewMat.getPos()) *
                           _InvertedViewMat.getJ());
  return dist > _LODRatio ? PSLod2 : PSLod1;
}

///=======================================================================================
void CParticleSystem::registerLocatedBindableExternID(uint32 id,
                                                      CPSLocatedBindable *lb) {
  NL_PS_FUNC_MAIN(CParticleSystem_registerLocatedBindableExternID)
  nlassert(lb);
  nlassert(lb->getOwner() &&
           lb->getOwner()->getOwner() ==
               this); // the located bindable must belong to that system
#ifdef NL_DEBUG
...
///=======================================================================================
void CParticleSystem::unregisterLocatedBindableExternID(
    CPSLocatedBindable *lb) {
...
///=======================================================================================
CPSLocatedBindable *CParticleSystem::getLocatedBindableByExternID(uint32 id,
                                                                  uint index) {
...
///=======================================================================================
const CPSLocatedBindable *
CParticleSystem::getLocatedBindableByExternID(uint32 id, uint index) const {
  NL_PS_FUNC_MAIN(CParticleSystem_getLocatedBindableByExternID)
  if (index >= _LBMap.count(id)) {
    return NULL;
  }
...
bool CParticleSystem::merge(CParticleSystemShape *pss) {
  ...
  nlassert(pss);
  nlassert(_Scene);
  CParticleSystem *duplicate =
      pss->instanciatePS(*this->_Scene); // duplicate the p.s. to merge
  // now we transfer the located of the duplicated ps to this object...
  for (TProcessVect::iterator it = duplicate->_ProcessVect.begin();
       it != duplicate->_ProcessVect.end(); ++it) {
    if (!attach(*it)) {
      for (TProcessVect::iterator clearIt = duplicate->_ProcessVect.begin();
           clearIt != it; ++it) {
        detach(getIndexOf(**it));
      }
      nlwarning("<CParticleSystem::merge> Can't do the merge : this causes the "
                "system to last forever, and it has been flagged with "
                "'BypassMaxNumIntegrationSteps'. Merge is not done.");
      return false;
    }
  }
  //
...
///=======================================================================================
void CParticleSystem::interpolateFXPosDelta(NLMISC::CVector &dest,
                                            TAnimationTime deltaT) {
  NL_PS_FUNC_MAIN(CParticleSystem_interpolateFXPosDelta)
  nlassert(_CoordSystemInfo.Matrix);
  dest = _CoordSystemInfo.CurrentDeltaPos -
         (deltaT * InverseTotalEllapsedTime) *
             (_CoordSystemInfo.Matrix->getPos() - _CoordSystemInfo.OldPos);
}

///=======================================================================================
void CParticleSystem::interpolateUserPosDelta(NLMISC::CVector &dest,
                                              TAnimationTime deltaT) {
  ...
  if (!_UserCoordSystemInfo) {
    ...
    CCoordSystemInfo &csi = _UserCoordSystemInfo->CoordSystemInfo;
    dest = csi.CurrentDeltaPos - (deltaT * InverseTotalEllapsedTime) *
                                     (csi.Matrix->getPos() - csi.OldPos);
  }
}

///=======================================================================================
void CParticleSystem::bindGlobalValueToUserParam(
    const std::string &globalValueName, uint userParamIndex) {
  ...
  if (globalValueName.empty()) // disable a user param global value
  {
    ...
    for (uint k = 0; k < MaxPSUserParam; ++k) {
      if (_UserParamGlobalValue[k] != NULL)
        return;
    }
    // no more entry used
    delete _UserParamGlobalValue;
  ...
  } else // enable a user param global value
  {
    if (!_UserParamGlobalValue) {
      ...
      _UserParamGlobalValue =
          new const TGlobalValuesMap::value_type *[MaxPSUserParam];
      std::fill(_UserParamGlobalValue, _UserParamGlobalValue + MaxPSUserParam,
                (TGlobalValuesMap::value_type *)NULL);
    }
    // has the global value be created yet ?
    TGlobalValuesMap::const_iterator it =
        _GlobalValuesMap.find(globalValueName);
    if (it != _GlobalValuesMap.end()) {
      // yes, make a reference on it
      _UserParamGlobalValue[userParamIndex] = &(*it);
    } else {
      // create a new entry
      std::pair<TGlobalValuesMap::iterator, bool> itPair =
          _GlobalValuesMap.insert(
              TGlobalValuesMap::value_type(globalValueName, 0.f));
...
std::string CParticleSystem::getGlobalValueName(uint userParamIndex) const {
  ...
  if (!_UserParamGlobalValue)
    return "";
  if (!_UserParamGlobalValue[userParamIndex])
    return "";
  return _UserParamGlobalValue[userParamIndex]->first;
}

///=======================================================================================
void CParticleSystem::setGlobalVectorValue(const std::string &name,
...
///=======================================================================================
CParticleSystem::CGlobalVectorValueHandle
CParticleSystem::getGlobalVectorValueHandle(const std::string &name) {
  NL_PS_FUNC(CParticleSystem_getGlobalVectorValueHandle)
  nlassert(!name.empty());
  TGlobalVectorValuesMap::iterator it = _GlobalVectorValuesMap.find(name);
  if (it == _GlobalVectorValuesMap.end()) {
    it = _GlobalVectorValuesMap
...
///=======================================================================================
bool CParticleSystem::canFinish(
    CPSLocatedBindable **lastingForeverObj /*= NULL*/) const {
  NL_PS_FUNC_MAIN(CParticleSystem_canFinish)
  if (hasLoop(lastingForeverObj))
    return false;
      ...
      if (loc->getLastForever()) {
        for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
          ...
          if (em && em->testEmitForever()) {
            if (lastingForeverObj)
              *lastingForeverObj = em;
            return false;
          }
...
///=======================================================================================
bool CParticleSystem::hasLoop(
    CPSLocatedBindable **loopingObj /*= NULL*/) const {
...
///=======================================================================================
void CParticleSystem::setAutoComputeDelayBeforeDeathConditionTest(
    bool computeAuto) {
  NL_PS_FUNC_MAIN(CParticleSystem_setAutoComputeDelayBeforeDeathConditionTest)
  if (computeAuto == _AutoComputeDelayBeforeDeathTest)
    return;
      ...
      while (!toVisitEmitter.empty()) {
        ...
        for (uint m = 0; m < loc->getNbBoundObjects(); ++m) {
          ...
          if (bind->getType() == PSEmitter) {
            const CPSEmitter *em =
                NLMISC::safe_cast<const CPSEmitter *>(loc->getBoundObject(m));
            const CPSLocated *emittedType = em->getEmittedType();
            // continue if there's no loop
            if (std::find(visitedEmitter.begin(), visitedEmitter.end(),
                          emittedType) == visitedEmitter.end()) {
              if (emittedType != NULL) {
                emitterFound = true;
                CToVisitEmitter tve;
                ...
                if (!loc->getLastForever()) {
                  ...
                  case CPSEmitter::regular: {
                    if (em->getMaxEmissionCount() != 0) {
                      float period = em->getPeriodScheme()
                                         ? em->getPeriodScheme()->getMaxValue()
                                         : em->getPeriod();
                      tve.Duration = duration + em->getEmitDelay() +
                                     period * em->getMaxEmissionCount();
                    } else {
                      tve.Duration = duration + em->getEmitDelay();
                    }
                  } break;
  ...
  for (uint k = 0; k < getNbProcess(); ++k) {
    if (getProcess(k)->isLocated()) {
      ...
      if (loc) {
        for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
          if (loc->getBoundObject(l)->getType() == PSEmitter) {
            if (loc->getBoundObject(l)->isActive())
              return true;
          }
        }
      }
...
void CParticleSystem::matchArraySize() {
  ...
  for (uint k = 0; k < getNbProcess(); ++k) {
    if (getProcess(k)->isLocated()) {
      CPSLocated *loc = static_cast<CPSLocated *>(getProcess(k));
      loc->resize(
          loc->getSize()); // match the max size with the number of instances
    }
  }
}
...
///=======================================================================================
void CParticleSystem::getTargeters(
    const CPSLocated *target,
  ...
  for (uint k = 0; k < getNbProcess(); ++k) {
    if (getProcess(k)->isLocated()) {
      ...
      if (loc) {
        for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
          CPSTargetLocatedBindable *targeter =
              dynamic_cast<CPSTargetLocatedBindable *>(loc->getBoundObject(l));
          if (targeter) {
...
///=======================================================================================
void CParticleSystem::matrixModeChanged(CParticleSystemProcess *proc,
...
///=======================================================================================
void CParticleSystem::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                               IDriver &drv) {
...
///=======================================================================================
void CParticleSystem::getSortingByEmitterPrecedence(
    std::vector<uint> &result) const {
  ...
  for (uint k = 0; k < _ProcessVect.size(); ++k) {
    if (_ProcessVect[k]->isLocated()) {
      ...
      for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
        if (loc->getBoundObject(l)->getType() == PSEmitter) {
          CPSEmitter *pEmit =
              NLMISC::safe_cast<CPSEmitter *>(loc->getBoundObject(l));
  ...
  for (uint k = 0; k < inDegree.size(); ++k) {
    // NB : could not do resize there because we keep iterators in the list, so
    // it's done in the previous loop
...
#define DO_SBEP_CHECK                                                          \
  {                                                                            \
    for (uint k = 0; k < degreeToNodes.size(); ++k) {                          \
      for (TProcessList::const_iterator it = degreeToNodes[k].begin();         \
           it != degreeToNodes[k].end(); ++it) {                               \
...
#define DO_SBEP_CHECK
  ...
  result.clear();
  if (degreeToNodes.empty())
    return;
  // now, do the sort -> add each node with a degree of 0, and removes arc to
  // their son (and insert in new good list according to their degree)
      ...
      for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
        if (loc->getBoundObject(l)->getType() == PSEmitter) {
          ...
          if (pEmit->getEmittedType()) {
            ...
            nlassert(degree != 0);
            degreeToNodes[degree - 1].splice(degreeToNodes[degree - 1].begin(),
                                             degreeToNodes[degree],
                                             nodeToIterator[emittedLocIndex]);
            nodeToIterator[emittedLocIndex] =
                degreeToNodes[degree - 1].begin(); // update iterator
...
void CParticleSystem::dumpHierarchy() {
  ...
  for (uint k = 0; k < _ProcessVect.size(); ++k) {
    ...
    if (loc) {
      ...
      for (uint l = 0; l < loc->getNbBoundObjects(); ++l) {
        CPSEmitter *emitter =
            dynamic_cast<CPSEmitter *>(loc->getBoundObject(l));
        if (emitter) {
          nlinfo("    emitter %s : emit %s @%p", emitter->getName().c_str(),
                 emitter->getEmittedType()
                     ? emitter->getEmittedType()->getName().c_str()
                     : "none",
                 emitter->getEmittedType());
        }
      }

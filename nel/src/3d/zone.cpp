...
void CZone::computeBBScaleBias(const CAABBox &bb) {
  ...
  ZoneBB.setHalfSize(ZoneBB.getHalfSize() +
                     CVector(NL3D_NOISE_MAX, NL3D_NOISE_MAX, NL3D_NOISE_MAX));
...
// ***************************************************************************
void CZone::build(uint16 zoneId, const std::vector<CPatchInfo> &patchs,
...
void CZone::build(const CZoneInfo &zoneInfo, uint32 numVertices) {
  ...
  for (j = 0; j < (sint)patchs.size(); j++) {
    ...
    pa.Flags |=
        NL_PATCH_SMOOTH_FLAG_MASK & (pi.Flags << NL_PATCH_SMOOTH_FLAG_SHIFT);
    ...
    /* Copy TileLightInfluences. It is possible that
       pi.TileLightInfluences.size()!= 0 and pi.TileLightInfluences.size()!=
       (uint)(pi.OrderS/2+1)*(pi.OrderT/2+1) Because of a preceding bug where
       pa.OrderS and pa.OrderT were not initialized before the
        pa.resetTileLightInfluences();
    */
    if (pi.TileLightInfluences.size() !=
        (uint)(pi.OrderS / 2 + 1) * (pi.OrderT / 2 + 1)) {
      pa.resetTileLightInfluences();
    } else {
    ...
    uint lumelCount =
        (pi.OrderS * NL_LUMEL_BY_TILE) * (pi.OrderT * NL_LUMEL_BY_TILE);
...
// ***************************************************************************
void CZone::retrieve(std::vector<CPatchInfo> &patchs,
...
void CZone::retrieve(CZoneInfo &zoneInfo) {
  ...
  for (j = 0; j < (sint)patchs.size(); j++) {
    ...
    // Smoothing flags
    pi.Flags =
        (pa.Flags & NL_PATCH_SMOOTH_FLAG_MASK) >> NL_PATCH_SMOOTH_FLAG_SHIFT;
...
void CZone::CPatchConnect::serial(NLMISC::IStream &f) {
  ...
  if (ver < 1)
    ...
    f.serial(ErrorSize);
  f.xmlSerial(BaseVertices[0], BaseVertices[1], BaseVertices[2],
              BaseVertices[3], "BASE_VERTICES");
  f.xmlSerial(BindEdges[0], BindEdges[1], BindEdges[2], BindEdges[3],
              "BIND_EDGES");
}
void CPatchInfo::CBindInfo::serial(NLMISC::IStream &f) {
  ...
  nlassert((NPatchs == 0) | (NPatchs == 1) | (NPatchs == 2) | (NPatchs == 4) |
           (NPatchs == 5));
...
void CZone::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  // If read, must create and init Patch Clipped state to true (clipped even if
  // not compiled)
  if (f.isReading()) {
    _PatchRenderClipped.resize((uint)Patchs.size());
    _PatchOldRenderClipped.resize((uint)Patchs.size());
...
void CZone::compile(CLandscape *landscape, TZoneMap &loadedZones) {
  ...
  for (zoneIt = neighborZones.begin(); zoneIt != neighborZones.end();
       zoneIt++) {
...
void CZone::release(TZoneMap &loadedZones) {
  ...
  // (so after rebindBorder() too...).
  for (j = 0; j < (sint)Patchs.size(); j++) {
    CPatch &pa = Patchs[j];
    pa.release();
  }

  // destroy/unlink the base vertices (internal..), according to present neigbor
  // zones.
  //=================================
...
// ***************************************************************************
void CZone::buildBindInfo(uint patchId, uint edge, CZone *neighborZone,
  ...
  nlassert(pcBind.NPatchs == 0 || pcBind.NPatchs == 1 || pcBind.NPatchs == 2 ||
           pcBind.NPatchs == 4 || pcBind.NPatchs == 5);
  ...
  if (pcBind.NPatchs == 5) {
    ...
    const CPatchInfo::CBindInfo &pcBindNeighbor =
        neighborZone->getPatchConnect(pcBind.Next[0])
            ->BindEdges[pcBind.Edge[0]];
...
void CZone::bindPatch(TZoneMap &loadedZones, CPatch &pa, CPatchConnect &pc,
  ...
  for (sint i = 0; i < 4; i++) {
    ...
    nlassert(pcBind.NPatchs == 0 || pcBind.NPatchs == 1 ||
             pcBind.NPatchs == 2 || pcBind.NPatchs == 4 || pcBind.NPatchs == 5);
    paBind.NPatchs = pcBind.NPatchs;
    ...
    if (paBind.NPatchs == 0 ||
        (itZoneMap = loadedZones.find(pcBind.ZoneId)) == loadedZones.end())
      paBind.Zone = NULL;
    else
    ...
    if (paBind.NPatchs == 5) {
      paBind.Edge[0] = pcBind.Edge[0];
      paBind.Next[0] =
          CZone::getZonePatch(loadedZones, pcBind.ZoneId, pcBind.Next[0]);
    ...
    if (paBind.NPatchs >= 2) {
      ...
      paBind.Next[1] =
          CZone::getZonePatch(loadedZones, pcBind.ZoneId, pcBind.Next[1]);
      // If not loaded, don't bind to this edge.
      if (!paBind.Next[1])
        paBind.NPatchs = 0;
    }
    if (paBind.NPatchs >= 4) {
      ...
      paBind.Next[2] =
          CZone::getZonePatch(loadedZones, pcBind.ZoneId, pcBind.Next[2]);
      paBind.Next[3] =
          CZone::getZonePatch(loadedZones, pcBind.ZoneId, pcBind.Next[3]);
      // If not loaded, don't bind to this edge.
...
void CZone::unbindPatch(CPatch &pa) {
  /*
      Remind: the old version with CPatch::unbindFrom*() doesn't work because of
     CZone::release(). This function first erase the zone from loadedZones...
      Not matter here. We use CPatch::unbind() which should do all the good job
     correctly (unbind pa from ohters , and unbind others from pa at same time).
  */
...
void CZone::clip(const std::vector<CPlane> &pyramid) {
  ...
  //-------------------
  // If there is a change in the Clip of the zone, or if patchs may have change
  // (ie ClipSide is undetermined).
  if (oldClipResult != ClipResult || oldClipResult == ClipSide) {
    // get BitSet as Raw Array of uint32
    uint32 *oldRenderClip =
        const_cast<uint32 *>(&_PatchOldRenderClipped.getVector()[0]);
...
void CZone::refineAll() {
  ...
  // DO NOT do a forceNoRenderClip(), to avoid big allocation of Near/Far VB
  // vertices in driver. DO NOT modify ClipResult, to avoid big allocation of
  // Near/Far VB vertices in driver.

  // refine ALL patchs (even those which may be invisible).
  CPatch *pPatch = &(*Patchs.begin());
  sint n;
  for (n = (sint)Patchs.size(); n > 0; n--, pPatch++) {
    // For Pacs construction: may exclude some patch from refineAll (for speed
    // improvement).
...
void CZone::preRender() {
  ...
  */
  ...
  // else If some patchs only are visible.
    ...
    // PreRender Pass, or updateTextureFarOnly(), according to
    // _PatchRenderClipped state.
    for (uint i = 0; i < Patchs.size(); i++) {
...
// ***************************************************************************
void CZone::changePatchTextureAndColor(sint numPatch,
  ...
  if (Compiled) {
    // If the patch is visible, then we must LockBuffers, because new VertexVB
    // may be created.
...
void CZone::refreshTesselationGeometry(sint numPatch) {
  ...
  // At next render, we must re-fill the entire unclipped VB, so change are
  // taken into account.
...
void CZone::debugBinds(FILE *f) {
  ...
  for (i = 0; i < (sint)PatchConnects.size(); i++) {
    ...
    for (sint j = 0; j < 4; j++) {
      ...
      fprintf(f, "    edge%d: Zone:%u. NPatchs:%u. ", j, (uint)bd.ZoneId,
              (uint)bd.NPatchs);
...
void CZone::applyHeightField(const CLandscape &landScape) {
  ...
  for (j = 0; j < (sint)patchs.size(); j++) {
    ...
    for (i = 0; i < 4; i++)
      p.Vertices[i] +=
          landScape.getHeightFieldDeltaZ(p.Vertices[i].x, p.Vertices[i].y);
    for (i = 0; i < 8; i++)
      p.Tangents[i] +=
          landScape.getHeightFieldDeltaZ(p.Tangents[i].x, p.Tangents[i].y);
    for (i = 0; i < 4; i++)
      p.Interiors[i] +=
          landScape.getHeightFieldDeltaZ(p.Interiors[i].x, p.Interiors[i].y);
  }
...
// ***************************************************************************
bool CPatchInfo::getNeighborTile(uint patchId, uint edge, sint position,
                                 uint &patchOut, sint &sOut, sint &tOut,
  ...
  case 4: {
    ...
    // Check neighbor
    uint neighborRealLength = (neighborEdge & 1) ? patchInfos[patchOut].OrderS
                                                 : patchInfos[patchOut].OrderT;
  ...
  case 5: {
    ...
    uint neighborEdgeCount =
        patchInfos[patchOut].BindEdges[neighborEdge].NPatchs;
    ...
    if ((neighborRealLength / neighborEdgeCount) == length) {
      ...
      for (neighborPosition = 0; neighborPosition < neighborEdgeCount;
           neighborPosition++) {
        // Found ?
        if (patchInfos[patchOut]
                .BindEdges[neighborEdge]
                .Next[neighborPosition] == patchId)
          break;
      }
      ...
      neighborPosition =
          (neighborPosition + 1) * (neighborRealLength / neighborEdgeCount) -
          position - 1;
...
bool CPatchInfo::getTileSymmetryRotate(const CTileBank &bank, uint tile,
...
bool CPatchInfo::transformTile(const CTileBank &bank, uint &tile,
    ...
    if (tile < (uint)bank.getTileCount()) {
      ...
      if (type == CTileBank::transition) {
        ...
        CTileSet::TFlagBorder oriented[4] = {
            pTileSet->getOrientedBorder(
                CTileSet::left,
                CTileSet::getEdgeType((CTileSet::TTransition)number,
                                      CTileSet::left)),
            pTileSet->getOrientedBorder(
                CTileSet::bottom,
                CTileSet::getEdgeType((CTileSet::TTransition)number,
        ...
        edges[0] = pTileSet->getOrientedBorder(CTileSet::left,
                                               oriented[(0 + transRotate) & 3]);
        edges[1] = pTileSet->getOrientedBorder(CTileSet::bottom,
                                               oriented[(1 + transRotate) & 3]);
        edges[2] = pTileSet->getOrientedBorder(CTileSet::right,
                                               oriented[(2 + transRotate) & 3]);
        edges[3] = pTileSet->getOrientedBorder(CTileSet::top,
                                               oriented[(3 + transRotate) & 3]);

        // Get the good tile number
        CTileSet::TTransition transition =
            pTileSet->getTransitionTile(edges[3], edges[1], edges[0], edges[2]);
...
void CPatchInfo::transform256Case(const CTileBank &bank, uint8 &case256,
                                  uint tileRotation, bool symmetry, uint rotate,
                                  bool goofy) {
  // Tile exist ?
  if ((rotate != 0) || symmetry) {
...
bool CPatchInfo::transform(std::vector<CPatchInfo> &patchInfo,
                           NL3D::CZoneSymmetrisation &zoneSymmetry,
                           const NL3D::CTileBank &bank, bool symmetry,
                           uint rotate, float snapCell, float weldThreshold,
  ...
  // Build the structure
  if (!zoneSymmetry.build(patchInfo, snapCell, weldThreshold, bank, error,
                          toOriginalSpace)) {
    return false;
  }
      ...
      for (uint edge = 0; edge < 4; edge++) {
        ...
        if ((bindEdge.NPatchs > 1) && (bindEdge.NPatchs != 5)) {
          for (next = 0; next < (uint)bindEdge.NPatchs / 2; next++) {
            swap(bindEdge.Next[bindEdge.NPatchs - next - 1],
                 bindEdge.Next[next]);
            swap(bindEdge.Edge[bindEdge.NPatchs - next - 1],
                 bindEdge.Edge[next]);
          }
        }
      ...
      for (u = 0; u < pi.OrderS; u++) {
        ...
        for (int l = 0; l < 3; l++) {
          ...
          if (element.Tile[l] != 0xffff) {
            ...
            bool goofy =
                symmetry &&
                (zoneSymmetry.getTileState(i, uSymmetry + v * pi.OrderS, l) ==
                 CZoneSymmetrisation::Goofy);

            // Transform the transfo
            if (getTileSymmetryRotate(bank, tile, tileSymmetry, tileRotate)) {
              // Transform the tile
              if (!transformTile(bank, tile, tileRotation, tileSymmetry,
                                 (4 - tileRotate) & 3, goofy)) {
                // Info
        ...
        if (element.Tile[0] != 0xffff) {
          ...
          if (is256x256) {
            ...
            bool tileSymmetry = symmetry;
            uint tileRotation =
                tiles[uSymmetry + v * pi.OrderS].getTileOrient(0);
            bool goofy =
                symmetry &&
                (zoneSymmetry.getTileState(i, uSymmetry + v * pi.OrderS, 0) ==
                 CZoneSymmetrisation::Goofy);

            // Transform the transfo
            getTileSymmetryRotate(bank, element.Tile[0], tileSymmetry,
                                  tileRotate);

            // Transform the case
            transform256Case(bank, uvOff, tileRotation, tileSymmetry,
                             (4 - tileRotate) & 3, goofy);

...
// ***************************************************************************************************
CPSSound::CPSSound()
    : _Gain(1.f), _GainScheme(NULL), _Pitch(1.f), _PitchScheme(NULL),
      _EmissionPercent(1), _SpawnSounds(false), _Mute(false),
      _SoundStopped(false), _SoundReactivated(false), _UseOriginalPitch(false) {
...
void CPSSound::step(TPSProcessPass pass) {
  ...
  CPSAttrib<UPSSoundInstance *>::iterator it = _Sounds.begin(), endIt;
  CPSAttrib<NLMISC::CVector>::const_iterator posIt = _Owner->getPos().begin();
  CPSAttrib<NLMISC::CVector>::const_iterator speedIt =
    ...
    currVol =
        _GainScheme
            ? (float *)_GainScheme->make(getOwner(), size - leftToDo, Gains,
                                         sizeof(float), toProcess, true)
            : &_Gain;
    if (!_UseOriginalPitch) {
      // compute pitch
      currPitch = _PitchScheme
                      ? (float *)_PitchScheme->make(getOwner(), size - leftToDo,
      ...
      do {
        ...
        {
          (*it)->setSoundParams(*currVol, localToWorld * *posIt,
                                localToWorld.mulVector(*speedIt), *currPitch);
          if ((*it)->isLooping() && !(*it)->isPlaying()) {
            // looping sources do not play when they are clipped, so "reminds"
            // the source to play when possible.
            (*it)->play();
          }
        }
        currVol += GainPtInc;
...
void CPSSound::setGainScheme(CPSAttribMaker<float> *Gain) {
  ...
  _GainScheme = Gain;
  if (_Owner) {
    if (_GainScheme && _GainScheme->hasMemory())
      _GainScheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  }
}
...
void CPSSound::setPitchScheme(CPSAttribMaker<float> *pitch) {
  ...
  _PitchScheme = pitch;
  if (_Owner) {
    if (_PitchScheme && _PitchScheme->hasMemory())
      _PitchScheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  }
}
...
void CPSSound::serial(NLMISC::IStream &f) {
  ...
  // causes unnecessary id lookups from string.
  if (f.isReading()) {
  ...
  if (f.isReading()) {
    f.serial(
        nbSounds); // we are very unlikely to save a system with sounds being
                   // played in it, but we need to keep datas coherency.
    if (_Owner) {
...
void CPSSound::newElement(const CPSEmitterInfo &info) {
  ...
  nlassert(_Owner);
  if (_GainScheme && _GainScheme->hasMemory())
    _GainScheme->newElement(info);
  if (_PitchScheme && _PitchScheme->hasMemory())
    _PitchScheme->newElement(info);
  ...
  if (!_Mute && !_SoundStopped && CParticleSystem::getSoundServer()) {
    if ((rand() % 99) * 0.01f < _EmissionPercent) {
      uint32 index =
          _Sounds.insert(CParticleSystem::getSoundServer()->createSound(
              _SoundName, _SpawnSounds));
      ...
      if (_Sounds[index]) {
        ...
        _Sounds[index]->setSoundParams(
            _GainScheme ? _GainScheme->get(getOwner(), 0) : _Gain,
            mat * _Owner->getPos()[index], _Owner->getSpeed()[index], pitch);
        _Sounds[index]->play();
      }
    } else {
      _Sounds.insert(NULL);
...
void CPSSound::deleteElement(uint32 index) {
  ...
  if (_GainScheme && _GainScheme->hasMemory())
    _GainScheme->deleteElement(index);
  if (_PitchScheme && _PitchScheme->hasMemory())
    _PitchScheme->deleteElement(index);
  if (_Sounds[index]) {
...
void CPSSound::resize(uint32 size) {
  ...
  nlassert(size < (1 << 16));
  if (_GainScheme && _GainScheme->hasMemory())
    _GainScheme->resize(size, getOwner()->getSize());
  if (_PitchScheme && _PitchScheme->hasMemory())
    _PitchScheme->resize(size, getOwner()->getSize());

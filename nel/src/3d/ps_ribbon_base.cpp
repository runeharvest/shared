...
static inline void BuildHermiteVector(const NLMISC::CVector &P0,
                                      ...
                                      NLMISC::CVector &dest, float lambda) {
...
//=======================================================
CPSRibbonBase::CPSRibbonBase()
    : _NbSegs(8), _SegDuration(0.02f), _Parametric(false), _RibbonIndex(0),
      _MatrixMode(FatherMatrix), _LastUpdateDate(0), _RibbonMode(VariableSize),
      _InterpolationMode(Hermitte), _RibbonLength(1),
      _SegLength(_RibbonLength / _NbSegs), _LODDegradation(1)
...
void CPSRibbonBase::updateGlobals() {
  ...
  if (!size)
    return;
  const TAnimationTime currDate =
      _Owner->getOwner()->getSystemDate() + CParticleSystem::RealEllapsedTime;
  if (currDate - _LastUpdateDate >= _SegDuration) {
    if (_RibbonIndex == 0)
      _RibbonIndex = _NbSegs + EndRibbonStorage;
    else
      --_RibbonIndex;

    /// decal date
    ::memmove(&_SamplingDate[1], &_SamplingDate[0],
              sizeof(float) * (_NbSegs + EndRibbonStorage));
  ...
  if (mm == _Owner->getMatrixMode()) {
    ...
    const CMatrix &mat = CPSLocated::getConversionMatrix(
        *_Owner->getOwner(), mm, _Owner->getMatrixMode());
    TPSAttribVector::iterator posIt = _Owner->getPos().begin();
    NLMISC::CVector *currIt = &_Ribbons[_RibbonIndex];
    uint k = size;
    for (;;) {
...
//=======================================================
void CPSRibbonBase::computeHermitteRibbon(
    uint index, NLMISC::CVector *dest,
    uint stride /* = sizeof(NLMISC::CVector)*/) {
  NL_PS_FUNC(CPSRibbonBase_CVector)
  nlassert(!_Parametric);
  NLMISC::CVector *startIt =
      &_Ribbons[(_NbSegs + 1 + EndRibbonStorage) * index];
...
//=======================================================
void CPSRibbonBase::computeLinearRibbon(uint index, NLMISC::CVector *dest,
                                        uint stride) {
...
/*
void CPSRibbonBase::computeLinearRibbon(uint index, NLMISC::CVector *dest, uint
stride)
{
    nlassert(!_Parametric);
    NLMISC::CVector *startIt = &_Ribbons[(_NbSegs + 1 + EndRibbonStorage) *
...
//=======================================================
void CPSRibbonBase::computeLinearCstSizeRibbon(
    uint index, NLMISC::CVector *dest,
    uint stride /* = sizeof(NLMISC::CVector)*/) {
  NL_PS_FUNC(CPSRibbonBase_CVector)
  nlassert(!_Parametric);
  ...
  /// Our goal here is to match the length of the ribbon, But if it isn't moving
  /// fast enough, we must truncate it
  for (;;) {
...
//=======================================================
void CPSRibbonBase::computeHermitteCstSizeRibbon(
    uint index, NLMISC::CVector *dest,
    uint stride /* = sizeof(NLMISC::CVector)*/) {
  NL_PS_FUNC(CPSRibbonBase_CVector)
  nlassert(!_Parametric);
  ...
  /// Our goal here is to match the length of the ribbon, But if it isn't moving
  /// fast enough, we must truncate it Having a constant speed over a hermite
  /// curve is expensive, so we make a (very) rough approximation...
  for (;;) {
    /// compute length between the 2 sampling points
    const float sampleLength = (*nextIt - *currIt).norm();
    if (sampleLength > 10E-6f) {
...
//=======================================================
void CPSRibbonBase::computeRibbon(uint index, NLMISC::CVector *dest,
                                  uint stride /* = sizeof(NLMISC::CVector)*/) {
  NL_PS_FUNC(CPSRibbonBase_CVector)
...
void CPSRibbonBase::dupRibbon(uint dest, uint src) {
  ...
  ::memcpy(&_Ribbons[dest * (_NbSegs + EndRibbonStorage + 1)],
           &_Ribbons[src * (_NbSegs + EndRibbonStorage + 1)],
           sizeof(NLMISC::CVector) * (_NbSegs + 1 + EndRibbonStorage));
}
...
void CPSRibbonBase::newElement(const CPSEmitterInfo &info) {
  ...
  const NLMISC::CVector &pos =
      _Owner->getPos()[index]; // get the pos of the new element;
...
void CPSRibbonBase::deleteElement(uint32 index) {
  ...
  if (index == (size - 1))
    return; // was the last element, no permutation needed.
...
void CPSRibbonBase::initDateVect() {
  ...
  std::fill(_SamplingDate.begin(),
            _SamplingDate.begin() + (_NbSegs + 1 + EndRibbonStorage), 0.f);
}
...
void CPSRibbonBase::updateLOD() {
  ...
  float lodRatio = _LODDegradation + (1.f - _LODDegradation) * squaredRatio *

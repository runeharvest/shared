...
bool CCoarseMeshBuild::build(const std::vector<CCoarseMeshDesc> &coarseMeshes,
                             std::vector<NLMISC::CBitmap> &bitmaps,
                             CStats &stats, float mulArea) {
  // 1. build the bitmap
  MapBitmapDesc desc;
...
bool CCoarseMeshBuild::buildBitmap(
    const std::vector<CCoarseMeshDesc> &coarseMeshes,
    std::vector<NLMISC::CBitmap> &bitmaps, CStats &stats, MapBitmapDesc &desc,
    float mulArea) {
  ...
  // 1. scan each bitmap: calc the area of the bitmap and it its name in the
  // maps sorted by area
      ...
      for (renderPass = 0; renderPass < numRenderPass; renderPass++) {
        ...
        if (texture) {
          ...
          if (desc.find(name) == desc.end()) {
            ...
            MapBitmapDesc::iterator ite =
                desc.insert(MapBitmapDesc::value_type(name, CBitmapDesc()))
                    .first;
            ...
            for (i = 0; i < bitmaps.size(); i++) {
              ...
              if (i != 0) {
                // New size
                if ((originalWidth != texture->getWidth()) ||
                    originalHeight != texture->getHeight()) {
            ...
            // Texture area
            uint area = descBitmap.Bitmaps[0].getWidth() *
                        descBitmap.Bitmaps[0].getHeight();
  ...
  do {
    ...
    for (uint v = 0; v < heightMax; v++) {
      for (uint u = 0; u < widthMax; u++) {
        ...
        mapInsertedBitmap::iterator toTest =
            inserted.lower_bound((sint)v - (sint)maxTexHeight);
        ...
        if (enter) {
          ...
          for (i = 0; i < bitmaps.size(); i++) {
            // Check..
            nlassert((ite->second->Bitmaps[0].getWidth() ==
                      ite->second->Bitmaps[i].getWidth()) &&
                     (ite->second->Bitmaps[0].getHeight() ==
                      ite->second->Bitmaps[i].getHeight()));
...
void CCoarseMeshBuild::expand(CBitmap &bitmap) {
  ...
  if ((width != 0) && (height != 0)) {
    ...
    uint32 *bottomDest =
        ((uint32 *)&(bitmap.getPixels()[0])) + (width + 2) * (height + 1) + 1;
    ...
    *(uint32 *)&(bitmap.getPixels()[0]) = *(uint32 *)&(copy.getPixels()[0]);
    ...
    // Rigth top
    *(((uint32 *)&(bitmap.getPixels()[0])) + width + 1) =
        *(((uint32 *)&(copy.getPixels()[0])) + width - 1);

    // Rigth bottom
    *(((uint32 *)&(bitmap.getPixels()[0])) + (width + 2) * (height + 2) - 1) =
        *(((uint32 *)&(copy.getPixels()[0])) + width * height - 1);

    // Left bottom
    *(((uint32 *)&(bitmap.getPixels()[0])) + (width + 2) * (height + 1)) =
        *(((uint32 *)&(copy.getPixels()[0])) + width * (height - 1));
  }
}
...
void CCoarseMeshBuild::remapCoordinates(
    const std::vector<CCoarseMeshDesc> &coarseMeshes, const MapBitmapDesc &desc,
      ...
      for (renderPass = 0; renderPass < numRenderPass; renderPass++) {
        ...
        if (texture) {
          ...
          const CIndexBuffer &primitiveBlock =
              meshGeom->getRdrPassPrimitiveBlock(matrixBlock, renderPass);
          ...
          while (iteRemap != vertexToRemap.end()) {
            ...
            UVCoordinate[0] =
                UVCoordinate[0] * descBitmap.FactorU + descBitmap.U;
            UVCoordinate[1] =
                UVCoordinate[1] * descBitmap.FactorV + descBitmap.V;

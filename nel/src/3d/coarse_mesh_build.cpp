		...
		for (uint v = 0; v < heightMax; v++)
		{
			for (uint u = 0; u < widthMax; u++)
			{
				...
				while (toTest != inserted.end())
				{
					...
					if ((v < otherV + otherHeight) && (v + heightTex > otherV) && (u < otherU + otherWidth) && (u + widthTex > otherU))
					{
						// Collision
						enter = false;
				...
				if (enter)
				{
					...
					{
						// Check..
						nlassert((ite->second->Bitmaps[0].getWidth() == ite->second->Bitmaps[i].getWidth()) && (ite->second->Bitmaps[0].getHeight() == ite->second->Bitmaps[i].getHeight()));

...
CSegRemanenceShape::CSegRemanenceShape()
    : _GeomTouched(true), _MatTouched(true), _TextureShifting(true),
      _NumSlices(8), _SliceTime(0.05f), _RollUpRatio(1.f), _AnimatedMat(NULL) {
...
//===========================================================
void CSegRemanenceShape::render(IDriver *drv, CTransformShape *trans,
                                bool opaquePass) {
  if ((!opaquePass && _Mat.getBlend()) || (opaquePass && !_Mat.getBlend())) {
...
CTransformShape *CSegRemanenceShape::createInstance(CScene &scene) {
  CSegRemanence *sr = NLMISC::safe_cast<CSegRemanence *>(
      scene.createModel(NL3D::SegRemanenceShapeId));
  sr->Shape = this;
...
//===========================================================
void CSegRemanenceShape::setBBox(const NLMISC::CAABBox &bbox) { _BBox = bbox; }
...
void CSegRemanenceShape::setupMaterial() {
  ...
  _Mat.setLighting(
      false); // lighting not supported (the vb has no normals anyway..)
  _MatTouched = false;
}
...
CSegRemanenceShape::CSegRemanenceShape(const CSegRemanenceShape &other)
    : IShape(other), _AnimatedMat(NULL) {
  copyFromOther(other);
}
...
//===========================================================
CSegRemanenceShape::~CSegRemanenceShape() { delete _AnimatedMat; }

//===========================================================
void CSegRemanenceShape::copyFromOther(const CSegRemanenceShape &other) {
  if (&other == this)
    return;
  CMaterialBase *otherAnimatedMat = other._AnimatedMat != NULL
                                        ? new CMaterialBase(*other._AnimatedMat)
                                        : NULL;
  delete _AnimatedMat;
...
//===========================================================
bool CSegRemanenceShape::clip(const std::vector<CPlane> &pyramid,

...
bool CMRMBuilder::edgeNearUniqueMatFace(const CMRMEdge &edge)
{
	...
	for (i = 0; i < (sint)deletedFaces.size(); i++)
	{
		...
		if (!edgeContinue(f.getEdge(0)) && !edgeContinue(f.getEdge(1)) && !edgeContinue(f.getEdge(2)))
			return true;
	}
		...
		for (i = 0; i < (sint)wedges.size(); i++)
		{
			...
			else
			{
				...
				else
				{
					...
					if (numWedge == face.getAssociatedWedge(attId, edgeV2) && faceShareWedges(&face, attId, edgeV1, edgeV2))
					{
...
void CMRMBuilder::init(const CMRMMesh &baseMesh)
{
	...
	for (attId = 0; attId < NumAttributes; attId++)
	{
		...
		{
			TmpAttributes[attId][i].Current = TmpAttributes[attId][i].Original = baseMesh.Attributes[attId][i];

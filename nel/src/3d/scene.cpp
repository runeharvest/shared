...
// The manager is limited to a square of 3000m*3000m around the camera. Beyond,
// models are clipped individually (bad!!).
...
void CScene::release() {
  ...
  while (it != _Models.end()) {
    ...
    if (tr != Root && tr != RootCluster &&
        tr != SonsOfAncestorSkeletonModelGroup)
      deleteModel(tr);
    // temp erase from the list
    else
      _Models.erase(it);
    // NB: important to take begin(), and not it++, cause ~CSkeletonModel() may
    // delete ScriptSpawned models
    it = _Models.begin();
  }
  ...
  if (Root)
    _Models.insert(Root);
  if (RootCluster)
    _Models.insert(RootCluster);
  if (SonsOfAncestorSkeletonModelGroup)
    _Models.insert(SonsOfAncestorSkeletonModelGroup);
  ...
  // DON'T reset the _LodCharacterManager, because it can be shared across
  // scenes
  /*
...
void CScene::initDefaultRoots() {
  ...
  // Create a SonsOfAncestorSkeletonModelGroup, for models which have a skeleton
  // ancestor
  SonsOfAncestorSkeletonModelGroup =
      static_cast<CRootModel *>(createModel(RootModelId));
  // must unlink it from all traversals, because special, only used in
  // CClipTrav::traverse()
...
void CScene::initQuadGridClipManager() {
  ...
  if (!_QuadGridClipManager) {
    // create the model
    _QuadGridClipManager =
        static_cast<CQuadGridClipManager *>(createModel(QuadGridClipManagerId));
    ...
    _QuadGridClipManager->init(
        NL3D_QuadGridClipClusterSize, NL3D_QuadGridClipNumDist,
        NL3D_QuadGridClipMaxDist, NL3D_QuadGridClipManagerRadiusMax);
  }
}
...
void CScene::endPartRender(bool keepTrav) {
  ...
  if (!keepTrav) {
    ...
    // Particle system handling (remove the resources of those which are too
    // far, as their clusters may not have been parsed). Note that only a few of
    // them are tested at each call
    _ParticleSystemManager.refreshModels(ClipTrav.WorldFrustumPyramid,
                                         ClipTrav.CamPos);
  ...
  uint64 total = PSStatsRegisterPSModelObserver +
   ...
   if (((double) total / (double) NLMISC::CSystemInfo::getProcessorFrequency())
  > 0.01)
   {
        nlinfo("***** PS STATS ****");
        #define PS_STATS(var) \
        nlinfo("time for " #var " = %.2f", (float) (1000 * ((double) var /
  (double) CSystemInfo::getProcessorFrequency())));
...
// ***************************************************************************
void CScene::renderPart(UScene::TRenderPart rp, bool doHrcPass, bool doTrav,
                        bool keepTrav) {
  nlassert(_IsRendering);
  ...
  if (_RenderedPart == UScene::RenderNothing) {
    ...
    if (doTrav) {
      ...
      if (_GlobalSystemTime == 0)
        ...
        _DeltaSystemTimeBetweenRender =
            fNewGlobalSystemTime - _GlobalSystemTime;
    ...
    // setup basic camera.
    ClipTrav.setFrustum(left, right, bottom, top, znear, zfar,
                        CurrentCamera->isPerspective());

    RenderTrav.setFrustum(left, right, bottom, top, znear, zfar,
                          CurrentCamera->isPerspective());
    RenderTrav.setViewport(_Viewport);

    LoadBalancingTrav.setFrustum(left, right, bottom, top, znear, zfar,
                                 CurrentCamera->isPerspective());
    ...
    // **** For all render traversals, traverse them (except the Hrc one), in
    // ascending order.
    if (doHrcPass)
      HrcTrav.traverse();
    else
    ...
    if (doTrav) {
      ...
      if (_RequestParticlesAnimate) {
        _ParticleSystemManager.processAnimate(
            _EllapsedTime); // deals with permanently animated particle systems
        _RequestParticlesAnimate = false;
      }
    }
  ...
  if (!keepTrav) {
    // Always must clear shadow caster (if render did not work because of
    // IDriver::isLost())
...
void CScene::updateWaitingInstances(double systemTimeEllapsed) {
  ...
  _ShapeBank->processWaitingShapes(); // Process waiting shapes load shape,
                                      // texture, and lightmaps and upload all
                                      // maps to VRAM pieces by pieces
  ...
  while (wimmIt != _WaitingInstances.end()) {
    ...
    if (st == CShapeBank::AsyncLoad_Error) {
      ...
      *(wimmIt->second) =
          _ShapeBank->addRef(wimmIt->first)->createInstance(*this);
...
// ***************************************************************************
void CScene::setDriver(IDriver *drv) { RenderTrav.setDriver(drv); }
...
void CScene::setShapeBank(CShapeBank *pShapeBank) { _ShapeBank = pShapeBank; }
...
CTransformShape *CScene::createInstance(const string &shapeName) {
  ...
  if (pTShp)
    pTShp->setDistMax(pTShp->Shape->getDistMax());
...
#else  // not GNUC
  CMeshBase *pMB =
      pTShp ? dynamic_cast<CMeshBase *>((IShape *)(pTShp->Shape)) : NULL;
#endif // not GNUC
    ...
    if (_AutomaticAnimationSet) {
      if (pMB->getAutoAnim()) {
        ...
        if (animID != CAnimationSet::NotFound) {
          ...
          // Gives this object ownership of the channel mixer so we don't need
          // to keep track of it
...
void CScene::createInstanceAsync(const string &shapeName,
  ...
  *pInstance = NULL;
  ...
  _WaitingInstances.insert(
      TWaitingInstancesMMap::value_type(shapeName, pInstance));
  // If the shape is not present in the bank
  if (_ShapeBank->getPresentState(shapeName) != CShapeBank::Present) {
    // Load it from file asynchronously
    _ShapeBank->loadAsync(toLowerAscii(shapeName), getDriver(), position, NULL,
                          selectedTexture);
  }
}
...
void CScene::animate(TGlobalAnimationTime atTime) {
  ...
  // First list all current AnimatedLightmaps (for faster vector iteration per
  // ig)
  ...
  for (itAnIgSet = _AnimatedIgSet.begin(); itAnIgSet != _AnimatedIgSet.end();
       itAnIgSet++) {
...
void CScene::setPolygonBalancingMode(TPolygonBalancingMode polBalMode) {
  LoadBalancingTrav.PolygonBalancingMode =
      (CLoadBalancingGroup::TPolygonBalancingMode)(uint)polBalMode;
}

// ***************************************************************************
CScene::TPolygonBalancingMode CScene::getPolygonBalancingMode() const {
  return (CScene::TPolygonBalancingMode)(
             uint)LoadBalancingTrav.PolygonBalancingMode;
}
...
NLMISC::CRGBA CScene::getSunAmbient() const { return RenderTrav.SunAmbient; }
NLMISC::CRGBA CScene::getSunDiffuse() const { return RenderTrav.SunDiffuse; }
NLMISC::CRGBA CScene::getSunSpecular() const { return RenderTrav.SunSpecular; }
...
void CScene::setLightTransitionThreshold(float lightTransitionThreshold) {
  LightTrav.LightingManager.setLightTransitionThreshold(
...
// ***************************************************************************
void CScene::setGlobalWindPower(float gwp) { _GlobalWindPower = gwp; }
// ***************************************************************************
...
// ***************************************************************************
CScene::ItSkeletonModelList
CScene::appendSkeletonModelToList(CSkeletonModel *skel) {
...
// ***************************************************************************
void CScene::registerModel(const CClassId &idModel, const CClassId &idModelBase,
                           CTransform *(*creator)()) {
...
CTransform *CScene::createModel(const CClassId &idModel) {
  ...
  if (itModel == _RegModels.end()) {
    ...
    // link model to Root in HRC and in clip. NB: if exist!! (case for the Root
    // and RootCluster :) )
    if (Root) {
      Root->hrcLinkSon(m);
      Root->clipAddChild(m);
...
void CScene::deleteModel(CTransform *model) {
  ...
  if (_IsRendering) {
    ...
    // and I prefer doing like this than to add a NULL ptr Test in each
    // Traversal (which I then must do in CRenderTrav)
...
void CScene::updateModels() {
  ...
  CTransform *next;
  while (model) {
    // next to update. get next now, because model->update() may remove model
    // from the list.
...
sint CScene::getAnimatedLightNameToIndex(const std::string &name) const {
  std::map<std::string, uint>::const_iterator ite =
      _AnimatedLightNameToIndex.find(name);
  ...
  for (i = 0; i < count; i++) {
    ...
    while (itSel != setTrackNames.end()) {
      ...
      if (strncmp(itSel->c_str(), "LightmapController.", 19) == 0) {
        ...
        if (_AnimatedLightNameToIndex.find(lightName) ==
            _AnimatedLightNameToIndex.end()) {
          ...
          _AnimatedLight.push_back(
              CAnimatedLightmap((uint)_LightGroupColor.size()));
          _AnimatedLightPtr.push_back(&_AnimatedLight.back());
          _AnimatedLightNameToIndex.insert(
              std::map<std::string, uint>::value_type(
                  lightName, (uint32)_AnimatedLightPtr.size() - 1));
          CAnimatedLightmap &animLM = _AnimatedLight.back();
          animLM.setName(*itSel);

          cm->addChannel(animLM.getName(), &animLM,
                         animLM.getValue(CAnimatedLightmap::FactorValue),
                         animLM.getDefaultTrack(CAnimatedLightmap::FactorValue),
...
// ***************************************************************************
void CScene::setShadowMapBlurSize(uint bs) { _ShadowMapBlurSize = bs; }
...
// ***************************************************************************
CInstanceGroup *
CScene::findCameraClusterSystemFromRay(CInstanceGroup *startClusterSystem,
  ...
  // **** Do a traverse starting from each start cluser, clipping the ray
  // instead of the camera pyramid
  uint i;
  ...
  for (i = 0; i < vClusterVisited.size(); i++) {
    // select only cluster where the EndPos lies in. Important else in
    // landscape, we'll always say that we are in a Son Cluster.
  ...
  // NB: still possible that the possibleClusterSystem is empty, if not in any
  // cluster for instance :)

  // **** From each possible clusterSystem, select the one that is the lower in
  // hierarchy common case
  if (possibleClusterSystem.empty())
    resultCS = NULL;
  else if (possibleClusterSystem.size() == 1) {
    // if it is the rootCluster set NULL (should have the same behavior but do
    // like standard case)
...
void CScene::renderOcclusionTestMeshsWithCurrMaterial() {
  for (std::set<CTransform *>::iterator it = _Models.begin();
       it != _Models.end(); ++it) {
    if ((*it)->isFlare()) {
...
void CScene::renderOcclusionTestMeshs() {
  ...
  IDriver::TPolygonMode oldPolygonMode =
      RenderTrav.getDriver()->getPolygonMode();
  CMaterial m;
  m.initUnlit();

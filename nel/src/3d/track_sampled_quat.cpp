...
void CQuatPack::pack(const CQuat &quat) {
  /*
      This is the most precise/faster compression we can have. Some other tries
     have been made.

      - deducing w from x,y,z is possible with w= 1-sqrt(x^2+y^2+z^2) (with
     tradeoff of the W sign) but very not precise.
      - Transform the quaternion to an AxisAngle is possible, but slower (some
     cos/sin or LUT). Axis is encoded with sint16, and angle is encoded with
     uint16.
      - The same than above, but encode the axis as X/Y only, and deduce Z from
          them, is possible but precision problems arise.

      You can see that the operation "deduce a 3/4 member from unit length rule"
     is definetly not precise.
...
// ***************************************************************************
void CTrackSampledQuat::build(const std::vector<uint16> &timeList,
                              const std::vector<CQuat> &keyList,
                              float beginTime, float endTime) {
  nlassert(endTime > beginTime ||
           (beginTime == endTime && keyList.size() <= 1));
...
// ***************************************************************************
const IAnimatedValue &CTrackSampledQuat::eval(const TAnimationTime &date,
  ...
  float interpValue;
  TEvalType evalType =
      evalTime(date, _Keys.size(), keyId0, keyId1, interpValue);
...
// ***************************************************************************
bool CTrackSampledQuat::applyTrackQuatHeaderCompressionPass0(
    class CTrackSampleCounter &quatCounter) {
  ...
  uint headerIndex;
  for (headerIndex = 0; headerIndex < quatCounter.TrackHeaders.size();
       headerIndex++) {
    CTrackSampleHeader &tsh = quatCounter.TrackHeaders[headerIndex];
    if (tsh.LoopMode == _LoopMode && tsh.BeginTime == _BeginTime &&
        tsh.EndTime == _EndTime && tsh.TotalRange == _TotalRange &&
        tsh.OOTotalRange == _OOTotalRange && tsh.DeltaTime == _DeltaTime &&
...
// ***************************************************************************
ITrack *CTrackSampledQuat::applyTrackQuatHeaderCompressionPass1(
    uint &globalKeyOffset, class CTrackSamplePack &quatPacker) {
  // if there is more than 1 timeBlock, fails
  if (_TimeBlocks.size() > 1)
    return NULL;
  ...
  uint headerIndex;
  for (headerIndex = 0; headerIndex < quatPacker.TrackHeaders.size();
       headerIndex++) {
  ...
  return new CTrackSampledQuatSmallHeader(&quatPacker, (uint8)headerIndex,
                                          (uint8)_Keys.size(), keyIndex);
}

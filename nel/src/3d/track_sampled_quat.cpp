...
bool CTrackSampledQuat::applyTrackQuatHeaderCompressionPass0(class CTrackSampleCounter &quatCounter)
{
	...
	for (headerIndex = 0; headerIndex < quatCounter.TrackHeaders.size(); headerIndex++)
	{
		...
		if (tsh.LoopMode == _LoopMode && tsh.BeginTime == _BeginTime && tsh.EndTime == _EndTime && tsh.TotalRange == _TotalRange && tsh.OOTotalRange == _OOTotalRange && tsh.DeltaTime == _DeltaTime && tsh.OODeltaTime == _OODeltaTime)
			break;
	}

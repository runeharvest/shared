...
CTextureFont::CTextureFont()
    : _CacheVersion(1), _TextureSizeX(512), _TextureSizeY(512),
      _TextureMaxW(4096), _TextureMaxH(4096), _PaddingL(0), _PaddingT(0),
      _PaddingR(1), _PaddingB(1), _MinGlyphSize(5), _MaxGlyphSize(200),
...
void CTextureFont::dumpTextureFont(const char *filename) {
  ...
  for (uint i = 0; i < (_TextureSizeX * _TextureSizeY); ++i) {
    bits[i * 4 + 0] = bits[i * 4 + 1] = bits[i * 4 + 2] = bits[i * 4 + 3] =
        src[i];
  }
...
void CTextureFont::clearAtlas() {
  ...
  // clear glyph cache
  for (std::map<SLetterKey, SLetterInfo>::iterator it = _Letters.begin();
       it != _Letters.end(); ++it) {
...
// ---------------------------------------------------------------------------
void CTextureFont::repackAtlas() { repackAtlas(_TextureSizeX, _TextureSizeY); }
...
void CTextureFont::repackAtlas(uint32 newW, uint32 newH) {
  ...
  for (std::map<SLetterKey, SGlyphInfo>::iterator it = _GlyphCache.begin();
       it != _GlyphCache.end(); ++it) {
    if (it->second.CacheVersion != _CacheVersion) {
      // TODO: must remove glyph from all letters before removing glyph from
      // cache continue;
    }
...
bool CTextureFont::resizeAtlas() {
  ...
  if (!getNextTextureSize(newW, newH)) {
    nlwarning("Font texture at maximum (%d,%d). Resize failed.", _TextureSizeX,
              _TextureSizeY);
    return false;
  }
...
void CTextureFont::doGenerate(bool async) {
  ...
  _Letters.size() * sizeof(SLetterInfo), _GlyphCache.size(), _GlyphCache.size()
  * sizeof(SGlyphInfo));
  //std::string fname = CFile::findNewFile("/tmp/font-texture.tga");
  std::string fname = toString("/tmp/font-texture-%p-%03d.tga", this,
  _CacheVersion); dumpTextureFont (fname.c_str());
  */
}
...
bool CTextureFont::reserveAtlas(const uint32 width, const uint32 height,
                                uint32 &x, uint32 &y) {
  if (_AtlasNodes.empty()) {
    nlwarning(
        "No available space in texture atlas (_AtlasNodes.empty() == true)");
    return false;
  }
  ...
  for (uint i = 0; i < _AtlasNodes.size(); ++i) {
    ...
    if (selY >= 0) {
      if (((selY + height) < bestHeight) ||
          ((selY + height) == bestHeight && _AtlasNodes[i].Width > 0 &&
           _AtlasNodes[i].Width < bestWidth)) {
  ...
  for (uint i = bestIndex + 1; i < _AtlasNodes.size(); i++) {
    if (_AtlasNodes[i].X < (_AtlasNodes[i - 1].X + _AtlasNodes[i - 1].Width)) {
      sint shrink =
          _AtlasNodes[i - 1].X + _AtlasNodes[i - 1].Width - _AtlasNodes[i].X;
...
void CTextureFont::copyGlyphBitmap(uint8 *bitmap, uint32 bitmapW,
                                   uint32 bitmapH, uint32 atlasX,
                                   uint32 atlasY) {
  for (uint bY = 0; bY < bitmapH; ++bY) {
    uint8 *pDst = &_Data[0][(atlasY + _PaddingT + bY) * _TextureSizeX + atlasX +
                            _PaddingL];
  ...
  if (_PaddingR > 0 || _PaddingB > 0 || _PaddingL > 0 || _PaddingT > 0) {
    for (uint i = 0; i < (bitmapH + _PaddingT + _PaddingB); ++i) {
      if (_PaddingT > 0)
        _Data[0][(atlasY + i) * _TextureSizeX + atlasX] = 0;
      if (_PaddingB > 0)
        _Data[0][(atlasY + i) * _TextureSizeX + atlasX + _PaddingL + bitmapW] =
            0;
    }

    for (uint i = 0; i < (bitmapW + _PaddingL + _PaddingR); ++i) {
      if (_PaddingL > 0)
        _Data[0][atlasY * _TextureSizeX + atlasX + i] = 0;
      if (_PaddingB > 0)
        _Data[0][(atlasY + _PaddingT + bitmapH) * _TextureSizeX + atlasX + i] =
            0;
    }
  }

  CRect r(atlasX, atlasY, bitmapW + _PaddingL + _PaddingR,
          bitmapH + _PaddingT + _PaddingB);
  touchRect(r);
}

// ---------------------------------------------------------------------------
CTextureFont::SGlyphInfo *CTextureFont::renderLetterGlyph(SLetterInfo *letter,
                                                          uint bitmapFontSize) {
  uint32 nPitch;
  sint32 left;
  sint32 top;
  ...
  uint8 *bitmap = letter->FontGenerator->getBitmap(
      letter->Char, bitmapFontSize, letter->Embolden, letter->Oblique,
      charWidth, charHeight, nPitch, left, top, advx, glyphIndex);
...
// ---------------------------------------------------------------------------
CTextureFont::SGlyphInfo *CTextureFont::findLetterGlyph(SLetterInfo *letter,
                                                        bool insert) {
  uint bitmapFontSize =
      max((sint)_MinGlyphSize, min((sint)_MaxGlyphSize, letter->Size));
...
// ---------------------------------------------------------------------------
CTextureFont::SLetterInfo *CTextureFont::findLetter(SLetterKey &k,
                                                    bool insert) {
  ...
  if (insert) {
    ...
    uint32 nPitch;
    letter->FontGenerator->getBitmap(
        letter->Char, letter->Size, letter->Embolden, letter->Oblique,
        letter->CharWidth, letter->CharHeight, nPitch, letter->Left,
        letter->Top, letter->AdvX, letter->GlyphIndex);
...
CTextureFont::SLetterInfo *CTextureFont::getLetterInfo(SLetterKey &k,
  ...
  if (letter->CharWidth == 0 || letter->CharHeight == 0)
    return letter;

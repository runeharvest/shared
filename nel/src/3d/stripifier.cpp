...
// ***************************************************************************
/*
    NVidia(tm) 's method get better performance (8ms on 50K faces meshe, instead
   of 8.9ms), but precomputing is much slower (1'40 instead of 0'??  :)  ).
*/
/*void		CStripifier::optimizeTriangles(const CIndexBuffer &in,
CIndexBuffer &out, uint cacheSize)
{
...
struct CVertexCache {
  ...
  bool isVertexInCache(uint vert) { return _VertexInCache[vert] == 3; }

  // return which vertex is at which place in the cache. 0xFFFFFFFF if the entry
  // is empty
  uint getVertexInCache(uint vertIdInCache) { return _Cache[vertIdInCache]; }
...
struct COrderFace {
  ...
  sint countCacheMiss(CVertexCache &vertexCache) {
    sint ret = 0;
    if (!vertexCache.isVertexInCache(v[0]))
      ret++;
    if (!vertexCache.isVertexInCache(v[1]))
      ret++;
    if (!vertexCache.isVertexInCache(v[2]))
      ret++;
    return ret;
  }
};
...
// ***************************************************************************
void CStripifier::optimizeTriangles(const CIndexBuffer &in, CIndexBuffer &out,
                                    uint cacheSize) {
  ...
  for (i = 0; i < numTris; i++) {
    ...
    while (nextToInsertFound) {
      ...
      if (!inFaces[nextToInsert].Inserted) {
        ...
        // look only for faces which use vertices in VertexCache, to get a face
        ...
        for (uint j = 0; j < cacheSize; j++) {
          ...
          while (corner) {
            ...
            if (!inFaces[faceId].Inserted) {
              ...
              // else the one which add the minimum of vertex possible:
              // nextToInsert
              else {
                // Add cost of faces that use vertices pushed out (better
                // results...)
                uint numVOut = c;
                uint k;
        ...
        // if nextToInsertFound, then nextToInsert has the face which add the
        // minimum of vertex possible in the cache
      }
    }
  }
}

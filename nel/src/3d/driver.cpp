...
bool IDriver::release(void) {
  ...
  while ((ittex = _TexDrvShares.begin()) != _TexDrvShares.end()) {
    // NB: at CTextureDrvShare deletion, this->_TexDrvShares is updated (entry
    // deleted);
    delete *ittex;
  }

  // Release refptr of TextureDrvInfos. Should be all null (because of precedent
  // pass).
  {
    CSynchronized<TTexDrvInfoPtrMap>::CAccessor access(&_SyncTexDrvInfos);
    TTexDrvInfoPtrMap &rTexDrvInfos = access.value();
  ...
  while ((itGPUPrg = _GPUPrgDrvInfos.begin()) != _GPUPrgDrvInfos.end()) {
    // NB: at IVertexProgramDrvInfos deletion, this->_GPUPrgDrvInfos is updated
    // (entry deleted);
    delete *itGPUPrg;
  }
...
GfxMode::GfxMode(uint16 w, uint16 h, uint8 d, bool windowed, bool offscreen,
                 uint frequency, sint8 aa, const std::string &displayDevice) {
...
IDriver::systemMessageBox(const char *message, const char *title,
  ...
  static const char *messages[typeCount] = {
      "Press any key...",      "(O)k or (C)ancel ?",
      "(Y)es or (N)o ?",       "(A)bort (R)etry (I)gnore ?",
      "(Y)es (N)o (C)ancel ?", "(R)etry (C)ancel ?"};
  ...
  for (;;) {
    ...
    case 'C':
    case 'c':
      if ((type == yesNoCancelType) || (type == okCancelType) ||
          (type == retryCancelType))
        return cancelId;
      break;
    case 'Y':
    case 'y':
...
void IDriver::profileTextureUsage(std::vector<std::string> &result) {
  ...
  for (; it != _TexDrvShares.end(); it++) {
    ...
    ITextureDrvInfos *gltext =
        (ITextureDrvInfos *)(ITextureDrvInfos *)(*it)->DrvTexture;
    ...
    if (texSet.insert(gltext).second) {
      ...
      tempInfo[infoKey].back().Line =
          toString("Type: %15s. ShareName: %s. Size: %d Ko", typeStr.c_str(),
                   shareName.c_str(), memCost / 1024);
  ...
  // Store into result, appending Tag for each Mo reached. +10* is for extra
  // lines and security
  result.clear();
  result.reserve(texSet.size() + 10 * (tempInfo.size()) +
                 totalSize / (1024 * 1024));
  ...
  for (itCat = tempInfo.begin(); itCat != tempInfo.end(); itCat++) {
    ...
    case ITexture::Auto:
      strUploadFormat = ("Format: Auto");
      break;
    case ITexture::RGBA8888:
      strUploadFormat = ("Format: RGBA8888");
      break;
    case ITexture::RGBA4444:
      strUploadFormat = ("Format: RGBA4444");
      break;
    case ITexture::RGBA5551:
      strUploadFormat = ("Format: RGBA5551");
      break;
    case ITexture::RGB888:
      strUploadFormat = ("Format: RGB888");
      break;
    case ITexture::RGB565:
      strUploadFormat = ("Format: RGB565");
      break;
    case ITexture::DXTC1:
      strUploadFormat = ("Format: DXTC1");
      break;
    case ITexture::DXTC1Alpha:
      strUploadFormat = ("Format: DXTC1Alpha");
      break;
    case ITexture::DXTC3:
      strUploadFormat = ("Format: DXTC3");
      break;
    case ITexture::DXTC5:
      strUploadFormat = ("Format: DXTC5");
      break;
    case ITexture::Luminance:
      strUploadFormat = ("Format: Luminance");
      break;
    case ITexture::Alpha:
      strUploadFormat = ("Format: Alpha");
      break;
    case ITexture::AlphaLuminance:
      strUploadFormat = ("Format: AlphaLuminance");
      break;
    case ITexture::DsDt:
      strUploadFormat = ("Format: DsDt");
      break;
    default:
      strUploadFormat = toString("Format??: %d", infoKey.UpLoadFormat);
      break;
    }
    ...
    result.push_back(
        toString("**** %s. %s ****",
                 infoKey.Category ? infoKey.Category->Name.c_str() : "",
    ...
    for (uint j = 0; j < infoVect.size(); j++) {
      ...
      if (tagTotal >= 1024 * 1024) {
        result.push_back(
            toString("---- %.1f Mo", float(curTotal) / (1024 * 1024)));
        tagTotal = 0;
      }
    }
    // append last line?

...
// ***************************************************************************
void CLodCharacterBuilder::setShape(const std::string &name,
  ...
  if (!_TmpScene) {
    ...
    // Must init Statics for scene (because use it in addAnim). NB: never mind
    // if done twice.
...
// ***************************************************************************
void CLodCharacterBuilder::addAnim(const char *animName, CAnimation *animation,
                                   float frameRate) {
  nlassert(frameRate > 0);
  nlassert(animation);

  /*	Create a Scene, a skeletonModel, an animation set, and a channel mixer
     to play the animation NB: no render is made and no driver is created. The
     scene is just here for correct creation of the skeleton Yoyo: This is a
     tricky way, but I found it the easier one...
  */
  ...
  //--------------
  // build an animation set with the only one animation. This animation will be
  // deleted with the animationSet
  ...
  CSkeletonModel *skeleton =
      (CSkeletonModel *)_SkeletonShape->createInstance(*_TmpScene);
  ...
  for (uint i = 0; i < dstAnim.NumKeys; i++, time += dt) {
    ...
    // Use the skeleton model to compute bone skin matrix, supposing an identity
    // skeleton worldMatrix
...
// ***************************************************************************
void CLodCharacterBuilder::applySkin(CSkeletonModel *skeleton,
                                     CVector *dstVertices) {

...
// ***************************************************************************
void CIGSurfaceLightBuild::buildSunDebugMesh(
    CMesh::CMeshBuild &meshBuild, CMeshBase::CMeshBaseBuild &meshBaseBuild,
  ...
  meshBaseBuild.Materials[0].setBlendFunc(CMaterial::srcalpha,
                                          CMaterial::invsrcalpha);

  meshBuild.VertexFlags =
    ...
    for (uint iSurf = 0; iSurf < it->second.Grids.size(); iSurf++) {
      ...
      for (y = 0; y < hVert; y++) {
        for (x = 0; x < wVert; x++) {
          uint vId = y * wVert + x;
          // Copy Pos.
          meshBuild.Vertices[vId0 + vId] =
              surface.Cells[vId].CenterPos + deltaPos;
          // Copy Color.
...
// ***************************************************************************
void CIGSurfaceLightBuild::buildPLDebugMesh(
    CMesh::CMeshBuild &meshBuild, CMeshBase::CMeshBaseBuild &meshBaseBuild,
    const CVector &deltaPos, const CInstanceGroup &igOut) {
  contReset(meshBuild);
  contReset(meshBaseBuild);
  meshBaseBuild.Materials.resize(1);
  meshBaseBuild.Materials[0].initUnlit();
  ...
  for (it = RetrieverGridMap.begin(); it != RetrieverGridMap.end(); it++) {
    ...
    if (itIg == igOut.getIGSurfaceLight().getRetrieverGridMap().end()) {
      nlwarning("buildPLDebugMesh fails to find retriever '%d' in igOut",
                it->first);
      continue;
    } else if (it->second.Grids.size() != itIg->second.Grids.size()) {
      nlwarning("buildPLDebugMesh find retriever '%d' in igOut, but with bad "
                "size: excepting: %d, get: %d",
...
// ***************************************************************************
void CIGSurfaceLightBuild::addDebugMeshFaces(CMesh::CMeshBuild &meshBuild,
                                             CSurface &surface, uint vId0,
  ...
  for (y = 0; y < hCell; y++) {
    for (x = 0; x < wCell; x++) {
      ...
      // Skip this cell??
      bool skip = false;
      if (!surface.Cells[v00].InSurface && !surface.Cells[v00].Dilated)
        skip = true;
      if (!surface.Cells[v10].InSurface && !surface.Cells[v10].Dilated)
        skip = true;
      if (!surface.Cells[v01].InSurface && !surface.Cells[v01].Dilated)
        skip = true;
      if (!surface.Cells[v11].InSurface && !surface.Cells[v11].Dilated)
        skip = true;

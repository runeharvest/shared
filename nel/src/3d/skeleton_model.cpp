...
void CSkeletonModel::registerBasic() {
  CScene::registerModel(SkeletonModelId, TransformShapeId,
...
// ***************************************************************************
void CSkeletonModel::registerToChannelMixer(CChannelMixer *chanMixer,
                                            const std::string &prefix) {
  /* add the Spawn Script value. The animation is evaluated at detail time, as
     the script evaluation. This seems dangerous (create and delete models at
     evalDetail time) but works because:
      - deletedModels() in a current CScene::render() are delayed to end of
     render() and are "temp removed" from the render trav
      - createdModels() in CSkeletonSpawnScript are delayed to the end of
     CScene::render()
      - if a skeleton is not visible, or in LOD form, then its sticked
     SpawnedModels are not visible too, whether or not they are too old
     regarding the animation time
  */
  _SpawnScriptChannelId =
      addValue(chanMixer, SpawnScriptValue, OwnerBit, prefix, true);
  ...
  for (uint i = 0; i < Bones.size(); i++) {
    // append  bonename.
    Bones[i].registerToChannelMixer(chanMixer,
                                    prefix + Bones[i].getBoneName() + ".");
  }
}
...
CSkeletonModel::CSkeletonModel() {
  ...
  // By default, no skins, hence, impossible to have transparent pass. But
  // opaque pass is always possible because of CLod rendering
  setOpacity(true);
  ...
  // LoadBalancing behavior. true because directly act on skins to draw all
  // their MRM level
  CTransform::setIsLoadbalancable(true);

  // Lighting behavior. Lightable because skins/stickedObjects may surely need
  // its LightContribution
  CTransform::setIsLightable(true);

  // Render behavior. Always renderable, because either render the skeleton as a
  // CLod, or render skins
...
void CSkeletonModel::decBoneUsage(uint i, TBoneUsageType boneUsageType) {
  ...
  // If the bone was used before (and now won't be more), must update
  // MustCompute.
  if (*usagePtr == 1)
    _BoneToComputeDirty = true;
...
// ***************************************************************************
void CSkeletonModel::flagBoneAndParents(uint32 boneId,
                                        std::vector<bool> &boneUsage) const {
...
void CSkeletonModel::updateBoneToCompute() {
  ...
  for (uint i = 0; i < _BoneUsage.size(); i++) {
    ...
    if (isDisplayedAsLodCharacter())
      ...
      _BoneUsage[i].MustCompute = (_BoneUsage[i].Usage & lod.ActiveBones[i]) |
                                  _BoneUsage[i].ForcedUsage;
    }
    // if CLodForcedUsage for the bone, it must be computed, whatever
    // _DisplayedAsLodCharacter state
    ...
    if (_BoneUsage[i].MustCompute) {
      ...
      if (_CurLod < skeShape->getNumLods() - 1 && _LodInterpMultiplier > 0) {
        ...
        // Lod interpolation on this bone ?? only if at next lod, the bone is
        // disabled. And only if it is not enabed because of a "Forced reason"
        // Must also have a father, esle can't interpolate.
        if (lodNext->ActiveBones[i] == 0 && _BoneUsage[i].ForcedUsage == 0 &&
            _BoneUsage[i].CLodForcedUsage == 0 && bc.Father)
      ...
      // This bone is not computed => take the valid boneSkinMatrix of his
      // father
      sint fatherId = Bones[i].getFatherId();
      if (fatherId < 0)
        ...
        // NB: father ValidBoneSkinMatrix already computed because of the
        // hierarchy order of Bones array.
        _BoneUsage[i].ValidBoneSkinMatrix =
            _BoneUsage[fatherId].ValidBoneSkinMatrix;
    }
  }

  // Enable SpawnScript animation only if we are not in CLod
  if (_SpawnScriptChannelId >= 0 && chanMixer)
    chanMixer->lodEnableChannel(_SpawnScriptChannelId,
...
struct CForceComputeBoneInfo {
  ...
  uint StickBoneID; // if the transform is a skeleton, gives the bone to which
                    // child of interest is sticked
};
...
bool CSkeletonModel::forceComputeBone(uint boneId) {
  ...
  for (;;) {
    currTransform =
        currTransform->_HrcParent
            ? currTransform->_HrcParent
            : currTransform
                  ->_FatherSkeletonModel; // find father transform (maybe a
                                          // skeleton or a std transform)
    if (!currTransform)
      break; // root reached ?
    ++numAncestors;
  }
  ...
  for (;;) {
    // if curr transform is a skeleton, animate all bone from stick bone to the
    // root bone
  ...
  // bones must be recomputed from father bone to sons, so must traverse bones
  // until root is reached to retrieve correct ordering
  CBone *OrderedBone[MaxNumBones];
  //
  const CMatrix *parentWorldMatrix = &CMatrix::Identity;
  for (std::vector<CForceComputeBoneInfo>::reverse_iterator it =
           ancestors.rbegin();
       it != ancestors.rend(); ++it) {
    // update world matrix (NB : the call to getmatrix will update the local
    // matrix)
    it->Transform->setWorldMatrix(*parentWorldMatrix *
...
CSkeletonModel::getActiveBoneSkinMatrix(uint boneId) const {
...
bool CSkeletonModel::bindSkin(CTransform *mi) {
  ...
  // setApplySkin() use _FatherSkeletonModel to computeBonesId() and to update
  // current skeleton bone usage.
  mi->setApplySkin(true);

  // Unlink the Skin from Hrc and clip, because SkeletonModel now does the job
  // for him. First ensure that the transform is not frozen (unlink from some
  // quadGrids etc...)
  mi->unfreezeHRC();
...
// ***************************************************************************
void CSkeletonModel::stickObjectEx(CTransform *mi, uint boneId,
                                   bool forceCLod) {
  nlassert(mi);

  // if "mi" is a skeleton, forceCLod must be true, for correct animation
  // purpose
...
void CSkeletonModel::detachSkeletonSon(CTransform *tr) {
  ...
  if (!wasSkinned) {
    // Then decrement Bone Usage RefCount. Decrement from CLodForcedUsage if was
    // sticked with forceCLod==true
    decForcedBoneUsageAndParents(tr->_FatherBoneId, tr->_ForceCLodSticked);
  } else {
    // it is skinned, advert the skinning is no more OK.
    // setApplySkin() use _FatherSkeletonModel to update current skeleton bone
    // usage.
...
void CSkeletonModel::traverseAnimDetail() {
  ...
  /* NB: If "this" skeleton has an AncestorSkeletonModel displayed but "this"
     skeleton is clipped, it will be still animDetailed. So its possible sticked
     sons will be displayed with correct WorldMatrix (if not themselves
     clipped).
  */

  /*	If the Root Skeleton Model (ie me or my AncestorSM) is asked to render a
     ShadowMap, BUT I am in CLod Form (and visible in HRC else won't be rendered
     in shadowMap...), then temporarly Avoid CLod!! To really compute the bones
     for this frame only.
  */
  ...
  */
  ...
  // If needed, let's know which bone has to be computed, and enable / disable
  // (lod) channels in channelMixer.
  updateBoneToCompute();
  ...
  // test if bones must be updated. either if animation change or if BoneUsage
  // change. Retrieve the WorldMatrix of the current CTransformShape.
  const CMatrix &modelWorldMatrix = getWorldMatrix();

  // must test / update the hierarchy of Bones.
  // Since they are orderd in depth-first order, we are sure that parent are
  // computed before sons.
  uint numBoneToCompute = (uint)_BoneToCompute.size();
  CSkeletonModel::CBoneCompute *pBoneCompute =
      numBoneToCompute ? &_BoneToCompute[0] : NULL;
  ...
  for (; numBoneToCompute > 0; numBoneToCompute--, pBoneCompute++) {
    ...
    // Lod interpolation on this bone .. only if interp is enabled now, and if
    // bone wants it
    if (lodNext && pBoneCompute->MustInterpolate) {
      ...
      pBoneCompute->Bone->interpolateBoneSkinMatrix(fatherMatrix,
                                                    lodBoneInterp);
    }
  }
  ...
  // Sticked Objects:
  // they will update their WorldMatrix after, because of the AnimDetail
  // traverse scheme: traverse visible Clip models, and if skeleton, traverse
  // Hrc sons.
  ...
  for (uint i = 0; i < _AnimDetailSkins.size(); i++) {
    // traverse it. NB: updateWorldMatrixFromFather() is called but no-op
    // because isSkinned()
...
void CSkeletonModel::computeLodTexture() {
  ...
  nlassert(mngr);
  /* Get the asyncTextureManager. This is a Hack. We use the AsyncTextureManager
     to store very low version of Textures (kept in DXTC1 format for minimum
     memory overhead). HENCE Lod Texture can work only with Async Textured
     instances!!
  */
  ...
  // **** start process. If cannot (TextureId==no more texture space), just
  // quit.
  ...
  for (; it != _Skins.end(); it++) {
    ...
    if (mbi && mbi->getAsyncTextureMode() && mbi->Shape) {
      ...
      if (lodText) {
        ...
        // ---- Build all bmps of the instance with help of the
        // asyncTextureManager
        ...
        for (uint i = 0; i < numMats; i++) {
          ...
          // if the material stage 0 is not textured, or has not a valid async
          // id, build the bitmap with a color.
          ...
          // So if we have no bmp here, build with material color, else build a
          // texture
          if (!coarseBitmap) {
...
float CSkeletonModel::computeDisplayLodCharacterPriority() const {
  ...
  if (_LodCharacterDistance > 0 && _CLodInstance.ShapeId >= 0) {
    ...
    // If has a skeleton ancestor, take his world position instead, because ours
    // is invalid.
...
void CSkeletonModel::computeCLodVertexAlpha(CLodCharacterManager *mngr) {
  ...
  if (lodShape) {
    ...
    // build an Id map, from Skeleton Ids to the lodShapes ids. (because may be
    // differents)
    ...
    for (it = _Skins.begin(); it != _Skins.end(); it++) {
      ...
      if (skinUsage) {
        ...
        for (uint i = 0; i < skinUsage->size(); i++) {
          // the id in the vector point to a bone in the skeleton. Hence use the
          // boneMap to translate it in the lodShape ids.
...
void CSkeletonModel::updateSkinRenderLists() {
  ...
  if (_SkinToRenderDirty) {
    ...
    // Parse to count new size of the arrays, and to build MRM info, and bone
    // Max sphere
    uint opaqueSize = 0;
    ...
    for (it = _Skins.begin(); it != _Skins.end(); it++) {
      ...
      if (skin->isTransparent())
        transparentSize++;
      // else may fill in opaquelist. NB: for optimisation, don't add in
      // opaqueList if added to the transperent list (all materials are
      // rendered)
      ...
      if (trShape) {
        ...
        if (skinLevelDetail) {
          ...
          // MRM Max skin setup.
          if (_DefaultMRMSetup) {
            // Get the maximum distance setup (ie the one which degrades the
            // less)
            _LevelDetail.DistanceCoarsest =
                max(_LevelDetail.DistanceCoarsest,
                    skinLevelDetail->DistanceCoarsest);
            _LevelDetail.DistanceMiddle = max(_LevelDetail.DistanceMiddle,
                                              skinLevelDetail->DistanceMiddle);
            _LevelDetail.DistanceFinest = max(_LevelDetail.DistanceFinest,
                                              skinLevelDetail->DistanceFinest);
          }
        }
      }
      ...
      const std::vector<NLMISC::CBSphere> *boneSphere =
          skin->getSkinBoneSphere();
        ...
        for (i = 0; i < boneUsage->size(); i++) {
          ...
          if (boneId > -1 && sphere.Radius >= 0) {
            if (sphereEmpty[boneId]) {
              ...
              Bones[boneId]._MaxSphere.setUnion(Bones[boneId]._MaxSphere,
                                                sphere);
            }
          }
        }
      ...
      _SupportFastIntersect =
          _SupportFastIntersect && skin->supportIntersectSkin();
    }
    ...
    // set the Transparency to the skeleton only if has at least one transparent
    // skin
...
void CSkeletonModel::renderCLod() {
  ...
  /* During night, and in the buildings, it may be better to use one of the
     other Points lights Test only with the first pointLight, for faster
     compute, even if It may fail in some cases.
  */
  CPointLight *mainPL = lightContrib->PointLight[0];
  if (mainPL) {
    CRGBA plDiffuse;
    // get the diffuse of the pointLight, attenuated from distance and
    // importance.
    plDiffuse.modulateFromuiRGBOnly(mainPL->getDiffuse(),
                                    lightContrib->AttFactor[0]);
    plDiffuse.A = 255;
  ...
  if (!mngr->addRenderCharacterKey(_CLodInstance, getWorldMatrix(), mainAmbient,
                                   mainDiffuse, mainLightDir)) {
    ...
    mngr->addRenderCharacterKey(_CLodInstance, getWorldMatrix(), mainAmbient,
                                mainDiffuse, mainLightDir);
  }
}
...
void CSkeletonModel::renderSkins() {
  ...
  float alphaMRM = _LevelDetail.getLevelDetailFromPolyCount(
  ...
  if (rdrTrav.isCurrentPassOpaque()) {
    ...
    // but if sticked (directly or not) to a skeleton, take its.
      ...
      setupCurrentLightContribution(&_AncestorSkeletonModel->_LightContribution,
                                    false);
    ...
    // NB: must have some transparent skins, since thee skeletonModel is
    // traversed in the transparent pass.
...
// ***************************************************************************
void CSkeletonModel::renderSkinList(
    NLMISC::CObjectVector<CTransform *, false> &skinList, float alphaMRM) {
  CRenderTrav &rdrTrav = getOwnerScene()->getRenderTrav();
  ...
  if (!rdrTrav.getMeshSkinManager()) {
    ...
    // render any skins which do not support SkinGrouping, and fill array of
    // skins to group
    for (uint i = 0; i < skinList.size(); i++) {
      // If don't support, or if too big to fit in the manager, just
      // renderSkin()
    ...
    // For each skin, have an index which gives the decal of the vertices in the
    // buffer
    ...
    while (skinId < skinsToGroup.size()) {
      ...
      while (skinId < skinsToGroup.size()) {
        ...
        sint numVerticesAdded = skinsToGroup[skinId]->renderSkinGroupGeom(
            alphaMRM, remainingVertices,
            vbDest + vertexSize * currentBaseVertex);
        // -1 means that this skin can't render because no space left for her.
        // Then stop for this block
      ...
      /* Render any primitives that are not specular. Group specular ones into
         specularRdrPasses. NB: this speed a lot (specular setup is heavy)!
      */
      ...
      if (!specularRdrPasses.empty()) {
        ...
        for (uint i = 0; i < specularRdrPasses.size(); i++) {
          ...
          skinsToGroup[specRdrPass.SkinIndex]->renderSkinGroupSpecularRdrPass(
              specRdrPass.RdrPassIndex);
        }
...
// ***************************************************************************
void CSkeletonModel::changeMRMDistanceSetup(float distanceFinest,
...
// ***************************************************************************
void CSkeletonModel::getWorldMaxBoneSpheres(
    std::vector<NLMISC::CBSphere> &dest) const {
  dest.clear();
  // Not visible => empty bbox
  if (!isClipVisible())
    return;
...
bool CSkeletonModel::computeRenderedBBoxWithBoneSphere(NLMISC::CAABBox &bbox,
  ...
  for (uint i = 0; i < _BoneToCompute.size(); i++) {
    ...
    const CMatrix &boneMat = computeInWorld ? bone->getWorldMatrix()
                                            : bone->getLocalSkeletonMatrix();
    CBSphere sphere;
...
bool CSkeletonModel::computeCurrentBBox(NLMISC::CAABBox &bbox,
                                        ...
                                        bool computeInWorld) {
  // animate all bones channels (detail only channels). don't bother cur lod
  // state.
  ...
  for (i = 0; i < Bones.size(); i++) {
    ...
    uint16 mustCompute = forceCompute
                             ? 1
                             : _BoneUsage[i].Usage | _BoneUsage[i].ForcedUsage |
                                   _BoneUsage[i].CLodForcedUsage;
...
// ***************************************************************************
void CSkeletonModel::getLightHotSpotInWorld(CVector &modelPos,
  ...
  if (isBoneComputed(0)) {
    ...
    /* Else return the skeleton pos. NB: this seems useless since bone 0 not
       computed means no Skins. But lighting computation is still done and may
       use a VisualCollisionEntity. This system cache some infos according to
       position. This is why we must return a position near the skeleton (else
       cache crash each frame => slowdown...)
    */
...
// ***************************************************************************
bool CSkeletonModel::fastIntersect(const NLMISC::CVector &p0,
                                   const NLMISC::CVector &dir, float &dist2D,
  ...
  // displayed as a CLod?
  if (isDisplayedAsLodCharacter()) {
    // must do the test with the CLod, because Bones are not animated at all
    // (hence skinning would be false)
    CLodCharacterManager *mngr = getOwnerScene()->getLodCharacterManager();
    if (!mngr)
      return false;

    // test the instance
    if (!mngr->fastIntersect(_CLodInstance, toRaySpace, dist2D, distZ,
                             computeDist2D))
      return false;
  } else {
    // For all skins
    ...
    for (it = _Skins.begin(); it != _Skins.end(); it++) {
      ...
      if (skin->intersectSkin(toRaySpace, skinDist2D, skinDistZ,
                              computeDist2D)) {
...
// ***************************************************************************
void CSkeletonModel::remapSkinBones(const std::vector<string> &bonesName,
                                    std::vector<sint32> &bonesId,
...
void CSkeletonModel::generateShadowMap(const CVector &lightDir) {
  ...
  // Compute the BBox in World, with bounding Box of Bones, and with BoundingBox
  // of sticked Objects
  CAABBox bbWorld;
  ...
  _ShadowMap->buildCasterCameraMatrix(lightDir, localPosMatrix, bbWorld,
                                      cameraMatrix);
  ...
  CMaterial &castMat =
      renderTrav.getShadowMapManager().getCasterShadowMaterial();
  ...
  ldir.z = 0;
  ldir.normalize();
  // NB: This way seems to works quite well, even if the worldBBox is changing
  // every frame.
  float lenXY =
      (CVector(bbWorld.getHalfSize().x, bbWorld.getHalfSize().y, 0)).norm();
  backPoint -= ldir * lenXY;
  // localPos.
  backPoint -= getWorldMatrix().getPos();

  // Compute LocalProjectionMatrix and other infos from cameraMatrix and
  // backPoint?
  _ShadowMap->buildProjectionInfos(cameraMatrix, backPoint,
                                   getShadowMapMaxDepth());
}

// ***************************************************************************
CShadowMap *CSkeletonModel::getShadowMap() { return _ShadowMap; }
...
void CSkeletonModel::updateShadowMap(IDriver * /* driver */) {
  ...
  if (_ShadowMap->getTextureSize() !=
      getOwnerScene()->getShadowMapTextureSize()) {
...
void CSkeletonModel::renderShadowSkins(CMaterial &castMat) {
  ...
  if (!rdrTrav.getShadowMeshSkinManager()) {
    // can occurs?????
    // ABORT!! ...  avoid Mesh Shadowing (free shadowMap)? Replace with a dummy
    // Shadow? For now, no-op...
  } else {
    uint i;
    ...
    while (skinId < skinsToGroup.size()) {
      ...
      while (skinId < skinsToGroup.size()) {
        // if success to fill the AGP
        sint numVerticesAdded = skinsToGroup[skinId]->renderShadowSkinGeom(
            remainingVertices, vbDest + vertexSize * currentBaseVertex);
        // -1 means that this skin can't render because no space left for her.
        // Then stop for this block
        if (numVerticesAdded == -1)
          break;
      ...
      for (uint i = startSkinId; i < skinId; i++) {
        // render the skin in the current buffer
        skinsToGroup[i]->renderShadowSkinPrimitives(castMat, driver,
                                                    baseVertices[i]);
      }
...
// ***************************************************************************
void CSkeletonModel::renderIntoSkeletonShadowMap(CSkeletonModel *rootSkeleton,
                                                 CMaterial &castMat) {
  // If even not visible, no-op
  if (!isHrcVisible() || !Shape)
    return;
  ...
  // The model Matrix is special here. It must be the Skeleton World Matrix,
  // minus The Root Skeleton pos.
  CMatrix localPosMatrix;
  localPosMatrix.setRot(getWorldMatrix());
  // NB: if this==rootSkeleton, then the final pos will be CVector::Null
  localPosMatrix.setPos(getWorldMatrix().getPos() -
                        rootSkeleton->getWorldMatrix().getPos());

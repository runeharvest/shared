...
void CQuadEffect::makeRasters(const TPoint2DVect &poly, float quadWidth,
                              float quadHeight, TRasters &dest, float &startY) {
  ...
  for (k = 0; k < size; ++k) {
    ...
    lel.push_front(CEdge(poly[k], poly[k == (size - 1) ? 0 : k + 1]));
    if (poly[k].y < highest) {
      highest = poly[k].y;
    }
  }
  ...
  do {
    ...
    while (size && lel.begin()->P1.y < (currY + quadHeight)) {
    ...
    if (aelSize) {
      ...
      for (elIt = ael.begin(); elIt != ael.end();) {
        if (elIt->P2.y <= currY) {
           ...
           * so we need to intersect it with the y = currY and y =  currY +
           * quadHeight lines
           */
          ...
          if (elIt->P1.y >= currY) {
            ...
            if (diff > epsilon) {
              inter = elIt->P1.x +
                      (elIt->P2.x - elIt->P1.x) * (currY - elIt->P1.y) / diff;
            } else {
              inter = elIt->P2.x;
            }
          ...
          if (elIt->P2.y <= currY + quadHeight) {
            ...
            diff = elIt->P2.y - elIt->P1.y;
            if (diff > epsilon) {
              inter = elIt->P1.x + (elIt->P2.x - elIt->P1.x) *
                                       (currY + quadHeight - elIt->P1.y) / diff;
            } else {
              inter = elIt->P2.x;
            }
...
void CQuadEffect::processPoly(const TPoint2DVect &poly, float quadWidth,
                              float quadHeight, TPoint2DVect &dest) {

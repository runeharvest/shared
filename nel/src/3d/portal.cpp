...
NLMISC::TStringId CPortal::getOcclusionModelId() { return _OcclusionModelId; }
...
bool CPortal::clipPyramid(CVector &observer, std::vector<CPlane> &pyramid) {
  ...
  if (p.Vertices.size() > 2) {
    ...
    CVector n =
        (p.Vertices[1] - p.Vertices[0]) ^ (p.Vertices[2] - p.Vertices[0]);
    ...
    for (i = 0; i < (p.Vertices.size()); ++i) {
      pyramid[i + 2].make(observer, p.Vertices[i],
...
// ***************************************************************************
void CPortal::resetClusterLinks() { _Clusters[0] = _Clusters[1] = NULL; }
...
// ***************************************************************************
bool CPortal::clipRay(const NLMISC::CVector &startWorld,
                      const NLMISC::CVector &endWorld) {
  if (_Poly.size() < 3)
    return false;

...
//=================================================================================================
bool CVPParser::parseOperand(CVPOperand &operand, bool outputOperand,
  ...
  if (outputOperand) {
    ...
    switch (*_CurrChar) {
    case 'o':
      result = parseOutputRegister(operand, errorOutput);
      break;
    case 'R':
      result = parseVariableRegister(operand, errorOutput);
      break;
    case 'A':
      result = parseAddressRegister(operand, errorOutput);
      break;
    case '-':
      errorOutput = "Negation not allowed on output register.";
      return false;
    default:
      errorOutput = "Output, Address, or Temporary register expected as an "
                    "output operand.";
      return false;
    }
    if (!result)
    ...
    switch (*_CurrChar) {
    case 'v':
      result = parseInputRegister(operand, errorOutput);
      break;
    case 'R':
      result = parseVariableRegister(operand, errorOutput);
      break;
    case 'c':
      result = parseConstantRegister(operand, errorOutput);
      break;
    case 'a':
      result = parseAddressRegister(operand, errorOutput);
      break;
    case '-': {
      ...
      case 'c':
        ...
        break;
      default:
        errorOutput = "Negation must be followed by an input register, a "
                      "variable register, or a constant.";
        return false;
        break;
      }
    } break;
    ...
    if (operand.Type != CVPOperand::AddressRegister) {
      if (!parseSwizzle(operand.Swizzle, errorOutput))
        return false;
      if (operand.Type == CVPOperand::Variable) {
        for (uint k = 0; k < 4; ++k) {
          if (!(_RegisterMask[operand.Value.VariableValue] &
                (1 << operand.Swizzle.Comp[k]))) {
            errorOutput =
...
bool CVPParser::parseInputRegister(CVPOperand &operand,
  ...
  if (isdigit(*_CurrChar)) {
    ...
    case OPOS:
      operand.Value.InputRegisterValue = CVPOperand::IPosition;
      break;
    case WGHT:
      operand.Value.InputRegisterValue = CVPOperand::IWeight;
      break;
    case NRML:
      operand.Value.InputRegisterValue = CVPOperand::INormal;
      break;
    case COL0:
      operand.Value.InputRegisterValue = CVPOperand::IPrimaryColor;
      break;
    case COL1:
      operand.Value.InputRegisterValue = CVPOperand::ISecondaryColor;
      break;
    case FOGC:
      operand.Value.InputRegisterValue = CVPOperand::IFogCoord;
      break;
    // texture argument
    case TEX0:
    case TEX1:
    ...
    case TEX6:
    case TEX7:
      operand.Value.InputRegisterValue = (CVPOperand::EInputRegister)(
          ((CVPOperand::ITex0 + strValue) & 0xff) - '0');
      break;
    default:
      errorOutput = "Can't parse index for input register.";
      return false;
...
bool CVPParser::parseOutputRegister(CVPOperand &operand,
  ...
  switch ((TArguments)strValue) {
  case HPOS:
    operand.Value.OutputRegisterValue = CVPOperand::OHPosition;
    break;
  case COL0:
    operand.Value.OutputRegisterValue = CVPOperand::OPrimaryColor;
    break;
  case COL1:
    operand.Value.OutputRegisterValue = CVPOperand::OSecondaryColor;
    break;
  case BFC0:
    operand.Value.OutputRegisterValue = CVPOperand::OBackFacePrimaryColor;
    break;
  case BFC1:
    operand.Value.OutputRegisterValue = CVPOperand::OBackFaceSecondaryColor;
    break;
  case FOGC:
    operand.Value.OutputRegisterValue = CVPOperand::OFogCoord;
    break;
  case PSIZ:
    operand.Value.OutputRegisterValue = CVPOperand::OPointSize;
    break;
  case TEX0:
    operand.Value.OutputRegisterValue = CVPOperand::OTex0;
    break;
  case TEX1:
    operand.Value.OutputRegisterValue = CVPOperand::OTex1;
    break;
  case TEX2:
    operand.Value.OutputRegisterValue = CVPOperand::OTex2;
    break;
  case TEX3:
    operand.Value.OutputRegisterValue = CVPOperand::OTex3;
    break;
  case TEX4:
    operand.Value.OutputRegisterValue = CVPOperand::OTex4;
    break;
  case TEX5:
    operand.Value.OutputRegisterValue = CVPOperand::OTex5;
    break;
  case TEX6:
    operand.Value.OutputRegisterValue = CVPOperand::OTex6;
    break;
  case TEX7:
    operand.Value.OutputRegisterValue = CVPOperand::OTex7;
    break;
  default:
...
bool CVPParser::parseConstantRegister(CVPOperand &operand,
  ...
  if (isdigit(*_CurrChar)) {
    ...
    index = 0;
    if (_CurrChar[1] == '0' && _CurrChar[2] == '.' && _CurrChar[3] == 'x') {
      _CurrChar += 4;
      skipSpacesAndComments();
      if (*_CurrChar == '+') {
        ++_CurrChar;
...
bool CVPParser::parseOp2(CVPInstruction &instr, std::string &errorOutput) {
  skipSpacesAndComments();
  // parse output
  if (!parseOperand(instr.Dest, true, errorOutput))
    return false;
  // Can't write in input or consant register
  if (instr.Dest.Type == CVPOperand::Constant ||
      instr.Dest.Type == CVPOperand::InputRegister) {
  ...
  skipSpacesAndComments();
  // parse src1
  if (!parseOperand(instr.Src1, false, errorOutput))
    return false;
...
bool CVPParser::parseOp3(CVPInstruction &instr, std::string &errorOutput) {
  ...
  skipSpacesAndComments();
  // parse src2
  if (!parseOperand(instr.Src2, false, errorOutput))
    return false;
  ...
  if (instr.Src1.Type == CVPOperand::Constant &&
    ...
    if (!(instr.Src1.Indexed == instr.Src2.Indexed &&
  ...
  if (instr.Src1.Type == CVPOperand::InputRegister &&
    ...
    // the index must be the same
    if (instr.Src1.Value.InputRegisterValue !=
        instr.Src2.Value.InputRegisterValue) {
      errorOutput =
...
bool CVPParser::parseOp4(CVPInstruction &instr, std::string &errorOutput) {
  ...
  skipSpacesAndComments();
  // parse src4
  if (!parseOperand(instr.Src3, false, errorOutput))
    return false;
  ...
  if (instr.Src3.Type == CVPOperand::Constant) {
    if (instr.Src1.Type == CVPOperand::Constant) {
      if (!(instr.Src1.Indexed == instr.Src3.Indexed &&
            instr.Src1.Value.ConstantValue == instr.Src3.Value.ConstantValue)) {
        errorOutput = "Can't read 2 different constant registers in a single "
                      "instruction.";
        return false;
      }
    }
    if (instr.Src2.Type == CVPOperand::Constant) {
      if (!(instr.Src2.Indexed == instr.Src3.Indexed &&
  ...
  if (instr.Src3.Type == CVPOperand::InputRegister) {
    if (instr.Src1.Type == CVPOperand::InputRegister) {
      if (instr.Src1.Value.InputRegisterValue !=
          instr.Src3.Value.InputRegisterValue) {
        errorOutput =
            "Can't read 2 different input registers in a single instruction.";
        return false;
      }
    ...
    if (instr.Src2.Type == CVPOperand::InputRegister) {
      if (instr.Src2.Value.InputRegisterValue !=
          instr.Src3.Value.InputRegisterValue) {
        errorOutput =
            "Can't read 2 different input registers in a single instruction.";
        return false;
  ...
  for (uint k = 0; k < prg.size(); ++k) {
    ...
    for (uint l = 0; l < numSrc; ++l) {
      ...
      if (src.Type == CVPOperand::InputRegister &&
          src.Value.InputRegisterValue == input)
        return true;
    }
  }
  return false;
...
//=================================================================================================
bool CVPParser::parse(const char *src, CVPParser::TProgram &result,
                      std::string &errorOutput) {
  if (!src)
    return false;
  //
  ...
  _LineIndex = 1;
  //
  // skipSpacesAndComments(); // in fact space are not allowed at the start of
  // the vertex program

  // parse version
  if (_CurrChar[0] != '!' || _CurrChar[1] != '!' || _CurrChar[2] != 'V' ||
      _CurrChar[3] != 'P' || _CurrChar[4] != '1' || _CurrChar[5] != '.' ||
  ...
  for (;;) {
    ...
    if (!parseInstruction(instr, errorMess, endEncoutered)) {
      errorOutput =
          std::string("CVPParser::parse : Error encountered at line ") +
          NLMISC::toString(_LineIndex) + std::string(" : ") + errorMess +
...
//=================================================================================================
static const char *instrToName[] = {"MOV  ", "ARL  ", "MUL  ", "ADD  ", "MAD  ",
                                    "RSQ  ", "DP3  ", "DP4  ", "DST  ", "LIT  ",
                                    "MIN  ", "MAX  ", "SLT  ", "SGE  ", "EXPP ",
                                    "LOG  ", "RCP  "};

//=================================================================================================
static const char *outputRegisterToName[] = {
    "HPOS", "COL0", "COL1", "BFC0", "BFC1", "FOGC", "PSIZ", "TEX0",
...
//=================================================================================================
static void dumpOperand(const CVPOperand &op, bool destOperand,
                        std::string &out) {
  out = op.Negate ? " -" : " ";
  switch (op.Type) {
  ...
  case CVPOperand::OutputRegister:
    ...
    out += "o[" +
           std::string(outputRegisterToName[op.Value.OutputRegisterValue]) +
           "]";
    break;

...
// ***************************************************************************
void CZoneCornerSmoother::buildPatchBindInfo(CPatch &pa,
  ...
  /*
      Some terminology here: an edge is supposed going from Corner=edge, to
     corner=(edge+1)&3. eg: edge 0 goes from corner0 to corner1.
  */
  ...
  for (edge = 0; edge < 4; edge++) {
    ...
    if (bindInfo.Zone) {
      ...
      // if not sameEdgeOnOrder (NB: all special cases of bind 1/X X/1 managed
      // :) ), not smoothed!
      if (!patchUvLocator.sameEdgeOrder())
        smoothEdge[edge] = false;

      // Manage bind 1/4 for the 2 patchs on the center of the bind.
      if (bindInfo.MultipleBindNum == 4 &&
          (bindInfo.MultipleBindId == 1 || bindInfo.MultipleBindId == 2)) {
        // easy, this edge starts and ends on a bind...
        cornerOnBind[edge] = true;
        cornerOnBind[(edge + 1) & 3] = true;
      }
      // else for case bind 1/2, and for case of patch 0 and patch 3 of the bind
      // 1/4.
        ...
        /*
            ...
            ----------|-----------
        */
        // If we are the patch0 on the neighbor, then we start on a bind, else
        // we ends.
...
// ***************************************************************************
void CZoneCornerSmoother::updateVertex(uint idVert, uint corner,
                                       bool smoothEdge[4],
                                       bool cornerOnBind[4]) {
...
// ***************************************************************************
void CZoneCornerSmoother::computeAllCornerSmoothFlags(
    CZone *zone, std::vector<CZone *> neighborZones) {
  nlassert(zone);
  ...
  for (i = 0; i < npatchs; i++) {
    ...
    for (corner = 0; corner < 4; corner++) {
      ...
      // for Bind with neighbor zones, must insert it in the map CTessVertex* ->
      // VertexId.
      IdVertexMap[pa.getCornerVertex(corner)] = idVert;
    }
  }

  // for all patchs of all neigbhors zone, update for vertices that are
  // connected to the centerZone.
  //==================
  for (uint nbZone = 0; nbZone < neighborZones.size(); nbZone++) {
    ...
    for (i = 0; i < neighborZone->getNumPatchs(); i++) {
      ...
      for (corner = 0; corner < 4; corner++) {
        ...
        // If this patch is binded on a vertex of the centerZone, must update
        // this vertex.
  ...
  for (i = 0; i < npatchs; i++) {
    ...
    for (corner = 0; corner < 4; corner++) {
      ...
      bool finalSmooth;
      finalSmooth =
          vert.Smoothed && (vert.VertexOnBind || vert.NPatchShared == 4);

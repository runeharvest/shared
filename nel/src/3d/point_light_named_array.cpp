...
// ***************************************************************************
void CPointLightNamedArray::build(
    const std::vector<CPointLightNamed> &pointLights,
...
void CPointLightNamedArray::setPointLightFactor(const CScene &scene) {
  ...
  for (i = 0; i < count; i++) {
    ...
    // Get the factor
    CRGBA factorAnimated = scene.getAnimatedLightFactor(
        lightGroup.AnimationLightIndex, lightGroup.LightGroup);
    CRGBA factorNotAnimated =
        scene.getLightmapGroupColor(lightGroup.LightGroup);
    ;
...
void CPointLightNamedArray::initAnimatedLightIndex(const CScene &scene) {
  ...
  for (i = 0; i < count; i++) {
    // Ref
    CPointLightGroup &lightGroup = _PointLightGroupMap[i];
    lightGroup.AnimationLightIndex =
        scene.getAnimatedLightNameToIndex(lightGroup.AnimationLight);
  }
}

...
const uint32 CVisualCollisionEntity::_StartPatchQuadBlockSize =
...
// ***************************************************************************
CVisualCollisionEntity::CVisualCollisionEntity(CVisualCollisionManager *owner)
...
// ***************************************************************************
CTrianglePatch *
CVisualCollisionEntity::getPatchTriangleUnderUs(const CVector &pos,
                                                CVector &res) {
  ...
  // Test GPT cache.
  //==================
  // If last call was valid, and if same pos (input or output), return cached
  // information
  if (_LastGPTValid && (pos == _LastGPTPosInput || pos == _LastGPTPosOutput)) {
    // copy from cache.
    res = _LastGPTPosOutput;
    /* don't modify _LastGPTPosInput cache, for best cache behavior in all
       cases. 1/ this is not necessary (here, if pos!=_LastGPTPosInput, THEN
       pos==_LastGPTPosOutput, no other possibilities) 2/ it causes problems
       when getPatchTriangleUnderUs() is called randomly with the unsnapped or
       snapped position: 1st Time: zin:9.0  =>  zout:9.5	cache fail (ok,
       1st time...) 2nd Time: zin:9.0  =>  zout:9.5	cache OK
       (_LastGPTPosInput.z==zin) 3rd Time: zin:9.5  =>  zout:9.5	cache OK
       (_LastGPTPosOutput.z==zin) 4th Time: zin:9.0  =>  zout:9.5	cache
       FAILS (_LastGPTPosInput.z= 9.5 => !=zin)
    */
...
// ***************************************************************************
void CVisualCollisionEntity::computeUvForPos(const CTrianglePatch &tri,
                                             const CVector &pos, CUV &uv) {
...
// ***************************************************************************
void CVisualCollisionEntity::snapToLandscapeCurrentTesselation(
    CVector &pos, const CTrianglePatch &tri) {
...
/*static bool	testLine(CVector &p1, CVector &p0, const CVector &pos0)
{
    ...
    float		a,b,c;		// 2D cartesian coefficients of line in
plane X/Y. float		norm;
    // Line p0-p1.
    a= -(p1.y-p0.y);
    b= (p1.x-p0.x);
...
// ***************************************************************************
bool CVisualCollisionEntity::triangleIntersect2DGround(CTriangle &tri,
                                                       const CVector &pos0) {
  CVector &p0 = tri.V0;
  CVector &p1 = tri.V1;
  CVector &p2 = tri.V2;
  ...
  /*if( testLine(p1, p0, pos0) && testLine(p2, p1, pos0) && testLine(p0, p2,
  pos0) )
  {
      nlinfo("Found Tri For Pos: %.07f, %.07f\n   P0: %.07f, %.07f\n   P1:
  %.07f, %.07f\n   P2: %.07f, %.07f", pos0.x, pos0.y, p0.x, p0.y, p1.x, p1.y,
  p2.x, p2.y);
  }*/

  // Test if the face enclose the pos in X/Y plane.
  // NB: compute and using a BBox to do a rapid test is not a very good idea,
  // since it will add an overhead which is NOT negligeable compared to the
  // following test.
...
// ***************************************************************************
bool CVisualCollisionEntity::triangleIntersect2DCeil(CTriangle &tri,
                                                     const CVector &pos0) {
  CVector &p0 = tri.V0;
  CVector &p1 = tri.V1;
  CVector &p2 = tri.V2;
...
// ***************************************************************************
bool CVisualCollisionEntity::triangleIntersect(CTriangle &tri,
                                               const CVector &pos0,
                                               const CVector &pos1,
                                               CVector &hit) {
  CVector &p0 = tri.V0;
  ...
  // OK!!
  // For cache completness, ensure that X and Y don't move, take same XY than
  // pos0
  hit.x = pos0.x;
  hit.y = pos0.y;
  hit.z = h;
...
void CVisualCollisionEntity::testComputeLandscape(const CVector &pos) {
  ...
  if (_CurrentBBoxValidity.getHalfSize() == CVector::Null ||
      !_CurrentBBoxValidity.include(pos)) {
...
void CVisualCollisionEntity::doComputeLandscape(const CVector &pos) {
  ...
  _CurrentBBoxValidity.setHalfSize(
      CVector(BBoxRadius, BBoxRadius, BBoxRadiusZ / 2));
  ...
  //==================
  // This parts try to keeps old patch blocks so they are not recomputed if they
  // already here.
  ...
  // Traverse all current patchBlocks, deleting old ones no longer needed, and
  // creating new ones. this algorithm suppose both array are sorted.
  uint iOld = 0;
  ...
  for (i = 0; i < (sint)_PatchQuadBlocks.size();) {
    ...
    // if no more old blocks, or if new Block is < than current, we must create
  ...
  // Compute a delta so elt position for CLandscapeCollisionGrid are positive,
  // and so fastFloor() used will work.
  CVector delta;
  ...
  // NB: 256 is a security. Because of size of patchs, a value of 32 at max
  // should be sufficient (64 for bigger patch (gfx)) we are large because
  // doesn't matter, the CLandscapeCollisionGrid tiles.
  delta.x += val;
  delta.y += val;
...
// ***************************************************************************
bool CVisualCollisionEntity::getStaticLightSetup(
    NLMISC::CRGBA sunAmbient, const CVector &pos,
  ...
  if (tri) {
    ...
    _Owner->_Landscape->appendTileLightInfluences(tri->PatchId, uv,
                                                  pointLightList);
...
// ***************************************************************************
bool CVisualCollisionEntity::getSurfaceInfo(const CVector &pos,
  ...
  if (tri) {
    ...
    // get the tileelement
    CTileElement *tileElm =
        _Owner->_Landscape->getTileElement(tri->PatchId, uv);
    if (tileElm) {
      // Valid tile ?

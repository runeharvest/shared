...
// ***************************************************************************
CLandscapeIGManager::CInstanceGroupElement::CInstanceGroupElement(
    UInstanceGroup *ig, const char *fileName) {
  Ig = ig;
  AddedToScene = false;
  if (fileName != NULL)
    FileName = fileName;
...
// ***************************************************************************
CLandscapeIGManager::CLandscapeIGManager() { _Scene = NULL; }
// ***************************************************************************
...
void CLandscapeIGManager::initIG(UScene *scene, const std::string &igDesc,
                                 UDriver *driver, uint selectedTexture,
    ...
    while (!file.eof()) {
      ...
      if (token != NULL) {
        if (_ZoneInstanceGroupMap.find(token) != _ZoneInstanceGroupMap.end())
          throw Exception(
              "CLandscapeIGManager::initIG() found 2 igs with same name in %s",
              igFile.c_str());
        else {
          ...
          if (ig) {
            ...
            CInstanceGroup &_ig =
                static_cast<CInstanceGroupUser *>(ig)->getInternalIG();
            ...
            for (i = 0; i < _ig.getNumInstance(); i++) {
              ...
              if (!shapeName.empty()) {
                if (toLowerAscii(CFile::getExtension(shapeName)) !=
                    "pacs_prim") {
                  // Insert a new shape ?
                  if (_ShapeAdded.find(shapeName) == _ShapeAdded.end()) {
                    ...
                    IShape *shape = NULL;
                    if (shapeBank->getPresentState(shapeName) ==
                    ...
                    if (shape) {
                      ...
                      *smartPtr = shape;
                      _ShapeAdded.insert(
                          TShapeMap::value_type(shapeName, smartPtr));
            ...
            nlwarning("CLandscapeIGManager::initIG() Can't load instance group "
                      "'%s' in '%s'",
                      token, igFile.c_str());
          }
        }
...
// ***************************************************************************
void CLandscapeIGManager::loadArrayZoneIG(
...
// ***************************************************************************
void CLandscapeIGManager::unloadArrayZoneIG(
    const std::vector<std::string> &names) {
  for (uint i = 0; i < names.size(); i++) {
...
void CLandscapeIGManager::reloadAllIgs() {
  ...
  while (_ZoneInstanceGroupMap.begin() != _ZoneInstanceGroupMap.end()) {
    ...
    bkupIgFileNameList.push_back(
        _ZoneInstanceGroupMap.begin()->second.FileName);
    bkupIgAddedToScene.push_back(
        _ZoneInstanceGroupMap.begin()->second.AddedToScene);
...
void CLandscapeIGManager::getAllIG(std::vector<UInstanceGroup *> &dest) const {
  ...
  // add the instances
  for (TZoneInstanceGroupMap::const_iterator it = _ZoneInstanceGroupMap.begin();
       it != _ZoneInstanceGroupMap.end(); ++it) {
    dest.push_back(it->second.Ig);
  }
}

// ***************************************************************************
void CLandscapeIGManager::getAllIGWithNames(
    std::vector<std::pair<UInstanceGroup *, std::string>> &dest) const {
  dest.clear();
  dest.reserve(_ZoneInstanceGroupMap.size());
  // add the instances
  for (TZoneInstanceGroupMap::const_iterator it = _ZoneInstanceGroupMap.begin();

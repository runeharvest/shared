...
// ***************************************************************************
void CMeshMorpher::init(CVertexBuffer *vbOri, CVertexBuffer *vbDst,
                        bool hasTgSpace) {
  _VBOri = vbOri;
  _VBDst = vbDst;
...
// ***************************************************************************
void CMeshMorpher::initSkinned(
    CVertexBuffer *vbOri, CVertexBuffer *vbDst, bool hasTgSpace,
    std::vector<CVector> *vVertices, std::vector<CVector> *vNormals,
...
void CMeshMorpher::update(std::vector<CAnimatedMorph> *pBSFactor) {
  ...
  if (_VBOri->getNumVertices() !=
      _VBDst->getNumVertices()) { // Because the original vertex buffer is not
      ...
      for (j = 0; j < rBS.VertRefs.size(); ++j) {
        ...
        if (_UseTgSpace)
          if (!rBS.deltaTgSpace.empty()) {
            CVector *pV =
                (CVector *)dstvba.getTexCoordPointer(vp, tgSpaceStage);
...
void CMeshMorpher::updateRawSkin(
    CVertexBuffer *vbOri,
  ...
  nlassert(vbOri->getVertexFormat() ==
  ...
  for (i = 0; i < BlendShapes.size(); ++i) {
    ...
    if (rFactor != 0.0f) {
      ...
      // don't know why, but cases happen where deltaNorm not empty while
      // deltaPos is

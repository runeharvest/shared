...
// ***************************************************************************
CShadowMap::~CShadowMap() { resetTexture(); }
...
// ***************************************************************************
void CShadowMap::buildCasterCameraMatrix(const CVector &lightDir,
                                         const CMatrix &localPosMatrix,
                                         const CAABBox &bbShape,
                                         CMatrix &cameraMatrix) {
  // compute the orthogonal LightSpace camera matrix. Remind that J look forward
  // and K is up here.
  cameraMatrix.setArbitraryRotJ(lightDir);

  CAABBox bbLocal;
  bbLocal = CAABBox::transformAABBox(cameraMatrix.inverted() * localPosMatrix,
                                     bbShape);
...
// ***************************************************************************
void CShadowMap::buildProjectionInfos(const CMatrix &cameraMatrix,
...
void CShadowMap::processFades() {
  ...
  /* if the fadeAround is 1, then release the texture
      Don't take _FinalFade because this last may change too much cause of
     TemporalInScreenFade. While FadeAround is somewhat stable (as entities and
     the camera don't move too much), TemporalInScreenFade is dependent of
     camera rotation.

      => _FinalFade allow to not draw too much shadows (CPU gain),
      while FadeAround allow in addition the capacity to not use too much
     texture memory
  */
  if (getFadeAround() == 1)
    resetTexture();
...
// ***************************************************************************
void CShadowMapProjector::applyToMaterial(const CMatrix &receiverWorldMatrix,
                                          CMaterial &material) {
  CMatrix osTextMat;

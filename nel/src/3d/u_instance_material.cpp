...
std::string UInstanceMaterial::getTextureFileName(uint stage) const {
  ...
  if (_MBI->getAsyncTextureMode()) {
    ...
    return NLMISC::safe_cast<CTextureFile *>(_Object->getTexture(stage))
        ->getFileName();
  }
}

// ***************************************************************************
void UInstanceMaterial::setTextureFileName(const std::string &fileName,
                                           uint stage) {
  if (stage >= IDRV_MAT_MAXTEXTURES) {
  ...
  if (_MBI->getAsyncTextureMode()) {
    if (!_AsyncTextureBlock->isTextureFile(stage)) {
      nlwarning("UInstanceMaterialUser::setTextureFileName : the texture is "
                "not a texture file");
      return;
    }
    ...
    CTextureFile *otherTex =
        dynamic_cast<CTextureFile *>(_Object->getTexture(stage));
    if (!otherTex) {
      nlwarning("UInstanceMaterialUser::setTextureFileName : the texture is "
                "not a texture file");
      return;
...
// ***************************************************************************
void UInstanceMaterial::setTextureMem(
    uint stage, uint8 *data, uint32 length, bool _delete, bool isFile /*=true*/,
  ...
  _Object->setTexture(
      (uint8)stage,
      new CTextureMem(data, length, _delete, isFile, width, height, texType));
}

// ***************************************************************************
bool UInstanceMaterial::isLighted() const { return _Object->isLighted(); }
...
void UInstanceMaterial::setOpacity(uint8 opa) { _Object->setOpacity(opa); }
...
CRGBA UInstanceMaterial::getEmissive() const { return _Object->getEmissive(); }

// ***************************************************************************

CRGBA UInstanceMaterial::getAmbient() const { return _Object->getAmbient(); }

// ***************************************************************************

CRGBA UInstanceMaterial::getDiffuse() const { return _Object->getDiffuse(); }

// ***************************************************************************

uint8 UInstanceMaterial::getOpacity() const { return _Object->getOpacity(); }

// ***************************************************************************

CRGBA UInstanceMaterial::getSpecular() const { return _Object->getSpecular(); }
...
void UInstanceMaterial::setColor(CRGBA rgba) { _Object->setColor(rgba); }

// ***************************************************************************

CRGBA UInstanceMaterial::getColor(void) const { return _Object->getColor(); }
...
void UInstanceMaterial::setBlend(bool active) { _Object->setBlend(active); }

// ***************************************************************************

void UInstanceMaterial::setBlendFunc(TBlend src, TBlend dst) {
  _Object->setBlendFunc((CMaterial::TBlend)(uint32)src,
                        (CMaterial::TBlend)(uint32)dst);
}
...
void UInstanceMaterial::setZWrite(bool active) { _Object->setZWrite(active); }
...
bool UInstanceMaterial::getBlend() const { return _Object->getBlend(); }
...
void UInstanceMaterial::enableUserTexMat(uint stage, bool enabled) {
  if (stage >= IDRV_MAT_MAXTEXTURES) {
    nlwarning("UInstanceMaterial::enableUserTexMat : stage %d is invalid",
              stage);
    return;
  }
...
void UInstanceMaterial::setUserTexMat(uint stage, const NLMISC::CMatrix &m) {
  ...
  if (!_Object->isUserTexMatEnabled(stage)) {
    nlwarning("UInstanceMaterial::setUserTexMat : texture stage %d has no user "
              "matrix.",
              stage);
  }
...
void UInstanceMaterial::setWrapS(uint stage, TWrapMode mode) {
  if (stage >= IDRV_MAT_MAXTEXTURES || _Object->getTexture(stage) == NULL) {
    nlwarning("UInstanceMaterial::setWrapS : stage %d is invalid or there's no "
              "texture",
              stage);
    return;
  }
...
void UInstanceMaterial::setWrapT(uint stage, TWrapMode mode) {
  if (stage >= IDRV_MAT_MAXTEXTURES || _Object->getTexture(stage) == NULL) {
    nlwarning("UInstanceMaterial::setWrapT : stage %d is invalid or there's no "
              "texture",
              stage);
    return;
  }
...
UInstanceMaterial::TWrapMode UInstanceMaterial::getWrapS(uint stage) const {
  if (stage >= IDRV_MAT_MAXTEXTURES ||
      _Object->getTexture(uint8(stage)) == NULL) {
    nlwarning("UInstanceMaterial::getWrapS : stage %d is invalid or there's no "
              "texture",
              stage);
    return Repeat;
  }
...
UInstanceMaterial::TWrapMode UInstanceMaterial::getWrapT(uint stage) const {
  if (stage >= IDRV_MAT_MAXTEXTURES ||
      _Object->getTexture(uint8(stage)) == NULL) {
    nlwarning("UInstanceMaterial::getWrapT : stage %d is invalid or there's no "
              "texture",
              stage);
    return Repeat;
  }

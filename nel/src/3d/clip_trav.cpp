...
bool CClipTrav::fullSearch(vector<CCluster *> &vCluster, const CVector &pos) {
  ...
  // else must filter to take only the ones that are the lower in the hierarchy
  // (same as in old code)
  else {
    uint i;
  ...
  for (i = 0; i < pIG->_ClusterInstances.size(); ++i)
  {
      ...
      {
          if (fullSearch (vCluster,
  pIG->_ClusterInstances[i]->Children[j]->Group, pos)) return true;
      }
  }
...
/// Set cluster tracking on/off (ie storage of thje visible cluster during clip
/// traversal)
...
void CClipTrav::traverse() {
  ...
  /* For all objects marked visible in preceding render, reset _Visible state
     here. NB: must reset _Visible State to false because sometimes
     traverseClip() are even not executed (Cluster clip, QuadGridClipManager
     clip...). And somes models read this _Visible state. eg:
     Skins/StickedObjects test the Visible state of their
  ...
  */
  ...
  for (i = 0; i < hrcTrav._MovingObjects.size(); ++i) {
    ...
    // Moving object in the world -> link to root or to the
  ...
  // =========================
  /* Can't do it in LoadBalancingTrav because sons with
     _AncestorSkeletonModel!=NULL may be hiden if a skeleton is displayed in
     CLod mode. So must do it here, then clip all sons of
     AncestoreSkeletonModelGroup.
  */
  ...
  // At the end of the clip traverse, must update clip for Objects which have a
  // skeleton ancestor
  // =========================
  // those are linked to the SonsOfAncestorSkeletonModelGroup, so traverse it
  // now.
  if (Scene->SonsOfAncestorSkeletonModelGroup)
    Scene->SonsOfAncestorSkeletonModelGroup->traverseClip();

  // For All Shadow Casters (skeletons + others), clip their ShadowMap possible
  // projection against the frustum only.
  // =========================
  /*
  ...
  */
  ...
  // =========================
  /*
      Done here, because AnimDetail and Render need correct lists. NB: important
     to do it before Render Traversal, because updateSkinRenderLists() may
     change the transparency flag!! NB: can't do it in any traverse() because
     must be sure that it is done (traverseHRC not called if
     SonOfAncestorSkeletonModel, and traverseClip not called if in a cluster).
  ...
  */
  ...
  for (itSkel = Scene->getSkeletonModelListBegin();
       itSkel != Scene->getSkeletonModelListEnd(); itSkel++) {
...
void CClipTrav::registerCluster(CCluster *pCluster) {
  pCluster->AccelIt = Accel.insert(pCluster->getBBox().getMin(),
                                   pCluster->getBBox().getMax(), pCluster);
}
...
struct CFadeShadowMapSort {
  ...
  bool operator<(const CFadeShadowMapSort &o) const { return Dist < o.Dist; }
};
...
void CClipTrav::clipShadowCasters() {
  ...
  // **** First select ShadowCasters that are visible or that may have their
  // shadow visible
  ...
  // For all casters
  for (itShadowCaster = Scene->getShadowCasterListBegin();
       itShadowCaster != Scene->getShadowCasterListEnd(); itShadowCaster++) {
    ...
    // Binded to an Ancestor skeleton?? If so, don't render since the Ancestor
    // Skeleton render all of his sons Additionally, no-op if this caster is
    // hidden in HRC!!
    if (sc->_AncestorSkeletonModel == NULL && sc->isHrcVisible()) {
      ...
      // else do a bigger test
        ...
        // If the model "sc" is a CSkeletonModel, It depends also on Sticked
        // objects/Skeletons. Build the sphere around the caster that can
        // receive shadow.
        CBSphere sphere;
        ...
        // NB: works because suppose that the Shadow BBox include the model
        // Center.
    ...
    // If the model is not visible, or if temporary bound to a
    // AncestorSkeletonModel
    else {
      // Increment the TemporalFadeOut. Because since will be hidden (or
      // sticked) for a long time, allow the process to free texture.
      shadowMap->TemporalOutScreenFade += dFade;
  ...
  // For InScreen Casters, increment or decrement their fadeInScreen. Also
  // resolve InScreenFadeAccum.
  ...
  // **** Use the rendered Skeletons ShadowMap to select the Ones that will be
  // generated this Frame.
  Scene->getRenderTrav().getShadowMapManager().selectShadowMapsToGenerate(
      Scene);

  /* **** Then for All ShadowCasters not visibles but that will generate their
     shadowMap,
      - we must compute the LightTraversal(for ShadowLight direction)
      - for skeleton models only, we must compute the AnimDetailTraversal (for
     bone animation) We MUST NOT flag the skeleton as Visible, and we MUST NOT
     insert in LoadBalancing (since won't be rendered) NB: Do nothing for Sons
     of the Ancestor Skeleton because: 1/ light do nothing with them (see std
     clip) 2/ animDetail will be called recursively (see std clip and
     CSkeletonModel::traverseAnimDetail())
  */
  for (itShadowCaster = Scene->getShadowCasterListBegin();
       itShadowCaster != Scene->getShadowCasterListEnd(); itShadowCaster++) {
    CTransform *sc = *itShadowCaster;

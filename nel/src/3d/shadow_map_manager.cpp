...
CShadowMapManager::CShadowMapManager() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    // Setup The Blur. NB: it will take advantage of Max 4 texture driver
    // support, but will still work with 2 or 3 (less beautifull).
    uint j;
    for (j = 1; j < 4; j++) {
      _BlurMaterial[i].texEnvOpRGB(j, CMaterial::InterpolateConstant);
      _BlurMaterial[i].texEnvArg0RGB(j, CMaterial::Texture,
                                     CMaterial::SrcColor);
      _BlurMaterial[i].texEnvArg1RGB(j, CMaterial::Previous,
                                     CMaterial::SrcColor);
      _BlurMaterial[i].texEnvOpAlpha(j, CMaterial::InterpolateConstant);
      _BlurMaterial[i].texEnvArg0Alpha(j, CMaterial::Texture,
                                       CMaterial::SrcAlpha);
      _BlurMaterial[i].texEnvArg1Alpha(j, CMaterial::Previous,
                                       CMaterial::SrcAlpha);
    }
    // Factor for Stage so the sum is 1.
    _BlurMaterial[i].texConstantColor(1,
                                      CRGBA(128, 128, 128, 128)); // factor= 1/2
  ...
  // *** Setup copy
  _CopyQuads.setVertexFormat(CVertexBuffer::PositionFlag |
  ...
  // build the texture
  _ClampTexture = new CTextureMem(tmpMem, 4 * clampTextSize * 1, true, false,
                                  clampTextSize, 1);
  ...
  // ---- Stage 0. Project the ShadowMap. Blend the color between ShadowColor
  // and White. setup texture coord gen
  _ReceiveShadowMaterial.enableUserTexMat(0, true);
  _ReceiveShadowMaterial.setTexCoordGen(0, true);
  _ReceiveShadowMaterial.setTexCoordGenMode(0,
                                            CMaterial::TexCoordGenObjectSpace);
  // Setup the stage so we interpolate ShadowColor and White (according to
  // shadowmap alpha) nico : with D3D driver, limitation of the number of per
  // stage constant (Only 1 if diffuse is used), so do a blend between inv
  // diffuse & black (instead of diffuse & white), which resolve to a modulate
  // between source alpha & inverse diffuse.  then invert result at subsequent
  // stage
  _ReceiveShadowMaterial.texEnvOpRGB(0, CMaterial::Modulate);
  _ReceiveShadowMaterial.texEnvArg0RGB(0, CMaterial::Diffuse,
                                       CMaterial::InvSrcColor);
  _ReceiveShadowMaterial.texEnvArg1RGB(0, CMaterial::Texture,
                                       CMaterial::SrcAlpha);
  // Take Alpha for AlphaTest only.
  _ReceiveShadowMaterial.texEnvOpAlpha(0, CMaterial::Replace);
  _ReceiveShadowMaterial.texEnvArg0Alpha(0, CMaterial::Texture,
                                         CMaterial::SrcAlpha);

  // ---- Stage 1. "Modulate" by Clamp Texture. Blend the color between stage0
  // color and White. setup texture coord gen
  _ReceiveShadowMaterial.enableUserTexMat(1, true);
  _ReceiveShadowMaterial.setTexCoordGen(1, true);
  _ReceiveShadowMaterial.setTexCoordGenMode(1,
                                            CMaterial::TexCoordGenObjectSpace);
  _ReceiveShadowMaterial.setTexture(1, _ClampTexture);
  // Setup the stage so we interpolate Shadow and White (according to clamp
  // alpha)
  _ReceiveShadowMaterial.texEnvOpRGB(1, CMaterial::Modulate);
  _ReceiveShadowMaterial.texEnvArg0RGB(
      1, CMaterial::Previous,
      CMaterial::SrcColor); // Color is inverted before the blend
  _ReceiveShadowMaterial.texEnvArg1RGB(1, CMaterial::Texture,
                                       CMaterial::SrcAlpha);
  // Take Alpha for AlphaTest only. (take 1st texture alpha...)
  _ReceiveShadowMaterial.texEnvOpAlpha(0, CMaterial::Replace);
  _ReceiveShadowMaterial.texEnvArg0Alpha(0, CMaterial::Previous,
                                         CMaterial::SrcAlpha);
...
// ***************************************************************************
CShadowMapManager::~CShadowMapManager() { clearAllShadowCasters(); }
...
void CShadowMapManager::renderGenerate(CScene *scene) {
  ...
  CSmartPtr<NL3D::ITexture> previousRenderTarget =
      driverForShadowGeneration->getRenderTarget();
  ...
  // Minimize the Dest Texture size, so the blurTexture don't get too heavy in
  // VRAM.
  uint32 textDestW = min(wndW, (uint32)NL3D_SMM_MAX_TEXTDEST_SIZE);
  uint32 textDestH = min(wndH, (uint32)NL3D_SMM_MAX_TEXTDEST_SIZE);

  // if not needed or if not possible, exit. test for wndSize is also important
  // when window is minimized
  if (_ShadowCasters.empty() || textDestW < baseTextureSize ||
      textDestH < baseTextureSize) {
    clearAllShadowCasters();
    return;
  }

  // If Needed to project some ShadowCaster, but none to compute this frame,
  // quit.
  ...
  updateBlurTexture(*driverForShadowGeneration, numTextW * baseTextureSize,
                    numTextH * baseTextureSize);
  ...
  while (numSC > 0) {
    ...
    driverForShadowGeneration->setRenderTarget(_BlurTexture[0], 0, 0,
                                               numTotalCol * baseTextureSize,
                                               numTotalLine * baseTextureSize);
    ...
    // Render the polygons with Smooth Anti-Alias. Less jittering for little
    // performance overcost
    if (_PolySmooth)
    ...
    for (i = 0; i < numPassSC; i++) {
      ...
      CViewport vp;
      vp.init(textX * baseTextureSize / (float)_BlurTextureW,
              textY * baseTextureSize / (float)_BlurTextureH, vpWidth,
              vpHeight);
      driverForShadowGeneration->setupViewport(vp);

      // TODO_SHADOW: optim: one big erase per pass, but just bbox needed
      // (according to number of SC to render) do a siccor or prefer do a
      // polygon clear?
      CScissor sic;
      sic.init(textX * baseTextureSize / (float)_BlurTextureW,
               textY * baseTextureSize / (float)_BlurTextureH, vpWidth,
               vpHeight);
    ...
    vp.initFullScreen();
    driverForShadowGeneration->setupViewport(vp);
    driverForShadowGeneration->setFrustum(0, (float)_BlurTextureW, 0,
                                          (float)_BlurTextureH, -1, 1, false);
    ...
    fillBlackBorder(driverForShadowGeneration, numPassSC, numTextW, numTextH,
                    baseTextureSize);
    ...
    for (i = 0; i < numBlur; i++) {
      ...
      driverForShadowGeneration->setRenderTarget(
          _BlurTexture[blurTarget], 0, 0, numTotalCol * baseTextureSize,
          numTotalLine * baseTextureSize);

      // blur
      applyFakeGaussianBlur(driverForShadowGeneration, numPassSC, numTextW,
                            numTextH, baseTextureSize, blurSource);
    ...
    for (i = 0; i < numPassSC; i++) {
      ...
      if (sm) {
        ...
        if (text) {
          ...
          // if (!driverForShadowGeneration->copyTargetToTexture (text, 0, 0,
          // textX*bts, textY*bts, bts, bts))
          {
            // Copy the texture
  ...
  // Restore driver state. (do it here because driverForShadowGeneration may be
  // the main screen).
  ...
  // ensure the Scene Driver has correct matrix setup (in common case where
  // AuxDriver == Std Driver)
...
void CShadowMapManager::renderProject(CScene *scene) {
  ...
  /* Fog Case: Since we do a modulate, we don't want to modulate the fog color
     with himself. Instead, if the shadowed pixel is in full fog, we have to
     modulate him with Blac (modulate with INVERSE-source color, actually ...)
  ...
  */
  ...
  /* Light case: CVisualCollisionManager use a fakeLight to avoid ShadowMapping
     on backFaces of meshs Hence must clean all lights, and enable only the
     Light0 in driver
  */
  ...
  for (uint i = 0; i < _ShadowCasters.size(); i++) {
    ...
    nlassert(sm);
    // NB: the ShadowCaster may not have a texture yet, for example because of
    // Generate selection... If the final fade is 1, don't render!
    if (sm->getTexture() && sm->getFinalFade() < 1) {
      ...
      /// Get The Mean Ambiant and Diffuse the caster receive (and cast, by
      /// approximation)
      CRGBA ambient, diffuse;
      computeShadowColors(scene, caster, ambient, diffuse);
      // In some case, the ambient may be black, which cause problems because
      // the shadow pop while diffuse fade. ThereFore, supose always a minimum
      // of ambiant 10.
      ambient.R = max(uint8(10), ambient.R);
      ambient.G = max(uint8(10), ambient.G);
      ambient.B = max(uint8(10), ambient.B);
      // copute the shadowColor so that modulating a Medium diffuse terrain will
      ...
      clamp(G, 1U, 256U);
      clamp(B, 1U, 256U);
      /* screen= text*(a+d*0.5) (mean value). if we do shadowColor=
         a/(a+d*0.5f), then we'll have "in theory"  screen= text*a
      */
      ...
      if (sm->getFinalFade() > 0) {
        ...
        clamp(factor, 0, 256);
        R = 255 * factor + R * (256 - factor);
        R >>= 8;
        G = 255 * factor + G * (256 - factor);
        G >>= 8;
        B = 255 * factor + B * (256 - factor);
        B >>= 8;
      }
      ...
      for (it = _ShadowReceiverGrid.begin(); it != _ShadowReceiverGrid.end();
           it++) {
        ...
        // see getReceiverRenderWorldMatrix() Doc for why using this instead of
        // getWorldMatrix()
        _ShadowMapProjector.applyToMaterial(
            receiver->getReceiverRenderWorldMatrix(), _ReceiveShadowMaterial);

        // cast the shadow on them
        receiver->receiveShadowMap(sm, casterPos, _ReceiveShadowMaterial);
      }

      // Additionaly, the VisualCollisionManager may manage some shadow
      // receiving
      CVisualCollisionManager *shadowVcm =
          scene->getVisualCollisionManagerForShadow();
      if (shadowVcm) {
        shadowVcm->receiveShadowMap(
            driver, sm, casterPos, _ReceiveShadowMaterial, _ShadowMapProjector);
      }
    }
  }
  ...
  /*{
      for(uint i=0;i<_ShadowCasters.size();i++)
      {
          ...
          if(sm)
          {
              CVector		p0= sm->LocalProjectionMatrix.getPos() +
  sc->getWorldMatrix().getPos(); IDriver		&drv= *driver;

              drv.setupModelMatrix(CMatrix::Identity);

              CDRU::drawWiredBox(p0, sm->LocalProjectionMatrix.getI(),
  sm->LocalProjectionMatrix.getJ(), sm->LocalProjectionMatrix.getK(),
  CRGBA::White, drv);
          }
      }
  }*/
  ...
  CopyQuads.setVertexFormat (CVertexBuffer::PositionFlag |
  CVertexBuffer::TexCoord0Flag); CopyQuads.setNumVertices(4);
  {
      CVertexBufferReadWrite vba;
      CopyQuads.lock (vba);
      vba.setVertexCoord (0, CVector (0, 0, 0));
...
// ***************************************************************************
void CShadowMapManager::computeShadowDirection(CScene *scene, CTransform *sc,
                                               CVector &lightDir) {
  ...
  for (uint i = 0; i < NL3D_MAX_LIGHT_CONTRIBUTION; i++) {
    ...
    color = pl->getDiffuse();
    lightDir +=
        plDir * (float)lc.AttFactor[i] * (float)(color.R + color.G + color.B);
  }
...
// ***************************************************************************
void CShadowMapManager::computeShadowColors(CScene *scene, CTransform *sc,
                                            CRGBA &ambient, CRGBA &diffuse) {
  const CLightContribution &lc = sc->getLightContribution();
  ...
  // Don't take the MergedPointLight into consideration (should add to the
  // diffuse part here, but rare case)
...
// ***************************************************************************
void CShadowMapManager::fillBlackBorder(IDriver *drv, uint numPassText,
                                        uint numTextW, uint numTextH,
...
// ***************************************************************************
void CShadowMapManager::setBlackQuad(uint index, sint x, sint y, sint w,
                                     sint h) {
  float x0 = (float)x;
  float y0 = (float)y;
...
void CShadowMapManager::updateBlurTexture(IDriver &drv, uint w, uint h) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    _BlurTexture[i] =
        new CTextureMem(tmpMem, 4 * _BlurTextureW * _BlurTextureH, true, false,
                        _BlurTextureW, _BlurTextureH);
...
// ***************************************************************************
void CShadowMapManager::copyScreenToBlurTexture(IDriver *drv, uint numPassText,
                                                uint numTextW, uint numTextH,
                                                uint baseTextureSize) {
  if (numPassText == 0)
    return;

  // TODO_SHADOW: optim: split into 2 copy for less pixel draw on the last line?
  // No because of OverHead?
  ...
  /* todo hulud shadows
  drv->copyFrameBufferToTexture(_BlurTexture, 0, 0, 0, 0, 0,
  numTotalCol*baseTextureSize, numTotalLine*baseTextureSize); */
}

// ***************************************************************************
void CShadowMapManager::applyFakeGaussianBlur(IDriver *drv, uint numPassText,
                                              uint numTextW, uint numTextH,
                                              uint baseTextureSize,
                                              uint blurSource) {
  if (numPassText == 0)
  ...
  // Split into 2 quads. one for the first full lines, and one for the last not
  // full line.
  uint index = 0;
  if (numFullLine)
    setBlurQuadFakeGaussian(index++, 0, 0, numTextW * baseTextureSize,
                            numFullLine * baseTextureSize);
  if (lastLineNumCol)
    setBlurQuadFakeGaussian(index++, 0, numFullLine * baseTextureSize,
                            lastLineNumCol * baseTextureSize, baseTextureSize);
...
// ***************************************************************************
void CShadowMapManager::setBlurQuadFakeGaussian(uint index, sint x, sint y,
                                                sint w, sint h) {
  float x0 = (float)x;
  float y0 = (float)y;
  float x1 = (float)x + (float)w;
  ...
  // NB: the order of the Delta (--,++,-+,+-) is made so it works well with 2,3
  // or 4 texture support.
...
struct CShadowMapSort {
  ...
  bool operator<(const CShadowMapSort &o) const { return Weight < o.Weight; }
};
...
void CShadowMapManager::selectShadowMapsToGenerate(CScene *scene) {
  ...
  for (i = 0; i < _ShadowCasters.size(); i++) {
    ...
    /* If the shadowMap exist, and if not totaly faded
        NB: take FinalFade here because if 1, it won't be rendered in
       renderProject() so don't really need to update (useful for update reason,
       but LastGenerationFrame do the job)
    */
    if (caster->getShadowMap() && caster->getShadowMap()->getFinalFade() < 1) {
      CShadowMapSort sms;
      sms.Caster = caster;
      // The Weight is the positive delta of frame
      sms.Weight = (float)(scene->getNumRender() -
                           caster->getShadowMap()->LastGenerationFrame);
  ...
  // **** Flag selecteds
  // For All selected models, indicate that they will generate shadowMap for
  // this Frame.
...
ITexture *CShadowMapManager::allocateTexture(uint textSize) {
  ...
  if (!_FreeShadowTextures.empty()) {
    ...
    // else, suppose that we still take this slot.
      ...
      // but since bad size, delete this slot from the map and create a new one
      // (below)
  ...
  ITexture *text;
  text = new CTextureMem(tmpMem, 4 * textSize * textSize, true, false, textSize,
                         textSize);
...
void CShadowMapManager::garbageShadowTextures(CScene *scene) {
  ...
  // For all Free Textures only, release the one that are no more of the wanted
  // default ShadowMap Size.
  ...
  if (_FreeShadowTextures.size() > NL3D_SMM_MAX_FREETEXT) {
    // Release the extra texture (Hysteresis: divide by 2 the max wanted free to
    // leave)

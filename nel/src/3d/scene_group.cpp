...
void CInstanceGroup::getInstanceMatrix(uint instanceNb,
                                       NLMISC::CMatrix &dest) const {
  dest.identity();
...
// ***************************************************************************
const CInstanceGroup::CInstance &
CInstanceGroup::getInstance(uint instanceNb) const {
...
// ***************************************************************************
void CInstanceGroup::build(
    const CVector &vGlobalPos, const TInstanceArray &array,
    const std::vector<CCluster> &Clusters, const std::vector<CPortal> &Portals,
...
// ***************************************************************************
void CInstanceGroup::build(const CVector &vGlobalPos,
...
// ***************************************************************************
void CInstanceGroup::retrieve(
    CVector &vGlobalPos, TInstanceArray &array, std::vector<CCluster> &Clusters,
  ...
  if (version >= 1) {
    ...
    } else // We are writing to the stream
    {
      ...
      for (i = 0; i < _ClusterInfos.size(); ++i) {
        ...
        for (j = 0; j < nNbPortals; ++j) {
          sint32 nPortalNb =
              (sint32)(_ClusterInfos[i]._Portals[j] - &_Portals[0]);
          f.serial(nPortalNb);
        }
      }
    }
...
// ***************************************************************************
void CInstanceGroup::setAddRemoveInstanceCallback(
    IAddRemoveInstance *callback) {
...
bool CInstanceGroup::addToScene(CScene &scene, IDriver *driver,
  ...
  for (i = 0; i < _Portals.size(); ++i)
    for (j = 0; j < 2; ++j) {
      if (_Portals[i]._Clusters[j] == NULL) {
        nlwarning("Portal %d (name:%s) is not linked to 2 clusters. Instance "
                  "Group Not Added To Scene.",
                  i, _Portals[i].getName().c_str());
      }
    }
  ...
  // Water surface may have a callback when they are created, and this callback
  // need their position Their position isn't set right now however, so must
  // call that callback later
  ...
  for (i = 0; i < _InstancesInfos.size(); ++i, ++it) {
    ...
    if (!shapeName.empty()) {
      if (!_InstancesInfos[i].DontAddToScene) {
        ...
        if (nlstricmp(NLMISC::CFile::getExtension(shapeName), "pacs_prim") ==
            0) {
...
void CInstanceGroup::getShapeName(uint instanceIndex,
  ...
  if (_TransformName != NULL && !rInstanceInfo.InstanceName.empty()) {
    shapeName = _TransformName->transformName(
...
// ***************************************************************************
// Private method
bool CInstanceGroup::addToSceneWhenAllShapesLoaded(CScene &scene,
                                                   IDriver *driver,
                                                   uint selectedTexture) {
  uint32 i, j;
    ...
    if (!rInstanceInfo.DontAddToScene) {
      if (_Instances[i]) {
        ...
        if (scene.getWaterCallback()) {
          ...
          if (wm) {
            const CWaterShape *ws =
                safe_cast<const CWaterShape *>((const IShape *)wm->Shape);
            scene.getWaterCallback()->waterSurfaceAdded(
                ws->getShape(), wm->getMatrix(), ws->isSplashEnabled(),
        ...
        if (rInstanceInfo.StaticLightEnabled) {
          // Count lights.
          uint numPointLights;
          for (numPointLights = 0;
               numPointLights < CInstanceGroup::NumStaticLightPerInstance;
               numPointLights++) {
          ...
          // Max allowed.
          numPointLights =
              min(numPointLights, (uint)NL3D_MAX_LIGHT_CONTRIBUTION);
          ...
          if (rInstanceInfo.LocalAmbientId == 0xFF)
            ...
            frozenAmbientlight =
                (CPointLight
                     *)(&_PointLightArray
                             .getPointLights()[rInstanceInfo.LocalAmbientId]);

          // Setup the instance.
          _Instances[i]->freezeStaticLightSetup(pls, numPointLights,
                                                rInstanceInfo.SunContribution,
                                                frozenAmbientlight);
        }
  ...
  // Attach the root of the instance group to the root of the hierarchy
  // traversal
  ...
  for (i = 0; i < _ClusterInstances.size(); ++i) {
    ...
    _ClusterInstances[i]->VisibleFromFather =
        _ClusterInfos[i].VisibleFromFather;
    _ClusterInstances[i]->FatherAudible = _ClusterInfos[i].FatherAudible;
    _ClusterInstances[i]->AudibleFromFather =
        _ClusterInfos[i].AudibleFromFather;
      ...
      if (!_InstancesInfos[i].Clusters.empty()) {
        ...
        for (j = 0; j < _InstancesInfos[i].Clusters.size(); ++j) {
          ...
          if (clusterInst < _ClusterInstances.size())
            ...
            nlwarning("IG: BUG: Cluster infos size %u, indexing %u",
                      (uint32)_ClusterInstances.size(), clusterInst);
        }
        // For the first time we have to set all the instances to NOT move (and
        // not be rebinded)
        _Instances[i]->freeze();
        _Instances[i]->setClusterSystem(this);
      } else {
        // These instances are not attached to a cluster at this level so we
        // cannot freeze them Moreover we must set their clustersystem they will
        // be tested against
...
// ***************************************************************************
// Search in the hierarchy of ig the most low level (child) ig that contains the
// clusters that are flagged to be visible from father or which father is
// visible
bool CInstanceGroup::linkToParent(CInstanceGroup *pFather) {
  ...
  /*
      for (i = 0; i < pFather->_ClusterInstances.size(); ++i)
      {
          ...
          {
              if
     (linkToParent(pFather->_ClusterInstances[i]->Children[j]->Group)) return
     true;
          }
      }
  */
    ...
    for (j = 0; j < this->_ClusterInstances.size(); ++j) {
      if ((this->_ClusterInstances[j]->FatherVisible) ||
        ...
        for (i = 0; i < pFather->_ClusterInstances.size(); ++i) {
          ...
          if (pFather->_ClusterInstances[i]->isIn(
                  this->_ClusterInstances[j]->getBBox())) {
            if (this->_ClusterInstances[j]->Father !=
                pFather->_ClusterInstances[i]) // and not already son of the
                                               // father cluster ?
            {
              // unlink from parent
              ...
              pFather->_ClusterInstances[i]->Children.push_back(
                  this->_ClusterInstances[j]);
              this->_ClusterInstances[j]->Father =
                  pFather->_ClusterInstances[i];
            }
            ret = true;
          }
        }
...
// ***************************************************************************
void CInstanceGroup::setBlendShapeFactor(const string &BlendShapeName,
                                         float rFactor) {
  for (uint32 i = 0; i < _Instances.size(); ++i) {
...
// ***************************************************************************
void CInstanceGroup::buildPointLightList(
    const std::vector<CPointLightNamed> &pointLightList,
    std::vector<uint> &plRemap) {
  // build.
...
// ***************************************************************************
void CInstanceGroup::displayDebugClusters(IDriver *drv,
    ...
    if (cluster) {
      ...
      if (numTotalVertices <= maxVertices) {
          ...
          if (poly.Vertices.size() >= 3) {
            ...
            for (k = 0; k < poly.Vertices.size() - 2; k++) {
              if (numTriIndexes < clusterTriangles.capacity()) {
                ibaCT.setTri(numTriIndexes, iVert + 0, iVert + k + 1,
                             iVert + k + 2);
                numTriIndexes += 3;
              }
            }
            ...
            for (k = 0; k < poly.Vertices.size(); k++) {
              if (numLineIndexes < clusterLines.capacity()) {
                ibaCL.setLine(numLineIndexes, iVert + k,
                              iVert + ((k + 1) % poly.Vertices.size()));
                numLineIndexes += 2;
              }
            }
        ...
        for (j = 0; j < cluster->_Portals.size(); j++) {
          ...
          if (portalVerts.size() >= 3) {
            ...
            for (k = 0; k < portalVerts.size() - 2; k++) {
              if (numTriIndexes < clusterTriangles.capacity()) {
                ibaPT.setTri(numTriIndexes, iVert + 0, iVert + k + 1,
                             iVert + k + 2);
                numTriIndexes += 3;
              }
            }
            ...
            for (k = 0; k < portalVerts.size(); k++) {
              if (numTriIndexes < clusterTriangles.capacity()) {
                ibaPL.setLine(numLineIndexes, iVert + k,
                              iVert + ((k + 1) % portalVerts.size()));
                numLineIndexes += 2;
              }
            }
        ...
        for (uint pass = 0; pass < 2; pass++) {
          ...
          drv->renderTriangles(clusterMat, 0,
                               clusterTriangles.getNumIndexes() / 3);
          ...
          drv->renderTriangles(portalMat, 0,
                               portalTriangles.getNumIndexes() / 3);

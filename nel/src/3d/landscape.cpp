...
/*
    ...
    A good BlockSize (in my opinion) is EstimatedMaxSize / 10, to have less
   memory leak as possible, and to make not so many system allocation.

    NL3D_TESSRDR_ALLOC_BLOCKSIZE is 2 times less, because elements are in Far
   zone or in Near zone only (approx same size...)
*/
...
/* This value is important, because faces will be inserted at maximum at this
   entry in the priority list. If not so big (eg 50 meters), a big bunch of
   faces may be inserted in this entry, which may cause slow down sometimes,
   when all this bunch comes to 0 in the priority list. To avoid such a thing,
   see CTessFacePriorityList::init(), and use of NL3D_REFINE_PLIST_DIST_MAX_MOD.
    Here, distMax= 2048*0.0625= 128
*/
#define NL3D_REFINE_PLIST_NUM_ENTRIES 2048
#define NL3D_REFINE_PLIST_DIST_MAX_MOD 0.7f
// For the Split priority list only, numbers of quadrants. MergeList has 0
// quadrants.
#define NL3D_REFINE_PLIST_SPLIT_NUMQUADRANT 16

/*
    OverHead size of one RollingTable of priority list is 8 *
   (NL3D_REFINE_PLIST_NUM_ENTRIES) So here, it is "only" 16K.

    Since we have 2 Priority list and 16 quadrants for the split one, the total
   overhead is 18*12.8= 288K
*/
...
const char *EBadBind::what() const throw() {
  ...
  for (it = BindErrors.begin(); it != BindErrors.end(); it++, numErr++) {
    ...
    sint y = it->ZoneId >> 8;
    sprintf(tmp, "zone%3d_%c%c.patch%3d;   ", y + 1, (char)('A' + (x / 26)),
            (char)('A' + (x % 26)), it->PatchId + 1);
...
CLandscape::CLandscape()
  ...
  _SplitPriorityList.init(
      NL3D_REFINE_PLIST_DIST_STEP, NL3D_REFINE_PLIST_NUM_ENTRIES,
      NL3D_REFINE_PLIST_DIST_MAX_MOD, NL3D_REFINE_PLIST_SPLIT_NUMQUADRANT);
  // See updateRefine* Doc in tesselation.cpp for why the merge list do not need
  // quadrants.
  _MergePriorityList.init(NL3D_REFINE_PLIST_DIST_STEP,
                          NL3D_REFINE_PLIST_NUM_ENTRIES,
                          NL3D_REFINE_PLIST_DIST_MAX_MOD, 0);
  // just for getTesselatedPos to work properly.
  _OldRefineCenter = CVector::Null;

  // create / Init the vegetable manager.
  _VegetableManager =
      new CVegetableManager(NL3D_LANDSCAPE_VEGETABLE_MAX_AGP_VERTEX_UNLIT,
                            NL3D_LANDSCAPE_VEGETABLE_MAX_AGP_VERTEX_LIGHTED);
  ...
  // Dynamic Lighting.
  _TextureDLM =
      new CTextureDLM(NL3D_LANDSCAPE_DLM_WIDTH, NL3D_LANDSCAPE_DLM_HEIGHT);
  ...
  CLandscapeGlobals::PassTriArray.setPreferredMemory(CIndexBuffer::RAMVolatile,
                                                     false);
  ...
  _TextureNearCategory =
      new ITexture::CTextureCategory("LANDSCAPE LIGHTMAP NEAR");
}
...
void CLandscape::init() {
  // Fill Far mat.
  // Must init his BlendFunction here!!! becaus it switch between blend on/off
  // during rendering.
...
// ***************************************************************************
uint CLandscape::getTileMaxSubdivision() { return _TileMaxSubdivision; }
...
bool CLandscape::addZone(const CZone &newZone) {
  ...
  // NB: adding a zone may add vertices in VB in visible patchs (because of
  // binds)=> buffers are locked.
  ...
  for (sint i = 0; i < zone->getNumPatchs(); i++) {
    ...
    // NB: the bbox of zone is used to remove patch. Hence it is VERY important
    // that zoneBBox includes all patchs bbox (else some patchs entries may not
    // be deleted in removeZone()).
  ...
  // Because bind may add faces in other (visible) zones because of enforced
  // split, we must check and update any FaceVector.
...
bool CLandscape::removeZone(uint16 zoneId) {
  // -1. Update globals
  updateGlobalsAndLockBuffers(CVector::Null);
  // NB: remove a zone may change vertices in VB in visible patchs => buffers
  // are locked.
  ...
  // use the quadgrid itself to find where patch are. do this using bbox of
  // zone.
  CAABBox zoneBBForErase = zone->getZoneBB().getAABBox();
  // Avoid precision problems by enlarging a little bbox size of zone for erase
  zoneBBForErase.setHalfSize(zoneBBForErase.getHalfSize() * 1.1f);
  ...
  // because of forceMerge() at unbind, removeZone() can cause change in faces
  // in other (visible) zones.
  updateTessBlocksFaceVector();
...
void CLandscape::buildZoneName(sint zoneId, std::string &zoneName) {
  ...
  sint x = zoneId & 255;
  sint y = zoneId >> 8;
  sprintf(tmp, "%d_%c%c", y + 1, (char)('A' + (x / 26)),
          (char)('A' + (x % 26)));
  zoneName = tmp;
}
// ***************************************************************************
...
void CLandscape::setDriver(IDriver *drv) {
  ...
  if (_Driver != drv) {
    ...
    _DriverOkForVegetable =
        _VertexShaderOk && (_Driver->getMaxVerticesByVertexBufferHard() >=
                            (uint)NL3D_LANDSCAPE_VEGETABLE_MAX_AGP_VERTEX_MAX);
  }
}

// ***************************************************************************
void CLandscape::clip(const CVector &refineCenter,
                      const std::vector<CPlane> &pyramid) {
  // -1. Update globals
  updateGlobalsAndLockBuffers(refineCenter);
  // NB: clip may add/remove vertices in VB in visible patchs => buffers are
  // locked.
...
void CLandscape::refine(const CVector &refineCenter) {
  ...
  // Update globals
  updateGlobalsAndLockBuffers(refineCenter);
  // NB: refine may change vertices in VB in visible patchs => buffers are
  // locked.
  ...
  /* While there is still face in list, update them
      NB: updateRefine() always insert the face in _***PriorityList, so face is
     removed from root***TessFaceToUpdate list. NB: it is possible ( with
     enforced merge() ) that faces dissapears from root***TessFaceToUpdate list
      before they are traversed here. It is why we must use a Circular list
     system, and not an array of elements. Basically. TessFaces are ALWAYS in a
     list, either in one of the entry list in _***PriorityList, or in
      root***TessFaceToUpdate list.

      It is newTessFace() and deleteTessFace() which insert/remove the nodes in
     the list.
  */
  ...
  while (rootMergeTessFaceToUpdate.nextInPList() !=
    ...
    // Get the face.
    CTessFace *face =
        static_cast<CTessFace *>(rootMergeTessFaceToUpdate.nextInPList());

    // update the refine of this face. This may lead in deletion (merge) of
    // other faces which are still in root***TessFaceToUpdate, but it's work.
  ...
  do {
    ...
    while (rootSplitTessFaceToUpdate.nextInPList() !=
      ...
      // Get the face.
      CTessFace *face =
          static_cast<CTessFace *>(rootSplitTessFaceToUpdate.nextInPList());
  ...
  // do it until we are sure no more split is needed, ie no more faces are
  // created
  ...
  // Before unlockBuffers, test for vegetable IG creation.
  {
    ...
    for (; vegetBlock != NULL;
         vegetBlock = (CLandscapeVegetableBlock *)vegetBlock->Next) {
...
void CLandscape::refineAll(const CVector &refineCenter) {
  // -1. Update globals
  updateGlobalsAndLockBuffers(refineCenter);
  // NB: refineAll may change vertices in VB in visible patchs => buffers are
  // locked.
...
// ***************************************************************************
void CLandscape::excludePatchFromRefineAll(sint zoneId, uint patch,
                                           bool exclude) {
...
void CLandscape::averageTesselationVertices() {
  // -1. Update globals
  updateGlobalsAndLockBuffers(CVector::Null);
  // NB: averageTesselationVertices may change vertices in VB in visible patchs
  // => buffers are locked.
  ...
  // averageTesselationVertices() should not cause change in faces in any
  // patchs. It should not happens, but check for security.
  nlassert(_TessBlockModificationRoot.getNextToModify() == NULL);
  updateTessBlocksFaceVector();
}
...
static inline void initPassTriArray(CPatchRdrPass & /* pass */,
  ...
  NL3D_LandscapeGlobals_PassTriCurPtr =
      CLandscapeGlobals::PassTriArrayIBA.getPtr();
  NL3D_LandscapeGlobals_PassTriFormat =
      CLandscapeGlobals::PassTriArrayIBA.getFormat();
}
...
static
...
#endif
    void
    drawPassTriArray(CMaterial &mat) {
  nlassert(CLandscapeGlobals::PassTriArray.isLocked());
  ...
  if (NL3D_LandscapeGlobals_PassNTri > 0) {
    ...
    CLandscapeGlobals::PatchCurrentDriver->activeIndexBuffer(
        CLandscapeGlobals::PassTriArray);
    CLandscapeGlobals::PatchCurrentDriver->renderSimpleTriangles(
        0, NL3D_LandscapeGlobals_PassNTri);
...
// ***************************************************************************
void CLandscape::render(const CVector &refineCenter, const CVector &frontVector,
  ...
  //====================
  updateGlobalsAndLockBuffers(refineCenter);
  // NB: render may change vertices in VB in visible patchs => buffers are
  // locked.
  ...
  // Tiles.
  for (itTile = TileRdrPassSet.begin(); itTile != TileRdrPassSet.end();
       itTile++) {
  ...
  // Lightmaps.
  for (sint lightRdrPass = 0; lightRdrPass < (sint)_TextureNears.size();
       lightRdrPass++) {
  ...
  // Reallocation Mgt. If any of the VB is reallocated, we must refill it
  // entirely. NB: all VBs are refilled entirely. It is not optimal (maybe 3*
  // too slow), but reallocation are supposed to be very rare.
  if (_Far0VB.reallocationOccurs() || _Far1VB.reallocationOccurs() ||
      _TileVB.reallocationOccurs())
    _RenderMustRefillVB = true;

  // VertexProgram dependency on RefineThreshold Management. If VertexShader,
  // and if the refineThreshold has changed since the last time, we must refill
  // All the VB, because data are out of date.
  ...
  if (!_VertexShaderOk) {
    ...
    /*
        Optim note: here, lot of vertices are
            ...
            2/ vertices are geomorphed, but not used (because o the Tessblock
       clip), because lot of vertices used by faces in small TessBlocks are
       still in MasterBlock.

        Some tries have been made to solve this, but result are even worse (2
       times or more), because: 1/
                - does not really matter edges of patchs (and corner) because
       the majority is in interior of patch.
                - in this case, we need to reset all the flags which is very
       costly (reparse all zones...) . 2/ Except for the old CTessBlockEdge
       management which not solve all the thing, the other solution is to test
       all faces not clipped (on a per TessBlock basis), to compute only
       vertices needed. But in this cases, result are worse, maybe because there
       is 6 times more tests, and with bad BTB cache.
    */
  }
  ...
  if (_VertexShaderOk) {
    ...
    for (uint i = 0; i < nbvp; ++i) {
      ...
      if (program) {
        // activate the program to set the uniforms in the program state for all
        // programs note: when uniforms are driver state, the indices must be
        ...
        driver->setUniform4f(IDriver::VertexProgram,
                             program->idx().ProgramConstants0, 0, 1, 0.5f, 0);
        // c[5] take RefineCenter
        driver->setUniform3f(IDriver::VertexProgram,
                             program->idx().RefineCenter, refineCenter);
        // c[6] take info for Geomorph trnasition to TileNear.
        driver->setUniform2f(IDriver::VertexProgram, program->idx().TileDist,
                             CLandscapeGlobals::TileDistFarSqr,
                             CLandscapeGlobals::OOTileDistDeltaSqr);
        // c[10] take the fog vector.
        driver->setUniformFog(IDriver::VertexProgram,
                              program->getUniformIndex(CProgramIndex::Fog));
        // c[12] take the current landscape Center / delta Pos to apply
        driver->setUniform3f(IDriver::VertexProgram,
                             program->idx().PZBModelPosition,
                             _PZBModelPosition);
      }
    }
  }
  ...
  // Yoyo: profile
  NL3D_PROFILE_LAND_SET(ProfNTileSetupMaterial,
                        driver->profileSetupedMaterials());
  H_BEFORE(NL3D_Landscape_Render_Tile);

  // First, update Dynamic Lighting for Near, ie multiply Dynamic Lightmap with
  // UserColor, and upload to texture.
  // ==================
  CPatchDLMContext *dlmCtxPtr = _PatchDLMContextList->begin();
  while (dlmCtxPtr != NULL) {
    // do it only if the patch has some Near stuff to render, and if it is
    // visible
    if (dlmCtxPtr->getPatch()->getFar0() == 0 &&
        !dlmCtxPtr->getPatch()->isRenderClipped()) {
      // upload lightmap into textureDLM, modulating before with patch
      // TileColor. NB: no-op if both src and dst are already full black.
  ...
  // Render Order. Must "invert", since initial order is NOT the render order.
  // This is done because the lightmap pass DO NOT have to do any renderTile(),
  // since it is computed in RGB0 pass.
  nlassert(NL3D_MAX_TILE_PASS == 5);
  static sint RenderOrder[NL3D_MAX_TILE_PASS] = {
      NL3D_TILE_PASS_RGB0, NL3D_TILE_PASS_RGB1, NL3D_TILE_PASS_RGB2,
      NL3D_TILE_PASS_LIGHTMAP, NL3D_TILE_PASS_ADD};
  // For ALL pass..
  for (i = 0; i < NL3D_MAX_TILE_PASS; i++) {
    sint passOrder = RenderOrder[i];

    // If VertexShader enabled, and if lightmap or post Add pass, must setup
    // good VertexProgram
    if (_VertexShaderOk) {
    ...
    // NB: important to set Replace and not modulate, because in case of
    // VerexProgram enabled, Diffuse o[COL0] is undefined.
    ...
    if (passOrder == NL3D_TILE_PASS_RGB0) {
      ...
      case NL3D_TILE_PASS_RGB2:
        ...
        TileMaterial.texEnvArg0Alpha(1, CMaterial::Texture,
                                     CMaterial::SrcAlpha);
        break;
      case NL3D_TILE_PASS_LIGHTMAP:
        ...
        // Setup the material envCombine so DynamicLightmap (stage 0) is added
        // to static lightmap.
      ...
      case NL3D_TILE_PASS_ADD:
        // Use srcalpha for src (and not ONE), since additive are blended with
        // alpha Cte/AlphaTexture
        TileMaterial.setBlendFunc(CMaterial::srcalpha, CMaterial::one);

        // for MAYBE LATER smooth night transition, setup Alpha cte of stage0,
        // and setup alpha stage.
        TileMaterial.texEnvOpAlpha(0, CMaterial::Replace);
        TileMaterial.texEnvArg0Alpha(0, CMaterial::Constant,
                                     CMaterial::SrcAlpha);
        ...
        TileMaterial.texEnvArg0Alpha(1, CMaterial::Texture,
                                     CMaterial::SrcAlpha);
        TileMaterial.texEnvArg1Alpha(1, CMaterial::Previous,
                                     CMaterial::SrcAlpha);
    ...
    if (passOrder == NL3D_TILE_PASS_RGB0) {
      ...
      for (itTile = TileRdrPassSet.begin(); itTile != TileRdrPassSet.end();
           itTile++) {
        // Get a ref on the render pass. Const cast work because we only modify
        // attribut from CPatchRdrPass that don't affect the operator< of this
        // class
        CPatchRdrPass &pass = const_cast<CPatchRdrPass &>(*itTile);
        // Enlarge PassTriArray as needed
        CRdrTileId *tileToRdr = pass.getRdrTileRoot(passOrder);
        uint32 numWantedIndex =
            countNumWantedIndex(tileToRdr, NL3D_TILE_PASS_RGB0);
        if (numWantedIndex) {
      ...
      driver->setupFog(driver->getFogStart(), driver->getFogEnd(),
                       CRGBA::White);

      // Lightmap Pass.
      /* \todo yoyo: TODO_CLOUD: setup stage2, and setup texcoord generation.
       COMPLEX because of interaction with Dynamic LightMap
      */
      ...
      for (sint lightRdrPass = 0; lightRdrPass < (sint)_TextureNears.size();
        ...
        if (numWantedIndex) {
          initPassTriArray(pass, numWantedIndex);
          // Setup Lightmap into stage1. Because we share UV with pass RGB0. So
          // we use UV1. Also, now stage0 is used for DynamicLightmap
  ...
  // Yoyo: profile
  NL3D_PROFILE_LAND_SET(ProfNFar0SetupMaterial,
                        driver->profileSetupedMaterials());
  H_BEFORE(NL3D_Landscape_Render_DLM);

  // First, update Dynamic Lighting for Far, ie multiply Dynamic Lightmap with
  // TextureFar, and upload to texture.
  // ==================
  dlmCtxPtr = _PatchDLMContextList->begin();
  while (dlmCtxPtr != NULL) {
    // do it only if the patch has some Far stuff to render, and if it is
    // visible
    if ((dlmCtxPtr->getPatch()->getFar0() > 0 ||
         dlmCtxPtr->getPatch()->getFar1() > 0) &&
        !dlmCtxPtr->getPatch()->isRenderClipped()) {
      // upload lightmap into textureDLM, modulating before with patch
      // TextureFar. NB: no-op if both src and dst are already full black.
  ...
  // Yoyo: profile
  NL3D_PROFILE_LAND_SET(ProfNFar1SetupMaterial,
                        driver->profileSetupedMaterials());
  ...
  if (isVegetableActive()) {
    /* Actually we modulate the DLM with an arbitrary constant for this reason:
        ...
        Doing this without using PixelShader / additional UVs seems to be
       impossible. And add new UVs (+700K in AGP) just for this is not worth the
       effort.

        We prefer using a constant to simulate the "global vegetable color",
       which is a big trick.

        Additionally, the vegetable take the diffuse lighting of landscape,
       which is false because it replaces the diffuse lighting it should have
       (ie with his own Normal and his own "global vegetable color")

        We can't do anything for "correct normal vegetable", but it is possible
       to replace landscape material with vegetable material, by dividing
       _DLMGlobalVegetableColor by LandscapeDiffuseMaterial. This is a very
       approximate result because of CRGBA clamp, but it is acceptable.
    */
    CRGBA vegetDLMCte;
    ...
    dlmCtxPtr = _PatchDLMContextList->begin();
    while (dlmCtxPtr != NULL) {
      // do it only if the patch has some vegetable stuff to render, and if it
      // is visible NB: we may have some vegetable stuff to render if the patch
      // has some TileMaterial created.
      if (dlmCtxPtr->getPatch()->getTileMaterialRefCount() > 0 &&
          !dlmCtxPtr->getPatch()->isRenderClipped()) {
        // NB: no-op if both src and dst are already full black.
        dlmCtxPtr->compileLighting(CPatchDLMContext::ModulateConstant,
                                   vegetDLMCte);
      }
  ...
  if (isVegetableActive()) {
    ...
    _VegetableManager->render(refineCenter, frontVector, vegetablePyramid,
                              _TextureDLM, driver);
  }
}
...
ITexture *CLandscape::findTileTexture(const std::string &textName, bool clamp) {
  ...
  // The object is not owned by this map. It will be own by the multiple RdrPass
  // via CSmartPtr. They will destroy it when no more points to them.
  if (!text) {
...
void CLandscape::loadTile(uint16 tileId) {
  ...
  if (textName.empty())
    ...
    // Fill rdrpass.
    CPatchRdrPass pass;
    // Avoid using Clamp for diffuse, because of recent NVidia GL drivers Bugs
    // in 77.72
    pass.TextureDiffuse =
        findTileTexture(TileBank.getAbsPath() + textName, false);

    // We may have an alpha part for additive.
    textName = tile->getRelativeFileName(CTile::alpha);
    if (!textName.empty())
      // Must Use clamp for alpha (although NVidia drivers are buggy), because
      // the texture doesn't tile at all
      pass.TextureAlpha =
          findTileTexture(TileBank.getAbsPath() + textName, true);
...
// ***************************************************************************
CPatchRdrPass *CLandscape::getTileRenderPass(uint16 tileId,
  ...
  //========================
  if (additiveRdrPass) {
    // NB: additive pass is not lighted by the lightmap, so there is no lighted
    // version of this rednerpass.
...
// ***************************************************************************
void CLandscape::getTileUvScaleBiasRot(uint16 tileId, CTile::TBitmap bitmapType,
                                       CVector &uvScaleBias, uint8 &rotAlpha) {
  CTileInfo *tile = TileInfos[tileId];
  // tile should not be NULL.
  // Because load of tiles are always done in getTileRenderPass(), and this
  // insertion always succeed.
  nlassert(tile);

  rotAlpha = 0;
  switch (bitmapType) {
  case CTile::diffuse:
    uvScaleBias = tile->DiffuseUvScaleBias;
    break;
  case CTile::additive:
    uvScaleBias = tile->AdditiveUvScaleBias;
    break;
  case CTile::alpha:
...
// ***************************************************************************
NLMISC::CSmartPtr<ITexture>
CLandscape::getTileTexture(uint16 tileId, CTile::TBitmap bitmapType,
                           CVector &uvScaleBias) {
  CPatchRdrPass *pass;
...
// ***************************************************************************
CTileElement *CLandscape::getTileElement(const CPatchIdent &patchId,
                                         const CUV &uv) {
  // \todo yoyo: TODO_ZONEID: change ZoneId in 32 bits...
  std::map<uint16, CZone *>::const_iterator it =
      Zones.find((uint16)patchId.ZoneId);
  if (it != Zones.end()) {
...
void CLandscape::flushTiles(IDriver *drv, uint32 tileStart, uint32 nbTiles) {
  ...
  for (sint tileId = tileStart; tileId < (sint)(tileStart + nbTiles);
       tileId++) {
    CTileInfo *tile = TileInfos[tileId];
    if (tile == NULL) {
...
void CLandscape::releaseTiles(uint32 tileStart, uint32 nbTiles) {
  ...
  // Textures are automaticly deleted by smartptr, but not their entry int the
  // map (TileTextureMap).
...
// ***************************************************************************
uint CLandscape::getTileLightMap(
  ...
  /*
    ...
    Why? : 2/ Unused near texture may be uncahced by opengl (and maybe by
    windows, to disk).

    (old reason, no longer valid, since lightmaps are unlinked from tiles.
      1/ There is an important issue with releasing texture nears: tiles may
    acces them (see getTileRenderPass())
    )
  */
...
// ***************************************************************************
void CLandscape::getTileLightMapUvInfo(uint tileLightMapId,
                                       CVector &uvScaleBias) {
  uint id, s, t;
  ...
  // But the real size of a minilightmap is 10 pixels, and we must reach the
  // pixel 1,1.
...
void CLandscape::releaseTileLightMap(uint tileLightMapId) {
  ...
  CTextureNear *nearText =
      (CTextureNear *)(ITexture *)nearRdrPass->TextureDiffuse;
...
// ***************************************************************************
void CLandscape::refillTileLightMap(
    uint tileLightMapId,
    CRGBA map[NL_TILE_LIGHTMAP_SIZE * NL_TILE_LIGHTMAP_SIZE]) {
  uint id, textNum;
...
// ***************************************************************************
CPatchRdrPass *CLandscape::getFarRenderPass(CPatch *pPatch, uint farIndex,
                                            float &farUScale, float &farVScale,
                                            float &farUBias, float &farVBias,
                                            bool &bRot) {
  // Check args
  nlassert(farIndex > 0);
  ...
  uint width =
      (pPatch->getOrderS() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >> (farIndex - 1);
  uint height =
      (pPatch->getOrderT() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >> (farIndex - 1);
  ...
  for (i = 0; i < _TextureFars.size(); i++) {
    CTextureFar *pTextureFar =
        (CTextureFar *)(&*(_TextureFars[i]->TextureDiffuse));
  ...
  pTextureFar->allocatePatch(pPatch, farIndex, farUScale, farVScale, farUBias,
                             farVBias, bRot);
...
// ***************************************************************************
void CLandscape::freeFarRenderPass(CPatch *pPatch, CPatchRdrPass *pass,
                                   uint farIndex) {
  // Get size of the far texture
  uint width =
      (pPatch->getOrderS() * NL_NUM_PIXELS_ON_FAR_TILE_EDGE) >> (farIndex - 1);
  ...
  for (sint i = 0; i < curZone.getNumPatchs(); i++) {
    ...
    for (sint j = 0; j < 4; j++) {
      ...
      if (bd.NPatchs == 1) {
        ...
        // If loaded zone.
        if (oZone) {
          const CZone::CPatchConnect &po =
              *(oZone->getPatchConnect(bd.Next[0]));
          const CPatchInfo::CBindInfo &bo = po.BindEdges[bd.Edge[0]];
          if (bo.NPatchs != 1 || bo.Next[0] != i || bo.Edge[0] != j)
            bindError.BindErrors.push_back(
                EBadBind::CBindError(curZone.getZoneId(), i));
        }
      }
    }
  }
...
// ***************************************************************************
void CLandscape::addTrianglesInBBox(const CPatchIdentEx &paIdEx,
                                    const CAABBox &bbox,
                                    std::vector<CTrianglePatch> &triangles,
                                    uint8 tileTessLevel) const {
...
// ***************************************************************************
void CLandscape::buildTrianglesInBBox(const CAABBox &bbox,
                                      std::vector<CTrianglePatch> &triangles,
                                      uint8 tileTessLevel) {
  // clear selection.
  triangles.clear();
...
// ***************************************************************************
void CLandscape::addPatchBlocksInBBox(
    const CPatchIdentEx &paIdEx, const CAABBox &bbox,
...
// ***************************************************************************
void CLandscape::buildPatchBlocksInBBox(
    const CAABBox &bbox, std::vector<CPatchBlockIdent> &paBlockIds) {
  // clear selection.
  paBlockIds.clear();
...
void CLandscape::fillPatchQuadBlock(CPatchQuadBlock &quadBlock) const {
  ...
  if (it != Zones.end()) {
    ...
    const CPatch *pa =
        const_cast<const CZone *>((*it).second)->getPatch(patchId);
    pa->fillPatchQuadBlock(quadBlock);
  }
}

// ***************************************************************************
void CLandscape::buildCollideFaces(const CAABBoxExt &bbox,
                                   vector<CTriangle> &faces, bool faceClip) {
...
// ***************************************************************************
void CLandscape::buildCollideFaces(sint zoneId, sint patch,
...
// ***************************************************************************
CVector CLandscape::getTesselatedPos(const CPatchIdent &patchId,
                                     const CUV &uv) const {
  // First, must update globals, for CTessFace::computeTesselatedPos() to work
  // properly.
  ...
  if (CLandscapeGlobals::VertexProgramEnabled) {
    /* because VertexProgram enabled, CTessVertex::Pos (geomorphed Pos) are not
       computed each frame Hence, CTessFace::computeTesselatedPos() will call
       CTessVertex::computeGeomPos() to have correct CTessVertex::Pos. ThereFore
       we must setup globals so CTessVertex::computeGeomPos() works properly.
    */

    // see copy in updateGlobalsAndLockBuffers(). NB: Just copy what needed
    // here!!!!
    ...
    CLandscapeGlobals::TileDistFarSqr = sqr(CLandscapeGlobals::TileDistFar);
    CLandscapeGlobals::OOTileDistDeltaSqr =
        1.0f / (CLandscapeGlobals::TileDistFarSqr -
  ...
  if (it != Zones.end()) {
    ...
    nlassert(patchId.PatchId < N);
    const CPatch *pa =
        const_cast<const CZone *>((*it).second)->getPatch(patchId.PatchId);
...
#define NL_TILE_FAR_SIZE_ORDER0                                                \
  (NL_NUM_PIXELS_ON_FAR_TILE_EDGE * NL_NUM_PIXELS_ON_FAR_TILE_EDGE)
#define NL_TILE_FAR_SIZE_ORDER1                                                \
  ((NL_NUM_PIXELS_ON_FAR_TILE_EDGE >> 1) *                                     \
   (NL_NUM_PIXELS_ON_FAR_TILE_EDGE >> 1))
#define NL_TILE_FAR_SIZE_ORDER2                                                \
  ((NL_NUM_PIXELS_ON_FAR_TILE_EDGE >> 2) *                                     \
   (NL_NUM_PIXELS_ON_FAR_TILE_EDGE >> 2))

// ***************************************************************************
// internal use
bool CLandscape::eraseTileFarIfNotGood(uint tileNumber, uint sizeOrder0,
                                       uint sizeOrder1, uint sizeOrder2) {
  // The same tiles ?
  bool bSame = true;
  ...
  if (TileFarBank.getTile(tileNumber)->isFill(CTileFarBank::diffuse)) {
    // Good diffuse size ?
    if ((TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::diffuse, CTileFarBank::order0) !=
         sizeOrder0) ||
        (TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::diffuse, CTileFarBank::order1) !=
         sizeOrder1) ||
        (TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::diffuse, CTileFarBank::order2) !=
         sizeOrder2)) {
  ...
  if (TileFarBank.getTile(tileNumber)->isFill(CTileFarBank::additive)) {
    // Good additive size ?
    if ((TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::additive, CTileFarBank::order0) !=
         sizeOrder0) ||
        (TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::additive, CTileFarBank::order1) !=
         sizeOrder1) ||
        (TileFarBank.getTile(tileNumber)
             ->getSize(CTileFarBank::additive, CTileFarBank::order2) !=
         sizeOrder2)) {
...
bool CLandscape::initTileBanks() {
  ...
  if (TileBank.getTileCount() == TileFarBank.getNumTile()) {
    ...
    for (int tileSet = 0; tileSet < TileBank.getTileSetCount(); tileSet++) {
      // Same tile128
      int tile;
      for (tile = 0; tile < TileBank.getTileSet(tileSet)->getNumTile128();
           tile++) {
        // tile number
        uint tileNumber = TileBank.getTileSet(tileSet)->getTile128(tile);

        // erase the tiles if not good
        bCompatibility &= eraseTileFarIfNotGood(
            tileNumber, NL_TILE_FAR_SIZE_ORDER0, NL_TILE_FAR_SIZE_ORDER1,
            NL_TILE_FAR_SIZE_ORDER2);
      }

      // Same tile256
      for (tile = 0; tile < TileBank.getTileSet(tileSet)->getNumTile256();
           tile++) {
        // tile number
        ...
        // erase the tiles if not good
        bCompatibility &= eraseTileFarIfNotGood(
            tileNumber, NL_TILE_FAR_SIZE_ORDER0 << 2,
            NL_TILE_FAR_SIZE_ORDER1 << 2, NL_TILE_FAR_SIZE_ORDER2 << 2);
      }
      ...
      for (tile = 0; tile < CTileSet::count; tile++) {
        // tile number
        uint tileNumber =
            TileBank.getTileSet(tileSet)->getTransition(tile)->getTile();
...
// ***************************************************************************
void CLandscape::setupStaticLight(const CRGBA &diffuse, const CRGBA &ambiant,
                                  float multiply) {
  ...
  for (int i = 0; i < 256; i++) {
    ...
    if (b > max)
      max = b;
    // Not << 8 but << 7 because the _LightValue color table represent a ramp
    // from 0 to 512
    r <<= 7;
    g <<= 7;
    b <<= 7;
...
CVector CLandscape::getHeightFieldDeltaZ(float x, float y) const {
  ...
  iy = (sint)floor(y);
  // out of the grid?
  if (ix < 0 || ix >= (sint)_HeightField.Width || iy < 0 ||
      iy >= (sint)_HeightField.Height)
    return CVector::Null;
...
float CLandscape::CBezierPatchZ::eval(float ps, float pt) const {
  ...
  p = Vertices[0] * s0 * t0 + Tangents[7] * s1 * t0 + Tangents[6] * s2 * t0 +
      Vertices[3] * s3 * t0;
  p += Tangents[0] * s0 * t1 + Interiors[0] * s1 * t1 + Interiors[3] * s2 * t1 +
       Tangents[5] * s3 * t1;
  p += Tangents[1] * s0 * t2 + Interiors[1] * s1 * t2 + Interiors[2] * s2 * t2 +
       Tangents[4] * s3 * t2;
  p += Vertices[1] * s0 * t3 + Tangents[2] * s1 * t3 + Tangents[3] * s2 * t3 +
       Vertices[2] * s3 * t3;
  ...
  for (y = 0; y < (sint)h; y++) {
    for (x = 0; x < (sint)w; x++) {
      ...
      static CDeltaPos deltas[8] = {
          {0, 0, 0, 1, 0, -1}, {0, 1, 0, 0, 0, 2},  {0, 1, 1, 1, -1, 1},
          {1, 1, 0, 1, 2, 1},  {1, 1, 1, 0, 1, 2},  {1, 0, 1, 1, 1, -1},
      ...
      for (tg = 0; tg < 8; tg++) {
        ...
        // borders case.
        if (x2 < 0 || x2 >= (sint)hf.getWidth() || y2 < 0 ||
            y2 >= (sint)hf.getHeight()) {
          float v, dv;
          // base of tangents.
          v = hf.getZ(x0, y0);
          // target tangents.
...
// ***************************************************************************
void CLandscape::getTessellationLeaves(
    std::vector<const CTessFace *> &leaves) const {
  leaves.clear();
...
void CLandscape::profileRender() {
  ...
  nlinfo("***** Landscape TextureFar Profile. NumTextureFar= %d",
  _TextureFars.size());
  ...
  {
      CPatchRdrPass	&pass= **itFar;
      CTextureFar *textureFar=
  safe_cast<CTextureFar*>((ITexture*)pass.TextureDiffuse);

      // Info
      uint	memUsage=
  textureFar->getPatchWidth()*textureFar->getPatchHeight()*NL_NUM_FAR_PATCHES_BY_TEXTURE*2;
      ...
      nlinfo("  * NumberOf Patch in the texture:%d",
  textureFar->getPatchCount());
      ...
      nlinfo("  * NumberOf Patch in frustum for this texture (Far0+Far1):%d",
  numRdrPatch);
  ...
  nlinfo("***** Landscape TextureFar Level Profile. TotalVideoMemory= %d",
  totalMemUsage); std::map<CVector2f, CTextureFarLevelInfo >::iterator
  itLevelFar= levelFarMap.begin(); while(itLevelFar!=levelFarMap.end())
  {
      nlinfo("  * Level PatchSize: (%d, %d). Total NumberOf Patch: %d. Use
  Percentage: %d %%", (uint)itLevelFar->first.x, (uint)itLevelFar->first.y,
  itLevelFar->second.NumUsedPatchs,
...
CTessFace *CLandscape::newTessFace() {
  ...
  // for refine() mgt, append the face to the list of newLeaves, so they will be
  // tested in refine()
...
// ***************************************************************************
CTileFace *CLandscape::newTileFace() { return TileFaceAllocator.allocate(); }

// ***************************************************************************
void CLandscape::deleteTessFace(CTessFace *f) {
  // for refine() mgt, must remove from refine priority list, or from the temp
  // rootTessFaceToUpdate list.
  f->unlinkInPList();
...
// ***************************************************************************
void CLandscape::setNoiseMode(bool enable) { _NoiseEnabled = enable; }

// ***************************************************************************
bool CLandscape::getNoiseMode() const { return _NoiseEnabled; }
...
void CLandscape::enableVegetable(bool enable) {
  ...
  if (!_VegetableManagerEnabled) {
    // Landscape always create ClipBlokcs, but IGs/addInstances() are created
    // only if isVegetableActive(). For all zones.
    for (ItZoneMap it = Zones.begin(); it != Zones.end(); it++) {
      // for all patch.
      sint N = (*it).second->getNumPatchs();
      for (sint i = 0; i < N; i++) {
...
// ***************************************************************************
void CLandscape::setupVegetableLighting(const CRGBA &ambient,
                                        const CRGBA &diffuse,
...
// ***************************************************************************
void CLandscape::setVegetableWind(const CVector &windDir, float windFreq,
...
// ***************************************************************************
void CLandscape::appendTileLightInfluences(
    const CPatchIdent &patchId, const CUV &uv,
...
void CLandscape::updateLightingTextureFar(float ratio) {
  ...
  if(YOYO_LandULTest)
  {
      nlinfo("YOYO_UL Far: %dK, %dK", (sint)_ULFarPixelsToUpdate/1024,
  (sint)_ULTotalFarPixels/1024);
  }*/
...
void CLandscape::updateLightingTextureNear(float ratio) {
  ...
  while (_ULNearPixelsToUpdate > 0 && _ULRootNearPatch) {
    ...
    _ULNearPixelsToUpdate -=
        _ULRootNearPatch->updateTessBlockLighting(_ULNearCurrentTessBlockId);
...
void CLandscape::computeDynamicLighting(const std::vector<CPointLight *> &pls) {
  ...
  for (i = 0; i < dlmPls.size(); i++) {
    ...
    for (it = _PatchQuadGrid.begin(); it != _PatchQuadGrid.end(); it++) {
      ...
      if (pa->getZone()
              ->getPatchBSphere(pa->getPatchId())
              .intersect(pl.BSphere)) {
...
// ***************************************************************************
void CLandscape::receiveShadowMap(IDriver *drv, CShadowMap *shadowMap,
  ...
  /*  substract the PZB from all coordinates.
      ...
      The rendered Triangles may be computed with VertexProgram, but
     _ShadowPolyReceiver does not. => there is a small float difference at the
     end Even if same vertex is produced in theory, VertexProgram may cause 2
     precision problems: 1/ On NVidia, even with a simple matrix mul VP, the
     precision result is not the same 2/ Our Landscape VP is not a simple matrix
     mul. Lot of vertex mul/add are done fpr geomorphs
  */
...
// ***************************************************************************
float CLandscape::getCameraCollision(const CVector &start, const CVector &end,
                                     float radius, bool cone) {
  return _ShadowPolyReceiver.getCameraCollision(
      start, end,
      cone ? CShadowPolyReceiver::CameraColCone
...
float CLandscape::getRayCollision(const CVector &start, const CVector &end) {
  return _ShadowPolyReceiver.getCameraCollision(
      start, end, CShadowPolyReceiver::CameraColSimpleRay, 0.f);
}

// ***************************************************************************
bool CLandscape::isTileCallback(ULandscapeTileCallback *cb) const {
  return std::find(_TileCallbacks.begin(), _TileCallbacks.end(), cb) !=
         _TileCallbacks.end();
}
...
void CLandscape::removeTileCallback(ULandscapeTileCallback *cb) {
  nlassert(cb);
  std::vector<ULandscapeTileCallback *>::iterator it =
      std::find(_TileCallbacks.begin(), _TileCallbacks.end(), cb);

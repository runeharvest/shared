...
// ***************************************************************************
/* LightQuadGrid setup. This is the same setup for StaticLightedModelQuadGrid
   setup. NB: with this setup, a light will lies into 4*4=16 squares of a
   quadGrid at max.
*/
...
// Factor for a level to the next: size/=factor, eltSize*=factor, and
// radiusLimit*=factor.
...
// this is used when the model is out of attBegin/attEnd, to modulate the
// influence
...
// ***************************************************************************
void CLightingManager::setLightTransitionThreshold(
    float lightTransitionThreshold) {
...
void CLightingManager::addDynamicLight(CPointLight *light) {
  ...
  // build the bounding sphere for this light, with the AttenuationEnd of the
  // light
  float radius = light->getAttenuationEnd();
  // if attenuation is disabled (ie getAttenuationEnd() return 0), then have a
  // dummy radius
  if (radius == 0)
  ...
  // Use the same level of _StaticLightedModelQuadGrid than me to not select too
  // many squares in the quadGrid
  _StaticLightedModelQuadGrid[qgId].select(bbmin, bbmax);
  // For all those models.
  CQuadGrid<CTransform *>::CIterator itModel =
      _StaticLightedModelQuadGrid[qgId].begin();
  while (itModel != _StaticLightedModelQuadGrid[qgId].end()) {
    ...
    if (plInfo.Sphere.include(modelPos)) {
      // yes, then this model must recompute his lighting, because a dynamic
      // light touch him.
      model->resetLighting();
    }
...
// ***************************************************************************
CLightingManager::CQGItLightedModel
...
// ***************************************************************************
CLightingManager::CQGItLightedModel
CLightingManager::insertStaticLightedModel(CTransform *model) {
  CQGItLightedModel ite;
  const CVector &worldPos = model->getWorldMatrix().getPos();

  // Insert the models in all levels, because addDynamicLight() may choose the
  // best suited level to select
  for (uint i = 0; i < NL3D_QUADGRID_LIGHT_NUM_LEVEL; i++) {
    ite.QgItes[i] =
        _StaticLightedModelQuadGrid[i].insert(worldPos, worldPos, model);
  }
...
// ***************************************************************************
void CLightingManager::computeModelLightContributions(
    NLMISC::CRGBA sunAmbient, CTransform *model,
    CLightContribution &lightContrib, ILogicInfo *logicInfo) {
  sint i;
  ...
  if (!lightContrib.FrozenStaticLightSetup) {
    ...
    if (!logicInfo) {
      ...
      logicInfo->getStaticLightSetup(sunAmbient, lightList,
                                     lightContrib.SunContribution,
                                     lightContrib.LocalAmbient);
    }
  }

  // Second pass, in the lightList, choose the best suited light to lit this
  // model
  //=========
  ...
  for (i = 0; i < (sint)lightList.size(); i++) {
    ...
    if (attEnd == 0) {
      ...
      if (distMinusRadius < attBegin) {
        ...
        // if < attBegin, inf should be ==1, but must select the nearest lights;
        // for better understanding of the scene
        inf = 1 + _OutOfAttLightInfFactor *
                      (attBegin - distMinusRadius); // inf E [1, +oo[
        ...
        // if >= attEnd, inf should be ==0, but must select the nearest lights;
        // for better understanding of the scene
        inf = _OutOfAttLightInfFactor *
              (attEnd - distMinusRadius); // inf E ]-oo, 0]
      }
    }
  ...
  if (startId < _MaxLightContribution) {
    ...
    // Any light under this minInfluence+deltaMinInfluence will be smoothly
    // darken.
    float minInfluenceStart = minInfluence + deltaMinInfluence;
    float OOdeltaMinInfluence = 1.0f / deltaMinInfluence;
    /* NB: this is not an error if we have only 3 light for example (assuming
       _MaxLightContribution=3), and still have minInfluenceStart>0. It's to
       have a continuity in the case of for example we have 3 lights in
       lightLists at frame T0, resulting in "doMerge=0"  and at frame T1 we have
       4 lights, the farthest having an influence of 0 or nearly 0.
    */
    ...
    for (i = startId; i < (sint)_MaxLightContribution; i++) {
      ...
      if (ligthSrcId >= lightList.size())
        ...
        if (inf >= minInfluenceStart) {
          // For Static LightSetup BiLinear to work correctly, modulate with
          // BkupInfluence don't worry about the precision of floor, because of
          // *255.
        ...
        // Compute the Final Att factor for models using Global Attenuation. NB:
        // modulate with Factor don't worry about the precision of floor,
        // because of *255. NB: compute att on the center of the model =>
        // modelRadius==0
        sint attFactor = NLMISC::OptFastFloor(
            lightContrib.Factor[i] *
            pl->computeLinearAttenuation(modelPos, distToModel));
    ...
    if (doMerge) {
      ...
      // For all lights in the lightList, merge Diffuse and Ambient term to
      // mergedAmbient.
      for (j = 0; j < lightToMergeWeight.size(); j++) {
        if (lightToMergeWeight[j] && lightList[j].Influence > 0) {
          ...
          // Attenuate the color of the light by biLinear and distance/spot
          // attenuation.
          float lightInf = attInf * bkupInf;
          // Modulate also by the percentage to merge
          lightInf *= lightToMergeWeight[j];
          // Add the full ambient term of the light, attenuated by light
          // attenuation and WeightMerge
          mergedAmbient.R += pl->getAmbient().R * lightInf;
          mergedAmbient.G += pl->getAmbient().G * lightInf;
          mergedAmbient.B += pl->getAmbient().B * lightInf;
          // Add 0.25f of the diffuse term of the light, attenuated by light
          // attenuation and WeightMerge mul by 0.25f, because this is the
          // averaged contribution of the diffuse part of a light to a sphere
          // (do the maths...).
      ...
      CRGBA amb;
      sint v;
      // Because of floating point error, it appears that sometime result may be
      // slightly below 0.
      // => clamp necessary
      v = NLMISC::OptFastFloor(mergedAmbient.R);
      fastClamp8(v);
      amb.R = v;
      v = NLMISC::OptFastFloor(mergedAmbient.G);
      fastClamp8(v);
      amb.G = v;
      v = NLMISC::OptFastFloor(mergedAmbient.B);
      fastClamp8(v);
      amb.B = v;
      amb.A = 255;
      lightContrib.MergedPointLight = amb;

      // Indicate we use the merged pointLight => the model must recompute
      // lighting each frame
      lightContrib.UseMergedPointLight = true;
    } else {
      // If the model is freezeHRC(), need to test each frame only if
      // MergedPointLight is used.
...
// ***************************************************************************
void CLightingManager::getDynamicPointLightList(
    const CVector &worldPos, std::vector<CPointLightInfluence> &lightList) {
  // For all quadGrids.

...
// ***************************************************************************
void CCameraCol::build(const CVector &start, const CVector &end, float radius,
                       bool cone) {
  // copy
  _Start = start;
  _End = end;
  ...
  _BBox.setHalfSize(_BBox.getHalfSize() +
                    CVector(_MaxRadius, _MaxRadius, _MaxRadius));
}
...
// ***************************************************************************
void CCameraCol::setApplyMatrix(const CCameraCol &other,
  ...
  if (matrix.hasScalePart()) {
    ...
    // Tricky code, deduce a uniform scale. Should not arise
      ...
      float meanScale =
          matrix.getI().norm() + matrix.getJ().norm() + matrix.getK().norm();
      meanScale /= 3;
      radius *= meanScale;
    }
  }
...
// ***************************************************************************
void CCameraCol::minimizeDistanceAgainstTri(const CVector &p0,
                                            const CVector &p1,
                                            const CVector &p2,
                                            float &sqrMinDist) {
  ...
  if (nVert) {
    /*
        Polygon nearest distance to a point is:
            ...
            - or the nearest distance to the plane (if the project lies in the
       polygon)
            - or the nearest distance to each edge

        NB: testing only points works with low radius, but may fails in general
       case
    */
    ...
    // **** get the nearest distance for all points (avoid precision problem if
    // doing only edge ones)
    sint i;
    for (i = 0; i < nVert; i++) {
    ...
    // need to do the poly inclusion test only if the plane dist is better than
    // the vertices
    ...
    // Camera Smooth mode. if the unmodulated distance is lower than the current
    // minDist, then this poly may be interesting, else don't have a chance
    else if (sqrPolyMinDist < sqrMinDist) {
      float sampleZSize = _RayLen / NL3D_CameraSmoothNumZSample;
      float sampleProjSize =
          2 * _MaxRadiusProj / NL3D_CameraSmoothNumAngleSample;
      ...
      for (sint tri = 0; tri < nVert - 2; tri++) {
        ...
        for (uint i = 0; i <= numSubdiv; i++) {
          for (uint j = 0; j <= numSubdiv - i; j++) {
            ...
            CVector sample =
                lp[0] * (float)i + lp[1] * (float)j + lp[2] * (float)k;
            ...
            sqrDist =
                _RayLen * angleFactor + sqrtf(sqrDist) * (1 - angleFactor);
            sqrDist = sqr(sqrDist);
...
// ***************************************************************************
void CCameraCol::intersectRay(const CVector &p0, const CVector &p1,
                              const CVector &p2, float &sqrMinDist) {
...
/*
            // Perspective Project each vertices on the plane normalized to the
   ray, at Start + rayNormed * 1
            // And then make local to the Point Start + rayNormed * 1 (ie
   _RayNorm since we are local to Start!). for(i=0;i<nVert;i++)
            {
                float	z= pIn[i] * _RayNorm;
                // cause of pyramid cliping, z should be >=0
                if(z>0)
            ...
            //	else this is the min distance of each edge/vertex against
   CVector::Null
            // test poly inclusion
            sint	sign= 0;
            for(i=0;i<nVert;i++)
            ...
            // if succeed, then the poly fully intersect the ray => just
   minimize if(i==nVert) sqrMinDist= sqrPolyMinDist;
            // else smooth distance!
            else
            {
                // must get min distance of each projected edge/vertex againt
   origin float	sqrMinRayDist= FLT_MAX;
                ...
                for(i=0;i<nVert;i++)
                {
                    ...
                    CVector	vDir= v1-v0;
                    // project on line
                    float	fLen= vDir.sqrnorm();	// same as  vDir * (v1 -
   v0)
                ...
                // compute the Cone Linear factor (like a spot light)
                float	angleFactor= (minRayDist-_MinRadiusProj) /
   (_MaxRadiusProj-_MinRadiusProj); clamp(angleFactor, 0.f, 1.f);
                // avoid C1 discontinuity when angleFactor==0
                angleFactor= sqr(angleFactor);

                // modulate, but to a bigger value! (ie raylen)
                sqrPolyMinDist= _RayLen * angleFactor + sqrtf(sqrPolyMinDist) *
   (1-angleFactor); sqrPolyMinDist= sqr(sqrPolyMinDist);

                // then if the modified distance is still lower than minDist,
   set if(sqrPolyMinDist<sqrMinDist) sqrMinDist= sqrPolyMinDist;
            }
*/
...
for(uint i=0;i<=numSubdiv;i++)
{
    for(uint j=0;j<=numSubdiv-i;j++)
    {
        ...
        // **** compute the Cone Linear factor (like a spot light)
        float	rayDist= proj.norm();
        float	angleFactor= (rayDist-_MinRadiusProj) /
(_MaxRadiusProj-_MinRadiusProj); clamp(angleFactor, 0.f, 1.f);
        // avoid C1 discontinuity when angleFactor==0
        angleFactor= sqr(angleFactor);

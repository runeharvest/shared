...
// 	symbolName[0] = '_';
// 	symbol = NULL;
// 	if (NSIsSymbolNameDefined (symbolName)) symbol = NSLookupAndBindSymbol
// (symbolName); 	free (symbolName); 	return symbol ? NSAddressOfSymbol (symbol) :
// NULL;
// }

// NSAddressOfSymbol, NSIsSymbolNameDefined, NSLookupAndBindSymbol are
// deprecated
#include <dlfcn.h>
void *nglGetProcAddress(const char *name) { return dlsym(RTLD_DEFAULT, name); }
...
// PFNGLPROGRAMSTRINGARBPROC nglProgramStringARB; PFNGLBINDPROGRAMARBPROC
// nglBindProgramARB; PFNGLDELETEPROGRAMSARBPROC
// nglDeleteProgramsARB; PFNGLGENPROGRAMSARBPROC
// nglGenProgramsARB; PFNGLPROGRAMENVPARAMETER4DARBPROC
// nglProgramEnvParameter4dARB; PFNGLPROGRAMENVPARAMETER4DVARBPROC
// nglProgramEnvParameter4dvARB; PFNGLPROGRAMENVPARAMETER4FARBPROC
// nglProgramEnvParameter4fARB; PFNGLPROGRAMENVPARAMETER4FVARBPROC
...
// nglGetProgramLocalParameterdvARB; PFNGLGETPROGRAMLOCALPARAMETERFVARBPROC
// nglGetProgramLocalParameterfvARB; PFNGLGETPROGRAMIVARBPROC
// nglGetProgramivARB; PFNGLGETPROGRAMSTRINGARBPROC
// nglGetProgramStringARB; PFNGLISPROGRAMARBPROC
// nglIsProgramARB;
...
#define CHECK_EXT(ext_str)                                                     \
  if (strstr(glext, ext_str) == NULL) {                                        \
    nlwarning("3D: OpengGL extension '%s' was not found", ext_str);            \
    return false;                                                              \
  } else {                                                                     \
    nldebug("3D: OpengGL Extension '%s' found", ext_str);                      \
  }

// Debug: don't return false if the procaddr returns 0
// It means that it can crash if nel calls this extension but at least we have a
// warning to know why the extension is available but not the procaddr
...
static bool setupEXTTextureEnvCombine(const char *glext) {
  ...
  return true;
#else
  return (strstr(glext, "GL_EXT_texture_env_combine") != NULL ||
          strstr(glext, "GL_ARB_texture_env_combine") != NULL);
#endif
}
...
static bool setupARBTextureCompression(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLCOMPRESSEDTEXSUBIMAGE3DARBPROC,
                glCompressedTexSubImage3DARB);
  CHECK_ADDRESS(PFNGLCOMPRESSEDTEXSUBIMAGE2DARBPROC,
                glCompressedTexSubImage2DARB);
  CHECK_ADDRESS(PFNGLCOMPRESSEDTEXSUBIMAGE1DARBPROC,
...
static bool setupNVTextureEnvCombine4(const char *glext) {
  ...
  // issue 310: disable extension to debug bug around
  // CDriverGL::setupSpecularPass()
  nlwarning("GL_NV_texture_env_combine4 disabled by request "
            "(DEBUG_OGL_COMBINE43_DISABLE)");
  return false;
#else
  return true;
#endif
...
static bool setupATITextureEnvCombine3(const char *glext) {
  ...
  // #ifdef NL_OS_MAC
  // // Water doesn't render on GeForce 8600M GT (on MAC OS X) if this extension
  // is enabled 	return false; #endif

  CHECK_EXT("GL_ATI_texture_env_combine3");
#ifdef DEBUG_OGL_COMBINE43_DISABLE
  // issue 310: disable extension to debug bug around
  // CDriverGL::setupSpecularPass()
  nlwarning("GL_ATI_texture_env_combine3 disabled by request "
            "(DEBUG_OGL_COMBINE43_DISABLE)");
  return false;
#else
  return true;
...
static bool setupNVVertexProgram(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLGETVERTEXATTRIBPOINTERVNVPROC,
  ...
  CHECK_ADDRESS(PFNGLREQUESTRESIDENTPROGRAMSNVPROC,
...
static bool setupEXTVertexShader(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLENABLEVARIANTCLIENTSTATEEXTPROC,
                glEnableVariantClientStateEXT);
  CHECK_ADDRESS(PFNGLDISABLEVARIANTCLIENTSTATEEXTPROC,
  ...
  CHECK_ADDRESS(PFNGLBINDTEXTUREUNITPARAMETEREXTPROC,
  ...
  CHECK_ADDRESS(PFNGLGETLOCALCONSTANTBOOLEANVEXTPROC,
                glGetLocalConstantBooleanvEXT);
  CHECK_ADDRESS(PFNGLGETLOCALCONSTANTINTEGERVEXTPROC,
                glGetLocalConstantIntegervEXT);
  CHECK_ADDRESS(PFNGLGETLOCALCONSTANTFLOATVEXTPROC,
                glGetLocalConstantFloatvEXT);

  // we require at least 128 instructions, 15 local register (r0, r1,..,r11) + 3
  // temporary vector for swizzle emulation + 1 vector for indexing temp + 3
  // temporary scalar for LOGG, EXPP and LIT emulation,  1 address register we
  // require 11 variants (4 textures + position + normal + primary color +
  // secondary color + weight + palette skin + fog) we also require 2 local
  // constants (0 and 1) 96 invariants (c[0], c[1] ..) + 1 invariants for fog
  // emulation (fog coordinate must range from 0 to 1 with EXT_VERTEX_shader)
  GLint numVSInst;
  ...
  if (numVSLocals < 4 * (12 + 4) + 1 + 3) {
    nlwarning("EXT_vertex_shader extension has not much register. Some vertex "
              "program may fail loading");
    return false;
  }
  //
  GLint numVSLocalConstants;
  glGetIntegerv(GL_MAX_VERTEX_SHADER_LOCAL_CONSTANTS_EXT, &numVSLocalConstants);
  if (numVSLocalConstants < 2)
    return false;
  //
  GLint numVSInvariants;
  glGetIntegerv(GL_MAX_OPTIMIZED_VERTEX_SHADER_INVARIANTS_EXT,
                &numVSInvariants);
  if (numVSInvariants < 96 + 1)
    return false;
  //
  GLint numVSVariants;
  glGetIntegerv(GL_MAX_VERTEX_SHADER_VARIANTS_EXT, &numVSVariants);
  if (numVSInvariants < 4)
    return false;
#endif
...
#ifndef USE_OPENGLES
  CHECK_ADDRESS(PFNWGLGETPIXELFORMATATTRIBIVARBPROC,
                wglGetPixelFormatAttribivARB);
  CHECK_ADDRESS(PFNWGLGETPIXELFORMATATTRIBFVARBPROC,
...
static bool setupATIVertexArrayObject(const char *glext) {
  ...
  if (strstr(glext, "GL_EXT_vertex_shader") != NULL) {
    ...
    CHECK_ADDRESS(PFNGLGETVARIANTARRAYOBJECTFVATIPROC,
...
static bool setupATIFragmentShader(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLSETFRAGMENTSHADERCONSTANTATIPROC,
...
#ifndef USE_OPENGLES
  CHECK_ADDRESS(PFNGLVERTEXATTRIBARRAYOBJECTATIPROC,
                glVertexAttribArrayObjectATI);
  CHECK_ADDRESS(PFNGLGETVERTEXATTRIBARRAYOBJECTFVATIPROC,
                glGetVertexAttribArrayObjectfvATI);
  CHECK_ADDRESS(PFNGLGETVERTEXATTRIBARRAYOBJECTIVATIPROC,
...
static bool setupARBFragmentProgram(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLPROGRAMENVPARAMETER4DVARBPROC,
                glProgramEnvParameter4dvARB);
  CHECK_ADDRESS(PFNGLPROGRAMENVPARAMETER4FARBPROC, glProgramEnvParameter4fARB);
  CHECK_ADDRESS(PFNGLPROGRAMENVPARAMETER4FVARBPROC,
                glProgramEnvParameter4fvARB);
  CHECK_ADDRESS(PFNGLPROGRAMLOCALPARAMETER4DARBPROC,
                glProgramLocalParameter4dARB);
  CHECK_ADDRESS(PFNGLPROGRAMLOCALPARAMETER4DVARBPROC,
                glProgramLocalParameter4dvARB);
  CHECK_ADDRESS(PFNGLPROGRAMLOCALPARAMETER4FARBPROC,
                glProgramLocalParameter4fARB);
  CHECK_ADDRESS(PFNGLPROGRAMLOCALPARAMETER4FVARBPROC,
                glProgramLocalParameter4fvARB);
  CHECK_ADDRESS(PFNGLGETPROGRAMENVPARAMETERDVARBPROC,
                glGetProgramEnvParameterdvARB);
  CHECK_ADDRESS(PFNGLGETPROGRAMENVPARAMETERFVARBPROC,
                glGetProgramEnvParameterfvARB);
  CHECK_ADDRESS(PFNGLGETPROGRAMLOCALPARAMETERDVARBPROC,
                glGetProgramLocalParameterdvARB);
  CHECK_ADDRESS(PFNGLGETPROGRAMLOCALPARAMETERFVARBPROC,
...
static bool setupARBVertexProgram(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLENABLEVERTEXATTRIBARRAYARBPROC,
                glEnableVertexAttribArrayARB);
  CHECK_ADDRESS(PFNGLDISABLEVERTEXATTRIBARRAYARBPROC,
  ...
  CHECK_ADDRESS(PFNGLGETVERTEXATTRIBPOINTERVARBPROC,
...
static bool setupFrameBufferObject(const char *glext) {
  ...
  CHECK_ADDRESS(PFNGLGETRENDERBUFFERPARAMETERIVOESPROC,
  ...
  CHECK_ADDRESS(PFNGLCHECKFRAMEBUFFERSTATUSOESPROC,
                glCheckFramebufferStatusOES);
  CHECK_ADDRESS(PFNGLFRAMEBUFFERRENDERBUFFEROESPROC,
                glFramebufferRenderbufferOES);
  CHECK_ADDRESS(PFNGLFRAMEBUFFERTEXTURE2DOESPROC, glFramebufferTexture2DOES);
  CHECK_ADDRESS(PFNGLGETFRAMEBUFFERATTACHMENTPARAMETERIVOESPROC,
  ...
  CHECK_ADDRESS(PFNGLCHECKFRAMEBUFFERSTATUSEXTPROC,
  ...
  CHECK_ADDRESS(PFNGLFRAMEBUFFERRENDERBUFFEREXTPROC,
  ...
  CHECK_ADDRESS(PFNGLGETRENDERBUFFERPARAMETERIVEXTPROC,
...
#ifndef USE_OPENGLES
  CHECK_ADDRESS(PFNGLRENDERBUFFERSTORAGEMULTISAMPLEEXTPROC,
...
void registerGlExtensions(CGlExtensions &ext) {
  ...
  if (nglVersion) {
    ...
    nlwarning("3D: Unable to get GL_VERSION, OpenGL 1.2 should be supported on "
              "all recent GPU...");
    ext.Version1_2 = true;
  }
  ...
  if (ext.ARBMultiTexture) {
    ...
    ext.NbTextureStages =
        (ntext < ((GLint)IDRV_MAT_MAXTEXTURES) ? ntext : IDRV_MAT_MAXTEXTURES);
  }
  ...
  ext.EXTTextureCompressionS3TC =
      (ext.ARBTextureCompression && setupEXTTextureCompressionS3TC(glext));
  ...
  // For now, the only way to know if emulation, is to test some extension which
  // exist only on GeForce3. if GL_NV_texture_shader is not here, then we are
  // not on GeForce3.
  ext.NVVertexProgramEmulated =
      ext.NVVertexProgram && (strstr(glext, "GL_NV_texture_shader") == NULL);
  ...
  if (ext.EXTTextureFilterAnisotropic) {
    ...
    glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT,
                &ext.EXTTextureFilterAnisotropicMaximum);
  }
  ...
  if (ext.NbTextureStages == 3 &&
      (ext.ATIVertexArrayObject || ext.ARBVertexBufferObject)) {
...
static bool setupWGLAMDGPUAssociation(const char *glext) {
  ...
  CHECK_ADDRESS(PFNWGLCREATEASSOCIATEDCONTEXTAMDPROC,
                wglCreateAssociatedContextAMD);
  CHECK_ADDRESS(PFNWGLCREATEASSOCIATEDCONTEXTATTRIBSAMDPROC,
                wglCreateAssociatedContextAttribsAMD);
  CHECK_ADDRESS(PFNWGLDELETEASSOCIATEDCONTEXTAMDPROC,
                wglDeleteAssociatedContextAMD);
  CHECK_ADDRESS(PFNWGLMAKEASSOCIATEDCONTEXTCURRENTAMDPROC,
                wglMakeAssociatedContextCurrentAMD);
  CHECK_ADDRESS(PFNWGLGETCURRENTASSOCIATEDCONTEXTAMDPROC,
                wglGetCurrentAssociatedContextAMD);
  CHECK_ADDRESS(PFNWGLBLITCONTEXTFRAMEBUFFERAMDPROC,
...
static bool setupWGLNVGPUAssociation(const char *glext) {
  ...
  CHECK_ADDRESS(PFNWGLENUMGPUSFROMAFFINITYDCNVPROC,
...
#if defined(NL_OS_UNIX) && !defined(NL_OS_MAC)
  CHECK_ADDRESS(PFNGLXQUERYCURRENTRENDERERINTEGERMESAPROC,
  ...
  if (ext.WGLNVGPUAffinity) {
    ...
    while (nwglEnumGpusNV(gpuIndex, &hGPU)) {
      ...
      while (nwglEnumGpuDevicesNV(hGPU, j, &gpuDevice)) {
        nlinfo("Device: %s / %s / flags: %u", gpuDevice.DeviceName,
               gpuDevice.DeviceString, (uint)gpuDevice.Flags);

        if (gpuDevice.Flags & DISPLAY_DEVICE_ATTACHED_TO_DESKTOP) {
          nlinfo("Virtual screen: (%d,%d)-(%d,%d)",
                 (sint)gpuDevice.rcVirtualScreen.left,
                 (sint)gpuDevice.rcVirtualScreen.top,
                 (sint)gpuDevice.rcVirtualScreen.right,
                 (sint)gpuDevice.rcVirtualScreen.bottom);
        }

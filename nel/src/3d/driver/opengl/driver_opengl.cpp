...
IDriver *createGlEsDriverInstance() { return new NLDRIVERGLES::CDriverGL; }

#else

IDriver *createGlDriverInstance() { return new NLDRIVERGL::CDriverGL; }
...
extern "C" {
IDriver *NL3D_createIDriverInstance() { return new CDriverGL; }

uint32 NL3D_interfaceVersion() { return IDriver::InterfaceVersion; }
}
...
bool CDriverGL::setupDisplay() {
  ...
  if (!_Extensions.ARBMultiTexture) {
    nlwarning("Missing Required GL extension: GL_ARB_multitexture. Update your "
              "driver");
  ...
  if (!_Extensions.EXTTextureEnvCombine) {
    nlwarning("Missing Important GL extension: GL_EXT_texture_env_combine => "
              "All envcombine are setup to GL_MODULATE!!!");
  }
  ...
  _DriverGLStates.init(_Extensions.ARBTextureCubeMap,
                       (_Extensions.NVTextureRectangle ||
                        _Extensions.EXTTextureRectangle ||
                        _Extensions.ARBTextureRectangle),
                       _MaxDriverLight);
...
#ifndef USE_OPENGLES
    glLightModeli((GLenum)GL_LIGHT_MODEL_COLOR_CONTROL_EXT,
  ...
  if (_Extensions.ARBVertexBufferObject) {
    ...
    _MaxVerticesByVBHard =
        std::numeric_limits<uint32>::max(); // cant' know the value..
  }
#ifndef USE_OPENGLES
  // Next with NVidia ext
    ...
    _MaxVerticesByVBHard = _Extensions.NVVertexArrayRangeMaxVertex;
             ...
             _Extensions.ATIVertexArrayObject) {
    // NB
    // on Radeon 9200 and below : ATI_vertex_array_object is better (no direct
    // access to AGP with ARB_vertex_buffer_object -> slow unlock) on Radeon
    // 9500 and above : ARB_vertex_buffer_object is better
  ...
  if (_SupportVBHard) {
    ...
    // If not success to allocate at least a minimum space in AGP, then disable
    // completely VBHard feature
  ...
  for (uint stage = 0; stage < inlGetNumTextStages(); stage++) {
    ...
    // set All TexGen by default to identity matrix (prefer use the
    // textureMatrix scheme)
...
#ifndef USE_OPENGLES
  // if EXTVertexShader is used, bind the standard GL arrays, and allocate
  // constant
  if (!_Extensions.NVVertexProgram && !_Extensions.ARBVertexProgram &&
      _Extensions.EXTVertexShader) {
    ...
    if (!_EVSPositionHandle || !_EVSNormalHandle || !_EVSColorHandle) {
      nlwarning("Unable to bind input parameters for use with "
                "EXT_vertex_shader, vertex program support is disabled");
      _Extensions.EXTVertexShader = false;
    } else {
      // bind texture units
      for (uint k = 0; k < 8; ++k) {
        _EVSTexHandle[k] = nglBindTextureUnitParameterEXT(
            GL_TEXTURE0_ARB + k, GL_CURRENT_TEXTURE_COORDS);
      }
      ...
      // Allocate invariants. One assitionnal variant is needed for fog
      // coordinate if fog bug is not fixed in driver version
      _EVSConstantHandle =
          nglGenSymbolsEXT(GL_VECTOR_EXT, GL_INVARIANT_EXT, GL_FULL_RANGE_EXT,
                           _EVSNumConstant + (_ATIFogRangeFixed ? 0 : 1));

      if (_EVSConstantHandle == 0) {
        nlwarning("Unable to allocate constants for EXT_vertex_shader, vertex "
                  "program support is disabled");
        _Extensions.EXTVertexShader = false;
      }
    }
  }
...
// ***************************************************************************
bool CDriverGL::stretchRect(ITexture * /* srcText */,
...
bool CDriverGL::supportBloomEffect() const {
  return (supportVertexProgram(CVertexProgram::nelvp) &&
          supportFrameBufferObject() && supportPackedDepthStencil() &&
...
bool CDriverGL::isTextureRectangle(ITexture *tex) const {
  return (!supportNonPowerOfTwoTextures() && supportTextureRectangle() &&
          tex->isBloomTexture() && tex->mipMapOff() &&
  ...
  if (supportFrameBufferObject() /* && supportPackedDepthStencil()*/) {
    if (tex) {
      CTextureDrvInfosGL *gltext =
          (CTextureDrvInfosGL *)(ITextureDrvInfos *)(tex->TextureDrvShare
                                                         ->DrvTexture);
...
bool CDriverGL::clear2D(CRGBA rgba) {
  ...
  glClearColor((float)rgba.R / 255.0f, (float)rgba.G / 255.0f,
               (float)rgba.B / 255.0f, (float)rgba.A / 255.0f);
...
#ifndef USE_OPENGLES
  /* Yoyo: must do this (GeForce bug ??) else weird results if end render with a
     VBHard. Setup a std vertex buffer to ensure NVidia synchronisation.
  */
  if (!_Extensions.ARBVertexBufferObject && _Extensions.NVVertexArrayRange) {
    ...
    //	NVidia: This also force a SetFence on if last VB was a VBHard, "closing"
    //it before swap.
    //
  ...
  /*set<IVertexBufferHardGL*>::iterator		itVBHard=
  _VertexBufferHardSet.Set.begin(); while(itVBHard !=
  _VertexBufferHardSet.Set.end() )
  {
      // Need only to do it for NVidia VB ones.
      if((*itVBHard)->NVidiaVertexBufferHard)
      {
          CVertexBufferHardGLNVidia	*vbHardNV=
  static_cast<CVertexBufferHardGLNVidia*>(*itVBHard);
  ...
  /* Need to Do this code only if Synchronisation PATCH before not done!
      AS NV_Fence GeForce Implementation says. Test each frame the NVFence,
     until completion. NB: finish is not required here. Just test. This is like
     a "non block synchronisation"
   */
  if (!_Extensions.ARBVertexBufferObject && _Extensions.NVVertexArrayRange) {
    set<IVertexBufferHardGL *>::iterator itVBHard =
        _VertexBufferHardSet.Set.begin();
  ...
  if (!_WndActive) {
    if (_AGPVertexArrayRange)
      _AGPVertexArrayRange->updateLostBuffers();
    if (_VRAMVertexArrayRange)
      _VRAMVertexArrayRange->updateLostBuffers();
  }
  ...
  // on ati, if the window is inactive, check all vertex buffer to see which one
  // are lost
  if (_AGPVertexArrayRange)
    _AGPVertexArrayRange->updateLostBuffers();
  if (_VRAMVertexArrayRange)
...
sint CDriverGL::getTotalVideoMemory() const {
  ...
  if (_Extensions.NVXGPUMemoryInfo) {
    ...
    nlinfo("3D: GL_GPU_MEMORY_INFO_DEDICATED_VIDMEM_NVX returned %d KiB",
           memoryInKiB);
    return memoryInKiB;
  }
  ...
  if (_Extensions.WGLAMDGPUAssociation) {
    ...
    nwglGetGPUInfoAMD(uGPUIDs[0], WGL_GPU_RAM_AMD, GL_UNSIGNED_INT,
                      sizeof(GLuint), &memoryInMiB);
  ...
  CGLError error = CGLGetParameter([_ctx CGLContextObj],
                                   kCGLCPCurrentRendererID, &rendererID);
  ...
  if (error == kCGLNoError) {
      ...
      for (GLint i = 0; i < nrend; ++i) {
        ...
        if (error == kCGLNoError) {
          ...
          if (thisRendererID == rendererID) {
            GLint memoryInMiB = 0;
            CGLError error = CGLDescribeRenderer(
                rend, i, kCGLRPVideoMemoryMegabytes, &memoryInMiB);

            if (error == kCGLNoError) {
              // convert in KiB
              return memoryInMiB * 1024;
            } else {
              nlwarning("3D: Unable to get video memory (%s)",
                        CGLErrorString(error));
            }
          }
        } else {
          nlwarning("3D: Unable to get renderer ID (%s)",
                    CGLErrorString(error));
        }
      }
    ...
    nlerror("3D: Unable to get current renderer ID (%s)",
            CGLErrorString(error));
  }
#else
  if (_Extensions.GLXMESAQueryRenderer) {
    ...
    if (nglXQueryCurrentRendererIntegerMESA(GLX_RENDERER_VIDEO_MEMORY_MESA,
                                            &memoryInMiB)) {
...
void CDriverGL::getBufferPart(CBitmap &bitmap, NLMISC::CRect &rect) {
  ...
  if (clipRect(rect)) {
    ...
    glReadPixels(rect.X, rect.Y, rect.Width, rect.Height, GL_RGBA,
                 GL_UNSIGNED_BYTE, bitmap.getPixels().getPtr());
  }
}

void CDriverGL::getZBufferPart(std::vector<float> &zbuffer,
                               NLMISC::CRect &rect) {
  ...
  if (clipRect(rect)) {
    ...
    glReadPixels(rect.X, rect.Y, rect.Width, rect.Height,
                 GL_DEPTH_COMPONENT16_OES, GL_FLOAT, &(zbuffer[0]));
#else
    glPixelTransferf(GL_DEPTH_SCALE, 1.0f);
    glPixelTransferf(GL_DEPTH_BIAS, 0.f);
    glReadPixels(rect.X, rect.Y, rect.Width, rect.Height, GL_DEPTH_COMPONENT,
                 GL_FLOAT, &(zbuffer[0]));
#endif
  }
}
...
bool CDriverGL::fillBuffer(CBitmap &bitmap) {
  ...
  if (rect.Width != bitmap.getWidth() || rect.Height != bitmap.getHeight() ||
      bitmap.getPixelFormat() != CBitmap::RGBA)
    return false;
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, rect.Width, rect.Height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, &(bitmap.getPixels()[0]));
  //	glTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_CROP_RECT_OES,
  //smBackgroundCrop,0);
  nglDrawTexfOES(0.f, 0.f, 0.f, 1.f, 1.f);
#else
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glDrawPixels(rect.Width, rect.Height, GL_RGBA, GL_UNSIGNED_BYTE,
               &(bitmap.getPixels()[0]));
#endif
...
void CDriverGL::copyFrameBufferToTexture(ITexture *tex, uint32 level,
                                         ...
                                         uint32 height, uint cubeFace /*= 0*/
) {
  H_AUTO_OGL(CDriverGL_copyFrameBufferToTexture)
  bool compressed = false;
  getGlTextureFormat(*tex, compressed);
  nlassert(!compressed);
  // first, mark the texture as valid, and make sure there is a corresponding
  // texture in the device memory
  setupTexture(*tex);
  CTextureDrvInfosGL *gltext =
      (CTextureDrvInfosGL *)(ITextureDrvInfos *)(tex->TextureDrvShare
                                                     ->DrvTexture);
  ...
  if (tex->isTextureCube()) {
    if (_Extensions.ARBTextureCubeMap) {
      ...
      glCopyTexSubImage2D(NLCubeFaceToGLCubeFace[cubeFace], level, offsetx,
                          offsety, x, y, width, height);
    }
  } else {
    glBindTexture(gltext->TextureMode, gltext->ID);
    glCopyTexSubImage2D(gltext->TextureMode, level, offsetx, offsety, x, y,
                        width, height);
  }
  // disable texturing.
...
#ifndef USE_OPENGLES
  /** Special : with vertex program, using the extension EXT_vertex_shader, fog
   * is emulated using 1 more constant to scale result to [0, 1]
   */
  if (_Extensions.EXTVertexShader && !_Extensions.NVVertexProgram &&
      !_Extensions.ARBVertexProgram) {
    if (!_ATIFogRangeFixed) {
      // last constant is used to store fog information (fog must be rescaled to
      // [0, 1], because of a driver bug)
      if (start != end) {
        float datas[] = {1.f / (start - end), -end / (start - end), 0, 0};
        nglSetInvariantEXT(_EVSConstantHandle + _EVSNumConstant, GL_FLOAT,
                           datas);
      } else {
...
// ***************************************************************************
void CDriverGL::profileRenderedPrimitives(CPrimitiveProfile &pIn,
...
void CDriverGL::enableUsedTextureMemorySum(bool enable) {
  ...
  if (enable) {
    nlinfo("3D: PERFORMANCE INFO: enableUsedTextureMemorySum has been set to "
           "true in CDriverGL");
...
bool CDriverGL::supportWaterShader() const {
  ...
  if (_Extensions.ARBFragmentProgram && ARBWaterShader[0] != 0)
    return true;

  if (!_Extensions.EXTVertexShader && !_Extensions.NVVertexProgram &&
      !_Extensions.ARBVertexProgram)
    return false; // should support vertex programs
  if (!_Extensions.NVTextureShader && !_Extensions.ATIFragmentShader &&
      !_Extensions.ARBFragmentProgram)
    return false;
  return true;
}

// ***************************************************************************
bool CDriverGL::supportTextureAddrMode(
    CMaterial::TTexAddressingMode /* mode */) const {
...
// ***************************************************************************
void CDriverGL::setMatrix2DForTextureOffsetAddrMode(const uint stage,
                                                    const float mat[4]) {
  H_AUTO_OGL(CDriverGL_setMatrix2DForTextureOffsetAddrMode)

  if (!supportTextureShaders())
    return;
...
void CDriverGL::checkForPerPixelLightingSupport() {
  ...
  // we need at least 3 texture stages and cube map support + EnvCombine4 or 3
  // support
  // TODO : support for EnvCombine3
  // TODO : support for less than 3 stages

  _SupportPerPixelShaderNoSpec =
      (_Extensions.NVTextureEnvCombine4 || _Extensions.ATITextureEnvCombine3) &&
      _Extensions.ARBTextureCubeMap && _Extensions.NbTextureStages >= 3 &&
      (_Extensions.NVVertexProgram || _Extensions.ARBVertexProgram ||
       _Extensions.EXTVertexShader);

  _SupportPerPixelShader =
      (_Extensions.NVTextureEnvCombine4 || _Extensions.ATITextureEnvCombine3) &&
      _Extensions.ARBTextureCubeMap && _Extensions.NbTextureStages >= 2 &&
      (_Extensions.NVVertexProgram || _Extensions.ARBVertexProgram ||
       _Extensions.EXTVertexShader);
}
...
// ***************************************************************************
void CDriverGL::setPerPixelLightingLight(CRGBA diffuse, CRGBA specular,
                                         float shininess) {
...
#ifndef USE_OPENGLES
  if (supportEMBM()) {
    std::fill(_StageSupportEMBM, _StageSupportEMBM + IDRV_MAT_MAXTEXTURES,
              false);
    if (_Extensions.ATIEnvMapBumpMap) {
      ...
      // setup each stage to apply the bump map to the next stage (or previous
      // if there's an unit at the last stage)
...
// **************************************************************************************
/** Water fragment program with extension ARB_fragment_program and a diffuse map
 * applied
 */
...
uint loadARBFragmentProgramStringNative(const char *prog,
  ...
  nglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                      (GLsizei)strlen(prog), prog);
  nglBindProgramARB(GL_FRAGMENT_PROGRAM_ARB, 0);
  glGetIntegerv(GL_PROGRAM_ERROR_POSITION_ARB, &errorPos);
  nglGetProgramivARB(GL_FRAGMENT_PROGRAM_ARB,
  ...
  if (errorPos == -1) {
    ...
    nlwarning("init fragment program failed: errorPos: %d isNative: %d: %s",
              errorPos, isNative,
 ...
 * Send fragment shader to fetch a perturbed envmap from the addition of 2
 * bumpmap The result is in R2 after the 2nd pass
 */
static void fetchPerturbedEnvMapR200() {
  ...
  // PASS 1 //
  ////////////
  nglSampleMapATI(GL_REG_0_ATI, GL_TEXTURE0_ARB,
                  GL_SWIZZLE_STR_ATI); // sample bump map 0
  nglSampleMapATI(GL_REG_1_ATI, GL_TEXTURE1_ARB,
                  GL_SWIZZLE_STR_ATI); // sample bump map 1
  nglPassTexCoordATI(GL_REG_2_ATI, GL_TEXTURE2_ARB,
                     GL_SWIZZLE_STR_ATI); // get texcoord for envmap

  nglColorFragmentOp3ATI(GL_MAD_ATI, GL_REG_2_ATI, GL_NONE, GL_NONE,
                         GL_REG_0_ATI, GL_NONE, GL_BIAS_BIT_ATI | GL_2X_BIT_ATI,
                         GL_CON_0_ATI, GL_NONE, GL_NONE, GL_REG_2_ATI, GL_NONE,
                         GL_NONE); // scale bumpmap 1 & add envmap coords
  nglColorFragmentOp3ATI(GL_MAD_ATI, GL_REG_2_ATI, GL_NONE, GL_NONE,
                         GL_REG_1_ATI, GL_NONE, GL_BIAS_BIT_ATI | GL_2X_BIT_ATI,
                         GL_CON_1_ATI, GL_NONE, GL_NONE, GL_REG_2_ATI, GL_NONE,
                         GL_NONE); // scale bumpmap 2 & add to bump map 1

  ////////////
  // PASS 2 //
  ////////////
  nglSampleMapATI(GL_REG_2_ATI, GL_REG_2_ATI,
                  GL_SWIZZLE_STR_ATI); // fetch envmap at perturbed texcoords
#endif
}
...
void CDriverGL::initFragmentShaders() {
  ...
  if (_Extensions.ARBFragmentProgram) {
    ...
    ARBWaterShader[0] = loadARBFragmentProgramStringNative(
        WaterCodeNoDiffuseForARBFragmentProgram, _ForceNativeFragmentPrograms);
    ARBWaterShader[1] = loadARBFragmentProgramStringNative(
        WaterCodeNoDiffuseWithFogForARBFragmentProgram,
        _ForceNativeFragmentPrograms);
    ARBWaterShader[2] = loadARBFragmentProgramStringNative(
        WaterCodeForARBFragmentProgram, _ForceNativeFragmentPrograms);
    ARBWaterShader[3] = loadARBFragmentProgramStringNative(
        WaterCodeWithFogForARBFragmentProgram, _ForceNativeFragmentPrograms);
    bool ok = true;
    for (uint k = 0; k < 4; ++k) {
      if (!ARBWaterShader[k]) {
        ok = false;
        deleteARBFragmentPrograms();
        nlwarning("WATER: fragment %d is not loaded, not using "
                  "ARB_fragment_program at all",
                  k);
        break;
      }
    }
  ...
  if (_Extensions.ATIFragmentShader) {
    ...
    if (!ATIWaterShaderHandle || !ATIWaterShaderHandleNoDiffuseMap) {
      ...
      glGetError();
      // Water shader for R200 : we just add the 2 bump map contributions (du,
      // dv). We then use this contribution to perturbate the envmap
      nglBindFragmentShaderATI(ATIWaterShaderHandleNoDiffuseMap);
      nglBeginFragmentShaderATI();
      //
      fetchPerturbedEnvMapR200();
      nglColorFragmentOp1ATI(GL_MOV_ATI, GL_REG_0_ATI, GL_NONE, GL_NONE,
                             GL_REG_2_ATI, GL_NONE, GL_NONE);
      nglAlphaFragmentOp1ATI(GL_MOV_ATI, GL_REG_0_ATI, GL_NONE, GL_REG_2_ATI,
                             GL_NONE, GL_NONE);
      //
      ...
      nglSampleMapATI(
          GL_REG_3_ATI, GL_TEXTURE3_ARB,
          GL_SWIZZLE_STR_ATI); // fetch envmap at perturbed texcoords
      nglColorFragmentOp2ATI(GL_MUL_ATI, GL_REG_0_ATI, GL_NONE, GL_NONE,
                             GL_REG_3_ATI, GL_NONE, GL_NONE, GL_REG_2_ATI,
                             GL_NONE,
                             GL_NONE); // scale bumpmap 1 & add envmap coords
      nglAlphaFragmentOp2ATI(GL_MUL_ATI, GL_REG_0_ATI, GL_NONE, GL_REG_3_ATI,
                             GL_NONE, GL_NONE, GL_REG_2_ATI, GL_NONE, GL_NONE);
    ...
    if (!ATICloudShaderHandle) {
      ...
      //
      nglSampleMapATI(GL_REG_0_ATI, GL_TEXTURE0_ARB,
                      GL_SWIZZLE_STR_ATI); // sample texture 0
      nglSampleMapATI(GL_REG_1_ATI, GL_TEXTURE1_ARB,
                      GL_SWIZZLE_STR_ATI); // sample texture 1
      // lerp between tex 0 & tex 1 using diffuse alpha
      nglAlphaFragmentOp3ATI(GL_LERP_ATI, GL_REG_0_ATI, GL_NONE,
                             GL_PRIMARY_COLOR_ARB, GL_NONE, GL_NONE,
                             GL_REG_0_ATI, GL_NONE, GL_NONE, GL_REG_1_ATI,
                             GL_NONE, GL_NONE);
      // nglAlphaFragmentOp1ATI(GL_MOV_ATI, GL_REG_0_ATI, GL_NONE, GL_REG_0_ATI,
      // GL_NONE, GL_NONE);
      //  output 0 as RGB
      // nglColorFragmentOp1ATI(GL_MOV_ATI, GL_REG_0_ATI, GL_NONE, GL_NONE,
      // GL_ZERO, GL_NONE, GL_NONE);
      //  output alpha multiplied by constant 0
      nglAlphaFragmentOp2ATI(GL_MUL_ATI, GL_REG_0_ATI, GL_NONE, GL_REG_0_ATI,
                             GL_NONE, GL_NONE, GL_CON_0_ATI, GL_NONE, GL_NONE);
      nglEndFragmentShaderATI();
      GLenum error = glGetError();
      nlassert(error == GL_NONE);
      nglBindFragmentShaderATI(0);
  ...
  // if none of the previous programs worked, fallback on NV_texture_shader, or
  // (todo) simpler shader
#endif
}
...
uint CDriverGL::getSwapVBLInterval() {
  ...
  if (_win && _Extensions.GLXEXTSwapControl) {
    ...
    nlwarning("The swap interval is %u and the max swap interval is %u", swap,
              maxSwap);
    return swap;
  } else if (_Extensions.GLXMESASwapControl) {
...
void CDriverGL::endProfileVBHardLock(vector<std::string> &result) {
  ...
  for (uint i = 0; i < _VBHardProfiles.size(); i++) {
    ...
    // Display in ms.
    float timeLock = (float)CTime::ticksToSecond(vbProf.AccumTime) * 1000 /
                     max(_NumVBHardProfileFrame, 1U);
    smprintf(tmp, tmpSize, "%16s%c: %2.3f ms", vbName,
             vbProf.Change ? '*' : ' ', timeLock);
    total += timeLock;
...
// ***************************************************************************
void CDriverGL::appendVBHardLockProfile(NLMISC::TTicks time,
                                        CVertexBuffer *vb) {
...
void CDriverGL::profileVBHardAllocation(std::vector<std::string> &result) {
  ...
  result.push_back(toString("Memory Allocated: %4d Ko in AGP / %4d Ko in VRAM",
                            getAvailableVertexAGPMemory() / 1000,
                            getAvailableVertexVRAMMemory() / 1000));
  result.push_back(toString("Num VBHard: %d", _VertexBufferHardSet.Set.size()));

  uint totalMemUsed = 0;
  set<IVertexBufferHardGL *>::iterator it;
  for (it = _VertexBufferHardSet.Set.begin();
       it != _VertexBufferHardSet.Set.end(); it++) {
  ...
  for (it = _VertexBufferHardSet.Set.begin();
    ...
    if (vbHard) {
      ...
      result.push_back(toString("  %16s: %4d ko (format: %d / numVerts: %d)",
                                vbHard->VB->getName().c_str(),
                                vSize * numVerts / 1000, vSize, numVerts));
    }
  }
}
...
bool CDriverGL::supportCloudRenderSinglePass() const {
  ...
  // return _Extensions.NVTextureEnvCombine4 || (_Extensions.ATIXTextureEnvRoute
  // && _Extensions.EXTTextureEnvCombine); there are slowdown for now with ati
  // fragment shader... don't know why
...
void CDriverGL::retrieveATIDriverVersion() {
  ...
  _ATIDriverVersion = 0;
  // we may need this driver version to fix flaws of previous ati drivers
  // version (fog issue with V.P)
#ifdef NL_OS_WINDOWS
  ...
  if (result == ERROR_SUCCESS) {
    ...
    for (;;) {
      ...
      result = RegEnumKeyExA(parentKey, keyIndex, subKeyName, &nameBufferSize,
    ...
    if (configFound) {
      ...
      if (result == ERROR_SUCCESS) {
        ...
        result =
            RegQueryValueExA(subKey, "DriverDesc", NULL, &valueType,
                             (unsigned char *)driverDesc, &driverDescBufSize);
        if (result == ERROR_SUCCESS && valueType == REG_SZ) {
          toLowerAscii(driverDesc);
          if (strstr(driverDesc, "radeon")) // is it a radeon card ?
          {
            char driverVersion[256];
            DWORD driverVersionBufSize =
                sizeof(driverVersion) / sizeof(driverVersion[0]);
            result = RegQueryValueExA(subKey, "DriverVersion", NULL, &valueType,
                                      (unsigned char *)driverVersion,
                                      &driverVersionBufSize);
            if (result == ERROR_SUCCESS && valueType == REG_SZ) {
              int subVersionNumber[4];
              if (sscanf(driverVersion, "%d.%d.%d.%d", &subVersionNumber[0],
                         &subVersionNumber[1], &subVersionNumber[2],
                         &subVersionNumber[3]) == 4) {
                _ATIDriverVersion = (uint)subVersionNumber[3];
                /** see if fog range for V.P is bad in that driver version (is
                 * so, do a fix during vertex program conversion to
                 * EXT_vertex_shader In earlier versions of the driver, fog
                 * coordinates had to be output in the [0, 1] range From
                 * the 6.14.10.6343 driver, fog output must be in world units
                 */
...
#elif defined(NL_OS_UNIX)
  // TODO for Linux: implement retrieveATIDriverVersion... assuming versions
  // under linux are probably different
#endif
}
...
// ***************************************************************************
void CDriverGL::endBench() { CHTimer::endBench(); }

// ***************************************************************************
void CDriverGL::displayBench(class NLMISC::CLog *log) {
  // diplay
  CHTimer::displayHierarchicalByExecutionPathSorted(log, CHTimer::TotalTime,
                                                    true, 48, 2);
...
void CDriverGL::deleteOcclusionQuery(IOcclusionQuery *oq) {
  ...
  nlassert((CDriverGL *)oqgl->Driver ==
  ...
  if (OcclusionType == NotAvailable) {
    if (Driver->_Extensions.NVOcclusionQuery) {
      ...
      if (result != GL_FALSE) {
        ...
        // Note : we could return the exact number of pixels that passed the
        // z-test, but this value is not supported by all implementation
        // (Direct3D ...)
      }
    } else {
      GLuint result;
...
// ***************************************************************************
void CDriverGL::getNumPerStageConstant(uint &lightedMaterial,
...
void displayGLError(GLenum error) {
  switch (error) {
  case GL_NO_ERROR:
    nlwarning("GL_NO_ERROR");
    break;
  case GL_INVALID_ENUM:
    nlwarning("GL_INVALID_ENUM");
    break;
  case GL_INVALID_VALUE:
    nlwarning("GL_INVALID_VALUE");
    break;
  case GL_INVALID_OPERATION:
    nlwarning("GL_INVALID_OPERATION");
    break;
  case GL_STACK_OVERFLOW:
    nlwarning("GL_STACK_OVERFLOW");
    break;
  case GL_STACK_UNDERFLOW:
    nlwarning("GL_STACK_UNDERFLOW");
    break;
  case GL_OUT_OF_MEMORY:
    nlwarning("GL_OUT_OF_MEMORY");
    break;
  default:
    nlwarning("GL_ERROR");
    break;

 ...
 *	All following call with OpenGL must be done with only one instance of
 this class:
...
public:
  ...
  void init(bool supportTextureCubeMap, bool supportTextureRectangle,
            uint maxLight);

  /// Reset all OpenGL states of interest to default, and update caching. This
  /// don't apply to light.
  ...
  /// enable/disable specific light. num must be < "maxLight" param set in
  /// init()
  ...
  /** Set z-bias
   * NB : this is done in window coordinate, not in world coordinate as with
   * CMaterial
   */
   ...
   *	NB: if CubeMap extension not supported, TextureCubeMap <=>
   *TextureDisabled.
   */
  void setTextureMode(TTextureMode texMode);
  TTextureMode getTextureMode() const {
    return _TextureMode[_CurrentActiveTextureARB];
  }
  // reset texture mode to the default (disabled) for the current stage. It
  // forces the state (useful after texture shaders)
  void resetTextureMode();
  // @}

  /// \name Vertex Array enabling.
  /// equivalent to glEnableClientState() / glDisableClientState(). NB: Not
  /// modified by forceDefaults()
  // @{
   ...
   */
  void enableVertexAttribArrayForEXTVertexShader(uint glIndex, bool enable,
                                                 uint *variants);

  // special version for ARB_vertex_program used with ARB_vertex_buffer or

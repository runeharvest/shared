...
bool CCocoaEventEmitter::processMessage(NSEvent *event, CEventServer *server)
{
	...
	if ((mousePos.x < 0.0 || mousePos.x > 1.0 || mousePos.y < 0.0 || mousePos.y > 1.0) && event.type != NSKeyDown && event.type != NSKeyUp)
	{
		return false;
	}

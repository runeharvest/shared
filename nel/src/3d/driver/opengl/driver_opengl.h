...
#define UNSUPPORTED_INDEX_OFFSET_MSG                                           \
  "Unsupported by driver, check IDriver::supportIndexOffset."
...
public:
  ...
  CShaderGL(IDriver *drv, ItMatDrvInfoPtrList it)
...
public:
  ...
  // NB: ptrs are invalid if VertexFormat does not support the compoennt. must
  // test VertexFormat, not the ptr.
  ...
  CVertexBufferInfo() { VBMode = TVBModeNone; }
...
public:
  ...
  virtual bool isLost() const {
    return false;
  } // there's no notion of 'lost device" in OpenGL
  ...
  virtual nlWindow getDisplay() { return _win; }
  ...
  /// For objects with caustics, setup the first texture (which actually is the
  /// one from the material)
  ...
  virtual bool isForceNormalize() const { return _ForceNormalize; }
  ...
  virtual bool supportIndexOffset() const {
    return false; /* feature only supported in D3D for now */
  }
  ...
  virtual bool renderRawQuads(CMaterial &Mat, uint32 startIndex,
                              uint32 numQuads);
  //
  virtual bool renderLinesWithIndexOffset(CMaterial & /* mat */,
                                          uint32 /* firstIndex */,
                                          uint32 /* nlines */,
                                          uint /* indexOffset */) {
    nlassertex(0, (UNSUPPORTED_INDEX_OFFSET_MSG));
    return false;
  }
  virtual bool renderTrianglesWithIndexOffset(CMaterial & /* mat */,
                                              uint32 /* firstIndex */,
                                              uint32 /* ntris */,
                                              uint /* indexOffset */) {
    nlassertex(0, (UNSUPPORTED_INDEX_OFFSET_MSG));
  ...
  virtual uint32 getImplementationVersion() const { return ReleaseVersion; }

  virtual const char *getDriverInformation() { return "Opengl 1.2 NeL Driver"; }
  ...
  virtual void setPerPixelLightingLight(CRGBA diffuse, CRGBA specular,
                                        float shininess);
  ...
  /// setup fog parameters. fog must enabled to see result. start and end are in
  /// [0,1] range.
...
private:
  virtual class IVertexBufferHardGL *
  createVertexBufferHard(uint size, uint numVertices,
...
private:
  ...
  friend bool GlWndProc(CDriverGL *driver, HWND hWnd, UINT message,
  ...
  bool convertBitmapToIcon(const NLMISC::CBitmap &bitmap,
  ...
  // To know if light setup has been changed from last render() ( any call to
  // setupViewMatrix() or setLight() ).
  bool _LightSetupDirty;

  // To know if the modelview matrix setup has been changed from last render()
  // (any call to setupViewMatrix() / setupModelMatrix() ).
  ...
  // Current View matrix, in NEL basis. This is the parameter passed in
  // setupViewMatrix*().
  ...
  // Precision ZBuffer: The Current cameraPosition, to remove from each model
  // Position.
  ...
  // Hence this matrix represent the Exact eye-space basis (only _ViewMtx is a
  // bit tricky).
  ...
  bool _LightDirty[MaxLight]; // Light that need a View position setup in
                              // refreshRenderSetup().
  ...
  // NB: CRefPtr are not used for mem/spped optimisation. setupMaterial() and
  // setupTexture() reset those states.
  CMaterial *_CurrentMaterial;
  CMaterial::TShader _CurrentMaterialSupportedShader;

  /* NB : this pointers handles the caching of glBindTexture() and
   * setTextureMode() calls.
   */
   ...
   */
  void setTextureShaders(const uint8 *addressingModes,
...
private:
  ...
  // Return monitor info and positon in multi monitor setup or false if monitor
  // not found.
  bool getMonitorByName(const std::string &name, sint32 &x, sint32 &y,
  ...
  bool getBestCursorSize(uint srcWidth, uint srcHeight, uint &dstWidth,
                         uint &dstHeight);
  ...
  // Force Activate Texture Environnement. no caching here. TexEnvSpecial is
  // disabled.
  ...
  void copyFrameBufferToTexture(ITexture *tex, uint32 level, uint32 offsetx,
                                uint32 offsety, uint32 x, uint32 y,
                                uint32 width, uint32 height, uint cubeFace = 0);
  ...
  /// Temp Variables computed in beginLightMapMultiPass(). Reused in
  /// setupLightMapPass().
  uint _NLightMaps;
  uint _NLightMapPerPass;
  uint _NLightMapPass;
  bool _LightMapNoMulAddFallBack;
  // This array is the LUT from lmapId in [0, _NLightMaps[, to original lightmap
  // id in material.
  ...
  TTexCubeVect _SpecularTextureCubes; // the cube maps used to compute the
                                      // specular lighting.

  /// get (and if necessary, build) a cube map used for specular lighting. The
  /// range for exponent is limited, and only the best fit is used
   ...
   */
  virtual bool supportPixelProgram(
      CPixelProgram::TProfile profile = CPixelProgram::arbfp1) const;
  ...
  // @{
  // Set parameters
  inline void setUniform4fInl(TProgram program, uint index, float f0, float f1,
                              float f2, float f3);
  inline void setUniform4fvInl(TProgram program, uint index, size_t num,
                               const float *src);
  ...
  bool setupPixelProgram(CPixelProgram *program,
                         GLuint id /*, bool &specularWritten*/);
  //@}

  /// \fallback for material shaders
  // @{
  /// test whether the given shader is supported, and gives back a supported
  /// shader
  ...
  // @{
  NLMISC::CSmartPtr<CTextureCube>
      _CausticCubeMap; // a cube map used for the rendering of caustics
...
private:
  ...
  // Handle of the first constant c[0]. In vertex program we have 96 constant
  // c[0] .. c[95]
  GLuint _EVSConstantHandle;
  // number of constant
  static const uint _EVSNumConstant;
  //
  bool setupEXTVertexShader(const CVPParser::TProgram &program, GLuint id,
...
public:
  ...
  const IVertexBufferHardGL *getCurrentVertexBufferHard() const {
    return _CurrentVertexBufferHard;
  }
...
private:
   ...
   * Parameters / part of the texture are ready to be changed in the gl after
   * that _CurrentTexture & _CurrentTextureInfoGL are not modified !
   */
  inline void bindTextureWithMode(ITexture &tex);
  /** Force to set clamp & wrap mode for the given texture
   * Setup is done for texture currently bind to the gl, so calling
   * bindTextureWithMode is necessary
   */
...
public:
   ...
   * This allow to activate only the gl arrays that are needed by a given
   * shader.
   */

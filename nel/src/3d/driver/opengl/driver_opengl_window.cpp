...
bool GlWndProc(CDriverGL *driver, HWND hWnd, UINT message, WPARAM wParam,
               LPARAM lParam) {
    ...
    if (driver->_WndActive) {
      if (driver->_CurrentMode.Windowed) {
        ...
        int status = XGetWindowProperty(
            driver->_dpy, driver->_win, XA_FRAME_EXTENTS, 0, 4, False,
            XA_CARDINAL, &type_return, &format_return, &nitems_return,
            &bytes_after_return, (unsigned char **)&data);

        // succeeded to retrieve decoration size
        if (status == Success && type_return == XA_CARDINAL &&
            format_return == 32 && nitems_return == 4 && data) {
          driver->_DecorationWidth = data[0];
          driver->_DecorationHeight = data[2];
        } else {
          // use difference between current position and previous one (set by
          // application)
        ...
        if (driver->_DecorationWidth < 0)
          driver->_DecorationWidth = 0;
        if (driver->_DecorationHeight < 0)
          driver->_DecorationHeight = 0;
...
bool CDriverGL::init(uintptr_t windowIcon, emptyProc exitFunc) {
  ...
  if (dc) {
    _NeedToRestoreGammaRamp =
        GetDeviceGammaRamp(dc, _GammaRampBackuped) != FALSE;
  ...
  if (XF86VidModeQueryExtension(_dpy, &event, &error) &&
      XF86VidModeQueryVersion(_dpy, &vm_major, &vm_minor)) {
  ...
  if (XQueryExtension(_dpy, "RENDER", &xrender_major, &xrender_event,
                      &xrender_error) &&
  ...
  XA_WM_WINDOW_TYPE_NORMAL =
      XInternAtom(_dpy, "_NET_WM_WINDOW_TYPE_NORMAL", False);
...
void CDriverGL::setWindowIcon(const std::vector<NLMISC::CBitmap> &bitmaps) {
  ...
  if (!icon_data.empty()) {
    // change window icon
    XChangeProperty(_dpy, _win, XA_WM_ICON, XA_CARDINAL, 32, PropModeReplace,
                    (const unsigned char *)&icon_data[0], icon_data.size());
  } else {
...
#endif
bool CDriverGL::setDisplay(nlWindow wnd, const GfxMode &mode, bool show,
                           bool resizeable) {
  ...
  if (mode.OffScreen) {
    ...
    AdjustWindowRectEx(&rc, GetWindowStyle(_win), GetMenu(_win) != NULL,
                       GetWindowExStyle(_win));
    SetWindowPos(_win, NULL, 0, 0, rc.right - rc.left, rc.bottom - rc.top,
    ...
    if (!nwglChoosePixelFormatARB(hdc, iattributes, fattributes, 20, pformat,
                                  &nformats)) {
      nlwarning(
    ...
    /* After determining a compatible pixel format, the next step is to create a
       pbuffer of the chosen format. Fortunately this step is fairly easy, as
       you merely select one of the formats returned in the list in step #2 and
       call the function: */
    int iattributes2[1] = {0};
    // int iattributes2[] = {WGL_PBUFFER_LARGEST_ARB, 1, 0};
    _PBuffer =
        nwglCreatePbufferARB(hdc, pformat[0], width, height, iattributes2);
    if (_PBuffer == NULL) {
      DWORD error = GetLastError();
      nlwarning("CDriverGL::setDisplay: wglCreatePbufferARB failed: 0x%x",
                error);
      wglDeleteContext(tempGLRC);
    ...
    /* After creating a pbuffer, you may use this functions to determine the
     * dimensions of the pbuffer actually created. */
    if (!nwglQueryPbufferARB(_PBuffer, WGL_PBUFFER_WIDTH_ARB, (int *)&width)) {
      DWORD error = GetLastError();
      nlwarning("CDriverGL::setDisplay: wglQueryPbufferARB failed: 0x%x",
                error);
      wglDeleteContext(tempGLRC);
      DestroyWindow(tmpHWND);
      _PBuffer = NULL;
    ...
    if (!nwglQueryPbufferARB(_PBuffer, WGL_PBUFFER_HEIGHT_ARB,
                             (int *)&height)) {
      DWORD error = GetLastError();
      nlwarning("CDriverGL::setDisplay: wglQueryPbufferARB failed: 0x%x",
                error);
    ...
    /* The next step is to create a device context for the newly created
       pbuffer. To do this, call to the function: */
    _hDC = nwglGetPbufferDCARB(_PBuffer);
    if (_hDC == NULL) {
      DWORD error = GetLastError();
      nlwarning("CDriverGL::setDisplay: wglGetPbufferDCARB failed: 0x%x",
                error);
      nwglDestroyPbufferARB(_PBuffer);
    ...
    /* The final step of pbuffer creation is to create an OpenGL rendering
       context and associate it with the handle for the pbuffer's device context
       created in step #4. This is done as follows */
    ...
    /* After a pbuffer has been successfully created you can use it for
       off-screen rendering. To do so, you'll first need to bind the pbuffer, or
       more precisely, make its GL rendering context the current context that
       will interpret all OpenGL commands and state changes. */
  ...
  NSOpenGLPixelFormatAttribute att[] = {NSOpenGLPFADoubleBuffer,
  ...
  static int sAttribList16bpp[] = {GLX_RGBA,
                                   GLX_DOUBLEBUFFER,
                                   GLX_DEPTH_SIZE,
                                   16,
                                   GLX_RED_SIZE,
  ...
  static int sAttribList24bpp[] = {GLX_RGBA,
                                   GLX_DOUBLEBUFFER,
                                   GLX_DEPTH_SIZE,
                                   24,
                                   GLX_RED_SIZE,
  ...
  XVisualInfo *visual_info =
      glXChooseVisual(_dpy, DefaultScreen(_dpy), sAttribList24bpp);
  if (visual_info == NULL)
  ...
  // KDE/GNOME has issues selecting correct fullscreen monitor if window is not
  // visible before setMode()
  if (!mode.Windowed) {
...
bool CDriverGL::saveScreenMode() {
  ...
  if (!res && _xrandr_version > 0) {
    XRRScreenConfiguration *screen_config =
        XRRGetScreenInfo(_dpy, RootWindow(_dpy, screen));
...
bool CDriverGL::restoreScreenMode() {
  ...
  if (!res && _xrandr_version > 0) {
    ...
    if (screen_config) {
      ...
      SizeID size =
          XRRConfigCurrentConfiguration(screen_config, &saved_rotation);
      if (XRRSetScreenConfig(_dpy, screen_config, root, _OldSizeID,
                             saved_rotation,
...
static sint modeInfoToFrequency(XF86VidModeModeInfo *info) {
  return (info->htotal && info->vtotal)
             ? (1000 * info->dotclock / (info->htotal * info->vtotal))
             : 0;
}
#endif // XF86VIDMODE
...
bool CDriverGL::setScreenMode(const GfxMode &mode) {
  ...
  if (getCurrentScreenMode(previousMode) && mode.Width == previousMode.Width &&
      mode.Height == previousMode.Height && mode.Depth == previousMode.Depth &&
  ...
  if (!found && _xrandr_version > 0) {
    ...
    if (screen_config) {
      ...
      if (size > -1) {
        if (XRRSetScreenConfig(_dpy, screen_config, root, size, saved_rotation,
                               CurrentTime) == RRSetConfigSuccess) {
          nlinfo("3D: Switching to XRandR mode %d: %dx%d", size,
                 sizes[size].width, sizes[size].height);
          found = true;
        } else {
          nlwarning("3D: XRRSetScreenConfig failed for mode %d: %dx%d", size,
                    sizes[size].width, sizes[size].height);
        }
      } else {
  ...
  if (!found && _xvidmode_version > 0) {
    ...
    int nmodes;
    if (XF86VidModeGetAllModeLines(_dpy, DefaultScreen(_dpy), &nmodes,
                                   &modes)) {
      for (int i = 0; i < nmodes; i++) {
        const uint16 freq = modeInfoToFrequency(modes[i]);

        nldebug("3D: Available mode - %dx%d %d Hz", modes[i]->hdisplay,
                modes[i]->vdisplay, (int)freq);
        if (modes[i]->hdisplay == mode.Width &&
            modes[i]->vdisplay == mode.Height /* && freq == mode.Frequency */) {
          if (XF86VidModeSwitchToMode(_dpy, DefaultScreen(_dpy), modes[i])) {
            nlinfo("3D: XF86VidMode Switching to mode %dx%d",
...
bool CDriverGL::createWindow(const GfxMode &mode) {
  ...
  window =
      CreateWindowW(L"NLClass", L"NeL Window", dwStyle, pos, pos, mode.Width,
                    mode.Height, hwndParent, NULL, GetModuleHandle(NULL), NULL);
  ...
  window =
      [[NSView alloc] initWithFrame:NSMakeRect(0, 0, mode.Width, mode.Height)];
  ...
  window = XCreateWindow(_dpy, root, 0, 0, mode.Width, mode.Height, 0,
  ...
  // normal window type
  XChangeProperty(_dpy, window, XA_WM_WINDOW_TYPE, XA_ATOM, 32, PropModeReplace,
                  (const unsigned char *)&XA_WM_WINDOW_TYPE_NORMAL, 1);
...
bool CDriverGL::destroyWindow() {
  ...
  if (_DestroyWindow &&
      _ctx) // FIXME: _DestroyWindow may need to be removed here as well
...
bool CDriverGL::setWindowStyle(EWindowStyle windowStyle) {
  ...
  bool isMaximized =
      GetWindowPlacement(_win, &wndpl) && (wndpl.showCmd == SW_SHOWMAXIMIZED);
  ...
  // enter fullscreen, leave windowed mode
    ...
    if (error != kCGLNoError)
      ...
      // put the view in fullscreen mode, hiding the dock but enabling the
      // menubar to pop up if the mouse hits the top screen border. NOTE:
      // withOptions:nil disables <CMD>+<Tab> application switching!
#ifdef NL_MAC_VERSION_10_6_UP
    [containerView()
        enterFullScreenMode:[NSScreen mainScreen]
                withOptions:
                    [NSDictionary
  ...
  if (attr.map_state != IsUnmapped) {
    ...
    xev.xclient.data.l[0] =
        windowStyle == EWSFullscreen ? _NET_WM_STATE_ADD : _NET_WM_STATE_REMOVE;
    xev.xclient.data.l[1] = XA_WM_STATE_FULLSCREEN;
    xev.xclient.data.l[2] = 0;
    xev.xclient.data.l[3] =
        1; // 1 for Application, 2 for Page or Taskbar, 0 for old source
    xev.xclient.data.l[4] = 0;
    if (!XSendEvent(_dpy, XDefaultRootWindow(_dpy), False,
    ...
    if (windowStyle == EWSFullscreen) {
      ...
      XChangeProperty(_dpy, _win, XA_WM_STATE, XA_ATOM, 32, PropModeReplace,
                      (const unsigned char *)&XA_WM_STATE_FULLSCREEN, 1);
    } else {
...
bool CDriverGL::getMonitorByName(const std::string &name, sint32 &x, sint32 &y,
  ...
  if (_xrandr_version >= 105) {
    ...
    XRRMonitorInfo *monitor =
        XRRGetMonitors(_dpy, RootWindow(_dpy, screen), 1, &nmonitors);
    if (!monitor)
      return false;
    ...
    XRRScreenResources *resources =
        XRRGetScreenResourcesCurrent(_dpy, RootWindow(_dpy, screen));
    if (!resources)
...
bool CDriverGL::setMode(const GfxMode &amode) {
  ...
  BOOL cursorPosOk = isSystemCursorInClientArea() && GetCursorPos(&cursorPos) &&
  ...
  if (!mode.Windowed) {
    ...
    // make sure resolution matches requested or currently active monitor's
    // resolution
    ...
    if (!mode.DisplayDevice.empty()) {
      ...
      if (getMonitorByName(mode.DisplayDevice, newX, newY, newW, newH)) {
        ...
        nlinfo("3D: XrandR: Reading requested monitor '%s' info failed, using "
               "'%s'",
               mode.DisplayDevice.c_str(), current.DisplayDevice.c_str());
    ...
    if (!mode.DisplayDevice.empty() &&
      ...
      if (xwa.map_state != IsUnmapped) {
        ...
        Bool res = XQueryPointer(_dpy, _win, &root_win, &child_win, &root_x,
      ...
      XWarpPointer(_dpy, None, root, None, None, None, None, newX + mouseX,
                   newY + mouseY);
...
#define GetModeBitsPerPixel(mode)                                              \
  GetDictionaryLong((mode), kCGDisplayBitsPerPixel)
...
bool CDriverGL::getModes(std::vector<GfxMode> &modes) {
  ...
  CGDisplayErr err =
      CGGetActiveDisplayList(kMaxDisplays, display, &numDisplays);
  ...
  for (CGDisplayCount i = 0; i < numDisplays; ++i) {
    ...
    for (CFIndex j = 0; j < CFArrayGetCount(modeList); ++j) {
      ...
      CGDisplayModeRef mode =
          (CGDisplayModeRef)CFArrayGetValueAtIndex(modeList, j);
      uint8 bpp = bppFromDisplayMode(mode);
#else
      CFDictionaryRef mode =
          (CFDictionaryRef)CFArrayGetValueAtIndex(modeList, j);
  ...
  if (!_dpy) {
    nlwarning("3D: Unable to list modes because Display is NULL, did you "
              "forget to call init() ?");
    return false;
  }
      ...
      if (output->crtc && output->connection == RR_Connected) {
        ...
        if (crtc) {
          std::map<int, int>::const_iterator it =
              resourceModeMap.find(crtc->mode);
  ...
  if (!found && _xvidmode_version > 0) {
    ...
    if (XF86VidModeGetAllModeLines(_dpy, screen, &nmodes, &ms)) {
      //			nlinfo("3D: %d available XF86VidMode modes:",
      //nmodes);
...
bool CDriverGL::getCurrentScreenMode(GfxMode &mode) {
  ...
  // get window position so we can compare monitors (or mouse position if window
  // not visible yet)
  ...
  if (useMouseForPosition) {
    sint rx, ry, wx, wy;
    uint mask;
    Bool res = XQueryPointer(_dpy, RootWindow(_dpy, screen), &child, &child,
                             &rx, &ry, &wx, &wy, &mask);
    if (res) {
      x = rx;
      y = ry;
    ...
    for (uint i = 0; i < resources->noutput; ++i) {
      ...
      if (output->crtc && output->connection == RR_Connected) {
        ...
        if (crtc) {
          ...
          if (x >= crtc->x && y >= crtc->y && x < (crtc->x + width) &&
              y < (crtc->y + height)) {
            found = true;
            mode.DisplayDevice = output->name;
            mode.Width = width;
  ...
  if (!found && _xrandr_version > 0) {
    ...
    if (screen_config) {
      ...
      if (nsizes > 0) {
        ...
        nlinfo("3D: Current XRandR mode %d: %dx%d, %dbit", size, mode.Width,
  ...
  if (!found && _xvidmode_version > 0) {
    ...
    if (XF86VidModeGetModeLine(_dpy, screen, &pixelClock, &xmode)) {
      ...
      nlinfo("3D: Current XF86VidMode mode: %dx%d, %d Hz, %dbit", mode.Width,
             mode.Height, mode.Frequency, mode.Depth);

      found = true;
    } else {
      nlwarning(
          "3D: XF86VidModeGetModeLine failed, cannot get current video mode");
    }
  }
  ...
  if (!found) {
    ...
    nldebug("Current mode: %dx%d, %d Hz, %dbit", mode.Width, mode.Height,
            mode.Frequency, mode.Depth);
  }
...
void CDriverGL::setWindowTitle(const ucstring &title) {
  ...
  [[containerView() window]
      setTitle:[NSString stringWithUTF8String:title.toUtf8().c_str()]];

#elif defined(NL_OS_UNIX)

#ifdef X_HAVE_UTF8_STRING
  // UTF8 properties
  Xutf8SetWMProperties(_dpy, _win, (char *)title.toUtf8().c_str(),
                       (char *)title.toUtf8().c_str(), NULL, 0, NULL, NULL,
                       NULL);
#else
  // standard properties
  XTextProperty text_property;
  if (XStringListToTextProperty((char **)&title.toUtf8().c_str(), 1,
                                &text_property) != 0) {
    XSetWMProperties(_dpy, _win, &text_property, &text_property, NULL, 0, NULL,
                     NULL, NULL);
  } else {
...
void CDriverGL::setWindowPos(sint32 x, sint32 y) {
  ...
  SetWindowPos(_win, NULL, x, y, 0, 0,
...
void CDriverGL::showWindow(bool show) {
  ...
  if (show) {
    // if window is mapped, then MapNotify event will not trigger and
    // XPeekIfEvent deadlocks
    XWindowAttributes attr;
    XGetWindowAttributes(_dpy, _win, &attr);
    if (attr.map_state == IsUnmapped) {
      XMapRaised(_dpy, _win);
      // sync to MapNotify event or setWindowSize() in GNOME might resize window
      // to wrong size
      XEvent event;
      nlctassert(sizeof(XPointer) >= sizeof(_win));
      XPeekIfEvent(_dpy, &event, WaitForNotify,
                   reinterpret_cast<XPointer>(_win));
    }
...
bool CDriverGL::createContext() {
  ...
  EGLint attribList[] = {
      ...
      //		EGL_SAMPLE_BUFFERS,	_CurrentMode.AntiAlias > -1 ?
      //1:0, 		EGL_SAMPLES,		samples,
  ...
  _EglContext =
      eglCreateContext(_EglDisplay, config, EGL_NO_CONTEXT, contextAttribs);
...
// --------------------------------------------------
IDriver::TMessageBoxId
CDriverGL::systemMessageBox(const char *message, const char *title,
                            IDriver::TMessageBoxType type,
                            TMessageBoxIcon icon) {
  H_AUTO_OGL(CDriverGL_systemMessageBox)
#ifdef NL_OS_WINDOWS
  switch (::MessageBoxW(NULL, nlUtf8ToWide(message), nlUtf8ToWide(title),
  ...
  nlstop;
#else  // NL_OS_WINDOWS
       // TODO: if user did not launch from console, then program "freezes"
       // without explanation or possibility to continue
...
void CDriverGL::setWindowSize(uint32 width, uint32 height) {
  ...
  if (_CurrentMode.Windowed) {
    // TODO: this gives wrong info for initial fullscreen window so limit for
  ...
  if ([containerView() isInFullScreenMode]) {
    ...
    CGLError error = CGLSetParameter((CGLContextObj)[_ctx CGLContextObj],
    ...
    if (_DestroyWindow) {
      ...
      rect = [[containerView() window]
          frameRectForContentRect:NSMakeRect(rect.origin.x, rect.origin.y,
                                             width, height)];
...
#elif defined(NL_OS_MAC)
  // TODO: Missing Mac Implementation for isActive (always true if a window is
  // set)
#elif defined(NL_OS_UNIX)
...
// ***************************************************************************
bool CDriverGL::setMonitorColorProperties(
  ...
  if (dc) {
    ...
    for (c = 0; c < 3; c++) {
      ...
      for (i = 0; i < 256; i++) {
        ...
        ramp[i + (c << 8)] =
            (WORD)min((int)65535, max(0, (int)(value * 65535)));
      }
    }
...
bool CDriverGL::convertBitmapToIcon(const NLMISC::CBitmap &bitmap, HICON &icon,
                                    uint iconWidth, uint iconHeight,
                                    uint iconDepth, const NLMISC::CRGBA &col,
                                    sint hotSpotX, sint hotSpotY, bool cursor) {
  CBitmap src = bitmap;
...
bool CDriverGL::convertBitmapToIcon(const NLMISC::CBitmap &bitmap,
  ...
  for (uint j = 0; j < pixels.size(); j += 4)
    icon[pos++] = pixels[j] << 16 | pixels[j + 1] << 8 | pixels[j + 2] |
                  pixels[j + 3] << 24;

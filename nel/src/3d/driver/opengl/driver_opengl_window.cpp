...
bool CDriverGL::init(uintptr_t windowIcon, emptyProc exitFunc)
{
	...
	if (XQueryExtension(_dpy, "RENDER", &xrender_major, &xrender_event, &xrender_error) && XRenderQueryExtension(_dpy, &xrender_event, &xrender_error))
	{
...
bool CDriverGL::setWindowStyle(EWindowStyle windowStyle)
{
	...
	else if (windowStyle == EWSFullscreen && ![containerView() isInFullScreenMode])
	{
		...
		[containerView() enterFullScreenMode:[NSScreen mainScreen]
		                         withOptions:
...
bool CDriverGL::getCurrentScreenMode(GfxMode &mode)
{
	...
	if (_xrandr_version >= 105)
	{
		...
		if (monitor)
		{
			...
			for (sint i = 0; i < nmonitors; ++i)
			{
				if ((x >= monitor[i].x && x < (monitor[i].x + monitor[i].width) && y >= monitor[i].y && y < (monitor[i].y + monitor[i].height)) || (monitor[i].primary && bestMatch == -1))
				{
					bestMatch = i;
				}
			}
...
#ifdef NL_OS_WINDOWS
	switch (::MessageBoxW(NULL, nlUtf8ToWide(message), nlUtf8ToWide(title), ((type == retryCancelType) ? MB_RETRYCANCEL : (type == yesNoCancelType) ? MB_YESNOCANCEL

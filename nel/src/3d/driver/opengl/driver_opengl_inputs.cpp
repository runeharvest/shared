...
CDriverGL::CCursor::CCursor()
    : ColorDepth(CDriverGL::ColorDepth32), OrigHeight(32), HotspotScale(1.f),
      HotspotOffsetX(0), HotspotOffsetY(0), HotSpotX(0), HotSpotY(0),
      Cursor(EmptyCursor), Col(CRGBA::White), Rot(0) {
#if defined(NL_OS_UNIX) && !defined(NL_OS_MAC)
  Dpy = NULL;
#endif
}
...
// *************************************************************************************
CDriverGL::CCursor::~CCursor() { reset(); }
...
// *************************************************************************************
CDriverGL::CCursor &
CDriverGL::CCursor::operator=(const CDriverGL::CCursor &from) {
  if (&from == this)
    return *this;
...
bool CDriverGL::isAlphaBlendedCursorSupported() {
  if (!_AlphaBlendedCursorSupportRetrieved) {
    ...
    // but can't be sure that using the win32 api 'SetCursor' uses the same
    // resources So far, seems to be supported on any modern card used by the
    // game anyway ...
    OSVERSIONINFO osvi;
...
// *************************************************************************************
void CDriverGL::addCursor(const std::string &name,
                          const NLMISC::CBitmap &cursorBitmap) {
  if (!isAlphaBlendedCursorSupported())
    return;
  ...
  curs.Src.blit(cursorBitmap, minX, minY, maxX - minX + 1, maxY - minY + 1, 0,
                0);
  ...
  if (tmpSize > destWidth || tmpSize > destHeight) // need to shrink ?
  {
    ...
    curs.HotspotScale *=
        std::min(float(destWidth) / tmpSize, float(destHeight) / tmpSize);
...
void CDriverGL::createCursors() {
  ...
  if (_dpy && _win && _BlankCursor == EmptyCursor) {
    ...
    _BlankCursor = XCreatePixmapCursor(_dpy, pixmap_no_data, pixmap_no_data,
                                       &black, &black, 0, 0);
...
void CDriverGL::updateCursor(bool forceRebuild) {
  setCursor(_CurrName, _CurrCol, _CurrRot, _CurrHotSpotX, _CurrHotSpotY,
            forceRebuild);
}

// *************************************************************************************
void CDriverGL::setCursor(const std::string &name, NLMISC::CRGBA col, uint8 rot,
                          sint hotSpotX, sint hotSpotY, bool forceRebuild) {
  // don't update cursor if it's hidden or if custom cursors are not suppported
  if (!isAlphaBlendedCursorSupported() || _CurrName == "none")
    return;
  ...
  if (rot > 3)
    rot = 3; // same than 'CViewRenderer::drawRotFlipBitmapTiled
  ...
  if (it != _Cursors.end()) {
    ...
    hotSpotY = (sint)(curs.HotspotScale *
                      ((curs.OrigHeight - hotSpotY) - curs.HotspotOffsetY));
    if (curs.Cursor == EmptyCursor || curs.HotSpotX != hotSpotX ||
        curs.HotSpotY != hotSpotY || curs.Col != col || curs.Rot != rot ||
        curs.ColorDepth != _ColorDepth || forceRebuild) {
      curs.reset();
...
//		if (CInputHandlerManager::getInstance()->hasFocus())
#ifdef NL_OS_WINDOWS
    {
      ...
      SetClassLongPtr(
          _win, GCLP_HCURSOR,
          (LONG_PTR)cursorHandle); // set default mouse icon to the last one
    }
#elif defined(NL_OS_MAC)
#elif defined(NL_OS_UNIX)
...
// *************************************************************************************
void CDriverGL::setCursorScale(float scale) { _CursorScale = scale; }

// *************************************************************************************
nlCursor CDriverGL::buildCursor(const CBitmap &src, NLMISC::CRGBA col,
  ...
  CBitmap rotSrc = src;
  if (rot > 3)
    rot = 3; // mimic behavior of 'CViewRenderer::drawRotFlipBitmapTiled' (why
             // not rot & 3 ??? ...)
  switch (rot) {
  case 0:
    break;
  ...
  convertBitmapToCursor(rotSrc, result, mouseW, mouseH,
                        _ColorDepth == ColorDepth16 ? 16 : 32, col, hotSpotX,
                        hotSpotY);
  return result;
}
...
void CDriverGL::setMousePos(float x, float y) {
  ...
  CGDisplayErr error = CGDisplayMoveCursorToPoint(
      ...
      CGPointMake(windowRect.origin.x + (viewRect.size.width * x),
                  firstScreenRect.size.height - windowRect.origin.y -
                      viewRect.size.height +
                      ((1.0 - y) * viewRect.size.height)));
...
void CDriverGL::setCapture(bool b) {
  ...
  if (b && isSystemCursorInClientArea() && !isSystemCursorCaptured()) {
    ...
    // if hardware mouse and not in client area, then force to update its aspect
    // by updating its pos
  ...
  if (b /* && isSystemCursorInClientArea() && !isSystemCursorCaptured()*/) // capture
                                                                           // the
                                                                           // cursor.
  {
    // capture the cursor
    XGrabPointer(_dpy, _win, True, 0, GrabModeAsync, GrabModeAsync, _win, None,
                 CurrentTime);
    _MouseCaptured = true;
  } else if (!b /* && isSystemCursorCaptured()*/) {
    // release the cursor
...
bool CDriverGL::isSystemCursorInClientArea() {
  if (!_CurrentMode.Windowed) {
    ...
    if ((cursPos.x < tl.x) || (cursPos.x >= br.x) || (cursPos.y < tl.y) ||
        (cursPos.y >= br.y)) {
      return false;
    }
#elif defined(NL_OS_MAC)
...
bool CDriverGL::getBestCursorSize(uint srcWidth, uint srcHeight, uint &dstWidth,
                                  uint &dstHeight) {
#ifdef NL_OS_WINDOWS
  ...
  Status status =
      XQueryBestCursor(_dpy, _win, srcWidth, srcHeight, &dstWidth, &dstHeight);
...
bool CDriverGL::convertBitmapToCursor(const NLMISC::CBitmap &bitmap,
                                      nlCursor &cursor, uint iconWidth,
                                      uint iconHeight, uint iconDepth,
                                      const NLMISC::CRGBA &col, sint hotSpotX,
                                      sint hotSpotY) {
#if defined(NL_OS_WINDOWS)

  return convertBitmapToIcon(bitmap, cursor, iconWidth, iconHeight, iconDepth,
                             col, hotSpotX, hotSpotY, true);
  ...
  if (XcursorSupportsARGB(_dpy)) {
    ...
    if (!image) {
      nlwarning("Failed to create a XcusorImage with size %ux%u", iconWidth,
                iconHeight);
    } else {
  ...
  if (_xrender_version > 0) {
    ...
    XImage *image = XCreateImage(_dpy, visual, 32, ZPixmap, 0, (char *)src32,
                                 iconWidth, iconHeight, 32, 0);
    ...
    Pixmap pixmap =
        XCreatePixmap(_dpy, _win, iconWidth, iconHeight, 32 /* defDepth */);

    if (!pixmap) {
      nlwarning("Failed to create a pixmap %ux%ux%d", iconWidth, iconHeight,
                32);
      return false;
    }
    ...
    sint res =
        XPutImage(_dpy, pixmap, gc, image, 0, 0, 0, 0, iconWidth, iconHeight);
    ...
    XRenderPictFormat *format =
        XRenderFindStandardFormat(_dpy, PictStandardARGB32);

...
CVBDrvInfosGL::CVBDrvInfosGL(CDriverGL *drv, ItVBDrvInfoPtrList it,
                             CVertexBuffer *vb)
...
// ***************************************************************************
uint8 *CVBDrvInfosGL::lock(uint /* first */, uint /* last */,
                           bool /* readOnly */) {
  ...
  if (touched || (VB.DrvInfos == NULL)) {
    ...
    if (VB.getNumVertices()) {
      ...
      ItVBDrvInfoPtrList it =
          _VBDrvInfos.insert(_VBDrvInfos.end(), (NL3D::IVBDrvInfos *)NULL);
      // create and set iterator, for future deletion.
      CVBDrvInfosGL *info = new CVBDrvInfosGL(this, it, &VB);
      *it = VB.DrvInfos = info;

      // Preferred memory, AGPVolatile only goes through when ARBMapBufferRange
      // is available
      ...
      if ((preferred == CVertexBuffer::RAMVolatile) ||
          (preferred == CVertexBuffer::AGPVolatile &&
           !_Extensions.ARBMapBufferRange))
        preferred = CVertexBuffer::RAMPreferred;
      const uint size = VB.capacity() * VB.getVertexSize();
      uint preferredMemory = _Extensions.DisableHardwareVertexArrayAGP
                                 ? CVertexBuffer::RAMPreferred
                                 : preferred;
      while (preferredMemory != CVertexBuffer::RAMPreferred) {
        // Vertex buffer hard
        info->_VBHard = createVertexBufferHard(
            size, VB.capacity(),
            (CVertexBuffer::TPreferredMemory)preferredMemory, &VB);
        if (info->_VBHard)
          break;

        if ((CVertexBuffer::TPreferredMemory)preferredMemory ==
      ...
      // Upload the data
      VB.setLocation(preferredMemory == CVertexBuffer::AGPVolatile
                         ? CVertexBuffer::AGPResident
                         : (CVertexBuffer::TLocation)preferredMemory);
    }
  }
...
bool CDriverGL::activeVertexBuffer(CVertexBuffer &VB) {
  ...
  if (!info->_VBHard || (info->_VBHard && !info->_VBHard->isInvalid())) {
    ...
    if (lastVBMode == CVertexBufferInfo::HwARB &&
        _LastVB.VBMode != CVertexBufferInfo::HwARB) {
...
bool CDriverGL::renderLines(CMaterial &mat, uint32 firstIndex, uint32 nlines) {
  ...
  if (_CurrentVertexBufferHard && _CurrentVertexBufferHard->isInvalid())
    return true;
  // render primitives.
  //==============================
  // start multipass.
  ...
  for (uint pass = 0; pass < nPass; pass++) {
    ...
    if (nlines) {
      if (_LastIB._Format == CIndexBuffer::Indices16) {
        glDrawElements(GL_LINES, 2 * nlines, GL_UNSIGNED_SHORT,
                       ((uint16 *)_LastIB._Values) + firstIndex);
      } else {
#ifdef USE_OPENGLES
        nlerror("not available in OpenGL ES 1.0, only use 16 bits indices");
#else
        nlassert(_LastIB._Format == CIndexBuffer::Indices32);
        glDrawElements(GL_LINES, 2 * nlines, GL_UNSIGNED_INT,
                       ((uint32 *)_LastIB._Values) + firstIndex);
#endif
      }
    }
  }
...
bool CDriverGL::renderTriangles(CMaterial &mat, uint32 firstIndex,
                                uint32 ntris) {
  ...
  for (uint pass = 0; pass < nPass; pass++) {
    ...
    if (ntris) {
      if (_LastIB._Format == CIndexBuffer::Indices16) {
        glDrawElements(GL_TRIANGLES, 3 * ntris, GL_UNSIGNED_SHORT,
                       ((uint16 *)_LastIB._Values) + firstIndex);
      } else {
#ifdef USE_OPENGLES
        nlerror("not available in OpenGL ES 1.0, only use 16 bits indices");
...
bool CDriverGL::renderSimpleTriangles(uint32 firstTri, uint32 ntris) {
  ...
  if (_LastIB._Format == CIndexBuffer::Indices16) {
    glDrawElements(GL_TRIANGLES, 3 * ntris, GL_UNSIGNED_SHORT,
                   ((uint16 *)_LastIB._Values) + firstTri);
  } else {
#ifdef USE_OPENGLES
    nlerror("not available in OpenGL ES 1.0, only use 16 bits indices");
#else
    nlassert(_LastIB._Format == CIndexBuffer::Indices32);
    glDrawElements(GL_TRIANGLES, 3 * ntris, GL_UNSIGNED_INT,
                   ((uint32 *)_LastIB._Values) + firstTri);
#endif
  }
...
bool CDriverGL::renderRawPoints(CMaterial &mat, uint32 startIndex,
                                uint32 numPoints) {
...
bool CDriverGL::renderRawLines(CMaterial &mat, uint32 startIndex,
                               uint32 numLines) {
...
bool CDriverGL::renderRawTriangles(CMaterial &mat, uint32 startIndex,
                                   uint32 numTris) {
...
bool CDriverGL::renderRawQuads(CMaterial &mat, uint32 startIndex,
                               uint32 numQuads) {
  ...
  for (uint pass = 0; pass < nPass; pass++) {
    ...
    if (startIndex < QUAD_BATCH_SIZE) {
      ...
      glDrawElements(GL_TRIANGLES, 6 * numQuadsToDraw, GL_UNSIGNED_SHORT,
                     defaultIndices + 6 * startIndex);
    ...
    while (numLeftQuads) {
      // TODO : resetting vertex pointer would avoid the need to rebuild indices
      // each times
      ...
      if (4 * numQuadsToDraw + currIndex <= (1 << 16)) {
        ...
        glDrawElements(GL_TRIANGLES, 6 * numQuadsToDraw, GL_UNSIGNED_INT,
                       indices);
#endif
      }
      numLeftQuads -= numQuadsToDraw;
...
void CDriverGL::setupUVPtr(uint stage, CVertexBufferInfo &VB, uint uvId) {
  ...
  if (VB.VertexFormat & (CVertexBuffer::TexCoord0Flag << uvId)) {
    ...
    if (uvType == CVertexBuffer::Float2 || uvType == CVertexBuffer::Float3) {
      ...
      case CVertexBufferInfo::HwATI:
        nglArrayObjectATI(
            GL_TEXTURE_COORD_ARRAY, numTexCoord, GL_FLOAT, VB.VertexSize,
            VB.VertexObjectId,
      ...
      case CVertexBufferInfo::HwARB:
        ...
        glTexCoordPointer(numTexCoord, GL_FLOAT, VB.VertexSize,
                          VB.ValuePtr[CVertexBuffer::TexCoord0 + uvId]);
        break;
...
// ***************************************************************************
IVertexBufferHardGL *
CDriverGL::createVertexBufferHard(uint size, uint numVertices,
...
void CDriverGL::setupGlArraysStd(CVertexBufferInfo &vb) {
  ...
  case CVertexBufferInfo::HwARB: {
    ...
    //-----------
    uint numVertexCoord =
        CVertexBuffer::NumComponentsType[vb.Type[CVertexBuffer::Position]];
    nlassert(numVertexCoord >= 2);
    _DriverGLStates.enableVertexArray(true);
    glVertexPointer(numVertexCoord, GL_FLOAT, vb.VertexSize,
                    vb.ValuePtr[CVertexBuffer::Position]);
    ...
    if (flags & CVertexBuffer::NormalFlag) {
      ...
      glNormalPointer(GL_FLOAT, vb.VertexSize,
                      vb.ValuePtr[CVertexBuffer::Normal]);
    } else {
    ...
    if (flags & CVertexBuffer::PrimaryColorFlag) {
      ...
      // Setup ATI VBHard or std ptr.
      glColorPointer(4, GL_UNSIGNED_BYTE, vb.VertexSize,
                     vb.ValuePtr[CVertexBuffer::PrimaryColor]);
    } else {
  ...
  case CVertexBufferInfo::HwATI: {
    ...
    nglArrayObjectATI(GL_VERTEX_ARRAY, numVertexCoord, GL_FLOAT, vb.VertexSize,
                      vb.VertexObjectId,
                      (ptrdiff_t)vb.ValuePtr[CVertexBuffer::Position]);
    // setup normal ptr.
    //-----------
    // Check for normal param in vertex buffer
    if (flags & CVertexBuffer::NormalFlag) {
      ...
      nglArrayObjectATI(GL_NORMAL_ARRAY, 3, GL_FLOAT, vb.VertexSize,
                        vb.VertexObjectId,
                        (ptrdiff_t)vb.ValuePtr[CVertexBuffer::Normal]);
    } else {
      _DriverGLStates.enableNormalArray(false);
    }
    ...
    if (flags & CVertexBuffer::PrimaryColorFlag) {
      ...
      nglArrayObjectATI(GL_COLOR_ARRAY, 4, GL_UNSIGNED_BYTE, vb.VertexSize,
                        vb.VertexObjectId,
                        (ptrdiff_t)vb.ValuePtr[CVertexBuffer::PrimaryColor]);
    } else {
      _DriverGLStates.enableColorArray(false);
    }
  } break;
...
#endif
    {
      // fix for ATI : when switching from Vertex Program to fixed Pipe, must
      // clean texture, otherwise texture may be disabled in next render (seems
      // to be a driver bug)
  ...
  if (_LastSetupGLArrayVertexProgram && !isVertexProgramEnabled()) {
    ...
    if (vp) {
      CVertexProgamDrvInfosGL *drvInfo =
          NLMISC::safe_cast<CVertexProgamDrvInfosGL *>(
              (IProgramDrvInfos *)vp->m_DrvInfo);
      if (drvInfo) {
        // Disable all VertexAttribs.
        for (uint value = 0; value < CVertexBuffer::NumValue; value++) {
          _DriverGLStates.enableVertexAttribArrayForEXTVertexShader(
              value, false, drvInfo->Variants);
        }
      }
    }
    // no more a vertex program setup.
...
void CDriverGL::setupGlArraysForNVVertexProgram(CVertexBufferInfo &vb) {
  ...
  for (uint value = 0; value < CVertexBuffer::NumValue; value++) {
    ...
    if (flags & flag) {
      /* OpenGL Driver Bug with VertexProgram, UChar4 type, and
         VertexArrayRange. Don't work and lead to very poor performance (1/10)
         (VAR is "disabled").
      */
      // Test if can use glColorPointer() / glSecondaryColorPointerEXT()
      // instead.
...
#ifndef USE_OPENGLES
            nglSecondaryColorPointerEXT(4, GL_UNSIGNED_BYTE, vb.VertexSize,
                                        vb.ValuePtr[value]);
#endif
          }
        } else {
...
#ifndef USE_OPENGLES
          nglVertexAttribPointerNV(glIndex, NumCoordinatesType[type],
                                   GLType[type], vb.VertexSize,
                                   vb.ValuePtr[value]);
#endif
        }
      }
      ...
      _DriverGLStates.enableVertexAttribArray(glIndex, false);
      /* OpenGL Driver Bug with VertexProgram, UChar4 type, and
         VertexArrayRange. Must also disable colorArray in standard gl calls.
      */
      if (glIndex == 3)
        _DriverGLStates.enableColorArray(false);
      else if (glIndex == 4)
...
// tells for each vertex argument if it must be normalized when it is an
// integral type
...
void CDriverGL::setupGlArraysForARBVertexProgram(CVertexBufferInfo &vb) {
  ...
  nlctassert(CVertexBuffer::NumValue ==
             sizeof(ARBVertexProgramMustNormalizeAttrib) /
                 sizeof(ARBVertexProgramMustNormalizeAttrib[0]));
      ...
      CVertexBuffer::TType type = vb.Type[value];
      {
        ...
        if (flags & flag) {
          ...
          nglVertexAttribArrayObjectATI(
              glIndex, NumCoordinatesType[type], GLType[type], mustNormalize,
              vb.VertexSize, vb.VertexObjectId, (ptrdiff_t)vb.ValuePtr[value]);
        } else {
          _DriverGLStates.enableVertexAttribArrayARB(glIndex, false);
        }
      }
...
#ifdef USE_OPENGLES
          glVertexPointer(NumCoordinatesType[type], GLType[type], vb.VertexSize,
                          vb.ValuePtr[value]);
#else
          nglVertexAttribPointerARB(glIndex, NumCoordinatesType[type],
                                    GLType[type], mustNormalize, vb.VertexSize,
                                    vb.ValuePtr[value]);
#endif
        } else {
          _DriverGLStates.enableVertexAttribArrayARB(glIndex, false);
    ...
    if (flags & flag & drvInfo->UsedVertexComponents) {
      ...
      if (vb.VBMode == CVertexBufferInfo::HwATI) {
        ...
        case CVertexBuffer::Position: // position
        {
          ...
          nglArrayObjectATI(GL_VERTEX_ARRAY, NumCoordinatesType[type],
                            GLType[type], vb.VertexSize, vb.VertexObjectId,
                            (ptrdiff_t)vb.ValuePtr[CVertexBuffer::Position]);
        } break;
        case CVertexBuffer::Weight: // skin weight
        {
          nlassert(NumCoordinatesType[type] ==
                   4); // variant, only 4 component supported
          nglVariantArrayObjectATI(
              drvInfo->Variants[CDriverGL::EVSSkinWeightVariant], GLType[type],
              vb.VertexSize, vb.VertexObjectId,
              (ptrdiff_t)vb.ValuePtr[CVertexBuffer::Weight]);
        } break;
        case CVertexBuffer::Normal: // normal
        {
          nlassert(NumCoordinatesType[type] ==
                   3); // must have 3 components for normals
          nglArrayObjectATI(GL_NORMAL_ARRAY, 3, GLType[type], vb.VertexSize,
                            vb.VertexObjectId, (ptrdiff_t)vb.ValuePtr[value]);
        } break;
        case CVertexBuffer::PrimaryColor: // color
        {
          ...
          nglArrayObjectATI(
              GL_COLOR_ARRAY, NumCoordinatesType[type], GLType[type],
              vb.VertexSize, vb.VertexObjectId,
              (ptrdiff_t)vb.ValuePtr[CVertexBuffer::PrimaryColor]);
        } break;
        case CVertexBuffer::SecondaryColor: // secondary color
        {
          ...
          nlassert(NumCoordinatesType[type] ==
                   4); // variant, only 4 component supported
          nglVariantArrayObjectATI(
              drvInfo->Variants[CDriverGL::EVSSecondaryColorVariant],
              GLType[type], vb.VertexSize, vb.VertexObjectId,
              (ptrdiff_t)vb.ValuePtr[CVertexBuffer::SecondaryColor]);
        } break;
        case CVertexBuffer::Fog: // fog coordinate
        {
          ...
          nlassert(NumCoordinatesType[type] ==
                   4); // variant, only 4 component supported
          nglVariantArrayObjectATI(
              drvInfo->Variants[CDriverGL::EVSFogCoordsVariant], GLType[type],
              vb.VertexSize, vb.VertexObjectId,
              (ptrdiff_t)vb.ValuePtr[CVertexBuffer::Fog]);
        } break;
        case CVertexBuffer::PaletteSkin: // palette skin
        {
          ...
          nlassert(NumCoordinatesType[type] ==
                   4); // variant, only 4 component supported
          nglVariantArrayObjectATI(
              drvInfo->Variants[CDriverGL::EVSPaletteSkinVariant], GLType[type],
              vb.VertexSize, vb.VertexObjectId,
              (ptrdiff_t)vb.ValuePtr[CVertexBuffer::PaletteSkin]);
        } break;
        ...
        case CVertexBuffer::TexCoord7: {
          _DriverGLStates.clientActiveTextureARB(value -
                                                 CVertexBuffer::TexCoord0);
          nglArrayObjectATI(GL_TEXTURE_COORD_ARRAY, NumCoordinatesType[type],
                            GLType[type], vb.VertexSize, vb.VertexObjectId,
                            (ptrdiff_t)vb.ValuePtr[value]);
        } break;
        default:
...
#endif
      {
        ...
        case CVertexBuffer::Weight: // skin weight
        {
          ...
          nglVariantPointerEXT(
              drvInfo->Variants[CDriverGL::EVSSkinWeightVariant], GLType[type],
              vb.VertexSize, vb.ValuePtr[value]);
#endif
        } break;
        case CVertexBuffer::Normal: // normal
        {
          nlassert(NumCoordinatesType[type] ==
                   3); // must have 3 components for normals
          glNormalPointer(GLType[type], vb.VertexSize,
                          vb.ValuePtr[CVertexBuffer::Normal]);
        } break;
        case CVertexBuffer::PrimaryColor: // color
        {
          nlassert(NumCoordinatesType[type] >=
                   3); // must have 3 or 4 components for primary color
          glColorPointer(NumCoordinatesType[type], GLType[type], vb.VertexSize,
                         vb.ValuePtr[value]);
        } break;
        case CVertexBuffer::SecondaryColor: // secondary color
        {
          ...
          nglVariantPointerEXT(
              drvInfo->Variants[CDriverGL::EVSSecondaryColorVariant],
              GLType[type], vb.VertexSize, vb.ValuePtr[value]);
#endif
        } break;
        case CVertexBuffer::Fog: // fog coordinate
        {
        ...
        case CVertexBuffer::TexCoord7: {
          _DriverGLStates.clientActiveTextureARB(value -
                                                 CVertexBuffer::TexCoord0);
          glTexCoordPointer(NumCoordinatesType[type], GLType[type],
                            vb.VertexSize, vb.ValuePtr[value]);
        } break;
        default:
          nlstop; // invalid value
...
bool CDriverGL::initVertexBufferHard(uint agpMem, uint vramMem) {
  ...
  if (agpMem > 0) {
    ...
    while (agpMem >= NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      if (_AGPVertexArrayRange->allocate(agpMem, CVertexBuffer::AGPPreferred)) {
        ...
        nlinfo("3D: Success to allocate %.1f Mo of AGP VAR Ram",
               agpMem / 1000000.f);
        break;
      } else {
        agpMem /= 2;
    ...
    if (agpMem < NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      nlinfo("3D: %.u vertices supported", _MaxVerticesByVBHard);
      nlinfo("3D: Failed to allocate %.1f Mo of AGP VAR Ram",
  ...
  if (vramMem > 0) {
    ...
    while (vramMem >= NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      if (_VRAMVertexArrayRange->allocate(vramMem,
...
void CDriverGL::fenceOnCurVBHardIfNeeded(IVertexBufferHardGL *newVBHard) {
  ...
  if (_CurrentVertexBufferHard == NULL ||
      _CurrentVertexBufferHard->VBType != IVertexBufferHardGL::NVidiaVB)
    return;
  ...
  if (_CurrentVertexBufferHard != newVBHard) {
    ...
    CVertexBufferHardGLNVidia *vbHardNV =
        static_cast<CVertexBufferHardGLNVidia *>(_CurrentVertexBufferHard);
    ...
    if (vbHardNV->GPURenderingAfterFence) {
      /*
          ...
          NB: if the fence was previously set. NV_Fence Specification says that
         the new ONE replaces it. This is EXACTLY what we wants, since the old
         one is no more interesting.

          NB: never insert a fence for said "Static Lock" VBHard. Those VBHard
         are said to be "static" therefore, user should never modify them (else
         lock() is much slower...)
      */
      if (!vbHardNV->getLockHintStatic())
        vbHardNV->setFence();
      // Since we have set a new Fence, we won't need to do it at next
      // vbHardNV->lock()

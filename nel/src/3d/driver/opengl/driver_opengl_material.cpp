...
#ifndef USE_OPENGLES
  case CMaterial::blendConstantColor:
    glenum = GL_CONSTANT_COLOR_EXT;
    break;
  case CMaterial::blendConstantInvColor:
    glenum = GL_ONE_MINUS_CONSTANT_COLOR_EXT;
    break;
  case CMaterial::blendConstantAlpha:
    glenum = GL_CONSTANT_ALPHA_EXT;
    break;
  case CMaterial::blendConstantInvAlpha:
...
static inline void
convTexAddr(ITexture *tex, CMaterial::TTexAddressingMode mode, GLenum &glenum) {
...
void CDriverGL::setupUserTextureMatrix(uint numStages, CMaterial &mat) {
  ...
  if ((_UserTexMatEnabled != 0 &&
    ...
    uint newMask = (mat.getFlags() & IDRV_MAT_USER_TEX_MAT_ALL) >>
    ...
    for (uint k = 0; k < numStages; ++k) {
      if (newMask & shiftMask) // user matrix for this stage
      {
        ...
        if ((newMask & shiftMask) != (_UserTexMatEnabled & shiftMask)) {
          _DriverGLStates.activeTextureARB(k);
          glLoadIdentity();
...
CMaterial::TShader CDriverGL::getSupportedShader(CMaterial::TShader shader) {
  ...
  case CMaterial::PerPixelLighting:
    return _SupportPerPixelShader ? CMaterial::PerPixelLighting
                                  : CMaterial::Normal;
  case CMaterial::PerPixelLightingNoSpec:
    return _SupportPerPixelShaderNoSpec ? CMaterial::PerPixelLightingNoSpec
                                        : CMaterial::Normal;
...
// --------------------------------------------------
void CDriverGL::setTextureShaders(const uint8 *addressingModes,
                                  const CSmartPtr<ITexture> *textures) {
  H_AUTO_OGL(CDriverGL_setTextureShaders)
  GLenum glAddrMode;
  for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; ++stage) {
    convTexAddr(textures[stage],
                (CMaterial::TTexAddressingMode)addressingModes[stage],
                glAddrMode);

    if (glAddrMode != _CurrentTexAddrMode[stage]) // addressing mode different
                                                  // from the one in the device?
    {
      _DriverGLStates.activeTextureARB(stage);
#ifndef USE_OPENGLES
...
bool CDriverGL::setupMaterial(CMaterial &mat) {
  ...
  if (touched) {
    ...
    */
    ...
    if (touched & (~_MaterialAllTextureTouchedFlag)) {
      ...
      // Since modified, must rebind all openGL states. And do this also for the
      // delete/new problem.
      /* If an old material is deleted, _CurrentMaterial is invalid. But this is
         grave only if a new material is created, with the same pointer (bad
         luck). Since an newly allocated material always pass here before use,
         we are sure to avoid any problems.
      */
  ...
  if (matShader != CMaterial::Water &&
      ((matShader != CMaterial::Program) ||
       (_LastSetuppedPP->features().MaterialFlags &
  ...
  if (matShader != CMaterial::LightMap &&
      ...
      && matShader != CMaterial::Cloud && matShader != CMaterial::Water &&
...
sint CDriverGL::beginMultiPass() {
  ...
  // Depending on material type and hardware, return number of pass required to
  // draw this material.
...
void CDriverGL::computeLightMapInfos(const CMaterial &mat) {
  ...
  // Compute number of lightmaps really used (ie factor not NULL), and build the
  // LUT.
  _NLightMaps = 0;
  ...
  // Can do more than 2 texture stages only if NVTextureEnvCombine4 or
...
sint CDriverGL::beginLightMapMultiPass() {
  ...
  // if the dynamic lightmap light has changed since the last render (should not
  // happen), resetup normal way is that setupLightMapDynamicLighting() is
  // called in setupMaterial() if shader different from prec
  ...
  // lightmap get no specular/ambient. Emissive and Diffuse are setuped in
  // setupLightMapPass()
...
void CDriverGL::setupLightMapPass(uint pass) {
  ...
  if (_NLightMaps == 0) {
    ...
    // setup color to 0 => blackness. in emissive cause texture can still be
    // lighted by dynamic light
  ...
  // For LMC (lightmap 8Bit compression) compute the total AmbientColor in
  // vertex diffuse need only if standard MulADD version
  ...
  for (uint stage = 0; stage < inlGetNumTextStages(); stage++) {
    ...
    if (stage < nstages - 1) {
      ...
      if (text) {
        ...
        // fallBack if extension MulAdd not found. just mul factor with
        // (Ambient+Diffuse)
        if (_LightMapNoMulAddFallBack) {
          // do not use constant color to blend lightmap, but incoming diffuse
          // color, for stage0 only.
          GLfloat glcol[4];
          ...
          if (_CurrentTexEnvSpecial[stage] != TexEnvSpecialLightMap) {
            ...
            if (_Extensions.NVTextureEnvCombine4) {
              ...
              glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND3_RGB_NV,
      ...
      // optim: do this only for first pass, and last pass only if
      // stage!=nLMapPerPass (meaning not the same stage as preceding passes).
      if (pass == 0 ||
          (pass == _NLightMapPass - 1 && stage != _NLightMapPerPass)) {
        // activate the texture at last stage.
        ITexture *text = mat.getTexture(0);
        activateTexture(stage, text);

        // setup ModulateRGB/ReplaceAlpha env. (this may disable possible
        // COMBINE4_NV setup).
  ...
  /* If multi-pass, then must setup a black Fog color for 1+ pass (just do it
     for the pass 1). This is because Transparency ONE/ONE is used.
  */
  ...
  if (!mat.getBlend()) {
    ...
    if (pass == 0) {
      ...
      // setup an Additive transparency (only for pass 1, will be kept for
      // successives pass).
    ...
    /* 1st pass, std alphaBlend. 2nd pass, add to background. Demo:
        ...
        l1: lightmap (or group of lightmap) of pass 1. (same thing with 2,3
       etc....) B:	Background. A:	Alpha of texture.

        finalResult= T*(l0+l1) * A + B * (1-A).

        We get it in two pass:
            fint=			T*l0 * A + B * (1-A).
            finalResult=	T*l1 * A + fint = T*l1 * A + T*l0 * A + B *
       (1-A)= T* (l0+l1) * A + B * (1-A)
    */
    if (pass == 0) {
      // no transparency for first pass.
  ...
  // Dynamic lighting: The influence of the dynamic light must be added only in
  // the first pass (only one time)
  if (pass == 0) {
    // If the lightmap is in x2 mode, then must divide effect of the dynamic
    // light too
    if (mat._LightMapsMulx2)
...
void CDriverGL::endLightMapMultiPass() {
  ...
  // Flag the fact that VertexSetup is dirty (special lightmap). reseted in
  // activeVertexBuffer(), and setupMaterial() NB: if no lightmaps, no
  // setupUVPtr() has been called => don't need to flag (important else crash if
  // graphist error while exporting a Lightmap material, with a
  // MeshVertexProgram (WindTree) )
  if (_NLightMaps != 0)
  ...
  // nothing to do with blending/lighting, since always setuped in
  // activeMaterial(). If material is the same, then it is still a lightmap
  // material (if changed => touched => different!) So no need to reset
  // blending/lighting here.
...
void CDriverGL::setupSpecularBegin() {
  ...
  // NB don't setup the TexEnv here (stage1 setuped in setupSpecularPass()
  // according to extensions) For all cases, setup the TexCoord gen for stage1
...
sint CDriverGL::beginSpecularMultiPass() {
  ...
  // Manage the rare case when the SpecularMap is not provided (fault of
  // graphist).
  ...
  if (_Extensions.NVTextureEnvCombine4 ||
      _Extensions.ATITextureEnvCombine3) // NVidia or ATI optimization
    return 1;
  else
    return 2;
}
...
void CDriverGL::setupSpecularPass(uint pass) {
  ...
  // Manage the rare case when the SpecularMap is not provided (error of a
  // graphist).
  if (mat.getTexture(1) == NULL) {
  ...
  if (_Extensions.NVTextureEnvCombine4) { // Ok we can do it in a single pass
...
// -DDEBUG_OGL_COMBINE43_DISABLE=ON disables
...
// a functor that can is used to generate a cube map used for specular / diffuse
// lighting
struct CSpecCubeMapFunctor : ICubeMapFunctor {
  ...
  virtual NLMISC::CRGBA operator()(const NLMISC::CVector &v) {
    ...
    // return Exp == 1.f ? CRGBA((uint8)(v.x*127+127), (uint8)(v.y*127+127),
    // (uint8)(v.z*127+127), 0): CRGBA::Black;
  }
...
CTextureCube *CDriverGL::getSpecularCubeMap(uint exp) {
  ...
  static float cubeMapExp[] = {1.f, 4.f, 8.f, 24.f, 48.f, 128.f, 256.f, 511.f};
  ...
  if (_SpecularTextureCubes[cubeMapIndex] !=
      NULL) // has the cube map already been cted ?
  {
  ...
  } else // build the cube map
  {
    ...
    static const CTextureCube::TFace numToFace[] = {
        CTextureCube::positive_x, CTextureCube::negative_x,
...
#ifdef NL_DEBUG
  nlassert(
      supportPerPixelLighting(false)); // make sure the hardware can do that
#endif
  return 1;
}
...
void CDriverGL::setupPPLNoSpecPass(uint pass) {
  ...
  // Stage 0 is rgb = DiffuseCubeMap * LightColor + DiffuseGouraud * 1 (TODO :
  // EnvCombine3)
  if (_CurrentTexEnvSpecial[0] != TexEnvSpecialPPLStage0) {
    // TexEnv is special.
    _CurrentTexEnvSpecial[0] = TexEnvSpecialPPLStage0;
...
void CDriverGL::setupWaterPassARB(const CMaterial &mat) {
  ...
  nglBindProgramARB(GL_FRAGMENT_PROGRAM_ARB,
                    ARBWaterShader[(_FogEnabled ? 1 : 0) |
                                   (mat.getTexture(3) != NULL ? 2 : 0)]);
  glEnable(GL_FRAGMENT_PROGRAM_ARB);

  // setup the constant
  if (mat.getTexture(0) && mat.getTexture(0)->isBumpMap()) {
    float factor = 0.25f * NLMISC::safe_cast<CTextureBump *>(mat.getTexture(0))
                               ->getNormalizationFactor();
    nglProgramEnvParameter4fARB(
        GL_FRAGMENT_PROGRAM_ARB, 0, 2.f * factor, -1.f * factor, 0.f,
        0.f); // scale_bias from [0, 1] to [-1, 1] and factor applied
  } else {
    nglProgramEnvParameter4fARB(
        GL_FRAGMENT_PROGRAM_ARB, 0, 2.f, -1.f, 0.f,
        0.f); // scale_bias from [0, 1] to [-1, 1] and factor applied
  }
  ...
  if (mat.getTexture(1) && mat.getTexture(1)->isBumpMap()) {
    float factor = NLMISC::safe_cast<CTextureBump *>(mat.getTexture(1))
                       ->getNormalizationFactor();
    nglProgramEnvParameter4fARB(
        GL_FRAGMENT_PROGRAM_ARB, 1, 2.f * factor, -1.f * factor, 0.f,
        0.f); // scale_bias from [0, 1] to [-1, 1] and factor applied
  } else {
    nglProgramEnvParameter4fARB(
        GL_FRAGMENT_PROGRAM_ARB, 1, 2.f, -1.f, 0.f,
        0.f); // scale_bias from [0, 1] to [-1, 1] and factor applied
  }

  if (_FogEnabled) {
    if (_FogStart == _FogEnd) {
      nglProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 2, 0.f, 0.f, 0.f,
                                  0.f);
    } else {
      /** Unfortunately, the EXT_vertex_shader extension has to output the fog
       * values in the [0, 1] range to work with the standard pipeline. So we
       * must add a special path for this case, where the fog coordinate is
       * 'unscaled' again. NB : this is fixed in later drivers
       * (from 6.14.10.6343), so check this
       */
      if (_Extensions.EXTVertexShader && !_ATIFogRangeFixed) {
        nglProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 2, 1.f, 0.f, 0.f,
                                    0.f);
      } else {
        nglProgramEnvParameter4fARB(GL_FRAGMENT_PROGRAM_ARB, 2,
                                    -1.f / (_FogEnd - _FogStart),
                                    _FogEnd / (_FogEnd - _FogStart), 0.f, 0.f);
      }
    }
  }
#endif
...
static const uint8 WaterNoDiffuseTexAddrMode[IDRV_MAT_MAXTEXTURES] = {
    CMaterial::FetchTexture, CMaterial::OffsetTexture, CMaterial::OffsetTexture,
...
void CDriverGL::setupWaterPassNV20(const CMaterial &mat) {
  ...
  nlctassert(IDRV_MAT_MAXTEXTURES ==
             4); // if this value changes, may have to change the arrays
                 // WaterNoDiffuseTexAddrMode & WaterTexAddrMode
...
#ifndef USE_OPENGLES
  nlassert(_CurrentMaterial->getShader() == CMaterial::Water);
  // NB : as fragment shaders / programs bypass the texture envs, no special env
  // enum is added (c.f CTexEnvSpecial)
  if (_Extensions.NVTextureShader)
    return;
  if (ARBWaterShader[0]) {

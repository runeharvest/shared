...
// ***************************************************************************
IVertexBufferHardGL::IVertexBufferHardGL(CDriverGL *drv, CVertexBuffer *vb)
    : VB(vb) {
...
// ***************************************************************************
CVertexArrayRangeNVidia::CVertexArrayRangeNVidia(CDriverGL *drv)
    : IVertexArrayRange(drv) {
...
// ***************************************************************************
bool CVertexArrayRangeNVidia::allocate(uint32 size,
  ...
  // init the allocator.
  if (_VertexArrayPtr) {
    /* Init with an alignment of 8. Not sure it is useful, but
      GDC01_Performance.pdf papers talks about "Data arrays must be 8-byte
      aligned". Don't know what "Data" is.
    */
...
void CVertexArrayRangeNVidia::enable() {
  ...
  if (_Driver->_CurrentVertexArrayRange != this) {
    ...
    if (_Driver->_NVCurrentVARSize != _VertexArraySize ||
        _Driver->_NVCurrentVARPtr != _VertexArrayPtr) {
      // Yoyo: NVidia Bug ?? must do this strange thing, this ensure that it is
      // correclty setuped.
      glFinish();
...
// ***************************************************************************
IVertexBufferHardGL *
CVertexArrayRangeNVidia::createVBHardGL(uint size, CVertexBuffer *vb) {
  H_AUTO_OGL(CVertexArrayRangeNVidia_createVBHardGL)
  // create a NVidia VBHard
  CVertexBufferHardGLNVidia *newVbHard =
      new CVertexBufferHardGLNVidia(_Driver, vb);
...
// ***************************************************************************
CVertexBufferHardGLNVidia::CVertexBufferHardGLNVidia(CDriverGL *drv,
                                                     CVertexBuffer *vb)
    : IVertexBufferHardGL(drv, vb) {
...
// ***************************************************************************
void CVertexBufferHardGLNVidia::initGL(CVertexArrayRangeNVidia *var,
                                       void *vertexPtr) {
...
void *CVertexBufferHardGLNVidia::lock() {
  ...
  // If the user lock an activated VBHard, after rendering some primitives, we
  // must stall the CPU Also always do this for Static Lock VBHard (because no
  // setFence are inserted after rendering
  if (GPURenderingAfterFence || _LockHintStatic) {
    // Set a new fence at the current position in the command stream, replacing
    // the old one
    setFence();
...
void CVertexBufferHardGLNVidia::setFence() {
  ...
  // NB: if the Fence was set and not finished, then the new one will KICK the
  // old.
...
void CVertexBufferHardGLNVidia::finishFence() {
  ...
  if (isFenceSet()) {
    // Stall CPU while the fence command is not reached in the GPU command
    // stream.
...
// ***************************************************************************
CVertexArrayRangeATI::CVertexArrayRangeATI(CDriverGL *drv)
    : IVertexArrayRange(drv) {
...
// ***************************************************************************
bool CVertexArrayRangeATI::allocate(uint32 size,
                                    CVertexBuffer::TPreferredMemory vbType) {
  H_AUTO_OGL(CVertexArrayRangeATI_allocate)
  nlassert(!_Allocated);

  // try to allocate AGP (suppose mean ATI dynamic) or VRAM (suppose mean ATI
  // static) data.
  switch (vbType) {
  case CVertexBuffer::AGPPreferred:
...
// ***************************************************************************
IVertexBufferHardGL *CVertexArrayRangeATI::createVBHardGL(uint size,
                                                          CVertexBuffer *vb) {
...
// ***************************************************************************
CVertexBufferHardGLATI::CVertexBufferHardGLATI(CDriverGL *drv,
                                               CVertexBuffer *vb)
    : IVertexBufferHardGL(drv, vb) {
...
// ***************************************************************************
void CVertexBufferHardGLATI::initGL(CVertexArrayRangeATI *var,
                                    void *vertexPtr) {
...
void CVertexBufferHardGLATI::unlock() {
  ...
  nglUpdateObjectBufferATI(getATIVertexObjectId(),
                           (ptrdiff_t)_VertexPtr - NL3D_DRV_ATI_FAKE_MEM_START,
                           _RAMMirrorVertexSize, _RAMMirrorVertexPtr,
                           GL_PRESERVE_ATI);
}
...
void CVertexBufferHardGLATI::unlock(uint start, uint end) {
  ...
  uint dstOffStart =
      ((ptrdiff_t)_VertexPtr - NL3D_DRV_ATI_FAKE_MEM_START) + srcOffStart;
  // copy.
  nglUpdateObjectBufferATI(getATIVertexObjectId(), dstOffStart, size,
                           (uint8 *)_RAMMirrorVertexPtr + srcOffStart,
                           GL_PRESERVE_ATI);
}
...
// ***************************************************************************
CVertexArrayRangeMapObjectATI::CVertexArrayRangeMapObjectATI(CDriverGL *drv)
    : IVertexArrayRange(drv), _VBType(CVertexBuffer::AGPPreferred),
      _SizeAllocated(0) {
  H_AUTO_OGL(CVertexArrayRangeMapObjectATI_CVertexArrayRangeMapObjectATI)
}

// ***************************************************************************
bool CVertexArrayRangeMapObjectATI::allocate(
    uint32 size, CVertexBuffer::TPreferredMemory vbType) {
...
// ***************************************************************************
IVertexBufferHardGL *
CVertexArrayRangeMapObjectATI::createVBHardGL(uint size, CVertexBuffer *vb) {
  H_AUTO_OGL(CVertexArrayRangeMapObjectATI_createVBHardGL)
  // create a ATI VBHard
  CVertexBufferHardGLMapObjectATI *newVbHard =
      new CVertexBufferHardGLMapObjectATI(_Driver, vb);
  uint vertexObjectId;
...
CVertexBufferHardGLMapObjectATI::CVertexBufferHardGLMapObjectATI(
    CDriverGL *drv, CVertexBuffer *vb)
    : IVertexBufferHardGL(drv, vb), _VertexPtr(NULL), _VertexArrayRange(NULL),
      _VertexObjectId(0) {
...
CVertexBufferHardGLMapObjectATI::~CVertexBufferHardGLMapObjectATI() {
  H_AUTO_OGL(
      CVertexBufferHardGLMapObjectATI_CVertexBufferHardGLMapObjectATIDtor)
  if (_VertexObjectId)
    nglFreeObjectBufferATI(_VertexObjectId);
#ifdef NL_DEBUG
  if (_VertexPtr) {
  ...
  if (_VertexArrayRange) {
    if (_Invalid) {
      if (VB->getLocation() != CVertexBuffer::NotResident) {
        // when the vb is put in tthe NotResident state, it is removed from that
        // list
...
void *CVertexBufferHardGLMapObjectATI::lock() {
  ...
  if (_VertexPtr)
    return _VertexPtr; // already locked
  if (_Invalid) {
    if (VB->getLocation() != CVertexBuffer::NotResident) {
...
// ***************************************************************************
void CVertexBufferHardGLMapObjectATI::unlock(uint /* start */,
                                             uint /* endVert */) {
  H_AUTO_OGL(CVertexBufferHardGLMapObjectATI_unlock)
...
// ***************************************************************************
void CVertexBufferHardGLMapObjectATI::initGL(CVertexArrayRangeMapObjectATI *var,
                                             uint vertexObjectID) {
...
void CVertexArrayRangeMapObjectATI::updateLostBuffers() {
  ...
  // Put all vb that have been lost in the NotResident state so that they will
  // be recomputed We do this only if the app is active, because if vb were
  // lost, it is likely that there are no resources available.
  nlassert(_Driver);
  if (_Driver->isWndActive()) {
    for (std::list<CVertexBufferHardGLMapObjectATI *>::iterator it =
             _LostVBList.begin();
         it != _LostVBList.end(); ++it) {
...
void CVertexArrayRangeMapObjectATI::dumpMappedBuffers() {
  ...
  for (std::list<CVertexBufferHardGLMapObjectATI *>::iterator it =
           _MappedVBList.begin();
       it != _MappedVBList.end(); ++it) {
    CVertexBufferHardGLMapObjectATI &vbati = **it;
    nlwarning("Buffer id = %u, size = %u, address = %p", vbati._VertexObjectId,
              vbati.VB->getVertexSize() * vbati.VB->getNumVertices(),
              vbati.getPointer());
  }
}
#endif
...
// ***************************************************************************
CVertexArrayRangeARB::CVertexArrayRangeARB(CDriverGL *drv)
    : IVertexArrayRange(drv), _VBType(CVertexBuffer::AGPPreferred),
      _SizeAllocated(0) {
...
// ***************************************************************************
bool CVertexArrayRangeARB::allocate(uint32 size,
                                    CVertexBuffer::TPreferredMemory vbType) {
...
// ***************************************************************************
IVertexBufferHardGL *CVertexArrayRangeARB::createVBHardGL(uint size,
                                                          CVertexBuffer *vb) {
...
void CVertexArrayRangeARB::updateLostBuffers() {
  ...
  nlassert(_Driver);
  if (_Driver->isWndActive()) {
    for (std::list<CVertexBufferHardARB *>::iterator it = _LostVBList.begin();
         it != _LostVBList.end(); ++it) {
      nlassert((*it)->_VertexObjectId);
...
// ***************************************************************************
CVertexBufferHardARB::CVertexBufferHardARB(CDriverGL *drv, CVertexBuffer *vb)
    : IVertexBufferHardGL(drv, vb), _VertexPtr(NULL), _VertexObjectId(0)
...
CVertexBufferHardARB::~CVertexBufferHardARB() {
  ...
  if (_Driver && _VertexObjectId) {
    if (_Driver->_DriverGLStates.getCurrBoundARBVertexBuffer() ==
        _VertexObjectId) {
      _Driver->_DriverGLStates.forceBindARBVertexBuffer(0);
    }
  }
...
void *CVertexBufferHardARB::lock() {
  ...
  if (_MemType == CVertexBuffer::AGPVolatile) {
    _VertexPtr =
        nglMapBufferRange(GL_ARRAY_BUFFER, 0, size,
...
// ***************************************************************************
void CVertexBufferHardARB::initGL(uint vertexObjectID,
                                  CVertexArrayRangeARB *var,
                                  CVertexBuffer::TPreferredMemory memType) {
...
void CVertexArrayRangeARB::dumpMappedBuffers() {
  nlwarning("*****************************************************");
  nlwarning("Mapped buffers :");
  for (std::list<CVertexBufferHardARB *>::iterator it = _MappedVBList.begin();
       it != _MappedVBList.end(); ++it) {
    CVertexBufferHardARB &vbarb = **it;
    nlwarning("Buffer id = %u, size = %u, address = %p", vbarb._VertexObjectId,
              vbarb.VB->getVertexSize() * vbarb.VB->getNumVertices(),
              vbarb.getPointer());
  }
}
#endif

...
#define NEL_MEASURE_UPLOAD_TIME_START                                          \
  NLMISC::TTicks startTick = CTime::getPerformanceTime();
#define NEL_MEASURE_UPLOAD_TIME_END                                            \
  NLMISC::TTicks endTick = CTime::getPerformanceTime();                        \
  nlinfo("3D: upload time = %.2f ms",                                          \
         (float)(1000 * (CTime::ticksToSecond(endTick) -                       \
                         CTime::ticksToSecond(startTick))));
#else
#define NEL_MEASURE_UPLOAD_TIME_START
#define NEL_MEASURE_UPLOAD_TIME_END
#endif
...
// ***************************************************************************
CTextureDrvInfosGL::CTextureDrvInfosGL(IDriver *drv, ItTexDrvInfoPtrMap it,
                                       CDriverGL *drvGl,
...
CTextureDrvInfosGL::~CTextureDrvInfosGL() {
  ...
  if (TextureUsedIdx < _Driver->_TextureUsed.size() &&
      _Driver->_TextureUsed[TextureUsedIdx] == this) {
...
CDepthStencilFBO::CDepthStencilFBO(CDriverGL *driver, uint width, uint height) {
  ...
  if (packedDepthStencil) {
    ...
    nglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH24_STENCIL8_EXT,
                              width, height);
  } else {
    nglBindRenderbufferEXT(GL_RENDERBUFFER_EXT, DepthFBOId);
    nglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_DEPTH_COMPONENT24, width,
                              height);
    /*
    nglBindRenderbufferEXT(GL_RENDERBUFFER_EXT, StencilFBOId);
    nglRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_STENCIL_INDEX8_EXT, width,
    height);
    */
  }
...
CDepthStencilFBO::~CDepthStencilFBO() {
  // driver remove
  m_Driver->_DepthStencilFBOs.erase(
      std::find(m_Driver->_DepthStencilFBOs.begin(),
                m_Driver->_DepthStencilFBOs.end(), this));
...
bool CTextureDrvInfosGL::initFrameBufferObject(ITexture *tex) {
  if (!InitFBO) {
    ...
    if (AttachDepthStencil) {
      for (std::vector<CDepthStencilFBO *>::iterator
               it(_Driver->_DepthStencilFBOs.begin()),
           end(_Driver->_DepthStencilFBOs.end());
           it != end; ++it) {
        if ((*it)->Width == tex->getWidth() &&
            (*it)->Height == tex->getHeight()) {
          DepthStencilFBO = (*it);
          break;
        }
      }
      if (!DepthStencilFBO) {
        DepthStencilFBO =
            new CDepthStencilFBO(_Driver, tex->getWidth(), tex->getHeight());
      }
      nglFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                                    GL_RENDERBUFFER_EXT,
                                    DepthStencilFBO->DepthFBOId);
      // nldebug("3D: glFramebufferRenderbufferExt(depth:24) = %X",
      // nglCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT));
      nglFramebufferRenderbufferEXT(
          GL_FRAMEBUFFER_EXT, GL_STENCIL_ATTACHMENT_EXT, GL_RENDERBUFFER_EXT,
          DepthStencilFBO->StencilFBOId);
      // nldebug("3D: glFramebufferRenderbufferExt(stencil:8) = %X",
      // nglCheckFramebufferStatusEXT(GL_FRAMEBUFFER_EXT));
    }
...
static inline CTextureDrvInfosGL *getTextureGl(ITexture &tex) {
  ...
  gltex = (CTextureDrvInfosGL *)(ITextureDrvInfos *)(tex.TextureDrvShare
                                                         ->DrvTexture);
  return gltex;
}
...
GLint CDriverGL::getGlTextureFormat(ITexture &tex, bool &compressed) {
  ...
  case ITexture::RGBA8888:
    return GL_RGBA8;
  case ITexture::RGBA4444:
    return GL_RGBA4;
  case ITexture::RGBA5551:
    return GL_RGB5_A1;
  case ITexture::RGB888:
    return GL_RGB8;
  case ITexture::RGB565:
    return GL_RGB5;
  case ITexture::Luminance:
...
static GLint getGlSrcTextureFormat(ITexture &tex, GLint glfmt) {
  ...
  if ((glfmt == GL_ALPHA8) || (glfmt == GL_LUMINANCE8_ALPHA8) ||
      (glfmt == GL_LUMINANCE8)) {
    switch (tex.getPixelFormat()) {
    case CBitmap::Alpha:
      return GL_ALPHA;
...
#endif
    void
    CDriverGL::bindTextureWithMode(ITexture &tex) {
  CTextureDrvInfosGL *gltext;
  gltext = getTextureGl(tex);
  // system of "backup the previous binded texture" seems to not work with some
  // drivers....
...
#endif
    void
    CDriverGL::setupTextureBasicParameters(ITexture &tex) {
  CTextureDrvInfosGL *gltext;
  gltext = getTextureGl(tex);
  // TODO: possible cache here, but beware, this is called just after texture
  // creation as well, so these fields haven't ever been filled.
  ...
  if (tex.isTextureCube()) {
    if (_Extensions.ARBTextureCubeMap) {
      glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_WRAP_S,
                      translateWrapToGl(ITexture::Clamp, _Extensions));
      ...
      if (_AnisotropicFilter > 1.f &&
          gltext->MinFilter > ITexture::NearestMipMapLinear)
        glTexParameteri(GL_TEXTURE_CUBE_MAP_ARB, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                        _AnisotropicFilter);
    }
  } else {
    glTexParameteri(gltext->TextureMode, GL_TEXTURE_WRAP_S,
                    translateWrapToGl(gltext->WrapS, _Extensions));
    glTexParameteri(gltext->TextureMode, GL_TEXTURE_WRAP_T,
                    translateWrapToGl(gltext->WrapT, _Extensions));
    glTexParameteri(gltext->TextureMode, GL_TEXTURE_MAG_FILTER,
                    translateMagFilterToGl(gltext));
    glTexParameteri(gltext->TextureMode, GL_TEXTURE_MIN_FILTER,
                    translateMinFilterToGl(gltext));

    if (_AnisotropicFilter > 1.f &&
        gltext->MinFilter > ITexture::NearestMipMapLinear)
      glTexParameteri(gltext->TextureMode, GL_TEXTURE_MAX_ANISOTROPY_EXT,
                      _AnisotropicFilter);
  }
  //
...
bool CDriverGL::setupTextureEx(ITexture &tex, bool bUpload, bool &bAllUploaded,
  ...
  // nldebug("3D: CDriverGL::setupTextureEx(%016p, %d, %d, %d)", &tex, bUpload,
  // bAllUploaded, bMustRecreateSharedTexture);
  bAllUploaded = false;
  ...
  if (!tex.TextureDrvShare) {
    ...
    ItTexDrvSharePtrList it = _TexDrvShares.insert(
        _TexDrvShares.end(), (NL3D::CTextureDrvShare *)NULL);
  ...
  if ((!tex.touched()) && (!mustCreate)) {
    ...
    if (tex.filterOrWrapModeTouched()) {
      ...
      //
      setupTextureBasicParameters(
          tex); // setup what has changed (will reset the touch flag)
  ...
  /* If an old texture is deleted, _CurrentTexture[*] and
     _CurrentTextureInfoGL[*] are invalid. But this is grave only if a new
     texture is created, with the same pointer (bad luck). Since an newly
     allocated texture always pass here before use, we are sure to avoid any
     problems.
  */
  ...
  if (tex.supportSharing()) {
    ...
    // Destructor of texture infos locks _SyncTexDrvInfos, so any existing
    // pointers must be deleted outside the lock
    ...
    // insert or get the texture.
    {
      ...
      if (itTex == rTexDrvInfos.end()) {
        ...
        itTex = (rTexDrvInfos.insert(make_pair(name, (ITextureDrvInfos *)NULL)))
                    .first;
        // keep old value, so it only gets deleted after we release the lock on
        // _SyncTexDrvInfos
        maybeDeleted = tex.TextureDrvShare->DrvTexture;
        // create and set iterator, for future deletion.
        itTex->second = tex.TextureDrvShare->DrvTexture =
            new CTextureDrvInfosGL(this, itTex, this, isTextureRectangle(&tex));
        ...
        if (bMustRecreateSharedTexture)
          ...
          // Do not need to reload this texture, even if the format/mipmap has
          // changed, since we found this couple in the map.
          mustLoadAll = false;
      }
    }
    // Do not test if part of texture may need to be computed, because Rect
    // invalidation is incompatible with texture sharing.
  } else {
    // If texture not already created.
    if (!tex.TextureDrvShare->DrvTexture) {
      // Must create it. Create auto a GL id (in constructor).
      // Do not insert into the map. This un-shared texture will be deleted at
      // deletion of the texture. Inform ITextureDrvInfos by passing NULL
      // _Driver.
      tex.TextureDrvShare->DrvTexture = new CTextureDrvInfosGL(
          NULL, ItTexDrvInfoPtrMap(), this, isTextureRectangle(&tex));
  ...
  if (mustLoadAll || mustLoadPart) {
    // system of "backup the previous binded texture" seems to not work with
    // some drivers....
    bindTextureWithMode(tex);
    ...
    if (mustLoadAll) {
      ...
      if (tex.isTextureCube()) {
        ...
        for (uint nText = 0; nText < 6; ++nText)
          if (pTC->getTexture((CTextureCube::TFace)nText) != NULL) {
            ...
            sint nMipMaps;
            if (glSrcFmt == GL_RGBA &&
                pTInTC->getPixelFormat() != CBitmap::RGBA)
            ...
            for (sint i = 0; i < nMipMaps; i++) {
              ...
              if (bUpload) {
                ...
                glTexImage2D(NLCubeFaceToGLCubeFace[nText], i, glfmt, w, h, 0,
                             glSrcFmt, glSrcType, ptr);
                bAllUploaded = true;
                NEL_MEASURE_UPLOAD_TIME_END
              } else {
                NEL_MEASURE_UPLOAD_TIME_START
                glTexImage2D(NLCubeFaceToGLCubeFace[nText], i, glfmt, w, h, 0,
                             glSrcFmt, glSrcType, NULL);
                NEL_MEASURE_UPLOAD_TIME_END
              }
        ...
        if (tex.getSize() > 0) {
          ...
          // DXTC: if same format, and same mipmapOn/Off, use
          // glTexCompressedImage*. We cannot build the mipmaps if they are not
          // here.
          if (_Extensions.EXTTextureCompressionS3TC &&
              sameDXTCFormat(tex, glfmt)) {
            sint nMipMaps = 1;
            ...
            // Degradation in Size allowed only if DXTC texture are provided
            // with mipmaps. Because use them to resize !!!
            uint decalMipMapResize = 0;
            if (_ForceTextureResizePower > 0 && tex.allowDegradation() &&
                nMipMaps > 1) {
              decalMipMapResize =
                  min(_ForceTextureResizePower, (uint)(nMipMaps - 1));
            }
            ...
            for (sint i = decalMipMapResize; i < nMipMaps; i++) {
              ...
              if (bUpload) {
#ifdef USE_OPENGLES
                glCompressedTexImage2D(GL_TEXTURE_2D, i - decalMipMapResize,
                                       glfmt, tex.getWidth(i), tex.getHeight(i),
                                       0, size, ptr);
#else
                nglCompressedTexImage2DARB(GL_TEXTURE_2D, i - decalMipMapResize,
                                           glfmt, tex.getWidth(i),
                                           tex.getHeight(i), 0, size, ptr);
#endif
                bAllUploaded = true;
              } else {
                // glCompressedTexImage2DARB (GL_TEXTURE_2D,
                // i-decalMipMapResize, glfmt,
                //							tex.getWidth(i),tex.getHeight(i),
                //0, size, NULL);
                NEL_MEASURE_UPLOAD_TIME_START

                glTexImage2D(gltext->TextureMode, i - decalMipMapResize, glfmt,
                             tex.getWidth(i), tex.getHeight(i), 0, glSrcFmt,
                             glSrcType, NULL);
                NEL_MEASURE_UPLOAD_TIME_END
              }
            ...
            for (sint i = 0; i < nMipMaps; i++) {
              ...
              if (bUpload) {
                NEL_MEASURE_UPLOAD_TIME_START
                glTexImage2D(gltext->TextureMode, i, glfmt, w, h, 0, glSrcFmt,
                             glSrcType, ptr);
                NEL_MEASURE_UPLOAD_TIME_END
                bAllUploaded = true;
              } else {
      ...
      // printf("%d,%d,%d\n", tex.getMipMapCount(), tex.getWidth(0),
      // tex.getHeight(0));
    ...
    //==================================
    // Replace parts of a compressed image. Maybe don't work with the actual
    // system of invalidateRect()...
      ...
      if (tex.getSize() > 0) {
        ...
        for (itRect = tex._ListInvalidRect.begin();
             itRect != tex._ListInvalidRect.end(); itRect++) {
          CRect &rect = *itRect;
          sint x0 = rect.X;
          sint y0 = rect.Y;
          ...
          for (sint i = 0; i < nMipMaps; i++) {
            ...
            for (sint yy = 0; yy < (y1 - y0); yy++) {
              ...
              glTexSubImage2D(GL_TEXTURE_2D, i, x0, y0 + yy, x1 - x0, 1,
                              glSrcFmt, glSrcType, row);
            }
#else
            ...
            if (bUpload)
              glTexSubImage2D(GL_TEXTURE_2D, i, x0, y0, x1 - x0, y1 - y0,
                              glSrcFmt, glSrcType, ptr);
            else
              glTexSubImage2D(GL_TEXTURE_2D, i, x0, y0, x1 - x0, y1 - y0,
                              glSrcFmt, glSrcType, NULL);
...
bool CDriverGL::uploadTexture(ITexture &tex, CRect &rect, uint8 nNumMipMap) {
  ...
  if (_Extensions.EXTTextureCompressionS3TC && sameDXTCFormat(tex, glfmt)) {
    ...
    if (_ForceTextureResizePower > 0 && tex.allowDegradation() &&
        nUploadMipMaps > 1) {
      decalMipMapResize =
          min(_ForceTextureResizePower, (uint)(nUploadMipMaps - 1));
    }
...
#endif
          GL_TEXTURE_2D, nNumMipMap - decalMipMapResize, x0, y0, (x1 - x0),
          (y1 - y0), glfmt, imageSize, ptr);
    } else {
      // The CompressedTexSubImage2DARB function do not work properly if width
      // or height of the mipmap is less than 4 pixel so we use the other form.
      // (its not really time critical to upload 16 bytes so we can do it twice
      // if texture is cut)
...
#endif
          GL_TEXTURE_2D, nNumMipMap - decalMipMapResize, glfmt, w, h, 0,
          imageSize, ptr);
    }
  } else {
    ...
    for (sint yy = 0; yy < (y1 - y0); yy++) {
      char *row = (char *)ptr + ((yy + y0) * w + x0) * pixelSize;
      glTexSubImage2D(GL_TEXTURE_2D, nNumMipMap, x0, y0 + yy, x1 - x0, 1,
                      glSrcFmt, glSrcType, row);
    }
...
//		glPixelStorei (GL_UNPACK_ALIGNMENT, 0);
    ...
    glPixelStorei(GL_UNPACK_SKIP_ROWS, y0);
    glPixelStorei(GL_UNPACK_SKIP_PIXELS, x0);
    glTexSubImage2D(GL_TEXTURE_2D, nNumMipMap, x0, y0, x1 - x0, y1 - y0,
                    glSrcFmt, glSrcType, ptr);
...
// ***************************************************************************
bool CDriverGL::uploadTextureCube(ITexture &tex, CRect & /* rect */,
                                  uint8 /* nNumMipMap */,
                                  uint8 /* nNumFace */) {
  ...
  if (this->_CurrentTexture[stage] != tex) {
    ...
    if (tex && tex->TextureDrvShare) {
      ...
      if (_SumTextureMemoryUsed) {
        ...
        if (gltext->TextureUsedIdx >= _TextureUsed.size() ||
            _TextureUsed[gltext->TextureUsedIdx] != gltext) {
      ...
      if (tex->isTextureCube()) {
        ...
        if (gltext->TextureMode == GL_TEXTURE_RECTANGLE_NV)
          textureMode = CDriverGLStates::TextureRect;
#endif
        _DriverGLStates.setTextureMode(
            /*CDriverGLStates::Texture2D*/ textureMode);
      ...
      if (_Extensions.ATITextureEnvCombine3) {
        // very strange bug with ATI cards : when a texture is set to NULL at a
        // stage, the stage is still active sometimes...
        activateTexEnvMode(
            stage,
            _TexEnvReplace); // set the whole stage to replace fix the problem
      }
#endif
    }
...
static const GLenum OperatorLUT[9] = {
    ...
    GL_ADD_SIGNED_EXT,  GL_INTERPOLATE_EXT, GL_INTERPOLATE_EXT,
    GL_INTERPOLATE_EXT, GL_INTERPOLATE_EXT, GL_BUMP_ENVMAP_ATI};

// This maps the CMaterial::TTexSource
static const GLenum SourceLUT[4] = {GL_TEXTURE, GL_PREVIOUS_EXT,
                                    GL_PRIMARY_COLOR_EXT, GL_CONSTANT_EXT};

// This maps the CMaterial::TTexOperand
static const GLenum OperandLUT[4] = {GL_SRC_COLOR, GL_ONE_MINUS_SRC_COLOR,
                                     GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA};

// This maps the CMaterial::TTexOperator, used for openGL Arg2 setup.
static const GLenum InterpolateSrcLUT[8] = {
    GL_TEXTURE, GL_TEXTURE,      GL_TEXTURE,           GL_TEXTURE,
    GL_TEXTURE, GL_PREVIOUS_EXT, GL_PRIMARY_COLOR_EXT, GL_CONSTANT_EXT};

#ifndef USE_OPENGLES

// ***************************************************************************
// Set general tex env using ENV_COMBINE4 for the current setupped stage (used
// by forceActivateTexEnvMode)
static void forceActivateTexEnvModeEnvCombine4(const CMaterial::CTexEnv &env) {
  ...
  case CMaterial::Replace:
    ...
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_RGB_EXT,
              SourceLUT[env.Env.SrcArg0RGB]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_RGB_EXT,
              OperandLUT[env.Env.OpArg0RGB]);
    // Arg1 = 0
  ...
  case CMaterial::Add:
    ...
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_RGB_EXT, GL_ONE_MINUS_SRC_COLOR);
    // Arg2 = env arg1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_RGB_EXT,
              SourceLUT[env.Env.SrcArg1RGB]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_RGB_EXT,
              OperandLUT[env.Env.OpArg1RGB]);
    // Arg3 = 1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE3_RGB_NV, GL_ZERO);
  ...
  case CMaterial::Mad:
    ...
    // Arg2 = env arg2
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_RGB_EXT,
              SourceLUT[env.Env.SrcArg2RGB]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_RGB_EXT,
              OperandLUT[env.Env.OpArg2RGB]);
    // Arg3 = 1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE3_RGB_NV, GL_ZERO);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND3_RGB_NV, GL_ONE_MINUS_SRC_COLOR);
    break;
  ...
  case CMaterial::Replace:
    ...
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_ALPHA_EXT,
              SourceLUT[env.Env.SrcArg0Alpha]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND0_ALPHA_EXT,
              OperandLUT[env.Env.OpArg0Alpha]);
    // Arg1 = 0
  ...
  case CMaterial::Add:
    ...
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_ALPHA_EXT, GL_ONE_MINUS_SRC_ALPHA);
    // Arg2 = env arg1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_ALPHA_EXT,
              SourceLUT[env.Env.SrcArg1Alpha]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_ALPHA_EXT,
              OperandLUT[env.Env.OpArg1Alpha]);
    // Arg3 = 1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE3_ALPHA_NV, GL_ZERO);
  ...
  case CMaterial::Mad:
    ...
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA_EXT,
              SourceLUT[env.Env.SrcArg1Alpha]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND1_ALPHA_EXT,
              OperandLUT[env.Env.OpArg1Alpha]);
    // Arg2 = env arg2
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE2_ALPHA_EXT,
              SourceLUT[env.Env.SrcArg2Alpha]);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND2_ALPHA_EXT,
              OperandLUT[env.Env.OpArg2Alpha]);
    // Arg3 = 1
    glTexEnvi(GL_TEXTURE_ENV, GL_SOURCE3_ALPHA_NV, GL_ZERO);
    glTexEnvi(GL_TEXTURE_ENV, GL_OPERAND3_ALPHA_NV, GL_ONE_MINUS_SRC_ALPHA);
    break;
...
// ***************************************************************************
void CDriverGL::forceActivateTexEnvMode(uint stage,
    ...
    if (env.Env.OpRGB > CMaterial::Replace) {
      ...
      if (env.Env.OpRGB >= CMaterial::InterpolateTexture) {
        glTexEnvi(GL_TEXTURE_ENV, GL_SRC2_RGB,
                  InterpolateSrcLUT[env.Env.OpRGB]);
    ...
    if (env.Env.OpAlpha > CMaterial::Replace) {
      ...
      if (env.Env.OpAlpha >= CMaterial::InterpolateTexture) {
        glTexEnvi(GL_TEXTURE_ENV, GL_SRC2_ALPHA,
                  InterpolateSrcLUT[env.Env.OpAlpha]);
  ...
  if (_Extensions.EXTTextureEnvCombine) {
    ...
    if ((env.Env.OpAlpha == CMaterial::Mad ||
      ...
      //=====
      if (env.Env.OpAlpha == CMaterial::Mad) {
        if (((SourceLUT[env.Env.SrcArg1Alpha] == GL_ZERO &&
              OperandLUT[env.Env.OpArg1Alpha] == GL_ONE_MINUS_SRC_ALPHA) ||
             (SourceLUT[env.Env.SrcArg1Alpha] == GL_ONE &&
              OperandLUT[env.Env.OpArg1Alpha] == GL_SRC_ALPHA)) // 1
            && ((SourceLUT[env.Env.SrcArg2Alpha] == GL_ONE &&
                 OperandLUT[env.Env.OpArg2Alpha] == GL_ONE_MINUS_SRC_ALPHA) ||
                (SourceLUT[env.Env.SrcArg2Alpha] == GL_ZERO &&
                 OperandLUT[env.Env.OpArg2Alpha] == GL_SRC_ALPHA))) // 0
        {
...
void CDriverGL::activateTexEnvMode(uint stage, const CMaterial::CTexEnv &env) {
  ...
  // If a special Texture environnement is setuped, or if not the same normal
  // texture environnement, must setup a new normal Texture environnement.
  if (_CurrentTexEnvSpecial[stage] != TexEnvSpecialDisabled ||
      _CurrentTexEnv[stage].EnvPacked != env.EnvPacked) {
...
void CDriverGL::setAnisotropicFilter(sint filtering) {
  ...
  if (!_Extensions.EXTTextureFilterAnisotropic)
    return;

  if (filtering < 0 ||
      filtering > _Extensions.EXTTextureFilterAnisotropicMaximum) {
...
void CDriverGL::swapTextureHandle(ITexture &tex0, ITexture &tex1) {
  ...
  /* Swap contents. Can't swap directly the pointers cause would have to change
     all CTextureDrvShare which point on Can't do swap(*t0, *t1), because must
     keep the correct _DriverIterator
  */
  swap(t0->ID, t1->ID);
...
uintptr_t CDriverGL::getTextureHandle(const ITexture &tex) {
  ...
  const CTextureDrvInfosGL *t0 =
      (const CTextureDrvInfosGL *)(const ITextureDrvInfos *)(tex.TextureDrvShare
                                                                 ->DrvTexture);
  if (!t0)
    return 0;
...
/*
    Under opengl, "render to texture" uses the frame buffer. The scene is
   rendered into the current frame buffer and the result is copied into the
   texture.

    setRenderTarget (tex) does nothing but backup the framebuffer area used and
   updates the viewport and scissor setRenderTarget (NULL) copies the modified
   framebuffer area into "tex" and then, updates the viewport and scissor
 */

bool CDriverGL::setRenderTarget(ITexture *tex, uint32 x, uint32 y, uint32 width,
  ...
  if (!_RenderTargetFBO && _TextureTarget && _TextureTargetUpload &&
      (_TextureTarget != tex || _TextureTargetCubeFace != cubeFace)) {
    // Flush it
    copyFrameBufferToTexture(_TextureTarget, _TextureTargetLevel,
                             _TextureTargetX, _TextureTargetY, 0, 0,
                             _TextureTargetWidth, _TextureTargetHeight,
  ...
  if (tex) {
    ...
    nlassertex(tex->getRenderTarget(),
               ("The texture must be a render target. Call "
                "ITexture::setRenderTarget(true)."));
...
bool CDriverGL::copyTargetToTexture(ITexture *tex, uint32 offsetx,
  ...
  copyFrameBufferToTexture(tex, mipmapLevel, offsetx, offsety, x, y, width,
                           height);
  return true;
}
...
bool CDriverGL::getRenderTargetSize(uint32 &width, uint32 &height) {
  ...
  NLMISC::CSmartPtr<ITexture> tex =
      _RenderTargetFBO ? _RenderTargetFBO
                       : (_TextureTarget ? _TextureTarget : NULL);
  if (tex) {
    width = tex->getWidth();
    height = tex->getHeight();
  } else {

 ...
 * $Id$
 *
 * \todo manage better the init/release system (if a throw occurs in the init,
 * we must release correctly the driver)
 */
...
CPixelProgamDrvInfosGL::CPixelProgamDrvInfosGL(CDriverGL *drv,
...
bool CDriverGL::activeARBPixelProgram(CPixelProgram *program) {
  ...
  if (program) {
    // Program setuped ?
    if (!CDriverGL::compilePixelProgram(program))
      return false;

    // Cast the driver info pointer
    CPixelProgamDrvInfosGL *drvInfo = safe_cast<CPixelProgamDrvInfosGL *>(
...
bool CDriverGL::setupPixelProgram(CPixelProgram *program,
                                  GLuint id /*, bool &specularWritten*/) {
  H_AUTO_OGL(CDriverGL_setupARBPixelProgram);

#ifndef USE_OPENGLES
  CPixelProgamDrvInfosGL *drvInfo = static_cast<CPixelProgamDrvInfosGL *>(
      (IProgramDrvInfos *)program->m_DrvInfo);
  ...
  nglProgramStringARB(GL_FRAGMENT_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                      source->SourceLen, source->SourcePtr);

...
// ***************************************************************************
/** Interface to a Big block of AGP memory either throurgh NVVertexArrayRange or
 * ATIVertexObject
 */
...
public:
  ...
  /// allocate a vertex array space. false if error. client must free before
  /// re-allocate.
  virtual bool allocate(uint32 size,
                        CVertexBuffer::TPreferredMemory vbType) = 0;
  /// free this space.
...
public:
  ...
  // test if buffer content is invalid. If so, no rendering should occurs
  // (rendering should silently fail)
...
// ***************************************************************************
/** Work only if ARRAY_RANGE_NV is enabled. else, only call to ctor/dtor/free()
 *is valid. any call to allocateVB() will return NULL.
 */
...
public:
  ...
  /// \name Implementation
  // @{
  /// allocate a vertex array sapce. false if error. must free before
  /// re-allocate.
  ...
  /// active this VertexArrayRange as the current vertex array range used. no-op
  /// if already setup.
  void enable();
...
public:
  ...
  // true if a setFence() has been done, without a finishFence(). NB: not the
  // same thing as nglTestFenceNV()!!
  ...
  void finishFence();
  // if fence is not set, no-op, else test nglTestFenceNV(), and update local
  // _FenceSet flag.
  void testFence();
...
// ***************************************************************************
/** Work only if ATIVertexArrayObject is enabled. else, only call to
 *ctor/dtor/free() is valid. any call to allocateVB() will return NULL.
 */
...
public:
  ...
  // Those methods read/write in _Driver->_CurrentVertexArrayRange.
  /** active this VertexArrayRange as the current vertex array range used. no-op
   *if already setup. NB: no-op for ATI, but ensure correct
   *_Driver->_CurrentVertexArrayRange value.
   */
  void enable();
  /** disable this VertexArrayRange. _Driver->_CurrentVertexArrayRange= NULL;
   *	NB: no-op for ATI, but ensure correct _Driver->_CurrentVertexArrayRange
   *value.
   */
  void disable();
...
private:
  ...
  // Allocator. NB: We don't have any Mem Ptr For ATI extension, so use a Fake
  // Ptr: NL3D_DRV_ATI_FAKE_MEM_START
  NLMISC::CHeapMemory _HeapMemory;
...
public:
  ...
  uint getATIVertexObjectId() const {
    return _VertexArrayRange->getATIVertexObjectId();
  }
 ...
 * We don't manage a heap there, we just allocate separate objects (can't get a
 * pointer on a portion of the buffer only ..) todo : test if such maneer is
 * still efficient (because of vb switching) NB : this is available only it
 * GL_ATI_map_object_buffer is available
 */
...
public:
  ...
  /// \name Implementation
  // @{
  /** Allocate a vertex array space. false if error. must free before
   * re-allocate. Will always succeed, because vb are not managed in a heap, but
   * are rather kept as separate objects
   */
  virtual bool allocate(uint32 size, CVertexBuffer::TPreferredMemory vbType);
  /// free this space.
  virtual void freeBlock();
...
/** vb hard using the ATI_map_object_buffer extension. Buffer are kept separate
 * rather than managed in a heap
 */
...
private:
  ...
  // if buffer has been invalidated, returns a dummy memory block and silently
  // fails rendering
...
#ifdef NL_DEBUG
  bool _Unmapping;
  std::list<CVertexBufferHardGLMapObjectATI *>::iterator
      _IteratorInMappedVBList;
#endif
};
...
public:
  ...
  // Those methods read/write in _Driver->_CurrentVertexArrayRange.
  /** active this VertexArrayRange as the current vertex array range used. no-op
   *if already setup. NB: no-op for ARB, but ensure correct
   *_Driver->_CurrentVertexArrayRange value.
   */
  void enable();
  /** disable this VertexArrayRange. _Driver->_CurrentVertexArrayRange= NULL;
   *	NB: no-op for ARB, but ensure correct _Driver->_CurrentVertexArrayRange
   *value.
   */
  void disable();
  // check & invalidate lost buffers
...
/** vb hard using the ARB_vertex_buffer_object extension. Buffer are kept
 * separate rather than managed in a heap
 */
...
public:
  ...
  /**	setup ptrs allocated by createVBHard()
   */
  void initGL(uint vertexObjectID, CVertexArrayRangeARB *var,

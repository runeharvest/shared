...
void CDriverGL::setLightInternal(uint8 num, const CLight &light) {
  ...
  if (num < _MaxDriverLight) {
    ...
    // don't know why, but with ATI cards, specular of 0 causes incorrect
    // rendering (random specular is added)
    if (_Extensions.ATITextureEnvCombine3) {
      // special case for ATI (there will be some specular, but there's a bug
      // otherwise)
...
void CDriverGL::enableLightInternal(uint8 num, bool enable) {
  ...
  if (num < _MaxDriverLight) {
    ...
    // If this light is dirty, and reenabled, then it must be refresh at next
    // render => set the global flag.

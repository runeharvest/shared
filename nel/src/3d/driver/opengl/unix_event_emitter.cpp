...
CUnixEventEmitter::CUnixEventEmitter()
    : _dpy(NULL), _win(0), _im(NULL), _ic(NULL), _driver(NULL) {
  _SelectionOwned = false;
}

CUnixEventEmitter::~CUnixEventEmitter() { closeIM(); }
...
void CUnixEventEmitter::createIM() {
  ...
  if (_im) {
    _ic = XCreateIC(_im, XNInputStyle, XIMPreeditNothing | XIMStatusNothing,
                    XNClientWindow, _win, XNFocusWindow, _win, NULL);
...
TKeyButton getKeyButton(uint32 state) {
  ...
  if (state & ControlMask)
    button |= ctrlKeyButton;
  if (state & ShiftMask)
    button |= shiftKeyButton;
  if (state & AltMask)
    button |= altKeyButton;
...
bool CUnixEventEmitter::processMessage(XEvent &event, CEventServer *server) {
  ...
  switch (event.type) {
  case ButtonPress: {
    // nlinfo("%d %d %d", event.xbutton.button, event.xbutton.x,
    // event.xbutton.y);
    ...
    case Button1:
      server->postEvent(new CEventMouseDown(
          fX, fY,
          (TMouseButton)(leftButton |
  ...
  case KeyPress: {
    ...
    // if key event is filtered, we shouldn't use XLookupString to retrieve
    // KeySym
    if (!filtered) {
...
#ifdef X_HAVE_UTF8_STRING
      if (_ic)
        c = Xutf8LookupString(_ic, &event.xkey, Text, sizeof(Text), &k,
                              &status);
#endif
    ...
    if (keyCode) {
      ...
      server->postEvent(new CEventKeyDown(
          key, getKeyButton(event.xbutton.state), firstTime, this));
    ...
    if (c > 0) {
      ...
      CEventChar *charEvent =
          new CEventChar(ucstr[0], getKeyButton(event.xbutton.state), this);
      ...
      for (int i = 0; i < c; i++) {
        CEventChar *charEvent =
            new CEventChar((u32char)(unsigned char)Text[i],
                           getKeyButton(event.xbutton.state), this);
  ...
  case KeyRelease: {
    if (!keyRepeat(_dpy, &event)) {
      ...
      server->postEvent(
          new CEventKeyUp(key, getKeyButton(event.xbutton.state), this));
  ...
  case SelectionRequest: {
    ...
    if (req.target == XA_TARGETS) {
      ...
      respond.xselection.property = req.property;
      std::string str = _CopiedString; // NLMISC::CUtfStringView(_CopiedString).toAscii();
                                       // // FIXME: Convert UTF-8 to local
      XChangeProperty(req.display, req.requestor, req.property, XA_STRING, 8,
                      PropModeReplace, (const unsigned char *)str.c_str(),
                      str.length());
    } else if (req.target == XA_UTF8_STRING) {
      respond.xselection.property = req.property;
      XChangeProperty(req.display, req.requestor, respond.xselection.property,
                      XA_UTF8_STRING, 8, PropModeReplace,
                      (const unsigned char *)_CopiedString.c_str(),
                      _CopiedString.length());
    } else {
      // Note: Calling XGetAtomName with arbitrary value crash the client, maybe
      // req.target have been sanitized by X11 server
  ...
  case SelectionNotify: {
    ...
    if (target == XA_TARGETS || target == XA_ATOM) {
      ...
      if (XGetWindowProperty(_dpy, _win, XA_NEL_SEL, 0, XMaxRequestSize(_dpy),
                             False, AnyPropertyType, &actualType, &actualFormat,
                             &nitems, &bytesLeft,
                             (unsigned char **)&supportedTargets) != Success)
        return false;
      ...
      for (uint i = 0; i < nitems; i++) {
        // nlwarning(" - Type=%s (%u)", XGetAtomName(_dpy, supportedTargets[i]),
        // (uint)supportedTargets[i]);
      ...
      XConvertSelection(_dpy, XA_CLIPBOARD, bestTarget, XA_NEL_SEL, _win,
                        CurrentTime);
  ...
  case ClientMessage:
    if ((event.xclient.format == 32) &&
        ((Atom)event.xclient.data.l[0] == XA_WM_DELETE_WINDOW)) {
...
bool CUnixEventEmitter::pasteTextFromClipboard(std::string &text) {
  ...
  XConvertSelection(_dpy, XA_CLIPBOARD, XA_TARGETS, XA_NEL_SEL, _win,
                    CurrentTime);

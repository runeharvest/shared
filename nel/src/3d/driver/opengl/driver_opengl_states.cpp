...
void CDriverGLStates::init(bool supportTextureCubeMap,
                           bool supportTextureRectangle, uint maxLight) {
  ...
  uint i;
  for (i = 0;
       i < sizeof(_TexCoordArrayEnabled) / sizeof(_TexCoordArrayEnabled[0]);
       i++) {
...
#ifndef NL3D_GLSTATE_DISABLE_CACHE
  if ((func != _CurStencilFunc) || (ref != _CurStencilRef) ||
      (mask != _CurStencilMask))
#endif
  {
    // new state
...
#ifndef NL3D_GLSTATE_DISABLE_CACHE
  if ((fail != _CurStencilOpFail) || (zfail != _CurStencilOpZFail) ||
      (zpass != _CurStencilOpZPass))
#endif
  {
    // new state
...
void CDriverGLStates::setVertexColorLighted(bool enable) {
...
#endif
  {
    ...
    if (_VertexColorLighted) {
      ...
      glDisable(GL_COLOR_MATERIAL);
      // Since we leave glColorMaterial mode, GL diffuse is now scracth. reset
      // him to current value.
      CRGBA diffCol;
...
//			{
//				nglTexGeniOES(GL_TEXTURE_GEN_STR_OES,
//GL_TEXTURE_GEN_MODE_OES, mode); 				glEnable(GL_TEXTURE_GEN_STR_OES);
//			}
//			else
//			{
//				glDisable(GL_TEXTURE_GEN_STR_OES);
...
void CDriverGLStates::enableSecondaryColorArray(bool enable) {
  ...
  if (_SecondaryColorArrayEnabled != enable) {
    ...
    // If disable
    if (!enable) {
      // GeForceFx Bug: Must reset Secondary color to 0 (if comes from a VP),
      // else bugs
...
// ***************************************************************************
void CDriverGLStates::enableVertexAttribArrayForEXTVertexShader(
    uint glIndex, bool enable, uint *variants) {
  ...
  if (_VertexAttribArrayEnabled[glIndex] != enable) {
...
#ifndef USE_OPENGLES
      if (enable)
        nglEnableVariantClientStateEXT(
            variants[CDriverGL::EVSSkinWeightVariant]);
      else
    ...
    case 5: // fog coordinate
      ...
      if (enable)
        nglEnableVariantClientStateEXT(
            variants[CDriverGL::EVSFogCoordsVariant]);
      else
        nglDisableVariantClientStateEXT(
            variants[CDriverGL::EVSFogCoordsVariant]);
#endif
    ...
    case 6: // palette skin
      ...
      if (enable)
        nglEnableVariantClientStateEXT(
            variants[CDriverGL::EVSPaletteSkinVariant]);
      else
        nglDisableVariantClientStateEXT(
            variants[CDriverGL::EVSPaletteSkinVariant]);
#endif

...
// ***************************************************************************
CVertexProgamDrvInfosGL::CVertexProgamDrvInfosGL(CDriverGL *drv,
...
bool CDriverGL::compileNVVertexProgram(CVertexProgram *program) {
  ...
  if (!source) {
    nlwarning("OpenGL driver only supports 'nelvp' profile, vertex program "
              "cannot be used");
    return false;
  }

  /** Check with our parser if the program will works with other implemented
   * extensions, too. (EXT_vertex_shader ..). There are some incompatibilities.
   */
  CVPParser parser;
  ...
  ItGPUPrgDrvInfoPtrList it = _GPUPrgDrvInfos.insert(
      _GPUPrgDrvInfos.end(), (NL3D::IProgramDrvInfos *)NULL);
  ...
  *it = drvInfo = new CVertexProgamDrvInfosGL(this, it);
  ...
  // Compile the program
  nglLoadProgramNV(GL_VERTEX_PROGRAM_NV, drvInfo->ID,
                   (GLsizei)source->SourceLen,
                   (const GLubyte *)source->SourcePtr);
  ...
  if (errorOff >= 0) {
    ...
    //_GPUPrgDrvInfos.erase(it); // not needed as ~IProgramDrvInfos() already
    //does it
    return false;
  }
...
bool CDriverGL::activeNVVertexProgram(CVertexProgram *program) {
  ...
  if (program) {
    // Driver info
    CVertexProgamDrvInfosGL *drvInfo = safe_cast<CVertexProgamDrvInfosGL *>(
...
// ***************************************************************************
#ifndef USE_OPENGLES
static inline GLenum convSwizzleToGLFormat(CVPSwizzle::EComp comp,
                                           bool negate) {
 ...
 */
static GLuint
convOutputRegisterToEXTVertexShader(CVPOperand::EOutputRegister r) {
  H_AUTO_OGL(convOutputRegisterToEXTVertexShader);

#ifndef USE_OPENGLES
  switch (r) {
  case CVPOperand::OHPosition:
    return GL_OUTPUT_VERTEX_EXT;
  case CVPOperand::OPrimaryColor:
    return GL_OUTPUT_COLOR0_EXT;
  case CVPOperand::OSecondaryColor:
    return GL_OUTPUT_COLOR1_EXT;
  case CVPOperand::OBackFacePrimaryColor:
    nlwarning("Backface color used in a vertex program is not supported by "
              "device, defaulting to front color");
    return GL_OUTPUT_COLOR0_EXT;
    break;
...
static uint convInputRegisterToVBFlag(uint index) {
  ...
  switch (index) {
  case CVPOperand::IPosition:
    return CVertexBuffer::PositionFlag;
  case CVPOperand::IWeight:
    return CVertexBuffer::WeightFlag;
  case CVPOperand::INormal:
    return CVertexBuffer::NormalFlag;
  case CVPOperand::IPrimaryColor:
    return CVertexBuffer::PrimaryColorFlag;
  case CVPOperand::ISecondaryColor:
    return CVertexBuffer::SecondaryColorFlag;
  case CVPOperand::IFogCoord:
    return CVertexBuffer::FogFlag;
  case CVPOperand::IPaletteSkin:
    return CVertexBuffer::PaletteSkinFlag;
  case CVPOperand::IEmpty:
    nlassert(0);
    break;
...
static void doSwizzle(GLuint res, GLuint in, GLenum outX, GLenum outY,
...
static void doWriteMask(GLuint res, GLuint in, GLenum outX, GLenum outY,
                        GLenum outZ, GLenum outW) {
...
#ifdef DEBUG_SETUP_EXT_VERTEX_SHADER
  nlinfo("3D: Write Mask : %c%c%c%c", outX ? 'x' : '-', outY ? 'y' : '-',
         outZ ? 'z' : '-', outW ? 'w' : '-');
#endif
#endif
}
 ...
 */
bool CDriverGL::setupEXTVertexShader(const CVPParser::TProgram &program,
                                     GLuint id, uint variants[EVSNumVariants],
  ...
  // variants[EVSSecondaryColorVariant] = nglGenSymbolsEXT(GL_VECTOR_EXT,
  // GL_VARIANT_EXT, GL_NORMALIZED_RANGE_EXT, 1);
  ...
  nglBeginVertexShaderEXT();
  {
    ...
    // allocate registers
    GLuint firstRegister = nglGenSymbolsEXT(
        GL_VECTOR_EXT, GL_LOCAL_EXT, GL_FULL_RANGE_EXT, 12); // 12 register
    GLuint firstTempRegister =
        nglGenSymbolsEXT(GL_VECTOR_EXT, GL_LOCAL_EXT, GL_FULL_RANGE_EXT,
                         4); // 4  temp register used for swizzle & indexing
    GLuint firstTempScalar =
        nglGenSymbolsEXT(GL_SCALAR_EXT, GL_LOCAL_EXT, GL_FULL_RANGE_EXT,
                         3); // 3  temp scalars used for EXPP & LOGG emulation
    GLuint firstAddressRegister =
        nglGenSymbolsEXT(GL_SCALAR_EXT, GL_LOCAL_EXT, GL_FULL_RANGE_EXT, 1);

    // allocate needed variants
    if (CVPParser::isInputUsed(program, CVPOperand::ISecondaryColor)) {
      variants[EVSSecondaryColorVariant] = nglGenSymbolsEXT(
          GL_VECTOR_EXT, GL_VARIANT_EXT, GL_NORMALIZED_RANGE_EXT, 1);

      if (!variants[EVSSecondaryColorVariant]) {
        nlwarning(
            "EXT_vertex_shader : can't allocate variant for secondary color");
        return false;
      }
    } else {
    ...
    if (CVPParser::isInputUsed(program, CVPOperand::IFogCoord)) {
      variants[EVSFogCoordsVariant] =
          nglGenSymbolsEXT(GL_VECTOR_EXT, GL_VARIANT_EXT, GL_FULL_RANGE_EXT, 1);
    ...
    if (CVPParser::isInputUsed(program, CVPOperand::IPaletteSkin)) {
      variants[EVSPaletteSkinVariant] =
          nglGenSymbolsEXT(GL_VECTOR_EXT, GL_VARIANT_EXT, GL_FULL_RANGE_EXT, 1);

      if (!variants[EVSPaletteSkinVariant]) {
        nlwarning(
            "EXT_vertex_shader : can't allocate variant for palette skin");
        return false;
      }
    } else {
    ...
    if (!_ATIFogRangeFixed) {
      fogTemp =
          nglGenSymbolsEXT(GL_VECTOR_EXT, GL_LOCAL_EXT, GL_FULL_RANGE_EXT, 1);
    }

    // local constant : 0 and 1
    GLuint cteOne = nglGenSymbolsEXT(GL_SCALAR_EXT, GL_LOCAL_CONSTANT_EXT,
                                     GL_FULL_RANGE_EXT, 1);
    GLuint cteZero = nglGenSymbolsEXT(GL_SCALAR_EXT, GL_LOCAL_CONSTANT_EXT,
                                      GL_FULL_RANGE_EXT, 1);
    ...
    if (firstTempRegister == 0) {
      nlwarning(
          "Unable to allocate local temp registers for EXT_vertex_shader");
      return false;
    }
    if (firstTempScalar == 0) {
      nlwarning(
          "Unable to allocate temp scalar registers for EXT_vertex_shader");
      return false;
    }
    ...
    for (uint k = 0; k < program.size(); ++k) {
      ...
      for (l = 0; l < numSrc; ++l) {
        ...
        case CVPOperand::InputRegister: {
          ...
          case 15: {
            EVS_INFO(
                ("Src = Tex" + toString(operand.Value.InputRegisterValue - 8))
                    .c_str());
            srcValue[l] = _EVSTexHandle[operand.Value.InputRegisterValue - 8];
            if (srcValue[l] == 0) {
              nlwarning("Trying to read an unaccessible texture coords for the "
                        "device when using EXT_vertex_shader, shader loading "
                        "failed.");
              return false;
            }
          } break;
        ...
        case CVPOperand::Constant:
          nlassert(uint(operand.Value.ConstantValue) <
                   _EVSNumConstant); // constant index too high
          srcValue[l] = _EVSConstantHandle + operand.Value.ConstantValue;
          EVS_INFO(("Src = constant" + toString(operand.Value.ConstantValue))
                       .c_str());
          break;
        case CVPOperand::Variable:
          srcValue[l] = firstRegister + operand.Value.VariableValue;
          EVS_INFO(("Src = variable register" +
                    toString(operand.Value.VariableValue))
                       .c_str());
          break;
        default:
          nlassert(0);
        ...
        if (operand.Type == CVPOperand::Constant && operand.Indexed) {
          nglShaderOp2EXT(GL_OP_INDEX_EXT, firstTempRegister + 3,
                          firstAddressRegister, srcValue[l]);
        ...
        if (!operand.Swizzle.isIdentity() || operand.Negate) {
          // if the instruction reads a scalar, no need for swizzle (except if
          // negate is used)
          if (!((program[k].Opcode == CVPInstruction::RSQ ||
            ...
            doSwizzle(
                firstTempRegister + l, srcValue[l],
      ...
      case CVPOperand::OutputRegister:
        if (_ATIFogRangeFixed ||
            destOperand.Value.OutputRegisterValue != CVPOperand::OFogCoord) {
          destValue = convOutputRegisterToEXTVertexShader(
              destOperand.Value.OutputRegisterValue);
        } else {
          destValue = fogTemp;
        }
        break;
      ...
      // Tells whether the output has already been written before the final
      // write mask. This happens with instructions LOG, EXPP, LIT, RSQ and RCP,
      // because they write their output component by components
      bool outputWritten = false;

      // test for write mask
      if (writeMask != 0x0f) {
        /** Don't know why, but on some implementation of EXT_vertex_shader,
         * can't write a single components to the fog coordinate.. So we force
         * the mask to 0xf (only the x coordinate is used anyway).
         */
        if (!(destOperand.Type == CVPOperand::OutputRegister &&
              destOperand.Value.OutputRegisterValue == CVPOperand::OFogCoord)) {
          // For instructions that write their output components by components,
          // we don't need an intermediary register
          if (opcode == CVPInstruction::LOG || opcode == CVPInstruction::EXPP ||
              opcode == CVPInstruction::LIT || opcode == CVPInstruction::RSQ ||
        ...
        if (destOperand.Type == CVPOperand::OutputRegister) {
          componentWritten[outputRegisterIndex] =
              0xf; // say all components have been written for that output
        }
      }
      ...
      case CVPInstruction::MAD:
        nglShaderOp3EXT(GL_OP_MADD_EXT, destValue, srcValue[0], srcValue[1],
                        srcValue[2]);
      ...
      case CVPInstruction::RSQ: {
        ...
        EVS_INFO("Extract component");
        ++numEC;
        nglShaderOp1EXT(GL_OP_RECIP_SQRT_EXT, firstTempScalar + 1,
                        firstTempScalar);
      ...
      case CVPInstruction::DST:
        doSwizzle(firstTempRegister, srcValue[0], GL_ONE_EXT, GL_Y_EXT,
                  GL_Z_EXT, GL_ONE_EXT);
        EVS_INFO("GL_OP_DOT4_EXT");
        ++numOp;
        doSwizzle(firstTempRegister + 1, srcValue[1], GL_ONE_EXT, GL_Y_EXT,
                  GL_ONE_EXT, GL_W_EXT);
        ++numSwizzle;
        nglShaderOp2EXT(GL_OP_MUL_EXT, destValue, firstTempRegister,
                        firstTempRegister + 1);
        EVS_INFO("GL_OP_MUL_EXT");
        ++numOp;
        break;
      case CVPInstruction::LIT: {
        uint writeMask = program[k].Dest.WriteMask;
        nglExtractComponentEXT(firstTempScalar, srcValue[0],
                               0); // extract X from the source
        if (writeMask & 4) {
          nglExtractComponentEXT(firstTempScalar + 1, srcValue[0],
                                 1); // extract Y from the source
          EVS_INFO("Extract component");
          ++numEC;
          nglExtractComponentEXT(firstTempScalar + 2, srcValue[0],
                                 3); // extract W from the source
          EVS_INFO("Extract component");
          ++numEC;
          // result = X > 0 ? Y^W : 0
          nglShaderOp2EXT(GL_OP_POWER_EXT, firstTempScalar + 2,
                          firstTempScalar + 1, firstTempScalar + 2);
          EVS_INFO("GL_OP_POWER_EXT");
          ++numOp;
          nglShaderOp2EXT(GL_OP_SET_GE_EXT, firstTempScalar + 1,
                          firstTempScalar, cteZero);
          EVS_INFO("GL_OP_SET_GE_EXT");
          ++numOp;
          nglShaderOp2EXT(GL_OP_MUL_EXT, firstTempScalar + 2,
                          firstTempScalar + 2, firstTempScalar + 1);
          EVS_INFO("GL_OP_MUL_EXT");
          ++numOp;
          // store result
        ...
        if (writeMask & 2) {
          // clamp N.L to [0, 1]
          nglShaderOp3EXT(GL_OP_CLAMP_EXT, firstTempScalar, firstTempScalar,
                          cteZero, cteOne);
        ...
        if (writeMask & (1 + 8)) {
          doSwizzle(destValue, destValue,
                    (writeMask & 1) ? GL_ONE_EXT : GL_X_EXT, GL_Y_EXT, GL_Z_EXT,
      ...
      case CVPInstruction::EXPP: {
        ...
        nglExtractComponentEXT(firstTempScalar + 2, srcValue[0],
                               compIndex); // extract W from the source
        EVS_INFO("Extract component");
        ++numEC;
        if (writeMask & 1) {
          nglShaderOp1EXT(GL_OP_FLOOR_EXT, firstTempScalar,
                          firstTempScalar + 2); // (int) W
          EVS_INFO("GL_OP_FLOOR_EXT");
          ++numOp;
          nglShaderOp1EXT(GL_OP_EXP_BASE_2_EXT, firstTempScalar,
                          firstTempScalar); // 2 ^ (int) W
          EVS_INFO("GL_OP_EXP_BASE_2_EXT");
          ++numOp;
        }
        if (writeMask & 2) {
          nglShaderOp1EXT(GL_OP_FRAC_EXT, firstTempScalar + 1,
                          firstTempScalar + 2); // frac(W)
          EVS_INFO("GL_OP_FRAC_EXT");
          ++numOp;
        }
        if (writeMask & 4)
          nglShaderOp1EXT(GL_OP_EXP_BASE_2_EXT, firstTempScalar + 2,
                          firstTempScalar + 2); // 2 ^W
        // store the results
        if (writeMask & 1) {
        ...
        if (writeMask & 8) {
          doSwizzle(destValue, destValue, GL_X_EXT, GL_Y_EXT, GL_Z_EXT,
                    GL_ONE_EXT);
          ++numSwizzle;
        }
      } break;
      case CVPInstruction::LOG: {
        ...
        // extract the component we need
        nglExtractComponentEXT(firstTempScalar, srcValue[0],
                               (GLuint)program[k].Src1.Swizzle.Comp[0]);
        EVS_INFO("Extract component");
        ++numEC;
        ...
        ++numOp;
        nglShaderOp2EXT(GL_OP_MAX_EXT, firstTempScalar, firstTempScalar,
                        firstTempScalar + 1);
        EVS_INFO("GL_OP_MAX_EXT");
        ++numOp;
        nglShaderOp1EXT(GL_OP_LOG_BASE_2_EXT, firstTempScalar,
                        firstTempScalar); // (int) W
      ...
      if (writeMask != 0x0f) {
        if ((destOperand.Type == CVPOperand::OutputRegister &&
          ...
          if ((maskedDestValue >= GL_OUTPUT_TEXTURE_COORD0_EXT &&
               maskedDestValue <= GL_OUTPUT_TEXTURE_COORD7_EXT) ||
            ...
            bool found = false;
            // if this was the last write for this output, must set unfilled
            // component NB : this loop could be optimized, but vertex program
            // are rather short for now ..
            for (uint m = k + 1; m < program.size(); ++m) {
              if (program[m].Dest.Type ==
                  CVPOperand::OutputRegister) // another output to this texture
                                              // ?
              {
                if (program[m].Dest.Value.OutputRegisterValue ==
                    program[k].Dest.Value.OutputRegisterValue) {
                  found = true;
                  break;
                }
              }
    ...
    if (componentWritten[CVPOperand::OPrimaryColor] == 0) {
      // we specify vertex coord has input for swizzle, but we don't read any
      // component..
      doSwizzle(GL_OUTPUT_COLOR0_EXT, _EVSPositionHandle, GL_ZERO_EXT,
                GL_ZERO_EXT, GL_ZERO_EXT, GL_ONE_EXT);
    ...
    if (componentWritten[CVPOperand::OSecondaryColor] == 0) {
      // we specify vertex coord has input for swizzle, but we don't read any
      // component..
      doSwizzle(GL_OUTPUT_COLOR1_EXT, _EVSPositionHandle, GL_ZERO_EXT,
                GL_ZERO_EXT, GL_ZERO_EXT, GL_ONE_EXT);
    ...
    nlassert(componentWritten[CVPOperand::OHPosition] ==
             0xf); // should have written all component of position

    glError = glGetError();
    nlassert(glError == GL_NO_ERROR);

    // if fog has been written, perform conversion (if there's no ATI driver
    // fix)
    if (!_ATIFogRangeFixed && componentWritten[CVPOperand::OFogCoord] == 0xf) {
      // Well this could be avoided, but we should make 2 cases for each vertex
      // program.. :(
      doSwizzle(firstTempRegister, _EVSConstantHandle + _EVSNumConstant,
                GL_X_EXT, GL_X_EXT, GL_X_EXT, GL_X_EXT);
      doSwizzle(firstTempRegister + 1, _EVSConstantHandle + _EVSNumConstant,
                GL_Y_EXT, GL_Y_EXT, GL_Y_EXT, GL_Y_EXT);
      nglShaderOp3EXT(GL_OP_MADD_EXT, firstTempRegister + 2, fogTemp,
                      firstTempRegister, firstTempRegister + 1);
  ...
  if (!optimizedShader) {
    nlwarning("Failed to optimize a vertex program with the EXT_vertex_shader "
              "extension, this shader will be disabled");
    return false;
  }
  ...
  for (k = 0; k < program.size(); ++k) {
    ...
    for (l = 0; l < numSrc; ++l) {
      ...
      if (op.Type == CVPOperand::InputRegister) {
        usedInputRegisters |=
            convInputRegisterToVBFlag(op.Value.InputRegisterValue);
      }
    }
  }
...
static const char *ARBVertexProgramInstrToName[] = {
    "MOV  ", "ARL  ", "MUL  ", "ADD  ", "MAD  ", "RSQ  ",
    "DP3  ", "DP4  ", "DST  ", "LIT  ", "MIN  ", "MAX  ",
    "SLT  ", "SGE  ", "EXP  ", "LOG  ", "RCP  "};

//=================================================================================================
static const char *ARBVertexProgramOutputRegisterToName[] = {
    "position",           "color.primary",        "color.secondary",
    "color.back.primary", "color.back.secondary", "fogcoord",
    "pointsize",          "texcoord[0]",          "texcoord[1]",
    "texcoord[2]",        "texcoord[3]",          "texcoord[4]",
    "texcoord[5]",        "texcoord[6]",          "texcoord[7]"};
...
//=================================================================================================
static void ARBVertexProgramDumpSwizzle(const CVPSwizzle &swz,
                                        std::string &out) {
...
//=================================================================================================
static void ARBVertexProgramDumpOperand(const CVPOperand &op, bool destOperand,
                                        std::string &out) {
  H_AUTO_OGL(ARBVertexProgramDumpOperand)
  out = op.Negate ? " -" : " ";
  switch (op.Type) {
  case CVPOperand::Variable:
    out += "R" + NLMISC::toString(op.Value.VariableValue);
    break;
  case CVPOperand::Constant:
    out += "c[";
    if (op.Indexed) {
  ...
  case CVPOperand::InputRegister:
    out += "vertex.attrib[" +
           NLMISC::toString((uint)op.Value.InputRegisterValue) + "]";
    break;
  case CVPOperand::OutputRegister:
    ...
    out +=
        "result." +
        std::string(
            ARBVertexProgramOutputRegisterToName[op.Value.OutputRegisterValue]);
    break;
  case CVPOperand::AddressRegister:
    out += "A0.x";
    break;
 ...
 */
static void ARBVertexProgramDumpInstr(const CVPInstruction &instr,
                                      std::string &out) {
  H_AUTO_OGL(ARBVertexProgramDumpInstr)
  nlassert(instr.Opcode < CVPInstruction::OpcodeCount);
  // Special case for EXP with a scalar output argument (y component) ->
  // translate to FRC
...
// ***************************************************************************
bool CDriverGL::setupARBVertexProgram(
    const CVPParser::TProgram &inParsedProgram, GLuint id,
  ...
  for (uint k = 0; k < parsedProgram.size(); ++k) {
    if (parsedProgram[k].Dest.Type == CVPOperand::OutputRegister &&
        parsedProgram[k].Dest.Value.OutputRegisterValue ==
            CVPOperand::OSecondaryColor) {
      writtenSpecularComponents |= parsedProgram[k].Dest.WriteMask;
    }
  }
  // tmp fix : write unwritten components of specular (seems that
  // glDisable(GL_COLOR_SUM_ARB) does not work in a rare case for me ...)
  if (writtenSpecularComponents != 0xf) {
    ...
    vpi.Src1.Value.InputRegisterValue =
        CVPOperand::IPosition; // tmp -> check that position is present
  ...
  */
  ...
  glGetError();
  nglProgramStringARB(GL_VERTEX_PROGRAM_ARB, GL_PROGRAM_FORMAT_ASCII_ARB,
                      (GLsizei)code.size(), code.c_str());
...
bool CDriverGL::compileEXTVertexShader(CVertexProgram *program) {
  ...
  *it = drvInfo = new CVertexProgamDrvInfosGL(this, it);
  ...
  if (!setupEXTVertexShader(parsedProgram, drvInfo->ID, drvInfo->Variants,
                            drvInfo->UsedVertexComponents)) {
    delete drvInfo;
    program->m_DrvInfo = NULL;
    //_GPUPrgDrvInfos.erase(it); // not needed as ~IProgramDrvInfos() already
    //does it
...
bool CDriverGL::activeVertexProgram(CVertexProgram *program) {
  ...
  if (program && !CDriverGL::compileVertexProgram(program))
    return false;

...
CVBDrvInfosD3D::CVBDrvInfosD3D(CDriverD3D *drv, ItVBDrvInfoPtrList it, CVertexBuffer *vb)
...
// nlinfo("lock from %s", VertexBufferPtr->getName().c_str());
	...
	if (Volatile)
	{
		...
		CVolatileVertexBuffer *&buffer = VolatileRAM ? (driver->_VolatileVertexBufferRAM[driver->_CurrentRenderPass & 1]) : (driver->_VolatileVertexBufferAGP[driver->_CurrentRenderPass & 1]);
		uint8 *ptr = (uint8 *)buffer->lock(end - begin, Stride, Offset);
		if (!ptr)
		{
			// buffer full, swap them
			CVolatileVertexBuffer *&bufferOther = VolatileRAM ? (driver->_VolatileVertexBufferRAM[(driver->_CurrentRenderPass + 1) & 1]) : (driver->_VolatileVertexBufferAGP[(driver->_CurrentRenderPass + 1) & 1]);
		...
		else
		{
			...
			do
			{
				success = _DeviceInterface->CreateVertexBuffer(size, RemapVertexBufferUsage[preferredMemory],
				              0, RemapVertexBufferPool[preferredMemory], &(info->VertexBuffer), NULL)
				    == D3D_OK;
				if (success)
					break;
    ...
    uint *stride)
{
	...
	while (ite != _VertexDeclarationList.end())
	{
		for (i = 0; i <= j; i++)
		{
			...
			if ((vertexElementNew.Stream != vertexElementOld.Stream) || (vertexElementNew.Type != vertexElementOld.Type) || (vertexElementNew.Offset != vertexElementOld.Offset) || (vertexElementNew.Method != vertexElementOld.Method) || (vertexElementNew.Usage != vertexElementOld.Usage) || (vertexElementNew.UsageIndex != vertexElementOld.UsageIndex))
			{
	...
	if (_AGPMemoryAllocated > 0)
	{
		...
		while (_AGPMemoryAllocated >= NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE)
		{
			    ...
			    == D3D_OK)
			{
				...
				if (((desc.Usage & (D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC)) == (D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC)) && (desc.Pool == D3DPOOL_DEFAULT))
				{
...
void CVolatileVertexBuffer::init(CVertexBuffer::TLocation location, uint size, uint maxSize, CDriverD3D *driver)
{
	...
	if (Driver->_DeviceInterface->CreateVertexBuffer(size, RemapVertexBufferUsage[location],
	        0, RemapVertexBufferPool[location], &VertexBuffer, NULL)
	    != D3D_OK)
	{
		...
		nlverify(Driver->_DeviceInterface->CreateVertexBuffer(size, RemapVertexBufferUsage[CVertexBuffer::RAMResident],
		             0, RemapVertexBufferPool[CVertexBuffer::RAMResident], &VertexBuffer, NULL)
		    != D3D_OK);

...
CVBDrvInfosD3D::CVBDrvInfosD3D(CDriverD3D *drv, ItVBDrvInfoPtrList it,
                               CVertexBuffer *vb)
    : IVBDrvInfos(drv, it, vb) {
...
#ifdef NL_DEBUG
  if (Locked) {
    nlinfo("VBuffer %s is still locked at destruction",
           VertexBufferPtr->getName().c_str());
...
// nlinfo("lock from %s", VertexBufferPtr->getName().c_str());
  ...
  if (dumpLockedBuffers) {
    ...
    for (std::set<CVBDrvInfosD3D *>::iterator it =
             driver->_LockedBuffers.begin();
         it != driver->_LockedBuffers.end(); ++it) {
      if (!(*it)->VertexBufferPtr) {
        nlinfo("Empty buffer");
      } else {
        nlinfo("Buffer %s at %p is Locked",
               (*it)->VertexBufferPtr->getName().c_str(), *it);
      }
    }
  }
#endif
  if (Volatile) {
    ...
    CVolatileVertexBuffer *&buffer =
        VolatileRAM
            ? (driver->_VolatileVertexBufferRAM[driver->_CurrentRenderPass & 1])
            : (driver
                   ->_VolatileVertexBufferAGP[driver->_CurrentRenderPass & 1]);
    uint8 *ptr = (uint8 *)buffer->lock(end - begin, Stride, Offset);
    if (!ptr) {
      // buffer full, swap them
      CVolatileVertexBuffer *&bufferOther =
          VolatileRAM ? (driver->_VolatileVertexBufferRAM
                             [(driver->_CurrentRenderPass + 1) & 1])
                      : (driver->_VolatileVertexBufferAGP
                             [(driver->_CurrentRenderPass + 1) & 1]);
    ...
    void *pbData;
    if (VertexBuffer->Lock(begin, end - begin, &pbData,
                           readOnly ? D3DLOCK_READONLY : 0) != D3D_OK)
      return NULL;
...
bool CDriverD3D::activeVertexBuffer(CVertexBuffer &VB) {
  ...
  const bool touched =
      (VB.getTouchFlags() & (CVertexBuffer::TouchedReserve |
                             CVertexBuffer::TouchedVertexFormat)) != 0;

  CVBDrvInfosD3D *info =
      static_cast<CVBDrvInfosD3D *>(static_cast<IVBDrvInfos *>(VB.DrvInfos));

  // Volatile buffers must be filled at each pass
  nlassertex(!info || !info->Volatile || VB.getKeepLocalMemory() ||
                 (info->VolatileLockTime == _CurrentRenderPass),
             ("Volatile buffers must be filled at each pass"));
  ...
  if (touched) {
    ...
    *ite = info;
    ...
    // Use vertex color ?
    info->UseVertexColor =
        (VB.getVertexFormat() & CVertexBuffer::PrimaryColorFlag) != 0;
    info->Stride = (uint8)VB.getVertexSize();

    // Create the vertex declaration
    if (!createVertexDeclaration(VB.getVertexFormat(), VB.getValueTypePointer(),
                                 &(info->VertexDecl), info->ColorOffset, false,
                                 false))
      return false;
    info->VertexDeclAliasDiffuseToSpecular = NULL;
    info->VertexDeclNoDiffuse = NULL;
    if (VB.hasValueEx(CVertexBuffer::PrimaryColor) &&
        !VB.hasValueEx(CVertexBuffer::SecondaryColor)) {
      uint colorOffset2;
      if (!createVertexDeclaration(VB.getVertexFormat(),
                                   VB.getValueTypePointer(),
                                   &(info->VertexDeclAliasDiffuseToSpecular),
                                   colorOffset2, true, false))
        return false;
    ...
    if (_NbNeLTextureStages == 3 && info->UseVertexColor) {
      ...
      // declared (example : lighted material + vertex color but, no
      // vertexColorLighted)
      uint colorOffset2;
      if (!createVertexDeclaration(
              VB.getVertexFormat(), VB.getValueTypePointer(),
              &(info->VertexDeclNoDiffuse), colorOffset2, false, true))
        return false;
    }
    ...
    if (info->Volatile) {
      ...
      bool success;
      do {
        success = _DeviceInterface->CreateVertexBuffer(
                      size, RemapVertexBufferUsage[preferredMemory], 0,
                      RemapVertexBufferPool[preferredMemory],
                      &(info->VertexBuffer), NULL) == D3D_OK;
        if (success)
          break;
  ...
  setVertexDecl(info->VertexDecl, info->VertexDeclAliasDiffuseToSpecular,
                info->VertexDeclNoDiffuse, info->Stride);
  // setVertexBuffer (info->VertexBuffer, info->Offset, info->Stride,
  // info->UseVertexColor, VB.getNumVertices(), VB.getPreferredMemory(),
  // info->Usage, info->ColorOffset);
  setVertexBuffer(info->VertexBuffer, info->Offset, info->Stride,
                  info->UseVertexColor, VB.getNumVertices(),
                  VB.getPreferredMemory(), info->Usage, info->ColorOffset);
  ...
  // For example, if the same vb is used for lightmap, then for standard shader,
  // then uv routing will be wrong
  ...
  /*
     ...
     static_cast<CVBDrvInfosD3D*>(static_cast<IVBDrvInfos*>(VB.DrvInfos));
      ...
      // No special flag for the lock, the driver should return a valid vertex
     buffer pointer with previous values. uint8 *out = info->lock (0, 0, false);
      nlassert (out);
      {
          temp.resize (size);
...
// ***************************************************************************
bool CDriverD3D::createVertexDeclaration(
    uint16 vertexFormat, const uint8 *typeArray,
    IDirect3DVertexDeclaration9 **vertexDecl, uint &colorOffset,
    bool aliasDiffuseToSpecular, bool bypassDiffuse, uint *stride) {
  ...
  if (aliasDiffuseToSpecular) {
    ...
    nlassert(vertexFormat &
             CVertexBuffer::PrimaryColorFlag); // diffuse required
    nlassert(
        !(vertexFormat &
          CVertexBuffer::SecondaryColorFlag)); // specular should not be used
  }
  ...
  for (i = 0; i < CVertexBuffer::NumValue; i++) {
    // Slot used ?
    if (vertexFormat & (1 << i)) {
      if ((i != CVertexBuffer::Weight && i != CVertexBuffer::PaletteSkin) ||
          _PixelShaderVersion !=
              D3DPS_VERSION(1,
                            4)) // fix for radeon 8500/9000/9200 : hand when
                                // this is declared and not used don't let gap
                                // for other cards else render bug on some ...
      {
        D3DVERTEXELEMENT9 &vertexElement = declaration.VertexElements[j];
        vertexElement.Stream = 0;
        vertexElement.Type =
            BYTE(RemapVertexBufferTypeNeL2D3D[(uint)typeArray[i]]);
        ...
        if (aliasDiffuseToSpecular && i == CVertexBuffer::PrimaryColor) {
          vertexElement.UsageIndex =
              1; // Map to specular stream -> this free PrimaryColor to build a
                 // constant Ueful to emulate per stage constant (which we can
                 // do on 2 stages only)
        } else {
          vertexElement.UsageIndex =
              BYTE(RemapVertexBufferIndexNeL2D3D[(uint)i]);
        }

        // nico : Fix for Radeon 7xxx series
        // Vertex declaration doesn't work when the vertex layout has vertex
        // color defined after tex coord. For example, the following layout
        // (Position/TexCoord0/Diffuse) will silently be converted into
        // (Position/Diffuse/TexCoord0) It seems that the driver tries to map
        // the vertex declaration to the matching FVF. FVF has a prefined order
        // and requires Diffuse to appear before texture coordinates in the
        // vertex. Don't know if it is a limitation of D3D related to the 7xxx
        // sries of if it is a driver bug. The D3D debug dll doesn't issue a
        // warning about it. To solve this 2 vertex streams are declared :
        // - First streams contains Position/Normal/Texcoord
        // - When vertex color are used, second stream contains Diffuse/Specular
        // vertex component(s) In fact the 2 streams map to the same vertex
        // buffer, but the 2nd stream has an added offset to point on the color
        // component I tried to add this offset directly into the vertex
        // declaration, but D3D complains about it... If the following field
        // contains a non 0 value, then a second stream must be used for
        // diffuse/specular with the given offset
  ...
  if (_DeviceInterface->CreateVertexDeclaration(
          declaration.VertexElements, &(declaration.VertexDecl)) != D3D_OK) {
    return false;
  }
...
bool CDriverD3D::initVertexBufferHard(uint agpMem, uint vramMem) {
  ...
  if (_AGPMemoryAllocated > 0) {
    ...
    _AGPMemoryAllocated =
        max(_AGPMemoryAllocated, (uint32)NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE);
    while (_AGPMemoryAllocated >= NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      IDirect3DVertexBuffer9 *vb;
      if (_DeviceInterface->CreateVertexBuffer(
              _AGPMemoryAllocated, D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC, 0,
              D3DPOOL_DEFAULT, &vb, NULL) == D3D_OK) {
        D3DVERTEXBUFFER_DESC desc;
        nlverify(vb->GetDesc(&desc) == D3D_OK);
        if (((desc.Usage & (D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC)) ==
             (D3DUSAGE_WRITEONLY | D3DUSAGE_DYNAMIC)) &&
            (desc.Pool == D3DPOOL_DEFAULT)) {
          nlinfo("%.d vertices supported", _MaxVerticesByVertexBufferHard);
          nlinfo("Success to allocate %.1f Mo of AGP VAR Ram",
                 _AGPMemoryAllocated / 1000000.f);
          vb->Release();
          break;
        } else
          vb->Release();
    ...
    if (_AGPMemoryAllocated < NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      ...
      nlwarning("Failed to allocate %.1f Mo of AGP VAR Ram",
                NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE / 1000000.f);
      ok = false;
    }
  }
  ...
  if (_VRAMMemoryAllocated > 0) {
    _VRAMMemoryAllocated &=
        ~15; // ensure 16-bytes aligned mem count (maybe useful :) ).
    _VRAMMemoryAllocated =
        max(_VRAMMemoryAllocated, (uint32)NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE);
    while (_VRAMMemoryAllocated >= NL3D_DRV_VERTEXARRAY_MINIMUM_SIZE) {
      IDirect3DVertexBuffer9 *vb;
      if (_DeviceInterface->CreateVertexBuffer(
              _VRAMMemoryAllocated, D3DUSAGE_WRITEONLY, 0, D3DPOOL_DEFAULT, &vb,
              NULL) == D3D_OK) {
        vb->Release();
        break;
      } else {
...
void CVolatileVertexBuffer::init(CVertexBuffer::TLocation location, uint size,
                                 uint maxSize, CDriverD3D *driver) {
  ...
  if (Driver->_DeviceInterface->CreateVertexBuffer(
          size, RemapVertexBufferUsage[location], 0,
          RemapVertexBufferPool[location], &VertexBuffer, NULL) != D3D_OK) {
    // Location in RAM must not failed
    nlassert(location != CVertexBuffer::RAMResident);

    // Allocate in RAM
    nlverify(Driver->_DeviceInterface->CreateVertexBuffer(
                 size, RemapVertexBufferUsage[CVertexBuffer::RAMResident], 0,
                 RemapVertexBufferPool[CVertexBuffer::RAMResident],
                 &VertexBuffer, NULL) != D3D_OK);
...
void *CVolatileVertexBuffer::lock(uint size, uint stride, uint &offset) {
  nlassertex(!Locked, ("Volatile buffer usage should follow an atomic "
                       "lock/unlock/render sequence"));
  H_AUTO_D3D(CVolatileVertexBuffer_lock)
  /* If not enough room to allocate this buffer, resise the buffer to
   * Size+Size/2 but do not reset CurrentIndex to be sure the buffer will be
   * large enough next pass. */
  ...
  if (CurrentIndex + size + stride > Size) {
    if (CurrentIndex + size > MaxSize && CurrentIndex != 0) {
      reset();
      if (size > MaxSize) {
        init(Location, std::max(std::min(Size + Size / 2, MaxSize), size),
             MaxSize, Driver);
      }
    } else {
      // Max size not reached, so reallocate
      init(Location,
           std::max(std::min(Size + Size / 2, MaxSize), CurrentIndex + size),
           MaxSize, Driver);
  ...
  // Lock the buffer, noblocking lock here if not the first allocation since a
  // reset
  ...
  if (CurrentIndex == 0) {
    ...
    nlverify(VertexBuffer->Lock(CurrentIndex, size, &pbData,

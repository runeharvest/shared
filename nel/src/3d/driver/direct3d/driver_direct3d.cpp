...
#endif
    void
    CDriverD3D::replaceArgumentAtStage(D3DTEXTURESTAGESTATETYPE state, DWORD stage, DWORD from, DWORD to)
{
...
#endif
    void
    CDriverD3D::replaceAllRGBArgumentAtStage(uint stage, DWORD from, DWORD to, DWORD blendOpFrom)
{
...
#endif
    void
    CDriverD3D::replaceAllAlphaArgumentAtStage(uint stage, DWORD from, DWORD to, DWORD blendOpFrom)
{
...
#endif
    void
    CDriverD3D::replaceAllArgumentAtStage(uint stage, DWORD from, DWORD to, DWORD blendOpFrom)
{
...
// So we can only emulate 1 per stage constant (it has already been setup in CDriverD3D::setupMaterial)
	...
	if (_NbNeLTextureStages == 3)
	{
		...
		if (_TextureStateCache[0][D3DTSS_COLOROP].Value == D3DTOP_MULTIPLYADD && _TextureStateCache[0][D3DTSS_COLORARG0].Value == D3DTA_DIFFUSE)
		{
...
bool CDriverD3D::handlePossibleSizeChange()
{
	...
	if (_CurrentMode.Windowed)
	{
		...
		if (((mode.Width != _CurrentMode.Width) || (mode.Height != _CurrentMode.Height)) && (mode.Width != 0) && (mode.Height != 0))
		{
			return reset(mode);
		}
	}
...
{
	switch (::MessageBoxW(_HWnd, nlUtf8ToWide(message), nlUtf8ToWide(title), ((type == retryCancelType) ? MB_RETRYCANCEL : (type == yesNoCancelType) ? MB_YESNOCANCEL
	        ...
	        ((icon == handIcon) ? MB_ICONHAND : (icon == questionIcon) ? MB_ICONQUESTION
...
void CDriverD3D::setColorMask(bool bRed, bool bGreen, bool bBlue, bool bAlpha)
{
	...
	setRenderState(D3DRS_COLORWRITEENABLE,
	    (bAlpha ? D3DCOLORWRITEENABLE_ALPHA : 0) | (bRed ? D3DCOLORWRITEENABLE_RED : 0) | (bGreen ? D3DCOLORWRITEENABLE_GREEN : 0) | (bBlue ? D3DCOLORWRITEENABLE_BLUE : 0));
}
...
void CDriverD3D::setPolygonMode(TPolygonMode mode)
{
	...
	setRenderState(D3DRS_FILLMODE, mode == Point ? D3DFILL_POINT : mode == Line ? D3DFILL_WIREFRAME
	                                                                            : D3DFILL_SOLID);
}
...
bool CDriverD3D::setMode(const GfxMode &mode)
{
	...
	if (!mode.Windowed)
	{
		...
		if (getModes(modes))
		{
			...
			{
				if (modes[i].Windowed == mode.Windowed && modes[i].Width == mode.Width && modes[i].Height == mode.Height && modes[i].Depth == mode.Depth && modes[i].Frequency == mode.Frequency)
				{
					found = true;
					break;
				}
...
bool CDriverD3D::fillPresentParameter(D3DPRESENT_PARAMETERS &parameters, D3DFORMAT &adapterFormat, const GfxMode &mode, const D3DDISPLAYMODE &adapterMode)
{
	...
	// Build a depth index
	const uint depthIndex = (mode.Depth == 16) ? 0 : (mode.Depth == 24) ? 1
	                                                                    : 2;
	...
	if (_DeviceInterface)
	{
		...
		if (_DeviceInterface->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &surface) == D3D_OK)
		{
			...
			if (surface->GetDesc(&desc) == D3D_OK)
			{
				...
				if ((desc.Format == D3DFMT_R8G8B8) || (desc.Format == D3DFMT_A8R8G8B8) || (desc.Format == D3DFMT_X8R8G8B8) || (desc.Format == D3DFMT_A8B8G8R8) || (desc.Format == D3DFMT_X8B8G8R8))
				{
					// Invert RGBA ?

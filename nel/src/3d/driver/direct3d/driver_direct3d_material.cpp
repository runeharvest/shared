...
const DWORD RemapTexArg0TypeNeL2D3D[CMaterial::TexOperatorCount] = {
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // Replace not used
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // Modulate not used
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
    ...
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // AddSigned not used
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // InterpolateTexture not used
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // InterpolatePrevious not used
    D3DTA_TFACTOR, // todo hulud constant color D3DTA_CONSTANT,
                   // // InterpolateDiffuse not used
    D3DTA_TFACTOR | D3DTA_ALPHAREPLICATE, // todo hulud constant color
...
void CMaterialDrvInfosD3D::buildTexEnv(uint stage,
  ...
  if (textured) {
    ...
    const DWORD *srcOp =
        (stage == 0) ? RemapTexArg0NeL2D3D : RemapTexArgNeL2D3D;

    ColorOp[stage] = ((stage == 0) ? RemapTexOpType0NeL2D3D
                                   : RemapTexOpTypeNeL2D3D)[env.Env.OpRGB];
    ...
    AlphaOp[stage] = ((stage == 0) ? RemapTexOpType0NeL2D3D
                                   : RemapTexOpTypeNeL2D3D)[env.Env.OpAlpha];
...
static inline void setShaderParam(CMaterialDrvInfosD3D *pShader, uint index,
                                  INT value) {
...
static inline void setShaderParam(CMaterialDrvInfosD3D *pShader, uint index,
  ...
  pShader->FXCache->Params.setVector(
      index, D3DXVECTOR4(values[0], values[1], values[2], values[3]));
}
...
static inline void setShaderParam(CMaterialDrvInfosD3D *pShader, uint index,
  ...
  nlassert(pShader->FXCache);
  pShader->FXCache->Params.setVector(
      index, D3DXVECTOR4(vector[0], vector[1], vector[2], vector[3]));
}
...
bool CDriverD3D::setupMaterial(CMaterial &mat) {
  ...
  if (!mat._MatDrvInfo) {
    ...
    ItMatDrvInfoPtrList it = _MatDrvInfos.insert(
        _MatDrvInfos.end(), (NL3D::IMaterialDrvInfos *)NULL);
  ...
  pShader = static_cast<CMaterialDrvInfosD3D *>(
      (IMaterialDrvInfos *)(mat._MatDrvInfo));

  // Now we can get the supported shader from the cache.
  CMaterial::TShader matShader =
      _PixelProgramUser ? CMaterial::Program : mat.getShader();
  ...
  }
  {
    ...
    if (matShader != _CurrentMaterialSupportedShader) {
      // if current shader is normal shader, then must restore uv routing,
      // because it may have been changed by a previous shader (such as
      // lightmap)
...

  {
    ...
    for (numUsedTexStages = 0; numUsedTexStages < IDRV_MAT_MAXTEXTURES;
         ++numUsedTexStages) {
      if (mat.getTexture(uint8(numUsedTexStages)) == NULL)
        break;
    }
    ...
    if (touched) {
      if (touched & IDRV_TOUCHED_SHADER) {
        ...
        if (mat.getShader() != CMaterial::Specular &&
            mat.getShader() != CMaterial::Normal) {
          pShader->FXCache = new CFXCache;
        }
      }
      /* Exception: if only Textures are modified in the material, no need to
         "Bind OpenGL States", or even to test for change, because textures are
         activated alone, see below. No problem with delete/new problem (see
         below), because in this case, IDRV_TOUCHED_ALL is set (see above).
      */
      ...
      if (touched & (~_MaterialAllTextureTouchedFlag)) {
        ...
        if (touched & IDRV_TOUCHED_LIGHTING) {
          ...
          if (mat.isLighted()) {
            ...
            NL_D3DCOLORVALUE_RGBA(pShader->Material.Emissive,
                                  mat.getEmissive());
        ...
        if (touched & IDRV_TOUCHED_ALPHA_TEST_THRE) {
          float alphaRef =
              (float)floor(mat.getAlphaTestThreshold() * 255.f + 0.5f);
        ...
        if (touched & IDRV_TOUCHED_TEXENV) {
          ...
          // Do not do it for Lightmap and per pixel lighting , because done in
          // multipass in a very special fashion. This avoid the useless
          // multiple change of texture states per lightmapped object. Don't do
          // it also for Specular because the EnvFunction and the TexGen may be
          // special.
          if (matShader == CMaterial::Normal) {
            uint stage;
            for (stage = 0; stage < maxTexture; ++stage) {
              // Build the tex env
              pShader->buildTexEnv(stage, mat._TexEnvs[stage],
                                   mat.getTexture(uint8(stage)) != NULL);
            }
          }
        }
          ...
          for (stage = 0; stage < maxTexture; ++stage) {
            ...
            if (text && text->isTextureCube()) {
              if (mat.getTexCoordGen(stage)) {
                // Texture cube
                pShader->TexGen[stage] =
                    RemapTexGenCubeTypeNeL2D3D[mat.getTexCoordGenMode(stage)];

                // Texture matrix
                pShader->ActivateSpecularWorldTexMT[stage] =
                    mat.getTexCoordGenMode(stage) ==
              ...
              if (mat.getTexCoordGen(stage)) {
                pShader->TexGen[stage] =
                    RemapTexGenTypeNeL2D3D[mat.getTexCoordGenMode(stage)];

                // Texture matrix
                pShader->ActivateInvViewModelTexMT[stage] =
                    mat.getTexCoordGenMode(stage) ==
        ...
        if (touched & (IDRV_TOUCHED_TEXENV | IDRV_TOUCHED_BLEND |
                       IDRV_TOUCHED_ALPHA_TEST)) {
          std::fill(pShader->RGBPipe, pShader->RGBPipe + IDRV_MAT_MAXTEXTURES,
                    true);
          std::fill(pShader->AlphaPipe,
                    pShader->AlphaPipe + IDRV_MAT_MAXTEXTURES, true);
          ...
          if (matShader == CMaterial::Normal) {
            ...
            _needPixelShader =
                (numConstants > 1) ||
                ((numConstants == 1) && pShader->NeedsConstantForDiffuse);
            ...
            if (_needPixelShader) {
              ...
              for (stage = 0; stage < (uint)maxTexture; stage++) {
                // Stage used ?
                normalShaderDesc.StageUsed[stage] =
                    mat.getTexture(uint8(stage)) != NULL;
                normalShaderDesc.TexEnvMode[stage] =
                    mat.getTexEnvMode(uint8(stage));
              }

              if (_PixelProgram) {
#ifdef NL_DEBUG_D3D
                // Check, should not occur
                nlassertex(_PixelShader, ("STOP : no pixel shader available. "
                                          "Can't render this material."));
#endif // NL_DEBUG_D3D

                // Build the pixel shader
                pShader->PixelShader =
                    buildPixelShader(normalShaderDesc, false);
                if (!mat.isLighted())
                  pShader->PixelShaderUnlightedNoVertexColor =
                      buildPixelShader(normalShaderDesc, true);
                else
                  pShader->PixelShaderUnlightedNoVertexColor = NULL;
              } else {
                // Possible configurations are :
                // unlighted, 1 constant diffuse + 1 per stage constant
                // unlighted, 2 constants (second constant emulated with
                // material emissive + diffuse), possibly with vertex color
                // (aliased to specular)

                // If there are no pixel shader then we're likely to have three
                // stage or less
                ...
                computeRelevantTexEnv(mat, pShader->RGBPipe,
                                      pShader->AlphaPipe);
              }
            } else {
              // compute relevant parts of the pipeline
        ...
        // Since modified, must rebind all D3D states. And do this also for the
        // delete/new problem.
        /* If an old material is deleted, _CurrentMaterial is invalid. But this
           is grave only if a new material is created, with the same pointer
           (bad luck). Since an newly allocated material always pass here before
           use, we are sure to avoid any problems.
        */
...

  {
    ...
    if (matShader == CMaterial::Normal ||
        ((matShader == CMaterial::Program) &&
         (_PixelProgramUser->features().MaterialFlags &
  ...
  // Do not do it for Lightmap and per pixel lighting , because done in
  // multipass in a very special fashion. This avoid the useless multiple change
  // of texture states per lightmapped object. Don't do it also for Specular
  // because the EnvFunction and the TexGen may be special.
  {
    ...
    if (matShader == CMaterial::Normal ||
      ...
      for (stage = 0; stage < maxTexture; ++stage) {
        ...
        if (text || (stage == 0)) {
          if (matShader == CMaterial::Program) {
            ...
            if (pShader->RGBPipe[stage]) {
              ...
              setTextureState(stage, D3DTSS_COLORARG1,
                              pShader->ColorArg1[stage]);
              if (pShader->NumColorArg[stage] > 1) {
                setTextureState(stage, D3DTSS_COLORARG2,
                                pShader->ColorArg2[stage]);
                if (pShader->NumColorArg[stage] > 2) {
                  setTextureState(stage, D3DTSS_COLORARG0,
                                  pShader->ColorArg0[stage]);
                }
              }
            }
            if (pShader->AlphaPipe[stage]) {
              setTextureState(stage, D3DTSS_ALPHAOP, pShader->AlphaOp[stage]);
              setTextureState(stage, D3DTSS_ALPHAARG1,
                              pShader->AlphaArg1[stage]);
              if (pShader->NumAlphaArg[stage] > 1) {
                setTextureState(stage, D3DTSS_ALPHAARG2,
                                pShader->AlphaArg2[stage]);
                if (pShader->NumAlphaArg[stage] > 2) {
                  setTextureState(stage, D3DTSS_ALPHAARG0,
                                  pShader->AlphaArg0[stage]);
                }
              }
            }
            // If there is one constant and the tfactor is not needed for
            // diffuse, use the tfactor as constant
            if (pShader->ConstantIndex == stage)
              setRenderState(D3DRS_TEXTUREFACTOR,
                             pShader->ConstantColor[stage]);
          } else {
            float colors[4];
        ...
        setTextureIndexMode(stage,
                            pShader->TexGen[stage] != D3DTSS_TCI_PASSTHRU,
                            pShader->TexGen[stage]);
        ...
        D3DXMATRIX userMtx;
        if (needUserMtx) {
          // If tex gen mode is used, or a cube texture is used, then use the
          // matrix 'as it'. Must build a 3x3 matrix for 2D texture coordinates
          // in D3D (which is kind of weird ...)
          if (pShader->TexGen[stage] != D3DTSS_TCI_PASSTHRU ||
              (mat.getTexture(uint8(stage)) &&
               mat.getTexture(uint8(stage))->isTextureCube())) {
        ...
        if (pShader->ActivateSpecularWorldTexMT[stage]) {
          ...
          if (!needUserMtx)
            setMatrix((D3DTRANSFORMSTATETYPE)(D3DTS_TEXTURE0 + stage),
          ...
          if (_NbNeLTextureStages == 3) {
            // Fix for Radeon 7xxx
            // In some situation, texture transform stays enabled after a
            // material change, so enable it all the time and use identity
            // matrix instead of the D3DTTFF_DISABLE flag
            setTextureState(stage, D3DTSS_TEXTURETRANSFORMFLAGS,
                            D3DTTFF_COUNT3);
            setMatrix((D3DTRANSFORMSTATETYPE)(D3DTS_TEXTURE0 + stage),
                      needUserMtx ? userMtx : _D3DMatrixIdentity);
          } else {
            // Set the driver matrix
            if (needUserMtx) {
              setTextureState(stage, D3DTSS_TEXTURETRANSFORMFLAGS,
  ...
  if (_CurrentMaterial != &mat) {
...

    {
      ...
      // Handle backside
      if (_CullMode == CCW) {
        setRenderState(D3DRS_CULLMODE, _DoubleSided      ? D3DCULL_NONE
                                       : _InvertCullMode ? D3DCULL_CCW
                                                         : D3DCULL_CW);
      } else {
        setRenderState(D3DRS_CULLMODE, _DoubleSided      ? D3DCULL_NONE
                                       : _InvertCullMode ? D3DCULL_CW
                                                         : D3DCULL_CCW);
      }
    }
...

    {
      ...
      setRenderState(D3DRS_ZWRITEENABLE,
                     (flags & IDRV_MAT_ZWRITE) ? TRUE : FALSE);
    ...
    case CMaterial::LightMap: {
      ...
      // if the dynamic lightmap light has changed since the last render (should
      // not happen), resetup normal way is that setupLightMapDynamicLighting()
      // is called at begin of setupMaterial() if shader different from prec
      ...
      if (mat.getBlend()) {
        if (mat._LightMapsMulx2) {
          ...
          case 0:
            activeShader(&_ShaderLightmap0BlendX2);
            break;
          case 1:
            activeShader(&_ShaderLightmap1BlendX2);
            break;
          case 2:
            activeShader(&_ShaderLightmap2BlendX2);
            break;
          case 3:
            activeShader(&_ShaderLightmap3BlendX2);
            break;
          default:
            activeShader(&_ShaderLightmap4BlendX2);
            break;
          }
        } else {
          switch (lightmapCount) {
        ...
        if (mat._LightMapsMulx2) {
          switch (lightmapCount) {
          case 0:
            activeShader(&_ShaderLightmap0X2);
            break;
          case 1:
            activeShader(&_ShaderLightmap1X2);
            break;
          case 2:
            activeShader(&_ShaderLightmap2X2);
            break;
          case 3:
            activeShader(&_ShaderLightmap3X2);
            break;
          default:
            activeShader(&_ShaderLightmap4X2);
            break;
          }
        } else {
          switch (lightmapCount) {
      ...
      CShaderDrvInfosD3D *shaderInfo = static_cast<CShaderDrvInfosD3D *>(
      ...
      // Set the ambiant for 8Bit Light Compression
      {
        ...
        for (lightmap = 0; lightmap < (sint)mat._LightMaps.size(); lightmap++) {
          if (lightmapMask & (1 << lightmap)) {
            ...
            r += ((uint32)ambFactor.R * ((uint32)lmcAmb.R + (lmcAmb.R >> 7))) >>
                 8;
            g += ((uint32)ambFactor.G * ((uint32)lmcAmb.G + (lmcAmb.G >> 7))) >>
                 8;
            b += ((uint32)ambFactor.B * ((uint32)lmcAmb.B + (lmcAmb.B >> 7))) >>
                 8;
          }
        }
      ...
      for (lightmap = 0; lightmap < (sint)mat._LightMaps.size(); lightmap++) {
        if (lightmapMask & (1 << lightmap)) {
          ...
          if (text != NULL &&
              !setShaderTexture(lightmapCount + 1, text, pShader->FXCache))
            return false;
          ...
          lmapFactor.R = (uint8)(((uint32)lmapFactor.R *
                                  ((uint32)lmcDiff.R + (lmcDiff.R >> 7))) >>
                                 8);
          lmapFactor.G = (uint8)(((uint32)lmapFactor.G *
                                  ((uint32)lmcDiff.G + (lmcDiff.G >> 7))) >>
                                 8);
          lmapFactor.B = (uint8)(((uint32)lmapFactor.B *
                                  ((uint32)lmcDiff.B + (lmcDiff.B >> 7))) >>
                                 8);
          lmapFactor.A = 255;

          // Set the lightmap color factor into shader
          setShaderParam(pShader, lightmapCount + 1,
                         (INT)NL_D3DCOLOR_RGBA(lmapFactor));
    ...
    case CMaterial::Specular: {
      ...
      if (texture->isTextureCube()) {
        ...
        setMatrix((D3DTRANSFORMSTATETYPE)(D3DTS_TEXTURE0 + 1),
                  _D3DSpecularWorldTex);
      }
      ...
      setTextureState(1, D3DTSS_COLORARG2,
    ...
    case CMaterial::Cloud: {
      ...
      // Get the shader
      nlassert(_CurrentShader);
      //				CShaderDrvInfosD3D *shaderInfo =
      //static_cast<CShaderDrvInfosD3D*>((IShaderDrvInfos*)_CurrentShader->_DrvInfo);
      ...
      // Set the texture
      if (!setShaderTexture(0, mat.getTexture(0), pShader->FXCache) ||
          !setShaderTexture(1, mat.getTexture(1), pShader->FXCache))
        return false;
    } break;
    case CMaterial::Water: {
      H_AUTO_D3D(CDriverD3D_setupMaterial_setupWaterShader)
      activeShader(mat.getTexture(3) ? &_ShaderWaterDiffuse
      ...
      if (_PixelShaderVersion < D3DPS_VERSION(2, 0)) {
        if (_PixelShaderVersion < D3DPS_VERSION(1, 4)) {
          if (mat.getTexture(1) && mat.getTexture(1)->isBumpMap()) {
            float factor = NLMISC::safe_cast<CTextureBump *>(mat.getTexture(1))
          ...
          if (mat.getTexture(0) && mat.getTexture(0)->isBumpMap()) {
            float factor = NLMISC::safe_cast<CTextureBump *>(mat.getTexture(0))
                               ->getNormalizationFactor();
        ...
        if (mat.getTexture(0) && mat.getTexture(0)->isBumpMap()) {
          float factor =
              0.25f * NLMISC::safe_cast<CTextureBump *>(mat.getTexture(0))
                          ->getNormalizationFactor();
...
static void add(std::set<uint> &dest, const std::set<uint> &toAdd) {
  for (std::set<uint>::const_iterator it = toAdd.begin(); it != toAdd.end();
       ++it) {
    dest.insert(*it);
  }
}
...
// ***************************************************************************
bool CDriverD3D::needsConstants(uint &numConstant, uint &firstConstant,
  ...
  for (uint i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
    ...
    for (uint l = 0; l < OpNumArg[texEnv.Env.OpRGB]; ++l) {
      ...
      if (texEnv.getColorArg(l) == CMaterial::Constant) {
        rgbPipe[1].insert(i);
      } else if (texEnv.getColorArg(l) == CMaterial::Previous) {
        if (texEnv.getColorOperand(l) == CMaterial::SrcColor ||
            texEnv.getColorOperand(l) == CMaterial::InvSrcColor) {
          add(rgbPipe[1], rgbPipe[0]);
        }
        if (texEnv.getColorOperand(l) == CMaterial::SrcAlpha ||
            texEnv.getColorOperand(l) == CMaterial::InvSrcColor) {
...
bool CDriverD3D::needsConstantForDiffuse(CMaterial &mat) {
  ...
  // Inspect the RGB & alpha pipe : if a diffuse value manage to propagate to
  // the last stage, and if the pipe is required for fragment blending, then
  // diffuse is useful in the shader, and thus must be replaced by constant for
  // unlighted material
  ...
  for (uint i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
    ...
    if (texEnv.Env.OpRGB == CMaterial::InterpolateDiffuse ||
        (texEnv.Env.OpRGB == CMaterial::InterpolatePrevious && i == 0)) {
      newPropRGB = true;
    }
    if (texEnv.Env.OpRGB == CMaterial::InterpolatePrevious) {
      if (propAlpha)
    ...
    if (texEnv.Env.OpAlpha == CMaterial::InterpolateDiffuse ||
        (texEnv.Env.OpAlpha == CMaterial::InterpolatePrevious && i == 0)) {
      newPropAlpha = true;
    }
    if (texEnv.Env.OpAlpha == CMaterial::InterpolatePrevious) {
      if (propAlpha)
    ...
    for (uint l = 0; l < OpNumArg[texEnv.Env.OpRGB]; ++l) {
      // color pipe
      if (texEnv.getColorArg(l) == CMaterial::Diffuse ||
          (texEnv.getColorArg(l) == CMaterial::Previous && i == 0)) {
        newPropRGB = true;
      } else if (texEnv.getColorArg(l) == CMaterial::Previous) {
        if (texEnv.getColorOperand(l) == CMaterial::SrcColor ||
            texEnv.getColorOperand(l) == CMaterial::InvSrcColor) {
    ...
    // alpha pipe
    for (uint l = 0; l < OpNumArg[texEnv.Env.OpAlpha]; ++l) {
      if (texEnv.getAlphaArg(l) == CMaterial::Diffuse ||
          (texEnv.getAlphaArg(l) == CMaterial::Previous && i == 0)) {
        newPropAlpha = true;
      }
      if (texEnv.getAlphaArg(l) == CMaterial::Previous) {
        if (propAlpha)
...
// ***************************************************************************
void CDriverD3D::computeRelevantTexEnv(CMaterial &mat,
                                       bool rgbPipe[IDRV_MAT_MAXTEXTURES],
                                       bool alphaPipe[IDRV_MAT_MAXTEXTURES]) {
  // TODO : see if worth the trouble (during profile, I see a very small gain :
  // 0.03% less time spent in driver for my test scene)
  ...
  uint numStages = 0;
  for (uint i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
    if (mat.getTexture(uint8(i)) == NULL)
      break;
    ++numStages;
  }
  ...
  */
  ...
  while (currStage >= 0) {
    ...
    if (rgbUsed) {
      ...
      for (uint l = 0; l < OpNumArg[texEnv.Env.OpRGB]; ++l) {
        if (texEnv.getColorArg(l) == CMaterial::Previous ||
            (currStage != 0 && texEnv.getColorArg(l) == CMaterial::Texture &&
             mat.getTexEnvOpRGB(currStage - 1) == CMaterial::EMBM)) {
          if (texEnv.getColorOperand(l) == CMaterial::SrcColor ||
              texEnv.getColorOperand(l) == CMaterial::InvSrcColor) {
            newRGBUsed = true;
          }
          if (texEnv.getColorOperand(l) == CMaterial::SrcAlpha ||
              texEnv.getColorOperand(l) == CMaterial::InvSrcAlpha) {
            newAlphaUsed = true;
          }
        }
      }
    ...
    if (alphaUsed) {
      ...
      for (uint l = 0; l < OpNumArg[texEnv.Env.OpAlpha]; ++l) {
        if (texEnv.getAlphaArg(l) == CMaterial::Previous ||
            (currStage != 0 && texEnv.getAlphaArg(l) == CMaterial::Texture &&
             mat.getTexEnvOpRGB(currStage - 1) == CMaterial::EMBM)) {
          if (texEnv.getAlphaOperand(l) == CMaterial::SrcAlpha ||
              texEnv.getAlphaOperand(l) == CMaterial::InvSrcAlpha) {
            newAlphaUsed = true;
          }
        }
      }
...
void buildColorOperation(
    string &dest, const char *prefix, const char *destSizzle, uint stage,
    CMaterial::TTexOperator &op, CMaterial::TTexSource src0,
    CMaterial::TTexSource src1, CMaterial::TTexSource src2,
    CMaterial::TTexOperand &op0, CMaterial::TTexOperand &op1,
    CMaterial::TTexOperand &op2, bool unlightedNoVertexColor) {
  ...
  if (op != CMaterial::Mad) {
    const char *remapPSThirdArguments =
        ((stage == 0)
             ? RemapPSThirdArguments0
             : RemapPSThirdArguments)[op][(uint)unlightedNoVertexColor];
    if (remapPSThirdArguments[0] != 0) {
      dest += ", ";
      dest += remapPSThirdArguments;

      // Need stage postfix ?
      if ((op == CMaterial::InterpolateTexture) ||
          (op == CMaterial::InterpolateConstant))
        dest += toString(stage);
  ...
  dest += ((stage == 0) ? RemapPSArguments0
                        : RemapPSArguments)[src0][(uint)unlightedNoVertexColor];
  ...
  if (op != CMaterial::Replace) {
    ...
    dest +=
        ((stage == 0) ? RemapPSArguments0
                      : RemapPSArguments)[src1][(uint)unlightedNoVertexColor];
  ...
  if (op == CMaterial::Mad) {
    ...
    dest +=
        ((stage == 0) ? RemapPSArguments0
                      : RemapPSArguments)[src2][(uint)unlightedNoVertexColor];
...
CDriverD3D::buildPixelShader(const CNormalShaderDesc &normalShaderDesc,
  ...
  std::list<CNormalShaderDesc> &normalPixelShaders =
      _NormalPixelShaders[(uint)unlightedNoVertexColor];
  ...
  for (i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
    ...
    if (normalShaderDesc.StageUsed[i]) {
      ...
      nlassert(
          texEnv.Env.OpRGB !=
          CMaterial::EMBM); // Pixel shader equivalent for EMBM not supported
                            // For now this is not a problem because
                            // buildPixelShader is used when the shader uses per
                            // stage color (which is not always supported by the
                            // fixed pixel pipe), and currently all our shaders
                            // with EMBM don't use that feature.
#endif
  ...
  for (i = 0; i < IDRV_MAT_MAXTEXTURES; i++) {
    ...
    if (normalShaderDesc.StageUsed[i]) {
      ...
      buildColorOperation(shaderText, "", ".xyz", i, texOp, src0, src1, src2,
                          op0, op1, op2, unlightedNoVertexColor);
      ...
      buildColorOperation(shaderText, "+", ".w", i, texOp, src0, src1, src2,
                          op0, op1, op2, unlightedNoVertexColor);
    }
    // No texture shader ?
    else if (i == 0) {
      ...
      buildColorOperation(shaderText, "", ".xyz", i, texOp, src0, src0, src0,
                          op0, op0, op0, unlightedNoVertexColor);

      // Alpha
      texOp = CMaterial::Replace;
      src0 = CMaterial::Diffuse;
      op0 = CMaterial::SrcAlpha;
      buildColorOperation(shaderText, "+", ".w", i, texOp, src0, src0, src0,
                          op0, op0, op0, unlightedNoVertexColor);
    }
  }
  ...
  if (D3DXAssembleShader(shaderText.c_str(), (UINT)shaderText.size(), NULL,
                         NULL, 0, &pShader, &pErrorMsgs) == D3D_OK) {
    IDirect3DPixelShader9 *shader;
    if (_DeviceInterface->CreatePixelShader(
            (DWORD *)pShader->GetBufferPointer(), &shader) == D3D_OK)
...
void CDriverD3D::getNumPerStageConstant(uint &lightedMaterial,

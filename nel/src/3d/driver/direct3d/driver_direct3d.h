...
// Define this to force the use of pixel shader in the normal shaders (default
// is undefined) #define NL_FORCE_PIXEL_SHADER_USE_FOR_NORMAL_SHADERS

// Define this to enable profiling by the NV Perf HUD tool (default is
// undefined) #define NL_D3D_USE_NV_PERF_HUD
...
public:
  ...
  ~CFpuRestorer() { _controlfp(_FPControlWord, ~0); }
...
class CFpuChecker {
public:
  CFpuChecker(const char *label) {
    _FPControlWord = _controlfp(0, 0);
    _Label = label;
  }
  ~CFpuChecker() {
    unsigned int newFP = _controlfp(0, 0);
    if ((newFP & (_MCW_DN | _MCW_IC | _MCW_RC | _MCW_PC)) !=
        (_FPControlWord & (_MCW_DN | _MCW_IC | _MCW_RC | _MCW_PC))) {
      nlwarning(_Label);
      nlassert(0);
    }
  }
...
public:
  ...
  TOcclusionQueryList::iterator
      Iterator;                 // iterator in owner driver list of queries
  TOcclusionType OcclusionType; // current type of occlusion
  uint VisibleCount;            // number of samples that passed the test
  bool QueryIssued;
  bool WasLost; // tells that query was lost, so calls to end() will have not
                // effects (there's no matching begin)
...
public:
  IShaderDrvInfos(CDriverD3D *drv, ItShaderDrvInfoPtrList it) {
    _Driver = drv;
    _DriverIterator = it;
  }
...
public:
  ...
  */
  ...
  CTextureDrvInfosD3D(IDriver *drv, ItTexDrvInfoPtrMap it, CDriverD3D *drvGl,
                      bool renderTarget);
  ~CTextureDrvInfosD3D();
...
public:
  ...
  uint ColorOffset; // Fix for Radeon 7xxx series -> see remarks in
                    // CDriverD3D::createVertexDeclaration
...
public:
  ...
  CDriverD3D *Driver;
  std::vector<uint32>
      RamVersion; // If device doesn't support 32 bit indexes, works in ram
                  // (unless it a 16 bit index buffer)
...
public:
  ...
  bool operator==(const CNormalShaderDesc &other) const {
    ...
    for (i = 0; i < IDRV_MAT_MAXTEXTURES; i++)
      if ((StageUsed[i] != other.StageUsed[i]) ||
          (StageUsed[i] && (TexEnvMode[i] != other.TexEnvMode[i])))
        return false;
    return true;
  }
...
public:
  ...
  // use STL allocator for fast alloc. this works because objects are small ( <
  // 128 bytes)
  void *operator new(size_t size) {
    return CStateRecord::Allocator.allocate(size);
  }
  void *operator new(size_t size, int /* blockUse */,
  ...
  void operator delete(void *block, int /* blockUse */,
                       char const * /* fileName */, int /* lineNumber */) {
    // TODO: add memory leaks detector
    CStateRecord::Allocator.deallocate((uint8 *)block, 1);
  }
...
template <class T> class CFXInputValue {
public:
  T Value;
  bool Set;
...
public:
  ...
  {
      return std::equal(Textures, Textures +
  CShaderDrvInfosD3D::MaxShaderTexture, other.Textures) && std::equal(Vectors,
  Vectors + CShaderDrvInfosD3D::MaxShaderTexture, other.Vectors) &&
             std::equal(Colors, Colors + CShaderDrvInfosD3D::MaxShaderTexture,
  other.Colors) && std::equal(Floats, Floats +
  CShaderDrvInfosD3D::MaxShaderTexture, other.Floats);
  }
  */
  void reset() {
...
public:
  ...
  bool NeedsConstantForDiffuse; // Must use TFactor if not vertex color in the
                                // vertex buffer
  bool MultipleConstantNoPixelShader; // Multiple constant are possibly needed
                                      // to setup the material. This flag is set
                                      // only if the device has no pixel shaders
                                      // In this case diffuse color will be
                                      // emulated by using an unlighted material
                                      // with ambient
  bool MultiplePerStageConstant; // Are there more than one per-stage constant
                                 // in the material ?
  uint8 ConstantIndex;  // Index of the constant color to use (when only one
                        // constant color is needed and NeedsConstantForDiffuse
                        // == false);
  uint8 ConstantIndex2; // stage at which the 2nd constant is used (for
                        // emulation without pixel shaders)

  CRGBA Constant2; // value of the 2nd constant being used (for emulation
                   // without pixel shaders)

  uint NumUsedTexStages; // Last number of textures that were set in the
                         // material Tex Env are only built for stages at which
                         // textures are set so if the number of used texture
  ...
  CMaterialDrvInfosD3D(IDriver *drv, ItMatDrvInfoPtrList it)
      : IMaterialDrvInfos(drv, it) {
  ...
  ~CMaterialDrvInfosD3D() { delete FXCache; }
...
/* Volatile buffers.
 *
 * The volatile buffer system is a double buffer allocated during the render
 * pass by objects that needs a temporary buffer to render vertex or indexes.
 * Each lock allocates a buffer region and locks it with the NOOVERWRITE flag.
 * Locks are not blocked. The buffer is reset at each frame begin. There is a
 * double buffer to take benefit of the "over swapbuffer" parallelisme.
 */
...
public:
  ...
  /* size is in bytes */
  void init(CVertexBuffer::TLocation location, uint size, uint maxSize,
            CDriverD3D *driver);
  void release();
  ...
  // Runtime reset (called at the beginning of the frame rendering), blocking
  // lock here.
  void reset();
};
...
public:
  ...
  /* size is in bytes */
  void init(CIndexBuffer::TLocation location, uint sizeInBytes, uint maxSize,
            CDriverD3D *driver, CIndexBuffer::TFormat format);
  void release();
...
public:
  ...
  virtual bool setDisplay(nlWindow wnd, const GfxMode &mode, bool show,
                          bool resizeable);
  ...
  virtual bool uploadTextureCube(ITexture & /* tex */,
                                 NLMISC::CRect & /* rect */,
                                 uint8 /* nNumMipMap */, uint8 /* nNumFace */) {
    return false;
  };
  ...
  virtual bool
  stretchRect(ITexture *srcText, NLMISC::CRect &srcRect, ITexture *destText,
              NLMISC::CRect &destRect); // Only 32 bits back buffer supported
  ...
  virtual bool setRenderTarget(ITexture *tex, uint32 x, uint32 y, uint32 width,
                               uint32 height, uint32 mipmapLevel,
                               uint32 cubeFace);
  virtual ITexture *getRenderTarget() const;
  virtual bool copyTargetToTexture(ITexture *tex, uint32 offsetx,
                                   uint32 offsety, uint32 x, uint32 y,
  ...
  virtual void setFrustum(float left, float right, float bottom, float top,
                          float znear, float zfar, bool perspective = true);
  ...
  virtual bool renderLinesWithIndexOffset(CMaterial &mat, uint32 firstIndex,
                                          uint32 nlines, uint indexOffset);
  virtual bool renderTrianglesWithIndexOffset(CMaterial &mat, uint32 firstIndex,
                                              uint32 ntris, uint indexOffset);
  virtual bool renderSimpleTrianglesWithIndexOffset(uint32 firstIndex,
                                                    uint32 ntris,
                                                    uint indexOffset);
  // Unindexed primitives with index offset
  virtual bool renderRawPoints(CMaterial &Mat, uint32 startIndex,
                               uint32 numPoints);
  virtual bool renderRawLines(CMaterial &Mat, uint32 startIndex,
                              uint32 numLines);
  virtual bool renderRawTriangles(CMaterial &Mat, uint32 startIndex,
                                  uint32 numTris);
  virtual bool renderRawQuads(CMaterial &Mat, uint32 startIndex,
                              uint32 numQuads);
  //
  ...
  // virtual	void
  // profileIBAllocation(std::vector<std::string> &result);
  ...
  virtual void setPerPixelLightingLight(CRGBA /* diffuse */,
                                        CRGBA /* specular */,
                                        float /* shininess */) {}
  ...
  // todo hulud d3d adressing mode
  virtual bool
  supportTextureAddrMode(CMaterial::TTexAddressingMode /* mode */) const {
    return false;
  };
  // todo hulud d3d adressing mode
  ...
  virtual bool supportIndexOffset() const {
    return true; /* always supported with D3D driver */
  }
  ...
  // Monitor properties
  virtual bool
  setMonitorColorProperties(const CMonitorColorProperties &properties);
   ...
   */
  virtual bool
  supportGeometryProgram(CGeometryProgram::TProfile profile) const {
    return false;
  }
   ...
   * the state of the active pixel program is undefined behaviour afterwards.
   */
  virtual bool compileGeometryProgram(CGeometryProgram *program) {
    return false;
  }
   ...
   * immediately.
   */
  virtual bool activeGeometryProgram(CGeometryProgram *program) {
    return false;
  }
  // @}
  ...
  virtual void setUniform4f(TProgram program, uint index,
                            const NLMISC::CVector &v, float f3);
  virtual void setUniform4f(TProgram program, uint index,
                            const NLMISC::CRGBAF &rgba);
  ...
  virtual bool setUniformDriver(
      TProgram program); // set all driver-specific features params (based on
                         // program->features->DriverFlags)
  virtual bool setUniformMaterial(
      TProgram program,
      CMaterial &material); // set all material-specific feature params (based
                            // on program->features->MaterialFlags)
  virtual void
  setUniformParams(TProgram program,
                   CGPUProgramParams &
                       params); // set all user-provided params from the storage
  ...
  virtual void startBench(bool wantStandardDeviation = false,
                          bool quick = false, bool reset = true);
...
public:
  ...
  struct CTextureIndexState : public CRenderVariable {
    CTextureIndexState() { Type = TextureIndexState; }
    DWORD StageID;
    DWORD TexGenMode;
    DWORD UVChannel;
    bool TexGen;
  ...
  struct CPixelShaderConstantState : public CRenderVariable {
    CPixelShaderConstantState() { Type = PixelShaderConstantState; }
    enum {
      Float = 0,
      Int,
      Undef,
  ...
  struct CSamplerState : public CRenderVariable {
    CSamplerState() { Type = SamplerState; }
    DWORD SamplerID;
  ...
  struct CVBState : public CRenderVariable {
    ...
    uint ColorOffset; // Fix for Radeon 7xxx series (see remark in
                      // CDriverD3D::createVertexDeclaration)
    virtual void apply(CDriverD3D *driver);
  };
  ...
  struct CMaterialState : public CRenderVariable {
    CMaterialState() {
      // Current.Power = -1.f;
      Current.Diffuse.r = Current.Diffuse.g = Current.Diffuse.b =
          Current.Diffuse.a = 0.f;
      Current.Ambient.r = Current.Ambient.g = Current.Ambient.b =
          Current.Ambient.a = 0.f;
      Current.Specular.r = Current.Specular.g = Current.Specular.b =
          Current.Specular.a = 0.f;
      Current.Emissive.r = Current.Emissive.g = Current.Emissive.b =
          Current.Emissive.a = 0.f;
      Current.Power = 0.f;
    }
  ...
  // Friends
  friend void D3DWndProc(CDriverD3D *driver, HWND hWnd, UINT message,
  ...
  // Replace all arguments of color / alpha operators in the pixel pipe with the
  // given value
  void replaceAllArgument(DWORD from, DWORD to, DWORD blendOpFrom);

  // Replace all arguments of color / alpha operators in the pixel pipe at the
  // given stage with the given value
  void replaceAllArgumentAtStage(uint stage, DWORD from, DWORD to,
                                 DWORD blendOpFrom);
  void replaceAllRGBArgumentAtStage(uint stage, DWORD from, DWORD to,
                                    DWORD blendOpFrom);
  void replaceAllAlphaArgumentAtStage(uint stage, DWORD from, DWORD to,
                                      DWORD blendOpFrom);

  // Replace a the given color / alpha op
  void replaceArgumentAtStage(D3DTEXTURESTAGESTATETYPE state, DWORD stage,
                              DWORD from, DWORD to);

  // Setup lighting & material so that it produces the given constant color
  // (useful to simulate a per stage constant using diffuse)
...
public:
  ...
  inline void setRenderState(D3DRENDERSTATETYPE renderState, DWORD value) {
    ...
    NL_D3D_CACHE_TEST(CacheTest_RenderState,
                      _renderState.Value != value || !_renderState.ValueSet)
...
public:
  ...
  inline void setTextureState(DWORD stage,
  ...
  inline void setTextureIndexMode(DWORD stage, bool texGenEnabled,
                                  DWORD value) {
...
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_TextureIndexMode,
                      _textureState.TexGen =
                          (texGenEnabled || _textureState.TexGenMode != value))
#endif // NL_D3D_USE_RENDER_STATE_CACHE
    {
      // Ref on the state
  ...
  inline void setTextureIndexUV(DWORD stage, DWORD value) {
    ...
    CTextureIndexState &_textureState = _TextureIndexStateCache[stage];
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_TextureIndexUV,
                      _textureState.TexGen || _textureState.UVChannel != value)
#endif
    {
  ...
  inline void setVertexProgram(LPDIRECT3DVERTEXSHADER9 vertexProgram,
    ...
    // Ref on the state
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_VertexProgram,
  ...
  inline void setPixelShader(LPDIRECT3DPIXELSHADER9 pixelShader) {
    ...
    // Ref on the state
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_PixelShader,
  ...
  inline void setSamplerState(DWORD sampler, D3DSAMPLERSTATETYPE samplerState,
                              DWORD value) {
  ...
  inline void setVertexBuffer(IDirect3DVertexBuffer9 *vertexBuffer, UINT offset,
                              UINT stride, bool useVertexColor, uint size,
                              CVertexBuffer::TPreferredMemory pm, DWORD usage,
                              uint colorOffset) {
    ...
    // Ref on the state
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    // NL_D3D_CACHE_TEST(CacheTest_VertexBuffer,
    // (_VertexBufferCache.VertexBuffer != vertexBuffer) ||
    // (_VertexBufferCache.Offset != offset) || (stride !=
    // _VertexBufferCache.Stride) || (colorOffset !=
    // _VertexBufferCache.ColorOffset))
    NL_D3D_CACHE_TEST(CacheTest_VertexBuffer,
                      (_VertexBufferCache.VertexBuffer != vertexBuffer) ||
                          (_VertexBufferCache.Offset != offset) ||
                          (stride != _VertexBufferCache.Stride) ||
                          (colorOffset != _VertexBufferCache.ColorOffset))
...
#endif // NL_D3D_USE_RENDER_STATE_CACHE
    {
       ...
       * Sometime, after a lock D3DLOCK_NOOVERWRITE, the D3DTSS_TEXCOORDINDEX
       *state seams to change. So, force it at every vertex buffer set.
       **/
      if (_IsGeforce) {
...
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_VertexBuffer,
                      enable != _VertexDeclCache.AliasDiffuseToSpecular)
#endif // NL_D3D_USE_RENDER_STATE_CACHE
    {
...
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_VertexBuffer,
                      enable != _VertexDeclCache.EnableVertexColor)
#endif // NL_D3D_USE_RENDER_STATE_CACHE
    {
  ...
  inline void setIndexBuffer(IDirect3DIndexBuffer9 *indexBuffer, uint offset) {
    ...
    // Ref on the state
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_IndexBuffer,
  ...
  inline void
  setVertexDecl(IDirect3DVertexDeclaration9 *vertexDecl,
                IDirect3DVertexDeclaration9 *vertexDeclAliasDiffuseToSpecular,
                IDirect3DVertexDeclaration9 *vertexDeclNoDiffuse, uint stride) {
    H_AUTO_D3D(CDriverD3D_setVertexDecl);
    nlassert(_DeviceInterface);

    // Ref on the state
#ifdef NL_D3D_USE_RENDER_STATE_CACHE
    NL_D3D_CACHE_TEST(CacheTest_VertexDecl,
                      stride != _VertexDeclCache.Stride ||
                          _VertexDeclCache.Decl != vertexDecl ||
                          _VertexDeclCache.DeclAliasDiffuseToSpecular !=
                              vertexDeclAliasDiffuseToSpecular ||
                          vertexDeclNoDiffuse != _VertexDeclCache.DeclNoDiffuse)
#endif // NL_D3D_USE_RENDER_STATE_CACHE
    {
      _VertexDeclCache.Decl = vertexDecl;
      _VertexDeclCache.DeclAliasDiffuseToSpecular =
  ...
  // Access texture states
  inline void setRenderTarget(IDirect3DSurface9 *target, ITexture *texture,
                              uint8 level, uint8 cubeFace) {
...
public:
  ...
  static inline CTextureDrvInfosD3D *getTextureD3D(ITexture &tex) {
    ...
    d3dtex = (CTextureDrvInfosD3D *)(ITextureDrvInfos *)(tex.TextureDrvShare
                                                             ->DrvTexture);
    return d3dtex;
  }

  // Get the d3dtext mirror of an existing setuped pixel program.
  static inline CPixelProgramDrvInfosD3D *
  getPixelProgramD3D(CPixelProgram &pixelProgram) {
    H_AUTO_D3D(CDriverD3D_getPixelProgramD3D);
    CPixelProgramDrvInfosD3D *d3dPixelProgram;
    d3dPixelProgram =
        (CPixelProgramDrvInfosD3D *)(IProgramDrvInfos *)(pixelProgram
                                                             .m_DrvInfo);
    return d3dPixelProgram;
  }

  // Get the d3dtext mirror of an existing setuped vertex program.
  static inline CVertexProgamDrvInfosD3D *
  getVertexProgramD3D(CVertexProgram &vertexProgram) {
    H_AUTO_D3D(CDriverD3D_getVertexProgramD3D);
    CVertexProgamDrvInfosD3D *d3dVertexProgram;
    d3dVertexProgram =
        (CVertexProgamDrvInfosD3D *)(IProgramDrvInfos *)(vertexProgram
                                                             .m_DrvInfo);
  ...
  bool isDepthFormatOk(UINT adapter, D3DDEVTYPE rasterizer,
                       D3DFORMAT DepthFormat, D3DFORMAT AdapterFormat,
                       D3DFORMAT BackBufferFormat);
  bool isTextureFormatOk(UINT adapter, D3DDEVTYPE rasterizer,
                         D3DFORMAT TextureFormat, D3DFORMAT AdapterFormat);
  bool fillPresentParameter(D3DPRESENT_PARAMETERS &parameters,
                            D3DFORMAT &adapterFormat, const GfxMode &mode,
                            const D3DDISPLAYMODE &adapterMode);

  // *** Texture helper

  D3DFORMAT getD3DDestTextureFormat(ITexture &tex);
  // Generates the texture, degades it, creates / resizes the d3d surface. Don't
  // fill the surface.
  bool generateD3DTexture(ITexture &tex, bool textureDegradation,
                          D3DFORMAT &destFormat, D3DFORMAT &srcFormat,
                          bool &cube);
  // Return the memory used by the surface described iwith the parameters
  uint32 computeTextureMemoryUsage(uint width, uint height, uint levels,
                                   D3DFORMAT destFormat, bool cube);
  // Upload a texture part
  bool uploadTextureInternal(ITexture &tex, NLMISC::CRect &rect,
                             uint8 destMipmap, uint8 srcMipmap,
                             D3DFORMAT destFormat, D3DFORMAT srcFormat);

  // *** Matrix helper

  // Update _D3DModelView and _D3DModelViewProjection matrices. Call this if the
  // model, the projection or the view matrix are modified
  void updateMatrices();
  // Update the projection matrix. Call this if the driver resolution, the
  // viewport or the frustum changes.
  ...
  bool createVertexDeclaration(uint16 vertexFormat, const uint8 *typeArray,
                               IDirect3DVertexDeclaration9 **vertexDecl,
                               uint &colorOffset, bool aliasDiffuseToSpecular,
                               bool bypassDiffuse, uint *stride = NULL);
  ...
  void activePass(uint pass) {
    ...
    if (_CurrentShader) {
      CShaderDrvInfosD3D *drvInfo = static_cast<CShaderDrvInfosD3D *>(
          (IShaderDrvInfos *)_CurrentShader->_DrvInfo);
  ...
  void beginMultiPass() {
    ...
    if (_CurrentShader) {
      ...
      if (_CurrentMaterialInfo->FXCache) {
        ...
        drvInfo->Effect->Begin(&_CurrentShaderPassCount,
  ...
  // render helpers
  bool renderPrimitives(D3DPRIMITIVETYPE primitiveType, uint numVertexPerPrim,
                        CMaterial &mat, uint firstVertex, uint32 nPrims);
  bool renderIndexedPrimitives(D3DPRIMITIVETYPE primitiveType,
                               uint numVertexPerPrim, CMaterial &mat,
                               uint32 firstIndex, uint32 nPrims,
                               uint indexOffset = 0);
  bool renderSimpleIndexedPrimitives(D3DPRIMITIVETYPE primitiveType,
                                     uint numVertexPerPrim, uint32 firstIndex,
                                     uint32 nPrims, uint indexOffset = 0);
  void convertToIndices16(uint firstIndex, uint numIndices);

  // Returns true if this material needs a constant color for the diffuse
  // component
  static bool needsConstantForDiffuse(CMaterial &mat);

  /* Returns true if this normal needs constant. If true, numConstant is the
  number of needed constants, firstConstant is the first constants needed. */
  static bool needsConstants(uint &numConstant, uint &firstConstant,
                             uint &secondConstant, CMaterial &mat);

  // For normal shader, compute part of the pipeline that are worth setting
  // For example : if alpha output if not used (no alpha test or blend), then
  // the alpha part won't be set at all
  static void computeRelevantTexEnv(CMaterial &mat,
                                    bool rgbPipe[IDRV_MAT_MAXTEXTURES],
                                    bool alphaPipe[IDRV_MAT_MAXTEXTURES]);

  // Build a pixel shader for normal shader
  IDirect3DPixelShader9 *
  buildPixelShader(const CNormalShaderDesc &normalShaderDesc,
...
private:
  ...
  NLMISC::CEventEmitterMulti
      _EventEmitter; // this can contains a win emitter and eventually a direct
                     // input emitter
...
private:
  ...
  uint _NbNeLTextureStages; // Number of texture stage for NeL (max
...
public:
  ...
  CVertexProgramConstantState
      _VertexProgramConstantCache[MaxVertexProgramConstantState];
  CPixelShaderConstantState
      _PixelShaderConstantCache[MaxPixelShaderConstantState];
...
private:
  ...
  CIndexBuffer::TFormat
      _CurrIndexBufferFormat; // updated at call to activeIndexBuffer
...
public:
  ...
  const std::vector<CTextureRef> &getCurrentShaderTextures() const {
    return _CurrentShaderTextures;
  }
...
private:
  // Optim: To not test change in Materials states if just texture has changed.
...
public:
  ...
  // this is the backup of standard lighting (cause GL states may be modified by
  // Lightmap Dynamic Lighting)
  CLight _UserLight0;
  bool _UserLightEnable[MaxLight];
  // methods to enable / disable DX light, without affecting
  // _LightMapDynamicLight*, or _UserLight0*
  void setLightInternal(uint8 num, const CLight &light);
  void enableLightInternal(uint8 num, bool enable);
  // on/off Lights for LightMap mode: only the first light is enabled in
  // lightmap mode
  ...
  static std::vector<uint16>
      _QuadIndices; // tmp : quads indices -> to allow support of quads on
                    // devices that don't have 32 bit indices
  ...
  // Test if cursor is in the client area. always true when software cursor is
  // used and window visible (displayed in software when DirectInput is used)
  ...
  bool convertBitmapToCursor(const NLMISC::CBitmap &bitmap, nlCursor &cursor,
                             uint iconWidth, uint iconHeight, uint iconDepth,
                             const NLMISC::CRGBA &col, sint hotSpotX,
                             sint hotSpotY);

  // build a cursor from src, src should have the same size that the hardware
  // cursor or a assertion is thrown
  nlCursor buildCursor(const NLMISC::CBitmap &src, NLMISC::CRGBA col, uint8 rot,
                       sint hotSpotX, sint hotSpotY);

  // reset the cursor shape to the system arrow
  void setSystemArrow();

  bool convertBitmapToIcon(const NLMISC::CBitmap &bitmap, HICON &icon,
                           uint iconWidth, uint iconHeight, uint iconDepth,
                           const NLMISC::CRGBA &col = NLMISC::CRGBA::White,
                           sint hotSpotX = 0, sint hotSpotY = 0,
                           bool cursor = false);
...
#define D3DCOLOR_NL_RGBA(rgba)                                                 \
  (NLMISC::CRGBA((uint8)((rgba >> 16) & 0xff), (uint8)((rgba >> 8) & 0xff),    \
                 (uint8)(rgba & 0xff), (uint8)((rgba >> 24) & 0xff)))
#define NL_D3DCOLORVALUE_RGBA(dest, rgba)                                      \
  dest.a = (float)rgba.A * (1.f / 255.f);                                      \
  dest.r = (float)rgba.R * (1.f / 255.f);                                      \
  dest.g = (float)rgba.G * (1.f / 255.f);                                      \
  dest.b = (float)rgba.B * (1.f / 255.f);
...
#define NL_D3DVECTOR_VECTOR(dest, vect)                                        \
  dest.x = (vect).x;                                                           \
  dest.y = (vect).y;                                                           \
  dest.z = (vect).z;
#define FTODW(f) (*((DWORD *)&(f)))
#define D3DCOLOR_FLOATS(floats, rgba)                                          \
  {                                                                            \
    floats[0] = (float)((rgba >> 16) & 0xff) * (1.f / 255.f);                  \
    floats[1] = (float)((rgba >> 8) & 0xff) * (1.f / 255.f);                   \
    floats[2] = (float)(rgba & 0xff) * (1.f / 255.f);                          \
    floats[3] = (float)((rgba >> 24) & 0xff) * (1.f / 255.f);                  \
  }
#define NL_FLOATS(floats, rgba)                                                \
  {                                                                            \
    floats[0] = (float)rgba.R * (1.f / 255.f);                                 \
    floats[1] = (float)rgba.G * (1.f / 255.f);                                 \
    floats[2] = (float)rgba.B * (1.f / 255.f);                                 \
    floats[3] = (float)rgba.A * (1.f / 255.f);                                 \
  }
...
inline void copyRGBA2BGRA(uint32 *dest, const uint32 *src, uint nbPixels) {
  ...
  while (nbPixels != 0) {
    ...
    *dest = (color & 0xff00ff00) | ((color & 0xff) << 16) |
            ((color & 0xff0000) >> 16);
    dest++;
    src++;
    nbPixels--;

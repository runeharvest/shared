...
void CDriverD3D::profileRenderedPrimitives(CPrimitiveProfile &pIn,
...
void CDriverD3D::enableUsedTextureMemorySum(bool enable) {
  if (enable)
    nlinfo("PERFORMANCE INFO: enableUsedTextureMemorySum has been set to true "
           "in CDriverD3D\n");
...
void CDriverD3D::endProfileVBHardLock(std::vector<std::string> &result) {
  ...
  //	float	volatileVBTimeLock =
  //(float)CTime::ticksToSecond(_VolatileVBLockTime)*1000 /
  //max(_NumVBHardProfileFrame,1U);
  result[_VBHardProfiles.size() + 1] =
      toString("Volatile Vertex Buffer lock time = %2.3f", _VolatileVBLockTime);
...
void CDriverD3D::appendVBHardLockProfile(NLMISC::TTicks time,
                                         CVertexBuffer *vb) {
...
void CDriverD3D::endProfileIBLock(std::vector<std::string> &result) {
  ...
  for (uint i = 0; i < _IBProfiles.size(); i++) {
    ...
    // Display in ms.
    float timeLock = (float)CTime::ticksToSecond(ibProf.AccumTime) * 1000 /
                     max(_NumIBProfileFrame, 1U);
    smprintf(tmp, tmpSize, "%16s%c: %2.3f ms", ibName,
             ibProf.Change ? '*' : ' ', timeLock);
    total += timeLock;

    result[i] = tmp;
  }
  result[_IBProfiles.size()] = toString("Total: %2.3f", total);
  float volatileIBTimeLock = (float)CTime::ticksToSecond(_VolatileIBLockTime) *
                             1000 / max(_NumIBProfileFrame, 1U);
  result[_IBProfiles.size() + 1] =
      toString("Volatile Index Buffer lock time = %2.3f", volatileIBTimeLock);
...
void CDriverD3D::profileVBHardAllocation(std::vector<std::string> &result) {
  ...
  for (it = _VertexBufferHardSet.begin(); it != _VertexBufferHardSet.end();
       it++) {
  ...
  for (it = _VertexBufferHardSet.begin(); it != _VertexBufferHardSet.end();
    ...
    if (vbHard) {
      ...
      result.push_back(toString("  %16s: %4d ko (format: %d / numVerts: %d)",
                                vbHard->VertexBufferPtr->getName().c_str(),
...
void CDriverD3D::profileIBAllocation(std::vector<std::string> &result) {
  ...
  uint totalMemUsed = 0;
  for (TIBDrvInfoPtrList::iterator it = _IBDrvInfos.begin();
       it != _IBDrvInfos.end(); ++it) {
  ...
  for (TIBDrvInfoPtrList::iterator it = _IBDrvInfos.begin();
    ...
    if (ib) {
      ...
      result.push_back(toString("  %16s: %4d ko ",
                                ib->IndexBufferPtr->getName().c_str(),
                                sizeof(uint32) * numIndex));
    }
  }
}
...
void CDriverD3D::startBench(bool wantStandardDeviation, bool quick,
                            bool reset) {
...
void CDriverD3D::endBench() { CHTimer::endBench(); }

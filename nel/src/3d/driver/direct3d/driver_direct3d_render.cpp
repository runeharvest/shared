...
// convert 32 bit indices to 16 bit indices on devices that don't have 32 bit
// indices
// ***************************************************************************
void CDriverD3D::convertToIndices16(uint firstIndex, uint numIndices) {
  ...
  CVolatileIndexBuffer *&buffer =
      _VolatileIndexBuffer16RAM[_CurrentRenderPass & 1];
  ...
  if (!ptr) {
    ...
    CVolatileIndexBuffer *&bufferOther =
        _VolatileIndexBuffer16RAM[(_CurrentRenderPass + 1) & 1];
  ...
  do {
#ifdef NL_DEBUG
    nlassertex(*currIndex < _MaxVertexIndex,
...
inline
#endif
    bool
    CDriverD3D::renderPrimitives(D3DPRIMITIVETYPE primitiveType,
                                 uint /* numVertexPerPrim */, CMaterial &mat,
                                 uint firstVertex, uint32 nPrims) {
  // Setup material
  if (!setupMaterial(mat))
    return false;
  if (nPrims == 0)
    return false;
  nlassertex(nPrims < _MaxPrimitiveCount,
             ("Number of max primitive at each calls limited in this "
              "implementation on current hradware"));
  if (_VertexBufferCache.VertexBuffer) {
    ...
    for (pass = 0; pass < _CurrentShaderPassCount; pass++) {
      // Active the pass
      activePass(pass);
      HRESULT r = _DeviceInterface->DrawPrimitive(
          primitiveType, firstVertex + _VertexBufferOffset, nPrims);
      nlassert(r == D3D_OK);
    }
    endMultiPass();
  }
...
inline
#endif
    bool
    CDriverD3D::renderIndexedPrimitives(D3DPRIMITIVETYPE primitiveType,
                                        uint numVertexPerPrim, CMaterial &mat,
  ...
  if (_MaxVertexIndex <= 0xffff &&
      _CurrIndexBufferFormat == CIndexBuffer::Indices32) {
  ...
  if (_VertexBufferCache.VertexBuffer && _IndexBufferCache.IndexBuffer) {
    ...
    for (pass = 0; pass < _CurrentShaderPassCount; pass++) {
      // Active the pass
      activePass(pass);
      // NB : indexOffset is actually a constant added to each index in the
      // current index buffer (actually may be implemented by moving vertex
      // pointer in the driver ...), whereas _IndexBufferOffset+firstIndex gives
      // an offset into the index buffer
      HRESULT r = _DeviceInterface->DrawIndexedPrimitive(
          primitiveType, _VertexBufferOffset + indexOffset, 0,
          _VertexBufferSize, firstIndex + _IndexBufferOffset, nPrims);
      nlassert(r == D3D_OK);
    }
    endMultiPass();
  }
...
inline
#endif
    bool
    CDriverD3D::renderSimpleIndexedPrimitives(D3DPRIMITIVETYPE primitiveType,
  ...
  if (_VertexBufferCache.VertexBuffer && _IndexBufferCache.IndexBuffer) {
    ...
    //  an offset into the index buffer
    HRESULT r = _DeviceInterface->DrawIndexedPrimitive(
        primitiveType, _VertexBufferOffset + indexOffset, 0, _VertexBufferSize,
...
bool CDriverD3D::renderLines(CMaterial &mat, uint32 firstIndex, uint32 nlines) {
  ...
  if (!renderIndexedPrimitives(D3DPT_LINELIST, 2, mat, firstIndex, nlines))
    return false;
  // Stats
...
bool CDriverD3D::renderTriangles(CMaterial &mat, uint32 firstIndex,
                                 uint32 ntris) {
  H_AUTO_D3D(CDriverD3D_renderTriangles)
  if (!renderIndexedPrimitives(D3DPT_TRIANGLELIST, 3, mat, firstIndex, ntris))
    return false;
  // Stats
...
bool CDriverD3D::renderSimpleTriangles(uint32 firstIndex, uint32 ntris) {
  ...
  if (!renderSimpleIndexedPrimitives(D3DPT_TRIANGLELIST, 3, firstIndex, ntris))
    return false;
  // Stats
  _PrimitiveProfileIn.NTriangles += ntris;
...
// ***************************************************************************
bool CDriverD3D::renderLinesWithIndexOffset(CMaterial &mat, uint32 firstIndex,
...
// ***************************************************************************
bool CDriverD3D::renderTrianglesWithIndexOffset(CMaterial &mat,
                                                uint32 firstIndex, uint32 ntris,
                                                uint indexOffset) {
...
// ***************************************************************************
bool CDriverD3D::renderSimpleTrianglesWithIndexOffset(uint32 firstIndex,
                                                      uint32 ntris,
                                                      uint indexOffset) {
...
// ***************************************************************************
bool CDriverD3D::renderRawPoints(CMaterial &mat, uint32 firstIndex,
                                 uint32 numPoints) {
...
// ***************************************************************************
bool CDriverD3D::renderRawLines(CMaterial &mat, uint32 firstIndex,
                                uint32 numLines) {
  H_AUTO_D3D(CDriverD3D_renderRawLines);
  if (!renderPrimitives(D3DPT_LINELIST, 2, mat, firstIndex, numLines))
    return false;
  // Stats
...
bool CDriverD3D::renderRawTriangles(CMaterial &mat, uint32 firstIndex,
                                    uint32 numTris) {
  H_AUTO_D3D(CDriverD3D_renderRawTriangles)
  if (!renderPrimitives(D3DPT_TRIANGLELIST, 3, mat, 3 * firstIndex, numTris))
    return false;
  // Stats
...
void CDriverD3D::setDebugMaterial() {
  ...
  _DeviceInterface->SetRenderState(D3DRS_TEXTUREFACTOR,
  ...
  _DeviceInterface->SetRenderState(D3DRS_DIFFUSEMATERIALSOURCE,
                                   D3DMCS_MATERIAL);
  _DeviceInterface->SetRenderState(D3DRS_SPECULARMATERIALSOURCE,
                                   D3DMCS_MATERIAL);
  _DeviceInterface->SetRenderState(D3DRS_AMBIENTMATERIALSOURCE,
                                   D3DMCS_MATERIAL);
  _DeviceInterface->SetRenderState(D3DRS_EMISSIVEMATERIALSOURCE,
                                   D3DMCS_MATERIAL);
...
// ***************************************************************************
bool CDriverD3D::renderRawQuads(CMaterial &mat, uint32 startIndex,
                                uint32 numQuads) {
  H_AUTO_D3D(CDriverD3D_renderRawQuads)
  if (numQuads == 0)
    return false;
  else if (_VertexBufferCache.VertexBuffer) {
    // Num of indexes needed
    const uint numQuadsNeeded = numQuads * 6;
    nlassert(numQuads <
             MAX_NUM_QUADS); // this limitation should suffice for now
    {
      if (_MaxVertexIndex <= 0xffff) {
        nlassert(_QuadIB);
        setIndexBuffer(_QuadIB, 0);
        _CurrIndexBufferFormat =
            CIndexBuffer::Indices16; // must set the format because we don't
                                     // call activeIndexBuffer
      } else {
        ...
        if (_QuadIndexes.getNumIndexes() < numQuadsNeeded) {
          // Resize it
          uint32 numIndexResize =
              IB_RESIZE_STRIDE *
              ((numQuadsNeeded + (IB_RESIZE_STRIDE - 1)) / IB_RESIZE_STRIDE);
      ...
      for (pass = 0; pass < _CurrentShaderPassCount; pass++) {
        ...
        activePass(pass);
        // fixVB(numQuads, 4);
        _DeviceInterface->DrawIndexedPrimitive(
            D3DPT_TRIANGLELIST, _VertexBufferOffset + startIndex, 0,
            numQuads * 4, 0, numQuads * 2);
      }
      endMultiPass();
    }
  }

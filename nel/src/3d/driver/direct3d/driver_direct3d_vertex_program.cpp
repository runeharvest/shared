...
CVertexProgamDrvInfosD3D::CVertexProgamDrvInfosD3D(IDriver *drv,
...
static const char *instrToName[] = {"mov  ", "mov  ", "mul  ", "add  ", "mad  ",
                                    "rsq  ", "dp3  ", "dp4  ", "dst  ", "lit  ",
...
static const char *outputRegisterToName[] = {"Pos", "D0",  "D1", "BFC0", "BFC1",
                                             "Fog", "Pts", "T0", "T1",   "T2",
                                             "T3",  "T4",  "T5", "T6",   "T7"};
...
void dumpOperand(const CVPOperand &op, bool destOperand, std::string &out,
  ...
  case CVPOperand::OutputRegister:
    ...
    out +=
        "o" + std::string(outputRegisterToName[op.Value.OutputRegisterValue]);
    break;
...
static const char *inputToDecl[CVPOperand::InputRegisterCount] = {
    "dcl_position v0",     "dcl_blendweight v1",
    "dcl_normal v2",       "dcl_color0 v3",
    "dcl_color1 v4",       "dcl_fog v5",
    "dcl_blendindices v6", "",
    "dcl_texcoord0 v8",    "dcl_texcoord1 v9",
    "dcl_texcoord2 v10",   "dcl_texcoord3 v11",
    "dcl_texcoord4 v12",   "dcl_texcoord5 v13",
    "dcl_texcoord6 v14",   "dcl_texcoord7 v15",
};
...
bool CDriverD3D::compileVertexProgram(NL3D::CVertexProgram *program) {
  ...
  if (program->m_DrvInfo == NULL) {
    ...
    if (!source) {
      nlwarning("Direct3D driver only supports 'nelvp' profile, vertex program "
                "cannot be used");
      return false;
    }
    ...
    *itTex = drvInfo = new CVertexProgamDrvInfosD3D(this, itTex);
     ...
     * extensions, too. (EXT_vertex_shader ..). There are some
     * incompatibilities.
     */
    CVPParser parser;
...
// Currently they hang when PaletteSkin / SkinWeight are present in the vertex
// declaration, but not used so disable them in the vertex declaration We don't
// use these component in vertex programs currently..
#ifdef NL_DEBUG
    ...
    if (D3DXAssembleShader(dest.c_str(), (UINT)dest.size(), NULL, NULL, 0,
                           &pShader, &pErrorMsgs) == D3D_OK) {
      if (_DeviceInterface->CreateVertexShader(
              (DWORD *)pShader->GetBufferPointer(),
              &(getVertexProgramD3D(*program)->Shader)) != D3D_OK)
        return false;
    } else {
...
bool CDriverD3D::activeVertexProgram(CVertexProgram *program) {
  ...
  if (program) {
    if (!CDriverD3D::compileVertexProgram(program))
      return false;

    CVertexProgamDrvInfosD3D *info =
        NLMISC::safe_cast<CVertexProgamDrvInfosD3D *>(
            (IProgramDrvInfos *)program->m_DrvInfo);
    _VertexProgramUser = program;
    setVertexProgram(info->Shader, program);

    /* D3DRS_FOGSTART and D3DRS_FOGEND must be set with [1, 0] else the fog
    doesn't work properly on VertexShader and non-VertexShader objects (random
    fog flicking) with Geforce4 TI 4200 (drivers 53.03 and 45.23). The other
    cards seam to interpret the "oFog"'s values using D3DRS_FOGSTART,
    D3DRS_FOGEND.

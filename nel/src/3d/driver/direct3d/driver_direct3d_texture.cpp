...
CTextureDrvInfosD3D::CTextureDrvInfosD3D(IDriver *drv, ItTexDrvInfoPtrMap it,
                                         CDriverD3D *drvD3D, bool renderTarget)
...
const D3DTEXTUREFILTERTYPE
    RemapMagTextureFilterTypeNeL2D3D[ITexture::MagFilterCount] = {
...
const D3DTEXTUREFILTERTYPE
    RemapMinTextureFilterTypeNeL2D3D[ITexture::MinFilterCount] = {
...
const D3DTEXTUREFILTERTYPE
    RemapMipTextureFilterTypeNeL2D3D[ITexture::MinFilterCount] = {
...
uint32 CDriverD3D::computeTextureMemoryUsage(uint width, uint height,
                                             uint levels, D3DFORMAT destFormat,
                                             bool cube) {
...
bool CDriverD3D::generateD3DTexture(ITexture &tex, bool textureDegradation,
  ...
  for (face = 0; face < faceCount; face++) {
    ...
    ITexture *texture = cube ? (static_cast<CTextureCube *>(&tex)->getTexture(
                                   (CTextureCube::TFace)face))
                             : &tex;

    // getD3DTextureFormat choose a d3d dest and a src format. src format should
    // be the tex one.
    if (face == 0) {
      srcFormatCompressed =
          RemapTextureFormatCompressedTypeNeL2D3D[texture->getPixelFormat()];
      ...
      nlassert(
          ...
          RemapTextureFormatCompressedTypeNeL2D3D[texture->getPixelFormat()]);
      nlassert(srcFormat ==
               RemapTextureFormatTypeNeL2D3D[texture->getPixelFormat()]);
    ...
    // Texture degradation
    if (_ForceTextureResizePower > 0 && texture->allowDegradation() &&
        (!srcFormatCompressed || levels > 1) && textureDegradation) {
      // New size
      width = max(1U, (UINT)(texture->getWidth(0) >> _ForceTextureResizePower));
      height =
          max(1U, (UINT)(texture->getHeight(0) >> _ForceTextureResizePower));
      levels = (levels > _ForceTextureResizePower)
      ...
      if (srcFormatCompressed) {
        if (cube && !_CubbedMipMapSupported) {
          ...
          firstMipMap = (texture->getMipMapCount() > _ForceTextureResizePower)
  ...
  // Got an old texture ?
  if (d3dtext->Texture &&
      ((d3dtext->Width != width) || (d3dtext->Height != height) ||
       (d3dtext->Levels != levels) || (d3dtext->FirstMipMap != firstMipMap) ||
       (d3dtext->DestFormat != destFormat) || (d3dtext->IsCube != cube) ||
  ...
  if (d3dtext->Texture == NULL) {
    ...
    uint32 textureMemory =
        computeTextureMemoryUsage(width, height, levels, destFormat, cube);
    ...
    if (cube) {
      createSuccess =
          _DeviceInterface->CreateCubeTexture(
              width, levels, renderTarget ? D3DUSAGE_RENDERTARGET : 0,
              destFormat, renderTarget ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
              &(d3dtext->TextureCube), NULL) == D3D_OK;
      d3dtext->Texture = d3dtext->TextureCube;
    } else {
      /*
                  // textures with mipmaps doesn't support not power of two
         sizes
                  // only DXTC formats are beginning with a 'D'
                  if (supportNonPowerOfTwoTextures() && (!isPowerOf2(width) ||
         !isPowerOf2(height)) && levels == 1)
                  {
                      // support for non-power of two sizes for textures need
         these lines _DeviceInterface->SetRenderState(D3DRS_WRAP0, 0);
                      _DeviceInterface->SetSamplerState(0, D3DSAMP_ADDRESSU,
         D3DTADDRESS_CLAMP); _DeviceInterface->SetSamplerState(0,
         D3DSAMP_ADDRESSV, D3DTADDRESS_CLAMP);
                      _DeviceInterface->SetSamplerState(0, D3DSAMP_ADDRESSW,
         D3DTADDRESS_CLAMP);
                  }
      */

      HRESULT hr = _DeviceInterface->CreateTexture(
          width, height, levels, renderTarget ? D3DUSAGE_RENDERTARGET : 0,
          destFormat, renderTarget ? D3DPOOL_DEFAULT : D3DPOOL_MANAGED,
          &(d3dtext->Texture2d), NULL);

      if (hr != D3D_OK) {
        nlwarning("CreateTexture failed with code 0x%x for texture %s in %ux%u",
                  hr, tex.getShareName().c_str(), width, height);
      }
...
inline void CDriverD3D::setupTextureWrapMode(ITexture &tex) {
  ...
  if (_AnisotropicFilter > 1 &&
      tex.getMinFilter() > ITexture::NearestMipMapLinear) {
    if (tex.isTextureCube()) {
      if (_AnisotropicMinCubeSupported)
        d3dtext->MinFilter = D3DTEXF_ANISOTROPIC;
    } else {
      if (_AnisotropicMinSupported)
        d3dtext->MinFilter = D3DTEXF_ANISOTROPIC;
    }
  }
}

bool CDriverD3D::setupTextureEx(ITexture &tex, bool bUpload, bool &bAllUploaded,
  ...
  if (tex.supportSharing()) {
    ...
    // insert or get the texture.
    {
      ...
      if (itTex == rTexDrvInfos.end()) {
        ...
        itTex->second = tex.TextureDrvShare->DrvTexture =
            new CTextureDrvInfosD3D(this, itTex, this, tex.getRenderTarget());
    ...
    if (!tex.TextureDrvShare->DrvTexture) {
      ...
      // _Driver.
      tex.TextureDrvShare->DrvTexture = new CTextureDrvInfosD3D(
          NULL, ItTexDrvInfoPtrMap(), this, tex.getRenderTarget());
  ...
  if (mustLoadAll || mustLoadPart) {
      ...
      if (generateD3DTexture(tex, true, destFormat, srcFormat, cube)) {
        if ((tex.getSize() > 0) || cube) {
          ...
          if (bUpload && !tex.getRenderTarget()) {
            ...
            UINT destLevel = 0;
            uint blockCount =
                d3dtext->Height >> (d3dtext->SrcCompressed ? 2 : 0);
            uint lineWidth = d3dtext->Width;
            const uint srcPixelSize = getPixelFormatSize(srcFormat);
            for (i = d3dtext->FirstMipMap;
                 i < uint(d3dtext->FirstMipMap) + uint(d3dtext->Levels);
                 i++, destLevel++) {
              // Size of a line
              const uint blockSize = ((lineWidth * srcPixelSize) >> 3)
                                     << (d3dtext->SrcCompressed ? 2 : 0);
              ...
              for (face = 0; face < numFaces; face++) {
                ...
                ITexture *texture =
                    d3dtext->IsCube
                        ? (static_cast<CTextureCube *>(&tex)->getTexture(
                              (CTextureCube::TFace)face))
                        : &tex;
                if (texture) {
                  // Same format ?
                  if (destFormat == srcFormat) {
                    ...
                    bool locked = false;
                    if (!d3dtext->IsCube)
                      locked = d3dtext->Texture2d->LockRect(destLevel, &rect,
                                                            NULL, 0) == D3D_OK;
                    else
                      locked = d3dtext->TextureCube->LockRect(
                                   RemapCubeFaceTypeNeL2D3D[face], destLevel,
                                   &rect, NULL, 0) == D3D_OK;
                    ...
                    if (locked) {
                      ...
                      for (block = 0; block < blockCount; block++) {
                        // Copy the block
                        const uint8 *src =
                            &(texture->getPixels(i)[block * blockSize]);
                        uint8 *dest =
                            ((uint8 *)rect.pBits) + block * rect.Pitch;
                        if (destFormat == D3DFMT_A8R8G8B8)
                          copyRGBA2BGRA((uint32 *)dest, (const uint32 *)src,
                                        blockSize >> 2);
                        else
                      ...
                      if (!d3dtext->IsCube)
                        ...
                        d3dtext->TextureCube->UnlockRect(
                            RemapCubeFaceTypeNeL2D3D[face], destLevel);
                    }
                  } else {
                    ...
                    if (!d3dtext->IsCube)
                      surfaceOk = d3dtext->Texture2d->GetSurfaceLevel(
                                      destLevel, &pDestSurface) == D3D_OK;
                    else
                      surfaceOk = d3dtext->TextureCube->GetCubeMapSurface(
                                      RemapCubeFaceTypeNeL2D3D[face], destLevel,
                                      &pDestSurface) == D3D_OK;
                    ...
                    if (surfaceOk) {
                      if (srcFormat == D3DFMT_A8R8G8B8) {
                        ...
                        for (line = 0; line < blockCount; line++) {
                          copyRGBA2BGRA((uint32 *)dest,
                                        (uint32 *)(src + line * blockSize),
                                        lineWidth);
                          dest += blockSize;
                        }
                        ...
                        D3DXLoadSurfaceFromMemory(pDestSurface, NULL, NULL,
                                                  &(_TempBuffer[0]), srcFormat,
                                                  blockSize, NULL, &srcRect,
                                                  D3DX_FILTER_NONE, 0);
                      } else {
                        RECT rect;
                        rect.top = 0;
                        ...
                        D3DXLoadSurfaceFromMemory(pDestSurface, NULL, NULL,
                                                  &(texture->getPixels(i)[0]),
                                                  srcFormat, blockSize, NULL,
              ...
              // Next level
              lineWidth = max((UINT)(lineWidth >> 1),
                              (UINT)(d3dtext->SrcCompressed ? 4 : 1));
    ...
    // system of invalidateRect()...
      ...
      D3DFORMAT srcFormat;
      bool cube;
      if (CDriverD3D::generateD3DTexture(tex, false, destFormat, srcFormat,
                                         cube)) {
        ...
        if (tex.getSize() > 0 && bUpload && !tex.getRenderTarget()) {
          ...
          for (itRect = tex._ListInvalidRect.begin();
            ...
            for (i = d3dtext->FirstMipMap;
              ...
              uploadTextureInternal(tex, rectDest, uint8(destLevel), uint8(i),
...
bool CDriverD3D::uploadTexture(ITexture &tex, CRect &rect, uint8 nNumMipMap) {
  ...
  // if the texture src is in DXTC MipMaped, and mipmapskip is enabled, skip
  // first levels the user want to upload
  ...
  return uploadTextureInternal(tex, rect, nNumMipMap - d3dtext->FirstMipMap,
                               nNumMipMap, destFormat, srcFormat);
}

// ***************************************************************************

bool CDriverD3D::uploadTextureInternal(ITexture &tex, CRect &rect,
  ...
  uint lineWidth =
      max(d3dtext->Width >> destMipmap, uint(d3dtext->SrcCompressed ? 4 : 1));
  ...
  // Size of a line
  const uint lineSize = ((lineWidth * pixelSize) >> 3)
                        << (d3dtext->SrcCompressed ? 2 : 0);
  ...
  if (destFormat == srcFormat) {
    ...
    const sint dataToCopy = (((x1Copy - x0) * pixelSize) >> 3)
                            << (d3dtext->SrcCompressed ? 2 : 0);
    if (d3dtext->Texture2d->LockRect(destMipmap, &rect, &region, 0) == D3D_OK) {
      ...
      for (line = lineStart; line < lineEnd; line++) {
        // Copy the line
        const uint8 *src =
            &(tex.getPixels(srcMipmap)[line * lineSize + offsetX]);
    ...
    // Surface has been retrieved ?
    if (d3dtext->Texture2d->GetSurfaceLevel(destMipmap, &pDestSurface) ==
        D3D_OK) {
      RECT destRect;
      destRect.top = y0;
      destRect.bottom = y1;
      ...
      if (srcFormat == D3DFMT_A8R8G8B8) {
        ...
        if (D3DXLoadSurfaceFromMemory(
                pDestSurface, NULL, &destRect, &(_TempBuffer[0]), srcFormat,
        ...
        srcRect.right = x1;
        const uint8 *src = &(tex.getPixels(srcMipmap)[0]);
        if (D3DXLoadSurfaceFromMemory(pDestSurface, NULL, &destRect, src,
                                      srcFormat, lineSize, NULL, &srcRect,
...
bool CDriverD3D::setRenderTarget(ITexture *tex, uint32 /* x */, uint32 /* y */,
  ...
  if (tex == NULL) {
    ...
    bool surfaceOk = false;
    if (!d3dtext->IsCube)
      surfaceOk = d3dtext->Texture2d->GetSurfaceLevel(mipmapLevel,
                                                      &pDestSurface) == D3D_OK;
    else
      surfaceOk = d3dtext->TextureCube->GetCubeMapSurface(
                      RemapCubeFaceTypeNeL2D3D[cubeFace], mipmapLevel,
                      &pDestSurface) == D3D_OK;
    if (surfaceOk) {
  ...
  if (!_DoubleSided) {
    if (_CullMode == CCW) {
      setRenderState(D3DRS_CULLMODE,
                     _InvertCullMode ? D3DCULL_CCW : D3DCULL_CW);
    } else {
      setRenderState(D3DRS_CULLMODE,
                     _InvertCullMode ? D3DCULL_CW : D3DCULL_CCW);
    }
  }
...
ITexture *CDriverD3D::getRenderTarget() const { return _RenderTarget.Texture; }

// ***************************************************************************

bool CDriverD3D::copyTargetToTexture(ITexture * /* tex */, uint32 /* offsetx */,
                                     uint32 /* offsety */, uint32 /* x */,

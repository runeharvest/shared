...
CD3DShaderFX::CD3DShaderFX() { _ShaderChanged = true; }
...
HRESULT CDriverD3D::SetPixelShaderConstantB(UINT StartRegister,
...
HRESULT CDriverD3D::SetPixelShaderConstantF(UINT StartRegister,
...
HRESULT CDriverD3D::SetPixelShaderConstantI(UINT StartRegister,
...
HRESULT CDriverD3D::SetSamplerState(DWORD Sampler, D3DSAMPLERSTATETYPE Type,
                                    DWORD Value) {
...
HRESULT CDriverD3D::SetTexture(DWORD Stage, LPDIRECT3DBASETEXTURE9 pTexture) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ref.D3DTexture == pTexture) {
      // Set the additionnal stage set by NeL texture (D3DSAMP_ADDRESSU,
      // D3DSAMP_ADDRESSV, D3DSAMP_MAGFILTER, D3DSAMP_MINFILTER and
      // D3DSAMP_MIPFILTER)
...
HRESULT CDriverD3D::SetTextureStageState(DWORD Stage,
...
HRESULT CDriverD3D::SetTransform(D3DTRANSFORMSTATETYPE State,
...
HRESULT CDriverD3D::SetVertexShaderConstantB(UINT StartRegister,
                                             CONST BOOL *pConstantData,
                                             UINT RegisterCount) {
...
HRESULT CDriverD3D::SetVertexShaderConstantF(UINT StartRegister,
                                             CONST FLOAT *pConstantData,
                                             UINT RegisterCount) {
...
HRESULT CDriverD3D::SetVertexShaderConstantI(UINT StartRegister,
                                             CONST INT *pConstantData,
                                             UINT RegisterCount) {
...
CShaderDrvInfosD3D::CShaderDrvInfosD3D(CDriverD3D *drv,
...
bool CDriverD3D::validateShader(CD3DShaderFX *shader) {
  ...
  CShaderDrvInfosD3D *shaderInfo =
      static_cast<CShaderDrvInfosD3D *>((IShaderDrvInfos *)shader->_DrvInfo);
  ...
  if (!shaderInfo->Validated) {
    ...
    while (hTechnique == NULL) {
      if (shaderInfo->Effect->FindNextValidTechnique(
              hCurrentTechnique, &hCurrentTechnique) != D3D_OK)
        return false;
      ...
      nlverify(shaderInfo->Effect->GetTechniqueDesc(hCurrentTechnique, &desc) ==
               D3D_OK);
      ...
      {
        D3DXTECHNIQUE_DESC desc;
        nlverify(shaderInfo->Effect->GetTechniqueDesc(hCurrentTechnique,
                                                      &desc) == D3D_OK);
        if (hTechnique)
          nlinfo("Shader \"%s\" : use technique \"%s\" with %d passes.",
                 shader->getName(), desc.Name, desc.Passes);
      }
#endif // NL_DEBUG_D3D
    }
...
bool CDriverD3D::activeShader(CD3DShaderFX *shd) {
  ...
  if (shd && shd->_ShaderChanged) {
    ...
    // Already setuped ?
    CShaderDrvInfosD3D *shaderInfo =
        static_cast<CShaderDrvInfosD3D *>((IShaderDrvInfos *)shd->_DrvInfo);
    if (!shd->_DrvInfo) {
      // insert into driver list. (so it is deleted when driver is deleted).
      ItShaderDrvInfoPtrList it = _ShaderDrvInfos.insert(
          _ShaderDrvInfos.end(), (NL3D::IShaderDrvInfos *)NULL);
    ...
    HRESULT hr = D3DXCreateEffect(_DeviceInterface, shd->getText(),
                                  (UINT)strlen(shd->getText()) + 1, NULL, NULL,
                                  0, NULL, &(shaderInfo->Effect), &pErrorMsgs);
    if (hr == D3D_OK) {
      // Get the texture handle
      uint i;
      for (i = 0; i < CShaderDrvInfosD3D::MaxShaderTexture; i++) {
        string name = "texture" + toString(i);
        shaderInfo->TextureHandle[i] =
            shaderInfo->Effect->GetParameterByName(NULL, name.c_str());
        name = "color" + toString(i);
        shaderInfo->ColorHandle[i] =
            shaderInfo->Effect->GetParameterByName(NULL, name.c_str());
        name = "factor" + toString(i);
        shaderInfo->FactorHandle[i] =
            shaderInfo->Effect->GetParameterByName(NULL, name.c_str());
        name = "scalarFloat" + toString(i);
        shaderInfo->ScalarFloatHandle[i] =
            shaderInfo->Effect->GetParameterByName(NULL, name.c_str());
      }
    } else {
...
static void setFX(CD3DShaderFX &s, const char *name, const char *prog,
                  CDriverD3D *drv) {
  H_AUTO_D3D(setFX)
...
bool CDriverD3D::setShaderTexture(uint textureHandle, ITexture *texture,
  ...
  if (texture) {
    if (cache) {
      ...
      effect->SetTexture(shaderInfo->TextureHandle[textureHandle],
                         d3dtext->Texture);
    }
  }
...
void CDriverD3D::notifyAllShaderDrvOfLostDevice() {
  for (TShaderDrvInfoPtrList::iterator it = _ShaderDrvInfos.begin();
       it != _ShaderDrvInfos.end(); ++it) {
    nlassert(*it);
...
public:
  ...
  virtual void apply(class CDriverD3D &drv) { drv.setMaterialState(Material); }
};
//
...
public:
  ...
  virtual void apply(class CDriverD3D &drv) { drv.setPixelShader(PixelShader); }
};
//
...
public:
  CStateRecordPixelShaderConstantB(DWORD startRegister, const BOOL *values,
                                   DWORD countVec4)
...
public:
  CStateRecordPixelShaderConstantF(DWORD startRegister, const FLOAT *values,
                                   DWORD countVec4)
      : StartRegister(startRegister) {
    Values.resize(countVec4 * 4);
    std::copy(values, values + countVec4 * 4, Values.begin());
...
public:
  CStateRecordPixelShaderConstantI(DWORD startRegister, const INT *values,
                                   DWORD countVec4)
      : StartRegister(startRegister) {
    Values.resize(countVec4 * 4);
    std::copy(values, values + countVec4 * 4, Values.begin());
...
public:
  CStateRecordTexture(DWORD stage, LPDIRECT3DBASETEXTURE9 texture,
                      ITexture *nelTexture)
      : Stage(stage), Texture(texture) {
    nlassert(Texture);
    H_AUTO_D3D(CDriverD3D_SetTexture)
    // if not a NeL texture, should add a reference on texture, else use refptr
    // instead
...
public:
  CStateRecordTextureStageState(DWORD stage, D3DTEXTURESTAGESTATETYPE type,
                                DWORD value)
...
public:
  CStateRecordTransform(D3DTRANSFORMSTATETYPE state, const D3DMATRIX *pMatrix)
      : State(state), Matrix(*pMatrix) {}
  virtual void apply(class CDriverD3D &drv) { drv.setMatrix(State, Matrix); }
};
//
...
public:
  CStateRecordVertexShaderConstantB(DWORD startRegister, const BOOL *values,
                                    DWORD countVec4)
      : StartRegister(startRegister) {
    Values.resize(countVec4 * 4);
    std::copy(values, values + countVec4 * 4, Values.begin());
...
public:
  CStateRecordVertexShaderConstantF(DWORD startRegister, const FLOAT *values,
                                    DWORD countVec4)
      : StartRegister(startRegister) {
    Values.resize(countVec4 * 4);
    std::copy(values, values + countVec4 * 4, Values.begin());
...
public:
  CStateRecordVertexShaderConstantI(DWORD startRegister, const INT *values,
                                    DWORD countVec4)
      : StartRegister(startRegister) {
    Values.resize(countVec4 * 4);
    std::copy(values, values + countVec4 * 4, Values.begin());
...
// state recorder
HRESULT STDMETHODCALLTYPE CFXPassRecorder::QueryInterface(REFIID riid,
                                                          LPVOID *ppvObj) {
  nlassert(Driver);
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::LightEnable(DWORD Index,
                                                       BOOL Enable) {
  nlassert(Driver);
  nlassert(Target);
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetLight(DWORD Index,
...
CFXPassRecorder::SetMaterial(CONST D3DMATERIAL9 *pMaterial) {
  nlassert(Driver);
  nlassert(Target);
...
HRESULT STDMETHODCALLTYPE
CFXPassRecorder::SetNPatchMode(FLOAT /* nSegments */) {
  nlassert(0); // not managed
  return D3D_OK;
}
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetPixelShaderConstantB(
    UINT StartRegister, CONST BOOL *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordPixelShaderConstantB(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetPixelShaderConstantF(
    UINT StartRegister, CONST FLOAT *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordPixelShaderConstantF(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetPixelShaderConstantI(
    UINT StartRegister, CONST INT *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordPixelShaderConstantI(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE
CFXPassRecorder::SetRenderState(D3DRENDERSTATETYPE State, DWORD Value) {
  nlassert(Driver);
  nlassert(Target);
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetSamplerState(
...
HRESULT STDMETHODCALLTYPE
CFXPassRecorder::SetTexture(DWORD Stage, LPDIRECT3DBASETEXTURE9 pTexture) {
  nlassert(Driver);
  nlassert(Target);
  // Look for the current texture
  uint i;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ref.D3DTexture == pTexture) {
      ...
      // D3DSAMP_ADDRESSV, D3DSAMP_MAGFILTER, D3DSAMP_MINFILTER and
      // D3DSAMP_MIPFILTER)
      Target->States.push_back(
          new CStateRecordTexture(Stage, pTexture, ref.NeLTexture));
      break;
    }
  }
  if (i == count) {
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetTextureStageState(
    DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(
      new CStateRecordTextureStageState(Stage, Type, Value));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetTransform(
...
HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetVertexShaderConstantB(
    UINT StartRegister, CONST BOOL *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordVertexShaderConstantB(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetVertexShaderConstantF(
    UINT StartRegister, CONST FLOAT *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordVertexShaderConstantF(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}

HRESULT STDMETHODCALLTYPE CFXPassRecorder::SetVertexShaderConstantI(
    UINT StartRegister, CONST INT *pConstantData, UINT RegisterCount) {
  nlassert(Driver);
  nlassert(Target);
  Target->States.push_back(new CStateRecordVertexShaderConstantI(
      StartRegister, pConstantData, RegisterCount));
  return D3D_OK;
}
...
CFXPassRecord::~CFXPassRecord() {
  for (std::vector<CStateRecord *>::iterator it = States.begin();
       it != States.end(); ++it) {
    delete *it;
  }
}
...
void CFXCache::setConstants(CShaderDrvInfosD3D *si) {
  ...
  for (uint k = 0; k < CFXInputParams::MaxNumParams; ++k) {
    if (Params.Textures[k].Set)
      si->Effect->SetTexture(si->TextureHandle[k], Params.Textures[k].Value);
    if (Params.Vectors[k].Set)
      si->Effect->SetFloatArray(si->FactorHandle[k],
                                (CONST FLOAT *)&(Params.Vectors[k].Value), 4);
    if (Params.Colors[k].Set)
      si->Effect->SetInt(si->ColorHandle[k], Params.Colors[k].Value);
    if (Params.Floats[k].Set)
      si->Effect->SetFloat(si->ScalarFloatHandle[k], Params.Floats[k].Value);
  }
}

void CFXCache::begin(CShaderDrvInfosD3D *si, CDriverD3D *driver) {
  nlassert(driver);
  // amortize cost of caching for animated material -> ensure that the
  // parameters don't change for a few number of display before doing caching
  if (Params.Touched) {
    Steadyness = 0;
    Passes.clear();
    NumPasses = 0;
  ...
  if (Passes.empty()) // must record shader ?
  {
    ...
    setConstants(si);
    //
    HRESULT r = si->Effect->Begin(&numPasses, D3DXFX_DONOTSAVESTATE |
...
void CFXCache::applyPass(class CDriverD3D &drv, CShaderDrvInfosD3D *si,
                         uint passIndex) {

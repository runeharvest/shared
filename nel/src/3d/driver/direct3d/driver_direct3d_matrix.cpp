...
void CDriverD3D::updateMatrices() {
  ...
  D3DXMatrixMultiply(&_D3DModelView,
                     &(_MatrixCache[remapMatrixIndex(D3DTS_WORLD)].Matrix),
                     &(_MatrixCache[remapMatrixIndex(D3DTS_VIEW)].Matrix));

  // Update view model projection matrix
  _D3DModelViewProjection = _D3DModelView;
  D3DXMatrixMultiply(
      &_D3DModelViewProjection, &_D3DModelViewProjection,
  ...
  setRenderState(D3DRS_NORMALIZENORMALS,
                 (_UserViewMtx.hasScalePart() || _UserModelMtx.hasScalePart() ||
                  _ForceNormalize)
                     ? TRUE
                     : FALSE);
}
...
void CDriverD3D::updateProjectionMatrix() {
  ...
  // In D3D, the center of the first screen pixel is [0.0,0.0]. Is NeL it is
  // [0.5,0.5]
  const float addW =
      (right - left) / (2 * (_Viewport.getWidth() * (float)clientWidth));
  const float addH =
      (bottom - top) / (2 * (_Viewport.getHeight() * (float)clientHeight));
  ...
  if (_FrustumPerspective) {
    D3DXMatrixPerspectiveOffCenterLH(&projection, left, right, bottom, top,
                                     _FrustumZNear, _FrustumZFar);
  } else {
    D3DXMatrixOrthoOffCenterLH(&projection, left, right, bottom, top,
                               _FrustumZNear, _FrustumZFar);
  }
...
void CDriverD3D::setFrustum(float left, float right, float bottom, float top,
...
void CDriverD3D::setupViewMatrixEx(const CMatrix &mtx,
...
void CDriverD3D::setupScissor(const class CScissor &scissor) {
  ...
  if (!_ScissorTouched && _Scissor.X == scissor.X && _Scissor.Y == scissor.Y &&
...
void CDriverD3D::setDepthRange(float znear, float zfar) {
  ...
  NL_D3D_CACHE_TEST(CacheTest_DepthRange,
                    znear != _DepthRangeNear || zfar != _DepthRangeFar)
#endif
  {

...
#define NL3D_CLOD_VERTEX_FORMAT                                                \
...
#define NL3D_CLOD_TEXT_NUM_IDS                                                 \
  NL3D_CLOD_TEXT_NLOD_WIDTH *NL3D_CLOD_TEXT_NLOD_HEIGHT
#define NL3D_CLOD_BIGTEXT_WIDTH NL3D_CLOD_TEXT_NLOD_WIDTH *NL3D_CLOD_TEXT_WIDTH
#define NL3D_CLOD_BIGTEXT_HEIGHT                                               \
  NL3D_CLOD_TEXT_NLOD_HEIGHT *NL3D_CLOD_TEXT_HEIGHT
...
CLodCharacterManager::CLodCharacterManager() {
  ...
  // The texture always reside in memory... This take 1Mo of RAM. (16*32*16*32 *
  // 4) NB: this is simplier like that, and this is not a problem, since only 1
  // or 2 Mo are allocated :o)
  _BigTexture->setReleasable(false);
  // create the bitmap.
  _BigTexture->resize(NL3D_CLOD_BIGTEXT_WIDTH, NL3D_CLOD_BIGTEXT_HEIGHT,
                      CBitmap::RGBA);
...
// ***************************************************************************
CLodCharacterManager::~CLodCharacterManager() { reset(); }
...
// ***************************************************************************
const CLodCharacterShapeBank *
CLodCharacterManager::getShapeBank(uint32 bankId) const {
      ...
      for (uint j = 0; j < _ShapeBankArray[i]->getNumShapes(); j++) {
        ...
        if (shape) {
          ...
          if (it == _ShapeMap.end())
            ...
            error = true;
            nlwarning("Found a Character Lod with same name in the manager: %s",
                      name.c_str());
          }
        }
      }
...
// ***************************************************************************
void CLodCharacterManager::beginRender(IDriver *driver,
  ...
  if (!mustChangeVertexStream) {
    ...
    mustChangeVertexStream =
        mustChangeVertexStream || _NumVBHard != _VertexStream.getNumVB();
  }
  // re-init?
  if (mustChangeVertexStream) {
    // chech offset
    ...
    // NB: addRenderCharacterKey() loop hardCoded for Vertex+UV+Normal+Color
    // only.
    ...
    _VertexStream.release();
    _VertexStream.init(
        driver, NL3D_CLOD_VERTEX_FORMAT, _MaxNumVertices, _NumVBHard,
        "CLodManagerVB",
        false); // nb : don't use volatile lock as we keep the buffer locked
  }
  ...
  // Alloc a minimum of primitives (2*vertices), to avoid as possible
  // reallocation in addRenderCharacterKey
...
// ***************************************************************************
static inline void computeLodLighting(CRGBA &lightRes,
                                      const CVector &lightObjectSpace,
                                      const CVector &normalPtr, CRGBA ambient,
                                      CRGBA diffuse) {
...
// ***************************************************************************
bool CLodCharacterManager::addRenderCharacterKey(
    CLodCharacterInstance &instance, const CMatrix &worldMatrix,
  ...
  // regroup all variables that will be accessed in the ASM loop (minimize cache
  // problems)
  uint numVertices;
  ...
  // For ASM / MMX, must set 0 to alpha part, because replaced by *alphaPtr
  // (with add)
  ambient.A = 0;
  diffuse.A = 0;
  ...
  vertPtr = clod->getAnimKey(instance.AnimId, instance.AnimTime,
                             instance.WrapMode, unPackScaleFactor);
  // if not found quit, return true
  if (!vertPtr)
    return true;
  // get num verts
  ...
  // if error, take 255 as alpha.
    ...
    // NB: still use an array. This case should never arise, but support it not
    // at full optim.
  ...
  // Do this after code above because we are sure that we will fill something
  // (numVertices>0)
  if (!_LockDone) {
  ...
  // Multiply light dir with transpose of worldMatrix. This may be not exact
  // (not uniform scale) but sufficient.
  ...
  a00 *= unPackScaleFactor.x;
  a01 *= unPackScaleFactor.y;
  a02 *= unPackScaleFactor.z;
  a10 *= unPackScaleFactor.x;
  a11 *= unPackScaleFactor.y;
  a12 *= unPackScaleFactor.z;
  a20 *= unPackScaleFactor.x;
  a21 *= unPackScaleFactor.y;
  a22 *= unPackScaleFactor.z;

  // get dst Array.
  uint8 *dstPtr;
  dstPtr = _VertexData + _CurrentVertexId * _VertexSize;

  /* PreCaching Note: CFastMem::precache() has been tested (done on the 4
     arrays) but not very interesting, maybe because the cache miss improve
     //ism a bit below.
  */
    ...
    if (numVertices) {
      ...
      /* NB: order is important for AGP filling optimisation in dstPtr
          ...
          - "uop" comment formating:
              A/B means "A micro-ops in port 0, and B micro-ops in port 2".
         (port 1 is very rare for FPU) A/B/C/D means "A micro-ops in port 0, B
         in port 2, C in port 3 and D in port 4". The number in () is the delay
         (if any).
          - the "compute lighting part" must done first, because of the "fistp
         f8" mem writes that must be place far away from the "mov eax, f8" read
         in clamp lighting part (else seems that it crashes all the //ism)
          - No need to Interleave on Pentium2+. But prevents "write/read stall"
         by putting the write far away from the next read. Else stall of 3
         cycles + BIG BREAK OF //ism (I think). This had save me 120 cycles /
         240 !!!

          BenchResults:
          - The "transform vertex part" and "all next part" cost 42 cycles, but
         is somewhat optimal: 63 uop (=> min 21 cycles), but 36 uop in the P0
         port (=> this is the bottleneck)
          - The lighting part adds 1 cycle only ?????  (44 cycles) But still
         relevant and optimal: 43 uop in port P0!!!!
          - The UV part adds 4 cycles (47) (should not since 0 in Port P0),
         still acceptable.
          - The clamp part adds 3 cycles (50), and add 11 cycles in "P0 or P1"
         (but heavy dependency) If we assume all goes into P1, it should takes
         0... still acceptable (optimal==43?)
          ...
          BENCH: ASM version: 91 cycles (Write in AGP, some cache miss problems,
         still good against 67) C version: 316 cycles.
      */
      __asm
      {
				mov		edi, dstPtr
       ...
       // flush stack
           ...
           // **** Modulate lighting modulate with diffuse color, add ambient
           // term, using MMX
           ...
           // NB: emms is not so bad on P2+: delay of 6, +11 (NB: far better
           // than no MMX instructions)
				emms // uop: 11/0 (6).  (?????)
...
#endif
  {
    ...
    for (; numVertices > 0;) {
      ...
      *(CUV *)(dstPtr + NL3D_CLOD_UV_OFF) = *uvPtr;
      ...
      // Compute Lighting.
      CRGBA lightRes;
      computeLodLighting(lightRes, lightObjectSpace, *normalPtr, ambient,
                         diffuse);
      ++normalPtr;
      lightRes.A = *alphaPtr;
      ++alphaPtr;
...

  {
    ...
    // Yoyo: there is an assert with getPtr(). Not sure, but maybe arise if
    // numTriIdxs==0
    if (numTriIdxs) {
      ...
      TLodCharacterIndexType *dstIdx =
          (TLodCharacterIndexType *)iba.getPtr() + _CurrentTriId;
...
// ***************************************************************************
void CLodCharacterManager::setupNormalCorrectionMatrix(
    const CMatrix &normalMatrix) {
...
// ***************************************************************************
CLodCharacterTmpBitmap::CLodCharacterTmpBitmap() { reset(); }
...
void CLodCharacterManager::initInstance(CLodCharacterInstance &instance) {
  ...
  // allocate an id. If cannot, then fill Uvs with 0 => filled with Black. (see
  // endTextureCompute() why).
  if (_FreeIds.empty()) {
...
// ***************************************************************************
CRGBA *
CLodCharacterManager::getTextureInstance(CLodCharacterInstance &instance) {
  ...
  return pix + yId * NL3D_CLOD_TEXT_HEIGHT * NL3D_CLOD_BIGTEXT_WIDTH +
         xId * NL3D_CLOD_TEXT_WIDTH;
}

// ***************************************************************************
bool CLodCharacterManager::startTextureCompute(
    CLodCharacterInstance &instance) {
  CRGBA *dst = getTextureInstance(instance);
  if (!dst)
    return false;

  // erase the texture with 0,0,0,255. Alpha is actually the min "Quality" part
  // of the CTUVQ.
...
// ***************************************************************************
void CLodCharacterManager::addTextureCompute(
    CLodCharacterInstance &instance, const CLodCharacterTexture &lodTexture) {
  CRGBA *dst = getTextureInstance(instance);
  if (!dst)
    return;
...
// ***************************************************************************
void CLodCharacterManager::endTextureCompute(CLodCharacterInstance &instance,
                                             uint numBmpToReset) {
  CRGBA *dst = getTextureInstance(instance);
  if (!dst)
    return;
  ...
  // If the id == 0 then must reset the 0,0 Pixel to black. for the "Not Enough
  // memory" case in initInstance().
  ...
  _BigTexture->touchRect(CRect(xId * NL3D_CLOD_TEXT_WIDTH,
                               yId * NL3D_CLOD_TEXT_HEIGHT,
...
// ***************************************************************************
bool CLodCharacterManager::fastIntersect(const CLodCharacterInstance &instance,

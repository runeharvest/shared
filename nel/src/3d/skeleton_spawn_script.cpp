...
void CSkeletonSpawnScript::parseCache(CScene *scene, CSkeletonModel *skeleton) {
  ...
  // **** compare the 2 set of script line to know what to add, and what to
  // remove. NB: this is an O(N2), but the number of spawned objects should be
  // small (0, 1 or 2 surely)
  ...
  for (uint i = 0; i < _Instances.size(); i++) {
    for (uint j = 0; j < newLines.size(); j++) {
      ...
      // NB: the script line contains an "instance number", so there is no risk
      // of "reuse same twice"
    ...
    if (dstToAdd[i]) {
      ...
      if (words.size() >= 3) {
        ...
        if (words[0] == "objw") {
          ...
          // inst is a number only used to generate line difference (for cache
          // comparison)
          ...
          req.Shape = words[2];
          // World Spawned Objects are sticked to the root bone (for CLod hiding
          // behavior)
          req.BoneId = 0;
          ...
          // get the bone name, but may have space in its name => words[3] is
...
#ifdef NL_DEBUG
            nlwarning("ERROR: SkeletonSpawnScript: boneId not found in: %s",
                      line.c_str());
#endif
          }
        }
...
// avoid flooding (animation)
#ifdef NL_DEBUG
          nlwarning("ERROR: SkeletonSpawnScript: error in command: %s",
                    line.c_str());
#endif
        }
      }
...
// ***************************************************************************
void CSkeletonSpawnScript::addModelCreationRequest(CSSSModelRequest &req,
                                                   CScene *scene) {
  // if the scene is currently rendering, then process the request at end of
  // CScene::render()

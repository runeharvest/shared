...
CPSTailDot::TVBMap CPSTailDot::_FadedVBMap;   // index / vertex buffers for
                                              // constant color with fading
CPSTailDot::TVBMap CPSTailDot::_ColoredVBMap; // index / vertex buffer + colors
CPSTailDot::TVBMap
    CPSTailDot::_FadedColoredVBMap; // index / vertex buffer + faded colors

//=======================================================
CPSTailDot::CPSTailDot()
    : _ColorFading(false), _GlobalColor(false), _Lighted(false),
...
void CPSTailDot::displayRibbons(uint32 nbRibbons, uint32 srcStep) {
  ...
  do {
    toProcess = std::min((uint)(nbRibbons - ribbonIndex) /* = left to do */,
                         numRibbonBatch);
    VB.setNumVertices((_UsedNbSegs + 1) * toProcess);
    {
      ...
      if (_ColorScheme) {
        _ColorScheme->makeN(this->_Owner, ribbonIndex, currVert + colorOffset,
                            vertexSize, toProcess, _UsedNbSegs + 1, srcStep);
      }
      uint k = toProcess;
      //////////////////////////////////////////////////////////////////////////////////////
      ...
      if (!_Parametric) {
        ...
        do {
          ...
          computeRibbon((uint)(fpRibbonIndex >> 16), (CVector *)currVert,
                        vertexSize);
...
CPSTailDot::CVBnPB &CPSTailDot::getVBnPB() {
  ...
  /// choose the right vb
  TVBMap &map = _ColorScheme
                    ? (_ColorFading ? _FadedColoredVBMap
                                    : _ColoredVBMap)         // per ribbon color
         ...
         // interseting, though they are only destroyed at exit
  {
    ...
    vb.setNumVertices((_UsedNbSegs + 1) *
                      numRibbonInVB); // 1 seg = 1 line + terminal vertices
    ...
    for (uint i = 0; i < numRibbonInVB; ++i) {
      for (uint k = 0; k < (_UsedNbSegs + 1); ++k) {
        ...
        if (_ColorScheme && _ColorFading) {
          vba.setTexCoord(vbIndex, 0, 0.5f - 0.5f * ((float)k / _UsedNbSegs),
                          0);

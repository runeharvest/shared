...
void CSceneUser::deletePlayListManager(UPlayListManager *playListManager) {
  ...
  _PlayListManagers.erase((CPlayListManagerUser *)playListManager,
                          "deletePlayListManager(): Bad PlayListManager ptr");
}

// ***************************************************************************

void CSceneUser::setPolygonBalancingMode(
    CSceneUser::TPolygonBalancingMode polBalMode) {
  NL3D_HAUTO_UI_SCENE;

  nlassert((uint)CScene::CountPolygonBalancing ==
           (uint)CSceneUser::CountPolygonBalancing);
  _Scene.setPolygonBalancingMode(
      (CScene::TPolygonBalancingMode)(uint)(polBalMode));
}
...
CSceneUser::TPolygonBalancingMode CSceneUser::getPolygonBalancingMode() const {
  ...
  nlassert((uint)CScene::CountPolygonBalancing ==
           (uint)CSceneUser::CountPolygonBalancing);
  return (CSceneUser::TPolygonBalancingMode)(
             uint)_Scene.getPolygonBalancingMode();
}
...
void CSceneUser::updateWaitingIG() {
  for (TWaitingIGList::iterator it = _WaitingIGs.begin();
       it != _WaitingIGs.end();) {
    bool erased = false;
...
uint32 CSceneUser::loadCLodShapeBank(const std::string &fileName) {
  ...
  // get the bank
  CLodCharacterShapeBank *bank =
      _Scene.getLodCharacterManager()->getShapeBank(bankId);
  nlassert(bank);
...
// ***************************************************************************
sint32 CSceneUser::getCLodAnimIdByName(uint32 shapeId,
  ...
  const CLodCharacterShape *shape =
      _Scene.getLodCharacterManager()->getShape(shapeId);
  if (shape)
...
// ***************************************************************************
void CSceneUser::render(bool updateWaitingInstancesFlag /*= true*/,
  ...
  // render the scene.
  {
    ...
    if (_Scene.getCam() == NULL)
      nlerror("render(): try to render with no camera linked (may have been "
              "deleted)");
    _Scene.render(true);
  }

  if (updateWaitingInstancesFlag)
    updateWaitingInstances();

  // Must restore the matrix context, so 2D/3D interface not disturbed.
  if (restoreMatrixContextAfterRender)
...
// ***************************************************************************
void CSceneUser::renderPart(TRenderPart rp, bool doHrcPass, bool doTrav,
                            bool keepTrav) {
...
// ***************************************************************************
void CSceneUser::endPartRender(bool updateWaitingInstancesFlag,
...
void CSceneUser::updateWaitingInstances() {
  // Update waiting instances
  {
    ...
    // Done after the _Scene.render because in this method the instance are
    // checked for creation
    std::map<UInstance *, CTransformShape *>::iterator it =
        _WaitingInstances.begin();
...
void CSceneUser::setCam(UCamera cam) {
  ...
  if (camera->getOwnerScene() != &_Scene)
    nlerror(
        "setCam(): try to set a current camera not created from this scene");
...
UInstanceGroup *
CSceneUser::findCameraClusterSystemFromRay(UInstanceGroup *startClusterSystem,
                                           const NLMISC::CVector &startPos,
                                           NLMISC::CVector &endPos) {
  NL3D_HAUTO_UI_SCENE;

  CInstanceGroupUser *uig =
      dynamic_cast<CInstanceGroupUser *>(startClusterSystem);
  CInstanceGroup *pIg = NULL;
  if (uig)
    pIg = &uig->_InstanceGroup;

  CInstanceGroup *resultIg =
      _Scene.findCameraClusterSystemFromRay(pIg, startPos, endPos);
  if (resultIg)
...
UCamera CSceneUser::createCamera() {
  ...
  if (model) {
    ...
    object->setFrustum(UCamera::DefLx, UCamera::DefLy, UCamera::DefLzNear,
                       UCamera::DefLzFar);
...
void CSceneUser::createInstanceAsync(const std::string &shapeName,
  ...
  //			nlerror("UScene::createInstance(): shape is not a
  //mesh");

  // The component is auto added/deleted to _Scene in ctor/dtor.
  //		return dynamic_cast<UInstance*>( _Transforms.insert(new
  //CInstanceUser(&_Scene, model)) );
}
...
void CSceneUser::createInstanceGroupAndAddToSceneAsync(
    const std::string &instanceGroup, UInstanceGroup **pIG,
    const NLMISC::CVector &pos, const NLMISC::CQuat &rot, uint selectedTexture,
    IAsyncLoadCallback *pCB) {
  NL3D_HAUTO_ASYNC_IG;

  _WaitingIGs.push_front(CWaitingIG(pIG, pos, rot, selectedTexture, pCB));
  UInstanceGroup::createInstanceGroupAsync(instanceGroup,
                                           &(_WaitingIGs.begin()->IGToLoad));
...
void CSceneUser::deleteInstanceGroup(UInstanceGroup *pIG) { delete pIG; }
...
UInstanceGroup	*CSceneUser::createInstanceGroup (const std::string
&instanceGroup)
{
...
void CSceneUser::setToGlobalInstanceGroup(UInstanceGroup *pIG) {
  ...
  pIGU->_InstanceGroup.setClusterSystemForInstances(
      _Scene.getGlobalInstanceGroup());
}
...
void CSceneUser::deleteVisualCollisionManager(UVisualCollisionManager *mgr) {
  ...
  CVisualCollisionManagerUser *vcmUser =
      dynamic_cast<CVisualCollisionManagerUser *>(mgr);
...
// ***************************************************************************
CSceneUser::CSceneUser(CDriverUser *drv, bool bSmallScene)
    : _Scene(bSmallScene) {
  nlassert(drv);
  _DriverUser = drv;
...
// ***************************************************************************
void CSceneUser::profileNextRender() { _Scene.profileNextRender(); }
// ***************************************************************************
...
// ***************************************************************************
uint CSceneUser::getFlareContext() const { return _Scene.getFlareContext(); }
...
// ***************************************************************************
void CSceneUser::setVisualCollisionManagerForShadow(
    UVisualCollisionManager *vcm) {
  if (vcm == NULL)
    _Scene.setVisualCollisionManagerForShadow(NULL);
  else {
    CVisualCollisionManagerUser *vcmUser =
        static_cast<CVisualCollisionManagerUser *>(vcm);
...
// ***************************************************************************
void CSceneUser::setLandscapePolyDrawingCallback(
    ILandscapePolyDrawingCallback *lpd) {
  _Scene.setLandscapePolyDrawingCallback(lpd);
}
// ***************************************************************************
ILandscapePolyDrawingCallback *
CSceneUser::getLandscapePolyDrawingCallback() const {
  return _Scene.getLandscapePolyDrawingCallback();
}
// ***************************************************************************
void CSceneUser::setupTransparencySorting(uint8 maxPriority /*=0*/,
                                          uint NbDistanceEntries /*=1024*/) {
  _Scene.getRenderTrav().setupTransparencySorting(maxPriority,
                                                  NbDistanceEntries);
}
...
void CSceneUser::setWaterEnvMap(UWaterEnvMap *waterEnvMap) {
  if (waterEnvMap) {
    if (((CWaterEnvMapUser *)waterEnvMap)->EnvMap.Driver != _DriverUser) {
      nlwarning("Water envmap can only be set in a scene that was created from "
                "the same driver");
      return;
    }
  }

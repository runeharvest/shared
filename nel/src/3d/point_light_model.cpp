...
void CPointLightModel::registerBasic() {
  CScene::registerModel(PointLightModelId, TransformId,
...
// ***************************************************************************
void CPointLightModel::setDeltaPosToSkeletonWhenOutOfFrustum(
...
void CPointLightModel::traverseLight() {
  ...
  // Note: any dynamic light is supposed to always move each frame, so they are
  // re-inserted in the quadGrid each frame.
  ...
  if (isHrcVisible()) {
    ...
    if (isClipVisible()) {
      ...
      if (PointLight.getType() == CPointLight::SpotLight) {
        ...
        if (_TimeFromLastClippedSpotDirection <= 0) {
          ...
          CVector interpSpotDir = actualSpotDir * (1 - t) +
                                  _LastWorldSpotDirectionWhenOutOfFrustum * t;
          // set the interpolated one.
          PointLight.setupSpotDirection(interpSpotDir);
        }
      }
    } else {
      // We are hidden because a skeleton has hide us, or created during anim
      // detail traversal (or else don't know why). NB: a point light can't be
      // in a cluster since it is not a transformShape...
      ...
      if (PointLight.getType() == CPointLight::SpotLight) {
        ...
        if (_TimeFromLastClippedSpotDirection != 1) {
          ...
          _LastWorldSpotDirectionWhenOutOfFrustum =
              PointLight.getSpotDirection();
          // reset time.
          _TimeFromLastClippedSpotDirection = 1;
        }

        // Don't need to modify PointLight spot direction since already setuped
        // (when model was visible)
      }
    }

    // now, insert this light in the quadGrid. NB: in CLightTrav::traverse(),
    // the quadGrid is cleared before here. This light will touch
    // (resetLighting()) any model it may influence.
    ...
    if (_InfluenceLightMap) {
      ...
      if (PointLight.getType() != CPointLight::SpotLight)
        vertexLight.setupPointLight(CRGBA::Black, PointLight.getDiffuse(),
                                    PointLight.getSpecular(),
                                    PointLight.getPosition(), CVector::K);
      else
        vertexLight.setupSpotLight(
            CRGBA::Black, PointLight.getDiffuse(), PointLight.getSpecular(),
            PointLight.getPosition(), PointLight.getSpotDirection(), 1,
            float(Pi) / 2);

      // setup attenuation
      vertexLight.setupAttenuation(PointLight.getAttenuationBegin(),
                                   PointLight.getAttenuationEnd());

      // setup hotspot for spotLight
      if (PointLight.getType() == CPointLight::SpotLight) {
        // hotspot == where attenuation should be 0.9
        vertexLight.setupSpotExponent(PointLight.getSpotAngleBegin() * 0.9f +
                                      PointLight.getSpotAngleEnd() * 0.1f);
      }

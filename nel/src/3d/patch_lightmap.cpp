...
// NB: if you want to change thoses values, see unpackLumelBlock, cause
// hardcoded.
static const uint NL3DDecompressLumelFactor0Case0[8] = {7, 0, 6, 5, 4, 3, 2, 1};
static const uint NL3DDecompressLumelFactor1Case0[8] = {0, 7, 1, 2, 3, 4, 5, 6};
...
inline uint8 getUnpackLumelBlock(const uint8 *src, uint pixel) {
  ...
  if (bits <= 5)
    ...
    code =
        ((src[offset] << (bits - 5)) | (src[offset + 1] >> (13 - bits))) & 0x7;

  // Case 0
  if (src[0] > src[1]) {
    // Decompress the data
    return (uint8)((NL3DDecompressLumelFactor0Case0[code] * src[0] +
                    NL3DDecompressLumelFactor1Case0[code] * src[1]) /
                   7);
  }
  // Case 1
  else {
    // Decompress the data
    if (code < 6)
      return (uint8)((NL3DDecompressLumelFactor0Case1[code] * src[0] +
                      NL3DDecompressLumelFactor1Case1[code] * src[1]) /
                     5);
    else if (code == 6)
      return 0;
    else
...
// ***************************************************************************
uint CPatch::evalLumelBlock(const uint8 *original, const uint8 *unCompressed,
...
// ***************************************************************************
void CPatch::packLumelBlock(uint8 *dest, const uint8 *source, uint8 alpha0,
                            uint8 alpha1) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (alpha0 > alpha1)
      // Case 0
      value[i] = (NL3DDecompressLumelFactor0Case0[i] * alpha0 +
                  NL3DDecompressLumelFactor1Case0[i] * alpha1) /
                 7;
    else {
      if (i < 6)
        value[i] = (NL3DDecompressLumelFactor0Case1[i] * alpha0 +
                    NL3DDecompressLumelFactor1Case1[i] * alpha1) /
                   5;
      else if (i == 6)
        value[i] = 0;
      else
...
void CPatch::getTileTileColors(uint ts, uint tt, CRGBA corners[4]) {
  for (sint i = 0; i < 4; i++) {
    CTileColor &tcol =
        TileColors[(tt + (i >> 1)) * (OrderS + 1) + (ts + (i & 1))];
    CRGBA &col = corners[i];
...
inline void bilinearColor(CRGBA corners[4], uint x, uint y, uint &R, uint &G,
                          uint &B) {
...
// ***************************************************************************
// bilinear at center of the pixels. x E [0, 3], y E [0, 3].
inline void bilinearColorAndModulate(CRGBA corners[4], uint x, uint y,
                                     CRGBA &res) {
  uint R, G, B;
...
// ***************************************************************************
// bilinear at center of the pixels. x E [0, 3], y E [0, 3].
inline void bilinearColorAndModulatex2(CRGBA corners[4], uint x, uint y,
                                       CRGBA &res) {
  uint R, G, B;
  bilinearColor(corners, x, y, R, G, B);
...
// ***************************************************************************
// bilinear at center of the pixels. x E [0, 3], y E [0, 3].
inline void bilinearColorDiv2AndAdd(CRGBA corners[4], uint x, uint y,
                                    CRGBA &res) {
  uint R, G, B;
  bilinearColor(corners, x, y, R, G, B);
...
// ***************************************************************************
void CPatch::modulateTileLightmapWithTileColors(uint ts, uint tt, CRGBA *dest,
                                                uint stride) {
...
// ***************************************************************************
void CPatch::modulateTileLightmapEdgeWithTileColors(uint ts, uint tt, uint edge,
                                                    CRGBA *dest, uint stride,
                                                    bool inverse) {
  // Get the tileColors around this tile
  CRGBA corners[4];
  getTileTileColors(ts, tt, corners);
  ...
  case 0:
    ...
    break;
  case 1:
    y = NL_LUMEL_BY_TILE - 1;
    break;
  case 2:
    x = NL_LUMEL_BY_TILE - 1;
    break;
  case 3:
    y = 0;
    break;
...
// ***************************************************************************
void CPatch::modulateTileLightmapPixelWithTileColors(uint ts, uint tt, uint s,
                                                     uint t, CRGBA *dest) {
  // Get the tileColors around this tile
  CRGBA corners[4];
  getTileTileColors(ts, tt, corners);
...
// ***************************************************************************
void CPatch::computeTileLightmapAutomatic(uint ts, uint tt, CRGBA *dest,
                                          uint stride) {
  uint x, y;
  for (y = 0; y < NL_LUMEL_BY_TILE; y++) {
    for (x = 0; x < NL_LUMEL_BY_TILE; x++) {
...
// ***************************************************************************
void CPatch::computeTileLightmapEdgeAutomatic(uint ts, uint tt, uint edge,
                                              CRGBA *dest, uint stride,
                                              bool inverse) {
  // get coordinate according to edge.
  uint x = 0, y = 0;
...
// ***************************************************************************
void CPatch::computeTileLightmapPixelAutomatic(uint ts, uint tt, uint s, uint t,
                                               CRGBA *dest) {
  float u, v;
...
// ***************************************************************************
void CPatch::getTileLumelmapPrecomputed(uint ts, uint tt, uint8 *dest,
                                        uint stride) {
  // Unpack the lumels
  uint8 buffer[NL_LUMEL_BY_TILE * NL_LUMEL_BY_TILE];
  unpackLumelBlock(buffer, &(CompressedLumels[(ts + (tt * OrderS)) *
...
// ***************************************************************************
void CPatch::getTileLumelmapPixelPrecomputed(uint ts, uint tt, uint s, uint t,
                                             uint8 &dest) const {
  // Return the lumel
  dest =
      getUnpackLumelBlock(&(CompressedLumels[(ts + (tt * OrderS)) *
                                             NL_BLOCK_LUMEL_COMPRESSED_SIZE]),
                          s + (t << 2));
}

// ***************************************************************************
void CPatch::computeTileLightmapPrecomputed(uint ts, uint tt, CRGBA *dest,
                                            uint stride) {
  // Lumel table
  ...
  for (y = 0; y < NL_LUMEL_BY_TILE; y++) {
    for (x = 0; x < NL_LUMEL_BY_TILE; x++) {
      // lumel
      dest[y * stride + x] =
          colorTable[buffer[x + (y << NL_LUMEL_BY_TILE_SHIFT)]];
    }
  }
}
...
// ***************************************************************************
void CPatch::computeTileLightmapEdgePrecomputed(uint ts, uint tt, uint edge,
                                                CRGBA *dest, uint stride,
                                                bool inverse) {
  // Lumel table
  const CRGBA *colorTable = getLandscape()->getStaticLight();
  ...
  // For each lumels
  const uint8 *src = &(
      CompressedLumels[(ts + (tt * OrderS)) * NL_BLOCK_LUMEL_COMPRESSED_SIZE]);
  uint x;
  if (inverse) {
    uint inverseStride = stride * (4 - 1);
    for (x = 0; x < 4; x++) {
      // lumel
      dest[inverseStride - x * stride] =
          colorTable[getUnpackLumelBlock(src, pixel)];
      pixel += delta;
    }
  } else {
    for (x = 0; x < 4; x++) {
...
// ***************************************************************************
void CPatch::computeTileLightmapPixelPrecomputed(uint ts, uint tt, uint s,
                                                 uint t, CRGBA *dest) {
  // Lumel table
  const CRGBA *colorTable = getLandscape()->getStaticLight();

  // Return the lumel
  *dest = colorTable[getUnpackLumelBlock(
      &(CompressedLumels[(ts + (tt * OrderS)) *
                         NL_BLOCK_LUMEL_COMPRESSED_SIZE]),
      s + (t << 2))];
}
...
// ***************************************************************************
void CPatch::computeTileLightmapEdge(uint ts, uint tt, uint edge, CRGBA *dest,
...
// ***************************************************************************
void CPatch::computeTileLightmapPixel(uint ts, uint tt, uint s, uint t,
                                      CRGBA *dest) {
  if (getLandscape()->getAutomaticLighting())
...
// ***************************************************************************
void CPatch::computeTileLightmapPixelAroundCorner(const CVector2f &stIn,
                                                  CRGBA *dest,
                                                  bool lookAround) {
  ...
  if (lookAround) {
    // try to know if we must go on a neighbor patch (maybe false with bind
    // X/1).
    if (u < 0 || u >= OrderS * NL_LUMEL_BY_TILE || v < 0 ||
        v >= OrderT * NL_LUMEL_BY_TILE)
  ...
  if (!mustLookOnNeighbor) {
    ...
    if (u < 0 || u >= OrderS * NL_LUMEL_BY_TILE || v < 0 ||
      ...
      // get this pixel.
      computeTileLightmapPixel(
          u >> NL_LUMEL_BY_TILE_SHIFT, v >> NL_LUMEL_BY_TILE_SHIFT,
          u & (NL_LUMEL_BY_TILE - 1), v & (NL_LUMEL_BY_TILE - 1), dest);
    }
  }
  // else get from the best neighbor patch.
    ...
    if (u < 0)
      edge = 0;
    else if (v >= OrderT * NL_LUMEL_BY_TILE)
      edge = 1;
    else if (u >= OrderS * NL_LUMEL_BY_TILE)
      edge = 2;
    else if (v < 0)
      edge = 3;
    ...
    if (bindInfo.Zone) {
      ...
      // retry only one time, so at next call, must find the data IN htis patch
      // (else abort).
...
void CPatch::computeNearBlockLightmap(uint uts, uint utt, CRGBA *lightText) {
  ...
  dest = lightText + NL_LUMEL_BY_TILE * NL_TILE_LIGHTMAP_SIZE +
         NL_TILE_LIGHTMAP_SIZE + 1;
  computeTileLightmap(ts, tt + 1, dest, NL_TILE_LIGHTMAP_SIZE);
  // compute tile 1,1 of the tessBlock. must decal of 1 pixel.
  dest = lightText + NL_LUMEL_BY_TILE * NL_TILE_LIGHTMAP_SIZE +
         NL_LUMEL_BY_TILE + NL_TILE_LIGHTMAP_SIZE + 1;
  ...
  for (edge = 0; edge < 4; edge++) {
    ...
    case 0:
      decalS = -1;
      dest = lightText + 0 + NL_TILE_LIGHTMAP_SIZE;
      break;
    case 1:
      decalT = 2;
      dest =
          lightText + 1 + (NL_TILE_LIGHTMAP_SIZE - 1) * NL_TILE_LIGHTMAP_SIZE;
      break;
    case 2:
      decalS = 2;
      dest = lightText + (NL_TILE_LIGHTMAP_SIZE - 1) + NL_TILE_LIGHTMAP_SIZE;
      break;
    case 3:
      decalT = -1;
      dest = lightText + 1;
    ...
    // If we are not on a border of a patch, just compute on the interior of the
    // patch.
    //==============
    if (!edgeBorder[edge]) {
      // find the result on the mirrored border of us. First tile.
      computeTileLightmapEdge(ts + decalS, tt + decalT, (edge + 2) & 3, dest,
                              stride, false);

      // find the result on the mirrored border of us. Second Tile.
      computeTileLightmapEdge(ts + decalS2, tt + decalT2, (edge + 2) & 3, dest2,
                              stride, false);
    }
    ...
    //==============
      ...
      if (getSmoothFlag(edge)) {
        ...
        if (bindInfo.Zone) {
          uvLocator.build(this, edge, bindInfo);
          // if there is not same tile order across the edge, invalidate the
          // smooth. This is rare, so don't bother.
          if (!uvLocator.sameEdgeOrder())
            bindInfo.Zone = NULL;
        }
      }

      // Fast reject: if no neighbor, or if not smoothed, or if edge order pb,
      // just copy from my interior.
      if (!bindInfo.Zone) {
        CRGBA *src = 0;
        switch (edge) {
        case 0:
          ...
          break;
        case 1:
          src = dest - NL_TILE_LIGHTMAP_SIZE;
          break;
        case 2:
          src = dest - 1;
          break;
        ...
        for (uint n = NL_LUMEL_BY_TILE * 2; n > 0;
             n--, src += stride, dest += stride)
          *dest = *src;
      }
      // else, ok, get from neighbor.
        ...
        patchId = uvLocator.selectPatch(stIn);
        uvLocator.locateUV(stIn, patchId, patchOut, stOut);
        // must find what edge on neighbor to compute, and if we must inverse
        // (swap) result. easy: the edge of the tile is the edge of the patch
        // where we are binded.
        edgeOut = bindInfo.Edge[patchId];
        // edge0 is oriented in T increasing order. edge1 is oriented in S
        // increasing order. edge2 is oriented in T decreasing order. edge3 is
        // oriented in S decreasing order. inverse is true if same sens on both
        // edges (because of mirroring, sens should be different).
        inverse = (edge >> 1) == (edgeOut >> 1);
        // compute the lightmap on the edge of the neighbor.
        patchOut->computeTileLightmapEdge((sint)floor(stOut.x),
                                          (sint)floor(stOut.y), edgeOut, dest,
                                          stride, inverse);
        ...
        inverse = (edge >> 1) == (edgeOut >> 1);
        patchOut->computeTileLightmapEdge((sint)floor(stOut.x),
                                          (sint)floor(stOut.y), edgeOut, dest2,
                                          stride, inverse);
      }
    }
  }
  ...
  for (corner = 0; corner < 4; corner++) {
    ...
    case 1:
      ...
      decalT = 2;
      dest =
          lightText + 0 + (NL_TILE_LIGHTMAP_SIZE - 1) * NL_TILE_LIGHTMAP_SIZE;
      break;
    case 2:
      decalS = 2;
      decalT = 2;
      dest = lightText + (NL_TILE_LIGHTMAP_SIZE - 1) +
             (NL_TILE_LIGHTMAP_SIZE - 1) * NL_TILE_LIGHTMAP_SIZE;
      break;
    case 3:
      decalS = 2;
      decalT = -1;
      dest = lightText + (NL_TILE_LIGHTMAP_SIZE - 1) + 0;
      break;
    };
    ...
    if (!cornerOnPatchCorner[corner] && !cornerOnPatchEdge[corner]) {
      ...
      uint subS, subT;
      if (decalS == -1)
        subS = NL_LUMEL_BY_TILE - 1;
      else
        subS = 0;
      if (decalT == -1)
        subT = NL_LUMEL_BY_TILE - 1;
      else
        subT = 0;
      ...
      // By default, fill the corner with our interior corner. Because other
      // methods may fail.
      CRGBA *src = 0;
      switch (corner) {
      case 0:
        src = dest + 1 + NL_TILE_LIGHTMAP_SIZE;
        break;
      case 1:
        src = dest + 1 - NL_TILE_LIGHTMAP_SIZE;
        break;
      ...
      *dest = *src;
      ...
      // get the coordinate of the corner, in our [0,Order] basis. get it at the
      // center of the pixel.
      CBindInfo bindInfo;
      CPatchUVLocator uvLocator;
      CVector2f stIn, stOut;
      ...
      if (decalS == -1)
        ...
        decX = 2 + semiLumelSize;
      if (decalT == -1)
        decY = -semiLumelSize;
      else
        decY = 2 + semiLumelSize;
      stIn.set(ts + decX, tt + decY);

      // if the corner is on One edge only of the patch.
      if (cornerOnPatchEdge[corner]) {
        // find the edge where to read this corner: hard edge after or before
        // this corner.
        if (edgeBorder[corner])
          edge = corner;
        else
          edge = (corner + 4 - 1) & 3;
        ...
        if (getSmoothFlag(edge)) {
          ...
          if (bindInfo.Zone) {
            ...
            // Get the Uv, in [0,Order?*NL_LUMEL_BY_TILE] basis (ie lumel
            // basis), and get from neighbor patch
            sint u, v;
            u = (sint)floor(stOut.x * NL_LUMEL_BY_TILE);
            v = (sint)floor(stOut.y * NL_LUMEL_BY_TILE);
            patchOut->computeTileLightmapPixel(
                u >> NL_LUMEL_BY_TILE_SHIFT, v >> NL_LUMEL_BY_TILE_SHIFT,
                u & (NL_LUMEL_BY_TILE - 1), v & (NL_LUMEL_BY_TILE - 1), dest);
          }
        }
        // else we must still smooth with our lumel on this patch, so get it
        // from neighbor on edge.
        else {
          // first, clamp to our patch (recenter on the previous pixel)
          if (stIn.x < 0)
            stIn.x += lumelSize;
          else if (stIn.x > OrderS)
            stIn.x -= lumelSize;
          else if (stIn.y < 0)
            stIn.y += lumelSize;
          else if (stIn.y > OrderT)
            stIn.y -= lumelSize;

          // Get the Uv, in [0,Order?*NL_LUMEL_BY_TILE] basis (ie lumel basis),
          // and get from this patch
          sint u, v;
          u = (sint)floor(stIn.x * NL_LUMEL_BY_TILE);
          v = (sint)floor(stIn.y * NL_LUMEL_BY_TILE);
          computeTileLightmapPixel(
      ...
      // else it is on a corner of the patch.
        ...
        // if the corner of the patch (same as tile corner) is smoothed, find on
        // neighbor
        if (getCornerSmoothFlag(corner)) {
          // retrieve neigbhor info. NB: use edgeId=corner, (corner X is the
          // start of the edge X)it works.
          ...
          if (bindInfo.Zone) {
            ...
            // same reasoning as in computeDisplaceCornerSmooth(), must find the
            // pixel on the neighbor of our neighbor. But the current corner may
            // be a corner on a bind X/1. All is managed by doing this way.
...
void CPatch::getTileLightMap(uint ts, uint tt, CPatchRdrPass *&rdrpass) {
  ...
  if (tessBlock.LightMapRefCount == 0) {
    // Compute the lightmap texture, with help of TileColors, with neighboring
    // info etc...
...
void CPatch::packShadowMap(const uint8 *pLumelSrc) {
  ...
  CompressedLumels.resize(numLumelBlock * numLineBlock *
                          NL_BLOCK_LUMEL_COMPRESSED_SIZE);
  ...
  for (uint lineBlock = 0; lineBlock < numLineBlock; lineBlock++) {
    ...
    for (uint lumelBlock = 0; lumelBlock < numLumelBlock; lumelBlock++) {
      ...
      // Eval error
      uint firstMethod = evalLumelBlock(originalBlock, uncompressedBlock,
                                        NL_LUMEL_BY_TILE, NL_LUMEL_BY_TILE);
      ...
      // Eval error
      uint secondMethod = evalLumelBlock(originalBlock, uncompressedBlock,
                                         NL_LUMEL_BY_TILE, NL_LUMEL_BY_TILE);

      // Second best ?
      if (secondMethod < firstMethod) {
        // Copy compressed data
        memcpy(compressedData, secondCompressedBlock,
               NL_BLOCK_LUMEL_COMPRESSED_SIZE);
      }
...
uint8 CPatch::getLumel(const CUV &uv) const {
  ...
  ts = NLMISC::OptFastFloor(uv.U * (w << 8) + 128);
  ts >>= 8;
  tt = NLMISC::OptFastFloor(uv.V * (h << 8) + 128);
  tt >>= 8;
  clamp(ts, 0, w - 1);
  clamp(tt, 0, h - 1);
  // get the lumel
  uint8 ret;
  getTileLumelmapPixelPrecomputed(
      ts >> NL_LUMEL_BY_TILE_SHIFT, tt >> NL_LUMEL_BY_TILE_SHIFT,
...
// ***************************************************************************
void CPatch::appendTileLightInfluences(
    const CUV &uv, std::vector<CPointLightInfluence> &pointLightList) const {
  /*
      WARNING !! only CPointLightNamed must be added here (used for convenience
     by CPatch::generateTileVegetable() )
  */
  ...
  if (getZone()->_PointLightArray.getPointLights().size() > 0) {
    ...
    zonePointLights = const_cast<CPointLightNamed *>(
        &(getZone()->_PointLightArray.getPointLights()[0]));
  }
  // For 4 corners.
  for (y = 0; y < 2; y++) {
    for (x = 0; x < 2; x++) {
      // get ref on TLI, and on corner.
      const CTileLightInfluence &tli =
          TileLightInfluences[(yTLI + y) * wTLI + xTLI + x];
...
CRGBA CPatch::getCurrentTLIColor(uint x, uint y) const {
  ...
  if (getZone()->_PointLightArray.getPointLights().size() > 0) {
    ...
    for (uint lid = 0; lid < CTileLightInfluence::NumLightPerCorner; lid++) {
      ...
      lightCol.modulateFromColorRGBOnly(
          lightCol, getLandscape()->getPointLightDiffuseMaterial());
...
// ***************************************************************************
void CPatch::getCurrentTileTLIColors(uint ts, uint tt,
...
// ***************************************************************************
void CPatch::addTileLightmapWithTLI(uint ts, uint tt, NLMISC::CRGBA *dest,
                                    uint stride) {
...
// ***************************************************************************
void CPatch::addTileLightmapEdgeWithTLI(uint ts, uint tt, uint edge,
...
// ***************************************************************************
void CPatch::addTileLightmapPixelWithTLI(uint ts, uint tt, uint s, uint t,
                                         NLMISC::CRGBA *dest) {
  // compute colors ar corners of the tile.
  CRGBA corners[4];
  getCurrentTileTLIColors(ts, tt, corners);
...
void CPatch::computeCurrentTLILightmapDiv2(NLMISC::CRGBA *array) const {
  ...
  // Compute corner at middle of horizontal TessBlock edges, and at middle of
  // TessBlock.
  for (y = 0; y < hTC; y++) {
    CRGBA *dst = array + y * wTC;
    for (x = 0; x < wTC - 1; x += 2) {
...
void CPatch::linkBeforeNearUL(CPatch *patchNext) {
  ...
  // first, unlink others from me. NB: works even if
...
uint CPatch::updateTessBlockLighting(uint numTb) {
  ...
  if (tessBlock.LightMapRefCount == 0)
    ...
    // Recompute the lightmap texture, with help of TileColors, with neighboring
    // info etc...
    CRGBA lightText[NL_TILE_LIGHTMAP_SIZE * NL_TILE_LIGHTMAP_SIZE];
    computeNearBlockLightmap(ts & (~1), tt & (~1), lightText);
...
void CPatch::addRefDLMContext() {
  ...
  if (_DLMContextRefCount == 0) {
    ...
    _DLMContext->generate(this, getLandscape()->getTextureDLM(),
                          getLandscape()->getPatchDLMContextList());
...
void CPatch::beginDLMLighting() {
  ...
  // Must bkup prec pointLightCount in OldPointLightCount, and reset
  // CurPointLightCount
...
void CPatch::endDLMLighting() {
  ...
  // delete reference from old pointLight influences, at prec render() pass.
  // _DLMContext may be deleted here, if no more lights use it, and if the patch
  // is not in Near.

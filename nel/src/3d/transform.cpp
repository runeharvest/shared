...
CTransform::CTransform() {
  ...
  /*
      Default are:
          ...
          IsUserLightable= 1			// default, the model may be
     lighted. IsFinalLightable= 0 IsNeedUpdateLighting= 0
  ...
  */
  ...
  // the model will be processed and so it'll may be inserted in LightingManager
  // (for example)
  _WorldDate = -1;
  _Frozen = false;
...
CTransform::~CTransform() {
  ...
  if (_FatherSkeletonModel) {
    /* If skinned, cannot detach me from skeleton here because
       detachSkeletonSon() use some virtual calls of transform: setApplySkin().
        Hence, It is the deriver job to detach himself from the skeleton.

        NB: test isSkinned(), not isSkinnable(), since isSkinned() is not
       virtual .... This means that if a Mesh isSkinnable(), but never skinned,
  ...
  // NB: not done for FrozenStaticLightSetup, because those lights don't owns
  // me.
  resetLighting();

  // Must also remove me from the lightingManager.
  // must test getOwnerScene() because of CCluster usage out of CScene (thanks
  // to mat!! :) )
  if (getOwnerScene()) {
    CLightTrav &lightTrav = getOwnerScene()->getLightTrav();
    _LightedModelIt =
        lightTrav.LightingManager.eraseStaticLightedModel(_LightedModelIt);
  }

  if (getChannelMixerOwnerShip())
    delete (CChannelMixer *)_ChannelMixer;
  ...
  setStateFlag(ForceClipRoot, false); // ensure that not 'glued' to the root so
                                      // that the following call will succeed
  hrcUnlink();
  clipUnlinkFromAll();
...
void CTransform::hide() {
  ...
  if (Visibility != CHrcTrav::Hide) {
    ...
    // If skinned, then must inform skeleton parent that it must recompute skin
    // render/animDetail lists
    if (isSkinned()) {
...
ITrack *CTransform::getDefaultTrack(uint valueId) {
  // Cyril: prefer do it here in CTransform, because of CCamera, CLight etc...
  // (which may not need a default value too!!)
...
// ***************************************************************************
void CTransform::registerToChannelMixer(CChannelMixer *chanMixer,
  ...
  setStateFlag(IsAnimDetailable,
               _ChannelMixer || getStateFlag(IsForceAnimDetail));
  ...
  // Deriver note: if necessary, call
  // BaseClass::registerToChannelMixer(chanMixer, prefix);
}
...
void CTransform::freezeHRC() {
  ...
  if (_FreezeHRCState == FreezeHRCStateDisabled) {
    ...
    /* If the transform is not frozen (ie staticaly inserted in a cluster),
        We must be sure it will be tested against QuadGridClipManager at next
       ClipTrav pass.
        => must make this object a "moving object" at next render=> dirt
       _LocalMatrixDate.
    */
    if (!_Frozen) {
      _TransformDirty = true;
  ...
  if (_FreezeHRCState != FreezeHRCStateDisabled) {
    ...
    if (_FreezeHRCState == CTransform::FreezeHRCStateEnabled) {
      ...
      if (isLightable()) {
        ...
        // NB: works if _LightedModelIt==NULL. result is that _LightedModelIt=
        // NULL.
        _LightedModelIt =
            lightTrav.LightingManager.eraseStaticLightedModel(_LightedModelIt);
      }
...
void CTransform::update() {
  ...
  if (_FreezeHRCState != CTransform::FreezeHRCStateDisabled) {
    ...
    if (_FreezeHRCState == CTransform::FreezeHRCStateRequest) {
      // Wait for next Hrc traversal to compute good _WorldMatrix for this model
      // and his sons. Also, next Hrc traversal will insert the model in the
      // LightingManager quadGrid (if lightable)
    ...
    // if the model is ready to be frozen in HRC, then do it!!
      ...
      // unLink this object from the validateList. NB: the list will still be
      // correclty parsed.
      unlinkFromUpdateList();

      // if lightable, the model is inserted in a quadgrid to update his
      // lighting only when dynamicLights touch him (since himself is static).
      if (isLightable()) {
        CLightTrav &lightTrav = getOwnerScene()->getLightTrav();
        ...
        _LightedModelIt =
            lightTrav.LightingManager.insertStaticLightedModel(this);
      }

      // Now this model won't be tested for validation nor for worldMatrix
      // update. End!!
...
void CTransform::setIsLightable(bool val) {
  ...
  setStateFlag(IsFinalLightable,
               (getStateFlag(IsLightable) && getStateFlag(IsUserLightable)));
}
// ***************************************************************************
...
// ***************************************************************************
void CTransform::setIsRenderable(bool val) { setStateFlag(IsRenderable, val); }
...
// ***************************************************************************
void CTransform::setIsSkeleton(bool val) { setStateFlag(IsSkeleton, val); }
// ***************************************************************************
void CTransform::setApplySkin(bool state) { setStateFlag(IsSkinned, state); }
...
void CTransform::updateWorld() {
  ...
  if (_HrcParent) {
    ...
    if (_HrcParent->_AncestorSkeletonModel)
      ...
      // else I have an ancestor skel model if I am sticked/binded directly to a
      // skeleton model.
  ...
  // else, default!!
    ...
    // at the root of the hierarchy, we have no parent, hence no
    // FatherSkeletonModel nor _AncestorSkeletonModel.
    _AncestorSkeletonModel = NULL;
  ...
  // Combine matrix
  if (_LocalDate > _WorldDate ||
      (_HrcParent && _HrcParent->_WorldDate > _WorldDate)) {
    // Must recompute the world matrix.  ONLY IF I AM NOT SKINNED/STICKED TO A
    // SKELETON in the hierarchy!
    if (_AncestorSkeletonModel == NULL) {
      ...
      if (!_Frozen && isTransformShape() && !getStateFlag(ForceClipRoot))
        getOwnerScene()->getHrcTrav()._MovingObjects.push_back(
            static_cast<CTransformShape *>(this));
    }
  }

  // Update dynamic lighting.
  /*
      If the model is not frozen in StaticLight, then must update lighting each
     frame. Even if the object doesn't move, a new dynamic light may enter in
     its aera. Hence we must test it in the light quadrid. StaticLight-ed
     Objects don't need it because they are inserted in a special quadgrid,
      where dynamics lights touch all StaticLight-ed object to force their
     computing

      NB: not done if _AncestorSkeletonModel!=NULL. no need because  in this
     case, result is driven by the _LightContribution of the
  ...
  */
  ...
  // If I have an ancestor Skeleton Model, I must be binded in ClipTrav to the
...
void CTransform::updateClipTravForAncestorSkeleton() {
  ...
  if (!_AncestorSkeletonModel &&
      _ClipLinkedInSonsOfAncestorSkeletonModelGroup) {
    // verify first I am really still linked to the
    // SonsOfAncestorSkeletonModelGroup. This test is important, because link
    // may have changed for any reason (portals, clipManager....).
    if (clipGetNumParents() == 1 &&
        clipGetParent(0) == getOwnerScene()->SonsOfAncestorSkeletonModelGroup) {
      // must unlink from ALL olds models.
      clipUnlinkFromAll();
...
void CTransform::traverseHrc() {
  // Recompute the matrix, according to _HrcParent matrix mode, and local
  // matrix.
  updateWorld();
...
void CTransform::setClusterSystem(CInstanceGroup *pCS) {
  if (pCS != NULL) {
    nlassert(!getStateFlag(
        ForceClipRoot)); // the transform must be linked to the root, and have
                         // not cluster system when this flag is set
  }
...
void CTransform::traverseClip() {
  // disable H_AUTO, because slowdown when lot of models (eg 1000-2000 tested in
  // forest) H_AUTO( NL3D_TransformClip );
  ...
  // if at least visible.
  if (_WorldVis) {
    // If linked to a SkeletonModel anywhere in the hierarchy, don't clip, and
    // use skeleton model clip result. This works because we are sons of a
    // special node which is not in the clip traversal, and which is traversed
    // at end of the traversal.
    if (_AncestorSkeletonModel != NULL) {
      _Visible = _AncestorSkeletonModel->isClipVisible();
      // Special test: if we are sticked to a skeletonModel, and if we are still
      // visible, maybe we don't have to
      if (_Visible && _FatherSkeletonModel) {
        // if our skeletonModel father is displayed with a Lod, maybe we are not
        // to be displayed
        if (_FatherSkeletonModel->isDisplayedAsLodCharacter()) {
          // We are visible only if we where sticked to the skeleton with
          // forceCLod==true. This is also true if we are actually a
          // skeletonModel
          if (!_ForceCLodSticked)
            // otherWise we are not visible. eg: this is the case of skins and
            // some sticked object
            _Visible = false;
        }
      }
  ...
  if (_Visible) {
    ...
    if (_AncestorSkeletonModel == NULL) {
      ...
      // don't insert if has an ancestorSkeletonModel, because in this case,
      // result is driven by the _LightContribution of the
      // _AncestorSkeletonModel.
      if (isLightable())
        scene->getLightTrav().addLightedModel(this);

      // If needed, insert the model in the animDetail list.
      // don't insert if has an ancestoreSkeletonModel, because in this case,
      // this ancestore will animDetail through the hierarchy...
      if (isAnimDetailable())
  ...
  if (!isSkinned() && _AncestorSkeletonModel) {
    ...
    if (!_FatherSkeletonModel) {
      ...
      if (!getStateFlag(SSSWO)) {
        const CMatrix &parentWM =
            _FatherSkeletonModel->Bones[_FatherBoneId].getWorldMatrix();
        // combine worldMatrix
        _WorldMatrix = parentWM * _LocalMatrix;
      }
      // Special SkeletonSpawnScript stick
      else {
        // The parent matrix must be computed from a special matrix given to the
        // skeleton model
        CMatrix parentWM;
        parentWM.setRot(CVector::I, _FatherSkeletonModel->getSSSWODir(),
                        CVector::K);
...
void CTransform::traverseAnimDetail() {
  // First, test if I must update my worldMatrix because of the ancestorSkeleton
  // scheme
  ...
  // NB: if want to add something, do it in
  // traverseAnimDetailWithoutUpdateWorldMatrix(), because CSkeletonModel
  // doesn't call CTransform::traverseAnimDetail()
}
...
// ***************************************************************************
void CTransform::freezeStaticLightSetup(
    CPointLight *pointLight[NL3D_MAX_LIGHT_CONTRIBUTION], uint numPointLights,
    uint8 sunContribution, CPointLight *frozenAmbientlight) {
  ...
  for (i = 0; i < numPointLights; i++) {
    ...
    // Compute static AttFactor Later because don't have WorlPosition of the
    // model here!!
...
void CTransform::traverseLight() {
  ...
  if (isNeedUpdateFrozenStaticLightSetup()) {
    ...
    // So we can compute AttFactor for each static light influencing this static
    // object
    ...
    for (uint i = 0; i < numPointLights; i++) {
      ...
      sint attFactor = NLMISC::OptFastFloor(
          255 * pl->computeLinearAttenuation(worldModelPos, distToModel));
  ...
  // see CTransform::clip(), here I am Lightable(), and I have no
  // _AncestorSkeletonModel So I am sure that I really need to recompute my
  // ModelLightContributions.
  CScene *scene = getOwnerScene();
  scene->getLightTrav().LightingManager.computeModelLightContributions(
      scene->getSunAmbient(), this, _LightContribution, _LogicInfo);
...
void CTransform::hrcUnlink() {
  ...
  // if ForceClipRoot flag is set, then the fx can't be linked elsewhere in the
  // hierarchy
  ...
  // I should recompute my worldMatrix (well not useful since not linked, but
  // still do it...)
  _WorldDate = -1;
}
...
void CTransform::setForceClipRoot(bool forceClipRoot) {
  if (forceClipRoot == (getStateFlag(ForceClipRoot) != 0))
    return;
  if (forceClipRoot) {

...
void CFontManager::computeString(NLMISC::CUtfStringView sv,
  ...
  size_t idx = 0;
  {
    ...
    for (NLMISC::CUtfStringView::iterator it(sv.begin()), end(sv.end());
      ...
      if (pLI != NULL) {
        if (pLI->glyph) {
          // If letter is heavily upscaled, then there is noticeable clipping on
          // edges fixing UV will make it bit better
          ...
          vba.setTexCoord(j, 0, pLI->glyph->U0 - hlfPixTexW,
                          pLI->glyph->V1 + hlfPixTexH);
          ++j;

          vba.setVertexCoord(j, x2, 0, z1);
          vba.setTexCoord(j, 0, pLI->glyph->U1 + hlfPixTexW,
                          pLI->glyph->V1 + hlfPixTexH);
          ++j;

          vba.setVertexCoord(j, x2, 0, z2);
          vba.setTexCoord(j, 0, pLI->glyph->U1 + hlfPixTexW,
                          pLI->glyph->V0 - hlfPixTexH);
          ++j;
...
// ***************************************************************************
void CFontManager::computeStringInfo(
    NLMISC::CUtfStringView sv, CFontGenerator *fontGen,
    const NLMISC::CRGBA &color, uint32 fontSize, bool embolden, bool oblique,
    IDriver *driver, CComputedString &output, bool keep800x600Ratio) {
  computeStringInfo(sv, sv.largestSize(), fontGen, color, fontSize, embolden,
                    oblique, driver, output, keep800x600Ratio);
}

// ***************************************************************************
void CFontManager::computeStringInfo(
    NLMISC::CUtfStringView sv, size_t len, CFontGenerator *fontGen,
    const NLMISC::CRGBA &color, uint32 fontSize, bool embolden, bool oblique,
    IDriver *driver, CComputedString &output, bool keep800x600Ratio) {
  output.Color = color;
  ...
  output.Length = idx;
  nlassert(output.Length ==
           std::min(len, NLMISC::CUtfStringView(output.Text).count()));
...
string CFontManager::getCacheInformation() const {
  string str;
  str = "MaxMemory: " + NLMISC::toString(_MaxMemory) +

...
/// inherit from CPSTargetLocatedBindable. Its called when one of the targets
/// has been detroyed
void CPSZone::releaseTargetRsc(CPSLocated *target) {
  NL_PS_FUNC(CPSZone_releaseTargetRsc)
  // tell the target that we were using collision infos and that we won't use
  // them anymore
...
/// this compute a new speed vector, so that the located will reach the correct
/// position at the next speed integration this create the illusion of collision
/*void CPSZone::bounce(uint32 locatedIndex, const CVector &bouncePoint, const
CVector &surfNormal, float elasticity, TAnimationTime ellapsedTime)
{
    CVector &speed = _Owner->getSpeed()[locatedIndex];
    const CVector &pos   = _Owner->getPos()[locatedIndex];
    CVector &bounceVect = elasticity  * (speed - 2.0f * (speed * surfNormal) *
surfNormal); // speed vector after collision
...
void CPSZonePlane::show() {
  ...
  for (TPSAttribVector::const_iterator it = _Owner->getPos().begin();
       it != _Owner->getPos().end(); ++it, ++k) {
...
void CPSZonePlane::computeCollisions(CPSLocated &target,
  ...
  for (planePosIt = _Owner->getPos().begin(), normalIt = _Normal.begin();
       planePosIt != planePosEnd; ++planePosIt, ++normalIt) {
    ...
    while (itPosBefore != itPosBeforeEnd) {
      ...
      if (posSide >= -epsilon && negSide <= epsilon) {
        ...
        // we translate the particle from an epsilon so that it won't get hooked
        // to the plane
        ci.NewPos = *itPosBefore + startEnd + PSCollideEpsilon * p.getNormal();
        const CVector &speed =
            target.getSpeed()[(uint32)(itPosBefore - posBefore)];
        ci.NewSpeed = _BounceFactor *
                      (speed - 2.0f * (speed * p.getNormal()) * p.getNormal());
...
void CPSZoneSphere::computeCollisions(CPSLocated &target,
                                      uint firstInstanceIndex,
                                      const NLMISC::CVector *posBefore,
                                      const NLMISC::CVector *posAfter) {
  ...
  for (spherePosIt = _Owner->getPos().begin(), radiusIt = _Radius.begin();
       spherePosIt != spherePosEnd; ++spherePosIt, ++radiusIt) {
      ...
      if (rOut > radiusIt->R2) {
        ...
        if (rIn <= radiusIt->R2) {
          ...
          const float b = 2.f * (pos * D - D * center), a = D * D,
                      c = (pos * pos) + (center * center) -
                          2.f * (pos * center) - radiusIt->R2;
          float d = b * b - 4 * a * c;
          if (d > 0.f) {
            d = sqrtf(d);
            ...
            ci.Dist = startEnd.norm();
            // we translate the particle from an epsilon so that it won't get
            // hooked to the sphere
            ci.NewPos = pos + startEnd + PSCollideEpsilon * normal;
            const CVector &speed =
                target.getSpeed()[(uint32)(itPosBefore - posBefore)];
            ci.NewSpeed =
                _BounceFactor * (speed - 2.0f * (speed * normal) * normal);
            ci.CollisionZone = this;
            CPSLocated::_Collisions[itPosBefore - posBefore].update(ci);
          }
        }
...
////////////////////////////////
void CPSZoneDisc::computeCollisions(CPSLocated &target, uint firstInstanceIndex,
                                    const NLMISC::CVector *posBefore,
                                    const NLMISC::CVector *posAfter) {
  ...
  float hitRadius2;
  // alpha is the ratio that gives the percent of endPos - startPos that hit the
  // disc
  CVector center;
  // cycle through the disc
  discPosEnd = _Owner->getPos().end();
  for (discPosIt = _Owner->getPos().begin(), radiusIt = _Radius.begin(),
      normalIt = _Normal.begin();
       discPosIt != discPosEnd; ++discPosIt, ++normalIt, ++radiusIt) {
...
void CPSZoneCylinder::performMotion(TAnimationTime ellapsedTime)
{
    ...
    TPSAttribVector::const_iterator cylinderPosIt, cylinderPosEnd, targetPosIt,
targetPosEnd; CVector dest; CPSCollisionInfo ci; CVector startEnd; uint32 k;
...
++it)
    {
        ...
        for (cylinderPosIt = _Owner->getPos().begin(); cylinderPosIt !=
cylinderPosEnd ; ++cylinderPosIt, ++dimIt, ++basisIt)
        {

            // we must setup the cylinder in the good basis

            const CMatrix &m = CPSLocated::getConversionMatrix(*it,
this->_Owner);
            ...
            // the pos projected (and scale) over the plane basis of the
cylinder, the pos minus the center CVector projectedPos, tPos;
            ...
            for (targetPosIt = (*it)->getPos().begin(), k = 0; targetPosIt !=
targetPosEnd; ++targetPosIt, ++k)
            {
                ...
                tPos = pos - center;
                projectedPos = (1 / dimIt->x) * (I * tPos) * I + (1 / dimIt->y)
* (J * tPos) * J;
                   ...
                   )
                {
                    ...
                    destProjectedPos = (1.f / dimIt->x) * (I * tPos) * I + (1.f
/ dimIt->y)  * (J * tPos) * J;
                    ...
                    {
                        // now, detect the closest hit point (the smallest
alpha, with alpha, the percent of the move vector
                        ...
                        //expressed the src and dest positions in the cylinder
basis

                        const float ox = tPos * I, oy = tPos * J, dx = (destTPos
- tPos) * I, dy = (destTPos - tPos) * J;

                        // coefficients of the equation : a * alpha ^ 2 + b *
alpha + c = 0 const float a = (dx * dx) / (dimIt->x * dimIt->x)
                        ...
                        if (alphaTop < alphaBottom && alphaTop < alphaCyl)
                        {
                            ...
                            if (alphaBottom < alphaCyl)
                            {
                                ...
                                ci.newPos = pos + startEnd - PSCollideEpsilon *
K; ci.dist = startEnd.norm(); ci.newSpeed = (-2.f * (speed * K)) * K + speed;
                                ci.collisionZone = this;
                            ...
                            else
                            {
                                ...
                                // normal at the hit point. It is the gradient
of the implicit equation x^2 / a^2 + y^2 / b^2 - R^ 2=  0
                                // so we got unormalized n =  (2 x / a ^ 2, 2 y
/ b ^ 2, 0) in the basis of the cylinder
                                // As we'll normalize it, we don't need  the 2
factor

                                float px = ox + alphaCyl * dx;
                                float py = oy + alphaCyl * dy;

                                CVector normal = px / (dimIt->x * dimIt->x) * I
+ py / (dimIt->y * dimIt->y) * J; normal.normalize();

                                ci.newPos = pos + startEnd - PSCollideEpsilon *
normal; ci.dist = startEnd.norm(); ci.newSpeed = (-2.f * (speed * normal)) *
normal + speed; ci.collisionZone = this;
...
void CPSZoneCylinder::computeCollisions(CPSLocated &target,
                                        uint firstInstanceIndex,
                                        const NLMISC::CVector *posBefore,
                                        const NLMISC::CVector *posAfter) {
  ...
  // cycle through the cylinders
  cylinderPosEnd = _Owner->getPos().end();
  for (cylinderPosIt = _Owner->getPos().begin(), basisIt = _Basis.begin(),
      dimIt = _Dim.begin();
       cylinderPosIt != cylinderPosEnd; ++cylinderPosIt, ++dimIt, ++basisIt) {
    // we must setup the cylinder in the good basis
    const CMatrix &m = CPSLocated::getConversionMatrix(&target, this->_Owner);
    // compute the new center pos
    CVector center = m * *cylinderPosIt;
    ...
    CVector J = m.mulVector(basisIt->Y);
    CVector K = m.mulVector(basisIt->X ^ basisIt->Y);
    // the pos projected (and scale) over the plane basis of the cylinder, the
    // pos minus the center
    ...
    while (itPosBefore != itPosBeforeEnd) {
      ...
      tPos = pos - center;
      projectedPos =
          (1 / dimIt->x) * (I * tPos) * I + (1 / dimIt->y) * (J * tPos) * J;
      if (!(((tPos * K) < dimIt->z) && ((tPos * K) > -dimIt->z) &&
            (projectedPos * projectedPos < 1.f))) {
        const CVector &dest = *itPosAfter;
        destTPos = dest - center;
        destProjectedPos = (1.f / dimIt->x) * (I * destTPos) * I +
                           (1.f / dimIt->y) * (J * destTPos) * J;
        // test whether the new position is inside the cylinder
        if (((destTPos * K) < dimIt->z) && ((destTPos * K) > -dimIt->z) &&
            (destProjectedPos * destProjectedPos < 1.f)) {
          // now, detect the closest hit point (the smallest alpha, with alpha,
          // the percent of the move vector to reach the hit point)
          ...
          const float ox = tPos * I, oy = tPos * J, dx = (destTPos - tPos) * I,
                      dy = (destTPos - tPos) * J;
          ...
          const float b = 2.f * ((ox * dx) / (dimIt->x * dimIt->x) +
                                 (oy * dy) / (dimIt->y * dimIt->y));
          const float c = (ox * ox) / (dimIt->x * dimIt->x) +
                          (oy * oy) / (dimIt->y * dimIt->y) - 1;
          // discriminant
          const float delta = b * b - 4.f * a * c;
          ...
          if (alphaTop < alphaBottom && alphaTop < alphaCyl) {
            ...
            // collision with the cylinder
            CVector startEnd = alphaCyl * (dest - pos);
            // normal at the hit point. It is the gradient of the implicit
            // equation x^2 / a^2 + y^2 / b^2 - R^ 2=  0 so we got unormalized n
            // =  (2 x / a ^ 2, 2 y / b ^ 2, 0) in the basis of the cylinder As
            // we'll normalize it, we don't need  the 2 factor
            float px = ox + alphaCyl * dx;
            float py = oy + alphaCyl * dy;
            CVector normal =
                px / (dimIt->x * dimIt->x) * I + py / (dimIt->y * dimIt->y) * J;
            normal.normalize();
            ci.NewPos = pos + startEnd + PSCollideEpsilon * normal;
            ci.Dist = startEnd.norm();
...
void CPSZoneRectangle::computeCollisions(CPSLocated &target,
                                         uint firstInstanceIndex,
                                         const NLMISC::CVector *posBefore,
                                         const NLMISC::CVector *posAfter) {
  ...
  for (rectanglePosIt = _Owner->getPos().begin();
       rectanglePosIt != rectanglePosEnd;
    ...
    while (itPosBefore != itPosBeforeEnd) {
      ...
      if (posSide >= -epsilon && negSide <= epsilon) {
        ...
        // tmp
        if (fabs((ci.NewPos - center) * X) < *widthIt &&
            fabs((ci.NewPos - center) * Y) <
                *heightIt) // check collision against rectangle
        {

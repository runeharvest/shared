		...
		while (itPosBefore != itPosBeforeEnd)
		{
			...
			if (rOut > radiusIt->R2)
			{
				...
				if (rIn <= radiusIt->R2)
				{
					...
					const float b = 2.f * (pos * D - D * center), a = D * D, c = (pos * pos) + (center * center) - 2.f * (pos * center) - radiusIt->R2;
					float d = b * b - 4 * a * c;
					if (d > 0.f)
					{
						d = sqrtf(d);
...
void CPSZoneDisc::show()
{
	...
	TPSAttribRadiusPair::const_iterator radiusIt = _Radius.begin();
	TPSAttribVector::const_iterator posIt = _Owner->getPos().begin(), endPosIt = _Owner->getPos().end(), normalIt = _Normal.begin();
	setupDriverModelMatrix();
	CMatrix mat;
		...
		while (itPosBefore != itPosBeforeEnd)
		{
			        ...
			        && (projectedPos * projectedPos < 1.f)))
			{
				    ...
				    && (destProjectedPos * destProjectedPos < 1.f))
				{
					...
					const float a = (dx * dx) / (dimIt->x * dimIt->x)
					    + (dy * dy) / (dimIt->y * dimIt->y);
					const float b = 2.f * ((ox * dx) / (dimIt->x * dimIt->x) + (oy * dy) / (dimIt->y * dimIt->y));
...
void CPSZoneCylinder::show()
{
	...
	TPSAttribVector::const_iterator dimIt = _Dim.begin(), posIt = _Owner->getPos().begin(), endPosIt = _Owner->getPos().end();

...
static bool getRayIntersectionT(std::vector<NLMISC::CVector> &vertices,
                                const std::vector<TIndex> &tris, float &dist2D,
  ...
  for (uint i = 0; i < numTris; i++) {
    ...
    // If some vertices are equal (cause of graphists, or cause of projection),
    // then  this triangle cannot include the ray. must do this, else it is
    // bugguy (if one degenerated triangle exist, will return true for all the
    // skin)
    if (p01.isNull() || p12.isNull() || p20.isNull())
      continue;

    /* Since the triangle is "rendered" in the ray "camera", the ray here is
       (Pos=Null,dir=K) Therefore we can do fast triangle intersection test,
       only testing the 2D intersection in the X/Y plane first
    */
    ...
    if (allInf || allSup) {
      ...
      // This code is called for a very small subset of faces, hence don't
      // bother optim.
      CPlane plane;
      plane.make(p0, p1, p2);
      // intersect with the ray. Since vertices are in ray basis, the Ray is
      // (Null, K).
      CVector hit = plane.intersect(CVector::Null, CVector::K);
      // then dist of Null to the tri is just Z.
      float distToTri = hit.z;
      // avoid problems if the plane is // to the ray. take maximum proj dist
      // with the 3 points
      ...
      // NB: it is possible than distToTri<0 (face behind or clipped by camera).
      // clamp then to 0
  ...
  if (minSkinDistZ == FLT_MAX) {
    // get the nearest distance to the ray (do the compute only if no IT found
    // at all => optim)
    const TIndex *pTri = &tris[0];
    ...
    if (computeDist2D) {
      for (uint i = 0; i < numTris; i++) {
        ...
        // If some vertices are equal (cause of graphists, or cause of
        // projection), then  this triangle cannot include the ray. must do
        // this, else it is bugguy (if one degenerated triangle exist, will
        // return true for all the skin)
        if (p01.isNull() || p12.isNull() || p20.isNull())
          continue;
        ...
        float fSeg;
        // project Null onto segment (Null-p0)*(p1-p0) => value between 0 and
        // p01sqdist (if in segment)
...
// ***************************************************************************
bool CRayMesh::getRayIntersection(std::vector<NLMISC::CVector> &vertices,
                                  const std::vector<uint32> &tris,
...
// ***************************************************************************
bool CRayMesh::fastIntersect(const NLMISC::CMatrix &worldMatrix,
                             const NLMISC::CVector &p0,
                             const NLMISC::CVector &dir, float &dist2D,
                             float &distZ, bool computeDist2D) const {
  if (empty())
    return false;
  ...
  return getRayIntersection(meshInRaySpace, Triangles, dist2D, distZ,
                            computeDist2D);
}

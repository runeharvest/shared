...
// sort triangles with 2 mm of precision. It leaves 64 m with an uint16, which
...
struct CSortTri {
  ...
  // distance.
  sint16 Dist;
  bool operator<(const CSortTri &o) const { return Dist > o.Dist; }
};
...
void CVegetableSortBlock::updateSortBlock(CVegetableManager &vegetManager) {
  ...
  while (ptrIg) {
    ...
    _NTriangles +=
        ptrIg->_RdrPass[NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT].NTriangles;
  ...
  if (_NTriangles == 0) {
    ...
    _SortedTriangleArray.setFormat(ZSortHardMode ? CIndexBuffer::Indices16
  ...
  // don't use resize because fill with default values. size() is the capacity
  // here.
  ...
  if (ZSortHardMode) {
    ...
    if (triIndices16.size() < _NIndices)
      triIndices16.resize(_NIndices);
  } else {
    if (triIndices32.size() < _NIndices)
      triIndices32.resize(_NIndices);
  }
  ...
  if (ZSortHardMode) {
    ...
    while (ptrIg) {
      CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
          ptrIg->_RdrPass[NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT];
  ...
  for (uint quadrant = 0; quadrant < NL3D_VEGETABLE_NUM_QUADRANT; quadrant++) {
    ...
    if (ZSortHardMode) {
      ...
      uint16 *pIdx =
          (uint16 *)ibaReadWrite.getPtr() + _SortedTriangleIndices[quadrant];
      ...
      uint32 *pIdx =
          (uint32 *)ibaReadWrite.getPtr() + _SortedTriangleIndices[quadrant];
      for (uint i = 0; i < _NTriangles; i++) {
        uint32 idTriIdx = triSort[i].TriIndex * 3;

...
NLMISC::CSmartPtr<CVertexProgramPerPixelLight>
    CMeshVPPerPixelLight::_VertexProgram[NumVp];
 ...
 * The position of light must be expressed in object space, and is stored in
 * c[4]
 */
...
CVertexProgramPerPixelLight::CVertexProgramPerPixelLight(uint vp) {
  ...
  // nelvp
  {
    ...
    nlassert(CMeshVPPerPixelLight::NumVp ==
             numvp); // make sure that it is in sync with header..todo : compile
                     // time assert :)

    // \todo yoyo TODO_OPTIM Manage different number of pointLights
    // NB: never call getLightVPFragmentNeLVP() with normalize, because already
    // done by PerPixel fragment before.
    std::string vpCode =
        std::string(vpName[vp]) +
        std::string("# ***************") // temp for debug
        + CRenderTrav::getLightVPFragmentNeLVP(
              m_FeaturesLighted.NumActivePointLights,
              m_FeaturesLighted.CtStartNeLVP, m_FeaturesLighted.SupportSpecular,
              m_FeaturesLighted.Normalize) +
        std::string("# ***************") // temp for debug
        + std::string(PPLightingVPCodeEnd);
#ifdef NL_DEBUG
    /** For test : parse those programs before they are used.
     * As a matter of fact some program will works with the NV_VERTEX_PROGRAM
     * extension, but won't with EXT_vertex_shader, because there are some
     * limitations (can't read a temp register that hasn't been written
     * before..)
     */
    CVPParser vpParser;
...
//=================================================================================
bool CMeshVPPerPixelLight::begin(IDriver *drv, CScene *scene,
  ...
  //
  enable(true,
         drv); // must enable the vertex program before the vb is activated
  ...
  if (strongestLightIndex == -1)
    return false; // if no strongest light, disable this vertex program
  // setup the strongest light
  ///\todo disabling of specular lighting with this shader
  const CLight &strongestLight =
      renderTrav->getDriverLight(strongestLightIndex);
  ...
  case CLight::DirectionalLight: {
    ...
    drv->setUniform3f(IDriver::VertexProgram, program->idx().StrongestLight,
                      lPos);
    _IsPointLight = false;
  } break;
...
//=================================================================================
void CMeshVPPerPixelLight::end(IDriver *drv) { enable(false, drv); }
  ...
  if (enabled != _Enabled) {
    ...
    if (enabled) {
      ...
      uint idVP = (drv->isForceNormalize() ? 4 : 0) |
                  (SpecularLighting ? 2 : 0) | (_IsPointLight ? 1 : 0);
      //
      if (drv->activeVertexProgram(
              (CVertexProgramPerPixelLight *)_VertexProgram[idVP])) {
...
//=================================================================================
bool CMeshVPPerPixelLight::setupForMaterial(const CMaterial &mat, IDriver *drv,
                                            CScene *scene) {
  bool enabled = (mat.getShader() == CMaterial::PerPixelLighting ||
                  mat.getShader() == CMaterial::PerPixelLightingNoSpec);
  bool change = (enabled != _Enabled);
  enable(enabled, drv); // enable disable the vertex program (for material that
                        // don't have the right shader)
  if (enabled) {
    CRenderTrav *renderTrav = &scene->getRenderTrav();
...
//=================================================================================
void CMeshVPPerPixelLight::setupForMaterial(const CMaterial &mat, IDriver *drv,
                                            CScene *scene, CVertexBuffer *vb) {

  if (setupForMaterial(mat, drv,
                       scene)) // a switch from v.p enabled / disabled force to
                               // reactivate the vertex buffer.
  {

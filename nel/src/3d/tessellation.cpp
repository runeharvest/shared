...
void CTileMaterial::appendTileToEachRenderPass(uint patchNumRenderableFaces) {
  for (uint i = 0; i < NL3D_MAX_TILE_PASS; i++) {
    ...
    if (rdrPass != NULL) {
      /* enlarge the capacity of the pass so it can renders the tile faces of
       *this patch. NumRenderableFaces is really too big since the tile-material
       *surely doesn't use all faces of the patch (except if same texture...)
       *	But doesn't matter. Even if all the visible Tile Surface
       *(80m*80m) is in the same pass, it leads to only 76K final in
       *CLandscapeGlobals::PassTriArray: 80*80(Visible surface at 80m max) /4
       *(2m*2m) *2(triangles) *2 (over-estimate) *3*4(triSize)= 76800
       */
...
void CTessVertex::computeGeomPos() {
  ...
  // Compute ErrorMetric modified by TileNear transition, only if TileNear
  // transition.
  if (sqrDist < CLandscapeGlobals::TileDistFarSqr) {
    // Soft optim: do it only if necessary, ie result of max(errorMetric,
    // errorMetricModified) is foreseeable here.
    if (pgeom < MaxNearLimit) {
      float f = (CLandscapeGlobals::TileDistFarSqr - sqrDist) *
...
CTessFace::CTessFace() {
  ...
  // Patch, SonLeft and SonRight nullity are very useful for MultiplePatch
  // faces, and CantMergeFace.
...
CTessFace::~CTessFace() {
  ...
  // Vertices and Uvs must be correctly cleared too (but difficult because of
  // sharing).
  /*
...
void CTessFace::computeTileErrorMetric() {
  // We must take a more correct errometric here: We must have sons face which
  // have lower projectedsize than father. This is not the case if Center of
  // face is taken (but when not in tile mode this is nearly the case). So take
  // the min dist from 3 points.
  ...
  // It is also VERY important to take the min of 3, to ensure the split in
  // TileMode when Far1 vertex begin to blend (see Patch::renderFar1() render).

  // NB: VertexProgram geomorph take sqrdist= (SplitPoint -
  // RefineCenter).sqrnorm(); It's OK because geomorph will start "far" after
  // the split.
  ...
  if (sqrdist < CLandscapeGlobals::TileDistFarSqr) {
    ...
    if (ErrorMetric < nearLimit) {
      if (sqrdist < CLandscapeGlobals::TileDistNearSqr) {
        ...
        // If threshold is big like 0.5, transition is still hard, and pops
        // occurs. But The goal is 0.005 and less, so don't bother.
      }
    }
  }
}
...
void CTessFace::updateErrorMetric() {
  ...
  //-----------------
  // Can't do it because geomorph is made on Graphic card, so the simplier is
  // the better.

  // TileMode Impact.
  //-----------------
  /* TileMode Impact. We must split at least at TileLimitLevel, but only if the
     triangle has a chance to enter in the TileDistFar sphere.
  */
  if (Level < Patch->TileLimitLevel &&
      sqrdist < sqr(CLandscapeGlobals::TileDistFar + MaxDistToSplitPoint)) {
...
// ***************************************************************************
inline float CTessFace::computeTileEMForUpdateRefine(float distSplitPoint,
                                                     float distMinFace,
                                                     float nearLimit) {
  float ema;
  ...
  if (distMinFace < CLandscapeGlobals::TileDistFar) {
    ...
    if (ema < nearLimit) {
      if (distMinFace < CLandscapeGlobals::TileDistNear) {
        ...
        // Smooth transition to the nearLimit of tesselation.
        float f = (CLandscapeGlobals::TileDistFarSqr - sqr(distMinFace)) *
                  CLandscapeGlobals::OOTileDistDeltaSqr;
        // sqr gives better result, by smoothing more the start of transition.
        f = sqr(f);
        f = sqr(f);
...
void CTessFace::computeSplitPoint() {
  if (isRectangular()) {
    // If it is a rectangular triangle, it will be splitted on the middle of
    // VBase/VLeft. see splitRectangular() conventions. So for good geomorph
    // compute per vertex, we must have this SplitPoint on this middle.
    SplitPoint = (VLeft->EndPos + VBase->EndPos) / 2;
  } else {
    // If it is a square triangle, it will be splitted on middle of
    // VLeft/VRight. So for good geomorph compute per vertex, we must have this
    // SplitPoint on this middle.
...
void CTessFace::allocTileUv(TTileUvId id) {
  ...
  for (sint i = 0; i < NL3D_MAX_TILE_FACE; i++) {
    if (TileFaces[i]) {
      ...
      // May Allocate/Fill VB. Do it after allocTileUv, because UVs are not
      // comuted yet.
    }
  }
}
...
// ***************************************************************************
void CTessFace::copyTileUv(TTileUvId dstId, CTessFace *srcFace,
                           TTileUvId srcId) {
  // TileFaces must have been build.
  nlassert(TileFaces[NL3D_TILE_PASS_RGB0]);

  // Since this a ptr-copy, no need to add/remove the renderlist of near
  // vertices.
  ...
  for (sint i = 0; i < NL3D_MAX_TILE_FACE; i++) {
    if (TileFaces[i]) {
      ...
      TileFaces[i]->V[IdUvBase]->initMiddleUv(
          *baseFace->TileFaces[i]->V[IdUvLeft],
          *baseFace->TileFaces[i]->V[IdUvRight]);
    }
  }
}
...
// ***************************************************************************
void CTessFace::initTileUvRGBA(sint pass, bool alpha, CParamCoord pointCoord,
                               CParamCoord middle, CUV &uv) {
...
// ***************************************************************************
void CTessFace::initTileUvLightmap(CParamCoord pointCoord, CParamCoord middle,
                                   CUV &uv) {
  // Get good coordinate according to patch orientation.
  uv.U = pointCoord.S <= middle.S ? 0.0f : 1.0f;
  uv.V = pointCoord.T <= middle.T ? 0.0f : 1.0f;
  ...
  CVector uvScaleBias;
  Patch->getTileLightMapUvInfo(TileMaterial->TileS, TileMaterial->TileT,
                               uvScaleBias);
...
void CTessFace::computeTileMaterial() {
  // 0. Compute TileId.
  //-------------------
  // Square Order Patch assumption: assume that when a CTessFace become a tile,
  // his base can ONLY be diagonal...
  /* a Patch:
      A ________
      |\      /|
      | \    / |
     C|__\B_/  |
      ...
      Here, if OrderS*OrderT=2*2, ABC is a new CTessFace of a Tile, and AB is
     the diagonal of the tile. Hence the middle of the tile is the middle of AB.
  ...
  */
  ...
  //--------------------------
  // if base neighbor is already at TileLimitLevel just ptr-copy, else create
  // the TileMaterial...
  nlassert(!FBase || FBase->Patch != Patch ||
           FBase->Level <= Patch->TileLimitLevel);
  bool copyFromBase;
  copyFromBase = (FBase && FBase->Patch == Patch);
  copyFromBase = copyFromBase && (FBase->Level == Patch->TileLimitLevel &&
                                  FBase->TileMaterial != NULL);
  // NB: because of delete/recreateTileUvs(), FBase->TileMaterial may be NULL,
  // even if face is at good TileLimitLevel.
  if (copyFromBase) {
    ...
    // First, get a lightmap for this tile. NB: important to do this after
    // appendTileMaterialToRenderList(), because use TessBlocks.
    Patch->getTileLightMap(
        ts, tt, TileMaterial->Pass[NL3D_TILE_PASS_LIGHTMAP].PatchRdrPass);

    // Fill pass of multi pass material.
    for (i = 0; i < NL3D_MAX_TILE_PASS; i++) {
      // Get the correct render pass, according to the tile number, and the
      // pass.
      if (i != NL3D_TILE_PASS_LIGHTMAP)
        TileMaterial->Pass[i].PatchRdrPass =
            Patch->getTileRenderPass(TileId, i);
    }
  ...
  // 2. Compute Uvs.
  //----------------
  // NB: TileMaterial is already setup. Useful for initTileUvLightmap() and
  // initTileUvRGBA().
  ...
  initTileUvLightmap(PVBase, middle,
                     TileFaces[NL3D_TILE_PASS_RGB0]->V[IdUvBase]->PUv1);
  ...
  for (sint i = 0; i < NL3D_MAX_TILE_FACE; i++) {
    ...
    if (TileMaterial->Pass[i].PatchRdrPass) {
      ...
      if (i == NL3D_TILE_PASS_RGB1 || i == NL3D_TILE_PASS_RGB2 ||
          i == NL3D_TILE_PASS_ADD)
        initTileUvRGBA(i, true, PVBase, middle,
                       TileFaces[i]->V[IdUvBase]->PUv1);
    }
  }
  ...
  if (copyFromBase) {
    ...
    initTileUvLightmap(PVLeft, middle,
                       TileFaces[NL3D_TILE_PASS_RGB0]->V[IdUvLeft]->PUv1);
    initTileUvLightmap(PVRight, middle,
                       TileFaces[NL3D_TILE_PASS_RGB0]->V[IdUvRight]->PUv1);
    // Init DLM Uv, in RGB0 pass, UV2.
    ...
    for (sint i = 0; i < NL3D_MAX_TILE_FACE; i++) {
      ...
      if (TileMaterial->Pass[i].PatchRdrPass) {
        ...
        nlassert(TileFaces[i]);
        // Compute RGB UV in UV0.
        initTileUvRGBA(i, false, PVLeft, middle,
                       TileFaces[i]->V[IdUvLeft]->PUv0);
        initTileUvRGBA(i, false, PVRight, middle,
                       TileFaces[i]->V[IdUvRight]->PUv0);
        // If transition tile, compute alpha UV in UV1.
        // Do it also for Additive, because may have Transition
        if (i == NL3D_TILE_PASS_RGB1 || i == NL3D_TILE_PASS_RGB2 ||
            i == NL3D_TILE_PASS_ADD) {
          initTileUvRGBA(i, true, PVLeft, middle,
                         TileFaces[i]->V[IdUvLeft]->PUv1);
          initTileUvRGBA(i, true, PVRight, middle,
                         TileFaces[i]->V[IdUvRight]->PUv1);
        }
      }
    }
...
void CTessFace::releaseTileMaterial() {
  ...
  if (FBase && FBase->Level == Patch->TileLimitLevel &&
      FBase->TileMaterial != NULL) {
    ...
    // Release the tile lightmap part. Do it before
    // removeTileMaterialFromRenderList().
    Patch->releaseTileLightMap(TileMaterial->TileS, TileMaterial->TileT);

    // Remove this material from the render list. DO it before deletion of
    // course :). NB: TileS/TileT still valid.
...
void CTessFace::splitRectangular(bool propagateSplit) {
  ...
  /*
     ...
     lb                    rb        lb        bot         rb
      ...
      Why? For symetry and bind/split reasons: FBase->SonLeft->VBase is always
     the good vertex to take (see vertex binding).
  */
  ...
  if (f1->FLeft == NULL || f1->FLeft->isLeaf()) {
    ...
    // For geomorph (VertexProgram or soft), compute MaxFaceSize and
    // MaxNearLimit.
  ...
  // In all case, must create new FarVertices, since rect split occurs on
  // border!!
  ...
  // Neigbors pointers of undetermined splitted face are not changed. Must
  // Doesn't change this. Used and Updated in section 5. ...

  // 3. Update Tile infos.
  //----------------------
  // There is no update tileinfo with rectangular patch, since tiles are always
  // squares. (TileLimitLevel>SquareLimitLevel).

  // NB: but must test update of tile info for neighboring, ie 2 faces around
  // the splits. For Vertex program only
  if (CLandscapeGlobals::VertexProgramEnabled) {
    // if neighbor face splitted, and if 2 different patchs, we must update the
    // Tile vertices because MaxFaceSize and MaxNearLimit may have changed.
    if (f0->FLeft != NULL && !f0->FLeft->isLeaf() &&
        f0->FLeft->Patch != Patch) {
  ...
  for (sint i = 0; i < 2; i++) {
    ...
    if (f->FLeft == NULL) {
      ...
      // Warning: at each iteration, the pointer of FLeft may change (because of
      // split() which can change the neighbor and so f).
      while (f->FLeft->isLeaf())
        f->FLeft->split();

      // There is a possible bug here (maybe easily patched). Sons may have be
      // propagated splitted. And problems may arise because this face hasn't
      // yet good connectivity (especially for rectangles!! :) ).
  ...
  //------------------------------------------------------
  // Since we are freshly splitted, unlink from any list, and link to the
  // MergePriorityList, because must look now when should merge.
...
void CTessFace::split(bool propagateSplit) {
  ...
  //	return;
  // since split() may reach LS_MAXLEVEL, but enforce splits which outpass this
  // stage!!
  ...
  SonLeft->FBase = FLeft;
  if (FLeft)
    FLeft->changeNeighbor(this, SonLeft);
  ...
  if (FRight)
    FRight->changeNeighbor(this, SonRight);
  ...
  // FBase->FBase==this. Must Doesn't change this. Used and Updated in
  // section 5. ...
  ...
  if (FBase == NULL || FBase->isLeaf()) {
    ...
    newVertex->EndPos =
        Patch->computeVertex(SonLeft->PVBase.getS(), SonLeft->PVBase.getT());
    ...
    // NB: this work with both rectangular and square triangles (see
    // splitRectangular()).
    ...
    SonLeft->VBase->MaxNearLimit =
        max(SonLeft->VBase->MaxNearLimit, computeNearLimit());
  }

  // Must create/link *->FVBase.
  // HERE, we must create a FarVertex too if the neighbor is not of the same
  // patch as me.
  if (FBase == NULL || FBase->isLeaf() || FBase->Patch != Patch) {
    ...
    // NB For VertexProgram only: no need to refill the Far vertex of neighbor,
    // because neighbor is of same Patch So MaxNearLimit and MaxFaceSize should
    // be the same.
  }
  ...
  // NB: must do it after compute of SonLeft->VBase->Pos for good filling in
  // VBuffer. There is no problem with rectangular patch, since tiles are always
  // squares. If new tile ....
  ...
  if (FBase == NULL) {
    ...
    // Warning: at each iteration, the pointer of FBase may change (because of
    // split() which can change the neighbor and so "this").
    while (FBase->isLeaf())
      FBase->split();

    // There is a possible bug here (maybe easily patched). Sons may have be
    // propagated splitted. And problems may arise because this face hasn't yet
    // good connectivity.
...
bool CTessFace::canMerge(bool testEm) {
  ...
  // Must not return false in preceding tests, because must set
  // RecursMarkCanMerge to false.
  RecursMarkCanMerge = false;
...
void CTessFace::doMerge() {
  ...
  if (!isRectangular()) {
    ...
    //-----------------------
    // Delete vertex, only if not already done by the neighbor (ie neighbor not
    // already merged to a leaf). NB: this work even if neigbor is rectnagular.
    ...
    //-------------------
    // Delete Uv.
    // Must do it for this and FBase separately, since they may not have same
    // tile level (if != patch).
    if (SonLeft->Level == Patch->TileLimitLevel) {
      ...
      // Delete Uv, only if not already done by the neighbor (ie neighbor not
      // already merged to a leaf). But Always delete if neighbor exist and has
      // not same tile as me. NB: this work with rectangular neigbor patch,
      // since sameTile() will return false if different patch.
    ...
    FLeft = SonLeft->FBase;
    if (FLeft)
      FLeft->changeNeighbor(SonLeft, this);
    FRight = SonRight->FBase;
    if (FRight)
      FRight->changeNeighbor(SonRight, this);
    // delete sons.
    Patch->getLandscape()->deleteTessFace(SonLeft);
    Patch->getLandscape()->deleteTessFace(SonRight);
    SonLeft = NULL;
    ...
    // Rectangular case.
    // Since minimum Order is 2, Sons of rectangular face are NEVER at
    // TileLimitLevel. => no Uv merge to do.
    ...
    //-----------------------
    // Delete vertex, only if not already done by the neighbor (ie neighbor not
    // already merged to a leaf). NB: this work even if neigbor is rectangular
    // (see tesselation rules in splitRectangular()).
    if (!FLeft || !FLeft->isLeaf())
      Patch->getLandscape()->deleteTessVertex(SonLeft->VBase);

    // Delete Far Vertex. Rect patch: neightb must be of a != pathc as me =>
    // must delete FarVertex.
  ...
  //------------------------------------------------------
  // Since we are freshly merged, unlink from any list, and link to the
  // SplitPriorityList, because must look now when we should split again.
  ...
  if (Father) {
    ...
    // If sons of father are both leaves (ie this, and the other (complexe case
    // if rectangle) )
...
void CTessFace::refineAll() {
  ...
  /*
      ...
      if(ps E [RefineThreshold, RefineThreshold*2]), the face must be splitted
     (ave leaves), and is geomorphed. if(ps>RefineThreshold*2), the face is
     fully splitted/geomoprhed (tests reported on sons...).
  */
  ...
  //-----------------------
  {
    ...
    if (isLeaf()) {
      if (ps > 1.0f && Level < (Patch->TileLimitLevel +
      ...
      if (ps < 1.0f) {
        ...
        // canMerge() test all the good thing: FBase==CantMergeFace, or this is
        // rectangular etc... The test is propagated to neighbors.
        if (canMerge(true)) {
          merge();
        }
      }
...
/*
    if(errorMetric<RefineThreshold), the face must be merged (ie have no
   leaves). if(errorMetric E [RefineThreshold, RefineThreshold*2]), the face
   must be splitted (ave leaves), and is geomorphed.
    if(errorMetric>RefineThreshold*2), the face is fully splitted/geomoprhed.
*/

// Compute distNormalSplitMerge: distance from refineCenter to normal
// split/merge (ie without tile transition):
/*
    normal ErrorMetric formula is:
        em = Size*OORefineThreshold/ dist^2;	with dist == (SplitPoint -
   CLandscapeGlobals::RefineCenter).norm() So inverse this function and we have:
...
/* When we are sure that CLandscapeGlobals::TileDistNear < distMinFace <
   CLandscapeGlobals::TileDistFar, the clamp in the original formula is skipped
...
/*
    ...
    In the 2 most simples cases, the action we want is "Know when we enters in a
   bounding volume" This fit well for Quadrant notion.

    In the case "TileDistNear to TileDistFar", the rule is too complicated and
   there is also the notion of "Know when we LEAVE the TileDistFar sphere around
   the face" which is incompatible with Quadrant behavior (since can go in any
   direction to leave the sphere).

    updateRefineMerge() are at least all notion of "Know when we LEAVE the
   SplitSphere around the SplitPoint" which is incompatible with Quadrant
   behavior. This is why updateRefineMerge() don't bother at all quadrant, and
   so the _MergePriorityList is inited with 0 quadrants.
...
void CTessFace::updateRefineSplit() {
  ...
  */
  ...
  //-----------------------
  // If splitted, then insertion in Landscape->MergePriorityList at 0 has been
  // done. so nothing to update. Else, must compute when whe should re-test.
  if (!splitted) {
    ...
    float distNormalSplitMerge =
        (float)sqrt(Size * CLandscapeGlobals::OORefineThreshold);

    // If the face is at its max subdivision
    if (Level >=
        Patch->TileLimitLevel + CLandscapeGlobals::TileMaxSubdivision) {
      // special case: the face do not need to be tested for splitting, because
      // Max subdivision reached. Hence just unlink from any list, and return.
      unlinkInPList();
      return;
    } else if (Level >= Patch->TileLimitLevel) {
      // Always normal ErrorMetric. Because Faces at Tile level decide to split
      // or merge their sons independently of "Tile ErrorMetric".

      // The test is "when do we enter in the Split area?", so we can use
      // Quadrant PriorityList
      quadrantId = Patch->getLandscape()->_SplitPriorityList.selectQuadrant(
          dirToSplitPoint);
      ...
      if (quadrantId > 0) {
        const CVector &quadrantDir =
            Patch->getLandscape()->_SplitPriorityList.getQuadrantDirection(
                quadrantId);

        // We must not approach the SplitPoint at distNormalSplitMerge
        float dMin = quadrantDir * dirToSplitPoint - distNormalSplitMerge;

        // If the dist with quadrant is too small then use the std way (without
        // quadrant).
      ...
      // Compute Distance of the face from RefineCenter. It is the min of the 3
      // points, as in computeTileErrorMetric().
      ...
      // compute the delta distance to the normal split point. See above for
      // doc.
      float normalEMDeltaDist;
      normalEMDeltaDist = distSplitPoint - distNormalSplitMerge;

      /*
      There is 3 possibles cases, according to level, and the distances
      minFaceDist:
      */
      ///	TileDistFar to +oo.
      if (distMinFace > CLandscapeGlobals::TileDistFar) {
        // The test is "when do we enter in the Split area OR in TileDistFar
        // area?", so we can use Quadrant PriorityList
        quadrantId = Patch->getLandscape()->_SplitPriorityList.selectQuadrant(
            dirToSplitPoint);

        // compute deltaDist as default "Direction less quadrant"
        minDeltaDistToUpdate = normalEMDeltaDist;
        // We must know when we enter in TileErrorMetric zone, because the
        // computing is different.
        minDeltaDistToUpdate = min(
            minDeltaDistToUpdate, distMinFace - CLandscapeGlobals::TileDistFar);
        ...
        if (quadrantId > 0) {
          ...
          // If the dist with quadrant is too small then use the std way
          // (without quadrant).
          if (dMin < NL3D_TESS_USE_QUADRANT_THRESHOLD)
            quadrantId = 0;
          // else ok, use quadrant behavior
      ...
      /// TileDistNear to TileDistFar.
        ...
        for (uint i = 0; i < nbRecurs; i++) {
          ...
          // If the em computed with this distance is still <1 (ie merged), then
          // we can move further.
          if (computeTileEMForUpdateRefine(distSplitPoint - pivotDeltaDist,
                                           distMinFace - pivotDeltaDist,
                                           nearLimit) < 1)
        ...
        // take the min with distance of distMinFace to the TileDistNear and
        // TileDistFar sphere, because formula change at those limits.
        minDeltaDistToUpdate =
            min(distTileTransSplitMerge,
                CLandscapeGlobals::TileDistFar - distMinFace);
        minDeltaDistToUpdate =
            min(minDeltaDistToUpdate,
                distMinFace - CLandscapeGlobals::TileDistNear);
      }
      /// 0 to TileDistNear.
      else {
        // because the face is not a Tile Level (ie
        // Level<Patch->TileLimitLevel), it should be splitted, and won't merge
        // until reaching at least the TileDistNear sphere. if not splited
        // (should not arise), force the split next time.
    ...
    // Until the RefineCenter move under minDeltaDistToUpdate, we don't need to
    // test face.
    Patch->getLandscape()->_SplitPriorityList.insert(
        quadrantId, minDeltaDistToUpdate, this);
  }
}
...
void CTessFace::updateRefineMerge() {
  ...
  */
  ...
  bool merged = false;
  {
    ...
    if (ps < 1.0f) {
      ...
      if (canMerge(true)) {
        ...
        // NB: here, merge() is not propagated to fathers (supposed to be not
        // very useful).
  ...
  // Insert the face in the priority list.
  //-----------------------
  // If merged, then insertion in Landscape->SplitPriorityList at 0 has been
  // done. so nothing to update. Else, must compute when whe should re-test.
  if (!merged) {
    ...
    // The distance of SplitPoint to center.
    float distSplitPoint =
        (SplitPoint - CLandscapeGlobals::RefineCenter).norm();
    // Compute distance from refineCenter to normal split/merge (ie without tile
    // transition).
    float distNormalSplitMerge =
        (float)sqrt(Size * CLandscapeGlobals::OORefineThreshold);
    ...
    if (Level >=
        Patch->TileLimitLevel + CLandscapeGlobals::TileMaxSubdivision) {
      // since the face is splitted, then must test always this face, because we
      // must merge it (as soon as it is possible).
      minDeltaDistToUpdate = 0;
    } else if (Level >= Patch->TileLimitLevel) {
      // Always normal ErrorMetric. Because Faces at Tile level decide to split
      // or merge their sons independently of "Tile ErrorMetric". since
      // splitted, compute distance to merge.
      minDeltaDistToUpdate = distNormalSplitMerge - distSplitPoint;
      // NB: it is possible that minDeltaDistToUpdate<0. A good example is when
      // we are enforced split. Then, distSplitMerge may be < distSplitPoint,
      // meaning we should have not split, but a neigbhor has enforced us. So
      // now, must test every frame if we can merge....
      ...
      */
      ///	TileDistFar to +oo.
      if (distMinFace > CLandscapeGlobals::TileDistFar) {
        // normal geomorph. Any face compute the distance to the SplitPoint, and
        // take min with distance to the TileDistFar sphere.
        minDeltaDistToUpdate = normalEMDeltaDist;
      ...
      /// TileDistNear to TileDistFar.
        ...
        for (uint i = 0; i < nbRecurs; i++) {
          float pivotDeltaDist = (maxDeltaDist - minDeltaDist) / 2;
          // If the em computed with this distance is still >1 (ie splitted),
      ...
      /// 0 to TileDistNear.
        ...
        // because the face is not a Tile Level (ie
        // Level<Patch->TileLimitLevel), it should be splitted, and won't merge
        // until reaching at least the TileDistNear sphere. Since splitted, Must
        // enter in TileErrorMetric area to know when to merge.
        minDeltaDistToUpdate = CLandscapeGlobals::TileDistNear - distMinFace;
      }
    }

    // Merge Refine Threshold: because of enforced splits, we have lot of faces
    // whit minDeltaDistToUpdate<0, because they alwayas want to merge. To avoid
    // this, add a delta, which delay the test for merge. The caveat is that
    // faces which do not need this may merge later. But 2 meters won't add too
    // many faces.
    ...
    // Until the RefineCenter move under minDeltaDistToUpdate, we don't need to
    // test face.
    Patch->getLandscape()->_MergePriorityList.insert(0, minDeltaDistToUpdate,
                                                     this);
  }
}
...
void CTessFace::unbind() {
  ...
  if (!isRectangular()) {
    ...
    if (FBase && FBase->Patch != Patch) {
      ...
      FBase = NULL;
      if (!isLeaf()) {
        // Duplicate the VBase of sons, so the unbind is correct and no vertices
        // are shared.
        ...
        *(SonLeft->VBase) = *old;
        ...
        // For geomorph (VertexProgram or soft), compute good MaxFaceSize and
        // MaxNearLimit (change since unbinded) update us.
  ...
  //==================
    ...
    if (FLeft && FLeft->Patch != Patch) {
      ...
      if (!isLeaf()) {
        // Duplicate the VBase of sons, so the unbind is correct and no vertices
        // are shared. NB: this code is a bit different from square case.
        CTessVertex *old = SonLeft->VBase;
        SonLeft->VBase = Patch->getLandscape()->newTessVertex();
        *(SonLeft->VBase) = *old;
  ...
  //==================
  if (!isLeaf()) {
    // update sons vertex pointers (since they may have been updated by me or my
    // grandfathers).
    if (!isRectangular()) {
      SonLeft->VLeft = VBase;
      SonLeft->VRight = VLeft;
...
void CTessFace::forceMerge() {
  ...
  if (!isLeaf()) {
    // First, force merge of Sons and neighbor sons, to have a diamond
    // configuration.
    SonLeft->forceMerge();
    SonRight->forceMerge();
...
void CTessFace::averageTesselationVertices() {
  ...
  // Special Rectangular case.
    ...
    // NB: here, just need to compute average of myself with FLeft, because my
    // neighbor FBase is on same patch (see splitRectangular()), and is average
    // with its FLeft neighbor is done on another branch of the recurs call.
    neighbor = FLeft;
  }

  /* Try to average with neighbor.
      - if no neighbor, no-op :).
      - if neighbor is bind 1/N (CantMergeFace), no-op too, because the vertex
     is a BaseVertex, so don't modify.
      - if my patch is same than my neighbor, then we are on a same patch :),
     and so no need to average.
  */
  if (neighbor != NULL && neighbor != &CantMergeFace &&
    ...
    CVector v0 =
        Patch->computeVertex(SonLeft->PVBase.getS(), SonLeft->PVBase.getT());
    CVector v1 = neighbor->Patch->computeVertex(
        neighbor->SonLeft->PVBase.getS(), neighbor->SonLeft->PVBase.getT());
...
void CTessFace::refreshTesselationGeometry() {
  ...
  /* NB: rectangular case: just need to take SonLeft->VBase, because my neighbor
     on FBase will compute his son on another branch of the recurs call.
  */
  // re-compute this position (maybe with new noise geometry in Tile Edition).
  SonLeft->VBase->EndPos =
      Patch->computeVertex(SonLeft->PVBase.getS(), SonLeft->PVBase.getT());
...
bool CTessFace::updateBindEdge(CTessFace *&edgeFace, bool &splitWanted) {
  ...
  /*
      ...
      Therefore, edgeFace is a valid pointer (either if it is FLeft, FRight or
     FBase).
  */
  ...
  //=====================================
    ...
    // FBase should be tested too. If edgeFace->FBase==this, I should be myself
    // a rectangular face.
...
void CTessFace::updateBindAndSplit() {
  ...
  /*
      Look at the callers, and you'll see that "this" is always a leaf.
      Therefore, FBase, FLeft and FRight are good pointers, and *FLeft and
     *FRight should be Ok too.
  */
  nlassert(isLeaf());
  ...
  if (!isRectangular() && fmult) {
    ...
    // In 1/4 case, fmult->SonLeft and fmult->SonRight are themselves MultiPatch
    // face. So it will recurse.
    ...
    //================================
    // Because *->VBase may have been merged to the multiple bind face,
    // Near/FarVertices which pointed on it must be setup. We do not have to
    // propagate this vertex ptr change since sons are leaves!!
    nlassert(SonLeft->isLeaf() && SonRight->isLeaf());
    ...
    // The only way to do this, is to unbind the patch from all (then the vertex
    // is cloned), then the merge will be Ok.
  }
  // Else if rectangular.
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (fmult) {
        ...
        // And update sons pointers, to good vertex (rectangular case, see
        // tesselation rules).
        fl->VBase = vert;
        fr->VLeft = vert;
    ...
    // After all updates done. recompute centers of both sons 's faces, and
    // update far vertices pointers.
    for (i = 0; i < 2; i++) {
      if (i == 0)
        f = f0;
      else
...
void CTessFace::updateBind() {
  /*
      Remind that updateBind() is called ONLY on the patch which is binded (not
     the neighbors). Since updateBind() is called on the bintree, and that
     precedent propagated split may have occur, we may not be a leaf here. So we
     are not sure that FLeft and FRight are good, and we doesn't need to update
     them (since we have sons). Also, since we are splitted, and correctly
     linked (this may not be the case in updateBindAndSplit()), FBase IS
      correct. His FBase neighbor and him form a diamond. So we don't need to
     update him.
...
// ***************************************************************************
static inline bool matchEdge(const CVector2f &uv0, const CVector2f &uv1,
                             const CVector2f &uva, const CVector2f &uvb) {
  if (uv0 == uva && uv1 == uvb)
    return true;
  if (uv0 == uvb && uv1 == uva)
    return true;
...
// ***************************************************************************
CTessFace *CTessFace::linkTessFaceWithEdge(const CVector2f &uv0,
                                           const CVector2f &uv1,
                                           CTessFace *linkTo) {
...
void CTessFace::deleteTileUvs() {
  // NB: NearVertices are removed from renderlist with deleteTileUv (called in
  // releaseTileMaterial()).
  ...
  if (!isLeaf()) {
    ...
    if (SonLeft->Level == Patch->TileLimitLevel) {
      ...
      // Delete Uv, only if not already done by the neighbor (ie neighbor has
      // yet TileFaces!!). But Always delete if neighbor exist and has not same
      // tile as me. NB: this work with rectangular neigbor patch, since
      // sameTile() will return false if different patch.
      if (!FBase || !FBase->SonLeft->emptyTileFaces() ||
          !sameTile(this, FBase)) {
        SonLeft->deleteTileUv(IdUvBase);
      }
      // In all case, must delete the tilefaces of those face.
      SonLeft->deleteTileFaces();
      SonRight->deleteTileFaces();
      // For createTileUvs, it is important to mark those faces as NO
      // TileMaterial.
...
void CTessFace::recreateTileUvs() {
  // NB: NearVertices are append to renderlist with allocTileUv (called in
  // computeTileMaterial()/heritTileMaterial()).

  if (!isLeaf()) {
    // Must recreate the materials of parent first.

    // There is no problem with rectangular patch, since tiles are always
    // squares. If new tile ....
    if (SonLeft->Level == Patch->TileLimitLevel) {
      SonLeft->computeTileMaterial();
      SonRight->computeTileMaterial();
...
void CTessFace::heritTileMaterial() {
  ...
  if (FBase != NULL && !FBase->isLeaf() &&
      FBase->SonLeft->TileMaterial != NULL && sameTile(this, FBase)) {
...
// ***************************************************************************
void CTessFace::getTesselatedPos(const CUV &uv, bool verifInclusion,
                                 CVector &ret) {
  ...
  if (isLeaf())
    ...
    // must subdivide.
    // if we are rectangular (strange tesselation), must test in both leaves,
...
// ***************************************************************************
void CTessFace::appendTessellationLeaves(

...
void CMeshGeom::build(CMesh::CMeshBuild &m, uint numMaxMaterial) {
  ...
  _Skinned = ((m.VertexFlags & CVertexBuffer::PaletteSkinFlag) ==
              CVertexBuffer::PaletteSkinFlag);
  ...
  /** If all texture coordinates are of dimension 2, we can setup the flags as
   * before. If this isn't the case, we must setup a custom format
   */
  ...
  } else // setup extended format
  {
    ...
    if (vbFlags & CVertexBuffer::PositionFlag)
      _VBuffer.addValueEx(CVertexBuffer::Position, CVertexBuffer::Float3);
    if (vbFlags & CVertexBuffer::NormalFlag)
      _VBuffer.addValueEx(CVertexBuffer::Normal, CVertexBuffer::Float3);
    if (vbFlags & CVertexBuffer::PrimaryColorFlag)
      _VBuffer.addValueEx(CVertexBuffer::PrimaryColor, CVertexBuffer::UChar4);
    if (vbFlags & CVertexBuffer::SecondaryColorFlag)
      _VBuffer.addValueEx(CVertexBuffer::SecondaryColor, CVertexBuffer::UChar4);
    if (vbFlags & CVertexBuffer::WeightFlag)
      _VBuffer.addValueEx(CVertexBuffer::Weight, CVertexBuffer::Float4);
    if (vbFlags & CVertexBuffer::PaletteSkinFlag)
      _VBuffer.addValueEx(CVertexBuffer::PaletteSkin, CVertexBuffer::UChar4);
    if (vbFlags & CVertexBuffer::FogFlag)
      _VBuffer.addValueEx(CVertexBuffer::Fog, CVertexBuffer::Float1);
  ...
  for (; N > 0; N--, pFace++) {
    ...
    ib.lock(iba);
    iba.setTri(index, pFace->Corner[0].VBId, pFace->Corner[1].VBId,
               pFace->Corner[2].VBId);
  }
  ...
  //============================
  for (mb = 0; mb < _MatrixBlocks.size(); mb++) {
    // NB: slow process (erase from a vector). Doens't matter since made at
    // build.
    vector<CRdrPass>::iterator itPass;
    for (itPass = _MatrixBlocks[mb].RdrPass.begin();
         itPass != _MatrixBlocks[mb].RdrPass.end();) {
  ...
  if (_Skinned) {
    ...
    for (matrixBlock = 0; matrixBlock < _MatrixBlocks.size(); matrixBlock++) {
      ...
      for (matrix = 0; matrix < mb.NumMatrix; matrix++) {
        ...
        if (ite == remap.end()) {
          // Insert it
          remap.insert(std::map<uint, uint>::value_type(mb.MatrixId[matrix],
                                                        currentBone));
  ...
  bool avoidVBHard =
      _Skinned || (_MeshMorpher && !_MeshMorpher->BlendShapes.empty());
  _VBuffer.setPreferredMemory(avoidVBHard ? CVertexBuffer::RAMPreferred
...
// ***************************************************************************
void CMeshGeom::render(IDriver *drv, CTransformShape *trans, float polygonCount,
                       uint32 rdrFlags, float globalAlpha) {
  nlassert(drv);
  ...
  if (bMorphApplied) {
    ...
    if (_Skinned) {
      _MeshMorpher->initSkinned(&_VBufferOri, &_VBuffer, useTangentSpace,
    ...
    {
      _MeshMorpher->init(&_VBufferOri, &_VBuffer, useTangentSpace);
  ...
  // since instance skin is invalid but mesh is skinned , we must copy
  ...
  if (useMeshVP) {
    ...
    useMeshVP = _MeshVertexProgram->begin(
        drv, ownerScene, mi, invertedObjectMatrix, renderTrav->CamPos);
      ...
      if (dynamic_cast<CMeshVPWindTree *>(&(*_MeshVertexProgram))) {
        ...
        for (uint mb = 0; mb < _MatrixBlocks.size(); mb++) {
          ...
          for (uint i = 0; i < mBlock.RdrPass.size(); i++) {
            ...
            mat.setLighting(true, mat.getEmissive(), mat.getAmbient(),
                            mat.getDiffuse(), mat.getSpecular());
          }
        }
      }
...
void CMeshGeom::renderSkin(CTransformShape *trans, float alphaMRM) {
  ...
  bool supportVertexStream =
      ((_VBuffer.getVertexFormat() &
        ~(CVertexBuffer::PaletteSkinFlag | CVertexBuffer::WeightFlag)) ==
       (CVertexBuffer::PositionFlag | CVertexBuffer::NormalFlag |
        CVertexBuffer::TexCoord0Flag)) &&
      (_VBuffer.getValueType(CVertexBuffer::TexCoord0) ==
       CVertexBuffer::Float2) &&
  ...
  if (supportVertexStream) {
    ...
    if (maxVertices >= _OriginalSkinVertices.size() && vertexSize == 32) {
      ...
      nlwarning("Unable to animate skinned model, too many vertices, or stream "
                "format unsupported");
    }
  } else {
...
// ***************************************************************************
void CMeshGeom::renderSimpleWithMaterial(IDriver *drv,
                                         const CMatrix &worldMatrix,
                                         CMaterial &mat) {
...
void CMeshGeom::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  if (f.isReading()) {
    ...
    // Warning, if you have skinned this shape, you can't write it anymore
    // because skinning id have been changed!
  ...
  if (f.isReading()) {
    // if >= version 2, reorder of triangles is precomputed, else compute it
    // now.
    if (ver < 2) {
      optimizeTriangleOrder();
    }
  ...
  // Skinning: If Version < 4, _BonesName are not present, must compute _BonesId
  // from localId Else it is computed at first computeBonesId().
  if (ver < 4)
    buildBoneUsageVer3();
  ...
  /*{
      ...
      nlinfo("YOYO: %d Vertices. %d Triangles.", _VBuffer.getNumVertices(),
  numTris);
  }*/
...
void CMeshGeom::compileRunTime() {
  ...
  bool supportMeshBlockRendering =
      !_Skinned && _MeshMorpher->BlendShapes.empty();

  // true only if one matrix block, and at least one rdrPass.
  supportMeshBlockRendering = supportMeshBlockRendering &&
                              _MatrixBlocks.size() == 1 &&
                              !_MatrixBlocks[0].RdrPass.empty();
  if (supportMeshBlockRendering && _MeshVertexProgram) {
    supportMeshBlockRendering = supportMeshBlockRendering &&
                                _MeshVertexProgram->supportMeshBlockRendering();
  }
  ...
  bool supportMBRPerMaterial =
      supportMeshBlockRendering && _MeshVertexProgram == NULL;
  ...
  for (i = 0; i < getNbMatrixBlock(); i++) {
    for (uint rp = 0; rp < getNbRdrPass(i); rp++) {
      ...
      if (pb.getFormat() == CIndexBuffer::Indices32) {
        memcpy(&indices[triIdx * 3], iba.getPtr(),
               pb.getNumIndexes() * sizeof(uint32));
      } else {
        // std::copy will convert from 16 bits index to 32 bit index
#ifdef NL_COMP_VC14
        std::copy((uint16 *)iba.getPtr(),
                  ((uint16 *)iba.getPtr()) + pb.getNumIndexes(),
                  stdext::make_checked_array_iterator(
                      &indices[triIdx * 3], indices.size() - triIdx * 3));
#else
        std::copy((uint16 *)iba.getPtr(),
                  ((uint16 *)iba.getPtr()) + pb.getNumIndexes(),
                  &indices[triIdx * 3]);
...
// ***************************************************************************
void CMeshGeom::buildSkin(CMesh::CMeshBuild &m,
                          std::vector<CFaceTmp> &tmpFaces) {
  sint i, j, k;
  TBoneMap remainingBones;
  list<uint> remainingFaces;

  // 0. normalize SkinWeights: for all weights at 0, copy the matrixId from 0th
  // matrix => no random/bad use of matrix.
  //================================
  for (i = 0; i < (sint)m.SkinWeights.size(); i++) {
    ...
    for (j = 1; j < NL3D_MESH_SKINNING_MAX_MATRIX; j++) {
      ...
      if (sw.Weights[j] == 0) {
        // Setup MatrixId so that this vertex do no use more matrix than it
        // really wants.
  ...
  // 1. build the list of used/remaining bones, in ascending order. (so we use
  // the depth-first topolgy of hierarchy).
  //================================
  for (i = 0; i < (sint)tmpFaces.size(); i++) {
  ...
  while (!remainingFaces.empty()) {
    ...
    //============================
    ItBoneMap itBone;
    for (itBone = remainingBones.begin(); itBone != remainingBones.end();
         itBone++) {
      itBone->second.Inserted = false;
    }

    // b. while still exist bones, try to insert faces which use them in
    // matrixBlock.
    //============================
    while (!remainingBones.empty()) {
      ...
      // If no more faces in the remainingFace list use this bone, remove it,
      // and continue.
      ...
      // traverse all faces, trying to insert them in current MatrixBlock
      // processed.
      ...
      for (itFace = remainingFaces.begin(); itFace != remainingFaces.end();) {
        ...
        // compute how many bones that are not in the current matrixblock this
        // face use.
        newBoneAdded = 0;
        for (i = 0; i < (sint)boneUse.size(); i++) {
          // if this bone is not inserted in the current matrix block, inform
          // it.
        ...
        // If this face do not add any more bone, we can insert it into the
        // current matrixblock. If it use the currentBoneId, and do not explode
        // max count, we allow insert it too in the current matrixblock.
        if (newBoneAdded == 0 ||
            (useCurrentBoneId &&
             newBoneAdded + matrixBlock.NumMatrix < IDriver::MaxModelMatrix)) {
          ...
          // do not append this face to the current matrix block, skip to the
          // next
          itFace++;
        }
      }

      // If no faces have been added during this pass, we are blocked, and
      // either the MatrixBlock may be full, or there is no more face. So quit
      // this block and process a new one.
      if (!faceAdded)
        break;
    }
  }
  // NB: at the end of this loop, remainingBones may not be empty(), but all
  // remainingBones should have RefCount==0.

  // 4. Re-order matrix use in MatrixBlocks, for minimum matrix change between
  // MatrixBlocks.
  //================================
  ...
  for (i = 1; i < (sint)_MatrixBlocks.size(); i++) {
    ...
    for (j = 0; j < (sint)mBlock.NumMatrix; j++) {
      ...
      // If not found, or if bigger than current array, fails (cant be
      // mirrored). Or if already mirrored.
      if (idLoc == -1 || idLoc >= (sint)mBlock.NumMatrix || idLoc == j) {
        // next id.
        j++;
      } else {
        // puts me on my mirrored location. and swap with the current one at
        // this mirrored location.
        swap(mBlock.MatrixId[j], mBlock.MatrixId[idLoc]);
        // mBlock.MatrixId[j] is now a candidate for mirror.
      }
    }

    // Then build the Remap table, to re-order faces matrixId which use this
    // matrix block.
    for (j = 0; j < (sint)mBlock.NumMatrix; j++) {
...
// ***************************************************************************
void CMeshGeom::CFaceTmp::buildBoneUse(
    vector<uint> &boneUse, vector<CMesh::CSkinWeight> &skinWeights) {
  boneUse.clear();
...
void CMeshGeom::computeBonesId(CSkeletonModel *skeleton) {
  ...
  if (!_BoneIdComputed) {
    ...
    if (skeleton) {
      ...
      // **** Remap matrix blocks
      for (uint matrixBlock = 0; matrixBlock < _MatrixBlocks.size();
           matrixBlock++) {
        // Ref on the matrix block
        CMatrixBlock &mb = _MatrixBlocks[matrixBlock];
...
void CMeshGeom::applySkin(void *dstVb, CSkeletonModel *skeleton) {
  ...
  for (uint mb = 0; mb < _MatrixBlocks.size(); mb++) {
    ...
    computeSkinMatrixes(skeleton, matrixes,
                        mb == 0 ? NULL : &_MatrixBlocks[mb - 1],
                        _MatrixBlocks[mb]);
    ...
    for (; size > 0; size--) {
      ...
      if (*pFlag == NL3D_SOFTSKIN_VMUSTCOMPUTE) {
        ...
        *pFlag = NL3D_SOFTSKIN_VCOMPUTED;
        ...
        // compute vertex part.
        computeSoftwarePointSkinning(matrixes, srcVector, psPal,
                                     (float *)srcWgt, (CVector *)dstVector);

        // compute normal part.
        computeSoftwareVectorSkinning(matrixes, srcNormal, psPal,
                                      (float *)srcWgt, (CVector *)dstNormal);
      }
...
// ***************************************************************************
void CMeshGeom::flagSkinVerticesForMatrixBlock(uint8 *skinFlags,
                                               CMatrixBlock &mb) {
  for (uint i = 0; i < mb.RdrPass.size(); i++) {
    ...
    // This may be better to flags in 2 pass (first traverse primitives, then
    // test vertices). Better sol for BTB..., because number of tests are
    // divided by 6 (for triangles).
...
// ***************************************************************************
void CMeshGeom::computeSoftwarePointSkinning(CMatrix3x4 *matrixes,
                                             CVector *srcVec,
                                             CPaletteSkin *srcPal,
                                             float *srcWgt, CVector *pDst) {
  CMatrix3x4 *pMat;
...
// ***************************************************************************
void CMeshGeom::computeSoftwareVectorSkinning(CMatrix3x4 *matrixes,
                                              CVector *srcVec,
                                              CPaletteSkin *srcPal,
                                              float *srcWgt, CVector *pDst) {
  CMatrix3x4 *pMat;
...
// ***************************************************************************
void CMeshGeom::computeSkinMatrixes(CSkeletonModel *skeleton,
                                    CMatrix3x4 *matrixes,
  ...
  for (uint idMat = 0; idMat < mBlock.NumMatrix; idMat++) {
    ...
    if (prevBlock && idMat < prevBlock->NumMatrix &&
        prevBlock->MatrixId[idMat] == curBoneId)
      continue;
...
// ***************************************************************************
void CMeshGeom::profileSceneRender(CRenderTrav *rdrTrav, CTransformShape *trans,
  ...
  if (triCount) {
    ...
    rdrTrav->Scene->incrementProfileTriVBFormat(
        rdrTrav->Scene->BenchRes.MeshProfileTriVBFormat,
...
// ***************************************************************************
bool CMeshGeom::intersectSkin(CTransformShape *mi, const CMatrix &toRaySpace,
  ...
  // use other Matrix than the mesh use).
  ...
  return _ShadowSkin.getRayIntersection(toRaySpace, *skeleton, matInfs, dist2D,
                                        distZ, computeDist2D);
}
...
void CMeshGeom::buildShadowSkin() {
  ...
  nlassert(_Skinned &&
           (_VBuffer.getVertexFormat() & CVertexBuffer::PaletteSkinFlag) &&
  ...
  // *** Copy VBuffer content
  {
    ...
    for (uint i = 0; i < numVertices; i++) {
      ...
      // Suppose the 0 matrix inf is the highest (we are at least sure it is not
      // 0) And SkinWeight Export show the 0th is the highest one...
  ...
  // But _ShadowSkin.Vertices[i].MatrixId is incorrect, since <
  // IDriver::MaxModelMatrix

  // *** Count number of triangles, and get start index of each matrix block in
  // the final Tri list
  uint numIndices = 0;
  uint mb;
...
void CMeshGeom::beginMesh(CMeshGeomRenderContext &rdrCtx) {
  if (rdrCtx.RenderThroughVBHeap) {
    ...
    if (_MeshVertexProgram != NULL &&
        _MeshVertexProgram->isMBRVpOk(rdrCtx.Driver)) {
      // Ok will use it.
...
// ***************************************************************************
void CMeshGeom::activeInstance(CMeshGeomRenderContext &rdrCtx,
  ...
  if (_SupportMBRFlags & MBRCurrentUseVP) {
    ...
    _MeshVertexProgram->beginMBRInstance(rdrCtx.Driver, rdrCtx.Scene, inst,
                                         invertedObjectMatrix);
  }
}
// ***************************************************************************
void CMeshGeom::renderPass(CMeshGeomRenderContext &rdrCtx,
...
bool CMeshGeom::getVBHeapInfo(uint &vertexFormat, uint &numVertices) {
  // CMeshGeom support VBHeap rendering, assuming supportMeshBlockRendering is
  // true.
  if (_SupportMBRFlags)
    /* Yoyo: If VertexProgram, DON'T SUPPORT!! because VB need to be activated
       AFTER meshVP activation NB: still possible with complex code to do it
       (sort per VP type (with or not)...), but tests in Ryzom shows that VBHeap
       is not really important (not so much different shapes...)
    */
    if (_MeshVertexProgram == NULL) {
...
void CMeshGeom::computeMeshVBHeap(void *dst, uint indexStart) {
  ...
  memcpy(dst, vba.getVertexCoordPointer(),
         _VBuffer.getNumVertices() * _VBuffer.getVertexSize());
...
void CMesh::CCorner::serial(NLMISC::IStream &f) {
  ...
  f.serial(Vertex);
  f.serial(Normal);
  for (int i = 0; i < CVertexBuffer::MaxStage; ++i)
    f.serial(Uvws[i]);
  f.serial(Color);
  f.serial(Specular);
}
...
void CMesh::serial(NLMISC::IStream &f) {
  ...
  Version 6:
      - cut in serialisation, because of:
          - bad ITexture serialisation (with no version....) => must cut.  (see
  CMeshBase serial).
          - because of this and to simplify, make a cut too in CMesh
  serialisation. NB : all old version code is dropped.
  */
...
// ***************************************************************************
uint CMesh::getNbMatrixBlock() const { return _MeshGeom->getNbMatrixBlock(); }
// ***************************************************************************
...
CMesh::getRdrPassPrimitiveBlock(uint matrixBlockIndex,
  ...
  return _MeshGeom->getRdrPassPrimitiveBlock(matrixBlockIndex,
                                             renderingPassIndex);
}
// ***************************************************************************
uint32 CMesh::getRdrPassMaterial(uint matrixBlockIndex,
                                 uint renderingPassIndex) const {
...
// ***************************************************************************
const CMeshGeom &CMesh::getMeshGeom() const { return *_MeshGeom; }
// ***************************************************************************
...
// ***************************************************************************
IMeshGeom *CMesh::supportMeshBlockRendering(CTransformShape *trans,
...
// ***************************************************************************
void CMesh::profileSceneRender(CRenderTrav *rdrTrav, CTransformShape *trans,
                               bool passOpaque) {
  // 0 or 0xFFFFFFFF
  uint32 mask = (0 - (uint32)passOpaque);
  uint32 rdrFlags;
...
void CMesh::compileRunTime() {
  ...
  if ((_CollisionMeshGeneration == AutoCameraCol && !_LightInfos.empty()) ||
    ...
    if (_MeshGeom->retrieveVertices(vertices) &&
        _MeshGeom->retrieveTriangles(indices)) {
      // ok, can build!
      _VisualCollisionMesh = new CVisualCollisionMesh;
      // if fails to build cause of too many vertices/indices for instance
      if (!_VisualCollisionMesh->build(
              vertices, indices,
              const_cast<CVertexBuffer &>(_MeshGeom->getVertexBuffer()))) {
        // delete
        delete _VisualCollisionMesh;
        _VisualCollisionMesh = NULL;
      }

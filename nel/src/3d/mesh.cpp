	...
	while (!remainingFaces.empty())
	{
		...
		while (!remainingBones.empty())
		{
			...
			for (itFace = remainingFaces.begin(); itFace != remainingFaces.end();)
			{
				...
				if (newBoneAdded == 0 || (useCurrentBoneId && newBoneAdded + matrixBlock.NumMatrix < IDriver::MaxModelMatrix))
				{
...
void CMesh::buildSystemGeometry()
{
	...
	if (!_MeshGeom->retrieveVertices(_SystemGeometry.Vertices) || !_MeshGeom->retrieveTriangles(_SystemGeometry.Triangles))
	{
		_SystemGeometry.clear();
	}

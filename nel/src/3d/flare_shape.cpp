...
CFlareShape::CFlareShape()
    : _Color(NLMISC::CRGBA::White), _DazzleColor(NLMISC::CRGBA::Black),
      _SizeDisappear(0.f), _ScaleWhenDisappear(false), _AngleDisappear(0.f),
      _Persistence(1), _Spacing(1), _Attenuable(false), _AttenuationRange(1.0f),
      _FirstFlareKeepSize(false), _DazzleEnabled(false),
      _DazzleAttenuationRange(0.f), _MaxViewDistRatio(0.9f),
      _InfiniteDist(false), _OcclusionMeshNotFound(false),
      _OcclusionTestMeshInheritScaleRot(false), _LookAtMode(true) {
  // init default pos
...
void CFlareShape::serial(NLMISC::IStream &f) {
  // Version 4 : - added occlusion test mesh, size reduction, angle modification
...
CTransformShape *CFlareShape::createInstance(CScene &scene) {
  CFlareModel *fm =
      NLMISC::safe_cast<CFlareModel *>(scene.createModel(FlareModelClassId));
  fm->Shape = this;
  fm->_Scene = &scene;
  // set default pos
...
void CFlareShape::setOcclusionTestMeshName(const std::string &shapeName) {
  if (shapeName == _OcclusionTestMeshName)
    return;
...
CMesh *CFlareShape::getOcclusionTestMesh(CShapeBank &sb) {
  ...
  if (_OcclusionMeshNotFound)
    return NULL;
  if (_OcclusionTestMeshName.empty())
    return NULL;
  ...
  if (!_OcclusionTestMesh) {
    _OcclusionMeshNotFound = true;
    nlwarning("%s is not a mesh. Mesh required for occlusion testing",
              _OcclusionTestMeshName.c_str());
    sb.release(mesh);
    return NULL;
  }
  return _OcclusionTestMesh;

...
void CAsyncTextureManager::CTextureEntry::createCoarseBitmap() {
  ...
  // remove all mipmaps, and convert to DXTC1 (if possible, ie if was DXTC5 or
  // DXTC3 as example)
...
CAsyncTextureManager::~CAsyncTextureManager() {
  ...
  nlassert(_WaitingTextures.empty() && _TextureEntryMap.empty() &&
           _CurrentUploadTexture == NULL && _CurrentTextureLodLoaded == NULL);
}
...
// ***************************************************************************
uint CAsyncTextureManager::addTextureRef(const string &textNameNotLwr,
  ...
  if (it == _TextureEntryMap.end()) {
    ...
    // else must async load it.
      ...
      // load it async.
      CAsyncFileManager3D::getInstance().loadTexture(text->Texture,
                                                     &text->Loaded, position);
    }
...
void CAsyncTextureManager::deleteTexture(uint id) {
  ...
  vector<uint>::iterator itWait =
      find(_WaitingTextures.begin(), _WaitingTextures.end(), id);
...
// ***************************************************************************
void CAsyncTextureManager::releaseTexture(uint id,
                                          CMeshBaseInstance *instance) {
  ...
  while (_CurrentUploadTexture) {
    ...
    if (uploadTexturePart(pText, pDriver, nTotalUploaded)) {
      ...
      if (_CurrentUploadTexture->isTextureEntry()) {
        uint i;
        CTextureEntry *textEntry =
            static_cast<CTextureEntry *>(_CurrentUploadTexture);
        // If we are here, the texture is finally entirely uploaded. Compile it!
        textEntry->UpLoaded = true;
        // Can Have lod if texture is DXTC and have mipMaps! Also disalbe if
        // system disable it
        textEntry->CanHaveLOD =
            validDXTCMipMap(pText) && _BaseLodLevel > _MaxLodLevel;
        // compute the size it takes in VRAM
        uint baseMipMapSize = pText->getSize(0) *
        ...
        if (textEntry->CanHaveLOD) {
          /* Allow only the MaxLod to be loaded async
              This is supposed to be faster since a fseek is much longer than a
             texture Read. Then it is more intelligent to read only One texture
             (the High Def), than to try to read intermediate ones (512, 256,
             128) because this made 3 more fseek.
          */
          ...
          // extra size of the lod only (important for LoadBalacing in
          // updateTextureLodSystem())
          textLod.ExtraSize = textEntry->BaseSize *
                                  (1 << (2 * (_BaseLodLevel - _MaxLodLevel))) -
                              textEntry->BaseSize;
        ...
        textEntry->TotalTextureSizeAsked =
            textEntry->BaseSize + textEntry->HDLod.ExtraSize;
      ...
      // else, stuff for textureLod.
        ...
        CTextureLod *textLod =
            static_cast<CTextureLod *>(_CurrentUploadTexture);
        // Swap the uploaded Driver Handle with the Main texture.
        pDriver->swapTextureHandle(*textLod->Texture,
                                   *textLod->TextureEntry->Texture);
        // Flag the Lod.
...
// ***************************************************************************
bool CAsyncTextureManager::uploadTexturePart(ITexture *pText, IDriver *pDriver,
                                             uint &nTotalUploaded) {
  uint nMipMap;
  ...
  if (_CurrentUploadTextureMipMap == 0 && _CurrentUploadTextureLine == 0) {
    ...
    if (!validDXTCMipMap(pText)) {
      /* For now, prefer do nothing, because this may be an error (texture not
         found) and the texture may not be used at all, so don't take VRAM for
         nothing.
          => if the texture is used, it will be loaded synchronously by the
         caller later in the process
          => frame freeze.
      */
      /*
      // upload All now.
      ...
      /* Even if the shared texture is still referenced and so still exist in
         driver, we MUST recreate with good size the texture. This is important
         for Texture Memory Load Balancing (this may means that is used
         elsewhere than in the CAsyncTextureManager) Hence:
         bMustRecreateSharedTexture==true
      */
  ...
  for (; _CurrentUploadTextureMipMap < nMipMap; _CurrentUploadTextureMipMap++) {
    ...
    uint nWeight =
        pText->getSize(nMM) - _CurrentUploadTextureLine * pText->getWidth(nMM);
    nWeight = (nWeight * CBitmap::bitPerPixels[pText->getPixelFormat()]) / 8;
    ...
    if ((nTotalUploaded + nWeight) > _MaxUploadPerFrame) {
      ...
      uint nLineWeight = (max(pText->getWidth(nMM), (uint32)4) *
                          CBitmap::bitPerPixels[pText->getPixelFormat()]) /
                         8;
      ...
      zeRect.set(0, _CurrentUploadTextureLine, pText->getWidth(nMM),
                 pText->getHeight(nMM));
      _CurrentUploadTextureLine = 0;
    }
...
// ***************************************************************************
void CAsyncTextureManager::getNextTextureToUpLoad(uint &nTotalColored,
                                                  IDriver *pDriver) {
    ...
    if (text->Loaded) {
      ...
      if (text->BuildFromHLSManager) {
        ...
        if (nTotalColored < _MaxHLSColoringPerFrame) {
          ...
          nlverify(
              HLSManager.buildTexture(text->HLSManagerTextId, *text->Texture));
          // Must validate the textureFile generation. NB: little weird since
          // this is not really a textureFile. But it is the easier way to do
          // it.
        ...
        // Else must quit and don't update any more texture this frame
        // (_CurrentUploadTexture==NULL)
        else
          return;
      }
...
void CAsyncTextureManager::updateTextureLodSystem(IDriver *pDriver) {
  ...
  for (i = (sint)lodArray.size() - 1; i >= 0; i--) {
    ...
    // if > max allowed, stop the pivot here. NB: the pivot is included in the
    // "must load them" part.
  ...
  if (currentLoadedSize < currentWantedSize) {
    unload = false;
    // search from end of the list to pivot (included), the first LOD (ie the
    // most important) to load.
    ...
    unload = true;
    // search from start to pivot (exclued), the first LOD (ie the less
    // important) to unload.
  ...
  if (!unload) {
    ...
    textLod->Lod->Texture->setFileName(
        textLod->Lod->TextureEntry->Texture->getFileName());
    ...
    // load it async.
    CAsyncFileManager3D::getInstance().loadTexture(
        textLod->Lod->Texture, &textLod->Lod->Loaded, textLod->Position);
  } else {
    // Swap now the lod.
    nlassert(textLod->Lod->Texture != NULL);
    // Swap the uploaded Driver Handle with the Main texture (ot get the Ugly
    // one)
    pDriver->swapTextureHandle(*textLod->Lod->Texture,
                               *textLod->Lod->TextureEntry->Texture);
    // Flag the Lod.

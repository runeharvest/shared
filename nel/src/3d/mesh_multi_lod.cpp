...
void CMeshMultiLod::build(CMeshMultiLodBuild &mbuild) {
  ...
  for (uint slot = 0; slot < mbuild.LodMeshes.size(); slot++) {
    ...
    // Coarse mesh ?
    if (mbuild.LodMeshes[slot].Flags &
  ...
  for (uint k = 0; k < mbuild.LodMeshes.size(); k++) {
    ...
    if (k == mbuild.LodMeshes.size() - 1) {
      _MeshVector[k].EndPolygonCount =
          _MeshVector[k].MeshGeom->getNumTriangles(endDist);
      if (startPolyCount == _MeshVector[k].EndPolygonCount)
        _MeshVector[k].EndPolygonCount = startPolyCount / 2;
    } else
      _MeshVector[k].EndPolygonCount =
          _MeshVector[k + 1].MeshGeom->getNumTriangles(endDist);

    // Calc A
    if (endDist == startDist)
      _MeshVector[k].A = 0;
    else
      _MeshVector[k].A = (_MeshVector[k].EndPolygonCount - startPolyCount) /
                         (endDist - startDist);

    // Calc A
    _MeshVector[k].B =
        _MeshVector[k].EndPolygonCount - _MeshVector[k].A * endDist;
  }
...
CTransformShape *CMeshMultiLod::createInstance(CScene &scene) {
  ...
  CMeshMultiLodInstance *mi =
      (CMeshMultiLodInstance *)scene.createModel(NL3D::MeshMultiLodInstanceId);
  mi->Shape = this;
...
void CMeshMultiLod::render(IDriver *drv, CTransformShape *trans,
  ...
  if ((instance->Lod1 != 0xffffffff) && (passOpaque == false)) {
    ...
    uint32 rdrFlags =
        IMeshGeom::RenderOpaqueMaterial | IMeshGeom::RenderTransparentMaterial |
        IMeshGeom::RenderGlobalAlpha | IMeshGeom::RenderGADisableZWrite;
    // NB: very important to render Lod1 first, because Lod0 is still rendered
    // with ZWrite enabled.
    renderMeshGeom(instance->Lod1, drv, instance, instance->PolygonCountLod1,
                   rdrFlags, 1.f - instance->BlendFactor, manager);
  }
  ...
  if ((instance->Flags & CMeshMultiLodInstance::Lod0Blend) == 0) {
    ...
    uint32 rdrFlags = IMeshGeom::RenderOpaqueMaterial |
    ...
    renderMeshGeom(instance->Lod0, drv, instance, instance->PolygonCountLod0,
                   rdrFlags, instance->BlendFactor, manager);
  }
}
...
float CMeshMultiLod::getNumTrianglesWithCoarsestDist(
    float distance, float coarsestMeshDist) const {
  ...
  if (meshCount > 0) {
    ...
    if (slot.MeshGeom) {
      ...
      if ( (i<meshCount-1) && _MeshVector[i+1].MeshGeom )
          ...
          realEndPolyCount=_MeshVector[i+1].MeshGeom->getNumTriangles
      (slot.DistMax); else
      ...
      return (goodPolyCount-slot.BeginPolygonCount) *
      (realEndPolyCount-slot.BeginPolygonCount) /
          (slot.EndPolygonCount-slot.BeginPolygonCount) +
      slot.BeginPolygonCount;*/
      return polyCount;
    }
  }
...
void CMeshMultiLod::clear() { _MeshVector.clear(); }
...
void CMeshMultiLod::renderMeshGeom(uint slot, IDriver *drv,
                                   CMeshMultiLodInstance *trans,
                                   float numPoylgons, uint32 rdrFlags,
                                   float alpha, CCoarseMeshManager *manager) {
  // Ref
  CMeshSlot &slotRef = _MeshVector[slot];

  // MeshGeom exist?
  if (slotRef.MeshGeom) {
    // NB Here, the meshGeom may still be a coarseMesh, but rendered through
    // CMeshGeom
    if (slotRef.Flags & CMeshSlot::CoarseMesh) {
      ...
      if (manager && (rdrFlags & IMeshGeom::RenderOpaqueMaterial)) {
        ...
        blender.prepareRenderForGlobalAlphaCoarseMesh(material, drv, newCol,
                                                      alpha, gaDisableZWrite);
        ...
        const_cast<CVertexBuffer &>(meshGeom->getVertexBuffer())
            .setPreferredMemory(CVertexBuffer::RAMPreferred, false);

        meshGeom->renderSimpleWithMaterial(drv, trans->getWorldMatrix(),
                                           material);
...
void CMeshMultiLod::renderCoarseMesh(uint slot, IDriver *drv,
                                     CMeshMultiLodInstance *trans,
                                     CCoarseMeshManager *manager) {
  ...
  if (trans->_LastCoarseMesh != meshGeom) {
    ...
    if (numTris == 0 || numVerts == 0)
      ...
      // Check setuped size.
      nlassert(trans->_CoarseMeshVB.size() >=
               numVerts * manager->getVertexSize());

      // Fill only UVs here. (Pos updated in Matrix pass. Color in Lighting
      // Pass)
      trans->setUVCoarseMesh(*meshGeom, manager->getVertexSize(),
                             manager->getUVOff());

      // Dirt the matrix
      trans->_LastLodMatrixDate = 0;
      // Dirt the lighting. NB: period maximum is 255. Hence the -256, to ensure
      // lighting compute now
  ...
  if (trans->_LastCoarseMesh) {
    ...
    if (trans->ITransformable::compareMatrixDate(trans->_LastLodMatrixDate)) {
      ...
      // Set matrix
      trans->setPosCoarseMesh(*meshGeom, trans->getMatrix(),
                              manager->getVertexSize());
    }

    // Lighting: test if must update lighting, according to date of HrcTrav (num
    // of CScene::render() call).
    sint64 currentDate = scene->getHrcTrav().CurrentDate;
    if (trans->_LastLodLightingDate <
        currentDate - scene->getCoarseMeshLightingUpdate()) {
      // reset the date.
      ...
      // Set color
      trans->setColorCoarseMesh(sunContrib, manager->getVertexSize(),
                                manager->getColorOff());
    }

    // Add dynamic to the manager
    if (!manager->addMesh(trans->_LastCoarseMeshNumVertices,
                          &trans->_CoarseMeshVB[0], slotRef.CoarseNumTris,
                          &slotRef.CoarseTriangles[0])) {
      // If failure, flush the manager
      manager->flushRender(drv);
      // then try to re-add. No-op if fails this time..
      manager->addMesh(trans->_LastCoarseMeshNumVertices,
                       &trans->_CoarseMeshVB[0], slotRef.CoarseNumTris,
                       &slotRef.CoarseTriangles[0]);
    }
  }
...
// ***************************************************************************
void CMeshMultiLod::changeMRMDistanceSetup(float distanceFinest,
  ...
  // ok, setup.
  mgeom->changeMRMDistanceSetup(distanceFinest, distanceMiddle,
                                distanceCoarsest);
}

// ***************************************************************************
IMeshGeom *CMeshMultiLod::supportMeshBlockRendering(CTransformShape *trans,
...
// ***************************************************************************
void CMeshMultiLod::profileMeshGeom(uint slot, CRenderTrav *rdrTrav,
                                    CMeshMultiLodInstance *trans,
                                    float numPoylgons, uint32 rdrFlags) {
  // Ref
  CMeshSlot &slotRef = _MeshVector[slot];
  ...
  if (slotRef.MeshGeom) {
    ...
    if (slotRef.Flags & CMeshSlot::CoarseMesh) {
      ...
      if (rdrFlags & IMeshGeom::RenderOpaqueMaterial) {
        slotRef.MeshGeom->profileSceneRender(rdrTrav, trans, numPoylgons,
                                             rdrFlags);
      }
    } else {
      slotRef.MeshGeom->profileSceneRender(rdrTrav, trans, numPoylgons,
...
// ***************************************************************************
void CMeshMultiLod::profileSceneRender(CRenderTrav *rdrTrav,
  ...
  if ((instance->Lod1 != 0xffffffff) && (passOpaque == false)) {
    ...
    // NB: very important to render Lod1 first, because Lod0 is still rendered
    // with ZWrite enabled.
    profileMeshGeom(instance->Lod1, rdrTrav, instance,
                    instance->PolygonCountLod1, rdrFlags);
  }
  ...
  if ((instance->Flags & CMeshMultiLodInstance::Lod0Blend) == 0) {
    ...
    if (_MeshVector[instance->Lod0].Flags & CMeshSlot::CoarseMesh) {
      ...
      rdrFlags |= ~mask & (IMeshGeom::RenderTransparentMaterial);
      // Only render the normal way the first lod
      profileMeshGeom(instance->Lod0, rdrTrav, instance,
                      instance->PolygonCountLod0, rdrFlags);
    }
  } else {
    // Should not be in opaque
    nlassert(passOpaque == false);
  ...
  if (manager) {
    ...
    for (uint i = 0; i < _MeshVector.size(); i++) {
      ...
      if (slotRef.Flags & CMeshSlot::CoarseMesh) {
        // Get a pointer on the geom mesh
        CMeshGeom *meshGeom = safe_cast<CMeshGeom *>(slotRef.MeshGeom);
        numVertices = max(numVertices,
                          (uint)meshGeom->getVertexBuffer().getNumVertices());
      }
    }
    ...
    if (slotRef.Flags & CMeshSlot::CoarseMesh) {
      ...
      if (meshGeom->getNbMatrixBlock() > 0) {
        ...
        for (uint i = 0; i < meshGeom->getNbRdrPass(0); i++) {
          slotRef.CoarseNumTris +=
              meshGeom->getRdrPassPrimitiveBlock(0, i).getNumIndexes() / 3;
        }
          ...
          for (uint i = 0; i < meshGeom->getNbRdrPass(0); i++) {
            ...
            if (pb.getFormat() == CIndexBuffer::Indices16) {
              if (sizeof(TCoarseMeshIndexType) == sizeof(uint16)) {
                memcpy(dstPtr, (uint16 *)ibaRead.getPtr(),
              ...
              if (sizeof(TCoarseMeshIndexType) == sizeof(uint32)) {
                memcpy(dstPtr, (uint32 *)ibaRead.getPtr(),
                       numTris * 3 * sizeof(uint32));
                dstPtr += numTris * 3;
              } else {
  ...
  if ((_CollisionMeshGeneration == AutoCameraCol && !_LightInfos.empty()) ||
    ...
    if (getNumSlotMesh()) {
      const CMeshGeom *meshGeom =
          dynamic_cast<const CMeshGeom *>(&getMeshGeom(0));
      if (meshGeom) {
        vector<CVector> vertices;
        vector<uint32> indices;
        if (meshGeom->retrieveVertices(vertices) &&
            meshGeom->retrieveTriangles(indices)) {
          // ok, can build!
...
void CMeshMultiLod::buildSystemGeometry() {
  ...
  if (getNumSlotMesh()) {
    ...
    // else it is a mrm geom?
      ...
      const CMeshMRMGeom *meshMRMGeom =
          dynamic_cast<const CMeshMRMGeom *>(&getMeshGeom(0));
      if (meshMRMGeom) {
        ...
        // retrieve geometry (if VB/IB not resident)
        if (!meshMRMGeom->buildGeometryForLod(lodId, _SystemGeometry.Vertices,

...
void CMeshMultiLod::render(IDriver *drv, CTransformShape *trans, bool passOpaque)
{
	...
	if ((instance->Lod1 != 0xffffffff) && (passOpaque == false))
	{
		...
		uint32 rdrFlags = IMeshGeom::RenderOpaqueMaterial | IMeshGeom::RenderTransparentMaterial | IMeshGeom::RenderGlobalAlpha | IMeshGeom::RenderGADisableZWrite;
	...
	else
	{
		...
		uint32 rdrFlags = IMeshGeom::RenderOpaqueMaterial | IMeshGeom::RenderTransparentMaterial | IMeshGeom::RenderGlobalAlpha;
...
void CMeshMultiLod::compileRunTime()
{
	...
	if ((_CollisionMeshGeneration == AutoCameraCol && !_LightInfos.empty()) || _CollisionMeshGeneration == ForceCameraCol)
	{
...
void CMeshMultiLod::buildSystemGeometry()
{
	...
	if (getNumSlotMesh())
	{
		...
		if (meshGeom)
		{
			...
			if (!meshGeom->retrieveVertices(_SystemGeometry.Vertices) || !meshGeom->retrieveTriangles(_SystemGeometry.Triangles))
			{
				_SystemGeometry.clear();
			}
		}

...
// ***************************************************************************
void CMeshMRMSkinnedGeom::CLod::getRdrPassPrimitiveBlock(
    uint renderPass, CIndexBuffer &block) const {
  ...
  uint i;
  for (i = 0; i < count; i++) {
    ibaWrite.setTri(i * 3, rd.PBlock[i * 3 + 0], rd.PBlock[i * 3 + 1],
                    rd.PBlock[i * 3 + 2]);
  }
}
...
void CMeshMRMSkinnedGeom::CLod::buildPrimitiveBlock(uint renderPass,
                                                    const CIndexBuffer &block) {
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::changeMRMDistanceSetup(float distanceFinest,
...
void CMeshMRMSkinnedGeom::build(CMesh::CMeshBuild &m, uint numMaxMaterial,
  ...
  // For each vertices
  uint vert;
  CPackedVertexBuffer::CPackedVertex *vertices =
      _VBufferFinal.getPackedVertices();
    ...
    for (weight = 0; weight < NL3D_MESH_MRM_SKINNED_MAX_MATRIX; weight++) {
      ...
      if ((vertices[vert].Weights[weight] > 0) || (weight == 0)) {
        ...
        std::map<uint, uint>::iterator ite =
            remap.find(vertices[vert].Matrices[weight]);
  ...
  for (lod = 0; lod < _Lods.size(); lod++) {
    ...
    for (matrix = 0; matrix < _Lods[lod].MatrixInfluences.size(); matrix++) {
      ...
      if (ite == remap.end()) {
        // Remove it
        _Lods[lod].MatrixInfluences.erase(_Lods[lod].MatrixInfluences.begin() +
                                          matrix);
        matrix--;
        continue;
      }
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::applyGeomorph(std::vector<CMRMWedgeGeom> &geoms,
                                        float alphaLod) {
  applyGeomorphWithVBHardPtr(geoms, alphaLod);
}

// ***************************************************************************
void CMeshMRMSkinnedGeom::applyGeomorphWithVBHardPtr(
    std::vector<CMRMWedgeGeom> &geoms, float alphaLod) {
  // no geomorphs? quit.
  if (geoms.empty())
    return;
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::applyGeomorphPosNormalUV0(
    std::vector<CMRMWedgeGeom> &geoms, uint8 *vertexPtr, uint8 *vertexDestPtr,
    sint32 vertexSize, float a, float a1) {
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::applyGeomorphPosNormalUV0Int(
    std::vector<CMRMWedgeGeom> &geoms, uint8 *vertexPtr, uint8 *vertexDestPtr,
    sint32 vertexSize, sint a, sint a1) {
  // For all geomorphs.
  uint nGeoms = (uint)geoms.size();
  CMRMWedgeGeom *ptrGeom = &(geoms[0]);
  uint8 *destPtr = vertexDestPtr;
  for (; nGeoms > 0; nGeoms--, ptrGeom++, destPtr += vertexSize) {
    ...
    /* Hulud
     * This is slow but, we don't care because this method is called for debug
     * purpose only (watch skin without skinning)
     */
...
// ***************************************************************************
bool CMeshMRMSkinnedGeom::clip(const std::vector<CPlane> &pyramid,
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::render(IDriver *drv, CTransformShape *trans,
  ...
  if (globalAlphaUsed) {
    ...
    for (uint i = 0; i < lod.RdrPass.size(); i++) {
      ...
      if (((mi->Materials[rdrPass.MaterialId].getBlend() == false) &&
        ...
        /* Hulud
         * This is slow but, we don't care because this method is called for
         * debug purpose only (watch skin without skinning)
         */
        CIndexBuffer block;
...
// ***************************************************************************
bool CMeshMRMSkinnedGeom::supportSkinGrouping() const { return true; }

// ***************************************************************************
sint CMeshMRMSkinnedGeom::renderSkinGroupGeom(CMeshMRMSkinnedInstance *mi,
                                              float alphaMRM,
                                              uint remainingVertices,
                                              uint8 *vbDest) {
  H_AUTO(NL3D_MeshMRMGeom_rdrSkinGrpGeom)

  // since not tested in supportSkinGrouping(), must test _Lods.empty(): no lod,
  // no draw
  if (_Lods.empty())
    return 0;
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::renderSkinGroupPrimitives(
    CMeshMRMSkinnedInstance *mi, uint baseVertex,
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::renderSkinGroupSpecularRdrPass(
    CMeshMRMSkinnedInstance *mi, uint rdrPassId) {
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::updateShiftedTriangleCache(
    CMeshMRMSkinnedInstance *mi, sint curLodId, uint baseVertex) {
  ...
  if (!mi->_ShiftedTriangleCache) {
    ...
    mi->_ShiftedTriangleCache->RawIndices.setFormat(
        NL_SKINNED_MESH_MRM_INDEX_FORMAT);
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::updateSkeletonUsage(CSkeletonModel *sm,
                                              bool increment) {
  // For all Bones used.
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::profileSceneRender(CRenderTrav *rdrTrav,
...
// ***************************************************************************
bool CMeshMRMSkinnedGeom::getSkinBoneBBox(CSkeletonModel *skeleton,
                                          NLMISC::CAABBox &bbox,
                                          uint boneId) const {
  ...
  // for true vertices
  const uint vertexCount = _VBufferFinal.getNumVertices();
  const CPackedVertexBuffer::CPackedVertex *vertices =
      _VBufferFinal.getPackedVertices();
...
bool CMeshMRMSkinned::isCompatible(const CMesh::CMeshBuild &m) {
  /* Optimised shape for skinned object with MRM, 1 UV coordinates, 1 to 4
  skinning weight and 256 matrices Tangeant space, vertex program, mesh block
  rendering and vertex buffer hard are not available. */
...
// ***************************************************************************
void CMeshMRMSkinned::build(CMeshBase::CMeshBaseBuild &mBase,
                            CMesh::CMeshBuild &m,
                            const CMRMParameters &params) {
...
// ***************************************************************************
void CMeshMRMSkinned::build(CMeshBase::CMeshBaseBuild &m,
                            const CMeshMRMSkinnedGeom &mgeom) {
  /// copy MeshBase info: materials ....
...
CTransformShape *CMeshMRMSkinned::createInstance(CScene &scene) {
  ...
  //===============================================
  CMeshMRMSkinnedInstance *mi = (CMeshMRMSkinnedInstance *)scene.createModel(
      NL3D::MeshMRMSkinnedInstanceId);
  mi->Shape = this;
...
// ***************************************************************************
bool CMeshMRMSkinned::clip(const std::vector<CPlane> &pyramid,
                           const CMatrix &worldMatrix) {
  return _MeshMRMGeom.clip(pyramid, worldMatrix);
}

// ***************************************************************************
void CMeshMRMSkinned::render(IDriver *drv, CTransformShape *trans,
                             bool passOpaque) {
  // 0 or 0xFFFFFFFF
...
// ***************************************************************************
void CMeshMRMSkinned::updateSkeletonUsage(CSkeletonModel *skeleton,
                                          bool increment) {
  _MeshMRMGeom.updateSkeletonUsage(skeleton, increment);
}

// ***************************************************************************
void CMeshMRMSkinned::changeMRMDistanceSetup(float distanceFinest,
                                             float distanceMiddle,
                                             float distanceCoarsest) {
...
// ***************************************************************************
IMeshGeom *
CMeshMRMSkinned::supportMeshBlockRendering(CTransformShape *trans,
                                           float &polygonCount) const {
  return NULL;
}

// ***************************************************************************
void CMeshMRMSkinned::profileSceneRender(CRenderTrav *rdrTrav,
                                         CTransformShape *trans,
                                         bool passOpaque) {
  // 0 or 0xFFFFFFFF
  uint32 mask = (0 - (uint32)passOpaque);
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::updateRawSkinNormal(bool enabled,
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::setShadowMesh(
...
// ***************************************************************************
sint CMeshMRMSkinnedGeom::renderShadowSkinGeom(CMeshMRMSkinnedInstance *mi,
                                               uint remainingVertices,
                                               uint8 *vbDest) {
...
// ***************************************************************************
void CMeshMRMSkinnedGeom::renderShadowSkinPrimitives(
    CMeshMRMSkinnedInstance *mi, CMaterial &castMat, IDriver *drv,
    uint baseVertex) {
  nlassert(drv);
...
// ***************************************************************************
bool CMeshMRMSkinnedGeom::intersectSkin(CMeshMRMSkinnedInstance *mi,
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::CPackedVertex::serial(
    NLMISC::IStream &f) {
  // Version
  f.serialVersion(0);
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::CPackedVertex::setNormal(
    const CVector &src) {
  CVector pos = src;
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::CPackedVertex::setPos(
    const NLMISC::CVector &pos, float scaleFactor) {
  CVector _pos = pos;
  _pos /= scaleFactor;
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::CPackedVertex::setUV(float _u,
                                                                    float _v) {
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::CPackedVertex::setWeight(
    uint weightId, float weight) {
...
void CMeshMRMSkinnedGeom::CPackedVertexBuffer::build(
    const CVertexBuffer &buffer,
    const std::vector<CMesh::CSkinWeight> &skinWeight) {
  const uint numVertices = buffer.getNumVertices();
  nlassert(numVertices == skinWeight.size());
  nlassert((buffer.getVertexFormat() &
  ...
  if (numVertices) {
    ...
    // Scale enough ?
    float max_width =
        std::max((float)fabs(_min.x),
                 std::max((float)fabs(_min.y), (float)fabs(_min.z)));
    max_width =
        std::max(max_width,
                 std::max((float)fabs(_max.x),
                          std::max((float)fabs(_max.y), (float)fabs(_max.z))));
...
void CMeshMRMSkinnedGeom::getVertexBuffer(CVertexBuffer &output) const {
  output.setVertexFormat(CVertexBuffer::PositionFlag |
...
void CMeshMRMSkinnedGeom::getSkinWeights(
    std::vector<CMesh::CSkinWeight> &skinWeights) const {
  const uint vertexCount = _VBufferFinal.getNumVertices();
...
uint CMeshMRMSkinnedGeom::NumCacheVertexNormal1 =
    NL_BlockByteL1 / sizeof(CRawVertexNormalSkinned1);
// Number of vertices per block to process with 2 matrix.
uint CMeshMRMSkinnedGeom::NumCacheVertexNormal2 =
    NL_BlockByteL1 / sizeof(CRawVertexNormalSkinned2);
// Number of vertices per block to process with 3 matrix.
uint CMeshMRMSkinnedGeom::NumCacheVertexNormal3 =
    NL_BlockByteL1 / sizeof(CRawVertexNormalSkinned3);
// Number of vertices per block to process with 4 matrix.
uint CMeshMRMSkinnedGeom::NumCacheVertexNormal4 =
    NL_BlockByteL1 / sizeof(CRawVertexNormalSkinned4);

...
void CHeatHaze::performHeatHaze(uint width, uint height, CScene &s,
                                IDriver *drv) {
  ...
  // Compute the right vector. This is done by intersecting the horizon plane
  // with a near plane. to do this, we transform the horizon plane into the view
  // basis.This may be optimized, but is not critical.
  //
  NLMISC::CPlane h;

...
// ***************************************************************************
void CIGSurfaceLight::build(const TRetrieverGridMap &retrieverGridMap,
                            float cellSize, const std::vector<uint> &plRemap) {
...
void CIGSurfaceLight::serial(NLMISC::IStream &f) {
  ...
  /*
  Version 1:
      - The retriever grid map is now a map<uint,CRetrieverLightGrid>. Discard
  compatibility but.
  */
...
// ***************************************************************************
bool CIGSurfaceLight::getStaticLightSetup(
    NLMISC::CRGBA sunAmbient, uint retrieverIdentifier, sint surfaceId,
    const CVector &localPos, std::vector<CPointLightInfluence> &pointLightList,
  ...
  // Else, ok, get it.
  rlg.Grids[surfaceId].getStaticLightSetup(sunAmbient, localPos, pointLightList,
                                           sunContribution, *this,
                                           localAmbient);
  return true;
}

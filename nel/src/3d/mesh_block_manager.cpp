...
// ***************************************************************************
void CMeshBlockManager::addInstance(IMeshGeom *meshGeom,
  ...
  CVBHeapBlock *hb =
      _VBHeapBlocks[meshGeom->_MeshVBHeapId & NL3D_MBM_VBHEAP_HEAP_MASK];
...
// ***************************************************************************
void CMeshBlockManager::flush(IDriver *drv, CScene *scene,
...
// ***************************************************************************
void CMeshBlockManager::render(CVBHeapBlock *vbHeapBlock, IMeshGeom *meshGeom,
  ...
  if (meshGeom->sortPerMaterial()) {
    ...
    for (uint rdrPass = 0; rdrPass < numRdrPass; rdrPass++) {
      ...
      while (instId != -1) {
        ...
        meshGeom->activeInstance(_RenderCtx, instInfo.MBI,
                                 instInfo.PolygonCount, NULL);

        // render the pass.
        meshGeom->renderPass(_RenderCtx, instInfo.MBI, instInfo.PolygonCount,
                             rdrPass);
  ...
  // else sort per instance first
    ...
    while (instId != -1) {
      ...
      bool needVBHeapLock = _RenderCtx.RenderThroughVBHeap &&
                            meshGeom->isActiveInstanceNeedVBFill();
      void *vbDst = NULL;
      if (needVBHeapLock) {
        // Lock the VBHeap
#if 0  // todo hulud remove / restore VBHeap
...
void CMeshBlockManager::freeMeshVBHeap(IMeshGeom *mesh) {
  ...
  uint meshId = (mesh->_MeshVBHeapId & NL3D_MBM_VBHEAP_MESH_MASK) >>
...
// ***************************************************************************
bool CMeshBlockManager::addVBHeap(IDriver *drv, uint vertexFormat,
                                  uint maxVertices) {

...
void CVectorPacker::writeSimpleValue(CBitMemStream &bits, sint32 delta) {
  ...
  if (_LastTag == AbsOrRLE) {
    // nlwarning("writing simple value %d (tag = %d)", (int) (uint16) delta,
    // (int) _LastTag);
  } else {
    // nlwarning("writing simple delta %d (tag = %d)", (int) delta, (int)
    // _LastTag);
  }
  ++_Count[_LastTag];
}
...
// ********************************************************************************************
void CVectorPacker::readSimpleValue(uint32 tag, std::vector<uint16> &v,
                                    CBitMemStream &bits) {
  uint32 croppedDelta;
  switch (tag) {
  case Rel0:
  ...
  // nlwarning("reading simple delta %d (tag = %d)", (int) (v[_ReadIndex] -
  // v[_ReadIndex - 1]), (int) tag);
  ++_ReadIndex;
}

// ********************************************************************************************
void CVectorPacker::flush(CBitMemStream &bits) {
  if (_LastDeltas.empty())
    return;
  if (_LastDeltas.size() > 4) {
    ...
    // nlwarning("begin RLE, length = %d, tag = %d", (int) length, (int)
    // repeatTag);
    bits.serial(length);
...
// ********************************************************************************************
void CVectorPacker::bufferizeDelta(uint32 tag, sint32 delta,
                                   CBitMemStream &bits) {
...
// ********************************************************************************************
void CVectorPacker::serialPackedVector16(std::vector<uint16> &v,
                                         NLMISC::IStream &f,
                                         const CFormat &format) {
  _Format = format;
  if (f.isReading()) {
    ...
    for (uint k = 0; k < v.size(); ++k) {
      if (k == 0) {
        ...
        if (delta >= getMin(_Format.Rel0NumBits) &&
            delta <= getMax(_Format.Rel0NumBits)) {
          bufferizeDelta(Rel0, delta, bits);
        } else if (delta >= getMin(_Format.Rel1NumBits) &&
                   delta <= getMax(_Format.Rel1NumBits)) {
          bufferizeDelta(Rel1, delta, bits);
        } else if (delta >= getMin(_Format.Rel2NumBits) &&
                   delta <= getMax(_Format.Rel2NumBits)) {
    ...
    f.serialCont(datas);
    /*
    if (_Count[Rel0] != 0 || _Count[Rel1] != 0 || _Count[Rel2] != 0 ||
    _Count[AbsOrRLE] != 0)
    {
...
static inline void pushVBLine2D(NLMISC::CVector *&dest,
                                const NLMISC::CVector &v0,
                                const NLMISC::CVector &v1) {
  dest->x = v0.x;
  dest->y = v0.y;
  dest->z = -0.5f;
  ++dest;
...
static inline void pushVBTri2D(NLMISC::CVector *&dest,
                               const NLMISC::CTriangle &tri) {
  dest->x = tri.V0.x;
  dest->y = tri.V0.y;
  dest->z = -0.5f;
  ++dest;
...
static inline void pushVBQuad2D(NLMISC::CVector *&dest,
                                const NLMISC::CQuad &quad) {
  dest->x = quad.V0.x;
  dest->y = quad.V0.y;
  dest->z = -0.5f;
  ++dest;
...
static inline void pushVBQuad(NLMISC::CVector *&dest,
                              const NLMISC::CQuad &quad) {
  *dest++ = quad.V0;
  *dest++ = quad.V1;
  *dest++ = quad.V2;
...
static void computeRastersUnion(const CPolygon2D::TRasterVect &inRaster0,
                                CPolygon2D::TRasterVect &inRaster1, sint minY0,
                                sint minY1, CPolygon2D::TRasterVect &outRaster,
                                sint &finalMinY) {
  if (inRaster0.empty()) {
    if (inRaster1.empty()) {
      outRaster.clear();
  ...
  sint maxY =
      std::max(minY0 + (sint)inRaster0.size(), minY1 + (sint)inRaster1.size());
  ...
  for (sint y = 0; y < (sint)outRaster.size(); ++y) {
    ...
    if (raster0Y >= 0 && raster0Y < (sint)inRaster0.size()) {
      ...
      {
        outRaster[y].first =
            std::min(outRaster[y].first, inRaster0[raster0Y].first);
        outRaster[y].second =
            std::max(outRaster[y].second, inRaster0[raster0Y].second);
      }
    }
    ...
    if (raster1Y >= 0 && raster1Y < (sint)inRaster1.size()) {
      ...
      {
        outRaster[y].first =
            std::min(outRaster[y].first, inRaster1[raster1Y].first);
        outRaster[y].second =
            std::max(outRaster[y].second, inRaster1[raster1Y].second);
      }
    }
  }
}

static void addQuadToSilhouette(const CVector &v0, const CVector &v1,
                                const CVector &v2, CVector &v3,
                                CPolygon2D::TRasterVect &sil, sint &minY,
                                float cellSize) {
...
void CPackedZone32::build(std::vector<const CTessFace *> &leaves,
                          float cellSize, std::vector<CInstanceGroup *> igs,
                          const TShapeCache &shapeCache,
                          const NLMISC::CAABBox &baseZoneBBox, sint32 zoneX,
                          sint32 zoneY) {
  nlassert(cellSize > 0);
  Verts.clear();
  Tris.clear();
  ...
  for (uint k = 0; k < igs.size(); ++k) {
    ...
    for (uint l = 0; l < igs[k]->getNumInstance(); ++l) {
      ...
      if (NLMISC::toLowerAscii(NLMISC::CFile::getExtension(
              igs[k]->getShapeName(l))) == "pacs_prim")
        continue;
      ...
      if (it != shapeCache.end()) {
        CAABBox xformBBox =
            CAABBox::transformAABBox(instanceMatrix, it->second.LocalBBox);
  ...
  TTriListGrid triListGrid; // grid for list of tris per grid cell (before
                            // packing in a single vector)
  ...
  for (uint k = 0; k < instances.size(); ++k) {
    addInstance(*instances[k].SI, instances[k].WorldMat, vertexGrid,
                triListGrid);
  }
  // pack tri lists
  for (uint y = 0; y < height; ++y) {
    for (uint x = 0; x < width; ++x) {
      if (!triListGrid(x, y).empty()) {
        ...
        std::copy(triListGrid(x, y).begin(), triListGrid(x, y).end(),
                  std::back_inserter(TriLists));
  ...
  _PackedLocalToWorld.set(1.f / (_WorldToLocal.x * (float)PACKED_COL_RANGE),
                          1.f / (_WorldToLocal.y * (float)PACKED_COL_RANGE),
                          1.f / (_WorldToLocal.z * (float)PACKED_COL_RANGE));
  // reorder vertices by distance for better compression : start with first
  // vertex then found closest vertex
  //
  /*
  ...
  for(uint k = 0; k < Verts.size(); ++k)
  {
      ...
      else
      {
          ...
          {
              uint32 dist = (uint32) abs((sint32) Verts[todo[l]].X - (sint32)
  lastPos.X)
                            + (uint32) abs((sint32) Verts[todo[l]].Y - (sint32)
  lastPos.Y)
                            + (uint32) abs((sint32) Verts[todo[l]].Z - (sint32)
  lastPos.Z); if (dist < bestDist)
              {
                  bestDist = dist;
                  best = l;
              }
...
// ***************************************************************************************
void CPackedZone32::addTri(const CTriangle &tri, TVertexGrid &vertexGrid,
                           TTriListGrid &triListGrid) {
  CPackedTri pt;
  ...
  //
  polyTri.Vertices[0].set((tri.V0.x - _Origin.x) / CellSize,
                          (tri.V0.y - _Origin.y) / CellSize);
  polyTri.Vertices[1].set((tri.V1.x - _Origin.x) / CellSize,
                          (tri.V1.y - _Origin.y) / CellSize);
  polyTri.Vertices[2].set((tri.V2.x - _Origin.x) / CellSize,
                          (tri.V2.y - _Origin.y) / CellSize);
  //
  ...
  for (sint y = 0; y < (sint)rasters.size(); ++y) {
    ...
    if (gridY < 0)
      continue;
    if (gridY >= (sint)triListGrid.getHeight())
      break;
    for (sint x = rasters[y].first; x <= rasters[y].second; ++x) {
      if (x < 0)
        continue;
      if (x >= (sint)triListGrid.getWidth())
        break;
...
uint32 CPackedZone32::allocVertex(const CVector &src, TVertexGrid &vertexGrid) {
  ...
  if (x == (sint)vertexGrid.getWidth())
    x = (sint)vertexGrid.getWidth() - 1;
  if (y == (sint)vertexGrid.getHeight())
    y = (sint)vertexGrid.getHeight() - 1;
  //
  CPackedVertex pv;
  ...
  for (std::list<uint32>::iterator it = vertexGrid(x, y).begin();
       it != vertexGrid(x, y).end(); ++it) {
    if (Verts[*it] == pv)
...
// ***************************************************************************************
void CPackedZone32::render(CVertexBuffer &vb, IDriver &drv, CMaterial &material,
                           CMaterial &wiredMaterial, const CMatrix &camMat,
                           uint batchSize, const CVector localFrustCorners[8]) {
  if (Tris.empty())
    return;
  ...
  silhouette.clear();
  addQuadToSilhouette(frustCorners[0], frustCorners[1], frustCorners[2],
                      frustCorners[3], silhouette, minY, CellSize);
  addQuadToSilhouette(frustCorners[1], frustCorners[5], frustCorners[6],
                      frustCorners[2], silhouette, minY, CellSize);
  addQuadToSilhouette(frustCorners[4], frustCorners[5], frustCorners[6],
                      frustCorners[7], silhouette, minY, CellSize);
  addQuadToSilhouette(frustCorners[0], frustCorners[4], frustCorners[7],
                      frustCorners[3], silhouette, minY, CellSize);
  addQuadToSilhouette(frustCorners[0], frustCorners[1], frustCorners[5],
                      frustCorners[4], silhouette, minY, CellSize);
  addQuadToSilhouette(frustCorners[3], frustCorners[7], frustCorners[6],
                      frustCorners[2], silhouette, minY, CellSize);
  //
  {
    ...
    for (sint y = 0; y < (sint)silhouette.size(); ++y) {
      ...
      for (sint x = minX; x <= maxX; ++x) {
        if (x < 0)
          continue;
        if (x >= (sint)Grid.getWidth())
          break;
        uint32 triRefIndex = Grid(x, gridY);
        if (triRefIndex == UndefIndex)
          continue;
        for (;;) {
          uint32 triIndex = TriLists[triRefIndex];
          if (triIndex == UndefIndex)
            break; // end of list
...
// ***************************************************************************************
void CPackedZone32::addInstance(const CShapeInfo &si,
                                const NLMISC::CMatrix &matrix,
                                TVertexGrid &vertexGrid,
                                TTriListGrid &triListGrid) {
...
CSmartPtr<CPackedZone16> CPackedZone32::buildPackedZone16() {
  if (Verts.size() > 65535)
    return NULL;
  if (Tris.size() > 65534)
    return NULL; // NB : not 65534 here because -1 is used to mark the end of a
                 // list
  if (TriLists.size() > 65534)
    return NULL;
  // can convert
  ...
  dest->_PackedLocalToWorld.set(
      1.f / (dest->_WorldToLocal.x * (float)PACKED_COL_RANGE),
...
// ***************************************************************************************
void CPackedZone16::render(CVertexBuffer &vb, IDriver &drv, CMaterial &material,
                           CMaterial &wiredMaterial, const CMatrix &camMat,
                           uint batchSize, const CVector localFrustCorners[8]) {
  if (Tris.empty())
    return;
...
void CPackedZone16::unpackTri(const CPackedTri16 &src, CVector dest[3]) const {
  // yes this is ugly code duplication of CPackedZone16::unpackTri but this code
  // is temporary anyway...
  // TODO: add 'multiply-add' operator
  dest[0].set(Verts[src.V0].X * _PackedLocalToWorld.x + _Origin.x,
              Verts[src.V0].Y * _PackedLocalToWorld.y + _Origin.y,
...
template <class T>
bool raytrace(T &packedZone, const NLMISC::CVector &start,
              const NLMISC::CVector &end, NLMISC::CVector &inter,
              std::vector<CTriangle> *testedTriangles = NULL,
              NLMISC::CVector *normal = NULL) {
  if (packedZone.Grid.empty())
    return false;
  CVector2f start2f((start.x - packedZone.Box.getMin().x) / packedZone.CellSize,
                    (start.y - packedZone.Box.getMin().y) /
                        packedZone.CellSize);
  CVector2f dir2f((end.x - start.x) / packedZone.CellSize,
                  (end.y - start.y) / packedZone.CellSize);
  sint x, y;
  CGridTraversal::startTraverse(start2f, x, y);
  do {
    if (x < 0)
      continue;
    if (x >= (sint)packedZone.Grid.getWidth())
      continue;
    if (y < 0)
      continue;
    if (y >= (sint)packedZone.Grid.getHeight())
      continue;
    ...
    if (triListIndex != T::UndefIndex) {
      ...
      CVector currInter;
      do {
        packedZone.unpackTri(packedZone.Tris[packedZone.TriLists[triListIndex]],
                             &tri.V0);
        if (testedTriangles) {
...
// ***************************************************************************************
bool CPackedZone32::raytrace(const NLMISC::CVector &start,
                             const NLMISC::CVector &end, NLMISC::CVector &inter,
                             std::vector<CTriangle> *testedTriangles /*= NULL*/,
                             NLMISC::CVector *normal /*= NULL*/) const {
  return NL3D::raytrace(*this, start, end, inter, testedTriangles, normal);
}

// ***************************************************************************************
bool CPackedZone16::raytrace(const NLMISC::CVector &start,
                             const NLMISC::CVector &end, NLMISC::CVector &inter,
                             std::vector<CTriangle> *testedTriangles /*= NULL*/,
                             NLMISC::CVector *normal /*= NULL*/) const {
  return NL3D::raytrace(*this, start, end, inter, testedTriangles, normal);
...
// ***************************************************************************************
void CPackedZone16::appendSelection(
  ...
  for (uint k = 0; k < localPoly.Vertices.size(); ++k) {
    localPoly.Vertices[k].x =
        (localPoly.Vertices[k].x - Box.getMin().x) / CellSize;
    localPoly.Vertices[k].y =
        (localPoly.Vertices[k].y - Box.getMin().y) / CellSize;
  }
...
// ***************************************************************************************
void CPackedZone32::appendSelection(
    const NLMISC::CPolygon2D &poly,
    std::vector<NLMISC::CTriangle> &selectedTriangles) const {

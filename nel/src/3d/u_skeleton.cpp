...
bool USkeleton::bindSkin(UInstance mi) {
  ...
  if (meshi == NULL) {
    nlerror(
        "USkeleton::bindSkin(): mi is not a MeshInstance or MeshMRMInstance");
    return false;
  }
  CSkeletonModel *object = getObjectPtr();
...
void USkeleton::changeMRMDistanceSetup(float distanceFinest,
                                       float distanceMiddle,
                                       float distanceCoarsest) {
  NL3D_HAUTO_UI_SKELETON;
  CSkeletonModel *object = getObjectPtr();
  object->changeMRMDistanceSetup(distanceFinest, distanceMiddle,
                                 distanceCoarsest);
}

// ***************************************************************************

bool USkeleton::computeRenderedBBox(NLMISC::CAABBox &bbox,
                                    bool computeInWorld) {
  NL3D_HAUTO_UI_SKELETON;
  CSkeletonModel *object = getObjectPtr();
...
bool USkeleton::computeCurrentBBox(NLMISC::CAABBox &bbox, UPlayList *playList,
                                   double playTime,

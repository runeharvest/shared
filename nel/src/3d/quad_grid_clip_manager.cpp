...
void CQuadGridClipManager::registerBasic() {
  CScene::registerModel(QuadGridClipManagerId, TransformId,
...
// ***************************************************************************
CQuadGridClipManager::~CQuadGridClipManager() { reset(); }

// ***************************************************************************
void CQuadGridClipManager::init(float clusterSize, uint numDist, float maxDist,
                                float radiusMax) {
  // reset first.
  reset();
...
void CQuadGridClipManager::updateClustersFromCamera(const CVector &camPos) {
  ...
  // keep an histeresis of one cluster: do not delete "young" clusters created
  // before.
  ...
  if (newY0 >= oldY0 + 1) // same reasoning.
    newY0--;
  if (newX1 <=
      oldX1 - 1) // NB: if newX1==oldX1-1, then newX1= oldX1 => no change.
    newX1++;
  ...
  if (newX0 != oldX0 || newX1 != oldX1 || newY0 != oldY0 || newY1 != oldY1) {
    ...
    for (y = newY0; y < newY1; y++) {
      for (x = newX0; x < newX1; x++) {
        CQuadGridClipCluster *&newCase =
            newQuadGridClusterCases[(y - newY0) * newWidth + (x - newX0)];
        ...
        if (x < oldX0 || x >= oldX1 || y < oldY0 || y >= oldY1) {
          ...
          // copy from old.
          CQuadGridClipCluster *oldCase =
              _QuadGridClusterCases[(y - _Y) * _Width + (x - _X)];
...
bool CQuadGridClipManager::linkModel(CTransformShape *pTfmShp) {
  ...
  if (x >= _X && x < _X + _Width && y >= _Y && y < _Y + _Height) {
    ...
    CQuadGridClipCluster *cluster =
        _QuadGridClusterCases[(y - _Y) * _Width + (x - _X)];

    // if this cluster is empty, add it now to the list of not empty (do the
    // test before add)
    if (cluster->isEmpty()) {
...
// ***************************************************************************
void CQuadGridClipManager::deleteCaseModels(CClipTrav *pClipTrav, sint x,
                                            sint y) {
...
// ***************************************************************************
void CQuadGridClipManager::newCaseModels(CQuadGridClipCluster *&cluster,
                                         const NLMISC::CAABBox &pivotBbox) {
...
void CQuadGridClipManager::profile() const {
  ...
  for (sint y = 0; y < _Height; y++) {
    for (sint x = 0; x < _Width; x++) {
      const CQuadGridClipCluster *cluster =
          _QuadGridClusterCases[y * _Width + x];
      if (cluster->isEmpty())
        numEmptyClusters++;
      else {
        CAABBox bb = cluster->getBBox();
        float area =
            (bb.getSize().x * bb.getSize().y) / (_ClusterSize * _ClusterSize);
        meanAreaRatio += area;
  ...
  if (numEmptyClusters == _Width * _Height) {
    ...
    nlinfo("    This Level has %d clusters not empty over %d", numClusters,
           _Width * _Height);
    ...
    for (uint lvl = 0; lvl < _NumDist + 1; lvl++) {
      nlinfo("    * Stats for Distance up to %f m",
             lvl < _NumDist ? (lvl + 1) * _MaxDist / _NumDist : -1.0f);

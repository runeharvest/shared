...
void CNoise3d::init(uint32 w, uint32 h, uint32 d)
{
	...
	if (_IsDriverSupportCloudSinglePass)
	{
		...
		_VertexBuffer.setVertexFormat(CVertexBuffer::PositionFlag | CVertexBuffer::PrimaryColorFlag | CVertexBuffer::TexCoord0Flag | CVertexBuffer::TexCoord1Flag);
	}
	else
	{
		_Mat = new CMaterial();
		...
		_Mat->setZFunc(CMaterial::always);
		_Mat->setZWrite(false);
		_VertexBuffer.setVertexFormat(CVertexBuffer::PositionFlag | CVertexBuffer::PrimaryColorFlag | CVertexBuffer::TexCoord0Flag);
	}
}

...
void CNoise3d::init(uint32 w, uint32 h, uint32 d) {
  ...
  _Tex = new CTextureMem(_Mem, _Width * _NbSliceW * _Height * _NbSliceH, true,
                         false, _Width * _NbSliceW, _Height * _NbSliceH,
                         CBitmap::Alpha);
...
void CNoise3d::render2passes(CQuadUV &qc, float wpos, float alpha) {
  // For the moment we do it in 2 passes : because wpos is a position between
  // slice we have to do : [ At0*wpos+At1*(1-wpos) ] * alpha this is done like
  // that : At0*[wpos*alpha] + At1*[(1-wpos)*alpha]
  ...
  *pUV = CUV(qc.Uv0.U * _ScaleW + _OffS[nSlice1].U,
             qc.Uv0.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv1.U * _ScaleW + _OffS[nSlice1].U,
             qc.Uv1.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv2.U * _ScaleW + _OffS[nSlice1].U,
             qc.Uv2.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv3.U * _ScaleW + _OffS[nSlice1].U,
             qc.Uv3.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);

  *pUV = CUV(qc.Uv0.U * _ScaleW + _OffS[nSlice2].U,
             qc.Uv0.V * _ScaleH + _OffS[nSlice2].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv1.U * _ScaleW + _OffS[nSlice2].U,
             qc.Uv1.V * _ScaleH + _OffS[nSlice2].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv2.U * _ScaleW + _OffS[nSlice2].U,
             qc.Uv2.V * _ScaleH + _OffS[nSlice2].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv3.U * _ScaleW + _OffS[nSlice2].U,
             qc.Uv3.V * _ScaleH + _OffS[nSlice2].V);
...
void CNoise3d::render(CQuadUV &qc, float wpos, float intensity) {
  ...
  *pVertices = qc.V3;
  ...
  CUV *pUV = vba.getTexCoordPointer(_NbVertices, 0);
  *pUV = CUV(qc.Uv0.U / _NbSliceW + _OffS[nSlice1].U,
             qc.Uv0.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv1.U / _NbSliceW + _OffS[nSlice1].U,
             qc.Uv1.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv2.U / _NbSliceW + _OffS[nSlice1].U,
             qc.Uv2.V * _ScaleH + _OffS[nSlice1].V);
  pUV = (CUV *)(((uint8 *)pUV) + nVSize);
  *pUV = CUV(qc.Uv3.U / _NbSliceW + _OffS[nSlice1].U,
             qc.Uv3.V * _ScaleH + _OffS[nSlice1].V);
...
void CNoise3d::renderGrid(uint32 nbw, uint32 nbh, uint32 w, uint32 h,
                          float UStart, float VStart, float WStart, float dU,
                          float dV, float dW, float intensity) {

  if (!_IsDriverSupportCloudSinglePass) {
    renderGrid2passes(nbw, nbh, w, h, UStart, VStart, WStart, dU, dV, dW,
                      intensity);
    return;
  }
  ...
  for (j = 0; j < nbh; ++j) {
    for (i = 0; i < nbw; ++i) {
      ...
      // If wpos is just on slice1 alpha must be one
      nAlphaPos =
          (uint8)(255 * (1.0f - _Depth * (wpos - (((float)nSlice1) / _Depth))));

      *pVertices = CVector((float)i * w, (float)j * h, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
      *pVertices = CVector((float)(i + 1) * w, (float)j * h, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
      *pVertices = CVector((float)(i + 1) * w, (float)(j + 1) * h, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);
      *pVertices = CVector((float)i * w, (float)(j + 1) * h, 0.0f);
      pVertices = (CVector *)(((uint8 *)pVertices) + nVSize);

      pUV0->U = UStart + _OffS[nSlice1].U;
      pUV0->V = VStart + _OffS[nSlice1].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = dU + _OffS[nSlice1].U;
      pUV0->V = VStart + _OffS[nSlice1].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = dU + _OffS[nSlice1].U;
      pUV0->V = dV + _OffS[nSlice1].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = UStart + _OffS[nSlice1].U;
      pUV0->V = dV + _OffS[nSlice1].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);

      pUV1->U = UStart + _OffS[nSlice2].U;
      pUV1->V = VStart + _OffS[nSlice2].V;
      pUV1 = (CUV *)(((uint8 *)pUV1) + nVSize);
      pUV1->U = dU + _OffS[nSlice2].U;
      pUV1->V = VStart + _OffS[nSlice2].V;
      pUV1 = (CUV *)(((uint8 *)pUV1) + nVSize);
      pUV1->U = dU + _OffS[nSlice2].U;
      pUV1->V = dV + _OffS[nSlice2].V;
      pUV1 = (CUV *)(((uint8 *)pUV1) + nVSize);
      pUV1->U = UStart + _OffS[nSlice2].U;
      pUV1->V = dV + _OffS[nSlice2].V;
      pUV1 = (CUV *)(((uint8 *)pUV1) + nVSize);
...
void CNoise3d::renderGrid2passes(uint32 nbw, uint32 nbh, uint32 w, uint32 h,
                                 float UStart, float VStart, float WStart,
  ...
  for (j = 0; j < nbh; ++j) {
    for (i = 0; i < nbw; ++i) {
      ...
      *pVertices = CVector((float)i * w, (float)(j + 1) * h, 0.0f);
      ...
      pUV0->U = UStart + _OffS[nSlice2].U;
      pUV0->V = VStart + _OffS[nSlice2].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = dU + _OffS[nSlice2].U;
      pUV0->V = VStart + _OffS[nSlice2].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = dU + _OffS[nSlice2].U;
      pUV0->V = dV + _OffS[nSlice2].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
      pUV0->U = UStart + _OffS[nSlice2].U;
      pUV0->V = dV + _OffS[nSlice2].V;
      pUV0 = (CUV *)(((uint8 *)pUV0) + nVSize);
...
// Accessors
// ------------------------------------------------------------------------------------------------
uint32 CNoise3d::getWidth() { return _Width; }

// ------------------------------------------------------------------------------------------------
uint32 CNoise3d::getHeight() { return _Height; }

// ------------------------------------------------------------------------------------------------
uint32 CNoise3d::getDepth() { return _Depth; }

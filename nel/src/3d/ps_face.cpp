...
public:
  ...
  static void drawFaces(T posIt, U indexIt, CPSFace &f, uint size,
                        uint32 srcStep) {
    ...
    if (f._PrecompBasis.size()) // do we use precomputed basis ?
    {
      do {
        {
          toProcess = leftFaces > (uint32)CPSQuad::quadBufSize
          ...
          if (f._SizeScheme) {
            ptSize = (float *)(f._SizeScheme->make(f._Owner, size - leftFaces,
      ...
      static CPlaneBasis
          planeBasis[CPSQuad::quadBufSize]; // buffer to compute each particle
                                            // basis
      ...
      do {
        {
          ...
          if (f._PlaneBasisScheme) {
            currBasis = (CPlaneBasis *)(f._PlaneBasisScheme->make(
                f._Owner, size - leftFaces, planeBasis, sizeof(CPlaneBasis),
                toProcess, true, srcStep));
          } else {
          ...
          endPosIt = posIt + toProcess;
          do {
            // we use this instead of the + operator, because we avoid 4
            // constructor calls this way
            CHECK_VERTEX_BUFFER(vb, currVertex);
            ((CVector *)currVertex)->x = (*posIt).x + *ptSize * currBasis->X.x;
            ((CVector *)currVertex)->y = (*posIt).y + *ptSize * currBasis->X.y;
            ((CVector *)currVertex)->z = (*posIt).z + *ptSize * currBasis->X.z;
...
void CPSFace::step(TPSProcessPass pass) {
  ...
  if (pass == PSToolRender) // edition mode only
  {
    ...
    if (!_PrecompBasis.empty()) // do we use precomputed basis ?
    {
      ...
      for (CPSVector<CPlaneBasisPair>::V::iterator it = _PrecompBasis.begin();
        ...
        // not optimized at all, but this will apply to very few elements
        // anyway...
        CMatrix mat;
        mat.rotate(CQuat(it->Axis,
                         CParticleSystem::EllapsedTime * it->AngularVelocity));
  ...
  if (step == (1 << 16)) {
    ...
    /// draw the faces
    CPSFaceHelper::drawFaces(_Owner->getPos().begin(), indexIt, *this,
                             numToProcess, step);
  } else {
    /// build index iterator
    CAdvance1616Iterator<CPSVector<uint32>::V::const_iterator, const uint32>
        indexIt(_IndexInPrecompBasis.begin(), 0, step);
    CPSFaceHelper::drawFaces(
        TIteratorVectStep1616(_Owner->getPos().begin(), 0, step), indexIt,

...
typedef std::vector<CVectInfo>
    TRibbonVect; // a vector used for intermediate computations
...
//=======================================================
CPSRibbonLookAt::CPSRibbonLookAt(){NL_PS_FUNC(CPSRibbonLookAt_CPSRibbonLookAt)}
...
static inline void
BuildSlice(const NLMISC::CMatrix &mat, CVertexBuffer &vb, uint8 *currVert,
           uint32 vertexSize, const NLMISC::CVector &I,
           const NLMISC::CVector &K, TRibbonVect::iterator pos,
           TRibbonVect::iterator prev, TRibbonVect::iterator next,
           float ribSize)
/// TODO: some optimisation to get a better speed
{
  ...
  if (prev->Proj.y > ZEpsilon &&
      next->Proj.y > ZEpsilon) // the 2 points are in front of the camera
  {
    ...
    *(NLMISC::CVector *)currVert =
        pos->Interp + ribSize * invTgNorm * (tangent.x * K - tangent.z * I);
  ...
  } else if (prev->Proj.y > ZEpsilon) // second point cross the near plane
  {
    ...
    *(NLMISC::CVector *)currVert =
        inter + ribSize * invTgNorm * (tangent.x * K - tangent.z * I);
    *(NLMISC::CVector *)(currVert + vertexSize) =
        inter + ribSize * invTgNorm * (-tangent.x * K + tangent.z * I);
...
void CPSRibbonLookAt::displayRibbons(uint32 nbRibbons, uint32 srcStep) {
  ...
  do {
    ...
    if (_ColorScheme) {
      ...
      ptCurrColor = (NLMISC::CRGBA *)_ColorScheme->make(
          this->_Owner, ribbonIndex, &colors[0], sizeof(NLMISC::CRGBA),
    ...
    VB.setNumVertices(2 * (_UsedNbSegs + 1) * toProcess);
    {
      ...
      for (uint k = ribbonIndex; k < ribbonIndex + toProcess; ++k) {
        ...
        TRibbonVect::iterator rIt = currRibbon.begin(),
                              rItEnd = currRibbon.end(),
                              rItEndMinusOne = rItEnd - 1;
        ...
        if (!_Parametric) {
          ...
          computeRibbon((uint)(fpRibbonIndex >> 16), &rIt->Interp,
                        sizeof(CVectInfo));
          do {
          ...
          _Owner->integrateSingle(date - _UsedSegDuration * (_UsedNbSegs + 1),
                                  ...
                                  (uint)(fpRibbonIndex >> 16), &rIt->Interp,
                                  sizeof(CVectInfo));
        ...
        BuildSlice(mat, VB, currVert, vertexSize, I, K, rIt, rIt, rIt + 1,
                   *ptCurrSize);
        ...
        for (;
             ;) // we assume at least 2 segments, so we must have a middle point
        {
          // build 2 vertices with the right tangent. /* to project 2 */ is old
          // projected point
          BuildSlice(mat, VB, currVert, vertexSize, I, K, rIt, rIt - 1, rIt + 1,
                     *ptCurrSize);
          // next position
          ++rIt;
          if (rIt == rItEndMinusOne)
            break;
          // next vertex
          currVert += vertexSizeX2;
        }
        currVert += vertexSizeX2;
        // last point.
        BuildSlice(mat, VB, currVert, vertexSize, I, K, rIt, rIt - 1, rIt,
                   *ptCurrSize);
    ...
    for (uint i = 0; i < numRibbonInVB; ++i) {
      for (uint k = 0; k < (_UsedNbSegs + 1); ++k) {
        vba.setTexCoord(vbIndex, 0,
                        CUV((1.f - k / (float)_UsedNbSegs), 0)); /// top vertex
        vba.setTexCoord(
            vbIndex + 1, 0,
            CUV((1.f - k / (float)_UsedNbSegs), 1)); /// bottom vertex
        if (k != _UsedNbSegs) {
...
//==========================================================================
void CPSRibbonLookAt::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                               IDriver &drv) {

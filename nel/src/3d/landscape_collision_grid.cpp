...
CLandscapeCollisionGrid::CLandscapeCollisionGrid(
    CVisualCollisionManager *owner) {
  _Owner = owner;
  // reset list to NULL.
  memset(_Grid, 0,
         NL_COLGRID_SIZE * NL_COLGRID_SIZE * sizeof(CVisualTileDescNode *));
  _Cleared = true;
...
// ***************************************************************************
CLandscapeCollisionGrid::~CLandscapeCollisionGrid() { clear(); }
...
// ***************************************************************************
void CLandscapeCollisionGrid::build(
    const std::vector<CPatchQuadBlock *> &quadBlocks, const CVector &delta) {
  sint x, y;
  static CVector2i
      floorVals[NL_PATCH_BLOCK_MAX_VERTEX * NL_PATCH_BLOCK_MAX_VERTEX];
...
// ***************************************************************************
void CLandscapeCollisionGrid::addQuadToGrid(uint16 paBlockId, uint16 quadId,
                                            sint x0, sint x1, sint y0,
                                            sint y1) {
...
// ***************************************************************************
CVisualTileDescNode *
CLandscapeCollisionGrid::select(const NLMISC::CVector &pos) {

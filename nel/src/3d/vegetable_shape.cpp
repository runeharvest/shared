...
void CVegetableShape::build(CVegetableShapeBuild &vbuild) {
  ...
  // Lighted ?
  if (vbuild.Lighted &&
      (vbuild.VB.getVertexFormat() & CVertexBuffer::NormalFlag))
    Lighted = true;
  else
    Lighted = false;
  ...
  // AlphaBlend: valid only for 2Sided and Unlit (or similar PreComputeLighting)
  // mode
  AlphaBlend =
      vbuild.AlphaBlend && DoubleSided && (!Lighted || PreComputeLighting);

  // BestSidedPreComputeLighting
  BestSidedPreComputeLighting =
      PreComputeLighting && vbuild.BestSidedPreComputeLighting;
  ...
  // Format of the VB.
  uint32 format;
  format = CVertexBuffer::PositionFlag | CVertexBuffer::TexCoord0Flag |

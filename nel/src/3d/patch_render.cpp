...
// ***************************************************************************
void CPatch::computeNewFar(const NLMISC::CBSphere &patchSphere, sint &newFar0,
                           sint &newFar1) {
  // Classify the patch.
  //========================
  float r = (CLandscapeGlobals::RefineCenter - patchSphere.Center).norm() -
            patchSphere.Radius;
  float rr = 0.0;
  ...
  newFar1 = 0;
  if (newFar0 < 3 &&
      rr > -(CLandscapeGlobals::FarTransition + 2 * patchSphere.Radius)) {
    newFar1 = newFar0 + 1;
  }
    ...
    if (newFar0 != Far0) {
      ...
      if (newFar0 > 0) {
        ...
        } else                              // get a new render pass
        {
          // Rotation boolean
          bool bRot;
          _PatchRdrPassFar0 = Zone->Landscape->getFarRenderPass(
              this, newFar0, Far0UScale, Far0VScale, Far0UBias, Far0VBias,
              bRot);
    ...
    if (newFar1 != Far1) {
      ...
      if (newFar1 > 0) {
        ...
        } else                              // get a new render pass
        {
          // Rotation boolean
          bool bRot;
          _PatchRdrPassFar1 = Zone->Landscape->getFarRenderPass(
              this, newFar1, Far1UScale, Far1VScale, Far1UBias, Far1VBias,
              bRot);
          nlassert(_PatchRdrPassFar1);
        ...
        switch (newFar1) {
        case 1:
          farDist = CLandscapeGlobals::TileDistNear;
          break;
        case 2:
          farDist = CLandscapeGlobals::Far0Dist;
          break;
        case 3:
          farDist = CLandscapeGlobals::Far1Dist;
          break;
        default:
          nlstop;
        };
...
void CPatch::preRender(const NLMISC::CBSphere &patchSphere) {
  ...
  if (changeTileMode) {
    ...
    // NB: Far0 and Far1 are still unmodified, so deleteVB() will do the good
    // job.
  ...
  for (; nTessBlock > 0; pTessBlock++, nTessBlock--) {
    ...
    if (tblock.FaceTileMaterialRefCount == 0) {
      ...
      // If we are in Tile/FarTransition
      if (doClipFar)
        tblock.clipFar(CLandscapeGlobals::RefineCenter,
  ...
  */
  ...
  if (fv) {
    ...
    */
    ...
    CHECK_IBA_RANGE(CLandscapeGlobals::PassTriArrayIBA,
...
#ifndef NL_LANDSCAPE_INDEX16
    memcpy(NL3D_LandscapeGlobals_PassTriCurPtr, fv + 1,
           nTriIndex * sizeof(uint32));
    NL3D_LandscapeGlobals_PassTriCurPtr =
        (uint32 *)NL3D_LandscapeGlobals_PassTriCurPtr + nTriIndex;
#else
    memcpy(NL3D_LandscapeGlobals_PassTriCurPtr, fv + 1,
           nTriIndex * sizeof(uint16));
    NL3D_LandscapeGlobals_PassTriCurPtr =
        (uint16 *)NL3D_LandscapeGlobals_PassTriCurPtr + nTriIndex;
#endif
...
void CTileMaterial::renderTile(uint pass) {
  // because precisely inserted in preRender(), and correctly tested, this tile
  // is to be rendered, If the pass is enabled.
  if (Pass[pass].PatchRdrPass) {
    // render tris of the good faceList.
    renderFaceVector(TileFaceVectors[pass]);

    // profile.
    NL3D_PROFILE_LAND_ADD_FACE_VECTOR(ProfNRdrTile[pass],
                                      TileFaceVectors[pass]);
  }
}
...
void CTileMaterial::renderTilePassRGB0() {
  // because precisely inserted in preRender(), and correctly tested, this tile
  // is to be rendered, this pass must be enabled!
...
void CTileMaterial::renderTilePassLightmap() {
  ...
  // render tris of the good faceList, ie the one of PassRGB0, because vertices
  // are reused.
  renderFaceVector(TileFaceVectors[NL3D_TILE_PASS_RGB0]);

  // profile.
  NL3D_PROFILE_LAND_ADD_FACE_VECTOR(ProfNRdrTile[NL3D_TILE_PASS_LIGHTMAP],
                                    TileFaceVectors[NL3D_TILE_PASS_RGB0]);
}
...
void CPatch::recreateTessBlockFaceVector(CTessBlock &block) {
  ...
  if (!isRenderClipped()) {
    ...
    // Or If Tile Mode.
      ...
      // No tiles in MasterBlock! So no need to call createFaceVectorTile(), if
      // this block is the MasterBlock.
...
// ***************************************************************************
void CPatch::updateFar0VBAlloc(CTessList<CTessFarVertex> &vertList,
                               bool alloc) {
  ...
  for (pVert = vertList.begin(); pVert; pVert = (CTessFarVertex *)pVert->Next) {
    if (alloc)
      pVert->Index0 =
          CLandscapeGlobals::CurrentFar0VBAllocator->allocateVertex();
    else
...
// ***************************************************************************
void CPatch::updateFar1VBAlloc(CTessList<CTessFarVertex> &vertList,
                               bool alloc) {
  // Traverse the vertList.
  CTessFarVertex *pVert;
  for (pVert = vertList.begin(); pVert; pVert = (CTessFarVertex *)pVert->Next) {
    if (alloc)
      pVert->Index1 =
          CLandscapeGlobals::CurrentFar1VBAllocator->allocateVertex();
    else
...
// ***************************************************************************
void CPatch::updateTileVBAlloc(CTessList<CTessNearVertex> &vertList,
                               bool alloc) {
  // Traverse the vertList.
  CTessNearVertex *pVert;
  for (pVert = vertList.begin(); pVert;
       pVert = (CTessNearVertex *)pVert->Next) {
    if (alloc)
      pVert->Index =
          CLandscapeGlobals::CurrentTileVBAllocator->allocateVertex();
    else
...
// ***************************************************************************
void CPatch::debugAllocationMarkIndicesFarList(
    CTessList<CTessFarVertex> &vertList, uint marker) {
  // Traverse the vertList.
  CTessFarVertex *pVert;
  for (pVert = vertList.begin(); pVert; pVert = (CTessFarVertex *)pVert->Next) {
    pVert->Index0 = marker;
...
void CPatch::debugAllocationMarkIndicesNearList(
    CTessList<CTessNearVertex> &vertList, uint marker) {
  // Traverse the vertList.
  CTessNearVertex *pVert;
  for (pVert = vertList.begin(); pVert;
       pVert = (CTessNearVertex *)pVert->Next) {
...
inline void CPatch::fillFar0VertexVB(CTessFarVertex *pVert) {
  ...
  if (!CLandscapeGlobals::VertexProgramEnabled) {
    ...
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor, CurVBPtr,
                    sizeof(CVector));
    *(CVector *)CurVBPtr =
        pVert->Src->Pos - CLandscapeGlobals::PZBModelPosition;
    // Set Uvs.
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff0,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff0) = uv;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff1,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff1) =
        uvDLM;
  } else {
    ...
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff1) =
    ...
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.GeomInfoOff,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.GeomInfoOff) =
        geomInfo;

    // v[11]== EndPos - StartPos
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentFar0VBInfo.DeltaPosOff,
                    sizeof(CVector))
...
inline void CPatch::fillFar1VertexVB(CTessFarVertex *pVert) {
  ...
  if (!CLandscapeGlobals::VertexProgramEnabled) {
    // Set Pos. Set it local to the current center of landscape
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor, CurVBPtr,
                    sizeof(CVector));
    *(CVector *)CurVBPtr =
        pVert->Src->Pos - CLandscapeGlobals::PZBModelPosition;
    // Set Uvs.
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentFar1VBInfo.TexCoordOff0,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.TexCoordOff0) = uv;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentFar1VBInfo.TexCoordOff1,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.TexCoordOff1) =
        uvDLM;
    // Set default color.
    static CRGBA col(255, 255, 255, 255);
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.ColorOff,
                    sizeof(CRGBA));
    ...
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.TexCoordOff1) =
    ...
    geomInfo.V = pVert->Src->MaxNearLimit;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.GeomInfoOff,
                    sizeof(CUV));
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.GeomInfoOff) =
        geomInfo;

    // v[11]== EndPos - StartPos
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.DeltaPosOff,
                    sizeof(CVector))
    *(CVector *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.DeltaPosOff) =
        pVert->Src->EndPos - pVert->Src->StartPos;

    // v[12]== Alpha information
    // Hopefully, fillVBFar1Only() is called each Time the Far1 change, in
    // preRender(). So TransitionSqrMin and OOTransitionSqrDelta in CPath are
    // valid.
    geomInfo.U = TransitionSqrMin;
    geomInfo.V = OOTransitionSqrDelta;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar1VBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentFar1VBInfo.AlphaInfoOff,
                    sizeof(CUV))
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentFar1VBInfo.AlphaInfoOff) =
        geomInfo;
  }
}
// ***************************************************************************
...
inline void CPatch::fillTileVertexVB(CTessNearVertex *pVert) {
  ...
  if (!CLandscapeGlobals::VertexProgramEnabled) {
    // Set Pos. Set it local to the current center of landscape
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor, CurVBPtr,
                    sizeof(CVector))
    *(CVector *)CurVBPtr =
        pVert->Src->Pos - CLandscapeGlobals::PZBModelPosition;
    // Set Uvs.
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff0,
                    sizeof(CUV))
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff0) =
        pVert->PUv0;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff1,
                    sizeof(CUV))
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff1) =
        pVert->PUv1;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor,
                    CurVBPtr +
                        CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff2,
                    sizeof(CUV))
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff2) =
        pVert->PUv2;
  } else {
    // Else must setup Vertex program inputs
    // v[0]== StartPos.
    ...
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.TexCoordOff2) =
    ...
    geomInfo.V = pVert->Src->MaxNearLimit;
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.GeomInfoOff,
                    sizeof(CUV))
    *(CUV *)(CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.GeomInfoOff) =
        geomInfo;

    // v[11]== EndPos - StartPos
    CHECK_VBA_RANGE(CLandscapeGlobals::CurrentTileVBInfo.Accessor,
                    CurVBPtr + CLandscapeGlobals::CurrentTileVBInfo.DeltaPosOff,
                    sizeof(CVector))
...
void CPatch::fillVB() {
  ...
  if (Far0 > 0 &&
      !CLandscapeGlobals::CurrentFar0VBAllocator->reallocationOccurs()) {
    // Fill Far0 VB.
    ...
    for (sint i = 0; i < (sint)TessBlocks.size(); i++) {
      ...
      if (tblock.visibleFar0())
        fillFar0VertexListVB(tblock.FarVertexList);
             ...
             !CLandscapeGlobals::CurrentTileVBAllocator->reallocationOccurs()) {
    // Fill Tile VB.
    // No Tiles in MasterBlock!!
    // Traverse the TessBlocks to add vertices.
    for (sint i = 0; i < (sint)TessBlocks.size(); i++) {
  ...
  //=======
  if (Far1 > 0 &&
      !CLandscapeGlobals::CurrentFar1VBAllocator->reallocationOccurs()) {
    // Fill VB.
...
// ***************************************************************************
void CPatch::computeGeomorphFar0VertexListVB(
    CTessList<CTessFarVertex> &vertList) {
  // Traverse the vertList.
  CTessFarVertex *pVert;
  for (pVert = vertList.begin(); pVert; pVert = (CTessFarVertex *)pVert->Next) {
    static uint8 *CurVBPtr;
...
// ***************************************************************************
void CPatch::computeGeomorphAlphaFar1VertexListVB(
    CTessList<CTessFarVertex> &vertList) {
  // Traverse the vertList.
  CTessFarVertex *pVert;
  for (pVert = vertList.begin(); pVert; pVert = (CTessFarVertex *)pVert->Next) {
    ...
    *(CVector *)CurVBPtr =
    ...
    // Prefer Use Pos, because of caching. So little difference between Soft and
    // VertexProgram mode.
...
// ***************************************************************************
void CPatch::computeGeomorphTileVertexListVB(
    CTessList<CTessNearVertex> &vertList) {
  // Traverse the vertList.
  CTessNearVertex *pVert;
  for (pVert = vertList.begin(); pVert;
       pVert = (CTessNearVertex *)pVert->Next) {
...
void CPatch::updateClipPatchVB(bool renderClipped) {
  ...
  if (renderClipped) {
    ...
    // Force clip state of all TessBlocks, so no allocation will be done on
    // Vertices in VB.
    if (!TessBlocks.empty()) {
      for (uint i = 0; i < TessBlocks.size(); i++) {
        CTessBlock &tblock = TessBlocks[i];
...
void CPatch::checkCreateVertexVBFar(CTessFarVertex *pVert) {
  ...
  // NB: must test Far0>0 because vertices are reallocated in preRender() if a
  // change of Far occurs.
...
void CPatch::checkDeleteVertexVBFar(CTessFarVertex *pVert) {
  ...
  // NB: must test Far0>0 because vertices are deleted in preRender() if a
  // change of Far occurs.
  if (!isRenderClipped() && Far0 > 0 && pVert->OwnerBlock->visibleFar0()) {
    CLandscapeGlobals::CurrentFar0VBAllocator->deleteVertex(pVert->Index0);
  }
...
// ***************************************************************************
void CPatch::fillFar0DLMUvOnlyVertexListVB(
    CTessList<CTessFarVertex> &vertList) {
  // The Buffers must have been locked
  nlassert(CLandscapeGlobals::CurrentFar0VBAllocator);
  nlassert(CLandscapeGlobals::CurrentFar0VBAllocator->bufferLocked());
  ...
  if (_DLMContext) {
    ...
    CTessFarVertex *pVert;
    for (pVert = vertList.begin(); pVert;
         pVert = (CTessFarVertex *)pVert->Next) {
      // Compute/build the new vertex.
      CurVBPtr =
          (uint8 *)CLandscapeGlobals::CurrentFar0VBInfo.VertexCoordPointer;
      CurVBPtr +=
          pVert->Index0 * CLandscapeGlobals::CurrentFar0VBInfo.VertexSize;
      ...
      // Set Uv DLM only (NB: same code for VertexProgram or not, only
      // TexCoordOff1 may change).
      CHECK_VBA_RANGE(CLandscapeGlobals::CurrentFar0VBInfo.Accessor,
                      CurVBPtr +
                          CLandscapeGlobals::CurrentFar0VBInfo.TexCoordOff1,
                      sizeof(CUV))
...
// ***************************************************************************
void CPatch::fillFar1DLMUvOnlyVertexListVB(
    CTessList<CTessFarVertex> &vertList) {
  // The Buffers must have been locked
  nlassert(CLandscapeGlobals::CurrentFar1VBAllocator);
  nlassert(CLandscapeGlobals::CurrentFar1VBAllocator->bufferLocked());
  ...
  if (_DLMContext) {
    ...
    for (pVert = vertList.begin(); pVert;
      ...
      // Compute/build the new vertex.
      CurVBPtr =
          (uint8 *)CLandscapeGlobals::CurrentFar1VBInfo.VertexCoordPointer;
      CurVBPtr +=
          pVert->Index1 * CLandscapeGlobals::CurrentFar1VBInfo.VertexSize;

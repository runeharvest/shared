...
CZoneSymmetrisation::getTileState(uint patch, uint tile, uint layer) const {
  nlassert(layer < 5);
...
CZoneSymmetrisation::TState
CZoneSymmetrisation::getTileBorderState(uint patch, uint tile) const {
...
CZoneSymmetrisation::TState
CZoneSymmetrisation::getOrientedTileBorderState(uint patch, uint tile) const {
...
void CZoneSymmetrisation::setTileState(uint patch, uint tile, uint layer,
                                       TState state) {
...
void CZoneSymmetrisation::setTileBorderState(uint patch, uint tile,
                                             TState state) {
...
void CZoneSymmetrisation::setOrientedTileBorderState(uint patch, uint tile,
                                                     TState state) {
...
void CZoneSymmetrisation::setOrientedTileCorner(uint patch, uint tile,
                                                bool corner) {
  uint16 &ref = _TilesLayerStates[patch][tile];
  ref &= ~(1 << 10);
...
REMARKS:
- I call "isotile" the set of connected tiles in the zone using the same tileset
and a continus rotation value.

This method determines the state of each tile in the zone. This state will be
used to transform the tile during symmetrisation of the zone.

The state can be Regular, Goofy or Nothing.

If the state is Nothing, this tile is in an "isotile" that is not connected to a
zone border. No matter the method you use to tranform it, it will not influence
the neighbor zones.

If the state is Regular, the rotation of this tile after symmetrisation will by
given by this formula : tilerot = (4-tilerot)&3 If the state is Goofy, the
rotation of this tile after symmetrisation will by given by this formula :
tilerot = (4-tilerot+2)&3

- Getting the state of the tile:
    - A) We flag all the zone patches as Nothing.
    - B) We need to select patches having an open edge on the zone border. To do
so, we use the snapCell and weldThreshold parameters.
    - C) Then, for each patches on the zone border, we need to know if they are
Goofy or Regular.
    ...
    - F) Then, we propagate the tile A flag across the tiles in the zone
following this rules:
        - If A is not Nothing do
            - For each neighbor B of A do
                - If B is different from A do
                    - If A is Regular (res Goofy), and B is Nothing, B will be
Regular (res Goofy)
                    - If A is Regular (res Goofy), and B is Goofy (res Regular),
-> Error
                    - Propagate B
*/

bool CZoneSymmetrisation::build(const std::vector<CPatchInfo> &patchInfo,
                                float snapCell, float weldThreshold,
                                const CTileBank &bank, CError &errorDesc,
  ...
  // B), C) and E) We need to select patches having an open edge on the zone
  // border. To do so, we use the snapCell and weldThreshold parameters
  for (i = 0; i < patchInfo.size(); i++) {
    // Ref on the patch
    const CPatchInfo &patch = patchInfo[i];
    ...
    TState patchState;
    if (!setTileState(patch, i, snapCell, weldThreshold, patchState,
                      toOriginalSpace, bank)) {
      // Push an error
      errorDesc.Errors.push_back("Patch nb " + toString(i) + " is invalid");
    }

    // Set the oriented patch state
    if (!setOrientedTileState(patch, i, snapCell, weldThreshold, patchState,
                              toOriginalSpace, bank)) {
      // Push an error
      errorDesc.Errors.push_back("Patch nb " + toString(i) + " is invalid");
    }
    ...
    for (t = 0; t < patch.OrderT; t++) {
      for (s = 0; s < patch.OrderS; s++) {
        if (!propagateTileState(i, s, t, patchInfo, bank, false)) {
          // Push an error
          errorDesc.Errors.push_back(
              "Error during propagation. Topology invalid.");
          return false;
        }
      }
    }
...
bool CZoneSymmetrisation::snapOnGrid(float &value, float resolution,
                                     float snap) {
  // Calc the floor
...
bool CZoneSymmetrisation::setTileState(const NL3D::CPatchInfo &patch,
                                       uint patchId, float snapCell,
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (((uint32)vertPosU[i] != 0x80000000) ||
        ((uint32)vertPosV[i] != 0x80000000)) {
      // Snapped on U or V ?
      bool snapU = (vertPosU[i] == vertPosU[(i + 1) & 3]) &&
                   ((uint32)vertPosU[i] != 0x80000000);
      bool snapV = (vertPosV[i] == vertPosV[(i + 1) & 3]) &&
                   ((uint32)vertPosV[i] != 0x80000000);
          ...
          for (layer = 0; layer < 3; layer++) {
            ...
            if (tile != NL_TILE_ELM_LAYER_EMPTY) {
              ...
              if ((tileSet < 0) || (tileSet >= bank.getTileSetCount())) {
                nlwarning("CZoneSymmetrisation::setTileState : tile %d has an "
                          "unknown tileSet (%d)",
                          tile, tileSet);
                return false;
              }
...
bool CZoneSymmetrisation::setOrientedTileState(
    const NL3D::CPatchInfo &patch, uint patchId, float snapCell,
    float weldThreshold, TState &state, const NLMISC::CMatrix &toOriginalSpace,
    const CTileBank &bank) {
  // Edges state
  TState edgesState[4] = {Nothing, Nothing, Nothing, Nothing};
      ...
      for (j = 0; j < tileCount; j++) {
        ...
        for (layer = 0; layer < 3; layer++) {
          ...
          if (tile != NL_TILE_ELM_LAYER_EMPTY) {
            ...
            bank.getTileXRef(tile, tileSet, number, type);
            if ((tileSet < 0) || (tileSet >= bank.getTileSetCount())) {
              nlwarning("CZoneSymmetrisation::setOrientedTileState : tile %d "
                        "has an unknown tileSet (%d)",
                        tile, tileSet);
              return false;
            }
...
CVector2f st2uv(sint s, sint t, const CPatchInfo &patch) {
  return CVector2f((((float)s) + 0.5f) / (float)patch.OrderS,
                   (((float)t) + 0.5f) / (float)patch.OrderT);
}
...
public:
  CFillStackNode(uint16 patch, uint16 s, uint16 t, uint8 rotate,
...
bool CZoneSymmetrisation::propagateTileState(
    uint patch, uint s, uint t, const std::vector<CPatchInfo> &patchInfo,
  ...
  for (layer = 0; layer < 3; layer++) {
    ...
    if (tileIndex != NL_TILE_ELM_LAYER_EMPTY) {
      ...
      if (tileIndex >= (uint)bank.getTileCount()) {
        nlwarning(
            "CZoneSymmetrisation::propagateTileState: Invalid tile index");
        return false;
      }
      ...
      if ((tileSetToPropagate < 0) ||
          (tileSetToPropagate >= bank.getTileSetCount())) {
        nlwarning("CZoneSymmetrisation::propagateTileState: tile %d has an "
                  "unknown tileSet (%d)",
                  tileIndex, tileSetToPropagate);
      } else {
        // Oriented ?
        ...
        if (!(oriented && getOrientedTileCorner(patch, tile))) {
          // Current node
          CFillStackNode currentNode(
              patch, s, t, currentPatchPtr->Tiles[tile].getTileOrient(layer),
              getTileState(patch, tile, layer));

          // Propagate non-Nothing tiles
          if ((!forceRegular && (currentNode.State != Nothing)) ||
              (forceRegular && (currentNode.State == Nothing))) {
            // Force to Regular ?
            if (forceRegular) {
            ...
            while (!stack.empty()) {
              ...
              do {
                ...
                // Get neighbor
                CFillStackNode neighborNode(currentNode.Patch, currentNode.S,
                                            currentNode.T, currentNode.Rotate,
                                            currentNode.State);
                ...
                // Is still in patch ?
                if ((neighborNode.S >= patchInfo[currentNode.Patch].OrderS) ||
                    (neighborNode.T >= patchInfo[currentNode.Patch].OrderT)) {
                  // No, found new patch
                  uint position;
                  switch (currentNode.Edge) {
                  case 0:
                  ...
                  case 1:
                    ...
                    break;
                  case 2:
                    position = patchInfo[currentNode.Patch].OrderT -
                               neighborNode.T - 1;
                    break;
                  case 3:
                    position = patchInfo[currentNode.Patch].OrderS -
                               neighborNode.S - 1;
                    break;
                  default:
                    position = 0;
                  ...
                  sint sOut;
                  sint tOut;
                  if (patchInfo[currentNode.Patch].getNeighborTile(
                          currentNode.Patch, currentNode.Edge, position,
                    ...
                    // Find neighbor
                    const CPatchInfo::CBindInfo &neighborBindInfo =
                        patchInfo[currentNode.Patch]
                            .BindEdges[currentNode.Edge];
                    uint edgePatch;
                    for (edgePatch = 0;
                         edgePatch < (uint)neighborBindInfo.NPatchs;
                         edgePatch++) {
                      if (neighborBindInfo.Next[edgePatch] ==
                          neighborNode.Patch)
                        break;
                    }
                    ...
                    // Rotation
                    neighborNode.Rotate =
                        (currentNode.Rotate + 2 +
                         neighborBindInfo.Edge[edgePatch] - currentNode.Edge) &
                        3;
                    ...
                    if ((neighborNode.Rotate ^ currentNode.Rotate) & 1) {
                      // Yes
                      neighborNode.State =
                          (neighborNode.State == Regular) ? Goofy : Regular;
                    }
                  } else {
                ...
                // Neighbor patch
                const CPatchInfo *neighborPatchPtr =
                    &(patchInfo[neighborNode.Patch]);

                // Get the tile index
                uint neighborTile =
                    neighborNode.S + neighborNode.T * neighborPatchPtr->OrderS;

                // Look for the same tile set in the new tile
                uint neighborLayer;
                for (neighborLayer = 0; neighborLayer < 3; neighborLayer++) {
                  // Get the tile index
                  uint neighborTileIndex =
                      neighborPatchPtr->Tiles[neighborTile].Tile[neighborLayer];
                  ...
                  if (neighborTileIndex != NL_TILE_ELM_LAYER_EMPTY) {
                    ...
                    bank.getTileXRef(neighborTileIndex, neighborTileSet,
                                     neighborNumber, neighborType);

                    // Same tileset ? Stop!
                    if ((neighborTileSet == tileSetToPropagate) &&
                        (neighborNode.Rotate ==
                         neighborPatchPtr->Tiles[neighborTile].getTileOrient(
                             neighborLayer)))
                      break;
                  }
                }
                ...
                if (neighborLayer < 3) {
                  // If oriented, must not be a corner
                  if (!(oriented && getOrientedTileCorner(neighborNode.Patch,
                                                          neighborTile))) {
                    // Propagate in the new node ?
                    TState neighborState = getTileState(
                        neighborNode.Patch, neighborTile, neighborLayer);
                    if (neighborState == Nothing) {
                      // Set the state
                      setTileState(neighborNode.Patch, neighborTile,
                                   neighborLayer, neighborNode.State);
                      ...
                      // nlwarning ("CZoneSymmetrisation::propagateTileState:
                      // error, find same iso surfaces with different state.");

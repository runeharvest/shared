...
CCoarseMeshManager::CCoarseMeshManager() {
  ...
  _Triangles.setPreferredMemory(CIndexBuffer::RAMVolatile,
...
bool CCoarseMeshManager::addMesh(uint numVertices, const uint8 *vBuffer,
                                 uint numTris,
...
void CCoarseMeshManager::flushRender(IDriver *drv) {
  ...
  if (!_VBuffer.isResident() &&
      drv->getVertexColorFormat() == CVertexBuffer::TBGRA) {
  ...
  for (std::vector<CMeshInfo>::iterator it = _Meshs.begin(); it != _Meshs.end();
       ++it) {
    // Copy Vertices to VBuffer
    uint baseVertex = currentNumVertices;
    CHECK_VBA_RANGE(_VBA, _VBA.getVertexCoordPointer(baseVertex),
                    it->NumVertices * _VBuffer.getVertexSize());
    CFastMem::memcpy(_VBA.getVertexCoordPointer(baseVertex), it->VBuffer,
                     it->NumVertices * _VBuffer.getVertexSize());
    ...
    TCoarseMeshIndexType *triDst =
        (TCoarseMeshIndexType *)_IBA.getPtr() + currentNumTriangles * 3;
    const TCoarseMeshIndexType *triSrc = it->IndexBuffer;
    uint numIdx = it->NumTris * 3;
    // NB: for the majority of CoarseMesh (4 faces==48 bytes of indices), not
    // interressant to use CFastMem::precache()

...
void CTessFacePListNode::linkInPList(CTessFacePListNode &root) {
  ...
  /*
      NB if list was empty (this, this), then
          ...
          root._NextTessFaceInPList->_PrecTessFaceInPList= this;	=>
     root._PrecTessFaceInPList= this root._NextTessFaceInPList= this
  */
}
...
void CTessFacePListNode::unlinkInPList() {
  /*
      ...
      If this node was the last of a list, then the root correctly get (&root,
     &root) after this.
  */
  // unlink old list from me.
  _PrecTessFaceInPList->_NextTessFaceInPList = _NextTessFaceInPList;
...
// ***************************************************************************
CTessFacePriorityList::~CTessFacePriorityList() { clear(); }

// ***************************************************************************
void CTessFacePriorityList::init(float distStep, uint numEntries,
                                 float distMaxMod, uint numQuadrant) {
  uint i;
  nlassert(distStep > 0);
  ...
  for (i = 0; i < _RollingTables.size(); i++) {
    ...
    // setup the quadrant direction. NB: 0 not used since "Direction less
    // rolling table"
    if (i >= 1) {
      uint idx = i - 1;
...
uint CTessFacePriorityList::selectQuadrant(const CVector &direction) {
  ...
  for (uint i = quadrantStart; i < quadrantEnd; i++) {
    // get the quadrant Id. Must mask for last (0). And Start at 1 in the
    // _RollingTables.
...
// ***************************************************************************
void CTessFacePriorityList::insert(uint quadrantId, float distance,
                                   CTessFace *value) {
...
// ***************************************************************************
void CTessFacePriorityList::shift(const CVector &direction,
  ...
  for (uint i = 0; i < _RollingTables.size(); i++) {
    ...
    // Else compute the effective distance we run to the plane.
      ...
      // For now, just clamp, but may be interesting to shift Back the rolling
      // table !!
    ...
    // For all elements of the entry 0 of , pull them, and insert in result
    // list.
    pulledElements.appendPList(rollTable.getRollTableEntry(0));

    // shift.
    rollTable.Remainder += shiftDistance;
    // If Remainder>=1, it means that we must shift the rolling table, and get
    // elements deleted.
...
// ***************************************************************************
CTessFacePriorityList::CRollingTable::~CRollingTable() { clearRollTable(); }
...
// ***************************************************************************
void CTessFacePriorityList::CRollingTable::insertInRollTable(uint entry,
                                                             CTessFace *value) {
...
// ***************************************************************************
CTessFacePListNode &
CTessFacePriorityList::CRollingTable::getRollTableEntry(uint entry) {
  CTessFacePListNode &root = _Entries[(entry + _EntryStart) & _MaskEntries];
  return root;
}
...
// ***************************************************************************
void CTessFacePriorityList::CRollingTable::shiftEntries(
    uint entryShift, CTessFacePListNode &pulledElements) {
  if (entryShift > 0) {

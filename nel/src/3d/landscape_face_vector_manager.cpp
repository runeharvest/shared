...
// ***************************************************************************
CLandscapeFaceVectorManager::~CLandscapeFaceVectorManager() { purge(); }
...
CLandscapeFaceVectorManager::createFaceVector(uint numTri) {
  ...
  if (_Blocks[blockId] == NULL) {
    ...
    uint sizeInByteToAllocate =
        (uint)max(sizeof(TLandscapeIndexType *),
                  (numTriMax * 3 + 1) * sizeof(TLandscapeIndexType));
    _Blocks[blockId] =
        new TLandscapeIndexType[(sizeInByteToAllocate +
                                 (sizeof(TLandscapeIndexType) - 1)) /

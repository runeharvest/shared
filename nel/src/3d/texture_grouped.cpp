...
///=====================================================================================================
/// This is used to get the size of a texture, with an optimisation in the case
/// of texture files
static inline void GetTextureSize(ITexture *tex, uint &width, uint &height) {
  if (tex->getClassName() == "CTextureFile") {
    ...
    if (!tf->getFileName().empty()) {
      try {
        CBitmap::loadSize(NLMISC::CPath::lookup(tf->getFileName()), srcWidth,
                          srcHeight);
        if (srcWidth == 0 || srcHeight == 0) {
          nlinfo("Unable to get size of texture : %s",
        ...
        nlinfo("unable to load size from a bitmap ! name = %s",
               tf->getFileName().c_str());
...
bool CTextureGrouped::areValid(CSmartPtr<ITexture> *textureTab, uint nbTex) {
  ...
  for (k = 0; k < nbTex; ++k) {
    if (textureTab[k]->getReleasable())
      textureTab[k]->release();
  }
  return result;
}
...
void CTextureGrouped::doGenerate(bool async) {
  ...
  if (_NbTex == 0) {
    ...
    const uint width = _Textures[0]->getWidth(),
               height = _Textures[0]->getHeight();
    ...
    // save sub bitmap size so that bitmaps won't be reloaded to get their size
    // the next time
...
void CTextureGrouped::forceGenUVs() {
  ...
  if (it == _NameToSize.end()) {
    doGenerate(); // this will retrieve size of sub-bitmaps
                  // TODO : only load the size... not a problem for now because
                  // textures can be cached at startup
    release();
...
void CTextureGrouped::genUVs(uint subBitmapHeight) {
  _TexUVs.resize(_NbTex);
  //
  const uint totalHeight =
      subBitmapHeight * _NbTex; // *it contains the height of bitmap
  const uint realHeight = NLMISC::raiseToNextPowerOf2(totalHeight);
  // TODO : better ordering of sub-bitmaps
  const float deltaV =
      realHeight ? (float(totalHeight) / float(realHeight)) * (1.0f / _NbTex)
                 : 0.f;
...
void CTextureGrouped::getTextures(CSmartPtr<ITexture> *textureTab) const {
  ...
  for (TTexList::const_iterator it = _Textures.begin(); it != _Textures.end();
       ++it, ++dest) {
    *dest = *it;
  }
}
...
void CTextureGrouped::serial(NLMISC::IStream &f) {
  ...
  if (f.isReading()) {
    ...
    f.serial(_NbTex);
    ITexture *ptTex;
    for (TTexList::iterator it = _Textures.begin(); it != _Textures.end();
         ++it) {
      ptTex = *it;
      f.serialPolyPtr(ptTex);
    }
...
///=====================================================================================================
void CTextureGrouped::displayIncompatibleTextureWarning(
    CSmartPtr<ITexture> *textureTab, uint nbTex) {
  nlwarning("=======================================================");
  nlwarning("CTextureGrouped : found incompatible textures, that differs by "
            "size or format :");
  for (uint k = 0; k < nbTex; ++k) {
    if (textureTab[k]) {

...
// Define this to render the zbuffers into a bitmap zbuffer and save it into a
// jpeg #define SAVE_ZBUFFER "c:/temp"
...
Documentation:
    ...
    To light a zone, you must first adding shadow caster triangles using the
addTriangle() methods. Triangles can come from landscape zones or IG meshes.
Then call the lighting process with ligth().
    ...
    light ()
        The lighting process uses a software zbuffers render to compute shadow
attenuation of the zone. CRenderZBuffer () (multithread)
            - Render shadow caster triangles into the light zbuffers for
shadows. Each z value is tested and written in the pixel and in the 8 neighbor
pixels.
            - There is a zbuffer per landscape softshadow sample and an
additionnal zbuffer for objects. So landscape triangles cast softshadows and
object (trees and building) cast antialiased shadows.
        ...
        buildZoneInformation ()
            ...
            - Compute lumel positions.
                Lumel position is the average of lumel triangles center
overlapping the lumel but using the shadow accuracy triangle position because we
need the same triangles than shadow caster polygons. Border lumel position are
extended to fit the patch border.

            - Tesselate to lumel accuracy (0.5 meter)

            - Compute lumel normal
                - Lumel normal is the average of lumel triangles normals. The
normals comes from the lumel accuracy to get more precise lighting. So normals
                are interpolated from the center of the lumel but they will be
rendered on the patch border. Unlike the position, we can extand the normal to
the border without loosing normal precision because normal interpolation is
                aligned with the tesselation. So we need some border normal
smoothing.

            - Border normal smoothing
                - Normals on the border of the patches are smoothed with
neighbor normals.
            ...
            - For each patches and for each lumels
                ...
                attenuation ()
                    - Compute shadow attenuation
                        - Get an antialised attenuation value in each lansdcape
softshadow zbuffer. Average it with jitter.
                        - Get an antialised attenuation value from the object
zbuffer.
                ...
                getSkyContribution ()
                    - Compute sky lighting (radiosity)
                        - Algorithm
                            ...
                            - Compute an approximation of the sky surface
visible from the lumel position
...
inline void transformVectorToZBuffer(const CZoneLighter::CZBuffer &zbuffer,
                                     const CVector &world, CVector &projected) {
...
inline float testZPercentageCloserFilter(
    float x, float y, float z, CZoneLighter::CZBuffer &zbuffer,
    const CZoneLighter::CLightDesc &description, bool &zBufferOverflowFlag) {
  // See "Rendering Antialiased Shadows With Depth Maps" Reeves, Salesint, Cook,
  // ACM 1987
  ...
  for (dy = 0; dy < 2; dy++)
    for (dx = 0; dx < 2; dx++) {
      ...
      if ((fx >= 0) && (fx < zbuffer.LocalZBufferWidth) && (fy >= 0) &&
          (fy < zbuffer.LocalZBufferHeight)) {
        const float zRed =
            zbuffer.Pixels[fx + (zbuffer.LocalZBufferHeight - 1 - fy) *
                                    zbuffer.LocalZBufferWidth];
  ...
  // Bilinear
  return (biliValues[0][0] * (1 - factorX) + biliValues[1][0] * factorX) *
             (1 - factorY) +
         (biliValues[0][1] * (1 - factorX) + biliValues[1][1] * factorX) *
             factorY;
}
...
float CZoneLighter::calcSkyContribution(sint s, sint t, float height,
                                        float skyIntensity,
  ...
  for (uint i = 0; i < 8; i++) {
    ...
    uint8 phi =
        getMaxPhi(s, t, deltaDirection[i][0], deltaDirection[i][1], height);
...
public:
  // Ctor
  CLightRunnable(uint process, CZoneLighter *zoneLighter,
...
public:
  // Ctor
  CRenderZBuffer(uint process, CZoneLighter *zoneLighter,
                 const CZoneLighter::CLightDesc *description,
                 uint firstTriangle, uint numTriangle,
                 const vector<CZoneLighter::CTriangle> *triangles) {
    _ZoneLighter = zoneLighter;
    _Description = description;
    _Process = process;
...
#define CLIPPED_ALL                                                            \
  (CLIPPED_TOP | CLIPPED_BOTTOM | CLIPPED_LEFT | CLIPPED_RIGHT)

void RenderTriangle(const CZoneLighter::CTriangle &triangle,
                    const CZoneLighter::CLightDesc &description,
                    CPolygon2D::TRasterVect &borders, CFastMutex &mutex,
                    CZoneLighter::CZBuffer &zbuffer, uint radius) {
  ...
  if (in == CLIPPED_ALL) {
    ...
    CVector vGradient;
    if (needUV) {
      gradientTriangle.computeGradient(triangle.U[0], triangle.U[1],
                                       triangle.U[2], uGradient);
      gradientTriangle.computeGradient(triangle.V[0], triangle.V[1],
                                       triangle.V[2], vGradient);
    }
    ...
    sint yMax =
        std::min((sint)(minimumY + borders.size()), zbuffer.LocalZBufferHeight);
    for (; y < yMax; y++) {
      // Ref on the raster
      ...
      for (; x < xMax; x++) {
        ...
        if (needUV) {
          ...
          uint8 alpha =
              ((const CRGBA *)&(
                   (*pixels)[(((uint)u) + ((uint)v) * width) * sizeof(CRGBA)]))
                  ->A;
        ...
        if (alphaTest) {
          ...
          for (d = 0; d < radius; d++) {
            ...
            if ((fx >= 0) && (fx < zbuffer.LocalZBufferWidth) && (fy >= 0) &&
                (fy < zbuffer.LocalZBufferHeight)) {
              float &zValue =
                  zbuffer.Pixels[fx + (zbuffer.LocalZBufferHeight - fy - 1) *
                                          zbuffer.LocalZBufferWidth];
...
void NL3D::CRenderZBuffer::run() {
  ...
  for (i = _FirstTriangle; i < _FirstTriangle + _NumTriangle; i++) {
    ...
    if ((triangle.Flags & CZoneLighter::CTriangle::DoubleSided) ||
        ((triangle.getPlane().getNormal() * _ZoneLighter->_SunDirection) > 0)) {
      // Landscape triangle ?
      if (triangle.Flags & CZoneLighter::CTriangle::Landscape) {
        // For each landscape zbuffer
        uint sample;
        const uint samples = _Description->SoftShadowSamplesSqrt *
                             _Description->SoftShadowSamplesSqrt;
        for (sample = 0; sample < samples; sample++) {
          RenderTriangle(triangle, *_Description, borders, _ZoneLighter->_Mutex,
                         _ZoneLighter->_ZBufferLandscape[sample], 9);
        }
      } else {
        // Render in a high resolution zbuffer
        RenderTriangle(triangle, *_Description, borders, _ZoneLighter->_Mutex,
                       _ZoneLighter->_ZBufferObject, 1);
      }
    }
...
public:
  CCalcLightableShapeRunnable(uint process, CZoneLighter *zoneLighter,
                              const CZoneLighter::CLightDesc *description,
                              CZoneLighter::TShapeVect *shapeToLit,
                              uint firstShape, uint lastShape)
      : _ZoneLighter(zoneLighter), _Description(description),
        _ShapesToLit(shapeToLit), _FirstShape(firstShape),
        _LastShape(lastShape), _Process(process) {}
  void run() {
    _ZoneLighter->processLightableShapeCalc(_Process, _ShapesToLit, _FirstShape,
                                            _LastShape, *_Description);
    _ZoneLighter->_ProcessExited++;
  }
...
void draw2dLine(CBitmap &bitmap, float x0, float y0, float x1, float y1,
                const CRGBA &color) {
...
void InitZBuffer(CZoneLighter::CZBuffer &zbuffer, const CVector &SunPosition,
                 const CMatrix &rayBasis, const CAABBoxExt &zoneBB,
                 uint zBufferPixelSize,
                 const CZoneLighter::CLightDesc &description) {
  // Clac the zbuffer world size
  const float zBufferWorldSize =
      (float)(tan(description.SunFOV / 2) * description.SunDistance * 2);
  ...
  zbuffer.WorldToZBufferFrustum.init(
      (float)zBufferWorldSize, (float)zBufferWorldSize, description.SunDistance,
      description.SunDistance * 2);
  ...
  transformVectorToZBuffer(zbuffer, CVector(bMin.x, bMax.y, bMin.z),
                           zbuffer.BoundingBoxVectors[0]);
  transformVectorToZBuffer(zbuffer, CVector(bMin.x, bMin.y, bMin.z),
                           zbuffer.BoundingBoxVectors[1]);
  transformVectorToZBuffer(zbuffer, CVector(bMax.x, bMin.y, bMin.z),
                           zbuffer.BoundingBoxVectors[2]);
  transformVectorToZBuffer(zbuffer, CVector(bMax.x, bMax.y, bMin.z),
                           zbuffer.BoundingBoxVectors[3]);
  transformVectorToZBuffer(zbuffer, CVector(bMin.x, bMax.y, bMax.z),
                           zbuffer.BoundingBoxVectors[4]);
  transformVectorToZBuffer(zbuffer, CVector(bMin.x, bMin.y, bMax.z),
                           zbuffer.BoundingBoxVectors[5]);
  transformVectorToZBuffer(zbuffer, CVector(bMax.x, bMin.y, bMax.z),
                           zbuffer.BoundingBoxVectors[6]);
  transformVectorToZBuffer(zbuffer, CVector(bMax.x, bMax.y, bMax.z),
                           zbuffer.BoundingBoxVectors[7]);
  ...
  zbuffer.LocalZBufferWidth =
      zbuffer.LocalZBufferXMax - zbuffer.LocalZBufferXMin;
  zbuffer.LocalZBufferHeight =
      zbuffer.LocalZBufferYMax - zbuffer.LocalZBufferYMin;

  // Resize and clear the zbuffer
  zbuffer.Pixels.resize(0);
  zbuffer.Pixels.resize(zbuffer.LocalZBufferWidth * zbuffer.LocalZBufferHeight,
                        FLT_MAX);
}
...
void SaveZBuffer(CZoneLighter::CZBuffer &zbuffer, const char *filename) {
  // Resize the bitmap
  CBitmap bitmap;
  bitmap.resize(zbuffer.LocalZBufferWidth, zbuffer.LocalZBufferHeight,
                CBitmap::Luminance);
  ...
  for (uint i = 0; i < samples; i++) {
    // Get the value
    float value = (zbuffer.Pixels[i] - zbuffer.LocalZBufferZMin) * 255 /
                  (zbuffer.LocalZBufferZMax - zbuffer.LocalZBufferZMin);
    clamp(value, 0, 255);
  ...
  // Draw some red lines
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[0].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[0].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[1].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[1].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[0].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[0].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[3].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[3].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[2].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[2].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[1].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[1].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
             ...
             CRGBA::Red);
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[4].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[4].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[5].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[5].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[4].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[4].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[7].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[7].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
             zbuffer.BoundingBoxVectors[6].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[6].y - (float)zbuffer.LocalZBufferYMin,
             zbuffer.BoundingBoxVectors[5].x - (float)zbuffer.LocalZBufferXMin,
             zbuffer.BoundingBoxVectors[5].y - (float)zbuffer.LocalZBufferYMin,
             CRGBA::Red);
  draw2dLine(bitmap,
      ...
      for (n = 1; n < filterRadius; n++) {
        ...
        if ((fx >= 0) && (fx < zbuffer.LocalZBufferWidth) && (fy >= 0) &&
          ...
          const float &testValue =
              zbuffer.Pixels[fx + fy * zbuffer.LocalZBufferWidth];
...
void CZoneLighter::light(CLandscape &landscape, CZone &output, uint zoneToLight,
   ...
   * - Create a heightfield used for global illumination. Cells are initialized
   * with -FLT_MAX
   * - Insert each shadow casting triangles in the quad grid and fill the
   * heightfield's cells overlapped by the bounding box of the triangle with the
   * max height of the triangle if its height is > than the current height in
   * the heightfield's cell.
   * -
   */
  ...
  if (pZone) {
    ...
    for (triangleId = 0; triangleId < size; triangleId++) {
      ...
      // Calc the plane
      triangle._Plane.make(triangle.Triangle.V0, triangle.Triangle.V1,
                           triangle.Triangle.V2);
    }

    // Create landscape zbuffers
    _ZBufferLandscape.resize(description.SoftShadowSamplesSqrt *
                             description.SoftShadowSamplesSqrt);

    uint sampleX;
    uint sampleY;
    for (sampleY = 0; sampleY < description.SoftShadowSamplesSqrt; sampleY++)
      for (sampleX = 0; sampleX < description.SoftShadowSamplesSqrt;
           sampleX++) {
        // *** Render the light zbuffer
        CZBuffer &zbuffer =
            _ZBufferLandscape[sampleX +
                              sampleY * description.SoftShadowSamplesSqrt];

        // Delta pos for area light
        float deltaX = ((float)sampleX + 0.5f -
                        (float)description.SoftShadowSamplesSqrt / 2.f) /
                       (float)description.SoftShadowSamplesSqrt;
        float deltaY = ((float)sampleY + 0.5f -
                        (float)description.SoftShadowSamplesSqrt / 2.f) /
                       (float)description.SoftShadowSamplesSqrt;
        CVector lightPos =
            _RayBasis.getI() * ((float)description.SunRadius * deltaX) +
            _RayBasis.getJ() * ((float)description.SunRadius * deltaY);
        lightPos = description.SunCenter -
                   (description.SunDirection * description.SunDistance) +
                   lightPos;

        InitZBuffer(zbuffer, lightPos, _RayBasis, zoneBB,
                    description.ZBufferLandscapeSize, description);
        nlinfo("Zbuffer %d size : %d x %d",
               sampleX + sampleY * description.SoftShadowSamplesSqrt,
               zbuffer.LocalZBufferWidth, zbuffer.LocalZBufferHeight);
      }

    // *** Init the zbuffer for the vegetation
    CVector lightPos = description.SunCenter -
                       (description.SunDirection * description.SunDistance);
    InitZBuffer(_ZBufferObject, lightPos, _RayBasis, zoneBB,
                description.ZBufferObjectSize, description);
    nlinfo("Zbuffer object size : %d x %d", _ZBufferObject.LocalZBufferWidth,
           _ZBufferObject.LocalZBufferHeight);
    ...
    for (uint process = 0; process < _ProcessCount; process++) {
      ...
      // Create a thread
      CRenderZBuffer *runnable =
          new CRenderZBuffer(process, this, &description, firstTriangle,
                             lastTriangle - firstTriangle, &obstacles);
    ...
    while (_ProcessExited != _ProcessCount) {
      ...
      progress("Render triangles",
               (float)_NumberOfPatchComputed / (float)obstacles.size());
    }

    // * Save the zbuffer
    uint sample;
    const uint samples =
        description.SoftShadowSamplesSqrt * description.SoftShadowSamplesSqrt;
#ifdef SAVE_ZBUFFER
    for (sample = 0; sample < samples; sample++) {
      // *** The zbuffer
      CZBuffer &zbuffer = _ZBufferLandscape[sample];

      string zbufferFilename =
          SAVE_ZBUFFER "/zbuffer_landscape_" + toString(sample) + ".jpg";
    ...
    _HeightFieldCellCount =
        (sint)(description.HeightfieldSize / _HeightfieldCellSize);
    nlassert(_HeightFieldCellCount != 0);
    _OrigineHeightField =
        zoneBB.getCenter() - CVector(description.HeightfieldSize / 2,
                                     description.HeightfieldSize / 2, 0);
    _HeightField.resize(_HeightFieldCellCount * _HeightFieldCellCount,
                        -FLT_MAX);
    ...
    for (triangleId = 0; triangleId < size; triangleId++) {
      ...
      if ((triangleId & 0xff) == 0)
        progress("Build quadtree and heightfield",
                 (float)triangleId / (float)size);
      ...
      if (triangle.Flags & CTriangle::Landscape) {
        ...
        sint minX =
            std::max(0, (sint)floor(0.5f + (minv.x - _OrigineHeightField.x) /
                                               _HeightfieldCellSize));
        sint maxX =
            std::min(_HeightFieldCellCount,
                     (sint)floor(0.5f + (maxv.x - _OrigineHeightField.x) /
                                            _HeightfieldCellSize));
        sint minY =
            std::max(0, (sint)floor(0.5f + (minv.y - _OrigineHeightField.y) /
                                               _HeightfieldCellSize));
        sint maxY =
            std::min(_HeightFieldCellCount,
                     (sint)floor(0.5f + (maxv.y - _OrigineHeightField.y) /
                                            _HeightfieldCellSize));
    ...
    buildZoneInformation(landscape, listZone, description);
  }
  ...
  if (patchCount) {
    ...
    for (process = 0; process < _ProcessCount; process++) {
      ...
      // Create a thread
      CLightRunnable *runnable =
          new CLightRunnable(process, this, &description);
      IThread *pThread = IThread::create(runnable);
      runnable->Thread = pThread;
    ...
    while (_ProcessExited != _ProcessCount) {
      ...
      // Call the progress callback
      progress("Lighting patches",
               (float)_NumberOfPatchComputed / (float)_PatchInfo.size());
    }
  } else {
...
float CZoneLighter::getSkyContribution(const CVector &pos,
  ...
  skyContributionTab[0][0] =
      calcSkyContribution(sInt - 1, tInt - 1, pos.z, skyIntensity, normal);
  skyContributionTab[1][0] =
      calcSkyContribution(sInt, tInt - 1, pos.z, skyIntensity, normal);
  skyContributionTab[1][1] =
      calcSkyContribution(sInt, tInt, pos.z, skyIntensity, normal);
  skyContributionTab[0][1] =
      calcSkyContribution(sInt - 1, tInt, pos.z, skyIntensity, normal);

  float sFact = s + 0.5f - sInt;
  float tFact = t + 0.5f - tInt;
  return (skyContributionTab[0][0] * (1.f - sFact) +
          skyContributionTab[1][0] * sFact) *
             (1.f - tFact) +
         (skyContributionTab[0][1] * (1.f - sFact) +
          skyContributionTab[1][1] * sFact) *
             tFact;
}
  ...
  while (patch != 0xffffffff) {
    ...
    for (uint lumel = 0; lumel < lumels.size(); lumel++) {
      ...
      if (description.SkyContribution) {
        skyContribution =
            getSkyContribution(lumels[lumel].Position, lumels[lumel].Normal,
                               description.SkyIntensity);
      } else {
        skyContribution = 0.f;
      }
      ...
      if (description.SunContribution) {
        sunContribution =
            (-lumels[lumel].Normal * _SunDirection) - skyContribution;
        clamp(sunContribution, 0.f, 1.f);
      } else
        sunContribution = 0;

      // Final lighting
      sint finalLighting = (sint)(255.f * (((float)patchInfo.Lumels[lumel]) *
                                               sunContribution / 255.f +
                                           skyContribution));
...
uint8 CZoneLighter::getMaxPhi(sint s, sint t, sint deltaS, sint deltaT,
                              float heightPos) const {
  // Start position
  s += deltaS;
  t += deltaT;
  ...
  // Distance increment
  float stepDistance =
      CVector(deltaS * _HeightfieldCellSize, deltaT * _HeightfieldCellSize, 0)
          .norm();
  ...
  // For all the line
  while ((s < _HeightFieldCellCount) && (t < _HeightFieldCellCount) &&
         (s >= 0) && (t >= 0)) {
    // Get height
...
bool CZoneLighter::isLumelOnEdgeMustBeOversample(
    uint patch, uint edge, sint s, sint t, const vector<bool> &binded,
  ...
  if (oversampleEdges[edge])
    ...
    if (binded[edge]) {
      // Lumel coord
      CVector2f lumelCoord(((float)(s + _GetNormalDeltaS[edge]) + 0.5f) / 4.f,
                           ((float)(t + _GetNormalDeltaT[edge]) + 0.5f) / 4.f);
      ...
      return (shadowBuffer[patchOut->getPatchId()]
                          [ss + (patchOut->getOrderS() << 2) * tt] != shadowed);
    } else {
...
void CZoneLighter::getNormal(const CPatch *pPatch, sint16 lumelS, sint16 lumelT,
                             vector<CPatchUVLocator> &locator,
                             const vector<CPatch::CBindInfo> &bindInfo,
                             const vector<bool> &binded, set<uint64> &visited,
                             float deltaS, float deltaT, uint rotation,
                             const CBezierPatch &bezierPatch, uint lastEdge) {
  // Build a desc srructure
  uint64 id = (uint64)lumelS | (((uint64)lumelT) << 16) |
              (((uint64)pPatch->getPatchId()) << 32) |
              (((uint64)pPatch->getZone()->getZoneId()) << 48);
  ...
  if (visited.insert(id).second) {
    ...
    if (sqDist < 1) {
      ...
      // Add normal
      _GetNormalNormal +=
          bezierPatch.evalNormal(((float)lumelS + 0.5f) / (float)orderSx4,
                                 ((float)lumelT + 0.5f) / (float)orderTx4);
      ...
      for (uint edge = 0; edge < 4; edge++) {
        ...
        if (edge != lastEdge) {
          ...
          // Neighbor
          if ((lumelS == _GetNormalBorderS[edge]) ||
              (lumelT == _GetNormalBorderT[edge])) {
            // Binded ?
            bool bind = binded[edge];
            bool smooth = pPatch->getSmoothFlag(edge);
            if (bind && smooth) {
              // Lumel coord
              CVector2f lumelCoord(
                  ((float)(lumelS + _GetNormalDeltaS[edge]) + 0.5f) / 4,
              ...
              CVector2f neighborUV;
              CPatch *patchOut;
              locator[edge].locateUV(lumelCoord, otherPatch, patchOut,
                                     neighborUV);
              ...
              // Next lumel
              getNormal(patchOut, newLumelS, newLumelT,
                        _Locator[zoneId][patchId], _BindInfo[zoneId][patchId],
                        _Binded[zoneId][patchId], visited,
                        deltaS + _GetNormalDeltaS[globalDirection],
                        deltaT + _GetNormalDeltaT[globalDirection], newRotation,
                        NewBezierPatch, newEdge);
            }
          } else {
            // Left internal
            getNormal(pPatch, lumelS + _GetNormalDeltaS[edge],
                      lumelT + _GetNormalDeltaT[edge], locator, bindInfo,
                      binded, visited,
                      deltaS + _GetNormalDeltaS[globalDirection],
                      deltaT + _GetNormalDeltaT[globalDirection], rotation,
                      bezierPatch, (edge + 2) & 0x3);
          }
        }
      }
    }
...
void CZoneLighter::addTriangles(CLandscape &landscape, vector<uint> &listZone,
                                uint order,
  ...
  for (uint leave = 0; leave < leavesCount; leave++) {
    ...
    // Add a triangle
    triangleArray.push_back(CTriangle(NLMISC::CTriangle(
        face->VBase->EndPos, face->VLeft->EndPos, face->VRight->EndPos)));
  }
...
void CZoneLighter::addTriangles(const IShape &shape, const CMatrix &modelMT,
                                std::vector<CTriangle> &triangleArray) {
  // Cast to CMesh
  ...
  // It is a CMeshMultiLod ?
    ...
    // Dynamic cast
    const CMeshMRMGeom *mrmGeomMesh =
        dynamic_cast<const CMeshMRMGeom *>(meshGeom);
    if (mrmGeomMesh) {
...
void CZoneLighter::addTriangles(const CMeshBase &meshBase,
  ...
  for (uint block = 0; block < numBlock; block++) {
    ...
    for (uint pass = 0; pass < numRenderPass; pass++) {
      ...
      const CIndexBuffer &primitive =
          meshGeom.getRdrPassPrimitiveBlock(block, pass);

      // Get the material
      const CMaterial &material =
          meshBase.getMaterial(meshGeom.getRdrPassMaterial(block, pass));
      ...
      bool doubleSided;
      if (getTexture(material, texture, clampU, clampV, alphaTestThreshold,
                     doubleSided)) {
        // Dump triangles
        ...
        if (iba.getFormat() == CIndexBuffer::Indices32) {
          ...
          for (tri = 0; tri < numTri; tri++) {
            // Vertex
            CVector v0 =
                modelMT * (*vba.getVertexCoordPointer(triIndex[tri * 3]));
            CVector v1 =
                modelMT * (*vba.getVertexCoordPointer(triIndex[tri * 3 + 1]));
            CVector v2 =
                modelMT * (*vba.getVertexCoordPointer(triIndex[tri * 3 + 2]));
            ...
            for (uint i = 0; i < 3; i++) {
              // Get UV coordinates
              const float *uv = (const float *)vba.getTexCoordPointer(
                  triIndex[tri * 3 + i], 0);
              if (uv) {
                // Copy it
                u[i] = uv[0];
            ...
            // Make a triangle
            triangleArray.push_back(
                CTriangle(NLMISC::CTriangle(v0, v1, v2), doubleSided, texture,
                          clampU, clampV, u, v, alphaTestThreshold));
          }
...
bool CZoneLighter::getTexture(const CMaterial &material, CBitmap *&result,
                              bool &clampU, bool &clampV,
                              uint8 &alphaTestThreshold, bool &doubleSided) {
  // Texture information, not NULL only if texture is used for alpha test
  result = NULL;
  clampU = false;
  clampV = false;
  ...
  if (material.getAlphaTest()) {
    ...
    if (texture && texture->supportSharing()) {
      ...
      if (ite != _Bitmaps.end()) {
        ...
        // No, add it
        ite = _Bitmaps
                  .insert(std::map<string, NLMISC::CBitmap>::value_type(
                      name, CBitmap()))
                  .first;
        result = &(ite->second);
...
void CZoneLighter::addTriangles(const CMeshBase &meshBase,
  ...
  for (uint pass = 0; pass < numRenderPass; pass++) {
    ...
    // Get the material
    const CMaterial &material =
        meshBase.getMaterial(meshGeom.getRdrPassMaterial(0, pass));
      ...
      if (iba.getFormat() == CIndexBuffer::Indices32) {
        ...
        for (tri = 0; tri < numTri; tri++) {
          ...
          for (uint i = 0; i < 3; i++) {
            // Get UV coordinates
            float *uv =
                (float *)vba.getTexCoordPointer(triIndex[tri * 3 + i], 0);
            if (uv) {
              // Copy it
              u[i] = uv[0];
              v[i] = uv[1];
          ...
          triangleArray.push_back(CTriangle(NLMISC::CTriangle(v0, v1, v2),
                                            doubleSided, texture, clampU,
                                            clampV, u, v, alphaTestThreshold));
        }
      } else if (iba.getFormat() == CIndexBuffer::Indices16) {
        const uint16 *triIndex = (const uint16 *)iba.getPtr();
        uint numTri = primitive.getNumIndexes() / 3;
...
const sint8 CZoneLighter::TriangleIndexes[10][2][3] = {
    {{0, 11, 6}, {0, 6, 9}},    {{9, 6, 4}, {4, 6, 14}},
    {{4, 14, 8}, {4, 8, 10}},   {{10, 8, 1}, {-1, -1, -1}},
    {{11, 5, 6}, {5, 13, 6}},   {{6, 13, 3}, {6, 3, 14}},
    {{3, 8, 14}, {-1, -1, -1}}, {{5, 12, 7}, {5, 7, 13}},
    {{7, 3, 13}, {-1, -1, -1}}, {{12, 2, 7}, {-1, -1, -1}}};

// ***************************************************************************

// lumel vertex ID, tesselation edge ID (0 for base<->right, 1 for base<->left),
// tesselation edge vertex 0, tesselation edge vertex 1
const sint8 CZoneLighter::VertexThanCanBeSnappedOnABorder[8][4] = {
    {0, 0, 0, 9},  {1, 0, 9, 4},  {2, 0, 4, 10}, {3, 0, 10, 1},
    {0, 1, 0, 11}, {4, 1, 11, 5}, {7, 1, 5, 12}, {9, 1, 12, 2},
};
...
void CZoneLighter::buildZoneInformation(CLandscape &landscape,
  ...
  for (uint zone = 0; zone < zoneCount; zone++) {
    ...
    for (patch = 0; patch < patchCount; patch++) {
      // Get a patch pointer
      const CPatch *pPatch =
          (const_cast<const CZone *>(landscape.getZone(listZone[zone])))
              ->getPatch(patch);
      ...
      for (edge = 0; edge < 4; edge++) {
        ...
        if (bindInfo[edge].NPatchs > 0) {
          ...
          // Same zone ?
          if ((listZone[zone] == _ZoneToLight) &&
              (bindInfo[edge].Zone->getZoneId() != _ZoneToLight)) {
            // oversample this edge
  ...
  if (_WaterShapes.size() != 0) // any water shape in this zone ?
  {
    ...
    makeQuadGridFromWaterShapes(
        landscape.getZone(_ZoneToLight)->getZoneBB().getAABBox());
  ...
  for (leave = 0; leave < leavesCount; leave++) {
    ...
    if (face->Patch->getZone()->getZoneId() == _ZoneToLight) {
      ...
      CVector interpolatedP[10] = {
          (pos[0] + pos[6]) / 2, (pos[4] + pos[6]) / 2, (pos[4] + pos[8]) / 2,
          (pos[1] + pos[8]) / 2, (pos[5] + pos[6]) / 2, (pos[3] + pos[6]) / 2,
          (pos[3] + pos[8]) / 2, (pos[5] + pos[7]) / 2, (pos[3] + pos[7]) / 2,
          (pos[2] + pos[7]) / 2,
      };
      ...
      for (i = 0; i < 8; i++) {
        ...
        if (snapedBorder[VertexThanCanBeSnappedOnABorder[i][1]]) {
          ...
          interpolatedP[VertexThanCanBeSnappedOnABorder[i][0]] =
              (pos[VertexThanCanBeSnappedOnABorder[i][2]] +
               pos[VertexThanCanBeSnappedOnABorder[i][3]]) /
              2;
        }
      }
      ...
      for (i = 0; i < 3; i++) {
        ...
        if (snapedCorner[tesselCornerIndex]) {
          // Compute the border vertex
          interpolatedP[VertexThanCanBeSnappedOnACorner[i][0]] =
              pos[tesselCornerIndex];
        }
      }

      float interpolatedS[10] = {
          (s0 + s6) / 2, (s4 + s6) / 2, (s4 + s8) / 2, (s1 + s8) / 2,
          (s5 + s6) / 2, (s3 + s6) / 2, (s3 + s8) / 2, (s5 + s7) / 2,
          (s3 + s7) / 2, (s2 + s7) / 2,
      };

      float interpolatedT[10] = {
          (t0 + t6) / 2, (t4 + t6) / 2, (t4 + t8) / 2, (t1 + t8) / 2,
          (t5 + t6) / 2, (t3 + t6) / 2, (t3 + t8) / 2, (t5 + t7) / 2,
          (t3 + t7) / 2, (t2 + t7) / 2,
      };
  ...
  for (patch = 0; patch < patchCount; patch++) {
    ...
    // Get a patch pointer
    const CPatch *pPatch =
        (const_cast<const CZone *>(landscape.getZone(_ZoneToLight)))
            ->getPatch(patch);
    uint orderS = pPatch->getOrderS();
    uint orderT = pPatch->getOrderT();
  ...
  for (leave = 0; leave < leavesCount; leave++) {
    ...
    if (face->Patch->getZone()->getZoneId() == _ZoneToLight) {
      ...
      // Coordinates
      float fS =
          (face->PVBase.getS() + face->PVLeft.getS() + face->PVRight.getS()) /
          3.f;
      float fT =
          (face->PVBase.getT() + face->PVLeft.getT() + face->PVRight.getT()) /
          3.f;
      ...
      // Normal
      CPlane plane;
      plane.make(face->VBase->EndPos, face->VLeft->EndPos,
                 face->VRight->EndPos);
  ...
  for (patch = 0; patch < patchCount; patch++) {
    ...
    for (uint edge = 0; edge < 4; edge++) {
      ...
      for (uint lumel = 0; lumel <= count; lumel++) {
        ...
        getNormal(pPatch, startS, startT, locator, bindInfo, binded,
                  visitedLumels, startS + 0.5f - origineS,
                  startT + 0.5f - origineT, 0, bezierPatch);
...
#define BLUR_SIZE 4
      for (uint i = 1; i < BLUR_SIZE; i++) {
        ...
        normals[i][edge] =
            normals[0][edge] * (1 - value) + normals[i][edge] * value;
        normals[i][edge].normalize();
        normals[count - i][edge] = normals[count][edge] * (1 - value) +
                                   normals[count - i][edge] * value;
    ...
    for (uint t = 0; t < lumelT; t++)
      for (uint s = 0; s < lumelS; s++) {
        ...
        // For each edge
        CVector normalS =
            bezierPatch.evalNormal(((float)s + 0.5f) / (float)lumelS,
                                   ((float)t + 0.5f) / (float)lumelT);
        float sFactor = 0;
        ...
        if ((sGood) && (tGood)) {
          if ((sFactor != BLUR_SIZE) || (tFactor != BLUR_SIZE))
            smoothNormal = normalS * (BLUR_SIZE - tFactor) +
                           normalT * (BLUR_SIZE - sFactor);
          else
        ...
        // Final normal
        lumels[lumelIndex].Normal =
            lumels[lumelIndex].Normal - purNormal + smoothNormal;
        lumels[lumelIndex].Normal.normalize();
...
// ***************************************************************************
void CZoneLighter::computeTileFlagsOnly(CLandscape &landscape, CZone &output,
                                        uint zoneToLight,
                                        const CLightDesc &description,
  ...
  if (zonew.getNumPatchs() == output.getNumPatchs()) {
    ...
    for (uint i = 0; i < (uint)zonew.getNumPatchs(); i++) {
      ...
      if (p0.getOrderS() != p1.getOrderS() ||
          p0.getOrderT() != p1.getOrderT()) {
        ok = false;
        break;
      }
    }
  ...
  if (!ok)
    throw Exception("The input zonew, and output zonel are too different: not "
                    "same patchs!!");
...
bool CZoneLighter::isLightableShape(IShape &shape) {
  ...
  if (dynamic_cast<CWaterShape *>(&shape) != NULL) {
    // check that this water surface has a diffuse map that is a CTextureFile
...
void CZoneLighter::lightShapes(uint zoneID, const CLightDesc &description) {
  ...
  for (uint k = 0; k < _LightableShapes.size(); ++k, ++process) {
    ...
    IThread *pThread = IThread::create(new CCalcLightableShapeRunnable(
        process, this, &description, &_LightableShapes, currShapeIndex,
        lastShapeIndex));
    pThread->start();
...
void CZoneLighter::processLightableShapeCalc(uint process,
  ...
  for (uint k = firstShape; k < lastShape; ++k) {
    nlassert(isLightableShape(
        *(*shapesToLit)[k].Shape)); // make sure it is a lightable shape
...
void CZoneLighter::lightSingleShape(CShapeInfo &si,
                                    const CLightDesc &description, uint cpu) {
  ...
  progress("Processing lightable shapes",
           (float)_NumLightableShapesProcessed / _LightableShapes.size());
  return;
}
...
// ***************************************************************************
void CZoneLighter::lightWater(CWaterShape &ws, const CMatrix &MT,
                              const CLightDesc &description, uint cpu) {
  try {
    /// get the diffuse map
    CTextureFile *diffuseTex =
        NLMISC::safe_cast<CTextureFile *>(ws.getColorMap());
    ...
    for (sint x = iMinU; x < iMaxU; ++x) {
      for (sint y = iMinV; y < iMaxV; ++y) {
        float factor;
        NLMISC::CVector pos =
            UVSpaceToWorldSpace * NLMISC::CVector(x + 0.5f, y + 0.5f, 0) +
        ...
        if (description.SkyContributionForWater) {
          factor += getSkyContribution(pos, NLMISC::CVector::K,
                                       description.SkyIntensity);
        }
        clamp(factor, 0.f, 1.f);
        uint intensity = (uint8)(255 * factor);
        NLMISC::CRGBA srcCol(intensity, intensity, intensity, 255);
    ...
    if (getExt(texFileName) != ".tga") {
      nlwarning("Zone lighter : error when lighting a water surface : input "
                "bitmap is not a tga file");
    } else {
      try {
        COFile of;
        of.open(texFileName);
        ...
        nlwarning(
            "Zone lighter : while lighting a water shape, writing %s failed! ",
            texFileName.c_str());
      }
    }
...
void CZoneLighter::makeQuadGridFromWaterShapes(NLMISC::CAABBox zoneBBox) {
  ...
  for (TShapeVect::iterator it = _WaterShapes.begin(); it != _WaterShapes.end();
       ++it, ++count) {
    ...
    progress("Building quadtree from water surfaces",
...
// ***************************************************************************
CZoneLighter::CPointLightRT::CPointLightRT() { RefCount = 0; }
...
// ***************************************************************************
void CZoneLighter::compilePointLightRT(uint gridSize, float gridCellSize,
                                       std::vector<CTriangle> &obstacles,
                                       bool doShadow) {
  uint i;
  ...
  if (doShadow) {
    ...
    if (!_StaticPointLights.empty()) {
      ...
      for (i = 0; i < _StaticPointLights.size(); i++) {
        // progress
        progress("Compute Influences of PointLights",
                 0.5f * i / (float)(_StaticPointLights.size() - 1));

        CPointLightRT &plRT = _StaticPointLights[i];
        // Create the cubeGrid
        plRT.FaceCubeGrid.create(plRT.PointLight.getPosition(),
                                 NL3D_ZONE_LIGHTER_CUBE_GRID_SIZE);
        ...
        if (plRT.PointLight.getType() != CPointLight::AmbientLight) {
          ...
          while (itObstacle != obstacleGrid.end()) {
            ...
            if (triBbox.intersect(bbox)) {
              // Test BackFace culling. Only faces which are BackFace the point
              // light are inserted. This is to avoid AutoOccluding problems
...
// ***************************************************************************
bool CZoneLighter::CPredPointLightToPoint::operator()(
    CPointLightRT *pla, CPointLightRT *plb) const {
  float ra = (pla->BSphere.Center - Point).norm();
  float rb = (plb->BSphere.Center - Point).norm();
  float infA =
      (pla->PointLight.getAttenuationEnd() - ra) * pla->OODeltaAttenuation;
  float infB =
      (plb->PointLight.getAttenuationEnd() - rb) * plb->OODeltaAttenuation;
...
void CZoneLighter::processZonePointLightRT(
    vector<CPointLightNamed> &listPointLight) {
  uint i;
  ...
  for (i = 0; i < patchForPLs.size(); i++) {
    // progress
    progress("Compute Influences of PointLights",
             0.5f + 0.5f * i / (float)patchForPLs.size());
    ...
    for (y = 0; y < pfpl.HeightTLI; y++) {
      for (x = 0; x < pfpl.WidthTLI; x++) {
        ...
        // Compute the Vertex, with Noise information (important for accurate
        // raytracing).
        pos = patch->computeVertex(s, t);
        // Use UnNoised normal from BezierPatch, because the lighting does not
        ...
        CQuadGrid<CPointLightRT *>::CIterator it =
            _StaticPointLightQuadGrid.begin();
        while (it != _StaticPointLightQuadGrid.end()) {
          ...
          if ((pl->BSphere.Center - pos) * normal > 0) {
            // Add 5cm else it fails in some case where ( pl->BSphere.Center -
            // pos ) * normal is nearly 0 and the point should be occluded.
            const float deltaY = 0.05f;
            CVector posToRT = pos + normal * deltaY;
            // Test if really in the radius of the light, if no occlusion, and
            // if in SpotAngle
        ...
        // truncate.
        lightInfs.resize(min((uint)lightInfs.size(),
                             (uint)CTileLightInfluence::NumLightPerCorner));
        ...
        for (lightInfId = 0; lightInfId < lightInfs.size(); lightInfId++) {
          ...
          tli.LightFactor[lightInfId] *=
              pl->PointLight.computeLinearAttenuation(pos);

          // Inc RefCount of the light.
          pl->RefCount++;
        }
        // Reset any empty slot to NULL.
        for (; lightInfId < CTileLightInfluence::NumLightPerCorner;
             lightInfId++) {
  ...
  for (i = 0; i < patchForPLs.size(); i++) {
    ...
    for (y = 0; y < h; y++) {
      for (x = 0; x < w; x++) {
        uint tliId = y * w + x;
        // For all light slot
        for (uint lightId = 0; lightId < CTileLightInfluence::NumLightPerCorner;
             lightId++) {
          ...
          if (tliSrc.Light[lightId] == NULL) {
            ...
            // Get Diffuse Factor.
            tliDst.setDiffuseLightFactor(
                lightId, (uint8)(tliSrc.LightFactor[lightId] * 255));
          }
        }
      }
    }
...
static inline bool operator<(const CTileOfPatch &lhs, const CTileOfPatch &rhs) {
  return lhs.Patch == rhs.Patch ? lhs.TileId < rhs.TileId
                                : lhs.Patch < rhs.Patch;
};
...
void CZoneLighter::computeTileFlagsForPositionTowardWater(
    const CLightDesc &lightDesc, std::vector<const CTessFace *> &tessFaces) {
  uint numTileAbove = 0;
  uint numTileBelow = 0;
  ...
  for (std::vector<const CTessFace *>::iterator it = tessFaces.begin();
       it != tessFaces.end(); ++it, ++triCount) {
    /// does the face belong to the zone to light ?
    if ((*it)->Patch->getZone()->getZoneId() != _ZoneToLight)
      continue;
    /// if the tile flags say that micro vegetation is disabled, just skip that
    if ((*it)->Patch->Tiles[(*it)->TileId].getVegetableState() ==
    ...
    if (tileIt == tiles.end()) // first time ?
    {
      ...
      b.extend(b.getMax() + lightDesc.VegetableHeight *
                                NLMISC::CVector::K); // adds vegetable height
      tiles[top] = b;
  ...
  for (TTileOfPatchMap::iterator tileIt = tiles.begin(); tileIt != tiles.end();
       ++tileIt, ++tileCount) {
    ...
    _WaterShapeQuadGrid.select(tileIt->second.getMin(),
                               tileIt->second.getMax());

    CTileElement &te =
        tileIt->first.Patch
            ->Tiles[tileIt->first.TileId]; // alias to the current tile element

    /// test more accurate intersection for each water shape
    TWaterShapeQuadGrid::CIterator qgIt;
    for (qgIt = _WaterShapeQuadGrid.begin(); qgIt != _WaterShapeQuadGrid.end();
         ++qgIt) {
    ...
    if ((tileCount % 50) == 0) {
      progress("Computing tile position towards water",
               (float)tileCount / totalTileCount);
    }
  }
...
float CZoneLighter::attenuation(const CVector &pos,
  ...
  for (sample = 0; sample < samples; sample++) {
    ...
    // Get the z
    float random = (float)_Random.rand() * description.SoftShadowJitter +
                   _Random.RandMax * (1.f - description.SoftShadowJitter);
    averageAttenuation +=
        random * testZPercentageCloserFilter(
                     zPos.x - (float)zbuffer.LocalZBufferXMin,
                     zPos.y - (float)zbuffer.LocalZBufferYMin, zPos.z, zbuffer,
                     description, _ZBufferOverflow);
    randomSum += random;
  }
  ...
  const float objectAttenuation = testZPercentageCloserFilter(
      zPos.x - (float)_ZBufferObject.LocalZBufferXMin,
      zPos.y - (float)_ZBufferObject.LocalZBufferYMin, zPos.z, _ZBufferObject,
      description, _ZBufferOverflow);

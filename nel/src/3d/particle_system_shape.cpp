...
CParticleSystemShape::CParticleSystemShape()
    : _MaxViewDist(100.f), _DestroyWhenOutOfFrustum(false),
      _DestroyModelWhenOutOfRange(false), _UsePrecomputedBBox(false),
      _Sharing(false), _NumBytesWanted(0) {
  /* ***********************************************
  ...
  _DefaultTriggerTrack.setDefaultValue(
      true); // by default, system start as soon as they are instanciated
}
...
void CParticleSystemShape::serial(NLMISC::IStream &f) {
  ...
  if (f.isReading()) {
    ...
    f.serialBufferWithSize((uint8 *)_ParticleSystemProto.buffer(),
                           _ParticleSystemProto.length());
  }
...
///===========================================================================
CParticleSystem *CParticleSystemShape::instanciatePS(
    CScene &scene,
    NLMISC::CContiguousBlockAllocator *blockAllocator /*= NULL*/) {
  if (_Sharing &&
      _SharedSystem !=
          NULL) // is sharing enabled, and is a system already instanciated
  {
    return _SharedSystem;
  }

  // avoid prb with concurrent thread (may happen if an instance group
  // containing ps is loaded in background)
  ...
  if (blockAllocator) {
    ...
    blockAllocator->init(
        _NumBytesWanted); // if size wanted is already known, set it
  }
#endif
  ...
  /*	NLMISC::TTicks end = NLMISC::CTime::getPerformanceTime();
      nlinfo("instanciation time = %.2f", (float) (1000 *
  ...
  */
  ...
  if (blockAllocator) {
    _NumBytesWanted =
        blockAllocator
            ->getNumAllocatedBytes(); // now we know the number of wanted bytes,
                                      // subsequent alloc can be much faster
...
CTransformShape *CParticleSystemShape::createInstance(CScene &scene) {
  CParticleSystemModel *psm = NLMISC::safe_cast<CParticleSystemModel *>(
      scene.createModel(NL3D::ParticleSystemModelId));
  psm->Shape = this;
  psm->_Scene = &scene; // the model needs the scene to recreate the particle
                        // system he holds
  // by default, we don't instanciate the system. It will be instanciated only
  // if visible and triggered psm->_ParticleSystem = instanciatePS(scene);
...
void CParticleSystemShape::render(IDriver *drv, CTransformShape *trans,
  ...
  if (passOpaque) {
    ...
    /*NLMISC::TTicks end = NLMISC::CTime::getPerformanceTime();
    nlinfo("Solid render time time = %.2f", (float) (1000 *
    NLMISC::CTime::ticksToSecond(end - start))); nlinfo("LookAt Render time =
    %.2f", (float) (1000 * NLMISC::CTime::ticksToSecond(PSLookAtRenderTime)));
  */
  } else {
    // PSLookAtRenderTime = 0;
    // NLMISC::TTicks start = NLMISC::CTime::getPerformanceTime();
    ...
    /*NLMISC::TTicks end = NLMISC::CTime::getPerformanceTime();
    nlinfo("Blend render time time = %.2f", (float) (1000 *
    NLMISC::CTime::ticksToSecond(end - start))); nlinfo("LookAt Render time =
    %.2f", (float) (1000 * NLMISC::CTime::ticksToSecond(PSLookAtRenderTime)));
  */
  }
...
///===========================================================================
void CParticleSystemShape::flushTextures(IDriver &driver,
  ...
  if (_SharedSystem) {
    ...
    PSBlockAllocator = &blockAllocator;
    blockAllocator.init(
        300000); // we release memory just after, and we don't want to fragment
                 // the memory, so provide large enough mem
#endif
    // serialize from the memory stream
    if (!_ParticleSystemProto
             .isReading()) // we must be sure that we are reading the stream
    ...
    _NumBytesWanted =
        blockAllocator
            .getNumAllocatedBytes(); // next allocation will be fast because we
    ...
    /*
        ...
        for(uint k = 0; k < myInstance->getNbProcess(); ++k)
        {
            ...
            for(uint l = 0; l < loc->getNbBoundObjects(); ++l)
            {
                ...
                {
                    nlwarning("PS %s uses central gravity",
    myInstance->getName().c_str()); break;
                }
            }
        }
    #endif */

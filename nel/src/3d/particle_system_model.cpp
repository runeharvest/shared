...
CParticleSystemModel::CParticleSystemModel()
    : _ParticleSystem(NULL), _Scene(NULL), _EllapsedTime(0.01f),
      _EllapsedTimeRatio(1.f), _AnimType(CParticleSystem::AnimVisible),
      _AutoGetEllapsedTime(true), _ToolDisplayEnabled(false),
      _TransparencyStateTouched(true), _LightableStateTouched(true),
      _EditionMode(false), _Invalidated(false), _InsertedInVisibleList(false),
      _InClusterAndVisible(false), _EmitterActive(true), _SoundActive(true),
      _BypassGlobalUserParam(0), _UserColor(CRGBA::White), _ZBias(0.f),
...
void CParticleSystemModel::setEditionMode(bool enable /*= true*/) {
  if (enable) {
    /// we need to have the system resources instanciated if we want to work
    /// with it
    if (!_ParticleSystem) {
      nlassert(_Scene);
      nlassert(Shape);
...
void CParticleSystemModel::registerPSModelObserver(IPSModelObserver *obs) {
  ...
  nlassert(
      !isPSModelObserver(obs)); // this observer has already been registered
...
void CParticleSystemModel::removePSModelObserver(IPSModelObserver *obs) {
  ...
  std::vector<IPSModelObserver *>::iterator it =
...
bool CParticleSystemModel::isPSModelObserver(IPSModelObserver *obs) {
  return std::find(_Observers.begin(), _Observers.end(), obs) !=
         _Observers.end();
}
...
void CParticleSystemModel::registerBasic() {
  // register the model
  CScene::registerModel(ParticleSystemModelId, TransformShapeId,
...
///=====================================================================================
/// Called when the resource (attached system) for this system must be
/// reallocated
void CParticleSystemModel::reallocRsc() {
  ...
  CParticleSystemShape *shape =
      NLMISC::safe_cast<CParticleSystemShape *>((IShape *)Shape);
  if (shape->isShared()) {
...
#else
  _ParticleSystem = NLMISC::safe_cast<CParticleSystemShape *>((IShape *)Shape)
  ...
  // touch user params animated value. If the system rsc have been released
  // before, this force to restore them
  for (uint k = 0; k < MaxPSUserParam; ++k) {
  ...
  //
  if (!_EmitterActive)
    _ParticleSystem->activateEmitters(false);
  if (!_SoundActive)
    _ParticleSystem->stopSound();
  //
  if (_ZBias != 0.f)
    _ParticleSystem->setZBias(_ZBias);
}
...
void CParticleSystemModel::releasePSPointer() {
  ...
  if (numRefs == 1) {
    // Backup user params (in animated value) so that they will be restored when
    // the system is recreated
    for (uint k = 0; k < MaxPSUserParam; ++k) {
  ...
  if (_ParticleSystem->getAnimType() == CParticleSystem::AnimAlways) {
    if (_AnimatedModelHandle.Valid) {
      _Scene->getParticleSystemManager().removePermanentlyAnimatedSystem(
          _AnimatedModelHandle);
    }
  }
  //
...
///=====================================================================================
void CParticleSystemModel::refreshRscDeletion(
    const std::vector<CPlane> &worldFrustumPyramid,
   ...
   * Simply because the clip traversal is not called if the cluster it is
   * inserted in is not parsed. This is not good, because we want to keep few
   * CParticleSystem instance. This method solve that problem. This is called by
   * the particle system manager when each scene has rendered
   */
...
void CParticleSystemModel::releaseRscAndInvalidate() {
  ...
  for (std::vector<IPSModelObserver *>::iterator it = copyVect.begin();
       it != copyVect.end(); ++it) {
    (*it)->invalidPS(
        this); // if this crash, then you forgot to call removePSModelObserver !
  }
#ifdef PS_FAST_ALLOC
  CParticleSystemShape *shape =
      NLMISC::safe_cast<CParticleSystemShape *>((IShape *)Shape);
...
ITrack *CParticleSystemModel::getDefaultTrack(uint valueId) {
  ...
  CParticleSystemShape *pss =
      NLMISC::safe_cast<CParticleSystemShape *>((IShape *)Shape);

  switch (valueId) {
  case PosValue:
    return pss->getDefaultPos();
  case RotQuatValue:
    return pss->getDefaultRotQuat();
  case ScaleValue:
    return pss->getDefaultScale();
  }
  if (valueId < OwnerBit)
    return CTransformShape::getDefaultTrack(valueId); // delegate to parent
...
///=====================================================================================
void CParticleSystemModel::registerToChannelMixer(
    CChannelMixer *chanMixer, const std::string &prefix /* =std::string() */) {
...
///=========================================================================================
bool CParticleSystemModel::checkAgainstPyramid(
    const std::vector<CPlane> &pyramid) const {
...
void CParticleSystemModel::traverseAnimDetail() {
  ...
  if (_Invalidated)
    return;
  if (getVisibility() == CHrcTrav::Hide)
    return;

  {
    ...
    // check for trigger. If the trigger is false, and there is a system
    // instanciated, we delete it.
    if (!_EditionMode) {
...

  {
    ...
    // the system or its center is in the view frustum, but it may not have been
    // instanciated from its shape now
    if (!ps) {
      nlassert(_Scene);
      nlassert(Shape);
      reallocRsc();
...

  {
    ...
    if (_InClusterAndVisible ||
        ps->getAnimType() == CParticleSystem::AnimInCluster) {
      bool animate = true;
      if (ps->isSharingEnabled()) /// with shared system, we only animate one
                                  /// version!
      {
      ...
      if (animate) {
        if (ps->getAnimType() !=
            CParticleSystem::AnimAlways) // if the animation is always
                                         // perfomred, then animation is done by
                                         // the particle system manager just
                                         // before the render trav
        {
          doAnimate();
        }
      }
...
void CParticleSystemModel::doAnimate() {
  ...
  //
  {
    ...
    // Set the 'hide' flag. This prevent trails from being created is the system
    // is hidden, moved, and then showed in the next frame.
...

  {
    ...
    if (isAutoGetEllapsedTimeEnabled()) {
      setEllapsedTime(ps->getScene()->getEllapsedTime() *
                      getEllapsedTimeRatio());
    }
  }
  {
  ...
  if (_ParticleSystem) {
    if (CTransform::isLightable()) {
      ...
      for (uint k = 0; k < NL3D_MAX_LIGHT_CONTRIBUTION; ++k) {
        if (lc.PointLight[k] == NULL)
          break;
        NLMISC::CRGBA currLightContrib;
        currLightContrib.modulateFromui(lc.PointLight[k]->getDiffuse(),
                                        lc.AttFactor[k]);
 ...
 * IMPORTANT : the _Visible attribute is interpreted as 'in traversed clusters'.
 * We need this because we want to know when a p.s is in clusters, but not
 * visible. As a matter of fact we may need to have system that are animated as
 * long as in cluster, but not visible.
 */
...
void CParticleSystemModel::traverseClip() {
  ...
  // forest) H_AUTO ( NL3D_Particles_Clip );
  ...
  if (ps) // system instanciated
  {
    ...
    if (checkDestroyCondition(ps))
      return;
    // check for anim mode change
    if (_AnimType != ps->getAnimType()) {
      CParticleSystemManager &psmgt = _Scene->getParticleSystemManager();
      if (_AnimType ==
          CParticleSystem::AnimAlways) // was previously always animated ?
      {
        if (_AnimatedModelHandle.Valid) {
  ...
  if (_AncestorSkeletonModel != NULL) {
    ...
    if (_Visible && _FatherSkeletonModel) {
      ...
      if (_FatherSkeletonModel->isDisplayedAsLodCharacter()) {
        ...
        // forceCLod==true. This is also true if we are actually a skeletonModel
    ...
    } else // not visible, may need animation however..
    {
      if (!ps) // no resc allocated
      {
        ...
        // NB : The test to see whether the system is not too far is performed
        // by the particle system manager
        if (!_EditionMode) {
          {
            MINI_TIMER(PSStatsInsertInVisibleList)
            insertInVisibleList();
  ...
  /** traverse the sons
   * we must do this before us, because this object may delete himself from the
   * scene
   */
  ...
  if (!ps) ///====================== system resource not allocated, test if it
           ///entered the scope
  {
    ...
    // the system wasn't present the last time, we use its center to see if it's
    // back in the view frustum, or if it is near enough. if this is the case,
    // we say it isn't clipped, so it will be reinstanciated from the shape
    ...
    if (!pss->_UsePrecomputedBBox) {
      ...
      for (sint i = 0; i < (sint)pyramid.size(); i++) {
        if ((pyramid[i] * mat).d >
            0.0f) // in its basis, the system is at the center
  ...
  nlassert(ps);
  /// Pyramid test. IMPORTANT : The test to see whether the system is not too
  /// far is performed by the particle system manager
  // In edition mode, it isn't done by the manager (system never removed), so we
  // do it here in this case
  if (_EditionMode) {
    CParticleSystemShape *pss =
        NLMISC::safe_cast<CParticleSystemShape *>((IShape *)Shape);
    nlassert(pss);
...
bool CParticleSystemModel::checkDestroyCondition(CParticleSystem *ps) {
  ...
  nlassert(ps);
  if (!_EditionMode) {
    /** NB : we don't do this test here for always animated system, as it is
     * done by the CParticleSystemManager, because this code is not sure to be
     * executed if the system has been clipped by a cluster
     */
...
//===================================================================
void CParticleSystemModel::bypassGlobalUserParamValue(uint userParamIndex,
                                                      bool byPass /*=true*/) {
  nlctassert(MaxPSUserParam < 8); // there should be less than 8 parameters
                                  // because of mask stored in a byte
  nlassert(userParamIndex < MaxPSUserParam);
  if (byPass)
    _BypassGlobalUserParam |= (1 << userParamIndex);
  else
    _BypassGlobalUserParam &= ~(1 << userParamIndex);
}

//===================================================================
bool CParticleSystemModel::isGlobalUserParamValueBypassed(
    uint userParamIndex) const {
  nlctassert(MaxPSUserParam < 8); // there should be less than 8 parameters
                                  // because of mask stored in a byte
  nlassert(userParamIndex < MaxPSUserParam);
...
void CParticleSystemModel::activateEmitters(bool active) {
  if (active == _EmitterActive)
    return;
  _EmitterActive = active;
  if (_ParticleSystem)
    _ParticleSystem->activateEmitters(active);
}
...
void CParticleSystemModel::setUserColor(NLMISC::CRGBA userColor) {
  if (_ParticleSystem)
    _ParticleSystem->setUserColor(userColor);
...
//===================================================================
void CParticleSystemModel::forceSetUserMatrix(
    const NLMISC::CMatrix &userMatrix) {
...
void CParticleSystemModel::reactivateSound() {
  if (_SoundActive)
    return;
  if (_ParticleSystem)
    _ParticleSystem->reactivateSound();
  _SoundActive = true;
}

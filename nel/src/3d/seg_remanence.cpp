...
CSegRemanence::CSegRemanence()
    : _NumSlices(0), _Started(false), _Stopping(false), _Restarted(false),
      _UnrollRatio(0), _SliceTime(0.05f), _AniMat(NULL), _LastSampleFrame(0) {
...
//===============================================================
CSegRemanence::CSegRemanence(CSegRemanence &other)
    : CTransformShape(other), _AniMat(NULL) {
  copyFromOther(other);
}
...
void CSegRemanence::copyFromOther(CSegRemanence &other) {
  ...
  CAnimatedMaterial *otherMat =
      other._AniMat != NULL ? new CAnimatedMaterial(*other._AniMat) : NULL;
  delete _AniMat;
  _AniMat = otherMat;
...
void CSegRemanence::registerBasic() {
  CScene::registerModel(SegRemanenceShapeId, TransformShapeId,
...
void CSegRemanence::render(IDriver *drv, CMaterial &mat) {
  ...
  CSegRemanenceShape *srs =
      NLMISC::safe_cast<CSegRemanenceShape *>((IShape *)Shape);
...
// draw wire frame version if needed
  ...
  if (!_Stopping) {
    if (_UnrollRatio != 1.f)
      _UnrollRatio =
          std::min(1.f, _UnrollRatio + scene->getEllapsedTime() /
                                           (srs->getNumSlices() * _SliceTime));
  } else {
    _UnrollRatio = std::max(
        0.f, _UnrollRatio - srs->getRollupRatio() * scene->getEllapsedTime() /
                                (srs->getNumSlices() * _SliceTime));
    if (_UnrollRatio == 0.f) {
      _Stopping = false;
      _Started = false;
...
void CSegRemanence::setupFromShape() {
  CSegRemanenceShape *srs =
      NLMISC::safe_cast<CSegRemanenceShape *>((IShape *)Shape);
  if (srs->getNumCorners() != _NumCorners ||
      srs->getNumSlices() != _NumSlices) {
...
void CSegRemanence::samplePos(double date) {
  ...
  if (!_Restarted) {
    if (offset > 0) {
      ...
      if (offset < _NumSlices + 1) {
        ...
        memmove(&_Pos[_NumCorners * offset], &_Pos[0],
                sizeof(_Pos[0]) * _NumCorners * (_NumSlices + 1 - offset));
      }
  ...
  while (currDate > _Samples[2].Date && head != endPtr) {
    ...
    for (uint k = 0; k < _NumCorners; ++k) {
      *head++ = lambda * (_Samples[3].Pos[k] - _Samples[2].Pos[k]) +
                _Samples[2].Pos[k];
    }
    currDate -= _SliceTime;
  }
  if (head != endPtr) {
      ...
      if (dt == 0) {
        ...
        for (uint k = 0; k < _NumCorners; ++k) {
          if (_Samples[2].Date != _Samples[0].Date) {
            T0 = (float)dt * (_Samples[2].Pos[k] - _Samples[0].Pos[k]) /
                 (float)(_Samples[2].Date - _Samples[0].Date);
          } else {
            T0 = NLMISC::CVector::Null;
          }
          if (_Samples[3].Date != _Samples[1].Date) {
            T1 = (float)dt * (_Samples[3].Pos[k] - _Samples[1].Pos[k]) /
                 (float)(_Samples[3].Date - _Samples[1].Date);
          } else {
            T1 = NLMISC::CVector::Null;
          }
          BuildHermiteVector(_Samples[1].Pos[k], _Samples[2].Pos[k], T0, T1,
                             *head, (float)lambda);
          ++head;
        }
      }
    ...
    /*
        ...
        while (currDate >= _Samples[1].Date)
        {
            float lambda = (float) ((currDate - _Samples[1].Date) /
       (_Samples[2].Date - _Samples[1].Date)); for(uint k = 0; k <
       _NumCorners;++k)
            {
                CVector T1 = 0.5f * (_Samples[3].Pos[k] - _Samples[1].Pos[k]);
                CVector T0 = 0.5f * (_Samples[2].Pos[k] - _Samples[0].Pos[k]);
                BuildHermiteVector(_Samples[1].Pos[k], _Samples[2].Pos[k], T0,
       T1, *head, lambda);
                ++ head;
            }
...
//===============================================================
void CSegRemanence::stop() { _Stopping = true; }
...
//===============================================================
void CSegRemanence::registerToChannelMixer(CChannelMixer *chanMixer,
                                           const std::string &prefix) {
  CTransformShape::registerToChannelMixer(chanMixer, prefix);
  if (_AniMat) {
    _AniMat->registerToChannelMixer(chanMixer,
                                    prefix + _AniMat->getMaterialName() + ".");
  }
}
...
ITrack *CSegRemanence::getDefaultTrack(uint valueId) {
  ...
  switch (valueId) {
  case PosValue:
    return srs->getDefaultPos();
  case RotQuatValue:
    return srs->getDefaultRotQuat();
  case ScaleValue:
    return srs->getDefaultScale();
  }
...
#endif
  {
    ...
    if (scene->getNumRender() != (_LastSampleFrame + 1)) {
      if (!isStopping()) {
        ...
        // ribbon started unrolling when it disapperaed from screen so simply
        // remove it
        stopNoUnroll();
      }
    }

...
///==================================================================
void CTextureFile::buildBitmapFromFile(NLMISC::CBitmap &dest,
                                       const std::string &fileName,
                                       bool asyncload, uint8 mipMapSkip,
  ...
  }
  {
    ...
    f.setCacheFileOnOpen(false); // asyncload); //AJM: significant performance
                                 // loss for caching when loading textures
  ...
  if (dest.PixelFormat == RGBA) {
    ...
    if (!file2.empty()) {
      ...
      if ((dest.getWidth() == bitmap.getWidth()) &&
          (dest.getHeight() == bitmap.getHeight())) {
        // Convert in Alpha
        if (bitmap.convertToType(CBitmap::Alpha)) {
          ...
          nlinfo("Can't convert the usercolor texture %s in alpha mode",
                 file2.c_str());
        }
      } else {
        // Error
        nlinfo("User color texture is not the same size than the texture. (Tex "
               ": %s, Usercolor : %s)",
  ...
  if (!isPowerOf2(dest.getWidth()) || !isPowerOf2(dest.getHeight())) {
    ...
    // If the user want to correct those texture so that their canvas is
    // enlarged
    if (enlargeCanvasNonPOW2Tex) {
      ...
      // Bug...
      nlwarning("TextureFile: %s is not a Power Of 2: %d,%d", fileName.c_str(),
...
// ***************************************************************************
void CTextureFile::enableSharing(bool enable) { _SupportSharing = enable; }

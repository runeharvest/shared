...
// ***************************************************************************
CInstanceLighter::CInstanceLighter() { _IGSurfaceLightBuild = NULL; }

// ***************************************************************************
void CInstanceLighter::init() {}

// ***************************************************************************
void CInstanceLighter::addTriangles(CLandscape &landscape,
                                    std::vector<uint> &listZone, uint order,
  ...
  for (uint leave = 0; leave < leavesCount; leave++) {
    ...
    triangleArray.push_back(
        CTriangle(NLMISC::CTriangle(face->VBase->EndPos, face->VLeft->EndPos,
                                    face->VRight->EndPos),
                  -1));
  }
...
// ***************************************************************************
void CInstanceLighter::addTriangles(const IShape &shape,
...
void CInstanceLighter::addTriangles(const CMeshGeom &meshGeom,
    ...
    for (uint pass = 0; pass < numRenderPass; pass++) {
      ...
      if (primitive.getFormat() == CIndexBuffer::Indices16) {
        ...
        for (tri = 0; tri < numTri; tri++) {
          ...
          // Make a triangle
          triangleArray.push_back(
              CTriangle(NLMISC::CTriangle(v0, v1, v2), instanceId));
        }
      } else {
...
void CInstanceLighter::addTriangles(const CMeshMRMGeom &meshGeom,
                                    const CMatrix &modelMT,
                                    std::vector<CTriangle> &triangleArray,
                                    sint instanceId) {
  // Get the vertex buffer
...
void CInstanceLighter::excludeAllPatchFromRefineAll(CLandscape &landscape,
...
// ***************************************************************************
void CInstanceLighter::light(const CInstanceGroup &igIn, CInstanceGroup &igOut,
                             const CLightDesc &lightDesc,
                             std::vector<CTriangle> &obstacles,
                             CLandscape *landscape,
                             CIGSurfaceLightBuild *igSurfaceLightBuild) {
  sint i;
  ...
  nlassert(lightDesc.OverSampling == 0 || lightDesc.OverSampling == 2 ||
           lightDesc.OverSampling == 4 || lightDesc.OverSampling == 8 ||
           lightDesc.OverSampling == 16);
  ...
  //-----------
  // Bkup SurfaceLightBuild to know if must light the surfaces, in differents
  // part of the process.
  ...
  if (_IGSurfaceLightBuild) {
    ...
    while (itSrc != _IGSurfaceLightBuild->RetrieverGridMap.end()) {
      ...
      if (numSurfaces > 0) {
        // Add it to the map,
        CIGSurfaceLight::CRetrieverLightGrid &rlgDst =
            _IGRetrieverGridMap[itSrc->first];
  ...
  for (i = 0; i < (sint)_Instances.size(); i++) {
    ...
    if (toLowerAscii(CFile::getExtension(name)) == "pacs_prim") {
      nlwarning(
          "EXPORT BUG: Can't read %s (not a shape), should not be part of .ig!",
          name.c_str());
      continue;
    }

    // Try to find the shape in the UseShapeMap.
    std::map<string, IShape *>::const_iterator iteMap =
        lightDesc.UserShapeMap.find(name);

    // If not found in userShape map, try to load it from the temp loaded
    // ShapeBank.
    if (iteMap == lightDesc.UserShapeMap.end()) {
      ...
      if (iteMap == shapeMap.end()) {
        ...
        if (!name.empty() && inputFile.open(name)) {
          ...
          // Get the pointer
          iteMap = shapeMap
                       .insert(std::map<string, IShape *>::value_type(
                           name, stream.getShapePointer()))
                       .first;
        } else {
          // Error
    ...
    //-----------
    {
      ...
      if (lightDesc.OverSampling == 0)
        ...
        // Apply an overSampling such that we see 4 points if we look on each
        // side of the bbox.
  ...
  for (i = 0; i < (sint)obstacles.size(); i++) {
    ...
    // Calc the plane
    triangle.Plane.make(triangle.Triangle.V0, triangle.Triangle.V1,
                        triangle.Triangle.V2);
  }

  // Lighting
  //========
  // Light With Sun: build the grid, and do it on all _Instances, using
  // _InstanceInfos Compute also Lighting on surface.
  computeSunContribution(lightDesc, obstacles, landscape);

  // Light With PointLights
  // build the cubeGrids
  compilePointLightRT(lightDesc.GridSize, lightDesc.GridCellSize, obstacles,
                      lightDesc.Shadow);
  ...
  if (_IGSurfaceLightBuild) {
    ...
    igOut.build(outGlobalPos, _Instances, outClusters, outPortals,
                pointLightList, &_IGRetrieverGridMap,
                _IGSurfaceLightBuild->CellSize);
  } else {
...
static void NEL3DCalcBase(CVector &direction, CMatrix &matrix) {
  direction.normalize();
  CVector I = (fabs(direction * CVector(1.f, 0, 0)) > 0.99)
                  ? CVector(0.f, 1.f, 0.f)
                  : CVector(1.f, 0.f, 0.f);
  CVector K = -direction;
  CVector J = K ^ I;
...
// ***************************************************************************
void CInstanceLighter::computeSunContribution(const CLightDesc &lightDesc,
                                              std::vector<CTriangle> &obstacles,
                                              CLandscape *landscape) {
  sint i;
  ...
  // If no Raytrace Shadow, easy,
    ...
    //==========
    for (i = 0; i < (sint)_Instances.size(); i++) {
      progress("Compute SunContribution on Instances",
               i / float(_Instances.size()));
      ...
      if (landscape) {
        ...
        if (VCE->getStaticLightSetup(NLMISC::CRGBA::Black, pos,
                                     dummyPointLightFromLandscape,
                                     landSunContribution, dummyAmbient)) {
    ...
    for (i = 0; i < (sint)obstacles.size(); i++) {
      ...
      // And set the coord in our world, because will be multiplied with
      // invRayBasis in insert()
      quadGrid.insert(rayBasis * triBBox.getMin(), rayBasis * triBBox.getMax(),
                      &obstacles[i]);
    }
...
// ***************************************************************************
CInstanceLighter::CPointLightRT::CPointLightRT() { RefCount = 0; }

// ***************************************************************************
bool CInstanceLighter::CPointLightRT::testRaytrace(const CVector &v,
...
void CInstanceLighter::addStaticPointLight(const CPointLightNamed &pln,
                                           const char *igName) {
  // NB: adding light more than 255 is allowed here, since the important thing
  // is to not overflow really useful lights
...
// ***************************************************************************
void CInstanceLighter::compilePointLightRT(uint gridSize, float gridCellSize,
                                           std::vector<CTriangle> &obstacles,
                                           bool doShadow) {
  uint i;
  ...
  if (doShadow) {
    ...
    for (i = 0; i < _StaticPointLights.size(); i++) {
      // progress
      progress("Compute Influences of PointLights 1/2",
               i / (float)_StaticPointLights.size());

      CPointLightRT &plRT = _StaticPointLights[i];
      // Create the cubeGrid
      plRT.FaceCubeGrid.create(plRT.PointLight.getPosition(),
                               NL3D_INSTANCE_LIGHTER_CUBE_GRID_SIZE);
      ...
      if (plRT.PointLight.getType() != CPointLight::AmbientLight) {
        ...
        while (itObstacle != obstacleGrid.end()) {
          ...
          /* Don't Test BackFace culling Here (unlike in CZoneLighter !!).
             For objects:
              AutoOccluding problem is avoided with _CurrentInstanceComputed
             scheme. Also, With pointLights, there is no multiSampling (since no
             factor stored) Hence we are sure that no Object samples will lies
             under floor, and that the center of the object is far away. For
             IGSurface lighting: notice that we already add 20cm in height
             because of "stairs problem" so floor/surface auto_shadowing is not
             a problem here...
          */
...
// ***************************************************************************
bool CInstanceLighter::CPredPointLightToPoint::operator()(
...
// ***************************************************************************
void CInstanceLighter::processIGPointLightRT(
  ...
  for (i = 0; i < _InstanceInfos.size(); i++) {
    ...
    // progress
    progress("Compute Influences of PointLights 2/2",
             i / (float)_InstanceInfos.size());
    ...
    while (it != _StaticPointLightQuadGrid.end()) {
      ...
      // Test if really in the radius of the light, no occlusion, not an
      // ambient, and in Spot Angle setup
      ...
      if (pl->PointLight.getType() == CPointLight::AmbientLight) {
        ...
        if (dRadius > 0) {
          // Take the best ambient light: the one which is further from the
          // circumference
    ...
    lightInfs.resize(min((uint)lightInfs.size(),
                         (uint)CInstanceGroup::NumStaticLightPerInstance));
    ...
    for (; lightInfId < CInstanceGroup::NumStaticLightPerInstance;
         lightInfId++) {
      ...
      if (cellInfo.InSurface) {
        ...
        while (it != _StaticPointLightQuadGrid.end()) {
          ...
          if (pl->PointLight.getType() == CPointLight::AmbientLight) {
            // If the instance is in radius of the ambiant light.
            float dRadius =
                pl->BSphere.Radius - (pl->BSphere.Center - pos).norm();
            if (dRadius > 0) {
              // Take the best ambient light: the one which is further from the
              // circumference
              if (dRadius > furtherAmbLight) {
        ...
        // truncate.
        lightInfs.resize(min((uint)lightInfs.size(),
                             (uint)CSurfaceLightGrid::NumLightPerCorner));
        ...
        // Reset any empty slot to NULL.
        for (; lightInfId < CSurfaceLightGrid::NumLightPerCorner;
             lightInfId++) {
  ...
  for (i = 0; i < _StaticPointLights.size(); i++) {
    ...
    if (plRT.RefCount > 0) {
      ...
      if (plId <= 0xFF) {
        ...
        // Set 0xFF. Special code indicating that the light CAN'T BE USED => any
        // instance using it is buggy (won't be lighted by this light).
        plRT.DstId = plId++;
      }
    }
  }
  ...
  for (i = 0; i < _Instances.size(); i++) {
    ...
    for (uint lightId = 0; lightId < CInstanceGroup::NumStaticLightPerInstance;
         lightId++) {
    ...
    // Ensure that all FF are at end of the list (possible because of the
    // TooManyStaticLight bug). But don't do a full sort, to preserve order due
    // to influence...
    nlctassert(CInstanceGroup::NumStaticLightPerInstance == 2);
    if (instDst.Light[0] == 0xFF)
      swap(instDst.Light[0], instDst.Light[1]);
  ...
  if (_IGSurfaceLightBuild) {
    ...
    while (!isEndCell()) {
      ...
      if (cellInfo.InSurface) {
        // Do it for PointLights
        for (uint lightId = 0; lightId < CSurfaceLightGrid::NumLightPerCorner;
             lightId++) {
          if (cellInfo.LightInfo[lightId] == NULL) {
            ...
            // Get index. NB: may still be 0xFF if 'Too many static light' bug.
            cell.Light[lightId] =
                reinterpret_cast<CPointLightRT *>(cellInfo.LightInfo[lightId])
                    ->DstId;
          }
        }

        // Ensure that all FF are at end of the list (possible because of the
        // TooManyStaticLight bug). But don't do a full sort, to preserve order
        // due to influence...
        nlctassert(CInstanceGroup::NumStaticLightPerInstance == 2);
        if (cell.Light[0] == 0xFF)
          swap(cell.Light[0], cell.Light[1]);
        ...
        if (cellInfo.LocalAmbientLight == NULL)
          ...
          // NB: may still be 0xFF if 'Too many static light' bug.
          cell.LocalAmbientId =
              ((CPointLightRT *)cellInfo.LocalAmbientLight)->DstId;
      }
...
// ***************************************************************************
void CInstanceLighter::lightIgSimple(CInstanceLighter &instLighter,
  ...
  if (lightDesc.Shadow) {
    ...
    for (i = 0; i < (sint)igIn.getNumInstance(); i++) {
      // progress
      instLighter.progress("Loading Shapes obstacles",
                           float(i) / igIn.getNumInstance());
...
void CInstanceLighter::beginCell() {
  if (_IGSurfaceLightBuild) {
    ...
    if (_ItRetriever != _IGRetrieverGridMap.end()) {
      _ItRetrieverInfo =
          _IGSurfaceLightBuild->RetrieverGridMap.find(_ItRetriever->first);
      nlassert(_ItRetrieverInfo !=
               _IGSurfaceLightBuild->RetrieverGridMap.end());
      // We are suze here that the retriever is not empty, and that the grid
      // herself is not empty too
      _ItSurfId = 0;
      _ItCellId = 0;
...
// ***************************************************************************
bool CInstanceLighter::isEndCell() { return _IsEndCell; }
...
// ***************************************************************************
CSurfaceLightGrid::CCellCorner &
CInstanceLighter::getCurrentNeighborCell(sint xnb, sint ynb) {
...
// ***************************************************************************
CIGSurfaceLightBuild::CCellCorner &
CInstanceLighter::getCurrentNeighborCellInfo(sint xnb, sint ynb) {
  nlassert(isCurrentNeighborCellInSurface(xnb, ynb));

  // get a ref on the current grid.
  CIGSurfaceLightBuild::CSurface &surfGrid =
      _ItRetrieverInfo->second.Grids[_ItSurfId];
  // copute coordinate of the current cellCorner.
  sint xCell, yCell;
  xCell = _ItCellId % surfGrid.Width;
  yCell = _ItCellId / surfGrid.Width;
...
void CInstanceLighter::dilateLightingOnSurfaceCells() {
  ...
  while (!isEndCell()) {
    ...
    if (!cellInfo.InSurface) {
      // Add Weighted influence of SunContribution, and get one of the
      // PointLightContribution (random).
      ...
      for (sint ynb = -1; ynb <= 1; ynb++) {
        for (sint xnb = -1; xnb <= 1; xnb++) {
          ...
          if (isCurrentNeighborCellInSurface(xnb, ynb)) {
            ...
            CIGSurfaceLightBuild::CCellCorner &nbCellInfo =
                getCurrentNeighborCellInfo(xnb, ynb);
            CSurfaceLightGrid::CCellCorner &nbCell =
                getCurrentNeighborCell(xnb, ynb);
            // Add SunContribution.
            wgtSunContribution += nbCell.SunContribution;
            wgtSunCount++;
            // Just Copy PointLight info.
            for (uint lightId = 0;
                 lightId < CSurfaceLightGrid::NumLightPerCorner; lightId++)

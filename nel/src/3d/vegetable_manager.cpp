...
CVegetableManager::CVegetableManager(uint maxVertexVbHardUnlit, uint maxVertexVbHardLighted,
    ...
    , _GlobalDensity(1.f)
    , _NumZSortBlendLayers(nbBlendLayers)
...
*/
// ***********************
const char *NL3D_FastBendProgram = "!!VP1.0																				\n\
...
const char *NL3D_BendProgramP0 = "!!VP1.0																				\n\
	# compute time of animation: time*freqfactor + phase.								\n\
	MAD	R0.x, c[17].x, v[9].z, v[9].y;	# R0.x= time of animation							\n\
																						\n\
	# animation: f(x)= cos(x). compute a high precision cosinus							\n\
...
const char *NL3D_UnlitVegetableProgram = "	MOV o[COL0].xyz, v[3];			# col.RGBA= vertex color							\n\
																						\n\
	MOV o[COL0].w, c[8].y;																\n\
";

// Unlit with AlphaBlend.
const char *NL3D_UnlitAlphaBlendVegetableProgram = "	MOV o[COL0].xyz, v[3];			# col.RGBA= vertex color							\n\
																						\n\
...
*/
// ***********************
const char *NL3D_CommonEndVegetableProgram = "	# compute in Projection space														\n\
	DP4 o[HPOS].x, c[0], R5;															\n\
	DP4 o[HPOS].y, c[1], R5;															\n\
	DP4 o[HPOS].z, c[2], R5;															\n\
	DP4 o[HPOS].w, c[3], R5;															\n\
...
// fogged version
const char *NL3D_VegetableProgramFog = "	DP4	o[FOGC].x, c[6], R5;															\n\
";
...
*/
// ***********************
const char *NL3D_SimpleStartVegetableProgram = "!!VP1.0																				\n\
...
// re-direction, using InstanceVertices;
	...
	if (instanceLighted)
	{
		...
		// Fill instance info
		CVegetableInstanceGroup::CVegetableLightedInstance &vli = vegetRdrPass.LightedInstances[vegetRdrPass.NLightedInstances];
		vli.Shape = shape;
...
bool CVegetableManager::doubleSidedRdrPass(uint rdrPass)
{
	...
	return (rdrPass == NL3D_VEGETABLE_RDRPASS_LIGHTED_2SIDED) || (rdrPass == NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED) || (rdrPass == NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT);
}
    ...
    ITexture *textureDLM, IDriver *driver)
{
	...
	*/
	...
	if (getVBAllocatorForRdrPassAndVBHardMode(NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT, 0).getNumUserVerticesAllocated() > 0 || getVBAllocatorForRdrPassAndVBHardMode(NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT, 1).getNumUserVerticesAllocated() > 0)
	{

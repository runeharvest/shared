...
// ***************************************************************************
CVegetableManager::CVegetableManager(uint maxVertexVbHardUnlit,
    ...
    : _ClipBlockMemory(NL3D_VEGETABLE_CLIP_BLOCK_BLOCKSIZE),
      ...
      _GlobalDensity(1.f), _NumZSortBlendLayers(nbBlendLayers),
      _ZSortLayerDistMax(blendLayerDistMax), _ZSortScene(NULL) {
  uint i;

  // Init all the allocators
  nlassert((uint)(CVegetableVBAllocator::VBTypeCount) == 2);
  _VBHardAllocator[CVegetableVBAllocator::VBTypeLighted].init(
      CVegetableVBAllocator::VBTypeLighted, maxVertexVbHardLighted);
  _VBHardAllocator[CVegetableVBAllocator::VBTypeUnlit].init(
      CVegetableVBAllocator::VBTypeUnlit, maxVertexVbHardUnlit);
  // Init soft one, with no vbHard vertices.
  _VBSoftAllocator[CVegetableVBAllocator::VBTypeLighted].init(
      CVegetableVBAllocator::VBTypeLighted, 0);
  _VBSoftAllocator[CVegetableVBAllocator::VBTypeUnlit].init(
      CVegetableVBAllocator::VBTypeUnlit, 0);
...
void CVegetableManager::createVegetableBlendLayersModels(CScene *scene) {
  ...
  for (uint i = 0; i < _NumZSortBlendLayers; i++) {
    ...
    _ZSortModelLayers[i] = (CVegetableBlendLayerModel *)scene->createModel(
        VegetableBlendLayerModelId);
    _ZSortModelLayersUW[i] = (CVegetableBlendLayerModel *)scene->createModel(
        VegetableBlendLayerModelId);
    // init owner.
...
/*
    ...
    v[4]  == SecondaryColor (==ambient if Lighted, and use only Alpha part for
   DLM if Unlit) v[8]  == Tex0 (xy) v[9]  == BendInfo (xyz) = {BendWeight/2,
   BendPhase, BendFrequencyFactor} NB: /2 because compute a quaternion

    Changes: If unlit, then small changes:
    v[0]  == Pos to center, with v[0].w == BendWeight * v[0].norm()
    v[9]  == BendInfo/BlendInfo (xyzw) = {v[0].norm(), BendPhase,
   BendFrequencyFactor, BlendDist}

    NB: v[9].w. is used only in Unlit+2Sided+AlphaBlend. But prefer do this for
   gestion purpose: to have only one VBAllocator for all modes.

    NB: Color and Secondary color Alpha Part contains Dynamic LightMap UV, (in 8
   bits).
    ...
    NB: DiffuseColor and AmbientColor of vertex must have been pre-multiplied by
   lightColor
    ...
    Fog Note:
    -----------
    Fog should be disabled, because not computed (for speed consideration and
   becasue micro-vegetation should never be in Fog).
...
\n\
    # Taylor serie: cos(x)= 1 - (1/2) a2 + (1/24) a4 - (1/720) a6 + (1/40320)
a8.		\n\
...
/*
    Bend start program:
        Result: bend pos into R5, and R7,R8,R9 is the rotation matrix for
   possible normal lighting.
*/
// ***********************
...
const string NL3D_BendProgram =
    string(NL3D_BendProgramP0) + string(NL3D_BendProgramP1);
...
public:
  ...
  CVertexProgramVeget(uint vpType, bool fogEnabled) {
    // nelvp
    {
      ...
      if (vpType == NL3D_VEGETABLE_RDRPASS_LIGHTED ||
          vpType == NL3D_VEGETABLE_RDRPASS_LIGHTED_2SIDED) {
...
void CVegetableManager::initVertexProgram(uint vpType, bool fogEnabled) {
  nlassert(
      _LastDriver); // update driver should have been called at least once !

  // create VP.
  _VertexProgram[vpType][fogEnabled ? 1 : 0] =
      new CVertexProgramVeget(vpType, fogEnabled);
}
...
// ***************************************************************************
CVegetableSortBlock *
CVegetableManager::createSortBlock(CVegetableClipBlock *clipBlock,
                                   const CVector &center, float radius) {
  nlassert(clipBlock);
...
CVegetableManager::createIg(CVegetableSortBlock *sortBlock) {
  nlassert(sortBlock);
  ...
  // Special Init: The ZSort rdrPass must start with the same HardMode than
  // SortBlock.
  ret->_RdrPass[NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT].HardMode =
      sortBlock->ZSortHardMode;
...
void CVegetableManager::deleteIg(CVegetableInstanceGroup *ig) {
  ...
  for (sint rdrPass = 0; rdrPass < NL3D_VEGETABLE_NRDRPASS; rdrPass++) {
    ...
    CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
        ig->_RdrPass[rdrPass];
    // which allocator?
    CVegetableVBAllocator &vbAllocator =
...
// ***************************************************************************
CVegetableShape *
CVegetableManager::getVegetableShape(const std::string &shape) {
  ...
  // else insert
  {
    ...
    try {
      if (!ret->loadShape(shape)) {
        // Warning
        nlwarning("CVegetableManager::getVegetableShape could not load shape "
                  "file '%s'",
                  shape.c_str());
      ...
      // Warning
      nlwarning("CVegetableManager::getVegetableShape error while loading "
                "shape file '%s' : '%s'",
                shape.c_str(), e.what());
...
// ***************************************************************************
uint CVegetableManager::getRdrPassInfoForShape(
    CVegetableShape *shape, TVegetableWater vegetWaterState,
...
// ***************************************************************************
void CVegetableManager::reserveIgAddInstances(
    CVegetableInstanceGroupReserve &vegetIgReserve, CVegetableShape *shape,
    TVegetableWater vegetWaterState, uint numInstances) {
  ...
  uint rdrPass;
  rdrPass = getRdrPassInfoForShape(shape, vegetWaterState, instanceLighted,
                                   instanceDoubleSided, instanceZSort,
                                   destLighted, precomputeLighting);

  // veget rdrPass
  CVegetableInstanceGroupReserve::CVegetableRdrPass &vegetRdrPass =
      vegetIgReserve._RdrPass[rdrPass];

  // Reserve space in the rdrPass.
  vegetRdrPass.NVertices += numInstances * shape->VB.getNumVertices();
  vegetRdrPass.NTriangles +=
      numInstances * (uint)shape->TriangleIndices.size() / 3;
...
// ***************************************************************************
void CVegetableManager::reserveIgCompile(
    CVegetableInstanceGroup *ig,
  ...
  for (rdrPass = 0; rdrPass < NL3D_VEGETABLE_NRDRPASS; rdrPass++) {
    ...
    uint numLightedInstances =
        vegetIgReserve._RdrPass[rdrPass].NLightedInstances;
    // reserve triangles indices and vertices for this rdrPass.
    vegetRdrPass.TriangleIndices.setFormat(vegetRdrPass.HardMode
  ...
  uint numZSortTris =
      vegetIgReserve._RdrPass[NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT]
          .NTriangles;
  // allocate sufficient space for all quadrants (1 alloc for all quadrants).
  ig->_TriangleQuadrantOrderArray.resize(numZSortTris *
...
inline void computeVegetVertexLighting(const CVector &rotNormal,
                                       const CVector &sunDir, CRGBA primaryRGBA,
                                       CRGBA secondaryRGBA,
                                       CVegetableLightEx &vegetLex,
                                       CRGBA diffusePL[2], CRGBA *dst) {
  float dpSun;
  float dpPL[2];
  CRGBA col;
  CRGBA resColor;
...
// ***************************************************************************
inline void computeVegetVertexLightingForceBestSided(
    const CVector &rotNormal, const CVector &sunDir, CRGBA primaryRGBA,
    CRGBA secondaryRGBA, CVegetableLightEx &vegetLex, CRGBA diffusePL[2],
    CRGBA *dst) {
  float dpSun;
  float dpPL[2];
  CRGBA col;
...
// ***************************************************************************
void CVegetableManager::addInstance(
    CVegetableInstanceGroup *ig, CVegetableShape *shape,
    const NLMISC::CMatrix &mat, const NLMISC::CRGBAF &ambientColor,
    const NLMISC::CRGBAF &diffuseColor, float bendFactor, float bendPhase,
    float bendFreqFactor, float blendDistMax, TVegetableWater vegetWaterState,
    CVegetableUV8 dlmUV) {
  sint i;
  ...
  bool bestSidedPrecomputeLighting =
      precomputeLighting && shape->BestSidedPreComputeLighting;
  ...
  // Copy Dynamic Lightmap UV in Alpha part (save memory for an extra cost of 1
  // VP instruction)
  ...
  if (!vegetRdrPass.HardMode) {
    ...
    if (allocator->exceedMaxVertexInBufferHard(shape->VB.getNumVertices())) {
      // if exceed, then must pass ALL the IG in software mode. vertices/faces
      // are correclty updated. special: if rdrPass is the ZSort one,
      if (rdrPass == NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT) {
        nlassert(ig->_SortOwner->ZSortHardMode);

        // must do it on ALL igs of the sortBlock, for less VBuffer mode
        // switching.
        CVegetableInstanceGroup *pIg =
            ig->_SortOwner->_InstanceGroupList.begin();
        while (pIg) {
        ...
        // just do it on this Ig (can mix hardMode in a SortBlock for normal
        // rdrPass)
  ...
  // dst info
  uint dstNormalOff =
      (destLighted ? dstVBInfo.getValueOffEx(NL3D_VEGETABLE_VPPOS_NORMAL) : 0);
  ...
  // For all vertices of shape, transform and store manager indices in temp
  // shape.
  for (i = 0; i < (sint)numNewVertices; i++) {
    ...
    CVector relPos = mat.mulVector(
        *(CVector *)srcPtr); // mulVector, because translation in v[center]
    ...
    if (shape->BendCenterMode == CVegetableShapeBuild::BendCenterNull)
      ...
      v.z = 0;
      bendCenterPos =
          mat.mulVector(v); // mulVector, because translation in v[center]
    }
    // copy
    ...
    *(CVector *)(dstPtr + dstCenterOff) = instancePos + bendCenterPos;
    ...
    if (!precomputeLighting) {
      ...
      *(CRGBA *)(dstPtr + dstColor0Off) = primaryRGBA;
      ...
      if (destLighted) {
        // normal
        *(CVector *)(dstPtr + dstNormalOff) =
            normalMat.mulVector(*(CVector *)(srcPtr + srcNormalOff));
      }
      ...
      *(CRGBA *)(dstPtr + dstColor1Off) = secondaryRGBA;
      ...
      if (!bestSidedPrecomputeLighting) {
        computeVegetVertexLighting(rotNormal, _DirectionalLight, primaryRGBA,
                                   secondaryRGBA, vegetLex, diffusePL,
                                   (CRGBA *)(dstPtr + dstColor0Off));
      } else {
        computeVegetVertexLightingForceBestSided(
            rotNormal, _DirectionalLight, primaryRGBA, secondaryRGBA, vegetLex,
            diffusePL, (CRGBA *)(dstPtr + dstColor0Off));
      }
    ...
    *(CUV *)(dstPtr + dstTex0Off) = *(CUV *)(srcPtr + srcTex0Off);
    ...
    // setup bend Weight.
    // if !destLighted, then VP is different, vertexBendWeight is stored in
    // v[0].w
    if (destLighted)
  ...
  if (rdrPass == NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT) {
    ...
    // resize the array. Actually only modify the number of triangles really
    // setuped.
    ...
    nlassert(ig->_TriangleQuadrantOrderNumTriangles *
                 NL3D_VEGETABLE_NUM_QUADRANT <=
             ig->_TriangleQuadrantOrderArray.size());

    // compute distance for each quadrant. Since we are not sure of the
    // sortBlockSize, mul with a (big: 16) security. NB: for landscape practical
    // usage, this left us with more than 1mm precision.
    ...
    for (uint quadId = 0; quadId < NL3D_VEGETABLE_NUM_QUADRANT; quadId++) {
      ...
      for (uint i = 0; i < numNewTris; i++) {
        ...
        // compress to sint16.
        ig->_TriangleQuadrantOrders[quadId][offTri + i] =
            (sint16)NLMISC::OptFastFloor(dist * distFactor);
      }
    }
  }
  ...
  nlassert(offTriIdx + numNewIndices <=
           vegetRdrPass.TriangleIndices.getNumIndexes());
  nlassert(offTriIdx + numNewIndices <=
           vegetRdrPass.TriangleLocalIndices.size());

  // insert list of vertices to delete in ig vertices.
  vegetRdrPass.Vertices.copy(offVertex, offVertex + numNewVertices,
                             &shape->InstanceVertices[0]);
...
// re-direction, using InstanceVertices;
  ...
  if (instanceLighted) {
    ...
    nlassert(vegetRdrPass.NLightedInstances <
             vegetRdrPass.LightedInstances.size());
...
// ***************************************************************************
void CVegetableManager::swapIgRdrPassHardMode(CVegetableInstanceGroup *ig,
                                              uint rdrPass) {
  CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
      ig->_RdrPass[rdrPass];

  // the allocator where vertices come from
  CVegetableVBAllocator &srcAllocator =
      getVBAllocatorForRdrPassAndVBHardMode(rdrPass, vegetRdrPass.HardMode);
  // the allocator where vertices will go
  CVegetableVBAllocator &dstAllocator =
      getVBAllocatorForRdrPassAndVBHardMode(rdrPass, !vegetRdrPass.HardMode);
  ...
  uint i;
  // Do it only for current Vertices setuped!!! because a swapIgRdrPassHardMode
  // awlays arise when the ig is in construcion. Hence here, we may have
  // vegetRdrPass.NVertices < vegetRdrPass.Vertices.size() !!!
  ...
  nlassert(vegetRdrPass.TriangleIndices.getNumIndexes() ==
           vegetRdrPass.TriangleLocalIndices.size());
  // Do it only for current Triangles setuped!!! same reason as vertices
  // For all setuped triangles indices
  CIndexBufferReadWrite ibaWrite;
  // For hard mode, uses faster 16 bit indices because the VB is not bigger than
  // 65K
  vegetRdrPass.TriangleIndices.setFormat(
      vegetRdrPass.HardMode
          ? CIndexBuffer::Indices32
          : CIndexBuffer::Indices16); // NB : this is not an error here :
                                      // vegetRdrPass.HardMode has not been
                                      // inverted yet
  vegetRdrPass.TriangleIndices.lock(ibaWrite);
  if (ibaWrite.getFormat() == CIndexBuffer::Indices16) {
    ...
    for (i = 0; i < vegetRdrPass.NTriangles * 3; i++) {
      ...
      // get the index in new VBufffer (dstAllocator), and copy to
      // TriangleIndices
...
// ***************************************************************************
void CVegetableManager::setDirectionalLight(const CRGBA &ambient,
                                            const CRGBA &diffuse,
                                            const CVector &light) {
...
void CVegetableManager::setupVertexProgramConstants(IDriver *driver,
  ...
  driver->setUniformMatrix(
      ...
      _ActiveVertexProgram->getUniformIndex(CProgramIndex::ModelViewProjection),
      IDriver::ModelViewProjection, IDriver::Identity);
  // c[6] take the Fog vector. After setupModelMatrix();
  if (fogEnabled) {
    driver->setUniformFog(
        IDriver::VertexProgram,
        _ActiveVertexProgram->getUniformIndex(CProgramIndex::Fog));
  }
  // c[8] take useful constants.
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().ProgramConstants0, 0, 1,
                       0.5f, 2);
  // c[9] take normalized directional light
  driver->setUniform3f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().DirectionalLight,
                       _DirectionalLight);
  // c[10] take pos of camera
  driver->setUniform3f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().ViewCenter, _ViewCenter);
  // c[11] take factor for Blend formula
  driver->setUniform1f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().NegInvTransDist,
                       -1.f / NL3D_VEGETABLE_BLOCK_BLEND_TRANSITION_DIST);

  // Bend.
  // c[16]= quaternion axis. w==1, and z must be 0
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().AngleAxis, _AngleAxis, 1);
  // c[17]=	{timeAnim, WindPower, WindPower*(1-WindBendMin)/2, 0)}
  driver->setUniform3f(IDriver::VertexProgram, _ActiveVertexProgram->idx().Wind,
                       (float)_WindAnimTime, _WindPower,
                       _WindPower * (1 - _WindBendMin) / 2);
  // c[18]=	High order Taylor cos coefficient: { -1/2, 1/24, -1/720, 1/40320
  // }
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().CosCoeff0, -1 / 2.f,
                       1 / 24.f, -1 / 720.f, 1 / 40320.f);
  // c[19]=	Low order Taylor cos coefficient: { 1, -1/2, 1/24, -1/720 }
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().CosCoeff1, 1, -1 / 2.f,
                       1 / 24.f, -1 / 720.f);
  // c[20]=	Low order Taylor sin coefficient: { 1, -1/6, 1/120, -1/5040 }
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().CosCoeff2, 1, -1 / 6.f,
                       1 / 120.f, -1 / 5040.f);
  // c[21]=	Special constant vector for quatToMatrix: { 0, 1, -1, 0 }
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().QuatConstants, 0.f, 1.f,
                       -1.f, 0.f);
  // c[22]=	{0.5f, Pi, 2*Pi, 1/(2*Pi)}
  driver->setUniform4f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().PiConstants, 0.5f, (float)Pi,
                       (float)(2 * Pi), (float)(1 / (2 * Pi)));
  // c[23]=	{NL3D_VEGETABLE_VP_LUT_SIZE, 0, 0, 0}.
  // NL3D_VEGETABLE_VP_LUT_SIZE==64.
  driver->setUniform1f(IDriver::VertexProgram,
                       _ActiveVertexProgram->idx().LUTSize,
  ...
  for (uint i = 0; i < NL3D_VEGETABLE_VP_LUT_SIZE; i++) {
    ...
    driver->setUniform4f(IDriver::VertexProgram,
                         _ActiveVertexProgram->idx().LUT[i], cur.x, cur.y,
                         delta.x, delta.y);
  }
}

// ***************************************************************************
void CVegetableManager::render(const CVector &viewCenter,
  ...
  */
  ...
  // Clip.
  //--------------------
  // For all current not empty clipBlocks, clip against pyramid, and insert
  // visibles in list.
  ...
  // The manager is identity in essence. But for ZBuffer improvements, must set
  // it as close to the camera. In the VertexProgram, _ViewCenter is substracted
  // from bent vertex pos. So take it as position.
  ...
  // Doing it incrementally allow change of of frequency each frame with good
  // results.
  _WindAnimTime += (_Time - _WindPrecRenderTime) * _WindFrequency;
  _WindAnimTime =
      fmod((float)_WindAnimTime, (float)NL3D_VEGETABLE_FREQUENCY_FACTOR_PREC);
  // NB: Leave timeBend (_WindAnimTime) as a time (ie [0..1]), because VP do a
  // "EXP time". For incremental computing.
  ...
  uint i;
  for (i = 0; i < NL3D_VEGETABLE_VP_LUT_SIZE; i++) {
    /* NB: this formula works quite well, because vertex BendFactor is expressed
       in Radian/2. And since animFactor==(_CosTable[i] + 1) E [0..2], we have
       here an arc-circle computing: dmove= Radius * AngleRadian/2 * animFactor.
       So at max of animFactor (ie 2), we have: dmove= Radius * AngleRadian,
       which is by definition an arc-circle computing... And so this approximate
       the Bend-quaternion Vertex Program.
    */
  ...
  if (textureDLM) {
    ...
    _VegetableMaterial.texEnvArg0RGB(0, CMaterial::Texture,
                                     CMaterial::SrcColor);
    _VegetableMaterial.texEnvArg1RGB(0, CMaterial::Diffuse,
                                     CMaterial::SrcColor);
    // stage1 RGB is Previous * Texture
    _VegetableMaterial.texEnvOpRGB(1, CMaterial::Modulate);
    _VegetableMaterial.texEnvArg0RGB(1, CMaterial::Texture,
                                     CMaterial::SrcColor);
    _VegetableMaterial.texEnvArg1RGB(1, CMaterial::Previous,
                                     CMaterial::SrcColor);
  } else {
    ...
    _VegetableMaterial.texEnvArg0RGB(1, CMaterial::Texture,
                                     CMaterial::SrcColor);
    _VegetableMaterial.texEnvArg1RGB(1, CMaterial::Diffuse,
                                     CMaterial::SrcColor);
  }
  // stage1 Alpha is always "Modulate texture with diffuse Alpha"
  _VegetableMaterial.texEnvOpAlpha(1, CMaterial::Modulate);
  _VegetableMaterial.texEnvArg0Alpha(1, CMaterial::Texture,
                                     CMaterial::SrcAlpha);
  _VegetableMaterial.texEnvArg1Alpha(1, CMaterial::Diffuse,
                                     CMaterial::SrcAlpha);
  ...
  */
  ...
  for (sint vbHardMode = 1; vbHardMode >= 0; vbHardMode--) {
    ...
    for (sint rdrPass = 0; rdrPass < NL3D_VEGETABLE_NRDRPASS; rdrPass++) {
      ...
      // which allocator?
      CVegetableVBAllocator &vbAllocator =
          getVBAllocatorForRdrPassAndVBHardMode(rdrPass, vbHardMode);
      ...
      if (vbAllocator.getNumUserVerticesAllocated() > 0) {
        ...
        while (ptrClipBlock) {
          ...
          CVegetableSortBlock *ptrSortBlock =
              ptrClipBlock->_SortBlockList.begin();
          while (ptrSortBlock) {
            // For all igs of the sortBlock
            CVegetableInstanceGroup *ptrIg =
                ptrSortBlock->_InstanceGroupList.begin();
            while (ptrIg) {
              // rdrPass
              CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
                  ptrIg->_RdrPass[rdrPass];

              // if this rdrPass is in same HardMode as we process now.
              if ((vegetRdrPass.HardMode && vbHardMode == 1) ||
                  (!vegetRdrPass.HardMode && vbHardMode == 0)) {
                // Ok, Render the faces.
                if (vegetRdrPass.NTriangles) {
                  driver->activeIndexBuffer(vegetRdrPass.TriangleIndices);
#ifdef NL_DEBUG
                  if (vegetRdrPass.HardMode) {
                    nlassert(vegetRdrPass.TriangleIndices.getFormat() ==
                             CIndexBuffer::Indices16);
                  } else {
                    nlassert(vegetRdrPass.TriangleIndices.getFormat() ==
                             CIndexBuffer::Indices32);
...
#endif
                  driver->renderSimpleTriangles(0, vegetRdrPass.NTriangles);
                }
              }
  ...
  for (i = 0; i < _NumZSortBlendLayers; i++) {
    ...
    // We must do it here, because if vegetableManger::render() is no more
    // called (eg: disabled), then the models must do nothing.
  ...
  if (getVBAllocatorForRdrPassAndVBHardMode(
          NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT, 0)
              .getNumUserVerticesAllocated() > 0 ||
      getVBAllocatorForRdrPassAndVBHardMode(
          NL3D_VEGETABLE_RDRPASS_UNLIT_2SIDED_ZSORT, 1)
              .getNumUserVerticesAllocated() > 0) {
    ...
    while (ptrClipBlock) {
      ...
      while (ptrSortBlock) {
        ...
        if (ptrSortBlock->_NTriangles != 0) {
          ...
          /* NB: compute radial distance (with norm()) instead of linear
             distance (DotProduct with front vector) get less "ZSort poping".
          */
          ...
          for (uint dirIdx = 0; dirIdx < NL3D_VEGETABLE_NUM_QUADRANT;
               dirIdx++) {
          ...
          p1DebugLines.push_back(ptrSortBlock->_Center +
          CVegetableQuadrant::Dirs[bestDirIdx]);*/
        }
    ...
    // leave AlphaTest but still kick low alpha values (for fillRate
    // performance)
    ...
    for (uint i = 0; i < sortVegetSbs.size(); i++) {
      ...
      // Avoid a floor(), using an OptFastFloor, but without the
      // OptFastFloorBegin() End() group.
      // => avoid the imprecision with such a trick; *256, then divide the
      // integer by 256.
      ...
      if (ptrSortBlock->_UnderWater)
        // range in the correct layermodel (NB: keep the same layer internal
        // order).
  ...
  /*for(uint l=0; l<p0DebugLines.size();l++)
  {
      ...
      CDRU::drawLine(p0DebugLines[l]+dv, p1DebugLines[l]+dv, CRGBA(255,0,0),
  *driver);
  }*/
...
void CVegetableManager::setupRenderStateForBlendLayerModel(IDriver *driver) {
  ...
  // Activate the unique material (correclty setuped for AlphaBlend in
  // render()).
  driver->setupMaterial(_VegetableMaterial);
}
...
// ***************************************************************************
void CVegetableManager::setWind(const CVector &windDir, float windFreq,
                                float windPower, float windBendMin) {
  // Keep only XY component of the Wind direction (because VP only support z==0
  // quaternions).
...
// ***************************************************************************
void CVegetableManager::setUpdateLightingTime(double time) { _ULTime = time; }
...
bool CVegetableManager::updateLightingIGPart() {
  ...
  while (_ULNVerticesToUpdate > 0) {
    ...
    CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
        _ULRootIg->_RdrPass[_ULCurrentIgRdrPass];
    ...
    _ULNVerticesToUpdate -= updateInstanceLighting(
        _ULRootIg, _ULCurrentIgRdrPass, _ULCurrentIgInstance);
...
// ***************************************************************************
uint CVegetableManager::updateInstanceLighting(CVegetableInstanceGroup *ig,
                                               uint rdrPassId,
                                               uint instanceId) {
  nlassert(ig);
  // get the rdrPass.
  ...
  CVegetableInstanceGroup::CVegetableRdrPass &vegetRdrPass =
      ig->_RdrPass[rdrPassId];
  // get the lighted instance.
  nlassert(instanceId < vegetRdrPass.LightedInstances.size());
  CVegetableInstanceGroup::CVegetableLightedInstance &vegetLI =
      vegetRdrPass.LightedInstances[instanceId];
  ...
  diffusePL[1] = CRGBA::Black;
  if (vegetLex.NumLights >= 1) {
    diffusePL[0].modulateFromColorRGBOnly(vegetLI.MatDiffuse,
                                          vegetLex.Color[0]);
    if (vegetLex.NumLights >= 2) {
      diffusePL[1].modulateFromColorRGBOnly(vegetLI.MatDiffuse,
                                            vegetLex.Color[1]);
    }
  }
  ...
  CVegetableVBAllocator *allocator;
  allocator =
      &getVBAllocatorForRdrPassAndVBHardMode(rdrPassId, vegetRdrPass.HardMode);
  const CVertexBuffer &dstVBInfo = allocator->getSoftwareVertexBuffer();

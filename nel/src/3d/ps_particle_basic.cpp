...
CPSParticle::CPSParticle()
    : _DisableAutoLOD(false), _UsesGlobalColorLighting(false) {
...
void CPSParticle::showTool() {
  ...
  const CVector tab[] = {
      2 * J,     I + J,      I + J,      2 * I + J, 2 * I + J,  I,          I,
      2 * I - J, 2 * I - J,  -.5f * J,   -.5f * J,  -2 * I - J, -2 * I - J, -I,
      -I,        -2 * I + J, -2 * I + J, -I + J,    -I + J,     2 * J};
...
void CPSParticle::computeSrcStep(uint32 &step, uint &numToProcess) {
  ...
  if (_DisableAutoLOD || !ps.isAutoLODEnabled() || !ps.isSharingEnabled() ||
      _Owner->getSize() == 0) // Should Auto-LOD be used ?
  {
    step = (1 << 16);
    ...
    if (LODRatio > ps.getAutoLODStartDistPercent()) {
      ...
      step = ps.getAutoLODMode()
                 ? // skip or limit number, depending on the mode
...
void CPSColoredParticle::setColorScheme(CPSAttribMaker<CRGBA> *col) {
  ...
  delete _ColorScheme;
  _ColorScheme = col;
  if (getColorOwner() && col->hasMemory())
    col->resize(getColorOwner()->getMaxSize(), getColorOwner()->getSize());
...
CPSColoredParticle::CPSColoredParticle()
    : _Color(CRGBA(255, 255, 255)),
      _ColorScheme(NULL){NL_PS_FUNC(CPSColoredParticle_CPSColoredParticle)}
...
void CPSSizedParticle::setSizeScheme(CPSAttribMaker<float> *size) {
  ...
  delete _SizeScheme;
  _SizeScheme = size;
  if (getSizeOwner() && size->hasMemory())
    size->resize(getSizeOwner()->getMaxSize(), getSizeOwner()->getSize());
}
...
CPSSizedParticle::CPSSizedParticle()
    : _ParticleSize(0.3f),
      _SizeScheme(NULL){NL_PS_FUNC(CPSSizedParticle_CPSSizedParticle)}
...
///===================================================================================
void CPSRotated2DParticle::setAngle2DScheme(
    CPSAttribMaker<float> *angle2DScheme) {
  NL_PS_FUNC(CPSRotated2DParticle_setAngle2DScheme)
  nlassert(angle2DScheme);
  delete _Angle2DScheme;
  _Angle2DScheme = angle2DScheme;
  if (getAngle2DOwner() && angle2DScheme->hasMemory())
    angle2DScheme->resize(getAngle2DOwner()->getMaxSize(),
                          getAngle2DOwner()->getSize());
}
...
CPSRotated2DParticle::CPSRotated2DParticle()
    : _Angle2D(0), _Angle2DScheme(NULL){NL_PS_FUNC(
                       CPSRotated2DParticle_CPSRotated2DParticle)}
...
///===================================================================================
void CPSTexturedParticle::setTextureIndexScheme(
    CPSAttribMaker<sint32> *animOrder) {
  ...
  if (getTextureIndexOwner() && animOrder->hasMemory())
    animOrder->resize(getTextureIndexOwner()->getMaxSize(),
                      getTextureIndexOwner()->getSize());
...
///===================================================================================
void CPSTexturedParticle::setTextureGroup(
    NLMISC::CSmartPtr<CTextureGrouped> texGroup) {
...
CPSTexturedParticle::CPSTexturedParticle()
    : _TexGroup(NULL), _TextureIndexScheme(NULL),
      _TextureIndex(0){NL_PS_FUNC(CPSTexturedParticle_CPSTexturedParticle)}
...
///===================================================================================
void CPSRotated3DPlaneParticle::setPlaneBasisScheme(
    CPSAttribMaker<CPlaneBasis> *basisMaker) {
  NL_PS_FUNC(CPSRotated3DPlaneParticle_setPlaneBasisScheme)
  nlassert(basisMaker);
  delete _PlaneBasisScheme;
  _PlaneBasisScheme = basisMaker;
  if (getPlaneBasisOwner() && basisMaker->hasMemory())
    basisMaker->resize(getPlaneBasisOwner()->getMaxSize(),
                       getPlaneBasisOwner()->getSize());
}
...
CPSMaterial::TBlendingMode CPSMaterial::getBlendingMode(void) const {
  ...
  if (_Mat.getBlend()) {
    ...
    if (srcBlend == CMaterial::one && destBlend == CMaterial::one)
      return add;
    if (srcBlend == CMaterial::zero && destBlend == CMaterial::srccolor)
      return modulate;
    if (srcBlend == CMaterial::srcalpha && destBlend == CMaterial::invsrcalpha)
      return alphaBlend;
...
///===================================================================================
void CPSMaterial::forceModulateConstantColor(bool force,
                                             const NLMISC::CRGBA &col) {
...
CPSMultiTexturedParticle::CPSMultiTexturedParticle()
    : _MainOp(Add), _AlternateOp(Add), _MultiTexState(TouchFlag),
      _BumpFactor(1.f) {
...
void CPSMultiTexturedParticle::enableMultiTexture(bool enabled /*= true*/) {
  ...
  if (isMultiTextureEnabled() == enabled)
    return;
  if (!enabled) {
    _Texture2 = NULL;
...
void CPSMultiTexturedParticle::enableAlternateTex(bool enabled /*= true*/) {
  ...
  nlassert(
      isMultiTextureEnabled()); // multitexturing must have been enabled first
  if (enabled == isAlternateTexEnabled())
    return;
...
//=======================================
void CPSMultiTexturedParticle::setupMaterial(ITexture *primary, IDriver *driver,
                                             CMaterial &mat,
                                             CVertexBuffer &vb) {
  NL_PS_FUNC(CPSMultiTexturedParticle_setupMaterial)
  /// if bump is used, the matrix must be setupped each time (not a material
  /// field)
  if (!_ForceBasicCaps && isMultiTextureEnabled() && _MainOp == EnvBumpMap) {
    if (driver->supportTextureAddrMode(CMaterial::OffsetTexture)) {
      ...
      if (tb != NULL) {
        ...
        const float bMat[] = {_BumpFactor * normFactor, 0.f, 0.f,
                              _BumpFactor * normFactor};
  ...
  if (!isTouched() && areBasicCapsForcedLocal() == areBasicCapsForced())
    return;
  ...
  if (!isMultiTextureEnabled()) {
    ...
    if (_MainOp != EnvBumpMap) {
      ...
      if (!_ForceBasicCaps &&
          (driver->supportTextureAddrMode(CMaterial::OffsetTexture) ||
           driver->supportEMBM())) // envbumpmap supported ?
      {
        CTextureBump *tb = dynamic_cast<CTextureBump *>((ITexture *)_Texture2);
        if (tb != NULL) {
          float normFactor = tb->getNormalizationFactor();
...
//=======================================
void CPSMultiTexturedParticle::setupMultiTexEnv(TOperator op, ITexture *tex1,
                                                ITexture *tex2, CMaterial &mat,
                                                IDriver &drv) {
...
void CPSMultiTexturedParticle::setUseLocalDate(bool use) {
  ...
  if (use)
    _MultiTexState |= ScrollUseLocalDate;
  else
    _MultiTexState &= ~ScrollUseLocalDate;
}
...
void CPSMultiTexturedParticle::setUseLocalDateAlt(bool use) {
  ...
  if (use)
    _MultiTexState |= ScrollUseLocalDateAlternate;
  else
    _MultiTexState &= ~ScrollUseLocalDateAlternate;
}

//==========================================
void CPSTexturedParticle::enumTexs(
    std::vector<NLMISC::CSmartPtr<ITexture>> &dest) {
...
//==========================================
void CPSMultiTexturedParticle::enumTexs(
    std::vector<NLMISC::CSmartPtr<ITexture>> &dest, IDriver &drv) {
  NL_PS_FUNC(CPSMultiTexturedParticle_enumTexs)
  if (_MainOp == EnvBumpMap && !_ForceBasicCaps) {
    if (drv.supportTextureAddrMode(CMaterial::OffsetTexture) ||
        drv.supportEMBM()) {
      if (_Texture2)
        dest.push_back(_Texture2);
    } else {
      if (_AlternateTexture2)
        dest.push_back(_AlternateTexture2);
    }
    return;
  }
  if (_Texture2)
...
bool CPSMultiTexturedParticle::isAlternateTextureUsed(IDriver &driver) const {
  ...
  if (!isTouched() && areBasicCapsForcedLocal() == areBasicCapsForced())
    return (_MultiTexState & AlternateTextureUsed) != 0;
  if (_MainOp != EnvBumpMap)
    return false;
  return _ForceBasicCaps ||
         (!driver.supportTextureAddrMode(CMaterial::OffsetTexture) &&
          !driver.supportEMBM());
}

...
CVertexBuffer
    CWaterEnvMap::_FlattenVB; // vb to map cube map top hemisphere to a 2D map
...
static const uint TEST_VB_NUM_TRIS =
    2 * TEST_VB_NUM_SEGMENT * TEST_VB_NUM_SLICE;
...
// *******************************************************************************
void CWaterEnvMap::init(uint cubeMapSize, uint projection2DSize,
                        TGlobalAnimationTime updateTime, IDriver &driver) {
  // Allocate cube map
  ...
  tb->resize(cubeMapSize,
             cubeMapSize); // Unfortunately,  must allocate memory in order for
                           // the driver to figure out the size that it needs to
                           // allocate for the texture, though its datas are
                           // never used (it is a render target)
  tb->Size = cubeMapSize;
...
void CWaterEnvMap::update(TGlobalAnimationTime time, IDriver &driver) {
  if (_LastRenderTime == time)
    return;
  _LastRenderTime = time;
  // First five updates are used to render the cubemap faces (bottom face is not
  // rendered) Sixth update project the cubemap into a 2D texture
  uint numTexToRender;
  if (_UpdateTime > 0) {
    uint64 currRenderTick =
        (uint64)(time / (_UpdateTime / (NUM_FACES_TO_RENDER + 1)));
  ...
  uint lastCubeFacesToRender = std::min(
      (uint)NUM_FACES_TO_RENDER,
      _NumRenderedFaces + numTexToRender); // we don't render negative Z (only
                                           // top hemisphere is used)
  for (uint k = _NumRenderedFaces; k < lastCubeFacesToRender; ++k) {
    driver.setRenderTarget(_EnvCubic, 0, 0, _EnvCubicSize, _EnvCubicSize, 0,
                           (uint32)k);
    render((CTextureCube::TFace)k, _StartRenderTime);
  }
  _NumRenderedFaces = lastCubeFacesToRender;
  if (_NumRenderedFaces == NUM_FACES_TO_RENDER &&
      (_NumRenderedFaces + numTexToRender) > NUM_FACES_TO_RENDER) {
    // render to 2D map
...
void CWaterEnvMap::doInit() {
  if (!_FlattenVBInitialized) {
    ...
    _MaterialPassThru.texEnvArg0Alpha(0, CMaterial::Constant,
                                      CMaterial::SrcAlpha);
...
void CWaterEnvMap::renderTestMesh(IDriver &driver)
{
    ...
    driver.setUniformMatrix(IDriver::VertexProgram,
testMeshVP->getUniformIndex(CProgramIndex::ModelViewProjection),
IDriver::ModelViewProjection, IDriver::Identity);
    driver.setUniform2f(IDriver::VertexProgram,
testMeshVP->idx().ProgramConstant0, 2.f, 1.f);
...
void CWaterEnvMap::initFlattenVB() {
  ...
  _FlattenVB.initEx();
  nlctassert(FVB_NUM_SIDES % 4 ==
             0); // number of sides must be a multiple of 4 so that sections
                 // sides will align with corners
  ...
  _FlattenIB.setNumIndexes(3 * FVB_NUM_TRIS);
  {
    ...
    for (uint l = 0; l < FVB_NUM_SIDES; ++l) {
      double angle = NLMISC::Pi * 0.25 +
                     2 * NLMISC::Pi * (double)l / (double)FVB_NUM_SIDES;
      for (uint k = 0; k < FVB_NUM_SECTIONS + 1; ++k) {
        ...
        if (k < FVB_NUM_SECTIONS) {
          ibrw.setTri(3 * 2 * (k + (l * FVB_NUM_SECTIONS)), getFVBVertex(k, l),
                      getFVBVertex(k + 1, l + 1), getFVBVertex(k + 1, l));
          ibrw.setTri(3 * (2 * (k + (l * FVB_NUM_SECTIONS)) + 1),
                      getFVBVertex(k, l), getFVBVertex(k, l + 1),
                      getFVBVertex(k + 1, l + 1));
        } else {
        ...
        CVector dir;
        // dir.sphericToCartesian(1.f, (float) angle, (float) (NLMISC::Pi * 0.5
        // * acos(std::max(0.f, (1.f - (float) k / (FVB_NUM_SECTIONS - 1))))));
        dir.sphericToCartesian(
            1.f, (float)angle,
            (float)acos(std::min(1.f, (float)k / (FVB_NUM_SECTIONS - 1))));
        vbrw.setValueFloat3Ex(CVertexBuffer::Position, getFVBVertex(k, l), x,
                              0.5f, y);
        vbrw.setValueFloat3Ex(CVertexBuffer::TexCoord0, getFVBVertex(k, l),
                              -dir.x, dir.z, -dir.y);
      }
    }
  }
...
void CWaterEnvMap::initTestVB() {
  ...
  _TestIB.setNumIndexes(3 * TEST_VB_NUM_TRIS);
  {
    ...
    for (uint k = 0; k < TEST_VB_NUM_SEGMENT; ++k) {
      float theta =
          2 * (float)(NLMISC::Pi * (double)k / (double)TEST_VB_NUM_SEGMENT);
      for (uint l = 0; l <= TEST_VB_NUM_SLICE; ++l) {
        float phi = (float)(NLMISC::Pi / 2 *
                            (1 - 2 * (double)l / (double)TEST_VB_NUM_SLICE));
        CVector pos;
        pos.sphericToCartesian(1.f, theta, phi);
#define VERT_INDEX(k, l)                                                       \
  ((l) + ((k) % TEST_VB_NUM_SEGMENT) * (TEST_VB_NUM_SLICE + 1))
        ...
        if (l != TEST_VB_NUM_SLICE) {
          ibrw.setTri(3 * triIndex++, VERT_INDEX(k, l), VERT_INDEX(k + 1, l),
                      VERT_INDEX(k + 1, l + 1));
          ibrw.setTri(3 * triIndex++, VERT_INDEX(k, l),
                      VERT_INDEX(k + 1, l + 1), VERT_INDEX(k, l + 1));
        }
      }
    }

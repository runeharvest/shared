    ...
    float xmin, float zmin, float xmax, float zmax)
{
	...
	if (((x + XMin) > xmax) || ((x + XMax) < xmin) || ((z + ZMin) > zmax) || ((z + ZMax) < zmin))
		return;

	// test if entirely in.
	bool allIn;
	allIn = ((x + XMin) >= xmin) && ((x + XMax) <= xmax) && ((z + ZMin) >= zmin) && ((z + ZMax) <= zmax);
	...
	else
	{
		...
		{
			if (((x + pIniPos0->x) > xmax) || ((x + pIniPos2->x) < xmin) || ((z + pIniPos0->z) > zmax) || ((z + pIniPos2->z) < zmin))
			{
    ...
    const NLMISC::CMatrix &scaleMatrix, float x, float z, float depth, float xmin, float zmin, float xmax, float zmax)
{
	...
	// test if entirely in.
	bool allIn;
	allIn = ((x + XMin) >= (xmin - 0.001f)) && ((x + XMax) <= (xmax + 0.001f)) && ((z + ZMin) >= (zmin - 0.001f)) && ((z + ZMax) <= (zmax + 0.001f));

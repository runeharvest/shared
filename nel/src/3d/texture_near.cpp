...
CTextureNear::CTextureNear(sint size) {
  ...
  // NB: this is simplier like that, and this is not a problem, since only 1 or
  // 2 Mo are allocated :o)
  setReleasable(false);
  // create the bitmap.
...
// ***************************************************************************
sint CTextureNear::getNbAvailableTiles() { return (sint)_FreeTiles.size(); }
// ***************************************************************************
uint CTextureNear::getTileAndFillRect(
...
// ***************************************************************************
void CTextureNear::refillRect(
    uint id, CRGBA map[NL_TILE_LIGHTMAP_SIZE * NL_TILE_LIGHTMAP_SIZE]) {
  ...
  dst += t * dstWidth + s;
  for (sint n = NL_TILE_LIGHTMAP_SIZE; n > 0;
       n--, src += NL_TILE_LIGHTMAP_SIZE, dst += dstWidth) {
    memcpy(dst, src, NL_TILE_LIGHTMAP_SIZE * sizeof(CRGBA));
  }

  // Invalidate the rectangle.
  ITexture::touchRect(
      CRect(s, t, NL_TILE_LIGHTMAP_SIZE, NL_TILE_LIGHTMAP_SIZE));
}

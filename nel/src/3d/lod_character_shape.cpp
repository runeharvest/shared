...
// ***************************************************************************
const CLodCharacterShapeBuild::CPixelInfo
    CLodCharacterShapeBuild::CPixelInfo::EmptyPixel(CVector::Null,
                                                    CVector(1000, 0, 0));
...
// ***************************************************************************
void CLodCharacterShapeBuild::compile(
    const std::vector<bool> &triangleSelection, uint textureOverSample) {
  ...
  // **** reset the texture and init with "empty" pixel (ie normal.x==1000,
  // which is not possible because normal.norm()==1)
  ...
  // **** For each triangle in the shape, polyfill this triangle from a texture
  // view, in the overSampledTexture
  CPolygon2D poly;
  ...
  for (uint i = 0; i < TriangleIndices.size(); i += 3) {
    ...
    triangleUV.computeGradient(pInf[0].Pos.x, pInf[1].Pos.x, pInf[2].Pos.x,
                               GradPosX);
    triangleUV.computeGradient(pInf[0].Pos.y, pInf[1].Pos.y, pInf[2].Pos.y,
                               GradPosY);
    triangleUV.computeGradient(pInf[0].Pos.z, pInf[1].Pos.z, pInf[2].Pos.z,
                               GradPosZ);
    triangleUV.computeGradient(pInf[0].Normal.x, pInf[1].Normal.x,
                               pInf[2].Normal.x, GradNormalX);
    triangleUV.computeGradient(pInf[0].Normal.y, pInf[1].Normal.y,
                               pInf[2].Normal.y, GradNormalY);
    triangleUV.computeGradient(pInf[0].Normal.z, pInf[1].Normal.z,
                               pInf[2].Normal.z, GradNormalZ);
      ...
      for (x = 0; x < (sint)_Width; x++) {
        ...
        for (yo = 0; yo < (sint)textureOverSample; yo++) {
          for (xo = 0; xo < (sint)textureOverSample; xo++) {
            ...
            const CPixelInfo &overPixel =
                overTextureInfo[(y * textureOverSample + yo) * wOver +
                                (x * textureOverSample + xo)];
  ...
  // **** Dilate texture 1 time: each empty pixel info get neighbor full pixel
  // (8box) copy the non dilated texture
  ...
  for (y = 0; y < (sint)_Height; y++) {
    ...
    for (x = 0; x < (sint)_Width; x++) {
      ...
      if (texelInf == CPixelInfo::EmptyPixel) {
        ...
        // For the 8 possible pixels (nb: look us too, but doesn't matter since
        // we are an empty pixel)
        for (sint yb = y0; yb <= y1; yb++) {
          for (sint xb = x0; xb <= x1; xb++) {
            // if the neighbor is not an empty pixel. NB: avoid override
            // problems using not Dilated texture
...
void CLodCharacterShapeBuild::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  if (f.isReading()) {
    ...
    std::copy(TriangleIndices.begin(), TriangleIndices.end(),
              saveVect.begin()); //  copy will do the job
...
// ***************************************************************************
void CLodCharacterShape::buildMesh(const std::string &name,
...
bool CLodCharacterShape::addAnim(const CAnimBuild &animBuild) {
  ...
  // Possible to have an Anim with just one key. setup an epsilon for animLength
  // if 0.
  ...
  // compute the UnPackScaleFactor ie maxSize, to be multiplied by max Abs value
  // of a sint16
...
// ***************************************************************************
const CLodCharacterShape::CVector3s *
CLodCharacterShape::getAnimKey(uint animId, TGlobalAnimationTime time,
                               bool wrapMode,
                               CVector &unPackScaleFactor) const {
...
// ***************************************************************************
void CLodCharacterShape::addBoneAlpha(uint boneId,
                                      std::vector<uint8> &tmpAlphas) const {
  // Yoyo: This is an error to not have the same skeleton that the one stored in
  // the lod shape. But must not crash

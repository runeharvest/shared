...
CPSShockWave::TVBMap
    CPSShockWave::_ColoredAnimTexVBMap; // vb ith unanimated texture
/////////////////////////////////
// CPSShockWave implementation //
/////////////////////////////////

/** Well, we could have put a method template in CPSShockWave, but some
...
public:
  template <class T>
  static void drawShockWave(T posIt, CPSShockWave &s, uint size,
                            uint32 srcStep) {
    ...
    const uint numShockWaveToDealWith =
        std::min(ShockWaveBufSize, s.getNumShockWavesInVB());
    ...
    do {
      toProcess =
          leftToDo > numShockWaveToDealWith ? numShockWaveToDealWith : leftToDo;
      vb->setNumVertices((toProcess * (s._NbSeg + 1)) << 1);
      {
        ...
        if (s._SizeScheme) {
          ptCurrSize = (float *)(s._SizeScheme->make(
              s._Owner, size - leftToDo, (void *)sizes, sizeof(float),
        ...
        if (s._PlaneBasisScheme) {
          ptCurrBasis = (CPlaneBasis *)(s._PlaneBasisScheme->make(
              s._Owner, size - leftToDo, (void *)planeBasis,
              sizeof(CPlaneBasis), toProcess, true, srcStep));
        } else {
          ptCurrBasis = &s._PlaneBasis;
        }

        if (s._Angle2DScheme) {
          ptCurrAngle = (float *)(s._Angle2DScheme->make(
              s._Owner, size - leftToDo, (void *)angles, sizeof(float),
              toProcess, true, srcStep));
        } else {
        ...
        do {
          ...
          for (k = 0; k <= s._NbSeg; ++k) {
            radVect = *ptCurrSize *
                      (CPSUtil::getCos((sint32)currAngle) * ptCurrBasis->X +
                       CPSUtil::getSin((sint32)currAngle) * ptCurrBasis->Y);
...
CPSShockWave::CPSShockWave(uint nbSeg, float radiusCut, CSmartPtr<ITexture> tex)
    : _NbSeg(nbSeg), _RadiusCut(radiusCut), _UFactor(1.f)
...
inline void CPSShockWave::setupUFactor() {
  ...
  if (_UFactor != 1.f) {
    ...
    CMatrix texMat;
    texMat.setRot(_UFactor * NLMISC::CVector::I, NLMISC::CVector::J,
                  NLMISC::CVector::K);
...
void CPSShockWave::draw(bool opaque) {
  ...
  if (step == (1 << 16)) {
    CPSShockWaveHelper::drawShockWave(_Owner->getPos().begin(), *this,
                                      numToProcess, step);
  } else {
    CPSShockWaveHelper::drawShockWave(
...
void CPSShockWave::updateVbColNUVForRender(uint32 startIndex, uint32 size,
                                           uint32 srcStep, CVertexBuffer &vb,
                                           IDriver &drv) {
  ...
  if (_ColorScheme) {
    ...
    _ColorScheme->makeN(_Owner, startIndex, vba.getColorPointer(),
                        vb.getVertexSize(), size, (_NbSeg + 1) << 1, srcStep);
  }
...
void CPSShockWave::getVBnPB(CVertexBuffer *&retVb, CIndexBuffer *&retPb) {
  ...
  TVBMap &vbMap =
      _ColorScheme == NULL
          ? (_TexGroup == NULL ? _VBMap : _AnimTexVBMap)
...
///=================================================================================
void CPSShockWave::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                            IDriver &drv) {

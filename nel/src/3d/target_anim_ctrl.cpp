...
void CTargetAnimCtrl::execute(CSkeletonModel *model, CBone *bone) {
  ...
  /* Get the Skeleton default WorldMatrix (bind pos). used later
      TRICK: to get the default Skeleton WorldMatrix, get it from Bone[0] (ie
     "Bip01") We cannot use the Default Pos/Rot of the skeleton model because of
     export bug (not exported...) So, since Bip01 as no Local Rot (yes, its
     true, exporter report all Bip01 rots on Skeleton), we are sure that Bip01
     BindPos is the default Skeleton World Matrix.
  */
  ...
  if (!Enabled) {
    ...
    // Get the current wanted WorldDirection into LocalSkeleton space (hence
    // using current skeleton WorldMatrix).
    CMatrix toLSSpace = model->getWorldMatrix();
    currentLSDirection = toLSSpace.getRot().conjugate() * CurrentWorldDirection;

    // Get the default WorldDirection into LocalSkeleton space (hence using
    // default skeleton WorldMatrix).
    CQuat defaultLSDirection = rootInvBP * DefaultWorldDirection;

    /* get the rotation to apply to the bone when it is in bind Pos. If this
       quat is identity, then the bone will be in default (or bind) pos. It is
       in essence the "rotation to apply to defaultDirection in LS space, in
       order to get the wanted current direction in LS space". The makeClosest()
       is here just to ensure that the resulting angle<Pi (for clamp direction
       later)
    */
  ...
  if (rotModAngle) {
    ...
    if (factor < 1) {
      ...
      currentLSRotation =
          CQuat::slerp(_LastLSRotation, currentLSRotation, factor);
      rotSpeedClamped = true;
    }
  }

  // if the rotation has not been clamped for speed consideration, and if
  // !Enabed, it's mean we have ended the transition => no more compute next
  // time
  ...
  // ***********
  // Get the bone Bind Pos in LocalSkeleton space (hence using Default Skeleton
  // WorldMatrix)
  CQuat boneBindPosInWorld =
      bone->getBoneBase().InvBindPos.getRot().conjugate();
  ...
  CBone *boneParent =
      bone->getFatherId() == -1 ? NULL : &model->Bones[bone->getFatherId()];
...
// ***************************************************************************
CQuat CTargetAnimCtrl::getCurrentLSRotationFromBone(CSkeletonModel *model,
                                                    CBone *bone) {

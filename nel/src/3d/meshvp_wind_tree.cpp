...
// ***************************************************************************
NLMISC::CSmartPtr<CVertexProgramWindTree>
    CMeshVPWindTree::_VertexProgram[CMeshVPWindTree::NumVp];
...
CVertexProgramWindTree::CVertexProgramWindTree(uint numPls, bool specular,
                                               bool normalize) {
  // lighted settings
  ...
  // nelvp
  {
    std::string vpCode =
        ...
        CRenderTrav::getLightVPFragmentNeLVP(numPls, VPLightConstantStart,
                                             specular, normalize) +
        WindTreeVPCodeEnd;

    CSource *source = new CSource();
    source->DisplayName = NLMISC::toString(
        "nelvp/MeshVPWindTree/%i/%s/%s", numPls, specular ? "spec" : "nospec",
...
void CMeshVPWindTree::initVertexPrograms() {
  ...
  if (!vpCreated) {
    ...
    for (uint i = 0; i < NumVp; i++) {
      ...
      // combine
      _VertexProgram[i] =
          new CVertexProgramWindTree(numPls, normalize, specular);
    }
  }
}
...
inline void CMeshVPWindTree::setupPerMesh(IDriver *driver, CScene *scene) {
  // process current times and current power. Only one time per render() and per
  // CMeshVPWindTree.
  if (scene->getCurrentTime() != _LastSceneTime) {
    ...
    for (i = 0; i < HrcDepth; i++) {
      _CurrentTime[i] +=
          dt * (Frequency[i] + FrequencyWindFactor[i] * windPower);
    ...
    for (i = 0; i < HrcDepth; i++) {
      _MaxDeltaPos[i] =
          scene->getGlobalWindDirection() * PowerXY[i] * windPower;
      _MaxDeltaPos[i].z = PowerZ[i] * windPower;
    }

    /* Update the Max amplitude distance
        in world space, since maxdeltaPos are applied in world space, see
       setupPerInstanceConstants()
    */
    _MaxVertexMove = 0;
    for (i = 0; i < HrcDepth; i++) {
  ...
  driver->setUniform4f(IDriver::VertexProgram,
                       program->idx().ProgramConstants[0], 0, 1, 0.5f, 2);
  // c[9] take other useful constants.
  driver->setUniform4f(IDriver::VertexProgram,
                       program->idx().ProgramConstants[1], 3.f, 0.f, -1.f,
                       -2.f);
  // c[10] take Number of phase (4) for level2 and 3. -0.01 to avoid int value
  // == 4.
  driver->setUniform4f(IDriver::VertexProgram,
                       program->idx().ProgramConstants[2], 4 - 0.01f, 0, 0, 0);
}

// ***************************************************************************
inline void CMeshVPWindTree::setupPerInstanceConstants(
    IDriver *driver, CScene *scene, CMeshBaseInstance *mbi,
  ...
  // maxDeltaPos in ObjectSpace. So same world Wind direction is applied to all
  // objects
...
// ***************************************************************************
bool CMeshVPWindTree::begin(IDriver *driver, CScene *scene,
  ...
  // Enable normalize only if requested by user. Because lighting don't manage
  // correct "scale lighting"
...
static inline void SetupForMaterial(const CMaterial &mat, CScene *scene) {
  CRenderTrav *renderTrav = &scene->getRenderTrav();
  renderTrav->changeVPLightSetupMaterial(mat,
                                         false /* don't exclude strongest */);
}

// ***************************************************************************
void CMeshVPWindTree::setupForMaterial(const CMaterial &mat, IDriver *drv,
                                       CScene *scene, CVertexBuffer *) {
  SetupForMaterial(mat, scene);
}

// ***************************************************************************
void CMeshVPWindTree::setupLighting(CScene *scene, CMeshBaseInstance *mbi,
                                    const NLMISC::CMatrix &invertedModelMat) {
  nlassert(scene != NULL);
  CRenderTrav *renderTrav = &scene->getRenderTrav();
...
// ***************************************************************************
bool CMeshVPWindTree::supportMeshBlockRendering() const { return true; }
...
void CMeshVPWindTree::beginMBRMesh(IDriver *driver, CScene *scene) {
  /* Since need a VertexProgram Activation before activeVBHard, activate a
     default one bet the common one will be "NoPointLight, NoSpecular, No
     ForceNormalize" => 0.
  */
  _LastMBRIdVP = 0;
...
// ***************************************************************************
void CMeshVPWindTree::beginMBRInstance(
    IDriver *driver, CScene *scene, CMeshBaseInstance *mbi,
    const NLMISC::CMatrix &invertedModelMat) {
  // Get how many pointLights are setuped now.
  nlassert(scene != NULL);
...
// ***************************************************************************
float CMeshVPWindTree::getMaxVertexMove() { return _MaxVertexMove; }

...
void CAnimationPlaylist::setSkeletonWeight(uint8 slot, uint skeletonId,
                                           bool inverted) {
...
void CAnimationPlaylist::setTimeOrigin(uint8 slot,
...
void CAnimationPlaylist::setStartWeight(uint8 slot, float startWeight,
...
float CAnimationPlaylist::getStartWeight(uint8 slot,
                                         TGlobalAnimationTime &time) const {
...
void CAnimationPlaylist::setEndWeight(uint8 slot, float endWeight,
                                      TGlobalAnimationTime time) {
...
float CAnimationPlaylist::getEndWeight(uint8 slot,
                                       TGlobalAnimationTime &time) const {
...
void CAnimationPlaylist::setupMixer(CChannelMixer &mixer,
                                    TGlobalAnimationTime time) const {
  // For each slot
  for (uint8 s = 0; s < CChannelMixer::NumAnimationSlot; s++) {
    ...
    if (animSet) {
      if (_Animations[s] != empty) {
        ...
        // Disable mode ?
        if ((_WrapMode[s] == Disable) &&
            ((wrappedTime < pAnimation->getBeginTime()) ||
             (wrappedTime > pAnimation->getEndTime())))
          enabled = false;
    ...
    if (enabled) {
      ...
      // *** Set the weight
      mixer.setSlotWeight(
          s, getWeightValue(_StartWeightTime[s], _EndWeightTime[s], time,
                            _StartWeight[s], _EndWeight[s], _Smoothness[s]));
    } else {
      // Disable this slot
      mixer.emptySlot(s);
    }
...
float CAnimationPlaylist::getWeightValue(TGlobalAnimationTime startWeightTime,
  ...
  // Linear value
  TGlobalAnimationTime linear =
      startWeight + (endWeight - startWeight) * (time - startWeightTime) /
                        (endWeightTime - startWeightTime);
...
CAnimationPlaylist::getWrapMode(uint8 slot) const {
...
TAnimationTime
CAnimationPlaylist::getLocalTime(uint8 slot, TGlobalAnimationTime globalTime,
  ...
  if (pAnimation) {
    ...
    TAnimationTime wrappedTime =
        pAnimation->getBeginTime() +
        (TAnimationTime)((globalTime - _TimeOrigin[slot]) * _SpeedFactor[slot]);
    ...
    case Repeat:
      // Mod repeat the time
      {
        ...
        if (wrappedTime >= pAnimation->getBeginTime())
          wrappedTime =
              pAnimation->getBeginTime() +
              (float)fmod(wrappedTime - pAnimation->getBeginTime(), length);
        else
          wrappedTime =
              pAnimation->getBeginTime() +
              (float)fmod(wrappedTime - pAnimation->getBeginTime(), length) +
...
float CAnimationPlaylist::getLocalWeight(
    uint8 slot, TGlobalAnimationTime globalTime) const {
  return getWeightValue(_StartWeightTime[slot], _EndWeightTime[slot],
                        globalTime, _StartWeight[slot], _EndWeight[slot],
                        _Smoothness[slot]);
}

...
static const uint EMITTER_BUFF_SIZE =
    512; // number of emitter to be processed at once
static const float EMIT_PERIOD_THRESHOLD =
    1.f / 75.f; // assuming the same behaviour than with a 75 hz rendering
bool CPSEmitter::_BypassEmitOnDeath = false;

//////////////////////
// STATIC FUNCTIONS //
//////////////////////
/** In an arrey of float, all value that are 0.f are replaced by
 * EMIT_PERIOD_THRESHOLD A period of 0 is allowed for emitter and means "emit at
 * each frame" This is deprecated now, and this helps to avoid that behaviour
 */
...
CPSEmitter::CPSEmitter()
    : _EmittedType(NULL), _SpeedInheritanceFactor(0.f), _EmissionType(regular),
      _Period(0.02f), _PeriodScheme(NULL), _GenNb(1), _GenNbScheme(NULL),
      _EmitDelay(0), _MaxEmissionCount(0), _SpeedBasisEmission(false),
      _ConsistentEmission(true), _BypassAutoLOD(false),
      _UserMatrixModeForEmissionDirection(false), _EmitTrigger(false),
  ...
  if (!_SpeedBasisEmission) {
    if (_SpeedInheritanceFactor == 0.f) {
      if (!_UserMatrixModeForEmissionDirection) {
        while (nbToGenerate > 0) {
          ...
          _EmittedType->postNewElement(pos, speed, *this->_Owner, index,
                                       _Owner->getMatrixMode(), 0.f);
        }
      } else {
        while (nbToGenerate > 0) {
          nbToGenerate--;
          emit(_Owner->getPos()[index], index, pos, speed);
          _EmittedType->postNewElement(pos, speed, *this->_Owner, index,
                                       _UserDirectionMatrixMode, 0.f);
        }
      }
    } else {
      while (nbToGenerate--) {
        emit(_Owner->getPos()[index], index, pos, speed);
        _EmittedType->postNewElement(
            pos, speed + _SpeedInheritanceFactor * _Owner->getSpeed()[index],
            *this->_Owner, 0, _Owner->getMatrixMode(), 0.f);
      }
    }
  } else {
    NLMISC::CMatrix m;
    ...
    if (_SpeedInheritanceFactor == 0.f) {
      while (nbToGenerate > 0) {
        nbToGenerate--;
        emit(_Owner->getPos()[index], index, pos, speed);
        _EmittedType->postNewElement(pos, m * speed, *this->_Owner, index,
                                     _Owner->getMatrixMode(), 0.f);
      }
    } else {
      while (nbToGenerate--) {
        ...
        _EmittedType->postNewElement(
            pos,
            m * speed + _SpeedInheritanceFactor * _Owner->getSpeed()[index],
            *this->_Owner, index, _Owner->getMatrixMode(), 0.f);
      }
    }
  }
}
...
void CPSEmitter::processEmitOutsideSimLoop(uint32 index, sint nbToGenerate) {
  ...
  if (!_SpeedBasisEmission) {
    if (_SpeedInheritanceFactor == 0.f) {
      ...
      while (nbToGenerate--) {
        emit(_Owner->getPos()[index], index, pos, speed);
        _EmittedType->newElement(
            pos, speed + _SpeedInheritanceFactor * _Owner->getSpeed()[index],
            this->_Owner, 0, _Owner->getMatrixMode(), true);
      }
    }
  } else {
    NLMISC::CMatrix m;
    ...
    if (_SpeedInheritanceFactor == 0.f) {
      ...
      while (nbToGenerate--) {
        ...
        _EmittedType->newElement(
            pos,
            m * speed + _SpeedInheritanceFactor * _Owner->getSpeed()[index],
            this->_Owner, index, _Owner->getMatrixMode(), true);
      }
    }
  }
}
...
inline void CPSEmitter::processEmitConsistent(const NLMISC::CVector &emitterPos,
                                              uint32 index, sint nbToGenerate,
    ...
    if (_SpeedInheritanceFactor == 0.f) {
      if (!_UserMatrixModeForEmissionDirection) {
        ...
        while (nbToGenerate > 0) {
          ...
          emit(emitterPos, index, pos, speed);
          _EmittedType->postNewElement(pos, speed, *this->_Owner, index,
                                       _UserDirectionMatrixMode, deltaT);
        }
      }
    } else {
      while (nbToGenerate--) {
        ...
        _EmittedType->postNewElement(
            pos, speed + _SpeedInheritanceFactor * _Owner->getSpeed()[index],
            *this->_Owner, index, _Owner->getMatrixMode(), deltaT);
      }
    }
  } else {
    NLMISC::CMatrix m;
...
bool CPSEmitter::setEmissionType(TEmissionType freqType) {
  ...
  if (_Owner && _Owner->getOwner()) {
    ...
    if (ps->getBypassMaxNumIntegrationSteps()) {
      if (!_Owner) {
        nlwarning("<CPSEmitter::setEmissionType> The emitter should be "
                  "inserted in a CPSLocated instance");
        nlassert(0);
      }
  ...
  if (_Owner && _Owner->getOwner()) {
    ...
    if (_EmittedType) {
      ...
      if (!ok) {
        ...
        nlwarning(
            "<CPSLocated::setEmittedType> Can't set new emitted type : this "
            "causes the system to last forever, and it has been flagged with "
            "'BypassMaxNumIntegrationSteps'. New emitted type is not set");
        return false;
      }
    }
    ps->systemDurationChanged();
...
void CPSEmitter::setPeriodScheme(CPSAttribMaker<float> *scheme) {
  ...
  if (_Owner && scheme->hasMemory())
    scheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  if (_Owner && _Owner->getOwner()) {
    _Owner->getOwner()->systemDurationChanged();
  }
}
...
void CPSEmitter::singleEmit(uint32 index, uint quantity) {
  ...
  nlassert(_Owner);
  const uint32 nbToGenerate =
      _GenNbScheme ? _GenNbScheme->get(_Owner, 0) : _GenNb;
...
void CPSEmitter::processRegularEmissionWithNoLOD(uint firstInstanceIndex) {
  ...
  TPSAttribTime::iterator phaseIt = _Phase.begin() + firstInstanceIndex,
                          endPhaseIt;
  TPSAttribUInt8::iterator numEmitIt =
      _NumEmission.begin() + firstInstanceIndex;

  // we don't use an iterator here
  // because it could be invalidated if size change (a located could generate
  // itself)
  do {
    ...
    if (_PeriodScheme) {
      // compute period
      // NB : we ask to clamp entry because life counter of emitter are
      // incremented, then spawn is called, and only after that dead emitters
      // are removed
      //      so we may have a life counter that is > to 1
      currEmitPeriod = (float *)(_PeriodScheme->make(
          _Owner, size - leftToDo, emitPeriod, sizeof(float), toProcess, true,
          1 << 16, true));
      if (emitThreshold) {

        /** Test if 'make' filled our buffer. If this is not the case, we assume
         * that values where precomputed, and that all null period have already
         * been replaced by the threshold
         */
        if (currEmitPeriod == emitPeriod) {
          // if there possibility to have 0 in the scheme ?
          if (_PeriodScheme->getMinValue() <= 0.f &&
              _PeriodScheme->getMaxValue() >= 0.f) {
      ...
      if (_EmitDelay == 0.f) // no emission delay
      {
        do {
          *phaseIt += CParticleSystem::EllapsedTime;
          if (*phaseIt >=
              *currEmitPeriod) // phase is greater than period -> must emit
          {
            if (*currEmitPeriod != 0) {
              *phaseIt -=
                  ::floorf(*phaseIt / *currEmitPeriod) * *currEmitPeriod;
            }
      ...
      } else // there's an emission delay
      {
        do {
          *phaseIt += CParticleSystem::EllapsedTime;
          if (*phaseIt >=
              *currEmitPeriod +
                  _EmitDelay) // phase is greater than period -> must emit
          {
            if (*currEmitPeriod != 0) {
              *phaseIt -= ::floorf((*phaseIt - _EmitDelay) / *currEmitPeriod) *
                          *currEmitPeriod;
            }
            const uint32 k = (uint32)(phaseIt - _Phase.begin());
            nbToGenerate = _GenNbScheme ? _GenNbScheme->get(_Owner, k) : _GenNb;
          ...
          if (*numEmitIt < _MaxEmissionCount) {
            *phaseIt += CParticleSystem::EllapsedTime;
                ...
                *currEmitPeriod) // phase is greater than period -> must emit
            {
              ...
              const uint32 k = (uint32)(phaseIt - _Phase.begin());
              nbToGenerate =
                  _GenNbScheme ? _GenNbScheme->get(_Owner, k) : _GenNb;
              processEmit(k, nbToGenerate);
              ++*numEmitIt;
            }
          }
          ...
          if (*numEmitIt < _MaxEmissionCount) {
            *phaseIt += CParticleSystem::EllapsedTime;
                    ...
                    _EmitDelay) // phase is greater than period -> must emit
            {
              if (*currEmitPeriod != 0) {
                *phaseIt -=
                    ::floorf((*phaseIt - _EmitDelay) / *currEmitPeriod) *
                    *currEmitPeriod;
              }
              const uint32 k = (uint32)(phaseIt - _Phase.begin());
              nbToGenerate =
...
///==========================================================================
void CPSEmitter::processRegularEmission(uint firstInstanceIndex,
                                        float emitLOD) {
...
/// private : generate the various position of an emitter in the given tab for
/// the given slice of time,
//  depending on whether its motion is parametric or incremental. This is used
//  to create emittees at the right position
...
static
...
#endif
    uint
    GenEmitterPositions(CPSLocated *emitter, CPSLocated *emittee,
                        uint emitterIndex, uint numStep,
                        TAnimationTime deltaT, /* fraction of time needed to
                                                  reach the first emission */
                        TAnimationTime step,
                        std::vector<NLMISC::CVector> &dest) {
  NL_PS_FUNC(GenEmitterPositions)
  const uint toProcess =
      std::max(1U, std::min(numStep, (uint)emittee->getMaxSize()));
  dest.resize(toProcess);

  if (!emitter->isParametricMotionEnabled()) // standard case : take current pos
                                             // and integrate
  {
    if (toProcess == 1) // only one emission -> takes current pos
    {
      dest[0] = emitter->getPos()[emitterIndex] -
                deltaT * emitter->getSpeed()[emitterIndex];
    } else {
  ...
  } else // compute parametric trajectory
  {
    emitter->integrateSingle(emitter->getOwner()->getSystemDate() +
                                 CParticleSystem::RealEllapsedTime - deltaT,
                             -step, toProcess, emitterIndex, &dest[0]);
  }
 ...
 */
static inline uint GenEmitterPositionsWithLOD(
    CPSLocated *emitter, CPSLocated *emittee, uint emitterIndex, uint numStep,
    TAnimationTime
        deltaT, /* fraction of time needed to reach the first emission */
    TAnimationTime step, float invLODRatio,
    std::vector<NLMISC::CVector> &dest) {
                                             ...
                                             // and integrate
  {
    if (toProcess == 1) // only one emission -> takes current pos
    {
      ...
      NLMISC::CVector speed =
          step * invLODRatio * emitter->getSpeed()[emitterIndex];
      do {
        --outIt;
        *outIt = pos;
        pos -= speed;
  ...
  {
    emitter->integrateSingle(emitter->getOwner()->getSystemDate() - deltaT -
                                 step * toProcess,
                             step, toProcess, emitterIndex, &dest[0]);
  }
...
///==========================================================================
void CPSEmitter::processRegularEmissionConsistent(uint firstInstanceIndex,
                                                  float emitLOD,
                                                  float inverseEmitLOD) {
  NL_PS_FUNC(CPSEmitter_processRegularEmissionConsistent)
  /// hmm some code factorisation would do no harm, but we want to keep tests
  /// outside the loops as much as possible...
  ...
  // Positions for the emitter. They are computed by using a parametric
  // trajectory or by using integration
              ...
              *currEmitPeriod) // phase is greater than period -> must emit
          {
            if (*currEmitPeriod != 0) {
              ...
              *phaseIt -= *currEmitPeriod * numEmissions;
              ...
              nbToGenerate = _GenNbScheme
                                 ? _GenNbScheme->get(_Owner, emitterIndex)
                                 : _GenNb;
              if (nbToGenerate) {
                float deltaT = std::max(0.f, *phaseIt);

                /// compute the position of the emitter for the needed dates
                numEmissions = GenEmitterPositionsWithLOD(
                    _Owner, _EmittedType, emitterIndex, numEmissions, deltaT,
                    *currEmitPeriod, inverseEmitLOD, emitterPositions);
                ...
                do {
                  --k;
                  processEmitConsistent(emitterPositions[k], emitterIndex,
                                        nbToGenerate, deltaT);
                  deltaT += deltaTInc;
                } while (k);
              }
                  ...
                  _EmitDelay) // phase is greater than period -> must emit
          {
            if (*currEmitPeriod != 0) {
              ...
              *phaseIt = std::min(*phaseIt, *currEmitPeriod + ellapsedTimeLOD +
              ...
              //
              uint numEmissions =
                  (uint)::floorf((*phaseIt - _EmitDelay) / *currEmitPeriod);
              *phaseIt -= *currEmitPeriod * numEmissions;
      ...
      if (_EmitDelay == 0.f) // no emission delay
      {
        do {
          if (*numEmitIt < maxEmissionCountLOD) {
            *phaseIt += ellapsedTimeLOD;
                ...
                *currEmitPeriod) // phase is greater than period -> must emit
            {
              if (*currEmitPeriod != 0) {
                 ...
                 */
                *phaseIt =
                    std::min(*phaseIt, *currEmitPeriod + ellapsedTimeLOD);
                //
                uint numEmissions = (uint)::floorf(*phaseIt / *currEmitPeriod);
                ...
                *phaseIt -= *currEmitPeriod * numEmissions;
                ...
                // nlassert(deltaT >= 0.f);
                uint emitterIndex = (uint)(phaseIt - _Phase.begin());
                if (*numEmitIt > _MaxEmissionCount) // make sure we don't go
            ...
            *numEmitIt =
                _MaxEmissionCount; // if the lod change, must ensure that the
          }
          ++phaseIt;
          currEmitPeriod += currEmitPeriodPtrInc;
          ++numEmitIt;
          ...
          if (*numEmitIt < maxEmissionCountLOD) {
                    ...
                    _EmitDelay) // phase is greater than period -> must emit
            {
              if (*currEmitPeriod != 0) {
                /** Must ensure phase is valid if period decrease over time
                 */
                *phaseIt = std::min(*phaseIt, *currEmitPeriod +
                                                  ellapsedTimeLOD + _EmitDelay);
                //
                uint numEmissions =
                    (uint)::floorf((*phaseIt - _EmitDelay) / *currEmitPeriod);
                *numEmitIt += numEmissions;
...
///==========================================================================
void CPSEmitter::processRegularEmissionConsistentWithNoLOD(
    uint firstInstanceIndex) {
  NL_PS_FUNC(CPSEmitter_processRegularEmissionConsistentWithNoLOD)
  /// hum, some code factorization would do no harm, but we want to keep tests
  /// outside the loops as much as possible...
              ...
              *currEmitPeriod) // phase is greater than period -> must emit
          {
            if (*currEmitPeriod != 0) {
              ...
              *phaseIt -= *currEmitPeriod * numEmissions;
              ...
              /// compute the position of the emitter for the needed dates
              numEmissions = GenEmitterPositions(
                  _Owner, _EmittedType, emitterIndex, numEmissions, deltaT,
                  *currEmitPeriod, emitterPositions);
            ...
            *phaseIt += CParticleSystem::EllapsedTime;
                ...
                *currEmitPeriod) // phase is greater than period -> must emit
            {
              if (*currEmitPeriod != 0) {
                 ...
                 */
                *phaseIt = std::min(
                    *phaseIt, *currEmitPeriod + CParticleSystem::EllapsedTime);
                //
                uint numEmissions = (uint)::floorf(*phaseIt / *currEmitPeriod);
                *numEmitIt += numEmissions;
                *phaseIt -= *currEmitPeriod * numEmissions;
...
void CPSEmitter::computeSpawns(uint firstInstanceIndex) {
  ...
  if (!size)
    return;
  if (CParticleSystem::EllapsedTime == 0.f)
  ...
  if (!_ConsistentEmission) {
    ...
    if (emitLOD != 1.f) {
      if (emitLOD != 0.f) {
        processRegularEmissionConsistent(firstInstanceIndex, emitLOD,
                                         1.f / emitLOD);
      }
    } else {
...
void CPSEmitter::newElement(const CPSEmitterInfo &info) {
  ...
  if (_PeriodScheme && _PeriodScheme->hasMemory())
    _PeriodScheme->newElement(info);
  if (_GenNbScheme && _GenNbScheme->hasMemory())
    _GenNbScheme->newElement(info);
}
...
inline void CPSEmitter::deleteElementBase(uint32 index) {
  ...
  if (_PeriodScheme && _PeriodScheme->hasMemory())
    _PeriodScheme->deleteElement(index);
  if (_GenNbScheme && _GenNbScheme->hasMemory())
    _GenNbScheme->deleteElement(index);
  _Phase.remove(index);
  if (_MaxEmissionCount != 0) {
...
void CPSEmitter::deleteElement(uint32 index) {
  ...
  if (_EmissionType == CPSEmitter::onDeath && _EmittedType && _Active) {
    if (!_BypassEmitOnDeath) {
      const uint32 nbToGenerate =
          _GenNbScheme ? _GenNbScheme->get(_Owner, index) : _GenNb;
...
void CPSEmitter::deleteElement(uint32 index,
  ...
  if (_EmissionType == CPSEmitter::onDeath && _EmittedType && _Active) {
    const uint32 nbToGenerate =
        _GenNbScheme ? _GenNbScheme->get(_Owner, index) : _GenNb;
    processEmitConsistent(_Owner->getPos()[index], index, nbToGenerate,
...
void CPSEmitter::resize(uint32 size) {
  ...
  nlassert(size < (1 << 16));
  if (_PeriodScheme && _PeriodScheme->hasMemory())
    _PeriodScheme->resize(size, _Owner->getSize());
  if (_GenNbScheme && _GenNbScheme->hasMemory())
    _GenNbScheme->resize(size, _Owner->getSize());
  _Phase.resize(size);
  if (_MaxEmissionCount != 0) {
...
///==========================================================================
void CPSEmitter::bounceOccurred(uint32 index,
...
void CPSEmitter::serial(NLMISC::IStream &f) {
  ...
  /// version 6  : the flag _EmitDirBasis no longer exist, it has been replaced
  /// by _UserMatrixModeForEmissionDirection
  //
  ...
  bool speedBasisEmission =
      _SpeedBasisEmission; // tmp copy because of bitfield serialization scheme
  ...
  if (ver > 1 && ver < 6) {
    ...
    if (emitDirBasis) {
      ...
      if (_Owner) {
        _UserDirectionMatrixMode = _Owner->getMatrixMode() == PSFXWorldMatrix
                                       ? PSIdentityMatrix
                                       : PSFXWorldMatrix;
      } else {
        _UserDirectionMatrixMode = PSFXWorldMatrix;
  ...
  if (ver >= 4) {
    bool consistentEmission =
        _ConsistentEmission; // tmp copy because of bitfield serialization
                             // scheme
  ...
  if (ver >= 5) {
    bool byassAutoLOD =
        _BypassAutoLOD; // tmp copy because of bitfield serialization scheme
    f.serial(byassAutoLOD);
    _BypassAutoLOD = byassAutoLOD;
  }
  if (ver >= 6) {
    bool userMatrixModeForEmissionDirection =
        _UserMatrixModeForEmissionDirection; // tmp copy because of bitfield
...
bool CPSEmitter::setMaxEmissionCount(uint8 count) {
  ...
  if (ps->getBypassMaxNumIntegrationSteps()) {
    ...
    if (testEmitForever()) {
      ...
      nlwarning(
          "<CPSEmitter::setMaxEmissionCount> can't set max emission count to %d  \
					   with the current configuration : the system has been flagged with     \
					   'BypassMaxNumIntegrationSteps', and should have a finite duration.    \
...
bool CPSEmitter::checkLoop() const {
  ...
  std::vector<const CPSLocated *> leftLoc(
      1); // the located that are left to see
  ...
  do {
    ...
    for (uint32 k = 0; k < curr->getNbBoundObjects(); ++k) {
      const CPSEmitter *emitter =
          dynamic_cast<const CPSEmitter *>(curr->getBoundObject(k));
      if (emitter && emitter->_EmittedType) {
        if (seenLocated.find(emitter->_EmittedType) ==
            seenLocated.end()) // not already seen this one ?
        {
...
bool CPSEmitter::testEmitForever() const {
  ...
  if (!_Owner) {
    nlwarning("<CPSEmitter::testEmitForever> The emitter should be inserted in "
              "a CPSLocated instance for this call to work.");
    nlassert(0);
    return true;
  }
  if (!_Owner->getLastForever())
    return false;
...
///==========================================================================
void CPSEmitterOmni::emit(const NLMISC::CVector &srcPos, uint32 index,
                          CVector &pos, CVector &speed) {
  NL_PS_FUNC(CPSEmitterOmni_emit)
  // TODO : verifier que ca marche si une particule s'emet elle-mem
  nlassert(_EmittedType);

  CVector v(((rand() % 1000) - 500) / 500.0f, ((rand() % 1000) - 500) / 500.0f,
            ((rand() % 1000) - 500) / 500.0f);
  v.normalize();
  v *= _EmitteeSpeedScheme ? _EmitteeSpeedScheme->get(_Owner, index)
                           : _EmitteeSpeed;
...
///==========================================================================
void CPSEmitterDirectionnal::emit(const NLMISC::CVector &srcPos, uint32 index,
                                  CVector &pos, CVector &speed) {
  NL_PS_FUNC(CPSEmitterDirectionnal_emit)
  // TODO : verifier que ca marche si une particule s'emet elle-mem
  nlassert(_EmittedType);

  speed = (_EmitteeSpeedScheme ? _EmitteeSpeedScheme->get(_Owner, index)
                               : _EmitteeSpeed) *
          _Dir;
  pos = srcPos;
}
...
///==========================================================================
void CPSEmitterDirectionnal::deleteElement(
    uint32 index, TAnimationTime timeUntilNextSimStep) {
...
///==========================================================================
void CPSEmitterRectangle::emit(const NLMISC::CVector &srcPos, uint32 index,
                               CVector &pos, CVector &speed) {
  NL_PS_FUNC(CPSEmitterRectangle_emit)
  CVector N = _Basis[index].X ^ _Basis[index].Y;
  pos = srcPos +
        ((rand() % 32000) * (1.f / 16000) - 1.f) * _Width[index] *
            _Basis[index].X +
        ((rand() % 32000) * (1.f / 16000) - 1.f) * _Height[index] *
            _Basis[index].Y;
  speed = (_EmitteeSpeedScheme ? _EmitteeSpeedScheme->get(_Owner, index)
                               : _EmitteeSpeed) *
...
void CPSEmitterRectangle::showTool(void) {
  ...
  for (uint k = 0; k < size; ++k) {
    ...
    CPSUtil::displayBasis(getDriver(), getLocalToWorldMatrix(), mat, 1.f,
                          *getFontGenerator(), *getFontManager());
    setupDriverModelMatrix();

    const CRGBA col = ((lb == NULL || this == lb) && loc == _Owner && index == k
                           ? CRGBA::Red
                           : CRGBA(127, 127, 127));
...
///==========================================================================
void CPSEmitterConic::emit(const NLMISC::CVector &srcPos, uint32 index,
                           CVector &pos, CVector &speed) {
  ...
  CVector dir((float)(rand() * divRand - 1), (float)(rand() * divRand - 1),
...
///==========================================================================
void CPSSphericalEmitter::emit(const NLMISC::CVector &srcPos, uint32 index,
                               CVector &pos, CVector &speed) {
...
///==========================================================================
void CPSRadialEmitter::emit(const NLMISC::CVector &srcPos, uint32 index,
                            NLMISC::CVector &pos, NLMISC::CVector &speed) {
...
///===============================================================================
void CPSEmitter::enableUserMatrixModeForEmissionDirection(
    bool enable /*=true*/) {
...
///===============================================================================
void CPSEmitter::setUserMatrixModeForEmissionDirection(
...
///==========================================================================
void CPSEmitter::updatePSRefCountForUserMatrixUsage(
    bool matrixIsNeededNow, bool matrixWasNeededBefore) {
...
bool CPSEmitter::isUserMatrixUsed() const {
  ...
  return !_SpeedBasisEmission && _UserMatrixModeForEmissionDirection &&
         _UserDirectionMatrixMode == PSUserMatrix;
}
...
void CPSEmitter::doEmitOnce(uint firstInstanceIndex) {
  ...
  if (!_EmittedType)
    return;
  if (!_GenNbScheme && _GenNb == 0)
    return;
  nlassert(_Owner);
  nlassert(
      CParticleSystem::InsideSimLoop); // should only be called by the sim loop
  float emitLOD;
  nlassert(_Owner);
  nlassert(_Owner->getOwner());
  ...
  if (_GenNbScheme) {
    ...
    while (leftToDo) {
      ...
      uint32 *numToEmitPtr = (uint32 *)_GenNbScheme->make(_Owner, k, numToEmit,
                                                          sizeof(uint32), true);
      leftToDo -= toProcess;
      while (toProcess) {
        CVector startPos;
        if (!_Owner->isParametricMotionEnabled()) {
          startPos = _Owner->getPos()[k] -
                     _Owner->getSpeed()[k] * CParticleSystem::EllapsedTime;
        } else {
        ...
        if (nbToGenerate > 0) {
          nbToGenerate =
              std::min(nbToGenerate, (sint32)_EmittedType->getMaxSize());
          processEmitConsistent(startPos, k, nbToGenerate,
                                _Owner->getAgeInSeconds(k) /
    ...
    for (uint k = firstInstanceIndex; k < _Owner->getSize(); ++k) {
      // retrieve previous position (because motion step is done before spawn
      // step)
      CVector startPos;
      if (!_Owner->isParametricMotionEnabled()) {
        startPos = _Owner->getPos()[k] -
...
std::string toString(NL3D::CPSEmitter::TEmissionType type) {
  ...
  nlctassert(NL3D::CPSEmitter::numEmissionType ==
             5); // If this ct assertion is raised, the content of TEmissionType
                 // has changed, so should change this function !
  switch (type) {
  case NL3D::CPSEmitter::regular:
    return "regular";
  case NL3D::CPSEmitter::onDeath:
    return "onDeath";
  case NL3D::CPSEmitter::once:
    return "once";
  case NL3D::CPSEmitter::onBounce:
    return "onBounce";
  case NL3D::CPSEmitter::externEmit:
    return "externEmit";
  default:
    nlassert(0);
    return "";

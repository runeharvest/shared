...
// ***************************************************************************
void CQuadGridClipClusterListDist::insertModel(uint distSetup,
...
// ***************************************************************************
void CQuadGridClipClusterQTreeNode::init(CQuadGridClipCluster *owner,
                                         uint level, bool rootNode,
  ...
  // Create the distMax list only if root or leaf. No models in interleaved
  // branches.
  if (LeafNode)
  ...
  if (visible) {
    ...
    if (unspecified) {
      if (LeafNode) {
        ...
        sint minDistSetup =
            (sint)floor(Owner->_NumDist * dist / Owner->_DistMax);
        // NB if too far, set _NumDist (ie will clip only the infinite objects
        // ones)
      ...
      // udpdate the sons, but don't clip, because we know they are fully
      // visible.
...
// ***************************************************************************
void CQuadGridClipClusterQTreeNode::insertModel(
    const NLMISC::CAABBox &worldBBox, uint distSetup, CTransformShape *model) {
  ...
  if (LeafNode) {
    if (Empty) {
      ...
      // extend the bbox with 2 corners of the incoming bbox (sufficient for an
      // AABBox).
  ...
  // else, recurs insert in branch
    ...
    if (Empty) {
      ...
      // extend the bbox with 2 corners of the son bbox (sufficient for an
      // AABBox).
...
// ***************************************************************************
void CQuadGridClipClusterQTreeNode::profileNumChildren(uint distLevel,
                                                       uint &result) const {
...
// ***************************************************************************
CQuadGridClipCluster::CQuadGridClipCluster(uint numDist, float distMax,
                                           const NLMISC::CAABBox &pivot) {
  _DistMax = distMax;
  _NumDist = numDist;
...
// ***************************************************************************
void CQuadGridClipCluster::addModel(const NLMISC::CAABBox &worldBBox,
                                    CTransformShape *model) {
  ...
  if (modelDistMax < 0)
    ...
    // NB: if >=_DistMax, then distSetup==_NumDist, meaning infinite distance
    // (never dist clipped)

...
CPSFanLight::TVBMap
    CPSFanLight::_ColoredVBMap; // fanlight, no texture, varying color
CPSFanLight::TVBMap
    CPSFanLight::_ColoredTexVBMap; // fanlight, textured, varying color
CPSFanLight::TIBMap CPSFanLight::_IBMap;

static const uint FanLightBufSize =
 ...
 * don't want friend with function template, so we use a static method template
 * of a friend class instead, which gives us the same result :)
 */
class CPSFanLightHelper {
public:
  template <class T, class U>
  static void drawFanLight(T posIt, U timeIt, CPSFanLight &f, uint size,
                           uint32 srcStep) {
    ...
    const uint maxNumFanLightToDealWith =
        std::min(FanLightBufSize, f.getNumFanlightsInVB());
    ...
    float *currentSizePt;  // it points either the particle constant size, or a
                           // size in a table
    float *currentAnglePt; // it points either the particle constant angle, or
                           // an angle in a table

    const uint32 currentSizePtIncrement =
        f._SizeScheme ? 1 : 0; // increment to get the next size for the size
                               // pointer. It is 0 if the size is constant
    const uint32 currentAnglePtIncrement =
        f._Angle2DScheme ? 1
                         : 0; // increment to get the next angle for the angle
                              // pointer. It is 0 if the size is constant
    ...
    do {
      ...
      vb->setNumVertices(toProcess * f._NbFans * 3);
      {
        ...
        if (f._ColorScheme) {
          // we change the color at each fan light center
          f._ColorScheme->make(f._Owner, size - leftToDo, vba.getColorPointer(),
                               vb->getVertexSize() * (f._NbFans + 2), toProcess,
                               false, srcStep);
        }
        if (f._SizeScheme) {
          currentSizePt = (float *)(f._SizeScheme->make(
              f._Owner, size - leftToDo, pSizes, sizeof(float), toProcess, true,
              srcStep));
        ...
        if (f._Angle2DScheme) {
          currentAnglePt = (float *)(f._Angle2DScheme->make(
              f._Owner, size - leftToDo, pAngles, sizeof(float), toProcess,
              true, srcStep));
        } else {
        ...
        for (; posIt != endPosIt; ++posIt, ++timeIt) {
          ...
          *(CVector *)ptVect = *posIt;
          ...
          firstSize = fanSize + (moveIntensity *
                                 CPSUtil::getCos(randomPhaseTab[0] + phaseAdd));
          *(CVector *)ptVect =
              (*posIt) +
              I * firstSize * (CPSUtil::getCos((sint32)currentAngle)) +
              K * firstSize * (CPSUtil::getSin((sint32)currentAngle));
          currentAngle += angleStep;
          ptVect += stride;
          fSize = firstSize;
          // computes other fans
          const sint32 upperBound =
              (sint32)(f._NbFans - f._PhaseSmoothness - 1);
          for (k = 1; k <= upperBound; ++k) {
            fSize = fanSize + (moveIntensity *
                               CPSUtil::getCos(randomPhaseTab[k] + phaseAdd));
            *(CVector *)ptVect =
                (*posIt) + I * fSize * (CPSUtil::getCos((sint32)currentAngle)) +
          ...
          *(CVector *)ptVect =
              (*posIt) +
              I * firstSize * (CPSUtil::getCos((sint32)*currentAnglePt)) +
...
void CPSFanLight::draw(bool opaque) {
  ...
  if (step == (1 << 16)) {
    CPSFanLightHelper::drawFanLight(_Owner->getPos().begin(),
                                    _Owner->getTime().begin(), *this,
                                    numToProcess, step);
  } else {
    CPSFanLightHelper::drawFanLight(
        TIteratorVectStep1616(_Owner->getPos().begin(), 0, step),
        TIteratorTimeStep1616(_Owner->getTime().begin(), 0, step), *this,
        numToProcess, step);
  }
...
///====================================================================================
CPSFanLight::CPSFanLight(uint32 nbFans)
    : _NbFans(nbFans), _PhaseSmoothness(0), _MoveIntensity(1.5f), _Tex(NULL),
      _PhaseSpeed(256) {
...
///====================================================================================
CPSFanLight::~CPSFanLight() { NL_PS_FUNC(CPSFanLight_CPSFanLight) }
...
void CPSFanLight::getVBnIB(CVertexBuffer *&retVb, CIndexBuffer *&retIb) {
  ...
  TVBMap &vbMap = _ColorScheme
                      ? (_Tex == NULL ? _ColoredVBMap : _ColoredTexVBMap)
                      : (_Tex == NULL ? _VBMap : _TexVBMap);
#ifdef NL_NAMED_INDEX_BUFFER
  const char *ibName =
      _ColorScheme ? (_Tex == NULL ? "_ColoredVBMap" : "_ColoredTexVBMap")
                   : (_Tex == NULL ? "_VBMap" : "_TexVBMap");
#endif
...
#ifdef NL_NAMED_INDEX_BUFFER
    if (pbIt->second.getName().empty())
      NL_SET_IB_NAME(pbIt->second, ibName);
#endif
    retIb = &(pbIt->second);
  } else // we need to create the vb
  {
    // create an entry (we setup the primitive block at the same time, this
    // could be avoided, but doesn't make much difference)
    ...
    for (k = 0; k < size; ++k) {
      ...
      if (!_Tex) {
        ...
        for (l = 1; l <= _NbFans + 1; ++l) {
          vba.setColor(l + k * (_NbFans + 2), CRGBA(0, 0, 0));
          vba.setTexCoord(l + k * (_NbFans + 2), 0,
                          NLMISC::CUV((l - 1) / (float)_NbFans, 1));
        }
      }
    }
...
///====================================================================================
void CPSFanLight::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                           IDriver &drv) {

...
void CShapeBankUser::reset() { _ShapeBank.reset(); }

// ***************************************************************************

void CShapeBankUser::setShapeCacheSize(const std::string &shapeCacheName,
                                       sint32 maxSize) {
...
void CShapeBankUser::linkShapeToShapeCache(const std::string &shapeName,
                                           const std::string &shapeCacheName) {
  _ShapeBank.linkShapeToShapeCache(shapeName, shapeCacheName);
}

// ***************************************************************************
void CShapeBankUser::preLoadShapesFromDirectory(
    const std::string &shapeCacheName, const std::string &path,
    const std::string &wildCardNotLwr, bool recurs,
    NLMISC::IProgressCallback *progress, bool flushTextures /*= false*/) {
  ...
  if (!listFile.empty())
    _ShapeBank.preLoadShapes(shapeCacheName, listFile, wildCardNotLwr, progress,
                             flushTextures, _DriverUser->getDriver());
}

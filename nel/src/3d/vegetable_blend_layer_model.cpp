...
void CVegetableBlendLayerModel::registerBasic() {
  CScene::registerModel(VegetableBlendLayerModelId, TransformId,
...
void CVegetableBlendLayerModel::render(IDriver *driver) {
  ...
  CVegetableVBAllocator *vbAllocator =
      &VegetableManager->getVBAllocatorForRdrPassAndVBHardMode(rdrPass, 1);
  ...
  for (uint i = 0; i < SortBlocks.size(); i++) {
    ...
    if (ptrSortBlock->ZSortHardMode != precVBHardMode) {
      ...
      CVegetableVBAllocator *vbAllocator =
          &VegetableManager->getVBAllocatorForRdrPassAndVBHardMode(
              rdrPass, ptrSortBlock->ZSortHardMode);
      vbAllocator->activate();
      // prec.
    ...
    if (ptrSortBlock->ZSortHardMode) {
      nlassert(ptrSortBlock->_SortedTriangleArray.getFormat() ==
  ...
  // refresh list now!
  // We must do it here, because if CVegetableManager::render() is no more
  // called (eg: disabled), then the blend layers models must do nothing.
  SortBlocks.clear();

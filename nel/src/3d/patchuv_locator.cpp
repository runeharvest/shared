...
// ***************************************************************************
void CPatchUVLocator::build(const CPatch *patchCenter, sint edgeCenter,
  ...
  // set it to true. false-d if one of the neighbor patch does not have same
  // number of tile.
  _SameEdgeOrder = true;
  ...
  for (sint i = 0; i < _NPatchs; i++) {
    ...
    if ((edgeNeighbor & 1) == 0) {
      ...
      scY = (float)paNeighbor->getOrderForEdge(edgeNeighbor) /
            (float)patchCenter->getOrderForEdge(edgeCenter);
      ...
      // Manage difference of Order at the edge.
      scX = (float)paNeighbor->getOrderForEdge(edgeNeighbor) /
            (float)patchCenter->getOrderForEdge(edgeCenter);
      // Manage bind on the edge.
      // If patchCenter is binded on a bigger
      if (bindInfo.MultipleBindNum != 0)
    ...
    switch (edgeCenter) {
    case 0:
      uvCenter.set(0, decal * ocT);
      break;
    case 1:
      uvCenter.set(decal * ocS, ocT);
      break;
    case 2:
      uvCenter.set(ocS, (1 - decal) * ocT);
      break;
    case 3:
      uvCenter.set((1 - decal) * ocS, 0);
      break;
    };
    ...
    switch (edgeNeighbor) {
    case 0:
      uvNeighbor.set(0, (1 - decal) * onT);
      break;
    case 1:
      uvNeighbor.set((1 - decal) * onS, onT);
      break;
    case 2:
      uvNeighbor.set(onS, decal * onT);
      break;
    case 3:
      uvNeighbor.set(decal * onS, 0);
      break;
    };
...
uint CPatchUVLocator::selectPatch(const CVector2f &uvIn) {
  if (_NPatchs == 1)
    ...
    case 1:
      ...
      break;
    case 2:
      selection = (ot - uvIn.y) / ot;
      break;
    case 3:
      selection = (os - uvIn.x) / os;
      break;
    }
...
// ***************************************************************************
void CPatchUVLocator::locateUV(const CVector2f &uvIn, uint patch,
                               CPatch *&patchOut, CVector2f &uvOut) {
  if (_NPatchs == 1) {

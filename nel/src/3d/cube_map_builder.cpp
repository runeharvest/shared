...
static uint8 *BuildCubeMapTex(const NLMISC::CVector &start,
                              ...
                              const NLMISC::CVector &vDir, uint size,
                              ICubeMapFunctor &f) {
...
CTextureCube *BuildCubeMap(sint mapSize, ICubeMapFunctor &f,
  ...
  for (k = 0; k < 6; ++k) {
    ...
    uint8 *map =
        luminanceOnly
            ? BuildCubeMapTexLuminance(start[k], uDir[k], vDir[k], mapSize, f)
            : BuildCubeMapTex(start[k], uDir[k], vDir[k], mapSize, f);
    faces[k]->setPointer(
        map, mapSize * mapSize * sizeof(uint8) * (luminanceOnly ? 1 : 4), true,
        false, mapSize, mapSize,
  ...
  static const CTextureCube::TFace toTC[] = {

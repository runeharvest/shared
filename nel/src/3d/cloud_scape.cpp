...
// ------------------------------------------------------------------------------------------------
CCloudScape::CCloudScape(NL3D::IDriver *pDriver)
    : _Noise3D(pDriver)
{
	_Driver = pDriver;
	// Misc purpose VB
	...
	for (i = 0; i < MAX_CLOUDS; ++i)
	{
		...
		for (;;)
		{
			...
			for (uint32 k = 0; k < i; ++k)
			{
				...
				if ((fabs(newX - c2.getX()) < (newSizeX / 2 + c2.getSizeX() / 2)) && (fabs(newY - c2.getY()) < (newSizeY / 2 + c2.getSizeY() / 2)) && (fabs(newZ - c2.getZ()) < (newSizeZ / 2 + c2.getSizeZ() / 2)))
					bRecalc = true;
			}
			if (!bRecalc) break;
		}
...
void CCloudScape::anim(double dt, NL3D::CCamera *pCamera)
{
	...
	if (_ResetCounter != _Driver->getResetCounter())
	{
		...
		while (i < MAX_CLOUDS)
		{
			while (_ShouldProcessCloud[i] && (!_AllClouds[i]._TexBill->setupedIntoDriver() || !_AllClouds[i]._TexOldBill->setupedIntoDriver()))
			{
...
void CCloudScape::makeHalfCloud()
{
	...
	if (_Generate)
	{
		...
		if ((FrontCSE.CloudIndex != -1) && (_ShouldProcessCloud[FrontCSE.CloudIndex] == true) && ((_CloudSchedulerLastAdded[FrontCSE.CloudIndex].ValidPos == false) || ((_CloudSchedulerLastAdded[FrontCSE.CloudIndex].ValidPos == true) && (_CloudSchedulerLastAdded[FrontCSE.CloudIndex].Pos == _CloudScheduler.begin()))))
		{
		...
		else if (_CurrentCSS.NbCloud != 0)
		{
			...
			for (i = 0; i < MAX_CLOUDS; ++i)
				if (_ShouldProcessCloud[i])
				{
					...
					float d = SQR(x + rC.getSizeX() / 2 - Viewer.x) + SQR(rC.getY() + rC.getSizeY() / 2 - Viewer.y) + SQR(rC.getZ() + rC.getSizeZ() / 2 - Viewer.z);
					float d05 = sqrtf(d);
	...
	else
	{
		if (_CurrentCloudInProcess != NULL)
		{
			...
			float d = sqrtf(SQR(c.getX() + c.getSizeX() / 2 - Viewer.x) + SQR(c.getY() + c.getSizeY() / 2 - Viewer.y) + SQR(c.getZ() + c.getSizeZ() / 2 - Viewer.z));
...
void CCloudScape::render()
{
	...
	for (i = 0; i < MAX_CLOUDS; ++i)
	{
		...
		sce.Cloud = &c;
		sce.Distance = sqrtf(SQR(c.getX() + c.getSizeX() / 2 - Viewer.x) + SQR(c.getY() + c.getSizeY() / 2 - Viewer.y) + SQR(c.getZ() + c.getSizeZ() / 2 - Viewer.z));
		nNbCloudToRender++;
	}

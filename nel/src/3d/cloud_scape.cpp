...
static const double MAX_CLOUDS_ANIM_DELTA_TIME =
    0.075; // maximum delta time handled by cloud animation, delta t above that
           // are clamped
static const double MIN_CLOUDS_ANIM_DELTA_TIME =
    0.005; // minimum delta time handled by clouds animation
static const double MAX_TIME_FOR_CLOUD_ANIM =
    0.02; // max number of second spent for cloud render before we check if too
          // slow
static const double MAX_FRAME_PERCENT_FOR_CLOUD_RENDERING =
...
void SCloudTexture3D::init(uint32 nWidth, uint32 nHeight, uint32 nDepth) {
  ...
  Tex = new CTextureMem(Mem, 4 * NbW * Width * NbH * Height, true, false,
                        NbW * Width, NbH * Height, CBitmap::RGBA);
  Tex2 = new CTextureMem(Mem2, 4 * NbW * Width * NbH * Height, true, false,
                         NbW * Width, NbH * Height, CBitmap::RGBA);
  TexBuffer = new CTextureMem(MemBuffer, 4 * Width * Height, true, false, Width,
                              Height, CBitmap::RGBA);
...
// ------------------------------------------------------------------------------------------------
void SCloudTextureClamp::init(uint32 nWidth, uint32 nHeight, uint32 nDepth,
      ...
      for (j = 0; j < NbH; ++j) {
        ...
        for (k = 0; k < Width; ++k)
          for (l = 0; l < Height; ++l) {
            ...
            float r = (x - xc) * (x - xc) / (Width * Width / 4.0f) +
                      (y - yc) * (y - yc) / (Height * Height / 4.0f) +
  ...
  Tex = new CTextureMem(Mem, NbW * Width * NbH * Height, true, false,
                        NbW * Width, NbH * Height, CBitmap::Alpha);
...
CCloudScape::CCloudScape(NL3D::IDriver *pDriver) : _Noise3D(pDriver) {
  _Driver = pDriver;
  // Misc purpose VB
  _VertexBuffer.setVertexFormat(CVertexBuffer::PositionFlag |
...
void CCloudScape::init(SCloudScapeSetup *pCSS, NL3D::CCamera *pCamera) {
  ...
  if (_CurrentCSS.NbCloud == 0) {
    ...
    for (i = 0; i < QUEUE_SIZE; ++i) {
      ...
      _CloudSchedulerLastAdded[cse.CloudIndex].Pos =
          _CloudScheduler.insert(_CloudScheduler.end(), cse);
...
void CCloudScape::anim(double dt, NL3D::CCamera *pCamera) {
  ...
  // If rendering was too slow and took too much time of the previous frame, we
  // decrease the max delta time to give clouds less processing time Otherwise a
  // cycle occurs, and slow rendering propagate from frame to frame

  if (dt != 0 && _LastAnimRenderTime > MAX_TIME_FOR_CLOUD_ANIM &&
      (_LastAnimRenderTime / dt) > MAX_FRAME_PERCENT_FOR_CLOUD_RENDERING) {
    // if cloud rendering take too much time of previous frame, allocate less
    // time for clouds NB : check is only done if clouds rendering is above a
    // given thrheshold, because if only clouds are rendered, then they may take
    // 100% of frame without having slow rendering
  ...
  if (dt > _MaxDeltaTime)
    dt = _MaxDeltaTime;
  if (dt < MIN_CLOUDS_ANIM_DELTA_TIME)
    dt = MIN_CLOUDS_ANIM_DELTA_TIME;
  ...
  if (_TimeNewCSS > _NewCSS.TimeToChange) {
    ...
    _CurrentCSS.WindSpeed =
        (_NewCSS.WindSpeed - _OldCSS.WindSpeed) * inter + _OldCSS.WindSpeed;
    _CurrentCSS.CloudSpeed =
        (_NewCSS.CloudSpeed - _OldCSS.CloudSpeed) * inter + _OldCSS.CloudSpeed;

    _CurrentCSS.Ambient.R =
        (uint8)((_NewCSS.Ambient.R - _OldCSS.Ambient.R) * inter +
                _OldCSS.Ambient.R);
    _CurrentCSS.Ambient.G =
        (uint8)((_NewCSS.Ambient.G - _OldCSS.Ambient.G) * inter +
                _OldCSS.Ambient.G);
    _CurrentCSS.Ambient.B =
        (uint8)((_NewCSS.Ambient.B - _OldCSS.Ambient.B) * inter +
                _OldCSS.Ambient.B);
    _CurrentCSS.Ambient.A =
        (uint8)((_NewCSS.Ambient.A - _OldCSS.Ambient.A) * inter +
                _OldCSS.Ambient.A);

    _CurrentCSS.Diffuse.R =
        (uint8)((_NewCSS.Diffuse.R - _OldCSS.Diffuse.R) * inter +
                _OldCSS.Diffuse.R);
    _CurrentCSS.Diffuse.G =
        (uint8)((_NewCSS.Diffuse.G - _OldCSS.Diffuse.G) * inter +
                _OldCSS.Diffuse.G);
    _CurrentCSS.Diffuse.B =
        (uint8)((_NewCSS.Diffuse.B - _OldCSS.Diffuse.B) * inter +
                _OldCSS.Diffuse.B);
    _CurrentCSS.Diffuse.A =
        (uint8)((_NewCSS.Diffuse.A - _OldCSS.Diffuse.A) * inter +
                _OldCSS.Diffuse.A);

    if (_NewCSS.NbCloud > _OldCSS.NbCloud) {
      // Add some clouds
      float slice =
          (_NewCSS.TimeToChange / 4) / (_NewCSS.NbCloud - _OldCSS.NbCloud);
      sint32 diffCloud = _NewCSS.NbCloud - _OldCSS.NbCloud;

      _CurrentCSS.NbCloud =
          _OldCSS.NbCloud + (1 + (uint32)(_TimeNewCSS / slice));
      ...
      for (i = 0; i < diffCloud; ++i) {
        ...
        if (_TimeNewCSS < i * slice)
          ...
          _CloudPower[_OldCSS.NbCloud + i] =
              (uint8)(255 * (_TimeNewCSS - i * slice) /
                      (3 * _NewCSS.TimeToChange / 4));
      }
    } else {
      // Remove some clouds
      ...
      if (diffCloud) {
        ...
        for (i = 0; i < diffCloud; ++i) {
          if (_TimeNewCSS < i * slice)
            ...
            _CloudPower[_OldCSS.NbCloud - i - 1] =
                (uint8)(255 - 255 * (_TimeNewCSS - i * slice) /
                                  (3 * _NewCSS.TimeToChange / 4));
        }
      }
    }
  ...
  if (_ResetCounter != _Driver->getResetCounter()) {
    ...
    while (i < MAX_CLOUDS) {
      while (_ShouldProcessCloud[i] &&
             (!_AllClouds[i]._TexBill->setupedIntoDriver() ||
              !_AllClouds[i]._TexOldBill->setupedIntoDriver())) {
...
void CCloudScape::makeHalfCloud() {
  ...
  if (_Generate) {
    ...
    // Is the cloud do not have another reference in the list add it now because
    ...
    if ((FrontCSE.CloudIndex != -1) &&
        ...
        ((_CloudSchedulerLastAdded[FrontCSE.CloudIndex].ValidPos == false) ||
         ((_CloudSchedulerLastAdded[FrontCSE.CloudIndex].ValidPos == true) &&
          (_CloudSchedulerLastAdded[FrontCSE.CloudIndex].Pos ==
           _CloudScheduler.begin())))) {
      ...
      for (i = 0; i < MAX_CLOUDS; ++i)
        if (_ShouldProcessCloud[i]) {
          ...
          float ExtrapolatedTime =
              ((0.04f / _NbHalfCloudToUpdate) * QUEUE_SIZE * 2);
          float x = rC.getLastX() + ExtrapolatedTime * _CurrentCSS.WindSpeed;
          // float d =
          // sqrtf(SQR(x+rC.getSizeX()/2-Viewer.x)+SQR(rC.getY()+rC.getSizeY()/2-Viewer.y)+
          //		SQR(rC.getZ()+rC.getSizeZ()/2-Viewer.z));
          float d = SQR(x + rC.getSizeX() / 2 - Viewer.x) +
                    SQR(rC.getY() + rC.getSizeY() / 2 - Viewer.y) +
                    SQR(rC.getZ() + rC.getSizeZ() / 2 - Viewer.z);
          float d05 = sqrtf(d);
      ...
      for (i = 0; i < MAX_CLOUDS; ++i)
        if (_ShouldProcessCloud[i]) {
          ...
          sint32 nbJeton =
              (sint32)(0.5f + (factor * _ExtrapolatedPriorities[i]));
    ...
    if (CloudIndexToAdd != -1) {
      ...
      if (_CloudSchedulerLastAdded[CloudIndexToAdd].ValidPos == true) {
        ...
        SCloudSchedulerEntry &lastCSE =
            *_CloudSchedulerLastAdded[CloudIndexToAdd].Pos;
        sint32 delta = _FrameCounter - lastCSE.Frame;
        lastCSE.DeltaNextCalc = delta;

        // But the cloud can be removed (if so we have to not process it
        // anymore)
        if (newCSE.Power == 0)
      ...
      // If the last cloud occurence of the cloud appear at beginning so no more
      // occurence in list
      if (_CloudSchedulerLastAdded[FrontCSE.CloudIndex].Pos ==
          _CloudScheduler.begin())
        _CloudSchedulerLastAdded[FrontCSE.CloudIndex].ValidPos = false;
    ...
    if (CSEToCalc.CloudIndex == -1) {
      ...
      // To go from Front cloud to CSEToCalc cloud we should take the front
      // DeltaNextCalc

      _CurrentCloudInProcessFuturTime =
          ((0.04 / _NbHalfCloudToUpdate) * FrontCSE.DeltaNextCalc * 2);
      c.setX((float)(c.getLastX() +
                     _CurrentCloudInProcessFuturTime * _CurrentCSS.WindSpeed));

      float d2D = sqrtf(SQR(c.getX() + c.getSizeX() / 2 - Viewer.x) +
                        SQR(c.getY() + c.getSizeY() / 2 - Viewer.y));
    ...
    if (_CurrentCloudInProcess != NULL) {
      ...
      float r = sqrtf(SQR(c.getSizeX() / 2) + SQR(c.getSizeY() / 2) +
                      SQR(c.getSizeZ() / 2));
      float d = sqrtf(SQR(c.getX() + c.getSizeX() / 2 - Viewer.x) +
                      SQR(c.getY() + c.getSizeY() / 2 - Viewer.y) +
...
void CCloudScape::render() {
  ...
  _Driver->setFrustum(f.Left, f.Right, f.Bottom, f.Top, f.Near, f.Far,
                      f.Perspective);
  ...
  for (i = 0; i < MAX_CLOUDS; ++i) {
    ...
    sce.Cloud = &c;
    sce.Distance = sqrtf(SQR(c.getX() + c.getSizeX() / 2 - Viewer.x) +
                         SQR(c.getY() + c.getSizeY() / 2 - Viewer.y) +
                         SQR(c.getZ() + c.getSizeZ() / 2 - Viewer.z));
    nNbCloudToRender++;
  }

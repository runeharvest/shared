...
CPSLocated::CPSLocated()
    : /*_MaxNumFaces(0),*/
      _Size(0), _MaxSize(DefaultMaxLocatedInstance), _CollisionInfoNbRef(0),
      _CollisionNextPos(NULL), _InitialLife(1.f), _LifeScheme(NULL),
      _InitialMass(1.f), _MassScheme(NULL), _LODDegradation(false),
      _ParametricMotion(false), _TriggerOnDeath(false), _LastForever(true),
      _TriggerID(NELID("NONE")), _NonIntegrableForceNbRefs(0),
...
const NLMISC::CMatrix &CPSLocated::getWorldToLocalMatrix() const {
  ...
  switch (getMatrixMode()) {
  case PSFXWorldMatrix:
    return _Owner->getInvertedSysMat();
  case PSIdentityMatrix:
    return NLMISC::CMatrix::Identity;
  case PSUserMatrix:
    return _Owner->getInvertedUserMatrix();
  default:
    nlassert(0);
  }
  nlassert(0);
...
bool CPSLocated::setLastForever() {
  ...
  if (_Owner && _Owner->getBypassMaxNumIntegrationSteps()) {
    ...
    if (!_Owner->canFinish()) {
      _LastForever = false;
      nlwarning("<CPSLocated::setLastForever> Can't set flag : this causes the "
                "system to last forever, and it has been flagged with "
                "'BypassMaxNumIntegrationSteps'. Flag is not set");
      return false;
    }
  }
  CHECK_PS_INTEGRITY
...
void CPSLocated::systemDateChanged() {
  ...
  CHECK_PS_INTEGRITY
  for (TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
       it != _LocatedBoundCont.end(); ++it) {
...
void CPSLocated::releaseRefTo(const CParticleSystemProcess *other) {
  ...
  // dtor observers
  {
    ...
    for (TDtorObserversVect::iterator it = _DtorObserversVect.begin();
         it != _DtorObserversVect.end(); ++it) {
...
void CPSLocated::releaseAllRef() {
  ...
  TDtorObserversVect copyVect(_DtorObserversVect.begin(),
                              _DtorObserversVect.end());
  // call all the dtor observers
  for (TDtorObserversVect::iterator it = copyVect.begin(); it != copyVect.end();
       ++it) {
...
void CPSLocated::notifyMotionTypeChanged(void) {
  ...
  CHECK_PS_INTEGRITY
  for (TLocatedBoundCont::const_iterator it = _LocatedBoundCont.begin();
       it != _LocatedBoundCont.end(); ++it) {
...
void CPSLocated::integrateSingle(float startDate, float deltaT, uint numStep,
                                 ...
                                 NLMISC::CVector *destPos, uint stride) const {
  NL_PS_FUNC(CPSLocated_integrateSingle)
  CHECK_PS_INTEGRITY
  nlassert(supportParametricMotion() && _ParametricMotion);
  if (_IntegrableForces.size() != 0) {
    bool accumulate = false;
    for (TForceVect::const_iterator it = _IntegrableForces.begin();
         it != _IntegrableForces.end(); ++it) {
      nlassert((*it)->isIntegrable());
      (*it)->integrateSingle(startDate, deltaT, numStep, this, indexInLocated,
                             destPos, accumulate, stride);
      accumulate = true;
    }
  } else // no forces applied, just deduce position from date, initial pos and
         // speed
  {
#ifdef NL_DEBUG
    NLMISC::CVector *endPos =
        (NLMISC::CVector *)((uint8 *)destPos + stride * numStep);
#endif
    const CPSLocated::CParametricInfo &pi = _PInfo[indexInLocated];
    destPos = FillBufUsingSubdiv(pi.Pos, pi.Date, startDate, deltaT, numStep,
                                 destPos, stride);
    if (numStep != 0) {
...
void CPSLocated::performParametricMotion(TAnimationTime date) {
  ...
  if (_IntegrableForces.size() != 0) {
    bool accumulate = false;
    for (TForceVect::iterator it = _IntegrableForces.begin();
         it != _IntegrableForces.end(); ++it) {
      nlassert((*it)->isIntegrable());
...
bool CPSLocated::supportParametricMotion(void) const {
  ...
  return _NonIntegrableForceNbRefs == 0 &&
         _NumIntegrableForceWithDifferentBasis == 0;
}

/// ***************************************************************************************
/** When set to true, this tells the system to use parametric motion. This is
 * needed in a few case only, and can only work if all the forces that apply to
 * the system are integrable
 */
...
void CPSLocated::setMatrixMode(TPSMatrixMode matrixMode) {
  ...
  if (matrixMode != getMatrixMode()) {
    ...
    for (TForceVect::iterator fIt = _IntegrableForces.begin();
         fIt != _IntegrableForces.end(); ++fIt) {
...
void CPSLocated::notifyMaxNumFacesChanged(void)
{
    ...
    // we examine whether we have particle attached to us, and ask for the max
number of faces they may want _MaxNumFaces  = 0; for
(TLocatedBoundCont::const_iterator it = _LocatedBoundCont.begin(); it !=
_LocatedBoundCont.end(); ++it)
    {
        if ((*it)->getType() == PSParticle)
        {
            uint maxNumFaces = ((CPSParticle *) (*it))->getMaxNumFaces();
            ///nlassertex(maxNumFaces < ((1 << 16) - 1), ("%s",
(*it)->getClassName().c_str())); _MaxNumFaces += maxNumFaces;
        }
    }
    CHECK_PS_INTEGRITY
}
...
uint CPSLocated::getNumWantedTris() const {
  ...
  for (TLocatedBoundCont::const_iterator it = _LocatedBoundCont.begin();
    ...
    if ((*it)->getType() == PSParticle) {
      numWantedTris +=
          NLMISC::safe_cast<CPSParticle *>(*it)->getNumWantedTris();
    }
  }
  CHECK_PS_INTEGRITY
...
bool CPSLocated::hasParticles() const {
  ...
  for (TLocatedBoundCont::const_iterator it = _LocatedBoundCont.begin();
       it != _LocatedBoundCont.end(); ++it) {
    if ((*it)->getType() == PSParticle && (*it)->hasParticles())
      return true;
  }
  CHECK_PS_INTEGRITY
  return false;
...
bool CPSLocated::hasEmitters() const {
  ...
  for (TLocatedBoundCont::const_iterator it = _LocatedBoundCont.begin();
       it != _LocatedBoundCont.end(); ++it) {
    if ((*it)->getType() == PSEmitter && (*it)->hasEmitters())
      return true;
  }
  CHECK_PS_INTEGRITY
  return false;
...
/// ***************************************************************************************
void CPSLocated::getLODVect(NLMISC::CVector &v, float &offset,
...
void CPSLocated::setInitialLife(TAnimationTime lifeTime) {
  ...
  /** Reset all particles current date to 0. This is needed because we do not
   * check if particle life is over when the date of the system has not gone
   * beyond the life duration of particles
   */
...
CPSLocated::getConversionMatrix(const CParticleSystem &ps,
                                TPSMatrixMode destMode, TPSMatrixMode srcMode) {
  ...
  case PSFXWorldMatrix:
    ...
    case PSFXWorldMatrix:
      return NLMISC::CMatrix::Identity;
    case PSIdentityMatrix:
      return ps.getInvertedSysMat();
    case PSUserMatrix:
      return ps.getUserToFXMatrix();
    default:
      nlassert(0);
    }
    break;
  ...
  case PSUserMatrix:
    switch (srcMode) {
    case PSFXWorldMatrix:
      return ps.getFXToUserMatrix();
    case PSIdentityMatrix:
      return ps.getInvertedUserMatrix();
    case PSUserMatrix:
      return NLMISC::CMatrix::Identity;
    default:
      nlassert(0);
...
NLMISC::CVector CPSLocated::computeI(void) const {
  ...
  if (getMatrixMode() == PSIdentityMatrix) {
    ...
    if (!sysMat.hasScalePart()) {
      // we must express the I vector in the system basis, so we need to
      // multiply it by the inverted matrix of the system
      return getWorldToLocalMatrix().mulVector(
          _Owner->getInvertedViewMat().getI());
    } else {
      return sysMat.getScaleUniform() *
             getWorldToLocalMatrix().mulVector(
                 _Owner->getInvertedViewMat().getI());
    }
  }
}
...
NLMISC::CVector CPSLocated::computeJ(void) const {
  ...
  if (getMatrixMode() == PSIdentityMatrix) {
    ...
    if (!sysMat.hasScalePart()) {
      // we must express the J vector in the system basis, so we need to
      // multiply it by the inverted matrix of the system
      return getWorldToLocalMatrix().mulVector(
          _Owner->getInvertedViewMat().getJ());
    } else {
      return sysMat.getScaleUniform() *
             getWorldToLocalMatrix().mulVector(
                 _Owner->getInvertedViewMat().getJ());
...
NLMISC::CVector CPSLocated::computeK(void) const {
  ...
  if (getMatrixMode() == PSIdentityMatrix) {
    ...
    if (!sysMat.hasScalePart()) {
      // we must express the K vector in the system basis, so we need to
      // multiply it by the inverted matrix of the system
      return getWorldToLocalMatrix().mulVector(
          _Owner->getInvertedViewMat().getK());
    } else {
      return sysMat.getScaleUniform() *
             getWorldToLocalMatrix().mulVector(
                 _Owner->getInvertedViewMat().getK());
...
NLMISC::CVector CPSLocated::computeKWithZAxisAligned(void) const {
  ...
  if (getMatrixMode() == PSIdentityMatrix) {
    ...
    if (!sysMat.hasScalePart()) {
      ...
      return getWorldToLocalMatrix().mulVector(
          CVector(0.f, 0.f, sysMat.getScaleUniform()));
    }
  }
}
...
CPSLocated::~CPSLocated() {
  ...
  // delete all bindable
  for (TLocatedBoundCont::iterator it2 = _LocatedBoundCont.begin();
       it2 != _LocatedBoundCont.end(); ++it2) {
    (*it2)->finalize();
    delete *it2;
  }
...
/// ***************************************************************************************
/**
 * sorted insertion  (by decreasing priority order) of a bindable (particle e.g
 * an aspect, emitter) in a located
 */
bool CPSLocated::bind(CPSLocatedBindable *lb) {
  NL_PS_FUNC(CPSLocated_bind)
  CHECK_PS_INTEGRITY
  nlassert(std::find(_LocatedBoundCont.begin(), _LocatedBoundCont.end(), lb) ==
           _LocatedBoundCont.end());
  TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
  while (it != _LocatedBoundCont.end() &&
         **it < *lb) // the "<" operator sort them correctly
  {
    ++it;
  }
  ...
  // any located bindable that is bound to us should have no element in it for
  // now !! we resize it anyway...
  ...
  if (_Owner) {
    ...
    if (ps->getBypassMaxNumIntegrationSteps()) {
      if (!ps->canFinish()) {
        ...
        nlwarning("<CPSLocated::bind> Can't bind the located : this causes the "
                  "system to last forever, and it has been flagged with "
                  "'BypassMaxNumIntegrationSteps'. Located is not bound.");
        return false;
      }
    }
...
void CPSLocated::remove(const CPSLocatedBindable *p) {
  ...
  CHECK_PS_INTEGRITY
  TLocatedBoundCont::iterator it =
      std::find(_LocatedBoundCont.begin(), _LocatedBoundCont.end(), p);
...
void CPSLocated::registerDtorObserver(CPSLocatedBindable *anObserver) {
  ...
  nlassert(std::find(_DtorObserversVect.begin(), _DtorObserversVect.end(),
                     anObserver) == _DtorObserversVect.end());
...
void CPSLocated::unregisterDtorObserver(CPSLocatedBindable *anObserver) {
  ...
  TDtorObserversVect::iterator it = std::find(
      _DtorObserversVect.begin(), _DtorObserversVect.end(), anObserver);
...
void CPSLocated::postNewElement(const NLMISC::CVector &pos,
  ...
  nlassert(CParticleSystem::InsideSimLoop); // should be called only inside the
                                            // sim loop!
  // In the event loop life of emitter is updated just after particles are
  // spawned, so we must check there if the particle wasn't emitted when the
  // emitter was already destroyed
  // When postNewElement is called, the particle and the emitter that created it
  // live at the same date, so EmitterLife - ParticleLife should be > 1.f
  float emitterLife;
  if (!emitterLocated.getLastForever()) {
    if (emitterLocated._LifeScheme) {
      emitterLife = emitterLocated._Time[indexInEmitter] -
                    lifeTime * CParticleSystem::RealEllapsedTimeRatio *
                        emitterLocated._TimeIncrement[indexInEmitter];
      if (emitterLife >= 1.f) {
        return; // emitter had finished its life
      }
    } else {
      emitterLife =
          emitterLocated._Time[indexInEmitter] * emitterLocated._InitialLife -
          lifeTime * CParticleSystem::RealEllapsedTimeRatio;
  ...
  if (emitterLocated.hasCollisionInfos()) {
    ...
    if (ci.Dist != -1.f) {
      ...
      if ((emitterLocated.getPos()[indexInEmitter] - ci.NewPos) *
              (pos - ci.NewPos) >
          0.f)
  ...
  if (!_Owner->getAutoCountFlag() && sp.MaxNumSpawns == sp.SpawnInfos.size())
    return; // no more place to spawn
  if (getMaxSize() >= ((1 << 16) - 1))
    return;
...
/// ***************************************************************************************
sint32 CPSLocated::newElement(const CPSSpawnInfo &si,
  ...
  // if the emitter is null, we assume that the coordinate are given in the
  // chosen basis for this particle type
  if (_MaxSize == _Size) {
    if (_Owner && _Owner->getAutoCountFlag() &&
        getMaxSize() < ((1 << 16) - 1)) {
      // we are probably in edition mode -> auto-count mode helps to compute
      // ideal particle array size but at the expense of costly allocations
      uint maxSize = getMaxSize();
      resize((uint32)std::min(
          (uint)NLMISC::raiseToNextPowerOf2(maxSize + 1),
          (uint)((1 << 16) - 1))); // force a reserve with next power of 2 (no
                                   // important in edition mode)
      resize(maxSize + 1);
  ...
  // During creation, we interpolate the position of the system (by using the
  // ellapsed time) if particle are created in world basis and if the emitter is
  // in local basis. Example a fireball FX let particles in world basis, but the
  // fireball is moving. If we dont interpolate position between 2 frames,
  // emission will appear to be "sporadic". For now, we manage the local to
  // world case. The world to local is possible, but not very useful
  switch (si.EmitterInfo.Loc ? si.EmitterInfo.Loc->getMatrixMode()
                             : this->getMatrixMode()) {
  case PSFXWorldMatrix:
    ...
    case PSIdentityMatrix: {
      ...
      creationIndex =
          _Pos.insert(_Owner->getSysMat() * si.SpawnPos + fxPosDelta);
    } break;
    case PSUserMatrix: {
      CVector fxPosDelta;
      _Owner->interpolateFXPosDelta(fxPosDelta, si.LifeTime);
      ...
      creationIndex = _Pos.insert(_Owner->getInvertedUserMatrix() *
                                  (_Owner->getSysMat() * si.SpawnPos +
                                   fxPosDelta - userMatrixPosDelta));
    } break;
    default:
      nlassert(0);
    }
  ...
  case PSIdentityMatrix:
    ...
    case PSFXWorldMatrix: {
      ...
      _Owner->interpolateFXPosDelta(fxPosDelta, si.LifeTime);
      creationIndex =
          _Pos.insert(_Owner->getInvertedSysMat() * (si.SpawnPos - fxPosDelta));
    } break;
    case PSIdentityMatrix: {
      creationIndex = _Pos.insert(si.SpawnPos);
    } break;
    case PSUserMatrix: {
      ...
      _Owner->interpolateUserPosDelta(userMatrixPosDelta, si.LifeTime);
      creationIndex = _Pos.insert(_Owner->getInvertedUserMatrix() *
                                  (si.SpawnPos - userMatrixPosDelta));
    } break;
    default:
      nlassert(0);
    }
  ...
  case PSUserMatrix:
    ...
    case PSFXWorldMatrix: {
      ...
      CVector userMatrixPosDelta;
      _Owner->interpolateUserPosDelta(userMatrixPosDelta, si.LifeTime);
      creationIndex = _Pos.insert(_Owner->getInvertedSysMat() *
                                  (_Owner->getUserMatrix() * si.SpawnPos +
                                   userMatrixPosDelta - fxPosDelta));
    } break;
    case PSIdentityMatrix: {
      CVector userMatrixPosDelta;
      _Owner->interpolateUserPosDelta(userMatrixPosDelta, si.LifeTime);
      creationIndex = _Pos.insert(_Owner->getUserMatrix() * si.SpawnPos +
                                  userMatrixPosDelta);
    } break;
    case PSUserMatrix: {
      creationIndex = _Pos.insert(si.SpawnPos);
      ...
      this->getMatrixMode()) // is speed vector expressed in the good basis ?
  {
    ...
    nlassert(_Owner);
    const NLMISC::CMatrix &convMat = getConversionMatrix(
        *_Owner, this->getMatrixMode(), si.SpeedCoordSystem);
    _Speed.insert(convMat.mulVector(si.Speed));
  }

  _InvMass.insert(1.f / ((_MassScheme && si.EmitterInfo.Loc)
                             ? _MassScheme->get(si.EmitterInfo)
                             : _InitialMass));
  ...
  if (getLastForever()) {
    ...
    const float totalLifeTime = (_LifeScheme && si.EmitterInfo.Loc)
                                    ? _LifeScheme->get(si.EmitterInfo)
                                    : _InitialLife;
  ...
  // test whether parametric motion is used, and generate the infos that are
  // needed then
  if (_ParametricMotion) {
    _PInfo.insert(CParametricInfo(
        _Pos[creationIndex], _Speed[creationIndex],
        _Owner->getSystemDate() + CParticleSystem::RealEllapsedTime -
            CParticleSystem::RealEllapsedTimeRatio * si.LifeTime));
  } else {
  ...
  for (TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
    ...
    // if element is an emitter, then must bias the emission time counter
    // because it will be updated of frameDT, but the particle has been alive
    // for (frameDT - deltaT)
    if ((*it)->getType() == PSEmitter) {
      CPSEmitter *pEmit = NLMISC::safe_cast<CPSEmitter *>(*it);
      pEmit->_Phase[creationIndex] -=
          std::max(0.f, (ellapsedTime - si.LifeTime));
    }
  }
  if (doEmitOnce && !CPSEmitter::getBypassEmitOnDeath()) {
    // can be called only outside the sim loop (when the user triggers an
  ...
  ++_Size; // if this is modified, you must also modify the getNewElementIndex
           // in this class because that method give the index of the element
           // being created for overrider of the newElement method of the
           // CPSLocatedClass (which is called just above)
...
/// ***************************************************************************************
sint32 CPSLocated::newElement(const CVector &pos, const CVector &speed,
                              CPSLocated *emitter, uint32 indexInEmitter,
                              TPSMatrixMode speedCoordSystem,
                              bool doEmitOnce /* = false */) {
  NL_PS_FUNC(CPSLocated_newElement)
  CPSSpawnInfo si;
...
static inline uint32 IDToLittleEndian(uint32 input) {
  ...
  return input;
#else
  return ((input & (0xff << 24)) >> 24) || ((input & (0xff << 16)) >> 8) ||
         ((input & (0xff << 8)) << 8) || ((input & 0xff) << 24);
#endif
}
...
void CPSLocated::resize(uint32 newSize) {
  ...
  if (newSize < _Size) {
    for (uint32 k = _Size - 1; k >= newSize; --k) {
      ...
      if (k == 0)
        break; // we're dealing with unsigned quantities
    }
    _Size = newSize;
  }
...
void CPSLocated::serial(NLMISC::IStream &f) {
  ...
  // version 7 : - removed the field _NbFramesToSkip to get some space (it is
  // never used)
  //			   - removed the requestStack (system graph can't
  //contains loops now)
  ...
  if (f.isReading()) {
    ...
    // this cause an assertion in CPSLocated::updateLife, because all particle
    // are assumed to have an age of 0 when the system is started
    // TODO : saving _Time is maybe unecessary... or find something better for
    // CPSLocated::updateLife
  ...
  if (ver < 7) {
    nlassert(f.isReading());
    // previously, there was a request stack serialized (because system
    // permitted loops)
    uint32 size;
    f.serial(size);
    nlassert(size == 0);
...
/// ***************************************************************************************
// integrate speed of particles. Makes eventually use of SSE instructions when
// present
static void IntegrateSpeed(uint count, float *src1, const float *src2,
                           float *dest, float ellapsedTime) {
  NL_PS_FUNC(IntegrateSpeed)
#if 0 // this works, but is not enabled for now. The precision is not that
      // good...
	/*
		#ifdef NL_OS_WINDOWS
  ...
  {
    {
      ...
      if (_Size != 0) // avoid referencing _Pos[0] if there's no size, causes
                      // STL vectors to assert...
        IntegrateSpeed(_Size * 3, &_Pos[0].x, &_Speed[0].x, &_Pos[0].x,
  ...
  } else {
    {
      ...
      IntegrateSpeed(_Size * 3, &_Pos[0].x, &_Speed[0].x,
                     &(*_CollisionNextPos)[0].x, CParticleSystem::EllapsedTime);
...
void CPSLocated::computeNewParticleMotion(uint firstInstanceIndex) {
  ...
  computeCollisions(firstInstanceIndex, &CParticleSystem::_SpawnPos[0],
                    &_Pos[0]);
}
...
void CPSLocated::updateCollisions() {
  ...
  if (getLastForever()) {
    while (currCollision) {
      ...
      std::swap(_Speed[currCollision->Index],
                currCollision->NewSpeed); // keep speed because may be needed
                                          // when removing particles
      // notify each located bindable that a bounce occurred ...
      for (TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
           it != _LocatedBoundCont.end(); ++it) {
        (*it)->bounceOccurred(
            currCollision->Index,
            computeDateFromCollisionToNextSimStep(
                currCollision->Index, getAgeInSeconds(currCollision->Index)));
      }
      if (currCollision->CollisionZone->getCollisionBehaviour() ==
          CPSZone::destroy) {
#ifdef NL_DEBUG
        nlassert(
            CParticleSystem::_ParticleRemoveListIndex[currCollision->Index] ==
            -1);
#endif
...
#endif
        CParticleSystem::_ParticleRemoveListIndex[currCollision->Index] =
            (sint)CParticleSystem::_ParticleToRemove.size() - 1;
      }
      currCollision = currCollision->Next;
    }
  } else {
    while (currCollision) {
      if (_Time[currCollision->Index] >= 1.f) {
        // check whether particles died before the collision. If so, just
        // continue (particle has already been inserted in the remove list), and
        // cancel the collision
        float timeToCollision =
            currCollision->Dist / _Speed[currCollision->Index].norm();
        if (_Time[currCollision->Index] / _TimeIncrement[currCollision->Index] -
                timeToCollision * CParticleSystem::RealEllapsedTimeRatio >=
            1.f) {
      ...
      if (!_LocatedBoundCont.empty()) {
        ...
        for (TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
             it != _LocatedBoundCont.end(); ++it) {
          (*it)->bounceOccurred(currCollision->Index,
...
void CPSLocated::doLODDegradation() {
  ...
  if (CParticleSystem::EllapsedTime > 0) {
    ...
    const uint maxToHave =
        (uint)(_MaxSize * _Owner->getOneMinusCurrentLODRatio());
  ...
  if (pass != PSMotion) {
    {
      ...
      */
      ...
      for (TLocatedBoundCont::iterator it = _LocatedBoundCont.begin();
           it != _LocatedBoundCont.end(); ++it) {
        if ((*it)->isActive()) {
          if ((*it)->getLOD() == PSLod1n2 ||
              _Owner->getLOD() ==
                  (*it)->getLOD()) // has this object the right LOD ?
          {
            (*it)->step(pass);
          }
        }
  ...
  if (!_LastForever) {
    if (_LifeScheme != NULL) {
      TPSAttribTime::iterator itTime = _Time.begin(),
                              itTimeInc = _TimeIncrement.begin();
      for (uint32 k = 0; k < _Size; ++k) {
        *itTime += CParticleSystem::RealEllapsedTime * *itTimeInc;
        if (*itTime >= 1.0f) {
          ...
          nlassert(CParticleSystem::_ParticleToRemove.size() <= _Size);
#endif
          CParticleSystem::_ParticleRemoveListIndex[k] =
              (sint)CParticleSystem::_ParticleToRemove.size() - 1;
        }
        ++itTime;
        ++itTimeInc;
    ...
    } else /// all particles have the same lifetime
    {
      if (_InitialLife != 0) {
        ...
        if (_Owner->getSystemDate() + 0.1f + 2.f * timeInc >=
            (_InitialLife - CParticleSystem::RealEllapsedTime)) {
          // NB : 0.1f + 2.f * timeInc added to avoid case were time of particle
          // is slighty greater than 1.f after life update because that test
          // failed
          TPSAttribTime::iterator itTime = _Time.begin();
          for (uint32 k = 0; k < _Size; ++k) {
            *itTime += timeInc;
...
static inline void removeParticleFromRemoveList(uint indexToRemove,
                                                uint arraySize) {
  NL_PS_FUNC(removeParticleFromRemoveList)
  if (indexToRemove != arraySize) {
    if (CParticleSystem::_ParticleRemoveListIndex[arraySize] != -1) {
      // when a particle is deleted, it is replaced by the last particle in the
      // array if this particle is to be deleted too, must update its new index
      // (becomes the index of the particle that has just been deleted)
      CParticleSystem::_ParticleToRemove
          [CParticleSystem::_ParticleRemoveListIndex[arraySize]] =
              indexToRemove;
      CParticleSystem::_ParticleRemoveListIndex[indexToRemove] =
          CParticleSystem::_ParticleRemoveListIndex[arraySize];
      CParticleSystem::_ParticleRemoveListIndex[arraySize] =
...
void checkRemoveArray(uint size) {
  ...
  for (uint k = 0; k < size; ++k) {
    if (CParticleSystem::_ParticleRemoveListIndex[k] != -1) {
      nlassert(std::find(CParticleSystem::_ParticleRemoveListIndex.begin(),
                         CParticleSystem::_ParticleRemoveListIndex.end(),
                         CParticleSystem::_ParticleRemoveListIndex[k]) !=
               CParticleSystem::_ParticleRemoveListIndex.end());
    }
  }
  for (uint k = 0; k < CParticleSystem::_ParticleToRemove.size(); ++k) {
    nlassert(CParticleSystem::_ParticleRemoveListIndex
                 [CParticleSystem::_ParticleToRemove[k]] == (sint)k);
  }
}
...
inline
#endif
    TAnimationTime
    CPSLocated::computeDateFromCollisionToNextSimStep(
        uint particleIndex, float particleAgeInSeconds) {
  NL_PS_FUNC(CPSLocated_computeDateFromCollisionToNextSimStep)
  // compute time from the start of the sim step to the birth of the particle
  // (or 0 if already born)
  float ageAtStart =
      CParticleSystem::RealEllapsedTime > particleAgeInSeconds
          ? CParticleSystem::RealEllapsedTime - particleAgeInSeconds
          : 0.f;
  ageAtStart /= CParticleSystem::RealEllapsedTimeRatio;
  // compute time to collision. The 'NewSpeed' field is swapped with speed of
  // particle at the sim step start when 'updateCollision' is called, and thus
  // contains the old speed.
  ...
  TAnimationTime result =
      CParticleSystem::EllapsedTime - ageAtStart - timeToCollision;
...
void CPSLocated::removeOldParticles() {
  ...
  // if there are emitters marked as 'on' death, should correct position by
  // moving backward (because motion is done on a whole time step, so particle
  // is further than it should be)
  if (getLastForever()) {
    // if the particle lasts for ever it can be destroyed only if it touch a
    // collision zone flaged as 'destroy' during the call to 'updateCollisions',
    // the list of particles to remove will be updated so just test it
    if (hasCollisionInfos()) {
      for (std::vector<uint>::iterator it =
               CParticleSystem::_ParticleToRemove.begin();
           it != CParticleSystem::_ParticleToRemove.end(); ++it) {
        if (_Collisions[*it].Dist != -1.f) {
          deleteElement(*it,
                        computeDateFromCollisionToNextSimStep(*it, _Time[*it]));
        } else {
          deleteElement(*it);
        }
  ...
  } else if (hasCollisionInfos()) // particle has collision, and limited
                                  // lifetime
  {
    float ellapsedTimeRatio =
        CParticleSystem::EllapsedTime / CParticleSystem::RealEllapsedTime;
    for (std::vector<uint>::iterator it =
             CParticleSystem::_ParticleToRemove.begin();
         it != CParticleSystem::_ParticleToRemove.end(); ++it) {
      ...
      if (_Collisions[*it].Dist == -1.f) {
        ...
        if (_Time[*it] > 1.f) {
          ...
          if (_LifeScheme) {
            _Pos[*it] -= _Speed[*it] *
                         ((_Time[*it] - 1.f) / _TimeIncrement[*it]) *
                         ellapsedTimeRatio;
            timeUntilNextSimStep = (_Time[*it] - 1.f) / _TimeIncrement[*it];
          } else {
            _Pos[*it] -= _Speed[*it] * ((_Time[*it] - 1.f) * _InitialLife) *
                         ellapsedTimeRatio;
        ...
        if (_LifeScheme) {
          timeUntilNextSimStep = computeDateFromCollisionToNextSimStep(
              *it, _Time[*it] / _TimeIncrement[*it]);
          ...
          timeUntilNextSimStep = computeDateFromCollisionToNextSimStep(
              *it, _Time[*it] * _InitialLife);
          // compute age of particle when collision occurred
          _Time[*it] -=
              timeUntilNextSimStep / (_InitialLife == 0.f ? 1.f : _InitialLife);
          NLMISC::clamp(_Time[*it], 0.f, 1.f); // avoid imprecisions
        }
      }
    ...
    if (!isParametricMotionEnabled()) {
      if (_LifeScheme) {
        for (std::vector<uint>::iterator it =
             ...
             it != CParticleSystem::_ParticleToRemove.end(); ++it) {
#ifdef NL_DEBUG
          for (std::vector<uint>::iterator it2 = it;
               it2 != CParticleSystem::_ParticleToRemove.end(); ++it2) {
            nlassert(*it2 < _Size);
          }
#endif
          TAnimationTime timeUntilNextSimStep;
          if (_Time[*it] > 1.f) {
            ...
            timeUntilNextSimStep =
                ((_Time[*it] - 1.f) / _TimeIncrement[*it]) * ellapsedTimeRatio;
            _Pos[*it] -= _Speed[*it] * timeUntilNextSimStep;

            // force time to 1 because emitter 'on death' may rely on the date
            // of emitter to compute its attributes
            _Time[*it] = 0.9999f;
          } else {
            timeUntilNextSimStep = 0.f;
          }
          ...
          removeParticleFromRemoveList(*it, _Size);
#ifdef NL_DEBUG
          for (std::vector<uint>::iterator it2 = it + 1;
               it2 != CParticleSystem::_ParticleToRemove.end(); ++it2) {
            nlassert(*it2 < _Size);
          }
#endif
        ...
        for (std::vector<uint>::iterator it =
          ...
          if (_Time[*it] > 1.f) {
            ...
            timeUntilNextSimStep =
                (_Time[*it] - 1.f) * _InitialLife * ellapsedTimeRatio;
            _Pos[*it] -= _Speed[*it] * timeUntilNextSimStep;
            // force time to 1 because emitter 'on death' may rely on the date
            // of emitter to compute its attributes
            _Time[*it] = 0.9999f;
      ...
      if (_LifeScheme) {
        for (std::vector<uint>::iterator it =
          ...
          if (_Time[*it] > 1.f) {
            // move position backward (compute its position at death)
            timeUntilNextSimStep = (_Time[*it] - 1.f) / _TimeIncrement[*it];
            computeParametricPos(_Owner->getSystemDate() +
...
void CPSLocated::addNewlySpawnedParticles() {
  ...
  if (spawns.SpawnInfos.empty())
    return;
  uint numSpawns = 0;
  if (!_Owner->getAutoCountFlag()) {
    CParticleSystem::_SpawnPos.resize(getMaxSize());
    numSpawns =
        std::min((uint)(_MaxSize - _Size), (uint)spawns.SpawnInfos.size());
  } else {
    numSpawns = (uint)spawns.SpawnInfos.size();
  }
  CParticleSystem::TSpawnInfoVect::const_iterator endIt =
      spawns.SpawnInfos.begin() + numSpawns;
  if (_LastForever) {
    for (CParticleSystem::TSpawnInfoVect::const_iterator it =
             spawns.SpawnInfos.begin();
         it != endIt; ++it) {
    ...
    for (CParticleSystem::TSpawnInfoVect::const_iterator it =
      ...
      if (_Time[insertionIndex] >= 1.f) {
#ifdef NL_DEBUG
        nlassert(CParticleSystem::_ParticleRemoveListIndex[insertionIndex] ==
                 -1);
#endif
        ...
        nlassert(CParticleSystem::_ParticleToRemove.size() <= _Size);
#endif
        CParticleSystem::_ParticleRemoveListIndex[insertionIndex] =
            (sint)CParticleSystem::_ParticleToRemove.size() - 1;
      }
    }
...
/// Setup the driver model matrix. It is set accordingly to the basis used for
/// rendering
/// ***************************************************************************************
...
void CPSLocated::registerIntegrableForce(CPSForce *f) {
  ...
  CHECK_PS_INTEGRITY
  nlassert(std::find(_IntegrableForces.begin(), _IntegrableForces.end(), f) ==
           _IntegrableForces.end()); // force registered twice
...
void CPSLocated::unregisterIntegrableForce(CPSForce *f) {
  ...
  CPSVector<CPSForce *>::V::iterator it =
      std::find(_IntegrableForces.begin(), _IntegrableForces.end(), f);
...
bool CPSLocated::isBound(const CPSLocatedBindable *lb) const {
  ...
  CHECK_PS_INTEGRITY
  TLocatedBoundCont::const_iterator it =
      std::find(_LocatedBoundCont.begin(), _LocatedBoundCont.end(), lb);
...
CPSLocatedBindable::CPSLocatedBindable()
    : _Owner(NULL), _ExternID(0), _LOD(PSLod1n2), _Active(true) {
...
void CPSLocatedBindable::setOwner(CPSLocated *psl) {
  ...
  if (psl == NULL) {
    releaseAllRef();
    if (_Owner) {
      // empty this located bindable. Need to be empty if it must be rebound to
      // another located.
...
/// ***************************************************************************************
void CPSLocatedBindable::displayIcon2d(const CVector tab[], uint nbSegs,
                                       float scale) {
  ...
  for (uint k = 0; k < size; ++k) {
    ...
    mat.setColor((lb == NULL || this == lb) && loc == _Owner && index == k
                     ? CRGBA::Red
                     : CRGBA(127, 127, 127));
...
void CPSLocatedBindable::setExternID(uint32 id) {
  ...
  if (id != 0) {
    if (ps)
      ps->registerLocatedBindableExternID(id, this);
    _ExternID = id;
  }
}
...
void CPSTargetLocatedBindable::serial(NLMISC::IStream &f) {
  ...
  if (f.isReading()) {
    ...
    for (TTargetCont::iterator it = _Targets.begin(); it != _Targets.end();
         ++it) {
      delete (*it);
    }
    _Targets.clear();
...
void CPSTargetLocatedBindable::finalize(void) {
  ...
  /** Release the collisionInfos we've querried. We can't do it in the dtor, as
   * calls to releaseTargetRsc wouldn't be polymorphics for derived class! And
   * the behaviour of releaseTergetRsc is implemented in derived class
   */
  for (TTargetCont::iterator it = _Targets.begin(); it != _Targets.end();
       ++it) {
    releaseTargetRsc(*it);
...
/// ***************************************************************************************
void CPSTargetLocatedBindable::releaseRefTo(
    const CParticleSystemProcess *other) {
  ...
  (*it)->unregisterDtorObserver(this);
  _Targets.erase(it);
  nlassert(std::find(_Targets.begin(), _Targets.end(), other) ==
           _Targets.end());
}
...
/// ***************************************************************************************
void CPSLocated::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                          IDriver &drv) {
...
/// ***************************************************************************************
void CPSLocated::computeCollisions(uint firstInstanceIndex,
  ...
  for (TDtorObserversVect::iterator it = _DtorObserversVect.begin();
    ...
    if ((*it)->getType() == PSZone) {
      static_cast<CPSZone *>(*it)->computeCollisions(*this, firstInstanceIndex,
                                                     posBefore, posAfter);
    }
  }
}
    ...
    if ((*it)->getType() == PSEmitter) {
      ...
      case CPSEmitter::once:
        ...
        if (_Owner->getSystemDate() == 0.f || includeEmitOnce) {
          ...
          // if firstInstanceIndex != 0 then we're dealing with newly created
          // particles, so do the spawn too
...
void CPSCollisionInfo::update(const CPSCollisionInfo &other) {
  ...
  if (Dist == -1) {
    ...
    CollisionZone = other.CollisionZone;
             ...
             Dist) // is the new collision better (e.g it happens sooner) ?
  {
    Dist = other.Dist;
    NewPos = other.NewPos;
    NewSpeed = other.NewSpeed;

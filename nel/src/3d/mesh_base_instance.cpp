...
// ***************************************************************************
CMeshBaseInstance::~CMeshBaseInstance() {
  // If AsyncTextureMode, must disable. This ensure that async loading stop, and
  // that no ref still exist in the AsyncTextureManager
...
void CMeshBaseInstance::registerBasic() {
  CScene::registerModel(MeshBaseInstanceId, TransformShapeId,
                        CMeshBaseInstance::creator);
}

// ***************************************************************************
void CMeshBaseInstance::registerToChannelMixer(CChannelMixer *chanMixer,
  ...
  for (i = 0; i < _AnimatedMaterials.size(); i++) {
    ...
    _AnimatedMaterials[i].registerToChannelMixer(
        chanMixer, prefix + _AnimatedMaterials[i].getMaterialName() + ".");
  }

  // Add any morph
  for (i = 0; i < _AnimatedMorphFactor.size(); i++) {
    _AnimatedMorphFactor[i].registerToChannelMixer(
        chanMixer, prefix + _AnimatedMorphFactor[i].getName());
  }
}
...
// ***************************************************************************
void CMeshBaseInstance::getLightMapName(uint32 nLightMapNb,
...
// ***************************************************************************
void CMeshBaseInstance::getBlendShapeName(uint32 nBlendShapeNb,
...
// ***************************************************************************
void CMeshBaseInstance::setBlendShapeFactor(const std::string &BlendShapeName,
                                            float rFactor) {
...
void CMeshBaseInstance::traverseHrc() {
  ...
  // if the base instance uses automatic animations, we must also setup the date
  // of the channel mixer controlling this object
  if (mb->getAutoAnim()) {
    ...
    if (chanMix) {
       ...
       */
      if (anim) {
        // Animation offset are setuped before clipping, they will be used for
        // detail too.
...
void CMeshBaseInstance::traverseAnimDetail() {
  ...
  if (count0 == count1) {
    for (uint i = 0; i < count0; ++i) {
      ...
      std::list<CMeshBase::CLightMapInfoList::CMatStage>::iterator ite =
          groupInfo.StageList.begin();
      while (ite != groupInfo.StageList.end()) {
        sint animatedLightmap = _AnimatedLightmap[i];
        if (animatedLightmap != -1) {
          CRGBA factor = getOwnerScene()->getAnimatedLightFactor(
              animatedLightmap, groupInfo.LightGroup);
          Materials[ite->MatId].setLightMapFactor(ite->StageId, factor);
        } else {
          CRGBA factor =
              getOwnerScene()->getLightmapGroupColor(groupInfo.LightGroup);
          Materials[ite->MatId].setLightMapFactor(ite->StageId, factor);
        }
        ite++;
      }
...
void CMeshBaseInstance::selectTextureSet(uint id) {
  ...
  for (uint k = 0; k < numMat; ++k) {
    ...
    for (uint l = 0; l < IDRV_MAT_MAXTEXTURES; ++l) {
      if (mat.getTexture(uint8(l)) &&
          mat.getTexture(uint8(l))->isSelectable()) {
        // use a smartPtr so the textFile will be released if just used to set
        // the name for AsyncTextures.
        CSmartPtr<ITexture> texNSV =
            mat.getTexture(uint8(l))->buildNonSelectableVersion(id);
        ...
        // Async case
          ...
          if (AsyncTextures[k].IsTextureFile[l]) {
            CTextureFile *textFile =
                safe_cast<CTextureFile *>((ITexture *)texNSV);
...
void CMeshBaseInstance::initAnimatedLightIndex(const CScene &scene) {
  /* Scan lightmaps used by the shape, and for each, bind the transform shape to
   * an animated lightmap index from the scene. This index will be used at
   * runtime to get quickly a lightmap factor. This index is not set in the
   * CShape because the CShape can be used with several CScene.
   */
  ...
  for (i = 0; i < count; i++) {
    ...
    _AnimatedLightmap[i] =
        scene.getAnimatedLightNameToIndex(lightInfo.AnimatedLight);
  }
...
// ***************************************************************************
bool CMeshBaseInstance::fastIntersect(const NLMISC::CVector &p0,
  ...
  CMeshBase *pMB = static_cast<CMeshBase *>(static_cast<IShape *>(Shape));
  return pMB->getSystemGeometry().fastIntersect(getWorldMatrix(), p0, dir,
...
void CMeshBaseInstance::enableAsyncTextureMode(bool enable) {
  ...
  if (_AsyncTextureMode) {
    ...
    for (uint i = 0; i < Materials.size(); i++) {
      for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; stage++) {
        ...
        CTextureFile *text =
            dynamic_cast<CTextureFile *>(Materials[i].getTexture(uint8(stage)));
        if (text) {
          ...
          // Now, must copy the textureFile, to Avoid writing in CMeshBase
          // TextureFile descriptor !!!
          CTextureFile *tf = new CTextureFile(*text);
          // setup a dummy texture => Instance won't block rendering because
          // texture not yet ready
  ...
  // else, AsyncTextureMode disabled
    ...
    // clear the array => ensure good work if enableAsyncTextureMode(true) is
    // made later
    contReset(_CurrentAsyncTextures);

    // For all TextureFiles in material, copy User setup from AsyncTextures, to
    // real fileName
    for (uint i = 0; i < Materials.size(); i++) {
      for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; stage++) {
        ...
        if (AsyncTextures[i].IsTextureFile[stage]) {
          ...
          CTextureFile *text =
              safe_cast<CTextureFile *>(Materials[i].getTexture(uint8(stage)));
...
void CMeshBaseInstance::startAsyncTextureLoading(
  ...
  CAsyncTextureManager *asyncTextMgr =
  ...
  /* for all new texture names to load, add them to the manager
      NB: done first before release because of RefCount Management (in case of
     same texture name).
  */
  for (i = 0; i < AsyncTextures.size(); i++) {
    for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; stage++) {
      if (AsyncTextures[i].IsTextureFile[stage]) {
        uint id;
        id = asyncTextMgr->addTextureRef(AsyncTextures[i].TextureNames[stage],
                                         this, position);
  ...
  for (uint i = 0; i < _CurrentAsyncTextures.size(); i++) {
    for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; stage++) {
      if (_CurrentAsyncTextures[i].IsTextureFile[stage]) {
        asyncTextMgr->releaseTexture(_CurrentAsyncTextures[i].TextIds[stage],
                                     this);
      }
    }
  }
...
bool CMeshBaseInstance::isAsyncTextureReady() {
  ...
  if (_AsyncTextureToLoadRefCount == 0) {
    // must copy all fileNames into the actual Texture Files. Those are the
    // valid ones now!
    for (uint i = 0; i < _CurrentAsyncTextures.size(); i++) {
      for (uint stage = 0; stage < IDRV_MAT_MAXTEXTURES; stage++) {
        if (_CurrentAsyncTextures[i].IsTextureFile[stage]) {
          ...
          CTextureFile *text =
              safe_cast<CTextureFile *>(Materials[i].getTexture(uint8(stage)));
          // Since the texture is really uploaded in the driver, the true driver
          // Texture Id will be bound to this texture.
          text->setFileName(_CurrentAsyncTextures[i].TextureNames[stage]);
          /* Since driver setup will only occurs when object become visible, it
             is a good idea to release Old driver info, because it may points to
             old driver texture data (eg: old shared textureFile). thus doing so
             release VRAM Texture Memory
          */

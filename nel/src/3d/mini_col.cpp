...
// ***************************************************************************
void CMiniCol::addFaces(const std::vector<CTriangle> &faces, uint16 zoneId,
                        uint16 patchId) {
...
// ***************************************************************************
void CMiniCol::removeLandScapePart(uint16 zoneId, uint16 patchId,
...
void CMiniCol::setCenter(const CVector &center) {
  ...
  for (itZone = _Zones.begin(); itZone != _Zones.end(); itZone++) {
    ...
    // Then for all patchs, must test if the patch must be inserted, or
    // rejected.
    //=============================================
    if (zoneIn) {
      for (sint i = 0; i < (sint)zone.Patchs.size(); i++) {
...
bool CMiniCol::snapToGround(CVector &pos, float hup, float hbot) {
  ...
  for (iFace = _Grid.begin(); iFace != _Grid.end(); iFace++) {
    ...
    // TOIMP: This is VERY SLOW!!! (hope that the quadtree will help, but it
    // still very slow...).
...
// ***************************************************************************
bool CMiniCol::getGroundNormal(const CVector &pos, CVector &normal, float hup,
                               float hbot) {
  CVector b1, b2;
  bool found = false;
  float height = 0.0;
...
// ***************************************************************************
void CMiniCol::getFaces(std::vector<CTriangle> &triresult,
                        const CAABBox &bbox) {
  triresult.clear();

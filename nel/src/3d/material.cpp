...
CMaterial &CMaterial::operator=(const CMaterial &mat) {
  ...
  if (mat._TexUserMat.get()) {
    CUniquePtr<CUserTexMat> texMatClone(
        new CUserTexMat(*(mat._TexUserMat))); // make cpy
...
void CMaterial::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  if (f.isReading()) {
    ...
    if ((_Flags & IDRV_MAT_USER_TEX_MAT_ALL)) // are there user textrue
                                              // coordinates matrix ?
    {
...
// ***************************************************************************
void CMaterial::decompUserTexMat(uint stage, float &uTrans, float &vTrans,
                                 float &wRot, float &uScale, float &vScale) {
  nlassert(stage < IDRV_MAT_MAXTEXTURES);
  nlassert(isUserTexMatEnabled(
      stage));     // must activate animated texture matrix for this stage
...
void CMaterial::selectTextureSet(uint index) {
  for (uint k = 0; k < IDRV_MAT_MAXTEXTURES; ++k) {
    if (_Textures[k] != NULL)
      _Textures[k]->selectTexture(index);
  }
}
...
bool CMaterial::isSupportedByDriver(IDriver &drv, bool forceBaseCaps) const {
  ...
  // special case for radeon : though 3 stages are supported, do as if there
  // were only 2, because of the texEnvColor feature not managed in Direct3D :
  // emulation is provided, but for no more than 2 constants (and if diffuse is
  // not used)
  if (numTexStages == 3)
    numTexStages = 2;
  if (forceBaseCaps)
    numTexStages = std::min(numTexStages, (uint)2);
  switch (getShader()) {
  case Normal: {
    if (getNumUsedTextureStages() > numTexStages)
      return false;
      ...
      if (getTexture(k)) {
        ...
        case InterpolateConstant:
          if (!drv.supportBlendConstantColor())
            return false;
        case EMBM:
          if (forceBaseCaps || !drv.supportEMBM() ||
              !drv.isEMBMSupportedAtStage(k))
            return false;
        case Mad:
          if (!drv.supportMADOperator())
            return false;
        default:
          break;
        }
  ...
  case PerPixelLighting:
    return drv.supportPerPixelLighting(true);
  case PerPixelLightingNoSpec:
    return drv.supportPerPixelLighting(false);
  case Cloud:
    return true;
  case Water:
    return true;

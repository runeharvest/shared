...
CAnimation::CAnimation()
    : _BeginTimeTouched(true), _EndTimeTouched(true), _AnimLoopTouched(true) {
  _MinEndTime = -FLT_MAX;
...
void CAnimation::addTrack(const std::string &name, ITrack *pChannel) {
  ...
  _IdByName.insert(
      TMapStringUInt::value_type(name, (uint32)_TrackVector.size()));
...
void CAnimation::applyTrackQuatHeaderCompression() {
  ...
  // **** second pass: fill, onlmy if some track matchs (fails for instance for
  // big animations)
  if (someTrackOK) {
    uint i;
    ...
    _TrackSamplePack->TrackHeaders.resize(
        (uint32)sampleCounter.TrackHeaders.size());
    ...
    for (i = 0; i < _TrackVector.size(); i++) {
      ITrack *track = _TrackVector[i];
      if (track) {
        ITrack *newTrack = track->applyTrackQuatHeaderCompressionPass1(
            globalKeyOffset, *_TrackSamplePack);
        // if compressed
        if (newTrack) {
...
void CAnimation::applyAnimHeaderCompression(
    CAnimationSet *animationSetOwner,
  ...
  for (it = _IdByName.begin(); it != _IdByName.end(); it++) {
    ...
    std::map<std::string, uint32>::const_iterator itChan =
        channelMap.find(it->first);

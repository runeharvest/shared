...
// ***************************************************************************
void CTextureCube::setTexture(TFace f, ITexture *t) { _Textures[f] = t; }
...
void CTextureCube::doGenerate(bool /* async */) {
  ...
  /*
  if( ( !isPowerOf2(pRefTex->getWidth()) ) || ( pRefTex->getWidth() !=
  pRefTex->getHeight() ) )
  {
      ...
      if( !isPowerOf2(nNewSize) )
      {
          ...
          nNewSize = getPowerOf2(nNewSize);			// 8 -> 3 | 32
  -> 5 nNewSize -= 1;
  // 3 -> 2 | 5 -> 4 nNewSize = 1<<nNewSize;
  // 2 -> 4 | 4 -> 16
      }
  ...
  for (i = 0; i < 6; ++i) {
    ...
    if (_Textures[i]->getUploadFormat() != pRefTex->getUploadFormat()) {
      nlwarning("Bad TextureCube: different UpLoad format: %s and %s",
                _Textures[i]->getShareName().c_str(),
                pRefTex->getShareName().c_str());
      // => replace the texture with the reference
      _Textures[i] = pRefTex;
    }
    // if Auto format, must have the same PixelFormat
    else if (_Textures[i]->getUploadFormat() == ITexture::Auto &&
             _Textures[i]->getPixelFormat() != pRefTex->getPixelFormat()) {
...
bool CTextureCube::isSelectable() const {
  for (uint i = 0; i < 6; ++i) {
    if (_Textures[i] && _Textures[i]->isSelectable())
      return true;
  }
  return false;
}
...
ITexture *CTextureCube::buildNonSelectableVersion(uint index) {
  ...
  (ITexture &)*tc.get() =
      (ITexture &)*this; // invoke ITexture = op for basics parameters

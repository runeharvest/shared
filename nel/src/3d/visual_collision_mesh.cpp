...
#define NL3D_VCM_SHADOW_NUM_CLIP_PLANE_SHIFT                                   \
  (1 << NL3D_VCM_SHADOW_NUM_CLIP_PLANE)
#define NL3D_VCM_SHADOW_NUM_CLIP_PLANE_MASK                                    \
  (NL3D_VCM_SHADOW_NUM_CLIP_PLANE_SHIFT - 1)
...
// ***************************************************************************
void CVisualCollisionMesh::CStaticGrid::create(
    uint nbQuads, uint nbElts, const NLMISC::CAABBox &gridBBox) {
  /* ***********************************************
...
// ***************************************************************************
void CVisualCollisionMesh::CStaticGrid::add(uint16 id,
...
// ***************************************************************************
uint CVisualCollisionMesh::CStaticGrid::select(const NLMISC::CAABBox &bbox,
...
// ***************************************************************************
bool CVisualCollisionMesh::build(const std::vector<CVector> &vertices,
                                 const std::vector<uint32> &triangles,
                                 CVertexBuffer &vbForShadowRender) {
  /* ***********************************************
   *	WARNING: This Class/Method must be thread-safe (ctor/dtor/serial): no
   *static access for instance It can be loaded/called through CAsyncFileManager
   *for instance
  ...
  if (triangles.empty())
    return false;
  // vertices and triangles id are stored in uint16 form. so their should not be
  // more than 65535*3 indices
...
// ***************************************************************************
float CVisualCollisionMesh::getCameraCollision(const CMatrix &instanceMatrix,
                                               CCameraCol &camCol) {
  ...
  for (uint i = 0; i < numSel; i++) {
    ...
    // build the triangle
    camColLocal.minimizeDistanceAgainstTri(_Vertices[_Triangles[triId * 3 + 0]],
...
// ***************************************************************************
NLMISC::CAABBox
CVisualCollisionMesh::computeWorldBBox(const CMatrix &instanceMatrix) {
  CAABBox ret;
  if (!_Vertices.empty()) {
...
// ***************************************************************************
void CVisualCollisionMesh::receiveShadowMap(
    const NLMISC::CMatrix &instanceMatrix,
    const CShadowContext &shadowContext) {
  // empty mesh => no op
  if (_Vertices.empty())
    return;
  ...
  CAABBox localBB;
  localBB = CAABBox::transformAABBox(instanceMatrix.inverted(),
                                     shadowContext.ShadowWorldBB);
  ...
  /*	We want to apply to plane this matrix: IM-1 * MCasterPos,
      where IM=instanceMatrix and MCasterPos= matrix translation of
     "shadowContext.CasterPos" BUT, since to transform a plane, we must do plane
     * M-1, then compute this matrix: localMat= MCasterPos-1 * IM
  */
  CMatrix localMat;
  localMat.setPos(-shadowContext.CasterPos);
  localMat *= instanceMatrix;
  // Allow max bits of planes clip.
  localClipPlanes.resize(
      min((uint)shadowContext.ShadowMap->LocalClipPlanes.size(),
          (uint)NL3D_VCM_SHADOW_NUM_CLIP_PLANE));
  ...
  if (currentTriIdx) {
    ...
    shadowContext.ShadowMapProjector.applyToMaterial(
        instanceMatrix, shadowContext.ShadowMaterial);
    // render

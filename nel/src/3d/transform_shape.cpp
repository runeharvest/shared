...
void CTransformShape::registerBasic() {
  CScene::registerModel(TransformShapeId, TransformId,
...
// ***************************************************************************
void CTransformShape::setupCurrentLightContribution(
    CLightContribution *lightContrib, bool useLocalAtt) {
...
void CTransformShape::changeLightSetup(CRenderTrav *rdrTrav) {
  ...
  rdrTrav->changeLightSetup(_CurrentLightContribution,
                            _CurrentUseLocalAttenuation);
}

// ***************************************************************************
uint CTransformShape::getNumMaterial() const { return 0; }
...
// ***************************************************************************
CMaterial *CTransformShape::getMaterial(uint /* materialId */) { return NULL; }
...
void CTransformShape::traverseRender() {
  ...
  // if the transform is lightable (ie not a fully lightmaped model), setup
  // lighting
  if (isLightable()) {
    ...
    // but if skinned/sticked (directly or not) to a skeleton, take its.
      ...
      _CurrentLightContribution =
          &((CTransformShape *)_AncestorSkeletonModel)->getLightContribution();
  }
...
void CTransformShape::traverseLoadBalancingPass0() {
  ...
  // If this isntance is binded or skinned to a skeleton, take the world matrix
...
void CTransformShape::traverseLoadBalancingPass1() {
  ...
  if (getTransformMode() == RotEuler || getTransformMode() == RotQuat) {
    factor = std::max(
        1.0f, std::max(getScale().x, std::max(getScale().y, getScale().z)));
    if (factor > 1) {
  ...
  _NumTrianglesAfterLoadBalancing =
      _LoadBalancingGroup->computeModelNbFace(_FaceCount);
}

// ***************************************************************************
void CTransformShape::getLightHotSpotInWorld(CVector &modelPos,
  ...
  // If the model is a big lightable, must take radius from aabbox, else suppose
  0 radius. if(isBigLightable())
  {

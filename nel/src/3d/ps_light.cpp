...
// ***************************************************************************************************************
CPSLight::CPSLight()
    : _Color(CRGBA::White), _ColorScheme(NULL), _AttenStart(0.1f),
      _AttenStartScheme(NULL), _AttenEnd(1.f),
      _AttenEndScheme(NULL){NL_PS_FUNC(CPSLight_CPSLight)}
  ...
  if (_Owner && _Owner->getOwner()) {
    ...
    for (uint k = 0; k < _Lights.getSize(); ++k) {
      if (_Lights[k])
        _Owner->getOwner()->getScene()->deleteModel(_Lights[k]);
    }
  } else {
#ifdef NL_DEBUG
    // check that all lights have been deleted
...
void CPSLight::serial(NLMISC::IStream &f) {
  ...
  // version 1 : in version 0, scheme where not resized correctly; Fixed in this
  // version version 0 : color, start attenuation radius, end attenuation
  // radius.
...
void CPSLight::step(TPSProcessPass pass) {
  ...
  while (numLeftLights) {
    ...
    if (_ColorScheme) {
      colPointer =
          (CRGBA *)_ColorScheme->make(_Owner, _Lights.getSize() - numLeftLights,
                                      colors, sizeof(CRGBA), toProcess, true);
      colStride = 1;
    } else {
      colPointer = &_Color;
      colStride = 0;
    ...
    if (_AttenStartScheme) {
      attenStartPointer = (float *)_AttenStartScheme->make(
          _Owner, _Lights.getSize() - numLeftLights, attenStart, sizeof(float),
          toProcess, true);
      attenStartStride = 1;
    } else {
    ...
    uint attenEndStride;
    if (_AttenEndScheme) {
      attenEndPointer = (float *)_AttenEndScheme->make(
          _Owner, _Lights.getSize() - numLeftLights, attenEnd, sizeof(float),
          toProcess, true);
      attenEndStride = 1;
    } else {
    ...
    do {
      if (!*lightIt) {
        ...
        if (scene) {
          *lightIt = NLMISC::safe_cast<CPointLightModel *>(
              scene->createModel(PointLightModelId));
          if (*lightIt) {
      ...
      if (*lightIt) {
        ...
        if (CParticleSystem::OwnerModel) {
          ...
          plm->setClusterSystem(
              CParticleSystem::OwnerModel->getClusterSystem());
        }
        ...
        if (*attenStartPointer != plm->PointLight.getAttenuationBegin() ||
          ...
          plm->PointLight.setupAttenuation(*attenStartPointer,
                                           *attenEndPointer);
        }
...
void CPSLight::setColorScheme(CPSAttribMaker<NLMISC::CRGBA> *scheme) {
  ...
  if (_Owner) {
    if (_ColorScheme && _ColorScheme->hasMemory())
      _ColorScheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  }
}
...
void CPSLight::setAttenStartScheme(CPSAttribMaker<float> *scheme) {
  ...
  if (_Owner) {
    if (_AttenStartScheme && _AttenStartScheme->hasMemory())
      _AttenStartScheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  }
}
...
void CPSLight::setAttenEndScheme(CPSAttribMaker<float> *scheme) {
  ...
  if (_Owner) {
    if (_AttenEndScheme && _AttenEndScheme->hasMemory())
      _AttenEndScheme->resize(_Owner->getMaxSize(), _Owner->getSize());
  }
}
...
void CPSLight::newElement(const CPSEmitterInfo &info) {
  ...
  if (_AttenStartScheme && _AttenStartScheme->hasMemory())
    _AttenStartScheme->newElement(info);
  if (_AttenEndScheme && _AttenEndScheme->hasMemory())
    _AttenEndScheme->newElement(info);
...
void CPSLight::deleteElement(uint32 index) {
  ...
  if (_AttenStartScheme && _AttenStartScheme->hasMemory())
    _AttenStartScheme->deleteElement(index);
  if (_AttenEndScheme && _AttenEndScheme->hasMemory())
    _AttenEndScheme->deleteElement(index);
  if (_Lights[index]) {
...
void CPSLight::resize(uint32 size) {
  ...
  nlassert(size < (1 << 16));
  if (_ColorScheme && _ColorScheme->hasMemory())
    _ColorScheme->resize(size, getOwner() ? getOwner()->getSize() : 0);
  if (_AttenStartScheme && _AttenStartScheme->hasMemory())
    _AttenStartScheme->resize(size, getOwner() ? getOwner()->getSize() : 0);
  if (_AttenEndScheme && _AttenEndScheme->hasMemory())
    _AttenEndScheme->resize(size, getOwner() ? getOwner()->getSize() : 0);
  _Lights.resize(size);
}
...
void CPSLight::releaseAllRef() {
  ...
  // delete all lights, because pointer to the scene is lost after detaching
  // from a system.
  for (uint k = 0; k < _Lights.getSize(); ++k) {
    if (_Lights[k]) {
      nlassert(_Owner &&
               _Owner->getScene()); // if there's an instance there must be a
                                    // scene from which it was created.
...
void CPSLight::show() {
  ...
  for (uint k = 0; k < _Lights.getSize(); ++k) {
    float radiusStart =
        _AttenStartScheme ? _AttenStartScheme->get(_Owner, k) : _AttenStart;
    float radiusEnd =
        _AttenEndScheme ? _AttenEndScheme->get(_Owner, k) : _AttenEnd;
    NLMISC::clamp(radiusStart, 0.f, radiusEnd);
    const NLMISC::CRGBA colStart =
        (((lb == NULL || this == lb) && loc == _Owner && index == k)
             ? CRGBA::Blue
             : CRGBA(0, 0, 127));
    const NLMISC::CRGBA colEnd =
        (((lb == NULL || this == lb) && loc == _Owner && index == k)
             ? CRGBA::Red
             : CRGBA(127, 0, 0));
    //
    CPSUtil::displayDisc(*getDriver(), radiusStart,
                         getLocalToWorldMatrix() * _Owner->getPos()[k], xzMat,
                         numSubdiv, colStart);
    CPSUtil::displayDisc(*getDriver(), radiusStart,
                         getLocalToWorldMatrix() * _Owner->getPos()[k], xyMat,
                         numSubdiv, colStart);
    CPSUtil::displayDisc(*getDriver(), radiusStart,
                         getLocalToWorldMatrix() * _Owner->getPos()[k], yzMat,
                         numSubdiv, colStart);
    //
    CPSUtil::displayDisc(*getDriver(), radiusEnd,
                         getLocalToWorldMatrix() * _Owner->getPos()[k], xzMat,
                         numSubdiv, colEnd);
    CPSUtil::displayDisc(*getDriver(), radiusEnd,
                         getLocalToWorldMatrix() * _Owner->getPos()[k], xyMat,

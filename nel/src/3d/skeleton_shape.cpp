...
void CSkeletonShape::build(const std::vector<CBoneBase> &bones) {
  ...
  for (i = 0; i < _Bones.size(); i++) {
    ...
    if (fatherId >= 0 && _Bones[fatherId].LodDisableDistance != 0) {
      ...
      if (_Bones[i].LodDisableDistance == 0)
        ...
        _Bones[i].LodDisableDistance =
            min(_Bones[i].LodDisableDistance, fatherDist);
    }
  }
  ...
  for (uint j = 1; j < _Lods.size(); j++, it++) {
    ...
    for (i = 0; i < _Bones.size(); i++) {
      float dist = _Bones[i].LodDisableDistance;
      // if the dist of the lod is greater (or equal) to the disableDist of the
      // bone, and if the bone is not "always enabled", disable the bone
...
CTransformShape *CSkeletonShape::createInstance(CScene &scene) {
  ...
  //===============================================
  CSkeletonModel *sm =
      (CSkeletonModel *)scene.createModel(NL3D::SkeletonModelId);
  sm->Shape = this;
...
// ***************************************************************************
bool CSkeletonShape::clip(const std::vector<CPlane> &pyramid,
...
// ***************************************************************************
void CSkeletonShape::getAABBox(NLMISC::CAABBox &bbox) const { bbox = _BBox; }
...
uint CSkeletonShape::getLodForDistance(float dist) const {
  ...
  while (end - 1 > start) {
    ...
    // return the lower_bound, ie return first start with
    // _Lods[pivot].Distance<=dist

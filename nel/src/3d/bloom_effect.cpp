...
CBloomEffect::~CBloomEffect() {
  if (_Init) {
    if (!_DisplayBlurMat.empty()) {
      if (_Driver)
        _Driver->deleteMaterial(_DisplayBlurMat);
    }

    if (!_DisplaySquareBlurMat.empty()) {
      if (_Driver)
        _Driver->deleteMaterial(_DisplaySquareBlurMat);
    }

    if (!_BlurMat.empty()) {
      if (_Driver)
        _Driver->deleteMaterial(_BlurMat);
    }
  }
}
...
void CBloomEffect::applyBloom() {
  ...
  if (_Driver->getPolygonMode() != UDriver::Filled)
    return;
  ...
  if (width >= 256)
    ...
    _BlurWidth = raiseToNextPowerOf2(width) / 2;
  if (height >= 256)
    _BlurHeight = 256;
  else
    _BlurHeight = raiseToNextPowerOf2(height) / 2;

  nlassert(!_BlurFinalTex);
  _BlurFinalTex = _Driver->getRenderTargetManager().getRenderTarget(
      _BlurWidth, _BlurHeight, true);
  nlassert(!_BlurHorizontalTex);
  _BlurHorizontalTex = _Driver->getRenderTargetManager().getRenderTarget(
      _BlurWidth, _BlurHeight, true);

  _DisplayBlurMat.getObjectPtr()->setTexture(0, _BlurFinalTex->getITexture());
  _DisplaySquareBlurMat.getObjectPtr()->setTexture(
      0, _BlurFinalTex->getITexture());
  _DisplaySquareBlurMat.getObjectPtr()->setTexture(
      1, _BlurFinalTex->getITexture());
...
void CBloomEffect::applyBlur() {
  ...
  drvInternal->setUniform4f(IDriver::VertexProgram, 8, 255.f, 255.f, 255.f,
                            255.f);
...
void CBloomEffect::doBlur(bool horizontalBlur) {
  ...
  if (!((CDriverUser *)_Driver)
           ->setRenderTarget(txt, 0, 0, _BlurWidth, _BlurHeight)) {
    nlwarning(
        "setRenderTarget return false with blur texture for bloom effect\n");
    return;
  }
  _Driver->setMatrixMode2D11();
  ...
  // set several decal constants in order to obtain in the render target texture
  // a mix of color of a texel and its neighbored texels on the axe of the pass.
  ...
  drvInternal->setUniform2f(IDriver::VertexProgram, 10,
                            (decalR / (float)_BlurWidth) * blurVec.x,
                            (decalR / (float)_BlurHeight) * blurVec.y);
  drvInternal->setUniform2f(IDriver::VertexProgram, 11,
                            (decal2R / (float)_BlurWidth) * blurVec.x,
                            (decal2R / (float)_BlurHeight) * blurVec.y);
  drvInternal->setUniform2f(IDriver::VertexProgram, 12,
                            (decalL / (float)_BlurWidth) * blurVec.x,
                            (decalL / (float)_BlurHeight) * blurVec.y);
  drvInternal->setUniform2f(IDriver::VertexProgram, 13,
                            (decal2L / (float)_BlurWidth) * blurVec.x,
                            (decal2L / (float)_BlurHeight) * blurVec.y);

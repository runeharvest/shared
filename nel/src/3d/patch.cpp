...
void CPatch::release() {
  if (Zone) {
    ...
    // Free renderPass of landscape, and maybe force computation of texture info
    // on next preRender(). Must do it here, before deletion of Zone, OrderS/T
    // etc...
    resetRenderFar();
  ...
  // the pathc is uncompiled. must do it after clearTessBlocks(), because may
  // use it for vegetable manager, and for updateLighting
  Zone = NULL;
...
void CPatch::computeDefaultErrorSize() {
  ...
  // \todo yoyo: TODO_NOISE: modulate this value with tangents (roundiness of
  // patch), and with the displacement map.
  ErrorSize = ((v1 - v0) ^ (v2 - v0)).norm();
}

// ***************************************************************************
void CPatch::buildBBoxFromBezierPatch(const CBezierPatch &p,
                                      CAABBox &ret) const {
  // Because of the structure of CAABBox, extend() is not fast enough for us.
  // first compute bmin, bmax, then compute the bbox.
  ...
  bmin -= vectorNoiseMax;
  bmax += vectorNoiseMax;
  // NB: this is not very optimal, since the BBox may be very too big. eg: patch
  // 16mx16m => bbox 18mx18m. But remind that tessblocks do not use this BBox,
  // and are computed with the real geometry.
...
// ***************************************************************************
void CPatch::addTrianglesInBBox(CPatchIdent paId, const CAABBox &bbox,
  ...
  addTrianglesInBBoxRecurs(paId, bbox, triangles, tileTessLevel, bpatch, 0,
                           OrderS, 0, OrderT);
}

// ***************************************************************************
void CPatch::addTrianglesInBBoxRecurs(CPatchIdent paId, const CAABBox &bbox,
                                      std::vector<CTrianglePatch> &triangles,
                                      uint8 tessLevel, const CBezierPatch &pa,
                                      uint8 s0, uint8 s1, uint8 t0,
                                      uint8 t1) const {
  ...
  //========================
    ...
    if (lenS > lenT) {
      ...
      // recurs left.
      addTrianglesInBBoxRecurs(paId, bbox, triangles, tessLevel, left, s0,
                               sMiddle, t0, t1);
      // recurs right.
      addTrianglesInBBoxRecurs(paId, bbox, triangles, tessLevel, right, sMiddle,
                               s1, t0, t1);
    } else {
      // subdivide.
      ...
      // recurs top.
      addTrianglesInBBoxRecurs(paId, bbox, triangles, tessLevel, top, s0, s1,
                               t0, tMiddle);
      // recurs bottom.
      addTrianglesInBBoxRecurs(paId, bbox, triangles, tessLevel, bottom, s0, s1,
                               tMiddle, t1);
    }
  }
}
...
// ***************************************************************************
void CPatch::addTileTrianglesInBBox(CPatchIdent paId, const CAABBox &bbox,
                                    std::vector<CTrianglePatch> &triangles,
  ...
  for (tl = 0; tl < tessLen; tl++) {
    ...
    for (sl = 0; sl < tessLen; sl++) {
      ...
      uv3.U = fs1;
      uv3.V = ft0;
      // evaluate patch (with noise). (NB: because of cache, patch decompression
      // cost nothing).
...
// ***************************************************************************
void CPatchQuadBlock::buildTileTriangles(uint8 quadId,
                                         CTrianglePatch triangles[2]) const {
...
void CPatch::fillPatchQuadBlock(CPatchQuadBlock &quadBlock) const {
  ...
  for (tl = 0; tl < lenT + 1; tl++) {
    ...
    for (sl = 0; sl < lenS + 1; sl++) {
      ...
      // Must use computeContinousVertex, to ensure continous coordinate on
      // patch edges
      quadBlock.Vertices[sl + tl * NL_PATCH_BLOCK_MAX_VERTEX] =
...
// ***************************************************************************
void CPatch::addPatchBlocksInBBox(
    CPatchIdent paId, const CAABBox &bbox,
    std::vector<CPatchBlockIdent> &paBlockIds) const {
  CBezierPatch &bpatch = *unpackIntoCache();

  // call with the whole root patch.
  addPatchBlocksInBBoxRecurs(paId, bbox, paBlockIds, bpatch, 0, OrderS, 0,
                             OrderT);
}

// ***************************************************************************
void CPatch::addPatchBlocksInBBoxRecurs(
    CPatchIdent paId, const CAABBox &bbox,
    std::vector<CPatchBlockIdent> &paBlockIds, const CBezierPatch &pa, uint8 s0,
    uint8 s1, uint8 t0, uint8 t1) const {
  uint8 lenS = s1 - s0, lenT = t1 - t0;
  nlassert(lenS > 0);
  nlassert(lenT > 0);
  ...
  //========================
    ...
    if (lenS > lenT) {
      ...
      uint8 sMiddle = (uint8)(((uint)s0 + (uint)s1) / 2);
      // recurs left.
      addPatchBlocksInBBoxRecurs(paId, bbox, paBlockIds, left, s0, sMiddle, t0,
                                 t1);
      // recurs right.
      addPatchBlocksInBBoxRecurs(paId, bbox, paBlockIds, right, sMiddle, s1, t0,
                                 t1);
    } else {
      // subdivide.
      CBezierPatch top, bottom;
      ...
      uint8 tMiddle = (uint8)(((uint)t0 + (uint)t1) / 2);
      // recurs top.
      addPatchBlocksInBBoxRecurs(paId, bbox, paBlockIds, top, s0, s1, t0,
                                 tMiddle);
      // recurs bottom.
      addPatchBlocksInBBoxRecurs(paId, bbox, paBlockIds, bottom, s0, s1,
                                 tMiddle, t1);
    }
  }
}
...
void CPatch::addRefTessBlocks() {
  ...
  if (TessBlocks.empty()) {
    ...
    //==========
    // append patch for Near updateLighting, since TessBlock lightmap may/will
    // exist.
...
void CPatch::clearTessBlocks() {
  ...
  //==========
  if (Zone) {
    // remove patch from Near updateLighting, since no more TessBlock lightmap
    // can exist.
...
void CPatch::dirtTessBlockFaceVector(CTessBlock &tb) {
  // If patch is visible, block's faceVector should exist, but are no more
  // valid.
  if (!isRenderClipped()) {
    // If this tessBlock not already notified to modification.
    if (!tb.isInModifyList()) {
      // Then append, and delete all FaceVector.
      // NB: delete FaceVector now, because the TessBlock himself may disapear
      // soon.
      tb.appendToModifyListAndDeleteFaceVector(
          getLandscape()->_TessBlockModificationRoot,
...
void CPatch::appendFaceToRenderList(CTessFace *face) {
  ...
  if (face->Level < TessBlockLimitLevel) {
    ...
    // Must enlarge the BSphere of the tesblock!!
    // We must do it on a per-face approach, because of tessblocks 's corners
    // which are outside of tessblocks.
    TessBlocks[numtb].extendSphereFirst(face->VBase->EndPos);
    TessBlocks[numtb].extendSphereAdd(face->VLeft->EndPos);
    TessBlocks[numtb].extendSphereAdd(face->VRight->EndPos);
    // I think this should be done too on StartPos, for geomorph (rare??...)
    // problems. \todo yoyo: is this necessary???
    ...
    // Update Tile render (no need to do it if face not at least at tessblock
    // level).
...
void CPatch::appendTileMaterialToRenderList(CTileMaterial *tm) {
  ...
  if (!tc.empty()) {
    ...
    //
    tai.Corners[0] =
        bpatch->eval((float)tm->TileS / OrderS, (float)tm->TileT / OrderT);
    tai.Corners[1] =
        bpatch->eval((tm->TileS + 1.f) / OrderS, (float)tm->TileT / OrderT);
    tai.Corners[2] =
        bpatch->eval((tm->TileS + 1.f) / OrderS, (tm->TileT + 1.f) / OrderT);
    tai.Corners[3] =
        bpatch->eval((float)tm->TileS / OrderS, (tm->TileT + 1.f) / OrderT);
    tai.Center =
        bpatch->eval((tm->TileS + 0.5f) / OrderS, (tm->TileT + 0.5f) / OrderT);
    //
    tai.Normal = bpatch->evalNormal((tm->TileS + 0.5f) / OrderS,
                                    (tm->TileT + 0.5f) / OrderT);
    tai.TileID =
        (uint64)tm; // pointer to tile material serves as a unique identifier
    //
    for (std::vector<ULandscapeTileCallback *>::const_iterator it = tc.begin();
         it != tc.end(); ++it) {
      (*it)->tileAdded(tai);
    }
  }
...
// ***************************************************************************
void CPatch::appendNearVertexToRenderList(CTileMaterial *tileMat,
                                          CTessNearVertex *nv) {
  nlassert(tileMat);
...
// ***************************************************************************
void CPatch::removeNearVertexFromRenderList(CTileMaterial *tileMat,
                                            CTessNearVertex *nv) {
  nlassert(tileMat);
...
void CPatch::makeRoots() {
  ...
  // We don't have to fill the Far vertices VB here, because this patch is still
  // not visible. NB: we can't because we don't have any driver here.
  ...
  /*
      ...
      For rectangles whith OrderT>OrderS. It is VERY IMPORTANT, for
     splitRectangular() reasons.
  ...
  */
  ...
  // NB: since no geomorph is made on endpoints (StartPos==EndPos) of patchs,
  // this is not useful. but it is important to ensure the VP or software
  // geomorph won't crash with bad float values. Init MaxFaceSize.
...
// ***************************************************************************
void CPatch::compile(CZone *z, uint patchId, uint8 orderS, uint8 orderT,
                     CTessVertex *baseVertices[4], float errorSize) {
  nlassert(z);
  Zone = z;
  ...
  // Once the patch is inserted and compiled in a zone, it is ready to be
  // rendered. init the MasterBlock.
  ...
  // Work, since patch 1xX are illegal. => The TileLimitLevel is at least 2
  // level distant from the time where the rectangular patch is said
  // "un-rectangular-ed" (tesselation looks like square). Hence, there is no
  // problem with rectangular UV geomorph (well don't bother me, make a draw :)
  // ).
  TileLimitLevel = pmin * 2 + pmax - pmin;
  // A TessBlock is a 2*2 tile. This simple formula works because patch 1xX are
  // illegal.
  TessBlockLimitLevel = TileLimitLevel - 2;
  // This tell us when the tess face is "un-rectangular-ed" (to say a square).
  // Before, it is a "rectangular" face, which has a strange fxxxxxg split. If
  // patch is square, then SquareLimitLevel=0 (ok!!).
...
CVector CPatch::computeVertex(float s, float t) const {
  ...
  if (getLandscape()->getNoiseMode()) {
    ...
    // unpack. Do it after computeNoise(), because this last may change the
    // cache.
...
CVector CPatch::computeContinousVertex(float s, float t) const {
  ...
  if (edgeIdS >= 0 && edgeIdT >= 0) {
    ...
    if (edgeIdS == 0 && edgeIdT == 3)
      return BaseVertices[0]->EndPos;
    if (edgeIdS == 0 && edgeIdT == 1)
      return BaseVertices[1]->EndPos;
    if (edgeIdS == 2 && edgeIdT == 1)
      return BaseVertices[2]->EndPos;
    if (edgeIdS == 2 && edgeIdT == 3)
      return BaseVertices[3]->EndPos;
    nlstop;
  ...
  // test if on an edge
    ...
    // else must get vertex pos of my neighbor, and average.
      ...
      // and compute vertex. NB: if binded on 2 or 4 patch, it is then possible
      // that stTileOut is on a a corner ( (0,0), (0,1) ...). In this case, we
      // must use the precomputed Vertex pos, for completeness.
      bool onCorner;
      CVector vertexOnNeb =
          nebPatch->computeVertexButCorner(stTileOut.x, stTileOut.y, onCorner);
...
void CPatch::averageTesselationVertices() {
  ...
  // Because CLandscape::averageTesselationVertices() is made on a strict order
  // for patchs (map of zones, then array of patchs), we are sure to overwrite
  // BaseVertices in this order.
  CTessVertex *a = BaseVertices[0];
  CTessVertex *b = BaseVertices[1];
  CTessVertex *c = BaseVertices[2];
  CTessVertex *d = BaseVertices[3];
...
void CPatch::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  // No more compatibility before version 2, because OrderS / OrderT not
  // serialized in preceding version Doens't matter since CZone::serial() do not
  // support it too.
  if (ver < 2) {
  ...
  f.xmlSerial(Interiors[0], Interiors[1], Interiors[2], Interiors[3],
              "INTERIORS");
  ...
  if (ver >= 1) {
    ...
    if (ver <= 6) {
      ...
      // read old version of tilesColors (ie with LightX/LightY/LightZ, which
      // are deprecated now)
      ...
      if (!TileColors.empty()) {
        memcpy(&TileColors[0], &tmpArray[0],
               TileColors.size() * sizeof(CTileColor));
      }
    } else {
  ...
  // Else cannot create here the TileColors, because we need the OrderS/OrderT
  // information... Done into CZone serial.
  if (ver >= 3) {
...
// ***************************************************************************
CTessFace *CPatch::linkTessFaceWithEdge(const CVector2f &uv0,
                                        const CVector2f &uv1,
                                        CTessFace *linkTo) {
  nlassert(Son0 && Son1);
...
void CPatch::bind(CBindInfo Edges[4], bool rebind) {
  ...
  // bind the Realtime bind info here (before any computeVertex, and before bind
  // too).
  sint i;
  for (i = 0; i < 4; i++) {
  ...
  if (!rebind) {
    ...
    c->Pos = c->StartPos = c->EndPos = computeVertex(1, 1);
    d->Pos = d->StartPos = d->EndPos = computeVertex(1, 0);
    // NB: no propagation problem, since the patch has root only (since has to
    // be unbound!!!) Recompute centers.
    Son0->computeSplitPoint();
    Son1->computeSplitPoint();
  } else {
    // Keep old Vertices as computed from neighbors, but reFill the 4
    // FarVertices. NB: don't do it on NearVertices because suppose that Near is
    // Off when a bind occurs (often far away).
  ...
  for (i = 0; i < 4; i++) {
    ...
    nlassert(bind.NPatchs == 0 || bind.NPatchs == 1 || bind.NPatchs == 2 ||
             bind.NPatchs == 4 || bind.NPatchs == 5);
    if (bind.NPatchs == 1) {
      // Bind me on Next.
      this->changeEdgeNeighbor(i,
                               bind.Next[0]->getRootFaceForEdge(bind.Edge[0]));
      // Bind Next on me.
      bind.Next[0]->changeEdgeNeighbor(bind.Edge[0],
      ...
      // Follow the conventions! Make a draw for understand. Small Patchs are
      // numbered in CCW.
      ...
      bind.Next[1]->changeEdgeNeighbor(bind.Edge[1],
                                       &CTessFace::MultipleBindFace);
      ...
      bind1_4[2 * i + 0].FBase =
          &CTessFace::MultipleBindFace; // to link correctly when the root face
                                        // will be splitted.
      bind1_4[2 * i + 0].SonRight =
          bind.Next[0]->getRootFaceForEdge(bind.Edge[0]);
      bind1_4[2 * i + 0].SonLeft =
          bind.Next[1]->getRootFaceForEdge(bind.Edge[1]);
      bind1_4[2 * i + 0].VBase =
          bind.Next[0]->getRootVertexForEdge(bind.Edge[0]);
      // Make second multiple face bind level1.
      bind1_4[2 * i + 1].FBase =
          &CTessFace::MultipleBindFace; // to link correctly when the root face
                                        // will be splitted.
      bind1_4[2 * i + 1].SonRight =
          bind.Next[2]->getRootFaceForEdge(bind.Edge[2]);
      bind1_4[2 * i + 1].SonLeft =
          bind.Next[3]->getRootFaceForEdge(bind.Edge[3]);
      bind1_4[2 * i + 1].VBase =
          bind.Next[2]->getRootVertexForEdge(bind.Edge[2]);
      ...
      bind.Next[1]->changeEdgeNeighbor(bind.Edge[1],
                                       &CTessFace::MultipleBindFace);
      bind.Next[2]->changeEdgeNeighbor(bind.Edge[2],
                                       &CTessFace::MultipleBindFace);
      bind.Next[3]->changeEdgeNeighbor(bind.Edge[3],
                                       &CTessFace::MultipleBindFace);
      ...
      /* I am binded to a bigger patch. There is 2 cases:
          - rebind=false. This is an original Bind of all patch of a zone.
              If my bigger patch has not be bound, I CANNOT do rebind faces,
         since my bigger neighbor patch is not correctly tesselated. Wait for
         the BiggerPatch do the correct bind (see above)
          - rebind=true. This is possible for a patch in border of zone to have
         some bind 1/X (even if Bind 1/X is not possible across zone, it IS
         possible that a patch containing a bind 1/X NOT ON EDGE OF ZONE exist
         (and they do exist...))

          If neighbor bind has been done (must be the case for rebind), MUST do
         the rebind (because of CZoneBindPatch() which first unbind() this, then
         bind())
      */
      ...
      // if my neighbor is bound, i have to do the bind (since CZoneBindPatch()
      // had call unbind)
      if (bind.Next[0]->_BindZoneNeighbor[bind.Edge[0]]) {
        ...
        uvi1.x *= OrderS;
        uvi1.y *= OrderT;
        // build a CPatchUVLocator to transpose coorindate ot this edge in
        // coordinate on the bigger Neighbor patch.
        CBindInfo bindInfo;
        ...
        // Now, traverse the tesselation and find the first CTessFace which use
        // this edge.
        CTessFace *faceNeighbor;
        faceNeighbor = patchNeighbor->linkTessFaceWithEdge(
            uvo0, uvo1, this->getRootFaceForEdge(i));
...
CPatchRdrPass *CPatch::getTileRenderPass(sint tileId, sint pass) {
  // All but lightmap.
  nlassert(pass == NL3D_TILE_PASS_RGB0 || pass == NL3D_TILE_PASS_RGB1 ||
           pass == NL3D_TILE_PASS_RGB2 || pass == NL3D_TILE_PASS_ADD);
...
// ***************************************************************************
void CPatch::getTileUvInfo(sint tileId, sint pass, bool alpha, uint8 &orient,
                           CVector &uvScaleBias, bool &is256x256,
                           uint8 &uvOff) {
  // All but lightmap.
  nlassert(pass == NL3D_TILE_PASS_RGB0 || pass == NL3D_TILE_PASS_RGB1 ||
           pass == NL3D_TILE_PASS_RGB2 || pass == NL3D_TILE_PASS_ADD);
  ...
  if (tileNumber == 0xFFFF) {
    ...
    CTile::TBitmap type;
    // If alpha wanted, return its UV info (works either for Alpha in Diffuse
    // Pass and Alpha in Additive Pass)
    if (alpha)
      type = CTile::alpha;
    else {
      if (additive)
    ...
    uint8 rotalpha;
    Zone->Landscape->getTileUvScaleBiasRot(uint16(tileNumber), type,
                                           uvScaleBias, rotalpha);
...
// ***************************************************************************
void CPatch::appendTessellationLeaves(
...
// ***************************************************************************
CLandscape *CPatch::getLandscape() const { return Zone->getLandscape(); }
...
void CPatch::getBindNeighbor(uint edge, CBindInfo &neighborEdge) const {
  ...
  if (_BindZoneNeighbor[edge] != NULL) {
    getZone()->buildBindInfo(PatchId, edge, _BindZoneNeighbor[edge],
                             neighborEdge);
  } else {
...
void CPatch::setupColorsFromTileFlags(const NLMISC::CRGBA colors[4]) {
  for (uint s = 0; s <= OrderS; ++s) {
    for (uint t = 0; t <= OrderT; ++t) {
      ...
      TileColors[s + t * (OrderS + 1)].Color565 =
          colors[(uint)(Tiles[index].getVegetableState())].get565();
    }
  }
}
...
void CPatch::copyTileFlagsFromPatch(const CPatch *src) {
  nlassert(OrderS == src->OrderS && OrderT == src->OrderT);
...
CTileElement *CPatch::getTileElement(const CUV &uv) {
  ...
  ts = NLMISC::OptFastFloor(uv.U * (OrderS << 8) + 128);
  ts >>= 8;
  tt = NLMISC::OptFastFloor(uv.V * (OrderT << 8) + 128);
  tt >>= 8;
  clamp(ts, 0, OrderS - 1);
  clamp(tt, 0, OrderT - 1);
...
uint32 CPatch::countNumTriFar0() const {
  uint32 numIndex =
      MasterBlock.Far0FaceVector ? *MasterBlock.Far0FaceVector : 0;
...
uint32 CPatch::countNumTriFar1() const {
  uint32 numIndex =
      MasterBlock.Far1FaceVector ? *MasterBlock.Far1FaceVector : 0;
  uint nTessBlock = TessBlocks.size();
  const CTessBlock *pTessBlock = nTessBlock > 0 ? &TessBlocks[0] : NULL;
  for (; nTessBlock > 0; pTessBlock++, nTessBlock--) {
    const CTessBlock &tblock = *pTessBlock;

...
void CZoneManager::checkZonesAround(uint x, uint y, uint area,
  ...
  uint32 i, j;
  for (i = 0; i < _LoadedZones.size(); ++i) {
    // If the loadedzone i do not appear in the zone list so we have to remove
    // it
    bool bFound = false;
...
// ------------------------------------------------------------------------------------------------
CZoneLoadingTask::CZoneLoadingTask(const std::string &sZoneName,
                                   TVolatileZonePtr *ppZone, CVector &pos,
                                   bool monochrome, float factor) {
  *ppZone = NULL;
  _Zone = ppZone;
  _ZoneName = sZoneName;
  Position = pos;
...
void CZoneLoadingTask::run(void) {
  ...
  if (file.open(zonePathLookup)) {
    ...
    *_Zone = ZoneTmp;
    ...
    // nldebug("CZoneLoadingTask::run(): File not found: %s",
    // zonePathLookup.c_str ());
    delete ZoneTmp;

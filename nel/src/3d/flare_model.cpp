...
CFlareModel::CFlareModel() {
  ...
  std::fill(_LastRenderIntervalBegin, _LastRenderIntervalBegin + MaxNumContext,
            (uint64)-2);
  std::fill(_LastRenderIntervalEnd, _LastRenderIntervalEnd + MaxNumContext,
            (uint64)-2);
  std::fill(_NumFrameForOcclusionQuery,
            _NumFrameForOcclusionQuery + MaxNumContext, 1);
  Next = NULL;
}
...
void CFlareModel::traverseRender() {
  ...
  nlassert(drv);
  // For now, don't render flare if occlusion query is not supported (direct
  // read of z-buffer is far too slow)
  ...
  if (fs->getFlareAtInfiniteDist()) {
    ...
    distIntensity = distRatio > fs->getMaxViewDistRatio()
  ...
  const sint xPos =
      (width >> 1) +
      (sint)(width * (((renderTrav.Near * pt.x) / pt.y) - middleX) /
             (renderTrav.Right - renderTrav.Left));
  const sint yPos =
      (height >> 1) -
      (sint)(height * (((renderTrav.Near * pt.z) / pt.y) - middleZ) /
             (renderTrav.Top - renderTrav.Bottom));
  // See if the flare was inside the frustum during the last frame
  // We can't use the scene frame counter because a flare can be rendered in
  // several viewport during the same frame The swapBuffer counter is called
  // only once per frame
  ...
  if (drv->supportOcclusionQuery()) {
    ...
    IOcclusionQuery *lastOQ =
        _OcclusionQuery[flareContext][OcclusionTestFrameDelay - 1];
    IOcclusionQuery *lastDQ =
        _DrawQuery[flareContext][OcclusionTestFrameDelay - 1];
    if (_LastRenderIntervalEnd[flareContext] + 1 == currFrame) {
      if (_LastRenderIntervalEnd[flareContext] -
              _LastRenderIntervalBegin[flareContext] >=
          OcclusionTestFrameDelay - 1) {
        // occlusion test are possibles if at least OcclusionTestFrameDelay
        // frames have ellapsed
        if (lastOQ) {
          ...
          case IOcclusionQuery::NotOccluded:
            if (occlusionTestMesh) {
              if (lastDQ) {
                if (lastDQ->getOcclusionType() !=
                    IOcclusionQuery::NotAvailable) {
                  visibilityRetrieved = true;
                  // eval the percentage of samples that are visible
                  // nlinfo("%d / %d", lastOQ->getVisibleCount(),
                  // lastDQ->getVisibleCount());
                  visibilityRatio = (float)lastOQ->getVisibleCount() /
                                    (float)lastDQ->getVisibleCount();
    ...
    if (issueNewQuery) {
      ...
      if (occlusionTestMesh) {
        ...
        // Insert in list of waiting flare. Don't do it now to avoid repeated
        // setup of test material (a material that don't write to color/zbuffer,
    ...
    visibilityRetrieved = true;
    // The device doesn't support asynchronous query -> must read the z-buffer
    // directly in a slow fashion
    CViewport vp;
    drv->getViewport(vp);
    // Read z-buffer value at the pos we are
    static std::vector<float> v(1);
    NLMISC::CRect rect(
        (sint32)(vp.getX() * width + vp.getWidth() * xPos),
        (sint32)(vp.getY() * height + vp.getHeight() * (height - yPos)), 1, 1);
    drv->getZBufferPart(v, rect);
    // Project in screen space
    float z = (float)(1.0 - (1.0 / pt.y - 1.0 / renderTrav.Far) /
                                (1.0 / renderTrav.Near - 1.0 / renderTrav.Far));
    //
  ...
  //	nlwarning("frame = %d, last frame = %d", (int) currFrame, (int)
  //_LastRenderIntervalEnd[flareContext]);
  if (_LastRenderIntervalEnd[flareContext] + 1 != currFrame) {
    // nlwarning("*");
  ...
  if (visibilityRetrieved) {
    ...
    if (visibilityRatio < _Intensity[flareContext]) {
      ...
      if (p == 0.f) {
        ...
        _Intensity[flareContext] -=
            1.f / p * (float)_Scene->getEllapsedTime() *
            (float)_NumFrameForOcclusionQuery[flareContext];
        if (_Intensity[flareContext] < visibilityRatio) {
          _Intensity[flareContext] = visibilityRatio;
        }
      }
      // nlwarning("intensity update < of %x : %f", (int) this,
      // _Intensity[flareContext]);
      ...
      if (p == 0.f) {
        ...
        // nlwarning("num frame = %d, currFrame = %d, ", (int)
        // _NumFrameForOcclusionQuery[flareContext], (int) currFrame);
        _Intensity[flareContext] +=
            1.f / p * (float)_Scene->getEllapsedTime() *
            (float)_NumFrameForOcclusionQuery[flareContext];
        if (_Intensity[flareContext] > visibilityRatio) {
  ...
  drv->setupModelMatrix(fs->getLookAtMode() ? CMatrix::Identity
                                            : getWorldMatrix());
  // we don't change the fustrum to draw 2d shapes : it is costly, and we need
  // to restore it after the drawing has been done we setup Z to be (near + far)
  // / 2, and setup x and y to get the screen coordinates we want
  ...
  const float aX =
      ((renderTrav.Right - renderTrav.Left) / (float)width) * zPosDivNear;
  const float bX =
      zPosDivNear * (middleX - 0.5f * (renderTrav.Right - renderTrav.Left));
  //
  const float aY =
      -((renderTrav.Top - renderTrav.Bottom) / (float)height) * zPosDivNear;
  const float bY =
      zPosDivNear * (middleZ + 0.5f * (renderTrav.Top - renderTrav.Bottom));
  ...
  const float norm =
      sqrtf((float)(((xPos - (width >> 1)) * (xPos - (width >> 1)) +
                     (yPos - (height >> 1)) * (yPos - (height >> 1))))) /
      (float)(width >> 1);
  ...
  /*if (fs->hasDazzle())
  {
      if (norm < fs->getDazzleAttenuationRange())
      {
          ...
          col.modulateFromui(dazzleColor, (uint) (255.f * _Intensity *
  dazzleIntensity)); material.setColor(col); material.setTexture(0, NULL);
  ...
  if (!fs->getAttenuable()) {
    col.modulateFromui(
        flareColor, (uint)(255.f * distIntensity * _Intensity[flareContext]));
  } else {
    if (norm > fs->getAttenuationRange() || fs->getAttenuationRange() == 0.f) {
      return; // nothing to draw;
    }
    col.modulateFromui(flareColor,
                       (uint)(255.f * distIntensity * _Intensity[flareContext] *
  ...
  if (tex) {
    {
      ...
      if (fs->getScaleWhenDisappear()) {
        size = _Intensity[flareContext] * fs->getSize(0) +
               (1.f - _Intensity[flareContext]) * fs->getSizeDisappear();
      } else {
        size = fs->getSize(0);
      }
      CVector rI, rK;
      if (fs->getFirstFlareKeepSize()) {
        size *= renderTrav.Near *
                (getWorldMatrix().getPos() - renderTrav.CamMatrix.getPos()) * J;
      }
      if (fs->getAngleDisappear() == 0.f) {
        ...
        float angle = (1.f - _Intensity[flareContext]) *
                      fs->getAngleDisappear() * (float)(NLMISC::Pi / 180);
  ...
  if (fs->_LookAtMode) {
    drv->setupModelMatrix(
        CMatrix::Identity); // look at mode is applied only to first flare
  }
  for (uint k = 1; k < MaxFlareNum; ++k) {
    tex = fs->getTexture(k);
    if (tex) {
      // compute vector that map to the center of the flare
      scrPos = (aX * (xPos + dX * fs->getRelativePos(k)) + bX) * I + zPos * J +
               (aY * (yPos + dY * fs->getRelativePos(k)) + bY) * K +
               renderTrav.CamMatrix.getPos();
...
void CFlareModel::initStatics() {
  if (!_OcclusionQuerySettuped) {
    ...
    _OcclusionQueryVB.setPreferredMemory(
        CVertexBuffer::RAMVolatile, false); // use ram to avoid stall, and don't
                                            // want to setup a VB per flare!
...
void CFlareModel::updateOcclusionQueryBegin(IDriver *drv) {
  ...
  initStatics();
  drv->setColorMask(false, false, false,
                    false); // don't write any pixel during the test
}
...
void CFlareModel::renderOcclusionMeshPrimitives(CMesh &mesh, IDriver &drv) {
  ...
  for (uint k = 0; k < numMatrixBlock; ++k) {
    ...
    for (uint l = 0; l < numRdrPass; ++l) {
      CIndexBuffer &ib =
          const_cast<CIndexBuffer &>(mesh.getRdrPassPrimitiveBlock(k, l));
...
void CFlareModel::renderOcclusionTestMesh(IDriver &drv) {
  ...
  if (!occlusionTestMesh)
    return;
  setupOcclusionMeshMatrix(drv, *_Scene);
  drv.activeVertexBuffer(
      const_cast<CVertexBuffer &>(occlusionTestMesh->getVertexBuffer()));

...
void CDeform2d::setupBuffer(uint width, uint height, uint xGranularity,
                            uint yGranularity, uint xQuad, uint yQuad) {
  _Width = width;
  _Height = height;
...
/// compute the UV perturbation at one position of the screen (with screen
/// mapped to 0..1, 0..1)
static inline void computePerturbation(const float x, const float y, float &dx,
                                       float &dy) {
  // dummy func for now
...
void CDeform2d::doDeform(const TPoint2DVect &surf, IDriver *drv,
                         IPerturbUV *uvp) {
  ...
  CQuadEffect::processPoly(surf, (float)_XGranularity, (float)_YGranularity,
                           dest);
  ...
  {
      CDRU::drawLine(dest[k].x, dest[k].y, dest[(k + 1) % dest.size()].x,
  dest[(k + 1) % dest.size()].y, *drv, CRGBA::Red);
  }*/
  ...
  for (it = dest.begin(); it != dest.end(); ++it) {
    ...
    // drv->copyFrameBufferToTexture(_Tex, 0, (uint32) it->x,(uint32) it->y,
    // (uint32) it->x, (uint32) it->y, _XGranularity, _YGranularity);
  }
  ...
  mat.setTexture(0, _Tex);
  {
    ...
    for (it = dest.begin(); it != dest.end(); ++it, k += 4) {
      ...
      vba.setVertexCoord(k + 1,
                         NLMISC::CVector(it->x + _XGranularity, 0, it->y));
      vba.setVertexCoord(k + 2, NLMISC::CVector(it->x + _XGranularity, 0,
                                                it->y + _YGranularity));
      vba.setVertexCoord(k + 3,
                         NLMISC::CVector(it->x, 0, it->y + _YGranularity));

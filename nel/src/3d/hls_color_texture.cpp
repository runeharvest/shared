...
// ***************************************************************************
CHLSColorTexture::CHLSColorTexture() { reset(); }
...
void CHLSColorTexture::addMask(const NLMISC::CBitmap &bmpIn, uint threshold) {
  ...
  for (uint i = 0; i < bitDataSize; i++) {
    _Texture[_BlockToCompressIndex + i] |=
        newMask.Data[newMask.MixtBlockIndex + i];
  }
}
...
// ***************************************************************************
void CHLSColorTexture::buildColorVersion(const CHLSColorDelta *colDeltaList,
                                         NLMISC::CBitmap &out) {
  ...
  // copy from texture (to have non colored version already copied, and also
  // ALPHA ok)
  memcpy(&dstTexture[0], &_Texture[0], dstTexture.size());

  // create a tmp uncompressed block array, which will receive coloring of mixt
  // blocks
  dstUnCompTexture.resize(numBlocks * BLOCK_NUM_PIXEL);

  // For all blockToCompress, uncompress them in dstUnCompTexture, because they
  // will blend with future mask coloring
  uint n = numBlocks;
  ...
  for (uint maskId = 0; maskId < _Masks.size(); maskId++) {
    ...
    while (n > 0) {
      ...
      for (; nBits > 0; nBits--) {
        ...
        if (bitMask & 1) {
          ...
          // If this block is "a block to recompress", then must uncompress it
          // in dstUnCompPtr
    ...
    // ---- for all mixtblock ot this mask, color, uncompress and blend in store
    // in dstUnCompTexture
    ...
    while (n > 0) {
      ...
      for (; nBits > 0; nBits--) {
        ...
        if (bitMask & 1) {
          ...
          // blend tmpColoredBlockRGBA into dstUnCompPtr, according to
          // alphaMixtBlock.
          for (uint i = 0; i < 16; i++) {
            dstUnCompPtr[i].blendFromuiRGBOnly(
                dstUnCompPtr[i], tmpColoredBlockRGBA[i], *alphaMixtBlock);
            // next pixel
            alphaMixtBlock++;
          }
        }
...
// ***************************************************************************
void CHLSColorTexture::colorizeDXTCBlockRGB(const uint8 *srcPtr, uint8 *dstPtr,
                                            uint8 dHue, uint dLum, uint dSat) {
  // get modifier.
  CFastHLSModifier &fastHLS = CFastHLSModifier::getInstance();

  // apply the color on the 2 DXTC colors
  *(uint16 *)(dstPtr + 8) =
      fastHLS.applyHLSMod(*(uint16 *)(srcPtr + 8), dHue, dLum, dSat);
  *(uint16 *)(dstPtr + 10) =
      fastHLS.applyHLSMod(*(uint16 *)(srcPtr + 10), dHue, dLum, dSat);
}

// ***************************************************************************
void CHLSColorTexture::uncompressBlockRGB(const uint8 *srcDXTC,
                                          CRGBA *dstRGBA) {
  CRGBA c[4];
...
// ***************************************************************************
void CHLSColorTexture::computeMinMax(sint *diffBlock, CVectorInt &v,
                                     sint mean[3], sint rgb0[3], sint rgb1[3]) {
  ...
  // avoid overflow. here, Higher possible bit is 16+8+2 (add of 3 values=> *4)
  // == 26 26-12= 14. 14+16=30 => ok.
  mind >>= 12;
  maxd >>= 12;
...
void CHLSColorTexture::compressBlockRGB(CRGBA *srcRGBA, uint8 *dstDXTC) {
  ...
  for (n = 16; n > 0; n--, src++) {
    ...
    mean[1] += src->G;
    mean[2] += src->B;
    // at same time, setup alpha to 0. Important for "compute bits" part (see
    // MMX)!!
    src->A = 0;
  }
  mean[0] >>= 4;
  ...
  sint coMat[3][3];
  // Apply std RGB factor (0.3, 0.56, 0.14) to choose the best Axis. This give
  // far much best results.
  ...
  // set a Fixed 16:16.
  CVectorInt vInt;
  // don't bother if OptFastFloorBegin() has been called. 16:16 precision is
  // sufficient.
  ...
  c[2].blendFromui(c[0], c[1], 85);
  c[3].blendFromui(c[0], c[1], 171);
  // it is important that c[] and src Alpha are set to 0, because of "pmaddwd"
  // use in MMX code...
  c[0].A = 0;
  c[1].A = 0;
  c[2].A = 0;
  ...
  if (CSystemInfo::hasMMX()) {
    ...
    for (n = 16; n > 0; n--, src++) {
      ...
      for(i=0;i<4;i++)
      {
          // applying factors such *23, *80, *6 gives better results, but slower
      (in MMX). uint	dist= sqr((sint)src->R-(sint)c[i].R); dist+=
      sqr((sint)src->G-(sint)c[i].G); dist+= sqr((sint)src->B-(sint)c[i].B);
          if(dist<minDist)
          {
              minDist= dist;
              id= i;

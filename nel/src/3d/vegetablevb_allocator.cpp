...
/*
    Once a reallocation of a VB occurs, how many vertices we add to the
...
// ***************************************************************************
CVegetableVBAllocator::~CVegetableVBAllocator() { clear(); }
...
void CVegetableVBAllocator::updateDriver(IDriver *driver) {
  ...
  if (_Driver == NULL || driver != _Driver ||
      (!_VBHard.isResident() && (_VBHard.capacity() != 0))) {
    // delete old VBHard.
    deleteVertexBufferHard();
    _Driver = driver;
    _VBHardOk =
        (_MaxVertexInBufferHard > 0) && (_Driver->supportVertexBufferHard());
    /* Because so much lock/unlock are performed during a frame (refine/clip
       etc...). we must disable VBHard for ATI Gl extension. NB: CLandscape
       don't do this and fast copy the entire VB each frame. This is not
       possible for vegetables because the VB describe all Vegetable around the
       camera, not only what is in frustrum. Hence a fast copy each frame would
       copy far too much unseen vertices (4x).
    */
...
// ***************************************************************************
bool CVegetableVBAllocator::exceedMaxVertexInBufferHard(
    uint numAddVerts) const {
...
uint CVegetableVBAllocator::allocateVertex() {
  ...
  if (_VertexFreeMemory.empty()) {
    ...
    // try to not overlap _MaxVertexInBufferHard limit, to avoid VBufferHard to
    // be disabled.
    if (_NumVerticesAllocated < _MaxVertexInBufferHard &&
        _NumVerticesAllocated + newResize > _MaxVertexInBufferHard) {
...
// ***************************************************************************
void CVegetableVBAllocator::allocateVertexBufferAndFillVBHard(
    uint32 numVertices) {
  ...
  if (_VBHardOk) {
    /* Prefer allocate the VBHard with the Max Vertex count only ONCE,
        ...
        The problem is with 50000 AGP vertices, it costs 3 Mo. If we do
       iterative allocation (500 Ko, 600 Ko, 700 Ko,.....) we may have problem
       with free holes (Vegetable could no more enter in the 16 or 8 Mo AGP
       limit!)
    */
  ...
  // nlinfo("VEGET: Alloc %d verts. %s", numVertices,
  // _VBHardOk?"VBHard":"VBSoft");
}
...
void CVegetableVBAllocator::setupVBFormat() {
  ...
  if (_Type == VBTypeLighted) {
    ...
    _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_NORMAL,
                       CVertexBuffer::Float3); // v[2]
    _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_BENDINFO,
                       CVertexBuffer::Float3); // v[9]
  }
  // If unlit
  else {
    // slightly different VertexProgram, v[0].w== BendWeight, and v[9].x==
    // v[0].norm()
    _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_POS, CVertexBuffer::Float4); // v[0]
    // Unlit VP has BlendDistance in v[9].w
    _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_BENDINFO,
                       CVertexBuffer::Float4); // v[9]
  }
  _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_COLOR0,
                     CVertexBuffer::UChar4); // v[3]
  _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_COLOR1,
                     CVertexBuffer::UChar4);                            // v[4]
  _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_TEX0, CVertexBuffer::Float2); // v[8]
  _VBSoft.addValueEx(NL3D_VEGETABLE_VPPOS_CENTER,
                     CVertexBuffer::Float3); // v[10]
  _VBSoft.initEx();
}

...
void CPackedWorld::build(std::vector<TPackedZoneBaseSPtr> &packedZones) {
  ...
  for (uint k = 0; k < packedZones.size(); ++k) {
    ...
    _Zones[k] = zi;
    sint zoneMinX =
        (sint)floorf(packedZones[k]->Box.getMin().x / 160.f) - (sint)_ZoneMinX;
    sint zoneMinY =
        (sint)floorf(packedZones[k]->Box.getMin().y / 160.f) - (sint)_ZoneMinY;
    sint zoneMaxX =
        (sint)floorf(packedZones[k]->Box.getMax().x / 160.f) - (sint)_ZoneMinX;
    sint zoneMaxY =
        (sint)floorf(packedZones[k]->Box.getMax().y / 160.f) - (sint)_ZoneMinY;
    for (sint y = zoneMinY; y <= zoneMaxY; ++y) {
      if (y < 0)
        continue;
      if (y >= (sint)_ZoneGrid.getHeight())
        break;
      for (sint x = zoneMinX; x <= zoneMaxX; ++x) {
        if (x < 0)
          continue;
        if (x >= (sint)_ZoneGrid.getWidth())
          break;
...
// *************************************************************************************************
bool CPackedWorld::raytrace(
    const NLMISC::CVector &start, const NLMISC::CVector &end, CVector &inter,
  ...
  do {
    ...
    for (uint k = 0; k < currZoneList.size(); ++k) {
      if (_Zones[currZoneList[k]].RaytraceCounter !=
          _RaytraceCounter) // already visited
      {
        NLMISC::CVector normalTmp;
        if (_Zones[currZoneList[k]].Zone->raytrace(
                start, currEnd, currInter, testedTriangles, &normalTmp)) {
          ...
          if (dist < bestDist) {
            ...
            inter = currInter;
            currEnd =
                currInter; // during search, just seek hit that are nearest
          }
        }
...
void CPackedWorld::select(
  ...
  for (sint y = minY; y < (sint)(minY + borders.size()); ++y) {
    if (y < 0 || y >= (sint)_ZoneGrid.getHeight())
      continue;
    for (sint x = borders[y - minY].first; x <= borders[y - minY].second; ++x) {
      if (x < 0 || x >= (sint)_ZoneGrid.getWidth())
        continue;
      {

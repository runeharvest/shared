...
#define NL3D_HAUTO_SET_CLUSTER_SYSTEM                                          \
  H_AUTO_USE(NL3D_Transform_Set_Cluster_System)
...
void UTransform::setClusterSystem(UInstanceGroup *pIG) {
  ...
  if (object->getForceClipRoot()) {
    nlwarning("Transform has been flagged to be glued to the root, and thus "
              "can't be clusterized. See UTransform::setForceClipRoot(bool).");
    return;
  }
...
void UTransform::getLastParentClusters(
    std::vector<CCluster *> &clusters) const {
  CTransform *object = getObjectPtr();
  CScene *scene = object->getOwnerScene();
  // look in the list of parent of the transform object and extract the CCluster
  // parents
  if (scene == NULL)
    return;
...
void UTransform::parent(UTransform newFather) {
  CTransform *object = getObjectPtr();
  if (object->getForceClipRoot()) {
    nlwarning("Transform has been flagged to be glued to the root, can't "
              "change parent. See UTransform::setForceClipRoot(bool).");
    return;
  }
  if (!newFather.empty()) {
    // link me to other.
...
// ***************************************************************************
bool UTransform::fastIntersect(const NLMISC::CVector &p0,

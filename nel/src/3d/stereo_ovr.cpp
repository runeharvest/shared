...
public:
  ...
  virtual void LogMessageVarg(OVR::LogMessageType messageType, const char *fmt,
                              va_list argList) {
    if (NLMISC::INelContext::isContextInitialised()) {
      ...
      if (IsDebugMessage(messageType))
        NLMISC::INelContext::getInstance().getDebugLog()->displayNL("OVR: %s",
                                                                    buffer);
      else
        NLMISC::INelContext::getInstance().getInfoLog()->displayNL("OVR: %s",
                                                                   buffer);
    }
  }
};
...
CStereoOVR::CStereoOVR(const CStereoOVRDeviceHandle *handle)
    : m_Stage(0), m_SubStage(0), m_OrientationCached(false), m_Driver(NULL),
      m_SceneTexture(NULL), m_GUITexture(NULL), m_PixelProgram(NULL),
  ...
  if (m_DevicePtr->HMDDevice) {
    ...
    nldebug("OVR: HScreenSize: %f, VScreenSize: %f",
            m_DevicePtr->HMDInfo.HScreenSize, m_DevicePtr->HMDInfo.VScreenSize);
    nldebug("OVR: VScreenCenter: %f", m_DevicePtr->HMDInfo.VScreenCenter);
    nldebug("OVR: EyeToScreenDistance: %f",
            m_DevicePtr->HMDInfo.EyeToScreenDistance);
    nldebug("OVR: LensSeparationDistance: %f",
            m_DevicePtr->HMDInfo.LensSeparationDistance);
    nldebug("OVR: InterpupillaryDistance: %f",
            m_DevicePtr->HMDInfo.InterpupillaryDistance);
    nldebug("OVR: HResolution: %i, VResolution: %i",
            m_DevicePtr->HMDInfo.HResolution, m_DevicePtr->HMDInfo.VResolution);
    nldebug("OVR: DistortionK[0]: %f, DistortionK[1]: %f",
            m_DevicePtr->HMDInfo.DistortionK[0],
            m_DevicePtr->HMDInfo.DistortionK[1]);
    nldebug("OVR: DistortionK[2]: %f, DistortionK[3]: %f",
            m_DevicePtr->HMDInfo.DistortionK[2],
            m_DevicePtr->HMDInfo.DistortionK[3]);
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 160
    // NL3D::CStereoOVR::CStereoOVR : OVR: HScreenSize: 0.149760, VScreenSize:
    // 0.093600 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp
    // 161 NL3D::CStereoOVR::CStereoOVR : OVR: VScreenCenter: 0.046800
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 162
    // NL3D::CStereoOVR::CStereoOVR : OVR: EyeToScreenDistance: 0.041000
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 163
    // NL3D::CStereoOVR::CStereoOVR : OVR: LensSeparationDistance: 0.063500
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 164
    // NL3D::CStereoOVR::CStereoOVR : OVR: InterpupillaryDistance: 0.064000
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 165
    // NL3D::CStereoOVR::CStereoOVR : OVR: HResolution: 1280, VResolution: 800
    // 2013/06/26 05:31:51 DBG 17a0 snowballs_client.exe stereo_ovr.cpp 166
    // NL3D::CStereoOVR::CStereoOVR : OVR: DistortionK[0]: 1.000000,
    // DistortionK[1]: 0.220000 2013/06/26 05:31:51 DBG 17a0
    // snowballs_client.exe stereo_ovr.cpp 167 NL3D::CStereoOVR::CStereoOVR :
    // OVR: DistortionK[2]: 0.240000, DistortionK[3]: 0.000000
...
void CStereoOVR::setDriver(NL3D::UDriver *driver) {
  ...
  NL3D::IDriver *drvInternal =
      (static_cast<CDriverUser *>(driver))->getDriver();

  if (drvInternal->supportBloomEffect() &&
      drvInternal->supportNonPowerOfTwoTextures()) {
  ...
  if (m_PixelProgram) {
    ...
    m_BarrelTex->resize(m_DevicePtr->HMDInfo.HResolution,
    m_DevicePtr->HMDInfo.VResolution);
...
void CStereoOVR::initCamera(uint cid, const NL3D::UCamera *camera) {
  ...
  float ar = (float)m_DevicePtr->HMDInfo.HResolution /
             ((float)m_DevicePtr->HMDInfo.VResolution * 2.0f);
  float fov =
      2.0f *
      atanf(
          (m_DevicePtr->HMDInfo.HScreenSize * 0.5f * 0.5f) /
          (m_DevicePtr->HMDInfo
               .EyeToScreenDistance)); //(float)NLMISC::Pi/2.f; // 2.0f *
                                       //atanf(m_DevicePtr->HMDInfo.VScreenSize
                                       /// 2.0f *
                                       //m_DevicePtr->HMDInfo.EyeToScreenDistance);
  m_LeftFrustum[cid].initPerspective(fov, ar, camera->getFrustum().Near,
  ...
  float eyeProjectionShift =
      viewCenter -
      m_DevicePtr->HMDInfo.LensSeparationDistance *
          0.5f; // docs say LensSeparationDistance, why not
                // InterpupillaryDistance? related to how the lenses work?
  float projectionCenterOffset =
      (eyeProjectionShift / (m_DevicePtr->HMDInfo.HScreenSize * 0.5f)) *
      (m_LeftFrustum[cid].Right -
       m_LeftFrustum[cid].Left); // used logic for this one, but it ends up
                                 // being the same as the one i made up
  ...
  m_ClippingFrustum[cid].Left =
      min(m_LeftFrustum[cid].Left, m_RightFrustum[cid].Left);
  m_ClippingFrustum[cid].Right =
      max(m_LeftFrustum[cid].Right, m_RightFrustum[cid].Right);
}
...
void CStereoOVR::getCurrentMatrix(uint cid, NL3D::UCamera *camera) const {
  ...
  if (m_Stage == 2) {
    ...
    translate.translate(
        CVector((m_DevicePtr->HMDInfo.InterpupillaryDistance * m_Scale) * -0.5f,
                0.f, 0.f));
  else
    translate.translate(
        CVector((m_DevicePtr->HMDInfo.InterpupillaryDistance * m_Scale) * 0.5f,
...
bool CStereoOVR::beginRenderTarget() {
  ...
  if (m_Driver && m_Stage == 3 &&
    ...
    uint32 width, height;
    m_Driver->getWindowSize(width,
                            height); // Temporary limitation, TODO: scaling!
    m_SceneTexture =
        m_Driver->getRenderTargetManager().getRenderTarget(width, height);
...
bool CStereoOVR::endRenderTarget() {
       ...
       UDriver::Filled)) // set to 4 to turn off distortion of 2d gui
  {
    ...
    m_Driver->getWindowSize(width, height);
    NL3D::IDriver *drvInternal =
        (static_cast<CDriverUser *>(m_Driver))->getDriver();
    NL3D::CMaterial *barrelMat = m_BarrelMat.getObjectPtr();
    ...
    float lensViewportShift =
        4.0f * lensShift / m_DevicePtr->HMDInfo.HScreenSize;
    ...
    drvInternal->setUniform2f(IDriver::PixelProgram,
                              m_PixelProgram->ovrIndices().Scale, scaleX,
                              scaleY);

    drvInternal->setUniform2f(IDriver::PixelProgram,
                              m_PixelProgram->ovrIndices().ScaleIn, scaleInX,
                              scaleInY);

    drvInternal->setUniform4fv(IDriver::PixelProgram,
                               m_PixelProgram->ovrIndices().HmdWarpParam, 1,
                               m_DevicePtr->HMDInfo.DistortionK);
...
NLMISC::CQuat CStereoOVR::getOrientation() const {
  ...
  matr.rotateX(NLMISC::Pi *
               0.5f); // fix this properly... :) (note: removing this allows you
                      // to use rift while lying down)
...
void CStereoOVR::getInterface2DShift(uint cid, float &x, float &y,
  ...
  rot = rot * modrot;
  float p =
      NLMISC::Pi + atan2f(2.0f * ((rot.x * rot.y) + (rot.z * rot.w)),
                          1.0f - 2.0f * ((rot.y * rot.y) + (rot.w * rot.w)));
  if (p > NLMISC::Pi)
    p -= NLMISC::Pi * 2.0f;
  float t =
      -atan2f(2.0f * ((rot.x * rot.w) + (rot.y * rot.z)),
              1.0f - 2.0f * ((rot.z * rot.z) +
                             (rot.w * rot.w))); // // asinf(2.0f * ((rot.x *
                                                // rot.z) - (rot.w * rot.y)));

  CVector rotshift = CVector(p, 0.f, t) * -distance;

  CVector proj =
      CStereoOVR::getCurrentFrustum(cid).project(vec + ipd + rotshift);
...
void CStereoOVR::listDevices(std::vector<CStereoDeviceInfo> &devicesOut) {
  ...
  OVR::DeviceEnumerator<OVR::HMDDevice> devices =
      s_DeviceManager->EnumerateDevices<OVR::HMDDevice>();
  uint id = 1;
  do {
    ...
    if (devices.IsAvailable()) {
      ...
      deviceInfoOut.Class =
          CStereoDeviceInfo::StereoHMD;               // 1; // OVR::HMDDevice
      ...
      stringstream ser;
      ser << id;
      deviceInfoOut.Serial =
          ser.str(); // can't get the real serial from the sdk...
...
bool CStereoOVR::isDeviceCreated() { return m_DevicePtr->HMDDevice != NULL; }

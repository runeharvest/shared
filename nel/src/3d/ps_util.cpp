...
//==========================================================================
void CPSUtil::displayBBox(NL3D::IDriver *driver, const NLMISC::CAABBox &box,
                          NLMISC::CRGBA col /* = NLMISC::CRGBA::White */) {
  NL_PS_FUNC(CPSUtil_displayBBox)
  CVector max = box.getMax(), min = box.getMin();
  CVertexBuffer vb;
...
//==========================================================================
void CPSUtil::displayArrow(IDriver *driver, const CVector &start,
                           const CVector &v, float size, CRGBA col1,
                           CRGBA col2) {
  ...
  static const TIndexType vTabIndexes[] = {1, 2, 4, 4, 2, 3, 1, 2, 0,
                                           2, 3, 0, 3, 4, 0, 4, 1, 0};
...
//==========================================================================
void CPSUtil::displayBasis(IDriver *driver, const CMatrix &modelMat,
                           const NLMISC::CMatrix &m, float size,
                           CFontGenerator &fg, CFontManager &fm) {
  ...
  displayArrow(driver, m.getPos(), m.getI(), size, CRGBA(127, 127, 127),
               CRGBA(0, 0, 80));

  displayArrow(driver, m.getPos(), m.getJ(), size, CRGBA(127, 127, 127),
               CRGBA(0, 0, 80));

  displayArrow(driver, m.getPos(), m.getK(), size, CRGBA(127, 127, 127),
               CRGBA(200, 0, 80));

  // draw the letters

  CPSUtil::print(driver, std::string("x"), fg, fm,
                 modelMat * m * CVector(1.4f * size, 0, 0), 15.0f * size);

  CPSUtil::print(driver, std::string("y"), fg, fm,
                 modelMat * m * CVector(0, 1.4f * size, 0), 15.0f * size);

  CPSUtil::print(driver, std::string("z"), fg, fm,
                 modelMat * m * CVector(0, 0, 1.4f * size), 15.0f * size);
};

//==========================================================================
void CPSUtil::print(IDriver *driver, const std::string &text,
                    CFontGenerator &fg, CFontManager &fm, const CVector &pos,
                    float size, NLMISC::CRGBA col /*= NLMISC::CRGBA::White*/) {
  NL_PS_FUNC(CPSUtil_print)
  CComputedString cptedString;
  fm.computeString(text, &fg, col, 16, false, false, driver, cptedString);
...
void CPSUtil::buildSchmidtBasis(const CVector &k_, NLMISC::CMatrix &result) {
  ...
  if ((1.0f - fabsf(k * CVector::I)) > epsilon) {
    i = k ^ CVector::I;
  } else if ((1.0f - fabs(k * CVector::J)) > epsilon) {
    i = k ^ CVector::J;
  } else {
    i = k ^ CVector::K;
  }
...
//==========================================================================
void CPSUtil::displaySphere(IDriver &driver, float radius,
                            const CVector &center, uint nbSubdiv, CRGBA color) {
  NL_PS_FUNC(CPSUtil_displaySphere)
  uint x, y, k;
  CVector p, p1, p2;

  static const CVector lK[] = {CVector::I,  -CVector::I, CVector::J,
                               -CVector::J, CVector::K,  -CVector::K};
  ...
  for (k = 0; k < 6; ++k) {
    ...
    for (x = 0; x < nbSubdiv; ++x) {
      for (y = 0; y < nbSubdiv; ++y) {
        p = ((2.f * x / float(nbSubdiv)) - 1.f) * I +
            ((2.f * y / float(nbSubdiv)) - 1.f) * J + K;
...
//==========================================================================
void CPSUtil::displayDisc(IDriver &driver, float radius, const CVector &center,
                          const CMatrix &mat, uint nbSubdiv, CRGBA color) {
  ...
  for (uint k = 0; k < nbSubdiv; ++k) {
    ...
    CDRU::drawLine(center +
                       radius * ((float)cos(theta) * I + (float)sin(theta) * J),
                   center + radius * ((float)cos(theta + thetaDelta) * I +
                                      (float)sin(theta + thetaDelta) * J),
                   color, driver);
    theta += thetaDelta;
  }
}
...
//==========================================================================
void CPSUtil::displayCylinder(IDriver &driver, const CVector &center,
                              const CMatrix &mat, const CVector &dim,
                              uint nbSubdiv, CRGBA color) {
  ...
  for (uint k = 0; k < nbSubdiv; ++k) {
    ...
    CDRU::drawLine(center + dim.z * K + dim.x * cosf(theta) * I +
                       dim.y * sinf(theta) * J,
                   center + dim.z * K + dim.x * cosf(theta + thetaDelta) * I +
                       dim.y * sinf(theta + thetaDelta) * J,
                   color, driver);
...
//==========================================================================
void CPSUtil::display3DQuad(IDriver &driver, const CVector &c1,
                            const CVector &c2, const CVector &c3,
                            const CVector &c4, CRGBA color) {

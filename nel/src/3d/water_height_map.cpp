...
CWaterHeightMap::CWaterHeightMap()
    : Date(-1), _WavesEnabled(false), _Damping(0.97f), _FilterWeight(4),
      _UnitSize(0.6f), _WaveIntensity(0), _WavePeriod(0),
      _WaveImpulsionRadius(3), _BorderWaves(true), _EmitEllapsedTime(0),
      _PropagateEllapsedTime(0), _PropagationTime(0.10f), _X(0), _Y(0),
      _NewX(0), _NewY(0), _CurrMap(0), _Size(0) {}
...
void CWaterHeightMap::updateUserPos() {
  ...
  nlassert(_Size != 0);
  if ((uint)x == _X && (uint)y == _Y)
    return;
  if ((uint)std::abs((sint)(x - _X)) < _Size &&
      (uint)std::abs((sint)(y - _Y)) <
          _Size) // are there common pixels with the previous location?
  {
    // compute zone

    sint XDivSize;
    if ((sint)_X >= 0)
      XDivSize = (sint)_X / (sint)_Size;
    else
      XDivSize = ((sint)(_X + 1) / (sint)_Size) - 1;

    sint YDivSize;
    if ((sint)_Y >= 0)
      YDivSize = (sint)_Y / (sint)_Size;
    else
      YDivSize = ((sint)(_Y + 1) / (sint)_Size) - 1;

    sint xDivSize;
    if (x >= 0)
      xDivSize = (sint)x / (sint)_Size;
    else
      xDivSize = ((sint)(x + 1) / (sint)_Size) - 1;

    sint yDivSize;
    if (y >= 0)
      yDivSize = (sint)y / (sint)_Size;
    else
      yDivSize = ((sint)(y + 1) / (sint)_Size) - 1;
    ...
    if (xDivSize != XDivSize || yDivSize != YDivSize) {
      ...
      for (uint k = 0; k < NumWaterMap; ++k) {
        makeCpy(k, (uint)(left - orgX + offsetX), (uint)(top - orgY + offsetY),
                (uint)(left - orgX), (uint)(top - orgY), (uint)(right - left),
                (uint)(bottom - top));
      }
    }
    ...
    if (x == (sint)_X) {
      ...
      if (x > (sint)_X) {
        if (y == (sint)_Y) {
          ...
          clearZone(_X + _Size - newOrgX, _Y - newOrgY, x - _X,
                    _Size - (_Y - y));
          clearZone(x - newOrgX, y - newOrgY, _Size, _Y - y);
        } else {
          clearZone(_X + _Size - newOrgX, y - newOrgY, x - _X,
                    _Size - (y - _Y));
...
void CWaterHeightMap::animatePart(float startTime, float endTime) {
  if (endTime < 0.5f * _PropagationTime) {
    // perform propagation
    propagate((uint)(_Size * 2.f * startTime / _PropagationTime),
              (uint)(_Size * 2.f * endTime / _PropagationTime));
  } else {
    ...
    if (startTime < 0.5f * _PropagationTime) {
      ...
      filter((uint)(_Size * 2.f * (startTime / _PropagationTime - 0.5f)),
             (uint)(_Size * 2.f * (endTime / _PropagationTime - 0.5f)));
    }
  }
}
...
void CWaterHeightMap::makeCpy(uint buffer, uint dX, uint dY, uint sX, uint sY,
...
void CWaterHeightMap::propagate(uint start, uint end) {
  ...
  // nlinfo("%d, %d, %d",  (_CurrMap + (NumWaterMap - 1)) % NumWaterMap,
  // _CurrMap,
  ...
  do {
    ...
    do {
      *dest =
          damping *
          (0.5f * (buf1[1] + buf1[-1] + buf1[sizeX2] + buf1[-sizeX2]) - *buf2);
      ++buf1;
      ++buf2;
      ++dest;
    } while (--x);
...
void CWaterHeightMap::filter(uint start, uint end) {
  ...
  do {
    ...
    do {
      *buf = totalBlurCoeff *
             (*buf * blurCoeff + buf[1] + buf[-1] + buf[sizeX2] + buf[-sizeX2]);
      // compute gradient
    ...
    if (!_BorderWaves) {
      if (_WaveIntensity != 0) {
        for (k = 0; k < numWaves; ++k) {
          perturbate(_NewX + rand() % _Size, _NewY + rand() % _Size,
                     _WaveImpulsionRadius, _WaveIntensity);
        }
      }
    } else {
      ...
      case 0: // top border
        for (k = 0; k < numWaves; ++k) {
          perturbate(_NewX + (uint)rand() % _Size, _NewY, _WaveImpulsionRadius,
                     _WaveIntensity);
        }
        break;
      case 1: // bottom border
        for (k = 0; k < numWaves; ++k) {
          perturbate(_NewX + (uint)rand() % _Size, _NewY + _Size - 1,
                     _WaveImpulsionRadius, _WaveIntensity);
        }
        break;
      case 2: // right border
        for (k = 0; k < numWaves; ++k) {
          perturbate(_NewX + _Size - 1, _NewY + (uint)rand() % _Size,
                     _WaveImpulsionRadius, _WaveIntensity);
        }
        break;
      case 3: // left border
        for (k = 0; k < numWaves; ++k) {
          perturbate(_NewX, _NewY + (uint)rand() % _Size, _WaveImpulsionRadius,
                     _WaveIntensity);
        }
        break;
      }
...
void CWaterHeightMap::clearArea(uint8 currMap, sint x, sint y, sint width,
                                sint height) {
  nlassert(_Size > 1);
  nlassert(width >= 0);
  nlassert(height >= 0);
...
void CWaterHeightMap::perturbate(sint x, sint y, sint radius, float intensity) {
  ...
  for (sint px = -radius + 1; px < radius; ++px) {
    for (sint py = -radius + 1; py < radius; ++py) {
      if ((uint)(x + px - orgX) < sizeX2 && (uint)(y + py - orgY) < sizeX2) {

        float dist =
            ((float)radius - sqrtf((float)(px * px + py * py))) / (float)radius;
        float v = dist < radius
                      ? intensity * cosf(dist * (float)NLMISC::Pi * 0.5f)
                      : 0.f;
...
void CWaterHeightMap::perturbate(const NLMISC::CVector2f &pos, float strenght,
                                 float radius) {
  const float invUnitSize = 1.f / _UnitSize;
  perturbate((sint)(pos.x * invUnitSize), (sint)(pos.y * invUnitSize),
             (sint)radius, strenght);
}
...
void CWaterHeightMap::perturbatePoint(const NLMISC::CVector2f &pos,
                                      float strenght) {
  const float invUnitSize = 1.f / _UnitSize;
  perturbatePoint((sint)(pos.x * invUnitSize), (sint)(pos.y * invUnitSize),
                  strenght);
}
...
void CWaterHeightMap::setWaves(float intensity, float period, uint radius,
                               bool border) {
...
float CWaterHeightMap::getHeight(const NLMISC::CVector2f &pos) {
  ...
  if ((uint)xPos - _X < _Size - 1 && (uint)yPos - _Y < _Size - 1)

  {
    ...
    return BilinFilter(
        lambda * map2[offset] + (1.f - lambda) * map1[offset], // top left
        lambda * map2[offset + 1] +
            (1.f - lambda) * map1[offset + 1], // top right
        lambda * map2[offset + sizeX2 + 1] +
            (1.f - lambda) * map1[offset + sizeX2 + 1], // bottom right
        lambda * map2[offset + sizeX2] +
            (1.f - lambda) * map1[offset + sizeX2], // bottom left
        deltaU, deltaV);
  } else
    return 0;
}

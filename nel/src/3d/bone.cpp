...
// ***************************************************************************
CBoneBase::CBoneBase()
    : DefaultPos(CVector(0, 0, 0)), DefaultRotEuler(CVector(0, 0, 0)),
      DefaultScale(UnitScale), DefaultPivot(CVector(0, 0, 0)),
      SkinScale(UnitScale) {
  FatherId = -1;
  UnheritScale = true;
...
ITrack *CBone::getDefaultTrack(uint valueId) {
  ...
  switch (valueId) {
  case PosValue:
    return &_BoneBase->DefaultPos;
  case RotEulerValue:
    return &_BoneBase->DefaultRotEuler;
  case RotQuatValue:
    return &_BoneBase->DefaultRotQuat;
  case ScaleValue:
    return &_BoneBase->DefaultScale;
  case PivotValue:
    return &_BoneBase->DefaultPivot;
  }
...
// ***************************************************************************
void CBone::registerToChannelMixer(CChannelMixer *chanMixer,
...
void CBone::compute(CBone *parent, const CMatrix &rootMatrix,
  ...
  // Else, son case, take world matrix from parent.
    ...
    if (_BoneBase->UnheritScale) {
      ...
      */
      ...
      // Faster compute of the translation part: just "trans + fatherScale MUL
      // -trans" where MUL is comp mul
      ...
      // Do: _LocalSkeletonMatrix= parent->_LocalSkeletonMatrix * invScaleComp *
      // localMatrix
      static CMatrix tmp;
    ...
    // Normal case.
      ...
      _LocalSkeletonMatrix.setMulMatrixNoProj(parent->_LocalSkeletonMatrix,
                                              localMatrix);
    }
  }
  ...
  if (_SkinScale == UnitScale) {
    ...
    _BoneSkinMatrix.setMulMatrixNoProj(_LocalSkeletonMatrix,
                                       _BoneBase->InvBindPos);
  } else {
    // Do: _BoneSkinMatrix= _LocalSkeletonMatrix * SkinScale *
    // _BoneBase->InvBindPos
    CMatrix tmp;
    CMatrix scaleMat;
...
// ***************************************************************************
void CBone::interpolateBoneSkinMatrix(const CMatrix &otherMatrix,
                                      float interp) {
...
// ***************************************************************************
void CBone::setSkinScale(CVector &skinScale) { _SkinScale = skinScale; }

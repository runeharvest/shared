...
NLMISC::TStringId CCluster::getSoundGroupId() { return _SoundGroupId; }
...
NLMISC::TStringId CCluster::getEnvironmentFxId() { return _EnvironmentFxId; }
...
void CCluster::unlinkFromParent() {
  ...
  if (Father) {
    Father->Children.erase(
        remove(Father->Children.begin(), Father->Children.end(), this),
        Father->Children.end());
    Father = NULL;
  }
}
...
// ***************************************************************************
bool CCluster::makeVolume(const CVector &p1, const CVector &p2,
                          const CVector &p3) {
  uint i;
  ...
  for (i = 0; i < _LocalVolume.size(); ++i) {
    ...
    if ((f1 < CLUSTERPRECISION) && (f2 < CLUSTERPRECISION) &&
        (f3 < CLUSTERPRECISION))
      return true;
  }
...
// ***************************************************************************
void CCluster::resetPortalLinks() { _Portals.clear(); }

// ***************************************************************************
void CCluster::link(CPortal *portal) { _Portals.push_back(portal); }
...
void CCluster::serial(NLMISC::IStream &f) {
  ...
  if (version >= 2) {
    ...
    //		nldebug("Cluster %s, sound group [%s]", Name.c_str(),
    //CStringMapper::unmap(_SoundGroupId).c_str());
  }
...
// ***************************************************************************
bool CCluster::clip() { return true; }
...
void CCluster::recursTraverseClip(CTransform *caller) {
  ...
  for (i = 0; i < getNbPortals(); ++i) {
    ...
    if (!backfaceclipped && pOtherSideCluster) {
      /* If the otherSide cluster is fully visible because the camera is IN,
         then don't need to clip. This is important to landscape test, to ensure
         that pyramid are strictly equal from 2 paths which come from the 2
         clusters where the camera start
      */
      if (pOtherSideCluster->isCameraIn() ||
          pPortal->clipPyramid(clipTrav.CamPos, clipTrav.WorldPyramid)) {
...
// ***************************************************************************
void CCluster::cameraRayClip(const CVector &start, const CVector &end,
                             std::vector<CCluster *> &clusterVisited) {
  uint i;
  if (_Visited)
    return;
  _Visited = true;

  // The cluster is visible because we are in it. add it to the list of cluster
  ...
  /* Link up in hierarchy. Test the Inverse Flag, cause the path is inverted
     here!!! ie: if I allow the camera to go out, it MUST can re-enter (ie if I
     am VisibleFromFather)
  */

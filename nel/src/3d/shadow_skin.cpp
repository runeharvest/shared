...
// ***************************************************************************
bool CShadowSkin::getRayIntersection(
    const CMatrix &toRaySpace, CSkeletonModel &skeleton,
    const std::vector<uint32> &matrixInfluences, float &dist2D, float &distZ,
    bool computeDist2D) {
  ...
  computeBoneMatrixes3x4PreMul(boneMat3x4, toRaySpace, matrixInfluences,
                               &skeleton);
  ...
  return CRayMesh::getRayIntersection(skinInRaySpace, Triangles, dist2D, distZ,
                                      computeDist2D);
}

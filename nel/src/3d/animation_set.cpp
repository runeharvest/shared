...
uint CAnimationSet::addAnimation(const char *name, CAnimation *animation) {
  // error to add an animation after a build() if the animation set is in
  // HeaderCompress mode
  ...
  _AnimationIdByName.insert(std::map<std::string, uint32>::value_type(
      name, (uint32)_Animation.size() - 1));
...
uint CAnimationSet::addSkeletonWeight(const char *name,
  ...
  // Add an entry name / animation
  _SkeletonWeightIdByName.insert(std::map<std::string, uint32>::value_type(
      name, (uint32)_SkeletonWeight.size() - 1));
...
void CAnimationSet::build() {
  ...
  while (ite != channelNames.end()) {
    // Insert an entry
    _ChannelIdByName.insert(
        std::map<std::string, uint32>::value_type(*ite, id++));
  ...
  // If the animation set is in HeaderOptim mode, reduce memory load by removing
  // map<string, trackId>
  if (_AnimHeaderOptimisation) {
    for (uint a = 0; a < _Animation.size(); a++) {
...
// ***************************************************************************
bool CAnimationSet::loadFromFiles(const std::string &path,
                                  bool recurse /* = true*/,
                                  const char *ext /*= "anim"*/,
  ...
  for (uint k = 0; k < anims.size(); ++k) {
    ...
    if (fileExt == ext) // an animation file ?
    {
      try {
        ...
        anim->serial(iFile);
        addAnimation(
            NLMISC::CFile::getFilenameWithoutExtension(anims[k]).c_str(),
            anim.release());
        iFile.close();
...
// ***************************************************************************
uint CAnimationSet::getAnimationSampleDivisor() const { return _SampleDivisor; }

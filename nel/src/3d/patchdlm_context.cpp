...
// ***************************************************************************
void CPatchDLMPointLight::compile(const CPointLight &pl,
                                  NLMISC::CRGBA landDiffMat, float maxAttEnd) {
  ...
  if (!IsSpot || CosMin < 0) {
    ...
    // Test 2 centers: Center of radius along Dir: Pos+Dir*AttMax/2, and
    // intersection of end cone with line (Pos,Dir) Don't know why but I think
    // they are sufficiently good :) See below for computing of those centers.

    /* compute radius of each sphere by taking max of 3 distances: distance to
       spotLight center, distance to spotLight forward extremity, and distance
       to spotLight circle interstion Cone/Sphere. (named DCCS) NB: Do the
       compute with radius=1 at first, then multiply later.
    */
    float radius1 = 0.5f; // =max(0.5, 0.5); max distance to spot center and
                          // extremity center :)
    ...
    float radius2 =
        max(CosMin,
            1 - CosMin); // max distance to spot center and extremity center :)
...
// ***************************************************************************
bool CPatchDLMContext::generate(CPatch *patch, CTextureDLM *textureDLM,
  ...
  if (!_DLMTexture->createLightMap(Width, Height, TextPosX, TextPosY)) {
    ...
    // NB: the context still work with NULL _DLMTexture, but do nothing
    // (excpetionnal case)
    _DLMTexture = NULL;
  }

  // If the lightmap is correclty allocated in the global texture, compute
  // UVBias.
  if (_DLMTexture) {
  ...
  // Bound 8bits UV for Vegetable. This is to ensure vegetable Dlm UVs won't
  // peek in neighbor lightmaps.
  sint tmpU, tmpV;
  // Bound U minimum
  ...
  for (y = 0; y < Height; y++, t += dt) {
    ...
    for (x = 0; x < Width; x++, s += ds) {
      ...
      // NB: use the bezier patch, and don't take Noise into account, for speed
      // reason.
  ...
  // If the next level has 1x1 cases, it is not useful (since same sphere as
  // entire Patch)
  while (nextBsx * nextBsy > 1 && nextLevel < NL_DLM_CLIP_NUM_LEVEL) {
    ...
    // NB: level 0 has 0 sons to skip, hence level1 must skip (1+0)*4= 4
    // (wSon==hSon==2) level2 must skip (1+4)*4= 20    (wSon==hSon==2)
    tmpClusterNumToSkip[nextLevel] =
        (1 + tmpClusterNumToSkip[curLevel]) * wSon * hSon;
      ...
      for (x = 0; x < nextBsx; x++) {
        ...
        if (wSon > 1 && hSon > 1) {
          ...
          bbox1 = CAABBox::computeAABBoxUnion(
              tmpBBoxes[curLevel][y2 * bsx + x2],
              tmpBBoxes[curLevel][y2 * bsx + x2 + 1]);
          bbox2 = CAABBox::computeAABBoxUnion(
              tmpBBoxes[curLevel][(y2 + 1) * bsx + x2],
              tmpBBoxes[curLevel][(y2 + 1) * bsx + x2 + 1]);
          // final father bbox.
          tmpBBoxes[nextLevel][y * nextBsx + x] =
              CAABBox::computeAABBoxUnion(bbox1, bbox2);
        } else if (wSon == 1) {
          CAABBox bbox1;
          bbox1 = CAABBox::computeAABBoxUnion(
              tmpBBoxes[curLevel][y2 * bsx + x2],
  ...
  // NB: the principle is recursive, but it is "iterated", with a stack-like:
  // tmpClusterX and tmpClusterY;
  ...
  // we start at curLevel (the highest Level), and we must fill all the squares
  // of this level
  ...
  while (curLevel < numLevels) {
    ...
    CAABBox bbox =
        tmpBBoxes[curLevel][tmpClusterY[curLevel] * tmpClusterWidth[curLevel] +
    ...
    // If not Leaf level, recurs. First pass, use curLevel params
    // (tmpClusterX...)
    if (curLevel > 0) {
      // compute info for next level.
      tmpClusterXMin[curLevel - 1] =
          tmpClusterX[curLevel] * tmpClusterWSon[curLevel];
      tmpClusterYMin[curLevel - 1] =
          tmpClusterY[curLevel] * tmpClusterHSon[curLevel];
      tmpClusterXMax[curLevel - 1] =
          (tmpClusterX[curLevel] + 1) * tmpClusterWSon[curLevel];
      tmpClusterYMax[curLevel - 1] =
          (tmpClusterY[curLevel] + 1) * tmpClusterHSon[curLevel];
    ...
    // If not Leaf level, recurs. Second pass, after tmpClusterX and tmpClusterY
    // of curLevel are changed
    if (curLevel > 0) {
  ...
  //============
  // NB: simpler to compute it at generate() time, even if not necessarly needed
  // for near
  computeTextureFar();
...
inline void	__stdcall fastClamp01(float &x)
{
    __asm
    {
        ...
        sbb ecx, ecx			// if attDist is negative, ecx==0 , else
0xFFFFFFFF. and eax, ecx			// if attDist is negative,
eax=0, else unchanged

        // clamp eax to 1 (NB: now we are sure eax>=0).
        cmp	eax, NL3D_Val1		// set carry if < Val1.
        sbb	ecx, ecx			// if < Val1, ecx==0xFFFFFFFF,
else 0. and	eax, ecx			// if < Val1, ecx= eax, else
ecx=0 not	ecx and	ecx, NL3D_Val1		// if > Val1, ecx== Val1, else
ecx= 0. add	eax, ecx			// finally, eax= val clamped
to 1.
...
inline void fastClamp01(float &x) { clamp(x, 0.f, 1.f); }
...
void CPatchDLMContext::addPointLightInfluence(const CPatchDLMPointLight &pl) {
  ...
  for (i = 0; i < _Clusters.size();) {
    ...
    if (_Clusters[i].BSphere.intersect(pl.BSphere)) {
      ...
      // if this cluster is a leaf, just go to next cluster (a parent or a
      // brother)
      if (_Clusters[i].NSkips == 0)
        i++;
  ...
  // precache loading, for better cache use. NB: precache the entire line,
  // ignoring clip result. Precache only if interesting. if( (endX -
  // startX)*4>=Width && (endY-startY)>=2)
  //{
  ...
  if (pl.IsSpot) {
    for (y = startY; y < endY; y++) {
      ...
      for (; nverts > 0; nverts--, vert++, dst++) {
        ...
        CRGBA col;
#ifdef NL_OS_MAC
        // OptFastFloor24 should compiles but it generates an internal compiler
        // error
        col.R = (uint8)floor(r);
        col.G = (uint8)floor(g);
        col.B = (uint8)floor(b);
#else
        // we need to do the 0xff mask or run time type check can break here
        // because sometime r g b are > 255
...
void CPatchDLMContext::compileLighting(TCompileType compType,
                                       CRGBA modulateCte) {
  // If srcTexture is full black, and if dst texture is already full black too,
  // don't need to update dst texture
  if (!(_IsSrcTextureFullBlack && _IsDstTextureFullBlack)) {
    ...
    if (!_LightMap.empty() && _DLMTexture) {
      ...
      // else the srcTexture is not full black (ie some pointLight influence
      // touch it),
      else {
...
// nlassert(_Patch->TileColors.size()>=0);
          ...
          _DLMTexture->modulateAndfillRect565(TextPosX, TextPosY, Width, Height,
                                              &_LightMap[0], tileColor);
        }
        // else if must modulate with textureFar
        else if (compType == ModulateTextureFar) {
          // modulate and fill dest.
          _DLMTexture->modulateAndfillRect8888(TextPosX, TextPosY, Width,
                                               Height, &_LightMap[0],
                                               &_TextureFar[0]);
        }
        // else if must modulate with constante
          ...
          // modulate and fill dest.
          _DLMTexture->modulateConstantAndfillRect(
              TextPosX, TextPosY, Width, Height, &_LightMap[0], modulateCte);
        }
        // else, no Modulate.
        else {
          // just copy lightmap to texture
          _DLMTexture->copyRect(TextPosX, TextPosY, Width, Height,
                                &_LightMap[0]);
        }
      }
    }
...
void CPatchDLMContext::computeTextureFar() {
  ...
  // resize tmp texture. keep a border of 1 pixel around this texture (for
  // average with border)
  uint tfWidth = os * 2 + 2;
        ...
        if (tileId != NL_TILE_ELM_LAYER_EMPTY) {
          ...
          if (pTile && pTile->isFill(CTileFarBank::diffuse)) {
            ...
            const CRGBA *srcPixel =
                pTile->getPixels(CTileFarBank::diffuse, CTileFarBank::order1);
            // compute src info, for this tile rot and 256x256 context.
            sint srcDeltaX = 0;
            sint srcDeltaY = 0;
            srcPixel = computeTileFarSrcDeltas(nRot, is256x256, uvOff, srcPixel,
                                               srcDeltaX, srcDeltaY);
            ...
            if (l == 0) {
              ...
              copyTileToTexture(srcPixel, srcDeltaX, srcDeltaY, dstPixel,
                                tfWidth);
            } else {
              // blend the tile content to the texture.
              blendTileToTexture(srcPixel, srcDeltaX, srcDeltaY, dstPixel,
                                 tfWidth);
            }
          } else
            // go to next tile.
  ...
  */
  ...
  for (y = 0; y < Height; y++) {
    for (x = 0; x < Width; x++, dst++) {
      ...
      dst->avg4RGBOnly(src[y2 * tfWidth + x2], src[y2 * tfWidth + x2 + 1],
                       src[(y2 + 1) * tfWidth + x2],
...
// ***************************************************************************
const CRGBA *
CPatchDLMContext::computeTileFarSrcDeltas(sint nRot, bool is256x256,
                                          uint8 uvOff, const CRGBA *srcPixel,
                                          sint &srcDeltaX, sint &srcDeltaY) {
...
// ***************************************************************************
void CPatchDLMContext::copyTileToTexture(const CRGBA *srcPixel, sint srcDeltaX,
                                         sint srcDeltaY, CRGBA *dstPixel,
                                         uint dstStride) {
...
// ***************************************************************************
void CPatchDLMContext::blendTileToTexture(const CRGBA *srcPixel, sint srcDeltaX,
                                          sint srcDeltaY, CRGBA *dstPixel,
                                          uint dstStride) {
  ...
  dst = &dstPixel[1];
  src = srcPixel[srcDeltaX];
  dst->blendFromuiRGBOnly(*dst, src, src.A);

  // second line.
  dst = &dstPixel[0 + dstStride];
  src = srcPixel[srcDeltaY];
  dst->blendFromuiRGBOnly(*dst, src, src.A);

  dst = &dstPixel[1 + dstStride];
  src = srcPixel[srcDeltaY + srcDeltaX];
  dst->blendFromuiRGBOnly(*dst, src, src.A);
}

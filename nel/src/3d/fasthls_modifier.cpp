...
// ***************************************************************************
uint16 CFastHLSModifier::applyHLSMod(uint16 colorIn, uint8 dHue, uint dLum,
                                     uint dSat) {
  ...
  */
  ...
  if (CSystemInfo::hasMMX()) {
    static uint64 mmInterpBufer[4] = {0, 0, 0,
                                      INT64_CONSTANT(0x00FF00FF00FF00FF)};
    ...
    __asm
    {
     ...
     // add dLum/dSat and clamp to 1.
         ...
         // Combine color and Luminance into MMX. interpolate 0->col or
         // col->white according to edx. Load Lum/(1-Lum) into MMX
			movd		mm2, eax
			movq		mm3, mmOne
			punpckldq	mm2, mm2 // mm2= 0000 00AA 0000 00AA
			packssdw	mm2, mm2 // mm2= 00AA 00AA 00AA 00AA
...
#pragma managed(push, off)
#endif
void CFastHLSModifier::convertDDSBitmapDXTC1Or1A(CBitmap &dst,
                                                 const CBitmap &src, uint8 dh,
                                                 uint dLum, uint dSat) {
  uint W = src.getWidth();
...
#pragma managed(push, off)
#endif
void CFastHLSModifier::convertDDSBitmapDXTC3Or5(CBitmap &dst,
                                                const CBitmap &src, uint8 dh,
                                                uint dLum, uint dSat) {
  uint W = src.getWidth();
  uint H = src.getHeight();
...
// ***************************************************************************
void CFastHLSModifier::convertDDSBitmap(CBitmap &dst, const CBitmap &src,
                                        uint8 dh, sint dl, sint ds) {
  nlassert(src.getPixelFormat() == dst.getPixelFormat());
  nlassert(src.getWidth() == dst.getWidth() &&
  ...
  if (src.getPixelFormat() == CBitmap::DXTC1 ||
      src.getPixelFormat() == CBitmap::DXTC1Alpha)
    convertDDSBitmapDXTC1Or1A(dst, src, dh, dLum, dSat);
  else if (src.getPixelFormat() == CBitmap::DXTC3 ||
           src.getPixelFormat() == CBitmap::DXTC5)
...
// ***************************************************************************
void CFastHLSModifier::convertRGBABitmap(CBitmap &dst, const CBitmap &src,
                                         uint8 dh, sint dl, sint ds) {
  nlassert(src.getPixelFormat() == dst.getPixelFormat());

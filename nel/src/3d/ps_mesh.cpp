...
CVertexBuffer
    CPSConstraintMesh::_PreRotatedMeshVBWithNormal; // mesh has normals
...
static CMesh *CreateDummyMesh(void) {
  ...
  // index for each face
  uint32 tab[] = {4, 1, 0, 4, 5, 1, 5, 2, 1, 5, 6, 2, 6, 3, 2, 6, 7, 3,
                  7, 0, 3, 7, 4, 0, 7, 5, 4, 7, 6, 5, 2, 0, 1, 2, 3, 0};
...
CTransformShape *CPSMesh::createInstance() {
  ...
  nlassert(scene); // the setScene method of the particle system should have
                   // been called
  ...
  instance->hide(); // the object hasn't the right matrix yet so we hide it.
                    // It'll be shown once it is computed
  return instance;
}
...
void CPSMesh::updatePos() {
  ...
  do {
    ...
    if (_SizeScheme) {
      ptCurrSize = (float *)(_SizeScheme->make(
          _Owner, size - leftToDo, &sizes[0], sizeof(float), toProcess, true));
    } else {
      ptCurrSize = &_ParticleSize;
    }

    if (_Angle2DScheme) {
      ptCurrAngle = (float *)(_Angle2DScheme->make(
          _Owner, size - leftToDo, &angles[0], sizeof(float), toProcess, true));
    } else {
      ptCurrAngle = &_Angle2D;
    }

    if (_PlaneBasisScheme) {
      ptBasis = (CPlaneBasis *)(_PlaneBasisScheme->make(
          _Owner, size - leftToDo, &planeBasis[0], sizeof(CPlaneBasis),
          toProcess, true));
    } else {
      ptBasis = &_PlaneBasis;
    }
    ...
    do {
      ...
      mat.setRot(ptBasis->X * CPSUtil::getCos((sint32)*ptCurrAngle) +
                     ptBasis->Y * CPSUtil::getSin((sint32)*ptCurrAngle),
                 ptBasis->X * CPSUtil::getCos((sint32)*ptCurrAngle + 64) +
                     ptBasis->Y * CPSUtil::getSin((sint32)*ptCurrAngle + 64),
                 ptBasis->X ^ ptBasis->Y);
      ...
      if (CParticleSystem::OwnerModel) {
        ...
        (*instanceIt)
            ->setClusterSystem(CParticleSystem::OwnerModel->getClusterSystem());
      }
...
#ifdef NL_DEBUG
    for (TInstanceCont::iterator it = _Instances.begin();
...
static void CheckForOpaqueAndTransparentFacesInMesh(const CMesh &m,
...
static bool CheckForLightableFacesInMesh(const CMesh &m) {
  ...
  for (uint k = 0; k < m.getNbRdrPass(0); ++k) {
    const CMaterial &currMat = m.getMaterial(m.getRdrPassMaterial(0, k));
    if (currMat.isLighted())
      return true;
  }
  return false;
}

/** Well, we could have put a method template in CPSConstraintMesh, but some
 * compilers want the definition of the methods in the header, and some
 * compilers don't want friend with function template, so we use a static method
 * template of a friend class instead, which gives us the same result :)
 */
class CPSConstraintMeshHelper {
public:
  template <class T>
  static void drawMeshs(T posIt, CPSConstraintMesh &m, uint size,
                        uint32 srcStep, bool opaque) {
    ...
    /// get a vb in which to write. It has the same format than the input mesh,
    /// but can also have a color flag added
    CPSConstraintMesh::CMeshDisplay &md = m._MeshDisplayShare.getMeshDisplay(
        m._Meshes[0], modelVb,
        modelVb.getVertexFormat() |
            (m._ColorScheme ? CVertexBuffer::PrimaryColorFlag : 0));

    m.setupRenderPasses((float)m._Owner->getOwner()->getSystemDate() -
                            m._GlobalAnimDate,
                        md.RdrPasses, opaque);
    ...
    // we don't have precomputed mesh there ... so each mesh must be
    // transformed, which is the worst case
    ...
    do {
      ...
      outVb.setNumVertices(toProcess * nbVerticesInSource);
      {
        ...
        if (m._SizeScheme) {
          ptCurrSize = (float *)(m._SizeScheme->make(m._Owner, size - leftToDo,
                                                     &sizes[0], sizeof(float),
                                                     toProcess, true, srcStep));
        } else {
          ptCurrSize = &m._ParticleSize;
        }

        if (m._PlaneBasisScheme) {
          ptBasis = (CPlaneBasis *)(m._PlaneBasisScheme->make(
              m._Owner, size - leftToDo, &planeBasis[0], sizeof(CPlaneBasis),
              toProcess, true, srcStep));
        } else {
        ...
        if (m._Meshes.size() == 1) {
          /// unmorphed case
          do {
            const uint8 *inVertex =
                (const uint8 *)vbaRead.getVertexCoordPointer();
            ...
            if (modelVb.getVertexFormat() & CVertexBuffer::NormalFlag) {
              ...
              do {
                ...
                // translate and resize the vertex (relatively to the mesh
                // origin)
                *(CVector *)outVertex = *posIt + sM * *(CVector *)inVertex;
                // copy the normal
                *(CVector *)(outVertex + outNormalOff) =
                    M * *(CVector *)(inVertex + inNormalOff);
          ...
          {
            currMorphValue = (float *)m._MorphScheme->make(
                m._Owner, size - leftToDo, &morphValues[0], sizeof(float),
                toProcess, true, srcStep);
            morphValueIncr = 1;
          } else /// constant case
          {
          ...
          do {
            ...
            if (modelVb.getVertexFormat() & CVertexBuffer::NormalFlag) {
              ...
              do {
                ...
                *(CVector *)outVertex =
                    *posIt +
                    sM * (opLambda * *(CVector *)m0 + lambda * *(CVector *)m1);
                // morph, and transform the normal
                *(CVector *)(outVertex + outNormalOff) =
                    M * (opLambda * *(CVector *)(m0 + inNormalOff) +
                         lambda * *(CVector *)(m1 + inNormalOff))
                            .normed();
  ...
  static void drawPrerotatedMeshs(T posIt, U indexIt, CPSConstraintMesh &m,
                                  uint size, uint32 srcStep, bool opaque) {
    ...
    const uint prerotatedModelSize =
        prerotVb.getVertexSize() * modelVb.getNumVertices();

    /// get a mesh display struct on this shape, with eventually a primary color
    /// added.
    CPSConstraintMesh::CMeshDisplay &md = m._MeshDisplayShare.getMeshDisplay(
        m._Meshes[0], modelVb,
        modelVb.getVertexFormat() |
    ...
    do {
      ...
      outVb.setNumVertices(toProcess * nbVerticesInSource);
      {
        ...
        do {
          uint8 *inVertex = (uint8 *)PrerotVba.getVertexCoordPointer() +
                            prerotatedModelSize * *indexIt; // prerotated vertex
          uint k = nbVerticesInSource;

          if (prerotVb.getVertexFormat() &
              CVertexBuffer::NormalFlag) // has it a normal ?
          {
            do {
              CHECK_VERTEX_BUFFER(outVb, outVertex);
              ...
              *(CVector *)outVertex =
                  *posIt + *ptCurrSize * *(CVector *)inVertex;
              // copy the normal
              *(CVector *)(outVertex + normalOff) =
                  *(CVector *)(inVertex + pNormalOff);
              inVertex += inVSize;
              outVertex += outVSize;
            } while (--k);
          } else {
...
CPSConstraintMesh::CPSConstraintMesh()
    : _NumFaces(0), _ModelBank(NULL), _ModulatedStages(0), _Touched(1),
      _HasOpaqueFaces(0), _VertexColorLightingForced(false),
      _GlobalAnimationEnabled(0), _ReinitGlobalAnimTimeOnNewElement(0),
      _HasLightableFaces(0), _ValidBuild(0), _MorphValue(0),
      _MorphScheme(NULL) {
  NL_PS_FUNC(CPSConstraintMesh_CPSConstraintMesh)
  if (CParticleSystem::getSerializeIdentifierFlag())
...
//====================================================================================
void CPSConstraintMesh::setShapes(const std::string *shapesNames,
                                  uint numShapes) {
...
#ifdef NL_COMP_VC14
  std::copy(_MeshShapeFileName.begin(), _MeshShapeFileName.end(),
            stdext::make_unchecked_array_iterator(shapesNames));
#else
...
bool CPSConstraintMesh::update(std::vector<sint> *numVertsVect /*= NULL*/) {
  ...
  std::fill(_MeshVertexBuffers.begin(), _MeshVertexBuffers.end(),
            (CVertexBuffer *)NULL);
  if (numVertsVect)
    numVertsVect->resize(_MeshShapeFileName.size());
  for (uint k = 0; k < _MeshShapeFileName.size(); ++k) {
    if (_ModelBank->getPresentState(_MeshShapeFileName[k]) ==
        CShapeBank::Present) {
      CMesh *mesh =
          dynamic_cast<CMesh *>(_ModelBank->addRef(_MeshShapeFileName[k]));
      if (!mesh) {
        nlwarning(
            "Tried to bind a shape that is not a mesh to a mesh particle : %s",
            _MeshShapeFileName[k].c_str());
        _ModelBank->release(is);
        ok = false;
        if (numVertsVect)
          (*numVertsVect)[k] = ShapeFileIsNotAMesh;
      } else {
        _Meshes[k] = mesh;
        /// get  the mesh format, or check that is was the same that previous
        /// shapes ' one
        if (k == 0) {
          vFormat = mesh->getVertexBuffer().getVertexFormat();
          numVerts = mesh->getVertexBuffer().getNumVertices();
          std::copy(mesh->getVertexBuffer().getUVRouting(),
                    mesh->getVertexBuffer().getUVRouting() +
                        CVertexBuffer::MaxStage,
                    uvRouting);
          if (numVertsVect)
            (*numVertsVect)[k] = (sint)numVerts;
        } else {
          ...
          if (!std::equal(mesh->getVertexBuffer().getUVRouting(),
                          mesh->getVertexBuffer().getUVRouting() +
                              CVertexBuffer::MaxStage,
                          uvRouting)) {
          ...
          if (numVertsVect)
            (*numVertsVect)[k] = (sint)mesh->getVertexBuffer().getNumVertices();
        }
      }
    } else {
      try {
        ...
        nlwarning("mesh not found : %s; used as a constraint mesh particle",
                  _MeshShapeFileName[k].c_str());
        // shape not found, so not present in the shape bank -> we create a
        // dummy shape
      }
      ...
      if (_ModelBank->getPresentState(_MeshShapeFileName[k]) !=
        ...
        ok = false;
        if (numVertsVect)
          (*numVertsVect)[k] = ShapeFileNotLoaded;
      } else {
        is = _ModelBank->addRef(_MeshShapeFileName[k]);
        if (!dynamic_cast<CMesh *>(is)) // is it a mesh
        {
          nlwarning("Tried to bind a shape that is not a mesh to a mesh "
                    "particle : %s",
                    _MeshShapeFileName[k].c_str());
          _ModelBank->release(is);
          ok = false;
          ...
          if (m.getVertexBuffer().getNumVertices() >
              ConstraintMeshMaxNumVerts) {
            nlwarning("Tried to bind a mesh that has more than %d vertices to "
                      "a particle mesh: %s",
                      (int)ConstraintMeshMaxNumVerts,
                      _MeshShapeFileName[k].c_str());
            _ModelBank->release(is);
            ok = false;
            if (numVertsVect)
              (*numVertsVect)[k] = ShapeHasTooMuchVertices;
          } else {
            _Meshes[k] = &m;
            if (k == 0) {
              vFormat = m.getVertexBuffer().getVertexFormat();
              numVerts = m.getVertexBuffer().getNumVertices();
              std::copy(m.getVertexBuffer().getUVRouting(),
                        m.getVertexBuffer().getUVRouting() +
                            CVertexBuffer::MaxStage,
                        uvRouting);
              if (numVertsVect)
                (*numVertsVect)[k] = numVerts;
            } else {
              ...
              if (otherVFormat != vFormat || otherNumVerts != numVerts ||
                  !(std::equal(m.getVertexBuffer().getUVRouting(),
                               m.getVertexBuffer().getUVRouting() +
                                   CVertexBuffer::MaxStage,
                               uvRouting))) {
                ok = false;
  ...
  CheckForOpaqueAndTransparentFacesInMesh(m, hasTransparentFaces,
                                          hasOpaqueFaces);
...
void CPSConstraintMesh::hintRotateTheSame(uint32 nbConfiguration,
  ...
  if (nbConfiguration) {
    ...
    for (uint k = 0; k < nbConfiguration; ++k) {
      ...
      _PrecompBasis[k].AngularVelocity =
          minAngularVelocity + (rand() % 20000) / 20000.f *
                                   (maxAngularVelocity - minAngularVelocity);
    }
...
void CPSConstraintMesh::fillIndexesInPrecompBasis(void) {
  ...
  for (CPSVector<uint32>::V::iterator it = _IndexInPrecompBasis.begin();
       it != _IndexInPrecompBasis.end(); ++it) {
    *it = rand() % nbConf;
  }
}
...
void CPSConstraintMesh::serial(NLMISC::IStream &f) {
  ...
  if (f.isReading()) {
    ...
    hintRotateTheSame(nbConfigurations, _MinAngularVelocity,
                      _MaxAngularVelocity);
  } else {
  ...
  if (ver > 3) // mesh morphing
  {
    if (!f.isReading()) {
      ...
      std::transform(meshNamesWithoutPath.begin(), meshNamesWithoutPath.end(),
                     meshNamesWithoutPath.begin(),
#ifndef NL_CPP17
                     std::ptr_fun(NLMISC::CFile::getFilename)
#else
                     std::bind(&NLMISC::CFile::getFilename,
                               std::placeholders::_1)
#endif
      );
...
CVertexBuffer &CPSConstraintMesh::makePrerotatedVb(const CVertexBuffer &inVb) {
  ...
  CVertexBuffer &prerotatedVb =
      inVb.getVertexFormat() & CVertexBuffer::NormalFlag
  ...
  // rotate basis
  // and compute the set of prerotated meshs that will then duplicated (with
  // scale and translation) to create the Vb of what must be drawn
  uint8 *outVertex = (uint8 *)vba.getVertexCoordPointer();
  for (CPSVector<CPlaneBasisPair>::V::iterator it = _PrecompBasis.begin();
       it != _PrecompBasis.end(); ++it) {
    // not optimized at all, but this will apply to very few elements anyway...
    CMatrix mat;
    mat.rotate(
        CQuat(it->Axis, CParticleSystem::EllapsedTime * it->AngularVelocity));
    ...
    if (inVb.getVertexFormat() & CVertexBuffer::NormalFlag) {
      ...
      do {
        ...
        *(CVector *)(outVertex + normalOff) =
            mat.mulVector(*(CVector *)(inVertex + pNormalOff));
        outVertex += vpSize;
        inVertex += vSize;
...
void CPSConstraintMesh::draw(bool opaque) {
  ...
  {
    if (step == (1 << 16)) {
      CPSConstraintMeshHelper::drawMeshs(_Owner->getPos().begin(), *this,
                                         numToProcess, step, opaque);
    } else {
      CPSConstraintMeshHelper::drawMeshs(
          TIteratorVectStep1616(_Owner->getPos().begin(), 0, step), *this,
          numToProcess, step, opaque);
    }
  } else {
    if (step == (1 << 16)) {
      CPSConstraintMeshHelper::drawPrerotatedMeshs(
          _Owner->getPos().begin(), _IndexInPrecompBasis.begin(), *this,
          numToProcess, step, opaque);
    } else {
      typedef CAdvance1616Iterator<CPSVector<uint32>::V::const_iterator, uint32>
          TIndexIterator;
      CPSConstraintMeshHelper::drawPrerotatedMeshs(
          TIteratorVectStep1616(_Owner->getPos().begin(), 0, step),
          TIndexIterator(_IndexInPrecompBasis.begin(), 0, step), *this,
          numToProcess, step, opaque);
    }
  }
}
...
//====================================================================================
void CPSConstraintMesh::setupMaterialColor(CMaterial &destMat,
                                           CMaterial &srcMat) {
  NL_PS_FUNC(CPSConstraintMesh_setupMaterialColor)
  if (destMat.getShader() != CMaterial::Normal)
    return;
...
//====================================================================================
void CPSConstraintMesh::setupRenderPasses(float date, TRdrPassSet &rdrPasses,
                                          bool opaque) {
  ...
  for (TRdrPassSet::iterator rdrPassIt = rdrPasses.begin();
    ...
    if ((opaque && Mat.getZWrite()) || (!opaque && !Mat.getZWrite())) {
      ...
      if (_ColorScheme != NULL) {
        forceVertexcolorLighting = _VertexColorLightingForced != 0
                                       ? true
                                       : SourceMat.getLightedVertexColor();
      } else {
        forceVertexcolorLighting = false;
      }
      if (forceVertexcolorLighting !=
          Mat.getLightedVertexColor()) // avoid to touch mat if not needed
      {
...
//====================================================================================
void CPSConstraintMesh::doRenderPasses(IDriver *driver, uint numObj,
                                       TRdrPassSet &rdrPasses, bool opaque) {
  ...
  for (TRdrPassSet::iterator rdrPassIt = rdrPasses.begin();
    ...
    if ((opaque && Mat.getZWrite()) || (!opaque && !Mat.getZWrite())) {
      ...
      rdrPassIt->PbTri.setNumIndexes(
          ((rdrPassIt->PbTri.capacity() / 3) * numObj / ConstraintMeshBufSize) *
          3);
      rdrPassIt->PbLine.setNumIndexes(((rdrPassIt->PbLine.capacity() / 2) *
                                       numObj / ConstraintMeshBufSize) *
                                      2);

      /// render the primitives
      driver->activeIndexBuffer(rdrPassIt->PbTri);
      driver->renderTriangles(rdrPassIt->Mat, 0,
                              rdrPassIt->PbTri.getNumIndexes() / 3);
      if (rdrPassIt->PbLine.getNumIndexes() != 0) {
        driver->activeIndexBuffer(rdrPassIt->PbLine);
        driver->renderLines(rdrPassIt->Mat, 0,
                            rdrPassIt->PbLine.getNumIndexes() / 2);
      }
    }
  }
}
...
//====================================================================================
void CPSConstraintMesh::computeColors(CVertexBuffer &outVB,
                                      const CVertexBuffer &inVB,
                                      uint startIndex, uint toProcess,
                                      uint32 srcStep, IDriver &drv,
                                      CVertexBufferReadWrite &vba,
                                      CVertexBufferRead &vbaIn) {
  NL_PS_FUNC(CPSConstraintMesh_computeColors)
  nlassert(_ColorScheme);
  // there are 2 case : 1 - the source mesh has colors, which are modulated with
  // the current color
  //					  2 - the source mesh has no colors :
  //colors are directly copied into the dest vb

  if (inVB.getVertexFormat() & CVertexBuffer::PrimaryColorFlag) // case 1
  {
    // TODO: optimisation : avoid to duplicate colors...
    _ColorScheme->makeN(_Owner, startIndex, vba.getColorPointer(),
                        outVB.getVertexSize(), toProcess, inVB.getNumVertices(),
                        srcStep);
    ...
    for (uint k = 0; k < toProcess; ++k) {
      NLMISC::CRGBA::modulateColors((CRGBA *)vDest, (CRGBA *)vSrc,
                                    (CRGBA *)vDest, numVerts, vSize, vSize);
      vDest += meshSize;
    }
  } else // case 2
  {
...
void CPSConstraintMesh::newElement(const CPSEmitterInfo &info) {
  ...
  if (_MorphScheme && _MorphScheme->hasMemory())
    _MorphScheme->newElement(info);
}
...
void CPSConstraintMesh::deleteElement(uint32 index) {
  ...
  if (_MorphScheme && _MorphScheme->hasMemory())
    _MorphScheme->deleteElement(index);
}
...
void CPSConstraintMesh::resize(uint32 size) {
  ...
  resizeColor(size);
  if (_MorphScheme && _MorphScheme->hasMemory())
    _MorphScheme->resize(size, _Owner->getSize());
}
...
static void DuplicatePrimitiveBlock(const CIndexBuffer &srcBlock,
                                    CIndexBuffer &destBlock, uint nbReplicate,
                                    uint vertOffset) {
  ...
  if (ibaRead.getFormat() == CIndexBuffer::Indices16) {
    ...
    for (k = 0; k < nbReplicate; ++k) {
      currTriPtr = triPtr;
      for (l = 0; l < numTri; ++l) {
        ibaWrite.setTri(3 * index, currTriPtr[0] + currVertOffset,
                        currTriPtr[1] + currVertOffset,
                        currTriPtr[2] + currVertOffset);
        currTriPtr += 3;
        ++index;
      }
    ...
    for (k = 0; k < nbReplicate; ++k) {
      ...
      for (l = 0; l < numTri; ++l) {
        ...
        //
        ibaWrite.setTri(3 * index, (uint16)(currTriPtr[0] + currVertOffset),
                        (uint16)(currTriPtr[1] + currVertOffset),
                        (uint16)(currTriPtr[2] + currVertOffset));
        currTriPtr += 3;
        ++index;
      }
      currVertOffset += vertOffset;
  ...
  // TODO quad / strips duplication : (unimplemented in primitive blocks for
  // now)
...
void CPSConstraintMesh::initPrerotVB() {
  ...
  _PreRotatedMeshVB.setNumVertices(ConstraintMeshMaxNumPrerotatedModels *
                                   ConstraintMeshMaxNumVerts);
  _PreRotatedMeshVB.setName("CPSConstraintMesh::_PreRotatedMeshVB");

  // position & normals
  _PreRotatedMeshVBWithNormal.setVertexFormat(CVertexBuffer::PositionFlag |
                                              CVertexBuffer::NormalFlag);
  _PreRotatedMeshVBWithNormal.setNumVertices(
      ConstraintMeshMaxNumPrerotatedModels * ConstraintMeshMaxNumVerts);
  _PreRotatedMeshVB.setName("CPSConstraintMesh::_PreRotatedMeshVBWithNormal");
}

//====================================================================================
CPSConstraintMesh::CMeshDisplay &
CPSConstraintMesh::CMeshDisplayShare::getMeshDisplay(
    CMesh *mesh, const CVertexBuffer &meshVB, uint32 format) {
  NL_PS_FUNC(CMeshDisplayShare_getMeshDisplay)
  nlassert(mesh);
  // linear search is ok because of small size
  for (std::list<CMDEntry>::iterator it = _Cache.begin(); it != _Cache.end();
       ++it) {
  ...
  nlinfo("mesh setup time = %.2f", (float) (1000 *
  NLMISC::CTime::ticksToSecond(end - start)));	*/
  return _Cache.front().MD;
}

//====================================================================================
void CPSConstraintMesh::CMeshDisplayShare::buildRdrPassSet(TRdrPassSet &dest,
                                                           const CMesh &m) {
  NL_PS_FUNC(CMeshDisplayShare_buildRdrPassSet)
  // we don't support skinning for mesh particles, so there must be only one
  // matrix block
  ...
  for (uint k = 0; k < m.getNbRdrPass(0); ++k) {
    ...
    DuplicatePrimitiveBlock(m.getRdrPassPrimitiveBlock(0, k), dest[k].PbTri,
                            ConstraintMeshBufSize, srcVb.getNumVertices());
  }
}

//====================================================================================
void CPSConstraintMesh::CMeshDisplayShare::buildVB(CVertexBuffer &dest,
                                                   const CVertexBuffer &meshVb,
                                                   uint32 destFormat) {
  NL_PS_FUNC(CMeshDisplayShare_buildVB)
  /// we duplicate the original mesh data's 'ConstraintMeshBufSize' times,
  /// eventually adding a color
  nlassert(destFormat == meshVb.getVertexFormat() ||
           destFormat == (meshVb.getVertexFormat() |
                          (uint32)CVertexBuffer::PrimaryColorFlag));
  ...
  } else // color added, but not available in src
  {
    ...
    for (uint k = 0; k < ConstraintMeshBufSize; ++k) {
      for (uint v = 0; v < meshVb.getNumVertices(); ++v) {
        // copy until color
        ::memcpy((void *)(outPtr + k * meshSize + v * outVSize),
                 (void *)(inPtr + v * inVSize), colorOff);
        // copy datas after color
        ::memcpy((void *)(outPtr + k * meshSize + v * outVSize + colorOff +
                          sizeof(uint8[4])),
                 (void *)(inPtr + v * inVSize + colorOff), inVSize - colorOff);
      }
    }
  }
}
...
CPSConstraintMesh::CGlobalTexAnim::CGlobalTexAnim()
    : TransOffset(NLMISC::CVector2f::Null), TransSpeed(NLMISC::CVector2f::Null),
      TransAccel(NLMISC::CVector2f::Null), ScaleStart(1, 1),
      ScaleSpeed(NLMISC::CVector2f::Null), ScaleAccel(NLMISC::CVector2f::Null),
      WRotSpeed(0), WRotAccel(0) {
...
//=====================================================================================
void CPSConstraintMesh::CGlobalTexAnim::buildMatrix(float date,
                                                    NLMISC::CMatrix &dest) {
  NL_PS_FUNC(CGlobalTexAnim_buildMatrix)
  float fDate = (float)date;
  float halfDateSquared = 0.5f * fDate * fDate;
  NLMISC::CVector2f pos =
      fDate * TransSpeed + halfDateSquared * fDate * TransAccel + TransOffset;
  NLMISC::CVector2f scale =
      ScaleStart + fDate * ScaleSpeed + halfDateSquared * fDate * ScaleAccel;
...
//=====================================================================================
void CPSConstraintMesh::setGlobalTexAnim(uint stage,
...
//=====================================================================================
const CPSConstraintMesh::CGlobalTexAnim &
CPSConstraintMesh::getGlobalTexAnim(uint stage) const {
...
void CPSConstraintMesh::setTexAnimType(TTexAnimType type) {
  ...
  nlassert(type < Last);
  if (type == getTexAnimType())
    return; // does the type of animation change ?
  switch (type) {
  case NoAnim:
...
void CPSConstraintMesh::restoreMaterials() {
  ...
  update();
  CMeshDisplay &md = _MeshDisplayShare.getMeshDisplay(
      _Meshes[0], getMeshVB(0),
      _Meshes[0]->getVertexBuffer().getVertexFormat() |

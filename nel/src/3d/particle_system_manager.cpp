...
CParticleSystemManager::TManagerList *CParticleSystemManager::ManagerList =
    NULL;
...
///=========================================================
void CParticleSystemManager::refreshModels(
    const std::vector<NLMISC::CPlane> &worldFrustumPyramid,
...
CParticleSystemManager::addSystemModel(CParticleSystemModel *model) {
  NL_PS_FUNC(CParticleSystemManager_addSystemModel)
#ifdef NL_DEBUG
  nlassert(std::find(_ModelList.begin(), _ModelList.end(), model) ==
           _ModelList.end());
#endif
...
CParticleSystemManager::addPermanentlyAnimatedSystem(CParticleSystemModel *ps) {
  NL_PS_FUNC(CParticleSystemManager_addPermanentlyAnimatedSystem)
#ifdef NL_DEBUG
  for (TAlwaysAnimatedModelList::iterator it =
           _PermanentlyAnimatedModelList.begin();
       it != _PermanentlyAnimatedModelList.end(); ++it) {
    nlassert(it->Model != ps);
  }
#endif
  CAlwaysAnimatedPS aaps;
  aaps.Model = ps;
  aaps.HasAncestorSkeleton =
      false; // even if there's an ancestor skeleton yet, force the manager to
             // recompute relative pos of the system when clipped
...
///=========================================================
void CParticleSystemManager::removePermanentlyAnimatedSystem(
    CParticleSystemManager::TAlwaysAnimatedModelHandle &handle) {
...
void CParticleSystemManager::processAnimate(TAnimationTime deltaT) {
  ...
  for (TAlwaysAnimatedModelList::iterator it =
           _PermanentlyAnimatedModelList.begin();
       it != _PermanentlyAnimatedModelList.end();) {
    ...
    if (ps) {
      ...
      // When a fx is sticked as a son of a skeleton model, the skeleton matrix
      // is not updated when the skeleton is not visible (clipped) This is a
      // concern when fx generate trails because when the skeleton becomes
      // visible again, a trail will appear between the previous visible pos and
      // the new visible pos to solve this : When the ancestor skeleton is
      // visible, we backup the relative position to the ancestor skeleton.
      //			- if it is not visible at start, we must
      //evaluate the position of the stick point anyway
      // When the father skeleton is clipped, we use the relative position
      if (psm.getAncestorSkeletonModel()) {
        if (psm.getAncestorSkeletonModel()->getVisibility() !=
            CHrcTrav::Hide) // matrix of ancestor is irrelevant if the system is
                            // hidden (because sticked to hidden parent for
                            // example)
        {
          if (!psm.isClipVisible()) // the system may not be visible because of
                                    // clod
          {
            if (!it->IsRelMatrix) // relative matrix already computed ?
            {
              ...
              it->OldAncestorMatOrRelPos =
                  it->OldAncestorMatOrRelPos.inverted() * psm.getWorldMatrix();
              it->IsRelMatrix = true;
            }
            psm.setWorldMatrix(
                psm.getAncestorSkeletonModel()->getWorldMatrix() *
                it->OldAncestorMatOrRelPos);
          } else {
      ...
      if (!psm.getEditionMode()) {
        // test deletion condition (no more particle, no more particle and
        // emitters)
...
void CParticleSystemManager::stopSound() {
  ...
  for (TModelList::iterator it = _ModelList.begin(); it != _ModelList.end();
       ++it) {
...
void CParticleSystemManager::stopSoundForAllManagers() {
  ...
  for (TManagerList::iterator it = getManagerList().begin();
       it != getManagerList().end(); ++it) {
    nlassert(*it);
    (*it)->stopSound();
  }
}

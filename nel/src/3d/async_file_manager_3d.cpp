...
void CAsyncFileManager3D::terminate() {
  if (_Instance != NULL) {
    ...
    INelContext::getInstance().releaseSingletonPointer("CAsyncFileManager3D",
                                                       _Instance);
    _Instance = NULL;
    delete afm;
  }
...
void CAsyncFileManager3D::loadMesh(const std::string &meshName, IShape **ppShp,
                                   IDriver *pDriver, const CVector &position,
                                   uint textureSlot) {
  if (meshName.empty())
    nlwarning("Can't load an empty filename");
  CAsyncFileManager::getInstance().addLoadTask(
      new CMeshLoad(meshName, ppShp, pDriver, position, textureSlot));
}
...
public:
  CLoadMeshCancel(const std::string &meshName) : _MeshName(meshName) {}
...
private:
  ...
  bool callback(const NLMISC::IRunnable *prunnable) const {
    const CAsyncFileManager3D::CMeshLoad *pML =
        dynamic_cast<const CAsyncFileManager3D::CMeshLoad *>(prunnable);
    if (pML != NULL) {
...
bool CAsyncFileManager3D::cancelLoadMesh(const std::string &sMeshName) {
  return CAsyncFileManager::getInstance().cancelLoadTask(
      CLoadMeshCancel(sMeshName));
}

// ***************************************************************************

void CAsyncFileManager3D::loadIG(const std::string &IGName,
                                 CInstanceGroup **ppIG) {
...
void CAsyncFileManager3D::loadIGUser(const std::string &IGName,
                                     UInstanceGroup **ppIG) {
...
private:
  ...
  bool callback(const NLMISC::IRunnable *prunnable) const {
    const CAsyncFileManager3D::CTextureLoad *pTL =
        dynamic_cast<const CAsyncFileManager3D::CTextureLoad *>(prunnable);
    if (pTL != NULL) {
...
bool CAsyncFileManager3D::cancelLoadTexture(CTextureFile *textFile) {
  return CAsyncFileManager::getInstance().cancelLoadTask(
      CLoadTextureCancel(textFile));
}

// ***************************************************************************

void CAsyncFileManager3D::loadFile(const std::string &sFileName,
                                   uint8 **ppFile) {
...
void CAsyncFileManager3D::loadFiles(const std::vector<std::string> &vFileNames,
...
CAsyncFileManager3D::CMeshLoad::CMeshLoad(const std::string &sMeshName,
                                          IShape **ppShp, IDriver *pDriver,
...
void CAsyncFileManager3D::CMeshLoad::run() {
  ...
  try {
    ...
    // If the shape is a mesh but the driver is not given or if the shape is not
    // a mesh then do not try to load the textures
        ...
        if (rMat.texturePresent(uint8(j))) {
          ...
          if ((pText != NULL) && (pText->supportSharing())) {
            ...
            if (!_pDriver->isTextureExist(*pText)) {
              ...
              TAlreadyPresentTextureSet::iterator aptmIt =
                  AlreadyPresentTextureSet.find(pText->getShareName());
              ...
              if (aptmIt == AlreadyPresentTextureSet.end()) {
                ...
                // And load it (to RAM only (upload in VRAM is done in the shape
                // bank))
                pText->generate(true);
              }
            }
    ...
    // Finally affect the pointer (Trans-Thread operation -> this operation must
    // be atomic)
    *_ppShp = mesh.getShapePointer();
...
// ***************************************************************************
CAsyncFileManager3D::CIGLoad::CIGLoad(const std::string &IGName,
                                      CInstanceGroup **ppIG) {
  _IGName = IGName;
  _ppIG = ppIG;
}
...
// ***************************************************************************
CAsyncFileManager3D::CIGLoadUser::CIGLoadUser(const std::string &IGName,
                                              UInstanceGroup **ppIG) {
  _IGName = IGName;
  _ppIG = ppIG;
}

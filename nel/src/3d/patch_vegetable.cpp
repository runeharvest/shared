...
// ***************************************************************************
void CPatch::generateTileVegetable(
    CVegetableInstanceGroup *vegetIg, uint distType, uint ts, uint tt,
  ...
  CTileElement::TVegetableInfo vegetWaterState =
      Tiles[tt * OrderS + ts].getVegetableState();
  ...
  const CTileVegetableDesc &tileVegetDesc =
      getLandscape()->getTileVegetableDesc(tileId);
  const std::vector<CVegetable> &vegetableList =
      tileVegetDesc.getVegetableList(distType);
  ...
  if (Tiles[tt * OrderS + ts].Tile[1] != NL_TILE_ELM_LAYER_EMPTY) {
    ...
    if (getLandscape()->getTileVegetableDesc(tileId1).empty())
      return;
    if (tileId2 != NL_TILE_ELM_LAYER_EMPTY &&
        getLandscape()->getTileVegetableDesc(tileId2).empty())
      return;
  }
  ...
  // Get approximate position for the tile (useful for noise). NB: eval() is
  // faster than computeVertex().
  ...
  // Compute also position on middle of 4 edges of this tile, for
  ...
  // must normalize the matrix. use the vector which is the most orthogonal to
  // tileNormal If tileNormal is much more a J vector, then use plane
  // (I,tileNormal), and vice-versa
  ...
  // say that ambient never change. VegetableManager handle the ambient and
  // diffuse itself (for precomputeLighting)
  ...
  // take only 2 first, computing direction to tilePos and computing
  // attenuation.
  vegetLex.NumLights =
      min((uint)CVegetableLightEx::MaxNumLight, (uint)lightList.size());
  for (i = 0; i < vegetLex.NumLights; i++) {
    // WARNING: can C cast to CPointLightNamed here because comes from
    // CPatch::appendTileLightInfluences() only!
  ...
  // bound them, ensuring 8Bits UV "uncompressed" by driver are in the lightmap
  // area.
  ...
  // First, for each vegetable, generate the number of instance to create, and
  ...
  for (i = 0; i < numVegetable; i++) {
    ...
    veget.generateGroupBiLinear(tilePos, tilePosBiLinear, tileNormal,
                                NL3D_PATCH_TILE_AREA, i + distAddSeed,
                                instanceUVArray[i]);
  }

  // Then, now that we kno how many instance to generate for each vegetable,
  // reserve space.
  ...
  for (i = 0; i < numVegetable; i++) {
    ...
    veget.reserveIgAddInstances(vegetIgReserve,
                                (CVegetable::TVegetableWater)vegetWaterState,
                                (uint)instanceUVArray[i].size());
  }
  ...
  for (i = 0; i < numVegetable; i++) {
    ...
    for (uint j = 0; j < instanceUV.size(); j++) {
      ...
      // instanceUV is in [0..1] interval, which maps to a tile, so explode to
      // the patch
      CVector instancePos;
      ...
      veget.generateInstance(
          vegetIg, matInstance, ambientF,
          diffuseColorF[(lumelT << NL_LUMEL_BY_TILE_SHIFT) + lumelS],
          (distType + 1) * NL3D_VEGETABLE_BLOCK_ELTDIST,
          (CVegetable::TVegetableWater)vegetWaterState, dlmUV8);
    }
  }
}
...
void CPatch::recreateAllVegetableIgs() {
  ...
  for (uint numtb = 0; numtb < TessBlocks.size(); numtb++) {
    // if the vegetableBlock is deleted, and if there is at least one Material
    // in the tessBlock, and if possible
    if (TessBlocks[numtb].VegetableBlock == NULL &&
        TessBlocks[numtb].TileMaterialRefCount > 0 &&
...
void CPatch::createVegetableBlock(uint numTb, uint ts, uint tt) {
  ...
  uint cbWidth = (tbWidth + nTbPerCb - 1) >>
  ...
  CBezierPatch *bpatch = unpackIntoCache();
  CVector center = bpatch->eval((float)(tbs * 2 + 1) / OrderS,
                                (float)(tbt * 2 + 1) / OrderT);
  // Lower-Left tile is (tbs*2, tbt*2)
  vegetBlock->init(center, getLandscape()->_VegetableManager,
                   VegetableClipBlocks[cbt * cbWidth + cbs], this, tbs * 2,
                   tbt * 2, getLandscape()->_VegetableBlockList);
...
void CPatch::releaseVegetableBlock(uint numTb) {
  ...
  if (TessBlocks[numTb].VegetableBlock) {
    ...
    TessBlocks[numTb].VegetableBlock->release(
        getLandscape()->_VegetableManager, getLandscape()->_VegetableBlockList);
    // delete.

...
static ITexture *CreateGradientTexture() {
  ...
  CUniquePtr<CTextureMem> tex(new CTextureMem(
      (uint8 *)&GradientB2W, sizeof(GradientB2W), false, /* dont delete */
      false,                                             /* not a file */
      2, 1));
...
const NLMISC::CVector CPSRibbon::Losange[] = {
    NLMISC::CVector(0, 1.f, 0), NLMISC::CVector(1.f, 0, 0),
    NLMISC::CVector(0, -1.f, 0), NLMISC::CVector(-1.f, 0, 0)};

const NLMISC::CVector CPSRibbon::HeightSides[] = {
    NLMISC::CVector(-0.5f, 1, 0),  NLMISC::CVector(0.5f, 1, 0),
    NLMISC::CVector(1, 0.5f, 0),   NLMISC::CVector(1, -0.5f, 0),
    NLMISC::CVector(0.5f, -1, 0),  NLMISC::CVector(-0.5f, -1, 0),
    NLMISC::CVector(-1, -0.5f, 0), NLMISC::CVector(-1, 0.5f, 0)};

const NLMISC::CVector CPSRibbon::Pentagram[] = {
    NLMISC::CVector(0, 1, 0), NLMISC::CVector(1, -1, 0),
    NLMISC::CVector(-1, 0, 0), NLMISC::CVector(1, 0, 0),
    NLMISC::CVector(-1, -1, 0)};
...
const uint CPSRibbon::NbVerticesInSimpleSegmentX =
    sizeof(CPSRibbon::SimpleSegmentX) / sizeof(CVector);

const NLMISC::CVector CPSRibbon::SimpleSegmentY[] = {
    NLMISC::CVector(0, 1, 0),
    NLMISC::CVector(0, -1, 0),
};
const uint CPSRibbon::NbVerticesInSimpleSegmentY =
    sizeof(CPSRibbon::SimpleSegmentY) / sizeof(CVector);

const NLMISC::CVector CPSRibbon::SimpleSegmentZ[] = {
    NLMISC::CVector(0, 0, 1),
    NLMISC::CVector(0, 0, -1),
};
const uint CPSRibbon::NbVerticesInSimpleSegmentZ =
    sizeof(CPSRibbon::SimpleSegmentZ) / sizeof(CVector);

const NLMISC::CVector CPSRibbon::SimpleBrace[] = {
    NLMISC::CVector(1, 0, 0), NLMISC::CVector(-1, 0, 0),
    NLMISC::CVector(0, 1, 0), NLMISC::CVector(0, -1, 0)};
const uint CPSRibbon::NbVerticesInSimpleBrace =
    sizeof(CPSRibbon::SimpleBrace) / sizeof(CVector);

const uint CPSRibbon::NbVerticesInTriangle =
    sizeof(CPSRibbon::Triangle) / sizeof(CVector);
const uint CPSRibbon::NbVerticesInLosange = sizeof(Losange) / sizeof(CVector);
const uint CPSRibbon::NbVerticesInHeightSide =
    sizeof(CPSRibbon::HeightSides) / sizeof(CVector);
const uint CPSRibbon::NbVerticesInPentagram =
    sizeof(CPSRibbon::Pentagram) / sizeof(CVector);
...
void CPSRibbon::serial(NLMISC::IStream &f) {
  ...
  if (ver == 1) {
    ...
    /// we had CPSParticle::serial(f), but this is not the base class anymore,
    /// so we emulate this... version 2 : auto-lod saved
...
//=======================================================
CPSRibbon::CPSRibbon()
    : _UFactor(1.f), _VFactor(1.f), _Orientation(FollowPath), _BraceMode(true),
      _ColorFading(true), _GlobalColor(false), _Lighted(false),
      _ForceLighted(false), _Touch(true) {
...
//=======================================================
CPSRibbon::~CPSRibbon() { NL_PS_FUNC(CPSRibbon_CPSRibbonDtor) }
...
void CPSRibbon::step(TPSProcessPass pass) {
  ...
  if (pass == PSMotion) {
  ...
  } else if ((pass == PSBlendRender && hasTransparentFaces()) ||
    ...
    /** We support Auto-LOD for ribbons, although there is a built-in LOD (that
     * change the geometry rather than the number of ribbons) that gives better
...
static inline uint8 *ComputeRibbonSliceFollowPath(const NLMISC::CVector &prev,
  ...
  if (fabsf(next.x - prev.x) > epsilon || fabsf(next.y - prev.y) > epsilon ||
...
static inline uint8 *ComputeRibbonSliceIdentity(const NLMISC::CVector &prev,
  ...
  do {
    ((NLMISC::CVector *)dest)
        ->set(size * shape->x + next.x, size * shape->y + next.y,
              size * shape->z + next.z);
    ++shape;
    dest += vertexSize;
  } while (shape != shapeEnd);
...
///=========================================================================
static inline uint8 *ComputeRibbonSliceFollowPathXY(
    const NLMISC::CVector &prev, const NLMISC::CVector &next,
    const NLMISC::CVector *shape, uint numVerts, uint8 *dest, uint vertexSize,
    float size, NLMISC::CMatrix &basis) {
...
// This is for untextured versions (no need to duplicate the last vertex of each
// slice)
static inline uint8 *ComputeUntexturedRibbonMesh(
    uint8 *destVb, uint vertexSize, const NLMISC::CVector *curve,
    const NLMISC::CVector *shape, uint numSegs, uint numVerticesInShape,
    float sizeIncrement, float size, CPSRibbon::TOrientation orientation) {
  ...
  case CPSRibbon::FollowPath:
    do {
      destVb = ComputeRibbonSliceFollowPath(curve[1], curve[0], shape,
                                            numVerticesInShape, destVb,
                                            vertexSize, size, basis);
      ++curve;
      size -= sizeIncrement;
    } while (--numSegs);
    break;
  case CPSRibbon::FollowPathXY:
    do {
      destVb = ComputeRibbonSliceFollowPathXY(curve[1], curve[0], shape,
                                              numVerticesInShape, destVb,
                                              vertexSize, size, basis);
      ++curve;
      size -= sizeIncrement;
  ...
  case CPSRibbon::Identity:
    do {
      destVb = ComputeRibbonSliceIdentity(curve[1], curve[0], shape,
                                          numVerticesInShape, destVb,
                                          vertexSize, size);
      ++curve;
      size -= sizeIncrement;
  ...
  return BuildRibbonFirstSlice(curve[0], numVerticesInShape, destVb,
                               vertexSize);
}
...
// This is used to compute a ribbon mesh from its curve and its base shape.
// (Textured Version)
static inline uint8 *ComputeTexturedRibbonMesh(
    uint8 *destVb, uint vertexSize, const NLMISC::CVector *curve,
    const NLMISC::CVector *shape, uint numSegs, uint numVerticesInShape,
    float sizeIncrement, float size, CPSRibbon::TOrientation orientation) {
  ...
  case CPSRibbon::FollowPath:
    do {
      uint8 *nextDestVb = ComputeRibbonSliceFollowPath(
          curve[1], curve[0], shape, numVerticesInShape, destVb, vertexSize,
          size, basis);
  ...
  case CPSRibbon::FollowPathXY:
    do {
      uint8 *nextDestVb = ComputeRibbonSliceFollowPathXY(
          curve[1], curve[0], shape, numVerticesInShape, destVb, vertexSize,
          size, basis);
      // duplicate last vertex ( equal first)
      *(NLMISC::CVector *)nextDestVb = *(NLMISC::CVector *)destVb;
  ...
  case CPSRibbon::Identity:
    do {
      uint8 *nextDestVb = ComputeRibbonSliceIdentity(curve[1], curve[0], shape,
                                                     numVerticesInShape, destVb,
                                                     vertexSize, size);
      // duplicate last vertex ( equal first)
      *(NLMISC::CVector *)nextDestVb = *(NLMISC::CVector *)destVb;
  ...
  return BuildRibbonFirstSlice(curve[0], numVerticesInShape + 1, destVb,
                               vertexSize);
}
...
void CPSRibbon::displayRibbons(uint32 nbRibbons, uint32 srcStep) {
  ...
  CParticleSystem &ps = *(_Owner->getOwner());
  bool useGlobalColor =
      ps.getColorAttenuationScheme() != NULL || ps.isUserColorUsed();
  ...
  static std::vector<NLMISC::CVector>
      ribbonPos; // this is where the position of each ribbon slice center i
                 // stored
  ...
  do {
    ...
    VB.setNumVertices((_UsedNbSegs + 1) * toProcess * numVerticesInSlice);
    {
      ...
      if (_SizeScheme) {
        ptCurrSize =
            (float *)_SizeScheme->make(this->_Owner, ribbonIndex, &sizes[0],
                                       sizeof(float), toProcess, true, srcStep);
      ...
      /// compute colors
      if (_ColorScheme) {
        _ColorScheme->makeN(this->_Owner, ribbonIndex, currVert + colorOffset,
                            vertexSize, toProcess,
                            numVerticesInSlice * (_UsedNbSegs + 1), srcStep);
      }
      uint k = toProcess;
      //////////////////////////////////////////////////////////////////////////////////////
      // interpolate and project points the result is directly setup in the
      // vertex buffer //
      //////////////////////////////////////////////////////////////////////////////////////
      if (!_Parametric) {
        //////////////////////
        // INCREMENTAL CASE //
        //////////////////////
        if (_Tex != NULL && !_BraceMode) // textured case : must duplicate last
                                         // vertex, unless in brace mod
        {
          do {
            ...
            computeRibbon((uint)(fpRibbonIndex >> 16), &ribbonPos[0],
                          sizeof(NLMISC::CVector));
            currVert = ComputeTexturedRibbonMesh(
                currVert, vertexSize, &ribbonPos[0], &_Shape[0], _UsedNbSegs,
                numVerticesInShape, ribbonSizeIncrement, *ptCurrSize,
                _Orientation);
        ...
        } else // untextured case
        {
          do {
            ...
            computeRibbon((uint)(fpRibbonIndex >> 16), &ribbonPos[0],
                          sizeof(NLMISC::CVector));
            currVert = ComputeUntexturedRibbonMesh(
                currVert, vertexSize, &ribbonPos[0], &_Shape[0], _UsedNbSegs,
                numVerticesInShape, ribbonSizeIncrement, *ptCurrSize,
                _Orientation);
            fpRibbonIndex += srcStep;
        ...
        if (_Tex != NULL) // textured case
        {
          do {
            ...
            _Owner->integrateSingle(date - _UsedSegDuration * (_UsedNbSegs + 1),
                                    _UsedSegDuration, _UsedNbSegs + 1,
                                    (uint)(fpRibbonIndex >> 16), &ribbonPos[0]);
...
static inline void setTri(CIndexBufferReadWrite &ibrw, const CVertexBuffer &vb,
                          uint32 triIndex, uint32 i0, uint32 i1, uint32 i2) {
...
CPSRibbon::CVBnPB &CPSRibbon::getVBnPB() {
  ...
  /// choose the right vb by building an index for lookup into 'vbMaps' defined
  /// above
  TVBMap &map =
      _VBMaps[(_BraceMode ? 8 : 0) |           // set bit 3 if brace mode
  ...
  const uint numVerticesInSlice =
      getNumVerticesInSlice(); /// 1 vertex added for textured ribbon (to avoid
                               /// texture stretching)
  const uint numVerticesInShape = (uint)_Shape.size();
  ...
  if (it != map.end()) {
    return it->second;
  } else // must create this vb, with few different size, it is still
         // interseting, though they are only destroyed at exit
  {
    const uint numRibbonInVB = getNumRibbonsInVB();
    CVBnPB &VBnPB = map[VBnPDIndex]; // make an entry
    CIndexBuffer &pb = VBnPB.PB;
    CVertexBuffer &vb = VBnPB.VB;
    vb.setPreferredMemory(
        CVertexBuffer::AGPVolatile,
        true); // keep local memory because of interleaved format
    /// set the vb format & size
    /// In the case of a ribbon with color and fading, we encode the fading in a
    /// texture If the ribbon has fading, but only a global color, we encode it
    /// in the primary color
    vb.setVertexFormat(
        CVertexBuffer::PositionFlag | /* alway need position */
        (_ColorScheme || _ColorFading ? CVertexBuffer::PrimaryColorFlag
                                      : 0) | /* need a color ? */
        ((_ColorScheme && _ColorFading) || _Tex != NULL
             ? CVertexBuffer::TexCoord0Flag
             : 0) | /* need texture coordinates ? */
        (_Tex != NULL && _ColorScheme && _ColorFading
             ? CVertexBuffer::TexCoord1Flag
             : 0) /* need 2nd texture coordinates ? */
    );
    vb.setNumVertices((_UsedNbSegs + 1) * numRibbonInVB *
                      numVerticesInSlice); // 1 seg = 1 line + terminal vertices
    pb.setFormat(NL_DEFAULT_INDEX_BUFFER_FORMAT);
    // set the primitive block size
    if (_BraceMode) {
      pb.setNumIndexes(6 * _UsedNbSegs * numRibbonInVB *
                       (uint32)(_Shape.size() / 2));
    } else {
    ...
    for (i = 0; i < numRibbonInVB; ++i) {
      for (k = 0; k < (_UsedNbSegs + 1); ++k) {
        ...
        if (k != _UsedNbSegs) /// there are alway one more slice than segments
                              /// in the ribbon...
        {
          if (_BraceMode) {
            uint vIndex = vbIndex;
            ...
            {
              setTri(ibaWrite, vb, pbIndex, vIndex, vIndex + numVerticesInSlice,
                     vIndex + numVerticesInSlice + 1);
              pbIndex += 3;
              setTri(ibaWrite, vb, pbIndex, vIndex,
                     vIndex + numVerticesInSlice + 1, vIndex + 1);
              pbIndex += 3;
              vIndex += 2;
            }
          } else {
            uint vIndex = vbIndex;
            for (l = 0; l < (numVerticesInShape - 1);
                 ++l) /// deals with each ribbon vertices
            {
              setTri(ibaWrite, vb, pbIndex, vIndex, vIndex + numVerticesInSlice,
                     vIndex + numVerticesInSlice + 1);
              pbIndex += 3;
            ...
            uint nextVertexIndex =
                (numVerticesInShape == numVerticesInSlice)
                    ? vIndex + 1 - numVerticesInShape // no texture -> we loop
                    : vIndex + 1; // a texture is used : use onemore vertex
            setTri(ibaWrite, vb, pbIndex, vIndex, vIndex + numVerticesInSlice,
                   nextVertexIndex + numVerticesInSlice);
            pbIndex += 3;
            setTri(ibaWrite, vb, pbIndex, vIndex,
                   nextVertexIndex + numVerticesInSlice, nextVertexIndex);
            pbIndex += 3;
          }
        }
        ...
        /// setup vb
        if (_BraceMode) {
          for (l = 0; l < numVerticesInSlice / 2;
               ++l) /// deals with each ribbon vertices
          {
            ...
            if (_Tex != NULL) {
              vba.setTexCoord(
                  vbIndex,
                  _ColorScheme && _ColorFading
                      ? 1
                      : 0, // must we use the second texture coord ? (when 1st
                           // one used by the gradient texture : we can't encode
                           // it in the diffuse as it encodes each ribbon color)
                  (float)k / _UsedNbSegs, // u
                  0.f                     // v
              );
              vba.setTexCoord(vbIndex + 1, _ColorScheme && _ColorFading ? 1 : 0,
                              (float)k / _UsedNbSegs, 1.f);
            }
            /// setup gradient
            if (_ColorFading) {
              ...
              if (_ColorScheme == NULL) {
                uint8 intensity =
                    (uint8)(255 * (1.f - ((float)k / _UsedNbSegs)));
              ...
              {
                vba.setTexCoord(vbIndex, 0,
                                0.5f - 0.5f * ((float)k / _UsedNbSegs), 0);
                vba.setTexCoord(vbIndex + 1, 0,
                                0.5f - 0.5f * ((float)k / _UsedNbSegs), 0);
              }
            }
            vbIndex += 2;
...
uint CPSRibbon::getNumRibbonsInVB() const {
  ...
  return std::max(
      1u, (uint)(vertexInVB / (numVerticesInSlice * (_UsedNbSegs + 1))));
}
...
inline void CPSRibbon::updateUntexturedMaterial() {
  ...
  CParticleSystem &ps = *(_Owner->getOwner());
  if (_ColorScheme) { // PER RIBBON COLOR
    if (ps.getForceGlobalColorLightingFlag() || usesGlobalColorLighting() ||
        ps.getColorAttenuationScheme() || ps.isUserColorUsed()) {
      if (_ColorFading) // global color + fading + per ribbon color
      {
        ...
        CPSMaterial::forceTexturedMaterialStages(
            2); // use constant color 0 * diffuse, 1 stage needed
      ...
      {
        CPSMaterial::forceTexturedMaterialStages(
            1); // use constant color 0 * diffuse, 1 stage needed
      ...
      {
        CPSMaterial::forceTexturedMaterialStages(
            0); // no texture use constant diffuse only
      }
    }
  } else // GLOBAL COLOR
  {
  ...
  if (_ColorScheme) { // PER RIBBON COLOR
    if (ps.getForceGlobalColorLightingFlag() || usesGlobalColorLighting() ||
      ...
      if (_ColorFading) // global color + fading + per ribbon color
      {
        ...
        CPSMaterial::forceTexturedMaterialStages(
            3); // use constant color 0 * diffuse, 1 stage needed
        SetupModulatedStage(_Mat, 0, CMaterial::Texture, CMaterial::Diffuse);
      ...
      if (_ColorFading) // per ribbon color, fading : 2 textures needed
      {
        ...
        SetupModulatedStage(_Mat, 0, CMaterial::Texture,
                            CMaterial::Diffuse); // texture * ribbon color
        SetupModulatedStage(_Mat, 1, CMaterial::Texture,
                            CMaterial::Previous); // * gradient
      } else // per color ribbon with no fading, and no global color
      {
        _Mat.setTexture(0, _Tex);
        CPSMaterial::forceTexturedMaterialStages(
            1); // no texture use constant diffuse only
        SetupModulatedStage(_Mat, 0, CMaterial::Texture, CMaterial::Diffuse);
      }
    }
  } else // GLOBAL COLOR
...
void CPSRibbon::setupTextureMatrix() {
  ...
  if (_UFactor != 1.f || _VFactor != 1.f) {
    ...
    CMatrix texMat;
    texMat.setRot(_UFactor * NLMISC::CVector::I, _VFactor * NLMISC::CVector::J,
                  NLMISC::CVector::K);
...
#ifdef NL_COMP_VC14
  std::copy(_Shape.begin(), _Shape.end(),
            stdext::make_unchecked_array_iterator(shape));
#else
...
///==================================================================================================================
void CPSRibbon::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                         IDriver &drv) {

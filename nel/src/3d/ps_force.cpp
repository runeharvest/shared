 ...
 * Constructor
 */
CPSForce::CPSForce() { NL_PS_FUNC(CPSForce_CPSForce) }
...
void CPSForceIntensity::setIntensityScheme(CPSAttribMaker<float> *scheme) {
  ...
  if (getForceIntensityOwner() && scheme->hasMemory())
    scheme->resize(getForceIntensityOwner()->getMaxSize(),
                   getForceIntensityOwner()->getSize());
}
...
void CPSDirectionnalForce::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < _Owner->getSize(); ++k) {
    ...
    {
      dir = _GlobalValueHandle
                .get(); // takes direction from global variable instead
    } else {
      dir = _Dir;
    }
    toAddLocal = CParticleSystem::EllapsedTime *
                 (_IntensityScheme ? _IntensityScheme->get(_Owner, k) : _K) *
                 dir;
    toAdd = CPSLocated::getConversionMatrix(&target, this->_Owner)
                .mulVector(toAddLocal); // express this in the target basis
    TPSAttribVector::iterator it = target.getSpeed().begin(),
                              itend = target.getSpeed().end();
...
void CPSDirectionnalForce::show() {
  ...
  // for each element, see if it is the selected element, and if yes, display in
  // red
  for (uint k = 0; k < _Owner->getSize(); ++k) {
    const CRGBA col =
        (((lb == NULL || this == lb) && loc == _Owner && index == k)
             ? CRGBA::Red
             : CRGBA(127, 127, 127));
    CPSUtil::displayArrow(getDriver(), _Owner->getPos()[k], dir, 1.f, col,
                          CRGBA(80, 80, 0));
  }
}
...
void CPSGravity::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < _Owner->getSize(); ++k) {
    ...
    } else // only the z component is not null, which should be the majority of
           // cases ...
    {
      for (; it != itend; ++it) {
        it->z += toAdd.z;
...
void CPSGravity::show() {
  ...
  for (TPSAttribVector::const_iterator it = _Owner->getPos().begin();
    ...
    CPSUtil::print(driver, std::string("G"), *getFontGenerator(),
                   *getFontManager(), pos, 80.0f * toolSize);
  }
}
...
void CPSGravity::integrate(float date, CPSLocated *src, uint32 startIndex,
                           uint32 numObjects, NLMISC::CVector *destPos,
                           NLMISC::CVector *destSpeed, bool accumulate,
                           uint posStride, uint speedStride) const {
  NL_PS_FUNC(CPSGravity_integrate)
#define NEXT_SPEED                                                             \
  destSpeed = (NLMISC::CVector *)((uint8 *)destSpeed + speedStride);
#define NEXT_POS destPos = (NLMISC::CVector *)((uint8 *)destPos + posStride);

  float deltaT;

  if (!destPos && !destSpeed)
    return;

  CPSLocated::TPSAttribParametricInfo::const_iterator it = src->_PInfo.begin() +
                                                           startIndex,
                                                      endIt =
                                                          src->_PInfo.begin() +
                                                          startIndex +
                                                          numObjects;
  if (!accumulate) // compute coords from initial condition, and applying this
                   // force
  {
    if (destPos && !destSpeed) // fills dest pos only
    {
      while (it != endIt) {
        ...
        destPos->z =
            it->Pos.z + deltaT * it->Speed.z - 0.5f * deltaT * deltaT * _K;
        ++it;
        NEXT_POS;
      }
...
void CPSGravity::integrateSingle(
    float startDate, float deltaT, uint numStep, const CPSLocated *src,
  ...
  if (numStep != 0) {
    if (!accumulate) {
      destPos = FillBufUsingSubdiv(startPos, pi.Date, startDate, deltaT,
                                   numStep, destPos, stride);
      if (numStep != 0) {
        ...
        do {
          ...
          destPos->z =
              startPos.z + currDate * startSpeed.z - _K * halfTimeSquare;
          currDate += deltaT;
...
void CPSCentralGravity::computeForces(CPSLocated &target) {
  ...
  nlassert(CParticleSystem::InsideSimLoop);
  // for each central gravity, and each target, we check if they are in the same
  // basis if not, we need to transform the central gravity attachment pos into
  // the target basis
...
void CPSCentralGravity::show() {
  ...
  const CVector tab[] = {-I - J, I - J,  -I + J, I + J,  I - J, I + J,
...
void CPSSpring::computeForces(CPSLocated &target) {
  ...
  // if not, we need to transform the spring attachment pos into the target
  // basis
  uint32 size = _Owner->getSize();
  for (uint32 k = 0; k < size; ++k) {
    const float ellapsedTimexK =
...
void CPSSpring::show() {
  ...
  static const CVector tab[] = {
      -I + 2 * J, I + 2 * J, I + 2 * J, -I + J,     -I + J,     I + J,
      I + J,      -I,        -I,        I,          I,          -I - J,
      -I - J,     I - J,     I - J,     -I - 2 * J, -I - 2 * J, I - 2 * J};
  const uint tabSize = sizeof(tab) / (2 * sizeof(CVector));
  const float sSize = 0.08f;
  displayIcon2d(tab, tabSize, sSize);
}
...
void CPSCylindricVortex::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < size; ++k) // for each vortex
  {
    ...
    TPSAttribVector::iterator speedIt = target.getSpeed().begin(),
                              speedItEnd = target.getSpeed().end();
    TPSAttribFloat::const_iterator invMassIt = target.getInvMass().begin();
    TPSAttribVector::const_iterator posIt = target.getPos().begin();
    ...
    for (; speedIt != speedItEnd; ++speedIt, ++invMassIt, ++posIt) {
      ...
      if (d2 < radius2) // not out of range ?
      {
        if (d2 > 10E-6) {
          ...
          tangentialSpeed =
              (*speedIt * realTangentialSpeed) * realTangentialSpeed;
          radialSpeed = (p * *speedIt) * p;
          // update radial speed;
          *speedIt -=
              _RadialViscosity * CParticleSystem::EllapsedTime * radialSpeed;
          // update tangential speed
          *speedIt -=
              _TangentialViscosity * intensity * CParticleSystem::EllapsedTime *
              (tangentialSpeed - (1.f - d * invR) * realTangentialSpeed);
        }
      }
    }
  }
...
void CPSCylindricVortex::show() {
  ...
  for (uint k = 0; k < _Owner->getSize(); ++k) {
    ...
    CPSUtil::displayDisc(*getDriver(), _Radius[k], _Owner->getPos()[k], m, 32,
                         col);
    CPSUtil::displayArrow(getDriver(), _Owner->getPos()[k], _Normal[k], 1.f,
                          col, CRGBA(200, 0, 200));
    // display a V letter at the center
    CPSUtil::print(getDriver(), std::string("v"), *getFontGenerator(),
                   *getFontManager(), _Owner->getPos()[k], 80.f);
  }
}
...
void CPSMagneticForce::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < _Owner->getSize(); ++k) {
    float intensity =
        CParticleSystem::EllapsedTime *
        (_IntensityScheme ? _IntensityScheme->get(_Owner, k) : _K);
    NLMISC::CVector toAdd =
        CPSLocated::getConversionMatrix(&target, this->_Owner)
            .mulVector(_Dir); // express this in the target basis
    TPSAttribVector::iterator it = target.getSpeed().begin(),
                              itend = target.getSpeed().end();
    // 1st case : non-constant mass
    if (target.getMassScheme()) {
...
const uint BFPredefinedNumInterp =
    256; /** this should divide BFNumPredefinedPos. This define the number
          ...
          */
const uint BFNumPrecomputedImpulsions =
    1024; /// used to avoid to have to call rand for each particle the force
          /// applies on...

NLMISC::CVector
    CPSBrownianForce::PrecomputedPos[BFNumPredefinedPos]; // after the sequence
                                                          // we must be back to
                                                          // the start position
NLMISC::CVector CPSBrownianForce::PrecomputedSpeed[BFNumPredefinedPos];
NLMISC::CVector
    CPSBrownianForce::PrecomputedImpulsions[BFNumPrecomputedImpulsions];

///==========================================================
CPSBrownianForce::CPSBrownianForce(float intensity /* = 1.f*/)
    : _ParametricFactor(1.f) {
...
///==========================================================
void CPSBrownianForce::integrate(float date, CPSLocated *src, uint32 startIndex,
                                 uint32 numObjects, NLMISC::CVector *destPos,
                                 NLMISC::CVector *destSpeed, bool accumulate,
                                 uint posStride, uint speedStride) const {
                   ...
                   // force
  {
    if (destPos && !destSpeed) // fills dest pos only
    {
      while (it != endIt) {
        ...
        destPos->set(
            it->Pos.x + deltaT * it->Speed.x + _K * PrecomputedPos[index].x,
...
///==========================================================
void CPSBrownianForce::integrateSingle(float startDate, float deltaT,
                                       uint numStep, const CPSLocated *src,
                                       uint32 indexInLocated,
                                       NLMISC::CVector *destPos,
                                       bool accumulate, uint stride) const {
  ...
  if (numStep != 0) {
    ...
    if (!accumulate) {
      /// fill start of datas (particle didn't exist at that time, so we fill by
      /// the start position)
      destPos = FillBufUsingSubdiv(startPos, pi.Date, startDate, deltaT,
                                   numStep, destPos, stride);
      if (numStep != 0) {
...
#endif
          uint index =
              (uint)(lookUpFactor * currDate) & (BFNumPredefinedPos - 1);
          destPos->x = startPos.x + currDate * startSpeed.x +
                       _K * PrecomputedPos[index].x;
          destPos->y = startPos.y + currDate * startSpeed.y +
                       _K * PrecomputedPos[index].y;
          destPos->z = startPos.z + currDate * startSpeed.z +
                       _K * PrecomputedPos[index].z;
          currDate += deltaT;
          destPos = (NLMISC::CVector *)((uint8 *)destPos + stride);
        } while (--numStep);
      }
...
void CPSBrownianForce::initPrecalc() {
  ...
  for (k = 0; k < (BFPredefinedNumInterp << 2); ++k) {
    for (l = 1; l < (BFNumPredefinedPos - 1); ++l) {
      PrecomputedPos[l] =
          0.5f * (PrecomputedPos[l - 1] + PrecomputedPos[l + 1]);
    }
  }
...
void CPSBrownianForce::computeForces(CPSLocated &target) {
  ...
  for (uint32 k = 0; k < _Owner->getSize(); ++k) {
    ...
    if (target.getMassScheme()) {
      ...
      do {
        uint toProcess =
            std::min((uint)(BFNumPrecomputedImpulsions - startPos), (uint)size);
        it2End = it2 + toProcess;
        do {
          float factor = intensityXtime * *invMassIt;
          it2->set(it2->x + factor * imp->x, it2->y + factor * imp->y,
                   it2->z + factor * imp->x);
          ++invMassIt;
          ++imp;
          ++it2;
      ...
      do {
        ...
        it2End = it2 + toProcess;
        float factor =
            intensity * CParticleSystem::EllapsedTime / target.getInitialMass();
        do {
          it2->set(it2->x + factor * imp->x, it2->y + factor * imp->y,
                   it2->z + factor * imp->x);
          ++imp;

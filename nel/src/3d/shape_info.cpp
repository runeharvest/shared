  ...
  for (uint block = 0; block < numBlock; block++) {
    ...
    for (uint pass = 0; pass < numRenderPass; pass++) {
      ...
      if (material.getBlend())
        continue; // don't test against transparent materials
      // if (material.getAlphaTest()) continue; // don't test against
...
// ***************************************************************************
void CShapeInfo::build(const CMeshBase &meshBase,
  ...
  for (uint pass = 0; pass < numRenderPass; pass++) {
    ...
    if (material.getBlend())
      continue; // don't test against transparent materials
    // if (material.getAlphaTest()) continue; // don't test against transparent
    // materials

...
void CMeshMRMGeom::CLod::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  // Serial array of InfluencedVertices. NB: code written so far for
  // NL3D_MESH_SKINNING_MAX_MATRIX==4 only.
...
void CMeshMRMGeom::CLod::buildSkinVertexBlocks() {
  ...
  // For all vertices, test if they are used, and build the according
  // SkinVertexBlocks;
...
// ***************************************************************************
void CMeshMRMGeom::changeMRMDistanceSetup(float distanceFinest,
                                          float distanceMiddle,
                                          float distanceCoarsest) {
  // check input.
  if (distanceFinest < 0)
    return;
  if (distanceMiddle <= distanceFinest)
    return;
  if (distanceCoarsest <= distanceMiddle)
    return;
...
// ***************************************************************************
void CMeshMRMGeom::build(CMesh::CMeshBuild &m,
  ...
  /// 0. First, make bbox.
  //======================
  // NB: this is equivalent as building BBox from MRM VBuffer, because
  // CMRMBuilder create new vertices which are just interpolation of original
  // vertices.
  ...
  if (_Skinned) {
    ...
    for (vert = 0; vert < _SkinWeights.size(); vert++) {
      ...
      for (weight = 0; weight < NL3D_MESH_SKINNING_MAX_MATRIX; weight++) {
        ...
        if ((_SkinWeights[vert].Weights[weight] > 0) || (weight == 0)) {
          // Look for it
          std::map<uint, uint>::iterator ite =
              remap.find(_SkinWeights[vert].MatrixId[weight]);

          // Find ?
          if (ite == remap.end()) {
            // Insert it
            remap.insert(std::map<uint, uint>::value_type(
            ...
            // Set the bone name
            _BonesName.push_back(
                m.BonesNames[_SkinWeights[vert].MatrixId[weight]]);
    ...
    for (lod = 0; lod < _Lods.size(); lod++) {
      ...
      for (matrix = 0; matrix < _Lods[lod].MatrixInfluences.size(); matrix++) {
        // Remap
        std::map<uint, uint>::iterator ite =
            remap.find(_Lods[lod].MatrixInfluences[matrix]);
...
// ***************************************************************************
void CMeshMRMGeom::applyGeomorph(std::vector<CMRMWedgeGeom> &geoms,
                                 float alphaLod) {
  applyGeomorphWithVBHardPtr(geoms, alphaLod, NULL);
}

// ***************************************************************************
void CMeshMRMGeom::applyGeomorphWithVBHardPtr(std::vector<CMRMWedgeGeom> &geoms,
                                              float alphaLod,
                                              uint8 *vertexDestPtr) {
  // no geomorphs? quit.
  if (geoms.empty())
  ...
  if (flags == (CVertexBuffer::PositionFlag | CVertexBuffer::NormalFlag |
                CVertexBuffer::TexCoord0Flag) &&
      _VBufferFinal.getValueType(CVertexBuffer::TexCoord0) ==
          CVertexBuffer::Float2) {
    // use a faster method
    applyGeomorphPosNormalUV0(geoms, vertexPtr, vertexDestPtr, vertexSize, a,
                              a1);
  } else {
    // for color interp
    uint i;
    ...
    for (i = 0; i < CVertexBuffer::MaxStage; i++) {
      if (flags & (CVertexBuffer::TexCoord0Flag << i)) {
        ...
        has3Coords[i] =
            (_VBufferFinal.getValueType(i + CVertexBuffer::TexCoord0) ==
    ...
    /* NB: optimisation: lot of "if" in this Loop, but because of BTB, they
       always cost nothing (prediction is good). NB: this also is why we unroll
       the 4 1st Uv. The other (if any), are done in the other loop. NB:
       optimisation for AGP write cominers: the order of write (vertex, normal,
       uvs...) is important for good use of AGP write combiners. We have 2
       version : one that tests for 3 coordinates texture coords, and one that
       doesn't
    */
...
// ***************************************************************************
void CMeshMRMGeom::applyGeomorphPosNormalUV0(std::vector<CMRMWedgeGeom> &geoms,
                                             uint8 *vertexPtr,
                                             uint8 *vertexDestPtr,
                                             sint32 vertexSize, float a,
                                             float a1) {
...
void CMeshMRMGeom::render(IDriver *drv, CTransformShape *trans,
                          float polygonCount, uint32 rdrFlags,
                          float globalAlpha) {
  nlassert(drv);
  if (_Lods.empty())
    return;
  ...
  // \toto yoyo: TODO_OPTIMIZE: allocate only what is needed for the current Lod
  // (Max of all instances, like the loading....) (see loadHeader()).
  ...
  bool useTangentSpace =
      _MeshVertexProgram && _MeshVertexProgram->needTangentSpace();
  ...
  // ========
  if (bMorphApplied) {
    // If _Skinned (NB: the skin is not applied) and if
    // lod.OriginalSkinRestored, then restoreOriginalSkinPart is not called but
    // mush morpher write changed vertices into VBHard so its ok. The unchanged
    // vertices are written in the preceding call to restoreOriginalSkinPart.
    if (_Skinned) {
      _MeshMorpher.initSkinned(
          &_VBufferOriginal, &_VBufferFinal, useTangentSpace,
          &_OriginalSkinVertices, &_OriginalSkinNormals,
          useTangentSpace ? &_OriginalTGSpace : NULL, false);
      _MeshMorpher.updateSkinned(mi->getBlendShapeFactors());
    } else // Not even skinned so we have to do all the stuff
    {
      _MeshMorpher.init(&_VBufferOriginal, &_VBufferFinal, useTangentSpace);
  ...
  // Skinning.
  //===========
  // if mesh is skinned (but here skin not applied), we must copy
  // vertices/normals from original vertices.
  if (_Skinned) {
  ...
  if (useMeshVP) {
    ...
    useMeshVP = _MeshVertexProgram->begin(
        drv, mi->getOwnerScene(), mi, invertedObjectMatrix, renderTrav->CamPos);
  }
  ...
  if (globalAlphaUsed) {
    bool gaDisableZWrite =
        (rdrFlags & IMeshGeom::RenderGADisableZWrite) ? true : false;
    ...
    for (uint i = 0; i < lod.RdrPass.size(); i++) {
      ...
      if (((mi->Materials[rdrPass.MaterialId].getBlend() == false) &&
           (rdrFlags & IMeshGeom::RenderOpaqueMaterial)) ||
          ((mi->Materials[rdrPass.MaterialId].getBlend() == true) &&
           (rdrFlags & IMeshGeom::RenderTransparentMaterial))) {
        // CMaterial Ref
        CMaterial &material = mi->Materials[rdrPass.MaterialId];

        // Use a MeshBlender to modify material and driver.
        CMeshBlender blender;
        blender.prepareRenderForGlobalAlpha(material, drv, globalAlpha,
                                            globalAlphaInt, gaDisableZWrite);

        // Setup VP material
        if (useMeshVP) {
          _MeshVertexProgram->setupForMaterial(material, drv, ownerScene,
                                               &_VBufferFinal);
        }
...
void CMeshMRMGeom::renderSkin(CTransformShape *trans, float alphaMRM) {
  ...
  // get the meshMRM instance. only CMeshMRMInstance is possible when skinned
  // (not MultiLod)
  ...
  /*
      YOYO: renderSkin() no more support vertexBufferHard()!!! for AGP Memory
     optimisation concern. AGP Skin rendering is made when supportSkinGrouping()
     is true Hence if a skin is to be rendered here, because it doesn't have a
     good vertex format, or it has MeshVertexProgram etc..., it will be rendered
     WITHOUT VBHard => slower.
  */
  ...
  bool bMorphApplied = !_MeshMorpher.BlendShapes.empty();
  bool useNormal =
      (_VBufferFinal.getVertexFormat() & CVertexBuffer::NormalFlag) != 0;
  bool useTangentSpace =
      _MeshVertexProgram && _MeshVertexProgram->needTangentSpace();
  ...
  // ========
  if (bMorphApplied) {
    // Since Skinned we must update original skin vertices and normals because
    // skinning use it
    _MeshMorpher.initSkinned(&_VBufferOriginal, &_VBufferFinal, useTangentSpace,
                             &_OriginalSkinVertices, &_OriginalSkinNormals,
                             useTangentSpace ? &_OriginalTGSpace : NULL, true);
    _MeshMorpher.updateSkinned(mi->getBlendShapeFactors());
  }
  ...
  if (!useNormal) {
    ...
    if (!useTangentSpace) {
      ...
      applySkinWithTangentSpace(lod, skeleton,
                                _VBufferFinal.getNumTexCoordUsed() - 1);
    }
  }
...
// ***************************************************************************
bool CMeshMRMGeom::supportSkinGrouping() const { return _SupportSkinGrouping; }

// ***************************************************************************
sint CMeshMRMGeom::renderSkinGroupGeom(CMeshMRMInstance *mi, float alphaMRM,
                                       uint remainingVertices, uint8 *vbDest) {
  H_AUTO(NL3D_MeshMRMGeom_rdrSkinGrpGeom)

  // NB: not need to test if _Lods.empty(), because already done through
  // supportSkinGrouping()
  ...
  // Apply morph
  if (bMorphApplied) {
    // No need to manage lod.OriginalSkinRestored, since in case of
    // SkinGroupGeom, the VBuffer final is not modified.

    // copy directly from the original VB, and apply BlendShapes. Dest is
    // directly the RawSkin
    _MeshMorpher.updateRawSkin(&_VBufferFinal, mi->_RawSkinCache->VertexRemap,
  ...
  // always RawSkin now in SkinGrouping, even with MeshMorpher
  {
    ...
    // RawSkin do all the job in optimized way: Skinning, copy to VBHard and
    // Geomorph.

    // skinning with normal, but no tangent space
    applyRawSkinWithNormal(lod, *(mi->_RawSkinCache), skeleton, vbDest,
                           alphaLod);
...
// ***************************************************************************
void CMeshMRMGeom::renderSkinGroupPrimitives(
    CMeshMRMInstance *mi, uint baseVertex,
    std::vector<CSkinSpecularRdrPass> &specularRdrPasses, uint skinIndex) {
  ...
  for (uint i = 0; i < lod.RdrPass.size(); i++) {
    ...
    /*if( material.getDiffuse()!=CRGBA(250, 251, 252) )
    {
        ...
        static CSmartPtr<ITexture>	pTexFile= new
    CTextureFile("fy_hom_visage_c1_fy_e1.tga"); material.setTexture(0, pTexFile
    );
        // Remove Specular.
    ...
    if (material.getShader() == CMaterial::Specular) {
      ...
      CShiftedTriangleCache::CRdrPass &shiftedRdrPass =
          mi->_ShiftedTriangleCache->RdrPass[i];

      // Render with the Materials of the MeshInstance.
      drv->activeIndexBuffer(mi->_ShiftedTriangleCache->RawIndices);
      drv->renderTriangles(material, shiftedRdrPass.Triangles,
...
// ***************************************************************************
void CMeshMRMGeom::renderSkinGroupSpecularRdrPass(CMeshMRMInstance *mi,
                                                  uint rdrPassId) {
  ...
  // Get the shifted triangles.
  CShiftedTriangleCache::CRdrPass &shiftedRdrPass =
      mi->_ShiftedTriangleCache->RdrPass[rdrPassId];
...
// ***************************************************************************
void CMeshMRMGeom::updateShiftedTriangleCache(CMeshMRMInstance *mi,
                                              sint curLodId, uint baseVertex) {
  ...
  if (!mi->_ShiftedTriangleCache) {
    ...
    for (i = 0; i < pbList.size(); i++) {
      mi->_ShiftedTriangleCache->RdrPass[i].NumTriangles =
          pbList[i]->getNumIndexes() / 3;
    ...
    if (ibaWrite.getFormat() == CIndexBuffer::Indices32) {
      ...
      uint indexTri = 0;
      for (i = 0; i < pbList.size(); i++) {
        CShiftedTriangleCache::CRdrPass &dstRdrPass =
            mi->_ShiftedTriangleCache->RdrPass[i];
...
sint CMeshMRMGeom::loadHeader(NLMISC::IStream &f) {
  ...
  */
  ...
  /* NB: prepare memory space too for vertices.
      \todo yoyo: TODO_OPTIMIZE. for now there is no Lod memory profit with
     vertices / skinWeights. But resizing arrays is a problem because of
     reallocation...
  */
  uint32 nWedges;
  f.serial(nWedges);
  ...
  // resest the Lod arrays. NB: each Lod is empty, and ready to receive Lod
  // data.
  // ==================
  contReset(_Lods);
...
void CMeshMRMGeom::load(NLMISC::IStream &f) {
  ...
  for (uint i = 0; i < _LodInfos.size(); i++) {
    ...
    serialLodVertexData(f, _LodInfos[i].StartAddWedge,
                        _LodInfos[i].EndAddWedges);
...
void CMeshMRMGeom::save(NLMISC::IStream &f) {
  ...
  */
  ...
  // Warning, if you have skinned this shape, you can't write it anymore because
  // skinning id have been changed!
  ...
  // write all dummy offset. For now (since we don't know what to set), compute
  // the offset of the sint32 to come back in serial lod parts below.
  ...
  for (i = 0; i < _LodInfos.size(); i++) {
    ...
    // come back to "relative lodOffset" absolute position in the stream. (temp
    // stored in lodOffset[i]).
...
// ***************************************************************************
void CMeshMRMGeom::serialLodVertexData(NLMISC::IStream &f, uint startWedge,
                                       uint endWedge) {
  /*
  Version 1:
      - serial SkinWeights per MRM, not per Lod
...
void CMeshMRMGeom::loadNextLod(NLMISC::IStream &f) {
  ...
  // read the lod vertex data.
  serialLodVertexData(f, _LodInfos[_NbLodLoaded].StartAddWedge,
                      _LodInfos[_NbLodLoaded].EndAddWedges);
  // if reading, must bkup all original vertices from VB.
  // this is done in serialLodVertexData(). by subset
...
// ***************************************************************************
void CMeshMRMGeom::bkupOriginalSkinVerticesSubset(uint wedgeStart,
                                                  uint wedgeEnd) {
  nlassert(_Skinned);
  ...
  if (_VBufferFinal.getVertexFormat() & CVertexBuffer::PositionFlag) {
    // copy vertices from VBuffer. (NB: useless geomorphed vertices are still
    // copied, but doesn't matter).
  ...
  if (_VBufferFinal.getVertexFormat() & CVertexBuffer::NormalFlag) {
    // copy normals from VBuffer. (NB: useless geomorphed normals are still
    // copied, but doesn't matter).
...
void CMeshMRMGeom::restoreOriginalSkinVertices() {
  ...
  if (_MeshVertexProgram && _MeshVertexProgram->needTangentSpace()) {
    ...
    for (uint i = 0; i < _VBufferFinal.getNumVertices(); ++i) {
      *(CVector *)vba.getTexCoordPointer(i, numTexCoords - 1) =
          _OriginalTGSpace[i];
    }
  }
}
...
void CMeshMRMGeom::computeBonesId(CSkeletonModel *skeleton) {
  ...
  if (!_BoneIdComputed) {
    ...
    if (skeleton) {
      ...
      // Find the Geomorph space: to process only real vertices, not geomorphed
      // ones.
      uint nGeomSpace = 0;
      uint lod;
      for (lod = 0; lod < _Lods.size(); lod++) {
      ...
      // Remap the vertex, and compute the bone spheres. see
      // CTransform::getSkinBoneSphere() doc. for true vertices
      uint vert;
        ...
        for (weight = 0; weight < NL3D_MESH_SKINNING_MAX_MATRIX; weight++) {
          ...
          if ((_SkinWeights[vert].Weights[weight] > 0) || (weight == 0)) {
            ...
            if (_BonesId[srcId] >= 0) {
              ...
              CVector p =
                  skeleton->Bones[_BonesId[srcId]].getBoneBase().InvBindPos *
                  vertex;
      ...
      for (lod = 0; lod < _Lods.size(); lod++) {
        // For each matrix used
        uint matrix;
        for (matrix = 0; matrix < _Lods[lod].MatrixInfluences.size();
             matrix++) {
          // Check
          nlassert(_Lods[lod].MatrixInfluences[matrix] < remap.size());

          // Remap
          _Lods[lod].MatrixInfluences[matrix] =
              remap[_Lods[lod].MatrixInfluences[matrix]];
        }
      }
...
void CMeshMRMGeom::updateSkeletonUsage(CSkeletonModel *sm, bool increment) {
  ...
  for (uint i = 0; i < _BonesIdExt.size(); i++) {
    ...
    if (boneId >= sm->Bones.size())
      nlerror(" Skin is incompatible with Skeleton: tries to use bone %d",
              boneId);
    // increment or decrement not Forced, because CMeshGeom use
    // getActiveBoneSkinMatrix().
    if (increment)
...
void CMeshMRMGeom::compileRunTime() {
  ...
  _SupportMeshBlockRendering =
      _SupportMeshBlockRendering && _MeshVertexProgram == NULL;
}

// ***************************************************************************
void CMeshMRMGeom::profileSceneRender(CRenderTrav *rdrTrav,
  ...
  if (triCount) {
    // tri per VBFormat
    rdrTrav->Scene->incrementProfileTriVBFormat(
        rdrTrav->Scene->BenchRes.MeshMRMProfileTriVBFormat,
    ...
    // rendered in BlockRendering, only if not transparent pass (known it if
    // RenderTransparentMaterial is set)
    if (supportMeshBlockRendering() &&
        (rdrFlags & IMeshGeom::RenderTransparentMaterial) == 0) {
      if (isMeshInVBHeap()) {
...
// ***************************************************************************
bool CMeshMRMGeom::buildGeometryForLod(uint lodId,
                                       std::vector<CVector> &vertices,
                                       std::vector<uint32> &triangles) const {
  if (lodId >= _Lods.size())
    return false;
  ...
  // **** First, for the lod indicate what vertex is used or not. Also index
  // geomorphs to know what real vertex is used
  ...
  // **** Fill the vertex geometry
  {
    ...
    const uint8 *pSrcVert =
        (const uint8 *)vba.getVertexCoordPointer((uint)geomorphs.size());
...
bool CMeshMRMGeom::supportMeshBlockRendering() const {
  /*
      ...
      The problem is that lock() unlock() on each instance, on the same VBHeap
     IS AS SLOWER AS VB switching.
...
bool CMeshMRMGeom::sortPerMaterial() const {
  // Can't do it per material since 2 lods may not have the same number of
  // RdrPass!!
  return false;
}
// ***************************************************************************
...
// ***************************************************************************
void CMeshMRMGeom::activeInstance(CMeshGeomRenderContext &rdrCtx,
                                  CMeshBaseInstance *inst, float polygonCount,
                                  void *vbDst) {
  H_AUTO(NL3D_MeshMRMGeom_RenderNormal);
  ...
  if (_MBRCurrentLodId > 0) {
    if (rdrCtx.RenderThroughVBHeap)
      applyGeomorphWithVBHardPtr(_Lods[_MBRCurrentLodId].Geomorphs, alphaLod,
                                 (uint8 *)vbDst);
    else
...
// ***************************************************************************
void CMeshMRMGeom::renderPass(CMeshGeomRenderContext &rdrCtx,
                              CMeshBaseInstance *mi, float polygonCount,
                              uint rdrPassId) {
  if (_Lods.empty())
    return;
  ...
  if (mi->Materials[rdrPass.MaterialId].getBlend() == false) {
    ...
    if (rdrCtx.RenderThroughVBHeap) {
      ...
      rdrCtx.Driver->renderTriangles(material, 0,
                                     rdrPass.VBHeapPBlock.getNumIndexes() / 3);
    } else {
      rdrCtx.Driver->activeIndexBuffer(rdrPass.PBlock);
      rdrCtx.Driver->renderTriangles(material, 0,
                                     rdrPass.PBlock.getNumIndexes() / 3);
    }
  }
}
...
bool CMeshMRMGeom::getVBHeapInfo(uint &vertexFormat, uint &numVertices) {
  // CMeshMRMGeom support VBHeap rendering, assuming _SupportMeshBlockRendering
  // is true
...
void CMeshMRMGeom::computeMeshVBHeap(void *dst, uint indexStart) {
  ...
  CVertexBufferRead vba;
  _VBufferFinal.lock(vba);
  memcpy(dst, vba.getVertexCoordPointer(),
         _VBufferFinal.getNumVertices() * _VBufferFinal.getVertexSize());
  ...
  for (uint lodId = 0; lodId < _Lods.size(); lodId++) {
    ...
    for (uint i = 0; i < lod.RdrPass.size(); i++) {
      ...
      nlassert(ibaRead.getFormat() ==
               CIndexBuffer::Indices32); // nico : apparently not called for now
...
CTransformShape *CMeshMRM::createInstance(CScene &scene) {
  ...
  //===============================================
  CMeshMRMInstance *mi =
      (CMeshMRMInstance *)scene.createModel(NL3D::MeshMRMInstanceId);
  mi->Shape = this;
...
void CMeshMRM::render(IDriver *drv, CTransformShape *trans, bool passOpaque) {
  ...
  rdrFlags =
      mask & (IMeshGeom::RenderOpaqueMaterial | IMeshGeom::RenderPassOpaque);
  rdrFlags |= ~mask & (IMeshGeom::RenderTransparentMaterial);
  // render the mesh
  _MeshMRMGeom.render(drv, trans, trans->getNumTrianglesAfterLoadBalancing(),
                      rdrFlags, 1);
}
...
// ***************************************************************************
const CMeshMRMGeom &CMeshMRM::getMeshGeom() const { return _MeshMRMGeom; }
...
// ***************************************************************************
void CMeshMRM::changeMRMDistanceSetup(float distanceFinest,
                                      float distanceMiddle,
                                      float distanceCoarsest) {
  _MeshMRMGeom.changeMRMDistanceSetup(distanceFinest, distanceMiddle,
                                      distanceCoarsest);
}

// ***************************************************************************
IMeshGeom *CMeshMRM::supportMeshBlockRendering(CTransformShape *trans,
                                               float &polygonCount) const {
...
// ***************************************************************************
void CMeshMRM::profileSceneRender(CRenderTrav *rdrTrav, CTransformShape *trans,
                                  bool passOpaque) {
  // 0 or 0xFFFFFFFF
  uint32 mask = (0 - (uint32)passOpaque);
  uint32 rdrFlags;
  ...
  rdrFlags |= ~mask & (IMeshGeom::RenderTransparentMaterial);
  // profile the mesh
  _MeshMRMGeom.profileSceneRender(
      rdrTrav, trans, trans->getNumTrianglesAfterLoadBalancing(), rdrFlags);
}
...
void CMeshMRM::buildSystemGeometry() {
  ...
  // don't build a system copy if skinned. In this case, ray intersection is
  // done through CSkeletonModel and intersectSkin() scheme
  ...
  if (!_MeshMRMGeom.buildGeometryForLod(lodId, _SystemGeometry.Vertices,
                                        _SystemGeometry.Triangles)) {
    _SystemGeometry.clear();
  }
...
// ***************************************************************************
void CMeshMRMGeom::updateRawSkinNormal(bool enabled, CMeshMRMInstance *mi,
                                       sint curLodId) {
  if (!enabled) {
    ...
    // If the instance has no RawSkin, or has a too old RawSkin cache, must
    // delete it, and recreate
    if (!mi->_RawSkinCache || mi->_RawSkinCache->MeshDataId != _MeshDataId) {
      // first delete if too old.
      if (mi->_RawSkinCache)
        mi->clearRawSkinCache();
      // Then recreate, and use _MeshDataId to verify that the instance works
      // with same data.
    ...
    /* If the instance rawSkin has a different Lod (or if -1), then must
       recreate it. NB: The lod may change each frame per instance, but suppose
       not so many change, so we can cache those data.
    */
    if (mi->_RawSkinCache->LodId != curLodId) {
      ...
      // The remap from old index in _VBufferFinal to RawSkin vertices (without
      // Geomorphs).
      ...
      // Remap for BlendShape. Lasts 2 bits tells what RawSkin Array to seek (1
      // to 4), low Bits indicate the number in them. 0xFFFFFFFF is a special
      // value indicating "NotUsed in this lod"
      ...
      for (i = 0; i < numGeoms; i++) {
        // NB: don't add "numGeoms" to the index because RawSkin look in a
        // TempArray in RAM, which start at 0...
      ...
      for (i = 0; i < skinLod.RdrPass.size(); i++) {
        // NB: since RawSkin is possible only with SkinGrouping, and since
        // SkniGrouping is possible only with no Quads/Lines, we should have
        // only Tris here. remap tris.
...
// ***************************************************************************
void CMeshMRMGeom::setShadowMesh(
    const std::vector<CShadowVertex> &shadowVertices,
    const std::vector<uint32> &triangles) {
  _ShadowSkin.Vertices = shadowVertices;
  _ShadowSkin.Triangles = triangles;
  // update flag. Support Shadow SkinGrouping if Shadow setuped, and if not too
  // many vertices.
  _SupportShadowSkinGrouping =
      !_ShadowSkin.Vertices.empty() &&
      NL3D_SHADOW_MESH_SKIN_MANAGER_VERTEXFORMAT ==
          CVertexBuffer::PositionFlag &&
...
// ***************************************************************************
sint CMeshMRMGeom::renderShadowSkinGeom(CMeshMRMInstance *mi,
                                        uint remainingVertices, uint8 *vbDest) {
  ...
  // skinning with normal, but no tangent space
  // For all matrix this Mesh use. (the shadow geometry cannot use other Matrix
  // than the mesh use). NB: take the best lod since the lower lods cannot use
  // other Matrix than the higher one.
...
// ***************************************************************************
void CMeshMRMGeom::renderShadowSkinPrimitives(CMeshMRMInstance *mi,
                                              CMaterial &castMat, IDriver *drv,
                                              uint baseVertex) {
  nlassert(drv);
  ...
  shiftedTris.setPreferredMemory(CIndexBuffer::RAMVolatile, false);
  {
    ...
    for (uint n = (uint)_ShadowSkin.Triangles.size(); n > 0;
         n--, src++, dst++) {
...
// ***************************************************************************
bool CMeshMRMGeom::getSkinBoneBBox(CSkeletonModel *skeleton,
                                   NLMISC::CAABBox &bbox, uint boneId) const {
...
// ***************************************************************************
bool CMeshMRMGeom::intersectSkin(CMeshMRMInstance *mi,
                                 const CMatrix &toRaySpace, float &dist2D,
  ...
  // Compute skinning with all matrix this Mesh use. (the shadow geometry cannot
  // use other Matrix than the mesh use). NB: take the best lod (_Lods.back())
  // since the lower lods cannot use other Matrix than the higher one.
  return _ShadowSkin.getRayIntersection(toRaySpace, *skeleton,
                                        _Lods.back().MatrixInfluences, dist2D,
                                        distZ, computeDist2D);
}

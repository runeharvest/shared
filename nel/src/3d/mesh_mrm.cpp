...
void CMeshMRMGeom::applyGeomorphWithVBHardPtr(std::vector<CMRMWedgeGeom> &geoms, float alphaLod, uint8 *vertexDestPtr)
{
	...
	if (flags == (CVertexBuffer::PositionFlag | CVertexBuffer::NormalFlag | CVertexBuffer::TexCoord0Flag) && _VBufferFinal.getValueType(CVertexBuffer::TexCoord0) == CVertexBuffer::Float2)
	{
		// use a faster method
...
void CMeshMRMGeom::compileRunTime()
{
	...
	else
	{
		...
		_SupportSkinGrouping = _VBufferFinal.getVertexFormat() == NL3D_MESH_SKIN_MANAGER_VERTEXFORMAT && _VBufferFinal.getNumVertices() < NL3D_MESH_SKIN_MANAGER_MAXVERTICES && !_MeshVertexProgram;

...
const char *CFontGenerator::getFT2Error(FT_Error fte) {
  ...
  for (uint32 i = 0; ft_errors[i].err_code != 0 || ft_errors[i].err_msg != 0;
       i++) {
...
std::string CFontGenerator::getFontFileName() const { return _FontFileName; }
...
static unsigned long nlFreetypeStreamIo(FT_Stream stream, unsigned long offset,
  ...
  if (count > 0) {
    ...
    if (file->seek(offset, IStream::begin)) {
      try {
        ...
        nlwarning("Unable to read %u bytes from position %u of %s", (uint)count,
                  (uint)offset, file->getStreamName().c_str());
        count = 0;
      }
    } else {
      nlwarning("Unable to seek to position %u of %s", (uint)offset,
                file->getStreamName().c_str());
      count = 0;
    }
  }
...
// helper to open a font and use our functions to handle BNP files and UTF-8
// filenames
static bool createFreetypeStream(const std::string &filename,
                                 FT_Open_Args &args) {
 ...
 * Constructor
 */
CFontGenerator::CFontGenerator(const std::string &fontFileName,
  ...
  if (NLMISC::startsWith(fontFileName, "ui")) {
    ...
    for (std::vector<std::string>::iterator it(_FontFileNames.begin()),
         end(_FontFileNames.end());
         it != end; ++it) {
      *it = CPath::lookup(*it);
    }
  } else {
  ...
  for (std::vector<std::string>::iterator it(_FontFileNames.begin()),
    ...
    if (error) {
      nlerror("FT_New_Face() failed with file '%s': %s", fileName.c_str(),
              getFT2Error(error));
    }
    ...
    if (fontEx.empty()) {
      ...
      fontEx = CPath::lookup(
          CFile::getFilenameWithoutExtension(fileName) + ".afm", false, false);
    }
    ...
    if (!fontEx.empty()) {
      ...
      if (error) {
        nlwarning("FT_Attach_File() failed with file '%s': %s", fontEx.c_str(),
                  getFT2Error(error));
        FT_Done_Face(face);
        continue;
      }
    ...
    if (error) {
      nlerror("FT_Select_Charmap() failed with file '%s': %s", fileName.c_str(),
              getFT2Error(error));
      FT_Done_Face(face);
      continue;
    }
...
CFontGenerator::~CFontGenerator() {
  for (std::vector<FT_Face>::iterator it(_Faces.begin()), end(_Faces.end());
       it != end; ++it)
    FT_Done_Face(*it);
  _Faces.clear();
...
void CFontGenerator::getSizes(u32char c, uint32 size, uint32 &width,
                              uint32 &height) {
  FT_Error error;
  FT_Face face;
  ...
  if (error) {
    // use fallback for glyph/character errors (composite char limit for
    // example)
...
uint8 *CFontGenerator::getBitmap(u32char c, uint32 size, bool embolden,
                                 bool oblique, uint32 &width, uint32 &height,
                                 uint32 &pitch, sint32 &left, sint32 &top,
...
void CFontGenerator::getKerning(u32char left, u32char right, sint32 &kernx) {
  if (!FT_HAS_KERNING(_Faces[0])) {
    ...
    FT_Vector kerning;
    FT_Error error =
        FT_Get_Kerning(_Faces[0], left, right, ft_kerning_default, &kerning);
    if (error) {
...
CFontGenerator::CFontGenerator(const std::string &fontFileName,
  ...
  HDC hdc = GetDC(NULL);
  nlassert(hdc);
  Dib = CreateDIBSection(hdc, &info, DIB_RGB_COLORS, (void **)&Buffer, NULL,
                         NULL);
...
uint8 *CFontGenerator::getBitmap(u32char c, uint32 size, bool embolden,
  ...
  if (hFont == NULL || CurrentFontSize != size) {
    ...
    hFont =
        CreateFont(size,                // logical height of font
  ...
  DWORD glyphIndex;
  if (GetGlyphIndicesW(hdcDib, &cc, 1, &glyphIndex,
  ...
  /*	{
          ...
          printf ("w: %d h: %d s: %d a: %d l: %d t: %d", width, height, size,
     advx, left, top); getchar();
      }
  */

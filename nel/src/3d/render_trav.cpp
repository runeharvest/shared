...
// default is undefined, allows to see which CTransformShape are displayed in a
// scene, useful for debugging #define NL_DEBUG_RENDER_TRAV
...
void CRenderTrav::traverse(UScene::TRenderPart renderPart, bool newRender,
  ...
  if (getDriver()->isLost())
    return; // device is lost so no need to render anything
  ...
  if (newRender) {
    ...
    if (_MeshSkinManager) {
      if (Driver != _MeshSkinManager->getDriver()) {
        ...
        _MeshSkinManager->init(Driver, NL3D_MESH_SKIN_MANAGER_VERTEXFORMAT,
                               NL3D_MESH_SKIN_MANAGER_MAXVERTICES,
                               NL3D_MESH_SKIN_MANAGER_NUMVB, "MRMSkinVB", true);
      }
    }
    ...
    if (_ShadowMeshSkinManager) {
      if (getAuxDriver() != _ShadowMeshSkinManager->getDriver()) {
        ...
        _ShadowMeshSkinManager->init(
            getAuxDriver(), NL3D_SHADOW_MESH_SKIN_MANAGER_VERTEXFORMAT,
            NL3D_SHADOW_MESH_SKIN_MANAGER_MAXVERTICES,
            NL3D_SHADOW_MESH_SKIN_MANAGER_NUMVB, "ShadowSkinVB", true);
      }
    }
    ...
    // This is done here and not in the addRenderModel because of the
    // LoadBalancing traversal which can modify the transparency flag (multi lod
    // for instance)
    ...
    for (uint k = 0; k <= (uint)_MaxTransparencyPriority; ++k) {
      _OrderTransparentListByPriority[k].reset(
          _CurrentNumVisibleModels); // all table share the same allocator
                                     // (CLayeredOrderingTable::shareAllocator
                                     // has been called) and an object can be
                                     // only inserted in one table, so we only
                                     // need to init the main allocator
    }
    ...
    uint32 transparentOtSize =
        _OrderTransparentListByPriority[0]
            .getSize(); // there is at least one list, and all list have the
    ...
    for (; nNbModels > 0; itRdrModel++, nNbModels--) {
      ...
      // Yoyo: skins are rendered through skeletons, so models WorldMatrix are
      // all good here (even sticked objects)
      ...
      if (pTransform->isTransparent()) {
        ...
        _OrderTransparentListByPriority
            [std::min(pTransform->getTransparencyPriority(),
                      _MaxTransparencyPriority)]
                .insert(pTransform->getOrderingLayer(), pTransform,
                        transparentOtMax - otId);
      }
    }
    // fast floor
  ...
  if (renderPart & UScene::RenderOpaque) {
    ...
    while (OrderOpaqueList.get() != NULL) {
      ...
      if (trShape) {
        const std::string *shapeName =
            Scene->getShapeBank()->getShapeNameFromShapePtr(trShape->Shape);
        if (shapeName) {
    ...
    /* Render MeshBlock Manager.
        Some Meshs may be render per block. Interesting to remove VertexBuffer
       and Material setup overhead. Faster if rendered before lods, for ZBuffer
       optimisation: render first near objects then far. Lods are usually far
       objects.
    */
    ...
    /* Render Scene CoarseMeshManager.
        Important to render them at end of Opaque rendering, because coarses
       instances are created/removed during this model opaque rendering pass.
    */
    if (Scene->getCoarseMeshManager())
      Scene->getCoarseMeshManager()->flushRender(Driver);

    /* Render ShadowMaps.
        Important to render them at end of Opaque rendering, because
       alphaBlended objects must blend with opaque objects shadowed. Therefore,
       transparent objects neither can't cast or receive shadows...

        NB: Split in 2 calls and interleave Landscape Rendering between the 2.
       WHY??? Because it is far more efficient for VBLock (but not for ZBuffer
       optim...) because in renderGenerate() the ShadowMeshSkinManager do lot of
       VBLocks that really stall (because only 2 VBHard with swap scheme).

        Therefore the first Lock that stall will wait not only for the first
       MeshSkin to finish but also for the preceding landscape render to finish
       too! => big STALL.
    */
  ...
  if (renderPart & UScene::RenderTransparent) {
    if (_FirstWaterModel) // avoid a lock if no water is to be rendered
    {
      ...
      if (numWantedVertices != 0) {
        CWaterModel::setupVertexBuffer(Scene->getWaterVB(), numWantedVertices,
                                       getDriver());
        //
        {
  ...
  if ((renderPart & UScene::RenderTransparent) &&
    ...
    // =============================
    // Render transparent materials (draw higher priority last, because their
    // appear in front)
    _CurrentPassOpaque = false;
    for (std::vector<CLayeredOrderingTable<CTransform>>::iterator it =
             _OrderTransparentListByPriority.begin();
         it != _OrderTransparentListByPriority.end(); ++it) {
...
// ***************************************************************************
void CRenderTrav::clearRenderList() { _CurrentNumVisibleModels = 0; }
...
void CRenderTrav::removeRenderModel(CTransform *m) {
  // NB: storing a 8 bit in CTransform, instead of a 32 bits, is just to save
  // space.
...
void CRenderTrav::resetLightSetup() {
  ...
  if (!LightingSystemEnabled) {
    // Dont modify Driver lights, but setup default lighting For VertexProgram
    // Lighting.
    _NumLightEnabled = 1;
    ...
    _DriverLight[0].setupDirectional(CRGBA::Black, CRGBA::Black, CRGBA::Black,
                                     _SunDirection);
    Driver->setLight(0, _DriverLight[0]);
    // setup NULL point lights (=> cache will fail), so no need to setup other
    // lights in Driver.
  ...
  // else, must setup the lights into driver.
    ...
    if (lightContribution) {
      ...
      if (lightContribution->UseMergedPointLight)
        finalAmbient.addRGBOnly(finalAmbient,
      ...
      //	NB: sunSetup can't change during renderPass, so need only to
      //test factor.
      if (ufactor != _LastSunFactor || finalAmbient != _LastFinalAmbient) {
        ...
        // setup driver light
        _DriverLight[0].setupDirectional(finalAmbient, sunDiffuse, sunSpecular,
                                         _SunDirection);
        Driver->setLight(0, _DriverLight[0]);
      }
      ...
      while (lightContribution->PointLight[plId] != NULL) {
        ...
        // NB: pointLight setup can't change during renderPass, so need only to
        // test pointer, attenuation mode and factor.
        if (pl != _LastPointLight[plId] || inf != _LastPointLightFactor[plId] ||
          ...
          if (useLocalAttenuation)
            ...
            pl->setupDriverLightUserAttenuation(_DriverLight[plId + 1],
                                                uint8(inf));
...
void CRenderTrav::prepareVPLightSetup() {
  ...
  // Must force real light setup at least the first time, in
  // changeVPLightSetupMaterial()
  _VPMaterialCacheDirty = true;
}

// ***************************************************************************
void CRenderTrav::beginVPLightSetup(CVertexProgramLighted *program,
  ...
  for (; i < MaxVPLight; i++) {
    ...
    if (program->idxLighted().Diffuse[i] != std::numeric_limits<uint>::max()) {
      Driver->setUniform4f(IDriver::VertexProgram,
                           program->idxLighted().Diffuse[i], 0.f, 0.f, 0.f,
                           0.f);
    }
  }
  ...
  if (supportSpecular) {
    ...
    for (; i < MaxVPLight; i++) {
      ...
      if (program->idxLighted().Specular[i] !=
          std::numeric_limits<uint>::max()) {
        Driver->setUniform4f(IDriver::VertexProgram,
                             program->idxLighted().Specular[i], 0.f, 0.f, 0.f,
                             0.f);
      }
    }
  }
  ...
  Driver->setUniform3f(IDriver::VertexProgram,
                       program->idxLighted().DirOrPos[0],
                       lightDir); // The sun is the same for every instance.
  ...
  if (supportSpecular) {
    ...
    Driver->setUniform3f(IDriver::VertexProgram,
                         program->idxLighted().EyePosition, eye);
  }
  ...
  for (i = 1; i < _VPNumLights; i++) {
    ...
    Driver->setUniform3f(IDriver::VertexProgram,
                         program->idxLighted().DirOrPos[i], lightPos);
  }
...
// ***************************************************************************
void CRenderTrav::changeVPLightSetupMaterial(const CMaterial &mat,
  ...
  uint strongestLightIndex =
      excludeStrongest ? getStrongestLightIndex() : _VPNumLights;

  // setup Ambient + Emissive
  color = _VPFinalAmbient * mat.getAmbient();
  color += mat.getEmissive();
  Driver->setUniform4f(IDriver::VertexProgram, program->idxLighted().Ambient,
                       color);

  // is the strongest light is not excluded, its index should have been setup to
  // _VPNumLights
  ...
  for (i = 0; i < strongestLightIndex; ++i) {
    ...
    Driver->setUniform4f(IDriver::VertexProgram,
                         program->idxLighted().Diffuse[i], color);
  }

  nlassert(_VPNumLights <= MaxVPLight);
  if (i != _VPNumLights) {
    color = _VPLightDiffuse[i] * matDiff;
    _StrongestLightDiffuse.set(
        (uint8)(255.f * color.R), (uint8)(255.f * color.G),
        (uint8)(255.f * color.B), (uint8)(255.f * color.A));
    // setup strongest light to black for the gouraud part
    Driver->setUniform4f(IDriver::VertexProgram,
                         program->idxLighted().Diffuse[i], 0.f, 0.f, 0.f, 0.f);
    ++i;
    // setup other lights
  ...
  if (program->featuresLighted().SupportSpecular) {
    for (i = 0; i < strongestLightIndex; ++i) {
      ...
      color.A = specExp;
      Driver->setUniform4f(IDriver::VertexProgram,
                           program->idxLighted().Specular[i], color);
    }

    if (i != _VPNumLights) {
      color = _VPLightSpecular[i] * matSpec;
      _StrongestLightSpecular.set(
          (uint8)(255.f * color.R), (uint8)(255.f * color.G),
          (uint8)(255.f * color.B), (uint8)(255.f * color.A));
  ...
  // setup at good place
  Driver->setUniform4fv(IDriver::VertexProgram,
                        program->idxLighted().DiffuseAlpha, 1, alphaCte);
}

// ***************************************************************************
sint CRenderTrav::getStrongestLightIndex() const {
  if (!_StrongestLightTouched)
    return -1;
  uint vpNumLights = min(_NumLightEnabled, (uint)MaxVPLight);
  // If there is only a directionnal light, use it
  // If there is any point light, use the nearest, or the directionnal light if
  // it is brighter
  if (vpNumLights == 0)
    return -1;
  if (vpNumLights == 1)
  ...
  float lumDir = _VPLightDiffuse[0].R + _VPLightDiffuse[0].G +
                 _VPLightDiffuse[0].B + _VPLightDiffuse[0].A +
                 _VPLightSpecular[0].R + _VPLightSpecular[0].G +
                 _VPLightSpecular[0].B + _VPLightSpecular[0].A;
  float lumOmni = _VPLightDiffuse[1].R + _VPLightDiffuse[1].G +
                  _VPLightDiffuse[1].B + _VPLightDiffuse[1].A +
                  _VPLightSpecular[1].R + _VPLightSpecular[1].G +
                  _VPLightSpecular[1].B + _VPLightSpecular[1].A;
  return lumDir > lumOmni ? 0 : 1;
}

// ***************************************************************************
void CRenderTrav::getStrongestLightColors(NLMISC::CRGBA &diffuse,
...
// ***************************************************************************
static void strReplaceAll(string &strInOut, const string &tokenSrc,
  ...
  if (profile() == nelvp) {
    ...
    if (m_FeaturesLighted.SupportSpecular) {
      ...
      for (uint i = 1; i < MaxLight; ++i) {
        m_IdxLighted.DirOrPos[i] =
            m_FeaturesLighted.CtStartNeLVP + (12 - 1) + i;
      }
...
// ***************************************************************************
std::string CRenderTrav::getLightVPFragmentNeLVP(uint numActivePointLights,
                                                 uint ctStart,
  ...
  // Replace all CTS+x with good index. do it for 15 possible indices: 0 to 14
  // if specular. run from 14 to 0 so CTS+14 will not be taken for a CTS+1 !!
  for (sint i = 14; i >= 0; i--) {
    char tokenSrc[256];
...
// ***************************************************************************
void CRenderTrav::clearRenderLandscapeList() { _LandscapeRenderList.clear(); }
...
// ***************************************************************************
void CRenderTrav::setupTransparencySorting(uint8 maxPriority /*=0*/,
                                           uint NbDistanceEntries /*=1024*/) {
  NLMISC::contReset(
      _OrderTransparentListByPriority); // avoid useless object copy when vector
                                        // is resized (every element is reseted
                                        // anyway)
  ...
  for (uint k = 0; k < _OrderTransparentListByPriority.size(); ++k) {
    ...
    if (k != 0)
      _OrderTransparentListByPriority[k].shareAllocator(
          _OrderTransparentListByPriority[0]); // node allocator is shared
                                               // between all layers
  }
...
void CRenderTrav::debugWaterModelMemory(const char *tag, bool dumpList) {
  ...
  while (curr) {
    ...
    if (dmp.ClippedPolyBegin == dmp.ClippedPolyEnd) {
      ...
      nlwarning("Current: Ptr:%p. List:%p/%p", dmp.Address,
                dmp.ClippedPolyBegin, dmp.ClippedPolyEnd);
      ...
      for (uint i = 0; i < _DebugWaterModelList.size(); i++) {
        ...
        nlwarning("List%02d: Ptr:%p. Array:%p/%p", i, bkup.Address,
                  bkup.ClippedPolyBegin, bkup.ClippedPolyEnd);
      }

...
void CRenderTrav::traverse(UScene::TRenderPart renderPart, bool newRender, bool generateShadows)
{
	...
	if ((renderPart & UScene::RenderTransparent) && (renderPart & UScene::RenderFlare))
	{
	...
	else
	{
		...
		if (lightContribution)
		{
			...
			while (lightContribution->PointLight[plId] != NULL)
			{
				...
				if (pl != _LastPointLight[plId] || inf != _LastPointLightFactor[plId] || useLocalAttenuation != _LastPointLightLocalAttenuation[plId])
				{
...
void CRenderTrav::changeVPLightSetupMaterial(const CMaterial &mat, bool excludeStrongest)
{
	...
	if (!_VPMaterialCacheDirty)
	{
		...
		if (_VPMaterialCacheEmissive == mat.getEmissive().getPacked() && _VPMaterialCacheAmbient == mat.getAmbient().getPacked() && _VPMaterialCacheDiffuse == mat.getDiffuse().getPacked())
		{
			// Same Diffuse part, test if same specular if necessary
			if (!program->featuresLighted().SupportSpecular || (_VPMaterialCacheSpecular == mat.getSpecular().getPacked() && _VPMaterialCacheShininess == mat.getShininess()))
			{
				// Then ok, skip.
				return;
			}
...
// ***************************************************************************
static const char *LightingVPFragmentNormalize = "	# normalize normal																	\n\
	DP3	R6.w, R6, R6;																	\n\
	RSQ	R6.w, R6.w;																		\n\
	MUL	R6, R6, R6.w;																	\n\
";
...
// The End code.
static const char *LightingVPFragmentNoSpecular_End = "	# output to o[COL0] only, replacing alpha with material alpha.						\n\
...
// The End code.
static const char *LightingVPFragmentSpecular_End = "	# output directly to secondary color.												\n\

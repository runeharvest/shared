...
/* This floor works only for floor with noise, because floor/ceil are only made
  on decimal coordinates: sTile =1.25 ....  NB: because of difference of mapping
  (rare case), we may have sometimes values with precision < 1/4 (eg 1.125).
  Just use f*256 to compute the floor.

  NB: using a fastFloor() (fistp changing the controlfp() is not very a good
  idea here, because computeNoise() are not "packed", so change on controlFp()
  would bee too frequent... And also because we need either floor() or ceil()
  here.
*/
...
inline float noiseFloorF(float f) { return (float)noiseFloor(f); }
inline float noiseCeilF(float f) { return (float)noiseCeil(f); }

#else

inline float noiseFloorF(float f) { return (float)floor(f); }
inline float noiseCeilF(float f) { return (float)ceil(f); }

inline sint noiseFloor(float f) { return (sint)floor(f); }
inline sint noiseCeil(float f) { return (sint)ceil(f); }

#endif

// ***************************************************************************
float CPatch::computeDisplaceRawInteger(sint ts, sint tt, sint ms,
                                        sint mt) const {
  // Choose the noiseMap.
  // ===============================
  clamp(ts, 0, OrderS - 1);
  ...
  // Get the tile for pass0. This is the principal tile, and this one tells what
  // noise to take.
  ...
  noiseMap = getZone()->getLandscape()->TileBank.getTileNoiseMap(tileNumber,
                                                                 tileSubNoise);
...
// ***************************************************************************
void CPatch::computeDisplaceRawCoordinates(float sTile, float tTile, float s,
                                           float t, sint &ts, sint &tt,
                                           sint &ms, sint &mt) const {
  // Choose the noiseMap.
  // ===============================
  ...
  // direct map (no bilinear, no round, the case where s,t < 1/4 of a tile is
  // very rare).
  ms = noiseFloor(uv.U);
  mt = noiseFloor(uv.V);

  // Manage Tiling (add NL3D_TILE_NOISE_MAP_SIZE*1 should be sufficient, but
  // take margin).
...
// ***************************************************************************
float CPatch::computeDisplaceRaw(float sTile, float tTile, float s,
                                 float t) const {
  sint ts, tt, ms, mt;
...
static inline void computeDisplaceBilinear(float sTile, float tTile,
                                           float &sInc, float &tInc, float &sa,
                                           float &ta, float &sa1, float &ta1) {
...
float CPatch::computeDisplaceInteriorSmooth(float s, float t) const {
  ...
  // NB: to have smooth transition, must keep the same (s,t), so we do a
  // transition with the noise tile of our neigbhor, but under us.

  // speed up, using just one computeDisplaceRawCoordinates(), and multiple
  // computeDisplaceRawInteger().
  sint ts, tt, ms, mt;
  computeDisplaceRawCoordinates(sTile, tTile, s, t, ts, tt, ms, mt);
  ...
  ret +=
      computeDisplaceRawInteger(ts + sIncInt, tt + tIncInt, ms, mt) * sa1 * ta1;

  return ret;
}

// ***************************************************************************
float CPatch::computeDisplaceEdgeSmooth(float s, float t, sint8 smoothBorderX,
  ...
  if (smoothBorderX == -1)
    edge = 0;
  else if (smoothBorderY == 1)
    edge = 1;
  else if (smoothBorderX == 1)
    edge = 2;
  else if (smoothBorderY == -1)
    edge = 3;
  else
  ...
  // Fast reject: if no neighbor, just do a simple
  // computeDisplaceInteriorSmooth.
  if (!bindInfo.Zone)
  ...
  // else, look for result in neighborhood.
    ...
    float sInc, tInc, sa, ta, sa1, ta1;
    computeDisplaceBilinear(sTile, tTile, sInc, tInc, sa, ta, sa1, ta1);
    // Manage limit case: if bilinear has not chosen the good direction (because
    // of floor and orientation). eg on Right edge: This case arise if
    // sDecimal==0, so if sa==sa1==0.5f. result is that smoothBorderX is != than
    // sInc on right border. same reasoning with downBorder (smoothBorderY=1).

    // NO NEED TO DO HERE, because sInc or tInc is not used if it is bad
    // (smoothBorder? used instead). and no need to correct sa, sa1, because in
    // this case they are both equal to 0.5.
    ...
    /* NB: there is floor problems with neighbors:
        - difference of orientation => uv.v=1. This point to the 1th tile. But
       this is not the same, if v goes to up, or goes to down.
        - if multiple bind, problem at limit (eg a bind 1/2 on edge 0 with
       OrdertT=8, when uv.v= 4). because, selection of the patch is dependent of
       orientation too. To avoid them, just take center of (sTile, tTile) to
       remove ambiguity. This works because computeDisplaceRaw() use sTile,
       tTile to get the noiseMap, so the decimal part is not used.

        Notice that we do this AFTER computeDisplaceBilinear() of course.
    */
    sTile = noiseFloor(sTile) + 0.5f;
    tTile = noiseFloor(tTile) + 0.5f;
    // If we were exactly on the superior edge, prec compute is false... so
    // correct this here.
    if (sTile > OrderS)
      sTile--;
    if (tTile > OrderT)
    ...
    if (smoothBorderX != 0) {
      ...
      ret += patchOut->computeDisplaceRaw(stTileOut.x, stTileOut.y, stOut.x,
                                          stOut.y) *
             sa1 * ta;
...
// ***************************************************************************
float CPatch::computeDisplaceRawOnNeighbor(float sTile, float tTile, float s,
                                           float t) const {
  sint edge = -1;
  ...
  if (sTile < 0)
    edge = 0;
  else if (tTile > OrderT)
    edge = 1;
  else if (sTile > OrderS)
    edge = 2;
  else if (tTile < 0)
    edge = 3;
  ...
  // else must find on neighbor.
    ...
    // Fast reject: if no neighbor on the edge, just do a simple
    // computeDisplaceRaw()
    if (!bindInfo.Zone) {
      return computeDisplaceRaw(sTile, tTile, s, t);
    }
    // else must find on neighbor.
      ...
      uvLocator.locateUV(stTileIn, patchId, patchOut, stTileOut);
      uvLocator.locateUV(stIn, patchId, patchOut, stOut);
      return patchOut->computeDisplaceRaw(stTileOut.x, stTileOut.y, stOut.x,
                                          stOut.y);
    }
  }
}
...
// ***************************************************************************
float CPatch::computeDisplaceCornerSmooth(float s, float t, sint8 smoothBorderX,
                                          sint8 smoothBorderY) const {
  // compute the value across the corner (enjoy!!)
  // NB: Only corners with Edges==4 and corners on a bind are correclty
  // supported. ignore problems with corner which nbEdges!=4, because Blend of
  // normals blend to 0 on corner (see computenoise()).
  ...
  // Fast reject: if no neighbor on one of the edge, just do a simple
  // computeDisplaceInteriorSmooth.
  if (!bindInfoX.Zone || !bindInfoY.Zone)
    ...
    // compute contribution of the patch on the left/right side. same reasoning
    // as in computeDisplaceEdgeSmooth();
    stIn.set(s, t);
    stTileIn.set(sTile + smoothBorderX, tTile);
    ...
    // compute contribution of the patch on the up/down side. same reasoning as
    // in computeDisplaceEdgeSmooth();
    stIn.set(s, t);
    stTileIn.set(sTile, tTile + smoothBorderY);
    ...
    /* compute contribution of the patch adjacent to me.
        There is multiple case to consider here. Take example with corner=0 (ie
       smBdX=smBdY=-1):
            - if we are a normal corner with 4 edges, take the result from the
       left patch of our top patch.
            - if the corner is on a bind Edge, the top patch may be the bigger
       patch, so don't take the result from his neighbor (of course).
            - if we are a normal corner with N!=4 edges, just do same thing than
       if N==4. this is false but don't bother.
    ...
    */
    ...
    uvLocatorY.locateUV(stTileIn, patchId, patchOut, stTileOut);
    uvLocatorY.locateUV(stIn, patchId, patchOut, stOut);
    ret += patchOut->computeDisplaceRawOnNeighbor(stTileOut.x, stTileOut.y,
                                                  stOut.x, stOut.y) *
           sa1 * ta1;
...
// ***************************************************************************
CVector CPatch::computeNormalEdgeSmooth(float s, float t, sint8 smoothBorderX,
                                        sint8 smoothBorderY) const {
  CBindInfo bindInfo;
  uint edge = 0;
  CBezierPatch *bpatch;
  ...
  if (getSmoothFlag(edge)) {
    ...
    if (!bindInfo.Zone)
      ...
      // Compute the coordinate on the border of the edge, and the coef of the
      // blend.
      float se = s;
      float te = t;
      float coef = 0.0;
      if (smoothBorderX == -1)
        se = noiseFloorF(se), coef = s - se;
      else if (smoothBorderX == 1)
        se = noiseCeilF(se), coef = se - s;
      else if (smoothBorderY == -1)
        te = noiseFloorF(te), coef = t - te;
      else if (smoothBorderY == 1)
        te = noiseCeilF(te), coef = te - t;
      coef = 0.5f + coef * 0.5f;

      // Compute contribution of the normal on the neighbor, on the border of
      // the edge.
      CPatchUVLocator uvLocator;
      ...
      r1 = bpatch->evalNormal(stOut.x / patchOut->getOrderS(),
                              stOut.y / patchOut->getOrderT());

      // NB: don't bother problems with bind 1/X and the choice of the patch,
      // because bind are C1, so normal is C0.
...
// ***************************************************************************
CVector CPatch::computeNormalOnNeighbor(float s, float t,
                                        uint edgeExclude) const {
  sint edge = -1;

  // look on what neighbor patch we must find the value (if any).
  if (s < 1 && edgeExclude != 0)
    edge = 0;
  else if (t > OrderT - 1 && edgeExclude != 1)
    edge = 1;
  else if (s > OrderS - 1 && edgeExclude != 2)
    edge = 2;
  else if (t < 1 && edgeExclude != 3)
    edge = 3;

  // If the location is In the patch, just return normal value. (case of a bind
  // 1/X).
  if (edge == -1) {
  ...
  // else must find on neighbor.
    ...
    // else must find on neighbor.
      ...
      return bpatch->evalNormal(stOut.x / patchOut->getOrderS(),
                                stOut.y / patchOut->getOrderT());
    }
  }
}
...
// ***************************************************************************
CVector CPatch::computeNormalCornerSmooth(float s, float t, sint8 smoothBorderX,
                                          sint8 smoothBorderY) const {
  CBindInfo bindInfoX;
  CBindInfo bindInfoY;
  uint edgeX = 0;
  ...
  if (getCornerSmoothFlag(corner)) {
    ...
    if (!bindInfoX.Zone || !bindInfoY.Zone)
      ...
      float coefY;
      if (smoothBorderX == -1)
        se = noiseFloorF(se), coefX = s - se;
      else
        se = noiseCeilF(se), coefX = se - s;
      if (smoothBorderY == -1)
        te = noiseFloorF(te), coefY = t - te;
      else
        te = noiseCeilF(te), coefY = te - t;
      coefX = 0.5f + coefX * 0.5f;
      coefY = 0.5f + coefY * 0.5f;

      // Compute our contribution.
      ret =
          bpatch->evalNormal(s / getOrderS(), t / getOrderT()) * coefX * coefY;
      ...
      uvLocatorX.locateUV(stIn, patchId, patchOut, stOut);
      bpatch = patchOut->unpackIntoCache();
      ret += bpatch->evalNormal(stOut.x / patchOut->getOrderS(),
                                stOut.y / patchOut->getOrderT()) *
             (1 - coefX) * coefY;
      ...
      */
      ...
      patchId = uvLocatorY.selectPatch(stIn);
      uvLocatorY.locateUV(stIn, patchId, patchOut, stOut);
      // Because we compute the normal exactly on the edge, we must inform this
      // method not to take us as neighbor. ugly but simpler.
      ret += patchOut->computeNormalOnNeighbor(stOut.x, stOut.y,
                                               bindInfoY.Edge[patchId]) *
             (1 - coefX) * (1 - coefY);
  ...
  // else must blend with 0.
    ...
    // To have smooth continuities with smooth on edge (if any), we must do
    // this.
    CVector rx, ry;
...
void CPatch::computeNoise(float s, float t, CVector &displace) const {
  ...
  // NB: because OrderS and OrderT >=2, smoothNormalBorderX=-1 and
  // smoothNormalBorderX=1 are exclusive (as smoothNormalBorderY).
  if (so < 1)
    smoothNormalBorderX = -1;
  else if (so > OrderS - 1)
    smoothNormalBorderX = 1;
  if (to < 1)
    smoothNormalBorderY = -1;
  else if (to > OrderT - 1)
    smoothNormalBorderY = 1;

  bool smoothNormalEdge =
      (smoothNormalBorderX != 0) != (smoothNormalBorderY != 0);
  bool smoothNormalCorner =
      (smoothNormalBorderX != 0) && (smoothNormalBorderY != 0);

  // Do same thing, but to know if we must compute a displace on an interior, on
  // an edge or on a corner.
  sint8 smoothDisplaceBorderX = 0;
  sint8 smoothDisplaceBorderY = 0;
  // NB: because OrderS and OrderT >=2, smoothBorderX=-1 and smoothBorderX=1 are
  // exclusive (as smoothBorderY).
  if (so < 0.5)
    smoothDisplaceBorderX = -1;
  else if (so > OrderS - 0.5)
    smoothDisplaceBorderX = 1;
  if (to < 0.5)
    smoothDisplaceBorderY = -1;
  else if (to > OrderT - 0.5)
    smoothDisplaceBorderY = 1;

  bool smoothDisplaceEdge =
      (smoothDisplaceBorderX != 0) != (smoothDisplaceBorderY != 0);
  bool smoothDisplaceCorner =
      (smoothDisplaceBorderX != 0) && (smoothDisplaceBorderY != 0);
  ...
  if (smoothDisplaceCorner)
    displaceValue = computeDisplaceCornerSmooth(so, to, smoothDisplaceBorderX,
                                                smoothDisplaceBorderY);
  else if (smoothDisplaceEdge)
    displaceValue = computeDisplaceEdgeSmooth(so, to, smoothDisplaceBorderX,
                                              smoothDisplaceBorderY);
  else
  ...
  if (smoothNormalCorner)
    displaceNormal = computeNormalCornerSmooth(so, to, smoothNormalBorderX,
                                               smoothNormalBorderY);
  else if (smoothNormalEdge)
    displaceNormal = computeNormalEdgeSmooth(so, to, smoothNormalBorderX,
                                             smoothNormalBorderY);
  else {
    // unpack...
    CBezierPatch *bpatch = unpackIntoCache();

...
static NLMISC::CVector2f toTex(NLMISC::CVector2f texCoord,
                               NLMISC::CVector2f uvScaleOffset[2]) {
  // return(EyeToSourceUVScale * flattened + EyeToSourceUVOffset);
  NLMISC::CVector2f vec = NLMISC::CVector2f(texCoord.x * uvScaleOffset[0].x,
                                            texCoord.y * uvScaleOffset[0].y) +
                          uvScaleOffset[1];
...
CStereoOVR::CStereoOVR(const CStereoOVRDeviceFactory *factory)
    : m_DevicePtr(NULL), m_Stage(0), m_SubStage(0), m_OrientationCached(false),
      m_Driver(NULL), m_SceneTexture(NULL), m_GUITexture(NULL),
      m_EyePosition(0.0f, 0.09f, 0.15f), m_Scale(1.0f),
      m_AttachedDisplay(false) {
  nlctassert(NL_OVR_EYE_COUNT == ovrEye_Count);

  if (factory->DetectId != s_DetectId) {
    nlwarning("OVR: Previous device info structures become invalid after "
              "listing devices");
    return;
  }

  m_DebugDevice = factory->DebugDevice;
  if (factory->DebugDevice)
    m_DevicePtr = ovrHmd_CreateDebug(factory->DebugDeviceType);
  else
    m_DevicePtr = ovrHmd_Create(factory->DeviceIndex);
  ...
  // nldebug("OVR: HScreenSize: %f, VScreenSize: %f",
  // m_DevicePtr->HMDInfo.HScreenSize, m_DevicePtr->HMDInfo.VScreenSize); // No
  // more support for physically non-square pixels? nldebug("OVR: VScreenCenter:
  // %f", m_DevicePtr->HMDInfo.VScreenCenter); nldebug("OVR:
  // EyeToScreenDistance: %f", m_DevicePtr->HMDInfo.EyeToScreenDistance);
  // nldebug("OVR: LensSeparationDistance: %f",
  // m_DevicePtr->HMDInfo.LensSeparationDistance); nldebug("OVR:
  // InterpupillaryDistance: %f", m_DevicePtr->HMDInfo.InterpupillaryDistance);
  nldebug("OVR: Resolution.w: %i, Resolution.h: %i", m_DevicePtr->Resolution.w,
          m_DevicePtr->Resolution.h);
  // nldebug("OVR: DistortionK[0]: %f, DistortionK[1]: %f",
  // m_DevicePtr->HMDInfo.DistortionK[0], m_DevicePtr->HMDInfo.DistortionK[1]);
  // nldebug("OVR: DistortionK[2]: %f, DistortionK[3]: %f",
  // m_DevicePtr->HMDInfo.DistortionK[2], m_DevicePtr->HMDInfo.DistortionK[3]);

  if (!ovrHmd_ConfigureTracking(
          m_DevicePtr,
          ovrTrackingCap_Orientation |
              ovrTrackingCap_MagYawCorrection, // | ovrTrackingCap_Position
          ovrTrackingCap_Orientation)) {
  ...
  eyeRenderDesc[ovrEye_Left] = ovrHmd_GetRenderDesc(
      m_DevicePtr, ovrEye_Left, m_DevicePtr->DefaultEyeFov[ovrEye_Left]);
  eyeRenderDesc[ovrEye_Right] = ovrHmd_GetRenderDesc(
      m_DevicePtr, ovrEye_Right, m_DevicePtr->DefaultEyeFov[ovrEye_Right]);
  nldebug("OVR: LEFT DistortedViewport: x: %i, y: %i, w: %i, h: %i",
  ...
  nldebug("OVR: LEFT PixelsPerTanAngleAtCenter: x: %f, y: %f ",
          eyeRenderDesc[0].PixelsPerTanAngleAtCenter.x,
          eyeRenderDesc[0].PixelsPerTanAngleAtCenter.y);
  nldebug("OVR: LEFT ViewAdjust: x: %f, y: %f, z: %f ",
          eyeRenderDesc[0].ViewAdjust.x, eyeRenderDesc[0].ViewAdjust.y,
          eyeRenderDesc[0].ViewAdjust.z);
  nldebug("OVR: RIGHT DistortedViewport: x: %i, y: %i, w: %i, h: %i",
  ...
  nldebug("OVR: RIGHT PixelsPerTanAngleAtCenter: x: %f, y: %f ",
          eyeRenderDesc[1].PixelsPerTanAngleAtCenter.x,
          eyeRenderDesc[1].PixelsPerTanAngleAtCenter.y);
  nldebug("OVR: RIGHT ViewAdjust: x: %f, y: %f, z: %f ",
          eyeRenderDesc[1].ViewAdjust.x, eyeRenderDesc[1].ViewAdjust.y,
          eyeRenderDesc[1].ViewAdjust.z);

  // 2014/08/04 19:54:25 DBG  a60 snowballs_client.exe stereo_ovr_04.cpp 171
  // NL3D::CStereoOVR::CStereoOVR : OVR: Resolution.w: 1280, Resolution.h: 800
  // 2014/08/04 19:54:25 DBG  a60 snowballs_client.exe stereo_ovr_04.cpp 189
  // NL3D::CStereoOVR::CStereoOVR : OVR: LEFT DistortedViewport: x: 0, y: 0, w:
  // 640, h: 800 2014/08/04 19:54:25 DBG  a60 snowballs_client.exe
  // stereo_ovr_04.cpp 190 NL3D::CStereoOVR::CStereoOVR : OVR: LEFT
  // PixelsPerTanAngleAtCenter: x: 363.247864, y: 363.247864 2014/08/04 19:54:25
  // DBG  a60 snowballs_client.exe stereo_ovr_04.cpp 191
  // NL3D::CStereoOVR::CStereoOVR : OVR: LEFT ViewAdjust: x: 0.031800, y:
  // 0.000000, z: 0.000000 2014/08/04 19:55:46 DBG 2e18 snowballs_client.exe
  // stereo_ovr_04.cpp 192 NL3D::CStereoOVR::CStereoOVR : OVR: RIGHT
  // DistortedViewport: x: 640, y: 0, w: 640, h: 800 2014/08/04 19:55:46 DBG
  // 2e18 snowballs_client.exe stereo_ovr_04.cpp 193
  // NL3D::CStereoOVR::CStereoOVR : OVR: RIGHT PixelsPerTanAngleAtCenter: x:
  // 363.247864, y: 363.247864 2014/08/04 19:55:46 DBG 2e18 snowballs_client.exe
  // stereo_ovr_04.cpp 194 NL3D::CStereoOVR::CStereoOVR : OVR: RIGHT ViewAdjust:
  // x: -0.031868, y: 0.000000, z: 0.000000

  // find out the recommended render target size
  ovrSizei fovTextureSize[ovrEye_Count];
  fovTextureSize[ovrEye_Left] = ovrHmd_GetFovTextureSize(
      m_DevicePtr, ovrEye_Left, eyeRenderDesc[ovrEye_Left].Fov, 1.0f);
  fovTextureSize[ovrEye_Right] = ovrHmd_GetFovTextureSize(
      m_DevicePtr, ovrEye_Right, eyeRenderDesc[ovrEye_Right].Fov, 1.0f);
  m_RenderTargetWidth =
      fovTextureSize[ovrEye_Left].w + fovTextureSize[ovrEye_Right].w;
  m_RenderTargetHeight =
      max(fovTextureSize[ovrEye_Left].h, fovTextureSize[ovrEye_Right].h);
  nldebug("OVR: RenderTarget: w: %u, h: %u", m_RenderTargetWidth,
          m_RenderTargetHeight);

  // 2014/08/04 20:22:03 DBG 30e4 snowballs_client.exe stereo_ovr_04.cpp 213
  // NL3D::CStereoOVR::CStereoOVR : OVR: RenderTarget: w: 2414, h: 1870 // That
  ...
  for (uint eye = 0; eye < ovrEye_Count; ++eye) {
    ...
    nldebug("OVR: EyeViewport: x: %f, y: %f, w: %f, h: %f",
            m_EyeViewport[eye].getX(), m_EyeViewport[eye].getY(),
            m_EyeViewport[eye].getWidth(), m_EyeViewport[eye].getHeight());
    ovrRecti eyeViewport;
    eyeViewport.Pos.x =
        (eyeRenderDesc[eye].DistortedViewport.Pos.x * m_RenderTargetWidth) /
        m_DevicePtr->Resolution.w;
    eyeViewport.Pos.y =
        (eyeRenderDesc[eye].DistortedViewport.Pos.y * m_RenderTargetHeight) /
        m_DevicePtr->Resolution.h;
    eyeViewport.Size.w =
        (eyeRenderDesc[eye].DistortedViewport.Size.w * m_RenderTargetWidth) /
        m_DevicePtr->Resolution.w;
    eyeViewport.Size.h =
        (eyeRenderDesc[eye].DistortedViewport.Size.h * m_RenderTargetHeight) /
        m_DevicePtr->Resolution.h;
    ...
    float horizontalFullFovInRadians = 2.0f * atanf
    (combinedTanHalfFovHorizontal); float aspectRatio =
    combinedTanHalfFovHorizontal / combinedTanHalfFovVertical; float
    m_EyeHFov[NL_OVR_EYE_COUNT]; float m_EyeAR[NL_OVR_EYE_COUNT]; m_EyeHFov[eye]
    = horizontalFullFovInRadians; m_EyeAR[eye] = aspectRatio; nldebug("OVR:
    HFOV: %f, AR: %f", horizontalFullFovInRadians, aspectRatio);
    m_EyeFrustumBase[eye].initPerspective(m_EyeHFov[eye], m_EyeAR[eye], 1.0f,
    100.f); nldebug("OVR: FOV: Left: %f, Right: %f, Down: %f, Up: %f", // DOUBLE
    CHECK m_EyeFrustumBase[eye].Left, m_EyeFrustumBase[eye].Right,
    m_EyeFrustumBase[eye].Bottom, m_EyeFrustumBase[eye].Top);*/
    m_EyeFrustumBase[eye].init(
        -fov.LeftTan, // OVR provides positive values
        fov.RightTan, // DEBUG: If renders shifted left and right, swap left and
                      // right
        -fov.DownTan,
    ...
    nldebug("OVR: FOV: Left: %f, Right: %f, Down: %f, Up: %f",
            m_EyeFrustumBase[eye].Left, m_EyeFrustumBase[eye].Right,
            m_EyeFrustumBase[eye].Bottom, m_EyeFrustumBase[eye].Top);

    // get distortion mesh
    ovrDistortionMesh meshData;
    ovrHmd_CreateDistortionMesh(
        m_DevicePtr, (ovrEyeType)eye, fov,
        ovrDistortionCap_Chromatic /*| ovrDistortionCap_TimeWarp*/ |
            ovrDistortionCap_Vignette, // I believe the timewarp gimmick screws
                                       // with parallax
        &meshData);
    ovrVector2f uvScaleOffset[2];

    // get parameters for programs
    ovrHmd_GetRenderScaleAndOffset(fov, fovTextureSize[eye], eyeViewport,
                                   (ovrVector2f *)uvScaleOffset);
    m_EyeUVScaleOffset[eye][0] =
        NLMISC::CVector2f(uvScaleOffset[0].x, uvScaleOffset[0].y);
    m_EyeUVScaleOffset[eye][1] =
        NLMISC::CVector2f(uvScaleOffset[1].x, uvScaleOffset[1].y);
    ...
    m_VB[eye].setNumVertices(meshData.VertexCount);
    {
      ...
      for (uint i = 0; i < meshData.VertexCount; ++i) {
        ...
        vba.setVertexCoord(i, (ov.ScreenPosNDC.x + 1.0f) * 0.5f,
                           (ov.ScreenPosNDC.y + 1.0f) * 0.5f, 0.5f);
    ...
    m_IB[eye].setNumIndexes(meshData.IndexCount);
    {
      ...
      for (uint i = 0; i + 2 < meshData.IndexCount; i += 3) {
        ...
        iba.setTri(i, meshData.pIndexData[i], meshData.pIndexData[i + 1],
                   meshData.pIndexData[i + 2]);
      }
    }
  ...
  // 2014/08/04 20:22:03 DBG 30e4 snowballs_client.exe stereo_ovr_04.cpp 222
  // NL3D::CStereoOVR::CStereoOVR : OVR: EyeViewport: x: 0.000000, y: 0.000000,
  // w: 0.500000, h: 1.000000 2014/08/04 22:28:39 DBG 3040 snowballs_client.exe
  // stereo_ovr_04.cpp 235 NL3D::CStereoOVR::CStereoOVR : OVR: HFOV: 2.339905,
  // AR: 0.916641 2014/08/04 20:22:03 DBG 30e4 snowballs_client.exe
  // stereo_ovr_04.cpp 222 NL3D::CStereoOVR::CStereoOVR : OVR: EyeViewport: x:
  // 0.500000, y: 0.000000, w: 0.500000, h: 1.000000 2014/08/04 22:28:39 DBG
  // 3040 snowballs_client.exe stereo_ovr_04.cpp 235
  // NL3D::CStereoOVR::CStereoOVR : OVR: HFOV: 2.339905, AR: 0.916641
...
bool CStereoOVR::attachToDisplay() {
  ...
  if ((m_DevicePtr->HmdCaps & ovrHmdCap_ExtendDesktop) !=
      ovrHmdCap_ExtendDesktop) {
    nldebug("OVR: Direct Rift");
    CDriverUser *dru = static_cast<CDriverUser *>(m_Driver);
    IDriver *drv = dru->getDriver();
    m_AttachedDisplay = ovrHmd_AttachToWindow(
        m_DevicePtr, (void *)drv->getDisplay(), NULL, NULL);
    if (!m_AttachedDisplay)
...
void CStereoOVR::initCamera(uint cid, const NL3D::UCamera *camera) {
  ...
  m_LeftFrustum[cid].init(
      ...
      camera->getFrustum().Near, camera->getFrustum().Far, true);
  ...
  m_ClippingFrustum[cid].init(min(m_EyeFrustumBase[ovrEye_Left].Left,
                                  m_EyeFrustumBase[ovrEye_Right].Left) *
                                  camera->getFrustum().Near,
                              max(m_EyeFrustumBase[ovrEye_Left].Right,
                                  m_EyeFrustumBase[ovrEye_Right].Right) *
                                  camera->getFrustum().Near,
                              min(m_EyeFrustumBase[ovrEye_Left].Bottom,
                                  m_EyeFrustumBase[ovrEye_Right].Bottom) *
                                  camera->getFrustum().Near,
                              max(m_EyeFrustumBase[ovrEye_Left].Top,
                                  m_EyeFrustumBase[ovrEye_Right].Top) *
                                  camera->getFrustum().Near,
                              camera->getFrustum().Near,
                              camera->getFrustum().Far, true);
}
...
const NL3D::CViewport &CStereoOVR::getCurrentViewport() const {
  if (m_Stage == 2)
    return m_RegularViewport;
  else if (m_Stage % 2)
    return m_EyeViewport[ovrEye_Left];
  else
    return m_EyeViewport[ovrEye_Right];
}

const NL3D::CFrustum &CStereoOVR::getCurrentFrustum(uint cid) const {
  if (m_Stage == 2)
    return m_OriginalFrustum[cid];
  else if (m_Stage % 2)
    return m_LeftFrustum[cid];
  else
    return m_RightFrustum[cid];
}
...
void CStereoOVR::getCurrentFrustum(uint cid, NL3D::UCamera *camera) const {
  if (m_Stage == 2)
    ...
    camera->setFrustum(m_LeftFrustum[cid]);
  else
    camera->setFrustum(m_RightFrustum[cid]);
}
...
void CStereoOVR::getCurrentMatrix(uint cid, NL3D::UCamera *camera) const {
  ...
  if (m_Stage == 2) {
    ...
    translate.translate(
        CVector(m_EyeViewAdjustX[ovrEye_Left] * m_Scale, 0.f, 0.f)); // ok
  else
    translate.translate(
        CVector(m_EyeViewAdjustX[ovrEye_Right] * m_Scale, 0.f, 0.f)); // ok
...
bool CStereoOVR::beginRenderTarget() {
  ...
  if (m_Driver && m_Stage == 2 &&
      (m_Driver->getPolygonMode() == UDriver::Filled)) {
    nlassert(!m_GUITexture);
    uint32 width, height;
    m_Driver->getWindowSize(width, height);
    m_GUITexture = m_Driver->getRenderTargetManager().getRenderTarget(
        width, height, true, UTexture::RGBA8888);
  ...
  if (m_Driver && m_Stage == 3 &&
      (m_Driver->getPolygonMode() == UDriver::Filled)) {
    nlassert(!m_SceneTexture);
    m_SceneTexture = m_Driver->getRenderTargetManager().getRenderTarget(
    ...
    // m_Driver->beginDefaultRenderTarget(m_RenderTargetWidth,
    // m_RenderTargetHeight); // DEBUG return true;
  }
...
void CStereoOVR::renderGUI() {
  ...
  /*
      {
          NLMISC::CLine line(NLMISC::CVector(0, 5, 2), NLMISC::CVector(0, 5,
     3));
...

      {
          ...
          mat.setColor(NLMISC::CRGBA::Red);
          line = NLMISC::CLine(NLMISC::CVector(0, 3, -3), NLMISC::CVector(0, 3,
     3)); // YPos m_Driver->drawLine(line, mat);

          mat.setColor(NLMISC::CRGBA::Green);
          line = NLMISC::CLine(NLMISC::CVector(3, 0, -3), NLMISC::CVector(3, 0,
     3)); // XPos m_Driver->drawLine(line, mat);

          mat.setColor(NLMISC::CRGBA::Magenta);
          line = NLMISC::CLine(NLMISC::CVector(0, -3, -3), NLMISC::CVector(0,
     -3, 3)); // YNeg m_Driver->drawLine(line, mat);

          mat.setColor(NLMISC::CRGBA::Cyan);
          line = NLMISC::CLine(NLMISC::CVector(-3, 0, -3), NLMISC::CVector(-3,
     0, 3)); // XNeg m_Driver->drawLine(line, mat);

          mat.setColor(NLMISC::CRGBA::Blue);
          line = NLMISC::CLine(NLMISC::CVector(0, -3, 3), NLMISC::CVector(0, 3,
     3)); // ZPos m_Driver->drawLine(line, mat);

          mat.setColor(NLMISC::CRGBA::Blue);
          line = NLMISC::CLine(NLMISC::CVector(0, -3, -3), NLMISC::CVector(0, 3,
     -3)); // ZNeg m_Driver->drawLine(line, mat);
      ...
      */
  {
    ...
    driver->renderTriangles(*umat.getObjectPtr(), 0,
                            nbQuads * 2); // renderRawQuads(umat, 0, 128);
...
bool CStereoOVR::endRenderTarget() {
  ...
  if (m_Driver && m_Stage == 6 &&
    ...
    // must clear everything to black (can we get a mesh to only handle the
    // parts outside of the distortion mesh?)
  ...
  /*if (m_Driver && m_Stage == 6 && (m_Driver->getPolygonMode() ==
  UDriver::Filled)) // set to 4 to turn off distortion of 2d gui
  {
      nlassert(m_SceneTexture);
      ...
      uint32 width, height;
      m_Driver->getWindowSize(width, height);
      NL3D::IDriver *drvInternal = (static_cast<CDriverUser
  *>(m_Driver))->getDriver(); NL3D::CMaterial *barrelMat =
  m_BarrelMat.getObjectPtr(); barrelMat->setTexture(0,
  m_SceneTexture->getITexture());
      ...
      float lensViewportShift = 4.0f * lensShift /
  m_DevicePtr->HMDInfo.HScreenSize;
...
NLMISC::CQuat CStereoOVR::getOrientation() const {
  ...
  ovrTrackingState ts = ovrHmd_GetTrackingState(
      m_DevicePtr, ovr_GetTimeInSeconds()); // TODO: Predict forward
  if (ts.StatusFlags & ovrStatus_OrientationTracked) {
    ...
    matr.rotateX(NLMISC::Pi *
                 0.5f); // fix this properly... :) (note: removing this allows
                        // you to use rift while lying down)
...
/// Get GUI shift
void CStereoOVR::getInterface2DShift(uint cid, float &x, float &y,
                                     float distance) const {}

void CStereoOVR::setEyePosition(const NLMISC::CVector &v) { m_EyePosition = v; }
...
bool CStereoOVR::isDeviceCreated() { return m_DevicePtr != NULL; }

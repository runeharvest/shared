...
void CPSQuad::initVertexBuffers() {
  ...
  for (uint k = 0; k < 32; ++k) {
    ...
    if (vb) // valid vb ?
    {
      ...
      /// setup vertex format
      if (k & (uint)VBCol)
        vf |= CVertexBuffer::PrimaryColorFlag;
      if (k & (uint)VBTex || k & (uint)VBTexAnimated)
        vf |= CVertexBuffer::TexCoord0Flag;
      if (k & (uint)VBTex2 || k & (uint)VBTex2Animated)
        vf |= CVertexBuffer::TexCoord1Flag;
  ...
  for (uint k = 0; k < 32; ++k)
  {
      ...
      if (vb) // valid vb ?
      {
          ...
          if (k & (uint) VBTex  || k & (uint) VBTexAnimated) vf |=
  CVertexBuffer::TexCoord0Flag; if (k & (uint) VBTex2 || k & (uint)
  VBTex2Animated) vf |= CVertexBuffer::TexCoord1Flag; vb->setVertexFormat(vf);
          vb->setNumVertices(quadBufSize << 2);
...
CVertexBuffer &CPSQuad::getNeededVB(IDriver &drv) {
  ...
  if (flags & VBTex) {
    /// check if multitexturing is enabled, and which texture are enabled and /
    /// or animated
    if (CPSMultiTexturedParticle::isMultiTextureEnabled()) {
      if (isAlternateTextureUsed(drv) && _AlternateTexture2) {
        if ((flags & VBTex) &&
            (_TexScrollAlternate[0].x != 0 || _TexScrollAlternate[0].y != 0))
          flags |= VBTexAnimated;
        if (_AlternateOp != Decal &&
            (_TexScrollAlternate[1].x != 0 || _TexScrollAlternate[1].y != 0)) {
          flags |= VBTex2 | VBTex2Animated;
        } else {
          flags |= VBTex2;
        }
      } else {
        if ((flags & VBTex) && (_TexScroll[0].x != 0 || _TexScroll[0].y != 0))
          flags |= VBTexAnimated;
        if (_Texture2) {
          if (_MainOp != Decal &&
              (_TexScroll[1].x != 0 || _TexScroll[1].y != 0)) {
            flags |= VBTex2 | VBTex2Animated;
          } else {
            flags |= VBTex2;
          }
...
///==================================================================================
CPSQuad::~CPSQuad(){NL_PS_FUNC(CPSQuad_CPSQuadDtor)}
...
bool CPSQuad::completeBBox(NLMISC::CAABBox &box) const {
  ...
  if (!_SizeScheme) {
    ...
    CPSUtil::addRadiusToAABBox(box,
                               std::max(fabsf(_SizeScheme->getMaxValue()),
                                        fabsf(_SizeScheme->getMinValue())));
  }
  return true;
}
...
static void FillQuadCoords(uint8 *dest, uint stride,
                           const NLMISC::CVector2f &speed, float time,
                           uint num) {
...
static void FillQuadCoordsLocalTime(uint8 *dest, uint stride,
                                    const NLMISC::CVector2f &speed,
                                    CPSLocated &srcLoc, uint startIndex,
                                    uint num, uint32 srcStep) {
...
//==============================================================
void CPSQuad::updateVbColNUVForRender(CVertexBuffer &vb, uint32 startIndex,
                                      uint32 size, uint32 srcStep,
                                      IDriver &drv) {
  ...
  if (_ColorScheme) {
    ...
    _ColorScheme->make4(_Owner, startIndex, vba.getColorPointer(),
                        vb.getVertexSize(), size, srcStep);
  }

  if (_TexGroup) // if it has a constant texture we are sure it has been
                 // setupped before...
  {
    sint32 textureIndex[quadBufSize];
    const uint32 stride = vb.getVertexSize(), stride2 = stride << 1,
                 stride3 = stride2 + stride, stride4 = stride2 << 1;
    ...
    if (_TextureIndexScheme) {
      currIndex = (sint32 *)_TextureIndexScheme->make(
          _Owner, startIndex, textureIndex, sizeof(sint32), size, true,
          srcStep);
      currIndexIncr = 1;
    } else {
    ...
    while (left--) {
      ...
      const CTextureGrouped::TFourUV &uvGroup =
          _TexGroup->getUVQuad((uint32)*currIndex);
  ...
  if (CPSMultiTexturedParticle::isMultiTextureEnabled() &&
    ...
    {
      if (!isAlternateTextureUsed(drv) || !isAlternateTexEnabled() ||
          !_AlternateTexture2) {
        if (_Tex && (_TexScroll[0].x != 0 || _TexScroll[0].y != 0)) {
          // animation of texture 1 with main speed
          if (!getUseLocalDate()) {
            FillQuadCoords((uint8 *)vba.getTexCoordPointer(0, 0),
                           vb.getVertexSize(), _TexScroll[0], date, size);
          } else {
            FillQuadCoordsLocalTime((uint8 *)vba.getTexCoordPointer(0, 0),
                                    vb.getVertexSize(), _TexScroll[0], *_Owner,
                                    startIndex, size, srcStep);
          }
        }
      } else {
        if (_Tex &&
          ...
          if (!getUseLocalDateAlt()) {
            FillQuadCoords((uint8 *)vba.getTexCoordPointer(0, 0),
                           vb.getVertexSize(), _TexScrollAlternate[0], date,
                           size);
          } else {
            FillQuadCoordsLocalTime((uint8 *)vba.getTexCoordPointer(0, 0),
                                    vb.getVertexSize(), _TexScrollAlternate[0],
                                    *_Owner, startIndex, size, srcStep);
          }
        }
      }
    }
    ...
    if (!isAlternateTextureUsed(drv)) {
      if (_Texture2 && (_TexScroll[1].x != 0 || _TexScroll[1].y != 0)) {
        ...
        if (!getUseLocalDate()) {
          FillQuadCoords((uint8 *)vba.getTexCoordPointer(0, 1),
                         vb.getVertexSize(), _TexScroll[1], date, size);
        } else {
          FillQuadCoordsLocalTime((uint8 *)vba.getTexCoordPointer(0, 1),
                                  vb.getVertexSize(), _TexScroll[1], *_Owner,
                                  startIndex, size, srcStep);
        }
      }
    } else {
      if (_AlternateTexture2 && isAlternateTexEnabled() &&
          (_TexScrollAlternate[1].x != 0 || _TexScrollAlternate[1].y != 0)) {
        // animation of texture 2 with alternate speed
        if (!getUseLocalDateAlt()) {
          FillQuadCoords((uint8 *)vba.getTexCoordPointer(0, 1),
                         vb.getVertexSize(), _TexScrollAlternate[1], date,
                         size);
        } else {
          FillQuadCoordsLocalTime((uint8 *)vba.getTexCoordPointer(0, 1),
                                  vb.getVertexSize(), _TexScrollAlternate[1],
                                  *_Owner, startIndex, size, srcStep);
        }
      }
    }
  ...
  if (isMultiTextureEnabled()) {
    ...
    if (_ColorScheme != NULL &&
        (ps.getColorAttenuationScheme() != NULL || ps.isUserColorUsed() ||
         ps.getForceGlobalColorLightingFlag() || usesGlobalColorLighting())) {
      if (ps.getForceGlobalColorLightingFlag() || usesGlobalColorLighting()) {
        CPSMaterial::forceModulateConstantColor(true,
                                                ps.getGlobalColorLighted());
      } else {
...
// *****************************************************************************************************
void CPSQuad::enumTexs(std::vector<NLMISC::CSmartPtr<ITexture>> &dest,
                       IDriver &drv) {
...
void CPSQuad::updateTexWrapMode(IDriver &drv) {
  ...
  if (isMultiTextureEnabled()) {
    if (_Tex) {
      if (isAlternateTextureUsed(drv)) {
        _Tex->setWrapS(_TexScroll[0].x == 0 ? ITexture::Clamp
                                            : ITexture::Repeat);
        _Tex->setWrapT(_TexScroll[0].y == 0 ? ITexture::Clamp
                                            : ITexture::Repeat);
      } else {
        _Tex->setWrapS(_TexScrollAlternate[0].x == 0 ? ITexture::Clamp
                                                     : ITexture::Repeat);
        _Tex->setWrapT(_TexScrollAlternate[0].y == 0 ? ITexture::Clamp
                                                     : ITexture::Repeat);
      }
    }
    ITexture *tex2 =
        isAlternateTextureUsed(drv) ? getTexture2Alternate() : getTexture2();
    if (tex2) {
    ...
    // nb for grouped texture we assume that no mipmapping is used, and that the
    // border is black (or white for modulate material mode)
  }
}

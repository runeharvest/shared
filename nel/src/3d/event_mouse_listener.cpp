...
CEvent3dMouseListener::CEvent3dMouseListener()
    : _CurrentModelRotationAxis(zAxis), _XModelTranslateEnabled(true),
      _YModelTranslateEnabled(true), _ZModelTranslateEnabled(true) {
  _Matrix.identity();
...
void CEvent3dMouseListener::enableModelTranslationAxis(TAxis axis,
                                                       bool enabled) {
  switch (axis) {
  case xAxis:
    _XModelTranslateEnabled = enabled;
    break;
  case yAxis:
    _YModelTranslateEnabled = enabled;
    break;
  case zAxis:
    _ZModelTranslateEnabled = enabled;
    break;
  }
}
...
bool CEvent3dMouseListener::isModelTranslationEnabled(TAxis axis) {
  switch (axis) {
  case xAxis:
    return _XModelTranslateEnabled;
    break;
  case yAxis:
    return _YModelTranslateEnabled;
    break;
  case zAxis:
    return _ZModelTranslateEnabled;
    break;
  default:
    return false;
    break;
...
void CEvent3dMouseListener::truncateVect(CVector &v) {
  if (!_XModelTranslateEnabled)
    v.x = 0.f;
  if (!_YModelTranslateEnabled)
    v.y = 0.f;
  if (!_ZModelTranslateEnabled)
    v.z = 0.f;
}
  ...
  if (event == EventMouseMoveId) {
    ...
    if (bRotate) {
      if (!_EnableModelMatrixEdition) {
        ...
        case xAxis:
          r = CQuat(CAngleAxis(_ModelMatrix.getI(),
                               (float)Pi * 2.f * (_X - mouseEvent->X)));
          break;
        case yAxis:
          r = CQuat(CAngleAxis(_ModelMatrix.getJ(),
                               (float)Pi * 2.f * (_X - mouseEvent->X)));
          break;
        case zAxis:
          r = CQuat(CAngleAxis(_ModelMatrix.getK(),
                               (float)Pi * 2.f * (_X - mouseEvent->X)));
          break;
        };
    ...
    if (bTranslateXY || bTranslateZ || bZoom) {
      ...
      _Viewport.getRayWithPoint(mouseEvent->X, mouseEvent->Y, pos, dir,
                                localViewMatrix, _Frustrum);
      ...
      if (bTranslateXY) {
        ...
        if (!_EnableModelMatrixEdition) {
          _Matrix.setPos(_Matrix.getPos() +
                         _Matrix.getK() * (vect.x + vect.y + vect.z));
        } else {
        ...
        if (!_EnableModelMatrixEdition) {
          ...
          direc.normalize();
          _ModelMatrix.setPos(_ModelMatrix.getPos() +
                              direc * (vect.x + vect.y + vect.z));
        }
      }
    }
    ...
    if (!_EnableModelMatrixEdition) {
      _Matrix.setPos(_Matrix.getPos() +
                     direc * (mouseEvent->Direction ? 0.1f : -0.1f));
    } else {
...
void CEvent3dMouseListener::setModelMatrixTransformMove(
    const NLMISC::CMatrix &transModelMove) {
  _ModelMatrixTransformMove = transModelMove;
}

void CEvent3dMouseListener::getModelMatrixTransformMove(
    NLMISC::CMatrix &transModelMove) const {

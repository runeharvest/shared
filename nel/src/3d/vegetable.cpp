...
// ***************************************************************************
// Generate random value, but seed is spacial. Take a high frequency, so it gets
// more the aspect of random.
...
void CVegetable::setAngleCeiling(float cosAngleMax) {
  ...
  // We must be at densityFactor==1, when cosAngle==-1, keeping the same
  // formula.
...
// ***************************************************************************
void CVegetable::generateGroupEx(float nbInst, const CVector &posInWorld,
  ...
  // compute angleFactor density. Use a quadratic, because
  // f'(_CosAngleMiddle)==0.
  ...
  // Now, 0<=nbInst<+oo. If we have 0.1, it means that we have 10% chance to
  // spawn an instance. So add a "random" value (with help of a noise with High
  // frequency) if nbInst==0, we should never have any instance (which may arise
  // if evalOneLevelRandom()==1). hence the 0.99f* which ensure that we do
  // nbInst+= [0..1[.
...
// ***************************************************************************
void CVegetable::generateGroup(const CVector &posInWorld,
...
// ***************************************************************************
void CVegetable::generateGroupBiLinear(
    const CVector &posInWorld, const CVector posInWorldBorder[4],
    const CVector &surfaceNormal, float area, uint vegetSeed,
    std::vector<CVector2f> &instances) const {
  sint i;
  const float evenDistribFact =
      12.25f; // an arbitrary value to have a higher frequency for random.
  ...
  // ==================
  generateGroupEx(nbInstAverage, posInWorld, surfaceNormal, vegetSeed,
                  instances);
  ...
  float OOEdgeDCY = 0.0;
  if (!middleX)
    OOEdgeDCX = 1.0f / edgeDensityCenterX;
  if (!middleY)
    OOEdgeDCY = 1.0f / edgeDensityCenterY;
  // for all instances
  for (i = 0; i < (sint)instances.size(); i++) {
    ...
    if (middleX) {
      ...
      if (densX < edgeDensityCenterX) {
        ...
        // (densX * OOEdgeDCX) E [0..1[. The more it is near 0, the more is has
        // chance to be swapped.
    ...
    if (middleY) {
      ...
      if (densY < edgeDensityCenterY) {
        ...
        // (densY * OOEdgeDCY) E [0..1[. The more it is near 0, the more is has
        // chance to be swapped.
        rdSwap += RandomGenerator.evalOneLevelRandom(randSeed);
        if (rdSwap < 1)
...
// ***************************************************************************
void CVegetable::reserveIgAddInstances(
    CVegetableInstanceGroupReserve &vegetIgReserve,
    TVegetableWater vegetWaterState, uint numInstances) const {
  nlassert(_Manager);

  if (_VegetableShape)
    _Manager->reserveIgAddInstances(
        vegetIgReserve, _VegetableShape,
        (CVegetableManager::TVegetableWater)vegetWaterState, numInstances);
}

// ***************************************************************************
void CVegetable::generateInstance(CVegetableInstanceGroup *ig,
  ...
  // ===============
  if (_VegetableShape) {
    _Manager->addInstance(
        ig, _VegetableShape, finalMatrix, ambient, diffuse, bendFactor,

...
// ***************************************************************************
void CAnimationOptimizer::setQuaternionThreshold(double lowPrecThre,
                                                 double highPrecThre) {
...
// ***************************************************************************
void CAnimationOptimizer::setVectorThreshold(double lowPrecThre,
                                             double highPrecThre) {
  nlassert(lowPrecThre >= 0);
  nlassert(highPrecThre >= 0);
...
// ***************************************************************************
void CAnimationOptimizer::optimize(const CAnimation &animIn,
                                   CAnimation &animOut) {
  // reset animOut
  contReset(animOut);
  ...
  // Parse all SSS shapes of the animation (important for preload of those
  // shapes)
...
bool CAnimationOptimizer::isTrackOptimisable(const ITrack *trackIn) {
  ...
  // If the track is a Linear, Bezier or a TCB track, suppose we can optimize
  // it. Constant may not be interressant....
...
ITrack *CAnimationOptimizer::optimizeTrack(const ITrack *trackIn) {
  ...
  if (dynamic_cast<const CAnimatedValueQuat *>(&valueType)) {
    ...
    // check if the sampled track can be reduced to a TrackDefaultQuat. Test
    // _QuatKeyList.
  ...
  //================
    ...
    // check if the sampled track can be reduced to a TrackDefaultVector. Test
    // _VectorKeyList.
...
// ***************************************************************************
void CAnimationOptimizer::sampleQuatTrack(const ITrack *trackIn,
...
bool CAnimationOptimizer::testConstantQuatTrack() {
  ...
  // Get the first sample as the reference quaternion, and test others from this
  // one.
...
void CAnimationOptimizer::optimizeQuatTrack() {
  ...
  for (uint i = 1; i < numSamples - 1; i++) {
    ...
    // If the next quaternion or the current quaternion are not on same
    // hemisphere than ref, abort.
    else if (CQuatD::dotProduct(quatCur, quatRef) < 0 ||
             CQuatD::dotProduct(quatNext, quatRef) < 0) {
      mustAdd = true;
    }
    // else, test interpolation
    else {
      // If the 3 quats are nearly equals, it is ok (avoid interpolation)
      if (nearlySameQuaternion(quatRef, quatCur) &&
          nearlySameQuaternion(quatRef, quatNext))
        mustAdd = false;
      else {
        // interpolate.
...
// ***************************************************************************
bool CAnimationOptimizer::nearlySameQuaternion(const CQuatD &quat0,
                                               const CQuatD &quat1) {
  // true if exactly same, or exactly inverse
  if (quat0 == quat1 || quat0 == -quat1)
    return true;

  // Else compute the rotation to go from qRef to q. Use double for better
  // presion.
  CQuatD quatDif;
  quatDif = quat1 * quat0.conjugate();
  // inverse the quaternion if necessary. ie make closest to the identity
  // quaternion.
  if (quatDif.w < 0)
    quatDif = -quatDif;
...
// ***************************************************************************
void CAnimationOptimizer::sampleVectorTrack(const ITrack *trackIn,
                                            float beginTime, float endTime,
                                            uint numSamples) {
  // resize tmp samples
  _TimeList.resize(numSamples);
...
bool CAnimationOptimizer::testConstantVectorTrack() {
  ...
  // Get the first sample as the reference Vectorer, and test others from this
  // one.
...
void CAnimationOptimizer::optimizeVectorTrack() {
  ...
  for (uint i = 1; i < numSamples - 1; i++) {
    ...
    // else, test interpolation
      ...
      if (nearlySameVector(vectorRef, vectorCur) &&
          nearlySameVector(vectorRef, vectorNext))
        mustAdd = false;
      else {
        // interpolate.
...
// ***************************************************************************
bool CAnimationOptimizer::nearlySameVector(const CVectorD &v0,
                                           const CVectorD &v1) {

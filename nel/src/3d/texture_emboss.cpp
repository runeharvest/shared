...
CTextureEmboss::CTextureEmboss()
    : _Ambient(CRGBA::Black), _Diffuse(CRGBA::White), _DisableSharing(false),
      _SlopeFactor(1.f) {
...
void CTextureEmboss::doGenerate(bool /* async */) {
  ...
  for (uint y = 0; y < height; ++y) {
    for (uint x = 0; x < width; ++x) {
      ...
      const CRGBA *pixelRight =
          (x != width - 1) ? (src + 1) : (src + 1 - width);
      const CRGBA *pixelLeft = (x != 0) ? (src - 1) : (src + width - 1);
      const CRGBA *pixelTop =
          (y != 0) ? (src - width) : (src + width * (height - 1));
      const CRGBA *pixelBottom =
          (y != (height - 1)) ? (src + width) : (src - width * (height - 1));

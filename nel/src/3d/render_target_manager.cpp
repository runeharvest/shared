...
NL3D::CTextureUser *
CRenderTargetManager::getRenderTarget(uint width, uint height, bool mode2D,
                                      UTexture::TUploadFormat format) {
  // Find or create a render target, short loop so no real optimization
  for (std::vector<CRenderTargetDescInt *>::iterator
           it(m_RenderTargets.begin()),
       end(m_RenderTargets.end());
       it != end; ++it) {
    CRenderTargetDescInt *desc = *it;
    if (!desc->InUse && desc->Width == width && desc->Height == height &&
        desc->Mode2D == mode2D && desc->Format == format) {
      desc->InUse = true;
      desc->Used = true;
  ...
  desc->TextureInterface->setUploadFormat(
      (ITexture::TUploadFormat)(uint32)format);
  desc->TextureInterface->resize(width, height);
  desc->TextureInterface->setFilterMode(ITexture::Linear,
  ...
  nlassert(!drvInternal->isTextureRectangle(
      desc->TextureInterface)); // Not allowed, we only support NPOT for render
                                // targets now.
  desc->Width = width;
  desc->Height = height;
  desc->Mode2D = mode2D;
...
void CRenderTargetManager::recycleRenderTarget(
    NL3D::CTextureUser *renderTarget) {
  for (std::vector<CRenderTargetDescInt *>::iterator
           it(m_RenderTargets.begin()),
       end(m_RenderTargets.end());
       it != end; ++it) {
...
void CRenderTargetManager::cleanup() {
  for (sint i = 0; i < (sint)m_RenderTargets.size(); ++i) {
    ...
    nlassert(!desc->InUse); // Assert for debugging, to not allow textures being
                            // carried over between frames. Optional assert
    if (!desc->InUse) {
      if (!desc->Used) {
        // No longer in use
        nldebug("3D: Release render target (%u x %u)", desc->Width,
                desc->Height);

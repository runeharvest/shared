...
CChannelMixer::~CChannelMixer() { resetChannels(); }
...
// ***************************************************************************
void CChannelMixer::evalSingleChannel(CChannel &chan, uint numActive,
  ...
  for (uint a = 0; a < numActive; a++) {
    ...
    if (blend != 0.0f) {
      ...
      const IAnimatedValue &trackResult =
          ((ITrack *)chan._Tracks[slot])
              ->eval(_SlotArray[slot]._Time, TempAnimatedValueBlock);

      // First track to be eval ?
      if (bFirst) {
        // If channel is a Quaternion animated Value, must store the first Quat.
        if (chan._IsQuat) {
          CAnimatedValueBlendable<NLMISC::CQuat> &quatValue =
              (CAnimatedValueBlendable<NLMISC::CQuat> &)trackResult;
        ...
        // If channel is a Quaternion animated Value, must makeClosest the ith
        // result of the track, from firstQuat.
        if (chan._IsQuat) {
          CAnimatedValueBlendable<NLMISC::CQuat> &quatValue =
              (CAnimatedValueBlendable<NLMISC::CQuat> &)trackResult;
...
void CChannelMixer::eval(bool detail, uint64 evalDetailDate) {
  ...
  // Setup an array of animation that are not empty and stay. HTimer: 0.0%
  // (because CLod skeletons not parsed here)
  uint numActive = 0;
  uint activeSlot[NumAnimationSlot];
  ...
  if (numActive == 1) {
    ...
    for (; numChans > 0; numChans--, channelArrayPtr++) {
      ...
      if (chan._Weights[slot] != 0.0f) {
        ...
        chan._Value->affect(((ITrack *)chan._Tracks[slot])
                                ->eval(slotTime, TempAnimatedValueBlock));
...
sint CChannelMixer::addChannel(const string &channelName,
                               IAnimatable *animatable, IAnimatedValue *value,
                               ITrack *defaultValue, uint32 valueId,
  ...
  // Get the channel Id having the same name than the tracks in this animation
  // set.
  ...
  if (iDInAnimationSet != CAnimationSet::NotFound) {
    ...
    entry._IsQuat = (typeid(*(entry._Value)) ==
                     typeid(CAnimatedValueBlendable<NLMISC::CQuat>)) != 0;
    ...
    entry._Value->affect(
        ((ITrack *)(entry._DefaultTracks))->eval(0, TempAnimatedValueBlock));
...
void CChannelMixer::applySkeletonWeight(uint slot, uint skeleton, bool invert) {
  ...
  // Something to change ?
  if ((pSkeleton != _SlotArray[slot]._SkeletonWeight) ||
      (invert != _SlotArray[slot]._InvertedSkeletonWeight)) {
    ...
    for (uint n = 0; n < sizeSkel; n++) {
      ...
      // Get the channel Id having the same name than the tracks in this
      // animation set.
      ...
      if (channelId != CAnimationSet::NotFound) {
        ...
        // Set the weight of this channel for this slot (only if channel
        // setuped!!)
...
void CChannelMixer::resetSkeletonWeight(uint slot) {
  ...
  if (_SlotArray[slot]._SkeletonWeight != NULL) {
    ...
    for (itChannel = _Channels.begin(); itChannel != _Channels.end();
         itChannel++) {
      // Reset
  ...
  for (itChannel = _Channels.begin(); itChannel != _Channels.end();
    ...
    for (s = 0; s < numAdd; s++) {
      ...
      if (_AnimationSet->isAnimHeaderOptimized())
        ...
        iDTrack =
            _SlotArray[addSlot[s]]._Animation->getIdTrackByChannelId(channelId);
      else
        // get by name
        iDTrack = _SlotArray[addSlot[s]]._Animation->getIdTrackByName(
            channel._ChannelName);

      // If this track exist
      if (iDTrack != CAnimation::NotFound) {
        // Set the track
        channel._Tracks[addSlot[s]] =
            _SlotArray[addSlot[s]]._Animation->getTrack(iDTrack);
      ...
      if (channel._InTheList) {
        ...
        if (!add) {
          ...
          if (channel._Object) {
            ...
            channel._Value->affect(((ITrack *)(channel._DefaultTracks))
...
// ***************************************************************************
void CChannelMixer::resetEvalDetailDate() { _LastEvalDetailDate = -1; }

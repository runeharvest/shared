...
// ***************************************************************************
void CAnimatedMaterial::setMaterial(CMaterial *pMat) { _Material = pMat; }
...
void CAnimatedMaterial::update() {
  if (isTouched(OwnerBit) && _Material != NULL /*&& _Material->isLighted()*/) {
    ...
    if (_Material->isLighted()) {
      _Material->setLighting(true, _Emissive.Value, _Ambient.Value, diff,
    ...
    for (uint k = 0; k < IDRV_MAT_MAXTEXTURES; ++k) {
      if (_Material->isUserTexMatEnabled(k)) {
        ...
        CMatrix convMat; // exported v are already inverted (todo : optim this
                         // if needed, this matrix shouldn't be necessary)
        ...
        CMatrix SR;
        SR.setRot(CVector(texMatAV._UScale.Value * fCos,
                          texMatAV._VScale.Value * fSin, 0.f),
                  CVector(-texMatAV._UScale.Value * fSin,
                          texMatAV._VScale.Value * fCos, 0.f),
                  CVector::K);
        CVector half(0.5f, 0.5f, 0.f);
        SR.setPos(SR.mulVector(-half - CVector(texMatAV._UTrans.Value,
                                               texMatAV._VTrans.Value, 0.f)) +
                  half);
...
IAnimatedValue *CAnimatedMaterial::getValue(uint valueId) {
  ...
  default: // this may be a texture animated value...
    if (valueId >= TextureMatValues && valueId < AnimValueLast) {
      ...
      switch (argID) {
      case 0:
        return &_TexAnimatedMatValues[texNum]._UTrans;
      case 1:
        return &_TexAnimatedMatValues[texNum]._VTrans;
      case 2:
        return &_TexAnimatedMatValues[texNum]._UScale;
      case 3:
        return &_TexAnimatedMatValues[texNum]._VScale;
      case 4:
        return &_TexAnimatedMatValues[texNum]._WRot;
      }
    }
    break;
  };
...
const char *CAnimatedMaterial::getValueName(uint valueId) const {
  ...
  default: // this may be a texture animated value...
    if (valueId >= TextureMatValues && valueId < AnimValueLast) {
      ...
      switch (argID) {
      case 0:
        return getTexMatUTransName(texNum);
      case 1:
        return getTexMatVTransName(texNum);
      case 2:
        return getTexMatUScaleName(texNum);
      case 3:
        return getTexMatVScaleName(texNum);
      case 4:
        return getTexMatWRotName(texNum);
      }
    }
    break;
  };
...
ITrack *CAnimatedMaterial::getDefaultTrack(uint valueId) {
  ...
  switch (valueId) {
  case AmbientValue:
    return &_MaterialBase->DefaultAmbient;
  case DiffuseValue:
    return &_MaterialBase->DefaultDiffuse;
  case SpecularValue:
    return &_MaterialBase->DefaultSpecular;
  case ShininessValue:
    return &_MaterialBase->DefaultShininess;
  case EmissiveValue:
    return &_MaterialBase->DefaultEmissive;
  case OpacityValue:
    return &_MaterialBase->DefaultOpacity;
  case TextureValue:
  ...
  default: // this may be a texture animated value...
    if (valueId >= TextureMatValues && valueId < AnimValueLast) {
      ...
      switch (argID) {
      case 0:
        return &_MaterialBase->DefaultTexAnimTracks[texNum].DefaultUTrans;
      case 1:
        return &_MaterialBase->DefaultTexAnimTracks[texNum].DefaultVTrans;
      case 2:
        return &_MaterialBase->DefaultTexAnimTracks[texNum].DefaultUTrans;
      case 3:
        return &_MaterialBase->DefaultTexAnimTracks[texNum].DefaultVTrans;
      case 4:
        return &_MaterialBase->DefaultTexAnimTracks[texNum].DefaultWRot;
      }
    }
    break;
  };
...
// ***************************************************************************
void CAnimatedMaterial::registerToChannelMixer(CChannelMixer *chanMixer,
                                               const std::string &prefix) {
  // For CAnimatedMaterial, channels are detailled (material rendered after
  // clip)!
  ...
  for (uint k = 0; k < IDRV_MAT_MAXTEXTURES; ++k) {
    for (uint l = 0; l < NumTexAnimatedValues; ++l) {
      addValue(chanMixer, TextureMatValues + l + k * NumTexAnimatedValues,
               OwnerBit, prefix, true);
    }
  }
}

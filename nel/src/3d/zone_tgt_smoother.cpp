...
void CZoneTgtSmoother::makeVerticesCoplanar(std::vector<CZoneInfo> &zones) {
  ...
  for (itVert = VertexMap.begin(); itVert != VertexMap.end(); itVert++) {
    ...
    // \todo yoyo: later: do it too on non border vertices if wanted (with a
    // normal threshold...).
    if (!vert.OnBorder)
      continue;
    ...
    for (itPatch = vert.Patchs.begin(); itPatch != vert.Patchs.end();
         itPatch++) {
      ...
      if (itPatch->Patch->BindEdges[e0].NPatchs != 1 ||
          itPatch->Patch->BindEdges[e1].NPatchs != 1) {
        bindFound = true;
        break;
      }
    }
    ...
    for (itPatch = vert.Patchs.begin(); itPatch != vert.Patchs.end();
      ...
      for (sint ed = 0; ed < 2; ed++) {
        ...
        for (tgt = 0; tgt < (sint)tangents.size(); tgt++) {
          if (tangents[tgt].ZoneId == zoneId &&
              tangents[tgt].PatchId == patchId &&
              tangents[tgt].EdgeId == edgeId)
            break;
        }
    ...
    if (tangents.size() != 4) {
      nlinfo("ERROR: vertex %d should have 4 tangents. It got %d. (MAXINDICES "
             "+1!!)",
             itVert->first, tangents.size());
      continue;
    }
    ...
    for (i = 0; i < (sint)tangents.size(); i++) {
      ...
      for (j = i + 1; j < (sint)tangents.size(); j++) {
        if (tangents[i].isOppositeOf(tangents[j])) {
          ...
          // Average the tangents. Normalize them before, to keep much as
          // possible the orientation.
    ...
    for (itPatch = vert.Patchs.begin(); itPatch != vert.Patchs.end();
      ...
      if (smoothEdge0 && smoothEdge1)
        pa.Patch.Interiors[itPatch->IdVert] =
            pa.Patch.Tangents[t0] + pa.Patch.Tangents[t1] - vertexValue;
    }
  }
}
   ...
   itPatch++)
        {
            ...
            // Setup the coplanared interior. just the sum of 2 vector tangents.
            pa.Patch.Interiors[itPatch->IdVert]= pa.Patch.Tangents[t0] +
   pa.Patch.Tangents[t1] - planeCenter;
        }
*/

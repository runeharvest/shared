...
void CMeshBase::serialMeshBase(NLMISC::IStream &f) {
  ...
  Version 1:
      - Cut in version because of badly coded ITexture* serialisation. throw an
  exception if find a version < 1. Version 0:
      - 1st version.
  */
...
void CMeshBase::instanciateMeshBase(CMeshBaseInstance *mi, CScene *ownerScene) {
  ...
  for (it = _AnimatedMaterials.begin(); it != _AnimatedMaterials.end(); it++) {
    ...
    // Must set the Animatable father of the animated material (the
    // mesh_base_instance!).
...
// ***************************************************************************
void CMeshBase::applyMaterialUsageOptim(const std::vector<bool> &materialUsed,

...
IIBDrvInfos::~IIBDrvInfos() { _Driver->removeIBDrvInfoPtr(_DriverIterator); }
...
void CIndexBuffer::setLocation(TLocation newLocation) {
  ...
  if (newLocation != NotResident) {
    ...
    nlassert(_NonResidentIndexes.size() / getIndexNumBytes() ==
             _Capacity); // Internal buffer must have the good size
    if (_Capacity != 0)
    ...
    if ((_Location == RAMResident) && (_PreferredMemory != RAMVolatile) &&
      ...
      uint size = std::min((uint)(_Capacity * getIndexNumBytes()),
                           (uint)(_ResidentSize * getIndexNumBytes()));
      if (size)
...
void CIndexBuffer::serial(NLMISC::IStream &f) {
  ...
  if (ver < 1) {
    ...
    // Read tri
    // NB : for backward compatibility, indices are always saved in 32 bit
    // format
...
void CIndexBuffer::fillBuffer() {
  if (DrvInfos && _KeepLocalMemory) {
    ...
    NLMISC::CFastMem::memcpy(dest, &(_NonResidentIndexes[0]),
                             _NbIndexes * getIndexNumBytes());

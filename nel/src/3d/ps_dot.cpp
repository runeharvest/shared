...
///===================================================================
template <class T>
inline void DrawDot(T it, CVertexBuffer &vb,
                    CPSAttribMaker<NLMISC::CRGBA> *colorScheme, uint leftToDo,
                    CPSLocated *owner, CMaterial &mat, IDriver *driver,
                    uint32 srcStep) {
  NL_PS_FUNC(DrawDot)
  nlassert(leftToDo != 0);
  ...
  do {
    ...
    vb.setNumVertices(toProcess); // because of volatile vb copy, indicate the
                                  // numebr of vertices to copy
    {
      CVertexBufferReadWrite vba;
      vb.lock(vba);
      if (colorScheme) {
        // compute the colors
        colorScheme->make(owner, total - leftToDo, vba.getColorPointer(),
                          vb.getVertexSize(), toProcess, false, srcStep);
      ...
      } else if (srcStep == (1 << 16)) // make sure we haven't got auto-lod and
                                       // that the step is 1.0
      {
        // there's no color information in the buffer, so we can copy it
        // directly
        NLMISC::CFastMem::memcpy(vba.getVertexCoordPointer(), &(*it),
                                 sizeof(NLMISC::CVector) * toProcess);
        it += toProcess;
      } else {
        itEnd = it + toProcess;
        uint8 *currPos = (uint8 *)vba.getVertexCoordPointer();
...
void CPSDot::draw(bool opaque) {
  ...
  if (step == (1 << 16)) {
    DrawDot(_Owner->getPos().begin(), vb, _ColorScheme, numToProcess, _Owner,
            _Mat, driver, step);
  } else {
    DrawDot(TIteratorVectStep1616(_Owner->getPos().begin(), 0, step), vb,
            _ColorScheme, numToProcess, _Owner, _Mat, driver, step);
  }
...
void CPSDot::initVertexBuffers() {
  ...
  _DotVbColor.setVertexFormat(CVertexBuffer::PositionFlag |
                              CVertexBuffer::PrimaryColorFlag);
  _DotVbColor.setPreferredMemory(

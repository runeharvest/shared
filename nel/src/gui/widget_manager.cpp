...
// ----------------------------------------------------------------------------
void CWidgetManager::SMasterGroup::addWindow(CInterfaceGroup *pIG,
                                             uint8 nPrio) {
...
// ----------------------------------------------------------------------------
CInterfaceGroup *
CWidgetManager::SMasterGroup::getWindowFromId(const std::string &winID) {
  for (uint8 i = 0; i < WIN_PRIORITY_MAX; ++i) {
    std::list<CInterfaceGroup *>::iterator it = PrioritizedWindows[i].begin();
    while (it != PrioritizedWindows[i].end()) {
      if ((*it)->getId() == winID)
...
void CWidgetManager::SMasterGroup::deactiveAllContainers() {
  ...
  // Make first a list of all window (Warning: all group container are not
  // window!)
  for (uint8 i = 0; i < WIN_PRIORITY_MAX; ++i) {
    std::list<CInterfaceGroup *>::iterator it = PrioritizedWindows[i].begin();
    while (it != PrioritizedWindows[i].end()) {
  ...
  // Then hide them. Must do this in 2 times, because setActive(false) change
  // PrioritizedWindows, and hence invalidate its.
...
void CWidgetManager::SMasterGroup::sortWorldSpaceGroup() {
  ...
  // Fill the sort table
  std::list<CInterfaceGroup *>::iterator it =
      PrioritizedWindows[WIN_PRIORITY_WORLD_SPACE].begin();
...
// ----------------------------------------------------------------------------
CInterfaceGroup *
CWidgetManager::getMasterGroupFromId(const std::string &MasterGroupName) {
...
// ----------------------------------------------------------------------------
void CWidgetManager::addWindowToMasterGroup(const std::string &sMasterGroupName,
                                            CInterfaceGroup *pIG) {
...
// ----------------------------------------------------------------------------
void CWidgetManager::removeWindowFromMasterGroup(
    const std::string &sMasterGroupName, CInterfaceGroup *pIG) {
  // Warning this function is not smart : its a o(n) !
  if (pIG == NULL)
    return;
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
...
void CWidgetManager::removeAllMasterGroups() {
  ...
  // Yoyo: important to not Leave NULL in the array, because of CGroupHTML and
  // LibWWW callback that may call CInterfaceManager::getElementFromId() (and
  // this method hates having NULL in the arrays ^^)
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::activateMasterGroup(const std::string &sMasterGroupName,
                                         bool bActive) {
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    if (pIEL != NULL) {
      ...
      if (m_LoggedMissingElement.find(sEltId) != m_LoggedMissingElement.end()) {
        ...
        nlwarning("Previously missing UI element with Id '%s' was now found!",
                  sEltId.c_str());
      }
#endif
      return pIEL;
...
// ------------------------------------------------------------------------------------------------
CInterfaceElement *CWidgetManager::getElementFromId(const std::string &sStart,
                                                    const std::string &sEltId) {
...
// ------------------------------------------------------------------------------------------------
CInterfaceElement *
CWidgetManager::getElementFromDefine(const std::string &defineId) {
  ...
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
    ...
    if (rMG.Group->getActive()) {
      for (uint8 nPriority = WIN_PRIORITY_MAX; nPriority > 0; nPriority--) {
        const std::list<CInterfaceGroup *> &rList =
            rMG.PrioritizedWindows[nPriority - 1];
...
// ***************************************************************************
void CWidgetManager::setWindowPriority(CInterfaceGroup *pWin,
                                       uint8 nNewPriority) {
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
       nMasterGroup++) {
    CWidgetManager::SMasterGroup &rMG = _MasterGroups[nMasterGroup];
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::enableModalWindow(CCtrlBase *ctrlLaunchingModal,
                                       CInterfaceGroup *pIG) {
...
void CWidgetManager::enableModalWindow(CCtrlBase *CtrlLaunchingModal,
  ...
  CInterfaceGroup *group =
      dynamic_cast<CGroupModal *>(getElementFromId(groupName));
  if (group) {
    // enable the modal
...
void CWidgetManager::disableModalWindow() {
  while (!_ModalStack.empty()) {
    ...
    _ModalStack
        .pop_back(); // must pop back as early as possible because 'setActive'
                     // may trigger another 'popModalWindow', leading to a crash
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::pushModalWindow(CCtrlBase *ctrlLaunchingModal,
                                     CInterfaceGroup *pIG) {
...
void CWidgetManager::popModalWindowCategory(const std::string &category) {
  for (;;) {
    ...
    if (!_ModalStack.back().ModalWindow)
      break;
    CGroupModal *gm = dynamic_cast<CGroupModal *>(
        (CInterfaceGroup *)(_ModalStack.back().ModalWindow));
    ...
    if (rMG.Group->getActive()) {
      for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; nPriority++) {
        ...
        for (itw = rList.begin(); itw != rList.end();) {
          ...
          itw++; // since setActive invalidate the iterator, be sure we move to
                 // the next one before
          pIG->setActive(false);
        }
      }
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::getViewsUnder(sint32 x, sint32 y,
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::getCtrlsUnder(sint32 x, sint32 y,
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::getGroupsUnder(sint32 x, sint32 y,
                                    std::vector<CInterfaceGroup *> &vIGL) {
  vIGL.clear();
...
void CWidgetManager::checkCoords() {
...

  {
    ...
    for (nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
         nMasterGroup++) {
      CWidgetManager::SMasterGroup &rMG = _MasterGroups[nMasterGroup];
      if (rMG.Group->getActive()) {
        for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; nPriority++) {
          std::list<CInterfaceGroup *> &rList =
              rMG.PrioritizedWindows[nPriority];
          std::list<CInterfaceGroup *>::const_iterator itw;
          for (itw = rList.begin(); itw != rList.end();) {
            CInterfaceGroup *pIG = *itw;
            itw++; // since checkCoords invalidate the iterator, be sure we move
                   // to the next one before
            if (pIG->getActive())
              pIG->checkCoords();
          }
        }
      ...
      if (rMG.Group->getActive()) {
        for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; nPriority++) {
          ...
          for (itw = rList.begin(); itw != rList.end(); itw++) {
            ...
            // updateCoords the window only if the master group is his parent
            // and if need it do it until updateCoords() no more invalidate
            // coordinates!!

            // add deadlock counter to prevent endless loop (Issue #73: web
            // browser long scroll lockup)
            int deadlock = 10;
            while (--deadlock > 0 && pIG->getParent() == rMG.Group &&
                   (pIG->getInvalidCoords() > 0)) {
              bRecomputeCtrlUnderPtr = true;
              // Update as many pass wanted (3 time for complex resizing, 1 for
              // scroll for example)
            ...
            if (pIG->getParent() == rMG.Group && !updateCoordCalled &&
                pIG->isNeedFrameUpdatePos()) {
...
// ----------------------------------------------------------------------------
CInterfaceGroup *
CWidgetManager::getWindowForActiveMasterGroup(const std::string &window) {
  // Search for all elements
  for (uint32 nMasterGroup = 0; nMasterGroup < _MasterGroups.size();
       nMasterGroup++) {
    const SMasterGroup &rMG = _MasterGroups[nMasterGroup];
    if (rMG.Group->getActive()) {
      CInterfaceElement *pEL =
          getElementFromId(rMG.Group->getId() + ":" + window);
      if (pEL && pEL->isGroup())
  ...
  if (getOverExtendViewText()) {
    ...
    if (groupOver) {
      ...
      if (vtDst != NULL) {
        ...
        // setup background
        CViewBitmap *pBack =
            dynamic_cast<CViewBitmap *>(groupOver->getView("midback"));
        CViewBitmap *pOutline =
            dynamic_cast<CViewBitmap *>(groupOver->getView("midoutline"));
        if (pBack)
        ...
        if (vtSrc->isClampRight()) {
          x += std::max(0, (groupOver->getXReal() + groupOver->getWReal()) -
                               (groupOver->getParent()->getXReal() +
                                groupOver->getParent()->getWReal()));
        } else {
          x += vtDst->getWReal() - vtSrc->getWReal();
          if (x >
              (groupOver->getXReal() - groupOver->getParent()->getXReal())) {
            x -= x -
                 (groupOver->getXReal() - groupOver->getParent()->getXReal());
          }
        }
        ...
        // TODO: there should be no overflow on y, unless barely visible and
...
void CWidgetManager::snapIfClose(CInterfaceGroup *group) {
  if (!group || _WindowSnapDistance == 0 ||
      _WindowSnapInvert != lastKeyEvent.isShiftDown())
    return;
...
// ----------------------------------------------------------------------------
uint CWidgetManager::adjustTooltipPosition(CCtrlBase *newCtrl,
                                           CInterfaceGroup *win,
                                           THotSpot ttParentRef,
                                           THotSpot ttPosRef, sint32 xParent,
                                           sint32 yParent, sint32 wParent,
                                           sint32 hParent) {
  ...
  if (ttPosRef == Hotspot_TTAuto || ttParentRef == Hotspot_TTAuto) {
    ...
    if (parentType == CCtrlBase::TTWindow ||
        parentType == CCtrlBase::TTSpecialWindow) {
  ...
  if ((x + groupContextHelp->getW()) >
      groupContextHelp->getParent()->getWReal()) {
    ++clampCount;
  ...
  if ((y + groupContextHelp->getH()) >
      groupContextHelp->getParent()->getHReal()) {
    ...
    if (pTxt != NULL) {
      ...
      if (parentType == CCtrlBase::TTWindow ||
        ...
        if (parentType == CCtrlBase::TTWindow)
          ...
          win = dynamic_cast<CInterfaceGroup *>(
              getElementFromId(newCtrl->getToolTipSpecialParent()));
      ...
      // **** resolve auto posref
      uint clampCount = adjustTooltipPosition(
          newCtrl, win, newCtrl->getToolTipParentPosRef(),
          newCtrl->getToolTipPosRef(), xParent, yParent, wParent, hParent);

      if (clampCount != 0) {
        // try to fallback on alternate tooltip posref
        uint altClampCount = adjustTooltipPosition(
            newCtrl, win, newCtrl->getToolTipParentPosRefAlt(),
            newCtrl->getToolTipPosRefAlt(), xParent, yParent, wParent, hParent);
...
void CWidgetManager::drawContextHelp() {
      ...
      if (newCtrl) {
        ...
        if (!newCtrl->getContextHelpActionHandler().empty()) {
          CAHManager::getInstance()->runActionHandler(
              newCtrl->getContextHelpActionHandler(), newCtrl,
              newCtrl->getContextHelpAHParams());
        }
  ...
  // ***************
  if (_CurCtrlContextHelp) {
    CInterfaceGroup *groupContextHelp = getWindowForActiveMasterGroup(
        _CurCtrlContextHelp->getContextHelpWindowName());
    ...
    if (groupContextHelp) {
       ...
       * the tooltip was generated by the current modal window
       */
      if (hasModal()) {
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::getNewWindowCoordToNewScreenSize(sint32 &x, sint32 &y,
                                                      sint32 w, sint32 h,
                                                      sint32 newScreenW,
                                                      sint32 newScreenH) const {
  ...
  /*
      ...
      We also cannot use specific code according to each window because user may
     completly modify his interface So the strategy is to dectect on which
     "side" (or center) the window is the best sticked, and then just move the
     window according to this position
  */

  // *** First detect from which screen position the window is the more sticked
  // (borders or center) In X: best hotspot is left, middle or right?
  sint32 posXToLeft = x;
  ...
  const sint32 middleYWeight = 6; // Avoid default Mission/Team/Map/ContactList
                                  // positions to be considered as "middle"
  ...
  // *** According to best matching hotspot, and new screen resolution, move the
  // window
  // x
...
void CWidgetManager::moveAllWindowsToNewScreenSize(uint32 newScreenW,
  ...
  if (_ScreenW > 0 && _ScreenH > 0 && newScreenW > 0 && newScreenH > 0 &&
        ...
        for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; nPriority++) {
          ...
          for (itw = rList.begin(); itw != rList.end(); itw++) {
            ...
            if (gc->getLayerSetup() != 0)
              continue;
            // should all be BL / TL
            if (gc->getParentPosRef() != Hotspot_BL ||
                gc->getPosRef() != Hotspot_TL)
              continue;
            ...
            sint32 w = pIG->getW(false); // the window may be hid, still get the
                                         // correct(or estimated) W
            sint32 h = pIG->getH(false); // the window may be hid, still get the
                                         // correct(or estimated) H

            // Compute the new coordinate
            getNewWindowCoordToNewScreenSize(x, y, w, h, newScreenW,
                                             newScreenH);
    ...
    for (itr = newScreenSizeHandlers.begin();
         itr != newScreenSizeHandlers.end(); ++itr) {
      INewScreenSizeHandler *handler = *itr;
      handler->process(newScreenW, newScreenH);
    }
  }

  // Now those are the last screen coordinates used for window position
  // correction
...
void CWidgetManager::updateAllLocalisedElements() {
  ...
  // If all conditions are OK, move windows so they fit correctly with new
  // screen size Do this work only InGame when Config is loaded
...
void CWidgetManager::drawViews(NL3D::UCamera camera) {
  ...
  // If an element has captured the keyboard, make sure it is alway visible (all
  // parent windows active)
  ...
  setGlobalColor(
      NLMISC::CRGBA((uint8)_RProp->getValue32(), (uint8)_GProp->getValue32(),
                    (uint8)_BProp->getValue32(), (uint8)_AProp->getValue32()));
  ...
  /*  Draw all the windows
      To minimize texture swapping, we first sort per Window, then we sort per
     layer, then we render per Global Texture. Computed String are rendered in
     on big drawQuads at last part of each layer
  */
    ...
    if (rMG.Group->getActive()) {
      ...
      for (uint8 nPriority = 0; nPriority < WIN_PRIORITY_MAX; ++nPriority) {
        ...
        for (itw = rList.begin(); itw != rList.end(); itw++) {
          ...
          {
            if (pIG->getActive()) {
              // Draw all the elements of this window in the layers in
              // ViewRendered
              pIG->draw();
              // flush the layers
              CViewRenderer::getInstance()->flush();
  ...
  for (itr = onWidgetsDrawnHandlers.begin();
       itr != onWidgetsDrawnHandlers.end(); ++itr) {
...
bool CWidgetManager::handleEvent(const CEventDescriptor &evnt) {
  ...
  } else if (evnt.getType() == CEventDescriptor::key) {
...
bool CWidgetManager::handleSystemEvent(const CEventDescriptor &evnt) {
  const CEventDescriptorSystem &systemEvent =
      reinterpret_cast<const CEventDescriptorSystem &>(evnt);
...
bool CWidgetManager::handleKeyboardEvent(const CEventDescriptor &evnt) {
  ...
  //		if ((eventDesc.getKeyEventType() ==
  //CEventDescriptorKey::keydown) && !_ModalStack.empty() &&
  //!eventDesc.getKeyAlt() && !eventDesc.getKeyCtrl() &&
  //!eventDesc.getKeyShift())
  // Hide menu (or popup menu) is ESCAPE pressed
  if (eventDesc.getKeyEventType() == CEventDescriptorKey::keydown &&
      eventDesc.getKey() == NLMISC::KeyESCAPE) {
    if (hasModal()) {
  ...
  if (eventDesc.getKeyEventType() == CEventDescriptorKey::keydown &&
      eventDesc.getKey() == NLMISC::KeyESCAPE) {
    // Get the last escapable active top window. NB: this is ergonomically
    // better.
  ...
  if (eventDesc.getKeyEventType() == CEventDescriptorKey::keychar &&
      eventDesc.getChar() == NLMISC::KeyRETURN && !eventDesc.getKeyCtrl()) {
    // If the  top window has Enter AH
    CInterfaceGroup *tw = getTopWindow();
    if (tw && !tw->getAHOnEnter().empty()) {
      // if the captured keyboard is in this Modal window, then must handle him
      // in priority
      if (getCaptureKeyboard() && getCaptureKeyboard()->getRootWindow() == tw) {
        ...
        // The window or modal control the OnEnter. Execute, and don't go to the
        // chat.
        CAHManager::getInstance()->runActionHandler(tw->getAHOnEnter(), tw,
                                                    tw->getAHOnEnterParams());
        handled = true;
      }
    }

    // else the 'return' key bring back to the last edit box (if possible)
    CCtrlBase *oldCapture = getOldCaptureKeyboard()
                                ? getOldCaptureKeyboard()
                                : getDefaultCaptureKeyboard();
    if (getCaptureKeyboard() == NULL && oldCapture && !handled) {
      /* If the editbox does not want to recover focus, then abort. This
         possibility is normaly avoided through setCaptureKeyboard() which
         already test getRecoverFocusOnEnter(), but it is still possible for the
         default capture (main chat) or the old captured window to not want to
         recover (temporary Read Only chat for instance)
      */
      if (!dynamic_cast<CGroupEditBoxBase *>(oldCapture) ||
          dynamic_cast<CGroupEditBoxBase *>(oldCapture)
...
bool CWidgetManager::handleMouseEvent(const CEventDescriptor &evnt) {
  ...
  if (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouseleftdown)
    _Pointer->setButtonState(static_cast<NLMISC::TMouseButton>(
        _Pointer->getButtonState() | NLMISC::leftButton));
  else if (eventDesc.getEventTypeExtended() ==
           CEventDescriptorMouse::mouserightdown)
    _Pointer->setButtonState(static_cast<NLMISC::TMouseButton>(
        _Pointer->getButtonState() | NLMISC::rightButton));
  else if (eventDesc.getEventTypeExtended() ==
           CEventDescriptorMouse::mouseleftup)
    _Pointer->setButtonState(static_cast<NLMISC::TMouseButton>(
        _Pointer->getButtonState() & ~NLMISC::leftButton));
  ...
  if (CInterfaceElement::getEditorMode()) {
    ...
    if ((eventDesc.getEventTypeExtended() ==
        ...
        (eventDesc.getEventTypeExtended() ==
         CEventDescriptorMouse::mouserightup))
      return true;
  }
  ...
  if (isMouseHandlingEnabled()) {
    ...
    if (_CapturedView != NULL && _CapturedView != getCapturePointerLeft() &&
    ...
    if (hasModal()) {
      ...
      if (mwi.ModalWindow) {
        // If we are not in "click out" mode so we dont handle controls other
        // than those of the modal
          ...
          if (pNewCurrentWnd != mwi.ModalWindow)
            if (eventDesc.getEventTypeExtended() ==
              ...
              if (!mwi.ModalHandlerClickOut.empty())
                CAHManager::getInstance()->runActionHandler(
                    mwi.ModalHandlerClickOut, NULL, mwi.ModalClickOutParams);

          // If the current window is not the modal and if must quit on click
          // out
          if (pNewCurrentWnd != mwi.ModalWindow && mwi.ModalExitClickOut) {
            // NB: don't force handle==true because to quit a modal does not
            // avoid other actions
            ...
            if (eventDesc.getEventTypeExtended() ==
                ...
                (eventDesc.getEventTypeExtended() ==
                 CEventDescriptorMouse::mouserightdown)) {
              clickedOutModalWindow = dynamic_cast<CGroupModal *>(
                  (CInterfaceGroup *)mwi.ModalWindow);
              // disable the modal
              popModalWindow();
              if (hasModal()) {
                // don't handle event unless it is a previous modal window
                if (!isPreviousModal(pNewCurrentWnd))
                  pNewCurrentWnd = NULL; // can't handle event before we have
                                         // left all modal windows
              }
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if ((pNewCurrentWnd != NULL) && (!hasModal()) &&
          (pNewCurrentWnd->getOverlappable())) {
      ...
      // This may happen when alt-tab has been used => the sheet is dragged but
      ...
      if (!CCtrlDraggable::getDraggedSheet()) {
        ...
        if (!captured) {
          ...
          uint nMaxDepth = 0;
          const std::vector<CCtrlBase *> &_CtrlsUnderPointer =
              getCtrlsUnderPointer();
          for (sint32 i = (sint32)_CtrlsUnderPointer.size() - 1; i >= 0; i--) {
            CCtrlBase *ctrl = _CtrlsUnderPointer[i];
            if (ctrl && ctrl->isCapturable() &&
                ctrl->isInGroup(pNewCurrentWnd)) {
              if (CInterfaceElement::getEditorMode() &&
                  !ctrl->isEditorSelectable())
                continue;
        ...
        if (clickedOutModalWindow &&
            !clickedOutModalWindow->OnPostClickOut.empty()) {
          CAHManager::getInstance()->runActionHandler(
              clickedOutModalWindow->OnPostClickOut, getCapturePointerLeft(),
              clickedOutModalWindow->OnPostClickOutParams);
        }
      }
      // if found
      if (captured) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      // Take the top most control.
      {
        ...
        if (clickedOutModalWindow &&
            !clickedOutModalWindow->OnPostClickOut.empty()) {
          CAHManager::getInstance()->runActionHandler(
              clickedOutModalWindow->OnPostClickOut, getCapturePointerRight(),
              clickedOutModalWindow->OnPostClickOutParams);
        }
      }
      // if found
    ...
    if (!handled) {
      if (((pNewCurrentWnd != NULL) && !hasModal()) ||
        ...
        // After handle event of a left click, may set window Top if movable
        // (infos etc...) if( (eventDesc.getEventTypeExtended() ==
        // CEventDescriptorMouse::mouseleftdown) && pNewCurrentWnd->isMovable()
        // )
        //	setTopWindow(pNewCurrentWnd);
      }
    }
    ...
    // If the current window is the modal, may Modal quit. Do it after standard
    // event handle
    if (hasModal() && pNewCurrentWnd == getModal().ModalWindow) {
      // NB: don't force handle==true because to quit a modal does not avoid
      // other actions
      CWidgetManager::SModalWndInfo mwi = getModal();
    ...
    // If the mouse is over a window, always consider the event is taken (avoid
    // click behind)
    handled |= isMouseOverWindow();

    // If mouse click was not on interface and we have keyboard captured, then
    // release keyboard
    if (!handled && getCaptureKeyboard() != NULL &&
        eventDesc.getEventTypeExtended() != CEventDescriptorMouse::mousemove)
...
bool CWidgetManager::handleMouseMoveEvent(const CEventDescriptor &eventDesc) {
  ...
  const CEventDescriptorMouse &e =
      static_cast<const CEventDescriptorMouse &>(eventDesc);
  ...
  // These are floats packed in the sint32 from the NEL events that provide them
  // as float see comment in CInputHandler::handleMouseMoveEvent
  sint32 newX = static_cast<sint32>(
      std::floor(*reinterpret_cast<float *>(&x) * screenW + 0.5f));
  sint32 newY = static_cast<sint32>(
      std::floor(*reinterpret_cast<float *>(&y) * screenH + 0.5f));
...
void CWidgetManager::stopDragging() {
  if (draggedElement != NULL) {
    CInterfaceGroup *g =
        getGroupUnder(draggedElement->getXReal(), draggedElement->getYReal());
    CInterfaceElement *e = draggedElement;
    CInterfaceGroup *tw = getTopWindow();
...
void CWidgetManager::movePointer(sint32 dx, sint32 dy) {
  ...
  if (newpy < 0)
    newpy = 0;
  if (newpx > (sint32)nScrW)
    newpx = nScrW;
  if (newpy > (sint32)nScrH)
    newpy = nScrH;
  dx = newpx - oldpx;
  dy = newpy - oldpy;
...
void CWidgetManager::setCaptureKeyboard(CCtrlBase *c) {
  CGroupEditBoxBase *oldEb =
      dynamic_cast<CGroupEditBoxBase *>((CCtrlBase *)_CaptureKeyboard);
  ...
  if (newEb) {
    ...
    if (!newEb->getAHOnFocus().empty()) {
      CAHManager::getInstance()->runActionHandler(newEb->getAHOnFocus(), newEb,
                                                  newEb->getAHOnFocusParams());
    }
  }
  _CaptureKeyboard = c;
  notifyElementCaptured(c);
...
void CWidgetManager::registerClockMsgTarget(CCtrlBase *vb) {
  ...
  if (isClockMsgTarget(vb)) {
    nlwarning("<CInterfaceManager::registerClockMsgTarget> Element %s is "
              "already registered",
              vb->getId().c_str());
    return;
  }
...
void CWidgetManager::unregisterClockMsgTarget(CCtrlBase *vb) {
  if (!vb)
    return;
  std::list<CCtrlBase *>::iterator it =
      std::find(_ClockMsgTargets.begin(), _ClockMsgTargets.end(), vb);
  if (it != _ClockMsgTargets.end()) {
    // instead of deleting, just mark as deleted incase we are inside iterating
    // loop, it will be removed in sendClockTickEvent
    (*it) = NULL;
  }
}
...
bool CWidgetManager::isClockMsgTarget(CCtrlBase *vb) const {
  std::list<CCtrlBase *>::const_iterator it =
      std::find(_ClockMsgTargets.begin(), _ClockMsgTargets.end(), vb);
...
void CWidgetManager::sendClockTickEvent() {
  ...
  for (std::list<CCtrlBase *>::iterator it = _ClockMsgTargets.begin();
       it != _ClockMsgTargets.end();) {
    CCtrlBase *ctrl = *it;
    if (ctrl) {
...
void CWidgetManager::makeWindow(CInterfaceGroup *group) {
  ...
  if (i == _MasterGroups.size()) {
    std::string stmp =
        std::string("not found master group for window: ") + group->getId();
...
void CWidgetManager::setGlobalColor(NLMISC::CRGBA col) {
  ...
  _GlobalColorForContent.A =
      (uint8)(((uint16)_GlobalColorForContent.A * (uint16)_ContentAlpha) >> 8);
}
...
void CWidgetManager::setContentAlpha(uint8 alpha) {
  ...
  _GlobalColorForContent.A = alpha; /*(uint8) (( (uint16) _GlobalColor.A *
                                       (uint16) _ContentAlpha) >> 8);*/
}
...
CInterfaceOptions *CWidgetManager::getOptions(const std::string &name) {
  std::map<std::string, NLMISC::CSmartPtr<CInterfaceOptions>>::iterator it =
      _OptionsMap.find(name);
...
void CWidgetManager::addOptions(std::string name, CInterfaceOptions *options) {
  _OptionsMap.insert(
      std::map<std::string, CInterfaceOptions *>::value_type(name, options));
}
...
void CWidgetManager::removeAllOptions() { _OptionsMap.clear(); }

bool CWidgetManager::serializeOptions(xmlNodePtr parentNode) const {
  if (parentNode == NULL)
    return false;

  std::map<std::string, NLMISC::CSmartPtr<CInterfaceOptions>>::const_iterator
      itr;
...
uint CWidgetManager::getUserDblClickDelay() {
  uint nVal = 50;
  NLMISC::CCDBNodeLeaf *pNL =
      CDBManager::getInstance()->getDbProp("UI:SAVE:DOUBLE_CLICK_SPEED");
  if (pNL != NULL)
    nVal = pNL->getValue32();

  uint dbclickDelay =
      (uint)(DOUBLE_CLICK_MIN +
             (DOUBLE_CLICK_MAX - DOUBLE_CLICK_MIN) * (float)nVal / 100.0f);
  return dbclickDelay;
}
...
void CWidgetManager::setupOptions() {
  ...
  if (opt != NULL) {
    ...
    _SystemOptions[OptionCtrlSheetGrayColor] =
        opt->getValue("ctrl_sheet_gray_color");
    _SystemOptions[OptionCtrlTextGrayColor] =
        opt->getValue("ctrl_text_gray_color");
    _SystemOptions[OptionCtrlSheetRedifyColor] =
        opt->getValue("ctrl_sheet_redify_color");
    _SystemOptions[OptionCtrlTextRedifyColor] =
        opt->getValue("ctrl_text_redify_color");
    _SystemOptions[OptionCtrlSheetGreenifyColor] =
        opt->getValue("ctrl_sheet_greenify_color");
    _SystemOptions[OptionCtrlTextGreenifyColor] =
        opt->getValue("ctrl_text_greenify_color");
    _SystemOptions[OptionViewTextOverBackColor] =
        opt->getValue("text_over_back_color");
    ...
    _SystemOptions[OptionTimeoutContextHtml] =
        opt->getValue("context_html_timeout");
...
float CWidgetManager::getAlphaRolloverSpeed() {
  if (_AlphaRolloverSpeedDB == NULL)
    _AlphaRolloverSpeedDB =
        CDBManager::getInstance()->getDbProp("UI:SAVE:ALPHA_ROLLOVER_SPEED");
  float fTmp = ROLLOVER_MIN_DELTA_PER_MS +
               (ROLLOVER_MAX_DELTA_PER_MS - ROLLOVER_MIN_DELTA_PER_MS) * 0.01f *
                   (100 - _AlphaRolloverSpeedDB->getValue32());
...
void CWidgetManager::setContainerAlpha(uint8 alpha) {
  ...
  c.A = alpha; /*(uint8) (( (uint16) _GlobalColor.A * (uint16) _ContainerAlpha)
                  >> 8);	*/
  setGlobalColor(c);
}
...
void CWidgetManager::updateGlobalAlphas() {
  if (!_GlobalContentAlphaDB) {
    ...
    _GlobalContainerRolloverFactorDB = CDBManager::getInstance()->getDbProp(
        "UI:SAVE:CONTAINER_ROLLOVER_FACTOR");
  ...
  _GlobalRolloverFactorContent =
      (uint8)_GlobalContentRolloverFactorDB->getValue32();
  _GlobalRolloverFactorContainer =
      (uint8)_GlobalContainerRolloverFactorDB->getValue32();
}
...
void CWidgetManager::registerNewScreenSizeHandler(
    INewScreenSizeHandler *handler) {
  std::vector<INewScreenSizeHandler *>::iterator itr = std::find(
      newScreenSizeHandlers.begin(), newScreenSizeHandlers.end(), handler);
...
void CWidgetManager::removeNewScreenSizeHandler(
    INewScreenSizeHandler *handler) {
  std::vector<INewScreenSizeHandler *>::iterator itr = std::find(
      newScreenSizeHandlers.begin(), newScreenSizeHandlers.end(), handler);
...
void CWidgetManager::registerOnWidgetsDrawnHandler(
    IOnWidgetsDrawnHandler *handler) {
  std::vector<IOnWidgetsDrawnHandler *>::iterator itr = std::find(
      onWidgetsDrawnHandlers.begin(), onWidgetsDrawnHandlers.end(), handler);
...
void CWidgetManager::removeOnWidgetsDrawnHandler(
    IOnWidgetsDrawnHandler *handler) {
  std::vector<IOnWidgetsDrawnHandler *>::iterator itr = std::find(
      onWidgetsDrawnHandlers.begin(), onWidgetsDrawnHandlers.end(), handler);
...
void CWidgetManager::updateAnims() {
  for (std::vector<CInterfaceAnim *>::size_type i = 0; i < activeAnims.size();
       i++)
    activeAnims[i]->update();
}

// ------------------------------------------------------------------------------------------------
void CWidgetManager::runProcedure(const std::string &procName,
                                  CCtrlBase *pCaller,
  ...
  for (uint i = 0; i < proc.Actions.size(); i++) {
    ...
    // run
    // nlwarning("step %d : %s, %s", (int) i, action.Action.c_str(),
    // params.c_str());
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::setProcedureAction(const std::string &procName,
                                        uint actionIndex, const std::string &ah,
...
void CWidgetManager::registerSelectionWatcher(
    IEditorSelectionWatcher *watcher) {
  std::vector<IEditorSelectionWatcher *>::iterator itr =
...
void CWidgetManager::unregisterSelectionWatcher(
    IEditorSelectionWatcher *watcher) {
  std::vector<IEditorSelectionWatcher *>::iterator itr =
      std::find(selectionWatchers.begin(), selectionWatchers.end(), watcher);
...
void CWidgetManager::onWidgetMoved(const std::string &oldid,
...
CInterfaceElement *CWidgetManager::addWidgetToGroup(std::string &group,
...
bool CWidgetManager::groupSelection() {
  ...
  CInterfaceGroup *g = static_cast<CInterfaceGroup *>(
...
bool CWidgetManager::createNewGUI(const std::string &project,
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::registerInterfaceScaleWatcher(
    IInterfaceScaleWatcher *watcher) {
...
// ------------------------------------------------------------------------------------------------
void CWidgetManager::unregisterInterfaceScaleWatcher(
    IInterfaceScaleWatcher *watcher) {

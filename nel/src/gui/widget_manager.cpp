...
void CWidgetManager::updateTooltipCoords(CCtrlBase *newCtrl)
{
	...
	if (groupContextHelp)
	{
		...
		if (pTxt != NULL)
		{
			...
			// **** resolve auto posref
			uint clampCount = adjustTooltipPosition(newCtrl, win, newCtrl->getToolTipParentPosRef(),
			...
			if (clampCount != 0)
			{
				...
				uint altClampCount = adjustTooltipPosition(newCtrl, win, newCtrl->getToolTipParentPosRefAlt(),
...
void CWidgetManager::moveAllWindowsToNewScreenSize(uint32 newScreenW, uint32 newScreenH, bool fixCurrentUI)
{
	...
	if (_ScreenW > 0 && _ScreenH > 0 && newScreenW > 0 && newScreenH > 0 && (_ScreenW != newScreenW || _ScreenH != newScreenH))
	{
...
bool CWidgetManager::handleKeyboardEvent(const CEventDescriptor &evnt)
{
	...
	if (eventDesc.getKeyEventType() == CEventDescriptorKey::keychar && eventDesc.getChar() == NLMISC::KeyRETURN && !eventDesc.getKeyCtrl())
	{
		...
		if (getCaptureKeyboard() == NULL && oldCapture && !handled)
		{
			...
			*/
			if (!dynamic_cast<CGroupEditBoxBase *>(oldCapture) || dynamic_cast<CGroupEditBoxBase *>(oldCapture)->getRecoverFocusOnEnter())
			{
...
bool CWidgetManager::handleMouseEvent(const CEventDescriptor &evnt)
{
	...
	else if (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouserightdown)
		_Pointer->setButtonState(static_cast<NLMISC::TMouseButton>(_Pointer->getButtonState() | NLMISC::rightButton));
	else if (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouseleftup)
		_Pointer->setButtonState(static_cast<NLMISC::TMouseButton>(_Pointer->getButtonState() & ~NLMISC::leftButton));
	...
	if (CInterfaceElement::getEditorMode())
	{
		...
		if ((eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouserightdown) || (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouserightup))
			return true;
	}
	...
	if (isMouseHandlingEnabled())
	{
		...
		if (getCapturePointerRight() != NULL && getCapturePointerLeft() != getCapturePointerRight())
			handled |= getCapturePointerRight()->handleEvent(evnt);

		if (_CapturedView != NULL && _CapturedView != getCapturePointerLeft() && _CapturedView != getCapturePointerRight())
			_CapturedView->handleEvent(evnt);
			...
			if (mwi.ModalWindow)
			{
				...
				else
				{
					...
					if (pNewCurrentWnd != mwi.ModalWindow)
						if (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouseleftdown || (eventDesc.getEventTypeExtended() == CEventDescriptorMouse::mouserightdown))
		...
		if (!handled)
		{
			if (((pNewCurrentWnd != NULL) && !hasModal()) || ((hasModal() && getModal().ModalWindow == pNewCurrentWnd)))
			{
...
{
	std::vector<INewScreenSizeHandler *>::iterator itr = std::find(newScreenSizeHandlers.begin(), newScreenSizeHandlers.end(), handler);
...
{
	std::vector<IOnWidgetsDrawnHandler *>::iterator itr = std::find(onWidgetsDrawnHandlers.begin(), onWidgetsDrawnHandlers.end(), handler);
...
{
	std::vector<IEditorSelectionWatcher *>::iterator itr = std::find(selectionWatchers.begin(), selectionWatchers.end(), watcher);

  ...
  if (_HtmlDownload) {
    CGroupHTML *groupHtml = dynamic_cast<CGroupHTML *>(
        CWidgetManager::getInstance()->getElementFromId(
            "ui:interface:webig:content:html"));
    if (groupHtml) {
      groupHtml->removeImageDownload(_HtmlDownload,
                                     dynamic_cast<CViewBase *>(this));
      _HtmlDownload = NULL;
    }
  }
}
...
std::string CViewBitmap::getProperty(const std::string &name) const {
  if (name == "color") {
  ...
  } else if (name == "txtoffsetx") {
    return toString(_TxtOffsetX);
  } else if (name == "txtoffsety") {
    return toString(_TxtOffsetY);
  } else if (name == "txtwidth") {
    return toString(_TxtWidth);
  } else if (name == "txtheight") {
    ...
    return align;
  } else if (name == "inherit_gc_alpha") {
    return toString(_InheritGCAlpha);
  } else
    return CViewBase::getProperty(name);
}

void CViewBitmap::setProperty(const std::string &name,
...
xmlNodePtr CViewBitmap::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "txtoffsetx",
             BAD_CAST toString(_TxtOffsetX).c_str());
  xmlSetProp(node, BAD_CAST "txtoffsety",
             BAD_CAST toString(_TxtOffsetY).c_str());
  ...
  xmlSetProp(node, BAD_CAST "inherit_gc_alpha",
             BAD_CAST toString(_InheritGCAlpha).c_str());
...
bool CViewBitmap::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _TxtOffsetX = 0;
  if (prop)
    fromString((const char *)prop, _TxtOffsetX);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"txtoffsety");
  _TxtOffsetY = 0;
  if (prop)
    fromString((const char *)prop, _TxtOffsetY);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"txtwidth");
  _TxtWidth = -1;
  if (prop)
    fromString((const char *)prop, _TxtWidth);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"txtheight");
  _TxtHeight = -1;
  if (prop)
    fromString((const char *)prop, _TxtHeight);
...
void CViewBitmap::draw() {
  ...
  if (getModulateGlobalColor()) {
    col.modulateFromColor(
        _Color, CWidgetManager::getInstance()->getGlobalColorForContent());
  } else {
    col = _Color;
    ...
    while (gr) {
      if (gr->isGroupContainer()) {
        ...
        col.A = (uint8)(((sint32)col.A *
                         ((sint32)gc->getCurrentContainerAlpha() + 1)) >>
                        8);
        break;
      }
      gr = gr->getParent();
  ...
  if (_Scale && !_Tile) {
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                          (uint8)_Rot, _Flip, _TextureId, col);
  } else {
    if (!_Tile) {
      rVR.draw11RotFlipBitmap(_RenderLayer, _XReal, _YReal, (uint8)_Rot, _Flip,
                              _TextureId, col);
    } else {
      rVR.drawRotFlipBitmapTiled(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                                 (uint8)_Rot, _Flip, _TextureId, _Align, col);
    }
  }
}
...
void CViewBitmap::setTexture(const std::string &TxName) {
  if (TxName.find("://") != string::npos || TxName.find("//") == 0) {
    ...
    if (groupHtml) {
      ...
      _TextureId.setTexture(localname.c_str(), _TxtOffsetX, _TxtOffsetY,
                            _TxtWidth, _TxtHeight, false);
      _HtmlDownload =
    ...
    _TextureId.setTexture(TxName.c_str(), _TxtOffsetX, _TxtOffsetY, _TxtWidth,
                          _TxtHeight, false);
  }
}
...
// ***************************************************************************
void CViewBitmap::setColorAsInt(sint32 col) { _Color.setPacked(col); }

// ***************************************************************************
sint32 CViewBitmap::getColorAsInt() const { return _Color.getPacked(); }

// ***************************************************************************
void CViewBitmap::setColorRGBA(NLMISC::CRGBA col) { _Color = col; }

// ***************************************************************************
NLMISC::CRGBA CViewBitmap::getColorRGBA() const { return _Color; }
...
// ***************************************************************************
sint32 CViewBitmap::getMinUsedW() const { return getMaxUsedW(); }

...
// ***************************************************************************
CGroupTab::CGroupTab(const TCtorParam &param) : CInterfaceGroup(param) {
  _Selection = -1;
  _NextSelection = -1;
  _BaseRenderLayer = 0;
  _Setuped = false;
...
xmlNodePtr CGroupTab::serialize(xmlNodePtr parentNode, const char *type) const {
  ...
  xmlSetProp(node, BAD_CAST "hide_out_tabs",
             BAD_CAST toString(_HideOutTabs).c_str());
...
void CGroupTab::setup() {
  ...
  */
  for (sint tabArrayIndex = -1; tabArrayIndex < 10; tabArrayIndex++) {
    ...
    // for all tab of this type (standard tab or array of tab), find the Buttons
    // and groups.
    uint tabIndex = 0;
    for (;;) {
      // find the ctrl named "tab0"
      CCtrlTabButton *but = dynamic_cast<CCtrlTabButton *>(
          getCtrl(toString("%s%d", prefix.c_str(), tabIndex)));
      if (!but)
        break;
...
int CGroupTab::luaAddTab(CLuaState &ls) {
  ...
  CCtrlTabButton *tabB =
      dynamic_cast<CCtrlTabButton *>(CLuaIHM::getUIOnStack(ls, 1));
  if (tabB) {
...
void CGroupTab::updateFirstTabButton() {
  if (!_HideOutTabs || (_Selection < 0) || _Buttons.empty() ||
      (_Parent->getWReal() < 0) || _FirstTabIndex >= (sint)_Buttons.size())
    return;
  ...
  if (!_AHOnChange.empty() && ((oldFirstTabIndex != _FirstTabIndex) ||
                               (oldLastTabIndex != _LastTabIndex)))
    CAHManager::getInstance()->runActionHandler(_AHOnChange, this,
                                                _ParamsOnChange);
}
...
void CGroupTab::updateCoords() {
  ...
  // special for groupTab. Because the ctrl may overlap each other from left to
  // right, they are inserted in reverse order. BUT, for correct TR/TL coord
  // handling, must updtae in the reverse sens too!
      ...
      if (i == index) {
        ...
        if (_Buttons[i]->_AHOnLeftClick2 != NULL)
          ...
          _Buttons[i]->_AHOnLeftClick2->execute(
              _Buttons[i], _Buttons[i]->getParamsOnLeftClick());
      }
    }
...
void CGroupTab::selectDefault(CCtrlTabButton *ifSelectionIs) {
  if (!_HideOutTabs && _Selection >= 0 && _Selection < (sint)_Buttons.size() &&
      _Buttons[_Selection] == ifSelectionIs) {
...
void CGroupTab::selectDefaultIfCurrentHid() {
  if (_Selection >= 0 && _Selection < (sint)_Buttons.size() &&
      ...
      _Buttons[_Selection]->getActive() == false) {
    selectDefault(_Buttons[_Selection]);
  }
}

// ***************************************************************************
sint CGroupTab::getSelection() const { return _Selection; }
...
void CCtrlTabButton::setProperty(const std::string &name,
...
xmlNodePtr CCtrlTabButton::serialize(xmlNodePtr parentNode,
...
bool CCtrlTabButton::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop)
    _AssociatedGroup = (const char *)prop;
...
void CCtrlTabButton::setActive(bool state) {
  if (state != getActive()) {
    ...
    // special for correct display of textbuttons. reset to 0 when the button is
    // hid
    if (state)
      setX(_DefaultX);
    else
      ...
      if (_Blinking) {
        ...
        if ((times.thisFrameMs - _BlinkDate) > dbclickDelay) {
          if (_BlinkState) {
            ...
            setTextModulateGlobalColorNormal(
                _TextModulateGlobalColorNormalBlink);
          }

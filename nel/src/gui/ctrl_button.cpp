...
void CCtrlButton::setProperty(const std::string &name,
...
xmlNodePtr CCtrlButton::serialize(xmlNodePtr parentNode,
...
void CCtrlButton::draw() {
  ...
  rVR.drawRotFlipBitmap(_RenderLayer, x, y, txw, txh, 0, false, nTxId, color);

  if ((_OverWhenPushed == false) &&
      (_Pushed == true ||
       (CWidgetManager::getInstance()->getCapturePointerLeft() == this)))
    return;
  ...
  if ((_Over && !editorMode)) {
    ...
    rVR.drawRotFlipBitmap(_RenderLayer + 1, x, y, txw, txh, 0, false,
                          _TextureIdOver, color);
  }
}
...
// ***************************************************************************
sint32 CCtrlButton::getMinUsedW() const { return getMaxUsedW(); }

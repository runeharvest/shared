...
CGroupParagraph::CGroupParagraph(const TCtorParam &param)
    : CInterfaceGroup(param), _Templ(TCtorParam()) {
  _IdCounter = 0;
  _AddElt = Bottom;
  _Align = Left;
  _Space = 0;
...
void CGroupParagraph::addChild(CViewBase *child, bool deleteOnRemove) {
  ...
  if (_Elements.size() >= 2) {
    setOrder((uint)_Elements.size() - 1,
             getOrder((uint)_Elements.size() - 2) + 1);
  }
  invalidateContent();
}

// ----------------------------------------------------------------------------
CGroupParagraph::~CGroupParagraph() { deleteAllChildren(); }

// ----------------------------------------------------------------------------
// Set Hotspot of the first element in reference to the group
/*void CGroupParagraph::setHSGroup (CViewBase *child, EAlign addElt, EAlign
align)
{
    switch (addElt)
    {
 ...
 */
/*void CGroupParagraph::setHSParent(CViewBase *view, EAlign addElt, EAlign
align, uint space)
{
...
void CGroupParagraph::setProperty(const std::string &name,
...
xmlNodePtr CGroupParagraph::serialize(xmlNodePtr parentNode,
...
// ----------------------------------------------------------------------------
void CGroupParagraph::addTextChild(const std::string &line,
                                   bool multiLine /*= true*/) {
  const string elid = _Id + ":el" + toString(_IdCounter);
  ++_IdCounter;
  CViewText *view = new CViewText(elid, string(""), _Templ.getFontSize(),
                                  _Templ.getColor(), _Templ.getShadow());
  view->_Parent = this;
  view->setMultiLine(multiLine);
  view->setTextMode(_Templ.getTextMode());
  if (multiLine)
    view->setMultiLineSpace(_Space);
  view->setText(line);
...
void CGroupParagraph::addTextChildID(uint32 nID, bool multiLine) {
  const string elid = _Id + ":el" + toString(_IdCounter);
  ++_IdCounter;
  CViewTextID *view = new CViewTextID(elid, nID, _Templ.getFontSize(),
                                      _Templ.getColor(), _Templ.getShadow());
  view->_Parent = this;
  view->setMultiLine(multiLine);
  if (multiLine)
...
// ----------------------------------------------------------------------------
void CGroupParagraph::addTextChildID(const std::string &dbPath,
                                     bool multiLine /*=true*/) {
  const string elid = _Id + ":el" + toString(_IdCounter);
  ++_IdCounter;
  CViewTextID *view = new CViewTextID(elid, dbPath, _Templ.getFontSize(),
                                      _Templ.getColor(), _Templ.getShadow());
  view->_Parent = this;
  view->setMultiLine(multiLine);
  if (multiLine)
...
void CGroupParagraph::delChild(CViewBase *childToDel) {
  ...
  if (posChildToDel == _Elements.size()) {
    nlwarning("Can't del child %s, it does not exist in the list",
              childToDel->getId().c_str());
    return;
  }
...
void CGroupParagraph::delChild(uint posChildToDel) {
  ...
  // Remove from drawing
  if (dynamic_cast<CInterfaceGroup *>(childToDel))
    delGroup(static_cast<CInterfaceGroup *>(childToDel), !ElementMustBeDeleted);
  else if (dynamic_cast<CCtrlBase *>(childToDel))
    delCtrl(static_cast<CCtrlBase *>(childToDel), !ElementMustBeDeleted);
  else
    delView(childToDel, !ElementMustBeDeleted);
...
/*void CGroupParagraph::removeHead ()
{
    if (_Elements.empty())
    {
        nlwarning("<CGroupParagraph::removeHead> Can't remove head, list is
empty"); return;
    }
...
// Remove from drawing
for (vector<CInterfaceGroup*>::iterator itg = _ChildrenGroups.begin(); itg !=
_ChildrenGroups.end(); itg++) if(*itg == pVB)
    {
        _ChildrenGroups.erase (itg);
        break;
    }
for (vector<CCtrlBase*>::iterator itc = _Controls.begin(); itc !=
_Controls.end(); itc++) if(*itc == pVB)
    {
        _Controls.erase (itc);
        break;
    }
for (vector<CViewBase*>::iterator itv = _Views.begin(); itv != _Views.end();
itv++) if(*itv == pVB)
    {
        _Views.erase (itv);
        break;
      ...
      for (i = 0; i < elmCount + 1; ++i) {
        ...
        if (i < elmCount) {
          ...
          if (_Elements[i].Element->getActive()) {
            ...
            // Is a view text ?
            CViewText *viewText =
                dynamic_cast<CViewText *>(_Elements[i].Element);
            if (viewText) {
              ...
              viewText->setFirstLineX(x +
                                      ((i == 0) ? _FirstViewIndentView : 0));
              viewText->setX(0);
              viewText->updateTextContext();
            } else {
              _Elements[i].Element->setX(x +
                                         ((i == 0) ? _FirstViewIndentView : 0));
            }
            ...
            if (viewText) {
              ...
              if (!viewText->getText().empty() &&
                  *(viewText->getText().rbegin()) == '\n') {
                changeLine = true;
              }
            } else {
              // If this element is too big for the line, place it next time
              if ((i != firstElementOnLine) &&
                  ((_Elements[i].Element->getX() +
                    _Elements[i].Element->getW()) > width)) {
                lastLineElement--;
                changeLine = true;
                i--;
              }
        ...
        if (changeLine) {
          ...
          for (j = firstElementOnLine; j < lastLineElement; j++) {
            // Is a view text ?
            uint newHeight;
            CViewText *viewTextOnLine =
                dynamic_cast<CViewText *>(_Elements[j].Element);

            // Element height
            if (viewTextOnLine) {
              // Height is just under the first line letter
              newHeight = viewTextOnLine->getFontHeight() -
                          viewTextOnLine->getFontLegHeight();

              // Leg height for this view
              legHeight =
                  std::max(legHeight, viewTextOnLine->getFontLegHeight());
              ...
              if (j == (lastLineElement - 1)) {
                ...
                if (numLine > 1) {
                  multiLineHeight =
                      (numLine - 2) * viewTextOnLine->getFontHeight() +
                      (numLine - 1) * viewTextOnLine->getMultiLineSpace();
                  lastLineWidth = _Indent + viewTextOnLine->getLastLineW();
                }
              }
            } else {
              newHeight = _Elements[j].Element->getH();
              CGroupHTMLInputOffset *inputOffset =
                  dynamic_cast<CGroupHTMLInputOffset *>(_Elements[j].Element);
          ...
          for (j = firstElementOnLine; j < lastLineElement; j++) {
            ...
            if (viewTextOnLine)
              newHeight = viewTextOnLine->getFontHeight() -
                          viewTextOnLine->getFontLegHeight() - offsetY;
            else
              newHeight = _Elements[j].Element->getH();
        ...
        for (j = 0; j < 3; j++) {
          if (j < links) {
            ...
            if (ctrl == NULL) {
              ...
              ctrl->setId(getId() + ":" + "links" + toString(i) + "-" +
                          toString(j));
              ctrl->setParent(this);
            ...
            if (j < 2)
              ...
              y = link.Link->getY() -
                  ((fontSize + fontLineSpace) * (link.Link->getNumLine() - 1));
            ...
            if ((j == 1) && (links == 3))
              height =
                  (fontSize + fontLineSpace) * (link.Link->getNumLine() - 2);
            else
              height = fontSize;
...
void CGroupParagraph::draw() {
  ...
  // false, rVR.getBlankTextureId(), CRGBA(0,255,0,255) );
  if (_Over || _TempOver) {
    CViewRenderer &rVR = *CViewRenderer::getInstance();
    ...
    if (_OverElt != -1) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, pVB->getXReal(), pVB->getYReal(),
                            pVB->getWReal(), pVB->getHReal(), 0, false,
...
struct CRemoveViewPred {
  bool operator()(const CGroupParagraph::CElementInfo &info) const {
    return dynamic_cast<CViewBase *>(info.Element) != NULL;
  }
};
...
struct CRemoveCtrlPred {
  bool operator()(const CGroupParagraph::CElementInfo &info) const {
    return dynamic_cast<CCtrlBase *>(info.Element) != NULL;
  }
};
...
struct CRemoveGroupPred {
  bool operator()(const CGroupParagraph::CElementInfo &info) const {
    return dynamic_cast<CInterfaceGroup *>(info.Element) != NULL;
  }
};
...
void CGroupParagraph::clearViews() {
  ...
  _Elements.erase(
      std::remove_if(_Elements.begin(), _Elements.end(), CRemoveViewPred()),
      _Elements.end());
...
void CGroupParagraph::clearControls() {
  ...
  // remove views from the list of elements
  _Elements.erase(
      std::remove_if(_Elements.begin(), _Elements.end(), CRemoveCtrlPred()),
      _Elements.end());
...
void CGroupParagraph::clearGroups() {
  ...
  // remove views from the list of elements
  _Elements.erase(
      std::remove_if(_Elements.begin(), _Elements.end(), CRemoveGroupPred()),
      _Elements.end());
...
// ----------------------------------------------------------------------------
bool CGroupParagraph::addChildAtIndex(CViewBase *child, uint index,
...
void CGroupParagraph::deleteAllChildren() {
  ...
  for (uint k = 0; k < numChildren; ++k) {
    delChild(numChildren - 1 -
             k); // delete in reverse order to avoid unnecessary vector copies
  }
}
...
uint CGroupParagraph::getNumActiveChildren() const {
  uint numChildren = 0;
  for (uint k = 0; k < _Elements.size(); ++k) {
    if (_Elements[k].Element->getActive())
      ++numChildren;
  }
  return numChildren;
}

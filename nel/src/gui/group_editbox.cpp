...
static u32char supportedCodepoint(u32char c) {
  if (c >= 0xFE00 && c < 0xFE10)
    ...
    return 0xFFFD; // UTF-16 surrogate, unmatched pair, invalid, set to
...
static ::u32string trimUnsupported(const ::u32string str) {
  ...
  for (::u32string::const_iterator it(str.begin()), end(str.end()); it != end;
       ++it) {
...
CGroupEditBox::CGroupEditBox(const TCtorParam &param)
    : CGroupEditBoxBase(param), _BlinkTime(0.f), _CursorPos(0),
      _MaxNumChar(std::numeric_limits<uint32>::max()), _MaxNumBytes(0),
      _MaxNumReturn(std::numeric_limits<uint32>::max()), _MaxFloatPrec(20),
      _MaxCharsSize(32768), _FirstVisibleChar(0), _LastVisibleChar(0),
      _SelectingText(false), _ViewText(NULL), _MaxHistoric(0),
      _CurrentHistoricIndex(-1), _PrevNumLine(1), _EntryType(Text),
      _Setupped(false), _BypassNextKey(false), _BlinkState(false),
      _CursorAtPreviousLineEnd(false), _LooseFocusOnEnter(true),
      _ResetFocusOnHide(false), _BackupFatherContainerPos(false),
      _WantReturn(false), _ClearOnEscape(false), _Savable(true),
      _DefaultInputString(false), _Frozen(false), _CanRedo(false),
      _CanUndo(false), _CursorTexID(-1), _CursorWidth(0),
      _IntegerMinValue(INT_MIN), _IntegerMaxValue(INT_MAX),
      _PositiveIntegerMinValue(0), _PositiveIntegerMaxValue(UINT_MAX),
      _ViewTextDeltaX(0)
...
CGroupEditBox::~CGroupEditBox() {
  ...
  if (CWidgetManager::getInstance()->getCaptureKeyboard() == this ||
      CWidgetManager::getInstance()->getOldCaptureKeyboard() == this) {
...
void CGroupEditBox::setProperty(const std::string &name,
  ...
  if (name == "onchange") {
    ...
    if (value == "integer")
      _EntryType = Integer;
    else if (value == "positive_integer")
      _EntryType = PositiveInteger;
    else if (value == "float")
      _EntryType = Float;
    else if (value == "positive_float")
      _EntryType = PositiveFloat;
    else if (value == "alpha")
      _EntryType = Alpha;
    else if (value == "alpha_num")
      _EntryType = AlphaNum;
    else if (value == "alpha_num_space")
...
xmlNodePtr CGroupEditBox::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "onchange_params",
             BAD_CAST _ParamsOnChange.c_str());
  xmlSetProp(node, BAD_CAST "on_focus_lost", BAD_CAST _AHOnFocusLost.c_str());
  xmlSetProp(node, BAD_CAST "on_focus_lost_params",
             BAD_CAST _AHOnFocusLostParams.c_str());
  xmlSetProp(node, BAD_CAST "on_focus", BAD_CAST _AHOnFocus.c_str());
  xmlSetProp(node, BAD_CAST "on_focus_params",
             BAD_CAST _AHOnFocusParams.c_str());
  xmlSetProp(node, BAD_CAST "max_num_chars",
             BAD_CAST toString(_MaxNumChar).c_str());
  xmlSetProp(node, BAD_CAST "max_num_bytes",
             BAD_CAST toString(_MaxNumBytes).c_str());
  xmlSetProp(node, BAD_CAST "max_num_return",
             BAD_CAST toString(_MaxNumReturn).c_str());
  xmlSetProp(node, BAD_CAST "max_chars_size",
             BAD_CAST toString(_MaxCharsSize).c_str());
  xmlSetProp(node, BAD_CAST "enter_loose_focus",
             BAD_CAST toString(_LooseFocusOnEnter).c_str());
  xmlSetProp(node, BAD_CAST "enter_recover_focus",
             BAD_CAST toString(_RecoverFocusOnEnter).c_str());
  xmlSetProp(node, BAD_CAST "prompt",
             BAD_CAST CUtfStringView(_Prompt).toUtf8().c_str());
  ...
  xmlSetProp(node, BAD_CAST "max_historic",
             BAD_CAST toString(_MaxHistoric).c_str());
  xmlSetProp(node, BAD_CAST "backup_father_container_pos",
             BAD_CAST toString(_BackupFatherContainerPos).c_str());
  xmlSetProp(node, BAD_CAST "want_return",
             BAD_CAST toString(_WantReturn).c_str());
  xmlSetProp(node, BAD_CAST "clear_on_escape",
             BAD_CAST toString(_ClearOnEscape).c_str());
  xmlSetProp(node, BAD_CAST "savable", BAD_CAST toString(_Savable).c_str());
  xmlSetProp(node, BAD_CAST "max_float_prec",
             BAD_CAST toString(_MaxFloatPrec).c_str());
...
bool CGroupEditBox::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  // NB: use InterfaceGroup "OnEnter" data. Different script params for an
  // historic reason
  CAHManager::getInstance()->parseAH(cur, "onenter", "params", _AHOnEnter,
                                     _AHOnEnterParams);
  ...
  if (prop)
    _AHOnChange = (const char *)prop;
  prop = (char *)xmlGetProp(cur, (xmlChar *)"onchange_params");
  if (prop)
    _ParamsOnChange = (const char *)prop;

  prop = (char *)xmlGetProp(cur, (xmlChar *)"on_focus_lost");
  if (prop)
    _AHOnFocusLost = (const char *)prop;
  prop = (char *)xmlGetProp(cur, (xmlChar *)"on_focus_lost_params");
  if (prop)
    _AHOnFocusLostParams = (const char *)prop;

  prop = (char *)xmlGetProp(cur, (xmlChar *)"on_focus");
  if (prop)
    _AHOnFocus = (const char *)prop;
  prop = (char *)xmlGetProp(cur, (xmlChar *)"on_focus_params");
  if (prop)
    _AHOnFocusParams = (const char *)prop;

  prop = (char *)xmlGetProp(cur, (xmlChar *)"max_num_chars");
  if (prop)
    fromString((const char *)prop, _MaxNumChar);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"max_num_bytes");
  if (prop)
    fromString((const char *)prop, _MaxNumBytes);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"max_num_return");
  if (prop)
    fromString((const char *)prop, _MaxNumReturn);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"max_chars_size");
  if (prop)
    fromString((const char *)prop, _MaxCharsSize);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"enter_loose_focus");
  if (prop)
    _LooseFocusOnEnter = convertBool(prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"enter_recover_focus");
  if (prop)
    _RecoverFocusOnEnter = convertBool(prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"reset_focus_on_hide");
  if (prop)
    _ResetFocusOnHide = convertBool(prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"prompt");
  if (prop)
    _Prompt = CUtfStringView((const char *)prop).toUtf32();

  prop = (char *)xmlGetProp(cur, (xmlChar *)"entry_type");
  _EntryType = Text;
  if (prop) {
    if (stricmp(prop, "text") == 0)
      _EntryType = Text;
    else if (stricmp(prop, "integer") == 0)
      _EntryType = Integer;
    else if (stricmp(prop, "positive_integer") == 0)
      _EntryType = PositiveInteger;
    else if (stricmp(prop, "float") == 0)
      _EntryType = Float;
    else if (stricmp(prop, "positive_float") == 0)
      _EntryType = PositiveFloat;
    else if (stricmp(prop, "alpha") == 0)
      _EntryType = Alpha;
    else if (stricmp(prop, "alpha_num") == 0)
      _EntryType = AlphaNum;
    else if (stricmp(prop, "alpha_num_space") == 0)
      _EntryType = AlphaNumSpace;
    else if (stricmp(prop, "password") == 0)
      _EntryType = Password;
    else if (stricmp(prop, "filename") == 0)
      _EntryType = Filename;
    else if (stricmp(prop, "playername") == 0)
      _EntryType = PlayerName;
    else
      nlwarning("<CGroupEditBox::parse> Unknown entry type %s",
                (const char *)prop);
  }
...
void CGroupEditBox::draw() {
  ...
  */
  ...
  if (_CurrSelection == this && _SelectCursorPos != _CursorPos) {
    ...
    sint32 minPos =
        min(_CursorPos, _SelectCursorPos) + (sint32)_Prompt.length();
    sint32 maxPos =
        max(_CursorPos, _SelectCursorPos) + (sint32)_Prompt.length();

    // get its position on screen
    float cxMinPos, cyMinPos;
    float cxMaxPos, cyMaxPos;
    float height;
    _ViewText->getCharacterPositionFromIndex(minPos, false, cxMinPos, cyMinPos,
                                             height);
    _ViewText->getCharacterPositionFromIndex(maxPos, false, cxMaxPos, cyMaxPos,
                                             height);
    ...
    if (cyMinPos != cyMaxPos) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _ViewText->getXReal() + cxMinPos,
                            _ViewText->getYReal() + cyMinPos,
                            _ViewText->getW() - cxMinPos, height, 0, 0,
                            blankTextId, col);

      // Draw One quad for all lines into the big selection (if any)
      sint32 cyBigQuad = cyMaxPos + height;
      if (cyBigQuad < cyMinPos) {
        rVR.drawRotFlipBitmap(_RenderLayer, _ViewText->getXReal(),
                              _ViewText->getYReal() + cyBigQuad,
                              _ViewText->getW(), cyMinPos - cyBigQuad, 0, 0,
                              blankTextId, col);
      }

      // draw the 3rd quad from the last line from start of line to max pos
      rVR.drawRotFlipBitmap(_RenderLayer, _ViewText->getXReal(),
                            _ViewText->getYReal() + cyMaxPos, cxMaxPos, height,
                            0, 0, blankTextId, col);
    } else {
      rVR.drawRotFlipBitmap(_RenderLayer, _ViewText->getXReal() + cxMinPos,
                            _ViewText->getYReal() + cyMinPos,
                            cxMaxPos - cxMinPos, height, 0, 0, blankTextId,
                            col);
    }
  ...
  if (CWidgetManager::getInstance()->getCaptureKeyboard() == this) {
    ...
    if (_BlinkState) // is the cursor shown ?
    {
      ...
      float cx, cy;
      float height;
      _ViewText->getCharacterPositionFromIndex(
          _CursorPos + (sint)_Prompt.length(), _CursorAtPreviousLineEnd, cx, cy,
          height);
      // display the cursor
      ...
      rVR.drawRotFlipBitmap(_RenderLayer + 1, _ViewText->getXReal() + cursorx,
                            _ViewText->getYReal() + cy, 3, height, 0, 0,
                            _CursorTexID);
    }
  }
}
...
void CGroupEditBox::copy() {
  ...
  if (CViewRenderer::getInstance()->getDriver()->copyTextToClipboard(
          getSelection()))
...
void CGroupEditBox::paste() {
  ...
  if (CViewRenderer::getInstance()->getDriver()->pasteTextFromClipboard(
          sString)) {
    // append string now
...
// ----------------------------------------------------------------------------
void CGroupEditBox::writeString(const std::string &str16, bool replace,
                                bool atEnd) {
  // For now, just trim unsupported codepoints to make emoji fallback to text
  // form
  ...
  case Password: {
    ...
    // remove '\r' characters
    toAppend.erase(std::remove(toAppend.begin(), toAppend.end(), (u32char)'\r'),
                   toAppend.end());
  } break;
  case PositiveInteger:
  case PositiveFloat: {
  ...
  if (replace) {
    _InputString =
        _InputString.substr(0, minPos) + toAdd + _InputString.substr(maxPos);
    _CursorPos = minPos + (sint32)toAdd.length();
  } else {
    if (atEnd) {
      _InputString =
          _InputString.substr(0, maxPos) + toAdd + _InputString.substr(maxPos);
      _CursorPos = maxPos;
      _SelectCursorPos = _CursorPos;
    } else {
      _InputString =
          _InputString.substr(0, minPos) + toAdd + _InputString.substr(minPos);
      _CursorPos = minPos + (sint32)toAdd.length();
  ...
  default:
    ...
    if (isKeyRETURN && !_WantReturn) {
      ...
      if (NLGUI::CDBManager::getInstance()
              ->getDbProp("UI:SAVE:CHAT:ENTER_DONT_QUIT_CB")
              ->getValue32() == 0) {
      ...
      _CurrSelection = NULL;
      _CursorAtPreviousLineEnd = false;
      CAHManager::getInstance()->runActionHandler(_AHOnEnter, this,
                                                  _AHOnEnterParams);
    } else {
      ...
      if ((rEDK.getChar() >= 32) || isKeyRETURN) {
        ...
        if (!c)
          return; // For now, just trim unsupported codepoints to make emoji
        ...
        if (_EntryType == PositiveInteger &&
          ...
          fromString(CUtfStringView(copyStr).toUtf8(), value);
          // if out of bounds, abort char
          if (value < _PositiveIntegerMinValue ||
              value > _PositiveIntegerMaxValue)
            return;
        }
        // verify max num char
        if ((uint)_InputString.length() < _MaxNumChar) {
          if (!_MaxNumBytes ||
              CUtfStringView(_InputString + c).toUtf8().size() <=
                  _MaxNumBytes) {
            makeTopWindow();
...
void CGroupEditBox::triggerOnChangeAH() {
  ...
  if (!_AHOnChange.empty())
    CAHManager::getInstance()->runActionHandler(_AHOnChange, this,
                                                _ParamsOnChange);
}

// ----------------------------------------------------------------------------
bool CGroupEditBox::expand() {
  if ((_EntryType == Integer) || (_EntryType == PositiveInteger) ||
      (_EntryType == Float) || (_EntryType == PositiveFloat) ||
      (_EntryType == AlphaNumSpace) || (_EntryType == AlphaNum) ||
      (_EntryType == Alpha) || (_EntryType == Filename) ||
      (_EntryType == PlayerName))
    return false;
...
bool CGroupEditBox::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::key) {
    ...
    // KEY EVENT //
    ///////////////
    const NLGUI::CEventDescriptorKey &rEDK =
        (const NLGUI::CEventDescriptorKey &)event;
    switch (rEDK.getKeyEventType()) {
    case NLGUI::CEventDescriptorKey::keychar:
      handleEventChar(rEDK);
      break;
    case NLGUI::CEventDescriptorKey::keystring:
    ...
    if (rEDK.getKeyEventType() == NLGUI::CEventDescriptorKey::keychar ||
        rEDK.getKeyEventType() == NLGUI::CEventDescriptorKey::keystring)
      return true;
    // Else filter the EventKeyDown AND EventKeyUp.
    else {
      // Look into the input handler Manager if the key combo has to be
      // considered as handled
      if ((CGroupEditBox::comboKeyHandler != NULL) &&
          comboKeyHandler->isComboKeyChat(rEDK))
        return true;
      else
        return false;
    }
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      _ViewText->getCharacterIndexFromPosition(
          eventDesc.getX() - _ViewText->getXReal(),
          eventDesc.getY() - _ViewText->getYReal(), newCurPos,
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      NLGUI::CEventDescriptorActiveCalledOnParent &activeEvent =
          (NLGUI::CEventDescriptorActiveCalledOnParent &)eventDesc;
...
void CGroupEditBox::setupDisplayText() {
  if (_ViewText) {
    ...
    if (_EntryType == Password) {
      usTmp = CUtfStringView(_Prompt + ::u32string(_InputString.size(), 0x2022))
                  .toUtf8();
    } else {
...
void CGroupEditBox::updateCoords() {
  ...
  if (_ViewText) {
    ...
    // if the length of the view text exceed our real size, ensure the Cursor
    // position is at least visible
    ...
    if (_ViewText->getWReal() > _WReal) {
      ...
      _ViewText->getCharacterPositionFromIndex(
          _CursorPos + (sint)_Prompt.size(), false, xCursVT, yTmp, hTmp);
      // Get the cursor pos from the BL of the edit box
      xCurs = xCursVT - (_XReal - _ViewText->getXReal());
      // If the cursor is outside the edit box move the view text to show the
      // cursor
      if (xCurs > _WReal) {
  ...
  if (_BackupFatherContainerPos) {
    CGroupContainerBase *gc =
        static_cast<CGroupContainerBase *>(getEnclosingContainer());
...
// ----------------------------------------------------------------------------
void CGroupEditBox::clearViews() { CInterfaceGroup::clearViews(); }
...
void CGroupEditBox::createViewText() {
  ...
  sint32 w, h;
  w = std::max(sint32(_ViewText->getFontWidth() *
                      CUtfStringView(_ViewText->getText()).count()),
               getW());
...
void CGroupEditBox::setup() {
  ...
  if (_ViewText == NULL)
    _ViewText =
        dynamic_cast<CViewText *>(CInterfaceGroup::getView("edit_text"));
  ...
  // For MultiLine editbox, clip the end space, else weird when edit space at
  // end of line (nothing happens)
  if (_ViewText)
...
// ***************************************************************************
sint32 CGroupEditBox::getMaxUsedW() const { return _W; }

// ***************************************************************************
sint32 CGroupEditBox::getMinUsedW() const { return _W; }
...
void CGroupEditBox::onLoadConfig() {
  // config is not saved when there's an empty string, so restore that default
  // state.
  setInputStringRef(::u32string());
  _CurrSelection = NULL;
  _PrevNumLine = 1;
...
void CGroupEditBox::onKeyboardCaptureLost() {
  if (!_AHOnFocusLost.empty())
    CAHManager::getInstance()->runActionHandler(_AHOnFocusLost, this,

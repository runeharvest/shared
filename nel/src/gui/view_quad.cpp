...
CViewQuad::CViewQuad(const TCtorParam &param)
    : CViewBase(param), _Color(CRGBA::White), _Additif(false), _UMin(0.f),
      _UMax(1.f), _WrapMode(Repeat) {
...
// *********************************************************************************
bool CViewQuad::parse(xmlNodePtr /* cur */,
...
void CViewQuad::updateCoords() {
  ...
  _RealQuad.set(_Quad.V0 + delta, _Quad.V1 + delta, _Quad.V2 + delta,
                _Quad.V3 + delta);
}
  ...
  /*if (_InheritGCAlpha)
  {
      ...
      while (gr)
      {
          if (gr->isGroupContainer())
          {
              ...
              col.A =
  (uint8)(((sint32)col.A*((sint32)gc->getCurrentContainerAlpha()+1))>>8); break;
          }
          gr = gr->getParent();
      }
  }*/
    ...
    if (_WrapMode == Repeat) {
      if (_UMax == _UMin) {
        ...
        float unitRatio =
            1.f /
            fabsf(umax -
                  umin); // ratio of the real quad delta x in screen for du = 1
        ...
        if (firstDeltaU != 0.f) {
          ...
          // start quad
          quv.V0 = srcQuad.V0;
          quv.V1 = blend(srcQuad.V0, srcQuad.V1,
                         std::min(1.f, (firstDeltaU * unitRatio)));
          quv.V2 = blend(srcQuad.V3, srcQuad.V2,
                         std::min(1.f, (firstDeltaU * unitRatio)));
          quv.V3 = srcQuad.V3;
          ...
          if (firstDeltaU * unitRatio >= 1.f)
            return;
        }

        // TODO optim: reuse of previous uv & pos ... (prb is that they are not
        // always computed)
      ...
      if (_UMin == _UMax) {
        ...
        float startRatio =
            -umin / (umax - umin); // start of unclamped u (actually (0.f -
                                   // umin) / (umax - umin) )
        if (umin < 0.f) {
          quv.V0 = srcQuad.V0;
        ...
        if (endRatio > 0.f) {
          ...
          // TODO optim: reuse of previous uv & pos ... (prb is that they are
          // not always computed)
...
// *********************************************************************************
void CViewQuad::setAlpha(sint32 a) { _Color.A = (uint8)a; }
...
// *********************************************************************************
void CViewQuad::setQuad(const NLMISC::CVector &start,
                        const NLMISC::CVector &end, float thickness) {
...
// *********************************************************************************
void CViewQuad::setQuad(const NLMISC::CVector &pos, float radius,
                        float angle /*=0.f*/) {
  if (angle == 0.f) {
...
// *********************************************************************************
void CViewQuad::setQuad(const std::string &texName,
                        const NLMISC::CVector &srcPos, float angle /*= 0.f*/,
                        float offCenter /* = 0.f*/) {
...
// *********************************************************************************
void CViewQuad::setAdditif(bool additif) { _Additif = additif; }

...
// ----------------------------------------------------------------------------
CGroupCell::CGroupCell(const TCtorParam &param) : CInterfaceGroup(param) {
  NewLine = false;
  TableRatio = 0.f;
  WidthWanted = 0;
  Height = 0;
...
std::string CGroupCell::getProperty(const std::string &name) const {
  ...
  } else if (name == "ignore_max_width") {
    return toString(IgnoreMaxWidth);
  } else if (name == "ignore_min_width") {
    return toString(IgnoreMinWidth);
  } else if (name == "add_child_w") {
    return toString(AddChildW);
  } else
    return CInterfaceGroup::getProperty(name);
}

void CGroupCell::setProperty(const std::string &name,
...
xmlNodePtr CGroupCell::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "left_margin",
             BAD_CAST toString(LeftMargin).c_str());
  xmlSetProp(node, BAD_CAST "nowrap", BAD_CAST toString(NoWrap).c_str());
  if (Background)
    xmlSetProp(node, BAD_CAST "bgcolor",
               BAD_CAST toString(Background->getColor()).c_str());

  if (WidthWanted != 0)
    xmlSetProp(node, BAD_CAST "width", BAD_CAST toString(WidthWanted).c_str());
  else
    xmlSetProp(node, BAD_CAST "width",
               BAD_CAST toString(TableRatio * 100.0f).c_str());

  xmlSetProp(node, BAD_CAST "height", BAD_CAST toString(Height).c_str());
  xmlSetProp(node, BAD_CAST "ignore_max_width",
             BAD_CAST toString(IgnoreMaxWidth).c_str());
  xmlSetProp(node, BAD_CAST "ignore_min_width",
             BAD_CAST toString(IgnoreMinWidth).c_str());
  xmlSetProp(node, BAD_CAST "add_child_w",
             BAD_CAST toString(AddChildW).c_str());
...
// ----------------------------------------------------------------------------
bool CGroupCell::parseCell(xmlNodePtr cur, CInterfaceGroup *parentGroup,
  ...
  if (!ptr) {
    ...
    std::string newId =
        NLMISC::toString("cell_%d_%d", (int)columnIndex, (int)rowIndex);
    xmlSetProp(cur, (const xmlChar *)"id", (const xmlChar *)newId.c_str());
  }
  // parse enclosed group
  if (!Group->parse(cur, this))
    return false;
  Group->setId(parentGroup->getId() + Group->getId());
  setEnclosedGroupDefaultParams();
  // parse the children
  bool ok = CWidgetManager::getInstance()->getParser()->parseGroupChildren(
      cur, Group, false);
  if (!ok)
    return false;
  // align
  ...
  if (ptr) {
    if (strcmp((const char *)ptr, "left") == 0) {
      ...
      nlwarning("<CGroupCell::parse> Unknwown or unsupported align mode : %s",
                (const char *)ptr);
    }
  }
  // v-align
  ...
  if (ptr) {
    if (strcmp((const char *)ptr, "top") == 0) {
      ...
      nlwarning("<CGroupCell::parse> Unknwown or unsupported valign mode : %s",
                (const char *)ptr);
    }
  }
  // left margin
...
void CGroupCell::draw() {
  if (CGroupCell::DebugUICell) {
    ...
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, _WReal, 1, 0, false,
                          rVR.getBlankTextureId(), CRGBA(0, 255, 255, 255));
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal, 1, _HReal, 0, false,
                          rVR.getBlankTextureId(), CRGBA(0, 255, 255, 255));
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal + _HReal - 1, _WReal, 1,
                          0, false, rVR.getBlankTextureId(),
                          CRGBA(0, 255, 255, 255));
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _WReal - 1, _YReal, 1, _HReal,
                          0, false, rVR.getBlankTextureId(),
                          CRGBA(0, 255, 255, 255));
  }
...
// ----------------------------------------------------------------------------
void CGroupCell::updateCoords() { CInterfaceGroup::updateCoords(); }

// ----------------------------------------------------------------------------
NLMISC_REGISTER_OBJECT(CViewBase, CGroupTable, std::string, "table");

CGroupTable::CGroupTable(const TCtorParam &param) : CInterfaceGroup(param) {
...
void CGroupTable::updateCoords() {
    ...
    if (!_ContentValidated) {
      ...
      for (i = 0; i < _Cells.size(); i++) {
        ...
        if (!cell->IgnoreMaxWidth)
          ...
          cell->WidthMax =
              cell->WidthWanted + additionnalWidth + cell->LeftMargin;

        sint32 cellWidth;
        if (!cell->IgnoreMinWidth)
          cellWidth = cell->NoWrap ? cell->WidthMax
                                   : cell->getMinUsedW() + cell->LeftMargin;
        else
        ...
        if ((cell->WidthWanted + additionnalWidth) * colspan >
            _Columns[column].WidthWanted)
          _Columns[column].WidthWanted =
              (sint32)((cell->WidthWanted + additionnalWidth) * colspan);
      ...
      // Get the width
      sint32 tableWidthMax =
          innerForceWidthMin ? innerForceWidthMin
                             : _LastParentW - tableBorderSpacing; // getWReal();
      sint32 tableWidthMin = std::max(
          innerForceWidthMin, (sint32)((float)tableWidthMax * TableRatio));
      tableWidthMax = std::max((sint32)0, tableWidthMax);
      tableWidthMin = std::max((sint32)0, tableWidthMin);

      // Get the width of the table and normalize percent of the cell (sum of
      // TableRatio must == 1)
      sint32 tableWidth = 0;
      ...
      if (innerForceWidthMin > 0)
        tableWidthMax = std::min(_LastParentW - tableBorderSpacing,
                                 std::max(tableWidthMax, tableWidth));
      else
        tableWidthMax = std::min(_LastParentW - tableBorderSpacing,
                                 std::max(tableWidthMax, tableMaxContentWidth));
      ...
      for (i = 0; i < _Columns.size(); i++) {
        if (_Columns[i].TableRatio > 0) {
          ...
          sint32 me =
              (sint32)((float)_Columns[i].Width / _Columns[i].TableRatio);

          tableWidthSizeAfterPercent =
              std::min(tableWidthMax, std::max(tableWidthSizeAfterPercent, me));
        }
      }
      for (i = 0; i < _Columns.size(); i++) {
        if (_Columns[i].TableRatio > 0) {
          ...
          if (_Columns[i].TableRatio < 1.f)
            cellSize = (sint32)((float)tableWidthSizeAfterPercent *
                                _Columns[i].TableRatio) +
                       1;
          else
            cellSize = 10000;

          sint32 diff =
              std::min(tableWidthMax, (std::max(_Columns[i].Width, cellSize) -
                                       _Columns[i].Width) +
                                          tableWidth) -
              tableWidth;
          tableWidth += diff;
          _Columns[i].Width += diff;
          // tableMaxContentWidth = std::max(tableMaxContentWidth,
          // (sint32)((float)_Columns[i].WidthMax / _Columns[i].TableRatio));
        }
      }
      ...
      for (i = 0; i < _Columns.size(); i++) {
        // Width for the cell
        sint32 diff = (sint32)((float)tableWidth * _Columns[i].TableRatio) -
                      _Columns[i].Width;
      ...
      sint32 finalWidth = std::max(
          tableWidthMin,
          std::min(std::max(tableWidth, tableMaxContentWidth), tableWidthMax));
        ...
        if (space > 0) {
          ...
          for (i = 0; i < _Columns.size(); i++) {
            ...
            if (_Columns[i].Width < _Columns[i].WidthWanted) {
              sint32 dif =
                  std::min(space, _Columns[i].WidthWanted - _Columns[i].Width);
              _Columns[i].Width += dif;
              space -= dif;
            }
          }
          ...
          if (space > 0) {
            ...
            // Distribute remaining space in resizable cells that have a
            // WidthMax
            sint32 sumDeltaWidth = 0;
            for (i = 0; i < _Columns.size(); i++) {
              if ((_Columns[i].TableRatio == 0) &&
                  (_Columns[i].WidthWanted == 0))
                sumDeltaWidth += std::max((sint32)0, _Columns[i].WidthMax -
                                                         _Columns[i].Width);
            }
            if (sumDeltaWidth) {
              ...
              for (i = 0; i < _Columns.size(); i++) {
                if ((_Columns[i].TableRatio == 0) &&
                    (_Columns[i].WidthWanted == 0)) {
                  sint32 marge = std::max((sint32)0, _Columns[i].WidthMax -
                                                         _Columns[i].Width);
                  sint32 cellPart =
                      std::min(marge, std::min(space, 1 + marge * toDistribute /
                                                              sumDeltaWidth));
                  if (cellPart) {
              ...
              if (sumDeltaWidth) {
                ...
                for (i = 0; i < _Columns.size(); i++) {
                  if ((_Columns[i].TableRatio == 0) &&
                      (_Columns[i].WidthWanted == 0)) {
                    sint32 cellPart =
                        std::min(space, 1 + toDistribute / sumDeltaWidth);
                    _Columns[i].Width += cellPart;
                    space -= cellPart;
                  }
                }
              ...
              if (space > 0) {
                ...
                if (space > 0) {
                  ...
                  if (innerWidth > 0 && finalWidth > innerWidth)
                    _Columns[_Columns.size() - 1].Width +=
      ...
      // *** Now we know each column width, resize cells and get the height for
      // each row
      ...
      for (i = 0; i < _Cells.size(); i++) {
        ...
        // inner
        cell->Group->setX(cellBorderPaddingLeft + alignmentX +
                          cell->LeftMargin);
        cell->Group->setW(columnWidth - widthReduceX - cellBorderPaddingLeft -
        ...
        sint32 cellHeight = std::max(
            (sint32)(cell->Height * rowspan),
            (sint32)(cell->Group->getH() * rowspan + cellBorderPadding));
      ...
      for (i = 0; i < _Cells.size(); i++) {
        ...
        // inner
        cell->Group->setY(
            -(alignmentY + cellBorderPaddingTop - cellBorderPaddingBottom));
      }
  ...
  for (uint32 i = 0; i < _Cells.size(); i++) {
    if (_Cells[i]->Border) {
      _Cells[i]->Border->setRect(_Cells[i]->_XReal, _Cells[i]->_YReal,
                                 _Cells[i]->_WReal, _Cells[i]->_HReal);
    }
...
void CGroupTable::setProperty(const std::string &name,
                              const std::string &value) {
  if (name == "border") {
    sint32 i;
    if (fromString(value, i)) {
...
xmlNodePtr CGroupTable::serialize(xmlNodePtr parentNode,
                                  const char *type) const {
  xmlNodePtr node = CInterfaceGroup::serialize(parentNode, type);
  if (node == NULL)
    return NULL;
  ...
  if (Border) {
    xmlSetProp(node, BAD_CAST "border",
               BAD_CAST toString(Border->getTopWidth()).c_str());
    xmlSetProp(node, BAD_CAST "bordercolor",
               BAD_CAST toString(Border->getTopColor()).c_str());
  }
  xmlSetProp(node, BAD_CAST "cellpadding",
             BAD_CAST toString(CellPadding).c_str());
  xmlSetProp(node, BAD_CAST "cellspacing",
             BAD_CAST toString(CellSpacing).c_str());
  if (Background) {
    xmlSetProp(node, BAD_CAST "bgcolor",
               BAD_CAST toString(Background->getColor()).c_str());
    // TODO: Texture
  ...
  if (ForceWidthMin != 0)
    xmlSetProp(node, BAD_CAST "width",
               BAD_CAST toString(ForceWidthMin).c_str());
  else
    xmlSetProp(node, BAD_CAST "width",
               BAD_CAST toString(TableRatio * 100.0f).c_str());

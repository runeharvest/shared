...
// ***************************************************************************
CViewTextID::CViewTextID(const std::string &id,
...
xmlNodePtr CViewTextID::serialize(xmlNodePtr parentNode,
  ...
  if (_DBTextId.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "textid",
               BAD_CAST _DBTextId.getNodePtr()->getFullName().c_str());
  else
  ...
  if (!_Initialized) {
    ...
    for (;;) {
      ...
      if (index == string::npos)
        break;
      result =
          result.substr(0, index) + result.substr(index + 7, result.size());
    }
...
// ***************************************************************************
uint32 CViewTextID::getTextId() const { return _TextId; }

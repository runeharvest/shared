...
// ----------------------------------------------------------------------------
void CDBViewBar::parseValProp(xmlNodePtr cur, CInterfaceProperty &dbProp,
...
std::string CDBViewBar::getProperty(const std::string &name) const {
  if (name == "value") {
    ...
    if (_Type == ViewBar_Mini)
      ...
      return "false";
  } else if (name == "ultra_mini") {
    if (_Type == ViewBar_UltraMini)
      return "true";
    else
      return "false";
  } else if (name == "mini_thick") {
...
void CDBViewBar::setProperty(const std::string &name,
...
xmlNodePtr CDBViewBar::serialize(xmlNodePtr parentNode,
  ...
  if (_Value.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "value",
               BAD_CAST _Value.getNodePtr()->getFullName().c_str());
  else
    xmlSetProp(node, BAD_CAST "value", BAD_CAST toString(_RangeInt).c_str());

  if (_Range.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "range",
               BAD_CAST _Range.getNodePtr()->getFullName().c_str());
  else
    xmlSetProp(node, BAD_CAST "range", BAD_CAST toString(_RangeInt).c_str());

  if (_Reference.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "reference",
               BAD_CAST _Reference.getNodePtr()->getFullName().c_str());
  else
    xmlSetProp(node, BAD_CAST "reference",
               BAD_CAST toString(_ReferenceInt).c_str());

  xmlSetProp(node, BAD_CAST "color_negative",
             BAD_CAST toString(_ColorNegative).c_str());
...
void CDBViewBar::setType(TViewBar vb) {
  ...
  switch (_Type) {
  case ViewBar_Normal:
    _Slot.setTexture("w_slot_jauge_1.tga");
    break;
  case ViewBar_Mini:
    _Slot.setTexture("w_slot_jauge_1_mini.tga");
    break;
  case ViewBar_UltraMini:
    _Slot.setTexture("w_slot_jauge_1_umin.tga");
    break;
  case ViewBar_MiniThick:
    _Slot.setTexture("w_slot_jauge_1_tmin.tga");
    break;
  }
  ...
  case ViewBar_Mini:
    ...
    break;
  case ViewBar_UltraMini:
    setTexture("w_jauge_fill_umin.tga");
    break;
  case ViewBar_MiniThick:
    setTexture("w_jauge_fill_tmin.tga");
    break;
  }
...
// ----------------------------------------------------------------------------
sint64 CDBViewBar::getCurrentValProp(const CInterfaceProperty &dbProp,
                                     sint32 intProp) {
...
void CDBViewBar::draw() {
  ...
  color.A = (uint8)(((sint32)color.A * ((sint32)CWidgetManager::getInstance()
  ...
  rVR.drawRotFlipBitmap(_RenderLayer, _XReal + 2, _YReal + deltaY, _WReal,
                        _HBar, 0, false, _TextureId, color);
}

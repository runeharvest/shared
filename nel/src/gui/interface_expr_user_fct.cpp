...
//////////////////////////////////////////////////////////////////////////////////////////////////////
// Evaluate all of the args, and return 0, so may be used to create dependances
// on database entries //
//////////////////////////////////////////////////////////////////////////////////////////////////////
...
#define CREATE_CMP_OPERATOR(op, name)                                          \
  static DECLARE_INTERFACE_USER_FCT(userFct##name) {                           \
    ...
    if (args.size() != 2 || !args[0].isNumerical() ||                          \
        !args[1].isNumerical()) {                                              \
...
static DECLARE_INTERFACE_USER_FCT(userMakeRGB) {
  ...
  NLMISC::CRGBA col((uint8)args[0].getInteger(), (uint8)args[1].getInteger(),
                    (uint8)args[2].getInteger(), nAlpha);
  result.setRGBA(col);
...
static DECLARE_INTERFACE_USER_FCT(secondsToTimeString) {
  ...
  if (nVal > 0) {
    sTmp = toString(nVal % 60) + " " + CI18N::get("uiMissionTimerMinute") +
           " " + sTmp;

    nVal = nVal / 60;

    if (nVal > 0) {
      sTmp = toString(nVal % 24) + " " + CI18N::get("uiMissionTimerHour") +
             " " + sTmp;
      ...
      if (nVal > 0) {
        sTmp =
            toString(nVal) + " " + CI18N::get("uiMissionTimerDay") + " " + sTmp;
      }
    }
  }
...
REGISTER_INTERFACE_USER_FCT("secondsToTimeStringShort",
...
static DECLARE_INTERFACE_USER_FCT(oldvalue) {
  ...
  CCDBNodeLeaf *nl =
      NLGUI::CDBManager::getInstance()->getDbProp(args[0].getString());
  if (!nl) {

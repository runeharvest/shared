...
// ***************************************************************************
CCtrlBaseButton::CCtrlBaseButton(const TCtorParam &param)
    : CCtrlBase(param), _Type(ToggleButton) {
  _Pushed = _Over = false;
  _Frozen = false;
  _FrozenHalfTone = true;
  _OverWhenPushed = true;
  _ColorOver = _ColorPushed = _ColorNormal = NLMISC::CRGBA(255, 255, 255, 255);
  _ModulateGlobalColorNormal = _ModulateGlobalColorPushed =
      _ModulateGlobalColorOver = true;
...
std::string CCtrlBaseButton::getProperty(const std::string &name) const {
  ...
  } else if (name == "frozen_half_tone") {
    return toString(_FrozenHalfTone);
  } else
    return CCtrlBase::getProperty(name);
}

void CCtrlBaseButton::setProperty(const std::string &name,
...
xmlNodePtr CCtrlBaseButton::serialize(xmlNodePtr parentNode,
  ...
  xmlNewProp(node, BAD_CAST "over_when_pushed",
             BAD_CAST toString(_OverWhenPushed).c_str());
  xmlNewProp(node, BAD_CAST "clicked_when_pushed",
             BAD_CAST toString(_ClickWhenPushed).c_str());
  xmlNewProp(node, BAD_CAST "color", BAD_CAST getColorAsString().c_str());
  xmlNewProp(node, BAD_CAST "col_pushed",
             BAD_CAST getColorPushedAsString().c_str());
  xmlNewProp(node, BAD_CAST "col_over",
             BAD_CAST getColorOverAsString().c_str());
  xmlNewProp(node, BAD_CAST "global_color_normal",
             BAD_CAST toString(_ModulateGlobalColorNormal).c_str());
  xmlNewProp(node, BAD_CAST "global_color_pushed",
             BAD_CAST toString(_ModulateGlobalColorPushed).c_str());
  xmlNewProp(node, BAD_CAST "global_color_over",
             BAD_CAST toString(_ModulateGlobalColorOver).c_str());
  ...
  xmlNewProp(node, BAD_CAST "onclick_l",
             BAD_CAST getAHString("onclick_l").c_str());
  xmlNewProp(node, BAD_CAST "params_l",
             BAD_CAST _getParamsOnLeftClick().c_str());
  xmlNewProp(node, BAD_CAST "ondblclick_l",
             BAD_CAST getAHString("ondblclick_l").c_str());
  xmlNewProp(node, BAD_CAST "params_dblclick_l",
             BAD_CAST _AHLeftDblClickParams.toString().c_str());
  xmlNewProp(node, BAD_CAST "onlongclick_l",
             BAD_CAST getAHString("onlongclick_l").c_str());
  xmlNewProp(node, BAD_CAST "params_longclick_l",
             BAD_CAST _AHLeftLongClickParams.toString().c_str());
  xmlNewProp(node, BAD_CAST "onclick_r",
             BAD_CAST getAHString("onclick_r").c_str());
  xmlNewProp(node, BAD_CAST "params_r",
             BAD_CAST _AHRightClickParams.toString().c_str());
  xmlNewProp(node, BAD_CAST "onclock_tick",
             BAD_CAST getAHString("onclock_tick").c_str());
  xmlNewProp(node, BAD_CAST "params_clock_tick",
             BAD_CAST _AHClockTickParams.toString().c_str());

  xmlNewProp(node, BAD_CAST "menu_l",
             BAD_CAST _ListMenuLeft.toString().c_str());
  xmlNewProp(node, BAD_CAST "menu_r",
             BAD_CAST _ListMenuRight.toString().c_str());

  if (_ListMenuLeft.toString() == _ListMenuRight.toString())
    xmlNewProp(node, BAD_CAST "menu_b",
               BAD_CAST _ListMenuLeft.toString().c_str());
  else
    xmlNewProp(node, BAD_CAST "menu_b", BAD_CAST "");

  xmlNewProp(node, BAD_CAST "frozen", BAD_CAST toString(_Frozen).c_str());
  xmlNewProp(node, BAD_CAST "frozen_half_tone",
             BAD_CAST toString(_FrozenHalfTone).c_str());
...
bool CCtrlBaseButton::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  CAHManager::getInstance()->parseAH(cur, "ondblclick_l", "params_dblclick_l",
                                     _AHOnLeftDblClick, _AHLeftDblClickParams);
  CAHManager::getInstance()->parseAH(cur, "onclick_r", "params_r",
                                     _AHOnRightClick, _AHRightClickParams);
  CAHManager::getInstance()->parseAH(cur, "onlongclick_l", "params_longclick_l",
                                     _AHOnLeftLongClick,
                                     _AHLeftLongClickParams);
  CAHManager::getInstance()->parseAH(cur, "onclock_tick", "params_clock_tick",
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_AHOnLeftDblClick) {
        if ((CCtrlBaseButton *)_LastLeftClickButton == this &&
            (T1 - _LastLeftClickDate) <
                CWidgetManager::getInstance()->getUserDblClickDelay()) {
          CAHManager::getInstance()->runActionHandler(_AHOnLeftDblClick, this,
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_LeftDblClickHandled) // no effect on mouse up after double click has
                                // been handled
      {
        _LeftDblClickHandled = false;
        return true;
      ...
      // Do not launch 2 times action handler if we are already pushed ! except
      // if we want.
      if (!_ClickWhenPushed) {
      ...
      */
      ...
      if (CWidgetManager::getInstance()->getCapturePointerLeft() == NULL)
        return true; // event handler may release cpature from this object (if
                     // it is removed for example)
    ...
    if (systemEvent.getEventTypeExtended() ==
      ...
      if (_AHOnClockTick != NULL) {
        CAHManager::getInstance()->runActionHandler(_AHOnClockTick, this,
                                                    _AHClockTickParams);
      }
      ...
      if (CWidgetManager::getInstance()->getCapturePointerLeft() == this) {
        if (!_LeftLongClickHandled) {
          ...
          CCDBNodeLeaf *pNL = NLGUI::CDBManager::getInstance()->getDbProp(
              "UI:SAVE:KEY_REPEAT_SPEED");
          if (pNL != NULL)
            nVal = pNL->getValue32();
          uint repeatDelay =
              (uint)(KEY_REPEAT_MIN +
                     (KEY_REPEAT_MAX - KEY_REPEAT_MIN) * (float)nVal / 100.0f);
          if ((T1 - _LeftLongClickDate) > repeatDelay) {
            _LeftLongClickHandled = true;
            CAHManager::getInstance()->runActionHandler(
                _AHOnLeftLongClick, this, _AHLeftLongClickParams);
          }
        }
      }
    }

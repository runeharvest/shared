...
void CCtrlPolygon::updateBoudingRect() {
  ...
  for (uint k = 0; k < numVerts; ++k) {
    ...
    computeScaledVertex(finalPos,
                        CVector2f(_Poly.Vertices[k].x, _Poly.Vertices[k].y));
...
void CCtrlPolygon::setVertices(const std::vector<NLMISC::CVector> &vertices) {
  ...
  // nlinfo("%d ms for CCtrlPolygon::setVertices", (int) (1000 *
  // CTime::ticksToSecond(endTime - startTime)));
}
...
// *********************************************************************************
// TMP TMP
void CCtrlPolygon::computeScaledVertex(NLMISC::CVector2f &dest,

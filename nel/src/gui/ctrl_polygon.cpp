...
// *********************************************************************************
CCtrlPolygon::CCtrlPolygon(const TCtorParam &param)
    : CCtrlBase(param)
{
	// Construct
	_Color = CRGBA::White;
...
void CCtrlPolygon::setVertices(const std::vector<NLMISC::CVector> &vertices)
{
	...
	if (vertices.size() == _Poly.Vertices.size() && std::equal(vertices.begin(), vertices.end(), _Poly.Vertices.begin())) return; // remains unchanged
...
{
	return (sint32)maxCorner.x < (cx + cw) && (sint32)minCorner.x >= cx && (sint32)maxCorner.y < (cy + ch) && (sint32)minCorner.y >= cy;
}
...
{
	return (sint32)minCorner.x >= (cx + cw) || (sint32)maxCorner.x < cx || (sint32)minCorner.y >= (cy + ch) || (sint32)maxCorner.y < cy;
}

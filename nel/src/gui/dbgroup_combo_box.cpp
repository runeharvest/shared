...
// Compare strings
static inline bool lt_text(const std::pair<int, std::string> &s1,
                           const std::pair<int, std::string> &s2) {
...
void CDBGroupComboBox::setProperty(const std::string &name,
...
xmlNodePtr CDBGroupComboBox::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "linked_to_db",
             BAD_CAST toString(_LinkedToDB).c_str());

  if (_Selection.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "value",
               BAD_CAST _Selection.getNodePtr()->getFullName().c_str());
  else
...
bool CDBGroupComboBox::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop)
    _AHOnSelectStart = (const char *)prop;
  ...
  if (prop)
    _AHOnChangeParams = (const char *)prop;
...
// ***************************************************************************
void CDBGroupComboBox::dirt() { _DirtySelection = true; }
...
// ***************************************************************************
CViewText *CDBGroupComboBox::getViewText() { return _ViewText; }
...
void CDBGroupComboBox::setup() {
  ...
  // get the button
  _SelectButton =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("select"));
...
int CDBGroupComboBox::luaInsertText(CLuaState &ls) {
  ...
  ucstring text;
  nlverify(CLuaIHM::pop(ls, text));
  insertText((uint)ls.toInteger(1),
             text.toUtf8()); // FIXME: Lua should just do UTF-8!
#else
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TSTRING);
  string text = ls.toString(2);
...
int CDBGroupComboBox::luaSetTexture(CLuaState &ls) {
  ...
  setTexture((uint)ls.toInteger(1),
             texture.toUtf8()); // FIXME: Lua should just do UTF-8!
#else
  CLuaIHM::checkArgType(ls, funcName, 2, LUA_TSTRING);
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    CDBGroupComboBox *pCB =
        dynamic_cast<CDBGroupComboBox *>(pCaller->getParent());
    if (pCB == NULL)
      return;
    // if no choice, return.
    ...
    if (!groupMenu) {
      groupMenu = dynamic_cast<CGroupMenu *>(
          CWidgetManager::getInstance()->getElementFromId(
    ...
    // Must ensure the combo menu has same windows priority than the combo box
    // window

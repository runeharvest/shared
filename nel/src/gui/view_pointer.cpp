...
// --------------------------------------------------------------------------------------------------------------------
CViewPointer::CViewPointer(const TCtorParam &param) : CViewPointerBase(param) {
  _TxIdDefault = -2;
  _TxIdMoveWindow = -2;
  _TxIdResizeBRTL = -2;
  _TxIdResizeBLTR = -2;
...
bool CViewPointer::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop)
    _TxDefault = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_move_window");
  if (prop)
    _TxMoveWindow = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_resize_BR_TL");
  if (prop)
    _TxResizeBRTL = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_resize_BL_TR");
  if (prop)
    _TxResizeBLTR = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_resize_TB");
  if (prop)
    _TxResizeTB = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_resize_LR");
  if (prop)
    _TxResizeLR = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_rotate");
  if (prop)
    _TxRotate = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_scale");
  if (prop)
    _TxScale = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_colpick");
  if (prop)
    _TxColPick = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_pan");
  if (prop)
    _TxPan = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_can_pan");
  if (prop)
    _TxCanPan = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_pan_r2");
  if (prop)
    _TxPanR2 = NLMISC::toLowerAscii((const char *)prop);

  prop = (char *)xmlGetProp(cur, (xmlChar *)"tx_can_pan_r2");
  if (prop)
    _TxCanPanR2 = NLMISC::toLowerAscii((const char *)prop);
...
void CViewPointer::draw() {
  ...
  if (CWidgetManager::getInstance()->isIngame())
    if (!_StringCursor) {
      ...
      templateParams.push_back(
          std::pair<std::string, std::string>("id", "string_cursor"));

      _StringCursor =
          CWidgetManager::getInstance()->getParser()->createGroupInstance(
              "string_cursor", "", templateParams);
      if (_StringCursor)
        _StringCursor->setParentPos(
            CWidgetManager::getInstance()->getElementFromId("ui:interface"));

      templateParams.clear();
      templateParams.push_back(
          std::pair<std::string, std::string>("id", "string_cursor_hardware"));
      _StringCursorHardware =
          CWidgetManager::getInstance()->getParser()->createGroupInstance(
              "string_cursor_hardware", "", templateParams);
      if (_StringCursorHardware)
        _StringCursorHardware->setParentPos(
            CWidgetManager::getInstance()->getElementFromId("ui:interface"));
    }

  CRGBA col;
  if (getModulateGlobalColor())
    col.modulateFromColor(_Color,
  ...
  if (CWidgetManager::getInstance()->getCapturePointerLeft() != NULL &&
    ...
    CCtrlMover *pCM = dynamic_cast<CCtrlMover *>(
  ...
  if (pCB != NULL) {
    if (drawResizer(pCB, col))
      return;
    if (drawColorPicker(pCB, col))
      return;
    if (drawRotate(pCB, col))
      return;
    if (drawPan(pCB, col))
      return;
    if (drawCustom(pCB))
  ...
  for (uint i = 0; i < vUP.size(); i++) {
    ...
    if (vLink != NULL) {
      ...
      if (vLink->getMouseOverShape(tooltip, rot, col)) {
        ...
        CInterfaceGroup *stringCursor =
            hwMouse ? _StringCursorHardware : _StringCursor;
        if (stringCursor) {
          stringCursor->setX(_PointerX);
          stringCursor->setY(_PointerY);
          stringCursor->updateCoords();
          stringCursor->draw();
          // if in hardware mode, force to draw the default cursor no matter
          // what..
          if (hwMouse)
  ...
  if (CWidgetManager::getInstance()->isMouseHandlingEnabled()) {
    // Sorts the controls according to their depth, to approximate as best the
    // CapturePointerLeft algo. Especially important so that Resizers controls
    // get the precedence over the move control (else could randomly bug like in
    // chat group)
    ...
    for (uint i = 0; i < rICL.size(); i++) {
      CCtrlDepthEntry cde;
      cde.Ctrl = rICL[i];
      // NB: not the exact CInterfaceManager getDepth test here, but should work
      // fine
    ...
    for (uint32 i = 0; i < sortedControls.size(); ++i) {
      ...
      if (overModalWindow) {
        if (!pCB->isSonOf(currModal))
          continue;
      }

      if (drawBrowse(pCB, col))
        return;
      if (drawResizer(pCB, col))
        return;
      if (drawColorPicker(pCB, col))
      ...
      if (_LastHightLight == NULL) {
        ...
        if ((pCM != NULL) && (pCM->canMove() == true)) {
          CGroupContainer *pGC =
              dynamic_cast<CGroupContainer *>(pCM->getParent());
          if (pGC != NULL && !pGC->isLocked()) {
...
void CViewPointer::drawCursor(sint32 texId, NLMISC::CRGBA col, uint8 rot) {
  ...
  if (!hwMouse) {
    ...
    rVR.getDriver()->setCursor(name, col, rot,
                               (uint32)std::max(getX() - xPos, (sint32)0),
                               (uint32)std::max(getY() - yPos, (sint32)0));
  }
}

...
static void drawGCTestQuad(sint renderLayer, sint32 xreal, sint32 yreal,
                           sint32 wreal, sint32 hreal, CRGBA color) {
  ...
  rVR.drawRotFlipBitmap(renderLayer, xreal, yreal, wreal, hreal, 0, false,
                        rVR.getBlankTextureId(), color);
...
// ***************************************************************************
CCtrlResizer::CCtrlResizer(const TCtorParam &param) : CCtrlBase(param) {
  WMin = WMax = 0;
  HMin = HMax = 0;
...
THotSpot CCtrlResizer::getRealResizerPos() const {
  ...
  if (parent) {
    ...
    if (!IsMaxH && parent->getPopupMinH() == parent->getPopupMaxH()) {
      resizerPos =
          (THotSpot)(resizerPos & ~(Hotspot_Bx | Hotspot_Mx | Hotspot_Tx));
    }
    if (parent->getPopupMinW() == parent->getPopupMaxW()) {
      resizerPos =
          (THotSpot)(resizerPos & ~(Hotspot_xR | Hotspot_xM | Hotspot_xL));
    }
    return resizerPos;
  }
  return _ResizerPos;
...
bool CCtrlResizer::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if ((CWidgetManager::getInstance()->getCapturePointerLeft() != this) &&
        !isIn(eventDesc.getX(), eventDesc.getY()))
      return false;
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_MouseDown) {
        // nlinfo("x = %d, y = %d", eventDesc.getX() + _XReal, eventDesc.getY()
        // + _YReal);
        ...
        if (gc && gc->getAHOnResizePtr() != NULL) {
          CAHManager::getInstance()->runActionHandler(
              gc->getAHOnResize(), gc, gc->getAHOnResizeParams());
        }
      }
      return true;
    }
...
sint32 CCtrlResizer::resizeW(sint32 dx) {
  ...
  if (_Parent->getPosRef() & Hotspot_xR) {
    if (_ResizerPos & Hotspot_xR) {
      clippedNewW = std::min((sint32)sw + _Parent->getW() - _Parent->getXReal(),
                             clippedNewW);
    } else {
    ...
    if (_ResizerPos & Hotspot_xL) {
      clippedNewW =
          std::min(clippedNewW, _Parent->getXReal() + _Parent->getW());
    } else {
...
COptionsContainerInsertion *CCtrlMover::getInsertionOptions() {
  ...
  insertionOptions =
      (COptionsContainerInsertion *)CWidgetManager::getInstance()->getOptions(
          "container_insertion_opt");
  return insertionOptions;
}
...
#ifdef DRAW_GC_TEST_QUADS
  drawGCTestQuad(_RenderLayer, _XReal, _YReal, _WReal, _HReal,
                 CRGBA(255, 0, 0, 127));
#endif
  ...
  if (_MovingInParentList) {
    ...
    if (_ParentScrollingUp) {
      ...
      if (gc->getYReal() < topPosY) {
        ...
        vr.drawRotFlipBitmap(gc->getRenderLayer(), px,
                             _ParentListTop - arrowHeight - 2, arrowWidth,
                             arrowHeight, 0, 0, options->TxId_T_Arrow);
      }
    }
    //
    if (_ParentScrollingDown) {
      sint32 bottomPosY = gl->getChild(gl->getNumChildren() - 1)->getYReal() -
                          gl->getChild(gl->getNumChildren() - 1)->getHReal();
      if (gc->getYReal() - gc->getHReal() > bottomPosY) {
        ...
        // insertion position is too high, just draw an arrow pointing to top
        sint32 px = gl->getXReal() + (gl->getWReal() >> 1) - (arrowWidth >> 1);
        vr.drawRotFlipBitmap(gc->getRenderLayer(), px, _ParentListBottom + 2,
                             arrowWidth, arrowHeight, 0, 0,
                             options->TxId_B_Arrow);
      }
    }
    ...
    if (!_ParentScrollingUp && !_ParentScrollingDown) {
      ...
      if (_InsertionIndex == (sint32)gl->getNumChildren()) {
        ...
        posY = gl->getChild(_InsertionIndex)->getYReal() +
               gl->getChild(_InsertionIndex)->getHReal();
      }
      // draw insertion bar
      //
      ...
      if (posY >= _ParentListBottom && posY <= _ParentListTop) {
        ...
        vr.drawRotFlipBitmap(gc->getRenderLayer(), gl->getXReal(), py,
                             gl->getWReal(), barHeight, 0, 0,
                             options->TxId_InsertionBar);
      }
    }
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (!_Moving && !_MovingInParentList) {
      if (_CanMove) {
        ...
        if (eventDesc.getEventTypeExtended() ==
          ...
          if (gc->getAHOnBeginMovePtr()) {
            CAHManager::getInstance()->runActionHandler(
                gc->getAHOnBeginMove(), gc, gc->getAHOnBeginMoveParams());
          }
          return true;
        }
      }
      ...
      if (eventDesc.getEventTypeExtended() ==
        ...
        // if the father is a container and is popable (but not poped), move it
        // only if the user has moved the mouse enough
        if (_Parent && !_MovingInParentList) {
          if (gc && gc->getLayerSetup() != 0) {
            if (gc->isMovableInParentList() && !gc->isPopable()) {
              if (abs(y - _Parent->getY()) >
                  DELTA_BEFORE_MOVING_IN_PARENT_LIST) {
                // There should be at least one other entry in the parent list
                CGroupList *parentList =
                    dynamic_cast<CGroupList *>(gc->getParent());
                if (!parentList)
                  return false;
                if (parentList->getNbElement() == 1)
                  return false;
            ...
            if (gc->isPopable()) {
              if (!gc->isMovableInParentList()) {
                if (abs(x - _Parent->getX()) > DELTA_BEFORE_POPUP ||
                    abs(y - _Parent->getY()) > DELTA_BEFORE_POPUP) {
                ...
                if (abs(y - _Parent->getY()) >
                  ...
                  updateInsertionIndex(parentList, eventDesc.getY());
                  return true;
                } else // the mouse should move on the side of the container to
                       // turn it into a popup
                  // if (_Parent->getX() - x > DELTA_BEFORE_POPUP || x -
                  // (_Parent->getX() + _Parent->getWReal()) >
                  // DELTA_BEFORE_POPUP)
                  if (abs(x - _Parent->getX()) > DELTA_BEFORE_POPUP) {
                    setPoped(gc, x, y, eventDesc);
                    return true;
        ...
        if (!_MovingInParentList) {
          ...
          if (gc->getAHOnMovePtr()) {
            ...
            // execute the AH
            CAHManager::getInstance()->runActionHandler(
                gc->getAHOnMovePtr(), this, gc->getAHOnMoveParams());
          }
        } else {
          if (!gc)
            return false;
          ...
          if (gl) {
            ...
            // the control is moving in its parent list, so the x coordinate
            // doesn't change
            y += _MoveDeltaYReal;
            // if the group is at the bottom of screen or at the bottom of the
            // list, must clamp & scroll down
            if (y < _ParentListBottom) {
              if (_ParentScrollingUp) {
                _ParentScrollingUp = false;
                CWidgetManager::getInstance()->registerClockMsgTarget(
                    this); // want to now when time pass
              }
              if (glSciY > gl->getYReal()) // is there need for scroll ?
              {
                ...
                if (_ParentScrollingDown) {
                  _ParentScrollingDown = false;
                  CWidgetManager::getInstance()->unregisterClockMsgTarget(
                      this); // want to now when time pass
                }
              }
              y = _ParentListBottom;
              ...
              if (topY > _ParentListTop) {
                ...
                if (glSciY + glSciH <
                    gl->getYReal() +
                        gl->getHReal()) // is there need for scroll ?
                {
                  if (!_ParentScrollingUp) {
...
void CCtrlMover::handleScrolling() {
  const uint pixPerMS = 7; // the number of millisecond to move of one pixel in
                           // the parent scrollbar
  CGroupContainer *gc = dynamic_cast<CGroupContainer *>(_Parent);
  if (!gc)
    return;
  CGroupList *gl = gc->getPreviousParentList();
  ...
  if (_ParentScrollingUp) {
    sint32 topPosY = gl->getChild(0)->getYReal();
    // check if we are really at the end of the list, if this is not the case,
    // we should perform scroll on parent container with a scroll bar
    if (gc->getYReal() < topPosY) {
...
bool CCtrlMover::runTitleActionHandler() {
  ...
  if (!gc)
    return false;
  CInterfaceGroup *gr =
      gc->isOpen() ? gc->getHeaderOpened() : gc->getHeaderClosed();
  if (gr && !gr->getLeftClickHandler().empty()) {
    CAHManager::getInstance()->runActionHandler(
        gr->getLeftClickHandler(), this, gr->getLeftClickHandlerParams());
    return true;
  }
  // try with the main group
  if (!gc->getLeftClickHandler().empty()) {
    CAHManager::getInstance()->runActionHandler(
        gc->getLeftClickHandler(), this, gc->getLeftClickHandlerParams());
    return true;
  }
  return false;
}
...
// ***************************************************************************
void CCtrlMover::setPoped(CGroupContainer *gc, sint32 x, sint32 y,
                          const NLGUI::CEventDescriptorMouse &eventDesc) {
  gc->setHighLighted(false);
  sint32 deltaX = x - _Parent->getX();
  sint32 deltaY = y - _Parent->getY();
  // pop up the window
  gc->popupCurrentPos(); // NB : this has the side effect of destroying this
                         // object during the call to CGroupContainer::update(),
                         // because the mover is recreated during the setup ! So
                         // from now we shouldn't use anything that use the
                         // 'this' pointer
                         // TODO : maybe there a more clean way to do that ?
                         // (except that we may not call update() in
                         // popupCurrentPos() )
  if (gc->isLockable()) {
    gc->setLocked(false);
  }
...
// ***************************************************************************
void CCtrlMover::setMovingInParent(
    CGroupContainer *gc, sint32 /* x */, sint32 y,
    const NLGUI::CEventDescriptorMouse &eventDesc) {
  if (!gc)
    return;
...
std::string CGroupContainer::getProperty(const std::string &name) const {
  if (name == "localize") {
    ...
    if (_TitleTextOpened == _TitleTextClosed)
      ...
      return "";
  } else if (name == "title_opened") {
    return _TitleTextOpened;
  } else if (name == "title_closed") {
    return _TitleTextClosed;
  } else if (name == "header_active") {
  ...
  } else if (name == "popable") {
    return toString(_Popable);
  } else if (name == "lockable") {
    return toString(_Lockable);
  } else if (name == "locked") {
    return toString(_Locked);
  } else if (name == "openable") {
    return toString(_Openable);
  } else if (name == "opened") {
    return toString(_Opened);
  } else if (name == "modal") {
    return toString(_Modal);
  } else if (name == "open_when_popup") {
    return toString(_OpenWhenPopup);
  } else if (name == "resizer") {
    return toString(_EnabledResizer);
  } else if (name == "resizer_top_size") {
    return toString(_ResizerTopSize);
  } else if (name == "on_open") {
    return getAHString(name);
  } else if (name == "on_open_params") {
    return _AHOnOpenParams.toString();
  } else if (name == "on_close") {
    return getAHString(name);
  } else if (name == "on_close_params") {
    return _AHOnCloseParams.toString();
  } else if (name == "on_close_button") {
    return getAHString(name);
  } else if (name == "on_close_button_params") {
    return _AHOnCloseButtonParams.toString();
  } else if (name == "on_move") {
    return getAHString(name);
  } else if (name == "on_move_params") {
    return _AHOnMoveParams.toString();
  } else if (name == "on_deactive_check") {
    return getAHString(name);
  } else if (name == "on_deactive_check_params") {
    return _AHOnDeactiveCheckParams.toString();
  } else if (name == "on_resize") {
    return getAHString(name);
  } else if (name == "on_resize_params") {
    return _AHOnResizeParams.toString();
  } else if (name == "on_alpha_settings_changed") {
    return getAHString(name);
  } else if (name == "on_alpha_settings_changed_params") {
    return _AHOnAlphaSettingsChangedParams.toString();
  } else if (name == "on_begin_move") {
    return getAHString(name);
  } else if (name == "on_begin_move_params") {
    return _AHOnBeginMoveParams.toString();
  } else if (name == "max_w") {
    return toString(_MaxW);
  } else if (name == "min_w") {
    return toString(_MinW);
  } else if (name == "pop_max_w") {
    return toString(_PopupMaxW);
  } else if (name == "pop_min_w") {
    return toString(_PopupMinW);
  } else if (name == "pop_max_h") {
    return toString(_PopupMaxH);
  } else if (name == "pop_min_h") {
    return toString(_PopupMinH);
  } else if (name == "movable_in_parent_list") {
    return toString(_MovableInParentList);
  } else if (name == "savable") {
    return toString(_Savable);
  } else if (name == "active_savable") {
    return toString(_ActiveSavable);
  } else if (name == "modal_parent") {
    return _ModalParentNames;
  } else if (name == "options") {
    return _OptionsName;
  } else if (name == "title_delta_max_w") {
    return toString(_TitleDeltaMaxW);
  } else if (name == "title_over_extend_view_text") {
    return toString(_TitleOverExtendViewText);
  } else if (name == "help_page") {
    return _HelpPage.toString();
  } else
    return CInterfaceGroup::getProperty(name);
}

void CGroupContainer::setProperty(const std::string &name,
...
xmlNodePtr CGroupContainer::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "content_y_offset",
             BAD_CAST toString(_ContentYOffset).c_str());
  ...
  xmlSetProp(node, BAD_CAST "header_active",
             BAD_CAST toString(_HeaderActive).c_str());

  if (_HeaderColor.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "header_color",
               BAD_CAST _HeaderColor.getNodePtr()->getFullName().c_str());
  else
    xmlSetProp(node, BAD_CAST "header_color", BAD_CAST "");

  xmlSetProp(node, BAD_CAST "right_button",
             BAD_CAST toString(_EnabledRightButton).c_str());
  xmlSetProp(node, BAD_CAST "help_button",
             BAD_CAST toString(_EnabledHelpButton).c_str());
  ...
  xmlSetProp(node, BAD_CAST "open_when_popup",
             BAD_CAST toString(_OpenWhenPopup).c_str());
  xmlSetProp(node, BAD_CAST "resizer",
             BAD_CAST toString(_EnabledResizer).c_str());
  xmlSetProp(node, BAD_CAST "resizer_top_size",
             BAD_CAST toString(_ResizerTopSize).c_str());

  xmlSetProp(node, BAD_CAST "on_open", BAD_CAST getAHString("on_open").c_str());
  ...
  xmlSetProp(node, BAD_CAST "on_move", BAD_CAST getAHString("on_move").c_str());
  ...
  xmlSetProp(node, BAD_CAST "movable_in_parent_list",
             BAD_CAST toString(_MovableInParentList).c_str());
  xmlSetProp(node, BAD_CAST "savable", BAD_CAST toString(_Savable).c_str());
  xmlSetProp(node, BAD_CAST "active_savable",
             BAD_CAST toString(_ActiveSavable).c_str());
  xmlSetProp(node, BAD_CAST "modal_parents",
             BAD_CAST _ModalParentNames.c_str());
  xmlSetProp(node, BAD_CAST "options",
             BAD_CAST _OptionsName.toString().c_str());
  xmlSetProp(node, BAD_CAST "title_delta_max_w",
             BAD_CAST toString(_TitleDeltaMaxW).c_str());
  xmlSetProp(node, BAD_CAST "title_over_extend_view_text",
             BAD_CAST toString(_TitleOverExtendViewText).c_str());
...
bool CGroupContainer::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _Localize = convertBool((const char *)ptr);
  ...
  CAHManager::getInstance()->parseAH(cur, "on_open", "on_open_params",
                                     _AHOnOpen, _AHOnOpenParams);
  CAHManager::getInstance()->parseAH(cur, "on_close", "on_close_params",
                                     _AHOnClose, _AHOnCloseParams);
  CAHManager::getInstance()->parseAH(cur, "on_close_button",
                                     "on_close_button_params", _AHOnCloseButton,
                                     _AHOnCloseButtonParams);
  CAHManager::getInstance()->parseAH(cur, "on_move", "on_move_params",
                                     _AHOnMove, _AHOnMoveParams);
  CAHManager::getInstance()->parseAH(
      cur, "on_deactive_check", "on_deactive_check_params", _AHOnDeactiveCheck,
      _AHOnDeactiveCheckParams);
  CAHManager::getInstance()->parseAH(cur, "on_resize", "on_resize_params",
                                     _AHOnResize, _AHOnResizeParams);
  CAHManager::getInstance()->parseAH(
      cur, "on_alpha_settings_changed", "on_alpha_settings_changed_params",
      _AHOnAlphaSettingsChanged, _AHOnAlphaSettingsChangedParams);
  CAHManager::getInstance()->parseAH(cur, "on_begin_move",
                                     "on_begin_move_params", _AHOnBeginMove,
...
void CGroupContainer::updateCoords() {
  ...
  if ((_LayerSetup == 0) && bHasChild && _EnabledResizer) {
    ...
    CViewRenderer &vr = *CViewRenderer::getInstance();
    vr.getScreenSize(sw, sh);
    // ensure the maxH is > minH and < sh  (prioritary > minH, in case of
    // sh<minh)
  ...
  if ((_LayerSetup == 0) && (!bHasChild) && _EnabledResizer) {
    ...
    CViewRenderer &vr = *CViewRenderer::getInstance();
    vr.getScreenSize(sw, sh);
    if ((sint32)sw > _PopupMinW && _W > (sint32)sw)
      _W = sw;
    if ((sint32)sh > _PopupMinH && _H > (sint32)sh)
      _H = sh;
  }
  ...
  if (_LayerSetup == 0) {
    ...
    setMaxH(std::numeric_limits<sint32>::max()); // No scrollbar for container
                                                 // of layer > 0
    newH = (pLayer->H_T - pLayer->InsetT);
  }
  ...
  if (_Opened) {
    ...
    if (_Content != NULL) {
      ...
      if ((bHasChild) || (!_EnabledResizer) ||
          (_LayerSetup > 0)) // Content is constant in H
      {
        ...
        _Content->setH(-newH -
                       pLayer->H_B); // Sub header and top, bottom bitmaps
      }
    }
    ...
    if (_LayerSetup == 0) {
      // zeH is the height to substract to total height of the container to
      // obtain height of the list
      sint32 zeH =
          (pLayer->H_T - pLayer->InsetT) + pLayer->H_B_Open + pLayer->H_EM_Open;
    ...
    newH += ((_List->getHReal() < _List->getMaxH()) ? _List->getHReal()
                                                    : _List->getMaxH());
    ...
    if ((bHasChild) || (!_EnabledResizer) ||
        (_LayerSetup > 0)) // H is depending on the header and content and list
      _H = newH;
  } else // Closed
  {
  ...
  if (_ViewOpenState != NULL)
    _ViewOpenState->updateCoords();
  if (_RightButton != NULL)
    _RightButton->updateCoords();
  if (_HelpButton != NULL)
    _HelpButton->updateCoords();
  if (_ScrollBar != NULL)
...
void CGroupContainer::updateResizerSize(CCtrlResizer *cr) {
  ...
  // Yoyo: The +1 stuff is important, maybe because of the HotSpot MiddleMiddle
  // style...

  // Choose H
  if (!(cr->getResizerPos() & Hotspot_Tx) &&
      !(cr->getResizerPos() & Hotspot_Bx)) {
    // if no special topH size
    if (_ResizerTopSize < 0)
      cr->setH(_H - 2 * options->ResizerSize + 1); // Bottom and Top same value
    else
      cr->setH(_H - options->ResizerSize - _ResizerTopSize +
  ...
  // Choose W
  if (!(cr->getResizerPos() & Hotspot_xR) &&
      !(cr->getResizerPos() & Hotspot_xL)) {
...
void CGroupContainer::draw() {
  ...
  if (_Blinking) {
    ...
    if (((_BlinkDT / 300) & 1) == 0) {
      ...
      newCol.G = ~newCol.G;
      newCol.B = ~newCol.B;
      if (abs(newCol.R - oldGlobalColor.R) < 64)
        newCol.R = 192;
      if (abs(newCol.G - oldGlobalColor.G) < 64)
        newCol.G = 192;
      if (abs(newCol.B - oldGlobalColor.B) < 64)
        newCol.B = 192;
  ...
  if (parentGC) {
    // _CurrentContainerAlpha = (uint8) (((uint16)
    // parentGC->getCurrentContainerAlpha() * (uint16) _ContainerAlpha) >> 8);
    _CurrentContainerAlpha = parentGC->getCurrentContainerAlpha();
  } else {
    _CurrentContainerAlpha =
        _UseGlobalAlpha
            ? CWidgetManager::getInstance()->getGlobalContainerAlpha()
            : _ContainerAlpha;
  }
  ...
  if (parentGC) {
    // _ICurrentRolloverAlpha = (uint8) (((uint16)
    // parentGC->_ICurrentRolloverAlpha * (uint16) _ICurrentRolloverAlpha) >>
    // 8);
    ...
    uint8 rolloverFactorContent =
        _UseGlobalAlpha
            ? (255 -
               CWidgetManager::getInstance()->getGlobalRolloverFactorContent())
            : _RolloverAlphaContent;
    _ICurrentRolloverAlphaContent =
        (uint8)(255 - rolloverFactorContent +
                rolloverFactorContent * _CurrentRolloverAlphaContent);
    uint8 rolloverFactorContainer =
        _UseGlobalAlpha ? (255 - CWidgetManager::getInstance()
                                     ->getGlobalRolloverFactorContainer())
                        : _RolloverAlphaContainer;
    _ICurrentRolloverAlphaContainer =
        (uint8)(255 - rolloverFactorContainer +
                rolloverFactorContainer * _CurrentRolloverAlphaContainer);
  }
  // Modulate alpha by rollover alpha
  col.A =
      (uint8)(((uint16)_ICurrentRolloverAlphaContainer * (uint16)col.A) >> 8);
  //
  COptionsLayer *pLayer = getContainerOptions();
  ...
  bool bHasChild = (_List->getNbElement() > 0);
  h = (pLayer->H_T - pLayer->InsetT) +
      (((!_Opened) || (!bHasChild)) ? pLayer->H_B : pLayer->H_B_Open);
  ...
  if (_LayerSetup == 0) {
    // Top Left
    rVR.drawRotFlipBitmap(rl, x, y + h - pLayer->H_TL, pLayer->W_TL,
                          pLayer->H_TL, 0, false, pLayer->TxId_TL, col);
    // Top
    if (pLayer->Tile_T == 0) // Tiling ?
      rVR.drawRotFlipBitmap(rl, x + pLayer->W_TL, y + h - pLayer->H_T,
                            w - (pLayer->W_TL + pLayer->W_TR), pLayer->H_T, 0,
                            false, pLayer->TxId_T, col);
    else
      rVR.drawRotFlipBitmapTiled(rl, x + pLayer->W_TL, y + h - pLayer->H_T,
                                 w - (pLayer->W_TL + pLayer->W_TR), pLayer->H_T,
                                 0, false, pLayer->TxId_T, pLayer->Tile_T - 1,
                                 col);
    // Top Right
    rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_TR, y + h - pLayer->H_TR,
                          pLayer->W_TR, pLayer->H_TR, 0, false, pLayer->TxId_TR,
                          col);

    if ((!_Opened) || (!bHasChild)) { // Not opened
      // Left
      if (pLayer->Tile_L == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x, y + pLayer->H_BL, pLayer->W_L,
                              h - (pLayer->H_BL + pLayer->H_TL), 0, false,
                              pLayer->TxId_L, col);
      else
        rVR.drawRotFlipBitmapTiled(rl, x, y + pLayer->H_BL, pLayer->W_L,
                                   h - (pLayer->H_BL + pLayer->H_TL), 0, false,
                                   pLayer->TxId_L, pLayer->Tile_L - 1, col);
      // Right
      if (pLayer->Tile_R == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_R, y + pLayer->H_BR,
                              pLayer->W_R, h - (pLayer->H_TR + pLayer->H_BR), 0,
                              false, pLayer->TxId_R, col);
      else
        rVR.drawRotFlipBitmapTiled(rl, x + w - pLayer->W_R, y + pLayer->H_BR,
                                   pLayer->W_R,
                                   h - (pLayer->H_TR + pLayer->H_BR), 0, false,
                                   pLayer->TxId_R, pLayer->Tile_R - 1, col);
      // Bottom Left
      rVR.drawRotFlipBitmap(rl, x, y, pLayer->W_BL, pLayer->H_BL, 0, false,
                            pLayer->TxId_BL, col);
      // Bottom
      if (pLayer->Tile_B == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_BL, y,
                              w - (pLayer->W_BL + pLayer->W_BR), pLayer->H_B, 0,
                              false, pLayer->TxId_B, col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_BL, y, w - (pLayer->W_BL + pLayer->W_BR),
            pLayer->H_B, 0, false, pLayer->TxId_B, pLayer->Tile_B - 1, col);
      // Bottom Right
      rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_BR, y, pLayer->W_BR,
                            pLayer->H_BR, 0, false, pLayer->TxId_BR, col);
      // Content
      if (pLayer->Tile_Blank == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_L, y + pLayer->H_B,
                              w - (pLayer->W_R + pLayer->W_L),
                              h - (pLayer->H_B + pLayer->H_T), 0, false,
                              pLayer->TxId_Blank, col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_L, y + pLayer->H_B,
            w - (pLayer->W_R + pLayer->W_L), h - (pLayer->H_B + pLayer->H_T), 0,
            false, pLayer->TxId_Blank, pLayer->Tile_Blank - 1, col);
    } else { // Opened
      // Left
      if (pLayer->Tile_L == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x, y + pLayer->H_BL_Open, pLayer->W_L,
                              h - (pLayer->H_BL_Open + pLayer->H_TL), 0, false,
                              pLayer->TxId_L, col);
      else
        rVR.drawRotFlipBitmapTiled(rl, x, y + pLayer->H_BL_Open, pLayer->W_L,
                                   h - (pLayer->H_BL_Open + pLayer->H_TL), 0,
                                   false, pLayer->TxId_L, pLayer->Tile_L - 1,
                                   col);
      // Right
      if (pLayer->Tile_R == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_R, y + pLayer->H_BR_Open,
                              pLayer->W_R,
                              h - (pLayer->H_TR + pLayer->H_BR_Open), 0, false,
                              pLayer->TxId_R, col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + w - pLayer->W_R, y + pLayer->H_BR_Open, pLayer->W_R,
            h - (pLayer->H_TR + pLayer->H_BR_Open), 0, false, pLayer->TxId_R,
            pLayer->Tile_R - 1, col);
      // Bottom Left
      rVR.drawRotFlipBitmap(rl, x, y, pLayer->W_BL_Open, pLayer->H_BL_Open, 0,
                            false, pLayer->TxId_BL_Open, col);
      // Bottom
      if (pLayer->Tile_B_Open == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_BL_Open, y,
                              w - (pLayer->W_BL_Open + pLayer->W_BR_Open),
                              pLayer->H_B_Open, 0, false, pLayer->TxId_B_Open,
                              col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_BL_Open, y,
            w - (pLayer->W_BL_Open + pLayer->W_BR_Open), pLayer->H_B_Open, 0,
            false, pLayer->TxId_B_Open, pLayer->Tile_B_Open - 1, col);
      // Bottom Right
      rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_BR_Open, y, pLayer->W_BR_Open,
                            pLayer->H_BR_Open, 0, false, pLayer->TxId_BR_Open,
                            col);
      // Content
      if (pLayer->Tile_Blank == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_L, y + pLayer->H_B_Open,
                              w - (pLayer->W_R + pLayer->W_L),
                              h - (pLayer->H_B_Open + pLayer->H_T), 0, false,
                              pLayer->TxId_Blank, col);
      else
        rVR.drawRotFlipBitmapTiled(rl, x + pLayer->W_L, y + pLayer->H_B_Open,
                                   w - (pLayer->W_R + pLayer->W_L),
                                   h - (pLayer->H_B_Open + pLayer->H_T), 0,
                                   false, pLayer->TxId_Blank,
                                   pLayer->Tile_Blank - 1, col);
      // ScrollBar Placement
      if (pLayer->Tile_M_Open == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x, _YReal + pLayer->H_EL_Open,
                              pLayer->W_M_Open, _HReal - h - pLayer->H_EL_Open,
                              0, false, pLayer->TxId_M_Open, col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x, _YReal + pLayer->H_EL_Open, pLayer->W_M_Open,
            _HReal - h - pLayer->H_EL_Open, 0, false, pLayer->TxId_M_Open,
            pLayer->Tile_M_Open - 1, col);
      // Ending Left
      rVR.drawRotFlipBitmap(rl, x, _YReal, pLayer->W_EL_Open, pLayer->H_EL_Open,
                            0, false, pLayer->TxId_EL_Open, col);
      // Ending Middle
      if (pLayer->Tile_EM_Open == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_EL_Open, _YReal,
                              w - (pLayer->W_EL_Open + pLayer->W_ER_Open),
                              pLayer->H_EM_Open, 0, false, pLayer->TxId_EM_Open,
                              col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_EL_Open, _YReal,
            w - (pLayer->W_EL_Open + pLayer->W_ER_Open), pLayer->H_EM_Open, 0,
            false, pLayer->TxId_EM_Open, pLayer->Tile_EM_Open - 1, col);
      // Ending Right
      rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_ER_Open, _YReal,
                            pLayer->W_ER_Open, pLayer->H_ER_Open, 0, false,
                            pLayer->TxId_ER_Open, col);
    }
  } else // Version for layer 1,2,3
  {
    ...
    if ((!_Opened) || (!bHasChild)) {
      ...
      if (pLayer->Tile_R == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_R, y + pLayer->H_BR,
                              pLayer->W_R, h - (pLayer->H_BL + pLayer->H_TL), 0,
                              false, pLayer->TxId_R, col);
      else
        rVR.drawRotFlipBitmapTiled(rl, x + w - pLayer->W_R, y + pLayer->H_BR,
                                   pLayer->W_R,
      ...
      if (pLayer->Tile_Blank == 0) // Tiling ?
        rVR.drawRotFlipBitmap(rl, x + pLayer->W_L, y + pLayer->H_B,
                              w - (pLayer->W_L + pLayer->W_R),
                              h - (pLayer->H_B + pLayer->H_T), 0, false,
                              pLayer->TxId_Blank, col);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_L, y + pLayer->H_B,
            w - (pLayer->W_L + pLayer->W_R), h - (pLayer->H_B + pLayer->H_T), 0,
            false, pLayer->TxId_Blank, pLayer->Tile_Blank - 1, col);
    } else {
      // Left
      if (pLayer->Tile_L == 0) // Tiling ?
  ...
  if (_LayerSetup == 0) {
    ...
    if (CWidgetManager::getInstance()->getTopWindow(
    ...
    if (_TitleClosed != NULL)
      _TitleClosed->setColor(c);
    if (_TitleOpened != NULL)
      _TitleOpened->setColor(c);
    if (_ViewOpenState != NULL)
      _ViewOpenState->setColor(c);
    if (_RightButton != NULL) {
  ...
  if (parentGC) {
    // _CurrentContentAlpha = (uint8) (((uint16) _ContentAlpha * (uint16)
    // parentGC->getCurrentContentAlpha()) >> 8);
    _CurrentContentAlpha = parentGC->getCurrentContentAlpha();
    _CurrentContainerAlpha = parentGC->getCurrentContainerAlpha();
  } else {
    _CurrentContentAlpha =
        _UseGlobalAlpha ? CWidgetManager::getInstance()->getGlobalContentAlpha()
                        : _ContentAlpha;
    _CurrentContainerAlpha =
        _UseGlobalAlpha
            ? CWidgetManager::getInstance()->getGlobalContainerAlpha()
  ...
  CWidgetManager::getInstance()->setContentAlpha(
      (uint8)(((uint16)_CurrentContentAlpha *
               (uint16)_ICurrentRolloverAlphaContent) >>
              8));
  // set content alpha multiplied by rollover alpha
  _CurrentContainerAlpha = (uint8)(((uint16)_CurrentContainerAlpha *
  ...
  if (_LayerSetup == 0) {
    if (_HeaderActive) {
      ...
      c.A = (uint8)(((uint16)_CurrentContentAlpha *
                     (uint16)_ICurrentRolloverAlphaContent) >>
                    8);
      // Left
      rVR.drawRotFlipBitmap(rl, x, y + h - pLayer->H_L_Header,
                            pLayer->W_L_Header, pLayer->H_L_Header, 0, false,
                            pLayer->TxId_L_Header, c);
      // Middle
      if (pLayer->Tile_M_Header == 0) // Tiling ?
        rVR.drawRotFlipBitmap(
            rl, x + pLayer->W_L_Header, y + h - pLayer->H_M_Header,
            w - (pLayer->W_L_Header + pLayer->W_R_Header), pLayer->H_M_Header,
            0, false, pLayer->TxId_M_Header, c);
      else
        rVR.drawRotFlipBitmapTiled(
            rl, x + pLayer->W_L_Header, y + h - pLayer->H_M_Header,
            w - (pLayer->W_L_Header + pLayer->W_R_Header), pLayer->H_M_Header,
            0, false, pLayer->TxId_M_Header, pLayer->Tile_M_Header - 1, c);
      // Right
      rVR.drawRotFlipBitmap(rl, x + w - pLayer->W_R_Header,
                            y + h - pLayer->H_R_Header, pLayer->W_R_Header,
                            pLayer->H_R_Header, 0, false, pLayer->TxId_R_Header,
                            c);
    }
  }
  ...
  if (_LayerSetup == 0)
    if ((_HighLighted) || (_CurrentContainerAlpha <= 128)) {
      ...
      // corners
      rVR.drawRotFlipBitmap(_RenderLayer, x, y + h - pLayer->H_T_HighLight,
                            pLayer->W_TL_HighLight, pLayer->H_TL_HighLight, 0,
                            false, pLayer->TxId_TL_HighLight, col);
      rVR.drawRotFlipBitmap(_RenderLayer, x + _WReal - pLayer->W_TR_HighLight,
                            y + h - pLayer->H_T_HighLight,
                            pLayer->W_TR_HighLight, pLayer->H_TR_HighLight, 0,
                            false, pLayer->TxId_TR_HighLight, col);
      rVR.drawRotFlipBitmap(_RenderLayer, x, _YReal, pLayer->W_BL_HighLight,
                            pLayer->H_BL_HighLight, 0, false,
                            pLayer->TxId_BL_HighLight, col);
      rVR.drawRotFlipBitmap(_RenderLayer, x + _WReal - pLayer->W_BR_HighLight,
                            _YReal, pLayer->W_BR_HighLight,
                            pLayer->H_BR_HighLight, 0, false,
                            pLayer->TxId_BR_HighLight, col);
      // border
      rVR.drawRotFlipBitmap(
          _RenderLayer, x + pLayer->W_TL_HighLight,
          y + h - pLayer->H_T_HighLight,
          _WReal - pLayer->W_TL_HighLight - pLayer->W_TR_HighLight,
          pLayer->H_T_HighLight, 0, false, pLayer->TxId_T_HighLight, col);
      rVR.drawRotFlipBitmap(
          _RenderLayer, x + pLayer->W_BL_HighLight, _YReal,
          _WReal - pLayer->W_BL_HighLight - pLayer->W_BR_HighLight,
          pLayer->H_B_HighLight, 0, false, pLayer->TxId_B_HighLight, col);
      rVR.drawRotFlipBitmap(_RenderLayer, x, _YReal + pLayer->H_B_HighLight,
                            pLayer->W_L_HighLight,
                            _HReal - pLayer->H_T_HighLight -
                                pLayer->H_B_HighLight,
                            0, false, pLayer->TxId_L_HighLight, col);
      rVR.drawRotFlipBitmap(
          _RenderLayer, x + _WReal - pLayer->W_R_HighLight,
          _YReal + pLayer->H_B_HighLight, pLayer->W_R_HighLight,
          _HReal - pLayer->H_T_HighLight - pLayer->H_B_HighLight, 0, false,
          pLayer->TxId_R_HighLight, col);
    }
  ...
  if (mousePointer) {
    ...
    if (dontFade || isOver) {
      ...
      _CurrentRolloverAlphaContent =
          std::min(1.f, _CurrentRolloverAlphaContent);

      _CurrentRolloverAlphaContainer += (float)(speed * times.frameDiffMs);
      _CurrentRolloverAlphaContainer =
          std::min(1.f, _CurrentRolloverAlphaContainer);
    } else {
      _CurrentRolloverAlphaContent -= (float)(speed * times.frameDiffMs);
      _CurrentRolloverAlphaContent =
          std::max(0.f, _CurrentRolloverAlphaContent);

      _CurrentRolloverAlphaContainer -= (float)(speed * times.frameDiffMs);
      _CurrentRolloverAlphaContainer =
          std::max(0.f, _CurrentRolloverAlphaContainer);
    }
  }
...
// ***************************************************************************
void CGroupContainer::clearViews() { CInterfaceGroup::clearViews(); }
...
void CGroupContainer::open() {
  ...
  _OpenAtStart = true;
  if (_TitleOpened != NULL)
    _TitleOpened->setActive(true);
  if (_TitleClosed != NULL)
    _TitleClosed->setActive(false);
  ...
  if (_HeaderClosed != NULL)
    _HeaderClosed->setActive(false);
  if (_HeaderOpened != NULL)
    _HeaderOpened->setActive(true);

  invalidateCoords();

  // call action handler if any
  if (_AHOnOpen != NULL) {
    CAHManager::getInstance()->runActionHandler(_AHOnOpen, this,
                                                _AHOnOpenParams);
  }
}
...
void CGroupContainer::close() {
  _Opened = false;
  if (_TitleOpened != NULL)
    _TitleOpened->setActive(false);
  if (_TitleClosed != NULL)
    _TitleClosed->setActive(true);
  if ((_Openable) && (_LayerSetup >= 0)) {
    COptionsLayer *pLayer = getContainerOptions();
    if (_ViewOpenState != NULL)
  ...
  if (_HeaderClosed != NULL)
    _HeaderClosed->setActive(true);
  if (_HeaderOpened != NULL)
    _HeaderOpened->setActive(false);
  invalidateCoords();

  // call action handler if any
  if (_AHOnClose != NULL) {
    CAHManager::getInstance()->runActionHandler(_AHOnClose, this,
                                                _AHOnCloseParams);
  }
}

// ***************************************************************************
void CGroupContainer::attachContainer(CGroupContainer *pIC,
                                      sint insertionOrder /* = -1 */) {
  if (_List == NULL) {
  ...
  if (insertionOrder == -1) {
    ...
    for (k = 0; k < (uint)_List->getNbElement(); ++k) {
      if (_List->getOrder(k) > (uint)insertionOrder)
        break;
    }
...
void CGroupContainer::detachContainer(CGroupContainer *pIC) {
  if (!pIC)
    return;
  nlassert(_List != NULL);
  nlassert(pIC->getProprietaryContainer() ==
           this);              // should be a son of that container
  nlassert(!pIC->isPopuped()); // if the container is poped, should pop it in
                               // before detaching it!
  _List->delChild(pIC);
...
void CGroupContainer::setup() {
  ...
  // Opened state view (tells the user if the container is openable (if the view
  // appears) and if its opened/closed
  updateViewOpenState();
  ...
  if (_Content != NULL) {
    ...
    _Content->setPosRef((THotSpot)((_Content->getPosRef() &
                                    (Hotspot_xL | Hotspot_xM | Hotspot_xR)) |
                                   Hotspot_Tx));
  ...
  if (_HeaderOpened != NULL) {
    ...
    _HeaderOpened->setPosRef(
        (THotSpot)((_HeaderOpened->getPosRef() &
                    (Hotspot_xL | Hotspot_xM | Hotspot_xR)) |
                   Hotspot_Tx));
  ...
  if (_HeaderClosed != NULL) {
    ...
    _HeaderClosed->setPosRef(
        (THotSpot)((_HeaderClosed->getPosRef() &
                    (Hotspot_xL | Hotspot_xM | Hotspot_xR)) |
                   Hotspot_Tx));
  ...
  if ((_EnabledRightButton) && (!((_LayerSetup > 0) && (!_Popable)))) {
    ...
    if (_LayerSetup == 0) {
      if (_Locked) {
        ...
        _RightButton->setTexturePushed(
            pLayer->getValStr("right_button_tx_locked"));
        ...
        if (_OldFatherContainer != NULL) {
          ...
          _RightButton->setTexturePushed(
              pLayer->getValStr("right_button_tx_popin"));
          _RightButton->setTextureOver(
              pLayer->getValStr("right_button_tx_over"));
        ...
        {
          _RightButton->setTexture(
              pLayer->getValStr("right_button_tx_deactive"));
          _RightButton->setTexturePushed(
              pLayer->getValStr("right_button_tx_deactive"));
          _RightButton->setTextureOver(
              pLayer->getValStr("right_button_tx_over"));
      ...
      if (_Popable) {
        ...
        _RightButton->setTexturePushed(
            pLayer->getValStr("right_button_tx_popup"));
...
void CGroupContainer::updateMover() {
  if (_Movable || _Popable || _Openable) {
    COptionsContainerMove *options = getMoveOptions();
    _Mover =
        new CCtrlMover(CViewText::TCtorParam(),
                       _Movable || _Popable || _MovableInParentList, _Openable);
...
// ***************************************************************************
// bMaxH is a boolean to know if the resizer act on the content or on the child
// list
void CGroupContainer::createResizer(uint index, THotSpot posRef, THotSpot type,
                                    sint32 offsetX, sint32 offsetY,
                                    bool bMaxH) {
...
void CGroupContainer::createResizerMaxH() {
  ...
  if (_EnabledResizer) {
    if (_Resizer[1] == NULL)
      createResizer(1, Hotspot_TR, Hotspot_TR, 0, 0, true);
    if (_Resizer[3] == NULL)
      createResizer(3, Hotspot_BR, Hotspot_BR, 0, 0, true);
    if (_Resizer[5] == NULL)
      createResizer(5, Hotspot_BL, Hotspot_BL, 0, 0, true);
    if (_Resizer[7] == NULL)
      createResizer(7, Hotspot_TL, Hotspot_TL, 0, 0, true);
  ...
  if (_Resizer[0] == NULL)
    createResizer(0, Hotspot_TL, Hotspot_TM, options->ResizerSize, 0, true);
  if (_Resizer[4] == NULL)
    createResizer(4, Hotspot_BR, Hotspot_BM, -options->ResizerSize, 0, true);
...
COptionsLayer *CGroupContainer::getContainerOptions(sint32 ls) {
  ...
  COptionsLayer *pLayer =
      (COptionsLayer *)CWidgetManager::getInstance()->getOptions(*sLayerName);
...
// ***************************************************************************
std::string CGroupContainer::getTitle() const { return _TitleTextOpened; }
...
// ***************************************************************************
std::string CGroupContainer::getTitleOpened() const { return _TitleTextOpened; }
...
// ***************************************************************************
std::string CGroupContainer::getTitleClosed() const { return _TitleTextClosed; }
...
// ***************************************************************************
bool CGroupContainer::getViewsUnder(sint32 x, sint32 y, sint32 clipX,
                                    sint32 clipY, sint32 clipW, sint32 clipH,
                                    std::vector<CViewBase *> &vVB) {
  bool bGrayed = isGrayed();
  if (bGrayed)
    return false;
...
// ***************************************************************************
bool CGroupContainer::getCtrlsUnder(sint32 x, sint32 y, sint32 clipX,
                                    sint32 clipY, sint32 clipW, sint32 clipH,
...
void CGroupContainer::popupCurrentPos() {
  if (!_Popable && !_MovableInParentList) {
    nlwarning("<CGroupContainer::popup> The window is not popable or cannot "
              "move in parent list.");
    return;
  }
  if (_LayerSetup == 0) {
  ...
  CGroupContainer *parentContainer =
      dynamic_cast<CGroupContainer *>(_Parent->getParent());
  if (!parentContainer) {
    nlwarning(
        "<CGroupContainer::popup> Container is not son of another container");
    return;
  }
...
// ***************************************************************************
void CGroupContainer::popin(sint32 insertPos /* = -1 */,
                            bool putBackInFatherContainer /*= true*/) {
  if (!_OldFatherContainer) {
    nlwarning("<popin> The window wasn't previously attached.(%s)",
              this->_Id.c_str());
    return;
  }
  ...
  if (!_Popable && !_MovableInParentList) {
    nlwarning(
        "<popin> The window is not popable or cannot move in parent list.(%s)",
        this->_Id.c_str());
    return;
  }
  if (!_MovingInParentList && _LayerSetup != 0) {
    nlwarning("<popin> The window should be in layer 0.(%s)",
              this->_Id.c_str());
    return;
  }
  if (!_Parent) {
  ...
  _Parent = NULL;
  _ParentPos = NULL;
  std::vector<CGroupContainer *>::iterator it =
      std::find(_PopedCont.begin(), _PopedCont.end(), this);
...
COptionsContainerMove *CGroupContainer::getMoveOptions() {
  ...
  if (moveOptions)
    return moveOptions;
  moveOptions =
      (COptionsContainerMove *)CWidgetManager::getInstance()->getOptions(
          "container_move_opt");
  return moveOptions;
}
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    ...
    if (!pIC->getAHOnDeactiveCheck().empty()) {
      CAHManager::getInstance()->runActionHandler(
          pIC->getAHOnDeactiveCheck(), pCaller,
          pIC->getAHOnDeactiveCheckParams());
    }
    ...
    if (CGroupContainer::_ValidateCanDeactivate) {
      ...
      if (!pIC->getAHOnCloseButton().empty()) {
        CAHManager::getInstance()->runActionHandler(
            pIC->getAHOnCloseButton(), pCaller,
            pIC->getAHOnCloseButtonParams());
      }
...
class CICHelp : public IActionHandler {
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    ...
    if (!pCaller)
      return;
    CGroupContainer *pIC =
        dynamic_cast<CGroupContainer *>(pCaller->getRootWindow());
    if (pIC == NULL)
      return;
    ...
    if (!helpPage.empty()) {
      ...
      CAHManager::getInstance()->runActionHandler("launch_help", NULL,
                                                  "url=" + helpPage);
    }
  }
};
...
bool CGroupContainer::hasKeyboardFocus() const {
  ...
  if (CWidgetManager::getInstance()->getCaptureKeyboard() != NULL) {
    const CGroupEditBox *geb = dynamic_cast<const CGroupEditBox *>(
...
void CGroupContainer::setModalParentList(const std::string &name) {
  ...
  for (uint i = 0; i < modalParents.size(); i++) {
    CGroupContainer *pGC = dynamic_cast<CGroupContainer *>(
        CWidgetManager::getInstance()->getElementFromId(modalParents[i]));
    if (pGC == NULL)
...
bool CGroupContainer::checkIfModal(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if ((eventDesc.getEventTypeExtended() ==
         NLGUI::CEventDescriptorMouse::mouseleftdown) ||
        (eventDesc.getEventTypeExtended() ==
         NLGUI::CEventDescriptorMouse::mouserightdown)) {
      bRet = blinkAllSons();
    }
    // Additionaly, if it is a UP, don't blink, but return false if some son
    // active
    if (bRet && ((eventDesc.getEventTypeExtended() ==
                  NLGUI::CEventDescriptorMouse::mouseleftup) ||
                 (eventDesc.getEventTypeExtended() ==
...
bool CGroupContainer::blinkAllSons() {
  ...
  uint i, j;
  // Recurs all sons (because allSons grow while sons are added). NB: if there
  // is a graph, it will freeze....
...
CRGBA CGroupContainer::getDrawnHeaderColor() const {
  ...
  if (CWidgetManager::getInstance()->getTopWindow(
          CWidgetManager::getInstance()->getLastTopWindowPriority()) !=
      static_cast<const CInterfaceGroup *>(this)) {
    if (_HeaderColor.getNodePtr() != NULL)
      c = _HeaderColor.getRGBA();
    if (isGrayed()) {
      c.R = c.R / 2;
...
void CGroupContainer::setRightButtonEnabled(bool enabled) {
  if (_EnabledRightButton == enabled)
    return;

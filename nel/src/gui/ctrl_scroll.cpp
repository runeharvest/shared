...
void CCtrlScroll::updateCoords()
{
	if (_Target)
	{
		...
		if (_Target->getActive())
		{
			...
			if (_Vertical)
			{
				if (_Target->getHReal() != _LastTargetHReal || _Target->getMaxHReal() != _LastTargetMaxHReal || _Target->getOfsY() != _LastTargetOfsY)
				{
			...
			{
				if (_Target->getWReal() != _LastTargetWReal || _Target->getMaxWReal() != _LastTargetMaxWReal || _Target->getOfsX() != _LastTargetOfsX)
				{
...
bool CCtrlScroll::handleEvent(const NLGUI::CEventDescriptor &event)
{
	...
	if (event.getType() == NLGUI::CEventDescriptor::mouse)
	{
		...
		if ((CWidgetManager::getInstance()->getCapturePointerLeft() != this) && (!((eventDesc.getX() >= _XReal) && (eventDesc.getX() < (_XReal + _WReal)) && (eventDesc.getY() > _YReal) && (eventDesc.getY() <= (_YReal + _HReal)))))
			return false;

...
// ------------------------------------------------------------------------------------------------
CCtrlScroll::CCtrlScroll(const TCtorParam &param) : CCtrlScrollBase(param) {
  _Vertical = true;
  _Aligned = 1;
  _TrackPos = 0;
  _TrackDispPos = 0;
...
std::string CCtrlScroll::getProperty(const std::string &name) const {
  ...
  if (name == "tx_bottomleft") {
  ...
  } else if (name == "tx_middle") {
    return getTextureMiddle();
  } else if (name == "tx_topright") {
  ...
  } else if (name == "end_params") {
    return _AHOnScrollEndParams;
  } else if (name == "onscrollcancel") {
    return _AHOnScrollCancel;
  } else if (name == "cancel_params") {
    ...
    if (_Target != NULL)
      ...
      return "";
  } else if (name == "target_stepx") {
    return toString(_TargetStepX);
  } else if (name == "target_stepy") {
    return toString(_TargetStepY);
  } else if (name == "step_value") {
...
void CCtrlScroll::setProperty(const std::string &name,
...
xmlNodePtr CCtrlScroll::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "tx_bottomleft",
             BAD_CAST getTextureBottomOrLeft().c_str());
  xmlSetProp(node, BAD_CAST "tx_middle", BAD_CAST getTextureMiddle().c_str());
  xmlSetProp(node, BAD_CAST "tx_topright",
             BAD_CAST getTextureTopOrRight().c_str());
  ...
  if (_IsDBLink)
    xmlSetProp(node, BAD_CAST "value",
               BAD_CAST _DBLink.getNodePtr()->getFullName().c_str());
  ...
  xmlSetProp(node, BAD_CAST "end_params",
             BAD_CAST _AHOnScrollEndParams.c_str());
  xmlSetProp(node, BAD_CAST "onscrollcancel",
             BAD_CAST _AHOnScrollCancel.c_str());
  xmlSetProp(node, BAD_CAST "cancel_params",
             BAD_CAST _AHOnScrollCancelParams.c_str());
  ...
  xmlSetProp(node, BAD_CAST "target_stepx",
             BAD_CAST toString(_TargetStepX).c_str());
  xmlSetProp(node, BAD_CAST "target_stepy",
             BAD_CAST toString(_TargetStepY).c_str());
  xmlSetProp(node, BAD_CAST "step_value",
             BAD_CAST toString(_StepValue).c_str());
  xmlSetProp(node, BAD_CAST "cancelable",
             BAD_CAST toString(_Cancelable).c_str());
...
bool CCtrlScroll::parse(xmlNodePtr node, CInterfaceGroup *parentGroup) {
  ...
  if (prop) {
    if (stricmp(prop, "T") == 0)
      _Aligned = 0;
    else if (stricmp(prop, "B") == 0)
      _Aligned = 1;
    else if (stricmp(prop, "L") == 0)
      _Aligned = 2;
    else if (stricmp(prop, "R") == 0)
      _Aligned = 3;
  }

  prop = (char *)xmlGetProp(node, (xmlChar *)"min");
  if (prop)
    fromString((const char *)prop, _Min);

  prop = (char *)xmlGetProp(node, (xmlChar *)"max");
  if (prop)
    fromString((const char *)prop, _Max);
  ...
  if (prop)
    fromString((const char *)prop, _TrackSize);

  // Read Action handlers
  prop = (char *)xmlGetProp(node, (xmlChar *)"onscroll");
  if (prop)
    _AHOnScroll = NLMISC::toLowerAscii(prop.str());
  prop = (char *)xmlGetProp(node, (xmlChar *)"params");
  if (prop)
    _AHOnScrollParams = string((const char *)prop);
  //
  prop = (char *)xmlGetProp(node, (xmlChar *)"onscrollend");
  if (prop)
    _AHOnScrollEnd = NLMISC::toLowerAscii(prop.str());
  prop = (char *)xmlGetProp(node, (xmlChar *)"end_params");
  if (prop)
    _AHOnScrollEndParams = string((const char *)prop);
  //
  prop = (char *)xmlGetProp(node, (xmlChar *)"onscrollcancel");
  if (prop)
    _AHOnScrollCancel = NLMISC::toLowerAscii(prop.str());
  prop = (char *)xmlGetProp(node, (xmlChar *)"cancel_params");
  if (prop)
    _AHOnScrollCancelParams = string((const char *)prop);
  ...
  if (prop) {
    ...
    if (group == NULL)
      group = dynamic_cast<CInterfaceGroup *>(
          CWidgetManager::getInstance()->getElementFromId(this->getId(),
                                                          prop.str()));
  ...
  if (prop)
    fromString((const char *)prop, _TargetStepX);
  prop = (char *)xmlGetProp(node, (xmlChar *)"target_stepy");
  if (prop)
    fromString((const char *)prop, _TargetStepY);
  ...
  if (prop)
    fromString((const char *)prop, _StepValue);

  prop = (char *)xmlGetProp(node, (xmlChar *)"cancelable");
  if (prop)
    _Cancelable = convertBool(prop);

  prop = (char *)xmlGetProp(node, (xmlChar *)"keyboard");
  if (prop)
    _Keyboard = convertBool(prop);
...
void CCtrlScroll::updateCoords() {
  if (_Target) {
    ...
    if (_Target->getActive()) {
      ...
      if (_Vertical) {
        if (_Target->getHReal() != _LastTargetHReal ||
            _Target->getMaxHReal() != _LastTargetMaxHReal ||
          ...
          if ((_Target->getHReal() <= _Target->getMaxHReal()) ||
            ...
            float factor =
                (float)_Target->getMaxHReal() / (float)_Target->getHReal();
            factor =
                _TrackSizeMin + factor * (this->getHReal() - _TrackSizeMin);
          ...
          if (_Target->getHReal() <= _Target->getMaxHReal()) {
            ...
            {
              float factor =
                  ((float)_Target->getHReal() - (float)_Target->getMaxHReal());
      ...
      {
        if (_Target->getWReal() != _LastTargetWReal ||
            _Target->getMaxWReal() != _LastTargetMaxWReal ||
          ...
          if ((_Target->getWReal() <= _Target->getMaxWReal()) ||
            ...
            float factor =
                (float)_Target->getMaxWReal() / (float)_Target->getWReal();
            factor =
                _TrackSizeMin + factor * (this->getWReal() - _TrackSizeMin);
            _TrackSize = (sint32)factor;
          }
          // Calculate pos of the track button
          if (_Target->getWReal() <= _Target->getMaxWReal()) {
            ...
            {
              float factor =
                  ((float)_Target->getWReal() - (float)_Target->getMaxWReal());
...
void CCtrlScroll::draw() {
  ...
  if (_Target) {
    if (_Vertical) {
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal + _TrackPos, _WReal, 4,
                            0, false, _TxIdB, col);
      if (_TileM == 0)
        rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal + _TrackPos + 4,
                              _WReal, _TrackSize - 8, 0, false, _TxIdM, col);
      else
        rVR.drawRotFlipBitmapTiled(_RenderLayer, _XReal, _YReal + _TrackPos + 4,
                                   _WReal, _TrackSize - 8, 0, false, _TxIdM,
                                   _TileM - 1, col);
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal,
                            _YReal + _TrackPos + _TrackSize - 4, _WReal, 4, 0,
                            false, _TxIdT, col);
    } else {
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _TrackPos, _YReal, 4, _HReal,
                            0, false, _TxIdB, col);
      if (_TileM == 0)
        rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _TrackPos + 4, _YReal,
                              _TrackSize - 8, _HReal, 0, false, _TxIdM, col);
      else
        rVR.drawRotFlipBitmapTiled(_RenderLayer, _XReal + _TrackPos + 4, _YReal,
                                   _TrackSize - 8, _HReal, 0, false, _TxIdM,
                                   _TileM - 1, col);
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _TrackPos + _TrackSize - 4,
                            _YReal, 4, _HReal, 0, false, _TxIdT, col);
    }
  } else {
    if (_Vertical) {
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal + _TrackDispPos,
                            _WReal, 4, 0, false, _TxIdB, col);
      if (_TileM == 0)
        rVR.drawRotFlipBitmap(_RenderLayer, _XReal, _YReal + _TrackDispPos + 4,
                              _WReal, _TrackSize - 8, 0, false, _TxIdM, col);
      else
        rVR.drawRotFlipBitmapTiled(
            _RenderLayer, _XReal, _YReal + _TrackDispPos + 4, _WReal,
            _TrackSize - 8, 0, false, _TxIdM, _TileM - 1, col);
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal,
                            _YReal + _TrackDispPos + _TrackSize - 4, _WReal, 4,
                            0, false, _TxIdT, col);
    } else {
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _TrackDispPos, _YReal, 4,
                            _HReal, 0, false, _TxIdB, col);
      if (_TileM == 0)
        rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _TrackDispPos + 4, _YReal,
                              _TrackSize - 8, _HReal, 0, false, _TxIdM, col);
      else
        rVR.drawRotFlipBitmapTiled(_RenderLayer, _XReal + _TrackDispPos + 4,
                                   _YReal, _TrackSize - 8, _HReal, 0, false,
                                   _TxIdM, _TileM - 1, col);
      rVR.drawRotFlipBitmap(_RenderLayer,
                            _XReal + _TrackDispPos + _TrackSize - 4, _YReal, 4,
                            _HReal, 0, false, _TxIdT, col);
    }
  }
}
...
bool CCtrlScroll::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      _MouseDownOffsetX =
          eventDesc.getX() - (getXReal() + (_Vertical ? 0 : _TrackPos));
      _MouseDownOffsetY =
          eventDesc.getY() - (getYReal() + (_Vertical ? _TrackPos : 0));

      // if target is a menu, hidde its sub menus
      if (_Target && _Target->getParent()) {
        CGroupSubMenuBase *menu =
            dynamic_cast<CGroupSubMenuBase *>(_Target->getParent());
        if (menu)
          menu->hideSubMenus();
      }
      return true;
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      _MouseDown = false;
      runAH(_AHOnScrollEnd,
            _AHOnScrollEndParams.empty()
                ? _AHOnScrollParams
                : _AHOnScrollEndParams); // backward compatibility
      return true;
    }
    if (eventDesc.getEventTypeExtended() ==
            NLGUI::CEventDescriptorMouse::mouserightdown &&
        _MouseDown && _Cancelable) {
      _MouseDown = false;
      setValue(_InitialValue);
      runAH(_AHOnScrollCancel,
            _AHOnScrollCancelParams); // backward compatibility
      return true;
    }
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (_MouseDown) {
        sint32 dx =
            eventDesc.getX() -
            (getXReal() + (_Vertical ? 0 : _TrackPos) + _MouseDownOffsetX);
        sint32 dy =
            eventDesc.getY() -
            (getYReal() + (_Vertical ? _TrackPos : 0) + _MouseDownOffsetY);
        if (dx != 0)
          moveTrackX(dx);
        if (dy != 0)
          moveTrackY(dy);
    ...
    const NLGUI::CEventDescriptorKey &eventDesc =
    ...
    if (eventDesc.getKeyEventType() == NLGUI::CEventDescriptorKey::keydown) {
      if (_Keyboard) {
        sint32 i = 0;
        // direction
        if (eventDesc.getKey() == KeyNEXT)
          i++;
        else if (eventDesc.getKey() == KeyPRIOR)
          i--;
        else
          return false;
...
sint32 CCtrlScroll::moveTrackX(sint32 dx) {
  ...
  if (newtpos < 0)
    newtpos = 0;
  if (newtpos > (getWReal() - tsize))
    newtpos = (getWReal() - tsize);
  dx = newtpos - tpos;
...
sint32 CCtrlScroll::moveTrackY(sint32 dy) {
  ...
  if (newtpos < 0)
    newtpos = 0;
  if (newtpos > (getHReal() - tsize))
    newtpos = (getHReal() - tsize);
  dy = newtpos - tpos;
...
void CCtrlScroll::computeTargetOfsFromPos() {
  if (_Vertical) {
    ...
    if (_Aligned == 1) // BOTTOM
      ...
      factor = factor *
               (1.0f - (_TrackPos / favoid0((float)(getHReal() - _TrackSize))));
    ...
    if (_Aligned == 2) // LEFT
      ...
      factor = factor *
               (1.0f - (_TrackPos / favoid0((float)(getWReal() - _TrackSize))));
...
int CCtrlScroll::luaEnsureVisible(CLuaState &ls) {
  ...
  for (uint hsIndex = 0; hsIndex < 2; ++hsIndex) {
    if (_Vertical) {
      ...
      } else if (NLMISC::nlstricmp(hsStr[hsIndex], "M") == 0) {
        hs[hsIndex] = Hotspot_Mx;
      } else if (NLMISC::nlstricmp(hsStr[hsIndex], "B") == 0) {
        hs[hsIndex] = Hotspot_Bx;
      } else {
        hs[hsIndex] = Hotspot_Bx;
        CLuaIHM::fails(ls, "%s : couldn't parse hotspot for vertical scrollbar",
                       funcName);
      }
    } else {
      ...
      } else if (NLMISC::nlstricmp(hsStr[hsIndex], "R") == 0) {
        hs[hsIndex] = Hotspot_xR;
      } else {
        hs[hsIndex] = Hotspot_xL;
        CLuaIHM::fails(ls,
                       "%s : couldn't parse hotspot for horizontal scrollbar",
                       funcName);
      }
    }
  }
...
// ------------------------------------------------------------------------------------------------
void CCtrlScroll::ensureVisible(CInterfaceElement *childElement,
                                THotSpot childHotSpot, THotSpot parentHotSpot) {
  if (!_Target)
  ...
  if (_Vertical) {
    ...
    if (hReal > maxHReal) {
      ...
      if (_Aligned == 0) {
        // Top aligned case
        sint32 offsetY =
            (_Target->getYReal() + _Target->getHReal() - childY) - enclosingDY;
        ...
        // Bottom aligned case
        sint32 offsetY =
            (maxHReal - enclosingDY) - (childY - _Target->getYReal());
    ...
    if (wReal > maxWReal) {
      ...
      if (_Aligned == 3) {
        // right aligned case
        sint32 offsetX =
            (_Target->getXReal() + _Target->getWReal() - childX) - enclosingDX;
        ...
        // Left aligned case
        sint32 offsetX =
            (maxWReal - enclosingDX) - (childX - _Target->getXReal());

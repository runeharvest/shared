...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::setRenderLayer(sint layer) { m_RenderLayer = layer; }
...
bool CSSBorderRenderer::hasInnerShape(CSSLineStyle style) const {
  return style == CSS_LINE_STYLE_DOUBLE || style == CSS_LINE_STYLE_GROOVE ||
...
void CSSBorderRenderer::computeValues() {
  ...
  m_Computed.Top = m_Border.Top.empty()
                       ? 0
                       : m_Border.Top.Width.calculate(0, m_FontSize,
                                                      m_RootFontSize, vpW, vpH);
  m_Computed.Right = m_Border.Right.empty()
                         ? 0
                         : m_Border.Right.Width.calculate(
                               0, m_FontSize, m_RootFontSize, vpW, vpH);
  m_Computed.Bottom = m_Border.Bottom.empty()
                          ? 0
                          : m_Border.Bottom.Width.calculate(
                                0, m_FontSize, m_RootFontSize, vpW, vpH);
  m_Computed.Left = m_Border.Left.empty()
                        ? 0
                        : m_Border.Left.Width.calculate(
                              0, m_FontSize, m_RootFontSize, vpW, vpH);
}

// ----------------------------------------------------------------------------
void CSSBorderRenderer::getAdjacentBorders(EBorderSide side,
                                           EBorderSide &adjBorderL,
                                           EBorderSide &adjBorderR) const {
  switch (side) {
  case BORDER_TOP:
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::getAdjacentBorderWidth(EBorderSide side,
                                               sint32 &adjWidthL,
                                               sint32 &adjWidthR) const {
  switch (side) {
  case BORDER_TOP:
  case BORDER_BOTTOM:
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::buildDotCornerStart(EBorderSide side, SDrawBorder shape,
                                            float x1, float y1, float radius) {
  NLMISC::CLine line;
  switch (side) {
  case BORDER_TOP:
    line.V0.set(m_XReal + m_Computed.Left, m_YReal + m_HReal - m_Computed.Top,
                0);
  ...
  case BORDER_RIGHT:
    ...
    line.V1.set(m_XReal + m_WReal - m_Computed.Right,
                m_YReal + m_Computed.Bottom, 0);
    buildDotCorner(shape, x1, y1, radius, line);
    break;
  case BORDER_BOTTOM:
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::buildDotCornerEnd(EBorderSide side, SDrawBorder shape,
                                          float x1, float y1, float radius) {
  NLMISC::CLine line;
  switch (side) {
  case BORDER_TOP:
    ...
    line.V1.set(m_XReal + m_WReal - m_Computed.Right,
                m_YReal + m_HReal - m_Computed.Top, 0);
    buildDotCorner(shape, x1, y1, radius, line);
    break;
  case BORDER_RIGHT:
    line.V0.set(m_XReal + m_WReal - m_Computed.Right,
                m_YReal + m_HReal - m_Computed.Top, 0);
  ...
  case BORDER_LEFT:
    ...
    line.V1.set(m_XReal + m_Computed.Left, m_YReal + m_HReal - m_Computed.Top,
                0);
    buildDotCorner(shape, x1, y1, radius, line);
    break;
  }
...
void CSSBorderRenderer::buildDashedBorder(EBorderSide side) {
  ...
  if (border.Style == CSS_LINE_STYLE_DOTTED) {
    // thick border with little or no content might try to draw larger dot that
    // fits
    ...
    if (adjWidthL > 0)
      innerWidth -= adjWidthL;
    if (adjWidthR > 0)
      innerWidth -= adjWidthR;
...
void CSSBorderRenderer::buildSolidBorder(EBorderSide side) {
  ...
  if (border.Style == CSS_LINE_STYLE_INSET &&
      (side == BORDER_TOP || side == BORDER_LEFT))
    shape.Color = blend(border.Color, CRGBA::Black, 0.5f);
  else if (border.Style == CSS_LINE_STYLE_OUTSET &&
           (side == BORDER_BOTTOM || side == BORDER_RIGHT))
    shape.Color = blend(border.Color, CRGBA::Black, 0.5f);
  ...
  if (hasInnerShape(border.Style)) {
    ...
    switch (side) {
    case BORDER_TOP:
      m_DrawBorders.back().Quad.V0.x -= iStart;
      m_DrawBorders.back().Quad.V0.y += iMiddle;
      m_DrawBorders.back().Quad.V1.x += iEnd;
      m_DrawBorders.back().Quad.V1.y += iMiddle;
      shape.Quad.V3.x += iStart;
      shape.Quad.V3.y -= iMiddle;
      shape.Quad.V2.x -= iEnd;
      shape.Quad.V2.y -= iMiddle;
      break;
    case BORDER_BOTTOM:
      m_DrawBorders.back().Quad.V2.x += iEnd;
      m_DrawBorders.back().Quad.V2.y -= iMiddle;
      m_DrawBorders.back().Quad.V3.x -= iStart;
      m_DrawBorders.back().Quad.V3.y -= iMiddle;
      shape.Quad.V1.x -= iEnd;
      shape.Quad.V1.y += iMiddle;
      shape.Quad.V0.x += iStart;
      shape.Quad.V0.y += iMiddle;
      break;
    case BORDER_RIGHT:
      m_DrawBorders.back().Quad.V3.x += iMiddle;
      m_DrawBorders.back().Quad.V3.y += iEnd;
      m_DrawBorders.back().Quad.V0.x += iMiddle;
      m_DrawBorders.back().Quad.V0.y -= iStart;
      shape.Quad.V2.x -= iMiddle;
      shape.Quad.V2.y -= iEnd;
      shape.Quad.V1.x -= iMiddle;
      shape.Quad.V1.y += iStart;
      break;
    case BORDER_LEFT:
      m_DrawBorders.back().Quad.V2.x -= iMiddle;
      m_DrawBorders.back().Quad.V2.y += iEnd;
      m_DrawBorders.back().Quad.V1.x -= iMiddle;
      m_DrawBorders.back().Quad.V1.y -= iStart;
      shape.Quad.V3.x += iMiddle;
      shape.Quad.V3.y -= iEnd;
      shape.Quad.V0.x += iMiddle;
      shape.Quad.V0.y += iStart;
      break;
    }
    m_DrawBorders.push_back(shape);
  }
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::makeBorderQuad(EBorderSide side, SDrawBorder &shape,
  ...
  shape.Quad.V3.x = x;
  shape.Quad.V3.y = y + quadH;
  shape.Quad.V2.x = x + quadW;
  shape.Quad.V2.y = y + quadH;
  shape.Quad.V1.x = x + quadW;
  shape.Quad.V1.y = y;
  shape.Quad.V0.x = x;
  shape.Quad.V0.y = y;
}
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::makeCornerQuad(EBorderSide side,
                                       SDrawBorder &shape) const {
  switch (side) {
  case BORDER_TOP_LEFT:
    shape.Quad.V0.x += m_Computed.Left;
    break;
  case BORDER_TOP_RIGHT:
    shape.Quad.V1.x -= m_Computed.Right;
    break;
  case BORDER_RIGHT_TOP:
    shape.Quad.V3.y -= m_Computed.Top;
    break;
  case BORDER_RIGHT_BOTTOM:
    shape.Quad.V0.y += m_Computed.Bottom;
    break;
  case BORDER_BOTTOM_LEFT:
    shape.Quad.V3.x += m_Computed.Left;
    break;
  case BORDER_BOTTOM_RIGHT:
    shape.Quad.V2.x -= m_Computed.Right;
    break;
  case BORDER_LEFT_TOP:
    shape.Quad.V2.y -= m_Computed.Top;
    break;
  case BORDER_LEFT_BOTTOM:
    shape.Quad.V1.y += m_Computed.Bottom;
    break;
  }
}

// ----------------------------------------------------------------------------
static bool getCircleLineIntersection(float x, float y, float r,
                                      const NLMISC::CLine &line,
                                      NLMISC::CLine &result) {
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::buildDotCorner(SDrawBorder shape, float cX, float cY,
                                       float cR, const NLMISC::CLine &line) {
...
// ----------------------------------------------------------------------------
void CSSBorderRenderer::buildDotSegments(SDrawBorder shape, float x, float y,
  ...
  if (2 * radius * m_Scale < 4) {
    makeBorderQuad(BORDER_TOP, shape, x - radius, y - radius, radius * 2,
                   radius * 2);
    m_DrawBorders.push_back(shape);
    return;
  }
  ...
  for (step = 0; step < (uint)arcSectors; step++) {
    ...
    shape.Quad.V2.x = x + radius * cosf(a1);
    shape.Quad.V2.y = y + radius * sinf(a1);
    shape.Quad.V3.x = x + radius * cosf(a2);
    shape.Quad.V3.y = y + radius * sinf(a2);
...
void CSSBorderRenderer::updateCoords() {
  ...
  if (m_Computed.Top > 0 && m_Border.Top.Color.A > 0) {
    if (m_Border.Top.Style == CSS_LINE_STYLE_DASHED ||
        m_Border.Top.Style == CSS_LINE_STYLE_DOTTED)
  ...
  if (m_Computed.Bottom > 0 && m_Border.Bottom.Color.A > 0) {
    if (m_Border.Bottom.Style == CSS_LINE_STYLE_DASHED ||
        m_Border.Bottom.Style == CSS_LINE_STYLE_DOTTED)
  ...
  if (m_Computed.Right > 0 && m_Border.Right.Color.A > 0) {
    if (m_Border.Right.Style == CSS_LINE_STYLE_DASHED ||
        m_Border.Right.Style == CSS_LINE_STYLE_DOTTED)
  ...
  if (m_Computed.Left > 0 && m_Border.Left.Color.A > 0) {
    if (m_Border.Left.Style == CSS_LINE_STYLE_DASHED ||
        m_Border.Left.Style == CSS_LINE_STYLE_DOTTED)

...
static void skipBlankAtEnd(string &end) {
  while (!end.empty()) {
    if ((end[end.size() - 1] == ' ' || end[end.size() - 1] == '\t' ||
         end[end.size() - 1] == '\r' || end[end.size() - 1] == '\n'))
...
// ------------------------------------------------------------------------------------------------
std::string IActionHandler::getParam(const string &Params,
...
// ------------------------------------------------------------------------------------------------
void IActionHandler::getAllParams(const string &Params,
                                  vector<pair<string, string>> &vAllParams) {
  string allparam = Params;
  skipBlankAtStart(allparam);
  while (!allparam.empty()) {
    std::string::size_type e = allparam.find('=');
...
// ------------------------------------------------------------------------------------------------
IActionHandler *CAHManager::getAH(const std::string &name,
                                  std::string &params) {
  // Special AH form?
...
// ------------------------------------------------------------------------------------------------
void CAHManager::parseAH(xmlNodePtr cur, const char *ahId, const char *paramId,
                         IActionHandler *&ahRet, std::string &paramRet) {
  CXMLAutoPtr prop;
  ...
  if (paramId) {
    ...
    */
    if ((const char *)prop &&
        (!paramSpecifiedInAH || strlen((const char *)prop) > 0))
      paramRet = string((const char *)prop);
  }
}

void CAHManager::parseAH(xmlNodePtr cur, const char *ahId, const char *paramId,
                         IActionHandler *&ahRet, CStringShared &paramRet) {
  CXMLAutoPtr prop;
...
// ------------------------------------------------------------------------------------------------
void CAHManager::runActionHandler(const string &ahCmdLine, CCtrlBase *pCaller,
                                  const string &ahUserParams) {
  if (ahCmdLine.empty())
    return;
...
// ------------------------------------------------------------------------------------------------
void CAHManager::runActionHandler(IActionHandler *pAH, CCtrlBase *pCaller,
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    // nlinfo("set %s %s %s %s", dblink.c_str(), property.c_str(),
    // propertyToEval.c_str(), expr.c_str());
    ...
    if (CInterfaceExpr::eval(expr, value, NULL)) {
      if (!dblink.empty()) {
        ...
        if (!value.toInteger()) {
          nlwarning("<CAHSet:execute> expression doesn't evaluate to a "
                    "numerical value");
        }
      ...
      if (!propertyToEval.empty()) {
        ...
        if (!CInterfaceExpr::eval(propertyToEval, res, NULL))
          return;
        res.toString();
      ...
      if (!property.empty()) {
        ...
        if (ig != NULL) {
          ...
          for (uint k = 0; k < targets.size(); ++k) {
            if (targets[k].Elem)
              targets[k].affect(value);
          }
        }
      }
    } else {
      nlwarning(
          "<CAHSet::execute> Couldn't evaluate expression to affect, expr = %s",
          expr.c_str());
    }
  }
};
  ...
  virtual void execute(CCtrlBase * /* pCaller */, const string &Params) {
    ...
    if ((pNBdst != NULL) && (pNBsrc != NULL)) {
      ...
      for (uint i = 0; i < nbLeaves; ++i) {
        ...
        CCDBNodeLeaf *pNLdst = dynamic_cast<CCDBNodeLeaf *>(
            pNBdst->getNode(ICDBNode::CTextId(sTmp)));
        if (pNLdst == NULL) {
...
public:
  virtual void execute(CCtrlBase *pCaller, const string &Params) {
    ...
    CInterfaceElement *pIE =
        CWidgetManager::getInstance()->getElementFromId(pCaller->getId(), elt);
    if (pIE == NULL)
      return;
...
public:
  ...
  virtual void update(ICDBNode *node) {
    ...
    switch (Target) {
    case ContentAlpha:
      AlphaChooserTarget->setContentAlpha((uint8)leaf->getValue32());
      break;
    case BgAlpha:
      AlphaChooserTarget->setContainerAlpha((uint8)leaf->getValue32());
      break;
    case RolloverAlphaContent:
      AlphaChooserTarget->setRolloverAlphaContent(
          (uint8)(255 - (uint8)leaf->getValue32()));
      break;
    case RolloverAlphaBG:
      AlphaChooserTarget->setRolloverAlphaContainer(
          (uint8)(255 - (uint8)leaf->getValue32()));
      break;
    }
  }
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    ...
    if (!_AlphaObserversAdded) {
      ...
      _UiVariableContentAlpha = NLGUI::CDBManager::getInstance()->getDbProp(
          "UI:VARIABLES:ALPHA_CONTENT");
      _UiVariableRolloverAlphaBG = NLGUI::CDBManager::getInstance()->getDbProp(
          "UI:VARIABLES:ALPHA_ROLLOVER_BG");
      _UiVariableRolloverAlphaContent =
          NLGUI::CDBManager::getInstance()->getDbProp(
              "UI:VARIABLES:ALPHA_ROLLOVER_CONTENT");
      ICDBNode::CTextId textIdBGAlpha, textIdContentAlpha,
          textIdRolloverAlphaBG, textIdRolloverAlphaContent;
      _UiVariableBGAlpha->addObserver(&_BgAlphaObs, textIdBGAlpha);
      _UiVariableContentAlpha->addObserver(&_ContentAlphaObs,
                                           textIdContentAlpha);
      _UiVariableRolloverAlphaBG->addObserver(&_RolloverAlphaBGObs,
                                              textIdRolloverAlphaBG);
      _UiVariableRolloverAlphaContent->addObserver(&_RolloverAlphaContentObs,
    ...
    _UiVariableRolloverAlphaBG->setValue32(255 -
                                           gc->getRolloverAlphaContainer());
    _UiVariableRolloverAlphaContent->setValue32(255 -
                                                gc->getRolloverAlphaContent());
    // enable observers
    ...
    NLGUI::CDBManager::getInstance()
        ->getDbProp("UI:VARIABLES:USER_ALPHA")
        ->setValue64(gc->isUsingGlobalAlpha() ? 0 : 1);
    // show the modal box
    CWidgetManager::getInstance()->enableModalWindow(
        gc, "ui:interface:define_ui_transparency");
  }
  ...
  CAHChooseUIAlpha() {
    ...
    _RolloverAlphaContentObs.Target =
...
class CAHUseGlobalAlphaSettings : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */,
                       const std::string & /* Params */) {
    if (AlphaChooserTarget) {
      AlphaChooserTarget->setUseGlobalAlpha(
          !AlphaChooserTarget->isUsingGlobalAlpha());
      NLGUI::CDBManager::getInstance()
          ->getDbProp("UI:VARIABLES:USER_ALPHA")
          ->setValue64(AlphaChooserTarget->isUsingGlobalAlpha() ? 0 : 1);
    }
  }
};
...
class CAHSetTransparent : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const std::string &Params) {
    CGroupContainerBase *pGC = dynamic_cast<CGroupContainerBase *>(
...
class CAHUnlockAllContainer : public IActionHandler {
  virtual void execute(CCtrlBase * /* pCaller */, const string & /* Params */) {
    ...
    for (uint32 nMasterGroup = 0; nMasterGroup < rVMG.size(); nMasterGroup++) {
      //			const CInterfaceManager::SMasterGroup &rMG =
      //rVMG[nMasterGroup];

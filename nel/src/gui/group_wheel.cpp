...
NLMISC_REGISTER_OBJECT(CViewBase, CInterfaceGroupWheel, std::string,
                       "group_wheel");
...
// *****************************************************************************************************************
CInterfaceGroupWheel::CInterfaceGroupWheel(const TCtorParam &param)
...
std::string CInterfaceGroupWheel::getProperty(const std::string &name) const {
  ...
  } else if (name == "on_wheel_down") {
    return getAHString(name);
  } else if (name == "on_wheel_down_params") {
    return _AHWheelDownParams;
  } else
    return CInterfaceGroup::getProperty(name);
}

void CInterfaceGroupWheel::setProperty(const std::string &name,
...
xmlNodePtr CInterfaceGroupWheel::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "on_wheel_up",
             BAD_CAST getAHString("on_wheel_up").c_str());

  xmlSetProp(node, BAD_CAST "on_wheel_up_params",
             BAD_CAST _AHWheelUpParams.toString().c_str());

  xmlSetProp(node, BAD_CAST "on_wheel_down",
             BAD_CAST getAHString("on_wheel_down").c_str());

  xmlSetProp(node, BAD_CAST "on_wheel_down_params",
             BAD_CAST _AHWheelDownParams.toString().c_str());
...
bool CInterfaceGroupWheel::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  if (!CInterfaceGroup::parse(cur, parentGroup))
    return false;
  CAHManager::getInstance()->parseAH(cur, "on_wheel_up", "on_wheel_up_params",
                                     _AHWheelUp, _AHWheelUpParams);
  CAHManager::getInstance()->parseAH(cur, "on_wheel_down",
                                     "on_wheel_down_params", _AHWheelDown,
                                     _AHWheelDownParams);
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      if (eventDesc.getWheel() > 0 && _AHWheelUp) {
        CAHManager::getInstance()->runActionHandler(_AHWheelUp, this,
                                                    _AHWheelUpParams);
        return true;
      } else if (_AHWheelDown) {
        CAHManager::getInstance()->runActionHandler(_AHWheelDown, this,
                                                    _AHWheelDownParams);
        return true;
      }
    }

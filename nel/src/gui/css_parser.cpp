...
TStyleVec CCssParser::parseDecls(const std::string &styleString) {
  ...
  while (pos < end) {
    ...
    styles.push_back(TStylePair(trim(styleString.substr(keyIndex, keyLength)),
                                trim(styleString.substr(pos, sep - pos))));
...
void CCssParser::parseStylesheet(const std::string &cssString,
                                 std::vector<CCssStyle::SStyleRule> &result) {
  _Rules.clear();
  _Style.clear();
...
//
// @internal
void CCssParser::parseRule(const std::string &selectorString,
...
// parse selector list
// @internal
std::vector<CCssSelector>
CCssParser::parse_selector(const std::string &sel, std::string &pseudoElement,
  ...
  while (pos < sel.size() && sel[pos] != ',') {
    ...
    if (sel[pos] == '#') {
      ...
      if (sel[pos] == ']') {
        ...
        if (sel[pos] != '=') {
          ...
          if (value.size() > 2 && value[value.size() - 2] == ' ') {
            ...
            if (lastChar == 'i' || lastChar == 'I' || lastChar == 's' ||
                lastChar == 'S') {
      ...
      if (key[0] == ':' || key == "after" || key == "before" || key == "cue" ||
          key == "first-letter" || key == "first-line") {
      ...
      // user action pseudo classes can be used after pseudo element (ie,
      // :focus, :hover) there is no support for those and its safe to just fail
      // the selector
...
void CCssParser::preprocess() {
  ...
  while (!is_eof()) {
    ...
    if (_Style[_Position] == 0x0C || _Style[_Position] == 0x0D) {
      uint len = 1;
      // CR, LF
      if (charsLeft >= 1 && _Style[_Position] == 0x0D &&
          _Style[_Position + 1] == 0x0A)
        len++;

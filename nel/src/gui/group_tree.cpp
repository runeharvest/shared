...
CGroupTree::SNode::~SNode() {
  makeOrphan();
  // IMPORTANT : must delete in reverse order because "makeOrphan" is called
  // when deleting sons, thus changing vector size...
...
void CGroupTree::SNode::makeOrphan() {
  ...
  if (Template != NULL) {
    if (Template->getParent()) {
      // don't delete because may want to keep it. NB: deleted by smartptr at
      // dtor
...
// ----------------------------------------------------------------------------
struct CNodeSorter {
  bool operator()(const CGroupTree::SNode *lhs,
                  const CGroupTree::SNode *rhs) const {
...
void CGroupTree::SNode::addChildSorted(SNode *pNode) {
  ...
  std::vector<SNode *>::iterator it =
      std::lower_bound(Children.begin(), Children.end(), pNode, CNodeSorter());
...
void CGroupTree::SNode::addChildSortedByBitmap(SNode *pNode) {
  if (!pNode)
    return;
  pNode->makeOrphan();
  std::vector<SNode *>::iterator it = std::lower_bound(
      Children.begin(), Children.end(), pNode, CNodeSorterByBitmap());
  Children.insert(it, pNode);
  pNode->setFather(this);
}
...
void CGroupTree::SNode::detachChild(SNode *pNode) {
  ...
  Children.erase(std::remove(Children.begin(), Children.end(), pNode),
                 Children.end());
  pNode->setFather(NULL);
}

// ----------------------------------------------------------------------------
void CGroupTree::SNode::deleteChild(SNode *pNode) { delete pNode; }
...
void CGroupTree::SNode::addChildAtIndex(SNode *pNode, sint index) {
  ...
  if (index < 0 || index > (sint)Children.size()) {
    nlwarning("<CGroupTree::SNode::addChildAtIndex> bad index %d (%d elements "
              "in the vector)",
              index, Children.size());
    return;
  }
...
// ----------------------------------------------------------------------------
CGroupTree::CGroupTree(const TCtorParam &param) : CInterfaceGroup(param) {
  _IdGenerator = 0;
  _XExtend = 0;
  _BmpW = 14;
  _BmpH = 14;
...
std::string CGroupTree::getProperty(const std::string &name) const {
  ...
  } else if (name == "col_select") {
    return toString(_SelectedColor);
  } else if (name == "col_over_back") {
    return toString(_OverColorBack);
  } else if (name == "fontsize") {
    return toString(_FontSize);
  } else if (name == "select_ancestor_on_close") {
    return toString(_SelectAncestorOnClose);
  } else if (name == "navigate_one_branch") {
    return toString(_NavigateOneBranch);
  } else if (name == "arbo_open_first") {
    return _ArboOpenFirst;
  } else if (name == "arbo_close_just_one") {
    return _ArboCloseJustOne;
  } else if (name == "arbo_son_without_son") {
    return _ArboSonWithoutSon;
  } else if (name == "arbo_son_last") {
    return _ArboSonLast;
  } else if (name == "arbo_son") {
    return _ArboSon;
  } else if (name == "arbo_x_extend") {
    return _ArboXExtend;
  } else if (name == "arbo_level") {
    return _ArboLevel;
  }
  if (name == "rectangle_outline") {
    return toString(_RectangleOutlineMode);
  } else if (name == "rectangle_x") {
    return toString(_RectangleX);
  } else if (name == "rectangle_y") {
    return toString(_RectangleY);
  } else if (name == "rectangle_w") {
    return toString(_RectangleW);
  } else if (name == "rectangle_h") {
    return toString(_RectangleH);
  } else if (name == "rectangle_drl") {
    return toString(_RectangleDeltaRL);
  } else
    return CInterfaceGroup::getProperty(name);
}

void CGroupTree::setProperty(const std::string &name,
...
xmlNodePtr CGroupTree::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "col_select",
             BAD_CAST toString(_SelectedColor).c_str());
  xmlSetProp(node, BAD_CAST "col_over_back",
             BAD_CAST toString(_OverColorBack).c_str());
  xmlSetProp(node, BAD_CAST "fontsize", BAD_CAST toString(_FontSize).c_str());
  xmlSetProp(node, BAD_CAST "select_ancestor_on_close",
             BAD_CAST toString(_SelectAncestorOnClose).c_str());
  xmlSetProp(node, BAD_CAST "navigate_one_branch",
             BAD_CAST toString(_NavigateOneBranch).c_str());
  xmlSetProp(node, BAD_CAST "arbo_open_first", BAD_CAST _ArboOpenFirst.c_str());
  xmlSetProp(node, BAD_CAST "arbo_close_just_one",
             BAD_CAST _ArboCloseJustOne.c_str());
  xmlSetProp(node, BAD_CAST "arbo_son_without_son",
             BAD_CAST _ArboSonWithoutSon.c_str());
  ...
  xmlSetProp(node, BAD_CAST "rectangle_outline",
             BAD_CAST toString(_RectangleOutlineMode).c_str());
  xmlSetProp(node, BAD_CAST "rectangle_x",
             BAD_CAST toString(_RectangleX).c_str());
  xmlSetProp(node, BAD_CAST "rectangle_y",
             BAD_CAST toString(_RectangleY).c_str());
  xmlSetProp(node, BAD_CAST "rectangle_w",
             BAD_CAST toString(_RectangleW).c_str());
  xmlSetProp(node, BAD_CAST "rectangle_h",
             BAD_CAST toString(_RectangleH).c_str());
  xmlSetProp(node, BAD_CAST "rectangle_drl",
             BAD_CAST toString(_RectangleDeltaRL).c_str());
...
// ----------------------------------------------------------------------------
void CGroupTree::checkCoords() { CInterfaceGroup::checkCoords(); }
...
void CGroupTree::drawSelection(sint x, sint y, sint w, CRGBA col) {
  ...
  if (!_RectangleOutlineMode) {
    rVR.drawRotFlipBitmap(_RenderLayer, _XReal + _OffsetX + x,
                          _YReal + _OffsetY + y, w, _BmpH, 0, false,
    ...
    rVR.drawRotFlipBitmap(rl, x, y, 1, h, 0, false, rVR.getBlankTextureId(),
                          col);
    rVR.drawRotFlipBitmap(rl, x + w - 1, y, 1, h, 0, false,
                          rVR.getBlankTextureId(), col);
    rVR.drawRotFlipBitmap(rl, x, y, w, 1, 0, false, rVR.getBlankTextureId(),
                          col);
    rVR.drawRotFlipBitmap(rl, x, y + h - 1, w, 1, 0, false,
                          rVR.getBlankTextureId(), col);
  }
}
...
void CGroupTree::draw() {
  ...
  if (!CWidgetManager::getInstance()->isMouseHandlingEnabled()) {
    bDisplayOver = false;
  } else if (CWidgetManager::getInstance()->getModalWindow() == NULL) {
  ...
  if ((_OverLine != -1) && bDisplayOver) {
    ...
    if (bDisplayOver) {
      ...
      if (_Lines[_OverLine].Node && _Lines[_OverLine].Node->DisplayText) {
        // Get the view text
        viewTextExtend =
            safe_cast<CViewText *>(_Lines[_OverLine].TextOrTemplate);
        // If this viewText is not too big,  no need
        if (viewTextExtend->getXReal() + viewTextExtend->getWReal() <=
            (clipx + clipw))
          viewTextExtend = NULL;
      }
      ...
      if (!viewTextExtend) {
        ...
        if (getModulateGlobalColor()) {
          ...
          col = _OverColor;
          col.A =
              (uint8)(((sint32)col.A * ((sint32)CWidgetManager::getInstance()
        ...
        drawSelection(
            getHrcIconXEnd(_Lines[_OverLine].Depth +
                           _Lines[_OverLine].getNumAdditionnalBitmap()),
            ((sint)_Lines.size() - _OverLine - 1) * _BmpH,
            _WReal -
                getHrcIconXEnd(_Lines[_OverLine].Depth +
                               _Lines[_OverLine].getNumAdditionnalBitmap()),
            col);
      ...
      // Draw extended over
        ...
        if (_SelectedLine == _OverLine) {
          ...
          sel.A = (uint8)((sel.A * ((sint32)CWidgetManager::getInstance()
                                        ->getGlobalColorForContent()
                                        .A +
                                    1)) >>
                          8);
        ...
        CWidgetManager::getInstance()->setOverExtendViewText(viewTextExtend,
                                                             col);
      }
    }
  }
  ...
  if (_SelectedLine != -1) {
    ...
    drawSelection(
        getHrcIconXEnd(_Lines[_SelectedLine].Depth +
                       _Lines[_SelectedLine].getNumAdditionnalBitmap()),
        ((sint)_Lines.size() - _SelectedLine - 1) * _BmpH,
        _WReal -
            getHrcIconXEnd(_Lines[_SelectedLine].Depth +
                           _Lines[_SelectedLine].getNumAdditionnalBitmap()),
        col);
...
void CGroupTree::selectLine(uint line, bool runAH /*= true*/) {
  ...
  if (!_Lines[line].Node->AHName.empty() && runAH) {
    ...
    _Lines[line].Node->AHName, this, _Lines[line].Node->AHParams );
    */
    if (!_CancelNextSelectLine) {
      _SelectedLine = line;
      _SelectedNode = _Lines[line].Node;
    }
    CAHManager::getInstance()->runActionHandler(_Lines[line].Node->AHName, this,
...
bool CGroupTree::rightButton(uint line) {
  ...
  if (line != (uint)_SelectedLine)
    selectLine(line, false);

  CAHManager::getInstance()->runActionHandler(
      _Lines[line].Node->AHNameRight, this, _Lines[line].Node->AHParamsRight);
  return true;
}

// ----------------------------------------------------------------------------
const std::string &CGroupTree::getSelectedNodeId() const {
  if (_SelectedLine >= 0 && _SelectedLine < (sint)_Lines.size() &&
      _Lines[_SelectedLine].Node) {
...
bool CGroupTree::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse && _OverLine >= 0) {
    ...
    bool bText = false;
    if (x >=
        (_XReal + getHrcIconXEnd(_Lines[_OverLine].Depth +
                                 _Lines[_OverLine].getNumAdditionnalBitmap())))
      bText = true;
    bool bIcon = false;
    ...
    bool toggleOne = (eventDesc.getEventTypeExtended() ==
                      NLGUI::CEventDescriptorMouse::mouseleftdown);
    bool toggleAll = (eventDesc.getEventTypeExtended() ==
                      NLGUI::CEventDescriptorMouse::mouserightdown);
    if (toggleOne || toggleAll) {
      ...
      if (bIcon) {
        ...
        if (changedNode) {
          // if "SelectAncestorOnClose" feature wanted,  if it was opened
          // before,  and if some node selected
          ...
          CAHManager::getInstance()->runActionHandler(
              changedNode->AHNameClose, this, changedNode->AHParamsClose);
        }
        forceRebuild();
      }
      return true;
...
void CGroupTree::rebuild() {
  ...
  for (uint i = 0; i < _Lines.size(); ++i)
    _Lines[i].TextOrTemplate->setY(
        _Lines[i].Node->YDecal + sizeH -
        ((1 + _Lines[i].TextOrTemplate->getY()) * _BmpH));
...
void CGroupTree::setRootNode(SNode *pNewRoot) {
  ...
  if (pNewRoot && !refPtrNewRoot) {
    ...
    nlwarning("Trying to set a node that is part of the tree as root node (not "
              "supported yet ...)");
  }
...
void CGroupTree::addTextLine(uint8 nDepth, CGroupTree::SNode *pNode) {
  ...
  if (nDepth > 0) {
    ...
    line.TextOrTemplate->setX(
        getHrcIconXEnd(nDepth - 1 + line.getNumAdditionnalBitmap()));
...
// ----------------------------------------------------------------------------
CViewBitmap *CGroupTree::createViewBitmap(uint line,
                                          const std::string &idPrefix,
                                          const std::string &texture) {
  CViewBitmap *pVB = new CViewBitmap(TCtorParam());
  pVB->setId(idPrefix + toString(_Lines.size()) + "_" +
             toString(_Lines[line].Bmps.size()));
  pVB->setParent(this);
...
void CGroupTree::addHierarchyBitmaps() {
  if (_RootNode)
    _RootNode->updateLastVisibleSon();
    ...
    for (uint nLine = 0; nLine < _Lines.size(); nLine++)
      if (nLayer <= _Lines[nLine].Depth) {
        ...
        if (_Lines[nLine].Depth == nLayer) {
          nCurRootLine = nLine;
          if (_Lines[nLine].Node ==
              _Lines[nCurRootLine].Node->Father->LastVisibleSon)
          ...
          if (haveSomeVisibleChild) {
            ...
            {
                pVB->setTexture("blank.tga");		// create a transparent
            bitmap to have correct "child_resize_w" pVB->setColor(CRGBA(0, 0, 0,
            0));
            }
            else
            {
          ...
          if (nCurRootLine != -1) {
            ...
            bool bFound = false;
            for (uint i = 0; i < _Lines[nCurRootLine].Node->Children.size();
                 ++i)
              if (_Lines[nLine].Node ==
                  _Lines[nCurRootLine].Node->Children[i]) {
                bFound = true;
                break;
              }
            if (bFound) {
              // is it the last child ?
              bool lastSonDisplay = _Lines[nLine].Node ==
                                    _Lines[nCurRootLine].Node->LastVisibleSon;

              // Special for _NavigateOneBranch mode
              if (_NavigateOneBranch) {
                // if node opened,  then his brother are hid! => he behaves like
                // a "last son"
                if (_Lines[nLine].Node->Opened)
                  lastSonDisplay = true;
              }
              ...
              // We have to not display a line if we have passed the last child
              // of this root We never have to display a line also if we are in
              // _NavigateOneBranch mode
  ...
  for (uint nLine = 0; nLine < _Lines.size(); nLine++) {
    if (!_Lines[nLine].Node->Bitmap.empty()) {
      CViewBitmap *pVB =
          createViewBitmap(nLine, "custom_bm", _Lines[nLine].Node->Bitmap);
...
// ***************************************************************************
CGroupTree::SNode *
CGroupTree::selectNodeByIdRecurse(SNode *pNode, const std::string &nodeId) {
  // select this node?
  if (pNode != _RootNode) {
    if (pNode->Id == nodeId)
      return pNode;
...
bool CGroupTree::selectNodeById(const std::string &nodeId, bool triggerAH) {
  ...
  if (selNode) {
    ...
    if (triggerAH) {
      ...
      CAHManager::getInstance()->runActionHandler(selNode->AHName, this,
                                                  selNode->AHParams);
    }
...
// ***************************************************************************
void CGroupTree::cancelNextSelectLine() { _CancelNextSelectLine = true; }
...
// ***************************************************************************
CGroupTree::SNode *
CGroupTree::SNode::luaGetNodeOnStack(CLuaState &ls,
                                     const char * /* funcName */) {
  SNode *node =
      dynamic_cast<CGroupTree::SNode *>(CLuaIHM::getReflectableOnStack(ls, 1));
...
int CGroupTree::SNode::luaGetChild(CLuaState &ls) {
  ...
  if (index < 0 || index >= (sint)Children.size()) {
    std::string range =
        Children.empty() ? "<empty>" : toString("[0, %d]", Children.size() - 1);
    CLuaIHM::fails(ls, "Bad index of tree node child : %d, range is %s",
                   (int)index, range.c_str());
  }

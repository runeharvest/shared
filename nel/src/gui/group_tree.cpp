...
void CGroupTree::draw()
{
	...
	else if (CWidgetManager::getInstance()->getModalWindow() == NULL)
	{
		...
		if ((x < clipx) || (x > (clipx + clipw)) || (y < clipy) || (y > (clipy + cliph)) || !bFound)
		{
			_OverLine = -1;
		}
		else
		{
			...
			{
				if ((y >= (_YReal + (sint32)(_Lines.size() - i - 1) * _BmpH)) && (y < (_YReal + (sint32)(_Lines.size() - i) * _BmpH)))
				{
					_OverLine = i;
				}
			}
...
bool CGroupTree::handleEvent(const NLGUI::CEventDescriptor &event)
{
	...
	if (event.getType() == NLGUI::CEventDescriptor::mouse && _OverLine >= 0)
	{
		...
		if (x >= (_XReal + getHrcIconXEnd(_Lines[_OverLine].Depth + _Lines[_OverLine].getNumAdditionnalBitmap())))
			bText = true;
		bool bIcon = false;
		if ((x > (_XReal + getHrcIconXStart(_Lines[_OverLine].Depth) - _XExtend)) && (x < (_XReal + getHrcIconXEnd(_Lines[_OverLine].Depth + _Lines[_OverLine].getNumAdditionnalBitmap()))))
			bIcon = true;

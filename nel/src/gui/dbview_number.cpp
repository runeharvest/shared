...
CDBViewNumber::CDBViewNumber(const TCtorParam &param) : CViewText(param) {
  _Positive = false;
  _Cache = 0;
...
void CDBViewNumber::setProperty(const std::string &name,
...
xmlNodePtr CDBViewNumber::serialize(xmlNodePtr parentNode,
...
bool CDBViewNumber::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr) {
    // Yoyo: verify doesn't entered a direct number :). MUST BE A CORRECT
    // DATABASE ENTRY
...
void CDBViewNumber::checkCoords() {
  ...
  if (_Cache != val) {
    _Cache = val;
    std::string value =
        _Format ? NLMISC::formatThousands(toString(val)) : toString(val);
    if (_Positive)
      setText(val >= 0 ? (_Prefix.toString() + value + _Suffix.toString())
                       : "?");
    else
      setText(_Prefix.toString() + value + _Suffix.toString());
  }
}

...
// *****************************************************************************************************************
CGroupHeader::CGroupHeader(const TCtorParam &param)
    : CGroupList(param), _HeaderMaxSize(32767) {}
...
void CGroupHeader::enlargeColumns(sint32 margin) {
  ...
  for (uint k = 0; k < entries.size(); ++k) {
    ...
    if (colEnclosing && !colEnclosing->getGroups().empty()) {
      ...
      if (col) {
        // enlarge to the max to be able to measure the sub text (they may clamp
        // themselves based on their first non-"child resizing" parent (see
        // CViewText::updateCoords)
        colEnclosing->setW(16384);
        colEnclosing->invalidateCoords();
        colEnclosing->updateCoords();

        // assume that first child is resizing from its children width (either
        // 'child_resize_w=true' or a CGroupList)
        entries[k]->setW(
            std::max(entries[k]->getMinSize(), col->getW() + margin));
  ...
  if (totalWidth > _HeaderMaxSize) {
    ...
    // search first parent that limit size, if it is larger then enlarge to fit
    // size
    CInterfaceGroup *limitingParent = getParent();
    while (limitingParent && (limitingParent->getResizeFromChildW() ||
                              dynamic_cast<CGroupList *>(limitingParent))) {
    ...
    if (limitingParent && limitingParent->getWReal() > totalWidth) {
      while (limitingParent->getWReal() > totalWidth &&
             totalWidth < _HeaderMaxSize) {
        ...
        for (uint k = 0; k < entries.size(); ++k) {
          ...
          entries[k]->invalidateCoords();
          ++totalWidth;
          if (limitingParent->getWReal() <= totalWidth ||
              totalWidth >= _HeaderMaxSize)
            break;
        }
      }
...
void CGroupHeader::resizeColumnsAndContainer(sint32 margin) {
  ...
  if (totalWidth <= _HeaderMaxSize) {
    ...
    if (limitingParent) {
      getParentContainer()->setW(totalWidth + getParentContainer()->getWReal() -
                                 limitingParent->getWReal());
    } else {
  ...
  if (!entries.empty()) {
    ...
    if (colEnclosing && !colEnclosing->getGroups().empty()) {
      CInterfaceGroup *col = colEnclosing->getGroups()[0];
      if (col) {
        // search first parent that limit size, if it is larger then enlarge to
        // fit size
        CInterfaceGroup *limitingParent = colEnclosing->getParent();
        while (limitingParent && (limitingParent->getResizeFromChildH() ||
                                  dynamic_cast<CGroupList *>(limitingParent)))
          limitingParent = limitingParent->getParent();
        if (limitingParent) {
          getParentContainer()->setH(col->getH() +
                                     getParentContainer()->getHReal() -
                                     limitingParent->getHReal());
        } else {
...
void CGroupHeader::setProperty(const std::string &name,
...
xmlNodePtr CGroupHeader::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "header_max_size",
             BAD_CAST toString(_HeaderMaxSize).c_str());
...
bool CGroupHeader::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  if (!CGroupList::parse(cur, parentGroup))
    return false;
  CXMLAutoPtr prop((const char *)xmlGetProp(cur, (xmlChar *)"header_max_size"));
  if (prop)
    fromString((const char *)prop, _HeaderMaxSize);
  return true;
}
...
public:
  CHeaderEntryResizer(bool rightSide, sint32 wMin)
      : CCtrlBase(TCtorParam()), _RightSide(rightSide), _Moving(false),
        _StartX(0), _OffsetX(0), _WMin(wMin) {}
  void release() {
  ...
  CInterfaceGroup *getTargetGroup() {
    ...
    if (getParent()->getParent() == _Parent->getParentPos())
      return NULL; // leftmost header
    ...
    if (_Parent) {
      if (event.getType() == NLGUI::CEventDescriptor::system) {
        ...
        if (eds.getEventTypeExtended() ==
            NLGUI::CEventDescriptorSystem::setfocus) {
      ...
      if (event.getType() == NLGUI::CEventDescriptor::mouse) {
        ...
        if (eventDesc.getEventTypeExtended() ==
          ...
          if (!this->isIn(eventDesc.getX(), eventDesc.getY()))
            return false;
        ...
        if (eventDesc.getEventTypeExtended() ==
          ...
          if (_Moving &&
            ...
            CGroupHeader *header =
                dynamic_cast<CGroupHeader *>(getParent()->getParent());
            if (header) {
              sint32 w = 0;
              ...
              if (excess) {
                // try to diminish the size of all headers starting from the
                // last
                for (sint k = header->getNumChildren() - 1;
                     k >= 0 && excess > 0; --k) {
                  if (header->getChild(k) == _TargetGroup)
                    break;
                  CGroupHeaderEntry *ghe =
                      dynamic_cast<CGroupHeaderEntry *>(header->getChild(k));
                  sint32 wGain = std::min(
                      excess,
                      std::max((sint32)0, ghe->getW() - ghe->getMinSize()));
                  if (wGain > 0) {
            ...
            CGroupHeaderEntry *ghe = dynamic_cast<CGroupHeaderEntry *>(
                (CInterfaceGroup *)_TargetGroup);
            if (ghe) {
              ghe->setW(_TargetGroup->getW());
              ghe->invalidateCoords();
              CAHManager::getInstance()->runActionHandler(
                  ghe->getAHOnResize(), ghe, ghe->getAHOnResizeParams());
            }
            return true;
          }
          _Moving = false;
  ...
  virtual bool getMouseOverShape(std::string &texName, uint8 &rot,
                                 NLMISC::CRGBA &col) {
...
private:
  NLMISC::CRefPtr<CInterfaceGroup>
      _TargetGroup; // group for which w is modified
...
// *****************************************************************************************************************
CGroupHeaderEntry::CGroupHeaderEntry(const TCtorParam &param)
    : CInterfaceGroup(param) {
  _MinSize = 4;
  _ResizerSize = 4;
}

xmlNodePtr CGroupHeaderEntry::serialize(xmlNodePtr parentNode,
                                        const char *type) const {
  ...
  xmlSetProp(node, BAD_CAST "resizer_size",
             BAD_CAST toString(_ResizerSize).c_str());
  xmlSetProp(node, BAD_CAST "target",
             BAD_CAST toString(_TargetColumnId).c_str());
  xmlSetProp(node, BAD_CAST "on_resize", BAD_CAST _AHOnResize.c_str());
  xmlSetProp(node, BAD_CAST "on_resize_params",
             BAD_CAST _AHOnResizeParams.c_str());
...
NLMISC_REGISTER_OBJECT(CViewBase, CGroupHeaderEntry, std::string,
                       "header_entry");

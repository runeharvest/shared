...
// ----------------------------------------------------------------------------
CGroupList::~CGroupList() { deleteAllChildren(); }
 ...
 */
void CGroupList::setHSParent(CViewBase *view, EAlign addElt, EAlign /* align */,
                             uint space) {
...
void CGroupList::setProperty(const std::string &name,
...
xmlNodePtr CGroupList::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "maxelements",
             BAD_CAST toString(_MaxElements).c_str());
  ...
  xmlSetProp(node, BAD_CAST "dynamic_display_size",
             BAD_CAST toString(_DynamicDisplaySize).c_str());
...
bool CGroupList::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _DynamicDisplaySize = false;
  if (ptr)
    _DynamicDisplaySize = convertBool(ptr);
...
// ----------------------------------------------------------------------------
void CGroupList::addTextChild(const std::string &line,
                              bool multiLine /*= true*/) {
...
// ----------------------------------------------------------------------------
void CGroupList::addTextChild(const std::string &line, const CRGBA &textColor,
                              bool multiLine /*= true*/) {
  const string elid = _Id + ":el" + toString(_IdCounter);
  ++_IdCounter;
  CViewText *view = new CViewText(elid, string(""), _Templ.getFontSize(),
...
// ----------------------------------------------------------------------------
void CGroupList::addTextChildID(const std::string &dbPath,
                                bool multiLine /*=true*/) {
  const string elid = _Id + ":el" + toString(_IdCounter);
  ++_IdCounter;
...
// ----------------------------------------------------------------------------
bool CGroupList::delChild(CViewBase *childToDel, bool noWarning,
...
bool CGroupList::delChild(uint posChildToDel, bool forceDontDelete) {
  ...
  bool elementMustBeDeleted =
      _Elements[posChildToDel].EltDeleteOnRemove && !forceDontDelete;
...
// ----------------------------------------------------------------------------
void CGroupList::setTextTemplate(const CViewText &templ) { _Templ = templ; }
...
void CGroupList::draw() {
  ...
  // 0,   false,   rVR.getBlankTextureId(),   CRGBA(0,  255,  0,  255) );
  if (_Over) {
    CViewRenderer &rVR = *CViewRenderer::getInstance();
...
struct CRemoveViewPred {
  bool operator()(const CGroupList::CElementInfo &info) const {
...
// ----------------------------------------------------------------------------
bool CGroupList::addChildAtIndex(CViewBase *child, uint index,
...
void CGroupList::swapChildren(uint index1, uint index2) {
  if (index1 >= _Elements.size() || index2 >= _Elements.size()) {
...
int CGroupList::luaGetChild(CLuaState &ls) {
  ...
  if (index < 0 || index >= (sint)_Elements.size()) {
    CLuaIHM::fails(ls,
                   "getChild : trying to access element %d in list '%s', which "
                   "has %d elements",

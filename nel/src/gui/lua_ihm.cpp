...
// ***************************************************************************
/*
IMPORTANT NOTE: we do this heavy double registration in this file because we
DON'T want to include luabind.hpp in every file. Compilation is VERY SLOW
*/
// ***************************************************************************
...
// declare ostream << operator for ucstring -> registration of ucstring iin
// luabind will build a 'tostring' function from it
...
struct CMiscFunctions {
  ...
  static void shellExecute(const std::string &operation,
...
#define LUA_REGISTER_BASIC(_type_)                                             \
  ...
  _type_ convert_lua_to_cpp(lua_State *L, luabind::detail::by_value<_type_>,   \
                            int index) {                                       \
    return (_type_)lua_tointeger(L, index);                                    \
  }                                                                            \
  int match_lua_to_cpp(lua_State *L, luabind::detail::by_value<_type_>,        \
                       int index) {                                            \
...
int CLuaIHM::luaUIIndex(CLuaState &ls) {
  ...
  CInterfaceElement *uiRelative =
      getUIRelative(dynamic_cast<CInterfaceElement *>(pRPT), propName);
  if (uiRelative) {
  ...
  // Yoyo: don't write any message or warning because this may be a feature (if
  // user want to test that something exit in the ui)
  ls.pushNil();
  return 1;
}
...
int CLuaIHM::luaUINewIndex(CLuaState &ls) {
  ...
  if (propName == lstr_Env) {
    ...
    throw ELuaIHMException("You cannot change the Env Table of '%s'",
                           name.c_str());
  }
  ...
  if (uiRelative) {
    // Exception!!! not allowed
    throw ELuaIHMException("You cannot write into the UI '%s' of '%s'",
                           propName, pIE->getId().c_str());
  }

  // ** Prop Not Found
  throw ELuaIHMException("Property '%s' not found in '%s' of type %s", propName,
                         pIE ? pIE->getId().c_str() : "<reflectable element>",
                         typeid(*pRPT).name());
...
int CLuaIHM::luaUINext(CLuaState &ls) {
  ...
  //  If object is a CInterfaceGroup, iteration is also done on sons (groups,
  //  controls & view).

  if (ls.getTop() != 2) {
    CLuaIHM::fails(ls, "__next metamethod require 2 arguments (table & key)");
  }
  CLuaIHM::check(ls, CLuaIHM::isReflectableOnStack(ls, 1),
                 "__next :  require ui element as first arg");
  CReflectableRefPtrTarget *reflectedObject =
      CLuaIHM::getReflectableOnStack(ls, 1);
  // To traverse all properties / field of the object, we must be able to
  // determine the next key from a previous key (keys are ordered) We use the
  // 'TValueType' enum to know which kind of property we are traversing, and an
  // index in this group of properties The key which uniquely identify an
  // element / property in the reflectable object
  struct CKey {
    enum TValueType {
    ...
    sint Index;
    const CClassInfo *ClassInfo; // if ValueType is "VTProp" -> give the class
                                 // for which property are currently enumerated
    //
    static int tostring(CLuaState &ls) // '__print' metamathod
    {
      ...
      case VTGroup:
        ls.push(toString("_Group %d", key.Index));
        break;
      case VTView:
        ls.push(toString("_View %d", key.Index));
        break;
      case VTCtrl:
        ls.push(toString("_Ctrl %d", key.Index));
        break;
      case VTProp:
        ls.push(key.ClassInfo->Properties[key.Index].Name);
        break;
      }
      return 1;
    }
  ...
  while (enumerate) {
    ...
    case CKey::VTProp:
      ...
      if ((sint)key.ClassInfo->Properties.size() == (key.Index + 1)) {
        key.ClassInfo =
            key.ClassInfo->ParentClass; // continue enumeration in parent class
        key.Index = -1;
      } else {
        ++key.Index;
        key.push(ls);
        CLuaIHM::luaValueFromReflectedProperty(
            ls, *reflectedObject, key.ClassInfo->Properties[key.Index]);
        return 2;
      }
      break;
    default:
...
CInterfaceElement *CLuaIHM::getUIOnStack(CLuaState &ls, sint index) {
  ...
  return dynamic_cast<CInterfaceElement *>(
      CLuaIHM::getReflectableOnStack(ls, index));
}

// ***************************************************************************
void CLuaIHM::checkArgTypeUIElement(CLuaState &ls, const char *funcName,
                                    uint index) {
  // H_AUTO(Lua_CLuaIHM_checkArgTypeUIElement)
  nlassert(index > 0);
  if (ls.getTop() < (int)index) {
    CLuaIHM::fails(
        ls, "%s : argument %d of expected type ui element was not defined",
        funcName, index);
  }
  if (!isUIOnStack(ls, index)) {
    CLuaIHM::fails(
        ls, "%s : argument %d of expected type ui element has bad type : %s",
        funcName, index, ls.getTypename(ls.type(index)), ls.type(index));
  }
}

// ***************************************************************************
CInterfaceElement *CLuaIHM::getUIRelative(CInterfaceElement *pIE,
...
void CLuaIHM::registerBasics(CLuaState &ls) {
  ...
  // RGBA
  module(L)[class_<NLMISC::CRGBA>("CRGBA")
                .def(constructor<>())
  ...
  // CVector2f
  module(L)[class_<NLMISC::CVector2f>("CVector2f")
...
int CLuaIHM::luaMethodCall(lua_State *ls) {
  ...
  const CReflectedProperty *prop =
      (const CReflectedProperty *)lua_touserdata(ls, lua_upvalueindex(1));
  CLuaState *state = (CLuaState *)lua_touserdata(ls, lua_upvalueindex(2));
  nlassert(prop);
  nlassert(prop->Type == CReflectedProperty::LuaMethod);
  nlassert(state);
  if (state->empty()) {
    state->push(NLMISC::toString(
        "Error while calling lua method %s:%s : no 'self' reference provided,  "
        "  did you you function call '.' instead of method call ':' ?",
  ...
  if (!pRPT) {
    state->push(NLMISC::toString(
        "Error while calling lua method %s:%s : 'self' pointer is nil or of "
        "bad type,    can't make the call.",
        prop->ParentClass->ClassName.c_str(), prop->Name.c_str()));
    lua_error(ls);
  }
  //
  ...
  if (pRPT) {
    ...
    try {
      ...
      // TODO : see if this is safe to call lua error there" ... (it does a long
      // jump)
      lua_error(ls);
    }
  }
...
int CLuaIHM::setOnDraw(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "setOnDraw() requires a UI object in param 1");
  CLuaIHM::check(ls, ls.isString(2),
                 "setOnDraw() requires a string in param 2");
  ...
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(pIE);
  if (!group)
    throw ELuaIHMException("setOnDraw(): '%s' is not a group",
                           pIE->getId().c_str());
...
int CLuaIHM::getOnDraw(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "getOnDraw() requires a UI object in param 1");
...
int CLuaIHM::addOnDbChange(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "addOnDbChange() requires a UI object in param 1");
  CLuaIHM::check(ls, ls.isString(2),
                 "addOnDbChange() requires a string in param 2");
  CLuaIHM::check(ls, ls.isString(3),
                 "addOnDbChange() requires a string in param 3");
  ...
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(pIE);
  if (!group)
    throw ELuaIHMException("addOnDbChange(): '%s' is not a group",
                           pIE->getId().c_str());
...
int CLuaIHM::removeOnDbChange(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "removeOnDbChange() requires a UI object in param 1");
  CLuaIHM::check(ls, ls.isString(2),
                 "removeOnDbChange() requires a string in param 2");
  ...
  CInterfaceGroup *group = dynamic_cast<CInterfaceGroup *>(pIE);
  if (!group)
    throw ELuaIHMException("removeOnDbChange(): '%s' is not a group",
                           pIE->getId().c_str());
...
int CLuaIHM::getUIId(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "getUIId() requires a UI object in param 1");
...
int CLuaIHM::runAH(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1) || ls.isNil(1),
                 "runAH() requires a UI object in param 1 (or Nil)");
  ...
  if (pIE) {
    ...
    if (!ctrl)
      throw ELuaIHMException("runAH(): '%s' is not a ctrl",
                             pIE->getId().c_str());
  }
...
int CLuaIHM::deleteUI(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, CLuaIHM::isUIOnStack(ls, 1),
                 "deleteUI() requires a UI object in param 1");
...
int CLuaIHM::deleteReflectable(CLuaState &ls) {
  ...
  CLuaIHM::check(
      ls, CLuaIHM::isReflectableOnStack(ls, 1),
      "deleteReflectable() requires a reflectable C++ object in param 1");
...
int CLuaIHM::runCommand(CLuaState &ls) {
  ...
  if (!NLMISC::ICommand::LocalCommands ||
      !NLMISC::ICommand::LocalCommands->count(ls.toString(1))) {
    nlwarning("'runCommand' : Command %s not found", ls.toString(1));
    ls.push(false);
    return 1;
  }
  std::string rawCommandString = ls.toString(1);
  NLMISC::ICommand *command =
      (*NLMISC::ICommand::LocalCommands)[ls.toString(1)];
  nlassert(command);
  ...
  ls.push(command->execute(rawCommandString, args, NLMISC::ErrorLog(), false,
                           true));
  return 1;
}
...
// ***************************************************************************
void CLuaIHM::luaValueFromReflectedProperty(
    CLuaState &ls, CReflectable &reflectedObject,
    const CReflectedProperty &property) {
  ...
  case CReflectedProperty::LuaMethod: {
    ...
    nlassert(property.LuaMethodRef.getLuaState() ==
             &ls); // only one single lua state supported for now
...
void CLuaIHM::luaValueToReflectedProperty(CLuaState &ls, int stackIndex,
  ...
  if (ls.isNil(stackIndex))
    throw ELuaIHMException("Trying to set nil to UI property '%s'",
                           property.Name.c_str());
  switch (property.Type) {
  case CReflectedProperty::Boolean: {
  ...
  case CReflectedProperty::UCStringRef: {
    ucstring val;
    // Additionaly return of CInterfaceExpr may be std::string... test std
    // string too
    if (ls.isString() || ls.isNumber() || ls.isInteger()) {
      ...
      if (!pop(ls, val)) {
        throw ELuaIHMException(
            "You must set a string,    number or ucstring to UI property '%s'",
            property.Name.c_str());
      }
    }
#endif
  ...
  case CReflectedProperty::RGBA: {
    ...
    if (pop(ls, color)) {
      ...
      throw ELuaIHMException("You must set a CRGBA to UI property '%s'",
                             property.Name.c_str());
    }
    return;
  }
...
void CLuaIHM::registerIHM(CLuaState &ls) {
  ...
  luabind::module(
      L)[luabind::def("findReplaceAll",
                      (std::string(*)(const std::string &, const std::string &,
                                      const std::string &)) &
                          findReplaceAll),
         luabind::def("findReplaceAll",
...
#if !FINAL_VERSION
         LUABIND_FUNC(openDoc), LUABIND_FUNC(launchProgram),
#endif
  ...
  // inside 'nlfile' table
  luabind::module(
      L, "nlfile")[luabind::def("getFilename", NLMISC::CFile::getFilename),
                   luabind::def("getExtension", NLMISC::CFile::getExtension),
                   luabind::def("getFilenameWithoutExtension",
...
double CLuaIHM::getPreciseLocalTime() {
  ...
  //  don't export these 2 function to lua directly here, because all uint64
  //  can't be represented with lua 'numbers'
...
// ***************************************************************************
void CLuaIHM::pushReflectableOnStack(CLuaState &ls,
                                     class CReflectableRefPtrTarget *pRPT) {
...
// ***************************************************************************
CReflectableRefPtrTarget *CLuaIHM::getReflectableOnStack(CLuaState &ls,
                                                         sint index) {
...
// ***************************************************************************
std::string CLuaIHM::findReplaceAll(const std::string &str,
...
#ifdef RYZOM_LUA_UCSTRING
// ***************************************************************************
ucstring CLuaIHM::findReplaceAll(const ucstring &str, const ucstring &search,
                                 const ucstring &replace) {
  // H_AUTO(Lua_CLuaIHM_findReplaceAll)
  return strFindReplaceAll(str, search, replace);
}

// ***************************************************************************
ucstring CLuaIHM::findReplaceAll(const ucstring &str, const std::string &search,
                                 const std::string &replace) {
  // H_AUTO(Lua_CLuaIHM_findReplaceAll)
  return findReplaceAll(str, ucstring::makeFromUtf8(search),
...
void CLuaIHM::checkArgCount(CLuaState &ls, const char *funcName, uint nArgs) {
  ...
  if (ls.getTop() != (sint)nArgs) {
    fails(ls,
          "%s() need exactly %d arguments (tips : check between method & "
          "function call)",
          funcName, nArgs);
  }
}
...
void CLuaIHM::checkArgMin(CLuaState &ls, const char *funcName, uint nArgs) {
  ...
  if (ls.getTop() < (sint)nArgs) {
    fails(ls,
          "%s() need at least %d arguments (tips : check between method & "
          "function call)",
          funcName, nArgs);
  }
}
...
// ***************************************************************************
void CLuaIHM::checkArgType(CLuaState &ls, const char *funcName, uint index,
                           int argType) {
  // H_AUTO(Lua_CLuaIHM_checkArgType)
  nlassert(index > 0);
  if (ls.getTop() < (int)index) {
    fails(ls, "%s : argument %d of expected type %s was not defined", funcName,
          index, ls.getTypename(argType));
  }
  if (ls.type(index) != argType) {
    fails(ls, "%s : argument %d of expected type %s has bad type : %s",
          funcName, index, ls.getTypename(argType),
          ls.getTypename(ls.type(index)), ls.type(index));
  }
}

// ***************************************************************************
void CLuaIHM::checkArgTypeRGBA(CLuaState &ls, const char *funcName,
                               uint index) {
  // H_AUTO(Lua_CLuaIHM_checkArgTypeRGBA)
  nlassert(index > 0);
  if (ls.getTop() < (int)index) {
    fails(ls, "%s : argument %d of expected type RGBA was not defined",
          funcName, index);
  }
  ls.pushValue(index);
  CRGBA dummy;
  if (!pop(ls, dummy)) {
    fails(ls, "%s : argument %d of expected type RGBA has bad type : %s",
          funcName, index, ls.getTypename(ls.type(index)), ls.type(index));
  }
}
...
#ifdef RYZOM_LUA_UCSTRING
// ***************************************************************************
void CLuaIHM::checkArgTypeUCString(CLuaState &ls, const char *funcName,
                                   uint index) {
  // H_AUTO(Lua_CLuaIHM_checkArgTypeUCString)
  nlassert(index > 0);
  if (ls.getTop() < (int)index) {
    fails(ls, "%s : argument %d of expected type ucstring was not defined",
          funcName, index);
  }
  ls.pushValue(index);
  ucstring dummy;
  if (!pop(ls, dummy)) {
    fails(ls, "%s : argument %d of expected type ucstring has bad type : %s",
          funcName, index, ls.getTypename(ls.type(index)), ls.type(index));
  }
}
#endif
...
// ***************************************************************************
void CLuaIHM::getPoly2DOnStack(CLuaState &ls, sint index,
                               NLMISC::CPolygon2D &dest) {

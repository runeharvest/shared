...
struct CNameToHtmlElement {
  HTMLElement ID;
  const char *Name;
  CNameToHtmlElement(HTMLElement id, const char *name) : ID(id), Name(name) {}
};
...
// Read a width HTML parameter. "100" or "100%". Returns true if percent (0 ~ 1)
// else false
...
bool scanHTMLColor(const char *src, CRGBA &dest) {
  ...
  if (*src == '#') {
    ...
    CRGBA result;
    src = scanColorComponent(src, result.R);
    if (!src)
      return false;
    src = scanColorComponent(src, result.G);
    if (!src)
      return false;
    src = scanColorComponent(src, result.B);
    if (!src)
      return false;
  ...
  if (strnicmp(src, "rgb(", 4) == 0 || strnicmp(src, "rgba(", 5) == 0) {
    ...
    if (parts.size() >= 3) {
      ...
      // R
      if (getPercentage(tmpv, tmpf, parts[0].c_str()))
        tmpv = 255 * tmpf;
      clamp(tmpv, 0, 255);
      result.R = tmpv;

      // G
      if (getPercentage(tmpv, tmpf, parts[1].c_str()))
        tmpv = 255 * tmpf;
      clamp(tmpv, 0, 255);
      result.G = tmpv;

      // B
      if (getPercentage(tmpv, tmpf, parts[2].c_str()))
        tmpv = 255 * tmpf;
      clamp(tmpv, 0, 255);
      result.B = tmpv;
  ...
  if (strnicmp(src, "hsl(", 4) == 0 || strnicmp(src, "hsla(", 5) == 0) {
    ...
    if (parts.size() >= 3) {
      ...
      float h, s, l;
      // hue
      if (!fromString(parts[0], tmpv))
        return false;
      tmpv = ((tmpv % 360) + 360) % 360;
      h = (float)tmpv / 360.0f;

      // saturation
      if (!getPercentage(tmpv, s, parts[1].c_str()))
        return false;
      clamp(s, 0.0f, 1.0f);

      // lightness
      if (!getPercentage(tmpv, l, parts[2].c_str()))
        return false;
      clamp(l, 0.0f, 1.0f);
...
std::string getRGBAString(const CRGBA &color) {
  return toString("rgba(%d, %d, %d, %.1f)", color.R, color.G, color.B,
                  color.A / 255.f);
}

// update HTTPCookies list
static void receiveCookie(const char *nsformat, const std::string &domain,
                          bool trusted) {
  // 0        1           2       3       4       5       6
  // domain	tailmatch	path	secure	expires	name	value
  // .app.ryzom.com	TRUE	/	 FALSE	1234	ryzomId
  // AAAAAAAA|BBBBBBBB|CCCCCCCC #HttpOnly_app.ryzom.com	FALSE	/	FALSE
  // 0	PHPSESSID	sess-id-value
  ...
  if (chunks[5] == "ryzomId") {
    ...
    if (trusted && CurrentCookie != chunks[6]) {
      ...
      nlwarning("received ryzomId cookie '%s' from trusted domain '%s'",
                CurrentCookie.c_str(), domain.c_str());
    }
  } else {
    uint32 expires = 0;
    ...
    // nlwarning("save domain '%s' cookie '%s' value '%s'", domain.c_str(),
    // chunks[5].c_str(), nsformat);
...
void sendCookies(CURL *curl, const std::string &domain, bool trusted) {
  ...
  if (trusted && !CurrentCookie.empty()) {
    // domain	tailmatch	path	secure	expires	name	value
    // .app.ryzom.com	TRUE	/	 FALSE	1234	ryzomId
    // AAAAAAAA|BBBBBBBB|CCCCCCCC #HttpOnly_app.ryzom.com	FALSE	/
    // FALSE	0	PHPSESSID	sess-id-value
    std::string cookie;
    // set tailmatch
  ...
  if (!HTTPCookies[domain].empty()) {
    for (std::map<std::string, std::string>::iterator it =
             HTTPCookies[domain].begin();
         it != HTTPCookies[domain].end(); it++) {
      curl_easy_setopt(curl, CURLOPT_COOKIELIST, it->second.c_str());
      // nlwarning("set domain '%s' cookie '%s'", domain.c_str(),
      // it->second.c_str());
    }
  }
}

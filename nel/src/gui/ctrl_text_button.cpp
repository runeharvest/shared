...
std::string CCtrlTextButton::getProperty(const std::string &name) const {
  ...
  if (!prop.empty())
    ...
    std::string tex;
    tex =
        CViewRenderer::getInstance()->getTextureNameFromId(_TextureIdNormal[0]);
    std::string::size_type i = tex.rfind("_l.tga");
    if (i != std::string::npos)
      tex = tex.substr(0, i);
    return tex;
  } else if (name == "tx_pushed") {
    std::string tex;
    tex =
        CViewRenderer::getInstance()->getTextureNameFromId(_TextureIdPushed[0]);
    std::string::size_type i = tex.rfind("_l.tga");
    if (i != std::string::npos)
      tex = tex.substr(0, i);
    return tex;
  } else if (name == "tx_over") {
    std::string tex;
...
xmlNodePtr CCtrlTextButton::serialize(xmlNodePtr parentNode,
  ...
  xmlNewProp(node, BAD_CAST "hardtext",
             BAD_CAST _ViewText->getHardText().c_str());
  ...
  xmlNewProp(node, BAD_CAST "text_underlined",
             BAD_CAST toString(_ViewText->getUnderlined()).c_str());

  tex =
      CInterfaceElement::HotSpotCoupleToString(_TextParentPosRef, _TextPosRef);
  xmlNewProp(node, BAD_CAST "text_posref", BAD_CAST tex.c_str());

  xmlNewProp(node, BAD_CAST "text_color_normal",
             BAD_CAST toString(_TextColorNormal).c_str());
  xmlNewProp(node, BAD_CAST "text_color_pushed",
             BAD_CAST toString(_TextColorPushed).c_str());
  xmlNewProp(node, BAD_CAST "text_color_over",
             BAD_CAST toString(_TextColorOver).c_str());

  xmlNewProp(node, BAD_CAST "text_shadow_color_normal",
             BAD_CAST toString(_TextShadowColorNormal).c_str());
  xmlNewProp(node, BAD_CAST "text_shadow_color_pushed",
             BAD_CAST toString(_TextShadowColorPushed).c_str());
  xmlNewProp(node, BAD_CAST "text_shadow_color_over",
             BAD_CAST toString(_TextShadowColorOver).c_str());
  ...
  xmlNewProp(node, BAD_CAST "force_text_over",
             BAD_CAST toString(_ForceTextOver).c_str());
  xmlNewProp(node, BAD_CAST "text_header_color",
             BAD_CAST toString(_TextHeaderColor).c_str());
...
bool CCtrlTextButton::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _TextModulateGlobalColorNormal = _TextModulateGlobalColorPushed =
      _TextModulateGlobalColorOver = getModulateGlobalColor();

  // Read special text global_color for each state
  prop = (char *)xmlGetProp(cur, (xmlChar *)"text_global_color_normal");
  if (prop)
    _TextModulateGlobalColorNormal = convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"text_global_color_pushed");
  if (prop)
    _TextModulateGlobalColorPushed = convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"text_global_color_over");
  if (prop)
    _TextModulateGlobalColorOver = convertBool(prop);
  prop = (char *)xmlGetProp(cur, (xmlChar *)"force_text_over");
  if (prop)
    _ForceTextOver = convertBool(prop);

  // read Text header color
  prop = (char *)xmlGetProp(cur, (xmlChar *)"text_header_color");
  if (prop)
    _TextHeaderColor = convertBool(prop);

  return true;
}

// ***************************************************************************
void CCtrlTextButton::setTexture(const std::string &l, const std::string &m,
                                 const std::string &r, bool updateHeight) {
  nlctassert(NumTexture == 3);
...
// ***************************************************************************
void CCtrlTextButton::setTexturePushed(const std::string &l,
                                       const std::string &m,
                                       const std::string &r) {
  nlctassert(NumTexture == 3);
...
// ***************************************************************************
void CCtrlTextButton::setTextureOver(const std::string &l, const std::string &m,
                                     const std::string &r) {
  nlctassert(NumTexture == 3);
...
void CCtrlTextButton::draw() {
  ...
  case RadioButton: {
    //			CViewPointer &rIP =
    //*CInterfaceManager::getInstance()->getPointer();
  ...
  txh = _HReal;
  nlctassert(NumTexture == 3);
  rVR.drawRotFlipBitmap(_RenderLayer, x, y, _BmpLeftW, txh, 0, false, pTxId[0],
                        color);
  rVR.drawRotFlipBitmap(_RenderLayer, x + _BmpLeftW, y, txw, txh, 0, false,
                        pTxId[1], color);
  rVR.drawRotFlipBitmap(_RenderLayer, x + _BmpLeftW + txw, y, _BmpRightW, txh,
                        0, false, pTxId[2], color);

  CCtrlBase *capturePointerLeft =
      CWidgetManager::getInstance()->getCapturePointerLeft();

  // *** Draw Over
  if ((!editorMode && _Over &&
       (_OverWhenPushed || !(_Pushed || capturePointerLeft == this)))) {
    if (!editorMode && (lastOver == false) && (_AHOnOver != NULL))
      CAHManager::getInstance()->runActionHandler(_AHOnOver, this,
                                                  _AHOverParams);
    ...
    uint layerOffset =
        _ForceTextOver ? 0 : 1; // Must write Over On Top of the text ?

    rVR.drawRotFlipBitmap(_RenderLayer + layerOffset, x, y, _BmpLeftW, txh, 0,
                          false, pTxId[0], color);
    rVR.drawRotFlipBitmap(_RenderLayer + layerOffset, x + _BmpLeftW, y, txw,
                          txh, 0, false, pTxId[1], color);
    rVR.drawRotFlipBitmap(_RenderLayer + layerOffset, x + _BmpLeftW + txw, y,
                          _BmpRightW, txh, 0, false, pTxId[2], color);
  }

  // *** Setup ViewText Color according to selected one (should be drawn after
  // because of eltorder) update header color?
  CRGBA viewTextColor;
  if (_TextHeaderColor) {
  ...
  if (_ViewText != NULL) {
    if (pTxId == _TextureIdNormal || editorMode) {
      if (_TextHeaderColor)
        viewTextColor.A = _TextColorNormal.A;
      else
        viewTextColor = _TextColorNormal;
      ...
      if (_TextHeaderColor)
        viewTextColor.A = _TextColorPushed.A;
      else
        viewTextColor = _TextColorPushed;
      _ViewText->setColor(viewTextColor);
      ...
      if (_TextHeaderColor)
        viewTextColor.A = _TextColorOver.A;
      else
        viewTextColor = _TextColorOver;
      _ViewText->setColor(viewTextColor);
...
// ***************************************************************************
sint32 CCtrlTextButton::getMaxUsedW() const { return _W; }

// ***************************************************************************
sint32 CCtrlTextButton::getMinUsedW() const { return _W; }
...
CViewText *CCtrlTextButton::getViewText() { return _ViewText; }

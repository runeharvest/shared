...
CHtmlElement::CHtmlElement(ENodeType type, std::string value)
    : ID(0), Type(type), Value(value), parent(NULL), previousSibling(NULL),
      nextSibling(NULL), childIndex(0) {}
...
// ***************************************************************************
void CHtmlElement::clearPseudo() { _Pseudo.clear(); }
...
void CHtmlElement::setPseudo(const std::string &key, const TStyle &style) {
  ...
  if (it != _Pseudo.end()) {
    ...
    for (TStyle::const_iterator itStyle = style.begin(); itStyle != style.end();
         ++itStyle) {
...
std::string CHtmlElement::serializeAttributes(bool escape) const {
  ...
  for (std::map<std::string, std::string>::const_iterator it =
    ...
    if (it->first == "class") {
      result += " class=\"";
      for (std::set<std::string>::const_iterator it2 = ClassNames.begin();
           it2 != ClassNames.end(); ++it2) {
      ...
      result += " " + it->first + "=\"" +
                (escape ? htmlEscape(it->second) : it->second) + "\"";
    }
  }
  return result;
}
...
std::string CHtmlElement::serializeChilds(bool escape) const {
  std::string result;
  for (std::list<CHtmlElement>::const_iterator it = Children.begin();
       it != Children.end(); ++it)
...
std::string CHtmlElement::serialize(bool escape) const {
  ...
  if (ID == HTML_AREA || ID == HTML_BASE || ID == HTML_BR || ID == HTML_COL ||
      ID == HTML_EMBED || ID == HTML_HR || ID == HTML_IMG || ID == HTML_INPUT ||
      ID == HTML_LINK || ID == HTML_META || ID == HTML_PARAM ||
      ID == HTML_WBR) {
    return result;
  }

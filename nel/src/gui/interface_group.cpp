...
#define IG_UNIQUE_ID(this)                                                     \
  ((void *)&(                                                                  \
      (this)->_GroupSizeRef)) // NB nico : use some pointer *inside*
                              // CInterfaceGroup as a unique id for lua registry
                              // (any field but the first), instead of using
                              // 'this'. 'this' is already used by
                              //  CLuaIHM::pushReflectableOnStack as unique id
                              //  to CInterfaceElement's ref pointers
...
// ------------------------------------------------------------------------------------------------
NLMISC_REGISTER_OBJECT(CViewBase, CInterfaceGroup, std::string,
                       "interface_group");
...
void CInterfaceGroup::setIdRecurse(const std::string &id) {
  ...
  for (std::vector<CInterfaceGroup *>::iterator it = _ChildrenGroups.begin();
       it != _ChildrenGroups.end(); ++it) {
    (*it)->setIdRecurse((*it)->getShortId());
  }
  for (std::vector<CCtrlBase *>::iterator it = _Controls.begin();
       it != _Controls.end(); ++it) {
    (*it)->setIdRecurse((*it)->getShortId());
  }
  for (std::vector<CViewBase *>::iterator it = _Views.begin();
       it != _Views.end(); ++it) {
    (*it)->setIdRecurse((*it)->getShortId());
  }
}
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::notifyActiveCalled(
    const NLGUI::CEventDescriptorActiveCalledOnParent &desc) {
  // notify children that the 'active' state of this group has changed
  for (std::vector<CInterfaceGroup *>::iterator it = _ChildrenGroups.begin();
       it != _ChildrenGroups.end(); ++it) {
    (*it)->handleEvent(desc);
  }
  for (std::vector<CCtrlBase *>::iterator ctrlIt = _Controls.begin();
       ctrlIt != _Controls.end(); ++ctrlIt) {
...
void CInterfaceGroup::setActive(bool state) {
  if (state != getActive()) {
    ...
    if (_AHOnActive != NULL && state) {
      CAHManager::getInstance()->runActionHandler(_AHOnActive, this,
                                                  _AHOnActiveParams);
    }
    if (_AHOnDeactive != NULL && !state) {
      CAHManager::getInstance()->runActionHandler(_AHOnDeactive, this,
                                                  _AHOnDeactiveParams);
    }
...
CInterfaceGroup::~CInterfaceGroup() {
  ...
  clearGroups();
  //	nlinfo ("%d seconds for clearGroups '%s'", (uint32)(ryzomGetLocalTime
  //()-initStart)/1000, _Id.c_str()); 	initStart = ryzomGetLocalTime ();
  clearControls();
  //	nlinfo ("%d seconds for clearControls '%s'", (uint32)(ryzomGetLocalTime
  //()-initStart)/1000, _Id.c_str()); 	initStart = ryzomGetLocalTime ();
  clearViews();
  //	nlinfo ("%d seconds for clearViews '%s'", (uint32)(ryzomGetLocalTime
  //()-initStart)/1000, _Id.c_str());
...
void CInterfaceGroup::clearViews() {
  ...
  while (!_Views.empty()) {
    ...
    delEltOrder(pVB);
    delete pVB;
    // slower than a _Views.clear() out of loop, but we have to keep a clean
    // array.
    _Views.pop_back();
  }
}
...
void CInterfaceGroup::clearControls() {
  // Yoyo: important to not Leave NULL in the array, because of ~CGroupHTML()
  // and LibWWW callback that may call CInterfaceManager::getElementFromId()
  // (and this method hates having NULL in the arrays ^^)
  while (!_Controls.empty()) {
    CCtrlBase *pCB = _Controls.back();
    delEltOrder(pCB);
    delete pCB;
    // slower than a _Controls.clear() out of loop, but we have to keep a clean
    // array.
    _Controls.pop_back();
  }
}
...
void CInterfaceGroup::clearGroups() {
  ...
  while (!_ChildrenGroups.empty()) {
    ...
    delEltOrder(pIG);
    delete pIG;
    // slower than a _ChildrenGroups.clear() out of loop, but we have to keep a
    // clean array.
...
bool CInterfaceGroup::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  // determine if the group is a window. If the property is not specified, set
  // it to false.
  ...
  CAHManager::getInstance()->parseAH(cur, "on_active", "on_active_params",
                                     _AHOnActive, _AHOnActiveParams);
  CAHManager::getInstance()->parseAH(cur, "on_deactive", "on_deactive_params",
                                     _AHOnDeactive, _AHOnDeactiveParams);

  // Read user max size
  ptr = (char *)xmlGetProp(cur, (xmlChar *)"max_w");
  if (ptr)
    NLMISC::fromString((const char *)ptr, _MaxW);
  ptr = (char *)xmlGetProp(cur, (xmlChar *)"max_h");
  if (ptr)
    NLMISC::fromString((const char *)ptr, _MaxH);
  ...
  if (ptr) {
    ...
    CWidgetManager::getInstance()->getParser()->addParentSizeMaxAssociation(
        this, idparent);
  }

  // left & right clicks
  CAHManager::getInstance()->parseAH(cur, "group_onclick_r", "group_params_r",
                                     _AHOnRightClick, _AHOnRightClickParams);
  CAHManager::getInstance()->parseAH(cur, "group_onclick_l", "group_params_l",
                                     _AHOnLeftClick, _AHOnLeftClickParams);

  // Each window (modal and groupContainer) can be validated by Enter. if "" =>
  // no op.
  CAHManager::getInstance()->parseAH(cur, "on_enter", "on_enter_params",
                                     _AHOnEnter, _AHOnEnterParams);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"win_priority");
  if (ptr)
    NLMISC::fromString((const char *)ptr, _Priority);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"use_cursor");
  if (ptr)
    _UseCursor = convertBool(ptr);

  // Each window (modal and groupContainer) can be escaped if "escapable" set
  // they can add an action handler before the hide
  CAHManager::getInstance()->parseAH(cur, "on_escape", "on_escape_params",
                                     _AHOnEscape, _AHOnEscapeParams);
  ...
  if (ptr)
    CWidgetManager::getInstance()->getParser()->addLuaClassAssociation(
        this, (const char *)ptr);
...
std::string CInterfaceGroup::getProperty(const std::string &name) const {
  ...
  } else if (name == "escapable") {
    return NLMISC::toString(_Escapable);
  } else if (name == "child_resize_w") {
    return NLMISC::toString(_ResizeFromChildW);
  } else if (name == "child_resize_h") {
    return NLMISC::toString(_ResizeFromChildH);
  } else if (name == "child_resize_wmargin") {
    return NLMISC::toString(_ResizeFromChildWMargin);
  } else if (name == "child_resize_hmargin") {
    return NLMISC::toString(_ResizeFromChildHMargin);
  } else if (name == "on_active") {
    return getAHString(name);
  } else if (name == "on_active_params") {
    return getOnActiveParams();
  } else if (name == "on_deactive") {
    return getAHString(name);
  } else if (name == "on_deactive_params") {
    return getOnDeactiveParams();
  } else if (name == "max_w") {
    return NLMISC::toString(_MaxW);
  } else if (name == "max_h") {
    return NLMISC::toString(_MaxH);
  } else if (name == "max_sizeref") {
    return getSizeRefAsString(_GroupSizeRef, _SizeDivW, _SizeDivH);
  } else if (name == "max_sizeparent") {
    return CWidgetManager::getInstance()
        ->getParser()
        ->getParentSizeMaxAssociation((CInterfaceElement *)this);
  } else if (name == "group_onclick_r") {
    return getAHString(name);
  } else if (name == "group_params_r") {
    return getRightClickHandlerParams();
  } else if (name == "group_onclick_l") {
    return getAHString(name);
  } else if (name == "group_params_l") {
    return getLeftClickHandlerParams();
  } else if (name == "on_enter") {
    return getAHString(name);
  } else if (name == "on_enter_params") {
    return getAHOnEnterParams();
  } else if (name == "win_priority") {
    return NLMISC::toString(_Priority);
  } else if (name == "use_cursor") {
    return NLMISC::toString(_UseCursor);
  } else if (name == "on_escape") {
    return getAHString(name);
  } else if (name == "on_escape_params") {
    return getAHOnEscapeParams();
  } else if (name == "lua_class") {
    return CWidgetManager::getInstance()->getParser()->getLuaClassAssociation(
        (CInterfaceGroup *)this);
  } else
    return CCtrlBase::getProperty(name);
}

void CInterfaceGroup::setProperty(const std::string &name,
  ...
  if (name == "overlappable") {
    ...
    CWidgetManager::getInstance()->getParser()->addLuaClassAssociation(this,
                                                                       value);
    return;
  } else
...
xmlNodePtr CInterfaceGroup::serialize(xmlNodePtr parentNode,
...
xmlNodePtr CInterfaceGroup::serializeGroup(xmlNodePtr parentNode,
                                           const char *type) const {
  xmlNodePtr node = CCtrlBase::serialize(parentNode, type);
  if (node == NULL)
    return NULL;

  xmlNewProp(node, BAD_CAST "overlappable",
             BAD_CAST NLMISC::toString(_Overlappable).c_str());
  xmlNewProp(node, BAD_CAST "escapable",
             BAD_CAST NLMISC::toString(_Escapable).c_str());
  xmlNewProp(node, BAD_CAST "child_resize_w",
             BAD_CAST NLMISC::toString(_ResizeFromChildW).c_str());
  xmlNewProp(node, BAD_CAST "child_resize_h",
             BAD_CAST NLMISC::toString(_ResizeFromChildH).c_str());
  xmlNewProp(node, BAD_CAST "child_resize_wmargin",
             BAD_CAST NLMISC::toString(_ResizeFromChildWMargin).c_str());
  xmlNewProp(node, BAD_CAST "child_resize_hmargin",
             BAD_CAST NLMISC::toString(_ResizeFromChildHMargin).c_str());

  xmlNewProp(node, BAD_CAST "on_active",
             BAD_CAST getAHString("on_active").c_str());
  xmlNewProp(node, BAD_CAST "on_active_params",
             BAD_CAST getOnActiveParams().c_str());
  xmlNewProp(node, BAD_CAST "on_deactive",
             BAD_CAST getAHString("on_deactive").c_str());
  xmlNewProp(node, BAD_CAST "on_deactive_params",
             BAD_CAST getOnDeactiveParams().c_str());
  ...
  xmlNewProp(
      node, BAD_CAST "max_sizeref",
      BAD_CAST getSizeRefAsString(_GroupSizeRef, _SizeDivW, _SizeDivH).c_str());
  xmlNewProp(node, BAD_CAST "max_sizeparent",
             BAD_CAST CWidgetManager::getInstance()
                 ->getParser()
                 ->getParentSizeMaxAssociation((CInterfaceElement *)this)
                 .c_str());
  xmlNewProp(node, BAD_CAST "group_onclick_r",
             BAD_CAST getAHString("group_onclick_r").c_str());
  xmlNewProp(node, BAD_CAST "group_params_r",
             BAD_CAST getRightClickHandlerParams().c_str());
  xmlNewProp(node, BAD_CAST "group_onclick_l",
             BAD_CAST getAHString("group_onclick_l").c_str());
  xmlNewProp(node, BAD_CAST "group_params_l",
             BAD_CAST getLeftClickHandlerParams().c_str());
  xmlNewProp(node, BAD_CAST "on_enter",
             BAD_CAST getAHString("on_enter").c_str());
  xmlNewProp(node, BAD_CAST "on_enter_params",
             BAD_CAST getAHOnEnterParams().c_str());
  xmlNewProp(node, BAD_CAST "win_priority",
             BAD_CAST NLMISC::toString(_Priority).c_str());
  xmlNewProp(node, BAD_CAST "use_cursor",
             BAD_CAST NLMISC::toString(_UseCursor).c_str());
  xmlNewProp(node, BAD_CAST "on_escape",
             BAD_CAST getAHString("on_escape").c_str());
  xmlNewProp(node, BAD_CAST "on_escape_params",
             BAD_CAST getAHOnEscapeParams().c_str());
  xmlNewProp(node, BAD_CAST "lua_class",
             BAD_CAST CWidgetManager::getInstance()
                 ->getParser()
                 ->getLuaClassAssociation((CInterfaceGroup *)this)
                 .c_str());
...
xmlNodePtr CInterfaceGroup::serializeTreeData(xmlNodePtr parentNode) const {
  ...
  xmlSetProp(node, BAD_CAST "node",
             BAD_CAST CInterfaceElement::stripId(getId()).c_str());
...
uint32 CInterfaceGroup::getMemory() {
  ...
  for (vector<CViewBase *>::const_iterator itv = _Views.begin();
       itv != _Views.end(); itv++) {
    CViewBase *pVB = *itv;
    Mem += pVB->getMemory();
  }

  for (vector<CCtrlBase *>::const_iterator itc = _Controls.begin();
       itc != _Controls.end(); itc++) {
    CCtrlBase *ctrl = *itc;
    Mem += ctrl->getMemory();
  }
  return Mem;
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::addElement(CInterfaceElement *child,
                                 sint eltOrder /*= -1*/) {
  if (!child) {
...
int CInterfaceGroup::luaAddGroup(CLuaState &ls) {
  ...
  CInterfaceGroup *group =
      dynamic_cast<CInterfaceGroup *>(CLuaIHM::getUIOnStack(ls, 1));
  if (group) {
...
// ------------------------------------------------------------------------------------------------
// id = incomplete path (text:list:a) lid complete one
// (ui:interface:content:text:list:a)
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::delGroup(CInterfaceGroup *child,
...
int CInterfaceGroup::luaGetGroup(CLuaState &ls) {
  ...
  if (index >= _ChildrenGroups.size()) {
    CLuaIHM::fails(
        ls,
        "getGroup : try to index group %s, but there are only %d son groups",
        ls.toString(1), (int)_ChildrenGroups.size());
  }
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::delView(const std::string &id,
                              bool dontDelete /* = false*/) {
  return delView(getView(id), dontDelete);
}

// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::delCtrl(const std::string &id,
                              bool dontDelete /* = false*/) {
  return delCtrl(getCtrl(id), dontDelete);
}

// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::delGroup(const std::string &id,
                               bool dontDelete /* = false*/) {
...
bool CInterfaceGroup::isChildGroup(const CInterfaceGroup *group) const {
  return std::find(_ChildrenGroups.begin(), _ChildrenGroups.end(), group) !=
         _ChildrenGroups.end();
}
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::renderWiredQuads(CInterfaceElement::TRenderWired type,
  ...
  sint32 oldSciX, oldSciY, oldSciW, oldSciH;
  makeNewClip(oldSciX, oldSciY, oldSciW, oldSciH);
  for (std::vector<CViewBase *>::iterator it = _EltOrder.begin();
       it != _EltOrder.end(); ++it) {
    if (*it)
      (*it)->renderWiredQuads(type, uiFilter);
  }
  restoreClip(oldSciX, oldSciY, oldSciW, oldSciH);
}
...
void CInterfaceGroup::checkCoords() {
  // Make XReal same as in updateCoords() as some elements (CViewText) depends
  // on it
  _XReal += _MarginLeft;
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::evalChildrenBBox(bool resizeFromChildW,
                                       bool resizeFromChildH, sint &width,
                                       sint &height) const {
  ...
  for (ite = _EltOrder.begin(); ite != _EltOrder.end(); ite++) {
    ...
    nlassert(pIE);
    if (pIE->getActive()) {
      const CInterfaceElement *el =
          pIE->getParentPos() ? pIE->getParentPos() : pIE->getParent();
      if (el == this) {
        // to compute the bbox, don't use direct sons that have a sizeref on the
        // resizing coordinate
      ...
      const CInterfaceGroup *sonGroup =
          dynamic_cast<const CInterfaceGroup *>(pIE);
...
bool CInterfaceGroup::isWindowUnder(sint32 x, sint32 y) {
  return ((x >= _XReal) && (x < (_XReal + _WReal)) && (y > _YReal) &&
          (y <= (_YReal + _HReal)));
}
...
CInterfaceGroup *CInterfaceGroup::getGroupUnder(sint32 x, sint32 y) {
  ...
  for (itg = _ChildrenGroups.begin(); itg != _ChildrenGroups.end(); itg++) {
    ...
    CInterfaceGroup *pChildUnder =
        pChild->getGroupUnder(x - _XReal, y - _YReal);
    if (pChildUnder != NULL) {
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::getViewsUnder(sint32 x, sint32 y, sint32 clipX,
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::getCtrlsUnder(sint32 x, sint32 y, sint32 clipX,
                                    sint32 clipY, sint32 clipW, sint32 clipH,
  ...
  for (itc = _EltOrder.begin(); itc != _EltOrder.end(); itc++) {
    if ((*itc)->isCtrl() &&
        !(*itc)->isGroup()) // must be a ctrl but not a group (parsed later)
    {
...
// ------------------------------------------------------------------------------------------------
bool CInterfaceGroup::getGroupsUnder(sint32 x, sint32 y, sint32 clipX,
                                     sint32 clipY, sint32 clipW, sint32 clipH,
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::computeClipContribution(sint32 &newSciXDest,
                                              sint32 &newSciYDest,
                                              sint32 &newSciWDest,
                                              sint32 &newSciHDest) const {
  ...
  if (_MaxWReal < _WReal) {
    if ((_PosRef == Hotspot_TR) || (_PosRef == Hotspot_MR) ||
        (_PosRef == Hotspot_BR))
  ...
  if (_MaxHReal < _HReal) {
    if ((_PosRef == Hotspot_TL) || (_PosRef == Hotspot_TM) ||
        (_PosRef == Hotspot_TR))
      newSciY = _YReal + _HReal - _MaxHReal;
    newSciH = _MaxHReal;
  }
  // Don't apply margins because HTML list marker is drawn outside group
  // paragraph inner content. Should not be an issue because horizontal scolling
  // not used.
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::computeCurrentClipContribution(
    sint32 oldSciX, sint32 oldSciY, sint32 oldSciW, sint32 oldSciH,
    sint32 &newSciXDest, sint32 &newSciYDest, sint32 &newSciWDest,
    sint32 &newSciHDest) const {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::makeNewClip(sint32 &oldSciX, sint32 &oldSciY,
                                  sint32 &oldSciW, sint32 &oldSciH) {
  CViewRenderer &rVR = *CViewRenderer::getInstance();
  rVR.getClipWindow(oldSciX, oldSciY, oldSciW, oldSciH);

  sint32 newSciX, newSciY, newSciW, newSciH;
  computeCurrentClipContribution(oldSciX, oldSciY, oldSciW, oldSciH, newSciX,
                                 newSciY, newSciW, newSciH);
  rVR.setClipWindow(newSciX, newSciY, newSciW, newSciH);
}

// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::getClip(sint32 &x, sint32 &y, sint32 &w,
                              sint32 &h) const {
  CViewRenderer &rVR = *CViewRenderer::getInstance();
  uint32 sw, sh;
  ...
  do {
    currGroup->computeCurrentClipContribution(sciX, sciY, sciW, sciH, sciX,
                                              sciY, sciW, sciH);
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::restoreClip(sint32 oldSciX, sint32 oldSciY,
                                  sint32 oldSciW, sint32 oldSciH) {
  CViewRenderer &rVR = *CViewRenderer::getInstance();
...
void CInterfaceGroup::setLeftClickHandler(const std::string &handler) {
  _AHOnLeftClick =
      CAHManager::getInstance()->getAH(handler, _AHOnLeftClickParams);
}

// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::setRightClickHandler(const std::string &handler) {
  _AHOnRightClick =
      CAHManager::getInstance()->getAH(handler, _AHOnRightClickParams);
}
...
void CInterfaceGroup::dumpGroups() const {
  ...
  for (uint k = 0; k < _ChildrenGroups.size(); ++k) {
    ...
    nlinfo("Group %d, name = %s, type=%s", k,
           _ChildrenGroups[k] ? _ChildrenGroups[k]->getId().c_str() : "???",
           typeName.c_str());
  }
}
...
void CInterfaceGroup::dumpEltsOrder() const {
  nlinfo("Num elements = %d, num groups = %d", (int)_EltOrder.size(),
         _ChildrenGroups.size());
  for (uint k = 0; k < _EltOrder.size(); ++k) {
    ...
    if (el) {
      ...
      typeName = ti.name();
      nlinfo("Element %d, name = %s, type=%s, x=%d, y=%d, parent_name=%s "
             "parentposname=%s xreal=%d, yreal=%d, wreal=%d, hreal=%d",
             k, el->getId().c_str(), typeName.c_str(), el->getX(), el->getY(),
             el->getParent() ? el->getParent()->getId().c_str() : "no parent",
             el->getParentPos() ? el->getParentPos()->getId().c_str()
                                : "parent",
             (int)el->getXReal(), (int)el->getYReal(), (int)el->getWReal(),
             (int)el->getHReal());
    } else {
...
sint32 CInterfaceGroup::getMaxUsedW() const {
  ...
  for (uint k = 0; k < _EltOrder.size(); ++k) {
    // Get the child width
    sint32 width =
        _EltOrder[k]->getMaxUsedW() + _EltOrder[k]->getXReal() - getXReal();
    if (width > maxWidth)
      maxWidth = width;
  }
  return maxWidth;
...
sint32 CInterfaceGroup::getMinUsedW() const {
  ...
  for (uint k = 0; k < _EltOrder.size(); ++k) {
    // Get the child width
    sint32 width =
        _EltOrder[k]->getMinUsedW() + _EltOrder[k]->getXReal() - getXReal();
    if (width > minWidth)
      minWidth = width;
  }
  return minWidth;
...
void CInterfaceGroup::clearAllEditBox() {
  for (uint k = 0; k < _ChildrenGroups.size(); ++k) {
    if (_ChildrenGroups[k])
      _ChildrenGroups[k]->clearAllEditBox();
  }
}
...
void CInterfaceGroup::restoreAllContainersBackupPosition() {
  for (uint k = 0; k < _ChildrenGroups.size(); ++k) {
    if (_ChildrenGroups[k])
      _ChildrenGroups[k]->restoreAllContainersBackupPosition();
  }
}
...
void CInterfaceGroup::setUseCursor(bool use) { _UseCursor = use; }
...
void CInterfaceGroup::pushLUAEnvTable() {
  ...
  if (!_LUAEnvTableCreated) {
    ...
    // Create a table and assign it in the REGISTRY."__ui_envtable"  table, with
    // a userdata ptr
...
void CInterfaceGroup::deleteLUAEnvTable(bool recurse) {
  if (_LUAEnvTableCreated) {
    ...
    // replace simply the table with Nil, letting LUA Garbage collector do the
    // realease stuff
    CLuaStackChecker lsc(lua);
    lua->push(IHM_LUA_ENVTABLE);
    lua->getTable(LUA_REGISTRYINDEX); // __ui_envtable
    lua->pushLightUserData(IG_UNIQUE_ID(
        this)); // NB nico : use some pointer *inside* that object as a unique
                // id (any field but the first), instead of using 'this'. 'this'
                // is already used by
...
// ------------------------------------------------------------------------------------------------
void CInterfaceGroup::addLuaScriptOnDBChange(const std::string &dbList,
  ...
  newLink->init(noTargets, noCdbTargets,
                NLMISC::toString("depends(%s)", dbList.c_str()), "lua", script,
                "", this);
}
...
void CInterfaceGroup::onWidgetDeleted(CInterfaceElement *e) {
  for (std::vector<CViewBase *>::iterator itr = _Views.begin();
       itr != _Views.end(); ++itr)
    (*itr)->onWidgetDeleted(e);

  for (std::vector<CCtrlBase *>::iterator itr = _Controls.begin();
       itr != _Controls.end(); ++itr)
    (*itr)->onWidgetDeleted(e);

  for (std::vector<CInterfaceGroup *>::iterator itr = _ChildrenGroups.begin();
       itr != _ChildrenGroups.end(); ++itr)
    (*itr)->onWidgetDeleted(e);
}

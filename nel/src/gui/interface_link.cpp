 ...
 */
static bool affect(const CInterfaceExprValue &value,
                   CInterfaceElement &destElem,
  ...
  case CReflectedProperty::SInt32:
    if (valueToAffect.toInteger()) {
      (destElem.*
       (property.SetMethod.SetSInt32))((sint32)valueToAffect.getInteger());
    } else {
  ...
  case CReflectedProperty::Float:
    if (valueToAffect.toDouble()) {
      (destElem.*
       (property.SetMethod.SetFloat))((float)valueToAffect.getDouble());
    } else {
  ...
  case CReflectedProperty::UCStringRef:
    if (valueToAffect.toString()) {
      (destElem.*(property.SetMethod.SetUCString))(
          ucstring::makeFromUtf8(valueToAffect.getString()));
    } else {
      return false;
    }
#endif
...
CInterfaceLink::~CInterfaceLink() {
  ...
  if (this == _NextUpdatedLink) {
    _NextUpdatedLink =
        _NextUpdatedLink->_NextTriggeredLink[1 - _CurrentTriggeredLinkList];
  }
...
bool CInterfaceLink::init(const std::vector<CTargetInfo> &targets,
                          const std::vector<CCDBTargetInfo> &cdbTargets,
                          const std::string &expr,
                          const std::string &actionHandler,
                          const std::string &ahParams,
                          const std::string &ahCond,
  ...
  if (!targets.empty()) {
    ...
    for (uint k = 0; k < targets.size(); ++k) {
      ...
      _Targets[k]._Property =
          elem->getReflectedProperty(targets[k].PropertyName);
      if (!_Targets[k]._Property) {
        nlwarning("<CInterfaceLink::init> : Can't retrieve property %s for "
                  "element %d.",
                  targets[k].PropertyName.c_str(), k);
        _Targets[k]._InterfaceElement = NULL;
        continue;
      }
    ...
    // There are no target for this link, so, put in a dedicated list to ensure
    // that the link will be destroyed at exit
    _LinksWithNoTarget.push_back(TLinkSmartPtr(this));
  }
...
void CInterfaceLink::createObservers(const TNodeVect &nodes) {
  for (std::vector<ICDBNode *>::const_iterator it = nodes.begin();
       it != nodes.end(); ++it) {
    if ((*it)->isLeaf()) {
...
void CInterfaceLink::updateAllLinks() {
  ...
  for (TLinkList::iterator it = _LinkList.begin(); it != _LinkList.end();
       ++it) {
...
void CInterfaceLink::update() {
  ...
  if (!_ParseTree)
    return;
  static TNodeVect
      observedNodes; // should not be too big, so ok to keep it static.
  ...
  tmpNodes.clear();
  std::set_difference(observedNodes.begin(), observedNodes.end(),
                      _ObservedNodes.begin(), _ObservedNodes.end(),
                      std::back_inserter(tmpNodes));
  // add these observers
  createObservers(tmpNodes);
  // compute lost observers
  tmpNodes.clear();
  std::set_difference(_ObservedNodes.begin(), _ObservedNodes.end(),
                      observedNodes.begin(), observedNodes.end(),
                      std::back_inserter(tmpNodes));
  ...
  if (_CDBTargets.size()) {
    ...
    if (resultCopy.toInteger()) {
      ...
      for (uint k = 0; k < _CDBTargets.size(); ++k) {
        ...
        if (!node) {
          node = _CDBTargets[k].Leaf =
              NLGUI::CDBManager::getInstance()->getDbProp(
                  _CDBTargets[k].LeafName, false);
        }
        if (node) {
          // assuming setvalue64 always works
          node->setValue64(resultValue);
        } else {
          nlwarning("CInterfaceLink::update: Node does not exist: '%s'",
                    _CDBTargets[k].LeafName.c_str());
        }
      }
    } else {
      nlwarning(
          "CInterfaceLink::update: Result conversion to db target failed");
    }
  }
  ...
  if (!_ActionHandler.empty()) {
    ...
    if (_AHCondParsed) // todo: maybe makes more sense to make condition also
                       // cover target
    {
      CInterfaceExprValue result;
    ...
    if (launch) {
      CAHManager::getInstance()->runActionHandler(_ActionHandler, _AHParent,
                                                  _AHParams);
...
struct CRemoveTargetPred {
  ...
  bool operator()(const CInterfaceLink::CTarget &target) const {
    return target._InterfaceElement == Target;
  }
};
...
void CInterfaceLink::removeTarget(CInterfaceElement *elem) {
  ...
  pred.Target = elem;
  _Targets.erase(std::remove_if(_Targets.begin(), _Targets.end(), pred),
                 _Targets.end());
}
...
bool CInterfaceLink::CTargetInfo::affect(const CInterfaceExprValue &value) {
  ...
  if (!property) {
    nlwarning("<CInterfaceLink::CTargetInfo::affect> : Can't retrieve property "
              "%s for element %s.",
              PropertyName.c_str(), Elem->getId().c_str());

    return false;
  }
  // try to affect the property
  if (!NLGUI::affect(value, *Elem, *property)) {
    nlwarning("<CInterfaceLink::CTargetInfo::affect> Couldn't convert the "
              "value to affect to %s",
              Elem->getId().c_str());
...
void CInterfaceLink::removeAllLinks() {
  ...
  while (curr != _LinkList.end()) {
    ...
    link.checkNbRefs();
    NLMISC::CSmartPtr<CInterfaceLink> linkPtr =
        *curr; // must keep a smart ptr because of the test in the loop
    for (k = 0; k < link._Targets.size(); ++k) {
      if (link._Targets[k]._InterfaceElement) {
        nlinfo("InterfaceElement %s was not remove.",
               link._Targets[k]._InterfaceElement->getId().c_str());
...
// ***************************************************************************
bool CInterfaceLink::splitLinkTarget(const std::string &target,
                                     CInterfaceGroup *parentGroup,
                                     std::string &propertyName,
                                     CInterfaceElement *&targetElm) {
  ...
  if (lastPos == (target.length() - 1)) {
    ...
    nlwarning("The target should at least contains a path and a property as "
              "follow 'path:property'");
    return false;
  }
  std::string elmPath;
  std::string elmProp;
...
// ***************************************************************************
bool CInterfaceLink::splitLinkTargets(
    const std::string &targets, CInterfaceGroup *parentGroup,
  ...
  for (uint k = 0; k < targetNames.size(); ++k) {
    ...
    if (!splitLinkTarget(
            targetNames[k].substr(startPos, lastPos - startPos + 1),
            parentGroup, ti.PropertyName, ti.Elem)) {
      // todo hulud interface syntax error
...
// ***************************************************************************
bool CInterfaceLink::splitLinkTargetsExt(
    const std::string &targets, CInterfaceGroup *parentGroup,
    std::vector<CInterfaceLink::CTargetInfo> &targetsVect,
  ...
  for (uint k = 0; k < targetNames.size(); ++k) {
    ...
    if (targetNames[k][startPos] == '@') {
      ...
      ti.LeafName =
          targetNames[k].substr((startPos + 1), (lastPos + 1) - (startPos + 1));
      ...
      if ((0 == ti.LeafName.compare(0, dbServer.size(), dbServer)) ||
        ...
        if (0 != ti.LeafName.compare(0, dbLocalR2.size(), dbLocalR2)) {
          // nlwarning("You are not allowed to write on 'SERVER:...' or
          // 'LOCAL:...' database");
          nlstop;
          return false;
        }
...
void CInterfaceLink::checkNbRefs() {
  uint numValidPtr = 0;
  for (uint k = 0; k < _Targets.size(); ++k) {
    if (_Targets[k]._InterfaceElement != NULL)
      ++numValidPtr;
  }
...
//===========================================================
void CInterfaceLink::setTargetProperty(const std::string &Target,
                                       const CInterfaceExprValue &val) {
  // Eval target !
...
void CInterfaceLink::unlinkFromTriggerList(uint list) {
  ...
  if (_PrevTriggeredLink[list]) {
    _PrevTriggeredLink[list]->_NextTriggeredLink[list] =
        _NextTriggeredLink[list];
  } else {
    // this was the first node
    _FirstTriggeredLink[list] = _NextTriggeredLink[list];
  }
  if (_NextTriggeredLink[list]) {
    _NextTriggeredLink[list]->_PrevTriggeredLink[list] =
        _PrevTriggeredLink[list];
  } else {
...
void CInterfaceLink::updateTrigeredLinks() {
  ...
  while (_CurrUpdatedLink) {
    ...
    while (_CurrUpdatedLink) {
      _NextUpdatedLink =
          _CurrUpdatedLink->_NextTriggeredLink[1 - _CurrentTriggeredLinkList];
      _CurrUpdatedLink->update();
      if (_CurrUpdatedLink) // this may be modified by the call (if current
                            // observer is removed)
      {

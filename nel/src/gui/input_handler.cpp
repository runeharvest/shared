...
namespace NLGUI {
CInputHandler::CInputHandler() { listener = NULL; }
...
bool CInputHandler::handleEvent(const NLMISC::CEvent &evnt) {
  ...
  else if (evnt == NLMISC::EventKeyDownId || evnt == NLMISC::EventKeyUpId ||
           evnt == NLMISC::EventCharId || evnt == NLMISC::EventStringId)
    return handleKeyboardEvent(evnt);
  else if (evnt == NLMISC::EventMouseMoveId ||
           evnt == NLMISC::EventMouseDownId || evnt == NLMISC::EventMouseUpId ||
...
bool CInputHandler::handleSetFocusEvent(const NLMISC::CEvent &evnt) {
  ...
  const NLMISC::CEventSetFocus *e =
      reinterpret_cast<const NLMISC::CEventSetFocus *>(&evnt);
...
bool CInputHandler::handleKeyboardEvent(const NLMISC::CEvent &evnt) {
  bool ok = false;
  if (evnt == NLMISC::EventKeyDownId || evnt == NLMISC::EventKeyUpId ||
      evnt == NLMISC::EventCharId || evnt == NLMISC::EventStringId)
    ok = true;

  nlassert(ok);

  return listener->handleEvent(NLGUI::CEventDescriptorKey(
      reinterpret_cast<const NLMISC::CEventKey &>(evnt)));
}
...
bool CInputHandler::handleMouseEvent(const NLMISC::CEvent &evnt) {
  ...
  else if (evnt == NLMISC::EventMouseDownId)
    return handleMouseButtonDownEvent(evnt);
  else if (evnt == NLMISC::EventMouseUpId)
    return handleMouseButtonUpEvent(evnt);
  else if (evnt == NLMISC::EventMouseDblClkId)
    return handleMouseDblClickEvent(evnt);
  else if (evnt == NLMISC::EventMouseWheelId)
...
bool CInputHandler::handleMouseMoveEvent(const NLMISC::CEvent &evnt) {
  const NLMISC::CEventMouseMove &mouseMoveEvent =
      static_cast<const NLMISC::CEventMouseMove &>(evnt);

  CEventDescriptorMouse eventDesc;
  float x = mouseMoveEvent.X;
  float y = mouseMoveEvent.Y;

  // These bloody hacks here are used so that we can send the x, and y float
  // coordinates from the NEL mouse move event, to the GUI event listener,
  // without having to change CEventDescriptorMouse or without having to couple
...
bool CInputHandler::handleMouseButtonDownEvent(const NLMISC::CEvent &evnt) {
  ...
  const NLMISC::CEventMouseDown *mouseDownEvent =
      static_cast<const NLMISC::CEventMouseDown *>(&evnt);
...
bool CInputHandler::handleMouseButtonUpEvent(const NLMISC::CEvent &evnt) {
  ...
  const NLMISC::CEventMouseUp *mouseUpEvent =
      static_cast<const NLMISC::CEventMouseUp *>(&evnt);
...
bool CInputHandler::handleMouseDblClickEvent(const NLMISC::CEvent &evnt) {
  ...
  const NLMISC::CEventMouseDblClk *dblClickEvent =
      static_cast<const NLMISC::CEventMouseDblClk *>(&evnt);
  ...
  if (dblClickEvent->Button & NLMISC::rightButton) {
    eventDesc.setEventTypeExtended(
        NLGUI::CEventDescriptorMouse::mouserightdblclk);
    return listener->handleEvent(eventDesc);
  }
...
bool CInputHandler::handleMouseWheelEvent(const NLMISC::CEvent &evnt) {
  ...
  const NLMISC::CEventMouseWheel *wheelEvent =
      static_cast<const NLMISC::CEventMouseWheel *>(&evnt);

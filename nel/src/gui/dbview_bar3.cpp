...
CDBViewBar3::CDBViewBar3(const TCtorParam &param)
  ...
  _Mini = false;
  _ColorsNegative[0] = _ColorsNegative[1] = _ColorsNegative[2] =
      NLMISC::CRGBA(0, 0, 0, 0);
  _ValueInt[0] = _ValueInt[1] = _ValueInt[2] = 0;
  _RangeInt[0] = _RangeInt[1] = _RangeInt[2] = 255;
}

// ----------------------------------------------------------------------------
void CDBViewBar3::parseValProp(xmlNodePtr cur, CInterfaceProperty &dbProp,
                               sint32 &intProp, const char *name) {
...
void CDBViewBar3::setValProp(const std::string &value,
                             CInterfaceProperty &dbProp, sint32 &intProp) {
  sint32 i;
...
std::string CDBViewBar3::getProperty(const std::string &name) const {
  ...
  } else if (name == "color1_negative") {
    return toString(_ColorsNegative[0]);
  } else if (name == "color2_negative") {
    return toString(_ColorsNegative[1]);
  } else if (name == "color3_negative") {
...
void CDBViewBar3::setProperty(const std::string &name,
...
xmlNodePtr CDBViewBar3::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "value1",
             BAD_CAST getValProp(_Value[0], _ValueInt[0]).c_str());
  xmlSetProp(node, BAD_CAST "value2",
             BAD_CAST getValProp(_Value[1], _ValueInt[1]).c_str());
  xmlSetProp(node, BAD_CAST "value3",
             BAD_CAST getValProp(_Value[2], _ValueInt[2]).c_str());

  xmlSetProp(node, BAD_CAST "range1",
             BAD_CAST getValProp(_Range[0], _RangeInt[0]).c_str());
  xmlSetProp(node, BAD_CAST "range2",
             BAD_CAST getValProp(_Range[1], _RangeInt[1]).c_str());
  xmlSetProp(node, BAD_CAST "range3",
             BAD_CAST getValProp(_Range[2], _RangeInt[2]).c_str());
  ...
  xmlSetProp(node, BAD_CAST "color1_negative",
             BAD_CAST toString(_ColorsNegative[0]).c_str());
  xmlSetProp(node, BAD_CAST "color2_negative",
             BAD_CAST toString(_ColorsNegative[1]).c_str());
  xmlSetProp(node, BAD_CAST "color3_negative",
             BAD_CAST toString(_ColorsNegative[2]).c_str());
...
// ----------------------------------------------------------------------------
sint32 CDBViewBar3::getCurrentValProp(const CInterfaceProperty &dbProp,
                                      sint32 intProp) {
...
std::string CDBViewBar3::getValProp(const CInterfaceProperty &prop,
                                    sint32 intProp) const {
...
void CDBViewBar3::draw() {
  ...
  if (_Mini) {
    for (uint32 i = 0; i < 3; ++i) {
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + 1, _YReal + i * 3 + 2,
                            _WReal, _BarH, 0, false, _TextureId, color);
    }
  } else {
    for (uint32 i = 0; i < 3; ++i) {
      float factor;
      ...
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal + 2, _YReal + i * 7 + 4,
                            _WReal, _BarH, 0, false, _TextureId, color);
    }
  }
}

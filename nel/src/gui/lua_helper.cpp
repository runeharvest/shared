...
const int MinGCThreshold = 128; // min value at which garbage collector will be
                                // triggered (in kilobytes)
// ***************************************************************************
CLuaState::CLuaState(bool debugger) {
  ...
  if (debugger) {
    ...
    if (!warningShown) {
      nldebug("Lua debugger was asked, but the static lua library against "
              "which the client was linked is too old. Please update to "
              "lua-5.0.2_nevrax. Debugging won't be available!");
      // MessageBox (NULL, "Lua debugger was asked, but the static lua library
      // against which the client was linked is too old. Please update to
      // lua-5.0.2_nevrax. Debugging won't be available!", "Lua support",
      // MB_OK);
      warningShown = true;
    }
#else
    ...
    if (LuaDebuggerModule) {
      TGetLuaIDEInterfaceVersion getVersion =
          (TGetLuaIDEInterfaceVersion)GetProcAddress(
              LuaDebuggerModule, "GetLuaIDEInterfaceVersion");
      nlassert(getVersion);
      int dllInterfaceVersion = getVersion();
      if (dllInterfaceVersion > LUA_IDE_INTERFACE_VERSION) {
        MessageBox(NULL,
                   "Lua debugger interface is newer than the application. "
                   "Debugging will be disabled. Please update your client",
                   "Lua support", MB_OK);
      } else if (dllInterfaceVersion < LUA_IDE_INTERFACE_VERSION) {
        MessageBox(NULL,
                   "Lua debugger interface is too old. Lua debugging will be "
                   "disabled. Please ask for a more recent dll.",
                   "Lua support", MB_OK);
      } else {
        TGetLuaIDEInterface getter = (TGetLuaIDEInterface)GetProcAddress(
            LuaDebuggerModule, "GetLuaIDEInterface");
        nlassert(getter);
        LuaDebuggerIDE = getter();
        LuaDebuggerIDE->prepareDebug("save\\___external_debug.lpr",
                                     l_realloc_func, l_free_func,
                                     Driver->getDisplay());
    ...
    if (NLMISC::CFile::fileExists("socket\\core.dll")) {
      ...
      if (!m_LuaSocket) {
        ...
        void *luaopen_socket_core =
            (void *)GetProcAddress(m_LuaSocket, "luaopen_socket_core");
        if (!luaopen_socket_core) {
          nlwarning(
              "Lua socket library loaded, but `luaopen_socket_core` not found");
  ...
  pushLightUserData(
      (void *)_State); // NB : as creator of the state,   we make the assumption
                       // that no one will be using this pointer in the registry
                       // (cf. ref manual about registry)
  pushLightUserData((void *)this);
  setTable(LUA_REGISTRYINDEX);
...
// ***************************************************************************
CLuaStackRestorer::CLuaStackRestorer(CLuaState *state, int finalSize)
    : _FinalSize(finalSize), _State(state) {}
...
#ifdef NL_OS_WINDOWS
static int NoOpReportHook(int /* reportType */, char * /* message */,
                          int * /* returnValue */) {
  return TRUE;
}
#endif
...
#ifdef NL_OS_WINDOWS
    nlassert(LuaDebuggerModule) _CrtSetReportHook(
        NoOpReportHook); // prevent dump of memory leaks at this point
    //::FreeLibrary(LuaDebuggerModule); // don't free the library now (seems
    //:that it destroy, the main window, causing
...
void CLuaState::loadScript(const std::string &code, const std::string &dbgSrc) {
  ...
  int result = lua_load(_State, CHelper::luaChunkReaderFromString, (void *)&rd,
                        dbgSrc.c_str()
...
// ***************************************************************************
void CLuaState::executeScriptInternal(const std::string &code,
                                      const std::string &dbgSrc, int numRet) {
...
bool CLuaState::executeFile(const std::string &pathName) {
  ...
  */
  ...
  // execute the script text,   with dbgSrc==filename (use @ for lua internal
  // purpose)
#ifdef _WIN32
  // Paths need to be correct for debugging to work
  std::string pathNameStandardized = pathName;
  if (pathNameStandardized.size() > 1) {
    if (pathNameStandardized[1] == ':' && pathNameStandardized[0] >= 'a' &&
        pathNameStandardized[0] <= 'z')
...
void CLuaState::executeSmallScript(const std::string &script) {
  ...
  if (it == _SmallScriptCache.end()) {
    ...
    // Assign the method to the NEL table:
    // NELSmallScriptTable[_SmallScriptPool]= function
  ...
  struct CForwarder {
    static int callFunc(lua_State *ls) {
      ...
      TLuaWrappedFunction func =
          (TLuaWrappedFunction)lua_touserdata(ls, lua_upvalueindex(1));
      ...
      try {
        ...
        // TODO : see if this is safe to call lua error there" ... (it does a
        // long jump)
        lua_error(ls);
      }
      return numResults;
...
// ***************************************************************************
const char *CLuaState::getTableStringValue(const char *name,
...
// ***************************************************************************
int CLuaState::pcallByNameGlobal(const char *functionName, int nargs,
                                 int nresults, int errfunc /*= 0*/) {
...
#endif
  return pcallByNameInternal(functionName, nargs, nresults, errfunc,
                             initialStackSize);
}

int CLuaState::pcallByName(const char *functionName, int nargs, int nresults,
...
int CLuaState::pcallByNameInternal(const char *functionName, int nargs,
                                   int nresults, int errfunc /*= 0*/,
...
void CLuaState::dumpStack() {
  ...
  for (int k = 1; k <= getTop(); ++k) {
    ...
    nlinfo("Stack entry %d : type = %s, value = %s", k, getTypename(type(-1)),
           value.c_str());
    pop();
  }
}
...
void CLuaState::getStackAsString(std::string &dest) {
  ...
  for (int k = 1; k <= getTop(); ++k) {
    ...
    dest += NLMISC::toString("Stack entry %d : type = %s, value = %s\n", k,
                             getTypename(type(-1)), value.c_str());
    pop();
  }
}
...
CLuaStackChecker::~CLuaStackChecker() {
  ...
  if (!_ExceptionContextCounter) {
    ...
    if (currSize != _FinalWantedSize) {
      ...
      if (assertWanted) {
        nlwarning(
            "Lua stack size error : expected size is %d, current size is %d",
            _FinalWantedSize, currSize);
        _State->dumpStack();
        nlassert(0);
      }
    }
  } else {
    // this object dtor was called because an exception was thrown, so let the
    // exception propagate (the stack must be broken, but because of the
    // exception, not because of code error)
    _State->setTop(_FinalWantedSize);
  }
}

// ***************************************************************************
void ELuaWrappedFunctionException::init(CLuaState *ls,
                                        const std::string &reason) {
  // H_AUTO(Lua_ELuaWrappedFunctionException_init)
  //  Print first Lua Stack Context
  if (ls) {
    ls->getStackContext(_Reason, 1); // 1 because 0 is the current C function =>
                                     // return 1 for script called
...
// ***************************************************************************
ELuaWrappedFunctionException::ELuaWrappedFunctionException(
    CLuaState *luaState) {
...
// ***************************************************************************
ELuaWrappedFunctionException::ELuaWrappedFunctionException(
    CLuaState *luaState, const std::string &reason) {
  // H_AUTO(Lua_ELuaWrappedFunctionException_ELuaWrappedFunctionException)
...
// H_AUTO(Lua_CLuaState_handleGC)
  ...
  if (gcCount >= _GCThreshold) {
    nlwarning(
        "Triggering GC : memory in use = %d kb, current threshold = %d kb",
        gcCount, _GCThreshold);
    lua_setgcthreshold(_State, 0);
    gcCount = getGCCount();
    _GCThreshold = std::max(MinGCThreshold, gcCount * 2);
    nlwarning("After GC : memory in use = %d kb, threshold = %d kb", gcCount,
              _GCThreshold);
  }
#endif
}

...
std::vector<CInterfaceElement::IDeletionWatcher *>
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::parseError(CInterfaceGroup *parentGroup,
                                   const char *reason) {
...
std::string CInterfaceElement::getProperty(const std::string &name) const {
  ...
  if (name == "posparent") {
    ...
    getPosParent(pp);
    return pp;
  } else if (name == "sizeparent") {
    std::string sp;
    getSizeParent(sp);
    return sp;
  } else if (name == "global_color") {
    return toString(_ModulateGlobalColor);
  } else if (name == "render_layer") {
    return toString(_RenderLayer);
  } else if (name == "avoid_resize_parent") {
    return toString(_AvoidResizeParent);
  } else {
    nlwarning("Invalid property '%s' queried for widget '%s'", name.c_str(),
              _Id.c_str());
    return "";
  }
}

void CInterfaceElement::setProperty(const std::string &name,
  ...
  if (name == "posparent") {
    ...
    nlwarning("Tried to set invalid property '%s' for widget '%s'",
              name.c_str(), _Id.c_str());
}

xmlNodePtr CInterfaceElement::serialize(xmlNodePtr parentNode,
  ...
  xmlNewProp(node, BAD_CAST "posref",
             BAD_CAST HotSpotCoupleToString(_ParentPosRef, _PosRef).c_str());
  ...
  xmlNewProp(node, BAD_CAST "global_color",
             BAD_CAST toString(_ModulateGlobalColor).c_str());
  xmlNewProp(node, BAD_CAST "render_layer",
             BAD_CAST toString(_RenderLayer).c_str());
  xmlNewProp(node, BAD_CAST "avoid_resize_parent",
             BAD_CAST toString(_AvoidResizeParent).c_str());
...
bool CInterfaceElement::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  _X = 0;
  if (ptr)
    fromString((const char *)ptr, _X);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"y");
  _Y = 0;
  if (ptr)
    fromString((const char *)ptr, _Y);
  ...
  if (ptr)
    fromString((const char *)ptr, _RenderLayer);

  ptr = (char *)xmlGetProp(cur, (xmlChar *)"avoid_resize_parent");
  if (ptr)
    _AvoidResizeParent = convertBool(ptr);
...
std::string
CInterfaceElement::getSizeRefAsString(const sint32 &sizeRef,
                                      const sint32 &sizeDivW,
                                      sint32 const &sizeDivH) const {
  std::string s;
  if ((sizeRef & 1) != 0) {
    s += "w";
    if (sizeDivW < 10)
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::parseSizeRef(const char *sizeRefStr, sint32 &sizeRef,
                                     sint32 &sizeDivW, sint32 &sizeDivH) {
  nlassert(sizeRefStr);
...
// ------------------------------------------------------------------------------------------------
sint32 CInterfaceElement::getInnerWidth() const { return _WReal - _MarginLeft; }
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::relativeSInt64Read(CInterfaceProperty &rIP,
                                           const string &prop, const char *val,
                                           const string &defVal) {
  if (val == NULL) {
    ...
    if (val[0] == ':')
      decal = 1;
    if (NLGUI::CDBManager::getInstance()->getDbProp(val + decal, false) !=
        NULL) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::relativeSInt32Read(CInterfaceProperty &rIP,
                                           const string &prop, const char *val,
                                           const string &defVal) {
  if (val == NULL) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::relativeBoolRead(CInterfaceProperty &rIP,
                                         const string &prop, const char *val,
                                         const string &defVal) {
  if (val == NULL) {
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::relativeRGBARead(CInterfaceProperty &rIP,
...
std::string CInterfaceElement::HotSpotCoupleToString(THotSpot parentPosRef,
                                                     THotSpot posRef) {
  std::string hs;
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::convertHotSpotCouple(const char *ptr,
...
// ------------------------------------------------------------------------------------------------
void CInterfaceElement::convertPixelsOrRatio(const char *ptr, sint32 &pixels,
                                             float &ratio) {
...
void CInterfaceElement::addLink(CInterfaceLink *link) {
  ...
  TLinkVect::const_iterator it =
      std::find(_Links->begin(), _Links->end(), linkPtr);
  if (it != _Links->end()) {
    // Link already appened : this can be the case when a link has several
    // targets property that belong to the same element, in this case, one
    // single ptr in the vector is enough. nlwarning("Link added twice");
  } else {
...
void CInterfaceElement::removeLink(CInterfaceLink *link) {
  ...
  TLinkVect::iterator it =
      std::find(_Links->begin(), _Links->end(), TLinkSmartPtr(link));
  if (it == _Links->end()) {
...
bool CInterfaceElement::isIn(sint x, sint y, uint width, uint height) const {
  return (x + (sint)width) >= _XReal && (y + (sint)height) > _YReal &&
         x < (_XReal + _WReal) && y <= (_YReal + _HReal);
}
...
void CInterfaceElement::invalidateCoords(uint8 numPass) {
  // Get the "Root Group" ie the 1st son of the master group of us (eg
  // "ui:interface:rootgroup" )
  CInterfaceGroup *parent = getParent();
  // if our parent is NULL, then we are the master group (error!)
  if (parent == NULL)
    return;
  // if our grandfather is NULL, then our father is the Master Group => we are
  // the "Root group"
...
// ***************************************************************************
void CInterfaceElement::resetInvalidCoords() { _InvalidCoords = 0; }
...
// ***************************************************************************
void CInterfaceElement::renderWiredQuads(TRenderWired type,
  ...
  if (((type == RenderView) && (ctrlBase == NULL) && (groupBase == NULL)) ||
    ...
    if (!uiFilter.empty() && (uiFilter.size() > _Id.size() ||
                              _Id.compare(_Id.size() - uiFilter.size(),
                                          string::npos, uiFilter) != 0))
      return;
    CViewRenderer &vr = *CViewRenderer::getInstance();
    vr.drawWiredQuad(_XReal, _YReal, _WReal, _HReal);
    drawHotSpot(_PosRef, CRGBA::Red);
    if (_Parent)
      _Parent->drawHotSpot(_ParentPosRef, CRGBA::Blue);
  }
}
...
int CInterfaceElement::luaSetPosRef(CLuaState &ls) {
  ...
  CLuaIHM::check(ls, ls.isString(1),
                 "setPosRef() requires a string in param 1");
...
CInterfaceElement *CInterfaceElement::clone() {
  ...
  if (_ParentPos == oldParent)
    _ParentPos = NULL;
  if (_ParentSize == oldParent)
    _ParentSize = NULL;
  ...
  try {
    ...
    std::copy(dupStream.buffer(), dupStream.buffer() + dupStream.length(),
              datas.begin());
...
void CInterfaceElement::getPosParent(std::string &id) const {
  ...
  // If pos parent and parent are the same then ofc the parent group is the pos
  // parent...
  CInterfaceElement *p = getParent();
  if (getParentPos() == p) {
    id = "parent";
...
void CInterfaceElement::parseSizeParent(const std::string &id) {
  ...
  CWidgetManager::getInstance()->getParser()->addParentSizeAssociation(this,
                                                                       spId);
}
...
void CInterfaceElement::getSizeParent(std::string &id) const {
  ...
  // If the size parent is the same as the group parent, then the size parent is
  // the parent ofc
  if (p == getParent()) {
    id = "parent";
    return;
...
void CInterfaceElement::getHSCoords(const THotSpot &hs, sint32 &x,
                                    sint32 &y) const {
  x = _XReal;
  y = _YReal;
  ...
  if ((hs & Hotspot_Mx) != 0)
    y += _HReal / 2;
  else if ((hs & Hotspot_Tx) != 0)
    y += _HReal;

  if ((hs & Hotspot_xM) != 0)
    x += _WReal / 2;
  else if ((hs & Hotspot_xR) != 0)
    x += _WReal;
}

void CInterfaceElement::getClosestHotSpot(const CInterfaceElement *other,
                                          THotSpot &hs) {
  /// Iterate over the following hotspots, calculate the distance and store the
  /// closest

  static THotSpot hslist[] = {Hotspot_BL, Hotspot_BR, Hotspot_MM, Hotspot_TL,
                              Hotspot_TR};
...
void CInterfaceElement::alignTo(CInterfaceElement *other) {
  ...
  // Calculate the difference between the hotspot we found and our current
  // position,
  sint32 dx = ox - x;
  sint32 dy = oy - y;
...
void CStringShared::deleteStringMapper() { delete _UIStringMapper; }
} // namespace NLGUI

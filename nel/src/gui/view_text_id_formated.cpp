...
NLMISC_REGISTER_OBJECT(CViewBase, CViewTextIDFormated, std::string,
                       "text_id_formated");
...
void CViewTextIDFormated::setProperty(const std::string &name,
...
xmlNodePtr CViewTextIDFormated::serialize(xmlNodePtr parentNode,
...
bool CViewTextIDFormated::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  if (!CViewTextID::parse(cur, parentGroup))
    return false;

...
CGroupScrollText::CGroupScrollText(const TCtorParam &param)
    : CInterfaceGroup(param), _List(NULL), _ScrollBar(NULL), _ButtonAdd(NULL),
      _ButtonSub(NULL), _Settuped(false), _InvertScrollBar(true),
      _ListHeight(0), _Scrolling(false), _ScrollDistance(0),
      _ClockMsgEventRegistered(false), _StartHeight(0), _EllapsedTime(0) {
...
void CGroupScrollText::setProperty(const std::string &name,
...
xmlNodePtr CGroupScrollText::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "invert_scroll_bar",
             BAD_CAST NLMISC::toString(_InvertScrollBar).c_str());
...
bool CGroupScrollText::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _InvertScrollBar = convertBool(ptr);
...
//========================================================================
void CGroupScrollText::draw() { CInterfaceGroup::draw(); }

//========================================================================
void CGroupScrollText::clearViews() { CInterfaceGroup::clearViews(); }
...
void CGroupScrollText::setup() {
  ...
  _ScrollBar =
      dynamic_cast<CCtrlScroll *>(CInterfaceGroup::getCtrl("scroll_bar"));
  _ButtonAdd =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("button_add"));
  _ButtonSub =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("button_sub"));
  _List = dynamic_cast<CGroupList *>(CInterfaceGroup::getGroup("text_list"));

  if (_ScrollBar == NULL)
    nlwarning("<setup> scroll bar 'scroll_bar' missing or bad type.(%s)",
              this->_Id.c_str());
  // Add and sub button are not required
  /*
  if(buttonAdd == NULL)
      nlwarning("Interface: CGroupScrollText: button 'button_add' missing or bad
  type"); if(buttonSub == NULL) nlwarning("Interface: CGroupScrollText: button
  'button_sub' missing or bad type");
  */
  if (_List == NULL)
  ...
  // actions
  if (_ButtonAdd)
    _ButtonAdd->setActionOnClockTick("gst_add");
  if (_ButtonSub)
    _ButtonSub->setActionOnClockTick("gst_sub");
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    ...
    CGroupScrollText *pST =
        dynamic_cast<CGroupScrollText *>(pCaller->getParent());
    if (pST == NULL)
      return;
    if (pST->getList() == NULL)
      return;
    ...
    //		pST->setH(std::min((sint32) pST->getMaxHeight(), (sint32)
    //(pST->_StartHeight + pST->_EllapsedTime / 9)));
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    ...
    if (!vt)
      return;
    pST->_EllapsedTime += times.frameDiffMs;
    //		pST->setH(std::max((sint32) pST->getMinHeight(), (sint32)
    //(pST->_StartHeight - pST->_EllapsedTime / 9)));
    pST->setH((sint32)(pST->_StartHeight - pST->_EllapsedTime / 9));
    pST->invalidateCoords();
  }

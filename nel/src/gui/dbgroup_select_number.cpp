...
namespace NLGUI {
NLMISC_REGISTER_OBJECT(CViewBase, CDBGroupSelectNumber, std::string,
                       "select_number");
...
void CDBGroupSelectNumber::setProperty(const std::string &name,
...
xmlNodePtr CDBGroupSelectNumber::serialize(xmlNodePtr parentNode,
  ...
  if (_Number.getNodePtr() != NULL)
    xmlSetProp(node, BAD_CAST "value",
               BAD_CAST _Number.getNodePtr()->getFullName().c_str());
  else
  ...
  xmlSetProp(node, BAD_CAST "delta",
             BAD_CAST toString(_DeltaMultiplier).c_str());
...
bool CDBGroupSelectNumber::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _LoopMode = convertBool(ptr);
  _MinValue = 0;
  ptr = xmlGetProp(cur, (xmlChar *)"min");
  if (ptr)
    fromString((const char *)ptr, _MinValue);
  _MaxValue = 0;
  ptr = xmlGetProp(cur, (xmlChar *)"max");
  if (ptr)
    fromString((const char *)ptr, _MaxValue);
  ptr = xmlGetProp(cur, (xmlChar *)"delta");
  if (ptr)
    fromString((const char *)ptr, _DeltaMultiplier);
...
void CDBGroupSelectNumber::setup() {
  ...
  _SlotNumber =
      dynamic_cast<CViewBitmap *>(CInterfaceGroup::getView("slot_number"));
  _TextNumber = dynamic_cast<CViewText *>(CInterfaceGroup::getView("number"));
  _ButtonUp =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("arrow_up"));
  _ButtonDown =
      dynamic_cast<CCtrlBaseButton *>(CInterfaceGroup::getCtrl("arrow_down"));

  // checks
  if (_SlotNumber == NULL)
    nlwarning("Interface: SelectNumberGroup: bitmap 'slot_number' missing or "
              "bad type");
  if (_TextNumber == NULL)
    nlwarning(
        "Interface: SelectNumberGroup: text view 'number' missing or bad type");
  if (_ButtonUp == NULL)
    nlwarning(
        "Interface: SelectNumberGroup: button 'arrow_up' missing or bad type");
  if (_ButtonDown == NULL)
    nlwarning("Interface: SelectNumberGroup: button 'arrow_down' missing or "
              "bad type");
  if (_SlotNumber == NULL || _TextNumber == NULL || _ButtonUp == NULL ||
      _ButtonDown == NULL)
    return;
...
// ***************************************************************************
void CDBGroupSelectNumber::draw() { CInterfaceGroup::draw(); }

// ***************************************************************************
void CDBGroupSelectNumber::clearViews() { CInterfaceGroup::clearViews(); }
...
public:
  virtual void execute(CCtrlBase *pCaller, const std::string & /* Params */) {
    CDBGroupSelectNumber *pSN =
        dynamic_cast<CDBGroupSelectNumber *>(pCaller->getParent());
    if (pSN == NULL)
      return;
    pSN->changeValue(+1);
  }

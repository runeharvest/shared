...
xmlNodePtr CInterfaceOptions::serialize(xmlNodePtr parentNode,
...
// ***************************************************************************
const CInterfaceOptionValue &
CInterfaceOptions::getValue(const string &sParamName) const {
  std::map<std::string, CInterfaceOptionValue>::const_iterator it =
      _ParamValue.find(toLowerAscii(sParamName));
...
// ***************************************************************************
const std::string &
CInterfaceOptions::getValStr(const std::string &sParamName) const {
...
// ***************************************************************************
NLMISC::CRGBA
CInterfaceOptions::getValColor(const std::string &sParamName) const {
...
COptionsLayer::COptionsLayer(const TCtorParam &param)
  ...
  TxId_TL = TxId_T = TxId_TR = TxId_L = TxId_R = TxId_Blank = TxId_BL = TxId_B =
      -2;
  TxId_BR = TxId_BL_Open = TxId_B_Open = TxId_BR_Open = TxId_EL_Open =
      TxId_EM_Open = TxId_ER_Open = -2;
  Tile_Blank = 0;
...
xmlNodePtr COptionsLayer::serialize(xmlNodePtr parentNode,
                                    const std::string &name) const {
...
// ----------------------------------------------------------------------------
NLMISC_REGISTER_OBJECT(CInterfaceOptions, COptionsContainerInsertion,
                       std::string, "container_insertion_opt");
...
xmlNodePtr
COptionsContainerInsertion::serialize(xmlNodePtr parentNode,
                                      const std::string &name) const {
  xmlNodePtr node = CInterfaceOptions::serialize(parentNode, name);
  if (node == NULL)
    return NULL;
...
// ***************************************************************************
NLMISC_REGISTER_OBJECT(CInterfaceOptions, COptionsContainerMove, std::string,
                       "container_move_opt");
...
xmlNodePtr COptionsContainerMove::serialize(xmlNodePtr parentNode,
                                            const std::string &name) const {
  xmlNodePtr node = CInterfaceOptions::serialize(parentNode, name);
  if (node == NULL)
    return NULL;
...
COptionsList::COptionsList(const TCtorParam &param) : CInterfaceOptions(param) {
  _NumParams = 0;
}

xmlNodePtr COptionsList::serialize(xmlNodePtr parentNode,
                                   const std::string &name) const {
  if (parentNode == NULL)
    return NULL;

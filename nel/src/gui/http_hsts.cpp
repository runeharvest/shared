...
CStrictTransportSecurity::~CStrictTransportSecurity() { save(); }
...
void CStrictTransportSecurity::set(const std::string &domain, uint64 expires,
...
bool CStrictTransportSecurity::get(const std::string &domain,
                                   SHSTSObject &hsts) const {
...
void CStrictTransportSecurity::serial(NLMISC::IStream &f) {
  try {
    ...
    if (f.isReading()) {
      ...
      f.serial(nbItems);
      for (THSTSObjectMap::iterator it = _Domains.begin(); it != _Domains.end();
           ++it) {
...
// ***************************************************************************
void CStrictTransportSecurity::setFromHeader(const std::string &domain,

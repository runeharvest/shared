...
void CViewText::setupDefault() {
  ...
  _FontSize = 12 + CWidgetManager::getInstance()
                       ->getSystemOption(CWidgetManager::OptionAddCoefFont)
                       .getValSInt32();
  _FontSizeCoef = true;
  _FontName.clear();
  _Embolden = false;
...
CViewText::CViewText(const TCtorParam &param) : CViewBase(param) {
  setupDefault();
...
/// constructor
// ***************************************************************************
CViewText::CViewText(const std::string &id, const std::string Text,
                     sint FontSize, NLMISC::CRGBA Color, bool Shadow,
                     bool ShadowOutline)
  ...
  _FontSize =
      FontSize + CWidgetManager::getInstance()
                     ->getSystemOption(CWidgetManager::OptionAddCoefFont)
                     .getValSInt32();
  _FontSizeCoef = true;
  _Color = Color;
...
std::string CViewText::getProperty(const std::string &name) const {
  ...
  if (!prop.empty())
    return prop;
  else if (name == "hardtext") {
    return getHardText();
  } else if (name == "hardtext_format") {
...
std::string CViewText::getTextProperty(const std::string &name) const {
  ...
  } else if (name == "shadow_outline") {
    return toString(_ShadowOutline);
  } else if (name == "shadow_color") {
...
bool CViewText::setTextProperty(const std::string &name,
                                const std::string &value) {
  if (name == "localize") {
    bool b;
    if (fromString(value, b)) {
      _Localized = b;
      setTextLocalized(_HardText.empty() ? _Text
                                         : _HardText); // FIXME: setCase?
      _TextLength = 0;
    }
    return true;
  } else if (name == "color") {
    ...
    if (fromString(value, i))
      _FontSize = i + CWidgetManager::getInstance()
                          ->getSystemOption(CWidgetManager::OptionAddCoefFont)
                          .getValSInt32();
    return true;
  } else if (name == "fontsize_coef") {
    ...
    if (_FontSizeCoef != oldValue) {
      if (_FontSizeCoef)
        _FontSize += CWidgetManager::getInstance()
                         ->getSystemOption(CWidgetManager::OptionAddCoefFont)
                         .getValSInt32();
      else
        _FontSize -= CWidgetManager::getInstance()
  ...
  if (name == "fontstyle") {
    ...
    if (fromString(value, c))
      _ShadowColor = c;
    return true;
  } else if (name == "shadow_x") {
    sint sx;
    if (fromString(value, sx))
      _ShadowX = sx;
    return true;
  } else if (name == "shadow_y") {
    sint sy;
...
bool CViewText::serializeTextOptions(xmlNodePtr node) const {
  ...
  xmlSetProp(node, BAD_CAST "global_color",
             BAD_CAST toString(_ModulateGlobalColor).c_str());
  ...
  xmlSetProp(node, BAD_CAST "fontsize_coef",
             BAD_CAST toString(_FontSizeCoef).c_str());
  ...
  xmlSetProp(node, BAD_CAST "shadow_outline",
             BAD_CAST toString(_ShadowOutline).c_str());
  xmlSetProp(node, BAD_CAST "shadow_color",
             BAD_CAST toString(_ShadowColor).c_str());
  ...
  xmlSetProp(node, BAD_CAST "multi_line",
             BAD_CAST toString(_MultiLine).c_str());
  ...
  xmlSetProp(node, BAD_CAST "multi_line_space",
             BAD_CAST toString(_MultiLineSpace).c_str());
  xmlSetProp(node, BAD_CAST "multi_line_maxw_only",
             BAD_CAST toString(_MultiLineMaxWOnly).c_str());
  xmlSetProp(node, BAD_CAST "multi_max_line",
             BAD_CAST toString(_MultiMaxLine).c_str());
  xmlSetProp(node, BAD_CAST "multi_min_line",
             BAD_CAST toString(_MultiMinLine).c_str());
  xmlSetProp(node, BAD_CAST "underlined",
             BAD_CAST toString(_Underlined).c_str());
  xmlSetProp(node, BAD_CAST "strikethrough",
             BAD_CAST toString(_StrikeThrough).c_str());
  xmlSetProp(node, BAD_CAST "case_mode",
             BAD_CAST toString(uint32(_CaseMode)).c_str());
  xmlSetProp(node, BAD_CAST "over_extend_view_text",
             BAD_CAST toString(_OverExtendViewText).c_str());
  xmlSetProp(node, BAD_CAST "over_extend_parent_rect",
             BAD_CAST toString(_OverExtendViewTextUseParentRect).c_str());
  xmlSetProp(node, BAD_CAST "auto_clamp",
             BAD_CAST toString(_AutoClamp).c_str());
  xmlSetProp(node, BAD_CAST "clamp_right",
             BAD_CAST toString(_ClampRight).c_str());
  xmlSetProp(node, BAD_CAST "auto_clamp_offset",
             BAD_CAST toString(_AutoClampOffset).c_str());
  xmlSetProp(node, BAD_CAST "continuous_update",
             BAD_CAST toString(_ContinuousUpdate).c_str());
  xmlSetProp(node, BAD_CAST "sizing_chars", BAD_CAST _FontSizingChars.c_str());
  xmlSetProp(node, BAD_CAST "sizing_fallback",
             BAD_CAST _FontSizingFallback.c_str());
...
xmlNodePtr CViewText::serialize(xmlNodePtr parentNode, const char *type) const {
  ...
  xmlSetProp(node, BAD_CAST "text",
             BAD_CAST(_HardText.empty() ? _Text.c_str() : _HardText.c_str()));
  xmlSetProp(node, BAD_CAST "hardtext_format",
             BAD_CAST _HardTextFormat.c_str());
...
void CViewText::parseTextOptions(xmlNodePtr cur) {
  ...
  if (prop)
    _Localized = convertBool((const char *)prop);
  ...
  _Embolden = false;
  if (prop) {
    if (nlstricmp("bold", (const char *)prop) == 0)
      _Embolden = true;
    else
      nlwarning("<CViewText::parse> bad fontweight '%s'", (const char *)prop);
  }

  prop = (char *)xmlGetProp(cur, (xmlChar *)"fontstyle");
  _Oblique = false;
  if (prop) {
    if (nlstricmp("oblique", (const char *)prop) == 0)
      _Oblique = true;
    else
      nlwarning("<CViewText::parse> bad fontstyle '%s'", (const char *)prop);
  }
  ...
  if (prop) {
    if (nlstricmp("clip_word", (const char *)prop) == 0)
      _TextMode = ClipWord;
    else if (nlstricmp("dont_clip_word", (const char *)prop) == 0)
      _TextMode = DontClipWord;
    else if (nlstricmp("justified", (const char *)prop) == 0)
      _TextMode = Justified;
    else if (nlstricmp("centered", (const char *)prop) == 0)
      _TextMode = Centered;
    else
      nlwarning("<CViewText::parse> bad text mode");
  }
...
void CViewText::draw() {
  ...
  if (getModulateGlobalColor()) {
    ...
    col = _Color;
    shcol = _ShadowColor;
    col.A = (uint8)(((sint)col.A * ((sint)CWidgetManager::getInstance()
                                        ->getGlobalColorForContent()
                                        .A +
                                    1)) >>
                    8);
    shcol.A = (uint8)(((sint)shcol.A * ((sint)CWidgetManager::getInstance()
                                            ->getGlobalColorForContent()
                                            .A +
                                        1)) >>
                      8);
  ...
  if ((_MultiLine) && (_Parent != NULL)) {
    ...
    if (_TextSelection) {
      ...
      for (sint i = 0; i < (sint)_Lines.size(); i++) {
        ...
        for (uint k = 0; k < currLine.getNumWords(); ++k) {
          ...
          sint cEnd = min(charIndex + (sint)currWord.Info.StringLength,
                          (sint)_TextSelectionEnd);

          // range must be valid
          if (cStart < cEnd) {
            // select subset. Arg, must skip spaces because not inserted in
            // VertexBuffer.
            setStringSelectionSkipingSpace(currWord.Index, currWord.Text,
                                           cStart - charIndex,
                                           cEnd - charIndex);
          } else {
            // select none
    ...
    for (sint i = (sint)_Lines.size() - 1; i >= 0; --i) {
      ...
      //
      // Does not give most accurate result when _WReal is much smaller than
      // parent, but _WReal also defines mouseover hotspot/tooltip area.
      //
      // May not work correctly in CGroupParagraph (multiple text elements).
      //
      if (_TextMode == Centered)
        px += (float)(_WReal * _Scale -
                      (currLine.getWidth() + (i == 0 ? _FirstLineX : 0.f))) /
              2.f;
      ...
      for (uint k = 0; k < currLine.getNumWords(); ++k) {
        ...
        if (_Underlined || _StrikeThrough) {
          line_width = (sint)floorf(currLine.getWidthWithoutSpaces() +
                                    currLine.getSpaceWidth());
        ...
        if (currWord.Format.TabX)
          px = max(
              px, (float)(_XReal * _Scale + currWord.Format.TabX * _FontWidth));

        // draw. We take floorf px to avoid filtering of letters that are not
        // located on a pixel boundary
        float fx = px / _Scale;
        float fy = y / _Scale;
        rVR.drawText(_RenderLayer, fx, fy, currWord.Index, ClipX, ClipY,
                     ClipX + ClipW, ClipY + ClipH, *TextContext);

        // Draw a line
        if (_Underlined)
          rVR.drawRotFlipBitmap(_RenderLayer, fx,
                                fy - _FontLegHeight * 0.6f / _Scale,
                                line_width / _Scale, 1.0f / _Scale, 0, false,
                                rVR.getBlankTextureId(), col);

        if (_StrikeThrough)
          rVR.drawRotFlipBitmap(_RenderLayer, fx,
                                fy + _FontHeight * 0.2f / _Scale,
                                line_width / _Scale, 1.0f / _Scale, 0, false,
                                rVR.getBlankTextureId(), col);
  ...
  // *** Single Line Version (currently no support for text justification)
    ...
    if (_LetterColors != NULL &&
        !TextContext->isSameLetterColors(_LetterColors, _Index)) {
    ...
    if (_TextSelection)
      // select subset. Arg, must skip spaces because not inserted in
      // VertexBuffer.
      setStringSelectionSkipingSpace(_Index, _Text, _TextSelectionStart,
                                     _TextSelectionEnd);
    ...
    // draw
    rVR.drawText(_RenderLayer, _XReal, y / _Scale, _Index, ClipX, ClipY,
                 ClipX + ClipW, ClipY + ClipH, *TextContext);

    // Draw a line
    if (_Underlined)
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal,
                            _YReal - _FontLegHeight * 0.3f / _Scale, _WReal, 1,
                            0, false, rVR.getBlankTextureId(), col);

    if (_StrikeThrough)
      rVR.drawRotFlipBitmap(_RenderLayer, _XReal,
                            _YReal + _FontHeight * 0.2f / _Scale, _WReal, 1, 0,
                            false, rVR.getBlankTextureId(), col);
    ...
    if (isSingleLineTextClamped() && _OverExtendViewText &&
      ...
      if (mouseIn) {
        ...
        CInterfaceGroup *pIG =
            CWidgetManager::getInstance()->getWindowUnder(x, y);
        ...
        if (bFound) {
          ...
          if (!gc || !gc->isMoving()) {
            CRGBA col = CWidgetManager::getInstance()
                            ->getSystemOption(
                                CWidgetManager::OptionViewTextOverBackColor)
                            .getValColor();
...
// ***************************************************************************
void CViewText::setTextAsUtf16(const ucstring &text) { setText(text.toUtf8()); }
#endif
...
void CViewText::setLocalized(bool localized) {
  ...
  nlassert(_Text.empty() ||
           ((_Localized && (NLMISC::startsWith(getHardText(), "ui"))) ==
            (_HardText.empty() == _Text.empty())));
}
...
void CViewText::setText(const std::string &text) {
  ...
  nlassert(_Text.empty() || ((_Localized && (NLMISC::startsWith(text, "ui"))) ==
                             (_HardText.empty() == _Text.empty())));
...
// ***************************************************************************
void CViewText::setFontSizing(const std::string &chars,
...
sint CViewText::getFontSize() const {
  if (_FontSizeCoef)
    return _FontSize - CWidgetManager::getInstance()
                           ->getSystemOption(CWidgetManager::OptionAddCoefFont)
                           .getValSInt32();
...
// ***************************************************************************
void CViewText::setColor(const NLMISC::CRGBA &color) { _Color = color; }
...
// ***************************************************************************
uint CViewText::getFontWidth() const { return _FontWidth / _Scale; }

// ***************************************************************************
uint CViewText::getFontHeight() const { return _FontHeight / _Scale; }

// ***************************************************************************
uint CViewText::getFontLegHeight() const { return _FontLegHeight / _Scale; }
...
uint CViewText::getMultiMinOffsetY() const {
  ...
  if (_MultiMinLine > _Lines.size()) {
    ...
    dy = (nbLines * _FontHeight + (nbLines - 1) * _MultiLineSpace * _Scale) /
         _Scale;
  }
  return dy;
}
...
// ***************************************************************************
void CViewText::flushWordInLine(std::string &ucCurrentWord, bool &linePushed,
                                const CFormatInfo &wordFormat) {
  NL3D::UTextContext *TextContext = CViewRenderer::getTextContext(_FontName);
  // create a new line?
  if (!linePushed) {
...
void CViewText::updateTextContextMultiLine(float nMaxWidth) {
  ...
  for (CUtfStringView::iterator it(sv.begin()), end(sv.end()); it != end;
       ++it, ++i) {
    if (isFormatTagChange(i, formatTagIndex)) {
      ...
      rWidthCurrentLine =
          max(rWidthCurrentLine, (float)wordFormat.TabX * _FontWidth);
    }
...
// ***************************************************************************
void CViewText::updateTextContextMultiLineJustified(float nMaxWidth,
                                                    bool expandSpaces) {
  NL3D::UTextContext *TextContext = CViewRenderer::getTextContext(_FontName);
  UTextContext::CStringInfo si;
  //
  ...
  bool breakLine;
  //
  vector<CWord> currLine; // if spaces are not expanded, all words of a line are
                          // inserted here (NB: index and stringInfo not filled)
  ...
  while (currPos != _Text.length()) {
    ...
    if (_Text[currPos] == '\n') {
      ...
      if (!isParagraphStart && numSpaces != 0 &&
          numWordsInLine == 0) // Are these the first spaces of the line ?
      {
        if (!_Lines.empty()) {
          /* Yoyo: I changed this (added the "cut space"), because in editBox,
             it is so strange when the word hit the end of line, and if you add
             spaces just after, nothing happens because cursor pos is clamped at
             end of line.
          */
          // Cannot put all of thoses spaces to the prec end of line?
          if (_MultiLineClipEndSpace &&
              precLineWidth + numSpaces * _SpaceWidth > nMaxWidth) {
            // put some of these spaces at the end of the previous line.
            sint maxNumSpaces =
                (sint)floorf((nMaxWidth - precLineWidth) / _SpaceWidth);
      ...
      for (i = (uint)spaceEnd; i < (uint)_Text.length(); i++) {
        ...
        if (c == ' ' || c == '\n')
          break;
        // If change of color tag, stop the word, but don't take the new color
        // now.
        if (isFormatTagChange(i, formatTagIndex))
          break;
      }
    ...
    if (!lineFeed && newLineWidth > nMaxWidth) {
      ...
      bool roomForThisWord =
          (numWordsInLine > 0) || ((newLineWidth - lineWidth) < nMaxWidth);

      // not enough room for that word
      // If it is the only word of the line, just split it
      // Otherwise, scale the spaces between words so that the line as the
      // maximum width
      if (roomForThisWord) {
        if (expandSpaces) {
          ...
          if (wordFormat.TabX > 0)
            ...
            float roomForSpaces =
                nMaxWidth - _Lines.back()->getWidthWithoutSpaces();
        ...
        // we dont change the position in the input string so that the current
        // will be processed on the next line
      } else // it is the only word on the line..
      {
        ...
        if (numSpaces * _SpaceWidth > nMaxWidth) {
          ...
          for (; wit != wend; ++wit) {
            ...
            if ((uint)(px + si.StringWidth) > nMaxWidth)
              break;
            px += si.StringWidth;
          }
          ptrdiff_t currChar = std::max(
              (ptrdiff_t)1,
              (ptrdiff_t)wit.ptr() -
                  (ptrdiff_t)wsv.ptr()); // must fit at least one character
                                         // otherwise there's an infinite loop
        ...
        breakLine) // '\n' was encoutered, or a linefeed has been asked
    {
      ...
      // expandSpaces => just add a empty line.
        ...
        if (numWordsInLine == 0) {
          // if nothing has been inserted in this line, create at least an empty
          // line
          _Lines.push_back(TLineSPtr(new CLine));
        }
        if (lineFeed) {
...
void CViewText::updateTextContext() {
  ...
  if ((_MultiLine) && (_Parent != NULL)) {
    ...
    case DontClipWord:
      updateTextContextMultiLineJustified(nMaxWidth, false);
      break;
    case Justified:
      updateTextContextMultiLineJustified(nMaxWidth, true);
      break;
    }

    // Special case for multiline limited in number of lines
    if (!_Lines.empty() && (_MultiMaxLine > 0) &&
        (_Lines.size() > _MultiMaxLine)) {
    ...
    for (uint i = 0; i < _Lines.size(); ++i) {
      rTotalW = std::max(_Lines[i]->getWidth() + ((i == 0) ? _FirstLineX : 0),
                         rTotalW);
    }
    _W = (sint)ceilf(rTotalW / _Scale);
    _H = std::max(
        _FontHeight / _Scale,
        ceilf((_FontHeight * _Lines.size() +
               std::max(0, sint(_Lines.size()) - 1) * rMultiLineSpace) /
              _Scale));

    // See if we should pretend to have at least X lines
    if (_MultiMinLine > 1)
      _H = std::max(_H, sint((_FontHeight * _MultiMinLine +
                              (_MultiMinLine - 1) * rMultiLineSpace) /
                             _Scale));
        ...
        for (uint j = 0; j < _Lines[i]->getNumWords(); ++j) {
          ...
          if (word.Format.IndexTt != -1) {
            if (_Tooltips.size() > (uint)word.Format.IndexTt) {
              ...
              sint y = (sint)ceilf(
                  ((_FontHeight + rMultiLineSpace) * (_Lines.size() - i - 1)) /
                  _Scale);
...
void CViewText::updateCoords() {
  if (_AutoClamp) {
    ...
    if (parent) {
      ...
      while (parent &&
             (parent->getResizeFromChildW() || parent->isGroupList())) {
        // NB nico : the dynamic_cast for CGroupList is bad!!
        // can't avoid it for now, because, CGroupList implicitly does a "resize
        // from child" in its update coords
        // ...
        parent = parent->getParent();
      }
      if (parent) {
        if (_ClampRight) {
          sint32 parentRight = parent->getXReal() + parent->getWReal() -
                               (sint32)_AutoClampOffset;
...
// ***************************************************************************
sint CViewText::getLineFromIndex(
    uint index, bool cursorDisplayedAtEndOfPreviousLine /* = true*/) const {
  ...
  if (_MultiLine) {
    ...
    for (sint i = 0; i < (sint)_Lines.size(); ++i) {
      CLine &currLine = *_Lines[i];
      uint newCharIndex = charIndex + currLine.getNumChars() +
                          currLine.getEndSpaces() + (currLine.getLF() ? 1 : 0);
      if (newCharIndex > index) {
        if (i != 0 && cursorDisplayedAtEndOfPreviousLine &&
            charIndex == index) {
          return i - 1;
        } else {
          return i;
...
sint CViewText::getLineStartIndex(uint line) const {
  ...
  for (uint i = 0; i < line; ++i) {
    CLine &currLine = *_Lines[i];
    charIndex += currLine.getNumChars() + currLine.getEndSpaces() +
                 (currLine.getLF() ? 1 : 0);
  }
...
// ***************************************************************************
void CViewText::getLineEndIndex(uint line, sint &index,
                                bool &endOfPreviousLine) const {
  ...
  index =
      startIndex + _Lines[line]->getNumChars() + _Lines[line]->getEndSpaces();
...
string CViewText::getColorAsString() const {
  return NLMISC::toString(_Color.R) + " " + NLMISC::toString(_Color.G) + " " +
         NLMISC::toString(_Color.B) + " " + NLMISC::toString(_Color.A);
}
...
// ***************************************************************************
NLMISC::CRGBA CViewText::getColorRGBA() const { return _Color; }

// ***************************************************************************
void CViewText::setColorRGBA(NLMISC::CRGBA col) { _Color = col; }

// ***************************************************************************
void CViewText::getCharacterPositionFromIndex(sint index,
  ...
  if (_MultiLine) {
    ...
    if (index >= (sint)_TextLength) {
      ...
      if (_Lines.empty()) {
        ...
        fx = lastLine.getWidth() +
             lastLine.getEndSpaces() * lastLine.getSpaceWidth();
    ...
    for (sint i = 0; i < (sint)_Lines.size(); ++i) {
      if (i != 0 && charIndex == (uint)index && cursorAtPreviousLineEnd) {
        ...
        fy =
            (_FontHeight + _MultiLineSpace * _Scale) * (_Lines.size() - i) + dy;
        fx = currLine.getWidth() +
             currLine.getEndSpaces() * currLine.getSpaceWidth();
        fx = std::min(fx, nMaxWidth);
      ...
      CLine &currLine = *_Lines[i];
      uint newCharIndex = charIndex + currLine.getNumChars() +
                          currLine.getEndSpaces() +
                          (_Lines[i]->getLF() ? 1 : 0);
      if ((sint)newCharIndex > index) {
        // ok, this line contains the character, now, see which word contains
        // it.
        fy =
            (_FontHeight + _MultiLineSpace * _Scale) * (_Lines.size() - 1 - i) +
            dy;
        ...
        for (uint k = 0; k < currLine.getNumWords(); ++k) {
          CWord &currWord = currLine.getWord(k);
          if ((sint)(charIndex + currWord.NumSpaces +
                     currWord.Info.StringLength) >= index) {
            ...
            if ((uint)(index - charIndex) > currWord.NumSpaces) {
              ...
              UTextContext::CStringInfo si = TextContext->getStringInfo(
                  currWord.Text,
                  (ptrdiff_t)index - charIndex - currWord.NumSpaces);
              fx = px + si.StringWidth +
                   currWord.NumSpaces * currLine.getSpaceWidth();
          ...
          px += currWord.NumSpaces * currLine.getSpaceWidth() +
                currWord.Info.StringWidth;
        }
      }
      charIndex = newCharIndex;
    }
...
// ***************************************************************************
// Tool fct : From a word and a x coordinate (font scale), give the matching
// character index
static uint getCharacterIndex(const std::string &textValue, float x,
  ...
  for (NLMISC::CUtfStringView::iterator it(sv.begin()), end(sv.end());
       it != end; ++it, ++i) {
    // get character width
    singleChar[0] = *it;
    ...
    if (px > x) {
      // if the half of the character is after the cursor, then prefer select
      // the next one (like in Word)
...
// ***************************************************************************
void CViewText::getCharacterIndexFromPosition(
    float x, float y, uint &index, bool &cursorAtPreviousLineEnd) const {
  NL3D::UTextContext *TextContext = CViewRenderer::getTextContext(_FontName);
  ...
  if (_MultiLine) {
    ...
    sint i;
    for (i = 0; i < line; ++i) {
      charPos += _Lines[i]->getNumChars() + _Lines[i]->getEndSpaces() +
                 (_Lines[i]->getLF() ? 1 : 0);
    }
    ...
    if (x >= (sint)currLine.getWidth()) {
      ...
      sint numSpaces =
          _SpaceWidth != 0
              ? (sint)(((float)x + _SpaceWidth / 2 - currLine.getWidth()) /
                       _SpaceWidth)
              : 0;
    ...
    for (uint k = 0; k < currLine.getNumWords(); ++k) {
      ...
      if (newPx >= x) // if the word contains the x position..
      {
        if (x < (px + spacesWidth)) {
          ...
          sint numSpaces =
              currLine.getSpaceWidth() != 0
                  ? (sint)((x + currLine.getSpaceWidth() / 2 - px) /
                           currLine.getSpaceWidth())
                  : 0;
          ...
          index = charPos + currWord.NumSpaces +
                  getCharacterIndex(currWord.Text, x - (px + spacesWidth),
                                    *TextContext);
          return;
        }
      }
...
// ***************************************************************************
void CViewText::setStringSelectionSkipingSpace(uint stringId,
                                               const std::string &text,
                                               sint charStart, sint charEnd) {
  ...
  CViewRenderer::getTextContext(_FontName)->setStringSelection(
      stringId, quadStart,
      quadSize); // FIXME: This assumes spaces are the only empty glyphs!
}
...
// ***************************************************************************
uint CViewText::getFirstLineX() const { return _FirstLineX / _Scale; }
...
// ***************************************************************************
CViewText::CLine::CLine()
    : _NumChars(0), _NumSpaces(0), _SpaceWidth(0.f), _StringLine(0),
      _WidthWithoutSpaces(0.f), _EndSpaces(0), _HasLF(false) {}

// ***************************************************************************
void CViewText::CLine::addWord(const std::string &text, uint numSpaces,
                               const CFormatInfo &wordFormat, float fontWidth,
                               NL3D::UTextContext &textContext) {
  CWord word;
...
// ***************************************************************************
void CViewText::CWord::build(const std::string &text,
                             NL3D::UTextContext &textContext, uint numSpaces) {
  Text = text;
  NumSpaces = numSpaces;
...
void CViewText::computeFontSize() {
  ...
  // font generator changes unknown glyphs to dot '.'. use fallback if it looks
  // odd
...
// ***************************************************************************
static inline bool isTooltipTag(const std::string &s, uint index,
                                uint textSize) {
  // Format is @{Huitt*}
  if (s[index] == '@') {
    if (textSize > index + 1 && s[index + 1] == '{') {
...
// ***************************************************************************
void CViewText::buildFormatTagText(
    const std::string &text, std::string &textBuild,
    std::vector<CViewText::CFormatTag> &formatTags,
...
void CViewText::setTextFormatTaged(const std::string &text) {
  ...
  // to allow cache (avoid infinite recurse in updateCoords() in some case),
  // compute in temp
  std::string tempText;
  ...
  if (!_MultiLine)
    nlwarning("ViewText isn't multilined : uc_hardtext_format will not act as "
              "wanted !\n%s",
              text.c_str());
}
...
void CViewText::setSingleLineTextFormatTaged(const std::string &text) {
  ...
  // test transformed text with current one
  if (tempText != _Text || !_LetterColors ||
      !_LetterColors->isSameLetterColors(letterColors)) {
  ...
  if (_MultiLine)
    nlwarning("ViewText is multilined : uc_hardtext_single_line_format will "
              "not act as wanted !\n%s",
              text.c_str());
}
...
bool CViewText::isFormatTagChange(uint textIndex, uint ctIndex) const {
  if (ctIndex >= _FormatTags.size())
    return false;
  // return true if the textIndex is > (eg if some skip with spaces) or =
  // (common case)
  return _FormatTags[ctIndex].Index <= textIndex;
}

// ***************************************************************************
void CViewText::getFormatTagChange(uint textIndex, uint &ctIndex,
                                   CFormatInfo &wordFormat) const {
  // support the possible case with multiple color tags with same textIndex.
  while (ctIndex < _FormatTags.size() &&
         _FormatTags[ctIndex].Index <= textIndex) {
    // Take the last tag.
...
TCaseMode CViewText::getCaseMode() const { return _CaseMode; }
...
#define SERIAL_UINT(val)                                                       \
  {                                                                            \
    uint32 tmp = (uint32)val;                                                  \
    f.serial(tmp);                                                             \
    val = (uint)tmp;                                                           \
  }
#define SERIAL_SINT(val)                                                       \
  {                                                                            \
    sint32 tmp = (sint32)val;                                                  \
    f.serial(tmp);                                                             \
    val = (sint)tmp;                                                           \
  }
  CViewBase::serial(f);

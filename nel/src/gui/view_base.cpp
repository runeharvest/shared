...
void CViewBase::dumpSize(uint depth /*=0*/) const {
  ...
  result += NLMISC::toString(
      "id=%s, w=%d, h=%d, x=%d, y=%d, wreal=%d, hreal=%d, xreal=%d, yreal=%d",
      shortID.c_str(), (int)_W, (int)_H, (int)_X, (int)_Y, (int)_WReal,
      (int)_HReal, (int)_XReal, (int)_YReal);
  nlinfo(result.c_str());
}
...
bool CViewBase::handleEvent(const NLGUI::CEventDescriptor &evnt) {
  if (evnt.getType() == NLGUI::CEventDescriptor::mouse) {
    const NLGUI::CEventDescriptorMouse &eventDesc =
        (const NLGUI::CEventDescriptorMouse &)evnt;

...
// Needed because otherwise GCC and co. omit the code in
// interface_expr_user_fct.cpp code causing the GUI not to work. It all happens
// because no function is called *directly* from that module.
struct LinkTrickster {
  LinkTrickster() { ifexprufct_forcelink(); }
};
...
void formatLuaCall(const std::string &expr, std::string &tempStr) {
  /* Call the LUA interface exp fct, with the script as line, and resolve string
     definition conflicts: eg:  replace lua:getSkillFromName('SM') into
          lua('getSkillFromName(\"SM\")')
  */
  tempStr =
      expr.substr(ExprLuaId.size()); // eg: tempStr= getSkillFromName('SM')
  while (strFindReplace(tempStr, "'", "\\\""))
    ; // eg: tempStr= getSkillFromName(\"SM\")
  tempStr = string("lua('") + tempStr +
            "')"; // eg: tempStr= lua('getSkillFromName(\"SM\")')
}

//==================================================================
bool CInterfaceExpr::eval(const std::string &expr, CInterfaceExprValue &result,
                          std::vector<ICDBNode *> *nodes,
                          bool noFctCalls /* = false */) {
...
CInterfaceExprNode *CInterfaceExpr::buildExprTree(const std::string &expr) {
  ...
  if (expr.compare(0, ExprLuaId.size(), ExprLuaId) == 0) {
    std::string tempStr;
    formatLuaCall(expr, tempStr);
    if (!buildExprTree(tempStr.c_str(), node))
      return NULL;
  } else {
    if (!buildExprTree(expr.c_str(), node))
      return NULL;
  }
...
//==================================================================
const char *CInterfaceExpr::evalExpr(const char *expr,
...
//==================================================================
const char *CInterfaceExpr::buildExprTree(const char *expr,
                                          CInterfaceExprNode *&result) {
  nlassert(expr != NULL);
  expr = skipBlank(expr);
  if (isalpha(*expr)) // alpha character means this is a function name
  {
...
//==================================================================
const char *CInterfaceExpr::evalFct(const char *expr,
                                    CInterfaceExprValue &result,
                                    std::vector<ICDBNode *> *nodes,
                                    bool noFctCalls) {
  if (!_UserFct)
  ...
  if (fctIt == _UserFct->end()) {
    nlwarning("<CInterfaceExpr::evalFct> : Unknown function %s",
              fctName.c_str());
    return NULL;
  }
  ...
  expr = skipBlank(expr);
  if (*expr != '(') {
    nlwarning("<CInterfaceExpr::evalFct> : '(' expected for function %s",
              fctName.c_str());
    return NULL;
  }
  ++expr;
  ...
  if (*expr != ')') {
    for (;;) {
      ...
      if (*expr == ')')
        break;
      // if it isn't the end of the expression, then we should find a ',' before
      // next argument
      if (*expr != ',') {
        nlwarning("<CInterfaceExpr::evalFct> : ',' expected in function %s",
                  fctName.c_str());
        return NULL;
      }
      ++expr;
  ...
  {
    if (fctIt->second(argList, result))
      return expr;
  } else {
    return expr;
  }
...
//==================================================================
const char *CInterfaceExpr::buildFctNode(const char *expr,
                                         CInterfaceExprNode *&result) {
  if (!_UserFct)
    _UserFct = new TUserFctMap;
  ...
  TUserFctMap::iterator fctIt = _UserFct->find(fctName);
  if (fctIt == _UserFct->end()) {
    nlwarning("<CInterfaceExpr::buildFctNode> : Unknown function %s",
              fctName.c_str());
    return NULL;
  }
  nlassert(fctIt->second != NULL);
  ...
  expr = skipBlank(expr);
  if (*expr != '(') {
    nlwarning("<CInterfaceExpr::buildFctNode> : '(' expected for function %s",
              fctName.c_str());
    return NULL;
  }
  ++expr;
...
//==================================================================
const char *CInterfaceExpr::evalDBEntry(const char *expr,
                                        CInterfaceExprValue &result,
                                        std::vector<ICDBNode *> *nodes) {
  std::string dbEntry;
  ...
  if (!expr)
    return NULL;
  // TestYoyo
  // nlassert(NLGUI::CDBManager::getInstance()->getDbProp(dbEntry, false) ||
  // CInterfaceManager::getInstance()->getDbBranch(dbEntry)); get the db value
...
//==================================================================
const char *CInterfaceExpr::buildDBEntryNode(const char *expr,
                                             CInterfaceExprNode *&result) {
  std::string dbEntry;
  bool indirection;
  ...
  if (indirection) {
    ...
    CInterfaceExprNodeDependantDBRead *node =
    ...
    // TestYoyo
    // nlassert(NLGUI::CDBManager::getInstance()->getDbProp(dbEntry, false) ||
    // CInterfaceManager::getInstance()->getDbBranch(dbEntry));
    CCDBNodeLeaf *nl = NLGUI::CDBManager::getInstance()->getDbProp(dbEntry);
    if (nl) {
...
//==================================================================
const char *CInterfaceExpr::unpackDBentry(const char *expr,
                                          std::vector<ICDBNode *> *nodes,
                                          std::string &dest,
                                          bool *hasIndirections /* = NULL*/) {
  ...
  for (;;) {
    if (*expr == '[') {
      ...
      // TestYoyo
      // nlassert(NLGUI::CDBManager::getInstance()->getDbProp(subEntry, false)
      // || CInterfaceManager::getInstance()->getDbBranch(subEntry));
...
bool CInterfaceExpr::evalAsInt(const std::string &expr, sint64 &dest) {
  ...
  if (!eval(expr, result))
    return false;
  if (!result.toInteger()) {
    nlwarning("<CInterfaceExpr::evalAsInt> Can't convert value to an integer, "
              "expr = %s",
              expr.c_str());
    return false;
  }
...
bool CInterfaceExpr::evalAsDouble(const std::string &expr, double &dest) {
  ...
  if (!eval(expr, result))
    return false;
  if (!result.toDouble()) {
    nlwarning("<CInterfaceExpr::evalAsDouble> Can't convert value to a double, "
              "expr = %s",
              expr.c_str());
    return false;
  }
...
bool CInterfaceExpr::evalAsBool(const std::string &expr, bool &dest) {
  ...
  if (!eval(expr, result))
    return false;
  if (!result.toBool()) {
    nlwarning("<CInterfaceExpr::evalAsBool> Can't convert value to a boolean, "
              "expr = %s",
              expr.c_str());
    return false;
  }
...
bool CInterfaceExpr::evalAsString(const std::string &expr, std::string &dest) {
  ...
  if (!eval(expr, result))
    return false;
  if (!result.toString()) {
    nlwarning("<CInterfaceExpr::evalAsString> Can't convert value to a string, "
              "expr = %s",
              expr.c_str());
    return false;
  }
...
bool CInterfaceExprValue::toBool() {
  ...
  case Boolean:
    return true;
  case Integer:
    setBool(_IntegerValue != 0);
    return true;
  case Double:
    setBool(_DoubleValue != 0);
    return true;
  case String:
    return evalBoolean(_StringValue.c_str()) != NULL;
  default:
    break;
  }
  return false;
...
bool CInterfaceExprValue::toInteger() {
  switch (_Type) {
  case Boolean:
    setInteger(_BoolValue ? 1 : 0);
    return true;
  case Integer:
    return true;
  case Double:
    setInteger((sint64)_DoubleValue);
    return true;
  case String:
    if (evalNumber(_StringValue.c_str()))
      return toInteger();
    return false;
  case RGBA:
    setInteger((sint64)_RGBAValue);
    return true;
  default:
    break;
  }
...
bool CInterfaceExprValue::toString() {
  switch (_Type) {
  case Boolean:
    setString(_BoolValue ? "true" : "false");
    return true;
  case Integer:
...
bool CInterfaceExprValue::toRGBA() {
  ...
  case Integer:
    setRGBA(NLMISC::CRGBA((uint8)(_IntegerValue & 0xff),
                          (uint8)((_IntegerValue >> 8) & 0xff),
                          (uint8)((_IntegerValue >> 16) & 0xff),
                          (uint8)((_IntegerValue >> 24) & 0xff)));
    return true;
...
const char *CInterfaceExprValue::evalBoolean(const char *expr) {
  nlassert(expr);
  expr = skipBlank(expr);
  if (toupper(expr[0]) == 'T' && toupper(expr[1]) == 'R' &&
      toupper(expr[2]) == 'U' && toupper(expr[3]) == 'E') {
    setBool(true);
    return expr + 4;
  }
  //
  if (toupper(expr[0]) == 'F' && toupper(expr[1]) == 'A' &&
      toupper(expr[2]) == 'L' && toupper(expr[3]) == 'S' &&
      toupper(expr[4]) == 'E') {
    setBool(false);
    return expr + 5;
  }
...
const char *CInterfaceExprValue::evalString(const char *expr) {
  ...
  for (;;) {
    if (*expr == '\0') {
      nlwarning("CInterfaceExprValue::evalString : end of buffer encountered "
                "in a string");
      return NULL;
    } else if (*expr == '\'') {
      ++expr;
    ...
    if (*expr == '\\') // special char
    {
      ...
      default:
        nlwarning(
            "CInterfaceExprValue::evalString : unknown escape sequence : \\%c",
            *expr);
        if (*expr)
          str += *expr;
        break;
      ...
      nlwarning("CInterfaceExprValue::evalString : line break encountered in a "
                "string");
      return NULL;
    } else {
      str += *expr;
...
void CInterfaceExprValue::setUserType(CInterfaceExprUserType *value) {
  if (_Type == UserType && value == _UserTypeValue)
    return;
  clean();
  _Type = UserType;
...
//==================================================================
CInterfaceExprValue::CInterfaceExprValue(const CInterfaceExprValue &other)
    : _Type(NoType) {
  *this = other;
}

...
CCtrlBase::TToolTipParentType
CCtrlBase::stringToToolTipParent(const std::string &str) {
...
bool CCtrlBase::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (prop)
    _ToolTipInstant = convertBool(prop);
...
void CCtrlBase::setProperty(const std::string &name, const std::string &value) {
  if (name == "tooltip") {
    ...
    else if (_ToolTipParentPosRef == Hotspot_TTAuto)
    ...
    else if (_ToolTipPosRef == Hotspot_TTAuto)
    ...
    else if (_ToolTipParentPosRefAlt == Hotspot_TTAuto)
    ...
    else if (_ToolTipPosRefAlt == Hotspot_TTAuto)
      _ToolTipPosRefAlt = _ToolTipParentPosRefAlt;
...
xmlNodePtr CCtrlBase::serialize(xmlNodePtr parentNode, const char *type) const {
  xmlNodePtr node = CInterfaceElement::serialize(parentNode, type);
  ...
  xmlNewProp(node, BAD_CAST "on_tooltip",
             BAD_CAST _OnContextHelp.toString().c_str());
  xmlNewProp(node, BAD_CAST "on_tooltip_params",
             BAD_CAST _OnContextHelpParams.toString().c_str());
  xmlNewProp(node, BAD_CAST "tooltip_parent",
             BAD_CAST tooltipParentToString(_ToolTipParent).c_str());
  xmlNewProp(node, BAD_CAST "tooltip_special_parent",
             BAD_CAST _ToolTipSpecialParent.toString().c_str());

  xmlNewProp(
      node, BAD_CAST "tooltip_posref",
      BAD_CAST TooltipHotSpotToString(_ToolTipParentPosRef, _ToolTipPosRef)
          .c_str());

  xmlNewProp(node, BAD_CAST "tooltip_posref_alt",
             BAD_CAST TooltipHotSpotToString(_ToolTipParentPosRefAlt,
                                             _ToolTipPosRefAlt)
                 .c_str());

  xmlNewProp(node, BAD_CAST "instant_help",
             BAD_CAST toString(_ToolTipInstant).c_str());

  return node;
}

// ***************************************************************************
void CCtrlBase::convertTooltipHotSpot(const char *prop, THotSpot &parentHS,
                                      THotSpot &childHS) {

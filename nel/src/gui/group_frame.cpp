...
// ***************************************************************************
CGroupFrame::CGroupFrame(const TCtorParam &param) : CInterfaceGroup(param) {
  _DisplayFrame = true;
...
void CGroupFrame::setProperty(const std::string &name,
...
xmlNodePtr CGroupFrame::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "display",
             BAD_CAST NLMISC::toString(_DisplayFrame).c_str());
...
bool CGroupFrame::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (_DispTypes.empty()) {
    ...
    rVR.getTextureSizeFromId(dt.BorderIds[TextBR], dt.RightBorder,
                             dt.BottomBorder);
    ...
    for (yId = 0; yId < 3; yId++) {
      for (xId = 0; xId < 3; xId++) {
        ...
        // Middle
          ...
          h = _HReal - _DispTypes[_DispType].TopBorder -
              _DispTypes[_DispType].BottomBorder;
        }
        // Bottom
        else {
        ...
        if (xId == 0) {
          ...
          w = _WReal - _DispTypes[_DispType].LeftBorder -
              _DispTypes[_DispType].RightBorder;
        } else {
        ...
        if (tile == 0)
          rVR.drawRotFlipBitmap(_RenderLayer, x, y, w, h, 0, false,
                                _DispTypes[_DispType].BorderIds[yId * 3 + xId],
                                col);
        else
          rVR.drawRotFlipBitmapTiled(
              _RenderLayer, x, y, w, h, 0, false,
              _DispTypes[_DispType].BorderIds[yId * 3 + xId], tile - 1, col);
      }
    }
  }
  // draw the components
...
void CGroupFrame::setupOptions() {
  ...
  if (pIO != NULL) {
    ...
    if (i == _DispTypes.size()) {
      ...
      dt.BorderIds[TextMM] =
          rVR.getTextureIdFromName(pIO->getValStr("tx_blank"));
      ...
      // get size
      rVR.getTextureSizeFromId(dt.BorderIds[TextTL], dt.LeftBorder,
                               dt.TopBorder);
      rVR.getTextureSizeFromId(dt.BorderIds[TextBR], dt.RightBorder,
                               dt.BottomBorder);
      _DispTypes.push_back(dt);

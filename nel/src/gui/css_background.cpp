...
void CSSBackground::setImage(const std::string &value) { image = value; }
...
void CSSBackground::setRepeat(const std::string &value) {
  ...
  if (parts[0] == "repeat")
    repeatX = CSS_VALUE_REPEAT;
  else if (parts[0] == "no-repeat")
    repeatX = CSS_VALUE_NOREPEAT;
  else if (parts[0] == "space")
    repeatX = CSS_VALUE_SPACE;
  else if (parts[0] == "round")
    repeatX = CSS_VALUE_ROUND;
  else
    repeatX = CSS_VALUE_REPEAT;

  if (parts[1] == "repeat")
    repeatY = CSS_VALUE_REPEAT;
  else if (parts[1] == "no-repeat")
    repeatY = CSS_VALUE_NOREPEAT;
  else if (parts[1] == "space")
    repeatY = CSS_VALUE_SPACE;
  else if (parts[1] == "round")
    repeatY = CSS_VALUE_ROUND;
  else
    repeatY = CSS_VALUE_REPEAT;
}
...
void CSSBackground::setOrigin(const std::string &value) {
  if (value == "border-box")
    origin = CSS_VALUE_BORDER_BOX;
  else if (value == "padding-box")
    origin = CSS_VALUE_PADDING_BOX;
  else if (value == "content-box")
    origin = CSS_VALUE_CONTENT_BOX;
  else
    origin = CSS_VALUE_PADDING_BOX;
}
...
void CSSBackground::setClip(const std::string &value) {
  if (value == "border-box")
    clip = CSS_VALUE_BORDER_BOX;
  else if (value == "padding-box")
    clip = CSS_VALUE_PADDING_BOX;
  else if (value == "content-box")
    clip = CSS_VALUE_CONTENT_BOX;
...
void CSSBackground::setAttachment(const std::string &value) {
  if (value == "fixed")
    attachment = CSS_VALUE_FIXED;
  else if (value == "local")
    attachment = CSS_VALUE_LOCAL;
  else if (value == "scroll")
    attachment = CSS_VALUE_SCROLL;
  else
    attachment = CSS_VALUE_SCROLL;
}
  ...
  for (uint index = 0; index < 3; index++) {
    if (parts[index] == "center") {
      ...
      if ((index + 1) < parts.size() &&
          getCssLength(fval, unit, parts[index + 1], true)) {
        newX.setFloatValue(fval, unit);
        index++;
      }
    } else if (isVerticalKeyword(parts[index])) {
  ...
  for (uint index = 0; index < 4; index += 2) {
    ...
    if (isHorizontalKeyword(parts[index])) {
      if (hasX)
        return;
      hasX = true;
      if (!getCssLength(fval, unit, parts[index + 1], true))
        return;
      newH = parts[index] == "left" ? CSS_VALUE_LEFT : CSS_VALUE_RIGHT;
      newX.setFloatValue(fval, unit);
    } else if (isVerticalKeyword(parts[index])) {

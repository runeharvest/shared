...
// ----------------------------------------------------------------------------
CInterfaceTrack::~CInterfaceTrack() { delete _TrackKeyFramer; }
...
bool CInterfaceTrack::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    _Dynamic = CInterfaceElement::convertBool(ptr);
  ...
  if (sTmp == "linear")
    ...
    nlwarning("track unknown type : %s, setting linear by default",
              (const char *)ptr);
  ...
  case Track_TCB:
    ...
    break;
  case Track_Bezier:
    _TrackKeyFramer = UTrackKeyframer::createBezierFloatTrack();
    break;
  default:
    nlstop;
    break;
  ...
  if (_Dynamic) {
    while (cur) {
      ...
      if (isdigit(k.Time[0]) || (k.Time[0] == '-')) {
        ...
        k.Time = toString(
            dTime * CWidgetManager::getInstance()
                        ->getSystemOption(CWidgetManager::OptionMulCoefAnim)
                        .getValFloat());
      }
      ...
      case Track_Bezier: {
        ...
        if (inTan)
          k.InTan = (const char *)inTan;
        if (outTan)
          k.OutTan = (const char *)outTan;
        if (step)
          k.Step = (const char *)step;
      } break;
      ...
      case Track_TCB: {
        ...
        if (tension)
          k.Tension = (const char *)tension;
        if (continuity)
          k.Continuity = (const char *)continuity;
        if (bias)
          k.Bias = (const char *)bias;
        if (easeto)
          k.EaseTo = (const char *)easeto;
        if (easefrom)
          k.EaseFrom = (const char *)easefrom;
      } break;
      default:
        nlstop;
        break;
  ...
  } else // Static parsing
  {
    while (cur) {
      ...
      TAnimationTime animTime =
          fAnimTime * CWidgetManager::getInstance()
                          ->getSystemOption(CWidgetManager::OptionMulCoefAnim)
                          .getValFloat();
      double animValue;
      ...
      case Track_Bezier: {
        ...
        if (inTan)
          fromString((const char *)inTan, k.TanIn);
        if (outTan)
          fromString((const char *)outTan, k.TanOut);
        if (step)
          k.Step = CInterfaceElement::convertBool(step);
      ...
      case Track_TCB: {
        ...
        if (tension)
          fromString((const char *)tension, k.Tension);
        if (continuity)
          fromString((const char *)continuity, k.Continuity);
        if (bias)
          fromString((const char *)bias, k.Bias);
        if (easeto)
          fromString((const char *)easeto, k.EaseTo);
        if (easefrom)
          fromString((const char *)easefrom, k.EaseFrom);
...
void CInterfaceTrack::eval() {
  ...
  case Track_TCB: {
    ...
    for (uint i = 0; i < _DynKeys.size(); ++i) {
      ...
      if (!rKey.Continuity.empty() &&
          CInterfaceExpr::eval(rKey.Continuity, res)) {
        if (!res.toDouble())
...
bool CInterfaceAnim::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (_Duration == 0)
    _Duration = 1.0;
  _Duration *= CWidgetManager::getInstance()
                   ->getSystemOption(CWidgetManager::OptionMulCoefAnim)
                   .getValFloat();
  ...
  if (ptr)
    _AHOnFinish = (const char *)ptr;
  ptr = (char *)xmlGetProp(cur, (xmlChar *)"on_finish_params");
  if (ptr)
    _AHOnFinishParams = (const char *)ptr;
...
void CInterfaceAnim::stop() {
  ...
  if (!_AHOnFinish.empty())
    CAHManager::getInstance()->runActionHandler(_AHOnFinish, _Parent,
                                                _AHOnFinishParams);
}

...
void CViewRenderer::checkNewScreenSize() {
  ...
  if (w != 0 && h != 0) {
    ...
    // Keep old coordinates (suppose resolution won't change, even if typically
    // false wen we swithch from outgame to ingame)
    _IsMinimized = true;
  }
}
...
void CViewRenderer::setInterfaceScale(float scale, sint32 width /*=0*/,
                                      sint32 height /*=0*/) {
  // prevent #div/0
  if (sint(scale * 100) > 0)
...
void CViewRenderer::setRenderStates() {
  ...
  _Material.setBlendFunc(NL3D::UMaterial::srcalpha,
...
NL3D::UDriver *CViewRenderer::getDriver() { return driver; }
...
// ***************************************************************************
bool CViewRenderer::registerFont(const std::string &name,
  ...
  std::string fontFile =
      NLMISC::startsWith(font, "ui") ? font : CPath::lookup(font, false);
  if (fontFile.empty()) {
...
// ***************************************************************************
void CViewRenderer::SImage::setupQuadUV(bool flipv, uint8 rot,
                                        CQuadColorUV &dest) {
  nlassert(rot <= 3);
  // Rotation is CW and flip is along x axis
  // Flip is vertical flip (this means we invert all y for a constant x)
  // The transforms are done in this order : first apply the flip (or not) and
  // then rotate
  ...
  // Take care that the origin in the texture is top left so to get the texture
  // in bottom-up we have to start at Max and go at Min. For left and right this
  // is Min to Max.
  ...
  dest.Uv0 =
      CUV(UVMin.U + UVTab[idx][0].U * du, UVMax.V + UVTab[idx][0].V * dv);
  dest.Uv1 =
      CUV(UVMin.U + UVTab[idx][1].U * du, UVMax.V + UVTab[idx][1].V * dv);
  dest.Uv2 =
      CUV(UVMin.U + UVTab[idx][2].U * du, UVMax.V + UVTab[idx][2].V * dv);
  dest.Uv3 =
      CUV(UVMin.U + UVTab[idx][3].U * du, UVMax.V + UVTab[idx][3].V * dv);

  /* // TRAP : Unrolled Version (To be tested to know if it is faster than the
     previous one) if (flipv)
      {
          switch (rot)
          {
              case 0:
 ...
 */
void CViewRenderer::drawRotFlipBitmapTiled(sint layerId, sint32 x, sint32 y,
                                           sint32 width, sint32 height,
                                           uint8 rot, bool flip, sint32 nTxId,
                                           uint tileOrigin, const CRGBA &col) {
  ...
  if ((x + width) > (_ClipX + _ClipW))
    width -= (x + width) - (_ClipX + _ClipW);
  if ((y + height) > (_ClipY + _ClipH))
    height -= (y + height) - (_ClipY + _ClipH);
 ...
 * drawBitmap
 */
void CViewRenderer::drawRotFlipBitmap(sint layerId, float x, float y,
                                      float width, float height, uint8 rot,
  ...
  if (!flipv && !rot) {
    qcoluv.Uv0.U = rI.UVMin.U;
    qcoluv.Uv0.V = rI.UVMax.V;
    qcoluv.Uv1.U = rI.UVMax.U;
    qcoluv.Uv1.V = rI.UVMax.V;
    qcoluv.Uv2.U = rI.UVMax.U;
    qcoluv.Uv2.V = rI.UVMin.V;
    qcoluv.Uv3.U = rI.UVMin.U;
    qcoluv.Uv3.V = rI.UVMin.V;
  }
  // else standard case
  else {
    if (rot > 3)
 ...
 * sTx must be lowered !!!
 */
void CViewRenderer::draw11RotFlipBitmap(sint layerId, sint32 x, sint32 y,
                                        uint8 rot, bool flipv, sint32 nTxId,
                                        const CRGBA &col) {
  if (nTxId < 0)
    return;
...
void CViewRenderer::drawQuad(
    sint layerId, const NLMISC::CQuadUV &quadUV, sint32 nTxId,
    NLMISC::CRGBA col /*=NLMISC::CRGBA(255,255,255,255)*/, bool additif,
    bool filtered) {
  ...
  //
  normedQuad.V0.set(quadUV.V0.x * _OneOverScreenW,
                    quadUV.V0.y * _OneOverScreenH, 0.f);
  normedQuad.V1.set(quadUV.V1.x * _OneOverScreenW,
                    quadUV.V1.y * _OneOverScreenH, 0.f);
  normedQuad.V2.set(quadUV.V2.x * _OneOverScreenW,
                    quadUV.V2.y * _OneOverScreenH, 0.f);
  normedQuad.V3.set(quadUV.V3.x * _OneOverScreenW,
                    quadUV.V3.y * _OneOverScreenH, 0.f);
  //
  float qXMin =
      minof(normedQuad.V0.x, normedQuad.V1.x, normedQuad.V2.x, normedQuad.V3.x);
  if (qXMin > _XMax)
    return;
  float qXMax =
      maxof(normedQuad.V0.x, normedQuad.V1.x, normedQuad.V2.x, normedQuad.V3.x);
  if (qXMax < _XMin)
    return;
  float qYMin =
      minof(normedQuad.V0.y, normedQuad.V1.y, normedQuad.V2.y, normedQuad.V3.y);
  if (qYMin > _YMax)
    return;
  float qYMax =
      maxof(normedQuad.V0.y, normedQuad.V1.y, normedQuad.V2.y, normedQuad.V3.y);
  if (qYMax < _YMin)
    return;
  //
  ...
  if (qXMin >= _XMin && qYMin >= _YMin && qXMax <= _XMax && qYMax <= _YMax) {
    ...
    normedQuad.Color0 = normedQuad.Color1 = normedQuad.Color2 =
        normedQuad.Color3 = col;
    ...
    if (!filtered) {
      ...
      layer.FilteredAdditifQuads.push_back(normedQuad);
    else
      layer.FilteredAlphaBlendedQuads.push_back(normedQuad);
    ...
    if (count >= 3) {
      ...
      if (!filtered) {
        ...
        tris = additif ? &layer.FilteredAdditifTris
                       : &layer.FilteredAlphaBlendedTris;
      }
...
void CViewRenderer::drawUnclippedTriangles(
    sint layerId, const std::vector<NLMISC::CTriangle> &tris,
    NLMISC::CRGBA col) {
  if (tris.empty())
    return;
  if (!_BlankGlobalTexture)
    return;
  // primary goal here is batching, so we prefer to draw the triangle with a
  // blank texture rather than switching material and having to flush all
  // primitives .
  layerId += VR_BIAS_LAYER;
  nlassert(layerId >= 0 && layerId < VR_NUM_LAYER);
  ...
  NLMISC::CTriangleColorUV *dest =
      &layer.FilteredAlphaBlendedTris[0] + startCount;
  _EmptyLayer[layerId] = false;
  do {
 ...
 * loadTextures
 */
bool CViewRenderer::loadTextures(const std::string &textureFileName,
                                 const std::string &uvFileName,
                                 bool uploadDXTC) {
  SGlobalTexture gt;
  // Load texture file
  ...
  // Force to generate the texture now. This way we can extract the mouse
  // bitmaps from it now without having to load it again. Its why we don't
  // release it at the end, because it is likely to be uploaded soon)
  ...
  while (!iFile.eof()) {
    ...
    sscanf(
        bufTmp, "%s %f %f %f %f", tgaName, &uvMinU, &uvMinV, &uvMaxU,
        &uvMaxV); // FIXME: Return value ignored, tgaName may be uninitialized
    SImage image;
    ...
    if (getTextureIdFromName(sTGAname) != -1) {
      string tmp = string("duplicate texture name in ") + textureFileName +
                   "(" + sTGAname + ")";
      nlwarning(tmp.c_str());
    } else {
      sint32 textureId = addSImage(image);
      // nlwarning("SIMAGE ADDED: id = %x, name = %s",  textureId,
      // image.Name.c_str());
      //  Insert in map.
      _TextureMap.insert(make_pair(image.Name, textureId));
    }

    // if this is a cursor texture, extract it now (supported for rgba only now,
    // because of the blit)
    if (texDatas && texDatas->getPixelFormat() == CBitmap::RGBA) {
      if (CViewRenderer::hwCursors->count(image.Name) > 0) {
        ...
        if (x1 != x0 && y1 != y0) {
          ...
          if (gt.Scale > 1.f) {
            curs.resample((sint)(curs.getWidth() / gt.Scale),
                          (sint)(curs.getHeight() / gt.Scale));
          }
...
bool CViewRenderer::loadTextureFromString(CViewRenderer::SGlobalTexture *gt,
                                          const std::string &data) {
  size_t pos = data.find(";base64,");
  if (pos == std::string::npos) {
    nlwarning("Data does not have 'data:image/...;base64,...' format '%s'",
              data.c_str());
    return false;
  }
  ...
  if (gt->Width == 0 || gt->Height == 0) {
    nlwarning(
        "Decoded image has width==0 || height==0, check image format. '%s'",
        data.c_str());
    return false;
  }

  UTextureMem *texture =
      driver->createTextureMem(btm.getWidth(), btm.getHeight(), CBitmap::RGBA);
  if (!texture) {
    nlwarning("Failed to create mem texture (%d,%d)", btm.getWidth(),
              btm.getHeight());
    return false;
  }
...
bool CViewRenderer::loadTextureFromFile(CViewRenderer::SGlobalTexture *gt,
  ...
  if (ifTmp.open(filename)) {
    ...
    if (gt->Width == 0 || gt->Height == 0) {
      nlwarning("Failed to load the texture '%s', please check image format",
                filename.c_str());
      return false;
    }
  }
...
void CViewRenderer::reloadTexture(sint32 texId, const std::string &name,
                                  bool uploadDXTC, bool bReleasable) {
  if ((uint)texId >= _SImageIterators.size()) {
    nlwarning("Invalid texture id %d, maximum is %u", texId,
              _SImageIterators.size());
    return;
  }
...
sint32 CViewRenderer::createTexture(const std::string &sGlobalTextureName,
  ...
  if (startsWith(sGlobalTextureName, "data:image/"))
    return createTextureFromDataURL(sGlobalTextureName, uploadDXTC,
                                    bReleasable);
  ...
  iTmp.GlobalTexturePtr = &(*ite);
  iTmp.UVMin =
      CUV(((float)offsetX) / ite->Width, ((float)offsetY) / ite->Height);
  iTmp.UVMax = CUV(((float)offsetX + width) / ite->Width,
                   ((float)offsetY + height) / ite->Height);
...
sint32 CViewRenderer::createTextureFromDataURL(const std::string &data,
                                               bool uploadDXTC,
                                               bool bReleasable) {
  ...
  std::string md5hash =
      getMD5((uint8 *)data.c_str(), (uint32)data.size()).toString();
...
void CViewRenderer::updateTexturePos(const std::string &texturefileName,
                                     sint32 offsetX /*=0*/,
                                     sint32 offsetY /*=0*/,
                                     sint32 width /*=-1*/,
                                     sint32 height /*=-1*/) {
  ...
  im->UVMax =
      CUV(((float)offsetX + width) / gw, ((float)offsetY + height) / gh);
}
...
void CViewRenderer::deleteTexture(sint32 textureId) {
  ...
  if (_SImageIterators[textureId] == _SImages.end()) {
    nlwarning("Can't delete texture with name %s",
              getTextureNameFromId(textureId).c_str());
    nlassert(0);
    return;
  }
  ...
  if (!(gt->FromGlobaleTexture)) {
    ...
    //  Erase the SImage
    // nlwarning("SIMAGE REMOVE : id = %x, name = %s", (int) textureId,
    // getSImage(textureId)->Name.c_str());
    ...
    if (ite == _SImages.end()) {
      // nlwarning("REMOVE GLOBAL TEXTURE : id of simage = %x", (int)
      // textureId);
      //  No, remove the global texture
      for (TGlobalTextureList::iterator iteGT = _GlobalTextures.begin();
           iteGT != _GlobalTextures.end(); iteGT++) {
        // This one ?
        if (&(*iteGT) == gt) {
 ...
 */
void CViewRenderer::getTextureSizeFromId(sint32 id, sint32 &width,
                                         sint32 &height) {
  if ((id < 0) || (id >= (sint32)_SImageIterators.size())) {
    width = height = 0;
  } else {
    SImage &rImage = *getSImage(id);
    width = (sint32)(((rImage.UVMax.U - rImage.UVMin.U) *
                      rImage.GlobalTexturePtr->Width /
                      rImage.GlobalTexturePtr->Scale) +
                     0.5f);
    height = (sint32)(((rImage.UVMax.V - rImage.UVMin.V) *
                       rImage.GlobalTexturePtr->Height /
                       rImage.GlobalTexturePtr->Scale) +
                      0.5f);
  }
}
...
CRGBA CViewRenderer::getTextureColor(sint32 id, sint32 x, sint32 y) {
  ...
  sint32 xConv =
      (sint32)((rImage.UVMin.U + xRatio * (rImage.UVMax.U - rImage.UVMin.U)) *
                   rGT.Width +
               0.5f);
  sint32 yConv =
      (rGT.Height - 1) -
      (sint32)((rImage.UVMin.V + yRatio * (rImage.UVMax.V - rImage.UVMin.V)) *
                   rGT.Height +
               0.5f);
...
// ***************************************************************************
sint32 CViewRenderer::getTypoTextureH(char /* c */) { return _TypoH; }
  ...
  for (uint layerId = 0; layerId < VR_NUM_LAYER; layerId++) {
    ...
    while (ite != _GlobalTextures.end()) {
      ...
      if (layer.NbQuads > 0 || !layer.Tris.empty()) {
        ...
        // Special Case if _WorldSpaceTransformation and _WorldSpaceScale,
        // enable bilinear
        if (_Bilinear || (_WorldSpaceTransformation && _WorldSpaceScale))
          ite->Texture->setFilterMode(UTexture::Linear,
        ...
        if (_Bilinear || (_WorldSpaceTransformation && _WorldSpaceScale))
          ite->Texture->setFilterMode(UTexture::Nearest,
      ...
      if (!layer.FilteredAlphaBlendedQuads.empty() ||
        ...
        // alpha blended
        if (!layer.FilteredAlphaBlendedQuads.empty()) {
          driver->drawQuads(&(layer.FilteredAlphaBlendedQuads[0]),
                            (uint32)layer.FilteredAlphaBlendedQuads.size(),
                            _Material);
        ...
        if (!layer.FilteredAdditifQuads.empty() ||
          ...
          if (!layer.FilteredAdditifQuads.empty()) {
            driver->drawQuads(&(layer.FilteredAdditifQuads[0]),
                              (uint32)layer.FilteredAdditifQuads.size(),
                              _Material);
    ...
    if (_WorldSpaceTransformation)
      textcontext->flushRenderBufferUnProjected(_StringRBLayers[layerId],
                                                false);
    else
...
void CViewRenderer::initIndexesToTextureIds() {
  ...
  if (_IndexesToTextureIds[0] != -1) {
    getTextureSizeFromId(_IndexesToTextureIds[0], _WFigurTexture,
                         _HFigurTexture);
  }
  if (_FigurSeparatorTextureId != -1) {
    getTextureSizeFromId(_FigurSeparatorTextureId, _WFigurSeparatorTexture,
...
void CViewRenderer::initTypo() {
  ...
  // since filename dose not support special char (?,. ....), specify a map from
  // char to string.
...
bool CViewRenderer::needClipping(const CQuad &q) {
  if ((q.V0.x >= _XMin) && (q.V0.y >= _YMin) && (q.V2.x <= _XMax) &&
      (q.V2.y <= _YMax))
    return false;
  else
    return true;
 ...
 * clip
 */
void CViewRenderer::clip(CQuadColorUV &qout, const CQuadColorUV &qin,
                         uint rot) {
  float ratio;
 ...
 */
void CViewRenderer::putQuadInLayer(SGlobalTexture &gt, sint layerId,
                                   const NLMISC::CQuadColorUV &qcoluv,
                                   uint rot) {
  layerId += VR_BIAS_LAYER;
  nlassert(layerId >= 0 && layerId < VR_NUM_LAYER);
...
void CViewRenderer::addSystemTexture(TSystemTexture e, const char *s) {
  ...
  if (_SystemTextures[e].Id != -1) {
    getTextureSizeFromId(_SystemTextures[e].Id, _SystemTextures[e].W,
                         _SystemTextures[e].H);
  }
}
...
// ***************************************************************************
void CViewRenderer::drawWiredQuad(sint32 x, sint32 y, sint32 width,
                                  sint32 height,
                                  NLMISC::CRGBA col /*=NLMISC::CRGBA::White*/) {
  driver->drawWiredQuad(x * _OneOverScreenW, y * _OneOverScreenH,
                        (x + width) * _OneOverScreenW,
                        (y + height) * _OneOverScreenH, col);
}

// ***************************************************************************
void CViewRenderer::drawFilledQuad(
    sint32 x, sint32 y, sint32 width, sint32 height,
    NLMISC::CRGBA col /*=NLMISC::CRGBA::White*/) {
  driver->drawQuad(x * _OneOverScreenW, y * _OneOverScreenH,
                   (x + width) * _OneOverScreenW,
                   (y + height) * _OneOverScreenH, col);
}
...
// ***************************************************************************
void CViewRenderer::drawCustom(sint32 x, sint32 y, sint32 width, sint32 height,
                               const NLMISC::CUV &uv0Min,
                               const NLMISC::CUV &uv0Max, NLMISC::CRGBA col,
                               NL3D::UMaterial Mat) {
  float dstXmin, dstYmin, dstXmax, dstYmax;
  ...
  qcoluv.Uv0.U = uv0Min.U;
  qcoluv.Uv0.V = uv0Max.V;
  qcoluv.Uv1.U = uv0Max.U;
  qcoluv.Uv1.V = uv0Max.V;
  qcoluv.Uv2.U = uv0Max.U;
  qcoluv.Uv2.V = uv0Min.V;
  qcoluv.Uv3.U = uv0Min.U;
  qcoluv.Uv3.V = uv0Min.V;
...
// ***************************************************************************
void CViewRenderer::drawCustom(sint32 x, sint32 y, sint32 width, sint32 height,
                               const CUV &uv0Min, const CUV &uv0Max,
                               const CUV &uv1Min, const CUV &uv1Max,
  ...
  qcoluv2.Uv0.U = uv0Min.U;
  qcoluv2.Uv0.V = uv0Max.V;
  qcoluv2.Uv1.U = uv0Max.U;
  qcoluv2.Uv1.V = uv0Max.V;
  qcoluv2.Uv2.U = uv0Max.U;
  qcoluv2.Uv2.V = uv0Min.V;
  qcoluv2.Uv3.U = uv0Min.U;
  qcoluv2.Uv3.V = uv0Min.V;

  qcoluv2.Uv02.U = uv1Min.U;
  qcoluv2.Uv02.V = uv1Max.V;
  qcoluv2.Uv12.U = uv1Max.U;
  qcoluv2.Uv12.V = uv1Max.V;
  qcoluv2.Uv22.U = uv1Max.U;
  qcoluv2.Uv22.V = uv1Min.V;
  qcoluv2.Uv32.U = uv1Min.U;
  qcoluv2.Uv32.V = uv1Min.V;
...
bool CViewRenderer::CTextureId::setTexture(const char *textureName,
                                           sint32 offsetX, sint32 offsetY,
                                           sint32 width, sint32 height,
                                           bool uploadDXTC, bool bReleasable) {
  ...
  if (_TextureId < 0)
    _TextureId = rVR.createTexture(textureName, offsetX, offsetY, width, height,
                                   uploadDXTC, bReleasable);
...
void CViewRenderer::drawText(sint layerId, float x, float y, uint wordIndex,
  ...
  if (_WorldSpaceTransformation) {
    textContext.printClipAtUnProjected(
        *getStringRenderBuffer(layerId), _CameraFrustum, _WorldSpaceMatrix, x,
        y, _CurrentZ, wordIndex, xmin, ymin, xmax, ymax);
  } else {
    textContext.printClipAt(*getStringRenderBuffer(layerId), x, y, wordIndex,
                            xmin, ymin, xmax, ymax);
  }
...
void CViewRenderer::setInterfaceDepth(const NLMISC::CVector &projCenter,
                                      float scale) {

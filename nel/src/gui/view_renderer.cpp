    ...
    sint32 nTxId, uint tileOrigin, const CRGBA &col)
{
	...
	// Is totally clipped ?
	if ((x > (_ClipX + _ClipW)) || ((x + width) < _ClipX) || (y > (_ClipY + _ClipH)) || ((y + height) < _ClipY))
		return;
	...
	for (uint py = 0; py <= numTileY; ++py)
	{
		...
		for (uint px = 0; px <= numTileX; ++px)
		{
			...
			if (!((qcoluv.V0.x > _XMax) || (qcoluv.V2.x < _XMin) || (qcoluv.V0.y > _YMax) || (qcoluv.V2.y < _YMin)))
...
void CViewRenderer::flush()
{
	...
	for (uint layerId = 0; layerId < VR_NUM_LAYER; layerId++)
	{
		...
		while (ite != _GlobalTextures.end())
		{
			...
			if (!layer.FilteredAlphaBlendedQuads.empty() || !layer.FilteredAlphaBlendedTris.empty() || !layer.FilteredAdditifQuads.empty() || !layer.FilteredAdditifTris.empty())
			{
				// setup the global texture to material
				_Material.setTexture(0, ite->Texture);
				...
				// additif
				if (!layer.FilteredAdditifQuads.empty() || !layer.FilteredAdditifTris.empty())
				{

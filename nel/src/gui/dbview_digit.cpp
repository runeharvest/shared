...
// ***************************************************************************
CDBViewDigit::CDBViewDigit(const TCtorParam &param) : CViewBase(param) {
  _NumDigit = 2;
  _WSpace = -1;
  _DivBase = 1;
}
...
void CDBViewDigit::setProperty(const std::string &name,
...
xmlNodePtr CDBViewDigit::serialize(xmlNodePtr parentNode,
...
bool CDBViewDigit::parse(xmlNodePtr cur, CInterfaceGroup *parentGroup) {
  ...
  if (ptr)
    fromString((const char *)ptr, _NumDigit);
  clamp(_NumDigit, 1, 10);

  ptr = xmlGetProp(cur, (xmlChar *)"wspace");
  if (ptr)
    fromString((const char *)ptr, _WSpace);
...
// ***************************************************************************
void CDBViewDigit::updateCoords() { CViewBase::updateCoords(); }
...
void CDBViewDigit::draw() {
  ...
  sint32 y = _YReal;
  for (sint i = 0; i < _NumDigit; i++) {
    rVR.drawRotFlipBitmap(_RenderLayer, x, y, wDigit, hDigit, 0, false,
                          _DigitId[i], _Color);
    // next digit
    x += wDigit + _WSpace;
  }

...
bool CGroupHTML::isBrowsing()
{
	...
	return _BrowseNextTime || _PostNextTime || _RenderNextTime || _Browsing || _WaitingForStylesheet || _CurlWWW;
}
...
bool CGroupHTML::translateChar(u32char &output, u32char input, u32char lastCharParam) const
{
	...
	case '\n': {
		...
		else
		{
			...
			keep = ((lastChar != (u32char)' ') && (lastChar != 0)) || getPRE() || (_CurrentViewImage && (lastChar == 0));
			if (!getPRE())
				input = (u32char)' ';
		}
	}
...
bool CGroupHTML::isSameStyle(CViewLink *text, const CStyleParams &style) const
{
	...
	return sameShadow && (style.TextColor == text->getColor()) && (style.FontFamily == text->getFontName()) && (style.FontSize == (uint)text->getFontSize()) && (style.Underlined == text->getUnderlined()) && (style.StrikeThrough == text->getStrikeThrough()) && (embolden == text->getEmbolden()) && (style.FontOblique == text->getOblique()) && (getLink() == text->Link) && (style.GlobalColorText == text->getModulateGlobalColor());
}
		...
		if (str[i] == '&' && (str.length() - i) >= 4)
		{
			...
			if (str[pos] == '#')
			{
				...
				for (last = pos; last < str.length(); ++last)
					if (!isDigit(str[last], base)) break;

...
namespace NLGUI {
CCssSelector::CCssSelector(std::string elm, std::string id, std::string cls,
                           char comb)
...
uint32 CCssSelector::specificity() const {
  ...
  if (!Class.empty())
    ret += 0x000100 * Class.size();
  if (!Attr.empty())
    ret += 0x000100 * Attr.size();
  // TODO: has different cases
  if (!PseudoClass.empty())
    ret += 0x000100 * PseudoClass.size();
...
void CCssSelector::addAttribute(const std::string &key, const std::string &val,
                                char op, bool cs) {
  if (cs) {
...
bool CCssSelector::matchAttributes(const CHtmlElement &elm) const {
  ...
  for (uint i = 0; i < Attr.size(); ++i) {
    if (!elm.hasAttribute(Attr[i].key))
      return false;
    ...
    case '|':
      ...
      if (value != Attr[i].value &&
          value.find(Attr[i].value + "-") == std::string::npos)
        return false;
      break;
    case '^':
      // prefix, starts with
      if (Attr[i].value.empty())
        return false;
      if (value.find(Attr[i].value) != 0)
        return false;
      break;
    case '$':
      // suffic, ends with
      if (Attr[i].value.empty() || value.size() < Attr[i].value.size())
        return false;
      if (Attr[i].value == value.substr(value.size() - Attr[i].value.size()))
        return false;
      break;
    case '*':
      if (Attr[i].value.empty())
        return false;
      if (value.find(Attr[i].value) == std::string::npos)
        return false;
      break;
    case ' ':
...
bool CCssSelector::matchPseudoClass(const CHtmlElement &elm) const {
  for (uint i = 0; i < PseudoClass.size(); i++) {
    if (PseudoClass[i] == "root") {
      ...
      if (elm.parent && !elm.parent->Children.empty())
        return false;
    } else {
...
bool CCssSelector::matchLang(const CHtmlElement &elm,

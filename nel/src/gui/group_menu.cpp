...
void CGroupSubMenu::checkCoords()
{
	...
	if (!((xMouse >= _XReal) && (xMouse < (_XReal + _WReal)) && (yMouse > _YReal) && (yMouse <= (_YReal + _HReal))))
		_MouseOver = -1;
			...
			if (_Lines[i].Selectable)
			{
				...
				if (refElm)
				{
					if (refElm->getActive() == true)
						if ((eventDesc.getY() > refElmYReal) && (eventDesc.getY() <= (refElmYReal + refElmHReal + _GroupList->getSpace())))
						{
							_MouseOver = i;
							break;
						}

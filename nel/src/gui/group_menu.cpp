...
// ------------------------------------------------------------------------------------------------
bool CViewTextMenu::getGrayed() const { return _Grayed; }

// ------------------------------------------------------------------------------------------------
void CViewTextMenu::setGrayed(bool g) { _Grayed = g; }
...
void CViewTextMenu::setChecked(bool c) {
  if (_CheckBox) {
    ...
    if (!pIO)
      return;
    _CheckBox->setTexture(
        pIO->getValStr(c ? "checked_bitmap" : "unchecked_bitmap"));
  }
  _Checked = c;
}
...
// ------------------------------------------------------------------------------------------------
CGroupSubMenu::~CGroupSubMenu() { removeAllUserGroups(); }
...
bool CGroupSubMenu::parse(xmlNodePtr cur, CInterfaceGroup *parent) {
  ...
  while (cur) {
    ...
    if (stricmp((char *)cur->name, "separator") == 0) {
    ...
    } else if (stricmp((char *)cur->name, "action") == 0) {
      ...
      if (cond)
        strCond = (const char *)cond;
      CXMLAutoPtr params((const char *)xmlGetProp(cur, (xmlChar *)"params"));
      if (params)
        strParams = (const char *)params;
      CXMLAutoPtr strCheckable(
          (const char *)xmlGetProp(cur, (xmlChar *)"checkable"));
      bool bCheckable = false;
      if (strCheckable)
        bCheckable = convertBool(strCheckable);
      CXMLAutoPtr strChecked(
          (const char *)xmlGetProp(cur, (xmlChar *)"checked"));
      bool bChecked = false;
      if (strChecked)
        bChecked = convertBool(strChecked);
      bool bFormatted = false;
      CXMLAutoPtr strFormatted(
          (const char *)xmlGetProp(cur, (xmlChar *)"formatted"));
      if (strFormatted)
        bFormatted = convertBool(strFormatted);

      pV = addLine(ucstrName, strAh, strParams, strId, strCond, strTexture,
                   bCheckable, bChecked, bFormatted);
      pV->setSerializable(false);

      CXMLAutoPtr strSelectable(
          (const char *)xmlGetProp(cur, (xmlChar *)"selectable"));
      bool bSelectable = true;
      if (strSelectable)
        bSelectable = convertBool(strSelectable);
      _Lines.back().Selectable = bSelectable;

      CXMLAutoPtr grayed((const char *)xmlGetProp(cur, (xmlChar *)"grayed"));
      bool bGrayed = false;
      if (grayed)
        bGrayed = convertBool(grayed);
      ...
      if (child != NULL) {
        if (_Lines.back().CheckBox) {
          _Lines.back().RightArrow =
              createRightArrow(_Lines.back().CheckBox, true);
        } else {
        ...
        CXMLAutoPtr MVL(
            (const char *)xmlGetProp(cur, (xmlChar *)"max_visible_line"));
        if (MVL) {
      ...
      // Add user groups
      // Left
      CXMLAutoPtr usergroup(
          (const char *)xmlGetProp(cur, (xmlChar *)"usergroup_l"));
      if (usergroup) {
        vector<pair<string, string>> vparams;
        CXMLAutoPtr ugparams(
            (const char *)xmlGetProp(cur, (xmlChar *)"usergroup_params_l"));
        if (ugparams) {
          IActionHandler::getAllParams((const char *)ugparams, vparams);
        }

        string completeId =
            _Parent->getId() + ":" + _Lines[_Lines.size() - 1].Id;
        CInterfaceGroup *pUGLeft =
            CWidgetManager::getInstance()->getParser()->createGroupInstance(
                (const char *)usergroup, completeId, vparams);
        if (pUGLeft)
      ...
      if (usergroup) {
        vector<pair<string, string>> vparams;
        CXMLAutoPtr ugparams(
            (const char *)xmlGetProp(cur, (xmlChar *)"usergroup_params_r"));
        if (ugparams) {
          IActionHandler::getAllParams((const char *)ugparams, vparams);
        }
...
// ------------------------------------------------------------------------------------------------
CViewBitmap *CGroupSubMenu::createIcon(CInterfaceElement *parentPos,
...
CViewBitmap *CGroupSubMenu::createCheckBox(bool checked) {
  ...
  if (pIO) {
    pVB->setTexture(
        pIO->getValStr(checked ? "checked_bitmap" : "unchecked_bitmap"));
  }
...
// ------------------------------------------------------------------------------------------------
CViewBitmap *CGroupSubMenu::createRightArrow(CInterfaceElement *parentPos,
                                             bool center) {
  // Put the left arrow to the line
  CViewBitmap *pVB = new CViewBitmap(CViewBase::TCtorParam());
  pVB->setSerializable(false);
  ...
  pVB->setX(MENU_WIDGET_X);
  pVB->setId(
      ID_MENU_SUBMENU); // rescale to parent in update coords if asked (not
                        // needed if there's already on the left a checkbox)
  addView(pVB);
  return pVB;
}
...
void CGroupSubMenu::updateCoords() {
  ...
  // The sub menu may go outside the screen in Y. => clamp it as possible
  {
    /* X/Y coords have normally been updated before by "parent" sub menus
        Why? because setSubMenu() is typically called in "parent first" order
       (or it is exactly what is done in ::parse())
        => Parent CGroupSubMenu::updateCoords() are called before their sons in
       CGroupMenu::updateCoords() !!!
        => No Need to call _SubMenus[RALineNb]->updateCoords() below !  (else
       would call too much time because of recursion!!)
    */
    ...
    if (hReal <= (sint32)screenH) {
      ...
      // compute the shift to apply to the Y of the sub menu, to respect (as
      // possible) the screen
      sint32 dyClamp = 0;
      if (yReal < 0)
        dyClamp = -yReal;
  ...
  // Set the arrows at the right positions (in _Views we have selection and the
  // right arrows)
  ...
  for (k = 0; k < _Lines.size(); ++k) {
    if (_Lines[k].UserGroupLeft) {
      ...
      maxLeftUGWidth =
          std::max(_Lines[k].UserGroupLeft->getWReal(), maxLeftUGWidth);
    }
  }
  ...
  for (i = 0; i < _Lines.size(); ++i) {
    if (_Lines[i].Separator != NULL) {
      ...
      if (_Lines[i].ViewText && _Lines[i].ViewText->getActive())
        activeLineSeen = true;
    }
  }
  ...
  if (_MaxVisibleLine > 0 && sint32(_Lines.size()) > _MaxVisibleLine) {
    for (k = 0; k < _Lines.size(); ++k)
      if (_Lines[k].ViewText) {
        ...
        sint32 widgetMaxH = 0;
        if (_Lines[k].UserGroupRight)
          widgetMaxH = _Lines[k].UserGroupRight->getHReal();
        if (_Lines[k].UserGroupLeft)
          widgetMaxH =
              std::max(widgetMaxH, _Lines[k].UserGroupLeft->getHReal());
        if (_Lines[k].CheckBox)
          widgetMaxH = std::max(widgetMaxH, _Lines[k].CheckBox->getHReal());
        if (_Lines[k].RightArrow)
          widgetMaxH = std::max(widgetMaxH, _Lines[k].RightArrow->getHReal());
        widgetMaxH = std::max(widgetMaxH, _Lines[k].ViewText->getHReal());
        _GroupList->setMaxH(widgetMaxH * _MaxVisibleLine +
                            _GroupList->getSpace() * (_MaxVisibleLine - 1));
        if (_ScrollBar == NULL) {
          _ScrollToView = true;
  ...
  for (k = 0; k < _Lines.size(); ++k) {
    ...
    if (_Lines[k].ViewText) {
      ...
      if (_Lines[k].UserGroupRight)
        widgetMaxH = _Lines[k].UserGroupRight->getHReal();
      if (_Lines[k].UserGroupLeft)
        widgetMaxH = std::max(widgetMaxH, _Lines[k].UserGroupLeft->getHReal());
      if (_Lines[k].CheckBox)
        widgetMaxH = std::max(widgetMaxH, _Lines[k].CheckBox->getHReal());
      if (_Lines[k].RightArrow)
        widgetMaxH = std::max(widgetMaxH, _Lines[k].RightArrow->getHReal());
  ...
  for (i = 1; i < _Views.size(); ++i) {
    ...
    if (pVB->getId() == ID_MENU_SUBMENU) {
      ...
      for (;;) {
        ...
        if (_SubMenus[RALineNb] !=
            NULL) // has a check box or an arrow to indicate submenu ?
        {
          break;
        }
        ++RALineNb;
      ...
      if (_Lines[RALineNb].CheckBox || _Lines[RALineNb].UserGroupRight) {
        ...
        if (_GroupList->getMaxH() >= limY && limY >= 0) {
          pVB->setY(refElmYReal + (refElmHReal - pVB->getHReal()) / 2 -
                    _GroupList->getYReal());
      ...
      if (_GroupMenu->SpawnOnMousePos) {
        ...
        // According to mouse position, set the sub menu on the left or right,
        // begin at top or bottom
        CViewRenderer &rVR = *CViewRenderer::getInstance();
        uint32 screenW, screenH;
        rVR.getScreenSize(screenW, screenH);
        if ((_GroupMenu->SpawnMouseX <= ((sint32)screenW / 2)) &&
            (_GroupMenu->SpawnMouseY <= ((sint32)screenH / 2))) {
        ...
        if ((_GroupMenu->SpawnMouseX <= ((sint32)screenW / 2)) &&
          ...
          _SubMenus[RALineNb]->setY(refElmHReal +
                                    (refElmYReal - _GroupList->getYReal()) -
                                    _GroupList->getHReal());
        }
        if ((_GroupMenu->SpawnMouseX > ((sint32)screenW / 2)) &&
            (_GroupMenu->SpawnMouseY <= ((sint32)screenH / 2))) {
        ...
        _SubMenus[RALineNb]->setParentPosRef(Hotspot_BR);
        _SubMenus[RALineNb]->setPosRef(Hotspot_BL);
        _SubMenus[RALineNb]->setY(16 + refElmYReal - _GroupList->getYReal() -
                                  _SubMenus[RALineNb]->getHReal());
      }
    ...
    if (maxViewW < (pVB->getWReal() + pVB->getX()))
      maxViewW = pVB->getWReal() + pVB->getX();
  }
    ...
    if (igr || igl) {
      ...
      if (refElm) {
        if (igr) {
          ...
          sint32 limY = refElmYReal + refElmHReal / 2 - _GroupList->getYReal();
          if (_GroupList->getMaxH() >= limY && limY >= 0) {
            igr->setY(refElmYReal + (refElmHReal - igr->getHReal()) / 2 -
                      _GroupList->getYReal());
        ...
        if (igl) {
          sint32 limY = refElmYReal + refElmHReal / 2 - _GroupList->getYReal();
          if (_GroupList->getMaxH() >= limY && limY >= 0) {
            igl->setY(refElmYReal + (refElmHReal - igl->getHReal()) / 2 -
                      this->getYReal());
  ...
  for (i = 0; i < _ChildrenGroups.size(); ++i) {
    ...
    if (pIG == NULL)
      continue;
    if (pIG->getId() != ID_MENU_SEPARATOR)
  ...
  if (_ScrollBar && _ScrollToView) {
    ...
    if (_Selected != -1 && _Lines[_Selected].ViewText != NULL)
      _ScrollBar->ensureVisible(_Lines[_Selected].ViewText, Hotspot_Tx,
                                Hotspot_Mx);
    else
...
void CGroupSubMenu::checkCoords() {
  ...
  // if the mouse goes out the window,  unselect all (because handleEvent may
  // not be called)
  sint xMouse = CWidgetManager::getInstance()->getPointer()->getX();
  sint yMouse = CWidgetManager::getInstance()->getPointer()->getY();
  if (!((xMouse >= _XReal) && (xMouse < (_XReal + _WReal)) &&
        (yMouse > _YReal) && (yMouse <= (_YReal + _HReal))))
    _MouseOver = -1;
  ...
  for (sint32 i = 0; i < (sint32)_Lines.size(); ++i) {
    if (_Lines[i].ViewText) {
      if (_Lines[i].ViewText->getGrayed()) // Colors when the text is grayed
      {
        ...
        _Lines[i].ViewText->setShadowColor(
            _Lines[i].ViewText->OldShadowColorGrayed);
      } else {
...
bool CGroupSubMenu::handleEvent(const NLGUI::CEventDescriptor &event) {
  ...
  if (event.getType() == NLGUI::CEventDescriptor::mouse) {
        ...
        if (refElm) {
          if (refElm->getActive() == true)
            if ((eventDesc.getY() > refElmYReal) &&
                (eventDesc.getY() <=
                 (refElmYReal + refElmHReal + _GroupList->getSpace()))) {
              _MouseOver = i;
              break;
            }
        }
    ...
    if (eventDesc.getEventTypeExtended() ==
      ...
      // If a line is selected and the line is not grayed and has right click
      // action handler
      if ((_MouseOver != -1) && (!_Lines[i].ViewText->getGrayed()) &&
          !_Lines[_MouseOver].AHRightClick.empty()) {
        CAHManager::getInstance()->runActionHandler(
            _Lines[_MouseOver].AHRightClick,
            CWidgetManager::getInstance()->getCtrlLaunchingModal(),
          ...
          if (isIn(eventDesc.getX(), eventDesc.getY())) {
            ...
            for (uint32 k = 0; k < _Lines.size(); ++k)
              if (_Lines[k].ViewText) {
                ...
                if (_Lines[k].CheckBox)
                  widgetMaxH =
                      std::max(widgetMaxH, _Lines[k].CheckBox->getHReal());
                if (_Lines[k].RightArrow)
                  widgetMaxH =
                      std::max(widgetMaxH, _Lines[k].RightArrow->getHReal());
                widgetMaxH =
                    std::max(widgetMaxH, _Lines[k].ViewText->getHReal());
    ...
    } else // no a lot of token left
    {
      ...
      if (pCurGSM->_SubMenus[i] == NULL) {
        ...
        if (pCurGSM->_Lines[i].UserGroupRight) {
          ...
          if (sUGid == sTok) {
            CInterfaceElement *pIE =
                pCurGSM->_Lines[i].UserGroupRight->getElement(id);
            return pIE;
          }
        }
        if (pCurGSM->_Lines[i].UserGroupLeft) {
          ...
          if (sUGid == sTok) {
            CInterfaceElement *pIE =
                pCurGSM->_Lines[i].UserGroupLeft->getElement(id);
            return pIE;
          }
        }
...
void CGroupSubMenu::addSeparatorAtIndex(uint index, const std::string &id) {
  ...
  // create the real separator. It may be larger than the group list, this is
  // why we create a separate group
  CInterfaceGroup *separator =
      CWidgetManager::getInstance()->getParser()->createGroupInstance(
          "menu_separator", "", NULL, 0);
  if (!separator)
    return;
...
// ------------------------------------------------------------------------------------------------
CViewTextMenu *
CGroupSubMenu::addLine(const std::string &name, const std::string &ah,
...
CViewTextMenu *CGroupSubMenu::addLineAtIndex(
    uint index, const std::string &name, const std::string &ah,
    const std::string &params, const std::string &id /*=""*/,
...
void CGroupSubMenu::removeLine(uint index) {
  ...
  if (_Lines[index].RightArrow)
    delView(_Lines[index].RightArrow);
  if (_Lines[index].CheckBox)
    delView(_Lines[index].CheckBox);
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setUserGroupRight(uint line, CInterfaceGroup *group,
                                      bool ownership) {
  if (line >= _Lines.size()) {
    nlwarning("bad index");
    return;
  ...
  if (_Lines[line].UserGroupRight) {
    delGroup(_Lines[line].UserGroupRight,
             !_Lines[line].UserGroupRightOwnership);
  }
  _Lines[line].UserGroupRight = group;
  _Lines[line].UserGroupRightOwnership = ownership;
  if (group) {
    CViewBase *prevElem =
        _Lines[line].CheckBox ? _Lines[line].CheckBox : _Lines[line].RightArrow;
    if (prevElem) {
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setUserGroupLeft(uint line, CInterfaceGroup *group,
                                     bool ownership) {
  if (line >= _Lines.size()) {
    nlwarning("bad index");
    return;
...
// ------------------------------------------------------------------------------------------------
CGroupSubMenu *
CGroupSubMenu::cloneMenu(CGroupSubMenu *appendToMenu, CGroupMenu *newFather,
                         CInterfaceGroup *initGroup /* = NULL */) const {
  CGroupSubMenu *copyMenu =
      appendToMenu ? appendToMenu : new CGroupSubMenu(CViewText::TCtorParam());
  ...
  for (uint k = 0; k < _Lines.size(); ++k) {
    if (_Lines[k].Separator) {
      ...
      CViewTextMenu *pV = NULL;
      pV = copyMenu->addLine(_Lines[k].ViewText->getText(), _Lines[k].AHName,
                             _Lines[k].AHParams, _Lines[k].Id, _Lines[k].Cond,
                             texture, _Lines[k].ViewText->getCheckable(),
                             _Lines[k].ViewText->getChecked(),
                             _Lines[k].ViewText->getFormatted());
    ...
    if (_SubMenus[k] != NULL) {
      ...
      if (copyMenu->_Lines.back().CheckBox) {
        copyMenu->_Lines.back().RightArrow =
            copyMenu->createRightArrow(copyMenu->_Lines.back().CheckBox, true);
      } else {
        copyMenu->_Lines.back().RightArrow =
            copyMenu->createRightArrow(copyMenu->_GroupList, false);
      }

      // and create the sub menu
      copyMenu->_SubMenus[k + startSize] =
          _SubMenus[k]->cloneMenu(NULL, newFather, copyMenu);
    }
  }
  if (!appendToMenu) {
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setActionHandlerParam(uint lineIndex,
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setRightClickHandler(uint lineIndex,
                                         const std::string &ah) {
  if (lineIndex > _Lines.size()) {
    nlwarning("Bad index");
    return;
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setRightClickHandlerParam(uint lineIndex,
                                              const std::string &params) {
  if (lineIndex > _Lines.size()) {
    nlwarning("Bad index");
    return;
...
// ------------------------------------------------------------------------------------------------
void CGroupSubMenu::setMaxVisibleLine(sint32 mvl) { _MaxVisibleLine = mvl; }
...
int CGroupSubMenu::luaAddLineAtIndex(CLuaState &ls) {
  ...
  addLineAtIndex((uint)ls.toInteger(1), arg2.toUtf8(), ls.toString(3),
                 ls.toString(4), ls.toString(5));
#else
  addLineAtIndex((uint)ls.toInteger(1), ls.toString(2), ls.toString(3),
                 ls.toString(4), ls.toString(5));
#endif
  return 0;
}
...
// ------------------------------------------------------------------------------------------------
CGroupMenu::CGroupMenu(const TCtorParam &param) : CGroupModal(param) {
  _Active = false;
  _Color = CRGBA::White;
...
std::string CGroupMenu::getProperty(const std::string &name) const {
  ...
  } else if (name == "color_over") {
    return toString(_ColorOver);
  } else if (name == "shadow_color_over") {
    return toString(_ShadowColorOver);
  } else if (name == "highlight_over") {
    return toString(_HighLightOver);
  } else if (name == "color_grayed") {
    return toString(_ColorGrayed);
  } else if (name == "shadow_color_grayed") {
...
void CGroupMenu::setProperty(const std::string &name,
...
xmlNodePtr CGroupMenu::serialize(xmlNodePtr parentNode,
  ...
  xmlSetProp(node, BAD_CAST "color_over",
             BAD_CAST toString(_ColorOver).c_str());
  xmlSetProp(node, BAD_CAST "shadow_color_over",
             BAD_CAST toString(_ShadowColorOver).c_str());
  xmlSetProp(node, BAD_CAST "highlight_over",
             BAD_CAST toString(_HighLightOver).c_str());
  xmlSetProp(node, BAD_CAST "color_grayed",
             BAD_CAST toString(_ColorGrayed).c_str());
  xmlSetProp(node, BAD_CAST "shadow_color_grayed",
             BAD_CAST toString(_ShadowColorGrayed).c_str());
...
bool CGroupMenu::parse(xmlNodePtr in, CInterfaceGroup *parentGroup) {
  ...
  if (prop) {
    ...
    CGroupMenu *gm = dynamic_cast<CGroupMenu *>(
        CWidgetManager::getInstance()->getElementFromId(prop.str()));
    if (!gm) {
      gm = dynamic_cast<CGroupMenu *>(
          CWidgetManager::getInstance()->getElementFromId(
              "ui:interface:" + std::string((const char *)prop)));
    }
    if (gm) {
...
void CGroupMenu::draw() {
  ...
  // false,  rVR.getBlankTextureId(),  CRGBA(255, 0, 0, 255) );
...
// ------------------------------------------------------------------------------------------------
void CGroupMenu::addLine(const std::string &name, const std::string &ah,
                         const std::string &params,
                         const std::string &id /* = std::string()*/,
                         const std::string &cond /*= std::string()*/,
                         const std::string &texture, bool checkable /*= false*/,
                         bool checked /*= false*/
) {
  if (_RootMenu == NULL) {
    _RootMenu = new CGroupSubMenu(CViewText::TCtorParam());
  ...
  _RootMenu->addLine(name, ah, params, id, cond, texture, checkable, checked,
                     _Formatted);
}
// ------------------------------------------------------------------------------------------------
void CGroupMenu::addLineAtIndex(
    uint index, const std::string &name, const std::string &ah,
  ...
  _RootMenu->addLineAtIndex(index, name, ah, params, id, cond, texture,
                            checkable, checked, _Formatted);
}
...
void CGroupMenu::setMinW(sint32 minW) {
  if (_RootMenu) {
    _RootMenu->_GroupList->setMinW(minW -
                                   _RootMenu->getResizeFromChildWMargin());
...
void CGroupMenu::setMinH(sint32 minH) {
  if (_RootMenu) {
    _RootMenu->_GroupList->setMinH(minH -
                                   _RootMenu->getResizeFromChildHMargin());
...
// ------------------------------------------------------------------------------------------------
void CGroupMenu::setActionHandlerParam(uint lineIndex,
                                       const std::string &params) {
  if (_RootMenu)
...
// ------------------------------------------------------------------------------------------------
void CGroupMenu::setRightClickHandlerParam(uint lineIndex,
                                           const std::string &params) {
  if (_RootMenu)
...
// ------------------------------------------------------------------------------------------------
void CGroupMenu::setUserGroupRight(uint line, CInterfaceGroup *gr,
                                   bool ownerShip /*=true*/) {
  if (_RootMenu) {
...
// ------------------------------------------------------------------------------------------------
void CGroupMenu::setUserGroupLeft(uint line, CInterfaceGroup *gr,
                                  bool ownerShip /*=true*/) {
  if (_RootMenu) {

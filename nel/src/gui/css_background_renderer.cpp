...
CSSBackgroundRenderer::CSSBackgroundRenderer()
    : CurrentAlpha(255), TextureId(-1), m_BorderX(0), m_BorderY(0),
      m_BorderW(0), m_BorderH(0), m_PaddingX(0), m_PaddingY(0), m_PaddingW(0),
      m_PaddingH(0), m_ContentX(0), m_ContentY(0), m_ContentW(0), m_ContentH(0),
...
void CSSBackgroundRenderer::draw() {
  ...
  for (uint i = 0; i < m_DrawQueue.size(); ++i) {
    ...
    rVR.drawQuad(m_RenderLayer, m_DrawQueue[i].Quad, m_DrawQueue[i].TextureId,
                 color, false);
  }
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::getPositioningArea(const CSSBackground &bg,
                                               sint32 &areaX, sint32 &areaY,
                                               sint32 &areaW,
                                               sint32 &areaH) const {
  switch (bg.origin) {
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::getPaintingArea(const CSSBackground &bg,
                                            sint32 &areaX, sint32 &areaY,
                                            sint32 &areaW,
                                            sint32 &areaH) const {
  switch (bg.clip) {
  ...
  if (m_FillViewport && m_Viewport) {
    ...
    areaW = std::max(areaX + areaW,
                     m_Viewport->getXReal() + m_Viewport->getWReal()) -
            newX;
    areaH = std::max(areaY + areaH,
                     m_Viewport->getYReal() + m_Viewport->getHReal()) -
            newY;
    areaX = newX;
    areaY = newY;
  }
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::calculateSize(const CSSBackground &bg, sint32 &texW,
                                          sint32 &texH) const {
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::calculatePosition(const CSSBackground &bg,
                                              sint32 &texX, sint32 &texY,
                                              sint32 &texW,
                                              sint32 &texH) const {
  sint32 areaX, areaY, areaW, areaH;
  getPositioningArea(bg, areaX, areaY, areaW, areaH);
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::getImageTile(sint32 &tilePos, sint32 &tileSize,
                                         sint32 &spacing, sint32 &tiles,
                                         sint32 areaPos, sint32 areaSize,
                                         CSSValueType repeat) const {
  switch (repeat) {
  ...
  default: {
    ...
    tiles =
        std::ceil((std::abs(areaPos - tilePos) + areaSize) / (float)tileSize);
    spacing = 0;
    break;
  }
  }
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::calculateTiles(const CSSBackground &bg,
                                           sint32 &texX, sint32 &texY,
                                           sint32 &texW, sint32 &texH,
                                           sint32 &tilesX, sint32 &tilesY,
                                           sint32 &spacingX,
                                           sint32 &spacingY) const {
  sint32 areaX, areaY, areaW, areaH;
  getPositioningArea(bg, areaX, areaY, areaW, areaH);

  // texX,texY is for position area (ie content-box), but painting area can be
  // bigger (ie border-box)
  ...
  if ((paintX + paintW) > (areaX + areaW))
    areaW +=
        std::ceil(((paintX + paintW) - (areaX + areaW)) / (float)texW) * texW;
  if (paintY < areaY)
    areaY -= std::ceil((areaY - paintY) / (float)texH) * texH;
  if ((paintY + paintH) > (areaY + areaH))
    areaH +=
        std::ceil(((paintY + paintH) - (areaY + areaH)) / (float)texH) * texH;
...
// ----------------------------------------------------------------------------
void CSSBackgroundRenderer::buildImageQuads(const CSSBackground &bg,
                                            sint32 textureId) {
  // TODO: m_Background should have textureId and that should be "reserved" in
  // CViewRenderer even before download is finished
  if (textureId < 0)
    return;
  ...
  calculateTiles(m_Background, texX, texY, texW, texH, tilesX, tilesY, spacingX,
                 spacingY);
  ...
  for (sint32 tileX = 0; tileX < tilesX; tileX++) {
    for (sint32 tileY = 0; tileY < tilesY; tileY++) {
      ...
      if (!(tileL >= clipL && tileR <= clipR && tileB >= clipB &&
            tileT <= clipT)) {
        float ratio;
        if (tileL < clipL) {

...
void CViewPolygon::setVertices(const std::vector<NLMISC::CVector> &vertices) {
  if (vertices.size() == _Poly.Vertices.size() &&
      std::equal(vertices.begin(), vertices.end(), _Poly.Vertices.begin()))
  ...
  if (_Poly.toConvexPolygons(polys, NLMISC::CMatrix::Identity)) {
    _Tris.clear();
    for (std::list<CPolygon>::iterator it = polys.begin(); it != polys.end();
         ++it) {
      it->toTriFan(_Tris);
    }
  } else {
...
static inline bool totallyInside(const CVector &minCorner,
                                 const CVector &maxCorner, sint32 cx, sint32 cy,
                                 sint32 cw, sint32 ch) {
  return (sint32)maxCorner.x < (cx + cw) && (sint32)minCorner.x >= cx &&
         (sint32)maxCorner.y < (cy + ch) && (sint32)minCorner.y >= cy;
}

static inline bool totallyOutside(const CVector &minCorner,
                                  const CVector &maxCorner, sint32 cx,
                                  sint32 cy, sint32 cw, sint32 ch) {
  return (sint32)minCorner.x >= (cx + cw) || (sint32)maxCorner.x < cx ||
         (sint32)minCorner.y >= (cy + ch) || (sint32)maxCorner.y < cy;
}
...
void CViewPolygon::draw() {
  ...
  if (_Touched) {
    ...
    for (uint k = 0; k < numTris; ++k) {
      winTris[k].V0.set((float)(_Tris[k].V0.x + cornerX),
                        (float)(_Tris[k].V0.y + cornerY), 0.f);
      winTris[k].V1.set((float)(_Tris[k].V1.x + cornerX),
                        (float)(_Tris[k].V1.y + cornerY), 0.f);
      winTris[k].V2.set((float)(_Tris[k].V2.x + cornerX),
                        (float)(_Tris[k].V2.y + cornerY), 0.f);
    }
    ...
    for (uint k = 0; k < numTris; ++k) {
      ...
      if (totallyOutside(minCorner, maxCorner, cx, cy, cw, ch))
        continue;
      if (totallyInside(minCorner, maxCorner, cx, cy, cw, ch)) {
        ...
        if (count >= 3) {
          for (uint k = 0; k < (uint)(count - 2); ++k) {
            _RealTris.push_back(
                NLMISC::CTriangle(pPos0[0], pPos0[k + 1], pPos0[k + 2]));
          }
        }
      }
    }
...
void CViewPolygon::updateCoords() {
  // assume that clipping will have to be done again, real update of triangle
  // will be done at render time
  _Touched = true;
}

// *********************************************************************************
void CViewPolygon::setAlpha(sint32 a) { _Color.A = (uint8)a; }

...
void xmlCheckNodeName(xmlNodePtr &node, const char *nodeName) {
  // Check node name
  if (node == NULL || ((const char *)node->name == NULL) ||
      (strcmp((const char *)node->name, nodeName) != 0)) {
    ...
    char tmp[512];
    smprintf(tmp, 512,
             "LogicStateMachine STATE_MACHINE XML Syntax error in block line "
             "%d, node %s should be %s",
             node ? (int)node->line : -1,
             node ? (const char *)node->name : "NULL", nodeName);
...
std::string getXMLProp(xmlNodePtr node, const char *propName) {
  ...
  if (name) {
    ...
    char tmp[512];
    smprintf(tmp, 512,
             "LogicStateMachine XML Syntax error in block %s line %d, aguments "
             "Name not found",
...
void CLogicStateMachine::setCurrentState(string stateName) {
  ...
  if (itStates != _States.end()) {
    ...
    nlwarning("(LOGIC)<CLogicStateMachine::setCurrentState> The state \"%s\" "
              "is not in the state machine \"%s\"",
              stateName.c_str(), _Name.c_str());
  }
...
//
//---------------------------------------------------
void CLogicStateMachine::getMessagesToSend(
    multimap<CEntityId, CMessage> &msgs) {
...
//
//---------------------------------------------------
bool CLogicStateMachine::getVariable(std::string &varName,
                                     CLogicVariable &var) {
...
//
//---------------------------------------------------
bool CLogicStateMachine::getCondition(const std::string &condName,
...
//
//---------------------------------------------------
void CLogicStateMachine::modifyVariable(string varName, string modifOperator,
                                        sint64 value) {
  map<string, CLogicVariable>::iterator itVar = _Variables.find(varName);
  if (itVar != _Variables.end()) {
  ...
  nlwarning("(LOGIC)<CLogicStateMachine::modifyVariable> The variable \"%s\" "
            "is not in the state machine \"%s\"",
            varName.c_str(), _Name.c_str());
...
void CLogicStateMachine::displayVariables() {
  ...
  pair<multimap<CEntityId, string>::iterator,
       multimap<CEntityId, string>::iterator>
      itVarsRng = allVariables.equal_range(unknown);
  ...
  */
  ...
  nlinfo("VARIABLES/COUNTERS in %s : %d/%d are registered : ", _Name.c_str(),
         allVariables.size(), allVariables.size() + unclaimedVariables.size());
  // display the registered variables
  for (itVars = allVariables.begin(); itVars != allVariables.end(); ++itVars) {
    map<string, CLogicVariable>::const_iterator itV =
        _Variables.find((*itVars).second);
    nlinfo("[%d] %s = %f", (uint8)(*itVars).first.getDynamicId(),
           (*itV).first.c_str(), (double)(*itV).second.getValue());
  }

  // display the unclaimed variables
  sort(unclaimedVariables.begin(), unclaimedVariables.end());
  vector<string>::iterator itUV;
  for (itUV = unclaimedVariables.begin(); itUV != unclaimedVariables.end();
       ++itUV) {
    map<string, CLogicVariable>::const_iterator itV = _Variables.find(*itUV);
    nlinfo("(-)%s = %f", (*itV).first.c_str(),
           (double)(*itV).second.getValue());
  }
...
void CLogicStateMachine::displayStates() {
  nlinfo("There are %d STATES in the state machine \"%s\": ", _States.size(),
         _Name.c_str());
...
void CLogicStateMachine::setVerbose(string varName, bool b) {
  if (varName == "all") {
    ...
    if (b) {
      nlinfo("the verbose mode has been activated for all the variables",
             varName.c_str());
    } else {
      nlinfo("the verbose mode has been desactivated for all the variables",
             varName.c_str());
    }
    return;
  }
  ...
  if (b) {
    nlinfo("the verbose mode for variable \"%s\" has been activated",
           varName.c_str());
  } else {
    nlinfo("the verbose mode for variable \"%s\" has been desactivated",
           varName.c_str());
  }
...
void CLogicStateMachine::write(xmlDocPtr doc) const {
  ...
  xmlNodePtr node =
      xmlNewDocNode(doc, NULL, (const xmlChar *)"STATE_MACHINE", NULL);
  xmlDocSetRootElement(doc, node);
  xmlSetProp(node, (const xmlChar *)"Name", (const xmlChar *)_Name.c_str());
  xmlSetProp(node, (const xmlChar *)"CurrentState",
             (const xmlChar *)_CurrentState.c_str());

  for (std::map<std::string, CLogicVariable>::const_iterator vit =
           _Variables.begin();
       vit != _Variables.end(); vit++) {
    (*vit).second.write(node);
  }

  for (std::map<std::string, CLogicCounter>::const_iterator cit =
           _Counters.begin();
       cit != _Counters.end(); cit++) {
    (*cit).second.write(node);
  }

  for (std::map<std::string, CLogicCondition>::const_iterator c2it =
           _Conditions.begin();
       c2it != _Conditions.end(); c2it++) {
    (*c2it).second.write(node);
  }

  for (std::map<std::string, CLogicState>::const_iterator sit = _States.begin();
       sit != _States.end(); sit++) {

...
void CLogicVariable::applyModification(string op, sint64 value) {
  if (op == "SET" || op == "set") {
    ...
    nlwarning("(LGCS)<CLogicVariable::applyModification> The operator \"%s\" "
              "is unknown",
              op.c_str());
    return;
  }
...
//
//---------------------------------------------------
void CLogicVariable::processLogic() {} // processLogic //
...
void CLogicVariable::write(xmlNodePtr node) const {
  xmlNodePtr elmPtr =
      xmlNewChild(node, NULL, (const xmlChar *)"VARIABLE", NULL);
  xmlSetProp(elmPtr, (const xmlChar *)"Name", (const xmlChar *)_Name.c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Value",
             (const xmlChar *)toString(_Value).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Verbose",
             (const xmlChar *)toString(_Verbose).c_str());
}
...
void CLogicCounter::write(xmlNodePtr node) const {
  ...
  xmlSetProp(elmPtr, (const xmlChar *)"Verbose",
             (const xmlChar *)toString(_Verbose).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Period",
             (const xmlChar *)toString(Period.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Phase",
             (const xmlChar *)toString(Phase.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Step",
             (const xmlChar *)toString(Step.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"LowLimit",
             (const xmlChar *)toString(LowLimit.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"HighLimit",
             (const xmlChar *)toString(HighLimit.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Mode",
             (const xmlChar *)toString(Mode.getValue()).c_str());
  xmlSetProp(elmPtr, (const xmlChar *)"Control",
             (const xmlChar *)toString(Control.getValue()).c_str());
}

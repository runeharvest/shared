...
const CVector &NLPACS::CLocalRetriever::getStartVector(uint32 chain, sint32 surface) const
{
	...
	const COrderedChain3f &ochain = onLeft ? _FullOrderedChains[_Chains[chain].getSubChains().front()] : _FullOrderedChains[_Chains[chain].getSubChains().back()];
...
{
	bool onLeft = _Chains[chain].getLeft() == surface;
	const COrderedChain3f &ochain = onLeft ? _FullOrderedChains[_Chains[chain].getSubChains().back()] : _FullOrderedChains[_Chains[chain].getSubChains().front()];
	...
	for (i = 0; i < _Surfaces.size(); ++i)
	{
		...
		for (;;)
		{
			...
			for (;;)
			{
				...
				else if ((best > 1.0e0f && loopCloseDistance < 3.0e-2f) || loopCloseDistance < 1.0e-3f)
				{
					break;
				}
...
void NLPACS::CLocalRetriever::computeTopologies()
{
	...
	for (character = 0; character < NumCreatureModels; ++character)
	{
		...
		{
			if (_Surfaces[surface]._Topologies[character] == -1 && _Surfaces[surface]._Character == character)
			{
				...
				while (!surfacesStack.empty())
				{
					...
					for (i = 0; i < current._Chains.size(); ++i)
					{
						...
						if (link >= 0 && link < (sint)_Surfaces.size() && _Surfaces[link]._Topologies[character] == -1 && _Surfaces[link]._Character >= character)
						{
	...
	for (i = 0; i < numEdges; ++i)
	{
		...
		else
		{
			...
			if (vertices[start].x == vertices[stop].x)
			{
				...
				if ((estim.y > vertices[start].y && estim.y > vertices[stop].y) || (estim.y < vertices[start].y && estim.y < vertices[stop].y))
					continue;
    ...
    NLPACS::CCollisionSurfaceTemp &cst) const
{
	...
	for (i = 0; i < cst.EdgeChainEntries.size(); ++i)
	{
		...
		if (_Chains[chain.getParentId()].getLeft() != surfaceId && _Chains[chain.getParentId()].getRight() != surfaceId)
			continue;
	...
	if (intersEnd > 0)
	{
		while (intersStart < intersections.size() && intersections[intersStart].In && intersections[intersStart].Position < 1.0e-4f)
			++intersStart;

		while (intersStart < intersEnd && !intersections[intersEnd - 1].In && intersections[intersEnd - 1].Position > 1.0f - 1.0e-4f)
			--intersEnd;
	...
	for (i = intersStart; i < intersEnd;)
	{
		...
		for (;;)
		{
			...
			if (thisOChainIndex < 0 || thisOChainIndex >= (sint)_Chains[thisChainId]._SubChains.size())
			{
				...
				thisOChainIndex = (thisChainForward == forward) ? 0 : (sint)_Chains[thisChainId]._SubChains.size() - 1;
			}

...
const CVector &NLPACS::CLocalRetriever::getStopVector(uint32 chain,
  ...
  bool onLeft = _Chains[chain].getLeft() == surface;
  const COrderedChain3f &ochain =
      onLeft ? _FullOrderedChains[_Chains[chain].getSubChains().back()]
             : _FullOrderedChains[_Chains[chain].getSubChains().front()];
...
sint32 surface) const
{
    return (_Chains[chain].getLeft() == surface) ? _Chains[chain].getStartTip()
: _Chains[chain].getStopTip();
}

uint16			NLPACS::CLocalRetriever::getStopTip(uint32 chain, sint32
surface) const
{
    return (_Chains[chain].getLeft() == surface) ? _Chains[chain].getStopTip() :
_Chains[chain].getStartTip();
}
...
sint32 surface, uint16 startTip)
{
...
void			NLPACS::CLocalRetriever::setStopTip(uint32 chain, sint32
surface, uint16 stopTip)
{
    if (_Chains[chain].getLeft() == surface)
...
uint32 NLPACS::CLocalRetriever::getPreviousChain(uint32 chain,
  ...
  return surf._Chains[sLoop[(loopIndex + sLoop.size() - 1) % sLoop.size()]]
      .Chain;
}
  ...
  /*
      ...
      for (i=0; i<_Tips.size(); ++i)
      {
          ...
          for (j=0; j<tip.Chains.size(); ++j)
          {
              ...
              {
                  if
     (_Chains[tip.Chains[j].Chain].getStartVector(_OrderedChains) != ptip)
                      nlwarning("chain %d is not stuck to tip %d",
     tip.Chains[j].Chain, i); _Chains[tip.Chains[j].Chain].setStartVector(ptip,
     _OrderedChains);
              }
              else
              {
                  if (_Chains[tip.Chains[j].Chain].getStopVector(_OrderedChains)
     != ptip) nlwarning("chain %d is not stuck to tip %d", tip.Chains[j].Chain,
     i); _Chains[tip.Chains[j].Chain].setStopVector(ptip, _OrderedChains);
              }
          }
      }
...
void NLPACS::CLocalRetriever::dumpSurface(uint surf,
  ...
  for (i = 0; i < surface._Chains.size(); ++i) {
    ...
    nlinfo("-- chain %d[%d]: %d sub left=%d right=%d start=%d stop=%d", i,
           chainId, chain.getSubChains().size(), chain.getLeft(),
           chain.getRight(), chain.getStartTip(), chain.getStopTip());
    ...
    for (j = 0; j < chain.getSubChains().size(); ++j) {
      ...
      nlinfo("     subchain %d[%d]: fwd=%d parent=%d idx=%d", j,
             chain.getSubChain(j), ochain.isForward(), ochain.getParentId(),
             ochain.getIndexInParent());
      for (k = 0; k < ochain.getVertices().size(); ++k)
        nlinfo("       v[%d]=(%.3f,%.3f,%.3f) (%d,%d)", k,
               ochain.getVertices()[k].x + vect.x,
               ochain.getVertices()[k].y + vect.y,
               ochain.getVertices()[k].z + vect.z, ochains.getVertices()[k].x,
               ochains.getVertices()[k].y);
    }
  }
...
float NLPACS::CLocalRetriever::distanceToBorder(
    const ULocalPosition &pos) const {
  if (!isLoaded())
    return 0.0f;
...
sint32 NLPACS::CLocalRetriever::addSurface(uint8 normalq, uint8 orientationq,
                                           uint8 mat, uint8 charact,
                                           uint8 level, bool isUnderWater,
                                           float waterHeight, bool clusterHint,
  ...
  surf._Flags |=
      (surf._IsCeiling) ? (1 << CRetrievableSurface::IsCeilingBit) : 0;
  surf._Flags |= (!surf._IsFloor && !surf._IsCeiling)
                     ? (1 << CRetrievableSurface::IsSlantBit)
                     : 0;
  surf._Flags |= clusterHint ? (1 << CRetrievableSurface::ClusterHintBit) : 0;

  surf._Flags |=
      (isUnderWater) ? (1 << CRetrievableSurface::IsUnderWaterBit) : 0;
  surf._WaterHeight = waterHeight;

  surf._Flags |= ((0xffffffff << (CRetrievableSurface::NormalQuantasStartBit)) &
                  CRetrievableSurface::NormalQuantasBitMask);
...
sint32 NLPACS::CLocalRetriever::addChain(const vector<CVector> &verts,
  ...
  if (newId > 65535)
    nlerror("in NLPACS::CLocalRetriever::addChain(): reached the maximum "
            "number of chains");
  ...
  if (leftSurface != NULL)
    leftSurface->_Chains.push_back(
        CRetrievableSurface::CSurfaceLink(newId, right));
  if (rightSurface != NULL)
    rightSurface->_Chains.push_back(
        CRetrievableSurface::CSurfaceLink(newId, left));
...
void NLPACS::CLocalRetriever::computeLoopsAndTips() {
  ...
  for (i = 0; i < _Surfaces.size(); ++i) {
    ...
    for (;;) {
      ...
      _Chains[surface._Chains[j].Chain].setLoopIndexes(i, loopId,
                                                       (uint)loop.size());
      ...
      for (;;) {
        //				loopCloseDistance =
        //hybrid2dNorm(loopStart-currentEnd);
        ...
        for (j = 0; j < chainFlags.size(); ++j) {
          ...
          //					float	d =
          //hybrid2dNorm(thisStart-currentEnd);
        ...
        if ((bestChain == -1 || best > 4.0e-2f) &&
            loopCloseDistance > 4.0e-2f) {
        ...
        _Chains[surface._Chains[bestChain].Chain].setLoopIndexes(
            i, loopId, (uint)loop.size());
  ...
  /*
      ...
      for (i=0; i<_Chains.size(); ++i)
      {
          ...
          {
              // get the tip id
              uint	thisTip = (whichTip) ? _Chains[i].getStopTip() :
  _Chains[i].getStartTip();

              if (thisTip != 0xffff && thisTip >= _Tips.size())
              {
                  nlwarning("in
  NLPACS::CLocalRetriever::computeLoopsAndTips()"); nlerror("checked a tip that
  doesn't exist on chain %d (tipId=%d)", i, thisTip);
              }

              // if it is unaffected yet creates an new tip and affect it to the
  common chains if (thisTip == 0xffff)
              {
                  uint	turn;
                  ...
                  for (turn=0; turn<=1; ++turn)
                  {
                      ...
                      sint32	surf = (!turn && !whichTip || turn && whichTip)
  ? _Chains[chain].getLeft() : _Chains[chain].getRight();

                      while (surf >= 0)
                      {

                          CChain	&nextChain = (turn) ? _Chains[chain =
  getNextChain(chain, surf)] : _Chains[chain = getPreviousChain(chain, surf)];
                          bool	isForward = (nextChain.getLeft() == surf);
  // tells if the left surf is the current surf bool	selectTip = isForward &&
  !turn || !isForward && turn; uint16	&tipRef = selectTip ? nextChain._StopTip
  : nextChain._StartTip; surf = (isForward) ? nextChain.getRight() :
  nextChain.getLeft();

                          if (tipRef != 0xffff && tipRef != tipId)
                          {
                              nlwarning("in
  NLPACS::CLocalRetriever::computeLoopsAndTips()"); nlerror("Trying to setup a
  already created tip (tipId=%d, previous=%d)", tipId, tipRef);
                          }
  ...
  //		{
  //
  nlwarning("NLPACS::CLocalRetriever::computeLoopsAndTips(): chain %d on edge %d
  has same StartTip and StopTip", i, _Chains[i].getEdge(), startTip, stopTip);
  //		}
  ...
  for (i = 0; i < _Surfaces.size(); ++i) {
    for (j = 0; j < _Surfaces[i]._Loops.size(); ++j) {
      ...
      for (k = 0; k < _Surfaces[i]._Loops[j].size(); ++k)
        _Surfaces[i]._Loops[j].Length +=
            _Chains[_Surfaces[i]._Chains[_Surfaces[i]._Loops[j][k]].Chain]
                .getLength();
    }
  }
}
...
//
void NLPACS::CLocalRetriever::buildSurfacePolygons(
    uint32 surface, list<CPolygon> &polygons) const {
  const CRetrievableSurface &surf = _Surfaces[surface];
...
//
void NLPACS::CLocalRetriever::build3dSurfacePolygons(
    uint32 surface, list<CPolygon> &polygons) const {
  const CRetrievableSurface &surf = _Surfaces[surface];
  ...
  for (i = 0; i < surf._Loops.size(); ++i) {
    ...
    for (j = 0; j < surf._Loops[i].size(); ++j) {
      ...
      if (chainforward) {
        for (k = 0; k < chain._SubChains.size(); ++k) {
          const COrderedChain3f &ochain =
              _FullOrderedChains[chain._SubChains[k]];
          bool ochainforward = ochain.isForward();
      ...
      if (_Surfaces[surface]._Topologies[character] == -1 &&
        ...
        while (!surfacesStack.empty()) {
          ...
          for (i = 0; i < current._Chains.size(); ++i) {
            ...
            sint32 link = (chain.getLeft() == surface) ? chain.getRight()
                                                       : chain.getLeft();
  ...
  for (surface = 0; surface < _Surfaces.size(); ++surface) {
    ...
    for (character = 0; character < NumCreatureModels; ++character)
      if (current._Topologies[character] >= 0)
        _Topologies[character][current._Topologies[character]].push_back(
            surface);
  }
}
...
bool NLPACS::CLocalRetriever::insurePosition(
    NLPACS::ULocalPosition &local) const {
  if (!_Loaded)
    return false;

  if (local.Surface < 0 || local.Surface >= (sint)_Surfaces.size()) {
    nlwarning("PACS: can't insure position to inexistant surface %d",
              local.Surface);
    return false;
  }
  ...
  // check if point is located on the good side of the border (and far enough to
  ...
  for (i = 0; i < surface.getChains().size(); ++i) {
    ...
    for (j = 0; j < chain.getSubChains().size(); ++j) {
      ...
      bool shouldBeUpper =
          !((isAtLeft ^ isForward) != 0); // shouldBeAtLeft for vertical segment
      ...
      for (k = 0; (sint)k < (sint)(ochain.getVertices().size() - 1); ++k) {
        ...
        CVector2f n =
            (shouldBeUpper ? CVector2f(-AB.y, AB.x) : CVector2f(AB.y, -AB.x))
                .normed();
        float d = (M - A) * n;
...
bool NLPACS::CLocalRetriever::testPosition(NLPACS::ULocalPosition &local,
  ...
  bool result = (cst.SurfaceLUT[local.Surface].Counter == 2 ||
                 cst.SurfaceLUT[local.Surface].OnVerticalEdge);
...
void NLPACS::CLocalRetriever::retrievePosition(
    CVector estimated, CCollisionSurfaceTemp &cst) const {
  if (!_Loaded)
    return;
  ...
  box.setMinMax(
      CVector(estimated.x - (float)BorderThreshold, _BBox.getMin().y, 0.0f),
  ...
  // for each ordered chain, checks if the estimated position is between the min
  // and max.
  for (i = 0; i < numEdges; ++i) {
    ochain = cst.EdgeChainEntries[i].OChainId;

    const COrderedChain &sub = _OrderedChains[ochain];
    const CVector2s &min = sub.getMin(), &max = sub.getMax();
    ...
    if (estim.y < min.y) {
      if (estim.x == max.x)
        continue;
      isUpper = false;
      //			nlinfo("Box: min(%d,%d) max(%d,%d) forward=%d
      //left=%d right=%d upper=false", min.x, min.y, max.x, max.y,
      //sub.isForward(), left, right);
    } else if (estim.y > max.y) {
      if (estim.x == max.x)
        continue;
      isUpper = true;
      //			nlinfo("Box: min(%d,%d) max(%d,%d) forward=%d
      //left=%d right=%d upper=true", min.x, min.y, max.x, max.y,
      //sub.isForward(), left, right);
    } else {
      const vector<CVector2s> &vertices = sub.getVertices();
      uint start = 0, stop = (uint)vertices.size() - 1;

      // then finds the smallest segment of the chain that includes the
      // estimated position.
      while (stop - start > 1) {
      ...
      if (vertices[start].x == vertices[stop].x) {
        ...
        while (stop < vertices.size() - 1 &&
               vertices[stop].x == vertices[stop + 1].x)
          ++stop;
        ...
        //				nlinfo("Edge: start(%d,%d) stop(%d,%d)
        //forward=%d left=%d right=%d border=true", vertices[start].x,
        //vertices[start].y, vertices[stop].x, vertices[stop].y,
        //sub.isForward(), left, right);
        continue;
      ...
      if (estim.y > vertices[start].y && estim.y > vertices[stop].y) {
        isUpper = true;
        //				nlinfo("Edge: start(%d,%d) stop(%d,%d)
        //forward=%d left=%d right=%d upper=true", vertices[start].x,
        //vertices[start].y, vertices[stop].x, vertices[stop].y,
        //sub.isForward(), left, right);
      }
      // second trivial case (down both tips)
        ...
        isUpper = false;
        //				nlinfo("Edge: start(%d,%d) stop(%d,%d)
        //forward=%d left=%d right=%d upper=false", vertices[start].x,
        //vertices[start].y, vertices[stop].x, vertices[stop].y,
        //sub.isForward(), left, right);
      }
      // full test...
      else {
        const CVector2s &vstart = vertices[start], &vstop = vertices[stop];

        sint16 intersect = vstart.y + (vstop.y - vstart.y) *
                                          (estim.x - vstart.x) /
                                          (vstop.x - vstart.x);

        isUpper = estim.y > intersect;
        // isOnBorder = (fabs(estim.y -
        // intersect)<BorderThreshold*Vector2sAccuracy);
        isOnBorder = (fabs((double)(estim.y - intersect)) <
                      (double)(BorderThreshold * Vector2sAccuracy));
        //				nlinfo("Edge: start(%d,%d) stop(%d,%d)
        //forward=%d left=%d right=%d upper=%s border=%s", vertices[start].x,
        //vertices[start].y, vertices[stop].x, vertices[stop].y,
        //sub.isForward(), left, right, isUpper ? "true":"false", isOnBorder ?
        //"true":"false");
      }
    }
    ...
    // Depending on the chain is forward, up the position, increase/decrease the
    // surface table...
    if (sub.isForward()) {
      if (isUpper) {
        cst.incSurface(left);
...
void NLPACS::CLocalRetriever::retrieveAccuratePosition(
    CVector2s estim, CCollisionSurfaceTemp &cst, bool &onBorder) const {
  if (!_Loaded)
    return;
  ...
  for (i = 0; i < numEdges; ++i) {
    ...
    if (estim.y < min.y) {
      ...
      // full test...
        ...
        sint32 det = (estim.y - vstart.y) * (vstop.x - vstart.x) -
                     (estim.x - vstart.x) * (vstop.y - vstart.y);
...
void NLPACS::CLocalRetriever::snapToInteriorGround(
    NLPACS::ULocalPosition &position, bool &snapped) const {
  if (!_Loaded)
    return;
  ...
  // from the preselect faces, look for the only face that belongs to the
  // surface and that contains the position
  ...
  for (it = selection.begin(); it != selection.end(); ++it) {
    ...
    if (f.Surface == (uint32)position.Surface) {
      ...
      if (n * pos2d + c < -1.0f / Vector2sAccuracy)
        continue;
      // Line p1-p2.
...
float NLPACS::CLocalRetriever::getHeight(
    const NLPACS::ULocalPosition &position) const {
  if (!_Loaded)
    return 0.0f;
  ...
  if (_Type == Interior) {
    ...
    if (_Surfaces[position.Surface].getQuadTree().getRoot() != NULL) {
      // find quad leaf.
      const CQuadLeaf *leaf = _Surfaces[position.Surface].getQuadTree().getLeaf(
          position.Estimation);
...
// ***************************************************************************
float NLPACS::CLocalRetriever::getInteriorHeightAround(
    const ULocalPosition &position, float outsideTolerance) const {
  if (!_Loaded)
    return 0.0f;
    ...
    for (it = selection.begin(); it != selection.end(); ++it) {
      ...
      if (f.Surface == (uint32)position.Surface) {
        ...
        if (a * (pos.x - v[0].x) + b * (pos.y - v[0].y) <
            -len * outsideTolerance)
          continue;
        // Line p1-p2.
        a = -(v[2].y - v[1].y);
        b = (v[2].x - v[1].x);
        len = sqrtf(a * a + b * b); // norm of the normal vector
        if (a * (pos.x - v[1].x) + b * (pos.y - v[1].y) <
            -len * outsideTolerance)
          continue;
        //  Line p2-p0.
        a = -(v[0].y - v[2].y);
        b = (v[0].x - v[2].x);
        len = sqrtf(a * a + b * b); // norm of the normal vector
        if (a * (pos.x - v[2].x) + b * (pos.y - v[2].y) <
            -len * outsideTolerance)
          continue;
...
void NLPACS::CLocalRetriever::findPath(
    const NLPACS::CLocalRetriever::CLocalPosition &A,
  ...
  if (A.Surface != B.Surface) {
    ...
    nlerror("Try to find a path between 2 points that are not in the same "
            "surface (A=%d, B=%d)",
            A.Surface, B.Surface);
  }

  CVector a = A.Estimation, b = B.Estimation,
  ...
  for (i = 0; i < cst.EdgeChainEntries.size(); ++i) {
    ...
    for (j = entry.EdgeStart; j < entry.EdgeEnd; ++j) {
      ...
      CVector p0 = chain[j].unpack3f(), p1 = chain[j + 1].unpack3f();

      float vp0 = (p0 - a) * n, vp1 = (p1 - a) * n;

      if (vp0 * vp1 <= 0.0f) {
        CVector np = CVector(p0.y - p1.y, p1.x - p0.x, 0.0f);

        float va = (a - p0) * np, vb = (b - p0) * np;

        // here we have an intersection
        if (va * vb <= 0.0f) {
          const CChain &parent = _Chains[chain.getParentId()];
          bool isIn = (va - vb < 0.0f) ^ (parent.getLeft() == surfaceId) ^
                      chain.isForward();

          intersections.push_back(CIntersectionMarker(
              va / (va - vb), entry.OChainId, uint16(j), isIn));
        }
      }
    }
  }
  ...
  if (intersEnd > 0) {
    while (intersStart < intersections.size() &&
           intersections[intersStart].In &&
           intersections[intersStart].Position < 1.0e-4f)
      ++intersStart;

    while (intersStart < intersEnd && !intersections[intersEnd - 1].In &&
           intersections[intersEnd - 1].Position > 1.0f - 1.0e-4f)
      --intersEnd;

    // Check intersections have a valid order
    if ((intersEnd - intersStart) & 1) {
      nlwarning("in NLPACS::CLocalRetriever::findPath()");
      nlerror("Found an odd (%d) number of intersections",
              intersections.size());
    }
    ...
    for (i = intersStart; i < intersEnd;) {
      ...
      const CChain &exitChain =
          _Chains[_OrderedChains[intersections[i].OChain].getParentId()];
      exitLoop = (exitChain.getLeft() == surfaceId) ? exitChain.getLeftLoop()
                                                    : exitChain.getRightLoop();
      ...
      const CChain &enterChain =
          _Chains[_OrderedChains[intersections[i].OChain].getParentId()];
      enterLoop = (enterChain.getLeft() == surfaceId)
                      ? enterChain.getLeftLoop()
                      : enterChain.getRightLoop();
  ...
  for (i = intersStart; i < intersEnd;) {
    uint exitChainId = _OrderedChains[intersections[i].OChain].getParentId(),
         enterChainId =
             _OrderedChains[intersections[i + 1].OChain].getParentId();
    ...
    float forwardLength =
        (exitChain.getLength() + enterChain.getLength()) * 0.5f;
    ...
    if (exitChainId != enterChainId) {
      for (j = (exitLoopIndex + 1) % loop.size(); j != enterLoopIndex;
           j = (j + 1) % loop.size())
      ...
      forward =
          !thisChainForward ^
          (_OrderedChains[intersections[i].OChain].getIndexInParent() <
           _OrderedChains[intersections[i + 1].OChain].getIndexInParent());
    }

    path.push_back(CVector2s(A.Estimation + intersections[i].Position *
                                                (B.Estimation - A.Estimation)));

    for (;;) {
      sint from = (thisOChainId == intersections[i].OChain)
                      ? intersections[i].Edge
                      : -1,
           to = (thisOChainId == intersections[i + 1].OChain)
                    ? intersections[i + 1].Edge
                    : -1;
      bool oforward =
          thisChainForward ^ forward ^ _OrderedChains[thisOChainId].isForward();
      ...
      thisOChainIndex = (thisChainForward ^ forward) ? thisOChainIndex - 1
                                                     : thisOChainIndex + 1;

      if (thisOChainIndex < 0 ||
          thisOChainIndex >= (sint)_Chains[thisChainId]._SubChains.size()) {
        if (forward) {
          loopIndex++;
        ...
        thisOChainIndex =
            (thisChainForward == forward)
                ? 0
                : (sint)_Chains[thisChainId]._SubChains.size() - 1;
      }

      thisOChainId = _Chains[thisChainId]._SubChains[thisOChainIndex];
    }

    path.push_back(CVector2s(A.Estimation + intersections[i + 1].Position *
                                                (B.Estimation - A.Estimation)));
    i += 2;
  }
...
void NLPACS::CLocalRetriever::testCollision(CCollisionSurfaceTemp &cst,
                                            const CAABBox &bboxMove,
                                            const CVector2f &transBase) const {
  if (!_Loaded)
    return;
  ...
  // NB: cst.OChainLUT is assured to be full of 0xFFFF after this call (if was
  // right before).
  ...
  for (i = 0; i < nEce; i++) {
    ...
    if (_Type == Interior &&
        CChain::isBorderChainId(this->getChains()[chainId].getRight())) {
      // then look for a door that match this edge
      uint l;
      for (l = 0; l < _ExteriorMesh.getLinks().size() &&
                  _ExteriorMesh.getLink(l).ChainId != chainId;
           ++l)
        ;
    ...
    if (chainLUT[chainId] == 0xFFFF) {
      ...
      cst.CollisionChains[ccId].LeftSurface.SurfaceId =
          this->getChains()[chainId].getLeft();
      cst.CollisionChains[ccId].RightSurface.SurfaceId =
          this->getChains()[chainId].getRight();
      // NB: cst.CollisionChains[ccId].*Surface.RetrieverInstanceId is not
      // filled here because we don't have this info at this level.
  ...
  // for all collisions chains inserted (starting from firstChainAdded), reset
  // LUT.
...
void NLPACS::CLocalRetriever::buildInteriorSurfaceBBoxes(
    std::vector<NLMISC::CAABBox> &surfaceBBoxes) const {
  ...
  for (uint iIntFace = 0; iIntFace < _InteriorFaces.size(); iIntFace++) {
    ...
    if (firstTriangle[intFace.Surface]) {
      surfaceBBoxes[intFace.Surface].setCenter(
          _InteriorVertices[intFace.Verts[0]]);
      firstTriangle[intFace.Surface] = false;
    } else
      surfaceBBoxes[intFace.Surface].extend(
          _InteriorVertices[intFace.Verts[0]]);
...
// ***************************************************************************
void NLPACS::CLocalRetriever::replaceChain(
    uint32 chainId,
    const std::vector<NLPACS::CLocalRetriever::CChainReplacement>
        &replacement) {
  // free subchains
  uint i, j;
  ...
  for (i = 0; i < replacement.size(); ++i) {
    ...
    if (CChain::isBorderChainId(right)) {
      ...
      if (border < (sint)_BorderChains.size() &&
          (chainId != _BorderChains[border] ||
           chainId != replacement[i].Chain)) {
        nlwarning("replaceChain(): replacement of a border is forced whereas "
                  "this border is already used and not replaced!");
      }

      if (border >= (sint)_BorderChains.size()) {
        if (border > (sint)_BorderChains.size()) {
          nlwarning("replaceChain(): _BorderChains size increased of more than "
                    "1 step, holes may result!");
        }
    ...
    if (left > (sint)_Surfaces.size())
      nlerror("left surface id MUST be id<%d (id=%d)", _Surfaces.size(), left);
    if (right > (sint)_Surfaces.size())
      nlerror("right surface id MUST be id<%d (id=%d)", _Surfaces.size(),
              right);
    ...
    // CRetrievableSurface	*leftSurface = (left>=0) ? &(_Surfaces[left]) :
    // NULL; CRetrievableSurface	*rightSurface = (right>=0) ?
    // &(_Surfaces[right]) : NULL;
    ...
    // make the chain and its subchains.
    chain.make(vertices, left, right, _OrderedChains, (uint16)newId,
               _FullOrderedChains, FreeOChains);
  }
    ...
    for (j = 0; j < _Surfaces[i]._Chains.size(); ++j) {
      if (_Surfaces[i]._Chains[j].Chain == (sint)chainId) {
        ...
        for (k = 0; k < replacement.size(); ++k) {
          ...
          link.Surface = (replacement[k].Left == (sint)i ? replacement[k].Right
                                                         : replacement[k].Left);
      ...
      for (k = 0; k < _Surfaces[i]._Loops[j].size(); ++k) {
        if (_Surfaces[i]._Loops[j][k] == chainId) {
          ...
          for (m = 0; m < replacement.size(); ++m)
            _Surfaces[i]._Loops[j].insert(_Surfaces[i]._Loops[j].begin() + k +
                                              m,
                                          uint16(replacement[m].Chain));
 ...
 */
bool NLPACS::CLocalRetriever::checkSurfacesIntegrity(
    NLMISC::CVector translation, bool verbose) const {
  bool success = true;
  uint surf;
 ...
 * Check surface integrity
 */
bool NLPACS::CLocalRetriever::checkSurfaceIntegrity(uint surf,
    ...
    for (i = 0; i < loop.size(); ++i) {
      ...
      for (j = 0; j < chain.getSubChains().size(); ++j) {
        ...
        for (k = 0; k + 1 < ochain.getVertices().size(); ++k) {
          edges.push_back(
              std::pair<CVector2s, CVector2s>(ochain[k], ochain[k + 1]));
        }
      }
    }
  }
  ...
  for (i = 0; i + 1 < edges.size(); ++i) {
    for (j = i + 1; j < edges.size(); ++j) {
      CVector2s a0 = edges[i].first, a1 = edges[i].second;
      CVector2s b0 = edges[j].first, b1 = edges[j].second;

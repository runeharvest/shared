...
void CPrimitiveWorldImage::deleteIt(CMoveContainer &container,
                                    uint8 worldImage) {
...
void CPrimitiveWorldImage::copy(const CPrimitiveWorldImage &source) {
  ...
  // Pointer into the 4 possibles sorted lists of movable primitives. Must be
  // NULL
  for (uint i = 0; i < 4; i++)
    _MoveElement[i] = NULL;
}
...
bool CPrimitiveWorldImage::evalCollision(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, double timeMin,
  ...
  // Mask test
  if (((primitive.getCollisionMaskInternal() &
        otherPrimitive.getOcclusionMaskInternal()) == 0) &&
      ((primitive.getOcclusionMaskInternal() &
        otherPrimitive.getCollisionMaskInternal()) == 0))
    return false;

  // Test time
  if ((!primitive.checkTestTime(testTime, maxTestIteration)) ||
      (!otherPrimitive.checkTestTime(testTime, maxTestIteration)))
    return false;
  ...
  case UMovePrimitive::_2DOrientedBox: {
    ...
    case UMovePrimitive::_2DOrientedBox:
      ...
      return evalCollisionOBoverOB(other, desc, timeMin, timeMax,
                                   firstContactTime, lastContactTime, primitive,
                                   otherPrimitive);
  ...
  case UMovePrimitive::_2DOrientedCylinder: {
    ...
    case UMovePrimitive::_2DOrientedBox: {
      // Make the test
      bool collid = other.evalCollisionOBoverOC(
          *this, desc, timeMin, timeMax, firstContactTime, lastContactTime,
...
const TCollisionSurfaceDescVector *CPrimitiveWorldImage::evalCollision(
    CGlobalRetriever &retriever, CCollisionSurfaceTemp &surfaceTemp,
  ...
  if (primitive.getPrimitiveTypeInternal() == UMovePrimitive::_2DOrientedBox) {
    // Local I
    CVector locI(
        (float)(_OBData.EdgeDirectionX[0] * primitive.getLength(0) / 2.0),
        (float)(_OBData.EdgeDirectionY[0] * primitive.getLength(1) / 2.0), 0);

    // Local J
    CVector locJ(
        (float)(_OBData.EdgeDirectionX[1] * primitive.getLength(0) / 2.0),
        (float)(_OBData.EdgeDirectionY[1] * primitive.getLength(1) / 2.0), 0);

    // Test
    return retriever.testBBoxMove(_Position.getGlobalPos(), _DeltaPosition,
                                  locI, locJ, surfaceTemp);
  } else {
    // Check
    nlassert(primitive.getPrimitiveTypeInternal() ==
             UMovePrimitive::_2DOrientedCylinder);

    // Test
    // nlinfo ("1) %f %f %f\n", _DeltaPosition.x, _DeltaPosition.y,
    // _DeltaPosition.z);

    return retriever.testCylinderMove(_Position.getGlobalPos(), _DeltaPosition,
...
void CPrimitiveWorldImage::doMove(CGlobalRetriever &retriever,
                                  CCollisionSurfaceTemp &surfaceTemp,
                                  double originalMax, double finalMax,
                                  bool keepZ /*= false*/) {
  ...
  // Make the move
  if (!keepZ) {
    _Position.setGlobalPos(retriever.doMove(_Position.getGlobalPos(),
                                            _DeltaPosition, (float)ratio,
                                            surfaceTemp, false),
                           retriever);
  } else {
    _Position.setGlobalPosKeepZ(retriever.doMove(_Position.getGlobalPos(),
                                                 _DeltaPosition, (float)ratio,
                                                 surfaceTemp, false),
                                retriever);
  }
...
bool CPrimitiveWorldImage::evalCollisionOBoverOB(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, double timeMin,
  ...
  nlassert(otherPrimitive.getPrimitiveTypeInternal() ==
           UMovePrimitive::_2DOrientedBox);
  ...
  for (pt = 0; pt < 4; pt++)
    for (seg = 0; seg < 4; seg++) {
      // Get collision time of the point over the segment
      CCollisionDesc d;
      if (other.evalCollisionPoverS(*this, d, pt, seg, primitive,
                                    otherPrimitive)) {
        // Find
        find = true;
...
bool CPrimitiveWorldImage::evalCollisionOBoverOC(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, double timeMin,
    double timeMax, double &firstContactTime, double &lastContactTime,
    CMovePrimitive &primitive, CMovePrimitive &otherPrimitive) {
  // Checks
...
bool CPrimitiveWorldImage::evalCollisionPoverS(CPrimitiveWorldImage &other,
                                               CCollisionDesc &desc,
                                               uint numPoint, uint numSeg,
  ...
  if (dotProd != 0) {
    ...
    double time =
        (normalSegX *
             (_OBData.PointPosX[numPoint] - other._OBData.PointPosX[numSeg]) +
         normalSegY *
             (_OBData.PointPosY[numPoint] - other._OBData.PointPosY[numSeg])) /
        dotProd;

    // Position of segment point at collision time
    const double segPosX =
        other._OBData.PointPosX[numSeg] + other._Speed.x * time;
    const double segPosY =
        other._OBData.PointPosY[numSeg] + other._Speed.y * time;
    ...
    if ((length >= 0) && (length <= otherPrimitive.getLength(numSeg & 1))) {
      ...
      // Included ?
      if ((ptPosZ <= segPosZ + otherPrimitive.getHeightInternal()) &&
          (ptPosZ + primitive.getHeightInternal() >= segPosZ)) {
...
bool CPrimitiveWorldImage::evalCollisionPoverOC(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, uint numPoint,
    double &firstContactTime, double &lastContactTime,
    CMovePrimitive &primitive, CMovePrimitive &otherPrimitive) {
  // Checks
  nlassert(primitive.getPrimitiveTypeInternal() ==
  ...
  double s0, s1;
  double squareRadius =
      otherPrimitive.getRadiusInternal() * otherPrimitive.getRadiusInternal();
  uint numSolution =
      secondDegree(_Bx * _Bx + _By * _By, 2.f * (_Ax * _Bx + _Ay * _By),
                   _Ax * _Ax + _Ay * _Ay - squareRadius, s0, s1);
  if (numSolution != 0) {
    // time
    double time;
    ...
    // Z Included ?
    if ((ptPosZ <= cylPosZ + otherPrimitive.getHeightInternal()) &&
        (ptPosZ + primitive.getHeightInternal() >= cylPosZ)) {
      // Ok Collision, fill the result
...
bool CPrimitiveWorldImage::evalCollisionSoverOC(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, uint numSeg,
    CMovePrimitive &primitive, CMovePrimitive &otherPrimitive) {
  // Checks
  nlassert(primitive.getPrimitiveTypeInternal() ==
           UMovePrimitive::_2DOrientedBox);
  ...
  if (dotProd != 0) {
    // Time of the collision
    double time =
        (otherPrimitive.getRadiusInternal() +
         normalSegX * (_OBData.PointPosX[numSeg] - other._3dInitPosition.x) +
         normalSegY * (_OBData.PointPosY[numSeg] - other._3dInitPosition.y)) /
        dotProd;
    ...
    // Position de contact
    const double contactX =
        cylPosX - normalSegX * otherPrimitive.getRadiusInternal();
    const double contactY =
        cylPosY - normalSegY * otherPrimitive.getRadiusInternal();
    ...
    if ((length >= 0) && (length <= primitive.getLength(numSeg & 1))) {
      ...
      // Included ?
      if ((cylPosZ <= segPosZ + primitive.getHeightInternal()) &&
          (cylPosZ + otherPrimitive.getHeightInternal() >= segPosZ)) {
        // Ok Collision, fill the result
...
bool CPrimitiveWorldImage::evalCollisionOCoverOC(
    CPrimitiveWorldImage &other, CCollisionDesc &desc, double timeMin,
    double timeMax, double &firstContactTime, double &lastContactTime,
    CMovePrimitive &primitive, CMovePrimitive &otherPrimitive) {
  // Checks
  ...
  double s0, s1;
  double radiusSquare =
      primitive.getRadiusInternal() + otherPrimitive.getRadiusInternal();
  radiusSquare *= radiusSquare;
  uint numSolution =
      secondDegree(_Bx * _Bx + _By * _By, 2.f * (_Ax * _Bx + _Ay * _By),
                   _Ax * _Ax + _Ay * _Ay - radiusSquare, s0, s1);
  if (numSolution != 0) {
    // time
    ...
    if ((timeMin < _timeMax) && (_timeMin < timeMax)) {
      ...
      // Z Included ?
      if ((cyl0PosZ <= cyl1PosZ + otherPrimitive.getHeightInternal()) &&
          (cyl0PosZ + primitive.getHeightInternal() >= cyl1PosZ)) {
        // Ok Collision, fill the result
        ...
        // Cylinder 1 position
        const double cyl1PosX =
            other._3dInitPosition.x + other._Speed.x * cyl1Time;
        const double cyl1PosY =
            other._3dInitPosition.y + other._Speed.y * cyl1Time;
        ...
        // Contact position
        desc.ContactPosition.x =
            desc.ContactNormal0.x * primitive.getRadiusInternal() + cyl0PosX;
        desc.ContactPosition.y =
            desc.ContactNormal0.y * primitive.getRadiusInternal() + cyl0PosY;
...
void CPrimitiveWorldImage::precalcPos(CMovePrimitive &primitive) {
  ...
  if (type == UMovePrimitive::_2DOrientedBox) {
    ...
    // First point
    _OBData.PointPosX[0] =
        cosinus * (-halfWidth) - sinus * (-halfDepth) + _3dInitPosition.x;
    _OBData.PointPosY[0] =
        sinus * (-halfWidth) + cosinus * (-halfDepth) + _3dInitPosition.y;
    ...
    // Direction
    double length0 =
        (primitive.getLength(0) == 0) ? 0.001 : primitive.getLength(0);
    double length1 =
        (primitive.getLength(1) == 0) ? 0.001 : primitive.getLength(1);
    ...
    for (i = 0; i < 4; i++) {
      ...
      // New direction
      _OBData.EdgeDirectionX[i] =
          (_OBData.PointPosX[next] - _OBData.PointPosX[i]) * oneOver;
      _OBData.EdgeDirectionY[i] =
          (_OBData.PointPosY[next] - _OBData.PointPosY[i]) * oneOver;
    }
  } else {
...
void CPrimitiveWorldImage::precalcBB(double beginTime, double endTime,
                                     CMovePrimitive &primitive) {
  // Type of the primitive
  uint type = primitive.getPrimitiveTypeInternal();
  ...
  if (type == UMovePrimitive::_2DOrientedBox) {
    ...
    _BBXMin = std::min(std::min(_BBXMin, _BBXMin + endTime * _Speed.x),
                       _BBXMin + beginTime * _Speed.x);
    _BBXMax = std::max(std::max(_BBXMax, _BBXMax + endTime * _Speed.x),
                       _BBXMax + beginTime * _Speed.x);
    _BBYMin = std::min(std::min(_BBYMin, _BBYMin + endTime * _Speed.y),
                       _BBYMin + beginTime * _Speed.y);
    _BBYMax = std::max(std::max(_BBYMax, _BBYMax + endTime * _Speed.y),
                       _BBYMax + beginTime * _Speed.y);

    /*
            // This code is faster but buggy..
            _BBXMin= _OBData.PointPosX[minX[orient]] + _Speed.x*beginTime;
            _BBXMin= std::min (_BBXMin, _OBData.PointPosX[minX[orient]] +
       _Speed.x*endTime);

            _BBYMin= _OBData.PointPosY[minY[orient]] + _Speed.y*beginTime;
            _BBYMin= std::min (_BBYMin, _OBData.PointPosY[minY[orient]] +
       _Speed.y*endTime);

            _BBXMax= _OBData.PointPosX[maxX[orient]] + _Speed.x*beginTime;
            _BBXMax= std::max (_BBXMax, _OBData.PointPosX[maxX[orient]] +
       _Speed.x*endTime);

            _BBYMax= _OBData.PointPosY[maxY[orient]] + _Speed.y*beginTime;
            _BBYMax= std::max (_BBYMax, _OBData.PointPosY[maxY[orient]] +
       _Speed.y*endTime);*/
  } else {
    // Should be a cylinder
    nlassert(type == UMovePrimitive::_2DOrientedCylinder);
...
void CPrimitiveWorldImage::addMoveElement(CMoveCell &cell, uint16 x, uint16 y,
...
void CPrimitiveWorldImage::addMoveElementendOfList(CMoveCell &cell, uint16 x,
                                                   uint16 y,
                                                   CMovePrimitive *primitive,
                                                   CMoveContainer &container) {
  // Find a free place
...
void CPrimitiveWorldImage::removeMoveElement(uint i, CMoveContainer &container,
                                             uint8 worldImage) {
  // Check
  nlassert(i < 4);
  ...
  for (uint i = 0; i < 4; i++) {
    ...
    if (_MoveElement[i]) {
      if (_MoveElement[i]->PreviousX)
        nlassertonce(_MoveElement[i]
                         ->PreviousX->Primitive->getWorldImage(worldImage)
                         ->_BBXMin <= _BBXMin);
      if (_MoveElement[i]->NextX)
        nlassertonce(_BBXMin <=
                     _MoveElement[i]
                         ->NextX->Primitive->getWorldImage(worldImage)
                         ->_BBXMin);
    }
  }
}
...
void CPrimitiveWorldImage::reaction(CPrimitiveWorldImage &second,
                                    const CCollisionDesc &desc,
                                    CGlobalRetriever *retriever,
                                    CCollisionSurfaceTemp &surfaceTemp,
                                    bool collision, CMovePrimitive &primitive,
                                    CMovePrimitive &otherPrimitive,
  ...
  UMovePrimitive::TReaction secondReaction =
      otherPrimitive.getReactionTypeInternal();

  // Overide collsion
  collision =
      collision && (primitive.isObstacle()) && (otherPrimitive.isObstacle());
  ...
  if (collision) {
    ...
    if (retriever) {
      ...
      UGlobalPosition newPosition =
          retriever->doMove(_Position.getGlobalPos(), _DeltaPosition,
    ...
    if (!secondConst) {
      ...
      case UMovePrimitive::Slide:
        ...
        // Reflexion speed
        newSpeed += (otherPrimitive.getAttenuation() * energySum / mass1) *
                    desc.ContactNormal1;
        break;
      case UMovePrimitive::Reflexion:
        // Remove projected speed
      ...
      if (retriever) {
        ...
        if (deltaDist == 0)
          ...
          deltaTime = (collisionPosition - second._Position.getPos()).norm() /
                      deltaDist;
        clamp(deltaTime, 0.0, 1.0);

        UGlobalPosition newPosition = retriever->doMove(
            second._Position.getGlobalPos(), second._DeltaPosition,
            (float)deltaTime, surfaceTemp, true);
        ...
        // Position at t=0
        second._3dInitPosition =
            second._Position.getPos() - newSpeed * desc.ContactTime;
        ...
        // Position at t=0
        second._3dInitPosition =
            collisionPosition - newSpeed * desc.ContactTime;
...
void CPrimitiveWorldImage::reaction(const CCollisionSurfaceDesc &surfaceDesc,
  ...
  if ((type == UMovePrimitive::Reflexion) || (type == UMovePrimitive::Slide)) {
    ...
    if (type == UMovePrimitive::Slide) {
      ...
      _Speed -= surfaceDesc.ContactNormal *
                (surfaceDesc.ContactNormal * _Speed -
                 NELPACS_DIST_BACK / (dt - surfaceDesc.ContactTime));
    }
    ...
    if (type == UMovePrimitive::Reflexion) {
      ...
      _Speed -= surfaceDesc.ContactNormal *
                (speedProj + speedProj * primitive.getAttenuation() -
                 NELPACS_DIST_BACK / (dt - surfaceDesc.ContactTime));
    }
  } else {
    // Stop ?
...
void CPrimitiveWorldImage::setGlobalPosition(const UGlobalPosition &pos,
                                             CMoveContainer &container,
                                             CMovePrimitive &primitive,
                                             uint8 worldImage) {
  // Cast type
...
void CPrimitiveWorldImage::setGlobalPosition(
    const NLMISC::CVectorD &pos, CMoveContainer &container,
    CMovePrimitive &primitive, uint8 worldImage, bool keepZ /*= false*/,
  ...
  if (retriever) {
    ...
    UGlobalPosition globalPosition =
        retriever->retrievePosition(pos, 1.0e10, type);
...
void CPrimitiveWorldImage::move(const NLMISC::CVectorD &speed,
                                CMoveContainer &container,
                                CMovePrimitive &primitive, uint8 worldImage) {
  // New speed

...
NLPACS::CSurfaceQuadTree::operator=(const NLPACS::CSurfaceQuadTree &quad) {
  if (&quad == this)
    return *this;
...
void NLPACS::CSurfaceQuadTree::init(float maxThickness, uint maxLevel,
                                    const CVector &center, float halfSize) {
  nlassert(maxLevel > 0);
  clear();
...
void NLPACS::CSurfaceQuadTree::compile() {
  if (_Root != NULL && !_Root->isLeaf() &&
    ...
    *((IQuadNode *)leaf) = *_Root;
    delete _Root;
    _Root = leaf;
  } else if (_Root != NULL && !_Root->isLeaf()) {
    ((CQuadBranch *)_Root)->reduceChildren();
  }
}

NLPACS::CQuadBranch::CQuadBranch(const NLPACS::CQuadBranch &branch)
    : NLPACS::IQuadNode(branch) {
  *this = branch;
}

NLPACS::CQuadBranch &
NLPACS::CQuadBranch::operator=(const NLPACS::CQuadBranch &branch) {
...
void NLPACS::CQuadBranch::reduceChildren() {
  ...
  for (i = 0; i < 4; ++i) {
    if (_Children[i] != NULL && !_Children[i]->isLeaf() &&
        _Children[i]->getMaxHeight() - _Children[i]->getMinHeight() <=
            _MaxThickness) {
      CQuadLeaf *leaf = new CQuadLeaf();
      *((IQuadNode *)leaf) = *_Children[i];
      delete _Children[i];
      _Children[i] = leaf;
    } else if (_Children[i] != NULL && !_Children[i]->isLeaf()) {
...
void NLPACS::CQuadBranch::addVertex(const CVector &v) {
  ...
  if (_Children[child] == NULL) {
    ...
    _Children[child]->_XCenter =
        _XCenter + _Children[child]->_HalfSize *
                       ((child == 1 || child == 2) ? 1.0f : -1.0f);
    _Children[child]->_YCenter =
        _YCenter + _Children[child]->_HalfSize *
                       ((child == 2 || child == 3) ? 1.0f : -1.0f);
  }
  ...
  */
  ...
  if (f.isReading()) {
    ...
    if (_Root == NULL) {
      ...
      childType = uint8(_Root->isLeaf() ? CQuadBranch::LeafChild
...
float NLPACS::CSurfaceQuadTree::getInterpZ(const CVector &v) const {
  ...
  for (side = 0; side < 4; ++side) {
    static const sint ct[4][4] = {{-1, 1, 3, -1},
                                  {-1, -1, 2, 0},
                                  {1, -1, -1, 3},
                                  {0, 2, -1, -1}}; // child table
    static const sint nt[4][4] = {{3, 1, 3, 1},
                                  {2, 0, 2, 0},
                                  {1, 3, 1, 3},
                                  {0, 2, 0, 2}}; // neighbor table
  ...
  for (i = 0; i < leaves.size(); ++i) {
    di = (float)sqrt(sqr(v.x - leaves[i]->_XCenter) +
                     sqr(v.y - leaves[i]->_YCenter)) *
         (float)pow(1.5, leaves[i]->_Level);
    wi = 1.0f / di;
    sum += wi;
    interpZ +=
        (leaves[i]->getMinHeight() + leaves[i]->getMaxHeight()) * 0.5f * wi;
  }

...
// ***************************************************************************
void CEdgeQuad::getGridBounds(sint32 &x0, sint32 &y0, sint32 &x1, sint32 &y1,
                              const CVector &minP, const CVector &maxP) const {
...
// ***************************************************************************
void CEdgeQuad::build(const CExteriorMesh &em, const CGlobalRetriever &global,
                      CCollisionSurfaceTemp &cst, uint32 thisInstance) {
  ...
  for (i = 0; i < (sint)edges.size() - 1; i++) {
    ...
    CVector opbegin = edges[i].Start, opend = edges[i + 1].Start;
    ...
    for (step = 0; step < numStep; ++step) {
      ...
      const TCollisionSurfaceDescVector *pcd =
          global.testCylinderMove(gp0, p1 - p0, 0.01f, cst);

      if (pcd == NULL) {
        //				nlwarning("in CEdgeQuad::build():
        //testCylinderMove() returned NULL");
        continue;
      }
      ...
      if (edges[i].Link != -1 && !cd.empty()) {
        ...
        nlwarning(
            "ERROR: exterior edge %d with interior link crosses some surfaces",
            i);
        cd.clear();
      }
      ...
      for (j = 0; j < (sint)cd.size() - 1; ++j) {
        s0 = op0 * (float)(1.0 - cd[j].ContactTime) +
             op1 * (float)(cd[j].ContactTime);
        s1 = op0 * (float)(1.0 - cd[j + 1].ContactTime) +
             op1 * (float)(cd[j + 1].ContactTime);

        mins.minof(s0, s1);
        maxs.maxof(s0, s1);

        // PrecisionPb: extend a little this edge. This is important for special
        // case like borders on zones.
        if (mins.x - maxs.x == 0)
        ...
        for (entry = 0; entry < _EdgeEntries.size(); ++entry) {
          if (_EdgeEntries[entry].EdgeId == edgeId &&
            ...
            if (_EdgeEntries[entry].ChainId != chainId ||
              ...
              nlwarning("In NLPACS::CEdgeQuad::build()");
              nlerror("exterior edge %d has different interior linkage",
                      edgeId);
            }
        ...
        for (y = y0; y < y1; y++) {
          for (x = x0; x < x1; x++) {
            ...
            for (it = tempQuad[y * _Width + x].begin();
                 it != tempQuad[y * _Width + x].end(); ++it)
              if (entry == *it)
                break;
  ...
  for (i = 0; i < (sint)_EdgeEntries.size(); ++i) {
    if (edges[_EdgeEntries[i].EdgeId].Link != -1 &&
        (_EdgeEntries[i].Interior.RetrieverInstanceId == -1 ||
         _EdgeEntries[i].Interior.SurfaceId == -1 ||
         _EdgeEntries[i].Exterior.RetrieverInstanceId == -1 ||
         _EdgeEntries[i].Exterior.SurfaceId == -1)) {
      nlwarning(
          "In NLPACS::CEdgeQuad::build(): exterior door %d has corrupted link",
          i);
    } else if (edges[_EdgeEntries[i].EdgeId].Link != -1) {
      nlinfo("Inst=%d ExtEdge=%d IntInst=%d IntSurf=%d IntChain=%d ExtInst=%d "
             "ExtSurf=%d",
             thisInstance, _EdgeEntries[i].EdgeId,
             _EdgeEntries[i].Interior.RetrieverInstanceId,
             _EdgeEntries[i].Interior.SurfaceId, _EdgeEntries[i].ChainId,
             _EdgeEntries[i].Exterior.RetrieverInstanceId,
             _EdgeEntries[i].Exterior.SurfaceId);
    }
  }
...
// ***************************************************************************
sint CEdgeQuad::selectEdges(const NLMISC::CAABBox &bbox,
...
sint CEdgeQuad::selectEdges(CVector start, CVector end,
                            CCollisionSurfaceTemp &cst) const {
  sint nRes = 0;
  sint i;
  uint16 *indexLUT = cst.OChainLUT;
  ...
  float minx = _X * _QuadElementSize, miny = _Y * _QuadElementSize,
  ...
  if (start.x < minx) {
    start.y =
        start.y + (end.y - start.y) * (minx - start.x) / (end.x - start.x);
    start.x = minx;
  }

  if (start.y < miny) {
    start.x =
        start.x + (end.x - start.x) * (miny - start.y) / (end.y - start.y);
    start.y = miny;
  }
...
void CEdgeQuad::serial(NLMISC::IStream &f) {
  ...
  */
  ...
  // Since we have only uint16 (see CEdgeChainEntry), serial them in a single
  // block.

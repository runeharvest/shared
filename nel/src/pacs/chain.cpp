...
//
void NLPACS::COrderedChain::traverse(sint from, sint to, bool forward,
                                     vector<NLPACS::CVector2s> &path) const {
  sint i;
  if (forward) {
    if (from < 0)
      from = 0;
    if (to < 0)
      to = (sint)_Vertices.size() - 1;

    for (i = from + 1; i <= to; ++i)
      path.push_back(_Vertices[i]);
  } else {
    if (from < 0)
      from = (sint)_Vertices.size() - 2;
    if (to < 0)
      to = -1;
...
float NLPACS::COrderedChain::distance(const CVector &position) const {
  ...
  for (i = 0; i + 1 < _Vertices.size(); ++i) {
    CVector2f a = _Vertices[i].unpack(), b = _Vertices[i + 1].unpack();
...
// thisId is the current id of the CChain, and edge is the number of the edge
// the CChain belongs to (-1 if none.)
void NLPACS::CChain::make(const vector<CVector> &vertices, sint32 left,
                          sint32 right, vector<COrderedChain> &chains,
                          uint16 thisId, vector<COrderedChain3f> &fullChains,
                          vector<uint> &useOChainId) {
  ...
  while (first < (sint)vertices.size() - 1) {
    ...
    if (forward)
      for (; last < (sint)vertices.size() &&
             isStrictlyLess(vertices[last - 1], vertices[last]);
           ++last)
        ;
    else
      for (; last < (sint)vertices.size() &&
             isStrictlyGreater(vertices[last - 1], vertices[last]);
           ++last)
        ;
    --last;
    ...
    if (useOChainId.empty()) {
      ...
      if (subChainId > 65535)
        nlerror("in NLPACS::CChain::make(): reached the maximum number of "
                "ordered chains");
...
void NLPACS::CChain::unify(vector<NLPACS::COrderedChain> &ochains) {
  ...
  snap = (ochains[_SubChains[0]].isForward())
             ? ochains[_SubChains[0]]._Vertices.back()
             : ochains[_SubChains[0]]._Vertices.front();

  for (i = 1; i < _SubChains.size(); ++i) {
    if (ochains[_SubChains[i]].isForward()) {
      if (ochains[_SubChains[i]]._Vertices.front() != snap)
        nlwarning("ochain %d and %d are not stuck together", _SubChains[i - 1],
                  _SubChains[i]);
...
//
void NLPACS::CChain::setStartVector(const NLPACS::CVector2s &v,
                                    vector<NLPACS::COrderedChain> &ochains) {
...
//
void NLPACS::CChain::setStopVector(const NLPACS::CVector2s &v,
                                   vector<NLPACS::COrderedChain> &ochains) {
...
//
NLPACS::CVector2s
NLPACS::CChain::getStartVector(vector<NLPACS::COrderedChain> &ochains) {
  if (ochains[_SubChains.front()].isForward())
...
//
NLPACS::CVector2s
NLPACS::CChain::getStopVector(vector<NLPACS::COrderedChain> &ochains) {
  if (ochains[_SubChains.back()].isForward())

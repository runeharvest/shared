...
NLPACS::CRetrieverInstance::CRetrieverInstance() { reset(); }
...
void NLPACS::CRetrieverInstance::init(const CLocalRetriever &retriever) {
  ...
  /*
      ...
      {
          CVector	pos =
     getGlobalPosition(retriever.getSurfaces()[i].getCenter());
...
void NLPACS::CRetrieverInstance::make(sint32 instanceId, sint32 retrieverId,
  ...
  if (_InstanceId != -1 || _RetrieverId != -1) {
    ...
    nlwarning("_InstanceId=%d _RetrieverId=%d _Orientation=%d", _InstanceId,
              _RetrieverId, _Orientation);
    nlwarning("instanceId=%d retrieverId=%d orientation=%d", instanceId,
              retrieverId, orientation);
...
void NLPACS::CRetrieverInstance::initEdgeQuad(NLPACS::CGlobalRetriever &gr) {
  ...
  if (lr.getType() != CLocalRetriever::Interior) {
    nlerror("Attempt to init the edgequad of instance %d whereas local "
            "retriever %d is not interior",
            _InstanceId, _RetrieverId);
  }

  // build the edge quad
  _ExteriorEdgeQuad.build(lr.getExteriorMesh(), gr, gr.getInternalCST(),
                          _InstanceId);
}
...
void NLPACS::CRetrieverInstance::linkEdgeQuad(NLPACS::CGlobalRetriever &gr) {
  ...
  for (i = 0; i < ee.size(); ++i) {
    ...
    if (edge.Link != -1) {
      ...
      if (link.SurfaceId != 0xFFFF && link.ChainId != 0xFFFF &&
          link.BorderChainId != 0xFFFF) {
        CRetrieverInstance::CLink &borderLink =
            _BorderChainLinks[link.BorderChainId];
        ...
        if ((borderLink.Instance == 0xFFFF && borderLink.SurfaceId == 0xFFFF) ||
            (borderLink.Instance ==
                 (uint16)(ee[i].Exterior.RetrieverInstanceId) &&
             borderLink.SurfaceId == (uint16)(ee[i].Exterior.SurfaceId))) {
          borderLink.Instance = (uint16)(ee[i].Exterior.RetrieverInstanceId);
          borderLink.SurfaceId = (uint16)(ee[i].Exterior.SurfaceId);
        } else {
          nlwarning("Instance %d, borderLink %d: link already set to inst=%d, "
                    "surf=%d, try to set to inst=%d, surf=%d",
                    _InstanceId, link.BorderChainId, borderLink.Instance,
                    borderLink.SurfaceId,
                    (uint16)(ee[i].Exterior.RetrieverInstanceId),
                    (uint16)(ee[i].Exterior.SurfaceId));
        }
      }
    }
  }
...
void NLPACS::CRetrieverInstance::link(
    CRetrieverInstance &neighbor, const vector<CLocalRetriever> &retrievers) {
  uint i, j;
  for (i = 0; i < _Neighbors.size(); ++i)
  ...
  vector<pair<CVector, CVector>> chainTips, nChainTips;
  ...
  */
  for (i = 0; i < nBorderChains.size(); ++i)
    nChainTips.push_back(
        make_pair(nRetriever.getStartVector(nBorderChains[i]) + translation,
                  nRetriever.getStopVector(nBorderChains[i]) + translation));

  for (i = 0; i < borderChains.size(); ++i) {
    // if the chain is already linked, just step
    if (_BorderChainLinks[i].Instance != 0xFFFF ||
    ...
    for (j = 0; j < nBorderChains.size(); ++j) {
      if (neighbor._BorderChainLinks[j].Instance != 0xFFFF ||
          neighbor._BorderChainLinks[j].BorderChainId != 0xFFFF ||
          neighbor._BorderChainLinks[j].ChainId != 0xFFFF ||
          neighbor._BorderChainLinks[j].SurfaceId != 0xFFFF)
        continue;

      float d = (chainTips[i].first - nChainTips[j].second).norm() +
                (chainTips[i].second - nChainTips[j].first).norm();
      if (d < bestDist) {
        bestDist = d;
        best = j;
      }
    ...
    _BorderChainLinks[i].ChainId =
        nBorderChains[_BorderChainLinks[i].BorderChainId];
    _BorderChainLinks[i].SurfaceId =
        (uint16)nChains[_BorderChainLinks[i].ChainId].getLeft();

    neighbor._BorderChainLinks[best].Instance = (uint16)_InstanceId;
    neighbor._BorderChainLinks[best].BorderChainId = (uint16)i;
    neighbor._BorderChainLinks[best].ChainId =
        borderChains[neighbor._BorderChainLinks[best].BorderChainId];
    neighbor._BorderChainLinks[best].SurfaceId =
        (uint16)chains[neighbor._BorderChainLinks[best].ChainId].getLeft();
  }
...
void NLPACS::CRetrieverInstance::retrievePosition(
    const NLMISC::CVector &estimated, const CLocalRetriever &retriever,
    CCollisionSurfaceTemp &cst, bool sortByDistance) const
// NLPACS::CLocalRetriever::CLocalPosition
// NLPACS::CRetrieverInstance::retrievePosition(const NLMISC::CVector
// &estimated, const CLocalRetriever &retriever, CCollisionSurfaceTemp &cst)
// const
{
  /*
      ...
      for (i=0; i<cst.PossibleSurfaces.size(); ++i)
      {
          ...
          if (cst.SurfaceLUT[surf].second != 0)
          {
              ...
              case CLocalRetriever::Landscape:
                  ...
                  leaf =
     retriever.getSurfaces()[surf].getQuadTree().getLeaf(localEstimated);
              ...
              // if it is closer to the estimation than the previous
     remembered... float	distance =
     (float)fabs(localEstimated.z-meanHeight); if (distance < bestDistance &&
     lfound)
              {
      ...
      if (bestSurf != -1)
      {
          ...
          retrieved.Estimation = CVector(localEstimated.x, localEstimated.y,
     bestHeight);
      }
      else
      {
...
void NLPACS::CRetrieverInstance::retrievePosition(
    const NLMISC::CVectorD &estimated, const CLocalRetriever &retriever,
    CCollisionSurfaceTemp &cst, bool sortByDistance) const {
  CVector localEstimated;

  //	nldebug("PACS: retrievePosition in instance %d (retriever %d)",
  //_InstanceId, _RetrieverId);
  ...
  case CLocalRetriever::Landscape:
    ...
    for (i = 0; i < cst.PossibleSurfaces.size(); ++i) {
      ...
      //			nldebug("PACS: surface %d: count %d", surf,
      //cst.SurfaceLUT[surf].Counter);
      ...
      if (cst.SurfaceLUT[surf].Counter == 2) {
        ...
        /*
                        ...
                        // search in the surface's quad tree for the actual
           height leaf =
           retriever.getSurfaces()[surf].getQuadTree().getLeaf(localEstimated);
                        // if there is no acceptable leaf, just give up
                        if (leaf != NULL)
                        {
                            meanHeight = leaf->getMaxHeight();
                            //meanHeight =
           retriever.getSurfaces()[surf].getQuadTree().getInterpZ(localEstimated);

                            // if it is closer to the estimation than the
           previous remembered... found = true; float	distance =
           (float)fabs(localEstimated.z-meanHeight);
                            cst.SortedSurfaces.push_back(CCollisionSurfaceTemp::CDistanceSurface(distance,
           (uint16)surf, (uint16)_InstanceId,
           cst.SurfaceLUT[surf].FoundCloseEdge));
                        }
        */
        ...
        found = true;
        float distance = sortByDistance
                             ? (float)fabs(localEstimated.z - meanHeight)
                             : meanHeight;
        cst.SortedSurfaces.push_back(CCollisionSurfaceTemp::CDistanceSurface(
            distance, (uint16)surf, (uint16)_InstanceId,
            cst.SurfaceLUT[surf].FoundCloseEdge));
      } else if (cst.SurfaceLUT[surf].Counter != 0) {
        nlwarning("PACS: unexpected surface (%d) count (%d) at instance %d "
                  "(pos=(%f,%f,%f))",
                  surf, cst.SurfaceLUT[surf].Counter, _InstanceId, estimated.x,
    ...
    for (i = 0; i < cst.PossibleSurfaces.size(); ++i) {
      ...
      if (cst.SurfaceLUT[surf].Counter == 2) {
        ...
        if (lfound) {
          ...
          found = true;
          float distance = sortByDistance
                               ? (float)fabs(localEstimated.z - lp.Estimation.z)
                               : lp.Estimation.z;
          cst.SortedSurfaces.push_back(CCollisionSurfaceTemp::CDistanceSurface(
              distance, (uint16)surf, (uint16)_InstanceId,
              cst.SurfaceLUT[surf].FoundCloseEdge));
...
void NLPACS::CRetrieverInstance::snapToInteriorGround(
    NLPACS::ULocalPosition &position,
    const NLPACS::CLocalRetriever &retriever) const {
  bool lfound;
  retriever.snapToInteriorGround(position, lfound);
}

void NLPACS::CRetrieverInstance::snap(
    NLPACS::ULocalPosition &position,
    const NLPACS::CLocalRetriever &retriever) const {
  if (_Type == CLocalRetriever::Landscape) {
    // search in the surface's quad tree for the actual height
    //		position.Estimation.z =
    //retriever.getSurfaces()[position.Surface].getQuadTree().getInterpZ(position.Estimation);

    position.Estimation.z = retriever.getHeight(position);

    /*
            const CQuadLeaf	*leaf =
       retriever.getSurfaces()[position.Surface].getQuadTree().getLeaf(position.Estimation);
            // if there is no acceptable leaf, just give up
            if (leaf != NULL)
            {
            ...
            {
                nlwarning("PACS: couldn't snap position (%f,%f,%f) on surface %d
       instance %d", position.Estimation.x, position.Estimation.y,
       position.Estimation.z, position.Surface, _InstanceId);
            }
    */
...
CVector NLPACS::CRetrieverInstance::getLocalPosition(
    const CVector &globalPosition) const {
  switch (_Orientation) {
  default:
    nlwarning("in NLPACS::CRetrieverInstance::getLocalPosition()");
    nlerror("unexpected orientation value (%d)", _Orientation);
  case 0:
    return CVector(+globalPosition.x - _Origin.x, +globalPosition.y - _Origin.y,
                   globalPosition.z - _Origin.z);
    break;
  case 1:
    return CVector(+globalPosition.y - _Origin.y, -globalPosition.x + _Origin.x,
                   globalPosition.z - _Origin.z);
    break;
  case 2:
    return CVector(-globalPosition.x + _Origin.x, -globalPosition.y + _Origin.y,
...
CVector NLPACS::CRetrieverInstance::getLocalPosition(
    const CVectorD &globalPosition) const {
  switch (_Orientation) {
  default:
    nlwarning("in NLPACS::CRetrieverInstance::getLocalPosition()");
    nlerror("unexpected orientation value (%d)", _Orientation);
  case 0:
    return CVector((float)(+globalPosition.x - _Origin.x),
                   (float)(+globalPosition.y - _Origin.y),
                   (float)(globalPosition.z - _Origin.z));
    break;
  case 1:
    return CVector((float)(+globalPosition.y - _Origin.y),
                   (float)(-globalPosition.x + _Origin.x),
                   (float)(globalPosition.z - _Origin.z));
    break;
  case 2:
    return CVector((float)(-globalPosition.x + _Origin.x),
...
CVector NLPACS::CRetrieverInstance::getGlobalPosition(
  ...
  default:
    ...
    nlerror("unexpected orientation value (%d)", _Orientation);
  case 0:
    return CVector(+localPosition.x + _Origin.x, +localPosition.y + _Origin.y,
                   localPosition.z + _Origin.z);
    break;
  case 1:
    return CVector(-localPosition.y + _Origin.x, +localPosition.x + _Origin.y,
                   localPosition.z + _Origin.z);
    break;
  case 2:
    return CVector(-localPosition.x + _Origin.x, -localPosition.y + _Origin.y,
...
CVectorD NLPACS::CRetrieverInstance::getDoubleGlobalPosition(
  ...
  default:
    ...
    nlerror("unexpected orientation value (%d)", _Orientation);
  case 0:
    return CVectorD(+(double)localPosition.x + (double)_Origin.x,
                    +(double)localPosition.y + (double)_Origin.y,
                    (double)localPosition.z + (double)_Origin.z);
    break;
  case 1:
    return CVectorD(-(double)localPosition.y + (double)_Origin.x,
                    +(double)localPosition.x + (double)_Origin.y,
                    (double)localPosition.z + (double)_Origin.z);
    break;
  case 2:
    return CVectorD(-(double)localPosition.x + (double)_Origin.x,
...
// ***************************************************************************
void NLPACS::CRetrieverInstance::testExteriorCollision(
    NLPACS::CCollisionSurfaceTemp &cst, const CAABBox &bboxMove,
    const CVector2f &transBase,
    const NLPACS::CLocalRetriever &retriever) const {
  sint i;
  ...
  for (i = 0; i < nEei; i++) {
    ...
    */
    ...
    CVector2f p1 =
        CVector2f(retriever._ExteriorMesh.getEdge(eee.EdgeId + 1).Start);
  ...
  for (i = firstChainAdded; i < (sint)cst.CollisionChains.size(); i++) {
    ...
    edgeLUT[ccId] = 0xFFFF;
    cst.CollisionChains[i].ChainId =
        _ExteriorEdgeQuad.getEdgeEntry(ccId).EdgeId;
  }
}
...
void NLPACS::CRetrieverInstance::resetBorderChainLinks(
    const vector<uint> &links) {
  uint i;

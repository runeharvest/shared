	...
	for (i = 0; i < ee.size(); ++i)
	{
		...
		if (edge.Link != -1)
		{
			...
			if (link.SurfaceId != 0xFFFF && link.ChainId != 0xFFFF && link.BorderChainId != 0xFFFF)
			{
				...
				if ((borderLink.Instance == 0xFFFF && borderLink.SurfaceId == 0xFFFF) || (borderLink.Instance == (uint16)(ee[i].Exterior.RetrieverInstanceId) && borderLink.SurfaceId == (uint16)(ee[i].Exterior.SurfaceId)))
				{
    ...
    const vector<CLocalRetriever> &retrievers)
{
	...
	*/
	...
	for (i = 0; i < borderChains.size(); ++i)
	{
		...
		if (_BorderChainLinks[i].Instance != 0xFFFF || _BorderChainLinks[i].BorderChainId != 0xFFFF || _BorderChainLinks[i].ChainId != 0xFFFF || _BorderChainLinks[i].SurfaceId != 0xFFFF)
			continue;
		...
		{
			if (neighbor._BorderChainLinks[j].Instance != 0xFFFF || neighbor._BorderChainLinks[j].BorderChainId != 0xFFFF || neighbor._BorderChainLinks[j].ChainId != 0xFFFF || neighbor._BorderChainLinks[j].SurfaceId != 0xFFFF)
				continue;

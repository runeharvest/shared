...
Doc:
    ...
    Their moves are evaluate all at once. All the collisions found are time
sorted in a time orderin table (_TimeOT). While the table is not empty, the
first collision occurred in time is solved and If a collision is found,
reaction() is called.
...
CMoveContainer::~CMoveContainer() { clear(); }
...
void CMoveContainer::init(double xmin, double ymin, double xmax, double ymax,
                          uint widthCellCount, uint heightCellCount,
                          double primitiveMaxSize, uint8 numWorldImage,
...
void CMoveContainer::init(CGlobalRetriever *retriever, uint widthCellCount,
                          uint heightCellCount, double primitiveMaxSize,
  ...
  // Init
  init(xmin, ymin, xmax, ymax, widthCellCount, heightCellCount,
       primitiveMaxSize, numWorldImage, maxIteration, otSize);
...
void CMoveContainer::evalCollision(double deltaTime, uint8 worldImage) {
  ...
  while (nextCollision) {
    ...
    if (!nextCollision->isCollisionAgainstStatic()) {
      ...
      CCollisionOTDynamicInfo *info =
          static_cast<CCollisionOTDynamicInfo *>(nextCollision);
...
bool CMoveContainer::testMove(UMovePrimitive *primitive, const CVectorD &speed,
                              double deltaTime, uint8 worldImage,
...
//	checkSortedList ();
  ...
  result = evalOneTerrainCollision(0, prim, primitiveWorldImage, true,
                                   testMoveValid, NULL, contactNormal);
  ...
  if (!result) {
    ...
    while (ite != _StaticWorldImage.end()) {
      ...
      result =
          evalOnePrimitiveCollision(0, prim, *ite, primitiveWorldImage, true,
                                    true, testMoveValid, NULL, contactNormal);
  ...
  if ((!result) &&
      (_StaticWorldImage.find(worldImage) == _StaticWorldImage.end()))
    result = evalOnePrimitiveCollision(0, prim, worldImage, primitiveWorldImage,
                                       true, false, testMoveValid, NULL,
                                       contactNormal);
...
void CMoveContainer::updateCells(CMovePrimitive *primitive, uint8 worldImage) {
  ...
  if (wI->getBBXMax() - wI->getBBXMin() > _CellWidth) {
    nlwarning("Primitives have moved more than a cell, width: %f.",
              (float)(wI->getBBXMax() - wI->getBBXMin()));
  }

  // Check BB height not too large
  if (wI->getBBYMax() - wI->getBBYMin() > _CellHeight) {
    nlwarning("Primitives have moved more than a cell, height: %f.",
              (float)(wI->getBBYMax() - wI->getBBYMin()));
  }
#endif
#endif
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (elm) {
      ...
      // Must remove it ?
      if ((elm->X < minx) || (elm->X > maxx) || (elm->Y < miny) ||
          (elm->Y > maxy)) {
        // Yes remove it
        ...
        // Update position
#ifndef TEST_CELL
        _VectorCell[worldImage][elm->X + elm->Y * _CellCountWidth]
            .updateSortedLists(elm, worldImage);
#endif
  ...
  for (y = miny; y <= (int)maxy; y++)
    for (x = minx; x <= (int)maxx; x++) {
      ...
      if (!found[i]) {
        ...
        // Add it in the list
        wI->addMoveElement(_VectorCell[worldImage][x + y * _CellCountWidth],
                           (uint16)x, (uint16)y, cx, cy, primitive, *this,
                           worldImage);
      }
...
void CMoveContainer::getCells(CMovePrimitive *primitive, uint8 worldImage,
                              uint8 primitiveWorldImage,
  ...
  for (y = miny; y <= (int)maxy; y++)
    for (x = minx; x <= (int)maxx; x++) {
      ...
      if (pcx < cx) {
        // In the left
        elementArray[i]->NextX =
            _VectorCell[worldImage][x + y * _CellCountWidth].getFirstX();
        elementArray[i]->PreviousX = NULL;
      } else {
        // In the right
        elementArray[i]->PreviousX =
            _VectorCell[worldImage][x + y * _CellCountWidth].getLastX();
...
bool CMoveContainer::evalOneTerrainCollision(
    double beginTime, CMovePrimitive *primitive, uint8 primitiveWorldImage,
  ...
  if (wI->getInitTime() != beginTime) {
    nlwarning("PACS: evalOneTerrainCollision() failure, wI->getInitTime() [%f] "
              "!= beginTime [%f]",
              wI->getInitTime(), beginTime);
    return false;
  }
  ...
  if (_Retriever) {
    ...
    const TCollisionSurfaceDescVector *result = wI->evalCollision(
        *_Retriever, _SurfaceTemp, _TestTime, _MaxTestIteration, *primitive);
    if (result) {
      ...
      for (uint c = 0; c < size; c++) {
        ...
        double contactTime =
            (_DeltaTime - beginTime) * desc.ContactTime + beginTime;

        /*
         *  If beginTime is 0.999999999 and desc.ContactTime<1.0, contactTime
         *will be 1.0. In this case, we force contactTime to be beginTime to
         *avoid collision at time == 1.0.
         **/
        if ((contactTime >= 1.0) && (beginTime < 1.0) &&
            (desc.ContactTime < 1.0))
        ...
        const CRetrievableSurface *surf =
            _Retriever->getSurfaceById(desc.ContactSurface);
...
bool CMoveContainer::evalOnePrimitiveCollision(
    double beginTime, CMovePrimitive *primitive, uint8 worldImage,
  ...
  // nlassert (wI->getInitTime()==beginTime);
  if (wI->getInitTime() != beginTime) {
    nlwarning("PACS: evalOnePrimitiveCollision() failure, wI->getInitTime() "
              "[%f] != beginTime [%f]",
              wI->getInitTime(), beginTime);
    return false;
  }
  ...
  for (uint i = 0; i < 4; i++) {
    ...
    if (elm) {
      ...
      while (other &&
             (wI->getBBXMin() -
                  other->Primitive->getWorldImage(worldImage)->getBBXMin() <
              _PrimitiveMaxSize)) {
        // Other primitive
        CMovePrimitive *otherPrimitive = other->Primitive;
        CPrimitiveWorldImage *otherWI =
            otherPrimitive->getWorldImage(worldImage);
        nlassert(otherPrimitive != primitive);

        // Continue the check if the other primitive is not int the modified
        // list or if its pointer is higher than primitive
        if (singleTest || ((!otherWI->isInModifiedListFlag()) ||
                           (primitive < otherPrimitive))) {
          // Look if valid in X
          if (wI->getBBXMin() < otherWI->getBBXMax()) {
            // Look if valid in Y
            if ((wI->getBBYMin() < otherWI->getBBYMax()) &&
                (otherWI->getBBYMin() < wI->getBBYMax())) {
              // If not already in collision with this primitive
              if (!primitive->isInCollision(otherPrimitive)) {
                if (evalPrimAgainstPrimCollision(
                        beginTime, primitive, otherPrimitive, wI, otherWI,
                        testMove, primitiveWorldImage, worldImage,
                        secondIsStatic, dynamicColInfo, contactNormal)) {
                  if (testMove)
                    return true;
                  found = true;
                }
      ...
      while (other &&
        ...
        // Continue the check if the other primitive is not in the modified list
        // or if its pointer is higher than primitive
        if (singleTest || ((!otherWI->isInModifiedListFlag()) ||
                           (primitive < otherPrimitive))) {
          // Look if valid in Y
          if ((wI->getBBYMin() < otherWI->getBBYMax()) &&
            ...
            if (!primitive->isInCollision(otherPrimitive)) {
              if (evalPrimAgainstPrimCollision(
                      beginTime, primitive, otherPrimitive, wI, otherWI,
                      testMove, primitiveWorldImage, worldImage, secondIsStatic,
                      dynamicColInfo, contactNormal)) {
                if (testMove)
                  return true;
                found = true;
              }
...
bool CMoveContainer::evalPrimAgainstPrimCollision(
    double beginTime, CMovePrimitive *primitive, CMovePrimitive *otherPrimitive,
    ...
    CCollisionOTDynamicInfo *dynamicColInfo, CVectorD * /* contactNormal */) {
  ...
  // Collision
  CCollisionDesc desc;
  if (wI->evalCollision(*otherWI, desc, beginTime, _DeltaTime, _TestTime,
                        _MaxTestIteration, firstTime, lastTime, *primitive,
                        *otherPrimitive)) {
    // Enter or exit
    bool enter =
        (beginTime <= firstTime) && (firstTime < _DeltaTime) &&
        ((primitive->getTriggerType() & UMovePrimitive::EnterTrigger) ||
         (otherPrimitive->getTriggerType() & UMovePrimitive::EnterTrigger));
    bool exit =
        (beginTime <= lastTime) && (lastTime < _DeltaTime) &&
        ((primitive->getTriggerType() & UMovePrimitive::ExitTrigger) ||
         (otherPrimitive->getTriggerType() & UMovePrimitive::ExitTrigger));
    bool overlap =
        (firstTime <= beginTime) && (lastTime > _DeltaTime) &&
        ((primitive->getTriggerType() & UMovePrimitive::OverlapTrigger) ||
         (otherPrimitive->getTriggerType() & UMovePrimitive::OverlapTrigger));
    bool contact =
        (beginTime < ((firstTime + lastTime) / 2)) && (firstTime <= _DeltaTime);
    bool collision =
        contact && (primitive->isObstacle() && otherPrimitive->isObstacle());
    ...
    /**
     * Raise Trigger !
     * For collisionnable primitives, trigger are raised here (in reaction)
     * because this is the moment we are sure the collision happened.
     *
     * For non collisionable primitves, the trigger is raised at collision time
     * because without OT, we can't stop evaluating collision on triggers.
     */
    if (primitive->isNonCollisionable() && (enter || exit || overlap)) {
      ...
      // If the other primitive is not an obstacle, skip it because it will
      // re-generate collisions.
      if (!collision)
        return false;
    }
    ...
    if (contact || enter || exit || overlap)
      newCollision(primitive, otherPrimitive, desc, contact, enter, exit,
                   overlap, firstWorldImage, secondWorldImage, secondIsStatic,
                   dynamicColInfo);
...
void CMoveContainer::evalAllCollisions(double beginTime, uint8 worldImage) {
  ...
  while (primitive) {
    ...
    found |= evalOneTerrainCollision(beginTime, primitive, primitiveWorldImage,
                                     false, testMoveValid, NULL, NULL);
    ...
    if (primitive->getCollisionMask()) {
      ...
      while (ite != _StaticWorldImage.end()) {
        // Eval in this world image
        found |= evalOnePrimitiveCollision(beginTime, primitive, *ite,
                                           primitiveWorldImage, false, true,
                                           testMoveValid, NULL, NULL);
    ...
    if (primitive->getCollisionMask()) {
      ...
      if (_StaticWorldImage.find(worldImage) == _StaticWorldImage.end())
        found |= evalOnePrimitiveCollision(beginTime, primitive, worldImage,
                                           primitiveWorldImage, false, false,
                                           testMoveValid, NULL, NULL);
    }
...
void CMoveContainer::newCollision(CMovePrimitive *first, CMovePrimitive *second,
                                  const CCollisionDesc &desc, bool collision,
                                  bool enter, bool exit, bool inside,
                                  uint firstWorldImage, uint secondWorldImage,
                                  bool secondIsStatic,
                                  CCollisionOTDynamicInfo *dynamicColInfo) {
  //	H_AUTO(PACS_MC_newCollision_short);

  nlassert((dynamicColInfo && first->isNonCollisionable()) ||
           (!dynamicColInfo && first->isCollisionable()));

  if (dynamicColInfo) {
    dynamicColInfo->init(first, second, desc, collision, enter, exit, inside,
    ...
    if (index < (int)_OtSize) {
      ...
      info->init(first, second, desc, collision, enter, exit, inside,
                 uint8(firstWorldImage), uint8(secondWorldImage),
                 secondIsStatic);
      ...
      if (index >= (int)_TimeOT.size()) {
        nlwarning("PACS: newCollision() failure, index [%d] >= "
                  "(int)_TimeOT.size() [%d], clamped to max",
                  index, (int)_TimeOT.size());
...
void CMoveContainer::newCollision(CMovePrimitive *first,
                                  const CCollisionSurfaceDesc &desc,
                                  uint8 worldImage, double beginTime,
                                  CCollisionOTStaticInfo *staticColInfo) {
  //	H_AUTO(PACS_MC_newCollision_long);

  // Check
  nlassert(_Retriever);
  nlassert((staticColInfo && first->isNonCollisionable()) ||
           (!staticColInfo && first->isCollisionable()));
  ...
  */
  ...
  if (staticColInfo) {
    ...
    UGlobalPosition endPosition =
        _Retriever->doMove(wI->getGlobalPosition(), wI->getDeltaPosition(),
...
void CMoveContainer::newTrigger(CMovePrimitive *first, CMovePrimitive *second,
                                const CCollisionDesc &desc, uint triggerType) {
  // Element index
...
CMovePrimitive *CMoveContainer::allocatePrimitive(uint8 firstWorldImage,
                                                  uint8 numWorldImage) {
  // Simply allocate
...
UMovePrimitive *
CMoveContainer::addNonCollisionablePrimitive(const UMovePrimitive *copyFrom) {
...
void CMoveContainer::removeNCFromModifiedList(CMovePrimitive *primitive,
                                              uint8 worldImage) {
...
void CMoveContainer::unlinkMoveElement(CMoveElement *element,
  ...
  // Unlink it
  CMoveCell &cell =
      _VectorCell[worldImage][element->X + element->Y * _CellCountWidth];
...
void CMoveContainer::reaction(const CCollisionOTInfo &first) {
  ...
  if (first.isCollisionAgainstStatic()) {
    ...
    // Cast
    const CCollisionOTStaticInfo *staticInfo =
        safe_cast<const CCollisionOTStaticInfo *>(&first);
    ...
    // Dynamic collision
    wI->reaction(
        staticInfo->getCollisionDesc(), staticInfo->getGlobalPosition(),
        *_Retriever, staticInfo->getDeltaTime(), _DeltaTime,
        *staticInfo->getPrimitive(), *this, staticInfo->getWorldImage());
  } else {
    // Cast
    const CCollisionOTDynamicInfo *dynInfo =
        safe_cast<const CCollisionOTDynamicInfo *>(&first);
    ...
    if (dynInfo->getFirstPrimitive()->isNonCollisionable())
      ...
      firstWI = dynInfo->getFirstPrimitive()->getWorldImage(
          dynInfo->getFirstWorldImage());
    ...
    if (dynInfo->getSecondPrimitive()->isNonCollisionable())
      ...
      secondWI = dynInfo->getSecondPrimitive()->getWorldImage(
          dynInfo->getSecondWorldImage());

    // Dynamic collision
    firstWI->reaction(
        *secondWI, dynInfo->getCollisionDesc(), _Retriever, _SurfaceTemp,
        dynInfo->isCollision(), *dynInfo->getFirstPrimitive(),
        *dynInfo->getSecondPrimitive(), this, dynInfo->getFirstWorldImage(),
    ...
    if (dynInfo->getFirstPrimitive()->isCollisionable()) {
      if (dynInfo->getFirstPrimitive()->isTriggered(
              *dynInfo->getSecondPrimitive(), dynInfo->isEnter(),
              dynInfo->isExit())) {
        if (dynInfo->isEnter())
          newTrigger(dynInfo->getFirstPrimitive(),
                     dynInfo->getSecondPrimitive(), dynInfo->getCollisionDesc(),
                     UTriggerInfo::In);
        if (dynInfo->isExit())
          newTrigger(dynInfo->getFirstPrimitive(),
                     dynInfo->getSecondPrimitive(), dynInfo->getCollisionDesc(),
...
void CMoveContainer::duplicateWorldImage(uint8 source, uint8 dest) {
  ...
  for (i = 0; i < cellCount; i++) {
    ...
    while (elm) {
      ...
      wIDest->addMoveElementendOfList(_VectorCell[dest][i], elm->X, elm->Y,
                                      elm->Primitive, *this);
...
UMoveContainer *UMoveContainer::createMoveContainer(
    double xmin, double ymin, double xmax, double ymax, uint widthCellCount,
    uint heightCellCount, double primitiveMaxSize, uint8 numWorldImage,
    uint maxIteration, uint otSize) {

  // Create a CMoveContainer
  return new CMoveContainer(xmin, ymin, xmax, ymax, widthCellCount,
                            heightCellCount, primitiveMaxSize, numWorldImage,
                            maxIteration, otSize);
}
...
UMoveContainer *UMoveContainer::createMoveContainer(
    UGlobalRetriever *retriever, uint widthCellCount, uint heightCellCount,
    double primitiveMaxSize, uint8 numWorldImage, uint maxIteration,
    uint otSize) {
  ...
  // Create a CMoveContainer
  return new CMoveContainer(r, widthCellCount, heightCellCount,
                            primitiveMaxSize, numWorldImage, maxIteration,
                            otSize);
}
...
// ***************************************************************************
void CMoveContainer::addCollisionnablePrimitiveBlock(
    UPrimitiveBlock *pb, uint8 firstWorldImage, uint8 numWorldImage,
    std::vector<UMovePrimitive *> *primitives, float orientation,
    const NLMISC::CVector &position, bool dontSnapToGround /* = false*/,
  ...
  for (prim = 0; prim < block->Primitives.size(); prim++) {
    ...
    UMovePrimitive *primitive =
        addCollisionablePrimitive(firstWorldImage, numWorldImage);
    ...
    // For each world image
    uint wI;
    for (wI = firstWorldImage; wI < (uint)(firstWorldImage + numWorldImage);
         wI++) {
      ...
      CVector finalPos;
      finalPos.x = cosa * desc.Position.x * scale.x -
                   sina * desc.Position.y * scale.y + position.x;
      finalPos.y = sina * desc.Position.x * scale.x +
                   cosa * desc.Position.y * scale.y + position.y;
      finalPos.z = desc.Position.z * scale.z + position.z;

      // Set the primtive orientation
      if (desc.Type == UMovePrimitive::_2DOrientedBox)
        primitive->setOrientation(
            (float)fmod((float)(desc.Orientation + orientation),
                        (float)(2.0f * Pi)),
            uint8(wI));
...
bool CMoveContainer::loadCollisionablePrimitiveBlock(
    const char *filename, uint8 firstWorldImage, uint8 numWorldImage,
    std::vector<UMovePrimitive *> *primitives, float orientation,
    const NLMISC::CVector &position, bool dontSnapToGround /*= false*/) {
  ...
  if (file.open(filename)) {
    ...
    if (input.init(file)) {
      ...
      // add primitives
      addCollisionnablePrimitiveBlock(&block, firstWorldImage, numWorldImage,
                                      primitives, orientation, position,
                                      dontSnapToGround);
...
// ***************************************************************************
void CMoveContainer::getPrimitives(
    std::vector<const UMovePrimitive *> &dest) const {
...
// ***************************************************************************
void UMoveContainer::getPACSCoordsFromMatrix(NLMISC::CVector &pos, float &angle,
...
// ***************************************************************************
bool CMoveContainer::evalNCPrimitiveCollision(double deltaTime,
                                              UMovePrimitive *primitive,
                                              uint8 worldImage) {
  ...
  if (!primitive->isCollisionable()) {
    ...
    do {
      ...
      if (beginTime >= 1.0) {
        nlwarning(
            "PACS: evalNCPrimitiveCollision() failure, beginTime [%f] >= 1.0",
            beginTime);
        return false;
      }
      ...
      if (evalOneTerrainCollision(beginTime, (CMovePrimitive *)primitive,
                                  worldImage, false, testMoveValid,
                                  &staticColInfo, NULL)) {
      ...
      while (ite != _StaticWorldImage.end()) {
        // Eval in this world image
        if (evalOnePrimitiveCollision(
                beginTime, (CMovePrimitive *)primitive, *ite, worldImage, false,
                true, testMoveValid, &dynamicColInfoWI0, NULL)) {
          // First collision..
          if (!firstCollision || (firstCollision->getCollisionTime() >
                                  dynamicColInfoWI0.getCollisionTime())) {
      ...
      if (_StaticWorldImage.find(worldImage) == _StaticWorldImage.end()) {
        if (evalOnePrimitiveCollision(beginTime, (CMovePrimitive *)primitive,
                                      worldImage, worldImage, false, false,
                                      testMoveValid, &dynamicColInfoWI, NULL)) {
          // First collision..
          if (!firstCollision || (firstCollision->getCollisionTime() >
                                  dynamicColInfoWI.getCollisionTime())) {
      ...
      if (firstCollision) {
        ...
        if (collisionTime == 1) {
          nlinfo("PACS: evalNCPrimitiveCollision() failure, collisionTime [%f] "
                 "== 1",
                 collisionTime);
          return false;
        }
        ...
        if (_Retriever && testMoveValid) {
          // Do move
          wI->doMove(*_Retriever, _SurfaceTemp, deltaTime, collisionTime,
                     ((CMovePrimitive *)primitive)->getDontSnapToGround());
        } else {
          // Do move
          wI->doMove(_DeltaTime);
        }

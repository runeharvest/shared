...
const float InsureSurfaceThreshold =
    0.5f; // the threshold distance between 2 surfaces below which we insure the
          // retrieved position to be inside the surface
...
void NLPACS::CGlobalRetriever::initQuadGrid() {
  ...
  for (i = 0; i < _Instances.size(); ++i)
    _InstanceGrid.insert(_Instances[i].getBBox().getMin(),
                         _Instances[i].getBBox().getMax(), i);
}
...
void NLPACS::CGlobalRetriever::initRetrieveTable() {
  ...
  for (i = 0; i < _Instances.size(); ++i) {
    if (_Instances[i].getInstanceId() != -1 &&
        _Instances[i].getRetrieverId() != -1) {
      const CLocalRetriever &retriever =
          getRetriever(_Instances[i].getRetrieverId());
...
bool NLPACS::CGlobalRetriever::selectInstances(
    const NLMISC::CAABBox &bbox, CCollisionSurfaceTemp &cst,
    UGlobalPosition::TType type) const {
  ...
  for (it = _InstanceGrid.begin(); it != _InstanceGrid.end(); ++it) {
    if ((type == UGlobalPosition::Landscape &&
         _Instances[*it].getType() == CLocalRetriever::Interior) ||
        (type == UGlobalPosition::Interior &&
         _Instances[*it].getType() == CLocalRetriever::Landscape))
      continue;
...
void NLPACS::CGlobalRetriever::check() const {
  ...
  for (i = 0; i < _Instances.size(); ++i) {
    ...
    if (instance.getRetrieverId() < 0 ||
        instance.getRetrieverId() >=
            (sint)_RetrieverBank->getRetrievers().size()) {
      nlwarning("Instance %d has wrong retriever reference", i);
      continue;
    }

    const CLocalRetriever &retriever =
        _RetrieverBank->getRetriever(instance.getRetrieverId());
    ...
    for (j = 0; j < retriever.getChains().size(); ++j) {
      ...
      for (k = 0; k < chain.getSubChains().size(); ++k) {
        if (chain.getSubChain(k) >= retriever.getOrderedChains().size()) {
          nlwarning(
              "retriever %d, chain %d: subchain %d reference is not valid",
              instance.getRetrieverId(), j, k);
          continue;
        }

        if (retriever.getOrderedChain(chain.getSubChain(k)).getParentId() !=
            j) {
          nlwarning("retriever %d, ochain %d: reference on parent is not valid",
                    instance.getRetrieverId(), chain.getSubChain(k));
          continue;
        }

        if (retriever.getOrderedChain(chain.getSubChain(k))
                .getIndexInParent() != k) {
          nlwarning("retriever %d, ochain %d: index on parent is not valid",
                    instance.getRetrieverId(), chain.getSubChain(k));
          continue;
        }
      }
      ...
      if (chain.getLeft() < 0 ||
          chain.getLeft() >= (sint)retriever.getSurfaces().size()) {
        nlwarning(
            "retriever %d, chain %d: reference on left surface is not valid",
            instance.getRetrieverId(), j);
      }

      if (chain.getRight() >= (sint)retriever.getSurfaces().size() ||
          (chain.getRight() <= CChain::getDummyBorderChainId() &&
           !CChain::isBorderChainId(chain.getRight()))) {
        nlwarning(
            "retriever %d, chain %d: reference on right surface is not valid",
            instance.getRetrieverId(), j);
      }
      ...
      if (CChain::isBorderChainId(chain.getRight())) {
        ...
        if (link < 0 || link >= (sint)instance.getBorderChainLinks().size()) {
          nlwarning("retriever %d, instance %d, chain %d: reference on right "
                    "link is not valid",
                    instance.getRetrieverId(), instance.getInstanceId(), j);
        } else {
          ...
          if (lnk.Instance != 0xFFFF || lnk.SurfaceId != 0xFFFF ||
            ...
            if (lnk.Instance >= _Instances.size() ||
                ...
                _Instances[lnk.Instance].getRetrieverId() >
                    (sint)_RetrieverBank->getRetrievers().size() ||
                lnk.SurfaceId >=
                    getRetriever(_Instances[lnk.Instance].getRetrieverId())
                        .getSurfaces()
                        .size() ||
                ((lnk.ChainId >=
                      getRetriever(_Instances[lnk.Instance].getRetrieverId())
                  ...
                  lnk.BorderChainId >=
                      getRetriever(_Instances[lnk.Instance].getRetrieverId())
                          .getBorderChains()
                          .size()) &&
                 instance.getType() != CLocalRetriever::Interior)) {
              nlwarning("retriever %d, instance %d, link %d: reference on "
                        "instance may be not valid [Inst=%d, Surf=%d, "
                        "Chain=%d, BorderChain=%d]",
                        instance.getRetrieverId(), instance.getInstanceId(),
                        link, lnk.Instance, lnk.SurfaceId, lnk.ChainId,
                        lnk.BorderChainId);
            }
          }
        }
...
float NLPACS::CGlobalRetriever::distanceToBorder(
    const UGlobalPosition &pos) const {
  if (pos.InstanceId < 0 || pos.InstanceId > (sint)_Instances.size())
    return 0.0f;

  return getRetriever(_Instances[pos.InstanceId].getRetrieverId())
      .distanceToBorder(pos.LocalPosition);
}

void NLPACS::CGlobalRetriever::getBorders(
    const UGlobalPosition &pos,
...
void NLPACS::CGlobalRetriever::getBorders(
    const CAABBox &sbox, std::vector<std::pair<NLMISC::CLine, uint8>> &edges) {
  edges.clear();
  ...
  for (inst = 0; inst < _InternalCST.CollisionInstances.size(); ++inst) {
    CRetrieverInstance &instance =
        _Instances[_InternalCST.CollisionInstances[inst]];
    CLocalRetriever &retriever =
        const_cast<CLocalRetriever &>(getRetriever(instance.getRetrieverId()));
    ...
    for (ece = 0; ece < _InternalCST.EdgeChainEntries.size(); ++ece) {
      ...
      //
      const CChain &fchain = retriever.getChain(
          retriever.getOrderedChain(entry.OChainId).getParentId());
      uint8 chainType =
          (fchain.getRight() >= 0 ? 1 : (fchain.isBorderChain() ? 2 : 0));
      ...
      if (chainType == 1) {
        ...
        bool luw = (lsurface.getFlags() &
                    (1 << CRetrievableSurface::IsUnderWaterBit)) != 0;
        bool ruw = (rsurface.getFlags() &
                    (1 << CRetrievableSurface::IsUnderWaterBit)) != 0;
      ...
      if (!retriever.getFullOrderedChains().empty()) {
        const COrderedChain3f &ochain =
            retriever.getFullOrderedChain(entry.OChainId);
...
void NLPACS::CGlobalRetriever::makeLinks(uint n) {
  ...
  for (i = 0; i < _InternalCST.CollisionInstances.size(); ++i) {
    CRetrieverInstance &neighbor =
        _Instances[_InternalCST.CollisionInstances[i]];
    ...
    try {
      ...
      nlwarning("caught an exception during linkage of %d and %d: %s",
                instance.getInstanceId(), neighbor.getInstanceId(), e.what());
    }
  }

  if (getRetriever(instance.getRetrieverId()).getType() ==
  ...
  if (initInstances) {
    ...
    for (n = 0; n < _Instances.size(); ++n)
      if (_Instances[n].getInstanceId() != -1 &&
          _Instances[n].getRetrieverId() != -1)
        _Instances[n].init(
            _RetrieverBank->getRetriever(_Instances[n].getRetrieverId()));
  }
...
const NLPACS::CRetrieverInstance &
NLPACS::CGlobalRetriever::makeInstance(uint32 retrieverId, uint8 orientation,
                                       const CVector &origin) {
  uint id;
  for (id = 0; id < _Instances.size() && _Instances[id].getInstanceId() != -1;
       ++id)
    ;
  ...
  if (hsize != CVector::Null) {
    ...
    _InstanceGrid.insert(instance.getBBox().getMin(),
                         instance.getBBox().getMax(), instance.getInstanceId());
  }
...
NLPACS::CGlobalRetriever::retrievePosition(const CVector &estimated,
...
NLPACS::UGlobalPosition
NLPACS::CGlobalRetriever::retrievePosition(const CVectorD &estimated,
                                           double threshold) const {
  return retrievePosition(estimated, threshold, UGlobalPosition::Unspecified);
}

NLPACS::UGlobalPosition
NLPACS::CGlobalRetriever::retrievePosition(const CVector &estimated) const {
  return retrievePosition(estimated, 1.0e10f, UGlobalPosition::Unspecified);
}

NLPACS::UGlobalPosition
NLPACS::CGlobalRetriever::retrievePosition(const CVectorD &estimated) const {
  return retrievePosition(estimated, 1.0e10, UGlobalPosition::Unspecified);
}

// Retrieves the position of an estimated point in the global retriever (double
// instead.)
NLPACS::UGlobalPosition NLPACS::CGlobalRetriever::retrievePosition(
    const CVectorD &estimated, double /* threshold */,
    NLPACS::UGlobalPosition::TType retrieveSpec) const {
  NLPACS_HAUTO_RETRIEVE_POSITION

  // the retrieved position
  CGlobalPosition result =
      CGlobalPosition(-1, CLocalRetriever::CLocalPosition(-1, estimated));

  if (!_BBox.include(CVector((float)estimated.x, (float)estimated.y,
                             (float)estimated.z))) {
  ...
  if (!_InternalCST.SortedSurfaces.empty()) {
    ...
    std::sort(_InternalCST.SortedSurfaces.begin(),
              _InternalCST.SortedSurfaces.end(),
              CCollisionSurfaceTemp::CDistanceSurface());

    uint selInstance;
    float bestDist = 1.0e10f;
    for (selInstance = 0; selInstance < _InternalCST.SortedSurfaces.size();
         ++selInstance) {
      uint32 id = _InternalCST.SortedSurfaces[selInstance].Instance;
      const CRetrieverInstance &instance = _Instances[id];

      if (instance.getType() == CLocalRetriever::Interior &&
          _InternalCST.SortedSurfaces[selInstance].Distance < bestDist + 6.0f)
        break;
    ...
    result.LocalPosition.Surface =
        _InternalCST.SortedSurfaces[selInstance].Surface;
    ...
    // if there are more than 1 one possible (and best matching) surface, insure
    // the position within the surface (by moving the point)
    //		if (_InternalCST.SortedSurfaces.size() >= 2 &&
    //			_InternalCST.SortedSurfaces[1].Distance-_InternalCST.SortedSurfaces[0].Distance
    //< InsureSurfaceThreshold)
    if (_InternalCST.SortedSurfaces[selInstance].FoundCloseEdge) {
      ...
      if (numMove > 1) {
        nldebug("PACS: insured position inside surface (%d,%d)-(%f,%f,%f), %d "
                "moves needed",
                result.InstanceId, result.LocalPosition.Surface, estimated.x,
                estimated.y, estimated.z, numMove - 1);
      }

      if (moved) {
        nlwarning("PACS: couldn't insure position (%.f,%.f) within the surface "
                  "(surf=%d,inst=%d) after 100 retries",
                  result.LocalPosition.Estimation.x,
                  result.LocalPosition.Estimation.y,
                  result.LocalPosition.Surface, result.InstanceId);
      }
    }

    // and after selecting the best surface (and some replacement) snap the
    ...
    if (PacsRetrieveVerbose)
      nlinfo("retrievePosition(%f,%f,%f) -> %d/%d/(%f,%f,%f) - %s/%s",
             estimated.x, estimated.y, estimated.z, result.InstanceId,
             result.LocalPosition.Surface, result.LocalPosition.Estimation.x,
             result.LocalPosition.Estimation.y,
             result.LocalPosition.Estimation.z,
             retriever.getIdentifier().c_str(),
             retriever.getType() == CLocalRetriever::Interior ? "Interior"
                                                              : "Landscape");
  } else {
    if (PacsRetrieveVerbose)
      nlwarning("PACS: unable to retrieve correct position (%f,%f,%f)",
...
// Retrieves the position of an estimated point in the global retriever using
// layer hint
NLPACS::UGlobalPosition
NLPACS::CGlobalRetriever::retrievePosition(const CVectorD &estimated, uint h,
                                           sint &res) const {
  // the retrieved position
  CGlobalPosition result =
      CGlobalPosition(-1, CLocalRetriever::CLocalPosition(-1, estimated));
  ...
  if (!_InternalCST.SortedSurfaces.empty()) {
    ...
    res = Failed;
    //		nlwarning("PACS: unable to retrieve correct position
    //(%f,%f,%f)", estimated.x, estimated.y, estimated.z); 		nlSleep(1);
  }
...
const string &NLPACS::CGlobalRetriever::getIdentifier(
    const NLPACS::UGlobalPosition &position) const {
  static const string nullString;

  if (position.InstanceId == -1)
    return nullString;

  return getRetriever(_Instances[position.InstanceId].getRetrieverId())
      .getIdentifier();
}

sint32 NLPACS::CGlobalRetriever::getLocalRetrieverId(
    const NLPACS::UGlobalPosition &position) const {
  if (position.InstanceId == -1)
    return -1;
...
bool NLPACS::CGlobalRetriever::buildInstance(const string &id,
  ...
  const CRetrieverInstance &instance =
      makeInstance(retrieverId, 0, CVector(position));

  // check make instance success
  if (instance.getInstanceId() == -1 ||
      instance.getRetrieverId() != retrieverId)
    return false;
...
void NLPACS::CGlobalRetriever::removeInstance(sint32 instanceId) {
  if (instanceId < 0 || instanceId >= (sint32)_Instances.size() ||
      _Instances[instanceId].getInstanceId() < 0) {
    nlwarning("CGlobalRetriever::removeInstance(): Can't unlink instance %d, "
              "doesn't exist",
              instanceId);
    return;
  }
...
//
/*
void		NLPACS::CGlobalRetriever::snapToInteriorGround(UGlobalPosition
&position) const
{
    ...
    const CLocalRetriever		&retriever =
getRetriever(instance.getRetrieverId());
...
CVector NLPACS::CGlobalRetriever::getGlobalPosition(
    const UGlobalPosition &global) const {
  if (global.InstanceId >= 0) {
    return _Instances[global.InstanceId].getGlobalPosition(
        global.LocalPosition.Estimation);
  } else {
...
CVectorD NLPACS::CGlobalRetriever::getDoubleGlobalPosition(
    const NLPACS::UGlobalPosition &global) const {
  if (global.InstanceId >= 0) {
    return _Instances[global.InstanceId].getDoubleGlobalPosition(
        global.LocalPosition.Estimation);
  } else {
    // it should be an error here
...
void NLPACS::CGlobalRetriever::findAStarPath(
    const NLPACS::UGlobalPosition &begin, const NLPACS::UGlobalPosition &end,
  ...
  // TODO: Use a smart allocator to avoid huge alloc/free and memory
  // fragmentation open is a priority queue (implemented as a stl multimap)
  ...
  for (;;) {
    ...
    if (node == endNode) {
      ...
      while (pathNode.InstanceId != -1) {
        ...
        CRetrieverInstance::CAStarNodeInfo &pathInfo =
            instance._NodesInformation[pathNode.NodeId];
      ...
      for (i = 0; i < path.size(); ++i) {
        ...
        const CLocalRetriever &retriever =
            _RetrieverBank->getRetriever(instance.getRetrieverId());
        nlinfo("pathNode %d = (Inst=%d, Node=%d, Through=%d)", i,
               path[i].InstanceId, path[i].NodeId, path[i].ThroughChain);
        if (path[i].ThroughChain != 0xffff) {
          const CChain &chain = retriever.getChain(path[i].ThroughChain);
          nlinfo("   chain: left=%d right=%d", chain.getLeft(),
                 chain.getRight());
          if (CChain::isBorderChainId(chain.getRight())) {
            CRetrieverInstance::CLink lnk = instance.getBorderChainLink(
                CChain::convertBorderChainId(chain.getRight()));
    ...
    const CLocalRetriever &retriever =
        _RetrieverBank->getRetriever(inst.getRetrieverId());
    ...
    nlinfo("examine node (instance=%d,surf=%d,cost=%g)", node.InstanceId,
           node.NodeId, inst._NodesInformation[node.NodeId].Cost);
    ...
    for (i = 0; i < chains.size(); ++i) {
      ...
      if (CChain::isBorderChainId(nextNodeId)) {
        ...
        CRetrieverInstance::CLink lnk =
            inst.getBorderChainLink(CChain::convertBorderChainId(nextNodeId));
        ...
        nextRetriever =
            &(_RetrieverBank->getRetriever(nextInstance->getRetrieverId()));
      ...
      CRetrieverInstance::CAStarNodeInfo &nextInfo =
          nextInstance->_NodesInformation[nextNode.NodeId];
      float stepCost =
          (nextInfo.Position - inst._NodesInformation[node.NodeId].Position)
              .norm();
      ...
      for (closeIt = close.begin();
           closeIt != close.end() && *closeIt != nextNode; ++closeIt)
        ;
      ...
      for (openIt = open.begin();
           openIt != open.end() && openIt->second != nextNode; ++openIt)
        ;
      ...
      nlinfo("  adding node (instance=%d,surf=%d) f=%g, through=%d",
             nextNode.InstanceId, nextNode.NodeId, nextInfo.F, i);
...
void NLPACS::CGlobalRetriever::findPath(
    const NLPACS::UGlobalPosition &begin, const NLPACS::UGlobalPosition &end,
    NLPACS::CGlobalRetriever::CGlobalPath &path, uint32 forbidFlags) const {
  ...
  for (i = 0; i < astarPath.size(); ++i) {
    ...
    const CLocalRetriever &retriever = _RetrieverBank->getRetriever(
        _Instances[surf.InstanceId].getRetrieverId());
    ...
    if (i == 0) {
      ...
      // else, take the previous value and convert it in the current instance
      // axis
      // TODO: avoid this if the instances are the same
      CVector prev = _Instances[path[i - 1].InstanceId].getGlobalPosition(
          path[i - 1].End.Estimation);
    ...
    if (i == astarPath.size() - 1) {
      ...
      for (j = 0; j < chain.getSubChains().size() && cumulLength <= midLength;
           ++j)
        cumulLength +=
            retriever.getOrderedChain(chain.getSubChain(j)).getLength();
      --j;
      const COrderedChain &ochain =
          retriever.getOrderedChain(chain.getSubChain(j));
      surf.End.Surface = astarPath[i].NodeId;
      {
        if (ochain.getVertices().size() & 1) {
          surf.End.Estimation =
              ochain[(uint)ochain.getVertices().size() / 2].unpack3f();
        } else {
          surf.End.Estimation =
              (ochain[(uint)ochain.getVertices().size() / 2].unpack3f() +
               ochain[(uint)ochain.getVertices().size() / 2 - 1].unpack3f()) *
              0.5f;
        }
      }
    }
...
// ***************************************************************************
const NLPACS::CRetrievableSurface *NLPACS::CGlobalRetriever::getSurfaceById(
    const NLPACS::CSurfaceIdent &surfId) const {
  if (surfId.RetrieverInstanceId >= 0 && surfId.SurfaceId >= 0) {
    sint32 locRetId =
        this->getInstance(surfId.RetrieverInstanceId).getRetrieverId();
...
// ***************************************************************************
void NLPACS::CGlobalRetriever::findCollisionChains(
    CCollisionSurfaceTemp &cst, const NLMISC::CAABBox &bboxMove,
    const NLMISC::CVector &origin) const {
  ...
  for (i = 0; i < (sint)cst.CollisionInstances.size(); i++) {
    ...
    const CLocalRetriever &localRetriever =
        _RetrieverBank->getRetriever(localRetrieverId);

    if (!localRetriever.isLoaded()) {
      nldebug("local retriever %d in %s not loaded, findCollisionChains in "
              "this retriever aborted",
              localRetrieverId, _RetrieverBank->getNamePrefix().c_str());
      continue;
    }
    ...
    if (retrieverInstance.getType() == CLocalRetriever::Interior)
      retrieverInstance.testExteriorCollision(cst, bboxMoveLocal, transBase,
                                              localRetriever);
    ...
    for (j = firstCollisionChain; j < nCollisionChain; j++) {
      ...
      // If RightSurface is not an "edgeId" ie a pointer on a neighbor surface
      // on another retrieverInstance.
      ...
      if (!originalChain.isBorderChainId(cc.RightSurface.SurfaceId)) {
        ...
        link = retrieverInstance.getBorderChainLink(
            CChain::convertBorderChainId(cc.RightSurface.SurfaceId));
    ...
    // For all collision chains added, look if they are a copy of preceding
    // collsion chain (same Left/Right). Then delete them.
    //================
    ...
    for (j = firstCollisionChain; j < nCollisionChain; j++) {
      ...
      for (sint k = 0; k < firstCollisionChain; k++) {
        ...
        if (cj.LeftSurface.RetrieverInstanceId !=
                cj.RightSurface.RetrieverInstanceId &&
            cj.LeftSurface == ck.RightSurface &&
            cj.RightSurface == ck.LeftSurface) {
          const CRetrieverInstance &instj = getInstance(
                                       cj.LeftSurface.RetrieverInstanceId),
                                   &instk = getInstance(
                                       ck.LeftSurface.RetrieverInstanceId);
          ...
          if (!retrj.isLoaded() || !retrk.isLoaded()) {
            nlwarning(
                "using not loaded retriever %d or %d in bank '%s', aborted",
                instj.getRetrieverId(), instk.getRetrieverId(),
                _RetrieverBank->getNamePrefix().c_str());
            continue;
          }

          nlassert(retrj.getChain(cj.ChainId).isBorderChain() &&
                   retrk.getChain(ck.ChainId).isBorderChain());

          if (instj.getBorderChainLink(
                       retrj.getChain(cj.ChainId).getBorderChainIndex())
                      .ChainId != ck.ChainId ||
              instk.getBorderChainLink(
                       retrk.getChain(ck.ChainId).getBorderChainIndex())
                      .ChainId != cj.ChainId) {
            continue;
          }
          ...
          if (j < nCollisionChain - 1) {
            swap(cst.CollisionChains[j],
                 cst.CollisionChains[nCollisionChain - 1]);
            // NB: some holes remain in cst._EdgeCollideNodes, but do not
            // matters since reseted at each collision test.
          }
        ...
        /*
                        // if same surface Ident Left/Right==Left/Right or
           swapped Left/Right==Right/Left if(
           cst.CollisionChains[j].sameSurfacesThan(cst.CollisionChains[k]) )
                        {
                            // remove this jth entry.
                            // by swapping with last entry. Only if not already
           last. if(j<nCollisionChain-1)
                            {
                                swap(cst.CollisionChains[j],
           cst.CollisionChains[nCollisionChain-1]);
                                // NB: some holes remain in
           cst._EdgeCollideNodes, but do not matters since reseted at
                                // each collision test.
                            }
...
void NLPACS::CGlobalRetriever::testCollisionWithCollisionChains(
    CCollisionSurfaceTemp &cst, const CVector2f &startCol,
    const CVector2f &deltaCol, CSurfaceIdent startSurface, float radius,
    const CVector2f bboxStart[4], TCollisionType colType) const {
  ...
  /*
      To manage recovery, we must use such an algorithm, so we are sure to trace
     the way across all surfaces really collided, and discard any other (such as
     other floor or ceiling).
  */
  for (;;) {
    ...
    for (i = 0; i < (sint)cst.CollisionChains.size(); i++) {
      ...
      /// TODO Tests Ben
      nlassert(colChain.LeftSurface.RetrieverInstanceId <
               (sint)_Instances.size());
      nlassert(colChain.RightSurface.RetrieverInstanceId <
               (sint)_Instances.size());

      // test only currentSurface/X. And don't test chains already tested
      // before.
      if (colChain.hasSurface(currentSurface) && !colChain.Tested) {
        ...
        if (colChain.ExteriorEdge &&
            colChain.LeftSurface.RetrieverInstanceId != -1) {
          bool enterInterior = (currentSurface.RetrieverInstanceId ==
                                colChain.RightSurface.RetrieverInstanceId);

          uint j;
          sint32 cmp = (colChain.LeftSurface.RetrieverInstanceId << 16) +
                       colChain.ChainId;
          for (j = 0;
               j < checkedExtEdges.size() && (checkedExtEdges[j].first != cmp);
               ++j)
            ;
        ...
        // If collision with this chain, must insert it in the array of
        // collision.
        //========================
        if (tMin < 1) {
          CSurfaceIdent collidedSurface =
              colChain.getOtherSurface(currentSurface);

          // if flag as an interior/landscape interface and leave interior surf,
          // retrieve correct surface
          if (colChain.ExteriorEdge && currentSurface == colChain.LeftSurface) {
            ...
            CVector ori =
                getInstance(startSurface.RetrieverInstanceId).getOrigin();
            ori.z = 0.0f;
            ...
            // NB: getMeanHeight() should work here since we are still deep in
            // the interior surface (edge collided with bounding volume)
          ...
          /// TODO Tests Ben
          nlassert(collidedSurface.RetrieverInstanceId <
                   (sint)_Instances.size());

          // insert or replace this collision in collisionDescs.
          // NB: yes this looks like a N algorithm (so N^2). But not so many
          // collisions may arise, so don't bother.
          sint indexInsert = (sint)cst.CollisionDescs.size();
          sint colFound = -1;

          // start to search with nextCollisionSurfaceTested, because can't
          // insert before.
          for (sint j = nextCollisionSurfaceTested;
               j < (sint)cst.CollisionDescs.size(); j++) {
            ...
            if (cst.CollisionDescs[j].ContactSurface == collidedSurface) {
              colFound = j;
              // if we have found our old collision, stop, there is no need to
              // search more.
              break;
            }
          }

          // Insert only if the surface was not already collided, or that new
          // collision arise before old.
          if (colFound == -1 || indexInsert <= colFound) {
            ...
            // if, by chance, indexInsert==colFound, just replace old collision
            // descriptor.
            if (colFound == indexInsert) {
              cst.CollisionDescs[indexInsert] = newCol;
            } else {
              // if any, erase old collision against this surface. NB: here,
              // colFound>indexInsert.
              if (colFound != -1)
                cst.CollisionDescs.erase(cst.CollisionDescs.begin() + colFound);

              // must insert the collision.
              cst.CollisionDescs.insert(
                  cst.CollisionDescs.begin() + indexInsert, newCol);
            }
          }
        }
      }
    ...
    // else next one.
      ...
      // NB: with this algorithm, we are sure that no more collisions will arise
      // before currentCollisionSurfaceTested. so just continue with following
      // surface.
      currentSurface =
          cst.CollisionDescs[nextCollisionSurfaceTested].ContactSurface;
      ...
      if (currentSurface.SurfaceId < 0)
        ...
        sint32 locRetId = this->getInstance(currentSurface.RetrieverInstanceId)
                              .getRetrieverId();
        ...
        if (currentSurface.SurfaceId < (sint)retr.getSurfaces().size()) {
          const CRetrievableSurface &surf =
              _RetrieverBank->getRetriever(locRetId).getSurface(
      ...
      if (isWall) {
        ...
        // Next time, we will test the following (NB: the array may grow during
        // next pass, or reorder, but only after nextCollisionSurfaceTested).
        nextCollisionSurfaceTested++;
      }
    }
  }
...
bool NLPACS::CGlobalRetriever::verticalChain(
    const CCollisionChain &colChain) const {
  // retrieve surfaces.
...
// ***************************************************************************
NLPACS::CSurfaceIdent NLPACS::CGlobalRetriever::testMovementWithCollisionChains(
    CCollisionSurfaceTemp &cst, const CVector2f &startCol,
    const CVector2f &endCol, CSurfaceIdent startSurface,
    UGlobalPosition &restart) const {
  ...
  /*
     ...
     other floor or ceiling).
      ...
      This function is quite different from testCollisionWithCollisionChains()
     because she must detect all collisions with all edges of any chains (and
     not the minimum collision with a chain). This is done in 3 parts:
  ...
  */
  ...
  for (i = 0; i < (sint)cst.CollisionChains.size(); i++) {
    ...
    if (colChain.ExteriorEdge) {
      sint32 cmp =
          (colChain.LeftSurface.RetrieverInstanceId << 16) + colChain.ChainId;

      uint j;
      for (j = 0;
           j < checkedExtEdges.size() && (checkedExtEdges[j].first != cmp); ++j)
        ;
      // if already crossed this edge, abort
      // this a door that is crossing a surface frontier
      if (j < checkedExtEdges.size())
    ...
    while (curEdge != (sint32)0xFFFFFFFF) {
      ...
      if (t == -1) {
        ...
        if (pmpb == CEdgeCollide::StartOnEdge) {
          ...
          /*
          // verify if it is an edge which separate 2 walls. in this case,
          ignore it. else, error. if(verticalChain(colChain))
          {
      ...
      if (t < 1) {
        // insert in list.
        cst.MoveDescs.push_back(
            CMoveSurfaceDesc(t, colChain.LeftSurface, colChain.RightSurface));
        cst.MoveDescs.back().ExteriorEdge = colChain.ExteriorEdge;
        cst.MoveDescs.back().ChainId = (uint16)colChain.ChainId;
        cst.MoveDescs.back().MovementSens =
            colEdge.Norm * (endCol - startCol) >= 0;
      }
  ...
  for (i = 0; i < (sint)cst.MoveDescs.size(); i++) {
    ...
    if (msd.hasSurface(currentSurface)) {
      ...
      if (msd.ExteriorEdge && msd.LeftSurface.RetrieverInstanceId != -1) {
        bool enterInterior = (currentSurface.RetrieverInstanceId ==
                              msd.RightSurface.RetrieverInstanceId);
        ...
        if (!enterInterior) {
          ...
          float ctime = (float)((double)(msd.ContactTime.Numerator) /
                                (double)(msd.ContactTime.Denominator));
          ...
          /* WE HAVE A PRECISION PROBLEM HERE (yoyo 12/04/2006)
              Since the point p has moved close to the exterior edge, because of
             precision, it may be actually OUT the interior surface!! thus
             getMeanHeight() will return 0!! Then the chosen landscape position
             can be completly false. eg: actual InteriorHeight: -84 new
             possibles landscape surfaces heights: -84 and -16 if we estimate by
             error InteriorHeight= 0, then we will have Best Landscape Surface
             == the one which has height=-16 !

              Hence we use a specific method that look a bit outisde the
             triangles
          */
...
NLPACS::CGlobalRetriever::testCylinderMove(const UGlobalPosition &startPos,
                                           const NLMISC::CVector &delta,
                                           float radius,
                                           CCollisionSurfaceTemp &cst) const {
  //	H_AUTO(PACS_GR_testCylinderMove);

  CSurfaceIdent startSurface(startPos.InstanceId,
                             startPos.LocalPosition.Surface);
  ...
  testCollisionWithCollisionChains(cst, startCol, deltaCol, startSurface,
                                   radius, obbDummy, CGlobalRetriever::Circle);
...
// ***************************************************************************
const NLPACS::TCollisionSurfaceDescVector *
NLPACS::CGlobalRetriever::testBBoxMove(const UGlobalPosition &startPos,
                                       const NLMISC::CVector &delta,
  ...
  CVector2f startCol(start.x, start.y);
  CVector2f deltaCol(delta.x, delta.y);
  testCollisionWithCollisionChains(cst, startCol, deltaCol, startSurface, 0,
                                   obbStart, CGlobalRetriever::BBox);
...
// ***************************************************************************
NLPACS::UGlobalPosition NLPACS::CGlobalRetriever::doMove(
    const NLPACS::UGlobalPosition &startPos, const NLMISC::CVector &delta,
    float t, NLPACS::CCollisionSurfaceTemp &cst, bool rebuildChains) const {
  ...
  if (!rebuildChains) {
    ...
    if (cst.PrecStartSurface != startSurface ||
        cst.PrecStartPos != startPos.LocalPosition.Estimation ||
        cst.PrecDeltaPos != delta || !cst.PrecValid) {
      // if not, just return start.
      // nlstop;
      // nlwarning ("BEN: you must fix this, it s happen!!!");
      return startPos;
    }
    // Since we are sure we have same movement than prec testMove(), no need to
    // rebuild cst.CollisionChains.
  } else {
    // we don't have same movement than prec testMove(), we must rebuild
    // cst.CollisionChains. Prec settings no more valids.
    cst.PrecValid = false;
  }
  ...
  if (rebuildChains) {
    ...
    // compute bboxmove.
    CAABBox bboxMove;
    // must add some extent, to be sure to include snapped CLocalRetriever
    // vertex (2.0f/256 should be sufficient). Nb: this include the precision
    // problem just below (move a little).
  ...
  // Normally, just one iteration is made in this loop (but if precision problem
  // (stopOnEdge, startOnEdge....).
  for (;;) {
    // must snap the end position.
    ...
    endSurface = testMovementWithCollisionChains(cst, startCol, endCol,
                                                 startSurface, restart);

    // if no precision problem, Ok, we have found our destination surface (or
    // anormal collide against a wall).
    if (endSurface.SurfaceId >= -1) {
      break;
    }
    // left an interior, retrieved position and ask to restart collision from
    // retrieved position
  ...
  if (endSurface.SurfaceId == -1)
    ...
    // get delta between startPos.instance and curInstance.
    // NB: for float precision, it is important to compute deltaOrigin, and
    // after compute newPos in local.
    CVector deltaOrigin;
    deltaOrigin = origin - getInstance(res.InstanceId).getOrigin();

    // Because Origin precision is 1 meter, and end precision is 1/1024 meter,
    // we have no precision problem. this is true because we cannot move more
    // than, say 4*160 meters in one doMove(). So global position should not be
    // bigger than 1024 * 1024/1024 meters.  => Hence 20 bits of precision is
...
// ***************************************************************************
const NLPACS::TCollisionSurfaceDescVector &
NLPACS::CGlobalRetriever::testBBoxRot(const CGlobalPosition &startPos,
                                      const NLMISC::CVector &locI,
                                      const NLMISC::CVector &locJ,
                                      CCollisionSurfaceTemp &cst) const {
...
void NLPACS::CGlobalRetriever::testRotCollisionWithCollisionChains(
    CCollisionSurfaceTemp &cst, const CVector2f & /* startCol */,
    CSurfaceIdent startSurface, const CVector2f bbox[4]) const {
  ...
  /*
      Test collisions with all collision chains. Then, to manage recovery, test
     the graph of surfaces.
  */
  // run all collisionChain.
  //========================
  for (i = 0; i < (sint)cst.CollisionChains.size(); i++) {
    ...
    while (curEdge != (sint32)0xFFFFFFFF) {
      ...
      if (colEdge.testBBoxCollide(bbox)) {
        ...
        cst.RotDescs.push_back(
            CRotSurfaceDesc(colChain.LeftSurface, colChain.RightSurface));
        break;
      }
  ...
  for (;;) {
    ...
    for (i = 0; i < (sint)cst.RotDescs.size(); i++) {
      ...
      if (cst.RotDescs[i].hasSurface(currentSurface) &&
    ...
    // get the next currentSurface from surface collided (traverse the graph of
    // collisions).
...
NLPACS::UGlobalRetriever *NLPACS::UGlobalRetriever::createGlobalRetriever(
    const char *globalRetriever, const NLPACS::URetrieverBank *retrieverBank) {

  // Cast
  //	nlassert (dynamic_cast<const NLPACS::CRetrieverBank*>(retrieverBank));
  const NLPACS::CRetrieverBank *bank =
      static_cast<const NLPACS::CRetrieverBank *>(retrieverBank);
...
void NLPACS::UGlobalRetriever::deleteGlobalRetriever(
    UGlobalRetriever *retriever) {
  // Cast
  nlassert(dynamic_cast<NLPACS::CGlobalRetriever *>(retriever));
  NLPACS::CGlobalRetriever *r =
      static_cast<NLPACS::CGlobalRetriever *>(retriever);
...
// ***************************************************************************
float NLPACS::CGlobalRetriever::getInteriorHeightAround(
    const UGlobalPosition &pos, float outsideTolerance) const {
  // for wrong positions, leave it unchanged
  if ((pos.InstanceId == -1) || (pos.LocalPosition.Surface == -1))
    return pos.LocalPosition.Estimation.z;
...
bool NLPACS::CGlobalRetriever::testRaytrace(const CVectorD & /* v0 */,
                                            const CVectorD & /* v1 */) {
...
void NLPACS::CGlobalRetriever::refreshLrAround(const CVector &position,
                                               float radius) {
  ...
  while (ite != _LrLoaderList.end()) {
    ...
    if (ite->Finished && ite->Successful) {
      ...
      const_cast<CRetrieverBank *>(_RetrieverBank)
          ->loadRetriever(ite->LrId, ite->_Buffer);
  ...
  for (i = 0; i < _InternalCST.CollisionInstances.size(); ++i) {
    uint lrId =
        (uint)(_Instances[_InternalCST.CollisionInstances[i]].getRetrieverId());
    newlr.insert(lrId);
    lrPosition.insert(map<uint, CVector>::value_type(
        lrId,
        _Instances[_InternalCST.CollisionInstances[i]].getBBox().getCenter()));
  }
  ...
  for (it = out.begin(); it != out.end(); ++it) {
    ...
    // nlinfo("Freed Lr '%s'", (_RetrieverBank->getNamePrefix() + "_" +
    // toString(*it) + ".lr").c_str());
  }
  ...
  while (iteIn != in.end()) {
    ...
    if (ite == _LrLoaderList.end()) {
      ...
      loader.LrId = *iteIn;
      loader.LoadFile =
          _RetrieverBank->getNamePrefix() + "_" + toString(loader.LrId) + ".lr";
...
// ***************************************************************************
void NLPACS::CGlobalRetriever::refreshLrAroundNow(const CVector &position,
                                                  float radius) {
  // check if retriever bank is all loaded, and if yes don't refresh it
  if (_RetrieverBank->allLoaded())
    return;
  ...
  for (it = in.begin(); it != in.end(); ++it) {
    string fname =
        _RetrieverBank->getNamePrefix() + "_" + toString(*it) + ".lr";
    CIFile f;
    if (!f.open(CPath::lookup(fname, false))) {
      nlwarning("Couldn't find file '%s' to load, retriever loading aborted",
                fname.c_str());
      continue;
    }
...
//
NLMISC_CATEGORISED_VARIABLE(nel, uint, PacsRetrieveVerbose,

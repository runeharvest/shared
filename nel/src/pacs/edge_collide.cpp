...
// ***************************************************************************
CRational64 CEdgeCollide::testPointMove(const CVector2f &start,
                                        const CVector2f &end,
  ...
  */
  ...
  // NB those edges are snapped (1/256 for edgeCollide, and 1/1024 for
  // start/end), so no float problem here. precision is 20 bits.
  CVector2f normEdge;
  CVector2f normMove;
  CVector2f deltaEdge;
  CVector2f deltaMove;

  // compute normal of the edge (not normalized, because no need, and for
  // precision problem).
  deltaEdge = P1 - P0;
  normEdge.x = -deltaEdge.y;
  normEdge.y = deltaEdge.x;

  // compute normal of the movement (not normalized, because no need, and for
  // precision problem).
  deltaMove = end - start;
  normMove.x = -deltaMove.y;
  normMove.y = deltaMove.x;

  // distance from points of movment against edge line. Use double, because of
  // multiplication. precision is now 43 bits.
  double moveD0 = (double)normEdge.x * (double)(start.x - P0.x) +
                  (double)normEdge.y * (double)(start.y - P0.y);
  double moveD1 = (double)normEdge.x * (double)(end.x - P0.x) +
                  (double)normEdge.y * (double)(end.y - P0.y);

  // distance from points of edge against movement line. Use double, because of
  // multiplication. precision is now 43 bits.
  double edgeD0 = (double)normMove.x * (double)(P0.x - start.x) +
                  (double)normMove.y * (double)(P0.y - start.y);
  double edgeD1 = (double)normMove.x * (double)(P1.x - start.x) +
                  (double)normMove.y * (double)(P1.y - start.y);

  // If both edges intersect lines (including endpoints), there is a collision,
  // else none.
  ...
  if (sgnMove0 == 0 && sgnMove1 == 0) {
    ...
    double moveA0 = (double)deltaEdge.x * (double)(start.x - P0.x) +
                    (double)deltaEdge.y * (double)(start.y - P0.y);
    double moveA1 = (double)deltaEdge.x * (double)(end.x - P0.x) +
                    (double)deltaEdge.y * (double)(end.y - P0.y);
    double edgeA0 = 0;
    double edgeA1 = (double)deltaEdge.x * (double)deltaEdge.x +
                    (double)deltaEdge.y * (double)deltaEdge.y;

    // Test is there is intersection (endpoints included). if yes, return -1.
    // else return 1 (no collision at all).
  ...
  // should not have this case, because tested before with (sgnMove==0 &&
  // sgnMove1==0).
  ...
  // Because of Division, there is a precision lost in double. So compute a
  // CRational64. First, compute numerator and denominator in the highest
  // precision. this is 1024*1024 because of prec multiplication.
  ...
  */
  /*
      if (numerator != numeratorInt)
          nlwarning("numerator(%f) != numeratorInt(%" NL_I64 "d)", numerator,
     numeratorInt); if (denominator != denominatorInt)
          nlwarning("denominator(%f) != denominatorInt(%" NL_I64 "d)",
...
// ***************************************************************************
static inline float testCirclePoint(const CVector2f &start,
                                    const CVector2f &delta, float radius,
  ...
  // As long as delta is not NULL (ensured in testCircleMove() ), this code
  // should not generate Divide by 0.

  // compute quadratic..
  relC = start - point;
  relV = delta;
  a = relV.x * relV.x + relV.y * relV.y;
  // a should be >0. BUT BECAUSE OF PRECISION PROBLEM, it may be ==0, and then
  // cause divide by zero (because b may be near 0, but not 0)
  if (a == 0) {
    // in this case the move is very small. return 0 if the point is in the
    // circle and if we go toward the point
...
// ***************************************************************************
float CEdgeCollide::testCircleMove(const CVector2f &start,
                                   const CVector2f &delta, float radius,
                                   CVector2f &normal) {
  ...
  // ===============================
  if (dist >= 0) {
    // if signs are equals, same side of the line, so we allow the circle to
    // leave the line.
    if (sensPos == sensSpeed)
      return 1;

    // if speed is 0, it means that movement is parralel to the line => never
    // collide.
    if (speed == 0)
      return 1;
    ...
    if (aProj >= A0 && aProj <= A1) {
      // collision occurs on interior of the edge. the normal to return is +-
      // Norm.
  ...
  // ===============================
    ...
    // There is just need to test if projection of circle's center onto the line
    // hit the segment. This is not a good test to know if a circle intersect a
    // segment, but other cases are managed with test of endPoints of the
    // segment after.
    ...
    if (aProj >= A0 && aProj <= A1) {
      // if signs are equals, same side of the line, so we allow the circle to
      // leave the edge.
      /* Special case: do not allow to leave the edge if we are too much in the
       edge. It is important for
       CGlobalRetriever::testCollisionWithCollisionChains() because of the
       "SURFACEMOVE NOT DETECTED" Problem.
       Suppose we can walk on this chain SA/SB (separate Surface A/SurfaceB).
       Suppose we are near this edge, and on Surface SA, and suppose there is
       another chain SB/SC the circle collide with. If we return 1 (no
       collision), SB/SC won't be detected (because only SA/?? chains will be
       tested) and so the cylinder will penetrate SB/SC... This case arise at
       best if chains SA/SB and chain SB/SC do an angle of 45deg
      */
      if (sensPos == sensSpeed && (-dist) < 0.5 * radius) {
        return 1;
      } else {
        // hit the interior of the edge, and sensPos!=sensSpeed. So must stop
        // now!! collision occurs on interior of the edge. the normal to return
        // is +- Norm.
        if (sensPos) // if algebric distance of start position was >0.
          normal = Norm;
        else
  ...
  // In this case, the Circle do not hit the edge on the interior, but may hit
  // on borders.
  // ===============================
  ...
  if (tmin < 1) {
    ...
    // and so we have this normal (the perpendicular of the tangent at this
    // point).
...
// ***************************************************************************
bool CEdgeCollide::testEdgeMove(const CVector2f &q0, const CVector2f &q1,
                                const CVector2f &delta, float &tMin,
                                float &tMax, bool &normalOnBox) {
  double a, b, cv, cc, d, e, f;
  CVector2d tmp;

  // compute D1 line equation of q0q1. bx - ay + c(t)=0, where c is function of
  // time [0,1].
  // ===========================
  tmp = q1 - q0; // NB: along time, the direction doesn't change.
  // Divide by norm()^2, so that  a projection on this edge is true if the proj
  // is in interval [0,1].
  tmp /= tmp.sqrnorm();
  a = tmp.x;
  b = tmp.y;
  // c= - q0(t)*CVector2d(b,-a).  but since q0(t) is a function of time t
  // (q0+delta*t), compute cv, and cc. so c= cv*t + cc.
  ...
  */
  ...
  // ===========================
  /*
      Now, we project x,y onto each line D1 and D2, which gives  u(t) and v(t),
     each one giving the parameter of the parametric line function. When it is
     in [0,1], we are on the edge.
  ...
  */
  ...
  // ===========================
  /*
      Now, for each edge, compute time interval where parameter is in [0,1]. If
     intervals overlap, there is a collision.
  */
  ...
  if (uv == 0 || fabs(uv) < EdgeCollideEpsilon) {
    // The intersection does not move along D1. Always projected on u(t)=uc. so
    // if in [0,1], OK, else never collide.
    if (uc < 0 || uc > 1)
      return false;
  ...
  if (vv == 0 || fabs(vv) < EdgeCollideEpsilon) {
    // The intersection does not move along D2. Always projected on v(t)=vc. so
    // if in [0,1], OK, else never collide.
    if (vc < 0 || vc > 1)
      return false;
    // else suppose "always valid".
  ...
  // order time interval.
  if (tu0 > tu1)
    swap(tu0, tu1); // now, [tu0, tu1] represent the time interval where line D2
                    // hit the edge D1.
  if (tv0 > tv1)
    swap(tv0, tv1); // now, [tv0, tv1] represent the time interval where line D1
                    // hit the edge D2.
  ...
  if (!allU && !allV) {
    ...
    // if allU && allV, this means delta is near 0, and so there is always
    // collision.
    tMin = -1000;
    tMax = 1000;
  }
...
// ***************************************************************************
float CEdgeCollide::testBBoxMove(const CVector2f &start, const CVector2f &delta,
                                 const CVector2f bbox[4], CVector2f &normal) {
  // distance from center to line.
  float dist = start * Norm + C;

  // test if the movement is against the line or not.
  bool sensPos = dist > 0;
  // if signs are equals, same side of the line, so we allow the circle to leave
  // the line.
  /*if(sensPos==sensSpeed)
      return 1;*/
  ...
  if (edgeCollided && tMin < 1 && tMax > 0) {
    ...
    if (normalOnBox) {
      ...
      // assume collision occurs on interior of the edge. the normal to return
      // is +- Norm.
      if (sensPos) // if algebric distance of start position was >0.
        normal = Norm;
      else
    ...
    if (tMin > 0)
      ...
      /* NB: 0.2 is an empirical value "which works well". Normally, 1 is the
         good value, but because of the "SURFACEMOVE NOT DETECTED" Problem (see
         testCircleMove()), we must be more restrictive.
      */
      if (tMax < 0.2 * (-tMin))
        return 1;
      else

...
bool CHttpPackageProvider::getFile(std::string &filePath, const CHashKey &hash,
  ...
  for (;;) {
    ...
    nldebug("Download streamed package '%s' from '%s'", name.c_str(),
            downloadUrl.c_str());

    FILE *fp = fopen(downloadPath.c_str(), "wb");
    if (fp == NULL) {
      nldebug("Unable to create file '%s' for '%s'", downloadPath.c_str(),
              name.c_str());
      return false;
    }
    ...
    if (curl) {
      ...
      if (downloadUrl.length() > 8 &&
          (downloadUrl[4] == 's' || downloadUrl[4] == 'S')) // 01234 https
      {
  ...
  if (!(outHash == hash)) {
    ...
    nlwarning("Invalid SHA1 hash for file '%s', download has hash '%s'",
              wantHashS.c_str(), outHashS.c_str());
    return false;
  }

...
public:
  struct CertEntry {
    ...
    bool operator==(const std::string &str) { return file == str; }
  };
  ...
  void init() {
    ...
    if ((res == CURLE_OK) && sessionInfo &&
        sessionInfo->backend == CURLSSLBACKEND_OPENSSL)
    ...
    // Fortunately, if OpenSSL isn't compiled with engines means we compiled it
    // ourself and CURL is a recent version
    ...
    // only use OpenSSL callback if not using Windows SSPI and using OpenSSL
    // backend
    if (useOpenSSLBackend && !(data && data->features & CURL_VERSION_SSPI)) {
      isUsingOpenSSLBackend = true;
    } else {
      // if CURL is using SSPI or SChannel under Windows or DarwinSSL under OS
      // X, we'll use native system CA Certs
  ...
  void addCertificatesFromFile(const std::string &cert) {
    ...
    if (!isUsingOpenSSLBackend) {
      nlinfo(
          "CURL not using OpenSSL backend! Unable to use custom certificates");
      return;
    } else {
    ...
    if (std::find(CertList.begin(), CertList.end(), cert) != CertList.end())
      return;
    ...
    if (bio) {
      ...
      if (info) {
        // iterate over all entries from the PEM file, add them to the
        // x509_store one by one
...
static CURLcode sslCtxFunction(CURL *curl, void *sslctx, void *parm) {
  ...
  if (x509CertListManager.CertList.size() > 0) {
    ...
    if (x509store) {
      ...
      for (uint i = 0, ilen = x509CertListManager.CertList.size(); i < ilen;
           ++i) {
        ...
        if (X509_STORE_add_cert(x509store, entry.cert) == 0) {
          ...
          if (ERR_GET_LIB(errCode) != ERR_LIB_X509 ||
            ...
            nlwarning("Error adding certificate %s: %s", entry.name.c_str(),
                      errorBuffer);
            // There seems to be intermittent issues (on windows) where cert
            // loading will fail for same 3 to 5 certs with an 'SSL_shutdown
            // while in init' error. It does not seem to be fatal for
            // connection. res = CURLE_SSL_CACERT;
          }
        } else {
...
void CCurlCertificates::useCertificates(CURL *curl) {
  // CURL must be valid, using OpenSSL backend and certificates must be loaded,
  // else return
  if (!curl || !x509CertListManager.isInitialized ||
      !x509CertListManager.isUsingOpenSSLBackend ||
      x509CertListManager.CertList.empty())
    return;

  curl_easy_setopt(curl, CURLOPT_SSLCERTTYPE, "PEM");

  // would allow to provide the CA in memory instead of using CURLOPT_CAINFO,
  // but needs to include and link OpenSSL
  if (curl_easy_setopt(curl, CURLOPT_SSL_CTX_FUNCTION, &sslCtxFunction) !=
      CURLE_OK) {
    nlwarning("Unable to support CURLOPT_SSL_CTX_FUNCTION, curl not compiled "
              "with OpenSSL ?");
  }

  // set both CURLOPT_CAINFO and CURLOPT_CAPATH to NULL to be sure we won't use
  // default values (these files can be missing and generate errors)

...
#define CASE_BRA_CONV(isa) \
	case XZ_ID_##isa: size = isa##_Convert(data, size, p->ip, p->encodeMode); break;
    ...
    ECoderStatus *status)
{
	...
	*status = (*destLen == 0 && *srcLen == 0) ? CODER_STATUS_FINISHED_WITH_MARK : CODER_STATUS_NOT_FINISHED;
	return res;
}

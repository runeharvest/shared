...
static SRes BraState_SetProps(void *pp, const Byte *props, size_t propSize,
                              ISzAllocPtr alloc) {
...
#define CASE_BRA_CONV(isa)                                                     \
  case XZ_ID_##isa:                                                            \
    size = isa##_Convert(data, size, p->ip, p->encodeMode);                    \
    break;
...
static SRes BraState_Code2(void *pp, Byte *dest, SizeT *destLen,
...
SRes BraState_SetFromMethod(IStateCoder *p, UInt64 id, int encodeMode,
                            ISzAllocPtr alloc) {
  CBraState *decoder;
...
static SRes SbState_SetProps(void *pp, const Byte *props, size_t propSize,
                             ISzAllocPtr alloc) {
  UNUSED_VAR(pp);
  UNUSED_VAR(props);
  UNUSED_VAR(alloc);
...
static void SbState_Init(void *pp) { SbDec_Init((CSbDec *)pp); }

static SRes SbState_Code2(void *pp, Byte *dest, SizeT *destLen, const Byte *src,
                          SizeT *srcLen, int srcWasFinished,
                          ECoderFinishMode finishMode,
                          // int *wasFinished
                          ECoderStatus *status) {
  CSbDec *p = (CSbDec *)pp;
  ...
  *status = (*destLen == 0 && *srcLen == 0) ? CODER_STATUS_FINISHED_WITH_MARK
...
static SRes Lzma2State_SetProps(void *pp, const Byte *props, size_t propSize,
                                ISzAllocPtr alloc) {
  if (propSize != 1)
    return SZ_ERROR_UNSUPPORTED;
  {
...
static SRes Lzma2State_Code2(void *pp, Byte *dest, SizeT *destLen,
                             const Byte *src, SizeT *srcLen, int srcWasFinished,
                             ECoderFinishMode finishMode,
                             // int *wasFinished,
                             ECoderStatus *status) {
  ...
  ELzmaStatus status2;
  /* ELzmaFinishMode fm = (finishMode == LZMA_FINISH_ANY) ? LZMA_FINISH_ANY :
   * LZMA_FINISH_END; */
  SRes res;
  UNUSED_VAR(srcWasFinished);
  if (spec->outBufMode) {
    ...
    res = Lzma2Dec_DecodeToDic(&spec->decoder, dicLimit, src, srcLen,
                               (ELzmaFinishMode)finishMode, &status2);
    *destLen = spec->decoder.decoder.dicPos - dicPos;
  } else
    res = Lzma2Dec_DecodeToBuf(&spec->decoder, dest, destLen, src, srcLen,
                               (ELzmaFinishMode)finishMode, &status2);
...
static SRes Lzma2State_SetFromMethod(IStateCoder *p, Byte *outBuf,
                                     size_t outBufSize, ISzAllocPtr alloc) {
...
static SRes Lzma2State_ResetOutBuf(IStateCoder *p, Byte *outBuf,
                                   size_t outBufSize) {
  CLzma2Dec_Spec *spec = (CLzma2Dec_Spec *)p->p;
...
static SRes MixCoder_SetFromMethod(CMixCoder *p, unsigned coderIndex,
                                   UInt64 methodId, Byte *outBuf,
                                   size_t outBufSize) {
...
static SRes MixCoder_ResetFromMethod(CMixCoder *p, unsigned coderIndex,
                                     UInt64 methodId, Byte *outBuf,
                                     size_t outBufSize) {
  IStateCoder *sc = &p->coders[coderIndex];
  switch (methodId) {
...
static SRes MixCoder_Code(CMixCoder *p, Byte *dest, SizeT *destLen,
                          int destFinish, const Byte *src, SizeT *srcLen,
  ...
  *srcLen = 0;
  ...
  if (p->numCoders != 1) {
    if (!p->buf) {
      p->buf = (Byte *)ISzAlloc_Alloc(
          p->alloc, CODER_BUF_SIZE * (MIXCODER_NUM_FILTERS_MAX - 1));
      if (!p->buf)
        return SZ_ERROR_MEM;
    }
  ...
  for (;;) {
    ...
    */
    ...
    for (i = 0; i < p->numCoders; i++) {
      ...
      res = coder->Code2(coder->p, dest2, &destLen2, src2, &srcLen2,
...
static BoolInt Xz_CheckFooter(CXzStreamFlags flags, UInt64 indexSize,
                              const Byte *buf) {
  return indexSize == (((UInt64)GetUi32(buf + 4) + 1) << 2) &&
         GetUi32(buf) == CrcCalc(buf + 4, 6) && flags == GetBe16(buf + 8) &&
         buf[10] == XZ_FOOTER_SIG_0 && buf[11] == XZ_FOOTER_SIG_1;
}
...
static BoolInt XzBlock_AreSupportedFilters(const CXzBlock *p) {
  ...
  for (i = 0; i < numFilters; i++) {
    ...
    if (f->id == XZ_ID_Delta) {
      if (f->propsSize != 1)
        return False;
    } else if (f->id < XZ_ID_Delta || f->id > XZ_ID_SPARC ||
...
static SRes XzDecMix_Init(CMixCoder *p, const CXzBlock *block, Byte *outBuf,
                          size_t outBufSize) {
  unsigned i;
  ...
  if (needReInit) {
    MixCoder_Free(p);
    for (i = 0; i < numFilters; i++) {
      RINOK(MixCoder_SetFromMethod(p, i, block->filters[numFilters - 1 - i].id,
                                   outBuf, outBufSize));
    }
    p->numCoders = numFilters;
  } else {
    RINOK(MixCoder_ResetFromMethod(p, 0, block->filters[numFilters - 1].id,
                                   outBuf, outBufSize));
  }
...
void XzUnpacker_Free(CXzUnpacker *p) { MixCoder_Free(&p->decoder); }
...
static void XzUnpacker_UpdateIndex(CXzUnpacker *p, UInt64 packSize,
                                   UInt64 unpackSize) {
  Byte temp[32];
  ...
  for (;;) {
    ...
    if (p->state == XZ_STATE_BLOCK) {
      ...
      */
      ...
      {
        res =
            MixCoder_Code(&p->decoder, (p->outBuf ? NULL : dest), &destLen2,
                          destFinish, src, &srcLen2, srcFinished2, finishMode2);

        *status = p->decoder.status;
        XzCheck_Update(&p->check,
                       (p->outBuf ? p->outBuf + p->outDataWritten : dest),
                       destLen2);
        if (!p->outBuf)
          dest += destLen2;
      ...
      {
        XzUnpacker_UpdateIndex(p, XzUnpacker_GetPackSizeForIndex(p),
                               p->unpackSize);
        ...
        *status = CODER_STATUS_NOT_SPECIFIED;
        ...
        if ((p->block.packSize != (UInt64)(Int64)-1 &&
             p->block.packSize != p->packSize) ||
            (p->block.unpackSize != (UInt64)(Int64)-1 &&
             p->block.unpackSize != p->unpackSize)) {
    ...
    // XZ_STATE_BLOCK_FOOTER can transit to XZ_STATE_BLOCK_HEADER without input
    // bytes
    ...
    case XZ_STATE_BLOCK_FOOTER: {
      if ((((unsigned)p->packSize + p->alignPos) & 3) != 0) {
        ...
        }
        {
          ...
          p->state = XZ_STATE_BLOCK_HEADER;
          p->pos = 0;
          if (XzCheck_Final(&p->check, digest) &&
              memcmp(digest, p->buf, checkSize) != 0)
            return SZ_ERROR_CRC;
          if (p->decodeOnlyOneBlock) {
            *status = CODER_STATUS_FINISHED_WITH_MARK;
            return SZ_OK;
...
SRes XzUnpacker_CodeFull(CXzUnpacker *p, Byte *dest, SizeT *destLen,
  ...
  return XzUnpacker_Code(p, NULL, destLen, src, srcLen, True, finishMode,
                         status);
}
...
BoolInt XzUnpacker_IsStreamWasFinished(const CXzUnpacker *p) {
  return (p->state == XZ_STATE_STREAM_PADDING) &&
         (((UInt32)p->padSize & 3) == 0);
}
...
static void XzDecMt_Callback_Parse(void *obj, unsigned coderIndex,
                                   CMtDecCallbackInfo *cc) {
  ...
  while (!coder->skipMode) {
    ...
    res = XzUnpacker_Code(&coder->dec, NULL, &destSize, cc->src, &srcSize2,
                          cc->srcFinished, CODER_FINISH_END, &status);
    ...
    if (res != SZ_OK) {
      cc->state = coder->parseState = MTDEC_PARSE_END;
      /*
      if (res == SZ_ERROR_MEM)
        return res;
      return SZ_OK;
    ...
    if (coder->dec.headerParsedOk) {
      ...
      if (XzBlock_HasUnpackSize(block)
        ...
        {
          if (block->unpackSize * 2 * me->mtc.numStartedThreads >
              me->props.memUseMax) {
    ...
    if (coder->inPreSize == coder->blockPackTotal) {
      if (srcSize == 0) {
...

        {
          ...
          {
            UInt64 required =
                (UInt64)blockMax * (me->mtc.numStartedThreads + 1) * 2;
...
static SRes XzDecMt_Callback_PreCode(void *pp, unsigned coderIndex) {
  ...
  {
    SRes res = XzDecMix_Init(&coder->dec.decoder, &coder->dec.block,
                             coder->outBuf, coder->outBufSize);
...
static SRes XzDecMt_Callback_Code(void *pp, unsigned coderIndex,
                                  ...
                                  UInt64 *inCodePos, UInt64 *outCodePos,
                                  int *stop) {
  CXzDecMt *me = (CXzDecMt *)pp;
  CXzDecMtThread *coder = &me->coders[coderIndex];
  ...
  *stop = True;
  ...
  if (coder->codeRes == SZ_OK) {
    ...
    res = XzUnpacker_Code(
        &coder->dec, NULL, &outSizeCur, src, &srcProcessed, srcFinished,
        // coder->finishedWithMark ? CODER_FINISH_END : CODER_FINISH_ANY,
        CODER_FINISH_END, &status);

    // PRF(printf(" res = %d, srcSize2 = %d, outSizeCur = %d", res,
    // (unsigned)srcProcessed, (unsigned)outSizeCur));
...
static SRes XzDecMt_Callback_Write(void *pp, unsigned coderIndex,
                                   BoolInt needWriteToStream, const Byte *src,
                                   size_t srcSize,
                                   // int srcFinished,
  ...
  *canRecode = False;
  ...
  // (coder->state == MTDEC_PARSE_END) means that there are no other working
  // threads so we can use mtc variables without lock
  ...
  me->mtc.mtProgress.totalInSize = me->mtc.inProcessed;
  {
...

    {
      ...
      res = XzUnpacker_Code(dec, NULL, &outSizeCur, src, &srcProcessed,
      ...
      for (;;) {
        ...
        if (inPos == inLim) {
          if (!me->mtc.readWasFinished) {
            ...
            me->mtc.readRes =
                ISeqInStream_Read(me->inStream, (void *)inData, &inLim);
        ...
        res = XzUnpacker_Code(dec, NULL, &outProcessed, inData + inPos,
                              &inProcessed,
  ...
  for (;;) {
    ...
    if (p->progress && res == SZ_OK) {
      ...
      if (inDelta >= (1 << 22) || outDelta >= (1 << 22)) {
        res = ICompressProgress_Progress(p->progress, p->inProcessed,
                                         p->outProcessed);
        if (res != SZ_OK) {
...
static SRes XzStatInfo_SetStat(const CXzUnpacker *dec, int finishMode,
                               UInt64 readProcessed, UInt64 inProcessed,
                               SRes res, ECoderStatus status,
                               BoolInt decodingTruncated, CXzStatInfo *stat) {
  UInt64 extraSize;
  ...
  if (res == SZ_OK) {
    ...
    } else if (!decodingTruncated ||
               finishMode) // (status == CODER_STATUS_NOT_FINISHED)
      res = SZ_ERROR_DATA;
...
SRes XzDecMt_Decode(CXzDecMtHandle pp, const CXzDecMtProps *props,
                    const UInt64 *outDataSize, int finishMode,
                    ISeqOutStream *outStream,
                    // Byte *outBuf, size_t *outBufSize,
                    ISeqInStream *inStream,
                    // const Byte *inData, size_t inDataSize,
                    CXzStatInfo *stat, int *isMT, ICompressProgress *progress) {
  CXzDecMt *p = (CXzDecMt *)pp;
#ifndef _7ZIP_ST
  BoolInt tMode;
#endif
  ...
  // p->outSize = 457; p->outSize_Defined = True; p->finishMode = False; // for
  // test
    ...
    *isMT = True;
...

    {
      ...
      if (!needContinue) {
        ...
        XzStatInfo_SetStat(dec, p->finishMode, p->mtc.readProcessed,
                           p->mtc.inProcessed, codeRes, status, truncated,
                           stat);
  ...
  *isMT = False;

  {
    ...
    XzStatInfo_SetStat(&p->dec, p->finishMode, p->readProcessed, p->inProcessed,
                       p->codeRes, p->status,
                       False, // truncated
                       stat);

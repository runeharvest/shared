...
#pragma pack(push, 1)
/* Most compilers works OK here even without #pragma pack(push, 1), but some GCC
 * compilers need it. */
...
#define PPMD_SetAllBitsIn256Bytes(p)                                           \
  {                                                                            \
    size_t z;                                                                  \
    for (z = 0; z < 256 / sizeof(p[0]); z += 8) {                              \
      p[z + 7] = p[z + 6] = p[z + 5] = p[z + 4] = p[z + 3] = p[z + 2] =        \
          p[z + 1] = p[z + 0] = ~(size_t)0;                                    \
    }                                                                          \
  }

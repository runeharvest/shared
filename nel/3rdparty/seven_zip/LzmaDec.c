...
Processing:
  ...
  RangeCoder is still without last normalization when (p->buf < bufLimit) is
being checked.
...
int MY_FAST_CALL LZMA_DECODE_REAL(CLzmaDec *p, SizeT limit,
                                  const Byte *bufLimit);

#else

static int MY_FAST_CALL LZMA_DECODE_REAL(CLzmaDec *p, SizeT limit,
                                         const Byte *bufLimit) {
          ...
          if (posSlot < kEndPosModelIndex) {
            ...
            do {
...

              {
                ...
                code -= range;
                t = (0 -
                     ((UInt32)code >> 31)); /* (UInt32)((Int32)code >> 31) */
...
#define kBadRepCode                                                            \
  (kBound0 + (((kRange0 - kBound0) >> kNumBitModelTotalBits)                   \
              << (kNumBitModelTotalBits - 1)))
...
SRes LzmaDec_DecodeToDic(CLzmaDec *p, SizeT dicLimit, const Byte *src,
  ...
  *status = LZMA_STATUS_NOT_SPECIFIED;
  ...
  if (p->remainLen > kMatchSpecLenStart) {
    ...
    p->code = ((UInt32)p->tempBuf[1] << 24) | ((UInt32)p->tempBuf[2] << 16) |
              ((UInt32)p->tempBuf[3] << 8) | ((UInt32)p->tempBuf[4]);
    p->range = 0xFFFFFFFF;
    p->tempBufSize = 0;

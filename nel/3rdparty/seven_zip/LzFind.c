...
static int LzInWindow_Create(CMatchFinder *p, UInt32 keepSizeReserv,
                             ISzAllocPtr alloc) {
...
int MatchFinder_Create(CMatchFinder *p, UInt32 historySize,
  ...
  /* we need one additional byte, since we use MoveBlock after pos++ and before
   * dictionary using */
      ...
      if (p->numHashBytes == 2)
        ...
        if (hs > (1 << 24)) {
          if (p->numHashBytes == 3)
            ...
            hs >>= 1;
          /* if (bigHash) mode, GetHeads4b() in LzFindMt.c needs (hs >= ((1 <<
           * 24) - 1))) */
        }
      }
      p->hashMask = hs;
...
*/
MY_FORCE_INLINE
static UInt32 *Hc_GetMatchesSpec(unsigned lenLimit, UInt32 curMatch, UInt32 pos,
  ...
  _cyclicBufferSize : 0)]; if (pb[maxLen] == cur[maxLen] && *pb == *cur)
      {
        UInt32 len = 0;
        while (++len != lenLimit)
          if (pb[len] != cur[len])
...
UInt32 *GetMatchesSpec1(UInt32 lenLimit, UInt32 curMatch, UInt32 pos,
                        const Byte *cur, CLzRef *son, UInt32 _cyclicBufferPos,
                        UInt32 _cyclicBufferSize, UInt32 cutValue,
  ...
  for (;;) {
    ...
    {
      CLzRef *pair =
          son + ((size_t)(_cyclicBufferPos - delta +
                          ((delta > _cyclicBufferPos) ? _cyclicBufferSize : 0))
                 << 1);
      const Byte *pb = cur - delta;
...
#define MF_PARAMS(p)                                                           \
  p->pos, p->buffer, p->son, p->cyclicBufferPos, p->cyclicBufferSize,          \
      p->cutValue

#define GET_MATCHES_FOOTER(offset, maxLen)                                     \
  offset =                                                                     \
      (unsigned)(GetMatchesSpec1((UInt32)lenLimit, curMatch, MF_PARAMS(p),     \
                                 distances + offset, (UInt32)maxLen) -         \
                 distances);                                                   \
  MOVE_POS_RET;

...
static void Lzma2DecMt_MtCallback_Parse(void *obj, unsigned coderIndex,
  ...
  if (cc->startCall) {
    if (!t->dec_created) {
      ...
      AlignOffsetAlloc_CreateVTable(&t->alloc);
      {
        ...
        t->alloc.offset =
            ((UInt32)coderIndex * ((1 << 11) + (1 << 8) + (1 << 6))) &
...

  {
...

    {
      ...
      for (;;) {
        ...
        status =
            Lzma2Dec_Parse(&t->dec, limit - t->dec.decoder.dicPos,
                           cc->src + cc->srcSize, &srcCur, checkFinishBlock);

        cc->srcSize += srcCur;

        if (status == LZMA2_PARSE_STATUS_NEW_CHUNK) {
          if (t->dec.unpackSize >
              me->props.outBlockMax - t->dec.decoder.dicPos) {
            overflow = True;
            break;
          }
          continue;
      ...
      if ((int)status != LZMA_STATUS_NEEDS_MORE_INPUT) {
        if (status == LZMA2_PARSE_STATUS_NEW_BLOCK) {
          ...
          if ((int)status != LZMA_STATUS_FINISHED_WITH_MARK) {
            ...
            if (unpackRem != 0) {
              /* we also reserve space for max possible number of output bytes
               * of current LZMA chunk */
  ...
  *canRecode = True;
  ...
  if (t->codeRes == SZ_OK)
    if ((int)t->parseStatus == LZMA_STATUS_FINISHED_WITH_MARK ||
        t->parseStatus == LZMA2_PARSE_STATUS_NEW_BLOCK)
      if (t->outPreSize != t->outCodeSize || t->inPreSize != t->inCodeSize)
        return SZ_ERROR_FAIL;
...
static SRes Lzma2Dec_Decode_ST(CLzma2DecMt *p
  ...
  for (;;) {
    ...
    res = Lzma2Dec_DecodeToDic(dec, dicPos + size, inData + inPos, &inProcessed,
                               finishMode, &status);
    ...
    needStop = (res != SZ_OK || (inProcessed == 0 && outProcessed == 0) ||
    ...
    if (needStop || outProcessed >= size) {
      SRes res2;
      {
        ...
        size_t written = ISeqOutStream_Write(
    ...
    if (p->progress) {
      ...
      if (inDelta >= (1 << 22) || outDelta >= (1 << 22)) {
        RINOK(ICompressProgress_Progress(p->progress, p->inProcessed,
                                         p->outProcessed));
        inPrev = p->inProcessed;
        outPrev = p->outProcessed;
      }
...
SRes Lzma2DecMt_Decode(CLzma2DecMtHandle pp, Byte prop,
                       const CLzma2DecMtProps *props, ISeqOutStream *outStream,
...
SRes Lzma2DecMt_Read(CLzma2DecMtHandle pp, Byte *data, size_t *outSize,
  ...
  *inStreamProcessed = 0;
  ...
  for (;;) {
    ...
    res = Lzma2Dec_DecodeToBuf(&p->dec, data, &outCur, p->inBuf + p->inPos,
                               &inCur, finishMode, &status);

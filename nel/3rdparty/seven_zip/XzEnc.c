...
#define XZ_GET_ESTIMATED_BLOCK_TOTAL_PACK_SIZE(unpackSize)                     \
  (XZ_BLOCK_HEADER_SIZE_MAX + XZ_GET_MAX_BLOCK_PACK_SIZE(unpackSize))
...
static SRes WriteBytesUpdateCrc(ISeqOutStream *s, const void *buf, size_t size,
                                UInt32 *crc) {
...
static SRes XzEncIndex_ReAlloc(CXzEncIndex *p, size_t newSize,
                               ISzAllocPtr alloc) {
...
static SRes XzEncIndex_PreAlloc(CXzEncIndex *p, UInt64 numBlocks,
                                UInt64 unpackSize, UInt64 totalSize,
                                ISzAllocPtr alloc) {
  UInt64 pos;
  {
    Byte buf[32];
...
static SRes XzEncIndex_AddIndexRecord(CXzEncIndex *p, UInt64 unpackSize,
                                      UInt64 totalSize, ISzAllocPtr alloc) {
  Byte buf[32];
...
static SRes XzEncIndex_WriteFooter(const CXzEncIndex *p, CXzStreamFlags flags,
                                   ISeqOutStream *s) {
  Byte buf[32];
  UInt64 globalPos;
...
static SRes SeqCheckInStream_Read(const ISeqInStream *pp, void *data,
                                  size_t *size) {
...
static size_t SeqSizeOutStream_Write(const ISeqOutStream *pp, const void *data,
                                     size_t size) {
...
static SRes SeqInFilter_Init(CSeqInFilter *p, const CXzFilter *props,
  ...
  RINOK(p->StateCoder.SetProps(p->StateCoder.p, props->props, props->propsSize,
                               alloc));
  ...
  *size = 0;
  ...
  for (;;) {
    ...
    }
    {
      ...
      SRes res;
      *size = sizeOriginal;
      res = p->StateCoder.Code2(p->StateCoder.p, (Byte *)data, size,
                                p->buf + p->curPos, &srcLen, p->srcWasFinished,
...
static SRes SbEncInStream_Read(const ISeqInStream *pp, void *data,
                               size_t *size) {
  ...
  for (;;) {
    if (p->enc.needRead && !p->enc.readWasFinished) {
      ...
      RINOK(p->inStream->Read(p->inStream, p->enc.buf + p->enc.readPos,
                              &processed));
  ...
  if (p->blockSize == XZ_PROPS__BLOCK_SIZE__SOLID) {
    ...
    if (p->blockSize == LZMA2_ENC_PROPS__BLOCK_SIZE__AUTO) {
      ...
      if (lzma2->blockSize == LZMA2_ENC_PROPS__BLOCK_SIZE__SOLID) {
        ...
        //   we calculate block size for lzma2 and use that block size for xz,
        //   lzma2 uses single-chunk per block
        CLzma2EncProps tp = p->lzma2Props;
        if (tp.numTotalThreads <= 0)
          tp.numTotalThreads = p->numTotalThreads;
...
static SRes Lzma2WithFilters_Create(CLzma2WithFilters *p, ISzAllocPtr alloc,
...
static SRes
Xz_CompressBlock(CLzma2WithFilters *lzmaf,

                 ISeqOutStream *outStream, Byte *outBufHeader, Byte *outBufData,
                 size_t outBufDataLimit,

                 ISeqInStream *inStream,
                 // UInt64 expectedSize,
                 const Byte *inBuf, // used if (!inStream)
                 size_t inBufSize,  // used if (!inStream), it's block size,
                                    // props->blockSize is ignored

                 const CXzProps *props, ICompressProgress *progress,
                 int *inStreamFinished, /* only for inStream version */
                 CXzEncBlockInfo *blockSizes, ISzAllocPtr alloc,
  ...
  */
...

  {
    ...
    if (!outStream) {
      outBuf =
          seqSizeOutStream.outBuf; //  + (size_t)seqSizeOutStream.processed;
      outSize =
          seqSizeOutStream.outBufLimit; // - (size_t)seqSizeOutStream.processed;
    }

    res = Lzma2Enc_Encode2(
        lzmaf->lzma2, outBuf ? NULL : &seqSizeOutStream.vt, outBuf,
        outBuf ? &outSize : NULL,
        ...
        useStream ? NULL : inBuf, useStream ? 0 : inBufSize,
  ...
  }
  {
    ...
    RINOK(WriteBytes(&seqSizeOutStream.vt, buf + (4 - padSize),
                     padSize +
                         XzFlags_GetCheckSize((CXzStreamFlags)props->checkId)));
...
static SRes CompressProgress_XzEncOffset_Progress(const ICompressProgress *pp,
                                                  UInt64 inSize,
                                                  UInt64 outSize) {
  const CCompressProgress_XzEncOffset *p =
      CONTAINER_FROM_VTBL(pp, CCompressProgress_XzEncOffset, vt);
  inSize += p->inOffset;
  outSize += p->outOffset;
...
static SRes XzEnc_MtCallback_Code(void *pp, unsigned coderIndex,
                                  unsigned outBufIndex, const Byte *src,
...

  {
    ...
    res = Xz_CompressBlock(&me->lzmaf_Items[coderIndex],

                           NULL, dest, dest + XZ_BLOCK_HEADER_SIZE_MAX,
                           me->outBufSize - XZ_BLOCK_HEADER_SIZE_MAX,

                           NULL,
                           // srcSize, // expectedSize
                           src, srcSize,

                           &me->xzProps, &progressThunk.vt, &inStreamFinished,
                           &blockSizes, me->alloc, me->allocBig);
...
static SRes XzEnc_MtCallback_Write(void *pp, unsigned outBufIndex) {
...

  {
    ...
    RINOK(WriteBytes(me->outStream, data + XZ_BLOCK_HEADER_SIZE_MAX,
                     (size_t)totalPackFull - bInfo->headerSize));
  }

  return XzEncIndex_AddIndexRecord(&me->xzIndex, bInfo->unpackSize,
                                   bInfo->totalSize, me->alloc);
}

#endif

SRes XzEnc_Encode(CXzEncHandle pp, ISeqOutStream *outStream,
  ...
  XzEncIndex_Init(&p->xzIndex);
  {
    ...
    RINOK(XzEncIndex_PreAlloc(&p->xzIndex, numBlocks, blockSize,
  ...
  if (props->numBlockThreads_Reduced > 1) {
    ...
    {
      size_t destBlockSize = XZ_BLOCK_HEADER_SIZE_MAX +
                             XZ_GET_MAX_BLOCK_PACK_SIZE(p->mtCoder.blockSize);
    ...
    for (;;) {
      ...
      */
      ...
      RINOK(Xz_CompressBlock(
          ...
          writeStartSizes ? p->outBufs[0] : NULL, bufData, bufSize,

          inStream,
          // rem,
          NULL, 0,

          props, progress ? &progress2.vt : NULL, &inStreamFinished,
          &blockSizes, p->alloc, p->allocBig));

      {
        UInt64 totalPackFull =
            blockSizes.totalSize + XZ_GET_PAD_SIZE(blockSizes.totalSize);

        if (writeStartSizes) {
          RINOK(WriteBytes(outStream, p->outBufs[0], blockSizes.headerSize));
          RINOK(WriteBytes(outStream, bufData,
                           (size_t)totalPackFull - blockSizes.headerSize));
        }

        RINOK(XzEncIndex_AddIndexRecord(&p->xzIndex, blockSizes.unpackSize,
                                        blockSizes.totalSize, p->alloc));
  ...
  return XzEncIndex_WriteFooter(&p->xzIndex, (CXzStreamFlags)props->checkId,
                                outStream);
}

...
#define Ppmd7_GetBinSumm(p)                                                    \
  &p->BinSumm[(size_t)(unsigned)Ppmd7Context_OneState(p->MinContext)->Freq -   \
              1]                                                               \
             [p->PrevSuccess +                                                 \
              p->NS2BSIndx[(size_t)Ppmd7_GetContext(p, p->MinContext->Suffix)  \
                               ->NumStats -                                    \
                           1] +                                                \
              (p->HiBitsFlag = p->HB2Flag[p->FoundState->Symbol]) +            \
              2 * p->HB2Flag[(unsigned)Ppmd7Context_OneState(p->MinContext)    \
                                 ->Symbol] +                                   \
              ((p->RunLength >> 26) & 0x20)]

...
#define MY_ALLOC(T, p, size, alloc)                                            \
  {                                                                            \
    if ((p = (T *)ISzAlloc_Alloc(alloc, (size) * sizeof(T))) == NULL)          \
      return SZ_ERROR_MEM;                                                     \
  }
...
#define MY_ALLOC_ZE_AND_CPY(to, size, from, alloc)                             \
  {                                                                            \
    if ((size) == 0)                                                           \
      ...
      MY_ALLOC_AND_CPY(to, size, from, alloc)                                  \
    }                                                                          \
  }
...
static MY_NO_INLINE SRes ReadBitVector(CSzData *sd, UInt32 numItems, Byte **v,
                                       ISzAllocPtr alloc) {
  Byte allAreDefined;
  Byte *v2;
  UInt32 numBytes = (numItems + 7) >> 3;
  *v = NULL;
  ...
  memset(v2, 0xFF, (size_t)numBytes);
  {
    ...
    if (numBits != 0)
      v2[(size_t)numBytes - 1] =
          (Byte)((((UInt32)1 << numBits) - 1) << (8 - numBits));
  }
  return SZ_OK;
}

static MY_NO_INLINE SRes ReadUi32s(CSzData *sd2, UInt32 numItems,
                                   CSzBitUi32s *crcs, ISzAllocPtr alloc) {
  UInt32 i;
  CSzData sd;
  UInt32 *vals;
  const Byte *defs;
...
static SRes ReadBitUi32s(CSzData *sd, UInt32 numItems, CSzBitUi32s *crcs,
                         ISzAllocPtr alloc) {
...
#define k_NumCodersStreams_in_Folder_MAX                                       \
  (SZ_NUM_BONDS_IN_FOLDER_MAX + SZ_NUM_PACK_STREAMS_IN_FOLDER_MAX)
...
static SRes ReadUnpackInfo(CSzAr *p, CSzData *sd2, UInt32 numFoldersMax,
...
static SRes ReadSubStreamsInfo(CSzAr *p, CSzData *sd, CSubStreamInfo *ssi) {
  ...
  if (!ssi->sdNumSubStreams.Data) {
    ...
    if (p->FolderCRCs.Defs)
      numSubDigests =
          numFolders - CountDefinedBits(p->FolderCRCs.Defs, numFolders);
  }
...
static SRes SzReadStreamsInfo(CSzAr *p, CSzData *sd, UInt32 numFoldersMax,
                              const CBuf *tempBufs, UInt32 numTempBufs,
                              UInt64 *dataOffset, CSubStreamInfo *ssi,
                              ISzAllocPtr alloc) {
  UInt64 type;
...
static SRes SzReadAndDecodePackedStreams(ILookInStream *inStream, CSzData *sd,
                                         CBuf *tempBufs, UInt32 numFoldersMax,
                                         UInt64 baseOffset, CSzAr *p,
                                         ISzAllocPtr allocTemp) {
  UInt64 dataStartPos;
  UInt32 fo;
  CSubStreamInfo ssi;

  RINOK(SzReadStreamsInfo(p, sd, numFoldersMax, NULL, 0, &dataStartPos, &ssi,
                          allocTemp));
  ...
  for (fo = 0; fo < p->NumFolders; fo++) {
    ...
    RINOK(SzAr_DecodeFolder(p, fo, inStream, dataStartPos, tempBuf->data,
                            tempBuf->size, allocTemp));
  }
...
static MY_NO_INLINE SRes ReadTime(CSzBitUi64s *p, UInt32 num, CSzData *sd2,
                                  const CBuf *tempBufs, UInt32 numTempBufs,
                                  ISzAllocPtr alloc) {
  CSzData sd;
  UInt32 i;
...
static SRes SzReadHeader2(CSzArEx *p, /* allocMain */
                          CSzData *sd, ILookInStream *inStream, CBuf *tempBufs,
...

  {
    ...
    if (type == k7zIdAdditionalStreamsInfo) {
      ...
      SzAr_Init(&tempAr);
      res = SzReadAndDecodePackedStreams(
          inStream, sd, tempBufs, NUM_ADDITIONAL_STREAMS_MAX,
    ...
    if (type == k7zIdMainStreamsInfo) {
      RINOK(SzReadStreamsInfo(&p->db, sd, (UInt32)1 << 30, tempBufs,
                              *numTempBufs, &p->dataPos, &ssi, allocMain));
...

  {
    ...
    for (;;) {
      ...
      if (type >= ((UInt32)1 << 8)) {
        ...
        */
        case k7zIdMTime:
          RINOK(ReadTime(&p->MTime, numFiles, sd, tempBufs, *numTempBufs,
                         allocMain));
          break;
        case k7zIdCTime:
          RINOK(ReadTime(&p->CTime, numFiles, sd, tempBufs, *numTempBufs,
                         allocMain));
          break;
        default: {
          SKIP_DATA(sd, size);
...

    {
        ...
        if (--remSubStreams == 0) {
          ...
          if (numSubStreams == 1 && SzBitWithVals_Check(&p->db.FolderCRCs, i)) {
            ...
            crcMask |= mask;
                     ...
                     (digestsDefs &&
                      SzBitArray_Check(digestsDefs, digestIndex))) {
            p->CRCs.Vals[i] =
                GetUi32(digestsVals + (size_t)digestsValsIndex * 4);
            digestsValsIndex++;
            crcMask |= mask;
          }
      ...
      for (;;) {
        ...
        /*
        {
          UInt64 folderUnpackSize = SzAr_GetFolderUnpackSize(&p->db,
        folderIndex); unpackPos += folderUnpackSize; if (unpackPos <
        folderUnpackSize) return SZ_ERROR_ARCHIVE;
        }
        */
        folderIndex++;
      }
...
static SRes SzReadHeader(CSzArEx *p, CSzData *sd, ILookInStream *inStream,
  ...
  res = SzReadHeader2(p, sd, inStream, tempBufs, &numTempBufs, allocMain,
                      allocTemp);
...
static SRes SzArEx_Open2(CSzArEx *p, ILookInStream *inStream,
                         ISzAllocPtr allocMain, ISzAllocPtr allocTemp) {
  ...
  RINOK(LookInStream_SeekTo(inStream, startArcPos + k7zStartHeaderSize +
                                          nextHeaderOffset));
  ...
  if (res == SZ_OK) {
    ...
    if (CrcCalc(buf.data, nextHeaderSizeT) == nextHeaderCRC) {
      ...
      if (res == SZ_OK && type == k7zIdEncodedHeader) {
        ...
        SzAr_Init(&tempAr);
        res = SzReadAndDecodePackedStreams(inStream, &sd, &tempBuf, 1,
                                           p->startPosAfterHeader, &tempAr,
                                           allocTemp);
        SzAr_Free(&tempAr, allocTemp);
...
SRes SzArEx_Open(CSzArEx *p, ILookInStream *inStream, ISzAllocPtr allocMain,
                 ISzAllocPtr allocTemp) {
...
SRes SzArEx_Extract(const CSzArEx *p, ILookInStream *inStream, UInt32 fileIndex,
                    UInt32 *blockIndex, Byte **tempBuf, size_t *outBufferSize,
  ...
  *outSizeProcessed = 0;
    ...
    *tempBuf = NULL;
    ...
    if (res == SZ_OK) {
      *outBufferSize = unpackSize;
      ...
      if (res == SZ_OK) {
        res = SzAr_DecodeFolder(&p->db, folderIndex, inStream, p->dataPos,
  ...
  if (res == SZ_OK) {
    ...
    *offset =
        (size_t)(unpackPos - p->UnpackPositions[p->FolderToFile[folderIndex]]);
    *outSizeProcessed =
        (size_t)(p->UnpackPositions[(size_t)fileIndex + 1] - unpackPos);
    if (*offset + *outSizeProcessed > *outBufferSize)
      return SZ_ERROR_FAIL;
    if (SzBitWithVals_Check(&p->CRCs, fileIndex))
      if (CrcCalc(*tempBuf + *offset, *outSizeProcessed) !=
          p->CRCs.Vals[fileIndex])
        res = SZ_ERROR_CRC;
  }
...
UInt16 *SzArEx_GetFullNameUtf16_Back(const CSzArEx *p, size_t fileIndex, UInt16
*dest)
{
  BoolInt needSlash;
  if (!p->FileNameOffsets)
...
p->FileNameOffsets[fileIndex]; SzArEx_GetFileNameUtf16(p, fileIndex, dest -
curLen); if (needSlash)
      *(dest - 1) = '/';
    needSlash = True;
    dest -= curLen;

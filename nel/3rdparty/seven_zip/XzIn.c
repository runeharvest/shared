...
/*
#define XZ_FOOTER_SIG_CHECK(p) (memcmp((p), XZ_FOOTER_SIG, XZ_FOOTER_SIG_SIZE)
== 0)
*/
...
SRes Xz_ReadHeader(CXzStreamFlags *p, ISeqInStream *inStream) {
  ...
  RINOK(SeqInStream_Read2(inStream, sig, XZ_STREAM_HEADER_SIZE,
                          SZ_ERROR_NO_ARCHIVE));
...
static SRes Xz_ReadIndex2(CXzStream *p, const Byte *buf, size_t size,
                          ISzAllocPtr alloc) {
  ...
  if (numBlocks != 0) {
    ...
    p->blocks = (CXzBlockSizes *)ISzAlloc_Alloc(alloc, sizeof(CXzBlockSizes) *
                                                           numBlocks);
    if (!p->blocks)
      return SZ_ERROR_MEM;
...
static SRes LookInStream_SeekRead_ForArc(ILookInStream *stream, UInt64 offset,
...
static SRes Xz_ReadBackward(CXzStream *p, ILookInStream *stream,
                            Int64 *startOffset, ISzAllocPtr alloc) {
  UInt64 indexSize;
...
#define TEMP_BUF_SIZE (1 << 10)
    ...
    pos -= XZ_STREAM_FOOTER_SIZE;
    RINOK(
        LookInStream_SeekRead_ForArc(stream, pos, buf, XZ_STREAM_FOOTER_SIZE));
    if (!XZ_FOOTER_SIG_CHECK(buf + 10))
      return SZ_ERROR_NO_ARCHIVE;
  }
...
SRes Xzs_ReadBackward(CXzs *p, ILookInStream *stream, Int64 *startOffset,
  ...
  *startOffset = endOffset;
  for (;;) {
    ...
    if (progress &&
        ICompressProgress_Progress(progress, endOffset - *startOffset,
                                   (UInt64)(Int64)-1) != SZ_OK)

...
#define XzBlock_HasUnsupportedFlags(p)                                         \
  (((p)->flags &                                                               \
    ~(XZ_BF_NUM_FILTERS_MASK | XZ_BF_PACK_SIZE | XZ_BF_UNPACK_SIZE)) != 0)

SRes XzBlock_Parse(CXzBlock *p, const Byte *header);
SRes XzBlock_ReadHeader(CXzBlock *p, ISeqInStream *inStream, BoolInt *isIndex,
...
SRes Xzs_ReadBackward(CXzs *p, ILookInStream *inStream, Int64 *startOffset,
...
typedef struct _IStateCoder {
  ...
  SRes (*SetProps)(void *p, const Byte *props, size_t propSize,
                   ISzAllocPtr alloc);
  void (*Init)(void *p);
  SRes (*Code2)(void *p, Byte *dest, SizeT *destLen, const Byte *src,
...
typedef struct {
  ...
  size_t outBufSize;
  size_t
      outDataWritten; // the size of data in (outBuf) that were fully unpacked
...
/*
  ...
  Interface-2 : Direct output buffer:
    ...
    xz unpacker doesn't allocate additional buffer for lzma2 dictionary in that
  mode.
    {
      XzUnpacker_Init()
  ...
  SZ_OK
    status:
      ...
      CODER_STATUS_NEEDS_MORE_INPUT - maybe there are more xz streams,
                                      call XzUnpacker_IsStreamWasFinished to
check that current stream was finished SZ_ERROR_MEM  - Memory allocation error
  SZ_ERROR_DATA - Data error
  SZ_ERROR_UNSUPPORTED - Unsupported method or method properties
  SZ_ERROR_CRC  - CRC error
  // SZ_ERROR_INPUT_EOF - It needs more bytes in input buffer (src).

  SZ_ERROR_NO_ARCHIVE - the error with xz Stream Header with one of the
following reasons:
    ...
    contains some another data. In that case you can call
XzUnpacker_GetExtraSize() function to get real size of xz stream.
*/
...
XzUnpacker_GetExtraSize() returns then number of uncofirmed bytes,
 if it's in (XZ_STATE_STREAM_HEADER) state or in (XZ_STATE_STREAM_PADDING)
state. These bytes can be some bytes after xz archive, or it can be start of new
xz stream.

Call XzUnpacker_GetExtraSize() after XzUnpacker_Code() function to detect real
size of xz stream in two cases, if XzUnpacker_Code() returns: res == SZ_OK &&
status == CODER_STATUS_NEEDS_MORE_INPUT res == SZ_ERROR_NO_ARCHIVE
*/
...
typedef struct {
  ...
  Byte DataAfterEnd;
  Byte DecodingTruncated; // Decoding was Truncated, we need only partial output
                          // data
...
SRes XzDecMt_Decode(
    CXzDecMtHandle p, const CXzDecMtProps *props,
    const UInt64 *outDataSize, // NULL means undefined
    int finishMode, // 0 - partial unpacking is allowed, 1 - xz stream(s) must
                    // be finished

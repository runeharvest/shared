...
static void Print(const char *s) { fputs(s, DEBUG_OUT_STREAM); }
...
static void PrintAddr(void *p) { PrintHex((UInt64)(size_t)(ptrdiff_t)p, 12); }
...
#ifdef _7ZIP_LARGE_PAGES
  SIZE_T size;
  GetLargePageMinimumP largePageMinimum = (GetLargePageMinimumP)GetProcAddress(
      GetModuleHandle(TEXT("kernel32.dll")), "GetLargePageMinimum");
  if (!largePageMinimum)
    return;
...
void *BigAlloc(size_t size) {
...
#ifdef _7ZIP_LARGE_PAGES
  {
    ...
    if (ps != 0 && ps <= (1 << 30) && size > (ps / 2)) {
      ...
      size2 = (size + ps) & ~ps;
      if (size2 >= size) {
        void *res = VirtualAlloc(NULL, size2, MEM_COMMIT | MEM_LARGE_PAGES,
                                 PAGE_READWRITE);
        if (res)
          return res;
      }
...
/*
#define MY_ALIGN_PTR_DOWN(p, align) ((void *)((char *)(p) -
((size_t)(UIntPtr)(p) & ((align) - 1))))
*/
...
/* we align ptr to support cases where CAlignOffsetAlloc::offset is not multiply
 * of sizeof(void *) */
...
static void *AlignOffsetAlloc_Alloc(ISzAllocPtr pp, size_t size) {
  ...
  pAligned = (char *)MY_ALIGN_PTR_DOWN((char *)adr + alignSize - p->offset +
                                           extra + ADJUST_ALLOC_SIZE,
                                       alignSize) +
             p->offset;
